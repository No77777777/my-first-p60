...
Copyright (c) 2010, 2014 - 2019, 2021, 2025 QUALCOMM Technologies, Inc(QTI).
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //components/rel/uim.mpss/6.1.0/pbm/src/qmi_pbm.c#4 $

when        who    what, where, why
--------    ---    ----------------------------------------------------------
04/06/25    skc    Support Emergency number upto 15 digits
07/29/21    shp    Support serving MCC based ECC number for china
10/29/19   sch     Fixing pbm compilation issues for HA builds
10/03/19   sch     Added support for features UIM capabilities for HA builds
02/14/19    tq     Fix KW error
03/15/18    tq     Add Null check before accessing cl_sp from command buffer
02/19/18    tq     Retry read record ind if it failed at QMI framework
04/21/17    bcho   Make local variable static to save stack
04/19/17    bcho   Moving QCRIL ECC DB to PBM
12/12/16    nr     Associate empty EFADN with valid USIM fields if any
10/07/16    nr     F3 improvements
05/19/16    sp     F3 improvements
30/12/15    dd     Place braces for all code blocks
12/08/15    lm     Remove compiler warnings
11/04/15    stv    Logging improvements
08/26/15    lm     Enable memory leak detection
...
01/23/15    nr     Cross mapping changes
01/19/15    stv    Need not clear the transaction info when client is invalid
...
#define  PBMI_HARDCODED_MCC_EMER_NUM_IND            0x17
#define  PBMI_SERVING_MCC_HARDCODED_EMER_NUM_IND    0x19
...
#define  PBMI_HARDCODED_MCC_EMER_NUM            0x18
#define  PBMI_SERVING_MCC_HARDCODED_EMER_NUM    0x1A
...
#define  PBMI_GET_CONFIGURATION_REQ_TLV     0x10
#define  PBMI_GET_CONFIGURATION_RSP_TLV     0x10
#define  PBMI_GET_CONFIGURATION_ECC_MASK    0x00000001
...
/*--------------------------------------------------------------------------
  Macro to control QMI PBM read record indication retry
----------------------------------------------------------------------------*/
#define QMI_PBM_READ_RECORD_IND_MAX_RETRY_COUNT   10
/* time interval between each retry in case on send indication failiure
   its value is in Msec */
#define QMI_PBM_READ_RECORD_IND_RETRY_INTERVAL    20
...
/* When count of ecc is large then the size of buffer storing all ecc details will be large.
   All ecc may not printed through log.To avoid above issue,
   print only length of 100 from buffer at once */
#define PBM_MAX_BUF_LENGTH_TO_PRINT               100
...
typedef enum
{
  ...
  PBMI_CMD_GET_CONFIGURATION,
  PBMI_CMD_GET_EMERGENCY_LIST_EXT,
  PBMI_CMD_MAX,
...
typedef struct qmi_pbmi_client_state_s
{
  ...
  struct
  {
    ...
    boolean                      reg_ext_emergency_list_events;
  }pbm_info;
  struct
  {
...
static dsm_item_type* qmi_pbmi_get_configuration(void*, void*, void*, dsm_item_type **);
static dsm_item_type* qmi_pbmi_get_emergency_list_ext(void*, void*, void*, dsm_item_type **);
...
static qmi_svc_cmd_hdlr_type  qmi_pbmi_cmd_callbacks[PBMI_CMD_MAX] =
{
  ...
  PBM_HDLR( PBMI_CMD_VAL_GET_CONFIGURATION,
                qmi_pbmi_get_configuration),
  PBM_HDLR( PBMI_CMD_VAL_GET_EMERGENCY_LIST_EXT,
                qmi_pbmi_get_emergency_list_ext)
};
...
static boolean qmi_pbmi_record_read_ind
(
  const qmi_pbm_if_info_item_s *const data_ptr
);
...
static void qmi_pbmi_get_emergency_list_ext_resp
(
  const qmi_pbm_if_info_item_s *const data_ptr
);
...
)
{
  ...
  if(cl_sp)
  {
    ...
    cl_sp->pbm_info.reg_ext_emergency_list_events  = FALSE;
...
)
{
  ...
  ASSERT(msg_hdr);
  ASSERT(sdu_in);
  ASSERT(msg_hdr->common_hdr.client_id >  QMI_SVC_CLID_UNUSED);
...
)
{
  ...
  /* In case of error indication is not needed to be freed as
     it is already freed in ds_qmi_fw_send_ind */

  return ds_qmi_fw_send_ind(&common_hdr, cmd_type, ind);
...
/*===========================================================================
  ...
  RETURN VALUE
    TRUE:  msg send successfully 
    FLASE: msg send failed 
...
boolean qmi_pbm_encode_and_send_msg
(
  qmi_pbm_if_info_item_s *     pbm_if_data_ptr
)
{
  ...
  switch (pbm_if_data_ptr->header.cmd)
  {
    ...
    case QMI_PBM_IF_CMD_EMERGENCY_LIST_IND:
      ...
      break;
    case QMI_PBM_IF_CMD_EXT_EMERGENCY_LIST_IND:
      qmi_pbmi_ext_emergency_list_ind(pbm_if_data_ptr);
      if(pbm_if_data_ptr->data.ext_emerg_list_ind.ext_emer_data != NULL)
      {
        QMI_PBM_IF_MEM_FREE(pbm_if_data_ptr->data.ext_emerg_list_ind.ext_emer_data);
      }
      break;
    ...
    case QMI_PBM_IF_CMD_RECORD_READ_EXT_IND:
      return_val = qmi_pbmi_record_read_ind(pbm_if_data_ptr);
      break;
    ...
    case QMI_PBM_IF_CMD_GAS_UPDATE_IND:
      ...
      break;
    case QMI_PBM_IF_CMD_GET_EMERGENCY_LIST_EXT_RESP:
      qmi_pbmi_get_emergency_list_ext_resp(pbm_if_data_ptr);
      if(pbm_if_data_ptr->data.emerg_list_resp.emer_data != NULL)
      {
        QMI_PBM_IF_MEM_FREE(pbm_if_data_ptr->data.emerg_list_resp.emer_data);
      }
...
)
{
  ...
  boolean                      ext_emer_list_ind_reg_old_mask_value         = FALSE;
  boolean                      ext_emer_list_ind_reg_new_mask_value         = FALSE;
  boolean                      send_ext_emer_list_ind_now                   = FALSE;
  ...
  ext_emer_list_ind_reg_old_mask_value = client_sp->pbm_info.reg_ext_emergency_list_events;
  ...
  if( (reg_mask & QMI_PBM_IF_IND_REG_EXT_EMER_LIST) == QMI_PBM_IF_IND_REG_EXT_EMER_LIST)
  {
    ext_emer_list_ind_reg_new_mask_value = TRUE;
  }
  ...
  client_sp->pbm_info.reg_ext_emergency_list_events = ext_emer_list_ind_reg_new_mask_value;
  ...
  reg_mask |= ext_emer_list_ind_reg_new_mask_value ? QMI_PBM_IF_IND_REG_EXT_EMER_LIST : 0;

  send_pb_ready_ind = (pb_ready_reg_new && !pb_ready_reg_old);
  send_emer_list_ind = (emer_list_reg_new && !emer_list_reg_old);
  send_ext_emer_list_ind_now = (ext_emer_list_ind_reg_new_mask_value && !ext_emer_list_ind_reg_old_mask_value);

  if( send_pb_ready_ind || send_emer_list_ind || send_ext_emer_list_ind_now)
  {
    ...
    pbm_if_data_ptr->data.ind_reg.send_ext_emer_list_ind = send_ext_emer_list_ind_now;
...
)
{
  ...
  if(client_sp->pbm_subs_info.is_bind_subs_recv)
  {
    if(FALSE == qmi_svc_put_param_tlv(&response,
...
)
{
  ...
  const qmi_pbm_if_pb_caps_s               *pb_caps         = NULL;
  const qmi_pbm_if_session_caps_s          *sess_caps       = NULL;
  uint16                                    l               = 0 ;
  uint8                                     i               = 0;
  uint8                                     j               = 0;
  boolean                                   result_added    = TRUE;
  ...
  if ( resp_ptr != NULL )
  {
    ...
    if( QMI_ERR_NONE == errval )
    {
      for(i=0; i<data_ptr->data.get_all_pb_caps_resp.num_of_sessions;i++)
      {
            ...
            qmi_pbmi_match_subs_session(cl_sp->pbm_subs_info.subs_id,sess_caps->session_type))
        {
          ...
          l = (uint16)resp_ptr->capability_basic_info_array_len;
          ...
          for(j=0;j<sess_caps->num_of_pbs;j++)
          {
            ...
            if( (pb_caps->pb_type == (uint16)QMI_PBM_IF_PB_ADN) )
            {
              if( pb_caps->pbset_info_valid )
              {
                ...
                l = (uint16)resp_ptr->pb_set_info_array_len;
              ...
              if( pb_caps->is_aas_cap_valid )
              {
                ...
                l = (uint16)resp_ptr->aas_capability_array_len ;
              ...
              if( pb_caps->is_gas_cap_valid )
              {
                ...
                l = (uint16)resp_ptr->gas_capability_array_len ;
              ...
              if( pb_caps->is_sec_name_supported )
              {
                ...
                l = (uint16)resp_ptr->second_name_capability_array_len ;
              ...
              if( pb_caps->is_email_valid )
              {
                ...
                l = (uint16)resp_ptr->email_capability_array_len;
              ...
              if( pb_caps->is_ad_num_valid )
              {
                ...
                l = (uint16)resp_ptr->ad_num_capability_array_len ;
              ...
              if( pb_caps->is_grp_valid )
              {
                ...
                l = (uint16)resp_ptr->group_capability_array_len ;
              ...
              l = (uint16)resp_ptr->hidden_records_capability_array_len ;
    ...
    if( FALSE == qmi_pbm_idl_message_encode(pbm_get_service_object_v01(),
                                            ...
                                            (void *)resp_ptr,
                                            (uint32_t) sizeof(pbm_get_all_pb_capabilities_resp_msg_v01),
                                            &response))
    {
...
)
{
  ...
  if((read_pbset_caps->start_id == 0 && start_id_found)
    || (read_pbset_caps->end_id == 0 && end_id_found))
    {
  ...
  while (*sdu_in)
  {
    ...
    switch (type)
    {
      ...
      case PBMI_WRITE_REC_EXT_NAME:
        ...
        if (!QMI_PBMI_PULL(sdu_in, rec->name_info.text, (uint16)(rec->name_info.len * 2)))
        {
          UIM_MSG_HIGH_0("Invalid length in TLV");
          errval = QMI_ERR_PB_TEXT_TOO_LONG;
          goto send_result;
      ...
      case PBMI_WRITE_REC_EXT_SNAME:
        ...
        if (!QMI_PBMI_PULL(sdu_in, rec->sname_info.text, (uint16)(rec->sname_info.len * 2)))
        {
          errval = QMI_ERR_MALFORMED_MSG;
          goto send_result;
        }
      ...
      case PBMI_WRITE_REC_EXT_EMAIL:
        ...
        for(i=0;i<rec->email_cnt;i++)
        {
          ...
          if ( !QMI_PBMI_PULL(sdu_in, email->text, (uint16)(email->len * 2)) )
          {
            errval = QMI_ERR_MALFORMED_MSG;
            goto send_result;
          }
  ...
  for( j = 0; j < PBMI_MAX_CLIDS; j++)
  {
    cl_sp = (qmi_pbmi_client_state_type *)pbm_sp->client[j];
          ...
          (cl_sp->common.clid == cl_id) )
      {
        ...
        UIM_MSG_HIGH_2("Record read ext undecoded ind for 0x%x recs is sent with start-id 0x%x",
...
)
{
  ...
  uint16                       len       = 0;
  uint8                        tag       = QMI_TYPE_REQUIRED_PARAMETERS;
  qmi_pbm_if_session_type      sess_type = QMI_PBM_IF_SESSION_MAX;
  ...
  for( j = 0; j < PBMI_MAX_CLIDS; j++)
  {
       ...
       cl_sp->pbm_info.reg_pb_ready_events)
    {
      ...
      for( sess_type = QMI_PBM_IF_SESSION_GW_PRIMARY;sess_type < QMI_PBM_IF_SESSION_MAX; sess_type++)
      {
        ...
        if(data_ptr->data.sim_ready_ind.slot_id == (uint32)QMI_MMGSDI_SLOT_TO_QMI_SLOT(mmgsdi_slot_id))
        {
          if(!cl_sp->pbm_subs_info.is_bind_subs_recv ||
          qmi_pbmi_match_subs_session(cl_sp->pbm_subs_info.subs_id,sess_type))
          {
...
)
{
  ...
  UIM_MSG_HIGH_2("qmi_pbm_binding_to_card_session: is_match 0x%x subs id 0x%x", is_match, subs_id);
...
/*===========================================================================
  FUNCTION QMI_PBM_POPULATE_MCC_BASED_ECC()

  DESCRIPTION
    populates MCC based emergency number into indication/response
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static qmi_error_e_type qmi_pbm_populate_mcc_based_ecc(
  dsm_item_type                     **dsm_item_pptr,
  uint16                              tlv_id,
  const qmi_pbm_if_mcc_ecc_data_type *mcc_ecc_data_ptr
...
)
{
  ...
  if(dsm_item_pptr    == NULL ||
     mcc_ecc_data_ptr == NULL)
  {
    return QMI_ERR_MALFORMED_MSG;
  }

  for(slot_index = PBM_SLOT_3_INDEX; slot_index >= 0; slot_index--)
  {
    const qmi_pbm_if_mcc_ecc_data_type *rec_mcc_ptr = NULL;

    rec_mcc_ptr = &mcc_ecc_data_ptr[slot_index];

    UIM_MSG_HIGH_3("Count of MCC based ECC numbers for slot index 0x%x : 0x%x, tlv_id : 0x%x",
                   slot_index,
                   rec_mcc_ptr->num_hcoded_ecc_entries,
                   tlv_id);

    if((rec_mcc_ptr->num_hcoded_ecc_entries > 0))
    {
      char ecc_buf_print[QMI_PBM_IF_MAX_HCODED_MCC_BASED_ECC * (QMI_PBM_IF_MAX_ECC_LEN + 1)];

      memset(ecc_buf_print, 0, sizeof(ecc_buf_print));

      for(ecc_index = (rec_mcc_ptr->num_hcoded_ecc_entries - 1); ecc_index >= 0; ecc_index--)
      {
        uint8 ecc_size = rec_mcc_ptr->hcoded_ecc[ecc_index].ecc_len +
                           sizeof(rec_mcc_ptr->hcoded_ecc[ecc_index].ecc_len);

        tot_len += ecc_size;

        if(!(QMI_SVC_PKT_PUSH(dsm_item_pptr,
                              (void*)rec_mcc_ptr->hcoded_ecc[ecc_index].ecc,
                              rec_mcc_ptr->hcoded_ecc[ecc_index].ecc_len) &&
             QMI_SVC_PKT_PUSH(dsm_item_pptr,
                              (void*)&rec_mcc_ptr->hcoded_ecc[ecc_index].ecc_len,
                              sizeof(rec_mcc_ptr->hcoded_ecc[ecc_index].ecc_len))))
        {
          dsm_free_packet(dsm_item_pptr);
          return QMI_ERR_NO_MEMORY;
        }
        qmi_pbmi_add_ecc(rec_mcc_ptr->hcoded_ecc[ecc_index].ecc,
                         rec_mcc_ptr->hcoded_ecc[ecc_index].ecc_len,
                         ecc_buf_print,
                         sizeof(ecc_buf_print) );
      }

      UIM_PBM_MSG_SPRINTF_2(" MCC ecc for slot 0x%x : %s",
                            slot_index, ecc_buf_print);
      tot_len += sizeof(rec_mcc_ptr->num_hcoded_ecc_entries);

      if(!( QMI_SVC_PKT_PUSH(dsm_item_pptr,
                             (void*)&rec_mcc_ptr->num_hcoded_ecc_entries,
                             sizeof(rec_mcc_ptr->num_hcoded_ecc_entries))) )
      {
        dsm_free_packet(dsm_item_pptr);
        return QMI_ERR_NO_MEMORY;
      }
  ...
  if(num_instances > 0)
  {
    if(!qmi_pbmi_fill_array_tlv(dsm_item_pptr, tlv_id, tot_len, num_instances))
    {
      dsm_free_packet(dsm_item_pptr);
      return QMI_ERR_NO_MEMORY;
    }
  ...
  return QMI_ERR_NONE;
}/* qmi_pbm_populate_mcc_based_ecc */


/*===========================================================================
  FUNCTION QMI_PBM_POPULATE_MCC_BASED_EXT_ECC()

  DESCRIPTION
    populates extended MCC based emergency number into indication/response
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static qmi_error_e_type qmi_pbm_populate_mcc_based_ext_ecc(
  dsm_item_type                         **dsm_item_pptr,
  uint16                                  tlv_id,
  const qmi_pbm_if_mcc_ext_ecc_data_type *mcc_ecc_data_ptr
...
)
{
  ...
  for (slot_index = PBM_SLOT_3_INDEX; slot_index >= 0; slot_index--)
  {
    const qmi_pbm_if_mcc_ext_ecc_data_type *rec_mcc_ptr = NULL;
    ...
    if ((rec_mcc_ptr->num_hcoded_ecc_entries > 0))
    {
      char ecc_buf_print[QMI_PBM_IF_MAX_HCODED_MCC_BASED_ECC * (QMI_PBM_IF_ECC_LEN15 + 1)];
  ...
  return QMI_ERR_NONE;
}/* qmi_pbm_populate_mcc_based_ext_ecc */
...
)
{
  ...
  uint8                                      num_instances            = 0;
  uint8                                      num_of_card_sessions     = 0;
  uint8                                      num_of_nv_entries        = 0;
  uint8                                      num_of_hcoded_entries    = 0;
  const qmi_pbm_if_ecc_session_ota_data     *rec_ota                  = NULL;
  const qmi_pbm_if_ecc_session_card_data    *rec_card                 = NULL;
  const qmi_pbm_if_cat_ecc_data_type        *rec_nv                   = NULL;
  const qmi_pbm_if_cat_ecc_data_type        *rec_hcoded               = NULL;
  uint16                                     tot_len                  = 0;
  ...
  for( j = 0; j < PBMI_MAX_CLIDS; j++)
  {
    ...
    {
      ind = NULL;
      num_of_ota_sessions = (uint8)data_ptr->data.emerg_list_ind.emer_data->num_of_ota_sessions;
      num_of_card_sessions = (uint8)data_ptr->data.emerg_list_ind.emer_data->num_of_card_sessions;
      num_of_nv_entries = (uint8)data_ptr->data.emerg_list_ind.emer_data->num_nv_ecc_entries;
      num_of_hcoded_entries = (uint8)data_ptr->data.emerg_list_ind.emer_data->num_hcoded_ecc_entries;
      ...
      if(num_of_hcoded_entries != 0)
      {
        ...
        for(i = (num_of_hcoded_entries-1);i >=0;i--)
        {
          ...
          {
            tot_len += (rec_hcoded->ecc_len + sizeof(rec_hcoded->ecc_len));
      ...
      /* Fill HARD CODED MCC Emergency info if present */
      if(QMI_ERR_NONE != qmi_pbm_populate_mcc_based_ecc(
                           &ind,
                           PBMI_HARDCODED_MCC_EMER_NUM_IND,
                           data_ptr->data.emerg_list_ind.emer_data->hcoded_mcc_ecc))
      {
        break;
      }

      /* Fill SERVING MCC HARD CODED Emergency info if present */
      if(QMI_ERR_NONE != qmi_pbm_populate_mcc_based_ecc(
                           &ind,
                           PBMI_SERVING_MCC_HARDCODED_EMER_NUM_IND,
                           data_ptr->data.emerg_list_ind.emer_data->hcoded_serving_mcc_ecc))
      {
        break;
      }
...
/*===========================================================================
  FUNCTION QMI_PBMI_EXT_EMERGENCY_LIST_IND()

  DESCRIPTION
    Send extended Emergency List Indication
  ...
  SIDE EFFECTS
    None
===========================================================================*/
void qmi_pbmi_ext_emergency_list_ind(
  const qmi_pbm_if_info_item_s *const data_ptr
)
{
  qmi_pbmi_state_type                           *pbm_sp                   = NULL;
  ...
  uint8                                          num_of_nv_entries        = 0;
  uint8                                          num_of_hcoded_entries    = 0;
  const qmi_pbm_if_ext_ecc_session_ota_data     *rec_ota                  = NULL;
  const qmi_pbm_if_ext_ecc_session_card_data    *rec_card                 = NULL;
  const qmi_pbm_if_cat_ext_ecc_data_type        *rec_nv                   = NULL;
  const qmi_pbm_if_cat_ext_ecc_data_type        *rec_hcoded               = NULL;
  uint16                                         tot_len                  = 0;
  
  if ( data_ptr->data.ext_emerg_list_ind.ext_emer_data == NULL )
  {
    UIM_MSG_HIGH_0("NULL emergency data in the indication");
    return;
  }
  ...
  for ( j = 0; j < PBMI_MAX_CLIDS; j++)
  {
    ...
    if (cl_sp != NULL &&
       (cl_sp->common.clid != QMI_SVC_CLID_UNUSED) &&
       cl_sp->pbm_info.reg_ext_emergency_list_events )
    {
      ind = NULL;
      num_of_ota_sessions = (uint8)data_ptr->data.ext_emerg_list_ind.ext_emer_data->num_of_ota_sessions;
      num_of_card_sessions = (uint8)data_ptr->data.ext_emerg_list_ind.ext_emer_data->num_of_card_sessions;
      num_of_nv_entries = (uint8)data_ptr->data.ext_emerg_list_ind.ext_emer_data->num_nv_ecc_entries;
      num_of_hcoded_entries = (uint8)data_ptr->data.ext_emerg_list_ind.ext_emer_data->num_hcoded_ecc_entries;

      UIM_MSG_HIGH_4("qmi_pbmi_ext_emergency_list_ind num of ota_sessions = 0x%x card_sessions = 0x%x nv_entries = 0x%x hcoded_entries = 0x%x  num of elenl_sessions = 0x%x",
                      num_of_ota_sessions,
                      num_of_card_sessions,
                      num_of_nv_entries,
                      num_of_hcoded_entries);
      ...
      for (i = (num_of_ota_sessions-1);i >=0;i--)
      {
        rec_ota = &data_ptr->data.ext_emerg_list_ind.ext_emer_data->ota_ecc[i];
        if ((!cl_sp->pbm_subs_info.is_bind_subs_recv ||
            qmi_pbmi_match_subs_session(cl_sp->pbm_subs_info.subs_id,rec_ota->sess_type)) &&
           (rec_ota->ecc_count > 0))
        {
          char ecc_buf_print[CM_MAX_EMERGENCY_NUM_COUNT * (QMI_PBM_IF_ECC_LEN15 + 1)];
          memset(ecc_buf_print, 0, sizeof(ecc_buf_print));
      ...
      for (i = (num_of_card_sessions-1);i >=0;i--)
      {
        rec_card = &data_ptr->data.ext_emerg_list_ind.ext_emer_data->card_ecc[i];
        if ((!cl_sp->pbm_subs_info.is_bind_subs_recv ||
               qmi_pbmi_match_subs_session(cl_sp->pbm_subs_info.subs_id,rec_card->sess_type) ||
               qmi_pbm_binding_to_card_session (cl_sp->pbm_subs_info.subs_id)) &&
               (rec_card->ecc_count > 0))
        {
          char *ecc_buf_print = QMI_PBM_IF_MEM_ALLOC(rec_card->ecc_count*(QMI_PBM_IF_ECC_LEN15 + 1));
          if (ecc_buf_print != NULL)
          {
            memset(ecc_buf_print, 0, (rec_card->ecc_count*(QMI_PBM_IF_ECC_LEN15 + 1)));
          }
          ...
          for (k =(rec_card->ecc_count-1);k >= 0 ; k--)
          {
            ...
            if (ecc_buf_print != NULL)
            {
              qmi_pbmi_add_ecc(rec_card->ecc_info[k].ecc,rec_card->ecc_info[k].ecc_len, ecc_buf_print ,  rec_card->ecc_count*(QMI_PBM_IF_ECC_LEN15 + 1));
            }
          }
          if (ecc_buf_print != NULL)
          {
            uint16    buf_length                         = rec_card->ecc_count * (QMI_PBM_IF_ECC_LEN15 + 1);
            char     *ecc_buf_print_temp                 = ecc_buf_print;
            char     *ecc_buf_to_print                   = QMI_PBM_IF_MEM_ALLOC(PBM_MAX_BUF_LENGTH_TO_PRINT + 1);

            if (ecc_buf_to_print != NULL)
            {
              memset(ecc_buf_to_print, 0, (PBM_MAX_BUF_LENGTH_TO_PRINT + 1));

              for (; buf_length > PBM_MAX_BUF_LENGTH_TO_PRINT ; buf_length = buf_length - PBM_MAX_BUF_LENGTH_TO_PRINT)
              {
                memscpy(ecc_buf_to_print, PBM_MAX_BUF_LENGTH_TO_PRINT, ecc_buf_print_temp, PBM_MAX_BUF_LENGTH_TO_PRINT);
                ecc_buf_to_print[PBM_MAX_BUF_LENGTH_TO_PRINT] = '\0';
                /*Length of ECC can be less than QMI_PBM_IF_ECC_LEN15 (15).In such scenario,there will be NO data
                  in "ecc_buf_print" at its end bytes.If initial byte of  ecc_buf_to_print
                  does NOT have valid data then do NOT print it */
                if (ecc_buf_to_print[0] != 0 )
                {
                  UIM_PBM_MSG_SPRINTF_2(" Card ecc for session type %d: %s",rec_card->sess_type, ecc_buf_to_print);
                }
                ecc_buf_print_temp = ecc_buf_print_temp + PBM_MAX_BUF_LENGTH_TO_PRINT;
              }
              if (buf_length > 0)
              {
                memscpy(ecc_buf_to_print, buf_length, ecc_buf_print_temp, buf_length);
                ecc_buf_to_print[buf_length] = '\0';
                if (ecc_buf_to_print[0] != 0 )
                {
                  UIM_PBM_MSG_SPRINTF_2(" Card ecc for session type %d: %s",rec_card->sess_type, ecc_buf_to_print);
                }
              ...
              QMI_PBM_IF_MEM_FREE(ecc_buf_to_print);
            }
            QMI_PBM_IF_MEM_FREE(ecc_buf_print);
          }
          tot_len += sizeof(rec_card->ecc_count) + sizeof(rec_card->sess_type);
      ...
      if (num_of_nv_entries != 0)
      {
        char ecc_buf_print[PBM_NV_EMERGENCY_NUMBERS * (QMI_PBM_IF_ECC_LEN15 + 1)];
        memset(ecc_buf_print, 0, sizeof(ecc_buf_print));
        for (i = (num_of_nv_entries-1);i >=0;i--)
        {
          rec_nv = &data_ptr->data.ext_emerg_list_ind.ext_emer_data->nv_ecc[i];
           /* Avoid populating more than 6 digit number in deprecated TLV */
          if (rec_nv->ecc_len > 0 && rec_nv->ecc_len  <= QMI_PBM_IF_MAX_ECC_LEN)
          {
            uint8 ecc_siz = rec_nv->ecc_len + sizeof(rec_nv->ecc_len);
            tot_len += ecc_siz;
            if (!(QMI_SVC_PKT_PUSH(&ind, (void*)rec_nv->ecc, rec_nv->ecc_len) &&
      ...
      if (num_of_hcoded_entries != 0)
      {
        ...
        for (i = (num_of_hcoded_entries-1);i >=0;i--)
        {
          rec_hcoded = &data_ptr->data.ext_emerg_list_ind.ext_emer_data->hcoded_ecc[i];
          /* Avoid populating more than 6 digit number in deprecated TLV */
          if (rec_hcoded->ecc_len > 0 && rec_hcoded->ecc_len <= QMI_PBM_IF_MAX_ECC_LEN)
          {
            tot_len += (rec_hcoded->ecc_len + sizeof(rec_hcoded->ecc_len));
            if (!(QMI_SVC_PKT_PUSH(&ind, (void*)rec_hcoded->ecc, rec_hcoded->ecc_len) &&
               QMI_SVC_PKT_PUSH(&ind, (void*)&rec_hcoded->ecc_len, sizeof(rec_hcoded->ecc_len))))
      ...
      if (num_of_hcoded_entries != 0)
      {
        char ecc_buf_print[QMI_PBM_IF_MAX_HCODED_ECC_EXT * (QMI_PBM_IF_ECC_LEN15 + 1)];
        memset(ecc_buf_print, 0, sizeof(ecc_buf_print));
        for (i = (num_of_hcoded_entries-1);i >=0;i--)
        {
          rec_hcoded = &data_ptr->data.ext_emerg_list_ind.ext_emer_data->hcoded_ecc[i];
      ...
      /* Fill HARD CODED MCC Emergency info if present */
      if (QMI_ERR_NONE != qmi_pbm_populate_mcc_based_ext_ecc(
                           &ind,
                           PBMI_HARDCODED_MCC_EMER_NUM_IND,
                           data_ptr->data.ext_emerg_list_ind.ext_emer_data->hcoded_mcc_ecc))
      {
        break;
      }
      ...
      if (QMI_ERR_NONE != qmi_pbm_populate_mcc_based_ext_ecc(
                           &ind,
                           PBMI_SERVING_MCC_HARDCODED_EMER_NUM_IND,
                           data_ptr->data.ext_emerg_list_ind.ext_emer_data->hcoded_serving_mcc_ecc))
      {
        break;
      }
#ifdef FEATURE_CUST_2
      /* Fill HARD CODED MCC Emergency info in the order of sub  if present */
      if (QMI_ERR_NONE != qmi_pbm_populate_mcc_based_ecc_sub_wise_ext(
                           &ind,
                           PBMI_HARDCODED_MCC_EMER_NUM_IND_SUB_WISE,
                           data_ptr->data.ext_emerg_list_ind.ext_emer_data->hcoded_mcc_ecc))
      {
        break;
      }
      
      /* Fill SERVING MCC HARD CODED Emergency info in the order of sub
         if present */
      if (QMI_ERR_NONE != qmi_pbm_populate_mcc_based_ecc_sub_wise_ext(
                          &ind,
                          PBMI_SERVING_MCC_HARDCODED_EMER_NUM_IND_SUB_WISE,
                          data_ptr->data.ext_emerg_list_ind.ext_emer_data->hcoded_serving_mcc_ecc))
      {
        break;
      }
      ...
      (void)qmi_pbmi_send_indication(cl_sp,
                                     PBMI_CMD_VAL_EXT_EMERGENCY_LIST_IND,
                                     ind );
    }
  }
} /* qmi_pbmi_ext_emergency_list_ind() */
...
/*===========================================================================
  FUNCTION QMI_PBMI_GET_EMERGENCY_LIST_EXT()
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static dsm_item_type * qmi_pbmi_get_emergency_list_ext
(
  void *            sp,
  void *            cmd_buf_p,
  void *            cl_sp,
...
)
{
  ...
  UIM_MSG_HIGH_0("qmi_pbmi_get_emergency_list_ext");

#ifdef REMOVE_QMI_PBM_GET_EMERGENCY_LIST_EXT_V01
  UIM_MSG_ERR_0("Unrecognized command");
  (void) sp;
  (void) cmd_buf_p;
  (void) cl_sp;
  ...
  pbm_if_data_ptr->header.cmd = QMI_PBM_IF_CMD_GET_EMERGENCY_LIST_EXT;
  pbm_if_data_ptr->header.handle = (uint32)cmd_buf_p;

  errval = qmi_pbm_create_and_send_cmd(QMI_PBM_CMD_PBM_IF_CB,pbm_if_data_ptr);
  return QMI_SVC_RESPONSE_PENDING;
#endif /* REMOVE_QMI_PBM_GET_EMERGENCY_LIST_EXT_V01 */
...
)
{
  qmi_cmd_buf_type                        *cmd_buf_p                   = NULL;
  dsm_item_type                           *response                    = NULL;
  qmi_error_e_type                         errval                      = QMI_ERR_NO_MEMORY;
  uint8                                    num_of_ota_sessions         = 0;
  uint8                                    num_instances               = 0;
  uint8                                    num_of_card_sessions        = 0;
  uint8                                    num_of_nv_entries           = 0;
...
-------------------------------------------------------------------------*/
  ...
  if( QMI_ERR_NONE == errval )
  {
    ...
    num_of_ota_sessions = (uint8)data_ptr->data.emerg_list_resp.emer_data->num_of_ota_sessions;
    num_of_card_sessions = (uint8)data_ptr->data.emerg_list_resp.emer_data->num_of_card_sessions;
    num_of_nv_entries = (uint8)data_ptr->data.emerg_list_resp.emer_data->num_nv_ecc_entries;
    num_of_hcoded_entries = (uint8)data_ptr->data.emerg_list_resp.emer_data->num_hcoded_ecc_entries;
...
/* Fill HARD CODED Emergency with category  info if present */
    ...
    for(i = (num_of_ota_sessions-1);i >=0;i--)
    {
         ...
         (rec_ota->ecc_count > 0))
      {
        for((k=rec_ota->ecc_count-1);k >= 0 ; k--)
        {
          tot_len += (rec_ota->ecc_info[k].ecc_len + sizeof(rec_ota->ecc_info[k].ecc_len) + sizeof(rec_ota->ecc_info[k].category));
    ...
    for(i = (num_of_card_sessions-1);i >=0;i--)
    {
      ...
      if(rec_card->ecc_count > 0)
      {
        for(k=(rec_card->ecc_count-1);k >= 0 ; k--)
        {
          tot_len += (rec_card->ecc_info[k].ecc_len + sizeof(rec_card->ecc_info[k].ecc_len) + sizeof(rec_card->ecc_info[k].category));
    ...
    else
    {
      for(i = (num_of_nv_entries-1);i >=0;i--)
      {
        ...
        if(rec_nv->ecc_len > 0)
        {
          tot_len += (rec_nv->ecc_len + sizeof(rec_nv->ecc_len));
  ...
  /* Fill NV Emergency info with category  if present */
    ...
    if(QMI_ERR_NONE != qmi_pbm_populate_mcc_based_ecc(
                         &response,
                         PBMI_HARDCODED_MCC_EMER_NUM,
                         data_ptr->data.emerg_list_resp.emer_data->hcoded_mcc_ecc))
    {
      errval = QMI_ERR_NO_MEMORY;
      goto send_result;
    }
    ...
    if(QMI_ERR_NONE != qmi_pbm_populate_mcc_based_ecc(
                         &response,
                         PBMI_SERVING_MCC_HARDCODED_EMER_NUM,
                         data_ptr->data.emerg_list_resp.emer_data->hcoded_serving_mcc_ecc))
    {
      errval = QMI_ERR_NO_MEMORY;
      goto send_result;
    }
...
/*===========================================================================
  FUNCTION QMI_PBMI_GET_EMERGENCY_LIST_EXT_RESP()

  DESCRIPTION
    Return all the applicable extended emergency numbers
...
{
  qmi_cmd_buf_type                            *cmd_buf_ptr                 = NULL;
  ...
  uint8                                        num_of_nv_entries           = 0;
  uint8                                        num_of_hcoded_entries       = 0;
  const qmi_pbm_if_ext_ecc_session_ota_data   *rec_ota_ptr                 = NULL;
  const qmi_pbm_if_ext_ecc_session_card_data  *rec_card_ptr                = NULL;
  const qmi_pbm_if_cat_ext_ecc_data_type      *rec_nv_ptr                  = NULL;
  const qmi_pbm_if_cat_ext_ecc_data_type      *rec_hcoded_ptr              = NULL;
  uint16                                       tot_len                     = 0;
  int                                          iter_session                = 0;
  int                                          iter_ecc_entries            = 0;
  int                                          iter_ecc_count              = 0;
  qmi_pbmi_client_state_type                  *cl_sp                       = NULL;

  cmd_buf_ptr = (qmi_cmd_buf_type *) data_ptr->header.handle;

  if (cmd_buf_ptr == NULL)
  {
    UIM_MSG_HIGH_0("Null cmd_buf ptr in qmi_pbmi_get_emergency_list_ext_resp");
    return;
  }

  cl_sp = qmi_pbmi_get_client_sp_by_cmd_buf(cmd_buf_ptr);

  if (cl_sp == NULL)
  {
    UIM_MSG_HIGH_0("Null client state ptr in qmi_pbmi_get_emergency_list_ext_resp");
    return;
  }
...
/*-------------------------------------------------------------------------
  Build the response from back to front
-------------------------------------------------------------------------*/
  errval = data_ptr->data.emerg_ext_list_resp.error;

  response_ptr = NULL;

  if (QMI_ERR_NONE == errval)
  {
    if(data_ptr->data.emerg_ext_list_resp.ext_emer_data == NULL)
    {
      UIM_MSG_HIGH_0("Null emergency data in qmi_pbmi_get_emergency_list_ext_resp");
      return;
    }
    num_of_ota_sessions = (uint8)data_ptr->data.emerg_ext_list_resp.ext_emer_data->num_of_ota_sessions;
    num_of_card_sessions = (uint8)data_ptr->data.emerg_ext_list_resp.ext_emer_data->num_of_card_sessions;
    num_of_nv_entries = (uint8)data_ptr->data.emerg_ext_list_resp.ext_emer_data->num_nv_ecc_entries;
    num_of_hcoded_entries = (uint8)data_ptr->data.emerg_ext_list_resp.ext_emer_data->num_hcoded_ecc_entries;
    ...
    if (num_of_hcoded_entries == 0)
    {
      UIM_MSG_HIGH_0("Hardcoded emergency numbers not present in qmi_pbmi_get_emergency_list_ext_resp");
    }
    else
    {
      for (iter_ecc_entries = (num_of_hcoded_entries-1); iter_ecc_entries >= 0; iter_ecc_entries--)
      {
        rec_hcoded_ptr = &data_ptr->data.emerg_ext_list_resp.ext_emer_data->hcoded_ecc[iter_ecc_entries];
        if (rec_hcoded_ptr->ecc_len > 0)
        {
          tot_len += (rec_hcoded_ptr->ecc_len + sizeof(rec_hcoded_ptr->ecc_len));
          if (!(QMI_SVC_PKT_PUSH(&response_ptr, (void*)rec_hcoded_ptr->ecc, rec_hcoded_ptr->ecc_len) &&
                QMI_SVC_PKT_PUSH(&response_ptr, (void*)&rec_hcoded_ptr->ecc_len, sizeof(rec_hcoded_ptr->ecc_len))))
          {
            errval = QMI_ERR_NO_MEMORY;
      ...
      if (num_instances > 0)
      {
        if (!qmi_pbmi_fill_array_tlv(&response_ptr, PBMI_GET_EMER_HCODED_EXT, tot_len, num_instances))
        {
          errval = QMI_ERR_NO_MEMORY;
          dsm_free_packet(&response_ptr);
        }
...
/* Fill HARD CODED Emergency with category  info if present */
    ...
    else
    {
      for (iter_ecc_entries = (num_of_hcoded_entries-1);iter_ecc_entries >=0;iter_ecc_entries--)
      {
        ...
        if (rec_hcoded_ptr->ecc_len > 0)
        {
          uint8 ecc_siz =  sizeof(rec_hcoded_ptr->category);
          tot_len += ecc_siz;
          if ( !(QMI_SVC_PKT_PUSH(&response_ptr, (void*)&rec_hcoded_ptr->category, sizeof(rec_hcoded_ptr->category)) ))
          {
            errval = QMI_ERR_NO_MEMORY;
            dsm_free_packet(&response_ptr);
            goto send_result;
      ...
      if (num_instances > 0)
      {
        if (!qmi_pbmi_fill_array_tlv(&response_ptr, PBMI_GET_EMER_HCODED_EXT_CAT, tot_len, num_instances))
        {
          errval = QMI_ERR_NO_MEMORY;
          dsm_free_packet(&response_ptr);
        }
    ...
    if (num_of_ota_sessions == 0)
    {
      UIM_MSG_HIGH_0("OTA emergency numbers not present in qmi_pbmi_get_emergency_list_ext_resp");
    }

    /* Fill OTA Emergency info if present */
    for (iter_session = (num_of_ota_sessions-1);iter_session >=0;iter_session--)
    {
      rec_ota_ptr = &data_ptr->data.emerg_ext_list_resp.ext_emer_data->ota_ecc[iter_session];
      if ((!cl_sp->pbm_subs_info.is_bind_subs_recv ||
         qmi_pbmi_match_subs_session(cl_sp->pbm_subs_info.subs_id,rec_ota_ptr->sess_type)) &&
         (rec_ota_ptr->ecc_count > 0))
      {
        for ((iter_ecc_count=rec_ota_ptr->ecc_count-1);iter_ecc_count >= 0 ; iter_ecc_count--)
        {
          tot_len += (rec_ota_ptr->ecc_info[iter_ecc_count].ecc_len + sizeof(rec_ota_ptr->ecc_info[iter_ecc_count].ecc_len) + sizeof(rec_ota_ptr->ecc_info[iter_ecc_count].category));
          if (!(QMI_SVC_PKT_PUSH(&response_ptr, (void*)rec_ota_ptr->ecc_info[iter_ecc_count].ecc, rec_ota_ptr->ecc_info[iter_ecc_count].ecc_len) &&
             QMI_SVC_PKT_PUSH(&response_ptr, (void*)&rec_ota_ptr->ecc_info[iter_ecc_count].ecc_len, sizeof(rec_ota_ptr->ecc_info[iter_ecc_count].ecc_len)) &&
             QMI_SVC_PKT_PUSH(&response_ptr, (void*)&rec_ota_ptr->ecc_info[iter_ecc_count].category, sizeof(rec_ota_ptr->ecc_info[iter_ecc_count].category))))
          {
            errval = QMI_ERR_NO_MEMORY;
            dsm_free_packet(&response_ptr);
            goto send_result;
        ...
        tot_len += sizeof(rec_ota_ptr->ecc_count) + sizeof(rec_ota_ptr->sess_type);
        if (!( QMI_SVC_PKT_PUSH(&response_ptr, (void*)&rec_ota_ptr->ecc_count, sizeof(rec_ota_ptr->ecc_count)) &&
           QMI_SVC_PKT_PUSH(&response_ptr, (void*)&rec_ota_ptr->sess_type, sizeof(rec_ota_ptr->sess_type))) )
        {
          errval = QMI_ERR_NO_MEMORY;
          dsm_free_packet(&response_ptr);
          goto send_result;
    ...
    if (num_instances > 0)
    {
      if (!qmi_pbmi_fill_array_tlv(&response_ptr, PBMI_GET_EMER_OTA, tot_len, num_instances))
      {
        errval = QMI_ERR_NO_MEMORY;
        dsm_free_packet(&response_ptr);
        goto send_result;
    ...
    if (num_of_card_sessions == 0)
    {
      UIM_MSG_HIGH_0("Card emergency numbers not present in qmi_pbmi_get_emergency_list_ext_resp");
    }

    for (iter_session = (num_of_card_sessions-1);iter_session >=0;iter_session--)
    {
      rec_card_ptr = &data_ptr->data.emerg_ext_list_resp.ext_emer_data->card_ecc[iter_session];
      if (rec_card_ptr->ecc_count > 0)
      {
        for (iter_ecc_count=(rec_card_ptr->ecc_count-1);iter_ecc_count >= 0 ; iter_ecc_count--)
        {
          tot_len += (rec_card_ptr->ecc_info[iter_ecc_count].ecc_len + sizeof(rec_card_ptr->ecc_info[iter_ecc_count].ecc_len) + sizeof(rec_card_ptr->ecc_info[iter_ecc_count].category));
          if (!(QMI_SVC_PKT_PUSH(&response_ptr, (void*)rec_card_ptr->ecc_info[iter_ecc_count].ecc, rec_card_ptr->ecc_info[iter_ecc_count].ecc_len) &&
             QMI_SVC_PKT_PUSH(&response_ptr, (void*)&rec_card_ptr->ecc_info[iter_ecc_count].ecc_len, sizeof(rec_card_ptr->ecc_info[iter_ecc_count].ecc_len)) &&
             QMI_SVC_PKT_PUSH(&response_ptr, (void*)&rec_card_ptr->ecc_info[iter_ecc_count].category, sizeof(rec_card_ptr->ecc_info[iter_ecc_count].category))))
          {
            errval = QMI_ERR_NO_MEMORY;
            dsm_free_packet(&response_ptr);
            goto send_result;
        ...
        tot_len += sizeof(rec_card_ptr->ecc_count) + sizeof(rec_card_ptr->sess_type);
        if (!(QMI_SVC_PKT_PUSH(&response_ptr, (void*)&rec_card_ptr->ecc_count, sizeof(rec_card_ptr->ecc_count)) &&
           QMI_SVC_PKT_PUSH(&response_ptr, (void*)&rec_card_ptr->sess_type, sizeof(rec_card_ptr->sess_type))) )
        {
          errval = QMI_ERR_NO_MEMORY;
          dsm_free_packet(&response_ptr);
          goto send_result;
    ...
    if (num_instances > 0)
    {
      if (!qmi_pbmi_fill_array_tlv(&response_ptr, PBMI_GET_EMER_CARD, tot_len, num_instances))
      {
        errval = QMI_ERR_NO_MEMORY;
        dsm_free_packet(&response_ptr);
        goto send_result;
    ...
    if (num_of_nv_entries == 0)
    {
      UIM_MSG_HIGH_0("NV emergency numbers not present in qmi_pbmi_get_emergency_list_ext_resp");
    }
    else
    {
      for (iter_ecc_entries = (num_of_nv_entries-1);iter_ecc_entries >=0;iter_ecc_entries--)
      {
        rec_nv_ptr = &data_ptr->data.emerg_ext_list_resp.ext_emer_data->nv_ecc[iter_ecc_entries];
        /* Avoid populating more than 6 digit number in deprecated TLV */
        if (rec_nv_ptr->ecc_len > 0 && rec_nv_ptr->ecc_len <= QMI_PBM_IF_MAX_ECC_LEN)
        {
          tot_len += (rec_nv_ptr->ecc_len + sizeof(rec_nv_ptr->ecc_len));
          if (!(QMI_SVC_PKT_PUSH(&response_ptr, (void*)rec_nv_ptr->ecc, rec_nv_ptr->ecc_len) &&
             QMI_SVC_PKT_PUSH(&response_ptr, (void*)&rec_nv_ptr->ecc_len, sizeof(rec_nv_ptr->ecc_len))))
          {
            errval = QMI_ERR_NO_MEMORY;
            dsm_free_packet(&response_ptr);
            goto send_result;
      ...
      if (num_instances > 0)
      {
        if (!qmi_pbmi_fill_array_tlv(&response_ptr, PBMI_GET_EMER_NV, tot_len, num_instances))
        {
          errval = QMI_ERR_NO_MEMORY;
          dsm_free_packet(&response_ptr);
          goto send_result;
    ...
    else
    {
      ...
      for (iter_ecc_entries = (num_of_hcoded_entries-1);iter_ecc_entries >=0;iter_ecc_entries--)
      {
        rec_hcoded_ptr = &data_ptr->data.emerg_ext_list_resp.ext_emer_data->hcoded_ecc[iter_ecc_entries];
        /* Avoid populating more than 6 digit number in deprecated TLV */
        if (rec_hcoded_ptr->ecc_len > 0 && rec_hcoded_ptr->ecc_len <= QMI_PBM_IF_MAX_ECC_LEN)
        {
          tot_len += (rec_hcoded_ptr->ecc_len + sizeof(rec_hcoded_ptr->ecc_len));
          if (!(QMI_SVC_PKT_PUSH(&response_ptr, (void*)rec_hcoded_ptr->ecc, rec_hcoded_ptr->ecc_len) &&
             QMI_SVC_PKT_PUSH(&response_ptr, (void*)&rec_hcoded_ptr->ecc_len, sizeof(rec_hcoded_ptr->ecc_len))))
      ...
      if (num_instances > 0)
      {
        if (!qmi_pbmi_fill_array_tlv(&response_ptr, PBMI_GET_EMER_HCODED, tot_len, num_instances))
        {
          errval = QMI_ERR_NO_MEMORY;
          dsm_free_packet(&response_ptr);
        }
    ...
    else
    {
      ...
      if (num_instances > 0)
      {
        if (!qmi_pbmi_fill_array_tlv(&response_ptr, PBMI_GET_EMER_NV_EXT, tot_len, num_instances))
        {
          errval = QMI_ERR_NO_MEMORY;
          dsm_free_packet(&response_ptr);
          goto send_result;
  ...
  /* Fill NV Emergency info with category  if present */
    ...
    else
    {
      for (iter_ecc_entries = (num_of_nv_entries-1);iter_ecc_entries >=0;iter_ecc_entries--)
      {
        ...
        if (rec_nv_ptr->ecc_len > 0)
        {
          uint8 ecc_siz = sizeof(rec_nv_ptr->category);
          tot_len += ecc_siz;
          if ( !(QMI_SVC_PKT_PUSH(&response_ptr, (void*)&rec_nv_ptr->category , sizeof(rec_nv_ptr->category)) ))
          {
            errval = QMI_ERR_NO_MEMORY;
            dsm_free_packet(&response_ptr);
            goto send_result;
      ...
      if (num_instances > 0)
      {
        if (!qmi_pbmi_fill_array_tlv(&response_ptr, PBMI_GET_EMER_NV_EXT_CAT, tot_len, num_instances))
        {
          errval = QMI_ERR_NO_MEMORY;
          dsm_free_packet(&response_ptr);
          goto send_result;
    ...
    if (QMI_ERR_NONE != qmi_pbm_populate_mcc_based_ext_ecc(
                         &response_ptr,
                         PBMI_HARDCODED_MCC_EMER_NUM,
                         data_ptr->data.emerg_ext_list_resp.ext_emer_data->hcoded_mcc_ecc))
    {
      errval = QMI_ERR_NO_MEMORY;
      goto send_result;
    }
    ...
    if (QMI_ERR_NONE != qmi_pbm_populate_mcc_based_ext_ecc(
                         &response_ptr,
                         PBMI_SERVING_MCC_HARDCODED_EMER_NUM,
                         data_ptr->data.emerg_ext_list_resp.ext_emer_data->hcoded_serving_mcc_ecc))
    {
      errval = QMI_ERR_NO_MEMORY;
      goto send_result;
    }
    ...
    if (QMI_ERR_NONE != qmi_pbm_populate_mcc_based_ecc_sub_wise_ext(
                         &response_ptr,
                         PBMI_HARDCODED_MCC_EMER_NUM_SUB_WISE,
                         data_ptr->data.emerg_ext_list_resp.ext_emer_data->hcoded_mcc_ecc))
    {
      errval = QMI_ERR_NO_MEMORY;
      goto send_result;
    ...
    if (QMI_ERR_NONE != qmi_pbm_populate_mcc_based_ecc_sub_wise_ext(
                         &response_ptr,
                         PBMI_SERVING_MCC_HARDCODED_EMER_NUM_SUB_WISE,
                         data_ptr->data.emerg_ext_list_resp.ext_emer_data->hcoded_serving_mcc_ecc))
    {
      errval = QMI_ERR_NO_MEMORY;
      goto send_result;
...
send_result:
  if (FALSE == qmi_pbmi_send_response( errval,
                                      ...
                                      response_ptr,FALSE))
  {
    UIM_MSG_HIGH_0 ("Unable to send response for PBM GET EMERGENCY LIST EXT operation");
  }
} /* qmi_pbmi_get_emergency_list_ext_resp */
...
/*===========================================================================
FUNCTION QMI_PBM_IF_TIMED_SLEEP

DESCRIPTION
  This function is a way to do a timed sleep and implement a
  non-deferrable timer. It also pets the dog in the meantime.
...
RETURN VALUE
  None
===========================================================================*/
static void qmi_pbm_if_timed_sleep(
  unsigned long millisec
)
{
#ifndef FEATURE_UIM_TEST_FRAMEWORK
  rex_sigs_type          sigs                          = 0x00;
  rex_sigs_type          dog_signal                    = 0x00;
  rex_sigs_type          timer_signal                  = 0x00;
  rex_timer_type         qmi_pbm_timed_sleep_timer;
  timer_group_type       qmi_pbm_always_on_timer_group;

  memset(&qmi_pbm_timed_sleep_timer, 0x00, sizeof(rex_timer_type));
  memset(&qmi_pbm_always_on_timer_group, 0x00, sizeof(timer_group_type));

  dog_signal = ((rex_sigs_type) 1 << (rex_sigs_type) QMI_PBM_DOG_RPT_TIMER_SIGNAL);
  timer_signal = ((rex_sigs_type) 1 << (rex_sigs_type) QMI_PBM_TIMED_SLEEP_SIGNAL);

  /* Define the timer */
  timer_def(&qmi_pbm_timed_sleep_timer,
            &qmi_pbm_always_on_timer_group,
            rex_self(),
            timer_signal,
            NULL,
            0);
  ...
  (void)rex_set_timer(&qmi_pbm_timed_sleep_timer,
                      millisec);

  do
  {
    sigs = rex_wait(dog_signal | timer_signal);
  ...
  } while((sigs & timer_signal) == 0 );

  (void)rex_clr_sigs(rex_self(),
                     timer_signal);

  /* Undefine the timer before exiting */
  (void)rex_undef_timer(&qmi_pbm_timed_sleep_timer);
#else
  #error code not present
#endif /* !FEATURE_UIM_TEST_FRAMEWORK */
} /* qmi_pbm_if_timed_sleep */


/*===========================================================================
  FUNCTION QMI_PBMI_BUILD_RECORD_READ_IND()

  DESCRIPTION
    Build Record Read indication
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static boolean qmi_pbmi_build_record_read_ind(
  const qmi_pbm_if_info_item_s *const   data_ptr,
        dsm_item_type                 **ind_pptr
)
{
  uint8                         num_recs                = 0;
  uint8                         num_instances           = 0;
  ...
  const qmi_pbm_if_rec_data_s  *rec_ptr                 = NULL;
  qmi_pbm_if_record_read_ind_s *ind_data_ptr            = NULL;
  uint16                        seq_num                 = 0;
  uint8                         sess_type               = 0;
  uint8                         tag                     = QMI_TYPE_REQUIRED_PARAMETERS;
  ...
  ind_data_ptr = (qmi_pbm_if_record_read_ind_s*)data_ptr->data.rec_read_ind;
  if(ind_data_ptr == NULL)
  {
    UIM_MSG_ERR_0("Record read Indication data is null");
    return FALSE;
  }

  num_recs = ind_data_ptr->num_of_recs;
  sess_type = ind_data_ptr->sess_type;
  pb_type = ind_data_ptr->pb_type;
  seq_num = ind_data_ptr->seq_num;
  ...
  if(pb_type == ((uint8)QMI_PBM_IF_PB_ADN))
  {
    ...
    for(i = (num_recs-1);i >=0;i--)
    {
      rec_ptr = &ind_data_ptr->rec_data[i];

      if(!(QMI_SVC_PKT_PUSH(ind_pptr, (void*)&rec_ptr->is_hidden, sizeof(rec_ptr->is_hidden)) &&
         QMI_SVC_PKT_PUSH(ind_pptr, (void*)&rec_ptr->rec_id, sizeof(rec_ptr->rec_id)) ) )
      {
        dsm_free_packet(ind_pptr);
        continue;
      }

      tot_len += sizeof(rec_ptr->is_hidden) + sizeof(rec_ptr->rec_id);
      num_instances++;
    }

    if( num_instances > 0)
    {
      if(!qmi_pbmi_fill_array_tlv(ind_pptr, hidden_tag, tot_len, num_instances ))
      {
        dsm_free_packet(ind_pptr);
      }
    }
    ...
    for(i = (num_recs-1);i >= 0;i--)
    {
      ...
      if(rec_ptr->email_cnt > 0)
      {
        for(k=0;k < rec_ptr->email_cnt; k++)
        {
          uint16 email_siz = rec_ptr->email_data[k].len + sizeof(rec_ptr->email_data[k].len);

          if(data_ptr->header.cmd == QMI_PBM_IF_CMD_RECORD_READ_EXT_IND)
          {
            /* for EXT read CMD max length that can be read is QMI_PBM_IF_MAX_TEXT_EXT_LEN * 2 */
            email_siz += rec_ptr->email_data[k].len;
          }

          if( !QMI_SVC_PKT_PUSH(ind_pptr, (void*)&rec_ptr->email_data[k], email_siz) )
          {
            dsm_free_packet(ind_pptr);
            break;
          }
        ...
        if(!( QMI_SVC_PKT_PUSH(ind_pptr, (void*)&rec_ptr->email_cnt, sizeof(rec_ptr->email_cnt)) &&
           QMI_SVC_PKT_PUSH(ind_pptr, (void*)&rec_ptr->rec_id, sizeof(rec_ptr->rec_id))) )
        {
          dsm_free_packet(ind_pptr);
          continue;
        }
        tot_len += sizeof(rec_ptr->email_cnt) + sizeof(rec_ptr->rec_id);
        num_instances++;
      }
    }
    ...
    if( num_instances > 0)
    {
      if(!qmi_pbmi_fill_array_tlv(ind_pptr, email_tag, tot_len, num_instances))
      {
        dsm_free_packet(ind_pptr);
      }
    }
    ...
    for(i = (num_recs-1);i >=0;i--)
    {
      ...
      if(rec_ptr->grp_cnt > 0)
      {
        for(k=0;k < rec_ptr->grp_cnt; k++)
        {
          if(!QMI_SVC_PKT_PUSH(ind_pptr, (void*)&rec_ptr->grp_id[k], sizeof(rec_ptr->grp_id[k])) )
          {
            dsm_free_packet(ind_pptr);
            break;
          }

          tot_len += sizeof(rec_ptr->grp_id[k]);
        }

        if(!( QMI_SVC_PKT_PUSH(ind_pptr, (void*)&rec_ptr->grp_cnt, sizeof(rec_ptr->grp_cnt)) &&
           QMI_SVC_PKT_PUSH(ind_pptr, (void*)&rec_ptr->rec_id, sizeof(rec_ptr->rec_id))) )
        {
          dsm_free_packet(ind_pptr);
          continue;
        ...
        tot_len += sizeof(rec_ptr->grp_cnt) + sizeof(rec_ptr->rec_id);
        num_instances++;
      }
    }
    ...
    if( num_instances > 0)
    {
      if(!qmi_pbmi_fill_array_tlv(ind_pptr, grp_tag, tot_len, num_instances))
      {
        dsm_free_packet(ind_pptr);
      }
    }
    ...
    for(i = (num_recs-1); i >= 0; i--)
    {
      ...
      if(rec_ptr->ad_num_cnt > 0)
      {
        for(k=0;k < rec_ptr->ad_num_cnt; k++)
        {
          uint8 tag_siz = sizeof(rec_ptr->ad_num_data[k].tag_id);
          uint8 num_siz = sizeof(rec_ptr->ad_num_data[k].ad_num.len) + rec_ptr->ad_num_data[k].ad_num.len + \
                          sizeof(rec_ptr->ad_num_data[k].ad_num.num_plan) + sizeof(rec_ptr->ad_num_data[k].ad_num.num_type);

          if(!(QMI_SVC_PKT_PUSH(ind_pptr, (void*)&rec_ptr->ad_num_data[k].tag_id, tag_siz) &&
             QMI_SVC_PKT_PUSH(ind_pptr, (void*)&rec_ptr->ad_num_data[k].ad_num, num_siz) ) )
          {
            dsm_free_packet(ind_pptr);
            break;
          }
        ...
        if( !( QMI_SVC_PKT_PUSH(ind_pptr, (void*)&rec_ptr->ad_num_cnt, sizeof(rec_ptr->ad_num_cnt)) &&
             QMI_SVC_PKT_PUSH(ind_pptr, (void*)&rec_ptr->rec_id, sizeof(rec_ptr->rec_id))) )
        {
          dsm_free_packet(ind_pptr);
          continue;
        ...
        tot_len += sizeof(rec_ptr->ad_num_cnt) + sizeof(rec_ptr->rec_id);
        num_instances++;
      }
    }
    ...
    if( num_instances > 0)
    {
      if(!qmi_pbmi_fill_array_tlv(ind_pptr, ad_num_tag, tot_len, num_instances))
      {
        dsm_free_packet(ind_pptr);
      }
    }
    ...
    {
      rec_ptr = &ind_data_ptr->rec_data[i];
      if(rec_ptr->sname_info.len > 0)
      {
        uint16 siz = sizeof(rec_ptr->sname_info.len) + rec_ptr->sname_info.len;
        if(data_ptr->header.cmd == QMI_PBM_IF_CMD_RECORD_READ_EXT_IND)
        {
          /* for EXT read CMD max length that can be read is QMI_PBM_IF_MAX_TEXT_EXT_LEN * 2 */
          siz += rec_ptr->sname_info.len;
        }

        if(!(QMI_SVC_PKT_PUSH(ind_pptr, (void*)&rec_ptr->sname_info, siz) &&
           QMI_SVC_PKT_PUSH(ind_pptr, (void*)&rec_ptr->rec_id, sizeof(rec_ptr->rec_id)) ) )
        {
          dsm_free_packet(ind_pptr);
          continue;
        ...
        tot_len += siz + sizeof(rec_ptr->rec_id);
        num_instances++;
      }
    }
    ...
    if( num_instances > 0)
    {
      if(!qmi_pbmi_fill_array_tlv(ind_pptr, sname_tag, tot_len, num_instances ))
      {
        dsm_free_packet(ind_pptr);
      }
    }
  ...
  for(i = (num_recs-1); i >= 0; i--)
  {
    ...
    rec_ptr = &ind_data_ptr->rec_data[i];
    name_siz = sizeof(rec_ptr->name_info.len) + rec_ptr->name_info.len;

    if(data_ptr->header.cmd == QMI_PBM_IF_CMD_RECORD_READ_EXT_IND)
    {
      /* for EXT read CMD max length that can be read is QMI_PBM_IF_MAX_TEXT_EXT_LEN * 2 */
      name_siz += rec_ptr->name_info.len;
    }

    num_siz = sizeof(rec_ptr->num_info.num_type) + sizeof(rec_ptr->num_info.num_plan) + \
              sizeof(rec_ptr->num_info.len) + rec_ptr->num_info.len;

    if(!(QMI_SVC_PKT_PUSH(ind_pptr, (void*)&rec_ptr->name_info, name_siz) &&
       QMI_SVC_PKT_PUSH(ind_pptr, (void*)&rec_ptr->num_info, num_siz) &&
       QMI_SVC_PKT_PUSH(ind_pptr, (void*)&rec_ptr->rec_id, sizeof(rec_ptr->rec_id))) )
    {
      dsm_free_packet(ind_pptr);
      UIM_MSG_HIGH_0("Cannot send Record read indication No Memory left!");
      QMI_PBM_IF_MEM_FREE(ind_data_ptr);
      return FALSE;
    }

    tot_len += name_siz + num_siz + sizeof(rec_ptr->rec_id);
    num_instances++;
  }

  tot_len += sizeof(num_instances) + sizeof(pb_type) + sizeof(sess_type) + \
             sizeof(seq_num);
  if(!(QMI_SVC_PKT_PUSH(ind_pptr, (void*)&num_instances, sizeof(num_instances)) &&
       QMI_SVC_PKT_PUSH(ind_pptr, (void*)&pb_type, sizeof(pb_type)) &&
       QMI_SVC_PKT_PUSH(ind_pptr, (void*)&sess_type, sizeof(sess_type)) &&
       QMI_SVC_PKT_PUSH(ind_pptr, (void*)&seq_num, sizeof(seq_num)) &&
       QMI_SVC_PKT_PUSH(ind_pptr, (void*)&tot_len, PBMI_TLV_LEN_SIZE) &&
       QMI_SVC_PKT_PUSH(ind_pptr, (void*)&tag, PBMI_TLV_TAG_SIZE)) )
  {
    dsm_free_packet(ind_pptr);
    UIM_MSG_ERR_0("Cannot send Record read indication No Memory left!");
    QMI_PBM_IF_MEM_FREE(ind_data_ptr);
    return FALSE;
  }

  return TRUE;
}/* qmi_pbmi_build_record_read_ind */
...
{
  qmi_pbmi_cmd_val_e_type       cmd_type     = 0;
  qmi_pbmi_state_type          *pbm_sp       = NULL;
  qmi_pbmi_client_state_type  * cl_sp        = NULL;
  uint8                         cl_id        = 0;
  int                           i            = 0;
  ...
  if(data_ptr == NULL)
  {
    UIM_MSG_ERR_0("Record read Data ptr is null");
    return FALSE;
  }
  ...
  cl_id = qmi_pbmi_get_rec_read_cl_id( ind_data_ptr->sess_type, ind_data_ptr->pb_type);
  ...
  for( i = 0; i < PBMI_MAX_CLIDS; i++)
  {
    cl_sp = (qmi_pbmi_client_state_type *)pbm_sp->client[i];
    ...
    {
      retry_count = 0;
      while(retry_count < QMI_PBM_READ_RECORD_IND_MAX_RETRY_COUNT)
      {
        dsm_item_type    *ind_ptr = NULL;

        if(!qmi_pbmi_build_record_read_ind(data_ptr,
                                           &ind_ptr))
        {
          break;
        }

        UIM_MSG_HIGH_2("Record read ind for 0x%x recs is sent with start-id 0x%x", ind_data_ptr->num_of_recs, ind_data_ptr->rec_data[0].rec_id);

        /* In cases where send indication has is failed from framework side,
           retry a few times to ensure that read indication is send successfully */
        status = qmi_pbmi_send_indication(cl_sp,
                                          cmd_type,
                                          ind_ptr);

        if(status)
        {
          /* Send indication has passed so break */
          break;
        }

        /* We are here means that send indication has failed.
           Increment retry count and try to send indicatio again */
        UIM_MSG_ERR_2("Record read ind has failed for 0x%x recs is sent with start-id 0x%x",
                       ind_data_ptr->num_of_recs, ind_data_ptr->rec_data[0].rec_id);

        qmi_pbm_if_timed_sleep(QMI_PBM_READ_RECORD_IND_RETRY_INTERVAL);
        retry_count++;
      }
      if(status == FALSE)
      {
  ...
  if(ind_data_ptr->seq_num == QMI_PBM_IF_MAX_SEQ_NUM || status == FALSE)
  {
    qmi_pbmi_clear_rec_read_clnt_data(ind_data_ptr->sess_type, ind_data_ptr->pb_type);
  }
  QMI_PBM_IF_MEM_FREE(ind_data_ptr);
  ...
  for( j = 0; j < PBMI_MAX_CLIDS; j++)
  {
       ...
       qmi_pbmi_match_subs_session(cl_sp->pbm_subs_info.subs_id,sess_type)))
    {
      ...
      if(!qmi_pbmi_fill_array_tlv(&ind, PBMI_AAS_ALPHA_EXT, tot_len,
                                     (uint8)(data_ptr->data.aas_update_ind.alpha_len/2)))
      {
        dsm_free_packet(&ind);
        break;
      }
      ...
      {
        aas_len = (uint8)data_ptr->data.aas_update_ind.alpha_len;
      }
      tot_len = sizeof(data_ptr->data.aas_update_ind.session_type) +
  ...
  for( j = 0; j < PBMI_MAX_CLIDS; j++)
  {
       ...
       qmi_pbmi_match_subs_session(cl_sp->pbm_subs_info.subs_id,sess_type)))
    {
      ...
      if(!qmi_pbmi_fill_array_tlv(&ind, PBMI_GAS_ALPHA_EXT, tot_len,
                                  (uint8)(data_ptr->data.gas_update_ind.grp_name_len/2)))
      {
        dsm_free_packet(&ind);
        break;
      }
      ...
      {
        gas_len = (uint8)data_ptr->data.gas_update_ind.grp_name_len;
      }
      tot_len = sizeof(data_ptr->data.gas_update_ind.session_type) +
...
)
{
  ...
  cl_sp_ptr->pbm_info.reg_ext_emergency_list_events = FALSE;
...
/*===========================================================================
  FUNCTION QMI_PBMI_GET_CONFIGURATIOJ()

  DESCRIPTION
    Used to get configuration of phonebook feature status
  ...
  SIDE EFFECTS
    None
===========================================================================*/
static dsm_item_type * qmi_pbmi_get_configuration
(
  void *            sp,
  void *            cmd_buf_p,
  void *            cl_sp,
...
)
{
  ...
  while (*sdu_in)
  {
    ...
    if (PBMI_GET_CONFIGURATION_REQ_TLV != type)
    {
      errval = QMI_ERR_MALFORMED_MSG;
      break;
    }
    ...
    if(!(tlv_data.config_mask & PBMI_GET_CONFIGURATION_ECC_MASK))
    {
      errval = QMI_ERR_INVALID_ARG;
      break;
    }
  ...
  UIM_MSG_HIGH_1("qmi_pbmi_get_configuration for client 0x%x", client_sp->common.clid);

  if(FALSE == qmi_svc_put_param_tlv(&response,
                                    PBMI_GET_CONFIGURATION_RSP_TLV,
                                    sizeof(ecc_flag),
                                    (void *) &(ecc_flag)) )
  {
    errval = QMI_ERR_NO_MEMORY;
