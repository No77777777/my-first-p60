...
Copyright (c) 2015 - 2018, 2021, 2025 QUALCOMM Technologies, Inc(QTI).
All Rights Reserved.
...
/*===========================================================================
    ...
    $Header: //components/rel/uim.mpss/6.1.0/pbm/src/qmi_pbm_if.h#4 $
...
02/04/16    ar     Remove the deprecated cm_mm_ss_info_s_type usage
08/26/15    lm     Enable memory leak detection for pbm
06/29/15    lm     Enable memory leak detection
02/13/15    nr     Clean up MSG_SPRINTF in PBM
...
typedef enum
{
  ...
  QMI_PBM_IF_IND_REG_DISABLE_PB_ALL_INIT_DONE_IND                       = 0x00000040,
  QMI_PBM_IF_IND_REG_EXT_EMER_LIST                                      = 0x00000080
...
typedef enum
{
  ...
  PBMI_CMD_VAL_GET_CONFIGURATION              = 0x0024,
  PBMI_CMD_VAL_GET_EMERGENCY_LIST_EXT         = 0x0025,
  PBMI_CMD_VAL_EXT_EMERGENCY_LIST_IND         = 0x0026,
...
typedef struct {
  uint8                         num_hcoded_ecc_entries;
  qmi_pbm_if_cat_ecc_data_type  hcoded_ecc[ QMI_PBM_IF_MAX_HCODED_MCC_BASED_ECC ];
} qmi_pbm_if_mcc_ecc_data_type;
...
typedef struct {
  ...
  qmi_pbm_if_mcc_ecc_data_type hcoded_mcc_ecc[PBM_NUM_SLOTS];

  qmi_pbm_if_mcc_ecc_data_type hcoded_serving_mcc_ecc[PBM_NUM_SLOTS];
} qmi_pbm_if_ecc_list_type;

typedef struct
{
  uint8 ecc_len;
  char  ecc[QMI_PBM_IF_ECC_LEN15];
}qmi_pbm_if_ext_ecc_data_type;
...
typedef struct
{
  ...
  uint8 ecc_len;
  char  ecc[QMI_PBM_IF_ECC_LEN15];
}qmi_pbm_if_cat_ext_ecc_data_type;


typedef struct{
  uint8  sess_type;
  uint8  ecc_count;
  qmi_pbm_if_cat_ext_ecc_data_type  ecc_info[QMI_PBM_IF_MAX_CARD_ECC];
}qmi_pbm_if_ext_ecc_session_card_data;

typedef struct{
  uint8  sess_type;
  uint8  ecc_count;
  qmi_pbm_if_cat_ext_ecc_data_type  ecc_info[CM_MAX_EMERGENCY_NUM_COUNT];
}qmi_pbm_if_ext_ecc_session_ota_data;

typedef struct{
  uint8                         num_hcoded_ecc_entries;
  qmi_pbm_if_cat_ext_ecc_data_type  hcoded_ecc[QMI_PBM_IF_MAX_HCODED_MCC_BASED_ECC];
} qmi_pbm_if_mcc_ext_ecc_data_type;

typedef struct {
  uint32 num_hcoded_ecc_entries;
  qmi_pbm_if_cat_ext_ecc_data_type  hcoded_ecc[QMI_PBM_IF_MAX_HCODED_ECC_EXT];

  uint32  num_nv_ecc_entries;
  qmi_pbm_if_cat_ext_ecc_data_type nv_ecc[PBM_NV_EMERGENCY_NUMBERS];

  uint32 num_of_card_sessions;
  qmi_pbm_if_ext_ecc_session_card_data card_ecc[QMI_PBM_IF_MAX_SESSIONS];

  uint32 num_of_ota_sessions;
  qmi_pbm_if_ext_ecc_session_ota_data ota_ecc[QMI_PBM_IF_MAX_SESSIONS];

  qmi_pbm_if_mcc_ext_ecc_data_type hcoded_mcc_ecc[PBM_NUM_SLOTS];

  qmi_pbm_if_mcc_ext_ecc_data_type hcoded_serving_mcc_ecc[PBM_NUM_SLOTS];
} qmi_pbm_if_ext_ecc_list_type;
...
typedef struct
{
  ...
  boolean send_ext_emer_list_ind;
...
{
  qmi_error_e_type    error;
  qmi_pbm_if_ext_ecc_list_type *ext_emer_data;
}qmi_pbm_if_get_emerg_ext_list_resp_s;
...
typedef struct
{
  ...
  union
  {
    ...
    qmi_pbm_if_get_emerg_ext_list_resp_s            emerg_ext_list_resp;
    ...
    qmi_pbm_if_ext_emer_ind_s                              ext_emerg_list_ind;
