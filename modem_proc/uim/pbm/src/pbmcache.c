...
$Header: //components/rel/uim.mpss/6.1.0/pbm/src/pbmcache.c#2 $
...
10/11/17    tq     correct the len calculation for alpha coding 
...
04/16/16    nr     Ext EFs shouldn't be cleared as part of FDN enable/disable
...
in the build.
===========================================================================*/
PB_CACHE                 *adn_cache_ptr[MAX_UIM_APPS]     = {NULL};
PB_CACHE                 *sdn_cache_ptr[MAX_UIM_APPS]     = {NULL};
PB_CACHE                 *fdn_cache_ptr[MAX_UIM_APPS]     = {NULL};
PB_CACHE                 *lnd_cache_ptr[MAX_UIM_APPS]     = {NULL};
PB_CACHE                 *grpname_cache_ptr[MAX_UIM_APPS] = {NULL};
PB_CACHE                 *aas_cache_ptr[MAX_UIM_APPS]     = {NULL};
PB_CACHE                 *hidden_cache_ptr[MAX_UIM_APPS]  = {NULL};
PB_CACHE                 *oci_cache_ptr[MAX_UIM_APPS]     = {NULL};
PB_CACHE                 *ici_cache_ptr[MAX_UIM_APPS]     = {NULL};
PB_CACHE                 *mbdn_cache_ptr[MAX_UIM_APPS]    = {NULL};
PB_CACHE                 *msisdn_cache_ptr[MAX_UIM_APPS]  = {NULL};
PB_CACHE                 *mbn_cache_ptr[MAX_UIM_APPS]     = {NULL};

/*===========================================================================
                     Declare data for the UIM extension records.
===========================================================================*/
pbm_ext_cache_s_type     *ext1_cache_ptr[MAX_UIM_APPS]    = {NULL};
pbm_ext_cache_s_type     *ext2_cache_ptr[MAX_UIM_APPS]    = {NULL};
pbm_ext_cache_s_type     *ext3_cache_ptr[MAX_UIM_APPS]    = {NULL};
pbm_ext_cache_s_type     *ext5_cache_ptr[MAX_UIM_APPS]    = {NULL};
pbm_ext_cache_s_type     *ext6_cache_ptr[MAX_UIM_APPS]    = {NULL};

/* Master USIM cache for ADN */
PB_MASTER_USIM_CACHE     *adn_m_usim_cache_ptr[MAX_UIM_APPS][PBM_MAX_NUM_PB_SETS] = {NULL};
...
PB_USIM_CACHE            *sne_cache_ptr[MAX_UIM_APPS][PBM_MAX_NUM_PB_SETS]        = {NULL};
PB_USIM_CACHE            *anr_cache_ptr[MAX_UIM_APPS][PBM_MAX_NUM_SAME_USIM_FILES * PBM_MAX_NUM_PB_SETS] = {NULL};
PB_USIM_CACHE            *email_cache_ptr[MAX_UIM_APPS][PBM_MAX_NUM_PB_SETS]      = {NULL};
PB_USIM_CACHE            *grp_cache_ptr[MAX_UIM_APPS][PBM_MAX_NUM_PB_SETS]        = {NULL};

PB_MASTER_SYNC_CACHE     *adn_m_sync_cache_ptr[MAX_UIM_APPS] = {NULL};

PB_SYNC1_CACHE           *psc_cache_ptr[MAX_UIM_APPS]        = {NULL};
PB_SYNC1_CACHE           *cc_cache_ptr[MAX_UIM_APPS]         = {NULL};
PB_SYNC1_CACHE           *puid_cache_ptr[MAX_UIM_APPS]       = {NULL};
PB_SYNC2_CACHE           *uid_cache_ptr[MAX_UIM_APPS]        = {NULL};
PB_SYNC2_CACHE           *pbc_cache_ptr[MAX_UIM_APPS]        = {NULL};
...
/*===========================================================================
FUNCTION PBM_SESSION_CACHE_FREE

DESCRIPTION
 This function deallocates all session related caches.

DEPENDENCIES

SIDE EFFECTS
===========================================================================*/
void pbm_session_cache_free(
pbm_session_enum_type session_type
)
{
  pbm_pb_set_type pbset_num = PBM_USIM_PB_INVALID;
  uint8           index     = 0;

  PBM_MEM_FREEIF(adn_cache_ptr[session_type]);
  PBM_MEM_FREEIF(sdn_cache_ptr[session_type]);
  PBM_MEM_FREEIF(msisdn_cache_ptr[session_type]);
  PBM_MEM_FREEIF(mbn_cache_ptr[session_type]);
  PBM_MEM_FREEIF(fdn_cache_ptr[session_type]);
  PBM_MEM_FREEIF(lnd_cache_ptr[session_type]);
  PBM_MEM_FREEIF(grpname_cache_ptr[session_type]);
  PBM_MEM_FREEIF(aas_cache_ptr[session_type]);
  PBM_MEM_FREEIF(mbdn_cache_ptr[session_type]);
  PBM_MEM_FREEIF(ici_cache_ptr[session_type]);
  PBM_MEM_FREEIF(oci_cache_ptr[session_type]);

  PBM_MEM_FREEIF(hidden_cache_ptr[session_type]);

  PBM_MEM_FREEIF(ext1_cache_ptr[session_type]);
  PBM_MEM_FREEIF(ext2_cache_ptr[session_type]);
  PBM_MEM_FREEIF(ext3_cache_ptr[session_type]);
  PBM_MEM_FREEIF(ext5_cache_ptr[session_type]);
  PBM_MEM_FREEIF(ext6_cache_ptr[session_type]);

  for(pbset_num = PBM_USIM_PB_ONE; pbset_num < PBM_MAX_NUM_PB_SETS; pbset_num++)
  {
    PBM_MEM_FREEIF(adn_m_usim_cache_ptr[session_type][pbset_num]);

    PBM_MEM_FREEIF(sne_cache_ptr[session_type][pbset_num]);
    PBM_MEM_FREEIF(email_cache_ptr[session_type][pbset_num]);
    PBM_MEM_FREEIF(grp_cache_ptr[session_type][pbset_num]);

    for(index = 0;index <PBM_MAX_NUM_SAME_USIM_FILES;index++)
    {
      PBM_MEM_FREEIF(anr_cache_ptr[session_type][(PBM_MAX_NUM_SAME_USIM_FILES * (uint8)pbset_num) + index]);
    }
  }

  PBM_MEM_FREEIF(adn_m_sync_cache_ptr[session_type]);
  PBM_MEM_FREEIF(psc_cache_ptr[session_type]);
  PBM_MEM_FREEIF(cc_cache_ptr[session_type]);
  PBM_MEM_FREEIF(puid_cache_ptr[session_type]);
  PBM_MEM_FREEIF(uid_cache_ptr[session_type]);
  PBM_MEM_FREEIF(pbc_cache_ptr[session_type]);

  for (index = 0; index < PBM_MAX_G_SIM_FIELD; index++)
  {
    PBM_MEM_FREEIF(g_sim_field_info_ptr[session_type][index]);
  }

  for (index = 0; index < PBM_NUM_FILES_IN_LOOKUP_TABLE; index++)
  {
    PBM_MEM_FREEIF(pbm_usim_path_lookup_table_ptr[session_type][index]);
  }

  PBM_MEM_FREEIF(pbr_file_attr_ptr[session_type]);

  return;
}/*pbm_session_cache_free*/


/*===========================================================================
FUNCTION PBM_SESSION_CACHE_INIT

DESCRIPTION
 This function allocates and initializes all session related caches to zeros.

DEPENDENCIES

SIDE EFFECTS
===========================================================================*/
pbm_return_type pbm_session_cache_init(
pbm_session_enum_type session_type
)
{
  pbm_pb_set_type pbset_num = PBM_USIM_PB_INVALID;
  ...
  PBM_CACHE_ALLOCATE_AND_INIT(adn_cache_ptr, PBM_FILE_ADN, session_type);
  PBM_CACHE_ALLOCATE_AND_INIT(sdn_cache_ptr, PBM_FILE_SDN, session_type);
  PBM_CACHE_ALLOCATE_AND_INIT(msisdn_cache_ptr, PBM_FILE_MSISDN, session_type);
  PBM_CACHE_ALLOCATE_AND_INIT(mbn_cache_ptr, PBM_FILE_MBN, session_type);
  PBM_CACHE_ALLOCATE_AND_INIT(fdn_cache_ptr, PBM_FILE_FDN, session_type);
  PBM_CACHE_ALLOCATE_AND_INIT(lnd_cache_ptr, PBM_FILE_LND, session_type);
  PBM_CACHE_ALLOCATE_AND_INIT(grpname_cache_ptr, PBM_FILE_GAS, session_type);
  PBM_CACHE_ALLOCATE_AND_INIT(aas_cache_ptr, PBM_FILE_AAS, session_type);
  PBM_CACHE_ALLOCATE_AND_INIT(mbdn_cache_ptr, PBM_FILE_MBDN, session_type);
  PBM_CACHE_ALLOCATE_AND_INIT(ici_cache_ptr, PBM_FILE_ICI, session_type);
  PBM_CACHE_ALLOCATE_AND_INIT(oci_cache_ptr, PBM_FILE_OCI, session_type);

  /* for internal hidden cache not populating pb_id */
  PBM_ALLOC_MEM_W_ERR_RET(hidden_cache_ptr[session_type], sizeof(PB_CACHE), NO_ACTION);

  PBM_ALLOC_MEM_W_ERR_RET(ext1_cache_ptr[session_type], sizeof(pbm_ext_cache_s_type), NO_ACTION);
  PBM_ALLOC_MEM_W_ERR_RET(ext2_cache_ptr[session_type], sizeof(pbm_ext_cache_s_type), NO_ACTION);
  PBM_ALLOC_MEM_W_ERR_RET(ext3_cache_ptr[session_type], sizeof(pbm_ext_cache_s_type), NO_ACTION);
  PBM_ALLOC_MEM_W_ERR_RET(ext5_cache_ptr[session_type], sizeof(pbm_ext_cache_s_type), NO_ACTION);
  PBM_ALLOC_MEM_W_ERR_RET(ext6_cache_ptr[session_type], sizeof(pbm_ext_cache_s_type), NO_ACTION);

  for(pbset_num = PBM_USIM_PB_ONE; pbset_num < PBM_MAX_NUM_PB_SETS; pbset_num++)
  {
    PBM_ALLOC_MEM_W_ERR_RET(adn_m_usim_cache_ptr[session_type][pbset_num], sizeof(PB_MASTER_USIM_CACHE), NO_ACTION);

    PBM_ALLOC_MEM_W_ERR_RET(sne_cache_ptr[session_type][pbset_num], sizeof(PB_USIM_CACHE), NO_ACTION);
    PBM_ALLOC_MEM_W_ERR_RET(email_cache_ptr[session_type][pbset_num], sizeof(PB_USIM_CACHE), NO_ACTION);
    PBM_ALLOC_MEM_W_ERR_RET(grp_cache_ptr[session_type][pbset_num], sizeof(PB_USIM_CACHE), NO_ACTION);

    for(index = 0;index <PBM_MAX_NUM_SAME_USIM_FILES;index++)
    {
      PBM_ALLOC_MEM_W_ERR_RET(anr_cache_ptr[session_type][(PBM_MAX_NUM_SAME_USIM_FILES * (uint8)pbset_num) + index], sizeof(PB_USIM_CACHE), NO_ACTION);
    }
  }

  PBM_ALLOC_MEM_W_ERR_RET(adn_m_sync_cache_ptr[session_type], sizeof(PB_MASTER_SYNC_CACHE), NO_ACTION);

  PBM_ALLOC_MEM_W_ERR_RET(psc_cache_ptr[session_type], sizeof(PB_SYNC1_CACHE), NO_ACTION);
  PBM_ALLOC_MEM_W_ERR_RET(cc_cache_ptr[session_type], sizeof(PB_SYNC1_CACHE), NO_ACTION);
  PBM_ALLOC_MEM_W_ERR_RET(puid_cache_ptr[session_type], sizeof(PB_SYNC1_CACHE), NO_ACTION);
  PBM_ALLOC_MEM_W_ERR_RET(uid_cache_ptr[session_type], sizeof(PB_SYNC2_CACHE), NO_ACTION);
  PBM_ALLOC_MEM_W_ERR_RET(pbc_cache_ptr[session_type], sizeof(PB_SYNC2_CACHE), NO_ACTION);

  for (index = 0; index < PBM_MAX_G_SIM_FIELD; index++)
  {
    PBM_ALLOC_MEM_W_ERR_RET(g_sim_field_info_ptr[session_type][index],
                            sizeof(pbm_g_other_field_info_s_type), NO_ACTION);
    memscpy(g_sim_field_info_ptr[session_type][index],
            sizeof(pbm_g_other_field_info_s_type),
            &g_sim_field_info_default[index],
            sizeof(pbm_g_other_field_info_s_type));
  }
  ...
  for (index = 0; index < PBM_NUM_FILES_IN_LOOKUP_TABLE; index++)
  {
    PBM_ALLOC_MEM_W_ERR_RET(pbm_usim_path_lookup_table_ptr[session_type][index],
                            sizeof(pbm_usim_path_lookup_table_element_T), NO_ACTION);
  ...
  PBM_ALLOC_MEM_W_ERR_RET(pbr_file_attr_ptr[session_type], sizeof(mmgsdi_get_file_attr_cnf_type), NO_ACTION);

  return PBM_SUCCESS;
}/*pbm_session_cache_init*/
...
)
{
  ...
  pbm_g_other_field_info_s_type *field_info_ptr   = NULL;
  ...
  if (cache->pb_id.device_type == PBM_ADN )
  {
    ...
    {
      field_info_ptr = g_sim_field_info_ptr[session_type][0];
      if( field_info_ptr == NULL)
      {
        return;
      }
...
void pbm_clear_hidden_cache(pbm_session_enum_type session_type)
{
    ...
    cache = hidden_cache_ptr[session_type];
    if (cache != NULL && cache->pb_cache_array != NULL)
    {
       /* Free all the records. */
       for (index = 1; index <= cache->num_of_records; index++)
       {
          if (cache->pb_cache_array[index] != NULL && cache->records_in_use != 0)
          {
             (void) pbm_cache_delete(cache, cache->pb_cache_array[index]->record_id);
          }
       }
...
void pbm_clear_usim_cache(pbm_session_enum_type session_type, PB_USIM_CACHE *cache)
{
   ...
   if (cache->pb_cache_array != NULL)
   {
      ...
      for (index = 1; index <= cache->num_of_records; index++)
      {
         if (cache->pb_cache_array[index] != NULL && cache->records_in_use != 0)
         {
            (void) pbm_usim_cache_delete(session_type, cache, index, TRUE);
         }
      }
...
void pbm_clear_index_cache(PB_MASTER_USIM_CACHE *m_usim_cache)
{
   ...
   if (m_usim_cache)
   {
      ...
      {
         for (index = 1; index <= m_usim_cache->num_caches; index++)
         {
            PBM_MEM_FREEIF(m_usim_cache->index_array[index]);
         }
      }
      PBM_MEM_FREEIF(m_usim_cache->index_array);
      memset(m_usim_cache, 0, sizeof(PB_MASTER_USIM_CACHE));
   }
}/*pbm_clear_index_cache*/
...
}/*pbm_sim_device_type_to_session_device_type*/
...
SIDE EFFECTS
===========================================================================*/
uint16 pbm_usim_get_first_available(
  PB_USIM_CACHE *usim_cache
)
{
  pbm_usim_cache_s_type **cache      = NULL;
  int                     cache_size = 0;
  uint16                  location   = 0;
...
SIDE EFFECTS
===========================================================================*/
pbm_return_type pbm_usim_cache_add(
  pbm_session_enum_type  session_type,
  PB_USIM_CACHE         *usim_cache,
  uint16                 location,
...
{
  pbm_usim_cache_s_type **cache_array   = NULL;
...
SIDE EFFECTS
===========================================================================*/
pbm_return_type pbm_usim_cache_delete(
  pbm_session_enum_type  session_type,
  PB_USIM_CACHE         *usim_cache,
  uint16                 location, 
  boolean                init
                                      ...
                                      boolean clear_adn_fdn_files_only)
{
  uint8            i            = 0;
  PB_CACHE        *cache        = NULL;
  pbm_file_type   *current_list = NULL;
  uint8            list_size    = 0;
  ...
  {
    /* Clear ext cache for all session types */
    current_list = pbm_extn_files;
    list_size    = ARR_SIZE(pbm_extn_files);

    for(i = 0; i < list_size; i++)
    {
      pbm_clear_ext_cache(session_type, current_list[i]);
    }

    /* Clear sync2 cache for all session types */
    pbm_clear_sync2_cache(uid_cache_ptr[session_type]);
    pbm_clear_sync2_cache(pbc_cache_ptr[session_type]);
  ...
  rex_leave_crit_sect(&pbm_crit_sect);
}/*pbm_clear_and_set_session_caches*/
...
void pbm_clear_usim_caches(pbm_session_enum_type session_type)
{
   ...
   rex_enter_crit_sect(&pbm_crit_sect);
   for (i = 0; i < ARR_SIZE(usim_files); i++)
   {
   ...
   for (pb_set = PBM_USIM_PB_ONE; pb_set < PBM_MAX_NUM_PB_SETS; pb_set++)
   {
      pbm_clear_index_cache(adn_m_usim_cache_ptr[session_type][pb_set]);
   }
   pbm_clear_sync_cache(adn_m_sync_cache_ptr[session_type]);

   rex_leave_crit_sect(&pbm_crit_sect);
}/*pbm_clear_usim_caches*/
...
)
{
  ...
  do
  {
    ...
    */
    ...
    else if((sim_coding_mode_enum_type)(record->sim_text)[0] == SIM_CODING_PURE_UCS2)
    {
      /*Since we have declared array as uint16 so reduce its size by half*/
      uint16 sim_text_buf[PBM_TEXT_SIZE_CHARS / PBM_WORD_SIZE];

      memset(sim_text_buf, 0xFF, sizeof(sim_text_buf));
      /* In case of 80 coding, Removing the header to count the length of the text data only */
      memscpy(sim_text_buf,
              sizeof(sim_text_buf),
              &(record->sim_text[1]),
              PBM_TEXT_SIZE_CHARS - 1);
      /* calculating the length of the text data using FFFF as delimiter */
      for(j = 0; field_lengths[i] <= PBM_MAX_TEXT_LENGTH && sim_text_buf[j] != 0XFFFF; j++)
      {
        field_lengths[i] += 2;
      }
    ...
    else
    {
      ...
      for(j = 0; field_lengths[i] <= PBM_MAX_TEXT_LENGTH && ((uint16*)record->sim_text)[j] != 0XFFFF; j++)
      {
        field_lengths[i] += 2;
      }
      if(field_lengths[i] > 0 && (sim_coding_mode_enum_type)(record->sim_text)[field_lengths[i] - 1] == 0xff) 
      {
        field_lengths[i] -= 1;
      }
    }
        ...
        pb_cache->m_sync_cache->sync_present)
    {
      ...
      for(pb_set_iterator = 0; pb_set_iterator < pb_set; pb_set_iterator++)
      {
        ...
        uid_location += (uint16)sync_cache->records_in_pb_set[pb_set_iterator];
      }

      uid_index = (uint16)(location - adn_records);
    ...
    if(pbm_is_rec_hidden(rec_id)) /* check if the record is hidden */
    {
      ...
      session_type = pbm_mapping_to_session(pb_id.pb_category, (pbm_slot_type) pb_id.slot_id,
                                            pb_id.prov_type);
      pb_cache = hidden_cache_ptr[session_type];
      if(pb_cache == NULL)
      {
        ret = FALSE;
        break;
...
SIDE EFFECTS
===========================================================================*/
boolean pbm_usim_cache_record_add(
  PB_USIM_CACHE               *usim_cache,
  const pbm_sim_record_s_type *record
)
{
  pbm_field_id_e_type    field_id                                   = 0;
  pbm_usim_cache_s_type *node                                       = NULL;
  uint32                 field_lengths[PBM_MAX_USIM_FIELDS_PER_REC] = {0};
  int                    num_fields                                 = 0;
  int                    i                                          = 0;
  int                    j                                          = 0;
...
SIDE EFFECTS
===========================================================================*/
pbm_return_type pbm_usim_cache_fields_add(
  pbm_session_enum_type   session_type,
  PB_USIM_CACHE          *usim_cache,
  uint16                  location,
...
)
{
  ...
  PBM_CHECK_PTR2_RET(usim_cache, fields, PBM_ERROR);
  ...
  node->nref = 1;
  node->num_fields = (uint16)num_fields;
  node->m_recid = (pbm_adn_record_id_type)adn_recid;
...
SIDE EFFECTS
===========================================================================*/
uint16 pbm_uim_generate_sync_values(
  PB_CACHE              *cache,
...
{
  uint16                uid            = 1;
  PB_MASTER_SYNC_CACHE *m_sync_cache   = NULL;
  int                   i              = 0;
...
{
  pbm_addr_cache_s_type *record       = NULL;
  PB_CACHE              *cache        = adn_cache_ptr[session_type];
  PB_CACHE              *cache_hidden = hidden_cache_ptr[session_type];
  uint16                 i            = 0;
  pbm_return_type        ret          = PBM_SUCCESS;

  PBM_CHECK_PTR3_RET(cache, cache->pb_cache_array, cache_hidden, PBM_ERROR);

  /* scan pbc cache to know which index is hidden contact in adn cache and 
     then use pbm_cache_delete to remove it from the cache */
  for( i = 1; i <= cache->num_of_records;i++ )
  {
  ...
  return ret;
}/*pbm_delete_hidden_contacts_from_adn_cache*/
...
pbm_return_type pbm_merge_hidden_contacts(pbm_session_enum_type session_type)
{
  ...
  /*scan pbc cache to know which index is hidden contact 
    and then add it to adn cache using pbm_cache_add */
...
)
{
   ...
   */
  /* pbm usim path lookup table initialization */
  for (file_index = 0; file_index < PBM_NUM_FILES_IN_LOOKUP_TABLE; file_index++)
  {
    if(pbm_usim_path_lookup_table_ptr[session_type][file_index] != NULL)
    {
      memscpy(pbm_usim_path_lookup_table_ptr[session_type][file_index],
              sizeof(pbm_usim_path_lookup_table_element_T),
              &pbm_usim_path_lookup_table_default[file_index],
              sizeof(pbm_usim_path_lookup_table_element_T));
    }
  }

  PBM_CACHE_INIT(adn_cache_ptr[session_type], PBM_FILE_ADN, session_type, rex_leave_crit_sect(&pbm_crit_sect));
  PBM_CACHE_INIT(sdn_cache_ptr[session_type], PBM_FILE_SDN, session_type, rex_leave_crit_sect(&pbm_crit_sect));
  PBM_CACHE_INIT(msisdn_cache_ptr[session_type], PBM_FILE_MSISDN, session_type, rex_leave_crit_sect(&pbm_crit_sect));
  PBM_CACHE_INIT(mbn_cache_ptr[session_type], PBM_FILE_MBN, session_type, rex_leave_crit_sect(&pbm_crit_sect));
  pb_id = fdn_cache_ptr[session_type]->pb_id;
  PBM_CACHE_INIT(fdn_cache_ptr[session_type], PBM_FILE_FDN, session_type, rex_leave_crit_sect(&pbm_crit_sect));
  fdn_cache_ptr[session_type]->pb_id = pb_id;
  PBM_CACHE_INIT(lnd_cache_ptr[session_type], PBM_FILE_LND, session_type, rex_leave_crit_sect(&pbm_crit_sect));
  PBM_CACHE_INIT(grpname_cache_ptr[session_type], PBM_FILE_GAS, session_type, rex_leave_crit_sect(&pbm_crit_sect));
  PBM_CACHE_INIT(aas_cache_ptr[session_type], PBM_FILE_AAS, session_type, rex_leave_crit_sect(&pbm_crit_sect));
  PBM_CACHE_INIT(mbdn_cache_ptr[session_type], PBM_FILE_MBDN, session_type, rex_leave_crit_sect(&pbm_crit_sect));
  PBM_CACHE_INIT(ici_cache_ptr[session_type], PBM_FILE_ICI, session_type, rex_leave_crit_sect(&pbm_crit_sect));
  PBM_CACHE_INIT(oci_cache_ptr[session_type], PBM_FILE_OCI, session_type, rex_leave_crit_sect(&pbm_crit_sect));
  ...
  pbm_mmgsdi_register_fcn(session_type);
  {
    ...
    UIM_MSG_HIGH_1("PBM queried for local phonebook 0x%x",result);
...
{
   pbm_session_enum_type    pbm_session_type  = PBM_SESSION_MAX;
   ...
   if (pbm_session_initializing & PBM_SESSIONID_TO_SESSION_MASK(pbm_session_type))
   {
     ...
     /* Checking async buffer, If no transaction is pending then re-starting 
        PBM init, else let it be taken care in read callback */
     for(index = 0; index < PBM_ASYNC_BUFFERS; index++)
     {
       if(pbm_buffers[index].record.session_type == pbm_session_type && 
          pbm_buffers[index].in_use)
       {
         flag = TRUE;
     ...
     if(flag == FALSE)
     {
       pbm_restart_fileinfo_internal(pbm_session_type);
     }
     return;
   }
   ...
   if (pbm_session_initializing & PBM_SESSIONID_TO_SESSION_MASK(pbm_session_type))
   {
     UIM_MSG_HIGH_1("PBM already session init 0x%x",pbm_session_type);
     PBM_SESSION_SET(pbm_session_restart, pbm_session_type);
     return;
   }
