...
GENERAL DESCRIPTION
  ...
  Copyright (c) 2016 - 2017, 2021 by QUALCOMM Technologies, Inc(QTI).
  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
$Header: //components/rel/uim.mpss/6.1.0/pbm/src/pbm_common_utils.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/29/21   shp     Support serving MCC based ECC number for china
06/01/17   nr      EM call handling on sub when voice call going on other sub
...
DESCRIPTION
  ...
  g_rest_field_info, g_sim_field_info_ptr, g_efs_field_info_ptr
...
)
{
  ...
  if (( field_group != ECC_GROUP) && ((ret = pbm_check_pb_ready(pb_id)) != PBM_SUCCESS) )
  {
    return ret;
  }
...
)
{
  ...
  {
    UIM_MSG_ERR_1("PB_SET 0x%x out of bounds ", pb_set);
    return FALSE;
  }
...
)
{
  ...
  int                            num_fields_with_complex_fields     = 0;
  uint32                        *new_rec_sizes                      = NULL;
  boolean                        converted_to_complex_anr           = FALSE;
  uint8                          cnt                                = 0;
  uint8                          valid_field_cnt                    = 0;
  pbm_return_type                ret                                = PBM_SUCCESS;
  pbm_field_type_e_type          uncompressed_field_type            = PBM_FT_NONE;
  boolean                        is_sups_string                     = FALSE ;
  uint8                         *uncompressed_data                  = NULL;

  PBM_CHECK_PTR3_RET(fields, new_fields, total_fields, PBM_ERROR);
  ...
  for(cnt = 0; cnt < num_fields; cnt++)
  {
    ...
    memset(uncompressed_data, 0, PBM_TEXT_SIZE_BYTES * sizeof(uint8));
      ...
      if(skip_text_fields == FALSE)
    {
      ...
      else
      {
        ...
        sim_coding_mode_enum_type coding_mode  = SIM_CODING_GSM7;

        coding_mode = (sim_coding_mode_enum_type)sfield_data_ptr[0];
        switch(coding_mode)
        {
          case SIM_CODING_PURE_UCS2:
          case SIM_CODING_UCS2_HALF_PAGE_1:
          case SIM_CODING_UCS2_HALF_PAGE_2:
            //length already counts in for NULL char at end  
            new_rec_sizes[valid_field_cnt] = sfield->data_len;
            break;

          default:
            while(sfield_data_ptr[valid_length++] != 0xFF);
            //append NULL char
            new_rec_sizes[valid_field_cnt] = valid_length;
            break;
        }
      }
    }
  ...
  for(cnt=0;cnt<num_fields;cnt++)
  {
    ...
    if (sfield->field_type == PBM_FT_BCD)
    {
      ...
      if( uncompressed_data[0] == '+' )
      {
        if ( pbm_util_is_valid_sups_string((char *)(uncompressed_data+1),
                                            (uint8)strlen((const char *)(uncompressed_data+1)),
                                            &pos_plus,
                                            FALSE))
        {
    ...
    {
        memscpy(uncompressed_data,PBM_TEXT_SIZE_BYTES,sfield_data_ptr,new_rec_sizes[valid_field_cnt]);
    ...
    pbm_fill_field(&uncompressed_fields[valid_field_cnt],
                    ...
                    uncompressed_data,
                    (uint16)new_rec_sizes[valid_field_cnt]);
    valid_field_cnt++;
  }
...
SIDE EFFECTS
===========================================================================*/
pbm_return_type pbm_check_pb_ready(
  pbm_phonebook_type pb_id
)
{
  pbm_return_type  ret    = PBM_ERROR;
...
/*===========================================================================
FUNCTION PBM_MAPPING_PBM_SESSION_TO_AS_ID_MASK

DESCRIPTION
  This function maps session to AS_ID mask
...
===========================================================================*/
uint8 pbm_mapping_pbm_session_to_as_id_mask(
  pbm_session_enum_type  pbm_session_type
)
{
  switch(pbm_session_type)
  {
    case PBM_SESSION_LPB_GW:
    case PBM_SESSION_LPB_1X:
      return SYS_MODEM_AS_ID_MASK_1;

    case PBM_SESSION_LPB_GW_SECONDARY:
    case PBM_SESSION_LPB_1X_SECONDARY:
      return SYS_MODEM_AS_ID_MASK_2;

    case PBM_SESSION_LPB_GW_TERTIARY:
    case PBM_SESSION_LPB_1X_TERTIARY:
      return SYS_MODEM_AS_ID_MASK_3;

    default:
      return SYS_MODEM_AS_ID_MASK_NONE;
  }
}/* pbm_mapping_pbm_session_to_as_id_mask */
...
/*===========================================================================
FUNCTION PBM_MATCH_FIELDS

DESCRIPTION
Local utility function used by pbm_match_found() to determine whether the
...
SIDE EFFECTS
===========================================================================*/
static boolean pbm_match_fields(
...
)
{
  ...
  if((category == PBM_CAT_NONE) || (cache_entry->category == (uint16) category))
  {
    ...
    if(cache_entry->slot != PBM_SLOT_NONE && 
       cache_entry->slot != ecc_slot)
    {
      return FALSE;
    }
    ...
    if(pbm_utils_is_phone_field(field_id))
    {
      ...
      if(strstr((char *)local_number,"+"))
      {
        ...
        data_size = (uint16)data_size_temp;
      }
      else
      {
      ...
      while ((sfield = pbm_enum_complex_next(result)) != NULL)
      {
                         ...
                         sfield->field_type))
        {
              ...
              sfield->field_type == PBM_FT_BCD)
          {
            ...
            cached_entry_len       = (uint16)strlen((const char *)cached_entry_data) + 1;
          }
          else if (sfield->field_type == PBM_FT_UCS2_COMPRESSED)
          {
            (void) pbmconvert_sim_to_string((uint8*)pbm_get_field_data_ptr(sfield,0),
             ...
             cached_entry_len       = (uint16)(2 * (pbmutils_wstrlen((uint16*)cached_entry_data) + 1));
          }
          else
          {
...
{
  pbm_addr_cache_s_type **cache      = NULL;
  int                     cache_size = 0;
  uint16                  location   = 0;
...
)
{
  ...
  location = pbm_record_id_to_location((uint16)record_id);
                                   ...
                                   const mmgsdi_file_security_type        *write)
{
   ...
   {
      cache->file_ids[pb_set]  = file_id;
      cache->file_type         = file_type;

      if (increment_num_files)
      {
         cache->num_files++;
         cache->num_of_records += num_records;
      } else
      {
         cache->num_files = 1;
         cache->num_of_records = num_records;
      ...
      {
        cache->record_length[pb_set] = record_length;
        cache->records_in_pb_set[pb_set] = num_records;
        if (cache->pb_id.device_type != PBM_GRPNAME && cache->pb_id.device_type != PBM_AAS)
        {
          cache->text_length[pb_set] = record_length - min_record_length;
        }
        else
        {
          cache->text_length[pb_set] = record_length;
        }
      }

      if (cache->num_of_records > 0)
      {
         cache->status = PBM_STATUS_NOT_READY;
      }
      else
      {
      ...
      if(write != NULL)
      {
         cache->write.protection_method = write->protection_method;
         cache->write.num_protection_pin = write->num_protection_pin;
         for(i=0; i < write->num_protection_pin; i++)
         {
            pin_type = write->protection_pin_ptr[i];
            if(pin_type == MMGSDI_PIN1)
            {
               cache->write.pin_protection_mask |= (1 << PBM_PIN_PIN1);
            }
            else if(pin_type == MMGSDI_PIN2)
            {
               cache->write.pin_protection_mask |= (1 << PBM_PIN_PIN2);
            }
            else if(pin_type == MMGSDI_ADM1)
            {
               cache->write.pin_protection_mask |= (1 << PBM_PIN_ADM);
...
)
{
   ...
   {
     pb_id.slot_id = pbm_util_get_slot_for_prov_session(session_type);
   }
