...
GENERAL DESCRIPTION
  ...
  Copyright (c) 2002 - 2019, 2021 by QUALCOMM Technologies, Inc(QTI).
  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
$Header: //components/rel/uim.mpss/6.1.0/pbm/src/pbmutils.c#4 $
...
11/16/17   tq      Avoid memory misalignment
10/18/17   tq      Corrected memory misalignment
08/01/17   tq      correct the len calculation for alpha coding 80
...
06/15/16   sp      Fix KW warning
05/19/16   nr      Logging improvements
05/03/16   nr      Logging improvements
04/29/16   nr      Adding support for special chinese character encoding in 0x80
04/18/16   sp      ECC only support in PBM
04/15/16   nr      Adding support for special chinese character encoding 
04/13/16   sp      Reduce global memory used by PBM
02/05/16   nr      Update ECC cache and send ready after NV Refresh
01/29/15   ar      Don't allow ADN write if input len > max_len for that PB set
01/28/16   na      Ensuring alpha length to be zero for EF-MDN capability
30/12/15   dd      Place braces for all code blocks
12/08/15   lm      Remove compiler warnings
11/25/15   sp      Fix to return correct length tlv for uncdecoded read record 
11/20/16   nr      Logging improvements
11/04/15   stv     Logging improvements
10/07/15   lm      Enable memory leak detection
09/30/15   lm      Fix compiler warning
09/16/15   nr      Defense-in-depth signedness issue in pbm_ascii_to_bcd
09/15/15   nr      Validating file attribute response for PBM EFS
09/09/15   nr      Correcting input validation
08/27/15   nr      Featurize code not used by some customers
08/05/15   nr      Operator specifc NV ECC numbers configuration
05/04/15   stv     Fix to convert compressed fileds to uncompressed while performing excat match
...
#define PBM_ICCID_PARSED_LENGTH              20
#define PBM_ICCID_IIN_START_POS              2
#define PBM_ICCID_IIN_SKIP_LITERAL          '0'
#define PBM_ICCID_INN_MAX_LEN                4

/* Note: When new fields are added to this table, update PBM_MAX_G_SIM_FIELD
 * if required */

 /* represent adn & fdn for all sessions */
pbm_g_other_field_info_s_type *g_sim_field_info_ptr[MAX_UIM_APPS][PBM_MAX_G_SIM_FIELD] = {NULL};

pbm_g_other_field_info_s_type  g_sim_field_info_default[PBM_MAX_G_SIM_FIELD] =
{
  /* ADN Group */
   {
   12,
...
/* Emergency Group */
{
   7,
   {
      ...
      { PBM_FIELD_SERVING_MCC_HARDCODED_ECC, 1, PBM_NUM_SIZE,        PBM_FT_PHONE,    TRUE,  TRUE}
   }
},
...
{
  int     index   = 0;          /* Control variable for loop. */
...
)
{
  ...
  num_len = (uint8) strlen((char *)ascii_number);
...
}/* pbm_get_primary_field_from_complex */
...
SIDE EFFECTS
===========================================================================*/
pbm_return_type pbm_compress_fields(
...
)
{
  ...
  uint8                   cnt                    = 0;
  uint8                   j                      = 0;
  pbm_field_type_e_type   compressed_field_type  = 0;
  ...
  for(cnt=0;cnt<num_fields;cnt++)
  {
    ...
    if (sfield->field_type == PBM_FT_PHONE)
    {
      ...
      {
        UIM_MSG_ERR_2("BCD to ASCII erro %d %d",
                       sfield_data_ptr[0], sfield_data_ptr[1]);
        PBM_MEM_FREEIF(new_rec_sizes);
        PBM_MEM_FREEIF(compressed_data);
        return PBM_ERROR;
      }
    ...
    {
      uint16 ucs_str_len = (uint16)pbmutils_wstrlen((uint16*)sfield_data_ptr);
      ...
      else if((sim_coding_mode_enum_type)compressed_data[0] == SIM_CODING_PURE_UCS2)
      {
        /*Since we have declared array as uint16 so reduce its size by half*/
        uint16 sim_text_buf[PBM_TEXT_SIZE_BYTES / PBM_WORD_SIZE]; 

        memset(sim_text_buf, 0xFF, sizeof(sim_text_buf));
        /* In case of 80 coding, Removing the header to compute the length of the text data only */
        memscpy(sim_text_buf,
                sizeof(sim_text_buf),
                &compressed_data[1],
                PBM_TEXT_SIZE_BYTES - 1);
        /* calculating the length of the text data in pair using FFFF as delimiter */
        for(j = 0; new_rec_sizes[cnt] <= PBM_MAX_TEXT_LENGTH && sim_text_buf[j] != 0XFFFF; j++)
        {
          new_rec_sizes[cnt] += 2;
        }
        /* adding header len of 80 coding */        
      ...
      else
      {
        ...
        for(j = 0; new_rec_sizes[cnt] <= PBM_MAX_TEXT_LENGTH && ((uint16*)compressed_data)[j] != 0XFFFF; j++)
        {
          new_rec_sizes[cnt] += 2;
        }
        if(new_rec_sizes[cnt] > 0 && (sim_coding_mode_enum_type)compressed_data[new_rec_sizes[cnt]-1] == 0xff)
        {
          new_rec_sizes[cnt] -= 1;
        }
      }
      if (new_rec_sizes[cnt] > 0)
      {
        new_rec_sizes[cnt] = (MIN(new_rec_sizes[cnt], PBM_MAX_TEXT_LENGTH) + 1);
      }
    }
    else
    {
  ...
  for(cnt=0;cnt<num_fields;cnt++)
  {
    ...
    {
      (void) pbmconvert_string_to_sim(sfield_data_ptr,
                                      (uint16)pbmutils_wstrlen((uint16*)sfield_data_ptr),
                                      PBM_FT_UCS2,
                                      compressed_data,
                                      PBM_TEXT_SIZE_BYTES,
                                      &chars_converted,
    ...
    pbm_fill_field(&(*new_fields)[cnt],
                   fields[cnt].field_id,
                   compressed_field_type,
                   (byte *)compressed_data,
                   (uint16)new_rec_sizes[cnt]);
...
SIDE EFFECTS
===========================================================================*/
void pbm_build_num_text_record_from_fields(
...
)
{
  ...
  return;
} /* pbm_build_num_text_record_from_fields */
...
SIDE EFFECTS
===========================================================================*/
void pbm_build_record_from_fields(
  pbm_field_s_type *fields,
  int               item_count,
  int               index,
  ...
  record->is_answered = new_record_ptr->is_answered;
  {
    ...
    if( ucs2_text_field_ptr != NULL )
    {
      ...
      (void) pbmconvert_ucs2_to_pbm8(ucs2_text_field_ptr,
                                     (uint16)pbmutils_wstrlen(ucs2_text_field_ptr),
...
===========================================================================*/
pbm_addr_cache_s_type * pbm_build_fields_from_record (
  pbm_phonebook_type   pb_id,
  pbm_record_s_type   *record
)
{
  pbm_addr_cache_s_type  *node                   = NULL;
  uint32                  field_lengths[6]       = {0} ; /* Sim records have at most 5 fields */
  uint32                  build_field_lengths[6] = {0};  /* build_field_lengths are the lengths actually to build */
  ...
  if (node)
  {
    ...
    */
    ...
    if (field_lengths[0] > PBM_MAX_TEXT_LENGTH)
    {
	...
	if ( node->num_fields > 1 )
	{
      ...
      if (field_lengths[1] > PBM_MAX_NUM_LENGTH)
      {
	    field_lengths[1] = PBM_MAX_NUM_LENGTH;
      }
      if (field_lengths[1])
      {
                                          ...
                                          pbm_file_type pbm_file)
{
      ...
      )
    {
        ...
        if(is_ef_init_in_table_ptr && (ret == PBM_SUCCESS))
        {
                           ...
                           pbm_file_type file_id)
{
   ...
   switch (file_id)
   {
      ...
      case PBM_FILE_ADN2:
      case PBM_FILE_ADN3:
         return adn_cache_ptr[session_type];

      case PBM_FILE_FDN:
         return fdn_cache_ptr[session_type];

      case PBM_FILE_SDN:
         return sdn_cache_ptr[session_type];

      case PBM_FILE_LND:
         return lnd_cache_ptr[session_type];

      case PBM_FILE_MSISDN:
         return msisdn_cache_ptr[session_type];

      case PBM_FILE_MBN:
         return mbn_cache_ptr[session_type];

      case PBM_FILE_EXT1:
         return ext1_cache_ptr[session_type];

      case PBM_FILE_EXT2:
         return ext2_cache_ptr[session_type];

      case PBM_FILE_EXT3:
         return ext3_cache_ptr[session_type];
      case PBM_FILE_ECC:
         return &ecc_sim_cache[session_type];

      case PBM_FILE_GAS:
         return grpname_cache_ptr[session_type];
      case PBM_FILE_AAS:
         return aas_cache_ptr[session_type];
      case PBM_FILE_OCI:
         return oci_cache_ptr[session_type];

      case PBM_FILE_ICI:
         return ici_cache_ptr[session_type];

      case PBM_FILE_MBDN:
         return mbdn_cache_ptr[session_type];

      case PBM_FILE_EXT5:
         return ext5_cache_ptr[session_type];

      case PBM_FILE_EXT6:
         return ext6_cache_ptr[session_type];

      case PBM_FILE_PSC:
         return psc_cache_ptr[session_type];

      case PBM_FILE_PUID:
         return puid_cache_ptr[session_type];

      case PBM_FILE_CC:
         return cc_cache_ptr[session_type];
      ...
      case PBM_FILE_EMAIL3:
         ...
         {
            return email_cache_ptr[session_type][pb_set];
         }
         else
         {
            return NULL;
      ...
      case PBM_FILE_SNE3:
         ...
         if ( pbm_validate_pb_set(pb_set) )
         {
            return sne_cache_ptr[session_type][pb_set];
         }
         else
         {
             return NULL;
      ...
      case PBM_FILE_GRP3:
         ...
         if ( pbm_validate_pb_set(pb_set) )
         {
             return grp_cache_ptr[session_type][pb_set];
         }
         else
         {
             return NULL;
      ...
      case PBM_FILE_ANR3:
         ...
         if ( pbm_validate_pb_set(pb_set) )
         {
             return anr_cache_ptr[session_type][PBM_USIM_SAMEFILE_INDEX(pb_set, 0)];
         }
         else
         {
             return NULL;
      ...
      case PBM_FILE_ANRA3:
         ...
         if ( pbm_validate_pb_set(pb_set) )
         {
            return anr_cache_ptr[session_type][PBM_USIM_SAMEFILE_INDEX(pb_set, 1)];
         }
         else
         {
            return NULL;
      ...
      case PBM_FILE_ANRB3:
         ...
         if ( pbm_validate_pb_set(pb_set) )
         {
             return anr_cache_ptr[session_type][PBM_USIM_SAMEFILE_INDEX(pb_set, 2)];
         }
         else
         {
             return NULL;
      ...
      case PBM_FILE_ANRC3:
         ...
         if ( pbm_validate_pb_set(pb_set) )
         {
             return anr_cache_ptr[session_type][PBM_USIM_SAMEFILE_INDEX(pb_set, 3)];
         }
         else
         {
             return NULL;
      ...
      case PBM_FILE_IAP3:
         ...
         if ( pbm_validate_pb_set(pb_set) )
         {
             return adn_m_usim_cache_ptr[session_type][pb_set];
         }
         else
         {
             return NULL;
      ...
      case PBM_FILE_UID2:
      case PBM_FILE_UID3:
         return uid_cache_ptr[session_type];

      case PBM_FILE_PBC:
      case PBM_FILE_PBC1:
      case PBM_FILE_PBC2:
      case PBM_FILE_PBC3:
         return pbc_cache_ptr[session_type];
      default:
         return NULL;
   }
...
SIDE EFFECTS
===========================================================================*/
pbm_pb_set_type pbm_file_id_to_pb_set(
  pbm_file_type           file_id
)
{
  switch (file_id)
...
SIDE EFFECTS
===========================================================================*/
pbm_adn_record_id_type pbm_location_to_adn_rec_id(
...
SIDE EFFECTS
===========================================================================*/
pbm_return_type pbm_record_write_check(
  pbm_phonebook_type      pb_id,
...
)
{
  ...
  if ((ret = pbm_check_pb_ready(pb_id)) != PBM_SUCCESS)
  {
    return ret;
  }
...
SIDE EFFECTS
===========================================================================*/
boolean pbm_compare_rec_ids(
  pbm_record_id_type rec_id,
  pbm_record_id_type rec_id_other
)
{
  pbm_phonebook_type pb_id       = {0};
  pbm_phonebook_type pb_id_other = {0};
...
)
{
  ...
  location = (int)pbm_record_id_to_location((uint16)rec_id);
...
SIDE EFFECTS
===========================================================================*/
PB_USIM_CACHE *pbm_get_next_usim_cache(
  pbm_record_id_type    rec_id,
...
)
{
  ...
  location =  pbm_record_id_to_location((uint16)rec_id);
...
SIDE EFFECTS
===========================================================================*/
pbm_pb_set_type pbm_sync2_record_index_to_pb_set(
  const PB_SYNC2_CACHE *cache,
  int                   index
)
{
  int                 i                   = 0;
  ...
  return pb_set;
}/* pbm_sync2_record_index_to_pb_set */
...
SIDE EFFECTS
===========================================================================*/
pbm_file_type pbm_sync2_record_index_to_filename(
  const PB_SYNC2_CACHE *cache,
  int                   index
)
{
  ...
  return file_id;
} /* pbm_sync2_record_index_to_filename */
...
SIDE EFFECTS
===========================================================================*/
uint16 pbm_record_id_to_fileloc(
  pbm_record_id_type rec_id
)
{
  PB_CACHE *pb_cache   = NULL;
...
SIDE EFFECTS
===========================================================================*/
int pbm_sync2_record_index_to_fileloc(
  const PB_SYNC2_CACHE *cache,
  int                   index
)
{
  ...
  return location;
}/* pbm_sync2_record_index_to_fileloc */
...
SIDE EFFECTS
===========================================================================*/
pbm_pb_set_type pbm_record_id_to_pb_set (
  pbm_record_id_type rec_id
)
{
  PB_CACHE       *pb_cache   = NULL;
  uint16          location   = 0;
  pbm_pb_set_type pb_set     = 0;
...
SIDE EFFECTS
===========================================================================*/
  pbm_return_type pbm_fill_adn_sfi(
...
)
{
  ...
  prev_rec = cache->pb_cache_array[pbm_record_id_to_location((uint16)rec_id)];
  if (prev_rec &&
...
SIDE EFFECTS
===========================================================================*/
boolean pbm_gsm_modified(
  pbm_record_id_type rec_id
)
{
  PB_CACHE       *pb_cache     = pbm_pb_id_to_cache(pbm_session_record_id_to_phonebook_type(rec_id));
  uint16          location     = pbm_record_id_to_location((uint16)rec_id);
...
SIDE EFFECTS
===========================================================================*/
pbm_pb_set_type pbm_adn_record_id_to_pb_set(
  pbm_session_enum_type  session_type,
  pbm_adn_record_id_type rec_id
)
{
...
SIDE EFFECTS
===========================================================================*/
pbm_file_type pbm_record_id_to_file_id(
  pbm_record_id_type rec_id
)
{
  pbm_phonebook_type pb_id    = pbm_session_record_id_to_phonebook_type(rec_id);
  int                location = (int)pbm_record_id_to_location((uint16)rec_id);
  PB_CACHE          *pb_cache = NULL;
  pbm_pb_set_type    pb_set;
...
===========================================================================*/
uint32 pbm_util_return_fileset_for_session(
  pbm_session_enum_type session_type
)
{
  uint32 fileset = 0;
  ...
  return fileset;
} /* pbm_util_return_fileset_for_session */
...
)
{
  ...
  switch (fields[0].field_id)
  {
    ...
    case PBM_FIELD_EMAIL:
      ...
      record->data.text.adn_record_id = (pbm_adn_record_id_type)rec_id;
      if (num_fields > 1)
      {
        UIM_MSG_ERR_2("Too many fields sent to pbm_build_record_from_usim_fields() %d %d",
                        num_fields, fields[0].field_id);
    ...
    case PBM_FIELD_ADDITIONAL_NUMBER_EXT:
      {
        ...
        record->data.anr.adn_record_id = (pbm_adn_record_id_type)rec_id;
      }
      break;
...
)
{
  ...
  pbm_sub_field_s_type       *sub_field        = NULL;
  pbm_field_s_type           *input_fields     = NULL;
  int                         calculated_size  = 0;
  uint32                     *sub_field_sizes  = NULL;
  void                       *temp             = NULL;

  PBM_CHECK_PTR2_RET(data_buf, field, PBM_ERROR);
  input_fields = (pbm_field_s_type*)data_buf;
  calculated_size = pbm_calculate_size_from_fields((pbm_field_s_type*)data_buf,num_fields);

  if (calculated_size > PBM_MAX_RECORD_SIZE || calculated_size > data_buf_size)
  {
    UIM_MSG_ERR_4("Invalid Record (have %d fields, calculated size %d, buffer size %d and max record size %d )",
                   num_fields, calculated_size,data_buf_size,PBM_MAX_RECORD_SIZE);
  ...
  complex_field->num_subfields = (uint16)num_fields;
  sub_field = PBM_FIELD_ALIGN(complex_field->buf);
...
DESCRIPTION
Calculates the complex fields size
...
)
{
  ...
  for(i=0;i<num_fields;i++)
  {
    alloc_size += (uint16)(PBM_SUB_FIELD_HEADER_SIZE + anr_data_sizes[i] + ALIGNMENT - 1);
  }
  return alloc_size;
...
)
{
  ...
  if (pbm_util_is_valid_sups_string((char *)local_number,
                                     (uint8)strlen((char *)local_number),
                                     &pos_plus, FALSE))
  {
...
)
{
  ...
  pbmutil_sups_start_s_type       *sups_start           = NULL;
...
{
  int                  ntokens        = 0;      /* Number of tokens we have saved. */
  char                *current_in     = NULL;   /* Current place in the input string. */
  char                *current_regex  = NULL;   /* Current place of the regular expression */
  ...
  char                *tmp_ptr        = NULL;   /* Work variable when we may need to save it. */
...
/*===========================================================================
FUNCTION pbm_util_is_valid_sups_string
...
)
{
  ...
  boolean                       pbm_ret = FALSE;
  pbmutil_sups_start_s_type     sups_start;
  void                         *pbmutil_tokens[PBMUTIL_SUPS_MAX_TOKENS];

  PBM_CHECK_PTR2_RET(sups_str, pos, pbm_ret);

  *pos = 0;
  memset(&sups_start, 0, sizeof(pbmutil_sups_start_s_type));
  if((sups_str_len > CM_MAX_NUMBER_CHARS) || (sups_str_len < MIN_SUPS_DIGITS))
  {
    UIM_MSG_ERR_0("pbm_util_is_valid_sups_string_find_pos_for_plus Length passed is invalid!");
    return pbm_ret;
  }
  /* check if the sups string is terminated by # and is valid. */
  if((sups_str[sups_str_len-1] != '#') || (!ISDIGIT(sups_str[sups_str_len-2])))
  {
    UIM_MSG_ERR_0("pbm_util_is_valid_sups_string_find_pos_for_plus Invalid Sups String!");
    return pbm_ret;
  }
  ...
  UIM_MSG_HIGH_1("pbm_util_is_valid_sups_string_find_pos_for_plus pbm_ret 0x%x", pbm_ret);
...
)
{
  ...
  {
    case ADN_GROUP:
      if(session_type < PBM_SESSION_MAX && g_sim_field_info_ptr[session_type][0] != NULL)
      {
        memscpy(field_info,sizeof(pbm_g_efs_field_info_s_type),
                g_sim_field_info_ptr[session_type][0],sizeof(pbm_g_other_field_info_s_type));
      }
      break;

    case FDN_GROUP:
      if(session_type < PBM_SESSION_MAX && g_sim_field_info_ptr[session_type][1] != NULL)
      {
        memscpy(field_info,sizeof(pbm_g_efs_field_info_s_type),
                g_sim_field_info_ptr[session_type][1],sizeof(pbm_g_other_field_info_s_type));
      }
      break;

    case GRPNAME_GROUP:
      if(session_type < PBM_SESSION_MAX && g_sim_field_info_ptr[session_type][2] != NULL)
      {
        memscpy(field_info,sizeof(pbm_g_efs_field_info_s_type),
                g_sim_field_info_ptr[session_type][2],sizeof(pbm_g_other_field_info_s_type));
      }
      break;

    case AAS_GROUP:
      if(session_type < PBM_SESSION_MAX && g_sim_field_info_ptr[session_type][3] != NULL)
      {
        memscpy(field_info,sizeof(pbm_g_efs_field_info_s_type),
              g_sim_field_info_ptr[session_type][3],sizeof(pbm_g_other_field_info_s_type));
      }
      break;
    ...
    case CALLHISTORY_GROUP:
      ...
      if(cache != NULL)
      {
        /* Ensuring that length of alpha string is reported to ZERO for EF-MDN in pbm capablity response */
        if(((PBM_MSISDN == pb_id.device_type) &&
             (USE_CDMA_SUPPORT(session_type) || 
              USE_CSIM_LOCAL_SUPPORT(session_type))) ||
            cache->text_length[0])
        {
          /* cache is having the text_length (alpha) as ZERO for EF-MDN so retrieving from it and adding 2bytes for null terminating
             characters. And it is stripped of in qmi_pbm_if */
...
/*===========================================================================
FUNCTION pbm_clear_ext_cache

DESCRIPTION

This function will delete ext ecc cache and free memory associated
with ext ecc cache
...
None
===========================================================================*/
void pbm_clear_ext_cache(
  pbm_session_enum_type session_type,
  pbm_file_type         file_type
)
{
  ...
  rex_enter_crit_sect(&pbm_crit_sect);
  cache = (pbm_ext_cache_s_type *) pbm_file_id_to_cache(session_type, file_type);
  if(cache != NULL && cache->in_use_array != NULL)
  {
    PBM_MEM_FREEIF(cache->in_use_array);
  }
  rex_leave_crit_sect(&pbm_crit_sect);
} /* pbm_clear_ext_cache */
...
)
{
  ...
  switch(evt)
  {
    ...
    case PBM_EVENT_REC_FAILED:
      ...
      UIM_MSG_HIGH_2("PBM sending event 0x%x to clients, for record id 0x%x",
                      evt,
                      evt_data.rec_id);
      break;
    ...
    case PBM_EVENT_CACHE_STATUS_UPDATE:
      ...
      /* FDN status is valid only if devie type is PBM_FDN */
      pNotify_evt_cmd->notify_data.session_data.pb_id.fdn_status = evt_data.pb_id.fdn_status;
      break;
    ...
    case PBM_EVENT_HIDDEN_CONTACTS_STATUS:
      ...
      UIM_MSG_HIGH_3("PBM sending event 0x%x to clients, status 0x%x,prov_type 0x%x",
                      evt,
    ...
    case PBM_EVENT_SIM_INIT_DONE:
      ...
      UIM_MSG_HIGH_2("PBM sending event 0x%x to clients, for slot id 0x%x",
                      evt,
                      evt_data.sim_id);
      break;

    default:
      PBM_MEM_FREEIF(pNotify_evt_cmd);
      return;
  }
...
/*===========================================================================
FUNCTION PBM_UTIL_GET_SLOT_FOR_PROV_SESSION

DESCRIPTION
    This function determines slot id for provisioning session.

SIDE EFFECTS
===========================================================================*/
pbm_slot_type pbm_util_get_slot_for_prov_session(
  pbm_session_enum_type        p_session_type
)
{
  pbm_slot_type slot_id = PBM_SLOT_1;

  if(pbm_session_info[p_session_type].slot_id != 0)
  {
    slot_id = pbm_session_info[p_session_type].slot_id;
    UIM_MSG_HIGH_2("slot id is already associated with session 0x%x and is 0x%x",
                   p_session_type,
                   slot_id);
  ...
  {
    if(p_session_type == PBM_SESSION_LPB_GW)
    {
      if(pbm_session_info[PBM_SESSION_LPB_GW_SECONDARY].slot_id == PBM_SLOT_1)
      {
        slot_id = PBM_SLOT_2;
      }
      else
    ...
    else if(p_session_type == PBM_SESSION_LPB_GW_SECONDARY)
    {
      if(pbm_session_info[PBM_SESSION_LPB_GW].slot_id == PBM_SLOT_1)
      {
        slot_id = PBM_SLOT_2;
      }
      else if(pbm_session_info[PBM_SESSION_LPB_GW].slot_id == PBM_SLOT_2)
      {
        slot_id = PBM_SLOT_1;
      }
      else
    ...
    else if(p_session_type == PBM_SESSION_LPB_1X)
    {
      if(pbm_session_info[PBM_SESSION_LPB_1X_SECONDARY].slot_id == PBM_SLOT_1)
      {
        slot_id = PBM_SLOT_2;
      }
      else
    ...
    else if(p_session_type == PBM_SESSION_LPB_1X_SECONDARY)
    {
      if(pbm_session_info[PBM_SESSION_LPB_1X].slot_id == PBM_SLOT_1)
      {
        slot_id = PBM_SLOT_2;
      }
      else if(pbm_session_info[PBM_SESSION_LPB_1X].slot_id == PBM_SLOT_2)
      {
        slot_id = PBM_SLOT_1;
      }
      else
  ...
  UIM_MSG_HIGH_2("pbm_util_get_slot_for_prov_session 0x%x is 0x%x",
                 p_session_type,
                 slot_id);
  return slot_id;
}/* pbm_util_get_slot_for_prov_session */


/*===========================================================================
FUNCTION PBM_GET_MCC_FROM_ICCID

DESCRIPTION
    This function decodes and retrieve MCC from ICCID.

SIDE EFFECTS
===========================================================================*/
pbm_return_type pbm_get_mcc_from_iccid(
  const  uint8         * iccid_data_ptr,
  uint16                 iccid_len,
  uint16               * mcc_ptr
)
{
  ...
  uint8  index                                     = 0;
  uint8  digit_cnt                                 = 0;
  uint8  parsed_iccid[PBM_ICCID_PARSED_LENGTH]     = {0};
  uint8  char_index                                = 0;

  /* Digit3 to Digit6 in parsed ICCID data denote country code. If Digit3 is 0, country
     code will start from Digit4. Below table is used to map country code to MCC */
  static const pbm_cc_mcc_map_type pbm_iin_mcc_map[] =
  {
    {"1242", 364}, {"1246", 342}, {"1264", 365}, {"1268", 344}, {"1284", 348},
    {"1345", 346}, {"1441", 350}, {"1473", 352}, {"1649", 338}, {"1664", 354},
    {"1684", 544}, {"1758", 358}, {"1767", 366}, {"1784", 360}, {"1787", 330},
    {"1809", 370}, {"1829", 370}, {"1849", 370}, {"1868", 374}, {"1869", 356},
    {"1876", 338}, {"1939", 330}, {"20"  , 602}, {"211" , 659}, {"212" , 604},
    {"213" , 603}, {"216" , 605}, {"218" , 606}, {"220" , 607}, {"221" , 608},
    {"222" , 609}, {"223" , 610}, {"224" , 611}, {"225" , 612}, {"226" , 613},
    {"227" , 614}, {"228" , 615}, {"229" , 616}, {"230" , 617}, {"231" , 618},
    {"232" , 619}, {"233" , 620}, {"234" , 621}, {"235" , 622}, {"236" , 623},
    {"237" , 624}, {"238" , 625}, {"239" , 626}, {"240" , 627}, {"241" , 628},
    {"242" , 629}, {"243" , 630}, {"244" , 631}, {"245" , 632}, {"248" , 633},
    {"249" , 634}, {"250" , 635}, {"251" , 636}, {"252" , 637}, {"253" , 638},
    {"254" , 639}, {"255" , 640}, {"256" , 641}, {"257" , 642}, {"258" , 643},
    {"260" , 645}, {"261" , 646}, {"262" , 647}, {"263" , 648}, {"264" , 649},
    {"265" , 650}, {"266" , 651}, {"267" , 652}, {"268" , 653}, {"269" , 654},
    {"27"  , 655}, {"291" , 657}, {"297" , 363}, {"298" , 288}, {"299" , 290},
    {"30"  , 202}, {"31"  , 204}, {"32"  , 206}, {"33"  , 208}, {"34"  , 214},
    {"350" , 266}, {"351" , 268}, {"352" , 270}, {"353" , 272}, {"354" , 274},
    {"355" , 276}, {"356" , 278}, {"357" , 280}, {"358" , 244}, {"359" , 284},
    {"36"  , 216}, {"370" , 246}, {"371" , 247}, {"372" , 248}, {"373" , 259},
    {"374" , 283}, {"375" , 257}, {"376" , 213}, {"377" , 212}, {"378" , 292},
    {"380" , 255}, {"381" , 220}, {"382" , 297}, {"385" , 219}, {"386" , 293},
    {"387" , 218}, {"389" , 294}, {"39"  , 222}, {"40"  , 226}, {"41"  , 228},
    {"420" , 230}, {"421" , 231}, {"423" , 295}, {"43"  , 232}, {"44"  , 234},
    {"45"  , 238}, {"46"  , 240}, {"47"  , 242}, {"48"  , 260}, {"49"  , 262},
    {"501" , 702}, {"502" , 704}, {"503" , 706}, {"504" , 708}, {"505" , 710},
    {"506" , 712}, {"507" , 714}, {"508" , 308}, {"509" , 372}, {"51"  , 716},
    {"52"  , 334}, {"53"  , 368}, {"54"  , 722}, {"55"  , 724}, {"56"  , 730},
    {"57"  , 732}, {"58"  , 734}, {"590" , 340}, {"591" , 736}, {"592" , 738},
    {"593" , 740}, {"595" , 744}, {"596" , 340}, {"597" , 746}, {"598" , 748},
    {"60"  , 502}, {"61"  , 505}, {"62"  , 510}, {"63"  , 515}, {"64"  , 530},
    {"65"  , 525}, {"66"  , 520}, {"670" , 514}, {"672" , 505}, {"673" , 528},
    {"674" , 536}, {"675" , 537}, {"676" , 539}, {"677" , 540}, {"678" , 541},
    {"679" , 542}, {"680" , 552}, {"682" , 548}, {"683" , 555}, {"685" , 549},
    {"686" , 545}, {"687" , 546}, {"688" , 553}, {"689" , 547}, {"691" , 550},
    {"73"  , 250}, {"74"  , 250}, {"76"  , 401}, {"77"  , 401}, {"78"  , 250},
    {"81"  , 440}, {"82"  , 450}, {"84"  , 452}, {"850" , 467}, {"852" , 454},
    {"853" , 455}, {"855" , 456}, {"856" , 457}, {"86"  , 460}, {"870" , 901},
    {"880" , 470}, {"886" , 466}, {"90"  , 286}, {"91"  , 404}, {"92"  , 410},
    {"93"  , 412}, {"94"  , 413}, {"95"  , 414}, {"960" , 472}, {"961" , 415},
    {"962" , 416}, {"963" , 417}, {"964" , 418}, {"965" , 419}, {"966" , 420},
    {"967" , 421}, {"968" , 422}, {"970" , 425}, {"971" , 424}, {"972" , 425},
    {"973" , 426}, {"974" , 427}, {"975" , 402}, {"976" , 428}, {"977" , 429},
    {"98"  , 432}, {"992" , 436}, {"993" , 438}, {"994" , 400}, {"995" , 282},
    {"996" , 437}, {"998" , 434}, {"1"   , 310}, {"7"   , 250}
  };
  ...
  *mcc_ptr = 0;
  ...
  for(index = 0; index < iccid_len && digit_cnt < PBM_ICCID_PARSED_LENGTH - 1; index++)
  {
    if((iccid_data_ptr[index] & 0x0F) <= 9)
    {
      parsed_iccid[digit_cnt] = (iccid_data_ptr[index]& 0x0F) + '0';
    }
    else
    {
      /* For invalid digit, use character 'a' */
      parsed_iccid[digit_cnt] = 'a';
    }
    digit_cnt++;

    if(((iccid_data_ptr[index] >> 4) & 0x0F) <= 9)
    {
      parsed_iccid[digit_cnt] = ((iccid_data_ptr[index] >> 4)& 0x0F) + '0';
    }
    else
    {
      /* For invalid digit, use character 'a' */
  ...
  if(digit_cnt <= PBM_ICCID_IIN_START_POS + PBM_ICCID_INN_MAX_LEN)
  {
    return PBM_ERROR;
  }

  char_index = PBM_ICCID_IIN_START_POS;

  if(PBM_ICCID_IIN_SKIP_LITERAL == parsed_iccid[char_index])
  {
    char_index++;
  }

  for(index = 0; index < ARR_SIZE(pbm_iin_mcc_map); index++)
  {
    if(0 == memcmp(pbm_iin_mcc_map[index].cc_ptr,
                   &parsed_iccid[char_index],
                   strnlen(pbm_iin_mcc_map[index].cc_ptr, PBM_ICCID_INN_MAX_LEN)))
    {
      *mcc_ptr = pbm_iin_mcc_map[index].mcc;
    }
  }

  return PBM_SUCCESS;
} /* pbm_get_mcc_from_iccid */


/*===========================================================================
FUNCTION PBM_GET_MCC_FROM_PLMN

DESCRIPTION
    This function decodes and retrieve MCC from PLMN.

SIDE EFFECTS
===========================================================================*/
pbm_return_type pbm_get_mcc_from_plmn(
...
)
{
     ...
     plmn.identity[1] != 0XFF )
  {
    ...
    mcc_digit_2 = plmn.identity[0] >> 0x04;
