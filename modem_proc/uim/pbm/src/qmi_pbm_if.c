...
Copyright (c) 2004 - 2018, 2021, 2025 QUALCOMM Technologies, Inc(QTI).
All Rights Reserved.
...
$Header: //components/rel/uim.mpss/6.1.0/pbm/src/qmi_pbm_if.c#4 $
...
11/17/16    ar     Correcting the type-casting logic
10/07/16    nr     F3 improvements
08/31/16    nr     F3 improvements
06/13/16    sp     Return QMI_ERR_NO_SIM when card is not present phase 2
06/13/16    sp     Return correct error code when card is not present in the slot
05/20/16    sp     F3 improvements
04/20/16    sp     Return the correct error for read record and delete all records
11/12/15    lm     Remove compiler warnings
11/04/15   stv     Logging improvements
09/15/15    gs     Populate the right error cause if the request comes before PB initialised
07/28/15    nr     Updating number type correctly in read undecoded API indication
06/12/15    nr     Updating TON/NPI only if we have number info in write req
...
qmi_pbm_if_ext_ecc_list_type* qmi_pbm_populate_ext_ecc_list(void);
/*===========================================================================
...
)
{
  ...
  switch(cmd)
  {
   ...
   case QMI_PBM_IF_CMD_READ_PBSET_CAPS:
    ...
    case QMI_PBM_IF_CMD_GET_EMERGENCY_LIST_EXT:
      pbm_if_data_ptr->header.cmd = QMI_PBM_IF_CMD_GET_EMERGENCY_LIST_EXT_RESP;
      pbm_if_data_ptr->data.emerg_ext_list_resp.error = err_val;
      if (QMI_ERR_NONE == err_val)
      {
        pbm_if_data_ptr->data.emerg_ext_list_resp.ext_emer_data = rsp;
      }
      break;
  ...
  (void)qmi_pbm_encode_and_send_msg(pbm_if_data_ptr);
  return;
}
...
)
{
  ...
  switch(ind_type)
  {
    ...
    case QMI_PBM_IF_CMD_EXT_EMERGENCY_LIST_IND:
      pbm_if_data_ptr->data.ext_emerg_list_ind.ext_emer_data = ind_data;
      break;
...
)
{
  switch(sess_type)
  {
    ...
    case QMI_PBM_IF_SESSION_1X_TERTIARY:
#endif
      return pbm_get_slot_info(qmi_pbm_if_qmi_session_to_pbm_session(sess_type));
...
)
{
  ...
  for(i=0;i<QMI_PBM_IF_MAX_SESSIONS;i++)
  {
    if(pbm_if_info.sess_data[i].sess_type == sess_type)
    {
      ...
      {
        case QMI_PBM_IF_PB_ADN:
          pbm_if_info.sess_data[i].pb_data.max_adn_recs = (uint16)max_recs;
          break;
        case QMI_PBM_IF_PB_FDN:
        ...
        case QMI_PBM_IF_PB_LND:
          ...
          break;
        case QMI_PBM_IF_PB_GAS:
          pbm_if_info.sess_data[i].pb_data.max_gas_recs = (uint8)max_recs;
          break;
        case QMI_PBM_IF_PB_AAS:
          pbm_if_info.sess_data[i].pb_data.max_aas_recs = (uint8)max_recs;
          break;
        default:
          return;
      }
...
{
  pbm_phonebook_type             pb_info;
  pbm_phonebook_type             type3_pb_info;
  pbm_cat_e_type                 cat              = PBM_CAT_NONE;
  pbm_extended_fileinfo_s_type   recs_info;
  pbm_extended_fileinfo_s_type   type3_recs_info;
  pbm_field_info_s_type         *fields           = NULL;
  ...
  if(ret != PBM_SUCCESS)
  {
    UIM_MSG_ERR_1("pbm_session_extended_file_info: returned 0x%x", ret);
    return ret;
  }
  if(pb_type == QMI_PBM_IF_PB_ADN)
  {
    ...
    if(ret != PBM_SUCCESS)
    {
      UIM_MSG_ERR_1("pbm_get_adn_pbset_capibilities: returned 0x%x", ret);
    ...
    if(ret != PBM_SUCCESS)
    {
      UIM_MSG_ERR_1("pbm_get_phonebook_usim_fields_capibilities: returned 0x%x", ret);
  ...
  pb_caps->used_recs = (uint16)recs_info.records_used;
  pb_caps->max_recs  = (uint16)recs_info.number_of_records;
  ...
  if(ret != PBM_SUCCESS)
  {
    UIM_MSG_ERR_1("pbm_session_get_field_info_count: returned 0x%x", ret);
    return ret;
  }
  ...
  if(f_cnt > 0)
  {
    ...
    if(ret != PBM_SUCCESS)
    {
      UIM_MSG_ERR_1("pbm_session_get_field_info: returned 0x%x", ret);
    ...
    for(i = 0; i < f_cnt; i++)
    {
      switch(fields[i].field_id)
      {
        case PBM_FIELD_PHONE_GENERIC:
          ...
          pb_caps->max_num_len = (uint8)(fields[i].max_length - 2);
          break;
        ...
        case PBM_FIELD_NAME:
          ...
          else
          {
            ...
            pb_caps->max_name_len = (uint8)(fields[i].max_length - 2);
          }
          break;
        ...
        case PBM_FIELD_ADDITIONAL_NUMBER:
          if(fields[i].num_items > 0)
          {
            ...
            {
              pb_caps->ad_num_cap.max_ad_nums = (uint8)(fields[i].num_items);
            }
            else
            {
            ...
            /* Subract 2 from max_length as PBM includes 2 extra bytes for nul and + */
            pb_caps->ad_num_cap.max_ad_num_len = (uint8)(fields[i].max_length - 2);
            ...
            {
              UIM_MSG_ERR_0 ("pbm_session_extended_file_info returned error");
            }
            else
            {
              pb_caps->ad_num_cap.max_ad_num_tag_len = (uint8)type3_recs_info.text_len;

              /* AAS Capabilities */
              pb_caps->is_aas_cap_valid = TRUE;
              pb_caps->aas_cap.max_recs = (uint8)type3_recs_info.number_of_records;
              pb_caps->aas_cap.used_recs = (uint8)type3_recs_info.records_used;
              pb_caps->aas_cap.max_aas_len = (uint8)type3_recs_info.text_len;

              pb_caps->pb_set_caps.aas_caps.max_records = (uint8)type3_recs_info.number_of_records;
              pb_caps->pb_set_caps.aas_caps.max_record_len = (uint8)type3_recs_info.text_len ;
              pb_caps->pb_set_caps.aas_caps.used_records = (uint8)type3_recs_info.records_used ;
            }
          }
          break;
        ...
        case PBM_FIELD_GRPNAME_ID:
          if(fields[i].num_items > 0)
          {
            ...
            pb_caps->grp_cap.max_grps = (uint8)(fields[i].num_items);
            ...
            {
              pb_caps->grp_cap.max_grp_tag_len = (uint8)type3_recs_info.text_len;

              /* GAS Capabilities */
              pb_caps->is_gas_cap_valid = TRUE;
              pb_caps->gas_cap.max_recs = (uint8)type3_recs_info.number_of_records;
              pb_caps->gas_cap.used_recs = (uint8)type3_recs_info.records_used;
              pb_caps->gas_cap.max_gas_len = (uint8)type3_recs_info.text_len;

              pb_caps->pb_set_caps.gas_caps.max_records = (uint8)type3_recs_info.number_of_records;
              pb_caps->pb_set_caps.gas_caps.max_record_len = (uint8)type3_recs_info.text_len ;
              pb_caps->pb_set_caps.gas_caps.used_records = (uint8)(type3_recs_info.records_used);
            }
          }
          break;
        ...
        case PBM_FIELD_OTHERNAME:
          if(fields[i].num_items > 0)
          {
            ...
            {
              /* Subract 2 from max_length as PBM includes 2 bytes for UCS2 null char */
              pb_caps->max_sec_name_len = (uint8)(fields[i].max_length - 2);
            }
        ...
        case PBM_FIELD_EMAIL:
          if(fields[i].num_items > 0)
          {
            ...
            pb_caps->email_cap.max_emails = (uint8)(fields[i].num_items);
            if((fields[i].max_length-2) > QMI_PBM_IF_MAX_SIM_NAME_LEN)
            {
              /* Maximum alpha length on card is 241 as per 3gpp 31.102 */
            ...
            {
              /* Subract 2 from max_length as PBM includes 2 bytes for UCS2 null char */
              pb_caps->email_cap.max_email_len = (uint8)(fields[i].max_length - 2);
            }
          }
          break;
  ...
  if(ecc_val != NULL)
  {
    switch(pbm_field_id)
    {
              ...
              if( pbm_ecc_ptr->card_ecc[i].sess_type == session_type)
              {
                if(pbm_ecc_ptr->card_ecc[i].ecc_count < QMI_PBM_IF_MAX_CARD_ECC)
                {
                  ...
                  memscpy( pbm_ecc_ptr->card_ecc[i].ecc_info[pbm_ecc_ptr->card_ecc[i].ecc_count].ecc ,
                           sizeof(pbm_ecc_ptr->card_ecc[i].ecc_info[pbm_ecc_ptr->card_ecc[i].ecc_count].ecc),
                           ecc_val, ecc_len-1);
        ...
        if(ecc_slot == PBM_SLOT_NONE)
        {
          if(pbm_ecc_ptr->num_hcoded_ecc_entries < QMI_PBM_IF_MAX_HCODED_ECC_EXT)
          {
            ...
            memscpy( pbm_ecc_ptr->hcoded_ecc[pbm_ecc_ptr->num_hcoded_ecc_entries++].ecc,
                     sizeof(pbm_ecc_ptr->hcoded_ecc[pbm_ecc_ptr->num_hcoded_ecc_entries++].ecc),
                     ecc_val, ecc_len-1);
          }
        }
        else if(ecc_slot == PBM_SLOT_1 ||
                ecc_slot == PBM_SLOT_2 ||
                ecc_slot == PBM_SLOT_3)
        {
          UIM_MSG_HIGH_2("qmi_pbm_if_fill_ecc_list for hardcoded ECC slot 0x%x current entries 0x%x",
                         ecc_slot,
                         pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries);

          if(pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries < QMI_PBM_IF_MAX_HCODED_MCC_BASED_ECC)
          {
            /*Copy the ecc value in the ecc cache*/
            pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].hcoded_ecc[pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries].ecc_len = emer_len;
            pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].hcoded_ecc[pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries].category = *emer_categ;
            memscpy( pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].hcoded_ecc[pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries++].ecc,
                     sizeof(pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].hcoded_ecc[pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries++].ecc),
                     ecc_val, ecc_len-1);
          }
        }
        break;
      ...
      case PBM_FIELD_SERVING_MCC_HARDCODED_ECC:
        if(ecc_slot == PBM_SLOT_1 ||
           ecc_slot == PBM_SLOT_2 ||
           ecc_slot == PBM_SLOT_3)
        {
          UIM_MSG_HIGH_2("qmi_pbm_if_fill_ecc_list for serving mcc hardcoded ECC slot 0x%x current entries 0x%x",
                       ecc_slot,
                       pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries);

          if(pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries < QMI_PBM_IF_MAX_HCODED_MCC_BASED_ECC)
          {
          /*Copy the ecc value in the ecc cache*/
          pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].hcoded_ecc[pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries].ecc_len = emer_len;
          pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].hcoded_ecc[pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries].category = *emer_categ;
          memscpy( pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].hcoded_ecc[pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries++].ecc,
                   sizeof(pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].hcoded_ecc[pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries++].ecc),
                   ecc_val, ecc_len-1);
          }
        }
        break;
              ...
              if( pbm_ecc_ptr->ota_ecc[i].sess_type == session_type)
              {
                if(pbm_ecc_ptr->ota_ecc[i].ecc_count < CM_MAX_EMERGENCY_NUM_COUNT)
                {
                  ...
                  memscpy(pbm_ecc_ptr->ota_ecc[i].ecc_info[pbm_ecc_ptr->ota_ecc[i].ecc_count].ecc ,
                          sizeof(pbm_ecc_ptr->ota_ecc[i].ecc_info[pbm_ecc_ptr->ota_ecc[i].ecc_count].ecc),
                          ecc_val, ecc_len-1);
...
/*=========================================================================
  ...
  FUNCTION:  qmi_pbm_if_fill_ext_ecc_list

===========================================================================*/
/*!
    @brief
    Update the extended list of emergency numbers based on the field id

    @return
    errno_enum_type
*/
/*=========================================================================*/
IxErrnoType qmi_pbm_if_fill_ext_ecc_list
(
  uint8                             session_type,
  pbm_field_id_e_type               pbm_field_id,
  uint8                            *emer_categ_ptr,
  char                             *ecc_val_ptr,
  uint16                            ecc_len,
  qmi_pbm_if_ext_ecc_list_type     *pbm_ecc_ptr,
  pbm_slot_type                     ecc_slot,
  uint8                            *sub_service_ptr,
  uint8                             sub_service_len
  
)
{
  IxErrnoType    ret               = E_SUCCESS;
  uint8          iter_session      = 0;
  uint8          emer_len          = 0;
  boolean        is_card_ecc_set   = FALSE;
  boolean        is_ota_ecc_set    = FALSE;

  ASSERT(ecc_val_ptr != NULL);

  /* Ignore Nul char sent in the numbers from PBM */
  emer_len = (uint8)(ecc_len - 1);

  /* Check for max length supported */
  if (emer_len > QMI_PBM_IF_ECC_LEN15)
  {
    return E_FAILURE;
  }
  ...
  if (pbm_ecc_ptr == NULL)
  {
    UIM_MSG_ERR_0("pbm_ecc_pointer is not allocated memory");
    return E_FAILURE;
  }
    ...
    switch (pbm_field_id)
    {
      case PBM_FIELD_NV_ECC:
        if (pbm_ecc_ptr->num_nv_ecc_entries < PBM_NV_EMERGENCY_NUMBERS)
        {
          /*Copy the ECC value in the ecc cache*/
          pbm_ecc_ptr->nv_ecc[pbm_ecc_ptr->num_nv_ecc_entries].ecc_len = emer_len;
          pbm_ecc_ptr->nv_ecc[pbm_ecc_ptr->num_nv_ecc_entries].category= *emer_categ_ptr;
          memscpy(pbm_ecc_ptr->nv_ecc[pbm_ecc_ptr->num_nv_ecc_entries++].ecc, QMI_PBM_IF_ECC_LEN15, ecc_val_ptr, ecc_len-1);
        }
        break;
      case PBM_FIELD_SIM1_ECC:
      case PBM_FIELD_SIM2_ECC:
        if(pbm_ecc_ptr->num_of_card_sessions <= QMI_PBM_IF_MAX_SESSIONS)
        {
          ...
          if (pbm_ecc_ptr->num_of_card_sessions != 0)
          {
            for (iter_session = 0; iter_session < QMI_PBM_IF_MAX_SESSIONS; iter_session++)
            {
              if (pbm_ecc_ptr->card_ecc[iter_session].sess_type == session_type)
              {
                if (pbm_ecc_ptr->card_ecc[iter_session].ecc_count < QMI_PBM_IF_MAX_CARD_ECC)
                {
                  pbm_ecc_ptr->card_ecc[iter_session].ecc_info[pbm_ecc_ptr->card_ecc[iter_session].ecc_count].ecc_len = emer_len;
                  pbm_ecc_ptr->card_ecc[iter_session].ecc_info[pbm_ecc_ptr->card_ecc[iter_session].ecc_count].category = *emer_categ_ptr;
                  memscpy(pbm_ecc_ptr->card_ecc[iter_session].ecc_info[pbm_ecc_ptr->card_ecc[iter_session].ecc_count].ecc ,
                           sizeof(pbm_ecc_ptr->card_ecc[iter_session].ecc_info[pbm_ecc_ptr->card_ecc[iter_session].ecc_count].ecc),
                           ecc_val_ptr, ecc_len-1);
                  pbm_ecc_ptr->card_ecc[iter_session].ecc_count++;
                  is_card_ecc_set = TRUE;
                }
                break;
              }
            }
            if (is_card_ecc_set == FALSE)
            {
              for (iter_session = 0; iter_session < QMI_PBM_IF_MAX_SESSIONS; iter_session++)
              {
                if ( pbm_ecc_ptr->card_ecc[iter_session].sess_type == QMI_PBM_IF_SESSION_MAX)
                {
                  if (pbm_ecc_ptr->card_ecc[iter_session].ecc_count < QMI_PBM_IF_MAX_CARD_ECC)
                  {
                    pbm_ecc_ptr->card_ecc[iter_session].sess_type = session_type;
                    pbm_ecc_ptr->card_ecc[iter_session].ecc_info[pbm_ecc_ptr->card_ecc[iter_session].ecc_count].ecc_len = emer_len;
                    pbm_ecc_ptr->card_ecc[iter_session].ecc_info[pbm_ecc_ptr->card_ecc[iter_session].ecc_count].category = *emer_categ_ptr;
                    memscpy(pbm_ecc_ptr->card_ecc[iter_session].ecc_info[pbm_ecc_ptr->card_ecc[iter_session].ecc_count].ecc ,
                             sizeof(pbm_ecc_ptr->card_ecc[iter_session].ecc_info[pbm_ecc_ptr->card_ecc[iter_session].ecc_count].ecc),
      ...
      case PBM_FIELD_HARDCODED_ECC:
        if (ecc_slot == PBM_SLOT_NONE)
        {
          if (pbm_ecc_ptr->num_hcoded_ecc_entries < QMI_PBM_IF_MAX_HCODED_ECC_EXT)
          {
            /*Copy the ECC value in the ecc cache*/
            pbm_ecc_ptr->hcoded_ecc[pbm_ecc_ptr->num_hcoded_ecc_entries].ecc_len = emer_len;
            pbm_ecc_ptr->hcoded_ecc[pbm_ecc_ptr->num_hcoded_ecc_entries].category = *emer_categ_ptr;
            memscpy( pbm_ecc_ptr->hcoded_ecc[pbm_ecc_ptr->num_hcoded_ecc_entries++].ecc,
                     sizeof(pbm_ecc_ptr->hcoded_ecc[pbm_ecc_ptr->num_hcoded_ecc_entries++].ecc),
                     ecc_val_ptr, ecc_len-1);
          }
                 ...
                 ecc_slot == PBM_SLOT_3)
        {
          UIM_MSG_HIGH_2("qmi_pbm_if_fill_ext_ecc_list for hardcoded ECC slot 0x%x current entries 0x%x",
                         ecc_slot,
                         pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries);
          ...
          if (pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries < QMI_PBM_IF_MAX_HCODED_MCC_BASED_ECC)
          {
            /*Copy the ECC value in the ecc cache*/
            pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].hcoded_ecc[pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries].ecc_len = emer_len;
            pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].hcoded_ecc[pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries].category = *emer_categ_ptr;
            memscpy( pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].hcoded_ecc[pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries++].ecc,
                     sizeof(pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].hcoded_ecc[pbm_ecc_ptr->hcoded_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries++].ecc),
                     ecc_val_ptr, ecc_len-1);
          }
      ...
      case PBM_FIELD_SERVING_MCC_HARDCODED_ECC:
        if (ecc_slot == PBM_SLOT_1 ||
            ...
            ecc_slot == PBM_SLOT_3)
        {
          UIM_MSG_HIGH_2("qmi_pbm_if_fill_ext_ecc_list for serving mcc hardcoded ECC slot 0x%x current entries 0x%x",
                       ecc_slot,
                       pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries);
          ...
          /*Copy the ecc value in the ecc cache*/
            pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].hcoded_ecc[pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries].ecc_len = emer_len;
            pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].hcoded_ecc[pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries].category = *emer_categ_ptr;
            memscpy(pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].hcoded_ecc[pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries++].ecc,
                   sizeof(pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].hcoded_ecc[pbm_ecc_ptr->hcoded_serving_mcc_ecc[ecc_slot - 1].num_hcoded_ecc_entries++].ecc),
                   ecc_val_ptr, ecc_len-1);
          }
      ...
      case PBM_FIELD_NETWORK_ECC:
        if (pbm_ecc_ptr->num_of_ota_sessions <= QMI_PBM_IF_MAX_SESSIONS )
        {
          ...
          if (pbm_ecc_ptr->num_of_ota_sessions != 0)
          {
            for (iter_session=0;iter_session<QMI_PBM_IF_MAX_SESSIONS;iter_session++)
            {
              if (pbm_ecc_ptr->ota_ecc[iter_session].sess_type == session_type)
              {
                if (pbm_ecc_ptr->ota_ecc[iter_session].ecc_count < CM_MAX_EMERGENCY_NUM_COUNT)
                {
                  pbm_ecc_ptr->ota_ecc[iter_session].ecc_info[pbm_ecc_ptr->ota_ecc[iter_session].ecc_count].ecc_len = emer_len;
                  pbm_ecc_ptr->ota_ecc[iter_session].ecc_info[pbm_ecc_ptr->ota_ecc[iter_session].ecc_count].category = *emer_categ_ptr;
                  memscpy(pbm_ecc_ptr->ota_ecc[iter_session].ecc_info[pbm_ecc_ptr->ota_ecc[iter_session].ecc_count].ecc ,
                          sizeof(pbm_ecc_ptr->ota_ecc[iter_session].ecc_info[pbm_ecc_ptr->ota_ecc[iter_session].ecc_count].ecc),
                          ecc_val_ptr, ecc_len-1);
                  pbm_ecc_ptr->ota_ecc[iter_session].ecc_count++;
                  is_ota_ecc_set = TRUE;
                }
                break;
              }
            }
            if (is_ota_ecc_set == FALSE)
            {
              for (iter_session=0; iter_session<QMI_PBM_IF_MAX_SESSIONS; iter_session++)
              {
                if ( pbm_ecc_ptr->ota_ecc[iter_session].sess_type == QMI_PBM_IF_SESSION_MAX)
                {
                  if (pbm_ecc_ptr->ota_ecc[iter_session].ecc_count < CM_MAX_EMERGENCY_NUM_COUNT)
                  {
                    pbm_ecc_ptr->ota_ecc[iter_session].sess_type = session_type;
                    pbm_ecc_ptr->ota_ecc[iter_session].ecc_info[pbm_ecc_ptr->ota_ecc[iter_session].ecc_count].ecc_len = emer_len;
                    pbm_ecc_ptr->ota_ecc[iter_session].ecc_info[pbm_ecc_ptr->ota_ecc[iter_session].ecc_count].category = *emer_categ_ptr;
                    memscpy( pbm_ecc_ptr->ota_ecc[iter_session].ecc_info[pbm_ecc_ptr->ota_ecc[iter_session].ecc_count].ecc ,
                             sizeof(pbm_ecc_ptr->ota_ecc[iter_session].ecc_info[pbm_ecc_ptr->ota_ecc[iter_session].ecc_count].ecc),
  ...
  {
    UIM_MSG_ERR_0("qmi_pbm_if_fill_ext_ecc_list : ecc_val passed is NULL");
    ret = E_FAILURE;
  }

  return ret;
} /* qmi_pbm_if_fill_ext_ecc_list */
...
)
{
  ...
  pbm_slot_type       ecc_slot      = PBM_SLOT_NONE;
  ...
  if(PBM_SUCCESS == pbm_session_enum_rec_init_ext(pb_id, (uint16)PBM_CAT_ECC, pbm_field_id, NULL, 0,0,&handle) )
  {
    ...
    while(PBM_SUCCESS == pbm_session_enum_next_rec_id_ext(&rec_id,handle))
    {
      ...
      /*read the record*/
      if(PBM_SUCCESS == pbm_session_record_read_ext(rec_id,&cat,&num_fields,(uint8*)data_buf, data_buf_size, &ecc_slot))
      {
        emer_categ = 0;
        ...
        for(i = 0; i < num_fields;i++)
        {
          ...
          if(data_buf[i].field_type == PBM_FT_PHONE)
          {
            ...
            if( qmi_pbm_if_fill_ecc_list(session_type, data_buf[i].field_id,
                                         &emer_categ, num, data_buf[i].data_len,
                                         pbm_ecc_ptr, ecc_slot) != E_SUCCESS )
            {
              break;
            }
          }
...
/*===========================================================================
  FUNCTION QMI_PBM_IF_GET_EXT_ECC_FOR_SESSION()

  DESCRIPTION
    Get extended Emergency numbers from PBM depending on the Session & Field ID passed.
    PBM_FIELD_NONE, PBM_FIELD_HARDCODED_ECC are passed as Field ID
    If PBM_FIELD_NONE is passed, get all the ECC (NV, SIM, Hardcoded,
    network). PBM_FIELD_HARDCODED_ECC just gets the hardcoded ECC which
    are applicable when sim is absent.
  ...
  SIDE EFFECTS
    None
===========================================================================*/
IxErrnoType qmi_pbm_if_get_ext_ecc_for_session
(
  uint8                           session_type,
  pbm_field_id_e_type             pbm_field_id,
  qmi_pbm_if_ext_ecc_list_type   *pbm_ecc_ptr,
...
)
{
  ...
  UIM_MSG_HIGH_0("Inside qmi_pbm_if_get_ext_ecc_for_session");
  ...
  if (PBM_SUCCESS == pbm_session_enum_rec_init_ext(pb_id, (uint16)PBM_CAT_ECC, pbm_field_id, NULL, 0, 0, &handle))
  {
    ...
    while (PBM_SUCCESS == pbm_session_enum_next_rec_id_ext(&rec_id,handle))
    {
      ...
      if (PBM_SUCCESS == pbm_session_record_read_ext(rec_id,&cat,&num_fields,(uint8*)data_buf, data_buf_size, &ecc_slot))
      {
        uint8* sub_service_ptr  = NULL;
        uint8  sub_service_len = 0;
        ...
        for (i = 0; i < num_fields; i++)
        {
          ...
          if (data_buf[i].field_type == PBM_FT_PHONE)
          {
            ...
            num = (char *)&(data_buf[i]) + offset;
            /*Store the ECC num in list. */
            if (qmi_pbm_if_fill_ext_ecc_list(session_type, 
                                             data_buf[i].field_id,
                                             &emer_categ,
                                             num,
                                             data_buf[i].data_len,
  ...
  return ret_errno;
} /* qmi_pbm_if_get_ext_ecc_for_session */
...
/*=========================================================================
  ...
  FUNCTION:  qmi_pbm_populate_ext_ecc_list

===========================================================================*/
/*!
    @brief
    Update the local extended list of emergency numbers based on the field id

    @return
    errno_enum_type
*/
/*=========================================================================*/
qmi_pbm_if_ext_ecc_list_type* qmi_pbm_populate_ext_ecc_list(
  void
)
{
  IxErrnoType                    ret_err              = E_FAILURE;
  uint32                         rec_id_list_max_size = 0;
  uint8                          i                    = 0;
  qmi_pbm_if_ext_ecc_list_type  *pbm_ecc_ptr          = NULL;
  pbm_record_id_type            *rec_id_list          = NULL;

  UIM_MSG_MED_0("Inside qmi_pbm_populate_ext_ecc_list");
  
  pbm_ecc_ptr = QMI_PBM_IF_MEM_ALLOC(sizeof(qmi_pbm_if_ext_ecc_list_type));
  ...
  memset(rec_id_list, 0, rec_id_list_max_size * sizeof(pbm_record_id_type));
  memset(pbm_ecc_ptr, 0, sizeof(qmi_pbm_if_ext_ecc_list_type));
  ...
  for (i = QMI_PBM_IF_SESSION_GW_PRIMARY; i < QMI_PBM_IF_SESSION_MAX; i++)
  {
    if (qmi_pbm_if_is_session_type_valid(i))
    {
      if ((ret_err = qmi_pbm_if_get_ext_ecc_for_session( i, PBM_FIELD_NONE , pbm_ecc_ptr , rec_id_list)) != E_SUCCESS)
      {
        UIM_MSG_ERR_1("Failure in getting ECC from PBM for session 0x%x" , i);
      }
    }
...
{
  qmi_pbm_if_ecc_list_type   *pbm_ecc_ptr=NULL;
  qmi_pbm_if_ext_ecc_list_type      *pbm_ext_ecc_ptr   = NULL;
  ...
  if (params_ptr->data.ind_reg.send_ext_emer_list_ind)
  {
    /* send the list of applicable emergency numbers */
    UIM_MSG_MED_0("Inside qmi_pbm_if_indication_register : Sending extended Emergency list Ind");
    
    pbm_ext_ecc_ptr = qmi_pbm_populate_ext_ecc_list();
 
    if (pbm_ext_ecc_ptr != NULL)
    {
      qmi_pbm_if_send_ind(QMI_PBM_IF_CMD_EXT_EMERGENCY_LIST_IND, pbm_ext_ecc_ptr, sizeof(qmi_pbm_if_ext_ecc_list_type));
    }
  }
...
)
{
  ...
  /* Check for the PB Ready */
  if(!qmi_pbm_if_is_pb_ready(data->session_type, data->pb_type))
  {
     qmi_pbm_if_send_response(params_ptr->header.handle,params_ptr->header.cmd,QMI_ERR_PB_NOT_READY,NULL,0,FALSE);
     return;
  }
    ...
    switch(data_buf[k].field_id)
    {
      ...
      case PBM_FIELD_NAME:
                ...
                cmd_type == QMI_PBM_IF_CMD_INTERNAL_HIDDEN_RECORDS_READ_EXT_IND)
        {
          ...
          rec_data->name_info.len = (uint8)MIN(QMI_PBM_IF_MAX_TEXT_EXT_LEN,(data_buf[k].data_len-2)/2);
        }
        else if(cmd_type == QMI_PBM_IF_CMD_INTERNAL_RECORD_READ_IND ||
                cmd_type == QMI_PBM_IF_CMD_INTERNAL_HIDDEN_RECORDS_READ_IND)
        {
          /* Exclude the terminating nul char(of 2 bytes) */
          rec_data->name_info.len = (uint8)(data_buf[k].data_len-2);
        }
      ...
      case PBM_FIELD_OTHERNAME:
                ...
                cmd_type == QMI_PBM_IF_CMD_INTERNAL_HIDDEN_RECORDS_READ_EXT_IND)
        {
          ...
          rec_data->sname_info.len =
             (uint8)(MIN(QMI_PBM_IF_MAX_TEXT_EXT_LEN,(data_buf[k].data_len-2)/2));
        }
        else if(cmd_type == QMI_PBM_IF_CMD_INTERNAL_RECORD_READ_IND ||
                cmd_type == QMI_PBM_IF_CMD_INTERNAL_HIDDEN_RECORDS_READ_IND)
        {
          /* Exclude the terminating nul char(of 2 bytes) */
          rec_data->sname_info.len = (uint8)(data_buf[k].data_len-2);
        }
        /* Second Name will be in UCS2. Copy as it is */
      ...
      case PBM_FIELD_EMAIL:
                ...
                cmd_type == QMI_PBM_IF_CMD_INTERNAL_HIDDEN_RECORDS_READ_IND)
        {
          ...
          rec_data->email_data[email_cnt].len = (uint8)(data_buf[k].data_len-2);
        }

        /* eMAIL will be in UCS2. Copy as it is */
...
}/* qmi_copy_pbm_rec_to_qmi_ind */


/*===========================================================================
  FUNCTION qmi_copy_pbm_rec_to_qmi_undecoded_ind ()

  DESCRIPTION
    Copy the PBM record content to QMI READ undecoded indication to clients.
...
)
{
    ...
    for(k=0;k<num_fields;k++)
    {
      ...
      switch(data_buf[k].field_id)
      {
        ...
        case PBM_FIELD_NAME:
          ...
          {
            rec_data->name_info.len = (uint8)(data_buf[k].data_len-1);
          }

          memscpy(rec_data->name_info.text, sizeof(rec_data->name_info.text),
        ...
        case PBM_FIELD_OTHERNAME:
          ...
          {
            rec_data->sname_info.len = (uint8)(data_buf[k].data_len-1) ;
          }

          memscpy(rec_data->sname_info.text, sizeof(rec_data->sname_info.text),
                  field_val, data_buf[k].data_len-1);
          break;
        ...
        case PBM_FIELD_EMAIL:
          if( email_cnt < QMI_PBM_IF_MAX_EMAIL )
          {
            ...
            {
              rec_data->email_data[email_cnt].len = (uint8)(data_buf[k].data_len-1) ;
            }
          }
  ...
  rec_data->email_cnt  = email_cnt;
  return QMI_RESULT_SUCCESS;
} /* qmi_copy_pbm_rec_to_qmi_undecoded_ind */
...
)
{
  ...
  for(rec_id = intr_ind.start_id;rec_id <= intr_ind.end_id;rec_id++)
  {
    ...
    if( QMI_PBM_IF_READ_RECORDS_MAX == i )
    {
      ...
      if(qmi_pbm_encode_and_send_msg(pbm_if_data_ptr) == TRUE && !is_last_ind)
      {
...
)
{
  ...
  for(pbset_id = intr_ind.start_id-1; pbset_id < intr_ind.end_id; pbset_id++)
  {
    ...
    for(l_field = 0; l_field < PBM_MAX_USIM_FIELDS_IN_PB_SET; l_field++)
    {
      ...
      {
        l_qmi_usim_caps->field_id = (uint32)qmi_pbm_if_map_field_to_qmi(l_pbm_usim_caps->field_id);
        l_qmi_usim_caps->mapping_type = (uint32)qmi_pbm_if_map_mapping_to_qmi(l_pbm_usim_caps->mapping_type);
        l_qmi_usim_caps->num_of_records = (uint16)l_pbm_usim_caps->num_of_records;
        l_qmi_usim_caps->num_free_records = (uint16)(l_pbm_usim_caps->num_of_records - l_pbm_usim_caps->records_in_use);
        l_qmi_usim_caps->record_length = (uint16)l_pbm_usim_caps->record_length;
        UIM_MSG_HIGH_3("pbset_id :0x%x l_qmi_usim_caps->field_id :0x%x num_of_records :0x%x",
                        pbset_id+1, l_field, l_qmi_usim_caps->num_of_records);
      }
    }
    i++;
    if(QMI_PBM_IF_READ_RECORDS_MAX == i)
...
)
{
  ...
  uint32                         rec_id                   = 0;
  uint32                         sub_fields_ptr_size      = 0;
  uint8                         *fields_ptr               = NULL;
  uint8                         *sub_fields_ptr           = NULL;
...

  {
    ...
    pbm_field_s_type         *ptyped_sub_fields_ptr[QMI_PBM_IF_MAX_AD_NUM] = {NULL};
...
)
{
  ...
  if(pbm_is_card_present_in_slot((uint8) pb_info.slot_id) == FALSE)
  {
    qmi_pbm_if_send_response(params_ptr->header.handle, params_ptr->header.cmd, QMI_ERR_NO_SIM, NULL, 0, FALSE);
    return;
  }
...
)
{
  ...
  else
  {
    ...
    if(in_data_ptr->number_info_valid)
    {
    ...
    else if(in_data_ptr->name_info_valid)
    {
  ...
  if(err_val == QMI_ERR_NONE)
  {
    ...
    if(in_data_ptr->name_info_valid)
    {
...
/*===========================================================================
  FUNCTION QMI_PBM_IF_GET_EXT_EMERGENCY_LIST()

  DESCRIPTION
    Return the consolidated extended list of emergency.numbers
  ...
  SIDE EFFECTS
    None
===========================================================================*/
void qmi_pbm_if_get_ext_emergency_list
(
  qmi_pbm_if_info_item_s *const params_ptr
)
{
  ...
  UIM_MSG_HIGH_0("qmi_pbm_if_get_ext_emergency_list" );

  pbm_ecc_ptr = qmi_pbm_populate_ext_ecc_list();
  ...
  if (pbm_ecc_ptr != NULL)
  {
    qmi_pbm_if_send_response(params_ptr->header.handle,
                             ...
                             QMI_ERR_NONE,
                             (void*)pbm_ecc_ptr,
                             sizeof(qmi_pbm_if_ext_ecc_list_type),
                             FALSE);
  }
  else
  {
...
} /* qmi_pbm_if_get_ext_emergency_list */
               ...
               switch(data_buf[k].field_id)
               {
                 case PBM_FIELD_NAME:
                   if(g_cnt < QMI_PBM_IF_MAX_GRP_RECS)
                   {
                     ...
                     /* Exclude the terminating nul char(of 2 bytes) */
                     grp_rsp->sess_grps[s_cnt].grp_data[g_cnt].grp_name_len = (uint8)(data_buf[k].data_len - 2);
...
)
{
  ...
  uint16                                   category        = 0;
  void                                    *field_val       = NULL;
  qmi_pbm_if_get_all_aas_resp_s           *aas_rsp         = NULL;
  boolean                                  card_present    = FALSE;
  qmi_pbm_if_info_item_s                  *pbm_if_data_ptr = FALSE;
                  ...
                  case PBM_FIELD_NAME:
                    if(a_cnt < QMI_PBM_IF_MAX_AAS_RECS)
                    {
                      ...
                      memscpy(aas_rsp->sess_aas[s_cnt].aas_data[a_cnt].alpha,
                              ...
                              field_val, (data_buf[k].data_len-2));
                      /* Exclude the terminating nul char(of 2 bytes) */
                      aas_rsp->sess_aas[s_cnt].aas_data[a_cnt].aas_len = (uint8)(data_buf[k].data_len - 2);
...
)
{
  ...
  if(pbm_is_card_present_in_slot((uint8) pb_id.slot_id) == FALSE)
  {
    qmi_pbm_if_send_response(params_ptr->header.handle, params_ptr->header.cmd, QMI_ERR_NO_SIM, NULL, 0, FALSE);
    return;
  }
...
)
{
  ...
  if( (notify_data->session_data.pb_id.device_type == PBM_ECC) || (notify_data->session_data.pb_id.device_type == PBM_ECC_OTHER) )
  {
   ...
   pbm_ext_ecc_ptr = qmi_pbm_populate_ext_ecc_list();
    ...
    if ( pbm_ecc_ptr != NULL )
      {
        UIM_MSG_HIGH_1("Inside qmi_pbm_if_handle_pb_ready  Sending emergency list Ind 0x%x" , pbm_ecc_ptr);
        qmi_pbm_if_send_ind(QMI_PBM_IF_CMD_EMERGENCY_LIST_IND, pbm_ecc_ptr , sizeof(qmi_pbm_if_ecc_list_type));
    }
    if (pbm_ext_ecc_ptr != NULL)
    {
      UIM_MSG_HIGH_1("Inside qmi_pbm_if_handle_pb_ready  Sending ext emergency list Ind 0x%x" , pbm_ext_ecc_ptr);
      qmi_pbm_if_send_ind(QMI_PBM_IF_CMD_EXT_EMERGENCY_LIST_IND, pbm_ext_ecc_ptr , sizeof(qmi_pbm_if_ext_ecc_list_type));
    }
  }
  else
...
)
{
  ...
  int                              ind_len     = 0;
  void                            *ind_data    = NULL;
  qmi_pbm_if_cmd_e_type            ind_type    = QMI_PBM_IF_CMD_MAX;
  pbm_phonebook_type               pb_id;
     ...
     (operation != QMI_PBM_IF_PB_REC_OP_DEFAULT))
  {
    if(PBM_GRPNAME == pb_id.device_type)
    {
      ...
      gas_ind->session_type = (uint8)sess_type;
      gas_ind->operation = operation;
      gas_ind->gas_id = (uint8)rec_id;
...
)
{
  ...
  switch (pbm_if_data_ptr->header.cmd)
  {
    ...
    case QMI_PBM_IF_CMD_GET_EMERGENCY_LIST:
      ...
      break;
    case QMI_PBM_IF_CMD_GET_EMERGENCY_LIST_EXT:
      qmi_pbm_if_get_ext_emergency_list(pbm_if_data_ptr);
      break;
    case QMI_PBM_IF_CMD_GET_PB_STATE:
