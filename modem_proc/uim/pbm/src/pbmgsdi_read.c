...
$Header: //components/rel/uim.mpss/6.1.0/pbm/src/pbmgsdi_read.c#1 $


when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/08/18   nr      Initialize sim_text field before parsing AAS and GAS data
04/19/17   bcho    Moving QCRIL ECC DB to PBM
10/03/16   dd      Removed lint comments
06/06/16   sp      Use correct cache type while reading EXT1 cache for ANR record 
05/23/16   sp      Fix Null check in pbm_mmgsdi_process_anr_data
...
#define PBM_IMSI_BYTE_1          0x01
#define PBM_IMSI_BYTE_2          0x02
#define PBM_IMSI_LOW_BYTE        0x0F
#define PBM_IMSI_HIGH_BYTE       0xF0
#define PBM_IMSI_MIN_LEN         0x03

const uint8 pbm_min_to_num[] =
{ 1, 2, 3,  4, 5, 6, 7, 8, 9, 0 };
...
{
  pbm_fixed_mdn_rec_s_type *fixed_mdn_record     = NULL;
  char                      number[PBM_NUM_SIZE] = {0}; /* + and null for string */
  PB_CACHE                 *cache                = (PB_CACHE *)
  ...
  /*Text field with 0x00 is interpreted as @ in GSM encoding, hence 
    the memset of text field should be with 0xFF */
   ...
   switch (sim_filetype)
   {
      ...
      case MMGSDI_CYCLIC_FILE:
         mmgsdi_retval = mmgsdi_session_read_record (session_id,
                                                     ...
                                                     (mmgsdi_len_type)required_data_len,
                                                     async_cb,
                                                     client_ref);
         break;
  ...
  const mmgsdi_cnf_type  *data_ptr)
{
  ...
  {
    UIM_MSG_ERR_1("PBM stopping init of SIM %d", session_type);
    pbm_free_record((int) client_ref);
    return;
  }
...
)
{
  ...
  if (status == MMGSDI_SUCCESS &&
      (unsigned int)cache->ext_rec_length == data_ptr->read_cnf.read_data.data_len)
  {
  ...
  else if(pbm_buffers[client_ref].record.file_id == PBM_FILE_ADN)
  {
    /* This else is to handle buggy card scenarios only when adn is master file*/
    if(data_ptr->read_cnf.accessed_rec_num <= cache->num_of_rec)
    {
      /* Unlink the extension record when record read of the extension file is not found
         or got error while reading on the card but linked to the main record */
      cache->in_use_array[data_ptr->read_cnf.accessed_rec_num].index      = 0;
      cache->in_use_array[data_ptr->read_cnf.accessed_rec_num].uim_device = PBM_FILE_NONE;
    }

    if(status == MMGSDI_NOT_FOUND)
    {
      ret_val = PBM_SUCCESS;
    }
    else
    {
      int                 record_num    = 0;
      pbm_file_type       file_id       = pbm_buffers[client_ref].record.file_id;
      PB_CACHE           *adn_cache_ptr = NULL;

      /* we will keep on getting the next used record until 
         we get the one eligible to be added in cache */
      record_num = pbm_get_next_used_record(session_type,
                                            pbm_buffers[client_ref].record.index);

      /* Free the main record and proceed with reading of remaining records 
         if the extension record read is failed */
      pbm_free_record((int) client_ref);

      adn_cache_ptr = (PB_CACHE *)pbm_file_id_to_cache(session_type,
                                                     pbm_buffers[client_ref].m_file_id);

      if (adn_cache_ptr == NULL ||
          ((record_num <= adn_cache_ptr->num_of_records) &&
           (pbm_sim_read(session_type, file_id, record_num, pbm_sim_read_cb) == PBM_SUCCESS)))
      {
        return;
      }

      (void)pbm_uim_cache_update (adn_cache_ptr, record_num, file_id, session_type);
      return;
    }
  }
  ...
  const mmgsdi_cnf_type    *data_ptr)
{
  ...
  if (status == MMGSDI_SUCCESS &&
      (unsigned int)iap_cache->iap_rec_len == data_ptr->read_cnf.read_data.data_len)
  {
  ...
  const mmgsdi_cnf_type      *data_ptr)
{
  ...
  {
    pbm_free_record((int) client_ref);
    UIM_MSG_ERR_2("No Cache found for session_type 0x%x, file_id 0x%x\n",
  ...
  if (status == MMGSDI_SUCCESS &&
      (unsigned int)cache->record_length[pb_set] == data_ptr->read_cnf.read_data.data_len)
  {
    if(file_id == PBM_FILE_GAS || file_id == PBM_FILE_AAS)
    {
      memset(&(pbm_buffers[client_ref].record.data.num_text),
             0x00,
             sizeof(pbm_buffers[client_ref].record.data.num_text));
      memset(pbm_buffers[client_ref].record.data.num_text.sim_text,
             0xFF,
    ...
    else
    {
      ...
      {
        UIM_MSG_ERR_2("PBM could not find a valid extension record for 0x%x from cache 0x%x",
                      file_id, cache);
      }
    }
  }
...
)
{
  ...
  {
    if (status != MMGSDI_SUCCESS ||
        ((unsigned int)usim_cache->record_length != data_ptr->read_cnf.read_data.data_len))
    {
    ...
    if (ext_cache == NULL || ext_cache->file_id == PBM_FILE_NONE)
    {
...
)
{
  ...
  text_length = usim_cache->record_length;
...
)
{
  ...
  /* if Additional Number Record Identifier is greater than number of record in EF AAS
   we have to consider that as invalid record .Also if Additional Number Record Identifier is OxFF
   it means it is free record In both cases we should ignore the record and proceed with the next one*/
  if(aas_cache_ptr[session_type] != NULL &&
     data_buffer[0] > aas_cache_ptr[session_type]->num_of_records )
  {
    return PBM_SUCCESS;
  }
...
)
{
  ...
  for (i = 0; i < MIN(usim_cache->record_length,PBM_MAX_NUM_GROUPS); i++)
  {
    ...
    if (data_buffer[i] != 0xFF &&
        ...
        data_buffer[i] < grpname_cache_ptr[session_type]->num_of_records)
    {
...
)
{
  ...
  pbm_session_enum_type  pbm_session_id    = PBM_SESSION_DEFAULT;
  ...
  if ( MMGSDI_SUCCESS != status || MMGSDI_SRV_AVAILABLE_CNF != data_type )
  {
    return;
  }
...
/*===========================================================================
FUNCTION PBM_ECC_MMGSDI_READ_CB

DESCRIPTION
  This function is a callback function to read IMSI/IMSI_M files
...
)
{
  ...
  if(cnf_type != MMGSDI_READ_CNF || 
     status != MMGSDI_SUCCESS)
  {
    return;
  }

  file_enum = (mmgsdi_file_enum_type)data_ptr->response_header.client_data;
  session_type = pbm_mmgsdi_session_to_pbm_session(data_ptr->response_header.session_id);

  /* Retrieve MCC value form read response */
  if(file_enum == MMGSDI_USIM_IMSI ||
     file_enum == MMGSDI_GSM_IMSI)
  {
    source = PBM_ECC_MCC_SOURCE_IMSI;

    if(data_ptr->read_cnf.read_data.data_len >= PBM_IMSI_MIN_LEN)
    {
      uint8 mcc_array[3] = {0};

      /* Decode MCC digits in ASCII */
      mcc_array[0] = (((data_ptr->read_cnf.read_data.data_ptr[PBM_IMSI_BYTE_1] & PBM_IMSI_HIGH_BYTE) >> 4) + '0');
      mcc_array[1] = ((data_ptr->read_cnf.read_data.data_ptr[PBM_IMSI_BYTE_2] & PBM_IMSI_LOW_BYTE) + '0');
      mcc_array[2] = (((data_ptr->read_cnf.read_data.data_ptr[PBM_IMSI_BYTE_2] & PBM_IMSI_HIGH_BYTE) >> 4) + '0');

      updated_mcc = (uint16)((mcc_array[0] - 0x30) * 100 + (mcc_array[1] - 0x30) * 10 + (mcc_array[2] - 0x30));
  ...
  else if(file_enum == MMGSDI_CSIM_IMSI_M||
          file_enum == MMGSDI_CDMA_IMSI_M)
  {
    source = PBM_ECC_MCC_SOURCE_IMSI_M;

    if(data_ptr->read_cnf.read_data.data_len != MMGSDI_CDMA_IMSI_LEN)
    {
      return;
    }

    min = (uint16)(((data_ptr->read_cnf.read_data.data_ptr[9] & 0x03) << 8) +
                     data_ptr->read_cnf.read_data.data_ptr[8]);

    if(min <= 999)
    {
      /* Converting Mobile Country Code */
      updated_mcc = (pbm_min_to_num[min/100]) * 100;
      min %= 100;
      updated_mcc = updated_mcc + (pbm_min_to_num[min / 10]) * 10;
      updated_mcc = updated_mcc + pbm_min_to_num[min % 10];
    }
  ...
  current_mcc = pbm_ecc_get_current_mcc(pbm_session_info[session_type].slot_id);

  /* Update MCC value*/
  pbm_ecc_update_mcc(PBM_MMGSDI_SLOT_TO_PBM_SLOT(pbm_session_info[session_type].slot_id), 
                     source,
                     updated_mcc);
