...
)
{
  ...
  while ( buf_ptr != NULL )
  {
    ...
    {
      UIM_MSG_LOW_2( "Found cmd list entry :(%d), handle %d",buf_ptr->pub.cmd, hdl );
      return buf_ptr;
    }
    else
    {
...
)
{
  ...
  UIM_MSG_LOW_1( "Command (%d) added to list ", cmd);
...
)
{
  ...
  status = qmi_pbm_cmd_list_new_all( (uint8)cmd, hdl, state, sub_id );
...
)
{
  ...
  if (buf_ptr != NULL)
  {
    ...
    UIM_MSG_LOW_2( "Update sub id field of cmd list entry : handle %d, sub id %lu",
