...
$Header: //components/rel/uim.mpss/6.1.0/pbm/src/pbm_conversion.c#2 $
...
01/28/16   nr      Fix for extension characters in 0x81 encoding
30/12/15   dd      Place braces for all code blocks
12/09/15   lm      Remove compiler warnings
09/02/15   stv     Fix for extension characters in 0x82 encoding
...
static boolean pbm_find_gsm_ext(uint8 second_octet, uint16 *new_value);

static void pbm_convert_gsm7_to_unicode(uint8 gsm7_char, uint16 *ucs2_char);
...
/*===========================================================================
FUNCTION pbm_find_gsm_ext

DESCRIPTION
  boolean pbm_find_gsm_ext(uint8 second_octet, uint16 *new_value)
...
/*===========================================================================
FUNCTION pbm_convert_gsm7_to_unicode

DESCRIPTION
  void pbm_convert_gsm7_to_unicode(uint8 gsm7_char, uint16 *ucs2_char)
...
SIDE EFFECTS
  None.
===========================================================================*/
static boolean pbmconvert_convert_one_ucs2_to_gsm7(
  uint16  ucs2_char,
  uint8  *gsm7_ptr,
  uint16 *gsm7_len
)
{
  uint32            index          = 0;                // Current index being searched.
  uint16            output_size    = 0;                // Max size we can write.
  CONVERSION_TABLE *search_result  = NULL;             // Bsearch result.
  CONVERSION_TABLE  key            = {0};              // Bsearch Key.
  ...
  return FALSE;
} /* pbmconvert_convert_one_ucs2_to_gsm7 */
...
SIDE EFFECTS
  None.
===========================================================================*/
static boolean pbmconvert_convert_ucs2_to_gsm7(
  uint16  in_ucs2,
  uint8 **out_octets,
  uint16 *out_len,
  uint16  out_base
)
{
  ...
  return FALSE;
}/* pbmconvert_convert_ucs2_to_gsm7 */
...
SIDE EFFECTS
  None.
===========================================================================*/
static boolean pbmconvert_convert_ucs2_to_ucs2_80 (
  uint16  in_ucs2,
  uint8 **out_octets,
  uint16 *out_len,
  uint16  out_base
)
{
  ...
  if (*out_len < 2)
  {
   ...
   return FALSE;
} /* pbmconvert_convert_ucs2_to_ucs2_80 */
...
SIDE EFFECTS
  None.
===========================================================================*/
static boolean pbmconvert_convert_ucs2_to_ucs2_81 (
  uint16  in_ucs2,
  uint8 **out_octets,
  uint16 *out_len,
  uint16  out_base
)
{
  ...
  if ((*out_len == 0) || (octets_written == 0))
  {
   ...
   return FALSE;
} /* pbmconvert_convert_ucs2_to_ucs2_81 */
...
SIDE EFFECTS
  None.
===========================================================================*/
static boolean pbmconvert_convert_ucs2_to_ucs2_82 (
  uint16  in_ucs2,
  uint8 **out_octets,
  uint16 *out_len,
  uint16  out_base
...
}/* pbmconvert_convert_ucs2_to_ucs2_82 */
...
SIDE EFFECTS
  None.
===========================================================================*/
static boolean pbmconvert_sim_ucs2_to_ucs2(
  uint8  **in_string,
  uint16  *in_len,
  uint16 **out_octets,
  uint16   ucs2_base
...
SIDE EFFECTS
  None.
===========================================================================*/
static boolean pbmconvert_sim_gsm7_to_ucs2(
  uint8  **in_string,
  uint16  *in_len,
  uint16 **out_octets,
  uint16   ucs2_base
...
{
  uint8 curr_value = 0;             /* Current octet being looked at. */
  ...
  if ((*in_len >= 1) && ((curr_value = **in_string) != 0xFF))
  {
    *in_string += 1;        // We looked at one character.
    ...
    if (curr_value == 0x001B)  /* Extension. */
    {
      if (*in_len >= 2) /* If there is a next octet, look for that. */
      {
        ...
        *in_string += 1;
        ...
        if (pbm_find_gsm_ext(second_octet, *out_octets))
        {
        ...
        else
        {
          ...
          if (second_octet == 0x1b)
          {
            ...
            pbm_convert_gsm7_to_unicode(0x1b, (uint16 *) *out_octets);
            *out_octets+=1;
            pbm_convert_gsm7_to_unicode(0x1b, (uint16 *) *out_octets);
            *out_octets+=1;
            *in_len-=2;
            ...
            if (*in_len >= 1)
            {
              ...
              pbm_convert_gsm7_to_unicode((**in_string & 0x7f),
                                      (uint16 *) *out_octets);
              *in_string += 1;
              *out_octets+=1;
              *in_len-=1;
          ...
          else
          {
            ...
            pbm_convert_gsm7_to_unicode(0x1b, (uint16 *) *out_octets);
            *out_octets+=1;
            pbm_convert_gsm7_to_unicode((second_octet & 0x7f),
                                    (uint16 *) *out_octets);
            *out_octets+=1;
    ...
    else
    {
      ...
      pbm_convert_gsm7_to_unicode(curr_value, (uint16 *) *out_octets);
      *out_octets+=1;
      *in_len-=1;
    }
  }
...
SIDE EFFECTS
  None.
===========================================================================*/
static boolean pbmconvert_sim_ucs2_halfpage_to_ucs2(
  uint8  **in_string,
  uint16  *in_len,
  uint16 **out_octets,
  uint16   ucs2_base
)
{
  ...
  return TRUE;
} /* pbmconvert_sim_ucs2_halfpage_to_ucs2 */
...
SIDE EFFECTS
  None.
===========================================================================*/
static boolean pbmconvert_convert_one_ucs2_to_pbm8(
  uint16 ucs2_char,
  uint8 *pbm8_ptr
)
{
  CONVERSION_TABLE *search_result  = NULL;
  CONVERSION_TABLE  key            = {0};
  ...
  return FALSE;
} /* pbmconvert_convert_one_ucs2_to_pbm8 */
...
SIDE EFFECTS
  None.
===========================================================================*/
boolean pbmconvert_ucs2_to_pbm8(
  uint16 *ucs2_str,
  uint16  ucs_str_len,
  uint8  *pbm8_str,
  uint16  pbm8_str_len
)
{
  ...
  uint16  *ucs2_ptr  = NULL;   /* Pointer to ucs2_string. */
  uint8   *pbm8_ptr  = NULL;   /* Pointer to the pbm8 string. */
  boolean  ret_val   = TRUE;   /* Can every character be converted. */
  uint8    pbm8_char = 0;      /* Character that it maps to. */
...
SIDE EFFECTS
  None.
===========================================================================*/
boolean pbmconvert_pbm8_to_ucs2(
  const uint8 *pbm8_str,
  uint16       pbm8_str_len,
  uint16      *ucs2_str, 
  uint16       ucs2_str_len
)
{
  ...
  boolean ret_val   = TRUE;  /* Can every character be converted. */
  uint16  ucs2_char = 0;       /* Character that it maps to. */
...
{
  const void  *a         = NULL;        /* a traverses s1 */
  const void  *b         = NULL;        /* b traverses s2 */
  uint16       c1        = 0;
  uint16       c2        = 0;
  pfsource_fn  src_fn1;
  pfsource_fn  src_fn2;
