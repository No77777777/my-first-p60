...
$Header: //commercial/MPSS.JO.1.2.r3/Main/modem_proc/uim/pbm/src/
pbm_common_lib.c#1 $
...
)
{
  ...
  pbm_device_type              pbm_default_pb_ids[] = { PBM_EFS, PBM_SDL, PBM_ADN,
                                                        PBM_FDN,PBM_SDN};
...
)
{
  ...
  *num_fields_ptr = 0;
  ...
  do
  {
    ...
    if(ecc_slot_ptr)
    {
      *ecc_slot_ptr   = cache->pb_cache_array[location]->slot;
    }
...
{
  return pbm_session_record_read_ext(rec_id,
                                     category_ptr,
                                     num_fields_ptr,
                                     data_buf,
                                     data_buf_size,
