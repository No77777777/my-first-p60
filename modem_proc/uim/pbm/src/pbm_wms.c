...
$Header: //components/rel/uim.mpss/6.1.0/pbm/src/pbm_wms.c#1 $
...
)
{
  ...
  /* Don't query WMS ready in case of 1x_teritary and proceed
   * with initlization as WMS don't open sessions on 1x_teritary
   */
  if(pbm_session_type == PBM_SESSION_LPB_1X_TERTIARY)
  {
...
)
{
  ...
  UIM_MSG_HIGH_7("PBM Receieved WMS ready wms_session 0x%x TxId 0x%x PBM Last TxId 0x%x pbm_session 0x%x pbm_wms_init_finished 0x%x pbm_session_subs_ready 0x%x pbr_status 0x%x",
