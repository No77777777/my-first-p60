...
Copyright (c) 2002 - 2017 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/pbm/src/pbmlib.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/20/17   tq      Fix KW warning
...
05/16/16   nr      Adding equivalent check for compressed field_type for PBM_FT_PHONE
04/18/16   sp      ECC only support in PBM
04/15/16   nr      Enhance write algo to check across all pb_sets for USIM files
04/13/16   sp      Reduce global memory used by PBM
03/14/16   nr      Check number for emergency on both subs when UE is in SS mode
02/01/16   nr      Skip FDN check IMS SIP URI for conference call
...
07/06/15   kr      Fix for potential pointer overflow in pbm_session_record_validate
...
)
{
  ...
  if(pb_id.device_type == PBM_ECC)
  {
    ...
    do
    {
      ...
      if ((ret = pbm_check_pb_ready(pb_id)) != PBM_SUCCESS && ret != PBM_ERROR_SYNC)
      {
        break;
      }
...
)
{
  ...
  *num_fields_ptr = 0;
        ...
        case PBM_ECC_OTHER:
          ...
          if ( NULL != (cache = pbm_device_id_to_cache(session_type, PBM_ECC_OTHER)))
          {
            if (cache->status == PBM_STATUS_READY)
            {
              ...
              {
                location -= (uint16)cache->num_of_records;
              }
            }
          }
          break;
  ...
  do
  {
    ...
    {
      //For circular buffers we need to so some special translation for location
      location = (uint16)PBM_CIRC_CACHE_NTHLOC(cache, location);
    }
  ...
  )
{
  ...
  do
  {
    ...
    {
      if(my_data_buf!=NULL && my_data_buf[l_j].field_id == PBM_FIELD_HIDDEN &&
         pbm_get_field_data_ptr(my_data_buf,l_j) != NULL && *((byte *) pbm_get_field_data_ptr(my_data_buf,l_j)) == TRUE &&
         (pbc_cache_ptr[session_type] == NULL || pbc_cache_ptr[session_type]->num_of_records < location))
      {
        ret = PBM_ERROR_MEM_FULL;
        break;
      }
...
)
{
  ...
  for (cidx = 0; cidx < caches_used; ++cidx)
  {
    ...
    for ( i = 1; i <= pb_cache->num_of_records; i++ )
    {
      ...
      {
        if (pbm_match_found(my_cat, field_id, data_ptr, data_size, flags,
                                                 ...
                                                 pbm_extended_fileinfo_s_type *info)
{
  ...
  uint32 i;
   ...
   } else
   {
      ...
      {
         rex_leave_crit_sect(&pbm_crit_sect);
         return pbm_check_pb_ready(pb_id);
      }
      session_type = pbm_mapping_to_session (pb_id.pb_category,(pbm_slot_type) pb_id.slot_id, pb_id.prov_type);
      info->number_of_records =  cache->num_of_records;
...
)
{
  ...
  for(i=0; i < PBM_MAX_NUM_PB_SETS; i++)
  {
    if(cache->records_in_pb_set[i] != 0)
    {
      ...
      info->num_pb_sets++;
      info->adn_caps[i].max_adn_records = (uint8)cache->records_in_pb_set[i];
      info->adn_caps[i].max_text_len = (uint8)cache->text_length[i];
      ...
      for ( k = 0; k < field_info->num_fields; k++)
      {
        ...
        {
          info->adn_caps[i].max_num_len = (uint8)field_info->info[k].max_length ;
          break;
        }
      }
...
)
{
  ...
  {
    if ( ( pb_ready_ret_val = pbm_check_pb_ready(pb_id) ) != PBM_SUCCESS)
    {
  ...
  int                     num_fields)
{
  ...
  int                      group                      = INVALID_GROUP;
  int                      x                          = 0;
  int                      j                          = 0;
  int                      counts[PBM_MAX_NUM_FIELDS] = {0};
  boolean                  record_required_fields[PBM_MAX_REQUIRED_FIELDS] = {TRUE};
  pbm_field_s_type        *ptyped_fields    = (pbm_field_s_type *)data_buf_ptr;
  pbm_record_id_type       pbm_cache_rec_id = rec_id;
  char                    *num_ptr          = NULL;
  pbm_field_s_type        *sub_fields_ptr   = NULL;
  pbm_return_type          ret              = PBM_SUCCESS;
  pbm_internal_field_info *field_info_ptr   = NULL;
  pbm_session_enum_type   session_type      = PBM_SESSION_DEFAULT;

  UIM_MSG_HIGH_7("pbm_session_record_validate::slot 0x%x device_type 0x%x category 0x%x rec_id: 0x%x category: 0x%x data_buf_size: 0x%x num_fields: 0x%x",
                  pb_id.slot_id, pb_id.device_type, pb_id.pb_category, rec_id, cat, data_buf_size, num_fields);
  ...
  /* Parameter check */
  ret = pbm_record_write_check(pb_id,rec_id,
                               (pbm_field_s_type *) data_buf_ptr,
                               num_fields);
  ...
  if (rec_id != PBM_SESSION_GET_FIRST_AVAILABLE)
  {
    ...
    if (pb_cache->circular)
    {
      ...
      location = (uint16)PBM_CIRC_CACHE_NTHLOC(pb_cache, location);
  ...
  if (num_fields)
  {
    ...
    calculated_size = pbm_calculate_size_from_fields((pbm_field_s_type*)data_buf_ptr,
                                                      num_fields);
    UIM_MSG_HIGH_1("Calculated Record size. is %d", calculated_size);

    if ((data_buf_size < calculated_size) ||
        (calculated_size > PBM_MAX_RECORD_SIZE))
    {
      return PBM_ERROR_INVALID_REC;
    }
  }
  ...
  if ((cat == PBM_CAT_ERROR) ||
      (cat != PBM_CAT_NONE && pb_id.device_type != pbm_cat_to_phonebook(cat)))
  {
    UIM_MSG_ERR_0("Invalid Record category");
    return PBM_ERROR_INVALID_REC;
  }
  ...
  if (pbm_cache_rec_id != PBM_SESSION_GET_FIRST_AVAILABLE)
  {
    pbm_phonebook_type pb_type = pbm_session_record_id_to_phonebook_type(pbm_cache_rec_id);
    if (pb_id.device_type != pb_type.device_type)
    {
      UIM_MSG_ERR_3("Can not move record %d from one phonebook %d to another %d",
                    pbm_cache_rec_id, pb_type.device_type,
                    pb_id.device_type);
      return PBM_ERROR_INVALID_REC;
    }
  }
  ...
  {
    UIM_MSG_ERR_1("Invalid Session %d", session_type);
    PBM_MEM_FREEIF (field_info_ptr);
    return PBM_ERROR;
  }
  ...
  {
    //and ton_not_int is flag to check TON is not International
    boolean                 ton_not_int     = FALSE;
    // num_plus is flag to check the + sign in number
    boolean                 num_plus        = FALSE;

    /* Check to see if this field is a required field.  If so,
      * mark it as no longer required since we have one. */
    for (j = 0; j < PBM_MAX_REQUIRED_FIELDS; j++)
    {
      if (g_required_fields[group][j] == ptyped_fields[x].field_id)
      {
        record_required_fields[j] = FALSE;
        break;
      }
    ...
    if(ptyped_fields[x].field_id == PBM_FIELD_AOC &&
       pb_id.device_type == PBM_DIALED)
    {
      /* Attempt to write to dialed call history for data call , return unsupported */
      PBM_MEM_FREEIF (field_info_ptr);
      return PBM_ERROR_NOT_SUPPORTED;
    }
    ...
    if(!(pbm_hidden_key_verified &
          PBM_SESSIONID_TO_SESSION_MASK(session_type)))
    {
      /* only while editing the contact we need to check if rec_id is hidden or not */
      if((rec_id != PBM_SESSION_GET_FIRST_AVAILABLE) &&
         (pbm_is_rec_hidden(rec_id)))
      {
        /*return error if an attempt is made to overwrite hidden record,while hidden
                 key is not verified*/
        ret = PBM_ERROR_RESTRICTED;
      }

      //shouldnt allow writing hidden records when key is not verified
      if(ptyped_fields[x].field_id == PBM_FIELD_HIDDEN &&
         *((byte *) pbm_get_field_data_ptr(ptyped_fields, x)) == TRUE)
      {
        /*return error if an attempt is made to write hidden record,while hidden
                  key is not verified*/
        ret = PBM_ERROR_RESTRICTED;
      }
    }
    ...
    if(ptyped_fields[x].field_id == PBM_FIELD_HARDCODED_ECC &&
       pb_id.device_type == PBM_ECC_OTHER)
    {
      PBM_MEM_FREEIF (field_info_ptr);
      return PBM_ERROR_NOT_SUPPORTED;
  ...
  * of occurrances match what we need. */
    for (j = 0; j < field_info_ptr->num_fields; j++)
    {
      if (ptyped_fields[x].field_id == field_info_ptr->info[j].field_id)
      {
        counts[j]++;

        if ((!pbm_compare_field_types(ptyped_fields[x].field_type,
                                   field_info_ptr->info[j].field_type)) ||
            (((field_info_ptr->info[j].num_items > 0) &&
            (counts[j] > field_info_ptr->info[j].num_items))))
        {
          UIM_MSG_ERR_3("Bad Field Value: ID=0x%X Type=0x%x Count=%d",
                         ptyped_fields[x].field_id,
                         ptyped_fields[x].field_type, counts[j] );
          UIM_MSG_ERR_2("for record %d, phonebook %d", pbm_cache_rec_id,
                        pb_id.device_type);
          PBM_MEM_FREEIF (field_info_ptr);
          return PBM_ERROR_INVALID_REC;
        }

        if ((ptyped_fields[x].field_type != PBM_FT_UCS2 &&
             ptyped_fields[x].data_len > field_info_ptr->info[j].max_length) ||
            (ptyped_fields[x].field_type == PBM_FT_UCS2 &&
             ptyped_fields[x].data_len > PBM_TEXT_SIZE_BYTES))
        {
          if (ptyped_fields[x].field_type == PBM_FT_PHONE)
          {
        ...
        {
          /* for EFS write PBM do not convert the names encodign it writes in EFS as
                    it is hence no check required all the fields with UCS2
                    field type have to go with this procedure of checking  if they can be
                    acoomodated in ASCII only */
          if (ptyped_fields[x].field_type== PBM_FT_UCS2)
          {
            /* check with utility if the name can be converted to pure ascii */
            sim_coding_mode_enum_type out_mode = SIM_CODING_GSM7;
            uint16    out_base      = 0;
            uint16    out_converts  = 0;
            /* Subract 2 from max_length as PBM includes 2 bytes for UCS2 null char */
            uint16    out_len       = (uint16)field_info_ptr->info[j].max_length - 2;
            void     *data_ptr      = pbm_get_field_data_ptr(ptyped_fields, x);
            uint16    ucs_str_len   = (uint16)pbmutils_wstrlen((uint16*)data_ptr);
            uint16    loc           = 0;

            /* In case of ADN write, check for the max length for the records
               in that PB set. If input length is more than max length ADN
               parameters in that PB SET return Error. */
            if(pb_id.device_type == PBM_ADN)
            {
              PB_CACHE        *cache_ptr      = NULL;
              pbm_pb_set_type  pb_setnum      = PBM_USIM_PB_INVALID;

              cache_ptr = pbm_pb_id_to_cache(pb_id);
              ...
              if(rec_id == PBM_SESSION_GET_FIRST_AVAILABLE)
              {
                if ((loc = pbm_get_first_available(cache_ptr, (num_fields ? FALSE : TRUE))) == 0)
                {
                  PBM_MEM_FREEIF (field_info_ptr);
              ...
              {
                loc = pbm_session_record_id_to_location(rec_id);
              }

              pb_setnum = pbm_pb_record_index_to_pb_set(cache_ptr, (int)loc);

              if(!pbm_validate_pb_set(pb_setnum))
              {
                PBM_MEM_FREEIF (field_info_ptr);
                return PBM_ERROR;
              }

              switch(ptyped_fields[x].field_id)
              {
                case PBM_FIELD_NAME:
                  out_len = (uint16)cache_ptr->text_length[pb_setnum];
                  break;

                case PBM_FIELD_EMAIL:
                  if(email_cache_ptr[session_type][pb_setnum] == NULL)
                  {
                    PBM_MEM_FREEIF (field_info_ptr);
                    return PBM_ERROR;
                  }
                  if(email_cache_ptr[session_type][pb_setnum]->mapping_type == PBM_MAPPING_TYPE_1)
                  {
                    out_len = (uint16)email_cache_ptr[session_type][pb_setnum]->record_length;
                  }
                  else if(email_cache_ptr[session_type][pb_setnum]->mapping_type == PBM_MAPPING_TYPE_2)
                  {
                    out_len = (uint16)email_cache_ptr[session_type][pb_setnum]->record_length - 2;
                  }
                  break;

                case PBM_FIELD_OTHERNAME:
                  if(sne_cache_ptr[session_type][pb_setnum] == NULL)
                  {
                    PBM_MEM_FREEIF (field_info_ptr);
                    return PBM_ERROR;
                  }
                  if(sne_cache_ptr[session_type][pb_setnum]->mapping_type == PBM_MAPPING_TYPE_1)
                  {
                    out_len = (uint16)sne_cache_ptr[session_type][pb_setnum]->record_length;
                  }
                  else if(sne_cache_ptr[session_type][pb_setnum]->mapping_type == PBM_MAPPING_TYPE_2)
                  {
                    out_len = (uint16)sne_cache_ptr[session_type][pb_setnum]->record_length - 2;
                  }
                  break;
            ...
            pbmconvert_parse_input(data_ptr, 
                                   ucs_str_len,
                                   &out_mode,
                                   &out_len,
                                   &out_base,&out_converts,
          ...
          else if((ptyped_fields[x].field_type == PBM_FT_PHONE) &&
                  (ptyped_fields[x].data_len == field_info_ptr->info[j].max_length) &&
                  (((char*)(pbm_get_field_data_ptr(ptyped_fields, x)))[0] != '+'))
          {
            PBM_MEM_FREEIF (field_info_ptr);
        ...
        if (PBM_FIELD_COMPLEX == ptyped_fields[x].field_id)
        {
          ...
          ret_val = pbm_enum_complex_init(&ptyped_fields[x], NULL, &result);
          ...
          if(anr_found == FALSE)
          {
            ...
            ptyped_fields[x].field_id, pbm_cache_rec_id, pb_id.device_type);
            PBM_MEM_FREEIF (field_info_ptr);
            return PBM_ERROR_INVALID_REC;
          }
        }
    ...
    if (ptyped_fields[x].field_id == PBM_FIELD_INDEX)
    {
      uint16  index     = 0;
      void   *data_ptr = pbm_get_field_data_ptr(ptyped_fields, x);
  ...
  // If field is complex then extract the number from it and check for + sign and TON
    ...
    {
      // PBM_MAX_SUB_FIELDS_IN_COMPLEX = 3
      uint32      anr_sub_fields_size[PBM_MAX_SUB_FIELDS_IN_COMPLEX] = {0};
      uint32      num_comp_fields                                    = 0;
      uint32      complex_sizes                                      = 0;
      void       *complex_field                                      = NULL;
      void       *ton_data_ptr                                       = NULL;
      ...
      num_comp_fields = pbm_complex_fill_sizes(anr_sub_fields_size,
                                               PBM_MAX_SUB_FIELDS_IN_COMPLEX,
                                               complex_field) ;
      if((num_comp_fields == 0 ) ||
         (!pbm_allocate_fields((uint8 **)&sub_fields_ptr, &complex_sizes,
                                num_comp_fields,
                                (const uint32 *)anr_sub_fields_size  )))
      {
        //print PBM error message
        UIM_MSG_ERR_0("No sub fields found in complex field");
        PBM_MEM_FREEIF (field_info_ptr)
        return PBM_ERROR ;
      }
      if( pbm_get_fields_from_complex(complex_field, sub_fields_ptr,
                                      (int*) &num_comp_fields) == PBM_SUCCESS)
      {
        boolean   num_plus_complex    = FALSE;
        boolean   ton_not_int_complex = FALSE;
        uint32    s_f_cnt             = 0;
        void     *sub_field_data_ptr  = NULL;
        ...
        for(s_f_cnt = 0; s_f_cnt < num_comp_fields; s_f_cnt++)
        {
          ...
          if ( sub_fields_ptr[s_f_cnt].field_id == PBM_FIELD_ADDITIONAL_NUMBER_TYPE )
          {
            byte anr_type = 0xFF;
            anr_type = *((byte *)pbm_get_field_data_ptr(sub_fields_ptr, s_f_cnt));

            /* if Additional Number Record Identifier is greater than number of record in EF AAS
               we have to consider that as invalid record and fail validation.Also if
               Additional Number Record Identifier is OxFF it means it is free record and write has to be failed*/
            if(aas_cache_ptr[session_type] == NULL)
            {
              pbm_free_fields((uint8 **)&sub_fields_ptr, num_comp_fields);
              PBM_MEM_FREEIF (field_info_ptr)
              return PBM_ERROR;
            }
            if(anr_type > (byte) (aas_cache_ptr[session_type]->num_of_records))
            {
              UIM_MSG_ERR_2("record number in EFAAS = 0x%x is not present and number of records = 0x%x ",
                             anr_type, aas_cache_ptr[session_type]->num_of_records);
              pbm_free_fields((uint8 **)&sub_fields_ptr, num_comp_fields);
              PBM_MEM_FREEIF (field_info_ptr);
              return PBM_ERROR_INVALID_REC;
             }
          ...
          if ( sub_fields_ptr[s_f_cnt].field_id == PBM_FIELD_ADDITIONAL_NUMBER )
          {
            ...
            {
              // flag to track + sign in number )
              num_plus_complex = TRUE;
            }
          }
          ...
          if (sub_fields_ptr[s_f_cnt].field_id == PBM_FIELD_GSM_TONNPI )
          {
            ...
            else if (((*((byte *)ton_data_ptr)& CM_TON_MASK )== CM_TON_NATIONAL) ||
                    ((*((byte *)ton_data_ptr)& CM_TON_MASK)==CM_TON_NETWORK_SPECIFIC))
            {
  ...
  //if field is generic phone number then check for + sign and TON
    ...
    {
      void  *ton_data_ptr = pbm_get_field_data_ptr(ptyped_fields, x);

      PBM_CHECK_PTR_RET(ton_data_ptr, PBM_ERROR);
      if ((*((byte *)ton_data_ptr) & CM_TON_MASK )== CM_TON_NATIONAL ||
          ((*((byte *)ton_data_ptr) & CM_TON_MASK )==CM_TON_NETWORK_SPECIFIC ))
      {
        ton_not_int = TRUE;
      }
...
SIDE EFFECTS
===========================================================================*/
static boolean pbm_number_search_in_cache (
  pbm_phonebook_type    pb_id,
  const byte           *number,
  int                   loc_number_len,
  pbm_record_s_type    *record,
...
{
  pbm_addr_cache_s_type *record_ptr                          = NULL;
  PB_CACHE              *cache                               = NULL;
  pbm_field_s_type      *field_ptr                           = NULL;
  pbm_field_s_type      *sfield                              = NULL;
  int                    i                                   = 0;
  int                    j                                   = 0;
  int                    k                                   = 0;
  ...
  PBM_CHECK_PTR_RET(number, FALSE);
  ...
  /* If the conference is a URI, then FDN will succeed(because we don’t support FDN checks on URIs).*/
  if(strstr((char *)local_number,"sip:") ||
     strstr((char *)local_number,"tel:") ||
     strstr((char *)local_number,"@"))
  {
    return TRUE;
  }
  ...
  if(strstr((char *)local_number,"+"))
  {
    ...
    if(!pbm_util_get_ss_ton_status_of_queried_number(&valid_ss_string,
                                                     ...
                                                     (int*)&loc_number_len, local_number))
    {
      UIM_MSG_ERR_2("check failed invalid ton match %d %d ",valid_ss_string, queried_number_has_plus);
      return FALSE;
    }
  }
  else /* + sign not present in the number queried */
  {
    /* since + sign is not present, match with numbers whose TON is not set */
      ...
      for (j = 0; j < record_ptr->num_fields; j++)
      {
        ...
        while ((sfield = pbm_enum_complex_next(result)) != NULL)
        {
          ...
          if (sfield->field_type == PBM_FT_PHONE || sfield->field_type == PBM_FT_BCD)
          {
            ...
            cached_num_data_len = (uint16)strlen(number_ptr);
            if ( PBM_FEATURE_ENABLED == pbm_nv_get_feature_status(PBM_FEATURE_FDN_SPECIAL_HANDLING) )
            {
              ...
              */
              if( ( ( (cache_number_contains_plus) && (number_ptr[1] == '7') && (cached_num_data_len >= 12)  ) &&
                   ...
                   (( (cache_number_contains_plus == FALSE) && (number_ptr[0] == '8') && (cached_num_data_len >= 11) )  &&
                    ( (queried_number_has_plus) && (local_number[0] == '7') && (loc_number_len >= 11)        ) ))
              {
                  /* offset for cache number should be calculated based on '+' and also to offset digits more than 10 */
                offset_for_cache_number = (offset_for_cache_number+1) + ((uint8)cached_num_data_len - offset_for_cache_number - 11);
            ...
            if (match_numbers)
            {
              ...
              if(number_ptr[0] == '+')
              {
                ...
                if(pbm_util_is_valid_sups_string(number_ptr+1,
                                                  (uint8)strlen(number_ptr+1), &pos_plus, FALSE))
                {
                  /* if the number is valid SS string */
                  if (pbm_util_is_valid_sups_string(number_ptr+1,
                      (uint8)strlen(number_ptr+1), &pos_plus, TRUE)   /* if the number is valid SS string */
                      )
                  {
                    ...
                    for (k=0; k < pos_plus && k < sizeof(cached_num) - 1; k++)
                    {
              ...
              if(offset_for_cache_number > sizeof(cached_num) - 1 || 
                 cached_num_data_len > sizeof(cached_num) - 1)  
              {
                continue;
              }

              match_fn_return = pbm_number_match_int((byte *) cached_num + offset_for_cache_number,
                                                     (byte) cached_num_data_len - offset_for_cache_number,
                                                     (local_number_modified != NULL) ? (byte*)local_number_modified : (byte*)local_number,
                                                     (byte) loc_number_len,
                                                     (pb_id.device_type == PBM_FDN) ? TRUE : FALSE);
...
)
{
...
#endif
   else if(session_type == PBM_SESSION_LPB_GW ||
   ...
   {
     pb_id.slot_id = (uint16) pbm_get_slot_info (session_type);
   }
