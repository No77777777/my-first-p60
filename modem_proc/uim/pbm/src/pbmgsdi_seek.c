...
$Header: //components/rel/uim.mpss/6.1.0/pbm/src/pbmgsdi_seek.c#1 $


when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/21/16   nr      Search all PB sets to link empty EFADN with valid USIM records
...
)
{
  ...
  if (status == MMGSDI_SUCCESS)
  {
    if ((data_type == MMGSDI_SEARCH_CNF) &&
        (pbm_seek_data[session_type].in_use_records != NULL))
    {
      const mmgsdi_seek_data_type    *seek_data  = NULL;
      uint8                           i          = 0;
      int                             start      = 0;  /* start index for this pb set in in_use_records[] */
  ...
  int                   prev_index)
{
      ...
      pbm_seek_data[session_type].status == PBM_SUCCESS)
  {
    for (; next_index <= num_records; ++next_index)
    {
      ...
      /* The following handling is needed to associate empty EFADN with valid USIM fields if any */
      if(pbm_seek_data[session_type].cur_file > 0 &&
         (pbm_seek_data[session_type].cur_file - 1) < PBM_MAX_NUM_PB_SETS &&
         (pbm_seek_data[session_type].file_ids[pbm_seek_data[session_type].cur_file - 1] == PBM_FILE_ADN  ||
          pbm_seek_data[session_type].file_ids[pbm_seek_data[session_type].cur_file - 1] == PBM_FILE_ADN1 ||
          pbm_seek_data[session_type].file_ids[pbm_seek_data[session_type].cur_file - 1] == PBM_FILE_ADN2 ||
          pbm_seek_data[session_type].file_ids[pbm_seek_data[session_type].cur_file - 1] == PBM_FILE_ADN3))
      {
        pbm_num_text_record_s_type record;
        pbm_phonebook_type         pb_id       = {PBM_GPB};

        pb_id = pbm_file_id_to_pb_id(session_type, 
                                     pbm_seek_data[session_type].file_ids[pbm_seek_data[session_type].cur_file - 1]);
        memset(&record, 0, sizeof(pbm_num_text_record_s_type));
        memset(record.sim_text, 0xFF, sizeof(record.sim_text));
        if (!pbm_cache_record_add(pb_id, &record, next_index))
        {
          UIM_MSG_ERR_1("PBM cache record add failed: record index 0x%x", next_index);
        }
      }
    }
  }
