...
GENERAL DESCRIPTION
  ...
  Copyright (c) 2016, 2019 by QUALCOMM Technologies, Inc(QTI).
  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
$Header: //components/rel/uim.mpss/6.1.0/pbm/src/pbmgsdi_write.c#1 $


when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/27/19   nr      Notify fdn_status to clients
...
)
{
  ...
  byte                             local_pbm_data_buffer[ PBM_MAX_DATA_BLOCK_LEN ] = {0};
                                     ...
                                     pbm_file_type m_filename)
{
   ...
   int                            i                = 0;
   pbm_return_type                pbm_retval       = PBM_SUCCESS;
...
{
  int                         i                = 0;     /* Control variable for loop */
  int                         ext_num_of_rec   = 0;
  pbm_ext_rec_in_use_s_type  *ext_rec_in_use   = NULL;
  int                         extension_record = 0;
...
)
{
  ...
  if (PBM_SUCCESS != (ret_val = pbm_check_pb_ready(pb_id)))
  {
...
{
  pbm_notify_event_e_type       event    = 0;
  pbm_writecb_data_s_type       cb_data  = {0};
...
{
  PB_CACHE              *cache               = pbm_pb_id_to_cache
                              (pbm_session_record_id_to_phonebook_type(rec_id));
  uint16                 location            = pbm_record_id_to_location
                                                         ((uint16)rec_id);
...
{
  pbm_addr_cache_s_type *cache_rec   = NULL;
  PB_CACHE              *cache       = NULL;
  pbm_addr_node_s_type  *curr_node   = NULL;
  pbm_addr_node_s_type  *prev_node   = NULL;
  boolean                found       = FALSE;
  ...
  location = pbm_record_id_to_location((uint16)write->rec_id);
