...
GENERAL DESCRIPTION
  ...
  Copyright (c) 2002 - 2020 by QUALCOMM Technologies, Inc(QTI).
  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
$Header: //components/rel/uim.mpss/6.1.0/pbm/src/pbmuim.c#4 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/27/20   sch     Added NV disable_pbm_synchronization
04/17/18   rps     Check file type before accessing cache for PB files
...
05/03/16   nr      Logging improvements
04/18/16   sp      ECC only support in PBM
04/13/16   sp      Reduce global memory used by PBM
02/16/16   nr      Continue reading EFADN though EFEXT1 file read is failing
02/08/16   stv     Ignore write to PUID and UID in case of delete ADN record
...
11/20/15   nr      Logging improvements
10/05/15   sp      Reduce signed/unsigned conversions in UIM
09/15/15   nr      Validating file attribute response for PBM EFS
06/16/15   nr      Validating EF-EXT data
05/19/15   stv     Fix to address multiple initialization in parallel on card session
01/24/15   stv     Need not include AAS/GAS in FDN enable/disable clearing
...
pbm_file_type pbm_extn_files[PBM_NUM_EXTN_FILES] = {
PBM_FILE_EXT1,
PBM_FILE_EXT2,
PBM_FILE_EXT3,
PBM_FILE_EXT5,
...
pbm_usim_path_lookup_table_element_T *pbm_usim_path_lookup_table_ptr[PBM_SESSION_MAX][PBM_NUM_FILES_IN_LOOKUP_TABLE] = {NULL};

pbm_usim_path_lookup_table_element_T  pbm_usim_path_lookup_table_default[PBM_NUM_FILES_IN_LOOKUP_TABLE] =
{
    /* 0 */
...
mmgsdi_get_file_attr_cnf_type  *pbr_file_attr_ptr[MAX_UIM_APPS];
...
)
{
  ...
  if (status == MMGSDI_SUCCESS)
  {
    ...
    PBM_CHECK_PTR2_RET(cache, cache->pb_cache_array, rex_leave_crit_sect(&pbm_crit_sect));
...
static pbm_return_type pbm_usim_read_pbr(pbm_session_enum_type session_type)
{
   ...
   for (index = 0; index < PBM_NUM_FILES_IN_LOOKUP_TABLE; index++)
   {
     if(pbm_usim_path_lookup_table_ptr[session_type][index] == NULL)
     {
       return PBM_ERROR;
     }
     memscpy(pbm_usim_path_lookup_table_ptr[session_type][index],
             sizeof(pbm_usim_path_lookup_table_element_T),
             &pbm_usim_path_lookup_table_default[index],
             sizeof(pbm_usim_path_lookup_table_element_T));
   }
...
SIDE EFFECTS
===========================================================================*/
void pbm_usim_read_pbr_attr_cb(
  mmgsdi_return_enum_type  status,
  mmgsdi_cnf_enum_type     data_type,
  const mmgsdi_cnf_type   *data_ptr
)
{
  ...
  {
    UIM_MSG_ERR_1("PBM stopping init of SIM %d", pbm_sess_type);
    return;
  }
  if (status == MMGSDI_SUCCESS && pbr_file_attr_ptr[pbm_sess_type] != NULL)
  {
    memscpy(pbr_file_attr_ptr[pbm_sess_type], sizeof(mmgsdi_get_file_attr_cnf_type),
            &(data_ptr->get_file_attr_cnf), sizeof(mmgsdi_get_file_attr_cnf_type));
    if(pbr_file_attr_ptr[pbm_sess_type]->file_attrib.file_info.linear_fixed_file.num_of_rec > PBM_MAX_ADN_CNT_SUPPORTED)
    {
      pbr_file_attr_ptr[pbm_sess_type]->file_attrib.file_info.linear_fixed_file.num_of_rec = PBM_MAX_ADN_CNT_SUPPORTED;
    }
    ...
    {
      UIM_MSG_ERR_1(" PBM: mmgsdi_session_read_record returned error  %d",ret);
    }
  }
  else
  {
...
)
{
  ...
  do
  {
    if (status != MMGSDI_SUCCESS)
    {
      UIM_MSG_ERR_3("GSDI error code 0x%x sim 0x%x MMGSDI file %d sim read",
    ...
    pbm_umts_populate_fid_lookup_table(data_ptr->read_cnf.read_data.data_ptr,
                                       (data_ptr->response_header.client_data == (uint32) MMGSDI_TELECOM_PBR)?1:0,
                                        ...
                                        &ad8_cnt,
                                         pbm_sess_type);
    if(pbr_file_attr_ptr[pbm_sess_type] != NULL &&
       data_ptr->read_cnf.accessed_rec_num <
       pbr_file_attr_ptr[pbm_sess_type]->file_attrib.file_info.linear_fixed_file.num_of_rec)
    {
      file_struct.file.file_enum = (mmgsdi_file_enum_type) data_ptr->response_header.client_data;
      file_struct.access_method = MMGSDI_EF_ENUM_ACCESS;
      mmgsdi_ret = mmgsdi_session_read_record (data_ptr->response_header.session_id,
                                               ...
                                               pbr_file_attr_ptr[pbm_sess_type]->file_attrib.file_info.linear_fixed_file.rec_len,
                                               pbm_mmgsdi_read_pbr_rec_cb,
                                               data_ptr->response_header.client_data); //this arguement is for client_ref
    ...
    else
    {
      ...
      {
        if( pbm_usim_path_lookup_table_ptr[pbm_sess_type][index] == NULL)
        {
          UIM_MSG_HIGH_1(" Unexpected NULL pbm_usim_path_lookup_table_ptr at index %d", index);
          return;
        }
        if ( pbm_usim_path_lookup_table_ptr[pbm_sess_type][index]->file_type != (int)PBM_MAPPING_UNKNOWN )
        {
          pbr_table.num_rows++;
        }
        index++;
      ...
      {
        if( pbm_usim_path_lookup_table_ptr[pbm_sess_type][tbl_index] == NULL)
        {
          UIM_MSG_HIGH_1(" Unexpected NULL pbm_usim_path_lookup_table_ptr at index %d", tbl_index);
          return;
        }
        if (pbm_usim_path_lookup_table_ptr[pbm_sess_type][tbl_index]->file_type != (int)PBM_MAPPING_UNKNOWN &&
            pbm_usim_path_lookup_table_ptr[pbm_sess_type][tbl_index]->init      == TRUE )
        {
...
DEPENDENCIES
  ...
  Table: pbm_usim_path_lookup_table_ptr[pbm_sess_type]
  ...
  pbm_session_enum_type pbm_sess_type)
{
  ...
  /* -------------------------------------------------------------------------
     Iterrate throught pbm_usim_path_lookup_table_ptr[pbm_sess_type] and populate the response
     appropriately
     -----------------------------------------------------------------------*/
          ...
          rsp_index <  rsp_p->num_rows)
  {
        ...
        pbm_usim_path_lookup_table_ptr[pbm_sess_type][tbl_index]->init      == TRUE )
    {
      rsp_p->pbr_tbl_p[rsp_index].file_name    =
        (int)pbm_usim_path_lookup_table_ptr[pbm_sess_type][tbl_index]->file_id;
      rsp_p->pbr_tbl_p[rsp_index].file_mapping =
        (pbm_pbr_mapping_enum_type)pbm_usim_path_lookup_table_ptr[pbm_sess_type][tbl_index]->file_type;
      /* Short File ID is not yet saved in the MMGSDI USIM Table */
      rsp_p->pbr_tbl_p[rsp_index].file_sfid    =
        pbm_usim_path_lookup_table_ptr[pbm_sess_type][tbl_index]->ef_sid;

      /* Set the Order after the IAP File */
      rsp_p->pbr_tbl_p[rsp_index].iap_order =
        pbm_usim_path_lookup_table_ptr[pbm_sess_type][tbl_index]->index_after_iap;
      ...
      */
      ...
      memscpy(rsp_p->pbr_tbl_p[rsp_index].path_p, (sizeof(word) * PBM_PATH_LOOKUP_TABLE_MAX_FILE_LEN),
             pbm_usim_path_lookup_table_ptr[pbm_sess_type][tbl_index]->paths,
         ...
         switch(pbm_file_id_to_cache_type(file_id))
         {
            case PBM_CACHE_TYPE_PB:
               ...
               ))
               {
                  ...
                  m_usim_cache = adn_m_usim_cache_ptr[pbm_sess_type][pb_set];
...
)
{
  ...
  while( index < rec_len && data_ptr[index] != 0xFF )
  {
    if( data_ptr[index] == 0xD8  || data_ptr[index] == 0xA8 )
    {
      ...
      index_offset = index++;
      while( index < (word)(index_offset+file_type_len_val))
      {
        if( data_ptr[index++] == 0xC0 )
        {
          adn_type1 = TRUE ;
          index = (word)(index_offset+file_type_len_val);
          break;
        }
      }
    }
...
static void pbm_util_dump_path_lookup_table ( pbm_session_enum_type pbm_sess_type )
{
  ...
  for ( file_num = 0; file_num < PBM_NUM_FILES_IN_LOOKUP_TABLE; file_num++ ) {
    if(pbm_usim_path_lookup_table_ptr[pbm_sess_type][file_num] != NULL)
    {
      UIM_MSG_HIGH_3("  %x,%x,%x",pbm_usim_path_lookup_table_ptr[pbm_sess_type][file_num]->file_id,
                  pbm_usim_path_lookup_table_ptr[pbm_sess_type][file_num]->init,
                  pbm_usim_path_lookup_table_ptr[pbm_sess_type][file_num]->paths[2]);
    }
  }
}/* pbm_util_dump_path_lookup_table */
...
)
{
  ...
  {
    if ( pbm_usim_path_lookup_table_ptr[pbm_sess_type][file_num] != NULL &&
         pbm_usim_path_lookup_table_ptr[pbm_sess_type][file_num]->file_id == file_id )
    {
...
DESCRIPTION
   ...
   pbm_usim_path_lookup_table_ptr[pbm_sess_type] used for keeping track of the
...
)
{
  ...
  PBM_CHECK_PTR_RET(pbm_usim_path_lookup_table_ptr[pbm_sess_type][file_num], 0);
  ...
  if (pbm_status == PBM_SUCCESS)
  {
    ...
    /*                               */
    pbm_usim_path_lookup_table_ptr[pbm_sess_type][file_num]->paths[2]  = (word)ef_id;
    pbm_usim_path_lookup_table_ptr[pbm_sess_type][file_num]->ef_sid    = ef_sid;
    pbm_usim_path_lookup_table_ptr[pbm_sess_type][file_num]->file_type = file_type;
    pbm_usim_path_lookup_table_ptr[pbm_sess_type][file_num]->init      = TRUE;

    if(USE_USIM_LOCAL_SUPPORT(pbm_sess_type) || USE_CSIM_LOCAL_SUPPORT(pbm_sess_type))
    {
       pbm_usim_path_lookup_table_ptr[pbm_sess_type][file_num]->paths[0] = 0x7FFF;
    }
    else
    {
       pbm_usim_path_lookup_table_ptr[pbm_sess_type][file_num]->paths[0] = 0x7F10;
    }

    if (tag == 0xA9 || tag == 0xD9 )
    {
      pbm_usim_path_lookup_table_ptr[pbm_sess_type][file_num]->index_after_iap = iap_index_order;
    }
...
}/* pbm_util_write_phonebook_path_lookup_table */
  ...
  pbm_session_enum_type pbm_sess_type)
{
  ...
  if (pbm_status == PBM_SUCCESS &&
      pbm_usim_path_lookup_table_ptr[pbm_sess_type][file_num] != NULL) 
  {
    ...
    memscpy( path_ptr->path_buf,sizeof(path_ptr->path_buf),
        pbm_usim_path_lookup_table_ptr[pbm_sess_type][file_num]->paths,
...
)
{
  ...
  if ( pbm_status == PBM_SUCCESS && 
       pbm_usim_path_lookup_table_ptr[pbm_sess_type][file_num] != NULL)
  {
    *out_is_init = pbm_usim_path_lookup_table_ptr[pbm_sess_type][file_num]->init;
  }
...
/*===========================================================================
FUNCTION PBM_UTIL_ACCESS_PATH_LOOKUP_TABLE
  ...
  pbm_file_type *file_enum_ptr)
{
  ...
  *file_enum_ptr = PBM_FILE_MAX_REQUESTS;
  ...
  if (len_to_be_used > 2)
  {
    for(file_num = 0; file_num < PBM_NUM_FILES_IN_LOOKUP_TABLE; file_num++)
    { 
      if(pbm_usim_path_lookup_table_ptr[session_type][file_num] != NULL &&
         pbm_usim_path_lookup_table_ptr[session_type][file_num]->init == TRUE)
      {
        /*
         * pbm_usim_path_lookup_table_ptr holds file path excluding the MF file id
         * without any holes like in uim path table.
         * so compare the rest of the path items except MFID.
         */
        if(!memcmp(pbm_usim_path_lookup_table_ptr[session_type][file_num]->paths,
                  (path_type.path_buf + 1),
                  (len_to_be_used - 1)*sizeof(uint16)))
        {
          *file_enum_ptr = pbm_usim_path_lookup_table_ptr[session_type][file_num]->file_id;
          break;
        }
      }
    }/* for(;;) */
...
===========================================================================*/
static void pbm_fileinfo_complete(
  pbm_session_enum_type session_type
)
{
  int           i                                = 0;
...
#define ALLOC_MEM(dest, num, size)                  \
  ...
  if (bAllocMasterUSIMCache)
  {
    for (j = 0; j < (int)PBM_MAX_NUM_PB_SETS; j++)
    {
      ...
      {
        if (adn_cache_ptr[session_type] != NULL &&
            j < adn_cache_ptr[session_type]->num_files)
        {
          if (pbm_fileinfo_requests[i].do_request)
          {
            PB_MASTER_USIM_CACHE *m_usim_cache =
              adn_m_usim_cache_ptr[session_type][j];
            if(m_usim_cache == NULL)
            {
              break;
            }
       ...
       i++)
  {
    ...
    switch (pbm_file_id_to_cache_type(file_id))
    {
      ...
      case PBM_CACHE_TYPE_PB:
      {
        ...
        {
          PB_CACHE *pb_cache_hidden = hidden_cache_ptr[session_type];
          if(pb_cache_hidden == NULL)
          {
            break;
          }
      ...
      case PBM_CACHE_TYPE_USIM:
      {
        ...
        if (usim_cache == NULL ||
            ...
            adn_cache_ptr[session_type] == NULL ||
            !adn_cache_ptr[session_type]->num_of_records)
        {
          break;
        }
        ...
        m_usim_cache = adn_m_usim_cache_ptr[session_type][pb_set];
        if(m_usim_cache == NULL)
        {
          break;
        }
        ...
        adn_records_this_file = adn_cache_ptr[session_type]->records_in_pb_set[pb_set];

        PBM_CHECK_PTR2_RET(adn_records_this_file, m_usim_cache->index_array, VOID);
...
SIDE EFFECTS
  Globals describing the world according to this SESSION are set.
===========================================================================*/
void pbm_make_fileinfo_request (
  pbm_session_enum_type session_type,
  uint32                start
)
{
  ...
  uint32                  i                 = 0; /* Index into pbm_fileinfo_requests array */
  uint32                  file_id           = 0;
...
SIDE EFFECTS
  None.
===============================================================================*/
boolean pbm_adn_caching_status(
  pbm_session_enum_type session_type
)
{
  uint16  slot_id  = 0;
...
void pbm_init_fileinfo_internal (pbm_session_enum_type session_type)
{
   ...
   else
   {
      ...
      else
      {
        ...
        adn_cache_ptr[session_type]->status = PBM_STATUS_NOT_AVAILABLE;
      }
...
SIDE EFFECTS
  Globals describing the world according to this SESSION are set.
===========================================================================*/
void pbm_start_fileinfo_internal (
  pbm_session_enum_type          session_type
)
{
  PBM_VALIDATE_SESSION_RET(session_type, VOID);
...
SIDE EFFECTS
  Globals describing the world according to this SIM are set.
===========================================================================*/
void pbm_restart_fileinfo_internal (
  pbm_session_enum_type        session_type
)
{
  UIM_MSG_ERR_1("PBM restarting init of session_type %d", session_type);
...
static void pbm_fill_pb_sync1cache_info(PB_SYNC1_CACHE *pSync1Cache,
                                        ...
                                        pbm_session_enum_type session_type)
{
   PBM_CHECK_PTR_RET(pSync1Cache,VOID);
                                        ...
                                        pbm_session_enum_type session_type)
{
   ...
   PBM_CHECK_PTR_RET(pSync2Cache,VOID);
   pb_set = pbm_file_id_to_pb_set(file_id);
   if ( ! pbm_validate_pb_set(pb_set) )
   {
      return ;
...
)
{
  ...
  pbm_file_type                       file_id                    = PBM_FILE_NONE;
  pbm_file_type                       file_id_in_same_pbset      = PBM_FILE_NONE;
  uint32                              client_ref                 = 0;
  int                                 i                          = 0;
  int                                 known_rec_len              = 0;
  ...
  const mmgsdi_file_security_type    *write                      = NULL;
  ...
  {
    UIM_MSG_ERR_1("invalid confirmation type 0x%x", data_type);
    return;
  }
  ...
  UIM_MSG_HIGH_4("file_id 0x%x session 0x%x status 0x%x session_init 0x%x",
                  file_id, session_type, status, pbm_session_initializing);
  ...
  if (MMGSDI_SUCCESS == status)
  {
    ...
    switch (file_type)
    {
      case MMGSDI_LINEAR_FIXED_FILE:
        ...
        /* As per spec TS 102.221 8.2.2.2 Linear fixed EF  maximum no of records shouldn't
           exceed more than 254 and record length shouldn't cross more than 255 bytes */
        if(number_of_records > PBM_MAX_RECS_IN_LINEAR_FIXED_CYCLIC_EF ||
           record_length > PBM_MAX_REC_LEN_IN_LINEAR_FIXED_EF)
        {
          number_of_records = 0;
          record_length     = 0;
        }
      ...
      case MMGSDI_CYCLIC_FILE:
        ...
        /* As per spec TS 102.221 8.2.2.3 Cyclic EF  maximum no of records shouldn't
           exceed more than 254 and record length shouldn't cross more than 254 bytes */
        if(number_of_records > PBM_MAX_RECS_IN_LINEAR_FIXED_CYCLIC_EF ||
           record_length > PBM_MAX_REC_LEN_IN_CYCLIC_EF)
        {
          number_of_records = 0;
          record_length     = 0;
        }
    ...
    UIM_MSG_HIGH_3("number_of_records 0x%x record_length 0x%x file_type 0x%x",
                   number_of_records,
                   record_length,
                   file_type);
        ...
        (record_length > 0))
    {
      switch (file_id)
      {
        ...
        case PBM_FILE_ADN3:
          ...
          pbm_fill_pb_cache_info(adn_cache_ptr[session_type],
                                 session_type,
                                 pb_set,
                                 record_length,
                                 PBM_EF_PHONEBOOK_MANDATORY_REC_LEN,
                                 file_id,
                                 file_type,
                                 (file_id == PBM_FILE_ADN) ? FALSE : TRUE,
                                 number_of_records,
                                 write);
          adn_cache_ptr[session_type]->m_usim_cache[pb_set] = adn_m_usim_cache_ptr[session_type][pb_set];

          UIM_MSG_HIGH_4(" PBM:found 0x%x records_in_pb_set 0x%x file_invalidated 0x%x, read_write_when_invalidated 0x%x",
		  	   adn_cache_ptr[session_type]->records_in_pb_set[pb_set],
                           pb_set,
          ...
          if ( (data_ptr->get_file_attr_cnf.file_attrib.file_info.linear_fixed_file.file_status.file_invalidated) &&
               (!data_ptr->get_file_attr_cnf.file_attrib.file_info.linear_fixed_file.file_status.read_write_when_invalidated) )
          {
            adn_cache_ptr[session_type]->status = PBM_STATUS_RESTRICTED;
          }

          if(adn_cache_ptr[session_type]->status == PBM_STATUS_RESTRICTED)
          {
            /* Notifying ADN Restricted status to QMI PBM to clear the ADN/FDN bitmask */
            pbm_notify_pb_cache_status_update(adn_cache_ptr[session_type]);
          }
          break;

        case PBM_FILE_SDN:
          if(sdn_cache_ptr[session_type] == NULL)
          {
            return;
          }
          pbm_fill_pb_cache_info(sdn_cache_ptr[session_type],
                                 session_type,
                                 PBM_USIM_PB_ONE,
                                 record_length,
                                 PBM_EF_PHONEBOOK_MANDATORY_REC_LEN,
        ...
        case PBM_FILE_MSISDN:
          if(msisdn_cache_ptr[session_type] == NULL)
          {
            return;
          }
          ...
          {
            known_rec_len = PBM_EF_PHONEBOOK_MANDATORY_REC_LEN;
          }

          pbm_fill_pb_cache_info(msisdn_cache_ptr[session_type],
                                 session_type,
                                 PBM_USIM_PB_ONE,
                                 record_length,
                                 known_rec_len,
        ...
        case PBM_FILE_MBN:
          if(mbn_cache_ptr[session_type] == NULL)
          {
            return;
          }

          pbm_fill_pb_cache_info(mbn_cache_ptr[session_type],
                                 session_type,
                                 PBM_USIM_PB_ONE,
                                 record_length,
                                 PBM_EF_PHONEBOOK_MANDATORY_REC_LEN,
        ...
        case PBM_FILE_FDN:
          if(fdn_cache_ptr[session_type] == NULL)
          {
            return;
          }

          pbm_fill_pb_cache_info(fdn_cache_ptr[session_type],
                                 session_type,
                                 PBM_USIM_PB_ONE,
                                 record_length,
                                 PBM_EF_PHONEBOOK_MANDATORY_REC_LEN,
        ...
        case PBM_FILE_LND:
          if(lnd_cache_ptr[session_type] == NULL)
          {
            return;
          }

          pbm_fill_pb_cache_info(lnd_cache_ptr[session_type],
                                 session_type,
                                      PBM_USIM_PB_ONE,
                                      record_length,
                                      PBM_EF_PHONEBOOK_MANDATORY_REC_LEN,
        ...
        case PBM_FILE_GAS:
        {
          pbm_g_other_field_info_s_type *field_info = g_sim_field_info_ptr[session_type][2];
          if(field_info ==NULL || grpname_cache_ptr[session_type] == NULL)
          {
            return;
          }
          pbm_fill_pb_cache_info(grpname_cache_ptr[session_type],
                                 session_type,
                                 PBM_USIM_PB_ONE,
                                 record_length,
                                 PBM_AAS_GAS_KNOWN_REC_LEN,
                                 file_id,
                                 file_type,
                                 FALSE,
                                 number_of_records,
        ...
        case PBM_FILE_AAS:
        {
          pbm_g_other_field_info_s_type *field_info = g_sim_field_info_ptr[session_type][3];
          if(field_info ==NULL || aas_cache_ptr[session_type] == NULL)
          {
            return;
          }

          pbm_fill_pb_cache_info(aas_cache_ptr[session_type],
                                 session_type,
                                 PBM_USIM_PB_ONE,
                                 record_length,
                                 PBM_AAS_GAS_KNOWN_REC_LEN,
        ...
        case PBM_FILE_OCI:
          if(oci_cache_ptr[session_type] == NULL)
          {
            return;
          }

          pbm_fill_pb_cache_info(oci_cache_ptr[session_type],
                                 session_type,
                                 PBM_USIM_PB_ONE,
                                 record_length,
                                 OCI_KNOWN_REC_LEN,
        ...
        case PBM_FILE_ICI:
          if(ici_cache_ptr[session_type] == NULL)
          {
            return;
          }

          pbm_fill_pb_cache_info(ici_cache_ptr[session_type],
                                      session_type,
                                      PBM_USIM_PB_ONE,
                                      record_length,
                                      ICI_KNOWN_REC_LEN,
        ...
        case PBM_FILE_MBDN:
          if(mbdn_cache_ptr[session_type] == NULL)
          {
            return;
          }
          pbm_fill_pb_cache_info(mbdn_cache_ptr[session_type],
                                 session_type,
                                 PBM_USIM_PB_ONE,
                                 record_length,
                                 PBM_EF_PHONEBOOK_MANDATORY_REC_LEN,
        ...
        case PBM_FILE_EXT1:
          if( adn_cache_ptr[session_type] != NULL && ext1_cache_ptr[session_type]!= NULL &&
              lnd_cache_ptr[session_type] != NULL && mbn_cache_ptr[session_type]!= NULL &&
              pbm_fill_pb_extcache_info(ext1_cache_ptr[session_type], file_id, file_type,
                                        number_of_records, record_length, session_type))
          {
            adn_cache_ptr[session_type]->extension_cache = ext1_cache_ptr[session_type];
            lnd_cache_ptr[session_type]->extension_cache = ext1_cache_ptr[session_type];
            mbn_cache_ptr[session_type]->extension_cache = ext1_cache_ptr[session_type];
            if (!USE_USIM_SUPPORT(session_type) && msisdn_cache_ptr[session_type] != NULL)
            {
              msisdn_cache_ptr[session_type]->extension_cache = ext1_cache_ptr[session_type];
            }
          }
          break;

        case PBM_FILE_EXT2:
          if (ext2_cache_ptr[session_type] != NULL && fdn_cache_ptr[session_type]!= NULL &&
               pbm_fill_pb_extcache_info(ext2_cache_ptr[session_type],
                                        file_id, file_type,
                                        number_of_records, record_length, session_type))
          {
            fdn_cache_ptr[session_type]->extension_cache = ext2_cache_ptr[session_type];
          }
          break;

        case PBM_FILE_EXT3:
          if (sdn_cache_ptr[session_type] != NULL && ext3_cache_ptr[session_type]!= NULL &&
               pbm_fill_pb_extcache_info(ext3_cache_ptr[session_type],
                                         file_id, file_type,
                                         number_of_records, record_length, session_type))
          {
            sdn_cache_ptr[session_type]->extension_cache = ext3_cache_ptr[session_type];
          }
          break;

        case PBM_FILE_EXT5:
          if (ici_cache_ptr[session_type] != NULL &&
              oci_cache_ptr[session_type] != NULL &&
              ext5_cache_ptr[session_type]!= NULL &&
              pbm_fill_pb_extcache_info(ext5_cache_ptr[session_type], file_id,
                                        file_type,
                                        number_of_records, record_length, session_type))
          {
            ici_cache_ptr[session_type]->extension_cache = ext5_cache_ptr[session_type];
            oci_cache_ptr[session_type]->extension_cache = ext5_cache_ptr[session_type];
            if (USE_USIM_SUPPORT(session_type) && msisdn_cache_ptr[session_type] != NULL)/* For USIM */
            {
              msisdn_cache_ptr[session_type]->extension_cache = ext5_cache_ptr[session_type];
            }
          }
          break;

        case PBM_FILE_EXT6:
          if (mbdn_cache_ptr[session_type] != NULL && ext6_cache_ptr[session_type] != NULL &&
              pbm_fill_pb_extcache_info(ext6_cache_ptr[session_type], file_id,
                                        file_type,
                                        number_of_records, record_length, session_type))
          {
            if (USE_USIM_SUPPORT(session_type)) /* For USIM */
            {
              mbdn_cache_ptr[session_type]->extension_cache = ext6_cache_ptr[session_type];
            }
          }
          break;
        ...
        case PBM_FILE_ANRC3:
        {
          ...
          if (usim_cache->mapping_type != PBM_MAPPING_TYPE_2 && 
              adn_m_usim_cache_ptr[session_type][pb_set] != NULL)
          {
            adn_m_usim_cache_ptr[session_type][pb_set]->num_caches++;
          }
        }
          break;
        ...
        case PBM_FILE_IAP:
          ...
          if(adn_m_usim_cache_ptr[session_type][pb_set] != NULL)
          {
            m_usim_cache                  = adn_m_usim_cache_ptr[session_type][pb_set];
        ...
        case PBM_FILE_IAP3:
          ...
          {
            m_usim_cache = adn_m_usim_cache_ptr[session_type][pb_set];
            if (adn_m_usim_cache_ptr[session_type][PBM_USIM_PB_ONE] == NULL ||
                adn_m_usim_cache_ptr[session_type][PBM_USIM_PB_ONE]->iap_rec_len !=
                record_length)
            {
        ...
        case PBM_FILE_PSC:
          pbm_fill_pb_sync1cache_info(psc_cache_ptr[session_type], file_id,
                                      file_type,
                                      1, PBM_PSC_LENGTH, session_type);
          break;

        case PBM_FILE_PUID:
          pbm_fill_pb_sync1cache_info(puid_cache_ptr[session_type], file_id,
                                      file_type, 1, PBM_PUID_LENGTH, session_type);
          break;

        case PBM_FILE_CC:
          pbm_fill_pb_sync1cache_info(cc_cache_ptr[session_type], file_id,
                                      file_type, 1, PBM_CC_LENGTH, session_type);
          break;

        case PBM_FILE_PBC:
        if(pbc_cache_ptr[session_type] != NULL)
        {
          pbc_cache_ptr[session_type]->file_ids[PBM_USIM_PB_ONE]          = file_id;
          pbc_cache_ptr[session_type]->file_type                          = file_type;
          pbc_cache_ptr[session_type]->num_of_records                     = number_of_records;
          pbc_cache_ptr[session_type]->records_in_pb_set[PBM_USIM_PB_ONE] = number_of_records;
          pbc_cache_ptr[session_type]->num_files                          = 1;
          pbc_cache_ptr[session_type]->record_length                      = record_length;
        }
          break;
        ...
        case PBM_FILE_PBC2:
        case PBM_FILE_PBC3:
          pbm_fill_pb_sync2cache_info(pbc_cache_ptr[session_type], file_id,
        ...
        case PBM_FILE_UID:
          ...
          if(uid_cache_ptr[session_type] != NULL)
          {
            uid_cache_ptr[session_type]->file_ids[pb_set]          = file_id;
            uid_cache_ptr[session_type]->file_type                 = file_type;
            uid_cache_ptr[session_type]->num_of_records            = number_of_records;
            uid_cache_ptr[session_type]->record_length             = record_length;
            uid_cache_ptr[session_type]->records_in_pb_set[pb_set] = number_of_records;
            uid_cache_ptr[session_type]->num_files                 = 1;
          }
          break;
        ...
        case PBM_FILE_UID2:
        case PBM_FILE_UID3:
          pbm_fill_pb_sync2cache_info(uid_cache_ptr[session_type], file_id,
                                      file_type, number_of_records, record_length, session_type);
          break;
  ...
  {
    void                         *cache      = NULL;
    pbm_cache_type_e_type         cache_type = PBM_CACHE_TYPE_UNKNOWN;

    /* If the IAP entry is present in PBR and file is not found on card */
    if(file_id == PBM_FILE_IAP || file_id == PBM_FILE_IAP1 ||
       file_id == PBM_FILE_IAP2 || file_id == PBM_FILE_IAP3)
    {
      PB_USIM_CACHE         *usim_cache     = NULL;
      pbm_file_type          file_id_loop   = PBM_FILE_NONE;
      uint32                 index          = 0;  /* Index into pbm_fileinfo_requests array */
       ...
       file_id == PBM_FILE_ADN2 || file_id == PBM_FILE_ADN3)
    {
      ...
      {
        if (pbm_file_id_to_pb_set(file_id) == pbm_file_id_to_pb_set(file_id_in_same_pbset))
        {
...
SIDE EFFECTS
===========================================================================*/
static void pbm_session_init_completed(
  pbm_session_enum_type session_type
)
{
  pbm_phonebook_type             pb_id    = {PBM_GPB};
  ...
  {
    field_info = g_sim_field_info_ptr[session_type][0];
    if(field_info == NULL)
    {
      return;
    }
  ...
  if (ds_ready_fn != NULL)
  {
    (*ds_ready_fn)((pbm_check_pb_ready(pb_id) == PBM_SUCCESS)?TRUE:FALSE);
  }
...
)
{
  ...
  if (NULL != cache)
  {
    if (PBM_SUCCESS == pbm_returned) /* If we can trust the data. */
    {
      ...
      /* Returing here in case of ADN and synchronisation is enabled
         as the intilization continues from reset PBC */
      if(pbm_uim_cache_update (cache, record_num, record->file_id, record->session_type))
      {
        return;
      }
    }
    ...
    {
      UIM_MSG_ERR_3("Unable to read from SIM, Error 0x%x after 0x%x of max 0x%x",
  ...
  {
    UIM_MSG_ERR_1("Could not find cache for phonebook 0x%x", pb_id.device_type);
  }
...
/*===========================================================================
FUNCTION PBM_UIM_CACHE_UPDATE

DESCRIPTION
  This function is to update cache state and globals based on reads
  status from card.
...
)
{
  ...
  if (record_num > cache->num_of_records)
  {
    /* check for the hidden key verified for this session type, if it is verified ,
     * merge the hidden cache in adn cache. No need to do anything related to 
     * hidden key,when EF-Hiddenkey is not present for that we will check for valid dir_index
     */
    if (file_id == PBM_FILE_ADN && pbm_session_info[session_type].dir_index != 0)
    {
      pbm_g_other_field_info_s_type *field_info  = NULL;
      uint8                          field_index = 0;
    ...
    else if(file_id == PBM_FILE_GAS && g_sim_field_info_ptr[session_type][2] != NULL)
    {
      g_sim_field_info_ptr[session_type][2]->info[0].num_items = 1;
      /* add 2 for UCS2 NULL char */
      g_sim_field_info_ptr[session_type][2]->info[0].max_length = cache->text_length[pb_set] + 2;
    }
    else if(file_id == PBM_FILE_AAS && g_sim_field_info_ptr[session_type][3] != NULL)
    {
      g_sim_field_info_ptr[session_type][3]->info[0].num_items = 1;
      /* add 2 for UCS2 NULL char */
      g_sim_field_info_ptr[session_type][3]->info[0].max_length = cache->text_length[pb_set] + 2;
    }
...
/*===========================================================================
FUNCTION PBM_DEL_LINKS_WRITE_CB
...
)
{
  uint32                     client_ref            = 0;
  pbm_session_enum_type      session_type          = PBM_SESSION_MAX ;
  uint16                     l_delete_link_index   = 0;
  PB_CACHE                  *cache                 = NULL;
  pbm_async_gsdi_buffer     *buffer                = NULL;
  pbm_return_type            ret                   = PBM_ERROR_MAX;
...
SIDE EFFECTS
===========================================================================*/
pbm_return_type pbm_uim_del_anrid(
  uint32            buffer_index,
...
)
{
  ...
  for (i = 1; i <= m_usim_cache->num_caches; i++)
  {
    ...
    {
      uint16 rec_num_in_file = (uint16)pbm_pb_record_index_to_fileloc(cache,delete_link_index);
      ...
      if (usim_index)
      {
        ...
        sim_rec->ext_id = (byte)pbm_check_extension_record(cache->extension_cache,
                                                           (int)rec_num_in_file,
                                                            usim_cache->file_id);
...
SIDE EFFECTS
===========================================================================*/
pbm_return_type pbm_uim_write (
  uint32 buffer_index
)
{
  pbm_write_record_s_type *write         = NULL;
  pbm_async_gsdi_buffer   *buffer        = NULL;
  ...
  while (buffer->state != PBM_WRITE_DONE)
  {
    switch (buffer->state)
    {
      ...
      case PBM_WRITE_LINKS_DELETED:
        ...
        buffer->record.index = (int)pbm_record_id_to_location((uint16)write->rec_id);
        buffer->findex = 0;
        ...
        PBM_MEM_ALLOC(buffer->caches_in_use,
                      ...
                      ((uint32) (adn_m_usim_cache_ptr[session_type][pb_set]->num_caches + 1)));
...
)
{
  ...
  *ret = PBM_ERROR;
  ...
  if (adn_m_sync_cache_ptr[buffer->record.session_type] != NULL &&
      (adn_m_sync_cache_ptr[buffer->record.session_type]->reset_pbc || buffer->findex))
  {
    bret = FALSE;
    *ret = PBM_SUCCESS;
  }
  else
  {
    ...
    buffer->record.index = (int)pbm_record_id_to_location((uint16)buffer->write->rec_id);
...
)
{
  ...
  pbm_write_record_s_type *write            = NULL;
  pbm_session_enum_type    session_type     = PBM_SESSION_MAX;
  pbm_file_type           *file_ptr         = NULL;
  uint16                   num_of_files     = 0;
  pbm_file_type            update_file_id[] = {PBM_FILE_PSC,
                                               PBM_FILE_CC,
                                               PBM_FILE_PUID,
                                               PBM_FILE_UID};
  pbm_file_type            delete_file_id[] = {PBM_FILE_PSC,
                                               PBM_FILE_CC};
  PBM_CHECK_PTR_RET(ret, FALSE);
  *ret = PBM_ERROR;
  ...
  if(write->curr_rec->num_fields == 0)
  {
    num_of_files = ARR_SIZE(delete_file_id);
    file_ptr = delete_file_id;
  }
  else
  {
    num_of_files = ARR_SIZE(update_file_id);
    file_ptr = update_file_id;
  }

  if (adn_m_sync_cache_ptr[session_type] != NULL &&
      !(adn_m_sync_cache_ptr[session_type]->sync_present))
  {
    *ret = PBM_SUCCESS;
    return FALSE;
  }

  while (buffer->findex < num_of_files)
  {
    switch (file_ptr[buffer->findex])
    {
      case PBM_FILE_PUID:
      case PBM_FILE_CC:
        buffer->record.file_id = file_ptr[buffer->findex];
      ...
      case PBM_FILE_UID:
        ...
        else
        {
          uint16 data;
          uint16 index;
          PB_CACHE *pb_cache = adn_cache_ptr[session_type];
  ...
  if (adn_cache_ptr[session_type] != NULL &&
      adn_cache_ptr[session_type]->status == PBM_STATUS_WRITE_SYNC)
  {
    PBM_SET_PB_CACHE_READY(adn_cache_ptr[session_type]);
  }
  ...
  *ret = PBM_ERROR;
  ...
  do
  {
    ...
    for(l_j = 0;l_j < record->num_fields; l_j++)
    {
      ...
      {
        pbc_data |= (uint16)(pbm_session_info[session_type].dir_index);
      }
    }

    //no need to write anything in the below cases
    if((adn_m_sync_cache_ptr[session_type] != NULL &&  pbc_cache_ptr[session_type] != NULL) &&
        ((pbm_session_info[session_type].dir_index == 0
        && adn_m_sync_cache_ptr[session_type]->reset_pbc == FALSE)
        || ( (pbc_data == 0) && (adn_m_sync_cache_ptr[session_type]->reset_pbc == FALSE) )
        || pbc_cache_ptr[session_type]->num_of_records < (int)pbm_record_id_to_location((uint16)buffer->write->rec_id)))
    {
   ...
   *ret = PBM_ERROR;
   ...
   if (record)
   {
      ...
      if (index_array)
      {
         ...
         if (adn_m_usim_cache_ptr[buffer->record.session_type] != NULL &&
             adn_m_usim_cache_ptr[buffer->record.session_type][pb_set]->iap_num_records
                        ...
                        pbm_sim_record_s_type *record)
{
   ...
   if (sync1_cache != NULL)
   {
      if (pbm_returned == PBM_SUCCESS &&
          adn_m_sync_cache_ptr[session_type] != NULL) /* If we can trust the data. */
      {
         switch (file_id)
         {
            case PBM_FILE_PSC:
               if(psc_cache_ptr[session_type] != NULL)
               {
                 psc_cache_ptr[session_type]->data.psc = record->data.dword;
                 adn_m_sync_cache_ptr[session_type]->psc_cache = psc_cache_ptr[session_type];
               }
               break;

            case PBM_FILE_PUID:
              if(puid_cache_ptr[session_type] != NULL)
              {
               puid_cache_ptr[session_type]->data.puid = record->data.word;
               adn_m_sync_cache_ptr[session_type]->puid_cache = puid_cache_ptr[session_type];
              }
               break;

            case PBM_FILE_CC:
              if(cc_cache_ptr[session_type] != NULL)
              {
               cc_cache_ptr[session_type]->data.cc = record->data.word;
               adn_m_sync_cache_ptr[session_type]->cc_cache = cc_cache_ptr[session_type];
              }
               break;
...
SIDE EFFECTS
===========================================================================*/
void pbm_sync2_read_cb (
  pbm_return_type        pbm_returned,
  pbm_sim_record_s_type *record
)
{
  PB_SYNC2_CACHE        *sync2_cache   = NULL;
  pbm_session_enum_type  session_type  = PBM_SESSION_MAX;
  int                    index         = 0;
  pbm_file_type          file_id       = PBM_FILE_NONE;
  int                    record_num    = 0;
  ...
  if (sync2_cache != NULL)
  {
    if (pbm_returned == PBM_SUCCESS) /* If we can trust the data. */
    {
      switch (file_id)
      {
        ...
        case PBM_FILE_UID2:
        case PBM_FILE_UID3:
          PBM_CHECK_PTR2_RET(uid_cache_ptr[session_type],uid_cache_ptr[session_type]->data_array, VOID);
          uid_cache_ptr[session_type]->data_array[index] = record->data.word;
          break;
        ...
        case PBM_FILE_PBC2:
        case PBM_FILE_PBC3:
          PBM_CHECK_PTR2_RET(pbc_cache_ptr[session_type],pbc_cache_ptr[session_type]->data_array, VOID);
          pbc_cache_ptr[session_type]->data_array[index] = record->data.word;
          break;
      ...
      if (record_num > sync2_cache->num_of_records)
      {
        switch (file_id)
        {
          ...
          case PBM_FILE_PBC2:
          case PBM_FILE_PBC3:
            PBM_CHECK_PTR2_RET(adn_m_sync_cache_ptr[session_type],adn_cache_ptr[session_type], VOID);
            adn_m_sync_cache_ptr[session_type]->pbc_cache = sync2_cache;
            adn_cache_ptr[session_type]->m_sync_cache =
                                   adn_m_sync_cache_ptr[session_type];
            break;
          ...
          case PBM_FILE_UID3:
            PBM_CHECK_PTR2_RET(adn_m_sync_cache_ptr[session_type],adn_cache_ptr[session_type], VOID);
            adn_m_sync_cache_ptr[session_type]->uid_cache = sync2_cache;
            if (adn_m_sync_cache_ptr[session_type]->psc_cache &&
                adn_m_sync_cache_ptr[session_type]->cc_cache &&
                adn_m_sync_cache_ptr[session_type]->puid_cache &&
                adn_m_sync_cache_ptr[session_type]->uid_cache)
            {
              adn_cache_ptr[session_type]->m_sync_cache =
                                       adn_m_sync_cache_ptr[session_type];
			  /* Disable or enable PBM synchronization according to NV disable_pbm_synchronization */
              if(pbm_nv_get_feature_status(PBM_FEATURE_DISABLE_PBM_SYNCHRONIZATION) == PBM_FEATURE_DISABLED)									   
			  {
              adn_cache_ptr[session_type]->m_sync_cache->sync_present = TRUE;
            }
			  else
			  {
			    adn_cache_ptr[session_type]->m_sync_cache->sync_present = FALSE;
		      }
            }
            break;
    ...
    if (pbm_returned == PBM_SUCCESS) /* If we can trust the data. */
    {
      ...
      for (i = 1; i <= m_usim_cache->num_caches && m_usim_cache->usim_cache_array; i++)
      {
            ...
            usim_loc <= usim_cache->num_of_records)
        {
          ...
          if( m_usim_cache->index_array  && m_usim_cache->index_array[i] && k == (uint8)record->index)
          {
            UIM_MSG_HIGH_3("Updating index array for ADN loc %d usim-%d loc %d",
                            record->index, usim_cache->file_id, usim_loc);
            m_usim_cache->index_array[i][record->index] =  (uint8)usim_loc;
          }

          if ( usim_cache->pb_cache_array && usim_cache->pb_cache_array[usim_loc] && k == record->index)
          {
            usim_cache->pb_cache_array[usim_loc]->m_recid= pbm_location_to_adn_rec_id(session_type,pb_set,(uint8)record->index);
...
SIDE EFFECTS
===========================================================================*/
pbm_return_type pbm_index_read(
  pbm_session_enum_type session_type,
  pbm_file_type         file_id,
  int                   record_num,
  void                  (*proc_func) ( pbm_return_type,
...
SIDE EFFECTS
===========================================================================*/
void pbm_sim_write_cb(
  mmgsdi_return_enum_type status,
  const mmgsdi_cnf_type  *data_ptr
)
{
  uint32                         client_ref    = 0;
  PB_CACHE                      *cache         = NULL;      /* The cache has info on extension records. */
  pbm_file_type                  file_id       = PBM_FILE_NONE;
  pbm_g_other_field_info_s_type *field_info    = NULL;
  int                            i             = 0;
  int                            j             = 0;
  ...
  if (pbm_buffers[client_ref].extension != 0xFF)
  {
    if (cache->extension_cache)
    {
      ...
      /* since the ext record and adn record has been written succesfully updating
         g_sim_field_info_ptr number lengths */
      ...
      if(!empty_ext)
      {
        field_info = g_sim_field_info_ptr[session_type][0]; /* ADN info */
        if(field_info == NULL)
        {
          pbm_uim_write_cb(client_ref, PBM_ERROR);
          return;
        ...
        field_info = g_sim_field_info_ptr[session_type][1]; /* FDN info*/
        if(field_info == NULL)
        {
          pbm_uim_write_cb(client_ref, PBM_ERROR);
          return;
...
)
{
  ...
  if (pbm_buffers[client_ref].extension != 0xFF)
  {
    if (adn_cache_ptr[pbm_buffers[client_ref].record.session_type] &&
        adn_cache_ptr[pbm_buffers[client_ref].record.session_type]->extension_cache)
    {
      rex_enter_crit_sect(&pbm_crit_sect);
      adn_cache_ptr[pbm_buffers[client_ref].record.session_type]->extension_cache->in_use_array[pbm_buffers[client_ref].extension].uim_device =
                                                                                                                       usim_cache->file_id;

      adn_cache_ptr[pbm_buffers[client_ref].record.session_type]->extension_cache->in_use_array[pbm_buffers[client_ref].extension].index =
  ...
  do
  {
    ...
    switch (record->fields[field_index].field_id)
    {
      case PBM_FIELD_EMAIL:
        usim_cache = email_cache_ptr[session_type][pb_set];
        if(usim_cache == NULL)
        {
          *ret = PBM_ERROR; 
           break;
        ...
        buffer->record.index = (int)index_array[usim_cache->order];
        buffer->findex_delta = 1;
        ...
        if (buffer->state == PBM_WRITE_PREV_USIM)
        {
          buffer->record.data.text.adn_record_id = (pbm_adn_record_id_type)buffer->write->rec_id;
        }
        else
        {
        ...
        case PBM_FIELD_ADDITIONAL_NUMBER_EXT:
          ...
          *ret = pbm_usim_write_anr(buffer_index);
          ...
          case PBM_FIELD_OTHERNAME:
            usim_cache = sne_cache_ptr[session_type][pb_set];
            if(usim_cache == NULL)
            {
              *ret = PBM_ERROR;
               break;
          ...
          case PBM_FIELD_GRPNAME_ID:
            usim_cache = grp_cache_ptr[session_type][pb_set];
            if(usim_cache == NULL)
            {
              *ret = PBM_ERROR;
               break;
...
SIDE EFFECTS
===========================================================================*/
pbm_return_type pbm_sync_write_sync1 (
  uint32 buffer_index
)
{
  byte                         *gsdi_buffer    = NULL;
  ...
  pbm_session_enum_type         session_type   = PBM_SESSION_MAX;
  pbm_file_type                 file_id        = PBM_FILE_NONE;
  pbm_return_type               pbm_retval     = PBM_ERROR_MAX;
  PB_SYNC1_CACHE               *sync_cache     = NULL;
  ...
  switch (file_id)
  {
    ...
    case PBM_FILE_CC:
    {
      ...
      gsdi_buffer[0] = (byte)((data & 0xFF00) >> 8);
      gsdi_buffer[1] = (byte)(data & 0x00FF);
      break;
    }
    ...
    case PBM_FILE_PSC:
    {
      ...
      gsdi_buffer[0] = (byte)((data & 0xFF000000) >> 24);
      gsdi_buffer[1] = (byte)((data & 0x00FF0000) >> 16);
      gsdi_buffer[2] = (byte)((data & 0x0000FF00) >> 8);
      gsdi_buffer[3] = (byte)((data & 0x000000FF));
      break;
    }
...
pbm_return_type pbm_iap_write (uint32 buffer_index)
{
   ...
   m_usim_cache = adn_m_usim_cache_ptr[buffer->record.session_type][pb_set];
   if (m_usim_cache == NULL || !m_usim_cache->iap_num_records)
   {
      UIM_MSG_ERR_0("IAP file not present");
      return PBM_ERROR;
   }
...
SIDE EFFECTS
===========================================================================*/
pbm_return_type pbm_usim_write_text (
  uint32  buffer_index,
  boolean gsm7_only
)
{
  ...
  PB_USIM_CACHE                  *usim_cache   = NULL;
  pbm_async_gsdi_buffer          *buffer       = NULL;
  mmgsdi_file_structure_enum_type filetype     = MMGSDI_MAX_FILE_STRUCTURE_ENUM;
  pbm_session_enum_type           session_type = PBM_SESSION_MAX;
  pbm_return_type                 pbm_retval   = PBM_ERROR_MAX;
  uint16                          text_size    = 0;
  pbm_text_record_s_type         *text_record  = NULL;
...
pbm_return_type pbm_usim_write_anr (uint32 buffer_index)
{
   ...
   pbm_async_gsdi_buffer *buffer       = NULL;
   pbm_session_enum_type  session_type = PBM_SESSION_MAX;
   pbm_anr_record_s_type *anr_record   = NULL;
   ...
   /* Do we need an extension record?  */
   ext_cache = ext1_cache_ptr[session_type];
   if(ext_cache == NULL)
   {
     return PBM_ERROR;
   }
...
pbm_return_type pbm_usim_write_grp_id (uint32 buffer_index)
{
   ...
   mmgsdi_file_structure_enum_type  filetype     = MMGSDI_MAX_FILE_STRUCTURE_ENUM;
   pbm_session_enum_type            session_type = PBM_SESSION_MAX;
   pbm_return_type                  pbm_retval   = PBM_ERROR;
   int                              i            = 0;
   ...
   for (i = 0; i < usim_cache->record_length; i++)
   {
      ...
      */
      if (buffer->record.data.grp.group_id[i] > 0 &&
          grpname_cache_ptr[session_type] != NULL &&
          buffer->record.data.grp.group_id[i] < grpname_cache_ptr[session_type]->num_of_records)
      {
...
{
   byte                            *gsdi_buffer  = NULL;
   PB_MASTER_USIM_CACHE            *index_cache  = NULL;
   pbm_async_gsdi_buffer           *buffer       = NULL;
   mmgsdi_file_structure_enum_type  filetype     = MMGSDI_MAX_FILE_STRUCTURE_ENUM;
   pbm_session_enum_type            session_type = PBM_SESSION_MAX;
   pbm_return_type                  pbm_retval   = PBM_ERROR;
...
)
{
  ...
  PBM_CHECK_PTR4_RET(pb_cache, pbc_cache_ptr[session_type],
                     pb_cache->m_sync_cache, pbc_cache_ptr[session_type]->data_array, VOID);
  pb_cache->m_sync_cache->reset_pbc = TRUE;

  for (i = index; i <= pbc_cache_ptr[session_type]->num_of_records; i++)
  {
    if (pbc_cache_ptr[session_type]->data_array[i] & PBM_PBC_MODIFIED_MASK)
    {
      pb_id = pbm_device_type_to_phonebook_type(session_type, PBM_ADN);
...
)
{
  ...
  index = pbm_record_id_to_location((uint16)cb_data->sess_rec_id);
...
SIDE EFFECTS
===========================================================================*/
pbm_return_type pbm_delete_usim_fields (
...
SIDE EFFECTS
===========================================================================*/
void pbm_usim_read_cb (
  pbm_return_type        pbm_returned,
  pbm_sim_record_s_type *record
)
{
  PB_USIM_CACHE         *usim_cache   = NULL;
  pbm_session_enum_type  session_type = PBM_USIM_PB_INVALID;
  pbm_file_type          file_id      = 0;
  int                    record_num   = 0;
  pbm_pb_set_type        pb_set       = PBM_USIM_PB_INVALID;
  ...
  if (usim_cache != NULL)
  {
    ...
    if ( pb_set == PBM_USIM_PB_ONE )
    {
      ...
      if (usim_cache->num_of_records)
      {
        ...
        pbm_g_other_field_info_s_type *field_info = g_sim_field_info_ptr[session_type][0];
