...
===========================================================================*/
/*-------------------------------------------------------------------------------------
     Copyright (c) 2002 - 2014, 2019, 2021 - 2022 QUALCOMM Technologies, Inc(QTI).
     All rights reserved.
...
$Header: //components/rel/uim.mpss/6.1.0/api/pbmlib.h#4 $
...
typedef enum
{
   ...
   PBM_ERROR_DEPRECATED,          /**< Record is locked -- Deprecate */
...
typedef pbm_return_type pbm_return_T;
...
{
   PBM_SLOT_NONE  = 0,
   PBM_FIRST_SLOT,               /**< Represents slot 1. */
...
typedef struct
{
  ...
  boolean                   fdn_status;  /** <Denotes the FDN status */
} pbm_phonebook_type;
...
/*=============================================================================
Field for serving mcc based emergency numbers
=============================================================================*/
/** @name Fields for serving mcc based Emergency Numbers
    @{ */
#define PBM_FIELD_SERVING_MCC_HARDCODED_ECC                  ((pbm_field_id_e_type) 0xFF5A)
  /**< Field value is an emergency number that is based on serving MCC. */
...
typedef enum
{
   ...
   PBM_EVENT_DEPRECATED_1,             /**< PBM is locked (data: lock_type) -- Deprecated */
   PBM_EVENT_DEPRECATED_2,             /**< PBM is unlocked (data: lock_type) -- Deprecated */
   ...
   /* 13 Please update this number if new event is added */
...
typedef union {
  ...
  pbm_device_type      pb_id;           /**< Phone book ID. */
   /**< ID 1 for slot 1. Add 1 for each additional slot. */
...
/**
    ...
    special_emergency_number_ptr will be set to TRUE if the number is emergency but configured
    as special emergemcy number(meaning srv_staus_flag byte is set in NV 73755 for the
    corresponing number). Also set special_emergency_number_subs_mask_ptr to notify
    on which SUB this number has to treat as differently based on service status & roam status.

    special_emergency_number_in_serving_mcc will be set to TRUE if the number is emergency but configured
    as special emergemcy number in serving mcc based emergency table.
...
typedef struct {
  ...
  boolean                    special_emergency_number;
  boolean                    special_emergency_number_in_serving_mcc;
  uint8                      special_emergency_number_subs_mask;
} pbm_ecc_call_resolution_type;
...
/*===========================================================================
FUNCTION PBM_SESSION_RECORD_READ_EXT
...
SIDE EFFECTS
===========================================================================*/
pbm_return_type pbm_session_record_read_ext(
  pbm_record_id_type            rec_id,
  uint16                       *category_ptr,
  int                          *num_fields_ptr,
  uint8                        *data_buf,
  uint32                        data_buf_size,
  pbm_slot_type                *ecc_slot_ptr);
...
DESCRIPTION
  This function searches the ECC phonebook for the number just passed in
  across all slots to determine if the current number is an Emergency number.
