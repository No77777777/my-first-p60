...
Copyright (c) 2008 - 2017, 2019, 2024 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
$Header: //components/rel/uim.mpss/6.1.0/api/uim_v.h#4 $
...
12/21/15   na      Do not allow silent recovery command in passive mode
...
typedef enum
{
  ...
  UIM_RESELECT_CUR_DIR_R, /* Select current direcotry of UIM */
  ...
  UIM_ISIM_AUTHENTICATE_R,                  /* ISIM Authentication report */
  ...
  UIM_STORE_ESN_MEID_ME_R,                  /* Store ESN_MEID_ME response */
  UIM_SET_APDU_BEHAVIOR_R,                  /* Transmit Behavior response */
  UIM_MAX_R
...
typedef enum {
  ...
  SW2_LAST_CMD_OF_CHAIN_EXP     = 0x83,
  ...
  SW2_UNDEF_99           = 0x99,
  SW2_MORE_DATA          = 0xF1,
  SW2_MORE_DATA_AND_FETCH = 0xF2, /* More data Available and Fetch */
...
typedef enum {
  ...
  UIM_RESELECT_CUR_DIR_F,        /* Reselt the current directory*/
  ...
  UIM_SET_APDU_BEHAVIOR_F,       /* Set the transmit behavior */
...
typedef enum {
  ...
  UIM_NULL_BYTE_ERR_S,
  UIM_CMD_TIMEOUT_ERR_S,      /*Non-telecom(SAM card) card timed out or interface error*/
  UIM_MAXIMUM_S
} uim_status_type;
...
typedef enum {
  ...
  UIM_FLUSH_COMPLETE     /* Flush all commadns including power_down and reset F*/
} uim_flush_type;

typedef enum {
  UIM_APDU_BEHAVIOR_FOR_TELECOM_APP = 0,
  /* APDU behavior for telecom app to issue GET RESPONSE for
     warning status words 62XX/63XX.
  */
  UIM_APDU_BEHAVIOR_NO_GET_RESPONSE_FOR_WARNING,
  /* For non-telecom applications, APDU behavior is to not issue automatic 
     GET RESPONSE command for warning status words 62XX/63XX.
  */
  UIM_APDU_BEHAVIOR_AUTOMATIC_GET_RESPONSE_FOR_WARNING
  /* For non-telecom applications, APDU behavior is to issue automatic 
     GET RESPONSE command for warning status words 62XX/63XX. 
  */
} uim_apdu_behavior_type;
...
typedef struct {
  ...
  boolean               (*test_mode_req_log_masking_fn_ptr)(const uint8 *input_ptr, uint8 *output_ptr, 
                                                            uint32 len);
  boolean               (*test_mode_rsp_log_masking_fn_ptr)(const uint8 *input_ptr, uint8 *output_ptr,
                                                            uint32 len);  
} uim_hdr_type;
...
typedef struct {
  ...
  uim_sw2_type         sw2;
  int                  cmd_status;
  uim_path_type        original_select_path;
  uim_file_type        original_file_type;
...
/* set transmit behavior command type*/
typedef struct {
  uim_hdr_type                         hdr;
  uim_apdu_behavior_type               apdu_behavior;
} uim_set_transmit_behavior_type;

/* options for type of card pup after reset */
typedef enum {
  UIM_PUP_OPTION_TELECOM,    /* Regular card pup - telecom card powerup */
  UIM_PUP_OPTION_NON_TELECOM, /* Pass through mode power up - SAM card mode powerup */
  UIM_PUP_OPTION_TELECOM_NOT_PUP_IF_ALREADY /* Do not power up if already powered up */
} uim_pup_option_type;

/* Power down command type*/
typedef struct {
  uim_hdr_type        hdr;
  uim_pup_option_type pup_option;
} uim_reset_cmd_type;
...
typedef union {
  ...
  uim_set_transmit_behavior_type        set_apdu_behavior;      /* Set APDU behavior */
  uim_reset_cmd_type                    reset;
} uim_cmd_type;
