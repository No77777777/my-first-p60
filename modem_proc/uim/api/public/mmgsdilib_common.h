...
/*=============================================================================
Copyright (c) 2010-2019 by QUALCOMM Technologies, Inc(QTI).
All Rights Reserved.
...
$Header: //components/rel/uim.mpss/6.1.0/api/public/mmgsdilib_common.h#4 $$ $DateTime: 2020/06/12 06:34:27 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     -----------------------------------------------------------
06/12/20   sg      Fix compilation issue by defining 5G network mask
...
08/21/18   vdc     Fix compilation issue when PS data OFF feature not present
...
04/19/17   tq      Added enums for ACDC,ProSe,Launch Pad,MCPTT and ePDG files
...
06/14/16   av      Expand card error enum to indicate max error codes
...
03/28/16   kv      Added sync API to extract remote SIM Params
03/16/16   gm      Removed "FEATURE_TABASCO_MODEM"
03/15/16   vr      SAM CARD support
02/12/16   vdc     Halt initialization if IMSI is not provisioned in card
...
01/14/16   vdc     Fix compilation issue
11/10/15   yt      Remove prov_app field from mmgsdi_app_info_type
...
06/03/15   hh      Convert MANAGE CHANNEL APDU to MMGSDI requests
...
/** Service category for ProSe in USIM. */
#define MMGSDI_USIM_PROSE_SRV                        0x1000
...
#define MMGSDI_TELECOM_MCPTT_DF_START                 0x35
...
#define MMGSDI_USIM_PROSE_DF_START                    0x46
#define MMGSDI_USIM_ACDC_DF_START                     0x47
...
#define MMGSDI_CD_DF_START                            0xC0
#define MMGSDI_NO_FILE_ENUM_START                     0xD0
...
=============================================================================*/
  ...
  typedef enum {
    ...
    /* 90 */
    MMGSDI_NOTIFY_SIM_PRE_INIT_REQ,
    MMGSDI_HIDE_CSIM_REQ,
    MMGSDI_MAP_SESSIONS_TO_SLOTS_REQ,
    MMGSDI_MANAGE_CHANNEL_REQ,
    MMGSDI_CONTINUE_DEACTIVATE_REQ,
    MMGSDI_NOTIFY_RECOVERY_COMPLETE_REQ,
    MMGSDI_START_POTENTIAL_SILENT_FILE_CHANGE_REQ,
    MMGSDI_NOTIFY_RECOVERY_START_REQ,
    MMGSDI_PURGE_TEMPORARY_IDENTITY_REQ,
    MMGSDI_RESTRICTED_SIM_ACCESS_REQ,

    /* 100 */
    MMGSDI_QSH_ACTION_EVENT_CFG_REQ,
    MMGSDI_SET_APDU_BHEAVIOR_REQ,
...
#define MMGSDI_SESSION_PROV_APP_INIT_COMPLETE_CNF          MMGSDI_SESSION_PROV_APP_INIT_COMPLETE_REQ
...
#define MMGSDI_HIDE_CSIM_CNF                               MMGSDI_HIDE_CSIM_REQ
#define MMGSDI_MAP_SESSIONS_TO_SLOTS_CNF                   MMGSDI_MAP_SESSIONS_TO_SLOTS_REQ
#define MMGSDI_MANAGE_CHANNEL_CNF                          MMGSDI_MANAGE_CHANNEL_REQ
#define MMGSDI_PURGE_TEMPORARY_IDENTITY_CNF                MMGSDI_PURGE_TEMPORARY_IDENTITY_REQ
...
#define MMGSDI_SET_APDU_BHEAVIOR_CNF                       MMGSDI_SET_APDU_BHEAVIOR_REQ
...
typedef enum {
  ...
  MMGSDI_MAPPING_IN_PROGRESS,         /**< A request of MAP_SESSIONS_TO_SLOTS api
                                          is in progress */
  MMGSDI_WRONG_FCP_TEMPLATE_TAG,      /**< Incorrect FCP template tag is received
                                           in SELECT response */
...
*/
typedef enum {
  /* MAJOR EXISTING CARD RELATED EVENTS (RECEIVED WITH CLIENT ID Event REG).
     Do not add any new card related event here; add it in the appropriate
     block below */
  MMGSDI_CARD_INSERTED_EVT        = 0x00000000,   /**< Card inserted. */
  MMGSDI_CARD_ERROR_EVT           = 0x00000001,   /**< Card error. */
  MMGSDI_CARD_REMOVED_EVT         = 0x00000002,   /**< Card removed. */
  MMGSDI_SIM_BUSY_EVT             = 0x00000003,   /**< SIM busy. */

  /* MAJOR EXISTING APP RELATED EVENTS (RECEIVED WITH Session ID Reg). Do not
     add any new card related event here; add it in the appropriate block
     below */
  MMGSDI_SUBSCRIPTION_READY_EVT   = 0x0000000C,
                                    /**< Subscription ready event. After
                                         PIN1_EVT and the session ID are
                                         assigned to a channel, the client
                                         can start making requests of the
                                         application. */
  MMGSDI_SESSION_CHANGED_EVT      = 0x0000000D,
                                    /**< Session changed event.The provisioning
                                         application changed, so the client's
                                         session is remapped to the new
                                         provisioning application automatically
                                         . Opening a session for a non
                                         provisioning application by using an
                                         AID can also produce this event. */
  MMGSDI_PIN1_EVT                 = 0x0000000E,
                                    /**< PIN1 authentication event. */
  MMGSDI_PIN2_EVT                 = 0x0000000F,
                                    /**< PIN2 authentication event. */
  MMGSDI_UNIVERSAL_PIN_EVT        = 0x00000010,
                                    /**< Universal PIN authentication event. */
  MMGSDI_REFRESH_EVT              = 0x00000011,
                                    /**< Application or card refresh event. */
  MMGSDI_PERSO_EVT                = 0x00000013,
                                    /**< Personalization event. */
  MMGSDI_EONS_EVT                 = 0x00000017,
                                    /**< EONS Event. */

  /* NEWER CARD RELATED EVENTS (RECEIVED WITH CLIENT ID Event REG).
     Any new card related event should be added in this block.
     When adding a new event here, also consider adding it to
     uimdiag_mmgsdi_event_map_table in UIMDIAG */
  MMGSDI_CARD_RELATED_EVTS_EXT      = 0x00000020,
  MMGSDI_DEPRECATED_EVT             = MMGSDI_CARD_RELATED_EVTS_EXT,
                                    /**< MMGSDI_TEST_SIM_EVT is deprecated. */
  MMGSDI_PROFILE_DATA_EVT,          /**< Profile data */
  MMGSDI_SAP_CONNECT_EVT,           /**< SAP is connected as a server
                                         or client. */
  MMGSDI_SAP_DISCONNECT_EVT,        /**< SAP server or client was
                                         disconnected. */
  MMGSDI_CARD_DETECTION_STARTED_EVT,/**< Card detection procedure started. */
  MMGSDI_RECOVERY_COMPLETE_EVT,     /**< Card recovery successfully
                                         completed */
  MMGSDI_PERSO_TEMPORARY_UNLOCK_EVT,/**< Perso Temporary unlock
                                         information event*/
  MMGSDI_TERMINAL_PROFILE_DL_EVT,   /**< Terminal Profile DL event */
  MMGSDI_PROFILE_SWITCHING_EVT,     /** < Switching of profile in the card is
                                          in progress */

  /* NEWER SESSION RELATED EVENTS (RECEIVED WITH Session ID Reg).
     Any new session related event should be added in this block.
     When adding a new event here, also consider adding it to
     uimdiag_mmgsdi_event_map_table in UIMDIAG */
  MMGSDI_SESSION_RELATED_EVTS_EXT   = 0x00000080,
  MMGSDI_SESSION_CLOSE_EVT          = MMGSDI_SESSION_RELATED_EVTS_EXT,
                                    /**< Session was closed. */
  ...
  MMGSDI_SESSION_LEGAL_SUBSCRIPTION_EVT,
                                    /**< Updating an illegal subscription
                                         back as ready */
  MMGSDI_PURGE_TEMPORARY_IDENTITY_STATUS_EVT,
                                    /**< Updating an purge temporary
                                         identity status */
...
typedef enum {
  ...
  /* (0x35)00 => 0 */
  MMGSDI_TELECOM_MCPTT_MST   = (MMGSDI_TELECOM_MCPTT_DF_START << 8),
                                   /**< MCPTT Service Table. */
...
/* EFs for MMS Notification */
  ...
  MMGSDI_USIM_IPS,                /**< IMEI(SV) Pairing status */
  MMGSDI_USIM_EPDG_ID,            /**< Home ePDG Identifier */
  MMGSDI_USIM_EPDG_SELECTION,     /**< ePDG Selection Information */
  MMGSDI_USIM_EPDG_IDEM,          /**< Emergency ePDG Identifier */
  MMGSDI_USIM_EPDG_SELECTION_EM,  /**< ePDG Selection Information for Emergency Services */  
  ...
  MMGSDI_USIM_PS_DATA_OFF,        /**< 3GPP PS Data Off. */
  ...
  /* ProSe EFs */
  /* (0x46)00 => 0 */
  MMGSDI_USIM_PROSE_MON     = (MMGSDI_USIM_PROSE_DF_START << 8),
                              /**< ProSe Monitoring Parameters. */
  MMGSDI_USIM_PROSE_ANN,      /**< ProSe Announcing Parameters. */
  MMGSDI_USIM_PROSE_FUNC,     /**< HPLMN ProSe Function. */
  MMGSDI_USIM_PROSE_RADIO_COM,/**< ProSe Direct Communication Radio Parameters */
  MMGSDI_USIM_PROSE_RADIO_MON,/**< ProSe Direct Discovery Monitoring Radio Parameters */
  MMGSDI_USIM_PROSE_RADIO_ANN,/**< ProSe Direct Discovery Announcing Radio Parameters */
  MMGSDI_USIM_PROSE_POLICY,   /**< ProSe Policy Parameters */
  MMGSDI_USIM_PROSE_PLMN,     /**< ProSe PLMN Parameters */
  MMGSDI_USIM_PROSE_GC,       /**< ProSe Group Counter */
  MMGSDI_USIM_PROSE_PST,      /**< ProSe Service Table */

  /* (0x46)0A => 10 */
  MMGSDI_USIM_PROSE_UIRC,     /**< ProSe Usage Information Reporting Configuration */
  MMGSDI_USIM_PROSE_GM_DICOVERY,    /**< ProSe Group Member Discovery Parameters. */
  MMGSDI_USIM_PROSE_RELAY,          /**< ProSe Relay Parameters. */
  MMGSDI_USIM_PROSE_RELAY_DISCOVERY,/**< ProSe Relay Discovery Parameters */

  /* ACDC EFs */
  /* (0x47)00 => 0 */
  MMGSDI_USIM_ACDC_LIST	 = (MMGSDI_USIM_ACDC_DF_START << 8),
                               /**< ACDC List. */
  ...
  MMGSDI_ISIM_IST,            /**< ISIM Service Table. */
  MMGSDI_ISIM_PCSCF,          /**< P-CSCF Address. */
  MMGSDI_ISIM_GBANL,          /**< GBA NAF List. */

  /* (0x6)0A => 10 */
  MMGSDI_ISIM_NAFKCA,         /**< NAF Key Centre Address. */
  MMGSDI_ISIM_SMS,            /**< Short messages. */
  MMGSDI_ISIM_SMSS,           /**< SMS status. */
  MMGSDI_ISIM_SMSR,           /**< Short message status reports. */
  MMGSDI_ISIM_SMSP,           /**< Short message service parameters. */
  MMGSDI_ISIM_UICCIARI,       /**< UICC IARI. */
  ...
  /* EFs at the Configuration data DF */
  /* (0x0C)00 => 0 */
  MMGSDI_CD_LAUNCH_PAD	   = (MMGSDI_CD_DF_START << 8),
                                  /**LAUNCH PAD */
...
/*===========================================================================
   ENUM:      MMGSDI_SLOT_ENUM_TYPE
=============================================================================*/
/** Type of the actual card belonging to the Slot. */
typedef enum {
  MMGSDI_CARD_TYPE_NONE       = 0x00000000,
  MMGSDI_CARD_LOCAL           = 0x00000001, /**< Local card is the one that is
                                            physically connected to the modem. */

  MMGSDI_CARD_REMOTE          = 0x00000002  /**< Remote card is the one that is
                                            accessed remotely by the modem. */
}mmgsdi_slot_enum_type;
...
typedef enum {
  ...
  MMGSDI_CARD_ERR_SAP_CONNECTED,               /**< Card unavailable as SAP is
                                                    connected */
  MMGSDI_CARD_ERR_CMD_TIMEOUT,                 /**< Command timeout */
  MMGSDI_CARD_ERR_COUNT,                       /**< Number of error values
                                                    above. Any new error value
                                                    should added before this */
...
typedef enum {
  ...
  MMGSDI_CARD_POWER_UP_NON_TELECOM,         /**< Performs an initial power up as NON TELECOM. */
...
MMGSDI_GSM_SRV_DATA_DL_SMSCB,         /**< Data Download via Short Message
  ...
  MMGSDI_USIM_SRV_RFU3,            /**< RFU. */
  MMGSDI_USIM_SRV_URI_SUPPORT,     /**< URI support by UICC. */

  /* 0x464 OR 0x400+100 */
  MMGSDI_USIM_SRV_EARFCN,          /**< Extended EARFCN support. */
  MMGSDI_USIM_SRV_PROSE,           /**< ProSe Service Table. */
  MMGSDI_USIM_SRV_USAT_APP_PAIRING,       /**< USAT Application Pairing */
  MMGSDI_USIM_SRV_MEDIA_TYPE,             /**< Media Type support */
  MMGSDI_USIM_SRV_IMS_DISCONNECT_CAUSE,   /**< IMS call disconnection cause */
  MMGSDI_USIM_SRV_URI_SUPPORT_SMS_CC,     /**< URI support for MO SHORT MESSAGE CONTROL */
  MMGSDI_USIM_SRV_EPDG_CONFIG_SUPPORT,    /**< ePDG configuration Information Support */
  MMGSDI_USIM_SRV_EPDG_CONFIG_CONFIG,     /**< ePDG configuration Information Config */
  MMGSDI_USIM_SRV_ACDC,                   /**< ACDC support */
  MMGSDI_USIM_SRV_MCPTT,                  /**< MCPTT support */

  /* 0x46E OR 0x400+110 */
  MMGSDI_USIM_SRV_EM_EPDG_CONFIG_SUPPORT, /**< ePDG configuration Information for Emergency Service Support */
  MMGSDI_USIM_SRV_EM_EPDG_CONFIG_CONFIG,  /**< ePDG configuration Information for Emergency Service Configured */
  ...
  /* 0x1000 OR 0x1000+0 */
  MMGSDI_USIM_PROSE_SRV_PARAMS = MMGSDI_USIM_PROSE_SRV,
                                      /**< ProSe Monitoring Parameters. */
  MMGSDI_USIM_PROSE_SRV_FUNC,         /**< HPLMN ProSe Function. */
  MMGSDI_USIM_PROSE_SRV_RADIO_COM,    /**< ProSe Direct Communication Radio Parameters */
  MMGSDI_USIM_PROSE_SRV_RADIO_MON,    /**< ProSe Direct Discovery Monitoring Radio Parameters */
  MMGSDI_USIM_PROSE_SRV_RADIO_ANN,    /**< ProSe Direct Discovery Announcing Radio Parameters */
  MMGSDI_USIM_PROSE_SRV_POLICY,       /**< ProSe Policy Parameters */
  MMGSDI_USIM_PROSE_SRV_GC,           /**< ProSe Group Counter */
  MMGSDI_USIM_PROSE_SRV_UIRC,         /**< ProSe Usage Information Reporting Configuration */
  MMGSDI_USIM_PROSE_SRV_COM_UIR,      /**< ProSe Communication Usage Information Reporting */
  MMGSDI_USIM_PROSE_SRV_GM_DISCOVERY, /**< ProSe Group Member Discovery parameters */
  
  /* 100A OR 0x1000 + 10 */
  MMGSDI_USIM_PROSE_SRV_RELAY,	      /**< ProSe Relay parameters */
...
/*===========================================================================
  ENUM:   MMGSDI_TEMPORARY_IDENTITY_ENUM_TYPE
===========================================================================*/
/** Enums for temporary identity
*/
typedef enum {
  MMGSDI_PURGE_TEMPORARY_IDENTITY_UNKNOWN,
  MMGSDI_PURGE_TEMPORARY_IDENTITY_GUTI
}mmgsdi_temporary_identity_enum_type;

/*===========================================================================
   ENUM:      MMGSDI_CARD_TYPE_ENUM_TYPE
=============================================================================*/
/** Type of the actual card belonging to the Slot. */
typedef enum {
  MMGSDI_CARD_TELECOM,     /**< Telecom card */
  MMGSDI_CARD_NON_TELECOM, /**< Non telecom card. */
  MMGSDI_CARD_NONE = 0x0F
}mmgsdi_card_type_enum_type;

/*===========================================================================
   ENUM:      MMGSDI_TRANSPORT_PROTOCOL_ENUM_TYPE
=============================================================================*/
/** Type of the protocol supported by the card. */
typedef enum {
  MMGSDI_T_0_PROTOCOL, /**< T = 0 protocol */
  MMGSDI_T_1_PROTOCOL, /**< T = 1 protocol */
  MMGSDI_NO_T_PROTOCOL = 0x0F
}mmgsdi_transport_protocol_enum_type;
...
*/
typedef uint32 mmgsdi_len_type;
...
*/
typedef uint32 mmgsdi_rec_num_type;
...
*/
typedef uint32 mmgsdi_offset_type;
...
*/
typedef struct {
  uint8              data_len;                      /**< Length of the data. */
...
typedef struct {
  ...
  mmgsdi_slot_enum_type     slot_type; /** Type of card in the slot */
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_PERSO_TEMP_UNLOCK_INFO_TYPE
-----------------------------------------------------------------------------*/
...
typedef struct {
  ...
  uint8                         iccid[MMGSDI_ICCID_LEN];
                                                /**< ICCID of the card */
  boolean                       is_euicc;       /**< Card is eUICC or not */
  mmgsdi_card_type_enum_type    card_type;      /**< Type of the card: telecom or non-telecom */
  uint8                         eid_len;        /**< Length of EID of the eUICC */
  uint8                         eid[MMGSDI_EID_LEN];
                                                /**< EID of the eUICC */
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_CARD_DETECTION_STARTED_EVT_INFO_TYPE
-----------------------------------------------------------------------------*/
/** @brief Information about a Card detection started event.
*/
typedef struct {
  mmgsdi_slot_id_enum_type slot; /**< Slot in which card detection is initiated. */
} mmgsdi_card_detection_started_evt_info_type;
...
typedef struct {
  ...
  union {
    mmgsdi_rec_num_type             rec_num;     /**< Record number accessed for
                                                     linear fixed or cyclic
                                                     file. */
    mmgsdi_offset_type              offset;      /**< Offset accessed for
                                                     transparent file. */
  } file_index;
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_RECOVERY_COMPLETE_EVT_INFO_TYPE
-----------------------------------------------------------------------------*/
/** @brief Information about profile data event.
*/
typedef struct {
  mmgsdi_slot_id_enum_type      slot;        /**< Slot on which the recovery is
                                                  successfully completed */
} mmgsdi_recovery_complete_evt_info_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_PURGE_TEMPORARY_IDENTITY_EVT_INFO_TYPE
-----------------------------------------------------------------------------*/
/** @brief Information about purge temporary identity status event.
*/
typedef struct {
  mmgsdi_return_enum_type                  status;              /**< status of the purge request */
  mmgsdi_temporary_identity_enum_type      temporary_identity;  /**< temporary identity. */
} mmgsdi_purge_temporary_identity_evt_info_type;
...
typedef struct {
  ...
  union {
    ...
    mmgsdi_session_close_evt_info_type                  session_close;
          /**< Session close event. */
    mmgsdi_card_detection_started_evt_info_type         card_detection;
          /**< Card detection started event. */
    ...
    mmgsdi_recovery_complete_evt_info_type              recovery_complete;
          /**< Recovery successfully completed event. */
    mmgsdi_profile_switch_evt_info_type                 profile_switch;
          /**< Profile switching successful event */
    mmgsdi_purge_temporary_identity_evt_info_type       purge_temporary_identity;
          /**< Purge temporary identity complete event */
  } data;
...
*/
typedef uint32 mmgsdi_file_size_type;
...
/* ----------------------------------------------------------------------------
   DATA TYPE:      MMGSDI_CHANNEL_ID_TYPES
-----------------------------------------------------------------------------*/
/** Channel id
*/
...
/* ----------------------------------------------------------------------------
   UNION:      MMGSDI_FILE_SECURITY_ATTRIBUTES_TYPE
-----------------------------------------------------------------------------*/
/** Used by the mmgsdi_file_security_attributes_type structure to provide additional
    file attributes.
*/
typedef union {
  ...
  mmgsdi_cyclic_file_info_type       cyclic_file;       /**< Cyclic file
                                                             attributes. */
} mmgsdi_file_security_attributes_type;
...
typedef struct {
  ...
  boolean                              is_arr_skipped;  /**< Used to keep track if the security
                                                             attributes is decoded or default attributes */
  uint16                               arr_file_id;     /**< Keeps track of security attribute's ARR 
                                                             file id. Value of 0 indicates that file does reference
                                                             ARR for security attributes */
  uint8                                arr_record_no;   /**< Record numbder of referenced record ARR file
                                                             to decode security attributes */
  mmgsdi_file_security_attributes_type file_info;       /**< Contains security attributes for the EF */
...
typedef struct {
  ...
  uint32                      max_size;             /**< Maximum size of the
                                                         data. */
  mmgsdi_channel_id_type      channel_id;           /**< Logical channel ID. */
...
typedef struct {
  ...
  mmgsdi_channel_id_type      channel_id;  /**< Channel ID. */
...
typedef struct {
  ...
  mmgsdi_slot_id_enum_type       slot_id;          /**< Slot ID. */
  mmgsdi_channel_id_type         channel_id;       /**< Logical channel ID that
                                                        is assigned to the
                                                        application. */
...
typedef struct {
  ...
  mmgsdi_channel_id_type         channel_id;   /**< If application is active on
                                                 Session, Channel ID
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_HIDE_CSIM_CNF_TYPE
-----------------------------------------------------------------------------*/
/** @brief Used when the hide csim request is acknowledged by MMGSDI.
*/
typedef struct {
  mmgsdi_response_header_type response_header; /**< Hide CSIM response
                                                    header information. */
} mmgsdi_hide_csim_cnf_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_MAP_SESSIONS_TO_SLOTS_CNF_TYPE
-----------------------------------------------------------------------------*/
/** @brief Used when MMGSDI responds to MMGSDI_MAP_SESSIONS_TO_SLOTS_REQ.
*/
typedef struct {
  mmgsdi_response_header_type  response_header; /**< Header information for the
                                                     response. */
} mmgsdi_map_sessions_to_slots_cnf_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_PURGE_TEMPORARY_IDENTITY_CNF_TYPE
-----------------------------------------------------------------------------*/
/** @brief Used when MMGSDI responds to MMGSDI_PURGE_TEMPORARY_IDENTITY_REQ
*/
typedef struct{
  mmgsdi_response_header_type  response_header; /**< Header information
                                                     for response */
} mmgsdi_purge_temporary_identity_cnf_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      MMGSDI_SET_APDU_BEHAVIOR_CNF_TYPE
-----------------------------------------------------------------------------*/
/** @brief Used when MMGSDI responds to MMGSDI_SET_APDU_BEHAVIOR_REQ
*/
typedef struct{
  mmgsdi_response_header_type  response_header; /**< Header information
                                                     for response */
} mmgsdi_set_apdu_behavior_cnf_type;
...
typedef struct {
  mmgsdi_response_header_type  response_header; /**< Manage Channel response
                                                     header information. */
  mmgsdi_channel_id_type       channel_id;      /**< channel_id that is opened,
...
typedef union {
  ...
  mmgsdi_set_sim_profile_cnf_type             set_sim_profile_cnf;
         /**< Set SIM profile  */
  mmgsdi_hide_csim_cnf_type                   hide_csim_cnf;
         /**< Hide CSIM request */
  mmgsdi_map_sessions_to_slots_cnf_type       map_sessions_to_slots_cnf;
         /**< Sessions to slots mapping */
  mmgsdi_manage_channel_cnf_type              manage_channel_cnf;
         /**< Manage Channel request */
  mmgsdi_purge_temporary_identity_cnf_type    purge_temporary_identity_cnf;
         /**< Purge temporary identity request */
  mmgsdi_set_apdu_behavior_cnf_type           set_apdu_behavior_cnf;
         /**< Set APDU behavior request */         
