...
Copyright (c) 2008-2017 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/api/gstk_exp_v.h#2 $
...
04/29/16   gs      Memory reduction for IoE
...
06/15/15   bd      Get active data subscription information from
...
typedef enum
{
  ...
  GSTK_CM_PH_EVENT_AVAILABLE_NETWORKS_CONF  = 0x03,
  /* Invalid CM_PH event For Internal use */
...
--  Enum: gstk_io_ctrl_enum_type
      ...
      gstk_io_ctrl() function. each enum is used for below purpose:
      ...
      20)GSTK_IO_SIM_INITIATED_MODIFIED_CC_RESP: Copy the call control response
         ...
         modified to another call type. Used by CM.
...
typedef enum {
  ...
  GSTK_IO_RESET_WMS_IS_CP_ACK_PENDING_FLAG,
  GSTK_IO_GET_WMS_IS_CP_ACK_PENDING_FLAG,
...
/*===========================================================================
--  Enum: gstk_call_evt_trigger_type
      Trigger for the Call Event
      GSTK_CALL_TRIGGER_INVALID
        Invalid Call Event trigger
      GSTK_CALL_TRIGGER_CM_CDMA
        CDMA call event received from CM
      GSTK_CALL_TRIGGER_CM_GW
        GW call event received from CM
      GSTK_CALL_TRIGGER_IMS_INT
        IMS call event received from CM
      GSTK_CALL_TRIGGER_IMS_EXT
        IMS call event received from external IMS client
===========================================================================*/
typedef enum
{
   GSTK_CALL_TRIGGER_INVALID    = 0x00,
   GSTK_CALL_TRIGGER_CM_CDMA,
   GSTK_CALL_TRIGGER_CM_GW,
   GSTK_CALL_TRIGGER_IMS_INT,
   GSTK_CALL_TRIGGER_IMS_EXT
}
gstk_call_evt_trigger_type;
...
typedef enum
{
  ...
  GSTK_DS_SYS_NETWORK_3GPP2,    /**< Network type 3gpp2 */
  GSTK_DS_SYS_NETWORK_WLAN,     /**< Network type WLAN */
...
typedef struct
{
  ...
  /* PLMN identity */
  byte                                plmn_info[GSTK_MAX_PLMN_LEN];
}
gstk_csg_info_type;
...
typedef struct
{
  ...
  gstk_general_result_enum_type      command_result;
  /*
  ** result_additional_info is populated when
  ** command_result == GSTK_ME_CURRENTLY_UNABLE_TO_PROCESS_COMMAND.
  ** result_additional_info->additional_info_ptr will point
  ** to gstk_additional_info_code_enum_type
  */
  gstk_additional_info_ptr_type      result_additional_info;
}
gstk_contactless_state_changed_cnf_type;
...
{
  gstk_hdr_type                      message_header;
  gstk_nmea_sentence_type            nmea_sentence;
  gstk_evt_cb_funct_type             client_cb;
}
gstk_envelope_geo_loc_report_type;
...
gstk_envelope_contactless_state_req_evt_ind_type;
...
{
   gstk_hdr_type                        message_header;
   ds_sys_wlan_ip_addr_type             ds_wlan_ip_addr_info;
}
gstk_ds_wlan_ip_addr_change_ind_type;

typedef struct
{
   gstk_hdr_type                        message_header;
   ds_sys_msisdn_info_per_apn_type      ds_msisdn_info_per_apn;
   ds_sys_subscription_enum_type        subscription_id;   
}
...
typedef union
{
    ...
    gstk_contactless_state_changed_cnf_type        contactless_state_changed_term_rsp_cnf;
    gstk_geo_loc_cnf_type                          gstk_geo_loc_cnf;
    ...
    gstk_envelope_contactless_state_req_evt_ind_type  contactless_state_req_envelope_ind;
    gstk_raw_envelope_ind_type                        raw_envelope_ind;
    gstk_envelope_geo_loc_report_type                 geo_loc_report;
    ...
    /* ds WLAN IP address change notification */
    gstk_ds_wlan_ip_addr_change_ind_type           ds_wlan_ip_addr_change_ind;
...
PARAMETERS
  ...
  data       : [Input] Pointer to the data buffer that contains the Proactive cmd.
                       If data == NULL, indicate that the current proactive command
