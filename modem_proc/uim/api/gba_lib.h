...
12/08/15   vdc     Limit GBA cmd queue size to 10
...
/* ----------------------------------------------------------------------------
   ENUM:      GBA_EVENTS_ENUM_TYPE

   DESCRIPTION:
     Enum for events sent by GBA
-------------------------------------------------------------------------------*/
typedef enum {
  GBA_KEY_EXPIRED_EVT,
  GBA_KEY_REPLACED_EVT,
  GBA_KEY_DISCARDED_EVT
} gba_key_event_enum_type;

/* ----------------------------------------------------------------------------
   ENUM:      GBA_BSF_PROTOCOL_ENUM_TYPE

   DESCRIPTION:
     Enum for BSF protocol
-------------------------------------------------------------------------------*/
typedef enum {
  GBA_BSF_PROTOCOL_HTTP  = 0,
  GBA_BSF_PROTOCOL_HTTPS = 1
...
typedef struct {
  ...
  gba_bsf_address_type             address;
  uint16                           port;
} gba_bsf_info_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:    GBA_EVENT_DATA_TYPE

   DESCRIPTION:
     Structure contains GBA event information
-----------------------------------------------------------------------------*/
typedef struct {
  gba_key_event_enum_type          evt;
...
/* ----------------------------------------------------------------------------
   DATA TYPE:      GBA_EVT_CALLBACK_TYPE
-----------------------------------------------------------------------------*/
/** @brief GBA event callback function.

    The parameter to the callback function contains the results of the
    client's request.
*/
typedef void (*gba_evt_callback_type) (const gba_event_data_type      * evt_ptr);

/* --------------------------------------------------------------------------
   STRUCTURE:      GBA_BOOTSTRAP_EXT_INFO_TYPE

   DESCRIPTION:
     apn_ptr:           Pointer to APN data (optional if default APN
                        can be used)
     bsf_info_ptr:      BSF address data (optional if default BSF is
                        to be used)
     evt_callback:      Callback if client is interested in receiving events
     evt_user_data_ptr: User data for the event callback
---------------------------------------------------------------------------*/
typedef struct {
  char                           * apn_ptr;
  gba_bsf_info_type              * bsf_info_ptr;
  gba_evt_callback_type            evt_callback;
  uint32                           evt_user_data;
} gba_bootstrap_ext_info_type;
