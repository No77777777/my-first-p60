...
$Header: //components/rel/uim.mpss/6.1.0/api/simlock_common.h#8 $$ $DateTime: 2020/08/04 23:12:22 $
...
#define SIMLOCK_CONFIG_NICKNAME_LEN_MAX  32
#define SIMLOCK_TOKEN_LEN                32
#define SIMLOCK_NUM_INACTIVE_CONFIG_MAX  8
...
typedef enum {
  ...
  SIMLOCK_ACTIVATE_CONFIG_MSG,
  SIMLOCK_GET_TOKEN_MSG,
  SIMLOCK_GET_RSU_MODE_MSG
...
typedef enum {
  ...
  /* If at least one card is present and considered invalid, then all cards are
     marked as invalid */
  SIMLOCK_SLOT_POLICY_WAIT_FOR_FULL_SERVICE_ON_SLOT1_OR_IMSI_MATCH,
  /* All slots will remain blocked until full network service is acquired 
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      SIMLOCK_NICKNAME_TYPE

   DESCRIPTION:
     This structure is for operator nickname data in SIMLock
------------------------------------------------------------------------------*/
typedef struct {
  uint8    nickname_len;
  uint8    nickname[SIMLOCK_CONFIG_NICKNAME_LEN_MAX];
} simlock_nickname_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      SIMLOCK_INACTIVE_CONFIG_DATA_TYPE

   DESCRIPTION:
     This structure consists of SIMLock inactive config data
-------------------------------------------------------------------------------*/
typedef struct {
  uint8                                      config_id;
  simlock_nickname_type                      config_nickname;
} simlock_inactive_config_data_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      SIMLOCK_INACTIVE_CONFIG_RESP_TYPE

   DESCRIPTION:
     This structure consists of SIMLock inactive config resp type
-------------------------------------------------------------------------------*/
typedef struct {
  uint8                                      count;
  simlock_inactive_config_data_type          inactive_config[SIMLOCK_NUM_INACTIVE_CONFIG_MAX];
} simlock_inactive_config_resp_type;
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      SIMLOCK_GET_RSU_MODE_MSG_RESP_TYPE

   DESCRIPTION:
     This structure is for the SIMLock response for getting the
     rsu mode
------------------------------------------------------------------------------*/
typedef struct {
  simlock_rsu_mode_enum_type      rsu_mode;
} simlock_get_rsu_mode_msg_resp_type;
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      SIMLOCK_ACTIVATE_CONFIG_MSG_REQ_TYPE

   DESCRIPTION:
     This structure is for the SIMLock request message for activating the configuration
------------------------------------------------------------------------------*/
typedef struct {
  uint8      config_id;
} simlock_activate_config_msg_req_type;
...
/* ----------------------------------------------------------------------------
   STRUCTURE:      SIMLOCK_GET_STATUS_MSG_REQ_TYPE

   DESCRIPTION:
     This structure is for the SIMLock request message to get the list of
     categories and its status
-------------------------------------------------------------------------------*/
typedef struct{
  boolean   inactive_config;
} simlock_get_status_msg_req_type;
...
typedef struct {
  ...
  union {
    ...
    simlock_activate_config_msg_req_type      activate_configuration;
    simlock_get_status_msg_req_type           get_status_msg;
  } message;
...
typedef struct {
  ...
  simlock_inactive_config_resp_type        inactive_config_data;
  simlock_disabled_slot_data_type          disabled_slots;
} simlock_get_status_msg_resp_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      SIMLOCK_GET_RSU_MODE_RESP_TYPE

   DESCRIPTION:
     This structure is for the SIMLock RSU mode
-------------------------------------------------------------------------------*/
typedef struct {
  simlock_rsu_mode_enum_type               rsu_mode;
} simlock_get_rsu_mode_resp_type;
...
typedef uint8  simlock_token_data_type[SIMLOCK_TOKEN_LEN];

/* ----------------------------------------------------------------------------
   STRUCTURE:      SIMLOCK_TOKEN_TYPE

   DESCRIPTION:
     This structure is for the SIMLock token type
-------------------------------------------------------------------------------*/
typedef struct {
  boolean                 token_present;
  simlock_token_data_type token_data;
} simlock_token_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      SIMLOCK_SLOT_TOKEN_DATA_TYPE
...
-------------------------------------------------------------------------------*/
typedef struct {
  simlock_token_type category_token_data[SIMLOCK_CATEGORY_COUNT_MAX];
} simlock_slot_token_data_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      SIMLOCK_GET_TOKEN_MSG_RESP_TYPE

   DESCRIPTION:
     This structure is for the SIMLock response corresponding to the get token
     request message
-------------------------------------------------------------------------------*/
typedef struct {
  simlock_slot_token_data_type   slot_token_data[SIMLOCK_SLOT_COUNT_MAX];
} simlock_get_token_msg_resp_type;
...
typedef struct {
  uint32                  num_retries_max;
  uint32                  curr_retries;
  simlock_token_type      token;
} simlock_add_lock_device_msg_resp_type;

/* ----------------------------------------------------------------------------
   STRUCTURE:      SIMLOCK_TOKEN_MSG_RESP_TYPE

   DESCRIPTION:
     This structure is for the SIMLock token response corresponding to the
     lock_ck, add_lock_ck and lock_hck requests
-------------------------------------------------------------------------------*/
typedef struct {
  simlock_token_type token;
} simlock_token_msg_resp_type;
...
typedef struct {
  ...
  union {
    ...
    simlock_token_msg_resp_type                token_resp;
    simlock_get_token_msg_resp_type            get_token_resp;
    simlock_get_rsu_mode_msg_resp_type         get_rsu_mode;
  } message;
