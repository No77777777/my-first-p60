...
GENERAL DESCRIPTION : ENHANCED STK layer SetUp call and Send DTMF support
          ...
          Copyright © 2008-2017 QUALCOMM Technologies, Inc (QTI).
          All Rights Reserved.
...
$Header: //components/rel/uim.mpss/6.1.0/estk/src/estk_cm_call.c#2 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/10/17   vr      Send user conf alpha always for setup call except CUST_1
12/07/16   vr      Copy call state for orig, incoming, end call event
10/12/16   vr      Update TON and NPI in CM call request
10/07/16   shr     GSTK/ESTK F3 reduction
09/16/16   vr      End the setup call after setup call TR expiry
07/11/16   vr      Allocating memory to additional info 2 ptr
...
03/28/16   gs      Init and activation with CM for Supplementary services
03/22/16   gs      Supplementary services init movement
03/16/16   gs      ESTK CM Init is moved
11/12/15   vr      F3 Logging inprovements
10/01/15   shr     Clear ESTK CM Call ID correctly
08/06/15   gs      Move to cm_util_default_gw_orig_params_per_sub API
06/25/15   gm      L4 compiler warning
06/05/15   gm      Split ME and slot specific NV
05/07/15   gm      Clean up redial values
03/11/15   vr      UT Klocwork errors fix and multi SIM SAT support
02/18/15   dy      Fix icon data not being freed in ESTK
...
void estk_cm_init( void )
{
  ...
  if((gstk_nv_get_me_feature_status(
       GSTK_CFG_FEATURE_ME_DISABLE_ESTK_CM_VOICE_CALL_INTERFACE) == FALSE) ||
     (gstk_nv_get_me_feature_status(
       GSTK_CFG_FEATURE_ME_DISABLE_ESTK_CM_SUPPS_INTERFACE) == FALSE))
  {
...
)
{
  ...
  *cmd_has_alpha_ptr = FALSE;
  ...
  {
    /* only for CUST_1 check for NO and NULL alpha NV */
#ifdef FEATURE_CUST_1
    if (gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_ESTK_QMI_SEND_NO_AND_NULL_ALPHA_IND))
...
)
{
  ...
  switch(cmd_ptr->cmd_data.call_event_info.evt_id)
  {
    ...
    case ESTK_CM_CALL_INCOM_EVT:
      UIM_MSG_HIGH_1("Received Call Orig/Incoming call Event for call id 0x%x",
                     cmd_ptr->cmd_data.call_event_info.call_id);
      /*Update the estk_cm_cs_call_connected variable only if the call is of CS type */
    ...
    case ESTK_CM_CALL_MOD_TO_SS_EVT:
      ...
      {
        estk_shared_info.call_info.call_id = ESTK_INVALID_CM_CALL_ID; /* not valid any more */
        estk_clear_dtmf_str();
  ...
  switch(call_event)
  {
    ...
    case CM_CALL_EVENT_INCOM:
      memset(&task_cmd_ptr->cmd_data.call_event_info,
             0x00,
             sizeof(task_cmd_ptr->cmd_data.call_event_info));
      task_cmd_ptr->cmd_data.call_event_info.evt_id = ESTK_CM_CALL_INCOM_EVT;
      task_cmd_ptr->cmd_data.call_event_info.call_type = (estk_cm_call_type_enum_type) call_info_ptr->call_type;
      break;
...
/*===========================================================================
FUNCTION:   estk_fill_cm_called_number

DESCRIPTION:
  Updates the CM called number parameter with setup call request values.

PARAMETERS:
  req_ptr        : [Input] gstk set up call request pointer
  called_num_ptr : [Output] CM called number pointer
...
===========================================================================*/
static void estk_fill_cm_called_number(
  const gstk_setup_call_req_type *req_ptr,
  cm_num_s_type                  *called_num_ptr
)
{
  ...
  cm_num_fill(called_num_ptr, (byte *) num_buf,
              (byte) digit_count, CM_DIGIT_MODE_4BIT_DTMF);
  switch(req_ptr->address.NPI)
  {
    case GSTK_UNKNOWN_NPI:
      called_num_ptr->number_plan = (uint8)CM_NUM_PLAN_UNKNOWN;
      break;
    case GSTK_ISDN_TELEPHONY_NPI:
      called_num_ptr->number_plan = (uint8)CM_NUM_PLAN_ISDN;
      break;
    case GSTK_DATA_NPI:
      called_num_ptr->number_plan = (uint8)CM_NUM_PLAN_DATA;
      break;
    case GSTK_TELEX_NPI:
      called_num_ptr->number_plan = (uint8)CM_NUM_PLAN_TELEX;
      break;
    case GSTK_PRIVATE_NPI:
      called_num_ptr->number_plan = (uint8)CM_NUM_PLAN_PRIVATE;
      break;
    case GSTK_EXT_RESERVED_NPI:
      called_num_ptr->number_plan = (uint8)CM_NUM_PLAN_RESERVED_EXTENSION;
      break;
    default:
      called_num_ptr->number_plan = (uint8)CM_NUM_PLAN_ISDN;
      break;
  }
  switch (req_ptr->address.TON)
  {
    case GSTK_INTERNATIONAL_NUM:
      called_num_ptr->number_type = CM_NUM_TYPE_INTERNATIONAL;
      break;
    case GSTK_NATIONAL_NUM:
      called_num_ptr->number_type = CM_NUM_TYPE_NATIONAL;
      break;
    case GSTK_NETWORK_SPECIFIC_NUM:
      called_num_ptr->number_type = CM_NUM_TYPE_NETWORK_SPECIFIC;
      break;
    default:
      called_num_ptr->number_type = CM_NUM_TYPE_UNKNOWN;
      break;
  }
...
static boolean estk_cm_orig_call(const gstk_cmd_from_card_type *cmd_ptr)
{
  ...
  estk_fill_cm_called_number(req_ptr, &called_number);
...
void estk_send_dtmf_cleanup(void)
{
  ...
  if(estk_curr_inst_ptr->curr_cmd.cmd.send_dtmf_pro_cmd_req.icon.size != 0)
  {
    gstk_free(estk_curr_inst_ptr->curr_cmd.cmd.send_dtmf_pro_cmd_req.icon.data);
    estk_curr_inst_ptr->curr_cmd.cmd.send_dtmf_pro_cmd_req.icon.data = NULL;
    estk_curr_inst_ptr->curr_cmd.cmd.send_dtmf_pro_cmd_req.icon.size = 0;
    estk_curr_inst_ptr->curr_cmd.cmd.send_dtmf_pro_cmd_req.icon.present = FALSE;
  }
...
void estk_send_DTMF(uint32 dummy)
{
       ...
       i++)
  {
    ...
    if ((uint8)(estk_shared_info.call_info.dtmf_str_ptr[i]) == 0xFF)
    {
...
void estk_handle_cm_cmd_reattempt_timer(uint32 dummy)
{
     ...
     (estk_shared_info.redial_in_progress == TRUE))
  {
    ...
    if(cm_mm_call_cmd_get_call_info_list(
         estk_call_err_cmd_cb,
         (void*)&(estk_shared_info.redial_cmd_ptr->hdr_cmd.cmd_detail_reference),
         estk_shared_info.cm_client_id) == FALSE)
    {
  ...
  sys_sys_mode_e_type cm_sys_mode)
{
  ...
  return cm_call_mode;
} /* estk_map_cm_sys_mode_to_call_mode */


/*===========================================================================
FUNCTION :  estk_cm_end_setup_call_request

DESCRIPTION:
  Sends call end request to the CM for the setup call.
...
RETURN VALUE:
  estk_result_enum_type
    ...
    ESTK_UNSUPPORTED     - If not supported
...
===========================================================================*/
estk_result_enum_type estk_cm_end_setup_call_request (
  void
)
{
  ...
  memset(&end_params, CM_CALL_CMD_PARAM_DEFAULT_VALUE, sizeof(end_params));

  end_params.call_id = estk_shared_info.call_info.call_id;

  if (cm_mm_call_cmd_end(estk_call_err_cmd_cb,
                         NULL,
                         estk_shared_info.cm_client_id,
                         1,
                         &end_params))
  ...
  return ESTK_ERROR;
} /* estk_cm_end_setup_call_request */
