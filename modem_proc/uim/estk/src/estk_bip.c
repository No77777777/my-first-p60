...
Copyright (c) 2010 - 2018 QUALCOMM Technologies, Inc (QTI) and its licensors.
...
$Header: //components/rel/uim.mpss/6.1.0/estk/src/estk_bip.c#4 $

when       who     what, where, why
--------   ---     --------------------------------------------------------------------
05/31/19   dt      Fix compilation warnings
04/01/19   ks      Do not send dummy DNS address TLV when no DNS is received
11/08/18   ks      Check for IPv4 dest address before setting pdp type to IPv4
10/05/18   gs      Move NETDOWN reason code before clean up
07/05/18   dd      Add check to avoid creation of duplicate profiles
06/25/18   dd      Modify persistence mask value
11/03/17   dt      update the profile number in always on APN case
10/31/17   dt      Do not send channel status indication for closed channel
10/09/17   dt      Do not process BIP over cellular when cellular bearer is null on index 0
09/22/17   shr     Revert support for BIP APN over-ride
08/24/17   gs      Populate the DS Sub ID based on the Slot ID
06/01/17   gs      Don't depend on DS Sys Status for establishing BIP on non-DDS
04/17/17   dt      Adding a condition to check if scws_overflow_buf_len is 0
03/29/17   shr     Add support for BIP APN over-ride
02/13/17   sk      Deprecate direct read of GCF NV 947
02/10/17   shr     Mark WLAN as unusable even for CLOSE CHANNEL with Reuse
01/30/17   gs      Support of Dual Data Capability
10/12/16   shr     Add support for OPEN CHANNEL with IPv6 dest. address and for
                   OPEN CHANNEL DNS over WLAN
09/29/16   shr     Remove unecessary initialization of DS profile library
09/23/16   shr     Clear input params before every DS get param API invocation
07/27/16   gs      Scenario based F3 reduction
07/21/16   vr      Don't cleanup send or receive data buffer during fallback
06/01/16   gm      Free PDN connection if not reusing
05/31/16   gs      F3 logging improvements
05/25/16   shr     F3 reduction: Malloc Failure, Memcpy, Null Ptr checks
05/13/16   gs      F3 logging improvements
04/21/16   gm      Allow SCWS send data even if destination address is not valid
04/01/16   shr     Add support for copying full profile info from existing profile
04/01/16   gm      Closing PDN connection after bringing up new PDN connection
03/28/16   gm      Removal of SCWS code for IOE devices
03/17/16   gm      3gpp2 feature controlled
03/16/16   gm      Removed "FEATURE_TABASCO_MODEM"
01/14/15   sam     Fix for TA.2.3 compilation dependency
11/25/15   shr     Loop through all DS RATs to determine if 1x/HRPD is priority
10/29/15   gm      Correction of bearer parameters
10/14/15   shr     QOS Flow request applicable only for EUTRAN bearer
10/09/15   gm      Converting to unsigned variables.
10/01/15   lm      Fix compilation issues on linux mob
09/24/15   shr     Resolve KW warnings
09/15/15   shr     Reduce F3 logging
08/18/15   shr     Ensure Link Drop is sent to corrct slot
08/10/15   lm      Fix compilation issues on linux mob
07/22/15   shr     Send link drop event to all open channels
07/17/15   shr     Add support for BIP over WLAN on slot 2
06/25/15   gm      L4 compiler warning
06/26/15   shr     Remove TR delay logic for CLOSE CHANNEL
06/26/15   gs      Update the Channel index while populating SCWS Channel Table
06/05/15   gm      Split ME and slot specific NV
05/14/15   gm      Fix to use first index of DS rat array as preferred
04/30/15   shr     Added support for WLAN IP address change event
04/27/15   shr     Allow DS to chose 3GPP/3GPP2 profiles when APN cannot be determined
03/17/15   shr     While processing SEND DATA, along with Link Drop envelope
                   (due to OOS/DS subsystem error), send error TR
03/12/15   dy      Fix channel reuse variables being reset before channel clean up
03/09/15   shr     Fixed incorrect usage of snprintf
03/04/15   shr     Read Default APN NV using GSTK NV API
02/25/15   gs      Send respective TR instead of Link Drop event
02/25/15   dy      Clean-up BIP F3s
02/25/15   dy      Fix channel reuse not working for DNS query open channel
02/24/15   dy      Fix starting channel reuse timer too early
02/24/15   dy      Fix channel reuse logic for NULL APN
02/18/15   shr     Added Channel Index validity check
02/06/15   dy      Reduce GSTK priv data size
02/04/15   vr/bd   Check for EPC profiles when APN profile is set
01/22/15   gs      Send the right TR cause when DS Service is not available
01/18/15   vr      estk_cm_cs_call_connected changed as counter
12/30/14   gm      Fix for rejecting the Open Channel with with Busy on Call
12/09/14   lj/bd   Deliver profile 254 to DS if null APN provisioned in EHRPD
12/08/14   dy      Re checking in backed out CR 743973
12/02/14   gs      Dereg the IP Filter after properly closing the socket
...
static estk_result_enum_type estk_bip_get_3gpp_profile_to_use(
  uint32                           ch_idx,
...
static boolean estk_bip_3gpp_profile_copy_profile_contents
(
...
static estk_result_enum_type estk_bip_set_net_policy_3gpp_profile(
  uint32                           ch_idx,
  estk_bip_apn_select_method_type  apn_select_method,
  ds_profile_num_type             *profile_num_ptr
);

static estk_result_enum_type estk_bip_check_3gpp_profile_usability(
  ds_profile_num_type               profile_num,
  gstk_pkt_data_address_enum_type   address_type,
  ds_profile_subs_etype             sub_id
);

static estk_result_enum_type estk_bip_copy_default_3gpp_profile_apn(
  uint32              ch_idx,
  ds_profile_num_type            profile_num,
  ds_profile_subs_etype       sub_id
);

static int32 estk_bip_set_default_apn_from_efs(uint32 ch_idx);

static estk_result_enum_type estk_bip_use_always_on_apn(uint32 ch_idx);

static estk_result_enum_type estk_bip_use_default_3gpp_profile_apn(
  uint32                           ch_idx,
  ds_profile_num_type             *profile_num_ptr
);
...
static estk_result_enum_type estk_bip_process_null_apn(
  uint32                           ch_idx,
  estk_bip_apn_select_method_type *apn_select_method_ptr,
  ds_profile_num_type             *profile_num_ptr
);

static estk_result_enum_type estk_bip_get_3gpp_profile(
  uint32                           ch_idx,
  estk_bip_apn_select_method_type *apn_select_method_ptr,
  ds_profile_num_type             *profile_num_ptr
);

static estk_result_enum_type estk_bip_set_net_policy_profile(
  gstk_access_technology_type       access_tech,
  uint32                            ch_idx,
  dss_net_policy_info_type         *net_policy_ptr
);

static boolean estk_bip_match_apn(
  uint32  index,
  char   *apn_ptr);

static boolean estk_bip_check_existing_gsm_cs_call(void);

boolean estk_check_cellular_availability(gstk_slot_id_enum_type slot_id);
...
===========================================================================*/
void estk_bringdown_iface(uint32 index, sint15 dss_nethandle)
{
...
void estk_bip_clean_up_reuse_ch(uint32 index)
{
  ...
  if(estk_curr_inst_ptr->keep_pdn_conn)
  {
    estk_curr_inst_ptr->close_nethandle_v4 = estk_curr_inst_ptr->bip_channel_table[index].sock_nethandle_v4;
    estk_curr_inst_ptr->bip_channel_table[index].sock_nethandle_v4 = DSS_ERROR;
    estk_curr_inst_ptr->close_nethandle_v6 = estk_curr_inst_ptr->bip_channel_table[index].sock_nethandle_v6;
...
===========================================================================*/
void estk_bip_clean_up_ch_table(uint32 index)
{
  ...
  if(index >= ESTK_MAX_BIP_CH)
  {
    UIM_MSG_HIGH_1("estk_bip_clean_up_ch_table, index=0x%x", index);
    return;
  }
    ...
    if(estk_curr_inst_ptr->bip_channel_table[index].is_scws_ch == FALSE)
    {
      ...
      if (delete_profile)
      {
        UIM_MSG_HIGH_1("ESTK_BIP Deleting profile number %d",
                       estk_curr_inst_ptr->bip_channel_table[index].profile_3gpp);
        (void)ds_profile_delete_per_sub(DS_PROFILE_TECH_3GPP,
                                estk_curr_inst_ptr->bip_channel_table[index].profile_3gpp,
                                estk_curr_inst_ptr->bip_channel_table[index].ds_sub_id);
      }
    }
  }
  ...
  */
    ...
    if(estk_curr_inst_ptr->bip_channel_table[index].is_scws_ch == FALSE)
    {
      ...
      if (delete_profile)
      {
        UIM_MSG_HIGH_1("ESTK_BIP Deleting profile number %d created by ESTK",
                       estk_curr_inst_ptr->bip_channel_table[index].profile_3gpp2);
        (void)ds_profile_delete_per_sub(DS_PROFILE_TECH_3GPP2,
                                estk_curr_inst_ptr->bip_channel_table[index].profile_3gpp2,
                                estk_curr_inst_ptr->bip_channel_table[index].ds_sub_id);
      }
    }
  ...
  else
  {
    ...
    {
      if(estk_curr_inst_ptr->bip_channel_table[index].is_scws_ch == FALSE &&
         estk_curr_inst_ptr->keep_pdn_conn == FALSE)
      {
  ...
  estk_scws_clear_send_receive_buf((uint8)index+1); /* ch_id = index+1 */
  ...
  estk_curr_inst_ptr->bip_channel_table[index].ds_sub_id             = DS_PROFILE_ACTIVE_SUBSCRIPTION_NONE;
  ...
  sint15 sock_fd)
{
  ...
  uint32                 prev_recv_buf_len  = 0;
  uint32                 recv_buf_len_avail = 0;
  sint15                 error_num          = DSS_ERROR;
  sint31                 dss_status         = DSS_ERROR;
  ...
  do
  {
    ...
    if(num_bytes_read > 0 && (uint32)num_bytes_read <= recv_buf_len_avail)
    {
      ...
      estk_curr_inst_ptr->bip_channel_table[index].recv_buf_len += (uint32)num_bytes_read;
...
)
{
  ...
  UIM_MSG_HIGH_1("Send Data len 0x%x",
  ...
  sint15 dss_nethandle)
{
  ...
  gstk_additional_info_ptr_type       additional_info;
  uint8                               addi_res   = 0;
  dss_net_down_reason_type            netdown_reason = DSS_NET_DOWN_REASON_NOT_SPECIFIED;
  sint15                              dss_errno      = 0;
  sint15                              dss_status     = 0;  
  gstk_general_result_enum_type       result         = GSTK_BEARER_INDEPENDENT_PROTOCOL_ERROR;
  estk_result_enum_type               estk_status    = ESTK_ERROR;
    ...
    if(estk_curr_inst_ptr->bip_channel_table[i].sock_nethandle == dss_nethandle)
    {
      ...
      switch(estk_curr_inst_ptr->bip_channel_table[i].current_command_id)
      {
        case GSTK_OPEN_CH_REQ:
          /* If open channel failure is due to Call control failure then send TR with
          appropriate cause code */
          dss_status = dss_last_netdownreason(dss_nethandle, &netdown_reason, &dss_errno);
  
          if ((dss_status == DSS_SUCCESS) && (netdown_reason == DSS_NET_DOWN_REASON_CALL_CONTROL_REJECTED))
          {
            result = GSTK_INTERACTION_WITH_CC_BY_SIM_OR_MO_SM_CONTROL_BY_SIM_PROBLEM;
            addi_res = (uint8)GSTK_ACTION_NOT_ALLOWED;
          }
          else
          {
            addi_res = (uint8)GSTK_NO_SPECIFIC_CAUSE_CAN_BE_GIVEN;
          ...
          if(estk_curr_inst_ptr->state == ESTK_WAIT_FOR_DISPLAY_ALPHA_CNF_ST)
          {
            ...
            estk_curr_inst_ptr->state = ESTK_PENDING_TR_ST;
            estk_curr_inst_ptr->result = result;
            estk_cache_additional_info(&additional_info);
            estk_curr_inst_ptr->extra_param = tr_extra_param;
          }
          else
        ...
        default:
             ...
             (estk_curr_inst_ptr->bip_channel_table[i].on_demand_link == TRUE)))
          {
            ...
            estk_status = estk_bip_send_ch_status_event(
                  ...
                  GSTK_CH_STATUS_LINK_DROPPED);
            /* if channel status event is not sent to card, 
               clean up channel related information */
            if(estk_status == ESTK_ERROR)
            {
              (void)estk_bip_clean_up_ch_table(i);
            }
  ...
  const estk_cmd_type *cmd_ptr)
{
  ...
  if(estk_curr_inst_ptr->keep_pdn_conn)
  {
    if(estk_curr_inst_ptr->close_nethandle_v4 != DSS_ERROR)
    {
      dss_status = dsnet_stop(estk_curr_inst_ptr->close_nethandle_v4,
                              &error );
      UIM_MSG_HIGH_2("dsnet_stop returns %d with error_num %d",
                     dss_status, error);

      dss_status = dsnet_release_handle(estk_curr_inst_ptr->close_nethandle_v4,
                                       &error );
      UIM_MSG_HIGH_2("dsnet_release_handle returns %d with error_num %d",
                     dss_status, error);
    }

    if(estk_curr_inst_ptr->close_nethandle_v6 != DSS_ERROR)
    {
      dss_status = dsnet_stop(estk_curr_inst_ptr->close_nethandle_v6,
                              &error );
      UIM_MSG_HIGH_2("dsnet_stop returns %d with error_num %d",
                     dss_status, error);

      dss_status = dsnet_release_handle(estk_curr_inst_ptr->close_nethandle_v6,
                                       &error );
      UIM_MSG_HIGH_2("dsnet_release_handle returns %d with error_num %d",
                     dss_status, error);
    }
    estk_curr_inst_ptr->keep_pdn_conn = FALSE;
    estk_curr_inst_ptr->close_nethandle_v4 = DSS_ERROR;
    estk_curr_inst_ptr->close_nethandle_v6 = DSS_ERROR;
  }
...
===========================================================================*/
estk_result_enum_type estk_bip_setup_data_filter(uint32 index)
{
...
/*===========================================================================
FUNCTION estk_util_convert_data_bit_rate_to_octet

DESCRIPTION
  Converts the NW passed bit rate to uint32.

PARAMETERS
  data_rate : [Input]  data rate in kbps
  nw_rate   : [Output] data rate in encoded 1 byte format
...
===========================================================================*/
void estk_util_convert_data_bit_rate_to_octet
(
  uint32 data_rate,
  byte   *nw_rate
)
{
  ...
  if(data_rate == 0)
  {
    *nw_rate = ESTK_EPS_0KB_BITRATE_VALUE;
    return;
  }
  if(data_rate < ESTK_EPS_8KB_BITRATE_MINIMUM_VALUE)
  {
    *nw_rate = (byte) (data_rate);
  }
  else if(data_rate < ESTK_EPS_64KB_BITRATE_MINIMUM_VALUE)
  {
    *nw_rate = (byte) ((data_rate - ESTK_EPS_8KB_BITRATE_MINIMUM_VALUE)/ESTK_EPS_8KB_BITRATE_MULTIPLE
                        + ESTK_EPS_8KB_BITRATE_START_RANGE);
  }
  else if(data_rate <= ESTK_EPS_MAX_1_OCTET_BITRATE)
  {
    *nw_rate = (byte) ((data_rate - ESTK_EPS_64KB_BITRATE_MINIMUM_VALUE)/ESTK_EPS_64KB_BITRATE_MULTIPLE
                        + ESTK_EPS_64KB_BITRATE_START_RANGE);
  }
  else
  {
    *nw_rate = ESTK_EPS_MAX_1_OCTET_BIT_VALUE;
  }
}

/*===========================================================================
FUNCTION estk_util_convert_data_bit_rate_to_extended
...
PARAMETERS
  data_rate :    [Input]  data rate in kbps
  nw_rate   :    [Output] data rate in encoded 1 byte format
  nw_rate_2 :    [Output] data rate in encoded extended format
...
===========================================================================*/
void estk_util_convert_data_bit_rate_to_extended
(
  uint64 data_rate,
  byte   *nw_rate,
  byte   *nw_rate_ex
)
{
  ...
  *nw_rate_ex = 0;
  ...
  else if (data_rate < ESTK_EPS_1MB_BITRATE_MINIMUM_VALUE)
  {
    *nw_rate = ESTK_EPS_MAX_1_OCTET_BIT_VALUE;
    *nw_rate_ex = (byte) ((data_rate - ESTK_EPS_100KB_BITRATE_BASE_RATE)/ESTK_EPS_100KB_BITRATE_MULTIPLE);
  }
  else if (data_rate < ESTK_EPS_2MB_BITRATE_MINIMUM_VALUE)
  {
    *nw_rate = ESTK_EPS_MAX_1_OCTET_BIT_VALUE;
    *nw_rate_ex = (byte) ((data_rate - ESTK_EPS_1MB_BITRATE_MINIMUM_VALUE)/ESTK_EPS_1MB_BITRATE_MULTIPLE
                           + ESTK_EPS_1MB_BITRATE_START_RANGE);
  }
  else if (data_rate <= ESTK_EPS_EXT_BITRATE_MAXIMUM_VALUE)
  {
    *nw_rate = ESTK_EPS_MAX_1_OCTET_BIT_VALUE;
    *nw_rate_ex = (byte) ((data_rate - ESTK_EPS_2MB_BITRATE_MINIMUM_VALUE)/ESTK_EPS_2MB_BITRATE_MULTIPLE
                           + ESTK_EPS_2MB_BITRATE_START_RANGE);
  }
  else
  {
    // treat all higher rates as 256mbps
    *nw_rate = ESTK_EPS_MAX_1_OCTET_BIT_VALUE;
    *nw_rate_ex = ESTK_EPS_2MB_BITRATE_END_RANGE;
  }
}
  ...
  switch(estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_type)
  {
    case GSTK_EUTRAN_EXT_PARAM:
      estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.max_bitrate_ul
        =  ds_pdn_qos->tx_ip_flow.data_rate.format.min_max.max_rate / 1000;
      estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.max_bitrate_dl
        =  ds_pdn_qos->rx_ip_flow.data_rate.format.min_max.max_rate / 1000;
      estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.guaranteed_bitrate_ul
        =  ds_pdn_qos->tx_ip_flow.data_rate.format.min_max.guaranteed_rate / 1000;
      estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.guaranteed_bitrate_dl
        =  ds_pdn_qos->rx_ip_flow.data_rate.format.min_max.guaranteed_rate / 1000;

      estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.max_sdu_size
        =  ds_pdn_qos->rx_ip_flow.nominal_sdu_size.size;

      /* Encoding Transfer delay as per 24.008 spec */
      if(ds_pdn_qos->rx_ip_flow.umts_params.u_params.trans_delay < 200)
      {
        estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.transfer_delay =
          (ds_pdn_qos->rx_ip_flow.umts_params.u_params.trans_delay) / 10;
      }
      else if(ds_pdn_qos->rx_ip_flow.umts_params.u_params.trans_delay < 1000)
      {
        estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.transfer_delay =
          (ds_pdn_qos->rx_ip_flow.umts_params.u_params.trans_delay - 200) / 50 + 0x10;
      }
      else if(ds_pdn_qos->rx_ip_flow.umts_params.u_params.trans_delay < 4000)
      {
        estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.transfer_delay =
          (ds_pdn_qos->rx_ip_flow.umts_params.u_params.trans_delay - 1000) / 100 + 0x20;
      }
      else
      {
         estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.transfer_delay = 0x3E;
      }

      switch (ds_pdn_qos->rx_ip_flow.umts_params.u_params.deliver_err_sdu)
      {
        case DS_UMTS_QOS_SDU_DLVR_NODETECT:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.delivery_of_err_sdu = 2;
          break;
        case DS_UMTS_QOS_SDU_DLVR_YES:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.delivery_of_err_sdu = 1;
          break;
        case DS_UMTS_QOS_SDU_DLVR_NO:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.delivery_of_err_sdu = 0;
          break;
        default:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.delivery_of_err_sdu = 3;
          break;
      }

      switch (ds_pdn_qos->rx_ip_flow.umts_params.u_params.dlvry_order)
      {
        case DS_UMTS_QOS_DORDER_YES:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.delivery_order = 1;
          break;
        case DS_UMTS_QOS_DORDER_NO:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.delivery_order = 0;
          break;
        default:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.delivery_order = 2;
          break;
      }

      switch (ds_pdn_qos->rx_ip_flow.trf_class)
      {
        case IP_TRF_CLASS_CONVERSATIONAL:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.traffic_class = 0;
          break;
        case IP_TRF_CLASS_STREAMING:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.traffic_class = 1;
          break;
        case IP_TRF_CLASS_INTERACTIVE:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.traffic_class = 2;
          break;
        case IP_TRF_CLASS_BACKGROUND:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.traffic_class = 3;
          break;
        default:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.traffic_class = 4;
          break;
      }

      switch (ds_pdn_qos->rx_ip_flow.umts_params.res_ber)
      {
        case UMTS_RES_BIT_ERR_RATE1:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.residual_bit_error_ratio = 1;
          break;
        case UMTS_RES_BIT_ERR_RATE2:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.residual_bit_error_ratio = 2;
          break;
        case UMTS_RES_BIT_ERR_RATE3:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.residual_bit_error_ratio = 3;
          break;
        case UMTS_RES_BIT_ERR_RATE4:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.residual_bit_error_ratio = 4;
          break;
        case UMTS_RES_BIT_ERR_RATE5:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.residual_bit_error_ratio = 5;
          break;
        case UMTS_RES_BIT_ERR_RATE6:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.residual_bit_error_ratio = 6;
          break;
        case UMTS_RES_BIT_ERR_RATE7:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.residual_bit_error_ratio = 7;
          break;
        case UMTS_RES_BIT_ERR_RATE8:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.residual_bit_error_ratio = 8;
          break;
        case UMTS_RES_BIT_ERR_RATE9:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.residual_bit_error_ratio = 9;
          break;
        case UMTS_RES_BIT_ERR_RATE10:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.residual_bit_error_ratio = 0xFF;
          break;
        default:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.residual_bit_error_ratio = 0;
          break;
      }

      switch (ds_pdn_qos->rx_ip_flow.umts_params.trf_pri)
      {
        case UMTS_TRF_HANDLING_PRI1:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.traffic_handling_pri = 1;
          break;
        case UMTS_TRF_HANDLING_PRI2:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.traffic_handling_pri = 2;
          break;
        case UMTS_TRF_HANDLING_PRI3:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.traffic_handling_pri = 3;
          break;
        default:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.traffic_handling_pri = 0;
          break;
      }

      switch (ds_pdn_qos->rx_ip_flow.pkt_err_rate.exponent)
      {
        case ESTK_EPS_PDP_ERR_RATE_EXP_1:
           estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.sdu_error_ratio = 7;
           break;
        case ESTK_EPS_PDP_ERR_RATE_EXP_2:
           estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.sdu_error_ratio = 1;
           break;
        case ESTK_EPS_PDP_ERR_RATE_EXP_3:
           if(ds_pdn_qos->rx_ip_flow.pkt_err_rate.multiplier == ESTK_EPS_PDP_ERR_RATE_MULT_7)
           {
             estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.sdu_error_ratio = 2;
           }
           else
           {
             estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.sdu_error_ratio = 3;
           }
           break;
        case ESTK_EPS_PDP_ERR_RATE_EXP_4:
           estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.sdu_error_ratio = 4;
           break;
        case ESTK_EPS_PDP_ERR_RATE_EXP_5:
           estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.sdu_error_ratio = 5;
           break;
        case ESTK_EPS_PDP_ERR_RATE_EXP_6:
           estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.sdu_error_ratio = 6;
           break;
        default:
           estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_ext_bearer.sdu_error_ratio = 0;
           break;
      }
      break;
    ...
    case GSTK_EUTRAN_MAPPED_UTRAN_PS:
      ...
      switch(ds_pdn_qos->tx_ip_flow.data_rate.format_type)
      {
        case DATA_RATE_FORMAT_MIN_MAX_EX_TYPE:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_mapped_utran_ps_bearer.data_rate_format
            = GSTK_DATA_RATE_FORMAT_MIN_MAX_EX_TYPE;

          estk_util_convert_data_bit_rate_to_extended(ds_pdn_qos->tx_ip_flow.data_rate.format.min_max_ex.max_rate,
                                                   &nw_rate,
                                                   &nw_rate_ext);
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_mapped_utran_ps_bearer.max_bitrate_ul
            = nw_rate;
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_mapped_utran_ps_bearer.max_bitrate_ul_ext
            = nw_rate_ext;

          estk_util_convert_data_bit_rate_to_extended(ds_pdn_qos->tx_ip_flow.data_rate.format.min_max_ex.guaranteed_rate,
                                                   &nw_rate,
                                                   &nw_rate_ext);
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_mapped_utran_ps_bearer.guaranteed_bitrate_ul
            =  nw_rate;
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_mapped_utran_ps_bearer.guaranteed_bitrate_ul_ext
            =  nw_rate_ext;
          break;

        case DATA_RATE_FORMAT_MIN_MAX_TYPE:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_mapped_utran_ps_bearer.data_rate_format
            = GSTK_DATA_RATE_FORMAT_MIN_MAX_TYPE;

          estk_util_convert_data_bit_rate_to_octet(ds_pdn_qos->tx_ip_flow.data_rate.format.min_max.max_rate,
                                                   &nw_rate);
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_mapped_utran_ps_bearer.max_bitrate_ul
            = nw_rate;

          estk_util_convert_data_bit_rate_to_octet(ds_pdn_qos->tx_ip_flow.data_rate.format.min_max.guaranteed_rate,
                                                   &nw_rate);
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_mapped_utran_ps_bearer.guaranteed_bitrate_ul
            =  nw_rate;
          break;
        ...
        default:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_mapped_utran_ps_bearer.data_rate_format
            = GSTK_DATA_RATE_FORMAT_MAX_TYPE;
          break;
      }

      switch(ds_pdn_qos->rx_ip_flow.data_rate.format_type)
      {
        case DATA_RATE_FORMAT_MIN_MAX_EX_TYPE:
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_mapped_utran_ps_bearer.data_rate_format
            = GSTK_DATA_RATE_FORMAT_MIN_MAX_EX_TYPE;

          estk_util_convert_data_bit_rate_to_extended(ds_pdn_qos->rx_ip_flow.data_rate.format.min_max.max_rate,
                                                   &nw_rate,
                                                   &nw_rate_ext);
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_mapped_utran_ps_bearer.max_bitrate_dl
            = nw_rate;
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_mapped_utran_ps_bearer.max_bitrate_dl_ext
            = nw_rate_ext;

          estk_util_convert_data_bit_rate_to_extended(ds_pdn_qos->rx_ip_flow.data_rate.format.min_max_ex.guaranteed_rate,
                                                   &nw_rate,
                                                   &nw_rate_ext);
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_mapped_utran_ps_bearer.guaranteed_bitrate_dl
            =  nw_rate;
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_mapped_utran_ps_bearer.guaranteed_bitrate_dl_ext
            =  nw_rate_ext;
          break;
        ...
        case DATA_RATE_FORMAT_MIN_MAX_TYPE:
          ...
          estk_util_convert_data_bit_rate_to_octet(ds_pdn_qos->rx_ip_flow.data_rate.format.min_max.max_rate,
                                                   &nw_rate);
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_mapped_utran_ps_bearer.max_bitrate_dl
            = nw_rate;

          estk_util_convert_data_bit_rate_to_octet(ds_pdn_qos->rx_ip_flow.data_rate.format.min_max.guaranteed_rate,
                                                   &nw_rate);
          estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_params.eutran_mapped_utran_ps_bearer.guaranteed_bitrate_dl
            =  nw_rate;
          break;
...
/* Call dss_netstatus to find the current network status */
  ...
  for(index = 0; index < ESTK_MAX_BIP_CH; index++)
  {
    ...
    if(estk_curr_inst_ptr->bip_channel_table[index].status == ESTK_CH_PENDING)
    {
      ...
      if(estk_curr_inst_ptr->bip_channel_table[index].nw_type != ESTK_NETWORK_WLAN)
      {
        /* For EUTRAN bearer, get updated QOS parameters from DS for PDN% */
        if((estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_type ==
            GSTK_EUTRAN_EXT_PARAM) ||
           (estk_curr_inst_ptr->bip_channel_table[index].bearer_desc.bearer_type ==
            GSTK_EUTRAN_MAPPED_UTRAN_PS))
        {
...
)
{
  ...
  UIM_MSG_HIGH_4("estk_get_profile_auth_param: ch_idx=0x%x, tech_type=0x%x, "
                 "auth_type=0x%x, sub_id=0x%x", ch_idx, tech_type, auth_type, sub_id);

  if(tech_type != DS_PROFILE_TECH_3GPP && tech_type != DS_PROFILE_TECH_EPC)
  {
    UIM_MSG_ERR_1("Unsupported API for the access tech 0x%x", tech_type);
    return ESTK_ERROR;
  }

  if(ch_idx >= ESTK_MAX_BIP_CH || auth_type == NULL)
  {
  ...
  do
  {
    ...
    if(tech_type == DS_PROFILE_TECH_EPC)
    {
      ESTK_BIP_GET_EPC_PROFILE_PARAM(api_ret_status, profile_hndl, profile_itr, profile_info,
         DS_PROFILE_TECH_3GPP, (ds_profile_identifier_type)DS_PROFILE_TECH_COMMON_PROFILE_PARAM_AUTH_PROTOCOL, sub_id);
    }
    else
    {
      ESTK_BIP_GET_PROFILE_PARAM(api_ret_status, profile_hndl, profile_itr, profile_info,
    ...
    *auth_type = profile_auth_type;
    ...
    if(estk_curr_inst_ptr->bip_channel_table[ch_idx].username_len == 0)
    {
      ...
      {
        ESTK_BIP_GET_EPC_PROFILE_PARAM(api_ret_status, profile_hndl, profile_itr, profile_info,
           DS_PROFILE_TECH_3GPP, (ds_profile_identifier_type)DS_PROFILE_TECH_COMMON_PROFILE_PARAM_USER_ID, sub_id);
      }
      else
      {
        ESTK_BIP_GET_PROFILE_PARAM(api_ret_status, profile_hndl, profile_itr, profile_info,
    ...
    if(estk_curr_inst_ptr->bip_channel_table[ch_idx].passwd_len == 0)
    {
      ...
      {
        ESTK_BIP_GET_EPC_PROFILE_PARAM(api_ret_status, profile_hndl, profile_itr, profile_info,
           DS_PROFILE_TECH_3GPP, (ds_profile_identifier_type)DS_PROFILE_TECH_COMMON_PROFILE_PARAM_AUTH_PASSWORD, sub_id);
      }
      else
      {
        ESTK_BIP_GET_PROFILE_PARAM(api_ret_status, profile_hndl, profile_itr, profile_info,
...
FUNCTION: estk_bip_get_3gpp_profile_to_use
...
PARAMETERS:
  ...
  apn_select_method:  [Input]  Method used to select the APN
...
)
{
  ...
  UIM_MSG_HIGH_3("estk_bip_get_3gpp_profile_to_use: ch_idx=0x%x apn_select_method=0x%x profile_num_ptr=0x%x",
  ...
  *profile_num_ptr = DS_PROFILE_UNSPECIFIED_NUM;
  ...
  /* Get an iterator first */
  api_ret_status = ds_profile_get_list_itr_per_sub(DS_PROFILE_TECH_3GPP,
                                           &profile_list,
                                           &profile_itr, sub_id);
  ...
  do
  {
    ...
    if(api_ret_status != DS_PROFILE_REG_RESULT_SUCCESS)
    {
      UIM_MSG_ERR_1("ds_profile_begin_transaction_per_sub failed %d",
                    api_ret_status);
      /* Release Iterator */
      (void)ds_profile_itr_destroy(profile_itr);
      return ESTK_ERROR;
    ...
    if(profile_info.len == open_ch_apn_len)
    {
      if(profile_info.len == 0 ||
         strncasecmp(profile_info.buf,
                     ...
                     profile_info.len) == 0)
      {
        UIM_MSG_HIGH_1("Found matching profile number with matching APN %d",
                       profile_list_info.num);
    ...
    if(*profile_num_ptr == profile_list_info.num)
    {
      (void)ds_profile_itr_destroy(profile_itr);
      return ESTK_SUCCESS;
    }
  ...
  return ESTK_ERROR;
} /* estk_bip_get_3gpp_profile_to_use */

/*===========================================================================
FUNCTION estk_bip_3gpp_profile_copy_profile_contents

DESCRIPTION
  The utility function copies the profile parameters from from_profile
  to to_profile
...
)
{
  ...
  char                        profile_buf[DS_PROFILE_MAX_PARAMETER_SIZE];
  ds_profile_identifier_type  ident;

  UIM_MSG_HIGH_2("estk_bip_3gpp_profile_copy_profile_contents to_profile_num:%d from_profile_num:%d",
                 to_profile_num, from_profile_num);

  if(to_profile_num == from_profile_num)
  {
    return FALSE;
  }
  ...
  do
  {
    /* Get profile handle for source profile */
    ...
    /* Get profile handle for destination profile */
    profile_status = ds_profile_begin_transaction_per_sub(
                         DS_PROFILE_TRN_RW,
                         DS_PROFILE_TECH_3GPP,
                         (ds_profile_num_type)to_profile_num,
    ...
    if( profile_status != DS_PROFILE_REG_RESULT_SUCCESS )
    {
      (void)ds_profile_end_transaction(from_profile_handle,
                                       DS_PROFILE_ACTION_CANCEL);
      ret_val = FALSE;
      break;
    }

    /* Copy all 3GPP profile parameters from source to dst. profile */
    for(ident = DS_PROFILE_3GPP_PROFILE_PARAM_MIN;
        ident <= DS_PROFILE_3GPP_PROFILE_PARAM_MAX;
        ident++ )
    {
      memset(profile_buf, 0, DS_PROFILE_MAX_PARAMETER_SIZE);
      info.buf = (void*)profile_buf;
      info.len = DS_PROFILE_MAX_PARAMETER_SIZE;

      /* Do not copy Profile Name or SUBS ID */
      if((ident == DS_PROFILE_3GPP_PROFILE_PARAM_PROFILE_NAME) ||
         (ident == DS_PROFILE_3GPP_PROFILE_PARAM_SUBS_ID))
      {
        continue;
      }
         ...
         DS_PROFILE_REG_RESULT_SUCCESS )
      {
        ...
        /* If PDP type copy fails, error out */
        if((profile_status != DS_PROFILE_REG_RESULT_SUCCESS) &&
           ((ident == DS_PROFILE_3GPP_PROFILE_PARAM_PDP_CONTEXT_PDP_TYPE) ||
            (ident == DS_PROFILE_3GPP_PROFILE_PARAM_LTE_ROAMING_PDP_TYPE) ||
            (ident == DS_PROFILE_3GPP_PROFILE_PARAM_UMTS_ROAMING_PDP_TYPE)))
        {
            UIM_MSG_ERR_1("Failed to copy PDP param: 0x%x", ident);
            ret_val = FALSE;
            break;
        }
      }
    ...
    /* Copy all general profile parameters from source to dst. profile */
    if(ret_val == TRUE)
    {
      for(ident = DS_PROFILE_GENERAL_PROFILE_PARAM_MIN;
          ident <= DS_PROFILE_GENERAL_PROFILE_PARAM_MAX;
          ident++ )
      {
        memset(profile_buf, 0, DS_PROFILE_MAX_PARAMETER_SIZE);
        info.buf = (void*)profile_buf;
        ...
        /* Do not copy SUBS ID */
        if(ident == DS_PROFILE_GENERAL_PROFILE_PARAM_SUBS_ID)
        {
          continue;
        }
    ...
    if (profile_status != DS_PROFILE_REG_RESULT_SUCCESS)
    {
      UIM_MSG_ERR_0("failed to commit to profile!");
      ret_val = FALSE;
    }
  ...
  return ret_val;
} /* estk_bip_3gpp_profile_copy_profile_contents */

/*===========================================================================

FUNCTION: estk_bip_set_net_policy_3gpp_profile
...
)
{
  ...
  ds_profile_3gpp_pdp_type_enum_type         home_pdp_type        = DS_PROFILE_3GPP_PDP_MAX;
  ds_profile_3gpp_roaming_pdp_type_enum_type roam_pdp_type        = DS_PROFILE_3GPP_ROAMING_PDP_MAX;
  ...
  gstk_open_ch_req_type                      *open_ch_cmd         = NULL;
  boolean                                    profile_to_be_copied = TRUE;
  ds_profile_num_type                        profile_num_to_copy  = DS_PROFILE_UNSPECIFIED_NUM;
  ...
  UIM_MSG_HIGH_3("estk_bip_set_net_policy_3gpp_profile: ch_idx=0x%x, apn_select_method=0x%x profile_num_ptr=0x%x",
                 ch_idx, apn_select_method, profile_num_ptr);
  ...
  /* Do not create profile in case if DNS is requestd and 3GPP profile is already 
  created for it */
  if((estk_curr_inst_ptr->bip_channel_table[ch_idx].profile_3gpp >= 0) && 
     (open_ch_cmd->dns_requested))
  {
    *profile_num_ptr = estk_curr_inst_ptr->bip_channel_table[ch_idx].profile_3gpp;
    return ESTK_SUCCESS;
  }
  ...
  /* Check if there is an existing 3GPP profile with matching APN */
  if(estk_bip_get_3gpp_profile_to_use(ch_idx, apn_select_method, &profile_num_to_copy, sub_id) != ESTK_SUCCESS)
  {
    profile_to_be_copied = FALSE;
  }
  /* If always ON profile is selected - this may not be a BIP created profile,
     hence use the profile as is */
  else if(apn_select_method == ESTK_BIP_APN_ALWAYS_ON)
  {
    *profile_num_ptr = profile_num_to_copy;
    return ESTK_SUCCESS;
  }
        ...
        &profile_handler) != DS_PROFILE_REG_RESULT_SUCCESS)
  {
    ...
    (void)ds_profile_delete_per_sub(DS_PROFILE_TECH_3GPP, *profile_num_ptr,sub_id);
    return ESTK_ERROR;
  }

  /* reset profile to default */
  if (ds_profile_reset_profile_to_default_per_sub(
        DS_PROFILE_TECH_3GPP,
        *profile_num_ptr, sub_id) != DS_PROFILE_REG_RESULT_SUCCESS)
  {
    UIM_MSG_ERR_0("failed to reset profile!");
  }

  /* Copy profile params into the newly created profile */
  if(profile_to_be_copied == TRUE)
  {
    if(estk_bip_3gpp_profile_copy_profile_contents(*profile_num_ptr,
                                                   profile_num_to_copy, sub_id) == FALSE)
    {
      UIM_MSG_ERR_0("Could not copy profile!");
      profile_to_be_copied = FALSE;
    }
  }
  ...
  /* set auth param */
  /* If EPC profiles exist, check if AUTH params can be used from the same */
  if(estk_curr_inst_ptr->bip_channel_table[ch_idx].apn_len > 0 &&
     (estk_get_profile_auth_param(ch_idx, DS_PROFILE_TECH_EPC, &profile_auth_type, sub_id) == ESTK_SUCCESS))
  {
    profile_info.len = sizeof(profile_auth_type);
    profile_info.buf = &profile_auth_type;
    if (ds_profile_set_param(
  ...
  /* If profile params were no copied, PDP type needs to be set explicity */
  if(profile_to_be_copied == FALSE)
  {
    home_pdp_type = DS_PROFILE_3GPP_PDP_IPV4V6;
    /* Roam PDP type needs to always be IPv4 for vzwadmin */
    if(estk_bip_match_apn(ch_idx, "vzwadmin") == TRUE)
    {
      roam_pdp_type = DS_PROFILE_3GPP_ROAMING_PDP_IP;
    }
    else
    {
      roam_pdp_type = DS_PROFILE_3GPP_ROAMING_PDP_IPV4V6;
    }
  }
  /* If GSTK_CFG_FEATURE_SLOT_ESTK_BIP_IP_DEFAULT is enabled, overwrite
     PDP type to IPv4 for OPEN CHANNEL DNS or when desitnation
     address is IPv4 */
  if(gstk_nv_get_feature_status(
       GSTK_CFG_FEATURE_SLOT_ESTK_BIP_IP_DEFAULT,
       estk_curr_inst_ptr->slot_id) == TRUE)
  {
    if ((open_ch_cmd != NULL && open_ch_cmd->dns_requested) ||
        (p_dest_addr_v4->ps_sin_family == DSS_AF_INET))
    {
      home_pdp_type = DS_PROFILE_3GPP_PDP_IP;
      roam_pdp_type = DS_PROFILE_3GPP_ROAMING_PDP_IP;
    }
  }

  UIM_MSG_HIGH_2("Home PDP type 0x%x Roam PDP type 0x%x",
                 home_pdp_type, roam_pdp_type);

  /* Set home PDP type */
  if (home_pdp_type != DS_PROFILE_3GPP_PDP_MAX)
  {

    profile_info.len = sizeof(home_pdp_type);
    profile_info.buf = &home_pdp_type;
  ...
  /* Set LTE and UMTS roam PDP type */
  if(roam_pdp_type != DS_PROFILE_3GPP_ROAMING_PDP_MAX)
  {
    profile_info.len = sizeof(roam_pdp_type);
    profile_info.buf = &roam_pdp_type;
    prof_item = (ds_profile_identifier_type)DS_PROFILE_3GPP_PROFILE_PARAM_LTE_ROAMING_PDP_TYPE;
    if (ds_profile_set_param(
            profile_handler,
            prof_item,
            &profile_info) != DS_PROFILE_REG_RESULT_SUCCESS)
    ...
    prof_item = (ds_profile_identifier_type)DS_PROFILE_3GPP_PROFILE_PARAM_UMTS_ROAMING_PDP_TYPE;
    if (ds_profile_set_param(
            profile_handler,
            prof_item,
            &profile_info) != DS_PROFILE_REG_RESULT_SUCCESS)
...
} /* estk_bip_set_net_policy_3gpp_profile */

/*=====================================================================================

FUNCTION: estk_bip_check_3gpp_profile_usability

DESCRIPTION :
 This function checks for usability of profile_num by checking the
 compatibility between the profile's pdp_type with address_type

PARAMETERS:
  profile_num:  [Input] 3GPP DS Profile number
  address_type: [Input] Type of address
  sub_id:       [Input] DS Sub Id
...
)
{
  ...
  UIM_MSG_HIGH_2("In estk_bip_check_3gpp_profile_usability profile_num: %d, address_type : %d",
  ...
  if(api_ret_status != DS_PROFILE_REG_RESULT_SUCCESS)
  {
    UIM_MSG_ERR_1("ds_profile_begin_transaction_per_sub failed %d", api_ret_status);
    return ESTK_ERROR;
  }
  ...
  return estk_status;
} /* estk_bip_check_3gpp_profile_usability */

/*===========================================================================

FUNCTION: estk_bip_copy_default_3gpp_profile_apn

DESCRIPTION :
  This function copies the default profile APN into
  the BIP channel table (if a valid APN is present)

PARAMETERS:
  ch_idx:      [Input] Index of the active channel
  profile_num: [Input] The default profile num
  sub_id:      [Input] DS Sub Id
...
)
{
  ...
  if(estk_curr_inst_ptr->bip_channel_table[ch_idx].apn_len != 0)
  {
    UIM_MSG_ERR_1("APN length non-zero: 0x%x",
                  estk_curr_inst_ptr->bip_channel_table[ch_idx].apn_len);
    return ESTK_ERROR;
  }
  ...
  /* Do not use default profile if it does not have a valid APN */
  if((profile_info.len == 0) ||
     (strlen(profile_info.buf) == 0))
  {
    UIM_MSG_ERR_1("Default profile: 0x%x has NULL APN", profile_num);
    /* Release Handle */
    (void)ds_profile_end_transaction(profile_hndl,
                                     DS_PROFILE_ACTION_CANCEL);
    return ESTK_ERROR;
  ...
  estk_curr_inst_ptr->bip_channel_table[ch_idx].apn = (uint8 *)GSTK_CALLOC(profile_info.len);

  if(estk_curr_inst_ptr->bip_channel_table[ch_idx].apn != NULL)
  {
    if(gstk_memcpy(estk_curr_inst_ptr->bip_channel_table[ch_idx].apn,
                   profile_info.buf,
                   profile_info.len,
                   profile_info.len,
       ...
       profile_info.len)
    {
      ...
      (void)ds_profile_end_transaction(profile_hndl,
                                       DS_PROFILE_ACTION_CANCEL);
      gstk_free(estk_curr_inst_ptr->bip_channel_table[ch_idx].apn);
      estk_curr_inst_ptr->bip_channel_table[ch_idx].apn = NULL;
      return ESTK_ERROR;
    }

    estk_curr_inst_ptr->bip_channel_table[ch_idx].apn_len = profile_info.len;

    for (i = 0; i < estk_curr_inst_ptr->bip_channel_table[ch_idx].apn_len; ++i)
    {
      UIM_MSG_HIGH_2("DEFAULT profile APN[%d] = 0x%x",
                     i, estk_curr_inst_ptr->bip_channel_table[ch_idx].apn[i]);
    }
...
} /* estk_bip_copy_default_3gpp_profile_apn */
...
DESCRIPTION :
  This function gets the default APN from EFS (if valid), and sets it
  in the BIP channel table
...
static int32 estk_bip_set_default_apn_from_efs(uint32 ch_idx)
{
  ...
  if(estk_curr_inst_ptr->bip_channel_table[ch_idx].apn_len == 0)
  {
    ...
    {
      if (estk_curr_inst_ptr->bip_channel_table[ch_idx].apn)
      {
      ...
      */
      estk_curr_inst_ptr->bip_channel_table[ch_idx].apn = (uint8 *)GSTK_CALLOC(apn_len + 1);
      ...
      if(estk_curr_inst_ptr->bip_channel_table[ch_idx].apn != NULL)
      {
        ...
        estk_curr_inst_ptr->bip_channel_table[ch_idx].apn_len = (uint32)apn_len;
        /* Add NULL character at end of APN */
        estk_curr_inst_ptr->bip_channel_table[ch_idx].apn[apn_len] = (uint8)0;

        for (i = 0; i < estk_curr_inst_ptr->bip_channel_table[ch_idx].apn_len; ++i)
        {
          UIM_MSG_HIGH_3("DEFAULT APN[%d] = 0x%x %c", i,
                          estk_curr_inst_ptr->bip_channel_table[ch_idx].apn[i],
                          estk_curr_inst_ptr->bip_channel_table[ch_idx].apn[i]);
        }
        ret = (int32)apn_len;
      }
      ...
      {
        UIM_MSG_ERR_1("Failed to allocate memory for APN!, apn_len: 0x%x", apn_len);
      }
    } /* if ((apn_len > 0) && (apn_len <= ESTK_APN_MAX_LENGTH)) */
    else
    {
...
FUNCTION: estk_bip_use_always_on_apn

DESCRIPTION :
  This function gets the always on APN (if a valid APN is available) and
  sets it in the BIP channel table
...
static estk_result_enum_type estk_bip_use_always_on_apn(uint32 ch_idx)
{
  ds_sys_apn_info_type *apn_info_ptr = NULL;
  int16                 dss_status   = DSS_ERROR;
  int16                 dss_errno    = 0;
  estk_result_enum_type estk_status  = ESTK_ERROR;
  size_t                apn_len      = 0;
  uint32                index        = 0;
  ...
  apn_info_ptr =
    (ds_sys_apn_info_type*)GSTK_CALLOC(sizeof(ds_sys_apn_info_type));

  if (apn_info_ptr == NULL)
  {
    UIM_MSG_ERR_0("malloc failed!");
    return ESTK_ERROR;
  }
  ...
  do
  {
    dss_status = ds_sys_conf_get_ex(DS_SYS_TECH_ALL,
                                 DS_SYS_CONF_APN_INFO,
    ...
    if (dss_status != DSS_SUCCESS)
    {
      UIM_MSG_ERR_2("failed to get default APN 0x%x, 0x%x",
                    dss_status, dss_errno);
      estk_status = ESTK_ERROR;
      break;
    }

    if(apn_info_ptr->curr_apn_info[DS_SYS_APN_TYPE_INTERNET].is_configured == FALSE)
    {
      UIM_MSG_ERR_0("Always ON APN is not configured");
      estk_status = ESTK_ERROR;
      break;
    }

    apn_len = strnlen(apn_info_ptr->curr_apn_info[DS_SYS_APN_TYPE_INTERNET].apn_name,
                      (DS_PROFILE_3GPP_MAX_APN_STRING_LEN + 1));

    if ((apn_len > 0) && (apn_len <= DS_PROFILE_3GPP_MAX_APN_STRING_LEN))
    {
      /*
        For 3GPP, DS uses strlen() to get length of APN (not apn_len)
        Hence, memory allocation size is apn_length + 1 in order to add
      ...
      */
      ...
      {
        if(gstk_memcpy(estk_curr_inst_ptr->bip_channel_table[ch_idx].apn,
                       apn_info_ptr->curr_apn_info[DS_SYS_APN_TYPE_INTERNET].apn_name,
                       apn_len,
                       apn_len,
                       sizeof(apn_info_ptr->curr_apn_info[DS_SYS_APN_TYPE_INTERNET].apn_name)) < apn_len)
        {
          gstk_free(estk_curr_inst_ptr->bip_channel_table[ch_idx].apn);
          estk_curr_inst_ptr->bip_channel_table[ch_idx].apn = NULL;
          estk_status = ESTK_ERROR;
        ...
        for (index = 0;
             index < estk_curr_inst_ptr->bip_channel_table[ch_idx].apn_len;
             index++)
        {
          UIM_MSG_HIGH_3("ALWAYS ON APN[%d] = 0x%x %c", index,
                          estk_curr_inst_ptr->bip_channel_table[ch_idx].apn[index],
                          estk_curr_inst_ptr->bip_channel_table[ch_idx].apn[index]);
        }
        estk_status = ESTK_SUCCESS;
        break;
  ...
  gstk_free(apn_info_ptr);

  return estk_status;
} /* estk_bip_use_always_on_apn */

/*===========================================================================

FUNCTION: estk_bip_use_default_3gpp_profile_apn

DESCRIPTION :
  This function gets the APN from the default DS 3GPP profile if a valid APN
  is present and sets it in the BIP channel table, and also gets
  the default DS profile number (if usable)

PARAMETERS:
  ch_idx:          [Input]  Index of the active channel
  profile_num_ptr: [Output] The profile num selected
...
)
{
  ...
  if((ch_idx >= ESTK_MAX_BIP_CH) ||
     (profile_num_ptr == NULL))
  {
    UIM_MSG_ERR_2("estk_bip_use_default_3gpp_profile_apn, ch_idx=0x%x profile_num_ptr=0x%x",
                  ch_idx, profile_num_ptr);
    return ESTK_BAD_INPUT_PARAM;
  }
  ...
  *profile_num_ptr = DS_PROFILE_UNSPECIFIED_NUM;
  ...
  /* Read the DS default profile */
  if(ds_profile_get_default_profile_num_per_subs(DS_PROFILE_TECH_3GPP,
                                        DS_PROFILE_3GPP_SOCKETS_PROFILE_FAMILY, sub_id,
                                        profile_num_ptr) == DS_PROFILE_REG_RESULT_SUCCESS)
  {
    /* Copy APN from DS default profile in to bip_channel_table[].apn */
    if(estk_bip_copy_default_3gpp_profile_apn(ch_idx, *profile_num_ptr, sub_id) != ESTK_SUCCESS)
    {
    ...
    /* Check if PDP type of profile matches the request. If not, do not use
       the profile number (instead, the APN copied from the default profile
       shall later be used to create a new profile) */
    if(estk_bip_check_3gpp_profile_usability(
         *profile_num_ptr,
         estk_curr_inst_ptr->curr_cmd.cmd.open_ch_pro_cmd_req.data_dest_address.address_type, sub_id)
         != ESTK_SUCCESS)
    {
      *profile_num_ptr = DS_PROFILE_UNSPECIFIED_NUM;
    }
  ...
  return ESTK_SUCCESS;
} /* estk_bip_use_default_3gpp_profile_apn */
...
PARAMETERS:
  ch_idx:                [Input]  Index of the active channel
  apn_select_method_ptr: [Output] The method used to select the APN
  profile_num_ptr:       [Output] The profile num selected
...
)
{
  ...
  boolean                 use_always_on_apn           = FALSE;
  ...
  if((ch_idx >= ESTK_MAX_BIP_CH) ||
     (apn_select_method_ptr == NULL) ||
     (profile_num_ptr == NULL))
  {
    UIM_MSG_ERR_3("estk_bip_process_null_apn, ch_idx=0x%x apn_select_method_ptr=0x%x profile_num_ptr=0x%x",
                  ch_idx, apn_select_method_ptr, profile_num_ptr);
    return ESTK_BAD_INPUT_PARAM;
  }

  /* Handle only cases where no valid APN is available already */
  if(estk_curr_inst_ptr->bip_channel_table[ch_idx].apn_len != 0)
  {
    UIM_MSG_ERR_1("APN length non-zero: 0x%x",
                  estk_curr_inst_ptr->bip_channel_table[ch_idx].apn_len);
  ...
  *apn_select_method_ptr = ESTK_BIP_APN_INVALID;
  *profile_num_ptr = DS_PROFILE_UNSPECIFIED_NUM;

  /* Check if ME had CSIM during previous power cycle */
  (void)gstk_nv_access_read(GSTK_NV_EXT_SLOT_CSIM_PRESENT,
                            estk_curr_inst_ptr->slot_id,
                            &nv_data);

  /* Check if using always on APN is allowed */
  use_always_on_apn = gstk_nv_get_feature_2_status(
                        GSTK_CFG_FEATURE_2_SLOT_ENABLE_BIP_ALWAYS_ON_APN_USE,
                        estk_curr_inst_ptr->slot_id);
  ...
  /* Try and retrieve the APN/profile to use in the following order:
     ...
     2. Use always ON APN if its valid and allowed to be used, else
     3. Use APN from default DS profile if its valid and allowed to be used, else
     4. Use DS sharing profile: 0xFE */
  for(index = (uint8)ESTK_BIP_APN_NV_73530;
      index <= (uint8)ESTK_BIP_APN_DS_SHARING;
      index++)
  {
    switch(index)
    {
      case ESTK_BIP_APN_NV_73530:
        ...
        if(mcfg_gcf_nv_get_status() && !nv_data.csim_present)
        {
          continue;
        }
        /* If valid APN cannot be retireved from NV 73530, try other methods */
        if(estk_bip_set_default_apn_from_efs(ch_idx) < 0)
        {
          continue;
        }
        else
        {
          *apn_select_method_ptr = ESTK_BIP_APN_NV_73530;
          return ESTK_SUCCESS;
        }
        break;

      case ESTK_BIP_APN_ALWAYS_ON:
         /* Always ON APN cannot be used for GCF usecase or
            if its not allowed per NV config */
        if((mcfg_gcf_nv_get_status() && !nv_data.csim_present) || (use_always_on_apn == FALSE))
        {
          continue;
        }
        /* If valid always on APN cannot be retrieved, try other methods */
        if(estk_bip_use_always_on_apn(ch_idx) != ESTK_SUCCESS)
        {
          continue;
        }
        else
        {
          *apn_select_method_ptr = ESTK_BIP_APN_ALWAYS_ON;
          return ESTK_SUCCESS;
        }
        break;

      case ESTK_BIP_APN_DEFAULT_PROFILE:
         /* Default DS 3GPP profile cannot be used if its not allowed per NV config */
        if(disable_default_profile_use == TRUE)
        {
          continue;
        }
        /* If valid always on APN cannot be retrieved from default
           profile, try other methods */
        if(estk_bip_use_default_3gpp_profile_apn(ch_idx, profile_num_ptr) != ESTK_SUCCESS)
        {
          continue;
        }
        else
        {
          *apn_select_method_ptr = ESTK_BIP_APN_DEFAULT_PROFILE;
          return ESTK_SUCCESS;
        }
        break;
      ...
      case ESTK_BIP_APN_DS_SHARING:
        ...
        *apn_select_method_ptr = ESTK_BIP_APN_DS_SHARING;
        return ESTK_SUCCESS;
    }
  }
...
FUNCTION: estk_bip_get_3gpp_profile

DESCRIPTION :
  This function gets the 3GPP profile to use

PARAMETERS:
  ch_idx:                [Input]  Index of the active channel
  apn_select_method_ptr: [Output] The method used to select the APN
  profile_num_ptr:       [Output] The profile num selected/created
...
)
{
  ...
  if((ch_idx >= ESTK_MAX_BIP_CH) ||
     (apn_select_method_ptr == NULL) ||
      (profile_num_ptr == NULL))
  {
    UIM_MSG_ERR_3("estk_bip_get_3gpp_profile, ch_idx:0x%x apn_select_method_ptr:0x%x profile_num_ptr:0x%x",
                  ch_idx, apn_select_method_ptr, profile_num_ptr);
  ...
  *apn_select_method_ptr = ESTK_BIP_APN_INVALID;
  ...
  if(estk_curr_inst_ptr->bip_channel_table[ch_idx].apn_len == 0)
  {
    if(estk_bip_process_null_apn(
         ch_idx, apn_select_method_ptr, profile_num_ptr) != ESTK_SUCCESS)
    {
      UIM_MSG_ERR_0("Null APN case: Could not set valid 3GPP profile");
      return ESTK_ERROR;
    }
  }
  else
  {
    *apn_select_method_ptr = ESTK_BIP_APN_OPEN_CH;
  }

  /* If a valid profile has not yet been retrieved and valid APN is available,
     set profile based on APN */
  if((*profile_num_ptr == DS_PROFILE_UNSPECIFIED_NUM) &&
     (estk_curr_inst_ptr->bip_channel_table[ch_idx].apn_len != 0))
  {
    if(estk_bip_set_net_policy_3gpp_profile(
         ch_idx, *apn_select_method_ptr, profile_num_ptr) != ESTK_SUCCESS)
    {
      return ESTK_ERROR;
    }
...
} /* estk_bip_get_3gpp_profile */

/*===========================================================================

FUNCTION: estk_bip_set_net_policy_profile
...
PARAMETERS:
  access_tech:    [Input]  Current access technology
  ch_idx:         [Input]  Index of the active channel
  net_policy_ptr: [Output] The net policy where the profile to be set
...
)
{
  ...
  struct ps_sockaddr_in           *p_dest_addr_v4_ptr = NULL;
  boolean                          profile_3gpp       = TRUE;
  boolean                          profile_3gpp2      = TRUE;
  gstk_open_ch_req_type           *open_ch_cmd_ptr    = NULL;
  estk_result_enum_type            estk_status        = ESTK_ERROR;
  estk_bip_apn_select_method_type  apn_sel_method     = ESTK_BIP_APN_INVALID;

  ESTK_RETURN_ERROR_IF_NULL_PTR(estk_curr_inst_ptr, ESTK_ERROR);

  open_ch_cmd_ptr = &estk_curr_inst_ptr->curr_cmd.cmd.open_ch_pro_cmd_req;

  UIM_MSG_HIGH_3("estk_bip_set_net_policy_ptr_profile: access_tech=0x%x ch_idx=0x%x net_policy_ptr=0x%x",
                 access_tech, ch_idx, net_policy_ptr);

  if(ch_idx >= ESTK_MAX_BIP_CH || net_policy_ptr == NULL)
  {
    return ESTK_BAD_INPUT_PARAM;
  }
  ...
  if (open_ch_cmd_ptr != NULL &&
      !open_ch_cmd_ptr->dns_requested &&
      p_dest_addr_v4_ptr == NULL)
  {
    UIM_MSG_ERR_0("invalid dest addr!");
    return ESTK_ERROR;
  }

  if (open_ch_cmd_ptr != NULL && !open_ch_cmd_ptr->dns_requested)
  {
    switch (p_dest_addr_v4_ptr->ps_sin_family)
    {
      case DSS_AF_INET:
        net_policy_ptr->family = DSS_AF_INET;
        break;
      case DSS_AF_INET6:
        net_policy_ptr->family = DSS_AF_INET6;
        break;
      default:
        UIM_MSG_ERR_1("invalid ps_sin_family 0x%x!",
                      p_dest_addr_v4_ptr->ps_sin_family);
        return ESTK_ERROR;
    }
  }
  ...
  {
    case GSTK_BEARER_DEFAULT:
      /* Get the 3GPP profile to use */
      estk_status = estk_bip_get_3gpp_profile(ch_idx, &apn_sel_method, &profile_num);
      if(estk_status != ESTK_SUCCESS)
      {
        /* Let DS chose the profile to use -will be used only
           if a valid 3GPP2 profile can be set next */
        profile_num  = DS_PROFILE_ALWAYS_SHARE_PROFILE_NUM;
        profile_3gpp = FALSE;
        UIM_MSG_ERR_0("Failed to set 3gpp profile, use profile 0xFE");
      }

      UIM_MSG_HIGH_2("Use profile 0x%x for 3GPP, APN select method: 0x%x",
                     profile_num, apn_sel_method);
      net_policy_ptr->umts.pdp_profile_num = profile_num;

      /* Get the 3GPP2 profile to use - we need set both 3gpp profile
         and 3gpp2 profile to support inter-RAT handover */
      estk_status = estk_bip_get_3gpp2_profile(ch_idx, apn_sel_method, &profile_num);
      if(estk_status != ESTK_SUCCESS)
      {
        /* Let DS chose the profile to use - will be used only if a
           valid 3GPP profile has been set */
        profile_num  = DS_PROFILE_ALWAYS_SHARE_PROFILE_NUM;
        profile_3gpp2 = FALSE;
        UIM_MSG_ERR_0("Failed to set 3gpp2 profile, use profile 0xFE");
      }

      UIM_MSG_HIGH_1("Use profile 0x%x for 3GPP2", profile_num);
      net_policy_ptr->cdma.data_session_profile_id = profile_num;

      /* Need to set atleast one of 3GPP or 3GPP2 profiles */
    ...
    case GSTK_EUTRAN_MAPPED_UTRAN_PS:
      ...
      /* Get the 3GPP2 profile to use in order to support inter-RAT handover */
      estk_status = estk_bip_get_3gpp2_profile(ch_idx, apn_sel_method, &profile_num);
  ...
  return ESTK_SUCCESS;
} /* estk_bip_set_net_policy_ptr_profile */
...
===========================================================================*/
static estk_result_enum_type estk_bip_check_and_start_fallback(uint32 index)
{
  boolean                                cellular_avail      = FALSE;
  ...
  if((estk_curr_inst_ptr->bip_channel_table[index].nw_type == ESTK_NETWORK_WLAN) &&
     (gstk_nv_get_feature_status(
        ...
        estk_curr_inst_ptr->slot_id) == TRUE))
  {
    cellular_avail = estk_check_cellular_availability(estk_curr_inst_ptr->slot_id);

    /* Fallback to Cellular */
    if(cellular_avail == TRUE)
    {
      gstk_status = gstk_io_ctrl(GSTK_IO_GET_ACTIVE_DATA_SUB_SLOT_ID,
                                 (void *)&estk_curr_inst_ptr->slot_id,
                                 (void *)&active_slot);

      /* Allow BIP on either on Activate slot or if the max activate data is more than 1 */
      if(((gstk_status == GSTK_SUCCESS) && (active_slot == estk_curr_inst_ptr->slot_id)) || 
           (estk_shared_info.max_active_data > 1))
      {
  ...
  /* If current bearer is Cellular, and BIP over WLAN support is present,
     Cellular has higher priority over WLAN, WLAN is usable and Bearer is
     default type, it means a fallback to WLAN can be attempted */
  ...
  uint16 address_family)
{
  ...
  if(estk_bip_set_net_policy_profile(estk_curr_access_tech,
                                     index,
...
===========================================================================*/
estk_result_enum_type estk_bip_establish_link(uint32 index, uint16 address_family)
{
...
===========================================================================*/
static estk_result_enum_type estk_modify_apn_based_on_mapping_table(uint32 index)
{
  int32                    idx         = 0;
  uint32                   i           = 0;
  uint8                   *new_apn     = NULL;
...
PARAMETERS:
  ...
  profile_tech_type : [Input] Profile tech type to search
...
static estk_result_enum_type estk_modify_apn_based_on_class_and_rat(
  ...
  ds_profile_tech_etype profile_tech_type)
{
  ...
  UIM_MSG_HIGH_4("estk_modify_apn_based_on_class_and_rat(): index=0x%x, "
                 "rat: 0x%x, "
                 "class_type: 0x%x, "
                 "profile_tech_type: 0x%x", index, rat, class_type, profile_tech_type);
  ...
  /* Get an iterator first */
  itr_status = ds_profile_get_list_itr_per_sub(profile_tech_type, &profile_list, &profile_itr, sub_id);
  ...
  {
    /* reset the length as DS API ds_profile_get_info_by_itr() might have changed it
       during prior iteration */
    profile_list_info.name->len = DS_PROFILE_3GPP_MAX_PROFILE_NAME_LEN;
    ...
    if(profile_tech_type == DS_PROFILE_TECH_EPC)
    {
      /* Get the APN class used for the access technology specified by variable "rat"
         from EPC profile */
      ESTK_BIP_GET_EPC_PROFILE_PARAM(
        api_ret_status,
        profile_hndl,
        profile_itr,
        profile_info,
        rat,
        (ds_profile_identifier_type)DS_PROFILE_TECH_COMMON_PROFILE_PARAM_APN_CLASS, sub_id);
    }
    else
    {
...
===========================================================================*/
static estk_result_enum_type estk_modify_apn_based_on_class(uint32 index)
{
  ds_profile_tech_etype rat = DS_PROFILE_TECH_INVALID;
  typedef struct
  {
    char *apn;
  ...
  /* look-up in EPC profiles */
  if(estk_modify_apn_based_on_class_and_rat(
                index,
                rat,
                apn_class_info[idx].apn_class,
                DS_PROFILE_TECH_EPC) != ESTK_SUCCESS)
  {
  /* If look-up in EPC profiles is not successful then search in technology specfic
     DS profiles */
...
FUNCTION: estk_bip_match_apn

DESCRIPTION :
  This function matches input APN with the APN corresponding to the
  BIP Channel

PARAMETERS:
  index   : [Input] Index of the channel to be populated
  apn_ptr : [Input] APN to be matched
...
static boolean estk_bip_match_apn(
  ...
  char   *apn_ptr)
{
  uint32 open_ch_apn_len = 0;
  uint32 input_apn_len   = 0;

  ESTK_RETURN_ERROR_IF_NULL_PTR(estk_curr_inst_ptr, FALSE);

  if ((apn_ptr == NULL) ||
      (index >= ESTK_MAX_BIP_CH))
  {
    return FALSE;
  }

  /* Check if APN is terminated by NULL */
  open_ch_apn_len = estk_curr_inst_ptr->bip_channel_table[index].apn_len;

  if((open_ch_apn_len > 0) &&
     (estk_curr_inst_ptr->bip_channel_table[index].apn[open_ch_apn_len - 1] == 0))
  {
    open_ch_apn_len = open_ch_apn_len - 1;
  }

  input_apn_len = strlen(apn_ptr);

  /*Check if the APN matches*/
  if((open_ch_apn_len == input_apn_len) &&
     (strncasecmp(apn_ptr,
                  (char *)estk_curr_inst_ptr->bip_channel_table[index].apn,
                  open_ch_apn_len) == 0))
  {
     UIM_MSG_HIGH_0("Found matching APN");
     return TRUE;
  }
  return FALSE;
} /* estk_bip_match_apn */
...
)
{
  ...
  *addi_res_ptr = (uint8)GSTK_NO_SPECIFIC_CAUSE_CAN_BE_GIVEN;
  ...
  cellular_avail = estk_check_cellular_availability(slot);
  ...
  *addi_res_ptr = (uint8)GSTK_NO_SERVICE_AVAILABLE;
  ...
  /* Retrive Policyman max data allowed capability */
  estk_retrieve_max_active_data_sub_info();
  
  /* Allow the data on requested slot, if slot is the active data slot OR if dual data is supported */
  if(((gstk_status == GSTK_SUCCESS) && (slot == active_slot)) || 
      (estk_shared_info.max_active_data > 1))
  {
    if(estk_bip_check_existing_gsm_cs_call())
    {
  ...
  /* When Dual Data not supported and if the BIP is requested onn inactivate data slot, 
       populate the right cause */
  else if((gstk_status == GSTK_SUCCESS) && (slot != active_slot))
  {
     *addi_res_ptr = (uint8)GSTK_NO_SPECIFIC_CAUSE_CAN_BE_GIVEN;
  }

  UIM_MSG_HIGH_3("WLAN available: 0x%x, Cellular data available: 0x%x, WLAN unusable: 0x%x",
                 wlan_avail, cellular_avail, estk_shared_info.wlan_unusable);
  ...
  estk_network_type nw_type)
{
  ...
  /* Get the DS Sub ID based on the Slot ID */
  (void) gstk_io_ctrl(GSTK_IO_SLOT_ID_TO_DS_SUBSCRIPTION_ID,
                      &estk_curr_inst_ptr->slot_id,
                      &estk_curr_inst_ptr->bip_channel_table[index].ds_sub_id);  
  ...
  /* Clear only if it is not fallback case */
  if (!in_fallback)
  {
  ...
  else
  {
    ...
    {
      if (estk_curr_inst_ptr->state == ESTK_IDLE_ST)
      {
        UIM_MSG_HIGH_0("BIP link successfully established - TR sent");
      }
      else
      {
        UIM_MSG_HIGH_0("Pending DS callbacks to determine status of BIP link establishment");
      }
      return ESTK_SUCCESS;
    }
  }
...
)
{
        ...
        GSTK_CFG_FEATURE_ME_ESTK_QMI_SEND_NO_AND_NULL_ALPHA_IND) == TRUE))
  {
    ...
    if(gstk_nv_get_me_feature_status(
         GSTK_CFG_FEATURE_ME_ESTK_IGNORE_ALPHA) == FALSE)
    {
...
===========================================================================*/
uint32 estk_util_min_3num(uint32 a, uint32 b, uint32 c)
{
  if (a<b)
  {
    if (a<c)
...
)
{
  ...
  copy_len = estk_util_min_3num(ESTK_BIP_MAX_CH_DATA_SIZE,
...
)
{
  ...
  {
    if((estk_curr_inst_ptr->bip_channel_table[index].first_dss_sendto_done) &&
       (estk_curr_inst_ptr->bip_channel_table[index].receive_data == FALSE))
    {
    ...
    for(ch_idx = 0; ch_idx < ESTK_MAX_BIP_CH; ch_idx++)
    {
      ...
      switch (estk_curr_inst_ptr->bip_channel_table[ch_idx].current_command_id)
      {
        ...
        case GSTK_CLOSE_CH_REQ:
          ...
          {
            estk_curr_inst_ptr->bip_channel_table[ch_idx].status = ESTK_CH_DROPPED;
            UIM_MSG_HIGH_0("Update Link Dropped");
          }
          break;
        case GSTK_SEND_DATA_REQ:
          (void)estk_bip_process_send_data_error(ch_idx);
          break;
        default:
          if ((estk_curr_inst_ptr->bip_channel_table[ch_idx].status == ESTK_CH_ACTIVE) ||
...
)
{
  ...
  for (i = 0; i < estk_num_of_instances; i++)
  {
    ...
    for(ch_idx = 0; ch_idx < ESTK_MAX_BIP_CH; ch_idx++)
    {
      UIM_MSG_HIGH_4("Channel Table Index: 0x%x, Status: 0x%x, NW type: 0x%x, Slot 0x%x",
                     ch_idx,
                     estk_instances_ptr[i]->bip_channel_table[ch_idx].status,
                     estk_instances_ptr[i]->bip_channel_table[ch_idx].nw_type,
                     estk_instances_ptr[i]->slot_id);

      /* Process WLAN starus change event only for Channels open over WLAN */
      if (estk_instances_ptr[i]->bip_channel_table[ch_idx].nw_type != ESTK_NETWORK_WLAN)
      {
        continue;
      }

      UIM_MSG_HIGH_1("Current Command: 0x%x",
                     estk_instances_ptr[i]->bip_channel_table[ch_idx].current_command_id);

      switch (estk_instances_ptr[i]->bip_channel_table[ch_idx].current_command_id)
      {
        case GSTK_OPEN_CH_REQ:
        case GSTK_CLOSE_CH_REQ:
          /* OPEN/CLOSE CHANNEL is in progress */
          if (estk_instances_ptr[i]->bip_channel_table[ch_idx].status == ESTK_CH_ACTIVE)
          {
            estk_instances_ptr[i]->bip_channel_table[ch_idx].status = ESTK_CH_DROPPED;
          }
          break;
        case GSTK_SEND_DATA_REQ:
          (void)estk_bip_process_send_data_error(ch_idx);
        ...
        default:
          if ((estk_instances_ptr[i]->bip_channel_table[ch_idx].status == ESTK_CH_ACTIVE) ||
              ((estk_instances_ptr[i]->bip_channel_table[ch_idx].status == ESTK_CH_PENDING) &&
               (estk_instances_ptr[i]->bip_channel_table[ch_idx].on_demand_link == TRUE)))
          {
            UIM_MSG_HIGH_0("send link drop envelope cmd");
            estk_instances_ptr[i]->bip_channel_table[ch_idx].status = ESTK_CH_DROPPED;
            ...
            (void)estk_bip_send_ch_status_event(
                    ...
                    estk_instances_ptr[i]->bip_channel_table[ch_idx].channel_id,
                    GSTK_CH_STATUS_LINK_DROPPED);
          }
          break;
      }
...
estk_result_enum_type estk_clean_up_bip_open_ch_timer_exp(void)
{
  ...
  if(estk_curr_inst_ptr->curr_bip_cmd_index >= ESTK_MAX_BIP_CH)
  {
    return ESTK_ERROR;
  }
...
void estk_bip_full_clean_up_slot(gstk_slot_id_enum_type slot_id)
{
  ...
  estk_instances_ptr[(uint32)slot_id - 1]->curr_bip_cmd_index = ESTK_MAX_BIP_CH;
...
/*===========================================================================
FUNCTION: estk_check_cellular_availability

DESCRIPTION :
  This function checks the cellular availability on DDS and non-DDS
...
RETURN VALUE:
  TRUE   : Cellular Available
  FALSE  : Cellular Not Available
...
boolean estk_check_cellular_availability(gstk_slot_id_enum_type slot_id)
{
  ...
  if((gstk_status == GSTK_SUCCESS) && (active_slot == slot_id))
  {
                        ...
                        (void *)&sys_status) == GSTK_SUCCESS)
    {
      ...
      if ((sys_status->avail_sys[0].technology ==
          ...
          (sys_status->avail_sys[0].technology ==
          GSTK_DS_SYS_NETWORK_3GPP2))
      {
  ...
  {
    /* In case where BIP can activte on both slots, on a non-DDS sub BIP / session 
       can be activated on-demand, so check only for service on that sub */
    if (gstk_io_ctrl(GSTK_IO_GET_CURR_LOC_STATUS,
                   (void *)&slot_id,
                   (void *)&loc_status) == GSTK_SUCCESS)
    {
  ...
  return cellular_avail;
} /* estk_check_cellular_availability */
