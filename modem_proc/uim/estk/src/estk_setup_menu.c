...
$Header: //components/rel/uim.mpss/6.1.0/estk/src/estk_setup_menu.c#3 $
...
===========================================================================*/
uint16 estk_setup_menu_pack_tlv(
  uint8        *out_buff_ptr,
...
)
{
  ...
  uint16           buff_size        = ESTK_SETUP_MENU_RAW_CMD_BUFFER_SIZE;
  uint8            setup_menu_hdr[] =
    { 0xD0, 0x00, 0x81, 0x03, 0x01, 0x25, 0x00, 0x82, 0x02, 0x81, 0x82};
  uint16           hdr_size         = sizeof(setup_menu_hdr);
  uint16           len              = 0;
  uint16           idx              = 0;
  uint16           i                = 0;
  ...
  /***** SETUP MENU *****/
  GSTK_UTIL_SELECTIVE_DUMP_BYTE_ARRAY("", raw_cmd_buff, idx);
  return ESTK_SUCCESS;
