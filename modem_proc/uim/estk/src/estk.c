...
GENERAL DESCRIPTION:ENHANCED STK layer
  ...
  functionality
          ...
          Copyright � 2009 - 2021 QUALCOMM Technologies, Inc(QTI).
          All Rights Reserved.
...
$Header: //components/rel/uim.mpss/6.1.0/estk/src/estk.c#6 $

when       who     what, where, why
--------   ---     --------------------------------------------------------------------
01/21/21   rm      Fixed function prototype issue
05/19/20   dt      Add API to check keep channel open status
...
08/01/17   gs      Cache WMS transport type
06/28/17   dt      Hold SEND SMS till CP ACK for SMS PP is received
04/21/17   gs      Don't check for transport capability while sending SMS
01/30/17   gs      Support of Dual Data Capability
12/23/16   gs      Register for SMS Transport Type
12/01/16   dt      KW fixes array out of bound in estk_gstk_init
10/24/16   gs      Changes in TP based on RAT and App Type
09/16/16   vr      End the setup call after setup call TR expiry
...
03/22/16   gs      ESTK cm init movement for IoE memory
10/01/15   shr     Clear ESTK CM Call ID correctly
...
05/13/15   vr      ESTK cleans up any data in shared_info only if it owns
...
03/19/15   dy      Fix setup evt list ch status notification causing error
02/24/15   dy      Fix starting channel reuse timer too early
02/10/15   dy      Fix send_dtmf not caching icon information
01/14/15   vr      Intialize call list info
01/14/15   vr      Correctly handle ESTK clean-up
...
static gstk_profile_dl_support_ext_type estk_profile_value_list[] =
{
  ...
  ,{1, GSTK_GEO_LOC_REPORT_SUPPORT, (uint8)GSTK_APP_NONE}
  ,{1, GSTK_GEO_LOC_REQUEST_SUPPORT, (uint8)GSTK_APP_NONE}
...
static gstk_toolkit_cmd_reg_info_type estk_format_reg_list[] =
{
  ...
  {{GSTK_REG_CATEGORY_N_CMD, GSTK_REG_GEO_LOC_REQ_EVT},             GSTK_HANDLE_MODEM_FUNC_ONLY}
};
...
===========================================================================*/
estk_result_enum_type estk_handle_user_or_disp_conf_timeout(void)
{
...
void estk_init( void )
{
  ...
  estk_shared_info.call_info.call_id               = ESTK_INVALID_CM_CALL_ID;
  ...
  rex_init_crit_sect(&estk_shared_info.estk_crit_sect);
  
  for(i = 0; i < GSTK_MAX_NUM_OF_INSTANCES; i++)
  {
    estk_shared_info.wms_info[i].trans_changed                 = FALSE;
    estk_shared_info.wms_info[i].as_id                         = SYS_MODEM_AS_ID_NONE;
    estk_shared_info.wms_info[i].wms_transport_type.reg_status = FALSE;
    estk_shared_info.wms_info[i].wms_transport_type.trans_type = WMS_MSG_TRANSPORT_TYPE_MAX;
  } 
} /* estk_init */
...
void estk_register_proactive_cmds(void)
{
  ...
  if(!(gstk_nv_get_me_feature_status(
       GSTK_CFG_FEATURE_ME_DISABLE_ESTK_CM_SUPPS_INTERFACE) == TRUE))
  {
    bitmask = bitmask | GSTK_REG_SEND_USSD_EVT;
    bitmask = bitmask | GSTK_REG_SEND_SS_EVT;
  }
  if(!(gstk_nv_get_me_feature_status(
       GSTK_CFG_FEATURE_ME_DISABLE_ESTK_CM_VOICE_CALL_INTERFACE) == TRUE))
  {
     bitmask = bitmask | GSTK_REG_SETUP_CALL_EVT;
     bitmask = bitmask | GSTK_REG_SEND_DTMF_EVT;
  }
  if(!(gstk_nv_get_me_feature_status(
       GSTK_CFG_FEATURE_ME_DISABLE_ESTK_WMS_INTERFACE) == TRUE))
  {
     bitmask = bitmask | GSTK_REG_SEND_SMS_EVT;
  }
  if(!(gstk_nv_get_me_feature_status(
       GSTK_CFG_FEATURE_ME_DISABLE_ESTK_BIP_INTERFACE) == TRUE))
  {
...
#ifdef FEATURE_GSTK_QMI_LOC
  bitmask = bitmask | GSTK_REG_GEO_LOC_REQ_EVT;
#endif

  UIM_MSG_LOW_1("estk_register_proactive_cmds: bitmask is 0x%x.", bitmask);
  ...
  if(bitmask > 0)
  {
    ...
    num_entries = (uint8)(size / sizeof(gstk_toolkit_cmd_reg_info_type));
...
void estk_gstk_init(uint8 gstk_num_of_instances)
{
  ...
  for (i = 0; ((i < estk_num_of_instances) && (i < GSTK_MAX_NUM_OF_INSTANCES)); ++ i)
  {
    ...
    estk_instances_ptr[i]->estk_cm_cs_call_connected = 0;
    ...
    rex_def_timer_ex(&(estk_instances_ptr[i]->user_or_disp_conf_timer),
                     ...
                     (i + 1));
#ifdef FEATURE_GSTK_QMI_LOC
    rex_def_timer_ex(&(estk_instances_ptr[i]->geo_loc_rsp_timer),
                     estk_geo_loc_rsp_timer_cb,
                     (i + 1));
#endif
    estk_instances_ptr[i]->curr_bip_cmd_index = ESTK_MAX_BIP_CH;
    estk_bip_channel_table_init(i);

    estk_instances_ptr[i]->dns_addrs_len = 0;
    estk_instances_ptr[i]->keep_ch_open = FALSE;
    estk_instances_ptr[i]->ch_open_id = ESTK_MAX_BIP_CH;
    estk_instances_ptr[i]->sms_link_ctrl_timer = (uint8) ESTK_SMS_LINK_CTRL_TIMER_MAX;
    ...
    estk_instances_ptr[i]->trans_type = WMS_MSG_TRANSPORT_TYPE_CS;
    estk_instances_ptr[i]->estk_geo_loc_engine_lock_status = FALSE;
    estk_instances_ptr[i]->is_3gpp2_sms = FALSE;
...
void  estk_free_cmd_buf_data(estk_cmd_type *cmd_ptr )
{
  ...
  case ESTK_WMS_CFG_EVT_CMD:
    break;
  case ESTK_WMS_MSG_EVENT_ACK_REPORT:
    break;
  default:
    break;
  } /* End of switch */
...
)
{
  ...
  if ((req_ptr == NULL) || (!req_ptr->pro_cmd_reg_data.gstk_req_rsp_ptr))
  {
    return;
  }
...
)
{
  ...
  if (estk_status == ESTK_SUCCESS)
  {
    ...
    case GSTK_GEO_LOC_REQ:
#ifdef FEATURE_GSTK_QMI_LOC
      estk_status = estk_process_geo_loc_req(gstk_req_rsp_ptr);
      cnf_type = GSTK_GEO_LOC_CNF;
#endif
      break;
    default:
      /* Clear saved command */
...
gstk_status_enum_type estk_process_cmd(void)
{
  ...
  while ((cmd_ptr = (estk_cmd_type *) q_get(&estk_cmd_q)) != NULL)
  {
    ...
    {
#if defined(FEATURE_GSTK_QMI_LOC) && !defined(FEATURE_UIM_TEST_FRAMEWORK)
      if (cmd_ptr->hdr.cmd == ESTK_LOC_GET_ENGINE_LOCK_IND)
      {
        result = estk_process_gps_data_cmd(cmd_ptr);
        break;
      }
#endif
    ...
    case ESTK_WMS_MSG_SUBMIT_RPT_EVT_CMD:
      ...
      break;
    case ESTK_WMS_MSG_EVENT_ACK_REPORT:
      result = estk_process_wms_event_ack_report_cmd(cmd_ptr);
      break;
    case ESTK_WMS_CFG_EVT_CMD:
...
#endif
      break;
    case ESTK_GEO_LOC_RSP_TIMER_EXP_CMD:
#ifdef FEATURE_GSTK_QMI_LOC
      result = estk_process_geo_loc_rsp_timeout();
#endif
      break;
    default:
      result = ESTK_ERROR;
...
)
{
  ...
  case GSTK_SEND_DTMF_REQ:
    ...
    /*
      Copy the icon into estk_curr_inst_ptr->curr_cmd
      as the icon information is sent after ESTK gets call list event
    */
    if(gstk_req_ptr->cmd.send_dtmf_pro_cmd_req.icon.present)
    {
      estk_curr_inst_ptr->icon_present = TRUE;
      send_dtmf_ptr->icon.present = TRUE;
      send_dtmf_ptr->icon.qualifier =
        gstk_req_ptr->cmd.send_dtmf_pro_cmd_req.icon.qualifier;
      send_dtmf_ptr->icon.height =
        gstk_req_ptr->cmd.send_dtmf_pro_cmd_req.icon.height;
      send_dtmf_ptr->icon.width =
        gstk_req_ptr->cmd.send_dtmf_pro_cmd_req.icon.width;
      send_dtmf_ptr->icon.size =
        gstk_req_ptr->cmd.send_dtmf_pro_cmd_req.icon.size;
      send_dtmf_ptr->icon.image_coding_scheme =
        gstk_req_ptr->cmd.send_dtmf_pro_cmd_req.icon.image_coding_scheme;
      send_dtmf_ptr->icon.rec_num =
        gstk_req_ptr->cmd.send_dtmf_pro_cmd_req.icon.rec_num;

      if (send_dtmf_ptr->icon.data != NULL)
      {
        gstk_free(send_dtmf_ptr->icon.data);
        send_dtmf_ptr->icon.data = NULL;
      }

      send_dtmf_ptr->icon.data =
        (uint8 *)GSTK_CALLOC(send_dtmf_ptr->icon.size);

      if (send_dtmf_ptr->icon.data == NULL)
      {
        send_dtmf_ptr->icon.size = 0;
        return ESTK_ERROR;
      }

      if (gstk_memcpy(send_dtmf_ptr->icon.data,
                      gstk_req_ptr->cmd.send_dtmf_pro_cmd_req.icon.data,
                      gstk_req_ptr->cmd.send_dtmf_pro_cmd_req.icon.size,
                      send_dtmf_ptr->icon.size,
                      gstk_req_ptr->cmd.send_dtmf_pro_cmd_req.icon.size) <
          gstk_req_ptr->cmd.send_dtmf_pro_cmd_req.icon.size)
  ...
  case GSTK_REFRESH_REQ:
    ...
    break;
  case GSTK_GEO_LOC_REQ:
    if (gstk_req_ptr->cmd.geo_loc_pro_cmd_req.icon.present)
    {
      estk_curr_inst_ptr->icon_present = TRUE;
    }
    break;
...
static void estk_cleanup_priv_data(gstk_cmd_enum_type  terminal_response_type)
{
  ...
  case GSTK_END_PROACTIVE_CMD_REQ:
    ...
    estk_curr_inst_ptr->state = ESTK_IDLE_ST;
    break;
  case GSTK_GEO_LOC_CNF:
    break;
  case GSTK_SEND_SMS_CNF:
    estk_curr_inst_ptr->is_3gpp2_sms = FALSE;
    break;
  default:
    break;
  }/* End of switch */
  ...
  if (estk_shared_info.call_info.dtmf_len <= 0)
  {
    if(estk_curr_inst_ptr->slot_id == estk_shared_info.slot_owns_cm_cli)
    {
      estk_shared_info.call_info.call_id = ESTK_INVALID_CM_CALL_ID;
      estk_shared_info.slot_owns_cm_cli = GSTK_SLOT_ID_MAX;
    }
  }
  if(estk_curr_inst_ptr->slot_id == estk_shared_info.slot_owns_wms_cli)
  {
    estk_shared_info.slot_owns_wms_cli = GSTK_SLOT_ID_MAX;
  }
  ...
  gstk_slot_id_enum_type    slot_id)
{
  ...
  switch(estk_curr_inst_ptr->curr_cmd.hdr_cmd.command_id)
  {
    ...
    case GSTK_SEND_DATA_REQ:
      ...
      break;
    case GSTK_SETUP_CALL_REQ:
      if (estk_shared_info.call_info.call_id != ESTK_INVALID_CM_CALL_ID)
      {
        /* End the setup call request sent to the CM */
        estk_result = estk_cm_end_setup_call_request();
      }
      terminal_response_type = GSTK_SETUP_CALL_CNF;
      break;
...
#ifdef FEATURE_GSTK_QMI_LOC
  /* 4. Clean up GPS data */
  estk_gps_full_clean_up_slot(slot_id);
#endif
...
static void estk_hot_swap_cleanup(gstk_slot_id_enum_type slot_id)
{
  ...
  estk_free_setup_menu_data(&estk_curr_inst_ptr->setup_menu_cmd);
#endif /*FEATURE_ESTK_SETUP_MENU_INTERFACE*/
  estk_curr_inst_ptr->estk_cm_cs_call_connected = 0;

  if(slot_id == estk_shared_info.slot_owns_cm_cli)
  {
    estk_shared_info.in_legacy_mode = FALSE;
    estk_shared_info.call_info.call_id = ESTK_INVALID_CM_CALL_ID;
    estk_shared_info.call_info.call_sys_mode = SYS_SYS_MODE_NO_SRV;
  ...
  if(slot_id == estk_shared_info.slot_owns_wms_cli)
  {
    estk_shared_info.slot_owns_wms_cli = GSTK_SLOT_ID_MAX;
  }
   ...
   const gstk_cmd_from_card_type  *gstk_req_rsp_ptr)
{
  ...
  const estk_proactive_cmd_rsp_map pro_cmd_rsp_map[] =
  {
    ...
    {GSTK_GEO_LOC_REQ,                  GSTK_GEO_LOC_CNF}
  }; /* pro_cmd_rsp_map */
...
/*===========================================================================
FUNCTION: estk_retrieve_max_active_data_sub_info

DESCRIPTION :
  This function combines GSTK NV and PM max activate data info to determine if dual data is allowed
...
SIDE EFFECTS:
  None
===========================================================================*/
void estk_retrieve_max_active_data_sub_info(void)
{
  policyman_item_collection_t const   *collect_item_ptr = NULL;
  policyman_item_id_t                  id               = POLICYMAN_ITEM_DEVICE_CONFIGURATION;
  uint32                               active_data      = 0;

  /* If dual data subscription is disabled then need not check for Dual Data Capability */
  if(gstk_nv_get_me_feature_status(GSTK_CFG_FEATURE_ME_DISABLE_DUAL_DATA))
  {
    return;
  }

  /* Reset this before requesting PM */
  estk_shared_info.max_active_data = 1;
  if (POLICYMAN_SUCCEEDED(policyman_get_items_block_msim((policyman_item_id_t const *)&id,
                                                     1, &collect_item_ptr)))
  {
    if((collect_item_ptr != NULL) && 
    	POLICYMAN_SUCCEEDED(policyman_device_config_max_active_data(collect_item_ptr->pItems[0],
  	                                              (size_t *)&active_data)))
    {
      if(active_data > 1)
      {
        estk_shared_info.max_active_data = (uint8) active_data;
      }
      else
      {
        UIM_MSG_MED_1("Active Data returned is 0x%x", active_data);
      }
      policyman_item_collection_release(collect_item_ptr);	  
    } 
    else
    {
      UIM_MSG_ERR_0("PM MAX Active Data Req Failed");
    }
  }
}/* estk_retrieve_max_active_data_sub_info */

/*===========================================================================
FUNCTION: estk_is_keep_channel_open_set

DESCRIPTION :
  This function determines if keep channel open bit is set for the last BIP channel
...
RETURN VALUE:
  TRUE - keep channel open is set
  FALSE- Keep channel open is not set
...
SIDE EFFECTS:
  None
===========================================================================*/
boolean estk_is_keep_channel_open_set(
gstk_slot_id_enum_type slot_id)
{
  uint32 index = 0;
  estk_priv_info_type *estk_ptr = NULL;
  ...
  estk_ptr = estk_instances_ptr[slot_id- 1];
  index = estk_ptr->ch_open_id;

  if (index < ESTK_MAX_BIP_CH &&
      estk_ptr->keep_ch_open &&
      estk_ptr->bip_channel_table[index].keep_ch_open_timer_p != NULL)
  {
...
} /* estk_check_if_keep_ch_open_is_set */
#endif /* FEATURE_ESTK */
