...
$Header: //components/rel/uim.mpss/6.1.0/estk/src/estk_priv.h#5 $
...
08/24/17   gs      Populate the DS Sub Id based on the slot
...
06/16/15   bd      Changes to support concatenated SMSs 
...
/*===========================================================================
  ...
  PARAMETERS:
      ...
      transport_protocol      - Packet Data Protocol
      nw_type                 - Network type
      first_dss_sendto_done   - Indicates First send to is success in case of UDP
      receive_data            - Indicates if the Receive Data is received
      port_number             - Port number received from card
      me_port_num             - Ephemeral port number from DS
      buffer_size             - Buffer Size
      open_ch_param_modified  - Used to populate the appropriate TR cause
      on_demand_link          - Handle On demand Link Establishment
      index                   - Stores the Channel Table index
      bearer_desc             - Bearer Parameters
      recv_buffer             - Receive Data Buffer
      send_buffer             - Send Data Buffer
      send_buf_len            - Send Data Buffer Length
      recv_buf_len            - Receive Data Buffer Length
      ...
      sock_nethandle          - Socket Nethandle
      iface_id                - Used in IP Reg Filter
      ds_netcb_status_v4      - DS V4 Net Status
      ds_netcb_status_v6      - DS V6 Net Status
      sock_nethandle_v4       - V4 Socket Handle
      sock_nethandle_v6       - V6 Socket Handle
      policy_info             - DSS Net Policy Information
      profile_3gpp            - 3GPP Profile
      profile_3gpp2           - 3GPP2 Profile
      keep_ch_open_timer_p    - Open Channel Timer
      send_data_timer_expired - Send Data Timer
      card_has_auth_params    - Identifies whether card sent Auth Params
      open_ch_cmd             - Stores GSTK Open Channel Request
      current_command_id      - Current BIP Command
      is_scws_ch              - True if request is SCWS, FALSE otherwise
      scws_overflow_buf       - Stores the SCWS Excess Data received
      scws_overflow_buf_len   - Contains the length of Excess Data Received
      receive_data_allowed    - Used in case of SCWS, to reject the receive data command
      ds_sub_id               - Contains the DS Sub ID corresponding to Slot 
...
typedef struct
{
  ...
  ds_profile_subs_etype                     ds_sub_id;
}estk_channel_type;
...
/*===========================================================================
  ...
  PARAMETERS:
    ...
    msg_trans_type         : WMS Transport infomation
    last_trans_type        : Last Transport type on which SMS is sent to WMS
...
typedef struct
{
  ...
  rex_timer_type                     geo_loc_rsp_timer;    
  uint8                              pref_nmea_types;
#endif  
  boolean                            keep_pdn_conn;
  sint15                             close_nethandle_v4;
  sint15                             close_nethandle_v6;  
  wms_msg_transport_type_e_type      trans_type;      /* Contains the transport type received from WMS */
  wms_msg_transport_type_e_type      last_trans_type; /* Stores the transport type on which last SMS sent to WMS */
  boolean                            estk_geo_loc_engine_lock_status;
  boolean                            is_3gpp2_sms;
}
estk_priv_info_type;
...
typedef struct
{
  ...
  estk_wms_msg_transport_type            wms_transport_type;
  boolean                                trans_changed;
  wms_msg_transport_nw_reg_status_e_type transport_reg_ims_status;
}estk_sms_info;
...
typedef struct
{
  ...
  uint8                       max_active_data;          /* Indicates the maximum allowed data sessions */
  estk_sms_info               wms_info[GSTK_MAX_NUM_OF_INSTANCES];
  rex_crit_sect_type          estk_crit_sect;
}
