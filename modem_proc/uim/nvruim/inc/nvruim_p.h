...
$Header: //components/rel/uim.mpss/6.1.0/nvruim/inc/nvruim_p.h#2 $ $DateTime: 2019/10/30 06:56:05 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/03/16   bcho    Queue NVRUIM read requests to high priority command queue
...
/* Number of 1X sessions supported */
#define NVRUIM_NUM_1X_SESSION_MAX              3

/* MAX number of Slots supported */
#define NVRUIM_MAX_CARD_COUNT  UIM_MAX_NUM_SLOTS

/* Name of NVRUIM client (to be notified to MMGSDI when doing a client id reg*/
#define NVRUIM_CLIENT_NAME              "nvruim"
...
/* ----------------------------------------------------------------------------
   ...
   ENUM:      NVRUIM_NV_CONTEXT_TYPE

   DESCRIPTION:
     Contains information regarding the nv context type
     NVRUIM_NV_CONTEXT_DEVICE_SPECIFIC is used explicitly for Device or ME
     specific NV/EFS Items
-------------------------------------------------------------------------------*/
typedef enum {
  NVRUIM_NV_CONTEXT_PRIMARY         = 0,
  NVRUIM_NV_CONTEXT_SECONDARY,
  NVRUIM_NV_CONTEXT_TERTIARY,
  NVRUIM_NV_CONTEXT_INVALID         = 0x7f
} nvruim_nv_context_type;

/* state of CDMA directory in card */
typedef enum {
  NVRUIM_CDMA_SUBSCRIPTION_NOT_PRESENT    = 0,
  NVRUIM_CDMA_SUBSCRIPTION_PRESENT,
  NVRUIM_CDMA_SUBSCRIPTION_READY
} nvruim_cdma_subscription_type;


extern nvruim_cdma_subscription_type nvruim_dir_present[NVRUIM_MAX_CARD_COUNT];
