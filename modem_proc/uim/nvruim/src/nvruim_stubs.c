...
$Header: //components/rel/uim.mpss/6.1.0/nvruim/src/nvruim_stubs.c#1 $ $DateTime: 2019/10/30 06:56:05 $ $Author: pwbldsvc $
...
04/20/16   ar      De-reference nv_cmd_ext_type rather than nv_cmd_type
04/20/16   ssr     Fix compilation error
...
)
{
  ...
  if (nv_cmd_ext_ptr->nvcmd->tcb_ptr  && nv_cmd_ext_ptr->nvcmd->done_q_ptr)
  {
    /* Put the data on done q provided by async client */
    (void)q_link(nv_cmd_ext_ptr->nvcmd, &nv_cmd_ext_ptr->nvcmd->link);
     q_put(nv_cmd_ext_ptr->nvcmd->done_q_ptr, &nv_cmd_ext_ptr->nvcmd->link);
  }

  if (nv_cmd_ext_ptr->nvcmd->tcb_ptr) 
  {
    /* Let the client know the NV command/request completed */
    (void) rex_set_sigs(nv_cmd_ext_ptr->nvcmd->tcb_ptr, nv_cmd_ext_ptr->nvcmd->sigs);
  }
  return NV_DONE_S;
...
DESCRIPTION
  This function is called by MMODE to switch to the incoming 1X session for
