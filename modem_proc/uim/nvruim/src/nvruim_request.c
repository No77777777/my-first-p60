...
04/04/16   ar      CDMA/HDR support on 2nd SUB
...
)
{
  ...
  /* If NVRUIM task is not ready then return failure */
  ...
  {
    if (nvruim_read(nv_cmd_ptr, context, &nv_status) == NV_RUIM_ITEM_NOT_SUPPORTED)
    {
  ...
  {
    if (nvruim_write(nv_cmd_ptr, context, &nv_status) == NV_RUIM_ITEM_NOT_SUPPORTED)
    {
      nv_status = NV_RUIM_NOT_SUPPORTED_S;
    }
  }
...
)
{
  ...
  if(cmd_ptr == NULL || NVRUIM_CONTEXT_NOT_VALID(nvruim_context))
  {
  ...
  do
  {
    ...
    nvruim_generic_data_ptr->rtre_config_init[nvruim_context] = TRUE;

    UIM_MSG_HIGH_3("nvruim_process_rtre_configuration for context: 0x%x cmd_ptr : 0x%x  for RTRE 0x%x",
                   nvruim_context, cmd_ptr, cmd_ptr->data_ptr->rtre_config);
    ...
    switch(cmd_ptr->data_ptr->rtre_config)
    {
      ...
      case NV_RTRE_CONFIG_RUIM_ONLY:
        ...
        nvruim_generic_data_ptr->rtre_control_value[nvruim_context] =
          NV_RTRE_CONTROL_USE_RUIM;
        nvruim_generic_data_ptr->rtre_polling_control_value[nvruim_context] =
      ...
      case NV_RTRE_CONFIG_RUIM_OR_DROP_BACK:
        ...
        if(nvruim_dir_present[nvruim_context] == NVRUIM_CDMA_SUBSCRIPTION_PRESENT ||
           nvruim_dir_present[nvruim_context] == NVRUIM_CDMA_SUBSCRIPTION_READY)
        {
          /* Set the control to use the R-UIM */
          nvruim_generic_data_ptr->rtre_control_value[nvruim_context] =
            NV_RTRE_CONTROL_USE_RUIM;
        ...
        UIM_MSG_HIGH_1("NVRUIM CDMA subscription state 0x%x",
                       nvruim_dir_present[nvruim_context]);
    ...
    if((nvruim_generic_data_ptr->rtre_control_value[nvruim_context] == NV_RTRE_CONTROL_USE_RUIM) &&
       (NV_RUIM_ZERO_ESN == nvruim_get_esn_usage(nvruim_map_context_to_nvruim_session(nvruim_context))))
    {
      nvruim_set_esn_usage(NV_RUIM_USE_ESN, nvruim_map_context_to_nvruim_session(nvruim_context));
    }

    /* Indicate the command has been successfully completed */
    if(nv_rtre_ctrl != NULL)
    {
      *nv_rtre_ctrl = nvruim_generic_data_ptr->rtre_control_value[nvruim_context];
    }
    if(nv_rtre_polling_ctrl != NULL)
    {
      *nv_rtre_polling_ctrl = nvruim_generic_data_ptr->rtre_polling_control_value[nvruim_context];
    }
  }
  while(0);
...
{
#ifdef FEATURE_UIM_RUN_TIME_ENABLE
  if(nvruim_generic_data_ptr != NULL && !NVRUIM_CONTEXT_NOT_VALID(context))
  {
    if(nvruim_generic_data_ptr->rtre_config_init[context] == FALSE)
    {
...
)
{
  ...
  if(nvruim_generic_data_ptr != NULL)
  {
    return nvruim_generic_data_ptr->rtre_control_value[nvruim_map_nvruim_session_to_context(nvruim_session)];
  }
#endif /* FEATURE_UIM_RUN_TIME_ENABLE */
...
FUNCTION NVRUIM_GET_NVRUIM_CONTEXT

DESCRIPTION
  This function converts uint16 value to nvruim_nv_context_type

RETURN VALUE
  nvruim_nv_context_type

DEPENDENCIES/SIDE EFFECTS/NOTES
  None

===========================================================================*/
static nvruim_nv_context_type nvruim_get_nvruim_context(
  uint16       context
)
{
  switch(context)
  {
    ...
    default:
      return NVRUIM_NV_CONTEXT_INVALID;
  }
} /* nvruim_get_nvruim_context */
...
static boolean nvruim_is_nv_item_supported (
  const nv_cmd_ext_type       *nv_cmd_ext_ptr
)
{
  ...
  if ((nv_cmd_ext_ptr == NULL) ||
      (nv_cmd_ext_ptr->nvcmd == NULL) ||
      (nv_cmd_ext_ptr->nvcmd->data_ptr == NULL))
  {
    return FALSE;
  }

  context        = nvruim_get_nvruim_context(nv_cmd_ext_ptr->context);
  nvruim_session = nvruim_map_context_to_nvruim_session(context);
  as_id          = nvruim_convert_context_to_as_id(context);
  if (nvruim_get_rtre_control_value(nvruim_session) == NV_RTRE_CONTROL_NO_RUIM)
  {
    return FALSE;
  }

  switch (nv_cmd_ext_ptr->nvcmd->item)
  {
    case NV_ESN_CHKSUM_I:
    case NV_ESN_I:
      if(nv_cmd_ext_ptr->nvcmd->cmd == NV_WRITE_F)
      {
        item_supported = FALSE;
      }
      else if(nv_cmd_ext_ptr->nvcmd->cmd == NV_READ_F)
      {
        nvruim_esn_usage_type esn_usage;

        esn_usage = nvruim_get_esn_usage(nvruim_session);
    ...
    case NV_MEID_I:
      ...
      if ((nv_cmd_ext_ptr->nvcmd->cmd == NV_READ_F) &&
          (nvruim_get_esn_usage(nvruim_session) != NV_RUIM_USE_SF_EUIMID &&
           nvruim_get_esn_usage(nvruim_session) != NV_RUIM_USE_SF_EUIMID_ESN))
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_PREF_MODE_I:
      if (nvruim_is_mmgsdi_item_enabled(MMGSDI_NV_PREF_MODE_NOT_FROM_RUIM_I,
                                        context) ||
          nv_cmd_ext_ptr->nvcmd->data_ptr->pref_mode.nam != NV_RUIM_NAM)
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_IMSI_ADDR_NUM_I:
      if((nv_cmd_ext_ptr->nvcmd->cmd == NV_READ_F) &&
         (nv_cmd_ext_ptr->nvcmd->data_ptr->min1.nam != NV_RUIM_NAM))
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_IMSI_T_ADDR_NUM_I:
      if((nv_cmd_ext_ptr->nvcmd->cmd == NV_READ_F) &&
         (nv_cmd_ext_ptr->nvcmd->data_ptr->imsi_t_s1.nam != NV_RUIM_NAM))
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_ACCOLC_I:
      if((nv_cmd_ext_ptr->nvcmd->cmd == NV_READ_F) &&
         (nv_cmd_ext_ptr->nvcmd->data_ptr->accolc.nam != NV_RUIM_NAM))
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_ROAMING_LIST_683_I:
      if((nv_cmd_ext_ptr->nvcmd->cmd == NV_READ_F) &&
         (nv_cmd_ext_ptr->nvcmd->data_ptr->roaming_list.nam != NV_RUIM_NAM))
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_DIR_NUMBER_I:
      if(nv_cmd_ext_ptr->nvcmd->data_ptr->dir_number.nam != NV_RUIM_NAM)
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_DIR_NUMBER_PCS_I:
      if((nv_cmd_ext_ptr->nvcmd->cmd == NV_WRITE_F) &&
         (nv_cmd_ext_ptr->nvcmd->data_ptr->mob_dir_number.nam != NV_RUIM_NAM))
      {
        item_supported = FALSE;
      }
      if((nv_cmd_ext_ptr->nvcmd->cmd == NV_READ_F) &&
         (nv_cmd_ext_ptr->nvcmd->data_ptr->dir_number.nam != NV_RUIM_NAM))
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_SID_NID_I:
      if((nv_cmd_ext_ptr->nvcmd->cmd == NV_WRITE_F) &&
         (nv_cmd_ext_ptr->nvcmd->data_ptr->sid_nid.nam != NV_RUIM_NAM))
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_HOME_SID_NID_I:
      if(nv_cmd_ext_ptr->nvcmd->data_ptr->home_sid_nid.nam != NV_RUIM_NAM)
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_ANALOG_HOME_SID_I:
      if(nv_cmd_ext_ptr->nvcmd->data_ptr->analog_home_sid.nam != NV_RUIM_NAM)
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_MOB_TERM_HOME_I:
      if(nv_cmd_ext_ptr->nvcmd->data_ptr->mob_term_home.nam != NV_RUIM_NAM)
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_MOB_TERM_FOR_SID_I:
      if(nv_cmd_ext_ptr->nvcmd->data_ptr->mob_term_for_sid.nam != NV_RUIM_NAM)
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_MOB_TERM_FOR_NID_I:
      if(nv_cmd_ext_ptr->nvcmd->data_ptr->mob_term_for_nid.nam != NV_RUIM_NAM)
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_ANALOG_FIRSTCHP_I:
      if(nv_cmd_ext_ptr->nvcmd->data_ptr->analog_firstchp.nam != NV_RUIM_NAM)
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_OTAPA_ENABLED_I:
      if(nv_cmd_ext_ptr->nvcmd->data_ptr->otapa_enabled.nam != NV_RUIM_NAM)
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_NAM_LOCK_I:
      if(nv_cmd_ext_ptr->nvcmd->data_ptr->nam_lock.nam != NV_RUIM_NAM)
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_ASSIGNING_TMSI_ZONE_LEN_I:
      if(nv_cmd_ext_ptr->nvcmd->data_ptr->assigning_tmsi_zone_length.nam != NV_RUIM_NAM)
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_ASSIGNING_TMSI_ZONE_I:
      if((nv_cmd_ext_ptr->nvcmd->cmd == NV_WRITE_F) &&
         (nv_cmd_ext_ptr->nvcmd->data_ptr->assigning_tmsi_zone.nam != NV_RUIM_NAM))
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_TMSI_CODE_I:
      if((nv_cmd_ext_ptr->nvcmd->cmd == NV_WRITE_F) &&
         (nv_cmd_ext_ptr->nvcmd->data_ptr->tmsi_code.nam != NV_RUIM_NAM))
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_TMSI_EXP_I:
      if((nv_cmd_ext_ptr->nvcmd->cmd == NV_WRITE_F) &&
         (nv_cmd_ext_ptr->nvcmd->data_ptr->tmsi_exp_timer.nam != NV_RUIM_NAM))
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_SMS_BC_SERVICE_TABLE_I:
      if (nvruim_is_bcsms_svc_avail(context) == FALSE)
      {
    ...
    case NV_SMS_SERVICE_OPTION_I:
      ...
      if ((nv_cmd_ext_ptr->nvcmd->cmd == NV_READ_F) &&
          (nvruim_is_smscap_svc_avail(context) == FALSE))
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_SMS_BC_CONFIG_I:
      ...
      if ((nv_cmd_ext_ptr->nvcmd->cmd == NV_READ_F) &&
          (nvruim_is_bcsms_svc_avail(context) == FALSE))
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_CGPS_1X_MPC_SERVER_ADDR_URL_I:
      if (nvruim_is_lbs_support_avail(context) == FALSE)
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_DS_QCMIP_I:
      if(uim_3gpd_support_per_sub(as_id) == NVRUIM_3GPD_MIP_NV_SIP_NV)
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_DS_SIP_PPP_SS_INFO_I:
      if((uim_3gpd_support_per_sub(as_id) == NVRUIM_3GPD_MIP_NV_SIP_NV) ||
         (uim_3gpd_support_per_sub(as_id) == NVRUIM_3GPD_MIP_RUIM_SIP_NV))
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_DS_SIP_ACTIVE_PROFILE_INDEX_I:
      if((uim_3gpd_support_per_sub(as_id) == NVRUIM_3GPD_MIP_RUIM_SIP_NV) ||
         (uim_3gpd_support_per_sub(as_id) == NVRUIM_3GPD_MIP_NV_SIP_NV))
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_DSAT707_CTA_TIMER_I:
      if ((nvruim_data_3gpd_ext_support_per_sub(as_id) == FALSE) ||
          (uim_3gpd_support_per_sub(as_id) == NVRUIM_3GPD_MIP_NV_SIP_NV))
      {
        item_supported = FALSE;
      }
    ...
    case NV_DS_MIP_RRQ_IF_TFRK_I:
      if ((nvruim_data_3gpd_ext_support_per_sub(as_id) == FALSE) ||
          (uim_3gpd_support_per_sub(as_id) == NVRUIM_3GPD_MIP_NV_SIP_NV) ||
          (uim_3gpd_support_per_sub(as_id) == NVRUIM_3GPD_MIP_NV_SIP_RUIM))
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_TCP_KEEPALIVE_IDLE_TIME_I:
      ...
      else if((uim_3gpd_support_per_sub(as_id) == NVRUIM_3GPD_MIP_NV_SIP_RUIM) &&
              (nv_cmd_ext_ptr->nvcmd->cmd == NV_READ_F))
      {
    ...
    case NV_HDR_AN_AUTH_NAI_I:
      if (uim_an_hrpd_support_per_sub(as_id) == NVRUIM_AN_HRPD_NO_SUPPORT)
      {
        item_supported = FALSE;
      }
      break;
    ...
    case NV_HDR_AN_AUTH_PASSWORD_I:
      if ((nv_cmd_ext_ptr->nvcmd->cmd == NV_WRITE_F) &&
          (uim_an_hrpd_support_per_sub(as_id) == NVRUIM_AN_HRPD_NO_SUPPORT))
      {
        item_supported = FALSE;
      }
      if (nv_cmd_ext_ptr->nvcmd->cmd == NV_READ_F)
...
{
#ifdef FEATURE_UIM_RUN_TIME_ENABLE
  return nvruim_generic_data_ptr->rtre_polling_control_value[nvruim_map_nvruim_session_to_context(nvruim_session)];
#else
...
)
{
  ...
  if ((nvruim_generic_data_ptr != NULL) && 
      (nvruim_get_rtre_control_value(nvruim_session) != NV_RTRE_CONTROL_USE_RUIM))
  {
...
)
{
  ...
  nv_cmd_ext_type              *nv_cmd_ext_ptr                     = (nv_cmd_ext_type *)in_ptr;
  ...
  if (nv_cmd_ext_ptr == NULL || nv_cmd_ext_ptr->nvcmd == NULL)
  {
    return NV_BADPARM_S;
  }

  if (nv_cmd_ext_ptr->nvcmd->cmd != NV_RTRE_OP_CONFIG_F)
  {
    UIM_MSG_ERR_0("NV_RUIM_CMD_PROCESS_RTRE_CONFIG_IOCTL:Invalid request type");
    nv_cmd_ext_ptr->nvcmd->status = NV_RUIM_NOT_SUPPORTED_S;
    return NV_RUIM_NOT_SUPPORTED_S;
  }

  if (nv_cmd_ext_ptr->nvcmd->sigs == 0)
  {
    UIM_MSG_ERR_0("NV_RUIM_CMD_PROCESS_RTRE_CONFIG_IOCTL:No sig provided");
    nv_cmd_ext_ptr->nvcmd->status = NV_BADPARM_S;
    return NV_BADPARM_S;
  }

  if (nv_cmd_ext_ptr->nvcmd->tcb_ptr)
  {
    ...
    nvruim_req_ptr->nvruim_req                       = NVRUIM_NV_ACCESS;
    nvruim_req_ptr->cmd.nvruim_access_req.nv_cmd_ptr = nv_cmd_ext_ptr->nvcmd;
    nvruim_req_ptr->cmd.nvruim_access_req.context    = nvruim_get_nvruim_context(nv_cmd_ext_ptr->context);
  ...
  else
  {
    ...
    nv_op_status = nvruim_process_rtre_configuration(nv_cmd_ext_ptr->nvcmd,
                                                     &dummy_rtre_control_value,
                                                     &dummy_rtre_polling_control_value,
                                                     nvruim_get_nvruim_context(nv_cmd_ext_ptr->context));
  }
...
)
{
  ...
  context = *(uint16 *)in_ptr;
  nvruim_session = nvruim_map_context_to_nvruim_session(nvruim_get_nvruim_context(context));

  *((uint16 *)out_ptr) = nvruim_get_nv_max_roaming_list_size(nvruim_session);
  return NV_DONE_S;
...
)
{
  ...
  nvruim_session_enum_type   nvruim_session      = NVRUIM_1X_PROV_PRI_SESSION;
  uint16                     context             = 0;
  nv_nam_ext_type            nvruim_nam_info     = {0};
  ...
  switch (type)
  {
    case NV_RUIM_CMD_GET_RTRE_CONTROL_VALUE_IOCTL:
      ...
      *((nv_rtre_control_type *)out_ptr) = nvruim_get_rtre_control_value(nvruim_session);
      break;
    ...
    case NV_RUIM_CMD_GET_RTRE_POLLING_CONTROL_VALUE_IOCTL:
      ...
      *((nv_rtre_polling_control_type *)out_ptr) =
        nvruim_get_rtre_polling_control_value(nvruim_session);
      break;
    ...
    case NV_RUIM_CMD_RTRE_NAM_IOCTL:
      ...
      nvruim_nam_info = *(nv_nam_ext_type *)in_ptr;
      nvruim_session = nvruim_map_context_to_nvruim_session(nvruim_get_nvruim_context(nvruim_nam_info.context));

      if ((nvruim_get_rtre_control_value(nvruim_session) == NV_RTRE_CONTROL_USE_RUIM) &&
          nvruim_nam_info.nam == NV_RUIM_NAM)
      {
    ...
    case NV_RUIM_CMD_RTRE_POLLING_NAM_IOCTL:
      ...
      if ((nvruim_get_rtre_polling_control_value(nvruim_session) == NV_RTRE_POLLING_CONTROL_POLL) &&
          nvruim_nam_info.nam == NV_RUIM_NAM)
      {
        *((boolean *)out_ptr) = TRUE;
      }
...
)
{
  ...
  if (nv_cmd_ext_ptr == NULL || nvruim_generic_data_ptr == NULL || nv_cmd_ext_ptr->nvcmd == NULL)
  {
  ...
  if (nv_cmd_ext_ptr->nvcmd->cmd != NV_READ_F &&
      nv_cmd_ext_ptr->nvcmd->cmd != NV_WRITE_F)
  {
    UIM_MSG_ERR_0("NV_RUIM_CMD_VERIFY_AND_ACCESS_IOCTL:Invalid request type");
    return NV_RUIM_NOT_SUPPORTED_S;
  }

  if (nvruim_is_nv_item_supported(nv_cmd_ext_ptr) == FALSE)
  {
    return NV_RUIM_NOT_SUPPORTED_S;
  }
  ...
  if (nv_cmd_ext_ptr->nvcmd->tcb_ptr)
  {
    ...
    nv_operation_status = nvruim_cmd(nvruim_req_ptr);
  }
  else
  {
    /* Client has made a sync NV access. Process in client context */
    nv_operation_status =
      nvruim_process_nv_access(nv_cmd_ext_ptr->nvcmd, nvruim_get_nvruim_context(nv_cmd_ext_ptr->context));
  }
...
)
{
  ...
  switch (type)
  {
    ...
    case NV_RUIM_CMD_ROAMING_LIST_MAX_SIZE_IOCTL:
      nv_operation_status =
        nvruim_process_roaming_list_size_nv_req(in_ptr, out_ptr);
      break;
