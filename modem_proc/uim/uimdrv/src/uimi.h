...
Copyright (c) 2000 - 2018, 2021, 2024 QUALCOMM Technologies, Inc (QTI) and its licensors. All Rights Reserved.
...
$Header: //components/rel/uim.mpss/6.1.0/uimdrv/src/uimi.h#7 $
...
01/19/16   gm      Changed the default transcation timer value to 30 sec during power up
...
06/15/16   gm      Stub out GPIO Info logging
...
11/02/15   sam     Added aram-app failure mask to workaround NV
...
#define UIM_RETURN_IF_NULL_PTR(p_var)                                      \
  if ((p_var) ==  NULL)                                                    \
  {                                                                        \
    UIM_MSG_ERR_0("Invalid input pointer");                                \
    return;                                                                \
  }

#define UIM_RETURN_IF_NULL_PTR_2(p_var1, p_var2)                          \
...
#define UIM_DELAY_BEFORE_CLOCK_STOP                             10
#define UIM_DELAY_BEFORE_CLOCK_STOP_MAX_LIMIT                   30
#define UIM_SLEEP_TIMER_INCREMENT                                5
#define UIM_MIN_RECOVERY_COUNT_FOR_DELAY                         3

/* Delay after clock start */
#define UIM_DELAY_AFTER_CLOCK_RESTART                            5

/* Disable poll during RF default*/
#define UIM_DISABLE_POLL_RF_OFF_DEFAULT                          1
...
#define UIM_EXTENDED_RECOVERY_NV_CURRENT_VERSION                  0x01

#ifdef FEATURE_SUPPORTS_NON_REMOVABLE_UICC
#error code not present
#endif

/* Masks corresponding to each work around */
#define UIM_MF_SELECTION_UPON_TWO_CONSECUTIVE_TECH_PROBLEMS                   0x00000001
#define UIM_MF_SELECTION_AFTER_STORE_ESN_ME_COMMAND                           0x00000002
#define UIM_MF_SELECTION_AFTER_LOCI_UPDATE_COMMAND                            0x00000004
#define UIM_MF_SELECTION_AFTER_VERIFY_PIN_COMMAND                             0x00000008
#define UIM_MF_SELECTION_AFTER_LOCATION_STATUS_ENVELOPE_COMMAND               0x00000010
#define UIM_MF_SELECTION_AFTER_TERMINAL_RESPONSE_COMMAND                      0x00000020
#define UIM_MF_SELECTION_AFTER_AID_SELECTION_FAILURE_ON_NON_ZERO_CHANNEL      0x00000040
#define UIM_MF_SELECTION_AFTER_EF_PSLOCI_UPDATE_COMMAND                       0x00000080
#define UIM_ADF_SELECTION_ON_OTHER_CHANNELS_UPON_ARAM_APP_FAILURE             0x00000100
#define UIM_MF_SELECTION_AFTER_CALL_CONNECT_ENVELOPE_COMMAND                  0x00000200
#define UIM_MF_SELECTION_AFTER_ENVELOPE_COMMAND                               0x00000400
#define UIM_MF_SELECTION_AFTER_CALL_CONTROL_ENVELOPE_COMMAND                  0x00000800
#define UIM_IGNORE_POLL_ERROR_UNDER_7F10_DIRECTORY                            0x00001000
#define UIM_IMSI_SELECTION_BEFORE_AUTH_COMMAND                                0x00004000
#define UIM_CONTINUE_RECOVERY_WITHOUT_TP                                      0x00008000
#define UIM_PROVIDE_GET_RSP_SW_FOR_SELECT_COMMAND                             0x00010000
/* 18th Bit is not used currently. Deleted with CR-2711775 */
#define UIM_IGNORE_POLL_ERROR                                                 0x00040000
#define UIM_IGNORE_VOLTAGE_CLASS_ERROR_IN_MF_SELECTION                        0x00080000
#define UIM_IGNORE_TECH_PROBLEM_ERROR_FOR_FETCH_COMMAND                       0x00100000

/* Default value of uim_busy_ind_timer in power up cases */
#define UIM_TRANSACTION_TIMER_VAL_DURING_POWER_UP_DEFAULT           30000
...
}uim_slot_mapping_preference_table_efs_data_type;
...
typedef PACKED struct PACKED_POST {
  ...
  uint32 uim_power_up_trans_timer_val_ms;
  /* version 1: RESERVED_FOR_FUTURE_USE size is 32 - 13 = 19 */
  uint8    reserved_for_future_use[19];
...
typedef PACKED struct PACKED_POST {
  ...
  boolean     extended_powerup_attempts_upon_card_removal;
  boolean     extended_powerup_attempts_from_boot_up;
  uint16      no_of_extended_power_up_attempts;
  /* version 1: RESERVED_FOR_FUTURE_USE size is 13 - 5 = 8 */
  uint16      reserved_for_future_use[8];
...
/* Strucure to store slot to be powered down returned by SIMLOCK */
typedef struct{
  rex_tcb_type  *tcb_ptr;
  boolean        slot_to_be_powered_down[UIM_MAX_INSTANCES];
}uim_simlock_user_type;
...
/* Structure to store NV for UIM boot-up APDU logs configuration */
typedef PACKED struct PACKED_POST{
  uint8         cache_mechanism;
  uint16        num_boot_up_apdu_log;
}uim_boot_up_apdu_log_configuration;
...
typedef enum {
  ...
  UIM_BUSY_RESPONSE_EVT_CLOCK_OFF
...
typedef struct {
  boolean  only_class_b_supported;
  boolean  only_class_c_supported; 
}uim_uicc_voltage_class_type;

typedef struct {
  boolean                     uim_atr_ta2_byte_present;
  boolean                     uim_atr_b5_bit_ta2;
  boolean                     uim_atr_b8_bit_ta2;
  uim_transport_protocol_type protocol;
}uim_global_specific_mode_type;
...
FUNCTION UIMDRV_ISO_SEND_APDU                          INTERNAL FUNCTION
...
boolean uim_can_ignore_timeout(rex_sigs_type sigs, uim_instance_global_type *uim_ptr);
