...
$Header: //components/rel/uim.mpss/6.1.0/uimdrv/src/uim_nvcfg.h#3 $
...
12/28/17    ku         Feature Guarding GET RESPONSE for status command
10/30/17    ku         Trigger recovery after bad SW for initial SELECT MF
04/13/17    ks         Add NV changes to support internal non removable UICC
...
12/05/15    ll         Change default SIMULATE_NULL_INS to 0xFE 
11/04/15    ll         Change default SIMULATE_NULL_INS to 0x99
03/25/15    hyo        Disable polling when RF is inactive
...
#define UIM_RFU_INDEX_DISABLE_POLL_RF_OFF                       10 /* RFU[10]-Disable polling when RF is off */

#ifdef FEATURE_SUPPORTS_NON_REMOVABLE_UICC
#error code not present
#endif /* FEATURE_SUPPORTS_NON_REMOVABLE_UICC */
...
#define UIM_FEATURES_STATUS_LIST_CURRENT_COUNT                  34
/* Default value of uim_trans_timer in case the EFS item is not set */
#define UIM_SIMULATE_NULL_INS_DEFAULT                           0xFE
...
typedef enum {
  ...
  UIMDRV_FEATURE_RECOVERY_ON_BAD_STATUS_WORD_ON_MF_SELECT,
  UIMDRV_FEATURE_ALLOW_GET_RESPONSE, 
  UIMDRV_FEATURE_READ_NUM_OF_VALID_APPS
...
typedef struct {
  ...
  /* Read number of Valid Apps */
  boolean  read_num_of_valid_app;
  /* Recovery in case of bad status word for select MF */
  boolean  recovery_on_bad_status_word_on_mf_select;
  /* Allow get response or not */
  boolean allow_get_response;
  /* version 9: RESERVED_FOR_FUTURE_USE size is 127 - 33 = 94 */
