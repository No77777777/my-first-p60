...
$Header: //components/rel/uim.mpss/6.1.0/uimdrv/src/hw/pmic/uimdrv_ldo_config.c#1 $
$DateTime: 2019/10/30 06:56:05 $
$Author: pwbldsvc $

when        who        what, where, why
------      ----       -----------------------------------------------------------
07/16/16    sam        Intial revision
============================================================================*/
...
void uimdrv_pmic_efs_get_user_ldo_config(uim_instance_enum_type uim_instance)
{
  ...
  uim_ldo_settings_efs_type   uim_ldo_settings_efs;
  ...
  memset(&uim_ldo_settings_efs, 0x00, sizeof(uim_ldo_settings_efs_type));
  ...
  if (UIM_COMMON_EFS_SUCCESS == uim_common_efs_read(UIM_COMMON_EFS_UIMDRV_LDO_SETTINGS,
                                                    ...
                                                    (uint8 *)&(uim_ldo_settings_efs),
                                                    sizeof(uim_ldo_settings_efs)))
  {
    /* copy what's read in from EFS to UIMDRV struct */
    uim_ptr->hardware.uim_ldo.customize_ldo_setting =
                                  uim_ldo_settings_efs.customize_ldo_setting;
    uim_ptr->hardware.uim_ldo.desired_voltage_class = (uim_voltage_class_type)
                                  uim_ldo_settings_efs.desired_voltage_class;
    uim_ptr->hardware.uim_ldo.desired_voltage = (uim_pmic_voltages_avail_type)
                                  uim_ldo_settings_efs.desired_voltage;
    uim_ptr->hardware.uim_ldo.pdown_allowed_upon_card_removal = 
                                  uim_ldo_settings_efs.pdown_allowed_upon_card_removal;
    uim_ptr->hardware.uim_ldo.pdown_allowed_upon_error = 
                                  uim_ldo_settings_efs.pdown_allowed_upon_error;

  }
  else
  {
#ifdef FEATURE_CUSTOMIZE_SLOT2_LDO_SETTINGS
    if (pmic_ref == E_PMIC_NPA_RESOURCE_UIM2)
    {
      uim_ptr->hardware.uim_ldo.customize_ldo_setting                 = TRUE;
      ...
      uim_ptr->hardware.uim_ldo.pdown_allowed_upon_card_removal       = TRUE;
    ...
    uim_ldo_settings_efs.customize_ldo_setting =
                        uim_ptr->hardware.uim_ldo.customize_ldo_setting;
    uim_ldo_settings_efs.desired_voltage_class =
                        (uint8)uim_ptr->hardware.uim_ldo.desired_voltage_class;
    uim_ldo_settings_efs.desired_voltage = 
                        (uint8) uim_ptr->hardware.uim_ldo.desired_voltage;
    uim_ldo_settings_efs.pdown_allowed_upon_card_removal =
                        uim_ptr->hardware.uim_ldo.pdown_allowed_upon_card_removal;
    uim_ldo_settings_efs.pdown_allowed_upon_error =
                        uim_ptr->hardware.uim_ldo.pdown_allowed_upon_error;
    ...
    if (UIM_COMMON_EFS_SUCCESS != uim_common_efs_write(UIM_COMMON_EFS_UIMDRV_LDO_SETTINGS,
                                                       UIM_COMMON_EFS_ITEM_FILE_TYPE,
                                                       (uim_common_efs_context_type)pmic_ref,
                                                       (const char*)&(uim_ldo_settings_efs),
                                                       sizeof(uim_ldo_settings_efs_type)))
    {
                                         ...
                                         npa_resource_state     req)
{
  ...
  UIMDRV_MSG_HIGH_2(uim_ptr->id,"uim_evaluate_req_ldo_voltage_mod - Evaluating req - 0x%x desired_voltage_class - 0x%x",
                    req, uim_ptr->hardware.uim_ldo.desired_voltage_class);
...
)
{
  ...
  if(( UIM_SLOT_3 == physical_slot ) &&
     (( DALCHIPINFO_FAMILY_MSM8909 == uimdrv_hw_nv_config_info.dalChipFamily) ||
      ( DALCHIPINFO_FAMILY_MSM8952 == uimdrv_hw_nv_config_info.dalChipFamily)) )
  {
