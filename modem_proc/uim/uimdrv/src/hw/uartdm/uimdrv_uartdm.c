/*============================================================================
  ...
  DEPENDENCIES: N/A
                ...
                Copyright (c) 2012 - 2018, 2020 QUALCOMM Technologies, Inc(QTI).
                All Rights Reserved.
...
$Header: //components/rel/uim.mpss/6.1.0/uimdrv/src/hw/uartdm/uimdrv_uartdm.c#4 $
...
05/18/17    ks         Disabling RX/TX twice to avoid bus hang
...
05/26/16    gm         F3 message removal
02/09/16    sam        Check if the latest CR write has overwritten any pending 
                       older CR write
...
09/23/15    ll         All CR access use prgCR function for cleaner code design
08/12/15    na         Support of command in progress
07/20/15    ks         Consolidating UIM bring-up changes
07/01/15    na         Corrected condition logic
06/25/15    ll         Clean up ATLAS HW CR feature flags
06/17/15    ll         Atlas HW CR -- UART auto restart WWT upon RX byte
...
#ifndef FEATURE_UIM_HABANERO_HWIO_MACROS
...
void  prgCR(uim_instance_enum_type uim_instance, dword  val)
{
    ...
    do
    {
      ...
      if(MSMU_SR_CMD_IN_PROG == (data & MSMU_SR_CMD_IN_PROG))
      {
        if( count >= UIM_MAX_NUM_SR_CHECK )
        {
          ...
          #error code not present
#else
          if(uim_ptr->debug.sr_check_failed_count == 0)
          {
            uim_ptr->debug.sr_check_failed_count++;
            UIMDRV_MSG_ERR_0(uim_instance,"SR check max count exceeded, trgigering recovery");
            uim_force_recovery(uim_ptr);
          }
          else
          {
            /* When the SR checks exceed for the second time, we ERR FATAL
               We may need to revisit this and remove ERR_FATAL incase 
  ...
  if (UIM_UART_DM_HW_VERSION_1_4_2 <= uim_hw_if.uart[uim_instance].uart_version)
  {
    ...
    if(MSMU_ISR_NO_FINISH_CMD_VIOL == (data & MSMU_ISR_NO_FINISH_CMD_VIOL))
    {
      UIMDRV_MSG_ERR_0(uim_instance, "CR register overwritten");
      uim_ptr->debug.tot_cr_overwrite_count++;
      
      /*clear the NO_FINISH_CMD_VIOL interrupt*/
      UIM_HWIO_OUT(uim_hw_if.uart[uim_instance].start_addr + UART_DM_ISR_CLR, MSMU_ISR_CLR_NO_FINISH_CMD_VIOL_INT);
      uim_log_uartdm_register(uim_instance, UART_DM_ISR_CLR, UIM_REG_WRITE, MSMU_ISR_CLR_NO_FINISH_CMD_VIOL_INT);
    }
  }
  else
  {
...
uint32 uim_bytes_waiting_in_rxfifo(uim_instance_global_type *uim_ptr)
{
  uint32 bytes = 0;
  uint32 num_bytes = 0;
...
uint32 uim_bytes_waiting_in_txfifo(uim_instance_global_type *uim_ptr)
{
  uint32 bytes = 0;
  uint32 num_bytes = 0;
  ...
  bytes = UIM_GET_NUM_BYTES_WAITING_IN_TX_SLOT( uim_ptr );
...
void UIM_SET_UARTDM_ACTIVE_MODE(uim_instance_global_type *uim_ptr)
{
  ...
  if(uim_hw_if.uart[uim_ptr->id].uart_version == 0)
  {
    ...
    UIMDRV_MSG_HIGH_1(uim_ptr->id, "UART HW version: 0x%x", uim_hw_if.uart[uim_ptr->id].uart_version);
...
void UIM_SET_UARTDM_INACTIVE_MODE(uim_instance_global_type *uim_ptr)
{
  ...
  /* Disable the TX and RX twice. 
     This is suggested by UARTDM team to avoid bus hang issues */
...
} /* uim_prg_uart_csr_by_div_factor */
