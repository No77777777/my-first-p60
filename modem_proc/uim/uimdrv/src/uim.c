...
Copyright (c) 2001 - 2021, 2025 QUALCOMM Technologies, Inc (QTI) and its licensors.
All Rights Reserved.
...
$Header: //components/rel/uim.mpss/6.1.0/uimdrv/src/uim.c#9 $
$DateTime: 2025/02/05 03:19:35 $
$Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/03/25   gnr     KW errors fix
09/01/21   ykr     Fix to restart the inactive poll timer
12/22/20   rm      Unmasking of Proactive commands when private logging 
                   is not enabled
03/13/20   rkg     Select IMSI before Authentication for buggy card
...
05/14/19   ykr     Continue PD polling when call is ongoing for rf inactive case
...
07/24/18   ku      Update proper logical slot id while retrieving ATR
04/17/18   ku      Unmask interrupt in case of powerup
03/26/18   ku      KW errors fix
03/12/18   ku      Mask inserted interrupt during power_up
02/01/18   ku      Do not set suscipcious signal in case of extended recovery
01/10/18   ku      Reset status Word of Report buffer to 0
12/21/17   gs      Do not pet the dog after the task power down
11/09/17   ku      Updating the max attempts  of recovery
10/31/17   gm     Set POLL timer when card removal suspicious is TRUE
10/10/17   ku      Adding prototype for uim_check_q_and_stop_clock
09/22/17   ak      Corrected SIM recovery count to 3 from 4
08/01/17   ku      KW errors fix
06/28/17   ks      POLL cmd irrespective of RF status during flaky hotswap
...
05/08/17   ks      Do not POLL in passive mode
05/04/17   vr      Send SW1 SW2 in fetch response from uimdrv to GSTK
05/02/17   ku      UIMDRV API for GET ATR request by other modules
...
01/23/17   gm      Revert 3V workaround for 8998 
01/09/17   ks      Clearing timers upon uim power down
12/27/16   gm      Remove unused function
11/28/16   ks      NV protection for power-up sequence transaction timer
11/24/16   ks      Adding support for spcom interface
11/18/16   ks      Setting the transaction timer to smaller value at card init
11/10/16   ks      Removing unneccesary inclusions.
11/08/16   ks      Setting the busy ind timer to smaller value at card init
10/03/16   dd      Removed lint comments
09/22/16   gm      Added support for set APDU behavior
09/12/16   ku      Removal of MFLO related dead codes
09/01/16   ks      Resetting Proactive Poll Interval flag appropriately
...
05/25/16   na      Removed unused enumeration uim_items_enum_type
05/24/16   ssr     F3 Optimization on curr path
05/27/16   sam     Check for command transacted flag when the internal 
...
04/25/16   hyo     MSM8998 V1 workaround for 3V support
...
03/08/16   ssr     Adding uimgsm,uimcdma stubs for IOE variant
02/11/16   ssr     remove F3 msg
02/08/16   na      Clear powering_down_task after uim_power_down in task pdown
...
12/28/15   ks      Fixing issue with presence detection polling
12/18/15   ssr     Added MCFG API to query the power down reason
12/21/15   na      Do not allow silent recovery im passive mode
...
10/26/15   vv      Use mcs wrappers for rcevt APIs
10/23/15   ks      Triggering extended recovery/new powerup on ICCID mismatch
10/23/15   na      Masking of interrupts before notifying an error of current command
09/25/15   sam     Remove the logic to store timeout info in EFS
09/14/15   sam     Explicit selection of ADF before Compute IP command if the 
...
08/20/15   ll      Notify MMGSDI UIM connection type upon link establishment
...
08/06/15   sam     Allow power_down_error notification when previous card error
                   notification was CARD_REMOVED
07/31/15   hyo     Merging duplicate set_timer in set poll timer
07/20/15   ks      Allowing AUTH command to card after 7FFF failure only if
                   the last succesful selected file is within ADF directory.
07/10/15   sam     remove UIM flushing in TASK_EXIT signal handling
07/10/15   sam     Avoid get response for select generated by fetch check request
07/07/15   hyo     Moving to QTF Dal implementation
07/03/15   ks      Flush the AUTH cmd if internal select with AID data fails
06/24/15   lm      Enable memory leak detection
06/18/15   ks      Consolidating the F3s that print the directory path
06/14/15   ssr     Fix code to add the AID len on status request
...
06/05/15   ssr     Allow UIM to get the DF name in status request when the last
...
05/11/15   ks      Prioritizing explicit MF selection than the POLL command
05/04/15   ks      Appropriate intermediate selection of CDMA DF
05/04/15   sam     Critical section for offtarget 
                   dal_interrupt_controller_register_isr
04/29/15   sam     check if current directory is ADF, for terminate app status cmd
04/23/15   ks      Selecting MF after 'terminate session - slect' on channel0
...
08/04/15   sam     1. adding back logic to select MF after 2 consecutive tech prob errors
                   2. Kw error fix.
...
03/23/15   ks      MF selection upon two consective technical problem errors
...
12/04/14   na      Clearing of response buffer upon receipt of command
...
/* Default value of uim_busy_ind_timer in power up cases */
#define UIM_BUSY_IND_TIMER_VAL_DURING_POWER_UP_DEFAULT              3000

extern rex_crit_sect_type uimdrv_pad_workaround_crit_sect;
/*-------------------------------------------------------------------------*/
/*                                                                         */
/*                               Macros                                    */
/*                                                                         */
...
void uim_register_rcevt_cm_sig
(
  uim_instance_global_type *uim_ptr
);
...
===========================================================================*/
void uim_telecom_process_card_response
(
...
===========================================================================*/
void uim_telecom_process_command
(
...
===========================================================================*/
void uim_telecom_process_cmd_err
(
  rex_sigs_type            *i_mask,   /* Mask of signals interested in */
  uim_instance_global_type *uim_ptr
);
...
{
  rex_sigs_type temp_poll_timer_sig = 0;

  UIM_RETURN_IF_NULL_PTR(uim_ptr);

  if ( (TRUE == uim_ptr->flag.uim_disable_polling || 
        UIM_PASSIVE_MODE == uim_ptr->command.mode) &&
       (FALSE == uim_ptr->hotswap.hotswap_card_removed_suspicious) )
  {
    /* When polling is disabled, do nothing and return */
    /* When modem is in passive mode, do not send POLL command */
    /* When card removal suspicious is set, we need to send POLL command */
    return;
  }
  ...
  else
  {
    ...
    temp_poll_timer_sig = rex_get_sigs(uim_ptr->tcb_ptr) & UIM_POLL_TIMER_SIG;
    ...
    (void) rex_set_timer(&uim_ptr->poll.wake_up_poll_timer, time +
                            wake_up_poll_offset);
    /* Restoring the UIM_POLL_TIMER_SIG so that STATUS COMMAND can be sent */
    (void) rex_set_sigs(uim_ptr->tcb_ptr, temp_poll_timer_sig);
  }
...
)
{
  ...
  uim_ptr->setting.presence_time_delay    = UIM_POLL_TIME;
  if(IS_BTSAP_HANDLE_ACTIVE(uim_ptr))
  {
    uim_ptr->setting.poll_time_delay        = uim_ptr->remote.poll_interval;
    uim_ptr->setting.proactive_time_delay   = uim_ptr->remote.poll_interval;
    uim_ptr->setting.presence_time_delay    = uim_ptr->remote.poll_interval;  
  }  
...
)
{
     ...
     (TRUE == uim_ptr->extended_recovery.extended_recovery_in_progress))
  {
    if(FALSE == uim_ptr->flag.temporary_card_error_notified)
    {
      /*this is the first time we are notifying after all the recoveries exhausted.
        ...
        caching pin data*/
      (void)uim_sm_mmgsdi_notify_card_error( uim_status_param, uim_ptr, TRUE);
      uim_ptr->flag.temporary_card_error_notified = TRUE;
    }
  }
  else
  {
    UIMDRV_MSG_HIGH_1(uim_ptr->id ,"notifying permanent card error uim_status_param : 0x%x", uim_status_param);
    uim_ptr->flag.temporary_card_error_notified = FALSE;
    uim_reset_notify_error_globals(uim_ptr);
    (void)uim_sm_mmgsdi_notify_card_error( uim_status_param, uim_ptr, FALSE);
  }

  /*reset the notify_card_indication flag, for next activation*/
  uim_ptr->flag.card_activation_indicated_to_mmgsdi = FALSE;
  /* Resetting the proactive polling enabled flag */
  ...
  {
    UIMDRV_PUP_MSG_HIGH_0(UIMLOG_MSG_1,uim_ptr->id,"Resetting necessary UIM globals after notifying clients of error");
...
{
  UIM_RETURN_IF_NULL_PTR_2(cmd_ptr, uim_ptr);
  RETURN_IF_INSTANCE_INVALID(uim_ptr->id);
...
)
{
  ...
  if ((flush_type == UIM_FLUSH_ALL) ||(flush_type == UIM_FLUSH_COMPLETE) )
  {
  ...
  for (i = 0; i < count; i++)
  {
    ...
    if ((cmd_ptr = (uim_cmd_type *)q_get( &uim_ptr->task.cmd_q )) != NULL)
    {
      ...
      /*UIM_FLUSH_COMPLETE - Flush complete command queue  
        UIM_FLUSH_ALL - Flush all commands except power down and power up cmd*/
      if((cmd_ptr->hdr.slot != uim_slot) ||
         ((flush_type != UIM_FLUSH_COMPLETE)&& ((cmd_ptr->hdr.command == UIM_RESET_F || cmd_ptr->hdr.command == UIM_POWER_DOWN_F))))
      {
...
)
{
  ...
  if(uim_ptr->state.current_protocol == UIM_T_1_PROTOCOL)
  {
    /*For t=1 the response block processing is done in UIM context when CMD_RSP_SIG is set.
      so the block processing should only be done if the CMD_RSP_SIG is through RX_ISR. 
      Any internally set CMD_RSP_SIG should not go into this T=1 block processing*/
    uim_ptr->t1.t_1_block_received = TRUE;
  }
#endif
...
)
{
  ...
  if((uim_ptr == NULL) || (rsp_ptr == NULL) || (uim_ptr->command.cmd_ptr == NULL) )
  {
    return UIM_CMD_ERR;
  }
...
{
  UIM_RETURN_IF_NULL_PTR_2(mask, uim_ptr);

  if(UIM_CARD_SAM  == uim_ptr->card_mode)
  {
    uim_sam_process_command(mask, uim_ptr);
  }
  else
  {
    uim_telecom_process_command(mask, uim_ptr);
  }
  return;
}
...
)
{
  ...
  if(uim_is_feature_sam_enabled() || uim_is_sub_manager_enabled())
  {
    UIMDRV_MSG_MED_3(uim_ptr->id, "Logical_id-0x%x, card_mode-0x%x, activity_status-0x%x", 
                                  uim_ptr->logical_id, uim_ptr->card_mode, uim_ptr->activity_status);
  }
...
/* A check is done to ensure the requested command carries the same
  ...
  else
  {
    ...
    {
      uim_ptr->command.static_cmd_buf.select.file_type = 
        uim_get_file_type(intermediate_select.path);
...
)
{
  ...
  uim_slot_type                  phy_slot                  = UIM_SLOT_NONE;
  ...
  phy_slot = uim_convert_logical_to_physical_slot_id(slot);

  uim_ptr = uim_get_instance_ptr((uim_instance_enum_type)(phy_slot - 1));
  ...
  /*No polling if UIM is in SAM card mode*/
  if ( (UIM_CARD_SAM  == uim_ptr->card_mode) ||
       (UIM_SLOT_STATE_INACTIVE == uim_ptr->activity_status) )
  {
    UIM_MSG_HIGH_2("power_control_ds- task is in mode 0x%x and activity status 0x%x",
                   uim_ptr->card_mode,
                   uim_ptr->activity_status);
    return (uim_ptr->setting.poll_time_delay);
  }
  ...
  if ((int)uim_ptr->state.ctl & UIM_TRAF_CHAN)
  {
    ...
    new_uim_poll_time_delay = MIN( uim_ptr->setting.proactive_time_delay,
                                   uim_ptr->setting.presence_time_delay);
  }
  ...
  if (new_uim_poll_time_delay != uim_ptr->setting.poll_time_delay)
  {
    ...
    UIMDRV_MSG_MED_1(uim_ptr->id, "Setting UIM poll timer to new delay: 0x%x",new_uim_poll_time_delay);
  }

  (void) rex_set_sigs(uim_ptr->tcb_ptr, UIM_STATE_TOGGLE_SIG);
  
  /* Restart the inactive poll timer */
                                    ...
                                    uim_ptr) == TRUE)
  {
    ...
    {
      UIMDRV_MSG_MED_1(uim_ptr->id, "Set the poll timer if poll timer is inactive: 0x%x", new_uim_poll_time_delay);
      /* Set the poll timer to the new delay */
      uim_set_poll_timer( new_uim_poll_time_delay,
                          uim_ptr->setting.wakeup_poll_offset,
                          uim_ptr);
  ...
  if (((int)uim_ptr->state.ctl & UIM_TRAF_CHAN) && 
      uim_nv_is_feature_enabled(UIMDRV_FEATURE_UIM_POLLING_ONLY_AT_POLLING_TIMER_EXPIRY,uim_ptr) == FALSE)
  {
...
{
  UIM_RETURN_IF_NULL_PTR_2(uim_ptr, mask);
...
)
{
  ...
  UIM_RETURN_IF_NULL_PTR_2(uim_ptr, mask);
  RETURN_IF_INSTANCE_INVALID(uim_ptr->id);
  UIMDRV_PUP_MSG_HIGH_0(UIMLOG_MSG_6, uim_ptr->id,"Internal command to Reset the UIM");
  ...
  if ((TRUE == uim_ptr->flag.uim_not_allow_local_card_power_up) ||
      (uim_ptr->state.status == UIM_POWERED_DOWN_S))
  {
    /* No Need to power up the local cardt */
    UIMDRV_MSG_HIGH_0(uim_ptr->id,
                      "uim_reset_uim is not allowed on local card or slot is disabled by simlock");

    /* Send error to the clients for slots which are disabled by SIMLOCK */ 
    if(uim_ptr->state.status == UIM_POWERED_DOWN_S)
    {
      uim_notify_error(UIM_POWER_DOWN_CMD_NOTIFICATION_S, uim_ptr);
    }
...
/* Before power up that slot first check card status, card is there or not */
      ...
      !uim_ptr->extended_recovery.extended_recovery_in_progress)
  {
    ...
    else
    {
      ...
      UIMDRV_PUP_MSG_HIGH_0(UIMLOG_MSG_7,uim_ptr->id,
      ...
      if (uim_ptr->state.status == UIM_UNINITIALIZED_S)
      {
        ...
        /*reset the card mode to TELECOM for Logical slot 1*/
        uim_sam_reset_card_mode(mask,uim_ptr);
      }
      uim_ptr->state.status = UIM_ERR_S;
  ...
  if (me_powerup)
  {
    ...
    {
      UIMDRV_MSG_LOW_0(uim_ptr->id,"HOTSWAP: uim_reset_uim due to card inserted command");
    }
  ...
  else
  {
    ...
    {
      /* Set command count to 1 for first recovery attempt */
      uim_ptr->command.static_cmd_buf.hdr.cmd_count = 1;
    }
    ...
    slot_id = (uim_slot_type)(uim_ptr->id + 1);

    uim_interface_gstk_send_recovery_cmd(slot_id, UIMDRV_RECOVERY_STAGE_START);
    ...
    {
      /*Convert  Physical to logical ID*/
      slot_id = uim_convert_physical_to_logical_slot_id(slot_id);
      uim_ptr->silent_recover.uim_silent_recovery_callback(slot_id,
...
)
{
  ...
  link_est_data.cdma_df_present = uim_ptr->state.dfs_1.cdma_df_present;
  link_est_data.gsm_df_present  = uim_ptr->state.dfs_1.gsm_df_present;
  ...
  link_est_data.atr_buf.num_bytes = uim_ptr->atr.atr_buf.num_bytes;
  ...
  /* Set the number of bytes copied */
  link_est_data.hist_char_buf.length = uim_ptr->atr.hist_char_buf.length;
  ...
  /* Set the number of bytes copied */
  link_est_data.iccid_buf.num_bytes = uim_ptr->cache.iccid_buf.num_bytes;
  
  if (uim_ptr->flag.is_euicc && 
     (uim_ptr->cache.eid_buf.num_bytes > 0))
  {
    /* Copy EID */
    uim_memscpy(link_est_data.eid_buf.data,
                sizeof(link_est_data.eid_buf.data),
                uim_ptr->cache.eid_buf.data,
                uim_ptr->cache.eid_buf.num_bytes);
			  
    /* Set the number of bytes of EID copied */
    link_est_data.eid_buf.num_bytes = uim_ptr->cache.eid_buf.num_bytes;
  }

    if(IS_BTSAP_HANDLE_ACTIVE(uim_ptr))
    {
      link_est_data.work_waiting_time = uim_ptr->remote.btsap.btsap_cmd_rsp_timer_value;
      link_est_data.slot_type  = UIM_CONNECTION_BTSAP;
    }
    else
    {
...
#endif
      {
        link_est_data.work_waiting_time = uim_ptr->card_cmd.work_waiting_time;
      }
      link_est_data.slot_type  = UIM_CONNECTION_ISO;
    }
  ...
  /*intialize the card mode to telecom*/
  link_est_data.card_type          = UIM_CARD_TELECOM;

  /*intialize the transmission protocol to T=0*/
  link_est_data.transmission_mode = UIM_T_0_PROTOCOL;

  /*set the card mode*/
  if(uim_is_feature_sam_enabled())
  {
    link_est_data.card_type = uim_ptr->card_mode;
  }
    /* The legacy ISO here is in a broader concept including local and remote (BTSAP) */
  UIMDRV_MSG_HIGH_1(uim_ptr->id,
                      "UIM link established with card, card_type 0x%x", link_est_data.card_type );


#ifdef FEATURE_UIM_T_1_SUPPORT
  /*set the transmission protocol*/
  link_est_data.transmission_mode = uim_ptr->state.current_protocol;
#endif

  link_est_data.is_euicc = uim_ptr->flag.is_euicc;
  /* Send the command to MMGSDI */
  (void)uim_interface_mmgsdi_notify_link_est_slot(link_est_data, (uim_slot_type)(uim_ptr->id + 1));
} /* uim_gsdi_link_est */


/*===========================================================================
FUNCTION UIM_RETRIEVE_ATR

DESCRIPTION
  This procedure handles the ATR request by MMGSDI and returns the desired ATR
...
===========================================================================*/
uim_return_type uim_retrieve_atr
(
  uim_slot_type            slot_id,
  uim_atr_buf_type       * atr_req_buf_ptr
)
{
  uim_return_type           uim_status       = UIM_GENERIC_ERROR;
  uim_instance_global_type *uim_ptr          = NULL;
  uim_slot_type             physical_slot_id = UIM_SLOT_NONE;

  if(atr_req_buf_ptr == NULL)
  {
    return uim_status;
  }

  physical_slot_id = uim_convert_logical_to_physical_slot_id(slot_id);
  uim_ptr = uim_get_instance_ptr((uim_instance_enum_type)(physical_slot_id - 1));
  ...
  if(uim_ptr->atr.atr_buf.num_bytes >= 2)
  {
    *atr_req_buf_ptr = uim_ptr->atr.atr_buf;
     uim_status = UIM_SUCCESS;
  }
...
)
{
  ...
  /* populate masking fn ptr for proactive cmd */
  ...
  if(!(mmgsdi_is_private_logging_enabled_for_slot((mmgsdi_slot_id_enum_type)uim_ptr->logical_id)))
  {
  uim_ptr->command.cmd_ptr->hdr.test_mode_rsp_log_masking_fn_ptr = gstk_log_masking_fn_proactive_cmd;
  }
...
void uim_init_critical_secs( uim_instance_global_type *uim_ptr )
{
  ...
  /* Critical Section use for ATR */
  rex_init_crit_sect(&uim_ptr->crit_sect.atr_ctl);
...
)
{
  ...
  ** -------------------- */
  ...
  /* Reset the APDU behavior to default behavior */
  for(num_ch = 0; num_ch<UIM_MAX_CHANNELS; num_ch++)
  {
...
void uim_init_common_critical_sections(void)
{
  ...
  /* Critical section used while getting GPIOID for batt_alarm */
  rex_init_crit_sect(&shared_globals_ptr->gpio_get_batt_gpioid_crit_section);
  
  /* Critical section used while updating cache for boot-up APDU logs */
  rex_init_crit_sect(&shared_globals_ptr->boot_up_apdu_log_cache.boot_up_apdu_log_cache_crit_section);  
  ...
  uimdrv_hw_nv_config_info.hw_nv_crit_sec_init = TRUE;
  
  /*intialize slot mapping critical section*/
  rex_init_crit_sect(&uim_slot_mapping.critical_section);

  /* Critical section used by Subscription manager */
  rex_init_crit_sect(&uim_slot_mapping.sm_data.critical_section);
...
void uim_1_init(void)
{
  ...
  uim_init_common_critical_sections();
  uimdrv_hal_init_function_ptr_tables();
  uimgen_hal_init_cmd_func_ptr_table();
  uimgen_hal_init_rsp_func_ptr_table();
...
{
  UIM_RETURN_IF_NULL_PTR_2(i_mask, uim_ptr);

  /*switch to SAM state machine if card mode is SAM*/
  if(UIM_CARD_SAM  == uim_ptr->card_mode)
  {
    uim_sam_process_cmd_err(i_mask,  uim_ptr);
  }
  else
  {
    uim_telecom_process_cmd_err(i_mask,  uim_ptr);
  }
  return;
}
...
)
{
      ...
      (uim_ptr->command.static_cmd_buf.hdr.command == UIM_INTERNAL_FETCH_F))
  {
    ...
    *i_mask |= UIM_CMD_Q_SIG;
	...
	if(uim_ptr->command.cmd_ptr != NULL)
	{
	  ...
	  uim_ptr->command.cmd_ptr->hdr.test_mode_req_log_masking_fn_ptr = NULL;
	}
#ifdef FEATURE_GSTK
    /* Here we pass NULL and data length of 0 to GSTK to tell it to reset
       gstk_proactive_cmd_in_progress to FALSE */
    (void)uim_interface_gstk_send_fetch_cmd_report(uim_ptr->command.static_cmd_buf.hdr.slot,
                                                   0, NULL, SW1_PROBLEM, SW2_SW1_PROBLEM);
  ...
  {
    UIMDRV_PUP_MSG_HIGH_0(UIMLOG_MSG_12,uim_ptr->id,"Process cmd error in external command");
    ...
    if (uim_ptr->command.cmd_ptr != NULL)
    {
      ...
      {
        UIMDRV_PUP_MSG_HIGH_0(UIMLOG_MSG_13,uim_ptr->id,"cmd count maxed out");
      ...
      else
      {
        ...
        UIMDRV_PUP_MSG_HIGH_1(UIMLOG_MSG_14,uim_ptr->id,"uim_command_requeued = %d",
  ...
  {
    UIMDRV_PUP_MSG_HIGH_0(UIMLOG_MSG_15,uim_ptr->id,"Process cmd error in internal command");
    ...
    if (++uim_ptr->command.static_cmd_buf.hdr.cmd_count > UIM_MAX_NUM_ATTEMPTS)
    {
      ...
      *i_mask |= UIM_CMD_Q_SIG;
      ...
      if((TRUE == uim_ptr->extended_recovery.extended_recovery_enabled) &&
         (TRUE == uim_ptr->extended_recovery.extended_recovery_in_progress))
       {
        /* Extended powerup results in error. call extended_recovery_evt_handler
           to clear extended recovery globals */
         uim_extended_recovery_event_handler(uim_ptr, UIM_EXTENDED_RECOVERY_EVT_TERMINATE);
       }
...
)
{
     ...
     || uim_ptr->state.adf_selection.required)
  {
    ...
    else if(uim_ptr->state.adf_selection.required)
    {
      ...
      uim_cmd_ptr->select.file_type             = uim_get_file_type(uim_cmd_ptr->select.path);
    }
...
{
  UIM_RETURN_IF_NULL_PTR_2(uim_ptr, i_mask);
  RETURN_IF_INSTANCE_INVALID(uim_ptr->id);

  /*switch to SAM state machine if card mode is SAM*/
  if(UIM_CARD_SAM  == uim_ptr->card_mode)
  {
    uim_sam_process_card_response(i_mask, uim_ptr);
  }
  else
  {
    uim_telecom_process_card_response(i_mask, uim_ptr);
  }
  return;
}
...
)
{
  ...
  if(status == UIM_CMD_FETCH &&
     !uim_interface_gstk_uim_fetch_allow((gstk_slot_id_enum_type)cmd_slot))
  {
    ...
    if(!uim_ptr->flag.static_buffer_used)
    {
      if (uim_ptr->command.cmd_ptr != NULL)
      {
        if(uim_ptr->command.cmd_ptr->hdr.command == UIM_RESET_F)
        {
          ...
          UIMDRV_PUP_MSG_HIGH_0(UIMLOG_MSG_17,uim_ptr->id,
  ...
  else if (status == UIM_CMD_ERR_WITH_SFI)
  {
    ...
    UIMDRV_PUP_MSG_HIGH_0(UIMLOG_MSG_19,uim_ptr->id, "SFI operation failed - Retrying command without using SFI");
  ...
  else if (status == UIM_SWITCH_VOLTAGE)
  {
    ...
    if(!uim_ptr->flag.static_buffer_used)
    {
      if (uim_ptr->command.cmd_ptr != NULL)
      {
        ...
        {
          /* Requeue the RESET F command */
          UIMDRV_PUP_MSG_HIGH_0(UIMLOG_MSG_20,uim_ptr->id,
  ...
  else if (status == UIM_SWITCH_MODE)
  {
    /*If SAM feature is not enabled then return error*/
    if(!uim_is_feature_sam_enabled())
    {
      status = UIM_CMD_ERR;
      uim_process_cmd_err(i_mask, uim_ptr);
    }
    UIMDRV_MSG_HIGH_0(uim_ptr->id,"UIM_SWITCH_MODE- telecom powerup failed. attempt SAM powerup");

    /*If reset F then report to clients and continue powerup as internal command*/
    if(!uim_ptr->flag.static_buffer_used &&
        uim_ptr->command.cmd_ptr != NULL  &&
        uim_ptr->command.cmd_ptr->hdr.command == UIM_RESET_F)
    {
      uim_process_generic_response(&uim_ptr->command.rsp_buf, uim_ptr);
      UIMDRV_MSG_HIGH_0(uim_ptr->id,"RESET_F - reported to clients and continue powerup usign internal powerup");
      uim_ptr->flag.command_in_progress = FALSE;
    }

    /*if recovery is ongoing the nattempt a fresh powerup with the new mode*/
    ...
    /*modify imask for SAM mode*/
    uim_sam_modify_imask(uim_ptr, i_mask);
...
void uim_start_initial_powerup(uim_instance_global_type *uim_ptr, rex_sigs_type *iMask_ptr)
{
  ...
  UIM_RETURN_IF_NULL_PTR_2(uim_ptr, iMask_ptr);
  RETURN_IF_INSTANCE_INVALID(uim_ptr->id)
...
{
  uim_shared_global_type *shared_global_ptr = NULL;
  ...
  shared_global_ptr = uim_get_common_globals_ptr();

  /* If NV is set to disable polling when RF is inactive, do not poll 
     only if there is no ongoing call*/
  if (TRUE == shared_global_ptr->uim_rf_is_inactive &&
      TRUE == uim_ptr->setting.config_params.future_use[UIM_RFU_INDEX_DISABLE_POLL_RF_OFF] &&
      FALSE == uim_ptr->hotswap.hotswap_card_removed_suspicious                            &&
      !(uim_ptr->state.uim_call_info.call_mask == MMGSDI_MASK_NON_PS_CALL_ONGOING          ||
        uim_ptr->state.uim_call_info.call_mask == MMGSDI_MASK_PS_CALL_ONGOING))
  {
    UIMDRV_MSG_MED_0(uim_ptr->id, "Poll disabled when RF is inactive");
    uim_ptr->flag.poll_pending = FALSE;
    uim_ptr->poll.expired_in_rf_off = TRUE;
    return;
  }
...
void uim_process_pending_fetch(uim_instance_global_type *uim_ptr)
{
  ...
  {
    if(uim_ptr->card_cmd.uimdrv_protocol == UIM_ICC)
    {
    ...
    else
    {
      ...
      uim_cmd_ptr->hdr.command			= UIM_RESELECT_CUR_DIR_F;
      uim_cmd_ptr->hdr.protocol			= uim_ptr->card_cmd.uimdrv_protocol;
      uim_cmd_ptr->hdr.slot	                = uim_ptr->id + 1 ;
      uim_cmd_ptr->hdr.cmd_hdr.task_ptr	        = uim_ptr->tcb_ptr;
      uim_cmd_ptr->hdr.cmd_hdr.sigs		= 0;
...
(
  uim_instance_global_type *uim_ptr,
  rex_sigs_type            *i_mask_ptr,
  RCINIT_GROUP              uim_group,
  RCEVT_SIGEX_SIGREX       *stop_sig_ptr
)
{
  if (uim_ptr ==  NULL || i_mask_ptr == NULL || stop_sig_ptr == NULL || uim_ptr->id >= UIM_MAX_INSTANCES)
  {
    return FALSE;
  }
  ...
  /* Setting the status to ERROR prior to invoking power down.
     ...
     cater to the user customized LDO settings if needed. */
  uim_ptr->state.status = UIM_ERR_S;
  /* Set here to prevent, after flushing command, a new command 
     get queued to uim that doesn't get cleaned which leads to
     potencial memleak */
  uim_ptr->flag.task_dereg_from_rcinit = TRUE;

  if(FALSE == m_HWConfig.uim_slot_config[uim_ptr->id].disableUim)
  {
    /* Processing of command (READ/SELECT etc) is aborted upon receipt
  ...
  /* Let us not be here time and again 
     powering_down_task flag is used in the uim_power_off_ldo_slot
     fn to prevent sending QMI notification*/
  ...
  *i_mask_ptr = 0x00;
  ...
  UIMDRV_PUP_MSG_HIGH_0(UIMLOG_MSG_22,uim_ptr->id, "Task clean up completed");
...
*                          that are set.
*/
static void uim_check_q_and_stop_clock(uim_instance_global_type *uim_ptr,
...
rex_sigs_type uim_get_imask(uim_instance_global_type *uim_ptr)
{
  ...
  else
  {
    i_mask = UIM_DOG_HB_RPT_SIG |
             ...
             UIM_RCEVT_CM_TASK_READY_SIG|
             UIM_RCEVT_DIAG_LOG_MASK_CHANGE_SIG|
...
FUNCTION UIM_REGISTER_RCEVT_CM_SIG

DESCRIPTION
  This function registers for cm ready signals
...
===========================================================================*/
void uim_register_rcevt_cm_sig(uim_instance_global_type *uim_ptr)
{
  ...
  UIM_MSG_HIGH_0("Reg for CM signal");

  mcs_rcevt_register_sigex_name("cm:ready",
                                RCEVT_SIGEX_TYPE_SIGREX,
                                &rcevt_siginfo_cm);
} /* uim_register_rcevt_cm_sig */


/*===========================================================================

FUNCTION UIM_REGISTER_RCEVT_DIAG_SIG

DESCRIPTION
  This function registers for DIAG ready signals
...
===========================================================================*/
static void uim_register_rcevt_diag_sig(uim_instance_global_type *uim_ptr)
{
  ...
  mcs_rcevt_register_sigex_name(DIAG_LOG_MASK_CHANGE_NOTIFY,
                                RCEVT_SIGEX_TYPE_SIGREX,
                                &rcevt_siginfo_diag);
} /* uim_register_rcevt_diag_sig */


/*===========================================================================

FUNCTION UIM_GET_SLOT_INFO_CB

DESCRIPTION
  This is the callback function called by SIMLOCK which informs
   which slot is not to be powered up
...
===========================================================================*/
static void uim_get_slot_info_cb(simlock_result_enum_type                    status,
                                 const simlock_message_response_data_type  * resp_ptr,
                                 const void                                * user_data_ptr)
{   
  uim_simlock_user_type  *resp_simlock_data_ptr  = (uim_simlock_user_type *)user_data_ptr;
  if(resp_simlock_data_ptr == NULL || resp_simlock_data_ptr->tcb_ptr == NULL)
  {
    ERR_FATAL("uim_get_slot_info_cb resp_simlock_data_ptr or tcb_ptr is null",0,0,0);
    return;
  }

  if(status == SIMLOCK_SUCCESS && resp_ptr != NULL)
  {
    uim_memscpy(resp_simlock_data_ptr->slot_to_be_powered_down,
    sizeof(resp_simlock_data_ptr->slot_to_be_powered_down),
    resp_ptr->message.get_status_resp.disabled_slots.slot,
    sizeof(resp_ptr->message.get_status_resp.disabled_slots.slot));
  }
  
  (void)rex_set_sigs(resp_simlock_data_ptr->tcb_ptr, UIM_GET_SLOT_INFO_SIG);
} /* uim_get_slot_info_cb */


/*===========================================================================

FUNCTION UIM_GET_SLOT_CONFIG_FROM_SIMLOCK
DESCRIPTION
  This is the function which calls SIMLOCK API 
   which will return the slots which are not to be powered up
...
===========================================================================*/
void uim_get_slot_config_from_simlock(uim_instance_global_type *uim_ptr)
{
  uim_simlock_user_type  simlock_data;
  
  UIM_RETURN_IF_NULL_PTR(uim_ptr);
  RETURN_IF_INSTANCE_INVALID(uim_ptr->id);

  memset(&simlock_data, 0x00 , sizeof(uim_simlock_user_type));

  simlock_data.tcb_ptr = uim_ptr->tcb_ptr;
  if(SIMLOCK_SUCCESS == simlock_get_status(uim_get_slot_info_cb, &simlock_data))
  {
    uim_pet_and_wait(UIM_GET_SLOT_INFO_SIG, uim_ptr);
    (void) rex_clr_sigs(simlock_data.tcb_ptr, UIM_GET_SLOT_INFO_SIG);  
    if(simlock_data.slot_to_be_powered_down[uim_ptr->id])
    {
      uim_ptr->state.status = UIM_POWERED_DOWN_S;
    }
    else
...
}/* uim_get_slot_config_from_simlock */ 
...
)
{
  ...
  /* Add driver's slot specfic thread id thread id to QSH */
#ifdef FEATURE_QSH_MDUMP
  ...
  /* Get parameters from Simlock which slots 
     are not to be powered up */
...
#ifdef FEATURE_UIM_POWERUP_LOGGING
  /*Intialize powerup logging module*/
  uim_pup_log_init(uim_ptr->id);
#endif
  ...
  ** ------------------------------------------------------------------ */
  ...
  /*Initialization of Subscription Manager 
    Ensure that Sub manager initalisation is done before slot mapping init*/
  uim_sm_init(uim_ptr);

  /*Intialize slot mapping globals*/
  uimdrv_slot_mapping_init(uim_ptr);

  /*set the activity status global for current task*/
  ...
  /*set cardmode for the current task/slot
    this shoud be called only after setting the imask*/
  uim_sam_set_card_mode(&i_mask,uim_ptr);
...
)
{
  ...
  /* Register for CM ready indication */
  uim_register_rcevt_cm_sig(uim_ptr);
  /* Register for DIAG log mask change indication */
  uim_register_rcevt_diag_sig(uim_ptr);  
...
)
{
  ...
  switch(src_cmd_ptr->hdr.command)
  {
    ...
    /* Memory De-coupling is not required for these comamnd */
    case UIM_SILENT_RECOVER_UICC_F:
    case UIM_RESELECT_CUR_DIR_F:
    case UIM_MAX_F:
      return FALSE;
    ...
    case UIM_STREAM_ISO7816_APDU_F:
      ...
      dest_cmd_ptr->stream_iso7816_apdu.cmd_data = (byte*)dest_cmd_ptr + sizeof(uim_cmd_type);

      if((dest_cmd_size - sizeof(uim_cmd_type)) >= src_cmd_ptr->stream_iso7816_apdu.num_cmd_bytes)
      {
        uim_memscpy((void*)(dest_cmd_ptr->stream_iso7816_apdu.cmd_data),
                    (dest_cmd_size - sizeof(uim_cmd_type)),
                    (void*)src_cmd_ptr->stream_iso7816_apdu.cmd_data,
                    src_cmd_ptr->stream_iso7816_apdu.num_cmd_bytes);
        return TRUE;
      }
      else
      {
        UIMDRV_MSG_ERR_2(uim_ptr->id,
                         ...
                         (dest_cmd_size - sizeof(uim_cmd_type)), src_cmd_ptr->stream_iso7816_apdu.num_cmd_bytes);
        return FALSE;
      }
...
)
{
  ...
  if(uim_ptr == NULL || NULL == uim_cmd_ptr || (uim_cmd_ptr->hdr.command > UIM_MAX_F))
  {
    return cmd_size;
  }
  ...
  switch(uim_cmd_ptr->hdr.command)
  {
    ...
    case UIM_SET_APDU_BEHAVIOR_F:
      break;
    case UIM_STREAM_ISO7816_APDU_F:
      cmd_size += uim_cmd_ptr->stream_iso7816_apdu.num_cmd_bytes;
      break;      
    case UIM_WRITE_BIN_CMD_F:
...
)
{
  ...
  UIM_RETURN_IF_NULL_PTR_2(uim_ptr, intermediate_sel_ptr);
...
)
{
  ...
  UIM_RETURN_IF_NULL_PTR_2(uim_ptr, intermediate_sel_ptr);
  UIM_RETURN_IF_NULL_PTR(cmd_aid_ptr);
...
)
{
  ...
  UIM_RETURN_IF_NULL_PTR_2(uim_ptr, intermediate_sel_ptr);
  UIM_RETURN_IF_NULL_PTR(aid_ptr);
  ...
  if(uim_ptr->command.command_requeue_counter == 0x00)
  {
    ...
    else
    {
      ...
      intermediate_sel_ptr->path.path[1] = 0x7FFF;
      intermediate_sel_ptr->path.len     = 2;
      /* Do not flush ext AUTH command if 7FFF selection fails.
         Flush the ext AUTH command if the select using AID data fails. */ 

      /* Allowing AUTH command to card after 7FFF failure only if 
         the last succesful selected file is within ADF directory, 
		 or allow for non spec compliant cards*/
      if(((uim_ptr->state.current_path[curr_ch].path.len >= 2)
         && (uim_ptr->state.current_path[curr_ch].path.path[1] == 0x7FFF))||
...
)
{
  ...
  if(!(class_byte & (UIM_LOGICAL_CHANNEL_MASK | UIM_UICC_EXTENDED_LC_CLASS) ))
  {
    ...
    {
      UIMDRV_PUP_MSG_ERR_1(UIMLOG_MSG_24,uim_ptr->id, "Stream on default with hdr->channel != 0 0x%x",
                                  ...
                                  uim_intermediate_select_type *intermediate_sel_ptr)
{
  ...
  if(intermediate_sel_ptr->is_needed)
  {
    ...
    {
      UIMDRV_PUP_MSG_HIGH_1(UIMLOG_MSG_27,uim_ptr->id, "AID is present: aid_length:0x%d",
...
)
{
  ...
  uint32                                cmd_size                  = 0;
  uim_slot_mapping_cmd_saved_data_type *slot_mapping_cmd_data_ptr = NULL;
  ...
  /*trasalte command data to redirect to the correct physical slot*/
  slot_mapping_cmd_data_ptr = uim_slot_mapping_translate_cmd(cmd_ptr);
  ...
  if(uim_ptr == NULL)
  {
    ...
    UIM_FREE(slot_mapping_cmd_data_ptr);
...
{
  UIM_RETURN_IF_NULL_PTR(report_ptr);
...
)
{
  ...
  {
    UIM_MSG_ERR_0("Command is not allowed in UIM_PASSIVE mode");
    return ( UIM_INCOMPATIBLE_STATE_ERR_S );
  }
  if ( (TRUE == uim_ptr->flag.uim_is_silent_recovery_in_progress) ||
       (NULL == (cmd_ptr = uim_malloc(sizeof(uim_cmd_type)))) ||
       ( UIM_CARD_SAM  == uim_ptr->card_mode) ||
       ( UIM_SLOT_STATE_INACTIVE == uim_ptr->activity_status)
     )
  {
    /* Either Slot is out of range or
       Memory Allocation is failed */
    UIM_MSG_ERR_3("SILENT RECOVERY error in processing the command slot 0x%x cmd mode 0x%x activity staus 0x%x ",
                          slot, uim_ptr->card_mode,uim_ptr->activity_status );

    return (UIM_GENERIC_ERROR);
  }
  
  /* Prevent subsequent processing of silent recovery */
  uim_ptr->flag.uim_is_silent_recovery_in_progress = TRUE;
...
===========================================================================*/
void uim_insert_cmd_at_head_of_queue
(
  uim_cmd_type             *cmd_ptr,
  uim_instance_global_type *uim_ptr
)
...
)
{
  ...
  switch (uim_ptr->card_cmd.uim_busy_ind_timer_state)
  {
    ...
    case UIM_BUSY_RESPONSE_TIMER_NOT_STARTED:
      ...
      {
        /* do not start the timer in SAM card mode */
        if (uim_ptr->card_mode != UIM_CARD_SAM)
        {
          /* Called in uim_send_apdu()/uim_t1_send_%_block() */
          /* Start timer */
          if(uim_ptr->flag.card_activation_indicated_to_mmgsdi == TRUE)
          {
            timer_set( &uim_ptr->card_cmd.uim_busy_ind_timer,
                       UIM_BUSY_IND_TIMER_VAL_DURING_POWER_UP_DEFAULT,
                       0, T_MSEC );
          }
          else
          {
    ...
    case UIM_BUSY_RESPONSE_TIMER_ACTIVE:
      if ( busy_timer_event == UIM_BUSY_RESPONSE_EVT_CMD_RSP_SIG ||
           ...
           busy_timer_event == UIM_BUSY_RESPONSE_EVT_POWER_DOWN )
      {
      ...
      else if ( busy_timer_event == UIM_BUSY_RESPONSE_EVT_BUSY_IND_TIMER_EXP_SIG )
      {
        ...
        UIMDRV_PUP_MSG_HIGH_0(UIMLOG_MSG_35,uim_ptr->id,"uim_notify_busy(TRUE, slot)");
        (void) uim_interface_mmgsdi_notify_sim_busy( TRUE, (uim_slot_type)(uim_ptr->id + 1));
    ...
    case UIM_BUSY_RESPONSE_TIMER_EXPIRED:
           ...
           busy_timer_event == UIM_BUSY_RESPONSE_EVT_NOTIFY_ERROR )
      {
        ...
        UIMDRV_PUP_MSG_HIGH_0(UIMLOG_MSG_36,uim_ptr->id,"uim_notify_busy(FALSE, slot)");
        (void) uim_interface_mmgsdi_notify_sim_busy( FALSE, (uim_slot_type)(uim_ptr->id + 1));
  ...
  switch (uim_ptr->card_cmd.uim_trans_timer_state)
  {
        ...
        if (uim_ptr->card_mode != UIM_CARD_SAM)
        {
          ...
          if(uim_ptr->flag.card_activation_indicated_to_mmgsdi == TRUE)
          {
            timer_set( &uim_ptr->card_cmd.uim_trans_timer,
                       uim_ptr->setting.uim_feature_busy_response.uim_power_up_trans_timer_val_ms,
                       0, T_MSEC );
          }
          else
          {
    ...
    case UIM_BUSY_RESPONSE_TIMER_ACTIVE:
      if ( trans_timer_event == UIM_BUSY_RESPONSE_EVT_CMD_RSP_SIG ||
           ...
           trans_timer_event == UIM_BUSY_RESPONSE_EVT_POWER_DOWN ||
           trans_timer_event == UIM_BUSY_RESPONSE_EVT_CLOCK_OFF)
      {
        /* Called in UIM_CMD_RSP_SIG handling in uim_task_common() */
      ...
      else if ( trans_timer_event == UIM_BUSY_RESPONSE_EVT_TRANS_TIMER_EXP_SIG )
      {
        ...
        if(uim_ptr->state.clock_is_on == FALSE)
        {
          UIMDRV_MSG_HIGH_0(uim_ptr->id,"The transaction timer expired while the clock is OFF. Ignore it");
          return;
        }
        ...
        */
        ...
        {
          UIMDRV_PUP_MSG_ERR_0(UIMLOG_MSG_37,uim_ptr->id,"DISABLE_RECOVERY_UPON_INFINITE_NULL.NULL bytes Maxed. UIM wll power down");
          (void)rex_set_sigs(uim_ptr->tcb_ptr, UIM_TRANSACTION_SIG);
        }
        else
        {
          UIMDRV_PUP_MSG_ERR_0(UIMLOG_MSG_38,uim_ptr->id,"UIM will force recovery due to trans timer timeout");
          /* Force recovery */
          uim_force_recovery(uim_ptr);
        }
      }
...
)
{
  ...
  switch(evt)
  {
    ...
    case UIM_EXTENDED_RECOVERY_EVT_TIMER_EXP:
      if(uim_hw_if.intctrl[uim_ptr->id].m_hs_enable == TRUE && uim_ptr->hotswap.card_status ==  UIM_CARD_REMOVED)
      {
        uim_hotswap_mask_interrupt(uim_ptr);
      }
...
FUNCTION UIM_IS_EXTENDED_APDU_SUPPORTED

DESCRIPTION
  whether extended APDU is supported or not.
...
===========================================================================*/
boolean uim_is_extended_apdu_supported
(
  uim_slot_type uim_slot
)
{
  ...
  if(uim_ptr->atr.hist_char_buf.length >= 4)
  {
    hist_byte_data_ptr = uim_ptr->atr.hist_char_buf.data;

    while(index < uim_ptr->atr.hist_char_buf.length - 3)
    {
      if(hist_byte_data_ptr[index] == UIM_ATR_HIST_CHAR_CARD_CAP_THIRD_TABLE)
      {
        if(hist_byte_data_ptr[index + 3] & 0x40)
        {
          flag = TRUE;
        }
        break;
      }
      index += (hist_byte_data_ptr[index] & 0x0F) + 1;
    }
  }
  
  return flag;
} /* uim_is_extended_apdu_supported */
...
)
{
  ...
  /*This is not applciable if the slot is in SAM mode*/
  if(UIM_CARD_SAM  == uim_ptr->card_mode)
  {
    return FALSE;
  }

  /*This is not applciable if the slot inactive*/
...
#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
  FUNCTION UIM_QSH_MDUMP_UIMDRV
  
  DESCRIPTION
   This function is used to dump globals for driver.
  ...
  SIDE EFFECTS
   None
===========================================================================*/
QSH_MDUMP_FN_ATTR void uim_qsh_mdump_uimdrv(void)
{
  uint8 index = 0;
  ...
  {
    qsh_mdump_collect_high(&uim_hw_if.uart[index].start_addr, sizeof(uim_hw_if.uart[index].start_addr));
  }
