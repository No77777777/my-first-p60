...
$Header: //components/rel/uim.mpss/6.1.0/uimdrv/src/uim_rxtx.c#2 $
...
11/17/15      ll         Remove the uart reset in hw wwt timeout handler
10/23/15      na         Masking of UART interrupts if error is notified to its client
09/02/15      hyo        Drivers and state machine abstraction
05/21/15      ks         ClockWork Error Fix
...
)
{
  ...
  if(uim_nv_is_feature_enabled(UIMDRV_FEATURE_DEBUG_LOG,
                               uim_ptr) == TRUE)
  {
    /* Logging of extended packtes */
    if(uim_ptr->rxtx_state_machine.rx_state == UIM_RX_RECEIVE_PPS)
    {
      for(i=0; i<size; i++)
      {
      ...
      uim_log_put_pps_data(TRUE, (uint8 *)buffer_ptr, size, uim_ptr);
    }
    else
    {
      uim_log_put_apdu_data(TRUE, TRUE, (uint8 *)buffer_ptr, size, uim_ptr);
    }
  }
...
)
{
  ...
  *status_ptr = MSMU_SR_RXBREAK;
  ...
  UIMDRV_PUP_MSG_ERR_0(UIMLOG_MSG_172,uim_ptr->id,"Received Rx break error, setting timeout signal");
...
)
{
  ...
  if (uim_ptr->rxtx_state_machine.rx_state == UIM_RX_PROCESS_IDLE)
  {
    UIMDRV_PUP_MSG_ERR_0(UIMLOG_MSG_173,uim_ptr->id,"parity error in IDLE");
    uim_uart_process_error(uim_ptr);
    if(TRUE == uim_ptr->flag.command_in_progress)
    {
      uim_force_recovery(uim_ptr);
...
#ifndef FEATURE_UIM_T_1_SUPPORT
...
#endif /* FEATURE_UIM_T_1_SUPPORT */
  {
    ...
    UIMDRV_PUP_MSG_ERR_0(UIMLOG_MSG_174,uim_ptr->id,"Maxed the parity error count");
    ret_value = FALSE;
  }
  else
  {
