...
Copyright (c) 1999-2010, 2012-2017 QUALCOMM Technologies, Inc (QTI) and its licensors.
All Rights Reserved.
...
$Header: //components/rel/uim.mpss/6.1.0/uimdrv/src/uimt1.c#2 $
...
12/14/15   ks      Fixing compiler warnings
...
)
{
  ...
  uimdrv_iso_t_1_send_s_block (uim_ptr->t1.t_1_node_address,
        ...
        case UIM_S_BLOCK_RESYNCH_RESPONSE:
          {
            ...
            uimdrv_iso_t_1_send_i_block (uim_ptr->t1.t_1_node_address, uim_ptr);
...
)
{
  ...
  {
    /* Send an R-block */
    uimdrv_iso_t_1_send_r_block (uim_ptr->t1.t_1_node_address,
    ...
    case SW1_CLA_NOT_SUPPORTED:
      {
        switch (rsp_ptr->sw2)                 /* Process the second status word */
        {
          ...
          case SW2_LAST_CMD_OF_CHAIN_EXP:
            {
              uim_ptr->card_cmd.resp_buf_ptr->cmd_status = UIM_LAST_CMD_OF_CHAIN_EXP;
            }
            break;                  
...
)
{
  ...
  UIMDRV_MSG_LOW_1(uim_ptr->id, "uim_process_t_1_block. t_1_protocol_state-0x%x", uim_ptr->t1.t_1_protocol_state );
  if (rsp_ptr == NULL)
  {
            ...
            case UIM_T_1_BLOCK_TYPE_R:
              {
                ...
                {
                  if( ++uim_ptr->t1.t_1_error_count > 4)
                  {
                    UIMDRV_MSG_LOW_0(uim_ptr->id,"R block. unexpected R block. error count > 4. process bad block");
                    uim_t_1_process_bad_block (uim_ptr);
                  }
                  else
                  {
