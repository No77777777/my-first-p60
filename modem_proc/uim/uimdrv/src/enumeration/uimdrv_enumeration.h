...
/*============================================================================
  ...
  DEPENDENCIES: N/A
                ...
                Copyright (c) 2012 - 2016, 2020 QUALCOMM Technologies, Inc(QTI) and its licensors.
                All Rights Reserved.
...
$Header: //components/rel/uim.mpss/6.1.0/uimdrv/src/enumeration/uimdrv_enumeration.h#5 $
...
11/03/20    rkg        QTANG2 BU changes 
...
#if defined(FEATURE_UIM_JOLOKIA_HWIO_MACROS)
#include "uimjolokiahwiomacros.h"
#elif defined(FEATURE_UIM_TABASCO_HWIO_MACROS)
#include "uimtabascohwiomacros.h"
#elif defined(FEATURE_UIM_ATHERTON)
#include "uimathertonhwiomacros.h"
#elif defined(FEATURE_UIM_QTANG2)
#include "uimqtang2hwiomacros.h"
#elif defined(FEATURE_UIM_HABANERO_HWIO_MACROS)
#include "uimkamortahwiomacros.h"
#else
#include "uimatlashwiomacros.h"
#endif /*FEATURE_UIM_JOLOKIA_HWIO_MACROS*/
...
#define SIZE_OF_USB_UICC_OBSOLETE                 4    //kept for backward compatibility
...
#define NUM_UIM_SLOTS_AVAILABLE_ON_8996           2
#define NUM_UIM_SLOTS_AVAILABLE_ON_9x55           2
...
/* UIM Slot config for EFS
   if enum size increases in uimSlotConfig,
   types needs to be changed to accomodate the change */
typedef PACKED struct PACKED_POST
{
  boolean                 disableUim;
  boolean                 enableUimHotswap;
  uint8                   uimHotswapPolarity;
  uint16                  uimControllerIRQNum;
  uint8                   uimUart;
  uint16                  uimUartIRQNum;
  uint8                   uimVcc;
  uint16                  uimResetGpioNum;
  uint8                   uimResetGpioFuncSel;
  uint8                   uimResetGpioDrvStrength;
  uint8                   uimResetGpioPullSetting;

  uint16                  uimDataGpioNum;
  uint8                   uimDataGpioFuncSel;
  uint8                   uimDataGpioDrvStrength;
  uint8                   uimDataGpioPullSetting;

  uint16                  uimClkGpioNum;
  uint8                   uimClkGpioFuncSel;
  uint8                   uimClkGpioDrvStrength;
  uint8                   uimClkGpioPullSetting;

  uint16                  uimCardDetectGpioNum;
  uint8                   uimCardDetectGpioFuncSel;
  uint8                   uimCardDetectGpioDrvStrength;
  uint8                   uimCardDetectGpioPullSetting;
}uimSlotConfigEfs;
...
typedef PACKED struct PACKED_POST
{
  ...
  /* USB UICC not supported, but in place for backward compatibility */
  boolean                 usb_uicc_obsolete[UIM_INSTANCES_CONFIGURABLE];
...
typedef PACKED struct PACKED_POST
{
  ...
  uint16                  uimBattAlarmGpioNum;
  uint8                   uimBattAlarmGpioFuncSel;
  uint8                   uimBattAlarmGpioDrvStrength;
  uint8                   uimBattAlarmGpioPullSetting;
  uimSlotConfigEfs        uim_slot_config[UIM_INSTANCES_CONFIGURABLE];
  uiccShutdownFeature     uicc_shutdown_feature;
  /* USB UICC not supported, but in place for backward compatibility */
  boolean                 usb_uicc_obsolete[UIM_INSTANCES_CONFIGURABLE];
  /* SWP (Single Wire Protocol) support for NFC interface */
...
typedef struct
{
  boolean SimClk_slot_diff_freq_support;
  boolean UartClk_300MHz_source_support;
}uim_clk_hardware_feature;
...
typedef struct
{
  ...
  boolean               simClkDiffFreqSupport;
  boolean               uartClk300MHzSrcSupport;

  uint16      controller_hs_irq_num[UIM_INSTANCES_CONFIGURABLE];
  uint16      uartdm_irq_num[UIM_INSTANCES_CONFIGURABLE];
  boolean     bIsExternalPullUPInstalled[UIM_INSTANCES_CONFIGURABLE];
}uim_hw_param_type;
...
typedef struct {
  ...
  uint8                        dalChipFamilyIndex;                   /* Chip family index for which HW enum being initialized */
  ...
  boolean                      hw_nv_crit_sec_init;                  /* TRUE: Critical section initialized 
                                                                        FALSE: Critical section uninitialized */
