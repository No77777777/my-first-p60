...
$Header: //components/rel/uim.mpss/6.1.0/uimdrv/src/uim_supply_voltage_indication.c#1 $
$DateTime: 2019/10/30 06:56:05 $
$Author: pwbldsvc $

when          who        what, where, why
--------      ----       ---------------------------------------------------------
11/08/17      ku         Reset the voltage indication ACK flag
...
)
{
  ...
  uim_instance = (uim_instance_enum_type) uim_slot - 1;

  if (uim_instance >= UIM_INSTANCE_INVALID || uim_instance < UIM_INSTANCE_1)
  {
    return;
  }
  ...
  UIMDRV_PUP_MSG_HIGH_1(UIMLOG_MSG_229,uim_ptr->id,"UIMDRV received ACK from QMI - 0x%x",received_ack);
...
)
{
  ...
  {
    uim_interface_qmi_uim_supply_voltage_ind((uim_ptr->id + 1),
                                             indication,
                                             uimdrv_qmi_power_management_callback);
