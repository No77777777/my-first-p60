/*============================================================================
  ...
  DEPENDENCIES: N/A
                ...
                Copyright (c) 2014 - 2018, 2024 QUALCOMM Technologies, Inc (QTI) and its licensors.
                All Rights Reserved.
...
$Header: //components/rel/uim.mpss/6.1.0/uimdrv/src/uim_rxstates.c#3 $
$DateTime: 2024/09/17 23:29:16 $
$Author: pwbldsvc $

when        who        what, where, why
------      ----       -----------------------------------------------------------
09/10/24    ac         Added handling of more data warning in fetch command
08/29/18    ng         Log commit for unhandled status words
12/29/17    ku         Feature gaurding get response for status
06/13/17    gm         Send GET-RESPONSE for status words 62 01 to 62 80
...
10/07/16    gm         Updating intermediate status words with first SWs
09/22/16    gm         Added support for set APDU behavior
05/26/16    gm         F3 message removal
03/28/16    sam        Support for SAM card
03/09/16    sam        removal of NV_UIM_PAR_ERR_WORKAROUND_I related code
01/27/16    sam        Treat t=1 response blocks as invalid if more than expected 
                       bytes are received from card
...
06/08/15    ks         Resetting UART upon bad status words
...
02/04/15    ks         MF selection when stream select receives '6A 82'
...
)
{
  ...
  {
    UIMDRV_PUP_MSG_ERR_0(UIMLOG_MSG_164,uim_ptr->id,"RX state machine not invoked - Error in rx sub state");
    rx_event_cb_err = TRUE;
  }
  return rx_event_cb_err;
} /* uimdrv_rx_event_cb */
  ...
  switch (uart_status)
  {
    case MSMU_SR_PF_ERR:
      ...
      else
      {
        ...
        UIMDRV_PUP_MSG_HIGH_0(UIMLOG_MSG_165,uim_ptr->id,"RX received with parity errors");
...
)
{
  ...
  switch (uim_ptr->rxtx_state_machine.rx_sub_state)
  {
    ...
    case UIM_RX_ATR_RECEIVED_WITH_PARITY_ERRORS:
      UIMDRV_PUP_MSG_HIGH_0(UIMLOG_MSG_166,uim_ptr->id,"RX received with parity errors while waiting for ATR");
      break;

    case UIM_T_1_RX_RECEIVED_WITH_PARITY_ERRORS:
      UIMDRV_PUP_MSG_HIGH_0(UIMLOG_MSG_167,uim_ptr->id,"T=1: RX received with parity errors, to send R block");
      if (interrupt_processed == MSMU_ISR_RXLEV)
      {
    ...
    case UIM_RX_OVERRUN:
      UIMDRV_PUP_MSG_HIGH_0(UIMLOG_MSG_168,uim_ptr->id,"RX state moved to overrun state to log RX bytes");
      uim_ptr->rxtx_state_machine.rx_sub_state = UIM_RX_PROCESS_NONE;
      uim_ptr->rxtx_state_machine.rx_state = UIM_RX_OVERRUN;
      break;

    default:
      UIMDRV_PUP_MSG_ERR_0(UIMLOG_MSG_169,uim_ptr->id,"RX sub state invalid");
    ...
    default:
      {
                                     ...
                                     uim_ptr) == TRUE)
        {
          ...
          uim_log_put_apdu_data(FALSE, TRUE, NULL, 0, uim_ptr);      
        }
...
)
{
  ...
  uim_log_put_atr_data((uint8 *)uart_rx_buf_ptr, no_of_bytes_received, uim_ptr);
...
)
{
  ...
  /* Logging of extended packets */	
  uim_log_put_pps_data(FALSE, (uint8 *)uart_rx_buf_ptr, no_of_bytes_received, uim_ptr);
    ...
    for ( i = 0; i < 4 && bytes_remaining > 0; i++)
    {
      ...
      *uim_ptr->card_cmd.resp_buf_byte_ptr++ = rx_value_array[i];
      ...
      if (uim_ptr->pps.num_bytes_to_skip == 0)
      {
        ...
        {
          UIMDRV_PUP_MSG_ERR_0(UIMLOG_MSG_170,uim_ptr->id,"Error processing PPS");
...
)
{
  ...
  /* Logging of extended packets */	
  uim_log_put_apdu_data(FALSE, FALSE, (uint8 *)uart_rx_buf_ptr, no_of_bytes_received, uim_ptr);
...
{
  /*for sam card irrespective of the status word set status as UIM_DONE*/
  ...
  switch (sw1)
  {
    ...
    case SW1_CLA_BAD:
      {
        uim_rx_isr_sw1_cla_bad(sw2, uim_ptr);
      } 
      break;
    ...
    case SW1_WARNINGS1:
      {
          ...
          if (uim_is_get_response_allowed(uim_ptr, SW1_WARNINGS1, sw2))
          {
            ...
            uim_ptr->card_cmd.get_resp_sw2   = sw2;       
            uim_rx_isr_sw1_resp_end (sw2, uim_ptr);
            uim_ptr->flag.force_get_response = FALSE;
          }
          /* Allow Get Response for GetStatus Request only when
             NV to allow get response is enabled */
                   ...
                   (uim_ptr->debug.last_sent_apdu.instrn  == STATUS) &&
                    uim_nv_is_feature_enabled(UIMDRV_FEATURE_ALLOW_GET_RESPONSE, uim_ptr))
          {
            uim_ptr->flag.sw1_warnings1_normal_end_occured = TRUE;
            uim_ptr->flag.force_get_response = TRUE;
            uim_ptr->card_cmd.get_resp_sw1 = SW1_WARNINGS1;
...
{
  UIMDRV_PUP_MSG_HIGH_0(UIMLOG_MSG_171,uim_ptr->id,"Process Unknown procedure byte");
...
#endif
    {
      uim_send_intermediate_apdu ( &uim_ptr->card_cmd.cmd_req, uim_ptr);
    }
  }
  ...
  if (uim_ptr->card_cmd.num_resp_bytes_rcvd_total < UIM_MAX_CHARS)
  {
    ...
    if(uim_processing_stream_apdu(uim_ptr))
    {
      ...
      if (uim_ptr->rxtx_state_machine.rx_state == UIM_RX_SW1_WARNINGS1 ||
          uim_ptr->rxtx_state_machine.rx_state == UIM_RX_SW1_WARNINGS2)
      {
        uim_ptr->card_cmd.resp_buf_ptr->sw2     = 0;
      }
      else
      {
        uim_ptr->card_cmd.resp_buf_ptr->sw2     = sw2;
      ...
      if(uim_ptr->rxtx_state_machine.rx_t1_block_sub_state == UIM_RX_T_1_RX_EPILOGUE_ST)
      {
        ...
        if (uim_ptr->card_cmd.num_resp_bytes_rcvd == uim_ptr->t1.epilogue_field_size)
        {
          ...
          if(0 != bytes_remaining)
          {
            ...
            /*if there are still bytes remainging even after all the expected bytes are received. 
            then this is a corrupted block. treat it as a timeout so that resend/resynch/reset can happen*/
            (void) rex_set_sigs( uim_ptr->tcb_ptr, UIM_CMD_RSP_TIMEOUT_SIG );
