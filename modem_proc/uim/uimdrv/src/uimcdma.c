...
Copyright (c) 2001-2009, 2011 - 2016, 2018 QUALCOMM Technologies, Inc (QTI) and its licensors.
All Rights Reserved.
...
$Header: //components/rel/uim.mpss/6.1.0/uimdrv/src/uimcdma.c#2 $
$DateTime: 2019/10/30 06:56:05 $
$Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
03/12/18   ku      Fixed KW errors
10/03/16   dd      Removed lint comments
06/06/16   na      Removed F3 traces
03/08/16   ssr     Adding uimcdma stubs for IOE variant
08/11/15   na     Compiler warnings removed
...
)
{
  ...
  {
     UIMDRV_PUP_MSG_HIGH_1(UIMLOG_MSG_39,uim_ptr->id, "Reporting error for CDMA command 0x%x ",
                       cmd_ptr->hdr.command );
...
)
{
  ...
  PUP_MSG_HIGH_2(UIMLOG_MSG_40, "UIM_%d: Received cdma command 0x%x ",
                        uim_ptr->id + 1,uim_ptr->command.cmd_ptr->hdr.command );
...
)
{
  ...
  UIMDRV_PUP_MSG_HIGH_1(UIMLOG_MSG_41,uim_ptr->id, "Reporting result for CDMA command 0x%x",
                    uim_ptr->command.cmd_ptr->hdr.command );
...
)
{
  ...
  switch(*uim_ptr->command.cdma_state_ptr)
  {
    ...
    case UIM_CDMA_DLOAD_REQ_ST:                    /* Download Request State */
    {
      ...
      uim_ptr->command.cdma_req_buf_ptr->apdu_hdr.p1 = 0x00;
      uim_ptr->command.cdma_req_buf_ptr->apdu_hdr.p2 = 0x00;
      uim_ptr->command.cdma_req_buf_ptr->apdu_hdr.p3 = (byte)(uim_ptr->command.cmd_ptr->download.block_len + 2);
    ...
    case UIM_CDMA_SSPR_CFG_REQ_ST:  /* SSPR Config Request State */
    {
      ...
      uim_ptr->command.cdma_req_buf_ptr->req.sspr_cfg_req.block_id =
                          (byte)uim_ptr->command.cmd_ptr->sspr_config.block_id;
