...
Copyright (c) 2008 - 2020 by QUALCOMM Technologies, Inc (QTI) and its licensors.  All Rights Reserved.
...
$Header: //components/rel/uim.mpss/6.1.0/uimdrv/inc/uim_p.h#3 $
...
05/24/16   na      Removed unused enumeration uim_items_enum_type
...
------------------------------------------------------------------------------*/
  ...
  #define UIM_READY_TO_SWITCH_SIG                  0x00002000 /* All tasks are ready to intiate slot switching */
  ...
  #define UIM_MCGF_NV_REFRESH_SIG                  0x00040000  /* MCFG NV refresh Feature */
  #define UIM_SUSPICIOUS_CARD_REM_SIG              0x00080000  /* Card Removed Suspicious signal */
  #define UIM_EXT_RECOVERY_TIMER_EXP_SIG           0x00100000  /*Extended recovery feature*/
  #define UIM_QMI_SANITY_TIMER_EXP_SIG             0x00200000  /* UIM QMI Sanity timer exp signal */
  #define UIM_QMI_ACK_RECEIVED_SIG                 0x00400000  /* QMI ACK received signal */
#ifdef FEATURE_UIM_TEST_FRAMEWORK
  #error code not present
#endif
  #define UIM_FETCH_PENDING_SIG                     0x01000000 /*  Fetch pending */
  #define UIM_RCEVT_CM_TASK_READY_SIG               0x02000000 /* CM ready signal */
  #define UIM_REMOTE_CONNECTION_UNAVAILABLE_SIG     0x04000000  /* Conn unavail signal */
#ifdef FEATURE_UIM_TEST_FRAMEWORK
#error code not present
#endif /* FEATURE_UIM_TEST_FRAMEWORK */
  #define UIM_GET_SLOT_INFO_SIG                     0x10000000  /* Get info from simlock about the slots not to be powered-up*/
  #define UIM_RCEVT_DIAG_LOG_MASK_CHANGE_SIG        0x20000000  /* DIAG log mask change signal */   
  #define UIM_REMOTE_CONNECTION_AVAILABLE_SIG       0x40000000  /* Conn Avail signal */
  #define UIM_SWITCH_LOGICAL_SLOT_SIG               0x80000000  /*Switch logical slot signal*/
/*----------------------------------------------------------------------------*/
...
/* Max number of EID bytes */
#define UIM_MAX_EID_SIZE                              16
...
#define UIM_ATR_HIST_CHAR_CARD_CAP_THIRD_TABLE      0x73
...
/* Number of subscriptions supported */
#ifdef FEATURE_UIM_SUPPORT_TRIPLE_SLOTS
  #define UIM_MAX_SUBSCRIPTIONS_SUPPORTED        3
#elif defined(FEATURE_UIM_SUPPORT_DUAL_SLOTS)
  #define UIM_MAX_SUBSCRIPTIONS_SUPPORTED        2
#else
#define UIM_MAX_SUBSCRIPTIONS_SUPPORTED          1
#endif
...
STRUCT:
  UIM_EID_BUF_TYPE
DESCRIPTION:
  Buffer To Store the EID info
------------------------------------------------------------------------------*/
typedef struct {
  byte num_bytes;
  byte data[UIM_MAX_EID_SIZE];
} uim_eid_buf_type;
...
ENUM:
  UIM_CARD_TYPE
DESCRIPTION:
  Indicates the type of card type
------------------------------------------------------------------------------*/
typedef enum{
  UIM_CARD_TELECOM,
  UIM_CARD_SAM
...
typedef struct {
  ...
  uim_slot_connection_enum_type slot_type;
  uim_card_type_enum           card_type;             /*card type used to powerup the current slot*/
  uim_transport_protocol_type  transmission_mode;     /*transmission protocol used in current slot*/
  boolean                       is_euicc;
...
/* ---------------------------------------------------------------
   ...
   ----------------------------------------------------------------*/
typedef struct {
  uint8   length;                       /*length of the EF-TST data*/
  uint8   data[UIM_EF_TST_MAX_LEN];     /*EF-TST data*/
...
typedef struct {
  ...
  uim_recovery_ef_tst_info_type ef_tst_data;     /* EF-TST file related data*/
  uim_apdu_behavior_type        apdu_behavior;   /* APDU behavior */
...
typedef struct{
  ...
  uim_slot_state_enum_type  activity_status;                 /* Activity status physical slot */
  uim_protocol_type         uim_protocol;                    /* Protocol of physical slot */
  uint8                     num_apps;                        /* Number of apps in physical slot */
  uint8                     atr_len;                         /* Length of ATR for the card inserted */
  uint8                     atr[UIM_MAX_ATR_CHARS];          /* ATR of given physical slot */
  boolean                   is_euicc;                        /* Card Type physical slot */  
  uim_status_type           uim_status;                      /* status of physical slot */
  uint8                     eid_len;                         /* Length of EID of a given slot */
  uint8                     eid[UIM_MAX_EID_SIZE];           /* EID of the given slot */
}uim_phy_slot_status_type;

/* ---------------------------------------------------------------
   STRUCTURE:      uim_logical_slot_config_type

   DESCRIPTION:
      Structure to hold the physical to logical slot mapping.
      The index of the array is the logica ID and Value is the physical ID
   ----------------------------------------------------------------*/
typedef struct {
  byte           num_slots;                          /*this indicates the num of valid entries that is
                                                        to be considered in the mapping array*/
  uim_slot_type  slot[UIM_MAX_NUMBER_INSTANCES];     /*the mappping array where:
                                                       index - Logical ID
                                                       value - Physical ID*/
} uim_logical_slot_config_type;

/* ---------------------------------------------------------------
   STRUCTURE:      uim_subscription_manager_client_type

   DESCRIPTION:
      This holds the client type that requested physical to logical slot mapping.
   ----------------------------------------------------------------*/
typedef enum
{
  UIMDRV_SUB_MGR_CLIENT_QMI_UIM,
  UIMDRV_SUB_MGR_CLIENT_SLOT_MAPPING_MODULE,
  UIMDRV_SUB_MGR_CLIENT_MAX
}uim_subscription_manager_client_enum_type;
...
===========================================================================*/
extern boolean uim_is_extended_apdu_supported
(
  uim_slot_type uim_slot
);
...
FUNCTION UIM_SWITCH_SLOT_MAPPING
...
RETURN VALUE
  UIM_PASS- if the slot mapping is accepted
...
===========================================================================*/
uim_rpt_status uim_switch_slot_mapping
(
  uim_logical_slot_config_type               *logical_slot_config_ptr,
  uim_subscription_manager_client_enum_type   client
);

/*===========================================================================

FUNCTION UIM_GET_LOGICAL_TO_PHYSICAL_MAPPING
...
RETURN VALUE
  UIM_PASS-  if valid slot mapping is being returned
  UIM_FAIL-  if existing slot mapping is invalid or slot mapping is not enabled

SIDE EFFECTS
  None

===========================================================================*/
uim_rpt_status uim_get_logical_to_physical_mapping
(
...
/*===========================================================================
  ...
  DESCRIPTION
   This procedure handles the ATR request by other modules and returns the desired ATR
