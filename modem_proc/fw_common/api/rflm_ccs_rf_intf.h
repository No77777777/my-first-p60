...
$Header: //source/qcom/qct/modem/fw/components/fw_common.mpss/rel/17.0/api/rflm_ccs_rf_intf.h#2 $

when         who     what, where, why
----------   ---     -----------------------------------------------------------
05/06/17     lw/al   TxAGC fixup for port switch
05/12/16     ck      Update payload size to fit shadow transactions for issue seq
10/21/15     hdz     Fixed compiling error when disable RFLM_CCS_SCRUTINY_MODE
09/01/15     rca     More MC Seq optimization changes (RFFE EXT Write setmode descr before monitor, split after target trans for grfc)
06/30/15     rca     MC Seq optimization changes (timing fixup table relocation, elmination of 2nd monitor descr, issue seq like descrs)
06/22/15     rca     More cleanup (bounds checking) for mc seq
06/09/15     rca     Support for mc seq data fixup table plus clean up
06/05/15     hdz     Added group to grfc to support 35 grfc
06/04/15     rca     Added qlnk_blk_addr in mc_seq intefaces
06/05/15     hdz     Added qlnk_blk pointer in ccs event
05/22/15     rca     Support for API interface change for Multi Chain Sequence
05/13/15     rca     Support for Multi Chain Sequence
02/05/15     pk      Added support to read the RFC status
...
// expand taskq num to include local task buffers from backend scheduling.
#define RFLM_CCS_RF_TASKQ_NUM_ITEMS_EXT     (RFLM_CCS_RF_TASKQ_NUM_ITEMS + RFLM_CCS_MMU_EARLY_ENQ_BUFFER_DEPTH)          
#endif
...
*/
#define RFLM_CCS_SEQ_MAX_TASKS             17

/*! @brief Maximum number of transactions allowed in one sequence.
*/
#define RFLM_CCS_SEQ_MAX_TRANSACTIONS      42
...
typedef struct 
{
  ...
  uint32 rflm_ccs_seq_data[RFLM_CCS_SEQ_MAX_SIZE_WORDS+RFLM_CCS_SEQ_MAX_TRANSACTIONS]; /*!< Internal sequence data */
...
#define RFLM_CCS_MC_SEQ_MAX_SIZE_WORDS    250
#define RFLM_CCS_MC_SEQ_DMA_CHAN_MAX      7
#define RFLM_CCS_MC_SEQ_MAX_TASKS         16
#define RFLM_CCS_MC_SEQ_MAX_TRANSACTIONS  128

/* Multi Chain Sequence API return vlaues */
typedef enum
{
  RFLM_CCS_MC_SEQ_ERR_NONE = 0,                  /*!< No error */
  RFLM_CCS_MC_SEQ_ERR_INVALID_TASK_TYPE,         /*!< Unsupported task type for mc seq */
  RFLM_CCS_MC_SEQ_ERR_MAX_CHAIN_ENTRIES_EXCEEDED,/*!< Max number of chain entries exceeded */
  RFLM_CCS_MC_SEQ_ERR_MAX_TRANSACTIONS_EXCEEDED, /*!< Max number of transactions exceeded */
  RFLM_CCS_MC_SEQ_ERR_MAX_TASK_SPLIT_EXCEEDED,   /*!< Not enough memory to support split of task */
  RFLM_CCS_MC_SEQ_ERR_INVALID_RFFE_TRIG_TYPE,    /*!< First RFFE task of chain must not be trig type IMMMEDIATE */
  RFLM_CCS_MC_SEQ_ERR_INVALID_GRFC_TRIG_TYPE,    /*!< ALL GRFC types must have NON IMMEDIATE trig type */
  RFLM_CCS_MC_SEQ_ERR_MAX_SEQ_SIZE_EXCEEDED,     /*!< Maximum MC SEQ size exceeded */
} rflm_ccs_rf_mc_seq_err_e;

/*! @brief Scratch memory for CCS mc sequence generator, allowing it to be
    re-entrant.
*/
typedef struct
{
  uint32 scratch[256];
} rflm_ccs_rf_mc_seq_scratch_t;

typedef struct
{
  uint16 size_bytes;     /*!< Represents size of sequence header and data */
  uint16 resources;      /*!< Bitmask of resources sequence uses (dma chan and bus resources) */
  uint16 duration_ustmr; /*!< Calculated duration of the entire seqence */

  /*! Fixup tables for DMA descr timing, source address, and link address in mc sequence. 
                           INTERNAL USAGE ONLY:  DON'T TOUCH!
      timing_fixup_offset[] - Represents WORD offsets from location AFTER mc seq header which
        identifies location of word in DMA sequence that represents time to be programmed
        into monitor.
      timing_fixup_data[] - Represents time offset that will be used in conjuction with base
        trigger time to construct absolute trigger time that will be programmed into monitor
      src_addr_fixup_offset[] - Represents WORD offsets from location AFTER mc seq header which
        identifies location of word in DMA sequence that represents source address DMA will read
      src_addr_fixup_data[] - Represents WORD offsets from location AFTER mc seq header which
        when combined with PDMEM allocated for the sequence will provide an absolute PDMEM address
        where DMA will read from (data)
      lnk_addr_fixup_offset[] - Represents WORD offsets from location AFTER mc seq header which
        identifies location of word in DMA sequence that represents link address connecting DMA descrs
      lnk_addr_fixup_data[] - Represents WORD offsets (Up to 2) from location AFTER mc seq header which
        when combined with PDMEM allocated for the sequence will provide an absolute PDMEM address
        where each DMA descr will link to */
  uint8 num_timing_fixups;
  uint8 num_src_addr_fixups;
  uint8 num_lnk_addr_fixups;
  uint8  timing_fixup_offset[RFLM_CCS_MC_SEQ_MAX_TASKS];
  uint8  src_addr_fixup_offset[RFLM_CCS_MC_SEQ_MAX_TASKS * 2];
  uint8  lnk_addr_fixup_offset[RFLM_CCS_MC_SEQ_MAX_TASKS * 5];
  uint32 timing_fixup_data[RFLM_CCS_MC_SEQ_MAX_TASKS];
  uint8  src_addr_fixup_data[RFLM_CCS_MC_SEQ_MAX_TASKS * 2];
  uint32 lnk_addr_fixup_data[RFLM_CCS_MC_SEQ_MAX_TASKS * 5];

  /*! Fixup table for transaction data. 
      First indices are per-task, providing an offset to the transactions
      for that task.
      byte_offset = fixup_table[fixup_table[task_idx]+transaction_idx]
      Byte offset is relative to start of "seq_header_and_data". */
  uint16 num_fixup_used;   /*!< Number of fixup entries used */
  uint16 fixup_table[RFLM_CCS_MC_SEQ_MAX_TASKS+RFLM_CCS_MC_SEQ_MAX_TRANSACTIONS]; /*!< BYTE offset to data */
  uint8 fixup_table_curr_idx[RFLM_CCS_MC_SEQ_MAX_TASKS]; /*!< Current index into fixup table per task */

  /*! Buffer to hold unfixed up internal sequence data (eventually copied to PDMEM and fixed up) */
  uint32 seq_header_and_data[(sizeof(ccs_rf_mc_seq_header_t)>>2) + RFLM_CCS_MC_SEQ_MAX_SIZE_WORDS + (RFLM_CCS_MC_SEQ_MAX_TASKS>>2)]; /*!< Internal sequence data */

  uint32 qlnk_blk_addr; /*!< rflm_qlnk_rf_event_block_type*/

} rflm_ccs_rf_mc_seq_t;


/*
@brief fixup data template
...
{
   struct{ 
    uint32 grfc_high_word;
    uint32 grfc_low_word;
   }grfc_data;
   uint8  rffe_data[8];
}rflm_ccs_fixup_data_t;

/*
@brief fixup data template 

@detail
The structure that holds two different fixup data for portA/B.
*/
typedef union
{
   struct{ 
...
extern uint32 rflm_ccs_status(void);
...
extern void rflm_ccs_check_all_tq_idle();

extern boolean rflm_ccs_rf_tq_is_payload_not_used
(
  rflm_ccs_rf_tq_handle_t *tq
);
...
extern rflm_ccs_rf_mc_seq_err_e rflm_ccs_rf_mc_seq_create
(
  rflm_ccs_rf_mc_seq_t *mc_seq,
  const ccs_rf_task_t tasks[],
  uint32 num_tasks,
  rflm_ccs_rf_mc_seq_scratch_t *scratch,
  void* qlnk_blk
);


extern rflm_ccs_rf_mc_seq_err_e rflm_ccs_rf_mc_seq_dma_time_src_link_write
(
  uint32 *ccs_dest_addr,
  uint32 trig_time_base,
  rflm_ccs_rf_mc_seq_t *mc_seq
);


extern int32 rflm_ccs_rf_enqueue_mc_seq
(
  rflm_ccs_enq_task_config_t *task_config_ptr,
  rflm_ccs_rf_mc_seq_t *mc_seq_ptr
);

/*----------------------------------------------------------------------------*/
extern void rflm_ccs_set_rffe_clk(uint8 *rffe_clk);
