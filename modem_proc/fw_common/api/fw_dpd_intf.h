...
$Header: //source/qcom/qct/modem/fw/components/fw_common.mpss/rel/17.0/api/fw_dpd_intf.h#1 $
$DateTime: 2019/06/16 13:29:39 $
$Author: deepemm $
...
//! @brief
// Maximum uint32 size of Tx sample captures -- enough for 12K samples.
//
#define FW_DPD_MAX_TX_CAPTURE_SIZE32 (FW_DPD_MAX_CAPTURE_SAMPLES / 2)
...
typedef struct
{
  ...
  union
  {
    ...
    struct
    {
      ...
      //! @brief
      // Forward path delay (relative to search center) in sample/1000 units.
      //
      int32 fwd_delay;
      ...
      //! @brief
      // Reflected path delay (relative to search center) in sample/1000 units.
      //
      int32 refl_delay;
      ...
      //! @brief
      // Delay until coupler switched for reflected measurement (in samples.)
      //
      int16 switch_samp;
    ...
    struct
    {
      ...
      //! @brief
      // Tx off correlation (dB x 100).
      //
      int16 tx_off_corr;

      //! @brief
      // Tx on correlation (dB x 100).
      //
      int16 tx_on_corr;
