...
$Header: //source/qcom/qct/modem/fw/components/fw_tdscdma.mpss/rel/17.0/api/tfw_sw_intf_msg.h#1 $
...
enum
{
 ...
 /** TX APT message */
  ...
  /** Debug MSg - TFW error recovery Enabled/ Disabled Status
   *  Interface */
  MSGR_DEFINE_UMID(TDSCDMA, FW, CMD, TRIGGER_ERR_RECOVERY,        0x97, tfw_trigger_error_recovery_cmd_t),

  MSGR_DEFINE_UMID(TDSCDMA, FW, CMD, TX_SW_PLT,                   0x99, tfw_tx_sw_plt_cmd_t),
  
 /*! \brief The last command message in the enum */                    
  TFW_CMD_LAST = TDSCDMA_FW_CMD_TX_SW_PLT,
...
enum
{
  ...
  MSGR_DEFINE_UMID(TDSCDMA, FW, RSP, RX_CONFIG_UPDATE,            0x1,  tfw_rx_config_update_rsp_t),
  /** Search Response */                                          
  ...
  /* this tx chain release confirm response will send to sw after tx chain released */ 
  MSGR_DEFINE_UMID(TDSCDMA, FW, RSP, TX_CHAIN_RELEASE_CONFIRM,    0x53, tfw_tx_chain_release_confirm_rsp_t),  
  /*! \brief The last response message in the enum */                   
  TFW_RSP_LAST = TDSCDMA_FW_RSP_TX_CHAIN_RELEASE_CONFIRM
};
...
typedef struct {
  ...
  */
  ...
  /*! \brief Indicates the tech that was active previously */
  uint32                	 prevTech;
  /*error Recovery Enable Indication*/
  uint8     errRecvryEnable;
...
/** Response to RX_CONFIG_UPDATE command.   
    \sa tfw_state_cfg_cmd_t.  
 */
typedef struct {
  ...
  /*! \brief Message header */
  msgr_hdr_struct_type      hdr;
  /*! \brief tag from rx_config_update; just echo back */
  uint32                    swUserData;
} tfw_rx_config_update_rsp_t;
/*\}*/
...
typedef enum {
  TFW_NO_ERROR = 0,
  TFW_TIMELINE_VIOLATION_ERR,
  TFW_IRAT_ERR,
  TFW_HW_VPE_ERR,
  TFW_HW_DEMBACK_ERR,
  TFW_HW_TDEC_ERR,
  TFW_HW_TX_STALL_ERR,
  TFW_HW_RXFE_STALL_ERR
} tfw_err_type_e;

#define TFW_ERROR_INFO_DM_BRIDGE_STUCK_BIT      0
#define TFW_ERR_MEM_PROF_SIZE                  16
 ...
 *
 *   This is the message definition for ERROR_RCVRY_STATE Indication
 */
typedef struct {
  /*! \brief Message header */
  msgr_hdr_struct_type             hdr;
  ...
  uint32                        memProf[TFW_ERR_MEM_PROF_SIZE];
...
typedef enum {
  TRG_ERR_NONE,
  TRG_ERR_BEFORE_FPACH_DECODE_ISR,
  TRG_ERR_AFTER_FPACH_DECODE_ISR,
  TRG_ERR_CONDITION_1,
  TRG_ERR_CONDITION_2,
  TRG_ERR_CONDITION_3,
  TRG_ERR_CONDITION_4,
...
/*! @brief Force TFW to error recovery cmd
 *
 *   This is the message definition for TRIGGER_ERR_RECOVERY command
 */
typedef struct {
  /*! \brief Message header */
  msgr_hdr_struct_type             hdr;
  /*! \brief Used for debug and testing between TL1 and TFW */
  tfw_trg_err_type_e         errorInfo; 
} tfw_trigger_error_recovery_cmd_t;  
...
typedef union {
  ...
  tfw_rx_config_update_rsp_t             rxCfgUpdateRsp;
  ...
  tfw_tx_chain_release_confirm_rsp_t     txChainReleaseConfirmRsp;
  } tfw_msg_rsp_u;
