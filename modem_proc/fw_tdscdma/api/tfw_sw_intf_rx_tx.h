...
$Header: //source/qcom/qct/modem/fw/components/fw_tdscdma.mpss/rel/17.0/api/tfw_sw_intf_rx_tx.h#1 $
...
typedef struct {
  ...
  */
  ...
  /*! \brief a tag from L1; FW won't use it and just echo it back; L1 will use it to control if need to reply to TDRM. */
  uint32 swUserData;
...
typedef struct {
  ...
  boolean downloadUk;
   ...
   /*! \brief flag tells FW if need to release tx chain  
   * 1 == release tx chain, 
   * 0 == not release tx chain 
   */
  boolean txChainRelease;
...
/*! @brief TX_CHAIN_RELEASE_CONFIRM response 
 *
 * This is a message definition for TX_CHAIN_RELEASE_CONFIRM response
 *
 */
/*@{*/ 
...
typedef struct
{
  ...
  msgr_hdr_struct_type hdr;
  /*! \brief Body of message */
  /* will always set finishFlag=1 in this msg */
  boolean finishFlag;
}tfw_tx_chain_release_confirm_rsp_t;
...
typedef struct
{
  ...
  /*-----------------------------------------------------------------------*/
  /*     DATA STRUCTURES FOR TX_SW_PLT COMMAND                           */
  /*-----------------------------------------------------------------------*/
/**
  This command is used for SW PLT TX testing purposes only
*/
                  
/* TX_SW_PLT command 
 *
 * This is a message definition for TX_SW_PLT command
 *
 */

typedef struct
{
  /* indicate bypassing  tx agc processing in SW PLT test */
  boolean txAgcBypass;
  /* SS value from the test vector */
  uint8   ss;
  /* midamble ID of Tx channel */
  uint16  midCellId;
  /* beta gain for data part of the physical channel */
  int16   betaD;
  /* beta gain for midamble */
  int16   betaMid;
} tfw_tx_sw_plt_config_t;
...
typedef struct
{
  ...
  msgr_hdr_struct_type     hdr;
  /* Number of sub-frame the txAgc test is conducted */
  tfw_tx_sw_plt_config_t   swPltCfg;
} tfw_tx_sw_plt_cmd_t ;
