...
$Header: //source/qcom/qct/modem/fw/components/fw_tdscdma.mpss/rel/17.0/api/tfw_sw_intf_log.h#1 $
...
#define TFW_DIAG_LOG_MASK_OFS_EXTEND( code ) \
  ( (uint64) 1 << ( (code) - TFW_DIAG_LOG_BASE_EXTEND_C ) )
...
#define TFW_DIAG_LOG_VALID_MASK ( \
  ...
  TFW_DIAG_LOG_MASK_OFS( TFW_DIAG_LOG_ECE_C                              ) | \
  0)

/*inform SW which packets are available in FW*/
#define TFW_DIAG_LOG_VALID_MASK_EXTEND ( \
  TFW_DIAG_LOG_MASK_OFS_EXTEND( TFW_DIAG_LOG_MCJD_IC_INFO_C              ) | \
  0)

/*used by SW*/
/*Desciption
if N is smaller than 64 the first N ids of mask[0] should be used for log packet enable status check. 
Otherwise, N-64 Ids of mask[1] should also be checked for log packet enable status.
*/
#define TFW_DIAG_LOG_OFS_MAX       (1+TFW_DIAG_LOG_MCJD_IC_INFO_C-TFW_DIAG_LOG_BASE_C)
