...
$Header: //components/rel/lte.mpss/6.6.1/api/lte_mac.h#1 $
...
#define LTE_MAC_MAX_LC              (LTE_MAX_ACTIVE_RB + 2)
/*! @brief define default subscribe ID */
#define LTE_MAC_DEFAULT_SUB_ID	 0

/*! @brief define maximal number of entity in MAC */
#define LTE_MAC_ENTITY_NUM  2
...
/*! @brief bitmask for LLM fast rach disabled */
#define LTE_MAC_LLM_FAST_RACH_DISABLED_BM       0

/*! @brief bitmask for LLM fast rach enabled in general case */
#define LTE_MAC_LLM_FAST_RACH_ENABLED_LEGACY_BM (0x1 << 0)

/*! @brief bitmask for LLM fast rach enabled in eNB activated low latency case */
#define LTE_MAC_LLM_FAST_RACH_ENABLED_NW_BM     (0x1 << 1)
...
/*! @brief Define MAC maximum number of scell */
#define LTE_MAC_MAX_CELL_ID_NUM 8
...
typedef struct
{
  ...
  uint32 taccumulate;
  uint32 treporting;
} lte_mac_ul_tput_info_start_req_msg_s;

typedef struct
{
  msgr_hdr_struct_type hdr;
  uint32 l2_reporting;
} lte_mac_ul_tput_info_start_cnf_msg_s;


typedef struct
{
  msgr_hdr_struct_type hdr;
} lte_mac_ul_tput_info_stop_req_msg_s;

/*! @brief this enum defines the aperiodic MCE status
*/
typedef enum
{
  LTE_MAC_ELS_APERIODIC_MCE_STATUS_ACK  =  0,
  LTE_MAC_ELS_APERIODIC_MCE_STATUS_NACK,
  LTE_MAC_ELS_APERIODIC_MCE_STATUS_ONGOING,
  LTE_MAC_ELS_APERIODIC_MCE_STATUS_MAX
} lte_mac_els_aperiodic_mce_status_e;
...
/*===========================================================================
  ...
  FUNCTION:  lte_mac_els_aperiodic_mce_status
  

===========================================================================*/
/*!
  @brief
  This functions return current status of the aperiodic MCE according to 
  specific rat index.
...
*/
/*=========================================================================*/
EXTERN lte_mac_els_aperiodic_mce_status_e lte_mac_els_aperiodic_mce_status
(
  uint8  rat_index,  /*!< RAT Index */
  uint8  as_id       /*!< sub id */
);

/*===========================================================================

  FUNCTION:  lte_mac_qos_llm_fast_rach_enabled_status
  

===========================================================================*/
/*!
  @brief
  This function returns LLM's fast rach enabled status.
  called by ML1.
  
  @return
  the return value is a bitmask:
  bit0: if fast rach is enabled or not in general case.
  bit1: if fast rach is enabled or not in eNB activated low latency case.
  bit2~7: reserved
  0: disabled; 1: enabled.
*/
/*=========================================================================*/
EXTERN uint8 lte_mac_qos_llm_fast_rach_enabled_status
(
  uint8  as_id  /*!< sub id */
);
