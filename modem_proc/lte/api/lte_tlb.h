...
$Header: //components/rel/lte.mpss/6.6.1/api/lte_tlb.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/03/17   vp      CR2024663: FR 42073: Added new API to expose UL packet size
03/15/16   el      CR 990353: Mode D implementation
...
/*! @brief define TLB/NAS message container size, for mode-d, the maximal
allowed number record is LTE_TLB_MAX_NUM_PROSE_PKT_COUNT, since for each pkt
count value, count will take 4 bytes
*/
#define LTE_TLB_MAX_NAS_MSG_CONTAINER_SIZE          512

/*! @brief define TLB default Protocal descriptor and security
*/
#define LTE_TLB_PRO_DES_SECURITY_MODE_DEFAULT_HDR   0x0F

/*! @brief define TLB mode D message maximum list size
*/
#define LTE_TLB_MODE_D_MSG_MAX_LIST_SIZE            32

/*! @brief define the maximal number of the stack    */
#define LTE_TLB_MAX_NUM_STACK                       2
...
/*! @brief Enumeration of TLB mode D action type
*/
typedef enum
{
  LTE_TLB_MODE_D_ANNOUCE,
  LTE_TLB_MODE_D_MONITOR,
  LTE_TLB_MODE_D_RESET
} lte_tlb_mode_d_action_e;
...
/*! @brief Message structure for TLB MODE D CFG Indication, it will 
contains a list of monitor with max number as LTE_TLB_MODE_D_MSG_MAX_LIST_SIZE,
and may need several messages to complete one configuration (when monitors 
received from SS are larger than LTE_TLB_MODE_D_MSG_MAX_LIST_SIZE) */
typedef struct
{
  msgr_hdr_s hdr;       /*!< Message router header */
  lte_tlb_mode_d_action_e  action;

  /*! Flag to indicate this is the last msg, if we split the 
  cfg to multiple msg due to LTE_TLB_MODE_D_MSG_MAX_LIST_SIZE
  limit */
  boolean   is_last_msg;
  /*! the message payload */
  /*! number of monitor in the below array */
  uint16    monitor_num;
  /*! Array of monitor in uint16 */
  uint16    monitor_list[LTE_TLB_MODE_D_MSG_MAX_LIST_SIZE];
} lte_tlb_mode_d_cfg_ind_s;

/*! @brief Message structure for TLB MODE D PKT Count Confirmation, 
it will contains a list of counter with max number as 
LTE_TLB_MODE_D_MSG_MAX_LIST_SIZE */
typedef struct
{
  msgr_hdr_s hdr;       /*!< Message router header */
  ...
  limit */
  boolean   is_last_msg;
  /*! number of counter in the below array */
  uint16 counter_num;
  /*! Array of counter in uint32 */
  uint32 counter_list[LTE_TLB_MODE_D_MSG_MAX_LIST_SIZE];
} lte_tlb_mode_d_pkt_count_cnf_s;
...
/*===========================================================================
  ...
  FUNCTION:  lte_tlb_mode_a_get_ul_packet_size

===========================================================================*/
/*!
    @brief
    This function returns the Mode A uplink packet size

    @detail
    This is called by data services within their task context

    @return
    Value of the uplink packet size. Default: 0xFFFF(-1) indicates invalid
    pkt size. No scaling is done    

    @see related_function()

*/
/*=========================================================================*/
EXTERN uint16 lte_tlb_mode_a_get_ul_packet_size
(
  lte_rb_id_t     rb_id          /*!< RB ID of the pdcp_pkt_ptr*/
);
