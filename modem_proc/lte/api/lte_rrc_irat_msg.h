...
$Header: //components/rel/lte.mpss/6.6.1/api/lte_rrc_irat_msg.h#1 $
...
typedef enum
{
  ...
  LTE_RRC_IRAT_TO_LTE_REDIR_FAILURE_CAUSE_NO_VALID_CANDIDATE   = 2,
...
/*! Below message should be sent by W-RRC when L2 ack is received 
    for the OTA message that carried EUTRA Capability container with
    truncated CA capabilities*/
typedef struct 
{
  /*message header type*/
  msgr_hdr_s msg_hdr; 

} lte_rrc_wcdma_eutra_cap_sent_ind_s;
...
enum
{
  ...
  /*RAT_xyz-> EUTRA Capabilities Request*/
  MSGR_DEFINE_UMID(LTE, RRC, REQ, NW_REQ_EUTRA_CAPABILITIES, 0xA3, lte_rrc_nw_req_eutra_capabilities_req_s),
  /* Special umid for TDSCDMA --> LTE capability request/response */
  MSGR_DEFINE_UMID(LTE, RRC, REQ, EUTRA_CAPABILITIES_UTRA_TDD, 0xA4, lte_rrc_eutra_capabilities_req_s),
  /* Special umid for TDSCDMA -> Dedicated Priorities Request */
  MSGR_DEFINE_UMID(LTE, RRC, REQ, TDSCDMA_GET_DEDICATED_PRI, 0xA5, lte_rrc_get_dedicated_pri_req_s),
};
...
typedef struct
{
  ...
  /*! Boolean to indicate if redirection is after CSFB */
...
/*! @brief Enum for specifying IRAT to LTE CA capabilities request type
*/
typedef enum
{
  LTE_RRC_CA_CAP_REQ_TYPE_FULL_CA  = 0,
  LTE_RRC_CA_CAP_REQ_TYPE_TRUNCATED_CA   = 1,
} lte_rrc_ca_cap_req_type_e;
...
typedef struct
{
  ...
  lte_rrc_ca_cap_req_type_e      lte_rrc_ca_req_type; /*!< Whether IRAT is requesting for Full or 
                                                        Truncated CA capabilities from LTE. Only used by
                                                        WCDMA for now */

  sys_plmn_id_s_type             camped_plmn;  /*! < Camped PLMN from W / TDS*/
}lte_rrc_eutra_capabilities_req_s;

#define LTE_RRC_NW_REQ_MAX_NUM_LTE_CAP_BAND 16

/*! @brief Message for RAT_xyz to LTE EUTRA NW Req Capabilities Request
 */
typedef struct
{
  msgr_hdr_s msg_hdr; /*!< Message router header */
  ...
  sys_sband_lte_e_type requested_bands[LTE_RRC_NW_REQ_MAX_NUM_LTE_CAP_BAND];
  
  sys_plmn_id_s_type             camped_plmn;  /*! < Camped PLMN from W / TDS*/
}lte_rrc_nw_req_eutra_capabilities_req_s;
...
typedef struct
{
  ...
  /* <TRM priority apply for current PLMN search >*/
  ...
  sys_srch_priority_e_type search_priority;
...
enum
{
  ...
  /*EUTRA->RAT_xyz Capabilities Response */
  MSGR_DEFINE_UMID(LTE, RRC, RSP, NW_REQ_EUTRA_CAPABILITIES, 0xA3, lte_rrc_eutra_capabilities_rsp_s),
  /* Special umid for TDSCDMA --> LTE capability request/response */
  MSGR_DEFINE_UMID(LTE, RRC, RSP, EUTRA_CAPABILITIES_UTRA_TDD, 0xA4, lte_rrc_eutra_capabilities_rsp_s),
  /* Special umid for TDSCDMA --> Dedicated Priorities Response */
  MSGR_DEFINE_UMID(LTE, RRC, RSP, TDSCDMA_GET_DEDICATED_PRI, 0xA5, lte_rrc_get_dedicated_pri_rsp_s),
};
...
/*!
    ...
    Return forbidden earfcn & pci to TDSCDMA/WCDMA during redirection to avoid ping-pong
    redirection between TDS/WCDMA and LTE.
*/
typedef struct
{
  /*! LTE frequency */
  lte_earfcn_t                       earfcn;
  /* Forbidden cell count */
  uint8                                   forbidden_cells_count;
  /* Forbidden cell information */
  lte_phy_cell_id_t               forbidden_cell[LTE_RRC_IRAT_MAX_EUTRA_CELLS_PER_FREQ]; 
  
} lte_rrc_irat_to_lte_redir_forbidden_cell_info_s;
  
typedef struct
{
  /*! Number of frequencies present */
  uint8                                             earfcn_count;  
  /*!< Composite of LTE Frequency with PCI info */
  lte_rrc_irat_to_lte_redir_forbidden_cell_info_s   earfcn_info[LTE_RRC_IRAT_MAX_REDIR_FREQ]; 
  
} lte_rrc_irat_to_lte_redir_forbidden_earfcn_info_s;
...
typedef struct
{
  ...
  /*!< Forbidden earfcn information >*/
  lte_rrc_irat_to_lte_redir_forbidden_earfcn_info_s   forbidden_earfcn; 
...
typedef struct
{
  ...
  /*!< Indicates if the preferred PLMN was found or not */
...
enum
{
  ...
  MSGR_DEFINE_UMID(LTE, RRC, IND, WCDMA_EUTRA_CAP_SENT, 0x8C, lte_rrc_wcdma_eutra_cap_sent_ind_s),
};
