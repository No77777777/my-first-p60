...
$Header: //components/rel/lte.mpss/6.6.1/api/lte_mac_msg.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
03/03/16   el      FR 28315 - MAC layer implementation
07/16/15   mg      CR839830: RAVE API to provide ul allowed tput
06/15/15   ru      CR820308: Mav Debug Snapshot Merged from following
...
*/
#define LTE_MAC_CFG_MAX_ELS_ACK_PER_TTI        20
...
enum
{
  ...
  MSGR_DEFINE_UMID(LTE,MAC,REQ,UL_TPUT_INFO_START,0x09,
                   lte_mac_ul_tput_info_start_req_msg_s),
  MSGR_DEFINE_UMID(LTE,MAC,REQ,UL_TPUT_INFO_STOP,0x0A,
                   lte_mac_ul_tput_info_stop_req_msg_s),
  MSGR_DEFINE_UMID(LTE,MAC,REQ,PAC_ADD,0x0B,
                   lte_mac_pac_add_req_s),
  MSGR_DEFINE_UMID(LTE,MAC,REQ,PAC_DEL,0x0C,
                   lte_mac_pac_del_req_s),
  MSGR_DEFINE_UMID(LTE,MAC,REQ,FILTER_ADD,0x0D,
                   lte_mac_filter_add_req_s),
  MSGR_DEFINE_UMID(LTE,MAC,REQ,FILTER_DEL,0x0E,
...
enum
{
  ...
  MSGR_DEFINE_UMID(LTE,MAC,CNF,UL_TPUT_INFO_START,0x07,
                   lte_mac_ul_tput_info_start_cnf_msg_s),
  MSGR_DEFINE_UMID(LTE,MAC,CNF,PAC_ADD,0x08,
                   lte_mac_pac_add_cnf_s),
  MSGR_DEFINE_UMID(LTE,MAC,CNF,FILTER_ADD,0x09,
...
enum
{
  ...
  MSGR_DEFINE_UMID(LTE,MAC,IND,HARQ_RESULT,0xC,
                   lte_mac_harq_result_ind_s),
  MSGR_DEFINE_UMID(LTE,MAC,IND,DIAG_DATA,0xD,
                   lte_mac_diag_data_ind_s),
  MSGR_DEFINE_UMID(LTE,MAC,IND,QSH_CB,0xE,
                   lte_mac_qsh_cb_ind_s),
  MSGR_DEFINE_UMID(LTE,MAC,IND,UL_TPUT_INFO,0xF,
                   lte_mac_ul_tput_info_ind_s),
  MSGR_DEFINE_UMID(LTE,MAC,IND,PAC_MATCH_RPT,0x10,
                   lte_mac_pac_match_rpt_ind_s),
  MSGR_DEFINE_UMID(LTE,MAC,IND,PAC_TX_STATUS_RPT,0x11,
                   lte_mac_pac_tx_status_rpt_ind_s),
  MSGR_DEFINE_UMID(LTE,MAC,IND,TX_HARQ_INFO,0x12,
                   lte_mac_tx_harq_info_ind_s),
  MSGR_DEFINE_UMID(LTE,MAC,IND,DATA_INACTIVITY,0x13,
                   lte_mac_data_inactivity_ind_s),
  MSGR_DEFINE_UMID(LTE,MAC,IND,EXT_HIGH_DATA_ARRIVAL,0x14,             
                   lte_mac_ext_hd_arrival_ind_s),
  MSGR_DEFINE_UMID(LTE,MAC,IND,NW_LOW_LATENCY_ACTIVATE,0x15,
