...
$Header: //components/rel/lte.mpss/6.6.1/api/lte_cphy_rssi_msg.h#2 $
...
typedef struct lte_api_signal_info_s{
  ...
  /*! UL CA is configured for the CC */  
  boolean              scc_ul_configured[CMAPI_LTE_ML1_MAX_SCC];
  ...
  /*! bw of scc */
  lte_bandwidth_e                scc_dl_bw[CMAPI_LTE_ML1_MAX_SCC]; 

  /*! SCC UL EARFCN */  
  int32                          scc_ul_channel[CMAPI_LTE_ML1_MAX_SCC];

  /*! SCC UL BW */ 
  lte_bandwidth_e                scc_ul_bw[CMAPI_LTE_ML1_MAX_SCC]; 

  /*! SCC UL TX power */ 
  lte_api_tx_pwr_info_s_type     scc_tx_power[CMAPI_LTE_ML1_MAX_SCC];
  ...
  /* Modulation (SCC) */
  lte_api_mod_s  scc_ul_modulation[CMAPI_LTE_ML1_MAX_SCC];
  lte_api_mod_s  scc_dl_modulation[CMAPI_LTE_ML1_MAX_SCC];
...
typedef struct
{
  ...
  /*! Flag to indicate if UL is enabled for this Scell, 
      Not applicable for PCell */
  boolean ul_enabled;
...
{   
  ...
  /* total number of Scell configured */
  uint16 num_scell_configured;
  
  /* total DL bandwidth for all serving cells ; in unit of RB */
  uint16 total_configured_bw;

  /* total number of unchanged Scell in array unchanged_scell_info*/
  uint16 num_unchanged_scell;

  /* SCCs that are configured, but have not changed its state for this event report*/
  lte_cphy_cmapi_cell_info_s unchanged_scell_info[LTE_CPHY_MAX_S_CELL -1 ];
...
typedef struct
{
  ...
  /*Scell info*/
  lte_cphy_cmapi_cell_info_s scell_info[CMAPI_LTE_ML1_MAX_SCC];

} lte_cphy_ca_event_ext_ind_s;

/*!
  @brief
  Per cell MIMO and modulation data
*/
typedef struct
{
  /*cell info for sanity*/
  lte_cphy_cmapi_cell_info_s cell_info;

  boolean dl_256qam;
  boolean ul_64qam;
  boolean mimo_4_layers;

} lte_cphy_cell_mimo_mod_info_s;

/*!
  @brief
  Indicates a MIMO and modulation event has occurred
*/
typedef struct
{
  /*! Msgr Header for the LTE_CPHY_MIMO_MOD_EVENT_IND */
  msgr_hdr_struct_type   msgr_hdr;

  uint8 num_updated_cells;

  lte_cphy_cell_mimo_mod_info_s cell_list[LTE_CPHY_MAX_SERVING_CELL];

} lte_cphy_mimo_mod_event_ind_s;
...
enum
{
  ...
  MSGR_DEFINE_UMID(LTE, CPHY, IND, CA_EVENT_EXT,
                   LTE_CPHY_IND_CA_EVENT_EXT_ID, lte_cphy_ca_event_ext_ind_s),

  MSGR_DEFINE_UMID(LTE, CPHY, IND, MIMO_MOD_EVENT,
                   LTE_CPHY_IND_MIMO_MOD_EVENT_ID, lte_cphy_mimo_mod_event_ind_s),
};
