...
/*===========================================================================
  ...
  Copyright (c) 2006 - 2016 QUALCOMM Technologies Incorporated. All Rights
  Reserved
...
$Header: //components/rel/lte.mpss/6.6.1/api/lte_cphy_msg.h#6 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/04/16   avj     CR 921214: PRACH Mask Index changes for contention-free RACH
03/03/16   el      FR 28315 - MAC layer implementation
03/02/16   cjb     CR 931109: Extend TPC_FREEZE to tuneaway zone and QBTA
02/25/16   sunita  FR 28315 - Add LTED interface
10/22/15   ymu     MAC requirement to send SR type back through SR confirmation MSG
...
02/20/15   su      Pass tx tune info to MAC for rach restart decision at DTA end.
...
/*! @brief defined the maximal number of Resource blocks in a single discovery period
*/
#define LTE_CPHY_MAX_NUM_RB_PER_LTED_DISCOVERY_PERIOD                    100
/*! @brief Maximum number of LTE D EARFCN*/
#define LTE_CPHY_DTOD_MAX_EARFCN_LIST 8

/*! @brief Maximum number of LTE D EARFCN*/
#define LTE_CPHY_DTOD_MAX_CFG_LIST 9

/*! @brief Maximum number of RX POOL INFO*/
#define LTE_CPHY_DTOD_MAX_RX_INFO_LIST 16

/*! @brief Maximum number of TX POOL INFO*/
#define LTE_CPHY_DTOD_MAX_TX_INFO_LIST 4

/*! @brief Maximum number of RX POOL INFO*/
#define LTE_CPHY_DTOD_MAX_INDEX_PAIR_LIST 64

/*! @brief MAX number of DL carriers supported in CA
    NOTE: This should always be >= LTE_LL1_DL_NUM_CARRIERS */
#define LTE_CPHY_DL_NUM_CARRIERS    3

#define LTE_CPHY_MAX_NUM_CELL_UNBAR 16

#define LTE_CPHY_PHR_INVALID_TAG                 0xff /*! for legacy PHR failue due to no grant to carry PHR payload */

/* UE Feature mask*/
#define LTE_CPHY_256QAM_SUPPORT                  0x1 /*! 256 QAM*/

#ifdef FEATURE_LTE_IMS_RTP_BLER_EVAL
#define LTE_ML1_IMS_RTP_BLER_EVAL_BUF            21
#endif
/*===========================================================================
...
/*! @brief Define the flow control indication which is sent from ML1 to MAC to
do flow control */
typedef struct
{
  ...
  lte_fc_cmd_e          cmd;
  /*! Final step timer value */
  uint32                step_timer;
  /*! target rate for user set in cfcm, only valid when the cmd is
  LTE_FC_CMD_SET_VALUE */
  uint32                target_rate;
}lte_cphy_mac_fc_ind_s;

/*! @brief MAC FC state indication which is send from MAC to ML1 to indicate
the current MAC FC state */
typedef struct
{
  /*! standard msgr header */
  msgr_hdr_struct_type       hdr;
  /*! Current MAC FC state */
  lte_fc_cmd_e               fc_state;
}lte_cphy_mac_fc_state_ind_s;
...
typedef struct {
  ...
  /*! Indicates the applicability of the alternative TBS index for the I_TBS 26 and 33
  (see TS 36.213 Table 7.1.7.2.1-1 to all subframes scheduled by DCI format 2C or 2D
  If set to invalid, UE shall use I_TBS 26 and 33 specified in Table 7.1.7.2.1-1 in
  TS 36.213 for all subframes instead.*/
  lte_l1_tbs_index_alt_r12_e tbs_idx_alt_r12;

} lte_cphy_pdsch_cfg_v1280_s;
...
typedef struct {
  ...
  */
  ...
  /*! Indicates if PDSCH cfg v1280 is present or not */
  boolean pdsch_cfg_r1280_present;

  /*! PDSCH cfg v1280, is signaled for serving freq only in case of TM9/10 */
  lte_cphy_pdsch_cfg_v1280_s pdsch_cfg_v1280;
...
typedef struct
{
  ...
  /*! NA if config is part of Scell. Window size for reception of RACH message2.
...
typedef struct {
  ...
  */
  ...
  /*! Parameter delta preamble msg 3 is present or not*/
  boolean delta_preamble_msg3_enabled;
...
typedef enum
{
  LTE_CPHY_POWER_CLASS3_DEFAULT = 0,
...
typedef struct {
  ...
  // Boolean to indicate if PMAX is from OTA/SIB1 or not
  // If the boolean is false then UE is using a default PAX MAX of LTE_DEFAULT_PMAX
  boolean is_pmax_from_ota;
...
typedef struct
{
  ...
  /*! Flag to indicate if CQI release 11 parameters are present or not */
  boolean cqi_rel11_param_present;

  /*! CQI release 11 parameters */
  lte_l1_cqi_rel11_param_s cqi_rel11_param;

  /*! Flag to indicate if CQI release 12 parameters are present or not */
  boolean cqi_rel12_param_present;

  /*! CQI release 12 parameters */
  lte_l1_cqi_rel12_param_s cqi_rel12_param;
...
typedef struct {
  ...
  */
  ...
  /*! Field to indicated if MIMO layers is configured
  TRUE: Configured only for TM3/4/9 and 10 (TBD)
  FALSE: ML1 has to follow legacy and revert any existing cfg */
  boolean max_mimo_lyr_cfged;

  /*! Configuration parameter for max mimo layer
  for TM9, RRC will give min (max CSI-RS ports, max mimo layer advertised for the
  corresponding band);
  for TM3/TM4: RRC will give based on ded cfg from NW if provided else */
  lte_l1_max_mimo_lyr_e max_num_mimo_lyr;
...
typedef enum
{
  LTE_CPHY_SR_RETX_NUM_DEFAULT = 0, /*!< NUM_DEFAULT means ML1 doesn¡¯t apply the sr_retx_num from MAC */
  LTE_CPHY_SR_RETX_NUM_1,            
  LTE_CPHY_SR_RETX_NUM_2,            
  LTE_CPHY_SR_RETX_NUM_3,            
  LTE_CPHY_SR_RETX_NUM_4,            
  LTE_CPHY_SR_RETX_NUM_5,            
  LTE_CPHY_SR_RETX_NUM_6
...
typedef struct
{
  ...
  /*! Max retx times SR, this is optional from MAC only for LTA scenario or low latency opt, 
    if it is set, ML1 need to follow MAC pattern*/
  uint8 sr_retx_num_opt;
...
/*!
  @brief
  An enum describing  if TX pool
  has to be released or set up
  */
typedef enum
{
  LTED_TX_RELEASE,
  LTED_TX_SETUP
}lte_cphy_dtod_tx_cfg_e;

/*!
  @brief
  An enum describing if Tx pool is
  scheduled or UE selected
  */
typedef enum
{
  LTED_TX_SCHEDULED,
  LTED_TX_UE_SELECTED
}lte_cphy_dtod_ded_setup_e;


/*!
  @brief
  An enum describing LTE D deployment
  model
  */
typedef enum
{
  LTED_MODEL_NONE,
  LTED_MODEL_A, /*EFS Config*/
  LTED_MODEL_B, /*LTE D ON INTERFREQ*/
  LTED_MODEL_C  /*LTE D ON SERVING*/
}lte_cphy_dtod_cfg_model_e;

/*! @brief
    Enumeration defines LTE D repition period in radio frames
*/
typedef enum
{
  LTED_DISC_PERIOD_32RF,
  LTED_DISC_PERIOD_64RF,
  LTED_DISC_PERIOD_128RF,
  LTED_DISC_PERIOD_256RF,
  LTED_DISC_PERIOD_512RF,
  LTED_DISC_PERIOD_1024RF,
  LTED_DISC_PERIOD_SPARE2,
  LTED_DISC_PERIOD_SPARE
} lte_cphy_dtod_disc_period_e;


/*! @brief
  Parameter struct for lte d earfcn list
*/
typedef struct
{

  /*! The num of earfcn in the list */
  uint8 num_earfcn;

  /*! The frequency ML1 needs to monitor */
  lte_earfcn_t dl_cell_frequency[LTE_CPHY_DTOD_MAX_EARFCN_LIST];

}lte_cphy_dtod_disc_inter_freq_list_s;


/*! @brief
  Parameter struct for lte d resource cfg
*/
typedef struct
{

 /* prb_num/prb_start/prb_end identify the
    discovery PRBs within a disc subframe*/
 uint8 prb_num;
 ...
 /* Indicates the offset of the first period of
   pool of resources within SFN cycle*/
 uint16 sl_offset_indicator;

 /*Indicates num of bits in subframe bitmap*/
 uint8 num_bits;

 /*Indicates the subframe bitmap indicating
 resources used for sidelink
 packed at LSB*/
 uint64 sl_sub_frame_bitmap;

}lte_cphy_dtod_sl_tf_resource_config_s;
...
/*! @brief enum for LTE-D pool selection
    for transmission
*/
typedef enum
{
 ...
 LTED_RSRP_BASED,
 LTED_MAX
}lte_cphy_dtod_tx_pool_selection_e;

/*! @brief enum for LTE-D tx probability
*/
typedef enum
{
  LTE_RRC_LTED_TX_PROB_P25 = 25,
  LTE_RRC_LTED_TX_PROB_P50 = 50,
  LTE_RRC_LTED_TX_PROB_P75 = 75,
  LTE_RRC_LTED_TX_PROB_P100 = 100,
  LTE_RRC_LTED_TX_PROB_INVALID    /* Not Used, Invalid value */
} lte_rrc_lted_tx_probability_e;


/*! @brief
  Parameter struct for lte d resource cfg
  Specifies the thresholds used to select a
  resource pool in RSRP based pool selection
*/
typedef struct
{
uint8 thresh_low;
...
}lte_cphy_dtod_pool_selection_config_s;

/*! @brief
  Parameter struct for lte d rx cfg
*/
typedef struct
{
  /*indicates the cyclic prefix length*/
  lte_l1_cp_e sl_cp_len;

  /*LTE D repetitions in radio frames*/
  lte_cphy_dtod_disc_period_e disc_period;

  uint8 numRetx;

  /*Indicates the number of times
  subframeBitmap occurs within a discPeriod. */
  uint8 numRepetition;

  /* Specifies a set of time/ frequency
   resources used for sidelink*/
  lte_cphy_dtod_sl_tf_resource_config_s sl_tf_resource_config ;

  /*Indicates DL/UL subframe configuration*/
  lte_cphy_tdd_cfg_param_s tdd_cfg;

}lte_cphy_dtod_disc_rx_pool_list_s;

/*! @brief
  Parameter struct for lte d tx cfg
*/
typedef struct
{
  ...
  lte_l1_ul_power_ctrl_alpha_e power_ctlr_alpha;

  /*Indicates if pool selection cfg has to be used or not
  TRUE: Add/Update/Start using it
  FALSE: Release/Stop using it */
  boolean pool_sel_cfg_enable;

  /*Indicates the mechanism for selecting a (transmission) pool*/
  lte_cphy_dtod_tx_pool_selection_e poolSelection;

  /*Specifies the thresholds used to select a
  resource pool in RSRP based pool selection*/
  lte_cphy_dtod_pool_selection_config_s poolSelection_cfg;

  lte_rrc_lted_tx_probability_e tx_prob;

  /*Indicates DL/UL subframe configuration*/
  lte_cphy_tdd_cfg_param_s tdd_cfg;

}lte_cphy_dtod_disc_tx_pool_list_s;

/* Indicates the probability of transmitting
  announcement in a discovery period when
  configured with a pool of resources*/
//lte_cphy_dtod_tx_probability_e txProbability;
...
{
  uint8 disc_sf_index;
  uint8 disc_prb_index;
}lte_cphy_dtod_index_pair_s;

/*! @brief
  Parameter struct for
  Affects the hopping performed as specificed in
  TS 36.213 [23, 14.1.1.2 and 14.1.1.4].
  In case value 504 is received, the value used by the UE is 510.
*/
typedef enum {
   LTED_HOPPING_CFG_N1,
   LTED_HOPPING_CFG_N5,
   LTED_HOPPING_CFG_NONE
} lte_cphy_dtod_hopping_cfg_e;


typedef uint16 lte_cphy_dtd_cfg_mask_t;

typedef enum
{
  LTE_CPHY_DTOD_CFG_NONE = 0x0000,
  LTE_CPHY_DTOD_CFG_DISC_CFG = 0x0001,
  LTE_CPHY_DTOD_CFG_EARFCN_LIST_CFG = 0x0002,
  /*! UPDATE the value below whenever any of the above bitmasks change */
  LTE_CPHY_DTOD_CFG_CURRENT_VALID_MASK_BITS = 0x0003
}lte_cphy_dtod_cfg_mask_e;
...
typedef struct
{
  uint8 a;
  uint8 b;
  lte_cphy_dtod_hopping_cfg_e c;
}lte_cphy_dtod_disc_hopping_cfg_s;

typedef struct
{
  uint8 lted_pool_id;

  lte_cphy_dtod_disc_tx_pool_list_s lted_disc_tx_pool_cfg;

}lte_cphy_dtod_pool_add_list_s;
/*! @brief
    Parameter struct for lte d cfg in conn mode
  for TX pool - scheduled
*/
typedef struct
{
  /* Tx pool cfg info*/
  lte_cphy_dtod_disc_tx_pool_list_s lted_disc_tx_pool;

  uint8 num_index_pair_list;

  lte_cphy_dtod_index_pair_s lted_index_pair[LTE_CPHY_DTOD_MAX_INDEX_PAIR_LIST];

  lte_cphy_dtod_disc_hopping_cfg_s  lted_disc_hopping_cfg;

}lte_cphy_dtod_scheduled_s;

/*! @brief
  Parameter struct for lte d cfg in conn mode
  for TX pool - ue selected
*/
typedef struct
{
 /* num of pools to release*/
  uint8 num_pool_release_list;

  /* List of pool id to be released*/
  uint8 lted_pool_release_list[LTE_CPHY_DTOD_MAX_TX_INFO_LIST];

  /* num of pools to add*/
  uint8 num_pool_add_list;

  /* List of pool id to be set up*/
  lte_cphy_dtod_pool_add_list_s lted_pool_add_list[LTE_CPHY_DTOD_MAX_TX_INFO_LIST];

}lte_cphy_dtod_ue_selected_s;


/*! @brief
  Parameter struct for lte d TX config
  received in conn mode
*/

typedef union
{
 /* Tx cfg is scheduled in conn mode*/
 lte_cphy_dtod_scheduled_s scheduled_tx_cfg;

 /* Tx cfg is UE selected in conn mode*/
 lte_cphy_dtod_ue_selected_s ue_selected_tx_cfg;

}lte_cphy_dtod_setup_u;



/*! @brief
  Parameter struct for lte d Tx/Rx info
  cfg
*/

typedef struct
{
  /*earfcn to indicate the cfg for */
  lte_earfcn_t earfcn;

  /*pci chosen for the earfcn */
  /*Change in PCI from prev cfg for same freq means,
  cfg related to prev PCI must be remvoed*/
  lte_phy_cell_id_t pci;

  /* Boolean to indicate if the above earfcn
      can do Tx*/
  boolean  is_tx_freq;

  /*For model A we need UTC time in micro sec*/
  uint32 utc_offset_model_a;

  /* num of Rx pool*/
  uint8 num_rx_pool_info;

  /*List of Rx pool cfg*/
  lte_cphy_dtod_disc_rx_pool_list_s rx_pool_info[LTE_CPHY_DTOD_MAX_RX_INFO_LIST];

  /* num of Tx pool*/
  uint8 num_tx_pool_info;

  /*List of Tx pool cfg*/
  lte_cphy_dtod_disc_tx_pool_list_s tx_pool_info[LTE_CPHY_DTOD_MAX_TX_INFO_LIST];

  /*Int to indicate the power info for LTE D TX*/
  int8 lted_discTxPowerInfo;

}lte_cphy_dtod_disc_config_s;
...
typedef struct
{
  ...
  /*! AdditionalSpectrumEmission   ::=   INTEGER   (1 .  . 32 )
      For B41 value will be 65535 if AMPR XML is present*/
  uint16 add_spectrum_emission;
...
enum
{
  ...
  LTE_CPHY_COMMON_CFG_MASK_BIT_POWER_CLASS_CFG = 0x10000,
  LTE_CPHY_COMMON_CFG_CURRENT_VALID_MASK_BITS = 0x1FFFF
};
...
typedef struct
{
  ...
  /*! AdditionalSpectrumEmission   ::=   INTEGER   (1 .  . 32 )
      For B41 value will be 65535 if AMPR XML is present*/
  uint16 add_spectrum_emission_scell;

  /*! flag which indicate if  p_max is presnt or not*/
  boolean p_max_present;
  ...
  /*! TODO:update for rel-11 Random Access Preamble timing for preamble format Range: 0-63
      Defined in 36.211 Table 5.7.1-2
      Defined in 36.311 Section 6.3.2 */
   boolean prach_cfg_scell_enabled;
...
enum
{
  ...
  LTE_CPHY_COMMON_MOBILITY_CFG_MASK_BIT_MBSFN_CFG = 0x1000,
  LTE_CPHY_COMMON_MOBILITY_CFG_MASK_BIT_MBSFN_AREAS_MCCH_SCHDL_CFG = 0x2000,
  LTE_CPHY_COMMON_MOBILITY_CFG_MASK_BIT_MBSFN_AREA_CFG = 0x4000,
  LTE_CPHY_COMMON_MOBILITY_CFG_MASK_BIT_PRACH_CFG = 0x8000,
  /*! UPDATE the value below whenever any of the above bitmasks change */
  LTE_CPHY_COMMON_MOBILITY_CFG_CURRENT_VALID_MASK_BITS = 0x7FFF
};
...
enum
{
  ...
  LTE_CPHY_DEDICATED_CFG_MASK_BIT_C_SCELL_CFG = 0x400000,
  LTE_CPHY_DEDICATED_CFG_MASK_BIT_EPDCCH_CFG = 0x800000,
  LTE_CPHY_DEDICATED_CFG_MASK_BIT_PDSCH_CFG_R11 = 0x1000000,
  LTE_CPHY_DEDICATED_CFG_MASK_BIT_ELS_DSDS_CFG = 0x2000000,
  /*! UPDATE the value below whenever any of the above bitmasks change */
  LTE_CPHY_DEDICATED_CFG_CURRENT_VALID_MASK_BITS = 0x1FFFFFF
} ;
...
enum
{
  ...
  LTE_CPHY_DEDICATED_SCELL_DL_CFG_MASK_BIT_HIGHER_MCS_CFG          = 0x0010,
  LTE_CPHY_DEDICATED_SCELL_DL_CFG_MASK_BIT_EPDCCH_CFG              = 0x0020,
  LTE_CPHY_DEDICATED_SCELL_DL_CFG_MASK_BIT_PDSCH_R11_CFG           = 0x0040,
...
typedef struct
{
  ...
  /*! Flag to indicate if Higher MCS Cfg is present or not */
  boolean higher_mcs_cfg_present;

  /*!Higher MCS Cfg */
  lte_l1_cqi_rel12_param_s higher_mcs_cfg;

  #ifdef FEATURE_LTE_EPDCCH
  /*! Parameters required to configure EPDCCH */
...
typedef struct {
  ...
  */
  ...
  /*L1 to look at p_srs_offset_Ap only when p_srs_offset_Ap_enabled is set to TRUE,
    if set to false ML1 should delete this field*/
  boolean p_srs_offset_Ap_enabled;
  /*! pSRS offset 0..15 */
  lte_l1_ul_power_ctrl_psrs_offsetAp_t p_srs_offset_Ap;
  ...
  /* ! deltaF-PUCCH-FormatX
      Parameter: ?F_PUCCH (F) for the PUCCH formats 1, 1b, 2, 2a, 2b, 3 and 1b
      with channel selection. See TS 36.213
      [23, 5.1.2] where deltaF-2 corresponds to -2 dB, deltaF0 corresponds to
      0 dB and so on. */
  lte_l1_delta_txd_pucch_format1bcs_e delta_txd_offset_fmt_1bcs;
...
typedef struct
{
  ...
  /*This flag tells whether PUSCH-ConfigDedicated-v1130 is being setup or release...TURE -> setup else release*/
  boolean dmrs_r11_enable;

  /* Below param are valid only if above flag is set to true, else ML1 shall reset these param if any */
  uint16 npusch_id_r11;
  uint16 ndmrs_csh_id;
...
enum
{
  ...
  LTE_CPHY_DEDICATED_SCELL_UL_CFG_MASK_BIT_STAG_CFG                = 0x0020
} ;
...
typedef struct
{
  ...
  /*! stag-Id --Cond ScellAdd
      Indicates the TAG of an SCell, see TS 36.321 [6]. If the field is not
      configured for an SCell (e.g. absent in MACMainConfigSCell),
      the SCell is part of the PTAG. 1...3
  */
  uint8 stag_id;
...
/*!
  @brief
  An enum describing the bit positions in the lte_cphy_c_scell_cfg_mask_t
*/
enum
{
  LTE_CPHY_CSCELL_CFG_MASK_BIT_COMMON_DL_CFG = 0x0001,
  LTE_CPHY_CSCELL_CFG_MASK_BIT_MBSFN_AREAS_MCCH_SCHDL_CFG = 0x0002,
  LTE_CPHY_CSCELL_CFG_MASK_BIT_MBSFN_AREA_CFG = 0x0004,
  LTE_CPHY_CSCELL_CFG_CURRENT_VALID_MASK_BITS = 0xFFFF
} ;

typedef uint32 lte_cphy_c_scell_cfg_mask_t;

/*!
  @brief
  SCell DL config parameters from including SIB2 channel configuration
  and eMBMS configuration
*/
typedef struct
{
  /*! Valid SCell ID ranges from 1-32; 0 is invalid and used for special cases */
  uint8 scell_id;

  /*! Irrespective of the valid Scell Id or invalid SCell Id; Scell info must always be valid */
  lte_cphy_cell_info_s c_scell_id_cell_info;

  /*! mask to indicate that which of the following three cfg is being modified */
  lte_cphy_c_scell_cfg_mask_t cfg_mask;

  /*! SCell config parameters from SIB2, will be included only if CA is not configured */
  lte_cphy_common_scell_dl_cfg_s c_scell_dl_cfg;
...
} lte_cphy_c_scell_dl_cfg_s;

/*!
  @brief
  Complete set of scell configuration for all configured SCells
*/
typedef struct
{
  /*! Bitmask which indicates which Scell config have been deleted SCellIndex-r10   ::=   INTEGER   (1 .  . 7 ).
  ...
  /*! Indicates the SCells to be released */
  uint32 c_scell_id_delete_bitmask;

  /*! Indicates number of entries in c_scell_id_remap_cfg_list valid*/
  uint8 num_c_scell_remap_cfg;

  /*! SCell ID remapped <from value bits 9-16> <to value bits 1-8>
  For e.g c_scell_id_remap_cfg_list[0] = 0x801, means scell id is remapped from 08 to 01 */
  uint16 c_scell_id_remap_cfg_list[LTE_CPHY_MAX_C_SCELL];

  /*! Indicates number of entries in scell_add_mod_list valid*/
  uint8 num_c_scell_add_mod_list;

  /*! Parameters required to bring up a configured Scell's */
  lte_cphy_c_scell_dl_cfg_s c_scell_dl_cfg_list[LTE_CPHY_MAX_C_SCELL];

} lte_cphy_c_scell_cfg_param_s;

/* ELS DSDS config type */
typedef enum
{
  LTE_CPHY_ELS_DSDS_FULL_TA = 0,
  LTE_CPHY_ELS_DSDS_PARTIAL_TA
}lte_cphy_els_dsds_config_e;


/* ELS DSDS config parameters */
typedef struct
{
  /*! Enable/disable field , the ELS DSDS enabled or disabled*/
  boolean enable;
  /*! ELS DSDS tuneaway type */
  lte_cphy_els_dsds_config_e els_dsds_config;
  /*! Indicates whether UE supports tune-away Back Indication.*/
  boolean tune_back_ind;
  /*! Indicates whether UE supports  SCell state synchronization.*/
  boolean scell_state_sync;
  /*! Indicates whether UE supports DL/UL link efficiency recovery*/
  boolean link_effic_recovery;
  /*! Indicates whether UE supports UL PUSCH Pre-Scheduling Request.*/
  boolean pusch_pre_schedule_req;
  /*! Indicates whether UE supports  UL timing sync recovery.*/
  boolean timing_sync;

} lte_cphy_els_dsds_config_s;
...
typedef struct
{
  ...
  /* Configured SCell(s) configuration parameters */
  lte_cphy_c_scell_cfg_param_s c_scell_cfg_param;
  ...
  lte_cphy_pdsch_cfg_dedicated_param_v1130_s pdsch_cfg_dedicated_v1130;
  #endif
  lte_cphy_els_dsds_config_s els_dsds_config;
...
enum
{
  ...
  LTE_CPHY_SIB_MASK_BIT_SIB19=0x80000,
  LTE_CPHY_SIB_MASK_VALID_BITS_MASK = 0x1FFFF
};

/*! @brief Mask used to represent SI-MSG bits */
#define LTE_CPHY_VALID_SIMSG_BITS_MASK (LTE_CPHY_SIB_MASK_VALID_BITS_MASK & \
...
typedef enum
{
  ...
  LTE_CPHY_RESEL_CANCEL_CAUSE_MPLMN_SEARCH,
  LTE_CPHY_RESEL_CANCEL_CAUSE_EM_CALL_NOT_ALLOWED,
  LTE_CPHY_RESEL_CANCEL_CAUSE_PAGE_MERGE_ONOING,
...
typedef enum
{
  ...
  LTE_CPHY_CELL_BAR_BAD_CELL,
  LTE_CPHY_CELL_BAR_DEPRIORITIZE, //Soft Barring
  LTE_CPHY_CELL_BAR_SSGCCS //SSCGCCS spl cause
...
typedef struct
{
  ...
  */
  ...
  /*! Indicates whether the neighbor frequency is combinable with active C-SCell or not
  Default is FALSE, and ML1 should use this field only when C-SCell is active and can assume the
  freq to be non-combinable until RRC states otherwise */
  boolean is_combinable_with_cscell;
...
typedef struct
{
  ...
  /*! Record the periodical report is for IRAT or not */
  boolean is_irat_report;
...
/*! @defgroup cphy_sap Control Plane (CPHY) Interface
  ...
  @detail The umids for this module are defined in @ref umids in the
          ...
          This interface is generated from
          $Header: //components/rel/lte.mpss/6.6.1/api/lte_cphy_msg.h#6 $ //source/qcom/qct/modem/lte/ML1/l1_common/main/latest/inc/lte_cphy_sap.h#46 $
...
typedef enum
{
  ...
  LTE_CPHY_START_REQ_SSIM_RESEL_FROM_DO_TO_LTE_STARTED        = 9,
    /*! ML1 use it to get the lock with ACQ_INV for IRAT PLMN SEARCH to LTE */
  LTE_CPHY_START_REQ_PLMN_SRCH_HIGH_PRIORITY_TO_LTE_STARTED   = 10,
  /*! Initial Service cause to notify if this is the first scan/acq/camp after LPM/Power Off
      Need to use ACQ_INV priority when this cause is SET */
  LTE_CPHY_START_REQ_AFTER_POWER_OFF_OR_LPM       = 11,
  LTE_CPHY_START_REQ_DO_TO_LTE_THREE_TECH_STARTED             = 12
...
typedef struct
{
  ...
  /*! UE Category on DL and UL cat if ul_ue_cat is set to FF*/
  uint8   ue_cat;

  /*! UE Category on UL, will be set to 0xFF if not applicable*/
  uint8   ul_ue_cat;
 ...
 /*!< Bit mask to indicate UE features like 256 QAM/4L MIMO etc
            Bit 0 (LSB) corresponds to whether 256 QAM is supported or not*/
  uint8 ue_feature_mask;
...
typedef struct
{
  ...
  /* Indicate DL earfcn is dummy or not */
  boolean  dl_earfcn_valid;

  /* LTE earfcn for ML1 for TRM reservation */
  lte_earfcn_t    earfcn;

  boolean  ul_earfcn_valid;

  /* LTE UL earfcn for ML1 for TRM reservation */
  ...
  /* ! UE Max capability, reference lte_rrc_multimode_subs_chgd_ind_s */
  uint32 subs_capability_mask[MAX_AS_IDS];
...
/*! Status of DO2L resel/redirection*/
typedef enum
{
  LTE_CPHY_DO_TO_LTE_STATUS_UNKNOWN = 0,
  LTE_CPHY_DO_TO_LTE_STATUS_STARTED,
  LTE_CPHY_DO_TO_LTE_STATUS_ENDED 
} lte_cphy_do_to_l_status_e;

/*!DO2L started request structure*/
typedef struct 
{
  /*! Message router header */
  msgr_hdr_struct_type msgr_hdr;

  /*! Status of DO to L procedure*/
  lte_cphy_do_to_l_status_e do_to_l_status;
  
} lte_cphy_do_to_lte_status_req_s;
...
typedef struct
{
  ...
  lte_ml1_nv_event_b2_thresh_adj_info_s lte_ml1_event_b2_thresh_adj_info; /*!< B2 Thresh update via EFS read by RRC */
  lte_ml1_nv_pingpong_resel_control_s  pingpong_resel_control; /*!< Idle Mode EFS for ping pong resel ("pingpong_resel_control") */

  lte_ml1_nv_qrxlev_qqual_val_s  qrxlev_qqual_val; /*!< Idle Mode EFS for customize qrxlev and qqual ("customize_qrxlev_qqual_min") */

  lte_ml1_nv_cat_degrade_s cat_degrade;  /*!< category downgrade info via EFS ("category_degrade") */
} lte_cphy_nv_update_req_s;

/*! Sent from DS->ML1.
DS is requesting for Achieavable throughput*/
typedef struct
{
  msgr_hdr_struct_type                msgr_hdr;  /*!< message header */
} lte_cphy_achievable_tput_info_req_s;

typedef struct
{
  msgr_hdr_struct_type             msgr_hdr;        /*!< message header */
  uint32                           ul_achievable_kbps;
  uint32                           dl_achievable_kbps;
} lte_cphy_achievable_tput_info_cnf_s;

/*! Sent from L1M-GM.
L1M is requesting for UL Configurable throughput*/
typedef struct
{
  msgr_hdr_struct_type                msgr_hdr;  /*!< message header */
} lte_cphy_ul_configurable_tput_info_req_s;

typedef struct
{
  msgr_hdr_struct_type             msgr_hdr;        /*!< message header */
  uint32                           ul_configurable_kbps;
} lte_cphy_ul_configurable_tput_info_cnf_s;
...
typedef struct
{
  ...
  /* Indicate ML1 whether need to check SSS SNR for acquisition
     Currently apply to first sub-group ACQ REQ in G2L list redir after CSFB*/
  boolean snr_check_needed;
...
/*! @brief
  This message is used for indicate trigger page merge evaluate again.
*/
typedef struct
{
  /*! Message router header */
...
} lte_cphy_page_merge_reevaluate_ind_s;
...
typedef enum
{
  LTE_CPHY_RLF_DEBUG_REASON_INVALID = 0,
  LTE_CPHY_RLF_DEBUG_REASON_VRLF_RLM = 0x1,
  LTE_CPHY_RLF_DEBUG_REASON_VRLF_CP_MISMATCH = 0x2,
  LTE_CPHY_RLF_DEBUG_REASON_VRLF_PCELL_SCELL_DRIFT = 0x4,
  /*! vRLF triggered due to Scell reference time and frame boundary drift */
  LTE_CPHY_RLF_DEBUG_REASON_VRLF_SCELL_DRIFT_BEYOND_TTL_RANGE = 0x8,
  /*! vRLF triggered due to SCC tuneaway, could be DTA or LTA */
  LTE_CPHY_RLF_DEBUG_REASON_VRLF_SCC_TA = 0x10,
  LTE_CPHY_RLF_DEBUG_REASON_NORMAL = 0x20,
  LTE_CPHY_RLF_DEBUG_REASON_RESET = 0x40,
  LTE_CPHY_RLF_DEBUG_REASON_TMR = 0x80,
  LTE_CPHY_RLF_DEBUG_REASON_RESET_1   = 0x100,
  LTE_CPHY_RLF_DEBUG_REASON_RESET_2   = 0x200,
  LTE_CPHY_RLF_DEBUG_REASON_RESET_3   = 0x400,
  LTE_CPHY_RLF_DEBUG_REASON_RESET_4   = 0x800,
  LTE_CPHY_RLF_DEBUG_REASON_RESET_5   = 0x1000,
  LTE_CPHY_RLF_DEBUG_REASON_RF_UNAVAILABLE   = 0x2000,
  LTE_CPHY_RLF_DEBUG_REASON_CDRX_COLLISION   = 0x4000,
  LTE_CPHY_RLF_DEBUG_REASON_MSG4_ACK_NOT_SENT   = 0x8000,
  LTE_CPHY_RLF_DEBUG_REASON_SIB_READ_FAILURE = 0x10000, /*! SIB Read Failure from CSP module*/
  LTE_CPHY_RLF_DEBUG_REASON_RACH_PROBLEM = 0x20000, /*! RACH problem indication from MAC*/
  LTE_CPHY_RLF_DEBUG_REASON_MAX_RETRX = 0x40000, /*! Maximum Retransmissions indication from RLC*/
  LTE_CPHY_RLF_DEBUG_REASON_VRLF_RECOVERY = 0x80000000, /*! Debug IND should only be sent on VRLF recovery*/
  LTE_CPHY_RLF_DEBUG_REASON_MAX = 0xFFFFFFFF,
} lte_cphy_rlf_debug_reason_e;
...
typedef struct
{
  ...
  msgr_hdr_struct_type msgr_hdr;
  /*! RLF debug reason */
  lte_cphy_rlf_debug_reason_e  rlf_debug_reason;
} lte_cphy_rlf_debug_ind_s;
...
typedef struct
{
  ...
  /*! Whether or not the freq is combinable with PCell's freq */
  boolean is_combinable;

} lte_cphy_candidate_freq_info_s;

/*! @brief
  Strongest cell candidates for configured SCell setup
*/
typedef struct
{
  /*! No. of frequencies passed in the array below */
  uint8 num_freq;

  /*!  List of candidate frequencies to be measured */
  lte_cphy_candidate_freq_info_s candidate_freq_info_list[LTE_CPHY_MEAS_MAX_NUM_FREQ];

} lte_cphy_scell_strongest_cell_param_s;

/*! @brief
  SCell Intra-freq meas params;
  The parameter values would be determined based on S-Cell SIB-based mapping
  as suggested by Systems team
*/
typedef struct
{

  /*! Intra-freq threshold for reselection */
  int16 q_hyst; /*!< QHyst from SIB3 of SCell */

  /*! Trigger quantity type for EUTRA reports */
  lte_cphy_meas_report_cfg_eutra_trig_qty_e trig_qty; /*!< RSRP or RSRQ based */

  /*! t_resel (in sec), how long the criteria should be met before reporting; */
  uint16 t_resel_sec; /*!< t_resel from SIB3 */

  /*! Blacklisted cells from SIB4 of SCell */
  uint8 blacklist_num_cells;
  lte_cphy_meas_eutra_black_cell_s blacklist[LTE_CPHY_MEAS_MAX_CELL_BLACK];

} lte_cphy_scell_intra_freq_param_s;

/*! @brief
  SCell Inter-freq meas params;
  The parameter values would be determined based on S-Cell SIB-based mapping
  as suggested by Systems team
*/
typedef struct
{
  /*! DL frequency to search/measure */
  lte_earfcn_t frequency;

  /*! Minimum Rx level to camp on cell (-70..-22) * 2 */
  lte_l1_q_rxlevmin_t q_rxlevmin; /*!< Qrxlevmin from SIB1 of SCell */

  /*! QrxlevminOffset from SIB1 of SCell */
  lte_l1_q_rxlevmin_offset_t q_rxlevmin_offset;

  /*! Serving threshold for reselection */
  int16 serv_thresh; /*!< threshServingLow from SIB1 of SCell */

  /*! Serving threshold for reselection */
  int16 neigh_thresh; /*!< threshX-Low for the neighbor freq from SIB5 of SCell
or default thresh-Low 6 dB if SIB5 for SCell is not available */

  /*! Trigger quantity type for EUTRA reports */
  lte_cphy_meas_report_cfg_eutra_trig_qty_e trig_qty; /*!< RSRP or RSRQ based */

  /*! t_resel (in sec), how long the criteria should be met before reporting; */
  uint16 t_resel_sec; /*!< t_resel from SIB3 of SCell */

} lte_cphy_scell_inter_freq_info_s;

/*! @brief
  SCell's SIB5 inter-freq param for all neighbor freqs */
typedef struct
{
  /*! No. of frequencies passed in the array below */
  uint8 num_freq;
  /*! List of inter-freq neighbors to be measured */
 lte_cphy_scell_inter_freq_info_s inter_freq_info[LTE_CPHY_MEAS_MAX_NUM_FREQ];

} lte_cphy_scell_inter_freq_param_s;

/*!
  @brief
  An enum describing the bit positions in the lte_cphy_scell_meas_cfg_mask_t
*/
enum
{
  LTE_CPHY_SCELL_MEAS_CFG_MASK_STRONGEST_CELL_CFG  = 0x01,
  LTE_CPHY_SCELL_MEAS_CFG_MASK_INTRA_FREQ_CFG     = 0x02,
  LTE_CPHY_SCELL_MEAS_CFG_MASK_INTER_FREQ_CFG    = 0x04,
};

/*! @brief
  SCell measurements config  mask to enable the SCell meas config.
*/
typedef uint16 lte_cphy_scell_meas_cfg_mask_t;

/*! @brief
  SCell Meas config info for each configured SCell */
typedef struct
{
  /*! SCell below is valid or not; Will be FALSE for Strongest cell report */
  boolean scell_info_valid;

  lte_cphy_cell_info_s scell_info;

  lte_cphy_scell_meas_cfg_mask_t scell_meas_cfg_mask;

  lte_cphy_scell_strongest_cell_param_s strongest_cell_cfg;
  lte_cphy_scell_intra_freq_param_s intra_freq_cfg;
  lte_cphy_scell_inter_freq_param_s inter_freq_cfg;
} lte_cphy_scell_meas_cfg_info_s;

/*! @brief
  SCell meas config request parameters for all SCell(s)
*/
typedef struct
{
  /*! Message header */
  ...
  /*! Num SCell info provided */
  uint8 num_scell;

  /*! Meas config parameters for this SCell */
  lte_cphy_scell_meas_cfg_info_s scell_meas_cfg_info[LTE_CPHY_MAX_C_SCELL];

} lte_cphy_scell_meas_cfg_req_s;
...
/*! @brief
  SCell Meas Report Ind payload with the SCell measurement results
*/
typedef struct
{
  /*! Message header */
  ...
  /*! S-Cell information related to the resel/HO candidates */
  lte_cphy_cell_info_s   scell_info;

  /*! Number of frequencies with candidates in the below array */
  uint8 num_freqs;

  /*! List of candidates per frequency */
  lte_cphy_cell_info_s   target_cell_info[LTE_CPHY_MEAS_MAX_NUM_FREQ];

  /*! The time at which this indication is sent */
  lte_ref_time_s  ref_time;

} lte_cphy_scell_meas_report_ind_s;
...
typedef struct
{
  ...
  /* Dedicated configuration parameters applicable for the PCell */
...
typedef struct
{
  ...
  /*! A bit-mask specifying the per-subconfiguration validity of the
    eMBMS Scell configuration request

    Bit turned on means configuration was successful. */
  lte_cphy_c_scell_cfg_mask_t scell_cfg_validity;
...
typedef struct
{
  ...
  /*! A bit-mask specifying the per-subconfiguration validity of the
    ...
    Bit turned on means configuration was successful. */
  lte_cphy_c_scell_cfg_mask_t scell_param_validity;
...
typedef struct
{
  ...
  /*! CSG cell */
  boolean csg_cell;
  /*! if this flag sets mL1 do PRx only aqcusition first and if fails will do Prx+DRx*/
  boolean             use_prx_only;
  /*! If this flag sets TRUE, Ml1 can report rssi ind, else cannot report  */
  boolean    report_rssi_allowed;
...
/*! @brief
  This msg is sent by RRC to ML1 for LTE D configuration
  recevied from SIB19
 */
typedef struct
{
 ...
 /* Indicates if action is to cfg or de cfg*/
 lte_l1_config_action_e action;

 /*! Enum to indicate the cfg model */
 lte_cphy_dtod_cfg_model_e cfg_model;

 /*!
 MIB params like BW from EFS/Canned MIB for Model A
 The field is not populated for Model B/C
 In the absence of this info from EFS, RRC will provide 20MHz
 */
 lte_bandwidth_e dl_bw_for_model_a;

 /*! The num of cgf list */
 uint8 num_cfg;

 /*! Pointer to discovery cfg for Rx/Tx */
 lte_cphy_dtod_disc_config_s *disc_cfg_ptr;

 /*! List of earfcn Ml1 has to monitor */
 lte_cphy_dtod_disc_inter_freq_list_s earfcn_list;

}lte_cphy_dtod_cfg_req_s;
...
}lte_cphy_dtod_cfg_cnf_s;

/*! @brief
  This msg is sent by RRC to ML1 to configure LTE D TX in conn mode for TX.
...
typedef struct
{
 ...
 /*! Mask of the commands to be configured */
 lte_cphy_dtd_cfg_mask_t cfg_mask;

  /* Indcates if action is to cfg or de cfg*/
 lte_l1_config_action_e action;

 /*! Enum to indicate if Tx Pool needs to be released
      or set up
      If release dont look into further structure*/
 lte_cphy_dtod_tx_cfg_e dedicated_tx_cfg;

 /*! Enum to indicate if Tx pool is scheduled or
      UE selected*/
 lte_cphy_dtod_ded_setup_e setup_choice;

 /* Based on the above selection choose the
     set up within the union*/
 lte_cphy_dtod_setup_u setup;

  /*Int to indicate the power info for LTE D TX*/
  int8 lted_discTxPowerInfo;

}lte_cphy_dtod_ded_cfg_req_s;
...
typedef struct
{
  ...
  boolean is_lted_cell_found;

}lte_cphy_dtod_cell_detect_ind_s;

/*! @brief
    Data Structure for LTE_CPHY_LTED_CELL_SEL_REQ

    @details
       RRC uses this command to:
         a) stop/start SIBs for the inter-freq LTED Cell
         b) SI scheduling info/TDD Cfg
         c) bar the cell/freq in the upcoming passes
*/
typedef struct
{
  msgr_hdr_struct_type             msgr_hdr;
  ...
  */
  ...
  /*! TDD cfg - applicable only if its TDD freq */
  lte_cphy_tdd_cfg_param_s        tdd_cfg;

} lte_cphy_lted_cell_sel_req_s;
...
typedef struct
{
  ...
  boolean is_lted_scan_done;

}lte_cphy_dtod_scan_ind_s;
...
typedef struct
{
  ...
  lte_earfcn_t         freq;
}lte_cphy_lted_resume_ind_s;

/*! @brief
    Enum for different SCell cancel causes
 */
typedef enum
{
  /*! No use case for this cause for now */
  LTE_CPHY_SCELL_CANCEL_CAUSE_NONE,
  /*! when RRC is not ok with the SIBs rcvd for the SCell; e.g.: it didnt find embms cov */
  LTE_CPHY_SCELL_CANCEL_CAUSE_SIB_READ_FAILURE,
  /*! when RRC SIB timeout happens, it didnt receive the necessary SIBs */
  LTE_CPHY_SCELL_CANCEL_CAUSE_SIB_READ_TIMEOUT,
  /*! RRC doesnt want the SCell anymore (general) */
  LTE_CPHY_SCELL_CANCEL_CAUSE_SCELL_ABORTED,
  /*! RRC doesnt want the target SCell for mobility, will resume back on source SCell
  prev_cell_info and curr_cell_info will be different for this cause code */
  LTE_CPHY_SCELL_CANCEL_CAUSE_SCELL_MOBILITY,
  /*! No use case for this cause for now */
  LTE_CPHY_SCELL_CANCEL_CAUSE_MAX
} lte_cphy_scell_cancel_cause_e;

/*! @brief
    SCell cancel request to indicate RRC is not going ahead with
    setup of the SCell
 */
typedef struct
{
  ...
  /*! scell info to fall back after cancellation (optional) */
  lte_cphy_cell_info_s           prev_cell_info;

  /*! scell info to cancel */
  lte_cphy_cell_info_s           curr_cell_info;

  /*! scell cancel cause */
  lte_cphy_scell_cancel_cause_e  cancel_cause;
} lte_cphy_scell_cancel_req_s;
...
/*! @brief
  this type lists the PDCCH order requests possible
*/
typedef enum
{
  /*! Start Req */
  LTE_CPHY_PDCCH_ORDER_RACH_START,
  /*! Stop Req */
  LTE_CPHY_PDCCH_ORDER_RACH_STOP
} lte_cphy_pdcch_order_req_type_e;
...
typedef struct
{
  ...
  /*! Request Type */
  lte_cphy_pdcch_order_req_type_e req;
...
} lte_cphy_configured_tput_info_ind_s;

typedef struct
{
  msgr_hdr_struct_type                msgr_hdr;         /*!< message header */
  uint32                              tmeas_ms;        /*! 50 ms - 500 ms*/
  uint16                              frequency;       /*! (Alpha = 1 / frequency)*/
} lte_cphy_achievable_tput_info_start_ind_s;

 typedef struct
 {
   msgr_hdr_struct_type                msgr_hdr;         /*!< message header */
 } lte_cphy_achievable_tput_info_stop_ind_s;
...
typedef struct
{
  ...
  /*! indicates that if this cell reselection is because of cell merge. */
  boolean is_for_cell_merge;
#endif
...
/*! @brief
    An enum describing RaPID offset compared to preambleID
*/
typedef enum
{
  LTE_CPHY_RA_PID_OFF_MINUS1 = 0,
  LTE_CPHY_RA_PID_OFF_MINUS2,
  LTE_CPHY_RA_PID_OFF_MAX
} lte_cphy_ra_pid_off_type_e;
...
typedef struct
{
  ...
  /* RaPID offset compared to preamble ID */
  lte_cphy_ra_pid_off_type_e ra_pid_off;

  /* TA value to be used. This value is not used in order to simplify the design */
  int16 ta_adjust_val;

} lte_cphy_ra_pid_off_ind_s;
...
typedef struct
{
  ...
  /* Octet of bits indicating active/inactive status for 16 SCells (8 C-SCELL)  */
  uint16                     scell_status_mask;
...
typedef struct
{
  ...
  /** Frequency of the cell; used with phy cell ID to identify
   *  the cell where SIBs are scheduled */
  lte_earfcn_t               freq;
  ...
  /** Previous Cell info whose freq/pci will be used to identify
   *  the component carrier to be used for SIB reading */
  lte_cphy_cell_info_s       prev_cell_info;
  ...
  */
  ...
  /** Context Specific SIB bitmask to determine which SIBs are high priority 
  case 1: It is used to indicate SIB2 is high priority when SIB2 collides with meas GAP */
  lte_cphy_sib_mask_t         priority_sib_mask;
...
typedef enum
{
  ...
  LTE_CPHY_IRAT_CGI_NOOP,                /*!CGI aborted at ML1 due to some higher priority request*/
...
typedef enum
{
  ...
  LTE_TRM_PRIORITY_ACQ_INV,     /*!< 3 */
  LTE_TRM_PRIORITY_CHAN_MAIN,  /*!< 4 */
  LTE_TRM_PRIORITY_SIGNALING,  /*!< 5 */
  LTE_TRM_PRIORITY_ACCESS,     /*!< 6*/
  LTE_TRM_PRIORITY_CHAN_MAIN_INV,  /*!< 7 */
  LTE_TRM_PRIORITY_HIGH        /*!< 8 */
} lte_trm_priority_e;
...
typedef enum
{
  LTE_CPHY_TUNEAWAY_NONE, /*! RF_UNAVAIL because of rfless wakeup */
  LTE_CPHY_TUNEAWAY_LTA,
  LTE_CPHY_TUNEAWAY_QTA,
  LTE_CPHY_TUNEAWAY_DTA,
  LTE_CPHY_TUNEAWAY_AGAP,
  LTE_CPHY_TUNEAWAY_LOCK_WAIT
} lte_cphy_tuneaway_t;
...
typedef struct
{
  ...
  /*! Chain mask: Chains that are impacted by tuneaway */
  uint8                             chain_mask[LTE_CPHY_DL_NUM_CARRIERS];

  /*! Indicates if more paging occasions available in current sib modification
      period. */
  boolean                           po_avail_in_curr_sib_mod_per;
...
typedef struct
{
  ...
  /*! [SFN|SUBF] value for actual tuneaway start. Range:[0 ... 10240].
      The value is valid only for QTA/LTA/DTA cases.  */
  uint16                            ta_start_time;
...
typedef struct
{
 ...
 /*Indicates LTE Procedure for which TRM priority req is sent to ML1*/
 sys_proc_type_e_type trm_procedure;
...
typedef struct
{
  ...
  /*! TRUE: use invert acq priority; FALSE: use acq priority */
  boolean                        inv_priority;
...
typedef enum
{
  LTE_CRS_RX_CHAIN_NO_COMB = 0,
  LTE_CRS_RX_CHAIN_WEIGHTED_NC_COMB,
  LTE_CRS_RX_CHAIN_WEIGHTED_COH_COMB,
  LTE_CRS_RX_CHAIN_MAX
} lte_crs_rx_chain_comb_mode_e;

typedef enum
{
  LTE_CRS_ML1_STATE_NO_SERVICE = 0,
  LTE_CRS_ML1_STATE_IDLE_FW_OFFLINE,
  LTE_CRS_ML1_STATE_IDLE_FW_ONLINE,
  LTE_CRS_ML1_STATE_CONN_NORMAL,
  LTE_CRS_ML1_STATE_CONN_CDRX,
  LTE_CRS_ML1_STATE_MAX
} lte_crs_ml1_lte_state_e;

#define LTE_CPHY_CRS_MAX_NGBR_CELLS 10
#define LTE_CPHY_CRS_MAX_EARFCNS    6

/*! LTE CRS cell structure */
typedef struct
{
  /*! Physical cell id */
  lte_phy_cell_id_t phy_cell_id;

  /*! Which Tx antenna that we need to measure */
  ...
  lte_crs_rx_chain_comb_mode_e ue_rx_chain_comb_mode;
  ...
  /*! Prune Size in Ts [0-2048] - default 2048 */
  uint16 prune_win_size;

  /*! symbol delay offset. - default 0 */
  int8 symbol_offset_index;

  /*! Number of consecutive measurements required */
  /* Default this to 1 for now */
  uint8 NumOfConsecutiveMeas;
} lte_cphy_crs_cell_info_s;

/*! LTE CRS assistance data per EARFCN */
typedef struct lte_cphy_crs_earfcn_cell_list_s
{
  /*! E-ARFCN */
  lte_earfcn_t earfcn;
  ...
  /*! List of cells to be measured */
  /* Understanding between ML1 and LocSW is that first
   cell in the array will always be serving cell */
  lte_cphy_crs_cell_info_s ngbrs[LTE_CPHY_CRS_MAX_NGBR_CELLS];

}lte_cphy_crs_earfcn_cell_list_s;

/*! LTE CRS update cells request structure */
typedef struct lte_cphy_crs_meas_req_s
{
  /* TODO: Add the msg handler struct from LOC SW
     This will be used to match the request to the response */
  /*! Number of earfcns */
  uint8 num_earfcns;

  /* Add Searcher neighbor list to this list for measurement */
  boolean add_found_neighbors;

  /* Priority of the added Searcher neighbor list relative to this list for measurement */
  boolean found_neighbors_priority; /* TRUE means searcher list is priority over this list.*/

  /*! List of earfcn cell lists */
  lte_cphy_crs_earfcn_cell_list_s earfcns[LTE_CPHY_CRS_MAX_EARFCNS];

} lte_cphy_crs_meas_req_s;
...
typedef struct
{
  ...
  /* the selected SR occasion offset w.r.t. CDRX cycle start time +
   * worst case sleep module wake up time + UL processing time) in ms  */
  uint32 tx_ref_offset;

  /* --------All below params are reported in Micro secs resolution---------- */
  /* UPO_CCST - Uplink Packet Offset wrt CDRX Cycle start time.*/
  uint32 ul_pkt_offset_with_cdrx_start;

  /*fixed at 3ms for current implementation*/
  uint16 dl_processing_time;

  /* Latency for packet from RTP to reach L2 buffer */
  uint16 ul_processing_time;

  /*on duration timer value in ms*/   
  uint32 on_duration_timer_val;

  /* The CDRX cycle lenght in ms, 0 indicates CDRX not configured */
  uint16 cdrx_cycle_len_ms;

  /* Delta between CDRX on duration end to the 
     point where IMS IND is sent*/
  uint16 delta_4;
  ...
  /* TRUE (SPS-like activated) or FALSE (SPS-like deactivate) */
  lte_cphy_cdrx_sps_type_e sps_like_act;

  /*-----------Retaining old variables(HI.2.0 specific) to avoid 2 way dep with IMS---------*/
  /* Should be removed after HI.4.0 is branched off.*/
  ...
  /* The CDRX cycle lenght in ms, 0 indicates CDRX not configured */
  uint32 cdrx_cycle_len_us;
}lte_cphy_cdrx_pkt_timing_ind_s;

#ifdef FEATURE_LTE_IMS_RTP_BLER_EVAL
/* Data collection for each decision cycle (500ms) */
typedef struct
{
  uint16                    ul_new_tx_cnt;
  uint16                    ul_retx_cnt;
  uint16                    ul_max_retx_cnt;
}lte_cphy_ims_rtp_tx_buff_s;

/* BLER calculation for a window 5/10sec */
typedef struct
{
  lte_cphy_ims_rtp_tx_buff_s ul_tx_buff[LTE_ML1_IMS_RTP_BLER_EVAL_BUF];
  uint16                     total_ul_retx;
  uint16                     total_ul_tx;
  int16                      total_ul_bler;
}lte_cphy_ims_rtp_bler_db_s;

/*! @brief
    Store values sent from IMS to ML1 under IMS_RTP_AUDIO_RA_BLER_REQ
    or IMS_RTP_DSDA_FB_BLER_REQ */
typedef struct
{
  lte_cphy_ims_rtp_bler_db_s ul_bler_db;
  int16                      bler_thresh;
  uint16                     eval_win_cycles;
  uint8                      bler_dir;
  uint8                      rat;
  uint8                      eval_type;
  uint8                      switch_type;
  uint8                      iter_count;
  boolean                    ims_rtp_ind_pending;
  boolean                    sliding_win_on;
}lte_cphy_ims_rtp_bler_req_msg_s;
#endif /* FEATURE_LTE_IMS_RTP_BLER_EVAL */
...
/******************************************************************************
                            LTE ELS related msg structs
*******************************************************************************/
enum
{
  MSGR_DEFINE_UMID(LTE, CPHY, IND, ELS_MCE_TRIGGER, LTE_CPHY_ELS_MCE_TRIGGER_ID,
                   lte_cphy_els_mce_trigger_ind_s ),

  MSGR_DEFINE_UMID(LTE, CPHY, IND, ELS_MCE_CANCEL, LTE_CPHY_ELS_MCE_CANCEL_ID,
                   lte_cphy_els_mce_cancel_ind_s),

  MSGR_DEFINE_UMID(LTE, CPHY, IND, ELS_MCE_NACK, LTE_CPHY_ELS_MCE_NACK_ID,
                   lte_cphy_els_mce_nack_ind_s),
};

/*! @brief defined maximal length allowed for partial rx tx cap payload,
maximal length should be 32 cells * 2 bytes = 64 bytes
*/
#define LTE_CPHY_ELS_PARTIAL_RX_TX_CAP_MAX_LEN                                64

/*! @brief defined maximal length allowed for UL/DL efficiency recovery payload,
maximal length should be 32 cells * 3 bytes = 96 bytes
*/
#define LTE_CPHY_ELS_UL_DL_LINK_RECOVERY_MAX_LEN                              96

/*! @brief defined maximal length allowed for scell state payload
*/
#define LTE_CPHY_ELS_SCELL_STATE_PAYLOAD_MAX_LEN                              4


/*! @brief this enum defines the ELS MCE type
*/
typedef enum
{
  LTE_CPHY_ELS_GAP_REG              = 0,
  LTE_CPHY_ELS_PUSCH_PRE_SCHEDULING = 1,
  LTE_CPHY_ELS_PERIOD_GAP_DEREG     = 2,
  LTE_CPHY_ELS_TUNEBACK_IND         = 3,
  LTE_CPHY_ELS_UL_DL_LINK_RECOVERY  = 4,
  LTE_CPHY_ELS_SCELL_STATE_SYNC     = 5,
  LTE_CPHY_ELS_UL_TIMING_SYNC       = 6,
  /*! This LTE_CPHY_ELS_APERIOD_GAP_REG is only for MAC internal usage */
  LTE_CPHY_ELS_APERIOD_GAP_REG      = 7,
  LTE_CPHY_ELS_MCE_TYPE_MAX         = 8
} lte_cphy_els_mce_type_e;

/*! @brief Define the structure to provide the information for periodic/aperiodic
gap registration */
typedef struct
{
  /*! The IDX field is the DRX group index of the concurrent RAT.
  Assuming the current LTE RAT is the first RAT */
  uint8                       rat_index;
  /*! Indicate if this is for periodic or aperiodic.
  1 means aperiodic. Only valid when type is
  LTE_CPHY_PERIOD_APERIOD_GAP_REGISTRATION */
  uint8                       is_aperiod;
  /*! Indicate if full tune mode or partial mode, 1 means full tune mode */
  uint8                       is_full_tune_mode;
  /*! The U field is a flag indicating update of one existing periodical
  gap registration indexed by RAT index. Invalid when gap is aperiodic */
  uint8                       u_field;
  /*! System Frame number */
  lte_l1_sfn_t                sys_fn;
  /*! Subframe number */
  lte_l1_subfn_t              sub_fn;
  /*! Drx cycle length index, Invalid when gap is aperiodic*/
  uint8                       drx_cycle_index;
  /*! Typical gap duration length in ms unit, Invalid when gap is aperiodic*/
  uint8                       typical_gap_len;
  /*! Maximum aperiodical gap length, Invalid when gap is periodic*/
  uint8                       max_gap_len;
  /*! Length of Partial tune Rx/Tx CAP payload */
  uint8                       payload_len;
  /*! Payload content of Partial tune Rx/Tx CAP*/
  uint8                       payload[LTE_CPHY_ELS_PARTIAL_RX_TX_CAP_MAX_LEN];
} lte_cphy_els_gap_reg_s;

/*! @brief Define the structure to provide the information for pusch pre scheduling */
typedef struct
{
  /*! The IDX field is the DRX group index of the concurrent RAT.
  Assuming the current LTE RAT is the first RAT */
  uint8                       rat_index;
  /*! The CAL field is a flag indicating if PUSCH pre-scheduling is requested
  or released. It's set to "0" for PUSCH pre-scheduling request and "1" for
  PUSCH pre-schedulin release.--Only valid when type is PUSCH pre scheduling */
  uint8                       cal_field;
  /*! The Schedule Advance field contains the preferred schedule advance in ms
  before the start time of periodical gap. This field is reported only when
  CAL filed is set to "0". */
  uint8                       schedule_advance;
  /*! This field contains the preferred UL grant size in byte before the start
  time of periodical gap.  This field is reported only when CAL filed is set to "0"*/
  uint8                       schedule_grant_size;
} lte_cphy_els_pusch_pre_scheduling_s;

/*! @brief Define the structure to provide the information for periodic
gap deregistration */
typedef struct
{
  /*! The IDX field is the DRX group index of the concurrent RAT.
  ...
  1 means aperiodic. */
  uint8                       is_aperiod;
} lte_cphy_els_period_gap_dereg_s;

/*! @brief Define the structure to provide the information for tuneback indication */
typedef struct
{
  /*! Indicate if this is for periodic or aperiodic.
  1 means aperiodic. */
  uint8                       is_aperiod;
} lte_cphy_els_tuneback_ind_s;

/*! @brief Define the structure to provide the information for UL/DL efficiency recovery */
typedef struct
{
  /*! Length of UL/DL efficiency recovery payload */
  uint8                       payload_len;
  /*! Payload content of UL/DL efficiency recovery */
  uint8                       payload[LTE_CPHY_ELS_UL_DL_LINK_RECOVERY_MAX_LEN];
} lte_cphy_els_ul_dl_link_recovery_s;

/*! @brief Define the structure to provide the information for  scell state synchronization */
typedef struct
{
  /*! Indicate if Ci(i = 31:8) field is included or not,
  It's set to "1" if there is any configured cell with ServCellIndex larger than 7;
  otherwise, it's set to "0".*/
  uint8                       rel_field;
  /*! Length of UL/DL efficiency recovery payload */
  uint8                       payload_len;
  /*! Payload content of scell state */
  uint8                       payload[LTE_CPHY_ELS_SCELL_STATE_PAYLOAD_MAX_LEN];
} lte_cphy_els_scell_state_sync_s;
...
typedef struct
{
  ...
  uint8                       tag_mask;
} lte_cphy_els_ul_timing_sync_s;

/*! @brief Union of all type of MAC MCE for ELS */
typedef union
{
  /*! Information required for periodic/aperiodic gap registration */
  lte_cphy_els_gap_reg_s                        gap_reg;
  /*! Information required for pusch pre scheduling  */
  lte_cphy_els_pusch_pre_scheduling_s           pusch_pre_scheduling;
  /*! Information required for periodic gap deregistration */
  lte_cphy_els_period_gap_dereg_s               period_gap_dereg;
  /*! Information required for tuneback indication */
  lte_cphy_els_tuneback_ind_s                   tuneback_ind;
  /*! Information required for UL/DL efficiency recovery */
  lte_cphy_els_ul_dl_link_recovery_s            ul_dl_link_recovery;
  /*! Information required for scell state synchronization */
  lte_cphy_els_scell_state_sync_s               scell_state_sync;
  /*! Information required for UL timing recovery */
  lte_cphy_els_ul_timing_sync_s                 ul_timing_sync;
} lte_cphy_els_mce_u;

/*! @brief define the msg send from ML1 to MAC to trigger building ELS MCE */
typedef struct
{
  msgr_hdr_struct_type        msg_hdr;  /*!< Message header */
  /*! the message payload is the pac info to add */
  /*! ELS MAC type */
  lte_cphy_els_mce_type_e     mce_type;
  /*! Priority for this MCE, range from 0 to 2, which maps to DSM_NORMAL = 0,
  DSM_HIGHEST = 1, DSM_PRIORITY_ENQ_HIGHEST = 2 */
  uint8                       priority;
  /*! information for constructing els mce */
  lte_cphy_els_mce_u          els_mce;
  /*! inform MAC if indication is needed when reTx failure */
  boolean                 reTx_ind;
  /*! winning client trm reason for p/ap gap reg only*/
  uint8                   trm_winning_reason;
} lte_cphy_els_mce_trigger_ind_s;

/*! @brief define the msg send from ML1 to MAC to cancal pending aperiodic MCE */
typedef struct
{
  msgr_hdr_struct_type        msg_hdr;  /*!< Message header */
} lte_cphy_els_mce_cancel_ind_s;

/*! @brief define the msg send from MAC to ML1 to notify periodic gap re-registration */
typedef struct
{
  msgr_hdr_struct_type        msg_hdr;  /*!< Message header */
  /*! The IDX field is the DRX group index of the concurrent RAT.
...
typedef msgr_hdr_struct_type lte_cphy_ttl_cor_ind_s;

/******************************************************************************
                                  ML1 -> MAC interface
*******************************************************************************/
/*  Use the union for payload and issue the command to use */

/*! @brief  Enum to indicate the request type to MAC */
typedef enum
{
  LTE_ML1_MAC_TAG_OOS_IND,
} lte_cphy_ml1_mac_msgr_msg_enum;


/*! @brief Inform MAC that the uplink has gone out of range for a SCELL */
typedef struct
{
  /*! STAG-Id-r11    Range (1..maxSTAG-r11). Cannot be 0 (PTAG)*/
  uint8  stag_id;

} lte_cphy_ml1_mac_out_of_sync_req_s;

typedef union
{
  lte_cphy_ml1_mac_out_of_sync_req_s oos_req;
}lte_cphy_ml1_mac_msgr_msg_u;
...
typedef struct
{
  ...
  /*! The request type */
  lte_cphy_ml1_mac_msgr_msg_enum req;

  /*! Payload, depends on the request type */
  lte_cphy_ml1_mac_msgr_msg_u    payload;

} lte_cphy_ml1_mac_msgr_msg_s;


/*! @brief
      Data Structure for LTE_CPHY_OOS_WARNING_REQ

    @details
      REQ sent from LTE RRC to ML1 to issue OOS warning for a quick reselection
 */
typedef struct
{
  /*!< message header */
   msgr_hdr_struct_type              msgr_hdr;
}lte_cphy_oos_warning_req_s;


  typedef enum
  {
    //invalid status
    LTE_ML1_QIPCALL_CALL_STATE_NULL = 0,
	//initiating a volte call
    LTE_ML1_QIPCALL_CALL_STATE_INITIATION,
	//received message183
    LTE_ML1_QIPCALL_CALL_STATE_SESSION_IN_PROGRESS,
	//received alerting
    LTE_ML1_QIPCALL_CALL_STATE_ALERTING,
	//call connected
    LTE_ML1_QIPCALL_CALL_STATE_CONNECTED,
	//call ended
    LTE_ML1_QIPCALL_CALL_STATE_END,
    LTE_ML1_QIPCALL_CALL_STATE_MAX = 0x7FFFFFFF
  }lte_ims_qipcall_call_state_to_lte_e;

  typedef struct
  {
    lte_ims_qipcall_call_state_to_lte_e qpCallState;
  }lte_ims_qpCallStatusToLte_s;

typedef struct
{
  /*! Message router header */
  msgr_hdr_struct_type   msgr_hdr;
  //current call is a security call
  boolean qpbIsSecurity;
  //judge if the call is MO or MT
  boolean qpbIsIncoming;
  lte_ims_qpCallStatusToLte_s                 qpcall_status;
  /*Flag to indicate whether block bSRVCC, block:1, unblock:0*/
  boolean bSRVCCIsBlocking;

} lte_ims_qpcall_status_ind_s;
...
typedef struct
{
  ...
  msgr_hdr_struct_type   msgr_hdr;
  //true: yes, false: no
  boolean is_mobility_to_wifi_possible;
  //always true for now since it applies to srvcc use cases; possible extension in the future to idle  mode if requirements arise
  boolean  is_ims_in_call; 
} lte_ims_wifi_mobility_status_s;


typedef enum
{
  //invalid status
    LTE_ML1_SRVCC_STATUS_INVALID = 0,
  //Entered SRVCC
    LTE_ML1_SRVCC_STATUS_ENTER,
  //Exit SRVCC
    LTE_ML1_SRVCC_STATUS_EXIT,
    LTE_ML1_SRVCC_STATUS_MAX = 0xFF
} lte_srvcc_status;
...
typedef struct
{
  ...
  msgr_hdr_struct_type   msgr_hdr;
  //ref enum def above
  lte_srvcc_status srvcc_status;
  boolean reserved1;
} lte_ims_srvcc_status_s;
...
typedef struct
{
  /*! Message router header */
  msgr_hdr_struct_type   msgr_hdr;
  /* Flag from RRC to enable/disable optimization, based on current cell's KPI */
  boolean kpi_based_adjust_enabled;
  /* NV setting A2/B2 thres/offset */
  int32   kpi_based_b2_rsrp_thres;
  int32   kpi_based_b2_rsrp_offset;
  int32   kpi_based_a2_rsrp_thres;
  int32   kpi_based_a2_rsrp_offset;
  int32   kpi_based_serv_cell_rsrp_thres;  /*! A2/B2 threshold when KPI feature is kicked in*/
  int32   kpi_based_b2_gsm_rssi_thres;  /*! B2 GSM neighbor cell RSSI threshold*/
  uint16  kpi_based_b2_ttt;  /*! B2 TTT when KPI feature is kicked in*/
} lte_cphy_kpi_based_adjust_ind_s;
...
enum
{
  ...
  MSGR_DEFINE_UMID(LTE, CPHY, REQ, DTOD_CFG, LTE_CPHY_DTOD_CFG_ID, lte_cphy_dtod_cfg_req_s),
  MSGR_DEFINE_UMID(LTE, CPHY, CNF, DTOD_CFG, LTE_CPHY_DTOD_CFG_ID, lte_cphy_dtod_cfg_cnf_s),

  MSGR_DEFINE_UMID(LTE, CPHY, REQ, DTOD_DED_CFG, LTE_CPHY_DTOD_DED_CFG_ID, lte_cphy_dtod_ded_cfg_req_s),
  MSGR_DEFINE_UMID(LTE, CPHY, CNF, DTOD_DED_CFG, LTE_CPHY_DTOD_DED_CFG_ID, lte_cphy_dtod_ded_cfg_cnf_s),
 ...
 MSGR_DEFINE_UMID(LTE, CPHY, REQ, TRM_UNLOCK_ALLOWED,
  ...
  MSGR_DEFINE_UMID(LTE, CPHY, REQ, LTED_SIB_SCHED,
                   LTE_CPHY_LTED_SIB_SCHED_ID, lte_cphy_sib_sched_req_s),

  MSGR_DEFINE_UMID(LTE, CPHY, REQ, LTED_CELL_SEL,
                   LTE_CPHY_LTED_CELL_SEL_ID, lte_cphy_lted_cell_sel_req_s),

  MSGR_DEFINE_UMID(LTE, CPHY, REQ, OOS_WARNING, LTE_CPHY_OOS_WARNING_ID, lte_cphy_oos_warning_req_s),
...
MSGR_DEFINE_UMID(LTE, CPHY, REQ, CDMA_CHANNEL_BAR,
  ...
  */
  ...
  MSGR_DEFINE_UMID(LTE, CPHY, REQ, SCELL_CANCEL,
                   LTE_CPHY_SCELL_CANCEL_ID, lte_cphy_scell_cancel_req_s),

  MSGR_DEFINE_UMID(LTE, CPHY, REQ, SCELL_MEAS_CFG,
                   LTE_CPHY_SCELL_MEAS_CFG_ID, lte_cphy_scell_meas_cfg_req_s),
  ...
  MSGR_DEFINE_UMID(LTE, CPHY, REQ, ACHIEVABLE_TPUT_INFO, LTE_CPHY_ACHIEVABLE_TPUT_INFO_ID, lte_cphy_achievable_tput_info_req_s),
  MSGR_DEFINE_UMID(LTE, CPHY, CNF, ACHIEVABLE_TPUT_INFO,
                    LTE_CPHY_ACHIEVABLE_TPUT_INFO_ID, lte_cphy_achievable_tput_info_cnf_s),

  MSGR_DEFINE_UMID(LTE, CPHY, REQ, UL_CONFIGURABLE_TPUT_INFO, LTE_CPHY_UL_CONFIGURABLE_TPUT_INFO_ID, lte_cphy_ul_configurable_tput_info_req_s),
  MSGR_DEFINE_UMID(LTE, CPHY, CNF, UL_CONFIGURABLE_TPUT_INFO,
                    LTE_CPHY_UL_CONFIGURABLE_TPUT_INFO_ID, lte_cphy_ul_configurable_tput_info_cnf_s),
...
#ifdef LTE_FEATURE_PAGE_SHARING
  MSGR_DEFINE_UMID(LTE, CPHY, REQ, PAGE_MERGE, LTE_CPHY_PAGE_MERGE_ID, lte_cphy_page_merge_req_s),
  MSGR_DEFINE_UMID(LTE, CPHY, CNF, PAGE_MERGE, LTE_CPHY_PAGE_MERGE_ID, lte_cphy_page_merge_cnf_s),

  MSGR_DEFINE_UMID(LTE, CPHY, REQ, PAGE_SPLIT, LTE_CPHY_PAGE_SPLIT_ID, lte_cphy_page_split_req_s),
  MSGR_DEFINE_UMID(LTE, CPHY, CNF, PAGE_SPLIT, LTE_CPHY_PAGE_SPLIT_ID, lte_cphy_page_split_cnf_s),
#endif
  MSGR_DEFINE_UMID(LTE, CPHY, REQ, DO_TO_LTE_STATUS, LTE_CPHY_DO_TO_L_STATUS_ID, lte_cphy_do_to_lte_status_req_s),				   
  /*=========================================================================*/
  /*                          Indications                                    */
  /*=========================================================================*/
  ...
  MSGR_DEFINE_UMID( LTE, CPHY, IND, CONFIGURED_TPUT_INFO,
                    LTE_CPHY_IND_CONFIGURED_TPUT_INFO_ID, lte_cphy_configured_tput_info_ind_s),

  MSGR_DEFINE_UMID( LTE, CPHY, IND, ACHIEVABLE_TPUT_INFO_START,
                     LTE_CPHY_IND_ACHIEVABLE_TPUT_INFO_START_ID, lte_cphy_achievable_tput_info_start_ind_s),

  MSGR_DEFINE_UMID( LTE, CPHY, IND, ACHIEVABLE_TPUT_INFO_STOP,
                     LTE_CPHY_IND_ACHIEVABLE_TPUT_INFO_STOP_ID, lte_cphy_achievable_tput_info_stop_ind_s),
  ...
  MSGR_DEFINE_UMID(LTE, CPHY, IND, RLF_DEBUG,
                    LTE_CPHY_IND_RLF_DEBUG_ID, lte_cphy_rlf_debug_ind_s),
  ...
  MSGR_DEFINE_UMID(LTE, CPHY, IND, PAGE_MERGE_REEVALUATE,
                   LTE_CPHY_IND_PAGE_MERGE_REEVALUATE_ID, lte_cphy_page_merge_reevaluate_ind_s),
  ...
  */
  ...
  /*! @brief This message is sent from MAC to ML1 to indicate the TA Value when
      RAID is off by one
  */
  MSGR_DEFINE_UMID(LTE, CPHY, IND, RA_PID_OFF,
                   LTE_CPHY_IND_RA_PID_OFF_ID, lte_cphy_ra_pid_off_ind_s),
  ...
  */
  ...
  /*! @brief Indication sent from ML1 to RRC for SCell meas report
    */
  MSGR_DEFINE_UMID(LTE, CPHY, IND, SCELL_MEAS_REPORT,
                   LTE_CPHY_IND_SCELL_MEAS_REPORT_ID, lte_cphy_scell_meas_report_ind_s),
  ...
  /* message ID: LTE_CPHY_TTL_CORR_IND from ML1 to RRC
   Requestion RRC to send a SIB1 Sched request */
  MSGR_DEFINE_UMID(LTE, CPHY, IND, TTL_COR,
                   LTE_CPHY_IND_TTL_COR_ID, lte_cphy_ttl_cor_ind_s),

  MSGR_DEFINE_UMID(LTE, CPHY, REQ, DDS_CHANGE,
                   LTE_CPHY_DDS_CHANGE_ID, lte_cphy_dds_change_req_s),

  MSGR_DEFINE_UMID(LTE, CPHY, IND, DTOD_CELL_DETECT,
                     LTE_CPHY_IND_DTOD_CELL_DETECT_ID, lte_cphy_dtod_cell_detect_ind_s),

  MSGR_DEFINE_UMID(LTE, CPHY, IND, DTOD_SCAN_COMPLETED,
                     LTE_CPHY_IND_DTOD_SCAN_ID, lte_cphy_dtod_scan_ind_s),

  MSGR_DEFINE_UMID(LTE, CPHY, IND, LTED_RESUME,
                     LTE_CPHY_IND_LTED_RESUME_ID, lte_cphy_lted_resume_ind_s),

  MSGR_DEFINE_UMID(LTE, CPHY, IND, LTED_BACKOFF,
                     LTE_CPHY_IND_LTED_BACKOFF_ID, lte_cphy_lted_backoff_ind_s),

  MSGR_DEFINE_UMID(LTE,CPHY, IND, VOLTE_STATUS,
                   LTE_CPHY_VOLTE_STATUS_ID, lte_ims_qpcall_status_ind_s),
  MSGR_DEFINE_UMID(LTE,CPHY, IND, KPI_BASED_THRES,
				   LTE_CPHY_KPI_BASED_THRES_ID, lte_cphy_kpi_based_adjust_ind_s),

 /*! @brief Message sent from IMS to ML1 to provide wifi mobility possible status ind*/
  MSGR_DEFINE_UMID(LTE,CPHY, IND, WIFI_MOBILITY_STATUS,
                   LTE_CPHY_IND_WIFI_MOBILITY_STATUS_ID, lte_ims_wifi_mobility_status_s),
                   
  /*! @brief Message sent from ML1 to IMS to provide srvcc enter/exit status ind*/
  MSGR_DEFINE_UMID(LTE,CPHY, IND, SRVCC_STATUS,
                   LTE_CPHY_IND_SRVCC_STATUS_ID, lte_ims_srvcc_status_s),
};
...
{
  msgr_hdr_struct_type msgr_hdr;
  /*!* Flag indicated if ML1 is in connected state*/
  boolean ml1_is_connected;
} lte_cphy_achievable_tput_status_ind_s;
...
typedef struct
{
  ...
  /*! valueTag corresponding to this time PHR */
  uint8 phr_value_tag;
  ...
  */
  ...
  /*! HARQ result - for which type is the HARQ reason report triggered */
  lte_l1_harq_fb_type_e harq_result_type;

  /*! HARQ result - the result payload ACK/NAK */
  lte_l1_harq_fb_value_e harq_result_value;

  /*!< harq result triggered reason, by TA or requested by MAC. Ture for TA */
  boolean                harq_result_triggered_by_tune_away;
...
typedef enum
{
    ...
    LTE_CPHY_SR_TRIGGER_BY_ML1  /*!< sr is triggered by ml1 */ 
}lte_cphy_sr_trigger_type_e;

/*! @brief Enum of SR boost types.
*/
typedef enum
{
    LTE_CPHY_SR_BOOST_NONE       =   0,              
    LTE_CPHY_SR_BOOST_FROM_FIRST,
    LTE_CPHY_SR_BOOST_FROM_SECOND,
    LTE_CPHY_SR_BOOST_BY_NV_CFG
}lte_cphy_sr_boost_type_e;
...
typedef struct
{
  ...
  lte_cphy_sr_boost_type_e boost_type;
...
typedef struct
{
  ...
  /* TRUE for INTERRUPT_CDRX_CYCLE type; FALSE for the legacy type */
  boolean interrupt_cdrx_sr;
...
{
  msgr_hdr_struct_type msgr_hdr;
} lte_cphy_sr_trigger_by_ml1_ind_s;;

typedef struct
{
  msgr_hdr_struct_type msgr_hdr;
  /* TRUE for RRC CONNECTION extension start; FALSE for RRC CONNECTION extension end */
  boolean rrc_connection_extension;
} lte_cphy_extend_rrc_connection_ind_s;
...
/*! @brief
  Sent when the category downgrade is detected.

  This is used to notify upper-layers UE category has to be changed
  */
typedef struct
{
  /*! Message header */
  msgr_hdr_struct_type msg_hdr;
  /*! In case ul64qam need to be disabled, set the flag to TRUE,
      In case ul64qam need to be enabled, set the flag to FALSE
      RRC will only honor the stauts in the last ind during one Connection */
  boolean disable_ul64qam;
} lte_cphy_cat_downgrade_ind_s;


/******************************************************************************
                              LTE-D external msg structs
*******************************************************************************/
enum
{
  MSGR_DEFINE_UMID(LTE, CPHY, REQ, ACTIVATE_LTED_TX, LTE_CPHY_ACTIVATE_LTED_TX_ID,
                   lte_cphy_activate_lted_tx_req_s ),
  MSGR_DEFINE_UMID(LTE, CPHY, IND, START_LTED_TX, LTE_CPHY_IND_START_LTED_TX_ID,
                   lte_cphy_start_lted_tx_ind_s ),
  MSGR_DEFINE_UMID(LTE, CPHY, REQ, PAC_TX, LTE_CPHY_PAC_TX_REQ_ID,
                   lte_cphy_pac_tx_req_s),
  MSGR_DEFINE_UMID(LTE, CPHY, CNF, PAC_TX, LTE_CPHY_PAC_TX_CNF_ID,
                   lte_cphy_pac_tx_cnf_s),
  MSGR_DEFINE_UMID(LTE, CPHY, REQ, ACTIVATE_LTED_DL, LTE_CPHY_ACTIVATE_LTED_DL_ID,
                   lte_cphy_activate_lted_dl_req_s),
  MSGR_DEFINE_UMID(LTE, CPHY, IND, LTED_START_DL_PERIOD, LTE_CPHY_IND_LTED_START_DL_PERIOD_ID,
                   lte_cphy_lted_start_dl_period_ind_s),
  MSGR_DEFINE_UMID(LTE, CPHY, IND, UL_PROSE_PAC_BUILD, LTE_CPHY_IND_UL_PROSE_PAC_BUILD_ID,
                   lte_cphy_ul_prose_pac_build_ind_s),
};
/*! @brief MAC req msg to notify ML1 to activate LTE-D UL Tx */
typedef struct
{
  msgr_hdr_struct_type                 hdr;  /*!< Message header */
  /*! Message payload, tx enable periodicity for QoS control purpose
  for now, we need to enable every period. If this is set to 0, that means
  to deactivate the lted tx */
  uint8 tx_enable_periodicity;
} lte_cphy_activate_lted_tx_req_s;

/*! @brief ML1 to nofity MAC the LTED tx is started */
typedef struct
{
  msgr_hdr_struct_type                 hdr;  /*!< Message header */
  /*! Message payload */
  /*! Estimation of the number of tx resource after tx_prob test */
  uint16 tx_resource_num;
} lte_cphy_start_lted_tx_ind_s;

/*! @brief MAC req msg to notify ML1 the number of PACs need to be txed */
typedef struct
{
  msgr_hdr_struct_type                 hdr;            /*!< Message header */
  /*! message payload  */
  uint16                               num_pac_to_send; /*!< Number of PACs need to be txed */
} lte_cphy_pac_tx_req_s;

/*! @brief ML1 to nofity MAC how many PACs was txed */
typedef struct
{
  msgr_hdr_struct_type                 hdr;            /*!< Message header */
  /*! message payload  */
  uint16                               num_pac_txed; /*!< Number of PACs txed */
} lte_cphy_pac_tx_cnf_s;

/*! @brief MAC req msg to notify ML1 to activate lted dl monitoring */
typedef struct
{
  msgr_hdr_struct_type                 hdr;  /*!< Message header */
  /*! message payload with priority_level, 0 means deactivate */
  uint8 num_active_filter;
} lte_cphy_activate_lted_dl_req_s;

/*! @brief ML1 to nofity MAC the dl monitoring start time */
typedef struct
{
  msgr_hdr_struct_type                 hdr;  /*!< Message header */
  /*! message payload to indicate the start time */
  /*! System Frame number */
  lte_l1_sfn_t sys_fn;
  /*! Subframe number */
  lte_l1_subfn_t  sub_fn;
  /*! UTC time counter of 1 ms units since 00:00:00 on 1 January, 1900 */
  uint64 utc_time;
} lte_cphy_lted_start_dl_period_ind_s;
...
typedef struct
{
  ...
  /*! used as PAC index to refer back in MAC */
  uint8 pac_index;
} lte_cphy_ul_prose_pac_build_ind_s;
...
enum
{
  MSGR_DEFINE_UMID(LTE, CPHY, IND, MAC_FC,
                   LTE_CPHY_IND_MAC_FC_ID, lte_cphy_mac_fc_ind_s),
  MSGR_DEFINE_UMID(LTE, CPHY, IND, MAC_FC_STATE,
                   LTE_CPHY_IND_MAC_FC_STATE_ID, lte_cphy_mac_fc_state_ind_s),
  ...
  MSGR_DEFINE_UMID(LTE, CPHY, IND, CAT_DOWNGRADE,
                   LTE_CPHY_IND_CAT_DOWNGRADE_ID, lte_cphy_cat_downgrade_ind_s),
  MSGR_DEFINE_UMID(LTE, CPHY, IND, ACHIEVABLE_TPUT_STATUS,
                   LTE_CPHY_ACHIEVABLE_TPUT_STATUS_ID, lte_cphy_achievable_tput_status_ind_s),

  MSGR_DEFINE_UMID(LTE, CPHY, IND, SR_TRIGGER_BY_ML1,
                   LTE_CPHY_IND_SR_TRIGGER_BY_ML1_ID, lte_cphy_sr_trigger_by_ml1_ind_s),
                   
  MSGR_DEFINE_UMID(LTE, CPHY, IND, EXTEND_RRC_CONNECTION,
                   LTE_CPHY_IND_EXTEND_RRC_CONNECTION_ID, lte_cphy_extend_rrc_connection_ind_s),
};
...
enum
{
  ...
  MSGR_DEFINE_UMID(LTE, CPHY, IND, MAC_TAG_OOS, LTE_CPHY_IND_MAC_TAG_OOS_ID,
                   lte_cphy_ml1_mac_msgr_msg_s),
};
...
typedef struct
{
  ...
  */
  ...
  /*! the carrier ID
      ...
      1: Secondary Carrier Component 0 (SCC0)
      2: Secondary Carrier Component 1 (SCC1)
      3: reserved for Secondary Carrier Component 2 (SCC2)
      4: reserved for Secondary Carrier Component 3 (SCC3)
      5-29: reserved for future usage
      30: reserved for SCC0 HORXD
      31: PCC HORXD
  */
  uint8                             carrier_id;
...
typedef enum{
  LTE_CPHY_SCELL_FC_START = 0x01,
  LTE_CPHY_SCELL_FC_END,
  LTE_CPHY_SCELL_FC_STATUS_MAX
}lte_cphy_scell_fc_status_e;
...
typedef struct
{
  ...
  /* the following is the staus code */
  /*
  LTE_CPHY_SCC_FC_START
  LTE_CPHY_SCC_FC_END
   */
  lte_cphy_scell_fc_status_e          fc_status;
} lte_cphy_scell_fc_status_ind_s;

/*! @brief
  Enum of UMID to Indicate SCell Thermal mitigation start/end
*/
enum
{
  /* message ID: LTE_CPHY_SCELL_FC_STATUS_IND */
  MSGR_DEFINE_UMID(LTE, CPHY, IND, SCELL_FC_STATUS,
                   LTE_CPHY_SCELL_FC_STATUS_ID, lte_cphy_scell_fc_status_ind_s)
};
...
/*! @brief Scheduler system time in the unit of subframe */
typedef uint16 lte_ml1_schdlr_sf_time_t;

/*! Current state of sleep */
typedef enum
{
  /*! No sleep */
  LTE_ML1_SLEEPMGR_STATE_NOT_SLEEPING,

  /*! Semi Light sleep */
  LTE_ML1_SLEEPMGR_STATE_SEMI_LIGHT_SLEEP,

  /*! Full Light sleep */
  LTE_ML1_SLEEPMGR_STATE_FULL_LIGHT_SLEEP,

  /*! Deep Light sleep */
  LTE_ML1_SLEEPMGR_STATE_DEEP_LIGHT_SLEEP,

  /*! Deep sleep */
  LTE_ML1_SLEEPMGR_STATE_DEEP_SLEEP,

  /*! Quick Light sleep */
  LTE_ML1_SLEEPMGR_STATE_QUICK_LIGHT_SLEEP,

  /*! Quick Deep Light sleep */
  LTE_ML1_SLEEPMGR_STATE_QUICK_DEEP_LIGHT_SLEEP,

} lte_ml1_sleepmgr_sleep_state_t;

/*! Type of Offline Wakeup */
typedef enum
{
  /*! No specific offline wakup type */
  LTE_ML1_SLEEPMGR_OFFLINE_WAKEUP_TYPE_NOT_SPECIFIED      = 0x0,
  /*! Enabled with serv meas only */
  LTE_ML1_SLEEPMGR_OFFLINE_SERV_MEAS                      = 0x1,
  /*! Enabled with serv + intra */
  LTE_ML1_SLEEPMGR_OFFLINE_INTRA_MEAS                     = 0x2,
  /*! Enabled with serv + intra, with neighbor MBSFN */
  LTE_ML1_SLEEPMGR_OFFLINE_MBSFN_INTRA_MEAS               = 0x4,
  /*! Enabled with search. Measurements possible */
  LTE_ML1_SLEEPMGR_OFFLINE_INTRA_SRCH                     = 0x8,
  /*! Enabled with interfreq */
  LTE_ML1_SLEEPMGR_OFFLINE_INTER                          = 0x10,

} lte_ml1_sleepmgr_offline_wakeup_type_e;

/*! @brief The scheduler arbitration mode enum */
typedef enum
{
  /*! Online arb mode */
  LTE_ML1_SCHDLR_ARB_MODE_ONLINE=0,
  /*! Online QTA arb mode */
  LTE_ML1_SCHDLR_ARB_MODE_ONLINE_QTA,
  /*! Offline arb mode */
  LTE_ML1_SCHDLR_ARB_MODE_OFFLINE,
  /*! Offline QTA arb mode */
  LTE_ML1_SCHDLR_ARB_MODE_OFFLINE_QTA,
  /*! IRAT QTA arb mode */
  LTE_ML1_SCHDLR_ARB_MODE_IRAT,
  /*! Max arb mode to be considered in cluster query for wakeup arb mode type */
  LTE_ML1_SCHDLR_ARB_MODE_WAKEUP_MAX = LTE_ML1_SCHDLR_ARB_MODE_IRAT,

  /* below arb modes are not considered for wakeup arb mode cluster query */
  /*! Offline Page Collision arb mode */
  LTE_ML1_SCHDLR_ARB_MODE_OFFLINE_PAGE_COLLISION,
  /*! Online Page Collision arb mode */
  LTE_ML1_SCHDLR_ARB_MODE_ONLINE_PAGE_COLLISION,
  /*! The number of the scheduler arbitration modes */
  LTE_ML1_SCHDLR_MAX_NUM_ARB_MODES
} lte_ml1_schdlr_arb_mode_e;
...
typedef struct
{
  ...
  /*! Time of STMR on */
  lte_ml1_schdlr_sf_time_t   stmr_on_time;
  ...
  /*! Chains granted on wakeup */
  uint8 chains_granted_mask[LTE_ML1_MAX_CC];

  /*! Chains granted but not tuned on wakeup */
  uint8 chains_granted_not_tuned_mask[LTE_ML1_MAX_CC];

  /*! Chains denied on wakeup */
  uint8 chains_denied_mask[LTE_ML1_MAX_CC];

  /*! Sleep state */
  lte_ml1_sleepmgr_sleep_state_t sleep_state;

  /*! Type of Offline Wakeup */
  lte_ml1_sleepmgr_offline_wakeup_type_e  offline_wakeup_enabled_mask;

  /*! Arb mode to which scheduler has been put on this wakeup*/
  lte_ml1_schdlr_arb_mode_e   arb_mode;

  /*! Flag to indicate that current wakeup is delayed offline wakeup*/
  boolean                    is_early_rf_wakeup;

} lte_ml1_sleepmgr_wakeup_isr_ind_s;

enum
{
  /* message ID: LTE_ML1_SLEEPMGR_WAKEUP_ISR_IND */
...
/*! @brief Enumeration for page merge cause.
*/
typedef enum
{
  /* default cause. */
  LTE_CPHY_PAGE_MERGE_DEFAULT                         = 0
    
}lte_cphy_page_merge_cause_e;


/*! @brief
  This message is used to tell ML1 to start page merge.
*/
typedef struct
{
  /*! Message header */
  ...
  /*! target cell for page merge. */
  lte_cphy_cell_info_s cell_info;

  /*! paging cycle in ms,
  decided by default paging cycle on DDS SUB,
  UE specific paging cycle and scaling factor on non DDS SUB. */
  uint16 paging_cycle;

  /*! reason to trigger page merge. */
  lte_cphy_page_merge_cause_e cause;
} lte_cphy_page_merge_req_s;

/*! @brief Enumeration for page merge status.
*/
typedef enum
{
  LTE_CPHY_PAGE_MERGE_SUCCESS                         = 0,
  LTE_CPHY_PAGE_MERGE_FAIL_WITH_CELL_BAR              = 1,
  LTE_CPHY_PAGE_MERGE_FAIL_DUE_TO_HO_ON_DDS_SUB       = 2,
  LTE_CPHY_PAGE_MERGE_FAIL_DUE_TO_OTHER_REASON        = 3,
}lte_cphy_page_merge_status;

/*! @brief
  This message is used to tell RRC the result of page merge.
*/
typedef struct
{
  /*! Message header */
  ...
  /*! page merge status. */
  lte_cphy_page_merge_status status;

  /*! page merged cell. */
  lte_cphy_cell_info_s cell_info;

  /*! cell bar remaining timer length in ML1. Uint is ms.
  it is valid when status is LTE_CPHY_PAGE_MERGE_FAIL_WITH_CELL_BAR.
  */
  uint64 cell_bar_timer_length;
} lte_cphy_page_merge_cnf_s;

/*! @brief Enumeration for page split cause.
*/
typedef enum
{
  /* default cause. */
  LTE_CPHY_PAGE_SPLIT_DEFAULT                         = 0,

  /* triggered by non DDS SUB. */

  LTE_CPHY_PAGE_SPLIT_CALL                            = 1,
  LTE_CPHY_PAGE_SPLIT_SIGNALLING                      = 2,
  LTE_CPHY_PAGE_SPLIT_PLMN_SEARCH                     = 3,
  LTE_CPHY_PAGE_SPLIT_SERVICE_REQ                     = 4,
  /* target cell is not suitable for merge. */
  LTE_CPHY_PAGE_SPLIT_CELL_NOT_SUITABLE               = 5,
  /* for cphy abort case, RRC internal use.*/
  LTE_CPHY_PAGE_SPLIT_CPHY_ABORT                      = 6,
  /* for DDS switch case. */
  LTE_CPHY_PAGE_SPLIT_DDS_SWITCH                      = 7,

  /* triggered by DDS SUB. */

  /* for L2X HO case. */
  LTE_CPHY_PAGE_SPLIT_IRAT_HO_ON_DDS_SUB              = 10,
  /* for RLF case. */
  LTE_CPHY_PAGE_SPLIT_RLF_ON_DDS_SUB                  = 11,
  /* for connected to idle case, include and L2G CCO case. */
  LTE_CPHY_PAGE_SPLIT_CONNECTED_TO_IDLE_ON_DDS_SUB    = 12,
    /*Merged cell reselection cancelled */
  LTE_CPHY_PAGE_SPLIT_CELL_MERGE_RESEL_CANCEL         = 13,
  
  LTE_CPHY_PAGE_SPLIT_MEGRGE_FAIL_ON_DIFFERNT_TAI     = 14,  
  
  LTE_CPHY_PAGE_SPLIT_MPLMN_SEARCH                    = 15,
  
  /* Split cause used in Idle+idle page merge */
  LTE_CPHY_PAGE_SPLIT_SPLIT_L2X_RESEL_ON_DDS_IDLE     = 16,
  LTE_CPHY_PAGE_SPLIT_L2X_REDIR_ON_DDS_SUB            = 17
}lte_cphy_page_split_cause_e;

/*! @brief
  This message is used to tell ML1 to start page split.
*/
typedef struct
{
  /*! Message header */
  ...
  /*! reason to trigger page split. */
  lte_cphy_page_split_cause_e split_cause;

  /*! target cell to recover for page split. */
  lte_cphy_cell_info_s cell_info;

  /*! split delay time by RRC, uint is ms. */
  uint16 split_delay_time;

} lte_cphy_page_split_req_s;

/*! @brief
  This message is used to tell RRC the confirmation of page split.
*/
typedef struct
{
  /*! Message header */
  ...
  /*! cell to recover after page split. */
  lte_cphy_cell_info_s cell_info;

} lte_cphy_page_split_cnf_s;

#endif /* LTE_FEATURE_PAGE_SHARING */

/*-----------------------------------------------------------------------*/
/*     Search to IMS External message definitions                          */
/*-----------------------------------------------------------------------*/

typedef enum
{
  LTE_CPHY_PRE_HO_REGION_INVALID,
  LTE_CPHY_PRE_HO_REGION_ENTER,
  LTE_CPHY_PRE_HO_REGION_EXIT,
}lte_cphy_pre_ho_event_type_e;

typedef enum
{
  /* default HO type */
  LTE_CPHY_PRE_HO_TYPE_NONE = 0,
  LTE_CPHY_PRE_HO_TYPE_L2L
}lte_cphy_pre_ho_type_e;

typedef struct
{
  lte_cphy_pre_ho_type_e         ho_type; /* Enum to indicate handover type */ 
  lte_cphy_pre_ho_event_type_e   event; /* Enum to indicate handover state */ 
} lte_cphy_pre_ho_ind_s;
...
typedef struct
{
  ...
  /*! payload for ho info */
  lte_cphy_pre_ho_ind_s   pre_ho_info;
}lte_cphy_ims_pre_ho_ind_msg_s;

/*-----------------------------------------------------------------------*/
/*     LTE ML1 Search External Indication messages                           */
/*-----------------------------------------------------------------------*/
/*! @brief external Indications that the LTE Search module sends  */
enum
{
  MSGR_DEFINE_UMID( LTE, CPHY, IND, IMS_PRE_HO,
                    LTE_CPHY_IMS_PRE_HO_ID, lte_cphy_ims_pre_ho_ind_msg_s ),
};
