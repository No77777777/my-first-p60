...
$Header: //components/rel/lte.mpss/6.6.1/api/lte_cphy_irat_meas_msg.h#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
06/30/15   lis     Support of L2L gap
04/28/14   lis     G2L Interface update to pass device id info 
...
#define LTE_CPHY_IMEAS_MAX_DETECTED_CELLS 8
...
typedef struct
{
  ...
  /*! The number of Tx antenna for L2L inter-freq measurement */
  uint8 eNb_tx_ant_ifreq;
...
/*!  This structure provides information of the srving cell for
     IRAT inter-freq new gap
  */
typedef struct
{
  lte_earfcn_t srving_freq;
  lte_bandwidth_idx_e srving_bw_idx;
}lte_cphy_irat_meas_lte_srving_s;


/*!  This structure provides L2L build script options, 
     srving info. and srch/meas callbacks for L2L IRAT inter-freq new gap
  */
typedef struct
{
  /*! L2L Inter-freq indicator, Indicator whether half script is supported for 
      this gap
  */
  boolean need_half_script_tune;
  
  lte_cphy_irat_meas_lte_srving_s ifreq_srving_cell;
  
}lte_cphy_irat_meas_lte_rf_param_type;
...
typedef union lte_cphy_irat_meas_rf_source_params_union
{
  ...
  lte_cphy_irat_meas_lte_rf_param_type    lte_params;
...
typedef struct
{
  ...
  /*! Diversity device ID. Set to RFM_INVALID_DEVICE if 
      LTE diversity isn't allocated */
...
} lte_cphy_irat_meas_target_rf_param_s;

typedef struct
{
  rfm_meas_common_param_type *common;

  lte_cphy_irat_meas_target_rf_param_s target;

  struct
  {
    boolean need_half_script_tune;
    boolean is_gap_aborted;
    /*L2L inter-freq TRM device transaction id*/
    uint32  transaction_id; 
  } flags;

} lte_cphy_irat_meas_rf_param_s;
...
typedef struct
{
  ...
  */
  ...
  /*! Structure containing the source rat parameters 
    ...
    @deprecated For THOR 2.0 use the new lte_cphy_irat_meas_rf_param_s field
   */
  lte_cphy_irat_meas_startup_rf_params_s rf_params;

  /*! RF parameter structure */
  lte_cphy_irat_meas_rf_param_s rf_param;
  ...
  */
  ...
  boolean qta_wait_for_build_script_cnf;

  boolean l2l_tune_back_only;
...
/*! @brief IRAT MEAS build rf script request
    used for X2L search/meas process before TIMED srch/meas req
    to build X2L full script in advnace before X2L opens
*/
typedef struct
{
  ...
  msgr_hdr_struct_type                      hdr;
  lte_cphy_irat_meas_rf_param_s rf_param;
  uint32 qta_req_offset;
} lte_cphy_irat_meas_build_rf_script_req_s;


/*! @brief IRAT MEAS build rf script confirmation
    used for X2L search/meas process before TIMED srch/meas req
    to indicate source tech the script building result
*/
...
typedef struct
{
  ...
  /*! The target frequency on which the script was built
  range 0...65535*/
  lte_earfcn_t       freq;

  /*! Boolean to indicate if the script is aborted  */
  boolean script_aborted;

} lte_cphy_irat_meas_build_rf_script_cnf_s;
...
enum
{
  ...
  MSGR_DEFINE_UMID(LTE, CPHY_IRAT_MEAS_W2L, REQ, BUILD_RF_SCRIPT, 0x9, 
					lte_cphy_irat_meas_build_rf_script_req_s),
  MSGR_DEFINE_UMID(LTE, CPHY_IRAT_MEAS_W2L, CNF, BUILD_RF_SCRIPT, 0x9, 
					lte_cphy_irat_meas_build_rf_script_cnf_s),
  ...
  MSGR_DEFINE_UMID(LTE, CPHY_IRAT_MEAS_G2L, REQ, BUILD_RF_SCRIPT, 0x9, 
                   lte_cphy_irat_meas_build_rf_script_req_s),
  MSGR_DEFINE_UMID(LTE, CPHY_IRAT_MEAS_G2L, CNF, BUILD_RF_SCRIPT, 0x9, 
                   lte_cphy_irat_meas_build_rf_script_cnf_s),
