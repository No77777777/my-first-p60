/*
  ...
  @detail
    ...
    $Header: //components/rel/lte.mpss/6.6.1/api/lte_l1_types.h#3 $
*/
...
/*! Maximum CC in the system possible including PCC */
#define LTE_ML1_MAX_CC                   (LTE_CPHY_MAX_S_CELL + 1)

/*! Maximum number of Stag Ids sent in RRC config req message */
#define LTE_CPHY_MAX_NUM_STAG   3
...
/*! @brief max number of mcc in white list*/
#define LTE_ML1_SM_IDLE_PINGPONG_MAX_MCC_NUM            20
...
typedef struct
{
  ...
  /*! Threshold to compare against the current Pcell RSRQ. We only apply the
      adjustments if Pcell RSRQ < RSRQ_THRESH  */
  int32 rsrq_thresh;

  /*! Threshold to compare against the current Pcell RSRP. We only apply the
      adjustments if Pcell RSRP < RSRP_THRESH  */
  int32 rsrp_thresh;

  /*! Adjustment to apply before eval and when sending MRs */
  int32 thresh1_adj;

} lte_ml1_nv_event_b2_thresh_adj_info_s;
...
/*! @brief
    AntennaInfoDedicated-v10i0  (ref: 36.331, 6.3.2)
*/
typedef enum
{
  LTE_L1_MAX_MIMO_LYR_TWO = 2,
  LTE_L1_MAX_MIMO_LYR_FOUR = 4,
  LTE_L1_MAX_MIMO_LYR_EIGHT = 8,
  LTE_L1_MAX_MIMO_LYR_INVALID = 0xFF,
}lte_l1_max_mimo_lyr_e;
...
/*! Struct for UE category degrade NV */
typedef struct
{
  /*! UE category degrade detect flag */
  uint8 cat_degrade_detect_enable;

  /*! PUSCH BLER rate MAX */
  uint8 pusch_bler_threshold_max;

  /*! PUSCH BLER rate MIN */
  uint8 pusch_bler_threshold_min;
  
  uint8 reserved;
}lte_ml1_nv_cat_degrade_s;
...
/*! @brief
  Cell Id value to be used for LTED for Model A (infra-less)
  */
#define LTE_L1_LTED_DEFAULT_PHY_CELL_ID 510
...
#define LTE_CPHY_EPDCCH_MAX_NUM_SET   2 //Max number of ePDCCH set per carrier
...
typedef enum {
  LTE_L1_DELTA_TXD_PUCCH_FORMAT1BCS_NONE = -1,
  LTE_L1_DELTA_TXD_PUCCH_FORMAT1BCS_0 = 0,
  LTE_L1_DELTA_TXD_PUCCH_FORMAT1BCS_1 = 1
}lte_l1_delta_txd_pucch_format1bcs_e;
...
/*! @brief
    pSRS offset: 0..31(ref: [36.331, 6.3.2], [36.213, 5.1.3.1.])
    Parameter: PSRS_OFFSET for periodic and aperiodic sounding reference signal 
    transmission repectively. See TS 36.213
    [23, 5.1.3.1]. For Ks=1.25, the actual parameter value is pSRS-Offset value 
    � 3. For Ks=0, the actual parameter value
    is -10.5 + 1.5*pSRS-Offset value. For Ks=0, E-UTRAN does not set values 
    larger than 26.
*/
typedef uint8 lte_l1_ul_power_ctrl_psrs_offset_t;

/*! @brief
    pSRS offsetAp: 0..31(ref: [36.331, 6.3.2], [36.213, 5.1.3.1.])
    Parameter: PSRS_OFFSET for periodic and aperiodic sounding reference signal 
    transmission repectively. See TS 36.213
    [23, 5.1.3.1]. For Ks=1.25, the actual parameter value is pSRS-Offset value 
    � 3. For Ks=0, the actual parameter value
...
typedef enum
{
  LTE_L1_CQI_SUB_MODE_NONE, /* Will be set when its getting released */
...
/*! @brief
  CQI R11 ReportPeriodicProcExtToAddModList
*/
typedef struct
{
  /* CQI-ReportPeriodicProcExtId is used to identify a periodic CQI reporting 
     configuration that E-UTRAN may configure in addition to the configuration 
     specified by the IE CQI-ReportPeriodic-r10. These additional configurations
     are specified by the IE CQI-ReportPeriodicProcExt-r11. The identity is unique 
     within the scope of a carrier frequency.
     Range:1-3 
  */
  uint8 ext_id;
  
  /*! CQI/PMI Periodicity and Offset Configuration Index.  Range: 0-1023*/
  ...
  /* Parameter RI cfg index. Range: 0...1023 */
  uint16 ri_cfg_index;

  /*! csi-ConfigIndex only if csi-SubframePatternConfig is configured. 
...
} lte_l1_cqi_reporting_periodic_proc_ext_add_mod_r11_s;

/*!
  @brief
  An enum describing the bit positions in the
  ext_id_add_rel_cfg_mask
  */
enum
{
  LTE_CPHY_DEDICATED_CQI_PERIODIC_PROC_EXT_NONE                = 0x0000,
  LTE_CPHY_DEDICATED_CQI_PERIODIC_PROC_EXT_ADD_MOD_MASK        = 0x0001,
  LTE_CPHY_DEDICATED_CQI_PERIODIC_PROC_EXT_REL_MASK            = 0x0002
} ;

/*! @brief
  CQI R11 reporting Periodic
*/
#define LTE_CQI_PERIODIC_MAX_PROC_EXT_ID 3
typedef struct
{
  /*! Parameter: Simultaneous-AN-and-CQI-Format3. see TS 36.213 TRUE indicates 
      that simultaneous transmission of ACK/NACK and CQI is allowed. */
  boolean sim_ack_nak_cqi_fmt_3;
  
  /*! If LTE_CPHY_DEDICATED_CQI_PERIODIC_PROC_EXT_ADD_MOD_MASK is set -> num_ext_id_add_mod_list
      needs to be processed
      If LTE_CPHY_DEDICATED_CQI_PERIODIC_PROC_EXT_REL_MASK is set -> ext_id_delete_bitmask
      needs to be processed
  */
  uint8 ext_id_add_rel_cfg_mask;
  
  /*! ReportPeriodicProcExtToReleaseList 
      For example ext_id_delete_list_bitmask = 0x3, means ext_id = 1 & 2 needs to be released
      Valid only if LTE_CPHY_DEDICATED_CQI_PERIODIC_PROC_EXT_REL_MASK is set 
      in ext_id_add_rel_cfg_mask*/ 
  uint8 ext_id_delete_bitmask;

  /*! Num of entires in ReportPeriodicProcExtToAddModList. Max is LTE_CQI_PERIODIC_MAX_PROC_EXT_ID 
      Valid only if LTE_CPHY_DEDICATED_CQI_PERIODIC_PROC_EXT_ADD_MOD_MASK is set 
      in ext_id_add_rel_cfg_mask*/
  uint8 num_ext_id_add_mod_list;
  /* Populated based on ext_id_add_mod_list */
  lte_l1_cqi_reporting_periodic_proc_ext_add_mod_r11_s ext_id_add_mod_info[LTE_CQI_PERIODIC_MAX_PROC_EXT_ID];
  

} lte_l1_cqi_reporting_periodic_r11_s;

/*! @brief
  CSI-IM-Config-r11
  CSI-IM-Config is the CSI Interference Measurement (IM) configuration that 
  E-UTRAN may configure on a serving frequency, see TS 36.213 [23, 7.2.6].
*/
typedef struct
{
  /* The IE CSI-IM-ConfigId is used to identify a CSI-IM configuration that 
     is configured by the IE CSI-IM-Config. The identity is unique within the 
     scope of a carrier frequency. Range:1-3 */
  uint8 cfg_id;

  /* resourceConfig 0...31
     Parameter: CSI reference signal configuration, 
     see TS 36.213 [23, 7.2.6] and TS 36.211 [21, table 6.10.5.2-1 and
     6.10.5.2-2] for 4 REs. */
  uint8 res_cfg_id;

  /* subframeConfig  0...154
     Parameter: ICSI-RS , see TS 36.213 [23, 7.2.6] and 
     TS 36.211 [21, table 6.10.5.3-1]. */
  uint8 sf_cfg_id;

}lte_l1_cqi_reporting_csi_im_add_mod_r11_s;
...
typedef struct
{
  /* The IE CSI-ProcessId is used to identify a CSI process that is configured 
     by the IE CSI-Process. The identity is unique within the 
     scope of a carrier frequency. 1...4*/
  uint8 proc_id;

  /* csi-RS-ConfigNZPId
     Refers to a CSI RS configuration using non-zero power transmission 
     that is configured for the same frequency as theCSI process. 1...3*/
  uint8 cfg_nzp_id;
  ...
  /* p-C-AndCBSRList
     A p-C-AndCBSRList including 2 entries indicates that the subframe patterns 
     configured for CSI (CQI/PMI/PTI/RI) reporting (i.e. as defined by field 
     csi-MeasSubframeSet1 and csi-MeasSubframeSet2) are to be used for this CSI
     process, while a single entry indicates that the subframe patterns are not 
     to be used for this CSI process. E-UTRAN does not include 2 entries in 
     p-C-AndCBSRList for CSI processes concerning a secondary frequency. E-UTRAN
     includes 2 entries in p-C-AndCBSRList when configuring both 
     cqi-pmi-ConfigIndex and cqi-pmi-ConfigIndex2.*/
   /* -8...15 */
   //int p_c[2];
   uint64 csr[2];   

  /* pmi-RI-Report
    See TS 36.213 [23, 7.2]. The presence of this field means PMI/RI reporting 
    is configured; otherwise the PMI/RI reporting is not configured. EUTRAN 
    configures this field only when transmissionMode is set to tm8, tm9 or tm10. 
    The UE shall ignore pmi-RI-Report-r9/ pmi-RI-Report-r10 when transmission 
    mode 10 is configured for the serving cell on this carrier frequency. */
  boolean pmi_ri_report_enabled;

  /* ri-Ref-CSI-ProcessId
     CSI process whose RI value the UE inherits when reporting RI, in the same 
     subframe, for CSI reporting. E-UTRAN ensures that the CSI process that inherits 
     the RI value is configured in accordance with the conditions specified in
      36.213 [23, 7.2.1, 7.2.2]. Range 1...4
  */
  boolean ri_ref_csi_proc_id_enabled;
  uint8 ri_ref_csi_proc_id;

  /* cqi-ReportPeriodicProcId
     Refers to a periodic CQI reporting configuration that is configured for the 
     same frequency as the CSI process. Value 0 refers to the set of parameters 
     defined by the REL-10 CQI reporting configuration fields, while the other 
     values refer tothe additional configurations E-UTRAN assigns by 
     CQI-ReportPeriodicProcExt-r11 (and as covered by CQIReportPeriodicProcExtId). */
  uint8 cqi_per_proc_id;
  ...
  /* trigger01
    Indicates whether or not reporting for this CSI-process is triggered by 
    CSI request field set to 01, for a CSI request applicable for the serving 
    cell on the same frequency as the CSI process, see TS 36.213 [23, table 7.2.1-1B].
    trigger10, trigger11
    Indicates whether or not reporting for this CSI-process is triggered by CSI
    request field set to 10 or 11, see TS 36.213 [23, table 7.2.1-1B]. EUTRAN 
    configures at most 5 CSI processes, across all serving frequencies, to be 
    triggered by a CSI request field set to value 10. The same restriction 
    applies for value 11. */
  boolean cqi_aperiodic_trigger01;
  boolean cqi_aperiodic_trigger10;
  boolean cqi_aperiodic_trigger11;  
  
}lte_l1_cqi_reporting_csi_proc_add_mod_r11_s;
...
enum
{
  LTE_CPHY_DEDICATED_CQI_CSI_IM_ADD_MOD_MASK        = 0x0001,
  LTE_CPHY_DEDICATED_CQI_CSI_IM_REL_MASK            = 0x0002,
  LTE_CPHY_DEDICATED_CQI_PROCESS_ADD_MOD_MASK       = 0x0004,
  LTE_CPHY_DEDICATED_CQI_PROCESS_REL_MASK           = 0x0008,  
} ;

/*! @brief
  CQI R11 reporting both
*/
#define LTE_CQI_PERIODIC_MAX_CSI_IM_ID 3
#define LTE_CQI_PERIODIC_MAX_CSI_PROC_ID 4
typedef struct
{
  /*! Tells which of the below config is valid */
  uint8 cfg_mask;
  
  /*! csi-IM-ConfigToReleaseList 
      For example csi_im_delete_bitmask = 0x3, means csi_im_id = 1 & 2 needs to be released
      Valid only if LTE_CPHY_DEDICATED_CQI_CSI_IM_REL_MASK is set 
      in cfg_mask*/ 
  uint8 csi_im_delete_bitmask;
  ...
  /* Num of entries on CSI-IM add mode list 
     Valid only if LTE_CPHY_DEDICATED_CQI_CSI_IM_ADD_MOD_MASK is set 
      in cfg_mask*/     
  uint8 num_csi_im_add_mod_list;
  /* Populated based on ext_id_add_mod_list */
  lte_l1_cqi_reporting_csi_im_add_mod_r11_s csi_im_add_mod_info[LTE_CQI_PERIODIC_MAX_CSI_IM_ID];
  
  /*! csi-ProcessToReleaseList 
      For example csi_proc_delete_bitmask = 0x3, means csi_im_id = 1 & 2 needs to be released
      Valid only if LTE_CPHY_DEDICATED_CQI_PROCESS_REL_MASK is set 
      in cfg_mask*/ 
  uint8 csi_proc_delete_bitmask;
  ...
  /*! csi-ProcessToAddModList
      ...
      configured for the serving cell on this carrier frequency.*/
  /* Num of entries on CSI-Process add mode list 
     Valid only if LTE_CPHY_DEDICATED_CQI_PROCESS_ADD_MOD_MASK is set 
      in cfg_mask*/     
  uint8 num_csi_proc_add_mod_list;
  /* Populated based on ext_id_add_mod_list */
  lte_l1_cqi_reporting_csi_proc_add_mod_r11_s csi_proc_add_mod_info[LTE_CQI_PERIODIC_MAX_CSI_PROC_ID];

} lte_l1_cqi_report_both_r11_s;
...
{
  /*! CQI R-11 config. */
  lte_l1_cqi_reporting_periodic_r11_s cqi_periodic_r11;
  /*! CQI ReportBoth R11. */
  lte_l1_cqi_report_both_r11_s cqi_both_r11;  
} lte_l1_cqi_rel11_param_s;
...
/*! @brief
    Indicates the applicability of the alternative CQI table (i.e. Table 7.2.3-2 in TS 36.213) 
    for both aperiodic and periodic CSI reporting for the concerned serving cell.
*/
typedef enum {
  LTE_L1_CQI_TABLE_FOR_ALL_SUBFRAMES = 0,  /*!< ALT CQI table applies to all the subframes and CSI processes */
  LTE_L1_CQI_TABLE_FOR_CSI_SUBFRAME_SET1, /*!< ALT CQI table applies to CSI subframe set1 */
  LTE_L1_CQI_TABLE_FOR_CSI_SUBFRAME_SET2,   /*!< ALT CQI table applies to CSI subframe set2 */
} lte_l1_alt_cqi_table_e;

/*! @brief
  CQI reporting Release 12 parameters
*/
typedef struct
{ 
  /*! If ALT CQI Table is enabled or not */
  boolean alt_cqi_table_enabled;

  /*! Indicates the applicability of the alternative CQI table (i.e. Table 7.2.3-2 in TS 36.213) 
    for both aperiodic and periodic CSI reporting for the concerned serving cell. 
    Value set to csi-SubframeSet1 or csi-SubframeSet2 only 
    -if transmissionMode is set in range tm1 to tm9 and
    -csi-SubframePatternConfig-r10 is configured for the concerned serving cell and 
    -different CQI tables apply to the two CSI subframe sets; 
    otherwise value set to allSubframes */
  lte_l1_alt_cqi_table_e alt_cqi_table;

}lte_l1_cqi_rel12_param_s;
...
}lte_ml1_nv_mcc_white_list_per_s;

typedef struct
{
  uint8  enabled;
  /* thresh to decide if ping-pong resel occurs */
  uint8  pingpong_detect_thresh;
  /* when pingpong resel times > pingpong_detect_thresh within this time duration, 
     pingpong is detected */
  uint16 pingpong_resel_detect_time;
  /* when ping-pong resel detected, start timer by minutes */
  uint16 pingpong_resel_bar_time;
  /* number of MCC in the MCC list */
  uint8  num_mcc;
  /* MCC white list */
  lte_ml1_nv_mcc_white_list_per_s mcc_white_list[LTE_ML1_SM_IDLE_PINGPONG_MAX_MCC_NUM];

}lte_ml1_nv_pingpong_resel_control_s;

typedef struct
{
  uint8          enable_cust_qrxlev_qqual;
  int8           Q_qualmin_th;
  int8           Q_qualmin_val;
  int16          Q_rxlev_th;
  int16          Q_rxlev_val;

}lte_ml1_nv_qrxlev_qqual_val_s;
...
/*! Resel priority value when a HST freq and serving freq are not configured in connection release.
    Priority should be treated as lowest the network configured priority 0..7 */
#define LTE_CPHY_MEAS_RESEL_PRIORITY_NW_LOWEST     0
...
typedef enum
{
  LTE_L1_MBSFN_MCH_SCHDL_PERIOD_4RF = 4,
...
typedef enum
{
    ...
    LTE_CPHY_OOS_REASON_CELL_SPLIT = 11,      /*!< 11 */
#endif
...
typedef enum
{
    ...
    LTE_CPHY_RLF_REASON_ELS_DSDS_SPV_TIMER_OUT   = 12       /*!< 12 */
...
/*! @brief L2-L1 bidirection used, 
  L2->L1 for informing what HARQ type is intrested by L2;
  L1->L2 what type of HARQ result is triggered
  this ENUM defination is inter-dependency with LTE_ML1_GM_MAP_HARQ_RESULT_TYPE_TO_TYPE_MASK */
typedef enum
{
  LTE_L1_HARQ_FB_TYPE_NULL,
  LTE_L1_HARQ_FB_TYPE_EARLY_TRIGGER,
  LTE_L1_HARQ_FB_TYPE_TERMINATION_TRIGGER,
  LTE_L1_HARQ_FB_TYPE_EARLY_AND_TERMINATION_TRIGGER
} lte_l1_harq_fb_type_e;

/*! @brief L2-L1 bidirection used, 
  L2->L1 for informing what HARQ value is intrested by L2;
  L1->L2 what value of HARQ result is feedbacked
  this ENUM defination is inter-dependency with LTE_ML1_GM_MAP_HARQ_RESULT_TYPE_TO_TYPE_MASK */
typedef enum
{
  LTE_L1_HARQ_FB_VALUE_NULL,
  LTE_L1_HARQ_FB_VALUE_ACK,
  LTE_L1_HARQ_FB_VALUE_NACK,
  LTE_L1_HARQ_FB_VALUE_ACK_AND_NACK
} lte_l1_harq_fb_value_e;
...
typedef struct
{
  ...
  uint64 ustmr_full;        /*!< Full XO count */ 
} lte_l1_stmr_l_s;
...
/*! @brief alternative TBS index for the I_TBS 26 and 33 
*/
typedef enum
{
  LTE_L1_TBS_INDEX_ALT_R12_INVALID, /*!< Used when RRC wants to release the previously configured value */
  LTE_L1_TBS_INDEX_ALT_R12_A26, /*!< Value a26 refers to the alternative TBS index ITBS 26A */
  LTE_L1_TBS_INDEX_ALT_R12_A33, /*!< Value a26 refers to the alternative TBS index ITBS 33A */
}lte_l1_tbs_index_alt_r12_e;

/*! @brief sleep type used in sleepmgr
*/
typedef enum
{
  /*! All sleep */
  LTE_L1_SLEEPMGR_ALL_SLEEP = 0,
  /*! Deep sleep */
  LTE_L1_SLEEPMGR_DEEP_SLEEP,
  /*! Deep Light sleep */
  LTE_L1_SLEEPMGR_DEEP_LIGHT_SLEEP,
  /*! Light sleep */
  LTE_L1_SLEEPMGR_LIGHT_SLEEP,
  LTE_L1_SLEEPMGR_MAX_SLEEP_TYPE
} lte_l1_sleepmgr_sleep_type_e;
