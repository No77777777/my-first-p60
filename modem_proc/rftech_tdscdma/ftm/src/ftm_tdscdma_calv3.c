...
/*===========================================================================
  ...
  $Header: //components/rel/rftech_tdscdma.mpss/3.11/ftm/src/ftm_tdscdma_calv3.c#1 $
...
11/02/15   jr      Fix internal Cal failure
09/17/15   kma     Added auto pin support for Pin cal
09/10/15   jz      Check and exit other tech on non-TDS devices
08/12/15   cac     Support for FBRx Gain Agc
07/10/15   ych     Added FTM exit mode in tear-down of radio
06/10/15   jak     Set ftm_tdscdma_current_mode for ESC cal
06/01/15   jak     Code clean up for ftm_tdscdma_set_secondary_chain()
06/01/15   jak     Add device parameter for ftm_tdscdma_set_mode()
05/30/15   jak     Remove KW warning because of ftm_tdscdma_calv3_tx_measure()
05/26/15   jak     Remove the dependency of FTM set TDS mode before cal node
...
01/13/15   jak     Keep div status consistent in radio setup and tear down
12/12/14   ych     Put TFW into standby state after TDS ESC Cal
...
/*----------------------------------------------------------------------------*/
extern boolean ftm_tdscdma_rx_div_enable; /*!< To track the current diversity status */
...
extern void ftm_tdscdma_config_fw_state 
(
...
ftm_rsp_pkt_type ftm_tdscdma_calv3_radio_setup(void *ftm_req_data)
{
  ...
  rfm_device_enum_type remapped_sdevice; /* remapped Slave device */
  ...
  /* 1st query mdevice assign to mdev*/
  rfc_common_get_remapped_device_info( &source_device_info , &remapped_device_info );
  mdev = remapped_device_info.device;

  /* 2nd query sdevice assign to sdev*/
  source_device_info.device = radio_setup->sdevice;
  rfc_common_get_remapped_device_info( &source_device_info , &remapped_device_info );
  sdev = remapped_device_info.device;  

  /* Extract the rest of Radio Setup parameter */
  mdev_state = radio_setup->mdevice_state;
  sdev_state = radio_setup->sdevice_state;
  num_of_chan = radio_setup->num_of_chan;

  /* Keep track of current prx/sec device for support for DR-DSDS RFcal and tx/rx split feature */
  ftm_tdscdma_current_pri_device = mdev;
  ftm_tdscdma_current_sec_device = sdev;
  u_tds_band.tdscdma_band = band;
  sys_band = rfcommon_core_convert_rfcom_band_to_sys_band(RFM_TDSCDMA_MODE_FTM, u_tds_band);
  ftm_tdscdma_current_tx_device = rfc_common_get_preferred_tx_device(sys_band);
  ...
  /* call this for setting up state macine & some variable */
  if ( ftm_tdscdma_pri_device_for_ftm_rfmode_enter == RFM_INVALID_DEVICE )
  {
    ftm_tdscdma_calv3_set_current_mode(band);
	
    for (int device_loop = 0;device_loop < RFM_MAX_DEVICES; device_loop++ )
    {
      FTM_MSG_1(FTM_MED,"Put tech to PARK mode for device = %d", device_loop);
      ftm_rfmode_enter(device_loop, FTM_STATE_PARK );
    }
  ...
  if ( mdev_state == MASTER_DEVICE_DISABLED )
  {
    ...
    if( mdev < RFM_MAX_DEVICES  )
    {
      ...
      /* KW fix */
      if ( ftm_tdscdma_current_tx_device < RFM_MAX_DEVICES )
      {
         /* we also need to take care tx device in rfm_mode due to tx/rx split feature */
	 ftm_rfmode_exit(ftm_tdscdma_current_tx_device, 0);	
      }
      else
      {
         FTM_MSG_1(FTM_ERROR, "Wrong tx device in teardown: %d!",ftm_tdscdma_current_tx_device);
      }
       ...
       ( mdev_state == MASTER_DEVICE_RXTX ) )
  {
    ...
    {
      if(ftm_tdscdma_rx_div_enable == FALSE)
      {
    ...
    {
    	rfdevice_tdscdma_tx_cmd_dispatch(ftm_tdscdma_current_tx_device, RFCOM_TDSCDMA_MODE, ftm_tdscdma_current_mode, RFDEVICE_GET_TX_AGC_LIMITS, &ftm_tdscdma_calv3_pdm_limits);
...
//  /* If it's a retune, don't take the wfm_type as the value is 0 
//     if it's dpd, after retune it should restore to dpd */
//  if ( mdev_state != MASTER_DEVICE_RETUNE )
  {
  ...
  while ( dev_mask != 0 )
  {
    ...
    if ( ( override_type & ( 1 << TX_OVR_POWER ) ) != 0 )
    {
      ...
      else if ( power_ovr_type == TX_OVR_POWER_ABS )
      {
        ...
        {
          ftm_tdscdma_calv3_apply_pin_alignment(ftm_tdscdma_current_tx_device,
                                                tx_override->
    ...
    if ( tx_override->debug_mode_en == TRUE )
    {
      FTM_MSG_6( FTM_HIGH, "ftm_tdscdma_calv3_sweep_tx_override: Dev %d , ftm_tdscdma_current_tx_device =%d "
  ...
  while ( dev_mask != 0 )
  {
    ...
    else
    {
           ...
           ( 1 << TX_MEAS_FB_GAIN ) ) != 0 )
      {
        ...
        fb_meas_inp.tx_device_id = ftm_tdscdma_current_tx_device;
        ...
        if ( fb_meas_res.status != RFCOMMON_FBRX_ERROR )
        {
          ...
          cal_result->tx_meas_param[device].fb_gain_meas.fb_meas_res.fbrx_mode = ftm_common_fbrx_get_mode(RFCOM_TDSCDMA_MODE, device, 0);
           ...
           ( 1 << TX_MEAS_FB_PWR ) ) != 0 )
      {
        ...
        hdet_obj = ftm_common_get_tuner_object(ftm_tdscdma_current_tx_device, RFDEVICE_HDET);
           ...
           ( 1 << TX_MEAS_FB_GAIN_AGC ) ) != 0 )
      {
        ...
        /* Fetch Input Params for FBRx capture */
		fb_meas_inp_agc.tx_device_id = ftm_tdscdma_current_tx_device;
        ...
        fb_meas_inp_agc.tech = RFCOM_TDSCDMA_MODE;
        fb_meas_inp_agc.band.tdscdma_band = rf_tdscdma_mc_state.curr_band_tx;
        ...
        if ( tx_measure->debug_mode_en == TRUE )
        {
		   FTM_MSG_9( FTM_MED, "TDSCDMA CalV3 DBG - FB Gain Cal Res: Status %d , Gain = %d,"
...
)
{
  ...
  /* Set FW to standby status*/
  //ftm_tdscdma_config_fw_state(RF_TDSCDMA_MDSP_TFW_STATE_STANDBY);

  /* call this for cleaning up state macine & some variable */
  if ( ftm_tdscdma_pri_device_for_ftm_rfmode_enter != RFM_INVALID_DEVICE )
  {
    ftm_rfmode_exit(ftm_tdscdma_pri_device_for_ftm_rfmode_enter , FTM_STATE_PARK );
  }
                                          ...
                                          boolean drx_enable_flag)
{
  ...
  {
    ftm_tdscdma_set_mode(mode, ftm_tdscdma_current_pri_device);
  }
...
)
{
  ...
  else
  {
    ...
    /* query RFC for remapped device ID */
    /* populate src query structure */
    source_device_info.device = payload->primary_device;
    source_device_info.tech = RFM_TDSCDMA_MODE;
    source_device_info.rxtx = RFC_CONFIG_RX;
    source_device_info.band = rf_tdscdma_mc_state.curr_band_tx;
    source_device_info.alt_path = 0; /* use 0 for alt_path for now */    

	/* query RFC to remap for primary device */
	rfc_common_get_remapped_device_info( &source_device_info , &remapped_device_info );
	u_tds_band.tdscdma_band = rf_tdscdma_mc_state.curr_band_tx;
	sys_band = rfcommon_core_convert_rfcom_band_to_sys_band(RFM_TDSCDMA_MODE,u_tds_band);
	remapped_tx_device = rfc_common_get_preferred_tx_device(sys_band);
    ...
    ret_val &= ftm_tdscdma_xpt_cal_config(remapped_tx_device,
                                         device_fb,
                                         payload->xpt_mode,
                                         TRUE );
    ...
    if ( rfcommon_xpt_is_capture_done() == TRUE )
    {
      ...
      if ( buff_alloc == TRUE )
      {
        ...
        if ((rfcommon_xpt_capture_type)payload->capture_type < XPT_CAPTURE_DELAY)
        {
  	    ...
  	    source_device_info.tech = RFM_TDSCDMA_MODE;
  	    source_device_info.rxtx = RFC_CONFIG_RX;
  	    source_device_info.band = rf_tdscdma_mc_state.curr_band_tx;  /* get band info from rf_tdscdma_mc_state strcutrue */
  	    source_device_info.alt_path = 0; /* use 0 for alt_path for now */  

		/*  query primary_device and assign to remapped device */
  		rfc_common_get_remapped_device_info( &source_device_info , &remapped_device_info );
		u_tds_band.tdscdma_band = rf_tdscdma_mc_state.curr_band_tx;
	        sys_band = rfcommon_core_convert_rfcom_band_to_sys_band(RFM_TDSCDMA_MODE,u_tds_band);
	        remapped_tx_device = rfc_common_get_preferred_tx_device(sys_band);
        ...
        FTM_MSG_9(FTM_MED, "ftm_tdscdma_cal_xpt_iq_capture: primary_device - %d, remapped_tx_device - %d, feedback_device - %d, capture_type - %d, num_samples - %d, first_trigger - %d, last_trigger - %d, proc_flag - %d, samp_rate - %d",
                  ...
                  &samp_rate);
        iq_cap_status =  ftm_tdscdma_xpt_iq_capture(remapped_tx_device,
                                                    payload->feedback_device,
                                                    payload->capture_type,
                                                    payload->num_samples,
                                                    payload->first_trigger,
        ...
        if ( iq_cap_status == FALSE )
        {
          ...
          {
          ftm_tdscdma_xpt_cal_config(remapped_tx_device,
                                     payload->feedback_device,
...
ftm_rsp_pkt_type ftm_tdscdma_calv3_feedback_setup(void *ftm_req_data)
{
  ...
  /* query RFC for remapped device ID */
  /* populate src query structure */
  source_device_info.device = feedback_setup->primary_device;
  source_device_info.tech = RFM_TDSCDMA_MODE;
  source_device_info.rxtx = RFC_CONFIG_RX;
  source_device_info.band = rf_tdscdma_mc_state.curr_band_tx;
  source_device_info.alt_path = 0; /* use 0 for alt_path for now */  

  /* query RFC for remapping device*/
  rfc_common_get_remapped_device_info( &source_device_info , &remapped_device_info );
  u_tds_band.tdscdma_band = rf_tdscdma_mc_state.curr_band_tx;
  sys_band = rfcommon_core_convert_rfcom_band_to_sys_band(RFM_TDSCDMA_MODE,u_tds_band);
  remapped_tx_device = rfc_common_get_preferred_tx_device(sys_band);
            ...
            FEEDBACK_SETUP_ACTION_MASK_CONFIGURE_RXLM_FOR_FBAGC )
  {
    ...
    {
      rf_tdscdma_fbrx_modem_chain = rfc_tdscdma_get_fb_chain(rf_tdscdma_mc_state.curr_tx_device,u_tds_band.tdscdma_band );
      ret_value = rfcommon_fbrx_enable( fbrx_lm_handle, rf_tdscdma_fbrx_modem_chain );
