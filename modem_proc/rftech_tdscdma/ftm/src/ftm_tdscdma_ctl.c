...
/*===========================================================================
  ...
  $Header: //components/rel/rftech_tdscdma.mpss/3.11/ftm/src/ftm_tdscdma_ctl.c#1 $
  $DateTime: 2021/03/09 05:09:33 $
  $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/11/18   chm     Add command handler for SECOND_CHAIN_TEST_CALL
03/17/18    rp     KW Fixes.
02/27/16   tik     Add support for FBRx RSB changes
01/13/16   nsh     To get preferred device during XO cal
12/21/15   jz      Use correct device id in rf_tdscdma_mdsp_get_raw_rx_agc()
12/10/15   jr      Phase 1 device status read
11/06/15   jr      Rx/Tx split feature on different WTR
10/31/15   jr      Enable XO cal on TDS node
09/22/15  jps/al   Add support for FTM_TDSCDMA_FBRX_SET_RX_TX_DELAY
09/10/15   dbz     [selftest] Add RSB correction for selftest captures
08/14/15   lwu     Add ftm_tdscdma_init() to do FTM TDSCDMA radio test initiate 
07/15/15   zbz     pass device id into ftm iq processing
07/15/15   jr      Add DR-DSDS RF Cal support
07/08/15   jak     Remove remap in ftm_tdscdma_set_mode
06/29/15   Saul    Use Tx device for fbrx-related RFC queries
06/27/15   jak     Use rx chain for ftm_tdscdma_get_rx_level_dbm
06/21/15   dbc     Add support for pa_tunable_cap API
06/17/15   ms/al   Add delay before RXAGC read to allow settling
06/15/15   jak     TDS FTM RSB cal & TDS FTM IQ capture fix 
06/09/15   jak     Switch Tx waveform back to SP before turn Tx off
06/09/15   nsh     lin state override support
06/01/15   jak     Use ftm current Tx device for ftm_tdscdma_xpt_cal_config/iq_capture()
06/01/15   jak     Code clean up for ftm_tdscdma_set_secondary_chain()
06/01/15   jak     Add device parameter for ftm_tdscdma_set_mode()
05/26/15   jak     Remove the checking criteria for TH in downloading uK
05/26/15   whz     Explicitly invalid cache for IQ capture
05/17/15   jr      Use dl_pwr in the ftm_iq_capture_analyze_samples_type structure
05/13/15   jak     Tx/Rx split feature - adaption to WTR interface change
04/14/15   jak     Tx/Rx split feature
04/01/15   al      Update self test search center
02/17/15   jr      Added embedded support for TDSCDMA Sensitivity/CNR calculation
...
01/13/15   jak     Update ftm_tdscdma_div_status in ftm_tdscdma_set_secondary_chain()
...
#define FTM_TDSCDMA_SELFTEST_SEARCH_CENTER 55
...
/* due to the following issue , we need to have this variable to track div state which is used for executing Rx burst script
Customer test procedure:
1.	Don’t set diversity flag (since they test prx only)
2.	Call tune_to_chan
3.	For prx only + Tx calibration (since we always calibrate Tx first, at the last of this step the phone is at Rx state)
4.	Set diversity flag = TRUE (use aux port to calibrate drx) 
5.	Call tune_to_chan again => During this step, RFSW will execute RxOff script (since the current state is Rx; need to shut off Rx for re-tune), since the flag has been set to Div_EN by Step 4, it will use prxdrx version of rxOff script which doesn’t exist and cause the crash – As Jory mentioned below. The script build is after the rxOff script execution and so it is too late
*/
boolean ftm_tdscdma_current_div_state = FALSE;

/* The purpose of having this variable is to track what is the device ID we used for ftm_rfmode_enter()
 So that we could use this one for ftm_rfmode_exit()*/
rfm_device_enum_type ftm_tdscdma_pri_device_for_ftm_rfmode_enter  = RFM_INVALID_DEVICE;
...
// Variable that is used as Response packet for GET IQ CAPTURE ANALYZE SAMPLES Command
static ftm_iq_capture_analyze_samples_rsp_pkt_type ftm_tdscdma_iq_capture_analyze_samples_rsp_pkt;
...
void ftm_tdscdma_set_lna_range(ftm_receive_chain_handle_type rx_chain, byte ftm_lna_range, byte ftm_rx_lin_state )
{
  ...
  if (ftm_lna_range < RF_TDSCDMA_MAX_LNA_GAIN_STATES)
  {
    ...
    rfdevice_tdscdma_rx_cmd_dispatch(device,
                                     ...
                                     rf_tdscdma_core_util_rx_mode_type(device),
...
int2 ftm_tdscdma_get_lna_offset(ftm_receive_chain_handle_type rx_chain, byte index, word expected_agc_val, byte rx_lin_state)
{
  ...
  if (index < RF_TDSCDMA_MAX_LNA_GAIN_STATES)
  {
    ...
    DALSYS_BusyWait(200); //add delay to allow settling before RXAGC read
...
===========================================================================*/
void ftm_tdscdma_set_mode(ftm_mode_id_type mode , rfm_device_enum_type prx_device)
{
   boolean change_mode = FALSE;
   rfm_device_enum_type drx_device; /* associated drx device */
   rfm_device_enum_type tx_device; /* associated tx device */   
   ...
   if (ftm_tdscdma_ctl_dbg_msg_flag == TRUE)
   {
   FTM_MSG_HIGH("ftm_tdscdma_set_mode:enter mode %d , prx_device %d",mode,prx_device,0);
   }

   /* then get drx & tx device base on remapped prx device */

   u_tds_band.tdscdma_band = ftm_tdscdma_util_convert_band_ftm_to_rfcom(mode);
   sys_band = rfcommon_core_convert_rfcom_band_to_sys_band(RFM_TDSCDMA_MODE, u_tds_band);


   if(rfc_common_check_band_supported(sys_band, prx_device) == FALSE)
   {
	 prx_device = rfc_common_get_preferred_rx_device(sys_band);
	 FTM_MSG_1(FTM_ERROR, "Device Phone mode Mismatch. Setting mode on the supported device %d"
		 "Allowed only in Xo Cal sequence", prx_device);
   }
   ...
   if (mode == FTM_PHONE_MODE_TDSCDMA_B34)
   {
      ...
      {
        ftm_tdscdma_current_pri_device = prx_device;
        ftm_tdscdma_current_sec_device = drx_device;
        ftm_tdscdma_current_tx_device = tx_device;
   ...
   /* assign the device ID for tracking ftm_rfmode_enter*/
   ftm_tdscdma_pri_device_for_ftm_rfmode_enter = ftm_tdscdma_current_pri_device;

   /* If it's already in DB_RF_TDSCDMA mode, skip setting the same mode again */
   if (ftm_get_current_state(ftm_tdscdma_pri_device_for_ftm_rfmode_enter) != FTM_STATE_TDSCDMA)
   {
      ...
      ftm_rfmode_enter(ftm_tdscdma_pri_device_for_ftm_rfmode_enter, FTM_STATE_TDSCDMA);
   ...
   if (ftm_tdscdma_ctl_dbg_msg_flag == TRUE)
   {
     FTM_MSG_HIGH("ftm_tdscdma_set_mode:done mode %d , prx_device %d",mode,prx_device,0);
   }
...
void ftm_tdscdma_set_tx(boolean state)
{
  ...
  if ( state == TRUE )
  {
    if(  (rfc_tdscdma_check_tx_capability(ftm_tdscdma_current_tx_device, ftm_tdscdma_current_mode)) == TRUE)
    {
  ...
  FTM_MSG_HIGH("ftm_tdscdma_set_tx() : state = %d , ftm_tdscdma_current_cal_state = %d , ftm_tdscdma_current_tx_device = %d "
  	, state , ftm_tdscdma_current_cal_state, ftm_tdscdma_current_tx_device);
...
/*===========================================================================
FUNCTION  FTM_TDSCDMA_SET_PA_TUNABLE_CAP        

DESCRIPTION
  This function applies an offsaet to the PA Tunable Cap.
...
SIDE EFFECTS
  None
===========================================================================*/
void ftm_tdscdma_set_pa_tunable_cap(byte pa_tunable_cap_offset)
{

  if (ftm_tdscdma_ctl_dbg_msg_flag == TRUE)
  {
      FTM_MSG_HIGH("ftm_tdscdma_set_pa_tunable_cap: offset %d", (int8)pa_tunable_cap_offset,0,0);
  }

  if ( !rfdevice_tdscdma_set_pa_output_cap( ftm_tdscdma_current_tx_device,
                                            ftm_tdscdma_current_mode, 
                                            (int8)pa_tunable_cap_offset) ) 
  {
    FTM_MSG_ERROR("ftm_tdscdma_set_pa_tunable_cap failed! device: %d, band: %d, offset: %d",
                  ftm_tdscdma_current_tx_device, ftm_tdscdma_current_mode, (int8)pa_tunable_cap_offset);
  }
...
void ftm_tdscdma_tune_to_chan(ftm_receive_chain_handle_type rx_chain, uint16 chan)
{
   ...
   if(!ftm_tdscdma_rx_div_enable)
   {
     ...
     if (rfm_get_calibration_state() && ftm_tdscdma_current_fw_state!= RF_TDSCDMA_MDSP_TFW_STATE_FTM_TRK)
     {
       // Primary chain only
   ...
   {
   /*enable tx*/
     if(  rfc_tdscdma_check_tx_capability(ftm_tdscdma_current_tx_device, ftm_tdscdma_target_mode))
     {
   ...
   if (ftm_tdscdma_cal_state_backup == FTM_TDSCDMA_CAL_STATE_TX)
   {
      ...
      {
        DALSYS_BusyWait(100);
        ftm_tdscdma_txagc_override_default();
        DALSYS_BusyWait(100);	  
...
//  FTM_MSG_HIGH("RxD nv value set to 0",0,0,0);
  ...
  FTM_MSG_HIGH("ftm_tdscdma_set_secondary_chain on_off = %d",on_off,0,0);
...
//#endif /* FTM_HAS_TDSCDMA_RX_DIVERSITY */
  ...
  ret_val = tdsrxd_set_rx_chain_status((tdsl1_cmapi_if_rx_status_type)second_chain);

  FTM_MSG_HIGH("FTM set RXD in NS chain:%d with ret_status %d", second_chain, ret_val, 0);
}
...
void ftm_tdscdma_toggle_modes( void )
{
      ...
      (ftm_current_rf_mode == FTM_DB_RF_GSM_1900) )
  {
    ...
    ftm_tdscdma_set_mode(FTM_PHONE_MODE_TDSCDMA_B34, ftm_tdscdma_current_pri_device);
    #ifdef FTM_HAS_GSM
  ...
  else
  {
    ...
    #endif
    ftm_tdscdma_set_mode(current_mode, ftm_tdscdma_current_pri_device);
  }
}
...
)
{
  ...
  /* assign tracking variable for tracking ftm_rfmode*/
  ftm_tdscdma_pri_device_for_ftm_rfmode_enter = device;
  ...
  if (ftm_tdscdma_ctl_dbg_msg_flag == TRUE)
  {
     FTM_MSG_HIGH("ftm_tdscdma_mode_enter:enter mode %d , DEV %d ",ftm_tdscdma_current_mode,device,0);
  }
...
)
{
  ...
  /* ftm_tdscdma_mode_exit() is often called while in tx. Wait 2ms here for TFW to properly
     shut down tx, before sending tfw_standby */
  ...
  ftm_tdscdma_pri_device_for_ftm_rfmode_enter  = RFM_INVALID_DEVICE;  
  ...
  if (ftm_tdscdma_ctl_dbg_msg_flag == TRUE)
  {
     FTM_MSG_HIGH("ftm_tdscdma_mode_exit: DEV %d ",device,0 ,0);
  }
...
)
{
  ...
  if (fw_state != RF_TDSCDMA_MDSP_TFW_STATE_STANDBY)
  {
    ...
    // Send this msg to TFW for the purpose of downloading uK 
...
)
{
  ...
  if (ftm_tdscdma_current_cal_state == FTM_TDSCDMA_CAL_STATE_RX)//Rx
  {
    ...
    if (rf_tdscdma_use_on_rumi == FALSE)
    {
      ...
      in_data.scriptInfo.rxOffScript.divEn = ftm_tdscdma_current_div_state; // shut off base on curr div state 
  ...
  else if (ftm_tdscdma_current_cal_state == FTM_TDSCDMA_CAL_STATE_TX)//Tx
  {
    ...
    {
	  // default the Tx waveform to SP 
	  ftm_tdscdma_switch_tx_waveform(FTM_TDSCDMA_TX_WAVEFORM_SPECIAL_BURST);	
    ...
    ** *******************************/
    ...
    else // non self-cal condition ( normal Rx Cal)
    {
      ...
      if (rf_tdscdma_use_on_rumi == FALSE)
      {
        ...
        ftm_tdscdma_current_div_state = in_data.scriptInfo.rxBurstScript.divEn; // record the div state we used for rxburst script
  ...
  else if (cal_state == FTM_TDSCDMA_CAL_STATE_TX)//Tx
  {
    ...
    ** *******************************/
    ...
    if (rf_tdscdma_use_on_rumi == FALSE)
    {
      ...
      in_data.scriptInfo.rxOffScript.callInCalMode = TRUE;	 
      in_data.scriptInfo.rxOffScript.bufId = 0; // Hard-coded bufID=0 for CAL mode
      in_data.scriptInfo.rxOffScript.divEn = ftm_tdscdma_rx_div_enable;
...
)
{
  ...
  ret_val = rf_tdscdma_mdsp_get_raw_rx_agc(ftm_tdscdma_get_rx_chain_handle(device), &mdsp_rxlvldBm10, &mdsp_rxlvlagc);
      ...
      if (status == FTM_RX_LEVEL_DBM_TEST_SUCCESS)
      {
        switch(i)
        {
           case 0:
             ...
             ret_val = rf_tdscdma_mdsp_get_raw_rx_agc(ftm_tdscdma_get_rx_chain_handle(device), 
           ...
           case 1:
             ...
             {                             
               ret_val = rf_tdscdma_mdsp_get_raw_rx_agc(ftm_tdscdma_get_rx_chain_handle(ftm_tdscdma_current_sec_device), 
                                                        &ftm_tdscdma_rx_level_dbm10[number_selected_rx_chains], 
                                                        &ftm_tdscdma_rx_level_agc[number_selected_rx_chains]);               
               if (ret_val == FALSE )
               {
  ...
  if(on_off)
  {
    switch(ftm_tdscdma_burst_tx_state)
    {
       case FTM_TDSCDMA_BURST_TX_OFF:
         ...
         {
           rflmTdsRfModeCtlState.isRfFtmContTxMode = TRUE;
         }
         else
         {
           rflmTdsRfModeCtlState.isRfFtmContTxMode = FALSE;
         }
...
void ftm_tdscdma_cmd_get_iq_data_from_fw(rfm_device_enum_type device)
{
    ...
    uint32 sample_size = (uint32)FTM_TDSCDMA_IQ_MAX_SAMPLES;

    /*Store the ant_port of IQ capture for future check , we assign ftm pri device which is assigned in ftm tdscdma set mode stage */
    ftm_tdscdma_iq_acquired_samples_data.device_with_IQ = ftm_tdscdma_current_pri_device;
    /*Set the data flag to false before getting IQ data*/
    ftm_tdscdma_iq_acquired_samples_data.data_avail_from_FW = FALSE;


    //FTM_MSG_HIGH("DEBUG RSB ftm_tdscdma_cmd_get_iq_data_from_fw: rfm_get_calibration_state = %d, ftm_tdscdma_current_fw_state= %d, device=%d", rfm_get_calibration_state(), ftm_tdscdma_current_fw_state, device);

    if (rfm_get_calibration_state())
    {  //User defined
     if (ftm_tds_iq_capture_data.SampleSize > 0 && ftm_tds_iq_capture_data.SampleSize < (uint32)FTM_TDSCDMA_IQ_MAX_SAMPLES)
     {
        sample_size = ftm_tds_iq_capture_data.SampleSize;
       // FTM_MSG_HIGH("DEBUG RSB ftm_tdscdma_cmd_get_iq_data_from_fw: ftm_tds_iq_capture_data.SampleSize = %d, ", ftm_tds_iq_capture_data.SampleSize, 0, 0);

     }
    }
    if (ftm_tdscdma_current_fw_state!= RF_TDSCDMA_MDSP_TFW_STATE_FTM_TRK)
    {
    ...
    rf_tdscdma_mdsp_send_mdsp_msg(RF_TDSCDMA_MDSP_IQ_CAPTURE_CFG_MSG, (((sample_size << 16) & 0xFFFF0000) | device ));

    error_code = ftm_tdscdma_rf_mode_blocking_cmd_handler(RF_TDSCDMA_MDSP_IQ_CAPTURE_CFG_MSG);

    FTM_MSG_HIGH("TDSCDMA IQ Capture: send cmd to FW to get data for ftm_tdscdma_current_pri_device:%d and error_code: %d", ftm_tdscdma_current_pri_device, error_code, 0);
}
...
ftm_iq_capture_error_code_type ftm_tdscdma_iq_capture_acquire_samples(rfcom_device_enum_type device, ftm_iq_capture_acquire_samples_type *data)
{
  ...
  if (ftm_tdscdma_iq_acquired_samples_data.device_with_IQ != ftm_tdscdma_current_pri_device)
  {
    FTM_MSG_ERROR( "ftm_tdscdma_current_pri_device = %d doesn't match the device that has IQ data captured from FW, device with IQ samples=%d",
      ftm_tdscdma_current_pri_device, ftm_tdscdma_iq_acquired_samples_data.device_with_IQ, 0);
    ret_value = FTM_IQ_GENERAL_FAILURE;
  }
  ...
  if(ftm_tdscdma_iq_acquired_samples_data.iq_buf == NULL)
 {
  ...
  if((ret_value == FTM_IQ_SUCCESS) && (NULL != raw_data_address))
  {
...
#if  ( (!(defined(TEST_FRAMEWORK))) && defined(FEATURE_TABASCO_MODEM) )    
    qurt_mem_cache_clean((qurt_addr_t)raw_data_address, 2*2*FTM_TDSCDMA_IQ_MAX_SAMPLES, QURT_MEM_CACHE_INVALIDATE, 1);    
#endif
    
    /*Get the samples from share mem with FW to the IQ data buffer allocated - No format conversion required for TABASCO*/
    #ifdef FEATURE_TABASCO_MODEM
...
//TFW does IQ capture only in TRK state (CX2). Hence only 20pcfl conversion is needed.
#if 0   
   // if (rfm_get_calibration_state())
   // {
   ...
   //     ftm_tdscdma_iq_acquired_samples_data.iq_buf, (uint32)(FTM_TDSCDMA_IQ_MAX_SAMPLES));
    //  FTM_MSG_HIGH("HACK %d TDSCDMA IQ samples sucessfully acquired for path %d using 10pcfl",
...
FUNCTION FTM_TDSCDMA_IQ_CAPTURE_ANALYZE_SAMPLES

DESCRIPTION
  This API is used to analyze the IQ data captured using FTM_IQ_CAPTURE_ACQUIRE_SAMPLES 
  command
...
DEPENDENCIES
  The IQ Capture ACQUIRE command should have been issued.
...
SIDE EFFECTS
 None
=============================================================================*/
ftm_rsp_pkt_type ftm_tdscdma_iq_capture_analyze_samples(rfcom_device_enum_type device, void *ftm_req_data)
{
  ftm_rsp_pkt_type rsp_pkt = {FTM_RSP_NO_RSP, 0, NULL, FALSE};  
  ftm_diagpkt_subsys_header_type *cmd_ptr = ( ftm_diagpkt_subsys_header_type*)ftm_req_data;
  ftm_pkt_type *cmd_ptr_pkt = (ftm_pkt_type *)ftm_req_data;
  ftm_iq_capture_analyze_samples_type *cmd_ptr_pkt_for_analyze_samples = NULL; 
  static int32 analysis_result[FTM_IQ_CAPTURE_ANALYZE_SAMPLES_MAX_PAYLOAD_SIZE];

  ftm_iq_capture_get_config_type cmd_ptr_pkt_for_get_config = {FTM_IQ_SUCCESS, 0,0,0,0}; 
  ftm_pkt_type ftm_pkt_type_obj; // create an object for acquire_samples_handler() api
  ftm_iq_capture_acquire_samples_type *cmd_ptr_pkt_for_acq_samples = NULL; 
 ...
 if(cmd_ptr == NULL )
  {
   ...
   FTM_MSG_ERROR("Message pointer is NULL",0,0,0);
   return(rsp_pkt);
  } 

  /* Get info from req packet */
  cmd_ptr_pkt_for_analyze_samples = &(cmd_ptr_pkt->rf_params.ftm_rf_factory_data.ftm_iq_capture_cmd.ftm_iq_capture_analyze_samples);
  analysis_id = cmd_ptr_pkt_for_analyze_samples->analysis_id;
  rx_chain =    cmd_ptr_pkt_for_analyze_samples->select_rx_chain;
  dl_pwr_lvl = (int16)(cmd_ptr_pkt_for_analyze_samples->dl_pwr & 0xffff);
  
  cmd_ptr_pkt_for_acq_samples = &(ftm_pkt_type_obj.rf_params.ftm_rf_factory_data.ftm_iq_capture_cmd.ftm_iq_capture_acquire_samples); 
  /*We need this for Acq samples handler   */
  cmd_ptr_pkt_for_acq_samples->sample_size = cmd_ptr_pkt_for_analyze_samples->sample_size;
  /* This should come from tools */
  cmd_ptr_pkt_for_acq_samples->carrier_id = 0x0 ; //LTE -- RFLTE_MC_PCELL_IDX
  
  /*  To get the Sampling Frequency we need to call this api    */
  ftm_tdscdma_iq_capture_get_config(&cmd_ptr_pkt_for_get_config);

  /* Acquire IQ samples  */
  ftm_tdscdma_iq_capture_acquire_samples(device, cmd_ptr_pkt_for_acq_samples);

  FTM_MSG_3(FTM_MED, "rx_chain = %d and num_samples = %d and samp_freq = %d", 
            rx_chain, cmd_ptr_pkt_for_analyze_samples->sample_size,
            cmd_ptr_pkt_for_get_config.sampling_freq);
  
  if (ftm_tdscdma_iq_acquired_samples_data.data_available == FALSE )
  {
    status = FTM_IQ_IQ_SAMPLES_UNAVAILABLE_ON_RFM_DEV;
  }
  
  else if(ftm_tdscdma_iq_acquired_samples_data.num_req_samples > FTM_TDSCDMA_IQ_MAX_SAMPLES)
  {
    status = FTM_IQ_PROCESSING_ERROR;
  }
  else if(ftm_tdscdma_iq_acquired_samples_data.iq_buf == NULL)
  {
  ...
  {    
    ...
    {
       if (enable_cton | enable_estimated_sensitivity)
       {
          ftm_common_analyze_iq_samples(&sens_est_result, ftm_tdscdma_iq_acquired_samples_data.iq_buf, 
                                        cmd_ptr_pkt_for_analyze_samples->sample_size,
                                        cmd_ptr_pkt_for_get_config.sampling_freq, FTM_RF_TECH_TDSCDMA, dl_pwr_lvl);
          ...
          FTM_MSG_3(FTM_HIGH, "ftm_tdscdma_iq_capture_analyze_samples_handler - "
  ...
  FTM_MSG_4(FTM_MED, "Done processing IQ samples - Status = %d and Analysis ID = %d and "
                     "EnableCToN = %d and EnableEst.Sens = %d",
                     status, analysis_id,
                     enable_cton, enable_estimated_sensitivity); 

  // Send the response pkt.
  ftm_tdscdma_iq_capture_analyze_samples_rsp_diag_pkt(&rsp_pkt, status, cmd_ptr->subsys_cmd_code, analysis_id, payload_size,(int8 *) analysis_result);
  return(rsp_pkt);
}
...
void ftm_tdscdma_iq_capture_analyze_samples_rsp_diag_pkt(ftm_rsp_pkt_type *rsp_pkt_ptr, ftm_iq_capture_error_code_type status,diagpkt_subsys_cmd_code_type cmd_id,uint32 analysis_id,uint16 size,int8 *analysis_result)
{
  if (rsp_pkt_ptr != NULL)
  {
   // Fill in the constant fields of response packet. Sample buffer will be filled after sanity check.
   ftm_tdscdma_iq_capture_analyze_samples_rsp_pkt.cmd_code = 75;
   ftm_tdscdma_iq_capture_analyze_samples_rsp_pkt.subsys_id = FTM;
   ftm_tdscdma_iq_capture_analyze_samples_rsp_pkt.subsys_cmd_code = cmd_id;
   ftm_tdscdma_iq_capture_analyze_samples_rsp_pkt.ftm_rf_cmd = FTM_IQ_CAPTURE_ANALYZE_SAMPLES;  
   ftm_tdscdma_iq_capture_analyze_samples_rsp_pkt.ftm_error_code= (uint16)status;
   ftm_tdscdma_iq_capture_analyze_samples_rsp_pkt.analysis_id = analysis_id;
   ftm_tdscdma_iq_capture_analyze_samples_rsp_pkt.reserved = (uint32)0;
   ftm_tdscdma_iq_capture_analyze_samples_rsp_pkt.payload_size=size;   
   
   // Fill in the constant fields of response packet.Size of the packet will be based on the requested sample size.
   rsp_pkt_ptr->cmd = FTM_RSP_DO_NEW_PKT;   
   rsp_pkt_ptr->pkt_payload = &(ftm_tdscdma_iq_capture_analyze_samples_rsp_pkt);
   ...
   {
      memscpy(ftm_tdscdma_iq_capture_analyze_samples_rsp_pkt.analysis_result,size,analysis_result,size);
...
)
{
  ...
  ftm_tdscdma_set_mode( rsp_pkt->curr_band, ftm_tdscdma_current_pri_device );
...
#endif  
   // when we are here , we should have valid current_tx_device
  device = ftm_tdscdma_current_tx_device;
  ...
  if (config)
  {
    ...
    default:
      ...
      if (!ftm_tdscdma_xpt_fb_path_buffer.is_buf_allocated)
      {
        ...
        ret_val &= rf_tdscdma_msm_update_dynamic_rxlm_buffer_xpt(ftm_tdscdma_xpt_fb_path_buffer.buf_idx, (rfm_device_enum_type)device,
                                                                 ftm_tdscdma_current_mode);
        ...
        rfcommon_msm_update_fbrx_iqmc(ftm_tdscdma_xpt_fb_path_buffer.buf_idx, 
                                      device,
         ...
         fbrx_band.tdscdma_band = ftm_tdscdma_current_mode;
  
         ret_val &= rfcommon_fbrx_mc_update_rsb_coeff( device,
						       ftm_tdscdma_txlm_buffer.buf_idx, 
			                               (lm_handle_type)ftm_tdscdma_xpt_fb_path_buffer.buf_idx, 
						       RFM_TDSCDMA_MODE, 
						       fbrx_band );																 
#endif
        ...
        ret_val &= rfc_tdscdma_setup_fb_path(device,
                                             ftm_tdscdma_current_mode,
                                             TRUE);
...
)
{
  ...
  case FTM_COMMON_XPT_TX_OVERRIDE_W_RC:
    final_delay = delay;
#if 0
    /* This API is not supported on wtr3925 */
    rfdevice_techX_tx_get_et_delay_compensation(device,
                                                ftm_tdscdma_current_mode,
                                                &delay_comp_x10ns);
    final_delay += delay_comp_x10ns;
#endif
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "ftm_tdscdma_xpt_process_override_txagc  delay_override_W_RC not supported", 0) ;
    break;

  case FTM_COMMON_XPT_TX_OVERRIDE_NV:
    final_delay = rf_tdscdma_core_xpt_path_delay_comp(
  ...
  case FTM_COMMON_XPT_TX_OVERRIDE_NV_WITH_TEMP_COMP:
    ...
    therm_scaled_value = rf_tdscdma_core_temp_comp_scale_therm(therm_raw_value, FALSE);
...
)
{
  ...
  if ( NULL == device_ptr)
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "ftm_tdscdma_xpt_override_rf_settings: Failed to get a valid pointer, ftm_tdscdma_current_tx_device = %d", ftm_tdscdma_current_tx_device);  	
  	return FALSE;
  }
  ...
  if ( rfcommon_debug_flags.use_rflm_txagc_override )
  {
    ...
    ret_val &= ftm_tdscdma_xpt_process_override_txagc( ftm_tdscdma_current_tx_device, 
                                                       params->xpt_mode,         
                                                       amam_bank_idx,    
                                                       amam_start_offset,
                                                       amam_tbl_size,    
    ...
    tech_band.tdscdma_band = ftm_tdscdma_current_mode;
    ret_val &= rfm_ftm_apply_xpt_tx_agc_override( RFM_TDSCDMA_MODE, ftm_tdscdma_current_tx_device,
                                                  tech_band );  	
  }
  else
  {	
  	/* Send override to RF*/
    ret_val &= ftm_tdscdma_xpt_override_txagc( ftm_tdscdma_current_tx_device,
                                               params->xpt_mode,
                                               amam_bank_idx,
                                               amam_start_offset,
                                               amam_tbl_size,
                                   ...
                                   uint32 * samp_rate)
{
  ...
  if ((rfcommon_xpt_capture_type)xpt_capture_type == XPT_CAPTURE_DELAY)
  {
    ...
    FTM_MSG_4(FTM_MED, "ftm_tdscdma_xpt_iq_capture: Push to delay map - Chan: %d, delayCapType: %d, CaptBuf: %d, ResBuf: %d",
              ftm_tdscdma_current_chan,
...
)
{
  ...
  if (selftest_error == FTM_FBRX_TEST_SUCCESS)
  {
    ...
    FTM_MSG_1(FTM_HIGH, "Selftest Time profiling TDSCDMA ExecTime %d ms", prof_time);
  }
...
FUNCTION ftm_tdscdma_txagc_override_default

DESCRIPTION
  This API is used to override txagc to default settings 
...
SIDE EFFECTS
  None
=============================================================================*/
void ftm_tdscdma_txagc_override_default(void)
{
  ...
  ftm_tdscdma_xpt_override_rf_settings( &ovr_params );
}

void ftm_tdscdma_override_tfw_ftm_trk_state()
{
   uint8 wb_adc_idx;
   ...
   DALSYS_BusyWait(2000);   
        ...
        /* It is required to reconfig fw state for starting iq capture */
   ftm_tdscdma_config_fw_state(RF_TDSCDMA_MDSP_TFW_STATE_STANDBY);
   FTM_MSG_HIGH("ftm_tdscdma_override_tfw_ftm_trk_state: send TFW to STANDBY", 0, 0, 0);
   ftm_tdscdma_config_fw_state(RF_TDSCDMA_MDSP_TFW_STATE_FTM_TRK);
   FTM_MSG_HIGH("ftm_tdscdma_override_tfw_ftm_trk_state: send TFW to FTM_TRK", 0, 0, 0);


   //Update RXLM buffer with TRK settings and config TFW
   rf_tdscdma_msm_update_rxlm_buffer(ftm_tdscdma_current_pri_device, ftm_tdscdma_current_chan, RFA_TDSCDMA_RXLM_MODE_TRK, ftm_tdscdma_rxlm_buffer[FTM_RECEIVE_CHAIN_0].buf_idx, &wb_adc_idx, RFDEVICE_PRX_OPER_MODE);

   rf_tdscdma_mdsp_send_mdsp_msg(RF_TDSCDMA_MDSP_RXLM_CFG_MSG,ftm_tdscdma_rxlm_buffer[FTM_RECEIVE_CHAIN_0].buf_idx);
   FTM_MSG_HIGH("ftm_tdscdma_override_tfw_ftm_trk_state: send TFW RXLM CFG. buf_idx = %d",
               ftm_tdscdma_rxlm_buffer[FTM_RECEIVE_CHAIN_0].buf_idx, 0, 0);

   if(ftm_tdscdma_rx_div_enable)
   {
      //Update RXLM buffer with TRK settings and config TFW
      rf_tdscdma_msm_update_rxlm_buffer(ftm_tdscdma_current_sec_device, ftm_tdscdma_current_chan, RFA_TDSCDMA_RXLM_MODE_TRK, ftm_tdscdma_rxlm_buffer[FTM_RECEIVE_CHAIN_1].buf_idx, &wb_adc_idx, RFDEVICE_DRX_OPER_MODE);

      rf_tdscdma_mdsp_send_mdsp_msg(RF_TDSCDMA_MDSP_RXDIVLM_CFG_MSG,(ftm_tdscdma_rxlm_buffer[FTM_RECEIVE_CHAIN_0].buf_idx<<4) + ftm_tdscdma_rxlm_buffer[FTM_RECEIVE_CHAIN_1].buf_idx);
      FTM_MSG_HIGH("ftm_tdscdma_override_tfw_ftm_trk_state: send TFW RXLM_RXD CFG. buf_idx = %d",
                    (ftm_tdscdma_rxlm_buffer[FTM_RECEIVE_CHAIN_0].buf_idx<<4) + ftm_tdscdma_rxlm_buffer[FTM_RECEIVE_CHAIN_1].buf_idx, 0, 0);
   ...
   {
      FTM_MSG_HIGH("skipping ftm_tdscdma_override_tfw_ftm_trk_state",
                    0, 0, 0);
   }
...
void ftm_tdscdma_override_tfw_ftm_cal_state()
{
   uint8 wb_adc_idx;
   ...
   DALSYS_BusyWait(2000);
   ftm_tdscdma_config_fw_state(RF_TDSCDMA_MDSP_TFW_STATE_STANDBY);
   FTM_MSG_HIGH("ftm_tdscdma_override_tfw_ftm_cal_state: send TFW to STANDBY", 0, 0, 0);
   ftm_tdscdma_config_fw_state(RF_TDSCDMA_MDSP_TFW_STATE_FTM_CAL);
   FTM_MSG_HIGH("ftm_tdscdma_override_tfw_ftm_cal_state: send TFW to FTM_CAL", 0, 0, 0);



   //Update RXLM buffer with ACQ settings and config TFW
   rf_tdscdma_msm_update_rxlm_buffer(ftm_tdscdma_current_pri_device, ftm_tdscdma_current_chan, RFA_TDSCDMA_RXLM_MODE_ACQ, ftm_tdscdma_rxlm_buffer[FTM_RECEIVE_CHAIN_0].buf_idx, &wb_adc_idx, RFDEVICE_PRX_OPER_MODE);

   rf_tdscdma_mdsp_send_mdsp_msg(RF_TDSCDMA_MDSP_RXLM_CFG_MSG,ftm_tdscdma_rxlm_buffer[FTM_RECEIVE_CHAIN_0].buf_idx);
   FTM_MSG_HIGH("ftm_tdscdma_override_tfw_ftm_cal_state: send TFW RXLM CFG. buf_idx = %d",
               ftm_tdscdma_rxlm_buffer[FTM_RECEIVE_CHAIN_0].buf_idx, 0, 0);

   if(ftm_tdscdma_rx_div_enable)
   {
      //Update RXLM buffer with ACQ settings and config TFW
      rf_tdscdma_msm_update_rxlm_buffer(ftm_tdscdma_current_sec_device, ftm_tdscdma_current_chan, RFA_TDSCDMA_RXLM_MODE_ACQ, ftm_tdscdma_rxlm_buffer[FTM_RECEIVE_CHAIN_1].buf_idx, &wb_adc_idx, RFDEVICE_DRX_OPER_MODE);

      rf_tdscdma_mdsp_send_mdsp_msg(RF_TDSCDMA_MDSP_RXDIVLM_CFG_MSG,(ftm_tdscdma_rxlm_buffer[FTM_RECEIVE_CHAIN_0].buf_idx<<4) + ftm_tdscdma_rxlm_buffer[FTM_RECEIVE_CHAIN_1].buf_idx);
      FTM_MSG_HIGH("ftm_tdscdma_override_tfw_ftm_cal_state: send TFW RXLM_RXD CFG. buf_idx = %d",
                    (ftm_tdscdma_rxlm_buffer[FTM_RECEIVE_CHAIN_0].buf_idx<<4) + ftm_tdscdma_rxlm_buffer[FTM_RECEIVE_CHAIN_1].buf_idx, 0, 0);
   ...
   {
      FTM_MSG_HIGH("skipping ftm_tdscdma_override_tfw_ftm_cal_state",
                    0, 0, 0);
   }
...
/*!
  @brief
  Does the TDSCDMA FTM radio test init
 
  @details
  This function is used to do the FTM TDSCDMA radio test init.
  All the APIs to be called by the common FTM RF test framework should be registered here
*/
void ftm_tdscdma_init()
{
  /*Register the Radio config Command*/
  ftm_rf_test_radio_config_register_tech_cb(RFM_TDSCDMA_MODE, ftm_tdscdma_start_radiotest);
  /*Register the rx measure*/
  ftm_rf_test_rx_measure_register_tech_cb(RFM_TDSCDMA_MODE, ftm_tdscdma_rftest_rx_measure);
  /*Register the tx control*/
  ftm_rf_test_tx_control_register_default_tech_cb(RFM_TDSCDMA_MODE,ftm_tdscdma_rftest_tx_control);
  /*Register all future commands here*/
}

uint16 ftm_tdscdma_get_lna_state()
{
  return ftm_tdscdma_burst_rx_params_shadow.computed_lna_state;
}

void ftm_tdscdma_fbrx_set_rx_tx_delay(boolean mode, uint32 delay)
{
  FTM_MSG_2(FTM_HIGH, "ftm_tdscdma_fbrx_set_rx_tx_delay(): mode: %d, fbrx_rx_tx_delay: %d", mode, delay);
  rflm_tds_mc_set_fbrx_rx_tx_delay(mode, delay);
}
...
/*!
  @brief
  This function handles the FTM command to read device status registers.
...
void ftm_tdscdma_device_status_read_handler(void *ftm_req_data)
{

    ftm_tdscdma_device_status_read_request_packet_type *header =
	                         (ftm_tdscdma_device_status_read_request_packet_type *)ftm_req_data;
    
    uint8   device_id;           /*!< device_id */
    uint8   channel;             /*!< channel */
    uint8   slave_id;            /*!< slave_id*/
    uint16  reg;                 /*!< register*/
    uint8   extended_type;       /*!< extended type*/  
    uint8   rx_tx_type;          /*!< extended type*/ 
    ...
    rx_tx_type = header->rx_tx_type;
	
    FTM_MSG_6( FTM_HIGH, "ftm_tdscdma_device_status_read_handler: device_id %d, channel %d, slave_id 0x%X, register 0x%X, extended type %d rx_tx_type =%d",
	                  device_id, channel, slave_id, reg, extended_type, rx_tx_type);

    rf_tdscdma_mc_set_device_status_registers(device_id, channel, slave_id, reg, extended_type, rx_tx_type);
...
/*!
  @brief
  This function handles the FTM command to enable/disable device status read.
...
void ftm_tdscdma_device_status_read_enable_handler(void *ftm_req_data)
{

   ftm_tdscdma_device_status_enable_request_packet_type *header =
	(ftm_tdscdma_device_status_enable_request_packet_type *)ftm_req_data;
    
    uint8   device_id;           /*!< device_id */      
    boolean enable = FALSE;
    device_id = header->device_id;
    enable = header->debug_en;
	
    FTM_MSG_2( FTM_HIGH, "ftm_tdscdma_device_status_read_enable_handler: device_id %d, enable %d",
	                  device_id, enable);

    rf_tdscdma_mc_set_device_status_enable(device_id, enable);
