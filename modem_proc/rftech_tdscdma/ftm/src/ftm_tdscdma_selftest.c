...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_tdscdma.mpss/3.11/ftm/src/ftm_tdscdma_selftest.c#1 $
...
)
{
  ...
  ftm_rf_tdscdma_fbrx_meas_tx_params_rsp_pkt->subsys_cmd_code = FTM_TDSCDMA_C;
...
)
{
  ...
  ftm_common_selftest_get_coupler_script(RFM_TDSCDMA_MODE, com_band, params.device);
...
boolean ftm_tdscdma_obtain_fbrx_gain_switchpoint_nv_item(const rfcom_tdscdma_band_type band, rfnv_item_id_enum_type *nv_item)
{
  boolean ret_value = TRUE;
...
boolean ftm_tdscdma_obtain_fbrx_gain_stage(uint8 * gain_stage)
{
  rfcom_tdscdma_band_type band;
  rfnv_item_id_enum_type nv_id;
  boolean nv_valid;
  ...
  // determine the correct NV item location
  nv_valid = ftm_tdscdma_obtain_fbrx_gain_switchpoint_nv_item(band, &nv_id);
