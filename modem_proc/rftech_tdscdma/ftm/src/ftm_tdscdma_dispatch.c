...
/*===========================================================================
  ...
  $Header: //components/rel/rftech_tdscdma.mpss/3.11/ftm/src/ftm_tdscdma_dispatch.c#1 $
...
10/10/15   jr      ADC read support with Rx/Tx split
09/22/15  jps/al   Add support for FTM_TDSCDMA_FBRX_SET_RX_TX_DELAY 
06/23/15   jak     Use FTM curr pri/sec device to check pri/sec operation
06/21/15   dbc     Added support for pa_tunable_cap API
...
ftm_receive_chain_handle_type ftm_tdscdma_get_rx_chain_handle( rfcom_device_enum_type device)
{
  ...
  /* Because of Tx/Rx split change , we could not always assume device 0 is pri or device 1 is sec , as 
    we set curr pri & sec device at ftm tds set mode time , we could compare the match one and know this 
    is pri or sec operation or not */

  if ( device == ftm_tdscdma_current_pri_device ) 
  {
    retval = FTM_RECEIVE_CHAIN_0;
  }
  if ( device == ftm_tdscdma_current_sec_device )
  {
...
ftm_rsp_pkt_type ftm_tdscdma_dispatch(rfm_device_enum_type device, ftm_pkt_type  *cmd_ptr)
{
  ...
  {
  case FTM_SET_MODE:
    ftm_tdscdma_set_mode(cmd_ptr->rf_params.ftm_rf_factory_data.mode, device);
    break;
  ...
  case FTM_SET_PA_TUNABLE_CAP:
    ftm_tdscdma_set_pa_tunable_cap((byte) cmd_ptr->rf_params.ftm_rf_factory_data.gen_b);
    break;
  ...
  case FTM_GET_ADC_VAL:
    ...
    {	
      if (is_wtr_hdet)
      {        
        rfdevice_tdscdma_tx_cmd_dispatch(rf_tdscdma_mc_get_curr_tx_device(), 
                                         RFCOM_TDSCDMA_MODE, 
      ...
      {
        (void)rfdevice_hdet_tdscdma_tx_hdet_read(rf_tdscdma_mc_get_curr_tx_device(),
                                                 RFCOM_BAND_TDSCDMA_ANY_BAND,
                                                 FALSE, 
                                                 &adc_data,
                                                 NULL,
 ...
 */
  ...
  case FTM_IQ_CAPTURE_ANALYZE_SAMPLES:
    rsp_pkt = ftm_tdscdma_iq_capture_analyze_samples(device,cmd_ptr);
    break;
 ...
 case FTM_INT_CALV3_RADIO_SETUP:
  ...
  case FTM_GET_TUNE_CODE:
    rsp_pkt = ftm_common_process_get_tune_code(cmd_ptr, 
                                               device,
                                               FTM_STATE_TDSCDMA,
  ...
  case FTM_TDSCDMA_FBRX_SET_RX_TX_DELAY:
    ftm_tdscdma_fbrx_set_rx_tx_delay(cmd_ptr->rf_params.ftm_rf_factory_data.ftm_tdscdma_fbrx_set_rx_tx_delay.mode,
                                     cmd_ptr->rf_params.ftm_rf_factory_data.ftm_tdscdma_fbrx_set_rx_tx_delay.fbrx_rx_tx_delay);
  break;

  case FTM_TDSCDMA_DEVICE_STATUS_READ:
    FTM_MSG_HIGH("ftm_tdscdma_dispatch: Calling ftm_tdscdma_device_status_read_handler for FTM_TDSCDMA_DEVICE_STATUS_READ case - %d",
   	           cmd_ptr->rf_params.ftm_rf_cmd,0,0 );
    ftm_tdscdma_device_status_read_handler(cmd_ptr);
    break;

   case FTM_TDSCDMA_DEVICE_STATUS_READ_ENABLE:
     FTM_MSG_HIGH("ftm_tdscdma_dispatch: Calling ftm_tdscdma_device_status_read_handler for FTM_TDSCDMA_DEVICE_STATUS_READ_ENABLE case - %d",
	            cmd_ptr->rf_params.ftm_rf_cmd,0,0 );
     ftm_tdscdma_device_status_read_enable_handler(cmd_ptr);
     break;   
