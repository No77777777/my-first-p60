...
$Header: //components/rel/rftech_tdscdma.mpss/3.11/rf/rfc/src/rfc_tdscdma.cpp#1 $

when       who     what, where, why
-------------------------------------------------------------------------------
01/21/20   hal     Support QSH minidump for RFTECH TDSCDMA
03/04/18   nsh     Support QAT3522 for TDSCDMA
05/05/16   jr      Reduce F3 messages
03/02/16   jr      Get FBRX Chain from RFC files
01/07/16   jr      Reduce F3 messages
12/20/15   hm      Move the Dummy tuner manager creation outside of ASDIV
...
// Get the address of rfc_tdscdma_ptr
rfc_tdscdma ** rfc_tdscdma::get_instance_address (void)
{
   return &rfc_tdscdma_ptr;
}
                                          ...
                                          rfc_signal_type sig_type, rfc_rxtx_enum_type rx_tx, boolean set_grfc, rfc_logic_type override_logic)
{
   ...
   if ( rf_signals != NULL )
   {
     ...
     /* Configure the specified GRFC signals to HW */
     rfc_common_config_signal( rf_card_sig_list_notime, rfcommon_get_gnss_blanking_is_enabled( RFCOM_TDSCDMA_MODE, band ) );
   }
}
  ...
  if (cfg->port_state == RFC_PORT_STATE_A)
  {
    ...
    while (i < device_info_ptr->num_devices)
    {
      ...
      switch(dev_type)
      {
        ...
        case RFDEVICE_LNA:
          if (cmn_dev_obj != NULL)
          {
            if (elna_instance < RFC_MAX_DEVICE_INSTANCES_PER_BAND)
            {
              tdscdma_rf_devices[cfg->logical_device][cfg->rx_tx][cfg->band][dev_type][elna_instance] = cmn_dev_obj;
              elna_instance++;
            }
            else
            {
              RF_MSG_1(RF_FATAL, "Unsupported number of eLNA instances!", 0); 
            }
            ...
            if(cfg->rx_tx == RFC_CONFIG_RX)
            {
              rfdevice_lna_cfg lna_cfg;

              lna_cfg.band.tdscdma_band = (rfcom_tdscdma_band_type)cfg->band;
              lna_cfg.mode = RFCOM_TDSCDMA_MODE;
              lna_cfg.rfm_device = cfg->logical_device;
              
              /* Update the Rx LNA Port state with temp_port_state and Call the LNA API below */
              ((rfdevice_lna *)cmn_dev_obj)->set_band_map(&lna_cfg,                                                          
                                                          device_info_ptr->rf_asic_info[i].data, 
                                                          RFC_ASIC_INFO_DATA_SIZE
                                                          );
            }            
          ...
          if (cmn_dev_obj != NULL)
          {
            ...
            { 
              if ( (cfg->rx_tx == RFC_CONFIG_TX) || (cfg->rx_tx == RFC_CONFIG_RX) )
              {
                ...
                tdscdma_rf_devices[cfg->logical_device][cfg->rx_tx][cfg->band][RFDEVICE_TUNER_MANAGER][tuner_instance] = cmn_dev_obj;
                tuner_instance++;
              }
            }
          }
     ...
     )
   {
    ...
    if (tuner_instance == 0 ) 
    {
      ...
      {
        tdscdma_rf_devices[cfg->logical_device][cfg->rx_tx][cfg->band][RFDEVICE_TUNER_MANAGER][tuner_instance] = tuner_mgr_obj;
        ...
        if ( asd_tuner_mgr != NULL )
        {
          ...
          if (((rfcommon_asdiv_tuner_manager*)asd_tuner_mgr)->is_dummy_tuner_needed(ant_num))   
          {
           (void)((rfcommon_asdiv_tuner_manager*)asd_tuner_mgr)->update_rfc_asdiv_config_info_dtuner(
                                                                                        ...
                                                                                        RFDEVICE_TUNER,  \
                                                                                        tuner_instance);
           MSG_2( MSG_SSID_RF_DEBUG, MSG_LEGACY_HIGH, "ASDIV: Dummy tuner for ant=%d, 0x%x", ant_num, tuner_mgr_obj);
...
boolean rfc_tdscdma::init_rfdevices_for_all_bands(void)
{
   ...
   */
  MSG(MSG_SSID_RF_DEBUG, MSG_LEGACY_HIGH, "Processing asdiv_config info table for TDSCDMA...");
       ...
       if(rfc_data->devices_cfg_data_get(&cfg, &device_info_ptr)== TRUE)
       {
           ...
           )
         {
              RF_MSG_3( RF_ERROR, " rfc_set_fbrx_mapping() failed for rfm device %d. "
                                  ...
                                  rfm_device,RFCOM_TDSCDMA_MODE,band);
         } 
       }
    }
  }
...
*/
uint32 rfc_tdscdma::get_fb_chain(rfm_device_enum_type tx_device, rfcom_tdscdma_band_type tx_band)
{
  rfc_cfg_params_type cfg;
  rfc_device_info_type *device_info_ptr = NULL;
  rfc_tdscdma_data *rfc_tdscdma_data_ptr = NULL;
  uint32 modem_chain = 1; //Default chain for FBRX on Jacala
  ...
  if (device_info_ptr == NULL)
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"get_fb_chain: RFC TDSCDMA Data device_info_ptr for Tx is NULL", 0);
       ...
       {           
         case RFDEVICE_XSW:
           ...
           if( device_obj != NULL )
           {
		      ...
		      status = rfdevice_xsw_wakeup(&xsw_config,(void*)device_obj,asdiv_script_ptr,script_exec_type,0);	//am
              xsw_config.trigger = RFDEVICE_XSW_TRIGGER_NONE; 
			  status = rfdevice_xsw_set_cross_switch_config(&xsw_config,(void*)device_obj,asdiv_script_ptr,script_exec_type,0);	//am
			  xsw_config.trigger = RFDEVICE_XSW_TRIGGER_ALL; 
			  status = rfdevice_xsw_trigger(&xsw_config,(void*)device_obj,asdiv_script_ptr,script_exec_type,0);	 //am
...
boolean rfc_tdscdma::generate_asdiv_xsw_sleep_script
( 
  uint32 position,
  rf_buffer_intf *asdiv_script_ptr,
  void *cb_data
