...
$Header: //components/rel/rftech_tdscdma.mpss/3.11/rf/rfc/src/rfc_card_tdscdma.cpp#1 $
...
07/06/15   jr      Add DR-DSDS RF Cal support
05/12/15   rcui    pass device number to rftdscdma_pm_info
03/30/15   nsh     Smartly handling TxRx and RxTx scenario for GRFC ASM
...
)
{
  ...
  rftdscdma_pm_info.rf_device = device;  
...
/*!
  @brief
  calls RFC function to configure fb chain
...
*/
uint32 rfc_tdscdma_get_fb_chain
(
   rfm_device_enum_type tx_device, 
   rfcom_tdscdma_band_type band)
{
  uint32 fb_modem_chain = 1; // Default Chain on Jacala
  rfc_tdscdma *rfc_obj = rfc_tdscdma::get_instance();

  if ( rfc_obj == NULL )
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_FATAL,"rfc_tdscdma_ptr is NOT created. ",0);
    return RFM_INVALID_DEVICE ;
  }

  fb_modem_chain = rfc_obj->get_fb_chain( tx_device, band);
...
boolean
rfc_tdscdma_get_asdiv_xsw_sleep_script
( 
  uint32 position,
  rf_buffer_intf *asdiv_script_ptr,
  void* cb_data
)
{
  ...
  status = rfc_tdscdma_obj->generate_asdiv_xsw_sleep_script((int)position ,asdiv_script_ptr,
                                                             cb_data
                                                             );
  ...
  if( status == FALSE )
  {
    RF_MSG( RF_ERROR, "rfc_tdscdma_get_asdiv_xsw_sleep_script : "
                      "script generation failed");
  }
...
boolean rfc_tdscdma_check_tx_capability(rfm_device_enum_type rfm_dev , rfcom_tdscdma_band_type band)
{
  rfc_device_info_type *device_info_ptr = NULL;
  rfc_tdscdma_data *rfc_data = NULL; 
  boolean band_supported = FALSE;
  rfc_cfg_params_type cfg = { RF_PATH_0, rfm_dev, RFC_CONFIG_TX, RFCOM_NUM_TDSCDMA_BANDS, RFC_REQ_DEFAULT_GET_DATA};

  rfc_data = rfc_tdscdma_data::get_instance(); 
  if (rfc_data == NULL)
  {
  ...
  // Get the TX support for given band
...
/*!
  @brief
  This function returns the address of rfc_tdscdma_ptr.
  ...
  @Returns
  Address of rfc_tdscdma_ptr

*/
void * rfc_tdscdma_get_address(void)
{
  rfc_tdscdma *rfc_obj = rfc_tdscdma::get_instance();
