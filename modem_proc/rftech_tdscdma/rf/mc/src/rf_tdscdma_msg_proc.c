...
$Header: //components/rel/rftech_tdscdma.mpss/3.11/rf/mc/src/rf_tdscdma_msg_proc.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/07/16   nsh     Support AutoPin on TA.2.3 + WTR3925
12/10/15   jr      Phase 1 device status read
11/17/15   hx      Power optimization when RxD is off
...
04/13/15   whz     Wakeup confirmation is not needed in wakepstep01 process for tabasco
03/17/15   whz     Fix TempComp on Jolokia
02/28/15   jr      Reduce debug messages from wakeup process
01/12/14   ych     Porting Reacq optimization from BO
...
rf_tdscdma_dispatch_outcome_t rf_tdscdma_proc_get_rf_txchain_no_req (
...
void rf_tdscdma_proc_rflm_device_read_status_trigger_rsp( void* cmd_ptr,
                                               rf_dispatch_cid_info_type *cid_info,
                                               void *cb_data );
...
rf_tdscdma_dispatch_outcome_t rf_tdscdma_proc_wakeup_req( rfa_tdscdma_wakeup_req_s* req_msg_ptr )
{
  ...
  if (req_msg_ptr->wakeup_step == RFA_TDSCDMA_WAKEUP_STEP0)
  {
    ...
    /* Enable ONLY when TL1 SW dependence presents
       Send back the CNF MSG */
#if ((defined FEATURE_THOR_MODEM) || (defined FEATURE_BOLT_MODEM) ) 
    /* Send back the CNF MSG */
    msgr_send_status = rf_tdscdma_send_msgr_cnf((msgr_hdr_struct_type *)&cnf_msg, 
    ...
    MSG_1(MSG_SSID_RF, MSG_LVL_HIGH,
          "RFA_RF_TDSCDMA_WAKEUP_REQ: req_msg_ptr->wakeup_step= %d and step0p1() and step0p2() is called", 
...
/*===========================================================================
  ...
  FUNCTION:  rf_tdscdma_proc_get_rf_txchain_no_req
...
*/
/*=========================================================================*/
rf_tdscdma_dispatch_outcome_t rf_tdscdma_proc_get_rf_txchain_no_req( rfa_tdscdma_get_rf_txchain_no_req_s* req_msg_ptr )
{
  rf_tdscdma_dispatch_outcome_t         ret_status = RF_TDSCDMA_DISPATCH_SUCESSFUL;
  ...
  {
    MSG(MSG_SSID_RF, MSG_LVL_ERROR, 
        "Pointer argument into rf_tdscdma_proc_get_rf_txchain_no_req is NULL");
    ret_status = RF_TDSCDMA_DISPATCH_NULL_PTR;
    return ret_status;
  }

  txchain_no = rf_tdscdma_mc_get_tx_modem_chain(req_msg_ptr->pri_device, req_msg_ptr->tx_device);
...
void rf_tdscdma_proc_rflm_autopin_trigger_rsp( void* cmd_ptr,
                                               ...
                                               void *cb_data )
{
  rflm_tds_trigger_autopin *params = (rflm_tds_trigger_autopin *)((rf_cmd_type *)cmd_ptr)->payload;

  rflm_handle_tx_t tx_handle = params->tx_handle;

  uint8 pa_state = params->pa_state;

  rflm_db10_t pin = params->pin;

  rf_tdscdma_core_txlin_autopin_update(pin, pa_state);

  RF_MSG_3( RF_HIGH, "rf_tdscdma_proc_rflm_autopin_trigger_rsp, Handle: %d, Pin: %d, PA state: %d", 
            tx_handle, pin, pa_state);
}
#endif
                                               ...
                                               void *cb_data )
{
 ...
 rflm_tds_trigger_device_read *device_read_params = (rflm_tds_trigger_device_read *)((rf_cmd_type *)cmd_ptr)->payload;
 rf_tdscdma_mc_device_status_trigger_read(device_read_params->ccsArg);
}
...
rf_tdscdma_dispatch_outcome_t rf_tdscdma_proc_mdsp_iqcapture_rsp_ftmrf( tfw_iq_capture_rsp_t* rsp_msg_ptr )
{
  ...
  MSG_6(MSG_SSID_RF, MSG_LVL_HIGH, "TDSCDMA IQ capture: data is got from FW, status=%d, startAddress=%d, startAddressDiv=%d, startTime=%d, numSamples=%d, antSel=%d", 
          rsp_msg_ptr->status, rsp_msg_ptr->startAddress, rsp_msg_ptr->startAddressDiv, rsp_msg_ptr->startTime, rsp_msg_ptr->numSamples, rsp_msg_ptr->antSel);
...
//TFW uses TRK (CX2) settings for IQ capture
#if 0
...
rf_tdscdma_dispatch_outcome_t rf_tdscdma_proc_mdsp_get_multi_synth_rsp(tfw_ftm_get_multi_synth_state_rsp_t * rsp_msg_ptr)
{
	...
	rfdevice_tdscdma_rx_cmd_dispatch(RFM_DEVICE_0, RFM_TDSCDMA_MODE,RFCOM_BAND_TDSCDMA_B34, 
									 ...
									 RFM_DEVICE_1, RFDEVICE_PRX_OPER_MODE, &rx_pll_status_bitmask 
...
boolean rf_tdscdma_msgr_rflm_register(void)
{
  ...
  /* Register command to Device Status read */
  ret_val &= rf_dispatch_register_command( rf_task_get_dispatch_handle(),
                                           MSGR_TECH_RFLM, 
                                           RFLM_TDSCDMA_TRIGGER_DEVICE_READ_CMD, 
                                           rf_tdscdma_proc_rflm_device_read_status_trigger_rsp,
                                           NULL,
                                           1<<RF_DISPATACH_MSGR_REG );
...
rf_tdscdma_dispatch_outcome_t rf_tdscdma_req_dispatch(rf_tdscdma_msg_u* msg_ptr)
{
  ...
  if (msg_ptr != NULL)
  {
    MSG_1(MSG_SSID_RF, MSG_LVL_HIGH, "RF TDSCDMA req dispatch, MSG ID=0x%x", msgr_hdr_ptr->id);
    ...
    case RFA_RF_TDSCDMA_GET_RF_TXCHAIN_NO_REQ:
      disp_status = rf_tdscdma_proc_get_rf_txchain_no_req((rfa_tdscdma_get_rf_txchain_no_req_s*) msg_ptr);
