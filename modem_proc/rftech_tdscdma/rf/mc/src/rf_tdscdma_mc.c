...
--------   ---     ----------------------------------------------------------
05/15/20   hal     Error callback hit invalid packet exception from qsh mdump callback 
02/16/20   hal     Kamorta GNSS Blanking FR
01/21/20   hal     Support QSH minidump for RFTECH TDSCDMA
01/19/20   hal     Kw fix
12/12/19   hal     Keep BC config NV valid
12/09/19   hal     Move BC config related code
06/06/18   nsh     QAT3522 Offset loss compensation using Char Data
03/04/18   nsh     Support QAT3522 for TDSCDMA
10/26/17   sk      Enable LTE B71 support
01/18/17   sk      Send Tx handle to XPT api's for LTE
10/19/16   nsh     Support HDET Reading Script building and processing
04/29/16   nsh     Make sure the 1st Thermal used is read from WTR instead default value
04/07/16   nsh     Support AutoPin on TA.2.3 + WTR3925
03/23/16   chm     [Ported]Add ASM disable tx in rf_tdscdma_mc_disable_tx to 
                   make sure rx_tx_vote bit cleared
03/04/16   jr      Set different Rx_TX delay for FBRx Cal
03/02/16   jr      Get FBRX Chain from RFC files
03/02/16   zhh     update definition of rfcommon_autopin_mc_enable_tx
03/27/16   nsh     Fix power jump issue during max power
02/27/16   whz     Save autopin WTR result
01/07/16   jr      Reduce F3 messages
12/10/15   jr      Phase 1 device status read
11/29/15   jr      NV support to enable/disable LPM feature
11/18/15   jr      Rx/Tx split feature on different WTR
11/17/15   whz     Push AutoPin config to DM
                   Use global variable to remember cmn_device_ptr
11/17/15   hx      Wake up power optimization when RxD is off.
11/11/15   whz     Pass in correct TxLM buffer index when building AutoPin event
11/10/15   whz     Check whether AutoPin is really enabled
11/05/15   whz     AutoPin on TA: online support
11/04/15   kma     Updated WTR interface
11/03/15   jr      Fix XPT IQ capture in Pin CHAR
10/31/15   jr      Enable XO cal on TDS node
10/30/15   nsh     Init tuner temp timer before starting it
10/06/15   whz     Add support to AutoPin script for TA
10/01/15   lwu     update diversity tuner states after BHO 
09/21/15   aa      Save TX/FBRx LM buffer info in FBRx state machine
09/14/15   jak     Fix rf_tds_bc_config mask for B39 in enter mode
09/10/15   jz      Use correct device ID for IP2 Cal
08/26/15   hx      Porting logic from CR672625 by adding protection for concurrent tuner configuration
08/24/15   whz     Remove unnessary DM buffer usage in init_rxagc
08/13/15   hx      Move PRX pwr wakeup vote after common vote
07/27/15   jr      Fix tempcomp update for tabasco
07/21/15   cdb     Add runtime check for Autopin enabled
07/10/15   jak     Use curr device in RF state var in disable_tx/sleep/exit_mode
07/14/15    aa     Added support for FBRx C1 NV container
07/13/15   ych     Enable WTR sleep/wakeup
07/10/15   jak     Device parameter checking for enter/enable_rx/enable_tx
07/06/15   jr      Abort pending AsDiv Event
06/27/15   jak     Pass device info parameter for rf_tdscdma_mc_get_tx_modem_chain()
06/26/15   jak     Update rfm_mode[dev] at the time of enter mode
06/25/15   jak     Add rf_tdscdma_mc_get_curr_pri_rx_device()
06/23/15   jak     Add pri_paired_device
06/22/15   jr      Asdiv current position update in wakeup/enter mode 
06/12/15   ndb     Cleanup the unused enums
06/09/15   lwu     Add check for IP2 Cal for sawless card specific
06/09/15   nsh     Support IP2 Cal
06/04/15   whz     Added AutoPin support
06/04/15   jak     fix KW warning in rf_tdscdma_mc_update_tuner_code()
06/03/15   jak     Init tuner ptr for rf_tdscdma_mc_update_tuner_code()
05/22/15   sub     Placeholder parameter to support fbrx interface change
05/29/15   jak     Remove the use of rfcommon_core_device_to_path()
05/28/15   jak     Pass PRX/DRX oprt mode to rf_tdscdma_msm_calc_iqmc_setting()
05/26/15   ndb     Move to new tuner interfaces
05/21/15   jak     Add new API of rf_tdscdma_mc_get_curr_tx_device()
04/23/15    ka     Remove hardcoded device 0 for fbrx
05/13/15   jak     Tx/Rx split feature - adaption to WTR interface change
04/29/15   jak     Tx/Rx split feature
04/08/15   jz      Disable ASM rx before sleep
04/08/15   lwu     Created is_bho_update parameter in rf_tdscdma_msm_update_txlm_buffer() 
04/08/15   lwu     Fixed BHO issue due to IMM write the script while update_bho   
03/17/15   whz     Fix TempComp on Jolokia/Tabasco
03/16/15   jyu     Fixed the bug for band port update after device and/or alt_path change
                   (need to update for all bands and for PRx and DRx 
                    regardless if RxD is on or not)
03/06/15   jyu     Need to call rfm_enter/exit_mode() in MC sleep/wakeup()
02/28/15   jr      Reduce debug messages from wakeup process
02/12/15   jr      Replace JOLOKIA feature with TABASCO
01/31/15   jak     Fix asdiv tune code update called in sleep state
01/28/15   whz     Explicity build HDET script
01/13/15   jyu/jps Fix concurrency manager crash during band change in FTM RF mode
01/13/15   jyu     Fix LNA gain stuck issue in N-FREQ
01/11/14   jyu     Add support for eLNA
...
12/15/14   nsh     Pass immediate flag in tds asdiv api
...
#define RF_TDSCDMA_DEVICE_STATUS_READ_DELAY 0
...
/* Time profiling flag */
boolean rf_tdscdma_mc_display_time_profile = TRUE;

/* Wakeup timeline debug flag */
boolean rf_tdscdma_mc_wakeup_time_debug = TRUE;
...
#ifdef FEATURE_QSH_MDUMP
/* extern variables for minidump */
extern rflm_tds_txlm_handle_user_data_t rflm_tds_txlm_handle_user_data;
extern rf_tdscdma_core_tx_multi_lin_data_type rf_tdscdma_core_tx_multi_lin_data[RFCOM_BAND_TDSCDMA_B39+1];
//extern rflm_ccs_crash_dump_t rflm_ccs_crash_dump;
extern rf_tdscdma_core_txlin_record_type rf_tdscdma_core_current_txlin_data;

//extern ftm_tdscdma_esc_ftm_cal_state_t ftm_tdscdma_esc_ftm_cal_state;
extern ftm_tdscdma_rftest_state_t ftm_tdscdma_rftest_tgt_state;
extern ftm_tdscdma_rftest_state_t ftm_tdscdma_rftest_curr_state;
extern ftm_tdscdma_cal_state_type ftm_tdscdma_current_cal_state;
#endif


/*HISTOGRAM logging*/
prof_hist_log_packet_handle_t rf_tds_hist_packet_handle;
prof_hist_param_t tds_mc_enable_rx_hist;
prof_hist_param_t tds_mc_enable_tx_hist;
prof_hist_param_t tds_mc_disable_tx_hist;
prof_hist_param_t tds_mc_update_bho_hist;
prof_hist_param_t tds_mc_wakeup_step0p1_hist;
prof_hist_param_t tds_mc_wakeup_step0p2_hist;
prof_hist_param_t tds_mc_wakeup_step1_hist;
prof_hist_param_t tds_mc_sleep_hist;


PROF_HIST_PACKET_DEFINE(rf_tds_hist_packet, ,0x18E9,
                         (tds_mc_enable_rx_hist, 4000, 100, 0, 3000, 1BYTE_SAT),
                         (tds_mc_enable_tx_hist, 4001, 100, 0, 6000, 1BYTE_SAT),
                         (tds_mc_disable_tx_hist, 4002, 100, 0, 1500, 1BYTE_SAT),
                         (tds_mc_update_bho_hist, 4003, 100, 0, 5000, 1BYTE_SAT),
                         (tds_mc_wakeup_step0p1_hist, 4004, 20, 0, 200, 1BYTE_SAT),
                         (tds_mc_wakeup_step0p2_hist, 4005, 20, 0, 200, 1BYTE_SAT),
                         (tds_mc_wakeup_step1_hist, 4006, 100, 0, 2000, 1BYTE_SAT),
                         (tds_mc_sleep_hist, 4007, 100, 0, 1500, 1BYTE_SAT));


#ifdef FEATURE_RF_TDS_ASDIV_ELNA_GAIN
boolean rf_tds_AsDiv_eLNA_gain_offset_B34_NV_present = FALSE;
boolean rf_tds_AsDiv_eLNA_gain_offset_B39_NV_present = FALSE;
boolean rf_tds_AsDiv_tx_offset_B34_NV_present = FALSE;
boolean rf_tds_AsDiv_tx_offset_B39_NV_present = FALSE;
#endif
...
#ifdef RF_TDSCDMA_RXD_TIMELINE_OPT
  RFA_TDSCDMA_WAKEUP_PRIM_ONLY,           //wake up rxd ind
#endif
  {0},                                    //device_status_read
...
#ifdef FEATURE_RF_TDS_ASDIV_ELNA_GAIN
extern int32 rflmTds_elna_gain_offset[RF_PATHS_TABLE_SIZ][RF_TDSCDMA_MAX_LNA_GAIN_STATES][RF_TDSCDMA_NUM_RX_LIN_STATES];
#endif
...
void rf_tdscdma_mc_sleep_rx_chain
(
  rfm_device_enum_type device,
  rf_path_enum_type path,
  rfcom_tdscdma_band_type band,
...
void rf_tdscdma_mc_update_gnss_blanking_state
(
  rfcom_tdscdma_band_type band,
  lm_handle_type txlm_buffer_idx
);
...
)
{
  ...
  if ( (rf_tdscdma_mc_state.rf_state[RF_PATH_0] == RF_TDSCDMA_STATE_INIT) || 
       (rf_tdscdma_mc_state.rf_state[RF_PATH_0] == RF_TDSCDMA_STATE_SLEEP) )
  {
  ...
  if (cb_func ==  NULL)
  {
    ...
    if (to_pos == position)
    {
      RF_MSG_2( RF_HIGH, "rf_tdscdma_mc_switch_ant: " 
...
#ifdef FEATURE_RF_TDS_ASDIV_ELNA_GAIN 
     /*build LNA script when eLNA is used*/
      if (rflmTdsAsdivElnaEnable)
      {
       /* Build Rx LNA scripts for primary chain */
       rf_tds_build_event_data.pri_device = rf_tdscdma_mc_state.curr_pri_device;
       rf_tds_build_event_data.pri_paired_device = rf_tdscdma_mc_state.curr_pri_paired_device;	  
       rf_tds_build_event_data.sec_device = rf_tdscdma_mc_state.curr_sec_device;
       rf_tds_build_event_data.tx_device = RFM_INVALID_DEVICE;
       ...
       if (rf_tdscdma_mc_state.curr_band_rx >= RFCOM_NUM_TDSCDMA_BANDS)
       {
         ERR_FATAL("Wrong band %d", rf_tdscdma_mc_state.curr_band_rx, 0, 0);
       }
       rf_tds_build_event_data.to_build_rx_lna_script_on_bands[rf_tdscdma_mc_state.curr_band_rx] = TRUE;
       
       /*While re-building the new LNA script, need to freeze the old LNA script execution*/
       /*set pending*/
       rflmTdsLnaPending = TRUE;
       /*update to position for rflm*/
       rflmTdsAsdivSwPosition = to_pos;  
       MSG(MSG_SSID_RF, MSG_LEGACY_HIGH,
           "rf_tdscdma_mc_ant_switch: Freeze the LNA script execution while re-build the LNA script.");
        
         /* Build Rx LNA scripts for primary chain*/
         rf_tdscdma_mdsp_rf_event_cmd_dispatch(RF_TDS_EVT_BUILD_RX_LNA, &rf_tds_build_event_data);

         /* Build Rx LNA scripts for secondary chain (always) */
         rf_tds_build_event_data.drx_enable = TRUE;
         rf_tdscdma_mdsp_rf_event_cmd_dispatch(RF_TDS_EVT_BUILD_RX_LNA, &rf_tds_build_event_data);
     
         MSG_2(MSG_SSID_RF, MSG_LEGACY_HIGH,
              "rf_tdscdma_mc_ant_switch: Re-build LNA script for Device %d, Band %d",
              rf_tdscdma_mc_state.curr_pri_device,
              rf_tdscdma_mc_state.curr_band_rx);
      }
#endif 
  ...
  rfcom_band_type_u curr_band_rx;
  curr_band_rx.tdscdma_band = rf_tdscdma_mc_state.curr_band_rx;
  asdiv_offset_tds = rfcommon_nv_get_asdiv_offset_data(RFM_TDSCDMA_MODE, curr_band_rx , to_pos);
  MSG_2(MSG_SSID_RF, MSG_LEGACY_HIGH,
          "RF_TDSCDMA_MC: rf_tdscdma_mc_ant_switch: For Ant Pos: %d, asdiv_offset_data is %d",
          to_pos, asdiv_offset_tds);
...
#ifdef FEATURE_RF_TDS_ASDIV_ELNA_GAIN 
      rflmTdsAsdivSwPosition = rf_tdscdma_mc_state.curr_ant_sw_pos;
      /*How to deal with pending*/
#endif 
...
void rf_tdscdma_mc_asdiv_sleep_script
(
  uint32 ant_position,
  rfm_device_enum_type pri_device,
  rfcom_tdscdma_band_type band
)
{
  ...
  script_status = rfc_tdscdma_get_asdiv_xsw_sleep_script(ant_position, /*!< New Antenna Position */
  ant_sw_script, /*!< pointer to buffer to store Script */
  (void*)(&asm_script_data) /*!< Callback data to be passed to the function */);
  ...
  {
    MSG_3(MSG_SSID_RF, MSG_LEGACY_ERROR,
    "RF_TDSCDMA_MC: rf_tdscdma_mc_asdiv_sleep_script(ant_pos %d, device %d, band %d) ERROR", ant_position, pri_device, band);
  }
...
)
{
  ...
  MSG_5(MSG_SSID_RF, MSG_LVL_HIGH,
        "rf_tdscdma_mc_enter_mode(pri_device=%d, sec_device=%d, rxlm_buf_idx=%d, rxlm_mode=%d, is_dsds=%d)",
        pri_device, sec_device, rxlm_buffer_idx, rxlm_mode, is_dsds);
  ...
  if (pri_device >= RFM_MAX_WAN_DEVICES)
  {
    RF_MSG_2( RF_ERROR, "rf_tdscdma_mc_enter_mode: Error api called with invalid device %d. err code %d. Cannot enter mode.",
              pri_device, RF_TDSCDMA_MC_INVALID_PARAM );
    return RF_TDSCDMA_MC_INVALID_PARAM;
  }  
...
#ifdef FEATURE_RF_TDS_ASDIV_ELNA_GAIN
  /*clear rflmTds_elna_gain_offset*/
  memset(rflmTds_elna_gain_offset, 0, sizeof(rflmTds_elna_gain_offset));
  /**/
  if (rf_tds_AsDiv_eLNA_gain_offset_B34_NV_present && rf_tds_AsDiv_eLNA_gain_offset_B39_NV_present)
  {
    rflmTdsAsdivElnaEnable = TRUE; 
  }
  MSG_1(MSG_SSID_RF, MSG_LVL_HIGH, "rflmTdsAsdivElnaEnable = %d", rflmTdsAsdivElnaEnable);
#endif
  ...
  /* define the temp comp timer */
  if ( ( !IS_FTM_IN_TEST_MODE() ) ||
       ( IS_FTM_IN_TEST_MODE() && ( !rfm_get_calibration_state() ) ) )
  {
    rf_tdscdma_core_temp_comp_init();
#ifdef FEATURE_RF_HAS_QTUNER
    rf_tdscdma_core_tuner_temp_comp_init();
...
#endif
/*put under FEATURE_RF_ASDIV after test*/
#ifdef FEATURE_RF_TDS_ASDIV_ELNA_GAIN 
  rflmTdsAsdivSwPosition = rf_tdscdma_mc_state.curr_ant_sw_pos;
  rflmTdsLnaPending = FALSE;
#endif
...
)
{
  ...
  MSG_4(MSG_SSID_RF, MSG_LVL_HIGH, 
        "rf_tdscdma_mc_exit_mode(curr_pri_device=%d (path %d), curr_sec_device=%d (path %d))", 
        rf_tdscdma_mc_state.curr_pri_device, pri_path, rf_tdscdma_mc_state.curr_sec_device, sec_path);
  ...
  {
    rf_tdscdma_mc_sleep_rx_chain(rf_tdscdma_mc_state.curr_pri_device,
                                 RF_PATH_0,
                                 rf_tdscdma_mc_state.curr_band_rx,
                                 rf_tdscdma_mc_state.curr_chan_rx,
                                 RFDEVICE_PRX_OPER_MODE);
    ...
    {
      rf_tdscdma_mc_sleep_rx_chain(rf_tdscdma_mc_state.curr_sec_device,
                                   RF_PATH_1,
                                   rf_tdscdma_mc_state.curr_band_rx,
                                   rf_tdscdma_mc_state.curr_chan_rx,
  ...
  if (rfcommon_nv_get_lpm_feature_enabled())
  {
    ...
    {
      MSG_1(MSG_SSID_RF, MSG_LVL_ERROR, "rf_tdscdma_mc_exit_mode(): Device: %d Pwr Up failed", rf_tdscdma_mc_state.curr_pri_device);
    }
    ...
    /*Reset WTR */
    MSG_1(MSG_SSID_RF_DEBUG, MSG_LVL_HIGH, "rf_tdscdma_mc_exit_mode: Reset WTR on device %d", rf_tdscdma_mc_state.curr_pri_device);
    rfdevice_tdscdma_rx_cmd_dispatch(rf_tdscdma_mc_state.curr_pri_device, 
                                     RFCOM_TDSCDMA_MODE, 
                                     rf_tdscdma_mc_state.curr_band_rx, 
                                     RFDEVICE_RESET_WAN, 
                                     rf_tdscdma_mc_state.curr_pri_paired_device,
                                     RFDEVICE_PRX_OPER_MODE,
                                     NULL
#ifdef FEATURE_RF_TDS_ASDIV_ELNA_GAIN
                                     ,FALSE
	...
	/* Update the rfm_mode for pri_device_*/  
	if(RFCOM_TDSCDMA_MODE == rfm_get_current_mode(rf_tdscdma_mc_state.curr_pri_device))
	{
	  rfm_exit_mode( rf_tdscdma_mc_state.curr_pri_device, RFCOM_PARKED_MODE );
	}
  ...
  rf_tdscdma_mc_state.curr_pri_paired_device= RFM_INVALID_DEVICE;
  ...
  /* un-define the temp comp timer */
  if ( ( !IS_FTM_IN_TEST_MODE() ) ||
       ( IS_FTM_IN_TEST_MODE() && ( !rfm_get_calibration_state() ) ) )
  {
    rf_tdscdma_core_temp_comp_deinit();
  }
...
)
{
  ...
  rf_time_tick_type overall_enable_rx_start_t = {0};
  rf_time_type enable_rx_time;
  rf_time_type overall_enable_rx_time;
  boolean script_status;
  ...
  if(rf_tdscdma_mc_display_time_profile)
  {
    ...
    overall_enable_rx_start_t = rf_time_get_tick(); /* time spent in API */
  }
  ...
  {
    if (sec_device >= RFM_MAX_WAN_DEVICES)
    {
  ...
  /* Sleep Rx chain(s) if there is band change */
  if (rf_tdscdma_mc_state.curr_band_rx != band)
  {
    if (rf_tdscdma_mc_state.curr_band_rx != RFCOM_BAND_TDSCDMA_INVALID)
    {
     ...
     MSG_2(MSG_SSID_RF, MSG_LVL_HIGH, 
           "rf_tdscdma_mc_enable_rx: Band change detected. From Band %d to Band %d", 
           rf_tdscdma_mc_state.curr_band_rx,
           band);   
  }
  else
  {
    /* Sleep diversity chain if there is RxD state change from RxD ON->OFF (within the same band) */
    if( (rf_tdscdma_mc_state.div_enable == TRUE)   &&
        (rx_path == RFA_TDSCDMA_RX_PATH_PRIM)       )
    {
      rf_tdscdma_mc_sleep_rx_chain(rf_tdscdma_mc_state.curr_sec_device,
                                   RF_PATH_1,
                                   rf_tdscdma_mc_state.curr_band_rx,
      ...
      MSG_1(MSG_SSID_RF, MSG_LVL_HIGH, 
           "rf_tdscdma_mc_enable_rx: RxD state change detected. From ON->OFF for Band %d", 
           rf_tdscdma_mc_state.curr_band_rx);     
    }
  }  
  ...
  rf_tdscdma_mc_state.curr_pri_paired_device= rfc_common_get_preferred_associated_rx_device(pri_device);  
  ...
  /* Set the flag for Cal mode for Rx_Tx delay */
  rflmTdsRfModeCtlState.isRfFtmCalMode = FALSE; 
  if ( IS_FTM_IN_TEST_MODE() && (rfm_get_calibration_state() == TRUE) )
  {
    rflmTdsRfModeCtlState.isRfFtmCalMode = TRUE; 
  }
  ...
  /*vote Prx wakeup after common wakeup*/
  //check RFNV_SPARE_1_I value to know LPM feature
  if (rfcommon_nv_get_lpm_feature_enabled())
  {
  if(rf_tdscdma_mc_state.curr_band_rx == band)
  ...
  else
  {
    ...
    //if (pri_path == RF_PATH_0)
    {
      ...
      rfdevice_tdscdma_rx_cmd_dispatch(pri_device,
                                       RFCOM_TDSCDMA_MODE,
                                       rf_tdscdma_mc_state.single_valid_band,
    ...
    if (rflm_tds_is_fbrx_enabled())
    {
      if(RFCOMMON_FBRX_ERROR == rfcommon_fbrx_mc_enter_mode(&rf_tdscdma_fbrx_lm_handle, RFM_TDSCDMA_MODE, 0))
      {
    ...
    /* Build Rx LNA scripts for primary chain */
    rf_tds_build_event_data.pri_device = pri_device;
    rf_tds_build_event_data.pri_paired_device = rfc_common_get_preferred_associated_rx_device(pri_device);	
    rf_tds_build_event_data.sec_device = sec_device;
    rf_tds_build_event_data.tx_device = RFM_INVALID_DEVICE;
    rf_tds_build_event_data.drx_enable = FALSE;
    rf_tds_build_event_data.is_irat = FALSE;
  ...
  if(rf_tdscdma_mc_display_time_profile)
  {
    overall_enable_rx_time = rf_time_get_elapsed(overall_enable_rx_start_t, RF_USEC);
    MSG_1(MSG_SSID_RF, MSG_LEGACY_MED, "Enable Rx time profile: %d usec", overall_enable_rx_time);
    
    /*updating the histogram for enable rx */
    prof_hist_update(&tds_mc_enable_rx_hist, overall_enable_rx_time);

    if (prof_hist_log_periodic(&rf_tds_hist_packet_handle))
    {
      prof_hist_clear(&tds_mc_enable_rx_hist);
    }
  }
...
)
{
  ...
  uint32 fbrx_chan_list[RFCOMMON_FBRX_1_CARRIER] = {0xFFFF};
  rftdscdma_mdsp_common_event_data_type rf_tds_build_event_data;
  ...
  if (rf_tdscdma_use_on_rumi == TRUE)
  {
    ...
    /* update TxLM setting */
    rf_tdscdma_msm_update_txlm_buffer(device, txlm_buffer_idx, band, FALSE, &adc_idx);
  ...
  /* due to Tx Rx split feature , we need to take care the mode of rfm_mode[tx_device] 
        or we will have some problem when calling some common function like using  
        rfcommon_core_get_tx_chain_from_device () */
  if(RFCOM_TDSCDMA_MODE != rfm_get_current_mode(device))
  {
    rfm_enter_mode(device, RFCOM_TDSCDMA_MODE, NULL, NULL, 0);
  }  
  ...
  /* suppose path is aligned with rx , here pass curr prx device instead of tx device */
  path = RF_PATH_0;
  ...
  {
    MSG_1(MSG_SSID_RF, MSG_LVL_ERROR, "rf_tdscdma_mc_enable_tx: Unsupported Device %d (path %d)", device);
    return RF_TDSCDMA_MC_INVALID_PARAM;
  }
  ...
  {
    rf_tdscdma_mc_state.curr_tx_device = device;  
  ...
  /* vote for TX VREGs */
  rfc_tdscdma_manage_vregs(rf_tdscdma_mc_state.curr_pri_device, rf_tdscdma_mc_state.curr_band_tx, RF_PATH_RXTX_STATE);
  ...
  {
    /* FBRX init */
    fbrx_status = rfcommon_fbrx_mc_init_tx(rf_tdscdma_fbrx_lm_handle, RFCOMMON_FBRX_TDS, txlm_buffer_idx, RFM_TDSCDMA_MODE, 0);
    /* Regsiter the VSWR post processing call back function to FBRX module */
    fbrx_status &= rfcommon_fbrx_mc_register_vswr_meas_call_back(rflm_tds_mc_post_process_fbrx_vswr_measurements,
                                                                 txlm_buffer_idx);
    ...
    if (fbrx_status == RFCOMMON_FBRX_SUCCESS)
    {
      fbrx_chan_list[0] = (uint32)tx_channel;
      fbrx_tx_params.apt_only_mode = FALSE;
      /* @todo: remove hard-coding, tech to update the fbrx chain from rfc */
      if (band < RFCOM_NUM_TDSCDMA_BANDS)   //KW fix ...
      {
      ...
      if (rfcommon_autopin_is_enabled(RFM_TDSCDMA_MODE))
      {
        rf_tdscdma_mc_cmn_device_ptr = rf_tdscdma_mc_get_curr_common_device(device,
                                                                            band);
#if defined(FEATURE_TABASCO_MODEM) && defined(FEATURE_RF_HAS_WTR2965)
    ...
    if (fbrx_status == RFCOMMON_FBRX_SUCCESS)
    {
      rf_tdscdma_fbrx_modem_chain = rfc_tdscdma_get_fb_chain(device,band );
      fbrx_status = rfcommon_fbrx_enable(rf_tdscdma_fbrx_lm_handle, rf_tdscdma_fbrx_modem_chain);
    }
    ...
    rflm_tds_mc_set_fbrx_lm_handle(rf_tdscdma_fbrx_lm_handle);
...
//  rf_tdscdma_mc_lm_bufs.tx_buf_wrk.buf_idx = txlm_buffer_idx;
       ...
       ( IS_FTM_IN_TEST_MODE() && ( !rfm_get_calibration_state() ) ) )
  {
    ...
    /* apply the fbrx temp comp one time */
    rf_tdscdma_core_temp_comp_fbrx_once(tx_channel);
  ...
  /* update the self-test flag */
  if ( ftm_common_get_selftest_capture_flag() )
  {
    rflmTdsRfModeCtlState.isSelfTestMode = TRUE;
  }
  else
  {
    rflmTdsRfModeCtlState.isSelfTestMode = FALSE;
  }
  ...
  if(rf_tdscdma_mc_display_time_profile || rf_tdscdma_mc_tx_time_debug)
  {
    ...
    /*updating the histogram for enable tx */
    prof_hist_update(&tds_mc_enable_tx_hist, overall_enable_tx_time);

    if (prof_hist_log_periodic(&rf_tds_hist_packet_handle))
    {
      prof_hist_clear(&tds_mc_enable_tx_hist);
    }
  }
...
rf_tdscdma_mc_status_type rf_tdscdma_mc_disable_tx(rfm_device_enum_type device)
{
  ...
  rf_time_tick_type overall_disable_tx_start_t = {0};
  rf_time_type disable_tx_time,overall_disable_tx_time;
  boolean status;

  if(rf_tdscdma_mc_display_time_profile)
  {
    disable_tx_start_t = rf_time_get_tick();
    overall_disable_tx_start_t = rf_time_get_tick();
  }

  /* suppose path is aligned with rx , here pass curr prx device instead of tx device */
  path = RF_PATH_0;
  MSG_2(MSG_SSID_RF, MSG_LVL_HIGH,"rf_tdscdma_mc_disable_tx, curr_tx_device=%d (path=%d)", rf_tdscdma_mc_state.curr_tx_device, path);
  ...
  /* Disable WTR Tx Path */
  rfdevice_tdscdma_tx_disable(rf_tdscdma_mc_state.curr_tx_device,
                              RFCOM_TDSCDMA_MODE,
                              rf_tdscdma_mc_state.curr_band_tx,
                              RFDEVICE_EXECUTE_IMMEDIATE,
                              NULL);

  /* ASM Disable tx to at least reset the rx_tx_vote bit in order to sleep  */
  rfdevice_tdscdma_asm_disable_tx(rf_tdscdma_mc_state.curr_tx_device, 
                                  rf_tdscdma_mc_state.curr_band_tx,
                                  NULL, 
				 				  RFDEVICE_EXECUTE_IMMEDIATE, 
  ...
  status = rfdevice_tdscdma_asm_sleep_tx(rf_tdscdma_mc_state.curr_tx_device,
                                         rf_tdscdma_mc_state.curr_band_tx,
                                         NULL,
                                         RFDEVICE_EXECUTE_IMMEDIATE,
                                         RFCOM_INVALID_TIMING_OFFSET);

  status = rfdevice_tdscdma_pa_sleep(rf_tdscdma_mc_state.curr_tx_device,
                                     rf_tdscdma_mc_state.curr_band_tx,
                                     NULL,
                                     RFDEVICE_EXECUTE_IMMEDIATE,
                                     RFCOM_INVALID_TIMING_OFFSET);
  ...
  if (!status)
  {
    MSG_2(MSG_SSID_RF, MSG_LVL_ERROR, "Error when put PA in sleep on Device %d, Band %d", rf_tdscdma_mc_state.curr_tx_device, rf_tdscdma_mc_state.curr_band_tx);
  }

  /* Disable QPOT*/
  if (rfdevice_tdscdma_use_papm(rf_tdscdma_mc_state.curr_tx_device, rf_tdscdma_mc_state.curr_band_tx))
  {
    status = rfdevice_tdscdma_papm_disable(rf_tdscdma_mc_state.curr_tx_device,
                                           rf_tdscdma_mc_state.curr_band_tx,
                                           NULL,
                                           RFDEVICE_EXECUTE_IMMEDIATE);
    ...
    if (!status)
    {
      MSG_2(MSG_SSID_RF, MSG_LVL_ERROR, "Error when disable power tracker on Device %d, Band %d", rf_tdscdma_mc_state.curr_tx_device, rf_tdscdma_mc_state.curr_band_tx);
    }
  }
  else
  {
    MSG_2(MSG_SSID_RF_DEBUG, MSG_LVL_MED, "papm device is not present for Device %d and Band %d", rf_tdscdma_mc_state.curr_tx_device, rf_tdscdma_mc_state.curr_band_tx);
  }
  ...
  if ( rf_tdscdma_core_xpt_is_enabled(rf_tdscdma_mc_state.curr_band_tx) )
  {
    ...
    if (!IS_FTM_IN_TEST_MODE())
    {
      rf_common_xpt_init_dpd(rf_tdscdma_mc_state.curr_tx_device, NULL);
    }
  }

#ifdef FEATURE_RF_HAS_QTUNER
    /* Tuner */
  (void)rf_tdscdma_core_antenna_tuner_tx_mode_disable(rf_tdscdma_mc_state.curr_tx_device,
                                                      rf_tdscdma_mc_state.curr_band_tx, 
  ...
  if ( rf_tdscdma_core_xpt_is_et_enabled(rf_tdscdma_mc_state.curr_band_tx) )
  {
#ifndef FEATURE_TABASCO_MODEM
    /* Disable ET DAC */
    rf_tdscdma_msm_disable_et_dac(rf_tdscdma_mc_lm_bufs.tx_buf_wrk.buf_idx);
#endif
    /* clear the flag */
    rflm_tds_mc_clear_et_dac_flag();
  }

  if( rflm_tds_is_fbrx_enabled() )
  {
    rfcommon_fbrx_mc_deinit_tx(rf_tdscdma_mc_lm_bufs.tx_buf_wrk.buf_idx);
  }

  /* vote off TX VREGs */
  rfc_tdscdma_manage_vregs(rf_tdscdma_mc_state.curr_pri_device, rf_tdscdma_mc_state.curr_band_rx, RF_PATH_RX_STATE);

  //check RFNV_SPARE_1_I value to know LPM feature
  if (rfcommon_nv_get_lpm_feature_enabled())
  {
    /*vote for wtr pwr sleep*/
    if(rfdevice_tdscdma_tx_pwr_sleep(device, 0))
    {
      if (rf_tdscdma_mc_tx_time_debug)
      {
  ...
  RFGNSS_MC(wwan_tx_status,ret_val)(rf_tdscdma_mc_state.curr_tx_device, FALSE);

  /* Update the rfm_mode for tx device to PARKED since we are disabling Tx */  
  if(RFCOM_TDSCDMA_MODE == rfm_get_current_mode(rf_tdscdma_mc_state.curr_tx_device))
  {
    rfm_exit_mode( rf_tdscdma_mc_state.curr_tx_device, RFCOM_PARKED_MODE );
  }

  if (!ret_val)
  {
    MSG_1(MSG_SSID_RF, MSG_LVL_ERROR, "rf_tdscdma_mc_enable_tx: RFGNSS_MC(device %d, TRUE) returns error", rf_tdscdma_mc_state.curr_tx_device);
  }
  ...
  rf_tdscdma_mc_state.curr_tx_device = RFM_INVALID_DEVICE;

  if(rf_tdscdma_mc_display_time_profile)
  {
    overall_disable_tx_time = rf_time_get_elapsed(overall_disable_tx_start_t, RF_USEC);
    MSG_1(MSG_SSID_RF, MSG_LEGACY_MED, "Disable Tx mode time profile: %d usec", overall_disable_tx_time);

    /*updating the histogram for disable tx */
    prof_hist_update(&tds_mc_disable_tx_hist, overall_disable_tx_time);

    if (prof_hist_log_periodic(&rf_tds_hist_packet_handle))
    {
      prof_hist_clear(&tds_mc_disable_tx_hist);
    }
  }
...
)
{
  ...
  MSG_4(MSG_SSID_RF, MSG_LVL_HIGH,"rf_tdscdma_mc_sleep(curr_pri_device=%d (path %d), curr_sec_device=%d (path %d))", rf_tdscdma_mc_state.curr_pri_device, pri_path, rf_tdscdma_mc_state.curr_sec_device, sec_path);
  ...
  if( (sec_path == RF_PATH_1) && (rf_tdscdma_mc_state.div_enable == TRUE) )
  {
    rf_tdscdma_mc_sleep_rx_chain(rf_tdscdma_mc_state.curr_sec_device,
                                 sec_path,
                                 rf_tdscdma_mc_state.curr_band_rx,
  ...
  {
    MSG_1(MSG_SSID_RF, MSG_LVL_ERROR, "rf_tdscdma_mc_sleep: Invalid sec_device %d", rf_tdscdma_mc_state.curr_sec_device);
  }
 ...
 if (rfcommon_nv_get_lpm_feature_enabled())
 {
  ...
  if( dev_voting_status == FALSE )
  {
    MSG_1(MSG_SSID_RF, MSG_LVL_ERROR, "rf_tdscdma_mc_sleep(): Device: %d Pwr Up failed", rf_tdscdma_mc_state.curr_pri_device);
  }
  ...
  {
    /* vote off VREGs */
    rfc_tdscdma_manage_vregs(rf_tdscdma_mc_state.curr_pri_device, rf_tdscdma_mc_state.curr_band_rx, RF_PATH_SLEEP_STATE);
  }
  ...
  if(rf_tdscdma_mc_state.div_enable)
  {
    rfm_exit_mode(rf_tdscdma_mc_state.curr_sec_device, RFCOM_PARKED_MODE);
  }
  ...
  if(rf_tdscdma_mc_display_time_profile)
  {
    ...
    MSG_1(MSG_SSID_RF, MSG_LEGACY_MED, "Sleep Timeline: Overall %d usec", overall_sleep_time);

    /*updating the histogram for sleep */
    prof_hist_update(&tds_mc_sleep_hist, overall_sleep_time);
  
    if (prof_hist_log_periodic(&rf_tds_hist_packet_handle))
    {
      prof_hist_clear(&tds_mc_sleep_hist);
    }
  }
...
(
  ...
  rfa_tdscdma_rxlm_mode_t rxlm_mode_div
#ifdef RF_TDSCDMA_RXD_TIMELINE_OPT
  ,rfa_tdscdma_wakeup_rxd_ind_t rx_rxd
#endif
#endif
...
)
{
  ...
  if((rx_path == RFA_TDSCDMA_RX_PATH_BOTH) ||
     (rx_path == RFA_TDSCDMA_RX_PATH_DIV)   )
  {
    if(RFCOM_TDSCDMA_MODE != rfm_get_current_mode(sec_device)) //to ensure DSDS_RF_TDSCDMA in wakeup0 for ASDiv
    {
      rfm_enter_mode(sec_device, RFCOM_TDSCDMA_MODE, NULL, NULL, 0);
    }    
  }
  ...
  /*Keep the third condition unchanged as we want to configure both path at this time*/
  ...
  else
  {
...
#ifdef RF_TDSCDMA_RXD_TIMELINE_OPT
    rf_tdscdma_mc_state.rxd_wakeup_state = rx_rxd;
#endif
    ...
    rf_tdscdma_mc_state.curr_pri_paired_device= rfc_common_get_preferred_associated_rx_device(prev_pri_device);  	
  ...
  if(rf_tdscdma_mc_display_time_profile)
  {
    ...
    if (rf_tdscdma_mc_wakeup_time_debug)
    {
      ...
      /*updating the histogram for wakeup step0p1 */
      prof_hist_update(&tds_mc_wakeup_step0p1_hist, overall_wakeup_time);
  
      if (prof_hist_log_periodic(&rf_tds_hist_packet_handle))
      {
        prof_hist_clear(&tds_mc_wakeup_step0p1_hist);
    }
  }
  }
...
(
  ...
  rfa_tdscdma_rxlm_mode_t rxlm_mode_div
#ifdef RF_TDSCDMA_RXD_TIMELINE_OPT
  ,rfa_tdscdma_wakeup_rxd_ind_t rx_rxd  //placeholder
#endif
#endif
  ,boolean* rebuild_ifreq_script
)
{
  ...
  if(rf_tdscdma_mc_display_time_profile)
  {
    ...
    if (rf_tdscdma_mc_wakeup_time_debug)
    {
      ...
      /*updating the histogram for wakeup step0p2 */
      prof_hist_update(&tds_mc_wakeup_step0p2_hist, overall_wakeup_time);
  
      if (prof_hist_log_periodic(&rf_tds_hist_packet_handle))
      {
        prof_hist_clear(&tds_mc_wakeup_step0p2_hist);
      }
    }
  }
...
)
{
 ...
 if (rfcommon_nv_get_lpm_feature_enabled())
 {
  ...
  if(((rf_tdscdma_mc_state.curr_rx_path == RFA_TDSCDMA_RX_PATH_BOTH)||
  	 (rf_tdscdma_mc_state.curr_rx_path == RFA_TDSCDMA_RX_PATH_DIV)) //check RxD status
#ifdef RF_TDSCDMA_RXD_TIMELINE_OPT
     && (rx_rxd == RFA_TDSCDMA_WAKEUP_PRIM_DIV) //check RxD status, not wakeup the sec for non-RxD case
#endif
	 )
  {
  ...
  if( ((rx_path == RFA_TDSCDMA_RX_PATH_BOTH) ||
      (rx_path == RFA_TDSCDMA_RX_PATH_DIV)    )
#ifdef RF_TDSCDMA_RXD_TIMELINE_OPT
/*skip configuration for second chain when L1 ind next wakeup is RxD only*/
       && (rx_rxd == RFA_TDSCDMA_WAKEUP_PRIM_DIV)
#endif
      )
  {
    rf_tdscdma_mc_config_rx_chain(sec_device,
  ...
  if(rf_tdscdma_mc_display_time_profile)
  {
    ...
    /*updating the histogram for wakeup step1 */
    prof_hist_update(&tds_mc_wakeup_step1_hist, overall_wakeup_time);
  
    if (prof_hist_log_periodic(&rf_tds_hist_packet_handle))
    {
      prof_hist_clear(&tds_mc_wakeup_step1_hist);
    }
...
uint16 rf_tdscdma_mc_get_rf_warmup_time(void)
{
  ...
  rfc_core_config = (rfc_tdscdma_core_config_type*)rfc_tdscdma_mode_config_get(rf_tdscdma_mc_state.curr_pri_device, RFC_MODE_CORE_DATA);
...
/*!
  ...
  @details
*/
uint16 rf_tdscdma_mc_get_tx_modem_chain(rfm_device_enum_type pri_device, rfm_device_enum_type tx_device)
{
  rfc_cfg_params_type cfg;
  rfc_device_info_type *device_info_ptr;
  uint32 tx_modem_chain = 0;
  rfc_cal_device_remap_info_type source_device_info; /* src of device info structure */
  rfc_cal_device_remap_info_type remapped_device_info; /* remapped device info structure */ 
  rfm_device_enum_type remapped_mdevice; /* remapped Master device */
  sys_band_class_e_type sys_band = SYS_BAND_CLASS_MAX;
  rfcom_band_type_u u_tds_band;
 
  /* rfc not configured, return error */
  if ( rf_tdscdma_mc_state.rf_state[RF_PATH_0] == RF_TDSCDMA_STATE_DEFAULT )
  {
	MSG(MSG_SSID_RF, MSG_LVL_ERROR, "rf_tdscdma_mc_get_tx_modem_chain: called in wrong state");
	return 0xFFFF;
  }

  MSG_2(MSG_SSID_RF_DEBUG, MSG_LVL_HIGH ,"rf_tdscdma_mc_get_tx_modem_chain:"
                                   " pri_device = %d , tx_device = %d", pri_device , tx_device  );  

  /* query RFC for remapped device ID */
  /* populate src query structure */
  source_device_info.device = pri_device;
  source_device_info.tech = RFM_TDSCDMA_MODE;
  source_device_info.rxtx = RFC_CONFIG_RX;
  source_device_info.band = RFCOM_BAND_TDSCDMA_B34;
  source_device_info.alt_path = 0; /* use 0 for alt_path for now */  

  /* query RFC for remapped Tx device */
  rfc_common_get_remapped_device_info( &source_device_info , &remapped_device_info );
  u_tds_band.tdscdma_band = (rfcom_tdscdma_band_type)source_device_info.band;
  sys_band = rfcommon_core_convert_rfcom_band_to_sys_band(RFM_TDSCDMA_MODE, u_tds_band);
  remapped_mdevice = rfc_common_get_preferred_tx_device(sys_band);

  cfg.band = RFCOM_BAND_TDSCDMA_B34;
  cfg.req = RFC_REQ_DEFAULT_GET_DATA;
  cfg.logical_device = remapped_mdevice;
  ...
  {
	tx_modem_chain = device_info_ptr->modem_chain;

    MSG_2(MSG_SSID_RF, MSG_LVL_HIGH ,"rf_tdscdma_mc_get_tx_modem_chain:"
              " remapped_mdevice = %d , tx_modem_chain = %d ", remapped_mdevice , tx_modem_chain  );	
  }
  else
  {
	  RF_MSG_1( RF_ERROR, "rf_tdscdma_mc_get_tx_modem_chain:"
				" could not get chain from dev %d", remapped_mdevice ); 
	return 0xFFFF;
  }

  return (uint16)tx_modem_chain;
}
...
)
{
  ...
  MSG_8(MSG_SSID_RF, MSG_LVL_HIGH,
        "rf_tdscdma_mc_update_bho(pri_device=%d, sec_device=%d, tx_device=%d, prim_chan=%d, tgt_chan=%d, rxlm_buf_idx=%d, rxlm_mode=%d, txlm_buf_idx=%d)",
        pri_device, sec_device, tx_device, tgt_prim_channel, tgt_channel, rxlm0_tgt_buffer_idx, rxlm_mode, txlm_tgt_buffer_idx);
  ...
  *tx_tgt_buffer_idx = rf_tdscdma_mc_dbl_bufs.tx_tgt_buf_idx;
...
#ifdef FEATURE_RF_HAS_QTUNER
  /* disable tuner */
  /* note: at this point tx_device/band/chan in mc_state hasn't been updated to the target device/band/chan yet */
  (void)rf_tdscdma_core_antenna_tuner_tx_mode_disable(rf_tdscdma_mc_state.curr_tx_device,
                                                      rf_tdscdma_mc_state.curr_band_tx, 
                                                      rf_tdscdma_mc_state.curr_chan_tx,
                                                      RFDEVICE_EXECUTE_IMMEDIATE,
  ...
  rf_tdscdma_mc_state.curr_pri_paired_device= rfc_common_get_preferred_associated_rx_device(pri_device);    
  rf_tdscdma_mc_state.curr_sec_device = sec_device;
  rf_tdscdma_mc_state.curr_tx_device  = tx_device;
  ...
  /* Update current common device pointer information */
  rf_tdscdma_mc_cmn_device_ptr = rf_tdscdma_mc_get_curr_common_device(tx_device,
                                                                      band);
  ...
  rf_tdscdma_mc_update_tx_mdsp(tx_device,
                               pri_path,
                               band,
                               tgt_channel,
                               txlm_tgt_buffer_idx,
  ...
  {
    /* FBRX init */
    fbrx_status = rfcommon_fbrx_mc_init_tx(rf_tdscdma_fbrx_lm_handle, RFCOMMON_FBRX_TDS, txlm_tgt_buffer_idx, RFM_TDSCDMA_MODE, 0);
    /* Regsiter the VSWR post processing call back function to FBRX module */
    fbrx_status &= rfcommon_fbrx_mc_register_vswr_meas_call_back(rflm_tds_mc_post_process_fbrx_vswr_measurements,
                                                                 txlm_tgt_buffer_idx);
    if (fbrx_status == RFCOMMON_FBRX_ERROR)
    ...
    if (fbrx_status == RFCOMMON_FBRX_SUCCESS)
    {
      fbrx_chan_list[0] = (uint32)tgt_channel;
      fbrx_tx_params.apt_only_mode = FALSE;
      /* @todo: remove hard-coding, tech to update the fbrx chain from rfc */
      if (band < RFCOM_NUM_TDSCDMA_BANDS)   //KW fix ...
      {
        fbrx_status = rfcommon_fbrx_mc_tune_to_chan(tx_device,
                                                                txlm_tgt_buffer_idx,
                                                                RFM_TDSCDMA_MODE,
                                                                *((rfcom_band_type_u *)(&band)),
                                                                fbrx_chan_list,
    ...
    if (fbrx_status == RFCOMMON_FBRX_SUCCESS)
    {
      rf_tdscdma_fbrx_modem_chain = rfc_tdscdma_get_fb_chain(tx_device,band );
      fbrx_status = rfcommon_fbrx_enable(rf_tdscdma_fbrx_lm_handle, rf_tdscdma_fbrx_modem_chain);
    }
  ...
  if (script_status)
  {
	MSG_8(MSG_SSID_RF_DEBUG, MSG_LVL_MED,
		  "Rx wakeup script built for pri_device %d, pri_paired_device %d, sec_device %d, tx_device %d, Band %d, Chan %d, Div_EN %d, is_BHO %d",
		  rf_tds_build_event_data.pri_device,
		  rf_tds_build_event_data.pri_paired_device,		  
  ...
  {
	MSG_8(MSG_SSID_RF, MSG_LVL_ERROR,
		  "Rx wakeup script built ERROR for pri_device %d, pri_paired_device %d, sec_device %d, tx_device %d, Band %d, Chan %d, Div_EN %d, is_BHO %d",
		  rf_tds_build_event_data.pri_device,
		  rf_tds_build_event_data.pri_paired_device,		  		  
		  rf_tds_build_event_data.sec_device,
		  rf_tds_build_event_data.tx_device,			
  ...
  if(rf_tdscdma_mc_display_time_profile || rf_tdscdma_mc_tx_time_debug)
  {
    ...
    /*updating the histogram for update bho */
    prof_hist_update(&tds_mc_update_bho_hist, overall_update_bho_time);

    if (prof_hist_log_periodic(&rf_tds_hist_packet_handle))
    {
      prof_hist_clear(&tds_mc_update_bho_hist);
    }
  }
...
)
{
  ...
  if (buf_swap == RFA_TDSCDMA_BUF_RXTX) /* 0ms BHO */
  {
    ...
    /* apply the fbrx temp comp one time */
    rf_tdscdma_core_temp_comp_fbrx_once(rf_tdscdma_mc_state.curr_chan_tx);
  }
...
rf_tdscdma_mc_status_type rf_tdscdma_mc_therm_backoff(uint8 backoff)
{
  ...
  if ( rf_tdscdma_mc_state.rf_state[RF_PATH_0] == RF_TDSCDMA_STATE_RXTX )
  {
    MSG_1(MSG_SSID_RF_DEBUG, MSG_LVL_HIGH, "rf_tdscdma_mc_therm_backoff: backoff=%d", backoff);
...
)
{
  ...
  if (is_wtr_hdet)
  {
    rfdevice_tdscdma_tx_cmd_dispatch(rf_tdscdma_mc_state.curr_tx_device,
                                     RFCOM_TDSCDMA_MODE,
                                     rf_tdscdma_mc_state.curr_band_tx,
  ...
  {
    (void)rfdevice_hdet_tdscdma_calc_hdet_measurement(rf_tdscdma_mc_state.curr_tx_device,
                                                      rf_tdscdma_mc_state.curr_band_tx,
                                                      &hdet_meas_data);
  }
...
/*!
  @brief
  Calculate HDET reading from the WTR register reads

  @details
*/

void rf_tdscdma_fbrx_read_hdet_calculate_reading(int32 fbrx_sum_i, int32 fbrx_sum_q, uint64 fbrx_sum_i_q_sqr, uint32 fbrx_accum, uint8 fbrx_accum_overflow, boolean* is_hdet_valid, uint16* hdet_reading)
{
  uint64 abs_sum_i = 0;
  uint64 abs_sum_q = 0;
  uint64 hdet_reading_u64 = 0;

  if ((fbrx_accum_overflow & (1<<1)) == 0)
  { 
    abs_sum_i = (uint64) abs(fbrx_sum_i);
    abs_sum_q = (uint64) abs(fbrx_sum_q);

    /* avg_hdet = (sum_i_and_q_squred - (sum_i^2+sum_q^2)/n)/n */
    hdet_reading_u64 =
     (uint64) ((fbrx_sum_i_q_sqr<<8)-((abs_sum_i*abs_sum_i+abs_sum_q*abs_sum_q)/fbrx_accum))/fbrx_accum;
    ...
    else
    {
      ...
      *hdet_reading = (uint16) ((hdet_reading_u64 & 0x1fffe000) >> 13 );
    }
...
void rf_tdscdma_mc_trigger_hdet(int16 rflm_txagcdb10)
{
  ...
  /* For HDET calculation */

  uint8 fbrx_sum_i_7_0 = 0;
  uint8 fbrx_sum_i_15_8 = 0;
  uint8 fbrx_sum_i_23_16 = 0;
  uint8 fbrx_sum_i_31_24 = 0;
  uint8 fbrx_sum_q_7_0 = 0;
  uint8 fbrx_sum_q_15_8 = 0;
  uint8 fbrx_sum_q_23_16 = 0;
  uint8 fbrx_sum_q_31_24 = 0;
  uint8 sum_i_q_07_00 = 0;
  uint8 sum_i_q_15_08 = 0;
  uint8 sum_i_q_23_16 = 0;
  uint8 sum_i_q_31_24 = 0;
  uint8 sum_i_q_39_32 = 0;
  uint8 fbrx_accum_count_h = 0;
  uint8 fbrx_accum_count_l = 0;

  int32 fbrx_sum_i = 0;         
  int32 fbrx_sum_q = 0;  
  uint64 fbrx_sum_i_q_sqr = 0;  
  uint32 fbrx_accum = 0;        
  uint8 fbrx_accum_overflow = 0;

  uint16 hdet_reading=0;
  boolean is_hdet_valid = FALSE;
  ...
  {     
  	(void)rfdevice_hdet_tdscdma_tx_hdet_read(rf_tdscdma_mc_state.curr_tx_device,
                                             rf_tdscdma_mc_state.curr_band_tx,
                                             TRUE, 
                                             &raw_reading,
                                             NULL,
  ...
  /* Disable HDET */
  rfdevice_hdet_tdscdma_tx_hdet_disable(rf_tdscdma_mc_state.curr_tx_device,rf_tdscdma_mc_state.curr_band_tx);

  uint8 * readbackPtr = (uint8*)(((rflm_ccs_rf_event_header_task_type *)&(((rflm_tds_ccs_event_t*)rflmTdsRfModeCtlState.rflmTdsDmTechBuffPtr)->doHdetEventHdetReadScript))->task_buf[0].task.rffe_read_ext.transaction_ptr);

  fbrx_sum_i_7_0 = *(readbackPtr+4);
  fbrx_sum_i_15_8 = *(readbackPtr + 5);
  fbrx_sum_i_23_16 = *(readbackPtr + 6);
  fbrx_sum_i_31_24 = *(readbackPtr + 7);
  fbrx_sum_q_7_0 = *(readbackPtr + 12);
  fbrx_sum_q_15_8 = *(readbackPtr + 13);
  fbrx_sum_q_23_16 = *(readbackPtr + 14);
  fbrx_sum_q_31_24 = *(readbackPtr + 15);
  sum_i_q_07_00 = *(readbackPtr + 20);
  sum_i_q_15_08 = *(readbackPtr + 21);
  sum_i_q_23_16 = *(readbackPtr + 22);
  sum_i_q_31_24 = *(readbackPtr + 23);
  sum_i_q_39_32 = *(readbackPtr + 28);
  fbrx_accum_count_h = *(readbackPtr + 36);
  fbrx_accum_count_l = *(readbackPtr + 44);

    /* sum_i is 32s16 */
  fbrx_sum_i = (int32) ( ((uint32)fbrx_sum_i_31_24 << 24) | ((uint32)fbrx_sum_i_23_16 << 16) | ((uint32)fbrx_sum_i_15_8 << 8) | (uint32)fbrx_sum_i_7_0 );

  /* sum_q is 32s16 */
  fbrx_sum_q = (int32) ( ((uint32)fbrx_sum_q_31_24 << 24) | ((uint32)fbrx_sum_q_23_16 << 16) | ((uint32)fbrx_sum_q_15_8 << 8) | (uint32)fbrx_sum_q_7_0 );

  /* sum_i_q_sqr is 40u24 */
  fbrx_sum_i_q_sqr = ((uint64)sum_i_q_39_32 << 32) | ((uint64)sum_i_q_31_24 << 24 ) | ((uint64)sum_i_q_23_16 << 16) | ((uint64)sum_i_q_15_08 << 8 ) | (uint64)sum_i_q_07_00;

  /* fbrx_accum is 12 bits */
  fbrx_accum = ( ( (uint32)fbrx_accum_count_h << 8 ) | (uint32)fbrx_accum_count_l ) & 0xfff;

  fbrx_accum_overflow = *(readbackPtr + 29);

  /* doesn't build script */
  rf_tdscdma_fbrx_read_hdet_calculate_reading( fbrx_sum_i,
                                               fbrx_sum_q,		   
                                               fbrx_sum_i_q_sqr,   
                                               fbrx_accum,		   
                                               fbrx_accum_overflow, 
  ...
  if (rf_tdscdma_mc_hdet_debug)
  {
    RF_MSG_7(RF_HIGH,"wtr2965_read_hdet: sum_i_q_sqr: %x%08x, "
                    "sum_i: %x, sum_q: %x, accum: %x, "
                    "overflow: %x, hdet_reading %d", 
                    fbrx_sum_i_q_sqr >> 32,fbrx_sum_i_q_sqr, 
                    fbrx_sum_i, fbrx_sum_q,
                    fbrx_accum, fbrx_accum_overflow,
                    hdet_reading);
  }

  hdet_meas_data.hdet_avg_measurement = hdet_reading;
...
{
#ifndef FEATURE_TABASCO_MODEM
  rf_tdscdma_core_temp_comp_timer_process();
#else
  uint16 therm_read;
...
void rf_tdscdma_mc_device_status_trigger_read(void* ccs_arg)
{
  rf_tdscdma_rf_event_device_status_trigger_read(ccs_arg);
}
...
)
{
  ...
  iqmc_cfg1_0 = rf_tdscdma_msm_calc_iqmc_setting(rf_tdscdma_mc_state.curr_pri_device,
                                                 rf_tdscdma_mc_state.curr_chan_rx,
  ...
  if (rf_tdscdma_mc_state.div_enable)
  {
    iqmc_cfg1_1 = rf_tdscdma_msm_calc_iqmc_setting(rf_tdscdma_mc_state.curr_sec_device,
                                                   rf_tdscdma_mc_state.curr_chan_rx,
...
)
{
...
#ifdef FEATURE_RF_TDS_ASDIV_ELNA_GAIN
  rfnv_tdscdma_rx_tbl_type *rfnv_rx_ptr = NULL;
  rfnv_tdscdma_tx_tbl_type *rfnv_tx_ptr = NULL;
#endif

  RF_MSG_1( RF_HIGH, "rf_tdscdma_mc_init: START. cmn success %d", common_init_successful );
  ...
  {
    RF_MSG( RF_ERROR, "rf_tdscdma_mc_init: NV init skipped due to cmn init failure. Cannot operate in TDSCDMA." );
  }
...
#ifdef FEATURE_RF_TDS_ASDIV_ELNA_GAIN
  /*Zero out the eLNA offset NVs for both bands*/
  rfnv_rx_ptr = rfnv_tdscdma_get_rx_tbl_ptr(RFNV_TDSCDMA_B34);
  rfnv_tx_ptr = rfnv_tdscdma_get_tx_tbl_ptr(RFNV_TDSCDMA_B34);
  if (!rf_tds_AsDiv_eLNA_gain_offset_B34_NV_present)
  {
    memset(&rfnv_rx_ptr->rx_elna_gain_offset_swpt, 0, sizeof(rf_tdscdma_elna_static_rx_data_type));
  }
  if (!rf_tds_AsDiv_tx_offset_B34_NV_present)
  {
    memset(&rfnv_tx_ptr->tx_elna_offset, 0, sizeof(rf_tdscdma_elna_static_tx_data_type));
  }
  rfnv_rx_ptr = rfnv_tdscdma_get_rx_tbl_ptr(RFNV_TDSCDMA_B39);
  rfnv_tx_ptr = rfnv_tdscdma_get_tx_tbl_ptr(RFNV_TDSCDMA_B39);
  if (!rf_tds_AsDiv_eLNA_gain_offset_B39_NV_present)
  {
    memset(&rfnv_rx_ptr->rx_elna_gain_offset_swpt, 0, sizeof(rf_tdscdma_elna_static_rx_data_type));
  }

  if (!rf_tds_AsDiv_tx_offset_B39_NV_present)
  {
    memset(&rfnv_tx_ptr->tx_elna_offset, 0, sizeof(rf_tdscdma_elna_static_tx_data_type));
  }
...
#ifdef FEATURE_QSH_MDUMP
  /* regiter QSH minidump callback function */
  ret_val = rf_qsh_cb_register(RFM_TDSCDMA_MODE,&rftech_tdscdma_QSH_mdump_cb);
  if(FALSE == ret_val)
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR, "MC Init register QSH minidump callback failed.");
  }
#endif
  ...
  RF_MSG( RF_HIGH, "rf_tdscdma_mc_init: END. success 1" );
...
)
{
     ...
     (wakeup_step_ind == RFA_TDSCDMA_WAKEUP_STEP0P1)  )
  {
    ...
    if (path == RF_PATH_0)
    {
      if ( (IS_FTM_IN_TEST_MODE() == TRUE) && (rfm_get_calibration_state() == FALSE) )
      {
        ...
        boolean cleanup_state;
        rfcom_band_type_u u_band;
        ftm_concurrency_manager_params_type conmgr_params;
        
        // Cleanup FTM concurrency manager state machine
        cleanup_state = ftm_concurrency_manager_cleanup_device_allocation(rf_tdscdma_mc_state.curr_pri_device, 
                                                                          RFM_TDSCDMA_MODE);
        // Always "blindly" reset Secondary device because pri_device/sec_device should always on the same tech, same channel
        cleanup_state = ftm_concurrency_manager_cleanup_device_allocation(rf_tdscdma_mc_state.curr_sec_device, 
                                                                          RFM_TDSCDMA_MODE);
    
        // Check FTM concurrency manager
        ...
        {        
          MSG_3(MSG_SSID_RF, MSG_LEGACY_HIGH,"Invalid combination to test FTM RF concurrency! Device: %d, band: %d, Path: %d", 
                    rf_tdscdma_mc_state.curr_pri_device,
                    band,
                    RFC_PRX_PATH);
        }
      ...
      for(loop_band = RFCOM_BAND_TDSCDMA_B34; loop_band <= RFCOM_BAND_TDSCDMA_B39; loop_band++)
      {
        ...
        if (alt_path_status == RFCMN_PATH_SEL_FAILED)
        {
          MSG_3(MSG_SSID_RF, MSG_LEGACY_HIGH,"cannot get alt_path for device %d, TDS band %d, rfc_band %d", 
        ...
        if (rfc_alt_path_status == FALSE)
        {
          MSG_3(MSG_SSID_RF, MSG_LEGACY_HIGH,"cannot get RFC alt_path for device %d, TDS band %d, rfc_band %d", 
                    rf_tdscdma_mc_state.curr_pri_device, loop_band, rfc_band);
        }
    ...
    /* Update rxlm buffer(both static and dynamic setting) */
    rf_tdscdma_msm_update_rxlm_buffer(device, rx_channel, rxlm_mode, rxlm_buffer_idx, &wb_adc_idx, rfdevice_op_mode);
    ...
    rflm_dm_get_handle_info(  rxlm_buffer_idx , &handle_meta);
    handle_meta.devide_id = device;
    handle_meta.device_id = device;
    rflm_dm_set_handle_info( rxlm_buffer_idx , &handle_meta );
    /* log channel number */
    rf_power_reporting_update_channel( device, rx_channel );
    #endif  
  ...
  if (path == RF_PATH_0)
  {
    ...
    else
    {
      ...
      if (script_result)
      {
        ...
        MSG_7(MSG_SSID_RF, MSG_LVL_MED,
              "Rx wakeup script built for pri_device %d, pri_paired_device %d, sec_device %d, Band %d, Chan %d, Div_EN %d, is_BHO %d",
              rf_tds_build_event_data.pri_device,
              rf_tds_build_event_data.pri_paired_device,              
              rf_tds_build_event_data.sec_device,
              rf_tds_build_event_data.band,
      ...
      else
      {
        MSG_7(MSG_SSID_RF, MSG_LVL_ERROR,
              "Rx wakeup script built ERROR for pri_device %d, pri_paired_device %d, sec_device %d, Band %d, Chan %d, Div_EN %d, is_BHO %d",
              rf_tds_build_event_data.pri_device,
              rf_tds_build_event_data.pri_paired_device,               
              rf_tds_build_event_data.sec_device,
              rf_tds_build_event_data.band,
  ...
  MSG_8(MSG_SSID_RF, MSG_LVL_HIGH, "rf_tdscdma_mc_update_rx_mdsp(device %d, path %d, band %d, rx_channel %d, rxlm_buffer_idx %d, rxlm_mode %d, call_in_wakup %d , rfdevice_op_mode %d) is complete", \
        device, path, band, rx_channel, rxlm_buffer_idx, rxlm_mode, call_in_wakeup, rfdevice_op_mode);
...
)
{
  ...
  if (path == RF_PATH_0)
  {
    ...
    rflm_dm_handle_meta_s handle_meta;
    rflm_dm_get_handle_info(  rxlm_buffer_idx , &handle_meta);
    handle_meta.devide_id = rf_tdscdma_mc_state.curr_pri_device;
    handle_meta.device_id = rf_tdscdma_mc_state.curr_pri_device;
    rflm_dm_set_handle_info( rxlm_buffer_idx , &handle_meta );
    /* log channel number */
    rf_power_reporting_update_channel( rf_tdscdma_mc_state.curr_pri_device, rx_channel );
    #endif  
        ...
        (rf_tdscdma_mc_state.curr_rx_path == RFA_TDSCDMA_RX_PATH_DIV)   )
    {
      ...
      rflm_dm_handle_meta_s handle_meta;
      rflm_dm_get_handle_info(  rxlm_buffer_idx , &handle_meta);
      handle_meta.devide_id = rf_tdscdma_mc_state.curr_sec_device;
      handle_meta.device_id = rf_tdscdma_mc_state.curr_sec_device;
      rflm_dm_set_handle_info( rxlm_buffer_idx , &handle_meta );
      /* log channel number */
      rf_power_reporting_update_channel( rf_tdscdma_mc_state.curr_sec_device, rx_channel );
      #endif                                    
    }
  ...
  /*=========================================================================*/
#ifdef FEATURE_RF_HAS_QTUNER
  (void)rf_tdscdma_core_antenna_tuner_init(device, band, RFDEVICE_EXECUTE_IMMEDIATE, NULL, rfdevice_op_mode);
  ...
  {
    MSG_2(MSG_SSID_RF, MSG_LVL_ERROR,
          "ASM Device wakeup failed on Device %d and Band %d",
          device, band);
  }

  elna_status = rfdevice_tdscdma_elna_wakeup(device,
                                             band,
                                             NULL,
                                             RFDEVICE_EXECUTE_IMMEDIATE,
                                             RFCOM_INVALID_TIMING_OFFSET);

  if(elna_status == FALSE)
  {
    MSG_2(MSG_SSID_RF, MSG_LVL_ERROR,
          "eLNA Device wakeup failed on Device %d and Band %d",
          device, band);
  }												  
  ...
  MSG_9(MSG_SSID_RF, MSG_LVL_HIGH, 
        "rf_tdscdma_mc_config_rx_chain(device %d, path %d, band %d, rx_channel %d, rxlm_buffer_idx %d, rxlm_mode %d, call_in_wakeup=%d, sw_tune_in_acq=%d, rfdevice_op_mode=%d) is complete", \
        device, path, band, rx_channel, rxlm_buffer_idx, rxlm_mode, call_in_wakeup, use_sw_tune_in_acq, rfdevice_op_mode);
...
/*!
  @brief
 Put individual Rx chain into sleep.

  @details
  - Put WTR into deep sleep
  - Put ASM into sleep
  - Put eLNA into sleep
  - Disable tuner  
...
)
{
 ...
 #ifdef FEATURE_RF_HAS_QTUNER
  (void)rf_tdscdma_core_antenna_tuner_disable(device,
                                              band,
                                              RFDEVICE_EXECUTE_IMMEDIATE,
                                              NULL,
                                              0,
  ...
  rfdevice_tdscdma_rx_sleep(device,
                            RFCOM_TDSCDMA_MODE,
                            band,
                            RFDEVICE_EXECUTE_IMMEDIATE,
                            NULL,
   ...
   // added for ASM off
  rfdevice_tdscdma_asm_disable_rx(device, 
                                  band,
                                  NULL, 
				 				  RFDEVICE_EXECUTE_IMMEDIATE, 
                                  RFCOM_INVALID_TIMING_OFFSET, 
  ...
  rfdevice_tdscdma_asm_sleep_rx(device,
                                band,
                                NULL,
                                RFDEVICE_EXECUTE_IMMEDIATE,
                                RFCOM_INVALID_TIMING_OFFSET);

  elna_status = rfdevice_tdscdma_elna_sleep(device,
                                            band,
                                            NULL,
                                            RFDEVICE_EXECUTE_IMMEDIATE,
                                            RFCOM_INVALID_TIMING_OFFSET);

  ant_position = rfcommon_asdiv_get_current_position(device);
  rf_tdscdma_mc_asdiv_sleep_script(ant_position, device, band);

  if(elna_status == FALSE) 
  {
    MSG_2(MSG_SSID_RF, MSG_LVL_ERROR,
          "eLNA Device sleep failed on Device %d and Band %d",
          device, band);
  }
  ...
  {
    /* Do once on primary chain ONLY */
    rfc_tdscdma_sleep(device);
  }

  MSG_5(MSG_SSID_RF, MSG_LVL_HIGH,
        "Sleep Rx chain for Device %d, Path %d, Band %d and Chan %d, rfdevice_op_mode %d",
        device, path, band, rx_channel, rfdevice_op_mode);
}
...
)
{
  ...
  rf_tdscdma_msm_update_txlm_buffer(device, txlm_buffer_idx, band, is_bho_update,&adc_idx);
  ...
  /* Build RF scripts (Coupler FWD and REV for VSWR)     										 */
  /*=========================================================================*/
  /*  Skipped in CAL mode                                                    */
  ...
  rf_tdscdma_mdsp_rf_event_cmd_dispatch(RF_TDS_EVT_COUPLER_FWD, &rf_tds_build_event_data);
  rf_tdscdma_mdsp_rf_event_cmd_dispatch(RF_TDS_EVT_COUPLER_REV, &rf_tds_build_event_data);
  ...
  /* calc PA comp and update data into RFLM */
  rf_tdscdma_core_calc_pa_comp_vs_freq(band, tx_channel);
  ...
  /* decide to enable GNSS blanking or not */
  rf_tdscdma_mc_update_gnss_blanking_state(band, txlm_buffer_idx);
  MSG_2(MSG_SSID_RF, MSG_LVL_HIGH, "rf_tdscdma_mc_update_tx_mdsp: GNSS Blanking, band %d, rxlm_buffer_idx %d", \
        band, txlm_buffer_idx);

  MSG_7(MSG_SSID_RF, MSG_LVL_HIGH, "rf_tdscdma_mc_update_tx_mdsp(device %d, path %d, band %d, rx_channel %d, rxlm_buffer_idx %d, buf_id=%d, adc_idx=%d) is complete", \
        device, path, band, tx_channel, txlm_buffer_idx, buf_id, adc_idx);
...
)
{
  ...
  /* Update rxlm buffer(both static and dynamic setting) */
  rf_tdscdma_msm_update_rxlm_buffer(RFM_DEVICE_0, rx_channel, rxlm_mode, rxlm_buffer_idx, &wb_adc_idx , RFDEVICE_PRX_OPER_MODE);
...
)
{
  ...
  /* Update rxlm buffer(both static and dynamic setting) */
  rf_tdscdma_msm_update_rxlm_buffer(device, rx_channel, rxlm_mode, rxlm_buffer_idx, &wb_adc_idx, RFDEVICE_PRX_OPER_MODE);

  MSG_4(MSG_SSID_RF, MSG_LVL_HIGH, "rf_tdscdma_mc_update_irat_rxlm_mode(%d, %d, %d, %d)", device, rx_channel, rxlm_buffer_idx, rxlm_mode);
...
{
  MSG_1(MSG_SSID_RF, MSG_LVL_ERROR, "rf_tdscdma_mc_set_sm_rfmode_tx: Device %d", device);
...
boolean rf_tdscdma_mc_program_qfe_scenario(void)
{
  ...
  if( (rf_tdscdma_mc_state.rf_state[RF_PATH_0] == RF_TDSCDMA_STATE_RX) || (rf_tdscdma_mc_state.rf_state[RF_PATH_0] == RF_TDSCDMA_STATE_RXTX) )
  {
    ...
    if((tuner_mgr_p != NULL) && (tuner_mgr_p[0] != NULL))
    {
      ...
      atuner_cfg_req.band.tdscdma_band = rf_tdscdma_mc_state.curr_band_rx;
      atuner_cfg_req.carrier_id        = RFCMN_ATUNER_CARRIER_ID_0;
      atuner_cfg_req.chan_num          = rf_tdscdma_mc_state.curr_chan_rx;
      atuner_cfg_req.mode              = RFM_TDSCDMA_MODE;
      atuner_cfg_req.rfm_device        = p_device;
      atuner_cfg_req.rx_tx             = RFCMN_ATUNER_PRX_DEVICE;
      
      while (tuner_mgr_p[tuner_mgr_index] != NULL)
      {
        status &= rfcommon_atuner_force_detune( tuner_mgr_p[tuner_mgr_index], 
                                                &atuner_cfg_req,
                                                &dummy_token,
                                                RFDEVICE_EXECUTE_IMMEDIATE,
                                                NULL,
    ...
    if((tuner_mgr_s != NULL) && (tuner_mgr_s[0] != NULL))
    {
      ...
      atuner_cfg_req.chan_num            = rf_tdscdma_mc_state.curr_chan_rx;
      atuner_cfg_req.mode                = RFM_TDSCDMA_MODE;
      atuner_cfg_req.rfm_device          = s_device;
      atuner_cfg_req.rx_tx               = RFCMN_ATUNER_DRX_DEVICE;
      
      while (tuner_mgr_s[tuner_mgr_index] != NULL)
      {
        status &= rfcommon_atuner_force_detune( tuner_mgr_s[tuner_mgr_index], 
                                                &atuner_cfg_req,
                                                &dummy_token,
                                                RFDEVICE_EXECUTE_IMMEDIATE,
                                                NULL,
...
void rf_tdscdma_mc_update_tuner_code(boolean is_immediate)
{
  ...
  void** tuner_mgr_p = NULL;
  void** tuner_mgr_s = NULL;
  ...
  if(rfcommon_asdiv_manager_try_enter_lock() == TRUE)
  {
   ...
   {
     MSG(MSG_SSID_RF_DEBUG, MSG_LEGACY_MED,  
         "rf_tdscdma_mc_update_tuner_code: [ASDIV] Switch in progress OR update in progress, Skip update");
  ...
  else if (rf_tdscdma_mc_state.rf_state[RF_PATH_0] == RF_TDSCDMA_STATE_RXTX )
  {
	...
	p_device = rf_tdscdma_mc_state.curr_tx_device; // if we are in connected state , need to pass Tx device instead
  ...
  if ( tuner_mgr_p != NULL )
  {
    tuner_cb_data->tuner_data[RF_TUNER_UPDATE_CHAIN_0].ant_tuner = tuner_mgr_p[0];
  }

  master_chain_token_ptr = &(tuner_cb_data->tuner_data[RF_TUNER_UPDATE_CHAIN_0].tuner_token);

  /*Init value*/
  *master_chain_token_ptr = ATUNER_INVALID_TOKEN_VAL;

  /* Obtain the RF script on Primary device */
  status_flag &= rf_tdscdma_core_antenna_tuner_update_tune_code( p_device, 
  												  tds_curr_band, 
  												  tds_curr_chan, 
  												  execution_type,
  												  tuner_script, 
  												  0,
  												  (uint32*)master_chain_token_ptr,
  												  RFDEVICE_PRX_OPER_MODE); // if we put Tx device , we don't have to worried this PRX/DRX OPER mode parameter
      ...
      rf_tdscdma_mc_state.curr_rx_path == RFA_TDSCDMA_RX_PATH_BOTH)
  {
   ...
   /* Update tune code for Secondary device */
    ...
    if ( tuner_mgr_s != NULL )
    {	 
      tuner_cb_data->tuner_data[RF_TUNER_UPDATE_CHAIN_1].ant_tuner = tuner_mgr_s[0];
    }

	 /*Obtain ccs event handle from call back data*/
	 slave_chain_token_ptr = &(tuner_cb_data->tuner_data[RF_TUNER_UPDATE_CHAIN_1].tuner_token);

	 /*Init value*/
	 *slave_chain_token_ptr = ATUNER_INVALID_TOKEN_VAL;
  
   /* Obtain the RF script on Secondary device */
   status_flag &= rf_tdscdma_core_antenna_tuner_update_tune_code( s_device, 
  												   tds_curr_band, 
  												   tds_curr_chan, 
  												   execution_type,
  												   tuner_script, 
  												   0,
  												   (uint32*)slave_chain_token_ptr,
  												   RFDEVICE_DRX_OPER_MODE);
  ...
  {
    MSG_1(MSG_SSID_RF_DEBUG, MSG_LEGACY_MED,
...
void rf_tdscdma_mc_rflm_init_rxagc(rflm_dm_handle_id_t rxagc_handle)
{
  ...
  MSG_1(MSG_SSID_RF_DEBUG, MSG_LEGACY_HIGH, "rf_tdscdma_mc_rflm_init_rxagc is obsolete and skipped", 0);
#if 0
  uint8 i;
...
)
{
  ...
  if (is_irat)
  {
    for (lin_state=0; lin_state <RFLM_TDS_NUM_LIN_STATES; lin_state++)
    {    
      // IRAT
      ...
      for (index=0; index< RF_TDSCDMA_MAX_LNA_GAIN_STATES ; index++)
      {
        rflmTdsRfModeCtlState.rxAgc[lin_state].irat.lnaOffset[index] = 
          data_for_cells[des_lin_state].cell_static_info.lna_gain_offsets[index];
      }
    }
  }
  else
...
/*!
  @brief
  This function is used to return Tx device which is being used
...
*/
rfm_device_enum_type rf_tdscdma_mc_get_curr_tx_device(void)
{
  rfm_device_enum_type tx_device = RFM_INVALID_DEVICE;
  
  if ( rf_tdscdma_mc_state.rf_state[RF_PATH_0] != RF_TDSCDMA_STATE_RXTX )
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rf_tdscdma_mc_get_curr_tx_device is called in the wrong state =%d", rf_tdscdma_mc_state.rf_state[RF_PATH_0]);
  }
  
  tx_device = rf_tdscdma_mc_state.curr_tx_device;

  MSG_1(MSG_SSID_RF_DEBUG, MSG_LVL_HIGH, "rf_tdscdma_mc_get_curr_tx_device curr_tx_device =%d", tx_device);
...
/*!
  @brief
  This function is used to return Pri Rx device which is being used

  @param 

  @return
  current Pri Rx device
 
*/
rfm_device_enum_type rf_tdscdma_mc_get_curr_pri_rx_device(void)
{
  rfm_device_enum_type rx_device = RFM_INVALID_DEVICE;
  
  if ( (rf_tdscdma_mc_state.rf_state[RF_PATH_0] != RF_TDSCDMA_STATE_RX) && (rf_tdscdma_mc_state.rf_state[RF_PATH_0] != RF_TDSCDMA_STATE_RXTX) )
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "rf_tdscdma_mc_get_curr_pri_rx_device is called in the wrong state =%d", rf_tdscdma_mc_state.rf_state[RF_PATH_0]);
  }
  
  rx_device = rf_tdscdma_mc_state.curr_pri_device;

  MSG_1(MSG_SSID_RF_DEBUG, MSG_LVL_HIGH, "rf_tdscdma_mc_get_curr_pri_rx_device curr_pri_device =%d", rx_device);
...
/*!
  @brief
  This function will update the registers to be read
...
void rf_tdscdma_mc_set_device_status_registers(uint8 device, uint8 channel, uint8 slave_id, uint16 reg, uint8 extented_type, uint8 rx_tx_type)
{
  // KW Fix
  if (device >= RF_TDSCDMA_DEVICE_STATUS_READ_DEVICE_SUPPORT)
  {
    MSG_1(MSG_SSID_RF_DEBUG, MSG_LVL_HIGH, "Set device status registers. Bad argument: device: %d", device);
    return ;
  }
  
  if(rf_tdscdma_mc_state.device_status_read.reg_length[device] < RF_TDSCDMA_DEVICE_STATUS_READ_REG_LIST_LENGTH)
  {
   rf_tdscdma_mc_state.device_status_read.reg_list[device][rf_tdscdma_mc_state.device_status_read.reg_length[device]].channel = channel;
   rf_tdscdma_mc_state.device_status_read.reg_list[device][rf_tdscdma_mc_state.device_status_read.reg_length[device]].slave_id = slave_id;
   rf_tdscdma_mc_state.device_status_read.reg_list[device][rf_tdscdma_mc_state.device_status_read.reg_length[device]].reg     = reg;
   rf_tdscdma_mc_state.device_status_read.reg_list[device][rf_tdscdma_mc_state.device_status_read.reg_length[device]].extended_type = extented_type;
   rf_tdscdma_mc_state.device_status_read.reg_list[device][rf_tdscdma_mc_state.device_status_read.reg_length[device]].rx_tx_type = (rf_tdscdma_device_status_rx_tx_enum_type)rx_tx_type;
   rf_tdscdma_mc_state.device_status_read.reg_length[device]++;
  }
  else
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, 
           "rflte_state_set_device_status_registers, "
           "Number registers listed already is %d ",
            RF_TDSCDMA_DEVICE_STATUS_READ_REG_LIST_LENGTH);
  }
}
...
/*!
  @brief
  This function will enable/disable device status read
...
void rf_tdscdma_mc_set_device_status_enable(uint8 device, boolean enable)
{
  // KW Fix
  if (device >= RF_TDSCDMA_DEVICE_STATUS_READ_DEVICE_SUPPORT)
  {
    MSG_1(MSG_SSID_RF_DEBUG, MSG_LVL_HIGH, "Set device status enable. Bad argument: device: %d", device);
    return;
  }
  
   rf_tdscdma_mc_state.device_status_read.device_enabled[device] = enable;
   
   if(!enable)
   {
     rf_tdscdma_mc_state.device_status_read.reg_length[device] = 0;
	 MSG(MSG_SSID_RF, MSG_LEGACY_ERROR, 
           "rflte_state_set_device_status_enable, "
           "Number registers listed is reset to 0 ");
   }
}

/*
  @brief  
   API to Append device status read registers
...
*/
boolean rf_tdscdma_mc_update_device_status_registers(uint8 device, rf_buffer_intf* script, 
                                                                  rf_tdscdma_device_status_rx_tx_enum_type rx_tx_enum)
{
  uint8 count = 0;
  boolean ret_val = FALSE;
  uint8 total_reg_cnt = 0;
  
  // KW Fix
  if (device >= RF_TDSCDMA_DEVICE_STATUS_READ_DEVICE_SUPPORT)
  {
    MSG_1(MSG_SSID_RF_DEBUG, MSG_LVL_HIGH, "Update device status registers. Bad argument: device: %d", device);
    return FALSE;
  }
  
  if( (rf_tdscdma_mc_state.device_status_read.reg_length[device]) > 0)
  {
    for(count = 0; count < (rf_tdscdma_mc_state.device_status_read.reg_length[device]); count++)
    {
      if(rx_tx_enum == rf_tdscdma_mc_state.device_status_read.reg_list[device][count].rx_tx_type)
      {
        rf_buffer_append_rffe((rf_buffer_intf*)script,
        rf_tdscdma_mc_state.device_status_read.reg_list[device][count].slave_id,
        rf_tdscdma_mc_state.device_status_read.reg_list[device][count].channel,
        rf_tdscdma_mc_state.device_status_read.reg_list[device][count].reg,
        0,
        RF_BUFFER_READ,
        RF_TDSCDMA_DEVICE_STATUS_READ_DELAY,
        rf_tdscdma_mc_state.device_status_read.reg_list[device][count].extended_type, 
        0, 
        0, 
        TRUE);
  ...
  {
    RF_MSG_1(RF_HIGH,"rflte_mc_update_device_status_registers device register list is empty for device %d", device);
    ret_val = FALSE;
  }
...
/*!
  @brief
  callback function, to add debug variables list to minidump

  @details
  To add debug variables list to minidump
...
*/
QSH_MDUMP_FN_ATTR
void rftech_tdscdma_QSH_mdump_cb (void)
{
  #define SIZEOF_POINTER   4
  
  /* Need to give the global variable address and size 
  qsh_mdump_collect_high((void *)&rflm_ccs_crash_dump, sizeof(rflm_ccs_crash_dump));
  qsh_mdump_collect_high((void *)rflm_ccs_resource_to_dma_chan, CCS_RF_MAX_BUS_RESOURCES * sizeof(rflm_ccs_resource_to_dma_chan)); */
  //qsh_mdump_collect_high((void *)dummy_var, (10 * sizeof(uint8)));

  /* Check if TDS is active, check mc state for general, check DmMeasBuffPtr for I-RAT X2T */
  if((RF_TDSCDMA_STATE_DEFAULT == rf_tdscdma_mc_state.rf_state[RF_PATH_0]) ||
      (NULL == rflmTdsRfModeCtlState.rflmTdsDmMeasBuffPtr))
  {
//    MSG_2(MSG_SSID_RF, MSG_LVL_HIGH, "Do not dump TDSCDMA minidump, rf state[%d], DmMeasBuffPtr[%d]", 
//      rf_tdscdma_mc_state.rf_state[RF_PATH_0], rflmTdsRfModeCtlState.rflmTdsDmMeasBuffPtr);
    return;
  }
      
  /* 1, RF general */
  qsh_mdump_collect_high((void *)&rflmTdsRfModeCtlState, sizeof(rflmTdsRfModeCtlState));
  //  Strore heap memory for Mdump.
  qsh_mdump_collect_high((void *)(rflmTdsRfModeCtlState.rflmTdsDmTechBuffPtr), sizeof(rflm_tds_ccs_event_t));
  qsh_mdump_collect_high((void *)(rflmTdsRfModeCtlState.rflmTdsDmMeasBuffPtr), sizeof(rflm_tds_meas_ccs_event_t) * RFLM_TDS_RX_TX_MAX_NUM_INTER_FREQ_BUFFERS);

  qsh_mdump_collect_high((void *)&rflm_tds_txlm_handle_user_data, sizeof(rflm_tds_txlm_handle_user_data));
  //qsh_mdump_collect_high((void *)&rflm_tds_is_sawless_enabled, sizeof(rflm_tds_is_sawless_enabled));
  
  /* 2, MC general */
  qsh_mdump_collect_high((void *)&rf_tdscdma_mc_lm_bufs, sizeof(rf_tdscdma_mc_lm_bufs));
  qsh_mdump_collect_high((void *)&rf_tdscdma_mc_state, sizeof(rf_tdscdma_mc_state));
  qsh_mdump_collect_high((void *)&rf_tdscdma_rxfreq_comp, sizeof(rf_tdscdma_rxfreq_comp));
  qsh_mdump_collect_high((void *)&rf_tdscdma_core_current_txlin_data, sizeof(rf_tdscdma_core_current_txlin_data));
  qsh_mdump_collect_high((void *)&rf_tdscdma_mc_dbl_bufs, sizeof(rf_tdscdma_mc_dbl_bufs));
  qsh_mdump_collect_high(rfc_tdscdma_get_address(), SIZEOF_POINTER);

  /* 3, FTM specific */
  //qsh_mdump_collect_high((void *)&ftm_tdscdma_esc_ftm_cal_state, sizeof(ftm_tdscdma_esc_ftm_cal_state));
  qsh_mdump_collect_high((void *)&ftm_tdscdma_rftest_tgt_state, sizeof(ftm_tdscdma_rftest_tgt_state));
  qsh_mdump_collect_high((void *)&ftm_tdscdma_rftest_curr_state, sizeof(ftm_tdscdma_rftest_curr_state));
  qsh_mdump_collect_high((void *)&ftm_tdscdma_current_cal_state, sizeof(ftm_tdscdma_current_cal_state));
}
#endif
...
{
  if (rfcommon_get_gnss_blanking_is_enabled(RFM_TDSCDMA_MODE, band))
  {
    rflmTdsRfModeCtlState.GnssBlakingState.enableGnssBlakingBitMask |= 1 << txlm_buffer_idx;
    MSG_3(MSG_SSID_RF, MSG_LVL_HIGH, "GNSS Blanking Enabled, band %d, txlm_buffer_idx[%d], Bitmask 0x%x",
      band, txlm_buffer_idx, rflmTdsRfModeCtlState.GnssBlakingState.enableGnssBlakingBitMask);
  }
  else 
  {
    /* clear the bit for that txlm handle to indicate not to do GNSS blanking for that TxLM handle */
    rflmTdsRfModeCtlState.GnssBlakingState.enableGnssBlakingBitMask &= ~(1 << txlm_buffer_idx);
    MSG_3(MSG_SSID_RF, MSG_LVL_HIGH, "GNSS Blanking Disabled, band %d, txlm_buffer_idx[%d], Bitmask 0x%x",
      band, txlm_buffer_idx, rflmTdsRfModeCtlState.GnssBlakingState.enableGnssBlakingBitMask);
  }
}
