...
$Header: //components/rel/rftech_tdscdma.mpss/3.11/rf/core/src/rf_tdscdma_core_txlin.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/18/17   sk      Send Tx handle to XPT api's for LTE
04/07/16   nsh     Support AutoPin on TA.2.3 + WTR3925
02/27/16   whz     Add logging of reference apin in autopin log pkts
12/17/15   whz     Check if pa_index is out of range
...
05/31/15   jak     Use curr_tx_device in rf_tdscdma_core_txlin_temp_comp_update
05/27/15   jr      Fix EPT failure
...
03/26/15   nsh     Attach GRFC PA_ON to common TxAGC
...
#define RF_TDSCDMA_MAX_ALLOWED_ASM_TRANS 4
                                      ...
                                      lm_handle_type txlm_buffer_idx )
{
  ...
  int32 xpt_delay = 0;
  int16 i;
  rfcom_bw_type_u rfm_bw,cal_rfm_bw;

  rfm_bw.tdscdma_bw = RFCOM_BW_TDSCDMA_1P4MHz;
  cal_rfm_bw.tdscdma_bw = RFCOM_BW_TDSCDMA_1P4MHz;
  ...
  /* Memset the APT Char table */
  memset(&curr_pa_state_apt_char_tbl,0,sizeof(rfnv_tx_static_data_type));
  ...
  rf_tdscdma_core_txlin_get_tx_device_info(rf_tdscdma_mc_state.curr_tx_device, band, &device_info);
  ...
  **********************************************************************/
  ...
  search_param.rfm_device = rf_tdscdma_mc_state.curr_tx_device;
  ...
  for (pa_index = 0; pa_index < rf_tdscdma_core_current_txlin_data.num_valid_pa_state; pa_index++)
  {
    ...
    status_flag = rfcommon_core_txlin_temp_vs_freq_comp_update(
                                                ...
                                                FALSE /* Dont skip Pout Comp when set up a chan */);
            ...
            (rf_tdscdma_core_current_txlin_data.multi_lin_rev != TX_MULTI_LIN_REV_1) )
    {
          ...
          (rf_tdscdma_core_current_txlin_data.current_aux_data[pa_index] == TX_LINEARIZER_ET_DPD_DATA) )
      {
        ...
        /* load dpd */
        status_flag = rfcommon_core_populate_tx_dpd(rf_tdscdma_mc_state.curr_tx_device,
                                                    NULL,
                                                    &txlin_final_table,
    ...
    status_flag = rfcommon_core_txlin_config_init_linearizer(RFM_TDSCDMA_MODE, 
                                                             rf_tdscdma_mc_state.curr_tx_device,
                                                             rfm_band,
  ...
  status_flag = rf_tdscdma_core_txlin_config_txagc(rf_tdscdma_mc_state.curr_tx_device,
                                                   band,
                                                   txlm_buffer_idx);
  if (!status_flag)
  {
  ...
  /* config et delay value to rflm */
  if(rf_tdscdma_core_xpt_is_et_enabled(band) && 
          (rf_tdscdma_core_current_txlin_data.multi_lin_rev != TX_MULTI_LIN_REV_1))
  {
    xpt_delay = rf_tdscdma_core_xpt_path_delay_comp(band,rf_chan_tx,FALSE,0);
    if ( !rflm_tds_mc_config_et_delay(txlm_buffer_idx, xpt_delay) )
    {
      RF_MSG( RF_ERROR, "rf_tdscdma_core_txlin_update: rflm_tds_mc_config_tx_et_delay failed!" );
    }
  } 
          ...
          (rf_tdscdma_core_current_txlin_data.multi_lin_rev != TX_MULTI_LIN_REV_1))
  {
    ...
    rf_common_xpt_commit_dpd_data(rf_tdscdma_mc_state.curr_tx_device, NULL);
  }
...
)
{
  ...
  boolean api_status = TRUE;
  rf_buffer_intf* rftdscdma_core_txagc_script_buffer_ptr =NULL;    
  ...
  uint8 asm_rffe_channel_num =0xFF;
  uint8 pa_rffe_channel_num = 0xFF;
  rf_buffer_intf* rf_cmn_txagc_asm_scpt_ptr = NULL;
  rf_buffer_intf* temporary_scpt_ptr = NULL;
  ...
  rf_cmn_txagc_asm_scpt_ptr = rf_buffer_create(0, 
                                               RFTDSCDMA_STANDALONE_MAX_RFFE_SCRIPT_SIZE,
                                               RFTDSCDMA_STANDALONE_MAX_GRFC_SCRIPT_SIZE);
 
  temporary_scpt_ptr = rf_buffer_create(0, 
                                        RFTDSCDMA_STANDALONE_MAX_RFFE_SCRIPT_SIZE,
                                        RFTDSCDMA_STANDALONE_MAX_GRFC_SCRIPT_SIZE);
  ...
  {    
    ...
    (void)rf_buffer_clear(rf_cmn_txagc_asm_scpt_ptr);
    (void)rf_buffer_clear(temporary_scpt_ptr);
    ...
    if (rfdevice_tdscdma_use_asm(tx_device,  band))
    {
      ...
      if(tx_band_config_ptr)
      {
        ...
        // Avoid to append ASM contents to append buffer in case of ASM & PA are on same RFFE bus
        if(!rfc_tdscdma_grfc_pa_existed())
        {
          // get RFFE bus on PA
          if ( device_info.pa_device != NULL )
          {
            ...
            temp_rffe_pa_buffer = rf_buffer_create(0, 
                                             RFTDSCDMA_STANDALONE_MAX_RFFE_SCRIPT_SIZE,
                                             RFTDSCDMA_STANDALONE_MAX_GRFC_SCRIPT_SIZE);


            rfdevice_set_pa_txagc( &pa_params_s, &pa_txagc_s , device_info.pa_device, 
                                   temp_rffe_pa_buffer,
                                   RFDEVICE_CREATE_SCRIPT, 
                                   RFCOM_INVALID_TIMING_OFFSET );
	  
            resource_script = rf_buffer_get_script_ptr(temp_rffe_pa_buffer, RF_HAL_BUS_RESOURCE_RFFE);   

            if (resource_script != NULL)
            {
              for (loop = 0; loop < resource_script->num_trans; loop++)
              {
                pa_rffe_channel_num = resource_script->script_ptr.rffe[loop].channel; //all trasactions channel number should be same
              }        
            }
            (void)rf_buffer_destroy(temp_rffe_pa_buffer);
          }
        }		
        ...
        else
        {
          ...
          resource_script = rf_buffer_get_script_ptr(temporary_scpt_ptr, RF_HAL_BUS_RESOURCE_RFFE);            

          if (resource_script != NULL)
          {
            /*add asm to append-tx buffer if trans limit - > 4 */
            if(resource_script->num_trans > RF_TDSCDMA_MAX_ALLOWED_ASM_TRANS)
            {
              for (loop = 0; loop < resource_script->num_trans; loop++)
              {
                MSG_6(MSG_SSID_RF, MSG_LVL_MED, 
                      "txAGC ASM Debug: loop %d, Addr 0%x, Data 0x%x, Start_delta = %d, slave_id =%d, channel=%d", 
                      loop, 
                      ...
                      resource_script->script_ptr.rffe[loop].slave_id,
                      resource_script->script_ptr.rffe[loop].channel);
				
                rf_buffer_append_rffe(rftdscdma_core_txagc_script_buffer_ptr,
                            resource_script->script_ptr.rffe[loop].slave_id,
                            resource_script->script_ptr.rffe[loop].channel,
                            resource_script->script_ptr.rffe[loop].addr,
                            resource_script->script_ptr.rffe[loop].data[0],
                            (resource_script->script_ptr.rffe[loop].rd_wr == RF_HAL_BUS_WRITE) ? RF_BUFFER_WRITE : RF_BUFFER_READ,
                            resource_script->script_ptr.rffe[loop].start_delta, 
                            resource_script->script_ptr.rffe[loop].extended_cmd, 
                            resource_script->script_ptr.rffe[loop].half_rate, 
                            resource_script->script_ptr.rffe[loop].rd_delay, 
                            TRUE);
              }
              /*ASM TXAGC is NULL-- All writes will goto append buffer*/
              if(rf_cmn_txagc_asm_scpt_ptr != NULL)
              {
                (void)rf_buffer_destroy(rf_cmn_txagc_asm_scpt_ptr);
                rf_cmn_txagc_asm_scpt_ptr = NULL;
              }
            }
            else
            {
              MSG_1(MSG_SSID_RF, MSG_LVL_MED, "pa_rffe_channel_num = %d", pa_rffe_channel_num);
            for (loop = 0; loop < resource_script->num_trans; loop++)
            {
	            MSG_6(MSG_SSID_RF, MSG_LVL_MED, 
		            "txAGC ASM Debug: loop %d, Addr 0%x, Data 0x%x, Start_delta = %d, slave_id =%d, channel=%d", 
                ...
                asm_rffe_channel_num = resource_script->script_ptr.rffe[loop].channel; //all trasactions channel number should be same

                if(asm_rffe_channel_num == pa_rffe_channel_num)
                {
                  rf_buffer_append_rffe(rf_cmn_txagc_asm_scpt_ptr,
                              resource_script->script_ptr.rffe[loop].slave_id,
                              resource_script->script_ptr.rffe[loop].channel,
                              resource_script->script_ptr.rffe[loop].addr,
                              resource_script->script_ptr.rffe[loop].data[0],
          ...
          /*distory ASM buffer, if no RFFE trans */
          if(rf_cmn_txagc_asm_scpt_ptr != NULL)
          { 
            resource_script = rf_buffer_get_script_ptr(temporary_scpt_ptr, RF_HAL_BUS_RESOURCE_RFFE);
            if((resource_script == NULL)|| (resource_script->num_trans ==0))
            {
              (void)rf_buffer_destroy(rf_cmn_txagc_asm_scpt_ptr);
              rf_cmn_txagc_asm_scpt_ptr = NULL;
            }        
          ...
          /* distry temporary_script_ptr */
          if(temporary_scpt_ptr != NULL)
          {
            (void)rf_buffer_destroy(temporary_scpt_ptr);
            temporary_scpt_ptr = NULL;
          }
    ...
    if (rfc_tdscdma_grfc_pa_existed())
    {
      ...
      if (!pa_status)
      {
        MSG_2(MSG_SSID_RF, MSG_LVL_ERROR, "PA configuration ERROR on Device %d and Band %d", tx_device, band);     
      }
      else
      {
        // Use this script buf only when ASM presented and configured correctly 
  ...
  api_status &= rfcommon_core_txlin_config_agc(RFM_TDSCDMA_MODE,
                                               ...
                                               RFCOM_TXLIN_TXAGC_TX_TO_TX,                       //uint16 script_buffer_id, use default buffer 
                                               ...
                                               NULL,                    // rf_buffer_intf* tech_append_tx_agc, 
                                               &timing_params);
  ...
  api_status &= rfcommon_core_txlin_config_agc(RFM_TDSCDMA_MODE,
                                                   ...
                                                   rfm_band,
                                                   txlm_buffer_index, 	  //int32 tx_handle_id, 
                                                   RFCOM_TXLIN_TXAGC_OFF_TO_TX,					   //uint16 script_buffer_id, use default buffer 
                                                   device_info.wtr_device,  // void* wtr_device,
                                                   device_info.pa_device,   // void* pa_device,
                                                   device_info.papm_device, // void* papm_device,
                                                   rf_cmn_txagc_asm_scpt_ptr,  // rf_buffer_intf* tech_asm_tx_agc,
...
)
{
  ...
  /* Power cap/boost algorithm */
  rfcommon_core_txlin_cap_boost_type cap_boost_data;
  boolean skip_pout_temp_comp = TRUE;
  ...
  /* only skip pout temp comp when there is FBRX */
#if defined(FEATURE_TABASCO_MODEM) && defined(FEATURE_RF_HAS_WTR2965) 
  skip_pout_temp_comp = FALSE;
#endif
  ...
  for ( pa_index = 0; pa_index < rf_tdscdma_core_current_txlin_data.num_valid_pa_state; pa_index++)
  {
    ...
    status_flag = rfcommon_core_txlin_temp_vs_freq_comp_update(
                                                   ...
                                                   pout_comp_val[pa_index],
                                                   rf_tdscdma_core_current_txlin_data.txlin_valid_entries[pa_index] ,
                                                   skip_pout_temp_comp /* skip Pout temp comp */ );
