...
$Header: //components/rel/rftech_tdscdma.mpss/3.11/rf/core/src/rf_tdscdma_core_temp_comp.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/06/16   wxl     Check autopin flag in DM for temp comp
02/27/16   jr      Appy pin/pout through temp comp update when WTR autopin support absent
...
05/18/15   jak     Use current Tx device for rf_tdscdma_core_temp_comp_timer()
...
{
  /* un-define the timer */
  timer_undef( &rf_tdscdma_core_temp_comp_clk_cb );
}
...
void rf_tdscdma_core_temp_comp_start(void)
{
  ...
  */
//TODO - to consider how to do in TA  
...
void rf_tdscdma_core_temp_comp_stop(void)
{
  ...
  if ( rf_tdscdma_core_temp_comp_started )
  {
    ...
    rf_tdscdma_mdsp_config_therm(FALSE, 2000);
  }
#else
...
{
  /* reuse below api, just to send RFLM_TDSCDMA_TRIGGER_THERM_CMD msg to rf task, 
     such that temp comp processing is done in rf task,  not in timer task */
...
void rf_tdscdma_core_temp_comp_timer_process(void)
{
 ...
 if (rf_tdscdma_mc_state.curr_pri_device != RFM_INVALID_DEVICE)
 {
   ...
   device_ptr = (rfdevice_trx_tdscdma_tx**) rfc_tdscdma_get_rf_device_obj(rf_tdscdma_mc_state.curr_tx_device, 
                                                                          RFC_CONFIG_TX, 
   ...
   if (cmn_device_ptr != NULL)
   {
     ...
     if (read_result)
     {
       MSG_2(MSG_SSID_RF, MSG_LEGACY_HIGH, "Therm_read: %d, device: %d", therm_read, rf_tdscdma_mc_state.curr_tx_device);
     ...
     {
       MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "Failed on getting therm_read for device: %d", rf_tdscdma_mc_state.curr_tx_device);
     }
   }
   else
   {
     MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "Can't get cmn_device_ptr for therm_read. device: %d", rf_tdscdma_mc_state.curr_tx_device);
   }   
 }
                                            ...
                                            uint16 tx_chan)
{
  ...
  int32 xpt_delay = 0;
  rfcom_tdscdma_band_type band = rf_tdscdma_core_util_get_band_from_uarfcn(tx_chan);
  int16 i;
  ...
  if ( (!rfcommon_autopin_is_enabled_flag_check(rf_tdscdma_mc_lm_bufs.tx_buf_wrk.buf_idx)) || (!rflm_autopin_sanity_check(rf_tdscdma_mc_lm_bufs.tx_buf_wrk.buf_idx)) || (!rflmTdsRfModeCtlState.autopinInfo.enabled) )
  {
  ...
  if( rf_tdscdma_core_temp_comp_is_enabled() )
  {
    ...
    {  
      fbrx_temp_comp_result = rfcommon_fbrx_mc_update_temp_comp(rf_tdscdma_mc_lm_bufs.tx_buf_wrk.buf_idx,
                                                                temperature,
                                                                rf_tdscdma_core_temp_comp_get_ref_temp_scaled(tx_chan),
                                                                tx_therm_bins,
  ...
  /* config et delay value to rflm, on temp comp */
  if(rf_tdscdma_core_xpt_is_et_enabled(band))
  {
    xpt_delay = rf_tdscdma_core_xpt_path_delay_comp(band, tx_chan, FALSE, 0);
    if ( !rflm_tds_mc_config_et_delay(rf_tdscdma_mc_lm_bufs.tx_buf_wrk.buf_idx, xpt_delay) )
    {
      RF_MSG( RF_ERROR, "temp comp: rflm_tds_mc_config_tx_et_delay failed!" );
    }
  } 
                                      ...
                                      int16 pout_comp_offset[] )
{
        ...
        pa_state++ )
  {
    ...
    temp_vs_freq_params.auto_pin_enable = rfcommon_autopin_is_enabled_flag_check(rf_tdscdma_mc_lm_bufs.tx_buf_wrk.buf_idx) && rflm_autopin_sanity_check(rf_tdscdma_mc_lm_bufs.tx_buf_wrk.buf_idx) && rflmTdsRfModeCtlState.autopinInfo.enabled ;
...
/*!
  ...
  @details
  This function returns scaled CAL ref temp value(0-255 range), computed from
  the Ref cal temperature ADC value stored in the TX multi-lin NV for the band
...
uint16 rf_tdscdma_core_temp_comp_get_ref_temp_scaled(uint16 tx_chan)
{
  ...
  {
    MSG_1( MSG_SSID_RF, MSG_LEGACY_ERROR, 
           "rf_tdscdma_core_temp_comp_get_ref_temp_bin_idx:"
           "Invalid TDSCDMA Tx Lin data", 0);
    return temperature;
  }

  /* scale raw to 255 range, don't save the raw */
  temperature = rf_tdscdma_core_temp_comp_scale_therm(tx_multilin_nv_cal_ptr->ref_lin_temp_adc, FALSE);
...
/*!
  ...
  @details
  This function does the fbrx temp comp once when tx changes to a new channel,
  using the last saved therm value.
...
void rf_tdscdma_core_temp_comp_fbrx_once(uint16 tx_chan)
{
  ...
  /* get the last saved scaled temp value */
  temperature = rf_tdscdma_core_temp_comp_value.last_scaled_temp_value;
