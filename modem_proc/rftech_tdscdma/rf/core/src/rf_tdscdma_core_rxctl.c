...
$Header: //components/rel/rftech_tdscdma.mpss/3.11/rf/core/src/rf_tdscdma_core_rxctl.c#1 $ 
...
/* RFLM_TDSCDMA_MAX_LNA_GAIN_STATES_4 == RF_TDSCDMA_MAX_LNA_GAIN_STATES*/
...
)
{
...
#ifdef FEATURE_RF_TDS_ASDIV_ELNA_GAIN
  uint8 asdiv_sw_position = 0;
  /*count of gain offset*/
  uint8 num_of_elna_gain_offset = 0 ; //rx_nv_tbl_ptr->rx_elna_gain_offset_swpt.gain_offset_cnt;
#endif
  ...
  num_of_elna_gain_offset = rx_nv_tbl_ptr->rx_elna_gain_offset_swpt.gain_offset_cnt;
...
#ifdef FEATURE_RF_TDS_ASDIV_ELNA_GAIN
  /*We don't update offset here to rx_gain_vs_freq as the function is called only when Rx chain is configed. 
    Update the offset at the last step to FW
    Assuming that the offset is for FW to use only and we don't use common RxAGC*/

  /*Adding AsDiv eLNA gain offset, no need to not consider LL and HL*/
  /* for each gain state:
          a. switch position = 1 is assumed;
          b. LL is assumed, make HL == LL;
  */

     asdiv_sw_position = 1; /*Hardcoded to 1*/
     if (nv_container_idx >= 2*RF_TDSCDMA_PATHS_TABLE_SIZ)
     {
        ERR_FATAL("Invalid nv_container_index=%d. Cannot initialize TDSCDMA Rx offset table", nv_container_idx, 0, 0);
     }
     for(i=0; i<RF_TDSCDMA_MAX_LNA_GAIN_STATES; i++)
     {
       /*Offset in q8 format*/
       /*LL/HL q8 format*/
       rflmTds_elna_gain_offset[rx_chain][i][0] = 
             ((rx_nv_tbl_ptr->rx_elna_gain_offset_swpt.elna_rx_gain_offset[nv_container_idx][asdiv_sw_position].gain_offset[i])<<8)/10;
       rflmTds_elna_gain_offset[rx_chain][i][1] = rflmTds_elna_gain_offset[rx_chain][i][0];
       /*local copy before format conversion, may not needed*/
       rf_tdscdma_rxfreq_comp.rx_elna_asdiv_offset[rx_chain][i][1]
         = rx_nv_tbl_ptr->rx_elna_gain_offset_swpt.elna_rx_gain_offset[nv_container_idx][asdiv_sw_position].gain_offset[i];
       rf_tdscdma_rxfreq_comp.rx_elna_asdiv_offset[rx_chain][i][1] = rf_tdscdma_rxfreq_comp.rx_elna_asdiv_offset[rx_chain][i][0];
     }
#endif
