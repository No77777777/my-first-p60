...
$Header: //components/rel/rftech_tdscdma.mpss/3.11/rf/core/src/rf_tdscdma_core_util.c#1 $

When       who     what, where, why
-------------------------------------------------------------------------------
01/07/16   jr      Reduce F3 messages
08/18/15   avi     Moved #define/Constants to rf_tdscdma_core_util.h
07/17/15   avi     Moved device util functions to separate file
...
/*!
  ...
  @return
  Prx or Drx operation rfdevice_rx_mode_type
*/
rfdevice_rx_mode_type rf_tdscdma_core_util_rx_mode_type
(
  rfm_device_enum_type device
)
{
  rfm_device_capability_type device_capability;
  rfdevice_rx_mode_type device_type = RFDEVICE_INVALID_OPER_MODE;

  device_capability = rfc_common_get_device_capability( device );

  if( device_capability == RFM_DEVICE_RX_SUPPORTED )
  {
    ...
    {
      device_type = RFDEVICE_PRX_OPER_MODE;
    }

    if ( device == RFM_DEVICE_1 || device == RFM_DEVICE_3 )
    {
      device_type = RFDEVICE_DRX_OPER_MODE;
    }
  }
  else 
  {
    RF_MSG_2(RF_HIGH,"rf_tdscdma_core_util_rx_mode_type: invalid rfm device capability[%d] for device %d in rx mode type",device_capability, device);    
  }

  RF_MSG_3(RF_HIGH,"rf_tdscdma_core_util_rx_mode_type: device = %d , capability = %d , device_type = %d",device, device_capability , device_type );	
