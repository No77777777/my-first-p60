...
$Header: //components/rel/rftech_tdscdma.mpss/3.11/api/rfc_tdscdma.h#1 $
...
07/11/15   jak     Public extend from rfa class to use RFA new/delete operator
...
public:
  ...
  static rfc_tdscdma ** get_instance_address(void);
  ...
  virtual uint32 get_fb_chain(rfm_device_enum_type tx_device, rfcom_tdscdma_band_type tx_band);
  ...
  virtual boolean generate_asdiv_xsw_sleep_script
  ( 
    uint32 position,
    rf_buffer_intf *asdiv_script_ptr,
    void *cb_data
