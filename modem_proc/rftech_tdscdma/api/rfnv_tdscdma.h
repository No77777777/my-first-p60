...
$Header: //components/rel/rftech_tdscdma.mpss/3.11/api/rfnv_tdscdma.h#1 $
...
#define RF_TDSCDMA_XPT_PATH_DELAY_ARRAY_SIZE 16

/*For eLNA switch point and gain offset*/
#define RF_TDSCDMA_ELNA_NUM_SWITCH_POINT 2
...
typedef PACK(struct)
{
  ...
  uint16 upper_bound_tx_chan[RF_TDSCDMA_XPT_PATH_DELAY_ARRAY_SIZE];
  int32  delay_value[RF_TDSCDMA_XPT_PATH_DELAY_ARRAY_SIZE];
} rf_tdscdma_nv_xpt_path_delay_type;

/*!
 @brief 
 eLNA Static Gain offset and swpt offset
 @details
 No need to be PACK struct and for internal use only
*/
typedef struct
{
  uint8  rx_num_gain_states;
  /*total elements for both gain offset and swpt*/
  uint8  num_of_elements;   
  uint8  gain_offset_cnt;
  uint8  swpt_offset_cnt;
  /*Not consider LL/HL here until confirmed
    put switch point here as in NV header file it is defined
    2*RF_TDSCDMA_PATHS_TABLE_SIZ for DR case
  */
  rfnv_asdiv_rx_gain_offset_type elna_rx_gain_offset[2*RF_TDSCDMA_PATHS_TABLE_SIZ][RF_TDSCDMA_ELNA_NUM_SWITCH_POINT];
  rfnv_asdiv_rx_switch_points_type elna_swpt_offset[2*RF_TDSCDMA_PATHS_TABLE_SIZ][RF_TDSCDMA_ELNA_NUM_SWITCH_POINT];
} rf_tdscdma_elna_static_rx_data_type;


/*!
 @brief 
 eLNA Static TX offset
 @details
 No need to be PACK struct and for internal use only
*/
typedef struct
{
  uint8  num_of_elements;   
  /*reserved more space and may be reduce later*/
  rfnv_asdiv_tx_gain_offset_type elna_tx_gain_offset[RF_TDSCDMA_PATHS_TABLE_SIZ][RF_TDSCDMA_ELNA_NUM_SWITCH_POINT]; 
} rf_tdscdma_elna_static_tx_data_type;
...
{  
  ...
  /*TDS RX Static NV for eLNA gain offset and swpt offset*/
  rf_tdscdma_elna_static_rx_data_type rx_elna_gain_offset_swpt; 
...
typedef struct
{
  ...
  rf_tdscdma_nv_xpt_path_delay_type xpt_path_delay;

  /* XPT path delay offset vs temp vs freq */
  rfnv_tx_delay_offsets_type tx_delay_offsets;
  /*elna asdiv tx offset for all PA state */
  rf_tdscdma_elna_static_tx_data_type tx_elna_offset;
