...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/1x/rf/src/rfm_1x.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
07/14/18   kk      Chnages to add Condition check to ignore invalid EcIo values
06/20/18   am      Fix bug in maat debug msg
05/15/18   am      Changes to support MAAT
05/03/16   rs      Update tuner scripts for Div when updating for Tx
03/14/16   pk      Changed the critical section locking mechanism for 1x2G QTA.
...
08/23/15   sty     call rf_cdma_mc_update_asm_state() only in TX state in 
                   rfm_1x_antenna_tuner_action_time()
08/20/15   hm      asm grfc changes
08/14/15   wwl     Add function to query max TX power via given antenna position
07/22/15   zhw     Update wakeup_tx_v2 ret_val correctly in failed cases
06/15/15   vc      Reduce DIAG logging volume from C2K
06/10/15   ndb     Cleanup the unused enums
05/25/15   ndb     Move to the new tuner interface
05/21/15   zhw     Skip checking curr_mode in rfm_1x_is_sleeping() for L->1x QTA
05/11/15   zhw     Disable Drx support during L->1x QTA
05/04/15   zhw     Fix Tx STM check in rfm APIs
04/28/15   zhw     Reworked Tuner API
04/26/15   zhw     Add rfm_1x_get_btf_v2() API to take both Tx & Rx devices
04/16/15   zhw     Set traffic state in sleep_tx explicitly
04/09/15   zhw     Fix tuner AOL Tx state check
04/02/15   zhw     Skip checking Tx device status in retune
03/30/15   zhw     Move enter_mode in the callflow. Touch up debug messages
03/30/15   zhw     MC changes to support seperate Tx device tune
                   - Implement retune_v2() API that takes Tx devices
                   - Support new CDMA state RF_CDMA_STATE_TX in MC code
03/23/15   wwl     update wakeup tx v2 function to have PRX device
03/16/15   zhw     Support Prx+Drx tune in 1x iRAT/MEAS tune-away call flow
03/04/15   wwl     Add new API functions to split RXTX
03/16/15   pk      Compiler warning Fix
01/21/15   wwl     Remove redundent code from rfm_1x_antenna_tuner_action_time()
                   The removed code would never be executed
12/01/14   as/spa  Add mutex protection to ASDIV operations
                   Skip update tune code if already under progress
11/26/14   wwl     Use new atuner api function to decide whether enable antenna
                   tuner action time to 1x RF SW
...
)
{
  ...
  {
    RF_MSG_DBG_5( RF_LOW, "rfm_1x_validate_state : Curr(Dev %d, Mode %d, "
              "State %d), Expected(Mode %d, State %d)", device, curr_mode,
              dev_status_r->data_1x.rf_state, RFM_1X_MODE, exp_state );
  }
...
)
{
  ...
  if ( are_tasks_running == TRUE )
  {
    ...
    if ( device_state == RFM_CDMA_HEALTHY_STATE )
    {
      ...
      ret_val = rf_1x_mc_wakeup_tx( device, device, txlm_handle );
...
/*!
  ...
  @details
  Moves the 1x radio state into the Tx state. Powers on all
  circuits required to transmit a 1x signal, and insures the transmitter
  is tuned to the correct band and channel.
  ...
  @PARAM band_chan
  TX band and channel that the device tunes to

  @return
  The anticipated time in microseconds remaining from the rfm_1x_wakeup_tx_v2()
  function return until the operation is complete.
*/
...
)
{
  ...
  RFM_DEBUG_EN_Q(&rfm_1x_wakeup_tx_v2, rx_device, txlm_handle, &callback, &userdata);

  /* Throw F3 Message */
  RF_MSG_2( RF_HIGH, "rfm_1x_wakeup_tx_v2 : START - Rx Dev %d, Tx Dev %d.", rx_device, tx_device );

  /* Query if all required tasks are running or not */
  are_tasks_running &= rf_cdma_required_tasks_running(rx_device);

  /* Perform RF action, only if all required tasks are running */
  if ( are_tasks_running == TRUE )
  {
    /* Validate the Currente State */
    device_state = rfm_1x_validate_state( rx_device, RF_CDMA_STATE_RX );

    /* Proceed only if State is valid, i.e. ret_val is non-negative  */
    if ( device_state == RFM_CDMA_HEALTHY_STATE )
    {
      /* If Tx is waken up on a different device than Prx device,
         enter 1x mode on Tx device */
      if ( tx_device != rx_device )
      {
        ret_val = rfm_enter_mode(tx_device, RFM_1X_MODE, NULL, NULL, 0);
      }

      /* Call 1x Main Control to Prepare Tx for Wakeup */
      ret_val = rf_1x_mc_wakeup_tx( tx_device, rx_device, txlm_handle );     
  ...
  /* Report wakeup time, Tx AGC and PA state */
  RF_MSG_4( RF_HIGH, "rfm_1x_wakeup_tx_v2 : [Status %d] - Rx Dev %d, Tx Dev %d, "
                     "TxWakeupTime %d ", ret_val, rx_device,
            tx_device, wakeup_time_val );

  if (ret_val != RFM_CDMA_HEALTHY_STATE)
  {
    RF_MSG_2( RF_ERROR, "rfm_1x_wakeup_tx_v2: API FAILED. Rx Dev %d, Tx Dev %d",
              rx_device, tx_device );
  }
...
)
{
  rfm_wait_time_t ret_val = RFM_CDMA_ERROR_FAILURE; /* Return value */
  rfm_1x_validate_t device_state; /* return value of device_state check */
  rfm_1x_validate_t device_state_1; /* return value of device_state check */
  ...
  if ( are_tasks_running == TRUE )
  {
    ...
    device_state_1 = rfm_1x_validate_state( device, RF_CDMA_STATE_TX );

    /* Proceed only if State is valid, i.e. ret_val is non-negative  */
    if ( device_state == RFM_CDMA_HEALTHY_STATE ||
         device_state_1 == RFM_CDMA_HEALTHY_STATE )
    {
  ...
  if ( rfm_cdma_is_band_chan_supported( device, band_chan ) == TRUE )
  {
    ...
    if ( are_tasks_running == TRUE )
    {
      ...
      if ( ( device_state_1 == RFM_CDMA_HEALTHY_STATE ) ||
           ( device_state_2 == RFM_CDMA_HEALTHY_STATE ) )
      {
        ret_val = rf_1x_mc_retune( device, device, rxlm_handle, band, chan,
                                      FALSE /*Not in IRAT mode */);
...
/*!
  ...
  @param device
  The RX device radio path to re-tune.

  @param tx_device
  The TX device radio path to re-tune.
...
)
{
  ...
  rfm_1x_validate_t device_state_3 = RFM_CDMA_HEALTHY_STATE; /* return value of
                                                         device_state check */
  rfm_1x_validate_t device_state_4 = RFM_CDMA_HEALTHY_STATE; /* return value of
                                                         device_state check */
  ...
  RFM_DEBUG_EN_Q(&rfm_1x_retune_v2, rx_device, tx_device, band_chan.band , band_chan.chan_num ) ;
  ...
  /* Check band and channel validity. If fails, set ret_val to error code and */
  /* jump straight to call back. If true, proceed.                            */
  if ( rfm_cdma_is_band_chan_supported( rx_device, band_chan ) == TRUE )
  {
    /* Acquire Critical Section Lock */
    rf_common_enter_critical_section(rf_cdma_get_lock_data());
    ...
    if ( are_tasks_running == TRUE )
    {
      /* Validate the Current State. Retune function can be called, only if RF
      is in Rx or RxTx state. */
      device_state_1 = rfm_1x_validate_state( rx_device, RF_CDMA_STATE_RX );
      device_state_2 = rfm_1x_validate_state( rx_device, RF_CDMA_STATE_RXTX );

      device_state_3 = rfm_1x_validate_state( tx_device, RF_CDMA_STATE_TX );
      device_state_4 = rfm_1x_validate_state( tx_device, RF_CDMA_STATE_RXTX );

      /* Proceed only if State is valid, i.e. ret_val is non-negative  */
      /* Skip checking Tx device status in retune. Due to undefined behavior */
      if (   ( device_state_1 == RFM_CDMA_HEALTHY_STATE ) ||
             ( device_state_2 == RFM_CDMA_HEALTHY_STATE )  )
      {
        ret_val = rf_1x_mc_retune( rx_device, tx_device,
                                   rxlm_handle, band, chan,
                                   FALSE /*Not in IRAT mode */);
  ...
  /* Report retune time and final Rx AGC and LNA state */
  RF_MSG_5( RF_HIGH, "rfm_1x_retune : [Status %d] - Dev %d, Band %d, "
            "Chan %d, RetuneTime %d ", ret_val, rx_device,
            band, chan,
            retune_time_val  );

  if (ret_val != RFM_CDMA_HEALTHY_STATE)
  {
    RF_MSG_3( RF_ERROR, "rfm_1x_retune_v2: API FAILED. Dev %d, band %d, chan %d",
              rx_device, band_chan.band, band_chan.chan_num );
  }
    ...
    */
    if ( ret_val == RFM_CDMA_HEALTHY_STATE )
    {
      ...
      ret_val = rf_1x_mc_enable_diversity( master_dev, rxlm_handle_master_dev,
                                           slave_dev, rxlm_handle_slave_dev,
                                           FALSE, RFDEVICE_MEAS_INVALID_SCRIPT );
 ...
 */
  ret_val = rf_cdma_mc_get_btf_delay( RFM_1X_MODE, device, device,
                                      dev_status_r,
                                      logical_dev, &btf_cx8 );
...
)
{
  ...
  /* Get the device Status for validation */
  dev_status_r = rf_cdma_get_device_status( tx_device );
  ...
  4) Analog Tx delay term (DAC output through antenna)
 */
  ret_val = rf_cdma_mc_get_btf_delay( RFM_1X_MODE, tx_device, rx_device,
                                      dev_status_r,
                                      logical_dev, &btf_cx8 );
  ...
  RF_MSG_3(RF_LOW, "rfm_1x_get_btf : BTF %d, Tx Dev %d, Rx Dev %d",
           btf_cx8, tx_device, rx_device);
...
)
{
  ...
  {
    RF_MSG(RF_HIGH, "rfm_1x_antenna_tuner_action_time: [ASDIV]Switch in "
                 "progress skip update");
  ...
  /* Prevent AOL event creation when it is not needed
     AOL events are created and destroyed even though they are not needed/ used.
     We need this clean up to stop excessive AOL messages.

     The correct solution would be for the tech L1s to not automatically start
     the 200ms tick timer and instead to request RFSW to find out if the timer
     should fire.
     Till we get L1 support to clean up, the proposed solution above is the
     cleanest RFSW only fix. Code change proposed will ignore the tick at the
     dispatch level and thus prevent un-necessary overhead.
  */
  if ( !rfcommon_atuner_is_update_required())
  {
    update_required = FALSE;
  }
    ...
    {    
               ...
               m_dev_r->data_1x.rf_state == RF_CDMA_STATE_TX )
      {
        ...
        if ( m_logical_dev->ant_tuner[band] != NULL )
        {
          ...
          tuner_cb_data = rfcommon_tuner_event_create_cb_data(tuner_event_buf,
                                                          RFM_1X_MODE,
                                                          m_dev_r->txlm_handle);
          if( tuner_cb_data != NULL )
          {
            ...
            /* Get the associated master device for Tx and find associated Drx 
               to update the tuner scripts */
            rfm_device_enum_type assoc_master_dev = rf_cdma_data_get_assoc_master_dev(device);

            /* Get Device Status */
            const rf_cdma_data_status_type* master_dev_r = 
              rf_cdma_get_device_status( assoc_master_dev );
            	   
			/* 5(2). Also Update Tune Code for Diversity Device, if applicable*/
			if( (api_status == TRUE) && (master_dev_r != NULL) && 
                (master_dev_r->assoc_dev != RFM_INVALID_DEVICE) )
			{
			  const rfc_cdma_logical_device_params_type *s_logical_dev;
		
			  s_logical_dev = rfc_cdma_get_logical_device_params( master_dev_r->assoc_dev );
		
			  if( s_logical_dev != NULL )
			  {
			...
			if( api_status == TRUE )
			{
			  *tuner_event_handle_ptr = NULL;
			  api_status &= rfcommon_mdsp_tuner_open_loop_queue_script( 
				                      tuner_event_buf,
			...
			{
              boolean asm_update_ret;

			  /* Call API to update GRFC ASM state */
              asm_update_ret = rf_cdma_mc_update_asm_state( device,
                                                            m_dev_r,
                                                            m_logical_dev );
              
              /* If ASM update failed, return failure */
              if ( asm_update_ret == FALSE )
              {
                RF_MSG_1( RF_ERROR, "rfm_1x_antenna_tuner_action_time: "
			  					  "[Status %d] Dev %d, "
                          " ASM update failure!", device );
                ret_val = RFM_CDMA_ERROR_FAILURE;
              }
  ...
  {
    RF_MSG_1( RF_MED, "rfm_1x_antenna_tuner_action_time: [Status %d] Dev %d, "
              "No update required",device );
...
/*!
  ...
  @param device
  RFM RX Device for which dynamic antenna tuner update is to be done.

  @param tx_device
  RFM TX Device for which dynamic antenna tuner update is to be done.
...
*/
int32
rfm_1x_antenna_tuner_action_time_v2
(
  const rfm_device_enum_type device,
  const rfm_device_enum_type tx_device
)
{
  ...
  RF_MSG_2( RF_MED, "rfm_1x_antenna_tuner_action_time_v2: [Dev %d] TxDev %d, " ,
            device, tx_device ) ;

  ret_val = rfm_1x_antenna_tuner_action_time(tx_device);
  return ret_val;
}
...
/*
  @brief
  Get max transmite power limit (mtpl) for given device and antenna position

  @detail
  This function is used by L1 to query MTPL on given TX device and antenna
  position. The function queries several sources of TX power limits. This
  includes PLIM TX override power, SV, thermal backoff and SAR based TX power
  limit per band per antenna position.

  @param device
  TX device to get MTPL

  @param antenna_position
  antenna position for mtpl
...
(
  rfm_device_enum_type device,
  int16 antenna_position
)
{
  int16 maxpower = 420; /* initialize not to do tx power limit override */
  int16 power = 420;
  int16 ret_pwr;
  int16 *max_pwr_limit_p;
  const uint16 *tx_therm_cal_p;
  rfm_cdma_band_class_type band;
  rf_cdma_data_status_type* dev_status; /* Device Status Write Pointer */
  ...
  /* Get Device related Parameter */
  dev_status = rf_cdma_get_mutable_device_status( device );
  if(NULL == dev_status)
  {
    RF_MSG_1( RF_ERROR, "%s: dev_status == NULL", __func__);
    return maxpower;
  }
  maxpower = MIN( maxpower, dev_status->plim.tx_override_pwr_limit_dbm10 );

  plim_data = rf_cdma_get_mutable_plim( device );
  if(NULL == plim_data)
  {
    RF_MSG_1( RF_ERROR, "%s: plim_data == NULL", __func__);
    return maxpower;
  }
  ...
  if( rfcommon_core_sar_get_state() == RF_SAR_STATE_DEFAULT )
  {
    ...
    maxpower = plim_data->max_power_limits.max_power_limit_vs_temp_vs_freq_dbm10;
  }
  else
  {
    ...
    else
    {
      rf_1x_config_data_ptr = rf_cdma_nv_get_1x_data(
                                       logical_dev->tx_nv_path[plim_data->band],
                                       plim_data->band );
      if( rf_1x_config_data_ptr == NULL )
      {
        RF_MSG_1( RF_ERROR, "%s: rf_1x_config_data_ptr == NULL", __func__);
      }
      else
      {
        ...
        {
          uint32 tx_therm_cal_u32[RF_CDMA_NV_TEMP_LIMIT_SIZE];

          max_pwr_limit_p = rf_cdma_nv_get_asdiv_sar_lim(
                                    (void*)rf_cdma_config_data_ptr,
                                    antenna_position);
          tx_therm_cal_p = &(rf_cdma_config_data_ptr->
                                    tx_cal.therm_cal_data.tx_therm_cal[0]);
          for(uint16 i = 0; i < RF_CDMA_NV_TEMP_LIMIT_SIZE; i++)
          {
            tx_therm_cal_u32[i] = *(tx_therm_cal_p + i);
          }
          power = rf_lininterp_u32_s16( tx_therm_cal_u32, max_pwr_limit_p,
                                        RF_CDMA_NV_TEMP_LIMIT_SIZE,
                 (uint32)dev_status->temp_comp.last_temp_sensor_read);
        }
        else
        {
          RF_MSG_1( RF_ERROR, "%s: rf_cdma_config_data_ptr == NULL", __func__);
        }
      }
    }
#endif
  ...
  ret_pwr = MIN(maxpower, power);
  RF_MSG_2(RF_MED, "%s: MPTL = %d", __func__, ret_pwr);
  return ret_pwr;
}
...
)
{
  ...
  /* Acquire try enter Critical Section Lock
  to return immediately if lock is not available*/
  rf_common_enter_critical_section ( rf_cdma_get_lock_data() ) ;
...
)
{
  ...
  /* Acquire Critical Section Lock 
  It is intentional not using try lock here as we have 
  already done the prep qta job (disabling ic as of now) and 
  we should revert it to procced anything futher*/
  rf_common_enter_critical_section(rf_cdma_get_lock_data());
...
/*!
  @brief
  Inform the RF driver the subscriber ID to use.

  @details
  The RF driver needs subscriber ID in order to allocate CCS TQ.
  This function allows L1 MUX to let the RF driver know the subscriber ID to use
  
  L1 and FTM can use this API to set subscriber ID. If not set, RF defaults to
  Subscriber 0.
...
*/
boolean
rfm_1x_set_subscriber_id
(
  uint8 sub_id
)
{
  return rf_cdma_data_set_subscriber_id(RFM_1X_MODE, sub_id);  
}

/*----------------------------------------------------------------------------*/
/*!
  @brief
  Populates 1x Tx details needed by the common MAAT module.

  @details
  This API will be called if there is a active 1x Tx device in TX or RXTX state.
  It will first set the active_tx flag to indicate the MAAT module that 1x is in
  connected state and has populated corresponding Rx and Tx info. This API will 
  continue to populate the Tx details needed by MAAT module like Tx band, channel,
  Tx RFM path and Tx Power in dBm10 units.

  @param tx_device
  The Tx device being used by 1x.

  @param tx_maat_info_ptr
  Pointer to hold the Tx paramters passed to common MAAT module.  

  @return
  void.
*/
void
rfm_1x_get_tx_maat_info
(
...
{
  rf_cdma_mdsp_tx_agc_logical_read_type tx_read; /* Variable for Tx Read */  
  int16 tc; /* turn-around const */
  const rfc_cdma_logical_device_params_type *logical_dev; /* Logical Device */
  const rf_cdma_data_status_type* dev_status_r = rf_cdma_get_device_status(tx_device);

  if (dev_status_r == NULL)
  {
    RF_MSG_1( RF_ERROR, "rfm_1x_get_tx_maat_info: NULL data for Tx device %d",
              tx_device );
    tx_maat_info_ptr->active_tx = FALSE;
    return;
  }
  ...
  if ( logical_dev == NULL )
  {
    RF_MSG( RF_ERROR, "rfm_1x_get_tx_maat_info: logical_dev is NULL." );
	tx_maat_info_ptr->active_tx = FALSE;
	return;
  }

  tx_maat_info_ptr->active_tx = TRUE;
  tx_maat_info_ptr->tx_band = dev_status_r->curr_band;
  tx_maat_info_ptr->tx_channel = dev_status_r->curr_chans[RFM_CDMA_CARRIER_0];
	
  /* Populate RFM path info*/
  tx_maat_info_ptr->tx_rfm_path.rfm_device = tx_device;
  tx_maat_info_ptr->tx_rfm_path.sig_path = 0xFFFFFFFF;
  ...
  tx_maat_info_ptr->txpwr_db10 = rf_cdma_convert_txagc( RFM_CDMA_AGC_FORMAT__DBM10, 
                                                        tc,
                                                        tx_read.total_power);		

  RF_MSG_5( RF_MED, "rfm_1x_get_tx_maat_info: 1x Tx Details to MAAT module - "
                "Tx_active %d, Tx_Band %d, Tx_Chan %d, Tx RFM Path %d %d %d",
                tx_maat_info_ptr->active_tx,
                tx_maat_info_ptr->tx_band,
                tx_maat_info_ptr->tx_channel,
                tx_maat_info_ptr->tx_rfm_path.rfm_device,
                tx_maat_info_ptr->tx_rfm_path.sig_path);
  
  RF_MSG_2( RF_HIGH, "rfm_1x_get_tx_maat_info: 1x Tx Power to MAAT module - "
                "RAW Tx power from FW = %d, Converted Tx Power in dBm10 = %d",
                tx_read.total_power, tx_maat_info_ptr->txpwr_db10);  
...
/*!
  @brief
  Populates 1x Rx details needed by the common MAAT module.

  @details
  This API will be called with the 1x PRx device as a parameter. The PRx device is 
  the associated master device linked with the Tx device. This API will populate 
  the Rx details needed by MAAT module like band, channel, PRx RFM path. It 
  then checks for associated DRx device and populates the DRx RFM path if any.
  This API will query 1x L1 API to get the combined EC/Io to be passed to 
  the MAAT module in dB10 units .

  @param rx_device
  The PRx device being used by 1x.

  @param rx_maat_info_ptr
  Pointer to hold the Rx paramters passed to common MAAT module.  

  @return
  TRUE  if SNR is successfully read 
  FALSE if SNR read not successful.

*/
boolean
rfm_1x_get_rx_maat_info
(
  rfm_device_enum_type rx_device,
  rfm_cdma_maat_rx_info_type_s *rx_maat_info_ptr
)
{
  rfm_device_enum_type assoc_div_dev;
  const rf_cdma_data_status_type* dev_status_r = rf_cdma_get_device_status(rx_device);
  const rf_cdma_data_status_type* drx_dev_status_r;
  int32 ecio_rx0;  /* EcIo on primary antenna (in dB100) */
  int32 ecio_rx1;  /* EcIo on diversity antenna (in dB100) */
  int32 ecio_comb; /* Combined EcIo (in dB100) */ 
  ...
  if (dev_status_r != NULL)
  {
    if (dev_status_r->rf_mode == RFM_1X_MODE)
    {
      ...
      {
        rx_maat_info_ptr->rx_band = dev_status_r->curr_band;
        rx_maat_info_ptr->rx_channel = dev_status_r->curr_chans[RFM_CDMA_CARRIER_0];

        /* Populate RFM path info*/
        rx_maat_info_ptr->prx_rfm_path.rfm_device= rx_device;
		rx_maat_info_ptr->prx_rfm_path.sig_path= 0xFFFFFFFF;

        assoc_div_dev = rf_cdma_data_get_assoc_dev(rx_device);

        if (assoc_div_dev != RFM_INVALID_DEVICE &&
            assoc_div_dev < RFM_MAX_WAN_DEVICES)
        {
          drx_dev_status_r = rf_cdma_get_device_status(assoc_div_dev);
          if (drx_dev_status_r != NULL)
          {
            if (drx_dev_status_r->rf_mode == RFM_1X_MODE)
            {
              if (drx_dev_status_r->data_1x.rf_state == RF_CDMA_STATE_RXDIV)
              {          
                rx_maat_info_ptr->drx_rfm_path.rfm_device = assoc_div_dev;
				rx_maat_info_ptr->drx_rfm_path.sig_path = 0xFFFFFFFF;
              }
              else
              {
                RF_MSG_2( RF_ERROR, "rfm_1x_get_rx_maat_info: "
                          "Associated DRx device %d not in RXDIV state. Current State: %d", 
                          assoc_div_dev, drx_dev_status_r->data_1x.rf_state );  	  
              }			  
            }
            else
            {
              RF_MSG_2( RF_ERROR, "rfm_1x_get_rx_maat_info: "
                        "Associated DRx device %d not in 1x Mode. Current Mode: %d", 
                        assoc_div_dev, drx_dev_status_r->rf_mode );    
            }			
          }
          else
          {
            RF_MSG_1( RF_ERROR, "rfm_1x_get_rx_maat_info: NULL data for associated DRx "
                                "device %d", assoc_div_dev );
          }		  
        }
        else
        {
          RF_MSG_2( RF_LOW, "rfm_1x_get_rx_maat_info: Invalid/Disabled DRx device: %d "
                        "associated with 1x PRx device %d. Setting invalid drx_rfm_path",
                        assoc_div_dev, rx_device);		  
          rx_maat_info_ptr->drx_rfm_path.rfm_device = RFM_INVALID_DEVICE;
          rx_maat_info_ptr->drx_rfm_path.sig_path = 0xFFFFFFFF;		    
        }

        srch_get_cdma_curr_ecio_all_chains( &ecio_comb, &ecio_rx0, &ecio_rx1 );
        api_status = TRUE;

        if (RFM_1x_MAAT_INVALID_C2K_EcIo != ecio_comb)
        {
           rx_maat_info_ptr->snr_db10 = (int16) (ecio_comb * (-0.1));
        }
        else
        {
           rx_maat_info_ptr->snr_db10 = RFM_1x_MAAT_INVALID_C2K_EcIo;
        }

        RF_MSG_6( RF_MED, "rfm_1x_get_rx_maat_info: 1x Rx Details to MAAT module - "
                      "Rx_Band %d, Rx_Chan %d, PRx RFM Path %d %d , DRx RFM Path %d %d ",
                      rx_maat_info_ptr->rx_band,
                      rx_maat_info_ptr->rx_channel,
                      rx_maat_info_ptr->prx_rfm_path.rfm_device,
                      rx_maat_info_ptr->prx_rfm_path.sig_path,
                      rx_maat_info_ptr->drx_rfm_path.rfm_device,
                      rx_maat_info_ptr->drx_rfm_path.sig_path);	

        RF_MSG_4( RF_HIGH, "rfm_1x_get_rx_maat_info: 1x SNR to MAAT module "
                      "in dB10 = %d, From L1 API : EcIo_comb = %d, EcIo_PRx = %d, "
                      "EcIo_DRx = %d", rx_maat_info_ptr->snr_db10, ecio_comb, ecio_rx0, ecio_rx1);	

      }
      else
      {
        RF_MSG_2( RF_ERROR, "rfm_1x_get_rx_maat_info: "
                  "Associated PRx device %d not in RX state. Current State: %d", 
                  rx_device, dev_status_r->data_1x.rf_state );  	  
      }
    }
    else
    {
      RF_MSG_2( RF_ERROR, "rfm_1x_get_rx_maat_info: "
                "Associated PRx device %d not in 1x Mode. Current Mode: %d", 
                rx_device, dev_status_r->rf_mode );    
    }
  }
  else
  {
    RF_MSG_1( RF_ERROR, "rfm_1x_get_rx_maat_info: NULL data for PRx device %d",
              rx_device );
  }
  return api_status;
}
