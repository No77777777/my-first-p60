...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/1x/rf/src/rf_1x_rx_agc.c#1 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------- 
08/19/16   pk      Added sem wait for FL_DRX override enable command also                 during update rxagc
08/16/16   rs      FL_DRX override command during update rxagc
...
)
{
  ...
  /* Adjust the LNA data when not in cal mode prior to sanity checking */
  if ( rfm_get_calibration_state() == FALSE )
  {
    /* Retrieve the current position */
    uint8 position = rf_cdma_data_get_ant_position ( device ) ;

       /* And if the switch position for the given NV path has auxillary data 
       adjust the Rx params accordingly */
    if ( TRUE == rf_1x_is_rx_aux_data_valid ( rf_1x_config_ptr , position ) )
    {
      if ( FALSE == 
           rf_cdma_rx_adjust_lna_info ( (void*)rf_1x_config_ptr ,
                                        RFM_1X_MODE , &rx_agc_param ) )
      {
        RF_MSG ( RF_ERROR , "rf_1x_configure_rx_agc(): "
                            "rf_cdma_rx_adjust_lna_info returned failure!!" ) ;
      }
      else
      {
        (void) rf_cdma_data_set_rxagc_status_for_ant_position ( device , TRUE ) ;
        RF_MSG_1 ( RF_LOW , "rf_1x_configure_rx_agc(): New RxAGC data loaded "
                            "for SwitchPosition:%d" , position ) ;
      }
    }
  }
...
)
{
  ...
  if ( rfm_get_calibration_state() == FALSE )
  {
    ...
    fwrsp_hk->fl_drx_override_rsp.device = device;
    fwrsp_hk->fl_drx_override_rsp.sema_token = rfcommon_semaphore_pop_item();
    fwrsp_hk->fl_drx_override_rsp.blocking_call = TRUE;

    rsp_cb_param.rsp_handler = rf_1x_mdsp_fl_drx_override_rsp_cb;
    rsp_cb_param.handler_data = &fwrsp_hk->fl_drx_override_rsp;
    ...
    if ( ret_val == FALSE )
    {
      ...
      RF_MSG ( RF_ERROR, "rf_1x_update_rx_agc():"
                 " fl_drx_override message sending failed ") ;
    }
    else
    {
      ...
      rfcommon_semaphore_wait( fwrsp_hk->fl_drx_override_rsp.sema_token );
    }

    /* Push back semaphore after wait if mssg was sent successfully,
       or immediately if mssg sending failed*/
    rfcommon_semaphore_push_item( fwrsp_hk->fl_drx_override_rsp.sema_token );
  }
  ...
  if ( ret_val == TRUE )
  {
    ...
    if ( ret_val == FALSE )
    {
      ...
      /* Message sending failed, free the semaphore */
      RF_MSG_2 ( RF_LOW, "rf_1x_update_rx_agc():"
                 " Freeing the semaphore and not waiting "   
                 " [D%d] IC_UPDATE pushed back : Sem @ 0x%x" , 
                 device , fwrsp_hk->ic_update_rsp.sema_token ) ;
    }
    else
    {
