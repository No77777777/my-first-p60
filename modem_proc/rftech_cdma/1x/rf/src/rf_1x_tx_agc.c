...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/1x/rf/src/rf_1x_tx_agc.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
04/12/16   vr      Legacy Autopin Support for WTR3925
12/24/15   vr      Autopin check-in_2
09/24/15   cd      Fixed linearizer config during temp comp, if autopin enabled
07/14/15    aa     Added support for FBRx C1 NV container
02/04/15   wwl     Get max linearzer cal power from TxAGC and update PLIM, if
                   the date is not available, use 30 dBm as the default value
...
)
{
  ...
  {
    RF_MSG( RF_ERROR, "rf_1x_configure_tx_agc: "
                      "Null pointer returned to tx multilin NV ");
    return FALSE;
  }
  ...
  else if ( dev_status_r->temp_comp.init_trigger == FALSE &&
    ...
    if ( result == TRUE )
    {
      ...
      if ( config_linearizer == TRUE ) 
      {
        ...
        if (NULL == log_data)
        {
          ...
          rf_1x_plim_update_lin_limit( device, 300 );
        }
        else
	    {
