...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/1x/rf/src/rf_1x_power_limiting.c#1 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------- 
08/15/15   sty     temp workaround for ASDIV deregister issue
07/27/15   pk      De-register AsDiv cb function during PLIM stop only in 
                   non-calibration state
06/16/15   wwl     Add support for new SAR implementation
06/19/15   wwl     Add message to report max power
03/30/15   zhw     Fix plim NV access. Use Tx NV path instead of Rx NV Path (2)
03/30/15   zhw     Fix plim NV access. Use Tx NV path instead of Rx NV Path
...
/*!
  @brief
  Call back function when antenna position change happens

  @param *asdiv_cb_data_ptr
  used to pass device number from the caller

  @param *asdiv_cb_sw_pos_data_ptr
  not really used
*/
...
{
  rf_plim_asdiv_cb_type *cb_data = (rf_plim_asdiv_cb_type*)asdiv_cb_data_ptr;

  MSG_2(MSG_SSID_RF, MSG_LEGACY_MED,
        "%s: asdiv cb dev %d", __func__, cb_data->device);

  rf_1x_plim_update_sar_state( cb_data->device );
  return NULL;
}
#endif
...
)
{
  ...
  {
#if defined(FEATURE_RF_ASDIV)
    int16 *sar_data_p = rf_cdma_nv_get_asdiv_sar_lim(
                                    (void *)rf_cdma_config_data_ptr,
                                    rfcommon_asdiv_get_current_position(device));
    /* Make a copy of SAR data when the SAR data pointer is valid */
    if(NULL != sar_data_p)
    {
#endif
...
)
{
       ...
       (plim_data->mode == RFM_1X_MODE) )
  {
...
#if defined(FEATURE_RF_ASDIV)
       int16 *sar_data_p = rf_cdma_nv_get_asdiv_sar_lim((void *)rf_cdma_config_data_ptr,
                                    rfcommon_asdiv_get_current_position(device));
       if(NULL != sar_data_p)
       {
         plim_data->max_power_limits.max_power_limit_sar_dbm10 = sar_data_p[0];
       }
       else
       {
         RF_MSG(RF_ERROR, "SAR temp comp data fetch failure");
       }
#else
       plim_data->max_power_limits.max_power_limit_sar_dbm10
    ...
    RF_MSG_4 ( RF_MED , "rf_1x_plim_update_temp_mdsp: "
              "1x PLIM: Started. Max plim SAR(dbm10) %d, "
              "Max plim vs temp vs freq %d (dbm10) min plim (dbm10) %d, "
              "Max power %d",
              plim_data->max_power_limits.max_power_limit_sar_dbm10,
...
)
{
  ...
  rf_1x_config_data_ptr = rf_cdma_nv_get_1x_data( logical_dev->tx_nv_path[band]
                                                 , band );
  if ( rf_1x_config_data_ptr == NULL )
  {
    RF_MSG( RF_ERROR,
    ...
    if ( plim_data->override_enabled == FALSE )
    {
      ...
      { 
         /* No active temp measurement updates yet, so always use index 0
               directly from the temp comp table. */
#if defined(FEATURE_RF_ASDIV)
         int16 *sar_lim = rf_cdma_nv_get_asdiv_sar_lim((void*)rf_cdma_config_data_ptr,
                                    rfcommon_asdiv_get_current_position(device));
         if(NULL != sar_lim)
         {
           plim_data->max_power_limits.max_power_limit_sar_dbm10 = sar_lim[0];
         }
#else
         plim_data->max_power_limits.max_power_limit_sar_dbm10
           = rf_cdma_config_data_ptr->tx_static.lim_vs_temp[rfcommon_core_sar_get_state()][0];
...
#if defined(FEATURE_RF_ASDIV)
    /* Register call back function into ASDIV, the function should be
     * called to update max TX power to MDSP whenever antenna position
     * changes */
    rf_cdma_data_status_type *dev_status = 
                                   rf_cdma_get_mutable_device_status( device );
    if(NULL != dev_status)
    {
      dev_status->plim_asdiv_cb_data.asdiv_cb_handler = rfcommon_asdiv_register_cb(
                       rf_1x_plim_asdiv_cb,
                       (void*)(&(dev_status->plim_asdiv_cb_data)),
                       &(dev_status->plim_asdiv_cb_data.asdiv_cb_sw_pos_data),
                       RFM_1X_MODE, ASDIV_CB_REPEAT );
    }
#endif

    if ( plim_data->debugging_enabled == TRUE )
    {
      RF_MSG_5( RF_LOW, "rf_1x_plim_start: 1x PLIM: Started. Max plim SAR(dbm10) %d, "
...
{
  boolean api_res = FALSE; /* API Result */
  ...
  if ( NULL != plim_data )
  {
    ...
    api_res = TRUE; /* start off with success */

#if defined(FEATURE_RF_ASDIV)
    /* deregister plim call back function */
    if((rfm_get_calibration_state() == FALSE )&&
       (RF_CDMA_PLIM_ENABLED == plim_data->state))
    {
      // callback function was registered in rf_1x_plim_start() and 
      // plim_data->state is updated accordingly; The callback should be
      // deregister in this stop function only when it was registered 
      // successfully. If the deregister operation fails, it will not 
      // affect the return indication
      const rf_cdma_data_status_type *dev_status = 
                                        rf_cdma_get_device_status(device);
      if(NULL != dev_status)
      {
        if(!rfcommon_asdiv_deregister_cb(dev_status->plim_asdiv_cb_data.\
                                       asdiv_cb_handler))
        {
          RF_MSG_1( RF_ERROR, "1x PLIM: deregister callback failed on device %d, "
                              " the callback function may not be register properly",
                              device );
          api_res = FALSE;
        }
      }
...
)
{
  ...
  RF_MSG_4 ( RF_MED , "rf_1x_plim_update_lin_limit: " 
             "1x PLIM: Started. Max plim SAR(dbm10) %d, "
             "Max plim vs temp vs freq %d (dbm10) min plim (dbm10) %d, " 
             " Max power %d",
             plim_data->max_power_limits.max_power_limit_sar_dbm10,
...
)
{
  ...
  RF_MSG_4 ( RF_MED , "rf_1x_plim_update_lim_vs_temp_vs_freq: " 
             "1x PLIM: Started. Max plim SAR(dbm10) %d, "
             "Max plim vs temp vs freq %d (dbm10) min plim (dbm10) %d, " 
             " Max power %d",
             plim_data->max_power_limits.max_power_limit_sar_dbm10,
