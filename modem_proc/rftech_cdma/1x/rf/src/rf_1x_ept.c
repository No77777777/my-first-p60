...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/1x/rf/src/rf_1x_ept.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
03/09/18   mk      KW Errors fix
03/02/16   rs      Add support for FBRx RSB changes
...
01/06/15   wwl     Add device parameter into rf_1x_xpt_configure_fb_rxlm( )
...
/* Global Variable to hold the rxlm handle for no Drx device */
lm_handle_type fb_rxlm_handle;
...
)
{
  ...
  rfm_cdma_power_mode_type power_mode = 0 ; /* Not used anymore */
  ...
  if ( curr_mode != RFM_1X_MODE )
  {
    RF_MSG_2( RF_ERROR, "rf_1x_xpt_configure_fb_rxlm: Dev[ x %d ] "
              "Bad Mode %d", device, curr_mode );
    return FALSE;
  } /* if ( curr_mode != RFM_1X_MODE ) */


  /* no diversity device, use primary instead to configure fb rxlm*/
  rf_cdma_data_status_type *dev_status_w_tx;/* Dev Status Read Ptr*/
  const rfc_cdma_logical_device_params_type *logical_dev_tx;
  /* Get Device Status */
  dev_status_w_tx = rf_cdma_get_mutable_device_status( device );
  /* Get Logical Device Param */
  logical_dev_tx = rfc_cdma_get_logical_device_params( device );

  /* Perform NULL Pointer check */
  if ( ( dev_status_w_tx == NULL ) || ( logical_dev_tx == NULL ) )
  {
    RF_MSG_1( RF_ERROR, "rf_1x_xpt_configure_fb_rxlm: Dev[ x %d ] NULL "
              "data", device );
    return FALSE;
  ...
  /* Udate RxLM Handle to the global variable to support the no drx device
  and also not to break the existing functionality*/
  fb_rxlm_handle = rxlm_fb_handle;
  ...
  /* check if we have valid drx path, if so then we can 
  take the state machine info from fb device or should be taken 
  from Prx Device.*/
   if ( logical_dev != NULL && dev_status_w !=NULL )
  {
    /* For the xpt capture we should query the modem chain to be used from 
    the xpt capture data structure of the RFC to support rx\tx split */
    dynamic_cfg.modem_chain = rfc_cdma_get_fbrx_modem_chain ( device , band ) ;
    device_id = rfdevice_id_get(logical_dev_tx->tx_device[band]);
  ...
  {
    /* no diversity device, use primary instead to configure fb rxlm*/
    const rfc_cdma_logical_device_params_type *logical_dev_ptx;
    /* Get Logical Device Param */
    logical_dev_ptx = rfc_cdma_get_logical_device_params( device );

    /* Perform NULL Pointer check */
    if ( logical_dev_ptx == NULL )
    {
      RF_MSG_1( RF_ERROR, "rf_1x_xpt_configure_fb_rxlm: Dev[ x %d ] NULL "
                "data", device );
      ret_val =  FALSE;
    ...
    {
      /* Hard code feedback modem chain to 1, internally the modem chain is
      hard coded to 4 at msm level anyway so this is a safe assumption at
      this level */
      dynamic_cfg.modem_chain = rfc_cdma_get_fbrx_modem_chain ( device , band ) ;
      device_id = rfdevice_id_get(logical_dev_tx->tx_device[band]);
      static_cfg.rxlm_adc = logical_dev_ptx->rxlm_adc;
      ret_val = TRUE;
    }
  ...
  RF_MSG_5( RF_MED, "rf_1x_xpt_configure_fb_rxlm : TxDev[ %d ] FB-Dev[ %d ] Dev ID[ %d]"
            "[Et Modem Chain %d][Status %d]",
            device, device_fb, device_id, dynamic_cfg.modem_chain , ret_val ) ;
...
)
{
  ...
  logical_dev_fb = rfc_cdma_get_logical_device_params( device);
  ept_data = rf_cdma_get_ept_data();
  ...
  {
    rxlm_handle_for_cap =  fb_rxlm_handle;
  }
