...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/1x/rf/src/rfm_1x_ftm.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
03/27/18   rp      KW Fixes.
03/23/16   sk      Update but not write FBRX ADC settings for LTE-tune away support
01/28/16   pk      Added a missing error check condition
01/27/15   pk     Apply dynamic ADC settings as part of FBRX AGC setup
08/07/15   pl      use assigned device in ftm_cdma_mc_dispatch to get RXAGC leve
04/28/15   zhw     Reworked Tuner API
04/15/15   zhw     Pass Prx WTR object to WTR driver in Drx case
04/15/15   zhw     Integrate device driver change with CDMA
                   Remove deprecated API
                   Fix CDMA RF Cal due to ULCA changes
03/30/15   zhw     Support RF_CDMA_STATE_TX in MC
03/04/15   spa     Add support for XPT cal when Drx path not present
01/06/15   wwl     Add device parameter into rfm_1x_ftm_config_fb_rxlm( )
...
*/
boolean
rfm_1x_ftm_set_tx_agc_override_immediate
(
  const rfm_device_enum_type device,
...
)
{
  ...
  {
    RF_MSG_1 ( RF_ERROR,
               "rfm_1x_ftm_set_tx_agc_override_immediate: NULL Dev %d data" , device ) ;
    return FALSE;
  }
  ...
  {
    RF_MSG_2 ( RF_ERROR ,
               "rfm_1x_ftm_set_tx_agc_override_immediate: Dev %d , Device State: %d"
  ...
  if ( logical_dev == NULL )
  {
    RF_MSG( RF_ERROR, "rfm_1x_ftm_set_tx_agc_override_immediate: "
  ...
  if ( status == TRUE )
  {
    ...
    {
      RF_MSG( RF_HIGH,  "rfm_1x_ftm_set_tx_agc_override_immediate: "
  ...
  if( status == TRUE )
  {
    ...
    RF_MSG( RF_HIGH,  "rfm_1x_ftm_set_tx_agc_override_immediate: "
            "pa_output_cap_offset is being set to 0" ) ;
  ...
  RF_MSG_5( RF_HIGH, "rfm_1x_ftm_set_tx_agc_override_immediate: RGI %d, SMPS %d, "
      ...
      if ( status == TRUE )
      {
        ...
        curr_tx_ovr.pa_range =
          rf_cdma_map_pa_state_to_gain_range( device, band,
                                              pa_state );
        curr_tx_ovr.txfe0_handle_id = rf_cdma_data_get_txlm_handle ( device ) ;
    ...
    else /* all checks passed */
    {
...

      {
        ...
        if ( dev_status_r->data_1x.rf_state == RF_CDMA_STATE_RXDIV )
        {
          ...
          {
            associated_rx_tranceiver = assco_logical_dev->rx_device[dev_status_r->curr_band];
          }
          else
          {
            RF_MSG_2( RF_ERROR, "rfm_1x_ftm_get_rx_agc_data: "
        ...
        rfdevice_cdma_rx_get_lna_state( &(rx_agc_data->gain_word), 
                                        dev_status_r->curr_band, 
                                        rfdevice_rx_mode, 
...
{
  void* smem_addr;
  rf_cdma_fwrsp_data_type *fwrsp_data = NULL;
  ...
  /* Get FW Response Data */
  fwrsp_data = rf_cdma_get_fwrsp_data(device);
...
)
{
  ...
  ret_val = rf_1x_xpt_configure_fb_rxlm( device, device_fb, rxlm_fb_handle,
                                         band, chan );
  ...
  for ( device = RFM_DEVICE_0; device < RFM_MAX_DEVICES; device++ )
  {
       ...
       )
    {
      ...
      if ( m_logical_dev->ant_tuner[band] != NULL )
      {
        ...
        switch ( m_dev_r->data_1x.rf_state )
        {
        ...
        case RF_CDMA_STATE_RXTX:
          ...
          break;
        default:
          tuner_config.rx_tx = RFCMN_ATUNER_INVALID_DEVICE;
          break;
        }
...
boolean
rfm_1x_ftm_config_rxlm_for_fbagc( rfm_device_enum_type device, 
                                  rfm_cdma_band_class_type band )
{
  ...
  if ( logical_dev != NULL )
  {
    /* @todo: remove hard-coding, tech to update the fbrx chain from rfc */
	    ...
	    RF_MSG_3( RF_ERROR, "kathir rfm_1x_ftm_config_rxlm_for_fbagc: "
                        " fbrx modem chain %d %d %d", fbrx_modem_chain,device, band);
		
    rfcommon_fbrx_msm_config_dynamic_adc_settings(fbrx_lm_handle, 
                                                 (uint32)rfdevice_id_get(logical_dev->tx_device[band]),fbrx_modem_chain,RFLM_TECH_1X) ;      

    RF_MSG_1( RF_HIGH, "rfm_1x_ftm_config_rxlm_for_fbagc: Dynamic ADC Settings applied for device_id %d", 
                       rfdevice_id_get(logical_dev->tx_device[band]) );

  }
  else
  {
    RF_MSG_1( RF_ERROR, "rfm_1x_ftm_config_rxlm_for_fbagc: Invalid device_id %d", 
                        device);  
    ret_value = FALSE;
  }
 ...
 /*!
 @brief
 This API is to find the assigned device

 @param is_pri_device
Indicate primary or diversity device need to be found.

 @return rfm_1x_device
 assigned device corresponding to legacy RF device.
*/

rfm_device_enum_type 
rfm_1x_ftm_get_active_device
(
	boolean is_pri_device
)
{
   rfm_device_enum_type rfm_1x_device;
   rfm_1x_device = rf_cdma_data_get_active_device(is_pri_device);
   return rfm_1x_device;
}/*rfm_1x_ftm_get_active_device*/
