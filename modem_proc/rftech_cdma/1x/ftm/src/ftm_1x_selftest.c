...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/1x/ftm/src/ftm_1x_selftest.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
09/03/15   dbz     Selftest call flow re-arch
08/18/15   zbz     Use correct 1x band to get selftest NVs
...
)
{
  ...
  *ftm_rf_cdma_payload_tx_params = &(ftm_rf_fbrx_iq_acquired_samples_data_ptr->meas_results);
  ...
  ftm_rf_fbrx_meas_tx_params_rsp_pkt->subsys_cmd_code = FTM_1X_C0_C;
  ...
  if( ftm_get_current_state(params->prx_device) != FTM_STATE_1X)
  {
    ftm_rf_fbrx_meas_tx_params_rsp_pkt->ftm_error_code = FTM_FBRX_TEST_GENERAL_FAILURE;
    FTM_MSG_1(FTM_HIGH, "FTM Mode is %d", ftm_get_current_state(params->prx_device));
...
)
{
  ...
  dev_status = rf_cdma_get_device_status ( params.tx_device );
  if ( dev_status != NULL )
  {
    fbrx_gs_info.override_enable = ftm_1x_obtain_fbrx_gain_stage(&(fbrx_gs_info.selftest_fbrx_gs), dev_status->curr_band);
...
)
{
  ...
  FTM_MSG_2(FTM_MED, "ftm_1x_fbrx_meas_tx_params: ftm_cdma_data_get_band_class for device %d returns %d", params.tx_device, curr_1x_band);
...
)
{
   ...
   curr_1x_band = ftm_cdma_data_get_band_class(params.tx_device);
   ...
   ftm_common_selftest_get_coupler_script(RFM_1X_MODE, com_band, params.tx_device);
  ...
  if (params.enable_vswr_meas_v2)
  {
   ...
   if (revert_fbrx == TRUE)
   {
     ...
     fbrx_modem_chain = rfc_cdma_get_fbrx_modem_chain(params.tx_device, com_band.cdma_band);
...
boolean ftm_1x_obtain_fbrx_gain_switchpoint_nv_item(const rfm_cdma_band_class_type band, rfnv_item_id_enum_type * nv_item)
{
  boolean ret_value = TRUE;
...
boolean ftm_1x_obtain_fbrx_gain_stage(uint8 * gain_stage, rfm_cdma_band_class_type band)
{
  rfnv_item_id_enum_type nv_id;
  boolean nv_valid;
  ...
  /* determine the correct NV item location */
  nv_valid =  ftm_1x_obtain_fbrx_gain_switchpoint_nv_item(band, &nv_id);
