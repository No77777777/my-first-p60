...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/1x/ftm/src/ftm_1x_control.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
03/09/18   mk      KW Errors fix
12/10/15   rs      KW Error fix
12/09/15   pk      Fix for crash during 1x to W transition due to rx\tx split
12/09/15   pk      Added support for CDMA rx\tx split
10/20/15   rs      Corrected the IQ Samples Analysis packet size
09/01/15   tjm/dbz Modified Meas Tx parameters handler
08/26/15   zhw     Fixed hard-coded ftm chain in iq capture payload
04/30/15   pl/spa  Support generation of sigle tone waveform
07/28/15   zhw     Fix incorrect Tx state machine check during 1x tear down[p2]
07/28/15   zhw     Fix incorrect Tx state machine check during 1x tear down
07/15/15   zbz     pass device id into ftm iq processing
06/10/15   dbc     Add ftm_1x_set_pa_tunable_cap() API
06/15/15   vc      Toggling Frame Valid flag while doing retune to avoid RFCAVFS HO failure.
05/21/15   zhw     XPT Char fix - use rfm_1x_retune_v2
...
05/20/15   zhw     Skip checking for rfm mode in enable_tx
...
03/10/15   pl      Update concurrency manager upon band and tech switch in FTM mode
03/10/15   pk      OFT Target compiler error fix.
03/04/15   pk      Added API to analyze the FTM IQ capture samples
01/15/15   cd      Init Tx Freq Error to zero during FTM wakeup Tx
...
#include "rfdevice_cdma_pa_intf.h"
...
/* Following headers are needed for tone gen API*/
#include "ftm_1x_msm.h"
#include "rfc_cdma.h"
...
/*!
  @brief
  Allocate txlm buffer and set the txlm vote count for the device
 
  @details
  This funciton will allocate the txlm buffer with the added vote for current 
  device
 
  @param device
  RF Device for which txlm buffer has to be done.
*/
lm_status_type
ftm_1x_vote_txlm_buffer
(
  rfm_device_enum_type tx_device
)
{
  lm_status_type tx_buffer_allocated = LM_SUCCESS; /* Tx Buffer */
  ftm_cdma_data_status_type *tx_device_status;
  const rfc_cdma_logical_device_params_type *tx_logical_dev; /* Logical Dev */

  tx_logical_dev = rfm_cdma_ftm_get_logical_device_params(tx_device);

  tx_device_status = ftm_cdma_data_get_device_ptr(tx_device);

  /* Allocate TXLM buffers, only if Chain is valid */
  if ( tx_device != RFM_INVALID_DEVICE && tx_logical_dev != NULL &&
       tx_device_status != NULL )
  {

    if( tx_device_status->txlm_handle_dev_vote == 0 )
    {
      if( tx_device_status->txlm_handle == FTM_INVALID_LM_BUFFER )
      {
        tx_buffer_allocated = txlm_allocate_buffer(tx_logical_dev->ftm_txlm_chain_onex,
                                                   LM_1X,
                                                   &tx_device_status->txlm_handle );
      }
      else
      {
        FTM_MSG_1(FTM_ERROR, "ftm_1x_vote_txlm_buffer: valid Tx handle found with no device voting  "
                            "on Tx device %d ", tx_device );        

      }
    }

    tx_device_status->txlm_handle_dev_vote |= (1 << tx_device);

    FTM_MSG_4(FTM_MED, "ftm_1x_vote_txlm_buffer: Tx device %d has txlm_handle "
                        "%d allocated. Rx device %d, "
                        "Device vote mask updated 0x%x",
                         tx_device, tx_device_status->txlm_handle,tx_device,
                         tx_device_status->txlm_handle_dev_vote );        

  }
  else
  {
    FTM_MSG_1(FTM_ERROR, "ftm_1x_vote_txlm_buffer: NULL device status pointers for "
                         "valid tx device %d", tx_device); 
  }

  return tx_buffer_allocated; 
...
/*!
  @brief
  De-allocate txlm buffer and reset the txlm vote count for the device
 
  @details
  This funciton will de-allocate the txlm buffer with the reset vote for current 
  device
 
  @param device
  RF Device for which txlm buffer has to be deallocated.
*/

lm_status_type
ftm_1x_unvote_txlm_buffer
(
  rfm_device_enum_type tx_device
)
{

  lm_status_type tx_buffer_deallocated = LM_INVALID_PARAMETER; /* Tx Buffer */

  ftm_cdma_data_status_type *tx_device_status;

  tx_device_status = ftm_cdma_data_get_device_ptr(tx_device);

  if( tx_device != RFM_INVALID_DEVICE && tx_device_status != NULL )
  {
    uint32 new_vote_mask = 0;

    /* Update the Vote mask only if the Calibration mode is disabled */
    new_vote_mask |= (1 << tx_device );
    new_vote_mask = ~new_vote_mask;

    tx_device_status->txlm_handle_dev_vote &= new_vote_mask;

    /* DeAllocate TxLM Buffer */
    if ( tx_device_status->txlm_handle_dev_vote == 0 )
    {
      tx_buffer_deallocated =
        txlm_deallocate_buffer( tx_device_status->txlm_handle );

      tx_device_status->txlm_handle = FTM_INVALID_LM_BUFFER;
    }

    FTM_MSG_4(FTM_MED, "ftm_1x_unvote_txlm_buffer: Tx device %d has txlm_handle "
                        "%d now. Rx device %d, "
                        "Device vote mask updated 0x%x",
                         tx_device, tx_device_status->txlm_handle,tx_device,
                         tx_device_status->txlm_handle_dev_vote );        
...
)
{
  ...
  rfm_device_enum_type m_device = 0xFFFF;  /* signifies the master-device */
   ...
   */
    ...
    /* validate band on diversity from concurrency manager */
    if ( ftm_cdma_control_is_tune_allowed( RFM_1X_MODE, device, 
                    dev_state.curr_band))
    ...
    {
       rfm_ret = FALSE;
       FTM_MSG_3 ( FTM_ERROR , "ftm_1x_set_channel: "
  ...
  case FTM_CDMA_STATE_RXDIV:
  {
    ...
    /* clean up master device from concurrency manager before retune */
    ftm_concurrency_manager_cleanup_device_allocation(m_device, RFM_1X_MODE);

    /* clean up assocaite device from concurrency manager before retune */
    ftm_concurrency_manager_cleanup_device_allocation(s_device, RFM_1X_MODE);

    /* validate new band for both master and assocaite device*/
    if ( ftm_cdma_control_is_tune_allowed( RFM_1X_MODE, m_device, 
                    m_dev_state.curr_band) && 
         ftm_cdma_control_is_tune_allowed( RFM_1X_MODE, s_device, 
                    s_dev_state.curr_band))
    {
      rfm_device_enum_type tx_device;
      ...
      (void)ftm_cdma_data_get_device_status( m_device, &rx_dev_state );
      ...
      rfm_ret &= ( rfm_1x_retune_v2( m_device, tx_device, m_dev_state.rxlm_handle, 
                                     band_chan, &ftm_1x_rfm_callback, &cb_payload ) >= 0 );
    }
    else
    {
       rfm_ret = FALSE;
       FTM_MSG_4( FTM_ERROR, "ftm_1x_set_channel: " 
                     "Concurrency manager returned failure for" 
                     " tech:%d for master device:%d and slave device:%d on band:%d", 
                     RFM_1X_MODE, m_device, s_device, m_dev_state.curr_band); 
    }
  ...
  case FTM_CDMA_STATE_RX:
    ftm_concurrency_manager_cleanup_device_allocation(device, RFM_1X_MODE);
                    ...
                    dev_state.curr_band))
    {
      ...
      rfm_ret &= ( rfm_1x_retune_v2( device, tx_device, dev_state.rxlm_handle, band_chan, 
  ...
  case FTM_CDMA_STATE_RXTX:
    /* clean up device from concurrency manager*/
    ftm_concurrency_manager_cleanup_device_allocation(device,RFM_1X_MODE);
    /* validate new band */
    if ( ftm_cdma_control_is_tune_allowed( RFM_1X_MODE, device, 
                    dev_state.curr_band))
...
)
{
  ...
  /* Get the Tx Device state */
  ftm_cdma_data_get_device_status(tx_device, &dev_state);
  ...
  */
  ...
  rfm_ret = ftm_1x_wakeup_tx_v2( tx_device, 
                                 prx_device, 
                                 &ftm_1x_rfm_callback, 
                                 &cb_payload );
  ...
  { 
    ftm_cdma_data_status_type prx_dev_state; /* hold current state of device */

    (void)ftm_cdma_data_get_device_status( prx_device, &prx_dev_state );

    /* Update Tx device state, band & chan info */
    ftm_cdma_data_update_rf_state(prx_device, next_rf_state);
    ftm_cdma_data_update_rf_state(tx_device, next_rf_state);
    ftm_cdma_data_update_band_chan(tx_device, prx_dev_state.curr_band, 
                                   prx_dev_state.curr_chan);
...
)
{
  ...
  */
  ...
  rfm_ret = ftm_1x_sleep_tx( device, 
...
)
{
  ...
  if ( state < RFM_PA_GAIN_STATE_NUM )
  {
    ...
    rfm_ret = rfm_1x_ftm_set_tx_agc_override_immediate( 
                               device, 
                               dev_ptr->curr_band,
                               state,
...
/*!
  @brief
  Apply an offset to the PA Tunable Cap.

  @details
  
  Associated FTM Command : <b>FTM_SET_PA_TUNABLE_CAP</b>

  @param device
  RF Device for which PA Tunable Cap offset is to be applied
 
  @param pa_tunable_cap_offset
  Signed 8 bit offset to apply to the default value of the PA Tunable Cap

  @return
  Status of execution completion of the function execution
*/
ftm_cdma_rsp_type
ftm_1x_set_pa_tunable_cap
(
  const rfm_device_enum_type device,
  byte pa_tunable_cap_offset
)
{
  ...
  if (dev_ptr == NULL)
  {
    FTM_MSG_1( FTM_ERROR, "ftm_1x_set_pa_tunable_cap: Failed to get a valid "
                          "device pointer for device %d", device );
    return FTM_CDMA_BAD_RSP; /* Failure */
  }

  if ( !rfdevice_cdma_set_pa_output_cap( device,
                                         dev_ptr->curr_band,
                                         (int8)pa_tunable_cap_offset )) 
  {
    FTM_MSG_3( FTM_ERROR, "rfdevice_cdma_set_pa_output_cap failed!: device: %d, band: %d, offset: %d", 
               device, dev_ptr->curr_band, (int8)pa_tunable_cap_offset ); 
  }
  else
  {
    ret_val = FTM_CDMA_GOOD_RSP;
...
)
{
  ...
  be set by the QPOET */ 
  rfm_ret &= rfm_1x_ftm_set_tx_agc_override_immediate( device, dev_ptr->curr_band,
                                                RFM_PA_GAIN_STATE_0,
                                                rgi_min, 0,/*betaP*/
...
)
{
  ...
  rfm_device_enum_type tx_device;
  rx_device = ftm_cdma_get_remapped_rx_device( ovr_params->device );
  (void)ftm_cdma_data_get_device_status( rx_device, &rx_dev_state );
  sys_band = rf_cdma_rf_band_type_to_sys_band_type( rx_dev_state.curr_band);
  tx_device = rfc_common_get_preferred_tx_device ( sys_band );

  dev_ptr = ftm_cdma_data_get_device_ptr( tx_device );
  ...
  if ( rfcommon_debug_flags.use_rflm_txagc_override )
  {
    ...
    ret_val &= rfm_ftm_apply_xpt_tx_agc_override( RFM_1X_MODE, tx_device,
                                                  rfm_band );
  }
  else
  {
...
)
{
  ...
  if(  dev_status == NULL )
  {
    FTM_MSG_1(FTM_ERROR,"ftm_1x_tear_down: NULL data for device %d ",
              device);
  ...
  else
  {
    ...
    {
      ftm_cdma_data_status_type *tx_dev_status = NULL; /* FTM Device Status */
      lm_status_type tx_buffer_deallocated = LM_INVALID_PARAMETER;
	  
      tx_dev_status = ftm_cdma_data_get_device_ptr(tx_device); 

      if ( tx_dev_status != NULL )
      {
        /* If Rx or Tx device tracks as RxTx state, 
           then disable the modulator */
        if( dev_status->rf_state == FTM_CDMA_STATE_RXTX )
        {
          /* Disable the modulator during tear down */
          ftm_1x_disable_modulator( tx_device );

          /* Tear down radio using RFM tear down API*/
          was_tear_down_req = rfm_1x_ftm_tear_down_radio( tx_device );

          tx_buffer_deallocated = ftm_1x_unvote_txlm_buffer(tx_device);

          if (tx_buffer_deallocated != LM_SUCCESS)
          {
            FTM_MSG(FTM_ERROR, "ftm_1x_tear_down: Buffer deallocation failed");
	    was_tear_down_req = FALSE;
          }

          tx_dev_status->rf_state = FTM_CDMA_STATE_SLEEP;
    ...
    if ( was_tear_down_req )
    {
      ...
      else
      {
          ...
          {  
            /* clean up concurrency manager for associated device*/ 
            ftm_concurrency_manager_cleanup_device_allocation(assoc_dev, RFM_1X_MODE);  
...
)
{
  ...
  tx_device = rfc_common_get_preferred_tx_device  ( SYS_BAND_BC0 ); 
  ...
  FTM_MSG(FTM_HIGH, 
          "ftm_1x_do_tx_dac_cal: Placeholder, DAC cal done in FED now") ;
  ...
  ftm_cdma_rsp = ftm_1x_set_tx_waveform( tx_device, CW_WAVEFORM, 
                                         CW_OFFSET_ZER0_KHZ);
  ...
  if ( ftm_cdma_data_get_device_status( tx_device, &dev_state ) == FALSE)
  {
  ...
  dac_cal_status &= rfm_cdma_ftm_get_min_max_avga_idx( tx_device, &rgi_min, 
                                                       &rgi_max);
  ...
  from the PA driver, this is the minimum possible bias voltage that can 
  be set by the QPOET */ 
  dac_cal_status &= rfm_1x_ftm_set_tx_agc_override_immediate( tx_device, RFM_CDMA_BC0,
                                                     rfm_pa_gain_state,
                                                     avga_idx, 0, /*betaP*/
                                                     RFDEVICE_PAPM_MIN_TX_BIAS_VOLTAGE, 
                                                     0, /*Quiscent current */
  ...
  ftm_cdma_rsp = ftm_1x_enable_tx( tx_device, device);
...
FUNCTION FTM_1X_IQ_CAPTURE_ANALYZE_SAMPLES
...
SIDE EFFECTS
 None
=============================================================================*/
void ftm_1x_iq_capture_analyze_samples
(
  rfcom_device_enum_type device, 
  ftm_iq_capture_analyze_samples_type *ftm_req_data,
  ftm_rsp_pkt_type* rsp_pkt_ptr
)
{

  /* Array that holds the IQ Analysis result, it now holds carrier to noise and estimated sensitivity of the captured IQ*/
  static int32 analysis_result[FTM_IQ_CAPTURE_ANALYZE_SAMPLES_MAX_PAYLOAD_SIZE];

  /* stucture used to get the IQ configuration for CDMA */
  ftm_iq_capture_get_config_type cmd_ptr_pkt_for_get_config = {FTM_IQ_SUCCESS, 0,0,0,0}; 
  
  /* create an object for acquire_sample() API */
  ftm_pkt_type ftm_pkt_type_obj; 

  /* Pointer to the structure that holds the input data for acquire_sample API*/
  ftm_iq_capture_acquire_samples_type *cmd_ptr_pkt_for_acq_samples = NULL; 

  /* sturcute that used to get the sensitivity estimation results (cton, sens, peak_freq) */
  ftm_common_sens_est_result_type sens_est_result = {0,0,0};

  /* pointer to store the address of CDMA IQ capture Data structure which points to information of acquired samples */
  ftm_cdma_iq_capture_data_type *iq_capture_data_ptr;

  /* IQ Capture Analyze samples rsp pkt ptr*/ 
  ftm_iq_capture_analyze_samples_rsp_pkt_type *analyze_samples_rsp_pkt_ptr 
  	= (ftm_iq_capture_analyze_samples_rsp_pkt_type *)rsp_pkt_ptr->pkt_payload;
	
  ftm_rf_technology_type tech = FTM_RF_TECH_CDMA;  
  ...
  /* start with Success flag and alter it if any sanity check fails */
  analyze_samples_rsp_pkt_ptr->ftm_error_code = FTM_IQ_SUCCESS;

 /* NULL Pointer check for the ftm request packet */
 if(ftm_req_data == NULL )
  {
   rsp_pkt_ptr->cmd = FTM_RSP_BAD_CMD;
   FTM_MSG_ERROR("Message pointer is NULL",0,0,0);
   return;
  } 

  /* Get info from req packet */
  analysis_id = ftm_req_data->analysis_id;
  rx_chain =    ftm_req_data->select_rx_chain;
  sample_size = ftm_req_data->sample_size;
  dl_pwr_lvl =  (int16)(ftm_req_data->dl_pwr & 0xffff);

  /* Fill the res pkt payload with necessary info */
  analyze_samples_rsp_pkt_ptr->sample_size = sample_size;
  analyze_samples_rsp_pkt_ptr->analysis_id = analysis_id;
  analyze_samples_rsp_pkt_ptr->select_rx_chain = rx_chain;
  analyze_samples_rsp_pkt_ptr->reserved = (uint32)0;
  analyze_samples_rsp_pkt_ptr->payload_size = (uint16)0;	
  ...
  cmd_ptr_pkt_for_acq_samples->sample_size = sample_size;
  ...
  ftm_1x_iq_capture_get_config(&cmd_ptr_pkt_for_get_config);

  /* Acquire IQ samples  */
  ftm_1x_iq_capture_acquire_samples(device, cmd_ptr_pkt_for_acq_samples);

  /* Get the pointer handler for IQ capture data structure */
  iq_capture_data_ptr=ftm_cdma_get_iq_capture_data_ptr();

  FTM_MSG_3(FTM_MED, "rx_chain = %d and num_samples = %d and samp_freq = %d", 
            rx_chain, sample_size,
            cmd_ptr_pkt_for_get_config.sampling_freq);
  
 
  if (iq_capture_data_ptr->data_available == FALSE )
  {
    FTM_MSG_ERROR( "IQ samples are not acquired, please acquire them first",0,0,0);      
    analyze_samples_rsp_pkt_ptr->ftm_error_code = FTM_IQ_IQ_SAMPLES_UNAVAILABLE;
  }
  
  if(iq_capture_data_ptr->req_samples > FTM_CDMA_IQ_MAX_SAMPLES)
  {
    FTM_MSG_ERROR( "Requested Sample size %d is greater than %d",sample_size,FTM_CDMA_IQ_MAX_SAMPLES,0);	   
    analyze_samples_rsp_pkt_ptr->ftm_error_code= FTM_IQ_GENERAL_FAILURE;  
  }
  
  if(iq_capture_data_ptr->iq_buf == NULL)
  {
    FTM_MSG_ERROR( "IQ capture buffer is NULL",0,0,0);      
    analyze_samples_rsp_pkt_ptr->ftm_error_code = FTM_IQ_IQ_SAMPLES_UNAVAILABLE;
  }
  
  if(iq_capture_data_ptr->device != device)
  {
	FTM_MSG_ERROR( "IQ samples are not acquired for this device: %d, please acquire them first",device,0,0);	   
	analyze_samples_rsp_pkt_ptr->ftm_error_code = FTM_IQ_IQ_SAMPLES_UNAVAILABLE;
  }
  
  if(analyze_samples_rsp_pkt_ptr->ftm_error_code == FTM_IQ_SUCCESS)  
  {    
    ...
    enable_estimated_sensitivity = (analysis_id & FTM_IQ_CAPTURE_MEASURE_ESTIMATED_SENSITIVITY)? TRUE: FALSE;
      ...
      {
        ftm_common_analyze_iq_samples(&sens_est_result, 
                                      iq_capture_data_ptr->iq_buf, 
                                      sample_size,
                                      cmd_ptr_pkt_for_get_config.sampling_freq,
                                      tech,
                                      dl_pwr_lvl);
        ...
        FTM_MSG_3(FTM_HIGH, "ftm_1x_iq_capture_analyze_samples_handler - "
	...
	analyze_samples_rsp_pkt_ptr->payload_size =  payload_size; 
    memscpy(analyze_samples_rsp_pkt_ptr->analysis_result,payload_size,(int8 *)analysis_result,payload_size);
	rsp_pkt_ptr->pkt_len = FTM_IQ_CAPTURE_ANALYZE_SAMPLE_HDR_SIZE + payload_size; 

	FTM_MSG_8(FTM_MED, "Done processing IQ samples - Status = %d and Analysis ID = %d and "
			  "EnableCToN = %d and EnableEst.Sens = %d,payload_size:%d %d %d %d",
              analyze_samples_rsp_pkt_ptr->ftm_error_code, analysis_id,
			  enable_cton, enable_estimated_sensitivity,payload_size,analysis_result[0],analysis_result[1],analysis_result[2]); 

  }  
  else
  {
	rsp_pkt_ptr->pkt_len = FTM_IQ_CAPTURE_ANALYZE_SAMPLE_HDR_SIZE;  
    FTM_MSG_2(FTM_ERROR,"Unable to capture IQ samples for CDMA for sample size %d status: %d",
		      ftm_req_data->sample_size,analyze_samples_rsp_pkt_ptr->ftm_error_code);
  }
...
)
{
  ...
  rsp_pkt_paylaod->subsys_cmd_code = ftm_chain ;
...
FUNCTION FTM_1X_CREATE_IQ_CAPTURE_analyze_SAMPLES_RSP_DIAG_PKT
...
void ftm_1x_create_iq_capture_analyze_samples_rsp_diag_pkt
(
  diagpkt_subsys_cmd_code_type ftm_chain,
  ftm_rsp_pkt_type *rsp_pkt
)
{
  // Get pointer to response packet
  ftm_iq_capture_analyze_samples_rsp_pkt_type *rsp_pkt_paylaod;

  rsp_pkt_paylaod=&(ftm_cdma_get_rsp_pkt_data_ptr()->iq_capture_analyze_rsp);
  ...
  rsp_pkt_paylaod->subsys_id = FTM;  
  rsp_pkt_paylaod->subsys_cmd_code = ftm_chain;
  rsp_pkt_paylaod->ftm_rf_cmd = FTM_IQ_CAPTURE_ANALYZE_SAMPLES;  
}
...
)
{
  ...
  (void)ftm_cdma_data_get_device_status( device, &rx_dev_state );
  sys_band = rf_cdma_rf_band_type_to_sys_band_type( rx_dev_state.curr_band);
  params.tx_device = rfc_common_get_preferred_tx_device ( sys_band );

  rf_time_tick_type prof_tick = rf_time_get_tick();
  uint8 selftest_error = ftm_common_selftest_main(params, NULL, rsp_pkt_ptr);

  if (selftest_error == FTM_FBRX_TEST_SUCCESS)
  {
    rf_time_type prof_time = rf_time_get_elapsed(prof_tick, RF_MSEC);
    FTM_MSG_1(FTM_HIGH, "Selftest Time profiling 1x ExecTime %d ms", prof_time);
  }
...
/*!
  @brief
  API to generate single tone at a frequency offset from the PLL freq
 
  @details
  This function is used to configure front end and baseband to transmit a tone
  at the requested frequency offset. I
 
  - wakeup Tx
  - Call the FED API for RF ON in immediate mode
  - Call MSM API to generate a tone
 
  Associated FTM Command :
  -# <b>FTM_TX_ARB_CW_TONE</b>
 
  @param device
  RF Device on which Tx is to be turned on or off
 
  @param Freq offset
  The frequency offset requested
 
  @return
  Status of execution completion of the function execution
*/
boolean
ftm_1x_control_gen_cw_tone
(
  rfm_device_enum_type device,
  int32 freq_offset
)
{
  ...
  if(ftm_mode != FTM_MODE)
  {
    FTM_MSG_1( FTM_ERROR, "ftm_1x_control_gen_cw_tone: Set Tx ON failed: "
                          "Not in FTM Mode- %d", ftm_mode ); 
    return FTM_CDMA_BAD_RSP; /* Failure */
  }
  ...
  if ( curr_mode != RFM_1X_MODE )
  {
    FTM_MSG_2( FTM_ERROR, "ftm_1x_control_gen_cw_tone:Set Tx ON failed: Invalid "
                          "Mode-%d on device %d", curr_mode, device );
    return FTM_CDMA_BAD_RSP; /* Failure */
  }
  ...
  {
    FTM_MSG(FTM_ERROR, "ftm_1x_control_gen_cw_tone: Valid Tx device not found - "
                       "bailing");
    return FTM_CDMA_BAD_RSP;
  }
  ...
  */
  if (next_rf_state == dev_state.rf_state)
  {
    FTM_MSG_2( FTM_HIGH, "ftm_1x_control_gen_cw_tone: No operations needed," 
               "Already in State=%d on device=%d", next_rf_state, device );
    return FTM_CDMA_GOOD_RSP;
  }
  ...
  /* Step 3: Enable RF and call MSM API to generate tone */
  if ( rfm_ret >= 0 )
  { 
    rflm_c2k_tx_rf_on_ctrl_in_t rflm_input_param;
    rflm_c2k_tx_rf_on_ctrl_out_t rflm_output_param;

    /*-----------------------RF ON ----------------------------*/
    rflm_input_param.immediate_flag = TRUE;
    rflm_input_param.rf_on = TRUE;

    rflm_c2k_tx_rf_on_ctrl( dev_state.txlm_handle,
                         &rflm_input_param,
                         &rflm_output_param );

    FTM_MSG_1(FTM_MED, "ftm_1x_control_gen_cw_tone: call tone gen msm for freq "
           "offset: %d Hz", freq_offset ); 

    /*-----------------------Generate Tone---------------------*/
    msm_ret_val = ftm_1x_msm_gen_cw_tone( freq_offset ) ;

  }
  else
  {
    FTM_MSG_1( FTM_ERROR, "ftm_1x_control_gen_cw_tone: rfm_1x_wakeup_tx "
                          "failed for Device %d", device );
  }  

  /* A rfm_ret that is less than 0 indicates that RFM API(s) failed */
  if (( rfm_ret < 0 )|| (msm_ret_val == FALSE))
  {
    ret_val = FALSE;
  }
  else
...
/*!
  @brief
  Wrapper API for ftm tx wakeup to support rx\tx split arhitecture 
 
  @details
  This funciton will allocate the txlm buffer with the added vote for current 
  device and calls the rfm tx wakeup API.
 
  @param device
  RF Device for which txlm buffer allocation and tx wakeup has to be done.
*/
...
)
{
  ...
  rfm_wait_time_t ftm_ret;
  ftm_cdma_data_status_type *tx_device_status;  
  lm_status_type tx_buffer_allocated = LM_INVALID_PARAMETER;

  /* call the ftm txlm buffer allocation API */
  tx_buffer_allocated = ftm_1x_vote_txlm_buffer(tx_device);

  /* Get the tx device status pointer */
  tx_device_status = ftm_cdma_data_get_device_ptr(tx_device);

  /* Call the rfm tx waekup API only if the txlm buffer allocation passed */  
  if ( tx_buffer_allocated == LM_SUCCESS && tx_device_status != NULL )
  {
    ftm_ret = rfm_1x_wakeup_tx_v2(tx_device, 
  	                          rx_device, 
		  		  tx_device_status->txlm_handle, 
				  callback, 
				  userdata);
  ...
  {
    FTM_MSG(FTM_ERROR, "ftm_1x_wakeup_tx_v2: Buffer allocation failed"
						" or tx_device_status is NULL");
    ftm_ret = -1;
  }
...
/*!
  @brief
  Wrapper API for ftm tx sleep to support rx\tx split arhitecture 
 
  @details
  This funciton will de-allocate the txlm buffer with the added unvote for 
  current device and calls the rfm tx sleep API.
 
  @param device
  RF Device for which txlm buffer deallocation and tx sleep has to be done.
*/
...
)
{
  ...
  /* Call the rfm sleep tx API to put the device in Rx mode */
  ftm_ret = rfm_1x_sleep_tx(device, 
		  	    txlm_handle, 
			    callback, 
                            userdata );

  /* De-allocate the txlm buffer allocated for this device */
  tx_buffer_deallocated = ftm_1x_unvote_txlm_buffer(device);
  
  if (tx_buffer_deallocated != LM_SUCCESS)
  {
    FTM_MSG(FTM_ERROR, "ftm_1x_sleep_tx: Buffer deallocation failed");
	ftm_ret = -1;
  }
