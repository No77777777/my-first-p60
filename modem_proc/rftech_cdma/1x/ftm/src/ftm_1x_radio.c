...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/1x/ftm/src/ftm_1x_radio.c#1 $
...
06/01/15   zhw     XPT Char fix - use rfm_1x_retune_v2
05/25/15   vc      Exit the rfmode in calv3 instead of tear down.
04/02/15   zhw     Bug fixes for retune_v2 support in radio test interface
04/02/15   zhw     Add support for retune_v2 in radio test interface
03/30/15   zhw     Fix radio_wakeup_tx. Use correct device status to get handle
                   Remove set_pa_access_probe_config before wakeup_tx since
                   Tx device is not yet ready   
03/26/15   wwl     Change wakeup TX command handler to call V2 API function
...
*/
typedef ftm_cdma_req_18_type ftm_1x_radio_tune_req_type;
...
*/
typedef ftm_cdma_req_17_type ftm_1x_radio_wakeup_tx_req_type;
...
)
{
  ...
  lm_handle_type rxlm_handle;
  ftm_cdma_data_status_type dev_status;
  ftm_cdma_data_status_type tx_dev_status;
  ftm_1x_rfm_callback_data_type cb_payload;
  ...
  /* Extract Useful Data */
  tx_device = (rfm_device_enum_type)header->tx_device;
  rx_device = (rfm_device_enum_type)header->rx_device;

  if ( rx_device >= RFM_MAX_DEVICES )
  {
    FTM_MSG_1( FTM_ERROR, "ftm_1x_radio_retune: Unsupported device %d", rx_device );
    ftm_rsp_data.cmd = FTM_RSP_BAD_PARM;
    return ftm_rsp_data;
  }
  ...
  /* Get RF Device Status */
  ftm_cdma_data_get_device_status( rx_device, &dev_status );
  rxlm_handle = dev_status.rxlm_handle;
  
  ftm_cdma_data_get_device_status( tx_device, &tx_dev_status );

  /* Call RFM Function to Wakeup Rx */
  cb_payload.device = rx_device; /* Populate the CB Payload */
  ...
  ftm_concurrency_manager_cleanup_device_allocation(rx_device, RFM_1X_MODE); 
  /* has an associate device*/ 
  if (dev_status.assoc_dev != RFM_INVALID_DEVICE )  
  { 
     /* clean up associated device from concurrency manager before retune */ 
     ftm_concurrency_manager_cleanup_device_allocation(dev_status.assoc_dev, RFM_1X_MODE); 

     /* validate new band for associated device*/ 
     if ( ftm_cdma_control_is_tune_allowed( RFM_1X_MODE, dev_status.assoc_dev,  
                       band_chan.band)) 
     { 
     ...
     { 
        ...
        rf_status = RFM_CDMA_ERROR_FAILURE; 
        FTM_MSG_3( FTM_ERROR, "ftm_1x_radio_retune: " 
                     "Concurrency manager returned failure for" 
                     " tech:%d for associate device %d on band:%d", 
                     RFM_1X_MODE, dev_status.assoc_dev,  
                     band_chan.band); 
     } 
  } 
  else
  ...
  if ( ftm_cdma_control_is_tune_allowed( RFM_1X_MODE, rx_device,  
                        band_chan.band) && (validation_flag == TRUE))  
  {
    rf_status = (rfm_1x_retune_v2(rx_device, tx_device, 
                                rxlm_handle,
  ...
  else
  {
    ...
    band-chan info */
    ftm_cdma_data_update_band_chan( rx_device, 
                          rf_cdma_sys_band_type_to_rf_band_type(band_chan.band),
                          (rfm_cdma_chan_type )band_chan.chan_num );  
  }
  
  FTM_MSG_4( FTM_LOW, "ftm_1x_radio_retune: Dev %d, Band %d, Chan %d, "\
             " RxLM %d : [Done]", rx_device, band_chan.band, band_chan.chan_num,\
             rxlm_handle );
...
)
{
  ...
  */
  ...
  if (rsp_for_1x_enter_mode.cmd != FTM_RSP_BAD_PARM)
  {
   ...
   if ( ftm_cdma_control_is_tune_allowed( RFM_1X_MODE, device_s, 
   ...
   {
      rf_status = RFM_CDMA_ERROR_FAILURE;
      FTM_MSG_3 ( FTM_ERROR , "ftm_1x_radio_enable_diversity: "
                   "Concurrency manager returned failure for"
                   " tech:%d for diversity device:%d and band:%d ",
                   RFM_1X_MODE, device_s ,
                  dev_status_m.curr_band) ;
   }
...
)
{
  ...
  ftm_cdma_data_status_type device_prx_status;
  ...
  /* Extract Useful Data */
  tx_device = (rfm_device_enum_type)header->tx_device;
  prx_device = (rfm_device_enum_type)header->prx_device;
  if ( tx_device >= RFM_MAX_DEVICES )
  {
    FTM_MSG_1( FTM_ERROR, "ftm_1x_radio_wakeup_tx: Unsupported device %d", tx_device );
    ftm_rsp_data.cmd = FTM_RSP_BAD_PARM;
    return ftm_rsp_data;
  }

  /* get a pointer to ftm_cdma_data */
  if (ftm_cdma_data_get_device_status( tx_device, &device_status))
  {
    ...
    rf_status = ftm_1x_wakeup_tx_v2( tx_device, 
                                     prx_device, 
                                     &ftm_1x_rfm_callback, 
                                     &cb_payload );
  ...
  {
    /* Update FTM Data Structure with current band and chan */
    ftm_cdma_data_update_rf_state ( tx_device, FTM_CDMA_STATE_RXTX );
    ftm_cdma_data_update_rf_state ( prx_device, FTM_CDMA_STATE_RXTX );
    if(ftm_cdma_data_get_device_status( prx_device, &device_prx_status))
    {
      /* Update FTM Data Structure with current band and chan */
      ftm_cdma_data_update_band_chan( tx_device, 
                           device_prx_status.curr_band,
                           device_prx_status.curr_chan );

    } 
  }

  FTM_MSG_2( FTM_LOW, "ftm_1x_radio_wakeup_tx: Dev %d, TxLM %d : [Done]",\
             tx_device, device_status.txlm_handle );
...
)
{
  ...
  if (ftm_cdma_data_get_device_status( device, &dev_status ))
  {
    ...
    cb_payload.device = device; /* Populate the CB Payload */
    rf_status = ftm_1x_sleep_tx( device, 
...
)
{
  ...
  /* Call RFM Function to set Tx power */
    rf_status = rfm_1x_ftm_set_tx_agc_override_immediate( device, dev_ptr->curr_band,
                                                   pa_state, avga_idx,
                                                   betap_gain, pa_bias, 
                                                   pa_q_current, 
                                                   pa_q_current_enable );
      ...
      if ( m_device != RFM_INVALID_DEVICE )
      {
        ...
        else
        {
          ...
          if ( rfm_ret != FALSE )
          {
            ...
            be set by the QPOET */ 
            rfm_ret &= rfm_1x_ftm_set_tx_agc_override_immediate( tx_device, header->band,
                                             RFM_PA_GAIN_STATE_0,
                                             rgi_min, 0,/*betaP*/ 
           ...
           {
             FTM_MSG(FTM_ERROR,"ftm_1x_calv3_configure_radio: Null tx dev status"
                               "PTR!");
           }
  ...
  if ( m_dev_mask != 0 )
  {
    ...
    while( m_dev_mask != 0 )
    {
      ...
      if ( m_device != RFM_INVALID_DEVICE )
      {
        ...
        from the PA driver, this is the minimum possible bias voltage that can 
        be set by the QPOET */ 
        rfm_ret &= rfm_1x_ftm_set_tx_agc_override_immediate( m_device,
        ...
        tx_device = rfc_common_get_preferred_tx_device ( band_chan.band );  


        rfm_ret &= (rfm_1x_retune_v2( m_device, tx_device, 
                                   m_dev_status->rxlm_handle,
                                   band_chan, &ftm_1x_rfm_callback, 
                                   &cb_payload ) >= 0 );
...
)
{
  ...
  for ( device_idx = RFM_DEVICE_0; device_idx < RFM_MAX_DEVICES; device_idx++ )
  {
    ...
    if ( curr_mode == RFM_1X_MODE )
    {
      ...
      if ( ftm_dev_status != NULL )
      {
        ...
        if ( dev_status != NULL )
        {
          ...
          if ( dev_status->data_1x.rf_state == RF_CDMA_STATE_RXTX ||
               dev_status->data_1x.rf_state == RF_CDMA_STATE_TX)
          {
      ...
      /* Tear down Radio */
      ftm_rfmode_exit(device_idx, FTM_STATE_PARK);
