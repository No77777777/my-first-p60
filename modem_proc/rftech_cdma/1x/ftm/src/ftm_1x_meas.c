...
$Header: //components/rel/rftech_cdma.mpss/3.11/1x/ftm/src/ftm_1x_meas.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/01/15   zhw     Fix get_tech_param() bug. Add FW control during tgt enter
...
static rfm_meas_tech_setup_params_type ftm_1x_meas_param;
...
)
{
  ...
  test_param_ptr->src_ca_path0_prx_rxlm_buf_idx = device_status->rxlm_handle;
...
)
{
  ...
  else if (test_param_ptr->src_tech == RFM_1X_MODE)
  {
    ...
    ftm_1x_meas_enter_param.band_chan.band = test_param_ptr->src_ca_path0_band ;
    ftm_1x_meas_enter_param.band_chan.chan_num = test_param_ptr->src_ca_path0_chan_car0;
...
)
{
     ...
     (test_param_ptr->tgt_tech == RFM_1X_MODE))
  {
    ...
    if(test_param_ptr->src_tech == RFM_1X_MODE)
    {      
      ftm_1x_meas_setup_param.band_chan.band = test_param_ptr->src_ca_path0_band ;
      ftm_1x_meas_setup_param.band_chan.chan_num = test_param_ptr->src_ca_path0_chan_car0;
    }
    else
    {
...
/*!
  ...
  API will return NULL pointer in case of failure
*/
static rfm_meas_tech_setup_params_type
ftm_1x_meas_get_tech_param
(
  rfm_meas_generic_params_type *test_param_ptr,
  boolean is_src
)
{
  ...
  {
    FTM_MSG(FTM_ERROR , 
        "ftm_1x_meas_get_tech_param : test param pointer is null ");
    return ftm_1x_meas_param;
  }
     ...
     (test_param_ptr->tgt_tech == RFM_1X_MODE))
  {
    ...
    if(test_param_ptr->src_tech == RFM_1X_MODE)
    {
      ftm_1x_meas_param.onex_params.band_chan.band = test_param_ptr->src_ca_path0_band;     
      ftm_1x_meas_param.onex_params.band_chan.chan_num = test_param_ptr->src_ca_path0_chan_car0;       
    }
    else
    {
      /* Call API to enable FW in MEAS state, when 1x is target */
      ftm_1x_meas_fw_on_off_control( ftm_1x_meas_get_current_device(test_param_ptr), 
                                     RFM_CDMA_FW_STATE_MEAS );
      ftm_1x_meas_param.onex_params.band_chan.band = test_param_ptr->tgt_band;     
      ftm_1x_meas_param.onex_params.band_chan.chan_num = test_param_ptr->tgt_chan;       
    }
  }
  /* 1x is not source or target, throw error message */
  else
  {
    /* Throw error message */
    FTM_MSG_2(FTM_ERROR , 
          "ftm_1x_meas_get_tech_param: ERROR :src tech %d and tgt tech %d", 
          test_param_ptr->src_tech, test_param_ptr->tgt_tech);
  }
...
)
{
  ...
  {
    /* 1x is source tech */
    ftm_1x_meas_exit_param.band_chan.band = test_param_ptr->src_ca_path0_band ;
    ftm_1x_meas_exit_param.band_chan.chan_num = test_param_ptr->src_ca_path0_chan_car0;
    /* cast to void pointer */
...
)
{
         ...
         (test_param_ptr->src_tech == RFM_1X_MODE) )
  {
    ...
    rfm_status = rfm_1x_sleep_rx( ftm_1x_meas_get_current_device(test_param_ptr), 
                                  test_param_ptr->src_ca_path0_prx_rxlm_buf_idx, 
                                  &ftm_1x_rfm_callback,
                                  &cb_payload );
