...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/1x/ftm/src/ftm_1x_calibration.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
03/09/18   mk      KW Errors fix
01/27/15   pk      Apply dynamic ADC settings as part of FBRX AGC setup
01/27/15   pk      FBRX Gain Cal fix
12/09/15   pk      Added support for CDMA rx\tx split
11/02/15   rs      Added xpt capture support for non-DRx devices
08/12/15   zhw     FBRX Gain Cal fix
...
07/24/15   wwl     Change to use TX device to get feedback modem chain
07/29/15   aa      Fix FBRx error status check 
07/09/15   zhw     KW Fix
06/01/15   zhw     KW Fix
05/27/15   zhw     KW Fix
05/11/15   zhw     Added F3 to identify invalid device during Cal.
04/09/15   zhw     Remap Cal input Rx device based on new RFC device mapping
03/30/15   zhw     Support split Tx device in RF Cal
...
12/17/14   wwl     change to search the FBRx modem chain value via primary device ID
...
)
{
  ...
  if ( ret_val == TRUE )
  {
    ...
    while ( rx_flag_mask != 0 )
    {
      ...
      if ( device != RFM_INVALID_DEVICE )
      {
        ...
        rx_device = ftm_cdma_get_remapped_rx_device( device );

        /* Get Calibration Data */
        cal_data = ftm_cdma_data_get_cal_v3_adr(rx_device);
        ...
        if ( cal_data != NULL )
        {
          ...
          if ( cal_data->curr_power_mode != rx_config->power_mode )
          {
            ...
            rfm_cdma_ftm_power_mode_override( rx_device, rx_config->power_mode );
            cal_data->curr_power_mode = rx_config->power_mode;
          }
      
          /* Setup LNA to appropriate LNA State with given LNA offset. */
          rfm_1x_ftm_setup_lna_calibration( rx_device,
                                            gain_state, 
                                            lna_gain_prev_state,
          ...
          {
            FTM_MSG_3( FTM_MED,"ftm_1x_calibration_sweep_configure_rx - Dev %d LNA %d "
                       "LNAOffset %d", rx_device, gain_state, lna_gain_prev_state );
          } /* if ( cal_data->debug_en == TRUE ) */
        }/* if ( cal_data != NULL ) */
      } /* if ( device != RFM_INVALID_DEVICE ) */
  ...
  {
    FTM_MSG_2( FTM_ERROR, "Cal v3 ftm_1x_calibration_sweep_configure_rx: Failed - "
  ...
  if ( ( ret_val == TRUE ) && ( dev_determined == TRUE ) )
  {
    ...
    if (dev_ptr == NULL)
    {
      FTM_MSG_1( FTM_ERROR, "Cal v3 ftm_1x_calibration_sweep_configure_tx: "
                 "Failed to get a valid device pointer for Tx device %d", tx_device );
      ret_val = FALSE;
    }
    else
    {
      ...
      if ( tx_config->rgi != FTM_CALIBRATION_INVALID_RGI )
      {
        ...
        ret_val &= rfm_1x_ftm_set_tx_agc_override_immediate( 
                        tx_device, dev_ptr->curr_band, 
                        tx_config->pa_state, 
                        tx_config->rgi, 0, 
      ...
      else
      {
        FTM_MSG_2( FTM_MED, "Cal v3 ftm_1x_calibration_sweep_configure_tx: "
                   "Tx Dev %d skipped Tx RGI %d override", tx_device, 
                   tx_config->rgi );
      } /* if ! ( tx_config->rgi != FTM_CALIBRATION_INVALID_RGI ) */
...
)
{
  ...
  if ( ( ret_val == TRUE ) && ( dev_determined == TRUE ) )
  {
    ...
    {
      /* Get Calibration Data */
      cal_data = ftm_cdma_data_get_cal_v3_adr(tx_device);

      if ( cal_data == NULL )
      {
        ERR_FATAL("NULL Cal data pointer. Critical error",0,0,0);
      }

      rfm_cdma_ftm_setup_hdet(tx_device, RFDEVICE_CDMA_HDET_HPM); 
      /* Allow for PDET settling time after setting up HDET */
      DALSYS_BusyWait( rfm_cdma_ftm_get_hdet_settling_time(tx_device));

      hdet_read = rfm_cdma_ftm_multi_read_hdet( tx_device,
    ...
    {
      rfm_cdma_ftm_setup_hdet( tx_device, RFDEVICE_CDMA_HDET_LPM );
      /* Allow for PDET settling time after setting up HDET */
      DALSYS_BusyWait( rfm_cdma_ftm_get_hdet_settling_time(tx_device));
    ...
    for ( readx = 0; readx < FTM_1X_RXAGC_RD_SAMPLES; readx++ )
    {
      ...
      while ( rx_flag_mask != 0 )
      {
        ...
        if ( device != RFM_INVALID_DEVICE )
        {
          ...
          agc_read_val = rfm_1x_ftm_get_rx_agc( rx_device );
           ...
           // }
            ...
            if ( cal_data->debug_en == TRUE )
            {
              FTM_MSG_7( FTM_MED,"ftm_1x_calibration_sweep_measure_rx - "
                     ...
                     "(in 1/10th : %d)", rx_device, gain_state, agc_avg_val, 
        ...
        {
          FTM_MSG( FTM_ERROR,"ftm_1x_calibration_sweep_measure_rx: Invalid Device!" );
        }
...
{
  boolean ret_val = TRUE;
  rfm_device_enum_type device_remap;    /* Var to hold primary device id */
  ftm_cdma_data_status_type dev_state;  /* Var to hold current state */

  rfm_device_enum_type device_remap_fb;    /* Var to hold feedback device id */
  ftm_cdma_data_status_type dev_fb_state; /* Var to hold current state */

  rfm_device_enum_type device_remap_tx;     /* Var to hold Tx device id */
  ...
  /*!!! To support splitted Tx and Rx device without changing Cal tool intf. 
    ...
    converted to associated Tx device based on RFC. This is valid until tool
    interface is updated to pass in both Rx & Tx devices*/
  device_remap = ftm_cdma_get_remapped_rx_device( feedback_setup->primary_device );
  device_remap_fb = ftm_cdma_get_remapped_rx_device( feedback_setup->feedback_device );

  (void)ftm_cdma_data_get_device_status( device_remap_fb, &dev_fb_state );
  ...
  device_remap_tx = rfc_common_get_preferred_tx_device  ( sys_band );

  /* Configure feedback ASM */
  ret_val &= rfm_cdma_ftm_config_fb_asm( device_remap_tx,
                                         dev_state.curr_band,
                                         TRUE);
                          ...
                          FEEDBACK_SETUP_ACTION_MASK_CONFIGURE_WTR )
  {
     ...
     {
        ret_val &= rfm_cdma_ftm_deconfig_fb_transceiver( device_remap_tx,
                                                         device_remap_fb, 
                                                     RFM_1X_MODE );
     }
  }
                          ...
                          FEEDBACK_SETUP_ACTION_MASK_CONFIGURE_RXLM_FOR_XPT )
  {
    ...
    ftm_rfmode_enter( device_remap_fb, FTM_STATE_1X );

    /* To support no-Drx devices we are passing xpt_fb_handle to 
    rfm_1x_ftm_config_fb_rxlm taken from Prx device. so in order not 
    to break the existing funcltionality on MTP we are assigning 
    the xpt_fb_handle from drx to prx device also */
    if (&dev_fb_state != NULL)
    {
      dev_state.xpt_fb_handle = dev_fb_state.xpt_fb_handle;
    }
    /* Configure feedback RxLM */
    ret_val &= rfm_1x_ftm_config_fb_rxlm( device_remap_tx,
                                          device_remap_fb, 
                          ...
                          FEEDBACK_SETUP_ACTION_MASK_CONFIGURE_RXLM_FOR_FBAGC )
  {
    ...
    ret_value = rfm_1x_ftm_config_rxlm_for_fbagc( device_remap_tx, 
                                                  dev_state.curr_band );
...
)
{
  ...
  ftm_cdma_data_set_band_mapping(mdev, band);
  ftm_cdma_data_set_band_mapping(sdev, band);

  mdev = ftm_cdma_get_remapped_rx_device( mdev );
  sdev = ftm_cdma_get_remapped_rx_device( sdev );
       ...
       ( mdev_state < MASTER_DEVICE_STATE_MAX) )
  {
    ...
    else
    {
                ...
                ( mdev_state == MASTER_DEVICE_RXTX ) )
      {
        ...
        if ( rfm_ret != FALSE )
        {
          ...
          */    
          ...
          {
            rfm_device_enum_type tx_device; /* Tx device */

            ftm_cdma_data_status_type *tx_dev_status = NULL; /* mDevice Status pointer */
            ...
            {
              FTM_MSG( FTM_ERROR, "ftm_1x_calibration_radio_setup: NULL Tx Device"
                                  "status PTR" );
              rfm_ret = FALSE;
            }
            ...
            /* Enable HDET */
            rfm_ret &= ( rfm_cdma_ftm_enable_hdet( tx_device ) == 1 );
            ...
            if ( rfm_ret == TRUE )
            {
              ...
              tx_dev_status->curr_band = band;
              tx_dev_status->curr_chan = channel;
              tx_dev_status->assoc_dev = RFM_INVALID_DEVICE;
              tx_dev_status->rf_state = FTM_CDMA_STATE_RXTX;
            } /* if ( rfm_ret == TRUE ) */
            else
            {
              FTM_MSG_1( FTM_ERROR, "ftm_1x_calibration_radio_setup: FTM data "
      ...
      else if ( mdev_state == MASTER_DEVICE_RETUNE )
      {
        ...
        rfm_ret &= rfm_cdma_ftm_get_min_max_avga_idx( tx_device, &rgi_min,
                                                      &rgi_max);
        ...
        be set by the QPOET */ 
        rfm_ret &= rfm_1x_ftm_set_tx_agc_override_immediate( tx_device,
        ...
        /* Disable HDET */
        rfm_ret &= ( rfm_cdma_ftm_disable_hdet( tx_device ) == 1 );

        cb_payload.device = mdev; /* Populate the CB Payload */

        rfm_ret &= (rfm_1x_retune_v2( mdev, tx_device, 
                                      mdev_status->rxlm_handle,
                                   band_chan, &ftm_1x_rfm_callback, 
                                   &cb_payload ) >= 0 );
...
)
{
  ...
  (void)ftm_cdma_data_get_device_status( tx_device, &dev_state );
  ...
  if( input_override_row->curr_tx_override.xpt_mode == RFCOMMON_MDSP_XPT_MODE_ET )
  {
   temp_vs_freq_params.ref_xpt_pin_offset = 
     (int32)rf_cdma_xpt_get_cfg_param( tx_device, XPT_CFG_CMN_REF_ET_PIN_OFFSET );
  }
  else if( input_override_row->curr_tx_override.xpt_mode  == RFCOMMON_MDSP_XPT_MODE_EPT )
  {
   temp_vs_freq_params.ref_xpt_pin_offset = 
     (int32)rf_cdma_xpt_get_cfg_param( tx_device, XPT_CFG_CMN_REF_EPT_PIN_OFFSET );
  }
...
)
{
  ...
  while ( dev_mask != 0 )
  {
    ...
    {
      rfm_device_enum_type tx_device; /* Tx device */
      rfm_device_enum_type rx_device; /* Tx device */
     ...
     sys_band = rf_cdma_rf_band_type_to_sys_band_type(rx_dev_state.curr_band);
      ...
      if (dev_ptr == NULL)
      {
        FTM_MSG_1( FTM_ERROR, "ftm_1x_calibration_v3_tx_override: "
                              ...
                              "device pointer for Tx device %d", 
                              tx_device );
        return FALSE;
      }
      ...
      if ( rfcommon_debug_flags.use_rflm_txagc_override )
      {
        ...
        ret_val &= rfm_1x_ftm_process_tx_override_params( tx_device,
      ...
      {
      ret_val &= rfm_1x_ftm_set_rf_tx_agc_override( tx_device,
                        override_row.curr_tx_override.xpt_mode,
                        override_row.curr_tx_override.amam_addr.mem_bank_id,
                        override_row.curr_tx_override.amam_addr.start_offset,
                        override_row.curr_tx_override.amam_addr.tbl_size,
    ...
    while ( dev_mask != 0 )
    {
      ...
      if ( device < FTM_CAL_V3_RX_MAX_DEV )
      {
             ...
             ( 1 << RX_MEAS_LNA_OFFSET ) ) != 0 )
        {
          ...
          /* Get RxAGC reading for the current device in raw format */
          rfm_1x_ftm_get_rx_agc_data( rx_device, &rx_agc_data,
         ...
         // }
          ...
          if ( cal_data->debug_en == TRUE )
          {
            FTM_MSG_3( FTM_MED, "Cal v3 ftm_1x_calibration_sweep_measure_rx - "
                       "RxAGC Sampling Dev %d, AGC %d (for Itr %d)", rx_device, 
                       agc_read_val, readx );
          } /* if ( cal_data->debug_en == TRUE ) */
          ...
          if ( readx == (FTM_1X_RXAGC_RD_SAMPLES - 1) )
          {
            ...
            else
            {
              ...
              FTM_MSG_3( FTM_ERROR, "ftm_1x_calibration_v3_rx_measure: "
                         "Bad GS or PM (Dev %d  Gain %d PM %d)", rx_device, 
...
)
{
  ...
  while ( dev_mask != 0 )
  {
    ...
    if ( device < FTM_CAL_V3_TX_MAX_DEV )
    {
      ...
      rfm_device_enum_type tx_device; /* Tx device */
      rfm_device_enum_type rx_device; /* Rx device */
           ...
           ( 1 << TX_MEAS_HDET ) ) != 0 )
      {
        ...
        if ( hdet_sample > 0 )
        {
          ...
          hdet_read = rfm_cdma_ftm_multi_read_hdet( tx_device, hdet_sample );
           ...
           ( 1 << TX_MEAS_LINEARIZER_THERM ) ) != 0 )
      {
        ...
        therm_read_valid = rfm_cdma_ftm_get_therm_read_in_cal( tx_device, 
                                                               &therm_read );
           ...
           ( 1 << TX_MEAS_FB_GAIN ) ) != 0 )
      {
        ...
        fb_meas_inp.tx_device_id = tx_device;
        ...
        fb_meas_inp.band.cdma_band = rf_cdma_data_get_curr_band((rfm_device_enum_type)tx_device);
        ...
        if ( fb_meas_res.status == RFCOMMON_FBRX_ERROR )
        {
          ...
          FTM_MSG_1( FTM_HIGH, "ftm_1x_calibration_v3_tx_measure: Dev %d, FB Gain Measurement "
                                "returned from FW is invalid, re-do capture", tx_device );
        }
        ...
        if ( fb_meas_res.status != RFCOMMON_FBRX_ERROR )
        {
          ...
          cal_result->tx_meas_param[device].fb_gain_meas.fb_meas_res.fbrx_mode = ftm_common_fbrx_get_mode(RFCOM_1X_MODE ,device, 0);
           ...
           ( 1 << TX_MEAS_FB_PWR ) ) != 0 )
      {
        ...
        {
          rfm_cdma_ftm_setup_hdet( tx_device, 
                tx_measure->tx_meas_param[device].fb_power.fp_power_params.fb_gain_state );

          /* Allow for PDET settling time after setting up HDET */
          DALSYS_BusyWait( rfm_cdma_ftm_get_hdet_settling_time(tx_device));

          hdet_read = rfm_cdma_ftm_multi_read_hdet( tx_device, fb_power_samples );
           ...
           ( 1 << TX_MEAS_FB_GAIN_AGC ) ) != 0 )
      {
        ...
        /* Fetch Input Params for FBRx capture */
		fb_meas_inp_agc.tx_device_id = tx_device;
        ...
        fb_meas_inp_agc.tech = RFCOM_1X_MODE;
        fb_meas_inp_agc.band.cdma_band = rf_cdma_data_get_curr_band((rfm_device_enum_type)tx_device);

  
        /* Call common FB Gain Agc Measurement function */
        fb_meas_res_agc = ftm_common_fbrx_capture_and_process_v2(fb_meas_inp_agc);

        if ( tx_measure->debug_mode_en == TRUE )
        {
		  FTM_MSG_9( FTM_MED, "ftm_1x_calibration_v3_tx_measure - FB Gain Cal Res: Status %d , Gain = %d,"
...
)
{
  ...
  else
  {
    ...
    /*!!! To support splitted Tx and Rx device without changing Cal tool intf. 
      ...
      converted to associated Tx device based on RFC. This is valid until tool
      interface is updated to pass in both Rx & Tx devices*/
    device_remap = ftm_cdma_get_remapped_rx_device( payload->primary_device );
    ...
    /* Perform EPT Configure. Pass tool input to FTM API */
    ret_val = ftm_1x_xpt_cal_config( payload->primary_device, 
...
)
{
  ...
  ftm_cdma_data_status_type rx_dev_state;  /* Var to hold current state */
  boolean ret_val = TRUE;
  rf_time_tick_type prof_cal_func; /* Function Profiling Variable */
  rf_time_type prof_cal_func_val = 0; /* Function Profiling Value Variable */
  ...
  else
  {
    ...
    /*!!! To support splitted Tx and Rx device without changing Cal tool intf. 
      ...
      interface is updated to pass in both Rx & Tx devices*/
    device_remap = ftm_cdma_get_remapped_rx_device( payload->primary_device );
    device_fb_remap = ftm_cdma_get_remapped_rx_device( payload->feedback_device );
    ...
    dev_status_r = rf_cdma_get_device_status( device_fb_remap );
    /*Perform NULL pointer check*/
    if ( dev_status_r  == NULL )
    {
      FTM_MSG_1( FTM_ERROR, "ftm_1x_calv3_xpt_iq_capture: dev_status_r [ %d ] "
                            "NULL Data ignore if device being used is no drx device", device_fb_remap );
    } /* if( dev_status_r  == NULL ) */
    ...
    if ( ( rfcommon_xpt_is_capture_done() == TRUE ) && ( ret_val == TRUE ) )
    {
      ...
      if ( buff_alloc == TRUE )
      {
        ...
        {
          FTM_MSG_3(FTM_MED,"ftm_1x_calv3_xpt_iq_capture: Push to DPD Map - "
                            "Chan: %d,  CaptBuf: %d, ResBuf: %d",
                                                 dev_state.curr_chan,
        ...
        /* RxLM buffer must have been set up properly before this and 
        to support no-Drx devices we are passing rxlm_handle of  
        Prx device. Anyhow the handle is not used inside the API so no harm */         
          samp_rate = rf_cdma_mc_get_samp_rate((uint8)dev_state.rxlm_handle);
        ...
        /*Update Pin IQ capture data into Pin map structure for PIN capture type*/
        if (capture_param.capture_type == XPT_CAPTURE_PIN)
        {
        ...
        {
          /*need to initialize the data structure correctly before we store it back to the shared data structure*/
