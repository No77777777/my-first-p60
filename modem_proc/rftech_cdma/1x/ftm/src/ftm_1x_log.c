...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/1x/ftm/src/ftm_1x_log.c#1 $
...
)
{
  ...
  tx_device = rfm_cdma_ftm_log_search_transmit_device(RFM_1X_MODE);
  ...
  {
    rfm_1x_ftm_get_tx_agc_data(tx_device, &tx_agc_data, 
                                RFM_CDMA_AGC_FORMAT__RAW );
  }
  ...
  log_data->onex_log.c0_agc[buf].hdet_raw_v2 = rfm_cdma_ftm_read_hdet( tx_device );
  log_data->onex_log.c0_agc[buf].hdet_raw = 0;
  log_data->onex_log.c0_agc[buf].txagc_from_hdet =
    rfm_cdma_ftm_convert_hdet_to_txagc(log_data->onex_log.c0_agc[buf].hdet_raw_v2,tx_device);
...
)
{
  ...
  log_data->onex_log.c1_agc[buf].hdet_raw_v2 = rfm_cdma_ftm_read_hdet(tx_device);
