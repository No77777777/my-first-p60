...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/common/ftm/inc/ftm_cdma_data.h#1 $
...
03/04/15   pk      Updated the ftm_cdma_rsp_pkt_type with new rsp pkt type for 
                   FTM IQ Caputure analysis
...
typedef struct
{
  ...
  */
  ...
  uint32 txlm_handle_dev_vote;
  /*!< This variable keeps track of vote from each Rx device to allocate 
  TxLM handle in FTM */
...
typedef union
{
  ...
  ftm_iq_capture_analyze_samples_rsp_pkt_type iq_capture_analyze_rsp;
  /*!< RSP packet for FTM_IQ_CAPTURE_ANALYZE_SAMPLES */ 
...
typedef struct
{
  rfm_cdma_band_class_type legacy_input_device_band[RFM_MAX_DEVICES];
  /*!< Book keeping for band passed in by tools using legacy input device. 
    Until tools interfaces are updated, the legacy input device will be
    remapped based on RFC mapping. Hence a seperate band record is kept
    for remapping query purpose*/
