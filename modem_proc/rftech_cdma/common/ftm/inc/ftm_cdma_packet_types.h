...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/common/ftm/inc/ftm_cdma_packet_types.h#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
01/21/19   aa      RxAGC based QXDM Command implementation
03/26/15   wwl     Add new request command type for wakeup TX
...
/*!
  @brief
  FTM CDMA Dispatch Request Packet Template 17. Replace Template 7 for Trx split
*/
typedef PACK(struct)
{
  uint8 cmd;
  ...
  uint8   tx_device;
  /*!< Tx RFM Device (rfm_device_enum_type) Under consideration */

  uint8   prx_device;
  /*!< Rx RFM Device (rfm_device_enum_type) Under consideration */

  uint32 lm_handle;
  /*!< LM Buffer Handle */

} ftm_cdma_req_17_type;


/*----------------------------------------------------------------------------*/
/*!
  @brief
  FTM CDMA Dispatch Request Packet Template 18. Replace template 2 for TRx split
*/
typedef PACK(struct)
{
  uint8 cmd;
  ...
  uint8   tx_device;
  /*!< Tx RFM Device (rfm_device_enum_type)  */
...
} ftm_cdma_req_18_type;

/*----------------------------------------------------------------------------*/
/*!
  @brief
  FTM CDMA Dispatch Request Packet Template 20.
  For example, If you want to crash the UE if only PRx_AGC value goes below 
  -80dBm till 16 Rx_update iterations, then following cmd needs to be passed:
   In HEX: 4B 0B 27 00 08 30 0E 00 0E 00 00 15 10 00
   In DEC: 75 11 39 00 07 48 15 00 15 00 00 80 16 00
*/
typedef PACK(struct)
{
  uint8 cmd;
  /*!< Command Identification. default value = 0x4B = DIAG_SUBSYS_CMD */

  uint8 sub_sys;
  /*!< Sub-system Identification. default value = 0x0B = DIAG_SUBSYS_FTM */

  uint16 mode;
  /*!< RF Mode Identification. default value0b39 = 0x0027(send as 27 00)
      = FTM_MODE_RFM_C */

  uint16 cdma_cmd;
  /*!< FTM Sub Command. default value = 0x12296 (send as 08 48)
     =  ftm_cdma_rxagc_debug (previously, FTM_CDMA_RFM_1X_WAKEUP_RX) */

  uint16 req_len;
  /*!< Length of Diag Request Packet to the Target
   default value = 0x000E (send as 0E 00)*/
  ...
  uint8 chain_type;
  /*!< PRx (0), DRx (1) or Both (2) */

  uint8 rxagc_debug_limit;
  /*!< Absolute RxAGC value in dBm which needs to taken as debug 
         threshold or Delta threshold (based on AGC_Delta_flag).
       If want to set as -70dBm, send  0x46*/

  uint8 cnt;
  /*!< Counter value which will be used to trigger crash once threshold
          is crossed and Cnt number of Rx update is called */

  uint8 agc_delta_flag;
  /*TRUE(1) = force crash UE in case delta value between PRx/DRx AGC 
              crosses some limit
    FALSE(0) = force crash UE based on RxAGC value of chain type*/

} ftm_cdma_req_20_type;
