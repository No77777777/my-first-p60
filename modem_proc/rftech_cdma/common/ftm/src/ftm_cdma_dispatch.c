...
$Header: //components/rel/rftech_cdma.mpss/3.11/common/ftm/src/ftm_cdma_dispatch.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
01/21/19   aa      RxAGC based QXDM Command implementation
03/09/18   mk      KW Errors fix
05/03/16   pk      Fix RFM Device in online mode for DRx
12/09/15   pk      Added support for CDMA rx\tx split
10/20/15   rs      Passing Tx device to hdet read API
08/26/15   zhw     Fixed hard-coded ftm chain in iq capture payload
04/30/15   pl/spa  Support generation of sigle tone waveform
03/16/15   jmf     [self-test] Fix CDMA FBRx Gain switching
08/13/15   yb      Fix tune code override
...
06/10/15   dbc     Add support for FTM_SET_PA_TUNABLE_CAP command
06/11/15   yb      Added support for Get Tune Code API
06/05/15   zhw     FTM Remap solution - FTM dispatch always take remapped Rx device
                   now. Remove remapping done inside FTM dispatch
05/21/15   zhw     XPT Char fix - use wakeup_tx_v2 in ftm_1x_enable_tx
04/09/15   zhw     Update remap FTM RF Rx device
...
03/04/15   pk      Added FTM_IQ_CAPTURE_ANALYZE_SAMPLES support                   
01/16/15   pl      Handle CDMA FTM_SET_TX_POWER_DBM with disable flag 
...
/*!
  @brief
  Helper function for Tx PLL override command

  @details
  This function will call RFM interface to Override the tx PLL to tune to the
  rx channel requested

  @param override_chan
  The rx channel to override the tx PLL to
...
static 
ftm_cdma_rsp_type
ftm_cdma_tx_pll_override
(
  uint16 override_chan
)
{
  boolean ret_val; /* return Value */
  /* Interface does not have device as a parameter right now, use device 0 
     only */ 
  rfm_device_enum_type device = RFM_DEVICE_0;

  /* Call RFM function */
  ret_val = rfm_cdma_ftm_tx_pll_override( device, override_chan );

  FTM_MSG_2( FTM_MED, "ftm_cdma_tx_pll_override : Device %d, override_chan %d", 
             device,
             override_chan );
...
/*===========================================================================
  ...
  FUNCTION:  ftm_cdma_gen_cw_tone

===========================================================================*/
/*!
  @brief
  This functions loads a predefined waveform and transmit a CW tone
...
)
{
  ...
  /* Call RFM function ,hard code device to zero for now */
  ret_val = ftm_1x_control_gen_cw_tone( device , freq_offset);
...
)
{
  ...
  if( enable_flag == TRUE )
  {
    ...
    if ( rf_status == TRUE  )
    {
      ftm_1x_tx_power_dbm = power_level;
  ...
  {
     FTM_MSG_3( FTM_MED, "ftm_1x_set_tx_power_dbm: enable flag is false, "
                         "api funcitonality is skipped Device %d, Enable Flag %d, "
                 "TxPwr(dBm10) %d : [Done]", device, enable_flag, power_level );
     return FTM_CDMA_GOOD_RSP;
  }
...
)
{
  ...
  rfm_device_enum_type tx_device, assigned_device;
  ...
  ftm_diagpkt_subsys_header_type *header = (ftm_diagpkt_subsys_header_type*)ftm; 
  ...
  pre-requisite is rx has to be tuned before calling any tx related
  dispatch commands */
  ...
  {
    RF_MSG( RF_HIGH, "ftm_cdma_dispatch: Rx is not tuned" );
  }
  ...
  case FTM_SET_TX_ON: /* Enable Tx */
      status = ftm_1x_enable_tx( tx_device, device );
    break;

  case FTM_SET_TX_OFF: /* Disable Tx */
      status = ftm_1x_disable_tx( tx_device);
    break;

  case FTM_SET_CDMA_CW_WAVEFORM: /* choose between CDMA or tone */
      status = ftm_1x_set_tx_waveform( tx_device, 
  ...
  case FTM_GET_ADC_VAL:
    rf_req->ftm_rf_factory_data.id_val.val = ftm_1x_read_hdet(tx_device);
  ...
  case FTM_SET_TUNER_TUNE_CODE_OVERRIDE:
    assigned_device = RFM_MAX_WAN_DEVICES;
    rf_state = rf_cdma_data_get_1x_rf_state(device);
    FTM_MSG_2(FTM_HIGH, "device:%d, rf_state:%d", device, rf_state);
    if( ((FTM_1X_C0_C == ftm_chain) && 
          ((RF_CDMA_STATE_RX == rf_state) || (RF_CDMA_STATE_RXTX == rf_state) || (RF_CDMA_STATE_TX == rf_state))) ||
        ((FTM_1X_C1_C == ftm_chain) &&
          (RF_CDMA_STATE_RXDIV == rf_state))
      )
    {
    ...
    else
    {
      if(FTM_1X_C0_C == ftm_chain)
      {
    assigned_device = rfm_1x_ftm_get_active_device (TRUE);
        tx_device = rfc_cdma_get_default_tx_logical_device( assigned_device );
    if(RF_CDMA_STATE_TX == rf_cdma_data_get_1x_rf_state(tx_device))
    {
      assigned_device = tx_device;
    }
      }
#ifdef FEATURE_MULTI_RX_CHAIN
      else if(FTM_1X_C1_C == ftm_chain)
      {
        assigned_device = rfm_1x_ftm_get_active_device (FALSE);
      }
#endif
    }

    FTM_MSG_4( FTM_HIGH, "ftm_cdma_dispatch: device %d, assigned Dev:%d, tx device %d, ftm_chain:%d",
             device, assigned_device, tx_device,ftm_chain);
  ...
  case FTM_GET_TUNE_CODE:
    ftm_rsp_data = ftm_common_process_get_tune_code(ftm,
                                       device,
                                       FTM_STATE_1X,
                                      ftm->rf_params.ftm_rf_factory_data.ftm_tuner_get_tune_code_data.mode,
                                      ftm->rf_params.ftm_rf_factory_data.ftm_tuner_get_tune_code_data.device_type);
    status = FTM_CDMA_NEW_RSP_PKT;
    break;

  case FTM_SET_TX_POWER_DBM:
      status = ftm_1x_set_tx_power_dbm( tx_device,
  ...
  case FTM_SET_TX_PWR_LIMIT:
      status = ftm_1x_set_tx_pwr_limit( tx_device, 
  ...
  case FTM_INT_SET_TX_ON:
    (void)ftm_1x_enable_tx( tx_device, device );
    status = FTM_CDMA_NO_RSP;
    break;
    
  case FTM_INT_SET_TX_OFF:
    (void)ftm_1x_disable_tx( tx_device );
    status = FTM_CDMA_NO_RSP;
    break;

  case FTM_INT_SET_LOWEST_TX_PWR:
    (void)ftm_1x_set_tx_lowest_power( tx_device );
    status = FTM_CDMA_NO_RSP;
    break;
  ...
  case FTM_IQ_CAPTURE_GET_SAMPLES: 
   /* Create response packet and fill in the basic feilds */
    ftm_1x_create_iq_capture_get_samples_rsp_diag_pkt(ftm_chain,&ftm_rsp_data);
   /* Based on the request and sanity checks fill in the rest of the feilds of the response packet */
    ftm_1x_iq_capture_get_samples( device, 
   ...
   case FTM_IQ_CAPTURE_ANALYZE_SAMPLES:
    /* Create response packet and fill in the basic feilds */
    ftm_1x_create_iq_capture_analyze_samples_rsp_diag_pkt(ftm_chain,&ftm_rsp_data);
    /* Based on the request and sanity checks fill in the rest of the feilds of the response packet */
    ftm_1x_iq_capture_analyze_samples( device, 
		    &ftm->rf_params.ftm_rf_factory_data.ftm_iq_capture_cmd.ftm_iq_capture_analyze_samples,
                                       &ftm_rsp_data );    
    status = FTM_CDMA_NEW_RSP_PKT;
    break;
  ...
  case FTM_TX_FREQ_OVERRIDE:
    status = ftm_cdma_tx_pll_override( rf_req->ftm_rf_factory_data.chan );
    break;

  case FTM_TX_ARB_CW_TONE:
    /* hard code device to zero */
    status = ftm_cdma_gen_cw_tone( RFM_DEVICE_0, 
                                   rf_req->ftm_rf_factory_data.gen_d);
    break;
...
)
{
  ...
  /* CDMA commands */
  /*--------------------------------------------------------------------------*/
  case FTM_CDMA_RELOAD_LNA_SCRIPT:
    /* FTM_CDMA_RELOAD_LNA_SCRIPT replaced by FTM_CDMA_RXAGC_DEBUG */
    ftm_rsp_data = ftm_cdma_rxagc_debug( ftm_req_data );
    break;
  ...
  switch ( data->ftm_hdr.cmd_id )
  {
    case FTM_1X_RX_LEVEL_REQ:
    {
      ...
      */
      ...
      rfm_device_enum_type assigned_dev;
	  ...
	  /*In NS testing, map FTM Chain 0 or 2 to Prx device and FTM Chain 1 or 3 to div device*/
      if ((param->rx_chain==0)||(param->rx_chain==2)) 
      {
        is_pri_device = TRUE;
      }
	  else if ((param->rx_chain==1)||(param->rx_chain==3))
      {
        is_pri_device = FALSE;
      }
      assigned_dev = rfm_1x_ftm_get_active_device (is_pri_device);

	  /* query to get RxAGC data */
      rxagc_is_valid = rfm_1x_ftm_get_rx_agc_data( assigned_dev, 
