...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/common/ftm/src/ftm_cdma_data.c#1 $
...
06/08/15   zhw     Support device 4/5 in existing FTM RF test framework
...
)
{
  ...
  for ( dev_idx = RFM_DEVICE_0; dev_idx < RFM_MAX_DEVICES; dev_idx++ )
  {
    if( rfc_cdma_get_logical_device_params(dev_idx) != NULL)
    {
      ...
      /* Initialize TxLM Device voting mask to 0*/
      ftm_cdma_data.rf_device_status[dev_idx]->txlm_handle_dev_vote = 0;
      ...
      ftm_cdma_data.legacy_input_device_band[dev_idx] = RFM_CDMA_MAX_BAND;
    }
    else
    {
      FTM_MSG_1( FTM_ERROR, "ftm_cdma_data_init_device_status: Null logical "
          "device for device id %d", dev_idx );
    }
  }
...
/*!
  @brief
  Destructor for the ftm_cdma_data module.

  @details
  Deallocates all resources that were obtained in ftm_cdma_data_init_device_status().
*/
void
ftm_cdma_cleanup_device_status
(
  void
)
{    
  ...
  for ( dev_idx = RFM_DEVICE_0; dev_idx < RFM_MAX_DEVICES; dev_idx++ )
  {
    if(NULL != ftm_cdma_data.rf_device_status[dev_idx])
    {
      modem_mem_free( ftm_cdma_data.rf_device_status[dev_idx], MODEM_MEM_CLIENT_RFA );
      ftm_cdma_data.rf_device_status[dev_idx] = NULL;
    }
  }
...
rfm_device_enum_type 
ftm_cdma_get_remapped_rx_device
( 
...
)
{
  ...
  curr_band = ftm_cdma_data.legacy_input_device_band[legacy_rx_device];

  mapped_rx_device = rfc_cdma_get_remapped_rx_device( legacy_rx_device, 
                                                      curr_band );

  FTM_MSG_3(FTM_HIGH,"ftm_cdma_get_remapped_rx_device: "
...
void
ftm_cdma_data_set_band_mapping
( 
  rfm_device_enum_type legacy_rx_device,
  rfm_cdma_band_class_type band
)
{
  ftm_cdma_data.legacy_input_device_band[legacy_rx_device] = band;

  return;
}

rfm_cdma_band_class_type
ftm_cdma_data_get_band_mapping
( 
   rfm_device_enum_type legacy_rx_device
)
{
  return ftm_cdma_data.legacy_input_device_band[legacy_rx_device];
}
