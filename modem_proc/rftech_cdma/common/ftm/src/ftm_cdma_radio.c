...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/common/ftm/src/ftm_cdma_radio.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
01/30/19   aa      KW Errors fix
...
/*!
  @brief
  Dispatch Request Packet for #FTM_CDMA_RXAGC_DEBUG Command
*/
typedef ftm_cdma_req_20_type ftm_cdma_rxagc_debug_req_type;
...
/*!
  @brief
  Dispatch Response Packet for #FTM_CDMA_RXAGC_DEBUG Command
*/
typedef ftm_cdma_rsp_1_type ftm_cdma_rxagc_debug_rsp_type;
...
/*!
  @brief
  Enables debug command based on RxAGC value

  @details
  This function will trigger API to force crash the UE based on the input parameters.
...
*/
ftm_rsp_pkt_type
ftm_cdma_rxagc_debug
(
  void  *ftm_req_data
)
{
  /* Request Packet */
  ftm_cdma_rxagc_debug_req_type* header = \
    (ftm_cdma_rxagc_debug_req_type*)ftm_req_data;

  /* Response Packets */
  ftm_rsp_pkt_type ftm_rsp_data;          /* Packet returned to Diag */
  ftm_cdma_rxagc_debug_rsp_type *cdma_rsp; /*Packet Payload for each cmd*/
  /* Extract Useful Data */
  uint8 chain_type = header->chain_type;
  uint8 rxagc_debug_limit = header->rxagc_debug_limit;
  boolean agc_delta_flag = (boolean)header->agc_delta_flag;
  uint8 cnt = header->cnt;
  boolean resp = FALSE;
  
  /* Create a new reponse packet in the ftm heap. This will also update first 
  10-bytes field in Packet Payload in the FTM response packet. These fields 
  are command(1), subsystem ID (1), Command Mode (2), FTM command (2), 
  Request Packet Length (2), and Rsesponse packet Lenth (2). */
  ftm_rsp_data = ftm_cdma_malloc_rsp_pkt( ftm_req_data,
                             sizeof(ftm_cdma_rxagc_debug_rsp_type) );
  if ( chain_type > 2 )
  {
    FTM_MSG( FTM_ERROR, "ftm_cdma_rxagc_debug: Unsupported chain type" );
    ftm_rsp_data.cmd = FTM_RSP_BAD_PARM;
    return ftm_rsp_data;
  }
  resp = rf_cdma_mdsp_rxagc_debug_cmd(chain_type, rxagc_debug_limit, 
                                      cnt, agc_delta_flag);
  /* Extract Packet Payload from New Response Packet, ftm_rsp_data */
  cdma_rsp = (ftm_cdma_rxagc_debug_rsp_type*)ftm_rsp_data.pkt_payload;
  if(NULL != cdma_rsp)
  {
    if(FALSE == resp)
    {
      cdma_rsp->rf_status = 0;
    }
    else
    {
      cdma_rsp->rf_status = 1;
      /* Update the Response Packet Field */
      FTM_MSG_4( FTM_HIGH, "ftm_cdma_rxagc_debug: chain_type=%d"
            "rxagc_debug_limit=%d cnt=%d agc_delta_flag=%d",
           chain_type, rxagc_debug_limit, cnt, agc_delta_flag);
    }
  }
  else
  {
    FTM_MSG( FTM_ERROR, "ftm_cdma_rxagc_debug : NULL cdma_rsp or memory not allocated" );
    ftm_rsp_data.cmd = FTM_RSP_BAD_PARM;
  }
  /* Return Response Packet */
  return ftm_rsp_data;
...
{
  ftm_rsp_pkt_type ftm_rsp_data = {FTM_RSP_DO_LEGACY, 0, NULL, FALSE};  /* return value */
  boolean rv;
...
)
{
  ...
  if ( dev_status != NULL )
  {
       ...
       (  dev_status->rf_state == FTM_CDMA_STATE_RXTX ))
    {
      ...
      ret_val = rfm_cdma_ftm_test_rx_on_off( force_microsleep_enable_disable,
                                            rf_off_ustmr_delta,
                                            rf_on_ustmr_delta, 
                                            duration_half_slots,
                                            sleep_mode , curr_mode ) ;
    }
    else
    {
