...
$Header: //components/rel/rftech_cdma.mpss/3.11/common/rf/src/rfc_cdma.cpp#1 $

when       who     what, where, why
-------------------------------------------------------------------------------- 
02/28/18   am      QAT3522 xsw tech side changes
05/11/16   rs      Removed excessive F3 msgs
01/27/16   pk      Send sleep vote for RF_PATH_TX_SLEEP_STATE state
...
05/12/15   rcui    change vreg voting to be based on device 
05/04/15   zhw     Use RFC Common API to get asscociated Tx Device
04/09/15   zhw     Remap Cal input Rx device based on new RFC device mapping
                   Rewrite 1x xpt config using new RFC mapping
04/02/15   zhw     NULL pointer check for get_modem_chain()
03/30/15   zhw     Add support for Split Tx Rx - default Tx device
03/05/15   zhw     Add support for querying default Tx RFM device given a Rx Dev
12/17/14   wwl     add new function rfc_cdma_get_fbrx_modem_chain
...
)
{
  ...
  if ( (vreg_path_num != RF_PATH_MAX) && (RF_CHIP_MAX != rf_chip) )
  {
    ...
    result &= vreg_param.set_device(device);
...
/*!
  @brief
  This function generates the XSW sleep script for the requested device
  and band.
  ...
  @param asdiv_script_ptr
  Output - The XSW script
 
  @return
  True on success and FALSE on failure
*/
boolean
rfc_cdma_generate_xsw_sleep_script
( 
  rfcommon_asdiv_position_type position,
  rf_buffer_intf *sleep_script_ptr,
  void* cb_data
)
{
  boolean status;
  ...
  if ( rfc_obj == NULL )
  {
    RF_MSG( RF_ERROR, "rfc_cdma_generate_xsw_sleep_script : Obj is NOT created");
    return FALSE;
  }
  
  if ( sleep_script_ptr == NULL )
  {
    RF_MSG( RF_ERROR, "rfc_cdma_generate_xsw_sleep_script : script ptr is "
                      "null"); 
    return FALSE;
  }

  status = rfc_obj->generate_xsw_sleep_script(position, sleep_script_ptr, cb_data );
  
  if( status == FALSE )
  {
    RF_MSG( RF_ERROR, "rfc_cdma_generate_xsw_sleep_script : "
                      "script generation failed");
  }
...
)
{
  ...
  if ( logical_dev != NULL )
  {
    ...
    {
      if ( logical_dev->rfc_tx_device_info.rfc_device_info[band] != NULL )
      {
        modem_chain = 
      ...
      else
      {
        RF_MSG_1 ( RF_ERROR , "rfc_cdma_get_tx_modem_chain(): " 
               "NULL RFC Device info for band %d" , band ) ;
      }
    }
    else
    {
...
/*!
  @brief
  This API returns the feedback RX modem chain based on the device and
  the band.
  ...
  @return
  Feedback Modem Chain
 
*/

uint32
rfc_cdma_get_fbrx_modem_chain
(
  rfm_device_enum_type device ,
  rfm_cdma_band_class_type band 
)
{
  ...
  rfc_cdma_card_intf *rfc_obj = rfc_cdma_card_intf::get_instance();
  ret = rfc_obj->get_fb_device_modem_chain(device, band, &modem_chain);

  if(FALSE == ret)
  {
    RF_MSG_1 ( RF_ERROR , "rfc_cdma_get_fbrx_modem_chain(): " 
             "fetch modem chain failed: %d" , band ) ;
  }
  return modem_chain;
}
...
*/
/*----------------------------------------------------------------------------*/
boolean rfc_cdma_get_fb_tranceivers
(
  rfm_device_enum_type device, 
  rfm_device_enum_type device_fb,
  rfm_cdma_band_class_type band,
...
)
{
  ...
  api_status &= rfc_obj->get_fb_tranceivers(device, device_fb, band,  
                                         tx_device_ptr, fb_device_ptr);
...
/*!
  @brief
  API to return the default paired Tx logical device for a given Rx device.
  If not default Tx device available, return INVALID Device.
  Temporarily maintain this API within C2K until RFC support is ready
...
rfc_cdma_get_remapped_rx_device
( 
  rfm_device_enum_type legacy_rx_device,
  rfm_cdma_band_class_type cdma_band
)
{ 
  rfm_device_enum_type rfc_rx_device ;
  ...
  source_device_info.tech = RFM_1X_MODE;
  source_device_info.band = cdma_band;
  ...
  return rfc_rx_device;
} /* rfc_cdma_get_default_tx_logical_device */
...
{ 
  return rfc_common_get_preferred_associated_tx_device( rx_device );
} /* rfc_cdma_get_default_tx_logical_device */

/*----------------------------------------------------------------------------*/
/*!
  @brief
  API to return the whether an NV path is valid for a given band, Rx/Tx.
...
)
{
  ...
  if ( rfc_obj == NULL )
  {
    RF_MSG( RF_ERROR, "rfc_cdma_is_nv_path_valid : Obj is NOT created");
    return FALSE;
  }

  return rfc_obj->is_nv_path_valid( nv_path, rf_band );
}
...
/*!
  @brief
  Set current Rx MDSP path for a given RFM device.

  @details
  This method will update current Rx MDSP path for a given RFM
  device.The MDSP path to Device mapping used to be static but now dynamic.
  The mapping follows - Prx wakeup - MDSP Chain 0. Drx enable - MDSP Chain 1.

  @param device
  RFM Device 

  @return rx_mdsp_path
  Current Rx MDSP path for a given RFM device.
*/
boolean 
rfc_cdma_set_rx_mdsp_path
(
  rfm_device_enum_type device,
...
)
{
  ...
  if ( rfc_obj == NULL )
  {
    RF_MSG( RF_ERROR, "rfc_cdma_set_rx_mdsp_path : Obj is NOT created");
    return FALSE;
  }

  return rfc_obj->set_rx_mdsp_path( device, rx_mdsp_path );
}/* rfc_cdma_set_rx_mdsp_path */
