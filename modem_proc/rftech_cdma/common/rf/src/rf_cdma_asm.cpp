...
$Header: //components/rel/rftech_cdma.mpss/3.11/common/rf/src/rf_cdma_asm.cpp#1 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------
01/05/16   rs      fix crash for FTM_GET_RFFE_DEVICE_INFO cmd
08/19/15   hm      Add API to update ASM state
...
/*----------------------------------------------------------------------------*/
  /*!
    @brief
    Sets switch state back to requested mode
   
    @param tech
    The tech for which restore needs to be done
  ...
  */
  boolean
  rf_cdma_asm_device_reset_switch_state
  (
    rf_cdma_asm* sm,
  ...
  {
    /* Call wrapper interface */
    return( sm->device_reset_switch_state( device, mode, band )); 
  }
...
/*----------------------------------------------------------------------------*/
  ...
  */
  ...
  {
    /* Call wrapper interface */
    return( sm->update_asm_state( device, mode, band )); 
  }
...
/*!
  @brief
  Reset device switch state after measurement gap

  @param RFM mode
  The requested RFM mode

  @param Band
  The requested band

  @retval TRUE
  If reset ASM device was success

  @retval FALSE
  If reset ASM device failed

*/
bool rf_cdma_asm::device_reset_switch_state
( 
  rfm_device_enum_type device,
  rfm_mode_enum_type mode,
  rfm_cdma_band_class_type band
)
{
  ...
  /* This can be Rx or Tx but we shouldnt be even cyclying to this API needs 
  to be fixed. */
  for ( i = 0; i < this->num_asm_instances[RX_ASM]; i++  )
  {
    /*Retrieve device object*/
    ...
    {
      /* Config ASM for Tx*/
      result &= rfdevice_asm_set_asm_xsw_port_config( asm_device, 
                                              device,
                                              mode, 
    ...
    {
      RF_MSG( RF_ERROR, "rf_cdma_asm::device_reset_switch_state: "
              "ASM is null" );
    }/*if ( asm_device == NULL )*/
  }

  return result;
}/* device_reset_switch_state */
...
*/
bool rf_cdma_asm::update_asm_state
( 
  rfm_device_enum_type device,
  rfm_mode_enum_type mode,
  rfm_cdma_band_class_type band
)
{
  ...
  for ( i = 0; i < this->num_asm_instances[RX_ASM]; i++  )
  {
    ...
    if ( asm_device != NULL )
    {
      ...
      /* update ASM for state*/
      result &= rfdevice_asm_update_state( &asm_cfg_s, 
                                           asm_device,
                                           NULL,
