...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/common/rf/src/rf_cdma_utils.c#1 $
...
)
{
  ...
  if ( (rf_path_enum_type)device >= RF_PATH_NUM || band >= RFM_CDMA_MAX_BAND )
  {
    RF_MSG_2(RF_ERROR,"rf_cdma_is_band_supported: Invalid band %d or device %d,"
                     ,band, device );
    return FALSE;
  }

  /* Get Band config NV */
  nv_mask = rfcommon_nv_get_rx_device_bc_config ( device ) ;   
...
boolean
rf_cdma_is_band_supported_on_any_device
(
  const rfm_cdma_band_class_type band
)
{
  ...
  for ( dev_idx = RFM_DEVICE_0; dev_idx < (rfm_device_enum_type)RF_PATH_NUM;
        dev_idx ++)
  {
    if ( rf_cdma_is_band_supported( dev_idx, band ))
    {
      band_supported = TRUE;
      break;
    }
