...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/common/rf/src/rf_cdma_data.c#1 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------ 
01/24/20   aa      HA QSH Minidump Changes 
05/02/18   rs      Memory cleanup during 1X deinit
01/15/18   am      F3 reduction
05/13/16   wxl     Add API to get tx device for 1x and hdr mode
05/03/16   rs      Update tuner scripts for Div when updating for Tx
03/03/16   rs      Added support for 1x2G QTA
03/03/16   rs      Added null pointer check in get_active_device
08/07/15   pl      use assigned device in ftm_cdma_mc_dispatch to get RXAGC level
05/04/15   zhw     Added best matched Tx device look up for FBRX DC Cal
03/26/15   zhw     1x QCTA Store source tech device in irat data for TQ usage
12/09/14   spa     Added missing include file for FBRx APIs
 ...
 /*--------------------------------------------------------------------------*/
  ...
  for ( dev_idx = RFM_DEVICE_0; dev_idx < RFM_MAX_DEVICES; dev_idx++ )
  {
    if( rfc_cdma_get_logical_device_params(dev_idx) != NULL )
    {
     ...
     /* Default assciated/slave device is Invalid Device */ 
     rf_cdma_data.rf_device_status[dev_idx]->assoc_master_dev = RFM_INVALID_DEVICE;
      ...
      rf_cdma_data.rf_device_status[dev_idx]->plim_asdiv_cb_data.device = dev_idx;
      rf_cdma_data.rf_device_status[dev_idx]->plim_asdiv_cb_data.asdiv_cb_sw_pos_data
                                              = ASDIV_POSITION_MAX;
      rf_cdma_data.rf_device_status[dev_idx]->plim_asdiv_cb_data.asdiv_cb_handler 
                                                                         = NULL;
#endif
  ...
  /* Default to SIM 0 */
  rf_cdma_data.subscriber_data.sim_id_1x = RFLM_SIM_0;

  rf_cdma_data.subscriber_data.sim_id_hdr = RFLM_SIM_0;
...
)
{
  ...
  if (
        ...
        &&
        (rf_cdma_data.rf_device_status[device]->assoc_master_dev==RFM_INVALID_DEVICE) 
     )
  {
    return TRUE;
  }
...
)
{
  ...
  {
    RF_MSG_DBG_1( RF_ERROR, "rf_cdma_data_get_hdr_rf_state: Invalid Device %d", 
              device );
  }
  return rf_state ;
} /* rf_cdma_data_get_hdr_rf_state */
...
rf_cdma_data_get_assoc_dev
(
  rfm_device_enum_type device
)
{
  rfm_device_enum_type assoc_dev = RFM_INVALID_DEVICE;

  if ( device >= RFM_MAX_DEVICES || rf_cdma_data.rf_device_status[device] == NULL )
  {
    RF_MSG_1( RF_ERROR, "rf_cdma_data_get_assoc_dev: Invalid Device %d", 
              device );
  }
  else
  {
    assoc_dev = rf_cdma_data.rf_device_status[device]->assoc_dev;
  }
...
/*!
  @brief
  Set assoc_master_dev for rf_cdma_data

  @details
  This function will set assoc_master_dev for rf_cdma_data.

  @param device
  RF device for which updates need to be done
 
  @param tx_bw_khz
  assoc_master_dev to be set
  
*/
void
rf_cdma_data_set_assoc_master_dev
(
  rfm_device_enum_type device,
  rfm_device_enum_type assoc_master_dev
)
{
  /*
     the arg: device should be a valid one
     arg: assoc_master_dev may be invalid - as in, RFM_MAX_DEVICES. 
 
     This check should never fail as, it is mandetory to enable Rx before any
     1x related operation.
 */
  if ( device >= RFM_MAX_DEVICES || rf_cdma_data.rf_device_status[device] == NULL )
  {
    RF_MSG_1( RF_ERROR, "rf_cdma_data_set_assoc_dev: Invalid Device %d", 
  ...
  {
    rf_cdma_data.rf_device_status[device]->assoc_master_dev = assoc_master_dev;
  }
} /* rf_cdma_data_set_assoc_master_dev */
...
/*!
  @brief
  Get assoc_master_dev for rf_cdma_data

  @details
  This function will get assoc_master_dev for rf_cdma_data.

  @param device
  device whose assoc_master_dev to be get  
*/

rfm_device_enum_type
rf_cdma_data_get_assoc_master_dev
(
  rfm_device_enum_type device
)
{
  rfm_device_enum_type assoc_master_dev = RFM_INVALID_DEVICE;
  ...
  {
    assoc_master_dev = rf_cdma_data.rf_device_status[device]->assoc_master_dev;
  }

  return assoc_master_dev;
} /* rf_cdma_data_get_assoc_master_dev */
...
/*!
  ...
  @param irat_tq_device
  Device used to allocate TQ in iRAT. QCTA requires 1x to use source tech primary
  device to allocate CCS TQ.
...
(
  ...
  uint32                          buffer_index ,
  lm_handle_type                  rxlm_handle,
  rfm_device_enum_type            irat_tq_device 
)
{
  ...
  if ( device < RFM_MAX_DEVICES )
  {
    ...
    irat_data->irat_tq_device =     irat_tq_device;
    success =  TRUE ;
  }
  else
  {
...
)
{
  ...
  if ( device < RFM_MAX_DEVICES )
  {
    ...
    irat_data->irat_tq_device = RFM_INVALID_DEVICE;
  }
  else
  {
...
/*!
  @brief
  rf_cdma_data_get_active_device


  @details
  This function will get active primary or diversity device.

  @param is_pri_device
  Indicate primary or diversity device need to be found.
 
  @return active_device
  assigned device corresponding to legacy RF device.
  
*/
rfm_device_enum_type
rf_cdma_data_get_active_device
(
   boolean is_pri_device
)
{
  rfm_device_enum_type dev_idx; /* Variable for Device Loop */
  rfm_device_enum_type active_device;

  active_device = RFM_INVALID_DEVICE;

  if (is_pri_device == TRUE) /* pri device */
  {
    for ( dev_idx = RFM_DEVICE_0; dev_idx < RFM_MAX_WAN_DEVICES; dev_idx++ )
    {
      if ( rf_cdma_data.rf_device_status[dev_idx] != NULL )
    {
	  if ((rf_cdma_data.rf_device_status[dev_idx]->data_1x.rf_state == RF_CDMA_STATE_RX) ||
	 	( rf_cdma_data.rf_device_status[dev_idx]->data_1x.rf_state == RF_CDMA_STATE_RXTX))
	  {
  ...
  else if (is_pri_device == FALSE)/* div device */
  {
    for ( dev_idx = RFM_DEVICE_0; dev_idx < RFM_MAX_WAN_DEVICES; dev_idx++ )
    {
      if ( rf_cdma_data.rf_device_status[dev_idx] != NULL )
      {
	     if (rf_cdma_data.rf_device_status[dev_idx]->data_1x.rf_state == RF_CDMA_STATE_RXDIV)
	 	{
		  active_device = dev_idx;
		  break;
	    }
  ...
  if (active_device ==RFM_INVALID_DEVICE)
  {
  	RF_MSG( RF_ERROR, "rf_cdma_data_get_active_device: No active Device.");
  }
...
/*!
  @brief
  rf_cdma_hdr_data_get_active_tx_device
...
)
{
  ...
  for ( dev_idx = RFM_DEVICE_0; dev_idx < RFM_MAX_WAN_DEVICES; dev_idx++ )
  {
    if ( rf_cdma_data.rf_device_status[dev_idx] != NULL )
    {
      if ((rf_cdma_data.rf_device_status[dev_idx]->data_1x.rf_state == RF_CDMA_STATE_TX) ||\
          (rf_cdma_data.rf_device_status[dev_idx]->data_1x.rf_state == RF_CDMA_STATE_RXTX) ||\
          (rf_cdma_data.rf_device_status[dev_idx]->data_hdr.rf_state == RF_CDMA_STATE_TX) ||\
          (rf_cdma_data.rf_device_status[dev_idx]->data_hdr.rf_state == RF_CDMA_STATE_RXTX))
      {
        active_device = dev_idx;
        break;
      }
...
/*!
  @brief
  Set antenna position for rf_cdma_data

  @details
  This function will set the antennae position from the common
  code and set it for rf_cdma_data. This happens per device.

  @param device
  RF device for which updates need to be done
 
  @param ant_position
  Antenna position as retrieved from RF code.
 
*/
void
rf_cdma_data_set_ant_position
(
  rfm_device_enum_type device,
  uint8 ant_position
)
{
  if ( device < RFM_MAX_DEVICES && rf_cdma_data.rf_device_status[device] != NULL )
  {
    rf_cdma_data.rf_device_status[device]->rf_cdma_ant_sw_data.antenna_position = 
      ant_position ;
  }
  else
  {
    RF_MSG_1( RF_ERROR, "rf_cdma_data_set_ant_position: Invalid Device %d", 
              device );
  }
} /* rf_cdma_data_set_ant_position */
...
/*!
  @brief
  Get antenna position from rf_cdma_data

  @details
  This function will get antenna position from rf_cdma_data.

 @details
  This function will get the antennae position from the common
  code and set it for rf_cdma_data. This happens per device.
 
  @return ant_position
*/
uint8
rf_cdma_data_get_ant_position
(
  rfm_device_enum_type device
)
{
  ...
  {
    RF_MSG_1( RF_ERROR, "rf_cdma_data_get_ant_position: Invalid Device %d", 
              device );
  }
  return ant_pos ;
} /* rf_cdma_data_get_ant_position */
...
/*!
  @brief
  Set Pout comp for AsDiv related operations for rf_cdma_data

  @details
  This function Pout comp for AsDiv related operations from the
  common code and set it for rf_cdma_data. This happens per
  device.
  ...
  @param pout_comp
  pout compensation that is applied for current Ant position.
 
*/
void
rf_cdma_data_set_pout_for_ant_position
(
  rfm_device_enum_type    device   ,
  int16                   pout_comp
)
{
  if ( device < RFM_MAX_DEVICES && rf_cdma_data.rf_device_status[device] != NULL )
  {
    rf_cdma_data.rf_device_status[device]->
      rf_cdma_ant_sw_data.pout_compensation_used = 
      pout_comp ;
  }
  else
  {
    RF_MSG_1( RF_ERROR, "rf_cdma_data_set_pout_for_ant_position: Invalid Device %d", 
              device );
  }
} /* rf_cdma_data_set_pout_for_ant_position */
...
/*!
  @brief
  Get Pout comp for AsDiv related operations for rf_cdma_data
 ...
 @details
  This function Pout comp for AsDiv related operations from the
  common code and get it from rf_cdma_data. This happens per
  device.
 
  @return pout_comp
  Pout Compensation currently applied for given ant position.
*/
int16
rf_cdma_data_get_pout_for_ant_position
(
  rfm_device_enum_type device
)
{
  ...
  {
    RF_MSG_1( RF_ERROR, "rf_cdma_data_get_pout_for_ant_position: Invalid Device %d", 
              device );
  }
  return pout_comp ;
} /* rf_cdma_data_get_pout_for_ant_position */
...
/*!
  @brief
  Set RxAGC Reloaded for AsDiv related operations for rf_cdma_data

  @details
  This function RxAGC Reloaded for AsDiv related operations from the
  common code and set it for rf_cdma_data. This happens per
  device.
  ...
  @param rxagc_status_comp
  rxagc_status compensation that is applied for current Ant position.
 
*/
void
rf_cdma_data_set_rxagc_status_for_ant_position
(
  rfm_device_enum_type    device   ,
  boolean                 rxagc_status_comp
)
{
  if ( device < RFM_MAX_DEVICES && rf_cdma_data.rf_device_status[device] != NULL )
  {
    rf_cdma_data.rf_device_status[device]->rf_cdma_ant_sw_data.was_rxagc_reloaded = 
      rxagc_status_comp ;
  }
  else
  {
    RF_MSG_1( RF_ERROR, "rf_cdma_data_set_rxagc_status_for_ant_position: Invalid Device %d", 
              device );
  }
} /* rf_cdma_data_set_rxagc_status_for_ant_position */
...
/*!
  @brief
  Get RxAGC Reloaded for AsDiv related operations for rf_cdma_data
 ...
 @details
  ...
  @return rxagc_status_comp
  Pout Compensation currently applied for given ant position.
*/
boolean
rf_cdma_data_get_rxagc_status_for_ant_position
(
  rfm_device_enum_type device
)
{
  int16 rxagc_status_comp = 0 ;
  if ( device < RFM_MAX_DEVICES && rf_cdma_data.rf_device_status[device] != NULL )
  {
    rxagc_status_comp = 
      rf_cdma_data.rf_device_status[device]-> rf_cdma_ant_sw_data.was_rxagc_reloaded ;
  ...
  {
    RF_MSG_1( RF_ERROR, "rf_cdma_data_get_rxagc_status_for_ant_position:" 
              "Invalid Device %d", device );
  }
  return rxagc_status_comp ;
} /* rf_cdma_data_get_rxagc_status_for_ant_position */
...
/*----------------------------------------------------------------------------*/
/*! Default Getter Method for subscriber ID */
rflm_sim_id_t
rf_cdma_data_get_subscriber_id
(
  rfm_mode_enum_type mode
)
{

  rflm_sim_id_t sim_id;

  if ( mode == RFM_1X_MODE )
  {
    sim_id = rf_cdma_data.subscriber_data.sim_id_1x;
  }
  else if ( mode == RFM_1XEVDO_MODE )
  {
    sim_id = rf_cdma_data.subscriber_data.sim_id_hdr;
  }
  else
  {
    sim_id = RFLM_MAX_SIM_ID;
  }
...
/*----------------------------------------------------------------------------*/
/*! Default Setter Method for subscriber ID */
boolean
rf_cdma_data_set_subscriber_id
(
  rfm_mode_enum_type mode,
  uint8 sub_id
)
{
  ...
  rflm_sim_id_t rflm_sim_id;

  if ( sub_id == 0 )
  {
    rflm_sim_id = RFLM_SIM_0;
  }
  else if ( sub_id == 1 )
  {
    rflm_sim_id = RFLM_SIM_1;
  }
  else
  {
    RF_MSG_1(RF_ERROR,"rf_cdma_data_set_subscriber_id invalid sub id %d", sub_id);
    api_status = FALSE;
  }
  ...
  if ( api_status == TRUE )
  {
    if ( mode == RFM_1X_MODE )
    {
      rf_cdma_data.subscriber_data.sim_id_1x = rflm_sim_id;
    }
    else if ( mode == RFM_1XEVDO_MODE )
    {
      rf_cdma_data.subscriber_data.sim_id_hdr =  rflm_sim_id;
    }
    else
    {
      RF_MSG_1(RF_ERROR,"rf_cdma_data_set_subscriber_id invalid mode id %d", mode);
      api_status = FALSE;   
    }
  }
  ...
  if ( api_status == TRUE )
  {
    RF_MSG_2(RF_MED,"rf_cdma_data_set_subscriber_id updated mode %d id %d", mode, sub_id);
  }
