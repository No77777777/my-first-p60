...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/common/rf/src/rf_cdma_nv.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
01/24/20   aa      HA QSH Minidump Changes 
05/28/19   mk      Memset parameters corrected
...
01/19/16   rs      Fixed bug in HDR AsDiv LNA offset NV Check
01/07/16   rs      Adding logic to correctly read BC0 AsDiv NV
01/06/16   pk      Added rx_gain_calibration value also to the fake nv init
09/05/15   sty     Fixed calls to memscpy
09/03/15   wwl     Replace memcpy by safer version memscpy
06/16/15   wwl     Add support for new SAR implementation
05/27/15   zhw     Map BC COnfig to logical device for CDMA
05/14/15   zhw     Added CDMA unified Rx static NV support
03/30/15   zhw     Support splitted Tx device during NV loading.
                   Remove hard-coded NV path to device mapping. Look up from RFC
01/16/15   zhw     CDMA predictive clipping NV based power thresh support
...
/*!
  @brief
   RF CDMA SAR NV structure with ASDiv awareness
 */
typedef struct
{
  uint32 band_ind;
  rfnv_tx_pwr_sar_1_8_temp_comp_type sar_asdiv_backoff_table \
                                         [RFM_CDMA_MAX_BAND] \
                                        [ASDIV_POSITION_MAX];
} rf_cdma_nv_sar_asdiv_type;
#endif
...
/*!
  ...
  If no valid Tx device is found. The API returns INVALID DEVICE, which will
  not be recognized as a valid Tx device by the caller.
  The NV loading procedure would then skip loading Tx related NV for this path
 
  @param nv_path
  The NV path requested
 
  @param band
  Band that NV is being loaded for
...
)
{
  ...
  /* Removed hard-coded NV path to device mapping. Look up from RFC */
  for (rfm_device_enum_type dev_idx = 0; dev_idx < RFM_MAX_WAN_DEVICES;
       dev_idx++ )
  {
    logical_dev = rfc_cdma_get_logical_device_params(dev_idx);
          
    if ( logical_dev != NULL &&
         logical_dev->tx_nv_path[band] == nv_path )
    {
      rf_device = dev_idx;
      break;     
    }
...
)
{
  ...
  rf_hdr_config_data->rx_static.lna_info_dor0_a.lin_info[1].sp_info[0].sp_mode = RF_CDMA_LNA_SWITCH_PT_MODE_FIXED;
  rf_hdr_config_data->rx_static.lna_info_dor0_a.lin_info[1].sp_info[0].sp_data.fixed.fall = -926;
  rf_hdr_config_data->rx_static.lna_info_dor0_a.lin_info[1].sp_info[0].sp_data.fixed.rise = -882;
  rf_hdr_config_data->rx_static.lna_info_dor0_a.lin_info[1].sp_info[0].post_fall_hold = 0;
  rf_hdr_config_data->rx_static.lna_info_dor0_a.lin_info[1].sp_info[0].post_rise_hold = 6800;

  rf_hdr_config_data->rx_static.lna_info_dor0_a.lin_info[1].sp_info[1].sp_mode = RF_CDMA_LNA_SWITCH_PT_MODE_FIXED;
  rf_hdr_config_data->rx_static.lna_info_dor0_a.lin_info[1].sp_info[1].sp_data.fixed.fall = -818;
  rf_hdr_config_data->rx_static.lna_info_dor0_a.lin_info[1].sp_info[1].sp_data.fixed.rise = -806;
  rf_hdr_config_data->rx_static.lna_info_dor0_a.lin_info[1].sp_info[1].post_fall_hold = 0;
  rf_hdr_config_data->rx_static.lna_info_dor0_a.lin_info[1].sp_info[1].post_rise_hold = 6800;
...
/*!
  @brief
  This is the function to copy NV data

  @param nvtbl_p
  Buffer to store the NV data

  @param src_p
  pointer to the uncompressed SAR data
...
static boolean
rf_cdma_nv_load_nv_sar_data
(
  rf_cdma_nv_sar_asdiv_type *nvtbl_p,
  rfnv_sar_asdiv_item_type src_p[],
  uint32 data_size
)
{
  uint32 i;
  ...
  boolean status = TRUE;
  boolean bc0_already_done = FALSE;
  uint32 num_band = data_size /
                   sizeof(rfnv_sar_asdiv_item_type);
  uint8 *data_p = NULL;
  uint8 *nvitem_p = NULL;
  uint16 cpysize;
  ...
  for(i = 0; i < num_band; i++)
  {
    ...
    /*Since BC0 data is already saved, ignore subsequent band=0(default) data*/
    if (( band == RFM_CDMA_BC0 ) && bc0_already_done )
    {
       continue;
    }
    ...
    {
      /* copy SAR data */
      data_p = (uint8*)(nvtbl_p->sar_asdiv_backoff_table[band]);
      nvitem_p = (uint8*)(src_p[i].nv_data);
      cpysize = sizeof(rfnv_tx_pwr_sar_1_8_temp_comp_type)*ASDIV_POSITION_MAX;
      memscpy(data_p, cpysize, nvitem_p, cpysize);
      nvtbl_p->band_ind |= 1 << band;

      /*Only first entry for band=RFM_CDMA_BC0 has valid values*/
      if ( band == RFM_CDMA_BC0 )
      {
         bc0_already_done = TRUE;
      }
    }
    else
    {
      RF_MSG_1( RF_ERROR, "rf_cdma_nv_load_nv_sra_data: "
                          "NV data is corrupted, band %d",
                          band );
      break;
    }
  }
...
/*!
  @brief
  Load SAR data from NV storage

  @param *sar_static_nv_data
  Buffer point to store loaded data

  @param sar_asdiv_nv_id
  NV item id

  @param sar_task_ptr
  Calling task's TCB pointer

  @param sar_task_wait_sig
  The signal the requesting task has allocated to the RF driver to wait
  on to indicate NV completion.

  @param sar_task_wait_handler
...
)
{
  ...
  rf_cdma_nv_sar_asdiv_type *sar_static_nv_data = NULL;
  ...
  RF_MSG_1(RF_HIGH,
           "rf_cdma_nv_load_sar_asdiv_nv: NV_ID: %d", sar_asdiv_nv_id);
  /* prepare the memory buffer for NV data */
  nv_data_ptr = (rfnv_item_type *)modem_mem_alloc( sizeof(rfnv_item_type),
                                                   MODEM_MEM_CLIENT_RFA );
  nv_dc_data_ptr = (rfnv_item_type *)modem_mem_alloc( sizeof(rfnv_item_type),
                                                      MODEM_MEM_CLIENT_RFA );
  sar_static_nv_data = (rf_cdma_nv_sar_asdiv_type*)
                        modem_mem_alloc( sizeof(rf_cdma_nv_sar_asdiv_type),
                                         MODEM_MEM_CLIENT_RFA );

  if( (NULL != nv_data_ptr)     &&
      (NULL != nv_dc_data_ptr)  &&
      (NULL != sar_static_nv_data))
  {
    /* init newly allocated mem to all 0's */
    memset( nv_data_ptr, 0, sizeof(rfnv_item_type));
    memset( nv_dc_data_ptr, 0, sizeof(rfnv_item_type));
    sar_static_nv_data->band_ind = 0;

    /* load compressed NV data */
    ...
    {
      RF_MSG_2( RF_HIGH,
                "rf_cdma_nv_load_sar_asdiv_nv id: "
                "%d NV read successful, size = %d",
                sar_asdiv_nv_id, nv_read_size);
      ...
      {
        rf_cdma_nv_load_nv_sar_data(sar_static_nv_data,
                             (rfnv_sar_asdiv_item_type *)nv_dc_data_ptr,
                             uncompress_size);
      }
      else
      {
        RF_MSG_2( RF_ERROR,
                  "rf_cdma_nv_load_sar_asdiv_nv: "
                  "decompression of SAR AsDiV NV %d failed!status:%d",
                  sar_asdiv_nv_id, uncompress_status );
        modem_mem_free( sar_static_nv_data, MODEM_MEM_CLIENT_RFA );
        sar_static_nv_data = NULL;
      }
    }
    else
    {
      RF_MSG_2(RF_ERROR,
               "rfcommon_load_wcdma_sar_asdiv_nv nv id: "
               "%d NV Read Failed err code %d",
               sar_asdiv_nv_id, nv_read_status);
      modem_mem_free( sar_static_nv_data, MODEM_MEM_CLIENT_RFA );
      sar_static_nv_data = NULL;
    }
  }
  ...
  {
    if(NULL != sar_static_nv_data)
    {
      modem_mem_free( sar_static_nv_data, MODEM_MEM_CLIENT_RFA );
      sar_static_nv_data = NULL;
    }
    RF_MSG( RF_ERROR,
            "rf_cdma_nv_load_sar_asdiv_nv: failed to allocate memory" );
  }
  ...
  if(NULL != nv_dc_data_ptr)
  {
    modem_mem_free( nv_dc_data_ptr, MODEM_MEM_CLIENT_RFA );
  }
  return sar_static_nv_data;
}
#endif
...
)
{
...
#if defined(FEATURE_RF_ASDIV)
  rf_cdma_nv_sar_asdiv_type *band_asdiv_sar_temp_p = NULL;
  int j;
#endif
       ...
       lim_vs_temp_returned_size >= sizeof(rfnv_tx_pwr_sar_0_temp_comp_type) )
  {
    ...
    for ( i = 0; i < RF_CDMA_NV_TEMP_LIMIT_SIZE; i++ )
    {
      ...
      */
#if defined(FEATURE_RF_ASDIV)
      tx_static_struct->lim_vs_temp[ASDIV_POSITION_0][0][i] =
        (lim_temp_sar_0_data_ptr->temp_comp[i] + 768 + 127)
        + 10*rf_cdma_get_turnaround_constant(band);

      tx_static_struct->lim_vs_temp[ASDIV_POSITION_1][0][i] =
        (lim_temp_sar_0_data_ptr->temp_comp[i] + 768 + 127)
        + 10*rf_cdma_get_turnaround_constant(band);
#else
...
#if defined(FEATURE_RF_ASDIV)
          tx_static_struct->lim_vs_temp[ASDIV_POSITION_0][sar_state][i] =
            (lim_temp_sar_1_8_data_ptr->temp_comp[sar_state-1][i] + 127)
            + 10*rf_cdma_get_turnaround_constant(band);

          tx_static_struct->lim_vs_temp[ASDIV_POSITION_1][sar_state][i] =
            (lim_temp_sar_1_8_data_ptr->temp_comp[sar_state-1][i] + 127)
            + 10*rf_cdma_get_turnaround_constant(band);
#else
...
#if defined(FEATURE_RF_ASDIV)
  /* Load new NV band-antenna-SAR data */
  band_asdiv_sar_temp_p = rf_cdma_nv_load_sar_asdiv_nv(
                                RFNV_CDMA_ANT_POS_BASED_SAR_BACK_OFF_LIMIT_I,
                                caller_tcb_ptr,
                                task_nv_wait_sig,
                                task_wait_func_ptr );

  /* process only if SAR NV valid */
  if( NULL != band_asdiv_sar_temp_p )
  {
    /* Process the SAR table */
    if(0 != ((1 << band) & band_asdiv_sar_temp_p->band_ind))
    {
      for(i = ASDIV_POSITION_0; i < ASDIV_POSITION_MAX; i++)
      {
        /* try to load new NV data into the data struct if it presents */
        for ( sar_state = 1;
              sar_state < RF_CDMA_NV_DSI_SAR_STATE_SIZE;
              sar_state++ )
        {
          ...
          {
            /* pick up the sar value array according to band and ASDIV
             * position, let the data pointer point to it */
            lim_temp_sar_1_8_data_ptr = 
                  &(band_asdiv_sar_temp_p->sar_asdiv_backoff_table[band][i]);

            /* load common static SAR tbl with raw values (needed for forced antenna determination) */
            sar_value_raw = \
                    (band_asdiv_sar_temp_p->sar_asdiv_backoff_table[band][i].temp_comp[sar_state-1][j]);

            tx_static_struct->sar_tbl[i][sar_state][j] = sar_value_raw;

            /* check to see if RAW SAR value contains force other antenna constant 0xFFFE
            *  otherwise keep the lim vs temp that was previously calculated for defualt usage
            *  The intent is not to use the force antenna code value as part of any lim vs temp
            *  calculations and go with defualt calculation as place holder.
            *
            *  In this condition, operation is expected to be forced to other antenna by L1
            */
            if ( sar_value_raw != RF_SAR_FORCE_OTHER_ANTENNA )
            {
               /* load the lim vs temp sar tbl with SAR values */
               tx_static_struct->lim_vs_temp[i][sar_state][j] =
               (lim_temp_sar_1_8_data_ptr->temp_comp[sar_state-1][j] + 127)
                  + 10*rf_cdma_get_turnaround_constant(band);
            }
          } /*  loop thru RF_CDMA_NV_TEMP_LIMIT_SIZE */
        }   /*  loop thru SAR states */
      }     /*  loop thru ASDiv positions */
    }       /*  loop thru bands */

    modem_mem_free( band_asdiv_sar_temp_p, MODEM_MEM_CLIENT_RFA );
  }
#endif
  ...
  else if ( nv_status == NV_NOTACTIVE_S )
  {
          ...
          rc_rate ++ )
    {
            ...
            pa_state++ )
      {
        ...
        /*No need to apply back off for PA state 3  in current target*/
        if ( fall_val < invalid_swpt_val && (pa_state < RFM_PA_GAIN_STATE_3))  
        {
        ...
        /*No need to apply back off for PA state 3  in current target*/
        if ( rise_val < invalid_swpt_val && (pa_state < RFM_PA_GAIN_STATE_3))
        {
...
)
{
  ...
  if ( ret_val == TRUE )
  {
    ...
    if ( tx_static_struct->pa_static_nv_version == RFCMN_NV_PA_STATIC_VER_1 )
    {
      ...
      /* Load QPA cap params */
      tx_static_struct->pa_static_output_cap_offset = 
        (rf_cdma_pa_cap_adj_type)pa_static_data->cmn_pa.header[7] ; 
...
)
{
  ...
  boolean retval = TRUE;
  rf_cdma_config_type* rf_common_config_ptr;
  rf_cdma_static_nv_items_map_type *static_nv_tbl = NULL; /* Pointer to Static NV */
  ...
  Rx only path */
  device = rf_cdma_nv_map_nv_path_to_device( nv_path, band );
  ...
  if ( retval == TRUE )
  {
    index_entry->last_nv_item_read = static_nv_tbl->rx_static_data ;
    retval = rf_cdma_nv_load_unified_rx_static ( &rf_1x_config_data->rx_static,
                                                 &rf_hdr_config_data->rx_static,

                                    /* For Tx compensation because of Rx loss */
                                                &rf_1x_config_data->tx_static,
                                                &rf_hdr_config_data->tx_static,

                                                 static_nv_tbl->rx_static_data ,
                                                 nv_path,
                                                 caller_tcb_ptr ,
                                                 task_nv_wait_sig ,
                                                 task_wait_func_ptr ) ;

    /* If unsuccessful in loading new unified NV, try loadin legacy NV */
    if ( retval == FALSE )
    {
      RF_MSG_2(RF_HIGH,"Failed to load unified Rx static NV. "
                       "Loading legacy Rx switch points path %d band %d",
                        nv_path, band );
...
)
{
  ...
  /* Loop through all NV paths and bands, determine whether the nv path is valid
  in RFC, then load NV. */
  for ( nv_path = RF_CDMA_NV_PATH_0; nv_path < RF_CDMA_NV_PATH_NUM; nv_path ++ )
  {
    ...
    {
      if ( rfc_cdma_is_nv_path_valid(nv_path, rf_band) == TRUE )
      {
        rf_cdma_config_band_index_type* index_entry;

        index_entry = rf_cdma_nv_alloc( nv_path, rf_band );
        ...
        index_entry->data_valid = 
          rf_cdma_nv_load_chain_band( rf_band,
                                      nv_path,
                                      index_entry,
                                      caller_tcb_ptr,
                                      task_nv_wait_sig,
...
)
{
  ...
  else 
  {
        ...
        memset ( &(cal_struct->rx_gain_calibration[0]) ,
                 0,sizeof(cal_struct->rx_gain_calibration) ) ;
        success = FALSE ;
  }
  return success ;
} /* rf_cdma_nv_load_rx_lna_data() */
...
static void
rf_cdma_nv_copy_onex_swpt_to_hdr_rev0_a
(
...
static void
rf_cdma_nv_copy_hdr_rev_0_a_swpt_to_hdr_revb_sc
(
  rf_hdr_rx_static_type *rx_hdr_ptr
)
{
...
static void
rf_cdma_nv_copy_hdr_rev_b_sc_swpt_to_hdr_rev_b_mc
(
  rf_hdr_rx_static_type *rx_hdr_ptr
)
{
...
)
{
  ...
  else if ( rx_swpt_nv_status == NV_NOTACTIVE_S )
  {
    ...
    1x Rx switchpoints */
    rf_cdma_nv_copy_onex_swpt_to_hdr_rev0_a( rx_1x_ptr, rx_hdr_ptr );
...
)
{
  ...
  else if ( rx_swpt_nv_status == NV_NOTACTIVE_S )
  {
    ...
    rf_cdma_nv_copy_hdr_rev_0_a_swpt_to_hdr_revb_sc( rx_hdr_ptr );
...
)
{
  ...
  else if ( rx_swpt_nv_status == NV_NOTACTIVE_S )
  {
    ...
    rf_cdma_nv_copy_hdr_rev_b_sc_swpt_to_hdr_rev_b_mc( rx_hdr_ptr );
 ...
 /* actual NV data size after decompression for Rx static NV */
  /* (1x+DORevA+RevBSC+RevBMC) * 6 Rx chains * size of switch point type + 
  (1x + HDR) * 6 * size of spur table 
  3 for common LNA gain adjustments: Gain, SWPTs and Tx Compensation because of
  Rx gain change.
  */
#define RF_CDMA_MAX_RX_CHAINS_SUPPORTED (6)
#define RF_CDMA_RX_STATIC_NV_MAX_SIZE ( (RF_CDMA_MAX_RX_CHAINS_SUPPORTED) + (3) )
#define RF_CDMA_MAX_SWITCH_POSITIONS_SUPPORTED (4)

/*----------------------------------------------------------------------------*/
/*!
  @brief
  Checks if auxillary Tx HDR NV data is valid applicable.
 
  @details
  Checks if auxillary Tx HDR NV data is valid applicable. On an
  Antennae switch, if the new path has addition static
  compensation loaded into NVs. This function checks the switch
  postion and and if the NV data pertains to it.
 
  @param rf_hdr_config_ptr
  A pointer to the HDR data structure that is necessary to
  describe the entire HDR configuration.
  
  @return
  boolean TRUE if success. FALSE if fail.
*/
boolean
rf_hdr_is_tx_aux_data_valid
(
  const rf_hdr_config_type*     rf_hdr_config_ptr  , /* Pointer to 1x NV config */
  uint8                         position
)
{
  boolean is_tx_aux_data_valid = FALSE ;
  if ( rf_hdr_config_ptr != NULL )
  {
    is_tx_aux_data_valid = 
      rf_hdr_config_ptr->tx_static.hdr_common_tx_aux_data.is_tx_aux_info_valid ;
    if ( is_tx_aux_data_valid == TRUE )
    {
      if ( rf_hdr_config_ptr->tx_static.hdr_common_tx_aux_data.tx_aux_info.
          switch_position != position )
      {
        is_tx_aux_data_valid == FALSE ; 
      }
    }
  }
  return is_tx_aux_data_valid ;
}

/*----------------------------------------------------------------------------*/
/*!
  @brief
  Checks if auxillary Tx 1x NV data is valid applicable.
 
  @details
  Checks if auxillary Tx 1x NV data is valid applicable. On an
  Antennae switch, if the new path has addition static
  compensation loaded into NVs. This function checks the switch
  postion and and if the NV data pertains to it.
 
  @param rf_1x_config_ptr
  A pointer to the 1x data structure that is necessary to
  describe the entire 1x configuration.
  
  @return
  boolean TRUE if success. FALSE if fail.
*/
boolean
rf_1x_is_tx_aux_data_valid
(
...
)
{
  ...
  {
    is_tx_aux_data_valid = 
      rf_1x_config_ptr->tx_static.onex_common_tx_aux_data.is_tx_aux_info_valid ;
    /* Check to see if the switch position also maatches for the given NV else
       return false */

    if ( is_tx_aux_data_valid == TRUE )
    {
      if ( rf_1x_config_ptr->tx_static.onex_common_tx_aux_data.tx_aux_info.
          switch_position != position )
      {
        is_tx_aux_data_valid == FALSE ; 
      }
...
/*!
  @brief
  Checks if auxillary Rx HDR NV data is valid applicable.
 
  @details
  Checks if auxillary Rx HDR NV data is valid applicable. On an
  Antennae switch, if the new path has addition static
  compensation loaded into NVs. This function checks the switch
  postion and and if the NV data pertains to it.
...
*/
boolean
rf_hdr_is_rx_aux_data_valid
(
  const rf_hdr_config_type*     rf_hdr_config_ptr , /* Pointer to 1x NV config */
  uint8                         position
)
{
  boolean is_rx_aux_data_valid = FALSE ;
  if ( rf_hdr_config_ptr != NULL )
  {
    is_rx_aux_data_valid = 
      rf_hdr_config_ptr->rx_static.hdr_common_rx_aux_data.is_rx_aux_info_valid ;

    /* Check to see if the switch position also maatches for the given NV else
       return false */
    if ( is_rx_aux_data_valid == TRUE )
    {
      if ( rf_hdr_config_ptr->rx_static.hdr_common_rx_aux_data.rx_aux_info.
          switch_position != position )
      {
        is_rx_aux_data_valid = FALSE ; 
      }
    }
  }
  return is_rx_aux_data_valid ;
}

/*----------------------------------------------------------------------------*/
/*!
  @brief
  Checks if auxillary Rx 1x NV data is valid applicable.
 
  @details
  Antennae switch, if Rthe new path has addition static
  compensation loaded into NVs. This function checks the switch
  postion and and if the NV data pertains to it.
...
)
{
  ...
  if ( rf_1x_config_ptr != NULL )
  {
    /* Check to see if the switch position also maatches for the given NV else
       return false */
    if ( rf_1x_config_ptr->rx_static.onex_common_rx_aux_data.is_rx_aux_info_valid
           == TRUE )
    {
      if ( rf_1x_config_ptr->rx_static.onex_common_rx_aux_data.rx_aux_info.
          switch_position == position )
      {
        is_rx_aux_data_valid = TRUE ; 
      }
      else
      {
        RF_MSG_2 ( RF_LOW , 
                   "rf_1x_is_rx_aux_data_valid(): ||AsDivPos: %d|NVpos: %d||" ,
                   position , rf_1x_config_ptr->rx_static.
                   onex_common_rx_aux_data.rx_aux_info.switch_position ) ;
      }
    }
  }
  return is_rx_aux_data_valid ;
...
/*!
  @brief
  Loads if auxillary Tx Offset data.
 
  @details
  On an antennae switch, if the new path has additional static
  compensation loaded into NVs. This function loads Tx related
  offsets for Pout adjustment for HDR and 1x.
 
  @parasm aux_asdiv_gain_offset
  A pointer to NV that contains all the information about Tx
  Auxillary data.
 
  @param onex_data
  A pointer to the 1x data structure that is necessary to
  describe the auxillary 1x configuration.
 
  @param hdr_data
  A pointer to the hdr data structure that is necessary to
  describe the auxillary hdr configuration.
 
  @return
  boolean TRUE if success. FALSE if fail.
*/
boolean  
rf_cdma_load_tx_aux_offset_data 
( 
  rfnv_asdiv_tx_gain_offset_type*     aux_asdiv_gain_offset , 
  rf_cdma_tx_aux_static_type*         onex_data             ,
  rf_cdma_tx_aux_static_type*         hdr_data 
) 
{

  boolean success = FALSE ;

  if ( aux_asdiv_gain_offset != NULL )
  {
    if (aux_asdiv_gain_offset->switch_position >= 
        RF_CDMA_MAX_SWITCH_POSITIONS_SUPPORTED )
    {
      success = FALSE ;
    }
    else
    { 
      onex_data->tx_aux_info.switch_position = 
        hdr_data->tx_aux_info.switch_position =
        aux_asdiv_gain_offset->switch_position ;

      onex_data->tx_aux_info.gain_offset = hdr_data->tx_aux_info.gain_offset =
         aux_asdiv_gain_offset->switch_point_offset ;

      /* validate the flag so that during runtime sanitization is not needed */
      onex_data->is_tx_aux_info_valid = 
        hdr_data->is_tx_aux_info_valid = TRUE ;
    }
  }
  else
  {
    RF_MSG ( RF_ERROR , "rf_cdma_load_tx_aux_offset_data(): Aux data is NULL " ) ;
  }
  return success ;
}
...
/*!
  @brief
  Loads if auxillary Rx switchpoints data.
 
  @details
  On an antennae switch, if the new path has additional static
  switchpoint offsets loaded into NVs. This function loads Rx
  switchpoints for adjustment for HDR or 1x.
 
  @parasm aux_asdiv_gain_offset
  A pointer to NV that contains all the information about Tx
  Auxillary data.
 
  @param rf_cdma_rx_aux_data
  A pointer to 1x or HDR data structure that is necessary to
...
*/
boolean
rf_cdma_nv_load_aux_rx_swpts
(
  rfnv_asdiv_rx_switch_points_type*       aux_nv_asdiv_gain_offset , 
  rf_cdma_rx_aux_static_type*             rf_cdma_rx_aux_data  
)
{
  boolean success = TRUE ;
   ...
   /* Load active switchpoint data for all gain steps */
   /* Populate for 1 linearity mode and then duplicate the data for other 
      linearity modes since the scalar data is applicable to all modes. The
      main NVs will dictate if the switchpoints are active or not, what kind
      of switchpoints are being used etc 
   */
   rf_cdma_rx_aux_data->cdma_aux_lna_info.lin_info[RFM_CDMA_POWER_MODE_NORMAL].
     lin_mode_active = TRUE ;

   for ( gain_step = RFM_LNA_GAIN_STEP_0_TO_1;
         gain_step < RFM_LNA_GAIN_STEP_NUM ; gain_step ++ )
   {
     /* Since the Aux NVs have only offsets only FIXED offsets are used. Note 
       the main NV can have OLTA switchpoints in which case after the final 
       switchpoints have been calculated, these switchpoints will be applied
       on top of switchpoints derived from OLTA switchpoints and then offset
       by these offsets  */
     rf_cdma_rx_aux_data->cdma_aux_lna_info.lin_info[RFM_CDMA_POWER_MODE_NORMAL].
       sp_info[gain_step].sp_mode
        = RF_CDMA_LNA_SWITCH_PT_MODE_FIXED ;

     rf_cdma_rx_aux_data->
       cdma_aux_lna_info.lin_info[RFM_CDMA_POWER_MODE_NORMAL].
       sp_info[gain_step].sp_data.fixed.fall =
     aux_nv_asdiv_gain_offset->switch_point_offset[gain_step].fall_offset ;
     rf_cdma_rx_aux_data->
       cdma_aux_lna_info.lin_info[RFM_CDMA_POWER_MODE_NORMAL].
       sp_info[gain_step].sp_data.fixed.rise =
     aux_nv_asdiv_gain_offset->switch_point_offset[gain_step].rise_offset ;
   }

   /* Now duplicate the data for all the other modes to allow easy indexing @ runtime*/
   for ( power_mode = ( RFM_CDMA_POWER_MODE_NORMAL + 1);
        power_mode < RFM_CDMA_POWER_MODE_MAX ;
        power_mode++ )
  {
    memscpy (  /* Dest */
       &(rf_cdma_rx_aux_data->cdma_aux_lna_info.lin_info[power_mode]) ,
       sizeof(rf_cdma_lna_lin_sp_info_type) , /* Dest Size */

       /* Source */
       &(rf_cdma_rx_aux_data->cdma_aux_lna_info.lin_info[RFM_CDMA_POWER_MODE_NORMAL]) ,
       sizeof(rf_cdma_lna_lin_sp_info_type) ) ; /* Src Size */
  }
   return success ;
}
...
*/
boolean  
rf_cdma_load_rx_aux_switchpoint_data 
( 
  rfnv_asdiv_rx_switch_points_type*     aux_asdiv_gain_offset , 
  rf_cdma_rx_aux_static_type*                  onex_data             ,
  rf_cdma_rx_aux_static_type*                  hdr_data 
) 
{
  ...
  if ( aux_asdiv_gain_offset != NULL )
  {
    /* 1x Aux data population */
    success =
       rf_cdma_nv_load_aux_rx_swpts ( aux_asdiv_gain_offset , onex_data ) ;

    /* HDR Aux data population */
    success =
       rf_cdma_nv_load_aux_rx_swpts ( aux_asdiv_gain_offset , hdr_data ) ;
  }
  else
  {
    RF_MSG ( RF_ERROR , "rf_cdma_load_aux_data(): Aux data is NULL " ) ;
  }
  return success ;
}
...
/*!
  @brief
  Loads if auxillary Rx offset data.
 
  @details
  On an antennae switch, if the new path has additional static
  offset offsets loaded into NVs. This function loads Rx offsets
  for adjustment for HDR or 1x.
...
*/
boolean
rf_cdma_nv_load_aux_rx_offsets
(
  rfnv_asdiv_rx_gain_offset_type*     aux_nv_asdiv_gain_offset , 
  rf_cdma_rx_aux_static_type*         rf_cdma_rx_aux_data  
)
{
  rfm_lna_gain_step_type gain_step = RFM_LNA_GAIN_STEP_0_TO_1 ;
  rf_cdma_rx_aux_data->rx_aux_info.switch_position = 
    aux_nv_asdiv_gain_offset->switch_position ;

  /* Load active switchpoint data for all gain steps */
  for ( ; gain_step < RFM_LNA_GAIN_STEP_NUM ; gain_step ++ )
  {
    rf_cdma_rx_aux_data->rx_aux_info.gain_offset[gain_step] = 
      aux_nv_asdiv_gain_offset->gain_offset[gain_step] ;
  }

  /* validate the flag so that during runtime sanitization is not needed */
  rf_cdma_rx_aux_data->is_rx_aux_info_valid = TRUE ;
  return TRUE ;
}
...
*/
boolean  
rf_cdma_load_rx_aux_offset_data 
( 
  rfnv_asdiv_rx_gain_offset_type*     aux_asdiv_gain_offset , 
  rf_cdma_rx_aux_static_type*         onex_data             ,
  rf_cdma_rx_aux_static_type*         hdr_data 
) 
{
  ...
  if ( aux_asdiv_gain_offset != NULL )
  {
    ...
    { 
      /* 1x Aux data population */
      success =
        rf_cdma_nv_load_aux_rx_offsets ( aux_asdiv_gain_offset , onex_data ) ;

      /* HDR Aux data population */
      success =
        rf_cdma_nv_load_aux_rx_offsets ( aux_asdiv_gain_offset , hdr_data ) ;
    }
  }
  else
  {
    RF_MSG ( RF_ERROR , "rf_cdma_load_rx_aux_offset_data(): Aux data is NULL " ) ;
  }
  return success ;
}
...
boolean
rf_cdma_nv_load_unified_rx_static
(
  rf_1x_rx_static_type* onex_rx_static_data,
  rf_hdr_rx_static_type* hdr_rx_static_data,

  /* To compensate Tx because of Rx factors */
  rf_1x_tx_static_type* onex_tx_static_data,
  rf_hdr_tx_static_type* hdr_tx_static_data,

  const rfnv_item_id_enum_type rx_static_data_item,
  const rf_cdma_nv_path_type nv_path,
...
)
{
  ...
  uint8 *rx_static_nv_data_ptr = NULL;
  /* Helper */
  ...
  /* actual NV data size after decompression for Rx static NV */
  /* (1x+DORevA+RevBSC+RevBMC + [3 constant LNA gain adjustment items]) * 6 Rx chains * size of switch point type + 
  (1x + HDR) * 6 * size of spur table*/
  uint32 rx_static_data_size = 
    RF_CDMA_RX_STATIC_NV_MAX_SIZE   * 4 * sizeof(rfnv_rx_swpt_type) +
    RF_CDMA_MAX_RX_CHAINS_SUPPORTED * 2 * sizeof(rfcommon_spur_table_type) ;
  ...
  boolean onex_swpt_loaded = FALSE;
  boolean hdrRev0A_swpt_loaded = FALSE;
  boolean hdrRevB_SC_swpt_loaded = FALSE;
  boolean hdrRevB_MC_swpt_loaded = FALSE;


  /* Initialize the Aux Data to 0 */
  memset ( &(onex_rx_static_data->onex_common_rx_aux_data) , 0 , 
           sizeof(rf_cdma_rx_aux_static_type) ) ;
  memset ( &(hdr_rx_static_data->hdr_common_rx_aux_data) , 0 , 
           sizeof(rf_cdma_rx_aux_static_type) ) ;
  ...
  {
    RF_MSG_1( RF_ERROR, "rf_cdma_nv_load_unified_rx_static() " 
              "- failed to allocate memory for %d bytes.", nv_item_size ) ;
    api_status = FALSE;
  }
  ...
  if( api_status == TRUE )
  {
    ...
    nv_read_status = rfcommon_rfnv_get_item(rx_static_data_item, 
                                            nv_data_raw_ptr,
                                            &nv_item_size,
                                            NULL,
                                            (rex_sigs_type) 0,
    ...
    {
      RF_MSG_1( RF_MED, "rf_cdma_nv_load_unified_rx_static() - "
                        "NV ID %d not active." , rx_static_data_item ) ;
      api_status = FALSE;
    }
    else if (nv_read_status == NV_FAIL_S)
    {
      RF_MSG_1( RF_ERROR, "rf_cdma_nv_load_unified_rx_static() - "
                          "NV ID %d read failed.", rx_static_data_item );
      api_status = FALSE;
    }
  }
  ...
  if( api_status == TRUE )
  {
    ...
    rx_static_nv_data_ptr = modem_mem_alloc(rx_static_data_size, MODEM_MEM_CLIENT_RFA);
  
    if (rx_static_nv_data_ptr == NULL)
    {
      RF_MSG_1( RF_ERROR, "rf_cdma_nv_load_unified_rx_static() - failed "
                          "to allocate memory for %d bytes.", rx_static_data_size ) ;
      api_status = FALSE;
    }
  }
  ...
  {
    /* Decompress NV data */
    uncompress_status = uncompress( (uint8 *)rx_static_nv_data_ptr,   
                                    &rx_static_data_size,
    ...
    {
      RF_MSG_1( RF_ERROR, "rf_cdma_nv_load_unified_rx_static() - "
                "decompression of NV ID %d failed.", rx_static_data_item ) ;
      api_status = FALSE;
    }
  }
  ...
  if( api_status == TRUE )
  {
    ...
    */ 
    ...
    if( (rx_static_data_size - num_bytes_parsed) < nv_element_size )
    {
      RF_MSG( RF_ERROR, "CDMA unified Rx static NV - Not enough elements present to extract "
  ...
  if( api_status == TRUE )
  {
    ...
    temp_data_ptr = rx_static_nv_data_ptr + num_bytes_parsed;
    ...
    {
      RF_MSG( RF_ERROR, "CDMA unified Rx static NV - Number of elements value of 0 "
              "is invalid.");
      api_status = FALSE;
    }
  }
  ...
  {  
    ...
    structure */
    rfnv_rx_swpt_type* rx_swpt_nv_ptr;
    rfcommon_spur_table_type* spur_nv_ptr;
    int tbl_entry;
    boolean aux_data_rx_offset_status = FALSE , aux_data_rx_swpts_status = FALSE , 
      aux_data_tx_offset_status = FALSE ; 

    /* Auxillary data holders */
    rf_cdma_rx_aux_static_type* onex_data = 
      &(onex_rx_static_data->onex_common_rx_aux_data) ;
    rf_cdma_rx_aux_static_type* hdr_data = 
      &(hdr_rx_static_data->hdr_common_rx_aux_data) ;


    for( type_id_idx = 0; 
         (type_id_idx < variant_marker.num_elements) && (api_status == TRUE); 
         type_id_idx++ )
    {
      ...
      if( (rx_static_data_size - num_bytes_parsed) < nv_element_size )
      {
        RF_MSG_1( RF_ERROR, "CDMA unified Rx static NV - Not enough elements left to extract "
      ...
      temp_data_ptr = rx_static_nv_data_ptr + num_bytes_parsed;
      temp_num_bytes_left = rx_static_data_size - num_bytes_parsed;
      if( temp_num_bytes_left <= 0 )
      {
        RF_MSG_2( RF_ERROR, "CDMA unified Rx static NV - "
                            "Not elements left to extract "
      ...
      switch( type_id )
      {
        case RFNV_DATA_TYPE_1X_LNA_SWITCHPOINTS:

          rx_swpt_nv_ptr = (rfnv_rx_swpt_type *)temp_data_ptr;

          api_status = rf_cdma_nv_load_rx_swpts(rx_swpt_nv_ptr,
                                            &(onex_rx_static_data->lna_info) );
          onex_swpt_loaded = TRUE;
           
          nv_element_size = sizeof(rfnv_rx_swpt_type);

          break;
        case RFNV_DATA_TYPE_DO_REVA_LNA_SWITCHPOINTS:

          rx_swpt_nv_ptr = (rfnv_rx_swpt_type *)temp_data_ptr;

          api_status = rf_cdma_nv_load_rx_swpts(rx_swpt_nv_ptr,
                                            &(hdr_rx_static_data->lna_info_dor0_a) );
          hdrRev0A_swpt_loaded = TRUE;

          nv_element_size = sizeof(rfnv_rx_swpt_type);

          break;
        case RFNV_DATA_TYPE_DO_REVB_SC_LNA_SWITCHPOINTS:

          rx_swpt_nv_ptr = (rfnv_rx_swpt_type *)temp_data_ptr;

          api_status = rf_cdma_nv_load_rx_swpts(rx_swpt_nv_ptr,
                                            &(hdr_rx_static_data->lna_info_dorb_sc) );
          hdrRevB_SC_swpt_loaded = TRUE;

          nv_element_size = sizeof(rfnv_rx_swpt_type);

          break;
        case RFNV_DATA_TYPE_DO_REVB_MC_LNA_SWITCHPOINTS:
          rx_swpt_nv_ptr = (rfnv_rx_swpt_type *)temp_data_ptr;

          api_status = rf_cdma_nv_load_rx_swpts(rx_swpt_nv_ptr,
                                            &(hdr_rx_static_data->lna_info_dorb_mc) );
          hdrRevB_SC_swpt_loaded = TRUE;

          nv_element_size = sizeof(rfnv_rx_swpt_type);

          break;
        case RFNV_DATA_TYPE_1X_SPUR_TABLE:
          /* If spur table read successfully, load the data structure */
          spur_nv_ptr = (rfcommon_spur_table_type *)temp_data_ptr;

          for ( tbl_entry = 0; tbl_entry < RFCOMMON_MAX_SPUR_ENTRIES; tbl_entry++ )
          {
            onex_rx_static_data->onex_spur_table.rfcommon_spur_tbl[tbl_entry].spur_at_freq_hz = 
                                spur_nv_ptr->rfcommon_spur_tbl[tbl_entry].spur_at_freq_hz;

            onex_rx_static_data->onex_spur_table.rfcommon_spur_tbl[tbl_entry].notch_depth = 
                                spur_nv_ptr->rfcommon_spur_tbl[tbl_entry].notch_depth;
          }              

          nv_element_size = sizeof(rfcommon_spur_table_type);

          break;
        case RFNV_DATA_TYPE_DO_SPUR_TABLE:        
          ...
          for ( tbl_entry = 0; tbl_entry < RFCOMMON_MAX_SPUR_ENTRIES; tbl_entry++ )
          {
            hdr_rx_static_data->hdr_spur_table.rfcommon_spur_tbl[tbl_entry].spur_at_freq_hz = 
                                 spur_nv_ptr->rfcommon_spur_tbl[tbl_entry].spur_at_freq_hz;

            hdr_rx_static_data->hdr_spur_table.rfcommon_spur_tbl[tbl_entry].notch_depth = 
                               spur_nv_ptr->rfcommon_spur_tbl[tbl_entry].notch_depth;
          }          
        ...
        /* RFNV_DATA_TYPE_ASDIV_RX_GAIN_OFFSET, 
           RFNV_DATA_TYPE_ASDIV_RX_SWITCH_POINTS & 
           RFNV_DATA_TYPE_ASDIV_TX_GAIN_OFFSET   
           All these 3 NV types must be present. All the values can be 0.
        */
        case RFNV_DATA_TYPE_ASDIV_RX_GAIN_OFFSET   : 
        {
          rfnv_asdiv_rx_gain_offset_type* aux_asdiv_gain_offset
             = (rfnv_asdiv_rx_gain_offset_type *)temp_data_ptr ;

          /* Aux data can fail loading but the NV can still be used  */
          api_status &= TRUE ; 

          aux_data_rx_offset_status = 
           rf_cdma_load_rx_aux_offset_data ( aux_asdiv_gain_offset ,
                                                          onex_data , hdr_data ) ;
          if ( aux_data_rx_offset_status == FALSE )
          {
            RF_MSG ( RF_ERROR , "rf_cdma_load_unified_rx_static(): "
                     "Call to rf_cdma_load_rx_aux_offset_data() failed" ) ;
          }
        ...
        {
          rfnv_asdiv_rx_switch_points_type* aux_sp_data =
            (rfnv_asdiv_rx_switch_points_type*) temp_data_ptr ;

          /* Aux data can fail loading but the NV can still be used  */
          api_status &= TRUE ; 

          aux_data_rx_swpts_status =
            rf_cdma_load_rx_aux_switchpoint_data ( aux_sp_data , onex_data ,
                                                              hdr_data ) ; 
          if ( aux_data_rx_swpts_status == FALSE )
          {
            RF_MSG ( RF_ERROR , "rf_cdma_load_unified_rx_static(): "
                     "Call to rf_cdma_load_rx_aux_switchpoint_data() failed" ) ;
          }
        ...
        {
          /* Auxillary data holders */
          rf_cdma_tx_aux_static_type* onex_tx_data = 
            &(onex_tx_static_data->onex_common_tx_aux_data) ;
          rf_cdma_tx_aux_static_type* hdr_tx_data = 
            &(hdr_tx_static_data->hdr_common_tx_aux_data) ;
          rfnv_asdiv_tx_gain_offset_type* aux_asdiv_gain_offset
             = (rfnv_asdiv_tx_gain_offset_type *)temp_data_ptr ;

          /* Aux data can fail loading but the NV can still be used  */
          api_status &= TRUE ; 
          aux_data_tx_offset_status =
            rf_cdma_load_tx_aux_offset_data ( aux_asdiv_gain_offset ,
                                              onex_tx_data , hdr_tx_data ) ;
          if ( aux_data_tx_offset_status == FALSE )
          {
            RF_MSG ( RF_ERROR , "rf_cdma_load_unified_rx_static(): "
                     "Call to rf_cdma_load_tx_aux_offset_data() failed" ) ;
          }
        ...
        default:
          {
            RF_MSG_2( RF_ERROR, "CDMA unified Rx static NV - "
                                "element of type ID %d not supported "
      ...
      {
        RF_MSG_2( RF_ERROR, "CDMA unified Rx static NV - Error extracting "
  ...
  if ( api_status == TRUE )
  {
    /* -------------  Switch point post processing -------------*/
    if ( onex_swpt_loaded == TRUE && hdrRev0A_swpt_loaded == FALSE )
    {
      /* If Rev0/RevA NV items are inactive, the switchpoint data
      must default to the data that is loaded for CDMA voice support i.e.
      1x Rx switchpoints */
      rf_cdma_nv_copy_onex_swpt_to_hdr_rev0_a( onex_rx_static_data, hdr_rx_static_data );
    }

    if ( onex_swpt_loaded == TRUE && hdrRevB_SC_swpt_loaded == FALSE )
    {
      /* If RevB SingleCarrier NV items are inactive, the switchpoint data
      must default to the data that is loaded for HDR Rev0/RevA switchpoints */
      rf_cdma_nv_copy_hdr_rev_0_a_swpt_to_hdr_revb_sc( hdr_rx_static_data );
    }

    if ( onex_swpt_loaded == TRUE && hdrRevB_MC_swpt_loaded == FALSE )
    {
      /* If RevB MultiCarrier NV items are inactive, the switchpoint data
      must default to the data that is loaded for HDR RevB SC switchpoints */
      rf_cdma_nv_copy_hdr_rev_b_sc_swpt_to_hdr_rev_b_mc( hdr_rx_static_data );
    }
  }/* if ( api_status == TRUE ) */
  ...
  if( rx_static_nv_data_ptr != NULL )
  {
    modem_mem_free( rx_static_nv_data_ptr, MODEM_MEM_CLIENT_RFA );
  }
...
/*!
  @brief
  Return the Tx configuration data for a given path and band.

  @details
  This function will return a pointer to the
  rf_cdma_tx_static_type structure that contains the
  configuration data for the requested device when operating on
  the requested band.
  ...
  chain and band.
*/
const rf_cdma_tx_static_type*
rf_cdma_nv_get_tx_static_data
(
  const rfm_device_enum_type device ,
  rfm_cdma_band_class_type band
)
{
  const rf_cdma_tx_static_type* tx_cdma_config_ptr = NULL ;
  const rfc_cdma_logical_device_params_type* logical_dev = 
    rfc_cdma_get_logical_device_params ( device );
  if ( NULL ==  logical_dev )
  {
    RF_MSG_1( RF_ERROR, "rf_cdma_nv_get_tx_static_data: Dev[ %d ] CDMA Common NV", 
                  device ) ; 
  }
  else
  {
    ...
    {
      RF_MSG_1( RF_ERROR, "rf_cdma_nv_get_tx_static_data: Dev[ %d ] Failed NV",
                device ); 
    } /* if ( NULL == rf_1x_config_ptr ) */
    else
    {
      ...
      else
      {
        ...
        {
          RF_MSG_1( RF_ERROR, "rf_cdma_nv_get_tx_static_data: Dev[ %d ] CDMA Tx NV",
                    device ); 
        } /* if ( NULL == tx_cdma_config_ptr ) */
      } /* if ( NULL == rf_1x_config_ptr->cdma_common_ptr ) */
    } /* ( NULL == rf_1x_config_ptr ) */
  } /* ( NULL == logical_device ) */
  return tx_cdma_config_ptr ;
} /* rf_cdma_nv_get_tx_static_data */
...
const
rf_cdma_tx_static_type*
rf_cdma_nv_get_tx_static_xpt_config
(
  rf_cdma_nv_path_type path,
  rfm_cdma_band_class_type band
)
{
  ...
  if ( cdma_config_ptr == NULL )
  {
    RF_MSG_1( RF_ERROR, "cdma_config_ptr get from static NV is NULL for band %d", band );
    return NULL;
  }
  return &cdma_config_ptr->tx_static;
} /* rf_cdma_nv_get_tx_static_xpt_config */
...
/*!
  @brief
  API function to get point to SAR state lim_vs_temp data by band and antenna
  position. There are two NV SAR data, old and new. The function tries new NV
  before the old one.

  @param rf_cdma_config_data_ptr
  Data struct point to old NV SAR data
...
int16 *
rf_cdma_nv_get_asdiv_sar_lim
(
  void* rf_cdma_config_data_ptr,
  rfcommon_asdiv_position_enum_type ant_pos
)
{
  int16 *retval;
  ...
  retval = (int16*)((rf_cdma_config_type*)rf_cdma_config_data_ptr)->
                                  tx_static.lim_vs_temp[ant_pos][sar_state];
  return retval;
}
...
/*!
  @brief
  API function to get point to SAR state SAR raw data by band and antenna
  position.  

  @param rf_cdma_config_data_ptr
  Data struct point to NV SAR data
...
uint16 *
rf_cdma_nv_get_asdiv_sar_value
(
  rf_cdma_config_type* rf_cdma_config_data_ptr,
  rfcommon_asdiv_position_enum_type ant_pos
)
{
  uint16 *sar_raw = 0;
  ...
  if (( sar_state < RF_SAR_STATE_MAX ) &&
      ( ant_pos < ASDIV_POSITION_MAX ) &&
      ( NULL != rf_cdma_config_data_ptr ))
  {
     sar_raw = (uint16 *) &(rf_cdma_config_data_ptr->tx_static.sar_tbl[ant_pos][sar_state][0]);
  }
...
rf_cdma_config_device_index_type*
rf_cdma_get_config_device_data
(
void
)
{
return &rf_cdma_config_device_index[0];
}
#endif
