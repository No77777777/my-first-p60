...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/common/rf/src/rfm_cdma_ftm.c#1 $
...
04/27/15   zhw     Added Tx split support in FTM logging
04/16/15   zhw     Use Tx device to set et path in case Drx/FB device is empty
04/09/15   zhw     Rewrite 1x xpt config using new RFC mapping
03/06/15   spa     Added support to tune Tx PLL to rx frequency
03/04/15   spa     set et path for primary device in config fb transceiver
...
)
{
  ...
  {
    RF_MSG_1( RF_ERROR, "rfdevice_hdet_cdma_multi_read_hdet: NULL "
...
*/
rfm_device_enum_type
rfm_cdma_ftm_log_search_transmit_device
(
  rfm_mode_enum_type mode
)
{
  ...
  for ( dev_idx = RFM_DEVICE_0; dev_idx < RFM_MAX_DEVICES; dev_idx++ )
  {
    ...
    if ( dev_status != NULL )
    {
      ...
      if ( ( dev_status->rf_mode == mode ) &&
           ( ( dev_status->data_1x.rf_state == RF_CDMA_STATE_TX )
             ...
             ( dev_status->data_1x.rf_state == RF_CDMA_STATE_RXTX )
             ||
             ( dev_status->data_hdr.rf_state == RF_CDMA_STATE_TX )
             ||
             ( dev_status->data_hdr.rf_state == RF_CDMA_STATE_RXTX ) )
         )
      {
    ...
    {
      RF_MSG_1( RF_HIGH, "rfm_cdma_ftm_log_search_transmit_device: Dev %d NULL"
                 "Data", dev_idx );
      device = RFM_INVALID_DEVICE;
    }
  } /* for ( dev_idx = RFM_DEVICE_0; dev_idx < RFM_MAX_DEVICES; dev_idx++ ) */
...
)
{
  ...
  RF_MSG_4 ( RF_MED , "rfm_cdma_ftm_config_fb_asm(): Status[%d] "
             "FB Dev: %d , band: %d , et_enable: %d" , ret_val ,
              device_fb , band , et_enable ) ;
...
)
{
  ...
  rfdevice_rxtx_common_class* tx_device_trx_tranceiver = NULL;
  rfdevice_rxtx_common_class* fb_device_trx_tranceiver = NULL;
  ...
  if ( logical_dev == NULL )
  {
    RF_MSG_1( RF_ERROR, "rfm_cdma_ftm_config_fb_transceiver: NULL logical"
                        "data for Tx device %d ", device );
    return FALSE;
  ...
  {    /* Get Tx coupler scripts */
    ...
    coupler_cfg.port = 0;
    ret_val &= rf_cdma_config_coupler( logical_dev->coupler_obj[band],
                                       coupler_cfg,
                                       NULL,
                                       RFDEVICE_EXECUTE_IMMEDIATE,
                                       0);
  ...
  else if ( logical_dev->ant_tuner[band] != NULL )
  {
    ...
    tuner_config.chan_num = (uint32)tx_chan;
    tuner_config.carrier_id = RFCMN_ATUNER_CARRIER_ID_0;
    tuner_config.rx_tx = RFCMN_ATUNER_TX_DEVICE;
    tuner_config.freq = rf_cdma_get_tx_carrier_freq(band_in_common_format.cdma_band,tx_chan);
  ...
  /*  WTR set up */
  ret_val &= rfc_cdma_get_fb_tranceivers( device, device_fb, band,
                                         &tx_device_trx_tranceiver,
                                         &fb_device_trx_tranceiver );

  if ( tx_device_trx_tranceiver == NULL )
  {
    RF_MSG( RF_ERROR, "rfm_cdma_ftm_config_fb_transceiver: "
                      "NULL Transceiver object");
    return FALSE;
  }

  /* FB Rx functionality */
  ret_val &= rfcommon_fbrx_mc_enable_rf_dev( tx_device_trx_tranceiver, fb_state );

  /* Cross-WTR FBRX Capture support */
  if (ret_val == TRUE && fb_device_trx_tranceiver != NULL )
  {
    if ( tx_device_trx_tranceiver != fb_device_trx_tranceiver )
    {
      rfdevice_trx_cdma_tx *fb_tranceiver_tx_object = NULL;

      fb_tranceiver_tx_object = (rfdevice_trx_cdma_tx*)rfdevice_cmn_get_tech_device(
                                             fb_device_trx_tranceiver,
                                             RFM_1X_MODE,
                                             RFDEVICE_TX,
                                             RFDEVICE_TRX_PATH_0);

      if (fb_tranceiver_tx_object!=NULL)
      {
        /* Acquire Critical Section Lock */
        rfdevice_cdma_tx_enter_critical_section(fb_tranceiver_tx_object);

        ret_val &= rfdevice_cdma_tx_tune_default_port(fb_tranceiver_tx_object,
                                                      RFM_1X_MODE,
                                                      (rfm_cdma_band_class_type)band,
                                                      (uint32)tx_chan,
                                                      1250,
        ...
        /* Release Critical Section Lock */
        rfdevice_cdma_tx_leave_critical_section(fb_tranceiver_tx_object);

        ret_val &= TRUE;
      }

      RF_MSG( RF_LOW, "Different FBRX tranceiver than TX " );
    }/* if ( tx_device_trx_tranceiver != fb_device_trx_tranceiver )  */
  }/* if ( ret_val == TRUE ) */
...
)
{
  ...
  /*  WTR set up */
  ret_val &= rfc_cdma_get_fb_tranceivers( device, device_fb,
                                          rf_cdma_data_get_curr_band(device),
                                          &tx_device_trx_tranceiver,
                                          &fb_device_trx_tranceiver );

  if ( tx_device_trx_tranceiver == NULL || fb_device_trx_tranceiver == NULL )
  {
    RF_MSG( RF_ERROR, "rfm_cdma_ftm_deconfig_fb_transceiver: "
                      "NULL Transceiver object");
    return FALSE;
  }

  ret_val &= rfcommon_fbrx_mc_disable_rf_dev( fb_device_trx_tranceiver );

  /* Call device interface to configure the ET Path */
  /*ret_val &= */ rfdevice_cmn_set_et_path( fb_device_trx_tranceiver,
                                       mode,
...
{
  return (
     rfm_cdma_mc_test_rx_on_off ( force_microsleep_enable_disable ,
...
/*!
  @brief
  Call CDMA MC API to override tx PLL

  @param rfm_device_enum_type: device to override PLL on

  @param override_chan: Rx channel to override PLL to

  @retval
  TRUE if override was success,
  FALSE if override failed

*/

boolean
rfm_cdma_ftm_tx_pll_override
(
  rfm_device_enum_type device,
  uint16 override_chan
)
{
  return ( rf_cdma_mc_tx_pll_override ( device , override_chan ) ) ;
}
