...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/common/rf/src/rf_cdma_temp_comp.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
03/20/17   rs      Cleared therm pending flags for sleep_tx
...
)
{
  ...
  if ( api_status == TRUE )
  {
    ...
    api_status &= rf_cdma_mdsp_configure_devices( device ,
                                                  ...
                                                  &event_handle,
                                                  (rf_cdma_mdsp_get_event_info(RFLM_CDMA_MDSP_CCS_EVENT_THERM_CONFIG_ID)) ,
                                                  therm_enable_script, 
                                                  0,
...
)
{
  ...
  if ( temp_comp_data->mode == RFM_1X_MODE )
  {
    ...
    therm_data = &rflm_c2k_get_data()->onex_data.therm_data;
  }
  else if ( temp_comp_data->mode == RFM_1XEVDO_MODE )
  {
    ...
    therm_data = &rflm_c2k_get_data()->hdr_data.therm_data;
  }
  else
  {
  ...
  if ( !( (dev_status_w->data_1x.rf_state == RF_CDMA_STATE_RXTX || 
           dev_status_w->data_1x.rf_state == RF_CDMA_STATE_TX) 
          ||
          (dev_status_w->data_hdr.rf_state == RF_CDMA_STATE_RXTX ||
           dev_status_w->data_hdr.rf_state == RF_CDMA_STATE_TX) )
       ||
  ...
  else
  {
    ...
    if ( temp_comp_data->quick_poll_countdown_timeout < 0 ) 
    {
      ...
      therm_data->therm_pending = FALSE;
      rf_common_leave_critical_section( &temp_comp_data->crit_section );
    ...
    else 
    {
      ...
      {
        RF_MSG_1( RF_LOW, "rf_cdma_temp_comp_read_handler: Therm not ready. Retry %d times",
                  temp_comp_data->quick_poll_countdown_timeout);      
      ...
      else
      {
        ...
        RF_MSG( RF_LOW, "rf_cdma_temp_comp_read_handler: MDSP Therm ready. Stop polling");
      }
    }/* if ( temp_comp_data->quick_poll_countdown_timeout >= 0 )  */
  ...
  *therm_read_in_process = FALSE;
  ...
  RF_MSG( RF_LOW, "rf_cdma_temp_comp_read_handler: Temp comp release!" );
  /* Turn on FED Flag */
