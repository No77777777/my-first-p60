...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/common/rf/src/rf_cdma_mc.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
01/24/20   aa      HA QSH Minidump Changes 
10/29/19   rs      Delay addition for QET5102 programming during tx tune
05/03/18   am      Fix PA & ASM programming sequence during tx sleep
03/09/18   mk      KW Errors fix
02/28/18   am      QAT3522 xsw tech side changes
01/30/18   am      reduced F3s further
...
11/02/15   pk      Use Script based write for XSW to fix DO2G QTA
11/02/15   rs      KW error Fix, added max band check
...
01/23/15   vr      Modified the condition in the set antenna position API to 
08/30/15   hj      Add tuner disable to rf_cdma_mc_configure_front_end_for_rx_sleep() for IRAT
08/31/15   wwl     Add autopin init function
08/17/15   zhw     Move 1x/HDR meas tune-away & reset script into private 
                   buffers
08/19/15   hm      Add API rf_cdma_mc_update_asm_state
08/17/15   zhw     Added support for explicit meas disable script API
08/10/15   zhw     Use TGT PRx Device for TQ Device if SRC Device is invalid
08/09/15   pk      Changing Error MSG to Low MSG in 
...
07/08/15   vc      Use 1x enum when querying alt path for HDR
06/19/15   pk      Remove extra F3 in rf_cdma_mc_is_band_chan_supported API
06/19/15   vc      Enabled the AsDiv Abort API
...
05/27/15   zhw     KW Fix
05/19/15   zhw     Skip IC control in QTA if IC is disabled
...
04/22/15   zhw     Temporarily config script in config_rx_wakeup helper for 
                   DO->G QTA
04/15/15   zhw     Integrate device driver change with CDMA
                   Remove deprecated API
04/09/15   zhw     Configure Tx on/off script regardless of Cal
...
02/06/15   vr[pl]  Modified the condition in the set antenna position API to 
...
12/16/14   spa     ELNA wakeup/sleep support 
...
extern ftm_cdma_data_type ftm_cdma_data;
...
)
{
  ...
  RF_MSG_1( RF_HIGH, "rf_cdma_mc_init: START. cmn success %d", common_init_successful );
  ...
  */
  ...
  {
    RF_MSG( RF_ERROR, "rf_cdma_mc_init: CDMA not initialized due to cmn init failure. Cannot operate in CDMA." );
    return FALSE;
  }
    ...
    if ( dev_status_w != NULL )
    {
      ...
      if ( logical_dev != NULL )
      {
        ...
        /* Initialize Temp Compensation */
          ...
          /* Initialize to TRUE to enable freq comp and temp comq together*/
      ...
      {
        RF_MSG_1 ( RF_ERROR , "rf_cdma_mc_init(): " 
             "Null logical device for RFM device: %d" , dev_idx );
      }
  ...
  ret_val = rf_qsh_cb_register(RFM_1X_MODE,&rf_cdma_QSH_mdump_cb);
  if(FALSE == ret_val)
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR, "MC Init register QSH minidump callback failed for 1x.");
  }

  ret_val = rf_qsh_cb_register(RFM_1XEVDO_MODE,&rf_cdma_QSH_mdump_cb);
  if(FALSE == ret_val)
  {
    MSG(MSG_SSID_RF, MSG_LEGACY_ERROR, "MC Init register QSH minidump callback failed for HDR.");
  }
#endif

  if(rfcommon_autopin_is_enabled(RFM_1X_MODE) ||
     rfcommon_autopin_is_enabled(RFM_1XEVDO_MODE))
  {
    rf_cdma_auto_pin_cmd_proc_init();
  }

  RF_MSG_1( RF_HIGH, "rf_cdma_mc_init: END. success %d", init_status );
...
)
{
  ...
  if ( rx == 0 || tx == 0 )
  {
    ...
    RF_MSG_DBG_2 ( RF_LOW , "rf_cdma_mc_is_band_chan_supported:"
  ...
  else
  {
    ...
    if ( FALSE == success )
    {
      RF_MSG_3 ( RF_LOW , "rf_cdma_mc_is_band_chan_supported()" 
                 "FAILED device[%d] band[%d] chan[%d] " ,
                 device ,  band , chan ) ;
    }
    ...
    {
      RF_MSG_DBG ( RF_ERROR , "rf_cdma_mc_is_band_chan_supported(): "  
    ...
    if ( api_status == TRUE )
    {
      ...
      api_status &= rf_cdma_mdsp_configure_devices( device ,
                    logical_dev->tx_mdsp_path,
                                                    ...
                                                    rf_on_event_handle,
                                                    (rf_cdma_mdsp_get_event_info(RFLM_CDMA_MDSP_CCS_EVENT_RF_ON_ID)) ,
                                                    on_settings, 
                                                    buffer_index,
    ...
    if ( api_status == TRUE )
    {
      ...
      api_status &= rf_cdma_mdsp_configure_devices( device ,
                    logical_dev->tx_mdsp_path,
                                                    ...
                                                    rf_off_event_handle,
                                                    (rf_cdma_mdsp_get_event_info(RFLM_CDMA_MDSP_CCS_EVENT_RF_OFF_ID)) ,
                                                    off_settings, 
                                                    buffer_index,
                    rf_cdma_data_get_txlm_handle (device) ,
                                                    NULL,
    ...
    if ( api_status == TRUE )
    {
      ...
      api_status &= rf_cdma_mdsp_configure_devices( device ,
                    logical_dev->rx_mdsp_path,
                                                    ...
                                                    rx_on_event_handle,
                                                    (rf_cdma_mdsp_get_event_info(RFLM_CDMA_MDSP_CCS_EVENT_RX_ON_ID)) ,
                                                    rx_on_settings, 
                                                    buffer_index,
                    rf_cdma_data_get_rxlm_handle (device) ,
                                                    NULL,
    ...
    if ( api_status == TRUE )
    {
      ...
      api_status &= rf_cdma_mdsp_configure_devices( device ,
                    logical_dev->rx_mdsp_path,
                                                    ...
                                                    rx_off_event_handle,
                                                    (rf_cdma_mdsp_get_event_info(RFLM_CDMA_MDSP_CCS_EVENT_RX_OFF_ID)) ,
                                                    rx_off_settings, 
                                                    buffer_index,
                    rf_cdma_data_get_rxlm_handle (device) ,
                                                    NULL,
...
)
{
  boolean api_status = TRUE ; 
  /* Buffr object to hold On scripts */
  rf_buffer_intf* rx_deactivate_milli_sleep_settings = NULL ;
  /* Buffr object to hold Off scripts */
  rf_buffer_intf* rx_activate_milli_sleep_settings = NULL ;
  const rfc_cdma_logical_device_params_type* logical_dev; /* Logical Device */
  rfcommon_mdsp_event_handle** rx_activate_milli_sleep_event_handle = NULL ;

  /*Get RF ON event handle */
  rfcommon_mdsp_event_handle** rx_deactivate_milli_sleep_event_handle =
    rf_cdma_data_get_event_handle( device, RF_CDMA_EVENT_RX_DEACTIVATE_MILLI_SLEEP ); 

  /* Check for NULL event handle*/
  if ( rx_deactivate_milli_sleep_event_handle == NULL )
  {
    RF_MSG( RF_ERROR, "rf_cdma_mc_configure_rx_milli_sleep: "
            "Null Rx on event handle");
    return FALSE;
  }

  /* Get RF OFF event handle */
  rx_activate_milli_sleep_event_handle =
    rf_cdma_data_get_event_handle( device, RF_CDMA_EVENT_RX_ACTIVATE_MILLI_SLEEP );

  /* Check for NULL event handle*/
  if ( rx_activate_milli_sleep_event_handle == NULL )
  {
    RF_MSG( RF_ERROR, "rf_cdma_mc_configure_rx_milli_sleep: "
            "Null Rx off event handle");
    return FALSE;
  ...
  /* Create generic buffer interface object to hold grfc settings 
     for Rx ON*/
  rx_deactivate_milli_sleep_settings = 
    rf_buffer_create ( CFW_RF_EVENT_MAX_NUM_OF_SBI , 
                       CFW_RF_EVENT_MAX_NUM_OF_RFFE , 
                       RF_CDMA_MAX_GRFC_SCRIPT_SIZE ) ;

  /*Check if valid pointer was returned for buffer object*/
  if( rx_deactivate_milli_sleep_settings == NULL )
  {
    RF_MSG( RF_ERROR, "rf_cdma_mc_configure_rx_milli_sleep: "
                      "Unable to create buffer object for on settings");
    api_status = FALSE;
  ...
  {
    /* Get scripts for Rx ON */
    api_status = rfdevice_cdma_get_discontinuous_rx_script(
                                                  logical_dev->rx_device[band], 
                                                  band , TRUE,/*Flag for Rx ON*/ 
                                                  rx_deactivate_milli_sleep_settings ) ;
    ...
    if ( api_status == TRUE )
    {
      ...
      api_status &= rf_cdma_mdsp_configure_devices( device ,
                    logical_dev->rx_mdsp_path,
                                                    ...
                                                    rx_deactivate_milli_sleep_event_handle,
                       (rf_cdma_mdsp_get_event_info(RFLM_CDMA_MDSP_CCS_EVENT_RX_DEACTIVATE_MILLI_SLEEP_ID)) ,
                                                    rx_deactivate_milli_sleep_settings, 
                                                    buffer_index,
                    rf_cdma_data_get_rxlm_handle (device) ,
                                                    NULL,
  ...
  if ( rx_deactivate_milli_sleep_settings != NULL )
  {
    rf_buffer_destroy( rx_deactivate_milli_sleep_settings ) ;
  }

  /*-----------------------Rx OFF EVENT----------------------------*/
  /* Create generic buffer interface object to hold settings for Rx OFF*/
  rx_activate_milli_sleep_settings = rf_buffer_create ( CFW_RF_EVENT_MAX_NUM_OF_SBI , 
                                    CFW_RF_EVENT_MAX_NUM_OF_RFFE , 
                                    RF_CDMA_MAX_GRFC_SCRIPT_SIZE ) ;

  /*Check if valid pointer was returned for buffer object*/
  if( rx_activate_milli_sleep_settings == NULL )
  {
    RF_MSG( RF_ERROR, "rf_cdma_mc_configure_rx_milli_sleep: "
                      "Unable to create buffer object for off settings");
    api_status = FALSE;
  ...
  else
  {
    ...
    api_status = rfdevice_cdma_get_discontinuous_rx_script(
                                                   logical_dev->rx_device[band] , 
                                                   band , FALSE,/*Flag for Rx off*/
                                                   rx_activate_milli_sleep_settings ) ;
      ...
      api_status &= rf_cdma_mdsp_configure_devices( device ,
                    logical_dev->rx_mdsp_path,
                        ...
                        RFLM_CDMA_MDSP_CCS_EVENT_RX_ACTIVATE_MILLI_SLEEP_ID , 
                                        rx_activate_milli_sleep_event_handle ,
                   (rf_cdma_mdsp_get_event_info(RFLM_CDMA_MDSP_CCS_EVENT_RX_ACTIVATE_MILLI_SLEEP_ID)) ,
                                            rx_activate_milli_sleep_settings , 
                                                    buffer_index             ,
                    rf_cdma_data_get_rxlm_handle (device) ,
                                                    NULL,
    ...
    {
      RF_MSG( RF_ERROR, "rf_cdma_mc_configure_rx_milli_sleep:Failed to generate "
                        "script for Rx OFF! " ); 
    }/*if ( api_status != TRUE )*/
  }/*if( off_settings != NULL )*/
  ...
  if( api_status == FALSE )
  {
    RF_MSG_2( RF_ERROR, "rf_cdma_mc_configure_rx_milli_sleep: mdsp Rx ONOFF "
              "configuration failed Device %d band %d", device, band );
  }
  ...
  if ( rx_activate_milli_sleep_settings != NULL )
  {
    rf_buffer_destroy( rx_activate_milli_sleep_settings ) ;
  }


  api_status &= rf_cdma_mc_cleanup_ccs_event ( device , 
                    RF_CDMA_EVENT_RX_DEACTIVATE_MILLI_SLEEP ) ;
  api_status &= rf_cdma_mc_cleanup_ccs_event ( device,
                    RF_CDMA_EVENT_RX_ACTIVATE_MILLI_SLEEP ) ;
...
)
{
  ...
  if ( logical_dev->ant_tuner[band] != NULL )
    ...
    rfcommon_atuner_config_req_type tuner_config;
    	
    band_in_common_format.cdma_band = band ;

    tuner_config.rfm_device = device;
    tuner_config.mode = mode;
    tuner_config.band = band_in_common_format;
    tuner_config.chan_num = (uint32)chan;
    tuner_config.carrier_id = RFCMN_ATUNER_CARRIER_ID_0;
    tuner_config.rx_tx = RFCMN_ATUNER_TX_DEVICE; 

    result &=
      rf_cdma_atuner_configure_for_tune_back( logical_dev->ant_tuner[band],
                                      &tuner_config,
                                      RFDEVICE_CREATE_SCRIPT,
                                      rf_event_script,
                                      0, /* No timing offset */
...
/*!
  @brief
  Creates RF device (ASM,XSW and LNA) script for Wakeup Rx based on RF mode 
  (1x/HDR)
  ...
  API queries for the current cross switch position and 
  then queries cross switch driver for the settings required for that xsw
  position
  API checks if an external LNA object is present, if thats the case, it queries
  for the scripts to enable that device on the Rx path 
...
)
{
  ...
  if ( NULL != logical_dev->sm[band] )
  {
    ...
    /* ALWAYS USE SCRIPT BASED for TUNE! */
    xsw_script_data.immediate = FALSE;
    xsw_script_data.rxtx = RFC_CONFIG_RX;

    /* Get current position of XSW */
    position = (rfcommon_asdiv_position_type) 
    ...
    { 
      ...
      lna_cfg.mode = mode;
      lna_cfg.trigger_type = RFDEVICE_LNA_TRIGGER_WAKEUP;
      /* The rest of the fields are dont care in this case*/
      lna_cfg.lna_gain = RFDEVICE_LNA_GAIN_INVALID;
      lna_cfg.system_gain = 0;
 
      /* Query for the wakeup script */
      result &= rfdevice_lna_wakeup( lna_device,
                                     &lna_cfg,
                                     rf_event_script,
                                     RFDEVICE_CREATE_SCRIPT ,
                                     0 /* No timing offset needed */  );
...
/*!
  ...
  that it searches for is present.
  API checks if an external LNA object is present, if thats the case, it queries
  for the scripts to disable (sleep) that device on the Rx path 
...
)
{
  ...
  if ( NULL != logical_dev->sm[band] )
  {
    ...
    { 
      ...
      lna_cfg.band = rf_band;
      lna_cfg.mode = mode;
      lna_cfg.trigger_type = RFDEVICE_LNA_TRIGGER_SLEEP;
      /* The rest of the fields are dont care in this case*/
      lna_cfg.lna_gain = RFDEVICE_LNA_GAIN_INVALID;
      lna_cfg.system_gain = 0;
 
      /* Query for the Sleep script */ 
      result &= rfdevice_lna_sleep( lna_device,
                                    &lna_cfg,
                                    rf_event_script,
                                    RFDEVICE_CREATE_SCRIPT,
                                    0 /* No timing offset needed */ );
...
)
{
  ...
  if ( NULL != logical_dev->qpoet_obj[band] )
  {
    void * qpoet_device = logical_dev->qpoet_obj[band];
     ...
     /* Query devices if qpoet object is QET5102 or not */
    no_bias = rfdevice_papm_standby_delay_req((void*)logical_dev->qpoet_obj[band]);
    ...
    else
    {
      ...
      RF_MSG( RF_HIGH,"rf_cdma_mc_create_tx_asm_pa_qpoet_wakeup_script: "
                      "bias not set during tx tune script prep" );
    }
  }/*if ( NULL != logical_dev->qpoet_obj[band] )*/
  ...
  if ( NULL != logical_dev->pa[band] )
  {
    const rf_cdma_tx_static_type* tx_cdma_config_ptr = 
      rf_cdma_nv_get_tx_static_data ( device , band ) ;
    if ( tx_cdma_config_ptr != NULL )
    {
      pa_params_s.pa_output_cap_offset = 
      tx_cdma_config_ptr->pa_static_output_cap_offset ;
    }
    else
    {
      pa_params_s.pa_output_cap_offset = 0 ;

      RF_MSG( RF_MED, "rf_cdma_mc_create_tx_asm_pa_qpoet_wakeup_script: "
                      "Setting pa_static_output_cap_offset() to 0 " ) ;
...
/*!
  ...
  Flag to specify whether the API should send the entire script
  out in the end

  @param is_irat_mode
  IRAT MODE script generation
...
)
{
  ...
  /* If in IRAT mode, use MEAS_CLEANUP buffer, which means only target
  tech call flow is supported. If source tech, need to add support for 
  using MEAS_STARTUP */
  ...
  {    
    if ( rf_mode == RFM_1X_MODE )
    {
      sleep_rx_script = rf_1x_meas_mdsp_buffer_get(RFM_MEAS_PROGRAM_CLEANUP); 
    }
    else
    {
      sleep_rx_script = rf_hdr_meas_mdsp_buffer_get(RFM_MEAS_PROGRAM_CLEANUP); 
    }
  }
    ...
    {
      /*--------------------------------------------------------------------------*/
    /* iRAT / meas mode: Use meas call flow to create disable script. The
    WTR API call is the same for Prx & Drx to query meas disable script*/
    if ( is_irat_mode == TRUE )
    {     
      rfdevice_rx_mode_type rx_mode = 
              is_slave_device?RFDEVICE_DRX_OPER_MODE:RFDEVICE_PRX_OPER_MODE;
      
      result &= rfdevice_cdma_create_rx_meas_disable_script(
                                                 logical_dev->rx_device[band],
                                                 sleep_rx_script,  
                                                 band,
                                                 RFDEVICE_MEAS_STARTUP_SCRIPT,
                                                 rx_mode );

    }
    else
    {
      /* non-iRAT mode: Regular call flow*/
      ...
      {
        RF_MSG_2( RF_ERROR, "rf_cdma_mc_configure_front_end_for_rx_sleep: "
        ...
        result &= rfdevice_cdma_create_rx_disable_diversity_script(
                                               ...
                                               associated_rx_tranceiver,
                                                 sleep_rx_script, 
                                                 rx_bw_khz, tx_bw_khz,
                                                 (rfdevice_trx_sub_type)rf_cdma_data_get_subscriber_id(rf_mode));
      #ifdef FEATURE_RF_POWER_REPORTING
      rflm_diag_power_log_trx_dynamic_event(RFLM_DIAG_POWER_RFA_RX_HANDLE, rxlm_handle, RFLM_DIAG_POWER_RFA_STATE_OFF);
      #endif
    ...
    result &= rf_cdma_mc_create_rx_asm_lna_sleep_script( device,logical_dev, 
                                                       band,
                                                       rf_mode,
                sleep_rx_script );

      result &= rf_cdma_mc_create_rx_xsw_sleep_script(device, rf_mode, band, sleep_rx_script );  
    ...
    if ( logical_dev->ant_tuner[band] != NULL)
    {
      ...
      tuner_config.rfm_device = device;
      tuner_config.mode = rf_mode;
      tuner_config.band = band_in_common_format;
      tuner_config.chan_num = (uint32)0; // Disable does not need chan
      tuner_config.carrier_id = RFCMN_ATUNER_CARRIER_ID_0;

      if ( is_slave_device == FALSE )
      {
        tuner_config.rx_tx = RFCMN_ATUNER_PRX_DEVICE; 
      }
      else
      {
        tuner_config.rx_tx = RFCMN_ATUNER_DRX_DEVICE; 
      }
      ...
      /*Create tune_back script for IRAT to diable tuner*/
      else
      {
        /*If Target and Source Tuners are same - this API will just program current state of Tuner, which is the same before IRAT gap. 
          If Target and Source Tuners are different, it will disable the Tuner on Target tech */
        result &= rf_cdma_atuner_configure_for_tune_back(logical_dev->ant_tuner[band],
                                                         &tuner_config,
                                                         RFDEVICE_CREATE_SCRIPT, 
                                                         sleep_rx_script, 
                                                         0,/* No timing offset */
  ...
  /* Skip script committing in irat mode */
  if ( result == TRUE && is_irat_mode == FALSE )
  {
    if ( immediate_write == FALSE  )
    {
      ...
      if ( is_slave_device == FALSE  )
      {
        event_id = RFLM_CDMA_MDSP_CCS_EVENT_SLEEP_RX_ID;
      }
      else
      {
        event_id = RFLM_CDMA_MDSP_CCS_EVENT_SLEEP_DRX_ID;
      }
    ...
    else
    {
      ...
      rf_cdma_mc_do_immediate_script_write( sleep_rx_script );
    }
  }
  ...
  /* Do not destroy meas script buffer in irat mode */
  if ( is_irat_mode == FALSE )
  {
    if ( sleep_rx_script != NULL )
    {
  ...
  if ( NULL != dev_status_r )
  {
    ...
    else
    {
      ...
      if ( logical_dev->ant_tuner[band] != NULL )
      {
        ...
        tuner_config.rfm_device = device;
        tuner_config.mode = dev_status_r->rf_mode;
        tuner_config.band = band_in_common_format;
        tuner_config.chan_num = (uint32)dev_status_r->lo_tuned_to_chan;
        tuner_config.carrier_id = RFCMN_ATUNER_CARRIER_ID_0;
      ...
      { /* Get Tx disable coupler scripts */
        ...
        coupler_cfg.direction = RFDEVICE_COUPLER_DIRECTION_REV;
        /* Use default gain and output type from rfc */
        coupler_cfg.gain = RFDEVICE_COUPLER_GAIN_DEFAULT;
        coupler_cfg.output_type = RFDEVICE_COUPLER_OUTPUT_TYPE_DEFAULT;
        /* Query for coupler port based on XSW position */
        coupler_cfg.port = rf_cdma_mc_get_coupler_port( device );
        result &= rf_cdma_config_coupler ( logical_dev->coupler_obj[band],
                                           coupler_cfg,
                                           sleep_tx_script,
                                           RFDEVICE_CREATE_SCRIPT,
                                           0 ) ;
    ...
    {   
      ...
      result &= rf_cdma_mdsp_configure_devices( device ,
                logical_dev->tx_mdsp_path,
                                                ...
                                                analog_script_event_handle,
                                                (rf_cdma_mdsp_get_event_info(RFLM_CDMA_MDSP_CCS_EVENT_SLEEP_TX_ID)),
                                                sleep_tx_script, 
                script_index,
                rf_cdma_data_get_txlm_handle (device) ,
                NULL, NULL );
      ...
      {
        /* Do immediate write for everything */
        rf_cdma_mc_do_immediate_script_write( sleep_tx_script );
      }
    }/*if( result == TRUE )*/
...
)
{
  ...
  tuner_config.chan_num = (uint32)chan;
  tuner_config.carrier_id = RFCMN_ATUNER_CARRIER_ID_0;
  tuner_config.freq = rf_cdma_get_rx_carrier_freq( band_in_common_format.cdma_band,chan);
  ...
  result &=
    rf_cdma_atuner_init( logical_dev->ant_tuner[band],
                         ...
                         ant_tuner_script_token_ptr );
  result &=
    rf_cdma_atuner_configure_for_rx( logical_dev->ant_tuner[band],
                                     &tuner_config,
                                     RFDEVICE_CREATE_SCRIPT,
                                     wakeup_rx_script,
                                     0, /* No timing offset */
...
/*!
  @brief
  Configures the RF Antenna tuner for tune-away
...
*/
boolean
rf_cdma_mc_configure_atuner_tune_away
(
  const rfm_device_enum_type device,
  const rfc_cdma_logical_device_params_type *logical_dev  ,
  const rfm_mode_enum_type rf_mode ,
...
)
{
  ...
  result &= rf_cdma_atuner_configure_for_tune_away( logical_dev->ant_tuner[band],
                                                    &tuner_config,
                                                    RFDEVICE_CREATE_SCRIPT,
                                                    wakeup_rx_script,
                                                    0, /* No timing offset */
  ...
  if ( result == TRUE )
  {
    RF_MSG_1 ( RF_LOW, "rf_cdma_mc_configure_atuner_tune_away: time profile"
               "Ant Tuner Time %d" , prof_ant_tuner_t_val ) ;
  }
  else
  {
    RF_MSG ( RF_ERROR ,
             "rf_cdma_mc_configure_atuner_tune_away: API failed.." ) ;
  }
...
/*!
  @brief
  Configures the RF Antenna tuner for tuneback
...
*/
boolean
rf_cdma_mc_configure_atuner_tune_back
(
  const rfm_device_enum_type device,
  const rfc_cdma_logical_device_params_type *logical_dev  ,
  const rfm_mode_enum_type rf_mode ,
...
)
{
  ...
  if ( result == TRUE )
  {
    RF_MSG_1 ( RF_LOW, "rf_cdma_mc_configure_atuner_tune_back: time profile"
               "Ant Tuner Time %d" , prof_ant_tuner_t_val ) ;
  }
  else
  {
    RF_MSG ( RF_ERROR ,
             "rf_cdma_mc_configure_atuner_tune_back: API failed.." ) ;
  }
...
)
{
  ...
  {
    /* Use 1x global meas script buffer */   
    if ( rf_mode == RFM_1X_MODE  )
    {
      wakeup_rx_script = rf_1x_meas_mdsp_buffer_get(RFM_MEAS_PROGRAM_STARTUP); 
    }
    else
    {
      wakeup_rx_script = rf_hdr_meas_mdsp_buffer_get(RFM_MEAS_PROGRAM_STARTUP);
    }
  ...
  else
  {
    ...
    if ( logical_dev->ant_tuner[band] != NULL )
    {
      ...
      if (is_irat_mode == TRUE)
      {
        if(meas_type == RFDEVICE_MEAS_STARTUP_SCRIPT)
        {
          result &= rf_cdma_mc_configure_atuner_tune_away (device, 
                                                           logical_dev, 
                                                           rf_mode, 
                                                           wakeup_rx_script , 
                                                           band, 
        ...
        else if(meas_type == RFDEVICE_MEAS_CLEANUP_SCRIPT)
        {
          result &= rf_cdma_mc_configure_atuner_tune_back (device, 
                                                           logical_dev, 
                                                           rf_mode, 
                                                           wakeup_rx_script , 
                                                           band, 
        ...
        {
          RF_MSG_1( RF_ERROR, "rf_cdma_mc_configure_front_end_for_rx_wakeup: Invalid Meas_type:%d",
                              meas_type);
          result = FALSE;
        }
      }
      ...
      {
        result &= rf_cdma_mc_configure_atuner_rx (device, 
                                                  logical_dev, 
                                                  rf_mode, 
                                                  wakeup_rx_script, 
                                                  band, 
    ...
    if ( is_slave_device == TRUE )
    {
      /* WTR driver has exposed the difference in Prx/Drx tune to MC layer. MC
         ...
         correctly contruct the scripts */
      rfm_device_enum_type associate_device;
      const rfc_cdma_logical_device_params_type *assco_logical_dev; 
      rfdevice_trx_cdma_rx* associated_rx_tranceiver = NULL;
      ...
      {
        RF_MSG_2( RF_ERROR, "rf_cdma_mc_configure_front_end_for_rx_wakeup: "
                            "NULL paired Rx object device %d, asssco_dev %d",
                            device, associate_device);
      }    
      ...
      if ( is_irat_mode == TRUE && meas_type == RFDEVICE_MEAS_STARTUP_SCRIPT )
      {
        operation_mode = RFDEVICE_OPER_MODE_MEAS;
      }
      else
      {
        operation_mode = RFDEVICE_OPER_MODE_STANDALONE;
      }
      ...
      result &= rfdevice_cdma_create_rx_enable_diversity_script(
                          ...
                          rf_mode, band, chan,
                          rx_bw_khz, tx_bw_khz, 
                          power_mode, wakeup_rx_script, device,
                          operation_mode , 
                rf_cdma_data_get_ant_position(associate_device), 
                (rfdevice_trx_sub_type)rf_cdma_data_get_subscriber_id(rf_mode));

      #ifdef FEATURE_RF_POWER_REPORTING
      /* associate handle with device */
      rflm_dm_handle_meta_s handle_meta;
      rflm_dm_get_handle_info(  rxlm_handle , &handle_meta);
      handle_meta.devide_id = device;
      handle_meta.device_id = device;
      rflm_dm_set_handle_info( rxlm_handle , &handle_meta );
      /* log channel */
      rf_power_reporting_update_channel( device, chan );
      /* log rx on */
      rflm_diag_power_log_trx_dynamic_event(RFLM_DIAG_POWER_RFA_RX_HANDLE, rxlm_handle, RFLM_DIAG_POWER_RFA_STATE_ON);
      #endif
      ...
      if ( is_irat_mode == FALSE )
      {
        ...
        else
        {
          ...
          result &= rfdevice_cdma_create_rx_tune_script( 
                      logical_dev->rx_device[band],
                      rf_mode, band, chan, rx_bw_khz, 
                      tx_bw_khz, power_mode, 
                    ...
                    (rfdevice_trx_sub_type)rf_cdma_data_get_subscriber_id(rf_mode),
                    device , rf_cdma_data_get_ant_position ( device ) ) ;
          ...
          handle_meta.device_id = device;
          rflm_dm_set_handle_info( rxlm_handle , &handle_meta );
          rflm_diag_power_log_trx_dynamic_event(
		  	RFLM_DIAG_POWER_RFA_RX_HANDLE, rxlm_handle, 
		  	RFLM_DIAG_POWER_RFA_STATE_ON);
          rf_power_reporting_update_channel( device, chan );
          #endif
        }
      }
    ...
    result &= rf_cdma_mc_create_rx_asm_xsw_lna_wakeup_script( device,
                                                      logical_dev,
                                                      band, 
                                                      rf_mode,
                                                      wakeup_rx_script );
  ...
  /* Skip configuring script in wakeup_rx()/enable_div() in iRAT mode, script
  will be configured in meas once wakeup_rx()/enable_div() finishes */
  if ( result == TRUE && is_irat_mode == FALSE )
  {
    if ( immediate_write == FALSE )
    {
      ...
      if ( is_slave_device != TRUE )
  {
        event_id = RFLM_CDMA_MDSP_CCS_EVENT_WAKEUP_RX_ID;
      }
      else
    {
        event_id = RFLM_CDMA_MDSP_CCS_EVENT_WAKEUP_DRX_ID;
    }

      result &= rf_cdma_mc_configure_script_helper(device, 
                                                    rf_mode,
                                                    rf_cdma_data_get_rxlm_handle (device),
                                                    script_index, 
                                                    wakeup_rx_script,
    ...
    {
      /* Do immediate write for everything */
      rf_cdma_mc_do_immediate_script_write( wakeup_rx_script );      
    }
  }/* if ( result == TRUE ) */
  else if ( is_irat_mode == TRUE && meas_type == RFDEVICE_MEAS_CLEANUP_SCRIPT  )
    {
    /* Configure the script for 1x2G\DO2G QTA */
    result &= rf_cdma_mc_configure_script_helper(device, 
                                              rf_mode,
                                                 rf_cdma_data_get_rxlm_handle (device),
                                                 script_index, 
  ...
  if ( ( result == TRUE ) && ( is_irat_mode == FALSE ) )
  {
    result = rf_cdma_mc_configure_rx_milli_sleep ( device , band , 
                                                   rf_mode , script_index  ) ;
    if ( FALSE == result )
    {
      RF_MSG_4 ( RF_ERROR , "rf_cdma_mc_configure_rffe_for_rx_wakeup: "
                 "rf_cdma_mc_configure_rx_milli_sleep() returned error "
                 "||DEV: %d|band: %d|rf_mode: %d|ScriptIndex: %d|| " ,
                 device , band , rf_mode , script_index ) ; 
    }
  }/*if ( result == TRUE )*/
  ...
  {
    /* De-allocate memory used for 1x2G\DO2G script generation */
...
)
{
  ...
  else
  {
    ...
    result &= 
      rfdevice_cdma_create_tx_tune_script( logical_dev->tx_device[band],
                                           ...
                                           &script_data_ptr ,
                                           (rfdevice_trx_sub_type)rf_cdma_data_get_subscriber_id(rf_mode), 
                                           FALSE /* The channel passed in is NOT an Rx channel*/,
                                           device );
    ...
    if ( logical_dev->ant_tuner[band] != NULL )
    {
      ...
      tuner_config.carrier_id = RFCMN_ATUNER_CARRIER_ID_0;
      tuner_config.rx_tx = RFCMN_ATUNER_TX_DEVICE; 
      tuner_config.freq = rf_cdma_get_tx_carrier_freq(band,chan);
  ...
  { 
    ...
    /* Get the forward script */
    result &= rf_cdma_config_coupler(logical_dev->coupler_obj[band],
                                         coupler_cfg , wakeup_tx_script ,
                                         RFDEVICE_CREATE_SCRIPT , 0 ) ;

    /* Populate the forward and reverse scripts for use in VSWR captures */
    result &= 
      rf_cdma_mc_populate_coupler_scripts (  device , band , 
                                             rf_mode , script_index ) ;
    if ( FALSE == result )
    {
      RF_MSG( RF_ERROR, "rf_cdma_mc_configure_front_end_for_tx_wakeup() "
                        "rf_cdma_mc_populate_coupler_scripts() Failed !!!");
    }
  }/*if( NULL != logical_dev->coupler_obj[band] )*/

  /* update rf_on/off settings in SM */
  result &= rf_cdma_mc_configure_tx_on_off( device, band, rf_mode , 
                                            script_index );
  

  /*Configure Tx RF On/Off script as part of conigure_front_end for 
  both Tx wakeup and Tx retune */
  if ( rfm_get_calibration_state() == TRUE )
  { 
    /* In cal mode, FW may not always trigger FED Tx On, so append RF_ON
       script to static event instead .. no need to append RF_OFF settings*/
    result &= rf_cdma_get_rf_onoff_settings( device, rf_mode, 
                                             band, TRUE /* RFON */, 
                                             logical_dev, wakeup_tx_script );
  }
  ...
  if ( result == TRUE )
  {
    ...
    rflm_cdma_mdsp_script_id_t event_id = RFLM_CDMA_MDSP_CCS_EVENT_WAKEUP_TX_ID ;
    ...
    {
      /* Do immediate write for everything */
      rf_cdma_mc_do_immediate_script_write( wakeup_tx_script );
    }
  } /* if ( result == TRUE )  */
...
boolean
rf_cdma_mc_configure_coupler_scripts
(
  rfm_device_enum_type      device ,
  rfm_cdma_band_class_type  band ,
  rfm_mode_enum_type        rf_mode ,
  uint32                                        buffer_index ,
  const rfc_cdma_logical_device_params_type*    logical_dev ,
  rf_cdma_event_type                            ev_t       ,
  rflm_cdma_mdsp_script_id_t                    ev_id ,    
  rfdevice_coupling_direction_type              coupler_direction 
)
{
  boolean api_status = TRUE ; 
  rf_buffer_intf* coupler_settings = NULL ;
  rfcommon_mdsp_event_handle** coupler_event_handle =
    rf_cdma_data_get_event_handle ( device, ev_t ) ; 

  /*-----------------------Coupler Forward EVENT----------------------------*/
  /* Create generic buffer interface object to forward coupler settings */
  coupler_settings = 
    rf_buffer_create ( CFW_RF_EVENT_MAX_NUM_OF_SBI , 
                       CFW_RF_EVENT_MAX_NUM_OF_RFFE , 
                       RF_CDMA_MAX_GRFC_SCRIPT_SIZE ) ;
  ...
  /*Check if valid pointer was returned for buffer object*/
  if( coupler_settings == NULL )
  {
    RF_MSG( RF_ERROR, "rf_cdma_mc_configure_coupler_scripts() "
      "Unable to create buffer object for coupler_settings " ) ;
    api_status = FALSE;
  }/*if( on_settings == NULL )*/
  else
  {
    ...
    { 
      ...
      coupler_cfg.mode = rf_mode;
      coupler_cfg.band.cdma_band = band;
      coupler_cfg.direction = coupler_direction;
      /* Use default gain and output type from rfc */
      coupler_cfg.gain = RFDEVICE_COUPLER_GAIN_DEFAULT;
      coupler_cfg.output_type = RFDEVICE_COUPLER_OUTPUT_TYPE_DEFAULT;
      /* Query for coupler port based on XSW position */
      coupler_cfg.port = rf_cdma_mc_get_coupler_port( device );
      api_status &= rf_cdma_config_coupler ( logical_dev->coupler_obj[band] ,
                                       coupler_cfg , coupler_settings ,
                                       RFDEVICE_CREATE_SCRIPT , 0 ) ;
                                                
      /* Send to MDSP layer if API was success till this point */
      if ( ( api_status == TRUE ) && ( coupler_event_handle != NULL) )
      {
        /* Configure Coupler event using the obtained settings */
        api_status &= 
          rf_cdma_mdsp_configure_devices ( device , logical_dev->tx_mdsp_path, 
            rf_mode, ev_id , coupler_event_handle ,
              (rf_cdma_mdsp_get_event_info ( ev_id) ) , coupler_settings , 
                buffer_index , rf_cdma_data_get_txlm_handle (device) , 
                  NULL, NULL ) ;
      }/*if ( api_status == TRUE )*/
      else
      {
        RF_MSG ( RF_ERROR, "rf_cdma_mc_configure_coupler_scripts()"
          "rfdevice_coupler_config() failed to retrieve forward settings !! " ); 
      }/*if ( api_status != TRUE )*/
    }/*if( CoupleObj = NULL )*/
    else
    {
    ...
    if ( api_status == FALSE )
    {
      RF_MSG_1 ( RF_ERROR, "rf_cdma_mc_configure_coupler_scripts() "
        "rf_cdma_mdsp_configure_devices () failed for ev_id: %d " , ev_id ) ;
    } 
  
    rf_buffer_destroy( coupler_settings ) ;

    /* Unconditionally clean events since DM has the coupler settings */
    (void) rf_cdma_mc_cleanup_ccs_event ( device , ev_t ) ;
  }
  return api_status ;
    }
/*----------------------------------------------------------------------------*/
/*!
  @brief
  Gets scripts for the coupler and places in the DM

  @param device
  Device associated with the coupler

  @param band
  Band associated with the coupler

  @param rf_mode
  RF mode associated with the coupler
...
(
  ...
  static uint32 allocated_blk_mask_fwd = 0;
  static uint32 allocated_blk_mask_rev = 0;



  if ( logical_dev == NULL )
  {
    RF_MSG( RF_ERROR, "rf_cdma_mc_configure_populate_scripts() "
                      "Null Logical Dev pointer");
  }
  else
  {
    ...
    rf_cdma_event_type ev_t = RF_CDMA_EVENT_RX_COUPLER_FORWARD_DIRECTION ;
    rflm_cdma_mdsp_script_id_t ev_id =
      RFLM_CDMA_MDSP_CCS_EVENT_COUPLER_FORWARD_POSITION_ID ; 
	uint32 tx_handle = rf_cdma_data_get_txlm_handle (device);
	boolean is_event_issue_seq;
	void* ptr_to_ccs_event;
	rflm_tech_id_t rflm_tech_id = RFLM_MAX_MODULE;
	
	rfcommon_mdsp_transaction_settings_type
    rf_cdma_mdsp_ccs_event_transaction_params =
    {
	...
	{
      rflm_tech_id = RFLM_TECH_HDR;
	}
	else
	{
	  api_status = FALSE;    
	  RF_MSG( RF_ERROR, "rf_cdma_mc_configure_populate_scripts() "
			  "NULL pointer for coupler FORWARD script " ) ;
	}
	
    api_status &= 
      rf_cdma_mc_configure_coupler_scripts ( device , band , rf_mode , 
                                             buffer_index , logical_dev , ev_t , 
                                             ev_id , coupler_direction ) ;

	ptr_to_ccs_event = rf_cdma_mdsp_modem_get_ccs_buffer ( 
		                                device, 
		                                tx_handle, 
		                                ev_id,
		                                buffer_index,
                                        ...
                                        &rf_cdma_mdsp_ccs_event_transaction_params, 
                                        &is_event_issue_seq );
	if (ptr_to_ccs_event != NULL)
	{	
	  if (allocated_blk_mask_fwd != 0)
	  {
        rflm_ccs_mmu_deallocate_static_region_block_mask(rflm_tech_id, allocated_blk_mask_fwd);
	  }
	  api_status &=  rflm_ccs_copy_static_event (
	  	 (rflm_ccs_rf_event_header_task_type *)ptr_to_ccs_event,
	  	 rflm_tech_id, &allocated_blk_mask_fwd);
	}
	else
	{
      api_status = FALSE;	 
    ...
    {
      RF_MSG( RF_ERROR, "rf_cdma_mc_configure_populate_scripts() "
          "rf_cdma_mc_configure_coupler_scripts() failed for FORWARD direction " ) ;
    }
    else
    {
      /* Configure Reverse Settings */
      coupler_direction = RFDEVICE_COUPLER_DIRECTION_REV ;
      ev_t = RF_CDMA_EVENT_RX_COUPLER_REVERSE_DIRECTION ;
      ev_id = RFLM_CDMA_MDSP_CCS_EVENT_COUPLER_REVERSE_POSITION_ID ;    
	  ...
	  if (ptr_to_ccs_event != NULL)
	  {	
	    if (allocated_blk_mask_rev != 0)
	    {
          rflm_ccs_mmu_deallocate_static_region_block_mask(rflm_tech_id, allocated_blk_mask_rev);		  
	    }
	    api_status &=  rflm_ccs_copy_static_event (
			(rflm_ccs_rf_event_header_task_type *)ptr_to_ccs_event,
			rflm_tech_id, &allocated_blk_mask_rev);
	  }
	  else
	  {
        api_status = FALSE;	 
  ...
  return api_status ;
} /* rf_cdma_mc_configure_populate_scripts() */
...
/*!
  ...
  The API updates the Pout as per NV.
  ...
  @param dev_r 
  Read Only device status for given RFM device
   
  @param logical_dev
  Read Only Logical Device for given RFM device
   
  @return
  boolean TRUE if success. FALSE if fail
*/
boolean
rf_cdma_tx_agc_asdiv_update 
( 
...
)
{
  ...
  int16 offset = 0 ;
  rfcom_band_type_u rf_band; 
  rf_band.cdma_band = rf_cdma_data_get_curr_band(tx_device);

  /* Retrieve the prev offset 1st thing so that it doesnt get overwritten */
  int16 previous_offset = rf_cdma_data_get_pout_for_ant_position ( tx_device ) ;
  if ( rf_mode == RFM_1X_MODE )
  {
    if ( rf_1x_is_tx_aux_data_valid ( rf_1x_config_ptr , position ) == TRUE )
    {
      offset = 
        rf_1x_config_ptr->tx_static.
        onex_common_tx_aux_data.tx_aux_info.gain_offset ;
    }
      /* Pick the offset based on the char'd value from the EFS */ 
        offset = offset + (int16)rfcommon_nv_get_asdiv_offset_data(rf_mode,
                     	rf_band,position); 
  }
  else if ( rf_mode == RFM_1XEVDO_MODE )
  {

    if ( rf_hdr_is_tx_aux_data_valid ( rf_hdr_config_ptr , position ) == TRUE )
    {
       offset = 
        rf_hdr_config_ptr->tx_static.
        hdr_common_tx_aux_data.tx_aux_info.gain_offset ;
    }
      /* Pick the offset based on the char'd value from the EFS */ 
       offset = offset + (int16)rfcommon_nv_get_asdiv_offset_data(rf_mode,
                    	rf_band,position);
  ...
  (void) rf_cdma_data_set_pout_for_ant_position ( tx_device , offset ) ;


  /* Retrieve previous offset to apply it if the new offset is 0 to bring it 
     initial linearizer values. Then  */
  if ( offset != 0 )
  {
    RF_MSG_1 ( RF_LOW , "rf_cdma_tx_agc_asdiv_update(): Pout Adjustment of %d" ,
               offset ) ;
    success = rf_cdma_tx_agc_update_with_offset ( tx_device , offset ) ;
  }
  else
  {
    /* Restore to pre-offset value */
    if ( previous_offset != 0 )
    {
      /* Negation will bring the offset effectively to 0 */     
      previous_offset = -1 * previous_offset ;
      RF_MSG_1 ( RF_LOW , "rf_cdma_tx_agc_asdiv_update(): Pout UnAdjustment of %d" ,
                 previous_offset ) ;
      success = rf_cdma_tx_agc_update_with_offset ( tx_device , previous_offset ) ;
    }
  }
...
)
{
  ...
  const rf_1x_config_type*  rf_1x_config_ptr =  NULL ; /* Pointer to 1x RF NV data */
  const rf_hdr_config_type* rf_hdr_config_ptr =  NULL ; /* Pointer to HDR RF NV data */
  rfm_cdma_band_class_type band ;
  boolean does_agc_need_reloading = FALSE ;


  /* Get Device Status */
  dev_r = rf_cdma_get_device_status( rx_device );
  ...
  band = dev_r->curr_band ;
  if( band >= RFM_CDMA_MAX_BAND )
  {
    RF_MSG_1( RF_ERROR, "rf_cdma_mc_set_antenna_to_position: invalid band %d", 
                        band );
    return FALSE; 
  }

  /* Update the CDMA data structure unconditionally */
  rf_cdma_data_set_ant_position ( rx_device , position ) ;
  if ( (rfm_l1_cb_type)cb_func == NULL)
  {
    /* Get Current switch position */
    if ( rfcommon_asdiv_get_current_position(rx_device) == 
       position )
    { 
      RF_MSG_2( RF_HIGH, "rf_cdma_mc_set_antenna_to_position: "
                       "Current position %d is same as the position requested %d", 
                       rfcommon_asdiv_get_current_position(rx_device) , 
                       position );
      return TRUE;
    }
  }

  /* Device Asleep or in Prep. Set Antenna and return */
  if ( ( ( dev_r->data_1x.rf_state == RF_CDMA_STATE_SLEEP ) &&
         ( dev_r->data_hdr.rf_state == RF_CDMA_STATE_SLEEP ) ) 
       ||
         ( dev_r->data_1x.rf_state == RF_CDMA_STATE_RX_PREP ) ||
         ( dev_r->data_hdr.rf_state == RF_CDMA_STATE_RX_PREP ) )
  {
  ...
  else
  {
    ...
    /* Query the RFC to get the tx device using the band info to support 
    rx\tx split */
    band_u.cdma_band = (rfm_cdma_band_class_type)band;
    sys_band = rfcommon_core_convert_rfcom_band_to_sys_band( RFM_1X_MODE, band_u );

    rfm_device_enum_type associated_dev = rf_cdma_data_get_assoc_dev(rx_device) ;

    rfm_device_enum_type tx_device = 
      rfc_common_get_preferred_tx_device( sys_band );
    rf_1x_config_ptr = 
    ...
    {
      /* Logic to make sure if RxAGC Reloading is required. */
      if ( TRUE ==  
           rf_cdma_data_get_rxagc_status_for_ant_position ( rx_device ) )
      {
        does_agc_need_reloading = TRUE ;
      }
      /* And if the switch position for the given NV path has auxillary data 
         adjust the Rx params accordingly */
      else if ( rf_1x_is_rx_aux_data_valid ( rf_1x_config_ptr , position ) == TRUE )
      {
        RF_MSG ( RF_LOW , 
                 "rf_cdma_mc_set_antenna_to_position(): Aux Rx NV Data found" ) ;
        does_agc_need_reloading = TRUE ;
        (void)
         rf_cdma_data_set_rxagc_status_for_ant_position ( rx_device , TRUE ) ;
        
      } /* Valid Aux Data */ 
    } /* 1x in RX or RXDIV state */
    else if (( dev_r->data_hdr.rf_state == RF_CDMA_STATE_RX ) ||
             ( dev_r->data_hdr.rf_state == RF_CDMA_STATE_RXDIV ) )
    {
     /* And if the switch position for the given NV path has auxillary data 
         adjust the Rx params accordingly */
      if ( rf_hdr_is_rx_aux_data_valid ( rf_hdr_config_ptr , position ) == TRUE )
      {
        RF_MSG ( RF_LOW , 
                 "rf_cdma_mc_set_antenna_to_position(): Aux Rx NV Data found" ) ;
    ...
    if ( FALSE == ( api_status &= 
                   rf_cdma_update_rx_agc_with_new_wtr_gain ( rx_device , dev_r , 
                                                       logical_dev , position ) ) )
    {
      RF_MSG ( RF_ERROR , "rf_cdma_mc_set_antenna_to_position(): "
             "rf_cdma_update_rx_agc_wiith_new_wtr_gain returned failure!!" ) ;
    }
    /* Leave Device Critical Section */
    rfdevice_cdma_rx_leave_critical_section(logical_dev->rx_device[band]);

    /* DRX Device Handling */
    if ( associated_dev != RFM_INVALID_DEVICE )
    {
      const rfc_cdma_logical_device_params_type* s_logical_dev = 
        rfc_cdma_get_logical_device_params ( associated_dev ) ;
      const rf_cdma_data_status_type* slave_dev_r = rf_cdma_get_device_status( associated_dev );

      /* Set DRx Antenna position */
      rf_cdma_data_set_ant_position ( associated_dev , position ) ;

      if ((s_logical_dev != NULL) && (slave_dev_r != NULL))
      {
        /* Enter Device Critical Section, so that the device call is thread safe */
        rfdevice_cdma_rx_enter_critical_section(s_logical_dev->rx_device[band]);
        if ( FALSE == ( api_status &= 
                     rf_cdma_update_rx_agc_with_new_wtr_gain ( associated_dev , slave_dev_r , 
                                                               s_logical_dev , position ) ) )
        {
          RF_MSG ( RF_ERROR , "rf_cdma_mc_set_antenna_to_position(): associated_dev "
                   "rf_cdma_update_rx_agc_wiith_new_wtr_gain returned failure!!" ) ;
        }
        /* Leave Device Critical Section */
        rfdevice_cdma_rx_leave_critical_section(s_logical_dev->rx_device[band]);
      }
      else
      {
        RF_MSG ( RF_ERROR , "rf_cdma_mc_set_antenna_to_position(): s_logical_dev"
							" or slave_dev_r is NULL" ) ;
      }
    }

    /* ********* If RxAGC loading Complete.. Now load Tx offsets *************/
    if ( ( tx_device != RFM_INVALID_DEVICE ) && ( does_agc_need_reloading == TRUE ) )
    {

      const rf_cdma_data_status_type* dev_r_tx = 
        rf_cdma_get_device_status( tx_device ) ;
      if ( dev_r_tx != NULL )
      {
        if ( ( dev_r_tx->data_1x.rf_state == RF_CDMA_STATE_RXTX )  ||
             ( dev_r_tx->data_1x.rf_state == RF_CDMA_STATE_TX )    ||
             ( dev_r_tx->data_hdr.rf_state == RF_CDMA_STATE_RXTX ) || 
             ( dev_r_tx->data_hdr.rf_state == RF_CDMA_STATE_TX ) )
        {
          api_status &= 
                rf_cdma_tx_agc_asdiv_update ( tx_device , position , 
                                              dev_r_tx->rf_mode , rf_1x_config_ptr , 
                                              rf_hdr_config_ptr ) ;
              RF_MSG ( RF_ERROR , "rf_cdma_mc_set_antenna_to_position(): "
                                  "rf_cdma_tx_agc_asdiv_update!!" ) ;
        } /* device in Tx state */
    ...
    xsw_script_data.device = rx_device;
    xsw_script_data.band = band;
    ...
    update_tuner_cb.cb_data = (uint8)rx_device;
...
/*!
  @brief
  API to vote XSW sleep state, this is used in meas exit to ensure
  the devices are put to sleep once the measurement gap is done

  @details
  This API queries the RFC for TRM NV port info

  @param device
  device for which XSW needs to vote sleep
...
)
{
   ...
   xsw_script_data.device = device;
   xsw_script_data.band = band;
   xsw_script_data.immediate = FALSE;/* immediate */
   xsw_script_data.rxtx = RFC_CONFIG_RX;
   ...
   /* Add settings for XSW position setting to the Rx sleep Script */
   api_status &= rfc_cdma_generate_xsw_sleep_script( position,
                                                     rf_event_script,
                                                     &xsw_script_data );
   if ( api_status == FALSE )
   {
	 RF_MSG( RF_ERROR, "rf_cdma_mc_create_rx_xsw_sleep_script: "
					   "Error !! ");
   }
...
)
{
  if ( device < RFCOM_MAX_DEVICES )
  {
    ...
    /* RESET Rx Mdsp Chain */
    rfc_cdma_set_rx_mdsp_path( device, RF_CDMA_MDSP_CHAIN_INVALID );
...
)
{
  ...
  */
  ...
  ret_val &= rf_cdma_msm_get_rxfe_delay( rf_cdma_data_get_rxlm_handle( rx_device ),
                                         rxfe_delay );
    ...
    if ( curr_mode == RFM_1X_MODE )
    {
      ...
      if ( rf_1x_config_ptr == NULL )
      {
        RF_MSG_3( RF_ERROR, "rf_cdma_mc_get_btf_delay: Invalid NV pointer on  "
                  "Dev %d, NV Path %d, Band %d", tx_device,
                  logical_dev->tx_nv_path[dev_status_r->curr_band],
                  dev_status_r->curr_band );
        ret_val = FALSE;
      }
...
/*!
  ...
  @param device
  The device(Tx Device) for which dynamic update needed
...
)
{
  ...
  /* Set the mode to 1x ignoring the curr_mode, since concurrency manager
  treats 1x and HDR the same way */
  rfm_mode = RFM_1X_MODE;
  ...
  if ( is_irat_mode == FALSE )
  {
    ...
    if ( alt_path_status == RFCMN_PATH_SEL_SUCCESS )
    {
      ...
      else
      {
        ...
        success = 
          rfc_update_alt_port_mapping ( device, rfm_mode, rfc_band, alt_path ) ; 
      }
      ...
      if (success == FALSE)
      {
        RF_MSG_3(RF_ERROR,"rf_cdma_mc_update_alt_rx_path:Falied to update " 
                 "Alt port map for tech:%d ,device:%d , band %d ",rfm_mode,
                 device, band );
      }
    }
    else
    {
      if (alt_path_status == RFCMN_PATH_SEL_NOT_SUPPORTED)
      {
        ...
        RF_MSG_3(RF_HIGH,"rf_cdma_mc_update_alt_rx_path: alt rx path selection" 
               "is not supported for tech:%d ,device:%d , band %d ",rfm_mode,
               device, band );
      }
      else
      /* Error fatal if path update failed since front end will not be 
      configured as expected if this fails */
      {
        ...
        RF_MSG_3(RF_ERROR,"rf_cdma_mc_update_alt_rx_path:Falied to get " 
               "Alt port for tech:%d ,device:%d , band %d ",rfm_mode,
               device, band );
      }
    }
  }/* if ( is_irat_mode == FALSE )*/
...
/*!
  ...
  @param rf_mode
  The RF mode (1x/EVDO) for which operation is requested
...
)
{
  ...
  if(dev_status_r == NULL || logical_dev == NULL)            
  {                 
    RF_MSG_1 ( RF_ERROR, "rf_cdma_mc_reset_device_states_after_meas_gap: "   
		"dev_status_r or logical_dev NULL pointer" 
		"for device %d", device );             
    return FALSE;       
   }   
    
   /* Get current state based on RF mode */
   if ( rf_mode == RFM_1XEVDO_MODE )
   {
     curr_state = rf_cdma_data_get_hdr_rf_state(device);
   }
   else if ( rf_mode == RFM_1X_MODE )
   {
     curr_state = rf_cdma_data_get_1x_rf_state(device);
  }        
   else
   {
     RF_MSG_1 ( RF_ERROR, "rf_cdma_mc_reset_device_states_after_meas_gap: "   
      "Invalid mode %d" , rf_mode );             
     return FALSE;       
  }        

   /* Perform tuner reset via dummy script if device is in RxTx state */   
   if ( curr_state == RF_CDMA_STATE_RXTX )
   {
    /* ************************************************************************/ 
  ...
  if ( ret_val == TRUE )
  {
    ...
    tuner_config.mode =  dev_status_r->rf_mode;
    tuner_config.band = band_in_common_format;
    tuner_config.chan_num = (uint32)dev_status_r->curr_chans[0];
    tuner_config.carrier_id = RFCMN_ATUNER_CARRIER_ID_0;
    tuner_config.rx_tx = RFCMN_ATUNER_TX_DEVICE; 

    /* Dummy script so use minimum size for RFFE */    
       dummy_script = (void *)rf_buffer_create(0, 
                        RF_CDMA_MAX_GRFC_SCRIPT_SIZE, 0);
   ...
   {
     RF_MSG_1 ( RF_MED, "rf_cdma_mc_reset_device_states_after_meas_gap: "   
     "Skipping tuner operation since device %d is not in RxTx state " , 
                device ); 
   }

   /* Proceed to reset ASM/XSW back to 1x\HDR mode */
   /* This section will be executed only if there is a valid ASM */ 
   if ( NULL != logical_dev->sm[dev_status_r->curr_band] )
   {
     /* Reset ASM */
     ret_val &= rf_cdma_asm_device_reset_switch_state( 
                                       logical_dev->sm[dev_status_r->curr_band],
                                       device,
                                       rf_mode, 
                                       dev_status_r->curr_band 
                                       );
...
)
{
  ...
  else
  {
    ...
    if ( ( ic_data_w->ic_supported ) && 
         ( ic_data_w->ic_state != RF_CDMA_IC_STOPPED ) )
    {    
      /* Stop IC , if applicable */
...
)
{
  ...
  {
    /* Restart IC, if applicable */
    if (  ic_data_w->ic_supported == TRUE )         
    {
...
/*!
  ...
  @param force_microsleep_enable_disable
  1: Enable the debug feature to force microsleep every half slot
  0: Disable the feature This flag is checked every half slot and will be
  latched at the next half slot.
 
  @param rf_off_ustmr_delta
  The delta from the USTMR execution time where the RF will be
  scheduled. A value of 0 results in an immediate RF off.
 
  @param rf_on_ustmr_delta
  The delta from the USTMR execution time where the RF will be scheduled.
  A value of 0 results in an immediate RF off.
 
  @param duration_half_slots
  For HDR, this field specifies the number of half slots to consecutively
  force sleep
  ...
  @param current_mode
  Specifies the RFM_MODE, i.e. 1x or HDR in this case.
 
  @return
  TRUE: API successful
  FALSE: API unsuccessful
*/
boolean
...

{
  ...
  if ( current_mode == RFM_1X_MODE)
  {
    api_status = 
      rf_1x_mdsp_test_rx_on_off ( force_microsleep_enable_disable ,
                                    rf_off_ustmr_delta , rf_on_ustmr_delta , 
                                    duration_half_slots , sleep_mode ,
                                    NULL , NULL ) ;
  }
  else if ( current_mode == RFM_1XEVDO_MODE )
  {
   api_status = 
      rf_hdr_mdsp_test_rx_on_off ( force_microsleep_enable_disable ,
                                    rf_off_ustmr_delta , rf_on_ustmr_delta , 
                                    duration_half_slots , sleep_mode ,
                                    NULL , NULL ) ;
  }
  ...
  {
    RF_MSG_1 ( RF_ERROR , "rfm_cdma_mc_test_rx_on_off: Bad RFM Mode: %d" ,
                 current_mode );
  }
...
/*! @brief 
  [[[[[FTM ONLY>]]]]]
  API to override tx PLL to tune to the Rx DL frequency

  @details
  API to force tx PLL to tune to the DL frequency requested.  
 
  @param device
  RF mode 1x/HDR
 
  @param override_chan
  Channel to which LO is tuned to uint16 lo_tuned_to_chan
 
  @return
  TRUE if tx tune is success
  FALSE if tx tune is failure
*/
boolean
...
)
{
  ...
  if ( dev_r == NULL || logical_dev == NULL )
  {
    RF_MSG_1( RF_ERROR, "rf_cdma_mc_tx_pll_override: Dev %d - NULL "
                        "device or RFC data", device );
    return FALSE;
  } /* if ( dev_r == NULL ) */

  /* Check if device is in rxtx state */
  if( ( dev_r->data_1x.rf_state == RF_CDMA_STATE_RXTX )
      ||
      ( dev_r->data_hdr.rf_state == RF_CDMA_STATE_RXTX ) )
  {
    ...
    if( wakeup_tx_script == NULL )
    {
      RF_MSG( RF_ERROR, "rf_cdma_mc_tx_pll_override: "
                        "Unable to create buffer object ");
      ret_val = FALSE;
    }/*if( wakeup_tx_script == NULL )*/
    else
    {
      ...
      /* check if xpt is enabled for HDR*/
      is_xpt_enabled_for_hdr = 
                            rf_cdma_xpt_is_enabled_for_hdr(dev_r->num_carriers);

      ret_val = 
        rfdevice_cdma_create_tx_tune_script( 
             logical_dev->tx_device[dev_r->curr_band], 
             dev_r->rf_mode, 
             dev_r->curr_band, 
             override_chan, 
             dev_r->tx_bw_khz, 
             ...
             is_xpt_enabled_for_hdr),
             &script_data_ptr,
             (rfdevice_trx_sub_type)rf_cdma_data_get_subscriber_id(dev_r->rf_mode),
             TRUE /* The channel passed in is an Rx channel */,
             device );
...
/*!
  @brief
  Helper function for configuring device scripts to CCS event
...
)
{
  ...
  if ( analog_script_event_handle == NULL )
  {
    RF_MSG_1 ( RF_ERROR  , "rf_cdma_mc_configure_script_helper: "
               "NULL event handle returned for event:%d " ,
               event_type ) ;
  }
  else
  ...
  result &= rf_cdma_mc_cleanup_ccs_event( device, event_type );
...
/*!
  @brief
  Update the ASM state to the one requested via QMI interface
 
  @details
  This API is called to update the state of the GRFC based ASM to the value
  requested via the QMI interface. The tuner API is called periodically by
  L1, when there is a request to change the state of a GRFC via the QMI
  interface, the GRFC ASM driver will update it through this API
 
  @param rfm_device_enum_type: device.
  The device for which state update is needed
 
  @retval TRUE if state is updated succesfully
  FALSE in case of failure to update state
*/
boolean
...
)
{
  ...
  /* This section will be executed only if there is a valid ASM */ 
  if ( (m_dev_r->curr_band < RFM_CDMA_MAX_BAND) && (NULL != m_logical_dev->sm[m_dev_r->curr_band]) )
  {
    /* Update ASM state*/
    ret_val = rf_cdma_asm_update_asm_state( 
                                          m_logical_dev->sm[m_dev_r->curr_band],
                                          device,
                                          m_dev_r->rf_mode,
                                          m_dev_r->curr_band );
  }/*if ( NULL != logical_dev->sm )*/
  ...
  {
    RF_MSG_1 ( RF_HIGH , "rf_cdma_mc_update_asm_state: " 
               "No ASM object for device : %d" ,device ) ;
    ret_val = TRUE;
  }
...
/*!
  @brief
  Prepare RF after AsDiv has concluded.

  @param device
  The device on which AsDiv will be performed.
...
) 
{
  ...
  /* Step 1: 
     Disable intelliceiver before entering AsDiv gap, FW loses the Rx chain when
     we enter the gap, therefore any intelliceiver operations are void, FW will
      reject RxAGC config and crash when IC update mssg is sent */
  
  success &= rf_cdma_mc_enable_ic ( device ) ;

  /* Other Post AsDiv steps here... */
...
*/
rf_cdma_mc_sar_forced_antenna_enum_type rf_cdma_mc_get_sar_forced_ant
(
rfm_device_enum_type device,
...
)
{
  ...
  const rf_1x_config_type* rf_1x_config_data_ptr = NULL;
  const rf_hdr_config_type* rf_hdr_config_data_ptr = NULL;
  const rfc_cdma_logical_device_params_type* logical_dev = NULL;
  rf_cdma_config_type* rf_cdma_config_data_ptr = NULL;
  uint16* raw_sar0_p = NULL;
  uint16* raw_sar1_p = NULL;
  uint16 raw_sar0 = 0;
  uint16 raw_sar1 = 0;
  rf_cdma_state_type rf_state;
  rfm_cdma_band_class_type band = RFM_CDMA_MAX_BAND;
  rfcommon_asdiv_position_type curr_antenna_pos;
  rf_cdma_mc_sar_forced_antenna_enum_type ant_forced_state = RF_CDMA_MC_SAR_ANTENNA_NONE;
  ...
  if( dev_status_r == NULL )
   {
     RF_MSG_1( RF_HIGH, "rf_cdma_mc_get_sar_forced_ant: Dev %d NULL Data",device );
     return ant_forced_state;
   }

  switch ( tech )
  {
     case RFM_1X_MODE:
        rf_state = dev_status_r->data_1x.rf_state;
     break;
     case RFM_1XEVDO_MODE:
        rf_state = dev_status_r->data_hdr.rf_state;
     break;
     default:
        RF_MSG_1( RF_HIGH, "rf_cdma_mc_get_sar_forced_ant: Invalid tech %d", tech );
        return ant_forced_state;
  }

   /* make sure mode is Tx for SAR information */
   if ( rf_state != RF_CDMA_STATE_RXTX &&
        rf_state != RF_CDMA_STATE_TX )
   {
      RF_MSG( RF_HIGH, "rf_cdma_mc_get_sar_forced_ant: Not in TX state");
      return ant_forced_state;
   }

   /* Prepare to get SAR value */
   ...
   /* parameter check */
   if ( (band < RFM_CDMA_BC0)       ||
        (band >= RFM_CDMA_MAX_BAND) ||
        (logical_dev == NULL) )
   {
      RF_MSG_1( RF_ERROR, "rf_cdma_mc_get_sar_forced_ant: Invalid Band or logical dev", band );
      return ant_forced_state;
   }

   /* get current antenna position */
   curr_antenna_pos = rfcommon_asdiv_get_current_position( device );

  switch ( tech )
  {
     case RFM_1X_MODE:
        rf_1x_config_data_ptr = rf_cdma_nv_get_1x_data( logical_dev->tx_nv_path[band], band );
        if( rf_1x_config_data_ptr == NULL )
        {
           RF_MSG( RF_ERROR, "%s: rf_1x_config_data_ptr == NULL");
           return ant_forced_state;
        }
        else
        {
     ...
     case RFM_1XEVDO_MODE:
        rf_hdr_config_data_ptr = rf_cdma_nv_get_hdr_data( logical_dev->tx_nv_path[band], band );
        if( rf_hdr_config_data_ptr == NULL )
        {
           RF_MSG( RF_ERROR, "rf_cdma_mc_get_sar_forced_ant: rf_hdr_config_data_ptr == NULL");
           return ant_forced_state;
        }
        else
        {
     ...
     default:
        RF_MSG_1( RF_ERROR, "rf_cdma_mc_get_sar_forced_ant: Invalid tech %d for NV ptr", tech );
        return ant_forced_state;
  }
   ...
   {
      raw_sar0_p = rf_cdma_nv_get_asdiv_sar_value( rf_cdma_config_data_ptr, 0 );
      if ( NULL != raw_sar0_p )
      {
         raw_sar0 = raw_sar0_p[0];
      }

      raw_sar1_p = rf_cdma_nv_get_asdiv_sar_value( rf_cdma_config_data_ptr, 1 );
      if ( NULL != raw_sar1_p )
      {
         raw_sar1 = raw_sar1_p[0];
      }
   }


  /* Check if AsDiv pos 0 is forced disabled
  * For condition if both antennas are forced disabled, preference is to pass through/config0 antenna
  */
  if( raw_sar0 == RF_SAR_FORCE_OTHER_ANTENNA )
  {
     ant_forced_state = RF_CDMA_MC_SAR_ANTENNA_CONFIG_1;
  }

  /* ant0 forced disable overrides ant1 forced disable */
  if( raw_sar1 == RF_SAR_FORCE_OTHER_ANTENNA )
  {
     ant_forced_state = RF_CDMA_MC_SAR_ANTENNA_CONFIG_0;
  }
...
void rf_cdma_QSH_mdump_cb(void)
{
  int16 device;
  ...
  for ( device = RFM_DEVICE_0;
        ...
        device++ )
  {
    if ( rfm_get_current_mode(device) == RFM_1X_MODE || rfm_get_current_mode(device) == RFM_1XEVDO_MODE)
    {	
      rf_cdma_config_device_index_type* temp_rf_cdma_get_config_device_data = rf_cdma_get_config_device_data();
      rflm_c2k_data_t* temp_rflm_cdma_data = rflm_c2k_get_data();
      qsh_mdump_collect_high((void *)temp_rf_cdma_get_config_device_data
                          , sizeof(rf_cdma_config_device_index_type)); 
      qsh_mdump_collect_high((void *)temp_rflm_cdma_data, sizeof(rflm_c2k_data_t)); 
      qsh_mdump_collect_high((void *)&rfm_cmd_buffs, sizeof(rfm_cmd_buffer_type)); 
	  
       if ( ftm_get_mode() == FTM_MODE )
       {
	   /*Check if the FTM Mode has 1X/HDR active or not*/
		ftm_state = ftm_get_current_state(device);
	   
	  if(ftm_state == FTM_STATE_1X || ftm_state == FTM_STATE_1X_IRAT 
                      || ftm_state == FTM_STATE_1X_NS ||
	   	    ftm_state == FTM_STATE_HDR || ftm_state == FTM_STATE_HDR_IRAT 
                    || ftm_state == FTM_STATE_HDR_NS)
	  {
		   ftm_cdma_data_type* temp_ftm_cdma_data= ftm_cdma_get_data();
           qsh_mdump_collect_high((void *)temp_ftm_cdma_data, sizeof(ftm_cdma_data_type)); 
         }
       }
	else
       {
         qsh_mdump_collect_high((void *)rf_cdma_get_data(), sizeof(rf_cdma_data_type)); 
        }
	break;
     }
  }
