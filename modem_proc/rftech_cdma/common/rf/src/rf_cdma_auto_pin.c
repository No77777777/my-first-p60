...
$DateTime: 2021/03/09 05:07:58 $ $Author: pwbldsvc $
$Header: //components/rel/rftech_cdma.mpss/3.11/common/rf/src/rf_cdma_auto_pin.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/12/16   vr      Legacy Autopin Support for WTR3925
03/10/16   rs      Changing RF_CDMA_CCS_EVENT* to RFLM_CDMA_MDSP_CCS_EVENT*
03/03/16   rs      Update Autopin log packet with PIN ref value
12/29/15   vr      Clear script buffer if Autopin is not supported
12/24/15   vr      Autopin check-in
...

{
  ...
  /* compute RF device script for Autopin G1and push it to firmware */
   ...
   rflm_cdma_mdsp_script_id_t event_id = RFLM_CDMA_MDSP_CCS_EVENT_AUTOPIN_TRIGGER_G1_ID ;
  ...
  if (result == FALSE)
  {
   ...
   else
   {
	 ...
	 result &= rf_cdma_mdsp_configure_devices( device ,
				  ...
				  analog_script_event_handle_2,
				  (rf_cdma_mdsp_get_event_info(RFLM_CDMA_MDSP_CCS_EVENT_AUTOPIN_TRIGGER_G2_ID)),
				  autopin_trigger_script_g2, 
				  0,
				  rf_cdma_data_get_txlm_handle (device) ,
				  NULL, NULL );
...
void rf_cdma_autopin_update_par_pin
(
  uint32 handle,
  uint8 pa_state,
  int16 pin_accum
