...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/common/rf/src/rf_cdma_intelliceiver.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
01/07/16   pk      Fix Intelliceiver Logic bug
11/26/15   rs      Added FL DRX override support
10/01/15   zhw     Undef timer properly before redefing
09/21/15   pk      Provide FW with last known RxAGC value when unfreezing RxAGC
09/23/15   zhw     Enable intelliceiver for PRx VCO devices
05/20/15   zhw     Remove redundant F3s
...
)
{
  ...
  if ( curr_mode == RFM_1X_MODE )
  {
    ...
    fwrsp_data = rf_cdma_get_fwrsp_data(ic_data->device);
    ...
    {
      /* since FL DRX is not active durint calibration no need to override it */
      if ( rfm_get_calibration_state() == FALSE )
      {
        /* Stop Discontinuous Rx */  
        fwrsp_data->fl_drx_override_rsp.device = ic_data->device;
        fwrsp_data->fl_drx_override_rsp.sema_token = rfcommon_semaphore_pop_item();
        fwrsp_data->fl_drx_override_rsp.blocking_call = TRUE;

        rsp_cb_param.rsp_handler = rf_1x_mdsp_fl_drx_override_rsp_cb;
        rsp_cb_param.handler_data = &fwrsp_data->fl_drx_override_rsp;

        ret_val = rf_1x_mdsp_fl_drx_override( logical_dev->rx_mdsp_path, 
	  	                              FALSE, /* Stop DRX */
        ...
        if ( ret_val == FALSE )
        {
          ...
          RF_MSG_2 ( RF_HIGH, "rf_cdma_ic_set_power_mode():"
                     " Freeing the semaphore and not waiting "   
                     " [D%d] IC_UPDATE pushed back : Sem @ 0x%x" , 
                     ic_data->device , fwrsp_hk->fl_drx_override_rsp.sema_token ) ;
        }
        else
        {
          RF_MSG_2( RF_HIGH, "[1X][RF->FW Sync] [D%d] IC_UPDATE : "
                    "Start Wait Sem @ 0x%x", ic_data->device,
                    fwrsp_hk->fl_drx_override_rsp.sema_token );
          rfcommon_semaphore_wait( fwrsp_data->fl_drx_override_rsp.sema_token );
        }

        /* Push back semaphore after wait if mssg was sent successfully,
        or immediately if mssg sending failed*/
        rfcommon_semaphore_push_item( fwrsp_data->fl_drx_override_rsp.sema_token );
      }

      rf_1x_mdsp_rx_agc_read_type rx_agc_data;

      /* Save the last known value of RxAGC before stopping it */
      rf_1x_mdsp_get_rx_agc_data( logical_dev->rx_mdsp_path, 
                                  &rx_agc_data );

      agc_mode_info.prev_agc = rx_agc_data.rx_agc;
  ...
  else if ( curr_mode == RFM_1XEVDO_MODE )
  {
    ...
    else
    {
      ...
      /* Save the last known value of RxAGC before stopping it */
      rf_hdr_mdsp_get_rx_agc_data( logical_dev->rx_mdsp_path, 
                                   &rx_agc_data );

      /* Prev AGC for all carriers is identical, hence hard code carrier to
      zero */
      acq_mode_data.params.resume_acq.prev_agc = 
...
/*! 
  @brief
  Tell IC SM that the device is primary.
 
  @details
  Tell IC SM that the device is primary so proper switchpoints
  can be selected.
...
*/
boolean
rf_cdma_set_ic_support
(
  rf_cdma_ic_data_type*     ic_data ,
  boolean                   is_ic_needed 
)
{
  boolean success = FALSE ;
  if ( ic_data != NULL )
  {
    ic_data->is_primary_rfm_device = is_ic_needed ;
    success = TRUE ;
  }
  return success ;
}
...
)
{
  ...
  /* Undef timer before re-initializing it*/
  timer_undef(&ic_data->ic_timer);
...
)
{
  ...
  if ( ( rf_cdma_debug_flags.use_icvr == FALSE ) || 
       ( rfc_cdma_is_rx_device ( device ) == FALSE ) )
  {
    ic_supported = FALSE;
  }
  else
...
)
{
  ...
  /* PM1 has the correct switchpoints when Jammer not present */
  if ( ( ic_data->is_primary_rfm_device == FALSE ) && 
       ( ( rfm_get_calibration_state() == FALSE )  ) )
  {
    return RFM_CDMA_POWER_MODE_1 ;
  }
