...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/common/rf/src/rf_cdma_tx_agc.c#1 $
...
08/31/15   wwl     Update rf_cdma_tx_agc_autopin_update to fix crash error
08/28/15   cdb     Remove Autopin featurization
06/23/15   pv      Intra-band ULCA LUT hook up.
06/25/15   wwl     Added AutoPin support
03/31/15   jr      Add ASM device to TxAGC configuration
03/30/15   zhw     New RF State CDMA_TX to support splitted Tx device
...
)
{
  ...
  rfm_bw.cdma_bw = RFCOM_BW_CDMA_1P4MHz;
  cal_rfm_bw.cdma_bw = RFCOM_BW_CDMA_1P4MHz;
  ...
  if( init_status == TRUE )
  {
    ...
    {
      init_status &= rf_common_xpt_init_dpd( device, NULL );
    }
          ...
          pa_state++)
    {
      ...
      if ( valid_linearizer_lut_data == TRUE )
      {
        ...
        if ( rf_cdma_debug_flags.disable_temp_comp == 0 )
        {
          ...
          RF_MSG_3( RF_LOW, "rf_cdma_tx_agc_load_linearizer: pa_state = %d, "
                    "aux_lin_data = %d, %d",
                    pa_state, dev_lin_data.aux_lin_data, dev_status_w->temp_comp.init_trigger);
    ...
    if ( ( dev_status_w->temp_comp.init_trigger == TRUE ) &&
         ...
         ( is_dpd_populated ) )
    {
      init_status &= rf_common_xpt_commit_dpd_data( device, NULL );
    }
...
)
{
  ...
  else
  {
    ...
    if ( dev_status_w->temp_comp.is_last_reading_valid == TRUE )
    {
      ...
      temp_vs_freq_params.auto_pin_enable = (rfcommon_autopin_is_enabled_flag_check(dev_status_w->txlm_handle) && rflm_autopin_sanity_check(dev_status_w->txlm_handle));
    ...
    autopin_template = (rflm_autopin_dm_template_t *)rflm_dm_get_autopin_buf_ptr(dev_status_w->txlm_handle);
    ...
    if (autopin_template == NULL)
    {
       RF_MSG_2(RF_ERROR,"rf_cdma_autopin_update_pin: Failed to get AutoPin buffer from DM for Tx handle %d, can't log ref pin %d", 
						dev_status_w->txlm_handle, dev_status_w->pin_comp_val_temp_vs_freq[pa_state]);
    }
    else
    {
    ...
    if(txlin_first_sweep_tbl.lin_data != NULL)
    {
      ...
      if(txlin_final_table.lin_data != NULL)
      {
        ...
        status_flag = rfcommon_core_txlin_temp_vs_freq_comp_update(
                                      dev_status->temp_comp.temp_comp_algo,
                                      &txlin_curr_table,
                                      &txlin_final_table,
        ...
        {
//#ifdef FEATURE_RF_HAS_WTR3925
 //         dev_status->pin_comp_val_temp_vs_freq[pa_index] = pin_comp_val;
//#endif
...
/*!
  @brief
  This function update the Tx linearizer with an offset to the
  Pout
  ...
  @param
  Offset value by which the Pout needs to be compensated value
...
)
{
  ...
  RF_MSG_2( RF_MED, "rf_cdma_tx_agc_adjust_pout: Pin: %d, PA state: %d",
            offset , pa_index );
    ...
    if(txlin_first_sweep_tbl.lin_data != NULL)
    {
      ...
      if(txlin_final_table.lin_data != NULL)
      {
        ...
        status_flag = rfcommon_core_txlin_temp_vs_freq_comp_update(
                                      ...
                                      &txlin_final_table,
                                      &txlin_first_sweep_tbl,
                                      offset , /* Pout compensated by offset */
                                      0 , /* 0 Pin Comp */
                                      RF_CDMA_MDSP_TX_LIN_TBL_SIZ,
                                      FALSE /* Dont skip_Pout_comp ? */ ) ;

        if( TRUE == status_flag )
        {
          /* Ask Chirag if this value needs to be appended or replace */
          dev_status->pout_comp_val_temp_vs_freq[pa_index] += offset ;
          ...
          {
            RF_MSG_1( RF_ERROR,
                      "rf_cdma_tx_agc_adjust_pout: update MDSP failure %d",
                      pa_index );
          }
        }
        else
        {
          RF_MSG_2( RF_ERROR,
                    "rf_cdma_tx_agc_adjust_pout: temp comp populate "
                    "failure for pa_index = %d pout_comp = %d",
                     pa_index , offset );
        }
      }
      else
      {
        RF_MSG( RF_ERROR, "rf_cdma_tx_agc_adjust_pout: "
                "Fail to allocate memory for processing linearizer final");
      }
    }
    else
  ...
  RF_MSG_1( RF_MED, "rf_cdma_tx_agc_adjust_pout: status: %d", status_flag);
...
*/
boolean
rf_cdma_tx_agc_update_with_offset
(
  rfm_device_enum_type  device ,
  int16                 offset
)
{
  ...
  if ( offset != 0 )
  {
      uint8 pa_state = RFM_PA_GAIN_STATE_0;
    /* Update the linearizer data for each PA gain state */
    for ( ; ( ( pa_state < RFCOM_TXLIN_NUM_LINEARIZERS ) && ( success == TRUE ) ) ;
              pa_state++ )
    {
      success = rf_cdma_tx_agc_adjust_pout ( device , offset , pa_state ) ;
    }
  }
  return success ;
}
