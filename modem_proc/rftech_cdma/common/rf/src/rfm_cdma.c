...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/common/rf/src/rfm_cdma.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
06/20/18   am      Initialize maat ptrs to default maxes 
05/15/18   am      Changes to support MAAT
01/30/18   am      F3 reduction
01/04/16   rs      F3 reduction to avoid excessive F3 logging
08/17/15   wwl     relocate rfm_cdma_get_mtpl() to rfm_1x.c
...
04/23/15   zhw     Handle Tx device in rfm_cdma_is_tunable_on_device
...
)
{
  ...
  if ( logical_dev == NULL )
  {
    RF_MSG_DBG_1 ( RF_ERROR, "rfm_cdma_is_tunable_on_device: NULL logical data for "
                        "device %d", device );
  }
  ...
  {
    /* Skip checking BC config if device is Tx device */
    if ( rfc_cdma_is_tx_device( device ) == FALSE )
    {
    ...
    {
      /* Do not check for BC Config for Tx device */
      result = TRUE;
    }
    ...
    if ( TRUE == result )
    {
      ...
      if ( TRUE == result   )
      {
        if ( FALSE == rfm_get_calibration_state() )
        {
          ...
          {
            nv_path = logical_dev->tx_nv_path[band];
          }
          else
          {
            nv_path = logical_dev->rx_nv_path[band];
          }

          result =
            rf_cdma_nv_device_data_is_valid( nv_path,
                                             band) ;
          if ( FALSE == result )
          {
            /* If NV isn't valid for this band, then it isn't supported. */
            RF_MSG_DBG_1 ( RF_MED, "rfm_cdma_is_tunable_on_device: "
      ...
      {
        RF_MSG_DBG_1 ( RF_MED, "rfm_cdma_is_tunable_on_device: Failed on device:%d "
                   , device ) ;
      }
    }
    else
    {
      RF_MSG_DBG_2 ( RF_HIGH, "rfm_cdma_is_tunable_on_device:On device:%d, Band %d "
...
/*!
  @brief
  Prepare RF to make it ready for impending AsDiv.

  @param device
  The device on which AsDiv will be performed.

  @param params
  AsDiv params that need to be configured.
...
)
{
  ...
  rf_time_type asdiv_prep_time ; /* Time spent in function */
  rf_time_tick_type asdiv_prep_tick ; /* Start time of function */

  /* Start to Time Measurement */
  asdiv_prep_tick = rf_time_get_tick() ;

  /* Acquire Critical Section Lock */
  rf_common_enter_critical_section ( rf_cdma_get_lock_data() ) ;

  /* This API will perform all pre-AsDiv preperations */
  result =  rf_cdma_mc_prep_asdiv ( device , params ) ;

  /* Release Critical Section Lock */
  rf_common_leave_critical_section(rf_cdma_get_lock_data());

  /* End Time Measurement */
  asdiv_prep_time = rf_time_get_elapsed( asdiv_prep_tick, RF_USEC ) ;

  RF_MSG_3 ( RF_MED , "rfm_cdma_prep_asdiv: [Status %d] Dev %d, "
             "TimeSpent %d us ", result, device , asdiv_prep_time ) ;
...
/*!
  @brief
  Tell RF that AsDiv has concluded so that RF can prepare for
  normal opecation.

  @param device
  The device on which AsDiv was performed.
...
)
{
  ...
  /* This API will perform all pre-AsDiv preperations */
  result =  rf_cdma_mc_end_asdiv ( device ) ;
  ...
  RF_MSG_3 ( RF_MED , "rfm_cdma_end_asdiv: [Status %d] Dev %d, "
...
/*!
  @brief
  Populates CDMA Rx & Tx details needed by the common MAAT module.

  @details
  This function will loop through all the possible devices and check RFC for TX 
  capable devices. Then check if they are active, if so then check if they are in 
  1x or HDR mode and in Tx active state.

  We will only populate Rx and Tx data for MAAT module if any one of 1x or HDR is
  in Tx state. If found first populate Tx details and call API to populate Rx 
  details using associated master device (PRx device) to the Tx device.
...
{
  rfm_device_enum_type dev_ctr;
  rfm_device_enum_type assoc_prx_dev; 
  boolean rx_info_status = FALSE;
  const rf_cdma_data_status_type* dev_status_r;
  
  if (tx_maat_info_ptr == NULL || rx_maat_info_ptr == NULL)
  {
    RF_MSG( RF_ERROR, "rfm_cdma_get_maat_info: "
                      "Tx or Rx MAAT info ptr is NULL");
    return FALSE;
  }
  
  /* Initialize maat ptrs to default values to prevent garbage value return */
  tx_maat_info_ptr->active_tx = FALSE;
  tx_maat_info_ptr->tx_band = RFM_CDMA_MAX_BAND;
  tx_maat_info_ptr->tx_rfm_path.rfm_device = RFM_INVALID_DEVICE;
  tx_maat_info_ptr->tx_rfm_path.sig_path = 0xFFFFFFFF;
  rx_maat_info_ptr->rx_band = RFM_CDMA_MAX_BAND;
  rx_maat_info_ptr->prx_rfm_path.rfm_device = RFM_INVALID_DEVICE;
  ...
  if (FTM_MODE == ftm_mode &&
                    FTM_STATE_1X_NS != ftm_get_current_state(RFM_DEVICE_0))
  {
    RF_MSG( RF_ERROR, "rfm_cdma_get_maat_info: "
                      "FTM mode doesn't support MAAT");
    return FALSE;
  }
  
  for ( dev_ctr = RFM_DEVICE_0; dev_ctr < RFM_MAX_WAN_DEVICES ; dev_ctr++ )
  {
    if (rfc_cdma_is_tx_device(dev_ctr) == TRUE)
    {
      dev_status_r = rf_cdma_get_device_status(dev_ctr);
      if (dev_status_r != NULL)
      {
        if (dev_status_r->rf_mode == RFM_1X_MODE)
        {
          if (dev_status_r->data_1x.rf_state == RF_CDMA_STATE_TX ||
              dev_status_r->data_1x.rf_state == RF_CDMA_STATE_RXTX )
          {
            rfm_1x_get_tx_maat_info(dev_ctr, tx_maat_info_ptr);
 
            assoc_prx_dev = rf_cdma_data_get_assoc_master_dev(dev_ctr);
 
            if (assoc_prx_dev != RFM_INVALID_DEVICE &&
                assoc_prx_dev < RFM_MAX_WAN_DEVICES)
            {
              rx_info_status = rfm_1x_get_rx_maat_info(assoc_prx_dev, rx_maat_info_ptr);
            }
            else
            {
              RF_MSG_3( RF_ERROR, "rfm_cdma_get_maat_info: "
                        "Invalid PRx device: %d associated with 1x Tx device: %d "
                        "in rf_state %d", assoc_prx_dev, dev_ctr, dev_status_r->data_1x.rf_state);
            }
            break;
          }
        }
        else if (dev_status_r->rf_mode == RFM_1XEVDO_MODE )
        {
          if (dev_status_r->data_hdr.rf_state == RF_CDMA_STATE_TX ||
              dev_status_r->data_hdr.rf_state == RF_CDMA_STATE_RXTX )
          {
            rfm_hdr_get_tx_maat_info(dev_ctr, tx_maat_info_ptr);
            ...
            if (assoc_prx_dev != RFM_INVALID_DEVICE &&
                assoc_prx_dev < RFM_MAX_WAN_DEVICES)
            {
              rx_info_status = rfm_hdr_get_rx_maat_info(assoc_prx_dev, rx_maat_info_ptr);
            }
            else
            {
              RF_MSG_3( RF_ERROR, "rfm_cdma_get_maat_info: "
                        "Invalid PRx device: %d associated with HDR Tx device %d "
                        "in rf_state %d", assoc_prx_dev, dev_ctr, dev_status_r->data_hdr.rf_state);
            }	
            break;
          }	  
        }
  ...
  if (tx_maat_info_ptr->active_tx == TRUE && rx_info_status == TRUE)
  {
    return TRUE;
  }
  else
