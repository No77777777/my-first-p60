...
$Header: //components/rel/rftech_cdma.mpss/3.11/common/rf/src/rfc_cdma_device_wrapper.cpp#1 $
...
04/15/15   zhw     Integrate device driver change with CDMA
                   Provide stubs for wrapper object
04/09/15   aks     Device driver updates for RFC re-design for Split Rx/Tx
...
/*!
  @brief
  Builds the Rx AGC preload scripts based on the new antenna switch status

  @details
  This function creates a script with WTR settings needed to reload AGC settings
  for diversity on a given Rx path based on the new antenna switch position.

  @param mode
  RF Mode for which Rx AGC reload script is to be created
  ...
  @param exec_type
  enum to indicate direct hardware write or prepare script
  ...
  Enum to indicate whether it's a MEAS or STANDALONE script
...
*/
boolean 
rfc_cdma_device_wrapper::get_rxagc_reload_script
(
  rfm_mode_enum_type mode,
  rfm_cdma_band_class_type band,
  rfm_cdma_chan_type chan,
...
)
{
  ...
  rfc_cdma_device_wrapper* paired_dev_wrapper = (rfc_cdma_device_wrapper*)paired_device;

  for ( device_index = 0; 
        device_index < num_rx_devices;
        device_index++ )
  {
    if ( rfdevice_cdma_rx_get_agc_reload_script( 
           rx_devices[device_index],
           paired_dev_wrapper->get_rx_dev_ptr(device_index),
           mode,
           band,
           chan,
           rx_bw_khz,
           ...
           use_alt_gain) == FALSE )
    {
      RF_MSG_1( RF_ERROR, "rfc_cdma_device_wrapper::get_rxagc_reload_script"
                          " failed for device# %d",
                          device_index );
      status = FALSE;
    }
...
/*!
  @brief
  Reads LNA gain word from device
 
  @param *lna_rd
  Pointer to UINT8 variable which will be updated
  with the current gain state
 
  @param band
  Band to which Rx chain was tuned 
 
  @param rx_mode
  Enum to indicate if Rx chain is used by tech for PRx/DRx operation
 
  @param paired_device
  Ptr to the paired device oject
...
boolean
rfc_cdma_device_wrapper::get_lna_state
(
  uint8* lna_rd,
  rfm_cdma_band_class_type band,
  rfdevice_rx_mode_type rx_mode,
...
)
{
  ...
  for ( device_index = 0;
        ...
        device_index++ )
  {
    if ( rfdevice_cdma_rx_get_lna_state(lna_rd, 
                                        band,
                                        rx_mode, 
                                        rx_devices[device_index],
                                        paired_dev_wrapper->get_rx_dev_ptr(device_index))
...
*/
boolean
rfc_cdma_device_wrapper::get_discontinuous_rx_script
(
  rfm_cdma_band_class_type band,
  boolean rx_on,
  rf_buffer_intf* on_off_setting_buf
)
{
  ...
  for ( device_index = 0; 
        ...
        device_index++ )
  {
    curr_dev_status = rfdevice_cdma_get_discontinuous_rx_script( 
                                         rx_devices[device_index], 
                                         band,
                                         rx_on, 
                                         on_off_setting_buf);
    ...
    if ( curr_dev_status == FALSE )
    {
      RF_MSG_1( RF_ERROR, "rfdevice_cdma_get_discontinuous_rx_script failed for "
                          "device# %d",
                device_index );
      overall_status = FALSE;
    }
...
/*! 
  ...
  @brief
  Returns the Rx device ptr corresponding to the device
  index passed
  ...
  @return
  Ptr to the CDMA Rx Object
 
  */

rfdevice_trx_cdma_rx* 
rfc_cdma_device_wrapper::get_rx_dev_ptr
( 
  uint8 device_idx
)
{
  if ( device_idx >= RFC_CDMA_RXDEV_WRAPPER_MAX_DEVICES ) 
  {
    RF_MSG_1(RF_ERROR, "get_rx_dev_ptr- Invalid device_idx:%d ", 
             device_idx );
    return NULL;
  }

  return rx_devices[device_idx];
