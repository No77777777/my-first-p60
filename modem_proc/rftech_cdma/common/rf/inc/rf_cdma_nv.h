...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/common/rf/inc/rf_cdma_nv.h#1 $
...
{
  uint16 band;
  rfnv_tx_pwr_sar_1_8_temp_comp_type nv_data[ASDIV_POSITION_MAX];
} rfnv_sar_asdiv_item_type;
#endif
...
typedef enum
{
  ...
  RF_CDMA_NV_PATH_4,
  /*!< Physical Path 4 for NV */

  RF_CDMA_NV_PATH_5,
  /*!< Physical Path 5 for NV */
...
typedef struct
{
  ...
  rfnv_item_id_enum_type rx_static_data ;
  /*!< Rx Static NV */
...
/*!
  @brief
  Structure to hold the CDMA's Rx path's auxillary info. 
*/
typedef struct
{
  ...
  /* Offsets to be applied to the Rx calibration data per gain state for
     this switch position.  Units are db10. */
  int16 gain_offset[8];
}
rf_cdma_rx_path_info ;
/*----------------------------------------------------------------------------*/
/*!
  @brief
  CDMA RF driver Rx switchpoint data structure for compensation
  to both 1x and HDR. 
...
typedef struct
{
  ...
  /*!
    ...
    here for 1x, HDR Rev0/RevA mode of operation. This data
    contains information whether the switchpoints are
    active/inactive for a particular linearity state. Each
    switchpoint structure contains the swpt type, the rise/fall
    data and the time hysteresis data. Hold data only if it is
    releavant to both 1x and HDR
  */
  rf_cdma_lna_info_type cdma_aux_lna_info ;

  /*!
    @brief
    Contains Rx auxillary path information.

    @details
    Contains Rx auxillary path information. This can contain
    information like AsDIV data etc.
  */ 
  rf_cdma_rx_path_info  rx_aux_info ; 
  
} rf_cdma_rx_aux_static_type ;
...
typedef struct rf_1x_rx_static_s
{
  ...
  */
  rfcommon_spur_table_type onex_spur_table;
    ...
    /*!
   @details
    The common LNA data is stored in static NV and is loaded into
    the RF driver here for HDR mode of operation.
  */
  rf_cdma_rx_aux_static_type onex_common_rx_aux_data ;
  ...
  */
  ...
  /*!
   @details
    ...
    the RF driver here for HDR mode of operation.
  */
  rf_cdma_rx_aux_static_type hdr_common_rx_aux_data ;
...
/*!
  @brief
  Structure to hold the CDMA's Tx path's auxillary info. 
*/
typedef struct
{
  ...
  /* Offsets to be applied to the Tx calibration data per gain state for
     this switch position.  Units are db10. */
  int16 gain_offset ;
} rf_cdma_tx_path_info ;


/*----------------------------------------------------------------------------*/
/*!
  @brief
  CDMA RF driver data structure for static Tx compensation to
  both 1x and HDR.
...
typedef struct
{
  ...
  /*!
    ...
    information like AsDIV data etc.
  */ 
  rf_cdma_tx_path_info  tx_aux_info ; 
  
} rf_cdma_tx_aux_static_type ;
...
typedef struct rf_1x_tx_static_s
{
  ...
  */
  ...
  /*!
   @details
    The common Tx static NV data loaded into the RF driver here for 
    1x mode of operation. 
  */
  rf_cdma_tx_aux_static_type onex_common_tx_aux_data ;
 ...
 */
 rf_cdma_tx_aux_static_type hdr_common_tx_aux_data ;
...
/* QPA cap adjustement for optimum matching */
typedef int8 rf_cdma_pa_cap_adj_type ;
...
typedef struct rf_cdma_tx_static_s
{
  ...
  */
#if defined(FEATURE_RF_ASDIV)
  int16 lim_vs_temp[ASDIV_POSITION_MAX][RF_CDMA_NV_DSI_SAR_STATE_SIZE][RF_CDMA_NV_TEMP_LIMIT_SIZE];
  ...
  /*!
    ...
    @details
    This item represent the raw contents of the ASDiv SAR table values.
  */
  uint16 sar_tbl[ASDIV_POSITION_MAX][RF_CDMA_NV_DSI_SAR_STATE_SIZE][RF_CDMA_NV_TEMP_LIMIT_SIZE];
#else
  ...
  */
  ...
  /*!
    @brief
    QPA matching is challenging for band edges. Small ACLR margins
    in band edges in some bands. QPA cap is part of the HW matching. Thus
    customers may tune QPA cap to optimize ACLR performance for
    some bands with low margins.
  */
  rf_cdma_pa_cap_adj_type pa_static_output_cap_offset ;
