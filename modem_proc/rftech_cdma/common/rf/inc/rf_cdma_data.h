...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/common/rf/inc/rf_cdma_data.h#1 $
...
typedef enum
{
  ...
  /*! Configure Rx Milli SLEEP ON */
  RF_CDMA_EVENT_RX_ACTIVATE_MILLI_SLEEP ,

  /*! Configure Rx Milli SLEEP OFF*/ 
  RF_CDMA_EVENT_RX_DEACTIVATE_MILLI_SLEEP ,

  /*! Configure Coupler Front Direction */
  RF_CDMA_EVENT_RX_COUPLER_FORWARD_DIRECTION ,

  /*! Configure Coupler Reverse Direction */
  RF_CDMA_EVENT_RX_COUPLER_REVERSE_DIRECTION ,
 
  /*! Configure AutoPin Gain1 */
  RF_CDMA_EVENT_AUTOPIN_TRIGGER_G1_ID,
  
  /*! Configure AutoPin Gain2 */
...
typedef struct 
{
  ...
  */
  ...
  /*! This stores the device to use for query TQ during iRAT. It's the source 
    tech primary device for now, to support QCTA.
  */
...
*/
typedef struct rf_plim_asdiv_cb_s
{
  rfm_device_enum_type            device;
  rfcommon_asdiv_position_type    asdiv_cb_sw_pos_data;
  rfcommon_asdiv_cb_handler_type  asdiv_cb_handler;
} rf_plim_asdiv_cb_type;
#endif
...
/*! 
  @brief
  Structure Containing Common Global Status of CDMA Antennae
  Switch information.
*/
typedef struct
{
  /*! 
    @brief 
    Keeps track Of Pout compensation so to be able to switch
    back
  */
  int16   pout_compensation_used ;

  /*! 
    @brief 
    Keeps track of if RxAGC was reloaded so to be able to switch
    back when eLNA is removed or original params need to be
    restored
  */
  boolean   was_rxagc_reloaded ;

  /*! 
    @brief 
    Keeps track Of antenna position in AsDiv scenario
  */
  uint8   antenna_position ;   
} rf_cdma_ant_switch_params ;
/*----------------------------------------------------------------------------*/
/*! 
  @brief
  Structure Containing Common Global Status of CDMA Driver.
...
#if defined(FEATURE_RF_ASDIV)
  rf_plim_asdiv_cb_type plim_asdiv_cb_data;
#endif

  rfcdma_autopin_state_type autopin_st;
 
  /*! Holder for antennae switch postion at the time of the tune */
  rf_cdma_ant_switch_params rf_cdma_ant_sw_data ;

  /*! This member will be used only when tuners are present on 
    Diversity and Tx path and not on Rx path. While updating Tuner scripts
    on Drx, get the associated Drx path from this member. */
...
/*----------------------------------------------------------------------------*/
/*! Structure containing Subscription specific info*/
typedef struct
{

  rflm_sim_id_t sim_id_1x;
  /*!< Current SIM ID for 1x */

  rflm_sim_id_t sim_id_hdr;
  /*! Current SIM ID for HDR */

} rf_cdma_subscriber_data_type;
...
typedef struct
{
  ...
  rf_cdma_subscriber_data_type subscriber_data;  
