...
$Header: //components/rel/rftech_cdma.mpss/3.11/common/rf/inc/rfc_cdma.h#1 $
...
03/30/15   zhw     Implement rfc_cdma_get_default_tx_logical_device
03/10/15   zhw     Remove Tuner featurization
03/02/15   zhw     Refactor RFC for splittingTx from Rx devices
12/17/14   wwl     add function prototype for rfc_cdma_get_fbrx_modem_chain
12/16/14   spa     Added ELNA object (per band) to logical device list
...
typedef struct rfc_cdma_logical_device_params_s
{
  ...
  */
  ...
  /*!
    @brief
    Table of pointers for the Tx antenna tuner managers for this device.
   
    @details
    Each band may have a different antenna tuner, so this is stored as
    a table of pointers.
  */
  rf_cdma_atuner* tx_ant_tuner[RFM_CDMA_MAX_BAND];
  ...
  */
  rf_cdma_coupler* coupler_obj [RFM_CDMA_MAX_BAND];

  /*!
    @brief
    Holds the pointer to any ELNA device (per band)
  */
  void* elna_obj [RFM_CDMA_MAX_BAND];
