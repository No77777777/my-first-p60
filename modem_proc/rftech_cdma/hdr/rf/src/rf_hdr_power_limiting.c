...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/hdr/rf/src/rf_hdr_power_limiting.c#1 $
...
)
{
  ...
  const rf_1x_config_type* rf_1x_config_data_ptr = 
      rf_cdma_nv_get_1x_data( logical_dev->tx_nv_path[plim_data->band] , 
                              plim_data->band);
  ...
  { /* If CCP is overriden by L1 dont bother getting a 
    ...
    if ( FALSE == rf_hdr_plim_get_ccp_override(plim_data) )
    {
      ...
      if ( NULL != rf_hdr_config_ptr )
      {
        ...
        if ( rf_hdr_get_common_pwr_values ( device, logical_dev , 
...
)
{
       ...
       (plim_data->mode == RFM_1XEVDO_MODE) )
  {
    ...
    RF_MSG_4 ( RF_MED , "rf_hdr_plim_update_temp_mdsp: "
              "HDR PLIM: Started. Max plim SAR(dbm10) %d, "
...
)
{
  ...
  rf_hdr_plim_update_sar_state( cb_data->device );
  return NULL;
}
#endif
  ...
  if ( plim_data->state == RF_CDMA_PLIM_DISABLED )
  {
    ...
    if(NULL != dev_status)
    {
      dev_status->plim_asdiv_cb_data.asdiv_cb_handler = rfcommon_asdiv_register_cb( 
                      ...
                      RFM_1XEVDO_MODE, ASDIV_CB_REPEAT );
    }
#endif
...
#ifdef FEATURE_RF_ASDIV
    if(RF_CDMA_PLIM_ENABLED == plim_data->state)
    {
      ...
      if(NULL != dev_status)
      {
        if(!rfcommon_asdiv_deregister_cb(dev_status->plim_asdiv_cb_data.\
                                       asdiv_cb_handler))
        {
          RF_MSG_1( RF_ERROR, "HDR PLIM: deregister callback failed on device %d, "
...
)
{
  ...
  RF_MSG_4 ( RF_MED , "rf_hdr_plim_update_sv_limit: "
            "HDR PLIM: Started. Max plim SAR(dbm10) %d, "
            "Max plim vs temp vs freq %d (dbm10) min plim (dbm10) %d, " 
            " Max power %d",
            plim_data->max_power_limits.max_power_limit_sar_dbm10,
...
)
{
  ...
  /* Leave critical Section */
  rf_common_leave_critical_section( &plim_data->crit_section );
  RF_MSG_4 ( RF_MED , "rf_hdr_plim_update_thermal_backoff_limit: "
            "HDR PLIM: Started. Max plim SAR(dbm10) %d, "
            "Max plim vs temp vs freq %d (dbm10) min plim (dbm10) %d, " 
            " Max power %d",
            plim_data->max_power_limits.max_power_limit_sar_dbm10,
...
)
{
  ...
  RF_MSG_4 ( RF_MED , "rf_hdr_plim_update_lin_limit: "
            "HDR PLIM: Started. Max plim SAR(dbm10) %d, "
            "Max plim vs temp vs freq %d (dbm10) min plim (dbm10) %d, " 
            " Max power %d",
            plim_data->max_power_limits.max_power_limit_sar_dbm10,
...
)
{
  ...
  RF_MSG_4 ( RF_MED , "rf_hdr_plim_update_lim_vs_temp_vs_freq: "
            "HDR PLIM: Started. Max plim SAR(dbm10) %d, "
            "Max plim vs temp vs freq %d (dbm10) min plim (dbm10) %d, " 
            " Max power %d",
            plim_data->max_power_limits.max_power_limit_sar_dbm10,
