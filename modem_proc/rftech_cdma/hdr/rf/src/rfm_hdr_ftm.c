...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/hdr/rf/src/rfm_hdr_ftm.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
07/09/14   zhw     Remove hard-coded RFM DEVICE in CDMA IQ Capture
...
)
{
  ...
  if ( ( logical_dev != NULL ) && ( dev_status_r != NULL ) )
  {
    ...
    {    
      ...
      {
        RF_MSG_2( RF_ERROR, "rf_hdr_ftm_get_rx_agc_data: "
...
)
{
  ...
  if( rf_state == RF_CDMA_STATE_RX || rf_state == RF_CDMA_STATE_RXTX ||
      device == RFM_DEVICE_0 /* Always use MDSP Chain 0 for Dev 0, for legacy
      QRCT support */ )
  {
    mdsp_chain = RF_CDMA_MDSP_CHAIN_0;
  }
  else if( rf_state == RF_CDMA_STATE_RXDIV ||
           device == RFM_DEVICE_1 /* Always use MDSP Chain 1 for Dev 1.
           for legacy QRCT support */)
  {
  ...
  else
  {
    ...
    RF_MSG_2 (RF_ERROR, "rfm_hdr_ftm_query_iq_sample_line_offset: device %d RF state %d "
                          "incorrect IQ Capture not supported", device, rf_state);
