...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/hdr/rf/src/rfm_hdr.c#1 $
...
08/09/17   am      UMTS denials because of HDR releasing chain lately
21/06/17   kk      modified try_enter_critical_section to enter_critical_section in rfm_hdr_prep_qta
...
03/03/16   rs      Moved prep\end QTA APIs to cdma layer and changed the 
                   API calls accordingly.
12/10/15   rs      Added try enter critical section for prep QTA
10/14/15   rs      Modified sleep API to fix the DO2L iRAT crash
09/01/15   pk      changed the rx_mdsp_path check in rfm_hdr_get_tx_adjust to
                   tx_mdsp_path to fix a logging issue in HDR CL Tx Power
08/23/15   sty     call rf_cdma_mc_update_asm_state() only in TX state in 
                   rfm_1x_antenna_tuner_action_time()
08/19/15   hm      Update ASM  state in periodic tuner API call
...
08/03/15   wwl     Add API header file for ASDiv SAR limits programmable
                   per RAT/band
06/03/14   spa     Update device state to Rx only in IRAT mode in exec wakeup Rx
01/13/15   wwl[pl] Use new atuner api function to decide whether enable antenna
06/10/15   ndb     Cleanup the unused enums
05/26/15   ndb     Move to new tuner interface
04/28/15   zhw     Reworked Tuner API
04/23/15   zhw     Add device number in error reporting message
04/02/15   zhw     HDR TRx split changes -- bring to same status as 1x
03/26/15   wwl     Update wakeup Tx V2 function to include PRX device, remove
                   band and channel parameters
03/04/15   wwl     Add new API functions to split RXTX
03/16/15   pk      Compiler warning Fix
01/21/15   wwl     Remove redundent code from rfm_hdr_antenna_tuner_action_time
...
/*! Difference between proposed 1X and HDR thresholds for MAAT module.
    
    @brief
    This is to make common threshold for both 1X and HDR as tuner manager 
    doesn't distinguish between 1X and HDR. Currenlty for computed threshold
    the threshold is -75dB10 while -120dB is the common threshold. Hence 
    -65dB10 is the bias value for the threshold */
#define RFM_HDR_ECIO_BAISED_THRESHOLD_DB10 65 
...
)
{
  ...
  if ( ret_val ==  RFM_CDMA_HEALTHY_STATE )
  {
    ...
    else
    {
      ...
      /* Set device state to Rx, since RxAGC has converged */
...
)
{
  ...
  if ( are_tasks_running == TRUE )
  {
    ...
    if ( device_state == RFM_CDMA_HEALTHY_STATE )
    {
      ...
      ret_val = rf_hdr_mc_wakeup_tx( device, device, txlm_handle );
...
/*!
  ...
  @return
  The anticipated time in microseconds remaining from the rfm_hdr_wakeup_tx_v2()
  function return until the operation is complete.
*/
rfm_wait_time_t
rfm_hdr_wakeup_tx_v2
(
  const rfm_device_enum_type device_tx,
  const rfm_device_enum_type device_prx,
  const lm_handle_type txlm_handle,
  const rfm_cb_handler_type callback,
  void* const userdata
)
{
  ...
  RFM_DEBUG_EN_Q(&rfm_hdr_wakeup_tx, device_tx, txlm_handle, &callback, &userdata);

  /* Throw F3 Message */
  RF_MSG_2( RF_HIGH, "rfm_hdr_wakeup_tx : START - Rx Dev %d Tx Dev %d",
            device_prx, device_tx );

    /* Query if all required tasks are running or not */
  are_tasks_running &= rf_cdma_required_tasks_running(device_prx);

  /* Perform RF action, only if all required tasks are running */
  if ( are_tasks_running == TRUE )
  {
    /* Validate the Currente State */
    device_state = rfm_hdr_validate_state( device_prx, RF_CDMA_STATE_RX );

    /* Proceed only if State is valid, i.e. device_state is non-negative  */
    if ( device_state == RFM_CDMA_HEALTHY_STATE )
    {
      /* Enter HDR mode on Tx device */
      rfm_enter_mode( device_tx, RFM_1XEVDO_MODE, NULL, NULL, 0 );

      /* Call 1x Main Control to Prepare Tx for Wakeup */
      ret_val = rf_hdr_mc_wakeup_tx( device_tx, device_prx, txlm_handle );
  ...
  /* Report wakeup time, Tx AGC and PA state */
  RF_MSG_4( RF_HIGH, "rfm_hdr_wakeup_tx : [Status %d] - Rx Dev %d, Tx Dev %d, "
                     "TxWakeupTime %d",
            ret_val, device_prx, device_tx, wakeup_time_val );

  if (ret_val != RFM_CDMA_HEALTHY_STATE)
  {
    RF_MSG_2( RF_ERROR, "rfm_hdr_wakeup_tx: API FAILED. Rx Dev %d, Tx Dev %d",
              device_prx, device_tx );
  }
...
)
{
  ...
  {
    /* Validate the Currente State */
    device_state_1 = rfm_hdr_validate_state( device, RF_CDMA_STATE_RXTX );
    device_state_2 = rfm_hdr_validate_state( device, RF_CDMA_STATE_TX );
...
)
{
  ...
  ret_val = rfm_hdr_retune(device, rxlm_handle, num_band_chan,
                           band_chan_array, carrier_index, callback, userdata);
...
)
{
  ...
  if ( are_tasks_running == TRUE )
  {
    ...
    if ( ( rfm_hdr_validate_state( device, RF_CDMA_STATE_RXTX ) ==
           RFM_CDMA_HEALTHY_STATE ) ||
         ( rfm_hdr_validate_state( device, RF_CDMA_STATE_TX ) ==
           RFM_CDMA_HEALTHY_STATE ) )
    {
      RF_MSG_1 ( RF_ERROR, "rfm_hdr_update_rev_link_carrier_set: device %d not "
               "RXTX or TX state" , device ) ;
    }
...
)
{
  ...
  *nominal_pwr_db = 0;
  ...
  /* Validate the current state */
  if ( ( rfm_hdr_validate_state( device, RF_CDMA_STATE_TX ) ==
             RFM_CDMA_HEALTHY_STATE ) ||
       ( rfm_hdr_validate_state( device, RF_CDMA_STATE_RXTX ) ==
             RFM_CDMA_HEALTHY_STATE ) ||
       ( rfm_hdr_validate_state( device, RF_CDMA_STATE_RX ) ==
             RFM_CDMA_HEALTHY_STATE ) )
  {
...
)
{
  ...
  if ( logical_dev->tx_mdsp_path >= RF_CDMA_MDSP_CHAIN_NUM )
  {
    RF_MSG_1( RF_ERROR, "rfm_hdr_get_tx_adjust: Invalid "
              "tx_mdsp_path %d", logical_dev->tx_mdsp_path );
    return 0xFFFF;
  }
...
)
{
  ...
  if ( logical_dev->tx_mdsp_path >= RF_CDMA_MDSP_CHAIN_NUM )
  {
    RF_MSG_2( RF_ERROR, "rfm_hdr_get_tx_pilot_power(): "
                        "Invalid tx_mdsp_path %d, device %d",
                        logical_dev->tx_mdsp_path , device);
    return 0xFFFF;
  }
...
)
{
  ...
  if ( logical_dev->tx_mdsp_path >= RF_CDMA_MDSP_CHAIN_NUM )
  {
    RF_MSG_2( RF_ERROR, "rfm_hdr_get_tx_total_power(): "
                        "Invalid tx_mdsp_path %d, device %d",
                        logical_dev->tx_mdsp_path, device );
    return 0xFFFF;
  }
  ...
  if ( NULL != dev_status_r )
  {
    ...
    {
      RF_MSG_DBG_1 ( RF_ERROR, "rfm_hdr_get_tx_total_power: Device not in RXTX "
                           "state - current state %d ",
                           dev_status_r->data_hdr.rf_state );
      tx_total = 0xFFFF;
    }
...
)
{
  ...
  if ( logical_dev->tx_mdsp_path >= RF_CDMA_MDSP_CHAIN_NUM )
  {
    RF_MSG_2( RF_ERROR, "rfm_hdr_get_filtered_tx_power(): "
                        "Invalid tx_mdsp_path %d, device %d",
                        logical_dev->tx_mdsp_path ,device );
    return filtered_tx_power ;
  }
...
)
{
  ...
  if ( logical_dev->tx_mdsp_path >= RF_CDMA_MDSP_CHAIN_NUM )
  {
    RF_MSG_2( RF_ERROR, "rfm_hdr_get_filtered_tx_power_v2(): "
                        "Invalid tx_mdsp_path %d, device %d",
                        logical_dev->tx_mdsp_path , device );
    return filtered_tx_power ;
  }
  ...
  if ( NULL != dev_status_r )
  {
    ...
    filtered_tx_power =
      rf_cdma_convert_txagc ( RFM_CDMA_AGC_FORMAT__DBM10,
                              tc , filtered_tx_power ) ;
    RF_MSG_2 ( RF_LOW , "rfm_hdr_get_filtered_tx_power_v2(): "
                        "filter_type : %d filtered Tx power : %d",
                         filter_type, filtered_tx_power ) ;
...
)
{
  ...
  {
    RF_MSG_1( RF_ERROR, "rfm_hdr_notify_data_rate: rf_hdr_config_ptr is NULL. "
                        "Device %d", device );
    return;
  }
...
)
{
  ...
  {
    RF_MSG(RF_HIGH, "rfm_hdr_antenna_tuner_action_time: [ASDIV]Switch "
					"in progress, Skip update");
  ...
  */
    ...
    else
    {
          ...
          m_dev_r->data_hdr.rf_state == RF_CDMA_STATE_RXDIV )
      {
        RF_MSG( RF_MED, "rfm_hdr_antenna_tuner_action_time: "
			   ...
			   m_dev_r->data_hdr.rf_state == RF_CDMA_STATE_TX )
      {
        ...
        tuner_config.mode = RFM_1XEVDO_MODE;
             ...
             ( band < RFM_CDMA_MAX_BAND ) )
        {
          ...
          else
          {
              boolean asm_update_ret;
                  ...
                  asm_update_ret = rf_cdma_mc_update_asm_state( device, m_dev_r,
                                                                m_logical_dev );
                  ...
                  if ( asm_update_ret == FALSE )
                  {
                    RF_MSG_1( RF_ERROR, "rfm_1x_antenna_tuner_action_time: "
                      "[Status %d] Dev %d, ASM update failure!",
                  device );
                    ret_val = RFM_CDMA_ERROR_FAILURE;
                  }
          }
  ...
  {
    RF_MSG_1( RF_MED, "rfm_hdr_antenna_tuner_action_time: [Status %d] Dev %d, "
...
)
{
  ...
  /* Acquire enter Critical Section Lock
...
*/
int32
rfm_hdr_antenna_tuner_action_time_v2
(
  const rfm_device_enum_type device,
  const rfm_device_enum_type tx_device
)
{
  ...
  RF_MSG_DBG_2( RF_MED, "rfm_hdr_antenna_tuner_action_time_v2: [Rx Dev %d] [TxDev %d], " ,
            device, tx_device ) ;

  ret_val = rfm_hdr_antenna_tuner_action_time ( tx_device ) ;  
...
)
{
  ...
  else
  {
    ...
    else
    {
      ...
      {
        RF_MSG_1( RF_ERROR, "%s: rf_hdr_config_data_ptr == NULL", __func__);
      }
      else
      {
...
*/
boolean
rfm_hdr_set_subscriber_id
(
  uint8 sub_id
)
{
  return rf_cdma_data_set_subscriber_id(RFM_1XEVDO_MODE, sub_id);  
}/* rfm_hdr_set_subscriber_id */

/*----------------------------------------------------------------------------*/
/*!
  @brief
  Populates HDR Tx details needed by the common MAAT module.

  @details
  This API will be called if there is a active HDR Tx device in TX or RXTX state.
  It will first set the active_tx flag to indicate the MAAT module that HDR is in
  ...
  @param tx_device
  The Tx device being used by HDR.
...
)
{
  ...
  if (dev_status_r == NULL)
  {
    RF_MSG_1( RF_ERROR, "rfm_hdr_get_tx_maat_info: NULL data for Tx device %d",
            tx_device );
  ...
  RF_MSG_5( RF_MED, "rfm_hdr_get_tx_maat_info: HDR Tx Details to MAAT module - "
  ...
  RF_MSG_2( RF_HIGH, "rfm_hdr_get_tx_maat_info: HDR Tx Power to MAAT module - "
...
/*!
  @brief
  Populates HDR Rx details needed by the common MAAT module.

  @details
  This API will be called with the HDR PRx device as a parameter. The PRx device is 
  ...
  This API will query the FW SMEM and calculate the SNR in dBm10 units to be 
  passed to the MAAT module.

  @param rx_device
  The PRx device being used by HDR.
...
)
{
  ...
  if (dev_status_r != NULL)
  {
    ...
    {
      if (dev_status_r->data_hdr.rf_state == RF_CDMA_STATE_RX ||
              dev_status_r->data_hdr.rf_state == RF_CDMA_STATE_RXTX )
      {
            ...
            assoc_div_dev < RFM_MAX_WAN_DEVICES)
        {
          ...
          {
            if (drx_dev_status_r->rf_mode == RFM_1XEVDO_MODE)
            {
              if (drx_dev_status_r->data_hdr.rf_state == RF_CDMA_STATE_RXDIV)
              {          
              ...
              {
                RF_MSG_2( RF_ERROR, "rfm_hdr_get_rx_maat_info: "
                          "Associated DRx device %d not in RXDIV state. Current State: %d", 
                          assoc_div_dev, drx_dev_status_r->data_hdr.rf_state );  	  
              }			  
            }
            else
            {
              RF_MSG_2( RF_ERROR, "rfm_hdr_get_rx_maat_info: "
                        "Associated DRx device %d not in HDR Mode. Current Mode: %d", 
                        assoc_div_dev, drx_dev_status_r->rf_mode );    
            }			
          }
          else
          {
            RF_MSG_1( RF_ERROR, "rfm_hdr_get_rx_maat_info: NULL data for associated DRx "
				                "device %d", assoc_div_dev );
          }		  
        }
        else
        {
          RF_MSG_DBG_2( RF_LOW, "rfm_hdr_get_rx_maat_info: Invalid/Disabled DRx device: %d "
                        "associated with HDR PRx device %d. Setting invalid drx_rfm_path",
                        assoc_div_dev, rx_device);
          rx_maat_info_ptr->drx_rfm_path.rfm_device = RFM_INVALID_DEVICE;
          rx_maat_info_ptr->drx_rfm_path.sig_path = 0xFFFFFFFF;		    
        }
  	
        if (rf_hdr_mdsp_get_snr(logical_dev->rx_mdsp_path, &snr) == TRUE)			
        {
          /* Converting SNR from the from Q9 format to Q0, 
             Computed EcIo = 1/(1+1/SNR) = SNR/(1+SNR)    */
          ecIo      = snr/512.0; 
          ecIo_db10 = 100*log10(ecIo/(1+ecIo));

          /* Adding bias value, This is to maintain common threshold for both 1X and HDR */
          rx_maat_info_ptr->snr_db10 = ecIo_db10-RFM_HDR_ECIO_BAISED_THRESHOLD_DB10;
		  
          api_status = TRUE;
        }
        else
        {
          RF_MSG( RF_ERROR, "rfm_hdr_get_rx_maat_info: SNR not populated correctly");
          rx_maat_info_ptr->snr_db10 = 0;
        }

        RF_MSG_6( RF_MED, "rfm_hdr_get_rx_maat_info: HDR Rx Details to MAAT module - "
        ...
        RF_MSG_2( RF_HIGH, "rfm_hdr_get_rx_maat_info: HDR SNR to MAAT module - "
                      "RAW SNR from FW = %d, Converted SNR in dB10 = %d",
                      snr, rx_maat_info_ptr->snr_db10);			
      ...
      else
      {
        RF_MSG_2( RF_ERROR, "rfm_hdr_get_rx_maat_info: "
                  ...
                  rx_device, dev_status_r->data_hdr.rf_state );  	  
      }
    }
    else
    {
      RF_MSG_2( RF_ERROR, "rfm_hdr_get_rx_maat_info: "
                "Associated PRx device %d not in HDR Mode. Current Mode: %d", 
                rx_device, dev_status_r->rf_mode );    
    }
  }
  else
  {
    RF_MSG_1( RF_ERROR, "rfm_hdr_get_rx_maat_info: NULL data for PRx device %d",
              rx_device );
  }
  return api_status;
}
