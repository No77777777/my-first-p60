...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/hdr/rf/src/rf_hdr_rx_agc.c#1 $
...
)
{
  ...
  if ( ret_val == TRUE )
  {
    ...
    if ( rfm_get_calibration_state() == FALSE )
    {
      ...
      if ( TRUE == rf_hdr_is_rx_aux_data_valid ( rf_hdr_config_ptr , position ) )
      {
        if ( FALSE == 
             rf_cdma_rx_adjust_lna_info ( (void*)rf_hdr_config_ptr ,
                                          RFM_1XEVDO_MODE , rx_agc_param ) )
        {
          RF_MSG ( RF_ERROR , "rf_hdr_rx_agc_compute_swpts(): "
        ...
        else
        {
          ...
          RF_MSG_1 ( RF_LOW , "rf_hdr_rx_agc_compute_swpts(): New RxAGC data loaded "
...
)
{
  ...
  if ( logical_dev == NULL || dev_status == NULL )
  {
    ...
    RF_MSG_1( RF_ERROR, "rf_hdr_update_rx_agc: Invalid Dev = %d",
              device );
  ...
  if ( ret_val == TRUE )
  {
    ...
    fwrsp_hk = rf_cdma_get_fwrsp_data( device );
      ...
      else
      {
        ...
        if ( ret_val == FALSE )
        {
          RF_MSG ( RF_ERROR, "rf_hdr_update_rx_agc(): "
                             "rf_1x_mdsp_resume_rx_agc failed "); 
          /* Message sending failed, free the semaphore */
          RF_MSG_2 ( RF_LOW, "rf_hdr_update_rx_agc():"
