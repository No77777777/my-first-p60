...
$Header: //components/rel/rftech_cdma.mpss/3.11/hdr/ftm/src/ftm_hdr_meas.c#1 $
...
static rfm_meas_tech_setup_params_type ftm_hdr_meas_param;
...
)
{
  ...
  {
    FTM_MSG(FTM_ERROR , 
        "ftm_hdr_meas_get_tech_param : test param pointer is null ");
    return ftm_hdr_meas_param;
  }
     ...
     (test_param_ptr->tgt_tech == RFM_1XEVDO_MODE))
  {
    ...
    if(test_param_ptr->src_tech == RFM_1XEVDO_MODE)
    {
      ftm_hdr_meas_param.hdr_params.band_chan.band = test_param_ptr->src_ca_path0_band;
      ftm_hdr_meas_param.hdr_params.band_chan.chan_num = test_param_ptr->src_ca_path0_chan_car0;
    }
    else
    {
      ftm_hdr_meas_fw_on_off_control( ftm_hdr_meas_get_current_device(test_param_ptr), 
                                      RFM_CDMA_FW_STATE_MEAS );
      ftm_hdr_meas_param.hdr_params.band_chan.band = test_param_ptr->tgt_band ;
      ftm_hdr_meas_param.hdr_params.band_chan.chan_num = test_param_ptr->tgt_chan;
    }
  ...
  else
  {
    /* Throw error message */
    FTM_MSG_2(FTM_ERROR , 
          "ftm_hdr_meas_get_tech_param: ERROR :src tech %d and tgt tech %d", 
...
)
{
  ...
  else if (test_param_ptr->src_tech == RFM_1XEVDO_MODE)
  {
    ...
    ftm_hdr_meas_enter_param.band_chan.band = test_param_ptr->src_ca_path0_band ;
    ftm_hdr_meas_enter_param.band_chan.chan_num = test_param_ptr->src_ca_path0_chan_car0;
...
)
{
     ...
     (test_param_ptr->tgt_tech == RFM_1XEVDO_MODE))
  {
    ...
    if(test_param_ptr->src_tech == RFM_1XEVDO_MODE)
    {
      ftm_hdr_meas_setup_param.band_chan.band = test_param_ptr->src_ca_path0_band ;
      ftm_hdr_meas_setup_param.band_chan.chan_num = test_param_ptr->src_ca_path0_chan_car0;
    }
    else
    {
...
)
{
  ...
  {
    /* hdr is source tech */
    ftm_hdr_meas_exit_param.band_chan.band = test_param_ptr->src_ca_path0_band ;
    ftm_hdr_meas_exit_param.band_chan.chan_num = test_param_ptr->src_ca_path0_chan_car0;
    /* cast to void pointer */
