...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/hdr/ftm/src/ftm_hdr_log.c#1 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------- 
03/27/18   rp      KW Fixes.
07/08/15   vc      Passing Tx Device to get hdr Tx data
...
)
{
  ...
  rfm_cdma_ftm_tx_agc_data_type tx_agc_data = {0};
  ...
  tx_device = rfm_cdma_ftm_log_search_transmit_device(RFM_1XEVDO_MODE);
  ...
  {
    rfm_hdr_ftm_get_tx_agc_data( tx_device, RFM_CDMA_CARRIER_0, &tx_agc_data,
  ...
  log_data->hdr_log.c0_agc[buf].hdet_raw_v2 = rfm_cdma_ftm_read_hdet( tx_device );
  log_data->hdr_log.c0_agc[buf].hdet_raw = 0;
  log_data->hdr_log.c0_agc[buf].txagc_from_hdet =
    rfm_cdma_ftm_convert_hdet_to_txagc(log_data->hdr_log.c0_agc[buf].hdet_raw_v2,tx_device);
  log_data->hdr_log.c0_agc[buf].therm_scaled = 0;
  log_data->hdr_log.c0_agc[buf].temp_index = 
    rfm_cdma_ftm_get_therm_index_remainder( tx_device, 
...
)
{
  ...
  log_data->hdr_log.c1_agc[buf].hdet_raw_v2 = rfm_cdma_ftm_read_hdet(tx_device);
  ...
  log_data->hdr_log.c1_agc[buf].therm_raw = 
    rfm_cdma_ftm_get_therm_reading( tx_device );
