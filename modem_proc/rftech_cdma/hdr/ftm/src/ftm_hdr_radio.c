...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/hdr/ftm/src/ftm_hdr_radio.c#1 $
...
*/
typedef ftm_cdma_req_17_type ftm_hdr_radio_wakeup_tx_req_type;
...
)
{
  ...
  /* clean up concurrency manager before retune for current device*/
     ftm_concurrency_manager_cleanup_device_allocation(device, RFM_1XEVDO_MODE);
  ...
  if (dev_status.assoc_dev != RFM_INVALID_DEVICE ) 
  {
     ...
     ftm_concurrency_manager_cleanup_device_allocation(dev_status.assoc_dev, 
                                                       RFM_1XEVDO_MODE);

    /* validate new band for associate device */
    if ( ftm_cdma_control_is_tune_allowed( RFM_1XEVDO_MODE, dev_status.assoc_dev, 
                    band_chan[0].band))
    {
    ...
    else
    {
       ...
       rf_status = RFM_CDMA_ERROR_FAILURE;
       FTM_MSG_3( FTM_ERROR, "ftm_hdr_radio_retune: "
                 "Concurrency manager returned failure for"
                 " tech:%d for associate device:%d on band:%d ", 
                 RFM_1XEVDO_MODE, dev_status.assoc_dev, band_chan[0].band);
  ...
  else
  {
   ...
   /* validate new band for current device */
  if ( ftm_cdma_control_is_tune_allowed( RFM_1XEVDO_MODE, device, 
                   band_chan[0].band) && (validation_flag == TRUE))
  {
    /* Call RFM Function to Wakeup Rx */
    cb_payload.device = device; /* Populate the CB Payload */
...
)
{
  ...
  /* Extract Useful Data */
  device = (rfm_device_enum_type)header->tx_device;
  rx_device = (rfm_device_enum_type)header->prx_device;

    cb_payload.device = device; /* Populate the CB Payload */
      
  rf_status = ftm_hdr_wakeup_tx (device, rx_device, 
                                 &ftm_hdr_rfm_callback, &cb_payload);
