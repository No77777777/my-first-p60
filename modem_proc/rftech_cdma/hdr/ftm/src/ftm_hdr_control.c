...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_cdma.mpss/3.11/hdr/ftm/src/ftm_hdr_control.c#1 $
...
)
{
       ...
       tx_device_status != NULL )
  {
    ...
    if( tx_device_status->txlm_handle_dev_vote == 0 )
    {
      ...
      {
        tx_buffer_allocated = txlm_allocate_buffer(tx_logical_dev->ftm_txlm_chain_hdr,
                                                   LM_HDR,
                                                   &tx_device_status->txlm_handle );
      }
      else
      {
        FTM_MSG_1(FTM_ERROR, "ftm_hdr_vote_txlm_buffer: valid Tx handle "
                             "found with no device voting  "
    ...
    FTM_MSG_4(FTM_MED, "ftm_hdr_vote_txlm_buffer: Tx device %d has txlm_handle "
  ...
  {
    FTM_MSG_1(FTM_ERROR, "ftm_hdr_vote_txlm_buffer: NULL device status pointers for "
...
*/
lm_status_type
ftm_hdr_unvote_txlm_buffer
(
  rfm_device_enum_type tx_device
)
{

  lm_status_type tx_buffer_deallocated = LM_SUCCESS; /* Tx Buffer */
  ...
  if( tx_device != RFM_INVALID_DEVICE && tx_device_status != NULL )
  {
    ...
    FTM_MSG_4(FTM_MED, "ftm_hdr_unvote_txlm_buffer: Tx device %d has txlm_handle "
                        "%d now. Rx device %d, "
                        "Device vote mask updated 0x%x",
                         tx_device, tx_device_status->txlm_handle,tx_device,
                         tx_device_status->txlm_handle_dev_vote );        
    ...
    if ( tx_device != RFM_INVALID_DEVICE)
    {
      ...
      if ( tx_dev_status != NULL )
      {
            ...
            tx_dev_status->rf_state ==  FTM_CDMA_STATE_RXTX )
        {
          ...
          was_tear_down_req = rfm_hdr_ftm_tear_down_radio( tx_device );
    ...
    if ( was_tear_down_req )
    {
      ...
      else
      {
          ...
          if ( assoc_dev_status->rf_state == FTM_CDMA_STATE_RXDIV )
          {
            ...
            ftm_concurrency_manager_cleanup_device_allocation(assoc_dev, RFM_1XEVDO_MODE); 
...
)
{
  ...
  rf_status = ftm_hdr_wakeup_tx (device, device, NULL, NULL);
...
)
{
  ...
  {   
    ...
    rf_status = ftm_hdr_wakeup_tx_v2( device, 
                                      rx_device, 
                                      callback, 
                                      userdata );
...
)
{
  ...
  if (ftm_cdma_data_get_device_status( device, &dev_status ))
  {
    if ( dev_status.rf_state == FTM_CDMA_STATE_RXTX )
    {
      ...
      rf_status = ftm_hdr_sleep_tx_v2( device, 
                                       txlm_handle, 
                                       callback,
                                       userdata );
...
)
{
  ...
  if (hdr_enter_mode_rf_status == RFM_CDMA_HEALTHY_STATE)
  {
    ...
    ftm_cdma_data_get_device_status( master_device, &dev_status_m );
     ...
     /* validate band on diversity*/
    if ( ftm_cdma_control_is_tune_allowed( RFM_1XEVDO_MODE, slave_device, 
    ...
    {
       rf_status = RFM_CDMA_ERROR_FAILURE;
       FTM_MSG_3 ( FTM_ERROR , "ftm_hdr_enable_diversity: "
                   "Concurrency manager returned failure for"
                   " tech:%d for diversity device:%d and band:%d ",
                   RFM_1XEVDO_MODE, slave_device ,
                  dev_status_m.curr_band) ;
...
)
{
  ...
  tx_buffer_allocated = ftm_hdr_vote_txlm_buffer(tx_device);

  tx_device_status = ftm_cdma_data_get_device_ptr(tx_device);
  
  if (tx_buffer_allocated == LM_SUCCESS && tx_device_status != NULL )
  {
    ftm_ret = rfm_hdr_wakeup_tx_v2(tx_device, 
  	                              rx_device, 
		  					      tx_device_status->txlm_handle, 
							      callback, 
							      userdata);
  ...
  {
    FTM_MSG(FTM_ERROR, "ftm_hdr_wakeup_tx_v2: Buffer allocation failed"
...
)
{
  ...
  ftm_ret = rfm_hdr_sleep_tx(device, 
		  	 		          txlm_handle, 
							  callback, 
							  userdata );
  ...
  {
    FTM_MSG(FTM_ERROR, "ftm_hdr_sleep_tx: Buffer deallocation failed");
	ftm_ret = -1;
  }
