...
$Header: //components/rel/rftech_gnss.mpss/3.11/gen8/rf/mc/src/rfm_gnss.c#2 $
$DateTime: 2024/03/11 23:52:17 $
$Author: mplcsds1 $
...
*/
uint32 rfm_gnss_get_device_id(void)
{
  rfdevice_id_enum_type device_id = RFDEVICE_INVALID;

  if ( FALSE == rfc_gnss_get_devices_info_data(&device_id))
  {
     MSG_ERROR("RF GNSS: Not able to get device ID.",0,0,0);
  }
...
boolean rfm_gnss_get_device_id_v2(rfdevice_id_enum_type *device_id_1, rfdevice_id_enum_type *device_id_2)
{
  ...
  if ( FALSE == rfc_gnss_get_devices_info_data_v2(device_id_1, device_id_2))
  {
     MSG_ERROR("RF GNSS: Not able to get device ID.",0,0,0);
	 return FALSE;
  }
