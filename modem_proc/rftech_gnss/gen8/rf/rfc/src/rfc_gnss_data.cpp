...
$DateTime: 2024/03/11 23:52:17 $ $Author: mplcsds1 $
$Header: //components/rel/rftech_gnss.mpss/3.11/gen8/rf/rfc/src/rfc_gnss_data.cpp#2 $
...
boolean rfc_gnss_get_devices_info_data_v2(rfdevice_id_enum_type *device_id_1, rfdevice_id_enum_type *device_id_2)
{
  ...
  if(device_info == NULL)
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "Can't get devices_info_data because device_info is NULL", 0);
    return FALSE;
  }
  ...
  else if (device_info->num_devices == 1)
  {
    *device_id_1 = device_info->rf_asic_info[0].device_id; 
	*device_id_2 = RFDEVICE_INVALID; 
    return TRUE;
  }
  else if (device_info->num_devices == 2)
  {
    *device_id_1 = device_info->rf_asic_info[0].device_id; 
	*device_id_2 = device_info->rf_asic_info[1].device_id;  
    return TRUE;
  }
  else
  {
  	MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR, "There are more than two devices defined in GNSS device info in RFC which is not expected. Please review RFC.", 0);
  	return FALSE;
  }
}
