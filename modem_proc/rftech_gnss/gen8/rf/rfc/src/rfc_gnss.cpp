...
$Header: //components/rel/rftech_gnss.mpss/3.11/gen8/rf/rfc/src/rfc_gnss.cpp#1 $

when       who     what, where, why
-------------------------------------------------------------------------------
10/13/15   Saul    GNSS Blanking
05/12/15   rcui    change set chip to set device
01/13/15   yb/dmc  Added support for Tuner devices for GPS
...
rfc_gnss::rfc_gnss()
{
  ...
  for(dev_type = 0; dev_type < RFDEVICE_TYPE_MAX_NUM; dev_type++)
  {
    ...
    {
      gnss_trx_devices[dev_type][dev_instance] = NULL;
    }
  }
  init_rf_devices();
                            ...
                            rfm_path_state state)
{
  ...
  vreg_param.set_device(RFM_DEVICE_0); 
  ...
  while (i < device_info->num_devices )
  {
    ...
    switch(dev_type)
    {
      case RFDEVICE_TRANSCEIVER:
        gnss_trx_devices[dev_type][trx_dev_instance] = cmn_dev_obj;
        trx_dev_instance++;
        break;

      case RFDEVICE_TUNER:
        gnss_trx_devices[dev_type][tuner_dev_instance] = cmn_dev_obj;
                    /*** Get Tuner manager ***/
        ...
        if (cmn_dev_obj != NULL)
        {
          gnss_trx_devices[RFDEVICE_TUNER_MANAGER][tuner_dev_instance] = cmn_dev_obj;
        }
        tuner_dev_instance++;
        break;
...
*/
void** rfc_gnss::get_rf_device_obj(rfdevice_type_enum_type dev_type)
{
  void** dev_obj = NULL;
  ...
  dev_obj = (void**)gnss_trx_devices[dev_type];
