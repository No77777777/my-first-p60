...
$DateTime: 2021/03/09 05:07:10 $ $Author: pwbldsvc $
$Header: //components/rel/rftech_gnss.mpss/3.11/gen8/rf/rfc/src/rfc_card_gnss.cpp#1 $

when       who     what, where, why
------------------------------------------------------------------------------- 
01/13/15   yb/dmc  Added support for tuner device
...
) 
{
  ...
  return((void**)(rfc_obj->get_rf_device_obj(dev_type)));
  }
...
/*!
   @brief
   GNSS tuner notification

*/
void rfc_gnss_tuner_notify(boolean enable)
{
  void** tuner_mgr_p = NULL;
  rfcommon_atuner_intf *rfc_gnss_tuner_obj = NULL;
  uint8 tuner_mgr_index                   = 0;

  tuner_mgr_p = rfc_gnss_get_dev_obj(RFDEVICE_TUNER_MANAGER);
  ...
  if( ( tuner_mgr_p != NULL ) && ( tuner_mgr_p[0] != NULL ) )
  {
    ...
    while ( tuner_mgr_p[tuner_mgr_index] != NULL )
    {
      if(enable == TRUE)
      {    
        if(((rfcommon_atuner_manager_intf*)(tuner_mgr_p[tuner_mgr_index]))->enable_gps() == FALSE)
        {
          MSG_ERROR("Failed to notify tuner manager 0x%x that GPS is enabled!", tuner_mgr_p[tuner_mgr_index], 0, 0);
        }
      }
      else
      {   
        if(((rfcommon_atuner_manager_intf*)(tuner_mgr_p[tuner_mgr_index]))->disable_gps() == FALSE)
        {
          MSG_ERROR("Failed to notify tuner manager 0x%x that GPS is disabled!", tuner_mgr_p[tuner_mgr_index] , 0, 0);
        }
      }
      tuner_mgr_index++;
    }
