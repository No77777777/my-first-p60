...
$Header: //components/rel/rftech_gnss.mpss/3.11/gen8/rf/core/src/rfgnss_core.c#1 $
$DateTime: 2021/03/09 05:07:10 $
$Author: pwbldsvc $
...
05/06/15   zg      Added support for two WTRs power voting
02/13/15   dps     Fixed KW errors
01/13/15   yb/dmc  Added notification for tuners
...
boolean rfgnss_core_device_seq_execute(rfm_gnss_device_seq_cmd_type cmd, void *data)
{
  ...
  switch (cmd)
  {
    ...
    case RFGNSS_DEVICE_CMD_L5_ML5_ELNA_INIT:
      rfc_gnss_tuner_notify(TRUE);
      rfgnss_ds->cmd_dispatch_fp(RFDEVICE_GNSS_CMD_L5_ML5_ELNA_INIT, data);
      ret_value = TRUE;
    break; 
