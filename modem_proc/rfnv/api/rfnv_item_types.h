...
Copyright (c) 2012 - 2024 by Qualcomm Technologies, Inc.  All Rights Reserved.
===========================================================================*/
...
$Header: //components/rel/rfnv.mpss/1.0/api/rfnv_item_types.h#290 $


when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/27/24   chm     Add new NV item type for rfm delay
...
#define RFNV_TX_MAX_PWR_ADJ_BASED_ON_REQ_ENTRIES  16
...
#define RFNV_MAX_NF_THRESHOLDS_TYPE 4
...
#define RFNV_ANT_ISO_MAX_RX_ANT 4
...
enum{
  ...
  RFNV_DATA_TYPE_NF_THRESHOLDS_TYPE                             = 621,
  RFNV_DATA_TYPE_TX_MAX_POWER_ADJ_BASED_ON_REQ                  = 622, /* rfnv_tx_max_power_adj_based_on_req_static_data_t */
  RFNV_DATA_TYPE_TX_SAR_POWER_LIMIT_CONTROL_V12                 = 623, /* rfnv_tx_sar_power_limit_control_v12_static_data_t */
  RFNV_DATA_TYPE_PEMAX_ADJUST_V2                                = 624, /* rfnv_pemax_adj_v2_static_data_t */
  RFNV_DATA_TYPE_FBRX_GAIN_V6                                   = 625, /* rfcommon_fbrx_ref_freq_gain_nv_v6_type */
  RFNV_DATA_TYPE_TX_ANT_RX_ANT_ISO_V1                           = 626, /* rfnv_data_type_tx_ant_rx_ant_iso_data_t_v1 */
  RFNV_DATA_TYPE_RX_PATH_SAW_TX_REJECTION_V1                    = 627, /* rfnv_data_type_rx_path_saw_tx_rejection_data_t_v1 */
  RFNV_DATA_TYPE_RX_FE_DATA_V1                                  = 628, /* rfnv_data_type_rx_fe_data_t_v1 */
  RFNV_DATA_TYPE_FBRX_DROOP_FIR_FILTER_COEFF_V4                 = 629, /* rfcommon_fbrx_nv_gs_droop_params_type_v2 */
  RFNV_DATA_TYPE_TX_SAPT_VSWR_DATA                              = 630, /* rfnv_tx_sapt_vswr_load_info_t */
  RFNV_DATA_ATUNER_ACTIVE_ASDIV_CORRECTION_TBL                  = 631,
  RFNV_DATA_ATUNER_ACTIVE_DETUNE_ASDIV_CORRECTION_TBL           = 632,
  RFNV_DATA_TYPE_RFM_INIT_DELAY                                 = 633, /* rfnv_data_type_rfm_init_delay_type*/
  RFNV_DATA_TYPE_ATUNER_PROXIMITY_DETECTION_ANT_GR_DELIMITER    = 634,
...
typedef PACK(struct)
{
  ...
  /* ca_active Bitmask for intra-tech combo check
     0x0 – skip intra-tech combo check
     bit0: 0x1 – No combo
     bit1: 0x2 – ULCA mode
     bit2: 0x4 – NR ++ NR DSDA mode */
  uint8 ca_active;
  ...
  /* -------------------------------------------ENDC------------------------------------------- */
  /* active_mask ENDC active status bitmask
     0x0 – skip inter-tech combo checking
     Bit0: 0x1 – no combo
     Bit1: 0x2 – ENDC mode
     Bit2: 0x4 – NR ++ LTE DSDA*/
  uint8 endc_active;
  ...
  /* mask of mimo chain(logical path), bit0 for PATH_TYPE_TX0, bit1 for PATH_TYPE_TX1 ...
     ...
     0x3 for MIMO case, PATH_TYPE_TX0 + PATH_TYPE_TX1
     ...
     0xff means any chain mask(MIMO or non-MIMO) is suitable */
  uint8 mimo_chain_mask;

  /* current mimo chain(logical path), 0 for PATH_TYPE_TX0, 1 for PATH_TYPE_TX1 ...
     0xFF means any chain is suitable.
     Ignore it if mimo_chain_mask = 0 (or 1)*/
  uint8 mimo_chain;
...
/* RFNV_DATA_TYPE_TX_SAR_POWER_LIMIT_CONTROL_V12 RFNV variant to be used with Tx Static NV */
typedef PACK(struct)
{
  /*------------------------Guidance------------------------*/
  /*
    1. You can not specify more than 1 tech_tech_active_mode_mask with non-0 value or with non-1 value in one NV entry.
       saying when lte_sub6_active_mode_mask = non-0 or non-1 value, sub6_sub6_active_mode_mask and sub6_mmw_active_mode_mask must be 0(skip checking) or 1(not for sub6_sub6 sub6_mmw combo)
  */
  /*--------------------------------------------------------*/

  /* sub_id of current NV entry
     1 means sub1,
     2 means sub2,
     0x0 and 0xFF mean both subs are applicable */
  uint8 sub_id;

  /* cc_id of current NV entry
     0 means current band is cc0 or PCC,
     1 means current band is cc1 or SCC,
     0xFF means suitable for all CCs*/
  uint8 cc_id;

  /* -------------------------------------------Specify SUB6+LTE combo------------------------------------------- */
  /* sub6+lte combo bitmask to specify any sub6+lte combo scenario(endc/dsda etc) 
          0x0 – skip SUB6+LTE(LTE+SUB6) combo checking
    bit0: 0x1 – Not applicable to sub6+lte combo
    bit1: 0x2 – ENDC combo
    bit2: 0x4 – NR ++ LTE DSDA combo
  */
  uint32 lte_sub6_active_mode_mask;

  /* band number for the other tech in ENDC, 0xFFFF means any band is suitable
     Ignore it if current mode is sub6+lte combo */
  uint32 lte_band_number;

  /* sub band number for the other tech in ENDC, 0xFF means any subband band is suitable
     Ignore it if current mode is sub6+lte combo */
  uint8 lte_sub_band_number;

  /* ant switch path for the other tech in ENDC, 0xFFFF means any ant switch path is suitable
     Ignore it if current mode is sub6+lte combo */
  uint16 lte_ant_switch_path;

  /* -------------------------------------------Specify SUB6+SUB6 combo------------------------------------------- */
  /* sub6+sub6 combo bitmask to specify any intra sub6 combo scenario(DSDA/ULCA/SRSCS/Switched UL/Dual UL/SUL, etc) 
          0x0 – skip intra SUB6 combo check
    bit0: 0x1 – Not applicable to intra sub6 combo
    bit1: 0x2 – ULCA combo
    bit2: 0x4 – NR ++ NR DSDA combo
    bit3: 0x8 – SRS carrier switching combo
    bit4: 0x10 – Switched UL combo
    bit5: 0x20 – Dual UL combo
    bit6: 0x40 – Supplemented UL combo
  */
  uint32 sub6_sub6_active_mode_mask;

  /* band number of the other band/CC/TECH(not FR2) in a given combo(ULMIMO/ULCA/ENDC/DSDA/SUL,etc)
     Fill in 0xFFFF if not specify band number for a given active_mode_mask */
  uint32 sub6_band_number;

  /* sub band number of the other band/CC/TECH(not FR2) in a given combo(ULMIMO/ULCA/ENDC/DSDA/SUL,etc)
     Fill in 0xFF if not specify sub band number for a given active_mode_mask */
  uint8 sub6_sub_band_number;

  /* signal path of the other band/CC/TECH(not FR2) in a given combo(ULMIMO/ULCA/ENDC/DSDA/SUL,etc)
     Fill in 0xFFFF if not specify signal path for a given active_mode_mask */
  uint16 sub6_sig_path;

  /* ant switch path of the other band/CC/TECH(not FR2) in a given combo(ULMIMO/ULCA/ENDC/DSDA/SUL,etc)
     Fill in 0xFFFF if not specify ant switch path for a given active_mode_mask */
  uint16 sub6_ant_switch_path;

  /* -------------------------------------------Specify SUB6+MMW combo------------------------------------------- */
  /* sub6+mmw combo bitmask to specify any sub6+mmw combo scenario(NRDC/FR1+FR2 ULCA, etc) 
          0x0 – skip SUB6+MMW combo checking
    bit0: 0x1 – Not applicable to sub6+mmw combo
    bit1: 0x2 – NRDC combo
    bit2: 0x4 – FR1+FR2 CA combo
    bit3: 0x8 – FR1+FR1 CA ++FR2 DC combo
  */

  uint32 sub6_mmw_active_mode_mask;

  /* FR2 band number, can be used in FR1+FR2 CA/DC sceanrios 
     Fill in 0xFFFF if not specify FR2 band number for a given active_mode_mask */
  uint32 mmw_band_number;
  ...
  /* mimo_chain of current NV entry to to differentiate TX0 and TX1 in ULMIMO.
     Set 0x0 to apply to only logical TX0 
     Set 0x1 to apply to only logical TX1
     Set 0xFF if not specify chain id
     Ignore it if mimo_chain_mask = 0 (or 1)*/
  uint8 mimo_chain;

  /* -------------------------------------------Power Class------------------------------------------- */
  /*! Power class of current path.
      Implemented as a bitmask.
        Bit0 - Power Class 1
        Bit1 - Power Class 2
        Bit2 - Power Class 3
        Bit3 - Power Class 4
        Bit4 - Power Class 1dot5
        Bit5 - Power Class 5
        ...
      If all Bits are enabled this backoff will be applied to all supported
      UE Power classes. */
...
} rfnv_tx_sar_power_limit_control_v12_static_data_t;
...
#define RFNV_PEMAX_DYNAMIC_ADJ_ENTRIES     21

/* RFNV_DATA_TYPE_PEMAX_ADJUST_V2 RFNV variant typeid xxx */
typedef PACK(struct)
{
  /*! NV container that this data item applies to. */
  uint8 nv_container_index;
  ...
  /*! Pemax adjust value in dB10 for foldable phone*/
  /*! 
      0: non-SRS pemax adj
      1: SRS pemax adj
  */
  int8 pemax_dynamic_adj_db10[RFNV_PEMAX_DYNAMIC_ADJ_ENTRIES];
} rfnv_pemax_adj_v2_static_data_t;
...
{
  uint8 number_of_valid_entries;
  nf_threshold_type thresholds;
} rfnv_nf_thresholds_type;
...
/* RFNV_DATA_TYPE_TX_SAPT_VSWR_DATA 630 */
typedef PACK(struct)
{
  /* Freq bin information */
 rfnv_data_freq_info_type freq_info;
 int16 vswr_mag[RFNV_CHAN_INFO_CHANNEL_LIST_SIZE];
 int16 vswr_phase[RFNV_CHAN_INFO_CHANNEL_LIST_SIZE];
}rfnv_tx_sapt_vswr_load_info_t;
...
/* RFNV_DATA_TYPE_TX_MAX_POWER_ADJ_BASED_ON_REQ
   RFNV variant typeid 622 */
typedef PACK(struct)
{
  /* NV Container Index for each Tx path for a given LTE band */
  uint8 nv_container_index;
  /* Reserved for now, not used. */
  uint8 reserved;
  /* Maximum power limit adjust */
  int16 pwr_adj_dbm10[RFNV_TX_MAX_PWR_ADJ_BASED_ON_REQ_ENTRIES];
} rfnv_tx_max_pwr_adj_based_on_req_static_data_type;
...
/*! RFNV_DATA_TYPE_TX_ANT_RX_ANT_ISO_V1 */
typedef PACK(struct)
{
   uint8 antenna_switch_path;
   uint16 low_band_iso_db10;
   uint16 mid_band_iso_db10;
   uint16 high_band_iso_db10;
   uint16 una_iso_db10;
   uint16 c_band_iso_db10;
} rfnv_data_type_rx_ant_iso_t;

typedef PACK(struct)
{
   rfnv_data_type_rx_ant_iso_t rx_ant_iso[RFNV_ANT_ISO_MAX_RX_ANT];
} rfnv_data_type_tx_ant_rx_ant_iso_data_t_v1;

/*! RFNV_DATA_TYPE_RX_PATH_SAW_TX_REJECTION_V1 */
typedef PACK(struct)
{
   uint16 low_band_rej_db10;
   uint16 mid_band_rej_db10;
   uint16 high_band_rej_db10;
   uint16 una_rej_db10;
   uint16 c_band_rej_db10;
} rfnv_data_type_rx_path_saw_tx_rejection_data_t_v1;

/*! RFNV_DATA_TYPE_RX_FE_DATA_V1 */
typedef PACK(struct)
{
   uint16 gain_g0_db10;
   uint16 nf_g0_db10;
} rfnv_data_type_rx_fe_data_t_v1;

/*! RFNV_DATA_TYPE_RFM_INIT_DELAY rfnv_data_type_rfm_init_delay_type */
typedef PACK(struct)
{
   /* delay before rfm_init called, controlled by static nv */
   uint64 delay_us;
}rfnv_data_type_rfm_init_delay_type;
