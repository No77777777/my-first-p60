...
$Header: //components/rel/rflm.mpss/2.11/autopin/inc/rflm_autopin_log.h#1 $
...
typedef struct
{
  ...
  uint32 seq_nr;  // 32bits
  int16 pin_init; // 16bits
  int16 pout_delta; // 16bits
  int32 amam_err; //32bits
  int16 reflog_max_idx;//16bits
  int16 ss_gain_db; //16bits
  int8 gain_comp_exp; // 8bits
  int8 one_shot_pin; // 8bits
  #if (RFLM_FEATURE_BUILD_MODE==RFLM_FEATURE_BUILD_NORMAL)
  int16 kernal_weights_real[FW_PSAMP_VOLTERRA_MAX_KERNELS]; // 16bits * 5
  int16 kernal_weights_imag[FW_PSAMP_VOLTERRA_MAX_KERNELS]; // 16bits * 5
  #else
  int16 kernal_weights_real[5]; // 16bits * 5
  int16 kernal_weights_imag[5]; // 16bits * 5	  
  #endif
  rflm_autopin_algo_type proc_type; // 8bits
  boolean status; // 8bits reserved for alignment
} rflm_autopin_dm_info_log_packet_v4;
...
typedef struct
{
  ...
  union rflm_autopin_dm_info_log_packet_versions
  {
    ...
    rflm_autopin_dm_info_log_packet_v4 v4;
  } versions;
...
typedef struct
{
  ...
  uint8 status;
  int8 reserved;
} rflm_autopin_iq_capture_log_packet_v4;
...
typedef struct
{
  ...
  union rflm_autopin_iq_capture_log_packet_versions
  {
    ...
    rflm_autopin_iq_capture_log_packet_v4 v4;
  } version;
