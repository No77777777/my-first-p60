...
$Header: //components/rel/rflm.mpss/2.11/autopin/src/rflm_autopin_log.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/03/16   zhh     Update the rflm_autopin_iq_capture_log_packet to V4
04/27/16   qzh     Update RFSW-FW autopin update result interface
04/27/16   qzh     Update log packet after FW release
03/17/16   zhh     Version 4 of 0x18FB log packets
01/11/16   whz     Version 3 log packets
11/06/15   whz     Log rotator
                                  ...
                                  rflm_autopin_proc_results_t *results)
{
  ...
  rflm_autopin_dm_info_log_packet_v4 *log_body = NULL;
  ...
  rval = rflm_diag_log_alloc(sizeof(rflm_autopin_dm_info_log_packet_v4),
  ...
  if (rval == RFLM_LOG_NEW_PACKET && p_buffer_user != NULL)
  {
    ...
    log_header->log_version = RFLM_AUTOPIN_LOG_VER_V4;
    log_header->num_sub_packets =
      (RFLM_LOG_MAX_BUF_SIZE_WORDS*4-4)/sizeof(rflm_autopin_dm_info_log_packet_v4);
  ...
  {
    log_body = (rflm_autopin_dm_info_log_packet_v4 *)p_buffer_user;
    ...
    log_body->run_decision = autopin_template->autopin_ctl.run_decision;
    ...
    log_body->pout_delta = autopin_template->autopin_ctl.pout_delta;
    log_body->amam_err = results->meas_error;
    log_body->reflog_max_idx = results->reflog_max_idx;
    log_body->ss_gain_db = results->ss_gain_db;
    log_body->gain_comp_exp = autopin_template->autopin_ctl.gain_comp_exp;
    log_body->one_shot_pin = autopin_template->autopin_ctl.one_shot_pin;
    log_body->proc_type = autopin_template->autopin_ctl.proc_type;
    memscpy(log_body->kernal_weights_imag, FW_PSAMP_VOLTERRA_MAX_KERNELS*sizeof(int16),
            results->kernal_weights_imag, FW_PSAMP_VOLTERRA_MAX_KERNELS*sizeof(int16));
    memscpy(log_body->kernal_weights_real, FW_PSAMP_VOLTERRA_MAX_KERNELS*sizeof(int16),
            results->kernal_weights_real, FW_PSAMP_VOLTERRA_MAX_KERNELS*sizeof(int16));

    log_body->status = results->status;
  }
...
void rflm_autopin_logging_iq_capture(fw_psamp_autopin_result_s *fw_results, uint32 seq_nr)
{
  ...
  if ((rval == RFLM_LOG_NEW_PACKET || rval == RFLM_SUCCESS) && (p_buffer_user != NULL))
  {
    ...
    //if (fw_results->status != 0 || fw_results->corr_snr < FWPSAMP_CORR_SNR_THRESHOLD)
    {
      ...
      memscpy(autopin_template->autopin_ctl.txiqbuf.iq_cap_buf,
              RFLM_AUTOPIN_LOG_TX_SAMPLE_SIZE * sizeof(uint32),
              fw_results->tx_capture,
              autopin_template->autopin_ctl.txiqbuf.num_samp * sizeof(uint32));

      memscpy(autopin_template->autopin_ctl.rxiqbuf.iq_cap_buf,
              RFLM_AUTOPIN_LOG_RX_SAMPLE_SIZE * sizeof(uint32),
              fw_results->rx_capture,
    ...
    log_body->rotator = fw_results->tx_freq;
    log_body->seq_nr = seq_nr;
    log_body->reserved = 0;
  }
...
void rflm_autopin_logging_iq_capture_volterra(fw_psamp_volterra_job_s *fw_results, uint32 seq_nr)
{
  rflm_diag_log_return_code_t rval = RFLM_SUCCESS;
  rflm_autopin_iq_capture_log_packet_t *log_header = NULL;
  rflm_autopin_iq_capture_log_packet_v4 *log_body = NULL;
  uint32 *p_buffer_user = NULL;
  rflm_autopin_dm_template_t *autopin_template = NULL;
  ...
  autopin_template = (rflm_autopin_dm_template_t *)rflm_dm_get_autopin_buf_ptr((rflm_handle_tx_t)(fw_results->param.handle));

  if (autopin_template == NULL)
  {
    RFLM_DIAG_MSG_ERROR("rflm_autopin_logging_iq_capture: No valid AutoPin DM buffer ptr with Tx handle %d", fw_results->param.handle);
    return;
  }
  ...
  rval = rflm_diag_log_alloc(sizeof(rflm_autopin_iq_capture_log_packet_v4),
                             autopin_template->autopin_ctl.logging_iq_capture_context_id,
                             &p_buffer_user);
  ...
  if ((rval == RFLM_LOG_NEW_PACKET) && (p_buffer_user != NULL))
  {
    ...
    log_header->log_version = RFLM_AUTOPIN_LOG_VER_V4;
    log_header->num_sub_packets =
      (RFLM_DIAG_LOG_MAX_PACKET_SIZE-4)/sizeof(rflm_autopin_iq_capture_log_packet_v4);
    log_header->reserved = 0;
    p_buffer_user++;
  }
  ...
  if ((rval == RFLM_LOG_NEW_PACKET || rval == RFLM_SUCCESS) && (p_buffer_user != NULL))
  {
    log_body = (rflm_autopin_iq_capture_log_packet_v4 *)p_buffer_user;
    ...
    //if (fw_results->status != 0 || fw_results->corr_snr < FWPSAMP_CORR_SNR_THRESHOLD)
    {
      ...
      memscpy(autopin_template->autopin_ctl.txiqbuf.iq_cap_buf,
              RFLM_AUTOPIN_LOG_TX_SAMPLE_SIZE * sizeof(uint32),
              fw_results->param.tx_capture,
              fw_results->param.tx_len* sizeof(uint32));

      memscpy(autopin_template->autopin_ctl.rxiqbuf.iq_cap_buf,
              RFLM_AUTOPIN_LOG_RX_SAMPLE_SIZE * sizeof(uint32),
              fw_results->param.rx_capture,
              fw_results->param.rx_len* sizeof(uint32));
    ...
    log_body->rx_len = fw_results->param.rx_len;
    log_body->tx_len = fw_results->param.tx_len;
    log_body->handle = fw_results->param.handle;
    log_body->tx_dc_i = fw_results->result.tx_dc_i;
    log_body->tx_dc_q = fw_results->result.tx_dc_q;
    log_body->tx_rms = fw_results->result.tx_rms;
    log_body->rx_dc_i = fw_results->result.rx_dc_i;
    log_body->rx_dc_q = fw_results->result.rx_dc_q;
    log_body->rx_rms = fw_results->result.rx_rms;
    log_body->peak_offset = fw_results->result.peak_offset;
    log_body->clip_tx_rms = fw_results->result.compress_tx;//Record the compress value
    log_body->clip_tx_mag = fw_results->result.compress_rx;//Record the compress value
    log_body->amam_err = fw_results->result.apin_err;
    log_body->corr_snr = fw_results->result.corr_snr;
    log_body->status = fw_results->result.status;
    log_body->rotator = 0;  //Not used currently, set to zero.
    log_body->seq_nr = seq_nr;
    log_body->reserved = 0;
  }
