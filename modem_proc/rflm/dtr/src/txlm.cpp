...
$Header: //components/rel/rflm.mpss/2.11/dtr/src/txlm.cpp#1 $
...
txlm::txlm()
{
  ...
  #if (RFLM_FEATURE_BUILD_MODE!=RFLM_FEATURE_BUILD_FW_LIB)
  rflm_dtr_tx_shared_reg_lock_init();
...
lm_status_type txlm::acquire_lock(lm_handle_type txlm_handle, rflm_rw_lock_t *rflm_dtr_handle_lock)
{
  ...
  RFLM_DTR_LOCK_LOG(txlm_handle, RFLM_DTR_LOCK, rflm_time_read_xo_time(),1,RFLM_WRITE_LOCK);
...
lm_status_type txlm::release_lock(lm_handle_type txlm_handle, rflm_rw_lock_t *rflm_dtr_handle_lock)
{
  ...
  RFLM_DTR_LOCK_LOG(txlm_handle, RFLM_DTR_UNLOCK, rflm_time_read_xo_time(),1,RFLM_WRITE_LOCK);
