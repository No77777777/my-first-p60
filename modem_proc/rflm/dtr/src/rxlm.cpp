...
$Header: //components/rel/rflm.mpss/2.11/dtr/src/rxlm.cpp#1 $
...
#ifdef FEEROC_MODEM_FEATURE
  if(modem_version == 0x200f0000)
    rxlm_update_dvga_params();
#endif
  rflm_dtr_rx_common_notch_init();
#endif /* FEATURE_TRITON_MODEM */

#ifdef FEATURE_TABASCO_MODEM
  rflm_dtr_rx_mutex_init();
#endif
...
lm_status_type rxlm::acquire_lock(lm_handle_type rxlm_handle, rflm_rw_lock_t* rflm_dtr_handle_lock)
{
  ...
  RFLM_DTR_LOCK_LOG(rxlm_handle, RFLM_DTR_LOCK, rflm_time_read_xo_time(),1,RFLM_WRITE_LOCK);
...
lm_status_type rxlm::release_lock(lm_handle_type rxlm_handle, rflm_rw_lock_t* rflm_dtr_handle_lock)
{
  ...
  RFLM_DTR_LOCK_LOG(rxlm_handle, RFLM_DTR_UNLOCK, rflm_time_read_xo_time(),1,RFLM_WRITE_LOCK);
