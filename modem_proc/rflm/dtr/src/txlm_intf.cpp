...
$Header: //components/rel/rflm.mpss/2.11/dtr/src/txlm_intf.cpp#1 $

when       who     what, where, why
-------------------------------------------------------------------------------
04/06/20   pvsr    Use FEATURE_HABANERO_MODEM in parallel to JACALA_MODEM_FEATURE flag
10/21/15   vkm      Fix compiler warnings
05/18/15   ag     Implementation of new DTR lock mechanism
...
{
  if(chain ==TXLM_CHAIN_1){
  RFLM_DIAG_ERR_FATAL("Invalid Tx chain index: %d", chain);}
...
lm_status_type txlm_deallocate_buffer(lm_handle_type txlm_handle)
{
  ...
  if(txlm_handle >= TXLM_MAX_BUFFERS)
  { 	
    DTR_DIAG_TRACE_MSG_ERROR("txlm_deallocate_buffer: Invalid txlm handle %d",txlm_handle);
    txlm_status = LM_INVALID_BUFFER_INDEX;
  }
  ...
  if (txlm_status == LM_SUCCESS)
  {
   ...
   } /*end if(txlm_handle >= TXLM_MAX_BUFFERS)*/
                                          ...
                                          lm_handle_type rflm_tx_handle)
{
  ...
  if(rflm_tx_handle >= TXLM_MAX_BUFFERS)
  {	 
    DTR_DIAG_TRACE_MSG_ERROR("rflm_deallocate_tx_handle: Invalid txlm handle %d",rflm_tx_handle);
    txlm_status = LM_INVALID_BUFFER_INDEX;
  }
  ...
  if (txlm_status == LM_SUCCESS)
  {
   ...
   }/*end if(rflm_tx_handle >= TXLM_MAX_BUFFERS)*/
  if (txlm::txlm_verbose_level > DTR_VERBOSE_LEVEL_0) 
  {
...
lm_status_type txlm_get_buffer_content(lm_handle_type txlm_handle, uint32 group_id, uint8 *addr)
{
  ...
  condition */
	...
	RFLM_DTR_LOCK_LOG(txlm_handle, RFLM_DTR_LOCK, rflm_time_read_xo_time(),0,RFLM_READ_LOCK);
  ...
  txlm_status = txlm_obj->get_buffer_content(txlm_handle, group_id, addr);
    ...
    RFLM_DTR_LOCK_LOG(txlm_handle, RFLM_DTR_UNLOCK, rflm_time_read_xo_time(),0,RFLM_READ_LOCK);
...

{
  ...
  /* the get_instance() should never return NULL as it returns the ptr to txlm
   object with new operator. So, there is no need to check for txlm_obj == NULL
   condition */
    RFLM_DTR_LOCK_LOG(handle, RFLM_DTR_LOCK, rflm_time_read_xo_time(),0,RFLM_READ_LOCK);
  ...
  // Unlcok the handle lock.
    ...
    RFLM_DTR_LOCK_LOG(handle, RFLM_DTR_UNLOCK, rflm_time_read_xo_time(),0,RFLM_READ_LOCK);
...
lm_status_type txlm_acquire_lock(lm_handle_type txlm_handle)
{
  ...
  condition */
    ...
    {	 
      DTR_DIAG_TRACE_MSG_ERROR("txlm_acquire_lock: Invalid txlm handle %d",txlm_handle);
      txlm_status = LM_INVALID_BUFFER_INDEX;
    }
...
lm_status_type txlm_release_lock(lm_handle_type txlm_handle)
{
  ...
  condition */
    ...
    txlm_status = txlm_obj->release_lock(txlm_handle, &rflm_dtr_handle_locks[txlm_handle]);
     ...
     {	  
       DTR_DIAG_TRACE_MSG_ERROR("txlm_release_lock: Invalid txlm handle %d",txlm_handle);
       txlm_status = LM_INVALID_BUFFER_INDEX;
     }
...
{
   txlm *txlm_obj = txlm::get_instance();
 #if defined(JACALA_MODEM_FEATURE) || defined(FEATURE_HABANERO_MODEM)
   return 0;//as iref_lut has only 1 table for SMIC
 #else
