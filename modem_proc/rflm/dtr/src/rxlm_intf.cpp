...
$Header: //components/rel/rflm.mpss/2.11/dtr/src/rxlm_intf.cpp#1 $
...
lm_status_type rxlm_deallocate_buffer(lm_handle_type rxlm_handle)
{
 ...
 if(rxlm_handle >= RXLM_MAX_BUFFERS)
 {  	  
   DTR_DIAG_TRACE_MSG_ERROR("rxlm_deallocate_buffer: Invalid rxlm handle %d",rxlm_handle);
   rxlm_status = LM_INVALID_BUFFER_INDEX;
 }
...
// Destroy the handle mutex
  ...
  }/*end if(rxlm_handle >= TXLM_MAX_BUFFERS)*/
                                         ...
                                         lm_handle_type rflm_rx_handle)
{
 ...
 if(rflm_rx_handle >= RXLM_MAX_BUFFERS)
 {  	  
   DTR_DIAG_TRACE_MSG_ERROR("rxlm_deallocate_rx_handle: Invalid rxlm handle %d",rflm_rx_handle);
   rxlm_status = LM_INVALID_BUFFER_INDEX;
 }
...
// Destroy the handle mutex
 ...
 } /*end if(rflm_rx_handle >= TXLM_MAX_BUFFERS)*/
...
lm_status_type rxlm_get_buffer_content(lm_handle_type rxlm_handle, uint32 group_id, uint8 *addr)
{
  ...
  condition */
    ...
    RFLM_DTR_LOCK_LOG(rxlm_handle, RFLM_DTR_LOCK, rflm_time_read_xo_time(),0,RFLM_READ_LOCK);
  ...
  // Unlcok the handle lock.
    ...
    RFLM_DTR_LOCK_LOG(rxlm_handle, RFLM_DTR_UNLOCK, rflm_time_read_xo_time(),0,RFLM_READ_LOCK);
...
lm_status_type rxlm_acquire_lock(lm_handle_type rxlm_handle)
{
  ...
  condition */
   ...
   rxlm_status = rxlm_obj->acquire_lock(rxlm_handle, &rflm_dtr_handle_locks[rxlm_handle]);   
         ...
         {	  
	   DTR_DIAG_TRACE_MSG_ERROR("rxlm_acquire_lock: Invalid rxlm handle %d",rxlm_handle);
	   rxlm_status = LM_INVALID_BUFFER_INDEX;
         }
