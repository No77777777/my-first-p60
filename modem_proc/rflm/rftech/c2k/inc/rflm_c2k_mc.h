...
$Header: //components/rel/rflm.mpss/2.11/rftech/c2k/inc/rflm_c2k_mc.h#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
04/27/20   aa      Check for CCS status of previous CCS callback.
03/02/20   mk      Check for CCS callback execution for RF ON-OFF event
01/21/19   aa      RxAGC based QXDM Command implementation
01/10/18   mk      Changed macro definition for RFLM_C2K_DEBUG_ON
                    disabled case to fix CW.
...
01/22/16   rs      Added API to update FBRx enable flag
12/24/15   vr      Autopin check-in_2
12/02/15   vr      Added Auto-Pin changes
11/20/15   vr      Autopin check-in
10/29/15   pk      Synchronized Therm Read with TxAGC event
06/15/15   vc      Flag to check hdet progress and skip txagc
04/15/15   pk      Alligning the structure rflm_c2k_lna_fsm_type to byte format
03/19/15   spa     Added flags for rf off requested and rf off action time 
...
/* CDMA RFLM Debug message enablement flag. Will be OFF in CRM builds */
//#define RFLM_C2K_DEBUG_ON 

#ifdef RFLM_C2K_DEBUG_ON
#define RFLM_C2K_QXDM_MSG_LOW   RFLM_DIAG_MSG_LOW
#define RFLM_C2K_QXDM_MSG_MED   RFLM_DIAG_MSG_MED
#define RFLM_C2K_QXDM_MSG_HIGH  RFLM_DIAG_MSG_HIGH
#define RFLM_C2K_QXDM_MSG_ERROR RFLM_DIAG_MSG_ERROR
#else
#define RFLM_C2K_QXDM_MSG_LOW(...)     (void)0
#define RFLM_C2K_QXDM_MSG_MED(...)     (void)0
#define RFLM_C2K_QXDM_MSG_HIGH(...)    (void)0
#define RFLM_C2K_QXDM_MSG_ERROR(...)   (void)0
#endif

/*----------------------------------------------------------------------------*/
/*! 
  @brief
  Define maximum number of LNA FSMs supported, 2 for HDR, 2 for 1x */
#define RFLM_C2K_MAX_LNA_FSM_SUPPORTED                                          4 
...
/*----------------------------------------------------------------------------*/
/*Size of the TxAGC Timing log buffer
*/
#define RFLM_TXAGC_UPDATE_BUFF_SIZE                                           50
/*----------------------------------------------------------------------------*/
/*! 
  @brief
...
typedef struct
{
  ...
  boolean enable_discontinous_rx ;

  /* TRUE: Enable | FALSE: Disable */
  boolean enable_dual_vswr_captures ;
...
/*----------------------------------------------------------------------------*/
/*! @brief 
  C2K structure for containing information pertaining to the TxAGC
           */
typedef struct 
{
  ...
  struct rflm_c2k_txagc_state_s
  {
	...
	/* If we are going to be in Constant Power or Not */
    boolean is_const_power ;

    /* Command from FW to skip TxAGC activities while FW is busy with 
       activities like ILPC or VSWR capture or processing etc */
    boolean ignore_txagc_updates ;

    /* PA additional offset in us to take care of  additional RFFE writes 
       in case of customer design*/
    int32 pa_addtl_offset ;	
  ...
  /* Contains Feedback-tuner-loops related information for C2K */
  struct rflm_c2k_tx_tuner_fb_info_t 
  {
    /*! @brief Keeps the info is CL event is pending 
        @note  Private Variable. Dont expose.
      */
    boolean is_cl_event_pending ;


    /*! @brief Keeps the info is AOL event is pending 
        @note  Private Variable. Dont expose.
      */
    boolean is_aol_event_pending ;
...
/*----------------------------------------------------------------------------*/
/*! @brief 
    RFLM TQP bookkeeping information */
typedef struct 
{
  /*! @brief 
    TRUE: TQP in use
    FALSE: TQP not in use */
  boolean if_tq_used ;

  /*! @brief 
    Primary Device holding on to the TQP
    rfm_dev == RFLM_CDMA_MAX_DEVICES when if_tq_used == FALSE */
  uint8   rfm_dev ;  
} rflm_c2k_tq_info  ;
...
typedef struct
{
  ...
  /*! Static Mem block ID for Txagc seq */
  uint8   txagc_static_mem_blk_indx;
  
  /*! Static Mem block ID for Txagc seq UPDATE*/  
  uint8   txagc_resend_mem_blk_index[2];
  
  /*! Static Mem block ID for static event */
  uint8   coupler_fwd_event_static_mem_blk_indx;
  
  /*! Static Mem block ID for static event */
  uint8   coupler_rev_event_static_mem_blk_indx;
...
/*
@brief
 Data structure holding the RxAGC debug command parameters.
*/
typedef struct
{
  boolean agc_delta_flag;
  uint8 chain_type; 
  uint8 rxagc_debug_limit;
  uint8 cnt;
  uint8 rx_update_count;
 }rflm_c2k_rxagc_debug_crash_s;
...
/*---------------------------------------------------------------------------*/
/*
Structure to log Timing related info for TxAGC Update
*/
typedef struct
{
  /*Time sent by firmware*/
  uint32 fw_action_time;

  /* Current tick count during logging*/
  uint32 curr_ustmr_time; 
  
} rflm_c2k_txagc_update_time_log_s;
...
typedef struct
{
  ...
  /* @brief To log TxAGC Update timing data*/
  rflm_c2k_txagc_update_time_log_s 
                    txagc_update_time_buff[RFLM_TXAGC_UPDATE_BUFF_SIZE]; 
  
  uint8 txagc_update_cnt;
  /*! 
    @brief
...
/*
  @brief
  Pointer to data structure holding debug command parameters.
  */
  rflm_c2k_rxagc_debug_crash_s* debug_cmd_data_ptr;

  uint32 prx_handle;

  int8 prev_txagc_update_task_idx;
}rflm_c2k_data_t;
...
/* This is maximum number of LNA states supported by RF CDMA FED driver 
   currently. The limitation stems from the heavy use of vectors(). These 
   vectors are highly amenable to Q6 intrinsic operations. These operations
   are very frequent and hence using Q6 intrinsics saves a lot of processing
   overhead.
*/
#define RFLM_C2K_NUM_LNA_STATES 5

/* The thresholds matrix is accessed as vect64 array union-ed with the 
   uint16 array. Fail the compilation if LNA states are increased. 
   If this happens the LNA FSM implementation needs to be looked at again */
COMPILE_ASSERT((RFLM_C2K_NUM_LNA_STATES-1) == 4)

/*----------------------------------------------------------------------------*/

/*!
  @name C2K LNA Thresholds 
 
  @brief
  This structure holds information related to C2K LNA
  Thresholds
 
  @details
  We use this matrix and spread the switchpoints for easy vector
  compare in Q6. Here is how it looks.
  Example switchpoints  :
  rise_threshold[] = { -660 , -562 , 381 , -127 , 0 , 0 } ;
  fall_threshold[] = { -700 , -602 , 421 , 127 , 0 , 0 } ;


  Example Output:
  FallThrshlds->matrix[0][0] : -660 r
  FallThrshlds->matrix[0][1] : -562 r
  FallThrshlds->matrix[0][2] : -381 r
  FallThrshlds->matrix[0][3] : -127 r

  RiseThrshlds->matrix[1][0] : -700 f
  FallThrshlds->matrix[1][1] : -562 r
  FallThrshlds->matrix[1][2] : -381 r
  FallThrshlds->matrix[1][3] : -127 r

  RiseThrshlds->matrix[2][0] : -700 f
  RiseThrshlds->matrix[2][1] : -602 f
  FallThrshlds->matrix[2][2] : -381 r
  FallThrshlds->matrix[2][3] : -127 r
 
  RiseThrshlds->matrix[3][0] : -700 f
  RiseThrshlds->matrix[3][1] : -602 f 
  RiseThrshlds->matrix[3][2] : -421 f
  FallThrshlds->matrix[3][3] : -127 r
 
  RiseThrshlds->matrix[4][0] : -700 f
  RiseThrshlds->matrix[4][1] : -602 f
  RiseThrshlds->matrix[4][2] : -421 f
  RiseThrshlds->matrix[4][3] : -127 f
  */
typedef struct {
  /*! Each row of the matrix will represent the thresholds to be 
    used by the state represented by the row index. In order to
    use intructions that operate on halfwords in a vector, 8-byte
    alignment enforced on the 2D-array. Due to the length of each
    row being 8 bytes exactly, subsequent row will automatically
    have the alignment
   
    */

  union {
    int16 matrix[RFLM_C2K_NUM_LNA_STATES][RFLM_C2K_NUM_LNA_STATES-1]  __attribute__ ((aligned (8)));
    vect64 matrix_vect[RFLM_C2K_NUM_LNA_STATES] ;
  } ;

} rflm_c2k_lna_thresholds_matrix_t ;

/*----------------------------------------------------------------------------*/
/*!
  @name rflm_c2k_lna_override_params_t
 
  @brief
  This structure holds override params related to C2K LNA FSM
 
  @note
  Please keep all the LNA FSM override params here in
  rflm_c2k_lna_override_params_t
*/
typedef struct
{
  /*! Override params for the hysteresis timer */
  rflm_c2k_rx_lna_hyst_operations_t hyst_timer_mode ;

} rflm_c2k_lna_override_params_t ;

/*----------------------------------------------------------------------------*/
/*!
  @name C2K LNA FSM info
 
  @brief
  This structure holds information related to C2K LNA FSM
 */
typedef struct 
{
  /*! Holds IP from FW. Used for logging and debugging. */
  rflm_c2k_lna_gain_update_in_t fw_inputs ;

  /*! Holds override params for LNA SM */
  rflm_c2k_lna_override_params_t lna_override_params ;

  /*! This vector holds the rise hystereis time as passed by SW.*/ 
  vect64 rise_hyst_timers ;

  /*! This vector holds the fall hystereis time as passed by SW.*/ 
  vect64 fall_hyst_timers ;

  /*! This vector holds the rise and fall hyst time down count. 
      At tune this count is initialized to rise and dall
      hysteresis count and then at every LNA update, this number
      is down counted until it reaches 0. During this time the
      LNA state is frozen */
  vect32 current_hyst_timer_val ;

  /*! Holds the current LNA state. Updated when AGC changes 
    along with other criteria satisfied */
  uint8 lna_state ;

  /*! Holds the previous LNA gain state for comparison to decide 
    if it is time to change LNA state */
  uint8 previous_lna_state ;

  /*! Points to the row in the states by thresholds matrix
     corresponding to thresholds for current state */
  vect64* current_thresholds ;           
  
  /*! This array contains fall and rise thresholds spread out to 
    suit easy comparison using the Q6 intrinsics */
  rflm_c2k_lna_thresholds_matrix_t thresholds ;
} rflm_c2k_lna_fsm_type ;


/*----------------------------------------------------------------------------*/
/*!
  @name C2K LNA FSM info with handle IDs to differentaiate
  between Rx chains
 
  @brief
  This structure holds pointers related to C2K LNA FSM global
  data structure and the corresponding handle IDs.
 */
typedef struct
{
  /*! The index that corresponds to the handle ID out of the global data 
    structure lna_sm_arr[RFLM_C2K_MAX_LNA_FSM_SUPPORTED]
    @note: The value of index == 0xFF indicates that this handle
    is ready for use */
  uint8                   index ;

  /*! The handle ID to associate with LNA FSM data structure */
  rflm_handle_rx_t        handle_id ;

  /*! The LNA FSM pointer that goes with the given handle ID */
  rflm_c2k_lna_fsm_type*  lna_sm_p ;
} rflm_c2k_lna_fsm_data_t ;
...
typedef struct
{
  ...
  /*! LNA FSM Information */
  rflm_c2k_lna_fsm_type* lna_sm  ;
}rflm_c2k_tech_data;

/*---------------------------------------------------------------------------*/
uint8
rflm_c2k_get_txagc_ccs_mem_block
(
...
/*---------------------------------------------------------------------------*/
uint8
rflm_c2k_get_coupler_static_event_ccs_mem_block
(
  rflm_cdma_mdsp_script_id_t script_id,
  rflm_tech_id_t  tech_id
);
...
/*---------------------------------------------------------------------------*/
void 
rflm_c2k_allocate_txagc_ccs_mem
(
  rflm_tech_id_t  tech_id 
);
...
/*---------------------------------------------------------------------------*/
void 
rflm_c2k_deallocate_txagc_ccs_mem
(
  rflm_tech_id_t  tech_id 
);

void 
rflm_c2k_allocate_coupler_static_event_ccs_mem
( 
  rflm_tech_id_t  tech_id , 
  rflm_cdma_mdsp_script_id_t script_id
);
...
void 
rflm_c2k_deallocate_coupler_static_event_ccs_mem
( 
  rflm_tech_id_t  tech_id , 
  rflm_cdma_mdsp_script_id_t script_id
);
...
/*----------------------------------------------------------------------*/
void rflm_c2k_mc_execute_rxagc_debug_cmd
(
    const rflm_c2k_rx_gain_update_in_t *rxagc_inputs  
);


/*---------------------------------------------------------------------------*/
rflm_c2k_lna_fsm_type*
rflm_c2k_get_lna_fsm_data 
( 
...
/*---------------------------------------------------------------------------*/
void
rflm_c2k_tx_set_cl_event 
( 
  rflm_c2k_tx_data_t*   tx_fbrx_info ,
  boolean               run_fbrx_capture 
) ;


/*---------------------------------------------------------------------------*/
boolean
rflm_c2k_tx_get_cl_event 
( 
  rflm_c2k_tx_data_t*   tx_fbrx_info 
) ;
...
/*---------------------------------------------------------------------------*/
rflm_ustmr_t
rflm_cdma_get_time_for_vswr_captures
(
  rflm_ustmr_t effect_time  
) ;
...
/*---------------------------------------------------------------------------*/
boolean
rflm_c2k_is_dual_capture_enabled 
( 
  void 
) ; 
...
/*---------------------------------------------------------------------------*/
boolean
rflm_c2k_tx_get_skip_txagc_updates_flag
( 
  rflm_c2k_tx_data_t*   txagc_data 
) ;


/*---------------------------------------------------------------------------*/
void
rflm_c2k_tx_set_skip_txagc_updates_flag
( 
  rflm_c2k_tx_data_t*   txagc_data ,
  boolean               ignore_txagc_updates_from_fw 
) ;


/*---------------------------------------------------------------------------*/
boolean
rflm_c2k_tx_get_const_power
( 
  rflm_c2k_tx_data_t*   txagc_data 
) ;
...
/*---------------------------------------------------------------------------*/
void
rflm_c2k_tx_set_const_power
( 
  rflm_c2k_tx_data_t*   txagc_data ,
  boolean               const_pwr_from_fw 
) ;


/*---------------------------------------------------------------------------*/
boolean
rflm_c2k_tx_get_aol_event 
( 
   rflm_c2k_tx_data_t*   tx_info 
) ; 


/*---------------------------------------------------------------------------*/
void
rflm_c2k_tx_set_aol_event_pending 
( 
  rflm_c2k_tx_data_t*   tx_info ,
  boolean               is_aol_pending 
) ;
...
/*---------------------------------------------------------------------------*/
int32 
rflm_c2k_get_pa_start_addtl_offset
(
  rflm_tech_id_t tech_id
);
...
/*---------------------------------------------------------------------------*/
void
rflm_c2k_set_pa_start_addtl_offset
(
  rflm_tech_id_t tech_id,
  int32 pa_addtl_offset
);
