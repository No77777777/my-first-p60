...
$Header: //components/rel/rflm.mpss/2.11/rftech/tdscdma/inc/rflm_tds_mc.h#1 $

when         who    what, where, why
--------   ---     ----------------------------------------------------------
02/16/20   hal     Kamorta GNSS Blanking FR
10/19/16   nsh     Add raw reading in HDET RSP
08/09/16   jr      Dynamically FTM task priority change in RFCAL mode
04/21/16   hx      Change value of RFLM_TDS_TUNER_AOL_EVENT 
...
09/22/15  jps/al   Use separate FBRX RX-TX relative delay for cont v/s burst TX
04/29/15   jps     Hook up common TXAGC logging
04/22/15   lwu     Updated API interface to allow FW query status of LNA script
04/22/15   jak     Add new function of rflm_tds_mc_enter_mode_no_tq_lock()
...
==============================================================================*/
//#ifdef FEATURE_RF_ASDIV  /*FEATURE_RF_ASDIV cannot be seen by rflm*/
  /*TDS location control feature which can be mainlined later*/
  /*feature can cut through to rf_tech, need mainline at FED side*/
 // #ifndef FEATURE_RF_TDS_ASDIV_ELNA_GAIN
 // #define FEATURE_RF_TDS_ASDIV_ELNA_GAIN
 // #endif
//#endif
...
extern rflm_handle_rx_t rflmFbrxHandle;
...
typedef enum
{
  ...
  */  
  ...
  RFLM_TDS_TUNER_AOL_EVENT = RFLM_MAX_NUM_HANDLE_SCRIPT_BUF -4,  /*change the event id from 47 to 46, avoid conflict in tuner side*/
...
typedef struct
{
  ...
  /* Coupler FWD/REV */
  rflm_ccs_rf_event_header_task_type* vswrCouplerFWDScriptPtr;
  rflm_ccs_rf_event_header_task_type* vswrCouplerREVScriptPtr;

  /* AutoPin write event*/
  rflm_ccs_rf_event_header_task_type* autopinWriteScriptPtrArray[2];

  /* DeviceRead Status event*/
  rflm_ccs_rf_event_header_task_type* deviceStatusReadScriptPtr[2];
...
typedef struct
{
  /* in enableGnssBlakingBitMask , each bit mean each lm handle ID , we use bit mask to indicate 
        where we want to do GNSS blanking for the given TxLM handle ID or not */
  uint32 enableGnssBlakingBitMask;
  boolean CurrGnssBlankingOn;
} rflm_tds_gnss_blank_state_t;
...
typedef struct
{
  ...
  boolean isSelfTestMode;
  boolean enableTxagcDiagMsg;
  boolean startIQCaptureMode;

  /* FBRX Rx Tx relative delay parameters */
  boolean isRfFtmContTxMode; /* 0: Burst TX mode, 1: Continuous TX mode */
  boolean isRfFtmCalMode;
  uint32 fbrx_rx_tx_delay[3]; /* Index0: burst mode relative delay, Index1: continuous TX mode relative delay */

  rflm_tds_autopin_info_t autopinInfo;
  /* GNSS Blanking FR */
  rflm_tds_gnss_blank_state_t GnssBlakingState;
...
typedef struct
{
  ...
  int32   LNALateBy;
  int32   LNALateByUsec;
} rflm_tds_single_slot_lna_info_t;
...
extern boolean is_sawless_enabled;
extern rflm_tds_single_slot_lna_info_t rflmTdsLNAInfo[RFLM_TDS_TS_INVALID];
...
typedef struct
{
  int32  et_delay_ns_x10;                       // delay value
  rflm_txagc_et_path_delay_t et_delay_reg[2];   // ET path delay register value, double buffer
  uint32 et_path_delay_addr;                    // address of current ET path delay register value
  boolean delay_data_idx;                       // index for the double buffer
} rflm_tds_et_delay_setting_s;
...
typedef struct
{
  ...
  rflm_tds_et_delay_setting_s etDelay;        ///< et delay settings 
  int8 pa_comp_up[RFLM_TXAGC_PA_STATE_MAX];
  int8 pa_comp_dn[RFLM_TXAGC_PA_STATE_MAX];
} rflm_tds_tx_data_s;
...
typedef struct
{
  ...
  rflm_handle_rx_t fbrxLmHandle;              ///< FBRxLM handle
...
{  
  boolean  rx_enable;      
  boolean  tx_enable;
}rflm_tds_device_status_enum_type;
...
#define RFLM_TDSCDMA_MAX_LNA_GAIN_STATES_4 4
extern boolean rflmTdsLnaPending; 
extern uint8 rflmTdsAsdivSwPosition;
/*global flag to check if the eLNA offset is enabled*/
extern boolean rflmTdsAsdivElnaEnable;
/*definition for gain offset*/
extern int32 rflmTds_elna_gain_offset[RFLM_TDS_NUM_RX_CHAINS][RFLM_TDSCDMA_MAX_LNA_GAIN_STATES_4][RFLM_TDS_NUM_LIN_STATES];
//#endif

extern rflm_tds_device_status_enum_type is_device_status_read_enable;
...
void rflm_tds_mc_enter_mode_no_tq_lock
(
   rflm_tds_enter_mode_in_t* in, 
   rflm_tds_enter_mode_out_t* out
);
...
(
...
#ifndef FEATURE_RF_HAS_WTR2965
  rflm_tds_fbrx_type_e* fbrx_type,
#endif
  ...
  rflm_tds_vswr_meas_t* vswr_meas,
#endif
  rflm_tds_ccs_exec_single_task_info_t* taskInfoPtr,
  rflm_txagc_seq_type_t sequence_id
);
...
void rflm_tds_set_pa_comp(int16* pa_comp_up,int16* pa_comp_dn);
...
boolean rflm_tds_mc_config_et_delay
(
  uint32 rflm_handle,
  int32 et_delay_val
);

void rflm_tds_mc_clear_et_dac_flag(void);

uint32 rflm_tds_mc_get_txlm_handle();

void rflm_tds_mc_disable_et_dac(void);

#ifndef FEATURE_RF_HAS_WTR2965
rflm_err_t rflm_tds_mc_start_iq_capture(rflm_tds_iq_cap_config_out_t *out);
#endif

void rflm_tds_mc_set_fbrx_lm_handle(rflm_handle_rx_t handle);

void rflm_tds_mc_post_process_fbrx_vswr_measurements
(
  rflm_handle_tx_t handle_id,
  rflm_fbrx_vswr_meas_data_t vswr_meas_data
);

uint32 rflm_tds_mc_get_fbrx_rx_tx_delay(void);

void rflm_tds_mc_set_fbrx_rx_tx_delay(boolean mode, uint32 fbrx_rx_tx_delay);
...
void ftm_set_task_pri(uint16 new_pri );
