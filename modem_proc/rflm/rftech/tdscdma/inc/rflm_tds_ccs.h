...
$Header: //components/rel/rflm.mpss/2.11/rftech/tdscdma/inc/rflm_tds_ccs.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/19/20   hal     Moving IRAT to MMU region and removing unused code on HA
02/15/16   jr/sbo  CCS Memory management (MMU) changes for TA/JACALA
...
04/08/15   lwu     Updated API interface to allow FW query status of LNA script
...
#define RFLM_TDS_RF_CCS_MEM_SIZE_IN_BYTES         (CCS_RF_MMU_STATIC_REGION_MEM_NUM_WORDS * 4) //3K memory
...
typedef enum
{
  ...
  /* VSWR Coupler FWD/REV */
  RFLM_TDS_COUPLER_FWD,
  RFLM_TDS_COUPLER_REV,
...
void rflm_tds_lna_event_callback(rflm_ccs_rf_tq_handle_t *tq, uint32 task_idx, ccs_rf_task_status_t status, void *arg);
...
void rflm_tds_txagc_rf_last_event_callback(rflm_ccs_rf_tq_handle_t *tq, uint32 task_idx, ccs_rf_task_status_t status, void *arg);

void rflm_tds_txagc_update_callback(rflm_ccs_rf_tq_handle_t *tq, uint32 task_idx, ccs_rf_task_status_t status, void *arg);
...
void rflm_tds_device_status_read_trigger_callback(rflm_ccs_rf_tq_handle_t *tq, uint32 task_idx, ccs_rf_task_status_t status, void *arg);
