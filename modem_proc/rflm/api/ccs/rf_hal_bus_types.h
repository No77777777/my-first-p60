...
$Header: //components/rel/rflm.mpss/2.11/api/ccs/rf_hal_bus_types.h#2 $
$DateTime: 2024/03/11 23:41:50 $
...
typedef struct
{
  rf_hal_bus_resource_script_type bus_data;                 
  rf_hal_mode_type mode;
} rf_hal_tech_bus_resource;

typedef struct
{
  rf_hal_tech_bus_resource *rflm_ccs_debug_tech_table;
  rf_hal_band_type band;                   
} rf_hal_tech_band_common_resource;


typedef struct
{
  rf_hal_tech_band_common_resource *rflm_ccs_tech_band_table;
  rflm_tech_id_t tech;                   
} rf_hal_tech_bus_common_resource;
...
typedef struct
{
  ...
  uint8 half_rate;          /*!< Use half-rate reads. Applicable only to RFFE */
  uint8 rd_delay;           /*!< Insert delay before reading. Application only to RFFE (and SSBI?) */
