...
$Header: //components/rel/rflm.mpss/2.11/api/ccs/rf_hal_rffe.h#1 $ 
...
/*! @brief Populate the hal script to write a single RFFE register 
 
  @details Populate the hal script to write a single RFFE transaction to a single device. Supports all
  MIPI RFFE command sequences, but only up to 1 byte of payload. 
  ...
  @param script
  Pointer to be filled with hal script information
...
boolean rfhal_rffe_populate_single_tranx_script
(
  uint8 channel,
  uint8 slave_id,
  uint16 reg,
...
/*! @brief Populate the hal scripts to write multiple RFFE registers (simple)
 
  @details Populate the hal scripts to write multiple RFFE registers to a single device. 
  Supports MIPI RFFE command sequences: reg0, write, write ext,
  but only up to 1 byte of payload per transaction. 
...
*/
boolean rfhal_rffe_populate_multi_tranx_script
(
  uint8 channel,
  uint8 slave_id,
  uint16* reg,
...
/*! @brief Process an entire script buffer. 
      ...
      @settings 
*/
rf_hal_bus_result_type rfhal_rffe_execute_buf_script_ext
(
  rf_hal_bus_rffe_type* script_ptr,
  uint32 count,
  rf_hal_bus_resource_script_settings_type* settings,
...
#define rfhal_rffe_execute_buf_script(script_ptr,count,settings,... ) \
  rfhal_rffe_execute_buf_script_ext( script_ptr,count,settings,(FALSE, ## __VA_ARGS__)) 
...
#define rfhal_rffe_execute_rf_buf(rf_buff,rffe_return_script,read_size,settings,max_read_size,...) \
	rfhal_rffe_execute_rf_buf_ext(rf_buff,rffe_return_script,read_size,settings,max_read_size,(FALSE, ## __VA_ARGS__)) 


boolean rf_hal_rffe_reg_read
(
  uint8 channel, 
  uint8 slave_id, 
  boolean rd_delay, 
