...
$Header: //components/rel/rflm.mpss/2.11/api/fw/rflm_api_gsm.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/23/2019  ap      Removing fw_enh_cb_tag
10/28/15   tws     Add FBRX VSWR support
08/14/15   tws     Add new message type RFLM_GSM_HMSC_MONITOR
08/18/15   sc      Deprecate subscription 3 reducing memory footprint
07/27/15   sc      Remove unrequired code for optimisation and cleanup
06/03/15   sc      Add defintions for enhanced FW callback for improved debug
05/27/15   sc      Add Tx band cancellation hook for GFW
05/19/15   tws     Add drif settings ptr so drif settings can be copied to
                   GFW for logging purposes.
05/07/15   sw      Introduce subscription ID to IRAT api
03/25/15   sc      Add wakeup Rx/Tx interface
...
typedef enum
{
  ...
  RFLM_GSM_TX_BAND_CANCELLATION,
  RFLM_GSM_TX_BURST_VSWR_MEASUREMENT,
...
/*===========================================================================*/
/*! @enum rflm_gsm_fw_cb_error_msg_type                                      */
/*! @brief FW callback error enums                                           */
/*! @details enum values are log2 to enable a mask of more than one error    */
typedef enum
{
  RFLM_GFW_CB_ERR_NONE           = 0,
  RFLM_GFW_CB_RX_START_TASK_LATE = 1<<1,
  RFLM_GFW_CB_TX_START_TASK_LATE = 1<<2,
}rflm_gsm_fw_cb_error_msg_type;



/*===========================================================================*/
/*! @fp rflm_gsm_enh_fw_cb_data_type                                         */
/*! @brief FW enhanced callback structure providing enhanced debug of CCS    */
/*!        activities to enable increased information during logging         */
/*! @details If FW have registered an enhanced callback RFLM GSM will execute*/
/*!          it passing this structure of populated information              */
/*! @param num_pending_events - events remaining on the CCS TQ pair          */
/*! @param fw_tag - FW defined tag passed in Rx/Tx CCS to tie up with cb     */
/*! @param err_msg - bitmask of errors where value is 0 if no error detected */
typedef struct
{
  uint32 num_pending_events;
  void * fw_tag;
  uint16 err_msg; //bit-mask
}rflm_gsm_enh_fw_cb_data_type;
...
/*===========================================================================*/
/*! @fp rflm_gsm_enh_fw_cb_type                                              */
/*! @brief fp definition of FW registered function to call every cb          */
/*! @details Call FW every GSM CCS cb with full details of event status      */
/*!          Required for FW to receive data from CCS processing             */
/*! @param void* - input arg pointer regsitered by FW to pass back           */
/*! @param rflm_gsm_enh_fw_cb_data_type - structure of data to pass to GFW   */
typedef void (*rflm_gsm_enh_fw_cb_type)(void*,rflm_gsm_enh_fw_cb_data_type*);
...
typedef struct
{
  ...
  void* fw_cb_arg;
  rflm_gsm_enh_fw_cb_type enh_callback;
...
/*============================================================================*/
/*! @struct rflm_gsm_get_tx_alpha_output_type                                  */
/*! @brief  This structure is for RF to return RF data to FW                  */
/*! @details FW create a variable of this type and pass in address as argument*/
/*!          GSM RF will use this information to calc output return values    */
typedef struct
{
  uint16 tx_alpha;
  uint16 tx_extended_alpha;
}rflm_gsm_get_tx_alpha_output_type;
...
typedef struct
{
  uint32 drif_settings[64];
} rflm_txfe_drif_settings_t;

typedef struct
{
  rflm_ustmr_t execution_time;
  rflm_dtr_tx_xfer_list_s *txfer_list_ptr;
  rflm_txfe_drif_settings_t *drif_ptr; 
...
/*============================================================================*/
/*! @struct rflm_gsm_init_ccs_output_type                                     */
/*! @brief Structure for init CCS script processing output params             */
/*! @details This structure is for passing information to GFW from FED        */
/*! @param execution_time - time take for script to execute                   */
typedef struct
{
  rflm_ustmr_t execution_time;
...
/*============================================================================*/
/*! @struct rflm_gsm_rx_wakeup_ccs_input_type                                 */
/*! @brief Structure for wakeup Rx CCS script processing input params         */
/*! @details This structure is for passing information to FED                 */
/*! @param trigger time - time for script to take effect                      */
/*! @param immediate flag - send script ASAP and ignore the trigger time      */
/*! @param rflm_gsm_intf_idx - interface index for script data retrieval      */
typedef struct
{
  rflm_ustmr_t trigger_time;
  boolean      immediate_trig_flag;
  uint32       rflm_gsm_intf_idx;
}rflm_gsm_rx_wakeup_ccs_input_type;



/*============================================================================*/
/*! @struct rflm_gsm_rx_wakeup_ccs_output_type                                */
/*! @brief Structure for wakeup Rx CCS script processing output params        */
/*! @details This structure is for passing information to GFW from FED        */
/*! @param execution_time - time take for script to execute                   */
typedef struct
{
  rflm_ustmr_t execution_time;
}rflm_gsm_rx_wakeup_ccs_output_type;



/*============================================================================*/
/*! @struct rflm_gsm_tx_wakeup_ccs_input_type                                 */
/*! @brief Structure for wakeup Tx CCS script processing input params         */
/*! @details This structure is for passing information to FED                 */
/*! @param trigger time - time for script to take effect                      */
/*! @param immediate flag - send script ASAP and ignore the trigger time      */
/*! @param rflm_gsm_intf_idx - interface index for script data retrieval      */
typedef struct
{
  ...
  boolean      immediate_trig_flag;
  uint32       rflm_gsm_intf_idx;
}rflm_gsm_tx_wakeup_ccs_input_type;



/*============================================================================*/
/*! @struct rflm_gsm_tx_wakeup_ccs_output_type                                */
/*! @brief Structure for wakeup Tx CCS script processing output params        */
/*! @details This structure is for passing information to GFW from FED        */
/*! @param execution_time - time take for script to execute                   */
typedef struct
{
  rflm_ustmr_t execution_time;
}rflm_gsm_tx_wakeup_ccs_output_type;
...
/*! @param *fw_enh_cb_tag - GFW tag to associate fw cb with Rx/Tx activity    */
typedef struct
{
  rflm_ustmr_t trigger_time;
  boolean      immediate_trig_flag;
...
/*============================================================================*/
/*! @struct rflm_gsm_vswr_input_type                                          */
/*! @brief Structure for Tx VSWR script processing input params               */
/*! @details This structure is for returning information from FED to GFW      */
/*! @param sub_id - The subscription id                                       */
/*! @param is_rach_burst - This is a RACH burst                               */
typedef struct
{
  rflm_gsm_subscription_id  sub_id;
}rflm_gsm_vswr_input_type;

/*============================================================================*/
/*! @struct rflm_gsm_vswr_output_type                                         */
/*! @brief Structure for Tx VSWR script processing output params              */
/*! @details This structure is for returning information from FED to GFW      */
/*! @param doFbrxCapture - Capture FBRX samples for processing in this frame  */
/*! @param vswr_meas_period - The VSWR measurement period                     */
/*! @param fbrx_buf_index - The RFLM buffer index for the FBRX chain          */
/*! @param execution_time - time take for script to execute                   */
typedef struct
{
  boolean      doFbrxCapture;
  uint16       vswr_meas_period;
  uint32       fbrx_buf_index;
  rflm_ustmr_t execution_time;
}rflm_gsm_vswr_output_type;
...
typedef enum
{
  RFLM_GSM_FBRX_MEAS_SUCCESS,             ///< indicates FBRX measurement is success
  RFLM_GSM_FBRX_MEAS_BAD_LSE,             ///< indicates FBRx measurement has LSE out of limits
  RFLM_GSM_FBRX_MEAS_BAD_GAIN_EST,        ///< indicates FBRx measurement has  bad incident gain
  RFLM_GSM_FBRX_MEAS_ABORT,               ///< indicates FBRx measurement has  been aborted
} rflm_gsm_fbrx_meas_status_e;
...
typedef struct
{
  ...
  int16 txagc_dB10;
  rflm_gsm_fbrx_meas_status_e meas_status[2]; /* meas status for reflected and incident power*/
  uint32 incident_cgain_est;
  uint32 reflected_cgain_est;
}rflm_gsm_fbrx_vswr_meas_data_t;
...
extern rflm_err_t rflm_gsm_get_tx_alpha_v2( rflm_handle_tx_t tx_handle,
                                            rflm_gsm_get_tx_alpha_input_type *rflm_gsm_vars_in,
                                            rflm_gsm_get_tx_alpha_output_type *rflm_gsm_vars_out );
...
/*============================================================================*/
/*! @fn rflm_gsm_prog_rx_wakeup_ccs_script                                    */
/*! @brief A command for FW to call FED for CCS wakeup Rx processing          */
/*! @details Wakeup Rx device to GSM mode                                     */
extern rflm_err_t rflm_gsm_prog_rx_wakeup_ccs_script( rflm_handle_rx_t rflm_handle,
                                                      rflm_gsm_rx_wakeup_ccs_input_type *script_vars_in,
                                                      rflm_gsm_rx_wakeup_ccs_output_type *script_vars_out );



/*============================================================================*/
/*! @fn rflm_gsm_prog_tx_wakeup_ccs_script                                    */
/*! @brief A command for FW to call FED for CCS wakeup Tx processing          */
/*! @details Wakeup Tx device to GSM mode                                     */
extern rflm_err_t rflm_gsm_prog_tx_wakeup_ccs_script( rflm_handle_tx_t rflm_handle,
                                                      rflm_gsm_tx_wakeup_ccs_input_type *script_vars_in,
                                                      rflm_gsm_tx_wakeup_ccs_output_type *script_vars_out );
...
extern rflm_err_t rflm_gsm_is_fbrx_vswr_capture_allowed( rflm_handle_tx_t rflm_handle,
                                                         rflm_gsm_vswr_input_type *script_vars_in,
                                                         rflm_gsm_vswr_output_type *script_vars_out );

void rflm_gsm_post_process_fbrx_vswr_measurements( rflm_handle_tx_t handle_id,
                                                   rflm_gsm_fbrx_vswr_meas_data_t vswr_meas_data );
