...
$Header: //components/rel/rflm.mpss/2.11/api/fw/rflm_api_wcdma.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/02/16   nv      Add API support to query FBRX handle
11/05/15    ag     Added Autopin events type for WCDMA
08/28/15    dw     Increase RFLM_WCDMA_MAX_NB to 10 for TH
08/25/15    aa     Enqueue FBRX gain state update
08/13/15    ac     api for fw to call when synch_point_time_stamp has exceeded the iq_gain_time
08/04/15    ac     prefetch change
07/28/15    yb     Added Coupler events
07/20/15    zhh    pass the mtpl_adj_db10 to FW
06/03/15    zhh    pass the LUT revert value to WFW
06/02/15    gh/zhh Added THERM_ENABLE event
04/30/15    ac     rx agc info parameters
04/17/14    dw     Add workarounds to avoid PA switching in high E-TCFI
...
typedef enum
{
  ...
  RF_WCDMA_EVENT_FBRX_CONFIG_COUPLER_FWD, 
  /*----------------------------------*/
  /*!FBRx Coupler REV event*/    
  RF_WCDMA_EVENT_FBRX_CONFIG_COUPLER_REV, 

  /*!THERM enable event*/    
  RF_WCDMA_EVENT_THERM_ENABLE,
  /*----------------------------------*/
  /*!FBRx GAIN state update event*/    
  RF_WCDMA_EVENT_FBRX_GAIN_UPDATE, 
  /*----------------------------------*/
...
typedef struct
{
  ...
  rflm_ustmr_t vswr_meas_start_time;       ///< Start time of the VSWR measurement first capture, start of BPG 2 in normal case, for HS depending on HS offset
  rflm_ustmr_t vswr_meas_end_time;         ///< End time of the VSWR measurement second capture, end of BPG 4 in normal case, for HS depending on HS offset
  uint32 reserved;
...
/*==============================================================================
  For rflm_wcdma_tx_proc_stable_pwr()
==============================================================================*/
/*! @brief  Output parameters for rflm_wcdma_tx_proc_stable_pwr API */
typedef struct
{
  boolean                    run_vswr;  ///< Flag indicating whether to run VSWR dual capture or not
  boolean                    run_swpt;  ///< Flag indicating whether to run Switchpoint capture or not
  uint8            offset_block_count;  ///< which block to start the FBRx capture from effect time of the power change
  rflm_fbrx_pwr_dependent_param_t fbrx_pwr_dep_params[2];    ///< FBRx related parameters for 2 captures. Index 0 maps to first capture, index 1 maps to second capture.
  uint32                     reserved;  ///< Reserved field for future use  
} rflm_wcdma_tx_proc_out_t;
...
{
  rflm_ustmr_t        effect_time;        ///< Time for gain update to take effect
  ///< waveform type: 0->R'99 (always)
  ///< For HS waveform WFW will pass BPG/HSoffset information along with HS waveform type.
  /*  
  For HS the return value = XY (decimal)
  X = 0/1/2/3 for R99/HSDPA/HSUPA/HSDPA+HSUPA
  Y =  BPG which TxAGC is scheduled for 

  | BPG | HSDPA | HSUPA | HSDPA + HSUPA |
  ---------------------------------------
  | 0   |  10   |  20   |   30          |
  | 1   |  11   |  21   |   31          |
  | 2   |  12   |  22   |   32          |
  ...
  For example for HS offset 3 we will have alternating 0, 3 for Waveform… 0 => BPG9 TxAGC and 3=> BPG2 HS TxAGC.
  */
  int8                waveform_type;      
...
typedef struct
{
  ...
  uint8                     digital_gain_update_block;         ///< Move IQ gain sync point
  uint32                    reserved;                         ///< Reserved for future use
...
typedef struct
{
  ...
  boolean            pa_oride;             ///< Indicate PA override from FW
  rflm_db10_t        out_pwr_pa_oride;     ///< Indicate TxAGC which PA will be switching to based on. 
                                           ///This power is only used to calculate the PA switching oride, 
                                           ///final output power is still based on the out_pwr input.
  uint32 reserved;                         ///< Reserved for future use
...
{  
  ...
  /* WTR Rx PLL settling time in STMR unit */
  uint32 pll_settling_time;

  /* Rx tune script execution time in STMR unit */
  uint32 rx_tune_time;
...
/*!
  ...
  @param *out    : pointer to out data structure for dual vswr captures.
...
/*!
  @brief
  API to disable PA switching timer hysterisis

  @details
  This API is another workaround for ETFCI 127 where FW will call to disable
  PA switching timer hysterisis to avoid frequent PA state switching causing BLER
 

  @param handle : Handle containing script data and control information
  @param disable_pa_hys: TRUE to disable PA timer hysterisis (Longest hysteris possible)
                         False to restore default timer hysterisis

  @retval Returns an error status

===========================================================================*/
extern rflm_err_t rflm_wcdma_txagc_disable_pa_hys(rflm_handle_tx_t handle_id, boolean disable_pa_hys);

/*----------------------------------------------------------------------------*/
/*@ brief  the pre-fetch function to be called once during tx enable for ccs memory 
  write 

  @detail
 pre-fetch function to be called once during tx enable for ccs memory 
  write 
...
*/
boolean
rflm_wcdma_ccs_txagc_prefetch
(
  uint32 handle
);
/*----------------------------------------------------------------------------*/

/*! @brief  output parameters for rflm_wcdma_tx_iq_gain_update_expire   API */
typedef struct
{
   /* for now print a msg or schedule an iq update*/
   uint32 reserved;
       
} rflm_wcdma_tx_iq_gain_update_expire_out_t;

/*! @brief  Input parameters for rflm_wcdma_tx_iq_gain_update_expire   API */
typedef struct
{
  int32               iq_gain_time;   
  rflm_ustmr_t        synch_point_time_stamp;
     
} rflm_wcdma_tx_iq_gain_update_expire_in_t;

/*----------------------------------------------------------------------------*/
/*!
  @brief
  rflm_wcdma_tx_iq_gain_update_expire api is called by fw when
  synch_point_time_stamp has exceeded the iq_gain_time
  ...
  @param handle : Handle containing script data and control information
  @param *in    : pointer to input data structure. 
  @param *out   : for now we are printing a msg, reserved field 
...
===========================================================================*/
rflm_err_t rflm_wcdma_tx_iq_gain_update_expire(rflm_handle_tx_t handle,
                                    rflm_wcdma_tx_iq_gain_update_expire_in_t *in,
                                    rflm_wcdma_tx_iq_gain_update_expire_out_t *out);
...
/*!
  @brief
  Wrapper API to return the FBRX LM handle for given TX handle.

  @details
  This API returns the FBRX LM handle to tech FW by querying common fw.

  @param handle_id             : input handle id 
  @param *fbrx_lm_handle   : pointer to data structure where the FBRX LM Handle wil be stored.

  @retval Returns an error status
===========================================================================*/
extern rflm_err_t rflm_wcdma_fbrx_get_fbrx_handle(rflm_handle_tx_t handle_id, uint32 *fbrx_lm_handle );
