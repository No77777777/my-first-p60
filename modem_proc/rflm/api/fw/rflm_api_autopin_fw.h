...
$Header: //components/rel/rflm.mpss/2.11/api/fw/rflm_api_autopin_fw.h#1 $

when       who    what, where, why
--------   ---    ------------------------------------------------------------- 
03/15/16   zhh    Update capture param to support AMAM based delta Pout calc
...
/*! @brief  Defines autopin processing algorithm for FW to use */
typedef enum
{
  RFLM_AUTOPIN_AMAM_LEGACY             = 0,  /* LEGACY AMAM Algorithm */
  RFLM_AUTOPIN_AMAM_ONESHOT            = 1,  /* 1 shot AMAM Algorithm */
  RFLM_AUTOPIN_ALGO_MAX
}rflm_autopin_algo_type;
...
typedef struct
{
  ...
  rflm_autopin_algo_type proc_type;    /* AutoPin algorithm type for FW capture processing */
  uint8                  kernel_order; /* Kernel vector order for polynomialfit, default 5 - for Kernel Vector [1 3 5 7 9]*/
