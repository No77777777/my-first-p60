...
$Header: //components/rel/rflm.mpss/2.11/api/fw/rflm_api_tds.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/22/15  jps/al   Use separate FBRX RX-TX relative delay for cont v/s burst TX
04/30/15   ych     Added fw cb handle to cleanup script
04/08/15   lwu     Adding IsreAcqRefTimeAdvanceReq to allow reftime shift when needed
03/18/15   spa     Remove include of rflm_rxagc_api.h
...
typedef struct
{
  ...
  rflm_tds_meas_tech_e measTech;
  boolean reAcqRefTimeAdvanceReq;
  void (*iratCleanupCallback)(void *);
...
{
  RFLM_TDS_FBRX_NONE,
  RFLM_TDS_FBRX_POWER_MEAS,
  RFLM_TDS_FBRX_VSWR_MEAS
} rflm_tds_fbrx_type_e;

/* extra parameters for VSWR meas */
typedef struct
{
  uint8 num_blocks_in_gap;                   ///< duration of the gap between the 2 capture blocks
  rflm_fbrx_pwr_dependent_param_t fbrx_pwr;  ///< FBRX power dependent parameters for the 2nd capture
} rflm_tds_vswr_meas_t;
#endif
...
#ifndef FEATURE_RF_HAS_WTR2965
  rflm_tds_fbrx_type_e fbrx_type;       ///< normal fbrx or vswr
#endif
...
#ifndef FEATURE_RF_HAS_WTR2965
  rflm_tds_vswr_meas_t vswr_meas;       ///< extra param needed for VSWR meas
  uint8 fbrx_gain_state[RFLM_TDS_MAX_NUM_SLOT_PER_SCHEDULE_POINT-1];
                                        ///< FBRX lna gain state being applied
#endif
  uint32  freqInKHz;
...
typedef enum
{
  RFLM_TDS_LNA_INFO_READY,
  RFLM_TDS_LNA_NOT_SCHEDULED,
  RFLM_TDS_LNA_INFO_NOT_READY  
} rflm_tds_get_lna_late_time_status_e;
...
typedef struct
{
  rflm_tds_get_lna_late_time_status_e returnStatus;
  int32 LNALateTime; // USTMR count
  uint32 LNAExpectedCompleteTime; // USTMR count
} rflm_tds_get_lna_late_time_out_t;

#ifndef FEATURE_RF_HAS_WTR2965
/*! @brief  Output parameters for rflm_tds_iq_cap_config API */
typedef struct
{
  rflm_handle_rx_t fbrxLmHandle;     //< FBRxLM handle
} rflm_tds_iq_cap_config_out_t;
#endif
...
#ifndef FEATURE_RF_HAS_WTR2965
extern rflm_err_t rflm_tds_fbrx_iq_cap_start(rflm_tds_iq_cap_config_out_t *out);
#endif

/*==============================================================================
 rflm_tds_fbrx_get_rx_tx_delay
 
  @brief
    API to query TDS FBRX TX RX relative delay
 
  @details
       API to query TDS FBRX TX RX relative delay
 
==============================================================================*/
extern uint32 rflm_tds_get_fbrx_rx_tx_delay(void);
