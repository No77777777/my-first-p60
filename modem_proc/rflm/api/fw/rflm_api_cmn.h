...
$Header: //components/rel/rflm.mpss/2.11/api/fw/rflm_api_cmn.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/16/16   sbo     RFLM CCSMMU Changes
09/02/15   ra      Support changes PDMEM Extension for device hopping
06/23/15   rp      Added Tabasco Spcific Feature to Dump Code.
06/23/15   rp      Adding API that can be called from FW to dump WTR GRFC
                   RXLM and TXLM regs
05/09/15   sc      Add and enable RF sequence number check
03/18/15   spa     Added enum rflm_rxagc_lna_state_e for LNA states
...
/* RFM REGISTERS DUMP MODE MASK */
/* 8 bit mask - LSB TO MSB : 3 bit WTR, 2 bit GRFC, 3 bit RXFE/TXFE regs */
#define RFLM_LOG_NO_REGS													 0
#define RFLM_LOG_CRIT_WTR_ONLY												 1
#define RFLM_LOG_ALL_WTR_ONLY												 2

#define RFLM_LOG_ALL_GRFC_ONLY												 8
#define RFLM_LOG_CRIT_WTR_ALL_GRFC 											 9
#define RFLM_LOG_ALL_WTR_ALL_GRFC 											10

#define RFLM_LOG_ALL_RXFE_ONLY 												32
#define RFLM_LOG_CRIT_WTR_ALL_RXFE 											33
#define RFLM_LOG_ALL_WTR_ALL_RXFE 											34
#define RFLM_LOG_ALL_GRFC_ALL_RXFE 											40
#define RFLM_LOG_CRIT_WTR_ALL_GRFC_ALL_RXFE 								41
#define RFLM_LOG_ALL_WTR_ALL_GRFC_ALL_RXFE 									42

#define RFLM_LOG_ALL_TXFE_ONLY 												64
#define RFLM_LOG_CRIT_WTR_ALL_TXFE 											65
#define RFLM_LOG_ALL_WTR_ALL_TXFE 											66
#define RFLM_LOG_ALL_GRFC_ALL_TXFE 											72
#define RFLM_LOG_CRIT_WTR_ALL_GRFC_ALL_TXFE 								73
#define RFLM_LOG_ALL_WTR_ALL_GRFC_ALL_TXFE 									74

/* Defining the places by which we need to shift to get RxLM and TxLM API BITMASK */
#define RFLM_SHIFT_COUNT_FOR_RXLM_GEN_REG_DUMP_API 							5
#define RFLM_SHIFT_COUNT_FOR_TXLM_GEN_REG_DUMP_API 							6
...
typedef enum 
{
  ...
  RFLM_ERR_RFSW_NOT_READY           = 105,  ///< RFSW have not completed by time RFLM called to process event
...
/*----------------------------------------------------------------------------*/
/*!
  @brief Extended: get TQ based on RFM device 
*/
boolean
rflm_cmn_get_tq_pair_extended
( 
   uint8                       device  ,
   rflm_tech_id_t              tech    ,
   uint8*                      tq_pair,
   uint16                      size_ccs_buffer 
) ;
/*----------------------------------------------------------------------------*/
uint16 rflm_cmn_get_tq_pair_ccs_buffer_size(uint8 tqp_idx );
/*----------------------------------------------------------------------------*/
/*!
...
#ifdef FEATURE_TABASCO_MODEM
void rflm_dump_regs(uint8 dump_mode_mask);
#endif
