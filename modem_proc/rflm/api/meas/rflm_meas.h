...
$Header: //components/rel/rflm.mpss/2.11/api/meas/rflm_meas.h#1 $
$DateTime: 2021/03/09 05:37:22 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
11/03/15   ak      Increase the meas Preload transactions to support IDLE mode diversity on 2965
10/12/15   ak      Increase Meas Preload Trasactions to support IDLE mode diversity in QTA on 29x5
09/01/15   hkm     Allow extra WTR transactions for preload scenario for x2g gaps on TQ4
...
#define RFLM_MEAS_MAX_PRELOAD_TRANS     180
/* 
With 220: +40 in pdmem exhausted. 
In pdmem +70: 30*4=120; 120/2=60 increased; So total 220+60=280
Currently seen 448 of size;
Left with 30(280-250); Now 10(80->100)
Seems different calc for tune-back events(RFLM_MEAS_MAX_TB_TRNSCVR_TRANS).
*/
#define RFLM_MEAS_MAX_TRNSCVR_TRANS     250
#define RFLM_MEAS_MAX_NON_TRNSCVR_TRANS 100  

/* Re-partition CCS for tune-back*/
#define RFLM_MEAS_MAX_TB_TRNSCVR_TRANS     330
...
#define RFLM_MEAS_TOTAL_BUF_SIZE_BYTE (CCS_RF_TASKQ_DATA_BUF_IRAT_NUM_WORDS)*(sizeof(uint32))
