...
$Header: //components/rel/rflm.mpss/2.11/api/gsm/rflm_gsm_dm.h#3 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/30/21   aad     [CR 3044674]increased RFGSM_RX_WAKEUP_MAX_TASKS_TASKQ_A from 8 to 12
09/07/21   aad     [FIX CR-2834297] to resolve modem crash
02/02/20   mpa     To support MiniDump Feature
11/14/19   mpa     changes to pass mutex pointer to QMUTEX apis
07/10/19   kpa     [2487271] Upgrade the QMUTEX to QURT apis
02/23/18   par     Increase size for RFGSM_TX_STOP_MAX_NON_SSBI_TRANSACTIONS_TASKQ_A
02/01/18   par     Increase FE sizes to for additional components.
10/10/17   par     Increase GSM DM for B71 RF card support
14/06/16   hkm     Increase FE sizes to allow detune feature.
05/03/16   par     DM usage optimizations
02/12/16   par     Fix Tx cal failures on 3925
11/10/15   tws     Add FBRX VSWR support
11/06/15   bp      Increase stop/start task q_b size
10/07/15   tws     Add variable to store tx_stop priority for HMSC case
07/14/15   sc      Use unique interface indexes for therm reads
07/01/15   tws     Increase Rx start RFFE TQA transactions to 58 to accommodate for JDET writes
06/30/15   tws     Increase TX start and cal transactions to 60
06/30/15   tws     Increase RX_WAKEUP transactions for WTR2955 sawless
06/26/15   jfc     Increase RFGSM_RX_START_MAX_TASKS_TASKQ_B from 5 to 6
06/11/15   tws     Increase therm_read transactions to 30 for WTR2955
06/09/15   piy     Increase RFGSM_RX_WAKEUP_MAX_NON_SSBI_TRANSACTIONS_TASKQ_A
                   to Enable Prx + Drx Wakeup
05/28/15   sc      Add unique interface index framework
05/13/15   tws     Mainline FEATURE_BOLT_MODEM
05/09/15   sc      Split Rx and Tx wakeup event memory
04/22/15   zbz     Increase max number of transactions for Tx/Rx wake up event
04/08/15   sc      Overlay Rx/Tx wakeup events with init event
04/01/15   sc      Add Rx/Tx wakeup CCS processing functionality
03/31/15   sc      Add Rx/Tx Wakeup event memory
03/31/15   ec      Increase Enter Mode Non-SSBI size and zero SSBI allowances
...
/* Unique interface data indexes */
/* Most events use a circular buffer of event indexes although in some cases */
/* events need to be retained for longer than the time it takes to wrap      */
/* around and therefore dedicated indexes are used that do not use the normal*/
/* index allocation method.                                                  */
typedef enum
{
  RFLM_GSM_DM_UNIQUE_THERM_READ_INDEX_SUB_1,
  RFLM_GSM_DM_UNIQUE_THERM_READ_INDEX_SUB_2,
  RFLM_GSM_DM_MAX_UNIQUE_INDEXES
}rflm_gsm_dm_unique_intf_data_idx;

#define RFLM_GSM_DM_THERM_READ_INTF_IDX_SUB_1 \
          ( RFLM_GSM_DM_MAX_INTF_DATA_ENTRIES + RFLM_GSM_DM_UNIQUE_THERM_READ_INDEX_SUB_1 )
#define RFLM_GSM_DM_THERM_READ_INTF_IDX_SUB_2 \
          ( RFLM_GSM_DM_MAX_INTF_DATA_ENTRIES + RFLM_GSM_DM_UNIQUE_THERM_READ_INDEX_SUB_2 )
...
#define RFGSM_INIT_MAX_NON_SSBI_TRANSACTIONS_TASKQ_A 281
...
/* Rx Wakeup */
#define RFGSM_RX_WAKEUP_MAX_TASKS_TASKQ_A 12
#define RFGSM_RX_WAKEUP_MAX_TASKS_TASKQ_B 1
#define RFGSM_RX_WAKEUP_MAX_SSBI_TRANSACTIONS_TASKQ_A     0
#define RFGSM_RX_WAKEUP_MAX_NON_SSBI_TRANSACTIONS_TASKQ_A 220
#define RFGSM_RX_WAKEUP_MAX_SSBI_TRANSACTIONS_TASKQ_B     0
#define RFGSM_RX_WAKEUP_MAX_NON_SSBI_TRANSACTIONS_TASKQ_B 1
#define RFGSM_RX_WAKEUP_TASK_A_PAYLOAD_SIZE \
        MAX_TRANS( (RFGSM_NON_SSBI_PAYLOAD_SIZE_32(RFGSM_RX_WAKEUP_MAX_NON_SSBI_TRANSACTIONS_TASKQ_A) + RFGSM_SSBI_PAYLOAD_SIZE_32(0, RFGSM_RX_WAKEUP_MAX_TASKS_TASKQ_A)), \
        (RFGSM_NON_SSBI_PAYLOAD_SIZE_32(RFGSM_MAX_NON_SSBI_ON_SSBI_SUPPORTED_DEVICE) + RFGSM_SSBI_PAYLOAD_SIZE_32(RFGSM_RX_WAKEUP_MAX_SSBI_TRANSACTIONS_TASKQ_A, RFGSM_RX_WAKEUP_MAX_TASKS_TASKQ_A)) )

#define RFGSM_RX_WAKEUP_TASK_B_PAYLOAD_SIZE \
                  RFGSM_NON_SSBI_PAYLOAD_SIZE_32(RFGSM_RX_WAKEUP_MAX_NON_SSBI_TRANSACTIONS_TASKQ_B)

RFGSM_CCS_EVENT_TYPE(rflm_gsm_ccs_rx_wakeup_task_q_a, RFGSM_RX_WAKEUP_MAX_TASKS_TASKQ_A, RFGSM_RX_WAKEUP_TASK_A_PAYLOAD_SIZE);
RFGSM_CCS_EVENT_TYPE(rflm_gsm_ccs_rx_wakeup_task_q_b, RFGSM_RX_WAKEUP_MAX_TASKS_TASKQ_B, RFGSM_RX_WAKEUP_TASK_B_PAYLOAD_SIZE);

/* Tx Wakeup */
#define RFGSM_TX_WAKEUP_MAX_TASKS_TASKQ_A 3
#define RFGSM_TX_WAKEUP_MAX_TASKS_TASKQ_B 1
#define RFGSM_TX_WAKEUP_MAX_SSBI_TRANSACTIONS_TASKQ_A     0
#define RFGSM_TX_WAKEUP_MAX_NON_SSBI_TRANSACTIONS_TASKQ_A 150
#define RFGSM_TX_WAKEUP_MAX_SSBI_TRANSACTIONS_TASKQ_B     0
#define RFGSM_TX_WAKEUP_MAX_NON_SSBI_TRANSACTIONS_TASKQ_B 1
#define RFGSM_TX_WAKEUP_TASK_A_PAYLOAD_SIZE \
        MAX_TRANS( (RFGSM_NON_SSBI_PAYLOAD_SIZE_32(RFGSM_TX_WAKEUP_MAX_NON_SSBI_TRANSACTIONS_TASKQ_A) + RFGSM_SSBI_PAYLOAD_SIZE_32(0, RFGSM_TX_WAKEUP_MAX_TASKS_TASKQ_A)), \
        (RFGSM_NON_SSBI_PAYLOAD_SIZE_32(RFGSM_MAX_NON_SSBI_ON_SSBI_SUPPORTED_DEVICE) + RFGSM_SSBI_PAYLOAD_SIZE_32(RFGSM_TX_WAKEUP_MAX_SSBI_TRANSACTIONS_TASKQ_A, RFGSM_TX_WAKEUP_MAX_TASKS_TASKQ_A)) )

#define RFGSM_TX_WAKEUP_TASK_B_PAYLOAD_SIZE \
                  RFGSM_NON_SSBI_PAYLOAD_SIZE_32(RFGSM_TX_WAKEUP_MAX_NON_SSBI_TRANSACTIONS_TASKQ_B)

RFGSM_CCS_EVENT_TYPE(rflm_gsm_ccs_tx_wakeup_task_q_a, RFGSM_TX_WAKEUP_MAX_TASKS_TASKQ_A, RFGSM_TX_WAKEUP_TASK_A_PAYLOAD_SIZE);
RFGSM_CCS_EVENT_TYPE(rflm_gsm_ccs_tx_wakeup_task_q_b, RFGSM_TX_WAKEUP_MAX_TASKS_TASKQ_B, RFGSM_TX_WAKEUP_TASK_B_PAYLOAD_SIZE);
...
#define RFGSM_TX_BAND_MAX_NON_SSBI_TRANSACTIONS_TASKQ_A 200
#else
#define RFGSM_TX_BAND_MAX_NON_SSBI_TRANSACTIONS_TASKQ_A 200
#endif
...
#define RFGSM_FRONTEND_CONFIG_MAX_TASKS_TASKQ_B 15
...
#define RFGSM_FRONTEND_CONFIG_MAX_NON_SSBI_TRANSACTIONS_TASKQ_B 28
...
#define RFGSM_TX_CAL_MAX_SSBI_TRANSACTIONS_TASKQ_A     0
...
#define RFGSM_RX_START_MAX_TASKS_TASKQ_B 8
#define RFGSM_RX_START_MAX_SSBI_TRANSACTIONS_TASKQ_A     0
#define RFGSM_RX_START_MAX_NON_SSBI_TRANSACTIONS_TASKQ_A 58
#define RFGSM_RX_START_MAX_SSBI_TRANSACTIONS_TASKQ_B     0
#define RFGSM_RX_START_MAX_NON_SSBI_TRANSACTIONS_TASKQ_B 20
...
#define RFGSM_RX_STOP_MAX_TASKS_TASKQ_B 8
#define RFGSM_RX_STOP_MAX_SSBI_TRANSACTIONS_TASKQ_A     0
#define RFGSM_RX_STOP_MAX_NON_SSBI_TRANSACTIONS_TASKQ_A 5
#define RFGSM_RX_STOP_MAX_SSBI_TRANSACTIONS_TASKQ_B     0
#define RFGSM_RX_STOP_MAX_NON_SSBI_TRANSACTIONS_TASKQ_B 12
...
#define RFGSM_TX_START_MAX_TASKS_TASKQ_B 12
...
#define RFGSM_TX_START_MAX_NON_SSBI_TRANSACTIONS_TASKQ_B 30
...
#define RFGSM_TX_TRANS_MAX_TASKS_TASKQ_B 5
...
#define RFGSM_TX_STOP_MAX_TASKS_TASKQ_B 8
#define RFGSM_TX_STOP_MAX_SSBI_TRANSACTIONS_TASKQ_A     5
#define RFGSM_TX_STOP_MAX_NON_SSBI_TRANSACTIONS_TASKQ_A 10
#define RFGSM_TX_STOP_MAX_SSBI_TRANSACTIONS_TASKQ_B     5
...
/* Coupler switch */
#define RFGSM_COUPLER_MAX_TASKS_TASKQ_B 2
#define RFGSM_COUPLER_MAX_SSBI_TRANSACTIONS_TASKQ_B     0
#define RFGSM_COUPLER_MAX_NON_SSBI_TRANSACTIONS_TASKQ_B 20
#define RFGSM_COUPLER_TASK_B_PAYLOAD_SIZE \
                  RFGSM_NON_SSBI_PAYLOAD_SIZE_32(RFGSM_COUPLER_MAX_NON_SSBI_TRANSACTIONS_TASKQ_B) + \
                  RFGSM_SSBI_PAYLOAD_SIZE_32(RFGSM_COUPLER_MAX_SSBI_TRANSACTIONS_TASKQ_B, RFGSM_COUPLER_MAX_TASKS_TASKQ_B)

RFGSM_CCS_EVENT_TYPE(rflm_gsm_ccs_coupler_task_q_b, RFGSM_COUPLER_MAX_TASKS_TASKQ_B, RFGSM_COUPLER_TASK_B_PAYLOAD_SIZE);
...
/* IRAT Trigger script */
#define RFGSM_MEAS_TRIGGER_MAX_SSBI_TRANSACTIONS_TASKQ_A 0
...
typedef struct
{
  rflm_gsm_ccs_rx_wakeup_task_q_a task_q_a;
  rflm_gsm_ccs_rx_wakeup_task_q_b task_q_b;
}rflm_gsm_rx_wakeup_data;

/* Tx Wakeup */
typedef struct
{
  rflm_gsm_ccs_tx_wakeup_task_q_a task_q_a;
  rflm_gsm_ccs_tx_wakeup_task_q_b task_q_b;
}rflm_gsm_tx_wakeup_data;
...
typedef enum
{ 
  RFLM_GSM_VSWR_MEAS_DISABLED,  
  RFLM_GSM_VSWR_MEAS_SLEEP,  
  RFLM_GSM_VSWR_MEAS_IDLE,
  RFLM_GSM_VSWR_MEAS_REQUESTED,      /*! @brief VSWR meas requested by tuner */
  RFLM_GSM_VSWR_MEAS_IN_PROGRESS,    /*! @brief VSWR meas this frame */
  RFLM_GSM_VSWR_INIT_FAILED,
  RFLM_GSM_VSWR_MEAS_MAX_STATE
} rflm_gsm_vswr_meas_state_type;

typedef struct
{ 
  uint32                        fbrx_buf_index;
  rflm_gsm_vswr_meas_state_type vswr_meas_state;
  uint16                        vswr_meas_duration_qs;
  uint16                        fbrx_ldo_settle_time_qs;
  boolean                       debug_disable_coupler_switch;
} rflm_gsm_fbrx_data_type;
...
typedef struct
{
  ...
  rflm_time_profile_meas_s rx_wakeup_prof_data;
  rflm_time_profile_meas_s tx_wakeup_prof_data;
  ...
  uint32 rx_wakeup_prof_data;
  uint32 tx_wakeup_prof_data;
...
typedef struct
{
  rflm_gsm_rx_wakeup_data        rfgsm_rx_wakeup_event;
  rflm_gsm_tx_wakeup_data        rfgsm_tx_wakeup_event;
  ...
  rflm_gsm_fbrx_data_type         rfgsm_fbrx_data;
}rflm_gsm_data_type;
...
typedef struct
{
  ...
  rflm_gsm_intf_data intf_data[RFLM_GSM_DM_MAX_INTF_DATA_ENTRIES + RFLM_GSM_DM_MAX_UNIQUE_INDEXES];
}rflm_gsm_intf;
...
void rflm_gsm_create_mutex( qurt_mutex_t *mutex );

/*---------------------------------------------------------------------------*/

void rflm_gsm_delete_mutex( qurt_mutex_t *mutex );

/*---------------------------------------------------------------------------*/

void rflm_gsm_lock_mutex( qurt_mutex_t *mutex );

/*---------------------------------------------------------------------------*/

void rflm_gsm_unlock_mutex( qurt_mutex_t *mutex );
...
rflm_gsm_data_type** rflm_gsm_dm_get_table_ptr_address( void );
