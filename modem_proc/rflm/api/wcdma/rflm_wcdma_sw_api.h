...
$Header: //components/rel/rflm.mpss/2.11/api/wcdma/rflm_wcdma_sw_api.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/13/17   sk      Max clock change to 844MHz
07/14/16   vs      Fix issue with TxAGC logging by increasing buffer to 4
06/07/16   ag      Changes as per new waveform  format
...
03/15/06   bn      Adaptive PA comp feature for WCDMA
02/02/06   nv      Enabling TXAGC fixup skip counter
12/10/15   ag      Add structure to hold extra capture time for autopin
11/18/15   asn     Add support to check if RACH or DCH
11/16/15   dw      Change cm_init to int16
11/10/15   ljz     Skip LDO on\off during selftest
11/05/15   ag      Added configs for Autopin
06/02/15   gh/zhh  Added therm_enable configs to rflm_wcdma_tx_dm_buf_type
                   Added fields for keeping track of timing
09/29/15   sd      FR30081 AsDiv with eLNA gain compensation 
09/26/15   bn      New variables for TxAGC timeline optimization
08/25/15   aa      Add input parameter for FBRX gain parameter, persist TPC size, store LS filter range
08/05/15   ac      prefetch changes for timeline optimisation
07/28/15   yb      FBRx VSWR support
07/20/15   nv      Set the TXAGC setpoint variable differently for different mode based on
                   flag from RFSW.
07/24/15   vs      Add phase info to TxAGC static LUT
02/04/15   sd      Support for GRFC elna
06/12/15   aa      AutoPin update linearizer api
06/04/15   rmb     Support for querying correct TQ buffer index maintained by RFWCDMA in CCS
                   for IRAT and standalone scenarios.
06/05/15   bn      0x1842 logging fixes
06/02/15   gh/zhh  Added therm_enable configs to rflm_wcdma_tx_dm_buf_type
04/30/15   ac      add 2nd buffer for tq for w plus w feature
03/20/15   rmb     Combine W2W, G2W and L2W neighbour enum to one.
02/27/15   cd      TxAGC updates for max power override support
02/11/15   dw      Change some of the CCS function prototype
...
#define RFLM_WCDMA_TXAGC_BUFFER_COUNT  8

#define RFLM_Q6_MAX_CPU_SPEED_KHZ 844800
...
/*! 
    ...
    @details
    This enum is primarily used to differentiate between home WCDMA
    and neighbour WCDMA measurement
*/
typedef enum
{
  /*! WCDMA Standalone or WCDMA Home*/
  RFLM_WCDMA_HOME_MEAS,

  /*! WCDMA Measurement as a Neighbour */
  RFLM_WCDMA_NEIGHBOUR_MEAS,
...
typedef enum
{
  RFLM_WCDMA_LNA_WTR_TYPE,

  RFLM_WCDMA_LNA_EXT_TYPE,

  RFLM_WCDMA_LNA_MAX_TYPE
} rflm_wcdma_lna_enum_type;
...
typedef struct
{
  ...
  int16    cm_init[RFLM_WDMA_MAX_NUM_LNA_STATES];
...
{
  /*! LPM config*/  
  rflm_wcdma_device_script_type drx[RFLM_WCDMA_DRX_CTL_NUM][RFLM_WCDMA_LNA_MAX_TYPE];
   
  /*! LNA config*/     
  rflm_wcdma_device_script_type lna[RFLM_WDMA_MAX_NUM_LNA_STATES][RFLM_WCDMA_LNA_MAX_TYPE];
  ...
  /*! Num of CCS tasks for each LNA state */
  uint8 num_ccs_tasks[RFLM_WDMA_MAX_NUM_LNA_STATES];
...
typedef struct
{
  ...
  /*device pair index to associate to the handle*/
  uint8 device_pair;
...
/*! 
  @brief
  Structure definition of IRAT related paramters in build scripts routine
...
}rflm_wcdma_meas_build_script_cfg_type;

/*----------------------------------------------------------------------------*/
/*! 
  @brief
  Structure definition of IRAT related paramters in Meas Enter routine
 
*/
typedef struct
{
  /*! RFM device that is used to reference correct TQ pair for RxAGC and LNA events 
    in X2W IRAT for Neighbour. This can be source tech or target tech RFM device */
  /* Remember this may not be that RFM device that is passed by L1 in Measurement Enter
  but RFM device that can be mapped to correct CCS TQ that can be used */
  uint8 device_ccs_tq;

  /*! Device pair index that maps to TQ pair index to be used for RxAGC and LNA events 
    in X2W IRAT for Neighbour  */
  uint8 device_pair_idx_ccs_tq;

}rflm_wcdma_meas_enter_cfg_type;

/*----------------------------------------------------------------------------*/
/*! 
  @brief
  Structure definition of IRAT measurement config parameters
 
*/
typedef struct
{
  /*! WCDMA Measurement related paramters in Build scripts */ 
  rflm_wcdma_meas_build_script_cfg_type build_script_config;

  /*! WCDMA Measurement related paramters in Meas enter */ 
  rflm_wcdma_meas_enter_cfg_type meas_enter_config;
...
typedef struct
{
  ...
  boolean ftm_selftest_running;

  /*! Variable to store max RACH power limit */
  int16 max_rach_pow_limit;

  /*! Variable to store max DCH power limit */
  int16 max_dch_pow_limit;
...
typedef struct
{
  ...
  /*! Tx AGC Offset for the FR 30081*/
  int8 asdiv_txagc_offset;
...
typedef struct
{
  ...
  /*! FBRx Coupler FWD event */
  rflm_wcdma_device_script_type coupler_fwd;

  /*! FBRx Coupler REV event */
  rflm_wcdma_device_script_type coupler_rev;
...
typedef struct
{
  rflm_tx_multi_carrier_cfg_type tx_state;
...
typedef struct
{
  uint8 waveform;                   ///< Waveform

  uint8 waveform_type;             ///< Waveform type

  uint32 tx_freq;                  ///Current Tx frequency
  
  boolean is_rach;                       ///< Value says if it is RACH or DCH
   
  boolean is_cal_mode;              ///<Value indicates if in CAL mode
  
  uint8 hs_offset;                  ///< HS offset
  ...
  uint32  digitial_gain_skip_count;  ///< wfw rflm digital gain skip counter


  uint32  txagc_update_fixup_skip_count;   ///< rflm txagc fixup skip counter

  boolean last_txagc_update_fixup_skipped; ///< Flag indicating that Last TxAGC was skipped
...
typedef struct 
{
  ...
  uint8  hs_offset;    ///<HS offset
  uint8  bpg;          ///<BPG
  boolean is_rach;     ///< Tells if RACH or DCH
  uint8  pa_state;     ///< pa_state
  uint8  pa_action;    ///< pa_action
  int16  out_pwr_raw;  ///<expected output power from FW in update
  int16  out_pwr;      ///< final output power
  int16  exp_out_pwr;  ///< expected output power programmed in calc
  uint8  calc_fbrx_gs; ///< calc_fbrx gains state
  boolean calc_txagc_in_pa_swpt;  /// calc txagc fixup is in PA switch point
  ...
  rflm_ustmr_t stableproc_start;      ///< fed_stableproc_start   
  rflm_ustmr_t stableproc_end;        ///< fed_stableproc_end
 
  rflm_ustmr_t cleanup_start;         ///< fed_cleanup_start 
  rflm_ustmr_t cleanup_end;           ///< fed_cleanup_start 
  
  rflm_ustmr_t callback;   ///< fed_txagc_callback
  ...
  uint32  digital_gain_skip_count; ///< Digital Gain Skip count;
  uint8   digital_gain_sync_block; ///< Digital gain sync block passed to FW
  boolean digital_gain_skip;
  uint32 reserved;
...
typedef enum
{
  RFLM_W_PA_COMP_UP,
  RFLM_W_PA_COMP_DOWN,
  RFLM_W_PA_COMP_INVALID
} rflm_pa_comp_update_cfg_t;


typedef struct
{
  boolean txagc_in_pa_swpt;
  rflm_pa_comp_update_cfg_t pa_comp_item;
  rflm_txagc_pa_state_data_t pa_idx_to_update;
  int8 pa_comp_up[RFLM_WCDMA_MAX_NUM_PA_STATES];
  int8 pa_comp_down[RFLM_WCDMA_MAX_NUM_PA_STATES];
  uint8 up_ubit;
  uint8 down_ubit;
  uint8 fbrx_gain_state;
  uint32 tx_freq_at_last_update;
  
}rflm_wcdma_tx_adaptive_pa_comp_type;


#define RFLM_W_TX_LOG_BUFFER_SIZE 4
typedef struct
{
  /*! Tx band config */ 
  ...
  /*! FBRx gain parameter */
  rflm_fbrx_get_exp_gain_in_t fbrx_gain_param_in;  

  /*! Adaptive PA comp */
  rflm_wcdma_tx_adaptive_pa_comp_type adaptive_pa_comp;
  
  /*! Store the LS filter range:
  TODO: Need to check whether it is possbile to remove this flag*/
  boolean txagc_in_ls_err_filter_range;
...
/*----------------------------------------------------------------------------*/
/*@ brief 
  Allocate TQ pair given device. For the device pair it shall be only allocated
  once and share the same TQ pair
 
  @detail
  The API will allocate TQ pair given device
  ...
  @param device_pair
  Device pair ID
  ToDo: device pair ID should be uses when we need to double buffer the CCS TQ
  allocation based on device pair 
  
  
  @return pass or failure
*/
boolean rflm_wcdma_allocate_ccs_tq_pair
( uint8 device, uint8 device_pair);
/*----------------------------------------------------------------------------*/
/*@ brief 
  Allocate TQ pair given device. For the device pair it shall be only allocated
  once and share the same TQ pair
  ...
  @return pass or failure
*/
uint8 rflm_wcdma_map_ccs_tq_pair_handle(rflm_handle_rx_t handle_id);
/*----------------------------------------------------------------------------*/
...
/*!
  @brief
  This API will update Pin and linearizer for W

  @details
*/
void
rflm_wcdma_update_pin(rflm_handle_tx_t handle_id, uint8 pa_state, rflm_db10_t pin);

/*----------------------------------------------------------------------------*/
/*!
  @brief
  This API will post the VSWR measurements to tuner framework

  @details
*/

void rflm_wcdma_post_process_fbrx_vswr_measurements
(
