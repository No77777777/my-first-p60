...
$Header: //components/rel/rflm.mpss/2.11/api/wcdma/rflm_wcdma_msg.h#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------------------------------
11/05/15   ag      Added UMID and associated cmd interface for Autopin  
03/12/15   gh/zhh  Added timing info to METER cmd payload
                   Added UMID and associated cmd interface
06/12/15   aa      AutoPin MSGR command
...
} rflm_wcdma_npa_issue_payload_type;
...
typedef struct 
{
  ...
  rflm_wcdma_npa_issue_payload_type msg_payload; 

} rflm_wcdma_npa_issue_cmd_t;

enum 
{
  MSGR_DEFINE_UMID( RFLM, WCDMA, CMD, NPA_ISSUE, 0x02, rflm_wcdma_npa_issue_cmd_t )
...
} rflm_wcdma_npa_cancel_payload_type;
...
typedef struct 
{
  ...
  rflm_wcdma_npa_cancel_payload_type msg_payload; 

} rflm_wcdma_npa_cancel_cmd_t;

enum 
{
  MSGR_DEFINE_UMID( RFLM, WCDMA, CMD, NPA_CANCEL, 0x03, rflm_wcdma_npa_cancel_cmd_t )
  
};

/*! Structure to define the payload for  PROCESS AUTO PIN */
typedef struct 
{
  rflm_handle_tx_t handle_id; 
  uint8 pa_state;
  int16 pin_error;
} rflm_wcdma_process_auto_pin_payload_type;


/*! Structure to define PROCESS AUTO PIN*/
typedef struct 
{
  msgr_hdr_s msg_hdr;
  /*!< Message router header */

  rflm_wcdma_process_auto_pin_payload_type msg_payload; 
  /*!< Payload for process auto pin command */
} rflm_wcdma_process_auto_pin_cmd_t;

enum 
{
  MSGR_DEFINE_UMID( RFLM, WCDMA, CMD, PROCESS_AUTO_PIN , 0x04, rflm_wcdma_process_auto_pin_cmd_t )
  /*!< UMID defining the TXAGC LOG PKT command */ 
};
...
enum 
{
  MSGR_DEFINE_UMID( RFLM, WCDMA, CMD,  TRIGGER_AUTO_PIN_READ, 0x05, rflm_wcdma_trigger_par_read_cmd_t )
