...
Copyright (c) 2013-2020  by Qualcomm Technologies, Inc. (QTI).  All Rights Reserved.
===========================================================================*/
...
$Header: //components/rel/rflm.mpss/2.11/api/txagc/rflm_txagc_api.h#1 $

when       who    what, where, why
--------   ---    ------------------------------------------------------------- 
03/24/20   pvsr    Increase max number of TxAGC buffers to 6
03/09/16   zhh     Save the DPD index during txagc_lookup for 1 shot autoPin
...
03/03/15   al     Increase RFLM_TXAGC_MAX_TASK_NUM
...
#define RFLM_TXAGC_MAX_TASK_NUM                  RFLM_CCS_RF_TASKQ_NUM_ITEMS-1
...
typedef enum
{
  ...
  RFLM_TXAGC_SET_PA_NON_TRIG_TASK,
  ...
  /*! @brief  ASM trigger task */
  RFLM_TXAGC_ASM_TRIG_TASK,
...
typedef enum
{
  ...
  RFLM_TXAGC_OFF_TO_TX,

  /*! @brief  not using this sequnec now */
  RFLM_TXAGC_BUFFER_2,
  
  /*! @brief  max type */
  RFLM_TXAGC_SEQ_MAX_TYPE
  
}rflm_txagc_seq_type_t;
...
typedef enum
{
  ...
  /*! @brief OFF_to_TX from Lo to Hi */  
  RFLM_TXAGC_FIXUP_Lo_2_hi,
...
typedef struct
{
  ...
  */
  ...
  /*! @brief  ccs transaction type list for all five scenarios */
...
typedef struct ALIGN(8)
{
  ...
  /*! @brief pout adjust table */
  int16 per_cc_pout_adj[RFLM_TXAGC_XPT_NUM_SLOT]
                [RFLM_TXAGC_XPT_NUM_SF]
                [RFLM_TXAGC_XPT_NUM_FREQ_COMP];    

  /*! @brief pout adjust table */
  int16 per_cc_pin_adj[RFLM_TXAGC_XPT_NUM_SLOT]
               [RFLM_TXAGC_XPT_NUM_SF];   
...
/*! @detail
  ...
  based on dm_handle_id (or txlm_handle_id and sequence_id)
  which are:
