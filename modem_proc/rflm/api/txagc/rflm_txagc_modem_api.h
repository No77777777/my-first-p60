...
$Header: //components/rel/rflm.mpss/2.11/api/txagc/rflm_txagc_modem_api.h#1 $

when       who    what, where, why
--------   ---    -------------------------------------------------------------
08/11/16   ndb     Added TxLM handle in rflm_txagc_load_dpd_cfg_t
01/04/16   am      Code changes to return mask to indicates what all CA IQ 
                   gain registers are programmed for given FED API Call.
12/01/15   rp      Code changes to apply proper CA IQ Gain for UpPTS SRS cases.
...
04/01/15    JJ      Latch DTR chain0 after chain1 to bypass DTR1 latch HW limitation
...
/*! 
   @brief
   This API will set ca iq gain in mixer
   ...
   @param slot idx
   Slot idex used to determine the set of CA IQ gain registers to be used.
...
*/
rflm_err_t
rflm_txagc_modem_set_ca_iq_gain
(
  uint32 iq_gain,
  uint32 nominal_iq_gain,
  uint32 handle_id,
  uint32 cell_index,
  uint8 drif_group_idx,
  uint8 drif_set_idx,
  rflm_lte_slot_start_timings_type_s  slot_idx,
  uint32 slot_timing,
