...
$Header: //components/rel/rflm.mpss/2.11/services/dm/inc/rflm_dm_buf_mgr_t.h#1 $
...
inline boolean MyList<MAX_NUM_ITEM>::is_idx_used(int32 idx)
{
  ...
  uint32 num_used_item = MAX_NUM_ITEM-mNumFreeItems;

  /* Loop through the Used list to check if the idx is currently in use */
  for(i=0; i<num_used_item;i++) 
  {
    if(mUsed[i]==idx) { break;}
  }

  /* If not found, return FALSE */
  if(i==num_used_item) { rv=FALSE; }
  return rv;
}
