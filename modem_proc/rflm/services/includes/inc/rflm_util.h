...
$Header: //components/rel/rflm.mpss/2.11/services/includes/inc/rflm_util.h#1 $

when         who     what, where, why
----------   ---     --------------------------------------------------------
02/23/16     sbo     Rename rflm math utility functions.
2015-02-03   jc      Removed unused deprecated macros
...
/*! @brief Generate a constant multiplicative inverse.
    ...
    To perform a divide, use the result with the rflm_mult_inv() function.
    Note than when den==1, result=0. This is interpreted appropriately
    by the rflm_mult_inv() function.
*/
...
*/
#define div_const(num, den) rflm_mult_inv(num, CONST_MULT_INV(den))
...
/* ------------------------------------------------------------------------- */
int32 rflm_lte_aprox_ulin_to_dB_linlut8(uint32 input, int32 Qi);
/* ------------------------------------------------------------------------- */
uint32 rflm_lte_approx_dB_to_ulin_linlut8(int32 input);
/* ------------------------------------------------------------------------- */
STATIC_INLINE uint32 rflm_mult_inv(uint32 num, uint32 inv) {
...
*/
STATIC_INLINE int32 rflm_mod_wrap(int32 x, int32 mod) {
...
/*! @brief Wrap the input x to be between [0,mod-1]. 
    ...
    DO NOT USE FOR POINTERS. Addresses >= 0x80000000 will not give correct results.
*/
STATIC_INLINE int32 rflm_mod_wrap_high(int32 x, int32 mod) {
  return rflm_mod_wrap(x,mod);
}
...
/*! @brief Wrap the input x to be between [0,mod-1].
    ...
    DO NOT USE FOR POINTERS. Addresses >= 0x80000000 will not give correct results.
*/
STATIC_INLINE int32 rflm_mod_wrap_low(int32 x, int32 mod) {
  return rflm_mod_wrap(x,mod);
}
...
/*! @brief Wrap the input x to be from [-mod/2,(mod+1)/2-1]. 
    ...
    DO NOT USE FOR POINTERS. Addresses >= 0x80000000 will not give correct results.
*/
STATIC_INLINE int32 rflm_mod_wrap_signed(int32 x, int32 mod) {
  if (x >= ((mod+1)/2))
  {
    x -= mod; 
  } else if (x < -(mod/2))
...
/*! @brief 16-bit addition, rflm_saturate at 16-bits. */
STATIC_INLINE int16 rflm_add_sath(int16 x, int16 y) { 
  return (int16)Q6_R_add_RlRl_sat(x,y); 
}

/*! @brief 32-bit addition, rflm_saturate at 32-bits. */
STATIC_INLINE int32 rflm_add_satw(int32 x, int32 y) { 
  return Q6_R_add_RR_sat(x,y); 
}

/*! @brief 16-bit subtraction, rflm_saturate at 16-bits. */
STATIC_INLINE int16 rflm_sub_sath(int16 x, int16 y) { 
  return (int16)Q6_R_sub_RlRl_sat(x,y); 
}

/*! @brief 32-bit subtraction, rflm_saturate at 32-bits. */
STATIC_INLINE int32 rflm_sub_satw(int32 x, int32 y) { 
  return Q6_R_sub_RR_sat(x,y); 
}

/*! @brief 32-bit negate, rflm_saturate at 32-bits. */
STATIC_INLINE int32 rflm_neg_satw(int32 x) { 
  return Q6_R_neg_R_sat(x); 
}

/*! @brief 32-bit absolute value, rflm_saturate at 32-bits. */
STATIC_INLINE int32 rflm_abs_satw(int32 x) { 
  return Q6_R_abs_R_sat(x); 
}

/*! @brief 32-bit signed maximum. */
STATIC_INLINE int32 rflm_maxw(int32 x, int32 y) { 
  return Q6_R_max_RR(x,y); 
}

/*! @brief 32-bit unsigned maximum. */
STATIC_INLINE uint32 rflm_maxuw(uint32 x, uint32 y) { 
  return Q6_R_maxu_RR(x,y); 
}

/*! @brief 64-bit signed maximum. */
STATIC_INLINE int64 rflm_maxd(int64 x, int64 y) { 
  return Q6_P_max_PP(x,y); 
}

/*! @brief 64-bit unsigned maximum. */
STATIC_INLINE uint64 rflm_maxud(uint64 x, uint64 y) { 
  return Q6_P_maxu_PP(x,y); 
}

/*! @brief 32-bit signed minimum. */
STATIC_INLINE int32 rflm_minw(int32 x, int32 y) { 
  return Q6_R_min_RR(x,y); 
}

/*! @brief 32-bit unsigned minimum. */
STATIC_INLINE uint32 rflm_minuw(uint32 x, uint32 y) { 
  return Q6_R_minu_RR(x,y); 
}

/*! @brief 64-bit signed minimum. */
STATIC_INLINE int64 rflm_mind(int64 x, int64 y) { 
  return Q6_P_min_PP(x,y); 
}

/*! @brief 64-bit unsigned minimum. */
STATIC_INLINE uint64 rflm_minud(uint64 x, uint64 y) { 
  return Q6_P_minu_PP(x,y); 
}

/*! @brief Count number of leading 0-bits in a 32-bit word. */
STATIC_INLINE int rflm_cl0w(int32 x) { 
  return Q6_R_cl0_R(x); 
}

/*! @brief Count number of leading 0-bits in a 64-bit word. */
STATIC_INLINE int rflm_cl0d(int64 x) { 
  return Q6_R_cl0_P(x); 
}

/*! @brief Count number of leading 1-bits in a 32-bit word. */
STATIC_INLINE int rflm_cl1w(int32 x) { 
  return Q6_R_cl1_R(x); 
}

/*! @brief Count number of leading 1-bits in a 64-bit word. */
STATIC_INLINE int rflm_cl1d(int64 x) { 
  return Q6_R_cl1_P(x); 
}

/*! @brief Count number of leading same-bits in a 32-bit word. */
STATIC_INLINE int rflm_clbw(int32 x) { 
  return Q6_R_clb_R(x); 
}

/*! @brief Count number of leading same-bits in a 64-bit word. */
STATIC_INLINE int rflm_clbd(int64 x) { 
  return Q6_R_clb_P(x); 
}

/*! @brief Count number of trailing 0-bits in a 32-bit word. */
STATIC_INLINE int rflm_ct0w(uint32 x) { 
  return Q6_R_ct0_R(x); 
}
 
/*! @brief Count number of trailing 0-bits in a 64-bit word. */
STATIC_INLINE int rflm_ct0d(uint64 x) { 
  return Q6_R_ct0_P(x); 
}
 
/*! @brief Count number of trailing 1-bits in a 32-bit word. */
STATIC_INLINE int rflm_ct1w(uint32 x) { 
  return Q6_R_ct1_R(x); 
}

/*! @brief Count number of trailing 1-bits in a 64-bit word. */
STATIC_INLINE int rflm_ct1d(uint64 x) { 
  return Q6_R_ct1_P(x); 
}

/*! @brief Clear bit "b" in 32-bit word "x" and return the updated "x". */
STATIC_INLINE int32 rflm_clrbitw(int32 x, int b) { 
  return Q6_R_clrbit_RR(x,b); 
}

/*! @brief Set bit "b" in 32-bit word "x" and return the updated "x". */
STATIC_INLINE int32 rflm_setbitw(int32 x, int b) { 
  return Q6_R_setbit_RR(x,b); 
}

/*! @brief Toggle bit "b" in 32-bit word "x" and return the updated "x". */
STATIC_INLINE int32 rflm_togglebitw(int32 x, int b) { 
...
STATIC_INLINE uint32 rflm_fextractuw(uint32 x, int width, int offset) {
  vect64 f;
  f.w[1] = width;
  f.w[0] = offset;
...
/*! @brief Extract a bitfield from 64-bit x and return LSb aligned.
    "width" bits starting from bit "offset". */
STATIC_INLINE uint64 rflm_fextractud(uint64 x, int width, int offset) {
  vect64 f;
  f.w[1] = width;
  f.w[0] = offset;
...
STATIC_INLINE uint32 rflm_finsertuw(uint32 dest, uint32 src, int width, int offset) {
  vect64 f;
  f.w[1] = width;
  f.w[0] = offset;
...
/*! @brief Insert a bitfield from "src" into "dest", both 64-bit.
    Take "width" LSb from "src", insert into "dest" starting at "offset".
    Returns updated "dest". */
STATIC_INLINE uint64 rflm_finsertud(uint64 dest, uint64 src, int width, int offset) {
  vect64 f;
  f.w[1] = width;
  f.w[0] = offset;
...
#define sat16 rflm_sath
STATIC_INLINE int16 rflm_sath(int32 x) {
  return (int16)Q6_R_sath_R(x); 
}

/*! @brief Saturate 32-bit signed value to unsigned 16-bits. */
#define satu16 rflm_satuh
STATIC_INLINE uint16 rflm_satuh(int32 x) {
  return (int16)Q6_R_satuh_R(x); 
}

/*! @brief Saturate 64-bit signed value to signed 32-bits. */
#define sat32 rflm_satw
STATIC_INLINE int32 rflm_satw(int64 x) {
  return Q6_R_sat_P(x); 
}

/*! @brief Saturate 32-bit signed value to arbitrary width.
    Width includes the sign bit, i.e. rflm_saturate(x,8) returns [-128,127].
    Don't use width == 0. 
*/
STATIC_INLINE int32 rflm_saturate(int32 x, int bits) {
  bits = 32 - bits;
...
/*! @brief 32-bit signed shift. Positive "s" values shift left, negative 
    shifts right. */
STATIC_INLINE int32 rflm_shiftw(int32 x, int s) {
  return Q6_R_asl_RR(x,s); 
}

/*! @brief 32-bit signed shift, rflm_saturate to 32 bits. Positive "s" values 
    shift left, negative shifts right. */
STATIC_INLINE int32 rflm_shift_satw(int32 x, int s) {
  return Q6_R_asl_RR_sat(x,s); 
}

/*! @brief 64-bit signed shift. Positive "s" values shift left, negative
    shifts right. */
STATIC_INLINE int64 rflm_shiftd(int64 x, int s) {
  return Q6_P_asl_PR(x,s); 
}

/*! @brief Multiply: signed 16x16 -> signed 32 bits. */
STATIC_INLINE int32 rflm_mpyh(int16 x, int16 y) {
...
STATIC_INLINE uint32 rflm_mpyuh(uint16 x, uint16 y) {
  return ((uint32) x * y);
}

/*! @brief Multiply: signed 32x32 -> signed 64 bits. */
STATIC_INLINE int64 rflm_mpyw(int32 x, int32 y) {
  return Q6_P_mpy_RR(x,y);
}

/*! @brief Multiply: unsigned 32x32 -> unsigned 64 bits. */
STATIC_INLINE uint64 rflm_mpyuw(uint32 x, uint32 y) {
...
STATIC_INLINE void rflm_nop(void) { 
  asm volatile ("nop" : : ); 
}
...
*/
STATIC_INLINE uint64 rflm_fw_get_pcycle_reg(void) {
  uint64 pcycle;
...
*/
STATIC_INLINE rflm_fw_usr_reg_t rflm_fw_get_usr_reg(void)
{
  rflm_fw_usr_reg_t reg;
...
*/
STATIC_INLINE void rflm_fw_set_usr_reg(rflm_fw_usr_reg_t reg)
{
...
STATIC_INLINE void rflm_dcfetch(void *addr) {
  Q6_dcfetch_A(addr);
}

/*! @brief Data cache line clean.
    Write the cache line containing "addr" from cache if dirty. */
STATIC_INLINE void rflm_dccleana(void *addr) {
...
STATIC_INLINE void rflm_dccleaninva(void *addr) {
...
STATIC_INLINE void rflm_dcinva(void *addr) {
...
STATIC_INLINE void rflm_dczeroa(void *addr) {
...
/*! @brief Instruction cache line invalidate. 
    ...
    NOTE: If addr is not cache line aligned, unrelated data may be
          discarded accidentally. */
STATIC_INLINE void rflm_icinva(void *addr) {
...
/*! @brief Issue an L2 fetch.
    ...
    This maps directly to the rflm_l2fetch() instruction. Refer to the Hexagon
    programmer's reference for details.
 
    For simple linear transfers, you may want to use rflm_l2fetch_buffer() instead,
    as it has simpler inputs.
*/
STATIC_INLINE void rflm_l2fetch
(
...
/*! @brief Initiate a fetch from external memory to L2 cache. 
    ...
    For 2-dimensional transfers, or for longer lengths, use rflm_l2fetch().
   
    A length of 0 *may* do a single line fetch, depending on the address alignment. 

    Also see rflm_l2fetch_is_active() to check if the fetch is complete.
    ...
    is best-effort.
*/
STATIC_INLINE void rflm_l2fetch_buffer
(
  void *addr,  /*!< (Virtual) address to start fetch */
...
*/
STATIC_INLINE boolean rflm_l2fetch_is_active(void)
{
  rflm_fw_usr_reg_t reg = rflm_fw_get_usr_reg();
  return (reg.PFA ? TRUE : FALSE);
}


/*! @brief Ensure that previous instructions have committed. */
STATIC_INLINE void rflm_isync(void) {
  asm volatile ("isync" : : : "memory" );
}

/*! @brief Force all pending transactions to complete. */
STATIC_INLINE void rflm_syncht(void) {
...
*/
STATIC_INLINE void rflm_compiler_barrier(void) {
  asm volatile ("" : : : "memory");
}
...
/*! @brief Define aliases for function overloading in C++.
    ...
    e.g. add_sat() instead of rflm_add_sath(). The compiler will pick the
    appropriate type, or flag an error if it's ambiguous.
*/

STATIC_INLINE int16 rflm_add_sat(int16 x, int16 y) { return rflm_add_sath(x,y); }
STATIC_INLINE int32 rflm_add_sat(int32 x, int32 y) { return rflm_add_satw(x,y); }
STATIC_INLINE int16 rflm_sub_sat(int16 x, int16 y) { return rflm_sub_sath(x,y); }
STATIC_INLINE int32 rflm_sub_sat(int32 x, int32 y) { return rflm_sub_satw(x,y); }
STATIC_INLINE int32 rflm_neg_sat(int32 x) { return rflm_neg_satw(x); }
STATIC_INLINE int32 rflm_abs_sat(int32 x) { return rflm_abs_satw(x); }
#ifndef TEST_FRAMEWORK
STATIC_INLINE  int32 rflm_max( int32 x,  int32 y) { return rflm_maxw(x,y);  }
STATIC_INLINE uint32 rflm_max(uint32 x, uint32 y) { return rflm_maxuw(x,y); }
STATIC_INLINE  int64 rflm_max( int64 x,  int64 y) { return rflm_maxd(x,y);  }
STATIC_INLINE uint64 rflm_max(uint64 x, uint64 y) { return rflm_maxud(x,y); }
STATIC_INLINE  int32 rflm_min( int32 x,  int32 y) { return rflm_minw(x,y);  }
STATIC_INLINE uint32 rflm_min(uint32 x, uint32 y) { return rflm_minuw(x,y); }
STATIC_INLINE  int64 rflm_min( int64 x,  int64 y) { return rflm_mind(x,y);  }
STATIC_INLINE uint64 rflm_min(uint64 x, uint64 y) { return rflm_minud(x,y); }
#else
#error code not present
#endif
STATIC_INLINE int rflm_cl0(int32 x)  { return rflm_cl0w(x); }
STATIC_INLINE int rflm_cl0(int64 x)  { return rflm_cl0d(x); }
STATIC_INLINE int rflm_cl1(int32 x)  { return rflm_cl1w(x); }
STATIC_INLINE int rflm_cl1(int64 x)  { return rflm_cl1d(x); }
STATIC_INLINE int rflm_clb(int32 x)  { return rflm_clbw(x); }
STATIC_INLINE int rflm_clb(int64 x)  { return rflm_clbd(x); }
STATIC_INLINE int rflm_ct0(uint32 x) { return rflm_ct0w(x); }
STATIC_INLINE int rflm_ct0(uint64 x) { return rflm_ct0d(x); }
STATIC_INLINE int rflm_ct1(uint32 x) { return rflm_ct1w(x); }
STATIC_INLINE int rflm_ct1(uint64 x) { return rflm_ct1d(x); }

STATIC_INLINE int32 rflm_clrbit(int32 x, int b)    { return rflm_clrbitw(x,b); }
STATIC_INLINE int32 rflm_setbit(int32 x, int b)    { return rflm_setbitw(x,b); }
STATIC_INLINE int32 rflm_togglebit(int32 x, int b) { return rflm_togglebitw(x,b); }
STATIC_INLINE uint32 rflm_fextract(uint32 x, int width, int offset) { return rflm_fextractuw(x,width,offset); }
STATIC_INLINE uint64 rflm_fextract(uint64 x, int width, int offset) { return rflm_fextractud(x,width,offset); }
STATIC_INLINE uint32 rflm_finsert(uint32 dest, uint32 src, int width, int offset) { return rflm_finsertuw(dest,src,width,offset); }
STATIC_INLINE uint64 rflm_finsert(uint64 dest, uint64 src, int width, int offset) { return rflm_finsertud(dest,src,width,offset); }

STATIC_INLINE int32 rflm_shift(int32 x, int s) { return rflm_shiftw(x,s); }
STATIC_INLINE int64 rflm_shift(int64 x, int s) { return rflm_shiftd(x,s); }
STATIC_INLINE int32 rflm_shift_sat(int32 x, int s) { return rflm_shift_satw(x,s); }

STATIC_INLINE  int32 rflm_mpy( int16 x,  int16 y) { return rflm_mpyh(x,y);  }
STATIC_INLINE uint32 rflm_mpy(uint16 x, uint16 y) { return rflm_mpyuh(x,y); }
STATIC_INLINE  int64 rflm_mpy( int32 x,  int32 y) { return rflm_mpyw(x,y);  }
STATIC_INLINE uint64 rflm_mpy(uint32 x, uint32 y) { return rflm_mpyuw(x,y); }
