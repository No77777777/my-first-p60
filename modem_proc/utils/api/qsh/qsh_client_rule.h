...
$Header: //components/rel/utils.mpss/5.4/api/qsh/qsh_client_rule.h#2 $

when       who     what, where, why
--------   ---     ------------------------------------------------------------- 
08/22/18   mm      CR 2300075: Rule debug logs
08/18/17   mm      CR 2095406: Rule-based QTrace flush
08/01/16   ca      CR: 1048777 Rule Equation.             
05/11/16   ca      CR 998215: Static Rules, DAL Timer changes
                   and other changes.
...
typedef enum
{
  ...
  QSH_CLIENT_RULE_TYPE_NO_ACTION      = 0x4,
  QSH_CLIENT_RULE_TYPE_QTRACE_FLUSH   = 0x5,
  QSH_CLIENT_RULE_TYPE_MAX            = 0x6,
  QSH_CLIENT_RULE_TYPE_DEFAULT        = 0xFFFFFFFF,
} qsh_client_rule_type;

/*! type of error 
     
    WARNING: Only new entries can be added to the end of the enum. 
    Modifying the value of existing enumerations will break QXDM support for conversion of strings to hex values
*/
typedef enum
{
  ...
  QSH_CLIENT_RULE_ERROR_IDX_RULE_ADD_DISABLED         = 0xC,
  QSH_CLIENT_RULE_ERROR_IDX_RULE_DELETE_DISABLED      = 0xD,
  QSH_CLIENT_RULE_ERROR_IDX_CFG_DISABLED              = 0xE,
  QSH_CLIENT_RULE_ERROR_IDX_HELP_DISABLED             = 0xF,
  ...
  QSH_CLIENT_RULE_ERROR_IDX_IDENTICAL_G_H_PARAMS      = 0x13,
  QSH_CLIENT_RULE_ERROR_IDX_INVALID_RULE_EQ_FORMAT    = 0x14,
  QSH_CLIENT_RULE_ERROR_IDX_EXCEED_MAX_RULES_PER_EQ   = 0x15,
  QSH_CLIENT_RULE_ERROR_IDX_INVALID_RULE_ID_IN_EQ     = 0x16,
  QSH_CLIENT_RULE_ERROR_IDX_UNUSED_RULES_IN_EQ        = 0x17,
  QSH_CLIENT_RULE_ERROR_IDX_EQ_DELETE_DISABLED        = 0x18, 
  QSH_CLIENT_RULE_ERROR_IDX_EQ_ADD_DISABLED           = 0x19,  
  QSH_CLIENT_RULE_ERROR_IDX_EXCEED_MAX_EQ             = 0x1A,
  QSH_CLIENT_RULE_ERROR_IDX_INTERNAL                  = 0x1B,
  QSH_CLIENT_RULE_ERROR_IDX_DUP_RULE_ID               = 0x1C,
  QSH_CLIENT_RULE_ERROR_IDX_DUP_EQ_ID                 = 0x1D,
  QSH_CLIENT_RULE_ERROR_IDX_INVALID_TIME_TO_EXPIRE    = 0x1E,
  QSH_CLIENT_RULE_ERROR_IDX_EQ_NOT_FOUND              = 0x1F,
  QSH_CLIENT_RULE_ERROR_IDX_MAX                       = 0x20,
...
typedef struct
{
  /*! handler client event info based on which the event is raised on the 
    handler client */
  qsh_client_rule_event_info_s  handler_info;
  /*! Handler event data */
  uint32                        event_data;
...
QTRACE_FLUSH_CMD: 
  ...
  time_to_action_us :  time interval in usec after which the assert is invoked on the client below.
                         0xFFFFFFFF implies data coming from generator client.
  client        : Client of buffer on which the flush of qtrace needs to be invoked.
  id            : ID of buffer on which the flush of qtrace needs to be invoked.
  QSHRequest add qtrace_flush <rule_ref_count> <client_g> <event_g> <subs_id_g> <time_to_action_us> <client>
  QSHRequest delete qtrace_flush <client_g> <event_g> <subs_id_g> <time_to_action_us> <client>
*/
typedef struct
{
  /*! Client*/
  ...
  /*! buffer id to flush */
  uint8                     id;
} qsh_client_rule_qtrace_flush_s; 
...
NO_ACTION_CMD: 
  ...
  time_to_action_us :  time interval in usec after which the assert is invoked on the client below.
                         0xFFFFFFFF implies data coming from generator client.
  QSHRequest add assert <rule_ref_count> <client_g> <event_g> <subs_id_g> <time_to_action_us> <client>
  QSHRequest delete assert <client_g> <event_g> <subs_id_g> <time_to_action_us> <client>
...
typedef union
{
  ...
  qsh_client_rule_no_action_s     no_action_params;
  qsh_client_rule_qtrace_flush_s  qtrace_flush_params;
...
{  
  ...
  qsh_client_rule_event_info_s  generator_info; 
