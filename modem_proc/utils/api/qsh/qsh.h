/*!
  @file
  qsh.h

  @brief
  The interface to the Qualcomm Sherlock Holmes feature implementation file.
*/

/*==============================================================================

  Copyright (c) 2018 QUALCOMM Technologies Incorporated. All Rights Reserved

  Qualcomm Proprietary

  Export of this technology or software is regulated by the U.S. Government.
  Diversion contrary to U.S. law prohibited.

  All ideas, data and information contained in or disclosed by
  this document are confidential and proprietary information of
  QUALCOMM Technologies Incorporated and all rights therein are expressly reserved.
  By accepting this material the recipient agrees that this material
  and the information contained therein are held in confidence and in
  trust and will not be used, copied, reproduced in whole or in part,
  nor its contents revealed in any manner to others without the express
  written permission of QUALCOMM Technologies Incorporated.

==============================================================================*/

/*==============================================================================

                        EDIT HISTORY FOR MODULE

This section contains comments describing changes made to the module.
Notice that changes are listed in reverse chronological order.

$Header: //components/rel/utils.mpss/5.4/api/qsh/qsh.h#2 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
10/23/17   am      CR 2053974: MDUMP Feature
10/11/15   ca      CR 920884: QSH rule management feature.
02/23/15   mm      Removed FEATURE_QSH_DUMP and FEATURE_QSH_EVENT_METRIC
02/16/15   mm      Defined FEATURE_QSH_DUMP and FEATURE_QSH_EVENT_METRIC
01/22/15   mm      Interface changes for dumping data structures
05/23/14   mm      Updated log interface and added standardized summary
03/24/14   vd      Added support for Ulog messages with 6-10 arguments.
03/12/14   vd      CR 630063: initial version                   
==============================================================================*/

#ifndef QSH_H
#define QSH_H

/*==============================================================================

                           INCLUDE FILES

==============================================================================*/

#include <comdef.h>
#include <customer.h>
#include <qsh_ext.h>
#include <qsh_client.h>
#include <qsh_invoke.h>
#include <qsh_types.h>
#include <qsh_util.h>
#include <qsh_legacy.h>
#include <qsh_client_rule.h>


/*==============================================================================

FUNCTION:  qsh_add_thread_id

==============================================================================*/
/*!
@brief 
QSH API to map thread_id to client id.

It does not check if client is registered for MDUMP as QSH don't want to put
limitation on client to register first and then update thread ID as then clients
would need to synchronize their threads for this registration.

*/
/*============================================================================*/
void qsh_add_thread_id
( 
  qsh_clt_e        client
);

#endif /* QSH_H */
