...
$Header: //components/rel/utils.mpss/5.4/api/qsh/qsh_client.h#2 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
07/22/19   rs      CR 2340301: Multi Metric configuration support
06/11/19   am      CR 2468421: FR55289 MDB collection
08/22/18   mm      CR 2300075: Rule debug logs
07/09/18   rs      CR 2275039: QSH Metric get last X entry changes
06/19/18   yx      CR 2263684: Ulog to QTrace migration in QSH_LOG
02/27/18   rs      CR 2055740: QSH Rearch
10/23/17   am      CR 2053974: MDUMP Feature
...
#define QSH_CHECK_LOG(expr, client, category, msg_type, summary, ...) \
  do { \
    if ((expr)) \
    { \
      QSH_LOG(client, category, msg_type, summary, ##__VA_ARGS__); \
    } \
  } while (0)
/*!
  @brief
...
typedef struct
{
  ...
  qsh_metric_sampling_period_ms_t  time_elapsed_ms;
...
/*!
  @brief
  Max metric config supported for Internal clients 
*/
  
#define QSH_CLIENT_METRIC_CFG_ARR_MAX           20
...
/*!
  @brief  
  Parameters passed to clients when QSH calls client callback for MDUMP 
  collection.
*/
typedef struct {

  /*! subscription id */
  sys_modem_as_id_e_type    crash_subs_id; 

  /*! Active tech */
  sys_sys_mode_e_type       active_rat; 

  /*! Crash category */
  qsh_mdump_crash_cat_e     crash_cat;

  /*! @TODO: Add running threads */
} qsh_client_mdump_s;
#endif
...
typedef struct {
  ...
  /*! Data ID to collect binary data */
  uint32                        data_id;

  /*! Sub ID to collect binary data */
...
/*!
  @brief  
  Parameters passed form clients to QSH for binary data collect
*/
typedef struct {
  ...
  /*! Address of bin data, NULL if no data available */
  uint32                        *address;

  /*! Size of bin data, 0 if no data available */
  uint32                        size;
} qsh_client_mdb_done_s;
...
/*!
  @brief
  Maximum number of data arguments following cmd_code.
*/
#define QSH_CLIENT_CFG_DATA_MAX \
  (8)
...
typedef struct
{
  /*! Multi metric config count */
  uint32                            metric_count;
  /*! Multi metric config array */
  qsh_client_metric_cfg_s           metric_cfg_arr[
    QSH_CLIENT_METRIC_CFG_ARR_MAX];
} qsh_client_metric_cfg_multi_s; 
/*!
  @brief
...
typedef union
{
  ...
  /*! config for QSH_ACTION_METRIC_CFG for multiple metrics*/
  qsh_client_metric_cfg_multi_s     metric_cfg_multi;
...
#ifdef FEATURE_QSH_MDUMP
  /*! config for QSH_ACTION_MINI_DUMP */
  qsh_client_mdump_s                mdump;
#endif

  /*! config for QSH_ACTION_MDB */
  qsh_client_mdb_s                  mdb;
...
{ 
  ...
  /*! return value for QSH Binary data Collect */
  qsh_client_mdb_done_s           mdb_done;
...
uint8* qsh_client_metric_get_last_x_th_entry
(
  qsh_client_metric_context_id_t  metric_context_id,
  uint32 last_x
);
...
/*!
  @brief
  Provides interface for client to indicate
...
/*!
  ...
  @return
  Error Status - Refer to the error code enum: qsh_client_rule_error_idx_e.
*/
/*============================================================================*/
