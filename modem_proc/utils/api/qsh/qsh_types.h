...
$Header: //components/rel/utils.mpss/5.4/api/qsh/qsh_types.h#2 $
...
05/19/17   sy      CR 2049903 Autogen clients
...
/*!
  @brief
  Client enum, exported in <qsh_types_clt_autogen.h>.
*/
...
typedef enum
{
  ...
  QSH_ACTION_CFG_IDX,
  QSH_ACTION_MDUMP_IDX,
  QSH_ACTION_MDB_IDX,
  QSH_ACTION_METRIC_CFG_MULTI_IDX,
  QSH_ACTION_MAX_IDX
} qsh_action_idx_e;
...
typedef enum
{
  ...
  /*! Client support for mini dump */
  QSH_ACTION_MDUMP                = 1 << QSH_ACTION_MDUMP_IDX,

  /*! MDB collection param */
  QSH_ACTION_MDB                  = 1 << QSH_ACTION_MDB_IDX,

  /*! Multi metric config(s) */
  QSH_ACTION_METRIC_CFG_MULTI     = 1 << QSH_ACTION_METRIC_CFG_MULTI_IDX
} qsh_action_e;
...
/*! @brief Type to represent element count */
typedef uint16 qsh_metric_element_count_t;

/*! @brief Type to represent metric sampling period */
typedef uint32 qsh_metric_sampling_period_ms_t;

/*! @brief Type to represent metric element index */
typedef int16 qsh_metric_element_idx_t;
...
typedef uint32 qsh_metric_element_size_t;

/*! Only for backward compatibility. Remove it later */
