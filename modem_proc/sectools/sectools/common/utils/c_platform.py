...
def is_windows_on_arm():
    wos = False
    if is_windows():
        try:
            output = subprocess.run(['systeminfo'], capture_output=True, text=True)
            if output.stdout:
                match = search(r"System Type:\s*(.*)", output.stdout)
                if match:
                    wos = ARM64 in match.group(1).strip().upper()
        except Exception:
            pass
    return wos
