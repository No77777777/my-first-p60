...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcueimeas.c#2 $
...
ueim_meas_id_list_struct_type ueim_meas_id_list_sim[MAX_RRC_STACKS][MAX_INTERNAL_MEAS];
#define ueim_meas_id_list ueim_meas_id_list_sim[WAS_ID_M]
#else
...
)
{
  ...
  /* Check if meas quantity,reporting quantity & Reporting criteria is received.
      ...
      reportCriteria.t ==  T_rrc_UE_InternalReportCriteria_ue_InternalReportingCriteria)
    {
      ...
      if (rrcueim_process_event_criteria(wrrc_as_id,rrc_mcm_ptr->measurementCommand.u.setup->u.ue_InternalMeasurement->
  ...
  else if (RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand,rrc_MeasurementCommand_modify))
  {
        ...
        rrc_MeasurementCommand_modify,measurementType))
    {
        ...
        reportCriteria.t == T_rrc_UE_InternalReportCriteria_ue_InternalReportingCriteria)
      {
        ...
        /* Event Triggered Reporting Chosen */
        if (rrcueim_process_event_criteria(wrrc_as_id,rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.ue_InternalMeasurement->
          reportCriteria.u.ue_InternalReportingCriteria,
          &l1_internal_meas_params->meas_object.u.ue_internal.u.evt_trig_crit) == FAILURE)
        {
  ...
  else
  {
    ...
    * Meas Identity, Transaction Id already set */
    ...
    rrcmeas_delete_addtl_meas(wrrc_as_id,&l1_internal_meas_params->meas_id);
  ...
  rrcmeas_send_l1_cmd(wrrc_as_id,NULL, l1_internal_meas_params, NULL, L1_MEAS_CTRL_PARMS);
...
)
{
    ...
    (l1_meas_report_ptr->u.ue_internal_meas_rpt.cell_measured_results_included == TRUE))
  {
    ...
    if (rrcueim_validate_measured_results(wrrc_as_id,
  ...
  * should be released by SEND_CHAIN using OSS compiler functions */
  ...
  rrcmeas_fill_meas_id(wrrc_as_id,&msg_ptr->message.u.measurementReport,l1_meas_report_ptr->meas_id, INTERNAL_MEAS);
  ...
  if (l1_meas_report_ptr->rpt_crit == L1_EVENT_TRIGGER_RPT)
  {
    ...
    /* Set Event ID */
    rrcueim_set_event_id(wrrc_as_id,&l1_meas_report_ptr->u.ue_internal_meas_rpt.evt_id,
      msg_ptr->message.u.measurementReport.eventResults.u.ue_InternalEventResults);
    ...
    (l1_meas_report_ptr->u.ue_internal_meas_rpt.evt_id == L1_INTERNAL_MEAS_EVENT_6G))
    {
      ...
      //  rrc_free(msg_ptr);
        rrcasn1_free_pdu(wrrc_as_id,rrc_UL_DCCH_Message_PDU,msg_ptr);
        msg_ptr = NULL;
        return;
      }
...
)
{
  ...
  if (ue_InternalMeasurement_ptr->reportCriteria.t == T_rrc_UE_InternalReportCriteria_ue_InternalReportingCriteria)
  {
    ...
    *mcm_rpt_crit_ptr = EVENT;
    ...
    /* Event Triggered Reporting Chosen */
    if (rrcueim_process_event_criteria(wrrc_as_id,ue_InternalMeasurement_ptr->reportCriteria.u.ue_InternalReportingCriteria,
...
)
{
  ...
  /* Check if meas quantity,reporting quantity & Reporting criteria is received.
    ...
    if(FAILURE == rrcueim_fill_mcm_info_r5(wrrc_as_id,rrc_mcm_ptr->measurementCommand.u.setup->u.ue_InternalMeasurement,
  ...
  else if (RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand,rrc_MeasurementCommand_r4_modify))
  {
    ...
    {
      if(FAILURE == rrcueim_fill_mcm_info_r5(wrrc_as_id,rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.ue_InternalMeasurement,
      &l1_internal_meas_params->meas_object.u.ue_internal, transaction_id, &mcm_rpt_crit))
      {
        rrc_free(l1_internal_meas_params);
        return FAILURE;
...
DESCRIPTION 
 This function processes the Measurement Control Message rel 10
...
)
{
  ...
  /* Check if meas quantity,reporting quantity & Reporting criteria is received.
    ...
    rrc_MeasurementCommand_r10_modify))
  {
    l1_internal_meas_params->meas_cmd = L1_MEAS_MODIFY;
