...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcllcrbie.c#3 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $

when        who     what, where, why
--------    ---     --------------------------------------------------------
01/03/18   sp      Made changes to support configuration with RB-ID 32
01/25/16   sg       Made changes to remove the GCF NV 947 read and get the value of this NV through an api
05/26/16   sg      Made changes to reduce F3s
04/27/16   sg      Made changes to reduce F3s
01/14/16   nr      Made changes to remove irrelevant F3s
11/24/15   nr      Made changes to cleanup unneccessary F3s 
05/20/15   aa      Added Message Highs for validation failures
04/08/15   as      L2-RRC interface changes for W+W
01/06/14   aa      Made changes to include the return value of a non-void function to fix errors
12/30/14   aa      Made changes to add the default switch case to fix errors
11/19/14   sg      Made changes to remove default switch cases for ASN1 IEs
                   which will never be hit
12/16/14   aa      Made changes to validate the radio bearer identity used in the array index
...
void rrcllc_check_and_set_srb3_srb4_priority(sys_modem_as_id_e_type wrrc_as_id);
#endif
/*===================================================================
*                            FUNCTIONS
====================================================================*/
...
#ifdef FEATURE_WCDMA_HS_RACH
    if(TRUE == rrchsrach_get_ready_for_common_edch(wrrc_as_id,config_ptr))
    {
...
)
{
  ...
  switch(rb_id)
  {
    case DCCH_UM_RADIO_BEARER_ID:
      ...
      if((UE_MAX_DL_UM_CHANNEL == get_dl_um_index(wrrc_as_id,rb_id,ordered_config_ptr)) ||
         (UE_MAX_UL_UM_CHANNEL == get_ul_um_index(wrrc_as_id,rb_id)))
      {
        status = FAILURE;
      }
      break;
    ...
    case DCCH_DT_LOW_PRI_RADIO_BEARER_ID:
      if(UE_MAX_AM_ENTITY == get_am_index(wrrc_as_id,rb_id))
      {
        status = FAILURE;
      }
      break;
    default:
      ...
      if((UE_MAX_DL_UM_CHANNEL != get_dl_um_index(wrrc_as_id,rb_id,ordered_config_ptr)) ||
         (UE_MAX_UL_UM_CHANNEL != get_ul_um_index(wrrc_as_id,rb_id)) ||
         (UE_MAX_AM_ENTITY != get_am_index(wrrc_as_id,rb_id)))
      {
        status = FAILURE;
      }
      break;
...

{
  ...
  */
  trch_idx = rrcllc_find_dl_dch_trch_idx(wrrc_as_id,trch_id);
  
  if (trch_idx == RRCLLC_INVALID_TRCH_IDX)
  {
   /* Store the RB-Mapping info */
    return(rrcllc_backup_dl_dch_from_ota(wrrc_as_id, trch_id,rb_id,logch_mode,(uint8) logical_channel_id));
  }
  ...
  * so find the real mode from DCH Info and send that in place of UE_MODE_MAX_NUM
    */
    rlc_lc_id = rrclcm_get_dl_rlc_lc_id(wrrc_as_id,(rrc_RB_Identity)rb_id);
    ...
    if (rrclcm_get_dl_rlc_mode(wrrc_as_id,rlc_lc_id, &logch_mode) == RRCLCM_FAILURE)
    {
  ...
  rrcllc_free_hanging_rb_mapping_info_index(wrrc_as_id,rb_id,DL_DCH_RB_MAPPING);
...
)
{
  ...
  if (allow_new_channels == FALSE)
  {
    ...
    if (rrcllc_compare_rlc_size_change(wrrc_as_id,ordered_config_ptr->am_config[entity_idx].rlc_dl_am_config.dl_rlc_size,
...
)
{
  ...
  {
    return rrcllc_update_dl_dch_ota(wrrc_as_id,rb_id,logch_mode,trch_id, ie_ptr->logicalChannelIdentity);
  }
  else
  {
    return rrcllc_update_dl_dch_ota(wrrc_as_id,rb_id,logch_mode,trch_id, RRCLLC_MAC_LOG_CHAN_ID_NOT_PRESENT);
  }
...
/* If Logical Channel ID is present, save MAC Logical Channel Identity */ 
  ...
  rrcllc_update_dl_ded_log_chl_info(wrrc_as_id,logical_chan_id,dlchan_info,rb_id,logch_mode);
  ...
  {
    if((trch_idx = rrcllc_find_dl_fach_trch_idx(wrrc_as_id)) >= UE_MAX_TRCH)
    {
      return FAILURE;
    }
    ...
    */
    ...
    {
      rrcdata_misc_info.default_cfg_ota_mapping[rb_id-1] |= FACH_MAPPING_PRESENT;
    }
#endif   
  return SUCCESS;
...
)
{
   ...
   * Note: The index should already be there.
   */
  trch_idx = rrcllc_find_ul_dch_trch_idx(wrrc_as_id,ordered_config_ptr, trch_id);

  if(trch_idx >= UE_MAX_TRCH)
  {
    /* Store the RB-Mapping info */
    return(rrcllc_backup_ul_dch_from_ota(wrrc_as_id,ie_ptr,rb_id,logch_mode));
  }
  ...
  if(get_rlc_size_list(wrrc_as_id,ie_ptr, rlc_size_rec, rb_id) == FAILURE)
  {
    return FAILURE;
  }
#else
  ...
  if(logch_mode == UE_MODE_MAX_NUM)
  {
     ...
     * so find the real mode from DCH Info and send that in place of UE_MODE_MAX_NUM
     */
    rlc_lc_id = rrclcm_get_ul_rlc_lc_id(wrrc_as_id,(rrc_RB_Identity)rb_id);
    if(rlc_lc_id  == RRCLCM_RLC_LC_ID_NOT_FOUND)
    {
    ...
    if(rrclcm_get_ul_rlc_mode(wrrc_as_id,rlc_lc_id, &logch_mode) == RRCLCM_FAILURE)
    {
  ...
  rrcllc_free_hanging_rb_mapping_info_index(wrrc_as_id,rb_id,UL_DCH_RB_MAPPING);
...
)
{
  ...
  rrcllc_update_ul_ded_log_chl_info(wrrc_as_id,ie_ptr, dlchan_info, rb_id, logch_mode);
  ...
  if(next_rrc_state == RRC_STATE_CELL_FACH ||
     ...
     next_rrc_state == RRC_STATE_URA_PCH)
  {
    if((trch_idx = rrcllc_find_ul_rach_trch_idx(wrrc_as_id)) >= UE_MAX_TRCH)
    {
      return FAILURE;
    }
...
)
{
  ...
  switch (ie_ptr->ul_TransportChannelType.t)
  {
    case T_rrc_UL_TransportChannelType_dch:
      ...
      {
        if (get_ul_dch_mapping_info(wrrc_as_id,ie_ptr, rb_id, logch_mode) == FAILURE)
        {
          return FAILURE;
        }
      }
    ...
    case T_rrc_UL_TransportChannelType_rach:
      ...
      {
        if (get_rach_mapping_info(wrrc_as_id,ie_ptr, rb_id, logch_mode,
      ...
      {
        if (get_rach_mapping_info(wrrc_as_id,ie_ptr, rb_id, logch_mode, RRC_STATE_CELL_DCH) == FAILURE)
        {
          return FAILURE;
        }
      }
...
)
{
  ...
  switch (ie_ptr->t)
  {
    ...
    case T_rrc_UL_LogicalChannelMappings_oneLogicalChannel:
      if(get_ul_single_logical_channel_info(wrrc_as_id,ie_ptr->u.oneLogicalChannel, rb_id, logch_mode)
         == FAILURE)
      {
        return FAILURE;
      }
  ...
  while((ie_ptr->n > lc_tc_cnt) && (lc_tc_cnt < MAX_DED_LOGCHAN_PER_TRCH))
  {
    ...
    switch (ie_ptr->elem[lc_tc_cnt].dl_TransportChannelType.t)
    {
      case T_rrc_DL_TransportChannelType_dch:
        if(ordered_config_ptr->rrc_state_indicator == RRC_STATE_CELL_DCH)
        {
          if(get_dl_dch_mapping_info(wrrc_as_id,&ie_ptr->elem[lc_tc_cnt], rb_id, logch_mode) == FAILURE)
          {
            return FAILURE;
          }
        }
      ...
      case T_rrc_DL_TransportChannelType_fach:
           ...
           ordered_config_ptr->rrc_state_indicator == RRC_STATE_URA_PCH)
        {
         ...
         /* If Logical Channel ID is present, save MAC Logical Channel Identity */ 
           ...
           {
             if(get_fach_mapping_info(wrrc_as_id,ie_ptr->elem[lc_tc_cnt].logicalChannelIdentity, rb_id, logch_mode,
                                      ordered_config_ptr->rrc_state_indicator) == FAILURE)
             {
               return FAILURE;
             }
           ...
           {
             if(get_fach_mapping_info(wrrc_as_id,RRCLLC_MAC_LOG_CHAN_ID_NOT_PRESENT, rb_id, logch_mode,
                                      ordered_config_ptr->rrc_state_indicator) == FAILURE)
             {
               return FAILURE;
             }
  ...
  */
  ...
  while((ie_ptr->n > rb_mux_cnt) && (rb_mux_cnt < MAX_RB_MUX_OPTIONS))
  {
    ...
    if(RRC_MSG_COMMON_BITMASK_IE(ie_ptr->elem[rb_mux_cnt],rrc_RB_MappingOption,dl_LogicalChannelMappingList))
    {
      ...
      if(get_dl_logical_channel_info(wrrc_as_id,&ie_ptr->elem[rb_mux_cnt].dl_LogicalChannelMappingList,
    ...
    if(RRC_MSG_COMMON_BITMASK_IE(ie_ptr->elem[rb_mux_cnt],rrc_RB_MappingOption,ul_LogicalChannelMappings))
    {
      ...
      if(get_ul_logical_channel_info(wrrc_as_id,&ie_ptr->elem[rb_mux_cnt].ul_LogicalChannelMappings,
                                     rb_id, logch_mode) == FAILURE)
      {
        return FAILURE;
      }
...
#ifdef FEATURE_WCDMA_HS_FACH
  rrcllc_backup_mapping_info_into_hanging(wrrc_as_id,ie_ptr,REL_99,rb_id,logch_mode);
#endif
  return SUCCESS;
...
)
{
  ...
  switch (ie_ptr->t)
  {
    case T_rrc_TransmissionRLC_Discard_timerBasedExplicit:
      ...
      temp_val = get_timer_discard(wrrc_as_id,ie_ptr->u.timerBasedExplicit->timerDiscard);
      ...
      /* Get the Timer_MRW */
      temp_val = get_timer_mrw(wrrc_as_id, ie_ptr->u.timerBasedExplicit->timerMRW);
      if(temp_val == RRCLLC_INVALID_RLC_DISCARD)
      {
        return FAILURE;
      }
      ...
      /* Get the MaxMRW */
      temp_val = get_maxmrw(wrrc_as_id, ie_ptr->u.timerBasedExplicit->maxMRW);
      if(temp_val == RRCLLC_INVALID_RLC_DISCARD)
      {
        return FAILURE;
      }
    ...
    case T_rrc_TransmissionRLC_Discard_maxDAT_Retransmissions:
      ...
      /* Get the Max_DAT */
      temp_val = get_max_dat(wrrc_as_id, ie_ptr->u.maxDAT_Retransmissions->maxDAT);
      if(temp_val == RRCLLC_INVALID_RLC_DISCARD)
      {
        return FAILURE;
      }
      ...
      /* Get the Timer_MRW */
      temp_val = get_timer_mrw(wrrc_as_id, ie_ptr->u.maxDAT_Retransmissions->timerMRW);
      if(temp_val == RRCLLC_INVALID_RLC_DISCARD)
      {
        return FAILURE;
      }
      ...
      /* Get the MaxMRW */
      temp_val = get_maxmrw(wrrc_as_id, ie_ptr->u.maxDAT_Retransmissions->maxMRW);
      if(temp_val == RRCLLC_INVALID_RLC_DISCARD)
      {
        return FAILURE;
      }
    ...
    case T_rrc_TransmissionRLC_Discard_noDiscard:
      ...
      /* Get the Max_DAT */
      temp_val = get_max_dat(wrrc_as_id, ie_ptr->u.noDiscard);
...
SIDE EFFECTS:
  None.
============================================================*/
static uint8 rrcllc_get_1st_hole_for_rlc_am(sys_modem_as_id_e_type wrrc_as_id)
{
...
)
{
  ...
  if(get_xmit_rlc_discard(wrrc_as_id,rlc_ul_info, &ie_ptr->transmissionRLC_Discard) == FAILURE)
  {
    return FAILURE;
  }
  ...
  if((rlc_ul_info->tx_window_size = get_window_size(wrrc_as_id,ie_ptr->transmissionWindowSize))
  ...
  {
    if(get_polling_info(wrrc_as_id,rlc_ul_info, &ie_ptr->pollingInfo) == FAILURE)
    {
      return FAILURE;
    }
  }
      ...
      case T_rrc_TransmissionRLC_Discard_timerBasedNoExplicit:
        {
          ordered_config_ptr->rlc_ul_um_parms.chan_info[entity_idx].timer_discard = 
              get_ul_um_timer_discard(wrrc_as_id,ie_ptr->transmissionRLC_Discard.u.timerBasedNoExplicit);
...
)
{
  ...
  if ((rlc_dl_info->rx_window_size = get_rx_window_size(wrrc_as_id, ie_ptr->receivingWindowSize))
    == RRCLLC_INVALID_WINDOW_SIZE)
  {
    return FAILURE;
  }
  ...
  if (get_dl_rlc_status_info(wrrc_as_id,rlc_dl_info, &ie_ptr->dl_RLC_StatusInfo) == FAILURE)
  {
    return FAILURE;
  }
  

  if(rrcllc_process_dl_pdu_size(wrrc_as_id,&ie_ptr->dl_RLC_PDU_size, entity_idx,allow_new_channels,reestablish_entity)
    == SUCCESS)
  {
      /* Set this RLC Size in the global structure rlc_size_per_rb */
    if (rrc_set_dl_am_rlc_pdu_size(wrrc_as_id,rb_id, ordered_config_ptr->am_config[entity_idx].rlc_dl_am_config.dl_rlc_size) == FAILURE)
    {
...
)
{
 ...
 == RRC_PDU_SIZE_NOT_SET )
      ...
      )
  {
    ...
    rrcllc_set_am_dl_rlc_size_ota(wrrc_as_id,ordered_config_ptr->am_config[entity_idx].common_config.rb_id,
...
)
{
  ...
  rlc_lc_id_type lc_id;
  uecomdef_status_e_type status_var = SUCCESS;
  entity_idx = get_ul_um_index(wrrc_as_id,rb_id);
  ...
  { 
    ...
    {
      entity_idx = rrcllc_get_1st_hole_for_rlc_ul_um(wrrc_as_id);
    ...
    if(status_var == FAILURE)
    {
       WRRC_MSG4_HIGH("Wrong UL UM params for RB-ID %d. allow_new_channels %d(FALSE->Not allowed to add), entity_idx %d (8 is invalid),rlc_ul_um_parms.nchan %d(>8 is invalid)",
                      rb_id,
                      allow_new_channels,
                      entity_idx,
                      ordered_config_ptr->rlc_ul_um_parms.nchan);
  ...
  ordered_config_ptr->rlc_ul_um_parms.chan_info[entity_idx].lc_type = 
      get_lc_type_by_um_rb_id(wrrc_as_id, rb_id);
...
)
{
  ...
  entity_idx = get_dl_um_index(wrrc_as_id,rb_id, config_ptr);
  ...
  if(entity_idx >= UE_MAX_DL_UM_CHANNEL)
  {
    ...
    {
      entity_idx = rrcllc_get_1st_hole_for_rlc_dl_um(wrrc_as_id,config_ptr);
    ...
    if(status_var == FAILURE)
    {
       WRRC_MSG4_HIGH("Wrong DL UM params for RB-ID %d. allow_new_channels %d(FALSE->Not allowed to add), entity_idx %d (8 is invalid),rlc_dl_um_parms.nchan %d(>8 is invalid)",
                      rb_id,
                      allow_new_channels,
                      entity_idx,
  ...
  lc_id = rrclcm_get_dl_rlc_lc_id(wrrc_as_id,rb_id);
...
)
{
  ...
  rlc_lc_id_type lc_id;
    ...
    case T_rrc_UL_RLC_Mode_ul_AM_RLC_Mode:
      ...
      entity_idx = get_am_index(wrrc_as_id,rb_id);
      if(entity_idx == UE_MAX_AM_ENTITY)
      {
        if (!allow_new_channels)
        {
        ...
        else
        {
          ...
          entity_idx = rrcllc_get_1st_hole_for_rlc_am(wrrc_as_id);
      ...
      if (get_ul_am_rlc_info(wrrc_as_id, rrc_ul_RLC_Mode->u.ul_AM_RLC_Mode, entity_idx) == FAILURE)
      {
        return FAILURE;
      }
    ...
    case T_rrc_UL_RLC_Mode_ul_UM_RLC_Mode:
      ...
      if(get_rlc_ul_um_info(wrrc_as_id, rb_id, allow_new_channels) == FAILURE)
      {
        return FAILURE;
      }
      ...
      {
        if (get_ul_um_rlc_info(wrrc_as_id,rrc_ul_RLC_Mode->u.ul_UM_RLC_Mode, entity_idx) == FAILURE)
        {
          return FAILURE;
        }
      }
    ...
    case T_rrc_UL_RLC_Mode_ul_TM_RLC_Mode: 
      ...
      lc_id = rrclcm_get_ul_rlc_lc_id(wrrc_as_id,rb_id);
      entity_idx = get_ul_tm_index(wrrc_as_id,lc_id);
      ...
      if (get_ul_tm_rlc_info(wrrc_as_id,rrc_ul_RLC_Mode->u.ul_TM_RLC_Mode, entity_idx) == FAILURE)
      {
        return FAILURE;
      }
 ...
 )
{
  ...
  lc_id = rrclcm_get_dl_rlc_lc_id(wrrc_as_id,rb_id);
  entity_idx = get_dl_tm_index(wrrc_as_id,lc_id);
...
)
{
  ...
  if(get_rlc_dl_um_info(wrrc_as_id,rb_id, allow_new_channels, ordered_config_ptr) == FAILURE)
  {
    return FAILURE;
  }
        
  if (rrc_set_dl_um_li(wrrc_as_id,rb_id, 
              rrc_dl_UM_RLC_Mode->dl_UM_RLC_LI_size) == FAILURE)
  {
    return FAILURE;
  }

  entity_idx = get_dl_um_index(wrrc_as_id,rb_id, ordered_config_ptr);
  if(entity_idx == UE_MAX_DL_UM_CHANNEL)
  {
...
)
{
  ...
  uint8 cnt;
    ...
    mac_dflow_index = rrcllc_find_dflow_index(wrrc_as_id,(uint8)dflow_id);
    /* If DFLOW Index does not exist, then create a new one */
    if (mac_dflow_index == UE_MAX_MAC_D_FLOW)
    {
      /* Create a new one */
      mac_dflow_index = rrcllc_allocate_dflow_index(wrrc_as_id);
      if(mac_dflow_index == UE_MAX_MAC_D_FLOW)
      {
    ...
    * so find the real mode from DCH Info and send that in place of UE_MODE_MAX_NUM
    ...
    WRRC_MSG2_HIGH("RRCHS:DFLOW %d, No of dedicated Ch %d",
      ...
      ordered_config_ptr->mac_dflow_info[mac_dflow_index].ndlchan);
    rrcllc_free_hanging_rb_mapping_info_index(wrrc_as_id,rb_id,MAC_D_RB_MAPPING);
...
)
{
  ...
  {
    if(get_ul_rlc_info(wrrc_as_id, &ie_ptr->ul_RLC_Mode,rb_id,allow_new_channels,&new_rlc_am_entity) == FAILURE)
    {
     return FAILURE;
    }
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_RLC_Info,dl_RLC_Mode))
  {
    switch (ie_ptr->dl_RLC_Mode.t)
    {
      case T_rrc_DL_RLC_Mode_dl_AM_RLC_Mode:
        ...
        if(get_dl_am_rlc_info(wrrc_as_id, ie_ptr->dl_RLC_Mode.u.dl_AM_RLC_Mode, entity_idx) == FAILURE)
        {
          return FAILURE;
        }
      ...
      case T_rrc_DL_RLC_Mode_dl_TM_RLC_Mode:
        if(get_dl_rlc_tm_info(wrrc_as_id,ie_ptr->dl_RLC_Mode.u.dl_TM_RLC_Mode->segmentationIndication,rb_id,allow_new_channels)
             == FAILURE)
        {
          return FAILURE;
        }
...
SIDE EFFECTS:
  None.
====================================================================*/
boolean rrcllc_get_rlc_size_change_status(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8 i;
  for(i = 0; i < UE_MAX_AM_ENTITY; i++)
  {
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_RLC_Info_r5,dl_RLC_Mode))
  {
    switch (ie_ptr->dl_RLC_Mode.t)
    {
      case T_rrc_DL_RLC_Mode_r5_dl_AM_RLC_Mode: /* rrc_dl_AM_RLC_Mode_r5_chosen */
        ...
        if (get_dl_am_rlc_info_r5(wrrc_as_id,ie_ptr->dl_RLC_Mode.u.dl_AM_RLC_Mode, 
 ...
 /* Put all the values in tmp structure. No there are no holes in tmp structure */
  for(ix = UE_MAX_DL_LOGICAL_CHANNEL; ix > 0; ix--)
  {
    ...
    if(config_ptr->dl_macehs_logchan_info[ix-1].queue_id!= INVALID_QUEUE_ID)
    {
      ...
      if (iy == (ix-1)) 
      {
        /*MACEHS: Rearranging MAC EHS Q complete*/
        break;
      }
    }
  } /* End of for (...) */
...
)
{
 ...
 /* If Logical Channel ID is present, save MAC Logical Channel Identity */
   if (RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_DL_LogicalChannelMapping_r5,logicalChannelIdentity))
   {
    return rrcllc_update_machs_ota(wrrc_as_id,ie_ptr->logicalChannelIdentity,rb_id,logch_mode,dflow_id);
   }
   else 
   {
    return  rrcllc_update_machs_ota(wrrc_as_id,RRCLLC_MAC_LOG_CHAN_ID_NOT_PRESENT,rb_id,logch_mode,dflow_id);
   }
}
...
)
{
  ...
  {      
    if (rb_id == ordered_config_ptr->cell_fach_rb_mapping_info.dl_dlchan_info[cnt].rb_id ) 
    {
      WRRC_MSG1_MED("RB_ID %d already present in cell_fach_rb_mapping_info",rb_id);
      return (SUCCESS);        
    }
  }

  rrcllc_update_dl_ded_log_chl_info_r5(wrrc_as_id,ie_ptr, dlchan_info, rb_id, logch_mode);
  ordered_config_ptr->cell_fach_rb_mapping_info.dl_ndlchan ++;
  ...
  */
  ...
  while ((ie_ptr->n > lc_tc_cnt) && (lc_tc_cnt < MAX_DED_LOGCHAN_PER_TRCH))
  {
    ...
    case T_rrc_DL_TransportChannelType_r5_hsdsch:
      ...
      if (ordered_config_ptr->rrc_state_indicator == RRC_STATE_CELL_DCH)
      {
        if (get_hsdsch_mapping_info_r5(wrrc_as_id,&ie_ptr->elem[lc_tc_cnt], rb_id, logch_mode) == FAILURE)
        {
          return FAILURE;
        }
      } 
    ...
    case T_rrc_DL_TransportChannelType_r5_dch:
      if (ordered_config_ptr->rrc_state_indicator == RRC_STATE_CELL_DCH)
      {
        if (get_dl_dch_mapping_info_r5(wrrc_as_id,&ie_ptr->elem[lc_tc_cnt], rb_id, logch_mode) == FAILURE)
        {
          return FAILURE;
        }
      } 
    ...
    case T_rrc_DL_TransportChannelType_r5_fach:
      if (ordered_config_ptr->rrc_state_indicator == RRC_STATE_CELL_FACH ||
        ...
        ordered_config_ptr->rrc_state_indicator == RRC_STATE_URA_PCH)
      {
        if (get_fach_mapping_info_r5(wrrc_as_id,&ie_ptr->elem[lc_tc_cnt], rb_id, logch_mode,
          ordered_config_ptr->rrc_state_indicator) == FAILURE)
        {
          return FAILURE;
        }
      ...
      * local mapping info structure.
        */
        if (get_fach_mapping_info_r5(wrrc_as_id,&ie_ptr->elem[lc_tc_cnt], rb_id, logch_mode, RRC_STATE_CELL_DCH) == FAILURE)
        {
          return FAILURE;
        }
      }
...
)
{
  ...
  */
  ...
  while ((ie_ptr->n > rb_mux_cnt) && (rb_mux_cnt < MAX_RB_MUX_OPTIONS_REL5))
  {
    ...
    {
      MSG_MED("Get RB %d Mapping Info for DL", rb_id,0,0);
      if (get_dl_logical_channel_info_r5(wrrc_as_id,&ie_ptr->elem[rb_mux_cnt].dl_LogicalChannelMappingList,
                                         rb_id, logch_mode) == FAILURE)
      {
        return FAILURE;
      }
...
#ifdef FEATURE_WCDMA_HS_FACH
  rrcllc_backup_mapping_info_into_hanging(wrrc_as_id,ie_ptr,REL_5,rb_id,logch_mode);
#endif
...
SIDE EFFECTS: None.
============================================================*/
static void init_rfc_3095_profiles(sys_modem_as_id_e_type wrrc_as_id,rohc_profiles_e_type *profile_list)
{
  uint32 index = 0;
...
)
{
   ...
   */
  index = get_am_index_wo_f3(wrrc_as_id,rb_id);
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_PDCP_Info,headerCompressionInfoList)) 
  {
    ...
    while (hdrCmprssnInfoList->n > index && (index < MAX_COMP_ENG_TYPES))
    {
          ...
          T_rrc_AlgorithmSpecificInfo_rfc2507_Info)
      {
       ...
       /* Maximum CID value for non-TCP connections. */
        ...
        WRRC_MSG7_HIGH("RFC 2507 info present ind %d F_MAX_PERIOD %d F_MAX_TIME %d F_MAX_HEADER %d TCP_SPACE %d NON_TCP_SPACE %d EXPECTED_REORDERING %d",index,
    ...
    if (! pdcp_validate_params_sub(wrrc_as_id, ordered_config_ptr->pdcp_parms.pdcp_config_req[idx].pdcp_cfg_req.pdcp_info,FALSE))
#else
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_PDCP_Info_r4,headerCompressionInfoList)) 
  {
    ...
    while (hdrCmprssnInfoList->n > index && (index < MAX_COMP_ENG_TYPES))
    {
      ...
      else if (hdrCmprssnInfoList->elem[index].algorithmSpecificInfo.t == T_rrc_AlgorithmSpecificInfo_r4_rfc3095_Info) 
      {
        ...
        init_rfc_3095_profiles(wrrc_as_id,pdcp_parms_ptr->pdcp_config_req[idx].pdcp_cfg_req.pdcp_info.
        ...
        while((rohcProflList->n > count) && count < RFC_3095_MAX_ROHC_PROFILES)
        {
          ...
          WRRC_MSG3_HIGH("RFC_3095 rohc profile count %d value %d = %d",count ,rohcProflList->elem[count],pdcp_parms_ptr->pdcp_config_req[idx].pdcp_cfg_req.pdcp_info.comp_eng_info[index].rohc_info.rohc_profiles[count]);
          count++;
        } // while(rohcProflList)
        ...
        WRRC_MSG4_HIGH("RFC_3095 rohc profile count %d to uncompressed,RFC 3095 ul max_cid %d,RFC 3095 dl max_cidk, RFC 3095 reverse decompression depth %d",
                 count,
    ...
    /* Call PDCP function to futher validate PDCP info */
#ifdef FEATURE_DUAL_WCDMA
    if (!pdcp_validate_params_sub(wrrc_as_id, pdcp_parms_ptr->pdcp_config_req[idx].pdcp_cfg_req.pdcp_info,FALSE))
#else
...
)
{
  ...
  */
  ...
  if ((temp_ie_ptr->rlc_InfoChoice.t == T_rrc_RLC_InfoChoice_r5_rlc_Info) &&
       ((logch_mode = get_rlc_mode_r5(wrrc_as_id, temp_ie_ptr->rlc_InfoChoice.u.rlc_Info)) == UE_MODE_MAX_NUM)
     )
  {
    return FAILURE;
  }
  ...
  if ((temp_ie_ptr->rlc_InfoChoice.t == T_rrc_RLC_InfoChoice_r5_rlc_Info) &&
      (get_rlc_info_r5(wrrc_as_id, temp_ie_ptr->rlc_InfoChoice.u.rlc_Info, 
  ...
  if (get_rb_mapping_info_r5(wrrc_as_id, &ie_ptr->rb_MappingInfo, local_rb_id, logch_mode)
    == FAILURE)
  {
    return FAILURE;
  }
  ...
  { 
    ...
    if (get_pdcp_info_later_than_r3(wrrc_as_id, &ordered_config_ptr->pdcp_parms,
...
)
{
  ...
  */
  if (RRC_CHECK_COMMON_MSG_TYPE(ie_ptr->rlc_InfoChoice,rrc_RLC_InfoChoice_r5_same_as_RB))
  {
    ...
    if ((srb_setup_ptr->n == srb_cnt) || (srb_cnt >= UE_MAX_SRB_SETUP))
    {
      ...
      if ((entity_idx = get_am_index(wrrc_as_id, temp_rb_id)) != UE_MAX_AM_ENTITY)
      {
      /* This RB is in AM mode. And its reference config is found in OC.
        */
        if ((new_entity_idx = rrcllc_get_1st_hole_for_rlc_am(wrrc_as_id)) == UE_MAX_AM_ENTITY)
        {
        ...
        {
          WRRC_MSG3_ERROR("Too many UL/DL AM channels or entities [DL,UL,entities] [%d,%d,%d]",
            ordered_config_ptr->rlc_am_parms.am_info[new_entity_idx]->ul_nchan, 
            ordered_config_ptr->rlc_am_parms.am_info[new_entity_idx]->ul_nchan, 
            ordered_config_ptr->rlc_am_parms.nentity);
          return FAILURE;
      ...
      if ((entity_idx = get_ul_um_index(wrrc_as_id, temp_rb_id))
        != UE_MAX_UL_UM_CHANNEL)
      {
        /* This RB is in UM mode */
        if(get_rlc_ul_um_info(wrrc_as_id, rb_id, TRUE) == FAILURE)
        {
          return FAILURE;
        }
      ...
      if ((entity_idx = get_dl_um_index(wrrc_as_id, temp_rb_id, ordered_config_ptr))
        != UE_MAX_DL_UM_CHANNEL)
      {
        /* This RB is in UM mode */
        if(get_rlc_dl_um_info(wrrc_as_id, rb_id, TRUE, ordered_config_ptr) == FAILURE)
        {
          return FAILURE;
        }
  ...
  */
  if (temp_ie_ptr != NULL)
  {
    ...
    /* Get the RLC info */
    if ((temp_ie_ptr->rlc_InfoChoice.t == T_rrc_RLC_InfoChoice_r5_rlc_Info) && 
        (get_rlc_info_r5(wrrc_as_id, temp_ie_ptr->rlc_InfoChoice.u.rlc_Info, rb_id, TRUE) == FAILURE))
    {
      return FAILURE;
    }
  }
  ...
  /* Use the IE Pointer that came as a parameter to get the RB Mapping info */
  if (get_rb_mapping_info_r5(wrrc_as_id, &ie_ptr->rb_MappingInfo, rb_id, logch_mode) == FAILURE)
  {
    return FAILURE;
  }
#ifdef FEATURE_CHANGE_SRB4_PRIRORITY
  rrcllc_check_and_set_srb3_srb4_priority(wrrc_as_id); 
#endif 

  if(FAILURE == rrcllc_validate_srb_id(wrrc_as_id,rb_id))
  {
    return FAILURE;
  }
...
)
{
  ...
  rlc_size_rec =
    &(ordered_config_ptr->rach_rlc_size_restriction_info.rlc_size_list[logch_idx]);
  if(get_rlc_size_list_r6(wrrc_as_id, ie_ptr, rlc_size_rec, rb_id) == FAILURE)
  {
    return FAILURE;
  }
  ...
  rrcllc_update_ul_ded_log_chl_info_r6(wrrc_as_id,ie_ptr, dlchan_info, rb_id, logch_mode);
  ordered_config_ptr->cell_fach_rb_mapping_info.ul_ndlchan ++;  
...
)
{
  ...
  if(RRC_CHECK_COMMON_MSG_TYPE( ie_ptr->ul_TrCH_Type,rrc_UL_LogicalChannelMapping_r6_ul_TrCH_Type_dch_rach_usch))
  {
    ...
    if(trch_idx >= UE_MAX_TRCH)
    {
      return(rrcllc_backup_ul_dch_from_ota_r6(wrrc_as_id, ie_ptr,rb_id, logch_mode));
    }
    ...
    for (cnt = 0; cnt < ordered_config_ptr->ul_cctrch.dch_info.dch_info[trch_idx].ndlchan; cnt++) 
    {
      ...
      {
        WRRC_MSG1_HIGH("RB_ID %d already present in dch_info", rb_id);
        return (SUCCESS); 
      }
    }
...
)
{
  ...
  mac_dflow_index = rrcllc_find_e_mac_d_flow_idx(wrrc_as_id,(uint8)dflow_id);
  /* If DFLOW Index does not exist, then create a new one */
  if (mac_dflow_index == UE_MAX_MAC_D_FLOW)
  {
    
    WRRC_MSG1_HIGH("EDCH Mac-d FLOW Index for DFLOW %d, not found", dflow_id);
    return(rrcllc_backup_eul_from_ota(wrrc_as_id,ie_ptr,rb_id,logch_mode));
  }
  else
  {
    for (index=0;
  ...
  rrcllc_free_hanging_rb_mapping_info_index(wrrc_as_id, rb_id, E_DCH_RB_MAPPING);
  return SUCCESS;
...
)
{
  ...
  if (RRC_CHECK_COMMON_MSG_TYPE( ie_ptr->ul_TrCH_Type,rrc_UL_LogicalChannelMapping_r6_ul_TrCH_Type_dch_rach_usch))
  {
    ...
    switch (ie_ptr->ul_TrCH_Type.u.dch_rach_usch->ul_TransportChannelType.t)
    {
      case T_rrc_UL_TransportChannelType_dch:
        if(ordered_config_ptr->rrc_state_indicator == RRC_STATE_CELL_DCH)
        {
          if (get_ul_dch_mapping_info_r6(wrrc_as_id,ie_ptr, rb_id, logch_mode) == FAILURE)
          {
            return FAILURE;
          }
        }
      ...
      case T_rrc_UL_TransportChannelType_rach:
        ...
        if(ordered_config_ptr->rrc_state_indicator == RRC_STATE_CELL_FACH ||
           ...
           ordered_config_ptr->rrc_state_indicator == RRC_STATE_URA_PCH)
        {
          if (get_rach_mapping_info_r6(wrrc_as_id, ie_ptr, rb_id, logch_mode,
                              ordered_config_ptr->rrc_state_indicator) == FAILURE)
          {
            return FAILURE;
          }
        ...
        {
          if (get_rach_mapping_info_r6(wrrc_as_id, ie_ptr, rb_id, logch_mode, RRC_STATE_CELL_DCH) == FAILURE)
          {
            return FAILURE;
          }
        }
  ...
  else if(RRC_CHECK_COMMON_MSG_TYPE( ie_ptr->ul_TrCH_Type, rrc_UL_LogicalChannelMapping_r6_ul_TrCH_Type_e_dch))/* rrc_ul_TrCH_Type_e_dch_chosen */
  {
    if(ordered_config_ptr->rrc_state_indicator == RRC_STATE_CELL_DCH)
    {
      if (logch_mode != UE_MODE_TRANSPARENT) 
      {
        if (get_edch_mapping_info_r6(wrrc_as_id, ie_ptr, rb_id, logch_mode
        ...
        rrcllc_update_mac_eul_action(wrrc_as_id, TRUE);
      }
      else
      {
...
/*Always backup the mapping in hanging rb independent of state*/
    ...
    {
      rrcllc_backup_eul_from_ota(wrrc_as_id, ie_ptr, rb_id, logch_mode);
    }
#endif
  }
...
)
{
  ...
  switch (ie_ptr->t)
  {
    ...
    case T_rrc_UL_LogicalChannelMappings_r6_oneLogicalChannel:
      if(get_ul_single_logical_channel_info_r6(wrrc_as_id, 
...
)
{
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_RLC_Info_r6,altE_bitInterpretation))
  {
    ...
    {
      if(mcfg_gcf_nv_get_status() == FALSE)
      {
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_RLC_Info_r6,dl_RLC_Mode))
  {
    switch (ie_ptr->dl_RLC_Mode.t)
    {
      case T_rrc_DL_RLC_Mode_r6_dl_AM_RLC_Mode: /* rrc_dl_AM_RLC_Mode_r5_chosen */
        ...
        WRRC_MSG2_MED("One-Sided Re-est from OTA %d,so set downlink local_reestablish_entity to [DL_UL/DL/UL/None]%d",ie_ptr->rlc_OneSidedReEst,reestablish_entity);
        if (get_dl_am_rlc_info_r5(wrrc_as_id, ie_ptr->dl_RLC_Mode.u.dl_AM_RLC_Mode, entity_idx, rb_id
      ...
      case T_rrc_DL_RLC_Mode_r6_dl_UM_RLC_Mode: /* rrc_dl_UM_RLC_r6_Mode_chosen NULL */
        ...
        if (RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_RLC_Info_r6,altE_bitInterpretation))
        {
          if(get_dl_rlc_um_info_r6(wrrc_as_id, ie_ptr->dl_RLC_Mode.u.dl_UM_RLC_Mode,rb_id,allow_new_channels,TRUE)
              == FAILURE)
          {
            return FAILURE;
          }
        ...
        {
          if(get_dl_rlc_um_info_r6(wrrc_as_id, ie_ptr->dl_RLC_Mode.u.dl_UM_RLC_Mode,rb_id,allow_new_channels,FALSE)
              == FAILURE)
          {
            return FAILURE;
          }
...
)
{
  ...
  */
  ...
  while ((ie_ptr->n > rb_mux_cnt) && (rb_mux_cnt < MAX_RB_MUX_OPTIONS_REL5))
  {
    ...
    {
      MSG_MED("Get RB %d Mapping Info for UL", rb_id,0,0);
      if (get_ul_logical_channel_info_r6(wrrc_as_id, &ie_ptr->elem[rb_mux_cnt].ul_LogicalChannelMappings,
                                      rb_id, logch_mode
           ,one_sided_reest_present,one_sided_reest,allow_new_channels) == FAILURE)
      {
        return FAILURE;
...
#ifdef FEATURE_WCDMA_HS_FACH
  rrcllc_backup_mapping_info_into_hanging(wrrc_as_id, ie_ptr, REL_6, rb_id, logch_mode);
#endif
  ...
  */
  ...
  if (temp_ie_ptr != NULL)
  {
    ...
    if ((temp_ie_ptr->rlc_InfoChoice.t == T_rrc_RLC_InfoChoice_r6_rlc_Info) && 
         ((logch_mode = get_rlc_mode_r6(wrrc_as_id, temp_ie_ptr->rlc_InfoChoice.u.rlc_Info)) == UE_MODE_MAX_NUM)
       )
    {
      return FAILURE;
    }
    ...
    /* Get the RLC info */
    if ((temp_ie_ptr->rlc_InfoChoice.t == T_rrc_RLC_InfoChoice_r6_rlc_Info) && 
         (get_rlc_info_r6(wrrc_as_id, temp_ie_ptr->rlc_InfoChoice.u.rlc_Info, rb_id, TRUE) == FAILURE))
    {
      return FAILURE;
    }
    if (temp_ie_ptr->rlc_InfoChoice.t == T_rrc_RLC_InfoChoice_r6_rlc_Info)
  ...
  /* Use the IE Pointer that came as a parameter to get the RB Mapping info */
  if (get_rb_mapping_info_r6(wrrc_as_id, &ie_ptr->rb_MappingInfo, rb_id, logch_mode
...
)
{
  ...
  {
    /* Get the RLC mode */
    if ((logch_mode = get_rlc_mode_r6(wrrc_as_id, &ie_ptr->rlc_Info)) == UE_MODE_MAX_NUM)
    {
      return FAILURE;
    }
    
    /* Save off the RLC Info */
    if (get_rlc_info_r6(wrrc_as_id, &ie_ptr->rlc_Info, ie_ptr->rb_Identity, FALSE) == FAILURE)
    {
      return FAILURE;
    }
  } /* end RLC Info Check */
...
if ((RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_RB_InformationReconfig_r6,rlc_Info)) && (ie_ptr->rb_Identity >= DCCH_AM_RADIO_BEARER_ID))
{
  if (ie_ptr->rlc_Info.rlc_OneSidedReEst == TRUE)
  {
    ...
    entity_idx = get_am_index_wo_f3(wrrc_as_id, ie_ptr->rb_Identity);
    if (entity_idx != UE_MAX_AM_ENTITY)
    {
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_RB_InformationReconfig_r6,rb_MappingInfo))
  {
    ...
    unmap_rb(wrrc_as_id, ie_ptr->rb_Identity, OC_AND_TOC);
    ...
    if (get_rb_mapping_info_r6(wrrc_as_id, &ie_ptr->rb_MappingInfo, (rb_id_type)ie_ptr->rb_Identity, logch_mode
...
)
{
  ...
  return(get_rb_mapping_info_r6(wrrc_as_id, &ie_ptr->rb_MappingInfo, 
                                ie_ptr->rb_Identity, 
                                UE_MODE_MAX_NUM
                                ,FALSE,FALSE,FALSE));
...
)
{
  ...
  */
  ...
  /* Save off the RLC Info */
  if ((temp_ie_ptr->rlc_InfoChoice.t == T_rrc_RLC_InfoChoice_r6_rlc_Info) &&
      (get_rlc_info_r6(wrrc_as_id,temp_ie_ptr->rlc_InfoChoice.u.rlc_Info, 
       local_rb_id, TRUE) == FAILURE))
  {
    return FAILURE;
  }
  ...
  /* Use the IE Pointer that came as a parameter to get the RB Mapping info */
  if (get_rb_mapping_info_r6(wrrc_as_id, &ie_ptr->rb_MappingInfo, local_rb_id, logch_mode
       ,one_sided_reest_present,one_sided_reest,TRUE) == FAILURE)
  {
    return FAILURE;
  }
  ...
  if (srb_info_setup_list_present)
  {
    ...
    while((srb_setup_r6_ptr->n > srb_cnt) && (srb_cnt < UE_MAX_SRB_SETUP))
    {
      ...
      */
      ...
      if(get_srb_info_to_setup_r6(wrrc_as_id,
    ...
    if(srb5_setup_ptr != NULL)
    {
      ...
      if(rrcllc_update_oc_with_srb5_rel6_ie(wrrc_as_id, srb5_setup_ptr, srb_id) == FAILURE)
      {
...
)
{
  if(rab_info_setup_list_present)
  {
    ...
    while((rab_setup_ptr->n > rab_cnt)&& (rab_cnt < MAX_RAB_TO_SETUP))
    {
      ...
      while((rb_setup_ptr->n > rb_cnt ) && (rb_cnt < MAX_RB_PER_RAB))
      {
        if(get_rb_info_to_setup_r6(wrrc_as_id,
...
)  
{
  ...
  if (rb_info_reconfig_list_present)
  {
    ...
    {
      if(get_rb_info_to_reconfig_r6(wrrc_as_id, &rb_reconfig_ptr->elem[idx]) == FAILURE)
      {
        return FAILURE;
      }
...
)  
{
  ...
  if(rb_info_affected_list_present)
  {
    ...
    {
      if(get_rb_info_affected_r6(wrrc_as_id, &rb_affected_ptr->elem[idx]) == FAILURE)
      {
        return (FAILURE);
      }
...
)
{
  ...
  if(temp_ie_ptr != NULL)
  {
    ...
    if((temp_ie_ptr->rlc_InfoChoice.t == T_rrc_RLC_InfoChoice_rlc_Info) && 
        ((logch_mode = get_rlc_mode(wrrc_as_id,temp_ie_ptr->rlc_InfoChoice.u.rlc_Info)) == UE_MODE_MAX_NUM)
      )
    {
      return FAILURE;
    }
    ...
    /* Get the RLC info */
    if((temp_ie_ptr->rlc_InfoChoice.t == T_rrc_RLC_InfoChoice_rlc_Info) && 
        (get_rlc_info(wrrc_as_id, temp_ie_ptr->rlc_InfoChoice.u.rlc_Info, rb_id, TRUE) == FAILURE))
    {
      return FAILURE;
    }
  }
  ...
  /* Use the IE Pointer that came as a parameter to get the RB Mapping info */
  if(get_rb_mapping_info(wrrc_as_id, &ie_ptr->rb_MappingInfo, rb_id, logch_mode) == FAILURE)
  {
    return FAILURE;
  }
#ifdef FEATURE_CHANGE_SRB4_PRIRORITY
...
)
{
  ...
  /* Save off the RLC Info */
  if((temp_ie_ptr->rlc_InfoChoice.t == T_rrc_RLC_InfoChoice_rlc_Info) && 
      (get_rlc_info(wrrc_as_id, temp_ie_ptr->rlc_InfoChoice.u.rlc_Info, 
                  local_rb_id, TRUE) == FAILURE))
  {
    return FAILURE;
  }
  ...
  /* Use the IE Pointer that came as a parameter to get the RB Mapping info */
  if(get_rb_mapping_info(wrrc_as_id, &ie_ptr->rb_MappingInfo, local_rb_id, logch_mode)
     == FAILURE)
  {
    return FAILURE;
  }
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_RB_InformationSetup,pdcp_Info))
  {
    if (rrc_cap_related_info.rrc_pdcp_enabled)
    {
      ...
      if (get_pdcp_info(wrrc_as_id,&ie_ptr->pdcp_Info, local_rb_id, CPDCP_SETUP) == FAILURE)
      {
        return FAILURE;
      }
    }
...
)
{
  ...
  unmap_rb(wrrc_as_id, *ie_ptr, OC_ONLY);
...
SIDE EFFECTS: None.
============================================================*/
void  rrcllc_delete_rlc_am_info(sys_modem_as_id_e_type wrrc_as_id,rrc_RB_Identity rb_id)
{
  uint8  count;
  ...
  for(count = 0; count < UE_MAX_AM_ENTITY; count ++)
  {
      ...
      )
    {
      ...
      ordered_config_ptr->rlc_am_parms.nentity --;
      rrcllc_init_am_dl_rlc_size(wrrc_as_id,ordered_config_ptr, count);
      break;
    }
  }  /* for (...) */
...
SIDE EFFECTS: None.
============================================================*/
void  rrcllc_delete_rlc_dl_tm_info(sys_modem_as_id_e_type wrrc_as_id,rlc_lc_id_type lc_id)
{
  ordered_config_type *config_ptr;
  /* Use Current Config when in Solo BCH Operation */
  if(rrcllc_is_cmd_under_process_for_solo_bch_operation(wrrc_as_id))
  {
    config_ptr = current_config_ptr;
  }
  else
...
SIDE EFFECTS: None.
============================================================*/
void  rrcllc_delete_rlc_dl_um_info(sys_modem_as_id_e_type wrrc_as_id,rlc_lc_id_type lc_id)
{
  uint8 count;
...
SIDE EFFECTS: None.
============================================================*/
void  rrcllc_delete_rlc_ul_tm_info(sys_modem_as_id_e_type wrrc_as_id,rlc_lc_id_type lc_id)
{
  /* Just decrement the count now since other parameters are not stored/used */
...
SIDE EFFECTS: None.
============================================================*/
void  rrcllc_delete_rlc_ul_um_info(sys_modem_as_id_e_type wrrc_as_id,rlc_lc_id_type lc_id)
{
  uint8 count;
...
)
{
  ...
  /* Get the RB Mapping info */
  return (get_rb_mapping_info(wrrc_as_id,&ie_ptr->rb_MappingInfo, 
          ie_ptr->rb_Identity, 
          UE_MODE_MAX_NUM));
...
)
{
  ...
  /* Get the RB Mapping info */
  return(get_rb_mapping_info_r5(wrrc_as_id, &ie_ptr->rb_MappingInfo, 
                                ie_ptr->rb_Identity, 
                                UE_MODE_MAX_NUM));
...
)
{
  ...
  {
    /* Get the RLC mode */
    if((logch_mode = get_rlc_mode(wrrc_as_id, &ie_ptr->rlc_Info)) == UE_MODE_MAX_NUM)
    {
      return FAILURE;
    }
  
    /* Save off the RLC Info */
    if(get_rlc_info(wrrc_as_id, &ie_ptr->rlc_Info, ie_ptr->rb_Identity, FALSE) == FAILURE)
    {
      return FAILURE;
    }
  } /* end RLC Info Check */
  ...
  if(RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_RB_InformationReconfig,rb_MappingInfo))
  {
    ...
    if(get_rb_mapping_info(wrrc_as_id, &ie_ptr->rb_MappingInfo, ie_ptr->rb_Identity, logch_mode)
      == FAILURE)
    {
      return FAILURE;
    }
  ...
  { 
    WRRC_MSG0_HIGH("PDPC info present in rb setup");
    if (get_pdcp_info(wrrc_as_id, &ie_ptr->pdcp_Info.pdcp_Info, ie_ptr->rb_Identity, CPDCP_RECONFIG) == FAILURE)
    {
      return FAILURE;
    }
  }
...
)
{
  ...
  {
    /* Get the RLC mode */
    if ((logch_mode = get_rlc_mode_r5(wrrc_as_id, &ie_ptr->rlc_Info)) == UE_MODE_MAX_NUM)
    {
      return FAILURE;
    }
    
    /* Save off the RLC Info */
    if (get_rlc_info_r5(wrrc_as_id, &ie_ptr->rlc_Info, ie_ptr->rb_Identity, FALSE) == FAILURE)
    {
      return FAILURE;
    }
  } /* end RLC Info Check */
...
if ((RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_RB_InformationReconfig_r5,rlc_Info)) && (ie_ptr->rb_Identity >= DCCH_AM_RADIO_BEARER_ID))
{
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_RB_InformationReconfig_r5,rb_MappingInfo))
  {
    ...
    if (get_rb_mapping_info_r5(wrrc_as_id, &ie_ptr->rb_MappingInfo, ie_ptr->rb_Identity, logch_mode)
      == FAILURE)
    {
      return FAILURE;
    }
...
uint8 mac_ehs_queue_index =0;
  ...
  if ((config_db == OC_AND_TOC) && (transition_config.toc_usage != TOC_INVALID))
  {
    ...
    if((rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH) && 
      (rrcllcoc_int_data.rrc_ordered_state == RRC_STATE_CELL_DCH))
    {
  ...
  {
    rrcllc_free_hanging_rb_mapping_info_index(wrrc_as_id, rb_id, ALL_RB_MAPPING);
#ifdef FEATURE_WCDMA_HS_FACH
    rrcllc_remove_rb_from_hanging(wrrc_as_id, rb_id);
#endif
  }
  while (iterations)
  {
    ...
    for(trch_idx = 0; trch_idx < UE_MAX_TRCH; trch_idx++)
    {
      ...
      for(logch_idx = 0; logch_idx < ndlchan; logch_idx++)
      {
        ...
        {
          remove_dl_ded_logch_entry(wrrc_as_id, tmp_config_ptr->fach_info[trch_idx].dlchan_info,
                                    logch_idx);
      ...
      for(logch_idx = 0; logch_idx < ndlchan; logch_idx++)
      {
        ...
        {
          remove_dl_ded_logch_entry(wrrc_as_id, tmp_config_ptr->dl_dch_info[trch_idx].dlchan_info,
                                    logch_idx);
      ...
      {      
        for (dflow_index = 0; dflow_index < UE_MAX_MAC_D_FLOW; dflow_index++)
        {
          ...
          for (logch_idx = 0; logch_idx < ndlchan; logch_idx++)
          {
            ...
            {
              WRRC_MSG4_HIGH("RRCHS:Delete mapping on DFLOW Id %d RB %d,L.ChIndex %d,RLCID %d",
                                    tmp_config_ptr->mac_dflow_info[dflow_index].mac_dflow_id,
                                    rb_id,dflow_index,logch_idx);
              
              remove_dl_ded_logch_entry(wrrc_as_id, tmp_config_ptr->mac_dflow_info[dflow_index].dlchan_info,
                                        logch_idx);
      ...
      for(logch_idx = 0; logch_idx <  ndlchan; logch_idx++)
      {
        ...
        {
          remove_dl_ded_logch_entry(wrrc_as_id, tmp_config_ptr->dsch_info[trch_idx].dlchan_info,
                                    logch_idx);
      ...
      for(logch_idx = 0; logch_idx < ndlchan; logch_idx++)
      {
        ...
        {
          remove_ul_ded_logch_entry(wrrc_as_id, tmp_config_ptr->ul_cctrch.dch_info.dch_info[trch_idx].dlchan_info,
                                    logch_idx);
    ...
    for (dflow_index = 0; dflow_index < MAX_E_DCH_MAC_D_FLOW; dflow_index++)
    {
      ...
      for(logch_idx = 0; logch_idx < ndlchan; logch_idx++)
      {
        ...
        {
          remove_e_ul_ded_logch_entry(wrrc_as_id, tmp_config_ptr->mac_e_config.e_mac_d_flow_info[dflow_index].
    ...
    for(logch_idx = 0; logch_idx < ndlchan; logch_idx++)
    {
      ...
      {
        remove_ul_ded_logch_entry(wrrc_as_id, tmp_config_ptr->ul_cctrch.rach_info.dlchan_info,
                                  logch_idx);
    ...
    for(logch_idx = 0; logch_idx <  ndlchan; logch_idx++)
    {
      ...
      {
        remove_dl_ded_logch_entry(wrrc_as_id, tmp_config_ptr->cell_fach_rb_mapping_info.dl_dlchan_info,
                                  logch_idx);
    ...
    for(logch_idx = 0; logch_idx <  ndlchan; logch_idx++)
    {
      ...
      {
        remove_rlc_size_list_entry(wrrc_as_id, tmp_config_ptr->rach_rlc_size_restriction_info.rlc_size_list,
                                   logch_idx);
        break;
      }
    }
    ...
    for(logch_idx = 0; logch_idx <  ndlchan; logch_idx++)
    {
      ...
      {
        remove_ul_ded_logch_entry(wrrc_as_id, tmp_config_ptr->cell_fach_rb_mapping_info.ul_dlchan_info,
                                  logch_idx);
    ...
    iterations --;
    rrcllc_rearrange_macehs_lc_index(wrrc_as_id,tmp_config_ptr);
...
)
{
  if(entry_to_remove == MAX_DED_LOGCHAN_PER_TRCH - 1)
  {
    rrcllc_reset_rach_rlc_size_lists(wrrc_as_id,rlc_size_list_array, entry_to_remove, FALSE);
  }
  else
  {
...
SIDE EFFECTS: None
============================================================*/
void rrcllc_copy_ordered_hanging_rb_mapping_to_current(sys_modem_as_id_e_type wrrc_as_id)
{
...
SIDE EFFECTS: None
============================================================*/
void rrcllc_copy_current_hanging_rb_mapping_to_ordered(sys_modem_as_id_e_type wrrc_as_id)
{
...
SIDE EFFECTS: None
============================================================*/
void rrcllc_init_ordered_hanging_rb_mapping_info(sys_modem_as_id_e_type wrrc_as_id)
{
...
)
{
  ...
  if( chan_type == UL_DCH_RB_MAPPING )
  {
    ...
    {
      if( rrcllc_check_rb_id_exist_rb_mapping(wrrc_as_id,
         ordered_config_ptr->ul_cctrch.dch_info.dch_info[index].dlchan_info[i].rb_id,
         &idx)   == FALSE )
      {
        if(rrcllc_find_free_hanging_rb_mapping_info(wrrc_as_id,&idx) == FAILURE)
        {
  ...
  else  if( chan_type == DL_DCH_RB_MAPPING )
  {
    ...
    for(i=0;i<ordered_config_ptr->dl_dch_info[index].ndlchan;i++ )
    {
      ...
      WRRC_MSG3_HIGH("Backup DL DCH data for RB_id %d at  index %d for DL trch_id %d ",
                        rrc_ordered_hanging_rb_mapping[idx].rb_id,idx,
                       rrc_ordered_hanging_rb_mapping[idx].dl_mapping_info.dch_map_info.tr_id);
    }
  }
  else if( chan_type == RACH_RB_MAPPING )
  {
    for(i=0;i<ordered_config_ptr->ul_cctrch.rach_info.ndlchan;i++ )
    {
      ...
      WRRC_MSG3_HIGH("Backup RACH data for RB_id %d at  index %d for trch_id %d ",
                         rrc_ordered_hanging_rb_mapping[idx].rb_id,idx,
  ...
  else if( chan_type == FACH_RB_MAPPING )
  {
    for(i=0;i<ordered_config_ptr->fach_info[index].ndlchan;i++ )
    {
      ...
      WRRC_MSG3_HIGH("Backup FACH data for RB_id %d at  index %d for trch_id %d ",
                        rrc_ordered_hanging_rb_mapping[idx].rb_id,idx,
  ...
  else  if( (chan_type == MAC_D_RB_MAPPING )&& (index < UE_MAX_MAC_D_FLOW ))
  {
    for(i=0;i<ordered_config_ptr->mac_dflow_info[index].ndlchan ;i++ )
    {
      ...
      WRRC_MSG3_HIGH("Backup MAC_Ddata for RB_id %d at  index %d for flow id %d ",
                    rrc_ordered_hanging_rb_mapping[idx].rb_id,idx,
                        rrc_ordered_hanging_rb_mapping[idx].dl_mapping_info.macd_map_info.mac_d_flow_id);
    }
  }
...
)
{
  ...
  /* Determine the type of Logical Channel mapped to DCH TrCH */
        ...
        rlc_lc_id = rrclcm_get_ul_rlc_lc_id(wrrc_as_id,(rrc_RB_Identity)rrc_ordered_hanging_rb_mapping[i].rb_id);
        if((rlc_lc_id  == RRCLCM_RLC_LC_ID_NOT_FOUND) && 
        ...
        if(rrclcm_get_ul_rlc_mode(wrrc_as_id,rlc_lc_id, &logch_mode) == RRCLCM_FAILURE)
        {  
          if(get_am_index_wo_f3(wrrc_as_id, rrc_ordered_hanging_rb_mapping[i].rb_id) != UE_MAX_AM_ENTITY)
          {
            logch_mode = UE_MODE_ACKNOWLEDGED_DATA;            
          }
          else if(get_ul_um_index(wrrc_as_id,(rrc_RB_Identity)rrc_ordered_hanging_rb_mapping[i].rb_id )!= 
            UE_MAX_UL_UM_CHANNEL)         
          {
            logch_mode = UE_MODE_UNACKNOWLEDGED;
          }
          else if(get_ul_tm_index(wrrc_as_id, rlc_lc_id)!= UE_MAX_UL_TM_CHANNEL)
          {
            logch_mode = UE_MODE_TRANSPARENT;
          }
          else
        ...
        ordered_config_ptr->ul_dch_trch_info[index].lg_ch_cnt = 
             ordered_config_ptr->ul_cctrch.dch_info.dch_info[index].ndlchan;
        ordered_config_ptr->reconfig_needed.uplink_mac = TRUE;
        WRRC_MSG3_HIGH("Restore UL DCH data for RB_id %d at  index %d for trch_id %d ",
                            rrc_ordered_hanging_rb_mapping[i].rb_id,i,
                            rrc_ordered_hanging_rb_mapping[i].ul_mapping_info.ul_dch_info.tr_id);
        rrcllc_free_hanging_rb_mapping_info_index(wrrc_as_id, rrc_ordered_hanging_rb_mapping[i].rb_id, UL_DCH_RB_MAPPING);
      }
    }
  }
  return ret_val;
...
SIDE EFFECTS: None
============================================================*/
uecomdef_status_e_type rrcllc_restore_dl_dch_rb_mapping_info(sys_modem_as_id_e_type wrrc_as_id, tr_ch_id_type index)
{
  uint8 i,logch_idx;
  tr_ch_id_type trch_id;
  uecomdef_logch_mode_e_type logch_mode;
  ...
  for(i=0;i<MAX_HANGING_RB_MAPPING_INFO;i++)
  {
       ...
       (rrc_ordered_hanging_rb_mapping[i].dl_mapping_info.dch_map_info.valid == TRUE ))
    {
      ...
      {       
        ...
        rlc_lc_id = rrclcm_get_dl_rlc_lc_id(wrrc_as_id, (rrc_RB_Identity)rrc_ordered_hanging_rb_mapping[i].rb_id);
        ...
        if(rrclcm_get_dl_rlc_mode(wrrc_as_id,rlc_lc_id, &logch_mode) == RRCLCM_FAILURE)
        {
          ...
          else if(get_dl_um_index(wrrc_as_id, (rrc_RB_Identity)rrc_ordered_hanging_rb_mapping[i].rb_id, 
              ordered_config_ptr)!= UE_MAX_DL_UM_CHANNEL) 
          {
            logch_mode = UE_MODE_UNACKNOWLEDGED;
          }
          else if(get_dl_tm_index(wrrc_as_id, rlc_lc_id)!= UE_MAX_DL_TM_CHANNEL)
          {
            logch_mode = UE_MODE_TRANSPARENT;
          }
          else
        ...
        ordered_config_ptr->reconfig_needed.downlink_mac = TRUE;
        WRRC_MSG3_HIGH("Restore DL DCH data for RB_id %d at  index %d for trch_id %d ",
                            rrc_ordered_hanging_rb_mapping[i].rb_id,i,
                           rrc_ordered_hanging_rb_mapping[i].dl_mapping_info.dch_map_info.tr_id);
        rrcllc_free_hanging_rb_mapping_info_index(wrrc_as_id, rrc_ordered_hanging_rb_mapping[i].rb_id, DL_DCH_RB_MAPPING);
      }
    } 
  }
  return ret_val;
...
SIDE EFFECTS: None
============================================================*/
uecomdef_status_e_type  rrcllc_restore_macd_rb_mapping_info(sys_modem_as_id_e_type wrrc_as_id, uint8 index)
{
  uint8 i,logch_idx;
  uint8 mac_dflow_id;
  uecomdef_logch_mode_e_type logch_mode;
  ...
  for(i=0;i<MAX_HANGING_RB_MAPPING_INFO;i++)
  {
         ...
         (rrc_ordered_hanging_rb_mapping[i].dl_mapping_info.macd_map_info.valid == TRUE ))
    {
      if(rrc_ordered_hanging_rb_mapping[i].dl_mapping_info.macd_map_info.mac_d_flow_id == mac_dflow_id )
      {
        ...
        rrcllc_free_hanging_rb_mapping_info_index(wrrc_as_id, rrc_ordered_hanging_rb_mapping[i].rb_id, 
                                                                                    MAC_D_RB_MAPPING);
        WRRC_MSG3_HIGH("Restored MAC_D data for RB_id %d at  index %d for flow id %d ",
                           rrc_ordered_hanging_rb_mapping[i].rb_id,i,
...
)
{
  ...
  /* Store the RB-Mapping info */
  if( rrcllc_check_rb_id_exist_rb_mapping(wrrc_as_id, rb_id, &idx) == FALSE )
  {
    if(rrcllc_find_free_hanging_rb_mapping_info(wrrc_as_id, &idx) == FAILURE)
    {
      WRRC_MSG0_ERROR("No space for RB Mapping backup for UL DCH ");
  ...
  {
    rlc_lc_id = rrclcm_get_ul_rlc_lc_id(wrrc_as_id, rb_id);
    if(rlc_lc_id  == RRCLCM_RLC_LC_ID_NOT_FOUND)
    {
      WRRC_MSG1_ERROR("Invalid DL LC ID for RB %d", rb_id);
      return FAILURE;
    ...
    if(rrclcm_get_ul_rlc_mode(wrrc_as_id, rlc_lc_id, &logch_mode) == RRCLCM_FAILURE)
    {
      if(get_am_index_wo_f3(wrrc_as_id,rb_id) != UE_MAX_AM_ENTITY)
      {
        logch_mode = UE_MODE_ACKNOWLEDGED_DATA;
      }
      else if(get_ul_um_index(wrrc_as_id, rb_id)!= UE_MAX_UL_UM_CHANNEL) 
      {
        logch_mode = UE_MODE_UNACKNOWLEDGED;
      }
      else if(get_ul_tm_index(wrrc_as_id, rlc_lc_id)!= UE_MAX_UL_TM_CHANNEL)
  ...
  WRRC_MSG3_HIGH(" Adding into Standalone Database for R6 E_DCH for RB-ID %d for FLOW ID %d  at idx %d",
                             rrc_ordered_hanging_rb_mapping[idx].rb_id,
                             rrc_ordered_hanging_rb_mapping[idx].ul_mapping_info.ul_mace.mac_e_flow_id,
                             idx);
...
SIDE EFFECTS: None
============================================================*/
uecomdef_status_e_type rrcllc_restore_ul_emac_rb_mapping_info(sys_modem_as_id_e_type wrrc_as_id, uint8 index)
{
  uint8 i,logch_idx,j;
  uint8 mac_eflow_id;
  uecomdef_logch_mode_e_type logch_mode;
         ...
         (rrc_ordered_hanging_rb_mapping[i].ul_mapping_info.ul_mace.valid == TRUE))
    {
      if((rrc_ordered_hanging_rb_mapping[i].ul_mapping_info.ul_mace.mac_e_flow_id == mac_eflow_id ) 
       ...
       /*Check if the RB ID already exists in OC. Issue is happenning in HS-RACH because 
        ...
        in OC twice  */
        && (FALSE == rrcllc_check_if_rbid_exists_in_config_ptr(wrrc_as_id, ordered_config_ptr,rrc_ordered_hanging_rb_mapping[i].rb_id,index))
#endif
        )
      {            
        ...
        WRRC_MSG3_HIGH("Restored MAC_E data for RB_id %d at  index %d for flow id %d ",
                          rrc_ordered_hanging_rb_mapping[i].rb_id,i,
...
SIDE EFFECTS: None
============================================================*/
boolean rrcllc_check_ordered_current_mapping_same(sys_modem_as_id_e_type wrrc_as_id,uint8 index )
{
  ...
  else
  {
	...
	{
    WRRC_MSG3_HIGH("CC/OC rb-d [%d/%d] at index %d ",rrc_current_hanging_rb_mapping[index].rb_id,
...
)
{
  ...
  WRRC_MSG3_HIGH(" Adding into Standalone Database for DL DCH for RB-ID  %d for Trch id %d  at idx %d",
...
)
{
  ...
  WRRC_MSG3_HIGH(" Adding into Standalone Database for R6 UL DCH for RB-ID  %d for Trch id %d  at idx %d",
                             rrc_ordered_hanging_rb_mapping[idx].rb_id,
                             rrc_ordered_hanging_rb_mapping[idx].ul_mapping_info.ul_dch_info.tr_id,
                             idx);
  /* Save the RLC Size List information */
#if defined(FEATURE_RRC_TFC_RLC_SIZE_FOR_DCH)
  if(get_rlc_size_list_r6(wrrc_as_id, ie_ptr, &rrc_ordered_hanging_rb_mapping[idx].ul_mapping_info.ul_dch_info.size_rest_info, rb_id) == FAILURE)
  {
    return FAILURE;
  }
#endif
...
)
{
  ...
  WRRC_MSG3_HIGH(" Adding into Standalone Database for UL DCH for RB-ID  %d for Trch id %d  at idx %d",
                             rrc_ordered_hanging_rb_mapping[idx].rb_id,
                             rrc_ordered_hanging_rb_mapping[idx].ul_mapping_info.ul_dch_info.tr_id,
                             idx);
    #if defined(FEATURE_RRC_TFC_RLC_SIZE_FOR_DCH)
  /* Save the RLC Size List information */
   if(get_rlc_size_list(wrrc_as_id, ie_ptr, &rrc_ordered_hanging_rb_mapping[idx].ul_mapping_info.ul_dch_info.size_rest_info, rb_id) == FAILURE)
  {
    return FAILURE;
  }
#endif
...
SIDE EFFECTS: 
============================================================*/
uecomdef_status_e_type rrcllc_validate_rb_mapping_info(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8 cntx = 0;
  uint8 cnty = 0;
  uint8 trch_idx = 0;
        ...
        for (cntx = 0; (cntx < config_ptr->dl_dch_info[trch_idx].ndlchan) && (cntx <MAX_DED_LOGCHAN_PER_TRCH); cntx++) 
        {
          if (RRCLLC_LC_TYPE_DCCH_OR_DCTCH(config_ptr->dl_dch_info[trch_idx].dlchan_info[cntx].chan_type)) 
          {
                ...
                && (cnty < MAX_DED_LOGCHAN_PER_TRCH)); cnty++) 
            {
              ...
              {
                WRRC_MSG3_ERROR("Cannot assign same log chl ID %d to 2 dif rbs: %d %d: fail",
  ...
  if(RRC_CHECK_COMMON_MSG_TYPE(ie_ptr->rlc_InfoChoice,rrc_RLC_InfoChoice_r5_same_as_RB))
  {
    ...
    if((srb_setup_ptr->n == srb_cnt) || (srb_cnt >= UE_MAX_SRB_SETUP))
    {
      ...
      if((entity_idx = get_am_index(wrrc_as_id,temp_rb_id)) != UE_MAX_AM_ENTITY)
      {
        /* This RB is in AM mode. And its reference config is found in OC.
         */
        if( (new_entity_idx = get_am_index_wo_f3(wrrc_as_id, rb_id)) == UE_MAX_AM_ENTITY)
        {
          if ((new_entity_idx = rrcllc_get_1st_hole_for_rlc_am(wrrc_as_id)) == UE_MAX_AM_ENTITY)
          {
            WRRC_MSG1_ERROR("Can't find space for new rb %d", rb_id);
...
)
{
  ...
  lc_index = rrcllc_find_lc_index_for_macehs(wrrc_as_id, ordered_config_ptr, logicalChannelIdentity,rb_id);
  if(lc_index == UE_MAX_DL_LOGICAL_CHANNEL)
  {
     /* Allocate LC index */
     lc_index = rrcllc_allocate_lc_index_macehs(wrrc_as_id, ordered_config_ptr, logicalChannelIdentity);
...
#ifdef FEATURE_WCDMA_HS_FACH
  lc_index = rrcllc_find_lc_index_for_efach(wrrc_as_id, transition_config.toc_ptr, logicalChannelIdentity,rb_id);
  if(lc_index == UE_MAX_DL_LOGICAL_CHANNEL)
  {
     /* Allocate LC index */
     lc_index = rrcllc_allocate_lc_index_efach(wrrc_as_id, transition_config.toc_ptr, logicalChannelIdentity);
  ...
  WRRC_MSG2_HIGH("lc_index %d allocated for LCH %d",lc_index, logicalChannelIdentity);
  if (logch_mode == UE_MODE_MAX_NUM)
  {
    rlc_lc_id = rrclcm_get_dl_rlc_lc_id(wrrc_as_id, (rrc_RB_Identity)rb_id);
    if (rlc_lc_id  == RRCLCM_RLC_LC_ID_NOT_FOUND)
...
)
{
  ...
  else if (mac_ehs_chosen == TRUE )
  {
    ...
    if (RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_DL_LogicalChannelMapping_r7,logicalChannelIdentity))
    {
      return rrcllc_update_mac_ehs_ota(wrrc_as_id, ie_ptr->logicalChannelIdentity,rb_id,logch_mode,macehs_queue_id);
    }
    else
    {
  ...
  */
  ...
  while ((ie_ptr->n > lc_tc_cnt) && (lc_tc_cnt < MAX_DED_LOGCHAN_PER_TRCH))
  {
    ...
    case T_rrc_DL_TransportChannelType_r7_hsdsch:
      ...
      if ( ordered_config_ptr->rrc_state_indicator == RRC_STATE_CELL_DCH )
      {
        if (get_hsdsch_mapping_info_r7(wrrc_as_id, &ie_ptr->elem[lc_tc_cnt], rb_id, logch_mode) == FAILURE)
        {
          return FAILURE;
        }
      } 
    ...
    case T_rrc_DL_TransportChannelType_r7_dch:
      if (ordered_config_ptr->rrc_state_indicator == RRC_STATE_CELL_DCH)
      {
        if (get_dl_dch_mapping_info_r7(wrrc_as_id, &ie_ptr->elem[lc_tc_cnt], rb_id, logch_mode) == FAILURE)
        {
          return FAILURE;
        }
      } 
...
)
{
  ...
  */
  ...
  while ((ie_ptr->n > rb_mux_cnt) && (rb_mux_cnt < MAX_RB_MUX_OPTIONS_REL5))
  {
    ...
    {
      MSG_MED("Get RB %d Mapping Info for DL", rb_id,0,0);
      if (get_dl_logical_channel_info_r7(wrrc_as_id,&ie_ptr->elem[rb_mux_cnt].dl_LogicalChannelMappingList,
                                         rb_id, logch_mode
                                  ) == FAILURE)
      {
        return FAILURE;
...
#ifdef FEATURE_WCDMA_HS_FACH
  rrcllc_backup_mapping_info_into_hanging(wrrc_as_id, ie_ptr, REL_7, rb_id, logch_mode);
#endif
...
)
{
  ...
  if(ie_ptr->dl_RLC_PDU_size.t == T_rrc_DL_AM_RLC_Mode_r7_dl_RLC_PDU_size_fixedSize)
  {
    ...
    if(rrcllc_process_dl_pdu_size(wrrc_as_id, ie_ptr->dl_RLC_PDU_size.u.fixedSize, entity_idx,allow_new_channels,reestablish_entity)
     == SUCCESS)
    {
      /* Set this RLC Size in the global structure rlc_size_per_rb */
      if (rrc_set_dl_am_rlc_pdu_size(wrrc_as_id, rb_id, ordered_config_ptr->am_config[entity_idx].rlc_dl_am_config.dl_rlc_size) == FAILURE)
  ...
  {
   rrcllc_set_am_dl_rlc_size_ota(wrrc_as_id,
    ...
    {
      rrcllc_process_flexible_pdu_dl_am(wrrc_as_id, LI_15_BITS, entity_idx, allow_new_channels, reestablish_entity);
    }
    else if (ie_ptr->dl_RLC_PDU_size.u.flexibleSize == rrc_DL_AM_RLC_Mode_r7_flexibleSize_size7)
    {
      rrcllc_process_flexible_pdu_dl_am(wrrc_as_id, LI_7_BITS, entity_idx, allow_new_channels, reestablish_entity);
    }
    else
    {
...
)
{
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_RLC_Info_r7,ul_RLC_Mode))
  {
    if(get_ul_rlc_info(wrrc_as_id, &ie_ptr->ul_RLC_Mode,rb_id,allow_new_channels,
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_RLC_Info_r7,dl_RLC_Mode))
  {
    switch (ie_ptr->dl_RLC_Mode.t)
    {
      case T_rrc_DL_RLC_Mode_r7_dl_AM_RLC_Mode: /* rrc_dl_AM_RLC_Mode_r7_chosen */
        ...
        if (get_dl_am_rlc_info_r7(wrrc_as_id, ie_ptr->dl_RLC_Mode.u.dl_AM_RLC_Mode, entity_idx, rb_id
          , allow_new_channels, reestablish_entity) == FAILURE)
        {
          return FAILURE;
        }
      ...
      case T_rrc_DL_RLC_Mode_r7_dl_TM_RLC_Mode:
        if(get_dl_rlc_tm_info(wrrc_as_id, ie_ptr->dl_RLC_Mode.u.dl_TM_RLC_Mode->segmentationIndication,
...
)
{
  ...
  {
    /* Get the RLC mode */
    if ((logch_mode = get_rlc_mode_r7(wrrc_as_id, &ie_ptr->rlc_Info)) == UE_MODE_MAX_NUM)
    {
      return FAILURE;
    }
    
    /* Save off the RLC Info */
    if (get_rlc_info_r7(wrrc_as_id, &ie_ptr->rlc_Info, ie_ptr->rb_Identity, FALSE) == FAILURE)
    {
      return FAILURE;
    }
  } /* end RLC Info Check */
...
if ((RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_RB_InformationReconfig_r7,rlc_Info)) &&(ie_ptr->rb_Identity >= DCCH_AM_RADIO_BEARER_ID))
{
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_RB_InformationReconfig_r7,rb_MappingInfo))
  {
    ...
    if (get_rb_mapping_info_r7(wrrc_as_id, &ie_ptr->rb_MappingInfo, (rb_id_type)ie_ptr->rb_Identity, logch_mode
        ,one_sided_reest_present,one_sided_reest,FALSE) == FAILURE)
    {
      return FAILURE;
    }
...
)
{
  ...
  /* Get the RB Mapping info */
  return(get_rb_mapping_info_r7(wrrc_as_id, &ie_ptr->rb_MappingInfo, 
                                ie_ptr->rb_Identity, 
                                UE_MODE_MAX_NUM
                                ,FALSE,FALSE,FALSE));
...
)
{
  ...
  */
  ...
  if ((temp_ie_ptr->rlc_InfoChoice.t == T_rrc_RLC_InfoChoice_r7_rlc_Info) && 
       ((logch_mode = get_rlc_mode_r7(wrrc_as_id,temp_ie_ptr->rlc_InfoChoice.u.rlc_Info)) == UE_MODE_MAX_NUM)
     )
  {
    return FAILURE;
  }
  ...
  /* Save off the RLC Info */
  if ((temp_ie_ptr->rlc_InfoChoice.t == T_rrc_RLC_InfoChoice_r7_rlc_Info) && 
      (get_rlc_info_r7(wrrc_as_id, temp_ie_ptr->rlc_InfoChoice.u.rlc_Info, local_rb_id, TRUE) == FAILURE))
  {
    return FAILURE;
  }
  ...
  /* Use the IE Pointer that came as a parameter to get the RB Mapping info */
  if (get_rb_mapping_info_r7(wrrc_as_id, &ie_ptr->rb_MappingInfo, local_rb_id, logch_mode
       ,one_sided_reest_present,one_sided_reest,TRUE) == FAILURE)
  {
    return FAILURE;
  }
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_RB_InformationSetup_r7,pdcp_Info))
  {
    ...
    { 
      WRRC_MSG0_HIGH("PDPC info present in rb setup");
      if (get_pdcp_info_later_than_r3(wrrc_as_id, &ordered_config_ptr->pdcp_parms,&ie_ptr->pdcp_Info, 
...
)  
{
  ...
  if (rb_info_reconfig_list_present)
  {
    ...
    while((rb_reconfig_ptr->n > idx) && (idx < MAX_RB))
    {
      if(get_rb_info_to_reconfig_r7(wrrc_as_id, &rb_reconfig_ptr->elem[idx]) == FAILURE)
      {
        return FAILURE;
      }
...
)  
{
  ...
  if(rb_info_affected_list_present)
  {
    ...
    while((rb_affected_ptr->n > idx) && (idx < MAX_RB))
    {
      if(get_rb_info_affected_r7(wrrc_as_id, &rb_affected_ptr->elem[idx]) == FAILURE)
      {
        return (FAILURE);
      }
  ...
  */
  if (RRC_CHECK_COMMON_MSG_TYPE(ie_ptr->rlc_InfoChoice,rrc_RLC_InfoChoice_r7_same_as_RB))
  {
    ...
    if ((srb_setup_ptr->n == srb_cnt) || (srb_cnt >= UE_MAX_SRB_SETUP))
    {
      ...
      if ((entity_idx = get_ul_um_index(wrrc_as_id, rrclcm_get_ul_rlc_lc_id(wrrc_as_id,temp_rb_id)))
        != UE_MAX_UL_UM_CHANNEL)
      {
        /* This RB is in UM mode */
        if(get_rlc_ul_um_info(wrrc_as_id, rb_id, TRUE) == FAILURE)
  ...
  if (temp_ie_ptr != NULL)
  {
    ...
    /* Get the RLC info */
    if ((temp_ie_ptr->rlc_InfoChoice.t == T_rrc_RLC_InfoChoice_r7_rlc_Info) && 
         (get_rlc_info_r7(wrrc_as_id, temp_ie_ptr->rlc_InfoChoice.u.rlc_Info, rb_id, TRUE) == FAILURE))
    {
      return FAILURE;
    }
  }
  ...
  /* Use the IE Pointer that came as a parameter to get the RB Mapping info */
  if (get_rb_mapping_info_r7(wrrc_as_id, &ie_ptr->rb_MappingInfo, rb_id, logch_mode
       ,one_sided_reest_present,one_sided_reest,TRUE) == FAILURE)
  {
    return FAILURE;
  }
...
)
{
  ...
  if (srb_info_setup_list_present)
  {
    ...
    while((srb_setup_r7_ptr->n > srb_cnt ) && (srb_cnt < UE_MAX_SRB_SETUP))
    {
      ...
      */
      ...
      /* Save off the RB Mapping info for each SRB */
      if(get_srb_info_to_setup_r7(wrrc_as_id,
  ...
  if(srb5_setup_ptr != NULL)
  {
    ...
    if(rrcllc_update_oc_with_srb5_rel7_ie(wrrc_as_id, srb5_setup_ptr, srb_id) == FAILURE)
    {
      WRRC_MSG0_ERROR("Rejecting SRB#5 configuration given by the network");
      return FAILURE;
    }
...
)
{
  if(rab_info_setup_list_present)
  {
    ...
    while((rab_setup_ptr->n > rab_cnt) && (rab_cnt < MAX_RAB_TO_SETUP))
    {
      ...
      while((rb_setup_ptr->n > rb_cnt) && (rb_cnt < MAX_RB_PER_RAB))
      {
        if(get_rb_info_to_setup_r7(wrrc_as_id,
           &rb_setup_ptr->elem[rb_cnt],
#ifdef FEATURE_UMTS_PDCP
           rab_setup_ptr->elem[rab_cnt].rab_Info.cn_DomainIdentity,
#endif /* FEATURE_UMTS_PDCP */           
...
)
{
  ...
  rlc_size_rec =
    &(ordered_config_ptr->rach_rlc_size_restriction_info.rlc_size_list[logch_idx]);
  if(get_rlc_size_list_r8(wrrc_as_id, ie_ptr, rlc_size_rec, rb_id) == FAILURE)
  {
    return FAILURE;
  }
  ...
  rrcllc_update_ul_ded_log_chl_info_r8(wrrc_as_id, ie_ptr, dlchan_info, rb_id, logch_mode);
  ordered_config_ptr->cell_fach_rb_mapping_info.ul_ndlchan ++;  
...
#ifdef FEATURE_WCDMA_DEFAULT_CFG_CELLFACH
    if ((rb_id > 0) && (FACH_MAPPING_PRESENT & rrcdata_misc_info.default_cfg_ota_mapping[rb_id-1]) && (rb_id <= NUM_SRB_DC_CELLFACH))
    {
      rrcdata_misc_info.default_cfg_ota_mapping[rb_id-1]|= FACH_RACH_MAPPING_PRESENT;
    }
#endif
  return SUCCESS;
...
)
{
       ...
       rrc_UL_LogicalChannelMapping_r8_ul_TrCH_Type_dch_rach_usch))
  {
    ...
    if(trch_idx >= UE_MAX_TRCH)
    {
      return(rrcllc_backup_ul_dch_from_ota_r8(wrrc_as_id, ie_ptr,rb_id, logch_mode));
    }
...
)
{
       ...
       rrc_UL_LogicalChannelMapping_r8_ul_TrCH_Type_dch_rach_usch))
  {
    ...
    switch (ie_ptr->ul_TrCH_Type.u.dch_rach_usch->ul_TransportChannelType.t)
    {
      case T_rrc_UL_TransportChannelType_dch:
        if(ordered_config_ptr->rrc_state_indicator == RRC_STATE_CELL_DCH)
        {
          if (get_ul_dch_mapping_info_r8(wrrc_as_id, ie_ptr, rb_id, logch_mode) == FAILURE)
          {
            return FAILURE;
          }
        }
      ...
      case T_rrc_UL_TransportChannelType_rach:
        if(ordered_config_ptr->rrc_state_indicator == RRC_STATE_CELL_FACH ||
           ...
           ordered_config_ptr->rrc_state_indicator == RRC_STATE_URA_PCH)
        {
          if (get_rach_mapping_info_r8(wrrc_as_id, ie_ptr, rb_id, logch_mode,
                              ordered_config_ptr->rrc_state_indicator) == FAILURE)
          {
            return FAILURE;
          }
        ...
        {
          if (get_rach_mapping_info_r8(wrrc_as_id, ie_ptr, rb_id, logch_mode, RRC_STATE_CELL_DCH) == FAILURE)
          {
            return FAILURE;
          }
        }
  	...
  	rrc_UL_LogicalChannelMapping_r8_ul_TrCH_Type_e_dch))/* rrc_ul_TrCH_Type_e_dch_chosen */
  {
    if(ordered_config_ptr->rrc_state_indicator == RRC_STATE_CELL_DCH)
    {
      if (logch_mode != UE_MODE_TRANSPARENT) 
      {
        if (get_edch_mapping_info_r8(wrrc_as_id, ie_ptr, rb_id, logch_mode
            ,one_sided_reest_present,one_sided_reest,allow_new_channels
#ifdef FEATURE_MAC_I 
            ,NULL
#endif /* FEATURE_MAC_I */
...
/*Always backup the mapping in hanging rb independent of state*/
    {
      rrcllc_backup_eul_from_ota_r8(wrrc_as_id, ie_ptr,rb_id,logch_mode);
    }
#endif
  }
  return SUCCESS;
...
)
{
  ...
  switch (ie_ptr->t)
  {
    ...
    case T_rrc_UL_LogicalChannelMappings_r8_oneLogicalChannel:
      if(get_ul_single_logical_channel_info_r8(wrrc_as_id,
         ie_ptr->u.oneLogicalChannel, rb_id, logch_mode
         , one_sided_reest_present, one_sided_reest,allow_new_channels) == FAILURE)
      {
        return FAILURE;
...
)
{
  ...
  if (srb_info_setup_list_present)
  {
    ...
    while((srb_setup_r8_ptr->n > srb_cnt ) && (srb_cnt < UE_MAX_SRB_SETUP))
    {
      ...
      */
      ...
      /* Save off the RB Mapping info for each SRB */
      if(get_srb_info_to_setup_r8(wrrc_as_id,
      ...
      {
         rrcdata_misc_info.default_cfg_recfg_list_srb_present[srb_setup_r8_ptr->elem[srb_cnt].rb_Identity-1] = TRUE;
      }
#endif   
      srb_cnt++;
    } /* end SRB setup */
  ...
  if(srb5_setup_ptr != NULL)
  {
    ...
    if(rrcllc_update_oc_with_srb5_rel8_ie(wrrc_as_id, srb5_setup_ptr, srb_id) == FAILURE)
    {
      WRRC_MSG0_ERROR("Rejecting SRB#5 configuration given by the network");
      return FAILURE;
    }
...
)
{
  ...
  */
  ...
  while ((ie_ptr->n > rb_mux_cnt) && (rb_mux_cnt < MAX_RB_MUX_OPTIONS_REL5))
  {
    ...
    {
      MSG_MED("Get RB %d Mapping Info for UL", rb_id,0,0);
      if (get_ul_logical_channel_info_r8(wrrc_as_id, &ie_ptr->elem[rb_mux_cnt].ul_LogicalChannelMappings,
                                      rb_id, logch_mode
                                    , one_sided_reest_present,one_sided_reest,allow_new_channels) == FAILURE)
      {
        return FAILURE;
...
#ifdef FEATURE_WCDMA_HS_FACH
  rrcllc_backup_mapping_info_into_hanging(wrrc_as_id, ie_ptr, REL_8, rb_id, logch_mode);
#endif
...
)
{
  ...
  */
  ...
  /* Use the IE Pointer that came as a parameter to get the RB Mapping info */
  if (get_rb_mapping_info_r8(wrrc_as_id,&ie_ptr->rb_MappingInfo, rb_id, logch_mode
       , one_sided_reest_present,one_sided_reest,TRUE) == FAILURE)
  {
    return FAILURE;
  }
...
)
{
  if(rab_info_setup_list_present)
  {
    ...
    while((rab_setup_ptr->n > rab_cnt) && (rab_cnt < MAX_RAB_TO_SETUP))
    {
      ...
      while((rb_setup_ptr->n > rb_cnt) && (rb_cnt < MAX_RB_PER_RAB))
      {
        if(get_rb_info_to_setup_r8(wrrc_as_id,
           &rb_setup_ptr->elem[rb_cnt],
#ifdef FEATURE_UMTS_PDCP
           rab_setup_ptr->elem[rab_cnt].rab_Info.cn_DomainIdentity,
#endif /* FEATURE_UMTS_PDCP */           
...
)
{
  ...
  */
  ...
  /* Save off the RLC Info */
  if ((temp_ie_ptr->rlc_InfoChoice.t == T_rrc_RLC_InfoChoice_r7_rlc_Info) && 
      (get_rlc_info_r7(wrrc_as_id,temp_ie_ptr->rlc_InfoChoice.u.rlc_Info, 
        local_rb_id, TRUE) == FAILURE))
  {
    return FAILURE;
  }
  ...
  /* Use the IE Pointer that came as a parameter to get the RB Mapping info */
  if (get_rb_mapping_info_r8(wrrc_as_id, &ie_ptr->rb_MappingInfo, local_rb_id, logch_mode
     , one_sided_reest_present,one_sided_reest,TRUE) == FAILURE)
  {
    return FAILURE;
  }
...
)  
{
  ...
  if (rb_info_reconfig_list_present)
  {
    ...
    while((rb_reconfig_ptr->n > idx) && (idx < MAX_RB))
    {
      if(get_rb_info_to_reconfig_r8(wrrc_as_id,&rb_reconfig_ptr->elem[idx]) == FAILURE)
      {
        return FAILURE;
      }
#ifdef FEATURE_WCDMA_DEFAULT_CFG_CELLFACH
      if ((rb_reconfig_ptr->elem[idx].rb_Identity) && (rb_reconfig_ptr->elem[idx].rb_Identity <= NUM_SRB_DC_CELLFACH))
      {rrcdata_misc_info.default_cfg_recfg_list_srb_present[rb_reconfig_ptr->elem[idx].rb_Identity-1] = TRUE;}
#endif
      idx++;
    } /* end RB reconfig */
  }
...
if ((RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_RB_InformationReconfig_r8,rlc_Info)) && (ie_ptr->rb_Identity >= DCCH_AM_RADIO_BEARER_ID))
{
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_RB_InformationReconfig_r8,rb_MappingInfo))
  {
    ...
    if (get_rb_mapping_info_r8(wrrc_as_id, &ie_ptr->rb_MappingInfo, (rb_id_type)ie_ptr->rb_Identity, logch_mode
       , one_sided_reest_present,one_sided_reest,FALSE) == FAILURE)
    {
      return FAILURE;
    }
...
)  
{
  ...
  if(rb_info_affected_list_present)
  {
    ...
    while((rb_affected_ptr->n > idx) && (idx < MAX_RB))
    {
      if(get_rb_info_affected_r8(wrrc_as_id, &rb_affected_ptr->elem[idx]) == FAILURE)
      {
        return (FAILURE);
      }
#ifdef FEATURE_WCDMA_DEFAULT_CFG_CELLFACH
      if ((rb_affected_ptr->elem[idx].rb_Identity) && (rb_affected_ptr->elem[idx].rb_Identity <= NUM_SRB_DC_CELLFACH))
      {rrcdata_misc_info.default_cfg_recfg_list_srb_present[rb_affected_ptr->elem[idx].rb_Identity-1] = TRUE;}
#endif
      idx++;
    }
  } /* end Affected RB */
...
)
{
  ...
  /* Get the RB Mapping info */
  return(get_rb_mapping_info_r8(wrrc_as_id, &ie_ptr->rb_MappingInfo, 
                                ie_ptr->rb_Identity, 
                                UE_MODE_MAX_NUM
                                ,FALSE,FALSE,FALSE));
...
)
{
  ...
  /* Save the RLC Size List information */
#if defined(FEATURE_RRC_TFC_RLC_SIZE_FOR_DCH)
  if(get_rlc_size_list_r8(wrrc_as_id, ie_ptr, &rrc_ordered_hanging_rb_mapping[idx].ul_mapping_info.ul_dch_info.size_rest_info, rb_id) == FAILURE)
  {
    return FAILURE;
  }
#endif
...
)
{
  ...
  if (mac_dflow_index == UE_MAX_MAC_D_FLOW)
  {
    ...
    return(rrcllc_backup_eul_from_ota_r8(wrrc_as_id, ie_ptr, rb_id, logch_mode));
  }
  else
  {
  ...
  else if (ie_ptr->ul_TrCH_Type.u.e_dch->rlc_PDU_Size.t == T_rrc_UL_LogicalChannelMapping_r8_rlc_PDU_Size_flexibleSize)
  {
    ...
    if(logch_mode == UE_MODE_ACKNOWLEDGED_DATA)
    {
      if(ie_ptr->ul_TrCH_Type.u.e_dch->rlc_PDU_Size.u.flexibleSize->m.lengthIndicatorSizePresent)
      {
        ...
        */
          entity_idx = get_am_index_in_config_ptr(wrrc_as_id, rb_id,oc_config_ptr);
          ...
          if(entity_idx < UE_MAX_AM_ENTITY)
          {
            ...
            if( ( (largestRLC_PDU_Size > 126) && (special_value_he_config == FALSE) ) || 
                (minRLC_PDU_Size > 126) )
            {
              WRRC_MSG3_ERROR("LI is 7, special_value_he_config %d largestRLC_PDU_Size: %d octets, minRLC_PDU_Size: %d octets ", 
...
)
{
  ...
  if(srb_setup_r8_ptr != NULL)
  {
    ...
    trch_idx = rrcllc_find_dl_dch_trch_idx(wrrc_as_id, (tr_ch_id_type)srb5_trch_id);
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_update_hs_rach_params(sys_modem_as_id_e_type wrrc_as_id,ordered_config_type* config_ptr)
{
  ...
  rrcllc_init_mac_hsrach_mapping_info(wrrc_as_id,&config_ptr->mac_hs_rach_config);
  ...
  for(i=0;i<MAX_HANGING_RB_MAPPING_INFO;i++)
  {
         ...
         (rrc_ordered_hanging_rb_mapping[i].ul_mapping_info.ul_mace.valid == TRUE))
    {
      ...
      if((index = rrchsrach_find_e_mac_d_flow_idx(wrrc_as_id, config_ptr,(uint8)rrc_ordered_hanging_rb_mapping[i].ul_mapping_info.ul_mace.mac_e_flow_id)) == MAX_E_DCH_MAC_D_FLOW)
      {
      ...
      if (UE_MODE_MAX_NUM == logch_mode)
      {
        if (rrclcm_get_ul_rlc_mode(wrrc_as_id, rlc_lc_id, &logch_mode) == RRCLCM_FAILURE)
        {
          if(get_am_index_in_config_ptr(wrrc_as_id,rrc_ordered_hanging_rb_mapping[i].rb_id,config_ptr) != UE_MAX_AM_ENTITY)
        {
          logch_mode = UE_MODE_ACKNOWLEDGED_DATA;
          }           
          else if(get_um_ul_index(wrrc_as_id, (rrc_RB_Identity)rrc_ordered_hanging_rb_mapping[i].rb_id,config_ptr)!= 
            UE_MAX_UL_UM_CHANNEL)         
          {
            logch_mode = UE_MODE_UNACKNOWLEDGED;
          }
          else if(get_ul_tm_index_in_config_ptr(wrrc_as_id,rlc_lc_id,config_ptr)!= UE_MAX_UL_TM_CHANNEL)
          {
            logch_mode = UE_MODE_TRANSPARENT;
          }
          else
...
)
{
  ...
  switch (ie_ptr->ul_TransportChannelType.t)
  {
    ...
    case T_rrc_UL_TransportChannelType_rach:
      ...
      if(get_rlc_size_list(wrrc_as_id, ie_ptr,&ul_map->rach_map_info.size_rest_info,
    ...
    switch (ie_ptr->ul_TrCH_Type.u.dch_rach_usch->ul_TransportChannelType.t)
    {
      ...
      case T_rrc_UL_TransportChannelType_rach:
        ...
        if(ie_ptr->ul_TrCH_Type.u.dch_rach_usch->m.logicalChannelIdentityPresent)
          ...
          ul_map->rach_map_info.log_channel_id = 0;
        if(get_rlc_size_list_r6(wrrc_as_id,ie_ptr,&ul_map->rach_map_info.size_rest_info,
                             rrc_ordered_hanging_rb[hrb_index].rb_id) == FAILURE)
        {
          memset(ul_map,0,sizeof(ul_mapping_info_type_));
          return INVALID_MAPPING;
    ...
    switch (ie_ptr->ul_TrCH_Type.u.dch_rach_usch->ul_TransportChannelType.t)
    {
      ...
      case T_rrc_UL_TransportChannelType_rach:
        ...
        if(ie_ptr->ul_TrCH_Type.u.dch_rach_usch->m.logicalChannelIdentityPresent)
          ...
          ul_map->rach_map_info.log_channel_id = 0;
        if(get_rlc_size_list_r8(wrrc_as_id, ie_ptr,&ul_map->rach_map_info.size_rest_info,
                             rrc_ordered_hanging_rb[hrb_index].rb_id) == FAILURE)
        {
          memset(ul_map,0,sizeof(ul_mapping_info_type_));
          return INVALID_MAPPING;
...
)
{
  ...
  if(rrc_ordered_hanging_rb[rb_index].mapping_info[hrb_index].dl_map.macehs_map_info.valid == TRUE )
  {
    ...
    lc_index = rrcllc_find_lc_index_for_efach(wrrc_as_id, config_ptr, logicalChannelIdentity,rb_id);
    if(lc_index == UE_MAX_DL_LOGICAL_CHANNEL)
    {
       /* Allocate LC index */
       lc_index = rrcllc_allocate_lc_index_efach(wrrc_as_id, config_ptr, logicalChannelIdentity);
    ...
    else
    {
      ...
      {
        WRRC_MSG3_ERROR("EFACH:LCH-ID %d already present for RB %d and duplicated for Rb %d",
    ...
    else if (rrclcm_get_dl_rlc_mode(wrrc_as_id, rlc_lc_id, &logch_mode) == RRCLCM_FAILURE)
    {
    ...
    WRRC_MSG4_HIGH("EFACH:Restore EHS mapping of RB-ID:%d, MACEHS: Queue ID %d mapped to LC id %d, lc_index %d",
    rb_id, config_ptr->efach_dl_logchan_info[lc_index].queue_id,config_ptr->efach_dl_logchan_info[lc_index].mac_id,lc_index);

  }
  else return FALSE;


  rrcllc_init_hsdpa_l1_queue(wrrc_as_id, config_ptr);
...
)
{
  ...
  if((rach_mapping !=NULL) && (rach_mapping->valid == TRUE))
  {
    ...
    if(rrclcm_get_ul_rlc_mode(wrrc_as_id, rlc_lc_id, &logch_mode) == RRCLCM_FAILURE)
    {
      ...
      else if(get_ul_um_index(wrrc_as_id,(rrc_RB_Identity)rb_id) != UE_MAX_DL_UM_CHANNEL) 
      {
        logch_mode = UE_MODE_UNACKNOWLEDGED;
      }
      else if(get_ul_tm_index(wrrc_as_id,rlc_lc_id)!= UE_MAX_DL_TM_CHANNEL)
      {
        logch_mode = UE_MODE_TRANSPARENT;
      }
      else
...
)
{
  ...
  if(fach_map_info->valid == TRUE)
  {
    ...
    if(rrclcm_get_dl_rlc_mode(wrrc_as_id, rlc_lc_id, &logch_mode) == RRCLCM_FAILURE)
    {
      ...
      else if(get_dl_um_index(wrrc_as_id, (rrc_RB_Identity)rb_id, 
...
)
{
  ...
  for(;comboCounter<MAX_MAPPING_COMBO;comboCounter++)
  {
    ...
    /*WRRC_MSG5_HIGH("RRC_HRB: UL:%d \tDL:%d \tRB-ID %d \thrb_index %d and combo_index %d",
               print_ul_type(combo_type),
               print_dl_type(combo_type),
               rrc_ordered_hanging_rb[hrb_index].rb_id,
...
)
{
  ...
  for(;mapping_cntr_combo < MAX_MAPPING_COMBO; mapping_cntr_combo++)
  {
    ...
    {
      if(rrcenhstate_check_for_queue_id(wrrc_as_id, transition_config.toc_ptr,
    ...
    {
#ifdef FEATURE_WCDMA_HS_RACH
      if(rrchsrach_check_for_edch_flow_in_sib5(wrrc_as_id, rrc_ordered_hanging_rb[hanging_rb_index].mapping_info[mapping_cntr_combo].ul_map.edch_map_info.mac_e_flow_id) == FALSE)
      {
...
uint8 rrcllc_get_rb_index_in_hanging_rb(sys_modem_as_id_e_type wrrc_as_id,rrc_RB_Identity rb_id)
{
...
)
{
  ...
  hrb_index = rrcllc_find_or_allocate_index_hanging(wrrc_as_id, rb_id);
  ...
  {
    case REL_99:
      status= rrcllc_backup_mapping_info_into_hanging_R99(wrrc_as_id, ie_ptr, hrb_index);
      break;
    case REL_5:
      status= rrcllc_backup_mapping_info_into_hanging_R5(wrrc_as_id, ie_ptr, hrb_index);
      break;
    case REL_6:
      status= rrcllc_backup_mapping_info_into_hanging_R6(wrrc_as_id, ie_ptr, hrb_index);
      break;
    case REL_7:
      status= rrcllc_backup_mapping_info_into_hanging_R7(wrrc_as_id, ie_ptr, hrb_index);
      break;
    case REL_8:
      status= rrcllc_backup_mapping_info_into_hanging_R8(wrrc_as_id, ie_ptr,hrb_index);
      break;
    default:
      status = FAILURE;
...
)
{
  ...
  for(;(ie_ptr->n > rb_mux_cnt) && (rb_mux_cnt < MAX_RB_MUX_OPTIONS);rb_mux_cnt++)
  {
    ...
    {
      ul_mapping_type |= get_ul_mapping_r99_hrb(wrrc_as_id, &ie_ptr->elem[rb_mux_cnt].ul_LogicalChannelMappings,&ul_map,hrb_index);
    }
    ...
    if(RRC_MSG_COMMON_BITMASK_IE(ie_ptr->elem[rb_mux_cnt],rrc_RB_MappingOption,dl_LogicalChannelMappingList))
    {
      ...
      for(lc_tc_cnt =0 ;(dl_list->n > lc_tc_cnt) && (lc_tc_cnt < MAX_DED_LOGCHAN_PER_TRCH);lc_tc_cnt++)
      {
        ...
        dl_mapping_type |= get_dl_mapping_r99_hrb(wrrc_as_id,&dl_list->elem[lc_tc_cnt],&dl_map,hrb_index);
        rrcllc_store_mapping_in_hanging_rb(wrrc_as_id, hrb_index,(dl_mapping_type|ul_mapping_type),&ul_map,&dl_map);
      }
    }
  } /* end loop of RB Mapping info list */
  return SUCCESS;
  ...
  for(;(ie_ptr->n > rb_mux_cnt) && (rb_mux_cnt < MAX_RB_MUX_OPTIONS_REL5);rb_mux_cnt++)
  {
    ...
    if(RRC_MSG_COMMON_BITMASK_IE(ie_ptr->elem[rb_mux_cnt],rrc_RB_MappingOption,dl_LogicalChannelMappingList))
    {
      ...
      for(lc_tc_cnt =0 ;(dl_list->n > lc_tc_cnt) && (lc_tc_cnt < MAX_DED_LOGCHAN_PER_TRCH);lc_tc_cnt++)
      {
        ...
        dl_mapping_type |= get_dl_mapping_r5_hrb(wrrc_as_id,&dl_list->elem[lc_tc_cnt],&dl_map,hrb_index);
        rrcllc_store_mapping_in_hanging_rb(wrrc_as_id, hrb_index, (dl_mapping_type|ul_mapping_type), &ul_map, &dl_map);
      }
    }
  } /* end loop of RB Mapping info list */
...
)
{
  ...
  for(;(ie_ptr->n > rb_mux_cnt) && (rb_mux_cnt < MAX_RB_MUX_OPTIONS_REL5);rb_mux_cnt++)
  {
    ...
    {
      ul_mapping_type = get_ul_mapping_r6_hrb(wrrc_as_id, &ie_ptr->elem[rb_mux_cnt].ul_LogicalChannelMappings,&ul_map,hrb_index);
    }
  ...
  for(;(ie_ptr->n > rb_mux_cnt) && (rb_mux_cnt < MAX_RB_MUX_OPTIONS_REL5);rb_mux_cnt++)
  {
    ...
    if(RRC_MSG_COMMON_BITMASK_IE(ie_ptr->elem[rb_mux_cnt],rrc_RB_MappingOption,dl_LogicalChannelMappingList))
    {
      ...
      for(lc_tc_cnt =0 ;(dl_list->n > lc_tc_cnt) && (lc_tc_cnt < MAX_DED_LOGCHAN_PER_TRCH);lc_tc_cnt++)
      {
        ...
        dl_mapping_type |= get_dl_mapping_r7_hrb(wrrc_as_id, &dl_list->elem[lc_tc_cnt],&dl_map,hrb_index);
        rrcllc_store_mapping_in_hanging_rb(wrrc_as_id, hrb_index, (dl_mapping_type|ul_mapping_type), &ul_map, &dl_map);
      }
    }
  } /* end loop of RB Mapping info list */
...
)
{
  ...
  for(;(ie_ptr->n > rb_mux_cnt) && (rb_mux_cnt < MAX_RB_MUX_OPTIONS_REL5);rb_mux_cnt++)
  {
    ...
    {
      ul_mapping_type = get_ul_mapping_r8_hrb(wrrc_as_id, &ie_ptr->elem[rb_mux_cnt].ul_LogicalChannelMappings, &ul_map, hrb_index);
    }
...
)
{
  ...
  {
    rrcenhstate_init_efach_mapping(wrrc_as_id,config_ptr);
  }
      ...
      if(rrc_ordered_hanging_rb[rb_index].mapping_info[hrb_index].combo_type == combo)
      {
        switch(combo & 0xF0)
        {
          case UL_RACH_MAPPING:
            ret_val = rrcllc_restore_rach_mapping(wrrc_as_id, config_ptr, rb_index, hrb_index);
            break;
          default:
            break;
        }
        switch(combo & 0xF)
        {
          case DL_MAC_EHS_MAPPING:
            ret_val = rrcllc_restore_macehs_mapping(wrrc_as_id, config_ptr, rb_index, hrb_index);
            break;
          case DL_FACH_MAPPING:
            ret_val = rrcllc_restore_fach_rb_mapping(wrrc_as_id, config_ptr, rb_index, hrb_index);
          default:
            break;
        }
      }
...
)
{
  ...
  for(;index<MAX_HANGING_RB_MAPPING_INFO;index++)
  {
    ...
    rrc_ordered_hanging_rb[index].rb_id = RRC_INVALID_RB_ID;
...
)
{
 ...
 for(rb_index=0;rb_index<MAX_HANGING_RB_MAPPING_INFO;rb_index++)
  {
    rrc_ordered_hanging_rb[rb_index].rb_id = RRC_INVALID_RB_ID;
    rrc_ordered_hanging_rb[rb_index].logch_mode = UE_MODE_MAX_NUM;
