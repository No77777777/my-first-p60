...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcrcr.c#5 $ $DateTime: 2021/12/17 02:27:47 $ $Author: pwbldsvc $
...
04/21/16   vs      Fixing P2 level KW errors
03/14/16   sp      Made changes to map connection release cause to qsh release cause
...
LOCAL rrcrcr_int_data_type rrcrcr_int_data_sim[MAX_RRC_STACKS];
#define rrcrcr_int_data rrcrcr_int_data_sim[WAS_ID_M]
#else
LOCAL rrcrcr_int_data_type rrcrcr_int_data;
#endif
/*===========================================================================
...
)
{
  ...
  rrcrcr_int_data.rrcrcr_substate = RRCRCR_INITIAL; 
  /* The following code is added since de-registration mechanism is not place.
     ...
     change manager */
  rrcrcr_int_data.rrc_ifreq_rdr_in_prgrs = FALSE;
  rrcscmgr_register_for_scn(wrrc_as_id,RRC_PROCEDURE_RCR,
                            RRC_STATE_WILDCARD,
...
)
{
  ...
  rrc_crash_debug_save_proc_substates(wrrc_as_id,RRC_PROCEDURE_RCR,rrcrcr_int_data.rrcrcr_substate,cmd_ptr->cmd_hdr.cmd_id);

  switch (rrc_state)
  {
    case RRC_STATE_DISCONNECTED:
      rrcrcr_process_disconnected_state(wrrc_as_id, cmd_ptr);
      break;

    case RRC_STATE_CELL_FACH:    
      rrcrcr_process_cell_fach_state(wrrc_as_id, cmd_ptr); 
      break;

    case RRC_STATE_CELL_DCH:     
      rrcrcr_process_cell_dch_state(wrrc_as_id, cmd_ptr); 
      break;
...
{
  switch (rrcrcr_int_data.rrcrcr_substate)
  {
    case RRCRCR_INITIAL: 
      rrcrcr_process_rcr_initial_substate(wrrc_as_id, cmd_ptr);
      break;
  
    case RRCRCR_WAIT_FOR_CELL_UPDATE_COMP_CNF:
      rrcrcr_process_rcr_wait_for_cell_update_comp_cnf_substate(wrrc_as_id, cmd_ptr);
      break;

    case RRCRCR_WAIT_FOR_L2ACK_FOR_RRC_CONN_REL_COMPLETE:
      rrcrcr_process_rcr_wait_for_l2ack_for_rrc_conn_rel_complete_substate(wrrc_as_id, cmd_ptr);
      break;

    case RRCRCR_WAIT_FOR_SRB_RELEASE_CNF:
      rrcrcr_process_rcr_wait_for_srb_release_cnf_substate(wrrc_as_id, cmd_ptr);
      break;

    default:     
      WRRC_MSG1_HIGH("Invalid substate in CELL_FACH state: %d",
                rrcrcr_int_data.rrcrcr_substate);
     break;
  }  
...
)
{
  switch (rrcrcr_int_data.rrcrcr_substate)
  {
    ...
    case RRCRCR_WAIT_FOR_T308_TIMER_EXPIRE:
      rrcrcr_process_rcr_wait_for_t308_timer_expire_substate(wrrc_as_id, cmd_ptr);
      break;
  ...
  switch (rrcrcr_int_data.rrcrcr_substate)
  {
    ...
    default:     
      WRRC_MSG2_HIGH("In Idle mode, Invalid cmd %d in RCR substate %d",
                cmd_ptr->cmd_hdr.cmd_id,rrcrcr_int_data.rrcrcr_substate);
     break;
  }  
} /* rrcrcr_process_disconnected_state */
...
/*================================================================
FUNCTION          RRCRCR_SSGCCS_SEND_RRC_CONNECTION_RELEASE_INFO
...
=================================================================*/
void rrcrcr_ssgccs_send_rrc_connection_release_info(sys_modem_as_id_e_type wrrc_as_id)
{
  wcdma_ssgccs_rrc_connection_release_info_ind_s connection_release_info;
  if(connection_release_3g_msg_handler == NULL)
  {
    WRRC_MSG0_ERROR("SSGCCS: Connection release message handler is NULL");
    return;
  }
  memset(&connection_release_info,0,sizeof(wcdma_ssgccs_rrc_connection_release_info_ind_s));
  connection_release_info.wrrc_as_id = wrrc_as_id;
  connection_release_info.mcc = RRC_CSP_GET_INT_MCC(rrc_csp_int_data.curr_acq_entry.plmn_id);
  connection_release_info.mnc = RRC_CSP_GET_INT_MNC(rrc_csp_int_data.curr_acq_entry.plmn_id);
  connection_release_info.cellid = rrc_csp_int_data.curr_acq_entry.cell_id;
  connection_release_info.rrc_release_cause = rrcdata_misc_info.rrc_release_cause;
  connection_release_info.redirection_info_present = rrcrcr_int_data.redirection_info_present;
  if(connection_release_info.redirection_info_present == TRUE)
  {
    connection_release_info.redirected_rat = rrcrcr_int_data.saved_redirect_info.choice;
  }

  WRRC_MSG0_HIGH("SSGCCS: RRC Connection release info sent to SSGCCS QMI Client");
  connection_release_3g_msg_handler(&connection_release_info);
}
    ...
    if (msg_ptr != NULL)
    {
      ...
      if(RRCRCR_INCLUDE_ERROR_INDICATION == error_ind_status)
      {
       RRC_MSG_COMMON_SET_BITMASK_IE_TYPE2(msg_ptr->message.u.rrcConnectionReleaseComplete,
        ...
        msg_ptr->message.u.rrcConnectionReleaseComplete.errorIndication.u.protocolError->
         diagnosticsType.t = rrcdata_logging_info.rrc_protocol_error_information.diagnosticsType.t;
        msg_ptr->message.u.rrcConnectionReleaseComplete.errorIndication.u.protocolError->
         diagnosticsType.u.type1 = rrcdata_logging_info.rrc_protocol_error_information.diagnosticsType.u.type1;
        ...
        rrc_log_protocol_error(wrrc_as_id, (uint8) (rrc_get_state(wrrc_as_id)),
                                ...
                                (uint8) rrcdata_logging_info.rrc_protocol_error_information.diagnosticsType.u.type1->protocolErrorCause                               
                               );
      }
    
      /* Set the Transaction Id */
      msg_ptr->message.u.rrcConnectionReleaseComplete.rrc_TransactionIdentifier =
        rrcrcr_int_data.rrcrcr_transaction_identifier;
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
      rrcqsh_log_uplink_dcch_ota_params(wrrc_as_id, WRRC_UL_DCCH_MESSAGETYPE_RRCCONNECTIONRELEASECOMPLETE, msg_ptr);
#endif
      ...
      {
        if(RRCSEND_SUCCESS ==
           rrcsend_send_ul_sdu(wrrc_as_id,RRC_PROCEDURE_RCR,msg_ptr,ul_lc_id,
                            RRCSEND_L2ACK_NOT_REQUIRED))
        {
          status = RRCRCR_SUCCESS;        

          /* start the timer T308 */
          rrctmr_start_timer(wrrc_as_id,RRCTMR_T_308_TIMER, 
                        rrcrcr_convert_t308_to_ms(wrrc_as_id, RRC_GET_T308()) );  
        }
      ...
      else if (rb_id == DCCH_AM_RADIO_BEARER_ID)
      {
           ...
           RRCSEND_L2ACK_REQUIRED))
        {
          ...
          rrcdata_logging_info.rrc_protocol_error_information.diagnosticsType.u.type1 = NULL;

          /* Start the sanity timer for L2 ACK */
          rrctmr_start_timer(wrrc_as_id,RRCTMR_RCR_L2ACK_TIMER, 
...
)
{
  ...
  uint32 idx, idy;
  uint32 earfcn;
  rrcrcr_int_data.redirection_info_present = TRUE;

  rrcrcr_int_data.saved_redirect_info.choice = RRC_INTER_SYSTEM_LTE;
  rrcrcr_int_data.saved_redirect_info.u.EUTRA_target_cell_info.earfcn_count = 0;

#ifdef FEATURE_DUAL_SIM
 /* Avoid redirection to L when Fallback is active with w-pref*/
  if((rrc_dsds_int_data.fallback_type == TRM_FALLBACK_UMTS_PREF))
  {
    WRRC_MSG0_HIGH("Ignore InterRAT Rdrct to LTE when fallback is active with W-Pref");
    rrcrcr_int_data.redirection_info_present = FALSE;
    return;
  }  
#endif
  idx = 0;
          ...
          (earfcn_count < LTE_RRC_IRAT_MAX_REDIR_FREQ))
  {
    ...
    rrcrcr_int_data.saved_redirect_info.u.EUTRA_target_cell_info.earfcn_info[earfcn_count].earfcn = EUTRA_target_freq_info_ptr->elem[idx].dlEUTRACarrierFreq;
    ...
    if(EUTRA_target_freq_info_ptr->elem[idx].m.eutraBlacklistedCellPerFreqListPresent)
    {
      ...
      {
        rrcrcr_int_data.saved_redirect_info.u.EUTRA_target_cell_info.earfcn_info[earfcn_count].blacklisted_cells[blacklisted_cells_count++] = 
           blacklisted_cell_info_ptr->elem[idy].physicalCellIdentity;
        idy++;
      }
    }
    rrcrcr_int_data.saved_redirect_info.u.EUTRA_target_cell_info.earfcn_info[earfcn_count++].blacklisted_cells_count = blacklisted_cells_count;
    idx++;
  }
  rrcrcr_int_data.saved_redirect_info.u.EUTRA_target_cell_info.earfcn_count = earfcn_count;
...
)
{
  if(RRC_MSG_COMMON_BITMASK_IE_PTR(rcr_860ext,rrc_RRCConnectionRelease_v860ext_IEs,redirectionInfo))
  {
    ...
    if(rcr_860ext->redirectionInfo.t == T_rrc_RedirectionInfo_v860ext_interRATInfo)
    {
      ...
      {
        rrcrcr_save_lte_redirection_info(wrrc_as_id,&(rcr_860ext->redirectionInfo.u.interRATInfo->u.eutra->eutra_TargetFreqInfoList));
      }
      else
      {
...
)
{
  ...
  switch(redirection_info_ptr->t)
  {
    case T_rrc_RedirectionInfo_r6_frequencyInfo:
      rrcrcr_int_data.saved_redirect_info.u.freq.uarfcn_UL_present = FALSE;
      rrcrcr_int_data.saved_redirect_info.choice = RRC_FREQ_REDIRECTION;
      rrcrcr_int_data.saved_redirect_info.u.freq.uarfcn_DL = (uint16)redirection_info_ptr->u.frequencyInfo->modeSpecificInfo.u.fdd->uarfcn_DL;
  
      if(RRC_MSG_COMMON_BITMASK_IE_TYPE2_PTR(redirection_info_ptr->u.frequencyInfo->modeSpecificInfo.u.fdd,
        uarfcn_UL))
      {
        rrcrcr_int_data.saved_redirect_info.u.freq.uarfcn_UL_present = TRUE;
        rrcrcr_int_data.saved_redirect_info.u.freq.uarfcn_UL = (uint16)redirection_info_ptr->u.frequencyInfo->modeSpecificInfo.u.fdd->uarfcn_UL;
      }
      ...
      acq_entry.freq = rrcrcr_int_data.saved_redirect_info.u.freq.uarfcn_DL;
      rrc_csp_send_l1_cmd(wrrc_as_id,RRC_PROCEDURE_CSP, CPHY_CELL_BAR_REQ, &acq_entry);
      break;
  
    case T_rrc_RedirectionInfo_r6_interRATInfo:
      rrcrcr_int_data.saved_redirect_info.choice = RRC_INTER_SYSTEM_GSM;
      rrcrcr_int_data.saved_redirect_info.u.GSM_cell_info_list.size = 0;
      if(RRC_CHECK_MSG_TYPE_IE_PTR(redirection_info_ptr->u.interRATInfo,gsm_TargetCellInfoListPresent))
      {
       uint32 local_idx=0;
        ...
        while ((GSM_target_cell_info_ptr->n > local_idx) && 
               (rrcrcr_int_data.saved_redirect_info.u.GSM_cell_info_list.size < MAX_RRC_RR_GSM_CELL_LIST_SIZE))
        {
               ...
               != INVALID_BAND)
          {
             ...
             /* Get bcch_ARFCN */
             rrcrcr_int_data.saved_redirect_info.u.GSM_cell_info_list.arfcns[rrcrcr_int_data.saved_redirect_info.u.GSM_cell_info_list.size].num
                = (uint16) GSM_target_cell_info_ptr->elem[local_idx].bcch_ARFCN;
    
             rrcrcr_int_data.saved_redirect_info.u.GSM_cell_info_list.arfcns[rrcrcr_int_data.saved_redirect_info.u.GSM_cell_info_list.size].band
                = gsm_band;
          }
          else
          {
          ...
          {
             rrcrcr_int_data.saved_redirect_info.u.GSM_cell_info_list.BSIC[rrcrcr_int_data.saved_redirect_info.u.GSM_cell_info_list.size].BSIC_NCC 
                    = GSM_target_cell_info_ptr->elem[local_idx].bsic.ncc;
             rrcrcr_int_data.saved_redirect_info.u.GSM_cell_info_list.BSIC[rrcrcr_int_data.saved_redirect_info.u.GSM_cell_info_list.size++].BSIC_BCC 
...
SIDE EFFECTS
  None
===========================================================================*/
wrrc_conn_rel_cause_e_type rrcrcr_map_qsh_release_cause(sys_modem_as_id_e_type wrrc_as_id,rrc_rel_cause_e_type rrc_release_cause)
{
...
static void rrcrcr_qsh_update_release_cause(sys_modem_as_id_e_type wrrc_as_id)
{
  wrrc_call_end_data_log_type call_end_data;
  call_end_data.type = WRRC_CALL_END_TYPE_RRC_CONNECTION_RELEASE;
  call_end_data.cause.conn_rel_cause = rrcrcr_map_qsh_release_cause(wrrc_as_id,rrcdata_misc_info.rrc_release_cause);
...
)
{
  ...
  rrc_plmn_identity_type camped_plmn = rrc_ccm_get_curr_camped_plmn(wrrc_as_id);
  ...
  {     
    case RRC_DOWNLINK_SDU_IND:
      /* Sets the cell reselection to false at the start of RRC Connection
         Release procedure */
      rrcrcr_int_data.rrcrcr_cell_reselection_occured = FALSE;

      /* Set boolean not to include error cause */
      rrcrcr_int_data.rrcrcr_error_ind = RRCRCR_NOT_INCLUDE_ERROR_INDICATION;
            ...
            T_rrc_DL_DCCH_MessageType_rrcConnectionRelease)
        {
          ...
          else /* DCCH ASN1 decoding is successful */
          {
              ...
              T_rrc_RRCConnectionRelease_later_than_r3 )
            {
              ...
              rrcrcr_int_data.rrcrcr_transaction_identifier = dcch_msg_ptr->message.u.
              ...
              if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5)
              {
                  ...
                  T_rrc_RRCConnectionRelease_criticalExtensions_1_r4)
                {
                      ...
                      criticalExtensions.u.r4.m.v4d0NonCriticalExtensionsPresent)
                  {
						...
						criticalExtensions.u.r4.v4d0NonCriticalExtensions.m.v690NonCriticalExtensionsPresent)
                    {
                          ...
                          rrcConnectionRelease_v690ext.m.redirectionInfo_v690extPresent)
                      {
                        ...
                        {
                           rrcrcr_save_redirection_info(wrrc_as_id,local_redirect_info_ptr);
                        }
                      }
                    }
                  }
				  	                                    ...
				  	                                    criticalExtensions.u.r4.rrcConnectionRelease_r4.releaseCause)
                  {
                    ...
                    rrc_qsh_report_rcr_event(wrrc_as_id);
#endif

                  }
                  /* Update RRC connection release cause */
                  rrcdata_misc_info.rrc_release_cause = 
                    rrcrcr_update_release_cause(wrrc_as_id,dcch_msg_ptr->message.u.
                    rrcConnectionRelease.u.later_than_r3.criticalExtensions.u.r4.
                   rrcConnectionRelease_r4.releaseCause);
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
                  rrcrcr_qsh_update_release_cause(wrrc_as_id);
#endif
                    ...
                    &&(rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL7))         
                  {
                    ...
                    {
                      rrcrcr_process_690non_critical_ext(wrrc_as_id,&dcch_msg_ptr->message.u.rrcConnectionRelease.u.later_than_r3.criticalExtensions.u.r4.
                  ...
                  if(rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL8)
                  {
                    ...
                    {
                      rrcrcr_process_860non_critical_ext(wrrc_as_id,&dcch_msg_ptr->message.u.rrcConnectionRelease.u.later_than_r3.criticalExtensions.u.r4.
                ...
                else
                {
                  ...
                  rrcdata_logging_info.rrc_protocol_error_information.diagnosticsType.t = 
                  T_rrc_ProtocolErrorInformation_diagnosticsType_type1;
                  
                  rrcdata_logging_info.rrc_protocol_error_information.diagnosticsType.u.type1 = 
                    rtxMemAllocTypeZ(&enc_ctxt,rrc_ProtocolErrorInformation_type1);

                  rrcdata_logging_info.rrc_protocol_error_information.diagnosticsType.u.type1->protocolErrorCause = 
                    rrc_ProtocolErrorCause_messageExtensionNotComprehended ;

                  rrcrcr_int_data.rrcrcr_n_308 = 0;
                  rrcrcr_int_data.rrcrcr_error_ind = RRCRCR_INCLUDE_ERROR_INDICATION;
                }
              }
              else
              {
            ...
            else
            {
                 ...
                 && (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL7))
              {
                if((RRC_MSG_COMMON_BITMASK_IE(dcch_msg_ptr->message.u.rrcConnectionRelease.u.r3,
                    ...
                    (dcch_msg_ptr->message.u.rrcConnectionRelease.u.r3.laterNonCriticalExtensions.m.
                     v690NonCriticalExtensionsPresent))
                {
                  rrcrcr_process_690non_critical_ext(wrrc_as_id,(rrc_RRCConnectionRelease_v690NonCriticalExtensions_1 *) (&dcch_msg_ptr->message.u.rrcConnectionRelease.u.r3.
              ...
              if(rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL8)
              {
                ...
                {
                  rrcrcr_process_860non_critical_ext(wrrc_as_id,&dcch_msg_ptr->message.u.rrcConnectionRelease.u.r3.laterNonCriticalExtensions.
            ...
            if(local_rplmn_information != NULL)
            {
              ...
              while((fdd_freq_list_ptr != NULL) && (fdd_freq_list_ptr->n > local_idx))
              {
                ...
                if(validate_fdd_frequency(wrrc_as_id,
                    &fdd_freq_list_ptr->elem[local_idx], rrc_ccm_get_curr_camped_plmn(wrrc_as_id))
                     == FAILURE)
#else
                if(validate_fdd_frequency(wrrc_as_id,&fdd_freq_list_ptr->elem[local_idx])
                     == FAILURE)
#endif
                {
                ...
                {
                  if(rrc_ccm_get_curr_camped_freq(wrrc_as_id) != (uint16)fdd_freq_list_ptr->elem[local_idx].uarfcn_DL)
                  {
                    if(rrccsp_update_acq_db(wrrc_as_id,(uint16)fdd_freq_list_ptr->elem[local_idx].uarfcn_DL) == SUCCESS)
                    {
                      WRRC_MSG0_HIGH("ACQ entry added to ACQ DB");
                      acq_db_updated = TRUE;
                      rrc_csp_int_data.attempt_freq_only_acq = TRUE;
                    }
                    else
                    {
                ...
                )
              {
                ...
                (void) rrccsp_update_acq_db(wrrc_as_id,rrc_ccm_get_curr_camped_freq(wrrc_as_id));
                acq_db_updated = TRUE;
              }
            ...
            switch(current_state)
            {
              case RRC_STATE_CELL_DCH:  
                /* reset the  counters */
                rrcdata_timers_counters_info.rrc_v_308 = 0;
                  ...
                  T_rrc_RRCConnectionRelease_later_than_r3))
                {
                  ...
                  if(RRCRCR_SUCCESS == 
                    rrcrcr_send_connection_rel_complete_msg(wrrc_as_id,
                    DCCH_UM_RADIO_BEARER_ID,
                    UE_MODE_UNACKNOWLEDGED,
                    rrcrcr_int_data.rrcrcr_error_ind) )
                  /* This function starts the Timer T308 */
                  {
                     /* Increment the number of retransmission */
                     rrcdata_timers_counters_info.rrc_v_308++; 
                     ...
                     if(   ((!acq_db_updated)
                        || ((RRC_CSP_GET_INT_MCC(camped_plmn) != JAPAN_MCC1)
                         ...
                         && (RRC_CSP_GET_INT_MCC(camped_plmn) != JAPAN_MCC3)
                         && (RRC_CSP_GET_INT_MCC(camped_plmn) != JAPAN_MCC4)))
                        && (!rrcrcr_int_data.redirection_info_present)
                       )
                     {
                       rrc_csp_get_aset_scr_pn(wrrc_as_id);
                     }

                     rrcrcr_int_data.rrcrcr_substate = RRCRCR_WAIT_FOR_T308_TIMER_EXPIRE;
                  }
                  else
                  {
                ...
                {
                  WRRC_MSG1_HIGH("N308 is %d", rrcrcr_int_data.rrcrcr_n_308);
              ...
              case RRC_STATE_CELL_FACH:
                ...
                if(RRCCU_NOT_COMPLETED == 
                     rrccu_get_cell_update_complete_status(wrrc_as_id,RRC_PROCEDURE_RCR) )
                {
                  if(   ((!acq_db_updated)
                     || ((RRC_CSP_GET_INT_MCC(camped_plmn) != JAPAN_MCC1)
                         && (RRC_CSP_GET_INT_MCC(camped_plmn) != JAPAN_MCC2)
                  ...
                  rrcrcr_int_data.rrcrcr_substate = RRCRCR_WAIT_FOR_CELL_UPDATE_COMP_CNF;
                }
                else
                {
                  /* Build RRC Connection Release complete message and send the
                ...
                rrc_DL_CCCH_Message_PDU)
      {
        ...
        if(rrcdata_misc_info.rrc_protocol_error_reject != TRUE) /* CCCH ASN1 decoding successful */
        {
          if( message_choice  == 
             T_rrc_DL_CCCH_MessageType_rrcConnectionRelease)
          {
              ...
              T_rrc_RRCConnectionRelease_CCCH_later_than_r3 )
            {
              ...
              if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5)
              {
                ...
                rrc_translate_urnti(wrrc_as_id,&ccch_msg_ptr->message.u.rrcConnectionRelease.
                   ...
                   T_rrc_RRCConnectionRelease_CCCH_criticalExtensions_3_criticalExtensions)
                {
                     ...
                     criticalExtensions.m.groupIdentityPresent)
                  {
                    ...
                    {
                      if(SUCCESS == rrc_compare_group_release_info(wrrc_as_id,grp_rel_info_ptr->data))
                      {
                    ...
                    T_rrc_RRCConnectionRelease_CCCH_criticalExtensions_3_criticalExtensions)
                {
                      ...
                      T_rrc_RRCConnectionRelease_CCCH_criticalExtensions_1_r5)
                  {
                    ...
                    if(rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL8)
                    {
                      ...
                      {
                        rrcrcr_process_860non_critical_ext(wrrc_as_id,&ccch_msg_ptr->message.u.rrcConnectionRelease.u.later_than_r3.
            ...
            else
            {
              ...
              if(rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL8)
              {
                ...
                {
                  rrcrcr_process_860non_critical_ext(wrrc_as_id,&ccch_msg_ptr->message.u.rrcConnectionRelease.u.r3.
                    laterNonCriticalExtensions.v690NonCriticalExtensions.v860NonCriticalExtensions.rrcConnectionRelease_v860ext);
                }
              }
            }

            if(U_RNTI_NOT_PRESENT == rrcllc_get_current_urnti(wrrc_as_id,&u_rnti_local) )
            {
              if(U_RNTI_NOT_PRESENT == rrcllc_get_urnti_from_toc(wrrc_as_id,&u_rnti_local))
              {
            ...
            WRRC_MSG3_HIGH("u_rnti_local %d, u_rnti %d, group_release_match %d is", u_rnti_local, u_rnti, group_release_match);
               ...
               ((FALSE == group_release_present) && (u_rnti_local == u_rnti)))
            {
              ...
              rrcdata_misc_info.rrc_release_cause = rrcrcr_update_release_cause(wrrc_as_id,rrc_release_cause_recvd);
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC              
              rrcrcr_qsh_update_release_cause(wrrc_as_id);
#endif              
              if(!rrcrcr_int_data.redirection_info_present)
              {
                /* Take UE to Idle via CSP by sending init cell sel with tx to discon */
                rrc_transition_to_disconnected_state(wrrc_as_id,RRC_PROCEDURE_RCR, RRC_TX_TO_DISCON_OTHER);
                  ...
                  rrcrcr_int_data.rrcrcr_substate = RRCRCR_WAIT_FOR_SRB_RELEASE_CNF;
                }
              else
              {
                rrcrcr_send_redirection_req(wrrc_as_id);
              }
            }
            else
            {
              ...
              rrc_crash_debug_remove_previous_ota(wrrc_as_id);
            }
            /*lint -restore */
          } 
          else
      ...
      /*Sending RRC connection release info to SSGCCS QMI client*/
       rrcrcr_ssgccs_send_rrc_connection_release_info(wrrc_as_id);

#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
      conn_rel_params.conn_rel_cause=rrcdata_misc_info.rrc_release_cause;
      conn_rel_params.redir_info_present= rrcrcr_int_data.redirection_info_present;
      conn_rel_params.redir_rat= rrcrcr_int_data.saved_redirect_info.choice;
      rrc_qsh_log_conn_rel_params(wrrc_as_id, &conn_rel_params);
#endif
      break;
    case RRC_STATE_CHANGE_IND:
      MSG_MED("Unexpected Event:%d in the substate: %d",
               cmd_ptr->cmd_hdr.cmd_id,rrcrcr_int_data.rrcrcr_substate,0);
      break;
...
)
{
  ...
  {     
    ...
    case RRC_STATE_CHANGE_IND:
      /* stop the timer T308 */
        rrctmr_stop_timer(wrrc_as_id, RRCTMR_T_308_TIMER);
      ...
      {
        WRRC_ERR1_FATAL("RRCSCMGR posted an unregistered event %d",
                   cmd_ptr->cmd.state_change_ind.new_state);
      }
      break;

     case RRC_T308_EXPIRED_IND:
      if(rrcdata_timers_counters_info.rrc_v_308 <= rrcrcr_int_data.rrcrcr_n_308)               
      {        
      ...
      {
	   if((rrcnv_data.rrc_fast_return_to_lte_after_csfb_nv) && (rrc_csp_int_data.csfb_extended_status == CSFB_STARTED))
       {
          if(!((rrc_csp_int_data.cs_rab_established) || ((mm_per_subs_is_ss_in_progress(wrrc_as_id)) && (rrcnv_data.rrc_fast_return_to_lte_after_csfb_ss_nv))))
          {
        rrc_csp_int_data.csfb_extended_status = CSFB_INVALID;
	     WRRC_MSG0_HIGH("CSFB_HACK: NO CS RAB present, no SS services hence resetting CSFB status");
	  }
  ...
  {     
    ...
    case RRC_CELL_UPDATE_COMPLETE_CNF:
      ...
      {                
        ...
        {
          WRRC_ERR0_FATAL("Unable to send RRC Conn Rel Complete");
        }
      }
      {
...
)
{
  ...
  {     
        ...
        {                 /* CCCH ASN1 decoding successful */
             ...
             T_rrc_DL_CCCH_MessageType_rrcConnectionRelease)
          {
            ...
            if (rrc_ReleaseCause_unspecified == rrc_release_cause_recvd)
            {
              ...
              rrctmr_stop_timer(wrrc_as_id, RRCTMR_RCR_L2ACK_TIMER);
    ...
    case RRC_NEW_CELL_IND:
      ...
      rrcrcr_int_data.rrcrcr_cell_reselection_occured = TRUE;
      MSG_LOW("New cell ind %d", rrcrcr_int_data.rrcrcr_cell_reselection_occured,0,0);
      break;
...
)
{
  ...
  /* Resets the RCR state machine */
  if (rrcrcr_int_data.rrcrcr_substate == RRCRCR_INITIAL)
  {
    return FALSE;
  }
  else
  {
    WRRC_MSG2_HIGH("RRCRCR st:%d, rrc_st:%d", rrcrcr_int_data.rrcrcr_substate, rrc_get_state(wrrc_as_id));
    return TRUE;
  }
...
{
#ifdef FEATURE_DUAL_SIM
  if((!rrc_dsds_int_data.rrc_is_sim_id_valid) || (wrrc_as_id != rrc_dsds_int_data.rrc_sim_id_recvd))
  {
    return FALSE;
  }
#endif
...
{
  return rrcrcr_int_data.rrc_ifreq_rdr_in_prgrs;
}
...
)
{
  ...
  {
    rrcmisc_int_data.rrc_high_mobility_ind_in_rcr = TRUE;
  }
  else
  {
    rrcmisc_int_data.rrc_high_mobility_ind_in_rcr = FALSE;
  }
  WRRC_MSG1_HIGH("rrcmisc_int_data.rrc_high_mobility_ind_in_rcr = %d",rrcmisc_int_data.rrc_high_mobility_ind_in_rcr);
}
...
SIDE EFFECTS
  None
===========================================================================*/
void rrcrcr_send_redirection_req(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  switch(rrcrcr_int_data.saved_redirect_info.choice)
  {
    case RRC_FREQ_REDIRECTION: 
      ...
      if((SUCCESS == validate_dl_arfcn( wrrc_as_id, 
                        rrcrcr_int_data.saved_redirect_info.u.freq.uarfcn_DL,
  ...
  #endif
      {
        if(rrccsp_update_acq_db(wrrc_as_id,rrcrcr_int_data.saved_redirect_info.u.freq.uarfcn_DL) == SUCCESS)
        {
      ...
      wtow_redir_event.target_freq = rrcrcr_int_data.saved_redirect_info.u.freq.uarfcn_DL;
      WRRC_EVENT_REPORT_PAYLOAD(EVENT_WCDMA_TO_WCDMA_REDIRECTION_START, sizeof(wtow_redir_event),
                 (void *)&wtow_redir_event);
      rrcrcr_int_data.rrc_ifreq_rdr_in_prgrs = TRUE;
      break;
    ...
    case RRC_INTER_SYSTEM_LTE:
      ...
      if(rrcmcm_is_dualmode_enabled(wrrc_as_id) && rrcrcr_int_data.saved_redirect_info.choice == RRC_INTER_SYSTEM_GSM
      ...
      value */
          ...
          cmd_ptr->cmd.redirect_req.choice = rrcrcr_int_data.saved_redirect_info.choice;
  
          if(rrcrcr_int_data.saved_redirect_info.u.GSM_cell_info_list.size != 0)
          {
            for(i = 0;i < rrcrcr_int_data.saved_redirect_info.u.GSM_cell_info_list.size;i++)
            {
                   /* Get bcch_ARFCN */
                   cmd_ptr->cmd.redirect_req.u.GSM_cell_info_list.arfcns[i].num
                      = rrcrcr_int_data.saved_redirect_info.u.GSM_cell_info_list.arfcns[i].num;
        
                   cmd_ptr->cmd.redirect_req.u.GSM_cell_info_list.arfcns[i].band
                      = rrcrcr_int_data.saved_redirect_info.u.GSM_cell_info_list.arfcns[i].band;
         
                   cmd_ptr->cmd.redirect_req.u.GSM_cell_info_list.BSIC[i].BSIC_NCC 
                      = rrcrcr_int_data.saved_redirect_info.u.GSM_cell_info_list.BSIC[i].BSIC_NCC;
        
                   cmd_ptr->cmd.redirect_req.u.GSM_cell_info_list.BSIC[i].BSIC_BCC
                      = rrcrcr_int_data.saved_redirect_info.u.GSM_cell_info_list.BSIC[i].BSIC_BCC;
            }
            cmd_ptr->cmd.redirect_req.u.GSM_cell_info_list.size = rrcrcr_int_data.saved_redirect_info.u.GSM_cell_info_list.size;
          }
...
#ifdef FEATURE_WCDMA_TO_LTE
      if(rrcmcm_is_lte_mode_enabled(wrrc_as_id) && rrcrcr_int_data.saved_redirect_info.choice == RRC_INTER_SYSTEM_LTE
           && ((rrc_curr_state == RRC_STATE_CELL_DCH) || (rrc_curr_state == RRC_STATE_CELL_FACH))
#ifdef FEATURE_FEMTO_CSG
           && (rrc_csp_int_data.network_select_mode != SYS_NETWORK_SELECTION_MODE_MANUAL_CSG)
#endif
      ...
      value */
          ...
          cmd_ptr->cmd.redirect_req.u.EUTRA_target_cell_info = rrcrcr_int_data.saved_redirect_info.u.EUTRA_target_cell_info;
...
#endif
      {
        ...
        {
          rrc_transition_to_disconnected_state(wrrc_as_id,RRC_PROCEDURE_RCR,
...
none
===========================================================================*/
rrcrcr_substate_e_type  rrcrcr_get_current_sub_state(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcrcr_int_data.rrcrcr_substate;
...
{
  qsh_mdump_collect_high(&rrcrcr_int_data,sizeof(rrcrcr_int_data));
}
#endif
