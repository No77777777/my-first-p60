...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcsend.c#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
10/12/15   nr      Made changes to get EFACH DRX status from config ptr in use
07/30/15   sp      NV changes to control l2 ack wait timer during reselection
06/03/15   sp      Made changes to use DSM API for setting priority.
...
rrc_UL_DCCH_Message *ul_dcch_msg_debug_ptr_sim[MAX_RRC_STACKS] = {NULL};  /* Pointer to Uplink DCCH message */
rrc_UL_CCCH_Message *ul_ccch_msg_debug_ptr_sim[MAX_RRC_STACKS] = {NULL};  /* Pointer to Uplink CCCH message */

/* Following two variables are used to get the ASN.1 decoding/encoding status when
there is any decoding or encoding failure */
rrc_encode_info_type rrc_encode_info_sim[MAX_RRC_STACKS];
rrc_decode_info_type rrc_decode_info_sim[MAX_RRC_STACKS];

#define ul_dcch_msg_debug_ptr ul_dcch_msg_debug_ptr_sim[WAS_ID_M]
#define ul_ccch_msg_debug_ptr ul_ccch_msg_debug_ptr_sim[WAS_ID_M]
...
extern void rrc_get_ul_log_message(sys_modem_as_id_e_type wrrc_as_id, pdu_buf_type encoded_sdu,uint8 channel_type,uint8 message_type,uint8 rb_id,void *msg_ptr,uecomdef_logchan_e_type rrclog_ul_lc);
...
)
{
  ...
  switch(ul_lc)
  {
    case UE_LOGCHAN_DCCH:
      asn1_msg_type = 
        rrc_get_ul_dcch_message_type(wrrc_as_id,encoded_sdu.value,
    ...
    case UE_LOGCHAN_CCCH:
      asn1_msg_type = 
        rrc_get_ul_ccch_message_type(wrrc_as_id,encoded_sdu.value,
                                     (uint32) encoded_sdu.length);
      switch(asn1_msg_type)
      {
        /* The following needs to be updated as and when new RRC
...
)
{
  ...
  (void) rrclcm_get_ul_lc_type(wrrc_as_id,lc_id, &rrclog_ul_lc);

  /* Determine and store RB and channel type info */
  rb_id = (uint8)rrclcm_get_ul_rb_id(wrrc_as_id,lc_id);
  switch(rrclog_ul_lc)
  {
  ...
  sig_msg_event.message_type = rrc_get_ul_message_type(wrrc_as_id,rrclog_ul_lc,
                                                       encoded_sdu);
  asn1_msg_type = rrc_get_ul_asn1_message_type(wrrc_as_id,rrclog_ul_lc,encoded_sdu);	
  rrc_get_ul_log_message(wrrc_as_id,encoded_sdu,channel_type,asn1_msg_type,rb_id,msg_ptr,rrclog_ul_lc);	
  WRRC_EVENT_REPORT_PAYLOAD(EVENT_RRC_MESSAGE_SENT, sizeof(sig_msg_event),
                       (void *)&sig_msg_event);
  ...
  if(log_record_ptr != NULL)
  {
    ...
    if((channel_type == RRCLOG_SIG_UL_CCCH) || (channel_type == RRCLOG_SIG_UL_DCCH))
    {
      /* Fill UARFCN and PSC info */
      if(RRC_STATE_CELL_DCH == rrc_get_state(wrrc_as_id))
      {
        log_record_ptr->uarfcn = rrc_ccm_get_curr_camped_freq(wrrc_as_id);
      ...
      {
        log_record_ptr->uarfcn = rrc_ccm_get_curr_camped_freq(wrrc_as_id);
        log_record_ptr->psc    = rrc_ccm_get_curr_camped_psc(wrrc_as_id);
      }
    }
  ...
  if(sdu_type == RRC_CCCH_UL_SDU)
    {
      msg_choice = rrc_get_ul_ccch_message_type(wrrc_as_id,encoded_sdu.value,
                                     (uint32) encoded_sdu.length);
    }
    else
    {
      msg_choice = rrc_get_ul_dcch_message_type(wrrc_as_id,encoded_sdu.value,
                                     (uint32) encoded_sdu.length);
    }
    rrc_crash_debug_save_ota(wrrc_as_id,(uint8 *) encoded_sdu.value,encoded_sdu.length,sdu_type,msg_choice,rrc_procedure);
...
{  
  ...
  if(RRCLCM_SUCCESS == rrclcm_get_ul_lc_type(wrrc_as_id,lc_id, &ul_lc_ch) )
  {
    switch (ul_lc_ch)
    {
      case UE_LOGCHAN_CCCH:
        ...
        if(RRCSMC_IP_NOT_STARTED == rrcsmc_check_integrity_status(wrrc_as_id) ||
      ...
      case UE_LOGCHAN_DCCH:
        ...
        else
        {
          ...
          *(ul_dcch_msg->integrityCheckInfo.messageAuthenticationCode.data + 3) = 
            (uint8)rrclcm_get_ul_rb_id(wrrc_as_id,lc_id);
    ...
    if(status != RRCSEND_SUCCESS)
    {
      ...
      */
      if(rrcasn1_free_pdu(wrrc_as_id,(int )pdu_num,msg_ptr)) 
      {
  ...
  else
  {
    ...
    WRRC_ERR1_FATAL("Logical channel is not setup %d",lc_id);
  }
  ...
  encoded_sdu = rrcasn1_encode_pdu(wrrc_as_id,msg_ptr, (int) pdu_num);

  if(encoded_sdu.value !=NULL)              /* Checks here whether encoding is
                                               successful */
  {
    rb_id = rrclcm_get_ul_rb_id (wrrc_as_id, lc_id);  /* Gets the RB Id from RLC LC Id. */

    if (rb_id == CCCH_RADIO_BEARER_ID)
    {
      /* Gets the lowest Transport Format from the LLC to enqueue TM SDU */
      rrc_tm_sdu_size_bits = rrcllc_calculate_ul_tm_ccch_rlc_size(wrrc_as_id);
      /*Gets last octet pad bits*/
      last_octet_pad_len = PU_GETPADBITS(&enc_ctxt);
#ifdef FEATURE_WCDMA_HS_RACH
      /*Dont trim CCCH messages when HS-RACH is on*/
      if(TRUE == rrchsrach_camped_cell_supports_hsrach(wrrc_as_id))
      {
        ...
        ((uint32)((encoded_sdu.length * 8) - last_octet_pad_len) <= rrc_tm_sdu_size_bits))
      {
        ...
        if( ul_ccch_msg != NULL )
        {
             ...
             (ul_ccch_msg->message.t != T_rrc_UL_CCCH_MessageType_rrcConnectionRequest))
          {
            ...
            (void)rrcsmc_append_uplink_integrity_check_info(wrrc_as_id, rrc_tm_sdu_ptr,
      ...
      else if(rrc_tm_sdu_size_bits != RRCLLC_INVALID_RLC_SIZE)
      {
        if( ul_ccch_msg != NULL )
        {
          if(ul_ccch_msg->message.t == T_rrc_UL_CCCH_MessageType_cellUpdate )
          {
            ...
            rrccu_reset_trim_bit_mask(wrrc_as_id);
            WRRC_MSG0_HIGH("CU size big so kick in CU trimming opt....");
            do
            {
              /*Start CU trimming here*/
              if(rrccu_trim_cu_msg_ul_tm_tb_size(wrrc_as_id,msg_ptr,(uint32)encoded_sdu.length) == TRUE)
              {
                /*Lets discard this encoded PDU*/
                rrcasn1_free_buf(wrrc_as_id,encoded_sdu.value);
                ...
                {/*ASN.1 encoding failed*/
                  ...
                  WRRC_ERR2_FATAL("ASN1 encoding failed reason %d [8=out of memory, 17 = asn1 constraint violation , every else others]  PDU no. %d",
          ...
          else if(ul_ccch_msg->message.t == T_rrc_UL_CCCH_MessageType_rrcConnectionRequest)
          {
            ...
            rrcrce_trim_conn_request_msg_ul_tm_tb_size(wrrc_as_id,msg_ptr,(uint32)encoded_sdu.length);
            ...
            if(encoded_sdu.value !=NULL)
            {
              ...
              */
              ...
              {
                WRRC_ERR2_FATAL("Even after trimming, encoded msg length:%d  exceeds:%d",
                          (encoded_sdu.length * 8),
                          rrc_tm_sdu_size_bits);
              }
            }
    ...
    else
    {
      /* Update Integrity Check Info  if Integrity Protection started */
        if(RRCSMC_IP_STARTED == rrcsmc_check_integrity_status(wrrc_as_id) )
        {
           if(rrcsmc_append_uplink_integrity_check_info(wrrc_as_id,
             (byte *) encoded_sdu.value, 
             (uint32) encoded_sdu.length * 8,
             rb_id))
           {
             WRRC_ERR0_FATAL("Integrity append failed");
           }
        }   
      /* rrcsmc_append_uplink_integrity_check_info */
      ...
      {
        rrcueci_ext_data.ueci_message_length  = encoded_sdu.length;
        WRRC_MSG1_HIGH("T304_DEBUG Stored UECI length : %d",rrcueci_ext_data.ueci_message_length );
      }							 
      ...
      {
        dsm_item_set_priority(dsm_item, DSM_HIGHEST);
        if (ul_dcch_msg->message.t == T_rrc_UL_DCCH_MessageType_measurementReport)
        {
          dsm_item_set_priority(dsm_item, DSM_NORMAL);
        }
      }
 #endif
    ...
    rrcsend_log_ul_sig_message(wrrc_as_id,encoded_sdu, lc_id,msg_ptr,rrc_procedure); 
    ...
    {      
      ...
      switch(rb_id)
      {
        ...
        case DCCH_DT_LOW_PRI_RADIO_BEARER_ID:
          if(l2ack_status == RRCSEND_L2ACK_REQUIRED)
          {
            ...
            if(RRCLCM_SUCCESS == rrclcm_get_mui(wrrc_as_id,rrc_procedure,
                                                rb_id, TRUE, &mui))
            {
      ...
      if(status == RRCSEND_SUCCESS)
      {
        dsm_enqueue(rrclcm_get_ul_watermark_ptr(wrrc_as_id,lc_id), &dsm_item);
      } 
      else
      {
  ...
  {  
    ...
    rrc_qsh_report_wait_l2_ack_event(wrrc_as_id, RRC_WAIT_L2_ACK_FOR_PCH_TRANS);
#endif
    if(((rrc_check_if_any_proc_waiting_for_l2_ack_for_pch_transition(wrrc_as_id,&proc_waiting_for_l2_ack)) 
           &&
            (proc_waiting_for_l2_ack  == rrc_procedure)
           &&
            (proc_waiting_for_l2_ack != RRC_PROCEDURE_CU))
          ||
          ((RRC_PROCEDURE_SMC == rrc_procedure) && (RRCSEND_L2ACK_REQUIRED == l2ack_status)))
    {
 #ifdef FEATURE_WCDMA_HS_FACH_DRX
      if(TRUE == rrcenhstate_get_e_fach_drx_status(wrrc_as_id,&(config_ptr->l1_hsdpa_info)))
      {
         /* Set timer value as 850 ms for EDRX*/
         l2_ack_timer_val = rrcnv_data.wcdma_rrc_defer_cel_resel_waiting_for_l2_ack.edrx_cell_nv_value;
      }
      else
 #endif
      {
         l2_ack_timer_val = rrcnv_data.wcdma_rrc_defer_cel_resel_waiting_for_l2_ack.legacy_cell_nv_value;
      }
      if(l2_ack_timer_val > 0)
      {
        rrctmr_start_timer(wrrc_as_id,RRCTMR_WAIT_FOR_L2_ACK_FOR_PCH_TRANSITION_TIMER, l2_ack_timer_val);
      }
    }
  }
...
)
{
  ...
  if (rrcsend_send_ul_sdu(wrrc_as_id,rrc_procedure,msg,lc_id,
