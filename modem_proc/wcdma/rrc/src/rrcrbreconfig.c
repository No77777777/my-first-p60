...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcrbreconfig.c#3 $   
...
09/15/16   sp      Made changes to reset CU registration info once reconfig procedure is successful
...
typedef struct
{
 ...
 /* Indicates if Fre-redirection is to be triggered in FACH->PCH case */
  ...
  boolean rrcrbrc_send_initiate_cu_flag;
  
  boolean rrcrbrc_delete_crnti;
...
LOCAL rrcrbrc_int_data_type rbrc_sim[MAX_RRC_STACKS];
#define rbrc rbrc_sim[WAS_ID_M]
#else
...
#endif
rrcrb_substate_e_type rrcrbrc_get_current_sub_state(sys_modem_as_id_e_type wrrc_as_id)
{
...
/*===========================================================================
 ...
 ===========================================================================*/
 rrc_state_e_type rrcrbrc_get_dest_state(sys_modem_as_id_e_type wrrc_as_id)
 {
   return rbrc.state_indicated_by_msg;
 }
/*===========================================================================

FUNCTION   rrcrbrc_get_initiate_cu_flag()

DESCRIPTION
 This function is used to return rrcrbrc_send_initiate_cu_flag 
...
===========================================================================*/
boolean rrcrbrc_get_initiate_cu_flag(sys_modem_as_id_e_type wrrc_as_id)
{
  return rbrc.rrcrbrc_send_initiate_cu_flag;
}
...
===========================================================================*/
boolean rrcrbrc_is_proc_successful(sys_modem_as_id_e_type wrrc_as_id)
{
...
===========================================================================*/
void rrcrbrc_update_failure_status(sys_modem_as_id_e_type wrrc_as_id,rrcrb_msg_status_e_type status )
{
...
===========================================================================*/
static void rrcrbrc_configure_rab_change(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8 rab_index =0;
...
#endif /*FEATURE_SCUDIF_ENHANCEMENT*/
  {
    ...
    rrcrb_update_amr_cctrch_id_for_reconfig(wrrc_as_id,nas_sync_ind);
  }
}
...
)
{
  ...
  (void)rrcsmc_append_start_list(wrrc_as_id,&msg_ptr->message.u.radioBearerReconfigurationComplete.ul_CounterSynchronisationInfo.startList,
...
)
{
  ...
  /*Reports Radio bearer reconfiguration failure to QSH*/
  rrc_qsh_report_failure_ota_msg_event(wrrc_as_id,WRRC_UL_DCCH_MESSAGETYPE_RADIOBEARERRECONFIGURATIONFAILURE);
#endif

#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
    rrcqsh_log_uplink_dcch_ota_params(wrrc_as_id, WRRC_UL_DCCH_MESSAGETYPE_RADIOBEARERRECONFIGURATIONFAILURE, ptr);
#endif
  ...
  if (ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
    ...
    {
      rrc_update_reconfig_waiting_for_l2ack_dch(wrrc_as_id, RRC_PROCEDURE_RBRC);
      status = rrcsend_send_ul_sdu(wrrc_as_id,RRC_PROCEDURE_RBRC,  /* Procedure is RBRC    */
...
===========================================================================*/     
static void send_rbrc_complete_message( sys_modem_as_id_e_type wrrc_as_id, boolean proc_wait_for_l2_ack )
{
  ...
  {
    rrcrbrc_append_start_list_to_rbrc_complete(wrrc_as_id, ptr);
  }
  else
  {
    if (rrcllc_get_rlc_size_change_status(wrrc_as_id) 
      && ((rbrc.state_indicated_by_msg == RRC_STATE_CELL_PCH || 
          ...
          || (rrc_get_rlc_size_change_status_for_procedure_id(wrrc_as_id,RRC_PROCEDURE_RBRC) == TRUE)))
    {
      ...
      else if (rrc_get_rlc_size_change_status_for_procedure_id(wrrc_as_id,RRC_PROCEDURE_RBRC) == TRUE)
      {
  ...
  if (ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
    ...
    (void) rex_set_timer(&rbrc.rrcrbrc_temp_timer, 5000);   /* 5 seconds */

    rbrc.rrcrbrc_sig = rex_wait(wrrc_as_id, RRCRBRC_TEMP_TIMER_SIG);
    MSG_LOW("RRCRBRC signal:%d", rbrc.rrcrbrc_sig, 0, 0);
    (void) rex_clr_sigs(rex_self(), RRCRBRC_TEMP_TIMER_SIG);
#endif /* FEATURE_RRC_DELAY_TX_ON_NEW_CFG */

#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
    rrcqsh_log_uplink_dcch_ota_params(wrrc_as_id, WRRC_UL_DCCH_MESSAGETYPE_RADIOBEARERRECONFIGURATIONCOMPLETE, ptr);
#endif
...
)
{
  ...
  if (rbrc.cn_info_saved != FALSE)
  {
    ...
    rrc_send_rrc_message_to_mm(wrrc_as_id, rbrc.mm_cmd_ptr);
    /* clear the flag */
...
)
{
  ...
  rrc_translate_nas_sync_indicator(wrrc_as_id,nas_Synchronisation_Indicator); 
   
  if( rbrc.rabs.rabs_in_msg[rab_index].nas_Synchronisation_Indicator == WCDMA_IVOCODER_ID_NONE)
  {
...
#endif
  rrc_set_ordered_codec_mode(wrrc_as_id,rbrc.rabs.rabs_in_msg[rab_index].nas_Synchronisation_Indicator);
  return SUCCESS;
}
 /*===========================================================================
...
)
{
  ...
  {
    rrc_RadioBearerReconfiguration_r10_IEs *rbrc_ptr = &msg_ptr->u.later_than_r3.criticalExtensions.u.
    criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.radioBearerReconfiguration_r10;

    if(RRCRB_R10_MSG_IE_PRESENT(rbrc_ptr,dl_InformationPerRL_List))
    {
  ...
  WRRC_MSG3_HIGH(" RRCRBRECONFIG: Received PSC: %d result:%d for msg_version:%d(0-MSG_REL99,1-MSG_REL5,2-MSG_REL6,3-MSG_REL7,4-MSG_REL8,5-MSG_REL9,6-MSG_REL10,other-MSG_INVALID) ",rrcrbrc_psc,result,msg_version);
  return result;
...
===========================================================================*/
static rrc_rb_search_e_type validate_rb_id(sys_modem_as_id_e_type wrrc_as_id,rrc_RB_Identity rb_id)
{
  ...
  {
    if(rrcllc_has_config_srb5(wrrc_as_id,CURRENT_CONFIG, rb_id) == TRUE)
    {
      rb_srch = RRC_RB_PRESENT;
    }
    else
    {
      ...
      rb_srch = rrc_find_rb_in_est_rabs (wrrc_as_id,rb_id);
    }
  }
...
/*RRC_GKG: Find out why RBE and RBR dedices based on DL Sync Info but RBRC dedices based on URNTI*/
  ...
  else if (msg_version == MSG_REL10)
  {
    if(RRCRB_R10_MSG_IE_PRESENT((&msg_ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.u.
       criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.radioBearerReconfiguration_r10),new_U_RNTI))
    { 
      result = TRUE; 
    }
  }
...
)
{
     ...
     )
  {
          ...
          ( rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH)))
    {
     WRRC_MSG2_ERROR("SRNS reloc with invalid states cur_state %d, msg_state %d",
             rrc_get_state(wrrc_as_id), rbrc.state_indicated_by_msg);
    ...
    else
    {
      ...
      rrc_translate_urnti (wrrc_as_id, &(msg_ptr->u.r3.radioBearerReconfiguration_r3.new_U_RNTI),
                            &rbrc.new_urnti);
    }
   ...
   */
  rbrc.trans_to_dch_ciph_config_needed = FALSE;
  if((rbrc.state_indicated_by_msg == RRC_STATE_CELL_DCH) && ( rrc_get_state(wrrc_as_id) != RRC_STATE_CELL_DCH))
  {
        ...
        if ( rab_list_ptr->elem[rab_index].cn_DomainIdentity  == rrc_CN_DomainIdentity_cs_domain )
        {
          ...
          else
          {
            ...
            WRRC_MSG3_HIGH("For RAB-Id %d, RAB type from NAS = %d, RAB type from EST_RABs = %d",rab_id, rab_type,
              rrc_get_rab_type_for_given_rab_id(wrrc_as_id,rbrc.rabs.rabs_in_msg[rab_index].rab_id));
            ...
            {
              if(rrcrbrc_update_nas_sync_ind(wrrc_as_id,rab_index,&(rab_list_ptr->elem[rab_index].nas_Synchronisation_Indicator))
                  == FAILURE)
              {
                return;
              }
  ...
  if (rrc_cap_related_info.rrc_pdcp_enabled) 
  {
    if (msg_ptr->u.r3.radioBearerReconfiguration_r3.rb_InformationReconfigList.n != 0)
    {
      ...
      while (msg_ptr->u.r3.radioBearerReconfiguration_r3.rb_InformationReconfigList.n > local_rb_idx) 
      {
        ...
        {
          if (rrc_get_rab_type_for_given_rb_id(wrrc_as_id,rb_info_ptr->rb_Identity) != RRCRB_PS_DATA_RAB)
          {
     ...
     if (rbrc.state_indicated_by_msg == RRC_STATE_URA_PCH)
     {
       ...
       else
       {
         ...
         if (msg_ptr->u.r3.radioBearerReconfiguration_r3.ura_Identity.numbits == 16)
         {
           ...
           rrccu_update_ura_id (wrrc_as_id,&(msg_ptr->u.r3.radioBearerReconfiguration_r3.ura_Identity));
         }
         else
         {
...
)
{
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(rbrc_ptr,rrc_RadioBearerReconfiguration_r5_IEs,dl_HSPDSCH_Information))
  {
    ...
    if( rrc_hsdpa_initial_validation(wrrc_as_id,rbrc_ptr->rrc_StateIndicator, 
  	...
  	rrc_RadioBearerReconfiguration_r5_IEs,new_U_RNTI))
  {
    ...
    else
    {
      ...
      rbrc.rbrc_for_srns_relocation = TRUE;
      /*store the new U-RNTI */
      rrc_translate_urnti (wrrc_as_id, &(rbrc_ptr->new_U_RNTI), &rbrc.new_urnti);
    }
  }
     ...
     (rbrc.state_indicated_by_msg == RRC_STATE_URA_PCH))
 {
   ...
   if (rbrc.state_indicated_by_msg == RRC_STATE_URA_PCH)
   {
     ...
     else
     {
       ...
       {
         rrccu_update_ura_id (wrrc_as_id,&(rbrc_ptr->ura_Identity));
       }
       else
       {
...
)
{
  ...
  {
    /* Check if network does not try to initiate HSDPA with invalid state */
    if (rrc_hsdpa_initial_validation_r6(wrrc_as_id,rbrc_ptr->rrc_StateIndicator, 
    ...
    for (rab_index = 0; rab_list_ptr->n > rab_index && rab_index < RRCRB_MAX_RAB_PER_MSG; rab_index++)
    {
      ...
      if ( rab_list_ptr->elem[rab_index].cn_DomainIdentity  == rrc_CN_DomainIdentity_cs_domain )
      {
        ...
        else
        {
          ...
          {
              if(rrcrbrc_update_nas_sync_ind(wrrc_as_id,rab_index,
...
)
{
  ...
  if (rbrc_ptr->specificationMode.u.complete.m.rab_InformationReconfigListPresent)
  {
    ...
    rbrc.status.failure_status = rrc_update_rab_info_reconfig_param(wrrc_as_id,
    &rbrc_ptr->specificationMode.u.complete.rab_InformationReconfigList, &rbrc.rabs ,&nas_sync_ind);
...
)
{
  ...
  */
  ...
  if( (rrc_get_state(wrrc_as_id)) == (rbrc.state_indicated_by_msg) )
  {
  ...
  WRRC_MSG3_HIGH("Sending CHANNEL_CONFIG_REQ RBs:%d, curr_st:%d, next_st:%d", 
             rb_index, rrc_get_state(wrrc_as_id), rbrc.state_indicated_by_msg);
...
===========================================================================*/
static void rrcrbrc_clear_procedure(sys_modem_as_id_e_type wrrc_as_id)
{
  uint32 rab_index;
  ...
  rbrc.rrcrbrc_send_initiate_cu_flag = FALSE;
  rbrc.rrcrbrc_delete_crnti          = FALSE;
  ...
  for (rab_index=0; rab_index<RRCRB_MAX_RAB_PER_MSG; rab_index++)
  {
    ...
    rbrc.rabs.rabs_in_msg[rab_index].nas_Synchronisation_Indicator = WCDMA_IVOCODER_ID_NONE;
  }
  rbrc.rabs.num_rabs_in_msg = 0;
  rbrc.status.failure_status = RRCRB_MSG_VALID;
  rbrc.trans_to_dch_ciph_config_needed = FALSE;
...
#endif
  rrc_common_procedure_clear(wrrc_as_id,RRC_PROCEDURE_RBRC);
}
...
)
{
  ...
  if (rbrc.cn_info_saved != FALSE)
  {
    if(rbrc.mm_cmd_ptr->cmd.hdr.message_id == (byte) RRC_SERVICE_IND_FROM_OTA_MSG )
    {
      ...
      rrc_ccm_update_service_info(wrrc_as_id, rbrc.mm_cmd_ptr);
    }
    /* Put the command on MM queue */
    rrc_send_rrc_message_to_mm(wrrc_as_id, rbrc.mm_cmd_ptr);
    /* clear the flag */
      ...
      (indicate_rlc == TRUE))
  {
    ...
    rrcrb_register_with_rlc_for_ul_data_ind (wrrc_as_id, RRC_PROCEDURE_RBRC);
  }
...
)
{
  ...
  oc_status = rrcllc_set_ordered_config(wrrc_as_id, RRC_PROCEDURE_RBRC,
  ...
  switch (oc_status)
  {
        ...
        || (rbrc.state_indicated_by_msg==RRC_STATE_CELL_PCH)))
    {
      ...
      if (rrcrbrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL99))
      {
        ...
        (rrccsp_check_initiate_cell_selection_handling(wrrc_as_id,&directed_cell_info) == FALSE))
      {
        ...
        {
          rbrc.rrcrbrc_send_initiate_cu_flag = TRUE;
          rbrc.rrcrbrc_delete_crnti          = TRUE;
        }
        else
        {
...

    {
         ...
         (rbrc.state_indicated_by_msg==RRC_STATE_CELL_FACH))
      {
        ...
        if((rbrc.rrcrbrc_send_initiate_cu_flag == FALSE) && 
          (rrcrb_validate_directed_cell_info (wrrc_as_id,directed_cell_info) == FALSE))
        {
           ...
           rrc_RadioBearerReconfiguration_r3_IEs,new_C_RNTI))
          {
            ...
            rrc_translate_crnti(wrrc_as_id,&msg_ptr->u.r3.radioBearerReconfiguration_r3.new_C_RNTI, 
                                &c_rnti);
          ...
          rrcrb_send_initiate_cell_selection_req(wrrc_as_id, RRC_PROCEDURE_RBRC,
      ...
      rrcrbrc_send_channel_config_req(wrrc_as_id);
            ...
            (rbrc.state_indicated_by_msg==RRC_STATE_CELL_FACH))
    {
      ...
      rrccu_start_t_305_timer(wrrc_as_id);
            ...
            (rbrc.state_indicated_by_msg==RRC_STATE_CELL_PCH))
    {
       rbrc.directed_cell_info = rrcrbrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL99);
      /*Send the RRCRBRC complete msg on old configuration */
      send_rbrc_complete_message(wrrc_as_id, TRUE);
      /*wait for L2ack */
    ...
    case ORDERED_CONFIG_INCOMPATABLE_SIMULTANEOUS_RECONFIG:
        ...
        /* Send the RB Release Failure Message */
         send_rbrc_failure_message(wrrc_as_id,rbrc.status.failure_status,
    ...
    case ORDERED_CONFIG_CONFIGURATION_SET_DEFERRED:
        WRRC_MSG0_HIGH("BUF_OTA:RBRC processing deferred due to LTA!");

        rbrc.curr_substate = RRCRB_INITIAL;
        rrc_buffer_ota_info.transaction_id = rbrc.tr_ids.accepted_transaction;
        /* Indicate that an error occured */
        ordered_config_err_occured = TRUE;
        break;
...
)
{
  ...
  {
    WRRC_MSG3_HIGH("Incomatible simultaneous reconfig! Sending RBRC failure trans_id:%d, accepted_trans_id:%d, rejected_trans_id:%d",
    ...
    if ((transaction_id != rbrc.tr_ids.accepted_transaction ) && (transaction_id != rbrc.tr_ids.rejected_transaction ))
    {
           ...
           (RRCCU_STARTED == rrccu_get_cell_update_started_status(wrrc_as_id) ))
      {
        ...
        else
        {
          ...
          (void)rrccu_register_for_cell_update_to_complete(wrrc_as_id, RRC_PROCEDURE_RBRC);
    ...
    return FAILURE;
          ...
          &&
         (rrc_buffer_ota_info.procedure == RRC_PROCEDURE_RBRC)
          &&
         (rrc_buffer_ota_info.transaction_id != RRCRB_INVALID_TRANSACTION_ID)
          &&
         (rrc_buffer_ota_info.transaction_id == transaction_id))
  {
    WRRC_MSG0_ERROR("BUF_OTA:Received Duplicate RBRC, Ignoring");
    return (FAILURE);
  }
#endif
...
)
{
  ...
  if (TRUE == rrcrbrc_validate_srns_relocation_info(wrrc_as_id,msg_ptr, msg_version))
  {
    ip_check_status = rrcsmc_check_integrity_for_srns_relocation(wrrc_as_id, cmd_ptr
                                                                 , msg_version
                                                                 );
...
)
{
    ...
    ||(rbrc.state_indicated_by_msg==RRC_STATE_CELL_PCH)))
  {
    ...
    if(rrcrbrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL8))
    {
      directed_cell_info.pscr_present = TRUE;
    ...
    (rbrc.state_indicated_by_msg==RRC_STATE_CELL_DCH)))
  {
    ...
    if ((current_state==RRC_STATE_CELL_FACH)&& (rbrc.state_indicated_by_msg==RRC_STATE_CELL_FACH))
    {
        ...
        (rrcrb_validate_directed_cell_info (wrrc_as_id,directed_cell_info) == FALSE))
      {
        ...
        {
          rbrc.new_crnti_valid = TRUE;
          rrc_translate_crnti(wrrc_as_id,&rbrc_ptr->new_C_RNTI, &c_rnti);
          rbrc.new_crnti = (uint16)c_rnti;
        }
        else
        {
  ...
  /*DCH -> Cell_PCH*/
    ...
    (rbrc.state_indicated_by_msg==RRC_STATE_CELL_PCH))
  {
    rbrc.directed_cell_info = rrcrbrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL8);
    /*Send the RRCRBRC complete msg on old configuration */
    send_rbrc_complete_message(wrrc_as_id, TRUE);
    /*wait for L2ack */
    rbrc.curr_substate = RRCRB_WAIT_FOR_L2_ACK;
  ...
  /*call the function that evaluates the directed cell info and set the flags accordingly */
    ...
    if(RRCRB_R8_MSG_IE_PRESENT(rbrc_ptr,rrc_RadioBearerReconfiguration_r8_IEs,new_C_RNTI))
    {
      ...
      {
        rrc_translate_urnti(wrrc_as_id,&rbrc_ptr->new_U_RNTI, &u_rnti);
        rnti_update_info.urnti_action = RRC_RNTI_UPDATE; 
        rnti_update_info.urnti        = u_rnti; 
      }
      else
...
)
{
  ...
  switch (oc_status)
  {
    case ORDERED_CONFIG_SET:
      if(msg_version == MSG_REL8)
      {
      rrcrbrc_process_after_oc_config_set_r8(wrrc_as_id,dcch_msg_ptr,
      ...
      if(msg_version == MSG_REL9)
      {
        rrcrbrc_process_after_oc_config_set_r9(wrrc_as_id,dcch_msg_ptr,
        &dcch_msg_ptr->message.u.radioBearerReconfiguration.u.later_than_r3.criticalExtensions.u.
      ...
      if(msg_version == MSG_REL10)
      {
        rrcrbrc_process_after_oc_config_set_r10(wrrc_as_id,dcch_msg_ptr,
        &dcch_msg_ptr->message.u.radioBearerReconfiguration.u.later_than_r3.criticalExtensions.u.
        criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.
        later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.radioBearerReconfiguration_r10);
      }
#endif
      break;
...
)
{
          ...
          T_rrc_DL_CommonInformation_r8_dl_dpchInfoCommon_dl_DPCH_InfoCommon)
      {
        ...
        */
        if (rrcsmc_tm_ciph_active(wrrc_as_id) == TRUE)
        {
          ...
          loc_ret_val = rrcrb_get_mac_d_hfn(wrrc_as_id,(RRC_MSG_COMMON_BITMASK_IE_PTR(rbrc_ptr->dl_CommonInformation.dl_dpchInfoCommon.u.dl_DPCH_InfoCommon,
		  	   rrc_DL_DPCH_InfoCommon_r6,mac_d_HFN_initial_value)),
               &rbrc_ptr->dl_CommonInformation.dl_dpchInfoCommon.u.dl_DPCH_InfoCommon->mac_d_HFN_initial_value,
               &mac_d_hfn_l);
  ...
  if (rbrc_ptr->specificationMode.u.complete.m.rab_InformationReconfigListPresent)
  {
    rbrc.status.failure_status = rrc_update_rab_info_reconfig_param_r8(wrrc_as_id,
  ...
  {
    /* Save the linked list head pointer */
    if (FAILURE == rrcllc_validate_rb_info_reconfig_list_r8(wrrc_as_id,
        &rbrc_ptr->specificationMode.u.complete.rb_InformationReconfigList))
    {
      rbrc.status.failure_status = RRCRB_MSG_INVALID_CONFIG;
      return;
...
)
{
  ...
  if (FAILURE == rrcrbrc_check_and_send_failure_for_simul_reconfig(wrrc_as_id,transaction_id))
  {
    return;
  }
  ...
  validate_rbrc_message_r8(wrrc_as_id, rbrc_ptr );

  if ( rbrc.status.failure_status != RRCRB_MSG_VALID )
  {
    /* RB Message was invalid. Send the RB Reconfig Failure Message */
     send_rbrc_failure_message(wrrc_as_id,rbrc.status.failure_status, rbrc.status.prot_err_cause);
  ...
  if(RRCRB_R8_MSG_IE_PRESENT(rbrc_ptr,rrc_RadioBearerReconfiguration_r8_IEs,cn_InformationInfo))
  {
    ...
    if (cn_info_ptr != NULL)
    {
      ...
      rbrc.mm_cmd_ptr = rrc_ccm_buid_service_ind_with_nas_cn_info_r6(wrrc_as_id,cn_info_ptr);
  ...
  rrcrbcommon_ext_data.rrc_change_of_ue_cap_response = FALSE;       
  
  if(RRCRB_R8_MSG_IE_PRESENT(rbrc_ptr,rrc_RadioBearerReconfiguration_r8_IEs,
    responseToChangeOfUE_Capability))
  {
    WRRC_MSG0_HIGH("REL8: Response for Change of UE Cap is given");
    rrcrbcommon_ext_data.rrc_change_of_ue_cap_response = TRUE;
  }
      ...
      rbrc_ptr->specificationMode.u.complete.rb_InformationReconfigList.n != 0 )
  {
    ...
    rrcrbrc_process_rb_reconfig_list_r8(wrrc_as_id,&rbrc_ptr->specificationMode.u.complete.rb_InformationReconfigList);

  /* update mac rab status to get tb size configuration*/
    rrcrb_update_mac_rab_status(wrrc_as_id,
    RRC_PROCEDURE_RBRC,
    RRC_PS_DOMAIN_CN_ID,
    NULL,
    TRUE);
  ...
  if (rbrc_ptr->specificationMode.u.complete.m.rb_InformationAffectedListPresent)
  {
    ...
    while ( rb_info_affected_list_r8_ptr->n > localrb_count )
    {
      ...
      if (! validate_rb_id (wrrc_as_id,rb_info_affected_list_r8_ptr->elem[localrb_count].rb_Identity))
      {
  ...
  rrcrbrc_update_ordered_config_r8 (wrrc_as_id, dcch_msg_ptr, msg_version);
  return;
...
)
{
    ...
    ||(rbrc.state_indicated_by_msg==RRC_STATE_CELL_PCH)))
  {
    ...
    if(rrcrbrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL9))
    {
      directed_cell_info.pscr_present = TRUE;
      directed_cell_info.pscr = rbrc_ptr->dl_InformationPerRL_List.elem[0].
                  modeSpecificInfo.u.fdd->primaryCPICH_Info.primaryScramblingCode;
  ...
  /*DCH -> Cell_PCH*/
    ...
    (rbrc.state_indicated_by_msg==RRC_STATE_CELL_PCH))
  {
    rbrc.directed_cell_info = rrcrbrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL9);
    /*Send the RRCRBRC complete msg on old configuration */
    send_rbrc_complete_message(wrrc_as_id, TRUE);
    /*wait for L2ack */
    rbrc.curr_substate = RRCRB_WAIT_FOR_L2_ACK;
...
)
{
  ...
  /* Validate the Message to make sure there is no invalid or incompatible configuration given by the message */
  validate_rbrc_message_r9(wrrc_as_id, rbrc_ptr );
...
/*===========================================================================
FUNCTION   rrcrbrc_process_after_oc_config_set_r10

DESCRIPTION
  This function processes the R10 RBRC OTA after OC is set by this OTA. So if we call this function, it means
  that ORDERED_CONFIG_SET is returned while setting OC by RBRC OTA.
...
SIDE EFFECTS
  None.
===========================================================================*/
void rrcrbrc_process_after_oc_config_set_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  rrc_DL_DCCH_Message * dcch_msg_ptr, 
...
)
{
  ...
  if((current_state==RRC_STATE_CELL_FACH) && 
    ...
    ||(rbrc.state_indicated_by_msg==RRC_STATE_CELL_PCH)))
  {
    if((RRCRB_R10_MSG_IE_PRESENT(rbrc_ptr,frequencyInfo)) &&
      (rbrc_ptr->frequencyInfo.modeSpecificInfo.t == T_rrc_FrequencyInfo_modeSpecificInfo_fdd))
    {
      directed_cell_info.freq_info_present = TRUE;
      directed_cell_info.uarfcn_dl  = rbrc_ptr->frequencyInfo.modeSpecificInfo.u.fdd->uarfcn_DL;
    ...
    if(rrcrbrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL10))
    {
      directed_cell_info.pscr_present = TRUE;
      directed_cell_info.pscr = rbrc_ptr->dl_InformationPerRL_List.elem[0].
                  modeSpecificInfo.u.fdd->primaryCPICH_Info.primaryScramblingCode;
    ...
    (rbrc.state_indicated_by_msg==RRC_STATE_CELL_DCH)))
  {
    ...
    if ((current_state==RRC_STATE_CELL_FACH)&& (rbrc.state_indicated_by_msg==RRC_STATE_CELL_FACH))
    {
      ...
      if((rbrc.rrcrbrc_send_initiate_cu_flag == FALSE) && 
        (rrcrb_validate_directed_cell_info (wrrc_as_id,directed_cell_info) == FALSE))
      {
        if(RRCRB_R10_MSG_IE_PRESENT(rbrc_ptr,new_C_RNTI))
        {
          rbrc.new_crnti_valid = TRUE;
          rrc_translate_crnti(wrrc_as_id,&rbrc_ptr->new_C_RNTI, &c_rnti);
          rbrc.new_crnti = (uint16)c_rnti;
  ...
  /*DCH -> Cell_PCH*/
    ...
    (rbrc.state_indicated_by_msg==RRC_STATE_CELL_PCH))
  {
    rbrc.directed_cell_info = rrcrbrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL10);
    /*Send the RRCRBRC complete msg on old configuration */
    send_rbrc_complete_message(wrrc_as_id, TRUE);
    /*wait for L2ack */
    rbrc.curr_substate = RRCRB_WAIT_FOR_L2_ACK;
  ...
  /*call the function that evaluates the directed cell info and set the flags accordingly */
    ...
    if(RRCRB_R10_MSG_IE_PRESENT(rbrc_ptr,new_C_RNTI))
    {
      ...
      /* check whether URNTI is also present in the message */
      if(RRCRB_R10_MSG_IE_PRESENT(rbrc_ptr,new_U_RNTI))
      {
        rrc_translate_urnti(wrrc_as_id,&rbrc_ptr->new_U_RNTI, &u_rnti);
        rnti_update_info.urnti_action = RRC_RNTI_UPDATE; 
        rnti_update_info.urnti        = u_rnti; 
...
/*===========================================================================
FUNCTION   VALIDATE_RBRC_MESSAGE_R10
...
SIDE EFFECTS
  None.
===========================================================================*/
static void validate_rbrc_message_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  rrc_RadioBearerReconfiguration_r10_IEs *rbrc_ptr /* Pointer to the R10 RB RC message */
)
{
  /*For CS RABs (if present) the codec type will be updated in rrc_ota_codec_mode_status
  even if RAB-InfoReconfig List isn't present*/
  ...
  switch (rbrc_ptr->rrc_StateIndicator)
  {
    ...
    default:
      /* No other states are supported right now. */
      WRRC_MSG1_HIGH("Unsupported state %d in RBRC R10 Msg", rbrc_ptr->rrc_StateIndicator);
      rbrc.status.failure_status = RRCRB_MSG_UNSUPPORTED_CONFIG;
      break;
  }
  ...
  /* Not support to the pre-config option. */
  if (rbrc_ptr->specificationMode.t != 
      T_rrc_RadioBearerReconfiguration_r10_IEs_specificationMode_complete)
  {
    WRRC_MSG0_ERROR("REL10: Non-support preconfig t in RBRC");
    rbrc.status.failure_status = RRCRB_MSG_UNSUPPORTED_CONFIG;
    return;
  }
  ...
  {
    /* check to see if dl-dpch-common info is present while going to dch */
    if ((RRCRB_R10_MSG_IE_PRESENT(rbrc_ptr,dl_CommonInformation)) &&
        (RRC_MSG_COMMON_BITMASK_IE(rbrc_ptr->dl_CommonInformation,
  ...
  2. If RB is being STOP/CONTINUE, then it can't be SRB1/2 and TM URBs
  3. Check for PDCP Info SRNS relocation etc.*/
  if((T_rrc_RadioBearerReconfiguration_r10_IEs_specificationMode_complete ==
     (rbrc_ptr->specificationMode.t) &&
      rbrc_ptr->specificationMode.u.complete.m.rb_InformationReconfigListPresent))
  {
    /* Save the linked list head pointer */
  ...
  */
  ...
  {
    /* If UTRAN DRX coefficient is not present then it is invalid configuration */
    if (!(RRCRB_R10_MSG_IE_PRESENT(rbrc_ptr,
      utran_DRX_CycleLengthCoeff)))
    {
      WRRC_MSG1_ERROR("REL10: UTRAN DRX Coef not preset, State:%d", rbrc.state_indicated_by_msg);
      rbrc.status.failure_status = RRCRB_MSG_INVALID_CONFIG;
      return;
    }
    if (rbrc.state_indicated_by_msg == RRC_STATE_URA_PCH)
    {
      /*If UE state is Cell_FACH then set the flag rrccu_ura_update_required if more than one URA-IDs are
      present in SIB2*/
      if (!(RRCRB_R10_MSG_IE_PRESENT(rbrc_ptr,ura_Identity)))
      {
        WRRC_MSG0_HIGH("REL10: URA-ID not present for URA_PCH state.");
        rrccu_clear_ura_id(wrrc_as_id);
      }
      else
      {
        ...
        {
          WRRC_MSG1_ERROR("REL10: Invalid URA-ID length%d", rbrc_ptr->ura_Identity.numbits);
          rbrc.status.failure_status = RRCRB_MSG_PROTOCOL_ERROR;
          rbrc.status.prot_err_cause = rrc_ProtocolErrorCause_asn1_ViolationOrEncodingError;
          return;
        }
...
/*===========================================================================
FUNCTION   process_rbrc_message_r10
...
SIDE EFFECTS
  None.
===========================================================================*/
static void process_rbrc_message_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  rrc_RRC_TransactionIdentifier transaction_id,
  rrc_DL_DCCH_Message *dcch_msg_ptr  ,
...
)
{
  ...
  WRRC_MSG1_MED("REL10: Recvd RBRC Msg sub_st%d", rbrc.curr_substate);
  ...
  /* Validate the Message to make sure there is no invalid or incompatible configuration given by the message */
  validate_rbrc_message_r10( wrrc_as_id, rbrc_ptr );
  ...
  if(RRCRB_R10_MSG_IE_PRESENT(rbrc_ptr,cn_InformationInfo))
  {
    cn_info_ptr = &(rbrc_ptr->cn_InformationInfo);
  ...
  if((RRCRB_R10_MSG_IE_PRESENT(rbrc_ptr,
    ueMobilityStateIndicator)) && (rbrc_ptr->ueMobilityStateIndicator == rrc_High_MobilityDetected_high_MobilityDetected))
  {
    WRRC_MSG0_HIGH("REL10: High Mobility Ind Set to TRUE in RBRC.");
    rbrc.high_mobility_ind = TRUE;
  }
  ...
  if(RRCRB_R10_MSG_IE_PRESENT(rbrc_ptr,
    supportForChangeOfUE_Capability))
  {
    WRRC_MSG1_HIGH("REL10: Support for Change of UE Cap is %d",rbrc_ptr->supportForChangeOfUE_Capability);
    if(rbrc_ptr->supportForChangeOfUE_Capability == TRUE)
    {
      rrccouec.rrc_ordered_rnc_cap_change_support = RNC_CAP_CHANGE_SUPPORTED;
    }
  ...
  if(RRCRB_R10_MSG_IE_PRESENT(rbrc_ptr,
    responseToChangeOfUE_Capability))
  {
    WRRC_MSG0_HIGH("REL10: Response for Change of UE Cap is given");
    rrcrbcommon_ext_data.rrc_change_of_ue_cap_response = TRUE;
  }
#endif /*FEATURE_WCDMA_REL7_SPECRS*/
  ...
  if (rbrc_ptr->specificationMode.u.complete.m.rb_InformationAffectedListPresent)
  {
    ...
    WRRC_MSG1_HIGH("REL10: Num of RBs to be affected:%d", localrb_count);
  }

  if (!is_valid_rbid)
  {
    /*one of the rab-id does not exist in established_rab*/
    WRRC_MSG1_ERROR("REL10: RB-id %d is not in EST_RABs", invalid_rbid);
  ...
  switch (oc_status)
  {
    case ORDERED_CONFIG_SET:
            ...
            ))
      {
        ...
        if (rrcrbrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, msg_version))
        {
          directed_cell_info.pscr_present = TRUE;
               ...
               (rbrc.state_indicated_by_msg==RRC_STATE_CELL_FACH))
      {
        ...
        rrccu_start_t_305_timer(wrrc_as_id);
               ...
               (rbrc.state_indicated_by_msg==RRC_STATE_CELL_PCH))
      {
        rbrc.directed_cell_info = rrcrbrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, msg_version);
        /*Send the RRCRBRC complete msg on old configuration */
        send_rbrc_complete_message(wrrc_as_id, TRUE);
        /*wait for L2ack */
        rbrc.curr_substate = RRCRB_WAIT_FOR_L2_ACK;
                ...
                (rbrc.state_indicated_by_msg==RRC_STATE_URA_PCH)))
      {
        ...
        if (crnti_present)
        {
          rrc_translate_crnti(wrrc_as_id,crnti_ptr, &c_rnti);
          /* Send a RNTI Update Request */
          rnti_update_info.crnti_action = RRC_RNTI_UPDATE;
          rnti_update_info.crnti        = (uint16)c_rnti;
          /* check whether URNTI is also present in the message */
...
)
{
  ...
  if (FAILURE == 
      rrcrbrc_check_and_send_failure_for_simul_reconfig(wrrc_as_id,
  ...
  message */
  validate_rbrc_message_r5(wrrc_as_id, msg_ptr );
  ...
  if (cn_info_ptr != NULL)
  {
    WRRC_MSG0_HIGH("Saving CN info for NAS");
       ...
       */
       primary_plmn_Id_present = rrcrbrc_check_and_get_primary_plmn_id(wrrc_as_id,msg_ptr,
                                                                  &primary_plmn_Identity,MSG_REL5);
    
    rbrc.mm_cmd_ptr = rrc_ccm_buid_service_ind_with_nas_cn_info(wrrc_as_id,cn_info_ptr,
  ...
  rrcrbrc_process_high_mobility_ind_in_non_critical_ext(wrrc_as_id,msg_ptr,MSG_REL5);
      ...
      (rbrc_ptr->specificationMode.u.complete.rb_InformationReconfigList.n != 0 ))
  {
    ...
    while (rbrc_ptr->specificationMode.u.complete.rb_InformationReconfigList.n >  localrb_count)
    {
      ...
      /*First check whether this rb-id exists in established_rab variable or not */
      if (! validate_rb_id(wrrc_as_id,(rb_info_reconfig_ptr->rb_Identity)))
      {
        /*rab-id does not exist in established_rab*/
        rbrc.status.failure_status = RRCRB_MSG_INVALID_CONFIG;
        is_valid_rbid = FALSE;
          ...
          rrc_RB_InformationReconfig_r5,rb_StopContinue))
      {
        ...
        lc_id = rrclcm_get_ul_rlc_lc_id(wrrc_as_id,(rb_info_reconfig_ptr->rb_Identity));
        ...
        if (rrclcm_get_ul_rlc_mode(wrrc_as_id,lc_id, &lc_mode) == RRCLCM_FAILURE)
        {
          /*rb-id does not exist in established_rab*/
          rbrc.status.failure_status = RRCRB_MSG_INVALID_CONFIG;
          is_valid_rbid = FALSE;
  ...
  if (is_valid_rbid)
  {
    ...
    if (rbrc_ptr->specificationMode.u.complete.m.rb_InformationAffectedListPresent)
    {
      ...
      {
        /*first check whether this rb-id exists in established_rab variable or not */
        if (! validate_rb_id (wrrc_as_id,(rb_info_affected_list_r5_ptr->elem[localrb_count].rb_Identity)))
        {
          /*rab-id does not exist in established_rab*/
          rbrc.status.failure_status = RRCRB_MSG_INVALID_CONFIG;
          is_valid_rbid = FALSE;
  ...
  rrcrbrc_update_ordered_config_r5 (wrrc_as_id, dcch_msg_ptr, msg_version);
...
)
{
  ...
  msg_ptr = &(dcch_msg_ptr->message.u.radioBearerReconfiguration);
  if (FAILURE == 
    rrcrbrc_check_and_send_failure_for_simul_reconfig(wrrc_as_id,transaction_id))
  {
    return;
  }
  ...
  if(rbrc_ptr != NULL)
  {
    validate_rbrc_message_r6(wrrc_as_id, rbrc_ptr );
  }
  else
  {
      ...
      criticalExtensions.u.r6.m.v6b0NonCriticalExtensionsPresent)
  {
    ...
    rrcrbrc_process_high_mobility_ind_in_non_critical_ext(wrrc_as_id,msg_ptr,MSG_REL6);
      ...
      rbrc_ptr->specificationMode.u.complete.rb_InformationReconfigList.n != 0 )
  {
    ...
    while (rbrc_ptr->specificationMode.u.complete.rb_InformationReconfigList.n > localrb_count)
    {
      ...
      {
        lc_id = rrclcm_get_ul_rlc_lc_id(wrrc_as_id,(rb_info_reconfig_r6_ptr->rb_Identity));

        (void) rrclcm_get_ul_rlc_mode(wrrc_as_id,lc_id, &lc_mode);
  ...
  if (is_valid_rbid)
  {
    ...
    if (rbrc_ptr->specificationMode.u.complete.m.rb_InformationAffectedListPresent)
    {
      ...
      {
        /*first check whether this rb-id exists in established_rab variable or not */
        if (! validate_rb_id (wrrc_as_id,(rb_info_affected_list_r6_ptr->elem[localrb_count].rb_Identity)))
        {
          /*rab-id does not exist in established_rab*/
          rbrc.status.failure_status = RRCRB_MSG_INVALID_CONFIG;
          is_valid_rbid = FALSE;
  ...
  /* For Cell_FACH or Cell_DCH continue with the ordered config setting */
  rrcrbrc_update_ordered_config_r6 (wrrc_as_id,dcch_msg_ptr, msg_version);
...
)
{
  ...
  switch (oc_status)
  {
        ...
        || (rbrc.state_indicated_by_msg==RRC_STATE_CELL_PCH)))
    {
      ...
      if (rrcrbrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL7 ))
      {
        directed_cell_info.pscr_present = TRUE;
        directed_cell_info.pscr = rbrc_ptr->dl_InformationPerRL_List.elem[0].
                                  modeSpecificInfo.u.fdd->primaryCPICH_Info.primaryScramblingCode;
...
)
{
  ...
  if(rbrc_ptr != NULL)
  {
    validate_rbrc_message_r7(wrrc_as_id,rbrc_ptr );
  }
  else
  {
    WRRC_MSG0_ERROR("rbrc_ptr is NULL.");
      ...
      rbrc_ptr->specificationMode.u.complete.rb_InformationReconfigList.n != 0 )
  {
    ...
    while (rbrc_ptr->specificationMode.u.complete.rb_InformationReconfigList.n > localrb_count)
    {
      ...
      {
        lc_id = rrclcm_get_ul_rlc_lc_id(wrrc_as_id,rb_info_reconfig_r7_ptr->rb_Identity);
  ...
  if (is_valid_rbid)
  {
    ...
    if (rbrc_ptr->specificationMode.u.complete.m.rb_InformationAffectedListPresent)
    {
      ...
      {
        /*first check whether this rb-id exists in established_rab variable or not */
        if (! validate_rb_id (wrrc_as_id,(rb_info_affected_list_r7_ptr->elem[localrb_count].rb_Identity)))
        {
          /*rab-id does not exist in established_rab*/
          rbrc.status.failure_status = RRCRB_MSG_INVALID_CONFIG;
          is_valid_rbid = FALSE;
  ...
  /* For Cell_FACH or Cell_DCH continue with the ordered config setting */
  rrcrbrc_update_ordered_config_r7 (wrrc_as_id,dcch_msg_ptr, msg_version);
} /* process_rbrc_message_r7 */
...
)
{
  ...
  if (msg_ptr->t != T_rrc_RadioBearerReconfiguration_r3)
  {
           ...
           T_rrc_RadioBearerReconfiguration_criticalExtensions_5_criticalExtensions))
    {
      ...
      { 
        ...
        if (FAILURE == rrcrbrc_validate_and_check_integrity_for_srns_relocation(wrrc_as_id,
        ...
        process_rbrc_message_r5(wrrc_as_id,cmd_ptr);
      }
               ...
               T_rrc_RadioBearerReconfiguration_criticalExtensions_3_r6))
      {
        ...
        process_rbrc_message_r6(wrrc_as_id,
          msg_ptr->u.later_than_r3.rrc_TransactionIdentifier,
               ...
               T_rrc_RadioBearerReconfiguration_criticalExtensions_2_r7))
      {
        ...
        process_rbrc_message_r7(wrrc_as_id,
          msg_ptr->u.later_than_r3.rrc_TransactionIdentifier,
          (rrc_DL_DCCH_Message *)cmd_ptr->cmd.downlink_sdu_ind.decoded_msg,
          &msg_ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.u.
        ...
        rrc_RadioBearerReconfiguration)))
      {
        ...
        process_rbrc_message_r8(wrrc_as_id,msg_ptr->u.later_than_r3.rrc_TransactionIdentifier,
          (rrc_DL_DCCH_Message *)cmd_ptr->cmd.downlink_sdu_ind.decoded_msg,
          &msg_ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.u.
          criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r8.radioBearerReconfiguration_r8);
      }
       ...
       rrc_RadioBearerReconfiguration)))
     {
       ...
       process_rbrc_message_r9(wrrc_as_id,msg_ptr->u.later_than_r3.rrc_TransactionIdentifier,
         (rrc_DL_DCCH_Message *)cmd_ptr->cmd.downlink_sdu_ind.decoded_msg,
         &msg_ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.u.
         criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.r9.radioBearerReconfiguration_r9);
     }
  ...
  rrc_RadioBearerReconfiguration)))
  {
    /* check for SRNS relocation scenario */
    if (FAILURE == rrcrbrc_validate_and_check_integrity_for_srns_relocation(wrrc_as_id,
    cmd_ptr, msg_ptr, msg_ptr->u.later_than_r3.rrc_TransactionIdentifier, MSG_REL10))
    {
      /* do not clear the procedure or change the rbr substate because
      some other rbreconfig procedure might be in progress. Just return from
      here */
    ...
    process_rbrc_message_r10(wrrc_as_id,msg_ptr->u.later_than_r3.rrc_TransactionIdentifier,
    (rrc_DL_DCCH_Message *)cmd_ptr->cmd.downlink_sdu_ind.decoded_msg,
    &msg_ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.u.
    criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.radioBearerReconfiguration_r10);
  }
      ...
      else
      {
        ...
        rrcrbrc_send_rbrc_failure_unsupported_config(wrrc_as_id,
            msg_ptr->u.later_than_r3.rrc_TransactionIdentifier);      
      }
  ...
  invalid or incompatible configuration given by the
  message */
  validate_rbrc_message(wrrc_as_id, msg_ptr );
     ...
     )
  {
    ...
    rbrc.mm_cmd_ptr = rrc_ccm_buid_service_ind_with_nas_cn_info(wrrc_as_id,
  ...
  rrcrbrc_process_high_mobility_ind_in_non_critical_ext(wrrc_as_id,msg_ptr,MSG_REL99);
     ...
     != 0 )
  {
    ...
    while (msg_ptr->u.r3.radioBearerReconfiguration_r3.rb_InformationReconfigList.n > localrb_count)
    {
	  	...
	  	rrc_RB_InformationReconfig,rb_StopContinue))
      {
        ...
        WRRC_MSG3_HIGH("%d[0:STOP/1:CONTINUE] recvd for RB %d lc_mode:%d ", 
  ...
  if (is_valid_rbid)
  {
      ...
      /*lint +e514*/
    {
      ...
      {
        /*first check whether this rb-id exists in established_rab variable or not */
        if (! validate_rb_id (wrrc_as_id,(rb_info_affected_list_ptr->elem[localrb_count].rb_Identity)))
        {
          /*rab-id does not exist in established_rab*/
          rbrc.status.failure_status = RRCRB_MSG_INVALID_CONFIG;
          is_valid_rbid = FALSE;
 ...
 /* For Cell_FACH or Cell_DCH continue with the ordered config setting */
 rrcrbrc_update_ordered_config (wrrc_as_id,dcch_msg_ptr);
...
)
{
  ...
  /* update mac rab status to get tb size configuration*/
    ...
    if (ptr->chan_cfg_status == RRCLLC_CHAN_CFG_FAILURE_OTHER)
    {
      ...
      {
        rbrc.curr_substate = RRCRB_WAIT_FOR_CELL_UPDATE_CNF;
        rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_RBRC, RRC_UNRECOVERABLE_ERROR, FALSE);
        return; 
      }
      ...
      configurations */
      if (OC_NOT_SET == rrcllc_reset_ordered_config(wrrc_as_id,RRC_PROCEDURE_RBRC
                                                    ,TRUE
                                                    ))
      {
      ...
      if(rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH && rbrc.state_indicated_by_msg == RRC_STATE_CELL_DCH)
      {
    ...
    if (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH && rbrc.state_indicated_by_msg == RRC_STATE_CELL_DCH)
    {
      ...
      rbrc.status.failure_status = RRCRB_PHY_CHAN_FAILURE;
      rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_RBRC, RRC_PHY_CHAN_FAILURE,TRUE);
    ...
    else if (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH && rbrc.state_indicated_by_msg == RRC_STATE_CELL_DCH)
    {
      rbrc.status.failure_status = RRCRB_PHY_CHAN_FAILURE;
      ...
      {
        rbrc.curr_substate = RRCRB_WAIT_FOR_CELL_UPDATE_CNF;
        rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_RBRC, RRC_RADIO_LINK_FAILURE, FALSE);
        return; 
      }
      ...
      else if(ptr->chan_cfg_status == RRCLLC_CHAN_CFG_FAILURE_BACK_TO_OLD_CONFIG)
      {
        ...
        /* Send RRC_CELL_UPDATE_INITIATE_REQ command to CELL UPDATE Proc */
        rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_RBRC, RRC_RADIO_LINK_FAILURE,TRUE);
        /* Wait for CELL UPDATE to get a valid C_RNTI */
        rbrc.curr_substate = RRCRB_WAIT_FOR_CELL_UPDATE_CNF;
        rbrc.rrcrbrc_send_initiate_cu_flag = FALSE;
        rbrc.rrcrbrc_delete_crnti          = FALSE;
  ...
  if(rrcrbcommon_ext_data.rrc_change_of_ue_cap_response)
  {
    rrccouec_process_completion_cap_change_with_reconfig(wrrc_as_id);
    rrcrbcommon_ext_data.rrc_change_of_ue_cap_response = FALSE;    
  }
#endif /*FEATURE_WCDMA_REL7_SPECRS*/
      ...
      (ordered_config.set_status == OC_SET_FOR_DCH_URA_PCH_TRANS))	
  {
    if(rbrc.high_mobility_ind == TRUE)
    {
      ...
      rrcrb_send_upd_ota_parms_ind(wrrc_as_id,RRC_PROCEDURE_RBRC, CPHY_HIGH_MOBILITY_INCL);
    }
  }
  ...
  rrcrbrc_configure_rab_change(wrrc_as_id);
       ...
       (rbrc.state_indicated_by_msg == RRC_STATE_URA_PCH)))
  {
    if (rbrc.state_indicated_by_msg == RRC_STATE_CELL_PCH) 
    {
      ...
      else
      {
        ...
        rrcrb_send_l1_drx_req (wrrc_as_id,RRC_PROCEDURE_RBRC);

        rrcrbrc_process_successful_proc_completion (wrrc_as_id,TRUE);
      }
      /* return from here */
      return;
    }
    else if (rbrc.state_indicated_by_msg == RRC_STATE_URA_PCH)
    {
      ...
      {
        rrcrb_send_ura_update_initiate_req(wrrc_as_id, RRC_PROCEDURE_RBRC,
                                            RRC_URA_RESELECTION,
                                            FALSE);
      }
    ...
    /* Coming here implies the "rbrc.rrcrbrc_send_initiate_cu_flag" is FALSE.
     ...
     * initiated when there is a valid C-RNTI and rbrc.rrcrbrc_send_initiate_cu_flag
     * is FALSE.
     */
  ...
  if (rbrc.rbrc_for_srns_relocation == TRUE)
  {
        ...
        )
    {
      ...
      rrcrb_send_rlc_re_est_req_to_smc(wrrc_as_id,RRC_PROCEDURE_RBRC, rb_type, TRUE);
  ...
  {   
    ...
    else
    {
        ...
        && (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH))
      {
        ...
        (void) rex_clr_sigs(rex_self(), RRCRBRC_TEMP_TIMER_SIG);
        (void) rex_set_timer(&rbrc.rrcrbrc_temp_timer, RRCRB_PND_CFG_DELAY);
    
        rbrc.rrcrbrc_sig = rex_wait(RRCRBRC_TEMP_TIMER_SIG);
        MSG_LOW("RRCRBRC signal:%d", rbrc.rrcrbrc_sig, 0, 0);
        (void) rex_clr_sigs(rex_self(), RRCRBRC_TEMP_TIMER_SIG);
        rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_RBRC, RRC_UNRECOVERABLE_ERROR, FALSE);
      } 
    ...
    else
    {
      ...
      rrcrbrc_send_indication_to_upper_layers(wrrc_as_id);
...
)
{
 ...
 /* Check if E-RNTI is not given in OTA then initiate cell update*/
    ...
    else
    {
      ...
      WRRC_MSG3_HIGH("Recvd state change %d to %d rbrc state %d, No action",
  ...
  else
  {
    ...
    other state changes */
    WRRC_MSG3_HIGH("Unsupported state change %d to %d rbrc state %d", ptr->previous_state,
...
)
{
  ...
  if ( ptr->status == SUCCESS )
  {
      ...
      else if(current_state==RRC_STATE_CELL_FACH) 
      {
        ...
        */
             ...
             cu_cause == rrc_CellUpdateCause_radiolinkFailure))
        {
          ...
          */
          if(SUCCESS != rrccu_register_for_cell_update_to_complete(wrrc_as_id,RRC_PROCEDURE_RBRC))
          {
    ...
    if (rbrc.rbrc_for_srns_relocation)
    {
          ...
          )
      {
        ...
        rrcrb_send_rlc_re_est_req_to_smc(wrrc_as_id,RRC_PROCEDURE_RBRC, rb_type, FALSE);
  ...
  else
  {
        ...
        (rbrc.state_indicated_by_msg == RRC_STATE_URA_PCH))
    {
      ...
      rrc_transition_to_disconnected_state(wrrc_as_id,RRC_PROCEDURE_RBRC,RRC_TX_TO_DISCON_OTHER);
    }
  }
      ...
      (ptr->status != SUCCESS))
  {
    ...
    {
      (void)rrcllc_reset_ordered_config(wrrc_as_id,RRC_PROCEDURE_RBRC,TRUE);
    }
  }
  else
  {
  ...
  if (rbrc.rbrc_for_srns_relocation == TRUE)
  {
    rrcdormancy_update_dormancy_for_srns_hho_cco(wrrc_as_id, RRC_PROCEDURE_RBRC);
  }
...
)
{
  ...
  case RRC_DOWNLINK_SDU_IND:
    ...
    process_rbrc_message( wrrc_as_id,cmd_ptr );
...
)
{
  ...
  case RRC_CHANNEL_CONFIG_CNF:
    ...
    rrcrbrc_process_channel_config_cnf(wrrc_as_id, &(cmd_ptr->cmd.chan_config_cnf) );
    break;

  case RRC_STATE_CHANGE_IND:

    /* Indication that RRC's state has changed */
    rrcrbrc_process_state_change_ind(wrrc_as_id, &(cmd_ptr->cmd.state_change_ind) );
    break;
...
)
{
  ...
  case RRC_CELL_UPDATE_COMPLETE_CNF:
      ...
      else if(rbrc.status.failure_status == RRCRB_MSG_VALID)
      {
        ...
        {
           send_rbrc_complete_message(wrrc_as_id, FALSE);
        }
        /*inform upper layers if necessary */
        rrcrbrc_send_indication_to_upper_layers(wrrc_as_id);
...
)
{
  ...
  case RRC_NEW_CELL_IND:
    rrcrbrc_process_new_cell_ind(wrrc_as_id, &(cmd_ptr->cmd.new_cell_ind) );
    break;
    ...
    if(rbrc.state_indicated_by_msg == RRC_STATE_CELL_FACH)
    {
      ...
      */
          ...
          )
      {
        ...
        config information to be requested from LLC. */
        rrcrbrc_send_channel_config_req(wrrc_as_id);
        /* Also reset the rbrc.rrcrbrc_send_initiate_cu_flag as cell update
...
)
{
  ...
  case RRC_RNTI_UPDATE_CNF:
    ...
    {
      rrcrbrc_process_rnti_update_cnf(wrrc_as_id);
    }
    else
    {
...
)
{
  ...
  case RRC_RE_EST_RLC_FOR_SRNS_CNF:
    ...
    rrcrbrc_process_rlc_reestablishment_cnf(wrrc_as_id  );
    break;
...
)
{
  ...
  case RRC_DOWNLINK_L2ACK_IND:
    ...
    /* Confirmation from LLC for the channel configurations */
    rrcrbrc_process_l2_ack_ind(wrrc_as_id, &(cmd_ptr->cmd.downlink_l2ack_ind) );
    break;
...
)
{
      ...
      (rbrc.curr_substate != RRCRB_WAIT_FOR_L2_ACK))
  {
    ...
    {
      WRRC_MSG3_HIGH("Recd L2ack for MUI:%d, Lc_Id:%d, rbrc_subst:%d",
               cmd_ptr->cmd.downlink_l2ack_ind.mui, 
               cmd_ptr->cmd.downlink_l2ack_ind.lc_id, rbrc.curr_substate);
    }
    else
    {
      WRRC_MSG3_HIGH("L2ack failed for MUI:%d, Lc_Id:%d, rbrc_subst:%d",
               cmd_ptr->cmd.downlink_l2ack_ind.mui, 
               cmd_ptr->cmd.downlink_l2ack_ind.lc_id, rbrc.curr_substate);
    }
...
)
{
      ...
      (rbrc.curr_substate != RRCRB_WAIT_FOR_L2_ACK))
  {
    ...
    substates. If rbrc substate does not indicate that we are not waiting for l2ack
    then just print the status */
    WRRC_MSG4_HIGH("RRCRBRC:L2ack %d (0-Success 1-Failure) for MUI:%d, Lc_Id:%d, rbe_subst:%d",
  ...
  case RRCRB_WAIT_FOR_CELL_UPDATE_CNF:
    /* Call the event handler for RRCRB_WAIT_FOR_CELL_UPDATE_CNF substate */
    rrcrbrc_fach_wfcucnf_event_handler(wrrc_as_id,  cmd_ptr);
    break;

  case RRCRB_WAIT_FOR_CELL_SELECTION_CNF:

    rrcrbrc_fach_wfcscnf_event_handler(wrrc_as_id,  cmd_ptr);
    break;

  case RRCRB_WAIT_FOR_CHAN_CFG_CNF:

    /* Call the event handler for RRCRB_WAIT_FOR_CHAN_CFG_CNF substate */
    rrcrbrc_fach_wfchcnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
    break;

  case RRCRB_WAIT_FOR_CRNTI_UPDATE_CNF:

      /* Call the event handler for RRCRB_WAIT_FOR_CRNTI_UPDATE_CNF substate */
      rrcrbrc_fach_wfcrntiupdatecnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;

  case RRCRB_WAIT_FOR_L2_ACK:

    /* Call the event handler for RRCRB_WAIT_FOR_L2_ACK substate */
    rrcrbrc_fach_wfl2ack_substate_event_handler(wrrc_as_id,  cmd_ptr );
    break;
...
)
{
  ...
  case RRCRB_WAIT_FOR_CELL_UPDATE_CNF:
    /* Call the event handler for RRCRB_WAIT_FOR_CELL_UPDATE_CNF substate */
    rrcrbrc_pch_wfcucnf_event_handler(wrrc_as_id,  cmd_ptr);
    break;

  case RRCRB_WAIT_FOR_CELL_SELECTION_CNF:

    rrcrbrc_pch_wfcscnf_event_handler(wrrc_as_id,  cmd_ptr);
    break;

  case RRCRB_WAIT_FOR_CHAN_CFG_CNF:

    /* Call the event handler for RRCRB_WAIT_FOR_CHAN_CFG_CNF substate */
    rrcrbrc_pch_wfchcnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
    break;
  ...
  default:
    ...
    MSG_MED("Command %x not processed in substate%d rrc_state%d", 
            cmd_ptr->cmd_hdr.cmd_id, rbrc.curr_substate, rrc_get_state(wrrc_as_id));
    break;
...
)
{
  ...
  switch ( rbrc.curr_substate )
  {
    ...
    case RRCRB_WAIT_FOR_CHAN_CFG_CNF:
      ...
      rrcrbrc_other_wfchcnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;
  
    case RRCRB_WAIT_FOR_L2_ACK:
  
      /* Call the event handler for WAIT_FOR_L2_ACK substate */
      rrcrbrc_other_wfl2ack_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;
  
    case RRCRB_WAIT_FOR_CELL_UPDATE_CNF:
      rrcrbrc_other_wfcucnf_substate_event_handler(wrrc_as_id,  cmd_ptr);
      break;

    case RRCRB_WAIT_FOR_CELL_SELECTION_CNF:
      rrcrbrc_other_wfcscnf_event_handler(wrrc_as_id,  cmd_ptr);
      break;
...
)
{
  ...
  rrc_crash_debug_save_proc_substates(wrrc_as_id,RRC_PROCEDURE_RBRC,rbrc.curr_substate,cmd_ptr->cmd_hdr.cmd_id);
  ...
  case RRC_STATE_CELL_FACH:    
    ...
    for CELL_FACH state */
    rrcrbrc_cell_fach_event_handler(wrrc_as_id, cmd_ptr);
    break;

  case RRC_STATE_CELL_DCH:    
    /* If it's CELL_DCH state, call the event handler
    for CELL_DCH state */
    rrcrbrc_cell_dch_event_handler(wrrc_as_id, cmd_ptr);
    break;
    ...
    case RRC_STATE_CELL_PCH:
      /* If it's CELL_DCH state, call the event handler
      for CELL_PCH state */
      rrcrbrc_pch_event_handler(wrrc_as_id, cmd_ptr);
      break;
    ...
    rrcrbrc_other_state_event_handler(wrrc_as_id, cmd_ptr);
    break;
...
{                                         
  ...
  if (srns_reloc == TRUE)
  {
    rrcrbrc_append_start_list_to_rbrc_complete(wrrc_as_id,msg_ptr);
  }
...
===========================================================================*/
void rrcrbrc_init_procedure(sys_modem_as_id_e_type wrrc_as_id)
{
  uint32 count;
  ...
  for ( count=0; count < RRCRB_MAX_RAB_PER_MSG; count++ )
  {
    ...
    rbrc.rabs.rabs_in_msg[count].nas_Synchronisation_Indicator = WCDMA_IVOCODER_ID_NONE;
  }
  rbrc.rabs.num_rabs_in_msg = 0;
  ...
  to disconnected state. */
  rrcscmgr_register_for_scn(wrrc_as_id, RRC_PROCEDURE_RBRC,  /* Procedure name */
  ...
  rex_def_timer(&rbrc.rrcrbrc_temp_timer, rex_self(), RRCRBRC_TEMP_TIMER_SIG);
}
...
/*===========================================================================
FUNCTION rrcrbrc_return_send_initiate_cu_flag
...
boolean rrcrbrc_return_send_initiate_cu_flag(sys_modem_as_id_e_type wrrc_as_id)
{
  return rbrc.rrcrbrc_send_initiate_cu_flag;
}
#endif
...
===========================================================================*/
void rrcrbrc_set_failure_cause(sys_modem_as_id_e_type wrrc_as_id, rrcrb_msg_status_e_type cause)
{
...
{
  qsh_mdump_collect_high(&rbrc,sizeof(rbrc));
}
#endif
