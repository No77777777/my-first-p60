...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcdispatcher.c#8 $   
...
31/08/18   ga      MAde cahnges for W-pref FR50563
...
02/15/18   sp      Made changes to post pending unlock request if L2 acks are received for all UL messages
04/25/17   dj      Made changes to have uarfcn and psc for wcdma signalling messages.
09/30/16   sas     Changes to pass correct UARFCN, PSC values in BCCH Log packet during reselection.
12/18/17   sp      Made changes to buffer multiple donwlink sdu inds when received during TA gap
11/13/17   sp      Made changes to save DDS sub id indicated by NAS
11/03/17   sas     Changes to remove LTE capability indication interface
04/27/17   gv      Disable LTE mode cap for E-call in limited service mode to avoid appending the LTE specific cap in rrc conn complete msg.
...
05/30/16   sp      Made changes to avoid stopping unlock by timer if there is pending L2 internal data
05/12/16   sn      f3 reduction changes BPLMN scenarios
05/09/16   sp      Made changes to have a maximum wait time for allowing LTAs despite of pending L2 acks
...
03/10/16   vi      Made changes to correctly send WCDMA capability to Wl1
02/11/16   vs      Changes to invalidate stale sibs of alternate sub in w plus w
02/18/16   sr      Changes to indicate duty cycle change to WL1
02/11/16   vs      Changes to avoid printing 2 events per OOS when caused by CS call on other sub
02/11/16   vs      Made changes to provide crash debug support to save TDS-RRC commads
01/12/16   vi      Fix to respond upper layers with DEACT_CNF when TDS is not loaded and to not send TDS STOP REQ to the sub if TDS is not present in SUB capability
01/11/16   gv      Fixed compilation issue
12/17/15   vi      Made changes to enable RRC<->TDS interface
11/17/15   sr      Debug code to detect out of sync between NAS and RRC 
11/13/15   sp      Made changes to log call end info in all cases whenver call is released
11/04/15   sr      Made changes to indicate if W sub is multimode or not
11/03/15   nr      W-RRC Changes for Subscription Id based communication with TC 
10/23/15   vi      Made changes to add psc and uarfcn info in wcdma signalling messages log packet.
10/13/15   vi      Made changes to pass LTE capability to WL1 whenever MULTIMODE_SUBS_CHG_IND is received.
09/10/15   sp      Made changes to log configuration info to QSH structures in inter task commands.
08/18/15   sr      Changes to pass AS_ID in MULTIMODE_SUBS_CHANGE_IND for NV read
08/05/15   vi      Made changes to clear msgr variant before processing l2w comamnds.
...
06/16/15   sp      Made changes to allow preemption for OC set cases in Cell_DCH state if activation time is sufficiently large
06/15/15   sn      Changes for new log packets for LAF
06/03/15   pr      W+W Phase8 Checkin.
06/03/15   aa      Made changes to start T302 after CMAC STATUS IND for CU
...
01/12/15   sp      Changes to use access correct structures while processing a command.
...
rrc_deact_status_type rrc_deact_status_sim[MAX_RRC_STACKS]= {RRC_NORMAL_OPERATION};
dog_report_type rrc_dog_rpt_var_sim[MAX_RRC_STACKS] = {0};
#define rrc_dog_rpt_var rrc_dog_rpt_var_sim[WAS_ID_M]
//used to get status of decoding.. if decoding error due to mem-allocation,
//then do not process the message any further..
extern rrc_decode_info_type rrc_decode_info_sim[MAX_RRC_STACKS];
extern trm_unlock_event_enum_t rrcwrm_unlock_type_sim[MAX_RRC_STACKS];
boolean rrc_dbg_is_hard_abort_sim[MAX_RRC_STACKS] = {FALSE};
#define rrc_deact_status rrc_deact_status_sim[WAS_ID_M]
#define rrc_decode_info rrc_decode_info_sim[WAS_ID_M]
#define rrcwrm_unlock_type rrcwrm_unlock_type_sim[WAS_ID_M]
...
boolean rrc_dbg_is_hard_abort = FALSE;
...
** ----------------------------------------------------------------------- */
extern void rrc_get_dl_log_message(sys_modem_as_id_e_type wrrc_as_id,rrc_cmd_type *cmd_ptr);

#ifdef FEATURE_WCDMA_FREE_FLOATING_TASK
extern void rrc_process_modem_stat_info(sys_modem_as_id_e_type wrrc_as_id,rrc_cmd_type *cmd_ptr);
#endif
...
===========================================================================*/
void rrc_event_wait( rex_sigs_type sig, sys_modem_as_id_e_type wrrc_as_id)
{
...
===========================================================================*/
static rrc_proc_e_type rrc_identify_procedure(sys_modem_as_id_e_type wrrc_as_id,
    ...
    case rrc_DL_DCCH_Message_PDU:
      ...
      switch(msg_type)
      {
        ...
        case T_rrc_DL_DCCH_MessageType_handoverFromUTRANCommand_EUTRA:
          if(rrcnv_data.wcdma_rrc_wtol_ps_ho_support_nv || rrcnv_data.wcdma_rrc_wtol_tdd_ps_ho_support_nv)
          {
            rrc_proc = RRC_PROCEDURE_IHO;
          }
          else
...
===========================================================================*/
static uecomdef_status_e_type rrc_perform_integrity_protection_check(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type *cmd_ptr,                 /* To hold received command */
...
{  
  ...
  *procedure = RRC_PROCEDURE_NONE;
  ...
  if (cmd_ptr->cmd.downlink_sdu_ind.dl_sdu != NULL) 
  {
    ...
    if(SUCCESS == rrcsmc_check_downlink_integrity(wrrc_as_id,cmd_ptr) )
    {   
       /* Identifes the to which procedure the message belongs to */
       *procedure = rrc_identify_procedure(wrrc_as_id,cmd_ptr); 
        ...
        */
       rrcdata_misc_info.rrc_cfn_before_decode[*procedure] = wl1_seq_get_cfn(wrrc_as_id);
       ...
       {
         message_choice = rrc_get_dl_ccch_message_type(wrrc_as_id,cmd_ptr->cmd.downlink_sdu_ind.dl_sdu,
                                                       cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_length);
         ...
         {
           if(RRC_STATE_CONNECTING != rrc_get_state(wrrc_as_id))
           {
             status = FAILURE;
             WRRC_MSG0_HIGH("UE not in Connecting state.Ignoring ConnSetup/Reject message");
             return status;
           }
           if ((rrcrce_verify_ue_identity(wrrc_as_id,cmd_ptr) == RRCRCE_SUCCESS) ||
               (rrcrce_verify_ue_identity_later_than_r3(wrrc_as_id,cmd_ptr) == RRCRCE_SUCCESS))
           {
             WRRC_MSG0_MED("UE Identity check passed");
           } 
           else
           {
             status = FAILURE;
      ...
      if( cmd_ptr->cmd.downlink_sdu_ind.decoded_msg == NULL )
      {
        ...
        {
          WRRC_ERR2_FATAL("Decoding failed. pdu type %d, pdu size %d", 
                    cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_type,
                    cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_length);
        }
      ...
      rrc_get_dl_log_message(wrrc_as_id, cmd_ptr);
    }
    else
    {
  ...
  if(SUCCESS == status)
  {
       ...
       (cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_type == rrc_DL_DCCH_Message_PDU))
    {
      ...
      if(!((*procedure == RRC_PROCEDURE_CU) && (cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_type == rrc_DL_CCCH_Message_PDU)))
      {
        ...
        rrc_crash_debug_save_ota(wrrc_as_id,cmd_ptr->cmd.downlink_sdu_ind.dl_sdu, cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_length,sdu_type,
...
===========================================================================*/
 uint8 rrc_get_dl_asn1_message_type(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type              *cmd_ptr /* To hold received command */
)
{
...
===========================================================================*/
static uint8 rrc_get_dl_message_type(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type              *cmd_ptr /* To hold received command */
)
{
  uint8 asn1_msg_type;
...
{  
  ...
  (void) rrclcm_get_dl_lc_type(wrrc_as_id,cmd_ptr->cmd.downlink_sdu_ind.lc_id,
                               &rrclog_dl_lc);
  ...
  sig_msg_event.message_type = rrc_get_dl_message_type(wrrc_as_id, cmd_ptr);
  WRRC_EVENT_REPORT_PAYLOAD(EVENT_RRC_MESSAGE_RECEIVED, sizeof(sig_msg_event),
    ...
    if(log_record_ptr != NULL)
    {
      ...
      if((channel_type == RRCLOG_SIG_DL_BCCH_BCH )|| (channel_type == RRCLOG_SIG_DL_BCCH_FACH))
      {
        log_record_ptr->channel_type |=  0x80;
  
      /* Fill UARFCN and PSC info */
      if((rrc_get_csp_substate(wrrc_as_id) == RRCCSP_SUIT_CELL_RESELECT_WT_SIB)||
         (rrc_get_csp_substate(wrrc_as_id) == RRCCSP_ACCEPT_CELL_RESELECT_WT_SIB)||
         (rrc_get_csp_substate(wrrc_as_id) == RRCCSP_SUIT_CELL_RESELECT_WT_SIB_PENDING)||
         (rrc_get_csp_substate(wrrc_as_id) == RRCCSP_SUIT_CELL_RESELECT_WT_SIB_PENDING))
      {
          log_record_ptr->uarfcn = rrc_csp_int_data.new_cell_acq_entry.freq;
          log_record_ptr->psc    = rrc_csp_int_data.new_cell_acq_entry.scr_code/16;
      }
      else
      {
          log_record_ptr->uarfcn = rrc_csp_int_data.curr_acq_entry.freq;
          log_record_ptr->psc  = rrc_csp_int_data.curr_acq_entry.scr_code/16;
      }
    }
      else if((channel_type == RRCLOG_SIG_DL_CCCH) || (channel_type == RRCLOG_SIG_DL_DCCH) || (channel_type == RRCLOG_SIG_DL_PCCH))
    {
        log_record_ptr->channel_type |=  0x80;
        ...
        {
          log_record_ptr->uarfcn = (uint16)rrc_ccm_get_curr_camped_freq(wrrc_as_id);
          (void)l1_get_hs_psc_sib_read(&scrcode,&pnpos,wrrc_as_id);
          log_record_ptr->psc = scrcode/16;
  }
  else
  {
          log_record_ptr->uarfcn = (uint16)rrc_ccm_get_curr_camped_freq(wrrc_as_id);
          log_record_ptr->psc    = (uint16)rrc_ccm_get_curr_camped_psc(wrrc_as_id);
        }
      }
...
{  
  ...
  if(cmd_ptr->cmd_hdr.cmd_id == RRC_DOWNLINK_SDU_IND)
  {
    ...
    MSG_LOW ("RLC ID received through RRC_DOWNLINK_SDU_IND command %d",
             lc_id,0,0);
    dl_wm_ptr = rrclcm_get_dl_watermark_ptr(wrrc_as_id,lc_id);
    ...
    { 
      ...
      else
      {
        ...
        rrc_log_dl_sig_message(wrrc_as_id, cmd_ptr);
...
FUNCTION  RRC_IS_SUB_MULTIMODE

DESCRIPTION
 
  This function checks if the sub is multimode sub.
...
RETURN VALUE
  ...
  SUCCESS: if sub is multimode sub
  else
  FAILURE
...
boolean rrc_is_sub_multimode(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrc_dsds_int_data.wcdma_capability_ind;
}
...
===========================================================================*/
uecomdef_status_e_type rrc_check_mm_cmds_for_simid_validation(sys_modem_as_id_e_type wrrc_as_id,rrc_cmd_type *cmd_ptr)
{
  ...
  if((cmd_ptr->cmd_hdr.cmd_id == RRC_SIM_INSERTED_REQ) ||
     ...
     || (cmd_ptr->cmd_hdr.cmd_id == RRC_DDS_SUB_IND)
#endif
    )
  {
      ...
      (cmd_ptr->cmd.rrc_service_req.network_select_mode == SYS_NETWORK_SELECTION_MODE_LIMITED))
  {
    ...
    rrc_dsds_int_data.rrc_sim_id_recvd = cmd_ptr->cmd.rrc_service_req.as_id;
  ...
  if(cmd_ptr->cmd_hdr.cmd_id == RRC_MULTIMODE_SUBS_CHGD_IND)
  {
    ...
    curr_as_id = rrc_dsds_int_data.rrc_sim_id_recvd;

    if(cmd_ptr->cmd.rrc_multimode_subs_chgd_ind.subs_capability[SYS_MODEM_AS_ID_1] == SUBS_CAPABILITY_MULTIMODE)
    {
      rrc_dsds_int_data.rrc_sim_id_recvd = SYS_MODEM_AS_ID_1;
    }
    else if(cmd_ptr->cmd.rrc_multimode_subs_chgd_ind.subs_capability[SYS_MODEM_AS_ID_2] == SUBS_CAPABILITY_MULTIMODE)
    {
      rrc_dsds_int_data.rrc_sim_id_recvd = SYS_MODEM_AS_ID_2;
    }
#ifdef FEATURE_TRIPLE_SIM
    else
    {
      rrc_dsds_int_data.rrc_sim_id_recvd = SYS_MODEM_AS_ID_3;
    }
#endif
    WRRC_MSG2_HIGH("AS-ID Changed to %d, previous one is %d",rrc_dsds_int_data.rrc_sim_id_recvd,curr_as_id);

    /* Re-read NVs only if the subscription has changed */
    if(curr_as_id != rrc_dsds_int_data.rrc_sim_id_recvd)
    {
      rrc_read_all_nv_items(rrc_dsds_int_data.rrc_sim_id_recvd);
      rrc_qsh_update_metric_database(rrc_dsds_int_data.rrc_sim_id_recvd);
    }
    ...
    if(cmd_ptr->cmd_hdr.cmd_id == RRC_MULTIMODE_SUBS_CHGD_IND)
    {
      WRRC_MSG1_HIGH("RRC_MULTIMODE_SUBS_CHGD_IND Recvd. capability mask %d",cmd_ptr->cmd.rrc_multimode_subs_chgd_ind.subs_capability_mask[wrrc_as_id]);

      if(cmd_ptr->cmd.rrc_multimode_subs_chgd_ind.subs_capability_mask[wrrc_as_id] & SYS_SYS_MODE_MASK_WCDMA)
      {
        rrc_dsds_int_data.rrc_is_sim_id_valid = TRUE;
        rrc_dsds_int_data.rrc_sim_id_recvd = wrrc_as_id;
        rrc_dsds_int_data.wcdma_capability_ind = TRUE;

       if(cmd_ptr->cmd.rrc_multimode_subs_chgd_ind.subs_capability_mask[wrrc_as_id] & SYS_SYS_MODE_MASK_LTE)
       {
         rrc_dsds_int_data.rrc_lte_capability_ind = TRUE;
       }
       else
       {
         rrc_dsds_int_data.rrc_lte_capability_ind = FALSE;
       }

       if(cmd_ptr->cmd.rrc_multimode_subs_chgd_ind.subs_capability_mask[wrrc_as_id] & SYS_SYS_MODE_MASK_TDS)
       {
         rrc_dsds_int_data.rrc_tds_capability_ind = TRUE;
       }
       else
       {
         rrc_dsds_int_data.rrc_tds_capability_ind = FALSE;
       }

       /* Change to detect the 1X SRLTE mode capability and store them to indicate to WL1 during WCDMA start. 
          RRC api will be called by WL1 */
       if((cmd_ptr->cmd.rrc_multimode_subs_chgd_ind.subs_capability_mask[wrrc_as_id] & SYS_SYS_MODE_MASK_CDMA) &&
          (cmd_ptr->cmd.rrc_multimode_subs_chgd_ind.subs_capability_mask[wrrc_as_id] & SYS_SYS_MODE_MASK_LTE))
       {
         rrc_dsds_int_data.rrc_1x_srlte_capability_ind = TRUE;
       }
       else
       {
         rrc_dsds_int_data.rrc_1x_srlte_capability_ind = FALSE;
       }
     }
#ifdef FEATURE_TRIPLE_SIM
    else if(cmd_ptr->cmd.rrc_multimode_subs_chgd_ind.subs_capability_mask[SYS_MODEM_AS_ID_3] & SYS_SYS_MODE_MASK_WCDMA)
    {
      rrc_dsds_int_data.rrc_is_sim_id_valid = TRUE;
      rrc_dsds_int_data.rrc_sim_id_recvd = SYS_MODEM_AS_ID_3;
    }
   ...
   {
     rrc_dsds_int_data.rrc_is_sim_id_valid = FALSE;
     rrc_dsds_int_data.rrc_lte_capability_ind = FALSE;
     rrc_dsds_int_data.wcdma_capability_ind = FALSE;
   }
   return SUCCESS;
  }
#endif
 ...
 */
  ...
  if(rrc_dsds_int_data.rrc_sim_id_recvd == sim_id_recvd)
  {
    return SUCCESS;
  }
  return FAILURE;
...
void rrc_check_rr_cmds_for_simid_validation(sys_modem_as_id_e_type wrrc_as_id,rrc_rr_cmd_type *cmd_ptr)
{
  sys_modem_as_id_e_type sim_id_recvd;
  ...
  if(rrc_dsds_int_data.rrc_sim_id_recvd != sim_id_recvd)
  {
    WRRC_MSG2_ERROR("INvalid sub-id %d passed. internal %d",sim_id_recvd,rrc_dsds_int_data.rrc_sim_id_recvd);
    return;
  }
  return;
}
...
uecomdef_status_e_type rrc_check_tc_cmds_for_simid_validation(sys_modem_as_id_e_type wrrc_as_id,rrc_cmd_type *cmd_ptr)
{
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
   ...
   default :
    sim_id_recvd = SYS_MODEM_AS_ID_NONE;
    break;
  }/* end switch */
...
===========================================================================*/
void rrc_dog_report_and_clear_sig(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  while((cmd_ptr = q_get(&rrc_cmd_q.rrc_l1_ff_cmd_q)) != NULL)
  {
    ...
    switch(cmd_ptr->cmd_hdr.cmd_id)
    {
      case RRC_CPHY_CHECK_SLEEP_REQ:
        rrcff_fail_l1_sleep_req(wrrc_as_id, cmd_ptr);
        break;
      default:
        WRRC_ERR1_FATAL("FF: Unexpected Message %d",cmd_ptr->cmd_hdr.cmd_id);
        break;
    }
...
void rrc_dispatch_l1_ff_commands(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_cmd_type *cmd_ptr;      /* To hold received command */
...
FUNCTION  RRC_DISPATCH_ALT_SUB_FF_COMMANDS

DESCRIPTION
 
  This function dispatches all commands received in RRC's ALT SUB free floating
  command queue. This function dequeues each command in the command queue
  until the command queue is empty. For each command, it looks at
  the command id and passes the command to the appropriate procedure
  for processing. The command is not freed by this function. Freeing the
...
===========================================================================*/
void rrc_dispatch_alt_sub_ff_commands(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_cmd_type *cmd_ptr;      /* To hold received command */
...
void rrc_dispatch_internal_commands(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  while( (cmd_ptr = q_get(&rrc_cmd_q.rrc_int_cmd_q)) != NULL)
  {
    if ((RRC_DOWNLINK_SDU_IND != cmd_ptr->cmd_hdr.cmd_id) && (RRC_QSH_PROCESS_CMD != cmd_ptr->cmd_hdr.cmd_id))
    {
      WRRC_MSG1_HIGH_OPT("Dispatching RRC int cmd rrc_cmd_e_type_value_0x%x",cmd_ptr->cmd_hdr.cmd_id);
    }
#ifdef FEATURE_DUAL_SIM
    if(rrcwrm_is_process_required(wrrc_as_id,cmd_ptr) == FALSE)
    {
    ...
    if (rrcmcm_is_process_required(wrrc_as_id,cmd_ptr) == FALSE) 
    {
    ...
    else if(rrc_deact_status != RRC_DEACT_COMPLETED)
    {
      ...
      rrc_crash_debug_save_internal_cmds(wrrc_as_id, cmd_ptr);
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
      wrrc_qsh_log_int_cmd_params(wrrc_as_id, cmd_ptr);
#endif
      ...
      switch( cmd_ptr->cmd_hdr.cmd_id )
      {
        case RRC_DOWNLINK_SDU_IND:
          ...
          the command. */
          if (RRC_DEQUEUE_SUCCESS == rrc_dequeue_dl_sdu (wrrc_as_id,cmd_ptr))
          {
            /* Before processing any SRB#2 message, process the SMC message if it is buffered.*/	
            if(cmd_ptr->cmd.downlink_sdu_ind.lc_id == rrclcm_get_dl_rlc_lc_id(wrrc_as_id,DCCH_AM_RADIO_BEARER_ID))
            {
            ...
            if(FAILURE == rrc_perform_integrity_protection_check(wrrc_as_id,cmd_ptr,
                            &rrc_proc_name) )
            {
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
              rrc_qsh_log_dl_ota_msg_id(wrrc_as_id, cmd_ptr, rrc_proc_name);
            ...
            else
            {
              ...
              if((rrc_proc_name != RRC_PROCEDURE_DDT) && 
                  (rrc_proc_name != RRC_PROCEDURE_SIB))
              {
                if(TRUE == rrcllc_curr_cmd_rel_all_req(wrrc_as_id))
                {   
              ...
              switch (rrc_proc_name)
              {
                ...
                case RRC_PROCEDURE_RCR:
                  rrcrcr_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;

                case RRC_PROCEDURE_DDT:
                  rrcddt_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;

                case RRC_PROCEDURE_SCR:
                  rrcscr_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;

                case RRC_PROCEDURE_RBE:
                  rrcrbe_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;

                case RRC_PROCEDURE_RBRC:
                  rrcrbrc_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;

                case RRC_PROCEDURE_TCR:
                  rrctcrc_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;

                case RRC_PROCEDURE_TFC:
                  rrctfc_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;

                case RRC_PROCEDURE_PCR:
                  rrcpcrc_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;

                case RRC_PROCEDURE_RBR:
                  rrcrbr_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;

                case RRC_PROCEDURE_PG1:
                  rrcpg1_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;

                case RRC_PROCEDURE_UECE:
                  rrcuece_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;

                case RRC_PROCEDURE_UECI:
                  rrcueci_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;

                case RRC_PROCEDURE_ASU:
                  rrcasu_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;
                ...
                case RRC_PROCEDURE_MCMR:
                  rrcmeas_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;               

                case RRC_PROCEDURE_PG2:
                  rrcpg2_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;               

                case RRC_PROCEDURE_UMI:
                  rrcumi_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break; 
  
                case RRC_PROCEDURE_CU:
                  rrccu_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;            

                case RRC_PROCEDURE_IHO:
                  rrciho_event_handler(wrrc_as_id, cmd_ptr );
                  break;
                ...
                case RRC_PROCEDURE_CHK:
                  rrcchk_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;  

#ifdef FEATURE_WCDMA_ETWS
                case RRC_PROCEDURE_BMC:
                  rrcbmc_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;
#endif
                ...
                case RRC_PROCEDURE_NONE:
                  ...
                  rrcmisc_procedure_event_handler(wrrc_as_id, cmd_ptr );
                  break;
       ...
       rrcdata_buffer_ota_during_activation_time(wrrc_as_id, cmd_ptr, rrc_proc_name );
              ...
              { 
                ...
                if(FALSE == rrc_qsh_prevent_dl_ota_param_log(wrrc_as_id, cmd_ptr))
                {
                  rrc_qsh_log_ota_params(wrrc_as_id, rrc_proc_name);
                }
                if((rrc_proc_name == RRC_PROCEDURE_ASU) || (rrc_proc_name == RRC_PROCEDURE_MCMR))
                {
                  if(rrc_qsh_metric_db[WRRC_QSH_METRIC_MEAS_ASU_OTA].data_ptr != NULL)
        ...
        case RRC_GET_SPECIFIC_SIBS_CNF:
          switch ( cmd_ptr->cmd.get_sibs_cnf.procedure )
          {
            ...
            case RRC_PROCEDURE_CCM:
              rrc_cell_change_manager(wrrc_as_id, cmd_ptr );
              break;

           
           case RRC_PROCEDURE_GPS:
              rrcgps_procedure_event_handler(wrrc_as_id, cmd_ptr );
              break;
        ...
        case RRC_NEW_STATE_IND:
          rrcscmgr_event_handler(wrrc_as_id, cmd_ptr );
          break;
             ...
             )
          {
            switch ( cmd_ptr->cmd.state_change_ind.procedure[count] )
            {
              ...
              case RRC_PROCEDURE_SCRR:
                rrcscrr_procedure_event_handler(wrrc_as_id, cmd_ptr );
                break;
  
              case RRC_PROCEDURE_IDT:
                rrcidt_procedure_event_handler(wrrc_as_id, cmd_ptr );
                break;
   
              case RRC_PROCEDURE_UDT:
                rrcudt_procedure_event_handler(wrrc_as_id, cmd_ptr );
                break;
              ...
              case RRC_PROCEDURE_LBT:
                rrclbt_procedure_event_handler(wrrc_as_id, cmd_ptr );
                break;
              ...
              case RRC_PROCEDURE_COUEC:
                rrccouec_procedure_event_handler(wrrc_as_id, cmd_ptr);
                break;
              ...
              case RRC_PROCEDURE_DORMANCY:
                rrcdormancy_procedure_event_handler(wrrc_as_id, cmd_ptr );
                break;  
               
#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
              case RRC_PROCEDURE_PSEUDO_MEAS:
		rrc_pseudo_meas_procedure_event_handler(wrrc_as_id, cmd_ptr );
               break;
#endif
...
//vn_fix_9
          ...
          rrcllc_process_cmd(wrrc_as_id, cmd_ptr );
// end_vn_fix_9
          break;
        ...
        case RRC_NEW_CELL_IND:
           ...
           * that need it. */
          rrc_ccm_propagate_new_cell_to_all_procs(wrrc_as_id, cmd_ptr );
          break;
        ...
        case RRC_LLC_CMD_CNF:
          ...
          switch (cmd_ptr->cmd.rrc_llc_cnf.procedure)
          {
            ...
            case RRC_PROCEDURE_MCM:
              ...
              rrcmcm_event_handler(wrrc_as_id, cmd_ptr );
              break;
...
#ifdef FEATURE_DUAL_SIM
          if((rrclcm_get_ul_rb_id(wrrc_as_id,cmd_ptr->cmd.downlink_l2ack_ind.lc_id) == DCCH_AM_RADIO_BEARER_ID) ||
             (rrclcm_get_ul_rb_id(wrrc_as_id,cmd_ptr->cmd.downlink_l2ack_ind.lc_id) == DCCH_DT_HIGH_PRI_RADIO_BEARER_ID))
          {
            rrc_reset_l2_ack_req_mask(wrrc_as_id,cmd_ptr->cmd.downlink_l2ack_ind.mui);
          }
          /* If all L2 acks are received, stop the LTA block timer */
          if(rrc_check_if_proc_l2_ack_req_mask_is_set(wrrc_as_id) == FALSE)
          {
            rrctmr_stop_timer(wrrc_as_id,RRCTMR_BLOCK_LTA_TIMER);
            rrc_check_and_post_unlock_by_tmr_expiry_if_pending(wrrc_as_id);
          }
#endif
          if(RRCLCM_SUCCESS == rrclcm_identify_proc(wrrc_as_id,cmd_ptr,
               rrclcm_get_ul_rb_id(wrrc_as_id,cmd_ptr->cmd.downlink_l2ack_ind.lc_id),
               &rrc_proc_name))
          {
            if((rrclcm_get_ul_rb_id(wrrc_as_id,cmd_ptr->cmd.downlink_l2ack_ind.lc_id) == DCCH_AM_RADIO_BEARER_ID) &&
                (rrc_check_if_any_proc_waiting_for_l2_ack_for_pch_transition(wrrc_as_id,&proc_waiting_for_l2_ack)) &&
            ...
            if( (rrcrbcommon_ext_data.rrc_wait_for_l2ack_for_reconfig == rrc_proc_name)
        ...
        case RRC_CU_TRANS_TO_DISCON_TIMER_EXPIRED:
          rrccu_tran_to_disconn_debug_handler(wrrc_as_id, cmd_ptr);
          break;
        ...
        case RRC_DCH_FACH_CELL_SELECTION_TIMER_EXPIRED_IND:
          WRRC_MSG1_HIGH( "DCH->FACH timer expired in CSP sstate %d",rrc_get_csp_substate(wrrc_as_id));
          rrc_cell_selection_procedure(wrrc_as_id,  cmd_ptr );
          break;

        case RRC_FACH_TO_FACH_PCH_CELL_SELECTION_TIMER_EXPIRED_IND:
          WRRC_MSG1_HIGH( "FACH->FACH/PCH timer expired in CSP sstate %d",rrc_get_csp_substate(wrrc_as_id));
          rrc_cell_selection_procedure(wrrc_as_id,  cmd_ptr );
          break;
        ...
        case RRC_DCH_PCH_CELL_SELECTION_TIMER_EXPIRED_IND:
          WRRC_MSG1_HIGH( "DCH->PCH timer expired in CSP sstate %d",rrc_get_csp_substate(wrrc_as_id));
          rrc_cell_selection_procedure(wrrc_as_id,  cmd_ptr );
          break;
        ...
        case RRC_CELL_UPDATE_COMPLETE_CNF:
          ...
          case RRC_MODE_CHANGE_IND:
            ...
            switch(cmd_ptr->cmd.mode_change_ind.procedure)
            {
              ...
              case RRC_PROCEDURE_PSEUDO_MEAS_RXAGC:
                rrc_start_rxagc_timer_on_mode_change (wrrc_as_id, cmd_ptr);
                break;
#endif
              case RRC_PROCEDURE_CELL_DEPRI:
                rrc_init_cell_depr_info(wrrc_as_id,cmd_ptr);
                break;
        ...
        case RRC_HO_CIPH_CNF:
          ...
          case RRC_RECONFIG_COMPLETE_IND:
            ...
            if(rrcsmc_is_reconfig_in_progress(wrrc_as_id) == TRUE)
            {
        ...
        case RRC_BPLMN_SRCH_TIMER_EXPIRED_IND:
          WRRC_MSG1_HIGH( "BPLMN Search Timer expired in CSP sstate %d",rrc_get_csp_substate(wrrc_as_id));
          rrc_cell_selection_procedure(wrrc_as_id,  cmd_ptr );
          break;
          ...
          switch(cmd_ptr->cmd.rrc_irat_abort_ind.procedure)
          {
            case RRC_PROCEDURE_CSP:
              WRRC_MSG2_HIGH("Dispatching IRAT_ABORT_IND to CSP in sstate %d curr_scan %d",
                              rrc_get_csp_substate(wrrc_as_id), rrc_csp_int_data.curr_scan);
              rrc_cell_selection_procedure(wrrc_as_id,  cmd_ptr );
              break;
        ...
        case RRC_WTOW_BPLMN_DRX_SRCH_TIMER_EXPIRED_IND:
          WRRC_MSG1_HIGH("DRX_SRCH_TIMER_EX fwd to CSP by dispatcher in CSP sstate %d",rrc_get_csp_substate(wrrc_as_id)); 
          rrc_cell_selection_procedure(wrrc_as_id,  cmd_ptr );
          break;

        case RRC_BPLMN_GUARD_SRCH_TIMER_EXPIRED_IND:
          WRRC_MSG1_HIGH("GUARD_SRCH_TIMER_EXP fwd to CSP by dispatcher in CSP sstate %d",rrc_get_csp_substate(wrrc_as_id)); 
          rrc_cell_selection_procedure(wrrc_as_id,  cmd_ptr );
          break;
        ...
        case RRC_BPLMN_SUSPEND_REQ:
          WRRC_MSG2_HIGH("RRC_BPLMN_SUSPEND_REQ fwd to CSP by dispatcher in CSP sstate %d from proc rrc_proc_e_type_value%d",
                          rrc_get_csp_substate(wrrc_as_id), cmd_ptr->cmd.rrc_csp_bplmn_suspend_req.procedure); 
          rrc_cell_selection_procedure(wrrc_as_id,  cmd_ptr );
          break;
        ...
        case RRC_BPLMN_SEARCH_RESUME_IND:
          WRRC_MSG1_MED("RRC_BPLMN_SEARCH_RESUME_IND fwd to CSP by dispatcher in sstate %d", rrc_get_csp_substate(wrrc_as_id)); 
          rrc_cell_selection_procedure(wrrc_as_id,  cmd_ptr );
          break;

        case RRC_MODE_STATUS_IND:
          {
            rrc_state_e_type rrc_curr_state = rrc_get_state(wrrc_as_id);
            WRRC_MSG1_HIGH("DUAL:RRC_MODE_STATUS_IND fwd to MEAS by dispatcher in rrc st %d", rrc_curr_state);
            rrcmeas_procedure_event_handler(wrrc_as_id,  cmd_ptr );
           
           if(rrcnv_data.rrc_delay_return_to_lte_nv.nv_enabled && rrcmisc_int_data.is_delay_wtol_resel_timer_expired)
           {
             WRRC_MSG1_HIGH("FRLTE_DELAY_ENH:RRC_MODE_STATUS_IND is NOT fwd to UECI in rrc st %d", rrc_curr_state);
             rrcmisc_int_data.is_delay_wtol_resel_timer_expired = FALSE;
           }
           else if((rrc_curr_state == RRC_STATE_CELL_FACH)
      ...
      case RRC_CSG_ASF_SRCH_TIMER_EXPIRY_IND:
        ...
        case RRC_QSH_PROCESS_CMD:
          rrc_process_qsh_cmd(wrrc_as_id, (qsh_client_cb_params_s*)&(cmd_ptr->cmd.rrc_qsh_cmd));
          break;
#endif
...
#ifdef FEATURE_DUAL_SIM
        case RRC_OUT_OF_SERVICE_INTERNAL_IND:
       ...
       case RRC_TRM_PRIORITY_CHANGE_IND:
#ifdef FEATURE_WCDMA_FAST_RETURN_TO_LTE_AFTER_CSFB
         rrcrce_check_rau_progress_for_ss(wrrc_as_id);
#endif
         WRRC_MSG0_HIGH("RRC_TRM_PRIORITY_CHANGE_IND received ");
         if(WRM_CLI_STATE_IS_IN_LOCK(WCDMA_TRM, WRM_PRIMARY_CLIENT_RRC, wrrc_as_id))
         {
           rrc_check_send_change_priority(wrrc_as_id);
         }
         else
         {
           /* Should be handled only when RRC is holding the lock */
         }  
         break;

       case RRC_ACQ_DUTY_CYCLE_CHANGE_IND:
         WRRC_MSG0_HIGH("RRC_ACQ_DUTY_CYCLE_CHANGE_IND received ");	   	
         if(rrc_csp_int_data.last_mm_primitive == RRC_CSP_SRVC_REQ_RCVD)
         {
           wl1_ds_update_duty_cycle(TRUE,wrrc_as_id);
         }
         break;		 
#endif
...
#ifdef FEATURE_DUAL_SIM
       case RRC_FALLBACK_IND:
       case RRC_START_G_MEAS_IND:
        if(((RRC_MODE_ACTIVE == rrcmcm_get_rrc_mode(wrrc_as_id))||(RRC_MODE_STANDBY == rrcmcm_get_rrc_mode(wrrc_as_id)))
          && (RRC_CAMPED_ON == rrc_ccm_get_curr_camping_status(wrrc_as_id,RRC_PROCEDURE_MCMR))
          && (rrc_get_state(wrrc_as_id) != RRC_STATE_CELL_DCH)
          && (OC_SET_FOR_CELL_DCH != rrcllc_get_ordered_config_status(wrrc_as_id))
          && ((rrcrce_get_rce_sub_state(wrrc_as_id) == RRCRCE_INITIAL)|| (rrcrce_get_rce_sub_state(wrrc_as_id) == RRCRCE_FINAL))
          && (WTOW_SEARCH_RESUME != rrc_csp_bplmn_srch_in_progress(wrrc_as_id))
#ifdef FEATURE_WCDMA_DEFERRED_SIB11_12_READING
          && !(rrcsib_is_sib11_11bis_12_read_deferred(wrrc_as_id, DEFERRED_UPDATE_SIB)) /*Deferred Sib reading is not pending*/
#endif
          )
         {
            WRRC_MSG0_HIGH("Dispatching RRC_FALLBACK_IND to Measurement Procedure");
            rrcmeas_procedure_event_handler(wrrc_as_id, cmd_ptr );
         }
         else
         {
            WRRC_MSG0_HIGH("Ignoring RRC_FALLBACK_IND");
         }
         break;
#endif
#ifdef FEATURE_WCDMA_ALLOW_LTA_DURING_ACT_TIME
        case RRC_ACTIVATION_TIMER_EXPIRY_FOR_BUFFER_OTA:
          ...
          {
            rrcdata_post_buffer_ota_after_activation_time_expiry(wrrc_as_id);
          }

        break;

        case RRC_POST_BUFFER_OTA_IND:
          rrcdata_process_buffered_ota_message(wrrc_as_id, cmd_ptr);
          break;
#endif  /*FEATURE_WCDMA_ALLOW_LTA_DURING_ACT_TIME*/
       case RRC_WT_FOR_L2_ACK_PCH_TRANSITION_OR_SMC_COMPLETE_TMR_EXP_IND:
        rrcsmc_process_smc_message_if_it_is_buffered(wrrc_as_id);
        break;
#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
...
#endif
#ifdef ELEVATOR_TRIGGER
       case RRC_PSEUDO_MEAS_RXAGC_TIMER_EXPIRED_IND:
        rrc_check_rxagc_criteria_and_post_pseudo_meas_ind(wrrc_as_id);
        break;
#endif   
#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
       case RRCTMR_ALLOW_EARFCN_FOR_PSEUDO_MEAS_EXPIRED_IND:
         rrc_pseudo_meas_clear_forbidden_earfcn(wrrc_as_id);
         break;
#endif
       case RRCTMR_DELAY_WTOL_RESEL_TIMER_EXPIRED_IND:
         WRRC_MSG0_HIGH("RRCTMR_DELAY_WTOL_RESEL_TIMER_EXPIRED_IND is received ");
         /*Delay WtoL RESEL timer has expired, reset the abnormal call failed global and send a new meas req for starting LTE measurements*/
         rrc_csp_int_data.abnormal_call_ended = FALSE;
        ...
        if((RRC_MODE_ACTIVE == rrcmcm_get_rrc_mode(wrrc_as_id)) &&
          (rrc_get_csp_substate(wrrc_as_id) == RRCCSP_CAMPED_SUIT_CELL) &&
          (rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_WTOW_BPLMN_DRX_SRCH_TIMER) == 0) &&
          ((rrc_get_state(wrrc_as_id) == RRC_STATE_DISCONNECTED) 
               || (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_PCH) || (rrc_get_state(wrrc_as_id) == RRC_STATE_URA_PCH) || (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH)))
        {
          rrc_cmd_type *cmd_ptr_buff;
          /* Get a RRC internal command buffer */
          if((cmd_ptr_buff = rrc_get_int_cmd_buf(wrrc_as_id)) != NULL)
          {
            ...
            rrcmisc_int_data.is_delay_wtol_resel_timer_expired = TRUE;
            WRRC_MSG0_HIGH("RRC_MODE_STATUS_IND is sent to dispatcher ");
          }
        }
       break;
       case RRCTMR_CELL_DEPRI_TIMER_EXPIRED_IND:
         ...
         break;
       case RRCTMR_CELL_DEPRI_NBR_TIMER_EXPIRED_IND:
         ...
         {
           WRRC_MSG0_HIGH("SSGCCS:Send DEPRI REQ to serv cell");
...
void rrc_dispatch_l1_commands( sys_modem_as_id_e_type wrrc_as_id )
{
  
  rrc_cmd_type *cmd_ptr;   /* To hold received command */

  rrc_csp_scan_e_type csp_curr_scan = rrccsp_get_curr_scan_no_f3(wrrc_as_id);

  /* We need to dequeue items from the l1
  command queue and process them till the queue is
  empty */
  while( (cmd_ptr = q_get(&rrc_cmd_q.rrc_l1_cmd_q)) != NULL)
  {
    ...
    else if(rrc_deact_status != RRC_DEACT_COMPLETED)
    {
      ...
      rrc_crash_debug_save_l1_to_rrc_cmds(wrrc_as_id, cmd_ptr);
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
      wrrc_qsh_log_l1_rrc_cmd_params(wrrc_as_id, cmd_ptr);
#endif        
      /* Switch on the command id of the received command, and
       * call the appropriate function to process each command. 
       */
      switch( cmd_ptr->cmd_hdr.cmd_id )
      {
...
#ifdef FEATURE_DUAL_SIM
            rrc_reset_oos_due_to_no_resource_flag(wrrc_as_id); /*Safe Reset*/
#endif
            if(rrc_get_state(wrrc_as_id) == RRC_STATE_DISCONNECTED || 
               rrc_get_state(wrrc_as_id) == RRC_STATE_CONNECTING)
            {
...
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
          wrrc_qsh_log_rlf_oos_reject_params(wrrc_as_id, WRRC_OOS, WRRC_METRIC_UPDATE);
          if(rrc_get_state(wrrc_as_id) != RRC_STATE_DISCONNECTED)
          { 
            rrccu_qsh_log_rlf_params(wrrc_as_id, SYS_RLF_WEAK_SIGNAL_OTHER);
          }
#endif
          break;
        ...
        case RRC_CPHY_RL_FAILURE_IND:
          if ((OC_SET_FOR_DCH_FACH_TRANS == rrcllc_get_ordered_config_status_wo_f3(wrrc_as_id)) )
          {
            /* UE will go to fach thru cell selection anyways. Ingore*/
            WRRC_MSG1_HIGH("Ignoring RL failure ind from L1, oc_status: %d",rrcllc_get_ordered_config_status(wrrc_as_id));
          }
          else
          {
            rrccu_procedure_event_handler(wrrc_as_id,  cmd_ptr );
...
#endif /*FEATURE_WCDMA_LOG_RRC_STATES*/  
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
          wrrc_qsh_log_rlf_oos_reject_params(wrrc_as_id, WRRC_RLF, WRRC_METRIC_UPDATE);
#endif 
          break;
        ...
        case RRC_CPHY_START_WCDMA_MODE_CNF:
          ...
          if(rrcllc_error_recovery_in_progress(wrrc_as_id))
          {
        ...
        case RRC_CPHY_STOP_WCDMA_MODE_CNF:
          ...
          rrcmeas_invalidate_qmi_current_freqs_info(wrrc_as_id);
          break;

        case RRC_CPHY_SUSPEND_WCDMA_MODE_CNF:
          WRRC_MSG0_MED("Recd RRC_CPHY_SUSPEND_WCDMA_MODE_CNF");
          rrcllc_process_cmd(wrrc_as_id, cmd_ptr );
          break;

        case RRC_CPHY_RESUME_WCDMA_MODE_CNF:
          WRRC_MSG0_MED("Recd RRC_CPHY_RESUME_WCDMA_MODE_CNF");
          rrcllc_process_cmd(wrrc_as_id, cmd_ptr );
          break;
        ...
        case RRC_CPHY_DL_WEAK_IND:
          if( (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH) && (rrc_sib_is_current_event_sib_read_in_dch(wrrc_as_id)))
          {
            rrc_proc_e_type proc_id = rrc_sib_get_proc_id_for_sib_read_in_dch(wrrc_as_id);
        ...
        case RRC_CPHY_MODEM_STATISTICS_RSP:
          WRRC_MSG0_HIGH_OPT("FF: Dispatching RRC_MODEM_STATISTICS_RSP to rrcdata");
          rrc_process_modem_stat_info(wrrc_as_id,  cmd_ptr );
          break;
#endif
...
void rrc_dispatch_mac_commands(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  while( (cmd_ptr = q_get(&rrc_cmd_q.rrc_mac_cmd_q)) != NULL)
  {
    ...
    {
      /* Save Crash debug info */
      rrc_crash_debug_save_l2_to_rrc_cmds(wrrc_as_id,cmd_ptr);
      /* Switch on the command id of the received command, and
       * call the appropriate function to process each command. 
       */
      switch( cmd_ptr->cmd_hdr.cmd_id )
      {
        ...
        case RRC_CMAC_STATUS_IND:
          if(rrcrce_get_rce_sub_state(wrrc_as_id) == RRCRCE_WAIT_FOR_RRC_CONN_SETUP)
          {
            rrcrce_procedure_event_handler(wrrc_as_id, cmd_ptr );   
            if(cmd_ptr->cmd.cmac_status_ind != RACH_TX_SUCCESS)
            {
              rrc_qsh_report_rach_failure_event(wrrc_as_id);
            }
          }
          else if((rrccu_return_cu_substate(wrrc_as_id) == RRCCU_WAIT_FOR_CELL_UPDATE_CONFIRM) ||
                  (rrccu_return_cu_substate(wrrc_as_id) == RRCCU_WAIT_FOR_URA_UPDATE_CONFIRM))
          {
            rrccu_procedure_event_handler(wrrc_as_id, cmd_ptr);
            if(cmd_ptr->cmd.cmac_status_ind != RACH_TX_SUCCESS)
            {
...
void rrc_dispatch_rlc_commands(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  while( (cmd_ptr = q_get(&rrc_cmd_q.rrc_rlc_cmd_q)) != NULL)
  {
...
#ifdef FEATURE_DUAL_SIM
    else if(rrcwrm_is_process_required(wrrc_as_id,cmd_ptr) == FALSE)
    {
      WRRC_MSG1_HIGH("DSIM_TUNEAWAY: CMD %d is Saved !",cmd_ptr->cmd_hdr.cmd_id);
      continue;
    }
...
void rrc_dispatch_mm_commands(sys_modem_as_id_e_type wrrc_as_id )
{
  ...
  while( (cmd_ptr = q_get(&rrc_cmd_q.rrc_mm_cmd_q)) != NULL)
  {
    ...
    else if(rrc_deact_status == RRC_NORMAL_OPERATION)
    {
      ...
      {
        /* Save Crash debug info */
        rrc_crash_debug_set_mm_to_rrc_cmd_id(wrrc_as_id,cmd_ptr);
      }
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
      wrrc_qsh_log_mm_rrc_cmd_params(wrrc_as_id, cmd_ptr);
#endif      
      /* Switch on the command id of the received command, and
       * call the appropriate function to process each command. 
       */
#ifdef FEATURE_DUAL_SIM
      if(rrc_check_mm_cmds_for_simid_validation(wrrc_as_id,cmd_ptr) == FAILURE)
      {
        WRRC_ERR1_FATAL("DSIM: Invalid SIM ID in MM cmd %x",cmd_ptr->cmd_hdr.cmd_id);
      }
#endif
      switch( cmd_ptr->cmd_hdr.cmd_id )
      {
        case RRC_SERVICE_REQ:
          ...
          if(rrc_csp_get_srch_order_special(wrrc_as_id) == SYS_SPECIAL_SBM_RAT_SEARCH_ORDER)
          {
        ...
        case RRC_CHANGE_MODE_IND:
          ...
          case RRC_G_MEAS_STOP_IND:
          WRRC_MSG0_HIGH("Recd: RRC_G_MEAS_STOP_IND");
          rrcmisc_procedure_event_handler(wrrc_as_id, cmd_ptr);
          break;
        ...
        case RRC_PLMN_LIST_REQ:
          ...
          rrc_log_search_request_info(wrrc_as_id,RRCLOG_NAS_PLMN_REQ,&cmd_ptr->cmd.plmn_list_req);
          rrcmcm_event_handler(wrrc_as_id,  cmd_ptr );
          break;
        ...
        case RRC_EST_REQ:
          ...
          {
            WRRC_MSG0_HIGH("Disabling LTE mode capability for E-call in Limited service");
        ...
        case RRC_ABORT_REQ:
          ...
          rrc_dbg_is_hard_abort = cmd_ptr->cmd.rrc_abort_req.hard_abort;
          rrciho_event_handler(wrrc_as_id,  cmd_ptr );
          break;
      ...
      case RRC_EQ_PLMN_LIST_CHANGE_IND:
        ...
        case RRC_INVALIDATE_SIM_DATA_REQ:
          ...
          /* Resetting the IMSI */
          memset(&rrcdata_sim_related_info.rrc_imsi,0, sizeof(rrc_imsi_gsm_map_type));
          rrcdata_sim_related_info.rrc_imsi_valid =FALSE;
          break;
        ...
        case RRC_PROCEDURE_IND:
          rrc_process_proc_ind(wrrc_as_id,cmd_ptr);
          break;

        case RRC_DDS_SUB_IND:
          rrc_process_dds_sub_id_ind(wrrc_as_id,cmd_ptr);
          break;
#endif

        case RRC_ABNORMAL_CALL_END_IND :
          WRRC_MSG0_HIGH("FRLTE_DELAY_ENH:RRC_ABNORMAL_CALL_END_IND received from NAS ");
          rrc_csp_int_data.abnormal_call_ended = TRUE;
          break;
    ...
    else if(rrc_deact_status == RRC_DEACT_COMPLETED)
    {
      ...
      {
        case RRC_DEACT_REQ:
          rrccsp_force_send_deact_cnf(wrrc_as_id);
          break;
        default :
...
static void rrc_dispatch_tc_commands(sys_modem_as_id_e_type wrrc_as_id )
{
  
  rrc_cmd_type *cmd_ptr;   /* To hold received command */

  /* We need to dequeue items from the TC
  command queue and process them till the queue is
  empty */

  MSG_MED("Dispatching TC Commands",0,0,0);
  while( (cmd_ptr = q_get(&rrc_cmd_q.rrc_tc_cmd_q)) != NULL)
  {

#ifdef FEATURE_DUAL_WCDMA
   if(rrc_check_tc_cmds_for_simid_validation(wrrc_as_id,cmd_ptr) == FAILURE)
    {
      WRRC_ERR1_FATAL("DSIM: Invalid SIM ID in MM cmd %x",cmd_ptr->cmd_hdr.cmd_id);
    }
   else
...
===========================================================================*/
void rrc_dispatch_rr_commands(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  while( (cmd_ptr = q_get(&rrc_cmd_q.rrc_rr_cmd_q)) != NULL)
  {
    WRRC_MSG1_HIGH_OPT("Dispatching RRC ext RR cmd rrc_cmd_e_type_value_0x%x",cmd_ptr->cmd_hdr.cmd_id );
    /*lint -e740*/
    /* Check if the incoming internal command is required a further process */
    if (rrcmcm_is_process_required(wrrc_as_id,(rrc_cmd_type *)cmd_ptr) == FALSE) 
    {
      /* Display a XDM message and disgard the command */
      WRRC_MSG1_HIGH("CMD: %d is disgarded!",cmd_ptr->cmd_hdr.cmd_id);
    }
    ...
    {
#ifdef FEATURE_DUAL_SIM
      rrc_check_rr_cmds_for_simid_validation(wrrc_as_id, cmd_ptr);
#endif
      if(rrc_deact_status == RRC_NORMAL_OPERATION)
      {
        ...
        {
          /*Save Crash debug info */
          rrc_crash_debug_set_rr_to_rrc_cmd_id(wrrc_as_id,cmd_ptr->cmd_hdr.cmd_id);
        }
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
        wrrc_qsh_log_rr_rrc_cmd_params(wrrc_as_id, cmd_ptr);
#endif
        /* Switch on the command id of the received command, and
           call the appropriate function to process each command. */
        switch(cmd_ptr->cmd_hdr.cmd_id)
        {
          ...
          case RR_INTERRAT_RESELECTION_ABORT_CNF:
            ...
            /*lint -e740*/
            rrc_cell_selection_procedure(wrrc_as_id,(rrc_cmd_type *)cmd_ptr);
            /*lint +e740*/
            break;
          ...
          case RR_INTERRAT_CC_ORDER_REJ:
            ...
            /*lint -e740*/
            rrccho_procedure_event_handler(wrrc_as_id,(rrc_cmd_type *)cmd_ptr);
            /*lint +e740*/
            break;
          ...
          case RRC_INTERRAT_RESELECTION_ABORT_REQ:
            ...
            rrcmcm_event_handler(wrrc_as_id,(rrc_cmd_type *)cmd_ptr);
            /*lint +e740*/
            break;
          ...
          case RRC_INTERRAT_HANDOVER_REQ:
            ...
            /*lint -e740*/
            rrcmcm_update_dual_mode_enabled_status(wrrc_as_id, TRUE);
            rrciho_event_handler(wrrc_as_id,(rrc_cmd_type *)cmd_ptr);
            /*lint +e740*/
            break;

          case RRC_NAS_DATA_IND:
            /*lint -e740*/
            rrcudt_procedure_event_handler(wrrc_as_id,(rrc_cmd_type *) cmd_ptr );
            /*lint +e740*/
            break;
          ...
          case RR_CIPHER_SYNC_IND:
            rrcsmc_procedure_event_handler(wrrc_as_id,(rrc_cmd_type *)cmd_ptr );
            break;
          ...
          case RR_DEDICATED_PRIORITIES_RSP:
           ...
           rrcmeas_procedure_event_handler(wrrc_as_id,(rrc_cmd_type *)cmd_ptr);
           break;
    ...
    rrc_free_rr_cmd_buf(wrrc_as_id,  cmd_ptr );
  }/* end while */
}/* end function */
...
void rrc_dispatch_lsm_commands(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_cmd_type *cmd_ptr;   
  MSG_MED("Dispatching LSM Commands",0,0,0);
  
  while( (cmd_ptr = q_get(&rrc_cmd_q.rrc_lsm_cmd_q)) != NULL)
  {
...
void rrc_dispatch_cm_commands(sys_modem_as_id_e_type wrrc_as_id )
{
  ...
  */
  ...
  while( (cmd_ptr = q_get(&rrc_cmd_q.rrc_cm_cmd_q)) != NULL)
  {
...
static void rrc_check_lte_cmds_for_simid_validation(sys_modem_as_id_e_type wrrc_as_id,uint8 variant)
{
  sys_modem_as_id_e_type sim_id_recvd ;
  ...
  {
    sim_id_recvd = SYS_MODEM_AS_ID_2;
  }
...
===========================================================================*/
void rrc_dispatch_lte_rrc_commands(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  while( (lte_cmd_ptr = q_get(&rrc_cmd_q.rrc_lte_cmd_q)) != NULL)
  {
    ...
    variant = MSGR_GET_VARIANT(&lte_cmd_ptr->cmd.msg_hdr);  // variant should be  same as the as_id passed to task

    /*Clear the MSGR variant to get the expected UMID*/
    ...
    /* Check if the incoming internal command is required a further process */
    if (rrcmcm_is_process_required(wrrc_as_id,rrc_cmd) == FALSE) 
    {
    ...
    {
#ifdef FEATURE_DUAL_SIM
      rrc_check_lte_cmds_for_simid_validation(wrrc_as_id,variant);  
#endif
      if(rrc_deact_status == RRC_NORMAL_OPERATION)
      {
...
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
        wrrc_qsh_log_lte_rrc_cmd_params(wrrc_as_id, lte_cmd_ptr);
#endif
        switch(lte_cmd_ptr->cmd.msg_hdr.id)
        {
          ...
          case WCDMA_RRC_LTE_ABORT_CGI_REQ:
            rrcmcm_event_handler(wrrc_as_id, rrc_cmd);
            break;
          case WCDMA_RRC_LTE_GET_PLMN_PRTL_RESULTS_REQ:
            rrc_cell_selection_procedure(wrrc_as_id, rrc_cmd);
            break;
          ...
          case LTE_RRC_EUTRA_CAPABILITIES_RSP:
            rrcueci_procedure_event_handler(wrrc_as_id, rrc_cmd);
            break;
          case WCDMA_RRC_LTE_UTRA_CAPABILITIES_REQ:
            rrciho_event_handler(wrrc_as_id, rrc_cmd);
            break;
          ...
          case WCDMA_RRC_LTE_DEPRI_FREQ_REQ:
            rrcmeas_procedure_event_handler(wrrc_as_id, rrc_cmd);
            break;
          ...
          case LTE_RRC_ACQ_DB_EARFCN_RSP:
            rrc_pseudo_meas_save_lte_acq_db_entries(wrrc_as_id,&(rrc_cmd->cmd.wcdma_rrc_lte_irat_cmd.cmd.lte_rrc_acq_db_earfcn));
            break;
#endif
          default :
...
static void rrc_check_qmi_cmds_for_simid_validation(sys_modem_as_id_e_type wrrc_as_id,wcdma_rrc_qmi_cmd_type *cmd_ptr)
{
  sys_modem_as_id_e_type sim_id_recvd;
...
===========================================================================*/
void rrc_dispatch_qmi_commands(sys_modem_as_id_e_type wrrc_as_id)
{
  wcdma_rrc_qmi_cmd_type *qmi_cmd_ptr;   /* To hold received command */
  rrc_cmd_type *rrc_cmd = rrc_malloc(sizeof(rrc_cmd_type));

  while((qmi_cmd_ptr = q_get(&rrc_cmd_q.rrc_qmi_cmd_q)) != NULL)
  {
    ...
    {
#ifdef FEATURE_DUAL_SIM
      rrc_check_qmi_cmds_for_simid_validation(wrrc_as_id,qmi_cmd_ptr);
#endif
      if(rrc_deact_status == RRC_NORMAL_OPERATION)
      {
        switch(qmi_cmd_ptr->cmd.msg_hdr.id)
        {
          case WCDMA_RRC_QMI_IDLE_DRX_INFO_REQ:
            rrcmisc_procedure_event_handler(wrrc_as_id,rrc_cmd);
            break;
          case WCDMA_RRC_QMI_PRIORITIZE_LTE_REQ:
            rrcmisc_procedure_event_handler(wrrc_as_id,rrc_cmd);
            break;
          case WCDMA_RRC_QMI_CURRENT_FREQS_INFO_REQ:
            rrcmeas_send_qmi_current_freqs_info_rsp(wrrc_as_id);
            break;
          default :
            WRRC_MSG1_HIGH("QMI: ID %xnot processed",qmi_cmd_ptr->cmd.msg_hdr.id);
            break;
        }
      } 
      else
    ...
    /*Put the cmd back to free queue*/
    q_put(&rrc_cmd_q.rrc_qmi_free_cmd_q,&(qmi_cmd_ptr->link));
  }
rrc_free(rrc_cmd);
}
#endif
...
===========================================================================*/
void rrc_dispatch_tdsrrc_commands(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_cmd_type *cmd_ptr;       /* To hold received command */
  ...
  while((cmd_ptr = q_get(&rrc_cmd_q.rrc_tdsrrc_cmd_q)) != NULL)
  {
    ...
    else
    {
      if(rrc_deact_status == RRC_NORMAL_OPERATION)
      {
        rrc_crash_debug_set_tdsrrc_to_rrc_cmd_id(wrrc_as_id,(rrc_tdsrrc_cmd_e_type)cmd_ptr->cmd_hdr.cmd_id);

       /* Switch on the command id of the received command, and
         call the appropriate function to process each command. */
        switch((rrc_tdsrrc_cmd_e_type)cmd_ptr->cmd_hdr.cmd_id)
        {
    ...
    /* The command has been processed. We can free the memory for this cmd */
    rrc_free_tdsrrc_cmd_buf( (rrc_tdsrrc_cmd_type *)cmd_ptr );
  }
}
#endif
...
void rrc_dispatcher ( sys_modem_as_id_e_type wrrc_as_id )
{
...
#ifdef FEATURE_DUAL_SIM
      | RRC_WRM_PREEMPTION_SIG
#endif
#ifdef FEATURE_WCDMA_FREE_FLOATING_TASK
      | RRC_L1_FF_CMD_Q_SIG
#ifdef FEATURE_DUAL_WCDMA 
      | RRC_ALT_SUB_FF_CMD_Q_SIG
#endif
#endif
#if defined (FEATURE_TDSCDMA_TO_WCDMA) || defined (FEATURE_WCDMA_TO_TDSCDMA)
  ...
  while( (rrc_received_sigs = rex_wait( rrc_requested_sigs )) != 0 )
  {
...
#ifdef FEATURE_DUAL_SIM
    if( rrc_received_sigs & RRC_WRM_PREEMPTION_SIG )
    {
      (void) rex_clr_sigs( rex_self(), RRC_WRM_PREEMPTION_SIG );
      rrcwrm_post_preemption_indication(wrrc_as_id);
    }
#endif
...
#ifdef FEATURE_DUAL_WCDMA 
      if( rrc_received_sigs & RRC_ALT_SUB_FF_CMD_Q_SIG )
      {
        (void) rex_clr_sigs( rex_self(), RRC_ALT_SUB_FF_CMD_Q_SIG );
         rrc_dispatch_alt_sub_ff_commands(wrrc_as_id);
      }
#endif
      ...
      if( rrc_received_sigs & RRC_L1_FF_CMD_Q_SIG )
      {
        ...
        rrc_dispatch_l1_ff_commands(wrrc_as_id);
      }
#endif
    ...
    if( rrc_received_sigs & RRC_INT_CMD_Q_SIG )
    {
      ...
      rrc_dispatch_internal_commands(wrrc_as_id);
    }
    ...
    if( rrc_received_sigs & RRC_L1_CMD_Q_SIG )
    {
      ...
      rrc_dispatch_l1_commands(wrrc_as_id);
    }
    ...
    if( rrc_received_sigs & RRC_MAC_CMD_Q_SIG )
    {
      ...
      rrc_dispatch_mac_commands(wrrc_as_id);
    }
    ...
    if( rrc_received_sigs & RRC_RLC_CMD_Q_SIG )
    {
      ...
      rrc_dispatch_rlc_commands(wrrc_as_id);
    }
    ...
    if( rrc_received_sigs & RRC_MM_CMD_Q_SIG )
    {
      ...
      rrc_dispatch_mm_commands(wrrc_as_id);
    }
    ...
    if( rrc_received_sigs & RRC_TC_CMD_Q_SIG )
    {
      ...
      rrc_dispatch_tc_commands(wrrc_as_id);
    }
    ...
    if( rrc_received_sigs & RRC_RR_CMD_Q_SIG )
    {
      ...
      rrc_dispatch_rr_commands(wrrc_as_id);
    }
    ...
    if( rrc_received_sigs & RRC_LSM_CMD_Q_SIG )
    {
      ...
      rrc_dispatch_lsm_commands(wrrc_as_id);
    }
#endif
    ...
    if( rrc_received_sigs & RRC_CM_CMD_Q_SIG )
    {
      ...
      rrc_dispatch_cm_commands(wrrc_as_id);
    }
    ...
    if(rrc_received_sigs & RRC_LTE_RRC_CMD_Q_SIG)
    {
      ...
      rrc_dispatch_lte_rrc_commands(wrrc_as_id);
    }
#endif
#if defined(FEATURE_THOR_MODEM) || defined(FEATURE_WCDMA_ATLAS_MODEM) || defined(FEATURE_TABASCO_MODEM)
    /* QMI CMD queue*/
    if(rrc_received_sigs & RRC_QMI_CMD_Q_SIG)
    {
      ...
      rrc_dispatch_qmi_commands(wrrc_as_id);
    }
#endif
#if defined (FEATURE_TDSCDMA_TO_WCDMA) || defined (FEATURE_WCDMA_TO_TDSCDMA)
    /*TDRRC CMD queue*/
    if(rrc_received_sigs & RRC_TDSRRC_CMD_Q_SIG)
    {
      ...
      rrc_dispatch_tdsrrc_commands(wrrc_as_id);
    }
#endif
...
{
  qsh_mdump_collect_high(&rrc_deact_status,sizeof(rrc_deact_status));
  qsh_mdump_collect_high(&rrc_dbg_is_hard_abort,sizeof(rrc_dbg_is_hard_abort));
  qsh_mdump_collect_high(&rrc_cmd_q,sizeof(rrc_cmd_q));
}
#endif
