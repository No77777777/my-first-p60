...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcsibdb.c#2 $    $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/06/18   sas     Minidump: Changes to collect only needed params of sibdb.
...
03/04/16   sn      Reverting earlier check-in
03/01/16   sn      Changes to fix deffered SIBs re-use issue
...
05/21/15   vi      Made changes to set default value of SIB 7 validity timer to 2 sec for legacy cell and 8sec for HS-FACH/EDRX cell
...
rrc_sibdb_type sibdb_sim[MAX_RRC_STACKS];   /* The SIB database */
#define sibdb sibdb_sim[WAS_ID_M]
#else
...
** ----------------------------------------------------------------------- */
#ifdef FEATURE_DUAL_WCDMA 
LOCAL rrcsibdb_int_data_type rrcsibdb_int_data_sim[MAX_RRC_STACKS];
rrcsibdb_pseudo_sib19_type rrc_psuedo_sib19_sim[MAX_RRC_STACKS];
rrc_SysInfoType19 * rrc_gsm_fallback_sib19_ptr_sim[MAX_RRC_STACKS];
#define rrcsibdb_int_data rrcsibdb_int_data_sim[WAS_ID_M]
#define rrc_psuedo_sib19 rrc_psuedo_sib19_sim[WAS_ID_M]
#define rrc_gsm_fallback_sib19_ptr rrc_gsm_fallback_sib19_ptr_sim[WAS_ID_M]
#else
rrcsibdb_pseudo_sib19_type rrc_psuedo_sib19;
LOCAL rrcsibdb_int_data_type rrcsibdb_int_data;
rrc_SysInfoType19 * rrc_gsm_fallback_sib19_ptr = NULL;
#endif
  ...
  switch( sib )
  {
    case rrc_MIB :
      if(sibdb.cell[index].sibs.mib == NULL)
      {
        if(sibdb.cell[index].sibs.mib_encoded_pdu_length != INVALID_PDU_LENGTH)
        {
          ...
          sibdb.cell[index].sibs.mib =  rrcasn1_decode_pdu( wrrc_as_id,
          ...
          if(sibdb.cell[index].sibs.mib == NULL)
          {
            WRRC_ERR0_FATAL("SIB_HEAP:Asn.1 decode failed from MIB");
          }
        ...
        {
          WRRC_ERR0_FATAL("SIB_HEAP:mib_encoded_ptr is NULL");
        }
      }
      else
      {
    ...
    case rrc_SIB1 :
      if(sibdb.cell[index].sibs.sib1 == NULL)
      {
        ...
        {
          /* ASN.1 decode the PDU */
          sibdb.cell[index].sibs.sib1 =  rrcasn1_decode_pdu( wrrc_as_id,
          ...
          if(sibdb.cell[index].sibs.sib1== NULL)
          {
            WRRC_ERR0_FATAL("SIB_HEAP:Asn.1 decode failed from SIB1");
          }
        ...
        {
          WRRC_ERR0_FATAL("SIB_HEAP:sib1_encoded_ptr is NULL");
        }
      }
      else
      {
    ...
    case rrc_SIB2 :
      if(sibdb.cell[index].sibs.sib2 == NULL)
      {
        ...
        {
          /* ASN.1 decode the PDU */
          sibdb.cell[index].sibs.sib2 =  rrcasn1_decode_pdu( wrrc_as_id,
          ...
          if(sibdb.cell[index].sibs.sib2== NULL)
          {
            WRRC_ERR0_FATAL("SIB_HEAP:Asn.1 decode failed from sib2");
          }
          ptr = (void *) sibdb.cell[index].sibs.sib2;
        }
        else
        {
          WRRC_ERR0_FATAL("SIB_HEAP:sib2_encoded_ptr is NULL");
        }
      }
      else
      {
    ...
    case rrc_SIB3 :
      if(sibdb.cell[index].sibs.sib3 == NULL)
      {
        ...
        {
          /* ASN.1 decode the PDU */
          sibdb.cell[index].sibs.sib3 =  rrcasn1_decode_pdu( wrrc_as_id,
          ...
          if(sibdb.cell[index].sibs.sib3== NULL)
          {
            WRRC_ERR0_FATAL("SIB_HEAP:Asn.1 decode failed from sib3");
          }
        ...
        {
          WRRC_ERR0_FATAL("SIB_HEAP:sib3_encoded_ptr is NULL");
        }
      }
      else
      {
    ...
    case rrc_SIB4 :
      ...
      if(sibdb.cell[index].sibs.sib4 == NULL)
      {
        ...
        {
          /* ASN.1 decode the PDU */
          sibdb.cell[index].sibs.sib4 =  rrcasn1_decode_pdu(wrrc_as_id,  
          ...
          if(sibdb.cell[index].sibs.sib4== NULL)
          {
            WRRC_ERR0_FATAL("SIB_HEAP:Asn.1 decode failed from sib4");
          }
          ptr = (void *) sibdb.cell[index].sibs.sib4;
        }
        else
        {
          WRRC_ERR0_FATAL("SIB_HEAP:sib4_encoded_ptr is NULL");
        }
      }
      else
      {
    ...
    case rrc_SIB5 :
      if(sibdb.cell[index].sibs.sib5 == NULL)
      {
        ...
        {
          /* ASN.1 decode the PDU */
          sibdb.cell[index].sibs.sib5 =  rrcasn1_decode_pdu( wrrc_as_id, 
        ...
        {
          WRRC_ERR0_FATAL("SIB_HEAP:sib5_encoded_ptr is NULL");
        }
      }
      else
      {
    ...
    case rrc_SIB6 :
      if(sibdb.cell[index].sibs.sib6 == NULL)
      {
        ...
        {
          /* ASN.1 decode the PDU */
          sibdb.cell[index].sibs.sib6 =  rrcasn1_decode_pdu( wrrc_as_id, 
        ...
        {
          WRRC_ERR0_FATAL("SIB_HEAP:sib6_encoded_ptr is NULL");
        }
      }
      else
      {
    ...
    case rrc_SIB7 :
      if(sibdb.cell[index].sibs.sib7 == NULL)
      {
        ...
        {
          /* ASN.1 decode the PDU */
          sibdb.cell[index].sibs.sib7 =  rrcasn1_decode_pdu( wrrc_as_id, 
          ...
          if(sibdb.cell[index].sibs.sib7== NULL)
          {
            WRRC_ERR0_FATAL("SIB_HEAP:Asn.1 decode failed from sib7");
          }
        ...
        {
          WRRC_ERR0_FATAL("SIB_HEAP:sib7_encoded_ptr is NULL");
        }
      }
      else
      {
    ...
    case rrc_SIB11 :
      if(sibdb.cell[index].sibs.sib11 == NULL)
      {
        ...
        {
          /* ASN.1 decode the PDU */
          sibdb.cell[index].sibs.sib11 =  rrcasn1_decode_pdu( wrrc_as_id, 
          ...
          if(sibdb.cell[index].sibs.sib11== NULL)
          {
            WRRC_ERR0_FATAL("SIB_HEAP:Asn.1 decode failed from sib11");
          }
        ...
        {
          WRRC_ERR0_FATAL("SIB_HEAP:sib11_encoded_ptr is NULL");
        }
      }
      else
      {
    ...
    case rrc_SIB12 :
      if(sibdb.cell[index].sibs.sib12 == NULL)
      {
        ...
        {
          /* ASN.1 decode the PDU */
          sibdb.cell[index].sibs.sib12 =  rrcasn1_decode_pdu( wrrc_as_id, 
          ...
          if(sibdb.cell[index].sibs.sib12== NULL)
          {
            WRRC_ERR0_FATAL("SIB_HEAP:Asn.1 decode failed from sib12");
          }
        ...
        {
          WRRC_ERR0_FATAL("SIB_HEAP:sib12_encoded_ptr is NULL");
        }
      }
      else
      {
    ...
    case rrc_SIB18 :
      if(sibdb.cell[index].sibs.sib18 == NULL)
      {
        ...
        {
          /* ASN.1 decode the PDU */
          sibdb.cell[index].sibs.sib18 =  rrcasn1_decode_pdu( wrrc_as_id, 
          ...
          if(sibdb.cell[index].sibs.sib18== NULL)
          {
            WRRC_ERR0_FATAL("SIB_HEAP:Asn.1 decode failed from sib18");
          }
          ptr = (void *) sibdb.cell[index].sibs.sib18;
        }
        else
        {
          WRRC_ERR0_FATAL("SIB_HEAP:sib18_encoded_ptr is NULL");
        }
      }
      else
      {
    ...
    case rrc_SB1 :
      if(sibdb.cell[index].sibs.sb1 == NULL)
      {
        ...
        {
          /* ASN.1 decode the PDU */
          sibdb.cell[index].sibs.sb1 =  rrcasn1_decode_pdu( wrrc_as_id, 
          ...
          if(sibdb.cell[index].sibs.sb1== NULL)
          {
            WRRC_ERR0_FATAL("SIB_HEAP:Asn.1 decode failed from sb1");
          }
        ...
        {
          WRRC_ERR0_FATAL("SIB_HEAP:sb1_encoded_ptr is NULL");
        }
      }
      else
      {
    ...
    case rrc_SB2 :
      if(sibdb.cell[index].sibs.sb2 == NULL)
      {
        ...
        {
          /* ASN.1 decode the PDU */
          sibdb.cell[index].sibs.sb2 =  rrcasn1_decode_pdu( wrrc_as_id, 
          ...
          if(sibdb.cell[index].sibs.sb2== NULL)
          {
            WRRC_ERR0_FATAL("SIB_HEAP:Asn.1 decode failed from sb2");
          }
        ...
        {
          WRRC_ERR0_FATAL("SIB_HEAP:sb2_encoded_ptr is NULL");
        }
      }
      else
      {
    ...
    case rrc_SIB11bis :
      if(sibdb.cell[index].sibs.sib11bis == NULL)
      {
        ...
        {
          /* ASN.1 decode the PDU */
          sibdb.cell[index].sibs.sib11bis =  rrcasn1_decode_pdu( wrrc_as_id, 
          ...
          if(sibdb.cell[index].sibs.sib11bis== NULL)
          {
            WRRC_ERR0_FATAL("SIB_HEAP:Asn.1 decode failed from sib11bis");
          }
        ...
        {
          WRRC_ERR0_FATAL("SIB_HEAP:sib11bis_encoded_ptr is NULL");
        }
      }
      else
      {
      ...
      break;
        ...
        case rrc_SIB19 :
          if(sibdb.cell[index].sibs.sib19 == NULL)
          {
            ...
            {
              /* ASN.1 decode the PDU */
              sibdb.cell[index].sibs.sib19 =  rrcasn1_decode_pdu( wrrc_as_id, 
              ...
              if(sibdb.cell[index].sibs.sib19== NULL)
              {
                WRRC_ERR0_FATAL("SIB_HEAP:Asn.1 decode failed from sib19");
              }
              ptr = (void *) sibdb.cell[index].sibs.sib19;
            }
            else
            {
              WRRC_ERR0_FATAL("SIB_HEAP:sib19_encoded_ptr is NULL");
            }
          }
          else
          {
...
/*lint +e661 */
  {
    ...
    if((req_sib == rrc_SIB19) &&
       ((rrc_dsds_int_data.fallback_type == TRM_FALLBACK_GSM_PREF) ||
        (rrc_dsds_int_data.fallback_type == TRM_FALLBACK_LTE_PREF)  ||
        (rrc_dsds_int_data.fallback_type == TRM_FALLBACK_UMTS_PREF)) &&  /*Fallback active on current NON-DDS sub*/
       (sibdb.active_cell_sibs_valid) &&
       (rrc_sibdb_index == sibdb.active_cell_index)
       && rrc_sibdb_return_psuedo_sib19(wrrc_as_id) == NULL) /*Make sure we prioritize Pseudo sib19 if available over fallback sib19 */
    {
      if(RRC_CAMPED_ON == rrc_ccm_get_curr_camping_status(wrrc_as_id,RRC_PROCEDURE_MCMR)) 
      {
        return rrc_get_gsm_fallback_sib19_ptr(wrrc_as_id);
      }
    }
#endif
    return( NULL );
  ...
  return( rrc_sibdb_get_sib_ptr_for_cell(wrrc_as_id, rrc_sibdb_index, req_sib ));
...
===========================================================================*/
void rrcsibdb_free_pseudo_sib19(sys_modem_as_id_e_type wrrc_as_id)
{
...
)
{
   ...
   rrc_SysInfoType1 *sib1_ptr = rrc_sibdb_return_sib_for_srv_cell(wrrc_as_id,rrc_SIB1);
  ...
  cell_info.cell_id = cell_id;
   ...
   pseudo_sib19_ptr->utra_PriorityInfoList.utra_ServingCell.priority = rrcnv_data.rrc_nv_psuedo_sib19_nv.utra_priority;
   pseudo_sib19_ptr->utra_PriorityInfoList.utra_ServingCell.s_PrioritySearch1 = rrcnv_data.rrc_nv_psuedo_sib19_nv.s_priority_search1;
   pseudo_sib19_ptr->utra_PriorityInfoList.utra_ServingCell.threshServingLow = rrcnv_data.rrc_nv_psuedo_sib19_nv.thresh_serving_low;
   ...
   for (i = 0; i < earfcn_list.count; i++)
   {
      ...
      pseudo_sib19_ptr->eutra_FrequencyAndPriorityInfoList.elem[i].measurementBandwidth = rrcnv_data.rrc_nv_psuedo_sib19_nv.eutra_meas_bandwidth;
      pseudo_sib19_ptr->eutra_FrequencyAndPriorityInfoList.elem[i].priority = rrcnv_data.rrc_nv_psuedo_sib19_nv.eutra_priority;
      pseudo_sib19_ptr->eutra_FrequencyAndPriorityInfoList.elem[i].threshXhigh = rrcnv_data.rrc_nv_psuedo_sib19_nv.eutra_thresh_high;
      pseudo_sib19_ptr->eutra_FrequencyAndPriorityInfoList.elem[i].threshXlow = rrcnv_data.rrc_nv_psuedo_sib19_nv.eutra_thresh_low;
      pseudo_sib19_ptr->eutra_FrequencyAndPriorityInfoList.elem[i].qRxLevMinEUTRA = rrcnv_data.rrc_nv_psuedo_sib19_nv.eutra_qrxlevmin;
...
)
{
  ...
  return( rrc_sibdb_return_sib_for_db_index
          (wrrc_as_id,  req_sib, /* requested sib */
...
FUNCTION RRC_SIBDB_FREE_ALL_DECODEDSIBS_FOR_A_CELL_FROM_OTHER_SUB

DESCRIPTION

  This function sets the SIB pointers which are received from alternate SUB to NULL.
...
===========================================================================*/
void rrc_sibdb_set_decoded_sibs_to_null
(
  sys_modem_as_id_e_type wrrc_as_id, 
...
)
{
    ...
    rrc_SysInfoType3 * sib3_ptr = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_SIB3, sibdb.active_cell_index);

    if (sib3_ptr)
    {
      uint32 cell_id = rrc_translate_cell_id(wrrc_as_id,&sib3_ptr->cellIdentity);

      if ((rrc_bcch_plmns_match(wrrc_as_id,rrc_psuedo_sib19.plmn_id,sibdb.cell[sibdb.active_cell_index].plmn_id))
      ...
      else
      {
         ...
         rrc_psuedo_sib19.cell_id = cell_id;
         rrc_psuedo_sib19.sib19_ptr = rrcsibdb_get_pseudo_sib19(wrrc_as_id,cell_id,sibdb.cell[sibdb.active_cell_index].plmn_id);
...
{ 
  ...
  {                                                  
    ...
    default:                                           
      WRRC_MSG1_MED("Unknown/Unsup value tag %d", RRCGET_SIB_TYPE_AND_TAG(sib_type_and_tag));  
      break;                                         
  } 
}                                                       
...
===========================================================================*/
void rrc_sibdb_clear_active_cell_status(sys_modem_as_id_e_type wrrc_as_id)
{
...
===========================================================================*/
void rrc_sibdb_init(sys_modem_as_id_e_type wrrc_as_id)
{
  uint32 index; /* Index to database */

  WRRC_MSG1_HIGH("Initializing the SIB-DB, power up init done %d",rrcsibproc_ext_data.rrcsib_powerup_init_done);
...
#ifdef FEATURE_WCDMA_DEFERRED_SIB11_12_READING
  rrcsib_collect_data.deferred_sib_info.val_tag.sib11_val_tag =0;
  rrcsib_collect_data.deferred_sib_info.val_tag.sib11bis_val_tag =0;
  rrcsib_collect_data.deferred_sib_info.val_tag.sib12_val_tag =0;
  rrcsib_collect_data.deferred_sib_info.val_tag.sib18_val_tag =0;
#ifdef FEATURE_WCDMA_TO_LTE
  rrcsib_collect_data.deferred_sib_info.val_tag.sib19_val_tag =0;
#endif
#endif
  ...
  if (rrcsibproc_ext_data.rrcsib_powerup_init_done == FALSE)
  {
    ...
    {
      rrcsibdb_int_data.rrcsib_cn[index].index_used = FALSE;
    }
  }
...
)
{
  ...
  if( sib_ptr != NULL )
  {
    ...
    occured */
    (void)rrcasn1_free_pdu(wrrc_as_id, pdu_num, sib_ptr );
  }
}
...
)
{
  ...
  if( sibdb.cell[index].sibs.sib1 != NULL )
  {
    rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType1_PDU,
                       (void *)sibdb.cell[index].sibs.sib1);
    sibdb.cell[index].sibs.sib1 = NULL;
  }
  if( sibdb.cell[index].sibs.sib2 != NULL )
  {
    rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType2_PDU,
  ...
  if( sibdb.cell[index].sibs.sib4 != NULL )
  {
    rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType4_PDU,
                       (void *)sibdb.cell[index].sibs.sib4);
    sibdb.cell[index].sibs.sib4 = NULL;
  }
  if( sibdb.cell[index].sibs.sib5 != NULL )
  {
    rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType5bis_PDU,
                       (void *)sibdb.cell[index].sibs.sib5);
    sibdb.cell[index].sibs.sib5 = NULL;
  }
  if( sibdb.cell[index].sibs.sib6 != NULL )
  {
    rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType6_PDU,
                       (void *)sibdb.cell[index].sibs.sib6);
    sibdb.cell[index].sibs.sib6 = NULL;
  }
  if( sibdb.cell[index].sibs.sib7 != NULL )
  {
    rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType7_PDU,
                       (void *)sibdb.cell[index].sibs.sib7);
    sibdb.cell[index].sibs.sib7 = NULL;
  }
  if( sibdb.cell[index].sibs.sib11 != NULL )
  {
    rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType11_PDU,
                       (void *)sibdb.cell[index].sibs.sib11);
    sibdb.cell[index].sibs.sib11 = NULL;
  }
  if( sibdb.cell[index].sibs.sib12 != NULL )
  {
    rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType12_PDU,
                       (void *)sibdb.cell[index].sibs.sib12);
    sibdb.cell[index].sibs.sib12 = NULL;
  }
  if( sibdb.cell[index].sibs.sib18 != NULL )
  {
    rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType18_PDU,
  ...
  if( sibdb.cell[index].sibs.sib11bis != NULL )
  {
    rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType11bis_PDU,
...
{    
  ...
  for( index=0; index<=SIBDB_MAX_INDEX; index++ )
  {
    ...
    if( sibdb.cell[index].index_used == TRUE )
    {
      ...
      if( (rrc_bcch_plmns_match(wrrc_as_id, (plmn), (sibdb.cell[index].plmn_id))) &&
...
{    
  ...
  for( index=0; index<=SIBDB_MAX_INDEX; index++ )
  {
    ...
    if( sibdb.cell[index].index_used == TRUE )
    {
      ...
      {
        if((rrcsib_is_need_to_cleanup_cell(wrrc_as_id,event_index) == TRUE) &&
            (sibdb.cell[index].six_hr_count  >=  MAX_CELL_INDEX_COUNT_SIXHR_CELL_SEL_RESEL))
        {
          WRRC_MSG3_HIGH("Index=%d, SIBs valid %d, Active index %d, ",index,sibdb.active_cell_sibs_valid,sibdb.active_cell_index);
          ...
          {
            rrc_sibdb_clear_a_cell_index(wrrc_as_id,index);
...
)
{
  ...
  {
    WRRC_ERR0_FATAL("Age index greater than MAX");
  }
...
)
{
  ...
  if(!( sibdb.cell[new_cell_index].age_index <= SIBDB_MAX_INDEX ))
  {
    WRRC_ERR0_FATAL("Age index is greater than MAX");
  }
...
{    
...
#endif
    rrc_sibdb_cell_header_init(wrrc_as_id, 0);   /* Initialize the cell header for this cell */
    sibdb.num_stored_cells++;   
#ifdef FEATURE_FEMTO_CSG
    WRRC_MSG3_HIGH("New SIBDB Index created - %d, %d num_cells, %d num_csg_cells", 0, sibdb.num_stored_cells, sibdb.num_csg_cells);
#else
  ...
  database. */
  if( (index = rrc_sibdb_find_valid_cell_index(wrrc_as_id,plmn, cell,event_index)) != SIBDB_INVALID_INDEX )
  {
    ...
    to be written again. */
    rrc_sibdb_adjust_age_index(wrrc_as_id, index );
    ...
    /*Stop SIB7 validity timer if we found SIB DB index and its is a different cell
      since we dont know when we last read SIB 7 on this cell*/
    if(sibdb.cell[index].cell_id != rrc_ccm_get_curr_camped_cell_id(wrrc_as_id))
    {
       WRRC_MSG0_MED("SIB DB index found for different cell -- Stop SIB7 validity timer");
  ...
  for( index=0; index<=SIBDB_MAX_INDEX; index++ )
  {
    ...
    {
      rrc_sibdb_adjust_age_index(wrrc_as_id, index ); /* Adjust age index for cells */
...
#endif	 
      rrc_sibdb_cell_header_init(wrrc_as_id,index);   /* Initialize the cell header */
      sibdb.num_stored_cells++;   
#ifdef FEATURE_FEMTO_CSG
      WRRC_MSG3_HIGH("New SIBDB Index created - %d, %d num_cells %d num_csg", index, sibdb.num_stored_cells, sibdb.num_csg_cells);
#else
  ...
  if(is_csg)
  {
    ...
    {
      /*replace femto cell*/
      rrc_sibdb_adjust_age_index(wrrc_as_id, csg_index );
      sibdb.cell[csg_index].plmn_id = plmn;
      sibdb.cell[csg_index].cell_id = cell;
      sibdb.cell[csg_index].csg_id = csg;
      rrc_sibdb_cell_header_init(wrrc_as_id,csg_index);
      WRRC_MSG3_HIGH("SIBDB Index created - %d, %d num_cells, %d num_csg",csg_index, sibdb.num_stored_cells, sibdb.num_csg_cells);
      return csg_index;
    }
    else
    {
      /*replace macro cell*/
      rrc_sibdb_adjust_age_index(wrrc_as_id, macro_index );
      ...
      rrc_sibdb_cell_header_init(wrrc_as_id,macro_index);
      sibdb.num_csg_cells++;
      WRRC_MSG3_HIGH("SIBDB Index created - %d, %d num_cells, %d num_csg",macro_index, sibdb.num_stored_cells, sibdb.num_csg_cells);
      return macro_index;
    }
  }
  else
  ...
  rrc_sibdb_adjust_age_index(wrrc_as_id, l_index );
  sibdb.cell[l_index].plmn_id = plmn;  /* Copy the PLMN id */
  sibdb.cell[l_index].cell_id = cell;  /* Copy the Cell id */
  rrc_sibdb_cell_header_init(wrrc_as_id,l_index); /* Initialize the cell header */
...
{                           
  ...
  if( (index = rrc_sibdb_find_cell_index(wrrc_as_id,plmn, cell)) != SIBDB_INVALID_INDEX )
  {
    /* Use the generic function to return sibs for a index */
    return( rrc_sibdb_return_sib_for_db_index(wrrc_as_id, req_sib, index ));
  }
  else
  {
    /* The requested SIB is not stored in the database. */
    WRRC_MSG3_HIGH("Match not found for given PLMN and cell id MCC %d, MNC %d, Cell ID %d", 
...
/*lint +e661 */
  ...
  if( sib == rrc_MIB )
  {
      ...
      )
    {
        ...
        if( sibdb.cell[dbindex].sib_valid[count] )
        {
          ...
          else
          {
            if(sibdb.cell[dbindex].sib_value_tag[count] != INVALID_MIB_SB_VALUE_TAG)
            {
              ...
              {
                if((sib_ptr = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,count, dbindex)) != NULL)
                {
                  WRRC_MSG2_HIGH("Sched. Info DB-Idx %d for SIB %d", dbindex, count);
                  rrcsib_update_sched_info_from_mib_sb(wrrc_as_id,(uint32)count,sib_ptr, index);
                }
              }
            }
            else
      ...
      {
        rrc_sibdb_copy_deferred_sibdb_to_tsibdb(wrrc_as_id,dbindex,index, mib_ptr, rrc_MIB);
      }
#endif
    ...
    while(sb_ptr->n > n_index)
    {
      ...
      a match */
      rrcsib_get_sib_type_from_sibsb_type_and_tag(wrrc_as_id,&sb_ptr->elem[n_index].sibSb_Type,
                                                  &sib_type_tag);
        ...
        if(sibdb.cell[dbindex].sib_valid[sib_type_tag.sib_type])
        {
             ...
             (sibdb.cell[dbindex].sib_value_tag[sib_type_tag.sib_type]))
          {
            ...
            else
            {
              ...
              {
                if((sib_ptr = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,sib_type_tag.sib_type, dbindex)) != NULL)
                {
                  WRRC_MSG2_HIGH("Sched. Info DB-Idx %d for SIB %d", dbindex, sib_type_tag.sib_type);
                  rrcsib_update_sched_info_from_mib_sb(wrrc_as_id,(uint32)sib_type_tag.sib_type,sib_ptr, index);
                }
              }
  ...
  else 
  {
    ...
    if( sib == rrc_SB1 )
    {
      ...
      {
        rrc_sibdb_copy_deferred_sibdb_to_tsibdb(wrrc_as_id,dbindex, index, sb1_ptr, rrc_SB1);
      }
#endif
    }
    else if( sib == rrc_SB2 ) 
    {
      ...
      {
        rrc_sibdb_copy_deferred_sibdb_to_tsibdb(wrrc_as_id,dbindex, index, sb2_ptr, rrc_SB2);
      }
#endif
    }
    ...
    while((ptr != NULL) && (ptr->n > n_index))
    {
      ...
      at the corrosponding SIBs stored value tag for
      a match */
      rrcsib_get_sib_type_from_sib_type_and_tag(wrrc_as_id,&ptr->elem[n_index].sib_Type,
                                                &sib_type_tag);
  ...
  boolean deferred_meas_read_supported = FALSE;
   if(rrcsib_can_event_support_deferred_sib(wrrc_as_id,index) == TRUE)
   {
     deferred_meas_read_supported =
                 rrcsib_can_nw_support_deferred_sib(wrrc_as_id,dbindex);
   }
#endif
  ...
  if( sibdb.cell[dbindex].sibs.mib != NULL )
  {
    ...
    while( sb_ptr->n > n_index )
    {
      ...
      switch(RRCGET_SIB_TYPE_AND_TAG((&(sb_ptr->elem[n_index].sibSb_Type))))
      {
        ...
        case RRCGET_COMMON_CHOICE_TYPE(rrc_SIBSb_TypeAndTag_sysInfoType11):
          ...
          {
            rrcsib_collect_data.deferred_sib_info.val_tag.sib11_val_tag = sb_ptr->elem[n_index].sibSb_Type.u.sysInfoType11;
            WRRC_MSG1_HIGH("Deferred SIB: SIB11 value tag stored %d", rrcsib_collect_data.deferred_sib_info.val_tag.sib11_val_tag);
          }
          else
#endif
        ...
        case RRCGET_COMMON_CHOICE_TYPE(rrc_SIBSb_TypeAndTag_sysInfoType12):
          ...
          {
            rrcsib_collect_data.deferred_sib_info.val_tag.sib12_val_tag = sb_ptr->elem[n_index].sibSb_Type.u.sysInfoType12;
            WRRC_MSG1_HIGH("Deferred SIB: SIB12 value tag stored %d", rrcsib_collect_data.deferred_sib_info.val_tag.sib12_val_tag);
          }
          else
#endif
        ...
        case RRCGET_COMMON_CHOICE_TYPE(rrc_SIBSb_TypeAndTag_sysInfoType18):
          ...
          {
            rrcsib_collect_data.deferred_sib_info.val_tag.sib18_val_tag = sb_ptr->elem[n_index].sibSb_Type.u.sysInfoType18;
            WRRC_MSG1_HIGH("Deferred SIB: SIB18 value tag stored %d", rrcsib_collect_data.deferred_sib_info.val_tag.sib18_val_tag);
          }
          else
#endif          
    ...
    if(EXTENSION_SIB_SCHED_PRESENT_IN_MIB(mib_ptr))
    {
      ...
      while( ext_siblist_ptr->n > n_index )
      {
        ...
        {
          WRRC_MSG1_MED("sib11bis:Updating vtag =%d for sib11bis",
              ext_siblist_ptr->elem[n_index].valueTagInfo.u.cellValueTag);
#ifdef FEATURE_WCDMA_DEFERRED_SIB11_12_READING
          ...
          {
            rrcsib_collect_data.deferred_sib_info.val_tag.sib11bis_val_tag = ext_siblist_ptr->elem[n_index].valueTagInfo.u.cellValueTag;
          }
          else
#endif
          {
    ...
    if(EXTENSION_SIB_SCHED_2_PRESENT_IN_MIB(mib_ptr))
        {
          ...
          while(ext_siblist2_ptr->n > n_index)
          {
            ...
            {
              WRRC_MSG1_MED("sib19:Updating vtag =%d for sib19",
              ext_siblist2_ptr->elem[n_index].valueTagInfo.u.cellValueTag);
#ifdef FEATURE_WCDMA_DEFERRED_SIB11_12_READING
              ...
              {
                rrcsib_collect_data.deferred_sib_info.val_tag.sib19_val_tag = ext_siblist2_ptr->elem[n_index].valueTagInfo.u.cellValueTag;
              }
              else
#endif  
              {
      ...
      while((ptr!=NULL) && (ptr->n > n_index))
      {
        ...
        switch(ptr->elem[n_index].sib_Type.t)
        {
          ...
          case RRCGET_COMMON_CHOICE_TYPE(rrc_SIB_TypeAndTag_sysInfoType11):
            ...
            {
              rrcsib_collect_data.deferred_sib_info.val_tag.sib11_val_tag = ptr->elem[n_index].sib_Type.u.sysInfoType11;
              WRRC_MSG1_HIGH("Deferred SIB: SIB11 value tag stored %d", rrcsib_collect_data.deferred_sib_info.val_tag.sib11_val_tag);
            }
            else
#endif
          ...
          case RRCGET_COMMON_CHOICE_TYPE(rrc_SIB_TypeAndTag_sysInfoType12):
            ...
            {
              rrcsib_collect_data.deferred_sib_info.val_tag.sib12_val_tag = ptr->elem[n_index].sib_Type.u.sysInfoType12;
              WRRC_MSG1_HIGH("Deferred SIB: SIB12 value tag stored %d", rrcsib_collect_data.deferred_sib_info.val_tag.sib12_val_tag);
            }
            else
        #endif
          ...
          case RRCGET_COMMON_CHOICE_TYPE(rrc_SIB_TypeAndTag_sysInfoType18):
            ...
            {
              rrcsib_collect_data.deferred_sib_info.val_tag.sib18_val_tag = ptr->elem[n_index].sib_Type.u.sysInfoType18;
              WRRC_MSG1_HIGH("Deferred SIB: SIB18 value tag stored %d", rrcsib_collect_data.deferred_sib_info.val_tag.sib18_val_tag);
            }
            else
        #endif
...
)
{
  ...
  {
    if( rrcsibdb_int_data.rrcsib_cn[count].index_used == FALSE )
    {
  ...
  {
    /* Ok to register this. */
    rrcsibdb_int_data.rrcsib_cn[count].procedure = procedure;   /* Copy procedure   */
    rrcsibdb_int_data.rrcsib_cn[count].sib = sib;         /* Copy sib type          */
    rrcsibdb_int_data.rrcsib_cn[count].index_used = TRUE; /* This index is now used */
    rrcsib_set_priority(wrrc_as_id,count,sib);    /*Set priority for this sibtype,which will
...
)
{
  ...
  for( count=0; count < RRC_TOT_SIB_CHG_N; count++ )
  {
    if( rrcsibdb_int_data.rrcsib_cn[count].index_used == TRUE )
    {
      if( (rrcsibdb_int_data.rrcsib_cn[count].procedure == procedure) &&
          (rrcsibdb_int_data.rrcsib_cn[count].sib == sib) )
      {
        /* Match found. This index is no longer used. */
        rrcsibdb_int_data.rrcsib_cn[count].index_used = FALSE;
        break; /* Break from for loop */
      }
    }
  }
...
===========================================================================*/
void rrc_sibdb_notify_sib_change_to_procs(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  for(priority = 1;priority<=rrcsibdb_int_data.rrcsib_cn_max_priority;priority++)
  {
    for(procedure=(uint8)MAX_NUM_OF_PROCEDURES;procedure>(uint8)0;procedure--)
    {
      if(rrcsib_is_it_registered_procedure_for_sibchange(wrrc_as_id,(rrc_proc_e_type)procedure) == TRUE)
      {
        if(rrcsib_check_and_prepare_sib_change_mask(wrrc_as_id,&sib_change_mask,(rrc_proc_e_type)procedure,priority) == TRUE)
        {
  ...
  */
  ...
  {
    if( (rrcumi_check_ue_timer_constants_update_status(wrrc_as_id, FALSE))
...
)
{
  ...
  {
    /* If the MIB is valid */
    mib = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_MIB,dbindex);
    if(mib != NULL)
    {
...
===========================================================================*/
void rrc_sibdb_clear_active_cell_sib_change(sys_modem_as_id_e_type wrrc_as_id)
{
  uint32 index;
...
===========================================================================*/
  uint32 rrc_get_serving_cell_mib_value_tag(sys_modem_as_id_e_type wrrc_as_id)
  {
    uint32 dbindex;
    if(!(sibdb.active_cell_sibs_valid))
    {
...
{    
  ...
  for( index=0; index<=SIBDB_MAX_INDEX; index++ )
  {
    ...
    if( sibdb.cell[index].index_used == TRUE )
    {
      ...
      if( (freq == sibdb.cell[index].freq) &&
          ...
          (rrc_bcch_plmns_match(wrrc_as_id, (plmn), (sibdb.cell[index].plmn_id))))
      {
        if((rrcsib_is_need_to_cleanup_cell(wrrc_as_id,event_index) == TRUE) &&
            (sibdb.cell[index].six_hr_count  >=  MAX_CELL_INDEX_COUNT_SIXHR_CELL_SEL_RESEL))
        {
  ...
  /*Here since we have found a SIBDB index with freq and psc
    SIB7 will not be decoded hence stop SIB7 validity timer if already running*/
  if((index != SIBDB_INVALID_INDEX) && (sibdb.cell[index].cell_id != rrc_ccm_get_curr_camped_cell_id(wrrc_as_id)))
  {  
     WRRC_MSG0_MED("SIB DB index found for different cell -- Stop SIB7 validity timer");
     rrctmr_stop_timer(wrrc_as_id, RRCTMR_SIB7_EXPIRATION_TIMER);
  }
...
{    
  ...
  for (count=0; count<(uint8)MAX_SIB_EVENTS_INDEX;count++)
  {
    ...
    if (sib_events_ptr->event[count].event_active == TRUE)
    {
      ...
      WRRC_MSG3_HIGH("SIB Heap: %d Ev_actv, %d SIBDB Cell Idx,DB idx found=%d", count, cell_index_for_current_active_event,
  ...
  for( count=0; count<=SIBDB_MAX_INDEX; count++ )
  {
        ...
        )
    {
      ...
      rrc_sibdb_cell_header_init(wrrc_as_id,count);
  ...
  if(status)
  {
    rrc_sibdb_adjust_age_index(wrrc_as_id,cell_index_for_current_active_event);
  }
...
void  rrc_sibdb_update_six_hr_timer(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8     count;
...
===========================================================================*/
void rrc_sibdb_clear_a_cell_index(sys_modem_as_id_e_type wrrc_as_id,uint32 dbindex)
{
...
#ifdef FEATURE_FEMTO_CSG
    rrc_sibdb_decrement_age_index(wrrc_as_id,dbindex);
#else
    rrc_sibdb_adjust_age_index(wrrc_as_id, dbindex ); /* Why do I need to do this??Adjust age index for cells */
#endif
    rrc_sibdb_cell_header_init(wrrc_as_id,dbindex);
...
)
{
  ...
  *sib_mask = 0;
  ...
  {
    if((rrcsibdb_int_data.rrcsib_cn[count].procedure == procedure)&&
        (rrcsibdb_int_data.rrcsib_cn[count].index_used == TRUE)&&
        (rrcsibdb_int_data.rrcsib_cn[count].priority== priority)
      )
    {
      sib_change_mask |= (MIB_BIT<<(rrcsibdb_int_data.rrcsib_cn[count].sib));
      /* Indicate which SIBs are changed */
      gen_mask = MIB_BIT;
      for(i = 0;i < RRC_TOT_SIBS;i++, gen_mask <<= 1)
      {
        if((sibdb.active_cell_sib_changed[i] == TRUE)&&
           (rrcsibdb_int_data.rrcsib_cn[count].sib ==(rrc_SIB_type)i)
          )
        {
          *sib_mask |= gen_mask;
          WRRC_MSG5_HIGH( 
...
===========================================================================*/
uint8 rrcsib_is_it_registered_procedure_for_sibchange(sys_modem_as_id_e_type wrrc_as_id,rrc_proc_e_type procedure)
{
  uint8 count;
  uint8 status=FALSE;
...
===========================================================================*/
void rrcsib_set_priority(sys_modem_as_id_e_type wrrc_as_id,uint32 index,rrc_SIB_type sib)
{
  ...
  switch(sib)
  {
    case rrc_SIB5:
    case rrc_SIB6:
      rrcsibdb_int_data.rrcsib_cn[index].priority = 1;
      break;
    ...
    case rrc_SIB19:
      ...
      rrcsibdb_int_data.rrcsib_cn[index].priority = 2;
      break;
     
    case rrc_SIB1:
    case rrc_SIB3:
    case rrc_SIB4:
      rrcsibdb_int_data.rrcsib_cn[index].priority = 3;
      break;
      
    case rrc_SIB2:
      rrcsibdb_int_data.rrcsib_cn[index].priority = 4;
      break;
      
    default:
      rrcsibdb_int_data.rrcsib_cn[index].priority = 5;
      break;
  }
  
  if(rrcsibdb_int_data.rrcsib_cn_max_priority < rrcsibdb_int_data.rrcsib_cn[index].priority)
  {
    rrcsibdb_int_data.rrcsib_cn_max_priority = rrcsibdb_int_data.rrcsib_cn[index].priority; 
  }
}
...
)
{
  ...
  for( count=0; count <=SIBDB_MAX_INDEX; count++ )
  {
    if( sibdb.cell[count].index_used == TRUE)
    {
      WRRC_MSG3_HIGH("Cell I.d =%d, fre=%d,scr_code=%d",sibdb.cell[count].cell_id,
...
)
{
  ...
  if(sibdb.cell[sibdb.active_cell_index].sib_valid[rrc_SIB11bis] &&
     (rrcsib_check_if_extension_sib_present(wrrc_as_id,sibdb.active_cell_index) == TRUE)
    )
  {
...
)
{
  ...
  sib3_ptr = rrc_sibdb_return_sib_for_srv_cell(wrrc_as_id, rrc_SIB3);
...
)
{
  ...
  if((rrcsib_collect_data.deferred_sib_info.cell_id != sibdb.cell[dbindex].cell_id) ||
      !(rrc_bcch_plmns_match(wrrc_as_id,rrcsib_collect_data.deferred_sib_info.plmn_id, sibdb.cell[dbindex].plmn_id))
     )
  {
  ...
  else
  {
    ...
    if( sib == rrc_MIB )
    {
      ...
      while( sb_ptr->n > idx )
      {
          ...
          at the corrosponding SIBs stored value tag for
          a match */
        rrcsib_get_sib_type_from_sibsb_type_and_tag(wrrc_as_id,&sb_ptr->elem[idx].sibSb_Type,
                                                    &sib_type_tag);
        if(((sib_type_tag.sib_type == rrc_SIB11) &&
            (sib_type_tag.sib_tag == rrcsib_collect_data.deferred_sib_info.val_tag.sib11_val_tag))
            ||
            ((sib_type_tag.sib_type == rrc_SIB12) &&
            (sib_type_tag.sib_tag == rrcsib_collect_data.deferred_sib_info.val_tag.sib12_val_tag))
            || ((sib_type_tag.sib_type == rrc_SIB18) &&
                 (sib_type_tag.sib_tag == rrcsib_collect_data.deferred_sib_info.val_tag.sib18_val_tag))
          )
        {
          if((sib_type_tag.sib_type == rrc_SIB11)&&(rrcsib_collect_data.deferred_sib_info.dsib.sib11.sib_tot_length > sib_events_ptr->event[index].tsib[rrc_SIB11].sib_tot_length))
          {
            WRRC_MSG2_HIGH("Deferred SIB: Copying %d segments of SIB11 from DSIB and its length %d", rrcsib_collect_data.deferred_sib_info.dsib.sib11.seg_index,
            ...
            b_copy( rrcsib_collect_data.deferred_sib_info.dsib.sib11.sib_data_ptr,  /* Source Pointer */
                    0,    /* Source Offset is always 0 since we copy all the received bits */
                    sib_events_ptr->event[index].tsib[rrc_SIB11].sib_data_ptr, /* Destination Pointer */
                    0,  /* Destination Offset */
                    rrcsib_collect_data.deferred_sib_info.dsib.sib11.sib_tot_length
                  );
          }
          else if((sib_type_tag.sib_type == rrc_SIB12)&&(rrcsib_collect_data.deferred_sib_info.dsib.sib12.sib_tot_length > sib_events_ptr->event[index].tsib[rrc_SIB12].sib_tot_length))
          {
            WRRC_MSG2_HIGH("Deferred SIB: Copying %d segments of SIB12 from DSIB and its length %d", rrcsib_collect_data.deferred_sib_info.dsib.sib12.seg_index,
            ...
            b_copy( rrcsib_collect_data.deferred_sib_info.dsib.sib12.sib_data_ptr,  /* Source Pointer */
                    0,    /* Source Offset is always 0 since we copy all the received bits */
                    sib_events_ptr->event[index].tsib[rrc_SIB12].sib_data_ptr, /* Destination Pointer */
                    0,  /* Destination Offset */
                    rrcsib_collect_data.deferred_sib_info.dsib.sib12.sib_tot_length
                  );
          }
          else if((sib_type_tag.sib_type == rrc_SIB18)&&(rrcsib_collect_data.deferred_sib_info.dsib.sib18.sib_tot_length > sib_events_ptr->event[index].tsib[rrc_SIB18].sib_tot_length))
          {
            WRRC_MSG2_HIGH("Deferred SIB: Copying %d segments of SIB18 from DSIB and its length %d", rrcsib_collect_data.deferred_sib_info.dsib.sib18.seg_index,
            ...
            b_copy( rrcsib_collect_data.deferred_sib_info.dsib.sib18.sib_data_ptr,  /* Source Pointer */
                    0,    /* Source Offset is always 0 since we copy all the received bits */
                    sib_events_ptr->event[index].tsib[rrc_SIB18].sib_data_ptr, /* Destination Pointer */
                    0,  /* Destination Offset */
                    rrcsib_collect_data.deferred_sib_info.dsib.sib18.sib_tot_length
                  );
          }
          ...
          {
            if(rrcsib_collect_data.deferred_sib_info.deferred_sib_seg_db[i].sib_type == sib_type_tag.sib_type)
            {
              (void) rrcsib_store_sib_seg_in_tsib_optim_db(wrrc_as_id,rrcsib_collect_data.deferred_sib_info.deferred_sib_seg_db[i].sib_type,
                                                   rrcsib_collect_data.deferred_sib_info.deferred_sib_seg_db[i].sfn_prime,
                                                   rrcsib_collect_data.deferred_sib_info.deferred_sib_seg_db[i].seg_index,
                                                   rrcsib_collect_data.deferred_sib_info.deferred_sib_seg_db[i].sib_seg_type,
                                                   rrcsib_collect_data.deferred_sib_info.deferred_sib_seg_db[i].seg_data_length,
                                                   rrcsib_collect_data.deferred_sib_info.deferred_sib_seg_db[i].seg_data_ptr,
                                                   index);
  
            }
          }
          (void) rrcsib_process_sib_seg_in_tsib_optim_db(wrrc_as_id,sib_type_tag.sib_type,index);
        }
        idx++;
      }
    }
    else if( sib == rrc_SB1 || sib == rrc_SB2 )
    {
      ...
      while( ptr->n > idx)
      {
          ...
          at the corrosponding SIBs stored value tag for
          a match */
        rrcsib_get_sib_type_from_sib_type_and_tag(wrrc_as_id,&ptr->elem[idx].sib_Type,
                                                    &sib_type_tag);
        if(((sib_type_tag.sib_type == rrc_SIB11) &&
            (sib_type_tag.sib_tag == rrcsib_collect_data.deferred_sib_info.val_tag.sib11_val_tag))
            ||
        ...
        {
          WRRC_MSG2_HIGH("Deferred SIB: Deferred SIB val tag %d, SIB val tag %d", rrcsib_collect_data.deferred_sib_info.val_tag.sib11_val_tag ,sib_type_tag.sib_tag);
        }
        idx++;
      }
    }
    ...
    while((ext_siblist_ptr != NULL) && (ext_siblist_ptr->n > idx))
    {
      ...
      {
        WRRC_MSG1_HIGH("Deferred SIB: Copying %d segments of SIB11bis from DSIB", rrcsib_collect_data.deferred_sib_info.dsib.sib11bis.seg_index);

        if(ext_siblist_ptr->elem[idx].valueTagInfo.u.cellValueTag == 
            rrcsib_collect_data.deferred_sib_info.val_tag.sib11bis_val_tag)
        {
          if(rrcsib_collect_data.deferred_sib_info.dsib.sib11bis.sib_tot_length > sib_events_ptr->event[index].tsib[rrc_SIB11bis].sib_tot_length)
          {
            ...
            b_copy( rrcsib_collect_data.deferred_sib_info.dsib.sib11bis.sib_data_ptr,  /* Source Pointer */
                    0,    /* Source Offset is always 0 since we copy all the received bits */
                    sib_events_ptr->event[index].tsib[rrc_SIB11bis].sib_data_ptr, /* Destination Pointer */
                    0,  /* Destination Offset */
                    rrcsib_collect_data.deferred_sib_info.dsib.sib11bis.sib_tot_length
                  );
          }
  ...
  while((ext_siblist2_ptr != NULL) && (ext_siblist2_ptr->n > idx))
  {
    ...
    {
      WRRC_MSG1_HIGH("Deferred SIB: Copying %d segments of SIB19 from DSIB", rrcsib_collect_data.deferred_sib_info.dsib.sib19.seg_index);
  
      if(ext_siblist2_ptr->elem[idx].valueTagInfo.u.cellValueTag == 
         rrcsib_collect_data.deferred_sib_info.val_tag.sib19_val_tag)
      {
        if(rrcsib_collect_data.deferred_sib_info.dsib.sib19.sib_tot_length > sib_events_ptr->event[index].tsib[rrc_SIB19].sib_tot_length)
        {
          ...
          b_copy( rrcsib_collect_data.deferred_sib_info.dsib.sib19.sib_data_ptr,  /* Source Pointer */
                  0,    /* Source Offset is always 0 since we copy all the received bits */
                  sib_events_ptr->event[index].tsib[rrc_SIB19].sib_data_ptr, /* Destination Pointer */
                  0,  /* Destination Offset */
                  rrcsib_collect_data.deferred_sib_info.dsib.sib19.sib_tot_length
                );
        }
...
{
  qsh_mdump_collect_high(&sibdb, FPOS(rrc_sibdb_type,cell));
  if(sibdb.active_cell_sibs_valid)
  {
    qsh_mdump_collect_high(&sibdb.cell[sibdb.active_cell_index], sizeof(rrc_sibdb_cell_header_type));
  }
  qsh_mdump_collect_high(&sibdb.sib_cont_sibdb_index, sizeof(sibdb.sib_cont_sibdb_index));
  if(sibdb.rrc_decoded_sib_cont != NULL)
  {
    qsh_mdump_collect_high(&sibdb.rrc_decoded_sib_cont, sizeof(sibdb.rrc_decoded_sib_cont));
    qsh_mdump_collect_high(sibdb.rrc_decoded_sib_cont, sizeof(rrc_System_Information_Container));
  }
}
#endif
...
FUNCTION RRC_GET_GSM_FALLBACK_SIB19_PTR

DESCRIPTION

This function returns composed sib19 pointer by initializing just the utra_ServingCell info based on NV configuration.
GSM priorities will be appended to the meas_req before sending to lower layers.
...
===========================================================================*/
rrc_SysInfoType19 * rrc_get_gsm_fallback_sib19_ptr(sys_modem_as_id_e_type wrrc_as_id)
{
   if (rrc_gsm_fallback_sib19_ptr == NULL)
   {
     rrc_gsm_fallback_sib19_ptr = (rrc_SysInfoType19 *) rrc_malloc (sizeof(rrc_SysInfoType19));

     rrc_gsm_fallback_sib19_ptr->m.gsm_PriorityInfoListPresent = 0;
     rrc_gsm_fallback_sib19_ptr->m.eutra_FrequencyAndPriorityInfoListPresent = 0;
     rrc_gsm_fallback_sib19_ptr->m.v920NonCriticalExtensionsPresent = 0;
     rrc_gsm_fallback_sib19_ptr->utra_PriorityInfoList.m.utran_FDD_FrequencyListPresent = 0;
     rrc_gsm_fallback_sib19_ptr->utra_PriorityInfoList.m.utran_TDD_FrequencyListPresent = 0;

     rrc_gsm_fallback_sib19_ptr->utra_PriorityInfoList.utra_ServingCell.priority = rrcnv_data.rrc_gsm_fallback_info.sib19_Serving_utra_priority;
     rrc_gsm_fallback_sib19_ptr->utra_PriorityInfoList.utra_ServingCell.s_PrioritySearch1 = rrcnv_data.rrc_gsm_fallback_info.sib19_Serving_utra_s_PrioritySearch1;
     rrc_gsm_fallback_sib19_ptr->utra_PriorityInfoList.utra_ServingCell.m.s_PrioritySearch2Present = 0;
     rrc_gsm_fallback_sib19_ptr->utra_PriorityInfoList.utra_ServingCell.threshServingLow = rrcnv_data.rrc_gsm_fallback_info.sib19_Serving_utra_threshServingLow;
   }
   return rrc_gsm_fallback_sib19_ptr;
}

/*===========================================================================

FUNCTION RRC_FREE_GSM_FALLBACK_SIB19_PTR
...
===========================================================================*/
void rrc_free_gsm_fallback_sib19_ptr(sys_modem_as_id_e_type wrrc_as_id)
{
  if(rrc_gsm_fallback_sib19_ptr != NULL)
  {
    rrc_free(rrc_gsm_fallback_sib19_ptr);
  }
}
