...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrccsputil.c#12 $    $DateTime: 2024/09/20 01:56:28 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/22/20   ha      Enable HI AC even when LO AC absen through NV
08/16/21   gun     Trigger WRLF to NAS once W search is complete and before starting OOS service search on G.
07/14/20   ak      Made changes for FR 61612
07/14/20   gcs     Made changes to pass an address in API rrcmeas_ssgccs_send_sib19_info and changed the input argument to pointer type while populating 
                   sib19 information to SSGCCS client in wcdma_ssgccs_3g_cell_info_ind_s
07/14/20   gcs     Made changes to use Correct PLMN ID and Cell ID to check in SIB database after cell selection or reselection is 
                   successful to Network shared Cell while updating WCDMA Cell information to SSGCCS Client
07/14/20   gcs     Made changes to use MIB PLMN for getting SIBs from SIBDB in case of valid network sharing to update wcdma cell information to SSGCCS client
...
05/31/17   ha      FR42544 MPLMN periodic scan search
...
05/17/18   gc      Set acq_status in SERVICE_CNF, in case on no service
04/10/18   gv      Made changes to send irat abort req to RR in WTOW_SEARCH_RESUME when NAS sends abort plmn search request. 
...
09/02/16   gv      Do not let UE camp on B19 cell when SIB5 band indicator shows B5 during inter-f redirection
07/20/16   ad      Made changes to update the duty cycle related variables when W gets the grant.
07/11/16   sr      Changes to update cell status properly for network sharing scenarios
07/01/16   vs      Changes to ignore reset l1 confirm command in wait acq pending/wait acq states, if iniate_cell_selection is pending already
06/30/16   sr      Changes to update CCM cell loss when serving cell is barred for non eplmn or forbidden plmn cases
05/12/16   sn      f3 reduction changes BPLMN scenarios
05/04/16   sn      f3 density reduction changes
04/28/16   vs      Changes to skip freq previously camped in idle mode, if its found to have forbidden PLMN during ACQ DB scan
04/26/16   sas     Cleanup any active BPLMN before initiating blind redirection to LTE.
04/25/16   nr      Made changes to fix MOB compilation errors
04/08/16   sas     Changes for FR30857
03/30/16   sn      Changes for FR30515
03/17/16   sn      Changes for FR30881
03/17/16   sas     Changes to not initiate legacy RLF recovery if E-call is released before RLF with data call.
...
01/25/16   sr      Made changes to indicate NO SERVICE to MM for emergency calls with WRLF feature for OOS
01/22/16   nr      Made changes to remove irrelevant F3s
01/18/16   gv      Made changes to request for TRM lock before entering connected mode
12/10/15   sr      Changes to inform NO-Service to NAS before suitable search end upon lock wait timer expiry during COOS in DCH
11/26/15   vs      Changes to cleanup unnecessary f3s
11/25/15   sn      Changes to delay handling NAS proc indication during QTA
11/23/15   sas     Changes to inform NO-Service to NAS before suitable search end upon lock wait timer expiry during COOS.
10/29/15   vi      Made changes to add as_id to RSSI APIs
10/01/15   sr      Changes to trigger OOS when lock wait timer for state transition cell selection expires
09/25/15   sr      Changes to make state transition cell selection atomic
09/22/15   sr      Made changes to not send BCH release for DL weak with no lock
09/14/15   sr      Made changes to get default band from RF api for lock request
09/10/15   sas     Made changes to not release lock during RLF/DCH-FACH transitions if RRC is holding lock with High priority.
09/08/15   sr      Changes to not modify lock priority when updating UI with limited service during WRLF
08/27/15   sr      Changes to initialize NAS proc type to NONE during init
08/26/15   sn      WCDMA RRC changes for FR28001
08/25/15   sas     New argument added in rrccsp_check_plmn_match() to update ACQDB on need purpose.
08/04/15   sas     Changes to wait for Channel rel cnf before requesting lock on next frequency.
07/28/15   sr      Changes to release WL1 lock for state transitions and COOS scenarios before acquiring lock for acquisition
...
06/29/15   vi      Made changes to add f3s for logging complete acess class info
06/15/15   sn      Changes for new log packets for LAF
06/15/15   sas     Fetch the sibs using PLMN and cell_id for sending cell_param_req to L1 during cell selection.
06/09/15   sas     Release lock at the start of OOS in connecting state and re-acquire lock with access urgent before transmitting connection request for Non CS call cases.
...
03/11/15   bc      Changes to add all the shared PLMNs of the current camped cell to the list to be
...
02/20/15   vi      Made changes to req PLMN to LTE if deprio db is valid
...
02/19/15   sa      Made changes to fix the compilation for QTF
02/09/15   sas     Update ACQ-DB when SIB reading is skipped for VPLMN.
12/31/14   bc      Changes to replace event_report_payload with WRRC_EVENT_REPORT_PAYLOAD.
...
12/16/14   sn      Changes to send idle request before PN acq
10/31/14   bc      Changes to support additional requirements in FR 20971.
11/27/14   sr      NV control for FR23776
11/27/14   sr      Code reduction to remove GET_ALL_SIBS handling
11/25/14   gv      Removal of code changes under GAN feature
...
/* Global to hold the last sent service indication data */
  ...
  boolean rrc_csp_internal_service_req;
  sys_specialization_t  search_order_specialization;
}rrccsputil_int_data_type;
...
rrc_csp_select_ctrl_data_type  rrc_csp_curr_select_data_sim[MAX_RRC_STACKS];
rrc_csp_select_ctrl_data_type  prev_rrc_csp_curr_select_data_sim[MAX_RRC_STACKS];
#define rrc_csp_curr_select_data rrc_csp_curr_select_data_sim[WAS_ID_M]
#define prev_rrc_csp_curr_select_data prev_rrc_csp_curr_select_data_sim[WAS_ID_M]
extern rrc_csp_acq_entry_type  list_db_sim[MAX_RRC_STACKS][MAX_ACQ_DB_ENTRIES];
#define list_db list_db_sim[WAS_ID_M]
#else
...
** ----------------------------------------------------------------------- */
#ifdef FEATURE_DUAL_WCDMA 
LOCAL rrccsputil_int_data_type rrccsputil_int_data_sim[MAX_RRC_STACKS];
#define rrccsputil_int_data rrccsputil_int_data_sim[WAS_ID_M]
#else
LOCAL rrccsputil_int_data_type rrccsputil_int_data;
#endif /* FEATURE_DUAL_WCDMA  */
/* -----------------------------------------------------------------------
...
===========================================================================*/
/***/ __WCDMA_RRC_CODE_SEGMENT_NON_DEMANDPAGED__ /***/
boolean rrc_csp_check_320ms_bplmn(sys_modem_as_id_e_type wrrc_as_id)
{
  boolean ret_val=FALSE;
  if( (rrc_csp_int_data.bplmn_nw_sel_mode != SYS_NETWORK_SELECTION_MODE_AUTOMATIC) &&
      ((rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_PCH) || (rrc_get_state(wrrc_as_id) == RRC_STATE_URA_PCH)) && 
      (rrcllc_get_drx_info(wrrc_as_id) == (MIN_WTOW_NORMAL_BPLMN_DRX_CYCLE_LEN/2))
     )
  {
    ret_val=TRUE;
  }
...
===========================================================================*/
/***/ __WCDMA_RRC_CODE_SEGMENT_NON_DEMANDPAGED__ /***/
uint16 rrc_sub_sfn(sys_modem_as_id_e_type wrrc_as_id,uint16 sfn, uint16 subtract_by)
{
  if (sfn < subtract_by) 
  {
...
SIDE EFFECTS
                  None
===========================================================================*/
static boolean  rrc_csp_bcch_and_nas_lais_match(sys_modem_as_id_e_type wrrc_as_id,rrc_lai_type bcch_lai, rrc_lai_type nas_lai)
{
  uint8  i;

  /* If both the LAC and PLMNs match, declare a match for LAI */
  if(rrc_bcch_and_nas_plmns_match(wrrc_as_id,bcch_lai.plmn, nas_lai.plmn))
  {
...
===========================================================================*/
void rrccsp_get_supported_band_list(sys_modem_as_id_e_type wrrc_as_id,rrcueci_band_storage_type * ueci_band)
{
  ...
  if(   (RRC_CSP_IMT2K_BAND & rrc_csp_int_data.wcdma_band_mask)
     || (rrccsp_is_band_class_enabled_in_ui_rf(wrrc_as_id,RRC_CSP_IMT2K_BAND, FALSE))
    )
  {
  ...
  if((RRC_CSP_PCS_BAND & rrc_csp_int_data.wcdma_band_mask)
     || (rrccsp_is_band_class_enabled_in_ui_rf(wrrc_as_id,RRC_CSP_PCS_BAND, FALSE))
    )
  {
  ...
  if((RRC_CSP_WCDMA_850_BAND & rrc_csp_int_data.wcdma_band_mask)
     || (rrccsp_is_band_class_enabled_in_ui_rf(wrrc_as_id,RRC_CSP_WCDMA_850_BAND, FALSE))
    )
  {
  ...
  if((RRC_CSP_WCDMA_800_BAND & rrc_csp_int_data.wcdma_band_mask)
     || (rrccsp_is_band_class_enabled_in_ui_rf(wrrc_as_id,RRC_CSP_WCDMA_800_BAND, FALSE))
    )
  {
  ...
  if((RRC_CSP_WCDMA_1800_BAND & rrc_csp_int_data.wcdma_band_mask)
     || (rrccsp_is_band_class_enabled_in_ui_rf(wrrc_as_id,RRC_CSP_WCDMA_1800_BAND, FALSE))
    )
  {
  ...
  if((RRC_CSP_WCDMA_900_BAND & rrc_csp_int_data.wcdma_band_mask)
     || (rrccsp_is_band_class_enabled_in_ui_rf(wrrc_as_id,RRC_CSP_WCDMA_900_BAND, FALSE))
    )
  {
  ...
  if((RRC_CSP_WCDMA_1700_BAND & rrc_csp_int_data.wcdma_band_mask)
     || (rrccsp_is_band_class_enabled_in_ui_rf(wrrc_as_id,RRC_CSP_WCDMA_1700_BAND, FALSE))
    )
  {
  ...
  if((RRC_CSP_WCDMA_1700_2100_BAND & rrc_csp_int_data.wcdma_band_mask)
     || (rrccsp_is_band_class_enabled_in_ui_rf(wrrc_as_id,RRC_CSP_WCDMA_1700_2100_BAND, FALSE))
    )
  {
  ...
  if((RRC_CSP_WCDMA_1500_BAND & rrc_csp_int_data.wcdma_band_mask)
     || (rrccsp_is_band_class_enabled_in_ui_rf(wrrc_as_id,RRC_CSP_WCDMA_1500_BAND, FALSE))
    )
  {
  ...
  if((RRC_CSP_WCDMA_BC19_BAND & rrc_csp_int_data.wcdma_band_mask)
     || (rrccsp_is_band_class_enabled_in_ui_rf(wrrc_as_id,RRC_CSP_WCDMA_BC19_BAND, FALSE))
    )
  {
...
#ifdef FEATURE_DUAL_SIM
  is_hplmn_or_ehplmn = mm_per_subs_is_bcch_plmn_an_hplmn(plmn_id,rrc_get_as_id(wrrc_as_id));
#else
...
#ifdef FEATURE_DUAL_SIM
  is_hplmn_or_ehplmn = reg_sim_per_subs_is_ehplmn(nas_plmn_id,rrc_get_as_id(wrrc_as_id));
#else
...
)
{
  ...
  home_plmn_id = mmumts_convert_nas_plmn_id_to_rrc_plmn_id(
                            reg_sim_per_subs_hplmn_get(rrc_get_as_id(wrrc_as_id)));
#else
  ...
  {
    if(( rrcdata_sim_related_info.rrc_hi_access_class & tmp_shift_bit_mask ) && 
       ( ac_barred_list.barred[i] == FALSE ))
    {
      ...
      if(((i == RRC_INT_ACCESS_CLASS_11_INDEX) ||
          ...
          (rrc_is_bcch_plmn_an_hplmn_or_ehplmn(wrrc_as_id,plmn_id)))
      {
...
)
{
  ...
  rrc_csp_substate_e_type curr_csp_substate = rrc_get_csp_substate(wrrc_as_id);  
  ...
  *cell_bar = RRC_CSP_CELL_BAR_INTRA_FREQ;
  ...
  WRRC_MSG2_MED("Sib_idx PLMN: MCC %d-MNC %d",RRC_CSP_GET_INT_MCC(sibidx_plmn_id),RRC_CSP_GET_INT_MNC(sibidx_plmn_id));
  ...
  {
    mib_ptr = (rrc_MasterInformationBlock*)rrc_sibdb_return_sib_for_cell(wrrc_as_id,rrc_MIB,sibidx_plmn_id,cell_id);
  
    sib3_ptr = (rrc_SysInfoType3 *) rrc_sibdb_return_sib_for_cell(wrrc_as_id,
                              rrc_SIB3, 
                              sibidx_plmn_id, cell_id);
    if( sib3_ptr == NULL )
    {
      /* SIB3 for this PLMN, CEll ID combination not present */
      WRRC_MSG3_ERROR("ERR: SIB3 for MCC %d, MNC %d, Cell ID %d not present",
    ...
    if((rrc_state != RRC_STATE_DISCONNECTED) && (rrc_state != RRC_STATE_CONNECTING))
    {
      if(sib3_ptr->sib4indicator == TRUE)
      {
        ...
        sib4_ptr = (rrc_SysInfoType4 *) rrc_sibdb_return_sib_for_cell(wrrc_as_id,
                                  rrc_SIB4, 
                                  sibidx_plmn_id, cell_id);
        if( sib4_ptr == NULL )
        {
  ...
  {
    WRRC_MSG0_MED("WTOW: reading SIB3 for cell bar/access restrictions");
    mib_ptr = (rrc_MasterInformationBlock*)rrc_bplmn_return_sib_for_cell(wrrc_as_id,rrc_MIB);
    sib3_ptr = (rrc_SysInfoType3 *) rrc_bplmn_return_sib_for_cell(wrrc_as_id, rrc_SIB3);
                              
    if( sib3_ptr == NULL )
    {
      /* SIB3 for this PLMN, CEll ID combination not present */
      WRRC_ERR0_FATAL("SIB3 empty when SIB_CNF returned with SUCCESS");
    } /* SIB3 not present */
  ...
  if(mib_ptr == NULL)
  {
    WRRC_ERR0_FATAL("MIB empty when SIB_CNF returned with SUCCESS");
  }

  if(rrccsp_bar_forbid_cells(wrrc_as_id,sib3_ptr,common_plmn_id,mib_ptr))
  {
    WRRC_MSG0_HIGH("Barring the F-cell");
    *cell_bar = RRC_CSP_CELL_BAR_INTRA_FREQ;
     rrc_csp_int_data.wcdma_cell_time_to_reselect = 300;
     return SUCCESS;
  }
                                                        ...
                                                        rrc_ReservedIndicator_notReserved ))
  {
    ...
    WRRC_MSG0_MED("Cell Not Barred, Not reserved");
                                                      ...
                                                      rrc_ReservedIndicator_reserved ))
  {
    ...
    if(rrc_csp_ignore_barred_rsv_status_due_to_ueut(wrrc_as_id,plmn_id))
    {
   ...
   if(( rrcdata_sim_related_info.rrc_hi_access_class != RRC_HI_ACCESS_CLASS_ABSENT ) &&
       (((rrcdata_sim_related_info.rrc_hi_access_class & RRC_ACCESS_CLASS_11_BIT) ||
         (rrcdata_sim_related_info.rrc_hi_access_class & RRC_ACCESS_CLASS_15_BIT)) &&
        (rrc_is_bcch_plmn_an_hplmn_or_ehplmn(wrrc_as_id,plmn_id))))
    {
...
SIDE EFFECTS
                  None
======================================================================================*/
boolean  rrc_check_if_dsac_is_present(sys_modem_as_id_e_type wrrc_as_id,  rrc_SysInfoType3   *sib3_ptr)
{
...
SIDE EFFECTS
                  None
======================================================================================*/
boolean  rrc_check_if_ppac_is_present(sys_modem_as_id_e_type wrrc_as_id,  rrc_SysInfoType3   *sib3_ptr)
{
  if(sib3_ptr->m.v4b0NonCriticalExtensionsPresent)
  {
    if(sib3_ptr->v4b0NonCriticalExtensions.m.v590NonCriticalExtensionPresent)
...
)
{
    ...
    T_rrc_DomainSpecificAccessRestriction_v670ext_restriction))
  {
       ...
       (dsac_local.cSDomainSpecificAccessRestriction.u.restriction->domainSpecficAccessClassBarredList.n != 0))
    {
      ...
      rrc_translate_access_class_barred_list(wrrc_as_id,&dsac_local.cSDomainSpecificAccessRestriction.u.restriction->domainSpecficAccessClassBarredList, 
    ...
    rrc_fill_access_class_info(wrrc_as_id,rrc_dsac_access_class_barred_list,
       ...
       (dsac_local.pSDomainSpecificAccessRestriction.u.restriction->domainSpecficAccessClassBarredList.n != 0))
    {
      ...
      rrc_translate_access_class_barred_list(wrrc_as_id,&dsac_local.pSDomainSpecificAccessRestriction.u.restriction->domainSpecficAccessClassBarredList,
                                             &rrc_dsac_access_class_barred_list);
    }
    else
    {
  ...
  if((rrcdata_sim_related_info.rrc_lo_access_class == RRC_LO_ACCESS_CLASS_ABSENT)&&(rrcnv_data.rrc_enable_hi_ac == FALSE))
  {
...
)
{
  ...
  if(ppac_local.locationRegistrationRestrictionIndicator == rrc_PagingPermissionWithAccessControlParameters_locationRegistrationRestrictionIndicator_all)
  {
    ...
    else /*Location/Registration Parameters are restricted for CS and PS domain. Look for AC barred list */
    {
      ...
      rrc_translate_ppac_access_class_barred_list(wrrc_as_id,ppac_local.locationRegistration.u.restriction, 
                                                  &rrc_ppac_access_class_barred_list);
      rrc_fill_access_class_info(wrrc_as_id,rrc_ppac_access_class_barred_list,
                                 &access_control,
                                 plmn_id,
                                 cell_id);
                            ...
                            )
{
  ...
  *access_control = RRC_CSP_NO_CELL_ACCESS;
  ...
  WRRC_MSG2_MED("SIB Index PLMN is MCC%d - MNC%d",RRC_CSP_GET_INT_MCC(sibidx_plmn_id), RRC_CSP_GET_INT_MNC(sibidx_plmn_id));
  ...
  else
  {
    ...
    if( sib3_ptr == NULL )
    {
      ...
      WRRC_ERR0_FATAL("SIB3 not read but sib_cnf returned w SUCCESS");
    }
    sib3_4_access_restriction_ptr = &(sib3_ptr->cellAccessRestriction);
  }
  ...
  WRRC_MSG2_HIGH("HI AC NV value %d LO AC %d",rrcnv_data.rrc_enable_hi_ac,rrcdata_sim_related_info.rrc_lo_access_class);
      ...
      accessClassBarredList))) 
  {
     ...
     */
      WRRC_MSG1_HIGH("DSAC: Access Class Barred List Not present. lo_access_class %d",rrcdata_sim_related_info.rrc_lo_access_class);
    MSG_MED("DSAC: Access Class Barred List Not present ",0,0,0);
    if(rrcdata_sim_related_info.rrc_lo_access_class != RRC_LO_ACCESS_CLASS_ABSENT)
    {
      MSG_MED("Cell %d Access: All calls", cell_id, 0, 0);
      MSG_LOW("AC bar list absent, valid UE Low AC %d", 
                          rrcdata_sim_related_info.rrc_lo_access_class, 0, 0); 
      WRRC_MSG1_HIGH("LO AC class present Access All calls Cell id %d",cell_id);
    ...
    else /* UE does not have a valid Low Access Class Only Emergency calls are allowed */
    {
      ...
      */
      ...
      WRRC_MSG1_HIGH("LO AC class absent Access Emergency calls Cell id %d",cell_id);
                                                        
      *access_control = RRC_CSP_NO_REGULAR_CELL_ACCESS;
       if((rrcnv_data.rrc_enable_hi_ac == TRUE)&&(rrcdata_sim_related_info.rrc_hi_access_class != RRC_HI_ACCESS_CLASS_ABSENT))
       {
         *access_control = RRC_CSP_CELL_ACCESS_ALLOWED;
         WRRC_MSG0_HIGH("HI AC present Allow all calls ");
       }
    }
  ...
  else  /* Access Class barred list is present */
  {
    ...
    rrc_translate_access_class_barred_list(wrrc_as_id,&sib3_4_access_restriction_ptr->accessClassBarredList,
                                           &rrc_int_access_class_barred_list);
    rrc_fill_access_class_info(wrrc_as_id,rrc_int_access_class_barred_list,
                               access_control,
                               plmn_id,
                               cell_id);
  }
  ...
  if((rrc_csp_int_data.curr_scan != RRC_CSP_WTOW_BPLMN_AUTOMATIC_SCAN)
     ...
     && (rrc_csp_int_data.curr_scan != RRC_CSP_PLMN_LIST_SCAN))
  {
    mib_ptr = (rrc_MasterInformationBlock *) rrc_sibdb_return_sib_for_cell(wrrc_as_id,
                                                                          rrc_MIB,
                                                                          sibidx_plmn_id,
                                                                          cell_id
                                                                         );
  ...
  {
    mib_ptr = (rrc_MasterInformationBlock *) rrc_bplmn_return_sib_for_cell(wrrc_as_id,
                                                                           rrc_MIB);
  }
  ...
  {
    /* SIB3 is NULL This should NOT happen */
    WRRC_ERR0_FATAL("MIB not read but sib_cnf returned w SUCCESS");
  }

  if(rrc_is_nw_shared(wrrc_as_id,mib_ptr))
  {
    rrc_translate_gsm_map_plmn_id(wrrc_as_id,&(mib_ptr->plmn_Type.u.gsm_MAP->plmn_Identity),&mib_PLMN_ID);
    if(!rrccsp_is_mib_plmn_valid(wrrc_as_id,mib_ptr) ||
       !rrc_bcch_plmns_match(wrrc_as_id,mib_PLMN_ID,plmn_id))
    {
      look_at_shared_list = TRUE;
    }
  }

  if(!look_at_shared_list && rrc_check_if_dsac_is_present(wrrc_as_id,sib3_ptr) == TRUE)
  {
    ...
    rrc_csp_find_dsac_restrictions(wrrc_as_id,dsac_local,
                                   rrc_dsac_info,
                                   &rrc_int_access_class_barred_list,
                                   plmn_id,
                                   cell_id
  ...
  if(!look_at_shared_list && rrcnv_data.rrc_ppac_support_nv && rrc_check_if_ppac_is_present(wrrc_as_id,sib3_ptr) == TRUE)
  {
    /* First store the access class pointer into a local data structure */
    ...
    rrc_csp_find_ppac_restrictions(wrrc_as_id,ppac_local,
                                   rrc_ppac_info,
                                   &rrc_int_access_class_barred_list,
                                   plmn_id,
                                   cell_id
  ...
  if(look_at_shared_list)
  {
    ...
    rrc_DomainSpecificAccessRestrictionParam_v670ext dsac_local;
    num_of_shared_plmns = rrccsp_construct_multiple_plmn_list_from_mib(wrrc_as_id,mib_ptr,shared_plmn_list);

    rrc_csp_fill_dsac_info_for_shared_plmns(wrrc_as_id,num_of_shared_plmns,
                                            sib3_ptr,
    ...
    {
      if(rrc_bcch_plmns_match(wrrc_as_id,plmn_id, dsac_info_for_shared_plmns[i].plmn_id))
      {
  ...
  if(look_at_shared_list && rrcnv_data.rrc_ppac_support_nv)
  {
    uint16 k = 0;
    rrc_ppac_info_for_shared_plmns_type ppac_info_for_shared_plmns[MAX_SHARED_PLMNS-1];
    rrc_PagingPermissionWithAccessControlParameters ppac_local;

    rrc_csp_fill_ppac_info_for_shared_plmns(wrrc_as_id,num_of_shared_plmns,
                                            sib3_ptr,
    ...
    for(i = 0;(i < MAX_SHARED_PLMNS-1) && (i < num_of_shared_plmns); i++)
    {
      if(rrc_bcch_plmns_match(wrrc_as_id,plmn_id, ppac_info_for_shared_plmns[i].plmn_id))
      {
        /* We are sure that this is not a mib common PLMN */
        break;
      }
  ...
  {
    WRRC_MSG2_MED("DSAC: CS cell access: %d, PS cell access: %d",rrc_dsac_info->cs_access_control
                                                           ,rrc_dsac_info->ps_access_control
                                                           );
  }
  ...
  {
    WRRC_MSG3_HIGH("PPAC: CS reg allowed: %d, PS reg allowed: %d, Page rsp allowed: %d",rrc_ppac_info->cs_reg_allowed
...
SIDE EFFECTS
                  None
===========================================================================*/
void rrc_fill_access_class_info(sys_modem_as_id_e_type wrrc_as_id,rrc_int_access_class_barred_list_type rrc_int_access_class_barred_list, 
                                rrc_csp_access_control_e_type *access_control,
                                rrc_plmn_identity_type  plmn_id,
                                rrc_int_cell_id_type    cell_id)
{  
    ...
    {
      if ( (rrcdata_sim_related_info.rrc_hi_access_class != RRC_HI_ACCESS_CLASS_ABSENT) && 
           (rrc_csp_verify_hi_ac_access(wrrc_as_id,plmn_id, 
                                          rrc_int_access_class_barred_list)
           == TRUE) )
      {
      ...
      else
      {
        ...
        if ((rrcdata_sim_related_info.rrc_lo_access_class != RRC_LO_ACCESS_CLASS_ABSENT) &&
            (rrc_int_access_class_barred_list.barred[rrcdata_sim_related_info.rrc_lo_access_class]
                  == FALSE))
        {
    ...
    else if ((rrcdata_sim_related_info.rrc_lo_access_class != RRC_LO_ACCESS_CLASS_ABSENT) &&
        (rrc_int_access_class_barred_list.barred[rrcdata_sim_related_info.rrc_lo_access_class]
              == FALSE))
    {
    ...
    else if ((rrcdata_sim_related_info.rrc_hi_access_class != RRC_HI_ACCESS_CLASS_ABSENT) &&
             (rrc_csp_verify_hi_ac_access(wrrc_as_id,plmn_id, 
                                          rrc_int_access_class_barred_list)
             == TRUE ))
    {
  ...
  WRRC_MSG4_HIGH("Cell Access Class Info : Cell Id - %d UE HI AC - 0x%x  UE LO AC - %d  Access Control - %d",cell_id,rrcdata_sim_related_info.rrc_hi_access_class,rrcdata_sim_related_info.rrc_lo_access_class, *access_control);
  return;
...
)   
{
  ...
  else
  {
    ...
    if(rrc_dsac_info->valid != TRUE)
    {
      ...
      rrc_translate_access_control_to_access_class(wrrc_as_id,access_control, 
                      &(service_info_ptr->cell_access));
    }
    else
    {
      /* Fill the cs access control information */
      rrc_translate_access_control_to_access_class(wrrc_as_id,rrc_dsac_info->cs_access_control,
                       &(service_info_ptr->cs_cell_access));

      /* Fill the ps access control information */
      rrc_translate_access_control_to_access_class(wrrc_as_id,rrc_dsac_info->ps_access_control, 
...
rrc_csp_find_forbid_lai(sys_modem_as_id_e_type wrrc_as_id,rrc_lai_type  *lai, 
                        rrc_csp_lai_status_e_type *lai_status)
{
  uint32 i;

#ifdef FEATURE_FEMTO_CSG
  rrc_csp_substate_e_type csp_substate = rrc_get_csp_substate(wrrc_as_id);
#endif
  ...
  *lai_status = RRC_CSP_LAI_OK;
  ...
  WRRC_MSG3_MED("CSG_CR: curr scan %d, sub state %d, csg id %d",
  ...
  {
    if( rrc_csp_bcch_and_nas_lais_match(wrrc_as_id,*lai, rrc_csp_int_data.forbid_lai_list.lai[i]) )
    {
...
boolean rrccsp_check_for_lai_forbidden(sys_modem_as_id_e_type wrrc_as_id,rrc_SysInfoType1 *sib1_ptr,
                             rrc_MasterInformationBlock *mib_ptr)
{
  rrc_csp_lai_status_e_type  lai_status;
  rrc_lai_type  lai;
  ...
  rrc_plmn_identity_type mib_plmn_id;
  rrc_translate_gsm_map_plmn_id(wrrc_as_id,&(mib_ptr->plmn_Type.u.gsm_MAP->plmn_Identity),&mib_plmn);
  if(rrc_csp_int_data.network_select_mode == SYS_NETWORK_SELECTION_MODE_AUTOMATIC)
  {
    ...
    lai.plmn = mib_plmn;
    if(rrc_csp_find_forbid_lai(wrrc_as_id,&lai, &lai_status) == FAILURE)
    {
    ...
    if(lai_status == RRC_CSP_FORBID_LAI)
    {
      ...
      rrccsp_send_cphy_cell_bar_req(wrrc_as_id,RRC_CSP_WCDMA_FREQ_BARRED, 
...
SIDE EFFECTS      None
===========================================================================*/
static void rrccsp_remove_acqdb_bands_from_prioritized_list(sys_modem_as_id_e_type wrrc_as_id)
{
  int8 idx,idy = 0;
  ...
  for(idx = (rrc_csp_curr_select_data.num_of_prioritized_bands-1); idx >= 0 ; idx--)
  {
    if(rrc_csp_int_data.deep_sleep_acq_db_band_mask & rrc_csp_curr_select_data.prioritized_bands[idx])
    {
      ...
      WRRC_MSG3_HIGH("WBP: Removing band %d, num_pri_bands: %d, idx: %d", rrc_csp_curr_select_data.prioritized_bands[idx], 
...
FUNCTION          RRC_DSDS_INIT_INT_DATA
...
SIDE EFFECTS
                  None
===========================================================================*/
void  rrc_dsds_init_int_data(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_dsds_int_data.preemption_handling_in_progress = FALSE;
  rrc_dsds_int_data.csp_waiting_for_wl1_to_release_lock = FALSE;
  rrc_stop_lock_wait_timer(wrrc_as_id);
  rrc_dsds_int_data.shld_acq_be_spltd = FALSE;
  rrc_dsds_int_data.tr_phy_chl_rel_sent = FALSE;
#ifdef FEATURE_WRLF_SYSTEM_SEL
  if(!RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS())
#endif
  {
    rrc_dsds_int_data.nas_proc_type = SYS_PROC_TYPE_NONE;   
  }
}
#endif
...
SIDE EFFECTS
                  None
===========================================================================*/
void  rrc_csp_init_int_data(sys_modem_as_id_e_type wrrc_as_id)
{ 
  ...
  rrccsputil_int_data.rrc_csp_internal_service_req = FALSE;
  rrccsputil_int_data.search_order_specialization = SYS_SPECIAL_NONE;
  ...
  rrc_csp_init_wtog_plmn_srch_int_data(wrrc_as_id);
...
#ifdef FEATURE_DUAL_SIM
  rrc_dsds_init_int_data(wrrc_as_id);
#endif
  ...
  rrccsp_free_sib_cont_list(wrrc_as_id);
  rrc_csp_int_data.sib_read_csfb_active = FALSE;
#endif

#ifdef FEATURE_LTE_TO_WCDMA
  rrctmr_stop_timer(wrrc_as_id, RRCTMR_LTE_TO_WCDMA_REDIR_SRCH_TIMER);
#endif
  ...
  rrc_csp_int_data.csfb_extended_status = CSFB_INVALID;
  rrc_csp_int_data.cs_rab_established = FALSE;
#endif
  ...
  rrc_csp_int_data.attempt_freq_only_acq = FALSE;
#ifdef FEATURE_DUAL_SIM
  rrc_csp_int_data.rrc_use_high_priority_for_bplmn = FALSE;
#endif
  rrc_csp_int_data.mplmn_wcdma_freq_list_present = FALSE;
  rrc_csp_int_data.mplmn_freq_list_index = 0;
  rrc_csp_int_data.freq_scan_list.priority_info = SYS_SRCH_PRIORITY_NORMAL;
...
SIDE EFFECTS
                  None
===========================================================================*/
void  rrc_csp_init_wtog_plmn_srch_int_data(sys_modem_as_id_e_type wrrc_as_id)
{ 
...
===========================================================================*/
boolean rrccsp_is_supported_band(sys_modem_as_id_e_type wrrc_as_id,rrc_csp_band_class_type band_class)
{
...
===========================================================================*/
boolean rrccsp_is_current_band(sys_modem_as_id_e_type wrrc_as_id,rrc_csp_band_class_type band_class,rrc_plmn_identity_type plmn_id)
{
  ...
  if((band_class & RRC_CSP_WCDMA_800_BAND) || (band_class & RRC_CSP_WCDMA_850_BAND))
  {
      ...
      )
    {
      if(rrc_csp_int_data.dest_freq_present)
      {
        ...
        return(rrccsp_is_supported_band(wrrc_as_id,band_class));
      }
    }
     ...
     */
    /* To pass GCF test case 34.123-1 TC8.1.2.22a when camped on B5 cell made changes
     * to not let UE camp on B19 cell when SIB5 band indicator shows B5
     */
    if(   (rrc_csp_int_data.curr_scan == RRC_CSP_LEAVING_CONN_MODE_SCAN)
       || ((rrc_csp_int_data.curr_scan == RRC_CSP_INTER_FREQ_REDIRECTION_SCAN) && rrc_csp_int_data.first_acq_on_redirected_freq))
    {
    ...
    if((band_class == rrc_get_frequency_band(wrrc_as_id,rrc_csp_int_data.curr_acq_entry.freq,plmn_id))
         && (rrccsp_is_supported_band(wrrc_as_id,band_class)))
    {
      WRRC_MSG2_MED("acq freq %d belongs to the band_class %d", rrc_csp_int_data.curr_acq_entry.freq, 
                                                           band_class);
      return TRUE;
    }
    else
  ...
  {
    if(band_class == rrc_get_frequency_band(wrrc_as_id,rrc_csp_int_data.curr_acq_entry.freq, 
...
rrccsp_get_next_band_to_be_scanned(sys_modem_as_id_e_type wrrc_as_id)
{
  uint16 tmp_shift_bit_mask = RRC_CSP_BAND_MAX >> 1;

  WRRC_MSG4_HIGH("Bands, First = %d, Current = %d, next = %d, Addl: addl_chnl_done_mask = %d",
  ...
  {
    WRRC_MSG1_HIGH("Band to be scanned: %d",rrc_csp_curr_select_data.first_band_to_be_scanned);
  ...
  {
    WRRC_MSG3_HIGH("WBP:prioritized_bands_read_idx: %d, num_of_prioritized_bands: %d ,next_band_mask : %d",
    ...
    )
    {
      ...
      {
        WRRC_MSG1_MED("Addl chl scan NOT needed for band %d", tmp_shift_bit_mask);
      ...
      else
      {
        ...
        WRRC_MSG2_MED("Addl chl scan needed for band %d. Added to addl_chnl_done_mask %d", tmp_shift_bit_mask,rrc_csp_curr_select_data.addl_chnl_done_mask);
      } 
    }
    else
    {
    ...
    {
      WRRC_MSG1_HIGH("Band to be scanned: %d, Additional channel scan is needed", tmp_shift_bit_mask);
  ...
  WRRC_MSG1_MED("WBP: next_band_mask : %d",rrc_csp_curr_select_data.next_band_mask);
...
)
{
...
#ifdef FEATURE_PCS_850_FREQ_SCAN_OPTIMIZATION
  boolean do_gsm_bands_scan_first = rrccsp_do_gsm_bands_scan_first(wrrc_as_id);
#endif
#endif
...
#endif
    else if(rrcnv_data.rrc_nv_channel_lock_item_value !=0)
    {
        ...
        case RRC_CSP_SPEC_PLMN:
          ...
          if(rrc_csp_curr_select_data.acq_db_in_use == TRUE)
          {
            if(rrc_csp_curr_select_data.acq_db_scan_info.start_with_first)
            {
              ...
              acq_entry_ptr = rrc_csp_get_first_match_acq_entry(wrrc_as_id,req_plmn_id,
              ...
              else
              {
                ...
                WRRC_EVENT_REPORT_PAYLOAD(EVENT_WCDMA_RRCCSP_SCAN_START, sizeof(rrclog_wcdma_rrccsp_scan_start_event_type),
            ...
            {
              acq_entry_ptr = rrc_csp_get_next_match_acq_entry(wrrc_as_id,req_plmn_id,
                                            rrc_csp_curr_select_data.acq_type);
            }
            ...
            if(acq_entry_ptr == NULL)
            {
              if(--rrc_csp_curr_select_data.num_acq_db_and_dest_freq_cycles > 0) 
              {
                ...
                if((   (rrc_csp_int_data.curr_scan == RRC_CSP_CELL_DCH_TO_CELL_FACH_SCAN) 
                    ...
                    || ((rrc_csp_int_data.curr_scan == RRC_CSP_OUT_OF_SERVICE_AREA_SCAN) 
                        && (RRC_STATE_CELL_DCH == rrc_get_state(wrrc_as_id)))
                   ) 
              ...
              else
              {
               ...
               {
#ifdef FEATURE_DUAL_SIM
                 if(!rrc_dsds_int_data.oos_due_to_no_resource)
#endif
                 {
                   WRRC_MSG0_HIGH("SVC_ST: Sending UI NO SERVICE in OOSC");
                   rrc_csp_send_svc_status_to_mm(wrrc_as_id,SYS_SRV_STATUS_NO_SRV, SYS_RAT_UMTS_RADIO_ACCESS, 0);
...
#ifdef FEATURE_FEMTO_CSG
                   ||(rrccsp_asf_actively_in_prog(wrrc_as_id))
#endif
                   )
                {
                ...
                /* After ACQ DB search, check if search should be done on GSM */
                          ...
                          && (rrcmcm_is_dualmode_enabled_no_f3(wrrc_as_id))
                ...
                else
                {
                    ...
                    )
                  {
                    ...
                    if(rrc_csp_curr_select_data.num_of_prioritized_bands != 0)
                    {
                      rrccsp_remove_acqdb_bands_from_prioritized_list(wrrc_as_id);
                      rrc_csp_curr_select_data.prioritized_bands_read_idx = 0;
                    }
                    rrc_csp_curr_select_data.current_band = rrccsp_get_next_band_to_be_scanned(wrrc_as_id);

                    WRRC_MSG3_HIGH("acq_db_mask = %d, next_mask = %d, flag_mask = %d",
                  ...
                  rrc_csp_init_freq_scan_data(wrrc_as_id);
                  rrc_csp_init_addl_chnl_scan_data(wrrc_as_id);
                }
          ...
          else /* Acq DB is not in use */
          {
             ...
             *  Selection) 
             */
            if(rrc_csp_get_next_freq_scan_entry(wrrc_as_id,&ret_freq) == FAILURE)
            {
               ...
               */
#ifdef FEATURE_WCDMA_MULTIBAND
              WRRC_MSG1_MED("Spec PLMN: No cell in Freq Band %d", 
                         rrc_csp_curr_select_data.current_band);
              ...
              if((rrc_csp_curr_select_data.current_band = 
                    rrccsp_get_next_band_to_be_scanned(wrrc_as_id)) != RRC_CSP_BAND_MAX)
              {
                all_search_exhausted = FALSE;
                WRRC_MSG1_MED("Start: Any PLMN freq scan on band %d", 
                         rrc_csp_curr_select_data.current_band);
                rrc_csp_init_freq_scan_data(wrrc_as_id);
              }
              else
...
#else
              WRRC_MSG0_MED("Spec PLMN: No cell in Freq Band");
              acq_entry_ptr = NULL;
              all_search_exhausted = TRUE;
#endif /* FEATURE_WCDMA_MULTIBAND */
            }
            else
            {
              ...
              while((rrc_csp_curr_select_data.scan_info.type_freq_scan == RRC_CSP_ACQ_SCAN) &&
                    (rrc_csp_should_freq_be_scanned(wrrc_as_id,rrc_csp_int_data.curr_req_plmn, ret_freq) == FALSE))
              {
              ...
              if(all_search_exhausted == FALSE)
              {
                ...
                WRRC_MSG2_MED("Freq %d scan type %d", ret_freq,
                         rrc_csp_curr_select_data.scan_info.type_freq_scan);
              }
#ifdef FEATURE_WCDMA_MULTIBAND
              else
        ...
        case RRC_CSP_ANY_PLMN:
...
#endif
                  WRRC_MSG0_MED("Start: Any PLMN freq scan");
                  rrc_csp_init_freq_scan_data(wrrc_as_id);
                  rrc_csp_init_addl_chnl_scan_data(wrrc_as_id);
                }
          ...
          else /* Acq DB is not in use */
          {
            ...
            if(rrc_csp_get_next_freq_scan_entry(wrrc_as_id,&ret_freq) == FAILURE)
            {
              ...
              rrc_csp_curr_select_data.scanned_band_mask |= rrc_csp_curr_select_data.current_band;
              WRRC_MSG1_HIGH("Scanned_band_mask = %d",rrc_csp_curr_select_data.scanned_band_mask);
              /* Scan the next band if supported for periodic scan search*/
              if(rrc_csp_int_data.mplmn_wcdma_freq_list_present)
              {
                all_search_exhausted = TRUE;
                WRRC_MSG1_HIGH("freq scan on band %d",rrc_csp_curr_select_data.current_band);
                acq_entry_ptr = NULL;
              }
              /* Scan the next band if supported */
              else if((rrc_csp_curr_select_data.current_band = 
                    rrccsp_get_next_band_to_be_scanned(wrrc_as_id)) != RRC_CSP_BAND_MAX)
              {
                all_search_exhausted = FALSE;
                WRRC_MSG1_MED("Start: Any PLMN freq scan on band %d", 
            ...
            else
            {
                    ...
                    (rrc_csp_should_freq_be_scanned(wrrc_as_id,rrc_csp_int_data.curr_req_plmn, ret_freq) == FALSE))
              {
                if(rrc_csp_int_data.mplmn_wcdma_freq_list_present)
                {
                  WRRC_MSG1_HIGH("MPLMN OPT:freq  %d is not corresponding to specified band. Do not scan",ret_freq);
                }
                else
                {
  ...
  if(all_search_exhausted == FALSE)
  {
    ...
    {
      WRRC_MSG2_MED("LFS: Returning type_freq_scan as RRC_CSP_ACQ_SCAN. acq_db_in_use %d, dest_freq_in_use %d",
  ...
  else
  {
      ...
      )
    {
      ...
      rrctmr_start_timer(wrrc_as_id, RRCTMR_FREQ_SCAN_TIMER, 
                          (unsigned long) rrcnv_data.wcdma_rrc_freq_scan_timer_nv);
...
SIDE EFFECTS
                  None
===========================================================================*/
static void rrc_csp_init_lfs_scan(sys_modem_as_id_e_type wrrc_as_id)
{
    ...
    )
  {
    ...
    rrccsp_update_match_acq_entries_into_list_db(wrrc_as_id);
...
===========================================================================*/
uecomdef_status_e_type
rrc_csp_init_select_data(sys_modem_as_id_e_type wrrc_as_id,rrc_requested_plmn_type  *req_plmn_ptr)
{

  WRRC_MSG1_MED("Initializing select data for new search. %d",rrc_csp_int_data.bplmn_new_srch);
  if(rrc_csp_int_data.bplmn_new_srch)
  {
...
#ifdef FEATURE_DUAL_SIM
      rrcwrm_reset_l2_ack_req_mask(wrrc_as_id);
#endif
    ...
    if(   (rrc_csp_int_data.curr_scan == RRC_CSP_CELL_DCH_TO_CELL_FACH_SCAN) 
       ...
       || (   (rrc_csp_int_data.curr_scan == RRC_CSP_OUT_OF_SERVICE_AREA_SCAN) 
           ...
           && (!rrccsp_is_state_transition_triggered_by_reconfig_msg(wrrc_as_id))
          )
      )
    {
    ...
    rrc_csp_curr_select_data.acq_db_scan_info.start_with_first    = TRUE;
      ...
      WRRC_MSG0_MED("Resetting the next_band_mask");
      rrc_csp_curr_select_data.next_band_mask = rrc_csp_int_data.wcdma_band_mask;

      WRRC_MSG0_MED("Resetting the variables for optimized band scan");
     ...
     WRRC_MSG1_MED("WCDMA Bandclass bitmask : %d",rrc_csp_int_data.wcdma_band_mask);

      rrccsp_prioritize_wcdma_bands_for_full_scan(wrrc_as_id);
      ...
      if(rrc_csp_curr_select_data.prioritized_bands_read_idx < rrc_csp_curr_select_data.num_of_prioritized_bands)
      {
        ...
        WRRC_MSG4_HIGH("WBP:prioritized_bands_read_idx: %d, num_of_prioritized_bands: %d, First Band to be scanned: %d, next_band_mask : %d",
                                             rrc_csp_curr_select_data.prioritized_bands_read_idx,
                                             rrc_csp_curr_select_data.num_of_prioritized_bands,
      ...
      WRRC_MSG0_MED("Resetting the addl_chnl_done_mask");
    ...
    WRRC_MSG1_MED("Acquisition Type: %d",rrc_csp_curr_select_data.acq_type);

    if(rrc_csp_int_data.curr_scan == RRC_CSP_GSM_TO_WCDMA_REDIRECTION_SCAN)
    {
      /*copy the target list to list_db*/
      (void)rrc_csp_lfs_update_list_db_with_target_fdd_list(wrrc_as_id);
    }
    else
    {
      rrc_csp_init_lfs_scan(wrrc_as_id);
    }
    /*Acq db set to FALSE for periodic frequency scan search*/
    if(rrc_csp_int_data.mplmn_wcdma_freq_list_present)
    {
      rrc_csp_curr_select_data.scan_info.type_freq_scan = RRC_CSP_ACQ_SCAN;  
      rrc_csp_curr_select_data.acq_db_in_use = FALSE;  
      WRRC_MSG0_HIGH("MPLMN:: setting to acq scan and acq db to false");
    }
  }
  else
  {
...
)
{
  ...
  if(SUCCESS == validate_dl_arfcn(wrrc_as_id,wcdma_dl_freq, 
...
#endif
  {
    ...
    *ret_type_freq_scan     = RRC_CSP_ACQ_SCAN;
     ...
     */
    if((rrccsp_get_curr_type_acq(wrrc_as_id) == RRC_CSP_ANY_PLMN)
       && (is_freq_in_overlapping_850_and_800_bands(wrrc_as_id, wcdma_dl_freq)))
    {
      if(rrccsp_is_supported_band(wrrc_as_id,RRC_CSP_WCDMA_850_BAND)
          && rrccsp_is_supported_band(wrrc_as_id,RRC_CSP_WCDMA_800_BAND))
      {
      ...
      else if(rrccsp_is_supported_band(wrrc_as_id,RRC_CSP_WCDMA_800_BAND))
      {
    ...
    {
      rrc_csp_curr_select_data.current_band = rrc_get_frequency_band(wrrc_as_id,
...
)
{
   ...
   */ 
  if(rrc_csp_init_select_data(wrrc_as_id,req_plmn_ptr) == SUCCESS)
  {
    *ret_type_acq_ptr = rrc_csp_curr_select_data.acq_type;
    ...
    if(rrcnv_data.rrc_nv_channel_lock_item_value != 0)
    {

      wcdma_dl_nv_freq = rrcnv_data.rrc_nv_channel_lock_item_value;
      nv_freq_valid = rrc_csp_try_cell_acq_on_nv_given_freq
                      (   wrrc_as_id,
    ...
    if(rrc_csp_int_data.aset_scr_code < 8192)
    {
      ...
      {
        WRRC_MSG2_HIGH("Connected Mode->Idle or DCH->FACH Trans. Attmpt acq on the best cell (scr %d, pn %d) in active set", 
      ...
      else
      {
        ...
        ret_status = 
        rrc_csp_continue_stored_initial_select_ctrl(wrrc_as_id,ret_type_acq_ptr, 
                                                    ret_acq_entry_ptr,
                                                    ret_type_freq_scan);
      }
    ...
    {
      rrc_csp_curr_select_data.scan_info.type_freq_scan = RRC_CSP_LIST_SCAN;
      WRRC_MSG1_MED("LFS_COOS: set curr frequency scan type =%d",
             rrc_csp_curr_select_data.scan_info.type_freq_scan);                 
      *ret_type_freq_scan = RRC_CSP_LIST_SCAN;
    }
...
SIDE EFFECTS
===========================================================================*/
static uecomdef_status_e_type rrccsp_fill_in_plmn_lai_domain_ac_info(sys_modem_as_id_e_type wrrc_as_id,rrc_service_info_type *service_info_ptr)
{
  ...
  */
  ...
  WRRC_MSG2_MED("SIB Index PLMN is MCC%d - MNC%d",RRC_CSP_GET_INT_MCC(plmn_id), RRC_CSP_GET_INT_MNC(plmn_id));
  /* 2. Get the cell access status of the cell. I.e., fill up service_info structure with 
   *    cell access status
   */
  if(rrc_csp_find_access_control(wrrc_as_id,&cell_access_status, 
                                 &rrc_dsac_info, 
  ...
  {
    WRRC_ERR0_FATAL("Failure finding cell access status"); 
  }

  (void) rrc_csp_find_rrc_cell_access(wrrc_as_id,service_info_ptr, 
                                      &rrc_dsac_info, 
                                      cell_access_status, 
   ...
   */
  sib1_ptr = (rrc_SysInfoType1 *) rrc_sibdb_return_sib_for_cell(wrrc_as_id,
                                            rrc_SIB1,
                                            plmn_id, 
                                            acq_entry_ptr->cell_id);
  if(sib1_ptr == NULL)
  {
    ...
    WRRC_MSG3_ERROR("ERR: SIB1 for MCC %d, MNC %d, Cell ID %d not present",
  ...
  if(rrccsp_fill_nas_common_info(wrrc_as_id,sib1_ptr, &(service_info_ptr->gsm_map_common_info)) == FAILURE)
  {
    return(FAILURE);
  }
  ...
  {            
    ...
    {
      if(rrccsp_fill_nas_cn_specific_info(wrrc_as_id,&cn_domain_sys_info_list_ptr->elem[idx], 
...
SIDE EFFECTS
                  None
===========================================================================*/
static boolean rrc_csp_check_lai_in_reject_list(sys_modem_as_id_e_type wrrc_as_id,inter_task_lai_T curr_lai)
{
  int i;
...
)
{
     ...
     (!service_info_valid))
  {
     ...
     */
    if(FAILURE == rrccsp_fill_in_plmn_lai_domain_ac_info(wrrc_as_id,&service_info))
    {
  ...
  if(!cell_barred)
  {
        ...
        && (rrc_csp_int_data.service_req_cause == RRC_MO_CS_CALL))
    {
           ...
           (!cell_forbid_reg_area))
        {
          /*lint -save -e644*/
#ifdef FEATURE_DUAL_SIM
          rrc_plmn_lai_allowed = mm_rrc_per_subs_plmn_lai_allowed(service_info,rrc_get_as_id(wrrc_as_id));
#else
    ...
    #ifdef FEATURE_DUAL_SIM
        is_forbidden_lai = mm_per_subs_check_lai_in_reject_list(rrc_csp_int_data.curr_lai,rrc_get_as_id(wrrc_as_id));
    #else
        is_forbidden_lai = mm_check_lai_in_reject_list(rrc_csp_int_data.curr_lai);
    #endif
#else
        is_forbidden_lai = rrc_csp_check_lai_in_reject_list(wrrc_as_id,rrc_csp_int_data.curr_lai);
#endif
        if(is_forbidden_lai)
        {
...
{        
  ...
  boolean dual_mode_enabled = rrcmcm_is_dualmode_enabled_no_f3(wrrc_as_id);
  ...
  if(cell_plmn_status != RRC_CSP_PLMN_SAME_AS_REQ)
  {
    ...
    rrc_csp_int_data.lcf_in_idle_mode_skip_status = RRC_CSP_SKIP_FREQ_DIFF_PLMN;
    WRRC_MSG0_HIGH("lcf_in_idle_mode_skip_status updated to skip trying Acq on Last camped freq for all raw scans");
  }
...
rrc_csp_gen_status_mask(sys_modem_as_id_e_type wrrc_as_id,rrc_csp_cell_bar_e_type  cell_bar_status,
                        rrc_csp_lai_status_e_type  lai_status)
{
...
===========================================================================*/
uecomdef_status_e_type
rrc_csp_fill_acq_entry_from_sib(sys_modem_as_id_e_type wrrc_as_id, rrc_sib_event_cnf_type *sib_cnf, 
                                 rrc_csp_acq_entry_type *acq_entry_ptr,
                                 rrc_csp_cell_bar_e_type  *cell_bar_status)
{
...
#ifdef FEATURE_WCDMA_MULTIBAND
  acq_entry_ptr->band_class = rrc_get_frequency_band(wrrc_as_id,acq_entry_ptr->freq, acq_entry_ptr->plmn_id);
  WRRC_MSG2_MED("Setting band_class %d for freq %d",
  ...
  if(!rrccsp_asf_actively_in_prog(wrrc_as_id))
  {
#endif
 ...
 /* Now generate status mask for this cell */
  ...
  {
    sib1_ptr = (rrc_SysInfoType1 *) rrc_bplmn_return_sib_for_cell(wrrc_as_id, rrc_SIB1);
  ...
  WRRC_MSG2_MED("Cause 15: lac[0] = %d, lac[1]=%d",rrc_csp_int_data.curr_lai.location_area_code[0],rrc_csp_int_data.curr_lai.location_area_code[1]);
  /*Convert RRC PLMN Id to NAS PLMN Id*/
  rrc_csp_int_data.curr_lai.PLMN_id= mmumts_convert_rrc_plmn_id_to_nas_plmn_id(lai.plmn);

  if(rrc_csp_find_forbid_lai(wrrc_as_id,&lai, &lai_status) 
     == FAILURE)
  {
    WRRC_MSG0_HIGH("Failure finding LAI status");
    return(FAILURE);
  ...
  if(rrc_csp_find_cell_bar_status(wrrc_as_id,acq_entry_ptr->plmn_id, 
                                  acq_entry_ptr->cell_id, 
                                  cell_bar_status
                                  ,sib_idx_plmn_id
                                  ) 
  ...
  acq_entry_ptr->status_mask = rrc_csp_gen_status_mask(wrrc_as_id,*cell_bar_status, 
                                                       lai_status);
  ...
  if(acq_entry_ptr->is_valid_nw_sharing)
  {
    ...
    if(rrc_is_bcch_plmn_an_hplmn_or_ehplmn(wrrc_as_id,acq_entry_ptr->plmn_id))
    {
    ...
    WRRC_MSG3_HIGH("NWS: ACQ Status mask : %d for MCC:MNC - %d:%d",
    ...
    while((shared_plmn_idx < MAX_SHARED_PLMNS) &&(shared_plmn_idx < rrc_csp_int_data.curr_nw_db_entry.num_of_shared_plmns))
    {
...
#endif
      (void)rrc_csp_find_forbid_lai(wrrc_as_id,&lai, &lai_status);
      ...
      if(rrc_is_bcch_plmn_an_hplmn_or_ehplmn(wrrc_as_id,lai.plmn))
      {
        if(!hplmn_cell_bar_status_found)
        {
          /* Generate Cell barred status based on SIB info for this cell */
          if(rrc_csp_find_cell_bar_status(wrrc_as_id,lai.plmn, 
                                          acq_entry_ptr->cell_id, 
                                          cell_bar_status
                                          ,sib_idx_plmn_id
                                          ) 
      ...
      rrc_csp_int_data.curr_nw_db_entry.shared_plmns_access_info[shared_plmn_idx++].status_mask 
                                                          = rrc_csp_gen_status_mask(wrrc_as_id,*cell_bar_status, lai_status);
...
SIDE EFFECTS
                  None
===========================================================================*/
void rrc_csp_send_l1_idle_req(sys_modem_as_id_e_type wrrc_as_id)
{

  rrc_cmd_type        *csp_out_cmd_ptr;

  /* Send the command */
  if((csp_out_cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id)) != NULL)
  {
    ...
    if(rrc_dsds_int_data.csp_waiting_for_wl1_to_release_lock)
    {
    ...
    rrc_put_int_cmd(wrrc_as_id, csp_out_cmd_ptr );
  }
...
)
{
  ...
  { 
    ...
    if((curr_csp_substate == RRCCSP_SUIT_CELL_RESELECT_WT_SIB)
       || (curr_csp_substate == RRCCSP_ACCEPT_CELL_RESELECT_WT_SIB))
    {
      WRRC_MSG3_HIGH("Cell/Freq Bar Req to be sent during CSP resel state %d for freq %d, scr_code %d", 
      ...
      if(!rrccsp_is_inter_frequency_cell_resel_in_progress(wrrc_as_id) 
         ...
         && (wl1_is_serv_freq_considered_barred(acq_entry.freq, acq_entry.scr_code,wrrc_as_id))
        )
      {
        if(rrcrce_is_rce_procedure_inactive(wrrc_as_id) && (rrc_get_state(wrrc_as_id) == RRC_STATE_DISCONNECTED))
        {
          WRRC_MSG0_HIGH("Serving Freq Barred. Update CCM with cell loss");
          rrc_ccm_update_with_cell_loss(wrrc_as_id);
        }
      }
    }
    else
    {
      WRRC_MSG3_HIGH("Cell/Freq Bar Req to be sent during CSP state %d for freq %d, scr_code %d", 
      ...
      if(rrcrce_is_rce_procedure_inactive(wrrc_as_id) && (rrc_get_state(wrrc_as_id) == RRC_STATE_DISCONNECTED) && (t_barred != 0))
      {
  ...
  rrc_csp_send_l1_cmd(wrrc_as_id,RRC_PROCEDURE_CSP, CPHY_CELL_BAR_REQ, &acq_entry);
...
)
{
  ...
  if((csp_out_cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id)) != NULL)
  {
    ...
    else if(type_freq_scan == RRC_CSP_LIST_SCAN)
    {
      ...
      if (WRM_G2W_TA_MODE_ACTIVE(wrrc_as_id)) 
      {
...
===========================================================================*/
void
rrc_csp_send_l1_cmd(sys_modem_as_id_e_type wrrc_as_id,rrc_proc_e_type  proc_id,
                    ...
                    rrc_csp_acq_entry_type  *acq_entry_ptr)
{
 ...
 rrc_CellSelectReselectInfoTreselectionScaling_v5c0ext *sib3_4_sel_resel_v5c0ext_ptr = NULL;
  ...
  if(acq_entry_ptr == NULL)
  {
    WRRC_ERR0_FATAL("ERR: Passed Ptr Union has NULL pointers");
  }
  ...
  if((csp_out_cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id)) != NULL)
  {
    ...
    switch(l1_cmd_id)
    {
      case CPHY_ACQ_REQ:
                ...
                (rrc_csp_int_data.interrat_type == RRC_CSP_INTERRAT_LTE_CGI))
        {
          csp_cmd_l1_req->acq.srch = L1_BPLMN_SRCH_TYPE;
             ...
             || ((mcfg_gcf_nv_get_status())&&(rrc_csp_int_data.curr_scan == RRC_CSP_CELL_SELECTION_SCAN))
#endif
               )
        {
        ...
        {
          WRRC_MSG3_HIGH("freq %d. MCC %d, MNC %d. Do NOT apply threshold", 
                                            acq_entry_ptr->freq,
                                            RRC_CSP_GET_INT_MCC(rrc_csp_int_data.curr_req_plmn.plmn_id),
                                            RRC_CSP_GET_INT_MNC(rrc_csp_int_data.curr_req_plmn.plmn_id));
        ...
        else
        {
          ...
          WRRC_MSG1_MED("Set unbar_freqs=%d",csp_cmd_l1_req->acq.unbar_freqs);
        }
#ifdef FEATURE_DUAL_SIM
          csp_cmd_l1_req->acq.do_acq_split = rrc_dsds_int_data.shld_acq_be_spltd;
#endif
        break;
      ...
      case CPHY_CELL_PARM_REQ:
        ...
        WRRC_MSG1_MED("RRC STATE = %d", rrc_state);
        ...
        WRRC_MSG1_MED("RRC Target STATE = %d", rrc_state);
#ifdef FEATURE_FEMTO_CSG
        if(acq_entry_ptr->csg_id != SYS_CSG_ID_INVALID)
        {
          ...
          if((rrc_csp_int_data.curr_acq_entry.cell_is_hybrid) && (!rrccsp_is_csg_white(wrrc_as_id,acq_entry_ptr->csg_id,acq_entry_ptr->plmn_id)))
          {
        ...
        WRRC_MSG2_MED("CSG: Femto status in cell parm request to l1 %d manual csg mode: %d",csp_cmd_l1_req->cell_parm.is_camped_cell_femto,csp_cmd_l1_req->cell_parm.is_manual_csg_mode);
#endif
        /* PLMN and Cell ID of the current camped on cell are known. Get SIB3 */
        if(acq_entry_ptr->is_valid_nw_sharing)
        {
        ...
        sib3_ptr = (rrc_SysInfoType3 *) rrc_sibdb_return_sib_for_cell(wrrc_as_id,
                                             rrc_SIB3, 
                                               plmn_id, 
                                               acq_entry_ptr->cell_id);
        sib11_ptr = (rrc_SysInfoType11*) rrc_sibdb_return_sib_for_cell(wrrc_as_id,
                                               rrc_SIB11, 
                                               plmn_id, 
                                             acq_entry_ptr->cell_id);

        sib12_ptr = (rrc_SysInfoType12*) rrc_sibdb_return_sib_for_cell(wrrc_as_id,
                                               rrc_SIB12, 
                                               plmn_id, 
                                               acq_entry_ptr->cell_id);
        if( sib3_ptr == NULL )
        {
          ...
          if(acq_entry_ptr->is_valid_nw_sharing)
          {
            WRRC_ERR3_FATAL("SIB3 for MCC %d, MNC %d, Cell ID %d not present", 
                       RRC_CSP_GET_INT_MCC(plmn_id),
                       RRC_CSP_GET_INT_MNC(plmn_id), 
                       acq_entry_ptr->cell_id);
          }
          ...
          wrrc_free_cmd_buf( wrrc_as_id, csp_out_cmd_ptr);
                 ...
                 rrc_CellSelectReselectInfoSIB_3_4_modeSpecificInfo_fdd))
        {
          ...
          if((sib3_4_sel_resel_ext_ptr != NULL) && (
            ...
            deltaQrxlevmin)))
          {
            WRRC_MSG3_HIGH("Serving Delta-Qrxlev=%d Qrxlev=%d, Sum=%d",
            ...
            hcs_ServingCellInformation))
        {
          ...
          {
            WRRC_MSG3_HIGH("Serving Delta-Qhcs=%d, Qhcs=%d, Sum=%d",
          ...
          if(RRC_MSG_COMMON_BITMASK_IE_TYPE2(sib3_4_sel_resel_info_ptr->hcs_ServingCellInformation,
             t_CR_Max))
          {
             rrc_csp_fill_tcrmax_params(wrrc_as_id,&sib3_4_sel_resel_info_ptr->hcs_ServingCellInformation.t_CR_Max,
              &csp_cmd_l1_req->cell_parm.hcs_info.t_crmax,
        ...
        if((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5) && (sib3_4_sel_resel_v5c0ext_ptr != NULL))
        {
          ...
          {
            rrc_csp_fill_tcrmax_params(wrrc_as_id,&sib3_4_sel_resel_v5c0ext_ptr->non_HCS_t_CR_Max,
             &csp_cmd_l1_req->cell_parm.nhcs_t_crmax,
        ...
        WRRC_MSG8_HIGH("Cell parm request parameters: t_Reselection_S in ms = %d, qhcs = %d,nhcs_t_crmax = %d, nhcs_n_cr = %d, nhcs_t_crmaxhyst = %d,tresel_scale = %d, tresel_interf_scale = %d, tresel_irat_scale = %d",
                        csp_cmd_l1_req->cell_parm.trselc_ms,
                        csp_cmd_l1_req->cell_parm.hcs_info.q_hcs,
                        csp_cmd_l1_req->cell_parm.nhcs_t_crmax,
                        csp_cmd_l1_req->cell_parm.nhcs_n_cr,
     ...
     case CPHY_CELL_TRANSITION_REQ:
       ...
       csp_cmd_l1_req->cell_transition.cell_trans_params.pccpch_tx_div_ind = rrcllc_get_correct_pccpch_tx_diversity_from_sibs(wrrc_as_id,rrc_get_state(wrrc_as_id));
       break;
      ...
      case CPHY_CELL_BAR_REQ:
        ...
        else if (rrc_csp_int_data.cell_bar_info == RRC_CSP_GSM_CELL_BARRED)
        {
          ...
          if(rrccho_is_wtog_cco_active(wrrc_as_id))
          {
            rrccho_fill_gsm_cell_info_for_cell_bar(wrrc_as_id,
          ...
          WRRC_MSG3_HIGH("Cell bar req sent to L1. cell_bar_reason: %d, T-barred tmr: %d, bcch_arfcn: %d",
      ...
      default:
        WRRC_ERR1_FATAL("L1 cmd 0x%x not supported",l1_cmd_id);
        /* Call the RRC function to release the Internal Buffer */
        wrrc_free_cmd_buf( wrrc_as_id, csp_out_cmd_ptr);
        csp_out_cmd_ptr = NULL;
    } /* switch(l1_cmd) */
...
SIDE EFFECTS      None.
===========================================================================*/
void rrc_csp_send_l1_drx_req(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  if((cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id)) != NULL)
  {
    ...
    /* Populate the cmd data */
    drx_len = rrcllc_get_drx_info(wrrc_as_id); 

    csp_cmd_l1_req->drx.drx_cycle_len = rrc_csp_set_drx_length(wrrc_as_id,drx_len);
...
SIDE EFFECTS      None.
===========================================================================*/
void rrc_csp_send_l1_drx_req_for_multiple_drx(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  if((cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id)) != NULL)
  {
    ...
    /* Populate the cmd data */
    drx_len=rrcllc_get_drx_info_for_multiple_drx(wrrc_as_id);
...
===========================================================================*/
uecomdef_status_e_type  
rrc_csp_send_sib_cmd(sys_modem_as_id_e_type wrrc_as_id,rrc_cmd_e_type sib_cmd, 
                     ...
                     boolean for_serving_cell)
{
  ...
  csp_out_cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id);
  /* Populate the cmd data */
      ...
      case RRC_GET_SPECIFIC_SIBS_REQ:
        ...
        else
        {
          csp_out_cmd_ptr->cmd.get_specific_sibs_req.sib_scan_type =
            RRC_SIB_NORMAL; 
#ifdef FEATURE_FEMTO_CSG
          if((((rrccsp_get_curr_nas_requested_csg_id(wrrc_as_id) != SYS_CSG_ID_INVALID)&&
             (rrc_csp_int_data.curr_scan == RRC_CSP_CELL_SELECTION_SCAN)&&(for_serving_cell))||
             ((rrccsp_get_curr_nas_requested_csg_id(wrrc_as_id) != SYS_CSG_ID_INVALID)&&
               rrc_csp_int_data.network_select_mode == SYS_NETWORK_SELECTION_MODE_MANUAL_CSG))
            )
            {  
              csp_out_cmd_ptr->cmd.get_specific_sibs_req.for_csg_cell = TRUE;
           ...
           (RRC_CSP_INTERRAT_LTE == rrc_csp_int_data.interrat_type))
        {
          ...
          if((acq_entry_ptr->freq == rrc_csp_int_data.dest_freq) &&
             (WCDMA_RRC_MAX_UTRA_CONT != (idx = rrccsp_get_idx_sib_cont_list(wrrc_as_id,acq_entry_ptr->scr_code/16))))
          {
...
#ifdef FEATURE_3GPP_CSFB
        if((rrc_csp_int_data.rrc_csfb_call_status) &&
...
SIDE EFFECTS
                  None.
===========================================================================*/
void rrc_csp_send_service_ind_dummy(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  WCDMA_MEMCPY(&mm_buf_ptr->cmd.rrc_service_ind, 
                   sizeof(rrc_service_ind_type),
                    &rrccsputil_int_data.rrc_service_ind_bkup,
                   sizeof(rrc_service_ind_type)); 
...
SIDE EFFECTS
                  None.
===========================================================================*/
void rrc_csp_send_service_ind(sys_modem_as_id_e_type wrrc_as_id)
{
  /* Send Service Indication to MM */
  rrc_csp_send_mm_cmd(wrrc_as_id,RRC_SERVICE_IND,
...
SIDE EFFECTS
                  None
===========================================================================*/
static boolean rrccsp_check_for_stop_wcdma_req(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_cmd_type *mm_cmd_ptr;
  boolean stop_req_sts = FALSE;

  mm_cmd_ptr = (rrc_cmd_type *)q_check (&rrc_cmd_q.rrc_mm_cmd_q);
  ...
  while(mm_cmd_ptr != NULL)
  {
    ...
    mm_cmd_ptr = (rrc_cmd_type *)q_next(&rrc_cmd_q.rrc_mm_cmd_q,&mm_cmd_ptr->cmd_hdr.link);
  }
  return stop_req_sts;
}
...
#endif
/*=============================================================================
FUNCTION          rrc_ssgccs_scheduling_info_of_sibs_in_mib_sb1_sb2

DESCRIPTION       This function updates the WCDMA SIB scheduling info to SSGCCS
                  QMI client 
...
SIDE EFFECTS
                  None
==============================================================================*/
void rrc_ssgccs_scheduling_info_of_sibs_in_mib_sb1_sb2(sys_modem_as_id_e_type wrrc_as_id,wcdma_ssgccs_3g_cell_info_ind_s *wcdma_info)
{
  ...
  uint8 count =0;
  uint8 sched_present_for_sibs[RRC_TOT_SIBS] = {0};
  
  mib_ptr = (rrc_MasterInformationBlock *)rrc_sibdb_return_sib_for_cell(wrrc_as_id, rrc_MIB,rrc_csp_int_data.curr_acq_entry.plmn_id,rrc_csp_int_data.curr_acq_entry.cell_id);
  if(rrc_csp_int_data.curr_acq_entry.is_valid_nw_sharing)
  {
    mib_ptr = (rrc_MasterInformationBlock *)rrc_sibdb_return_sib_for_cell(wrrc_as_id, rrc_MIB,rrc_csp_int_data.ssgccs_nws_mib_plmn_id,rrc_csp_int_data.ssgccs_nws_cell_id);
  }
  if(mib_ptr != NULL)
  {
    uint8 i = 0,j = 0;
    for (i =0; i< mib_ptr->sibSb_ReferenceList.n;i++)
    {
      rrcsib_get_sib_type_from_sibsb_type_and_tag(wrrc_as_id,&mib_ptr->sibSb_ReferenceList.elem[i].sibSb_Type,
                                                    &sib_type_tag);
      if(sib_type_tag.sib_type != rrc_MAX_SIB)
      {
        sched_present_for_sibs[sib_type_tag.sib_type] = TRUE;
      }
    }
    if(EXTENSION_SIB_SCHED_PRESENT_IN_MIB(mib_ptr))
    {
      for(i = 0; i<mib_ptr->v690NonCriticalExtensions.v6b0NonCriticalExtensions.masterInformationBlock_v6b0ext.extSIBTypeInfoSchedulingInfo_List.n; i++)
      {
        if( RRCCHECK_SIB_TypeExt(
          (&(mib_ptr->v690NonCriticalExtensions.v6b0NonCriticalExtensions.masterInformationBlock_v6b0ext.extSIBTypeInfoSchedulingInfo_List.elem[i].extensionSIB_Type)),systemInfoType11bis))
        {
          sched_present_for_sibs[rrc_SIB11bis] = TRUE;
        }
      }
    }
#ifdef FEATURE_WCDMA_TO_LTE
    if((mib_ptr->m.v690NonCriticalExtensionsPresent) &&
           ...
           (mib_ptr->v690NonCriticalExtensions.v6b0NonCriticalExtensions.v860NonCriticalExtensions.masterInformationBlock_v860ext.m.extSIBTypeInfoSchedulingInfo_ListPresent))
    {
      for(j = 0; j<mib_ptr->v690NonCriticalExtensions.v6b0NonCriticalExtensions.v860NonCriticalExtensions.masterInformationBlock_v860ext.extSIBTypeInfoSchedulingInfo_List.n;j++)
      {
        if(mib_ptr->v690NonCriticalExtensions.v6b0NonCriticalExtensions.v860NonCriticalExtensions.masterInformationBlock_v860ext.extSIBTypeInfoSchedulingInfo_List.elem[j].extensionSIB_Type2.t & T_rrc_SIB_TypeExt2_systemInfoType19)
        {
          sched_present_for_sibs[rrc_SIB19] = TRUE;
        }
      }
    }
#endif
  }
  sb1_ptr = (rrc_SysInfoTypeSB1 *)rrc_sibdb_return_sib_for_cell(wrrc_as_id, rrc_SB1,rrc_csp_int_data.curr_acq_entry.plmn_id,rrc_csp_int_data.curr_acq_entry.cell_id);
  sb2_ptr = (rrc_SysInfoTypeSB2 *)rrc_sibdb_return_sib_for_cell(wrrc_as_id, rrc_SB2,rrc_csp_int_data.curr_acq_entry.plmn_id,rrc_csp_int_data.curr_acq_entry.cell_id);
  if(rrc_csp_int_data.curr_acq_entry.is_valid_nw_sharing)
  {
    sb1_ptr = (rrc_SysInfoTypeSB1 *)rrc_sibdb_return_sib_for_cell(wrrc_as_id, rrc_SB1,rrc_csp_int_data.ssgccs_nws_mib_plmn_id,rrc_csp_int_data.ssgccs_nws_cell_id);
    sb2_ptr = (rrc_SysInfoTypeSB2 *)rrc_sibdb_return_sib_for_cell(wrrc_as_id, rrc_SB2,rrc_csp_int_data.ssgccs_nws_mib_plmn_id,rrc_csp_int_data.ssgccs_nws_cell_id);
  }
  if(sb1_ptr != NULL)
  {
    uint8 i = 0,j = 0;
    for (i =0; i< sb1_ptr->sib_ReferenceList.n;i++)
    {
      rrcsib_get_sib_type_from_sib_type_and_tag(wrrc_as_id,&sb1_ptr->sib_ReferenceList.elem[i].sib_Type,
                                                    &sib_type_tag);
      if(sib_type_tag.sib_type != rrc_MAX_SIB)
      {
        sched_present_for_sibs[sib_type_tag.sib_type] = TRUE;
    ...
    if(EXTENSION_SIB_SCHED_PRESENT_IN_SB1_SB2(sb1_ptr,rrc_SysInfoTypeSB1,sysInfoTypeSB1))
    {
      for(i = 0; i<sb1_ptr->v6b0NonCriticalExtensions.sysInfoTypeSB1_v6b0ext.extSIBTypeInfoSchedulingInfo_List.n; i++)
      {
        if(RRCCHECK_SIB_TypeExt((&(sb1_ptr->v6b0NonCriticalExtensions.sysInfoTypeSB1_v6b0ext.extSIBTypeInfoSchedulingInfo_List.elem[i].extensionSIB_Type)),systemInfoType11bis))
        {
          sched_present_for_sibs[rrc_SIB11bis] = TRUE;
        }
      }
...
#ifdef FEATURE_WCDMA_TO_LTE
    for(j = 0; j<sb1_ptr->v6b0NonCriticalExtensions.v860NonCriticalExtensions.sysInfoTypeSB1_v860ext.extSIBTypeInfoSchedulingInfo_List.n; j++)
    {
      if((sb1_ptr->m.v6b0NonCriticalExtensionsPresent) &&
               (sb1_ptr->v6b0NonCriticalExtensions.m.v860NonCriticalExtensionsPresent) &&
               (sb1_ptr->v6b0NonCriticalExtensions.v860NonCriticalExtensions.sysInfoTypeSB1_v860ext.m.extSIBTypeInfoSchedulingInfo_ListPresent))
      {
        if(sb1_ptr->v6b0NonCriticalExtensions.v860NonCriticalExtensions.sysInfoTypeSB1_v860ext.extSIBTypeInfoSchedulingInfo_List.elem[j].extensionSIB_Type2.t & T_rrc_SIB_TypeExt2_systemInfoType19)
        {
          sched_present_for_sibs[rrc_SIB19] = TRUE;
        }
      }
  ...
  else if(sb2_ptr != NULL)
  {
    uint8 i = 0,j = 0;
    for (i =0; i< sb2_ptr->sib_ReferenceList.n;i++)
    {
      rrcsib_get_sib_type_from_sib_type_and_tag(wrrc_as_id,&sb2_ptr->sib_ReferenceList.elem[i].sib_Type,
                                                    &sib_type_tag);
      if(sib_type_tag.sib_type != rrc_MAX_SIB)
      {
        sched_present_for_sibs[sib_type_tag.sib_type] = TRUE;
    ...
    if(EXTENSION_SIB_SCHED_PRESENT_IN_SB1_SB2(sb2_ptr,rrc_SysInfoTypeSB2,sysInfoTypeSB2))
    {
      for(i = 0; i<sb2_ptr->v6b0NonCriticalExtensions.sysInfoTypeSB2_v6b0ext.extSIBTypeInfoSchedulingInfo_List.n; i++)
      {
        if(RRCCHECK_SIB_TypeExt((&(sb2_ptr->v6b0NonCriticalExtensions.sysInfoTypeSB2_v6b0ext.extSIBTypeInfoSchedulingInfo_List.elem[i].extensionSIB_Type)),systemInfoType11bis))
        {
          sched_present_for_sibs[rrc_SIB11bis] = TRUE;
        }
      }
...
#ifdef FEATURE_WCDMA_TO_LTE
    for(j = 0; j<sb2_ptr->v6b0NonCriticalExtensions.v860NonCriticalExtensions.sysInfoTypeSB2_v860ext.extSIBTypeInfoSchedulingInfo_List.n; j++)
    {
      if((sb2_ptr->m.v6b0NonCriticalExtensionsPresent) &&
         (sb2_ptr->v6b0NonCriticalExtensions.m.v860NonCriticalExtensionsPresent) &&
         (sb2_ptr->v6b0NonCriticalExtensions.v860NonCriticalExtensions.sysInfoTypeSB2_v860ext.m.extSIBTypeInfoSchedulingInfo_ListPresent))
      {
        if(sb2_ptr->v6b0NonCriticalExtensions.v860NonCriticalExtensions.sysInfoTypeSB2_v860ext.extSIBTypeInfoSchedulingInfo_List.elem[j].extensionSIB_Type2.t & T_rrc_SIB_TypeExt2_systemInfoType19)
        {
          sched_present_for_sibs[rrc_SIB19] = TRUE;
        }
      }
  ...
  {
    if((sched_present_for_sibs[count]) == TRUE)
    {
      switch(count)
      {
        case 0:
          wcdma_info->sib_bitmask |= WCDMA_SSGCCS_MIB_BIT;
          break;
        case 1:
          wcdma_info->sib_bitmask |= WCDMA_SSGCCS_SB_1_BIT;
          break;
        case 2:
          wcdma_info->sib_bitmask |= WCDMA_SSGCCS_SB_2_BIT;
          break;
        case 3:
          wcdma_info->sib_bitmask |= WCDMA_SSGCCS_SIB_1_BIT;
          break;
        case 4:
          wcdma_info->sib_bitmask |= WCDMA_SSGCCS_SIB_2_BIT;
          break;
        case 5:
          wcdma_info->sib_bitmask |= WCDMA_SSGCCS_SIB_3_BIT;
          break;
        case 6:
          wcdma_info->sib_bitmask |= WCDMA_SSGCCS_SIB_4_BIT;
          break;
        case 7:
          wcdma_info->sib_bitmask |= WCDMA_SSGCCS_SIB_5_BIT;
          break;
        case 8:
          wcdma_info->sib_bitmask |= WCDMA_SSGCCS_SIB_6_BIT;
          break;
        case 9:
          wcdma_info->sib_bitmask |= WCDMA_SSGCCS_SIB_7_BIT;
          break;
        case 10:
          wcdma_info->sib_bitmask |= WCDMA_SSGCCS_SIB_11_BIT;
          break;
        case 11:
          wcdma_info->sib_bitmask |= WCDMA_SSGCCS_SIB_12_BIT;
          break;
        case 12:
          wcdma_info->sib_bitmask |= WCDMA_SSGCCS_SIB_18_BIT;
          break;
        case 13:
          wcdma_info->sib_bitmask |= WCDMA_SSGCCS_SIB_5bis_BIT;
          break;
        case 14:
          wcdma_info->sib_bitmask |= WCDMA_SSGCCS_SIB_11BIS_BIT;
          break;
        case 15:
          wcdma_info->sib_bitmask |= WCDMA_SSGCCS_SIB_19_BIT;
          break;
        default:
          WRRC_MSG0_HIGH("SSGCCS: No SIB Scheduled in MIB/SB1/SB2");
          break;
      }
    }
  }
  WRRC_MSG1_HIGH("SSGCCS:scheduled sibmask = %d in MIB/SB1/SB2",wcdma_info->sib_bitmask);
}

/*===========================================================================

FUNCTION          RRC_CSP_SSGCCS_SEND_CELL_INFO

DESCRIPTION       This function sends the WCDMA cell info to SSGCCS QMI client 
...
void rrc_csp_ssgccs_send_cell_info(sys_modem_as_id_e_type wrrc_as_id           )
{
  rrc_SysInfoType3 *sib3_ptr;
  rrc_SysInfoType11 *sib11_ptr;
  wcdma_ssgccs_3g_cell_info_ind_s wcdma_info;
  uint8 i;

  if(cell_info_3g_msg_handler == NULL)
  {
    WRRC_MSG0_ERROR("SSGCCS: cell info msg handler is NULL");
    return;
  }
  memset(&wcdma_info,0,sizeof(wcdma_ssgccs_3g_cell_info_ind_s));
  wcdma_info.wrrc_as_id = wrrc_as_id;
  wcdma_info.cellid = rrc_csp_int_data.curr_acq_entry.cell_id;
  wcdma_info.psc = (uint16)(rrc_csp_int_data.curr_acq_entry.scr_code/16);
  wcdma_info.uarfcn = rrc_csp_int_data.curr_acq_entry.freq;
  wcdma_info.mcc = RRC_CSP_GET_INT_MCC(rrc_csp_int_data.curr_acq_entry.plmn_id);
  wcdma_info.mnc = RRC_CSP_GET_INT_MNC(rrc_csp_int_data.curr_acq_entry.plmn_id);

  if(rrc_csp_int_data.curr_acq_entry.is_valid_nw_sharing)
  {
    wcdma_info.shared_nw_mib_plmn_mcc = RRC_CSP_GET_INT_MCC(rrc_csp_int_data.ssgccs_nws_mib_plmn_id);
    wcdma_info.shared_nw_mib_plmn_mnc = RRC_CSP_GET_INT_MNC(rrc_csp_int_data.ssgccs_nws_mib_plmn_id);
    wcdma_info.num_of_shared_plmns = rrc_csp_int_data.curr_nw_db_entry.num_of_shared_plmns;
    WRRC_MSG1_HIGH("SSGCCS:NW is shared and number of shared PLMNs %d", wcdma_info.num_of_shared_plmns);
    for(i = 0;(i < MAX_SHARED_PLMNS) && ( i< wcdma_info.num_of_shared_plmns) ;i++)
    {
      wcdma_info.shared_plmn_list[i].mcc = RRC_CSP_GET_INT_MCC(rrc_csp_int_data.curr_nw_db_entry.shared_plmns_access_info[i].plmn_id);
      wcdma_info.shared_plmn_list[i].mnc = RRC_CSP_GET_INT_MNC(rrc_csp_int_data.curr_nw_db_entry.shared_plmns_access_info[i].plmn_id);
    }
  }

  sib3_ptr = (rrc_SysInfoType3 *)rrc_sibdb_return_sib_for_cell(wrrc_as_id, rrc_SIB3,rrc_csp_int_data.curr_acq_entry.plmn_id,rrc_csp_int_data.curr_acq_entry.cell_id);
  if(rrc_csp_int_data.curr_acq_entry.is_valid_nw_sharing)
  {
    WRRC_MSG2_HIGH(" SSGCCS:MIB PLMN (%d-%d) ",RRC_CSP_GET_INT_MCC(rrc_csp_int_data.ssgccs_nws_mib_plmn_id),RRC_CSP_GET_INT_MNC(rrc_csp_int_data.ssgccs_nws_mib_plmn_id));
    sib3_ptr = (rrc_SysInfoType3 *)rrc_sibdb_return_sib_for_cell(wrrc_as_id, rrc_SIB3,rrc_csp_int_data.ssgccs_nws_mib_plmn_id,rrc_csp_int_data.ssgccs_nws_cell_id);
  }

  if( sib3_ptr != NULL )
  {
    wcdma_info.s_Intrasearch = (int16)sib3_ptr->cellSelectReselectInfo.modeSpecificInfo.u.fdd->s_Intrasearch;
    wcdma_info.s_Intersearch = (int16)sib3_ptr->cellSelectReselectInfo.modeSpecificInfo.u.fdd->s_Intersearch;
    wcdma_info.q_QualMin = (int16)sib3_ptr->cellSelectReselectInfo.modeSpecificInfo.u.fdd->q_QualMin;
    wcdma_info.q_RxlevMin = (int16)((sib3_ptr->cellSelectReselectInfo.modeSpecificInfo.u.fdd->q_RxlevMin*2) + 1);
    wcdma_info.q_Hyst_l_S = sib3_ptr->cellSelectReselectInfo.q_Hyst_l_S;
    wcdma_info.t_Reselection_S = sib3_ptr->cellSelectReselectInfo.t_Reselection_S;
    wcdma_info.maxAllowedUL_TX_Power = (int16)(sib3_ptr->cellSelectReselectInfo.maxAllowedUL_TX_Power);
  }
  wcdma_info.curr_cell_access_status = rrc_csp_int_data.curr_cell_access_status;
  sib11_ptr = (rrc_SysInfoType11 *)rrc_sibdb_return_sib_for_cell(wrrc_as_id, rrc_SIB11,rrc_csp_int_data.curr_acq_entry.plmn_id,rrc_csp_int_data.curr_acq_entry.cell_id);
  if(rrc_csp_int_data.curr_acq_entry.is_valid_nw_sharing)
  {
    sib11_ptr = (rrc_SysInfoType11 *)rrc_sibdb_return_sib_for_cell(wrrc_as_id, rrc_SIB11,rrc_csp_int_data.ssgccs_nws_mib_plmn_id,rrc_csp_int_data.ssgccs_nws_cell_id);
  }
  if(sib11_ptr !=NULL)
  {
    uint8 i = 0,j = 0,count = 0,k=0;
    boolean freq_present = FALSE;

    /*W intra neighbors*/
    wcdma_info.sib11_info.intra_f_nbr_cnt= rrcmeas_ext_data.rrcmeas_sib11->intra_f.cell_list.num_cell_add;
    for(k=0;(k<rrcmeas_ext_data.rrcmeas_sib11->intra_f.cell_list.num_cell_add)&&(k<SSGCCS_L1_MAX_CELL_MEAS);k++)
    {
      wcdma_info.sib11_info.psc[k] = rrcmeas_ext_data.rrcmeas_sib11->intra_f.cell_list.add_cell[k].psc;
    }
    /*W inter neighbours*/
    for(i=0, count=0; (i< SSGCCS_L1_MAX_CELL_MEAS) && (count < SSGCCS_MAX_INTER_FREQ) ; i++)
    {
      if (rrcifreq_ext_data.inter_freq_cell_list[i].cell_position == OCCUPIED)
      {
        freq_present = FALSE;
        for(j=0; j<count ; j++)
        {
          if(wcdma_info.sib11_info.ifreq_nbr[j] == rrcifreq_ext_data.inter_freq_cell_list[i].dl_freq)
          {
            freq_present = TRUE;
            break;
          }
        ...
        if(!freq_present)
        {
          wcdma_info.sib11_info.ifreq_nbr[count++] = rrcifreq_ext_data.inter_freq_cell_list[i].dl_freq;
        }
      }
    }

    wcdma_info.sib11_info.ifreq_nbr_cnt = count;

    /*GSM neighbours*/
    for(i=0, count=0; (i< SSGCCS_L1_MAX_CELL_MEAS) && (count < SSGCCS_MAX_GSM_NBR_FREQ) ; i++)
    {
      if (rrcirat_ext_data.inter_rat_cell_list[i].cell_position == OCCUPIED)
      {
        freq_present = FALSE;
        for(j=0; j<count ; j++)
        {
          if(wcdma_info.sib11_info.gsm_arfcns[j] == rrcirat_ext_data.inter_rat_cell_list[i].arfcn)
          {
            freq_present = TRUE;
            break;
          }
        ...
        if(!freq_present)
        {
          wcdma_info.sib11_info.gsm_arfcns[count++] = rrcirat_ext_data.inter_rat_cell_list[i].arfcn;
        }
      }
    }
    wcdma_info.sib11_info.gsm_nbr_cnt = count;
  }

  rrcmeas_ssgccs_send_sib19_info(wrrc_as_id,&wcdma_info); 
  rrc_ssgccs_scheduling_info_of_sibs_in_mib_sb1_sb2(wrrc_as_id,&wcdma_info);
  WRRC_MSG0_HIGH(" SSGCCS: Sending WCDMA cell info to SSGCCS QMI client ");
  cell_info_3g_msg_handler(&wcdma_info);
}
...
===========================================================================*/
void
rrc_csp_send_mm_cmd(sys_modem_as_id_e_type wrrc_as_id,rrc_to_mm_sink_e_type  cmd_id, 
                    ...
                    rrc_csp_acq_entry_type *acq_entry_ptr)
{
...
#ifdef FEATURE_WCDMA_MULTIBAND
  rrc_csp_band_class_type band_class = rrc_get_frequency_band(wrrc_as_id,acq_entry_ptr->freq, acq_entry_ptr->plmn_id);
  sys_band_mask_e_type sys_band = rrc_convert_band_to_sys_band(wrrc_as_id,band_class);
#endif
  ...
  /* Hack */
  if(rrccsputil_int_data.search_order_specialization  == SYS_SPECIAL_SBM_RAT_SEARCH_ORDER)
  {
#ifdef FEATURE_RAT_PRIORITY_LIST
    srv_req_band_pref = rrc_extract_gw_band_pref_from_service_req(wrrc_as_id,&rrccsputil_int_data.rrc_csp_service_req.rat_pri_list_info);
#else
    srv_req_band_pref = rrccsputil_int_data.rrc_csp_service_req.band_pref;
#endif  
      ...
      if((rrc_csp_int_data.band_pref == SYS_BAND_MASK_WCDMA_IX_1700) || (rrc_csp_int_data.band_pref == SYS_BAND_MASK_WCDMA_XI_1500))
      {
        ...
        if(srv_req_band_pref & ~band_pref)
        {
          ...
          WCDMA_MEMCPY(&rrc_cmd_ptr->cmd.rrc_service_req, 
                       ...
                       &rrccsputil_int_data.rrc_csp_service_req, 
                       sizeof(rrc_service_req_type));
...
#endif  
          rrccsputil_int_data.rrc_csp_internal_service_req = TRUE;
#ifdef FEATURE_DUAL_WCDMA
          rrc_put_mm_cmd_sub(wrrc_as_id, rrc_cmd_ptr);
#else
  ...
  if(RRC_SERVICE_CNF == cmd_id)
  {
    if(MM_AS_SERVICE_AVAILABLE == service_status)
    {
      ...
      rrcmeas_send_clear_ded_pri_ind_set_owner(wrrc_as_id);

#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
      /* Start pseudo meas algorithm if LTE is added to the capability */
      rrc_pseudo_meas_start_algo_on_capability_change(wrrc_as_id);
#endif
    }
#ifdef FEATURE_MSIM_L_PLUS_L
    else if(LTE_RRC_DEDICATED_PRI_OWNER_WCDMA != lte_rrc_dedicated_pri_get_owner(rrc_get_as_id(wrrc_as_id)))
#else
    else if(LTE_RRC_DEDICATED_PRI_OWNER_WCDMA != lte_rrc_dedicated_pri_get_owner())
#endif
    {
      rrcmeas_invalidate_dedicated_priority_info(wrrc_as_id);
    }
  }
#endif

#ifdef FEATURE_FEMTO_CSG
  if( (TRUE == rrc_csg_int_data.is_whitelist_valid) &&
         (MM_AS_SERVICE_AVAILABLE == service_status)
      && (rrc_csp_int_data.curr_type_acq == RRC_CSP_SPEC_PLMN)
      && (rrc_ccm_get_curr_camped_csg_id(wrrc_as_id) == SYS_CSG_ID_INVALID)
      && (rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_CSG_ASF_SRCH_TIMER) == 0)
   )
  {
    if((rrc_csp_int_data.curr_scan == RRC_CSP_LEAVING_CONN_MODE_SCAN)||
       (rrc_csp_int_data.curr_scan == RRC_CSP_CELL_DCH_TO_CELL_PCH_SCAN)||
    ...
    {
      rrctmr_start_timer(wrrc_as_id,RRCTMR_CSG_ASF_SRCH_TIMER, RRC_CSG_DISCONN_TRANS_ASF_SRCH_TIMER_IN_MS);
    }
    else
    {
      rrctmr_start_timer(wrrc_as_id,RRCTMR_CSG_ASF_SRCH_TIMER, (rrcnv_data.wcdma_asf_timer_nv)*60000);
    }
  }
#endif
  ...
  WRRC_MSG3_MED("curr_scan %d, prev_curr_scan %d, svc_srch_bgnd %d", 
                                           rrc_csp_int_data.curr_scan, 
  ...
  switch(cmd_id)
  {
    case RRC_SERVICE_CNF:
    {
      ...
      /* Stop the duty cycle change timer and reset WL1 */
      rrctmr_stop_timer(wrrc_as_id, RRCTMR_ACQ_DTY_CYC_CHANGE_TIMER);
      wl1_ds_update_duty_cycle(FALSE,wrrc_as_id);	
      ...
      {
       rrc_release_lock(wrrc_as_id);
      }
      mm_buf_ptr->cmd.rrc_service_cnf.as_id = rrc_get_as_id(wrrc_as_id);
#endif
      ...
      if(RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS() && 
        ...
        (service_status == MM_AS_SERVICE_AVAILABLE) && (rrc_get_csp_substate(wrrc_as_id) == RRCCSP_CELL_SEL_WT_SIB))
      {
        hack_srvc_avail_for_lim_srvc = TRUE;
        WRRC_MSG0_HIGH("WRLF:Sending SERVICE_CNF as srvc_available to update UI with Lim srvc.");
      }
#endif
      rrcmcm_reset_last_act_cause(wrrc_as_id);
      /* Fill up header */
        ...
        if((rrc_csp_int_data.ue_in_deep_sleep) && (rrc_csp_int_data.srvc_req_scan_scope != SYS_SCAN_SCOPE_ACQ_DB))
        {
          ...
          { 
            if(rrc_csp_int_data.deep_sleep_no_svc_tmr_expired)
            {
              ...
              rrctmr_start_timer(wrrc_as_id,  RRCTMR_DEEP_SLEEP_NO_SVC_TIMER, 
                                    RRCTMR_DEEP_SLEEP_NO_SVC_TIMER_IN_MS );
#else
              rrctmr_start_timer(wrrc_as_id, RRCTMR_DEEP_SLEEP_NO_SVC_TIMER, 
                                  (unsigned long) rrcnv_data.wcdma_rrc_deep_sleep_no_svc_timer_nv );
#endif
      ...
      else  
      {
...
#endif
        WRRC_MSG1_MED("Reporting Cell_ID :%d to NAS",acq_entry_ptr->cell_id);

        if(rrc_csp_find_access_control(wrrc_as_id,&access_control, &rrc_dsac_info,
                                      acq_entry_ptr->plmn_id,
                                      acq_entry_ptr->cell_id
                                      ,common_plmn_id
                                      ,&rrc_ppac_info
        ...
        (void) rrc_csp_find_rrc_cell_access(wrrc_as_id,&(mm_buf_ptr->cmd.rrc_service_cnf.service_info), &rrc_dsac_info,
        ...
        if(RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS() && !hack_srvc_avail_for_lim_srvc)
        {
          ...
          WRRC_MSG3_HIGH("WRLF: Set mcc[0]=0x%x, mcc[1]=0x%x, mcc[2]=0x%x in RRC_SERVICE_CNF. ",
        ...
        {
          /* SIB3 for this PLMN, CEll ID combination not present */
          WRRC_ERR3_FATAL("SIB1 for MCC %d, MNC %d, Cell ID %d not present",
                        RRC_CSP_GET_INT_MCC(common_plmn_id), 
                        RRC_CSP_GET_INT_MNC(common_plmn_id),
                                         acq_entry_ptr->cell_id);
        } /* SIB1 not present */

        if(rrccsp_fill_nas_common_info(wrrc_as_id,sib1_ptr, 
                                       &mm_buf_ptr->cmd.rrc_service_cnf.
                                         service_info.gsm_map_common_info)
          == FAILURE)
        {
          WRRC_ERR0_FATAL("Not able to send MM command");
        }
        ...
        if(!rrc_csp_int_data.deep_sleep_no_svc_tmr_expired)
        {
          ...
          rrctmr_stop_timer(wrrc_as_id, RRCTMR_DEEP_SLEEP_NO_SVC_TIMER);
          rrc_csp_int_data.deep_sleep_no_svc_tmr_expired = TRUE;
        }

        if(!rrc_csp_int_data.full_scan_needed)
        {
          rrctmr_stop_timer(wrrc_as_id, RRCTMR_FREQ_SCAN_TIMER);
        ...
        if(RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS() && !hack_srvc_avail_for_lim_srvc 
           && (RRCMCM_WAIT_STOP_CNF != rrcmcm_get_substate(wrrc_as_id)) && (FALSE == rrccsp_check_for_stop_wcdma_req(wrrc_as_id)))
        {
          /* Send RAB_EST_IND to NAS if UE continues with the existing connection */
          rrcrbe_send_rabm_est_ind_to_upper_layers(wrrc_as_id);
          ...
          {
            rrctmr_stop_timer(wrrc_as_id, RRCTMR_CONN_MODE_OOS_TIMER);
            rrc_csp_int_data.oos_splmn_srch_tmr_expd = TRUE;
          }
        }
#endif
    ...
    case RRC_SERVICE_IND_FROM_OTA_MSG:
      {
        ...
        if(cmd_id == RRC_SERVICE_IND)
        {
          mm_buf_ptr->cmd.rrc_service_ind.as_id = rrc_get_as_id(wrrc_as_id);
        }
        else
        {
          mm_common_buf_ptr->cmd.rrc_service_ind_from_ota_msg.as_id = rrc_get_as_id(wrrc_as_id);
        }
#endif
        /* Fill up header */
        if(cmd_id == RRC_SERVICE_IND_FROM_OTA_MSG)
        ...
        else if(cmd_id == RRC_SERVICE_IND)   
        {
           ...
           if((service_status == MM_AS_NO_SERVICE)&&(rrc_get_state(wrrc_as_id) == RRC_STATE_DISCONNECTED))
           {
              rrcrce_force_initialize(wrrc_as_id, FALSE);
           }
        }
        else
        {
        ...
        {
          rrctmr_stop_timer(wrrc_as_id, RRCTMR_CONN_MODE_OOS_INITIAL_NO_DS_TIMER);
          rrc_csp_int_data.oos_initial_no_deep_sleep_tmr_expd = TRUE;
        }
        ...
        /* Get the cell access status of the camped on cell */
          if(cmd_id == RRC_SERVICE_IND_FROM_OTA_MSG)
          {
            ...
            (void) rrc_csp_find_rrc_cell_access(wrrc_as_id,&(mm_common_buf_ptr->cmd.rrc_service_ind_from_ota_msg.service_info), &rrc_dsac_info,access_control, acq_entry_ptr->status_mask);
            rrc_csp_int_data.is_dsac_valid  = rrc_dsac_info.valid;
          ...
          else if(cmd_id == RRC_SERVICE_IND)
          {
            ...
            (void) rrc_csp_find_rrc_cell_access(wrrc_as_id,&(mm_buf_ptr->cmd.rrc_service_ind.service_info), 
          ...
          if(((rrc_state == RRC_STATE_CELL_FACH) ||
              (rrc_state == RRC_STATE_CELL_PCH) ||
              (rrc_state == RRC_STATE_URA_PCH)) &&
              (FALSE == rrccho_is_wtog_cco_active(wrrc_as_id)))
          {
  ...
  /*Populate DSAC information in mm buffer */
        ...
        else
        {
          ...
          */
               ...
               (rrc_csp_int_data.prev_curr_scan == RRC_CSP_OUT_OF_SERVICE_AREA_SCAN))
          {
            ...
            rrc_csp_send_svc_status_to_mm(wrrc_as_id,SYS_SRV_STATUS_NONE, SYS_RAT_UMTS_RADIO_ACCESS, 0);
          }
#endif
#endif
#ifdef FEATURE_WRLF_SYSTEM_SEL
          if(RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS() ||
		  	      (rrc_csp_int_data.curr_scan == RRC_CSP_OUT_OF_SERVICE_AREA_SCAN && (RRC_EST_EMERGENCY_CALL == rrcrce_get_rrc_est_cause(wrrc_as_id))))
          {
            ...
            if(rrc_csp_int_data.csfb_extended_status == CSFB_CS_RB_RELEASED)
            {
              rrcscr_send_close_session_ind(wrrc_as_id,RRC_CS_DOMAIN_CN_ID,RRC_MAX_SESSION_CLOSE_CAUSE);
            }
#endif
          }
          else
          ...
          {
#ifdef FEATURE_DUAL_SIM
            if (rrc_dsds_int_data.oos_due_to_no_resource != TRUE)
#endif
            {
              rrctmr_stop_timer(wrrc_as_id, RRCTMR_CSG_ASF_SRCH_TIMER);
        ...
        if((cmd_id == RRC_SERVICE_IND) && (rrc_state == RRC_STATE_DISCONNECTED))
        {
          ...
          if(rrc_csp_int_data.rrc_csfb_call_status)
          {
            WRRC_MSG0_HIGH("DSIM: CSFB call. Do not release the lock");
          }
          else
#endif
          {
        ...
        if(((rrc_state == RRC_STATE_CELL_PCH) || (rrc_state == RRC_STATE_URA_PCH))
          &&!(rrccu_check_cu_active_before_lock_rel(wrrc_as_id)))
        {
          rrc_release_lock(wrrc_as_id);
        }
        ...
        if(cmd_id == RRC_SERVICE_IND)
        {
          if((rrc_dsds_int_data.oos_due_to_no_resource) && (service_status == MM_AS_NO_SERVICE))
          {
    ...
    default:
      WRRC_ERR1_FATAL("Invalid cmd ID 0x%x for send_mm_cmd", cmd_id); 
  ...
  if(cmd_id == RRC_SERVICE_IND)
  {
      WCDMA_MEMCPY(&rrccsputil_int_data.rrc_service_ind_bkup, 
                   sizeof(rrc_service_ind_type),
                   &mm_buf_ptr->cmd.rrc_service_ind, 
                   sizeof(rrc_service_ind_type));  
  }
  ...
  /* Put the command on MM queue */
#ifdef FEATURE_FEMTO_CSG
   WRRC_MSG2_MED("CSG: Reporting CSG ID: %d, is_hybrid : %d to NAS",acq_entry_ptr->csg_id,rrc_csp_int_data.curr_acq_entry.cell_is_hybrid);
#endif
   WRRC_MSG8_HIGH("DSAC/PPAC:params are DSAC Present, Cell Access, CS Cell Access and PS Cell Access %d %d %d %d PPAC valid, CS reg Allowed, PS reg Allowed, Page rsp %d %d %d %d", 
   ...
   if(cmd_id == RRC_SERVICE_IND_FROM_OTA_MSG)
   {
     WRRC_MSG3_HIGH(" mcc[0]=0x%x, mcc[1]=0x%x, mcc[2]=0x%x in RRC_SERVICE_IND_FROM_OTA_MSG ",
     ...
     rrc_send_rrc_message_to_mm(wrrc_as_id,mm_common_buf_ptr);
   }
   else if(cmd_id == RRC_SERVICE_IND || cmd_id == RRC_SERVICE_CNF)
   {
     if(cmd_id == RRC_SERVICE_IND)
     {
       rrc_log_search_confirm_info(wrrc_as_id,RRCLOG_NAS_SERVICE_IND,&mm_buf_ptr->cmd.rrc_service_ind);
     }
     else
     {
       rrc_log_search_confirm_info(wrrc_as_id,RRCLOG_NAS_SERVICE_CNF,&mm_buf_ptr->cmd.rrc_service_cnf);
     }
     rrc_send_rrc_message_to_mm(wrrc_as_id,mm_buf_ptr);
   }
   else
   {
     rrc_send_rrc_message_to_mm(wrrc_as_id,mm_buf_ptr);       /* More defensive check*/
   }

  /* When service is available sending 3G cell info to SSGCCS QMI Client */
  if((cmd_id == RRC_SERVICE_IND_FROM_OTA_MSG || cmd_id == RRC_SERVICE_IND || cmd_id == RRC_SERVICE_CNF)&& (service_status == MM_AS_SERVICE_AVAILABLE))
  {
    rrc_csp_ssgccs_send_cell_info(wrrc_as_id);
  }
  ...
  if(cmd_id == RRC_SERVICE_CNF)
  {
    if(service_status == MM_AS_SERVICE_AVAILABLE)
    {
      rrctmr_stop_timer(wrrc_as_id, RRCTMR_RSSI_BER_TIMER);
      rrc_rssi_ber_timeout_handler(wrrc_as_id);
  ...
  if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5)
  {
    if(cmd_id == RRC_SERVICE_IND_FROM_OTA_MSG || cmd_id == RRC_SERVICE_IND || cmd_id == RRC_SERVICE_CNF)
    {
      if(service_status == MM_AS_SERVICE_AVAILABLE)
      {
        ...
        new_hs_ind = rrc_get_3g_cell_capability_indicator(wrrc_as_id);
        if(cmd_id == RRC_SERVICE_CNF)
        {
          rrcdata_hs_info.hsdpa_hsupa_ind = new_hs_ind;
          rrc_send_3g_cell_capability_information(wrrc_as_id,rrcdata_hs_info.hsdpa_hsupa_ind);
        }
        else
        {
          if (new_hs_ind == rrcdata_hs_info.hsdpa_hsupa_ind)
          {
            WRRC_MSG1_HIGH("RRCHS:New cell same HS Cap %d",rrcdata_hs_info.hsdpa_hsupa_ind);
          }
          else
          {
            rrcdata_hs_info.hsdpa_hsupa_ind = new_hs_ind;
            WRRC_MSG1_HIGH("RRCHS:Send new HS Cap %d", rrcdata_hs_info.hsdpa_hsupa_ind);
            rrc_send_3g_cell_capability_information(wrrc_as_id,rrcdata_hs_info.hsdpa_hsupa_ind);
          }
        }
      }
      else
      {
        ...
        if(cmd_id == RRC_SERVICE_CNF)
        {
          rrc_send_3g_cell_capability_information(wrrc_as_id, SYS_HS_IND_HSDPA_HSUPA_UNSUPP_CELL);
        }
        else
        {
          if(rrcdata_hs_info.hsdpa_hsupa_ind != SYS_HS_IND_HSDPA_HSUPA_UNSUPP_CELL)
          {
            rrc_send_3g_cell_capability_information(wrrc_as_id, SYS_HS_IND_HSDPA_HSUPA_UNSUPP_CELL);
          }
        }
  ...
  rrc_csp_int_data.last_mm_primitive = 
    rrc_csp_mm_primitive_sent(wrrc_as_id,cmd_id, service_status);
...
#ifdef FEATURE_DUAL_SIM
  rrc_reset_oos_due_to_no_resource_flag(wrrc_as_id);
#endif
...
===========================================================================*/
void
rrc_csp_send_mm_plmn_list_cnf_cmd(sys_modem_as_id_e_type wrrc_as_id,boolean srch_done)
{
  ...
  if(!srch_done)
  {
    if(rrc_csp_int_data.freq_scan_list.priority_info == SYS_SRCH_PRIORITY_NORMAL)
    {
...
#ifdef FEATURE_DUAL_SIM
  mm_buf_ptr->cmd.rrc_plmn_list_cnf.as_id = rrc_get_as_id(wrrc_as_id);
#endif
  ...
  rrc_log_search_confirm_info(wrrc_as_id,RRCLOG_NAS_PLMN_CNF,&mm_buf_ptr->cmd.rrc_plmn_list_cnf);
  ...
  if(rrc_csp_cmd_to_mm_needed(wrrc_as_id,&mm_cmd_id,
                              rrc_csp_int_data.last_mm_primitive, 
                              rrc_csp_int_data.curr_service_status)
     == TRUE)
  {
    ...
    rrc_csp_send_mm_cmd(wrrc_as_id,mm_cmd_id,
                        rrc_csp_int_data.curr_service_status, 
                        &rrc_csp_int_data.curr_sel_plmn,
                        &rrc_csp_int_data.curr_acq_entry);
  }
...
#ifdef FEATURE_DUAL_SIM
  rrc_csp_int_data.rrc_drx_count = 0;
  rrc_csp_int_data.rrc_use_high_priority_for_bplmn = FALSE;
#endif
...
SIDE EFFECTS
                  None
===========================================================================*/
void rrc_csp_send_mm_plmn_list_cnf_as_aborted(sys_modem_as_id_e_type wrrc_as_id,uint8 trans_id)
{
  mm_cmd_type *mm_buf_ptr;
...
SIDE EFFECTS
                  None
===========================================================================*/
void rrc_csp_send_mm_plmn_list_cnf_as_success(sys_modem_as_id_e_type wrrc_as_id,uint8 trans_id)
{
  mm_cmd_type *mm_buf_ptr;
...
===========================================================================*/
void  
rrc_csp_bplmn_send_mm_plmn_list_cnf_cmd(sys_modem_as_id_e_type wrrc_as_id,wtow_bplmn_srch_status_type status, uint8 trans_id)
{
  mm_cmd_type *mm_buf_ptr;
  uint32 i, j;  
  ...
  else
  {
    ...
    {
      /*to report PLMNs only for periodic scan search*/
      if(rrc_csp_int_data.freq_scan_list.priority_info != SYS_SRCH_PRIORITY_NORMAL)
      {
        rrc_csp_int_data.plmn_list.num_plmns = rrc_csp_int_data.bplmn_fnd_list_wr_index;
      }
    ...
    {
      rrc_csp_int_data.plmn_list.num_plmns = rrc_csp_wtog_bplmn_int_data.wtog_gsm_plmns_found = 0;
      WRRC_ERR0_FATAL("ERR: status");
    }                                            
    WRRC_MSG2_MED("status = %d, total plmns %d", status,
                    rrc_csp_int_data.plmn_list.num_plmns + rrc_csp_wtog_bplmn_int_data.wtog_gsm_plmns_found);
  }  
...
WRRC_MSG2_MED("CSG CSG ID %d  cell is hybrid : %d",mm_buf_ptr->cmd.rrc_plmn_list_cnf.avail_plmn_list.plmn[i].csg_info.csg_id, rrc_csp_int_data.plmn_list.plmn[i].csg_info.hybrid_cell);
#endif
    mm_buf_ptr->cmd.rrc_plmn_list_cnf.avail_plmn_list.plmn[i].rat = rrc_csp_int_data.plmn_list.plmn[i].rat;
    mm_buf_ptr->cmd.rrc_plmn_list_cnf.avail_plmn_list.plmn[i].service_capability = rrc_csp_int_data.plmn_list.plmn[i].service_capability;
    mm_buf_ptr->cmd.rrc_plmn_list_cnf.avail_plmn_list.plmn[i].signal_quality = rrc_csp_int_data.plmn_list.plmn[i].signal_quality;
  ...
  for(j=0; j < rrc_csp_wtog_bplmn_int_data.wtol_plmns_found && i < RRC_MAX_PLMNS; j++, i++)
  {
    ...
    mm_buf_ptr->cmd.rrc_plmn_list_cnf.avail_plmn_list.plmn[i].csg_info.csg_id= 
                                            rrc_csp_wtog_bplmn_int_data.lte_plmn_list.plmn[j].csg_info.csg_id;
    WRRC_MSG1_MED("CSG: Adding LTE CSG ID %d",mm_buf_ptr->cmd.rrc_plmn_list_cnf.avail_plmn_list.plmn[i].csg_info.csg_id);
#endif
  ...
  else if(status == WTOW_SEARCH_ABORT)
  {
    if (list_cnf_for_new_req == FALSE)
    {
      memset(&rrc_csp_int_data.bplmn_search_timer_value,0,sizeof(sys_manual_search_timer_value_type));
      rrc_csp_int_data.send_partial_results = FALSE;
      rrctmr_stop_timer(wrrc_as_id, RRCTMR_BPLMN_PARTIAL_REPORT_TIMER);
    } 
    /* Set the status field to MM_AS_ABORTED */
    mm_buf_ptr->cmd.rrc_plmn_list_cnf.status = MM_AS_ABORTED;
    if((rrc_dsds_int_data.oos_due_to_no_resource) && (rrc_get_state(wrrc_as_id) == RRC_STATE_DISCONNECTED))
    {
  ...
  if(status != WTOW_SEARCH_PARTIAL)
  {
    ...
    rrc_csp_int_data.bplmn_data_saved = FALSE;
#ifdef FEATURE_DUAL_SIM
    rrc_csp_int_data.rrc_drx_count = 0;
    rrc_csp_int_data.rrc_use_high_priority_for_bplmn = FALSE;
#endif
  }
  WRRC_MSG4_HIGH("RRC_PLMN_LIST_CNF: n/w sel mode is %d, Trans I.D is %d, svc_srch_bgnd is %d status is %d",rrc_csp_int_data.bplmn_nw_sel_mode, trans_id, rrc_csp_int_data.service_search_bgnd, mm_buf_ptr->cmd.rrc_plmn_list_cnf.status);
  rrc_log_search_confirm_info(wrrc_as_id,RRCLOG_NAS_PLMN_CNF,&mm_buf_ptr->cmd.rrc_plmn_list_cnf);
  rrc_send_rrc_message_to_mm(wrrc_as_id,mm_buf_ptr);
...
DESCRIPTION       This sub specific API provides partial found PLMN results to RR during ongoing GtoW
                  Manual PLMN Search
...
SIDE EFFECTS
                  None
===========================================================================*/
boolean wrrc_get_inter_rat_bplmn_prtl_result_sub_id(sys_detailed_plmn_list_s_type *available_plmn_list, sys_modem_as_id_e_type wrrc_as_id)
{
  int i;
  boolean status;
...
#ifdef FEATURE_DUAL_SIM
  if (wrrc_as_id != rrc_get_as_id(wrrc_as_id))
  {
    WRRC_MSG2_ERROR("AS-id mismatch %d, internal %d",wrrc_as_id, rrc_get_as_id(wrrc_as_id));
  }
#endif
  ...
  for(i=0; i<rrc_csp_int_data.bplmn_fnd_list_wr_index; ++i)
  {
    ...
    WRRC_MSG3_HIGH("MPLMN Opt: plmns in partial result api PLMN %d-%d", RRC_CSP_GET_INT_MCC(nas_to_rrc_plmn_id),
...
{
  return(wrrc_get_inter_rat_bplmn_prtl_result_sub_id(available_plmn_list, SYS_MODEM_AS_ID_1));
}
...
SIDE EFFECTS
                  None
===========================================================================*/
void rrccsp_send_mplmn_partial_report_to_mm(sys_modem_as_id_e_type wrrc_as_id)
{
  int i; 
  ...
  {
    rrctmr_start_timer(wrrc_as_id,RRCTMR_BPLMN_PARTIAL_REPORT_TIMER, (unsigned long)(1000*rrc_csp_int_data.bplmn_search_timer_value.MorePLMNTimer));
  }
#ifdef FEATURE_WCDMA_TO_LTE  
     ...
     && rrc_csp_int_data.bplmn_interrat_type == RRC_CSP_INTERRAT_LTE)
  {
    ...
    rrc_send_lte_cmd(wrrc_as_id,&plmn_srch_req.msg_hdr, LTE_RRC_WCDMA_GET_PLMN_PRTL_RESULTS_REQ, sizeof(lte_irat_get_plmn_prtl_results_req_s));
  }
  else 
#endif  
  {
  if(rrc_csp_wtog_bplmn_int_data.wtog_bplmn_srch_active
     && rrc_get_rr_inter_rat_bplmn_prtl_result(wrrc_as_id,&available_plmn_list))
  {
  ...
  rrc_csp_int_data.send_partial_results = TRUE;
  rrc_csp_bplmn_send_mm_plmn_list_cnf_cmd(wrrc_as_id,WTOW_SEARCH_PARTIAL, rrc_csp_int_data.bplmn_trans_id);
  }
} /* rrc_csp_send_mm_plmn_list_cnf_as_aborted */
...
===========================================================================*/
uecomdef_status_e_type
rrc_csp_send_ccm_cmd(sys_modem_as_id_e_type wrrc_as_id,rrc_cmd_e_type  ccm_cmd, 
                     ...
                     rrc_csp_acq_entry_type  *acq_entry_ptr)
{
  ...
  else
  {
    common_plmn_id = acq_entry_ptr->plmn_id;
       ...
       */
          ...
          || RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS()
#endif
        ) && (RRC_STATE_CELL_FACH == rrc_get_state(wrrc_as_id)))
      {
      ...
      WRRC_MSG3_HIGH("Scan %d,cell chg type %d, RRC st %d", rrc_csp_int_data.curr_scan,
                  csp_out_cmd_ptr->cmd.cell_change_req.cell_change_type,
                  rrc_get_state(wrrc_as_id));
      ...
      (void)rrc_csp_find_rrc_cell_access(wrrc_as_id,&dummy_service_info, &rrc_dsac_info,
                                                          access_control,acq_entry_ptr->status_mask);
      if(rrc_dsac_info.valid != TRUE)
      {
  ...
  if((rrccsp_get_curr_nas_requested_csg_id(wrrc_as_id) != SYS_CSG_ID_INVALID)&&
    ...
    )
    {
       if((acq_entry_ptr->csg_id  != SYS_CSG_ID_INVALID))
       {
         rrccsp_add_macro_fp_info_in_acq_db(wrrc_as_id,&rrc_csp_int_data.curr_acq_entry);
...
/* Macro to femto reselections */
  ...
  {
    if((rrc_ccm_get_curr_camped_csg_id(wrrc_as_id) == SYS_CSG_ID_INVALID)&&
       (csp_out_cmd_ptr->cmd.cell_change_req.csg_id != SYS_CSG_ID_INVALID))
    {
      if(RRC_CAMPED_ON == rrc_ccm_get_curr_camping_status(wrrc_as_id,RRC_PROCEDURE_CSP))
      {
        rrc_csp_int_data.last_csg_rat     = RRC_CSG_WCDMA;
        rrc_csp_int_data.fp_macro.freq    = rrc_ccm_get_curr_camped_freq(wrrc_as_id);
        rrc_csp_int_data.fp_macro.pci     = rrc_ccm_get_curr_camped_psc(wrrc_as_id);
        rrc_csp_int_data.fp_macro.plmn    = rrc_ccm_get_curr_camped_plmn(wrrc_as_id);
        rrc_csp_int_data.fp_macro.cell_id = rrc_ccm_get_curr_camped_cell_id(wrrc_as_id);
      }
      else
      {
        rrc_csp_int_data.last_csg_rat = RRC_CSG_NONE;
      }
      rrccsp_add_macro_fp_info_in_acq_db(wrrc_as_id,&rrc_csp_int_data.new_cell_acq_entry);
...
===========================================================================*/
void rrccsp_get_serving_pccpch_parms(sys_modem_as_id_e_type wrrc_as_id,uint16 *scr_code_ptr, boolean *sttd_ptr)
{
...
===========================================================================*/
void rrccsp_get_current_plmn_cell_id(sys_modem_as_id_e_type wrrc_as_id,rrc_plmn_identity_type *plmn_id, rrc_int_cell_id_type *cell_id)
{
...
===========================================================================*/
void rrccsp_get_neighbor_pccpch_parms(sys_modem_as_id_e_type wrrc_as_id,uint16 *scr_code_ptr, rrc_freq_type *freq_ptr)
{
...
===========================================================================*/
void  rrc_csp_handle_no_available_freq(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_to_mm_sink_e_type   mm_cmd_id; 
  ...
  if(rrc_csp_int_data.curr_scan == RRC_CSP_PLMN_LIST_SCAN)
  {
     ...
     */
    rrccsp_check_and_start_plmn_list_full_freq_scan_timer(wrrc_as_id);

    /* Send the PLMN_LIST_CNF to MM */
    rrc_csp_send_mm_plmn_list_cnf_cmd(wrrc_as_id, TRUE);

    /* Reset the CSP internal data */
    rrc_csp_init_int_data(wrrc_as_id);
...
===========================================================================*/
void rrc_csp_send_chan_release(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_cmd_type        *csp_out_cmd_ptr;
...
===========================================================================*/
uecomdef_status_e_type  rrc_csp_send_chan_reconfig(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_state_e_type rrc_state = rrc_get_state(wrrc_as_id);
  rrc_cmd_type        *csp_out_cmd_ptr;
  /* Start with sending an RRC_CHANNEL_CONFIG_REQ to LLC to release all channels.
...
===========================================================================*/
rrc_csp_substate_e_type  rrccsp_rel_chls_and_go_to_idle(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_state_e_type rrc_state = rrc_get_state(wrrc_as_id);

  WRRC_MSG1_HIGH("UE going from %d to IDLE",rrc_state);
  
  rrccsp_cleanup_bplmn_state(wrrc_as_id);

  /* Release all channels and put lower layers to Idle  */
  rrc_csp_send_chan_release(wrrc_as_id);
           ...
           || (rrc_csp_int_data.curr_scan == RRC_CSP_GTOW_BPLMN_MANUAL_SCAN))
  {
    ...
    rrc_csp_int_data.rrc_rr_plmn_srch_cnf.preferred_plmn_found = FALSE;
      
    /* stop WCDMA stack */
    rrccsp_send_mode_change_req_to_mcm(wrrc_as_id, RRC_MODE_INACTIVE);
    return(RRCCSP_WT_STOP_MODE_CHANGE_CNF);
  }
  else
  {
    rrc_csp_handle_no_available_freq(wrrc_as_id);
...
)
{
  if(type_freq_scan != RRC_CSP_NO_SCAN)
  {
    ...
    {
      rrc_csp_send_l1_idle_req(wrrc_as_id);
    }
  }
  else
  {
    WRRC_ERR0_FATAL("type_freq_scan set to NO_SCAN");
  }
  ...
  if(type_freq_scan == RRC_CSP_ACQ_SCAN)
  {
    ...
    WRRC_MSG3_HIGH("Starting Acq on frequency: %d Acq_mode: %d, Acq_type: %d",
                    acq_entry_ptr->freq,rrc_csp_int_data.acq_mode,rrc_csp_int_data.acq_type);

    /* Initialize the current acquisition count in CSP internal database */
    rrc_csp_int_data.curr_acq_count = 0;
    rrc_csp_int_data.num_scr_code_det = 0;

    rrc_csp_send_l1_cmd(wrrc_as_id,RRC_PROCEDURE_CSP, CPHY_ACQ_REQ, acq_entry_ptr);
    MSG_MED("L1 cmd 0x%x sent", CPHY_ACQ_REQ, 0, 0);
    }
  else  /* Type of scan - Not Acquisition */
  {
    /* Send a frequency scan request */
    rrc_csp_send_l1_freq_scan_cmd(wrrc_as_id,RRC_PROCEDURE_CSP, acq_entry_ptr->freq, type_freq_scan);
...
SIDE EFFECTS
                  None
===========================================================================*/
void rrc_csp_send_tr_phy_chl_release(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_cmd_type        *csp_out_cmd_ptr;
  /* Start with sending an RRC_CHANNEL_CONFIG_REQ to LLC to release all channels.
   */
...
SIDE EFFECTS
                  None
===========================================================================*/
void rrc_csp_send_bch_release(sys_modem_as_id_e_type wrrc_as_id,boolean cnf_required)
{
  rrc_cmd_type        *csp_out_cmd_ptr;
  /* Start with sending an RRC_CHANNEL_CONFIG_REQ to LLC to release all channels.
   */
  WRRC_MSG0_HIGH("BCH teardown command is sent to RRC-LLC.");
  if((csp_out_cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id)) != NULL)
  {
    /* Populate the cmd data */
    csp_out_cmd_ptr->cmd_hdr.cmd_id = RRC_CHANNEL_CONFIG_REQ;
...
)
{
  ...
  switch(last_mm_primitive)
  {
    ...
    default:
      *mm_cmd_id = RRC_MM_SINK_CMD_MAX;
      WRRC_MSG1_ERROR("Invalid event %d when sending MM cmd", last_mm_primitive);
      break;
  }
  ...
  if(*mm_cmd_id == RRC_SERVICE_IND || *mm_cmd_id == RRC_SERVICE_CNF )
  {
     ...
     * if UE is not in Cell_DCH State */
    rrc_notify_cell_id_change(wrrc_as_id);
  }
  return(cmd_to_mm);
}   /* rrc_csp_cmd_to_mm_needed */
...
SIDE EFFECTS: NONE
=============================================================================*/
void rrc_csp_get_gsm_rr_arfcns(sys_modem_as_id_e_type wrrc_as_id)
{
  rr_rrc_gsm_cell_list_type gsm_cell_list;

  if(rrcmcm_is_dualmode_enabled(wrrc_as_id))
  {
#ifdef FEATURE_DUAL_SIM
    (void) rr_rrc_get_acq_chnl_info_sub_id(&gsm_cell_list, rrc_get_as_id(wrrc_as_id));
#else
  ...
  rrccsp_convert_active_gsm_channels_to_umts_uarfcns(wrrc_as_id, gsm_cell_list);
#endif
...
===========================================================================*/
void  rrc_csp_init_data_from_service_req(sys_modem_as_id_e_type wrrc_as_id,rrc_service_req_type  *rrc_service_req)
{
  uint32 i;
  ...
  rrc_log_search_request_info(wrrc_as_id,RRCLOG_NAS_SERVICE_REQ,rrc_service_req);
#ifdef FEATURE_DUAL_SIM
  rrc_dsds_int_data.csp_waiting_for_wl1_to_release_lock = FALSE;
#endif
  ...
  rrc_csp_int_data.lcf_in_idle_mode_skip_status = RRC_CSP_SKIP_FREQ;
  ...
  if(rrc_service_req->requested_plmn.csg_id != SYS_CSG_ID_INVALID)
  {
    ...
    if(!rrccsp_is_csg_white(wrrc_as_id,rrc_service_req->requested_plmn.csg_id,rrc_service_req->requested_plmn.plmn_id))
    {
  ...
  rrc_csp_int_data.band_pref = rrc_extract_gw_band_pref_from_service_req(wrrc_as_id,&rrc_csp_int_data.rat_pri_list_info);
#endif
  if(rrc_csp_int_data.power_up_service_req)
  {
    ...
    if(status == POLICYMAN_STATUS_SUCCESS)
    {
      rrccsputil_int_data.search_order_specialization = specialization;
    }
#endif
    WRRC_MSG0_MED("Search Order Specialization : Policyman specialization Initialization during Power UP");
...
/* Hack */
  if(rrccsputil_int_data.search_order_specialization  == SYS_SPECIAL_SBM_RAT_SEARCH_ORDER)
  {
    WRRC_MSG0_MED("Search Order Specialization : Policyman specialization enabled");
    if(!rrccsputil_int_data.rrc_csp_internal_service_req)
    {
      WRRC_MSG0_HIGH("Search Order Specialization : Backing up Service Request");
      WCDMA_MEMCPY(&rrccsputil_int_data.rrc_csp_service_req, 
                   sizeof(rrc_service_req_type),
                   rrc_service_req, 
                   sizeof(rrc_service_req_type));
    }
  ...
  WRRC_MSG9_HIGH("Service request parameters Network Select Mode = %d, Service req cause : %d, Scan Scope : %d skip gsm flag: %d, skip gsm from NAS: %d, UE in deep sleep %d, threshold count %d, GSM mode %d, LTE mode %d",
                  rrc_csp_int_data.network_select_mode, rrc_csp_int_data.service_req_cause, 
                  rrc_csp_int_data.srvc_req_scan_scope,rrc_csp_int_data.skip_gsm_search,
                  rrc_service_req->grat_scan_done,rrc_csp_int_data.ue_in_deep_sleep,rrc_csp_int_data.agc_threshold_cnt,
                  rrcmcm_is_dualmode_enabled_no_f3(wrrc_as_id),rrcmcm_is_lte_mode_enabled_wo_f3(wrrc_as_id));
   ...
   */
  rrcrce_force_initialize(wrrc_as_id, TRUE);
  }
  ...
  rrccsp_update_band_preference(wrrc_as_id);
  ...
  /* Reset the next_band_mask appropriately */
  WRRC_MSG0_MED("Resetting the next_band_mask and addl_chnl_done_mask");
  rrc_csp_curr_select_data.next_band_mask = rrc_csp_int_data.wcdma_band_mask;
  rrc_csp_curr_select_data.addl_chnl_done_mask = 0;
  ...
  else
  {
    rrc_csp_curr_select_data.current_band = 
      rrc_csp_curr_select_data.first_band_to_be_scanned;
    WRRC_MSG1_MED("1st band to be scanned = %d",
               rrc_csp_curr_select_data.first_band_to_be_scanned);
  }
...
===========================================================================*/
void  rrc_csp_init_bplmn_data(sys_modem_as_id_e_type wrrc_as_id)
{
  uint32 i;
  ...
  if(rrc_csp_int_data.bplmn_new_srch) 
  {
    ...
    rrc_csp_curr_select_data.first_band_to_be_scanned = RRC_CSP_BAND_MAX;
      ...
      {
        rrc_csp_curr_select_data.current_band = rrc_csp_curr_select_data.first_band_to_be_scanned;
        WRRC_MSG1_MED("1st band to be scanned = %d",rrc_csp_curr_select_data.first_band_to_be_scanned);
      }
...
#endif /* FEATURE_WCDMA_MULTIBAND */
    WRRC_MSG2_MED("UE in DS %d, thresh cnt %d", rrc_csp_int_data.ue_in_deep_sleep,rrc_csp_int_data.agc_threshold_cnt);
    ...
    if (rrc_csp_int_data.ue_in_deep_sleep) 
    {
      ...
      WRRC_MSG1_HIGH("Updated AGC thresh cnt %d",rrc_csp_int_data.agc_threshold_cnt);
    }
    else
    {
      rrc_csp_int_data.agc_threshold_cnt = 0;
...
rrc_csp_start_sel_on_mm_req(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_csp_freq_scan_e_type  type_freq_scan;
  ...
  rrcmeas_send_get_dedicated_priority_req(wrrc_as_id);
#endif

  /* Set current scan to "Cell Selection".*/   
  rrc_csp_int_data.curr_scan = RRC_CSP_CELL_SELECTION_SCAN;
  WRRC_MSG0_HIGH("Current scan is set to RRC_CSP_CELL_SELECTION_SCAN");

#ifdef FEATURE_LTE_TO_WCDMA
  if(rrcmeas_ext_data.rrc_deprioritization_db.is_deprio_db_valid == TRUE)
  {
     ...
     if(rrc_csp_int_data.curr_req_plmn.plmn_type == RRC_GSM_MAP_SPECIFIC_PLMN_REQ)
     {
        ...
        rrc_send_lte_cmd(wrrc_as_id,&lte_deprio_plmn_ind.msg_hdr,LTE_RRC_DEPRI_REQUESTED_PLMN_IND,sizeof(lte_rrc_depri_requested_plmn_ind_s));
  ...
  rr_rrc_set_band_pref_internal(wrrc_as_id,rrc_csp_int_data.band_pref);
  ...
  if(rrc_csp_start_cell_selection(wrrc_as_id,&rrc_csp_int_data.curr_req_plmn, 
    ...
    == FAILURE)
  {
    ...
    if(type_freq_scan == RRC_CSP_GSM_SCAN)
    {
      ...
      rrccsp_send_mode_change_req_to_mcm(wrrc_as_id, RRC_MODE_STANDBY);
              ...
              && RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS())
    {
      ...
      if(rrc_csp_int_data.curr_scan == RRC_CSP_CELL_SELECTION_SCAN)
      {
        rrc_csp_send_mm_cmd(wrrc_as_id,RRC_SERVICE_CNF,
                               rrc_csp_int_data.curr_service_status, 
                               &rrc_csp_int_data.curr_sel_plmn,
                            &rrc_csp_int_data.curr_acq_entry);
      }
      else /* curr_scan = RRC_CSP_LEAVING_CONN_MODE_SCAN*/
      {
        rrc_csp_send_mm_cmd(wrrc_as_id,RRC_SERVICE_IND,
                            ...
                            &rrc_csp_int_data.curr_sel_plmn,
                            &rrc_csp_int_data.curr_acq_entry);
        rrc_csp_send_disable_bplmn_ind(wrrc_as_id, TRUE);
      }
      return RRCCSP_NO_CELL_SELECTED;
    }
    else
#endif
    {
      ...
      return (rrccsp_rel_chls_and_go_to_idle(wrrc_as_id));
    }
  }
  ...
  if(type_freq_scan == RRC_CSP_NO_SCAN)
  {
    ...
    WRRC_ERR0_FATAL("NO_SCAN returned");
  }

#ifdef FEATURE_DUAL_SIM
  /* Stop RSSI reporting timer when going to wait for lock state, so that RRC does not report invalid values */
  rrctmr_stop_timer(wrrc_as_id, RRCTMR_RSSI_BER_TIMER);
  rrc_dsds_int_data.shld_acq_be_spltd = TRUE;

  /* Before starting the search stop the timer and reset WL1 */
  rrctmr_stop_timer(wrrc_as_id, RRCTMR_ACQ_DTY_CYC_CHANGE_TIMER);
  wl1_ds_update_duty_cycle(FALSE,wrrc_as_id);

  /* Start the timer afresh for the search */
  rrctmr_start_timer(wrrc_as_id,RRCTMR_ACQ_DTY_CYC_CHANGE_TIMER,RRCTMR_ACQ_DUTY_CYCLE_BUMP_TIME_IN_MS);
  
  rrc_start_lock_wait_timer(wrrc_as_id,RRCTMR_DEFAULT_SEARCH_TIME_IN_MS,TRUE);
  next_substate = rrc_csp_wait_for_wrm_lock(wrrc_as_id,&type_freq_scan);
  if(type_freq_scan == RRC_CSP_NO_SCAN)
  {
  ...
  rrc_csp_send_next_freq_to_l1(wrrc_as_id,&rrc_csp_int_data.curr_acq_entry, type_freq_scan);
...
===========================================================================*/
boolean l1_rrc_is_bplmn_active(sys_modem_as_id_e_type wrrc_as_id)
{
  boolean ret_sts = FALSE;
  
  if(WTOW_SEARCH_STOP != rrc_csp_bplmn_srch_in_progress(wrrc_as_id))  
  {
    ret_sts = TRUE;
  }
  return ret_sts;
...
===========================================================================*/
void rrccsp_cleanup_bplmn_state(sys_modem_as_id_e_type wrrc_as_id)
{
   ...
   */
#ifdef FEATURE_DUAL_SIM
  rrc_csp_handle_bplmn_suspend_lock_rel_offset_tmr_running(wrrc_as_id);
#endif
  ...
  {
    WRRC_MSG0_HIGH_OPT("WTOW: No BPLMN cleanup required");
  }
  ...
  else
  {
        ...
        || (rrc_csp_int_data.curr_scan == RRC_CSP_WTOW_BPLMN_MANUAL_SCAN))
    {
      ...
      rrccsp_reset_bplmn_list_search_state(wrrc_as_id,WTOW_SEARCH_ABORT, TRUE, TRUE);
    }
    else
    {
      ...
      if(WTOW_SEARCH_RESUME == rrc_csp_bplmn_srch_in_progress(wrrc_as_id))
      {
        ...
        else
        {
          ...
          {
            rrccsp_send_irat_bplmn_abort_req(wrrc_as_id);
          }
          rrccsp_reset_bplmn_list_search_state(wrrc_as_id,WTOW_SEARCH_ABORT, TRUE, FALSE);
        }
      }
      else if(WTOW_SEARCH_SUSPEND == rrc_csp_bplmn_srch_in_progress(wrrc_as_id))
      {
      ...
      {
        WRRC_ERR0_FATAL("WTOW ERR: should never come here");
      }
      else
      {
...
===========================================================================*/
void rrccsp_cleanup_and_restart_bplmn(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  else
  {
    ...
    {
      WRRC_MSG0_HIGH("WTOW: Send sleep ind to L1");
      rrccsp_reset_bplmn_list_search_state(wrrc_as_id,WTOW_SEARCH_SUSPEND, FALSE, TRUE);
    }
    else
    {
      if(WTOW_SEARCH_RESUME == rrc_csp_bplmn_srch_in_progress(wrrc_as_id))
      {
        ...
        {
          rrccsp_reset_bplmn_list_search_state(wrrc_as_id,WTOW_SEARCH_SUSPEND, FALSE, FALSE);
        }
      }
      else if(WTOW_SEARCH_SUSPEND == rrc_csp_bplmn_srch_in_progress(wrrc_as_id))
      {
    ...
    rrccsp_reset_bplmn_list_search_state(wrrc_as_id,WTOW_SEARCH_START, FALSE, FALSE);
    rrccsp_generate_bplmn_rat_srch_order_update_curr_srch_rat(wrrc_as_id,rrc_csp_int_data.bplmn_nw_sel_mode);
    ...
    if(rrc_csp_int_data.bplmn_nw_sel_mode == SYS_NETWORK_SELECTION_MODE_MANUAL)
    {
      ...
      {
        rrccsp_update_wtow_manual_bplmn_list_with_rplmn(wrrc_as_id,rrc_csp_int_data.curr_acq_entry.plmn_id,
...
===========================================================================*/
rrc_csp_substate_e_type
rrc_csp_release_chls_and_start_sel_on_mm_req(sys_modem_as_id_e_type wrrc_as_id)
{
...
/* Abort PG1 whenever we trigger a new camping procedure */
  rrcpg1_check_and_abort_bcch_mod_info_processing(wrrc_as_id);
  ...
  {
    rrc_csp_cleanup_inter_freq_redirection_state(wrrc_as_id);
  }               

  next_substate = rrc_csp_start_sel_on_mm_req(wrrc_as_id);
...
===========================================================================*/
rrc_csp_substate_e_type
rrc_csp_start_bplmn_srch(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_csp_freq_scan_e_type  type_freq_scan;
     ...
     == FAILURE)
  {
       ...
       || (rrc_csp_int_data.curr_scan == RRC_CSP_WTOW_BPLMN_MANUAL_SCAN))
    {
      ...
      {
        rrccsp_reset_bplmn_list_search_state(wrrc_as_id,WTOW_SEARCH_STOP, TRUE, TRUE);
      }
      else
      { 
        rrccsp_reset_bplmn_list_search_state(wrrc_as_id,WTOW_SEARCH_SUSPEND, FALSE, TRUE);
        rrc_csp_int_data.plmn_srch_order.curr_rat_srch_idx++;
        rrc_csp_init_wtog_plmn_srch_int_data_across_rats(wrrc_as_id);
  ...
  else  /* Another freq found */
  {
       ...
       || (rrc_csp_int_data.curr_scan == RRC_CSP_WTOW_BPLMN_MANUAL_SCAN))
    {
      ...
      lock_wait_timer = rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_WTOW_BPLMN_DRX_SRCH_TIMER) - 2;
    }
    else if (rrc_csp_int_data.curr_scan == RRC_CSP_PLMN_LIST_SCAN)
    {
    ...
    else
    {
      ...
      lock_wait_timer = rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_BPLMN_SRCH_TIMER) - 2;
    }
    rrc_start_lock_wait_timer(wrrc_as_id,lock_wait_timer,TRUE);
    rrc_dsds_int_data.shld_acq_be_spltd = TRUE;
    next_substate = rrc_csp_wait_for_wrm_lock(wrrc_as_id,&type_freq_scan);
    if(type_freq_scan == RRC_CSP_NO_SCAN)
    {
...
===========================================================================*/
rrc_csp_substate_e_type rrc_csp_resume_wtog_bplmn_srch(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_csp_substate_e_type next_substate;
  ...
  {
    rrc_csp_wtog_bplmn_int_data.wtog_rem_drx_tmr_value = rrctmr_get_remaining_time(wrrc_as_id, RRCTMR_WTOW_BPLMN_DRX_SRCH_TIMER);
    ...
    {
      WRRC_MSG2_MED("WTOG: remaining DRX tmr %d >= min %d. Do GSM Search. Suspend W", 
                                  rrc_csp_wtog_bplmn_int_data.wtog_rem_drx_tmr_value,
                                  MIN_WTOW_BPLMN_SRCH_TIMER_IN_MS - WTOG_BPLMN_SEARCH_OVERHEAD_TIMER_IN_MS);
    ...
    rrctmr_stop_timer(wrrc_as_id, RRCTMR_WTOW_BPLMN_DRX_SRCH_TIMER);
    rrc_csp_int_data.bplmn_srch_tmr_expired = TRUE;
  }
...
===========================================================================*/
static rrc_csp_substate_e_type rrc_csp_continue_oos_in_connected_mode_search(sys_modem_as_id_e_type wrrc_as_id,
                              rrc_csp_freq_scan_e_type  *type_freq_scan_ptr)
{
  ...
  else
  {
           ...
           )
    {
      ...
      rrctmr_start_timer(wrrc_as_id,RRCTMR_CONN_MODE_OOS_DS_TIMER, 
                         RRCTMR_CONN_MODE_OOS_DS_TIMER_IN_MS);
      rrc_csp_int_data.oos_conn_mode_ds_tmr_expd = FALSE;

      rrccsp_send_conn_mode_oos_ds_req(wrrc_as_id);
...
===========================================================================*/
rrc_csp_substate_e_type
rrc_csp_continue_cell_selection(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  if(rrcrcr_is_ifreq_rdr_in_prgrs(wrrc_as_id) == TRUE)
  {
     rrclog_wcdma_to_wcdma_redir_end_event_type wtow_redirct_event;
     wtow_redirct_event.status = WTOW_FAILURE;
     WRRC_EVENT_REPORT_PAYLOAD(EVENT_WCDMA_TO_WCDMA_REDIRECTION_END, sizeof(wtow_redirct_event),
                 (void *)&wtow_redirct_event);
     rrcrcr_reset_ifreq_rdr_in_prgrs(wrrc_as_id);
  }
      ...
      (rrc_csp_curr_select_data.scan_info.type_freq_scan == RRC_CSP_LIST_SCAN))
  {
    ...
    if(((temp_acq_entry = rrc_csp_get_list_db_match_acq_entry(wrrc_as_id,&type_freq_scan))  != NULL))
    {
    ...
    else
    {
        ...
        )
      {
        ...
        {
          rrctmr_start_timer(wrrc_as_id,RRCTMR_GTOW_REDIRECTION_LFS_TIMER,RRCTMR_GTOW_REDIRECTION_LFS_TIMER_IN_MS);
#ifdef FEATURE_DUAL_SIM
          rrc_release_lock(wrrc_as_id);
#endif
...
#endif
        {
          rrctmr_stop_timer(wrrc_as_id, RRCTMR_GTOW_REDIRECTION_LFS_TIMER);
               ...
               (rrc_csp_int_data.curr_scan == RRC_CSP_CELL_SELECTION_SCAN))
    {
      ...
      return(RRCCSP_NO_CELL_SELECTED);
               ...
               && (rrcmcm_is_dualmode_enabled_no_f3(wrrc_as_id))
                 && (!rrc_csp_int_data.gsm_svc_srch_done)
                   && (!rrc_csp_int_data.ue_in_deep_sleep)
                     && (rrccsp_do_gsm_bands_scan_first(wrrc_as_id))
    ...
    ))
  {
    ...
    else if(rrc_csp_int_data.curr_scan == RRC_CSP_OUT_OF_SERVICE_AREA_SCAN)
    {
      ...
      if(RRC_EST_EMERGENCY_CALL == rrcrce_get_rrc_est_cause(wrrc_as_id))
      {
             /*
              1. CSFB to 3G with emergency call in progress
              2. NAS sent Soft Abort Req to WRRC
              3. WRRC is performing OOS recovery search upon receiving RL failure 
                 from lower layer and the RRC_ABORT_REQ is pended
              4. W triggers G search and G cell is found.
              5. GSM sends RR_ACTIVATION_IND to NAS 
              6. NAs doesnt respond to G as NAS is waiting for ABORT CNF from WRRC.

               After step 3 RRC will send the service_ind to NAS for RLF
              */

		rrc_csp_int_data.curr_service_status	 = MM_AS_NO_SERVICE;
		rrc_csp_int_data.curr_sel_plmn.plmn_type = RRC_GSM_MAP_PLMN_SEL;

            /*Trigger WRLF to NAS once W search is complete and before
             starting OOS service search on G. 
	     */    
	     rrc_csp_send_mm_cmd(wrrc_as_id,RRC_SERVICE_IND,
						  rrc_csp_int_data.curr_service_status, 
						  &rrc_csp_int_data.curr_sel_plmn,
      ...
      next_substate = rrc_csp_continue_oos_in_connected_mode_search(wrrc_as_id,&type_freq_scan);
     ...
     */
            ...
            || (rrc_csp_int_data.curr_scan == RRC_CSP_WTOW_BPLMN_MANUAL_SCAN))
    {
      WRRC_MSG0_MED("WTOW: no more freq fnd. W Search completed");
...
#endif
        return(rrc_csp_resume_wtog_bplmn_srch(wrrc_as_id));
      }
    }
            ...
            && (rrc_csp_int_data.bplmn_nw_sel_mode == SYS_NETWORK_SELECTION_MODE_MANUAL))
    {
      ...
      next_substate = rrccsp_continue_bplmn_in_frgrnd(wrrc_as_id);
...
#ifdef FEATURE_DUAL_SIM
  if(
      (rrc_dsds_int_data.wcdma_is_ue_in_dual_standby) &&
      (rrc_csp_int_data.curr_scan != RRC_CSP_OUT_OF_SERVICE_AREA_IN_IDLE_STATE_SCAN) &&
      (rrc_csp_int_data.curr_scan != RRC_CSP_INTER_FREQ_REDIRECTION_SCAN) &&
      (!((rrc_csp_int_data.curr_scan == RRC_CSP_LEAVING_CONN_MODE_SCAN) && (!RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS())))
     )
  {
    rrc_dsds_int_data.shld_acq_be_spltd = TRUE;
    next_substate = rrc_csp_wait_for_wrm_lock(wrrc_as_id,&type_freq_scan);
...
===========================================================================*/
rrc_csp_substate_e_type
rrc_csp_release_chls_and_continue_cell_selection(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_csp_substate_e_type   next_substate;
  rrc_state_e_type rrc_state = rrc_get_state(wrrc_as_id);
  ...
  else
  {
    ...
    if((WRM_STATE_IS_IN_LOCK(WCDMA_TRM, wrrc_as_id)) &&
        ((rrc_csp_int_data.curr_scan == RRC_CSP_OUT_OF_SERVICE_AREA_SCAN) ||
         (rrc_csp_int_data.curr_scan == RRC_CSP_CELL_SELECTION_SCAN) ||
         (rrc_csp_int_data.curr_scan == RRC_CSP_PLMN_LIST_SCAN) 
#ifdef FEATURE_WRLF_SYSTEM_SEL
    ...
    {
      rrc_dsds_int_data.csp_waiting_for_wl1_to_release_lock = TRUE;
      rrc_csp_send_l1_idle_req(wrrc_as_id);

      return (rrc_get_csp_substate(wrrc_as_id));
    }
#endif

    next_substate = rrc_csp_continue_cell_selection(wrrc_as_id);
  }
...
===========================================================================*/
rrc_csp_substate_e_type
rrc_csp_release_bch_and_continue_cell_selection(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_csp_substate_e_type   next_substate;
  ...
  if(rrc_dsds_int_data.wcdma_is_ue_in_dual_standby)
  {
    ...
    if((rrc_csp_int_data.curr_scan == RRC_CSP_WTOW_BPLMN_AUTOMATIC_SCAN)||
       (rrc_csp_int_data.curr_scan == RRC_CSP_WTOW_BPLMN_MANUAL_SCAN))
    {
      rrc_csp_send_bch_release(wrrc_as_id, TRUE);
    }
    else
    {
      if(!rrc_dsds_int_data.rrc_dl_wk_no_res)
      {
        rrc_csp_send_bch_release(wrrc_as_id, FALSE);
      }		
      rrc_dsds_int_data.rrc_dl_wk_no_res = FALSE;
      rrc_csp_send_l1_idle_req(wrrc_as_id);
    }
    return (rrc_get_csp_substate(wrrc_as_id));
  }
...
===========================================================================*/
rrc_csp_substate_e_type
rrc_csp_start_sel_redirect_freq_in_connecting_state(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  rrc_csp_send_tr_phy_chl_release(wrrc_as_id);
  WRRC_MSG1_HIGH("Acq req for redirected freq %d sent to L1", 
                              rrc_csp_int_data.curr_acq_entry.freq);

  /* Send redirected frequency to L1 for acquisition */
  rrc_csp_send_next_freq_to_l1(wrrc_as_id,&rrc_csp_int_data.curr_acq_entry, RRC_CSP_ACQ_SCAN);
...
===========================================================================*/
rrc_csp_substate_e_type
rrc_csp_start_sel_out_of_service_area(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_csp_freq_scan_e_type  type_freq_scan;
  ...
  rrc_csp_int_data.ue_in_conn_oos = FALSE;
  if(!( (RRC_EST_EMERGENCY_CALL == rrcrce_get_rrc_est_cause(wrrc_as_id)) &&
        (rrcdt_is_cs_domain_open(wrrc_as_id))
      )
    )
  {
  ...
  rrctmr_start_timer(wrrc_as_id,RRCTMR_CONN_MODE_OOS_TIMER, RRCTMR_CONN_MODE_OOS_TIMER_IN_MS);
   ...
   */
  #else
  rrctmr_start_timer(wrrc_as_id,RRCTMR_CONN_MODE_OOS_INITIAL_NO_DS_TIMER, 
   ...
   */
  rrc_csp_int_data.last_camped_freq_in_connected_mode = rrc_ccm_get_curr_camped_freq(wrrc_as_id);
...
#ifdef FEATURE_DUAL_SIM
  if(TRM_TRAFFIC != rrc_dsds_int_data.rrc_trm_priority)
  {
    rrc_release_lock(wrrc_as_id);
  }

  if((rrc_csp_int_data.curr_scan == RRC_CSP_LEAVING_CONN_MODE_SCAN) && RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS())
  { /*For AcqDB scan during COOS, start a timer less than 30secs and inform NAS about NO service upon expiry*/
    rrc_start_lock_wait_timer(wrrc_as_id,RRCTMR_CONN_MODE_OOS_TIMER_IN_MS-10000,TRUE);
  }
  else
  {
    rrc_start_lock_wait_timer(wrrc_as_id,RRCTMR_DEFAULT_SEARCH_TIME_IN_MS,TRUE);
...
===========================================================================*/
rrc_csp_substate_e_type
rrc_csp_start_sel_out_of_service_area_in_idle_state(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_csp_freq_scan_e_type  type_freq_scan;
  rrc_csp_substate_e_type   next_substate = RRCCSP_NO_CELL_SELECTED;
  ...
  rrcsib_force_sib_event_init(wrrc_as_id, RRCSIB_ABORT_DUE_TO_OOS);

  /* Manual BPLMN search is in progress */
  /*MPLMN freq do frgnd search only for high/normal priority search*/
  if((WTOW_SEARCH_STOP != rrc_csp_bplmn_srch_in_progress(wrrc_as_id))&& 
     (rrc_csp_int_data.bplmn_nw_sel_mode == SYS_NETWORK_SELECTION_MODE_MANUAL)&&
     (rrc_csp_int_data.freq_scan_list.priority_info != SYS_SRCH_PRIORITY_LOW)
#ifdef FEATURE_DUAL_SIM
      && (!rrc_get_oos_due_to_no_resource_flag_status(wrrc_as_id)) /* Abort MPLMN search for no lock case instead of continuing in foreground*/
#endif
    )
  {
    next_substate = rrccsp_continue_bplmn_in_frgrnd(wrrc_as_id);
...
===========================================================================*/
rrc_csp_substate_e_type
rrc_csp_start_sel_out_of_service_area_in_connecting_state(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_csp_freq_scan_e_type  type_freq_scan;
  ...
  rrc_start_lock_wait_timer(wrrc_as_id, RRCTMR_PHYCHANFAILURE_TIMER_IN_MS,TRUE);  //Lock grant should be immediate, as L1 will be holding the lock.
  rrc_dsds_int_data.shld_acq_be_spltd = TRUE;
  next_substate = rrc_csp_wait_for_wrm_lock(wrrc_as_id, &type_freq_scan);
  if(type_freq_scan == RRC_CSP_NO_SCAN)
  {
...
===========================================================================*/
rrc_csp_substate_e_type
rrc_csp_start_sel_from_dch_to_fach(sys_modem_as_id_e_type wrrc_as_id,rrc_cmd_type  *rrc_cmd_ptr)
{
  rrc_csp_freq_scan_e_type  type_freq_scan = RRC_CSP_NO_SCAN;
  ...
  redirection_barred = rrccsp_check_redirection_barred(wrrc_as_id, rrc_cmd_ptr);
  ...
  rrc_csp_get_aset_scr_pn(wrrc_as_id);
  ...
  /* For DCH to FACH reconfig, channels might have been dropped already as part of 
     WL1 lock release. If this flag is set, then don't send channel release to L1
     as it will result in a crash because WL1 might have moved to LM_IDLE already
   */
#ifdef FEATURE_DUAL_SIM
  if(!rrc_dsds_int_data.tr_phy_chl_rel_sent)
#endif
  {
    rrc_csp_send_tr_phy_chl_release(wrrc_as_id);
  }	
   ...
   * DCH to FACH.
   */                       
  if(rrccsp_is_state_transition_triggered_by_reconfig_msg(wrrc_as_id))
  {
    WRRC_MSG0_HIGH("OOSC: DCH->FACH due to reconfiguration");
    rrctmr_start_timer(wrrc_as_id,RRCTMR_DCH_FACH_CELL_SELECTION_TIMER, 
                       (RRCTMR_DCH_FACH_CELL_SELECTION_TIMER_IN_MS + rrcllc_get_dch_fach_act_time_in_ms(wrrc_as_id)));

    WRRC_MSG2_HIGH("DCH->FACH timer: started %d ms + act_time %d", RRCTMR_DCH_FACH_CELL_SELECTION_TIMER_IN_MS,
             rrcllc_get_dch_fach_act_time_in_ms(wrrc_as_id));
     ...
     )
  {
    ...
    if(type_freq_scan == RRC_CSP_NO_SCAN)
    {
      return next_substate;
          ...
          (rrc_csp_int_data.dest_freq_present))
  {
    /* Acquire lock with ACQ_INV as we want to prioritize state transition scans from lower priority activities */
    lock_status = rrc_wrm_acquire_lock_on_redirected_freq(wrrc_as_id,rrc_csp_int_data.dest_freq,TRM_ACQUISITION_INV, RRCTMR_DCH_FACH_CELL_SELECTION_TIMER_IN_MS + rrcllc_get_dch_fach_act_time_in_ms(wrrc_as_id));

    if(lock_status != RRC_WRM_LOCK_ACQUIRED)
    {
      rrctmr_stop_timer(wrrc_as_id,RRCTMR_DCH_FACH_CELL_SELECTION_TIMER);
      next_substate = rrccsp_send_cell_update_initiate_req(wrrc_as_id, RRC_REENTERED_SERVICE_AREA);
      return next_substate;
    }
  }
  else
  {
    rrc_start_lock_wait_timer(wrrc_as_id,(RRCTMR_DCH_FACH_CELL_SELECTION_TIMER_IN_MS + rrcllc_get_dch_fach_act_time_in_ms(wrrc_as_id)) - 10,TRUE);
    next_substate = rrc_csp_wait_for_wrm_lock(wrrc_as_id,&type_freq_scan);
    if(type_freq_scan == RRC_CSP_NO_SCAN)
    {
      return next_substate;
  ...
  if(type_freq_scan == RRC_CSP_LAST_ASET_SCAN)
  {
    ...
    rrc_csp_send_l1_cmd(wrrc_as_id,RRC_PROCEDURE_CSP, CPHY_ACQ_REQ, &rrc_csp_int_data.curr_acq_entry);
    WRRC_MSG3_HIGH("ACQ_REQ for the best cell in active set. Freq %d, SCR = %d, PN = %d",
                                                    rrc_csp_int_data.curr_acq_entry.freq,
                                                    rrc_csp_int_data.curr_acq_entry.scr_code,
                                                    rrc_csp_int_data.pn_pos);
...
===========================================================================*/
rrc_csp_substate_e_type
rrc_csp_start_sel_from_dch_to_pch(sys_modem_as_id_e_type wrrc_as_id,rrc_cmd_type  *rrc_cmd_ptr)
{
  rrc_csp_freq_scan_e_type  type_freq_scan = RRC_CSP_NO_SCAN;
  boolean  redirection_barred = FALSE;
   ...
   */
  rrctmr_start_timer(wrrc_as_id, RRCTMR_DCH_PCH_CELL_SELECTION_TIMER, 
  ...
  {
    /* Acquire lock with ACQ_INV as we want to prioritize state transition scans from lower priority activities */
    lock_status = rrc_wrm_acquire_lock_on_redirected_freq(wrrc_as_id,rrc_csp_int_data.dest_freq,TRM_ACQUISITION_INV, RRCTMR_DCH_PCH_CELL_SELECTION_TIMER_IN_MS);

    if(lock_status != RRC_WRM_LOCK_ACQUIRED)
    {
      rrctmr_stop_timer(wrrc_as_id,RRCTMR_DCH_PCH_CELL_SELECTION_TIMER);
      next_substate = rrccsp_send_cell_update_initiate_req(wrrc_as_id, RRC_REENTERED_SERVICE_AREA);
      return next_substate;
    }
  }
  ...
  {
    rrc_start_lock_wait_timer(wrrc_as_id,RRCTMR_DCH_PCH_CELL_SELECTION_TIMER_IN_MS - 10,TRUE);
    next_substate = rrc_csp_wait_for_wrm_lock(wrrc_as_id,&type_freq_scan);
    if(type_freq_scan == RRC_CSP_NO_SCAN)
    {
      return next_substate;
...
===========================================================================*/
rrc_csp_substate_e_type
rrc_csp_start_sel_from_fach_to_fach(sys_modem_as_id_e_type wrrc_as_id,rrc_cmd_type  *rrc_cmd_ptr)
{
  rrc_csp_freq_scan_e_type  type_freq_scan = RRC_CSP_NO_SCAN;
  boolean  redirection_barred = FALSE;
#ifdef FEATURE_DUAL_SIM
  ...
  rrctmr_start_timer(wrrc_as_id,RRCTMR_FACH_TO_FACH_PCH_CELL_SELECTION_TIMER, 
  ...
  {
    /* Acquire lock with ACQ_INV as we want to prioritize state transition scans from lower priority activities */
    lock_status = rrc_wrm_acquire_lock_on_redirected_freq(wrrc_as_id,rrc_csp_int_data.dest_freq,TRM_ACQUISITION_INV, RRCTMR_FACH_TO_FACH_CELL_SELECTION_TIMER_IN_MS);

    if(lock_status != RRC_WRM_LOCK_ACQUIRED)
    {
      rrctmr_stop_timer(wrrc_as_id,RRCTMR_FACH_TO_FACH_PCH_CELL_SELECTION_TIMER);
      next_substate = rrccsp_send_cell_update_initiate_req(wrrc_as_id, RRC_REENTERED_SERVICE_AREA);
      return next_substate;
    }
  }
  ...
  {
    rrc_start_lock_wait_timer(wrrc_as_id,RRCTMR_FACH_TO_FACH_CELL_SELECTION_TIMER_IN_MS - 10,TRUE);
    next_substate = rrc_csp_wait_for_wrm_lock(wrrc_as_id,&type_freq_scan);
    if(type_freq_scan == RRC_CSP_NO_SCAN)
    {
      return next_substate;
...
===========================================================================*/
rrc_csp_substate_e_type
rrc_csp_start_sel_from_fach_to_pch(sys_modem_as_id_e_type wrrc_as_id,rrc_cmd_type  *rrc_cmd_ptr)
{
  rrc_csp_freq_scan_e_type  type_freq_scan = RRC_CSP_NO_SCAN;
  boolean  redirection_barred = FALSE;
#ifdef FEATURE_DUAL_SIM
  ...
  {
    /* Acquire lock with ACQ_INV as we want to prioritize state transition scans from lower priority activities */
    lock_status = rrc_wrm_acquire_lock_on_redirected_freq(wrrc_as_id,rrc_csp_int_data.dest_freq,TRM_ACQUISITION_INV, RRCTMR_FACH_TO_PCH_CELL_SELECTION_TIMER_IN_MS);
  ...
  {
    rrc_start_lock_wait_timer(wrrc_as_id,RRCTMR_FACH_TO_PCH_CELL_SELECTION_TIMER_IN_MS - 10,TRUE);
    next_substate = rrc_csp_wait_for_wrm_lock(wrrc_as_id,&type_freq_scan);
    if(type_freq_scan == RRC_CSP_NO_SCAN)
    {
      return next_substate;
...
===========================================================================*/
rrc_csp_substate_e_type
rrc_csp_start_sel_leaving_conn_mode(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_csp_freq_scan_e_type  type_freq_scan;
  ...
  /* Trigger FR2LTE only if CS RAB has been established and then released or SS Request triggered to RRC when UE moves
   * from LTE to WCDMA due to CSFB call/SS and Fallback is not active with W-pref
   */
#ifdef FEATURE_WCDMA_FAST_RETURN_TO_LTE_AFTER_CSFB
  if(((rrc_csp_int_data.csfb_extended_status == CSFB_CS_RB_RELEASED) || 
    ((rrc_csp_int_data.csfb_extended_status == CSFB_STARTED) && ((rrc_csp_int_data.cs_rab_established) || ((mm_per_subs_is_ss_in_progress(wrrc_as_id)) && (rrcnv_data.rrc_fast_return_to_lte_after_csfb_ss_nv)))))
      &&(!(rrccsp_block_frlte_for_curr_camped_plmn(wrrc_as_id)))
      && rrcmcm_is_lte_mode_enabled(wrrc_as_id)
#ifdef FEATURE_WRLF_SYSTEM_SEL
      && (!RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS())
#endif
#ifdef FEATURE_DUAL_SIM
      &&(rrc_dsds_int_data.fallback_type != TRM_FALLBACK_UMTS_PREF)
#endif
     )
  {
    if((rrc_csp_int_data.abnormal_call_ended || rrcmisc_is_emergency_call_ongoing(wrrc_as_id)) && rrcnv_data.rrc_delay_return_to_lte_nv.nv_enabled)
    {
      rrcmisc_int_data.is_emergency_call = FALSE;
      rrc_csp_int_data.csfb_extended_status = CSFB_INVALID;
      WRRC_MSG1_HIGH("FRLTE_DELAY_ENH:CSFB call ended abnormally,ignoring Fast Return to LTE and start resele delay timer, current lte resel flag %d ", rrc_csp_int_data.allow_lte_resel);
      if((rrctmr_get_remaining_time(wrrc_as_id, RRCTMR_DELAY_WTOL_RESEL_TIMER))   == 0)
      {
        rrctmr_start_timer(wrrc_as_id, RRCTMR_DELAY_WTOL_RESEL_TIMER, 1000*rrcnv_data.rrc_delay_return_to_lte_nv.timer_val);
      }
    }
    else
    {
...
#endif
#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
  if((rrc_pseudo_meas_triggered_conn_abort_req(wrrc_as_id) == TRUE)
      &&
      rrcmcm_is_lte_mode_enabled(wrrc_as_id)
#ifdef FEATURE_WRLF_SYSTEM_SEL
      && (!RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS())
     ...
     )
  {
    ...
    WRRC_MSG0_HIGH("W_TO_PSEUDO_MEAS:Suspend WCDMA and start WTOL Redirection");
    rrc_csp_int_data.saved_rrc_cmd.cmd.redirect_req.proc_id = RRC_PROCEDURE_CSP;
    rrc_csp_int_data.interrat_type = RRC_CSP_INTERRAT_LTE;

    rrc_pseudo_meas_fetch_redirection_results(wrrc_as_id,&rrc_csp_int_data.saved_rrc_cmd.cmd.redirect_req);
    rrc_csp_int_data.curr_scan = RRC_CSP_INTER_RAT_REDIRECTION_SCAN;
    /* Send change mode request to MCM */
    rrccsp_send_mode_change_req_to_mcm(wrrc_as_id, RRC_MODE_STANDBY);
    next_substate = RRCCSP_WT_SUSPEND_MODE_CHANGE_CNF;
  ...
  WRRC_MSG3_HIGH("Leaving Conn mode. nw_sel_mode %d, rrc_state %d, srch_st %d", 
                                      rrc_csp_int_data.bplmn_nw_sel_mode,
                                      rrc_get_state(wrrc_as_id), rrc_csp_int_data.bplmn_srch_vars_status);
  ...
  if(!rrc_csp_int_data.bplmn_guard_srch_tmr_expired && (!rrc_csp_int_data.bplmn_data_saved) && (rrc_csp_int_data.bplmn_srch_vars_status != WTOW_SEARCH_START))
  {
    ...
    rrc_csp_int_data.bplmn_data_saved = TRUE;
    rrc_csp_int_data.bplmn_srch_status_svd = rrc_csp_int_data.bplmn_new_srch;
    rrc_csp_cleanup_needed_bplmn_data(wrrc_as_id);
  }
  ...
  if(rrc_csp_int_data.curr_scan == RRC_CSP_OUT_OF_SERVICE_AREA_SCAN)
  {
    /*Call the NAS API*/
#ifdef FEATURE_DUAL_SIM
     mm_registration_in_progress = mm_per_subs_rrc_registration_in_progress(rrc_get_as_id(wrrc_as_id));
#else
    ...
    if(mm_registration_in_progress)
    {
       ...
       * NO SERVICE to MM.
       */
      next_substate = rrccsp_rel_chls_and_go_to_idle(wrrc_as_id);;
      return next_substate;
    }
  }
  ...
  if((!rrcrcr_is_ifreq_rdr_in_prgrs(wrrc_as_id)) && (!rrc_csp_int_data.attempt_freq_only_acq)) 
  {
    rrc_csp_get_aset_scr_pn(wrrc_as_id);
  }
...
===========================================================================*/
rrc_csp_substate_e_type
rrc_csp_start_sel_for_plmn_list(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_csp_freq_scan_e_type  type_freq_scan;
  rrc_csp_substate_e_type   next_substate = RRCCSP_NO_CELL_SELECTED;
...
#ifdef FEATURE_DUAL_SIM
  /* For foreground manual search use ACQ_INV always */
  rrc_csp_int_data.rrc_use_high_priority_for_bplmn = TRUE;
#endif

  if(rrc_csp_int_data.network_select_mode == SYS_NETWORK_SELECTION_MODE_MANUAL)
  {
    WRRC_MSG1_HIGH("BST band pref incl %d",rrc_csp_int_data.rrc_bst_band_pref_incl);
    if(rrc_csp_int_data.rrc_bst_band_pref_incl)
    {
      /* Update GSM with UE's band preference */
      rr_rrc_set_band_pref_internal(wrrc_as_id,rrc_csp_int_data.rat_pri_band_pref);

      /* Update GSM with BST band preference */
      rr_rrc_set_bst_band_pref_internal(wrrc_as_id,rrc_csp_int_data.bplmn_band_pref);  
    }
    else
    {
      /* Update GSM with UE's band preference */
      rr_rrc_set_band_pref_internal(wrrc_as_id,rrc_csp_int_data.bplmn_band_pref);
  
      /* Update GSM with BST band preference */
      rr_rrc_set_bst_band_pref_internal(wrrc_as_id, 0);
    }
  }
  else
  {
  ...
  rrccsp_generate_rat_order_for_manual_plmn_srch(wrrc_as_id);
  ...
  else
  {
    ...
    rrc_start_lock_wait_timer(wrrc_as_id,RRCTMR_DEFAULT_SEARCH_TIME_IN_MS,TRUE);
    rrc_dsds_int_data.shld_acq_be_spltd = TRUE;
    if((rrc_csp_wait_for_wrm_lock(wrrc_as_id,&type_freq_scan) == RRCCSP_NO_CELL_SELECTED) && (type_freq_scan == RRC_CSP_NO_SCAN))
    {
      return (RRCCSP_NO_CELL_SELECTED);
    }
#endif
...
===========================================================================*/
void rrccsp_update_plmn_list(sys_modem_as_id_e_type wrrc_as_id,rrc_plmn_identity_type plmn_id, rrc_int_cell_id_type cell_id)
{
  /* Temporary storage for LAI status */
  rrc_csp_lai_status_e_type  lai_status;
  ...
  #error code not present
#else
  rrcff_l1_get_rssi(wrrc_as_id,&l1_rssi);
  signal_strength = l1_rssi.rssi_value;
#endif
#endif
  ...
  if(rrc_csp_int_data.curr_scan == RRC_CSP_CELL_SELECTION_SCAN)
  {
    ...
    if(!rrc_is_s_criteria_met(wrrc_as_id,common_plmn_id,cell_id))
    {
  ...
  if((rrc_csp_int_data.curr_scan == RRC_CSP_OUT_OF_SERVICE_AREA_SCAN) ||
      ((rrc_csp_int_data.curr_scan == RRC_CSP_LEAVING_CONN_MODE_SCAN) && 
       (rrc_csp_int_data.prev_curr_scan == RRC_CSP_OUT_OF_SERVICE_AREA_SCAN)))
    {
      if((!rrc_csp_int_data.oosc_plmn_found) && (rrc_is_s_criteria_met(wrrc_as_id,plmn_id,cell_id)))
      {
        ...
        rrc_csp_send_svc_status_to_mm(wrrc_as_id,SYS_SRV_STATUS_LIMITED, SYS_RAT_UMTS_RADIO_ACCESS,signal_strength);
      }
      else
      {
  ...
  if(get_sib1_ptr == TRUE)
  {
    ...
    if(rrc_csp_int_data.curr_scan == RRC_CSP_PLMN_LIST_SCAN)
    {
      sib1_ptr = (rrc_SysInfoType1 *) rrc_bplmn_return_sib_for_cell(wrrc_as_id,
                                             rrc_SIB1);
    }
    else
    {
    ...
    {
      /* SIB3 for this PLMN, CEll ID combination not present */
      WRRC_MSG3_ERROR("SIB1 for MCC %d, MNC %d, Cell ID %d not present",
                        RRC_CSP_GET_INT_MCC(common_plmn_id), 
                        RRC_CSP_GET_INT_MNC(common_plmn_id),
                        cell_id);
       ...
       && is_current_cell_nw_shared)
  {
    for(i=0; i< rrc_csp_int_data.curr_nw_db_entry.num_of_shared_plmns; i++)
    {
      ...
      if( rrc_bcch_and_nas_plmns_match(wrrc_as_id,
    ...
    do
    {
          ...
          (rrc_csp_int_data.curr_req_plmn.plmn_type != RRC_GSM_MAP_ANY_PLMN_REQ))
      {
        if( rrc_bcch_and_nas_plmns_match(wrrc_as_id,
              plmn_id, rrc_csp_int_data.curr_req_plmn.plmn_id))
        {
          WRRC_MSG0_HIGH("Not adding the PLMN to list as it is the requested PLMN by NAS");
          continue;
        }
      }
      ...
      if(rrc_csp_int_data.plmn_list.num_plmns < RRC_MAX_PLMNS)
      {
        ...
        if(rrc_csp_int_data.curr_scan == RRC_CSP_PLMN_LIST_SCAN)
        {
          WRRC_MSG1_MED("WTOW: SIB1/3 not read. Setting defaults. Scan = %d",rrc_csp_int_data.curr_scan);
        ...
        if(rrc_csp_find_cell_bar_status(wrrc_as_id,plmn_id, 
                                        cell_id, 
                                        &cell_bar_status
                                        ,common_plmn_id
                                      ) 
...
===========================================================================*/
void rrccsp_update_background_plmn_list(sys_modem_as_id_e_type wrrc_as_id,rrc_plmn_identity_type plmn_id, rrc_int_cell_id_type cell_id)
{
  /* Temporary storage for LAI status */
  rrc_csp_lai_status_e_type  lai_status;
  ...
  do
  {
    ...
    if(rrc_csp_int_data.bplmn_fnd_list_wr_index < SYS_PLMN_LIST_MAX_LENGTH)
    {
      ...
      else if(rrc_csp_int_data.curr_scan == RRC_CSP_GTOW_BPLMN_AUTOMATIC_SCAN)
      {
        ...
        {
          if(rrc_bcch_and_nas_plmns_match(wrrc_as_id,rrc_csp_int_data.curr_acq_entry.plmn_id,
...
===========================================================================*/
void rrccsp_update_wtow_background_plmn_list(sys_modem_as_id_e_type wrrc_as_id,
  rrc_plmn_identity_type plmn_id, rrc_int_cell_id_type cell_id)
{
  ...
  l1_get_rssi_during_wtow_bplmn(&l1_rssi,wrrc_as_id);
  signal_strength = l1_rssi.rssi_value;
  WRRC_MSG1_HIGH("RSSI Signal Strength is %d",signal_strength);
...
===========================================================================*/
void rrccsp_update_wtow_manual_bplmn_list_with_rplmn(sys_modem_as_id_e_type wrrc_as_id,rrc_plmn_identity_type plmn_id, 
                                                     rrc_int_cell_id_type cell_id)
{
  uint32         num_plmns;
  uint32         i;
  ...
  rrc_csp_band_class_type csp_band_class = rrc_get_frequency_band(wrrc_as_id,rrc_ccm_get_curr_camped_freq(wrrc_as_id),
                                                                  rrc_ccm_get_curr_camped_plmn(wrrc_as_id));

#ifdef FEATURE_FEMTO_CSG
/* For manual CSG searches add current camped PLMN info, only if serving cell is a CSG*/
  if((rrc_csp_int_data.service_search_bgnd == SYS_SERVICE_SEARCH_CSG)&&(rrc_ccm_get_curr_camped_csg_id(wrrc_as_id) == SYS_CSG_ID_INVALID))
  {
  ...
  if(rrc_csp_int_data.freq_scan_list.priority_info != SYS_SRCH_PRIORITY_NORMAL)
  {
    WRRC_MSG0_HIGH("Inquisitive search do not add current rgstrd PLMN info");
    return;
  }
  ...
  WRRC_MSG1_MED("RSSI Signal Strength is %d",signal_strength);
    ...
    WRRC_MSG3_HIGH("Adding RPLMN %d-%d to the list RSSI Signal Strength is %d", RRC_CSP_GET_INT_MCC(plmn_id),
                                            RRC_CSP_GET_INT_MNC(plmn_id), signal_strength);
...
rrc_csp_substate_e_type
rrccsp_continue_bplmn_in_frgrnd(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  if(rrc_csp_int_data.bplmn_srch_vars_status == WTOW_SEARCH_SUSPEND)
  {
    ...
    if(!rrc_csp_int_data.bplmn_guard_srch_tmr_expired)
    {
      rrctmr_stop_timer(wrrc_as_id, RRCTMR_BPLMN_GUARD_SRCH_TIMER);
    ...
    WRRC_MSG1_HIGH("WTOG: Updating GSM with band preferences for PLMN search,BST band pref incl %d",rrc_csp_int_data.rrc_bst_band_pref_incl);
    if(rrc_csp_int_data.rrc_bst_band_pref_incl)
    {
      /* Update GSM with UE's band preference */
      rr_rrc_set_band_pref_internal(wrrc_as_id,rrc_csp_int_data.rat_pri_band_pref);
    ...
    if(!rrc_csp_wtog_bplmn_int_data.wtog_bplmn_srch_active)
    {
      ...
      next_substate = rrc_csp_start_bplmn_srch(wrrc_as_id);
    }
    ...
    else
    {
      ...
      /* Send idle request to L1 followed by suspend request */
      rrc_csp_send_l1_idle_req(wrrc_as_id);
      next_substate = rrc_csp_resume_wtog_bplmn_srch(wrrc_as_id);
    }
  }
  else
  {
    ...
    rrccsp_reset_bplmn_list_search_state(wrrc_as_id,WTOW_SEARCH_STOP, FALSE, FALSE);
    rrc_csp_int_data.bplmn_trans_id= trans_id;
    rrc_csp_int_data.service_search_bgnd = srvc_srch_bgnd;
    next_substate = rrc_csp_start_sel_for_plmn_list(wrrc_as_id);
  }
...
===========================================================================*/
boolean  rrccsp_is_state_transition_triggered_by_reconfig_msg(sys_modem_as_id_e_type wrrc_as_id)
{
...
===========================================================================*/
boolean  rrccsp_is_selected_cell_utran_directed(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  if(rrc_csp_int_data.dest_freq_present)
  {
    if(rrc_ccm_get_curr_camped_freq(wrrc_as_id) == rrc_csp_int_data.dest_freq)
    {
      if(rrc_csp_int_data.dest_psc_present)
      {
        if(rrc_ccm_get_curr_camped_psc(wrrc_as_id) == rrc_csp_int_data.dest_psc)
        {
          return(TRUE);
        }
      }
...
===========================================================================*/
void  
rrc_csp_send_mm_act_ind(sys_modem_as_id_e_type wrrc_as_id)
{
  mm_cmd_type *mm_buf_ptr;
#ifdef FEATURE_FEMTO_CSG
  ...
  if(rrc_csp_int_data.curr_scan == RRC_CSP_GSM_TO_WCDMA_REDIRECTION_SCAN)
  {
    ...
    rrcmcm_update_wcdma_activation_cause(wrrc_as_id, RRCMCM_INTERRAT_CELL_REDIRECTION);
  }
  else
  {
    mm_buf_ptr->cmd.rrc_activation_ind.status = MM_AS_INTERRAT_RESELECTION;
    /*update the cause for activation indication*/
    rrcmcm_update_wcdma_activation_cause(wrrc_as_id, RRCMCM_INTERRAT_CELL_RESELECTION);
       ...
       (rrc_csp_int_data.interrat_type == RRC_CSP_INTERRAT_LTE))
    {
       ...
       resel_rsp.w_cell_info.frequency      = rrc_ccm_get_curr_camped_freq(wrrc_as_id);
       resel_rsp.w_cell_info.phy_cell_id    = rrc_ccm_get_curr_camped_psc(wrrc_as_id);
       resel_rsp.w_cell_info.cell_identity  = rrc_ccm_get_curr_camped_cell_id(wrrc_as_id);
       resel_rsp.w_cell_info.plmn           = mmumts_convert_rrc_plmn_id_to_nas_plmn_id(rrc_ccm_get_curr_camped_plmn(wrrc_as_id));
       resel_rsp.w_cell_info.csg_id         = rrc_ccm_get_curr_camped_csg_id(wrrc_as_id);
       ...
       rrc_send_lte_cmd(wrrc_as_id,&resel_rsp.msg_hdr,WCDMA_RRC_LTE_RESEL_RSP,sizeof(lte_rrc_wcdma_resel_rsp_s));
...
#ifdef FEATURE_DUAL_SIM
  mm_buf_ptr->cmd.rrc_activation_ind.as_id = rrc_get_as_id(wrrc_as_id);
#endif
...
===========================================================================*/
void rrccsp_fill_info_for_rr(sys_modem_as_id_e_type wrrc_as_id,sys_network_selection_mode_e_type *network_select_mode)
{
...
===========================================================================*/
void rrccsp_set_ue_in_deep_sleep(sys_modem_as_id_e_type wrrc_as_id)
{
...
===========================================================================*/
void rrccsp_update_band_preference(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_csp_int_data.wcdma_band_mask = 0;
  ...
  {
    WRRC_MSG0_MED("IMT2k band supported by the UE");
  ...
  {
    WRRC_MSG0_MED("1900 band supported by the UE");
    rrc_csp_int_data.wcdma_band_mask = rrc_csp_int_data.wcdma_band_mask |
                                       RRC_CSP_PCS_BAND;
  }
  ...
  {
    WRRC_MSG0_MED("850 band supported by the UE");
    rrc_csp_int_data.wcdma_band_mask = rrc_csp_int_data.wcdma_band_mask |
  ...
  {
    WRRC_MSG0_MED("800 MHz band supported by the UE");
    rrc_csp_int_data.wcdma_band_mask = rrc_csp_int_data.wcdma_band_mask |
  ...
  {
    WRRC_MSG0_MED("1800 MHz band supported by the UE");
    rrc_csp_int_data.wcdma_band_mask = rrc_csp_int_data.wcdma_band_mask |
  ...
  {
    WRRC_MSG0_MED("900 MHz band supported by the UE");
    rrc_csp_int_data.wcdma_band_mask = rrc_csp_int_data.wcdma_band_mask |
  ...
  {
    WRRC_MSG0_MED("1700 MHz band supported by the UE");
    rrc_csp_int_data.wcdma_band_mask = rrc_csp_int_data.wcdma_band_mask |
  ...
  {
    WRRC_MSG0_MED("1700_2100 (AWS) MHz band supported by the UE");
    rrc_csp_int_data.wcdma_band_mask = rrc_csp_int_data.wcdma_band_mask |
  ...
  if(rrc_csp_int_data.band_pref & SYS_BAND_MASK_WCDMA_XI_1500)
  {
    WRRC_MSG0_MED("1500 MHz band supported by the UE");
    rrc_csp_int_data.wcdma_band_mask = rrc_csp_int_data.wcdma_band_mask |
  ...
  {
    WRRC_MSG0_MED("BC19: band supported by the UE");
    rrc_csp_int_data.wcdma_band_mask = rrc_csp_int_data.wcdma_band_mask |
   ...
   */
  rrccsp_update_band_class_in_acq_db(wrrc_as_id);
#endif
...
===========================================================================*/
boolean rrccsp_is_band_class_enabled_in_ui_rf(sys_modem_as_id_e_type wrrc_as_id,rrc_csp_band_class_type band_class,
                                              boolean update_wcdma_band_mask)
{
   ...
   */
  #ifdef FEATURE_DUAL_SIM
  ui_rf_band_capability = (sd_ss_band_pref_e_type) sd_misc_get_subs_supp_band_pref(rrc_get_as_id(wrrc_as_id));
  #else
  ui_rf_band_capability = (sd_ss_band_pref_e_type) sd_misc_get_supp_band_pref();
  #endif
  /*WRRC_MSG3_HIGH("UI_RF: ui_rf_band_capability = %d, band_class %d, update_w_bm %d", 
                                                                ui_rf_band_capability, 
                                                                band_class,
                                                                update_wcdma_band_mask);*/
  ...
  if(band_class == RRC_CSP_PCS_BAND)
  {
    if(ui_rf_band_capability & SD_SS_BAND_PREF_WCDMA_II_PCS_1900)
    {
      ...
      {
        WRRC_MSG1_MED("UI_RF: Add it to the wcdma_band_mask %d", rrc_csp_int_data.wcdma_band_mask);
    ...
    {
      WRRC_MSG1_MED("UI_RF: 1900 band NOT supported by the UI/RF. wcdma_band_mask %d", 
  ...
  else if(band_class == RRC_CSP_IMT2K_BAND)
  {
    ...
    {
      WRRC_MSG1_MED("UI_RF: IMT2k band NOT supported by the UI/RF. wcdma_band_mask %d", 
                                            rrc_csp_int_data.wcdma_band_mask);
    }
  }
  ...
  else if(band_class == RRC_CSP_WCDMA_800_BAND)
  {
    ...
    {
      WRRC_MSG1_MED("UI_RF: 800 band NOT supported by the UI/RF. wcdma_band_mask %d", 
                                            rrc_csp_int_data.wcdma_band_mask);
    }
  }
  ...
  else if(band_class == RRC_CSP_WCDMA_850_BAND)
  {
    ...
    {
      WRRC_MSG1_MED("UI_RF: 850 band NOT supported by the UI/RF. wcdma_band_mask %d", 
                                            rrc_csp_int_data.wcdma_band_mask);
    }
  }
  ...
  else if(band_class == RRC_CSP_WCDMA_1800_BAND)
  {
    ...
    {
      WRRC_MSG1_MED("UI_RF: 1800 band NOT supported by the UI/RF. wcdma_band_mask %d", 
                                            rrc_csp_int_data.wcdma_band_mask);
    }
  }
  ...
  else if(band_class == RRC_CSP_WCDMA_900_BAND)
  {
    ...
    {
      WRRC_MSG1_MED("UI_RF: 900 band NOT supported by the UI/RF. wcdma_band_mask %d", 
                                            rrc_csp_int_data.wcdma_band_mask);
    }
  }
  ...
  else if(band_class == RRC_CSP_WCDMA_1700_BAND)
  {
    ...
    {
      WRRC_MSG1_MED("UI_RF: 1700 band NOT supported by the UI/RF. wcdma_band_mask %d", 
                                            rrc_csp_int_data.wcdma_band_mask);
    }
  }
  ...
  else if(band_class == RRC_CSP_WCDMA_1700_2100_BAND)
  {
    ...
    {
      WRRC_MSG1_MED("UI_RF: 1700_2100 band NOT supported by the UI/RF. wcdma_band_mask %d", 
                                            rrc_csp_int_data.wcdma_band_mask);
    }
  }
  ...
  else if(band_class == RRC_CSP_WCDMA_1500_BAND)
  {
    ...
    {
      WRRC_MSG1_MED("UI_RF: 1500 band NOT supported by the UI/RF. wcdma_band_mask %d", 
                                            rrc_csp_int_data.wcdma_band_mask);
    }
  }
  ...
  else if(band_class == RRC_CSP_WCDMA_BC19_BAND)
  {
    ...
    {
      WRRC_MSG1_MED("UI_RF: BC19: band NOT supported by the UI/RF. wcdma_band_mask %d", 
                                            rrc_csp_int_data.wcdma_band_mask);
    }
  }
  ...
  if(update_wcdma_band_mask && status)
  {
     ...
     * 800 as well as 850 MHz bands.
     */
    WRRC_MSG1_HIGH("UI_RF: Updated WCDMA bandmask %d",rrc_csp_int_data.wcdma_band_mask);   
    rrccsp_update_band_class_in_acq_db(wrrc_as_id);
#endif
  }
...
boolean rrccsp_check_gsm_bands_supported(sys_modem_as_id_e_type wrrc_as_id)
{
  sd_ss_band_pref_e_type band_capability;
  
  #ifdef FEATURE_DUAL_SIM
  band_capability = sd_misc_get_subs_supp_band_pref(rrc_get_as_id(wrrc_as_id));
  #else
...
===========================================================================*/
uecomdef_status_e_type  rrccsp_update_acq_db(sys_modem_as_id_e_type wrrc_as_id,rrc_freq_type freq)
{
  ...
  if(rrc_csp_add_acq_entry(wrrc_as_id,&temp_acq_entry,TRUE) == SUCCESS)
  {
...
DESCRIPTION
  ...
  This sub specific function is called by RR to update the band preference
  in RRC.
...
rrc_update_band_preference_sub_id(band_pref_mask, SYS_MODEM_AS_ID_1);
...
DESCRIPTION
  ...
  This sub specific function is called by RR to validate the WCDMA ARFCN it
...
) 
{
  ...
  if(validate_dl_arfcn(wrrc_as_id, UARFCN, 
...
{
  /*RRC_VALIDATE_FUNC_CALL();*/
  return (rrc_is_arfcn_valid_sub_id(UARFCN, rr_plmn_id, SYS_MODEM_AS_ID_1));
} /* rrc_is_arfcn_valid */
#else
/*===========================================================================
...
DESCRIPTION
  ...
  This sub specific function is called by RR to inform RRC that UE is going to DEEP SLEEP
  state
...
===========================================================================*/
void rrc_set_ue_in_deep_sleep_sub_id( boolean value,sys_modem_as_id_e_type wrrc_as_id)
{
  RRC_VALIDATE_SUB_ID(wrrc_as_id);
#ifdef FEATURE_DUAL_SIM
  if (wrrc_as_id != rrc_get_as_id(wrrc_as_id))
...
{
  /*RRC_VALIDATE_FUNC_CALL();*/
  rrc_set_ue_in_deep_sleep_sub_id(value, SYS_MODEM_AS_ID_1);
} /* rrc_set_ue_in_deep_sleep */


/*===========================================================================

FUNCTION RRC_SET_UE_IN_DEEP_SLEEP_SUB

DESCRIPTION

  This function is called by CM to inform RRC that UE is going to DEEP SLEEP
  state.
...
===========================================================================*/
void rrc_set_ue_in_deep_sleep_sub(sys_modem_as_id_e_type wrrc_as_id, boolean value )
{
  RRC_VALIDATE_SUB_ID(wrrc_as_id);
  WRRC_MSG1_HIGH("Deep sleep indicated by CM %d",value);
  rrc_csp_int_data.ue_in_deep_sleep = value;
} /* rrc_set_ue_in_deep_sleep */
...
===========================================================================*/
boolean rrccsp_is_in_bplmn_list 
(
  sys_modem_as_id_e_type wrrc_as_id,
  rrc_plmn_identity_type plmn_id,
  rrc_MasterInformationBlock *mib_ptr
                                 ) 
{
  ...
  WRRC_MSG3_HIGH("BPLMN: SIB querying if MCC %d, MNC %d on freq %d is in BPLMN list",
                            RRC_CSP_GET_INT_MCC(plmn_id), RRC_CSP_GET_INT_MNC(plmn_id), 
                            rrc_csp_int_data.curr_acq_entry.freq);
   ...
   */
  rrc_csp_add_freq_to_available_freq_list(wrrc_as_id,rrc_csp_int_data.curr_acq_entry.freq);
    ...
    )
  {
    ...
    {
      WRRC_MSG1_HIGH("SSB: %d, Read SIBs for ALL PLMNs",rrc_csp_int_data.service_search_bgnd);
    ...
    if(rrc_is_nw_shared(wrrc_as_id,mib_ptr))
    {
      ...
      rrccsp_find_shared_plmns(wrrc_as_id,
                               mib_ptr,
                               plmn_id,
                               &bplmn_nw_db_entry
                               );
    ...
    if(plmns_shared == FALSE)
    {
      if(rrc_csp_int_data.curr_scan == RRC_CSP_GTOW_BPLMN_AUTOMATIC_SCAN)
      {
        for(i = 0; i < rrc_csp_int_data.bplmn_num_req; ++i)
        {
          if(rrc_bcch_and_nas_plmns_match(wrrc_as_id,plmn_id, rrc_csp_int_data.bplmn_req_list[i].plmn_id))
          {
      ...
      else if(rrc_csp_int_data.curr_scan == RRC_CSP_WTOW_BPLMN_AUTOMATIC_SCAN)
      {
        for(i = 0; i < rrc_csp_int_data.wtow_bplmn_num_req; ++i)
        {
          if(rrc_bcch_and_nas_plmns_match(wrrc_as_id,plmn_id, rrc_csp_int_data.wtow_bplmn_req_list[i].plmn_id))
          {
            WRRC_MSG0_HIGH("BPLMN:SIB proc -> plmn is in BPLMN list");
            rrc_free(bplmn_acq_entry);
            return TRUE;
    ...
    {
      if(rrccsp_is_any_of_bcch_plmns_in_bplmn_list(wrrc_as_id,bplmn_nw_db_entry))
      {
        rrc_free(bplmn_acq_entry);
        return TRUE;
      }
...
#ifdef FEATURE_WCDMA_MULTIBAND
  bplmn_acq_entry->band_class  = rrc_get_frequency_band(wrrc_as_id,
                                                  rrc_csp_int_data.curr_acq_entry.freq,
                                                  plmn_id);
...
#endif
  rrccsp_add_acq_db_and_nw_db_entries(wrrc_as_id,bplmn_acq_entry,&bplmn_nw_db_entry);
...
===========================================================================*/
boolean rrccsp_is_sib_mcc_and_850_800_current_band_comb_valid(sys_modem_as_id_e_type wrrc_as_id,rrc_plmn_identity_type plmn_id) 
{
  WRRC_MSG3_HIGH("MCC_850_800: plmn_mcc=%d, current_band=%d, current_freq=%d",
                                     RRC_CSP_GET_INT_MCC(plmn_id),
                                     rrc_csp_curr_select_data.current_band, 
                                     rrc_csp_int_data.curr_acq_entry.freq);
  ...
  if((((RRC_CSP_GET_INT_MCC(plmn_id) == JAPAN_MCC1)
       ...
       && (!rrccsp_is_current_band(wrrc_as_id,RRC_CSP_WCDMA_800_BAND,plmn_id)))
     ||(((RRC_CSP_GET_INT_MCC(plmn_id) != JAPAN_MCC1)
       && (RRC_CSP_GET_INT_MCC(plmn_id) != JAPAN_MCC2)
       && (RRC_CSP_GET_INT_MCC(plmn_id) != JAPAN_MCC3)
       && (RRC_CSP_GET_INT_MCC(plmn_id) != JAPAN_MCC4))
       && (!rrccsp_is_current_band(wrrc_as_id,RRC_CSP_WCDMA_850_BAND,plmn_id))))
  {
...
===========================================================================*/
void rrc_csp_init_inter_freq_redirection_data(sys_modem_as_id_e_type wrrc_as_id)
{       
  /* Save the last camped frequency. This frequency will be specifically
   * scanned during the full frequency scan
   */
  ...
  rrccsp_update_acq_type_for_em_call_ifreq_redirection(wrrc_as_id, RRC_CSP_ANY_PLMN);
#endif
...
===========================================================================*/
boolean rrccsp_is_ue_in_true_limited_service(sys_modem_as_id_e_type wrrc_as_id)
{
  if(rrc_csp_int_data.network_select_mode == SYS_NETWORK_SELECTION_MODE_LIMITED)
  {
    return TRUE;
...
===========================================================================*/
void rrc_csp_cleanup_inter_freq_redirection_state(sys_modem_as_id_e_type wrrc_as_id)
{   
  /* Stop the wait timer if it was active */
  if(!rrc_csp_int_data.start_acq_on_last_camped_freq)
  {
    rrctmr_stop_timer(wrrc_as_id, RRCTMR_FREQ_REDIRECT_WAIT_TIMER);
   ...
   */
  rrccsp_update_acq_type_for_em_call_ifreq_redirection(wrrc_as_id, RRC_CSP_SPEC_PLMN);
#endif
...
DESCRIPTION:
  This sub specific function is called by RR to check if a particular WCDMA cell is 
  barred or not
...
(
  uint16 freq, uint16 scr_code, sys_modem_as_id_e_type wrrc_as_id
)
{
  ...
  switch(wl1_check_cell_barred(freq, scr_code,wrrc_as_id))
  {
...
{
/*RRC_VALIDATE_FUNC_CALL();*/
return (rrc_rr_check_cell_barred_sub_id( freq, scr_code, SYS_MODEM_AS_ID_1));
...
===========================================================================*/
void rrc_csp_send_cell_param_req_for_pch_fach(sys_modem_as_id_e_type wrrc_as_id)
{   
  ...
  WRRC_MSG2_MED("Sib_idx PLMN: MCC %d-MNC %d",RRC_CSP_GET_INT_MCC(common_plmn_id),RRC_CSP_GET_INT_MNC(common_plmn_id));
  /* PLMN and Cell ID of the current camped on cell are known. Get SIB3 */
  sib3_ptr = (rrc_SysInfoType3 *) rrc_sibdb_return_sib_for_cell(wrrc_as_id,
                                        rrc_SIB3, 
                                        common_plmn_id, 
  ...
  if( sib3_ptr == NULL )
  {
    /* SIB3 for this PLMN, Cell ID combination not present. 
     */
    WRRC_MSG3_MED("SIB3 for MCC %d, MNC %d, Cell ID %d not present", 
  ...
  if(sib3_ptr->sib4indicator == TRUE)
  {
     sib4_ptr = (rrc_SysInfoType4*) rrc_sibdb_return_sib_for_srv_cell(wrrc_as_id,
                                    rrc_SIB4); 
  }
    ...
    cellSelectReselectInfoPCHFACH_v5b0extPresent))
  {
    ...
    rrc_csp_send_l1_cmd(wrrc_as_id,RRC_PROCEDURE_CSP, CPHY_CELL_PARM_REQ,
                        &rrc_csp_int_data.curr_acq_entry);
...
SIDE EFFECTS:
  None
====================================================================*/
void rrccsp_send_conn_mode_oos_ds_req (sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_cmd_type *cmd_ptr;

  /* Get a RRC internal command buffer */
  cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id);
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrccsp_send_conn_mode_oos_ds_wakeup_req (sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_cmd_type *cmd_ptr;
  ...
  lock_req_bands.bands[0].band = rrc_return_default_band_lock_req(wrrc_as_id);  
#endif

#ifndef FEATURE_TRIPLE_SIM
#ifdef FEATURE_DUAL_SIM
  if(rrc_block_wait_to_acquire_lock_for_time(wrrc_as_id,300, TRM_ACQUISITION, lock_req_bands, FALSE) == FAILURE)
  {
    return FAILURE;
  }
#endif
#else
  if(rrc_block_wait_to_acquire_lock_for_time(wrrc_as_id,200, TRM_ACQUISITION, lock_req_bands, FALSE ) == FAILURE)
  {
    return FAILURE;
  }
#endif
...
SIDE EFFECTS      None
===========================================================================*/
sys_band_mask_e_type rrc_convert_band_to_sys_band(sys_modem_as_id_e_type wrrc_as_id,rrc_csp_band_class_type band)
{
  ...
  WRRC_MSG2_MED("band %d, sys_band %d", band, ret_sys_band);
  return(ret_sys_band);
...
SIDE EFFECTS      None
===========================================================================*/
sys_band_class_e_type rrc_convert_band_to_sys_band_class(sys_modem_as_id_e_type wrrc_as_id,rrc_csp_band_class_type band)
{
...
SIDE EFFECTS      None
===========================================================================*/
rrc_csp_band_class_type rrc_convert_sys_band_class_to_rrc_band(sys_modem_as_id_e_type wrrc_as_id,sys_band_class_e_type band_class)
{
...
SIDE EFFECTS:
         None.
====================================================================*/
boolean rrccsp_skip_sibs_for_fplmn(sys_modem_as_id_e_type wrrc_as_id,
  rrc_plmn_identity_type plmn_id
  , rrc_MasterInformationBlock *mib_ptr
  )
{
  ...
  WRRC_MSG3_MED("FPLMN: curr_type_acq=%d, curr_scan=%d, n/w_sel_mode=%d",
                                                rrc_csp_int_data.curr_type_acq,
                                                rrc_csp_int_data.curr_scan,
     ...
     )
  {
    ...
    do
    {
      ...
      {
        WRRC_MSG0_MED("FPLMN: BCCH PLMN is not forbidden ");
    ...
    */
    ...
    rrccsp_add_plmn_acq_db_and_nw_db_entries(wrrc_as_id, plmn_id, fplmn_nw_db_entry, is_valid_nw_sharing);
    WRRC_MSG0_MED("FPLMN: BCCH PLMN is forbidden PLMN");
    rrc_free(fplmn_nw_db_entry);
    return TRUE;
  }
  rrc_free(fplmn_nw_db_entry);
...
SIDE EFFECTS:
         None.
====================================================================*/
boolean rrccsp_skip_sibs_for_vplmn(sys_modem_as_id_e_type wrrc_as_id,
  rrc_plmn_identity_type plmn_id
  , rrc_MasterInformationBlock *mib_ptr
  )
{
  ...
  if(((rrc_csp_int_data.network_select_mode == SYS_NETWORK_SELECTION_MODE_AUTOMATIC) ||
      (rrc_csp_int_data.network_select_mode == SYS_NETWORK_SELECTION_MODE_MANUAL)) &&
      (rrc_csp_int_data.curr_type_acq == RRC_CSP_SPEC_PLMN) &&
      (rrccsp_check_plmn_match(wrrc_as_id,plmn_id,mib_ptr, FALSE) == FALSE) &&
  ...
  if(TRUE == ret_val)
  { /* Add the PLMN to ACQ DB */
    rrccsp_add_plmn_acq_db_and_nw_db_entries(wrrc_as_id, plmn_id, vplmn_nw_db_entry, is_valid_nw_sharing);
  }
  rrc_free(vplmn_nw_db_entry);
  return ret_val;
}
...
SIDE EFFECTS      None
===========================================================================*/
boolean rrccsp_do_gsm_bands_scan_first(sys_modem_as_id_e_type wrrc_as_id)
{
  if(rrccsp_is_supported_band(wrrc_as_id,RRC_CSP_PCS_BAND) 
     || rrccsp_is_supported_band(wrrc_as_id,RRC_CSP_WCDMA_850_BAND)
     || rrccsp_is_supported_band(wrrc_as_id,RRC_CSP_WCDMA_900_BAND)
    )
  {
    return TRUE;
  }
...
DESCRIPTION:
          This sub specific function is called by GSM-RR to get WCDMA channel info. GSM-RR
...
SIDE EFFECTS: NONE
=============================================================================*/
void rr_get_rrc_acq_chnl_info_sub_id(rrc_rr_wcdma_acq_chnl_info_type *acq_chnl_info,sys_modem_as_id_e_type wrrc_as_id)
{
  uint32 i = 0;
  RRC_VALIDATE_SUB_ID(wrrc_as_id);
#ifdef FEATURE_DUAL_SIM
  ...
  for(i = 0; i < rrc_csp_curr_select_data.num_available_freq; ++i)
  {
    ...
    acq_chnl_info->chnl_info[i].band = rrc_convert_band_to_sys_band(wrrc_as_id,
                                        rrc_csp_curr_select_data.available_freq_list_band[i]);

    WRRC_MSG3_HIGH("[%d] Freq %d & Band %d added to acq chnl info", i,
...
{
/*RRC_VALIDATE_FUNC_CALL();*/
  rr_get_rrc_acq_chnl_info_sub_id(acq_chnl_info, SYS_MODEM_AS_ID_1);
}
...
DESCRIPTION:
          This sub specific function is called by both RRC and GSM-RR to clear
...
SIDE EFFECTS: NONE
=============================================================================*/
void rrc_clear_acq_chnl_info_sub_id(sys_modem_as_id_e_type wrrc_as_id)
{
  RRC_VALIDATE_SUB_ID(wrrc_as_id);
#ifdef FEATURE_DUAL_SIM
  if (wrrc_as_id != rrc_get_as_id(wrrc_as_id))
...
{
  /*RRC_VALIDATE_FUNC_CALL();*/
  rrc_clear_acq_chnl_info_sub_id(SYS_MODEM_AS_ID_1);
}
#endif /* FEATURE_UMTS_ACQ_CHNL_INFO_SHARING */
...
SIDE EFFECTS      None
===========================================================================*/
boolean rrccsp_is_primary_plmn_id_present(sys_modem_as_id_e_type wrrc_as_id)
{
  boolean status = FALSE;
...
SIDE EFFECTS      None
===========================================================================*/
boolean rrccsp_is_camped_cell_network_shared(sys_modem_as_id_e_type wrrc_as_id)
{
  boolean status = FALSE;
  rrc_MasterInformationBlock *mib;
  mib = (rrc_MasterInformationBlock*)rrc_sibdb_return_sib_for_srv_cell(wrrc_as_id, rrc_MIB);

  if(rrc_csp_int_data.primary_plmn_Id_present && (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH))
  {
  ...
  if(mib != NULL)
  {
    if(rrc_is_nw_shared(wrrc_as_id,mib))
    {
      status = TRUE;
    }
    else
...
SIDE EFFECTS      None
===========================================================================*/
boolean rrccsp_is_mib_plmn_valid(sys_modem_as_id_e_type wrrc_as_id,rrc_MasterInformationBlock *mib_ptr)
{
...
SIDE EFFECTS      None
===========================================================================*/
uint8 rrccsp_construct_multiple_plmn_list_from_mib(sys_modem_as_id_e_type wrrc_as_id,
                                                   rrc_MasterInformationBlock *mib_ptr,
                                                   rrc_plmn_identity_type *shared_PLMN_list)
{
  ...
  {
    rrc_translate_gsm_map_plmn_id_with_optional_mcc(wrrc_as_id,&is_mcc_valid, &(multiple_PLMN_List.multiplePLMNs.elem[n_index]), &plmn_id);
...
SIDE EFFECTS      None
===========================================================================*/
static void rrccsp_print_shared_PLMNs(sys_modem_as_id_e_type wrrc_as_id,int number_of_PLMNs, rrc_plmn_identity_type *shared_PLMN_list)
{
  int     i;
...
SIDE EFFECTS      None
===========================================================================*/
void rrccsp_prioritize_the_list_of_shared_plmns(sys_modem_as_id_e_type wrrc_as_id)
{
  int i =0 ;
  ...
  WRRC_MSG3_HIGH("NWS: Current Scan: %d, PLMN tobe prioritized: MCC:MNC - %d:%d",
                                                                      rrc_csp_int_data.curr_scan,
     ...
     (rrc_csp_int_data.curr_scan == RRC_CSP_CELL_SELECTION_SCAN))
  {
    ...
    {
      if(rrc_is_bcch_plmn_an_hplmn_or_ehplmn(wrrc_as_id,bcch_plmn_list->shared_plmns_access_info[i].plmn_id))
      {
  ...
  for(i = 0; i < bcch_plmn_list->num_of_shared_plmns; i++)
  {
    if(rrc_bcch_and_nas_plmns_match(wrrc_as_id,bcch_plmn_list->shared_plmns_access_info[i].plmn_id,
...
SIDE EFFECTS      None
===========================================================================*/
void rrccsp_find_shared_plmns(sys_modem_as_id_e_type wrrc_as_id,
                              rrc_MasterInformationBlock* mib_ptr,
                              rrc_plmn_identity_type plmn_id,
                              ...
                              )
{
  ...
  num_of_shared_plmns = rrccsp_construct_multiple_plmn_list_from_mib(wrrc_as_id,
                                                                     mib_ptr,
                                                                     shared_plmn_list);

  rrccsp_print_shared_PLMNs(wrrc_as_id,num_of_shared_plmns,shared_plmn_list);
...
)
{
      ...
      if(rrc_bcch_and_nas_plmns_match(wrrc_as_id,acq_entry_ptr->plmn_id, reference_plmn))
      {
        WRRC_MSG0_MED("PLMN is the same as requested BCCH and NAS PLMN match");
      ...
      {
        WRRC_MSG0_MED("CSG: Skip equivalent PLMNs check");
        cell_plmn_status = RRC_CSP_PLMN_DIFF_THAN_REQ;
      }
#endif
      else
      {
        if((rrccsp_check_for_eplmn(wrrc_as_id,
                  mmumts_convert_rrc_plmn_id_to_nas_plmn_id(
                    reference_plmn))) == TRUE)
        {
          WRRC_MSG0_MED("NAS PLMN is in the eq PLMN list");
          if((rrccsp_check_for_eplmn(wrrc_as_id,
                    mmumts_convert_rrc_plmn_id_to_nas_plmn_id(
                     acq_entry_ptr->plmn_id))) == TRUE)
          {
            WRRC_MSG0_MED("BCCH PLMN is in the eq PLMN list");
            cell_plmn_status = RRC_CSP_PLMN_SAME_AS_REQ;
          }
          else
          {
            WRRC_MSG0_MED("BCCH PLMN not in eq PLMN list");
            cell_plmn_status = RRC_CSP_PLMN_DIFF_THAN_REQ;
          }
        }
        else
        {
          WRRC_MSG0_MED("NAS PLMN not in eq PLMN list");
          cell_plmn_status = RRC_CSP_PLMN_DIFF_THAN_REQ;
        }
      }
         ...
         (rrc_csp_int_data.curr_scan == RRC_CSP_CELL_SELECTION_SCAN))
      {
        if(rrc_is_bcch_plmn_an_hplmn_or_ehplmn(wrrc_as_id,acq_entry_ptr->plmn_id))
        {
          WRRC_MSG0_MED("BCCH PLMN is in the HPLMN/EHPLMN list ");
          cell_plmn_status = RRC_CSP_PLMN_SAME_AS_REQ;
        }
      }
...
SIDE EFFECTS
===========================================================================*/
boolean rrccsp_check_plmn_match(sys_modem_as_id_e_type wrrc_as_id, rrc_plmn_identity_type plmn_id
  , rrc_MasterInformationBlock *mib_ptr, boolean update_acq_db)
{
   rrc_csp_plmn_status_e_type cell_plmn_status;
    ...
    do
    {
      ...
      {
        cell_plmn_status = rrccsp_get_cell_plmn_status(wrrc_as_id,acq_entry, rrc_csp_int_data.curr_req_plmn.plmn_id);
        if(cell_plmn_status == RRC_CSP_PLMN_SAME_AS_REQ)
        {
          plmns_match = TRUE;
        }
  ...
  {
    rrccsp_add_plmn_acq_db_and_nw_db_entries(wrrc_as_id, acq_entry->plmn_id, nw_db_entry, acq_entry->is_valid_nw_sharing);
  }
  rrc_free(acq_entry);
...
SIDE EFFECTS      None
===========================================================================*/
void rrccsp_reset_num_tries_dest_freq(sys_modem_as_id_e_type wrrc_as_id)
{
...
SIDE EFFECTS      None
===========================================================================*/
rrc_csp_cell_actual_vs_expected_e_type rrccsp_get_cell_status(sys_modem_as_id_e_type wrrc_as_id,
                                                              rrc_csp_plmn_status_e_type* cell_plmn_status,
                                                              rrc_csp_acq_entry_type* acq_entry_ptr
                                                              ,rrc_csp_nw_sharing_entry_type* nw_db_entry_ptr
                                                              )
  ...
  do
  {
    ...
    {
      *cell_plmn_status = rrccsp_get_cell_plmn_status(wrrc_as_id,acq_entry_ptr, rrc_csp_int_data.curr_req_plmn.plmn_id);
      if(*cell_plmn_status == RRC_CSP_PLMN_SAME_AS_REQ)
      {
        plmns_match = TRUE;
      }
      ...
      if(RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS())
      {
        cell_plmn_status_wrt_rlf_plmn = rrccsp_get_cell_plmn_status(wrrc_as_id, acq_entry_ptr, rrc_csp_int_data.rlf_plmn_id);
      }
#endif
    } /* end PLMN request for a specific PLMN */
    ...
    if(RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS())
    {
      cell_status = rrc_csp_compare_expected_cell_for_conn_oos(wrrc_as_id,*cell_plmn_status,
...
#endif
    {
    cell_status = rrc_csp_compare_expected_cell(wrrc_as_id,*cell_plmn_status,
                                         acq_entry_ptr,
                                         rrc_csp_int_data.curr_type_acq);
    }
...
SIDE EFFECTS      None
===========================================================================*/
rrc_csp_cell_actual_vs_expected_e_type rrccsp_get_cell_status_during_bplmn_search(sys_modem_as_id_e_type wrrc_as_id,
                                                                                  rrc_csp_acq_entry_type* acq_entry_ptr
                                                                                  ,rrc_csp_nw_sharing_entry_type* nw_db_entry_ptr
                                                                                  )
{
  ...
  do
  {
    ...
    if(rrc_csp_int_data.service_search_bgnd != SYS_SERVICE_SEARCH_BACKGROUND_MANUAL)
    {
      if(!wtow_search)
      {
        rrccsp_update_background_plmn_list(wrrc_as_id,acq_entry_ptr->plmn_id, acq_entry_ptr->cell_id);
      }
      else
      {
        rrccsp_update_wtow_background_plmn_list(wrrc_as_id,acq_entry_ptr->plmn_id, acq_entry_ptr->cell_id);
      }
    }
    ...
    {
      if(rrc_bcch_and_nas_plmns_match(wrrc_as_id,acq_entry_ptr->plmn_id, hplmn_id))
      {
      ...
      else
      {
        ...
        else
        {
          ...
          if(rrc_csp_int_data.curr_scan == RRC_CSP_GTOW_BPLMN_AUTOMATIC_SCAN)
          {
            ...
            {
              if(rrc_bcch_and_nas_plmns_match(wrrc_as_id,acq_entry_ptr->plmn_id,
...
SIDE EFFECTS      None
===========================================================================*/
void rrc_csp_add_freq_to_available_freq_list(sys_modem_as_id_e_type wrrc_as_id,rrc_freq_type freq)
{
  uint32 i;
  ...
  {
    WRRC_MSG1_MED("freq %d already in list. Not adding",freq);
  }
  ...
  if(rrc_csp_curr_select_data.acq_db_in_use)
  {
    ...
    curr_band = rrc_get_frequency_band(wrrc_as_id,freq,plmn_id);
  }
  else
  {
  ...
  detected_wcdma_frequency.frequency = rrccsp_derive_freq_from_uarfcn(wrrc_as_id,freq,curr_band);
  detected_wcdma_frequency.bandwidth = 50;/*50*100KHz*/
  detected_wcdma_frequency.rat = SYS_RAT_UMTS_RADIO_ACCESS;
  rrc_csp_add_frequency_to_detected_cell_list(wrrc_as_id,&detected_wcdma_frequency);
#endif
...
SIDE EFFECTS      None
===========================================================================*/
boolean rrccsp_does_mcc_belong_to_US(sys_modem_as_id_e_type wrrc_as_id,rrc_plmn_identity_type plmn_id)
{
...
boolean rrc_csp_ignore_barred_rsv_status_due_to_ueut(sys_modem_as_id_e_type wrrc_as_id,rrc_plmn_identity_type plmn_id)
{
  rrc_plmn_identity_type hplmn_id;
  ...
  if(rrcnv_data.rrc_ignore_cell_bar_nv == FALSE)
  {
    WRRC_MSG1_HIGH(" Ignore Cell Bar Staus is %d so return",rrcnv_data.rrc_ignore_cell_bar_nv);
    return FALSE;
  }
#ifdef FEATURE_DUAL_SIM
...
boolean rrc_csp_ignore_barred_rsv_status_due_to_ueut(sys_modem_as_id_e_type wrrc_as_id,rrc_plmn_identity_type plmn_id)
{
  ...
  if(rrcdata_sim_related_info.rrc_rcvd_admin_data != 4)
  {
    WRRC_MSG1_HIGH("Received admin data in SIM Insterted Req is %d",rrcdata_sim_related_info.rrc_rcvd_admin_data);
    return FALSE;
  }
...
SIDE EFFECTS: NONE
=============================================================================*/
void rrc_csp_send_svc_status_to_mm(sys_modem_as_id_e_type wrrc_as_id,sys_srv_status_e_type svc_st, 
                                   ...
                                   sys_signal_strength_type sig_st )
{
...
#endif
  {
    ...
    if((rrc_dsds_int_data.oos_due_to_no_resource == TRUE)&&(svc_st == SYS_SRV_STATUS_NO_SRV))
    {
       return;
    }
    /* Get a buffer from MM task */
...
#ifdef FEATURE_DUAL_SIM
    mm_buf_ptr->cmd.rrc_conn_mode_service_status_ind.as_id = rrc_get_as_id(wrrc_as_id);
#endif
    ...
    WRRC_MSG1_HIGH("OOSC:Connected mode OOS is sent as %d to MM/ Dispatching out MM cmd RRC_CONNECTED_MODE_SERVICE_STATUS_IND",
      ...
      WRRC_MSG3_HIGH("SVC_ST: Svc st %d, plmn mcc %d, mnc %d",mm_buf_ptr->cmd.rrc_conn_mode_service_status_ind.service_status,
                                           RRC_CSP_GET_INT_MCC(rrc_csp_int_data.curr_acq_entry.plmn_id), 
                                           RRC_CSP_GET_INT_MNC(rrc_csp_int_data.curr_acq_entry.plmn_id));
  ...
  {
    /* With WRLF feature, NAS will take care of updating UI during connected mode OOS except for emergency calls */
    return;
  }	
#endif
...
===========================================================================*/
boolean 
rrccsp_is_band_already_added_to_prioritized_list(sys_modem_as_id_e_type wrrc_as_id,rrc_csp_band_class_type freq_band, uint8 num_of_bands)
{
  uint8 i;
...
SIDE EFFECTS      None.
===========================================================================*/
rrc_freq_type rrccsp_get_serving_frequency(sys_modem_as_id_e_type wrrc_as_id)
{
...
SIDE EFFECTS      None.
===========================================================================*/
rrc_freq_type rrccsp_get_neighbor_frequency(sys_modem_as_id_e_type wrrc_as_id)
{
...
SIDE EFFECTS      None.
===========================================================================*/
rrc_plmn_identity_type rrccsp_get_curr_nas_requested_plmn(sys_modem_as_id_e_type wrrc_as_id)
{
  if(WTOW_SEARCH_RESUME == rrc_csp_bplmn_srch_in_progress(wrrc_as_id))
  {
...
SIDE EFFECTS      None.
===========================================================================*/
rrc_int_csg_id_type rrccsp_get_curr_nas_requested_csg_id(sys_modem_as_id_e_type wrrc_as_id)
{
...
{
  rrc_state_e_type curr_rrc_state = rrc_get_state(wrrc_as_id);
  if((rrc_csp_int_data.curr_scan == RRC_CSP_OUT_OF_SERVICE_AREA_SCAN) &&
...
{
  rrc_csp_substate_e_type  curr_substate = rrc_get_csp_substate(wrrc_as_id);
  ...
  if (NULL != directed_cell_info)
  {
     ...
     ))
    {
      ...
      WRRC_MSG3_HIGH("Redirection info frequency %d ,PSC present %d and PSC is %d ",rrc_csp_int_data.dest_freq,
...
)
{
  ...
  if(rrc_csp_bplmn_srch_in_progress(wrrc_as_id) == WTOW_SEARCH_RESUME)
  {
...
FUNCTION          rrc_csp_save_freq_list_from_nas

DESCRIPTION       This function extracts frequencies from NAS
...
SIDE EFFECTS      None
===========================================================================*/
void rrc_csp_save_freq_list_from_nas
(
  sys_modem_as_id_e_type wrrc_as_id, 
  sys_arfcn_list_s_type *freq_scan_list,
  sys_preferred_plmn_list_s_type *preferred_plmn_list,
...
)
{
  ...
  if(freq_scan_list->wcdma_count > 0)
  {
    rrc_csp_int_data.mplmn_wcdma_freq_list_present = TRUE;
  }
  if(preferred_plmn_list->count > 0)
  {
    rrc_csp_save_preferred_plmn_list(wrrc_as_id,preferred_plmn_list);
  }
  WRRC_MSG3_HIGH("MPLMN Periodic scan search: W_freq_list_count:%d Pref_plmn_list_count:%d priority:%d",freq_scan_list->wcdma_count,preferred_plmn_list->count,rrc_csp_int_data.freq_scan_list.priority_info);

  for(rat_idx =0;rat_idx < rat_pri_list->num_items && rat_idx< SYS_MAX_ACQ_PREF_RAT_LIST_NUM ; rat_idx++)
  {
    if(rat_pri_list->priority_list_info[rat_idx].acq_sys_mode == SYS_SYS_MODE_WCDMA)
    {
       for(w_idx =0; w_idx < freq_scan_list->wcdma_count && w_idx < MAX_WCDMA_ARFCN_LIST_COUNT ; w_idx ++)
       {
         rrc_csp_int_data.freq_scan_list.wcdma_freq_list[w_idx] = freq_scan_list->wcdma_arfcn[w_idx];
         WRRC_MSG1_HIGH("MPLMN:: W freq %d",rrc_csp_int_data.freq_scan_list.wcdma_freq_list[w_idx]);
       }
    }
    if(rat_pri_list->priority_list_info[rat_idx].acq_sys_mode == SYS_SYS_MODE_LTE)
    {
      for(l_idx =0; l_idx < freq_scan_list->lte_count && l_idx < MAX_LTE_ARFCN_LIST_COUNT ; l_idx ++)
      {
        rrc_csp_int_data.freq_scan_list.lte_freq_list[l_idx] = freq_scan_list->lte_arfcn[l_idx];
        WRRC_MSG1_HIGH("MPLMN:: L  freq %d",rrc_csp_int_data.freq_scan_list.lte_freq_list[l_idx]);
      }
    }
  }
  rrc_csp_int_data.freq_scan_list.total_wcdma_freq = w_idx;
  rrc_csp_int_data.freq_scan_list.total_lte_freq = l_idx;
}
#ifdef FEATURE_RAT_PRIORITY_LIST
/*===========================================================================
...
)
{
  ...
  wcdma_complete_band_mask = rrccsp_get_sys_wcdma_band_mask(wrrc_as_id);
...
)
{
    ...
    rrc_csp_int_data.band_pref           = rrc_extract_gw_band_pref_from_rat_pri_list(wrrc_as_id,&rrc_csp_int_data.rat_pri_list_info);
    rrccsp_update_band_preference(wrrc_as_id);
    rr_rrc_set_band_pref_internal(wrrc_as_id,rrc_csp_int_data.band_pref);
    rrcmcm_update_mode_capability(wrrc_as_id,&rrc_csp_int_data.rat_pri_list_info);
}
...
{
  rrc_csp_int_data.bplmn_lte_mode_enabled = rrcmcm_is_mode_enabled_in_rat_pri_list(wrrc_as_id,rat_pri_list,SYS_SYS_MODE_LTE);
  rrc_csp_int_data.bplmn_lte_band_pref = rrc_extract_lte_band_pref_from_rat_pri_list(wrrc_as_id,rat_pri_list);
}
#ifdef LTE_BAND_NUM
/*===========================================================================
...
SIDE EFFECTS      None.
===========================================================================*/
boolean rrccsp_check_if_lte_band_pref_is_set(sys_modem_as_id_e_type wrrc_as_id, sys_lte_band_mask_e_type lte_band_mask)
{
...
===========================================================================*/
#ifdef LTE_BAND_NUM
sys_lte_band_mask_e_type rrccsp_get_lte_band_pref(sys_modem_as_id_e_type wrrc_as_id)
#else
sys_band_mask_type rrccsp_get_lte_band_pref(sys_modem_as_id_e_type wrrc_as_id)
#endif

{
  return (rrc_extract_lte_band_pref_from_rat_pri_list(wrrc_as_id,&rrc_csp_int_data.rat_pri_list_info));
}
...
)
{
  ...
  sys_band_mask_type gw_band_pref = rrc_extract_gw_band_pref_from_rat_pri_list(wrrc_as_id,&rrc_csp_int_data.rat_pri_list_info);
...
)
{
  ...
  if((!frequency_found) && (rrc_csp_int_data.detected_frequency_list.num_freqs < LTE_IRAT_MAX_DETECTED_FREQ))
  {
    ...
    WRRC_MSG2_MED("Added absolute frequency %d @ idx %d to detected frequency list",detected_frequency->frequency,idx);
  }
}
...
)
{
  ...
  {
    rrc_csp_add_frequency_to_detected_cell_list(wrrc_as_id,&detected_frequency_list->freq[idx]);
  }
  return;
}
...
)
{
  ...
  rrc_csp_print_rat_search_order(wrrc_as_id);
}
...
#ifdef FEATURE_PCS_850_FREQ_SCAN_NEW_OPTIMIZATION
  do_gsm_bands_scan_first = rrccsp_do_gsm_bands_scan_first(wrrc_as_id);
#endif
...
)
{
  ...
  {
     rrccsp_generate_rat_order_for_auto_plmn_srch(wrrc_as_id);
  }
  if(rrc_csp_int_data.plmn_srch_order.num_rats !=0)
  {
    ...
    if(rrc_csp_int_data.plmn_srch_order.rat[0] == SYS_RAT_UMTS_RADIO_ACCESS)
    {
      WRRC_MSG0_MED("Do WTOW search first");
      rrc_csp_wtog_bplmn_int_data.wtog_bplmn_srch_active = FALSE;
    }
#ifdef FEATURE_WCDMA_TO_LTE
    else if(rrc_csp_int_data.plmn_srch_order.rat[0] == SYS_RAT_LTE_RADIO_ACCESS)
    {
      WRRC_MSG0_MED("Do WTOL search first");
      rrc_csp_wtog_bplmn_int_data.wtog_bplmn_srch_active = TRUE;
      rrc_csp_int_data.bplmn_interrat_type = RRC_CSP_INTERRAT_LTE;
    }
#endif
    ...
    {
      WRRC_MSG0_MED("Do WTOG search first");
      rrc_csp_wtog_bplmn_int_data.wtog_bplmn_srch_active = TRUE;
#ifdef FEATURE_WCDMA_TO_LTE
      rrc_csp_int_data.bplmn_interrat_type = RRC_CSP_INTERRAT_GSM;
#endif
...
{
  uint8 rat_idx;
  WRRC_MSG4_HIGH("RAT Search order for PLMN List Req. Number of RATs: %d, Index-RAT: 0 - %d, 1 - %d, 2 - %d",
                  rrc_csp_int_data.plmn_srch_order.num_rats,
                  rrc_csp_int_data.plmn_srch_order.rat[0],
                  rrc_csp_int_data.plmn_srch_order.rat[1],
                  rrc_csp_int_data.plmn_srch_order.rat[2]
                  );
}
...
SIDE EFFECTS
                  None
===========================================================================*/
sys_band_mask_type wcdma_rrc_get_band_from_dl_uarfcn_sub
(
  sys_modem_as_id_e_type wrrc_as_id,
  uint16 uarfcn,
...
)
{
  ...
  RRC_VALIDATE_SUB_ID(wrrc_as_id);
  csp_band_class = 
  rrc_get_frequency_band(wrrc_as_id,
  ...
  {
    *is_band_valid = TRUE;
    return (rrc_convert_band_to_sys_band(wrrc_as_id,csp_band_class));
  }
...
{
  /*RRC_VALIDATE_FUNC_CALL();*/
  return wcdma_rrc_get_band_from_dl_uarfcn_sub(SYS_MODEM_AS_ID_1, uarfcn, camped_plmn_id, is_band_valid);
}
#endif

/*===========================================================================

FUNCTION          WCDMA_CONVERT_UARFCN_SUB
...
SIDE EFFECTS
                  None
===========================================================================*/
boolean wcdma_convert_uarfcn_sub
(
  sys_modem_as_id_e_type wrrc_as_id,
...
{
  rrc_csp_band_class_type source_band_class = rrc_get_frequency_band(wrrc_as_id,(rrc_freq_type)source_uarfcn,
                                              mmumts_convert_nas_plmn_id_to_rrc_plmn_id(camped_plmn_id));
  ...
  if(rrccsp_get_uarfcn(wrrc_as_id,source_uarfcn, source_band_class, target_band_class, target_uarfcn) == TRUE)
  {
    return TRUE;
  } 
  return FALSE; 
...
{
  /*RRC_VALIDATE_FUNC_CALL();*/
  return wcdma_convert_uarfcn_sub(SYS_MODEM_AS_ID_1, source_uarfcn, band_num, target_uarfcn, camped_plmn_id);
}
/*===========================================================================

FUNCTION          WCDMA_DERIVE_FREQ_FROM_UARFCN_SUB
...
SIDE EFFECTS
                  None
===========================================================================*/
errno_enum_type wcdma_derive_freq_from_uarfcn_sub
(
  sys_modem_as_id_e_type wrrc_as_id,
  uint16 uarfcn,
  sys_plmn_id_s_type camped_plmn_id,
...
{
  rrc_csp_band_class_type csp_band_class = rrc_get_frequency_band(wrrc_as_id,(rrc_freq_type)uarfcn,
                                              mmumts_convert_nas_plmn_id_to_rrc_plmn_id(camped_plmn_id));
  RRC_VALIDATE_SUB_ID(wrrc_as_id);
  
  if(RRC_CSP_BAND_MAX != csp_band_class )
  {
    *freq = rrccsp_derive_freq_from_uarfcn(wrrc_as_id,uarfcn,csp_band_class);
    return E_SUCCESS;
  }
  else
  {
...
{
  /*RRC_VALIDATE_FUNC_CALL();*/
  return wcdma_derive_freq_from_uarfcn_sub(SYS_MODEM_AS_ID_1, uarfcn, camped_plmn_id, freq);
}
#ifdef FEATURE_WRLF_SYSTEM_SEL
...
#ifdef FEATURE_DUAL_SIM
    mm_buf_ptr->cmd.rrc_suitable_search_end_ind.as_id = rrc_get_as_id(wrrc_as_id);
#endif
    /* Put the command on MM queue */
...
SIDE EFFECTS
                  None
===========================================================================*/
void rrc_csp_send_disable_bplmn_ind(sys_modem_as_id_e_type wrrc_as_id,boolean status)
{
  mm_cmd_type *mm_buf_ptr;
...
#ifdef FEATURE_DUAL_SIM
  mm_buf_ptr->cmd.rrc_disable_bplmn_ind.as_id = rrc_get_as_id(wrrc_as_id);
#endif 
  /* Put the command on MM queue */
...
{
  if(0 != rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_DSIM_BPLMN_LOCK_REL_OFFSET_TIMER))
  {
    rrctmr_stop_timer(wrrc_as_id, RRCTMR_WTOW_BPLMN_DRX_SRCH_TIMER);
    rrc_csp_int_data.bplmn_srch_tmr_expired = TRUE;
    rrc_csp_send_l1_cmd(wrrc_as_id,RRC_PROCEDURE_CSP, CPHY_GO_TO_SLEEP_IND, 
                                     &rrc_csp_int_data.curr_acq_entry);
    rrctmr_stop_timer(wrrc_as_id, RRCTMR_DSIM_BPLMN_LOCK_REL_OFFSET_TIMER);
  }
}
...
SIDE EFFECTS      None
===========================================================================*/
boolean rrc_is_ps_abort_required(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_state_e_type rrc_state = rrc_get_state(wrrc_as_id);

  if((RRC_MODE_ACTIVE == rrcmcm_get_rrc_mode(wrrc_as_id))
      && rrcscr_get_signalling_connection_sts(wrrc_as_id,RRC_PS_DOMAIN_CN_ID)
...
SIDE EFFECTS      None
===========================================================================*/
void rrc_reset_oos_due_to_no_resource_flag(sys_modem_as_id_e_type wrrc_as_id)
{
  if(rrc_dsds_int_data.oos_due_to_no_resource)
  {
    WRRC_MSG0_HIGH("DSIM: resetting flag oos_due_to_no_resource.");
    rrc_dsds_int_data.oos_due_to_no_resource = FALSE;
...
SIDE EFFECTS      None
===========================================================================*/
void rrc_set_oos_due_to_no_resource_flag(sys_modem_as_id_e_type wrrc_as_id)
{
  WRRC_MSG0_HIGH("DSIM: setting flag oos_due_to_no_resource.");
  rrc_dsds_int_data.oos_due_to_no_resource = TRUE;
...
SIDE EFFECTS      None
===========================================================================*/
boolean rrc_get_oos_due_to_no_resource_flag_status(sys_modem_as_id_e_type wrrc_as_id)
{
  WRRC_MSG1_MED("DSIM: oos_due_to_no_resource.: %d", rrc_dsds_int_data.oos_due_to_no_resource);
  return (rrc_dsds_int_data.oos_due_to_no_resource);
}
#endif
...
SIDE EFFECTS      None
===========================================================================*/
boolean rrccsp_check_sib1_validity(sys_modem_as_id_e_type wrrc_as_id, rrc_SysInfoType1  *sib1)
{
  /* This stores the GSM-MAP NAS system info */
  rrc_gsm_map_nas_sys_info_type   nas_common_info;
  /* This stores the LAI received in SIB1 */
...
boolean rrccsp_channel_locking_nv_status(sys_modem_as_id_e_type wrrc_as_id)
{
  if(rrcnv_data.rrc_nv_channel_lock_item_value !=0)
  {
    WRRC_MSG1_HIGH("NV LOCKING STATUS %d",rrcnv_data.rrc_nv_channel_lock_item_value);
    return TRUE;
  }
  else
  {
...
uint16 rrccsp_get_locking_nv_value(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcnv_data.rrc_nv_channel_lock_item_value;
}
...
SIDE EFFECTS      None
===========================================================================*/
void rrccsp_init_first_band_to_be_scanned(sys_modem_as_id_e_type wrrc_as_id)
{
#ifdef FEATURE_WCDMA_MULTIBAND
...
void rrccsp_set_est_req_for_emerg_call(sys_modem_as_id_e_type wrrc_as_id)
{
...
void rrc_csp_save_barred_freq(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  if(rrc_csp_int_data.curr_scan == RRC_CSP_CELL_SELECTION_SCAN)
  {
    if(rrc_csp_curr_select_data.num_of_barred_freq < RRC_MAX_OVERLAP_FREQ)
    {
      ...
      WRRC_MSG3_HIGH("Adding WCDMA ARFCN freq %d from band %d. Total no of frequencies not to be skipped is %d",
...
boolean rrccsp_get_lte_resel_allow_status(sys_modem_as_id_e_type wrrc_as_id)
{
...
boolean rrccsp_check_if_em_plmn(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  if((rrccsputil_int_data.search_order_specialization  == SYS_SPECIAL_SBM_RAT_SEARCH_ORDER) || (rrccsputil_int_data.search_order_specialization  == SYS_SPECIAL_SBM_PERIODIC_SEARCH))
  {  
...
boolean rrccsp_is_csg_white(sys_modem_as_id_e_type wrrc_as_id,uint32 csg_id, rrc_plmn_identity_type plmn_id)
{
  uint32 i=0;

  for(i=0;i<rrc_csg_int_data.rrc_csg_white_list.length;i++)
  {
    if((csg_id == rrc_csg_int_data.rrc_csg_white_list.csg_list[i].csg_id)&&(sys_plmn_match(mmumts_convert_rrc_plmn_id_to_nas_plmn_id(plmn_id),rrc_csg_int_data.rrc_csg_white_list.csg_list[i].plmn_id)))
      return TRUE;
  }
  return FALSE;
}
...
boolean rrccsp_asf_actively_in_prog(sys_modem_as_id_e_type wrrc_as_id)
{
...
boolean rrccsp_csg_in_whitelist(sys_modem_as_id_e_type wrrc_as_id,uint32 csg_id, rrc_plmn_identity_type plmn_id)
{
  ...
  if((rrccsp_get_curr_nas_requested_csg_id(wrrc_as_id) != SYS_CSG_ID_INVALID)&&
     (rrc_csp_int_data.curr_scan == RRC_CSP_CELL_SELECTION_SCAN)&&
     (rrc_get_csp_substate(wrrc_as_id) == RRCCSP_CELL_SEL_WT_SIB)
#ifdef FEATURE_WRLF_SYSTEM_SEL
     &&(!RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS())
#endif
    )
  ...
  if(rrccsp_is_csg_white(wrrc_as_id,csg_id,plmn_id))
  {
    return TRUE;
  }
  else
...
boolean rrccsp_is_csg_white_empty(sys_modem_as_id_e_type wrrc_as_id)
{
  return (rrc_csg_int_data.rrc_csg_white_list.length);
}
/*===========================================================================
...
void rrccsp_csg_set_fp_info(sys_modem_as_id_e_type wrrc_as_id)
{
  if(rrc_csp_int_data.service_search_bgnd == SYS_SERVICE_SEARCH_CSG)
  {
    if(RRC_CAMPED_ON == rrc_ccm_get_curr_camping_status(wrrc_as_id,RRC_PROCEDURE_CSP))
...
SIDE EFFECTS      None
===========================================================================*/
static void rrccsp_check_cell_bar_status(sys_modem_as_id_e_type wrrc_as_id,rrc_SysInfoType3 *sib3_ptr, 
      rrc_plmn_identity_type  plmn_id, rrc_csp_cell_bar_e_type  *cell_bar)
{
  ...
  {
    /* SIB3 for this PLMN, CEll ID combination not present */
    WRRC_ERR0_FATAL("CSG: SIB3 empty.");
  } /* SIB3 not present */
  if((rrc_csp_int_data.curr_scan != RRC_CSP_WTOW_BPLMN_AUTOMATIC_SCAN)
     && (rrc_csp_int_data.curr_scan != RRC_CSP_WTOW_BPLMN_MANUAL_SCAN)
     && (rrc_csp_int_data.curr_scan != RRC_CSP_GTOW_BPLMN_MANUAL_SCAN)
     ...
     && (rrc_csp_int_data.curr_scan != RRC_CSP_PLMN_LIST_SCAN))
  {
    ...
    if((rrc_get_state(wrrc_as_id) != RRC_STATE_DISCONNECTED) && (rrc_get_state(wrrc_as_id) != RRC_STATE_CONNECTING))
    {
      if(sib3_ptr->sib4indicator == TRUE)
      {
...
void rrccsp_initial_suitability_check(sys_modem_as_id_e_type wrrc_as_id,rrc_MasterInformationBlock *mib_ptr, rrc_SysInfoType3 *sib3_ptr,
                                      rrc_plmn_identity_type mib_plmn_id, 
                                      #ifdef FEATURE_FEMTO_CSG
                                      uint32 csg_id,
                                      #endif
...
//  rrc_csp_plmn_status_e_type  plmn_status;
  ...
  uint8 i;
  rrc_csp_substate_e_type csp_substate;
...
//  plmn_same_or_eq = FALSE;
  ...
  *status = SIB_EVENT_SUCCEEDED;
#ifdef FEATURE_FEMTO_CSG
  csp_substate = rrc_get_csp_substate(wrrc_as_id);

  is_csg_white = rrccsp_csg_in_whitelist(wrrc_as_id,csg_id, mib_plmn_id);
  if(rrccsp_asf_actively_in_prog(wrrc_as_id))
  {
    rrccsp_prune_acqdb_after_SIB(wrrc_as_id,csg_id,is_csg_white);
  }
  ...
  if(csg_id != SYS_CSG_ID_INVALID)
  {
    ...
    if((csp_substate == RRCCSP_SUIT_CELL_RESELECT_WT_SIB)||
    ...
    WRRC_EVENT_REPORT_PAYLOAD(EVENT_WCDMA_RRC_CSG_DETECTED, sizeof(rrclog_wcdma_csg_cell_detected_event_type),
    ...
    if((!is_csg_white) && (CSG_IND_IN_MIB(mib_ptr)))
    {
      if(rrc_csp_int_data.curr_scan == RRC_CSP_CELL_SELECTION_SCAN)
      {
        if(rrc_is_s_criteria_met(wrrc_as_id,mib_plmn_id,rrc_translate_cell_id(wrrc_as_id,&sib3_ptr->cellIdentity)))
        {
          rrc_csp_int_data.plmn_fnd_during_fgnd_svc_srch = TRUE;
        }
      }
      ...
      rrc_csp_int_data.bar_non_white_csg = TRUE;
      rrccsp_send_cphy_cell_bar_req(wrrc_as_id,RRC_CSP_WCDMA_PSC_BARRED, 1280);
    ...
    else if((rrccsp_asf_actively_in_prog(wrrc_as_id)) && (!is_csg_white) && (!CSG_IND_IN_MIB(mib_ptr)) 
        && (!((rrc_ccm_get_curr_camped_freq(wrrc_as_id) == rrccsp_get_serving_frequency(wrrc_as_id)))))
    {
  ...
  if(rrccsp_bar_forbid_cells(wrrc_as_id,sib3_ptr,mib_plmn_id,mib_ptr))
  {
    WRRC_MSG0_HIGH("Barring the F-cell");
    rrc_csp_int_data.bar_for_resel_only = TRUE;
    rrccsp_send_cphy_cell_bar_req(wrrc_as_id,RRC_CSP_WCDMA_PSC_BARRED,300);
    rrc_csp_int_data.bar_for_resel_only = FALSE;
...
/*
  ...
  */
  ...
  do
  {
    ...
    if(rrc_is_bcch_plmn_an_hplmn_or_ehplmn(wrrc_as_id,mib_plmn_id))
    {
      if(!hplmn_cell_bar_status_found)
      {
        rrccsp_check_cell_bar_status(wrrc_as_id, sib3_ptr, mib_plmn_id, &cell_bar_status);
        hplmn_cell_bar_status = cell_bar_status;
        hplmn_cell_bar_status_found = TRUE;
      }
      else
  ...
  if(hplmn_cell_bar_status_found == TRUE)
  {
    ...
    if(cell_bar_status == RRC_CSP_CELL_BAR_INTRA_FREQ)
    {
      ...
      /* Send Cell bar request to L1 */
      rrccsp_send_cphy_cell_bar_req(wrrc_as_id,RRC_CSP_WCDMA_PSC_BARRED, 
...
FUNCTION          rrccsp_bar_forbid_cells

DESCRIPTION       This function will bar the cells whose entries are present
                  in EFS

DEPENDENCIES      None

RETURN VALUE      TRUE if barred
                  FALSE otherwise

SIDE EFFECTS      None
===========================================================================*/

boolean rrccsp_bar_forbid_cells(sys_modem_as_id_e_type wrrc_as_id,rrc_SysInfoType3 *sib3_ptr,
                                      rrc_plmn_identity_type plmn_id, rrc_MasterInformationBlock *mib_ptr)
{

  uint32 cell_id = rrc_translate_cell_id(wrrc_as_id,&sib3_ptr->cellIdentity);
  ...
  do
  {
    ...
    for(i = 0; i < rrcnv_data.num_forbid_cells; i++)
    {
      if(
          (sys_plmn_match(rrcnv_data.rrc_nv_forbid_cells[i].sys_plmn_id,sys_plmn_id)) &&
          (rrcnv_data.rrc_nv_forbid_cells[i].cell_id == cell_id)
        )
      {
        return TRUE;
      }
...
void rrccsp_reset_wait_l1_rel_lock(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_dsds_int_data.csp_waiting_for_wl1_to_release_lock = FALSE;
}
#endif
...
int16 rrccsp_convert_rcsp_meas_to_report(sys_modem_as_id_e_type wrrc_as_id,int16 rscp)
{
  if(rscp < -120)
  {
    return -5;
...
uint8 rrccsp_convert_ecio_meas_to_report(sys_modem_as_id_e_type wrrc_as_id,int16 ecio_2x)
{
  if(ecio_2x >= 0)
  {
    return 49;
...
SIDE EFFECTS      None
===========================================================================*/
uint32 rrc_csp_set_drx_length(sys_modem_as_id_e_type wrrc_as_id,uint32 drx_len)
{

  if(rrcnv_data.rrc_constant_320ms_drx_nv == TRUE)
  {
     WRRC_MSG0_HIGH("rrcnv_data.rrc_constant_320ms_drx_nv is SET");
     if(drx_len % 32 == 0 )
     {
  ...
  else if(rrcnv_data.rrc_half_drx_len_nv == TRUE)
  {
     rrc_state_e_type  rrc_state =  rrc_get_state(wrrc_as_id);
...
SIDE EFFECTS      None
===========================================================================*/
boolean rrccsp_check_redirection_barred(sys_modem_as_id_e_type wrrc_as_id,rrc_cmd_type  *rrc_cmd_ptr)
{
  ...
  if(rrc_cmd_ptr->cmd.initiate_cell_sel_ind.dest_freq_present)
  {
    ...
    l1_bar_status = wl1_check_cell_barred_and_get_bar_time (dest_freq, dest_scr_code, &wcdma_cell_time_to_reselect, &is_non_white_csg,&is_bar_resel,wrrc_as_id);
    
    WRRC_MSG3_HIGH("Checking if redirection to barred Freq/PSC: bar_type %d, l1_bar_status %d., dest_scr_code %d",bar_type, l1_bar_status,dest_scr_code);
...
DESCRIPTION       This function returns search order specialization as indicated by policy man

DEPENDENCIES
                  None.

RETURN VALUE
                  Returns the value of search order specialization

SIDE EFFECTS      None.
===========================================================================*/
sys_specialization_t rrc_csp_get_srch_order_special
(
  sys_modem_as_id_e_type wrrc_as_id
)
{
  return rrccsputil_int_data.search_order_specialization;
}
...
SIDE EFFECTS      None
===========================================================================*/
boolean l1_rrc_check_sfn_det_acq(sys_modem_as_id_e_type wrrc_as_id,uint16 freq, uint16 psc)
{
  uint8 idx = 0;
  ...
  {
    for( ; idx < rrc_SIB_container.num_cont; idx ++)
    {
...
FUNCTION rrc_is_frlte_delay_nv_enabled

DESCRIPTION
  Indicates whether frlte delay nv is enabled.
...
SIDE EFFECTS
  None
===========================================================================*/
boolean rrc_is_delay_return_to_lte_nv_enabled(sys_modem_as_id_e_type wrrc_as_id)
{
  WRRC_MSG1_HIGH("FRLTE_DELAY_ENH:Delay Return to LTE NV enable status %d ", rrcnv_data.rrc_delay_return_to_lte_nv.nv_enabled);
  return (rrcnv_data.rrc_delay_return_to_lte_nv.nv_enabled);
}
/*===========================================================================
...
DESCRIPTION
  ...
  This function is called by SSGCCS to send CELL_BAR_REQ to L1
  so that the corresponding cell or frequency or active set can be barred.
...
===========================================================================*/
void rrccsp_ssgccs_cell_bar_req 
(
  sys_modem_as_id_e_type wrrc_as_id, 
  wcdma_ssgccs_rrc_cell_bar_info_ind_s ssgccs_cell_bar_info
)
{
  rrc_csp_acq_entry_type acq_entry;
  rrc_state_e_type rrc_state = rrc_get_state(wrrc_as_id);
  if(ssgccs_cell_bar_info.ssgccs_bar_info == SSGCCS_WCDMA_PSC_BARRED)
  {
    rrc_csp_int_data.wcdma_cell_time_to_reselect = ssgccs_cell_bar_info.ssgccs_tbarred;
    rrc_csp_int_data.cell_bar_info = RRC_CSP_WCDMA_PSC_BARRED;
    if(ssgccs_cell_bar_info.bar_for_resel_only == TRUE)
    {
      rrc_csp_int_data.bar_for_resel_only = TRUE;
    }
    acq_entry.freq = ssgccs_cell_bar_info.uarfcn;
    acq_entry.scr_code = (uint16)(ssgccs_cell_bar_info.psc << 4);
    WRRC_MSG2_HIGH("SSGCCS: Cell bar request received for PSC %d barring and bar for resel only %d", ssgccs_cell_bar_info.psc,ssgccs_cell_bar_info.bar_for_resel_only);
    rrc_csp_send_l1_cmd(wrrc_as_id,RRC_PROCEDURE_CSP, CPHY_CELL_BAR_REQ, &acq_entry);
    if(ssgccs_cell_bar_info.bar_for_resel_only == TRUE)
    {
      rrc_csp_int_data.bar_for_resel_only = FALSE;
  ...
  else if(ssgccs_cell_bar_info.ssgccs_bar_info == SSGCCS_WCDMA_FREQ_BARRED)
  {
    rrc_csp_int_data.wcdma_cell_time_to_reselect = ssgccs_cell_bar_info.ssgccs_tbarred;
    rrc_csp_int_data.cell_bar_info = RRC_CSP_WCDMA_FREQ_BARRED;
    if(ssgccs_cell_bar_info.bar_for_resel_only == TRUE)
    ...
    acq_entry.freq = ssgccs_cell_bar_info.uarfcn;
    acq_entry.scr_code = (uint16)(ssgccs_cell_bar_info.psc << 4);
    WRRC_MSG2_HIGH("SSGCCS: Cell bar request received for FREQ %d barring and bar for resel only %d", ssgccs_cell_bar_info.uarfcn,ssgccs_cell_bar_info.bar_for_resel_only);
    rrc_csp_send_l1_cmd(wrrc_as_id,RRC_PROCEDURE_CSP, CPHY_CELL_BAR_REQ, &acq_entry);
    if(ssgccs_cell_bar_info.bar_for_resel_only == TRUE)
    {
      rrc_csp_int_data.bar_for_resel_only = FALSE;
  ...
  else if((ssgccs_cell_bar_info.ssgccs_bar_info == SSGCCS_WCDMA_ASET_BARRED) && (rrc_state == RRC_STATE_CELL_DCH))
  {
    WRRC_MSG0_HIGH(" SSGCCS: Cell bar request received for ASET barring in DCH state ");
    rrccsp_send_cphy_cell_bar_req(wrrc_as_id,RRC_CSP_WCDMA_ASET_BARRED,ssgccs_cell_bar_info.ssgccs_tbarred);
  }
  else
  {
    WRRC_MSG0_HIGH(" SSGCCS: Cell bar request not received for fake Cell ");
  }
} /* rrccsp_ssgccs_cell_bar_req */