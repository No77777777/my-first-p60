...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcrbcommon.c#4 $    $DateTime: 2025/09/25 00:41:30 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------  
10/07/24   mahens  Fixed KW errors
09/22/17   sp      Mini dump phase-2 changes
02/21/18   nr      Added code to support LTA during activation time
11/03/27   ak      Made changes to clear toc usage change flag when procedure is cleared
03/10/16   nr      Fixed KW errors
01/06/16   as      Changes are made to send STOP events only if there is change in vocoder type
11/24/15   nr      Made changes to reduce the F3s printed in get_am_index
07/13/15   sa      Made changes for enabling the FET with CS with PS0x0 configuration 
07/02/15   vg      Made changes to remove compilation warnings due to double parentheses -2nd part.
...
01/13/15   aa      Made changes to F3 for fach to DCH scenario in directed cell validation
01/09/15   sp      Made changes to correct typo due to global re-org changes in rrcrb_is_cu_going_to_be_triggered() function
...
rrcrbcommon_ext_data_type rrcrbcommon_ext_data_sim[MAX_RRC_STACKS];
#define rrcrbcommon_ext_data rrcrbcommon_ext_data_sim[WAS_ID_M]
#else
rrcrbcommon_ext_data_type rrcrbcommon_ext_data;
#endif
...
{
  return rrcrbcommon_ext_data.mac_rab_status;
}
/*===========================================================================
...
{
  return rrcrbcommon_ext_data.rrcrb_cs_data_call;
}
/***/ __WCDMA_RRC_CODE_SEGMENT_NON_DEMANDPAGED_END__ /***/
...
)
{
  ...
  { 
   rb_info_reconfig_ptr = &rrc_rb_information_reconfig_list->elem[rb_idx];
    ...
    if (RRC_RB_NOT_PRESENT == validate_rb_id_in_reconfig_list(wrrc_as_id,
        ...
        rrc_RB_InformationReconfig_r6,rb_StopContinue))
    {
      ...
      lc_id = rrclcm_get_ul_rlc_lc_id(wrrc_as_id,
              rb_info_reconfig_ptr->rb_Identity);
      ...
      else if (rrclcm_get_ul_rlc_mode(wrrc_as_id,lc_id, &lc_mode) == RRCLCM_FAILURE)
      {
        msg_not_valid = TRUE;
    ...
    { 
      ...
      {
        /* Validate PDCP info */
        if (rrc_get_rab_type_for_given_rb_id(wrrc_as_id,
...
===========================================================================*/
void rrcrb_send_rb_proc_compl_ind(sys_modem_as_id_e_type wrrc_as_id)
{
...
{
  return (rrcrbe_get_new_crnti_valid(wrrc_as_id) ||
          rrcrbr_get_new_crnti_valid(wrrc_as_id) ||
          rrcrbrc_get_new_crnti_valid(wrrc_as_id) ||
          rrcpcrc_get_new_crnti_valid(wrrc_as_id) ||
          rrctcrc_get_new_crnti_valid(wrrc_as_id) );
}
/*===========================================================================
...
{
  boolean status;
  status = (rrcrbe_is_srns_in_progress(wrrc_as_id) || rrcrbrc_is_srns_in_progress(wrrc_as_id) ||
          rrcpcrc_is_srns_in_progress(wrrc_as_id) || rrctcrc_is_srns_in_progress(wrrc_as_id) ||
          rrcrbr_is_srns_in_progress(wrrc_as_id) || rrccu_is_srns_in_progress(wrrc_as_id) ||
          rrcumi_is_srns_in_progress(wrrc_as_id)
         );
  if(status == TRUE)
  {
...
{
  boolean status;
  status = (   rrcrbe_get_initiate_cu_flag( wrrc_as_id)
          || rrctcrc_get_initiate_cu_flag( wrrc_as_id)
          || rrcpcrc_get_initiate_cu_flag( wrrc_as_id)
          || rrcrbr_get_initiate_cu_flag( wrrc_as_id)
          || rrcrbrc_get_initiate_cu_flag(wrrc_as_id) );
  if(status == TRUE)
  {
...
)
{
  ...
  rrcrbcommon_ext_data.rrcrb_cs_data_call = FALSE;
  ...
  if (go_back_to_old_status) 
  {
    ...
    for (count = 0; count < MAX_RAB_TO_SETUP; count ++) 
    {
          ...
          (rrc_est_rabs.rabs[count].num_rbs_for_rab > 0))
      {
        ...
        {
          if (rrcrbcommon_ext_data.mac_rab_status == NO_RABS) 
          {
            rrcrbcommon_ext_data.mac_rab_status = CS_RABS;
            if (RRCRB_CS_DATA_RAB == rrc_est_rabs.rabs[count].rab_type)
            {
               rrcrbcommon_ext_data.rrcrb_cs_data_call = TRUE;
            }
          }
          else if (rrcrbcommon_ext_data.mac_rab_status == PS_RABS)
          {
            rrcrbcommon_ext_data.mac_rab_status = CS_PLUS_PS_RABS;
        ...
        else if (rrc_est_rabs.rabs[count].cn_domain == RRC_PS_DOMAIN_CN_ID) 
        {
          if (rrcrbcommon_ext_data.mac_rab_status == NO_RABS) 
          {
            rrcrbcommon_ext_data.mac_rab_status = PS_RABS;
          }
          else if (rrcrbcommon_ext_data.mac_rab_status == CS_RABS) 
          {
            rrcrbcommon_ext_data.mac_rab_status = CS_PLUS_PS_RABS;
            /* once we have CS+PS, no need to go thru entire est_rabs list */
            break;
  ...
  else if (proc_id == RRC_PROCEDURE_IHO)
  {
    ...
    {
      rrcrbcommon_ext_data.mac_rab_status = CS_RABS;
      if (TRUE == rrciho_is_cs_data_rab_present(wrrc_as_id))
      {
        rrcrbcommon_ext_data.rrcrb_cs_data_call = TRUE;
      }	  
    }
  ...
  else if (proc_id == RRC_PROCEDURE_RBE)
  {
    if (rrcrbcommon_ext_data.mac_rab_status == NO_RABS)
    {
      ...
      {
        rrcrbcommon_ext_data.mac_rab_status = CS_RABS;
        if (TRUE == rrcrbe_is_cs_data_rab_present(wrrc_as_id))	
        {
           rrcrbcommon_ext_data.rrcrb_cs_data_call = TRUE;
        }
      }
            ...
            )
        {
          ...
          for( count2=0; count2<MAX_RB_PER_RAB; count2++ )
          {
            if( est_rabs_copy_local_ptr->rabs[count1].rb_for_rab[count2].rb_id == rb_info_rel_list_ptr->elem[rb_idx])
            {
              ...
              WRRC_MSG3_HIGH("MAC_RAB_STATUS: RB removed rbid:%d, rabid:%d, domain:%d",
  ...
  WRRC_MSG1_HIGH("MAC_RAB_STATUS %d", rrcrbcommon_ext_data.mac_rab_status);
...
)
{
  ...
  {
    rrc_freq = rrc_ccm_get_curr_camped_freq(wrrc_as_id);
	WRRC_MSG4_HIGH("This F3 is not valid for FACH to DCH transition:Camped freq: %d directed freq: %d Camped_psc: %d, directed_psc: %d",
		rrc_freq, directed_cell_info.uarfcn_dl, rrc_ccm_get_curr_camped_psc(wrrc_as_id),directed_cell_info.pscr);
                  ...
                  
    {
      ...
      {
        if (rrc_ccm_get_curr_camped_psc(wrrc_as_id) == directed_cell_info.pscr)
        {
  ...
  else
  {
    ...
    if (directed_cell_info.pscr_present)
    {
      WRRC_MSG2_HIGH(" Current_psc: %d, directed_psc: %d", rrc_ccm_get_curr_camped_psc(wrrc_as_id),directed_cell_info.pscr);
      if (rrc_ccm_get_curr_camped_psc(wrrc_as_id) == directed_cell_info.pscr)
      {
...
)
{
  ...
  WRRC_MSG3_HIGH("RNTI_UPDATE_REQ CRNTI:%d, URNTI:%d rrc_proc: %d",
...
)
{
  ...
  rrc_cmd_type *int_cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id);
...
SIDE EFFECTS      None.
===========================================================================*/
void rrcrb_send_l1_drx_req(sys_modem_as_id_e_type wrrc_as_id, rrc_proc_e_type rrc_proc)
{
  ...
  rrc_cmd_l1_req->drx.drx_cycle_len =  rrc_csp_set_drx_length(wrrc_as_id,drx_len);
...
SIDE EFFECTS      None.
===========================================================================*/
void rrcrb_send_upd_ota_parms_ind(sys_modem_as_id_e_type wrrc_as_id,rrc_proc_e_type rrc_proc, uint8 bit_mask_ind)
{
...
)
{
  ...
  WRRC_MSG3_HIGH("CELL_UPDATE_INITIATE Proc:%d, Cause:%d, cnf_reqd:%d", 
...
)
{
  ...
  WRRC_MSG3_HIGH("URA_UPDATE_INITIATE Proc:rrc_proc_e_type_value%d, Cause:%d, cnf_reqd:%d", 
            procedure, cause, cnf_reqd);
  rrc_put_int_cmd(wrrc_as_id, int_cmd_ptr);
...
)
{
  ...
  if(rrcllc_return_freq_for_fach_pch(wrrc_as_id,&int_cmd_ptr->cmd.initiate_cell_sel_ind.dest_freq)
     == SUCCESS)
  {
  ...
  if(rrcllc_return_psc_for_fach_pch(wrrc_as_id,&int_cmd_ptr->cmd.initiate_cell_sel_ind.dest_psc)
     == SUCCESS)
  {
  ...
  WRRC_MSG3_HIGH("INITIATE_CELL_SELECTION Proc:rrc_proc_e_type_value%d, Cause:%d, Dest_st:%d", 
...
{
  rrc_cmd_type    *llc_cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id);
  uint32           count      = 0;
  uint32           rab_count  = 0;
  uint32           rb_count   = 0;
  ...
  /* Now fill the L2 cmd header */
   ...
   for (rab_count=0; rab_count<MAX_RAB_TO_SETUP; rab_count++)
   {
             ...
             )
     {
       ...
       {
         rrc_cmd_l2_req->ul_reg_data_ind.lc_id[count++] =
            rrclcm_get_ul_rlc_lc_id(wrrc_as_id,rrc_est_rabs.rabs[rab_count].rb_for_rab[rb_count].rb_id);
       }
     } /*end of if*/
   }/* rab_count loop */
...
)
{
     ...
     switch (proc_id)
     {
       case  RRC_PROCEDURE_RBE:
         proc_sub_st = rrcrbe_get_current_sub_state(wrrc_as_id);
         break;
       case  RRC_PROCEDURE_RBR:
         proc_sub_st = rrcrbr_get_current_sub_state(wrrc_as_id);
         break;
       case  RRC_PROCEDURE_RBRC:
         proc_sub_st = rrcrbrc_get_current_sub_state(wrrc_as_id);
         break;
       case  RRC_PROCEDURE_TCR:
         proc_sub_st = rrctcrc_get_current_sub_state(wrrc_as_id);
         break;
       case  RRC_PROCEDURE_PCR:
         proc_sub_st = rrcpcrc_get_current_sub_state(wrrc_as_id);
         break;
       default:
         break;
     }
...
)
{
  ...
  uint32 prev_dl_amr_mode = WCDMA_IVOCAMRWB_CODEC_MODE_UNDEF;
  uint32 new_dl_amr_mode = WCDMA_IVOCAMRWB_CODEC_MODE_UNDEF;
  ...
  if(!rrcllc_check_if_new_tfcs_present(wrrc_as_id))
  {
  ...
  if(TRUE == rrc_is_voice_rab_present(wrrc_as_id))
  {
    ...
    if( codec_type != rrcdata_codec_info.rrc_active_codec)
    {
      rrc_set_current_codec_mode(wrrc_as_id,codec_type);
   
      memset(&rrcrbcommon_ext_data.amr_rb_info, 0, sizeof(rrc_rb_info_for_amr_type));
        /* Initialize RAB type to unknown as Circuit Switched Voice RAB maps to value 0 */
      rrcrbcommon_ext_data.amr_rb_info.rab_type = RRCRB_UNKNOWN_RAB;
    
      for(i=0; i<MAX_RAB_TO_SETUP; i++)
      {
        if(rrc_est_rabs.rabs[i].rab_type == RRCRB_CS_VOICE_RAB)
        {
          rrcrbcommon_ext_data.amr_rb_info.rab_type = RRCRB_CS_VOICE_RAB;
          rrcrbcommon_ext_data.amr_rb_info.num_rbs_for_rab= rrc_est_rabs.rabs[i].num_rbs_for_rab;
   
          for(j=0; j< rrcrbcommon_ext_data.amr_rb_info.num_rbs_for_rab; j++)
          {
            rrcrbcommon_ext_data.amr_rb_info.rb_id[j] = rrc_est_rabs.rabs[i].rb_for_rab[j].rb_id;
          }
    ...
    rrc_set_current_codec_mode(wrrc_as_id,codec_type);
    if( codec_type == WCDMA_IVOCODER_ID_AMRWB)
    {
      amr_info.mode_type = WCDMA_IVOCODER_ID_AMRWB;
      if(FAILURE == rrcllc_get_amr_wb_mapping(wrrc_as_id,&(amr_info)))
      {
    ...
    else
    {
      ...
      if(FAILURE == rrcllc_get_amr_mapping(wrrc_as_id,&(amr_info)))
      {
        WRRC_MSG0_ERROR("No AMR Mapping");
        return;
      }
   ...
   /*****************************/
	/* For WB, we don't need to reconfigure the logical channels as it's not possible to change the same */
    if((WCDMA_IVOCODER_ID_AMRWB == codec_type) || (WCDMA_IVOCODER_ID_AMR == codec_type))
    {
	...
	/* For NB, we need to reconfigure the logical channels as it's possible to change the same */
    if (WCDMA_IVOCODER_ID_AMR == codec_type)
    {
      rrc_configure_voice_adapter(wrrc_as_id,&amr_info, TRUE, FALSE,WCDMA_IVOICE_EVENT_SET_LOGICAL_CHANNELS);	
    }

/********************************/
/* Downlink AMR rate processing logic */
/********************************/
    if(codec_type == WCDMA_IVOCODER_ID_AMR)
    {
      dl_mode_changed = rrc_get_dl_amr_mode_given_rbs(wrrc_as_id,&prev_dl_amr_mode, &new_dl_amr_mode, &rrcrbcommon_ext_data.amr_rb_info);    
      WRRC_MSG2_HIGH("WV_WRRC: DL -> prev mode=%d, new mode=%d", prev_dl_amr_mode, new_dl_amr_mode);
      ...
      if (new_dl_amr_mode >= WCDMA_IVOCAMR_CODEC_MODE_1020)
        { 
          if((i < MAX_RAB_TO_SETUP) && (rrc_est_rabs.rabs[i].num_rbs_for_rab == 3))
          { 
            if(rrc_get_amr_mode_bitrate(wrrc_as_id,prev_dl_amr_mode) == RRC_AMR_MODE_LOW_BITRATE)
            {
            rrc_configure_voice_adapter(wrrc_as_id,&amr_info,FALSE,TRUE, WCDMA_IVOICE_EVENT_SET_LOGICAL_CHANNELS);
            }
          }
          else
          {
        ...
        {
          if(rrc_get_amr_mode_bitrate(wrrc_as_id,prev_dl_amr_mode) == RRC_AMR_MODE_HIGH_BITRATE)
          {
          rrc_configure_voice_adapter(wrrc_as_id,&amr_info, FALSE, TRUE,WCDMA_IVOICE_EVENT_SET_LOGICAL_CHANNELS);
          }
        }
...
)
{
  ...
  for(rb_idx = 0;((rb_idx < rrcllcoc_int_data.rrc_rb_list_mapped_to_dch.num_of_RBs)&&(*rb_count < MAX_RB));rb_idx++)
  {
    /*Check whether the RB is already part of chan_config_req*/
    if(MAX_RB == rrcllc_check_if_rbid_present_in_chan_config(wrrc_as_id,chan_config_req,
        rrcllcoc_int_data.rrc_rb_list_mapped_to_dch.rrc_rb_mapped_to_dch[rb_idx]))
    {
      WRRC_MSG1_HIGH("Fill RB %d from rrc_rb_list_mapped_to_dch", rrcllcoc_int_data.rrc_rb_list_mapped_to_dch.
            rrc_rb_mapped_to_dch[rb_idx]);
      chan_config_req->rb[(*rb_count)].rb_id = rrcllcoc_int_data.rrc_rb_list_mapped_to_dch.rrc_rb_mapped_to_dch[rb_idx];
#ifndef FEATURE_MAC_I	
      /*Reestab_entity has been set while adding rbs to rrc_rb_list_mapped_to_dch*/
      if((am_entity_idx =get_am_index_wo_f3(wrrc_as_id,rrcllcoc_int_data.rrc_rb_list_mapped_to_dch.rrc_rb_mapped_to_dch[rb_idx])) 
          < UE_MAX_AM_ENTITY)
      {
        WRRC_MSG2_HIGH("Re-establish RB-ID %d at AM index %d in OC",rrcllcoc_int_data.rrc_rb_list_mapped_to_dch.
  ...
  rrcllc_init_rb_list_mapped_to_dch(wrrc_as_id);
}
/*===========================================================================
...
)
{
     ...
     transition_config.toc_usage == TOC_FOR_FACH_AND_PCH_WITH_DCH_INFO)
  {
    ...
    */
    if(rrchsrach_camped_cell_supports_hsrach(wrrc_as_id) == TRUE)
    {
      rrcllc_update_hs_rach_params(wrrc_as_id,ordered_config_ptr);
      oc_updated_with_hsrach_params = TRUE;
    }
#endif
  }  
  else if((rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH) || (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH))
  {
    dest_state = RRC_STATE_CELL_DCH;
  }
  else
  {
    WRRC_MSG1_ERROR("didnt expect this function to be called in %d state",rrc_get_state(wrrc_as_id));
    return;
  }
  ...
  for(rab_idx = 0; ((rab_idx < MAX_RAB_TO_SETUP) && (*rb_count < MAX_RB)); rab_idx++)
  {
    ...
    for(rb_idx = 0; (rb_idx < rrc_est_rabs.rabs[rab_idx].num_rbs_for_rab) && (*rb_count < MAX_RB); rb_idx++)
    {
      ...
      if((rrcllc_check_if_rlc_mode_is_am(wrrc_as_id,rb_id)) == TRUE)
      {
        chan_idx = rrcllc_check_if_rbid_present_in_chan_config(wrrc_as_id,chan_config_req,rb_id);
       /*Check whether the RB is already part of chan_config_req*/
        if( chan_idx >= MAX_RB )
        {
          chan_idx = *rb_count;
        ...
        if((am_entity_idx = get_am_index_in_config_ptr(wrrc_as_id,rb_id,config_ptr)) >= UE_MAX_AM_ENTITY)
        {
        ...
        if(config_ptr->rlc_am_parms.am_info[am_entity_idx]->reestab_entity != RLC_RE_ESTABLISH_DL_UL)
        {
          ...
          if(config_ptr->rlc_am_parms.am_info[am_entity_idx]->reestab_entity == RLC_RE_ESTABLISH_NONE )
          {
            ...
            if ((rrcllc_get_am_dl_rlc_size_ota(wrrc_as_id,rb_id) == RRCLLC_INVALID_RLC_SIZE) &&
                (  (transition_config.toc_usage == TOC_FOR_DCH_AND_FACH)  ||
                   (config_ptr->rlc_am_parms.am_info[am_entity_idx]->rlc_dl_am_config.dl_rlc_size != 
                      rrcllc_get_dl_ded_rlc_size(wrrc_as_id,dest_state,rb_id))
                ))
            {
#ifdef FEATURE_MAC_I
              ...
              WRRC_MSG3_HIGH("DL RB %d,size changed from %d to %d",rb_id,
               config_ptr->rlc_am_parms.am_info[am_entity_idx]->rlc_dl_am_config.dl_rlc_size,
               rrcllc_get_dl_ded_rlc_size(wrrc_as_id,dest_state,rb_id));
            }
          }
      ...
      else if ((rrcllc_check_if_rlc_mode_is_um(wrrc_as_id,rb_id)) == TRUE)				
      {
         chan_idx = rrcllc_check_if_rbid_present_in_chan_config(wrrc_as_id,chan_config_req,rb_id);
       /*Check whether the RB is already part of chan_config_req*/      
        if( MAX_RB == chan_idx )
        {
          ...
          if(transition_config.toc_usage == TOC_FOR_DCH_AND_FACH)
          {
            if((um_ul_entity_idx = get_um_ul_index(wrrc_as_id,rb_id, transition_config.toc_ptr)) 
          ...
          {
            if((um_ul_entity_idx = get_ul_um_index(wrrc_as_id,rb_id)) 
                        >= UE_MAX_UL_UM_CHANNEL)
            {
          ...
          if(transition_config.toc_usage == TOC_FOR_DCH_AND_FACH)
          {
            if((um_dl_entity_idx =  get_dl_um_index(wrrc_as_id,rb_id,transition_config.toc_ptr)) 
              >= UE_MAX_UL_UM_CHANNEL)
            {
          ...
          {
            if((um_dl_entity_idx = get_dl_um_index(wrrc_as_id,rb_id,ordered_config_ptr)) 
          ...
          {
            WRRC_MSG1_ERROR("didnt expect this function to be called in rrc_state_e_type_value%d state",rrc_get_state(wrrc_as_id));
          }
  ...
  {
#ifdef FEATURE_WCDMA_HS_RACH
    rrcllc_init_mac_hsrach_mapping_info(wrrc_as_id,&ordered_config_ptr->mac_hs_rach_config);
#endif
...
)
{
  ...
  if((rrcllc_check_if_rbid_present_in_chan_config(wrrc_as_id,chan_config_req,DCCH_UM_RADIO_BEARER_ID) == MAX_RB) &&
      (num_rb < MAX_RB))
  {
    chan_config_req->rb[num_rb].rb_id = DCCH_UM_RADIO_BEARER_ID;
    chan_config_req->rb[num_rb++].rb_config = RECONFIG_RB;
  }
  if((rrcllc_check_if_rbid_present_in_chan_config(wrrc_as_id,chan_config_req,DCCH_AM_RADIO_BEARER_ID) == MAX_RB) &&
      (num_rb < MAX_RB))
  {
    chan_config_req->rb[num_rb].rb_id = DCCH_AM_RADIO_BEARER_ID;
    chan_config_req->rb[num_rb++].rb_config = RECONFIG_RB;
  }
  if((rrcllc_check_if_rbid_present_in_chan_config(wrrc_as_id,chan_config_req,DCCH_DT_HIGH_PRI_RADIO_BEARER_ID) == MAX_RB) &&
      (num_rb < MAX_RB))
  {
  ...
  else if((rrcllc_check_if_rbid_present_in_chan_config(wrrc_as_id,chan_config_req,DCCH_DT_LOW_PRI_RADIO_BEARER_ID) == MAX_RB) &&
          (num_rb < MAX_RB))
  {
  ...
  for(i=0;((i<MAX_HANGING_RB_MAPPING_INFO) && (*rb_count < MAX_RB));i++)
  {
    if(rrc_current_hanging_rb_mapping[i].in_use == TRUE)
    {
      ...
      if(rrc_ordered_hanging_rb_mapping[i].in_use == TRUE)
      {
         ...
         */
        if(FALSE == rrcllc_check_ordered_current_mapping_same( wrrc_as_id, i))
        {
          /* Skipping the case when the RB-ID was already present in the OTA message. */
          if( MAX_RB == rrcllc_check_if_rbid_present_in_chan_config(wrrc_as_id,
      ...
      if( fill_rb_info == TRUE)
      {
        ...
        if( (RRCLCM_RLC_LC_ID_NOT_FOUND == rrclcm_get_dl_rlc_lc_id
                (wrrc_as_id, rrc_current_hanging_rb_mapping[i].rb_id )) &&
...
)
{
  ...
  (void) rrcllc_get_ordered_config_state_and_proc(wrrc_as_id,proc_id, &process_state);
  if (((*proc_id == RRC_PROCEDURE_RBE || *proc_id == RRC_PROCEDURE_RBRC || 
      ...
      ( rrcrb_get_proc_sub_state(wrrc_as_id,*proc_id) == RRCRB_WAIT_FOR_CELL_SELECTION_CNF ))||
      ((*proc_id == RRC_PROCEDURE_CU) && (rrccu_return_cu_substate(wrrc_as_id) == RRCCU_WAIT_FOR_UE_TO_CAMP_ON)&&
        rrccu_check_if_cell_selection_in_progress(wrrc_as_id) == TRUE)
      )
  {
...
)
{
  ...
  if( rabs_info->rabs_in_msg[rab_index].nas_Synchronisation_Indicator == WCDMA_IVOCODER_ID_NONE)
  {
...
#endif
  rrc_set_ordered_codec_mode(wrrc_as_id,rabs_info->rabs_in_msg[rab_index].nas_Synchronisation_Indicator);
  return SUCCESS;
}
  ...
  for(rab_index = 0; (rab_list_ptr->n > rab_index) && ( rab_index < RRCRB_MAX_RAB_PER_MSG); rab_index++)
  {
    ...
    if ( rab_list_ptr->elem[rab_index].cn_DomainIdentity  == rrc_CN_DomainIdentity_cs_domain )
    {
      ...
      else
      {
        ...
        {
          if(rrc_update_nas_sync_ind_r7(wrrc_as_id,rab_index, rabs_info, 
...
{
  rrcrbcommon_ext_data.rrc_wait_for_l2ack_for_reconfig = proc_id;
}
...
)
{
  ...
  switch(rrcrbcommon_ext_data.rrc_wait_for_l2ack_for_reconfig)
  {
...
)
{
  ...
  for (rab_index = 0; (rab_list_ptr->n > rab_index) && (rab_index < RRCRB_MAX_RAB_PER_MSG); rab_index++)
  {
    ...
    /* Check if the RAB id is already in ESTABLISHED_RABS */
    if (rrc_find_rab_in_established_rabs(wrrc_as_id,rab_id) != RRC_RAB_FOUND)
    {
    ...
    if ( rab_list_ptr->elem[rab_index].cn_DomainIdentity  == rrc_CN_DomainIdentity_cs_domain )
    {
      ...
      else
      {
        /* For CS RABs, Call MN layer to verify this RAB */
#ifdef FEATURE_DUAL_SIM
        switch (rrcmn_verify_rab_subs((radio_access_bearer_T)rab_id ,rrc_get_as_id(wrrc_as_id)))
#else
  ...
  { 
    ...
    if (rrc_cap_related_info.rrc_pdcp_enabled)  
    {
      ...
      if(rrcllc_validate_pdcp_info_in_rb_info_reconfig_list_r8
        (wrrc_as_id, rb_information_reconfig_ptr) == FALSE)
      {
        msg_not_valid = TRUE;
        break;
      }
...
{ 
  	...
  	rrc_RB_InformationReconfig_r8,pdcp_Info))
  {
    ...
    if(rrc_get_rab_type_for_given_rb_id(wrrc_as_id,rb_reconfig_ptr->rb_Identity) != RRCRB_PS_DATA_RAB)
    {
...
)
{
  ...
  switch (procedure)
  {
    ...
    case  RRC_PROCEDURE_TCR:
    case  RRC_PROCEDURE_PCR:
      if(RRCRB_WAIT_FOR_L2_ACK == rrcrb_get_proc_sub_state(wrrc_as_id,procedure))
      {
...
)
{
  ...
  {
    case RRC_PROCEDURE_RBE:
      cu_status = rrcrbe_get_initiate_cu_flag(wrrc_as_id);
      break;
    case RRC_PROCEDURE_RBR:
       cu_status = rrcrbr_get_initiate_cu_flag(wrrc_as_id);
      break;
    case RRC_PROCEDURE_RBRC:
      cu_status = rrcrbrc_get_initiate_cu_flag(wrrc_as_id);
      break;
    case  RRC_PROCEDURE_TCR:
      cu_status = rrctcrc_get_initiate_cu_flag(wrrc_as_id);
      break;
    case  RRC_PROCEDURE_PCR:
      cu_status = rrcpcrc_get_initiate_cu_flag(wrrc_as_id);
      break;
  ...
  if((cu_status) || (rrccu_return_cu_substate(wrrc_as_id) == RRCCU_WAIT_FOR_RB_PROC_COMPL_IND) )
  {
    return TRUE;
  }
  return FALSE;
...
{
  if(RRCRB_WAIT_FOR_CELL_UPDATE_CNF == rrcrb_get_proc_sub_state(wrrc_as_id, rrc_buffer_ota_info.procedure))
  {
      
    switch(rrc_buffer_ota_info.procedure)
    {
      case RRC_PROCEDURE_RBE:
        rrcrbe_set_failure_cause(wrrc_as_id, RRCRB_PHY_CHAN_FAILURE);
        break;
      
      case RRC_PROCEDURE_RBRC:
        rrcrbrc_set_failure_cause(wrrc_as_id, RRCRB_PHY_CHAN_FAILURE);
        break;
      
      case RRC_PROCEDURE_TCR:
        rrctcrc_set_failure_cause(wrrc_as_id, RRCRB_PHY_CHAN_FAILURE);
        break;
    
      case RRC_PROCEDURE_PCR:
        rrcpcrc_set_failure_cause(wrrc_as_id, RRCRB_PHY_CHAN_FAILURE);
        break;
      
      case RRC_PROCEDURE_RBR:
        rrcrbr_set_failure_cause(wrrc_as_id, RRCRB_PHY_CHAN_FAILURE);
        break;
      ...
      default:
        WRRC_MSG1_ERROR("Invalid procedure id passed. something wrong %d", rrc_buffer_ota_info.procedure);
      break;
    }
...
FUNCTION  rrcrbcommon_init_procedure

DESCRIPTION

  This function initializes the RRCRBCOMMON Procedure
...
===========================================================================*/
void rrcrbcommon_init_procedure
(
sys_modem_as_id_e_type wrrc_as_id
)
{
...
#endif
  rrcrbcommon_ext_data.rrc_wait_for_l2ack_for_reconfig = MAX_NUM_OF_PROCEDURES;
...
size_t wrrc_qsh_dump_rb_proc_debug(sys_modem_as_id_e_type wrrc_as_id, uint8* dump_addr, size_t size)
{
 ...
 WCDMA_MEMCPY(&rb_proc_globals,sizeof(wrrc_qsh_dump_tag_rb_proc),
  ...
  rab_info = rrc_get_rabs_in_established_rabs_for_given_domain(wrrc_as_id, RRC_PS_DOMAIN_CN_ID);;
  rb_proc_globals.rrc_rbe_substate = rrcrbe_get_current_sub_state(wrrc_as_id);
  rb_proc_globals.rrc_rbrc_substate = rrcrbrc_get_current_sub_state(wrrc_as_id);
  rb_proc_globals.rrc_pcr_substate = rrcpcrc_get_current_sub_state(wrrc_as_id);
  rb_proc_globals.rrc_tcr_substate = rrctcrc_get_current_sub_state(wrrc_as_id);
  rb_proc_globals.rrc_rbr_substate = rrcrbr_get_current_sub_state(wrrc_as_id);
  rb_proc_globals.num_of_ps_rabs = rab_info.num_rabs;
  rab_info = rrc_get_rabs_in_established_rabs_for_given_domain(wrrc_as_id, RRC_CS_DOMAIN_CN_ID);
  rb_proc_globals.num_of_cs_rabs = rab_info.num_rabs;
  rb_proc_globals.rrcrbe_send_initiate_cu_flag = rrcrbe_return_send_initiate_cu_flag(wrrc_as_id);
  rb_proc_globals.rrcrbrc_send_initiate_cu_flag = rrcrbrc_return_send_initiate_cu_flag(wrrc_as_id);
  rb_proc_globals.rrcpcrc_send_initiate_cu_flag = rrcpcr_return_send_initiate_cu_flag(wrrc_as_id);
  rb_proc_globals.rrctcrc_send_initiate_cu_flag = rrctcrc_return_send_initiate_cu_flag(wrrc_as_id);
  rb_proc_globals.rrcrbr_send_initiate_cu_flag = rrcrbr_return_send_initiate_cu_flag(wrrc_as_id);
...
FUNCTION   rrc_common_procedure_clear

DESCRIPTION 
    function to clear common data for all reconfiguration procedures
...
void rrc_common_procedure_clear(sys_modem_as_id_e_type wrrc_as_id,rrc_proc_e_type proc)
{
  rrc_check_and_init_rrc_reestab_entity(wrrc_as_id, proc);
  rrcllc_reset_toc_usage_change_oos_to_dch(wrrc_as_id);
}
...
{
  qsh_mdump_collect_high(&rrcrbcommon_ext_data,sizeof(rrcrbcommon_ext_data));
}
#endif