...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcmcm.c#8 $    $DateTime: 2020/08/04 01:07:37 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
03/08/20   gunturu     Abort blind redirection for FRLTE cases upon voice call.
...
05/31/17   ha      FR 42544 MPLMN periodic scan search
...
03/21/18   gc      Initialize change_mode_req.reason for fixing KW error
01/30/18   sas     Supervision timer changes in LTA gap.
12/12/17   sp      Process DDS_SUB_IND when in inactive state
10/03/17   sp      Changes for LTA timeline optimization
05/20/16   sr      f3 reduction changes for RCE/UECI/DT
05/20/16   gv      Made changes to prevent reset of CSFB call status at WCDMA stop
...
04/20/16   sr      Made changes to reduce the LTA lock request duration to 150 ms
...
02/11/16   sr      Made changes to pass activity as IRAT_CGI for START WCDMA lock request
...
12/04/15   sr      Changes to acquire lock in WRLF service request for CS with TRAFFIC for START WCDMA
11/26/15   vs      Changes to cleanup unnecessary f3s
11/25/15   sn      Changes to delay handling NAS proc indication during QTA
11/18/15   gv      Made changes to pass right band to which reselection has to be attempted as part of WCDMA start
11/17/15   sr      Debug code to detect out of sync between NAS and RRC 
11/16/15   gv      Made changes to take TRM lock with appropriate priority to start WCDMA for service search
11/12/15   sp      Made changes to log call end info in all cases whenver call is released
09/29/15   gv      Release the TRM lock though CSFB call is set but when scan type is not redirection
09/14/15   sr      Made changes to get default band from RF api for lock request
09/08/15   sas     Made changes to Perform TRM exchange with IRAT client before sending W2X redirection_req.
...
07/12/15   sn      Changes to send activity type in start WCDMA request and
                   request lock with ACQ_INV for manual PLMN searches
...
06/01/15   sas     Made changes to indicate RLC about the LTA/QTA start/stop status.
...
03/17/15   sr      Indicate LTE capability to WL1
03/03/15   sn      Changes for FR 24681
12/30/14   bc      Replacing ERR_FATAL with MSG_ERR0_FATAL, MSG_ERR1_FATAL, MSG_ERR2_FATAL.
12/16/14   sn      Changes to raise an assert if incorrect cause is received in 
                   stop WCDMA request 
12/16/14   sr      Made changes to reset csfb_call_status on stopping W Stack.
...
LOCAL rrcmcm_int_data_type rrcmcm_int_data_sim[MAX_RRC_STACKS];
extern boolean rrc_dbg_is_hard_abort_sim[MAX_RRC_STACKS];
#define rrcmcm_int_data rrcmcm_int_data_sim[WAS_ID_M]
#define rrc_dbg_is_hard_abort rrc_dbg_is_hard_abort_sim[WAS_ID_M]
#else
LOCAL rrcmcm_int_data_type rrcmcm_int_data;
extern boolean rrc_dbg_is_hard_abort;
#endif
...
FUNCTION  RRCMCM_GET_SVD_PLMN_REQ_PTR

DESCRIPTION

  This function returns the address of the saved plmn list req.
...
RETURN VALUE
  ...
  Pointer to last saved plmn list req 
...
===========================================================================*/
rrc_plmn_list_req_type* rrcmcm_get_svd_plmn_req_ptr(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcmcm_int_data.rrc_svd_plmn_req_ptr;
}
/*===========================================================================

FUNCTION  RRCMCM_FREE_SVD_PLMN_REQ_PTR

DESCRIPTION

  This function will free the memory held by saved plmn list req.
...
===========================================================================*/
void rrcmcm_free_svd_plmn_req_ptr(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_free(rrcmcm_int_data.rrc_svd_plmn_req_ptr);
}
/*===========================================================================

FUNCTION  RRCMCM_GET_ACT_CAUSE

DESCRIPTION

  This function returns the mcm activation cause
...
===========================================================================*/
rrcmcm_activation_cause rrcmcm_get_act_cause(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcmcm_int_data.rrcmcm_act_cause;
}
...
===========================================================================*/
static void rrcmcm_send_change_mode_req(sys_modem_as_id_e_type wrrc_as_id, rrc_mode_e_type mode, rrc_proc_e_type proc_id )
{
...
===========================================================================*/
void rrcmcm_set_establishment_cause(sys_modem_as_id_e_type wrrc_as_id)
{
   ...
   */
  if(rrcdata_misc_info.rrc_establishment_cause == RRC_EST_REGISTRATION)
  {
    if(rrcmcm_int_data.rrcmcm_last_act_cause == RRCMCM_INTERRAT_CELL_RESELECTION)
    {
      WRRC_MSG0_HIGH("Updating Est Cause as INTERRAT_CELL_RESELECTION");    
      rrcdata_misc_info.rrc_establishment_cause = RRC_EST_INTER_RAT_CELL_RESELECT;
    }
    else if(rrccho_is_gtow_pcco_in_progress(wrrc_as_id))
    {
      WRRC_MSG0_HIGH("Updating Est Cause as INTER_RAT_CELL_CHANGE_ORDER");
      rrcdata_misc_info.rrc_establishment_cause = RRC_EST_INTER_RAT_CELL_CHANGE_ORDER;
    }
    else
    {
    // Add more cases here
  ...
  rrcmcm_int_data.rrcmcm_last_act_cause = RRCMCM_ACT_CAUSE_NONE;
...
===========================================================================*/
static void rrcmcm_init_data(sys_modem_as_id_e_type wrrc_as_id, rrcmcm_init_e_type init_type )
{                   
  if( (init_type == INIT_ALL) || (init_type == INIT_STATE_VARS_ONLY) )
  {
    /* Reset variables */
    rrcmcm_int_data.rrcmcm_state = RRCMCM_INITIAL;
    rrcmcm_int_data.stop_cause = RRC_RAT_CHANGE;
    rrcmcm_int_data.rrcmcm_est_req_saved_flag = FALSE;
    rrcmcm_int_data.rrcmcm_send_stop_cnf_to_mm_flag = FALSE;
  }
  
  if( (init_type == INIT_ALL) || (init_type == INIT_CMD_STORE_ONLY) )
  {
    /* init cmd_store */
    memset(&rrcmcm_int_data.rrcmcm_cmd_store,0,sizeof(rrcmcm_int_data.rrcmcm_cmd_store));
    rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag = RRCMCM_RR_ABORT_NONE;
    rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag = RRCMCM_CMD_NONE;
    rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_change_mode_req.mode = RRC_MODE_MAX;
  }
...
)
{
  /*copy the contents of est_req from the command locally and set saved flag
    to true*/
  WCDMA_MEMCPY(&rrcmcm_int_data.rrcmcm_est_req, 
               sizeof(rrc_est_req_type),
               &cmd_ptr->cmd.rrc_est_req, 
               sizeof(rrc_est_req_type));
  rrcmcm_int_data.rrcmcm_est_req_saved_flag = TRUE;
...
)
{
  ...
  if(rrc_est_mode == RRC_MODE_ACTIVE)
  {
    ...
    WCDMA_MEMCPY(&est_req_cmd->cmd.rrc_est_req,
                 sizeof(rrc_est_req_type), 
                 &rrcmcm_int_data.rrcmcm_est_req, 
                 sizeof(rrc_est_req_type));
    
    /*call rrcrce procedure as it would have been called if not for STANDBY state*/
    rrcrce_procedure_event_handler(wrrc_as_id, est_req_cmd );
  }
...
)
{
  ...
  rr_send_rrc_message_to_rr_internal(wrrc_as_id,(void *)(interrat_plmn_srch_cnf), RRC_INTERRAT_PLMN_SRCH_CNF,
...
)
{
  ...
  rr_send_rrc_message_to_rr_internal(wrrc_as_id,(void *)(&(rrc_interrat_reselect_rej)), RRC_INTERRAT_RESELECTION_REJ,
...
)
{
  ...
  plmn_srch_rsp->preferred_plmn_found = rrc_csp_int_data.rrc_rr_plmn_srch_cnf.preferred_plmn_found;
  plmn_srch_rsp->available_plmn_list.length = 0;
  plmn_srch_rsp->detected_frequency_list.num_freqs = 0;
  rrc_send_lte_cmd(wrrc_as_id,&plmn_srch_rsp->msg_hdr,WCDMA_RRC_LTE_PLMN_SRCH_RSP,sizeof(lte_irat_plmn_srch_rsp_s));
...
)
{
  ...
  rrc_send_lte_cmd(wrrc_as_id,&resel_failed_rsp.msg_hdr,WCDMA_RRC_LTE_RESEL_FAILED_RSP,sizeof(wcdma_rrc_lte_resel_failed_rsp_type));
}
#endif
...
===========================================================================*/
boolean rrcmcm_is_process_required(sys_modem_as_id_e_type wrrc_as_id, rrc_cmd_type *cmd_ptr )
{                   
  ...
  if(cmd_ptr->cmd_hdr.cmd_id == RRC_DDS_SUB_IND)
  {
    return TRUE;
  }
  ...
  if(cmd_ptr->cmd_hdr.cmd_id == RRC_START_G_MEAS_IND)
  {
    return TRUE;
  }
#endif

  if(cmd_ptr->cmd_hdr.cmd_id == RRC_ABNORMAL_CALL_END_IND || cmd_ptr->cmd_hdr.cmd_id == RRCTMR_DELAY_WTOL_RESEL_TIMER_EXPIRED_IND)
  {
    return TRUE;
  }
  ...
  if(cmd_ptr->cmd_hdr.cmd_id == RRC_UE_MODE_IND)
  {
    ...
    {
      rrcmcm_int_data.is_ue_in_sglte_mode = TRUE;
    }
    else
    {
      rrcmcm_int_data.is_ue_in_sglte_mode = FALSE;
    }
    return FALSE;
  }
  ...
  if(RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS())
  {
    if((rrc_rr_cmd_e_type)cmd_ptr->cmd_hdr.cmd_id == RRC_INTERRAT_RESELECTION_REQ)
    {
      ...
      rrccsp_send_gtow_reselection_reject(wrrc_as_id);
      return FALSE;
    }
    else 
    if((rrc_rr_cmd_e_type)cmd_ptr->cmd_hdr.cmd_id == RRC_INTERRAT_PLMN_SRCH_REQ)
    {
      ...
      rrcmcm_send_gtow_bplmn_search_cnf_failed(wrrc_as_id);
      return FALSE;
    }
    else if(cmd_ptr->cmd_hdr.cmd_id == RRC_PLMN_LIST_REQ)
    {
      if(rrc_csp_is_suitable_srch_ended(wrrc_as_id))
      {
        ...
        if(rrcrce_is_abort_in_progress(wrrc_as_id) && !rrc_dbg_is_hard_abort)
        {
          WRRC_ERR0_FATAL("While in COOS, MPLMN search request received without a hard abort");
        }
         
        rrcmcm_int_data.rrc_svd_plmn_req_ptr = (rrc_plmn_list_req_type *)rrc_malloc(sizeof(rrc_plmn_list_req_type));
        WCDMA_MEMCPY(rrcmcm_int_data.rrc_svd_plmn_req_ptr,
      ...
      rrc_csp_send_mm_plmn_list_cnf_as_aborted(wrrc_as_id,cmd_ptr->cmd.plmn_list_req.trans_id);
      }
      return FALSE;
    }
    else if((cmd_ptr->cmd_hdr.cmd_id == RRC_BPLMN_SEARCH_ABORT_REQ)&&
            (rrcmcm_int_data.rrc_svd_plmn_req_ptr != NULL)) 
    {
      rrc_csp_send_mm_plmn_list_cnf_as_aborted(wrrc_as_id,rrcmcm_int_data.rrc_svd_plmn_req_ptr->trans_id);
      rrc_free(rrcmcm_int_data.rrc_svd_plmn_req_ptr);
      return FALSE;
    } 
    ...
    else if(cmd_ptr->cmd_hdr.cmd_id == RRC_LTE_RRC_CMD)
    {
      switch(cmd_ptr->cmd.wcdma_rrc_lte_irat_cmd.cmd.msg_hdr.id)
      {
        case WCDMA_RRC_LTE_RESEL_REQ:
          ...
          rrccsp_send_ltow_reselection_reject(wrrc_as_id);
          break;
        ...
        case WCDMA_RRC_LTE_PLMN_SRCH_REQ:
          ...
          rrcmcm_send_ltow_bplmn_search_cnf_failed(wrrc_as_id);
          break;
   ...
   */
  if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag == RRCMCM_CMD_MODE_CHANGE)
  {
  ...
  {
    WRRC_MSG1_HIGH("Received EUTRA cap rsp in rrc mode %d.", rrcmcm_int_data.rrc_mode);
    return TRUE;
  }
#endif

  /* Check the command identifier based the RRC mode */
  switch (rrcmcm_int_data.rrc_mode)
  {
    case RRC_MODE_INACTIVE:
    {
      rrc_rr_cmd_e_type rrc_rr_cmd;
      if(cmd_ptr->cmd_hdr.cmd_id != RRC_QSH_PROCESS_CMD)
      {
...
/*lint +e408*/
        ...
        case RRC_ADD_ACQ_ENTRY:
#endif
#ifdef FEATURE_DUAL_SIM
        case RRC_PROCEDURE_IND:
#endif
...
/*lint +e408*/
        ...
        case RRC_LLC_CMD_REQ:
          ...
          case RRC_QMI_CMD:
            ...
            {
              case WCDMA_RRC_QMI_IDLE_DRX_INFO_REQ:
              case WCDMA_RRC_QMI_PRIORITIZE_LTE_REQ:
              case WCDMA_RRC_QMI_CURRENT_FREQS_INFO_REQ:
                return_status = TRUE;
                break;
              default :
                return_status = FALSE;
      ...
      switch (cmd_ptr->cmd_hdr.cmd_id)
      {
        case RRC_EST_REQ:
          ...
          if((RRC_CSP_WTOG_BPLMN_AUTOMATIC_SCAN == curr_scan)
              ...
              || ((RRC_CSP_INTER_RAT_REDIRECTION_SCAN == curr_scan) && (cmd_ptr->cmd.rrc_est_req.cause == RRC_EST_DETACH))
                  /*Abort blind redirection for FRLTE cases upon voice call */
              || ((RRC_CSP_INTER_RAT_REDIRECTION_SCAN == curr_scan) && 
                  ...
                  ((cmd_ptr->cmd.rrc_est_req.cause == RRC_EST_ORIG_CONV_CALL) || (cmd_ptr->cmd.rrc_est_req.cause == RRC_EST_EMERGENCY_CALL)))
            )
          {
            rrcmcm_send_irat_abort_ind(wrrc_as_id); 
          }

          rrcmcm_save_est_req(wrrc_as_id, cmd_ptr);
          return_status = FALSE;
          break;
        ...
        case RRC_ABORT_REQ:
           ...
           */  
          if (rrcmcm_int_data.rrcmcm_est_req_saved_flag == TRUE)
          {
            WRRC_MSG0_HIGH("Abort Req in Standby mode with pending Est_req.Clearing Est_Req");
            rrcmcm_int_data.rrcmcm_est_req_saved_flag = FALSE;
            (void)rrcrce_send_rrc_abort_cnf(wrrc_as_id);
            return_status = FALSE;
          }
          else
          {
            /* Abort IRAT activity when ABORT REQ is received from NAS */
            if((rrcmcm_int_data.procedure_id == RRC_PROCEDURE_CSP) &&
...
/*lint +e408*/
        ...
        default :
          ...
          WRRC_MSG1_ERROR("Unsupported RRC Mode: %d", rrcmcm_int_data.rrc_mode);
          return_status = FALSE;
          break;
      }
    }
  ...
  if((rrc_rr_cmd_e_type)cmd_ptr->cmd_hdr.cmd_id == RRC_RR_STOP_WCDMA_REQ)
  {
    ...
    if((rrcmcm_int_data.rrc_mode == RRC_MODE_INACTIVE) && (rr_cmd->cmd.rrc_rr_stop_wcdma_req.stop_req_cause == POWER_OFF))
    {
      WRRC_MSG0_HIGH("Rxed RRC_RR_STOP_WCDMA_REQ in Inactive state");
      rrc_csp_write_nv_acq_info(wrrc_as_id);
    ...
    */
#ifdef FEATURE_DUAL_SIM
    rrc_set_as_id(wrrc_as_id,rr_cmd->cmd.rrc_rr_stop_wcdma_req.as_id);
#endif
    rrcmcm_int_data.rrcmcm_cmd_store.rrc_rr_stop_wcdma_cause = rr_cmd->cmd.rrc_rr_stop_wcdma_req.stop_req_cause;
    rrccsp_send_rrc_rr_stop_wcdma_cnf(wrrc_as_id,rrcmcm_int_data.rrcmcm_cmd_store.rrc_rr_stop_wcdma_cause); 
    return_status = FALSE;           
  }
  ...
  if((rrc_tdsrrc_cmd_e_type)cmd_ptr->cmd_hdr.cmd_id == RRC_TDSRRC_STOP_WCDMA_REQ)
  {
    ...
    if((rrcmcm_int_data.rrc_mode == RRC_MODE_INACTIVE) && (cmd_ptr->cmd.rrc_tdsrrc_stop_wcdma_req.stop_req_cause == POWER_OFF))
    {
      WRRC_MSG0_HIGH("Rxed RRC_TDSRRC_STOP_WCDMA_REQ in Inactive state");
      rrc_csp_write_nv_acq_info(wrrc_as_id);
      wl1_clear_barred_cells(wrrc_as_id);
    }
    rrcmcm_int_data.rrcmcm_cmd_store.rrc_rr_stop_wcdma_cause = cmd_ptr->cmd.rrc_tdsrrc_stop_wcdma_req.stop_req_cause;
    rrccsp_send_rrc_tdsrrc_stop_wcdma_cnf(wrrc_as_id,rrcmcm_int_data.rrcmcm_cmd_store.rrc_rr_stop_wcdma_cause); 
    return_status = FALSE;
  }
#endif
...
===========================================================================*/
rrcmcm_state_e_type rrcmcm_get_substate(sys_modem_as_id_e_type wrrc_as_id)
{
  /* return the current RRC mode */
  return (rrcmcm_int_data.rrcmcm_state);
}
/*===========================================================================
...
{
  /* return the current RRC mode */
  return (rrcmcm_int_data.rrc_mode);
}
/*===========================================================================
...
===========================================================================*/
void rrcmcm_update_rrc_mode_to_active(sys_modem_as_id_e_type wrrc_as_id)
{
  if(RRC_MODE_INTERRAT_IN_PROGRESS == rrcmcm_int_data.rrc_mode)
  {
    WRRC_MSG0_HIGH("RRC Mode updated to Active from IRAT_IN_PROGRESS");
    rrcmcm_int_data.rrc_mode = RRC_MODE_ACTIVE;
    rrcsmc_reset_pending_cs_key_to_be_activated(wrrc_as_id);
  }
  else
  {
    WRRC_MSG1_ERROR("Invalid request for rrc_mode update to active.Current rrc_mode is %d",rrcmcm_int_data.rrc_mode);
#ifdef FEATURE_WCDMA_HANDLE_MDSP_HALT
    /*Still set to ACTIVE*/
    rrcmcm_int_data.rrc_mode = RRC_MODE_ACTIVE;
#endif
...
===========================================================================*/
boolean rrcmcm_is_dualmode_enabled(sys_modem_as_id_e_type wrrc_as_id)
{
  WRRC_MSG1_HIGH("Dual mode status %d", rrcmcm_int_data.rrc_dual_mode_enabled);
  return (rrcmcm_int_data.rrc_dual_mode_enabled);
}
...
===========================================================================*/
boolean rrcmcm_is_dualmode_enabled_no_f3(sys_modem_as_id_e_type wrrc_as_id)
{
  /*WRRC_MSG1_HIGH("Dual mode status %d", rrcmcm_int_data.rrc_dual_mode_enabled);*/
  return (rrcmcm_int_data.rrc_dual_mode_enabled);
}
...
#endif
boolean rrcmcm_is_dualmode_enabled_wo_f3(sys_modem_as_id_e_type wrrc_as_id)
{
  return (rrcmcm_int_data.rrc_dual_mode_enabled);
}
...
{
  WRRC_MSG1_HIGH("LTE_IRAT: LTE mode enabled status %d ",rrcmcm_int_data.rrc_lte_mode_enabled);
  return rrcmcm_int_data.rrc_lte_mode_enabled;
}
...
===========================================================================*/
  ...
  __attribute__((section(".uncompressible.text")))
#endif
boolean rrcmcm_is_lte_mode_enabled_wo_f3
(
sys_modem_as_id_e_type wrrc_as_id
)
{
...
{
  WRRC_MSG2_HIGH("LTE_IRAT: LTE mode enabled status old:%d, new:%d ",rrcmcm_int_data.rrc_lte_mode_enabled,status);
  rrcmcm_int_data.rrc_lte_mode_enabled = status;
}
#endif
...
)
{
  ...
  rrcmcm_int_data.rrc_dual_mode_enabled = FALSE;
#if defined (FEATURE_WCDMA_TO_LTE) || defined (FEATURE_LTE_TO_WCDMA)
  rrcmcm_int_data.rrc_lte_mode_enabled = FALSE;
#endif
...
===========================================================================*/
void rrcmcm_register_for_mode_change(sys_modem_as_id_e_type wrrc_as_id,
...
)
{
  ...
  else
  {
    ...
    if (rrcmcm_int_data.rrcmcm_notification_counter < (uint16)RRCMCM_MAX_NOTIFICATIONS) 
    {
      /* Store the request into the mode change notification list */
      rrcmcm_int_data.rrcmcm_notifications[rrcmcm_int_data.rrcmcm_notification_counter].procedure = procedure;
      rrcmcm_int_data.rrcmcm_notifications[rrcmcm_int_data.rrcmcm_notification_counter].from_mode = from_mode;
      rrcmcm_int_data.rrcmcm_notifications[rrcmcm_int_data.rrcmcm_notification_counter].to_mode = to_mode;
      rrcmcm_int_data.rrcmcm_notification_counter++;
    }
    else
    {
      /* the list is full */
...
===========================================================================*/
static void rrcmcm_send_start_wcdma_req
(
  sys_modem_as_id_e_type wrrc_as_id
#ifdef FEATURE_DUAL_SIM
  ,wrm_activity_enum_type start_reason
#endif
)
{
  ...
  l1_cmd_ptr->cmd.start.is_ue_in_sglte_mode = rrcmcm_int_data.is_ue_in_sglte_mode;
#ifdef FEATURE_UMTS_1X_HANDOVER_UMTSMSM
  #error code not present
#endif

#ifdef FEATURE_DUAL_SIM
  l1_cmd_ptr->cmd.start.lte_capability_ind = rrc_dsds_int_data.rrc_lte_capability_ind;
  ...
  l1_cmd_ptr->cmd.start.reason = start_reason;
  l1_cmd_ptr->cmd.start.band_class = rrc_return_default_band_lock_req(wrrc_as_id);
  if(start_reason == RRC_IRAT_RESELECTION)
  {
    l1_cmd_ptr->cmd.start.band_class = rrc_convert_band_to_sys_band_class(wrrc_as_id, rrcmcm_int_data.start_band_class);
  }

  input_info.client_id = wrm_get_trm_client_id(WCDMA_TRM, wrrc_as_id);
  input_info.info_type = TRM_IDLE_COLL_INFO;
  trm_get_info(&input_info, &output_info);
  if(output_info.info.idle_coll_info.action_info.asid == rrc_get_as_id(wrrc_as_id))
  {
    rrc_dsds_int_data.fallback_type         = output_info.info.idle_coll_info.data.idle_fallback_data.fallback_type;
    rrc_dsds_int_data.least_priority_rat    = output_info.info.idle_coll_info.data.idle_fallback_data.least_priority_rat;
    if((rrc_dsds_int_data.fallback_type == TRM_FALLBACK_GSM_PREF) ||
       (rrc_dsds_int_data.fallback_type == TRM_FALLBACK_LTE_PREF)  ||
       (rrc_dsds_int_data.fallback_type == TRM_FALLBACK_UMTS_PREF))
    {
      WRRC_MSG2_HIGH("Fallback is active. fallback_type:%d(0-None,1-G_Pref,2-L_Pref,3-W_Pref), least_priority_rat:%d(5-W,9-L)",rrc_dsds_int_data.fallback_type, rrc_dsds_int_data.least_priority_rat);
    }
  }
#endif
...
)
{
   l1_req_cmd * llc_cmd_l1_req = NULL;
    ...
    if(rrcmcm_int_data.procedure_id == RRC_PROCEDURE_IHO)
    {
      /* Get the activation time for WtoG handover.. We will suspend WCDMA mode
        only after this activation time*/
      if( rrciho_get_activation_time_for_wtog_ho(wrrc_as_id,&(cmd_ptr->cmd.rrc_llc_req.l1_cmd.act_time)) 
        == SUCCESS)
      {
        cmd_ptr->cmd.rrc_llc_req.l1_cmd.act_time_type = L1_ACTIVATION_TIME_CFN;
        cmd_ptr->cmd.rrc_llc_req.l1_cmd.sdu_time      = rrcdata_misc_info.rrc_cfn_before_decode[rrcmcm_int_data.procedure_id]; 
      }
      else
      {
    ...
    else if(rrcmcm_int_data.procedure_id == RRC_PROCEDURE_CHO)
    {
      /* Get the activation time for WtoG Cell Change Order. We will suspend WCDMA mode
      only after this activation time*/
      if( rrccho_get_activation_time_for_wtog_cho(wrrc_as_id,&(cmd_ptr->cmd.rrc_llc_req.l1_cmd.act_time)) 
        == SUCCESS)
      {
        cmd_ptr->cmd.rrc_llc_req.l1_cmd.act_time_type = L1_ACTIVATION_TIME_CFN;
        cmd_ptr->cmd.rrc_llc_req.l1_cmd.sdu_time      = rrcdata_misc_info.rrc_cfn_before_decode[rrcmcm_int_data.procedure_id]; 
    ...
    llc_cmd_l1_req->suspend_req.cause = rrcmcm_map_irat_reason(wrrc_as_id,irat_reason);
    WRRC_MSG1_MED("IRAT_reason in Suspend Req is %d...Sending CPHY_SUSPEND_WCDMA_MODE_REQ to L1",
...
)
{
    ...
    /* Fill in the irat reason for suspension */
    llc_cmd_l1_req->resume_req.cause = rrcmcm_map_irat_reason(wrrc_as_id,irat_reason);
    WRRC_MSG1_MED("IRAT_reason in Resume Req is %d...Sending CPHY_RESUME_WCDMA_MODE_REQ to L1",
          llc_cmd_l1_req->resume_req.cause);

  /* Now send the primitive to L1 */
  rrc_put_int_cmd(wrrc_as_id, cmd_ptr);
#ifdef FEATURE_DUAL_SIM
  if(irat_reason == INTERRAT_DSIM_TUNEAWAY)
  {
    /* This is to prevent back to back LTA gaps. Timer value for now is 200ms 
     * out of which 100ms is for resume and channel config
     */
    rrctmr_start_timer(wrrc_as_id,RRCTMR_DELAY_LTA_TIMER, 200);
  }
#endif
  return;
...
===========================================================================*/
static void rrcmcm_send_stop_wcdma_cnf(sys_modem_as_id_e_type wrrc_as_id,boolean append_rel_ind)
{
...
#ifdef FEATURE_DUAL_SIM
  mm_buf_ptr->cmd.rrc_stop_wcdma_mode_cnf.as_id = rrc_get_as_id(wrrc_as_id);
#endif
    ...
    )
  {
    ...
    (void) rrcsmc_append_hfns_to_rel_ind(wrrc_as_id,&(mm_buf_ptr->cmd.rrc_stop_wcdma_mode_cnf.start_cs[0]),
  ...
  rrc_csp_int_data.rrc_csfb_reset_while_waiting_for_act_rsp = FALSE;
#endif
#endif
...
if(l1_get_remaining_time_for_backoff_dc_cap(wrrc_as_id) == 0)
{
  ...
  /* W is stopped before 20 min timer started or timer has expired,
...
===========================================================================*/
static void rrcmcm_send_service_cnf_with_failure(sys_modem_as_id_e_type wrrc_as_id)
{
  mm_cmd_type *mm_buf_ptr;
  ...
  if(rrcmcm_int_data.rrcmcm_act_cause == RRCMCM_INTERRAT_CC_ORDER_GTOW)
  {
    mm_buf_ptr->cmd.rrc_service_cnf.status = MM_AS_INTERRAT_CC_ORDER;
  }
  /* Check current RRC state */
  else if (rrcmcm_int_data.rrcmcm_act_cause == RRCMCM_INTERRAT_HANDOVER_GTOW) 
  {
    /* Set status to MM_AS_INTERRAT_HANDOVER */
    mm_buf_ptr->cmd.rrc_service_cnf.status = MM_AS_INTERRAT_HANDOVER;
  }
  else if(rrcmcm_int_data.rrcmcm_act_cause == RRCMCM_INTERRAT_CC_ORDER_WTOG_FAILED)
  {
...
===========================================================================*/
static void rrcmcm_send_stop_lower_layer_req(sys_modem_as_id_e_type wrrc_as_id, boolean due_to_stop_mode )
{
  rrc_cmd_type *cmd_ptr;

  /* Check if WTOG abort is in progress. If so, wait for RR to respond*/
  if ( (rrcmcm_int_data.rrc_mode == RRC_MODE_STANDBY  ) &&
       (rrccsp_check_wtog_abort_in_progress(wrrc_as_id,rrcmcm_int_data.stop_cause)) 
     )
  {
  ...
  if(RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS())
  {
    rrccsp_check_and_teardown_sbcch_during_wrlf(wrrc_as_id);
  }
#endif
  rrccsp_check_and_teardown_sbcch_during_bplmn(wrrc_as_id);

  rrccsp_cleanup_bplmn_state(wrrc_as_id);

  if(rrc_csp_int_data.curr_scan == RRC_CSP_PLMN_LIST_SCAN)
  {
    rrc_csp_send_mm_plmn_list_cnf_as_aborted(wrrc_as_id,rrc_csp_int_data.bplmn_trans_id);
  }
  ...
  /* Set the cause field */
  if (rrcmcm_int_data.stop_cause == RRC_DEEP_SLEEP)
  {
    ...
    if(due_to_stop_mode)
    {
      rrccsp_set_ue_in_deep_sleep(wrrc_as_id);
    }
  }
  else
  {
  ...
  /* Put the command on the internal queue */
  rrc_put_int_cmd(wrrc_as_id,  cmd_ptr );
  WRRC_MSG1_HIGH("RRC_STOP_LOWER_LAYER_REQ to LLC:%d", rrcmcm_int_data.stop_cause);
...
===========================================================================*/
static void rrcmcm_send_mode_change_ind(sys_modem_as_id_e_type wrrc_as_id,
  rrc_proc_e_type procedure,
  rrc_mode_e_type from_mode,
  rrc_mode_e_type to_mode 
)
...
===========================================================================*/
static void rrcmcm_send_change_mode_cnf(sys_modem_as_id_e_type wrrc_as_id,boolean status)
{
  rrc_cmd_type *cmd_ptr;
  ...
  /* Fill in procedure id that will receive this indication */
  cmd_ptr->cmd.change_mode_cnf.procedure = rrcmcm_int_data.procedure_id;
  cmd_ptr->cmd.change_mode_cnf.status = status;
  if(rrcmcm_int_data.rrcmcm_state == RRCMCM_WAIT_STOP_CNF_FOR_MODE_CHANGE)
  {
    cmd_ptr->cmd.change_mode_cnf.mode = RRC_MODE_CHANGE;
  }
  else
  {
    cmd_ptr->cmd.change_mode_cnf.mode = rrcmcm_int_data.rrc_mode;
  }
  
#ifdef FEATURE_DUAL_SIM
  cmd_ptr->cmd.change_mode_cnf.fail_type = rrc_dsds_int_data.wl1_failure_cause;
#endif

  /* Put the command on the internal queue */
  rrc_put_int_cmd(wrrc_as_id,  cmd_ptr );
  WRRC_MSG1_HIGH("RRC_CHANGE_MODE_CNF sent to %d", rrcmcm_int_data.procedure_id);
  ...
  if(rrcmcm_int_data.rrcmcm_est_req_saved_flag == TRUE)
  {
    rrcmcm_process_est_req(wrrc_as_id,rrcmcm_int_data.rrc_mode);
  }
#ifdef FEATURE_DUAL_SIM
  /* Do not release lock for standby if W2G IHO is active because lock needs to be exchanged */
  if((rrcmcm_int_data.rrc_mode == RRC_MODE_STANDBY) && !(is_iho_w2g_active(wrrc_as_id)) && !(is_redir_w2x_active(wrrc_as_id)))
  {
#ifdef FEATURE_TRIPLE_SIM
   /* Make sure the tuneaway type is LTA before releasing lock  */
    wcdma_rrc_intlock(wrrc_as_id);
    if(rrcwrm_release_lock_if_current_tuneaway_is_lta(wrrc_as_id) == TRUE)
    {
#endif
    rrc_release_lock(wrrc_as_id);
#ifdef FEATURE_TRIPLE_SIM
    }
    wcdma_rrc_intlfree(wrrc_as_id);
#endif
  }
#endif
  return;
...
===========================================================================*/
void rrcmcm_send_irat_abort_ind(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_cmd_type *cmd_ptr;
  ...
  /* Fill in procedure id that will receive this indication */
  cmd_ptr->cmd.rrc_irat_abort_ind.procedure = rrcmcm_int_data.procedure_id;
  
  /* Put the command on the internal queue */
  rrc_put_int_cmd(wrrc_as_id,  cmd_ptr );
  WRRC_MSG1_HIGH("RRC_IRAT_ABORT_IND sent to %d", rrcmcm_int_data.procedure_id);
...
===========================================================================*/
static void rrcmcm_update_mode(sys_modem_as_id_e_type wrrc_as_id,
...
)
{
  ...
  if (rrcmcm_int_data.rrc_mode != new_mode) 
  {
    /* Check through the mode change notification table to send 
    RRC_MODE_CHANGE_IND to the registerred procedures */
    for (i = 0; i < rrcmcm_int_data.rrcmcm_notification_counter; i++) 
    {
      /* If the registerred condition is met, then send the indication */
      if ( ((rrcmcm_int_data.rrcmcm_notifications[i].from_mode == rrcmcm_int_data.rrc_mode) ||
        (rrcmcm_int_data.rrcmcm_notifications[i].from_mode == RRC_MODE_WILDCARD)) &&
           ((rrcmcm_int_data.rrcmcm_notifications[i].to_mode == new_mode)  ||
        (rrcmcm_int_data.rrcmcm_notifications[i].to_mode == RRC_MODE_WILDCARD)))
      {
        rrcmcm_send_mode_change_ind(wrrc_as_id,rrcmcm_int_data.rrcmcm_notifications[i].procedure,
            rrcmcm_int_data.rrcmcm_notifications[i].from_mode,
            rrcmcm_int_data.rrcmcm_notifications[i].to_mode);
      }
    }    
  }
  
  /* update the RRC Mode and RAT Preference */
  rrcmcm_int_data.rrc_mode = new_mode;

  if(rrcmcm_int_data.rrc_mode == RRC_MODE_ACTIVE)
  {
    rrcsmc_reset_pending_cs_key_to_be_activated(wrrc_as_id);
  }
  ...
  if(new_mode == RRC_MODE_INACTIVE)
  {
    ...
    if((rrcmcm_int_data.procedure_id == RRC_PROCEDURE_IHO)
       ...
       || ((rrc_csp_int_data.rrc_csfb_call_status) && 
           (rrccsp_get_curr_scan_no_f3(wrrc_as_id) == RRC_CSP_GSM_TO_WCDMA_REDIRECTION_SCAN)
           )
#endif
      )
    {
...
#endif
#if defined(FEATURE_THOR_MODEM) || defined(FEATURE_WCDMA_ATLAS_MODEM) || defined(FEATURE_TABASCO_MODEM)
  rrc_update_cxm_with_state(wrrc_as_id,rrc_get_state(wrrc_as_id));
#endif
...
)
{
  ...
  WRRC_MSG4_HIGH("Rvd RRC_CHANGE_MODE_REQ from proc:rrc_proc_e_type_value%d. Current RRC Mode:%d,Requested mode:%d, change reason %d",
                  change_mode_req.procedure,rrcmcm_int_data.rrc_mode,change_mode_req.mode,change_mode_req.reason);

  /* Save the destination procedure for confirmation */
  rrcmcm_int_data.procedure_id = change_mode_req.procedure;
  ...
  switch (change_mode_req.mode)
  {
    ...
    case RRC_MODE_STANDBY:
      ...
      switch(rrcmcm_int_data.rrc_mode)
      {
        ...
        case RRC_MODE_ACTIVE:
          ...
          rrcmcm_send_suspend_wcdma_mode_req(wrrc_as_id,change_mode_req.reason);  

          /* Change the current mode to Standby */
          rrcmcm_update_mode(wrrc_as_id, RRC_MODE_STANDBY);

          rrcmcm_int_data.rrcmcm_state = RRCMCM_WAIT_SUSPEND_CNF;
          break;
        /* Current RRC Mode is Standby */
        case RRC_MODE_STANDBY:
          /* Send Mode confirmation right away */ 
          rrcmcm_send_change_mode_cnf(wrrc_as_id, TRUE);
        ...
        case RRC_MODE_INTERRAT_IN_PROGRESS:
          ...
          WRRC_ERR0_FATAL("Invalid change mode req.Curr mode IRAT_IN_PROGRESS, Req mode STANDBY");
          break;
#endif
        /* Others */
        default:
          /* Invald mode change request */
          WRRC_MSG1_ERROR("Mode change invald from %d to Standby", rrcmcm_int_data.rrc_mode);
          break;
      }
      break;
    ...
    case RRC_MODE_ACTIVE:
      ...
      switch(rrcmcm_int_data.rrc_mode)
      {
        case RRC_MODE_STANDBY:
#ifdef FEATURE_DUAL_SIM
            rrc_perform_trm_exchange(wrrc_as_id);
            if(rrc_state == RRC_STATE_CELL_DCH || rrc_state == RRC_STATE_CELL_FACH || rrc_state == RRC_STATE_CONNECTING)
            {
              rrc_csp_scan_e_type curr_scan = rrccsp_get_curr_scan_no_f3(wrrc_as_id);
              trm_reason_enum_t  priority = rrc_determine_trm_priority(wrrc_as_id);
              ...
              if(curr_scan == RRC_CSP_UNLOCK_BY_SCAN)
              {
                ...
                */
                wl1_fill_tuneaway_duration(0,wrrc_as_id);
                /* Stop the lock wait timer if running */
                rrc_stop_lock_wait_timer(wrrc_as_id);
                rrctmr_start_timer(wrrc_as_id,RRCTMR_MAX_TA_DURATION_TIMER, rrc_get_max_ta_duration(wrrc_as_id));

                rrc_wrm_populate_bands_for_lock_req(wrrc_as_id, &lock_req_bands, rrc_ccm_get_curr_camped_freq(wrrc_as_id));
                /*Send the tune away start event for QXDM logging*/
                WRRC_EVENT_REPORT(EVENT_WCDMA_RRC_DSIM_TUNEAWAY_START);
                rrc_send_rlc_ta_status(wrrc_as_id, RLC_LTA_START_IND, RLC_TA_SUCC_MAX_ENUM);
                if((rrc_block_wait_to_acquire_lock_for_time(wrrc_as_id,150, priority, lock_req_bands,TRUE) == FAILURE)
                    &&((rrctmr_get_remaining_time(wrrc_as_id, RRCTMR_MAX_TA_DURATION_TIMER)) != 0))
                {
                  /* Send a change mode cnf with failure */
                  rrc_dsds_int_data.wl1_failure_cause = WL1_FAILED_NO_WRM_LOCK;
                  rrcmcm_update_mode(wrrc_as_id, RRC_MODE_ACTIVE);
                  rrcmcm_send_change_mode_cnf(wrrc_as_id, FALSE);
                  rrcmcm_init_data(wrrc_as_id, INIT_ALL);
                  rrc_send_rlc_ta_status(wrrc_as_id, RLC_LTA_STOP_IND, RLC_FAIL_IND);
                  return;
                }

                /*Send the tune away stop event for QXDM logging*/
                WRRC_EVENT_REPORT(EVENT_WCDMA_RRC_DSIM_TUNEAWAY_STOP);
                
              }
              else
              if(rrc_acquire_lock_for_resuming_on_wcdma(wrrc_as_id) == FAILURE)
              {
          ...
          rrcmcm_send_resume_wcdma_mode_req(wrrc_as_id,change_mode_req.reason);
          rrcmcm_int_data.rrcmcm_state = RRCMCM_WAIT_RESUME_CNF;
          break;
        case RRC_MODE_ACTIVE:
          /* Send Mode confirmation right away */
          rrcmcm_send_change_mode_cnf(wrrc_as_id, TRUE);
        ...
        default:
          /* Invald mode change request */
          WRRC_MSG1_ERROR("Mode change invald from %d to Active", rrcmcm_int_data.rrc_mode);
          break;
      }           
      break;
    ...
    case RRC_MODE_INTERRAT_IN_PROGRESS:
      /* Check for the current RRC mode */
      if (rrcmcm_int_data.rrc_mode == RRC_MODE_INACTIVE) 
      {
        rrc_init_dynamic_ptrs(wrrc_as_id);
        /*Update the substate and mode before posting the req to L1 as L1 will start processing before we updating the substate*/
        rrcmcm_update_mode(wrrc_as_id, RRC_MODE_INTERRAT_IN_PROGRESS);
        rrcmcm_int_data.rrcmcm_state = RRCMCM_WAIT_START_CNF;
        /* Start WCDMA mode */
#ifdef FEATURE_DUAL_SIM
        switch(change_mode_req.reason)
        {
          /*IHO start reason doesnt matter. As RRC already
            takes lock with correct reason*/
          case INTERRAT_IHO:
            start_reason = RRC_CS_HANDOVER;
            break;

          case INTERRAT_CCO:
            start_reason = RRC_CELL_CHANGE_ORDER;
            break;

          default:
            start_reason = RRC_SERVICE_SEARCH;
            break;
        }
        rrcmcm_send_start_wcdma_req(wrrc_as_id,start_reason);
#else
        rrcmcm_send_start_wcdma_req(wrrc_as_id);
#endif
      }
      else
      {
        /* Invald mode change request */
        WRRC_MSG1_ERROR("Mode change invald from %d to INP", rrcmcm_int_data.rrc_mode);
      }
      break;
    ...
    case RRC_MODE_INACTIVE:
      ...
      if((change_mode_req.procedure == RRC_PROCEDURE_CHO) && (rrcmcm_int_data.rrc_mode == RRC_MODE_STANDBY))
      {
        ...
        rrcmcm_send_stop_lower_layer_req(wrrc_as_id,due_to_stop_mode);
        rrcmcm_int_data.rrcmcm_state = RRCMCM_WAIT_STOP_CNF;
      }
      /* Check the current RRC mode */
      else if (rrcmcm_int_data.rrc_mode == RRC_MODE_INTERRAT_IN_PROGRESS) 
      {
        due_to_stop_mode = TRUE;
        /* send stop wcdma request */
        rrcmcm_send_stop_lower_layer_req(wrrc_as_id,due_to_stop_mode);
...
#ifdef FEATURE_DUAL_SIM
      else if((change_mode_req.procedure == RRC_PROCEDURE_CSP) && (rrcmcm_int_data.rrc_mode == RRC_MODE_ACTIVE))
      {
        rrcmcm_int_data.stop_cause = RRC_RAT_CHANGE;
        due_to_stop_mode = TRUE;
        /* send stop wcdma request */
      ...
      {
        /* Invald mode change request */
        WRRC_MSG1_ERROR("Mode change invald from %d to Inactive", rrcmcm_int_data.rrc_mode);
      }      
      break;
    ...
    case RRC_MODE_CHANGE:
      if(rrcmcm_int_data.rrc_mode == RRC_MODE_INACTIVE)
      {
        rrcmcm_int_data.rrcmcm_state = RRCMCM_WAIT_STOP_CNF_FOR_MODE_CHANGE;
        rrcmcm_send_change_mode_cnf(wrrc_as_id, TRUE);
        rrcmcm_send_mode_change_ind(wrrc_as_id,RRC_PROCEDURE_IHO, 
                                    RRC_MODE_CHANGE,
                                    RRC_MODE_INACTIVE);
        rrcmcm_update_mode(wrrc_as_id, RRC_MODE_INACTIVE);
        rrcmcm_init_data(wrrc_as_id, INIT_ALL);
      }
      else
      {
        ...
        rrcmcm_int_data.stop_cause = RRC_DEEP_SLEEP;
...
===========================================================================*/
static void rrcmcm_send_lte_interrat_plmn_rsp_with_failure(sys_modem_as_id_e_type wrrc_as_id)
{
...
===========================================================================*/
boolean rrcmcm_acquire_lock_and_calc_rem_dur
(
  sys_modem_as_id_e_type wrrc_as_id,
  uint32* inter_rat_timer_val,
...
/* This is to make sure that W being the dest RAT takes care of trm exchange if performed by source RAT */
  ...
  if(*inter_rat_timer_val < 120)
  {
    return FALSE;
  }

  rrc_start_lock_wait_timer(wrrc_as_id,*inter_rat_timer_val - 120,TRUE);
  /* TRM priority and lock request time same as used by L1 for start wcdma req */
  wrm_lock_status = rrc_acquire_wrm_lock(wrrc_as_id,priority,50, lock_req_bands);
 
  if(wrm_lock_status == RRC_WRM_LOCK_ACQUIRED) 
  {
    rem_lock_timer = rrctmr_get_remaining_time(wrrc_as_id, RRCTMR_WRM_LOCK_TIMER);
    *inter_rat_timer_val=(uint32)rem_lock_timer+120; 
    rrc_stop_lock_wait_timer(wrrc_as_id);
    return TRUE;
  }
  else if(wrm_lock_status == RRC_ABORT_FROM_RR)
  {
    ...
    switch( rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag)
    {
    
#ifdef FEATURE_LTE_TO_WCDMA

      case RRCMCM_CMD_LTE_PLMN_SRCH_REQ:
        rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag = RRCMCM_LTE_ABORT_PLMN_SRCH_REQ;
      break;

      case RRCMCM_CMD_LTE_IRAT_REQ:
        if(rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_lte_irat_cmd.cmd.wcdma_rrc_lte_irat_cmd.cmd.msg_hdr.id == WCDMA_RRC_LTE_GET_CGI_REQ)
        {
          rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag = RRCMCM_LTE_ABORT_CGI_REQ;
        }
        else
        { 
          /* No action */
      ...
      case RRCMCM_CMD_INTERRAT_PLMN_SRCH_REQ:
        rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag = RRCMCM_RR_INTERRAT_PLMN_ABORT_REQ;
      break;
...
===========================================================================*/
static void rrcmcm_send_interrat_plmn_cnf_with_failure(sys_modem_as_id_e_type wrrc_as_id)
{
...
===========================================================================*/
static void rrcmcm_handle_start_wcdma_failure_for_dual_sim(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  */
  ...
  if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag == RRCMCM_CMD_MODE_CHANGE)
  {
    rrcmcm_int_data.procedure_id = RRC_PROCEDURE_CSP;
    rrcmcm_int_data.rrcmcm_state = RRCMCM_WAIT_STOP_CNF_FOR_MODE_CHANGE;
    rrcmcm_send_change_mode_cnf(wrrc_as_id, TRUE);
    rrcmcm_update_mode(wrrc_as_id, RRC_MODE_INACTIVE);
    rrcmcm_init_data(wrrc_as_id, INIT_ALL);
  }
  else if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag == RRCMCM_CMD_STOP_WCDMA_REQ)
  {
    rrcmcm_int_data.rrcmcm_send_stop_cnf_to_mm_flag = TRUE;
    /* Clear RRC change mode requests,if any */
    rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_change_mode_req.mode = RRC_MODE_MAX;
  
    /*TBD: check if mode_change_ind needs to be sent or not*/
    rrcmcm_send_mode_change_ind(wrrc_as_id,RRC_PROCEDURE_CSP, rrcmcm_int_data.rrc_mode, 
                                RRC_MODE_INACTIVE);
    rrcmcm_send_mode_change_ind(wrrc_as_id,RRC_PROCEDURE_CCM, rrcmcm_int_data.rrc_mode,
                                RRC_MODE_INACTIVE);
    rrcmcm_update_mode(wrrc_as_id, RRC_MODE_INACTIVE);
    /* Send a Stop WCDMA Mode Confirmation to MM */
    rrcmcm_send_stop_wcdma_cnf(wrrc_as_id, TRUE);
    rrcmcm_init_data(wrrc_as_id, INIT_ALL);
  }
  else if((rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag == RRCMCM_RR_INTERRAT_PLMN_ABORT_REQ)
           ||(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag == RRCMCM_RR_INTERRAT_RESELECTION_ABORT_REQ)
           ||(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag == RRCMCM_RR_INTERRAT_REDIRECT_ABORT_REQ)
           ||(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag == RRCMCM_RR_INTERRAT_PLMN_ABORT_REQ)
#ifdef FEATURE_LTE_TO_WCDMA
           ||(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag == RRCMCM_LTE_ABORT_PLMN_SRCH_REQ)
           ||(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag == RRCMCM_LTE_ABORT_REDIR_REQ)
           ||(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag == RRCMCM_LTE_ABORT_RESEL_REQ)
           ||(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag == RRCMCM_LTE_ABORT_CGI_REQ)
#endif
         )
  {
    if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag == RRCMCM_RR_INTERRAT_PLMN_ABORT_REQ)
    {
      rrccsp_send_bplmn_abort_cnf_to_rr(wrrc_as_id);
    }
    else if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag == RRCMCM_RR_INTERRAT_RESELECTION_ABORT_REQ)
    {
      rrccsp_send_interrat_reselection_abort_cnf(wrrc_as_id);
    }
    else if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag == RRCMCM_RR_INTERRAT_REDIRECT_ABORT_REQ)
    {
      rrccsp_send_gtow_redirection_abort_cnf(wrrc_as_id);
    }
...
#ifdef FEATURE_LTE_TO_WCDMA
    else if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag == RRCMCM_LTE_ABORT_PLMN_SRCH_REQ)
    {
      rrccsp_send_lte_interrat_plmn_srch_abort_rsp(wrrc_as_id);
    }
    else if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag == RRCMCM_LTE_ABORT_REDIR_REQ)
    {
      rrccsp_send_lte_interrat_redirection_abort_rsp(wrrc_as_id);
    }
    else if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag == RRCMCM_LTE_ABORT_RESEL_REQ)
    {
      rrccsp_send_lte_interrat_reselection_abort_rsp(wrrc_as_id);
    }
    else if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag == RRCMCM_LTE_ABORT_CGI_REQ)
    {
      rrccsp_send_lte_interrat_cgi_abort_rsp(wrrc_as_id);
    }
  ...
  */
  ...
  {
    if(rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_gsm_irat_cmd.cmd_hdr.cmd_id== RRC_INTERRAT_RESELECTION_REQ)
    {
      /* Send the Reselection Rej to GSM RR */
      rrccsp_send_interrat_reselection_reject(wrrc_as_id);
    }
    else if(rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_gsm_irat_cmd.cmd_hdr.cmd_id == RRC_INTERRAT_REDIRECT_REQ)
    {
      /* Send the Redirection Rej to GSM RR */
      rrccsp_send_gtow_redirection_reject(wrrc_as_id);
    }
#ifdef FEATURE_LTE_TO_WCDMA
    else if(rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_lte_irat_cmd.cmd.wcdma_rrc_lte_irat_cmd.cmd.msg_hdr.id == WCDMA_RRC_LTE_RESEL_REQ)
    {
      rrccsp_send_lte_interrat_reselection_failed_rsp(wrrc_as_id);
    }
    else if(rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_lte_irat_cmd.cmd.wcdma_rrc_lte_irat_cmd.cmd.msg_hdr.id == WCDMA_RRC_LTE_REDIR_REQ)
    {
      rrccsp_send_lte_interrat_redirection_failed_rsp(wrrc_as_id);
    }
    else if(rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_lte_irat_cmd.cmd.wcdma_rrc_lte_irat_cmd.cmd.msg_hdr.id == WCDMA_RRC_LTE_GET_CGI_REQ)
    {
      rrccsp_send_lte_interrat_cgi_rsp(wrrc_as_id);
    }
#endif
  ...
  else if(rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_gsm_irat_cmd.cmd_hdr.cmd_id == RRC_INTERRAT_PLMN_SRCH_REQ)
  {
    rrcmcm_send_interrat_plmn_cnf_with_failure(wrrc_as_id);
    rrcmcm_update_mode(wrrc_as_id, RRC_MODE_INACTIVE);
    rrcmcm_init_data(wrrc_as_id, INIT_ALL);
  }
#ifdef FEATURE_LTE_TO_WCDMA
  else if(rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_lte_irat_cmd.cmd.wcdma_rrc_lte_irat_cmd.cmd.msg_hdr.id == WCDMA_RRC_LTE_PLMN_SRCH_REQ)
  {
    rrcmcm_send_lte_interrat_plmn_rsp_with_failure(wrrc_as_id);
    rrcmcm_update_mode(wrrc_as_id, RRC_MODE_INACTIVE);
    rrcmcm_init_data(wrrc_as_id, INIT_ALL);
  }
#endif
  else
  {
    ...
    rrcmcm_send_start_wcdma_req(wrrc_as_id,RRC_SERVICE_SEARCH);
    rrcmcm_int_data.rrcmcm_state = RRCMCM_WAIT_START_CNF;
...
===========================================================================*/
static void rrcmcm_acquire_lock_for_wcdma_start
(
  sys_modem_as_id_e_type wrrc_as_id,
  uint32* timer_val,
  trm_reason_enum_t priority
...
{
  if(rrcmcm_acquire_lock_and_calc_rem_dur(wrrc_as_id,timer_val,priority))
  {
    rrcmcm_int_data.rrcmcm_state = RRCMCM_WAIT_START_CNF;
    rrcmcm_send_start_wcdma_req(wrrc_as_id,RRC_BPLMN_SEARCH);
  }
  else
  {
    rrc_stop_lock_wait_timer(wrrc_as_id);
    rrcmcm_handle_start_wcdma_failure_for_dual_sim(wrrc_as_id);
  }
}
...
)
{
  ...
  change_mode_req.reason = INTERRAT_REASON_NONE;
  /* For a mode change set the cause to be sent to L1 as DEEP SLEEP */
  rrcmcm_int_data.stop_cause = RRC_DEEP_SLEEP;

  process_change_mode_req(wrrc_as_id,change_mode_req);
}

...
===========================================================================*/
static void rrcmcm_initial_substate_event_handler(sys_modem_as_id_e_type wrrc_as_id,
...
)
{
     ...
     switch(rrcmcm_int_data.rrc_mode)
     {
      case RRC_MODE_INACTIVE:
       ...
       case RRC_MODE_INTERRAT_IN_PROGRESS:
         ...
         rrc_cell_selection_procedure( wrrc_as_id, cmd_ptr );                
         break;
  ...
  else if(rrc_rr_cmd == RRC_INTERRAT_PLMN_SRCH_REQ)
  {
      switch(rrcmcm_int_data.rrc_mode)
      {
        case RRC_MODE_INACTIVE:
          ...
          /* Process the request */
          rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_gsm_irat_cmd = *((rrc_rr_cmd_type *)cmd_ptr);     // Store the command
          rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag = RRCMCM_CMD_INTERRAT_PLMN_SRCH_REQ ;                    // Set the cmd flag
          rrc_init_dynamic_ptrs(wrrc_as_id);

#ifdef FEATURE_DUAL_SIM
          if(rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_gsm_irat_cmd.cmd.interrat_plmn_srch_req.high_priority)
          {
          rrcmcm_acquire_lock_for_wcdma_start(wrrc_as_id,&rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_gsm_irat_cmd.
                cmd.interrat_plmn_srch_req.timer_val,TRM_ACQUISITION_INV);
          }
          else
          {
            rrcmcm_acquire_lock_for_wcdma_start(wrrc_as_id,&rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_gsm_irat_cmd.
                cmd.interrat_plmn_srch_req.timer_val,TRM_ACQUISITION);
          }
#else
          /*Update the substate and mode before posting the req to L1 as L1 will start processing before we updating the substate*/
          rrcmcm_int_data.rrcmcm_state = RRCMCM_WAIT_START_CNF;                                                     // Set substate to RRCMCM_WAIT_START_CNF
          rrcmcm_send_start_wcdma_req(wrrc_as_id);                                                            // Start wCDMA
#endif
          break;
  ...
  else if(rrc_rr_cmd == RRC_INTERRAT_RESELECTION_REQ)
  {
    switch(rrcmcm_int_data.rrc_mode)
    {
      case RRC_MODE_INACTIVE:
      {
        rrc_interrat_reselect_req_type *gsm_resel_req = &((rrc_rr_cmd_type *)cmd_ptr)->cmd.interrat_reselection_req;
        WRRC_MSG0_HIGH("Rvd RRC_INTERRAT_RESELECTION_REQ from RR");
        /* Start WCDMA mode process */
        rrcmcm_int_data.rrc_mode = RRC_MODE_INTERRAT_IN_PROGRESS;
        rrcmcm_int_data.procedure_id = RRC_PROCEDURE_CSP;
        rrc_init_dynamic_ptrs(wrrc_as_id);
        /*Update the substate and mode before posting the req to L1 as L1 will start processing before we updating the substate*/
        rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_gsm_irat_cmd = *((rrc_rr_cmd_type *)cmd_ptr);
        rrcmcm_int_data.start_band_class = rrc_get_frequency_band(
                                              wrrc_as_id, 
                                             (rrc_freq_type)(gsm_resel_req->UARFCN),
                                              mmumts_convert_nas_plmn_id_to_rrc_plmn_id(gsm_resel_req->requested_PLMN));
        rrcmcm_int_data.rrcmcm_state = RRCMCM_WAIT_START_CNF;
#ifdef FEATURE_DUAL_SIM
        rrcmcm_send_start_wcdma_req(wrrc_as_id,RRC_IRAT_RESELECTION);
#else
        rrcmcm_send_start_wcdma_req(wrrc_as_id);
#endif
        break;
  ...
  {
    WRRC_MSG1_HIGH("Rvd RRC_INTERRAT_RESELECTION_ABORT_REQ from RR in Initial substate,rrc_mode: %d",rrcmcm_int_data.rrc_mode);
      ...
      switch(rrcmcm_int_data.rrc_mode)
      {
        ...
        default:
          ...
          WRRC_MSG1_HIGH("RRC_INTERRAT_RESELECTION_ABORT_REQ in Invalid mode,rrc_mode: %d",rrcmcm_int_data.rrc_mode);
          break;
      }
  }
    ...
    switch(rrcmcm_int_data.rrc_mode)
      {
        case RRC_MODE_INACTIVE:
          ...
          rrcmcm_int_data.rrcmcm_state = RRCMCM_WAIT_START_CNF;
#ifdef FEATURE_DUAL_SIM
          rrcmcm_send_start_wcdma_req(wrrc_as_id,RRC_IRAT_REDIRECTION);
#else
          rrcmcm_send_start_wcdma_req(wrrc_as_id);
#endif
  ...
  {
    WRRC_MSG1_HIGH("Rvd RRC_INTERRAT_REDIRECT_ABORT_REQ from RR in Initial substate,rrc_mode: %d",rrcmcm_int_data.rrc_mode);
    ...
    break;
        ...
        default:
          /* We should not be getting command in any other substate */
          WRRC_MSG1_HIGH("RRC_INTERRAT_REDIRECT_ABORT_REQ in Invalid mode,rrc_mode: %d",rrcmcm_int_data.rrc_mode);
    break;
      }
  }
  else
...
/*lint +e740*/
    ...
    case RRC_MODE_CHANGE_REQ:
      ...
      switch(rrcmcm_int_data.rrc_mode)
      {
        case RRC_MODE_STANDBY:
          ...
          rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_mm_req_cmd = *cmd_ptr;
          rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag = RRCMCM_CMD_MODE_CHANGE_REQ;
    ...
    case RRC_DEACT_REQ:
      ...
      if(rrcmcm_int_data.rrc_mode == RRC_MODE_STANDBY)
      {
        ...
        rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_mm_req_cmd = *cmd_ptr;
        rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag = RRCMCM_CMD_DEACT_REQ;
    ...
    case RRC_SERVICE_REQ:
      ...
      {       
        ...
        case RRC_MODE_ACTIVE:
#ifdef FEATURE_RAT_PRIORITY_LIST
          rrcmcm_update_mode_capability_from_service_req(wrrc_as_id,&cmd_ptr->cmd.rrc_service_req.rat_pri_list_info);
#else
          /* Set the dual-mode enable flag */
          rrcmcm_int_data.rrc_dual_mode_enabled = cmd_ptr->cmd.rrc_service_req.dual_mode;
#endif
        ...
        case RRC_MODE_INACTIVE:
          ...
          /*Update the substate and mode before posting the req to L1 as L1 will start processing before we updating the substate*/
          rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag = RRCMCM_CMD_SERVICE_REQ;
          rrcmcm_int_data.rrcmcm_state = RRCMCM_WAIT_START_CNF;
          /* Send a Start WCDMA mode request to L1 */
#ifdef FEATURE_DUAL_SIM
          if(((cmd_ptr->cmd.rrc_service_req.network_select_mode == SYS_NETWORK_SELECTION_MODE_LIMITED) && 
              (cmd_ptr->cmd.rrc_service_req.service_req_cause == RRC_MO_CS_CALL))
#ifdef FEATURE_3GPP_CSFB
              || (rrc_csp_int_data.rrc_csfb_call_status)
#endif
              || ((TRM_TRAFFIC == rrc_determine_trm_priority(wrrc_as_id)) && (RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS()))
            )
          {
            rrc_out_of_lock_wait_e_type wrm_lock_status;
            wrm_freq_input_type lock_req_bands;
            /* Exchange is not expected to be successful in this case. For protection sake retained the code */
            rrc_perform_trm_exchange(wrrc_as_id);
            lock_req_bands.num_bands = 1;
            lock_req_bands.bands[0].band = rrc_return_default_band_lock_req(wrrc_as_id);
            rrc_start_lock_wait_timer(wrrc_as_id,RRCTMR_DEFAULT_WRM_LOCK_WAIT_TIME_IN_MS,TRUE);
            wrm_lock_status = rrc_acquire_wrm_lock(wrrc_as_id, TRM_TRAFFIC, 0xFFFFFFFF, lock_req_bands);
            rrc_stop_lock_wait_timer(wrrc_as_id);
            if(wrm_lock_status == RRC_WRM_LOCK_ACQUIRED)
            {
              rrcmcm_send_start_wcdma_req(wrrc_as_id,RRC_CSFB_CALL);
            }
            else /* Recovery Handling */
            {
              rrcmcm_send_start_wcdma_req(wrrc_as_id,RRC_SERVICE_SEARCH);
            }
          }
        ...
        default:
          ...
          rrcmcm_send_service_cnf_with_failure(wrrc_as_id);
          break;
      }
      break;    
    ...
    case RRC_CHANGE_MODE_IND:
      {
        ...
        boolean prev_dual_mode_status = rrcmcm_int_data.rrc_dual_mode_enabled;
        boolean mode_status_changed = FALSE;

#if defined (FEATURE_WCDMA_TO_LTE) || defined (FEATURE_LTE_TO_WCDMA)
        boolean prev_lte_mode_status = rrcmcm_int_data.rrc_lte_mode_enabled;
#endif
        WRRC_MSG0_HIGH("RRC_CHANGE_MODE_IND is received from MM");


#ifdef FEATURE_RAT_PRIORITY_LIST
        gw_band_pref = rrc_extract_gw_band_pref_from_rat_pri_list(wrrc_as_id,
                                   &cmd_ptr->cmd.rrc_change_mode_ind.rat_pri_list_info);
        rrcmcm_update_mode_capability(wrrc_as_id,&cmd_ptr->cmd.rrc_change_mode_ind.rat_pri_list_info);
        if(!prev_dual_mode_status && rrcmcm_int_data.rrc_dual_mode_enabled)  
        {
          WRRC_MSG1_HIGH("GSM mode enabled gsm_mode %d",
                rrcmcm_int_data.rrc_dual_mode_enabled);
          mode_status_changed = TRUE;
        }
#if defined (FEATURE_WCDMA_TO_LTE) || defined (FEATURE_LTE_TO_WCDMA)
        if(prev_lte_mode_status != rrcmcm_int_data.rrc_lte_mode_enabled)
        {
          WRRC_MSG2_HIGH("LTE_IRAT:new lte_mode %d, prev lte_mode %d",
                rrcmcm_int_data.rrc_lte_mode_enabled,prev_lte_mode_status);
          mode_status_changed = TRUE;
        }
#endif
        ...
        rrcmcm_int_data.rrc_dual_mode_enabled = cmd_ptr->cmd.rrc_change_mode_ind.dual_mode;
        if(!prev_dual_mode_status && rrcmcm_int_data.rrc_dual_mode_enabled)
        {
          mode_status_changed = TRUE;
        }
        ...
        if (TRUE == rrccsp_check_if_3g_band_pref_changed(wrrc_as_id,gw_band_pref))
        {
        ...
        {
          /* Set the dual-mode enable flag */
          rr_rrc_set_band_pref_internal(wrrc_as_id,gw_band_pref);
#ifdef FEATURE_DUAL_WCDMA
          rrc_update_band_preference_sub_id(gw_band_pref, wrrc_as_id);
#else
          rrc_update_band_preference(gw_band_pref);
#endif
#ifdef FEATURE_RAT_PRIORITY_LIST
          rrc_csp_update_rat_pri_list_info(wrrc_as_id,&cmd_ptr->cmd.rrc_change_mode_ind.rat_pri_list_info);
#endif
          /* Update GSM with UE's band preference */
          WRRC_MSG0_HIGH("DUAL: Updated rr and rrc band preferences");
          rrcmcm_send_dual_mode_info_to_meas_ueci(wrrc_as_id, cmd_ptr);
        }
      }

      break;
    case RRC_PSEUDO_LTE_LIST_UPDATE_IND:
        cmd_ptr_buff = rrc_get_int_cmd_buf(wrrc_as_id);
        cmd_ptr_buff->cmd_hdr.cmd_id = RRC_MODE_STATUS_IND;
        rrcmeas_procedure_event_handler(wrrc_as_id,cmd_ptr_buff);
        rrc_free(cmd_ptr_buff);
        break;
        
    /* RRC PLMN List request */
    case RRC_PLMN_LIST_REQ:
      WRRC_MSG0_MED("Rx:RRC_PLMN_LIST_REQ");
      ...
      switch(rrcmcm_int_data.rrc_mode)
      {
        ...
        case RRC_MODE_INACTIVE:
          ...
          rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_mm_req_cmd = *cmd_ptr;
          rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag = RRCMCM_CMD_PLMN_LIST_REQ;
          ...
          /* Send a Start WCDMA mode request to L1 */
#ifdef FEATURE_DUAL_SIM
          rrcmcm_send_start_wcdma_req(wrrc_as_id,RRC_MPLMN_SEARCH);
#else
          rrcmcm_send_start_wcdma_req(wrrc_as_id);
#endif
          break;
        ...
        case RRC_MODE_STANDBY:
          ...
          WRRC_MSG0_MED("Received RRC_PLMN_LIST_REQ in Standby Mode");

          curr_rrc_state = rrc_get_state(wrrc_as_id);

          /* Do not honor PLMN_LIST_REQ in DCH/FACH states*/
          if(((curr_rrc_state == RRC_STATE_CELL_DCH) || (curr_rrc_state == RRC_STATE_CELL_FACH)) && 
             !(rrcrce_is_abort_in_progress(wrrc_as_id))&&
             !(rrciho_return_iho_abort(wrrc_as_id)==1) /*Abort for WtoX IRAT procedure is not active*/
            ...
            )
          {
            ...
            rrcmcm_send_plmn_list_cnf_with_failure(wrrc_as_id,cmd_ptr->cmd.plmn_list_req.network_select_mode,
          ...
          /* Also fail the PLMN_LIST_REQ if UE is in PCH states and an OOS is in progress */
                   ...
                   (RRC_CSP_OUT_OF_SERVICE_AREA_SCAN == rrccsp_get_curr_scan_no_f3(wrrc_as_id)))
          {
        ...
        case RRC_MODE_INTERRAT_IN_PROGRESS:
          if(rrccho_is_gtow_pcco_in_progress(wrrc_as_id))
          {
            ...
            */
            WRRC_MSG1_HIGH("Failing PLMN_LIST_REQ in RRC mode %d because GTOW PCCO is in progress",rrcmcm_int_data.rrc_mode);
            rrcmcm_send_plmn_list_cnf_with_failure(wrrc_as_id,cmd_ptr->cmd.plmn_list_req.network_select_mode,
                                                   cmd_ptr->cmd.plmn_list_req.trans_id
                                                   ,cmd_ptr->cmd.plmn_list_req.service_search
                                                   );    
        ...
        default:
          WRRC_MSG1_HIGH("Received RRC_PLMN_LIST_REQ in invalid RRC mode: %d",rrcmcm_int_data.rrc_mode);
          rrcmcm_send_plmn_list_cnf_with_failure(wrrc_as_id,cmd_ptr->cmd.plmn_list_req.network_select_mode,
                                                 cmd_ptr->cmd.plmn_list_req.trans_id
                                                 ,cmd_ptr->cmd.plmn_list_req.service_search
                                                 );    
    ...
    case RRC_STOP_WCDMA_MODE_REQ:
      ...
      rrcmcm_int_data.stop_cause = cmd_ptr->cmd.stop_wcdma_mode_req.cause;
      WRRC_MSG1_HIGH("Received RRC_STOP_WCDMA_REQ with Cause %d",rrcmcm_int_data.stop_cause);

      /* Reset the flag which indicates service request or PLMN list request is saved */
      if((rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag == RRCMCM_CMD_SERVICE_REQ) ||
          (rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag == RRCMCM_CMD_PLMN_LIST_REQ))
      {
      ...
      rrccsp_check_and_send_uoosc_end_ind(wrrc_as_id);
#endif
#endif
      /* If the stop cause is RRC_WTOG_CC_ORDER,send a stop-cnf to MM
         We would stop only after a a successful CCO Cnf from RR */
      if(rrcmcm_int_data.stop_cause == RRC_WTOG_CC_ORDER)
      {
        /* Return a Stop WCDMA Confirmation to MM */
        WRRC_MSG1_HIGH("Received RRC_STOP_WCDMA_REQ with Cause RRC_WTOG_CC_ORDER in Initial substate,rrc_mode:%d",rrcmcm_int_data.rrc_mode);

        /*Raise an assert if CCO is not in progress*/
        if (!rrccho_is_wtog_cco_active(wrrc_as_id))
        {
          WRRC_ERR0_FATAL("WtoG CHO is not in progress but received stop with cause RRC_WTOG_CC_ORDER");
        }
        rrcmcm_send_stop_wcdma_cnf(wrrc_as_id,FALSE); 
      }

      /* Check if RRC is Inactive mode */
      else if (rrcmcm_int_data.rrc_mode == RRC_MODE_INACTIVE) 
      {
        /* Return a Stop WCDMA Confirmation to MM */
        rrcmcm_send_stop_wcdma_cnf(wrrc_as_id, TRUE);
      }
    ...
    case RRC_CHANGE_MODE_REQ:
      ...
      process_change_mode_req(wrrc_as_id, cmd_ptr->cmd.change_mode_req );
      break;
    ...
    case RRC_ACTIVATION_RSP:
...
#ifdef FEATURE_WCDMA_HANDLE_MDSP_HALT
      if(rrccsp_is_rrc_waiting_for_act_rsp_to_handle_mdsp_halt(wrrc_as_id))
      {
        if(rrcmcm_int_data.rrcmcm_act_cause == RRCMCM_INTERRAT_CC_ORDER_GTOW)
        {
          rrccho_handle_mdsp_halt(wrrc_as_id);
        }
        else
        {
          rrccsp_handle_activation_rsp_for_mdsp_halt(wrrc_as_id);
        }
        break;
      }
#endif
      if((rrcmcm_int_data.rrcmcm_act_cause == RRCMCM_INTERRAT_CELL_RESELECTION) ||
          (rrcmcm_int_data.rrcmcm_act_cause == RRCMCM_INTERRAT_CELL_REDIRECTION))
      {
        ...
        if(rrcmcm_int_data.rrcmcm_act_cause == RRCMCM_INTERRAT_CELL_REDIRECTION)
        {
          rrcmcm_int_data.rrcmcm_last_act_cause = RRCMCM_ACT_CAUSE_NONE;
        }
        else
        {
          rrcmcm_int_data.rrcmcm_last_act_cause = RRCMCM_INTERRAT_CELL_RESELECTION;
        }
        ...
        rrcmcm_int_data.rrcmcm_act_cause = RRCMCM_ACT_CAUSE_NONE;
      ...
      else if( rrcmcm_int_data.rrcmcm_act_cause ==  RRCMCM_INTERRAT_HANDOVER_GTOW )
      {
        ...
        rrccsp_update_plmn_info_for_gtow_handover(wrrc_as_id,&cmd_ptr->cmd.activation_rsp);
        ...
        rrciho_send_sync_ind_for_gtow_ho(wrrc_as_id);
        /* Set the last activation cause here */
        rrcmcm_int_data.rrcmcm_last_act_cause = RRCMCM_INTERRAT_HANDOVER_GTOW;
      }    
      else if( rrcmcm_int_data.rrcmcm_act_cause == RRCMCM_INTERRAT_CC_ORDER_GTOW)
      {
        /* Set the last activation cause */
        rrcmcm_int_data.rrcmcm_last_act_cause = RRCMCM_INTERRAT_CC_ORDER_GTOW;
        rrccho_procedure_event_handler(wrrc_as_id, cmd_ptr);
      ...
      {
        /* Set the last activation cause */
        rrcmcm_int_data.rrcmcm_last_act_cause = RRCMCM_INTERRAT_CC_ORDER_WTOG_FAILED;
        rrccho_procedure_event_handler(wrrc_as_id, cmd_ptr);
        WRRC_MSG0_HIGH("FWD RRC_ACTIVATION_RSP to CHO");
      }
    ...
    case RRC_LTE_RRC_CMD:
      {
        switch(cmd_ptr->cmd.wcdma_rrc_lte_irat_cmd.cmd.msg_hdr.id)
        {
          case WCDMA_RRC_LTE_RESEL_REQ:
            switch(rrcmcm_int_data.rrc_mode)
            {
              case RRC_MODE_INACTIVE:
              {
                ...
                WRRC_MSG1_HIGH("LTOW:WCDMA_RRC_LTE_RESEL_REQ from LTE RRC, Initial Substate,rrc_mode %d",rrcmcm_int_data.rrc_mode);
                rrc_init_dynamic_ptrs(wrrc_as_id);
                rrcmcm_int_data.rrc_mode = RRC_MODE_INTERRAT_IN_PROGRESS;
                rrcmcm_int_data.procedure_id = RRC_PROCEDURE_CSP;
                rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag = RRCMCM_CMD_LTE_IRAT_REQ;
                rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_lte_irat_cmd = *cmd_ptr;
                rrcmcm_int_data.start_band_class = rrc_get_frequency_band(
                                                       wrrc_as_id, 
          ...
          case WCDMA_RRC_LTE_ABORT_RESEL_REQ:
            WRRC_MSG1_HIGH("LTOW: WCDMA_RRC_LTE_ABORT_RESEL_REQ from LTE RRC in Initial substate,rrc_mode: %d",rrcmcm_int_data.rrc_mode);
            switch(rrcmcm_int_data.rrc_mode)
            {
              case RRC_MODE_INTERRAT_IN_PROGRESS:
                rrcmcm_send_irat_abort_ind(wrrc_as_id);
              ...
              default:
                /* We should not be getting command in any other substate */
                WRRC_MSG1_HIGH("LTOW: WCDMA_RRC_LTE_ABORT_RESEL_REQ  in Invalid mode,rrc_mode: %d",rrcmcm_int_data.rrc_mode);
                break;
            }
            break;

          case WCDMA_RRC_LTE_PLMN_SRCH_REQ:
            WRRC_MSG1_HIGH("LTOW: WCDMA_RRC_LTE_PLMN_SRCH_REQ from LTE RRC in Initial substate, rrc_mode %d",rrcmcm_int_data.rrc_mode);
            switch(rrcmcm_int_data.rrc_mode)
            {
              case RRC_MODE_INACTIVE:
                rrc_init_dynamic_ptrs(wrrc_as_id);
                rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_lte_irat_cmd = *cmd_ptr;
                rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag = RRCMCM_CMD_LTE_PLMN_SRCH_REQ ;

#ifdef FEATURE_DUAL_SIM
                if(rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_lte_irat_cmd.cmd.
                   wcdma_rrc_lte_irat_cmd.cmd.wcmd_rrc_lte_plmn_srch_req.high_priority)
                {
                  rrcmcm_acquire_lock_for_wcdma_start(wrrc_as_id,&rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_lte_irat_cmd.cmd.
                    wcdma_rrc_lte_irat_cmd.cmd.wcmd_rrc_lte_plmn_srch_req.timer_val,TRM_ACQUISITION_INV);
                }
                else
                {
                rrcmcm_acquire_lock_for_wcdma_start(wrrc_as_id,&rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_lte_irat_cmd.cmd.
                      wcdma_rrc_lte_irat_cmd.cmd.wcmd_rrc_lte_plmn_srch_req.timer_val,TRM_ACQUISITION);
                }
#else
                rrcmcm_int_data.rrcmcm_state = RRCMCM_WAIT_START_CNF;
                rrcmcm_send_start_wcdma_req(wrrc_as_id);
                ...
                break;
              default:
                WRRC_MSG1_ERROR("LTOW: WCDMA_RRC_LTE_PLMN_SRCH_REQ in invalid RRC mode %d",rrcmcm_int_data.rrc_mode);
                break;
            }
            break;
          ...
          case WCDMA_RRC_LTE_ABORT_PLMN_SRCH_REQ:
            WRRC_MSG1_HIGH("LTOW: WCDMA_RRC_LTE_ABORT_PLMN_SRCH_REQ from LTE RRC in Initial Substate, rrc_mode %d",rrcmcm_int_data.rrc_mode);
            switch(rrcmcm_int_data.rrc_mode)
            {
             case RRC_MODE_INACTIVE:
               rrccsp_send_lte_interrat_plmn_srch_abort_rsp(wrrc_as_id);
              ...
              default:
                WRRC_MSG1_ERROR("LTOW::WCDMA_RRC_LTE_ABORT_PLMN_SRCH_REQ in an Initial state/Invalid mode %d",rrcmcm_int_data.rrc_mode);  
                break;
            }
            break;

          case WCDMA_RRC_LTE_REDIR_REQ:
            WRRC_MSG1_HIGH("LTOW: Rvd WCDMA_RRC_LTE_REDIR_REQ from LTE RRC in Initial substate,rrc_mode: %d",rrcmcm_int_data.rrc_mode);
            ...
            switch(rrcmcm_int_data.rrc_mode)
            {
              case RRC_MODE_INACTIVE:
                ...
                if(rrc_csp_int_data.rrc_csfb_call_status)
                {
                  ...
                  lock_req_bands.num_bands = 1;
                  lock_req_bands.bands[0].band = rrc_return_default_band_lock_req(wrrc_as_id);
                  rrc_start_lock_wait_timer(wrrc_as_id,RRCTMR_WRM_LOCK_WAIT_TIMER_FOR_INTERRAT_IN_MS,TRUE);
                  wrm_lock_status = rrc_acquire_wrm_lock(wrrc_as_id, TRM_TRAFFIC, 0xFFFFFFFF, lock_req_bands);
                  rrc_stop_lock_wait_timer(wrrc_as_id);
                  if(wrm_lock_status == RRC_WRM_LOCK_ACQUIRED)
                  {
          ...
          case WCDMA_RRC_LTE_ABORT_REDIR_REQ:
            WRRC_MSG1_HIGH("LTOW: Rvd WCDMA_RRC_LTE_ABORT_REDIR_REQ from LTE RRC in Initial substate,rrc_mode: %d",rrcmcm_int_data.rrc_mode);
            ...
            switch(rrcmcm_int_data.rrc_mode)
            {
              ...
              default:
                ...
                rrccsp_send_lte_interrat_redirection_abort_rsp(wrrc_as_id);
                /* We should not be getting command in any other substate */
                WRRC_MSG1_ERROR("LTOW: WCDMA_RRC_LTE_ABORT_REDIR_REQ in Invalid mode,rrc_mode: %d",rrcmcm_int_data.rrc_mode);
                break;
            }
            break;
          ...
          case WCDMA_RRC_LTE_GET_CGI_REQ:
            ...
            {
              case RRC_MODE_INACTIVE:
                WRRC_MSG1_HIGH("LTOW:WCDMA_RRC_LTE_GET_CGI_REQ from LTE RRC, Initial Substate,rrc_mode %d",rrcmcm_int_data.rrc_mode);
                rrc_init_dynamic_ptrs(wrrc_as_id);
                rrcmcm_int_data.rrc_mode = RRC_MODE_INTERRAT_IN_PROGRESS;
                rrcmcm_int_data.procedure_id = RRC_PROCEDURE_CSP;
                rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag = RRCMCM_CMD_LTE_IRAT_REQ;
...
#ifdef FEATURE_DUAL_SIM
                rrcmcm_acquire_lock_for_wcdma_start(wrrc_as_id,&rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_lte_irat_cmd.cmd.
                  wcdma_rrc_lte_irat_cmd.cmd.wcdma_rrc_lte_cgi_req.time_available_for_cgi,TRM_ACQUISITION);
#else
                rrcmcm_int_data.rrcmcm_state = RRCMCM_WAIT_START_CNF;
                rrcmcm_send_start_wcdma_req(wrrc_as_id);
#endif
          ...
          case WCDMA_RRC_LTE_ABORT_CGI_REQ:
            WRRC_MSG1_HIGH("LTOW: WCDMA_RRC_LTE_ABORT_CGI_REQ from LTE RRC in Initial substate,rrc_mode: %d",rrcmcm_int_data.rrc_mode);
            switch(rrcmcm_int_data.rrc_mode)
            {
              case RRC_MODE_INTERRAT_IN_PROGRESS:
                rrcmcm_send_irat_abort_ind(wrrc_as_id);
              ...
              default:
                /* We should not be getting command in any other substate */
                WRRC_MSG1_HIGH("LTOW: WCDMA_RRC_LTE_ABORT_CGI_REQ  in Invalid mode,rrc_mode: %d",rrcmcm_int_data.rrc_mode);
                break;
            }
            break;

          case LTE_RRC_UTRA_SERVING_CELL_INFO_REQ:
            WRRC_MSG1_HIGH("LTOW: LTE_RRC_UTRA_SERVING_CELL_INFO_REQ from LTE RRC in Initial substate,rrc_mode: %d",rrcmcm_int_data.rrc_mode);
            switch(rrcmcm_int_data.rrc_mode)
            {
              case RRC_MODE_INACTIVE:
                serving_cell_info.utra_cell_info.rat = SYS_RAT_UMTS_RADIO_ACCESS;
                serving_cell_info.utra_cell_info.is_rat_active = FALSE;
                rrc_send_lte_cmd(wrrc_as_id,&serving_cell_info.msg_hdr, LTE_RRC_UTRA_SERVING_CELL_INFO_RSP, sizeof(lte_rrc_utra_serving_cell_info_rsp_s));
                break;
              case RRC_MODE_ACTIVE:
                rrc_cell_selection_procedure(wrrc_as_id, cmd_ptr);
                break;
              default:
                /* We should not be getting command in any other substate */
                WRRC_MSG1_HIGH("LTOW: LTE_RRC_UTRA_SERVING_CELL_INFO_REQ  in Invalid mode,rrc_mode: %d",rrcmcm_int_data.rrc_mode);
                break;
            }
            break;

          default :
            WRRC_MSG3_HIGH("LTOW: Invalid LTE RRC cmd %x in MCM initial substate and rrc_mode ",
                 cmd_ptr->cmd.wcdma_rrc_lte_irat_cmd.cmd.msg_hdr.id,rrcmcm_int_data.rrcmcm_state,rrcmcm_int_data.rrc_mode);
            break;
        }
      }
      break;
    ...
    default:
      ...
      WRRC_MSG2_HIGH("Ignoring MCM event %x in substate %d",
               cmd_ptr->cmd_hdr.cmd_id, rrcmcm_int_data.rrcmcm_state);
      break;
   }
  }
  return;
...
===========================================================================*/
static void rrcmcm_wait_start_cnf_substate_event_handler(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type *cmd_ptr /* Received command                   */
)
{
  boolean due_to_stop_mode = FALSE;
  ...
  else
  {
   switch( cmd_ptr->cmd_hdr.cmd_id )
   {
    ...
    case RRC_SERVICE_REQ:
      if((rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag == RRCMCM_CMD_INTERRAT_PLMN_SRCH_REQ) &&
         (rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag != RRCMCM_RR_INTERRAT_PLMN_ABORT_REQ))
      { 
        /* RR is expected to abort the search. This should ideally never happen*/
        WRRC_ERR0_FATAL("Rx:RRC_SERVICE_REQ when doing BPLMN search - This should never happen");
      }
      else
      {
    ...
    /* Mode change indication from MM */
    case RRC_CHANGE_MODE_IND:
      WRRC_MSG1_HIGH("DUAL:No processing is done for the message RRC_CHANGE_MODE_IND in mcm sstate=%d",rrcmcm_int_data.rrcmcm_state);
      break;
    ...
    case RRC_STOP_WCDMA_MODE_REQ:
      ...
      rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag = RRCMCM_CMD_STOP_WCDMA_REQ;
      break;
    ...
    case RRC_PLMN_LIST_REQ:
      ...
      { 
        /* RR is expected to abort the search. This should ideally never happen*/
        WRRC_ERR0_FATAL("Rx:RRC_PLMN_LIST_REQ when doing BPLMN search - This should never happen");
      }
      else
      {
        /* Store the command */
    ...
    case RRC_CPHY_START_WCDMA_MODE_CNF:
      ...
      else if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag == RRCMCM_CMD_LTE_IRAT_REQ)
      {
        WRRC_MSG0_HIGH("LTOW: Continue processing LTE->WCDMA request after start cnf");
        rrc_cell_selection_procedure(wrrc_as_id,&rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_lte_irat_cmd);
        rrcmcm_init_data(wrrc_as_id, INIT_ALL);
        rrctmr_start_timer(wrrc_as_id, RRCTMR_RSSI_BER_TIMER,
                             RRCTMR_RSSI_BER_REPORT_TIME );
      }
      else if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag == RRCMCM_CMD_LTE_PLMN_SRCH_REQ)
      {
        rrcmcm_update_mode(wrrc_as_id, RRC_MODE_INTERRAT_IN_PROGRESS);
      ...
      else if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag == RRCMCM_CMD_INTERRAT_PLMN_SRCH_REQ)
      {
        ...
        /*lint -e740*/
        rrc_cell_selection_procedure(wrrc_as_id, (rrc_cmd_type *)(&rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_gsm_irat_cmd) );
        /*lint +e740*/
      ...
      else if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag == RRCMCM_CMD_MODE_CHANGE)
      {
        /* Update RRC Mode and RAT preference, and
           send the mode change indication if needed */
        rrcmcm_update_mode(wrrc_as_id, RRC_MODE_ACTIVE);

        rrcmcm_process_mode_change_req(wrrc_as_id);
      }      
    ...
    /* Clear RRC change mode requests,if any */
      ...
      else if (rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag == RRCMCM_CMD_SERVICE_REQ)
      {
...
#ifdef FEATURE_RAT_PRIORITY_LIST
        rrcmcm_update_mode_capability_from_service_req(wrrc_as_id,&rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_mm_req_cmd.cmd.rrc_service_req.rat_pri_list_info);
#else
        /* Set the dual-mode enable flag */
        rrcmcm_int_data.rrc_dual_mode_enabled = rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_mm_req_cmd.cmd.rrc_service_req.dual_mode;
        WRRC_MSG1_HIGH("Setting rrc_dual_mode_enabled to %d", rrcmcm_int_data.rrc_dual_mode_enabled);
#endif

        /* Invoke the initial cell selection */
        rrc_cell_selection_procedure(wrrc_as_id,(rrc_cmd_type *) &rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_mm_req_cmd);
      ...
      else if (rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag == RRCMCM_CMD_PLMN_LIST_REQ)
      {
        /* Update RRC Mode and RAT preference (2.0), and
           send the mode change indication if needed */
        rrcmcm_update_mode(wrrc_as_id, RRC_MODE_ACTIVE);
      ...
      else 
      {
       /* Check if RRC is in INTERRAT_IN_PROGRESS mode */
        if (rrcmcm_int_data.rrc_mode == RRC_MODE_INTERRAT_IN_PROGRESS) 
        {
          if (rrcmcm_int_data.procedure_id == RRC_PROCEDURE_CSP) 
          {
            /*lint -e740*/
        ...
        else if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_change_mode_req.mode != RRC_MODE_MAX)
        {
          WRRC_MSG2_HIGH("Pending Ch-Mode-Req to mode %d in MCM-Sub-State %d",
                   rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_change_mode_req.mode, rrcmcm_int_data.rrcmcm_state);

          //send change mode req and reset the cmd_store flag
          rrcmcm_send_change_mode_req(wrrc_as_id,rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_change_mode_req.mode, 
                                      rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_change_mode_req.proc);
          rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_change_mode_req.mode = RRC_MODE_MAX;
          rrcmcm_int_data.rrcmcm_state = RRCMCM_INITIAL;
        }
        else
        { 
          ...
          if (rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag != RRCMCM_CMD_PLMN_LIST_REQ)
          {
#ifdef FEATURE_RAT_PRIORITY_LIST
            rrcmcm_update_mode_capability_from_service_req(wrrc_as_id,&rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_mm_req_cmd.cmd.rrc_service_req.rat_pri_list_info);
#else
          ...
          rrc_cell_selection_procedure(wrrc_as_id, &rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_mm_req_cmd );
        }
    ...
    case RRC_CHANGE_MODE_REQ:
      ...
      {
        rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag = RRCMCM_CMD_MODE_CHANGE;
      }
      else
      {
         ...
         */
        WRRC_MSG3_HIGH("Buffering CHANGE_MODE_REQ.req_mode %d,req_proc %d,mcm_substate %d",
                 cmd_ptr->cmd.change_mode_req.mode,
                 cmd_ptr->cmd.change_mode_req.procedure,
                 rrcmcm_int_data.rrcmcm_state);
        rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_change_mode_req.mode = cmd_ptr->cmd.change_mode_req.mode;
        rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_change_mode_req.proc = cmd_ptr->cmd.change_mode_req.procedure;
      }
      break;
        ...
        switch(cmd_ptr->cmd.wcdma_rrc_lte_irat_cmd.cmd.msg_hdr.id)
        {
          case WCDMA_RRC_LTE_ABORT_RESEL_REQ:
            WRRC_MSG2_HIGH("LTOW: WCDMA_RRC_LTE_ABORT_RESEL_REQ from LTE RRC in MCM state %d, RRC mode %d",
                rrcmcm_int_data.rrcmcm_state,rrcmcm_int_data.rrc_mode);
            rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag = RRCMCM_LTE_ABORT_RESEL_REQ;
            break;
          case WCDMA_RRC_LTE_ABORT_PLMN_SRCH_REQ:
          ...
          case WCDMA_RRC_LTE_ABORT_CGI_REQ:
            ...
            break;
          default :
            WRRC_MSG3_HIGH("LTOW: Invalid cmd %x from LTE RRC in MCM state %d, RRC mode %d",
                cmd_ptr->cmd.wcdma_rrc_lte_irat_cmd.cmd.msg_hdr.id,rrcmcm_int_data.rrcmcm_state,rrcmcm_int_data.rrc_mode);
            break;
        }
        break;
...
===========================================================================*/
static void rrcmcm_wait_stop_cnf_substate_event_handler(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type *cmd_ptr  /* Received command                   */
)
{
   ...
   case RRC_CHANGE_MODE_IND:
    ...
    case RRC_STOP_LOWER_LAYER_CNF:
      ...
      if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag == RRCMCM_RR_INTERRAT_PLMN_ABORT_REQ)
      {
        ...
        {
          /* Go back to initial state */
          rrcmcm_init_data(wrrc_as_id, INIT_STATE_VARS_ONLY);
          rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag = RRCMCM_RR_ABORT_NONE;
          rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_change_mode_req.mode = RRC_MODE_MAX;
        }
        else
      ...
      {
        if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag == RRCMCM_LTE_ABORT_RESEL_REQ)
        {
      ...
      else
      {
        ...
        if (rrcmcm_int_data.rrc_mode == RRC_MODE_INTERRAT_IN_PROGRESS) 
        {
          ...
          if(rrcmcm_int_data.rrcmcm_send_stop_cnf_to_mm_flag)
          {
            /* Send a Stop WCDMA Mode Confirmation to MM */
            rrcmcm_send_stop_wcdma_cnf(wrrc_as_id, TRUE);
          }
    ...
    case  RRC_CONTINUE_STOP_MODE_REQ:
       ...
       rrcmcm_send_stop_lower_layer_req(wrrc_as_id, TRUE);
      break;
    ...
    case RRC_LTE_RRC_CMD:
      ...
      {
        case WCDMA_RRC_LTE_ABORT_RESEL_REQ:
          WRRC_MSG1_HIGH("LTOW: WCDMA_RRC_LTE_ABORT_RESEL_REQ in wt_stop_cnf, rrc_mode %d",rrcmcm_int_data.rrc_mode);
          rrcmcm_send_irat_abort_ind(wrrc_as_id);
          break;
        case WCDMA_RRC_LTE_ABORT_PLMN_SRCH_REQ:
          WRRC_MSG1_HIGH("LTOW: WCDMA_RRC_LTE_ABORT_PLMN_SRCH_REQ in wt_stop_cnf, rrc_mode %d",rrcmcm_int_data.rrc_mode);
          if(rrcmcm_int_data.rrc_mode == RRC_MODE_INTERRAT_IN_PROGRESS)
          {
            rrc_cell_selection_procedure(wrrc_as_id, cmd_ptr);
          }
          ...
          {
            WRRC_MSG1_ERROR("LTOW: WCDMA_RRC_LTE_ABORT_PLMN_SRCH_REQ in wt_stop_cnf, Invalid rrc_mode %d",rrcmcm_int_data.rrc_mode);
          }
          break;
        case WCDMA_RRC_LTE_ABORT_REDIR_REQ:
          WRRC_MSG1_HIGH("LTOW: WCDMA_RRC_LTE_ABORT_REDIR_REQ in wt_stop_cnf, rrc_mode %d",rrcmcm_int_data.rrc_mode);
          rrcmcm_send_irat_abort_ind(wrrc_as_id);
          break;
        case WCDMA_RRC_LTE_ABORT_CGI_REQ:
          WRRC_MSG1_HIGH("LTOW: WCDMA_RRC_LTE_ABORT_CGI_REQ in wt_stop_cnf, rrc_mode %d",rrcmcm_int_data.rrc_mode);
          rrcmcm_send_irat_abort_ind(wrrc_as_id);
          break;
        default:
...
===========================================================================*/
static void rrcmcm_wait_stop_cnf_for_mode_change_substate_event_handler(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type *cmd_ptr  /* Received command                   */
)
{
  /* Switch on the received command. */
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    ...
    case  RRC_CONTINUE_STOP_MODE_REQ:
      WRRC_MSG0_HIGH("Rvd RRC_CONTINUE_STOP_MODE_REQ ");
      rrcmcm_send_stop_lower_layer_req(wrrc_as_id, FALSE);
      break;
...
===========================================================================*/
static void rrcmcm_wait_suspend_cnf_substate_event_handler(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type *cmd_ptr  /* Received command */
)
{
  boolean due_to_stop_mode = FALSE;
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    ...
    case RRC_LLC_CMD_CNF:
      ...
      if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag == RRCMCM_CMD_STOP_WCDMA_REQ)
      {
      ...
      else if (cmd_ptr->cmd.rrc_llc_cnf.ll_cnf == RRC_LL_CPHY_SUSPEND_CNF) 
      {
        ...
        {
          WRRC_MSG0_MED("Rvd CPHY_SUSPEND_WCDMA_MODE_CNF");
          ...
          else if(rrcmcm_int_data.rrcmcm_est_req_saved_flag == TRUE)
          {
        ...
        else
        {
          ...
          WRRC_ERR0_FATAL("WCDMA L1 Suspend failed!");
        }
      }
      else
      {
  ...
  case RRC_PLMN_LIST_REQ:
    ...
    default:
      /* Unexpected command */
      WRRC_MSG2_ERROR("Unexpected CMD: %x in State: %d", cmd_ptr->cmd_hdr.cmd_id, rrcmcm_int_data.rrcmcm_state);
      break;
  }
  return;
...
===========================================================================*/
static void rrcmcm_wait_resume_cnf_substate_event_handler(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type *cmd_ptr  /* Received command */
)
{
  ...
  case RRC_LLC_CMD_CNF:
    ...
    {
      WRRC_MSG1_MED("CPHY_RESUME_WCDMA_MODE_CNF: %d", cmd_ptr->cmd.rrc_llc_cnf.cnf.resume_cnf.status);
        /* Change the RRC mode */
      rrcmcm_update_mode(wrrc_as_id, RRC_MODE_ACTIVE);
      ...
      if (cmd_ptr->cmd.rrc_llc_cnf.cnf.resume_cnf.status == FALSE)
      {
        ...
        if((rrccsp_get_curr_scan_no_f3(wrrc_as_id) == RRC_CSP_UNLOCK_BY_SCAN) && ((rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH) || (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH)))
        {
          wl1_fill_tuneaway_duration(0xFFFF,wrrc_as_id);
          rrc_send_rlc_ta_status(wrrc_as_id, RLC_LTA_STOP_IND, RLC_FAIL_IND);
        }
        ...
        rrc_dsds_int_data.wl1_failure_cause = cmd_ptr->cmd.rrc_llc_cnf.cnf.resume_cnf.fail_type;
#endif
        ...
        PCCPCH setup, right after a RESUME_CNF_FAILURE and even before CSP procedure 
          ...
          if(rrcmcm_int_data.rrcmcm_est_req_saved_flag == TRUE)
          {
            if(rrccsp_get_curr_scan_no_f3(wrrc_as_id) == RRC_CSP_INTER_RAT_REDIRECTION_SCAN)
            {
              WRRC_MSG0_HIGH("WTOL:WTOG: Process the buffered EST_REQ.");
              rrcmcm_process_est_req(wrrc_as_id, RRC_MODE_ACTIVE);
            }
            else
            {
              rrcmcm_int_data.rrcmcm_est_req_saved_flag = FALSE;
              ...
              mm_cmd_ptr = mm_rrc_get_cmd_buf(RRC_ABORT_IND);
                ...
                (void) rrcsmc_append_hfns_to_rel_ind(wrrc_as_id,mm_cmd_ptr->cmd.rrc_abort_ind.hfn_cs,
                  mm_cmd_ptr->cmd.rrc_abort_cnf.hfn_ps);
                
#ifdef FEATURE_DUAL_SIM
                mm_cmd_ptr->cmd.rrc_abort_ind.as_id = rrc_get_as_id(wrrc_as_id);
#endif
        ...
        else if((rrcmcm_int_data.procedure_id == RRC_PROCEDURE_IHO) || (rrcmcm_int_data.procedure_id == RRC_PROCEDURE_CHO))
        {
          /* Send a change mode cnf with failure */
          rrcmcm_send_change_mode_cnf(wrrc_as_id, FALSE);
        }
        else
        {
          ...
          WRRC_MSG1_ERROR("ERR: W resume for proc rrc_proc_e_type_value%d other than CSP, IHO, CHO", rrcmcm_int_data.procedure_id);
          ...
          rrcmcm_send_change_mode_cnf(wrrc_as_id, FALSE);
#else
          WRRC_ERR1_FATAL("Send resume cnf with failure to the calling proc rrc_proc_e_type_value%d", rrcmcm_int_data.procedure_id);
#endif
        } 
      }
        ...
        (rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag == RRCMCM_CMD_DEACT_REQ)  )
      {
        ...
        WRRC_MSG1_HIGH("Forwarding buffered MM request %d to CSP",
          rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag);
...
===========================================================================*/
void rrcmcm_event_handler(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type *cmd_ptr                      /* Pointer to command */
)
{
  WRRC_MSG2_HIGH_OPT("function rrcmcm_event_handler is called in rrcmcm_state = rrcmcm_state_e_type_value%d / Event is received by MCM: %x  ",rrcmcm_int_data.rrcmcm_state, cmd_ptr->cmd_hdr.cmd_id);
  rrc_crash_debug_log_proc_substates(wrrc_as_id,RRC_PROCEDURE_MCM,rrcmcm_int_data.rrcmcm_state,cmd_ptr);

  /* Check the procedure substate and procedure the event accordingly */  
  switch( rrcmcm_int_data.rrcmcm_state )
  {
    case RRCMCM_INITIAL:
      /* Call the event handler for Initial substate */
      rrcmcm_initial_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;

    case RRCMCM_WAIT_START_CNF:
      /* Call the event handler for waiting for start cnf substate */
      rrcmcm_wait_start_cnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;
  
    case RRCMCM_WAIT_STOP_CNF:
      /* Call the event handler for wait for stop cnf substate */
      rrcmcm_wait_stop_cnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;

    case RRCMCM_WAIT_SUSPEND_CNF:
      /* Call the event handler for wait for suspend substate */
      rrcmcm_wait_suspend_cnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;

    case RRCMCM_WAIT_RESUME_CNF:
      /* Call the event handler for wait for resume substate */
      rrcmcm_wait_resume_cnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;

    case RRCMCM_WAIT_STOP_CNF_FOR_MODE_CHANGE:
      /* Call the event handler for wait for stop cnf (Mode change) substate */
      rrcmcm_wait_stop_cnf_for_mode_change_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;

    default:
      /* Invalid substate - should never happen. Clear
         all procedure vars */
      WRRC_MSG1_ERROR("Invalid MSM Substate %d", rrcmcm_int_data.rrcmcm_state);
      rrcmcm_int_data.rrcmcm_state = RRCMCM_INITIAL;
      break;
  }/* end switch */
  return;
...
===========================================================================*/
void rrcmcm_update_wcdma_activation_cause(sys_modem_as_id_e_type wrrc_as_id, rrcmcm_activation_cause act_cause )
{
  /*update the activation cause*/
  rrcmcm_int_data.rrcmcm_act_cause = act_cause;
...
===========================================================================*/
void rrcmcm_update_dual_mode_enabled_status(sys_modem_as_id_e_type wrrc_as_id, boolean status )
{
  
  /*update the dual_mode_enabled status*/
  rrcmcm_int_data.rrc_dual_mode_enabled = status;
...
{
  if(rrcmcm_int_data.rrcmcm_est_req_saved_flag)
  {
    WRRC_MSG1_HIGH("Est Cause is %d", rrcmcm_int_data.rrcmcm_est_req.cause); 
    return rrcmcm_int_data.rrcmcm_est_req.cause;
  }
  else
  {
...
/*===========================================================================
FUNCTION rrc_reset_mcm_irat_resel_act_cause_sub
...
SIDE EFFECTS
  None
===========================================================================*/
void rrc_reset_mcm_irat_resel_act_cause_sub
(
  sys_modem_as_id_e_type wrrc_as_id
)
{
  RRC_VALIDATE_SUB_ID(wrrc_as_id);
  if(RRCMCM_INTERRAT_CELL_RESELECTION == rrcmcm_int_data.rrcmcm_last_act_cause)
  {
    rrcmcm_int_data.rrcmcm_last_act_cause = RRCMCM_ACT_CAUSE_NONE;
  }
...
)
{
  ...
  rrc_reset_mcm_irat_resel_act_cause_sub(SYS_MODEM_AS_ID_1);
...
)
{
  ...
  rrc_put_int_cmd(wrrc_as_id,cmd_ptr_buff );
...
===========================================================================*/
void rrcmcm_init_procedure(sys_modem_as_id_e_type wrrc_as_id)
{
  /* Initialize data for the module */
  rrcmcm_int_data.rrc_mode = RRC_MODE_INACTIVE;
  rrcmcm_int_data.rrc_dual_mode_enabled = TRUE;
  rrcmcm_int_data.rrcmcm_notification_counter = 0;
  rrcmcm_int_data.rrcmcm_state = RRCMCM_INITIAL;
  rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag = RRCMCM_CMD_NONE;
  rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_change_mode_req.mode = RRC_MODE_MAX;
  rrcmcm_int_data.rrcmcm_act_cause = RRCMCM_ACT_CAUSE_NONE;
...
FUNCTION  rrcmcm_return_rrcmcm_cmd_flag
...
rrcmcm_cmd_flag_e_type rrcmcm_return_rrcmcm_cmd_flag(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag;
}
/*===========================================================================

FUNCTION  rrcmcm_return_gsm_irat_cmd
...
uint32 rrcmcm_return_gsm_irat_cmd(sys_modem_as_id_e_type wrrc_as_id)
{
  return (uint32)rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_gsm_irat_cmd.cmd_hdr.cmd_id;
}
#ifdef FEATURE_LTE_TO_WCDMA

/*===========================================================================

FUNCTION  rrcmcm_return_lte_irat_cmd
...
uint32 rrcmcm_return_lte_irat_cmd(sys_modem_as_id_e_type wrrc_as_id)
{
  return (uint32)rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_lte_irat_cmd.cmd_hdr.cmd_id;
}
#endif
/*===========================================================================

FUNCTION  rrcmcm_return_rrcmcm_abort_flag
...
uint8 rrcmcm_return_rrcmcm_abort_flag(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_abort_flag;
}

/*===========================================================================

FUNCTION  rrcmcm_return_mcm_state
...
__attribute__((section(".uncompressible.text")))
#endif
uint8 rrcmcm_return_mcm_state(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcmcm_int_data.rrcmcm_state;
}
...
FUNCTION  rrcmcm_return_procedure_id
...
uint8 rrcmcm_return_procedure_id(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcmcm_int_data.procedure_id;
}


/*===========================================================================

FUNCTION  rrcmcm_return_stop_cause
...
uint8 rrcmcm_return_stop_cause(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcmcm_int_data.stop_cause;
}

/*===========================================================================

FUNCTION  rrcmcm_return_ue_in_sg_lte_mode
...
__attribute__((section(".uncompressible.text")))
#endif
boolean rrcmcm_return_ue_in_sg_lte_mode(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcmcm_int_data.is_ue_in_sglte_mode;
}

/*===========================================================================

FUNCTION  rrcmcm_return_send_stop_cnf_to_mm_flag
...
boolean rrcmcm_return_send_stop_cnf_to_mm_flag(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcmcm_int_data.rrcmcm_send_stop_cnf_to_mm_flag;
}


/*===========================================================================

FUNCTION  rrcmcm_return_est_req_saved_flag
...
boolean rrcmcm_return_est_req_saved_flag(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcmcm_int_data.rrcmcm_est_req_saved_flag;
}
...
===========================================================================*/
void rrc_send_rlc_ta_status(sys_modem_as_id_e_type wrrc_as_id, rlc_ta_ind_e_type tune_away_ind, rlc_ta_success_e_type tune_away_succ)
{
  ...
  WRRC_MSG2_MED("Sending RLC_UL_TA_IND  TA-Type:%d(0-LtaStart,1-LtaStop,2-QtaStart,3-QtaStop), TA-Status:%d(0-success,1-fail)",tune_away_ind,tune_away_succ);
  rrc_send_l2_ul_cmd(wrrc_as_id, l2_ul_cmd_ptr);
}

/*===========================================================================

FUNCTION   rrcmcm_get_lte_cmd_id

DESCRIPTION

  This function retrieves the stored LTE RRC command id.
...
RETURN VALUE
  ...
  Returns command id in msgr_umid_type
...
===========================================================================*/
msgr_umid_type rrcmcm_get_lte_cmd_id(sys_modem_as_id_e_type wrrc_as_id)
{
  if(rrcmcm_int_data.rrcmcm_cmd_store.rrcmcm_cmd_flag == RRCMCM_CMD_LTE_IRAT_REQ) 
  {
    return (rrcmcm_int_data.rrcmcm_cmd_store.interrat_cmd.rrcmcm_lte_irat_cmd.cmd.wcdma_rrc_lte_irat_cmd.cmd.msg_hdr.id);
  }
  else
  {
    return  INVALID_MSGR_UMID;
  }
}
#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
...
{
  qsh_mdump_collect_high(&rrcmcm_int_data,sizeof(rrcmcm_int_data));
}
#endif
/*===========================================================================

FUNCTION  rrc_mcm_get_mode_change_req_proc_id

DESCRIPTION

  This function returns the procedure which requested for mode change.
...
rrc_proc_e_type rrc_mcm_get_mode_change_req_proc_id(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcmcm_int_data.procedure_id;
}

