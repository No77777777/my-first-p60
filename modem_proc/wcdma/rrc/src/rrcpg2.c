...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcpg2.c#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/04/16   sn      f3 density reduction changes
06/10/15   vi      Made changes to ignore page when page is received while recovering from RLF
...
LOCAL rrcpg2_int_data_type rrcpg2_int_data_sim[MAX_RRC_STACKS];
#define rrcpg2_int_data rrcpg2_int_data_sim[WAS_ID_M]
#else
LOCAL rrcpg2_int_data_type rrcpg2_int_data;
#endif
...
)
{
  ...
  { 
    case rrc_CN_DomainIdentity_cs_domain:
      rrcpg2_int_data.rrcpg2_cn_domain_id = RRC_CS_DOMAIN_CN_ID;
      break;

    case rrc_CN_DomainIdentity_ps_domain:
      rrcpg2_int_data.rrcpg2_cn_domain_id = RRC_PS_DOMAIN_CN_ID;
      break;
  ...
  switch(dcch_msg_ptr->message.u.pagingType2.pagingCause)
  {
    case rrc_PagingCause_terminatingConversationalCall:
      rrcpg2_int_data.rrcpg2_paging_cause = RRC_PAGE_CAUSE_TERM_CONV_CALL;
      break;

    case rrc_PagingCause_terminatingStreamingCall:
      rrcpg2_int_data.rrcpg2_paging_cause = RRC_PAGE_CAUSE_TERM_STREAM_CALL;
      break;

    case rrc_PagingCause_terminatingInteractiveCall:
      rrcpg2_int_data.rrcpg2_paging_cause = RRC_PAGE_CAUSE_TERM_INTERACT_CALL;
      break;

    case rrc_PagingCause_terminatingBackgroundCall:
      rrcpg2_int_data.rrcpg2_paging_cause = RRC_PAGE_CAUSE_TERM_BACKGND_CALL;
      break;

    case rrc_PagingCause_terminatingHighPrioritySignalling:
      rrcpg2_int_data.rrcpg2_paging_cause = RRC_PAGE_CAUSE_HIGH_PRI_SIGNALLING;
      break;

    case rrc_PagingCause_terminatingLowPrioritySignalling:
      rrcpg2_int_data.rrcpg2_paging_cause = RRC_PAGE_CAUSE_LOW_PRI_SIGNALLING;
      break;

    case rrc_PagingCause_terminatingCauseUnknown:
      rrcpg2_int_data.rrcpg2_paging_cause = RRC_PAGE_CAUSE_UNKNOWN;
      break;
  ...
  switch( dcch_msg_ptr->message.u.pagingType2.pagingRecordTypeID)
  {
    case rrc_PagingRecordTypeID_imsi_GSM_MAP:
      rrcpg2_int_data.rrcpg2_page_ue_id = RRC_PAGE_WITH_IMSI;
      break;

    case rrc_PagingRecordTypeID_tmsi_GSM_MAP_P_TMSI:
      if(rrcpg2_int_data.rrcpg2_cn_domain_id == RRC_CS_DOMAIN_CN_ID)
      {
        rrcpg2_int_data.rrcpg2_page_ue_id = RRC_PAGE_WITH_TMSI;
      }
      else if(rrcpg2_int_data.rrcpg2_cn_domain_id == RRC_PS_DOMAIN_CN_ID)
      {
        rrcpg2_int_data.rrcpg2_page_ue_id = RRC_PAGE_WITH_PTMSI;
      }
      break;
  ...
  paging_event_info.paging_cause = rrcpg2_int_data.rrcpg2_paging_cause;
  /* Send the event */
...
)
{
  ...
  if(cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_type == rrc_DL_DCCH_Message_PDU) 
  {
    ...
    if( message_choice == T_rrc_DL_DCCH_MessageType_pagingType2 )
    {
      ...
      else
      {
        ...
        rrcpg2_int_data.rrcpg2_transaction_id = 
        ...
        if(RRCPG2_SUCCESS == rrcpg2_validate_paging_type_msg(wrrc_as_id,dcch_msg_ptr) )
        {
          ...
          if( (mm_cmd_ptr =  mm_rrc_get_cmd_buf(RRC_PAGE_IND)) != NULL)
          {
            ...
            rrcpg2_update_rrc_page_ind(wrrc_as_id,dcch_msg_ptr);

            /* Fill in parameters fro RRC_PAGE_IND command */

            mm_cmd_ptr->cmd.rrc_page_ind.cn_domain_id = rrcpg2_int_data.rrcpg2_cn_domain_id;

            mm_cmd_ptr->cmd.rrc_page_ind.paging_cause = rrcpg2_int_data.rrcpg2_paging_cause;

            mm_cmd_ptr->cmd.rrc_page_ind.page_ue_id = rrcpg2_int_data.rrcpg2_page_ue_id;
          ...
          {
            WRRC_ERR1_FATAL(" Failed allocated memory for MM Command: %d",
                       RRC_PAGE_IND);
          }
          ...
          else
          {
            if(SUCCESS != rrcsend_send_rrc_status_message(wrrc_as_id,
               T_rrc_ProtocolErrorMoreInformation_type1_ie_ValueNotComprehended,
                                             // The protocol error cause to be placed in
                                             // the message
               rrcpg2_int_data.rrcpg2_transaction_id,        // Set transaction id 
               rrc_ReceivedMessageType_pagingType2, 
                                             // Received message identification
               RRC_PROCEDURE_PG2,            // RRC procedure
               lc_id,                        /* Logical Channel Id */
  ...
  page_info.page_record_match = rrcpg2_validate_paging_type_msg(wrrc_as_id,dcch_msg_ptr);
  page_info.cn_id = rrcpg2_int_data.rrcpg2_cn_domain_id;
  page_info.paging_cause = rrcpg2_int_data.rrcpg2_paging_cause; 
  rrc_qsh_log_paging_params(wrrc_as_id, RRC_PROCEDURE_PG2,&page_info);
#endif  
...
)
{
  ...
  WRRC_MSG1_HIGH_OPT("function rrcpg2_procedure_event_handler is called in rrc_state = rrc_state_e_type_value %d",rrc_state);
  rrc_crash_debug_save_proc_substates(wrrc_as_id,RRC_PROCEDURE_PG2,RRC_INVALID_PROC_SUBSTATE,cmd_ptr->cmd_hdr.cmd_id);
  ...
  switch (rrc_state)
  {
    ...
    case RRC_STATE_URA_PCH:
       ...
       * we get this message as we move from fach/dch to cell_pch/ura_pch state */
        rrcpg2_process_paging_type2_msg(wrrc_as_id,cmd_ptr); 
        break;
...
void rrcpg2_init_procedure(sys_modem_as_id_e_type wrrc_as_id)
{
  rrcpg2_int_data.rrcpg2_transaction_id = 0;
