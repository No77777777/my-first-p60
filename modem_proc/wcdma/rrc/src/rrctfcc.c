...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrctfcc.c#2 $   

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/14/20   sp      Mini dump phase-2 changes
04/29/16   nr      Added code to support LTA during activation time
...
LOCAL mac_tfc_subset_restriction_info_type mac_tfc_subset_info_sim[MAX_RRC_STACKS];
LOCAL rrctfcc_substates_e_type rrctfcc_substate_sim[MAX_RRC_STACKS];

#define mac_tfc_subset_info mac_tfc_subset_info_sim[WAS_ID_M]
#define rrctfcc_substate rrctfcc_substate_sim[WAS_ID_M]

#else
LOCAL mac_tfc_subset_restriction_info_type mac_tfc_subset_info;
LOCAL rrctfcc_substates_e_type rrctfcc_substate;
#endif
...
)
{
  ...
  /*Reports transport format combination control failure to QSH*/
  rrc_qsh_report_failure_ota_msg_event(wrrc_as_id,WRRC_UL_DCCH_MESSAGETYPE_TRANSPORTFORMATCOMBINATIONCONTROLFAILURE);
#endif
  ...
  if (ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
    ...
    send_status = rrcsend_send_ul_sdu(wrrc_as_id,RRC_PROCEDURE_TFC,
                                      msg_ptr,
                                      ul_lc_id,
...
===========================================================================*/     
static uint16 rrctfcc_get_duration( sys_modem_as_id_e_type wrrc_as_id,rrc_TFC_ControlDuration tfcc_duration)
{
...
)
{
  ...
  tfcc_rb_id = rrclcm_get_dl_rb_id(wrrc_as_id,cmd_ptr->cmd.downlink_sdu_ind.lc_id);
  ...
  {
    if((rrcllc_has_config_srb5(wrrc_as_id,CURRENT_CONFIG, (uint16)tfcc_rb_id) == TRUE)
      &&(rrctfcc_substate != RRCTFCC_WAIT_FOR_RECONFIG_CFG_CNF))
    {  
      tfc_subset_idx = (*((uint8 *)cmd_ptr->cmd.downlink_sdu_ind.dl_sdu)) >> 5;
  
      WRRC_MSG3_HIGH("TFCC received on TM mode for RBId %d Current config has %d subset lists; TFC subset index -> %d", tfcc_rb_id,current_config_ptr->ul_tfc_subset_list.no_of_subset_lists, tfc_subset_idx);
      ...
      if(tfc_subset_idx < current_config_ptr->ul_tfc_subset_list.no_of_subset_lists)
      {
        ...
        rrctfcc_update_tfcc_restriction_in_mac(wrrc_as_id,&mac_tfc_subset_info, &current_config_ptr->ul_tfc_subset_list.tfc_subset_info[tfc_subset_idx]);
      }
      else
      {
        invalid_config = TRUE;
  ...
  if (!invalid_config && tm_tfcc_msg_recvd == FALSE)
  {
        ...
        tfc_ControlDuration))
    {
      mac_tfc_subset_info.control_duration = 
        rrctfcc_get_duration(wrrc_as_id,dcch_msg_ptr->message.u.
                             transportFormatCombinationControl.tfc_ControlDuration);
    }

    WRRC_MSG3_HIGH("RBId %d, TransId %d t %d", mac_tfc_subset_info.rb_id, 
  ...
  if (invalid_config)
  {
        ...
        tfcc_rb_id == DCCH_UM_RADIO_BEARER_ID)
    {
      ...
      {
        rrctfcc_send_tfcc_config_failure(wrrc_as_id,(uint8)dcch_msg_ptr->message.u.
    ...
    if(rrctfcc_substate != RRCTFCC_WAIT_FOR_RECONFIG_CFG_CNF)
    {
      rrctfcc_clear_procedure(wrrc_as_id);
    }
  }
  ...
  else if(rrc_get_state(wrrc_as_id)== RRC_STATE_CELL_FACH)
  {
    ...
    {
      rrcllc_update_tfcc_restriction_in_cc_oc(wrrc_as_id,&mac_tfc_subset_info);
    }
    rrctfcc_clear_procedure(wrrc_as_id);
  }
  else
...
{
  rrc_crash_debug_save_proc_substates(wrrc_as_id,RRC_PROCEDURE_TFC,rrctfcc_substate,cmd_ptr->cmd_hdr.cmd_id);
  ...
  switch ( cmd_ptr->cmd_hdr.cmd_id )
  {
    case RRC_DOWNLINK_SDU_IND:
      rrctfcc_process_message(wrrc_as_id,  cmd_ptr );
      break;
    case RRC_CMAC_UL_TFC_SUBSET_CFG_CNF:
      rrctfcc_process_mac_config_cnf(wrrc_as_id, cmd_ptr);
      break;
    case RRC_STATE_CHANGE_IND:
      rrctfcc_clear_procedure(wrrc_as_id);
      break;
    case RRC_RECONFIG_COMPLETE_IND:
      rrctfcc_process_reconfig_cnf(wrrc_as_id);
      break;
...
)
{
  ...
  else
  {
    ...
    {
      rrctfcc_send_tfcc_config_failure(wrrc_as_id,
...
{
  rrctfcc_substate = RRCTFCC_INITIAL;
  rrcscmgr_register_for_scn(wrrc_as_id, RRC_PROCEDURE_TFC,  /* Procedure name */
...
rrctfcc_substates_e_type rrctfcc_get_current_substate(sys_modem_as_id_e_type wrrc_as_id)
{
...
{
  qsh_mdump_collect_high(&rrctfcc_substate,sizeof(rrctfcc_substate));
}
#endif
