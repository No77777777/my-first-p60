...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrciho.c#3 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/14/20   sp      Mini dump phase-2 changes
11/20/18   ak      Made changes for FR 53313
11/14/16   ha      Made changes to include optional IEs in utran classmark change info message
...
03/09/18   as      Made changes to send sync_ind for inter-RAT HO if fake security is enabled
12/18/17   sas     Changes to retain lock with ACCESS priority until UMI is received after Handover.
07/31/17   gv      Changes to support L+L
06/05/17   nr      Made changes to do conditional check for handovertype before accessing members in eutra union datatype
05/20/16   gv      Made changes to reset CSFB call status at the end of successful W2G HO
05/04/16   sn      f3 density reduction changes
04/08/16   sp      Made changes to send select_owner event to voice module only if CS RABs are present during L to W HO
02/11/16   sp      Made Changes to log misc commands in crash debug info
02/01/16   sp      Made changes to report select owner event to voice module during L/G to W HO
01/12/16   nr      Made changes to report failure events to QSH
11/04/15   sp      Made changes to add call end metrics - FR#30856
10/26/15   sas     Made Changes to fail HO from UTRAN to GSM, when the OTA has no proper padded bits for GSM command list.
...
08/04/15   sg      Made changes to fill edch category to 6 in WCDMA  capability container
08/17/15   sas     Changes to req LTE about the capabilities during G2W Handover. 
...
07/01/15   sr      Made changes to request priority as TRAFFIC for PSHO with CSFB call status set
...
05/25/15   sa      Made changes to support eAMR for FR25951 Single VOiceSesion
05/05/15   vi      Changes to support  LTE-U feature protected by feature flag LTE_BAND_NUM
05/04/15   bc      Changes to handle PSHO Abort req when failure response is already sent for PSHO.
04/21/15   bc      Made changes to handle ABORT REQ from NAS when WTOX handover is not yet completed.
...
20/03/15   ad      Made changes not to apply the pending cs keys on wtog handover
...
12/04/14   sr      Global restructuring
...
typedef struct
{
  ...
  rrciho_state_e_type rrciho_state;

  /* Abort indicator */
  boolean rrciho_abort;

  rrc_abort_req_type *rrc_abort_req_ptr;

  /* GSM Command list */
  rrc_GSM_MessageList gsm_commands;
} rrciho_int_data_type;
/* -----------------------------------------------------------------------
...
rrciho_ext_data_type rrciho_ext_data_sim[MAX_RRC_STACKS] = {0};

extern rrc_encode_info_type rrc_encode_info_sim[MAX_RRC_STACKS];
extern cell_info_type cell_id_dch_info_sim[MAX_RRC_STACKS];

#define rrciho_ext_data rrciho_ext_data_sim[WAS_ID_M]
#define rrc_encode_info rrc_encode_info_sim[WAS_ID_M]
#define cell_id_dch_info cell_id_dch_info_sim[WAS_ID_M]
#else
rrciho_ext_data_type rrciho_ext_data;
...
** ----------------------------------------------------------------------- */
#ifdef FEATURE_DUAL_WCDMA
rrciho_int_data_type rrciho_int_data_sim[MAX_RRC_STACKS]= {0};
#define rrciho_int_data rrciho_int_data_sim[WAS_ID_M]
#else
rrciho_int_data_type rrciho_int_data;
#endif
...
FUNCTION  RRCIHO_IS_HO_FOR_CS

DESCRIPTION

  This function checks if the HO is for CS domain
...
===========================================================================*/
boolean rrciho_is_ho_for_cs(sys_modem_as_id_e_type wrrc_as_id)
{
  return (rrciho_int_data.gtow_info.ho_msg_present && rrciho_int_data.gtow_info.is_cs_rab_present);
}

/*===========================================================================

FUNCTION  RRCIHO_IS_HO_FOR_PS

DESCRIPTION

  This function checks if the HO is for PS domain
...
===========================================================================*/
boolean rrciho_is_ho_for_ps(sys_modem_as_id_e_type wrrc_as_id)
{
  return (rrciho_int_data.gtow_info.ho_msg_present && rrciho_int_data.gtow_info.is_ps_rab_present);
}
...
===========================================================================*/
static void rrciho_init_data(sys_modem_as_id_e_type wrrc_as_id )
{
#ifdef FEATURE_LTE_TO_WCDMA
  uint8 idx;
#endif
  /* Reset variables */
  rrciho_int_data.rrciho_state = RRCIHO_INITIAL;
  /* free GSM_commands if not do so */
  if(rrciho_int_data.gsm_commands.n !=0)
  {
    rrc_free(rrciho_int_data.gsm_commands.elem);
  }
  
  rrciho_int_data.gsm_commands.n=0;
  
  /* Set Abort indication to False */
  rrciho_int_data.rrciho_abort = FALSE;
  rrciho_int_data.procedure_info.act_time_present=FALSE;

  /* initialise the nas_Synchronisation_Indicator to default value */
#ifdef FEATURE_LTE_TO_WCDMA
  for(idx =0 ; idx < MAX_RAB_TO_SETUP ; idx++)
  {
    rrciho_int_data.gtow_info.est_rabs[idx].nas_Synchronisation_Indicator = WCDMA_IVOCODER_ID_NONE;
  }
#else
  rrciho_int_data.gtow_info.est_rabs.nas_Synchronisation_Indicator = WCDMA_IVOCODER_ID_NONE;
#endif

#ifdef FEATURE_WCDMA_TO_LTE
  if((rrciho_int_data.procedure_info.handover_type == T_rrc_DL_DCCH_MessageType_handoverFromUTRANCommand_EUTRA)
    && (rrciho_int_data.procedure_info.parms.eutra.eutra_container_size != 0))
  {
    rrc_free(rrciho_int_data.procedure_info.parms.eutra.eutra_container_ptr);
    rrciho_int_data.procedure_info.parms.eutra.eutra_container_ptr = NULL;
    rrciho_int_data.procedure_info.parms.eutra.eutra_container_size = 0;
  ...
  /*Reset handover type*/
  rrciho_int_data.procedure_info.handover_type = 0;
...
===========================================================================*/
static void send_nas_data_transfer_ind(sys_modem_as_id_e_type wrrc_as_id)
{
...
===========================================================================*/
static void wrrc_qsh_log_iho_call_end_stats(sys_modem_as_id_e_type wrrc_as_id,sys_handover_failure_e_type fail_type)
{
  ...
  wrrc_qsh_log_conn_end_info(wrrc_as_id,WRRC_METRIC_LOG,&call_end_data);
}
#endif
...
===========================================================================*/
static void send_handover_from_utran_failure( sys_modem_as_id_e_type wrrc_as_id )
{
 ...
 RRC_MSG_COMMON_SET_BITMASK_IE_TYPE2(ptr->message.u.handoverFromUTRANFailure,interRAT_HO_FailureCause);
  ...
  ptr->message.u.handoverFromUTRANFailure.rrc_TransactionIdentifier =
    rrciho_int_data.procedure_info.transaction_id;

  /* Fill the failure cause */
  ptr->message.u.handoverFromUTRANFailure.interRAT_HO_FailureCause.t =
    rrciho_int_data.procedure_info.failure_cause;

  /* Check the failure cause for the protocol error */
  if (rrciho_int_data.procedure_info.failure_cause == T_rrc_InterRAT_HO_FailureCause_protocolError)
  {
    ...
    ptr->message.u.handoverFromUTRANFailure.interRAT_HO_FailureCause.u.protocolError->diagnosticsType.u.type1->protocolErrorCause =
      rrciho_int_data.procedure_info.protocol_error;

  }

  if(rrciho_int_data.procedure_info.failure_cause == T_rrc_InterRAT_HO_FailureCause_interRAT_ProtocolError)
  {
         ...
         && (rrciho_int_data.procedure_info.parms.eutra.eutra_container_size != 0))
    {
      ...
      eutra_ho_msg_ptr->eutra_Message.numocts = rrciho_int_data.procedure_info.parms.eutra.eutra_container_size;
      eutra_ho_msg_ptr->eutra_Message.data = rrciho_int_data.procedure_info.parms.eutra.eutra_container_ptr;
    ...
    if (rrciho_int_data.gsm_commands.n != 0)
    {
      ...
      ptr->message.u.handoverFromUTRANFailure.interRATMessage.u.gsm.gsm_MessageList = rrciho_int_data.gsm_commands;
    }

  }

#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
  /*Reports W to X handover failure to QSH*/
  rrc_qsh_report_w2x_irat_ho_fail_event(wrrc_as_id,rrciho_int_data.procedure_info.handover_type);
  /*Reports handover failure to QSH*/
  rrc_qsh_report_failure_ota_msg_event(wrrc_as_id,WRRC_UL_DCCH_MESSAGETYPE_HANDOVERFROMUTRANFAILURE);
#endif

  /*log the error*/
  rrc_log_protocol_error(wrrc_as_id, (uint8) (rrc_get_state(wrrc_as_id)),
                          (uint8) RRC_PROCEDURE_IHO,
                          (uint8) (rrcho_map_inter_rat_cause_to_diag_cause(wrrc_as_id,rrciho_int_data.procedure_info.failure_cause)),
                          (uint8) rrciho_int_data.procedure_info.protocol_error                   
                        );

  /* Get the RLC logical channel id for this msg from Logical Channel Manager */
  ul_lc_id = rrclcm_check_ul_rlc_lc_id ( wrrc_as_id, UE_LOGCHAN_DCCH,
  ...
  if(ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
    ...
    status = rrcsend_send_ul_sdu(wrrc_as_id,RRC_PROCEDURE_IHO,  /* Procedure is RBE    */
  ...
  {
    uplink_ota.ul_ota_type = RRC_OTA_IHO;
    uplink_ota.msg_id = WRRC_UL_DCCH_MESSAGETYPE_HANDOVERFROMUTRANFAILURE;
    uplink_ota.ul_ota.ul_ota_parms.failure_cause = rrciho_int_data.procedure_info.failure_cause;
  }
  else
  {
    rrc_qsh_log_ho_from_utran_fail_params(wrrc_as_id, rrciho_int_data.procedure_info.failure_cause);
  }
  if(rrciho_int_data.procedure_info.handover_type == T_rrc_DL_DCCH_MessageType_handoverFromUTRANCommand_EUTRA)
  {
    wrrc_qsh_log_iho_call_end_stats(wrrc_as_id,SYS_HANDOVER_FAILURE_W_TO_L_FAILURE);
  }
  else if(rrciho_int_data.procedure_info.handover_type == T_rrc_DL_DCCH_MessageType_handoverFromUTRANCommand_GSM)
  {
    wrrc_qsh_log_iho_call_end_stats(wrrc_as_id,SYS_HANDOVER_FAILURE_W_TO_G_FAILURE);
  }
#endif
  return;
...
===========================================================================*/
static void send_rr_interrat_handover_req( sys_modem_as_id_e_type wrrc_as_id )
{

  ASSERT( rrciho_int_data.procedure_info.handover_type ==
                               T_rrc_DL_DCCH_MessageType_handoverFromUTRANCommand_GSM );

  /*if ciphering is turned on then set cipher_enabled flag to true
    else set it to false*/
  if (rrcsmc_is_ciphering_enabled(wrrc_as_id) == RRCSMC_SECURITY_STARTED)
  {
    rrciho_int_data.procedure_info.parms.gsm.cipher_enabled = TRUE;
  }
  else
  {
    rrciho_int_data.procedure_info.parms.gsm.cipher_enabled = FALSE;
  }

  rrccsp_fill_info_for_rr(wrrc_as_id,&(rrciho_int_data.procedure_info.parms.gsm.network_select_mode));

#ifdef FEATURE_RAT_PRIORITY_LIST
  rrc_csp_get_rat_pri_list_info(wrrc_as_id,&rrciho_int_data.procedure_info.parms.gsm.rat_pri_list_info);
#endif
  ...
  memset(&rrciho_int_data.procedure_info.parms.gsm.dedicated_priority_info,0,sizeof(lte_irat_dedicated_priority_info_s));
  rrcmeas_populate_dedicated_priority_info_for_irat_req(wrrc_as_id,&rrciho_int_data.procedure_info.parms.gsm.dedicated_priority_info);
#endif

#ifdef FEATURE_WCDMA_FAST_RETURN_TO_LTE_AFTER_CSFB 
  if(rrc_csp_int_data.csfb_extended_status != CSFB_INVALID)
  {
    WRRC_MSG1_HIGH("CSFB HACK: csfb_extended_status=%d",rrc_csp_int_data.csfb_extended_status);
    rrciho_int_data.procedure_info.parms.gsm.csfb_call_status=TRUE;
  }
  else
  {
    rrciho_int_data.procedure_info.parms.gsm.csfb_call_status=FALSE;
  }
#endif
  /* Send the RR_INTERRAT_HANDOVER_REQ message to RR */
  rr_send_rrc_message_to_rr_internal(wrrc_as_id,(void *)(&(rrciho_int_data.procedure_info.parms.gsm)), RR_INTERRAT_HANDOVER_REQ,
                sizeof(rr_interrat_handover_req_type));

#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
  if(rrc_qsh_events_db.wtog_ho.status == TRUE)
  {
    QSH_LOG(QSH_CLT_WRRC,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"Sub:%d WtoG HO start",(wrrc_as_id+1));
    rrc_qsh_report_event(wrrc_as_id, WRRC_QSH_EVENT_WTOG_HO_REQ);
  }
#endif
...
===========================================================================*/
static void rrciho_send_lte_ps_ho_req(sys_modem_as_id_e_type wrrc_as_id )
{
  ...
  msgr_init_hdr_attach(&rrciho_int_data.procedure_info.parms.eutra.msg.msg_hdr, MSGR_WCDMA_RRC, LTE_RRC_WCDMA_PSHO_REQ, 0, 1);

#ifdef FEATURE_DUAL_SIM
#ifdef FEATURE_MSGR_VARIANT
  msgr_set_hdr_variant(&rrciho_int_data.procedure_info.parms.eutra.msg.msg_hdr,rrc_get_as_id(wrrc_as_id));
#else
  msgr_set_hdr_inst(&rrciho_int_data.procedure_info.parms.eutra.msg.msg_hdr,SYS_AS_ID_TO_INST_ID(rrc_get_as_id(wrrc_as_id)));
#endif
#endif


  // Set the HandoverToUTRACommand as a DSM attachment
  msgr_attach_ptr = msgr_get_attach(&rrciho_int_data.procedure_info.parms.eutra.msg.msg_hdr, 0);
  ASSERT(msgr_attach_ptr != NULL);
  msgr_set_dsm_attach(msgr_attach_ptr, rrciho_int_data.procedure_info.parms.eutra.dsm_ptr);

  ASSERT(rrciho_int_data.procedure_info.parms.eutra.dsm_ptr != NULL);

  rrccsp_fill_info_for_rr(wrrc_as_id,&(rrciho_int_data.procedure_info.parms.eutra.msg.irat_nas_info.network_select_mode));

  rrciho_int_data.procedure_info.parms.eutra.msg.irat_nas_info.req_plmn = 
      mmumts_convert_rrc_plmn_id_to_nas_plmn_id(rrccsp_get_curr_nas_requested_plmn(wrrc_as_id));

#ifdef FEATURE_RAT_PRIORITY_LIST
  rrc_csp_get_rat_pri_list_info(wrrc_as_id,&rrciho_int_data.procedure_info.parms.eutra.msg.irat_nas_info.rat_pri_list);
#endif

  /* Get EUTRA timing tx info from L1 */
  l1_get_eutra_measurement_data(&rrciho_int_data.procedure_info.parms.eutra.msg.timing_tx_info, wrrc_as_id);
  if(rrciho_int_data.procedure_info.parms.eutra.msg.timing_tx_info.number_of_cells > 0)
  {
    rrciho_int_data.procedure_info.parms.eutra.msg.timing_tx_info_present = TRUE;
  }
  else
  {
    rrciho_int_data.procedure_info.parms.eutra.msg.timing_tx_info_present = FALSE;
  }

  rrciho_int_data.procedure_info.parms.eutra.msg.wtol_fdd_supported = FALSE;
  rrciho_int_data.procedure_info.parms.eutra.msg.wtol_tdd_supported = FALSE;
  if((lte_rrc_lte_fdd_is_supported(rrccsp_get_lte_band_pref(wrrc_as_id)))&&(rrcnv_data.wcdma_rrc_wtol_ps_ho_support_nv))
  {
    rrciho_int_data.procedure_info.parms.eutra.msg.wtol_fdd_supported = TRUE;
  }
  if((lte_rrc_lte_tdd_is_supported(rrccsp_get_lte_band_pref(wrrc_as_id)))&&(rrcnv_data.wcdma_rrc_wtol_tdd_ps_ho_support_nv))
  {
    rrciho_int_data.procedure_info.parms.eutra.msg.wtol_tdd_supported = TRUE;
  }

  rrciho_int_data.procedure_info.parms.eutra.msg.utra_cell_info.rat = SYS_RAT_UMTS_RADIO_ACCESS;
  rrciho_int_data.procedure_info.parms.eutra.msg.utra_cell_info.frequency = cell_id_dch_info.uarfcn_dl;
  rrciho_int_data.procedure_info.parms.eutra.msg.utra_cell_info.phy_cell_id = cell_id_dch_info.psc;
  rrciho_int_data.procedure_info.parms.eutra.msg.utra_cell_info.is_rat_active = TRUE;

  if(cell_id_dch_info.cell_id != WCDMA_INVALID_CELL_ID)
  {
    rrciho_int_data.procedure_info.parms.eutra.msg.utra_cell_info.is_cell_global_id_present = TRUE;
    rrciho_int_data.procedure_info.parms.eutra.msg.utra_cell_info.serving_cell_global_id.plmn_id = mmumts_convert_rrc_plmn_id_to_nas_plmn_id(cell_id_dch_info.plmn_id);
    rrciho_int_data.procedure_info.parms.eutra.msg.utra_cell_info.serving_cell_global_id.cell_id = cell_id_dch_info.cell_id;
  ...
  {
    rrciho_int_data.procedure_info.parms.eutra.msg.utra_cell_info.is_cell_global_id_present = FALSE;
  }

  /* Save the umid for debugging purposes */
  rrc_crash_debug_save_rrc_to_lte_cmds(wrrc_as_id, LTE_RRC_WCDMA_PSHO_REQ);
  msgr_send(&rrciho_int_data.procedure_info.parms.eutra.msg.msg_hdr, sizeof(lte_rrc_wcdma_psho_req_s));
  WRRC_MSG0_HIGH("WTOL: LTE_RRC_WCDMA_PSHO_REQ sent to LTE RRC");

#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
  if(rrc_qsh_events_db.wtol_ho.status == TRUE)
  {
    QSH_LOG(QSH_CLT_WRRC,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"Sub:%d WtoL HO start",(wrrc_as_id+1));
    rrc_qsh_report_event(wrrc_as_id, WRRC_QSH_EVENT_WTOL_HO_REQ);
  }
#endif
...
===========================================================================*/
static void rrciho_send_abort_handover_to_lte(sys_modem_as_id_e_type wrrc_as_id )
{
  ...
  rrc_send_lte_cmd(wrrc_as_id,&(abort_psho_req_ptr->msg_hdr),LTE_RRC_WCDMA_ABORT_PSHO_REQ, sizeof(lte_rrc_wcdma_abort_psho_req_s));
...
===========================================================================*/
static void send_interrat_handover_req( sys_modem_as_id_e_type wrrc_as_id )
{

  switch ( rrciho_int_data.procedure_info.handover_type )
  {
    case T_rrc_UL_DCCH_MessageType_handoverFromUTRANFailure:
      send_rr_interrat_handover_req(wrrc_as_id);
      break;

#ifdef FEATURE_WCDMA_TO_LTE
    case T_rrc_DL_DCCH_MessageType_handoverFromUTRANCommand_EUTRA:
      rrciho_send_lte_ps_ho_req(wrrc_as_id);
      break;
#endif
...
===========================================================================*/
static void send_abort_handover_req( sys_modem_as_id_e_type wrrc_as_id )
{
  rr_interrat_handover_abort_req_type abort_req;

  /* Send the RR_INTERRAT_HANDOVER_REQ message to RR */
  rr_send_rrc_message_to_rr_internal(wrrc_as_id,(void *)(&(abort_req)), RR_INTERRAT_HANDOVER_ABORT_REQ,
...
===========================================================================*/
static void send_change_mode_req( sys_modem_as_id_e_type wrrc_as_id, rrc_mode_e_type mode )
{
  rrc_cmd_type *cmd_ptr;
...
===========================================================================*/
static void send_channel_config_req_to_llc(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_cmd_type *cmd_ptr;
...
===========================================================================*/
static void rrciho_delete_stored_gtow_info(sys_modem_as_id_e_type wrrc_as_id)
{
  /*delete the HO to Utran msg by calling rrcasn1_free_pdu*/
  if(rrciho_int_data.gtow_info.ho_msg_present == TRUE)
  {

    /*free the msg_ptr previously allocated.. if failure, flag an error*/
    rrcasn1_free_pdu(wrrc_as_id,(int)rrc_HandoverToUTRANCommand_PDU, rrciho_int_data.gtow_info.ho_ptr);
      /*clean the msg_ptr data structure*/
      rrciho_int_data.gtow_info.ho_msg_len = 0;
      rrciho_int_data.gtow_info.ho_msg_present = FALSE;
    rrciho_int_data.gtow_info.ho_ptr = NULL;
  }

  /*delete default config info if it was expanded*/
  if(rrciho_int_data.gtow_info.def_config_ptr_present == TRUE)
  {
    if(rrciho_int_data.gtow_info.rbs_ptr != NULL)
    {
      rrcasn1_free_pdu(wrrc_as_id,(int)rrc_DL_DCCH_Message_PDU, rrciho_int_data.gtow_info.rbs_ptr);
      /*clean the msg_ptr data structure*/
      rrciho_int_data.gtow_info.def_config_ptr_present = FALSE;
      rrciho_int_data.gtow_info.rbs_ptr =NULL;
    }
    else
    {
      /*free the msg_ptr previously allocated.. if failure, flag an error*/
      rrciho_free_sib16(wrrc_as_id);
        /*clean the msg_ptr data structure*/
        rrciho_int_data.gtow_info.def_config_ptr_present = FALSE;
      }
    }
  /*delete class mark information*/
  if(rrciho_int_data.gtow_info.class_mark_info_present == TRUE)
  {
    ...
    // functions.
    rrc_free(rrciho_int_data.gtow_info.class_mark_ptr);

    /*reset the static variables*/
    rrciho_int_data.gtow_info.class_mark_info_present = FALSE;
    rrciho_int_data.gtow_info.class_mark_ptr = NULL;
  }
  ...
  /*set the other stored vars to false or default.  do not delete rab_info_included
    ...
    SYNC_IND to MM*/
  rrciho_int_data.gtow_info.act_time_present = FALSE;
  rrciho_int_data.gtow_info.ciph_algo_present = FALSE;
  rrciho_int_data.gtow_info.num_srb = 0;
  rrciho_int_data.gtow_info.maxAllowedUL_TX_Power = 0;
  rrciho_int_data.gtow_info.diversity_info_present = FALSE;
  rrciho_int_data.gtow_info.diversity_info = FALSE;
  rrciho_int_data.gtow_info.pn_offset_present = FALSE;
  rrciho_int_data.gtow_info.pn_offset = 0;
...
===========================================================================*/
static void rrciho_initialize_srb_list(sys_modem_as_id_e_type wrrc_as_id,rrc_SRB_InformationSetupList *srb_ptr)
{
  uint8 srb_count=0;
  ...
  while ((srb_ptr!=NULL )&& (srb_ptr->n > srb_count ))
  {
    ...
    rrciho_int_data.gtow_info.srb_id[srb_count] = (uint8)rb_id;
  ...
  rrciho_int_data.gtow_info.num_srb = srb_count;
...
===========================================================================*/
static void rrciho_initialize_srb_list_r5(sys_modem_as_id_e_type wrrc_as_id,rrc_SRB_InformationSetupList_r5 *srb_ptr)
{
  uint8 srb_count=0;
  rrc_RB_Identity rb_id = CCCH_RADIO_BEARER_ID;
...
===========================================================================*/
static void rrciho_initialize_srb_list_r6(sys_modem_as_id_e_type wrrc_as_id,rrc_SRB_InformationSetupList_r6 *srb_ptr)
{
  uint8 srb_count=0;
  rrc_RB_Identity rb_id = CCCH_RADIO_BEARER_ID;
...
===========================================================================*/
static void rrciho_initialize_rb_list(sys_modem_as_id_e_type wrrc_as_id,rrc_RB_InformationSetupList *rb_ptr
                               #ifdef FEATURE_LTE_TO_WCDMA
                                    , uint8 index
                               #endif
                               )
...
#ifndef FEATURE_LTE_TO_WCDMA
    rrciho_int_data.gtow_info.est_rabs.rb_for_rab[rb_count].rb_id = rb_ptr->elem[rb_count].rb_Identity;
#else
    rrciho_int_data.gtow_info.est_rabs[index].rb_for_rab[rb_count].rb_id = rb_ptr->elem[rb_count].rb_Identity;
#endif /* FEATURE_LTE_TO_WCDMA */    
    /* Increment count of RBs */
    rb_count++;
  }/* end while */
...
#ifndef FEATURE_LTE_TO_WCDMA
  rrciho_int_data.gtow_info.est_rabs.num_rbs_for_rab = rb_count;
#else
  rrciho_int_data.gtow_info.est_rabs[index].num_rbs_for_rab = rb_count;
#endif /* FEATURE_LTE_TO_WCDMA */
...
===========================================================================*/
static void rrciho_initialize_rb_list_r5(sys_modem_as_id_e_type wrrc_as_id,rrc_RB_InformationSetupList_r5 *rb_ptr
                                  #ifdef FEATURE_LTE_TO_WCDMA
                                  , uint8 index
                                  #endif
                                  )
...
===========================================================================*/
static void rrciho_initialize_rb_list_r6(sys_modem_as_id_e_type wrrc_as_id,rrc_RB_InformationSetupList_r6 *rb_ptr
                                  #ifdef FEATURE_LTE_TO_WCDMA
                                  , uint8 index
                                  #endif
                                  )
...
)
{
  ...
  while ((rb_ptr!=NULL )&&( rb_ptr->n > rb_count ) && (rb_count < MAX_RB_PER_RAB))
  {
    ...
    rrciho_int_data.gtow_info.est_rabs.rb_for_rab[rb_count].rb_id = rb_ptr->elem[rb_count].rb_Identity;
#else
    rrciho_int_data.gtow_info.est_rabs[index].rb_for_rab[rb_count].rb_id = (uint8)(rb_ptr->elem[rb_count].rb_Identity);
#endif
    /* Increment count of RBs */
    rb_count++;
...
)
{
  ...
  rrc_RAB_InformationSetupList_r7 *rab_list_r7 = NULL;
      ...
      ho_utran_r7 = &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r7
   ...
   if(T_rrc_RAB_Identity_gsm_MAP_RAB_Identity == ho_utran_r7->specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.rab_Identity.t)
   {
     *rab_identity_choice = T_rrc_RAB_Identity_gsm_MAP_RAB_Identity;
     ...
     rrciho_int_data.gtow_info.est_rabs.rab_id =
        rrcrb_translate_gsm_map_rab_id(wrrc_as_id,ho_utran_r7->specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.
                                       rab_Identity.u.gsm_MAP_RAB_Identity);
   
     rrciho_int_data.gtow_info.est_rabs.re_est_timer = ho_utran_r7->specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.re_EstablishmentTimer;
   
     rb_list_r7 =&ho_utran_r7->specificationMode.u.complete.rab_InformationSetupList.elem[0].rb_InformationSetupList;
   
     rrciho_initialize_rb_list_r7(wrrc_as_id,rb_list_r7);
     ...
     {
       rrciho_int_data.gtow_info.est_rabs.nas_Synchronisation_Indicator = WCDMA_IVOCODER_ID_AMR;
       ...
       {
         rrciho_int_data.gtow_info.est_rabs.nas_Synchronisation_Indicator = 
             rrc_translate_nas_sync_indicator(wrrc_as_id,&(rab_list_r7->elem[0].rab_Info.nas_Synchronisation_Indicator));
         
         if( rrciho_int_data.gtow_info.est_rabs.nas_Synchronisation_Indicator == WCDMA_IVOCODER_ID_NONE )
         {
           rrciho_int_data.procedure_info.failure_cause = (uint32)RR_RRC_INVALID_CONFIGURATION;
           WRRC_MSG1_HIGH("Invalid configuration .. codec not supported %d",rrciho_int_data.gtow_info.est_rabs.nas_Synchronisation_Indicator );
           return NULL;
         }
       }
       rrc_set_ordered_codec_mode(wrrc_as_id,rrciho_int_data.gtow_info.est_rabs.nas_Synchronisation_Indicator);
       WRRC_MSG1_HIGH("CSoHS:Codec in rrc_ota_codec_mode_status %d",rrcdata_codec_info.rrc_ota_codec_mode_status);   
     }
...
#ifdef FEATURE_LTE_TO_WCDMA
  est_rabs = &(rrciho_int_data.gtow_info.est_rabs[index]);
#else
  est_rabs = &(rrciho_int_data.gtow_info.est_rabs);
#endif /* FEATURE_LTE_TO_WCDMA */

  if(rab_info.cn_DomainIdentity == rrc_CN_DomainIdentity_cs_domain )
  {
    est_rabs->nas_Synchronisation_Indicator = WCDMA_IVOCODER_ID_AMR;
     /* in the G-> W handover check the Nas-Sync-indicator for the codec type */        
    if(RRC_MSG_COMMON_BITMASK_IE(rab_info,rrc_RAB_Info,nas_Synchronisation_Indicator))
    {
      est_rabs->nas_Synchronisation_Indicator = 
          rrc_translate_nas_sync_indicator(wrrc_as_id,&(rab_info.nas_Synchronisation_Indicator));
      
      if(est_rabs->nas_Synchronisation_Indicator == WCDMA_IVOCODER_ID_NONE )
      {
        rrciho_int_data.procedure_info.failure_cause = (uint32)RR_RRC_INVALID_CONFIGURATION;
    ...
    rrc_set_ordered_codec_mode(wrrc_as_id,est_rabs->nas_Synchronisation_Indicator);
  }
...
)
{
  ...
  if(rab_info->cn_DomainIdentity  == rrc_CN_DomainIdentity_cs_domain )
  {
    ...
    {
      est_rabs->nas_Synchronisation_Indicator =  rrc_translate_nas_sync_indicator(wrrc_as_id,
...
===========================================================================*/
static uecomdef_status_e_type rrciho_extract_and_fill_in_rab_rb_info(sys_modem_as_id_e_type wrrc_as_id,
                                                              void *rab_info_list,
                                                              ...
                                                              )
{
  ...
  if(release == MSG_REL99)
  {
    ...
    do 
    {
      ...
      if(T_rrc_RAB_Identity_gsm_MAP_RAB_Identity == rab_list->elem[idx].rab_Info.rab_Identity.t)
      {
        ...
        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].rab_id =
           rrcrb_translate_gsm_map_rab_id(wrrc_as_id,rab_list->elem[idx].rab_Info.
                                       rab_Identity.u.gsm_MAP_RAB_Identity);

        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].re_est_timer = rab_list->elem[idx].rab_Info.re_EstablishmentTimer;

        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].cn_domain = rab_list->elem[idx].rab_Info.cn_DomainIdentity;

        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].rab_id_is_gsm_map = TRUE;

        rb_list = &rab_list->elem[idx].rb_InformationSetupList;

        rrciho_initialize_rb_list(wrrc_as_id,rb_list,number_of_rabs);

        (void)rrc_update_rab_type_in_rab_info(wrrc_as_id,
                                                 &(rab_list->elem[idx]),
                                                 &(rrciho_int_data.gtow_info.est_rabs[number_of_rabs]));

        WRRC_MSG3_HIGH("RAB_ID:%d,Domain:%d,re_est_tmr:%d",
                                                     rrciho_int_data.gtow_info.est_rabs[number_of_rabs].rab_id,
                                                     rrciho_int_data.gtow_info.est_rabs[number_of_rabs].cn_domain,
                                                     rrciho_int_data.gtow_info.est_rabs[number_of_rabs].re_est_timer);

        if(rab_list->elem[idx].rab_Info.cn_DomainIdentity == rrc_CN_DomainIdentity_cs_domain)
        {
          rrciho_int_data.gtow_info.cs_rab_index = number_of_rabs;
          rrciho_int_data.gtow_info.is_cs_rab_present = TRUE;
          if(iho_interrat_type == RRCIHO_INTERRAT_LTE)
          {
            rrciho_int_data.gtow_info.est_rabs[number_of_rabs].rab_type = RRCRB_CS_VOICE_RAB;
          }
        }
        else if(rab_list->elem[idx].rab_Info.cn_DomainIdentity == rrc_CN_DomainIdentity_ps_domain)
        {
          rrciho_int_data.gtow_info.is_ps_rab_present = TRUE;
        }

         /* in the G-> W handover check the Nas-Sync-indicator for the codec type */
        if((rab_list->elem[idx].rab_Info.cn_DomainIdentity == rrc_CN_DomainIdentity_cs_domain) && 
           (rrciho_update_nas_sync_ind(wrrc_as_id,rab_list->elem[idx].rab_Info, number_of_rabs) == FAILURE))
        {
          return FAILURE;
        }
        number_of_rabs++;
  ...
  else if(release == MSG_REL5)
  {
    ...
    do 
    {
      ...
      if(T_rrc_RAB_Identity_gsm_MAP_RAB_Identity == rab_list_r5->elem[idx].rab_Info.rab_Identity.t)
      {
        ...
        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].rab_id =
           rrcrb_translate_gsm_map_rab_id(wrrc_as_id,rab_list_r5->elem[idx].rab_Info.
                                       rab_Identity.u.gsm_MAP_RAB_Identity);

        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].re_est_timer = rab_list_r5->elem[idx].rab_Info.re_EstablishmentTimer;

        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].cn_domain = rab_list_r5->elem[idx].rab_Info.cn_DomainIdentity;

        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].rab_id_is_gsm_map = TRUE;

        rb_list_r5 = &rab_list_r5->elem[idx].rb_InformationSetupList;

        rrciho_initialize_rb_list_r5(wrrc_as_id,rb_list_r5,number_of_rabs);

        (void)rrc_update_rab_type_in_rab_info_r5(wrrc_as_id,
        ...
        else if(rab_list_r5->elem[idx].rab_Info.cn_DomainIdentity == rrc_CN_DomainIdentity_ps_domain)
        {
         ...
         /* in the G-> W handover check the Nas-Sync-indicator for the codec type */
        if((rab_list_r5->elem[idx].rab_Info.cn_DomainIdentity == rrc_CN_DomainIdentity_cs_domain) && 
           (rrciho_update_nas_sync_ind(wrrc_as_id,rab_list_r5->elem[idx].rab_Info, number_of_rabs) == FAILURE))
        {
          return FAILURE;
        }
        number_of_rabs++;
  ...
  else if(release == MSG_REL6)
  {
    ...
    do 
    {
      ...
      if(T_rrc_RAB_Identity_gsm_MAP_RAB_Identity == rab_list_r6->elem[idx].rab_Info.rab_Identity.t)
      {
        ...
        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].rab_id =
           rrcrb_translate_gsm_map_rab_id(wrrc_as_id,rab_list_r6->elem[idx].rab_Info.
                                       rab_Identity.u.gsm_MAP_RAB_Identity);

        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].re_est_timer = rab_list_r6->elem[idx].rab_Info.re_EstablishmentTimer;

        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].cn_domain = rab_list_r6->elem[idx].rab_Info.cn_DomainIdentity;

        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].rab_id_is_gsm_map = TRUE;

        rb_list_r6 = &rab_list_r6->elem[idx].rb_InformationSetupList;

        rrciho_initialize_rb_list_r6(wrrc_as_id,rb_list_r6,number_of_rabs);

        (void)rrc_update_rab_type_in_rab_info_r6(wrrc_as_id,
        ...
        if(rab_list_r6->elem[idx].rab_Info.cn_DomainIdentity == rrc_CN_DomainIdentity_ps_domain)
        {
          ...
          if(rab_list_r6->elem[idx].rab_Info.cn_DomainIdentity == rrc_CN_DomainIdentity_cs_domain )
          {
            rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].nas_Synchronisation_Indicator = WCDMA_IVOCODER_ID_AMR;
         /* in the G-> W handover check the Nas-Sync-indicator for the codec type */
            if(rab_list_r6->elem[idx].rab_Info.m.nas_Synchronisation_IndicatorPresent)
            {
              rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].nas_Synchronisation_Indicator = 
                  rrc_translate_nas_sync_indicator(wrrc_as_id,&(rab_list_r6->elem[idx].rab_Info.nas_Synchronisation_Indicator));
              
              if( rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].nas_Synchronisation_Indicator == WCDMA_IVOCODER_ID_NONE )
        {
                rrciho_int_data.procedure_info.failure_cause = (uint32)RR_RRC_INVALID_CONFIGURATION;
                WRRC_MSG1_HIGH("Invalid configuration .. codec not supported %d",rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].nas_Synchronisation_Indicator );
          return FAILURE;
        }
            }
            rrc_set_ordered_codec_mode(wrrc_as_id,rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].nas_Synchronisation_Indicator);
          }
        number_of_rabs++;
      }
      else
  ...
  else if(release == MSG_REL7)
  {
    ...
    do 
    {
      ...
      if(T_rrc_RAB_Identity_gsm_MAP_RAB_Identity == rab_list_r7->elem[idx].rab_Info.rab_Identity.t)
      {
        ...
        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].rab_id =
           rrcrb_translate_gsm_map_rab_id(wrrc_as_id,rab_list_r7->elem[idx].rab_Info.rab_Identity.u.gsm_MAP_RAB_Identity);
      
        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].re_est_timer = rab_list_r7->elem[idx].rab_Info.re_EstablishmentTimer;

        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].cn_domain = rab_list_r7->elem[idx].rab_Info.cn_DomainIdentity;

        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].rab_id_is_gsm_map = TRUE;

        rb_list_r7 = &rab_list_r7->elem[idx].rb_InformationSetupList;

        rrciho_initialize_rb_list_r7(wrrc_as_id,rb_list_r7, number_of_rabs);

        (void)rrc_update_rab_type_in_rab_info_r7(wrrc_as_id,
        ...
        if(rab_list_r7->elem[idx].rab_Info.cn_DomainIdentity == rrc_CN_DomainIdentity_cs_domain )
        {
          ...
          {
            rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].nas_Synchronisation_Indicator = 
                rrc_translate_nas_sync_indicator(wrrc_as_id,&(rab_list_r7->elem[idx].rab_Info.nas_Synchronisation_Indicator));
          ...
          )
  {
    ...
    do 
    {
      ...
      if(T_rrc_RAB_Identity_gsm_MAP_RAB_Identity == rab_list_r8->elem[idx].rab_Info.rab_Identity.t)
      {
        ...
        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].rab_id =
           rrcrb_translate_gsm_map_rab_id(wrrc_as_id,rab_list_r8->elem[idx].rab_Info.rab_Identity.u.gsm_MAP_RAB_Identity);
      
        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].re_est_timer = rab_list_r8->elem[idx].rab_Info.re_EstablishmentTimer;
        // Nitin Change from rab_list_r6 to rab_list_r8
        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].cn_domain = rab_list_r8->elem[idx].rab_Info.cn_DomainIdentity;
    
        rrciho_int_data.gtow_info.est_rabs[number_of_rabs].rab_id_is_gsm_map = TRUE;
  
        rb_list_r8 = &rab_list_r8->elem[idx].rb_InformationSetupList;
      
        rrciho_initialize_rb_list_r8(wrrc_as_id,rb_list_r8, number_of_rabs);
  
        (void)rrc_update_rab_type_in_rab_info_r8(wrrc_as_id,
        ...
        if(rab_list_r8->elem[idx].rab_Info.cn_DomainIdentity == rrc_CN_DomainIdentity_cs_domain )
        {
          ...
          {
            rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].nas_Synchronisation_Indicator = 
                rrc_translate_nas_sync_indicator(wrrc_as_id,&(rab_list_r8->elem[idx].rab_Info.nas_Synchronisation_Indicator));
  ...
  rrciho_int_data.gtow_info.number_of_est_rabs = number_of_rabs;

  /*set rab included flag to true.  also domain id is always cn domain*/
  rrciho_int_data.gtow_info.rab_info_included = TRUE;
...
===========================================================================*/
static uecomdef_status_e_type rrciho_validate_and_extract_srb_rab_info( sys_modem_as_id_e_type wrrc_as_id)
{
...
#ifdef FEATURE_LTE_TO_WCDMA
  rrciho_int_data.gtow_info.cs_rab_index = 0;
  rrciho_int_data.gtow_info.is_cs_rab_present = FALSE;
  rrciho_int_data.gtow_info.is_ps_rab_present = FALSE;
  rrciho_int_data.gtow_info.number_of_est_rabs= 0;
#endif /* FEATURE_LTE_TO_WCDMA */

  /*if the configuration received is complete then process the struct for
    handover complete */
  if(rrciho_int_data.gtow_info.config_type == IHO_GTOW_COMPLETE_CONFIG)
  {
    if(rrciho_int_data.gtow_info.msg_rel_version == MSG_REL99)
    {
      /* Assign signalling radio bearers list to a local list */
      srb_list = &rrciho_int_data.gtow_info.ho_ptr->u.r3.handoverToUTRANCommand_r3.
                               specificationMode.u.complete.srb_InformationSetupList;
    }
    else if((rrciho_int_data.gtow_info.msg_rel_version == MSG_REL5) && (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5))
    {
      /* Assign signalling radio bearers list to a local list */
      srb_list_r5 = &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.r5.handoverToUTRANCommand_r5.
        specificationMode.u.complete.srb_InformationSetupList;
    }
    else if((rrciho_int_data.gtow_info.msg_rel_version == MSG_REL6) && (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL6))
    {
      srb_list_r6 = &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.
        criticalExtensions.u.r6.handoverToUTRANCommand_r6.specificationMode.u.complete.srb_InformationSetupList;
    }
    else if((rrciho_int_data.gtow_info.msg_rel_version == MSG_REL7) && (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL7))
    {
      srb_list_r7 = &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.
    ...
    else if((rrciho_int_data.gtow_info.msg_rel_version == MSG_REL8) && (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL8))
    {
      srb_list_r8 = &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.
...
#ifdef FEATURE_WCDMA_REL9
    else if((rrciho_int_data.gtow_info.msg_rel_version == MSG_REL9) && (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL9))
    {
      srb_list_r8 = &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.
...
#endif /* FEATURE_WCDMA_REL9 */
#ifdef FEATURE_WCDMA_REL10
    else if((rrciho_int_data.gtow_info.msg_rel_version == MSG_REL10) && (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL10))
    {
      srb_list_r8 = &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.
              criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.handoverToUTRANCommand_r10.specificationMode.u.complete.srb_InformationSetupList;
    }
#endif /* FEATURE_WCDMA_REL10 */
    ...
    if(rrciho_int_data.gtow_info.msg_rel_version == MSG_REL99)
    {
      ...
      if(rrciho_int_data.gtow_info.ho_ptr->u.r3.handoverToUTRANCommand_r3.specificationMode.u.complete.m.rab_InformationSetupListPresent)
      {

#ifdef FEATURE_LTE_TO_WCDMA
        if((rrciho_extract_and_fill_in_rab_rb_info(wrrc_as_id,(void *) &rrciho_int_data.gtow_info.ho_ptr->u.r3.handoverToUTRANCommand_r3.
        ...
        /* Check if the RAB id is GSM_MAP type. We don't support any
           other type */
        if(T_rrc_RAB_Identity_gsm_MAP_RAB_Identity == rrciho_int_data.gtow_info.ho_ptr->u.r3.handoverToUTRANCommand_r3.specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.rab_Identity.t)
        {
          rab_identity_choice = T_rrc_RAB_Identity_gsm_MAP_RAB_Identity;

          rrciho_int_data.gtow_info.est_rabs.rab_id =
             rrcrb_translate_gsm_map_rab_id(wrrc_as_id, rrciho_int_data.gtow_info.ho_ptr->u.r3.handoverToUTRANCommand_r3.
                                         specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.
                                         rab_Identity.u.gsm_MAP_RAB_Identity);

          rrciho_int_data.gtow_info.est_rabs.re_est_timer = rrciho_int_data.gtow_info.ho_ptr->u.r3.handoverToUTRANCommand_r3.
                 specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.re_EstablishmentTimer;

          rrciho_int_data.gtow_info.est_rabs.cn_domain = rrciho_int_data.gtow_info.ho_ptr->u.r3.handoverToUTRANCommand_r3.
                 specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.cn_DomainIdentity;

          rb_list = &rrciho_int_data.gtow_info.ho_ptr->u.r3.handoverToUTRANCommand_r3.
                                specificationMode.u.complete.rab_InformationSetupList.elem[0].
                                rb_InformationSetupList;

          rrciho_initialize_rb_list(wrrc_as_id,rb_list);

          rab_list = &rrciho_int_data.gtow_info.ho_ptr->u.r3.handoverToUTRANCommand_r3.specificationMode.u.complete.
           rab_InformationSetupList;
           /* in the G-> W handover check the Nas-Sync-indicator for the codec type */
          if(rrciho_update_nas_sync_ind(wrrc_as_id,rab_list->elem[0].rab_Info) == FAILURE)
          {
            rrciho_int_data.procedure_info.failure_cause = (uint32)RR_RRC_INVALID_CONFIGURATION;
            return FAILURE;
          }
    ...
    {
      if(rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.
            r5.handoverToUTRANCommand_r5.specificationMode.u.complete.m.rab_InformationSetupListPresent)
      {

#ifdef FEATURE_LTE_TO_WCDMA
        if((rrciho_extract_and_fill_in_rab_rb_info(wrrc_as_id,(void *) &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.r5.
        ...
        /* Check if the RAB id is GSM_MAP type. We don't support any
           other type */
        if(T_rrc_RAB_Identity_gsm_MAP_RAB_Identity == rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.r5.handoverToUTRANCommand_r5.specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.rab_Identity.t)
        {
          rab_identity_choice = T_rrc_RAB_Identity_gsm_MAP_RAB_Identity;
          ...
          rrciho_int_data.gtow_info.est_rabs.rab_id =
             rrcrb_translate_gsm_map_rab_id(wrrc_as_id,rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.r5.handoverToUTRANCommand_r5.
                                         specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.
                                         rab_Identity.u.gsm_MAP_RAB_Identity);

          rrciho_int_data.gtow_info.est_rabs.re_est_timer = rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.r5.handoverToUTRANCommand_r5.
                                         specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.re_EstablishmentTimer;

          rrciho_int_data.gtow_info.est_rabs.cn_domain = rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.r5.handoverToUTRANCommand_r5.
                                         specificationMode.u.complete.rab_InformationSetupList.
                                         elem[0].rab_Info.cn_DomainIdentity;

          rb_list_r5 = &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.r5.handoverToUTRANCommand_r5.
            specificationMode.u.complete.rab_InformationSetupList.elem[0].
            rb_InformationSetupList;

          rrciho_initialize_rb_list_r5(wrrc_as_id,rb_list_r5);

          rab_list_r5 = &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.r5.handoverToUTRANCommand_r5.specificationMode.u.complete.
           rab_InformationSetupList;

          /* in the G-> W handover check the Nas-Sync-indicator for the codec type */
          if(rrciho_update_nas_sync_ind(wrrc_as_id,rab_list_r5->elem[0].rab_Info) == FAILURE)
          {
            rrciho_int_data.procedure_info.failure_cause = (uint32)RR_RRC_INVALID_CONFIGURATION;
            return FAILURE;
          }
    ...
    {
      if(rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r6.handoverToUTRANCommand_r6.
         specificationMode.u.complete.m.rab_InformationSetupListPresent)
      {

#ifdef FEATURE_LTE_TO_WCDMA
        if((rrciho_extract_and_fill_in_rab_rb_info(wrrc_as_id,(void *) &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r6.
        ...
        /* Check if the RAB id is GSM_MAP type. We don't support any
           other type */
        if(T_rrc_RAB_Identity_gsm_MAP_RAB_Identity == rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r6
           .handoverToUTRANCommand_r6.specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.rab_Identity.t)
        {
          ...
          rrciho_int_data.gtow_info.est_rabs.rab_id =
             rrcrb_translate_gsm_map_rab_id(wrrc_as_id,rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r6.
                                            handoverToUTRANCommand_r6.specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.
                                            rab_Identity.u.gsm_MAP_RAB_Identity);

          rrciho_int_data.gtow_info.est_rabs.re_est_timer = rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.
          ...
          rrciho_int_data.gtow_info.est_rabs.cn_domain = rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.
            u.r6.handoverToUTRANCommand_r6.specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.cn_DomainIdentity;

          rb_list_r6 = &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.
            u.r6.handoverToUTRANCommand_r6.specificationMode.u.complete.rab_InformationSetupList.elem[0].rb_InformationSetupList;

          rrciho_initialize_rb_list_r6(wrrc_as_id,rb_list_r6);

          rab_list_r6 = &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.
          ...
          if(rab_list_r6->elem[0].rab_Info.cn_DomainIdentity == rrc_CN_DomainIdentity_cs_domain )
          {
            ...
            {
              rrciho_int_data.gtow_info.est_rabs.nas_Synchronisation_Indicator = 
                  rrc_translate_nas_sync_indicator(wrrc_as_id,(ASN1BitStr32  *)&(rab_list_r6->elem[0].rab_Info.nas_Synchronisation_Indicator));
    ...
    {
      if(rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r7.handoverToUTRANCommand_r7.
         specificationMode.u.complete.m.rab_InformationSetupListPresent)
      {
#ifdef FEATURE_LTE_TO_WCDMA
        if((rrciho_extract_and_fill_in_rab_rb_info(wrrc_as_id,(void *) &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.
        ...
        rab_list_r7 = rrciho_update_rab_info_r7(wrrc_as_id,&rab_identity_choice);
    ...
    {
      if(rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r8.handoverToUTRANCommand_r8.
         specificationMode.u.complete.m.rab_InformationSetupListPresent)
      {
#ifdef FEATURE_LTE_TO_WCDMA
        if((rrciho_extract_and_fill_in_rab_rb_info(wrrc_as_id,(void *) &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.
       ...
       rab_list_r8 = rrciho_update_rab_info_r8(wrrc_as_id,&rab_identity_choice 
#ifdef FEATURE_WCDMA_REL9
                                               ,MSG_REL8
#endif /* FEATURE_WCDMA_REL9 */
                                               );
    ...
    {
      if(rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.r9.handoverToUTRANCommand_r9.
         specificationMode.u.complete.m.rab_InformationSetupListPresent)
      {
#ifdef FEATURE_LTE_TO_WCDMA
        if((rrciho_extract_and_fill_in_rab_rb_info(wrrc_as_id,(void *) &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.
       ...
       rab_list_r8 = rrciho_update_rab_info_r8(wrrc_as_id,&rab_identity_choice,MSG_REL9);
#endif /*FEATURE_LTE_TO_WCDMA */
      }
    }
    ...
    {
      if(rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.handoverToUTRANCommand_r10.
         specificationMode.u.complete.m.rab_InformationSetupListPresent)
      {
#ifdef FEATURE_LTE_TO_WCDMA
        if((rrciho_extract_and_fill_in_rab_rb_info(wrrc_as_id,(void *) &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.
              u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.handoverToUTRANCommand_r10.specificationMode.u.complete.rab_InformationSetupList, MSG_REL10)) == FAILURE)
        {
          rrciho_int_data.procedure_info.failure_cause = (uint32)RR_RRC_INVALID_CONFIGURATION;
          return FAILURE;
        }
       ...
       rab_list_r8 = rrciho_update_rab_info_r8(wrrc_as_id,&rab_identity_choice,MSG_REL10);
#endif /*FEATURE_LTE_TO_WCDMA */
      }
    }
    ...
    */
    if((rab_list!=NULL) && (rab_list->n != 0))
    {
      (void)rrc_update_rab_type_in_rab_info(wrrc_as_id,
         &(rab_list->elem[0]),
         &(rrciho_int_data.gtow_info.est_rabs));
      }
    ...
    else if((rab_list_r8!=NULL) && (rab_list_r8->n != 0))
    {
      ...
      WRRC_MSG3_HIGH("RAB_ID:%d,Domain:%d,re_est_tmr:%d",
                        rrciho_int_data.gtow_info.est_rabs.rab_id,
                        rrciho_int_data.gtow_info.est_rabs.cn_domain,
                        rrciho_int_data.gtow_info.est_rabs.re_est_timer);
#endif /* FEATURE_LTE_TO_WCDMA */
...
/* Processing for default configs received in HO msg*/
    rrciho_int_data.gtow_info.rbs_ptr = NULL;
    rrciho_int_data.gtow_info.sib16_ptr = NULL;

    switch(rrciho_int_data.gtow_info.default_config_id)
    {
...
#ifdef FEATURE_WCDMA_REL10
      case RRC_DEF_CONFIG_24:
#endif
      { 

        if((rrciho_int_data.gtow_info.default_config_id == RRC_DEF_CONFIG_0 ) ||
           (rrciho_int_data.gtow_info.default_config_id == RRC_DEF_CONFIG_1 ) 
           ||(rrciho_int_data.gtow_info.default_config_id == RRC_DEF_CONFIG_22 )
#ifdef FEATURE_WCDMA_REL10
           ||(rrciho_int_data.gtow_info.default_config_id == RRC_DEF_CONFIG_24 )
#endif
          )
        {
          WRRC_MSG1_HIGH("G->W Signalling only default config Id %d chosen",rrciho_int_data.gtow_info.default_config_id);
        }
        /*first get the sib16 pointer.. currently only supporting default configuration.  will support
          real sib's soon*/
        rrciho_int_data.gtow_info.sib16_ptr =
          (rrc_SysInfoType16 *)
        rrcasn1_decode_pdu( wrrc_as_id,(void *)default_config_info[rrciho_int_data.gtow_info.default_config_id].default_config_ptr,
                            rrc_SysInfoType16_PDU,
                            default_config_info[rrciho_int_data.gtow_info.default_config_id].default_config_len
                            );
    
        rrciho_int_data.gtow_info.def_config_ptr_present = TRUE;
    
        /*initialize signalling radio bearer list*/
        srb_list = &rrciho_int_data.gtow_info.sib16_ptr->preDefinedRadioConfiguration.
                                   predefinedRB_Configuration.srb_InformationList;
      
        rb_list = &rrciho_int_data.gtow_info.sib16_ptr->preDefinedRadioConfiguration.
    ...
    case RRC_DEF_CONFIG_14:
      {
        ...
        {
          rrciho_int_data.gtow_info.rbs_ptr =
          (rrc_DL_DCCH_Message *)
          rrcasn1_decode_pdu(wrrc_as_id, (void *)default_config_info[rrciho_int_data.gtow_info.default_config_id].default_config_ptr,
          ...
          if(rrciho_int_data.gtow_info.rbs_ptr == NULL)
          {
            WRRC_ERR0_FATAL("No memory for RBS ptr");
          }   
          rrciho_int_data.gtow_info.def_config_ptr_present = TRUE;
          WRRC_MSG1_HIGH("Decoded DEFAULT CONFIG %d Successfully from internal RB setup message", rrciho_int_data.gtow_info.default_config_id );
    
          /*initialize signalling radio bearer list*/
          srb_list_r5 = &rrciho_int_data.gtow_info.rbs_ptr->message.u.radioBearerSetup.u.later_than_r3.criticalExtensions.u.
            criticalExtensions.u.r5.radioBearerSetup_r5.srb_InformationSetupList;
        
          rb_list_r5 = &rrciho_int_data.gtow_info.rbs_ptr->message.u.radioBearerSetup.u.later_than_r3.criticalExtensions.u.
      ...
      default:
        WRRC_MSG1_ERROR("Unsupported Def Config ID %d", rrciho_int_data.gtow_info.default_config_id);
        rrciho_int_data.procedure_info.failure_cause = (uint32)RR_RRC_INVALID_CONFIGURATION;
        return FAILURE;
    }
    ...
    if(rrciho_int_data.gtow_info.msg_rel_version == MSG_REL99)
    {
      rab_bit_mask =rrciho_int_data.gtow_info.ho_ptr->u.r3.handoverToUTRANCommand_r3.specificationMode.u.
       preconfiguration.m.rab_InfoPresent;
    }
    else if((rrciho_int_data.gtow_info.msg_rel_version == MSG_REL5) && (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5))
    {
      rab_bit_mask = rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.r5.handoverToUTRANCommand_r5.specificationMode.u.
       preconfiguration.m.rab_InfoPresent;
    }
    else if((rrciho_int_data.gtow_info.msg_rel_version == MSG_REL6) && (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL6))
    {
      rab_bit_mask = rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r6.handoverToUTRANCommand_r6.
        specificationMode.u.preconfiguration.m.rab_InfoPresent;
    }
    else if((rrciho_int_data.gtow_info.msg_rel_version == MSG_REL7) && (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL7))
    {
      rab_bit_mask = rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r7.handoverToUTRANCommand_r7.
        specificationMode.u.preconfiguration.m.rab_InfoPresent;
    }
    else if((rrciho_int_data.gtow_info.msg_rel_version == MSG_REL8) && (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL8))
    {
      rab_bit_mask = rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r8.handoverToUTRANCommand_r8.
        specificationMode.u.preconfiguration.m.rab_InfoPresent;
    }
#ifdef FEATURE_WCDMA_REL9
    else if((rrciho_int_data.gtow_info.msg_rel_version == MSG_REL9) && (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL9))
    {
      rab_bit_mask = rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.
    ...
    {
      rab_bit_mask = rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.
    	later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.handoverToUTRANCommand_r10.specificationMode.u.preconfiguration.m.rab_InfoPresent;
    }
    ...
    if(rab_bit_mask)
    {
      ...
      if(rrciho_int_data.gtow_info.msg_rel_version == MSG_REL99)
      {
        rab_identity_choice = rrciho_int_data.gtow_info.ho_ptr->u.r3.handoverToUTRANCommand_r3.specificationMode.u.
           preconfiguration.rab_Info.rab_Identity.t;
        if(rrciho_update_def_config_nas_sync_ind(wrrc_as_id,
           &(rrciho_int_data.gtow_info.ho_ptr->u.r3.handoverToUTRANCommand_r3.specificationMode.u.preconfiguration.rab_Info)
#ifdef FEATURE_LTE_TO_WCDMA
           , rrciho_int_data.gtow_info.cs_rab_index
#endif /* FEATURE_LTE_TO_WCDMA */
           ) == FAILURE)
        {
          rrciho_int_data.procedure_info.failure_cause = (uint32)RR_RRC_INVALID_CONFIGURATION;
      ...
      {
        rab_identity_choice = rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.r5.handoverToUTRANCommand_r5.specificationMode.u.
           preconfiguration.rab_Info.rab_Identity.t;
        if( rrciho_update_def_config_nas_sync_ind(wrrc_as_id,
            &(rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.r5.handoverToUTRANCommand_r5.
        ...
        if( rrciho_int_data.gtow_info.default_config_id ==  RRC_DEF_CONFIG_13)
        {
#ifndef FEATURE_LTE_TO_WCDMA
          rrciho_int_data.gtow_info.est_rabs.nas_Synchronisation_Indicator = WCDMA_IVOCODER_ID_AMRWB;
#else
          rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].nas_Synchronisation_Indicator = WCDMA_IVOCODER_ID_AMRWB;
#endif 
          rrc_set_ordered_codec_mode(wrrc_as_id, WCDMA_IVOCODER_ID_AMRWB);
        }
      }

      else if((rrciho_int_data.gtow_info.msg_rel_version == MSG_REL6) && (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL6))
      {
        rab_identity_choice = rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r6.
          handoverToUTRANCommand_r6.specificationMode.u.preconfiguration.rab_Info.rab_Identity.t;
        if( rrciho_update_def_config_nas_sync_ind(wrrc_as_id,
            &(rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r6.
        ...
        if( (rrciho_int_data.gtow_info.default_config_id ==  RRC_DEF_CONFIG_16) ||
            ( rrciho_int_data.gtow_info.default_config_id ==  RRC_DEF_CONFIG_21)
        || ( rrciho_int_data.gtow_info.default_config_id ==  RRC_DEF_CONFIG_13)
         )
        {
#ifndef FEATURE_LTE_TO_WCDMA
          rrciho_int_data.gtow_info.est_rabs.nas_Synchronisation_Indicator = WCDMA_IVOCODER_ID_AMRWB;
      ...
      {
        rab_identity_choice = rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r7.
          handoverToUTRANCommand_r7.specificationMode.u.preconfiguration.rab_Info.rab_Identity.t;
        if( rrciho_update_def_config_nas_sync_ind(wrrc_as_id,
            &(rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r7.
      ...
      {
        rab_identity_choice = rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r8.
          handoverToUTRANCommand_r8.specificationMode.u.preconfiguration.rab_Info.rab_Identity.t;
        if( rrciho_update_def_config_nas_sync_ind(wrrc_as_id,
            &(rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r8.
      ...
      {
        rab_identity_choice = rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.r9.
          handoverToUTRANCommand_r9.specificationMode.u.preconfiguration.rab_Info.rab_Identity.t;
        if( rrciho_update_def_config_nas_sync_ind(wrrc_as_id,
            &(rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.
      ...
      {
        rab_identity_choice = rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.
         handoverToUTRANCommand_r10.specificationMode.u.preconfiguration.rab_Info.rab_Identity.t;
        if( rrciho_update_def_config_nas_sync_ind(wrrc_as_id,
        &(rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.
        later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.handoverToUTRANCommand_r10.specificationMode.u.preconfiguration.rab_Info)
      #ifdef FEATURE_LTE_TO_WCDMA
        ,rrciho_int_data.gtow_info.cs_rab_index
      #endif /* FEATURE_LTE_TO_WCDMA */
       ) == FAILURE)
      ...
      if (rab_identity_choice == T_rrc_RAB_Identity_gsm_MAP_RAB_Identity)
      {
        ...
        if(rrciho_int_data.gtow_info.msg_rel_version == MSG_REL99)
        {
          rab_id = rrcrb_translate_gsm_map_rab_id(wrrc_as_id,rrciho_int_data.gtow_info.ho_ptr->u.r3.handoverToUTRANCommand_r3.
                                         specificationMode.u.preconfiguration.rab_Info.
                                         rab_Identity.u.gsm_MAP_RAB_Identity);
        }
        else if((rrciho_int_data.gtow_info.msg_rel_version == MSG_REL5) && (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5))
        {
          rab_id = rrcrb_translate_gsm_map_rab_id(wrrc_as_id,rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.r5.handoverToUTRANCommand_r5.
                                         specificationMode.u.preconfiguration.rab_Info.
                                         rab_Identity.u.gsm_MAP_RAB_Identity);
        }
        else if((rrciho_int_data.gtow_info.msg_rel_version == MSG_REL6) && (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL6))
        {
          rab_id = rrcrb_translate_gsm_map_rab_id(wrrc_as_id,rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.
        ...
        {
          rab_id = rrcrb_translate_gsm_map_rab_id(wrrc_as_id,rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.
                                            criticalExtensions.u.r7.handoverToUTRANCommand_r7.specificationMode.u.
                                            preconfiguration.rab_Info.rab_Identity.u.gsm_MAP_RAB_Identity);
        }
        else if((rrciho_int_data.gtow_info.msg_rel_version == MSG_REL8) && (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL8))
        {
          rab_id = rrcrb_translate_gsm_map_rab_id(wrrc_as_id,rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.
      ...
      {
       rab_id = rrcrb_translate_gsm_map_rab_id(wrrc_as_id,rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.
                             criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.handoverToUTRANCommand_r10.specificationMode.u.
                               preconfiguration.rab_Info.rab_Identity.u.gsm_MAP_RAB_Identity);
      }
      ...
      WRRC_MSG4_HIGH( 
            ...
            rrciho_int_data.gtow_info.default_config_id, rab_type,rab_id, re_est_timer);		

#ifndef FEATURE_LTE_TO_WCDMA

        rrciho_int_data.gtow_info.est_rabs.rab_id = rab_id;
        rrciho_int_data.gtow_info.est_rabs.re_est_timer = re_est_timer;
        rrciho_int_data.gtow_info.est_rabs.rab_type = rab_type;
        ...
        {
          rrciho_initialize_rb_list_r8(wrrc_as_id,rb_list_r8);
        }
#else
        ...
        rrciho_int_data.gtow_info.is_cs_rab_present = TRUE;
        rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].rab_id = rab_id;
        rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].re_est_timer = re_est_timer;
        rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].rab_type = rab_type;

        rrciho_int_data.gtow_info.rab_info_included = TRUE;
        rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].cn_domain = RRC_CS_DOMAIN_CN_ID;

        /* Store this info locally so that ESTABLISHED_RABS can
           be updated later, when these RABs are established */
        rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].rab_id_is_gsm_map = TRUE;

        rrciho_int_data.gtow_info.number_of_est_rabs = 1;

        if (rb_list != NULL) 
        {
          rrciho_initialize_rb_list(wrrc_as_id,rb_list, rrciho_int_data.gtow_info.cs_rab_index);
        }
        else if (rb_list_r5!= NULL) 
        {
          rrciho_initialize_rb_list_r5(wrrc_as_id,rb_list_r5, rrciho_int_data.gtow_info.cs_rab_index);
        }
        else if (rb_list_r6 != NULL) 
        {
          rrciho_initialize_rb_list_r6(wrrc_as_id,rb_list_r6, rrciho_int_data.gtow_info.cs_rab_index);
        }
        else if (rb_list_r7 != NULL) 
        {
          rrciho_initialize_rb_list_r7(wrrc_as_id,rb_list_r7, rrciho_int_data.gtow_info.cs_rab_index);
        }
        else if (rb_list_r8 != NULL) 
        {
          rrciho_initialize_rb_list_r8(wrrc_as_id,rb_list_r8, rrciho_int_data.gtow_info.cs_rab_index);
        }
  ...
  if(NULL != srb_list)
  {
    rrciho_initialize_srb_list(wrrc_as_id,srb_list);
  }
  else if (NULL != srb_list_r5) 
  {
    rrciho_initialize_srb_list_r5(wrrc_as_id,srb_list_r5);
  }
  else if (NULL != srb_list_r6) 
  {
    rrciho_initialize_srb_list_r6(wrrc_as_id,srb_list_r6);
  }
  else if (NULL != srb_list_r7) 
  {
    rrciho_initialize_srb_list_r7(wrrc_as_id,srb_list_r7);
  }
  else if (NULL != srb_list_r8) 
  {
    rrciho_initialize_srb_list_r8(wrrc_as_id,srb_list_r8);
  }
  ...
  if(rab_info_list_present == TRUE)
  {
    ...
    /*set rab included flag to true.  also domain id is always cn domain*/
    rrciho_int_data.gtow_info.rab_info_included = TRUE;
    rrciho_int_data.gtow_info.est_rabs.cn_domain = RRC_CS_DOMAIN_CN_ID;

    /* Store this info locally so that ESTABLISHED_RABS can
       be updated later, when these RABs are established */
    rrciho_int_data.gtow_info.est_rabs.rab_id_is_gsm_map = TRUE;     
  }

#else
  WRRC_MSG3_HIGH("CS RAB Present: %d, CS RAB Index : %d, PS RAB Present: %d",rrciho_int_data.gtow_info.is_cs_rab_present,
    rrciho_int_data.gtow_info.cs_rab_index,rrciho_int_data.gtow_info.is_ps_rab_present);

if(((rrciho_int_data.gtow_info.is_cs_rab_present == TRUE) && (iho_interrat_type == RRCIHO_INTERRAT_LTE) && (rrciho_int_data.gtow_info.cs_ho_is_supported == FALSE))||
    ((rrciho_int_data.gtow_info.is_ps_rab_present == TRUE) && (iho_interrat_type == RRCIHO_INTERRAT_LTE) && (rrciho_int_data.gtow_info.ps_ho_is_supported == FALSE)))
  {
    WRRC_MSG2_HIGH("CS supported: %d, PS supported: %d",rrciho_int_data.gtow_info.cs_ho_is_supported,rrciho_int_data.gtow_info.ps_ho_is_supported);
    rrciho_int_data.procedure_info.failure_cause = (uint32)RR_RRC_INVALID_CONFIGURATION;
    return FAILURE;    
  }
#endif /* FEATURE_LTE_TO_WCDMA */
...
void  rrciho_set_tx_dix_mode_info(sys_modem_as_id_e_type wrrc_as_id,rrc_TX_DiversityMode tx_div_mode)
{
  switch(tx_div_mode)
  {
    ...
    case rrc_TX_DiversityMode_closedLoopMode1:
      ...
      rrciho_int_data.gtow_info.diversity_info_present = TRUE;
      rrciho_int_data.gtow_info.diversity_info = TRUE;
    break;
...
)
{
  ...
  rrciho_int_data.gtow_info.msg_rel_version = MSG_INVALID;

  memset((void *)&scrambling_code_type, 0, sizeof(rrc_scr_code_type));

  if(ho_ptr->t == T_rrc_HandoverToUTRANCommand_r3)
  {
    rrciho_int_data.gtow_info.msg_rel_version = MSG_REL99;
    ...
    {
      rrciho_int_data.gtow_info.activation_time =
       ...
       rrc_HandoverToUTRANCommand_r3_IEs_specificationMode_complete ))
    {
      ...
      {
        rrciho_set_tx_dix_mode_info(wrrc_as_id,dl_common_info->modeSpecificInfo.u.fdd->tx_DiversityMode);
      }
      else
      {
  ...
  {
    rrciho_int_data.gtow_info.msg_rel_version = MSG_REL5;

    WRRC_MSG0_HIGH("RRCIHO - Rel5 message received");

#ifdef FEATURE_LTE_TO_WCDMA
    /* In case of PSHO from EUTRA, only process complete config */
    if((rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.r5.handoverToUTRANCommand_r5.specificationMode.t ==
       T_rrc_HandoverToUTRANCommand_r5_IEs_specificationMode_preconfiguration )
      && (rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.r5.handoverToUTRANCommand_r5.specificationMode.u.preconfiguration.preConfigMode.t ==
    ...
    /* Check if Complete Configuration is sent and set that t in the
      procedure data*/
    if(RRC_CHECK_COMMON_MSG_TYPE(rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.r5.handoverToUTRANCommand_r5.specificationMode,
       rrc_HandoverToUTRANCommand_r5_IEs_specificationMode_complete ))
    {
      ...
      {
        rrciho_set_tx_dix_mode_info(wrrc_as_id,dl_common_info_r4->modeSpecificInfo.u.fdd->tx_DiversityMode);
      }
      else
      {
        /* If the IE is absent then set diversity_info_present to FALSE so that meas data from other
  ...
  {
    rrciho_int_data.gtow_info.msg_rel_version = MSG_REL6;

    WRRC_MSG0_HIGH("RRCIHO - Rel6 message received");

#ifdef FEATURE_LTE_TO_WCDMA
    /* In case of PSHO from EUTRA, only process complete config */
    if((rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r6.handoverToUTRANCommand_r6.specificationMode.t ==
       T_rrc_HandoverToUTRANCommand_r6_IEs_specificationMode_preconfiguration )
      && (rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r6.handoverToUTRANCommand_r6.
    ...
    /* Check if Complete Configuration is sent and set that t in the
      procedure data*/
    if(RRC_CHECK_COMMON_MSG_TYPE(rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r6.handoverToUTRANCommand_r6.specificationMode,
       rrc_HandoverToUTRANCommand_r6_IEs_specificationMode_complete ))
    {
      ...
      {
        rrciho_set_tx_dix_mode_info(wrrc_as_id,dl_common_info_r6->modeSpecificInfo.u.fdd->tx_DiversityMode);
      }
      else
      {
        /* If the IE is absent then set diversity_info_present to FALSE so that meas data from other
          ...
          (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL7))
  {
    ...
    rrciho_int_data.gtow_info.msg_rel_version = MSG_REL7;
   ...
   /*In R7 IHO, IE "supportForChangeOfUE_Capability" mandatory. We need to store it in
    ...
    {
      rrccouec.rrc_ordered_rnc_cap_change_support = RNC_CAP_CHANGE_SUPPORTED;
    }
    else
    {
      rrccouec.rrc_ordered_rnc_cap_change_support = RNC_CAP_CHANGE_UNSUPPORTED;
    }
       ...
       T_rrc_HandoverToUTRANCommand_r7_IEs_specificationMode_complete)
    {
     ...
     {
       rrciho_set_tx_dix_mode_info(wrrc_as_id,dl_common_info_r7->modeSpecificInfo.u.fdd->tx_DiversityMode);
     }
     else
     {
       /* If the IE is absent then set diversity_info_present to FALSE so that meas data from other
          ...
          (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL8))
  {
    ...
    rrciho_int_data.gtow_info.msg_rel_version = MSG_REL8;
   ...
   /*In R7 IHO, IE "supportForChangeOfUE_Capability" mandatory. We need to store it in
       ...
       rrc_HandoverToUTRANCommand_r8_IEs_specificationMode_complete))
    {
      ...
      {
        rrciho_set_tx_dix_mode_info(wrrc_as_id,dl_common_info_r8->modeSpecificInfo.u.fdd->tx_DiversityMode);
      }
      else
      {
        /* If the IE is absent then set diversity_info_present to FALSE so that meas data from other
            ...
            (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL9))
  {
    ...
    rrciho_int_data.gtow_info.msg_rel_version = MSG_REL9;
      ...
      else
      {
        WRRC_MSG0_ERROR("Default config info is for TDD mode ... rejecting ...");
            ...
            (ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.t == 
              T_rrc_HandoverToUTRANCommand_criticalExtensions_later_r8_criticalExtensions) &&
            (ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.t == 
              T_rrc_HandoverToUTRANCommand_later_r8_criticalExtensions_later_r9_criticalExtensions) &&
            (ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.t == 
              T_rrc_HandoverToUTRANCommand_LaterCriticalExtensions_r10) &&
            (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL10))
  {
    rrc_HandoverToUTRANCommand_r10_IEs * ho_to_utran_ptr_r10;
    rrciho_int_data.gtow_info.msg_rel_version = MSG_REL10;
  
    WRRC_MSG0_HIGH("RRCIHO - Rel10 message received");
    ho_to_utran_ptr_r10 = &ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.
    later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.handoverToUTRANCommand_r10;
  
  
#ifdef FEATURE_LTE_TO_WCDMA
    /* In case of PSHO from EUTRA, only process complete config */
    if((ho_to_utran_ptr_r10->specificationMode.t ==
    T_rrc_HandoverToUTRANCommand_r10_IEs_specificationMode_preconfiguration)
    && (ho_to_utran_ptr_r10->specificationMode.u.preconfiguration.preConfigMode.t == 
          T_rrc_HandoverToUTRANCommand_r10_IEs_preConfigMode_predefinedConfigIdentity)
    && ( iho_interrat_type == RRCIHO_INTERRAT_LTE))
    {
      WRRC_MSG0_HIGH("LTOW_PSHO:Predefined Config ID present. Send failure to LRRC.");
      return FAILURE;
...
#endif
    if(RRC_MSG_COMMON_BITMASK_IE_PTR(ho_to_utran_ptr_r10,
           rrc_HandoverToUTRANCommand_r10_IEs,cipheringAlgorithm))
    {
      ciphering_algo_present = TRUE;
      /*Copy Ciphering Algo to local struct*/
      ciphering_algo_r7 = 
      ho_to_utran_ptr_r10->cipheringAlgorithm;
    }
 ...
 /*In R7 IHO, IE "supportForChangeOfUE_Capability" mandatory. We need to store it in
  "rrc_ordered_rnc_cap_change_support" so that later once the procedure is over successfully, global
  "rrc_rnc_cap_change_support" can be updated accordingly. */  
    if(ho_to_utran_ptr_r10->supportForChangeOfUE_Capability == TRUE)
    {
      rrccouec.rrc_ordered_rnc_cap_change_support = RNC_CAP_CHANGE_SUPPORTED;
    }
    else
    ...
    /* Check if Complete Configuration is sent and set that t in the
procedure data*/
    if(RRC_CHECK_COMMON_MSG_TYPE(ho_to_utran_ptr_r10->specificationMode,
        rrc_HandoverToUTRANCommand_r10_IEs_specificationMode_complete))
    {
      config_type = IHO_GTOW_COMPLETE_CONFIG;
  
      WRRC_MSG0_HIGH("RRCIHO - complete configuration received");
  
      dl_info_per_rl_r7 =&ho_to_utran_ptr_r10->specificationMode.u.complete.dl_InformationPerRL_List;
      ...
      if(dl_info_per_rl_r7->elem[0].modeSpecificInfo.t == T_rrc_DL_InformationPerRL_r7_modeSpecificInfo_fdd)
      {
        ...
        freq_info = ho_to_utran_ptr_r10->specificationMode.u.complete.frequencyInfo.modeSpecificInfo.u.fdd;
    ...
    else if(ho_to_utran_ptr_r10->specificationMode.u.preconfiguration.preConfigMode.t == 
           T_rrc_HandoverToUTRANCommand_r10_IEs_preConfigMode_defaultConfig)
    {
      config_type = IHO_GTOW_DEFAULT_CONFIG;
      
      WRRC_MSG0_HIGH("RRCIHO - Rel10: default configuration received");
      
      default_config_mode = ho_to_utran_ptr_r10->specificationMode.u.preconfiguration.preConfigMode.u.defaultConfig.defaultConfigMode;
      
      if(default_config_mode == rrc_DefaultConfigMode_fdd)
      {
        default_config_id = ho_to_utran_ptr_r10->specificationMode.u.preconfiguration.preConfigMode.u.defaultConfig.defaultConfigIdentity;
      
        if(ho_to_utran_ptr_r10->specificationMode.u.
            preconfiguration.modeSpecificInfo.t == T_rrc_HandoverToUTRANCommand_r10_IEs_modeSpecificInfo_fdd)
        {
          dl_info_per_rl_default_r6 = &ho_to_utran_ptr_r10->specificationMode.u.preconfiguration.modeSpecificInfo.u.fdd.dl_InformationPerRL_List;
      
          /* get the primary scrambling code */
          scrambling_code_type = (uint16)( ho_to_utran_ptr_r10->specificationMode.u.preconfiguration.modeSpecificInfo.
                u.fdd.dl_InformationPerRL_List.elem[0].primaryCPICH_Info.primaryScramblingCode);
      
          /* Check if the UARFCN is supported */
          freq_info =  &ho_to_utran_ptr_r10->specificationMode.u.
          preconfiguration.modeSpecificInfo.u.fdd.frequencyInfo;
        }
        else
        {
  ...
  {
    rrc_print_supported_asn1_rel_version(wrrc_as_id);
    rrciho_int_data.procedure_info.failure_cause = (uint32)RR_RRC_WCDMA_PROTOCOL_ERROR;
  ...
  if(default_config_mode == rrc_DefaultConfigMode_fdd)
  {
    if(config_type == IHO_GTOW_DEFAULT_CONFIG)
    {
      ...
      case RRC_DEF_CONFIG_24:
#endif
        rrciho_int_data.gtow_info.default_config_id = default_config_id;
  ...
  switch(rrciho_int_data.gtow_info.msg_rel_version )
  {
    case MSG_REL5:
    ...
    default:
      WRRC_MSG1_HIGH("Invalid message version, return FAILURE",rrciho_int_data.gtow_info.msg_rel_version);
      return FAILURE;
  }

#ifdef FEATURE_MCC_AND_850_800_BAND_COMBINATION
  if(validate_fdd_frequency(wrrc_as_id, freq_info, plmn_id) == FAILURE)
#else
  if(validate_fdd_frequency(wrrc_as_id, freq_info) == FAILURE)
#endif
  {
    rrciho_int_data.procedure_info.failure_cause = (uint32)RR_RRC_FREQUENCY_NOT_IMPLEMENTED;
  ...
  else
  {
    ...
    rrciho_int_data.gtow_info.freq = (uint16)freq_info->uarfcn_DL;
  }

  if(ciphering_algo_present)
  {
    rrciho_int_data.gtow_info.ciph_algo_present  = ciphering_algo_present;
    if((rrciho_int_data.gtow_info.msg_rel_version == MSG_REL7)
     || (rrciho_int_data.gtow_info.msg_rel_version == MSG_REL8)
#ifdef FEATURE_WCDMA_REL9
     || (rrciho_int_data.gtow_info.msg_rel_version == MSG_REL9)
#endif /* FEATURE_WCDMA_REL9 */
    ...
    {
      rrciho_int_data.gtow_info.cipheringAlgorithm = rrcsmc_get_int_cipher_algo_from_dl_sdu_ind(wrrc_as_id,
                                              &ciphering_algo_r7,
                                              MSG_REL7);
    }
    else
  ...
  rrciho_int_data.gtow_info.config_type = config_type;
  rrciho_int_data.gtow_info.scr_code    = scrambling_code_type;

  /* check if we are successful in extracting the SRB and RB information */
  return rrciho_validate_and_extract_srb_rab_info(wrrc_as_id);
...
)
{
      ...
      count ++)
  {
    ...
    if((meas_data->wcdma_cell_measurements[count].primary_scrambling_code == rrciho_int_data.gtow_info.scr_code) &&
       (meas_data->wcdma_cell_measurements[count].uarfcn == rrciho_int_data.gtow_info.freq))
    {
      ...
      {
        rrciho_int_data.gtow_info.pn_offset_present = TRUE;
        rrciho_int_data.gtow_info.pn_offset = meas_data->wcdma_cell_measurements[count].PN_Offset;
      }
      else
      {
        rrciho_int_data.gtow_info.pn_offset_present = FALSE;
      ...
      if(iho_interrat_type == RRCIHO_INTERRAT_LTE)
      {
        ...
        if(FALSE == rrciho_int_data.gtow_info.diversity_info_present)
        {
...
===========================================================================*/
static void rrciho_send_srvcc_handover_failure_ind(sys_modem_as_id_e_type wrrc_as_id)
{
...
#ifdef FEATURE_DUAL_SIM
  mm_buf_ptr->cmd.rrc_srvcc_handover_failure_ind.as_id = rrc_get_as_id(wrrc_as_id);
#endif 
...
===========================================================================*/
static void rrciho_send_ltow_ps_handover_to_utran_success(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  rrciho_delete_stored_gtow_info(wrrc_as_id);

  /*initialize the HO parameters to default*/
  rrciho_init_data(wrrc_as_id);

  /*set HO status to success*/
  ltow_ho_cnf.handover_status = LTOW_PSHO_SUCCESS;

  rrc_send_lte_cmd(wrrc_as_id,&ltow_ho_cnf.msg_hdr,WCDMA_RRC_LTE_PSHO_RSP,sizeof(wcdma_rrc_lte_psho_rsp_type));
  WRRC_MSG0_HIGH("LTOW: WCDMA_RRC_LTE_PSHO_RSP success sent to LTE RRC");

#ifdef WCDMA_RRC_INDICATE_CODEC_TO_CM    
    if(rrciho_int_data.gtow_info.is_cs_rab_present)
    {
      rrc_send_hho_cmd_to_cm(wrrc_as_id,SYS_HO_COMPLETE,SYS_HO_TYPE_LTE_UMTS_SRVCC);      
    }
#endif
  return;
...
===========================================================================*/
static void rrciho_send_ltow_ps_handover_to_utran_failure(sys_modem_as_id_e_type wrrc_as_id)
{
  /*declare var of struct handover confirm type*/
  wcdma_rrc_lte_psho_rsp_type ltow_ho_cnf;
  ...
  switch( rrciho_int_data.procedure_info.failure_cause)
  {
  ...
  /* CSFB call is a failure hence the lock needs to be exchanged with IRAT client if TRM exchange
     was success earlier */
  rrc_check_and_perform_trm_exchange(wrrc_as_id);
  ...
  if(rrciho_int_data.gtow_info.is_cs_rab_present)
  {
    ...
    rrciho_send_srvcc_handover_failure_ind(wrrc_as_id);

#ifdef WCDMA_RRC_INDICATE_CODEC_TO_CM
    rrc_send_hho_cmd_to_cm(wrrc_as_id,SYS_HO_FAIL,SYS_HO_TYPE_LTE_UMTS_SRVCC);      
#endif
...
===========================================================================*/
static void rrciho_send_ltow_ps_handover_to_utran_abort_cnf(sys_modem_as_id_e_type wrrc_as_id)
{
  /*declare var of struct handover confirm type*/
  ...
  rrc_send_lte_cmd(wrrc_as_id,&ltow_abort_ho_cnf.msg_hdr,WCDMA_RRC_LTE_ABORT_PSHO_RSP,sizeof(wcdma_rrc_lte_abort_psho_rsp_type));
...
===========================================================================*/
static void rrciho_process_channel_config_cnf(sys_modem_as_id_e_type wrrc_as_id,uint32 current_mode)
{
  rrcllc_amr_mapping_info_type amr_info = {0}; /* To store AMR mode info incase a voice RAB has been setup */
#ifdef FEATURE_WCDMA_REL7_SPECRS 
  rrc_save_rnc_cap_change_support(wrrc_as_id);
#endif
  ...
  rrc_set_current_codec_mode(wrrc_as_id,current_mode);
  
  if( current_mode == WCDMA_IVOCODER_ID_AMRWB)
  {
    /* Configure the codec for WB AMR */
    amr_info.mode_type = WCDMA_IVOCODER_ID_AMRWB;
    (void) rrcllc_get_amr_wb_mapping(wrrc_as_id,&(amr_info));
  }
  else
  {
    amr_info.mode_type = WCDMA_IVOCODER_ID_AMR;
    /*Now MVS needs to be configured. The structure amr_info will be filled by this function. */
    (void)rrcllc_get_amr_mapping(wrrc_as_id,&(amr_info));
  }
  /*Configure voice adapter to register RLC queues for AMR Call.*/
  rrc_configure_voice_adapter(wrrc_as_id,&amr_info,FALSE,FALSE,WCDMA_IVOICE_EVENT_REQUEST_CODEC_MODE);
  rrc_configure_voice_adapter(wrrc_as_id,&amr_info,FALSE,FALSE,WCDMA_IVOICE_EVENT_REQUEST_SCR_MODE);
  rrc_configure_voice_adapter(wrrc_as_id,&amr_info,FALSE,FALSE,WCDMA_IVOICE_EVENT_SET_LOGICAL_CHANNELS);
  rrc_configure_voice_adapter(wrrc_as_id,&amr_info,FALSE,FALSE,WCDMA_IVOICE_EVENT_REQUEST_START);
	WRRC_MSG6_HIGH("WV_WRRC: configured codec_mode: %d, scr_mode: %d, ul_amr_a_id: %d, ul_amr_b_id:%d, ul_amr_c_id:%d, codec_mode_type: %d",
		amr_info.amr_mode, amr_info.scr_mode, amr_info.ul_amr_a_id, amr_info.ul_amr_b_id, amr_info.ul_amr_c_id, amr_info.mode_type);
  return;
}
...
===========================================================================*/
struct rrc_SysInfoType16 * rrciho_get_sib16_ptr(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  return rrciho_int_data.gtow_info.sib16_ptr;
}
...
)
{
  ...
  if((rrciho_int_data.gtow_info.msg_rel_version == MSG_REL5)
     || (rrciho_int_data.gtow_info.msg_rel_version == MSG_REL6)
     || (rrciho_int_data.gtow_info.msg_rel_version == MSG_REL7)
     || (rrciho_int_data.gtow_info.msg_rel_version == MSG_REL8)
#ifdef FEATURE_WCDMA_REL9
  ...
  { 
    if(rrciho_int_data.gtow_info.rbs_ptr != NULL)
    {
      df_iho_ptr = (void *)&rrciho_int_data.gtow_info.rbs_ptr->message.u.radioBearerSetup.u.later_than_r3.criticalExtensions.u.criticalExtensions.u.r5.radioBearerSetup_r5;
      *df_config_id = rrciho_int_data.gtow_info.default_config_id;
    }
  }
  /*return the ptr to the sib16*/
  return df_iho_ptr;  
...
===========================================================================*/
static uecomdef_status_e_type rrciho_send_channel_config_for_handover_to_utran(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  oc_status =   rrcllc_set_ordered_config(wrrc_as_id,RRC_PROCEDURE_IHO,
                                          RRC_STATE_CELL_DCH,
                                          rrc_HandoverToUTRANCommand_PDU,
                                          rrciho_int_data.gtow_info.ho_ptr);
  ...
  if(rrc_qsh_metric_db[WRRC_QSH_METRIC_OTA].data_ptr != NULL)
  {
    ...
    rrc_qsh_log_ota_params(wrrc_as_id, RRC_PROCEDURE_IHO);
   ...
   */
#ifndef FEATURE_LTE_TO_WCDMA
  if(RRCRB_CS_VOICE_RAB == rrciho_int_data.gtow_info.est_rabs.rab_type)
#else
  if(rrciho_int_data.gtow_info.is_cs_rab_present)
#endif
  {
    uint32 codec_info =WCDMA_IVOCODER_ID_NONE;
    dch_id_mapping dch_mapping[MAX_DCH_VOICE_CHANNEL];
    (void)rrcllc_derive_amr_rate(wrrc_as_id,&codec_info,dch_mapping);
    if(codec_info != WCDMA_IVOCODER_ID_NONE )
    {
      rrcllcoc_update_oc_orderd_codec_mode(wrrc_as_id,codec_info);
#ifndef FEATURE_LTE_TO_WCDMA
      rrciho_int_data.gtow_info.est_rabs.nas_Synchronisation_Indicator = codec_info;
      WRRC_MSG1_HIGH("Setting Codec Info as %d in gtow_info",rrciho_int_data.gtow_info.est_rabs.nas_Synchronisation_Indicator);  
#else
      if(RRCRB_CS_VOICE_RAB == rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].rab_type)
      {
        rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].nas_Synchronisation_Indicator = codec_info;
  ...
  for(i=0; (i < rrciho_int_data.gtow_info.num_srb) && (i < UE_MAX_SRB_SETUP); i++)
  {
    cmd_ptr->cmd.chan_config_req.rb[i].rb_id = rrciho_int_data.gtow_info.srb_id[i];
    cmd_ptr->cmd.chan_config_req.rb[i].rb_config = ESTABLISH_RB;
  }

#ifdef FEATURE_LTE_TO_WCDMA
  for(j = 0; (j < rrciho_int_data.gtow_info.number_of_est_rabs) && (j < MAX_RAB_TO_SETUP); j++)
  {
    for(i=0;(i < rrciho_int_data.gtow_info.est_rabs[j].num_rbs_for_rab) && (i < MAX_RB_PER_RAB); i++)
    {
      if((number_of_rbs + rrciho_int_data.gtow_info.num_srb) < MAX_RB)
      {
        /*set the rb's that need to be initialized */
        cmd_ptr->cmd.chan_config_req.rb[(number_of_rbs + rrciho_int_data.gtow_info.num_srb)].rb_id =
          (uint16)(rrciho_int_data.gtow_info.est_rabs[j].rb_for_rab[i].rb_id);

        cmd_ptr->cmd.chan_config_req.rb[(number_of_rbs + rrciho_int_data.gtow_info.num_srb)].rb_config =
          ESTABLISH_RB;
  ...
  if((number_of_rbs + rrciho_int_data.gtow_info.num_srb) > MAX_RB)
  {
    ...
    send_change_mode_req( wrrc_as_id,RRC_MODE_INACTIVE);
    rrciho_int_data.procedure_info.failure_cause = (uint32)RR_RRC_PHYSICAL_CHANNEL_FAILURE;
    rrciho_int_data.rrciho_state = RRCIHO_WAIT_INACTIVE_CNF;
  ...
  cmd_ptr->cmd.chan_config_req.num_rb_to_config =
    (int)(rrciho_int_data.gtow_info.num_srb + number_of_rbs);

#else
  for(i=0;i < rrciho_int_data.gtow_info.est_rabs.num_rbs_for_rab; i++)
  {
    /*set the rb's that need to be initialized */
    cmd_ptr->cmd.chan_config_req.rb[(i+rrciho_int_data.gtow_info.num_srb)].rb_id =
      (rrciho_int_data.gtow_info.est_rabs.rb_for_rab[i].rb_id);

    cmd_ptr->cmd.chan_config_req.rb[(i+rrciho_int_data.gtow_info.num_srb)].rb_config =
      ESTABLISH_RB;
  }

  /* Set the channel config cnf required to true */
  cmd_ptr->cmd.chan_config_req.num_rb_to_config =
    (int)(rrciho_int_data.gtow_info.num_srb + rrciho_int_data.gtow_info.est_rabs.num_rbs_for_rab);
#endif /* FEATURE_LTE_TO_WCDMA */
  ...
  rrciho_int_data.rrciho_state = RRCIHO_WAIT_CHAN_CONFIG_GTOW_CNF;
...
===========================================================================*/
static void rrciho_send_acq_req(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_cmd_type *cmd_ptr;
  ...
  cmd_ptr->cmd.rrc_csp_acq_req.freq = rrciho_int_data.gtow_info.freq;

  /* Fill in the scrambling code */
  cmd_ptr->cmd.rrc_csp_acq_req.scr_code = (uint16)(rrciho_int_data.gtow_info.scr_code * 16);

  /* Fill in the pn_offset information */
  cmd_ptr->cmd.rrc_csp_acq_req.pn_offset_present = rrciho_int_data.gtow_info.pn_offset_present;
  cmd_ptr->cmd.rrc_csp_acq_req.pn_pos = rrciho_int_data.gtow_info.pn_offset;

  cmd_ptr->cmd.rrc_csp_acq_req.network_select_mode = rrciho_int_data.gtow_info.network_select_mode;

  WRRC_MSG2_HIGH("PN_OFFSET_PRESENT %d, PN_OFFSET %d", rrciho_int_data.gtow_info.pn_offset_present, rrciho_int_data.gtow_info.pn_offset);
  /* Fill in the diversity information */
  cmd_ptr->cmd.rrc_csp_acq_req.sttd_indicator_present =
    rrciho_int_data.gtow_info.diversity_info_present;
  cmd_ptr->cmd.rrc_csp_acq_req.sttd_indicator =
    rrciho_int_data.gtow_info.diversity_info;
  ...
  /* Change the procedure state to wait for change mode confirmation */
  rrciho_int_data.rrciho_state = RRCIHO_WAIT_ACQ_CNF;
...
===========================================================================*/
static void rrciho_send_handover_to_utran_abort_cnf(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  rr_send_rrc_message_to_rr_internal(wrrc_as_id,(void *)(&(gtow_abort_cnf)), RRC_INTERRAT_HANDOVER_ABORT_CNF,
                            sizeof(rrc_interrat_handover_abort_cnf_type));

#ifdef WCDMA_RRC_INDICATE_CODEC_TO_CM
    /*Send  GtoW hand over failure   */
    rrc_send_hho_cmd_to_cm(wrrc_as_id,SYS_HO_FAIL,SYS_HO_TYPE_GSM_UMTS);
...
===========================================================================*/
static void rrciho_send_handover_to_utran_failure(sys_modem_as_id_e_type wrrc_as_id)
{
  /*declare var of struct handover confirm type*/
  ...
  /* Send the RR_INTERRAT_HANDOVER_REQ message to RR */
  rr_send_rrc_message_to_rr_internal(wrrc_as_id,(void *)(&(gtow_ho_cnf)), RRC_INTERRAT_HANDOVER_CNF,
  ...
  rrc_set_ordered_codec_mode(wrrc_as_id,rrc_get_current_codec_mode(wrrc_as_id));
  return;
}/*rrciho_send_handover_to_utran_failure*/
/*===========================================================================
...
===========================================================================*/
static void rrciho_send_handover_to_utran_complete(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  if (ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
    ...
    if((rrciho_int_data.gtow_info.gsm_ciph_status == TRUE)&&
      (rrcsmc_tm_ciph_active(wrrc_as_id) == TRUE))
    {
     RRC_MSG_COMMON_SET_BITMASK_IE(msg_ptr->message.u.handoverToUTRANComplete,
      ...
      rrciho_int_data.gtow_info.count_c_act_time = (wl1_seq_get_cfn(wrrc_as_id) + 250) & 0xF8;

      msg_ptr->message.u.handoverToUTRANComplete.count_C_ActivationTime =
        (uint8)rrciho_int_data.gtow_info.count_c_act_time;
    }
    ...
    (void)rrcsmc_append_start_list(wrrc_as_id,
      &msg_ptr->message.u.handoverToUTRANComplete.startList,
      RRCSMC_APPEND_START);
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
    rrcqsh_log_uplink_dcch_ota_params(wrrc_as_id, WRRC_UL_DCCH_MESSAGETYPE_HANDOVERTOUTRANCOMPLETE, msg_ptr);
#endif
   /* Send the HO to UTRAN on the Uplink DCCH
    */
    send_status = rrcsend_send_ul_sdu(wrrc_as_id,RRC_PROCEDURE_IHO,
                                        msg_ptr,
                                        ul_lc_id,
...
===========================================================================*/
static void rrciho_send_handover_to_utran_success(sys_modem_as_id_e_type wrrc_as_id)
{
  /*declare var of struct handover confirm type*/
  rrc_interrat_handover_cnf_type gtow_ho_cnf;
  ...
  rrc_send_hho_cmd_to_cm(wrrc_as_id,SYS_HO_COMPLETE,SYS_HO_TYPE_GSM_UMTS);
#endif
...
===========================================================================*/
static void rrciho_init_default_configuration(sys_modem_as_id_e_type wrrc_as_id)
{
...
#ifdef FEATURE_WCDMA_REL10
  default_config_info[RRC_DEF_CONFIG_24].default_config_len = default_config_24_len;
  default_config_info[RRC_DEF_CONFIG_24].default_config_ptr = default_config_24;
#endif
...
===========================================================================*/
static void rrciho_event_log_handover_to_utran(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  start_event_log.uarfcn = rrciho_int_data.gtow_info.freq;
  start_event_log.scr_code = rrciho_int_data.gtow_info.scr_code;

  WRRC_EVENT_REPORT_PAYLOAD(EVENT_GSM_TO_WCDMA_HANDOVER_START,
...
===========================================================================*/
static uecomdef_status_e_type rrciho_get_lock_for_iho(sys_modem_as_id_e_type wrrc_as_id,wrm_freq_input_type lock_req_bands,trm_reason_enum_t priority)
{
  uecomdef_status_e_type status = SUCCESS;
  rrc_out_of_lock_wait_e_type  lock_status;

  lock_status = rrc_acquire_lock_for_IHO_or_CCO(wrrc_as_id,RRCTMR_WRM_LOCK_WAIT_TIMER_FOR_INTERRAT_IN_MS, lock_req_bands,priority);
  ...
  if(lock_status == RRC_ABORT_FROM_RR)
  {
    ...
    if(iho_interrat_type == RRCIHO_INTERRAT_LTE)
    {
      rrciho_send_ltow_ps_handover_to_utran_abort_cnf(wrrc_as_id);
    }
    else
#endif
    {
      rrciho_send_handover_to_utran_abort_cnf(wrrc_as_id);
    }
    status = FAILURE;
  }
  else if(lock_status != RRC_WRM_LOCK_ACQUIRED)
  {
    ...
    if(iho_interrat_type == RRCIHO_INTERRAT_LTE)
    {
      rrciho_int_data.procedure_info.failure_cause = (uint32)LTOW_PSHO_INVALID_STATE;
      /*send a handover to utran failure and reset IHO procedure's state
        machine*/
      rrciho_send_ltow_ps_handover_to_utran_failure(wrrc_as_id);
    }
    else
#endif
    {
      rrciho_int_data.procedure_info.failure_cause = (uint32)RR_RRC_INVALID_STATE;
      /*send a handover to utran failure and reset IHO procedure's state
        machine*/
      rrciho_send_handover_to_utran_failure(wrrc_as_id);
    }
...
===========================================================================*/
static void process_handover_to_utran_command(sys_modem_as_id_e_type wrrc_as_id,
...
)
{
...
#endif
  rrcllc_amr_mapping_info_type amr_info = {0}; /* To store AMR mode info incase a voice RAB has been setup */
  /* Report select owner event to voice module before HO starts */
  rrc_send_select_owner_event(wrrc_as_id);
#ifdef FEATURE_LTE_TO_WCDMA
  ...
  rrciho_init_default_configuration(wrrc_as_id);
...
#ifdef FEATURE_RAT_PRIORITY_LIST
  rrc_csp_update_rat_pri_list_info(wrrc_as_id,&cmd_ptr->cmd.interrat_handover_req.rat_pri_list_info);
#endif
  ...
  rrcmeas_update_dedicated_priority_info_from_irat_req(wrrc_as_id,&cmd_ptr->cmd.interrat_handover_req.dedicated_priority_info,
  ...
  /*the HO message is the 1st byte from the beginning of the message.
    ...
    byte*/
    rrciho_int_data.gtow_info.ho_msg_len = cmd_ptr->cmd.interrat_handover_req.ho_cmd[0];

    rrciho_int_data.gtow_info.network_select_mode = cmd_ptr->cmd.interrat_handover_req.network_select_mode;

  /*get the ptr to the byte stream of the HandoverToUTRANCommand*/
    rrciho_int_data.gtow_info.ho_ptr =
      (rrc_HandoverToUTRANCommand *) rrcasn1_decode_pdu( wrrc_as_id,  (void *)ho_msg_ptr,
                                                          rrc_HandoverToUTRANCommand_PDU,
                                                          rrciho_int_data.gtow_info.ho_msg_len
                                                          );

     /* Save GtoW Utran message with type RRC_HO_TO_UTRAN  */
   rrc_crash_debug_save_ota(wrrc_as_id,ho_msg_ptr, rrciho_int_data.gtow_info.ho_msg_len,RRC_HO_TO_UTRAN,0xFF,RRC_PROCEDURE_IHO);

  if(rrciho_int_data.gtow_info.ho_ptr == NULL)
  {
  ...
  else
  {
    ...
    rrciho_int_data.gtow_info.ho_msg_present = TRUE;
  }

  /* Verify the handover from UTRAN command */
  if (verify_and_extract_handover_to_utran_command( wrrc_as_id,rrciho_int_data.gtow_info.ho_ptr
#ifdef FEATURE_MCC_AND_850_800_BAND_COMBINATION
      ...
      ) == FAILURE)
  {
    /* Send handover To UTRAN faliure to RR*/
    rrciho_send_handover_to_utran_failure(wrrc_as_id);
    rrc_free_dynamic_ptrs(wrrc_as_id);
  }
  else
  {
  ...
  rrc_perform_trm_exchange(wrrc_as_id);

  rrc_wrm_populate_bands_for_lock_req(wrrc_as_id,&lock_req_bands, rrciho_int_data.gtow_info.freq);
  ...
  if(rrciho_get_lock_for_iho(wrrc_as_id,lock_req_bands,TRM_TRAFFIC) == FAILURE)
  {
    return;
  }
#endif
...
#ifdef WCDMA_RRC_INDICATE_CODEC_TO_CM
    rrc_send_codec_info_to_cm( wrrc_as_id,rrc_get_ordered_codec_mode(wrrc_as_id),RRC_SAMPLING_RATE_NB);
#endif
    /*ho cmd is kosher.. so log the event that HO is starting*/
    rrciho_event_log_handover_to_utran(wrrc_as_id);

    /*process measurement data received in the handover to utran command*/
    process_meas_data_for_handover_to_utran_command(wrrc_as_id,&cmd_ptr->cmd.interrat_handover_req.interrat_measurement_data);

#ifdef FEATURE_WCDMA_FAST_RETURN_TO_LTE_AFTER_CSFB 
    WRRC_MSG1_HIGH("CSFB: CSFB Call status indicated by GRR %d ",cmd_ptr->cmd.interrat_handover_req.csfb_call_status);
    if((rrcnv_data.rrc_fast_return_to_lte_after_csfb_nv) && (cmd_ptr->cmd.interrat_handover_req.csfb_call_status))
    {
      rrc_csp_int_data.csfb_extended_status = CSFB_STARTED;
    ...
    send_change_mode_req(wrrc_as_id,RRC_MODE_INTERRAT_IN_PROGRESS);

    /* Change the procedure state to wait for change mode confirmation */
    rrciho_int_data.rrciho_state = RRCIHO_WAIT_IRAT_IN_PROGRESS_CNF;
  }
  ...
  if((cmd_ptr->cmd.interrat_handover_req.cipher_enabled == TRUE) &&
     (rrcsmc_ext_data.nv_ciphering_enabled))
  {
    rrciho_int_data.gtow_info.gsm_ciph_status = TRUE;
  }
  else
  {
    rrciho_int_data.gtow_info.gsm_ciph_status = FALSE;
  }

#ifdef FEATURE_WCDMA_TO_LTE
  if(rrcmcm_is_lte_mode_enabled(wrrc_as_id) &&
#ifdef LTE_BAND_NUM
     (rrccsp_check_if_lte_band_pref_is_set(wrrc_as_id,rrccsp_get_lte_band_pref(wrrc_as_id)) == TRUE))
#else
     (rrccsp_get_lte_band_pref(wrrc_as_id) != 0))
#endif
  {
    rrc_plmn_identity_type src_rat_plmn_id = mmumts_convert_nas_plmn_id_to_rrc_plmn_id(cmd_ptr->cmd.interrat_handover_req.plmn_id);
    rrcueci_send_eutra_capabilitites_req_to_lte(wrrc_as_id, rrcueci_is_truncated_ca_needed(wrrc_as_id, src_rat_plmn_id));
  }
#endif
...
)
{
  ...
  if(rrciho_int_data.procedure_info.act_time_present == TRUE)
  {
    *act_time = (uint16)(rrciho_int_data.procedure_info.activation_time);
    return SUCCESS;
  }
  else
  {
...
===========================================================================*/
static boolean rrciho_extract_single_gsm_message(sys_modem_as_id_e_type wrrc_as_id,
...
)
{
  ...
  index =rrc_get_used_ctxt_idx_for_asn1_ptr(wrrc_as_id,cmd_ptr->cmd.downlink_sdu_ind.decoded_msg);
  ...
  if(total_pad_bits == 0)
  {
    ...
    rrciho_int_data.procedure_info.failure_cause = T_rrc_InterRAT_HO_FailureCause_protocolError;
    rrciho_int_data.procedure_info.protocol_error = rrc_ProtocolErrorCause_informationElementMissing;
    return FALSE;
  }
  ...
  rrciho_int_data.procedure_info.parms.gsm.gsm_command_list.number_of_commands = 1;
  rrciho_int_data.procedure_info.parms.gsm.gsm_command_list.gsm_commands[0].message_length =
    (uint8)(total_pad_bits/8);

  /*total number of bytes*/
  b_copy( cmd_ptr->cmd.downlink_sdu_ind.dl_sdu,
          (uint16)(num_bits_in_msg),
          rrciho_int_data.procedure_info.parms.gsm.gsm_command_list.gsm_commands[0].payload,
          0,
...
===========================================================================*/
static boolean verify_and_extract_handover_from_utran_command_r6(sys_modem_as_id_e_type wrrc_as_id,
...
)
{
  ...
  rrc_qsh_log_ho_to_gsm_params(wrrc_as_id, &ho_gsm_params);
#endif
  rrciho_int_data.procedure_info.handover_type = T_rrc_DL_DCCH_MessageType_handoverFromUTRANCommand_GSM;
      ...
      rrc_HandoverFromUTRANCommand_GSM_r6_IEs,toHandoverRAB_Info)) 
  {
    ...
    while (rab_info_list->n > local_idx) 
    {
      if (rab_info_list->elem[local_idx].cn_DomainIdentity == rrc_CN_DomainIdentity_ps_domain) 
      {
        ...
        rrciho_int_data.procedure_info.transaction_id =  msg_ptr->u.later_than_r3.rrc_TransactionIdentifier;
        rrciho_int_data.procedure_info.failure_cause = T_rrc_InterRAT_HO_FailureCause_configurationUnacceptable;
        break;
      }
      local_idx++;
    }
  ...
  if(OC_NOT_SET != rrcllc_get_ordered_config_status(wrrc_as_id)
#ifdef FEATURE_WCDMA_ALLOW_LTA_DURING_ACT_TIME
     || rrcdata_check_if_ota_is_buffered(wrrc_as_id)
#endif  /*FEATURE_WCDMA_ALLOW_LTA_DURING_ACT_TIME*/
   )
  {
    rrciho_int_data.procedure_info.failure_cause = T_rrc_InterRAT_HO_FailureCause_unspecified;
    return (FALSE);
  }
  ...
  {
    rrciho_int_data.procedure_info.parms.gsm.frequency_band = RRC_RR_DCS_1800;
  }
  else
  {
    rrciho_int_data.procedure_info.parms.gsm.frequency_band = RRC_RR_PCS_1900;
  }
       ...
       == T_rrc_HandoverFromUTRANCommand_GSM_r6_IEs_gsm_message_single_GSM_Message)
  {
    ...
    if(!(rrciho_extract_single_gsm_message(wrrc_as_id,cmd_ptr)))
    {
      return (FALSE);
    }
  }
  else
  {
    ...
    if ( (gsm_cmd_ptr == NULL ) || (gsm_cmd_ptr->n ==0))
    {
      ...
      rrciho_int_data.procedure_info.failure_cause = T_rrc_InterRAT_HO_FailureCause_protocolError;
      rrciho_int_data.procedure_info.protocol_error = rrc_ProtocolErrorCause_ie_ValueNotComprehended;
    ...
    while ((gsm_cmd_ptr->n >  idx) && (gsm_cmd_ct < RRC_RR_MAX_GSM_COMMANDS ))
    {
      ...
      rrciho_int_data.procedure_info.parms.gsm.gsm_command_list.gsm_commands[gsm_cmd_ct-1].message_length =
      ...
      WCDMA_MEMCPY((rrciho_int_data.procedure_info.parms.gsm.gsm_command_list.gsm_commands[gsm_cmd_ct-1].payload),
                   sizeof(uint8) * RRC_RR_MAX_GSM_MESSAGE_LENGTH,
                   gsm_cmd_ptr->elem[idx].data,
                   MIN( rrciho_int_data.procedure_info.parms.gsm.gsm_command_list.gsm_commands[gsm_cmd_ct-1].message_length,
    ...
    rrciho_int_data.procedure_info.parms.gsm.gsm_command_list.number_of_commands = gsm_cmd_ct;
  }
...
===========================================================================*/
static boolean verify_and_extract_handover_from_utran_command(sys_modem_as_id_e_type wrrc_as_id,
  rrc_HandoverFromUTRANCommand_GSM *msg_ptr,
  rrc_cmd_type *cmd_ptr                      /* Received command */
)
{
  ...
  if (msg_ptr->t == T_rrc_HandoverFromUTRANCommand_GSM_later_than_r3)
  {
        ...
        T_rrc_HandoverFromUTRANCommand_GSM_criticalExtensions_1_r6) 
    {
      ...
      return verify_and_extract_handover_from_utran_command_r6(wrrc_as_id,msg_ptr, cmd_ptr);
    }
    else
    {
      /* Critical extension is not supported in this release, send handover from
         UTRAN failure to UTRAN */
      rrciho_int_data.procedure_info.transaction_id = 0; /* transaction id unknown */
      rrciho_int_data.procedure_info.failure_cause = T_rrc_InterRAT_HO_FailureCause_protocolError;
      rrciho_int_data.procedure_info.protocol_error = rrc_ProtocolErrorCause_messageExtensionNotComprehended;
  ...
  else
  {
    ...
    {
      /* Convert message length from the number of bits to number of bytes */
      rrciho_int_data.procedure_info.parms.gsm.gsm_command_list.gsm_commands[gsm_cmd_ct].message_length =
      ...
      /* Extract the GSM command */
      WCDMA_MEMCPY((rrciho_int_data.procedure_info.parms.gsm.gsm_command_list.gsm_commands[gsm_cmd_ct].payload),
                   sizeof(uint8) * RRC_RR_MAX_GSM_MESSAGE_LENGTH,
                   gsm_cmd_ptr->elem[gsm_cmd_ct].data,
                   MIN( rrciho_int_data.procedure_info.parms.gsm.gsm_command_list.gsm_commands[gsm_cmd_ct].message_length,
                            sizeof(uint8)*RRC_RR_MAX_SINGLE_GSM_MESSAGE_LENGTH ));
...
)
{
  ...
  */
  ...
  start_event_log.receive_cfn = wl1_seq_get_cfn(wrrc_as_id);

  WRRC_EVENT_REPORT_PAYLOAD(EVENT_WCDMA_INTER_RAT_HANDOVER_START,
                       sizeof(start_event_log),
                       (void *)&start_event_log);
...
)
{
  ...
  */
  ...
  WRRC_EVENT_REPORT_PAYLOAD(EVENT_WCDMA_INTER_RAT_HANDOVER_END,
...
===========================================================================*/
static void process_handover_from_utran_command_gsm(sys_modem_as_id_e_type wrrc_as_id,
  rrc_DL_DCCH_Message *dcch_ptr,    /* Pointer to the DCCH message */
  rrc_cmd_type *cmd_ptr             /* Received command */
)
{
  ...
  /* Verify the handover from UTRAN command */
  if (verify_and_extract_handover_from_utran_command(wrrc_as_id,msg_ptr, cmd_ptr) == FALSE)
  {
    /* Send handover from UTRAN faliure */
    send_handover_from_utran_failure(wrrc_as_id);
  }
  else
  {
    ...
    if (rrc_get_state(wrrc_as_id) != RRC_STATE_CELL_DCH)
    {
      ...
      rrciho_int_data.procedure_info.failure_cause = T_rrc_InterRAT_HO_FailureCause_protocolError;
      rrciho_int_data.procedure_info.protocol_error = rrc_ProtocolErrorCause_messageNotCompatibleWithReceiverState;
      send_handover_from_utran_failure(wrrc_as_id);
    ...
    if (rrcmcm_is_dualmode_enabled_no_f3(wrrc_as_id) == FALSE)
    {
    ...
    l1_get_interrat_measurement_data(&rrciho_int_data.procedure_info.parms.gsm.interrat_measurement_data,wrrc_as_id);

    /*Event logging for HO From UTRAN command*/
    rrciho_event_log_handover_from_utran(wrrc_as_id, T_rrc_DL_DCCH_MessageType_handoverFromUTRANCommand_GSM );
   ...
   if(WTOW_SEARCH_STOP != rrc_csp_bplmn_srch_in_progress(wrrc_as_id))
   {
    ...
    send_change_mode_req(wrrc_as_id,RRC_MODE_STANDBY);
    
    if(rrc_sib_is_current_event_sib_read_in_dch(wrrc_as_id))
    {
      WRRC_MSG0_HIGH("Force SIB Init in DCH ");
      rrcsib_force_sib_event_init(wrrc_as_id, RRCSIB_ABORT_DUE_SIB_READ_IN_DCH_NO_PCCPCH_DROP);
    }
#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
    /* Suspend Fake measurements before handling any other command */
    rrc_check_and_suspend_pseudo_meas_procedure(wrrc_as_id,RRC_PROCEDURE_IHO);
#endif
    /* Change the procedure state to wait for change mode confirmation */
    rrciho_int_data.rrciho_state = RRCIHO_WAIT_STANDBY_CNF;
    return;
  }
...
)
{
  ...
  if (interrat_ho_cnf->ho_status == LTE_RRC_IRAT_TO_LTE_PSHO_SUCCESS)
  {
    ...
    rrcdormancy_update_dormancy_for_srns_hho_cco(wrrc_as_id, RRC_PROCEDURE_IHO);
    rrciho_event_log_handover_from_utran_end_event_indicator(wrrc_as_id,T_rrc_DL_DCCH_MessageType_handoverFromUTRANCommand_EUTRA,SUCCESS,RRCIHO_EVENT_FAILURE_CAUSE_NOT_VALID);

    /* Procedure ends */
    rrciho_int_data.rrciho_state = RRCIHO_WAIT_STATE_CHANGE_IND;
  }
  else
  {
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
    wrrc_qsh_log_iho_stats(wrrc_as_id, IRAT_MOB_FAIL, WRRC_METRIC_UPDATE);
#endif
    /* Inter-RAT handover failed: Resume the WCDMA mode and revert back to
       UTRAN configuration. */
    send_change_mode_req(wrrc_as_id,RRC_MODE_ACTIVE);
    ...
    switch(interrat_ho_cnf->failure_cause)
    {
      ...
      case LTE_RRC_IRAT_TO_LTE_PSHO_FAILURE_CAUSE_PROTOCOL_ERROR:
        rrciho_int_data.procedure_info.failure_cause = T_rrc_InterRAT_HO_FailureCause_interRAT_ProtocolError;
        break;
      ...
      case LTE_RRC_IRAT_TO_LTE_PSHO_FAILURE_CAUSE_RACH_FAILED:
        rrciho_int_data.procedure_info.failure_cause = T_rrc_InterRAT_HO_FailureCause_physicalChannelFailure;
        break;
    ...
    rrciho_event_log_handover_from_utran_end_event_indicator(wrrc_as_id,T_rrc_DL_DCCH_MessageType_handoverFromUTRANCommand_EUTRA,FAILURE,rrciho_int_data.procedure_info.failure_cause);

    /* Set the procedure state to wait for change mode confirmation */
    rrciho_int_data.rrciho_state = RRCIHO_WAIT_ACTIVE_CNF;
  }
  return;
...
)
{
  ...
  rrciho_int_data.procedure_info.handover_type = T_rrc_DL_DCCH_MessageType_handoverFromUTRANCommand_EUTRA;
  ...
  /* Extract the transaction identifier from the command */
  rrciho_int_data.procedure_info.transaction_id = msg_ptr->rrc_TransactionIdentifier;
  ...
  rrciho_int_data.procedure_info.parms.eutra.dsm_ptr = NULL;
  num_bytes = dsm_pushdown (&rrciho_int_data.procedure_info.parms.eutra.dsm_ptr, 
      (void *)msg_ptr->criticalExtensions.u.r8.handoverFromUTRANCommand_EUTRA_r8.eutra_Message.data, 
      container_size, 
      RRC_DSM_POOL_ID(container_size));

  if((num_bytes != container_size) || (rrciho_int_data.procedure_info.parms.eutra.dsm_ptr == NULL))
  {
  ...
  rrciho_int_data.procedure_info.parms.eutra.eutra_container_size = (uint32)container_size;
  rrciho_int_data.procedure_info.parms.eutra.eutra_container_ptr = (byte *) rrc_malloc(container_size);
  WCDMA_MEMCPY(rrciho_int_data.procedure_info.parms.eutra.eutra_container_ptr,
               container_size,
...
)
{
  ...
  {
    wrrc_qsh_log_iho_stats(wrrc_as_id, LTE_RAT_CSFB_MOB, WRRC_METRIC_UPDATE);
  }
#endif
  
  
  /* Verify the handover from UTRAN command */
  if (verify_and_extract_handover_from_utran_command_eutra(wrrc_as_id,msg_ptr) == FALSE)
  {
    /* Send handover from UTRAN faliure */
    send_handover_from_utran_failure(wrrc_as_id);
  }
  else
  {
    ...
    /* Check if UE is dual-mode enabled */
    if (rrcmcm_is_lte_mode_enabled(wrrc_as_id) == FALSE)
    {
      /* UE is not enabled for dual-mode, send failure to UTRAN */
      rrciho_int_data.procedure_info.failure_cause = T_rrc_InterRAT_HO_FailureCause_configurationUnacceptable;
      send_handover_from_utran_failure(wrrc_as_id);
...
===========================================================================*/
static void process_interrat_handover_cnf(sys_modem_as_id_e_type wrrc_as_id,
...
)
{
  ...
  {
    rrcsmc_copy_pending_keys_to_saved_security_key_set(wrrc_as_id);
    /* Abort Dormancy if IHO proceed to successful processing. */
    rrcdormancy_update_dormancy_for_srns_hho_cco(wrrc_as_id, RRC_PROCEDURE_IHO);
    /* Interrat handover succeeded: inform DT to forward bufferred NAS signaling
       messages to the GSM RR */
    send_nas_data_transfer_ind(wrrc_as_id);
  ...
  else
  {
      ...
      wrrc_qsh_log_iho_stats(wrrc_as_id, IRAT_MOB_FAIL, WRRC_METRIC_UPDATE);
#endif
    rrc_configure_voice_adapter(wrrc_as_id,&amr_info,FALSE,FALSE,WCDMA_IVOICE_EVENT_SELECT_OWNER);
    /* Inter-RAT handover failed: Resume the WCDMA mode and revert back to
       UTRAN configuration. */
    send_change_mode_req(wrrc_as_id,RRC_MODE_ACTIVE);
    ...
    case RRC_RR_GSM_PROTOCOL_ERROR:
      ...
      {
        rrciho_int_data.gsm_commands.elem = rrc_malloc(number_of_commands* sizeof(rrc_GSM_MessageList_element));
          /* Extract and save original GSM commands */
        gsm_command_ptr = &rrciho_int_data.gsm_commands;
...
===========================================================================*/
static void process_handover_to_utran_command_from_eutran(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type * cmd_ptr
)
{
  ...
  memset(rrciho_int_data.gtow_info.psho_security_keys.ck, 0, 
         LTE_IRAT_MAX_CK_LENGTH);
  memset(rrciho_int_data.gtow_info.psho_security_keys.ik, 0, 
         LTE_IRAT_MAX_IK_LENGTH);  
  memset(rrciho_int_data.gtow_info.psho_security_keys.ck_srvcc, 0, 
         LTE_IRAT_MAX_CK_LENGTH);
  memset(rrciho_int_data.gtow_info.psho_security_keys.ik_srvcc, 0, 
         LTE_IRAT_MAX_IK_LENGTH);  
...
#ifdef FEATURE_RAT_PRIORITY_LIST
  rrc_csp_update_rat_pri_list_info(wrrc_as_id,&lte_psho_req->rat_pri_list_info);
#endif

  rrciho_int_data.gtow_info.network_select_mode = lte_psho_req->network_select_mode;
  ...
  rrciho_int_data.gtow_info.ho_msg_len = dsm_pkt_len;
...
/* Save GtoW Utran message with type RRC_HO_TO_UTRAN  */
  rrc_crash_debug_save_ota(wrrc_as_id,ho_msg_ptr, rrciho_int_data.gtow_info.ho_msg_len,RRC_HO_TO_UTRAN, 0xFF,RRC_PROCEDURE_IHO);
#ifdef FEATURE_LTE_TO_WCDMA
          rrciho_int_data.gtow_info.cs_ho_is_supported = lte_psho_req->cs_ho_is_supported;
          rrciho_int_data.gtow_info.ps_ho_is_supported = lte_psho_req->ps_ho_is_supported;
#endif
  ...
  else
  {
    ...
    rrc_perform_trm_exchange(wrrc_as_id);
    rrc_wrm_populate_bands_for_lock_req(wrrc_as_id,&lock_req_bands, rrciho_int_data.gtow_info.freq);
  /* Take lock with TRAFFIC priority for SRVCC and CSFB calls and ACCESS for PS HO */
   if((rrciho_int_data.gtow_info.is_cs_rab_present) || (rrc_csp_int_data.rrc_csfb_call_status))
   {
  ...
  /* Even after WCDMA has received the lock, call the below API to update WRRC client lock status 
     For non CSFB, acquire lock here */
  if(rrciho_get_lock_for_iho(wrrc_as_id,lock_req_bands,priority) == FAILURE)
  {
    return;
  }
#endif
    ...
    {
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
      wrrc_qsh_log_iho_stats(wrrc_as_id, LTE_RAT_SRVCC_MOB, WRRC_METRIC_UPDATE);
#endif
      /* Report select owner event to voice module before HO starts */
      rrc_send_select_owner_event(wrrc_as_id);
      rrc_send_hho_cmd_to_cm(wrrc_as_id,SYS_HO_START,SYS_HO_TYPE_LTE_UMTS_SRVCC);      
      rrc_send_codec_info_to_cm(wrrc_as_id, rrc_get_ordered_codec_mode(wrrc_as_id),RRC_SAMPLING_RATE_NB);
    }
#endif

    WCDMA_MEMCPY(rrciho_int_data.gtow_info.psho_security_keys.ck,
                 sizeof(uint8) * LTE_IRAT_MAX_CK_LENGTH,
                 lte_psho_req->security_keys.ck,
                 LTE_IRAT_MAX_CK_LENGTH );
    WCDMA_MEMCPY(rrciho_int_data.gtow_info.psho_security_keys.ik,
                 sizeof(uint8) * LTE_IRAT_MAX_IK_LENGTH,
                 lte_psho_req->security_keys.ik,
                 LTE_IRAT_MAX_IK_LENGTH );
    WCDMA_MEMCPY(rrciho_int_data.gtow_info.psho_security_keys.ck_srvcc,
                 sizeof(uint8) * LTE_IRAT_MAX_CK_LENGTH,
                 lte_psho_req->security_keys.ck_srvcc,
                 LTE_IRAT_MAX_CK_LENGTH );
    WCDMA_MEMCPY(rrciho_int_data.gtow_info.psho_security_keys.ik_srvcc,
                 sizeof(uint8) * LTE_IRAT_MAX_IK_LENGTH,
                 lte_psho_req->security_keys.ik_srvcc,
                 LTE_IRAT_MAX_IK_LENGTH );

    /*process measurement data received in the handover to utran command*/
    process_meas_data_for_handover_to_utran_command(wrrc_as_id, &lte_psho_req->interrat_measurement_data);
  ...
  if(rrciho_int_data.gtow_info.ciph_algo_present && rrcsmc_ext_data.nv_ciphering_enabled)
  {
    rrciho_int_data.gtow_info.gsm_ciph_status = TRUE;
  }
  else
...
#endif
  {
    rrc_plmn_identity_type src_rat_plmn_id = mmumts_convert_nas_plmn_id_to_rrc_plmn_id(lte_psho_req->plmn_id);
    rrcueci_send_eutra_capabilitites_req_to_lte(wrrc_as_id, rrcueci_is_truncated_ca_needed(wrrc_as_id, src_rat_plmn_id));
  }
#endif
...
===========================================================================*/
static void rrciho_initial_substate_event_handler(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type *cmd_ptr /* Received command */
)
{
  ...
  {
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
    if(rrc_qsh_events_db.gtow_ho.status == TRUE)
    {
      QSH_LOG(QSH_CLT_WRRC,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"Sub:%d GtoW HO start",(wrrc_as_id+1));
      rrc_qsh_report_event(wrrc_as_id, WRRC_QSH_EVENT_GTOW_HO_REQ);
    }
#endif
...
rrc_send_hho_cmd_to_cm(wrrc_as_id,SYS_HO_START,SYS_HO_TYPE_GSM_UMTS);
#endif
/*lint -e740*/
      /* Process the DCCH message: Handover From UTRAN Command */
      process_handover_to_utran_command(wrrc_as_id, (rrc_rr_cmd_type *)cmd_ptr);
/*lint +e740*/
  }
  else if( ((rrc_rr_cmd_e_type)cmd_ptr->cmd_hdr.cmd_id == RRC_INTERRAT_HANDOVER_ABORT_REQ) &&
           (rrcmcm_get_rrc_mode(wrrc_as_id) == RRC_MODE_INTERRAT_IN_PROGRESS))
  {
    ...
    rrciho_int_data.rrciho_abort = TRUE; /* Set abort to true */
    send_change_mode_req(wrrc_as_id, RRC_MODE_INACTIVE);
    rrciho_int_data.procedure_info.failure_cause = (uint32)RR_RRC_INVALID_STATE;
    rrciho_int_data.rrciho_state = RRCIHO_WAIT_INACTIVE_CNF;
  }
  else
  {
    ...
    case RRC_DOWNLINK_SDU_IND:
      ...
      {
        rrciho_int_data.procedure_info.failure_cause = T_rrc_InterRAT_HO_FailureCause_protocolError;
        rrciho_int_data.procedure_info.protocol_error = rrc_ProtocolErrorCause_asn1_ViolationOrEncodingError;
        send_handover_from_utran_failure(wrrc_as_id);
        WRRC_MSG0_HIGH("Handover to UTRAN failed because of ASN1 decoding error ");
        return;
      }
      else if (rrccu_get_cell_update_started_status_with_cause(wrrc_as_id,&cu_cause) == RRCCU_STARTED) 
      {
      ...
      else if(rrcrce_is_abort_in_progress(wrrc_as_id))
      {
      ...
      */
      switch ( dcch_msg_ptr->message.t )
      {
        case T_rrc_DL_DCCH_MessageType_handoverFromUTRANCommand_GSM:
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
          wrrc_qsh_log_iho_stats(wrrc_as_id, GSM_RAT_MOB, WRRC_METRIC_UPDATE);
#endif
          process_handover_from_utran_command_gsm(wrrc_as_id,dcch_msg_ptr, cmd_ptr);
          break;

        #ifdef FEATURE_WCDMA_TO_LTE
        case T_rrc_DL_DCCH_MessageType_handoverFromUTRANCommand_EUTRA:
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
          wrrc_qsh_log_iho_stats(wrrc_as_id, LTE_RAT_MOB, WRRC_METRIC_UPDATE);
#endif
          rrciho_process_handover_from_utran_command_eutra(wrrc_as_id,dcch_msg_ptr, cmd_ptr);
          break;
        #endif
    ...
    case RRC_STATE_CHANGE_IND:
      ...
      rrc_configure_voice_adapter(wrrc_as_id,&amr_info,FALSE,FALSE,WCDMA_IVOICE_EVENT_REQUEST_STOP);
      break;

    case RRC_ABORT_REQ:
      /* IHO is not active, forward the command to RCE for processing */
      rrcrce_procedure_event_handler(wrrc_as_id,  cmd_ptr );
      break;
...
/*lint -e408*/
    ...
    case RRC_LTE_RRC_CMD:
      ...
      {
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
        if(rrc_qsh_events_db.ltow_ho.status == TRUE)
        { 	
          QSH_LOG(QSH_CLT_WRRC,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"Sub:%d LtoW HO start",(wrrc_as_id+1));
          rrc_qsh_report_event(wrrc_as_id, WRRC_QSH_EVENT_LTOW_HO_REQ);
        }
#endif
        process_handover_to_utran_command_from_eutran(wrrc_as_id,cmd_ptr);
      }
    ...
    default:
      /* umexpected enevts */
      WRRC_MSG2_ERROR("Unexpected IHO CMD %x in substate %d", cmd_ptr->cmd_hdr.cmd_id, rrciho_int_data.rrciho_state);
      break;
    }
  }
  return;
...
===========================================================================*/
static void rrciho_wait_standby_cnf_substate_event_handler(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type *cmd_ptr /* Received command */
)
{
  rrcllc_amr_mapping_info_type amr_info = {0}; /* To store AMR mode info incase a voice RAB has been setup */
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    ...
    case RRC_CHANGE_MODE_CNF:
      ...
      if (rrciho_int_data.rrciho_abort == TRUE)
      {
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
         wrrc_qsh_log_iho_stats(wrrc_as_id, IRAT_MOB_FAIL, WRRC_METRIC_UPDATE);
#endif
      ...
      else
      {
        ...
        */
        ...
        send_interrat_handover_req(wrrc_as_id);

        /* Change the procedure state to wait for handover confirmation */
        rrciho_int_data.rrciho_state = RRCIHO_WAIT_INTERRAT_HO_CNF;
      }
      break;
    ...
    case RRC_STATE_CHANGE_IND:
      ...
      rrc_configure_voice_adapter(wrrc_as_id,&amr_info,FALSE,FALSE,WCDMA_IVOICE_EVENT_REQUEST_STOP);
      WRRC_MSG1_HIGH("RRC State Change to IDLE in %d", rrciho_int_data.rrciho_state);
      rrciho_init_data(wrrc_as_id);
    ...
    case RRC_ABORT_REQ:
      /* Set abort to true */
      rrciho_int_data.rrciho_abort = TRUE;
      if(rrciho_int_data.rrc_abort_req_ptr != NULL)
      {
        rrc_free(rrciho_int_data.rrc_abort_req_ptr);
      }
      rrciho_int_data.rrc_abort_req_ptr = (rrc_abort_req_type *)rrc_malloc(sizeof(rrc_abort_req_type));
      WCDMA_MEMCPY(rrciho_int_data.rrc_abort_req_ptr,
    ...
    default:
      ...
      WRRC_MSG2_ERROR("Unexpected event %x in state %d",
           cmd_ptr->cmd_hdr.cmd_id, rrciho_int_data.rrciho_state);
      break;
  }
  return;
}
...
===========================================================================*/
static void rrciho_wait_interrat_ho_cnf_substate_event_handler(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type *cmd_ptr  /* Received command */
)
{
  rrcllc_amr_mapping_info_type amr_info = {0}; /* To store AMR mode info incase a voice RAB has been setup */ 
...
/*lint -e740*/
      ...
      process_interrat_handover_cnf(wrrc_as_id,&(((rrc_rr_cmd_type *)cmd_ptr)->cmd.interrat_handover_cnf));
/*lint +e740*/
  }
  else
  {
   switch( cmd_ptr->cmd_hdr.cmd_id )
   {
    ...
    case RRC_LTE_RRC_CMD:
      if(cmd_ptr->cmd.wcdma_rrc_lte_irat_cmd.cmd.msg_hdr.id == LTE_RRC_WCDMA_PSHO_RSP)
      {
        ...
        rrciho_process_wtol_ps_handover_cnf(wrrc_as_id,&(cmd_ptr->cmd.wcdma_rrc_lte_irat_cmd.cmd.lte_rrc_wcdma_psho_rsp));
      }
      break;
#endif
    ...
    case RRC_ABORT_REQ:
      ...
      */
      ...
      {
        case T_rrc_DL_DCCH_MessageType_handoverFromUTRANCommand_GSM:
          send_abort_handover_req(wrrc_as_id);
          rrciho_int_data.rrciho_state = RRCIHO_WAIT_ABORT_CNF;
          break;

#ifdef FEATURE_WCDMA_TO_LTE
        case T_rrc_DL_DCCH_MessageType_handoverFromUTRANCommand_EUTRA:
          rrciho_send_abort_handover_to_lte(wrrc_as_id);
          rrciho_int_data.rrciho_state = RRCIHO_WAIT_ABORT_CNF;
          break;
#endif
...
===========================================================================*/
static void rrciho_wait_active_cnf_substate_event_handler(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type *cmd_ptr  /* Received command */
)
{
    ...
    case RRC_CHANGE_MODE_CNF:
      ...
      if( cmd_ptr->cmd.change_mode_cnf.status == TRUE )
      {
        ...
        if(TRUE == rrc_is_voice_rab_present(wrrc_as_id))
        {
          ...
          rrc_indicate_layer1_next_config_is_for_voice(wrrc_as_id);
        }
        /* Send Channel Config Request to LLC to revert back to UTRAN configuration */
        send_channel_config_req_to_llc(wrrc_as_id);

        /* Set procedure state to wait for Channel Config Confirmation */
        rrciho_int_data.rrciho_state = RRCIHO_WAIT_CHANNEL_CONFIG_CNF;
      }
      else /* change mode failure */
      {
        ...
        if (rrciho_int_data.rrciho_abort == TRUE)
        {
          ...
          WCDMA_MEMCPY(&abort_cmd_local_ptr->cmd.rrc_abort_req,
                       sizeof(rrc_abort_req_type),
                       rrciho_int_data.rrc_abort_req_ptr,
                       sizeof(rrc_abort_req_type));
          hard_abort = rrciho_int_data.rrc_abort_req_ptr->hard_abort;
          rrcrce_procedure_event_handler(wrrc_as_id,abort_cmd_local_ptr);
        ...
        if ( (int_cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id)) != NULL)
        {
          ...
          rrciho_int_data.rrciho_state = RRCIHO_WAIT_CELL_UPDATE_CNF;
        }
      }
      break;
...
===========================================================================*/
static void rrciho_wait_abort_cnf_substate_event_handler(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type *cmd_ptr  /* Received command */
)
{
  rrcllc_amr_mapping_info_type amr_info = {0}; /* To store AMR mode info incase a voice RAB has been setup */ 
...
===========================================================================*/
static void rrciho_wait_channel_config_cnf_substate_event_handler(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type *cmd_ptr  /* Received command */
)
{
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    ...
    case RRC_CHANNEL_CONFIG_CNF:
      ...
      rrcllcpcie_initialise_sync_a_post_veri_info(wrrc_as_id);
...
But same way, I'm also configuring MVS for CSoHSPA and sending PDCP_CSOHS_HANDOVER_END to PDCP.*/
      if( rrc_is_voice_rab_present(wrrc_as_id) )
      {
          rrc_configure_voice_adapter(wrrc_as_id,&amr_info,FALSE, FALSE,WCDMA_IVOICE_EVENT_SELECT_OWNER);
          /* Set the amr parameters */
          rrciho_process_channel_config_cnf(wrrc_as_id,rrcdata_codec_info.rrc_active_codec);
        }
      }
      ...
      if (cmd_ptr->cmd.chan_config_cnf.chan_cfg_status == RRCLLC_CHAN_CFG_SUCCESS)
      {
        ...
        rrcllc_copy_ordered_hanging_rb_mapping_to_current(wrrc_as_id);
        ...
        rrccu_post_buffered_commands(wrrc_as_id);
    ...
    default:
      /* Unexpected command */
      WRRC_MSG2_ERROR("Unexpected CMD %x in state %d", cmd_ptr->cmd_hdr.cmd_id, rrciho_int_data.rrciho_state);
      break;
  }
  return;
...
===========================================================================*/
static void rrciho_wait_acq_cnf_event_handler(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type *cmd_ptr  /* Received command */
)
{
  /* Switch on the received command. */
  ...
  else
  {
    ...
    case RRC_CSP_ACQ_CNF:
      ...
      else
      {
        ...
        if(rrciho_int_data.gtow_info.is_cs_rab_present)
          {
            ...
            rrcrb_update_mac_rab_status(wrrc_as_id,RRC_PROCEDURE_IHO, RRC_CS_DOMAIN_CN_ID,
                                        NULL, FALSE);
          }

        /* In case of CS+PS handover, update MAC RAB status again for PS RAB */
        if(rrciho_int_data.gtow_info.is_ps_rab_present)
        {
          /* Update mac_rab_Status based on the type of handover */
          rrcrb_update_mac_rab_status(wrrc_as_id,RRC_PROCEDURE_IHO, RRC_PS_DOMAIN_CN_ID,
                                      NULL, FALSE);
        }
#else 
        ...
        if(RRCRB_CS_VOICE_RAB == rrciho_int_data.gtow_info.est_rabs.rab_type ||
           RRCRB_CS_DATA_RAB == rrciho_int_data.gtow_info.est_rabs.rab_type )
        {
          /* Update mac_rab_Status blindly to CS_RABS since IHO can set up only CS rabs  */
          rrcrb_update_mac_rab_status(wrrc_as_id,RRC_PROCEDURE_IHO, RRC_CS_DOMAIN_CN_ID,
                                      NULL, FALSE);
        ...
        /*send channel config req to LLC to go to DCH state, if OC set
          ...
          the Handover */
        if(rrciho_send_channel_config_for_handover_to_utran(wrrc_as_id) != SUCCESS)
        {
          WRRC_MSG0_HIGH("Ordered Config Set failed");
          send_change_mode_req(wrrc_as_id,RRC_MODE_INACTIVE);
          rrciho_int_data.procedure_info.failure_cause = (uint32)RR_RRC_PHYSICAL_CHANNEL_FAILURE;
...
===========================================================================*/
static void rrciho_wait_inactive_cnf_event_handler(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type *cmd_ptr  /* Received command */
)
{
  /* Switch on the received command. */
...
===========================================================================*/
static void rrciho_wait_irat_in_progress_cnf_event_handler(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type *cmd_ptr  /* Received command */
)
{
  /* Switch on the received command. */
  ...
  else
  {
    ...
    case RRC_CHANGE_MODE_CNF:
      ...
      else
      {
        ...
        rrciho_send_acq_req(wrrc_as_id);
  ...
  else
  {
    ...
    case RRC_HO_CIPH_CNF:
      ...
      else
      {
        ...
        if(iho_interrat_type == RRCIHO_INTERRAT_LTE)
        {
          rrciho_send_ltow_ps_handover_to_utran_success(wrrc_as_id);
          ...
          if(rrciho_int_data.gtow_info.is_cs_rab_present && rrciho_int_data.gtow_info.is_ps_rab_present)
          {
            int_cmd_ptr->cmd.rrc_open_domain_ind.domain = RRC_CS_PLUS_PS_DOMAIN;
          }
          else if(rrciho_int_data.gtow_info.is_cs_rab_present)
          {
            int_cmd_ptr->cmd.rrc_open_domain_ind.domain = RRC_CS_DOMAIN;
          }
          else if(rrciho_int_data.gtow_info.is_ps_rab_present)
          {
...
#endif
        {
          rrciho_send_handover_to_utran_success(wrrc_as_id);
        }

        /*set Connection Setup status to true*/
        rrcrce_change_substate_to_rrcrce_final(wrrc_as_id);
...
FUNCTION  rrciho_wait_state_change_event_handler

DESCRIPTION

  This function handles the events received by this module when
  the procedure is Waiting for State change indication.
...
===========================================================================*/
static void rrciho_wait_state_change_event_handler
(
  sys_modem_as_id_e_type wrrc_as_id, 
  rrc_cmd_type *cmd_ptr  /* Received command */
)
...
)
{
  ...
  WRRC_EVENT_REPORT_PAYLOAD(EVENT_WCDMA_PHYCHAN_CFG_CHANGED,
...
===========================================================================*/
static void rrciho_wait_chan_config_gtow_cnf_event_handler(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type *cmd_ptr  /* Received command */
)
{
  ...
  else
  {
    ...
    case RRC_CHANNEL_CONFIG_CNF:
          ...
          ( cmd_ptr->cmd.chan_config_cnf.chan_cfg_status != RRCLLC_CHAN_CFG_SUCCESS) )
      {
...
#ifndef FEATURE_LTE_TO_WCDMA
        rrciho_int_data.gtow_info.est_rabs.nas_Synchronisation_Indicator= rrc_get_current_codec_mode(wrrc_as_id);
        rrc_set_ordered_codec_mode(wrrc_as_id,rrciho_int_data.gtow_info.est_rabs.nas_Synchronisation_Indicator);
#else
        if((rrciho_int_data.gtow_info.is_cs_rab_present) &&
           (RRCRB_CS_VOICE_RAB == rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].rab_type))
        {
          rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].nas_Synchronisation_Indicator= rrc_get_current_codec_mode(wrrc_as_id);
          rrc_set_ordered_codec_mode(wrrc_as_id,rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].nas_Synchronisation_Indicator);
        }
#endif /* FEATURE_LTE_TO_WCDMA */
        send_change_mode_req(wrrc_as_id,RRC_MODE_INACTIVE);
        ...
        {
          rrciho_int_data.procedure_info.failure_cause = (uint32)RR_RRC_PHYSICAL_CHANNEL_FAILURE;
          rrciho_event_phychan_cfg_status_handover_to_utran(wrrc_as_id, FALSE);
        }
      }
      else
      {
        rrc_cell_params_int_data.prev_cell_id = WCDMA_INVALID_CELL_ID;
        cell_id_dch_info.psc =0;
        cell_id_dch_info.uarfcn_dl = 0;
        /*Notify the Cell_Id to registered entities*/
        rrc_notify_cell_id_in_dch(wrrc_as_id);

        /* Copy the Ordered RB Mapping into the current RB-Mapping database*/
        rrcllc_copy_ordered_hanging_rb_mapping_to_current(wrrc_as_id);

        /*log an event in phy chan config is SUCCESSFUL*/
        rrciho_event_phychan_cfg_status_handover_to_utran(wrrc_as_id, TRUE);

        if(TRUE == rrciho_int_data.gtow_info.rab_info_included)
        {
#ifdef FEATURE_LTE_TO_WCDMA
          for(j = 0; j < rrciho_int_data.gtow_info.number_of_est_rabs; j++)
          {
            /*add code here to update established rabs*/
            rrc_add_rab_to_established_rabs(wrrc_as_id,&(rrciho_int_data.gtow_info.est_rabs[j]));
          }
          if(rrciho_int_data.gtow_info.is_cs_rab_present)
          {
              /*send activation command to vocoder*/
            rrciho_process_channel_config_cnf(wrrc_as_id,rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].nas_Synchronisation_Indicator);
          }
#else
          /*add code here to update established rabs*/
          rrc_add_rab_to_established_rabs(wrrc_as_id,&(rrciho_int_data.gtow_info.est_rabs));

          if(RRCRB_CS_VOICE_RAB == rrciho_int_data.gtow_info.est_rabs.rab_type)
          {
              /*send activation command to vocoder*/
            rrciho_process_channel_config_cnf(wrrc_as_id,rrciho_int_data.gtow_info.est_rabs.nas_Synchronisation_Indicator);
          }
#endif /* FEATURE_LTE_TO_WCDMA */
        }
        ...
        rrcllc_clear_ordered_config(wrrc_as_id);
        ...
        WRRC_MSG2_HIGH("GSM Ciph Status: %d, Ciph Algo IE Present: %d",
           rrciho_int_data.gtow_info.gsm_ciph_status,
           rrciho_int_data.gtow_info.ciph_algo_present);
          ...
          (rrciho_int_data.gtow_info.ciph_algo_present == FALSE ))
        {
          ...
          if(iho_interrat_type == RRCIHO_INTERRAT_LTE)
          {
            ...
            if(rrciho_int_data.gtow_info.is_ps_rab_present)
            {
              ...
              WCDMA_MEMCPY(ho_ciph_req.security_keys[RRC_PS_DOMAIN_CN_ID].ciphering_key,
                           ...
                           rrciho_int_data.gtow_info.psho_security_keys.ck,
                           LTE_IRAT_MAX_CK_LENGTH );
              WCDMA_MEMCPY(ho_ciph_req.security_keys[RRC_PS_DOMAIN_CN_ID].integrity_key,
                           sizeof(byte) * RRC_MAX_INTEGRITY_KEY_LENGTH,
                           rrciho_int_data.gtow_info.psho_security_keys.ik,
                           LTE_IRAT_MAX_IK_LENGTH );

              rrcsmc_save_security_key_set_from_lte(wrrc_as_id,ho_ciph_req, RRC_PS_DOMAIN_CN_ID);
            }
            ...
            if(rrciho_int_data.gtow_info.is_cs_rab_present)
            {
              WCDMA_MEMCPY(ho_ciph_req.security_keys[RRC_CS_DOMAIN_CN_ID].ciphering_key,
                           sizeof(byte) * RRC_MAX_CIPHERING_KEY_LENGTH,
                           rrciho_int_data.gtow_info.psho_security_keys.ck_srvcc,
                           LTE_IRAT_MAX_CK_LENGTH );
              WCDMA_MEMCPY(ho_ciph_req.security_keys[RRC_CS_DOMAIN_CN_ID].integrity_key,
                           sizeof(byte) * RRC_MAX_INTEGRITY_KEY_LENGTH,
                           rrciho_int_data.gtow_info.psho_security_keys.ik_srvcc,
                           LTE_IRAT_MAX_IK_LENGTH );

              rrcsmc_save_security_key_set_from_lte(wrrc_as_id,ho_ciph_req, RRC_CS_DOMAIN_CN_ID);
            }
          }
          else
#endif
           ...
           */
          rrcsmc_check_and_update_security_key_set_gen_in_other_rat(wrrc_as_id, RRC_CS_DOMAIN_CN_ID);
        ...
        else
        {
          ...
          int_cmd_ptr->cmd.ho_ciph_req.ciph_algo = rrciho_int_data.gtow_info.cipheringAlgorithm;
#ifndef FEATURE_LTE_TO_WCDMA
...
#else
         if((iho_interrat_type == RRCIHO_INTERRAT_LTE) && 
              ((rrciho_int_data.gtow_info.is_ps_rab_present)&&(rrciho_int_data.gtow_info.is_cs_rab_present)))
          {
            ...
            WCDMA_MEMCPY(int_cmd_ptr->cmd.ho_ciph_req.security_keys[RRC_CS_DOMAIN_CN_ID].integrity_key,
              ...
              (rrciho_int_data.gtow_info.is_ps_rab_present))
          {
            ...
            WCDMA_MEMCPY(int_cmd_ptr->cmd.ho_ciph_req.security_keys[RRC_PS_DOMAIN_CN_ID].integrity_key,
              ...
              (rrciho_int_data.gtow_info.is_cs_rab_present))
          {
            int_cmd_ptr->cmd.ho_ciph_req.domain_id = RRC_CS_DOMAIN_CN_ID;
          ...
          rrciho_int_data.rrciho_state = RRCIHO_WAIT_CIPH_CONFIG_GTOW_CNF;
        }
      }
...
===========================================================================*/
static void rrciho_wait_cell_update_cnf_substate_event_handler(sys_modem_as_id_e_type wrrc_as_id,rrc_cmd_type *cmd_ptr)
{
  rrcllc_amr_mapping_info_type amr_info = {0}; /* To store AMR mode info incase a voice RAB has been setup */
...
===========================================================================*/
static boolean rrccsp_is_wcdma_band_enabled(sys_modem_as_id_e_type wrrc_as_id)
{
...
)
{
  ...
  rrcsmc_append_start_per_domain(wrrc_as_id,(byte *) (ptr->uE_SecurityInformation.u.present.
                                           start_CS.data),
                                 RRC_CS_DOMAIN_CN_ID,
                                 RRCSMC_APPEND_START);
  ...
  if(!use_compressed_irat_ho_info)
  {
    ...
    (void)rrcueci_append_ue_radio_access_capability(wrrc_as_id,&(cap_ptr->ue_RadioAccessCapability),
    ...
    if ((error_stat= rrc_enc_UE_RadioAccessCapabilityInfo_PDU(wrrc_as_id,cap_ptr,&ptr->ue_CapabilityContainer.u.present))
        !=0)
        {
        WRRC_ERR2_FATAL("Error in ASN1 encoding %d  error %d",
 ...
 RRC_MSG_COMMON_SET_BITMASK_IE_TYPE2(ptr->v390NonCriticalExtensions.u.present.interRATHandoverInfo_v390ext,
    ...
    )
  {
   ...
   RRC_RESET_MSG_IE_PRESENT(ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.
    ...
    if((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5) && (!use_compressed_irat_ho_info) && (rrcueci_ext_data.rrc_ue_pos_cap_pch))
    {
  ...
  if(rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5)
  {
    ...
    if(rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL10)
    {
      ...
      ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
      v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.interRATHandoverInfo_v4b0ext.accessStratumReleaseIndicator = rrc_AccessStratumReleaseIndicator_rel_10;
      WRRC_MSG0_HIGH("AccessStratumIndicator indicator set to Rel-10 in Handover Classmark info");
    }
    else
#endif /* FEATURE_WCDMA_REL10 */
    ...
    if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL6)
    {
        ...
        v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
        v690NonCriticalExtensions.interRATHandoverInfo_v690ext.ue_RadioAccessCapabilityComp2
        .fddPhysicalChannelCapab_hspdsch_edch.physicalChannelCapabComp_hspdsch_r6 = rrcnv_data.nv_hsdpa_category;
        ...
        .fddPhysicalChannelCapab_hspdsch_edch.physicalChannelCapability_edch_r6.fdd_edch.u.supported->edch_PhysicalLayerCategory =
         MIN(rrcnv_data.wcdma_nv_hsupa_category,RRC_WCMDA_HSUPA_DEFAULT_CATEGORY);
      ...
      if(l2w_psho_enabled)
      {
        ...
        rrcsmc_append_start_per_domain(wrrc_as_id,(byte *) (start_ps->data),
                                         RRC_PS_DOMAIN_CN_ID,
                                         RRCSMC_APPEND_START);
      }
#endif /* FEATURE_LTE_TO_WCDMA */
    ...
    if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL7)
    {
      ...
      if(!(rrcnv_data.rrc_disable_cpc_fdpch_nv & NV_FDPCH_DISABLE))
      {
        RRC_MSG_COMMON_SET_BITMASK_IE(ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
          v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
          v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.interRATHandoverInfo_v6e0ext,
      ...
      if(rrcnv_data.nv_hsdpa_category > 8 )
      { 
         RRC_MSG_COMMON_SET_BITMASK_IE(ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
         v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
         v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
         ...
         v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
         interRATHandoverInfo_v770ext.ue_RadioAccessCapabilityComp2.hsdsch_physical_layer_category_ext =
               rrcnv_data.nv_hsdpa_category;
          
      }
      else
      {
          WRRC_MSG1_HIGH("RRCHSPA+: 64 QAM feature dfiened but NV returns %d , should be 13 or 14",rrcnv_data.nv_hsdpa_category);
      }

      if(!(rrcnv_data.rrc_disable_cpc_fdpch_nv & (NV_EFDPCH_DISABLE|NV_FDPCH_DISABLE)))
      {
        RRC_MSG_COMMON_SET_BITMASK_IE(ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
             v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
             v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
      ...
      if(rrcueci_nv_supports_cpc_dtx_on_dpcch(wrrc_as_id) == TRUE)
      {
...
RRC_MSG_COMMON_SET_BITMASK_IE(ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
           ...
           v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
           interRATHandoverInfo_v770ext.ue_RadioAccessCapabilityComp2,
           rrc_UE_RadioAccessCapabilityComp2_v770ext,slotFormat4);
      
        ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
           v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
           v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
           interRATHandoverInfo_v770ext.ue_RadioAccessCapabilityComp2.slotFormat4 =
           rrc_UE_RadioAccessCapabilityComp2_v770ext_slotFormat4_true;
      }

#ifdef FEATURE_WCDMA_HS_FACH
      if(rrcenhstate_nv_supports_e_fach(wrrc_as_id) == TRUE)
      {
        ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
             v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
             v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
    ...
    if (TRUE == rrcueci_nv_supports_edpcch_pwr_boost(wrrc_as_id))
    {
    ...
    if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL8)
    {
      ...
      if(!RRC_CHECK_MSG_TYPE_IE (ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
         v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
         v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
         v790nonCriticalExtensions.v860NonCriticalExtensions.v880NonCriticalExtensions.interRATHandoverInfo_v880ext, ue_RadioAccessCapabilityInfoPresent))
      {
         RRC_MSG_COMMON_SET_BITMASK_IE_TYPE2(ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
         v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
         v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
         v790nonCriticalExtensions.v860NonCriticalExtensions.v880NonCriticalExtensions.interRATHandoverInfo_v880ext, ue_RadioAccessCapabilityInfo);
         ...
         v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
         v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
         v790nonCriticalExtensions.v860NonCriticalExtensions.v880NonCriticalExtensions.interRATHandoverInfo_v880ext.ue_RadioAccessCapabilityInfo, supportForPriorityReselectionInUTRAN);
      ...
      if((rrcueci_nv_supports_dc_hsdpa(wrrc_as_id) == TRUE) && (!rrccsp_check_if_em_plmn(wrrc_as_id)))
      {
        RRC_MSG_COMMON_SET_BITMASK_IE_TYPE2(ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
          v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
          v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
          v790nonCriticalExtensions.v860NonCriticalExtensions.interRATHandoverInfo_v860ext.ue_RadioAccessCapabilityComp2, hsdsch_physical_layer_category_ext2);
           ...
           ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
            ...
            v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
            v790nonCriticalExtensions.v860NonCriticalExtensions.interRATHandoverInfo_v860ext.ue_RadioAccessCapabilityComp2.
            hsdsch_physical_layer_category_ext2 = MIN(rrcnv_data.nv_hsdpa_category, RRC_DEFAULT_DC_HSDPA_CATEGORY);
      }
      else
      ...
      if(rrcmcm_is_lte_mode_enabled(wrrc_as_id))
      {
        ...
        if(rrcueci_append_ue_radio_access_capability_fdd3_list(wrrc_as_id,&local_v860ext.ue_RadioAccessCapabBandFDDList3,lte_band_pref))
        {
          RRC_MSG_COMMON_SET_BITMASK_IE_TYPE2(ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
          v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
          v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
...
#endif
#ifdef FEATURE_MAC_I
      if(rrcueci_nv_supports_mac_i(wrrc_as_id) == TRUE)
      {
    ...
    if (rrcueci_nv_supports_dc_hsupa(wrrc_as_id) == TRUE)
    {
...
#ifdef FEATURE_WCDMA_REL10  
#ifdef FEATURE_WCDMA_3C_HSDPA
       if((rrcueci_nv_supports_3c_hsdpa(wrrc_as_id) == TRUE) && (!rrccsp_check_if_em_plmn(wrrc_as_id)))
       {
         if(!RRC_CHECK_MSG_TYPE_IE (ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
                   v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
                   v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
         ...
         if(!RRC_CHECK_MSG_TYPE_IE (ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
                   v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
                   v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
                   v790nonCriticalExtensions.v860NonCriticalExtensions.v880NonCriticalExtensions.v920NonCriticalExtensions, v8b0NonCriticalExtensionsPresent))
         {
           RRC_MSG_COMMON_SET_BITMASK_IE_TYPE2(ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
                   v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
                   v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
                   v790nonCriticalExtensions.v860NonCriticalExtensions.v880NonCriticalExtensions.v920NonCriticalExtensions, v8b0NonCriticalExtensions);
         }
          if(!RRC_CHECK_MSG_TYPE_IE (ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
                   v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
                   v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
                   v790nonCriticalExtensions.v860NonCriticalExtensions.v880NonCriticalExtensions.v920NonCriticalExtensions.v8b0NonCriticalExtensions, v950NonCriticalExtensionsPresent))
         {
           RRC_MSG_COMMON_SET_BITMASK_IE_TYPE2(ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
                   v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
                   v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
                   v790nonCriticalExtensions.v860NonCriticalExtensions.v880NonCriticalExtensions.v920NonCriticalExtensions.v8b0NonCriticalExtensions, v950NonCriticalExtensions);
         }
         if(!RRC_CHECK_MSG_TYPE_IE (ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
                   v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
                   v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
                   v790nonCriticalExtensions.v860NonCriticalExtensions.v880NonCriticalExtensions.v920NonCriticalExtensions.v8b0NonCriticalExtensions.
                   v950NonCriticalExtensions, va40NonCriticalExtensionsPresent))
         {
           RRC_MSG_COMMON_SET_BITMASK_IE_TYPE2(ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
                   v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
                   v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
                   v790nonCriticalExtensions.v860NonCriticalExtensions.v880NonCriticalExtensions.v920NonCriticalExtensions.v8b0NonCriticalExtensions.
                   v950NonCriticalExtensions, va40NonCriticalExtensions);
         }
         if(!RRC_CHECK_MSG_TYPE_IE (ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
                   v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
                   v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
                   v790nonCriticalExtensions.v860NonCriticalExtensions.v880NonCriticalExtensions.v920NonCriticalExtensions.v8b0NonCriticalExtensions.
                   v950NonCriticalExtensions.va40NonCriticalExtensions.interRATHandoverInfo_va40ext, ue_RadioAccessCapabilityComp2Present))
         {
           RRC_MSG_COMMON_SET_BITMASK_IE_TYPE2(ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
                   v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
                   v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
                   v790nonCriticalExtensions.v860NonCriticalExtensions.v880NonCriticalExtensions.v920NonCriticalExtensions.v8b0NonCriticalExtensions.
                   v950NonCriticalExtensions.va40NonCriticalExtensions.interRATHandoverInfo_va40ext, ue_RadioAccessCapabilityComp2);
         }
         if(!RRC_CHECK_MSG_TYPE_IE (ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
                   v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
                   v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
                   v790nonCriticalExtensions.v860NonCriticalExtensions.v880NonCriticalExtensions.v920NonCriticalExtensions.v8b0NonCriticalExtensions.
                   v950NonCriticalExtensions.va40NonCriticalExtensions.interRATHandoverInfo_va40ext.ue_RadioAccessCapabilityComp2, hsdsch_physical_layer_category_ext4Present))
         {
           RRC_MSG_COMMON_SET_BITMASK_IE_TYPE2(ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
                   v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
                   v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
                   v790nonCriticalExtensions.v860NonCriticalExtensions.v880NonCriticalExtensions.v920NonCriticalExtensions.v8b0NonCriticalExtensions.
                   v950NonCriticalExtensions.va40NonCriticalExtensions.interRATHandoverInfo_va40ext.ue_RadioAccessCapabilityComp2, hsdsch_physical_layer_category_ext4);
         }
         ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
                  v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
                  v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
                  v790nonCriticalExtensions.v860NonCriticalExtensions.v880NonCriticalExtensions.v920NonCriticalExtensions.v8b0NonCriticalExtensions.
                  v950NonCriticalExtensions.va40NonCriticalExtensions.interRATHandoverInfo_va40ext.ue_RadioAccessCapabilityComp2.
                  hsdsch_physical_layer_category_ext4 = RRC_DEFAULT_3C_HSDPA_CATEGORY;

            rrccsp_get_supported_band_list(wrrc_as_id,&ueci_band);
            
            if(0 == ueci_band.num_supported_bands)
            {
              WRRC_ERR0_FATAL("NO FDD bands supported");
            }
         ...
         if((RRC_SB_3C_BANDII_SUPPORTED & rrcnv_data.rrc_feature_nv_item_value) && 
         ...
         if(num_of_sb_3c)
         {
           if(!RRC_CHECK_MSG_TYPE_IE (ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
                    ...
                    v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
                    v790nonCriticalExtensions.v860NonCriticalExtensions.v880NonCriticalExtensions.v920NonCriticalExtensions.v8b0NonCriticalExtensions.
                    v950NonCriticalExtensions.va40NonCriticalExtensions.interRATHandoverInfo_va40ext, ue_RadioAccessCapabilityInfoPresent))
           {
            RRC_MSG_COMMON_SET_BITMASK_IE_TYPE2(ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
              v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
              v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
              v790nonCriticalExtensions.v860NonCriticalExtensions.v880NonCriticalExtensions.v920NonCriticalExtensions.v8b0NonCriticalExtensions.
                    v950NonCriticalExtensions.va40NonCriticalExtensions.interRATHandoverInfo_va40ext, ue_RadioAccessCapabilityInfo);
           }
           temp_fdd_list4_ptr = &(ptr->v390NonCriticalExtensions.u.present.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
                    v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v4d0NonCriticalExtensions.v590NonCriticalExtensions.
                    v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
                    v790nonCriticalExtensions.v860NonCriticalExtensions.v880NonCriticalExtensions.v920NonCriticalExtensions.v8b0NonCriticalExtensions.
                    v950NonCriticalExtensions.va40NonCriticalExtensions.interRATHandoverInfo_va40ext.ue_RadioAccessCapabilityInfo.ue_RadioAccessCapabBandFDDList4);
  ...
  /* Encodes the OTA message using ASN1. The memory for msg_ptr is allocated
     by the RRC procedure and the memory for encoded_sdu will be allocated
     by the following function */
  encoded_sdu = rrcasn1_encode_pdu(wrrc_as_id,ptr, rrc_InterRATHandoverInfo_PDU);
                                             ...
                                             /* ASN1 Encoding is failed        */
  {
    ...
    {
      WRRC_ERR2_FATAL("No memory for encoding %d %d",
                rrc_encode_info.encode_status,
                rrc_encode_info.pdu_num);
    }
    else if (rrc_encode_info.encode_status == ASN1_CONSTRAINT_VIOLATED)
    {
      WRRC_ERR2_FATAL("Constraint violation in ASN1 encoding %d %d",
                        rrc_encode_info.encode_status,
                        rrc_encode_info.pdu_num);
    }
    else
    {
      WRRC_ERR2_FATAL("ASN1 encoding failed %d %d",
                        rrc_encode_info.encode_status,
                        rrc_encode_info.pdu_num);
    }
  }
  ...
  if(encoded_sdu.length != 0)
  {
    ...
    rrciho_int_data.gtow_info.class_mark_ptr = rrc_malloc(encoded_sdu.length);
    WCDMA_MEMCPY(rrciho_int_data.gtow_info.class_mark_ptr,
                 encoded_sdu.length,
                 encoded_sdu.value,
                 encoded_sdu.length);
    
    WRRC_MSG2_HIGH("IHO_DEBUG: Encoded msg 1st two bytes = %x, %x ",*(rrciho_int_data.gtow_info.class_mark_ptr),*(rrciho_int_data.gtow_info.class_mark_ptr+1));

    /*copy the message pointer and the message length to the RR parameters*/
    *(msg_ptr) = rrciho_int_data.gtow_info.class_mark_ptr;
    *(msg_len) = (uint8)encoded_sdu.length;
    ...
    rrciho_int_data.gtow_info.class_mark_info_present = TRUE;
  ...
  // functions. This function should free memory allocated for msg_ptr and as
  // well as linked lists in the message if they are any allocated.
  rrcasn1_free_pdu(wrrc_as_id, (int) rrc_InterRATHandoverInfo_PDU, ptr);
  ptr = NULL;
...
DESCRIPTION
  ...
  This sub specific function builds a INTERRAT HANDOVER INFO message type which will be
  sent by GSM to UTRAN thru GERAN . An ASN1 encoded message and message
  length is populated by RRC, the message and msg_length is used by RR to
  send this as part of Classmark Change to UTRAN via GERAN
...
#if defined (FEATURE_LTE_TO_WCDMA) || defined (FEATURE_WCDMA_TO_LTE)
  rrcmcm_update_lte_mode_enabled_status(wrrc_as_id,FALSE);
#endif
#ifdef FEATURE_DUAL_SIM
  rrciho_ext_data.irat_plmn_id = rrc_get_rr_curr_camped_plmn_sub_id(rrc_get_as_id(wrrc_as_id));
#else
  rrciho_ext_data.irat_plmn_id = rrc_get_rr_curr_camped_plmn();
#endif

  if(!rrccsp_is_wcdma_band_enabled(wrrc_as_id))
  {
...
)
{
  ...
  switch( rrciho_int_data.rrciho_state )
  {
    ...
    case RRCIHO_WAIT_CELL_UPDATE_CNF:
      WRRC_MSG1_HIGH("IHO proc has ongoing WtoG HO iho substate %d", rrciho_int_data.rrciho_state);
      return TRUE;

    default:
      WRRC_MSG1_HIGH("IHO proc has NO WtoG HO ongoing iho substate %d",rrciho_int_data.rrciho_state);
      return FALSE;
  }
}
/*===========================================================================
...
DESCRIPTION
  ...
  This sub specific function deletes the CLASSMARK_CHANGE_HANDOVER_INFO asn1 type
...
SIDE EFFECTS
  None.
===========================================================================*/
void rrc_delete_classmark_change_handover_info_sub_id( sys_modem_as_id_e_type wrrc_as_id)
{
#ifdef FEATURE_DUAL_SIM
  if (wrrc_as_id != rrc_get_as_id(wrrc_as_id))
  {
...
{
  rrc_delete_classmark_change_handover_info_sub_id(SYS_MODEM_AS_ID_1);
}
...
===========================================================================*/
void rrciho_send_sync_ind_for_gtow_ho(sys_modem_as_id_e_type wrrc_as_id)
{
 ...
 */
  ...
  {
    /*Delay Dwngrd to BGT by couple of secs to receive UMI */
    rrctmr_start_timer(wrrc_as_id, RRCTMR_TRM_PRIORITY_CHANGE_TIMER, 2000);
  }
#endif
#ifdef FEATURE_LTE_TO_WCDMA
#ifdef FEATURE_WCDMA_FAST_RETURN_TO_LTE_AFTER_CSFB
  if((iho_interrat_type == RRCIHO_INTERRAT_LTE) && (rrciho_int_data.gtow_info.is_cs_rab_present) && rrcnv_data.rrc_wtol_redirect_after_srvcc_nv)
  {
    rrc_csp_int_data.csfb_extended_status = CSFB_STARTED;
  }
#endif
  /* is_cs_rab_present is not true for default config 0 and 1
    so change the below condition to handle this scenario */
  if((iho_interrat_type == RRCIHO_INTERRAT_GSM) || (rrciho_int_data.gtow_info.is_cs_rab_present))
  {
#endif /* FEATURE_LTE_TO_WCDMA */
...
#ifdef FEATURE_DUAL_SIM
    mm_cmd_ptr->cmd.rrc_sync_ind.as_id = rrc_get_as_id(wrrc_as_id);
#endif
  ...
  /* Fill in parameters fro RRC_SYNC_IND command */
    ...
    /*set the rab info if it was included in the handover to utran
      message*/
    if(rrciho_int_data.gtow_info.rab_info_included == TRUE)
    {
      mm_cmd_ptr->cmd.rrc_sync_ind.rab_id_present = TRUE;
#ifndef FEATURE_LTE_TO_WCDMA
      mm_cmd_ptr->cmd.rrc_sync_ind.rab_info.rab_id = rrciho_int_data.gtow_info.est_rabs.rab_id;

      if(rrciho_int_data.gtow_info.est_rabs.rab_type == RRCRB_CS_VOICE_RAB)
      {
...
#ifdef FEATURE_WCDMA_FAST_RETURN_TO_LTE_AFTER_CSFB
        if(rrc_csp_int_data.csfb_extended_status == CSFB_STARTED)
        {
          rrc_csp_int_data.cs_rab_established = TRUE;
        }
#endif
      }
      else if(rrciho_int_data.gtow_info.est_rabs.rab_type == RRCRB_CS_DATA_RAB)
      {
...
#else
      mm_cmd_ptr->cmd.rrc_sync_ind.rab_info.rab_id = rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].rab_id;

      if(rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].rab_type == RRCRB_CS_VOICE_RAB)
      {
        mm_cmd_ptr->cmd.rrc_sync_ind.rab_info.cs_rab_type = RRCMN_CS_VOICE_CALL;
#ifdef FEATURE_WCDMA_FAST_RETURN_TO_LTE_AFTER_CSFB
        if(rrc_csp_int_data.csfb_extended_status == CSFB_STARTED)
      ...
      else if(rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].rab_type == RRCRB_CS_DATA_RAB)
      {
        /*HO of CS Data call from G to W is for futuristic use. Right now we don't use it.*/
        mm_cmd_ptr->cmd.rrc_sync_ind.rab_info.cs_rab_type = RRCMN_CS_DATA_CALL;        
      }
      ...
      rrciho_int_data.gtow_info.rab_info_included = FALSE;

      /* Fill in the codec type used in RRC_SYNC_IND*/
      if(mm_cmd_ptr->cmd.rrc_sync_ind.cn_domain_id == RRC_CS_DOMAIN_CN_ID)
      {
        if(rrc_get_current_codec_mode(wrrc_as_id) == WCDMA_IVOCODER_ID_AMRWB )
        {
    ...
    #error code not present
#endif
    rrc_send_rrc_message_to_mm(wrrc_as_id,mm_cmd_ptr);               /* Sends the command to MM */
    ...
    rrcdt_check_fake_security_and_send_sync_ind(wrrc_as_id, RRC_CS_DOMAIN_CN_ID);
  ...
  {
    uint16 i;
    for(i = 0; i < rrciho_int_data.gtow_info.number_of_est_rabs; i++)
    {
      if(rrciho_int_data.gtow_info.est_rabs[i].cn_domain == RRC_PS_DOMAIN_CN_ID)
      {
        ...
        rabm_ptr->cmd.rrc_establish_ind.rabid = (rabid_T)(rrciho_int_data.gtow_info.est_rabs[i].rab_id);
#ifdef FEATURE_DUAL_SIM
        rabm_ptr->cmd.rrc_establish_ind.as_id = rrc_get_as_id(wrrc_as_id);
#endif
    
        /* Put the command on RABM queue */
        WRRC_MSG1_HIGH("LTOW_PSHO: Sending RAB_ESTABLISH_IND to RABM RABid %d",
                                                              rrciho_int_data.gtow_info.est_rabs[i].rab_id);
        rrc_crash_debug_save_misc_cmds(wrrc_as_id,RRC_RABMAS_RAB_ESTABLISH_IND);
        rabm_put_cmd(rabm_ptr);
      }
    }
    rrcdt_check_fake_security_and_send_sync_ind(wrrc_as_id, RRC_PS_DOMAIN_CN_ID);
  }
#endif /* FEATURE_LTE_TO_WCDMA */

  /*send the OTA message to UTRAN*/
  rrciho_send_handover_to_utran_complete(wrrc_as_id);
    ...
    )
  {
    ...
    (void)rrcsmc_step_2_ho_ciph_req(wrrc_as_id,rrciho_int_data.gtow_info.count_c_act_time);
  }
  ...
  if(iho_interrat_type == RRCIHO_INTERRAT_LTE)
  {
    rrciho_send_cipher_sync_ind(wrrc_as_id);
  }
#endif
...
)
{
  ...
  rrciho_ext_data.irat_plmn_id = utra_cap_req->camped_plmn;
  /*update the modes and band preferences*/
#ifdef FEATURE_DUAL_WCDMA
  rrc_update_band_preference_sub_id(utra_cap_req->rat_capabilities.wcdma_band_preference, wrrc_as_id);
#else
  rrc_update_band_preference(utra_cap_req->rat_capabilities.wcdma_band_preference);
#endif
  rrcmcm_update_lte_mode_enabled_status(wrrc_as_id, TRUE);

  if(!utra_cap_req->rat_capabilities.geran_is_supported)
  {
    rrcmcm_update_dual_mode_enabled_status(wrrc_as_id, FALSE);
  }
  else
  {
    rr_rrc_set_band_pref_internal(wrrc_as_id,utra_cap_req->rat_capabilities.geran_band_preference);
    rrcmcm_update_dual_mode_enabled_status(wrrc_as_id, TRUE);
  }
  ...
  */
  rrc_get_utran_classmark_change_handover_info(wrrc_as_id,
  ...
  rrc_send_lte_cmd(wrrc_as_id,&utra_cap_rsp.msg_hdr,WCDMA_RRC_LTE_UTRA_CAPABILITIES_RSP,sizeof(wcdma_rrc_lte_utra_capabilities_rsp_type));
  WRRC_MSG0_HIGH("LTOW: WCDMA_RRC_LTE_UTRA_CAPABILITIES_RSP sent to LTE RRC");
#ifdef FEATURE_DUAL_WCDMA
  rrc_delete_classmark_change_handover_info_sub_id(wrrc_as_id);
#else
...
===========================================================================*/
void rrciho_event_handler(sys_modem_as_id_e_type wrrc_as_id,
  rrc_cmd_type *cmd_ptr                      /* Pointer to command */
)
{
  WRRC_MSG2_HIGH_OPT("function rrciho_event_handler is called in rrciho_state rrciho_state_e_type_value%d : Event is received by IHO: %d", rrciho_int_data.rrciho_state,cmd_ptr->cmd_hdr.cmd_id);
  rrc_crash_debug_log_proc_substates(wrrc_as_id,RRC_PROCEDURE_IHO,rrciho_int_data.rrciho_state,cmd_ptr);
  ...
  {
    rrciho_handle_utra_cap_req_from_lte(wrrc_as_id,&cmd_ptr->cmd.wcdma_rrc_lte_irat_cmd.cmd.wcdma_rrc_lte_utra_capabilities_req);
    return;
  }
#endif
  ...
  switch( rrciho_int_data.rrciho_state )
  {
    ...
    case RRCIHO_WAIT_CHAN_CONFIG_GTOW_CNF:
      ...
      rrciho_wait_chan_config_gtow_cnf_event_handler(wrrc_as_id,  cmd_ptr );
      break;

    case RRCIHO_WAIT_CIPH_CONFIG_GTOW_CNF:
      /* Call the event handler for wait for ciph config state to go to dch in
         case of a GSM to WCDMA transition */
      rrciho_wait_ciph_config_gtow_cnf_event_handler(wrrc_as_id,  cmd_ptr );
      break;

    case RRCIHO_WAIT_STATE_CHANGE_IND:
      rrciho_wait_state_change_event_handler(wrrc_as_id, cmd_ptr );
      break;
    ...
    default:
      /* Invalid substate - should never happen. Clear
         all procedure vars */
      rrciho_init_data(wrrc_as_id);
      WRRC_ERR1_FATAL("Invalid IHO Substate %d", rrciho_int_data.rrciho_state);
      break;
  }/* end switch */
  return;
...
===========================================================================*/
void rrciho_init_procedure(sys_modem_as_id_e_type wrrc_as_id)
{
  rrciho_int_data.gsm_commands.n =0;
  ...
  rrciho_int_data.gtow_info.predef_config_id = 0;
  rrciho_int_data.gtow_info.config_type = IHO_GTOW_PREDEF_CONFIG;

  rrciho_init_default_configuration(wrrc_as_id);
  /* Register the state change from any state to Idle */
  rrcscmgr_register_for_scn(wrrc_as_id, RRC_PROCEDURE_IHO,      /* Procedure name */
...
#ifndef FEATURE_LTE_TO_WCDMA
    rrciho_int_data.gtow_info.est_rabs.rb_for_rab[rb_count].rb_id = (uint8)(rb_ptr->elem[rb_count].rb_Identity);
#else
    rrciho_int_data.gtow_info.est_rabs[index].rb_for_rab[rb_count].rb_id = (uint8)(rb_ptr->elem[rb_count].rb_Identity);
#endif
    rb_count++;
...
)
{
  ...
  while ((srb_ptr!=NULL )&& (srb_ptr->n > srb_count ))
  {
    ...
    rrciho_int_data.gtow_info.srb_id[srb_count] = rb_id;
    srb_count++;
...
)
{
...
#endif /* FEATURE_WCDMA_REL9 */
#ifdef FEATURE_WCDMA_REL10
    rrc_HandoverToUTRANCommand_r10_IEs * ho_utran_r10;
#endif /* FEATURE_WCDMA_REL10 */
    ...
    if (release ==  MSG_REL8)
    {
      ...
      ho_utran_r8 = &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.r8
   ...
   if(T_rrc_RAB_Identity_gsm_MAP_RAB_Identity == ho_utran_r8->specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.rab_Identity.t)
   {
     *rab_identity_choice = T_rrc_RAB_Identity_gsm_MAP_RAB_Identity;
     ...
     rrciho_int_data.gtow_info.est_rabs.rab_id =
        rrcrb_translate_gsm_map_rab_id(wrrc_as_id,ho_utran_r8->specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.
                                       rab_Identity.u.gsm_MAP_RAB_Identity);
   
     rrciho_int_data.gtow_info.est_rabs.re_est_timer = ho_utran_r8->specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.re_EstablishmentTimer;
     ...
     if(rab_list_r8->elem[0].rab_Info.cn_DomainIdentity == rrc_CN_DomainIdentity_cs_domain )
     {
       ...
       {
         rrciho_int_data.gtow_info.est_rabs.nas_Synchronisation_Indicator = 
             rrc_translate_nas_sync_indicator(wrrc_as_id,&(rab_list_r8->elem[0].rab_Info.nas_Synchronisation_Indicator));
   ...
   return rab_list_r8;
    ...
    {
      ho_utran_r9 = &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.
        later_r8_criticalExtensions.u.r9.handoverToUTRANCommand_r9;
      if(T_rrc_RAB_Identity_gsm_MAP_RAB_Identity == ho_utran_r9->specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.rab_Identity.t)
      {
        *rab_identity_choice = T_rrc_RAB_Identity_gsm_MAP_RAB_Identity;
        ...
        rrciho_int_data.gtow_info.est_rabs.rab_id =
           rrcrb_translate_gsm_map_rab_id(wrrc_as_id,ho_utran_r9->specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.
                                          rab_Identity.u.gsm_MAP_RAB_Identity);
      
        rrciho_int_data.gtow_info.est_rabs.re_est_timer = ho_utran_r9->specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.re_EstablishmentTimer;
...
#ifdef FEATURE_WCDMA_REL10
    else if (release ==	MSG_REL10)
    {
       ho_utran_r10 = &rrciho_int_data.gtow_info.ho_ptr->u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.
         later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.handoverToUTRANCommand_r10;
       if(T_rrc_RAB_Identity_gsm_MAP_RAB_Identity == ho_utran_r10->specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.rab_Identity.t)
      {
         *rab_identity_choice = T_rrc_RAB_Identity_gsm_MAP_RAB_Identity;
         ...
         rrciho_int_data.gtow_info.est_rabs.rab_id =
         rrcrb_translate_gsm_map_rab_id(wrrc_as_id,ho_utran_r10->specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.
          rab_Identity.u.gsm_MAP_RAB_Identity);
         
         rrciho_int_data.gtow_info.est_rabs.re_est_timer = ho_utran_r10->specificationMode.u.complete.rab_InformationSetupList.elem[0].rab_Info.re_EstablishmentTimer;
         
         rb_list_r8 =&ho_utran_r10->specificationMode.u.complete.rab_InformationSetupList.elem[0].rb_InformationSetupList;
          rrciho_initialize_rb_list_r8(wrrc_as_id,rb_list_r8);
         
         rab_list_r8 = &ho_utran_r10->specificationMode.u.complete.rab_InformationSetupList;
...
===========================================================================*/
boolean is_iho_w2g_active(sys_modem_as_id_e_type wrrc_as_id)
{
  if((rrciho_int_data.rrciho_state != RRCIHO_INITIAL) &&
     ((rrciho_int_data.procedure_info.handover_type == T_rrc_DL_DCCH_MessageType_handoverFromUTRANCommand_GSM)
#ifdef FEATURE_LTE_TO_WCDMA
     || (rrciho_int_data.procedure_info.handover_type == T_rrc_DL_DCCH_MessageType_handoverFromUTRANCommand_EUTRA)
#endif
...
{
  if(rrciho_int_data.gtow_info.sib16_ptr != NULL)
  {
    rrcasn1_free_pdu(wrrc_as_id,(int)rrc_SysInfoType16_PDU, rrciho_int_data.gtow_info.sib16_ptr);
    rrciho_int_data.gtow_info.sib16_ptr = NULL;
  }
  else
  {
...
===========================================================================*/
boolean rrciho_is_gtow_ho_active(sys_modem_as_id_e_type wrrc_as_id)
{
  /* Check the procedure substate and procedure the event accordingly */
  switch( rrciho_int_data.rrciho_state )
  {
    ...
    case RRCIHO_WAIT_CHAN_CONFIG_GTOW_CNF:
    case RRCIHO_WAIT_CIPH_CONFIG_GTOW_CNF:
      WRRC_MSG1_HIGH("IHO proc has ongoing GTOW HO iho substate %d", rrciho_int_data.rrciho_state);
      return TRUE;
...
===========================================================================*/
boolean rrciho_check_and_handle_mdsp_halt_during_gtow_handover(sys_modem_as_id_e_type wrrc_as_id)
{
  boolean handled_mdsp_halt = TRUE;
  WRRC_MSG1_HIGH("IHO state %d",rrciho_int_data.rrciho_state);
  switch(rrciho_int_data.rrciho_state)
  {
    case RRCIHO_WAIT_CHAN_CONFIG_GTOW_CNF:
      rrciho_int_data.gtow_info.est_rabs.nas_Synchronisation_Indicator= rrc_get_current_codec_mode(wrrc_as_id);
...
===========================================================================*/
boolean rrciho_check_and_handle_mdsp_halt_during_wtog_handover(sys_modem_as_id_e_type wrrc_as_id)
{
  boolean handled_mdsp_halt = TRUE;
  WRRC_MSG1_HIGH("W2G IHO substate %d ",rrciho_int_data.rrciho_state);
  switch(rrciho_int_data.rrciho_state)
  {
    case RRCIHO_WAIT_INTERRAT_HO_CNF:
    case RRCIHO_WAIT_ABORT_CNF:
...
boolean rrciho_is_cs_data_rab_present (sys_modem_as_id_e_type wrrc_as_id)
{
  boolean status = FALSE;
#ifdef FEATURE_LTE_TO_WCDMA
  if(rrciho_int_data.gtow_info.is_cs_rab_present)
  {
    if ( RRCRB_CS_DATA_RAB == rrciho_int_data.gtow_info.est_rabs[rrciho_int_data.gtow_info.cs_rab_index].rab_type )
    {
      status = TRUE;
    }
  }
#else
  if(rrciho_int_data.gtow_info.est_rabs.rab_type == RRCRB_CS_DATA_RAB)
  { 
    status =  TRUE;
  }
#endif /* FEATURE_LTE_TO_WCDMA */  
...
)
{
  ...
  oc_status = rrcllc_get_ordered_config_state_and_proc(wrrc_as_id,&proc_id,&process_state);

  if (!rrciho_is_gtow_ho_active(wrrc_as_id)|| (RRC_PROCEDURE_IHO != proc_id))
  {
  ...
  if(rrciho_int_data.gtow_info.is_ps_rab_present)
  {
    for(i = 0; i < rrciho_int_data.gtow_info.number_of_est_rabs; i++)
    {
      if(rrciho_int_data.gtow_info.est_rabs[i].cn_domain == RRC_PS_DOMAIN_CN_ID)
      {
        ...
        rb_id = rrciho_int_data.gtow_info.est_rabs[i].rb_for_rab[0].rb_id;
...
===========================================================================*/
void rrciho_send_cipher_sync_ind(sys_modem_as_id_e_type wrrc_as_id)
{
  /*declare var of struct abort confirm type*/
  ...
  rr_send_rrc_message_to_rr_internal(wrrc_as_id,(void *)(&(gtow_cipher_sync_ind)), RRC_CIPHER_SYNC_IND,
...
none
===========================================================================*/
uint8  rrciho_get_current_substate(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrciho_int_data.rrciho_state;
}
...
#endif
boolean rrciho_return_interrat_type(sys_modem_as_id_e_type wrrc_as_id)
{
#ifdef FEATURE_LTE_TO_WCDMA
...
===========================================================================*/
  ...
  __attribute__((section(".uncompressible.text")))
#endif
uint8 rrciho_return_iho_abort(sys_modem_as_id_e_type wrrc_as_id)
{
  return ((rrciho_int_data.rrciho_abort == TRUE)?1:0);
}
...
===========================================================================*/
  ...
  __attribute__((section(".uncompressible.text")))
#endif
uint8 rrciho_return_iho_substate(sys_modem_as_id_e_type wrrc_as_id)
{
  return (uint8)rrciho_int_data.rrciho_state;
}
...
===========================================================================*/
  ...
  __attribute__((section(".uncompressible.text")))
#endif
uint32 rrciho_return_default_config_id(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrciho_int_data.gtow_info.default_config_id;
}
...
===========================================================================*/
  ...
  __attribute__((section(".uncompressible.text")))
#endif
uint8 rrciho_return_config_type(sys_modem_as_id_e_type wrrc_as_id)
{
  return (uint8)rrciho_int_data.gtow_info.config_type;
}
...
{
  qsh_mdump_collect_high(&rrciho_int_data,sizeof(rrciho_int_data));
}
#endif
