...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcllctrchie.c#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $

when        who     what, where, why
--------    ---     ---------------------------------------------------------
01/03/18   sp      Made changes to support configuration with RB-ID 32
12/05/16   ha      KW error
05/16/16   sg      Made changes for F3 reduction
03/10/16   nr      Fixed KW errors
01/21/16   sp      Added Checks to avoid NULL pointer access
10/14/15   sp      Made changes to add call end metrics - FR#30856
05/12/15   aa      Added Message Highs for validation failures
03/20/14   sp      Made changes to backout UL/DL TFC validation
12/30/14   bc      Replacing ERR_FATAL with MSG_ERR0_FATAL, MSG_ERR1_FATAL, MSG_ERR2_FATAL.
12/26/14   sg      Made changes to reject the configuration if SRBs are mapped to EDCH 
                   transport channels and EDCH transmission variable is FALSE.
11/19/14   sg      Made changes to remove default switch cases 
                   for ASN1 IEs which will never be hit
12/16/14   sp      Made changes to add validation for UL/DL TFC Info
...
rrcllc_dl_dch_info_buffer_type  dl_dch_info_buffer_sim[MAX_RRC_STACKS]; 

/* This is a temporary storage buffer for all uplink DCH info */
rrcllc_ul_dch_info_buffer_type  ul_dch_info_buffer_sim[MAX_RRC_STACKS];

#ifdef FEATURE_WCDMA_HS_FACH
rb_mapping_info_type_  *rrc_ordered_hanging_rb_sim[MAX_RRC_STACKS];
rb_mapping_info_type_  *rrc_current_hanging_rb_sim[MAX_RRC_STACKS];
#endif

rb_mapping_info_type *rrc_ordered_hanging_rb_mapping_sim[MAX_RRC_STACKS];
rb_mapping_info_type *rrc_current_hanging_rb_mapping_sim[MAX_RRC_STACKS];

#define dl_dch_info_buffer dl_dch_info_buffer_sim[WAS_ID_M]
#define ul_dch_info_buffer ul_dch_info_buffer_sim[WAS_ID_M]
#ifdef FEATURE_WCDMA_HS_FACH
...
\
            ...
            if (ie_ptr->elem[idx].m.powerOffsetInformationPresent)\
                (void)get_ul_gain_factor_info (wrrc_as_id,config_ptr->rrc_ul_cctrch_params.gain_factor_parms, &ie_ptr->elem[idx].powerOffsetInformation,(uint8)tfc_idx);\
\
...
\
            /* Save the Power Offset Information */\
            if (ie_ptr->elem[idx].m.powerOffsetInformationPresent)\
                (void)get_ul_gain_factor_info (wrrc_as_id,prach_sel_info_ptr->gain_factor_parms, &ie_ptr->elem[idx].powerOffsetInformation,(uint8)tfc_idx);\
\
            /* Get the pointer to the next TFC */\
            idx++;\
\
...
)
{
    ...
    */
    if(ordered_config_ptr->rrc_state_indicator == RRC_STATE_CELL_DCH)
    {
       ...
       */
      trch_idx = rrcllc_find_ul_dch_trch_idx(wrrc_as_id,ordered_config_ptr,
                                             trch_id);
      ...
      if(rrcllc_restore_ul_dch_rb_mapping_info(wrrc_as_id,trch_idx) == FAILURE)
      {
      ...
      if(get_ul_transport_format_set(wrrc_as_id,ordered_config_ptr, 
 ...
 PDU size has changed.*/ 
      ...
      rrcllc_get_rb_for_rb_list_mapped_to_dch(wrrc_as_id,trch_id, 
...
)
{
  ...
  ordered_config_ptr->l1_hsdpa_info.hs_dsch_info.harq_info.num_proc = 
    rrc_convert_num_harq_process(wrrc_as_id, harq_info_ptr);
...
)
{
 ...
 idx =0;
    ...
    while((num_tb_ptr->n  > idx) && (tf_idx < UE_MAX_TF))
    {
      ...
      oc_tf_ptr->ntblks = get_tfs_num_tb(wrrc_as_id,&num_tb_ptr->elem[idx]);

      /* Save RLC Size */
      oc_tf_ptr->tb_size = (uint16) get_tfs_ded_rlc_size(wrrc_as_id,&tfs_ptr->elem[idy]);
#ifdef FEATURE_RRC_TFC_RLC_SIZE_FOR_DCH
      ...
      if (get_logical_channel_list(wrrc_as_id,&tfs_ptr->elem[idy].logicalChannelList,
                                   rec) == FAILURE)
      {
        return FAILURE;
      }
  ...
  if(get_ul_semi_static_transport_format_info(wrrc_as_id,&ie_ptr->semistaticTF_Information, oc_ssp_ptr)
    == FAILURE)
  {
    return FAILURE;
  }
  ...
  */
  ...
  while((tfs_ptr->n > idy) && (tf_idx < UE_MAX_TF))
  {
    ...
    while((num_tb_ptr->n > idx) && (tf_idx < UE_MAX_TF))
    {
      ...
      /* Save RLC Size */
      oc_tf_ptr->tb_size = (uint16) get_tfs_com_rlc_size(wrrc_as_id,&tfs_ptr->elem[idy]);
...
)
{
  ...
  switch (ie_ptr->t)
  {
    ...
    case T_rrc_TransportFormatSet_dedicatedTransChTFS:
        rv = get_ul_ded_trch_dynamic_tfs_info (wrrc_as_id,config_ptr,
    ...
    case T_rrc_TransportFormatSet_commonTransChTFS:
        rv = get_ul_common_trch_dynamic_tfs_info (wrrc_as_id,config_ptr,
...
)
{
  ...
  switch (ie_ptr->t)
  {
    /* Common Transport Channel */
    case T_rrc_TransportFormatSet_commonTransChTFS:
        rv = get_ul_common_trch_dynamic_tfs_info (wrrc_as_id,NULL,
                                                  ie_ptr->u.commonTransChTFS,
                                                  oc_tfs_ptr, oc_ssp_ptr);
        break;
      ...
      case T_rrc_TFCS_normalTFCI_Signalling:
        ...
        switch (ie_ptr->u.normalTFCI_Signalling->t)
        {
          case T_rrc_ExplicitTFCS_Configuration_complete:
            ...
            if (get_ul_ctfc_info_for_prach_db(wrrc_as_id,prach_sel_info_ptr, 
...
)
{
  ...
  return get_ul_ctfc_info(wrrc_as_id,config_ptr, ie_ptr);
...
)
{
  ...
  switch (ie_ptr->t)
  {
    case T_rrc_ExplicitTFCS_Configuration_complete:
      /* Initialize the Uplink TFI table first */
      rrcllc_init_ul_tfi_table(wrrc_as_id,config_ptr);

      /* Get and save the CTFC chosen */
      if (get_ul_ctfc_info(wrrc_as_id,config_ptr, ie_ptr->u.complete) == FAILURE)
      {
        return FAILURE;
      }
      break;
    ...
    case T_rrc_ExplicitTFCS_Configuration_addition:
      if(get_ul_add_tfcs_info(wrrc_as_id,config_ptr, ie_ptr->u.addition) == FAILURE)
      { 
        return FAILURE;
      } 
      break;
    ...
    case T_rrc_ExplicitTFCS_Configuration_removal:
      if(get_ul_remove_tfcs_info(wrrc_as_id,config_ptr, ie_ptr->u.removal) == FAILURE)
      {
        return FAILURE;
      }
      break;
    ...
    case T_rrc_ExplicitTFCS_Configuration_replacement:
      ...
      if(get_ul_remove_tfcs_info(wrrc_as_id,config_ptr, &ie_ptr->u.replacement->tfcsRemoval)
          == FAILURE)
      {
        return FAILURE;
      }
      ...
      if(get_ul_add_tfcs_info(wrrc_as_id,config_ptr, &ie_ptr->u.replacement->tfcsAdd) == FAILURE)
      {
        return FAILURE;
      }
      break;
...
)
{
  ...
  rrcllcoc_int_data.ul_tfcs_rcvd = TRUE;
  /* Get TFCS based on TFCI Signaling type */
  switch (ie_ptr->t)
  {
      /* Normal TFCI Signaling - Explicit TFCS Info */
      case T_rrc_TFCS_normalTFCI_Signalling:
          rv = get_ul_normal_tfci_info(wrrc_as_id,config_ptr, ie_ptr->u.normalTFCI_Signalling);
          break;
    ...
    if(RRC_MSG_COMMON_BITMASK_IE_PTR(ie_ptr,rrc_UL_CommonTransChInfo,modeSpecificInfo))
    {
      switch(ie_ptr->modeSpecificInfo.t)
      {
        case T_rrc_UL_CommonTransChInfo_modeSpecificInfo_fdd:
          ...
          if(get_ul_tfcs_info(wrrc_as_id,ordered_config_ptr, &ie_ptr->modeSpecificInfo.u.fdd->ul_TFCS) == FAILURE)
          {
            return FAILURE;
          }
...
)
{
  ...
  {
    ix = rrcllc_find_ul_dch_trch_idx(wrrc_as_id,ordered_config_ptr,
    ...
    rrcllc_backup_rbmapping_info(wrrc_as_id,ix,UL_DCH_RB_MAPPING );
    /* Clear the RB Mapping info for this TrCH */
    rrcllc_init_ul_dch_rb_mapping_info(wrrc_as_id,ordered_config_ptr, ix);
    ...
    if(rrcllc_deallocate_ul_dch_trch_idx(wrrc_as_id,ix) == FAILURE)
    {
      return FAILURE;
    }
    ...
    rrcllc_init_dch_rlc_size_info(wrrc_as_id, ix);
#endif/*#ifdef FEATURE_RRC_TFC_RLC_SIZE_FOR_DCH */
  }
  else
  {
    ix = rrcllc_find_ul_rach_trch_idx(wrrc_as_id);
    ...
    rrcllc_backup_rbmapping_info(wrrc_as_id,ix,RACH_RB_MAPPING);

    rrcllc_init_rach_rb_mapping_info(wrrc_as_id,ordered_config_ptr);
  }

  /* RRCEUL-query why is this cleared ir-respective of DCH or RACH trch 
     delete information */
  /* Clear the TFS for this TrCH */
  rrcllc_init_ul_tfs_table(wrrc_as_id,ordered_config_ptr, ix);
...
)
{
  if (RRC_CHECK_COMMON_MSG_TYPE_PTR(ie_ptr,rrc_UL_TransportChannelIdentity_r6_dch_usch))
  {
     ...
     */
    rrcllcoc_int_data.ul_tfcs_needed = TRUE;
    return (process_ul_deleted_trch_info_dch_usch(wrrc_as_id,
  ...
  else if (RRC_CHECK_COMMON_MSG_TYPE_PTR(ie_ptr,rrc_UL_TransportChannelIdentity_r6_e_dch))
  {
    ...
    e_mac_d_flow_idx =
      rrcllc_find_e_mac_d_flow_idx(wrrc_as_id,(uint8)ie_ptr->u.e_dch);
    ...
    {
      rrcllc_backup_rbmapping_info(wrrc_as_id,e_mac_d_flow_idx,E_DCH_RB_MAPPING );
      rrcllc_init_e_dch_rbmapping_info(wrrc_as_id,ordered_config_ptr, e_mac_d_flow_idx);
...
)
{
  ...
  MSG_MED("Get UL Add/Reconf TrCH",0,0,0);
    ...
    if(rrcllc_dch_usch_add_trch_info(wrrc_as_id,(tr_ch_id_type)ie_ptr->transportChannelIdentity,
...
)
{
    ...
    for (trch_idx = (int8)(tfcs_ptr->nchan-1); trch_idx >= 0 && trch_idx < UE_MAX_TRCH; trch_idx--)
    {
  ...
  switch (ie_ptr->t)
  {
    ...
    case T_rrc_ExplicitTFCS_Configuration_complete:
      /* First initialize all DL CTFCs in L1 and local structures */ 
      {
        if (sccpch_with_ctch)
        {
          rrcllc_init_dl_ctfcs_for_sccpch_with_ctch(wrrc_as_id,config_ptr);
        }
        else
        {
          rrcllc_init_dl_ctfcs(wrrc_as_id,config_ptr);
        }
      }
...
)
{
  ...
  rrcllcoc_int_data.dl_tfcs_rcvd = TRUE;
  switch (ie_ptr->t)
  {
    case T_rrc_TFCS_normalTFCI_Signalling:
      if(get_dl_tfcs_explicit_configuration(wrrc_as_id,config_ptr,
...
)
{
  ...
  rrc_DedicatedDynamicTF_InfoList * tf_info_list=NULL;
  rrc_DedicatedDynamicTF_Info_numberOfTbSizeList * num_tb_list;

  /* get the semistatic information */
  if(get_dl_semistatic_tf_info(wrrc_as_id,&(ie_ptr->semistaticTF_Information), dl_tfs_info)
     == FAILURE)
  {
    return FAILURE;
  }
  ...
  /* -------------------------------------------- */
  if(tf_info_list != NULL)
  {
    while (tf_info_list->n > idx)
    {
      /* Get the RLC size for this TF info and adjust that to TB size */
      tmp_tb_size = get_tfs_ded_rlc_size(wrrc_as_id,&(tf_info_list->elem[idx]));
      ...
      while (num_tb_list->n > idy)
      {
        ...
        if ((dl_tfs_info->tf_info_array[cur_tf_idx]->num_tb =
             get_tfs_num_tb(wrrc_as_id,&(num_tb_list->elem[idy]))) == 0xFF)
        {
          return FAILURE;
        }
...
)
{
  ...
  uint8 cur_tf_idx = 0;
  uint32 idx =0,idy=0;
  rrc_CommonDynamicTF_InfoList * tf_info_list=NULL;
  ...
  if(tf_info_list != NULL)
  {
    while (tf_info_list->n > idx)
    {
      ...
      tmp_tb_size = get_tfs_com_rlc_size(wrrc_as_id,&(tf_info_list->elem[idx]));
      if(tmp_tb_size == RRCLLC_INVALID_RLC_SIZE)
      {
        return FAILURE;
      }
...
)
{
  switch (ie_ptr->t)
  {
    case T_rrc_TransportFormatSet_dedicatedTransChTFS:
      ...
      if(get_dl_tfs_dedicated_trch(wrrc_as_id,(ie_ptr->u.dedicatedTransChTFS),
    ...
    case T_rrc_TransportFormatSet_commonTransChTFS:
      ...
      if(get_dl_tfs_common_trch(wrrc_as_id,(ie_ptr->u.commonTransChTFS), dl_com_tfs_info)
         == FAILURE)
      {
        return FAILURE;
      }
        ...
        if(ie_ptr->modeSpecificInfo.u.fdd->m.dl_ParametersPresent)
        {
          switch(ie_ptr->modeSpecificInfo.u.fdd->dl_Parameters.t)
          {
            ...
            case T_rrc_DL_CommonTransChInfo_dl_Parameters_dl_DCH_TFCS:
              ...
              if(get_dl_tfcs_info(wrrc_as_id,ordered_config_ptr,
            ...
            case T_rrc_DL_CommonTransChInfo_dl_Parameters_sameAsUL:
              ...
              else
              {
                ...
                rrcllc_copy_ul_tfcs_info_to_dl_tfcs(wrrc_as_id);
              }
              break;
          } /* end TFCS switch */
...
)
{
  ...
  {
    ix = rrcllc_find_dl_dch_trch_idx(wrrc_as_id,(tr_ch_id_type)ie_ptr->dl_TransportChannelIdentity);
    if(ix >= UE_MAX_TRCH)
    {
    ...
    rrcllc_backup_rbmapping_info(wrrc_as_id,ix,DL_DCH_RB_MAPPING );
    /* Clear the RB Mapping info for this TrCH */
    rrcllc_init_dl_dch_rb_mapping_info(wrrc_as_id,ordered_config_ptr, ix);

    /* Clear the TFS for this TrCH */
    rrcllc_init_dl_tfs_table(wrrc_as_id,ordered_config_ptr, ix);
    ...
    /* Indicate that the old index is now available */
    if(rrcllc_deallocate_dl_dch_trch_idx(wrrc_as_id,ix) == FAILURE)
    {
      return FAILURE;
    }
  }
  else  /* Must be FACH */
  {
    ...
    ix = rrcllc_find_dl_fach_trch_idx(wrrc_as_id);
    if(ix >= UE_MAX_TRCH)
    {
    ...
    rrcllc_backup_rbmapping_info(wrrc_as_id,ix,FACH_RB_MAPPING );

    /* Clear the RB Mapping info for this TrCH */
    rrcllc_init_fach_rb_mapping_info(wrrc_as_id,ordered_config_ptr, ix);
...
)
{
  if(trch_idx < UE_MAX_TRCH)
  {
    ...
    rrcllc_init_dl_dch_rb_mapping_info(wrrc_as_id,ordered_config_ptr, trch_idx);

    /* Clear the TFS (L1) for this TrCH */
    rrcllc_init_dl_tfs_table(wrrc_as_id,ordered_config_ptr, trch_idx);
    ...
    if(rrcllc_deallocate_dl_dch_trch_idx(wrrc_as_id,trch_idx) == FAILURE)
    {
      return FAILURE;
    }
...
)
{
  ...
  */
  if(ordered_config_ptr->rrc_state_indicator == RRC_STATE_CELL_DCH)
  {
     ...
     * Note: The index should already be there.
     */
    trch_idx = rrcllc_find_dl_dch_trch_idx(wrrc_as_id,(tr_ch_id_type)ie_ptr->dl_transportChannelIdentity);
    ...
    if(rrcllc_restore_dl_dch_rb_mapping_info(wrrc_as_id,trch_idx) == FAILURE)
    {
      /* Validation was done prior to adding it to the Standalone RB-Mapping backup info */
    ...
    * to come from an uplink TrCH ID.*/
    switch(ie_ptr->tfs_SignallingMode.t)
    {
      ...
      case T_rrc_DL_AddReconfTransChInformation2_tfs_SignallingMode_explicit_config:
        /* get the Transport Format Set for this TrCH */
        if(get_dl_transport_format_set(wrrc_as_id,ie_ptr->tfs_SignallingMode.u.explicit_config,
        ...
        */
        while(ul_ptr != NULL && ul_ptr->n > idx)
        {
          ...
          { 
            ...
            if(get_dl_transport_format_set(wrrc_as_id,&ul_ptr->elem[idx].transportFormatSet,
                                           &ordered_config_ptr->dl_trch_info[trch_idx],  /* dedicated trch */
                                           multiple_ded_log_chan, /* mulitple logical channels? */
                                           &ordered_config_ptr->dl_trch_info[trch_idx]   /* common trch */
                                          ) == FAILURE)
        ...
        if(!got_one)
        {
           ...
           * Note: The index should already be there.
           */
          ul_trch_idx = rrcllc_find_ul_dch_trch_idx(wrrc_as_id,
              ordered_config_ptr,
    ...
    /*Find Rbs mapped to this DCH and fill them in rb_list_mapped_to_dch if their RLC
 PDU size has changed.*/ 
    rrcllc_get_rb_for_rb_list_mapped_to_dch(wrrc_as_id,(tr_ch_id_type)ie_ptr->dl_transportChannelIdentity, 
  ...
  {
    WRRC_ERR0_FATAL("Something special happened(Bad)");
  }
...
)
{
  ...
  */
  if(ordered_config_ptr->rrc_state_indicator == RRC_STATE_CELL_DCH)
  {
     ...
     * Note: The index should already be there.
     */
    trch_idx = rrcllc_find_dl_dch_trch_idx(wrrc_as_id,(tr_ch_id_type)ie_ptr->transportChannelIdentity);
    if(trch_idx >= UE_MAX_TRCH)
    {
      WRRC_MSG1_ERROR("DL DCH TrCH ID %d, no idx found",
    ...
    */
    ...
    /*Find Rbs mapped to this DCH and fill them in rb_list_mapped_to_dch if their RLC
 PDU size has changed.*/  
    rrcllc_get_rb_for_rb_list_mapped_to_dch(wrrc_as_id,(tr_ch_id_type)ie_ptr->transportChannelIdentity, 
              ordered_config_ptr->rrc_state_indicator, DL); 
  } /* end CELL_DCH check */
...
void  rrcllc_copy_dl_dch_trch_info_to_buffer(sys_modem_as_id_e_type wrrc_as_id,uint32 trch_idx)
{
  ...
  /* Copy MAC data next */
  (void)rrcllc_copy_dl_dch_mac_info(wrrc_as_id,&dl_dch_info_buffer.mac_info, 
...
void  rrcllc_copy_dl_dch_trch_info_from_buffer(sys_modem_as_id_e_type wrrc_as_id,uint32 trch_idx)
{
  ...
  /* Copy MAC data next */
  (void)rrcllc_copy_dl_dch_mac_info(wrrc_as_id,&ordered_config_ptr->dl_dch_info[trch_idx],
...
)
{
  ...
  rrcllc_init_dl_ctfcs(wrrc_as_id,ordered_config_ptr);
  ...
  for(ix = 0; ix < ordered_config_ptr->ul_tfcs.ntfc; ix++)
  {
    ...
    WRRC_MSG3_HIGH("ULTFC No %d,Flag [0:A,1NA,2M, 3RM] %d CTFC Value %d ", ix,
...
)
{
  ...
  /* Copy MAC data next */
  (void)rrcllc_copy_dl_dch_mac_info(wrrc_as_id,&ordered_config_ptr->dl_dch_info[dest_trch_idx], 
...
SIDE EFFECTS:
=============================================================================*/
static void  rrcllc_sort_dl_dch_trch_index_array(sys_modem_as_id_e_type wrrc_as_id,
...
)
{
  ...
  {
    WRRC_ERR2_FATAL("num_idx %d greater than UE_MAX_TRCH %d",num_index, UE_MAX_TRCH);
  }
  ...
  for(index_for_sorting = 1; index_for_sorting < num_index; index_for_sorting ++)
  {
       ...
       ( dch_trch_info_ptr[index_for_sorting].trch_id != 0))
    {
       ...
       * data at current sorted index.
       */
      rrcllc_copy_dl_dch_trch_info_to_buffer(wrrc_as_id,index_for_sorting);
      to_be_compared_index = index_for_sorting;
           ...
           )
      {
        ...
        rrcllc_copy_dl_dch_trch_info_from_one_to_next_index(wrrc_as_id,
                                     to_be_compared_index,
                                     to_be_compared_index-1);
        to_be_compared_index --;
      }
  
      rrcllc_copy_dl_dch_trch_info_from_buffer(wrrc_as_id,to_be_compared_index);
    }
  } /* for(...) */
...
)
{
  ...
  while((dl_trch_delete_ptr != NULL) && (dl_trch_delete_ptr->n > idx))
  {
     ...
     */
    if(get_dl_deleted_trch(wrrc_as_id,&dl_trch_delete_ptr->elem[idx]) == FAILURE)
    {
      return FAILURE;
    }
     ...
     * UE needs to reject the message 
     */
    rrcllcoc_int_data.dl_tfcs_needed = TRUE;
    /* get next TrCH Info */
    idx++;
  } /* end while */
  idx =0;
  ...
  while((dl_trch_add_reconfig_ptr !=NULL) && (dl_trch_add_reconfig_ptr->n > idx) )
  {
    ...
    if((rrcllc_get_dl_dch_trch_idx(wrrc_as_id,trch_id)) == RRCLLC_INVALID_TRCH_IDX)
    {
   ...
   */
  rrcllc_fill_holes_in_dl_dch_trch_idx(wrrc_as_id);

  /* Now is the time and place for sorting all transport channel
   * info based on TrCH IDs. Call the right function.
   */

  rrcllc_sort_dl_dch_trch_index_array(wrrc_as_id,&ordered_config_ptr->dl_dch_trch_info[0], 
                                     ordered_config_ptr->mac_dl_parms.num_dchs
                                       );
  return(SUCCESS);
} /* rrcllc_add_delete_and_arrange_dl_dch_trch_info */
...
)
{
  ...
  {
    ix = rrcllc_find_dl_dch_trch_idx(wrrc_as_id,(tr_ch_id_type)ie_ptr->dl_TransportChannelType.u.dch);
    if(ix >= UE_MAX_TRCH)
    {
      WRRC_MSG1_HIGH("No DL DCH index for TrCH ID %d",
    ...
    rrc_DL_TrCH_TypeId2_r5_hsdsch))
  {
    ...
    if ((ix = rrcllc_find_dflow_index(wrrc_as_id,(uint8)ie_ptr->dl_TransportChannelType.u.hsdsch)) >= UE_MAX_MAC_D_FLOW)
    {
    ...
    */
    ...
    else
    {
      ...
      {
        rrcllc_free_hanging_rb_mapping_info_index(wrrc_as_id,ordered_config_ptr->mac_dflow_info[ix].dlchan_info[i].rb_id, MAC_D_RB_MAPPING);
      }
    }
...
SIDE EFFECTS:
=============================================================================*/
static void  rrcllc_copy_mac_ul_dch_mapping_info(sys_modem_as_id_e_type wrrc_as_id,
...
SIDE EFFECTS:
=============================================================================*/
static void rrcllc_copy_mac_ul_dch_tfs_info(sys_modem_as_id_e_type wrrc_as_id,
      ...
      )      
{
  ...
  /* Copy MAC data next */
  rrcllc_copy_mac_ul_dch_mapping_info(wrrc_as_id,&ordered_config_ptr->ul_cctrch.dch_info.dch_info[dest_trch_idx],
                                      &ordered_config_ptr->ul_cctrch.dch_info.dch_info[src_trch_idx]);

  rrcllc_copy_mac_ul_dch_tfs_info(wrrc_as_id,&ordered_config_ptr->ul_tfcs.tfs_info[dest_trch_idx], 
...
)
{
  ...
  {
    /* Clear the RB Mapping info (MAC) for this TrCH */
    rrcllc_init_ul_dch_rb_mapping_info(wrrc_as_id,ordered_config_ptr, trch_idx);

    /* Clear the TFS (L1) for this TrCH */
    rrcllc_init_ul_tfs_table(wrrc_as_id,ordered_config_ptr, trch_idx);
    ...
    /* Free the TrCH index for further use */
    if(rrcllc_deallocate_ul_dch_trch_idx(wrrc_as_id,trch_idx) == FAILURE)
    {
      return FAILURE;
    }
...
void  rrcllc_copy_ul_dch_trch_info_to_buffer(sys_modem_as_id_e_type wrrc_as_id,uint32 trch_idx)
{
  ...
  /* Copy MAC data next */
  rrcllc_copy_mac_ul_dch_mapping_info(wrrc_as_id,&ul_dch_info_buffer.mac_dch_info,
                                      &ordered_config_ptr->ul_cctrch.dch_info.dch_info[trch_idx]);

  rrcllc_copy_mac_ul_dch_tfs_info(wrrc_as_id,&ul_dch_info_buffer.mac_tfs_info, 
...
void  rrcllc_copy_ul_dch_trch_info_from_buffer(sys_modem_as_id_e_type wrrc_as_id,uint32 trch_idx)
{
  ...
  /* Copy MAC data next */
  rrcllc_copy_mac_ul_dch_mapping_info(wrrc_as_id,&ordered_config_ptr->ul_cctrch.dch_info.dch_info[trch_idx],
                                      &ul_dch_info_buffer.mac_dch_info);

  rrcllc_copy_mac_ul_dch_tfs_info(wrrc_as_id,&ordered_config_ptr->ul_tfcs.tfs_info[trch_idx],
...
SIDE EFFECTS:
=============================================================================*/
static void  rrcllc_sort_ul_dch_trch_index_array(sys_modem_as_id_e_type wrrc_as_id,
  rrcllcoc_trch_info_type *dch_trch_info_ptr, 
  uint32  num_index
)
{
  ...
  for(index_for_sorting = 1; index_for_sorting < num_index; index_for_sorting ++)
  {
        ...
        dch_trch_info_ptr[index_for_sorting-1].trch_id)
    {
       ...
       * data at current sorted index.
       */
      rrcllc_copy_ul_dch_trch_info_to_buffer(wrrc_as_id,index_for_sorting);
      ...
      {
         rrcllc_copy_ul_dch_trch_info_from_one_to_next_index(wrrc_as_id,
                                       to_be_compared_index, 
                                       to_be_compared_index-1);
        to_be_compared_index --;
      }
  
      rrcllc_copy_ul_dch_trch_info_from_buffer(wrrc_as_id,to_be_compared_index);
    }
  } /* for(...) */
...
)
{
  ...
  while((ul_trch_delete_ptr !=NULL )&& (ul_trch_delete_ptr->n > idx))
  {
     ...
     * and update transport channel count in L1, MAC and local data.
     */
    if(get_ul_deleted_trch(wrrc_as_id,&ul_trch_delete_ptr->elem[idx]) == FAILURE)
    {
      return FAILURE;
    }
    /* set ul_tfcs_needed flag to true.  Later if common info is not present,
  ...
  while((ul_trch_add_reconfig_ptr != NULL ) && (ul_trch_add_reconfig_ptr->n > idx) )
  {
    ...
    if((rrcllc_get_ul_dch_trch_idx(wrrc_as_id,trch_id)) == RRCLLC_INVALID_TRCH_IDX)
    {
      WRRC_MSG1_ERROR("Couldn't find idx, TrCH ID %d", trch_id);
      return(FAILURE);
    }
   ...
   */
  rrcllc_fill_holes_in_ul_dch_trch_idx(wrrc_as_id);

  /* Now is the time and place for sorting all transport channel
   * info based on TrCH IDs. Call the right function.
   */
  rrcllc_sort_ul_dch_trch_index_array(wrrc_as_id,&ordered_config_ptr->ul_dch_trch_info[0], 
...
)
{
  ...
  if(rel_ver == MSG_REL99)
  {
    ...
    ||(rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL10 && rel_ver == MSG_REL10)
  ...
  case T_rrc_TFC_Subset_minimumAllowedTFC_Number:
    ...
    {
      WRRC_MSG3_ERROR("Allowed TFC number=%d exceeds UE's capability=%d"
    ...
    case T_rrc_TFC_Subset_restrictedTrChInfoList:
      if(tfc_subset_ptr->u.restrictedTrChInfoList != NULL)
      {
            ...
            while(i < UE_MAX_TRCH)
            {
              if(restricted_trch_tfc_ptr->elem[idx].restrictedTrChIdentity == ordered_config_ptr->ul_dch_trch_info[i].trch_id)
              {
                if(ordered_config_ptr->ul_dch_trch_info[i].dch_present == TRUE)
                {
                  ...
                  tr_ch_indx[no_of_tr_ch_ids] = rrcllc_find_ul_dch_trch_idx(wrrc_as_id,ordered_config_ptr, ordered_config_ptr->ul_dch_trch_info[i].trch_id);
                  ...
                  WRRC_MSG3_HIGH("Index=%d, DCH Index=%d, Restricting Transport Channel ID=%d", i, ordered_config_ptr->ul_dch_trch_idx_in_use[i], tr_ch_indx[no_of_tr_ch_ids]);
        ...
        for(j=0;j<ordered_config_ptr->ul_tfcs.ntfc && (j < UE_MAX_TFC);j++)
        {
          ...
          {
            if(check_if_tfi_present_in_allowed_list(wrrc_as_id,restricted_trch_tfc_ptr,j,k)
                 == FAILURE)
            {
              tfci_allowed = FALSE;
              break;
            ...
            while(i < UE_MAX_TRCH)
            {
              if(restricted_trch_tfc_ptr->elem[idy].restrictedTrChIdentity == ordered_config_ptr->ul_dch_trch_info[i].trch_id)
              {
                if(ordered_config_ptr->ul_dch_trch_info[i].dch_present == TRUE)
                {
                  ...
                  if(rrcllc_find_ul_dch_trch_idx(wrrc_as_id, ordered_config_ptr, 
...
/*============================================================================
FUNCTION: rrcllc_add_delete_and_arrange_dl_dch_trch_info2(sys_modem_as_id_e_type wrrc_as_id)
...
)
{
  ...
  * and update transport channel count in L1, MAC and local data.
    */
    if(get_dl_deleted_trch_r5(wrrc_as_id,&dl_trch_delete_ptr->elem[idx]) == FAILURE)
    {
      return FAILURE;
    }
    /* get next TrCH Info */
  ...
  while((dl_trch_add_reconfig_ptr !=NULL) && (dl_trch_add_reconfig_ptr->n > idx) )
  {
    ...
    dflow_queue_head_ptr->elem[idy].mac_dFlowId,
          ...
          if ((dflow_index = rrcllc_get_dflow_index(wrrc_as_id,
  ...
  rrcllc_rearrange_dflow_index(wrrc_as_id);
      ...
      (ordered_config_ptr->mac_dflow_info[cnt].mac_dflow_id != INVALID_DFLOW_ID))
    {
      ...
      for (queue_cnt = 0; queue_cnt < ordered_config_ptr->mac_dflow_info[cnt].no_of_queues; queue_cnt++)
      {
        if (ordered_config_ptr->mac_dflow_info[cnt].queue_info[queue_cnt].queue_id == deleted_queue_id)
        {
          ...
          WRRC_MSG3_HIGH("RRCHS :Queue %d del from Dflow %d No. of queues after deleting",  deleted_queue_id,
...
)
{
  ...
  WRRC_MSG3_HIGH("RRCHS:DFLOWId %d, Queue %d, Exists already[Yes/No] %d", 
  ...
  if (queue_already_present)
  {
      ...
      (rrc_queue_info_ptr->mac_d_PDU_SizeInfo_List.n >0))
    {
      ...
      while ((pdu_list_ptr->n >idx) && (pdu_size_count < UE_MAX_HS_NO_OF_PDU_SIZE_PER_QUEUE)) 
      {
        ...
        pdu_size_count++)
        {
          ...
          {
            WRRC_MSG5_HIGH("RRCHS:PDU Index %d existing in queue %d, Overwrite PDUSize %d with %d,PduIdx %d",
              pdu_list_ptr->elem[idx].mac_d_PDU_Index,
              rrc_queue_info_ptr->mac_hsQueueId,
        ...
        if (pdu_index_present == FALSE)
        {
              ...
              (UE_MAX_HS_NO_OF_PDU_SIZE_PER_QUEUE-1))
          {
            ...
            WRRC_MSG3_HIGH("RRCHS:PDU size space available Set PDU index %d, Size %d No of PDU Sizes %d",
                       pdu_list_ptr->elem[idx].mac_d_PDU_Index,
                       pdu_list_ptr->elem[idx].mac_d_PDU_Size,
                     ordered_config_ptr->mac_dflow_info[dflow_index].queue_info[queue_index].no_of_pdu_sizes);
          }
    ...
    ordered_config_ptr->mac_dflow_info[dflow_index].queue_info[queue_index].window_size = 
     ...
     get_hs_window_size(wrrc_as_id,rrc_queue_info_ptr->mac_hsWindowSize);

    ordered_config_ptr->mac_dflow_info[dflow_index].queue_info[queue_index].release_timer = 
    
     get_hs_release_timer(wrrc_as_id,rrc_queue_info_ptr->reorderingReleaseTimer);
  ...
  else
  {
    ...
    while ((pdu_list_ptr->n > idx) && (pdu_size_count < UE_MAX_HS_NO_OF_PDU_SIZE_PER_QUEUE)) 
    {
      WRRC_MSG4_HIGH("RRCHS:DFLOWId %d, PDUIndex %d, Size %d set in Queue %d", 
        rrc_queue_info_ptr->mac_dFlowId,
        pdu_list_ptr->elem[idx].mac_d_PDU_Index,
        pdu_list_ptr->elem[idx].mac_d_PDU_Size,
        rrc_queue_info_ptr->mac_hsQueueId);
    ...
    WRRC_MSG3_HIGH("RRCHS:No PduSizes %d, Queue %d No of queues are %d", pdu_size_count, rrc_queue_info_ptr->mac_hsQueueId, ordered_config_ptr->mac_dflow_info[dflow_index].no_of_queues);
...
SIDE EFFECTS:
=============================================================================*/
static uecomdef_status_e_type set_dflow_info(  sys_modem_as_id_e_type wrrc_as_id,rrc_AddOrReconfMAC_dFlow * rrc_hsdsch)
{
    uint8 dflow_index;
    ...
    (rrc_hsdsch->mac_hs_DelQueue_List.n > 0))
  {
    ...
    while (deleted_queue_list_ptr->n > idx)
    {
      ...
      if (rrcllc_delete_queue_from_dflow(wrrc_as_id,deleted_queue_list_ptr->elem[idx].mac_hsQueueId) == SUCCESS)
      {
        idx++;
      }
      else
    ...
    rrcllc_rearrange_queue_index(wrrc_as_id);
  }
  ...
  while (queue_dflow_head_ptr->n > idx)
  {
    ...
    if ((dflow_index = rrcllc_find_dflow_index(wrrc_as_id,(uint8)queue_dflow_head_ptr->elem[idx].mac_dFlowId)) == UE_MAX_MAC_D_FLOW)
    {
      WRRC_MSG1_ERROR("RRCHS:Index not found for flow Id %d", queue_dflow_head_ptr->elem[idx].mac_dFlowId);
      return FAILURE;
    }
    if(rrcllc_restore_macd_rb_mapping_info(wrrc_as_id,dflow_index) ==FAILURE)
    {
      /* Validation was done prior to adding it to the Standalone RB-Mapping backup info */
    ...
    if ((queue_index = rrcllc_get_queue_index(wrrc_as_id,dflow_index, queue_dflow_head_ptr->elem[idx].mac_hsQueueId, &queue_present)) == UE_MAX_HS_QUEUE)
    {
    ...
    if (set_queue_info(wrrc_as_id,dflow_index, queue_index, &queue_dflow_head_ptr->elem[idx], queue_present) == FAILURE)
    {
    ...
    {
      rrcllc_remove_dflow_queue_mapping(wrrc_as_id,&queue_dflow_head_ptr->elem[idx]);
    }
...
)
{
     ...
     &&(mac_ehs_info->mac_ehs_AddReconfQueue_List.n > 0))
  {
    ...
    while (macehs_queue_head_ptr->n > idx)
    {
      ...
      if ((macehs_q_id = rrcllc_get_macehs_queue_index(wrrc_as_id,(uint8)macehs_queue_head_ptr->elem[idx].mac_ehs_QueueId)) 
        == UE_MAX_HS_QUEUE)
      {
        return FAILURE;
      }
      else
      {
        ...
        ordered_config_ptr->mac_ehs_queue_info[macehs_q_id].window_size = 
          get_hs_window_size(wrrc_as_id,macehs_queue_head_ptr->elem[idx].mac_ehsWindowSize);
        
        ordered_config_ptr->mac_ehs_queue_info[macehs_q_id].release_timer = 
          get_hs_release_timer(wrrc_as_id,macehs_queue_head_ptr->elem[idx].reorderingReleaseTimer);
...
{  
  ...
  if ( (RRC_MSG_COMMON_BITMASK_IE_TYPE2_PTR(mac_ehs_info,
    mac_ehs_AddReconfQueue_List)) &&
      (mac_ehs_info->mac_ehs_AddReconfQueue_List.n > 0))
  {
    if(rrcllc_add_macehs(wrrc_as_id,mac_ehs_info) == FAILURE)
    {
      return FAILURE;
    }
  }
...
)
{
  ...
  if(config_ptr->mac_dl_parms.mac_hs_or_ehs == MAC_EHS)
  {
    for(idx = 0; idx < config_ptr->mac_dl_parms.efach_ndlchans; idx++)
    {
      ...
      WRRC_MSG2_MED("EFACH:Queue %d mapped to LCH %d",config_ptr->efach_dl_logchan_info[idx].queue_id,
...
)
{
  ...
  /* Get the transport channel index for this transport channel ID.
    ...
    if (RRC_CHECK_COMMON_MSG_TYPE(ie_ptr->dl_TransportChannelType,rrc_DL_TrCH_TypeId1_r5_dch))
      {
        ...
        trch_idx = rrcllc_find_dl_dch_trch_idx(wrrc_as_id,(tr_ch_id_type)ie_ptr->dl_TransportChannelType.u.dch);
    ...
    */
    ...
    rrc,addOrReconfMAC_dFlow) )
      {
        if (set_dflow_info(wrrc_as_id,&ie_ptr->tfs_SignallingMode.u.hsdsch->addOrReconfMAC_dFlow) == FAILURE)
        {
    ...
    rrc_HSDSCH_Info,harqInfo))
      {
        ...
        if (set_harq_information(wrrc_as_id,ordered_config_ptr,&ie_ptr->tfs_SignallingMode.u.hsdsch->harqInfo) == FAILURE)
        {
 ...
 PDU size has changed.*/ 
  ...
  rrc_DL_TrCH_TypeId1_r5_dch))
    {
      rrcllc_get_rb_for_rb_list_mapped_to_dch(wrrc_as_id,(tr_ch_id_type)ie_ptr->dl_TransportChannelType.u.dch, 
                ordered_config_ptr->rrc_state_indicator, DL);  
    }  
  } /* end CELL_DCH check */
...
===============================================================================================*/
static void rrcllc_copy_pdu_info_per_queue(sys_modem_as_id_e_type wrrc_as_id,uint8 dflow_index, uint8 queue_index)
{
  uint8 index = 0;
...
===============================================================================================*/
static boolean rrcllc_check_if_queue_exists(sys_modem_as_id_e_type wrrc_as_id,uint8 queue_id)
{
  uint8 index = 0;
...
)
{
  ...
  rrcllc_init_hsdpa_l1_queue(wrrc_as_id,ordered_config_ptr);
    ...
    && (queue_cnt < UE_MAX_HS_QUEUE); queue_cnt++)
      {
        if (ordered_config_ptr->mac_dflow_info[cnt].queue_info[queue_cnt].queue_id != INVALID_QUEUE_ID)
        {
          ...
          if (rrcllc_check_if_queue_exists(wrrc_as_id,ordered_config_ptr->mac_dflow_info[cnt].queue_info[queue_cnt].
            queue_id) == TRUE)
          {
            continue;
          }
          /* Call a function to store this queue value in L1 structure */
          rrcllc_copy_pdu_info_per_queue(wrrc_as_id,cnt, queue_cnt);
...
)
{
  ...
  if ((ul_deleted_trch_info_list_present) && (ul_deleted_trch_list_ptr != NULL))
  {
    while (ul_deleted_trch_list_ptr->n >idx)
    {
       ...
       * and update transport channel count in L1, MAC and local data.
       */
      if(get_ul_deleted_trch_info_list_r6(wrrc_as_id,&ul_deleted_trch_list_ptr->elem[idx]) == FAILURE)
      {
        return FAILURE;
      }
      /* get next TrCH Info */
    ...
    while(ul_deleted_trch_list_ptr->n > idx) 
    {
      ...
      {
        rrcllc_fill_holes_in_e_mac_d_flow_idx(wrrc_as_id);
        break;
      }
      idx++;
    }
  ...
  if ((ul_add_recfg_trch_info_list_present) && (ul_trch_add_reconfig_ptr != NULL))
  {
    ...
    while(ul_trch_add_reconfig_ptr->n >idx ) 
    {
               ...
               (ul_trch_add_reconfig_ptr->elem[idx].u.e_dch->m.addReconf_MAC_d_FlowListPresent)) 
      {
        ...
        {
          if((rrcllc_get_e_mac_dflow_idx(wrrc_as_id,(uint8)e_dch_mac_d_flow_list_ptr->elem[idy].
...
)
{
  ...
  {
    if(get_ul_info_common_for_all_trch_r5(wrrc_as_id,ie_ptr,
...
)
{
  ...
  if(temp_tti == E_TTI_2MS)
  {
     ...
     */
    if(
       (rrcnv_data.wcdma_nv_hsupa_category == 0)||
       (rrcnv_data.wcdma_nv_hsupa_category == 1)||
       (rrcnv_data.wcdma_nv_hsupa_category == 3)||
       (rrcnv_data.wcdma_nv_hsupa_category == 5)
      )
    {
      WRRC_MSG1_ERROR("RRCEUL:Error N/W try to config 2msec TTI when UE cat = %d which is not 2 or 4 or 6", 
        rrcnv_data.wcdma_nv_hsupa_category);
      return(FAILURE);
    }
  }
  ...
  */
  ...
  {
    rrcllc_update_mac_eul_action(wrrc_as_id, TRUE);
    rrcllc_update_l1_e_req_mask_for_e_dl_info(wrrc_as_id, TRUE);
    rrcllc_update_l1_e_req_mask_for_e_ul_info(wrrc_as_id, TRUE);
     ...
     */
    rrcllc_update_e_dl_channel_mask_for_e_active_set_rl_to_reconfig(wrrc_as_id);
  ...
  {       
    ...
    if(e_mac_d_flow_list_ptr != NULL)
    {
      ...
      {       
        if ((e_mac_d_flow_idx = rrcllc_find_e_mac_d_flow_idx(wrrc_as_id,
        ...
        {
          if(rrcllc_restore_ul_emac_rb_mapping_info(wrrc_as_id,e_mac_d_flow_idx) == FAILURE)
          {
            /* Validation was done prior to adding it to the Standalone RB-Mapping backup info */
        ...
        rrc_E_DCH_AddReconf_MAC_d_Flow,mac_d_FlowMultiplexingList))
          {
            ...
            ordered_config_ptr->mac_e_config.e_mac_d_flow_info[e_mac_d_flow_idx].e_mac_d_flow_mux_list =
              rrcllcpcie_convert_asn1_oss_bitstring_to_uint8(wrrc_as_id,
...
)
{
  ...
  if (RRC_CHECK_COMMON_MSG_TYPE_PTR(ie_ptr,rrc_UL_AddReconfTransChInformation_r6_dch_usch))
  {
    ...
    if(rrcllc_dch_usch_add_trch_info(wrrc_as_id,(tr_ch_id_type)ie_ptr->u.dch_usch->transportChannelIdentity,
  ...
  {
    if (FAILURE == process_added_reconfig_e_dch_info(wrrc_as_id,ie_ptr))
    {
      return FAILURE;
    }
  }
...
)
{
  if(ul_add_recfg_trch_info_list_present)
  {
    ...
    {
      if(get_ul_added_reconfig_trch_r6(wrrc_as_id,&ul_trch_add_reconfig_ptr->elem[idx]) == FAILURE)
      {
        return FAILURE;
      }
  ...
  if(rrcllc_check_ul_tfs_cnt(wrrc_as_id) == FAILURE)
  {
    return FAILURE;
  }
        ...
        while(ul_ptr != NULL && ul_ptr->n > idx)
        {
          ...
          { 
            ...
            WRRC_MSG2_MED("Get TFS for DL TrCH Id %d using UL TrCH ID %d", 
              ie_ptr->tfs_SignallingMode.u.sameAsULTrCH->ul_TransportChannelIdentity,
              ul_ptr->elem[idx].u.dch_usch->transportChannelIdentity);
            if(get_dl_transport_format_set(wrrc_as_id,&ul_ptr->elem[idx].u.dch_usch->transportFormatSet,
              &ordered_config_ptr->dl_trch_info[trch_idx],  /* dedicated trch */
              multiple_ded_log_chan, /* mulitple logical channels? */
              &ordered_config_ptr->dl_trch_info[trch_idx]   /* common trch */
              ) == FAILURE)
...
)
{
  ...
  if(dl_add_reconf_trch_info_list_present)
  {
    ...
    {
      if(get_dl_added_reconfig_trch_r6(wrrc_as_id,&dl_trch_add_reconfig_ptr->elem[idx],
  ...
  increases from 1 to 2. Needs to adjsut the tb_szie to
      ...
      ordered_config_ptr->dl_dch_trch_info[idx].lg_ch_cnt_down))
    {
      ...
      if (!(dl_add_reconf_trch_info_list_present &&
        find_trch_id_in_config_trch_list_r5(wrrc_as_id,dl_trch_add_reconfig_ptr,
  ...
  if(rrcllc_check_dl_tfs_cnt(wrrc_as_id) == FAILURE)
  {
    return FAILURE;
  }
...
)
{
  ...
  /* DL FACH */
    ...
    if(tmp_config_ptr->dl_fach_trch_idx_in_use[trch_idx] == TRUE)
    {
      ...
      {
         /* Clear the RB Mapping info for this TrCH */
         rrcllc_init_fach_rb_mapping_info(wrrc_as_id,tmp_config_ptr, trch_idx);
        
         /* Clear the TFS for this TrCH */
         rrcllc_init_dl_tfs_table(wrrc_as_id,tmp_config_ptr, trch_idx);
   ...
   /* DL DCH */
       ...
       (tmp_config_ptr->dl_dch_trch_idx_in_use[trch_idx] == TRUE))
    {
      if(pre_ndlchan == 0)
      {
         ...
         /* Clear the RB Mapping info for this TrCH */
          rrcllc_init_dl_dch_rb_mapping_info(wrrc_as_id,tmp_config_ptr, trch_idx);
        ...
        (tmp_config_ptr->ul_dch_trch_idx_in_use[trch_idx] == TRUE))
    {
      if(pre_ndlchan == 0)
      {
         ...
         rrcllc_init_ul_dch_rb_mapping_info(wrrc_as_id,tmp_config_ptr, trch_idx);
   
   
        /* Clear the TFS for this TrCH */
        rrcllc_init_ul_tfs_table(wrrc_as_id,tmp_config_ptr, trch_idx);
...
#ifdef FEATURE_RRC_TFC_RLC_SIZE_FOR_DCH
        /* Remove the RLC size restriction info */
        rrcllc_init_dch_rlc_size_info(wrrc_as_id,trch_idx);
#endif
  ...
  for(trch_idx = 0; trch_idx < MAX_E_DCH_MAC_D_FLOW; trch_idx++)
  {
    ...
    if(tmp_config_ptr->mac_e_config.e_mac_d_flow_info[trch_idx].e_mac_d_flow_id != INVALID_DFLOW_ID)
    {
      if(pre_ndlchan ==0 )
      {
        ...
        rrcllc_init_e_dch_rbmapping_info(wrrc_as_id,tmp_config_ptr, trch_idx);
  ...
  /* Now is the time and place for sorting all transport channel
    * info based on TrCH IDs. Call the right function.
    */
  rrcllc_sort_ul_dch_trch_index_array(wrrc_as_id,&tmp_config_ptr->ul_dch_trch_info[0], 
...
void rrcllc_get_rb_id_list(sys_modem_as_id_e_type wrrc_as_id, uint8 *rb_id_list, rrc_RB_InformationReleaseList  *rb_release_ptr)
{
  uint8 index;
  uint8 rb_id = RRC_INVALID_RB_ID;
  ...
  {
    rb_id = rrclcm_get_dl_rb_id(wrrc_as_id,ordered_config_ptr->rlc_dl_tm_parms.chan_info[index].lc_id);
    if((rb_id > MAX_RB) || (rb_id <= CCCH_RADIO_BEARER_ID))
      continue;
    rb_id_list[rb_id] = TRUE;
  }
...
)
{
  uint8 cnt,trch_idx;
  uint8 index;
  boolean rb_ul[MAX_RB+1]={FALSE},rb_dl[MAX_RB+1]={FALSE};

  rrcllc_get_rb_id_list(wrrc_as_id,rb_dl,rb_release_ptr);
  WCDMA_MEMCPY(rb_ul,
               sizeof(rb_ul),
               rb_dl,
               sizeof(rb_dl));
  ...
  for(trch_idx=0;trch_idx< UE_MAX_TRCH;trch_idx++)
  {
        ...
        (ordered_config_ptr->dl_dch_trch_idx_in_use[trch_idx] == TRUE) )
    {
      ...
      {
        if(ordered_config_ptr->dl_dch_info[trch_idx].dlchan_info[cnt].rb_id <= MAX_RB)
        {
        ...
        (ordered_config_ptr->ul_dch_trch_idx_in_use[trch_idx] == TRUE))
    {
      ...
      {
        if(ordered_config_ptr->ul_cctrch.dch_info.dch_info[trch_idx].dlchan_info[cnt].rb_id <= MAX_RB)
        {
  ...
  for(index = 0; index < UE_MAX_MAC_D_FLOW;  index++)
  {
       ...
       (ordered_config_ptr->mac_dflow_info[index].no_of_queues > 0))
    {
      ...
      {
        if(ordered_config_ptr->mac_dflow_info[index].dlchan_info[cnt].rb_id <= MAX_RB)
        {
  ...
  for(index = 0; index < UE_MAX_DL_LOGICAL_CHANNEL;  index++)
  {
    ...
    for (cnt = 0; cnt < UE_MAX_HS_QUEUE; cnt++) 
    {
      ...
      {
        if(ordered_config_ptr->dl_macehs_logchan_info[index].rb_id <= MAX_RB)
        {
    ...
    for(index = 0; index < ordered_config_ptr->mac_e_config.num_e_mac_d_flow;  index++)
    {
      for (cnt = 0; cnt < ordered_config_ptr->mac_e_config.e_mac_d_flow_info[index].num_logch; cnt++) 
      {
        ...
        )
        {
          ...
          {
            if(ordered_config_ptr->mac_e_config.e_mac_d_flow_info[index].e_ul_logch_info[cnt].rb_id <= MAX_RB)
            {
  ...
  for(index=0;index < (MAX_RB+1) ; index++)
  {
    if(rb_dl[index])
    {
         ...
         restricted_info->restriction_info.trch_restriction_info_list.trch_restriction_info[i].trch_id))
    {
      ...
      if(restricted_info->restriction_info.trch_restriction_info_list.trch_restriction_info[i].ntf)
      {
        ...
        while(j< restricted_info->restriction_info.trch_restriction_info_list.trch_restriction_info[i].ntf)
        {
          ...
          {
            WRRC_MSG3_HIGH("TFCC: Found the Allowed TF index %d for trch_idx %d in TFCI %d",j,trch_index,tfci_index);
            return SUCCESS;
          }
          j++;
        }
  ...
  switch(restricted_info->tfci_restriction_choice)
  {
    ...
    case RESTRICTED_TFCI:
      ...
      for(i=0;i<config_ptr->ul_tfcs.ntfc && (i < UE_MAX_TFC);i++)
      {
        ...
        {
          if(rrcllc_check_if_tfi_present_in_allowed_list_in_tfcc(wrrc_as_id,config_ptr,restricted_info,i,j)
               == FAILURE)
          {
            tfci_allowed = FALSE;
            break;
  ...
  */
  if(ordered_config_ptr->rrc_state_indicator == RRC_STATE_CELL_DCH)
  {
        ...
        while(ul_ptr_r7 != NULL && ul_ptr_r7->n > idx)
        {
          ...
          { 
            ...
            WRRC_MSG2_MED("Get TFS for DL TrCH Id %d using UL TrCH ID %d", 
              ie_ptr->tfs_SignallingMode.u.sameAsULTrCH->ul_TransportChannelIdentity,
              ul_ptr_r7->elem[idx].u.dch_usch->transportChannelIdentity);
            if(get_dl_transport_format_set(wrrc_as_id,&ul_ptr_r7->elem[idx].u.dch_usch->transportFormatSet,
              &ordered_config_ptr->dl_trch_info[trch_idx],  /* dedicated trch */
              multiple_ded_log_chan, /* mulitple logical channels? */
              &ordered_config_ptr->dl_trch_info[trch_idx]   /* common trch */
              ) == FAILURE)
   ...
   rrc,dl_MAC_HeaderType))
        {
          if (ie_ptr->tfs_SignallingMode.u.hsdsch->dl_MAC_HeaderType.t == T_rrc_HSDSCH_Info_r7_dl_MAC_HeaderType_mac_hs)
          {
            ...
            if (set_dflow_info(wrrc_as_id,ie_ptr->tfs_SignallingMode.u.hsdsch->dl_MAC_HeaderType.u.mac_hs) == FAILURE)
            {
              WRRC_MSG0_ERROR("RRCHS:Invalid DFLOW info");
              return FAILURE;
            }
   ...
   rrc_HSDSCH_Info_r7,harqInfo))
        {
          MSG_MED("RRCHS:HARQ info present",0,0,0);
          if (set_harq_information_r7(wrrc_as_id,&ie_ptr->tfs_SignallingMode.u.hsdsch->harqInfo) == FAILURE)
          {
            WRRC_MSG0_ERROR("RRCHS:Unable to set HARQ information");
            return FAILURE;
          }
...
)
{
  ...
  if(dl_add_reconf_trch_info_list_present)
  {
    ...
    while((idx <= UE_MAX_TRCH) && (dl_trch_add_reconfig_ptr->n > idx))
    {
      if(get_dl_added_reconfig_trch_r7( wrrc_as_id,&dl_trch_add_reconfig_ptr->elem[idx],
  ...
  increases from 1 to 2. Needs to adjsut the tb_szie to
      ...
      ordered_config_ptr->dl_dch_trch_info[idx].lg_ch_cnt_down))
    {
      /* TrCH IE is present and the TrCH ID is appear in the TrCH info table. */
      if (!(dl_add_reconf_trch_info_list_present &&
        find_trch_id_in_config_trch_list_r7(wrrc_as_id,dl_trch_add_reconfig_ptr,
        ordered_config_ptr->dl_dch_trch_info[idx].trch_id)))
      {
        /* Added MAC head size to the current TB size.  */
        if (ordered_config_ptr->dl_dch_trch_info[idx].lg_ch_cnt_up &&
...
)
{
  ...
  else if (RRC_CHECK_COMMON_MSG_TYPE(ie_ptr->dl_TransportChannelType,rrc_DL_TrCH_TypeId2_r7_hsdsch))
  {
    ...
    {
      if ((ix = rrcllc_find_dflow_index(wrrc_as_id,(uint8)ie_ptr->dl_TransportChannelType.u.hsdsch->u.mac_hs)) >= UE_MAX_MAC_D_FLOW)
      {
    ...
    else if(ie_ptr->dl_TransportChannelType.u.hsdsch->t == T_rrc_DL_TrCH_TypeId2_r7_hsdsch_mac_ehs )
    {
      ...
      if (rrcllc_delete_queue_from_macehs(wrrc_as_id,ordered_config_ptr, ie_ptr->dl_TransportChannelType.u.hsdsch->u.mac_ehs) == FAILURE)
      {
  ...
  {
    // ix = rrcllc_find_dl_fach_trch_idx(wrrc_as_id,ie_ptr->value.dl_TransportChannelIdentity); 
    ix = rrcllc_find_dl_fach_trch_idx(wrrc_as_id);
    if(ix == RRCLLC_INVALID_TRCH_IDX)
    {
      WRRC_MSG0_ERROR("No FACH index for TrCH ID");
...
)
{
  ...
  {
    if (FAILURE == process_added_reconfig_e_dch_info_r7( wrrc_as_id,ie_ptr))
    {
      return FAILURE;
    }
  }
...
)
{
  if(ul_add_recfg_trch_info_list_present)
  {
    ...
    while((idx <= UE_MAX_TRCH) && (ul_trch_add_reconfig_ptr->n > idx))
    {
      if(get_ul_added_reconfig_trch_r7(wrrc_as_id,&ul_trch_add_reconfig_ptr->elem[idx]) == FAILURE)
      {
        return FAILURE;
      }
...
)
{
  ...
  * and update transport channel count in L1, MAC and local data.
    */
    if(get_dl_deleted_trch_r7(wrrc_as_id,&dl_trch_delete_ptr->elem[idx]) == FAILURE)
    {
      return FAILURE;
    }
    /* get next TrCH Info */
    ...
    dl_trch_add_reconfig_ptr->n > idx) )
  {
     ...
     dl_MAC_HeaderType.u.mac_hs->m.mac_hs_AddReconfQueue_ListPresent) &&
            ...
            dl_MAC_HeaderType.u.mac_hs->mac_hs_AddReconfQueue_List.n > 0))
          {
            ...
            while (dflow_queue_head_ptr->n > idy)
            {
              ...
              /* If DFLOW Index found then return it Else allocate a new index */
              if ((dflow_index = rrcllc_get_dflow_index(wrrc_as_id,(uint8)dflow_queue_head_ptr->elem[idy].mac_dFlowId)) 
                == UE_MAX_MAC_D_FLOW)
              {
                WRRC_MSG1_HIGH("RRCHS:DFLOW index %d absent",dflow_index);
                return FAILURE;
   ...
   hsdsch->dl_MAC_HeaderType.t == T_rrc_DL_TrCH_TypeId2_r7_hsdsch_mac_ehs)
        {
        if(macehs_add_or_del_q_info(wrrc_as_id,dl_trch_add_reconfig_ptr->elem[idx].tfs_SignallingMode.u.hsdsch->dl_MAC_HeaderType.u.mac_ehs) == FAILURE)            
           return FAILURE;
        }
      }
    } /* End of if of rrc_DL_TrCH_TypeId1_r5_hsdsch chosen */
  ...
  /* Now Remove holes in DFLOW Index */
  rrcllc_rearrange_dflow_index(wrrc_as_id);
  rrcllc_rearrange_macehs_queue_index(wrrc_as_id,ordered_config_ptr);
  rrcllc_fill_holes_in_dl_dch_trch_idx(wrrc_as_id);
 ...
 )
{
  ...
  config_ptr = rrcllc_get_config_ptr_in_use(wrrc_as_id);
...
SIDE EFFECTS:
=============================================================================*/
tr_ch_id_type rrcllc_get_ctch_trch_id(sys_modem_as_id_e_type wrrc_as_id)
{
  uint32 trch_idx;
  ordered_config_type *config_ptr;
...
)
{
  if(ul_add_recfg_trch_info_list_present)
  {
    ...
    while((idx <= UE_MAX_TRCH) && (ul_trch_add_reconfig_ptr->n > idx))
    {
      if(get_ul_added_reconfig_trch_r8(wrrc_as_id,&ul_trch_add_reconfig_ptr->elem[idx]) == FAILURE)
      {
        return FAILURE;
      }
...
)
{
  ...
  {
    if (FAILURE == process_added_reconfig_e_dch_info_r8( wrrc_as_id,ie_ptr))
    {
      return FAILURE;
    }
  }
...
)
{
  ...
  if(dl_add_reconf_trch_info_list_present)
  {
    ...
    while((idx <= UE_MAX_TRCH) && (dl_trch_add_reconfig_ptr->n > idx))
    {
      if(get_dl_added_reconfig_trch_r8(wrrc_as_id,&dl_trch_add_reconfig_ptr->elem[idx],
          ul_trch_add_reconfig_ptr
        ) == FAILURE)
      {
        return FAILURE;
        ...
        while(ul_ptr_r8 != NULL && ul_ptr_r8->n > idx)
        {
          ...
          { 
            ...
            WRRC_MSG2_MED("Get TFS for DL TrCH Id %d using UL TrCH ID %d", 
              ie_ptr->tfs_SignallingMode.u.sameAsULTrCH->ul_TransportChannelIdentity,
              ul_ptr_r8->elem[idx].u.dch_usch->transportChannelIdentity);
            if(get_dl_transport_format_set(wrrc_as_id,&ul_ptr_r8->elem[idx].u.dch_usch->transportFormatSet,
              &ordered_config_ptr->dl_trch_info[trch_idx],  /* dedicated trch */
              multiple_ded_log_chan, /* mulitple logical channels? */
              &ordered_config_ptr->dl_trch_info[trch_idx]   /* common trch */
              ) == FAILURE)
     ...
     &&(mac_ehs_info->mac_ehs_AddReconfQueue_List.n > 0))
  {
    ...
    while (macehs_queue_head_ptr->n > idx)
    {
      ...
      else
      {
        ...
        /* End of while of pdu_list_ptr */       
        ordered_config_ptr->mac_ehs_queue_info[macehs_q_id].window_size = 
          get_hs_window_size_r9(wrrc_as_id,macehs_queue_head_ptr->elem[idx].mac_ehsWindowSize);
...
{  
  ...
  if ( (RRC_MSG_COMMON_BITMASK_IE_TYPE2_PTR(mac_ehs_info,
    mac_ehs_AddReconfQueue_List)) &&
      (mac_ehs_info->mac_ehs_AddReconfQueue_List.n > 0))
  {
    if(rrcllc_add_macehs_r9(wrrc_as_id,mac_ehs_info) == FAILURE)
    {
      return FAILURE;
    }
  }
...
)
{
  ...
  if(dl_add_reconf_trch_info_list_present)
  {
    ...
    while((idx <= UE_MAX_TRCH) && (dl_trch_add_reconfig_ptr->n > idx))
    {
      if(get_dl_added_reconfig_trch_r9(wrrc_as_id,&dl_trch_add_reconfig_ptr->elem[idx],
          ul_trch_add_reconfig_ptr
        ) == FAILURE)
      {
        return FAILURE;
  ...
  increases from 1 to 2. Needs to adjsut the tb_szie to
      ...
      ordered_config_ptr->dl_dch_trch_info[idx].lg_ch_cnt_down))
    {
      /* TrCH IE is present and the TrCH ID is appear in the TrCH info table. */
      if (!(dl_add_reconf_trch_info_list_present &&
        find_trch_id_in_config_trch_list_r9(wrrc_as_id,dl_trch_add_reconfig_ptr,
        ordered_config_ptr->dl_dch_trch_info[idx].trch_id)))
      {
        /* Added MAC head size to the current TB size.  */
        if (ordered_config_ptr->dl_dch_trch_info[idx].lg_ch_cnt_up &&
  ...
  * and update transport channel count in L1, MAC and local data.
    ...
    dl_trch_add_reconfig_ptr->n > idx) )
  {
   ...
   hsdsch->dl_MAC_HeaderType.t == T_rrc_HSDSCH_Info_r9_dl_MAC_HeaderType_mac_ehs)
        {
          if(macehs_add_or_del_q_info_r9(wrrc_as_id,dl_trch_add_reconfig_ptr->elem[idx].tfs_SignallingMode.u.hsdsch->dl_MAC_HeaderType.u.mac_ehs) == FAILURE)            
          {
             return FAILURE;
          }
        }
