...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcpcreconfig.c#3 $   
...
typedef struct
{
  ...
  boolean rrcpcrc_send_initiate_cu_flag;
  
  boolean rrcpcrc_delete_crnti;
...
LOCAL rrcpcrc_int_data_type pcrc_sim[MAX_RRC_STACKS];
#define pcrc pcrc_sim[WAS_ID_M]
#else
...
#endif
rrcrb_substate_e_type rrcpcrc_get_current_sub_state(sys_modem_as_id_e_type wrrc_as_id)
{
...
===========================================================================*/
rrc_state_e_type rrcpcrc_get_dest_state(sys_modem_as_id_e_type wrrc_as_id)
{
  return pcrc.state_indicated_by_msg;
}
/*===========================================================================

FUNCTION   rrcpcrc_get_initiate_cu_flag()

DESCRIPTION
 This function is used to return rrcpcrc_send_initiate_cu_flag 
...
===========================================================================*/
boolean rrcpcrc_get_initiate_cu_flag(sys_modem_as_id_e_type wrrc_as_id)
{
  return pcrc.rrcpcrc_send_initiate_cu_flag;
}
/*===========================================================================
...
===========================================================================*/
boolean rrcpcrc_is_proc_successful(sys_modem_as_id_e_type wrrc_as_id)
{
...
===========================================================================*/
void rrcpcrc_update_failure_status(sys_modem_as_id_e_type wrrc_as_id,rrcpc_msg_status_e_type status )
{
...
)
{
  ...
  (void)rrcsmc_append_start_list(wrrc_as_id,&msg_ptr->message.u.physicalChannelReconfigurationComplete.
...
)
{
  ...
  if (MSG_REL10 == msg_version)
  {
    rrc_PhysicalChannelReconfiguration_r10_IEs *pcreconfig_ptr = &msg_ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.
    criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.physicalChannelReconfiguration_r10;

    if (RRCRB_R10_MSG_IE_PRESENT(pcreconfig_ptr,dl_InformationPerRL_List))
    {
      if(pcreconfig_ptr->dl_InformationPerRL_List.elem[0].modeSpecificInfo.t ==
      T_rrc_DL_InformationPerRL_r8_modeSpecificInfo_fdd)
      {
        rrcpcrc_psc =   pcreconfig_ptr->dl_InformationPerRL_List.elem[0].modeSpecificInfo.
        u.fdd->primaryCPICH_Info.primaryScramblingCode;
        result = TRUE;
      }
    }
  ...
  WRRC_MSG3_HIGH("Primary SCR in PCRC message %d result:%d for MSG_VERSION: %d(0-MSG_REL99,1-MSG_REL5,2-MSG_REL6,3-MSG_REL7,4-MSG_REL8,5-MSG_REL9,6-MSG_REL10,other-MSG_INVALID)",rrcpcrc_psc,result,msg_version);
  return result;
...
)
{
  ...
  /*Reports Physical Channel Reconfiguration failure to QSH*/
  rrc_qsh_report_failure_ota_msg_event(wrrc_as_id,WRRC_UL_DCCH_MESSAGETYPE_PHYSICALCHANNELRECONFIGURATIONFAILURE);
#endif

#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
  rrcqsh_log_uplink_dcch_ota_params(wrrc_as_id, WRRC_UL_DCCH_MESSAGETYPE_PHYSICALCHANNELRECONFIGURATIONFAILURE, ptr);
#endif
  ...
  if(ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
    ...
    {
      rrc_update_reconfig_waiting_for_l2ack_dch(wrrc_as_id, RRC_PROCEDURE_PCR);
      status = rrcsend_send_ul_sdu(wrrc_as_id,RRC_PROCEDURE_PCR,  /* Procedure is PCR    */
...
===========================================================================*/     
static void send_pcrc_complete_message(sys_modem_as_id_e_type wrrc_as_id,  boolean proc_wait_for_l2_ack)
{
  ...
  {
    rrcpcrc_append_start_list_to_pcrc_complete(wrrc_as_id, ptr);
  }
  else
  {
    if (rrcllc_get_rlc_size_change_status(wrrc_as_id) 
        && ((pcrc.state_indicated_by_msg == RRC_STATE_CELL_PCH || 
            ...
            || (rrc_get_rlc_size_change_status_for_procedure_id(wrrc_as_id, RRC_PROCEDURE_PCR) == TRUE)))
    {
      ...
      else if (rrc_get_rlc_size_change_status_for_procedure_id(wrrc_as_id, RRC_PROCEDURE_PCR) == TRUE)
      {
  ...
  if(ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
    ...
    (void) rex_set_timer(&pcrc.rrcpcrc_temp_timer, 5000);   /* 5 seconds */

    pcrc.rrcpcrc_sig = rex_wait(RRCPCRC_TEMP_TIMER_SIG);
    MSG_LOW("RRCPCRC signal:%d", pcrc.rrcpcrc_sig, 0, 0);
    (void) rex_clr_sigs(rex_self(), RRCPCRC_TEMP_TIMER_SIG);
#endif /* FEATURE_RRC_DELAY_TX_ON_NEW_CFG */
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
  rrcqsh_log_uplink_dcch_ota_params(wrrc_as_id, WRRC_UL_DCCH_MESSAGETYPE_PHYSICALCHANNELRECONFIGURATIONCOMPLETE, ptr);
#endif
...
)
{
  ...
  {
    rrc_PhysicalChannelReconfiguration_r10_IEs *pcreconfig_ptr = &msg_ptr->u.later_than_r3.
    criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.physicalChannelReconfiguration_r10;
  
    if(RRCRB_R10_MSG_IE_PRESENT (pcreconfig_ptr,
    dl_CounterSynchronisationInfo))
    {
      return TRUE;
    }
...
)
{
  ...
  if ( RRC_MSG_COMMON_BITMASK_IE_PTR(msg_ptr,rrc_PhysicalChannelReconfiguration_r5_IEs,dl_HSPDSCH_Information))
  {
    ...
    status = rrc_hsdpa_initial_validation(wrrc_as_id,msg_ptr->rrc_StateIndicator, 
        ...
        rrc_PhysicalChannelReconfiguration_r5_IEs,dl_CounterSynchronisationInfo)) 
  {
          ...
          ( rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH)))
    {
      WRRC_MSG2_ERROR("PCRC : SRNS reloc with invalid states cur_state %d, msg_state %d",
             rrc_get_state(wrrc_as_id), pcrc.state_indicated_by_msg);
      pcrc.status.failure_status=RRCPC_MSG_INVALID_CONFIG;
      return;
    }
    ...
    else
    {
      ...
      if (RRC_MSG_COMMON_BITMASK_IE_PTR(msg_ptr, rrc_PhysicalChannelReconfiguration_r5_IEs,new_U_RNTI))
      {
        ...
        rrc_translate_urnti (wrrc_as_id, &(msg_ptr->new_U_RNTI),
                              &pcrc.new_urnti);
      }
    }
  }
   ...
   */
  pcrc.trans_to_dch_ciph_config_needed = FALSE;
  if((pcrc.state_indicated_by_msg == RRC_STATE_CELL_DCH) && ( rrc_get_state(wrrc_as_id) != RRC_STATE_CELL_DCH))
  {
      ...
      (pcrc.state_indicated_by_msg == RRC_STATE_URA_PCH))
  {
     ...
     if (pcrc.state_indicated_by_msg == RRC_STATE_URA_PCH)
     {
       ...
       else
       {
         ...
         {
           rrccu_update_ura_id (wrrc_as_id, &(msg_ptr->ura_Identity));
         }
         else
         {
...
)
{
      ...
      rrc_PhysicalChannelReconfiguration_r6_IEs,dl_HSPDSCH_Information))
  {
    ...
    /* Check if network does not try to initiate HSDPA with invalid state */
    status = rrc_hsdpa_initial_validation_r6(wrrc_as_id,msg_ptr->rrc_StateIndicator, 
...
)
{
  ...
  {
    WRRC_MSG3_HIGH("Incomatible simultaneous reconfig! Rejecting PCRC message,transaction_id:%d,accepted_transaction:%d rejected_trans_id:%d",
    ...
    if ((transaction_id != pcrc.tr_ids.accepted_transaction) && (transaction_id != pcrc.tr_ids.rejected_transaction))
    {
           ...
           (RRCCU_STARTED == rrccu_get_cell_update_started_status(wrrc_as_id) ))
      {
        ...
        {
          send_pcrc_failure_message(wrrc_as_id, pcrc.status.failure_status,
        ...
        else
        {
          ...
          if (SUCCESS != rrccu_register_for_cell_update_to_complete(wrrc_as_id, RRC_PROCEDURE_PCR))
          {
...
)
{
       ...
       rrc_PhysicalChannelReconfiguration_r3_IEs,dl_CounterSynchronisationInfo))
  {
    ...
    else
    {
      ...
      {
        pcrc.new_urnti_valid = TRUE;
        rrc_translate_urnti (wrrc_as_id, &(msg_ptr->u.r3.physicalChannelReconfiguration_r3.new_U_RNTI),
                              &pcrc.new_urnti);
      }
    }
  }
     ...
     if (pcrc.state_indicated_by_msg == RRC_STATE_URA_PCH)
     {
       ...
       else
       {
         ...
         if (msg_ptr->u.r3.physicalChannelReconfiguration_r3.ura_Identity.numbits == 16)
         {
           ...
           rrccu_update_ura_id (wrrc_as_id, &(msg_ptr->u.r3.physicalChannelReconfiguration_r3.ura_Identity));
         }
         else
         {
           WRRC_MSG1_ERROR("Invalid URA ID length%d",
...
)
{
  ...
  ptr = rrc_get_int_cmd_buf(wrrc_as_id);
  {
    ...
    if( (rrc_get_state(wrrc_as_id)) == (pcrc.state_indicated_by_msg) )
    {
    ...
    {
      rrc_fill_rb_id_chan_req_from_est_rabs(wrrc_as_id,&rb_count, &ptr->cmd.chan_config_req);
    }

    if((pcrc.state_indicated_by_msg == RRC_STATE_CELL_DCH) 
          && (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH))
    {
      rrc_fill_srb_in_chan_config_req(wrrc_as_id,&rb_count, &ptr->cmd.chan_config_req);
    }
...
===========================================================================*/
static void rrcpcrc_clear_procedure(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  pcrc.rrcpcrc_send_initiate_cu_flag = FALSE;
  pcrc.rrcpcrc_delete_crnti = FALSE;
  ...
  rrc_common_procedure_clear(wrrc_as_id,RRC_PROCEDURE_PCR);
}
...
)
{
  ...
  if (pcrc.cn_info_saved != FALSE)
  {
    ...
    {
       rrc_ccm_update_service_info(wrrc_as_id, pcrc.mm_cmd_ptr);
    }
    rrc_send_rrc_message_to_mm(wrrc_as_id, pcrc.mm_cmd_ptr);
    pcrc.cn_info_saved = FALSE;
  }
      ...
      (indicate_rlc == TRUE))
  {
    ...
    rrcrb_register_with_rlc_for_ul_data_ind (wrrc_as_id, RRC_PROCEDURE_PCR);
  ...
  rrcpcrc_clear_procedure(wrrc_as_id);
  ...
  rrccu_reset_registration_info_for_procedure(wrrc_as_id,RRC_PROCEDURE_PCR);
...
)
{
  ...
  if (FAILURE == 
      rrcpcrc_check_and_send_failure_for_simul_reconfig(wrrc_as_id,
  ...
  message */
  validate_pcrc_r5(wrrc_as_id,  pcrc_ptr );
  ...
  else
  {
    ...
    if(RRC_MSG_COMMON_BITMASK_IE_PTR(pcrc_ptr,rrc_PhysicalChannelReconfiguration_r5_IEs,cn_InformationInfo))
    {
       ...
       * v690NonCriticalExtensions
       */
       primary_plmn_Id_present = rrcpcr_check_and_get_primary_plmn_id(wrrc_as_id,msg_ptr,
                                                                  &primary_plmn_Identity,MSG_REL5);
      pcrc.mm_cmd_ptr = rrc_ccm_buid_service_ind_with_nas_cn_info(wrrc_as_id,
    ...
    rrcpcrc_process_high_mobility_ind_in_non_critical_ext(wrrc_as_id,msg_ptr,MSG_REL5);
   
    /* Need to send reconfig IE info. First set ORDERED_CONFIG
    so that LLC can process the necessary parameters. */
    oc_status = rrcllc_set_ordered_config(wrrc_as_id, RRC_PROCEDURE_PCR,
    ...
    switch( oc_status )
    {
      ...
      case ORDERED_CONFIG_SET:
                 ...
                 ||(pcrc.state_indicated_by_msg==RRC_STATE_URA_PCH)))
        {
          ...
          if (rrcpcrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL5))
          {
             ...
             (rrccsp_check_initiate_cell_selection_handling(wrrc_as_id,&directed_cell_info) == FALSE))
          {
            ...
            {
              pcrc.rrcpcrc_send_initiate_cu_flag = TRUE;
              pcrc.rrcpcrc_delete_crnti          = TRUE;
            }
            else
            {
           ...
           )
        {
             ...
             (pcrc.state_indicated_by_msg==RRC_STATE_CELL_FACH))
          {
            ...
            if ((pcrc.rrcpcrc_send_initiate_cu_flag == FALSE)&&
                (rrcrb_validate_directed_cell_info (wrrc_as_id,directed_cell_info) == FALSE))
            {
              ...
              if(RRC_MSG_COMMON_BITMASK_IE_PTR(pcrc_ptr, rrc_PhysicalChannelReconfiguration_r5_IEs,new_C_RNTI))
              {
                ...
                rrc_translate_crnti(wrrc_as_id,&pcrc_ptr->new_C_RNTI, 
                                    &c_rnti);
              ...
              rrcrb_send_initiate_cell_selection_req(wrrc_as_id, RRC_PROCEDURE_PCR,
          ...
          rrcpcrc_send_channel_config_req( wrrc_as_id  );
                ...
                (pcrc.state_indicated_by_msg==RRC_STATE_CELL_FACH))
        {
          ...
          rrccu_start_t_305_timer(wrrc_as_id);
                ...
                (pcrc.state_indicated_by_msg==RRC_STATE_CELL_PCH))
        {
          pcrc.directed_cell_info = rrcpcrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL5);
         /*Send the PCRC complete msg on old configuration */
         send_pcrc_complete_message(wrrc_as_id, TRUE);
         /*wait for L2ack */
                 ...
                 (pcrc.state_indicated_by_msg==RRC_STATE_URA_PCH)))
        {
             ...
             rrc_PhysicalChannelReconfiguration_r5_IEs,new_C_RNTI))
          {
            ...
            {
              rrc_translate_urnti(wrrc_as_id,&pcrc_ptr->new_U_RNTI, 
                                  &u_rnti);
      ...
      case ORDERED_CONFIG_NOT_SET_OTHER:
        ...
        case ORDERED_CONFIG_CONFIGURATION_SET_DEFERRED:
          WRRC_MSG0_HIGH("BUF_OTA:PCR processing deferred due to LTA!");
          pcrc.curr_substate = RRCRB_INITIAL;
          rrc_buffer_ota_info.transaction_id = pcrc.tr_ids.accepted_transaction;
        /* Indicate that an error occured */
          ordered_config_err_occured = TRUE;
          break;
...
)
{
  ...
  invalid or incompatible configuration given by the
  message */
  validate_pcrc_r6(wrrc_as_id, pcrc_ptr );
  ...
  else
  {
    ...
    {
      WRRC_MSG0_HIGH("Saving CN info for NAS");
      pcrc.mm_cmd_ptr = rrc_ccm_buid_service_ind_with_nas_cn_info_r6(wrrc_as_id,
                        &(pcrc_ptr->cn_InformationInfo));
        ...
        criticalExtensions.u.r6.m.v6b0NonCriticalExtensionsPresent)
    {
      ...
      rrcpcrc_process_high_mobility_ind_in_non_critical_ext(wrrc_as_id,msg_ptr,MSG_REL6);
      ...
      case ORDERED_CONFIG_SET:
       ...
       /* Populate the local variables to indicate if Re-direction info is present */
               ...
               ||(pcrc.state_indicated_by_msg==RRC_STATE_URA_PCH)))
        {
          ...
          if (rrcpcrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL6))
          {
            directed_cell_info.pscr_present = TRUE;
            directed_cell_info.pscr = pcrc_ptr->dl_InformationPerRL_List.elem[0].
                                      modeSpecificInfo.u.fdd->primaryCPICH_Info.primaryScramblingCode;
                ...
                (pcrc.state_indicated_by_msg==RRC_STATE_CELL_FACH))
        {
          ...
          rrccu_start_t_305_timer(wrrc_as_id);		  
                ...
                (pcrc.state_indicated_by_msg==RRC_STATE_CELL_PCH))
        {
          pcrc.directed_cell_info = rrcpcrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL6);
         /*Send the PCRC complete msg on old configuration */
         send_pcrc_complete_message(wrrc_as_id, TRUE);
         /*wait for L2ack */
         pcrc.curr_substate = RRCRB_WAIT_FOR_L2_ACK;
...
WRRC_MSG1_HIGH("Received PCRC msg rel 7, substate %d", pcrc.curr_substate);
  ...
  invalid or incompatible configuration given by the
  message */
  validate_pcrc_r7(wrrc_as_id, pcrc_ptr );
    ...
    switch( oc_status )
    {
      case ORDERED_CONFIG_SET:
                 ...
                 ||(pcrc.state_indicated_by_msg==RRC_STATE_URA_PCH)))
        {
          ...
          if (rrcpcrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL7))
          {
            directed_cell_info.pscr_present = TRUE;
            directed_cell_info.pscr = pcrc_ptr->dl_InformationPerRL_List.elem[0].
                                      modeSpecificInfo.u.fdd->primaryCPICH_Info.primaryScramblingCode;
                ...
                (pcrc.state_indicated_by_msg==RRC_STATE_CELL_FACH))
        {
          ...
          rrccu_start_t_305_timer(wrrc_as_id);		  
                ...
                (pcrc.state_indicated_by_msg==RRC_STATE_CELL_PCH))
        {
          pcrc.directed_cell_info = rrcpcrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL7);
         /*Send the PCRC complete msg on old configuration */
         send_pcrc_complete_message(wrrc_as_id, TRUE);
         /*wait for L2ack */
         pcrc.curr_substate = RRCRB_WAIT_FOR_L2_ACK;
...
)
{
    ...
    (rrcrb_validate_directed_cell_info (wrrc_as_id,directed_cell_info) == FALSE))
  {
    ...
    {
      pcrc.new_crnti_valid = TRUE;
      rrc_translate_crnti(wrrc_as_id,&pcrc_ptr->new_C_RNTI, &c_rnti);
      pcrc.new_crnti = (uint16)c_rnti;
    }
    else
    {
...
)
{
      ...
      ||(pcrc.state_indicated_by_msg==RRC_STATE_URA_PCH)))
  {
    ...
    if (rrcpcrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL8))
    {
      directed_cell_info.pscr_present = TRUE;
      directed_cell_info.pscr = pcrc_ptr->dl_InformationPerRL_List.elem[0].
                              modeSpecificInfo.u.fdd->primaryCPICH_Info.primaryScramblingCode;
    ...
    (pcrc.state_indicated_by_msg==RRC_STATE_CELL_DCH)))
  {
    if ((current_state==RRC_STATE_CELL_FACH)&& 
     (pcrc.state_indicated_by_msg==RRC_STATE_CELL_FACH))
    {
      if(rrcpcrc_process_after_oc_config_set_for_fach_to_fach_r8(wrrc_as_id,msg_ptr, directed_cell_info))	
      return;
    }/* if CELL_FACH->CELL_FACH reconfiguration */
  ...
  /*Cell_DCH -> Cell_PCH*/
      ...
      (pcrc.state_indicated_by_msg==RRC_STATE_CELL_PCH))
  {
    pcrc.directed_cell_info = rrcpcrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL8);
      ...
      (pcrc.state_indicated_by_msg==RRC_STATE_URA_PCH)))
  {
    ...
    if(RRCRB_R8_MSG_IE_PRESENT(pcrc_ptr,rrc_PhysicalChannelReconfiguration_r8_IEs,new_C_RNTI))
    {
      ...
      {
        rrc_translate_urnti(wrrc_as_id,&pcrc_ptr->new_U_RNTI, &u_rnti);
        rnti_update_info.urnti_action = RRC_RNTI_UPDATE; 
        rnti_update_info.urnti        = u_rnti; 
      }
      else
...
)
{
    ...
    dl_CounterSynchronisationInfo))
  {
    ...
    else
    {
      ...
      {
        pcrc.new_urnti_valid = TRUE;
        rrc_translate_urnti (wrrc_as_id, &(msg_ptr->new_U_RNTI), &pcrc.new_urnti);
      }
    }
  }
...
)
{
  ...
  if (FAILURE == rrcpcrc_check_and_send_failure_for_simul_reconfig(wrrc_as_id,
        msg_ptr->u.later_than_r3.rrc_TransactionIdentifier))
  {
    return;
  }
  ...
  invalid or incompatible configuration given by the
  message */
  validate_pcrc_r8(wrrc_as_id, pcrc_ptr );
  ...
  if( pcrc.status.failure_status != RRCPC_MSG_VALID )
  {
    ...
    /* Send the PC Reconfig Failure Message */
    send_pcrc_failure_message(wrrc_as_id, pcrc.status.failure_status, pcrc.status.prot_err_cause);
  ...
  else
  {
    ...
    /* Need to send reconfig IE info. First set ORDERED_CONFIG
    so that LLC can process the necessary parameters. */
    oc_status = rrcllc_set_ordered_config(wrrc_as_id,RRC_PROCEDURE_PCR, pcrc.state_indicated_by_msg, 
                                                            rrc_DL_DCCH_Message_PDU,(void *)dcch_msg_ptr );
    switch(oc_status)
    {
      case ORDERED_CONFIG_SET:
        rrcpcrc_process_after_oc_config_set_r8(wrrc_as_id,dcch_msg_ptr,pcrc_ptr);
        break;
...
)
{
      ...
      ||(pcrc.state_indicated_by_msg==RRC_STATE_URA_PCH)))
  {
    ...
    if (rrcpcrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL9))
    {
      directed_cell_info.pscr_present = TRUE;
      directed_cell_info.pscr = pcrc_ptr->dl_InformationPerRL_List.elem[0].
                              modeSpecificInfo.u.fdd->primaryCPICH_Info.primaryScramblingCode;
    ...
    (pcrc.state_indicated_by_msg==RRC_STATE_CELL_DCH)))
  {
    if ((current_state==RRC_STATE_CELL_FACH)&& 
     (pcrc.state_indicated_by_msg==RRC_STATE_CELL_FACH))
    {
      if(rrcpcrc_process_after_oc_config_set_for_fach_to_fach_r9(wrrc_as_id,msg_ptr, directed_cell_info))
      return;
    }/* if CELL_FACH->CELL_FACH reconfiguration */
  ...
  /*Cell_DCH -> Cell_PCH*/
      ...
      (pcrc.state_indicated_by_msg==RRC_STATE_CELL_PCH))
  {
    pcrc.directed_cell_info = rrcpcrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL9);
...
)
{
  ...
  invalid or incompatible configuration given by the
  message */
  validate_pcrc_r9(wrrc_as_id,  pcrc_ptr );
  ...
  else
  {
    ...
    {
      case ORDERED_CONFIG_SET:
        rrcpcrc_process_after_oc_config_set_r9(wrrc_as_id,dcch_msg_ptr,pcrc_ptr);
        break;
...
/*===========================================================================
FUNCTION   rrcpcrc_process_after_oc_config_set_for_fach_to_fach_r10

DESCRIPTION
  This function processes the R10 PCRC OTA after OC is set by this OTA and When FACH to FACH
  state transition is triggered by the OTA.
...
SIDE EFFECTS
  None.
===========================================================================*/
static boolean rrcpcrc_process_after_oc_config_set_for_fach_to_fach_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  rrc_PhysicalChannelReconfiguration * msg_ptr,
  rrcrb_directed_cell_info_type  directed_cell_info
...
{
  rrc_int_c_rnti_type                 c_rnti;           /* Local C_RNTI value */
  rrc_PhysicalChannelReconfiguration_r10_IEs *pcrc_ptr = NULL;
  ...
  if ((pcrc.rrcpcrc_send_initiate_cu_flag == FALSE)&&
    (rrcrb_validate_directed_cell_info (wrrc_as_id, directed_cell_info) == FALSE))
  {
    if(RRCRB_R10_MSG_IE_PRESENT(pcrc_ptr,new_C_RNTI))
    {
      pcrc.new_crnti_valid = TRUE;
      rrc_translate_crnti(wrrc_as_id,&pcrc_ptr->new_C_RNTI, &c_rnti);
      pcrc.new_crnti = (uint16)c_rnti;
...
/*===========================================================================
FUNCTION   rrcpcrc_process_after_oc_config_set_r10

DESCRIPTION
  This function processes the R10 PCRC OTA after OC is set by this OTA. So if we call this function, it means
  that ORDERED_CONFIG_SET is returned while setting OC by PCRC OTA.
...
SIDE EFFECTS
  None.
===========================================================================*/
static void rrcpcrc_process_after_oc_config_set_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  rrc_DL_DCCH_Message *dcch_msg_ptr,
  rrc_PhysicalChannelReconfiguration_r10_IEs *pcrc_ptr  
)
{
  rrc_int_c_rnti_type                 c_rnti;           /* Local C_RNTI value */
  rrc_int_u_rnti_type                 u_rnti;           /* Local U_RNTI value*/
  ...
  if((current_state==RRC_STATE_CELL_FACH) && 
      ...
      ||(pcrc.state_indicated_by_msg==RRC_STATE_URA_PCH)))
  {
    if((RRCRB_R10_MSG_IE_PRESENT(pcrc_ptr,frequencyInfo)) &&
      (RRC_CHECK_COMMON_MSG_TYPE(pcrc_ptr->frequencyInfo.modeSpecificInfo,
      rrc_FrequencyInfo_modeSpecificInfo_fdd)))           
    {
      directed_cell_info.freq_info_present = TRUE;
    ...
    if (rrcpcrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL10))
    {
      directed_cell_info.pscr_present = TRUE;
      directed_cell_info.pscr = pcrc_ptr->dl_InformationPerRL_List.elem[0].
                              modeSpecificInfo.u.fdd->primaryCPICH_Info.primaryScramblingCode;
    ...
    (pcrc.state_indicated_by_msg==RRC_STATE_CELL_DCH)))
  {
    if ((current_state==RRC_STATE_CELL_FACH)&& 
     (pcrc.state_indicated_by_msg==RRC_STATE_CELL_FACH))
    {
      if(rrcpcrc_process_after_oc_config_set_for_fach_to_fach_r10(wrrc_as_id,msg_ptr, directed_cell_info))
      return;
    }/* if CELL_FACH->CELL_FACH reconfiguration */
  ...
  /*Cell_DCH -> Cell_PCH*/
      ...
      (pcrc.state_indicated_by_msg==RRC_STATE_CELL_PCH))
  {
    pcrc.directed_cell_info = rrcpcrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL10);
      ...
      (pcrc.state_indicated_by_msg==RRC_STATE_URA_PCH)))
  {
    ...
    if(RRCRB_R10_MSG_IE_PRESENT(pcrc_ptr,new_C_RNTI))
    {
      ...
      /* check whether URNTI is also present in the message */
      if(RRCRB_R10_MSG_IE_PRESENT(pcrc_ptr,new_U_RNTI))
      {
        rrc_translate_urnti(wrrc_as_id,&pcrc_ptr->new_U_RNTI, &u_rnti);
        rnti_update_info.urnti_action = RRC_RNTI_UPDATE; 
        rnti_update_info.urnti        = u_rnti; 
...
SIDE EFFECTS
  None.
===========================================================================*/
static void validate_pcrc_r10
(
	sys_modem_as_id_e_type wrrc_as_id,
  rrc_PhysicalChannelReconfiguration_r10_IEs *msg_ptr /* Pointer to the PCRC message */
)
{
  /* Assume the message is valid for now */
  pcrc.status.failure_status = RRCPC_MSG_VALID;
  ...
  is not supported in the first release. This may be supported
  later */                                    
  if(RRCRB_R10_MSG_IE_PRESENT(msg_ptr,dl_CounterSynchronisationInfo))
  {
    /*check whether the SRNS relocation is within Cell_DCH or not.
     SRNS relocation information can come in reconfiguration messages
     only when the state transition is Cell_DCH->Cell_DCH */
    ...
    else
    {
      ...
      pcrc.pcrc_for_srns_relocation = TRUE;
      /* store if new U-RNTI is present in the message */
      if(RRCRB_R10_MSG_IE_PRESENT(msg_ptr,
        new_U_RNTI))
      {
        pcrc.new_urnti_valid = TRUE;
        rrc_translate_urnti (wrrc_as_id, &(msg_ptr->new_U_RNTI), &pcrc.new_urnti);
    ...
    (rrc_get_state(wrrc_as_id) != RRC_STATE_CELL_DCH))
  {
    ...
    if((RRCRB_R10_MSG_IE_PRESENT(msg_ptr,
      dl_CommonInformation)) && (RRC_MSG_COMMON_BITMASK_IE(msg_ptr->dl_CommonInformation,
      rrc_DL_CommonInformation_r10,dl_dpchInfoCommon)) && (T_rrc_DL_CommonInformation_r10_dl_dpchInfoCommon_dl_DPCH_InfoCommon
      == msg_ptr->dl_CommonInformation.dl_dpchInfoCommon.t))
    {
      //check if ciphering is active in mac for cs domain, if it is then,
      //start 2 tiered HHO procedure STEP_1 and STEP_2 for rl-re-establishment
  ...
  {
    /* If UTRAN DRX coefficient is not present then it is invalid configuration */
    if (!(RRCRB_R10_MSG_IE_PRESENT(msg_ptr, utran_DRX_CycleLengthCoeff)))
    {
      WRRC_MSG1_ERROR("UTRAN DRX Coef not preset, State:%d", pcrc.state_indicated_by_msg);
      pcrc.status.failure_status = RRCPC_MSG_INVALID_CONFIG;
      return;
    ...
    {
      /*should we do it after getting l2ack for response??*/
      if (!(RRCRB_R10_MSG_IE_PRESENT(msg_ptr,ura_Identity)))
      {
        WRRC_MSG0_HIGH("URA ID not present");
        rrccu_clear_ura_id (wrrc_as_id);
      }
...
}/* end validate_pcrc_r10() */



/*===========================================================================
FUNCTION   PROCESS_PCRC_R10

DESCRIPTION
  This function processes the received Rel 10 PC Reconfig Message. It first validates the message. 
  If the message has an invalid or unsupported configuration, a PCRC Failure message will be sent to
  the UTRAN. If the message is valid, it will be processed as follows -
...
SIDE EFFECTS
  None.
===========================================================================*/
static void process_pcrc_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  rrc_RRC_TransactionIdentifier transaction_id,
  rrc_DL_DCCH_Message *dcch_msg_ptr  ,
  rrc_PhysicalChannelReconfiguration_r10_IEs *pcrc_ptr   /* Pointer to the R10 PC Reconfig message */
)
{
  rrc_PhysicalChannelReconfiguration *msg_ptr = NULL;  /* Pointer to the RB Setup message */
  rrcllc_oc_status_e_type          oc_status;                     /* To hold ordered_config status   */
  ...
  msg_ptr = &dcch_msg_ptr->message.u.physicalChannelReconfiguration;
  WRRC_MSG1_HIGH("REL10: Received PC-Reconfig OTA, PCRC-substate %d", pcrc.curr_substate);
  ...
  invalid or incompatible configuration given by the
  message */
  validate_pcrc_r10(wrrc_as_id, pcrc_ptr );

  if( pcrc.status.failure_status != RRCPC_MSG_VALID )
  {
    /*  Message was invalid. */
    WRRC_MSG0_ERROR("REL10: PCRC Message validation failed");
  ...
  else
  {
    ...
    /* Check if there is CN information present. If so, we need to send the information to the 
    Cell Change manager */
    if(RRCRB_R10_MSG_IE_PRESENT(pcrc_ptr,
      cn_InformationInfo))
    {
      WRRC_MSG0_HIGH("Saving CN info for NAS");
      pcrc.mm_cmd_ptr = rrc_ccm_buid_service_ind_with_nas_cn_info_r6(wrrc_as_id,
    ...
    {
      case ORDERED_CONFIG_SET:
        rrcpcrc_process_after_oc_config_set_r10(wrrc_as_id,dcch_msg_ptr,pcrc_ptr);
        break;
...
)
{
  ...
  if (TRUE == rrcpcrc_validate_srns_relocation_info(wrrc_as_id,msg_ptr, msg_version))
  {
    ...
    switch (ip_check_status)
    {
      ...
      case RRCSMC_INCOMPATIBLE_SECURITY_RECONFIGURATION:
        ...
        else
        {
          ...
          {
            WRRC_MSG0_HIGH("Cell Update Started! Register for CU CNF");
            (void)rrccu_register_for_cell_update_to_complete(wrrc_as_id, RRC_PROCEDURE_PCR);
            /* Wait until Cell update procedure is completed */
            pcrc.curr_substate = RRCRB_WAIT_FOR_CELL_UPDATE_CNF; 
          }
          else
...
)
{
  ...
  if (msg_ptr->t != T_rrc_PhysicalChannelReconfiguration_r3)
  {
           ...
           T_rrc_PhysicalChannelReconfiguration_criticalExtensions_5_criticalExtensions))
    {
      ...
      { 
        ...
        if (FAILURE == rrcpcrc_validate_and_check_integrity_for_srns_relocation(wrrc_as_id,
        ...
        process_pcrc_r5(wrrc_as_id, cmd_ptr);
      }
        ...
        (RRCRB_R8_CRITICAL_EXT_PRESENT_PTR(msg_ptr, rrc_PhysicalChannelReconfiguration)))
      {
        ...
        process_pcrc_r8(wrrc_as_id, msg_ptr->u.later_than_r3.rrc_TransactionIdentifier,
          (rrc_DL_DCCH_Message *)cmd_ptr->cmd.downlink_sdu_ind.decoded_msg,
          &msg_ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.
          criticalExtensions.u.criticalExtensions.u.r8.physicalChannelReconfiguration_r8);
      }
        ...
        (RRCRB_R9_CRITICAL_EXT_PRESENT_PTR(msg_ptr, rrc_PhysicalChannelReconfiguration)))
      {
        ...
        /*Now process the R8 PCRC OTA and then return.*/
        process_pcrc_r9(wrrc_as_id, msg_ptr->u.later_than_r3.rrc_TransactionIdentifier,
          (rrc_DL_DCCH_Message *)cmd_ptr->cmd.downlink_sdu_ind.decoded_msg,
          &msg_ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.
          criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.r9.physicalChannelReconfiguration_r9);
      }
#endif /* FEATURE_WCDMA_REL9 */
#ifdef FEATURE_WCDMA_REL10
      /*Now we check whether R10 PCRC is present or not. */
      else if((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL10) && 
        (RRCRB_R10_CRITICAL_EXT_PRESENT_PTR(msg_ptr, rrc_PhysicalChannelReconfiguration)))
      {
        /* check for SRNS relocation scenario */
        if (FAILURE == rrcpcrc_validate_and_check_integrity_for_srns_relocation(wrrc_as_id,
        ...
        /*Now process the R8 PCRC OTA and then return.*/
        process_pcrc_r10(wrrc_as_id, msg_ptr->u.later_than_r3.rrc_TransactionIdentifier,
      (rrc_DL_DCCH_Message *)cmd_ptr->cmd.downlink_sdu_ind.decoded_msg,
      &msg_ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.
      criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.physicalChannelReconfiguration_r10);
      }
#endif /* FEATURE_WCDMA_REL10 */
      ...
      else
      {
        ...
        rrcpcrc_send_pcrc_failure_unsupported_config(wrrc_as_id,
            msg_ptr->u.later_than_r3.rrc_TransactionIdentifier);      
      }
  ...
  invalid or incompatible configuration given by the
  message */
  validate_pcrc_message(wrrc_as_id, msg_ptr );
  ...
  else
  {
    ...
    rrcpcrc_process_high_mobility_ind_in_non_critical_ext(wrrc_as_id,msg_ptr,MSG_REL99);
    ...
    switch( oc_status )
    {
      ...
      case ORDERED_CONFIG_SET:
               ...
               ||(pcrc.state_indicated_by_msg==RRC_STATE_URA_PCH)))
        {
          ...
          if (rrcpcrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL99))
          {
           ...
           )
        {
             ...
             (pcrc.state_indicated_by_msg==RRC_STATE_CELL_FACH))
          {
                ...
                (rrcrb_validate_directed_cell_info (wrrc_as_id, directed_cell_info) == FALSE))
            {
              ...
              {
                pcrc.new_crnti_valid = TRUE;
                rrc_translate_crnti(wrrc_as_id,&msg_ptr->u.r3.physicalChannelReconfiguration_r3.new_C_RNTI, 
                                    &c_rnti);
                pcrc.new_crnti = (uint16)c_rnti;
              }
              else
                ...
                (pcrc.state_indicated_by_msg==RRC_STATE_CELL_FACH))
        {
          ...
          rrccu_start_t_305_timer(wrrc_as_id);
                ...
                (pcrc.state_indicated_by_msg==RRC_STATE_CELL_PCH))
        {
          pcrc.directed_cell_info = rrcpcrc_is_pcpich_info_present(wrrc_as_id,msg_ptr, MSG_REL99);
...
)
{
  ...
  {
    rrcrb_update_amr_cctrch_id_for_reconfig(wrrc_as_id,rrcdata_codec_info.rrc_active_codec);
  }
  ...
  if( ptr->chan_cfg_status != RRCLLC_CHAN_CFG_SUCCESS )
  {
    ...
    if (ptr->chan_cfg_status == RRCLLC_CHAN_CFG_FAILURE_OTHER)
    {
      ...
      if(rrccu_return_cu_substate(wrrc_as_id) == RRCCU_WAIT_FOR_PENDING_CONFIG)
      {
        ...
        pcrc.curr_substate = RRCRB_WAIT_FOR_CELL_UPDATE_CNF;
        rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_PCR, RRC_UNRECOVERABLE_ERROR, FALSE);
        return; 
      }
      ...
      configurations */
      (void)rrcllc_reset_ordered_config(wrrc_as_id,RRC_PROCEDURE_PCR,TRUE);
      ...
      if(rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH && pcrc.state_indicated_by_msg == RRC_STATE_CELL_DCH)
      {
    ...
    if (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH && pcrc.state_indicated_by_msg == RRC_STATE_CELL_DCH)
    {
      ...
      rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_PCR, RRC_PHY_CHAN_FAILURE,TRUE);
    ...
    else if (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH && pcrc.state_indicated_by_msg == RRC_STATE_CELL_DCH)
    {
      pcrc.status.failure_status = RRCPC_PHY_CHAN_FAILURE;
      if(rrccu_return_cu_substate(wrrc_as_id) == RRCCU_WAIT_FOR_PENDING_CONFIG)
      {
        ...
        pcrc.curr_substate = RRCRB_WAIT_FOR_CELL_UPDATE_CNF;
        rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_PCR, RRC_RADIO_LINK_FAILURE, FALSE);
        return; 
      }
      ...
      else if(ptr->chan_cfg_status == RRCLLC_CHAN_CFG_FAILURE_BACK_TO_OLD_CONFIG)
      {
        ...
        /* Send RRC_CELL_UPDATE_INITIATE_REQ command to CELL UPDATE Proc */
        rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_PCR, RRC_RADIO_LINK_FAILURE,TRUE);
        /* Wait for CELL UPDATE to get a valid C_RNTI */
        pcrc.curr_substate = RRCRB_WAIT_FOR_CELL_UPDATE_CNF;
        pcrc.rrcpcrc_send_initiate_cu_flag = FALSE;
        pcrc.rrcpcrc_delete_crnti = FALSE;
      ...
      (ordered_config.set_status == OC_SET_FOR_DCH_URA_PCH_TRANS))	
  {
    ...
    {
      rrcrb_send_upd_ota_parms_ind(wrrc_as_id,RRC_PROCEDURE_PCR, CPHY_HIGH_MOBILITY_INCL);
    }
  }  
       ...
       (pcrc.state_indicated_by_msg == RRC_STATE_URA_PCH)))
  {
    if (pcrc.state_indicated_by_msg == RRC_STATE_CELL_PCH) 
    {
      ...
      else
      {
        ...
        rrcrb_send_l1_drx_req (wrrc_as_id, RRC_PROCEDURE_PCR);

        rrcpcrc_process_successful_proc_completion (wrrc_as_id, TRUE);
      }
      /* return from here */
      return;
    }
    else if (pcrc.state_indicated_by_msg == RRC_STATE_URA_PCH)
    {
      ...
      {
        rrcrb_send_ura_update_initiate_req(wrrc_as_id, RRC_PROCEDURE_PCR,
                                            RRC_URA_RESELECTION,
                                            FALSE);
      }
    ...
    /*Following case will occur when RLC RESET happens on SRB2 during DCH->FACH Transition*/
      ...
      (cu_cause == rrc_CellUpdateCause_rlc_unrecoverableError) && (rrccu_get_rb_id_unrec_error(wrrc_as_id) == DCCH_AM_RADIO_BEARER_ID))
    {
  ...
  if (pcrc.pcrc_for_srns_relocation == TRUE)
  {
    if (RRC_STATE_CELL_DCH == curr_state)
    {
      ...
      rrcrb_send_rlc_re_est_req_to_smc(wrrc_as_id,RRC_PROCEDURE_PCR, rb_type, TRUE);
  ...
  {   
    ...
    2> include the IE "Reconfiguration Status Indicator" and set it to TRUE.*/
      ...
      { 
        ...
        (void) rex_clr_sigs(rex_self(), RRCPCRC_TEMP_TIMER_SIG);
        (void) rex_set_timer(&pcrc.rrcpcrc_temp_timer, RRCRB_PND_CFG_DELAY);  
...
)
{
 ...
 /* Check if E-RNTI is not given in OTA then initiate cell update*/
           ...
           ptr->new_state == RRC_STATE_URA_PCH)
  {
    ...
    {
      WRRC_MSG3_HIGH("Recvd state change %d to %d pcrc state %d, No action",
...
)
{
  ...
  if( ptr->status == SUCCESS )
  {
    ...
    if (pcrc.pcrc_for_srns_relocation)
    {
      if (RRC_STATE_CELL_DCH == rrc_get_state(wrrc_as_id))
      {
        ...
        rrcrb_send_rlc_re_est_req_to_smc(wrrc_as_id,RRC_PROCEDURE_PCR, rb_type, FALSE);
  ...
  else
  {
    ...
    if (pcrc.pcrc_for_srns_relocation) 
    {
      ...
      rrc_transition_to_disconnected_state(wrrc_as_id,RRC_PROCEDURE_PCR,RRC_TX_TO_DISCON_OTHER);
    }
  }
  ...
  if(pcrc.pcrc_for_srns_relocation == TRUE)
  {
    rrcdormancy_update_dormancy_for_srns_hho_cco(wrrc_as_id, RRC_PROCEDURE_PCR);
  }
...
)
{
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    ...
    case RRC_DOWNLINK_SDU_IND:
      ...
      process_pcrc_message(wrrc_as_id, cmd_ptr );
      
      break;


    case RRC_COMPRESSED_MODE_IND:
      rrcpcrc_dch_compressed_mode_handler(wrrc_as_id,&cmd_ptr->cmd.cm_ind);
      break;
    ...
    default:
      ...
      substate */
      WRRC_MSG3_HIGH("Command 0x%x ignored in substate%d rrc_state%d", 
              cmd_ptr->cmd_hdr.cmd_id, pcrc.curr_substate, rrc_get_state(wrrc_as_id));
      break;
  }
}
/*===========================================================================
...
)
{
  ...
  case RRC_RE_EST_RLC_FOR_SRNS_CNF:
    ...
    rrcpcrc_process_rlc_reestablishment_cnf(wrrc_as_id  );
    break;

  case RRC_STATE_CHANGE_IND:

    /* Indication that RRC's state has changed */
    rrcpcrc_process_state_change_ind(wrrc_as_id, &(cmd_ptr->cmd.state_change_ind) );
    break;
...
)
{
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    ...
    case RRC_CHANNEL_CONFIG_CNF:
      ...
      rrcpcrc_process_channel_config_cnf(wrrc_as_id, &(cmd_ptr->cmd.chan_config_cnf) );
      break;
  ...
  switch ( cmd_ptr->cmd_hdr.cmd_id )
  {
    case RRC_CELL_UPDATE_COMPLETE_CNF:
      ...
      else if(pcrc.status.failure_status == RRCPC_MSG_VALID)
      {
        ...
        {
          send_pcrc_complete_message(wrrc_as_id,FALSE);
        }
        /* prints rate for each transport channel */
        rrcllc_print_trans_chnl_rate(wrrc_as_id);
        /*if next state is Cell_PCH or URA_PCH then we have to 
...
)
{
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    ...
    case RRC_DOWNLINK_L2ACK_IND:
      ...
      /* Confirmation from LLC for the channel configurations */
      rrcpcrc_process_l2_ack_ind(wrrc_as_id, &(cmd_ptr->cmd.downlink_l2ack_ind) );
      break;
...
)
{
  ...
  switch ( cmd_ptr->cmd_hdr.cmd_id )
  {
    ...
    case RRC_NEW_CELL_IND:
      rrcpcrc_process_new_cell_ind(wrrc_as_id, &(cmd_ptr->cmd.new_cell_ind) );
      break;
...
)
{
  ...
  case RRC_RNTI_UPDATE_CNF:
    ...
    {
      rrcpcrc_process_rnti_update_cnf(wrrc_as_id);
    }
    else
    {
...
)
{
  ...
  WRRC_MSG4_HIGH("PCR:L2ack status %d <0-Success 1-Failure> for MUI:%d, Lc_Id:%d, rbe_subst:%d",
...
)
{
  ...
  switch( pcrc.curr_substate )
  {
    ...
    case RRCRB_WAIT_FOR_CELL_UPDATE_CNF:
      /* Call the event handler for RRCRB_WAIT_FOR_CELL_UPDATE_CNF substate */
      rrcpcrc_fach_wfcucnf_event_handler(wrrc_as_id,  cmd_ptr);
      break;

    case RRCRB_WAIT_FOR_CHAN_CFG_CNF:
      /* Call the event handler for RRCRB_WAIT_FOR_CHAN_CFG_CNF substate */
      rrcpcrc_fach_wfchcnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;

  case  RRCRB_WAIT_FOR_CELL_SELECTION_CNF:
      /* Call the event handler for RRCRB_WAIT_FOR_CHAN_CFG_CNF substate */
      rrcpcrc_fach_wfcscnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;


    case RRCRB_WAIT_FOR_L2_ACK:
      /* Call the event handler for RRCRB_WAIT_FOR_L2_ACK substate */
      rrcpcrc_fach_wfl2ack_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;

    case RRCRB_WAIT_FOR_CRNTI_UPDATE_CNF:
        /* Call the event handler for RRCRB_WAIT_FOR_CRNTI_UPDATE_CNF substate */
        rrcpcrc_fach_wfcrntiupdatecnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
        break;
...
)
{
  ...
  case RRCRB_WAIT_FOR_CELL_UPDATE_CNF:
    /* Call the event handler for RRCRB_WAIT_FOR_CELL_UPDATE_CNF substate */
    rrcpcrc_pch_wfcucnf_event_handler(wrrc_as_id,  cmd_ptr);
    break;

  case RRCRB_WAIT_FOR_CELL_SELECTION_CNF:

    rrcpcrc_pch_wfcscnf_event_handler(wrrc_as_id,  cmd_ptr);
    break;

  case RRCRB_WAIT_FOR_CHAN_CFG_CNF:

    /* Call the event handler for RRCRB_WAIT_FOR_CHAN_CFG_CNF substate */
    rrcpcrc_pch_wfchcnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
    break;
  ...
  default:
    ...
    /* No other substates processed for this RRC state    */
    WRRC_MSG3_HIGH("Command %x not processed in substate%d rrc_state%d", 
            cmd_ptr->cmd_hdr.cmd_id, pcrc.curr_substate, rrc_get_state(wrrc_as_id));
    break;
...
)
{
  ...
  switch( pcrc.curr_substate )
  {
    ...
    case RRCRB_WAIT_FOR_CHAN_CFG_CNF:
      ...
      rrcpcrc_other_wfchcnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;

    case RRCRB_WAIT_FOR_L2_ACK:
     
      /* Call the event handler for WAIT_FOR_L2_ACK substate */
      rrcpcrc_other_wfl2ack_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;

    case RRCRB_WAIT_FOR_CELL_UPDATE_CNF:
      rrcpcrc_other_wfcucnf_substate_event_handler(wrrc_as_id,  cmd_ptr);
      break;
...
)
{
  ...
  rrc_crash_debug_save_proc_substates(wrrc_as_id,RRC_PROCEDURE_PCR,pcrc.curr_substate,cmd_ptr->cmd_hdr.cmd_id);
  ...
  switch (rrc_state)
  {
    ...
    case RRC_STATE_CELL_FACH:    
      ...
      for CELL_FACH state */
      rrcpcrc_cell_fach_event_handler(wrrc_as_id, cmd_ptr);
      break;

    case RRC_STATE_CELL_DCH:    
      /* If it's CELL_DCH state, call the event handler
      for CELL_DCH state */
      rrcpcrc_cell_dch_event_handler(wrrc_as_id, cmd_ptr);
      break;
    ...
    case RRC_STATE_URA_PCH:
      /* If it's CELL_DCH state, call the event handler
      for CELL_PCH state */
      rrcpcrc_pch_event_handler(wrrc_as_id, cmd_ptr);
      break;
    ...
    case RRC_STATE_DISCONNECTED: 
      ...
      rrcpcrc_other_state_event_handler(wrrc_as_id, cmd_ptr);
      break;
...
{                                         
 ...
 RRC_RESET_MSG_IE_PRESENT(msg_ptr->message.u.physicalChannelReconfigurationComplete);
  ...
  if (srns_reloc == TRUE)
  {
    rrcpcrc_append_start_list_to_pcrc_complete(wrrc_as_id,msg_ptr);
  }
...
===========================================================================*/
void rrcpcrc_init_procedure(sys_modem_as_id_e_type wrrc_as_id)
{
//  uint32 count;
  ...
  to disconnected state. */
  rrcscmgr_register_for_scn(wrrc_as_id, RRC_PROCEDURE_PCR,  /* Procedure name */
  ...
  rex_def_timer(&pcrc.rrcpcrc_temp_timer, rex_self(), RRCPCRC_TEMP_TIMER_SIG);
...
)
{
  ...
  /* Send a Delete measurement Req to L1 */
    rrcmeas_cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id);
    ...
    rrc_put_int_cmd(wrrc_as_id,rrcmeas_cmd_ptr);
  ...
  if(ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
    ...
    status = rrcsend_send_ul_sdu(wrrc_as_id,RRC_PROCEDURE_PCR, (void *)ptr, 
...
/*===========================================================================
FUNCTION rrcpcr_return_send_initiate_cu_flag
...
boolean rrcpcr_return_send_initiate_cu_flag(sys_modem_as_id_e_type wrrc_as_id)
{
  return pcrc.rrcpcrc_send_initiate_cu_flag;
}
#endif
...
{
  qsh_mdump_collect_high(&pcrc,sizeof(pcrc));
}
#endif
...
===========================================================================*/
void rrcpcrc_set_failure_cause(sys_modem_as_id_e_type wrrc_as_id, rrcrb_msg_status_e_type cause)
{
