...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcsmc.c#3 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
07/14/20   gcs     Made changes to send SMC info to SSGCCS module when SMC received for later than r3
...
05/16/18   sg      Made changes to remove setting of new_key_for_start_value flag for keys received from other RAT
27/04/18   sg      Made changes to add wait till stand by end substate handling in DCH state machine
...
01/03/18   sp      Made changes to support configuration with RB-ID 32
08/29/17   sg       Made changes to remove the GCF NV 947 read and get the value of this NV through an api
03/03/17   sg      Made changes to avoid printing security keys based on test mode
09/06/16   sp      Modified the conditional logics used to detect SRNS relocation cases
01/22/16   nr      Made changes to remove irrelevant F3s
01/12/16   nr      Made changes to report failure events to QSH
11/25/15   as      Made changes to reset the CS tm affected flag during PS RAB setup
10/27/15   sp      Made changes to ignore SMC if CU has started
09/09/15   sp      Made changes to add resel failure, rrc conn rel events
08/13/15   sg      Made changes to update the same HFN in use at RLC when DL RB is re-established during DCH to FACH transition with CU
08/11/15   sg      Made changes to not include HFN of the RBs which have pending ciphering configuration in START value calculation
07/30/15   sp      NV changes to control l2 ack wait timer during reselection
06/30/15   sp      Fixed QSH indentation errors
03/10/15   sg      Made changes to copy a valid key to int key old data base before integrity is started
...
01/13/15   sp      Made changes to reset stored security keys flag after DDS switch
12/30/14   bc      Replacing ERR_FATAL with MSG_ERR0_FATAL, MSG_ERR1_FATAL, MSG_ERR2_FATAL.
12/26/14   sg      Made changes to indicate cipher sync to GSM when going to IDLE after
                   applying new key                    
...
{\
  if(rrc_get_test_mode_logging_status(wrrc_as_id) == TRUE)\
  {\
    ...
    WRRC_MSG4_HIGH("INT KEY 1: 0x%x 2: 0x%x 3: 0x%x 4: 0x%x", temp_key[0],temp_key[1],temp_key[2],temp_key[3]);\
  }\
}
#else /*__GNUC__*/
#define PRINT_INT_KEY(key_info)\
...
\
  ...
  \
            ...
            {\
              ...
              WRRC_MSG4_HIGH("CIPHER KEY 1: 0x%x 2: 0x%x 3: 0x%x 4: 0x%x", temp_key[0],temp_key[1],temp_key[2],temp_key[3]);\
  \
              break;\
  \
          /* Invalid Key type  */ \
...
typedef struct
{
  ...
  boolean feature_rollback_security_config;
  
  /*maintain status of failure cause*/
  rrcsmc_config_status_e_type rrcsmc_failure_type;
  
  rrc_ProtocolErrorCause rrcsmc_prot_err_cause;
  
  /* file global var contains info about suspend status and info of rlc rb's*/
  rrcsmc_suspend_stop_rlc_type smc_rb_suspend_or_stop_info;
  
  /*Temporary variable to store the max count c values*/
  rrcsmc_rel_rbs_max_count_c max_count_c;
  
  /*struct to save Security Mode Command message till we have received
    Suspend_Cnf from LLC*/
  rrcsmc_security_mode_command_type saved_security_mode_command_msg;
  
  /*struct to save SRNS Security Info till we have received
    Suspend_Cnf from LLC*/
  rrcsmc_srns_info saved_srns_info;
  ...
  boolean l2_ack_rcvd_for_smc_complete_flag;
  
  boolean rrcsmc_send_l2_ack_ind_to_ddt_flag;
  
  boolean extract_cause_rel_ind;
  ...
  */
  boolean is_revert_to_old_cipher_variable_reqd;
  ...
  /* Global to hold the second SMC message if SMC messages are received back to back*/
  smc_msg_type saved_smc_message;
  
}rrcsmc_int_data_type;
...
rrcsmc_ext_data_type rrcsmc_ext_data_sim[MAX_RRC_STACKS];

#define nv_fake_security_enabled nv_fake_security_enabled_sim[WAS_ID_M]
#define rrcsmc_ext_data rrcsmc_ext_data_sim[WAS_ID_M]
#else
extern boolean nv_fake_security_enabled;
rrcsmc_ext_data_type rrcsmc_ext_data;
#endif
...
** ----------------------------------------------------------------------- */
#ifdef FEATURE_DUAL_WCDMA 
LOCAL rrcsmc_int_data_type rrcsmc_int_data_sim[MAX_RRC_STACKS];
...
LOCAL rrcsmc_security_variables_backup backup_of_security_variables_sim[MAX_RRC_STACKS];
  
  /*maintain state of SMC procedure*/
LOCAL rrcsmc_substate_e_type  rrcsmc_substate_sim[MAX_RRC_STACKS];
  ...
  /*static var to hold all kinds of security config which is not calculated
    ...
    OTA message */
LOCAL rrcsmc_security_config_info_type  rrcsmc_int_ciph_info_sim[MAX_RRC_STACKS];

#define rrcsmc_int_data rrcsmc_int_data_sim[WAS_ID_M]
#define backup_of_security_variables backup_of_security_variables_sim[WAS_ID_M]
#define rrcsmc_substate rrcsmc_substate_sim[WAS_ID_M]
#define rrcsmc_int_ciph_info rrcsmc_int_ciph_info_sim[WAS_ID_M]

#else
LOCAL rrcsmc_int_data_type rrcsmc_int_data;

/* This variable is supposed to be used to take back-up of all security related
   global variables. This will be used to revert back to old configuration when
   cell update is triggered during secruity reconfiguration or during physical
   channel establishment failure */
LOCAL rrcsmc_security_variables_backup backup_of_security_variables;
...
** ----------------------------------------------------------------------- */
/*===========================================================================
FUNCTION rrc_get_test_mode_logging_status
DESCRIPTION
This function gets the test mode logging status to enable or disable security keys logging
DEPENDENCIES
None.
RETURN VALUE
TRUE/FALSE
...
boolean rrc_get_test_mode_logging_status(sys_modem_as_id_e_type wrrc_as_id)
{
  return mmgsdi_is_test_mode_logging_enabled_for_subscription(wrrc_as_id);
}
...
)
{
  ...
  { 
    case rrc_DL_DCCH_Message_PDU :
      ...
      msg_type = rrc_get_dl_dcch_message_type(wrrc_as_id,cmd_ptr->cmd.downlink_sdu_ind.dl_sdu,
...
/*===========================================================================
FUNCTION rrcsmc_get_srns_info_from_dl_sdu_ind_r10

DESCRIPTION
  This function extracts ip check info/integrity protection mode info/ciphering mode info from SRNS 
  relocation triggering messages for Release 10 version of OTAs. In summary, input srns_info has to be filled
  from OTA in cmd_ptr.

  NOTE:  There is no R10 UTRANMobilityInformation and URAUpdateConfirm messages in ASN 9.5.
  Right now, only 5 Reconfig OTAs + CU CNF can do SRNS relocation for R10 OTA.
...
SIDE EFFECTS
  None
===========================================================================*/
uecomdef_status_e_type rrcsmc_get_srns_info_from_dl_sdu_ind_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  rrcsmc_srns_relocation_security_info_r7_type * srns_info,
  rrc_cmd_type *cmd_ptr
  ...
  { 
    case rrc_DL_DCCH_Message_PDU :
      ...
      switch (msg_type)
      {
        case T_rrc_DL_DCCH_MessageType_cellUpdateConfirm:
        {
          rrc_CellUpdateConfirm_r10_IEs *cucnf_ptr = 
            RRC_EXTRACT_R10_MSG_POINTER(dcch_msg_ptr,cellUpdateConfirm,cellUpdateConfirm_r10);
      
           if(RRCRB_R10_MSG_IE_PRESENT(cucnf_ptr,integrityProtectionModeInfo))
          {
            srns_info->integrity_info_present = TRUE;
            srns_info->integrity_protection_mode_info = &(cucnf_ptr->integrityProtectionModeInfo);
          }
           if(RRCRB_R10_MSG_IE_PRESENT(cucnf_ptr,cipheringModeInfo))
          {
            srns_info->ciphering_mode_info_present = TRUE;
            srns_info->ciphering_mode_info = &(cucnf_ptr->cipheringModeInfo);
          }
        ...
        case T_rrc_DL_DCCH_MessageType_radioBearerReconfiguration:
        {
          rrc_RadioBearerReconfiguration_r10_IEs *rbrc_ptr = 
            RRC_EXTRACT_R10_MSG_POINTER(dcch_msg_ptr,radioBearerReconfiguration,radioBearerReconfiguration_r10);

           if(RRCRB_R10_MSG_IE_PRESENT(rbrc_ptr,integrityProtectionModeInfo))
          {
            srns_info->integrity_info_present = TRUE;
            srns_info->integrity_protection_mode_info = &(rbrc_ptr->integrityProtectionModeInfo);
          }
           if(RRCRB_R10_MSG_IE_PRESENT(rbrc_ptr,cipheringModeInfo))
          {
            srns_info->ciphering_mode_info_present = TRUE;
            srns_info->ciphering_mode_info = &(rbrc_ptr->cipheringModeInfo);
          }
        ...
        case T_rrc_DL_DCCH_MessageType_radioBearerRelease:
        {
          rrc_RadioBearerRelease_r10_IEs* rb_release_ptr = 
            RRC_EXTRACT_R10_MSG_POINTER(dcch_msg_ptr,radioBearerRelease,radioBearerRelease_r10);
 
           if(RRCRB_R10_MSG_IE_PRESENT(rb_release_ptr,integrityProtectionModeInfo))
          {
            srns_info->integrity_info_present = TRUE;
            srns_info->integrity_protection_mode_info = &(rb_release_ptr->integrityProtectionModeInfo);
          }
           if(RRCRB_R10_MSG_IE_PRESENT(rb_release_ptr,cipheringModeInfo))
          {
            srns_info->ciphering_mode_info_present = TRUE;
            srns_info->ciphering_mode_info =  &(rb_release_ptr->cipheringModeInfo);
          }
        ...
        case T_rrc_DL_DCCH_MessageType_radioBearerSetup:
        {
          rrc_RadioBearerSetup_r10_IEs* rb_setup_ptr = 
            RRC_EXTRACT_R10_MSG_POINTER(dcch_msg_ptr,radioBearerSetup,radioBearerSetup_r10);

           if(RRCRB_R10_MSG_IE_PRESENT(rb_setup_ptr,integrityProtectionModeInfo))
          {
            srns_info->integrity_info_present = TRUE;
            srns_info->integrity_protection_mode_info = &(rb_setup_ptr->integrityProtectionModeInfo);
          }
           if(RRCRB_R10_MSG_IE_PRESENT(rb_setup_ptr,cipheringModeInfo))
          {
            srns_info->ciphering_mode_info_present = TRUE;
            srns_info->ciphering_mode_info = &(rb_setup_ptr->cipheringModeInfo);
          }
        ...
        case T_rrc_DL_DCCH_MessageType_transportChannelReconfiguration:
        {
          rrc_TransportChannelReconfiguration_r10_IEs *tcreconfig_ptr =
            RRC_EXTRACT_R10_MSG_POINTER(dcch_msg_ptr,transportChannelReconfiguration,transportChannelReconfiguration_r10);

           if(RRCRB_R10_MSG_IE_PRESENT(tcreconfig_ptr,integrityProtectionModeInfo) )
          {
            srns_info->integrity_info_present = TRUE;
            srns_info->integrity_protection_mode_info =   &(tcreconfig_ptr->integrityProtectionModeInfo);
          }
           if(RRCRB_R10_MSG_IE_PRESENT(tcreconfig_ptr,cipheringModeInfo) )
          {
            srns_info->ciphering_mode_info_present = TRUE;
            srns_info->ciphering_mode_info =  &(tcreconfig_ptr->cipheringModeInfo);
          }
        ...
        case T_rrc_DL_DCCH_MessageType_physicalChannelReconfiguration:
        {
          rrc_PhysicalChannelReconfiguration_r10_IEs *pcreconfig_ptr =
            RRC_EXTRACT_R10_MSG_POINTER(dcch_msg_ptr,physicalChannelReconfiguration,physicalChannelReconfiguration_r10);

           if(RRCRB_R10_MSG_IE_PRESENT(pcreconfig_ptr,integrityProtectionModeInfo))
          {
            srns_info->integrity_info_present = TRUE;
            srns_info->integrity_protection_mode_info = &(pcreconfig_ptr->integrityProtectionModeInfo);
          }
           if(RRCRB_R10_MSG_IE_PRESENT(pcreconfig_ptr,cipheringModeInfo))
          {
            srns_info->ciphering_mode_info_present = TRUE;
            srns_info->ciphering_mode_info = &(pcreconfig_ptr->cipheringModeInfo);
          }
...
{
  WRRC_MSG1_HIGH("HHO debug. mac_start_value_to_transmit is 0x%x", rrcsmc_int_data.mac_start_value_to_transmit >> RRC_MAC_HFN_OFFSET);
  return (rrcsmc_int_data.mac_start_value_to_transmit >> RRC_MAC_HFN_OFFSET);
}
/***/ __WCDMA_RRC_CODE_SEGMENT_NON_DEMANDPAGED_END__ /***/
...
)
{
  ...
  { 
    case rrc_DL_DCCH_Message_PDU :
      ...
      msg_type = 
        rrc_get_dl_dcch_message_type(wrrc_as_id,cmd_ptr->cmd.downlink_sdu_ind.dl_sdu,
        cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_length);
      switch (msg_type)
      {
...
#endif /* FEATURE_WCDMA_REL9 */
#ifdef FEATURE_WCDMA_REL10
                    else if (0x00 == b_unpackb(dl_sdu_ptr, 
                         DL_DCCH_LATER_THAN_R3_CRIT_EXTN_CRIT_EXTN_CRIT_EXTN_CRIT_EXTN_CRIT_EXTN_LATER_THAN_R8_CRIT_EXTN_R9_LATER_THAN_R9_CRIT_EXTN_R10_POS,
                         NUM_OF_BITS_FOR_DL_DCCH_LATER_THAN_R3_CRIT_EXTN_CRIT_EXTN_CRIT_EXTN_CRIT_EXTN_CRIT_EXTN_LATER_THAN_R8_CRIT_EXTN_R9_LATER_THAN_R9_CRIT_EXTN_R10))
                     {
                       /* check the value of bit which whether integrity protection
                       mode info is included or not */
                       if (0x00 != b_unpackb(dl_sdu_ptr, 
                             DL_DCCH_R10_INTEGRITY_PROTECTION_MODE_INFO_PRESENT_POS,
                             NUM_OF_BITS_FOR_DL_DCCH_R10_INTEGRITY_PROTECTION_MODE_INFO_PRESENT))
                       { 
                          WRRC_MSG0_HIGH("RRCR10: SRNS relocation case detected - intProtModeInfo present");
                          return(TRUE);
                       }
                     }
...
===========================================================================*/
rrc_cn_domain_identity_e_type rrcsmc_get_latest_configured_cn_domain(sys_modem_as_id_e_type wrrc_as_id)
{
  if (FALSE == rrcsmc_int_data.is_revert_to_old_cipher_variable_reqd)
  {
  ...
  else
  {
    ...
    */
    return(backup_of_security_variables.rrcsmc_int_ciph_info_bkp.latest_config_cn_id);
  }
...
===========================================================================*/
static void rrcsmc_clear_cipher_config_db(sys_modem_as_id_e_type wrrc_as_id)
{
  /*reset contents of global ciphering database*/
  rrcsmc_ext_data.cipher_config_db.l1_cipher_info.cipher_key_req.req_mask = 0x00;

  /*reset MAC information in global database*/
  rrcsmc_ext_data.cipher_config_db.mac_cipher_info[RRC_CS_DOMAIN_CN_ID].cipher_enable = FALSE;
  rrcsmc_ext_data.cipher_config_db.mac_cipher_info[RRC_PS_DOMAIN_CN_ID].cipher_enable = FALSE;
  ...
  rrcsmc_ext_data.cipher_config_db.mac_cipher_info[RRC_CS_DOMAIN_CN_ID].cipher_cfg_type = 
  NON_HO_CIPHER_CFG;  
  rrcsmc_ext_data.cipher_config_db.mac_cipher_info[RRC_PS_DOMAIN_CN_ID].cipher_cfg_type = 
  NON_HO_CIPHER_CFG;   
  rrcsmc_ext_data.cipher_config_db.mac_ciph_config_affected = FALSE;
    
  /*reset the number of Ciphered UL and DL RLC channels*/
  rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.nchan = 0;
  rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.nchan = 0;
  ...
  if(rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.num_rb != 0)
  {
    rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.num_rb = 0;
  }

  rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.num_rb=0;

  /*reset the number of ciphered TM radio bearers*/
  rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[RRC_CS_DOMAIN_CN_ID].num_rb=0;
  rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[RRC_PS_DOMAIN_CN_ID].num_rb=0;

  /*set the ciphering affected flag for all domains false*/
  rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[RRC_CS_DOMAIN_CN_ID].affected=FALSE;
  rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[RRC_PS_DOMAIN_CN_ID].affected=FALSE;
}
/*===========================================================================
...
)
{
  ...
  rrcsmc_int_data.integrity_protection_info.reconfig = FALSE;
  rrcsmc_int_data.integrity_protection_info.ip_status = RRCSMC_SECURITY_NOT_STARTED;        
  rrcsmc_int_data.integrity_protection_info.modify_received = FALSE;


  /*set ciphering reconfig info to false*/
  rrcsmc_int_data.ciphering_info.reconfig = FALSE;
  ...
  PRINT_CIPHER_INT_KEY(RRC_PRINT_CIPHERING_KEY,rrcsmc_int_data.saved_security_key_set)
  ...
  rrcsmc_update_ciph_keys_used(wrrc_as_id, RRC_CS_DOMAIN_CN_ID);
  rrcsmc_update_ciph_keys_used(wrrc_as_id, RRC_PS_DOMAIN_CN_ID);
  ...
  rrcsmc_int_data.NEW_TM_RB_IND_FLAG = FALSE;
  rrcsmc_int_data.START_VALUE_TO_TRANSMIT_FLAG=FALSE;
  rrcsmc_int_data.start_value_to_transmit=0;
  rrcsmc_int_data.start_value_to_transmit_cs_hho=0;
  rrcsmc_int_data.mac_start_value_to_transmit=0;
 ...
 rrcsmc_int_ciph_info.saved_integrity_algo[RRC_PS_DOMAIN_CN_ID] = UE_WCDMA_INTEGRITY_PROT_ALGO_UIA1;
  ...
  rrcsmc_int_data.saved_security_mode_command_msg.dpch_ciph_act_time_present = FALSE;

  rrcsmc_int_data.l2_ack_rcvd_for_smc_complete_flag = FALSE;
  rrcsmc_int_data.rrcsmc_send_l2_ack_ind_to_ddt_flag= FALSE;

  /*clear cipher configuration database as we are going to idle state*/
  rrcsmc_clear_cipher_config_db(wrrc_as_id);
  
  /* Discard buffered SMC command */
  if(rrcsmc_int_data.saved_smc_message.dl_sdu_ptr != NULL)
  {
    rrc_free(rrcsmc_int_data.saved_smc_message.dl_sdu_ptr);
    rrcsmc_int_data.saved_smc_message.dl_sdu_length = 0;
  }
...
static boolean rrcsmc_is_cs_key_pending(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  else if( rrcsmc_int_data.integrity_protection_info.ip_status == RRCSMC_SECURITY_STARTED )
  {
    /*dl_new_key_flag_per_rb is being used for the case where integrity is started but not ciphering*/
    return rrcsmc_int_data.integrity_protection_info.count_i_info[RRC_CS_DOMAIN_CN_ID].dl_new_key_flag_per_rb[DCCH_AM_RADIO_BEARER_ID];  
  }
  else
  {
...
void rrcsmc_copy_pending_keys_to_saved_security_key_set(sys_modem_as_id_e_type wrrc_as_id)
{
  if(TRUE == rrcsmc_is_cs_key_pending(wrrc_as_id))
  {
    ...
    rrcsmc_int_data.saved_security_key_set.grat_key_set_valid = TRUE;

    /*Save ciphering key here*/
    if(rrcsmc_int_ciph_info.int_ciph_info[RRC_CS_DOMAIN_CN_ID].ciph_keys_info.key_used == 1)
    {
      WCDMA_MEMCPY(rrcsmc_int_data.saved_security_key_set.ciphering_key,
    ...
    WCDMA_MEMCPY(rrcsmc_int_data.saved_security_key_set.integrity_key,
    ...
    rrcsmc_int_data.integrity_protection_info.count_i_info[RRC_CS_DOMAIN_CN_ID].dl_new_key_flag_per_rb[DCCH_AM_RADIO_BEARER_ID] = FALSE;
    rrcsmc_int_data.saved_security_key_set.pending_cs_key_to_be_activated = TRUE;
  }
}
/*===========================================================================
...
===========================================================================*/
void rrcsmc_reset_pending_cs_key_to_be_activated(sys_modem_as_id_e_type wrrc_as_id)
{
  rrcsmc_int_data.saved_security_key_set.pending_cs_key_to_be_activated = FALSE;
}
/*===========================================================================
...
===========================================================================*/
void rrcsmc_reset_smc_ip_started_flag(sys_modem_as_id_e_type wrrc_as_id)
{
  rrcsmc_int_data.integrity_protection_info.ip_status = RRCSMC_SECURITY_NOT_STARTED;
}
/*===========================================================================
...
===========================================================================*/
boolean rrcsmc_tm_ciph_active(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcsmc_ext_data.cipher_config_db.mac_cipher_info[RRC_CS_DOMAIN_CN_ID].cipher_enable;
}
/*===========================================================================
...
)
{
  ...
  if (rrcsmc_int_data.NEW_TM_RB_IND_FLAG == TRUE)
  {
    rrcsmc_int_data.NEW_TM_RB_IND_FLAG = FALSE;
    return TRUE;
  }
...
)
{
  ...
  {
    start_list->hfn_rab =
    rrcsmc_convert_START_to_count_c_i(wrrc_as_id,
...
)
{
  /*if start value to transmit is set, then sent the start value, else set
    flag to false*/
  if (rrcsmc_int_data.START_VALUE_TO_TRANSMIT_FLAG == TRUE)
  {
    rrcsmc_convert_count_c_i_to_START(wrrc_as_id,start_value, rrcsmc_int_data.start_value_to_transmit);

    WCDMA_MEMCPY(rrcsmc_int_ciph_info.int_ciph_info[rrcsmc_int_data.start_value_to_transmit_domain].hfn_latest,
                 sizeof(byte) * RRC_MAX_HFN_LENGTH,
                 start_value,
                 RRC_MAX_START_LENGTH);

    rrcsmc_int_ciph_info.start_value_update_flag[rrcsmc_int_data.start_value_to_transmit_domain] 
      = TRUE;
...
)
{
  ...
  WRRC_MSG2_HIGH("GET SVTF:%d SVT:0x%x",
           rrcsmc_int_data.START_VALUE_TO_TRANSMIT_FLAG,
           rrcsmc_int_data.start_value_to_transmit);
...
)
{
  ...
  if (rrcsmc_int_data.START_VALUE_TO_TRANSMIT_FLAG == TRUE)
  {
    return rrcsmc_int_data.start_value_to_transmit;    
  }
...
{
  start_list->hfn_rab = rrcsmc_int_data.start_value_to_transmit;    
  start_list->hfn_rab >>= RRC_RLC_AM_HFN_OFFSET;

  if(rrcsmc_int_ciph_info.latest_config_cn_id == rrcsmc_get_start_value_to_transmit_domain(wrrc_as_id))
  {
    start_list->hfn_srb = rrcsmc_int_data.start_value_to_transmit; 
    start_list->hfn_srb >>= RRC_RLC_AM_HFN_OFFSET;
  }
}
...
)
{
  ...
  if(rrcsmc_int_data.rrcsmc_send_l2_ack_ind_to_ddt_flag == TRUE)
  {
...
)
{
  ...
  {
    WRRC_MSG3_HIGH("CN ID=%d,sync_ind_sent status for CS=%d,PS=%d",
      cn_id,
    ...
    if((rrcsmc_substate == RRCSMC_WAIT_FOR_L2_ACK) && (rb_id <=DCCH_DT_LOW_PRI_RADIO_BEARER_ID) &&
       (rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id].
             dl_count_i_act_time_pending[rb_id] == FALSE))
    {
      /*Check: Can we hardcode DCCH_DT_HIGH_PRI_RADIO_BEARER_ID for DDT?? */
      rrclcm_internal_post_l2_ack_ind_for_proc(wrrc_as_id,RRC_PROCEDURE_SMC, DCCH_AM_RADIO_BEARER_ID);
    }
    ...
    {
      rrcsmc_int_data.rrcsmc_send_l2_ack_ind_to_ddt_flag=TRUE;
      return TRUE;
    }
  }
  else
...
)
{
  ...
  rrcsmc_ext_data.cipher_config_db.l1_cipher_info.cipher_key_req.req_mask |= L1_DL_CIPHER_KEY_REQ;
  rrcsmc_ext_data.cipher_config_db.l1_cipher_info.cipher_key_req.req_mask |= L1_UL_CIPHER_KEY_REQ;
  ...
  if(config_cause == CIPH_CONFIG_UL_KEYS)
  {
    ...
    rrcsmc_ext_data.cipher_config_db.l1_cipher_info.cipher_key_req.ul_cipher_info.num_keys = 4;
    rrcsmc_ext_data.cipher_config_db.l1_cipher_info.cipher_key_req.dl_cipher_info.num_keys = 0;
  }
  else if(config_cause == CIPH_CONFIG_DL_KEYS)
  {
    /*set the number of keys being given to l1 to 4 for DL,as we will be configuring 
      ...
      (depending on the activation time)*/
    rrcsmc_ext_data.cipher_config_db.l1_cipher_info.cipher_key_req.ul_cipher_info.num_keys = 0;
    rrcsmc_ext_data.cipher_config_db.l1_cipher_info.cipher_key_req.dl_cipher_info.num_keys = 4;
  }
  else
  {
  ...
  if(config_cause != CIPH_CONFIG_DL_KEYS)
  {
    ...
    for (i=0; i<RRC_MAX_CN_DOMAINS; i++)/*for cs and ps cn domain id's*/
    {
      ...
      {
        rrcsmc_ext_data.cipher_config_db.l1_cipher_info.cipher_key_req.ul_cipher_info.
        key_info[key_id].key_id = (uint8) key_id;
        rrcsmc_ext_data.cipher_config_db.l1_cipher_info.cipher_key_req.ul_cipher_info.
        key_info[key_id].algorithm = UE_WCDMA_CIPHER_ALGO_UEA1;

        WCDMA_MEMCPY(rrcsmc_ext_data.cipher_config_db.l1_cipher_info.cipher_key_req.ul_cipher_info.key_info[key_id].key_val, 
  ...
  if(config_cause != CIPH_CONFIG_UL_KEYS)
  {
    ...
    for (i=0; i<RRC_MAX_CN_DOMAINS; i++)/*for cs and ps cn domain id's*/
    {
      for (j=0; j<MAX_CIPH_KEYS_PER_DOMAIN; j++)/*num of keys per domain*/
      {
        rrcsmc_ext_data.cipher_config_db.l1_cipher_info.cipher_key_req.dl_cipher_info.
        key_info[key_id].key_id = (uint8) key_id;
        rrcsmc_ext_data.cipher_config_db.l1_cipher_info.cipher_key_req.dl_cipher_info.
        key_info[key_id].algorithm = UE_WCDMA_CIPHER_ALGO_UEA1;
        WCDMA_MEMCPY(rrcsmc_ext_data.cipher_config_db.l1_cipher_info.cipher_key_req.dl_cipher_info.key_info[key_id].key_val,
                     sizeof(uint8) * L1_NUM_BYTES_CIPHER_KEY,
                     rrcsmc_int_ciph_info.int_ciph_info[i].ciph_keys_info.key[j],
                     RRC_MAX_CIPHERING_KEY_LENGTH);
        key_id++;
...
{
  rrcsmc_int_data.max_count_c.count_c_cs = rrcsmc_int_ciph_info.max_count_c_rel_rbs[RRC_CS_DOMAIN_CN_ID];
  rrcsmc_int_data.max_count_c.count_c_ps = rrcsmc_int_ciph_info.max_count_c_rel_rbs[RRC_PS_DOMAIN_CN_ID];
}
...
{
  rrcsmc_int_ciph_info.max_count_c_rel_rbs[RRC_CS_DOMAIN_CN_ID]=rrcsmc_int_data.max_count_c.count_c_cs;
  rrcsmc_int_ciph_info.max_count_c_rel_rbs[RRC_PS_DOMAIN_CN_ID]=rrcsmc_int_data.max_count_c.count_c_ps;
}
/*===========================================================================
...
)
{
  ...
  {
    if(rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id].
       dl_count_i_act_time_pending[i] != TRUE)
    {
      max_count_i_dl = 
        (max_count_i_dl < rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].dl_count_i[i])?
        rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].dl_count_i[i]:max_count_i_dl; 
    }
  }        
  ...
  for ( i=0; i < RRC_MAX_SRB; i++)
  {
    ...
    {
      max_count_i_ul = 
        (max_count_i_ul < rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].ul_count_i[i])?
        rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].ul_count_i[i]:max_count_i_ul; 
    }
  }        
  ...
  */
  ...
  if(rrcsmc_int_ciph_info.ciph_status[domain_id] == RRCSMC_SECURITY_STARTED)
  {
    ...
    (void) rlc_rrc_get_hfn_req(wrrc_as_id, local_hfn_type_ptr);

    /* hfn_valid will be FALSE if there is any ciphering config pending for this RB */
    /* 
        8.5.9 START value calculation
        
        NOTE: Here, "most recently configured" means that if there is more than one key in use for a CN domain, due to
        non-expiry of the ciphering and/or integrity protection activation time for any signalling radio bearers
        and/or radio bearers, do not include the COUNT-I/COUNT-C for these signalling radio bearers and/or
        radio bearers in the calculation of the START X'.
      
      */
    for (i=0; i < local_hfn_type_ptr->ul_nchan; i++)
    {
      if(FALSE == local_hfn_type_ptr->hfn_per_rb_ul[i].hfn_valid)
      {
    ...
    for (i=0; i < local_hfn_type_ptr->ul_nchan; i++)
    {
      ...
      if(domain_id == RRC_CS_DOMAIN_CN_ID)
      {
        ...
        {
          switch(rrclcm_get_ul_rb_id(wrrc_as_id,(uint8)local_lc_id))
          {
    ...
    {
      if(FALSE == local_hfn_type_ptr->hfn_per_rb_dl[i].hfn_valid)
      {
    ...
    for (i=0; i < local_hfn_type_ptr->dl_nchan; i++)
    {
      ...
      if(domain_id == RRC_CS_DOMAIN_CN_ID)
      {
        if(rrcsmc_int_ciph_info.latest_config_cn_id == RRC_CS_DOMAIN_CN_ID)
        {
          switch(rrclcm_get_dl_rb_id(wrrc_as_id,(uint8)local_lc_id))
    ...
    if((domain_id == RRC_CS_DOMAIN_CN_ID) && 
     (rrcsmc_ext_data.cipher_config_db.mac_cipher_info[RRC_CS_DOMAIN_CN_ID].cipher_enable == TRUE))
    {
      /*get the latest MAC HFN's*/
      (void) mac_rrc_get_hfn_info(wrrc_as_id, &mac_hfn);
  ...
  if(domain_id==RRC_CS_DOMAIN_CN_ID)
  {
    ...
    /*Reset the HFN value*/
    rrcsmc_int_ciph_info.max_count_c_rel_rbs[RRC_CS_DOMAIN_CN_ID]=0;
    rrcsmc_int_data.max_count_c.count_c_cs=0;
  }
  else
  {
    ...
    /*Reset the HFN value*/
    rrcsmc_int_ciph_info.max_count_c_rel_rbs[RRC_PS_DOMAIN_CN_ID]=0;
    rrcsmc_int_data.max_count_c.count_c_ps=0;
  }
  
  max_start = (max_start_c > max_start_i)? max_start_c:max_start_i;
  
  if((rrcsmc_int_data.extract_cause_rel_ind == TRUE) && 
     (rrcsmc_int_ciph_info.start_value_update_flag[domain_id] == TRUE))
  {
    max_update = rrcsmc_convert_START_to_count_c_i(wrrc_as_id,
      rrcsmc_int_ciph_info.int_ciph_info[domain_id].hfn_latest, 0);
    ...
    WRRC_MSG4_HIGH("MAX START: 0x%x hfn_latest: 0x%x 0x%x 0x%x",
    max_start,
    rrcsmc_int_ciph_info.int_ciph_info[domain_id].hfn_latest[0],
...
FUNCTION rrc_get_hfn_for_rb

DESCRIPTION
  This function returns currently used hfn for a RB from RLC
...
uint32 rrc_get_hfn_for_rb(sys_modem_as_id_e_type wrrc_as_id, uint8 rb_id, boolean direction)
{
  uint32 local_hfn = 0;
  uint8 i;
  rlc_hfn_req_type *local_hfn_type_ptr = (rlc_hfn_req_type *)rrc_malloc(sizeof(rlc_hfn_req_type));
  ...
  /* direction = TRUE means DL */
  if(direction == TRUE)
  {
    for(i=0;i<local_hfn_type_ptr->dl_nchan;i++)
    {
      if(rb_id == rrclcm_get_dl_rb_id(wrrc_as_id, local_hfn_type_ptr->hfn_per_rb_dl[i].rlc_id))
      {
        local_hfn = local_hfn_type_ptr->hfn_per_rb_dl[i].hfn;
        WRRC_MSG2_HIGH("RB id %d found, dl_hfn: 0x%x", rb_id, local_hfn);
        break;
      }
    }
  }
  else
  {
    for(i=0;i<local_hfn_type_ptr->ul_nchan;i++)
    {
      if(rb_id == rrclcm_get_ul_rb_id(wrrc_as_id, local_hfn_type_ptr->hfn_per_rb_ul[i].rlc_id))
      {
        local_hfn = local_hfn_type_ptr->hfn_per_rb_ul[i].hfn;
        WRRC_MSG2_HIGH("RB id: %d found, ul_hfn: 0x%x", rb_id, local_hfn);
        break;
      }
    }
  }	
...
)
{
  ...
  /*copy the contents of locally stored keys into cipher global db*/
  rrcsmc_update_cphy_key_req(wrrc_as_id,config_cause);

  /*update the key indices if incase authentication is done.*/
  rrcsmc_update_ciph_keys_used(wrrc_as_id,latest_config_cn_domain);
  ...
  for (i=0; i< rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.num_rb; i++)
  {
    ...
    if(
        ...
        rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[i].domain_id))
    {
      /*set HFN*/
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].hfn = 
      rrcsmc_convert_START_to_count_c_i(wrrc_as_id,
          rrcsmc_int_ciph_info.int_ciph_info[rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[i].domain_id].hfn, 0);

      /*change HFN offset for RLC AM*/
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].hfn >>= RRC_RLC_AM_HFN_OFFSET;

      /*set rb_id and lc_id*/
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].rb_id = 
      rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[i].rb_id;

      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].lc_id = 
      rrclcm_get_ul_rlc_lc_id(wrrc_as_id,rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[i].rb_id);

      /*set ciphering activation time*/
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].ciph_act_sn = 
      rrcsmc_int_data.ciphering_info.ul_ciph_act_info.sq_num[rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.
      ciphering[i].lc_id];

      /*set cipher mode to true*/
        rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].cipher_mode = TRUE;
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].ciph_algo = 
          rrcsmc_int_ciph_info.saved_ciph_algo[rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[i].domain_id];

      /*set key index*/
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].cipher_key_id = 
      rrcsmc_get_ciph_key_idx(wrrc_as_id,rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[i].domain_id);	

      /*set flag to ciph_started*/
      if (rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[i].rb_status == CIPH_FIRST_TIME)
      {
        rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[i].rb_status=CIPH_STARTED;
      }

      /*increment the number of channels by 1*/
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.nchan++;
    }
  }

  /*loop through the number of UM radiobearers configured and update global
    database used by LLC to configure RLC with ciphering parameters*/
  for (i=0; i < rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.num_rb; i++)
  {
    /*if domain id is same as domain id of latest configured cn domain only then
      update ciphering parameters*/
    if (
...
#endif        
        (latest_config_cn_domain == 
        rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[i].domain_id))
    {
      /*set HFN*/
      if(rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.nchan < (UE_MAX_UL_UM_CHANNEL + UE_MAX_AM_ENTITY))
      {
        rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[rrcsmc_ext_data.cipher_config_db.
        rlc_ul_cipher_info.nchan].hfn = 
        rrcsmc_convert_START_to_count_c_i(wrrc_as_id,
          rrcsmc_int_ciph_info.int_ciph_info[rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[i].domain_id].hfn, 0);
        ...
        rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[i].rb_id;

        rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.
        nchan].lc_id = 
        rrclcm_get_ul_rlc_lc_id(wrrc_as_id,rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[i].rb_id);

        /*set ciphering activation sequence number*/
        rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[rrcsmc_ext_data.cipher_config_db.
        rlc_ul_cipher_info.nchan].ciph_act_sn = 
        rrcsmc_int_data.ciphering_info.ul_ciph_act_info.sq_num[rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.
          ciphering[rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.nchan].lc_id];
        ...
        rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[rrcsmc_ext_data.cipher_config_db.
          rlc_ul_cipher_info.nchan].ciph_algo = 
            rrcsmc_int_ciph_info.saved_ciph_algo[rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[i].domain_id];

        /*set key index*/
        rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[rrcsmc_ext_data.cipher_config_db.
        rlc_ul_cipher_info.nchan].cipher_key_id = 
        rrcsmc_get_ciph_key_idx(wrrc_as_id,rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[i].domain_id);
      }
      else
      {
        WRRC_ERR0_FATAL("Invalid number of channels received");
      }

      /*set flag to ciph_started*/
      if (rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[i].rb_status == CIPH_FIRST_TIME)
      {
        rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[i].rb_status=CIPH_STARTED;
      }
  ...
  if (rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[latest_config_cn_domain].
      affected == TRUE)
  {
    /*set cipher mode to true*/
      rrcsmc_ext_data.cipher_config_db.mac_cipher_info[latest_config_cn_domain].cipher_algo = 
    ...
    */
    if((rrcsmc_substate == RRCSMC_WAIT_FOR_CIPH_CONFIG_CNF) &&
       (TRUE == mac_rrc_get_cipher_pending_status(wrrc_as_id)))
    {
      WRRC_MSG0_HIGH("Sending SMC_INITIATED_NON_HO_CIPHER_CFG to MAC (Second Pending)");
      rrcsmc_ext_data.cipher_config_db.mac_cipher_info[latest_config_cn_domain].cipher_cfg_type = 
    ...
    */ 
    if (FALSE == rrcsmc_ext_data.cipher_config_db.mac_cipher_info[latest_config_cn_domain].cipher_enable)
    {
    ...
    rrcsmc_ext_data.cipher_config_db.mac_cipher_info[latest_config_cn_domain].cipher_enable = TRUE;

    /*check if dpch ciph activation time is present or not and flag error*/
    if (rrcsmc_int_data.saved_security_mode_command_msg.dpch_ciph_act_time_present)
    {
      rrcsmc_ext_data.cipher_config_db.mac_cipher_info[latest_config_cn_domain].
      ciph_act_time = rrcsmc_int_data.saved_security_mode_command_msg.dpch_ciph_act_time; 
    }
    else
    {
    ...
    rrcsmc_ext_data.cipher_config_db.mac_cipher_info[latest_config_cn_domain].hfn = 
    rrcsmc_convert_START_to_count_c_i(wrrc_as_id,
    ...
    /*set key index*/
    rrcsmc_ext_data.cipher_config_db.mac_cipher_info[latest_config_cn_domain].key_idx =
    rrcsmc_get_ciph_key_idx(wrrc_as_id,latest_config_cn_domain);
    ...
    rrcsmc_ext_data.cipher_config_db.mac_ciph_config_affected = TRUE;
  ...
  rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.nchan = 
  rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.nchan;

  for (i=0; i<rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.nchan; i++)
  {
    /*set key id, hfn, cipher_mode, rb_id and activation time for UM/AM*/
    rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.ciphering[i].cipher_key_id = 
    rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].cipher_key_id;

    rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.ciphering[i].cipher_mode =
    rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].cipher_mode;

    rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.ciphering[i].hfn =
    rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].hfn;

    rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.ciphering[i].rb_id = 
    rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].rb_id;

    rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.ciphering[i].ciph_algo = 
    rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].ciph_algo;

    rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.ciphering[i].lc_id =
    rrclcm_get_dl_rlc_lc_id(wrrc_as_id,rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.
                            ciphering[i].rb_id);

    rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.ciphering[i].ciph_act_sn =
    rrcsmc_int_data.ciphering_info.dl_ciph_act_info.sq_num_for_complete
    [rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].rb_id];
  }
}
/*===========================================================================
...
)
{
  ...
  rrcsmc_int_data.start_value_to_transmit = rrcsmc_get_max_start_per_domain(wrrc_as_id,rrcsmc_int_data.start_value_to_transmit_domain);
  if(START_THRESHOLD == rrcsmc_int_data.start_value_to_transmit)
  {
    //don't increment the start value, ref TS25.331 ch 8.5.9
    WRRC_MSG1_HIGH("Max Start Value=0x%x reached the Threshold",rrcsmc_int_data.start_value_to_transmit);
    is_start_val_reached_threshold = TRUE;
  }
  else
  {
    /*increment by 2*/
    rrcsmc_int_data.start_value_to_transmit = rrcsmc_int_data.start_value_to_transmit+2;    
  }
  rrcsmc_int_data.start_value_to_transmit = (rrcsmc_int_data.start_value_to_transmit << 12);
  ...
  if(rrcsmc_int_data.start_value_to_transmit_domain == RRC_CS_DOMAIN_CN_ID)
  {
    rrcsmc_int_data.mac_start_value_to_transmit = rrcsmc_int_data.start_value_to_transmit;
    ...
    if((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5) && 
       (rrcllc_timing_init_in_progress(wrrc_as_id)) &&
       (RRC_STATE_CELL_DCH == rrc_get_state(wrrc_as_id)) &&
       (OC_SET_FOR_CELL_DCH == rrcllc_get_ordered_config_status(wrrc_as_id)) &&
        !(is_start_val_reached_threshold))
    {
      rrcsmc_int_data.mac_start_value_to_transmit = rrcsmc_int_data.mac_start_value_to_transmit >> RRC_MAC_HFN_OFFSET;
      rrcsmc_int_data.mac_start_value_to_transmit += 1;
      rrcsmc_int_data.mac_start_value_to_transmit = rrcsmc_int_data.mac_start_value_to_transmit << RRC_MAC_HFN_OFFSET;

      WRRC_MSG1_HIGH("Incremented mac_start_value_to_transmit by 1 for TI HHO condition. Mac Start Value 0x%x",rrcsmc_int_data.mac_start_value_to_transmit);
    }
  }
  ...
  for (i=0; i< rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.num_rb; i++)
  {
    ...
    if ((rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[i].rb_status == CIPH_FIRST_TIME) &&
        (rrcsmc_int_ciph_info.ciph_status[rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[i].domain_id] 
         == RRCSMC_SECURITY_STARTED))
    {
      /*set rb id and lc_id*/
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].rb_id = 
      ...
      /*we will start ciphering immediately for rlc rb's, so set act sequence
        number to zero*/
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].ciph_act_sn = 0;
      ...
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].hfn = 
      (rrcsmc_int_data.start_value_to_transmit >> RRC_RLC_AM_HFN_OFFSET);

      /*if SRB get cipher key id of latest configured cn domain, otherwise
        set key id of the domain for which this RB belongs*/
      if (rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[i].is_rb_srb == TRUE)
      {
        rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].cipher_key_id = 
        rrcsmc_get_ciph_key_idx(wrrc_as_id,rrcsmc_int_ciph_info.latest_config_cn_id);
      }
      else
      {
        rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].cipher_key_id = 
  ...
  num_ul_rb_added = rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.nchan;

  for (i=0; i<rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.num_rb; i++,num_ul_rb_added++)
  {
    ...
    /*only if starting ciphering for this RB, we should set ciphering parameters.
      ...
      being STARTED or RESTARTED.*/
    if ((rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[i].rb_status == CIPH_FIRST_TIME) &&
        (rrcsmc_int_ciph_info.ciph_status[rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[i].domain_id] 
         == RRCSMC_SECURITY_STARTED))    
    {
      /*set rb_id and lc_id*/
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[num_ul_rb_added].rb_id = 
      rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[i].rb_id;
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[num_ul_rb_added].lc_id = 
      rrclcm_get_ul_rlc_lc_id(wrrc_as_id,rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[i].rb_id);

      /*we will start ciphering immediately for rlc rb's, so set act sequence
            * number to zero*/
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[num_ul_rb_added].ciph_act_sn = 0;

      /*set cipher mode to true*/
        rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[num_ul_rb_added].cipher_mode = TRUE;
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[num_ul_rb_added].ciph_algo = 
            rrcsmc_int_ciph_info.saved_ciph_algo[rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[i].domain_id];

      /* Set HFN and offset form UM RB's*/

      /* Legacy Code */
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[num_ul_rb_added].hfn = 
      (rrcsmc_int_data.start_value_to_transmit >> RRC_RLC_UM_HFN_OFFSET);
 

      /*if SRB get cipher key id of latest configured cn domain, otherwise
        set key id of the domain for which this RB belongs*/
      if (rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[i].is_rb_srb == TRUE)
      {
        rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[num_ul_rb_added].cipher_key_id = 
        rrcsmc_get_ciph_key_idx(wrrc_as_id,rrcsmc_int_ciph_info.latest_config_cn_id);
      }
      else
      {
  ...
  for (i=0; i < RRC_MAX_CN_DOMAINS; i++)
  {
    ...
    if ((rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[i].affected == TRUE) &&
        (rrcsmc_int_ciph_info.ciph_status[i] == RRCSMC_SECURITY_STARTED))
    {
      /*set ciphering enable flag to true*/
        rrcsmc_ext_data.cipher_config_db.mac_cipher_info[i].cipher_enable = TRUE;
      rrcsmc_ext_data.cipher_config_db.mac_cipher_info[i].cipher_algo = 
               rrcsmc_int_ciph_info.saved_ciph_algo[i];

      /*set tm ciphering activation time*/
      rrcsmc_ext_data.cipher_config_db.mac_cipher_info[i].ciph_act_time = 
      rrcsmc_int_data.tm_ciph_act_cfn;
      ...
      if(rrcsmc_int_ciph_info.start_value_update_flag[i] == TRUE)
      {
        ...
        rrcsmc_ext_data.cipher_config_db.mac_cipher_info[i].hfn = 
          rrcsmc_convert_START_to_count_c_i(wrrc_as_id,
      ...
      rrcsmc_ext_data.cipher_config_db.mac_cipher_info[i].hfn = 
      (rrcsmc_ext_data.cipher_config_db.mac_cipher_info[i].hfn >> RRC_MAC_HFN_OFFSET);

      /*get key index*/
      rrcsmc_ext_data.cipher_config_db.mac_cipher_info[i].key_idx =
      rrcsmc_get_ciph_key_idx(wrrc_as_id,(rrc_cn_domain_identity_e_type) i);
    }
  }
...
)
{
  ...
  rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[domain_id].affected = FALSE;
  rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[domain_id].num_rb = 0;
  rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[domain_id].rb_status = CIPH_NOT_STARTED;
  
  /* set ciphering enabled flag to false */
  rrcsmc_ext_data.cipher_config_db.mac_cipher_info[domain_id].cipher_enable = FALSE;
...
(
  ...
  rrc_cn_domain_identity_e_type  domain_id_list[MAX_RB+1],
...
)
{
  ...
  uint32 local_hfn_per_ul_rb[MAX_RB+1],local_hfn_per_dl_rb[MAX_RB+1];
  ...
  (void) rlc_rrc_get_hfn_req(wrrc_as_id, local_hfn_type_ptr);
    ...
    WRRC_MSG4_HIGH("Max count c before releasing RB for cs domain = %x ps domain = %x  ul_nchan: %d, dl_nchan: %d",
    rrcsmc_int_data.max_count_c.count_c_cs,rrcsmc_int_data.max_count_c.count_c_ps,
    local_hfn_type_ptr->ul_nchan,local_hfn_type_ptr->dl_nchan
   );
   

  /*store the hfn values for each dl UM/AM RBs*/
  for(i=0;i<(MAX_RB+1);i++)
  {
  ...
  {
    local_lc_id=local_hfn_type_ptr->hfn_per_rb_dl[i].rlc_id;
    local_rb_id=rrclcm_get_dl_rb_id(wrrc_as_id,local_lc_id);
    ...
    switch(local_rb_id)
    {
      ...
      default:
      {
        ...
        {
          /* Now processing RB's */
          if(rrc_find_cn_domain_id_for_rb_in_est_rabs(wrrc_as_id,local_rb_id, &local_cn_domain_id) == TRUE)
          {
          ...
          if(rrclcm_get_dl_rlc_mode(wrrc_as_id,local_lc_id, &local_mode)==RRCLCM_SUCCESS)
          {
  ...
  {
    local_lc_id=local_hfn_type_ptr->hfn_per_rb_ul[i].rlc_id;
    local_rb_id=rrclcm_get_ul_rb_id(wrrc_as_id,local_lc_id);
     ...
     case CCCH_RADIO_BEARER_ID:
      ...
      default:
      {
        if(local_rb_id <= MAX_RB)
        {
          ...
          if(rrclcm_get_ul_rlc_mode(wrrc_as_id,local_lc_id, &local_mode)==RRCLCM_SUCCESS)
          {
            if(local_mode==UE_MODE_UNACKNOWLEDGED)
            {
  ...
  if(proc_id == RRC_PROCEDURE_RBE || proc_id == RRC_PROCEDURE_RBR || proc_id == RRC_PROCEDURE_CU)
  {
    ...
    {
      local_rb_id = rrclcm_get_ul_rb_id(wrrc_as_id,rlc_req_type_ptr->ul_tm.chan_info[i].lc_id);
      /* Processing SRB's first skipping SRB0*/
      switch(local_rb_id)
      {
        case CCCH_RADIO_BEARER_ID:
    ...
    {
      local_rb_id = rrclcm_get_dl_rb_id(wrrc_as_id,rlc_req_type_ptr->dl_tm.chan_info[i].lc_id);
      /* Processing SRB's first skipping SRB0*/
      switch(local_rb_id)
      {
        case CCCH_RADIO_BEARER_ID:
    ...
    {
      local_rb_id=rrclcm_get_ul_rb_id(wrrc_as_id,rlc_req_type_ptr->ul_um.chan_info[i].lc_id);
    ...
    {
      local_rb_id=rrclcm_get_dl_rb_id(wrrc_as_id,rlc_req_type_ptr->dl_um.chan_info[i].lc_id);
  ...
  rrcsmc_int_data.max_count_c.count_c_cs=(rrcsmc_int_data.max_count_c.count_c_cs>max_count_c_cs)?
                          rrcsmc_int_data.max_count_c.count_c_cs:max_count_c_cs;
  rrcsmc_int_data.max_count_c.count_c_ps=(rrcsmc_int_data.max_count_c.count_c_ps>max_count_c_ps)?
                          rrcsmc_int_data.max_count_c.count_c_ps:max_count_c_ps;

  WRRC_MSG2_HIGH("Max count c for released RB for cs domain = %x ps domain = %x ",rrcsmc_int_data.max_count_c.count_c_cs,rrcsmc_int_data.max_count_c.count_c_ps);
...
if(rrcsmc_ext_data.nv_ciphering_enabled)
{
  ...
  if(proc_id == RRC_PROCEDURE_RBE || proc_id == RRC_PROCEDURE_RBR || proc_id == RRC_PROCEDURE_CU)
  {
     WRRC_MSG4_HIGH("N_RB %d, S_V_T %d, N_TM %d CS_TM_AFF %d ",
              rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[0].num_rb, 
              rrcsmc_int_data.START_VALUE_TO_TRANSMIT_FLAG,
      rrcsmc_int_data.NEW_TM_RB_IND_FLAG,
      rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[0].affected
   );
  ...
  if (proc_id == RRC_PROCEDURE_RBR || proc_id == RRC_PROCEDURE_CU || proc_id == RRC_PROCEDURE_RBE)
  {
    ...
    memset (local_domain_id_list, (int)RRC_CS_DOMAIN_CN_ID, (MAX_RB+1) * sizeof(rrc_cn_domain_identity_e_type));
    ...
    rrcsmc_calc_max_count_c_released_rb(wrrc_as_id,rlc_req_type_ptr,local_domain_id_list,proc_id);
    ...
    for (i=0; i < rlc_req_type_ptr->ul_tm.nchan; i++)
    {
      ...
      if(local_rb_id <= MAX_RB)
      {
        ...
        if(rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[local_domain_id].num_rb != 0)
        {
          ...
          if(local_rb_id != 0)
          {
            ...
            else
            {
              ...
              {
                rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[local_domain_id].num_rb--;

                if (rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[local_domain_id].num_rb == 0)
                {
                  rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[local_domain_id].affected = FALSE;
                  rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[local_domain_id].rb_status = CIPH_NOT_STARTED;
                  rrcsmc_ext_data.cipher_config_db.mac_cipher_info[local_domain_id].cipher_enable = FALSE;
                }
              }
            }
          }
  ...
  if ( proc_id == RRC_PROCEDURE_RBE )
  {
    ...
    local_est_rab_ptr = rrcrbe_get_est_rab_ptr(wrrc_as_id);
    ...
    for (i=0; (i < rlc_req_type_ptr->am.nentity) && (rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.num_rb < UE_MAX_AM_ENTITY); i++)
    {
      ...
      rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.num_rb].rb_id =
      ...
      {
        rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.num_rb].
        is_rb_srb=TRUE;
        rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.num_rb].
      ...
      if((rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.num_rb].
          is_rb_srb == TRUE)&&
         (rrcsmc_get_security_start_status(wrrc_as_id) == TRUE)
        )
      {
        ...
        rrcsmc_int_data.START_VALUE_TO_TRANSMIT_FLAG = TRUE;
        rrcsmc_int_data.start_value_to_transmit_domain =  rrcsmc_int_ciph_info.latest_config_cn_id;
        ...
        if(rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.num_rb].rb_id
           == DCCH_DT_LOW_PRI_RADIO_BEARER_ID)
        {
          ...
          (is_srb4_est_and_security_started == FALSE))
      {
        /*this flag is queried by the reconfig procedure and indicates that new 
          START values need to be transmitted as AM/UM radio bearers are setup*/
        rrcsmc_int_data.START_VALUE_TO_TRANSMIT_FLAG = TRUE;
        rrcsmc_int_data.start_value_to_transmit_domain = RRC_PS_DOMAIN_CN_ID;
      }

      /*initialize ciph first time and increment the number of rb's counter*/
      rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.
      num_rb].rb_status = CIPH_FIRST_TIME;
      rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.num_rb++;
      ...
      if(rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.num_rb == UE_MAX_AM_ENTITY)
      {
    ...
    for (i=0; (i< rlc_req_type_ptr->ul_um.nchan) && (rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.num_rb < UE_MAX_UL_UM_CHANNEL); i++)
    {
      ...
      rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.num_rb].rb_id = 
      (uint8) rrclcm_get_ul_rb_id(wrrc_as_id,rlc_req_type_ptr->ul_um.chan_info[i].lc_id);

      /*check if SRB or not and set the flag appropriately*/
      if (rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.num_rb].
          rb_id == DCCH_UM_RADIO_BEARER_ID )
      {
        rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.num_rb].
        is_rb_srb=TRUE;
      ...
      rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.num_rb++;
      ...
      if(local_rb_id <= MAX_RB)
      {
        ...
        if(rrcsmc_int_ciph_info.ciph_status[local_domain_id] == RRCSMC_SECURITY_STARTED)
        {
          /*if TM rb's are being configured for the first time, set affected flag to
            TRUE*/
          if (rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[local_domain_id].affected != TRUE)
          {
            rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[local_domain_id].affected = TRUE;
            rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[local_domain_id].rb_status = CIPH_FIRST_TIME;
          }
          ...
          /*If SRB-4 is getting set up through this RBsetup msg & LATEST CONFIGURED CN DOMAIN
           ...
           If so skip setting start_value_to_transmit_domain as its taken care in the handling 
           for SRB-4 above */
          if ((rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[local_domain_id].num_rb == 1)&&
              (is_srb4_est_and_security_started == FALSE))
          {
            /*If a new TM Radio Bearer is being added, then set this flag for the
              reconfig procedure to query TM RB ciphering act CFN.
             */
            rrcsmc_int_data.NEW_TM_RB_IND_FLAG = TRUE;      
    
            /*this flag is queried by the reconfig procedure and indicates that new 
              START values need to be transmitted as AM/UM radio bearers are setup*/
            rrcsmc_int_data.START_VALUE_TO_TRANSMIT_FLAG = TRUE;
            rrcsmc_int_data.start_value_to_transmit_domain = RRC_CS_DOMAIN_CN_ID;
          }
        }
        else
        {
          ...
          /*If SRB-4 is getting set up through this RBsetup msg & LATEST CONFIGURED CN DOMAIN
           Is initialized then N/W should take care of associating CN domain of any RB getting set 
           up with this message with  LATEST CONFIGURED CN DOMAIN as per 8.2.2.2. 
           If so skip setting rrcsmc_int_data.start_value_to_transmit_domain as its taken care in the handling 
           for SRB-4 above */
          if((rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[local_domain_id].num_rb == 0)&&
             (is_srb4_est_and_security_started == FALSE))
          {
    ...
    /*update the global ciphering database which will be used by LLC to configure
      RLC/MAC/L1*/
    rrcsmc_update_global_ciph_db_for_reconfig(wrrc_as_id);
...
if(rrcsmc_ext_data.nv_ciphering_enabled)
{
  ...
  local_latest_config_cn_domain = rrcsmc_get_latest_configured_cn_domain(wrrc_as_id);
  
  /*hardcode ciphering configuration for RB 2*/
  rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[count].is_rb_srb = TRUE;
  rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[count].domain_id = local_latest_config_cn_domain;

  rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[count].rb_id = 
  DCCH_AM_RADIO_BEARER_ID; 

  rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[count].rb_status = 
  CIPH_FIRST_TIME;

  rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.num_rb++;
  count = rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.num_rb;

  /*hardcode ciphering configuration for RB 3*/
  rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[count].is_rb_srb = TRUE;
  rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.config_info[count].domain_id = 
  ...
  if (RRCLCM_RLC_LC_ID_NOT_FOUND != 
      rrclcm_check_ul_rlc_lc_id(wrrc_as_id,UE_LOGCHAN_DCCH,
                                DCCH_DT_LOW_PRI_RADIO_BEARER_ID, 
  ...
  rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[count].is_rb_srb = TRUE;
  rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[count].domain_id = 
                                                     local_latest_config_cn_domain;

  rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[count].rb_id = 
  DCCH_UM_RADIO_BEARER_ID; 

  rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[count].rb_status = 
  CIPH_FIRST_TIME;
    ...
    if (rrc_est_rabs.rabs[count].rab_id != RRC_INVALID_RAB_ID)
    {
      for (i=0; i< rrc_est_rabs.rabs[count].num_rbs_for_rab; i++)
      {
            ...
            RRC_RB_STARTED)
        {
          ...
          local_lc_id = rrclcm_get_ul_rlc_lc_id(wrrc_as_id,rrc_est_rabs.rabs[count].
                                                rb_for_rab[i].rb_id);
          if(rrc_est_rabs.rabs[count].cn_domain == RRC_CS_DOMAIN_CN_ID)
          {
          ...
          if (local_lc_id < RRCLCM_UL_UM_START_IDX)
          {
            if(local_domain_id ==  local_latest_config_cn_domain)
            {
              ...
              rrcsmc_int_data.ciph_rb_info.tm_rb_ciph_config[local_domain_id].rb_status =
              CIPH_FIRST_TIME;  
                   ...
                   local_lc_id < RRCLCM_UL_AM_DATA_START_IDX+UE_MAX_UL_AM_DATA_CHANNEL)
          {
            ...
            {
              WRRC_ERR1_FATAL("AM num_rb is out or range: %d",rrcsmc_int_data.ciph_rb_info.am_rb_ciph_config.num_rb);
            }
          }
          /*check if lc_id of the Radio Bearer belongs to UM Mode*/
                   ...
                   local_lc_id < RRCLCM_UL_AM_DATA_START_IDX)
          {
            ...
            /*initialize value of UM radio bearers.  set SRB true to false.  set rb id
              cn domain and increment num_rb*/
            if(rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.num_rb < UE_MAX_UL_UM_CHANNEL)
            {
              rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.config_info[rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.num_rb].
              is_rb_srb = FALSE;
            ...
            {
              WRRC_ERR1_FATAL("UM num_rb is out or range: %d",rrcsmc_int_data.ciph_rb_info.um_rb_ciph_config.num_rb);
            }
  ...
  /*update the global ciphering database which will be used by LLC to configure
    RLC/MAC/L1*/
  rrcsmc_update_global_ciph_db_for_ciph_start(wrrc_as_id,local_latest_config_cn_domain
                                              ,config_cause
  );
  ...
  for (j=0; j<RRC_MAX_CN_DOMAINS; j++)
  {
    ...
    for ( i=0; i < RRC_MAX_SRB; i++)
    {
      /*initialize uplink count_i with seq_num = 0 and the default HFN received
        for this domain */
      rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].dl_count_i[i] = 
      rrcsmc_convert_START_to_count_c_i(wrrc_as_id,
                                       rrcsmc_int_ciph_info.int_ciph_info[domain_id].hfn,
                                       init_seq_num);

      /*initialize the downlink init_flag to false*/
      rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].dl_count_i_init_flag[i]=FALSE;

      /*initialize downlink count_i with seq_num = 0 and the default HFN received
        for this domain */      
      rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].ul_count_i[i] =       
      rrcsmc_convert_START_to_count_c_i(wrrc_as_id,
                                       rrcsmc_int_ciph_info.int_ciph_info[domain_id].hfn,
                                       init_seq_num);

      /*init the UL/DL act_time_pending_flag to false*/
      rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].
      dl_count_i_act_time_pending[i] = FALSE;
      rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].
      ul_count_i_act_time_pending[i] = FALSE;

      /*init the UL/DL act_time_pending_flag to false*/
      rrcsmc_int_data.integrity_protection_info.dl_act_seq_num[i] = 0;
      rrcsmc_int_data.integrity_protection_info.ul_act_seq_num[i] = 0;
      ...
      rrcsmc_int_data.integrity_protection_info.dl_latest_config_cn_id_old_1_valid[i] = FALSE;
...
)
{
  ...
  for (i=0; i<RRC_MAX_CN_DOMAINS; i++)
  {
    ...
    if (sim_insert_req_ptr->cipher_info[i].domain_id == RRC_CS_DOMAIN_CN_ID)
    {
      ...
      rrcsmc_reset_saved_security_key_set_status(wrrc_as_id);
    }
    else
    {
      domain_id = RRC_PS_DOMAIN_CN_ID;    
   ...
   *authentication use 1
    ...
    if(rrc_get_test_mode_logging_status(wrrc_as_id) == TRUE)
    {
      WRRC_MSG4_HIGH("INT KEY 1: 0x%x 2: 0x%x 3: 0x%x 4: 0x%x",
    ...
    WRRC_MSG3_HIGH("EST START: 0x%x 0x%x 0x%x", sim_insert_req_ptr->cipher_info[i].hfn[0],
  ...
  rrcsmc_init_integrity_protection_info(wrrc_as_id);
...
===========================================================================*/
static void rrcsmc_ul_purge_wm_except_srb2(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8 count=0;
  rlc_lc_id_type srb4_lc_present = rrclcm_check_ul_rlc_lc_id(wrrc_as_id,UE_LOGCHAN_DCCH,
                                 DCCH_DT_LOW_PRI_RADIO_BEARER_ID, 
                                 UE_MODE_ACKNOWLEDGED_DATA);
  ...
  req_cmd_ptr->cmd_data.ul_purge_wm_ind.lc_id[count] = rrclcm_get_ul_rlc_lc_id(wrrc_as_id, DCCH_UM_RADIO_BEARER_ID);
  ...
  req_cmd_ptr->cmd_data.ul_purge_wm_ind.lc_id[count] = rrclcm_get_ul_rlc_lc_id(wrrc_as_id, DCCH_DT_HIGH_PRI_RADIO_BEARER_ID);
  ...
  if (RRCLCM_RLC_LC_ID_NOT_FOUND != srb4_lc_present)
  {
    ...
    req_cmd_ptr->cmd_data.ul_purge_wm_ind.lc_id[count] = rrclcm_get_ul_rlc_lc_id(wrrc_as_id, DCCH_DT_LOW_PRI_RADIO_BEARER_ID);
    req_cmd_ptr->cmd_data.ul_purge_wm_ind.num_sdus[count++] = MAX_SRB2_SDUS_TOBE_PURGED;
  }
  ...
  rrc_send_l2_ul_cmd(wrrc_as_id,req_cmd_ptr);
...
===========================================================================*/
static void rrcsmc_ul_purge_wm_srb2(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  req_cmd_ptr->cmd_data.ul_purge_wm_ind.lc_id[0] = rrclcm_get_ul_rlc_lc_id(wrrc_as_id, DCCH_AM_RADIO_BEARER_ID);
...
)
{
  ...
  /* Suspend SRB1 */
  rrcsmc_int_data.smc_rb_suspend_or_stop_info.suspend_or_stopped_info.suspend_stop[num_ch_to_suspend_ul].lc_id = 
    rrclcm_get_ul_rlc_lc_id(wrrc_as_id, DCCH_UM_RADIO_BEARER_ID);
  num_ch_to_suspend_ul++;

  /* Suspend SRB2 if this suspension is due to SMC */
  if(RRCSMC_SMC_CMD == suspend_cmd)
  {
    srb2_lc_id = rrclcm_get_ul_rlc_lc_id(wrrc_as_id, DCCH_AM_RADIO_BEARER_ID);
    rrcsmc_int_data.smc_rb_suspend_or_stop_info.suspend_or_stopped_info.suspend_stop[num_ch_to_suspend_ul].lc_id = srb2_lc_id;
    num_ch_to_suspend_ul++;
  }

  /* Suspend SRB3 */
  rrcsmc_int_data.smc_rb_suspend_or_stop_info.suspend_or_stopped_info.suspend_stop[num_ch_to_suspend_ul].lc_id = 
    rrclcm_get_ul_rlc_lc_id(wrrc_as_id, DCCH_DT_HIGH_PRI_RADIO_BEARER_ID);
  num_ch_to_suspend_ul++;

  if(suspend_cmd == RRCSMC_SRNS_CMD)
  {
    rrcsmc_int_data.smc_rb_suspend_or_stop_info.dl_stop_info.lc_id[num_ch_to_stop_dl] = 
      rrclcm_get_dl_rlc_lc_id(wrrc_as_id, DCCH_UM_RADIO_BEARER_ID);
    num_ch_to_stop_dl++;
  
    rrcsmc_int_data.smc_rb_suspend_or_stop_info.dl_stop_info.lc_id[num_ch_to_stop_dl] = 
      rrclcm_get_dl_rlc_lc_id(wrrc_as_id, DCCH_DT_HIGH_PRI_RADIO_BEARER_ID);
    num_ch_to_stop_dl++;
  }
  ...
  {
    rrcsmc_int_data.smc_rb_suspend_or_stop_info.suspend_or_stopped_info.suspend_stop[num_ch_to_suspend_ul].lc_id = 
      rrclcm_get_ul_rlc_lc_id(wrrc_as_id, DCCH_DT_LOW_PRI_RADIO_BEARER_ID);
    num_ch_to_suspend_ul++;

    if(suspend_cmd == RRCSMC_SRNS_CMD)
    {
      rrcsmc_int_data.smc_rb_suspend_or_stop_info.dl_stop_info.lc_id[num_ch_to_stop_dl] = 
        rrclcm_get_dl_rlc_lc_id(wrrc_as_id, DCCH_DT_LOW_PRI_RADIO_BEARER_ID);
      num_ch_to_stop_dl++;
    }
  }
  ...
  for(count = 0; count < MAX_RAB_TO_SETUP; count++)
  {
    if (rrc_est_rabs.rabs[count].rab_id != RRC_INVALID_RAB_ID)
    {
      ...
      if( (suspend_cmd == RRCSMC_SMC_CMD) && 
          (rrcsmc_int_data.saved_security_mode_command_msg.domain_id == RRC_CS_DOMAIN_CN_ID) &&
      ...
      for (i=0; i< rrc_est_rabs.rabs[count].num_rbs_for_rab; i++)
      {
        if(suspend_cmd==RRCSMC_SRNS_CMD)
        {
          local_lc_id = rrclcm_get_dl_rlc_lc_id(wrrc_as_id,rrc_est_rabs.rabs[count].rb_for_rab[i].rb_id);
          if(local_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
          {
            if ( local_lc_id >= RRCLCM_DL_UM_START_IDX && 
                 local_lc_id < (RRCLCM_DL_AM_DATA_START_IDX + UE_MAX_DL_AM_DATA_CHANNEL))
            {
              rrcsmc_int_data.smc_rb_suspend_or_stop_info.dl_stop_info.lc_id[num_ch_to_stop_dl]= (uint8) local_lc_id;
              num_ch_to_stop_dl++;
            }
          }
        }

        local_lc_id = rrclcm_get_ul_rlc_lc_id(wrrc_as_id,rrc_est_rabs.rabs[count].rb_for_rab[i].rb_id);

        /*check if it is a AM or UM RLC RB only*/
        if ( local_lc_id >= RRCLCM_UL_UM_START_IDX && 
             local_lc_id < (RRCLCM_UL_AM_DATA_START_IDX + UE_MAX_UL_AM_DATA_CHANNEL) )
        {
          rrcsmc_int_data.smc_rb_suspend_or_stop_info.suspend_or_stopped_info.suspend_stop[num_ch_to_suspend_ul].lc_id = (uint8) local_lc_id;
          num_ch_to_suspend_ul++;
        }
      }
    }
  ...
  rrcsmc_int_data.smc_rb_suspend_or_stop_info.suspend_or_stopped_info.nchan= (uint8) num_ch_to_suspend_ul;
  rrcsmc_int_data.smc_rb_suspend_or_stop_info.dl_stop_info.nchan = (uint8) num_ch_to_stop_dl;

  /*this cmd initiates suspension of RLC radio bearers*/

  /* Fill in the command parameters for SUSPENDING RLC RB's */
  rrcsmc_ul_req.nchan = 
      rrcsmc_int_data.smc_rb_suspend_or_stop_info.suspend_or_stopped_info.nchan;

  rrcsmc_dl_stop_req.nchan= 
      rrcsmc_int_data.smc_rb_suspend_or_stop_info.dl_stop_info.nchan;

  /* In a loop set the contents of the suspend_req */
  for (i = 0; i < rrcsmc_int_data.smc_rb_suspend_or_stop_info.suspend_or_stopped_info.nchan; i++)
  {
    rrcsmc_ul_req.suspend_stop[i].lc_id = 
     rrcsmc_int_data.smc_rb_suspend_or_stop_info.suspend_or_stopped_info.suspend_stop[i].lc_id;
    ...
    */ 
       ...
       (RRCSMC_SMC_CMD == suspend_cmd))
    {
      ...
      pending_status = rlc_get_srb_ul_ciph_status
       ...
       WRRC_MSG4_HIGH("SRB2 Ciph Status: pending_status %d Ciph Status: vt_s %d, act_sn %d, wm_num_rlc_pdus %d",
      ...
      else // there is a pending config
      {
        ...
        if(rrcnv_data.rrc_set_srb2_act_time_nv == FALSE)
        {
  ...
  for (i=0; (i < rrcsmc_int_data.smc_rb_suspend_or_stop_info.dl_stop_info.nchan) && (i < (UE_MAX_DL_UM_CHANNEL + UE_MAX_AM_ENTITY)); i++)
  {
    rrcsmc_dl_stop_req.lc_id[i] = 
    rrcsmc_int_data.smc_rb_suspend_or_stop_info.dl_stop_info.lc_id[i];
  }
  ...
  if(suspend_cmd == RRCSMC_SRNS_CMD)
  {
    ...
    rrcsmc_ul_purge_wm_srb2(wrrc_as_id);
  }
  ...
  crlc_ul_perform_suspend_or_stop_functionality(wrrc_as_id, &rrcsmc_ul_req,
                                   &rrcsmc_ul_cnf);

  /*Call RLC fun here  for  stoping DL(AM & UM) RB's & SRB's*/
  if(suspend_cmd == RRCSMC_SRNS_CMD)
  {
    crlc_dl_perform_stop_functionality(wrrc_as_id, &rrcsmc_dl_stop_req);
  }
  
  if(suspend_cmd == RRCSMC_SMC_CMD)
  {
    /*process suspend cnf for SMC & send SMC complete*/
    rrcsmc_process_suspend_cnf(wrrc_as_id,&rrcsmc_ul_cnf);
  }
  else
  {
    /*process suspend cnf for SRNS relocation*/
    rrcsmc_process_suspend_srns_cnf(wrrc_as_id);
  }  
}
...
)
{
  ...
  rrc_state_e_type rrc_state = rrc_get_state(wrrc_as_id); /* local var to store rrc state*/
    ...
    )
  {
    ...
    else
    {
      ...
      if(rrcsmc_return_saved_security_key_set_status(wrrc_as_id,domain_id) == TRUE)
      {
        WRRC_MSG1_HIGH("New CS key rexed invalidating saved security key set=%d",
                                                                           rrcsmc_int_data.saved_security_key_set.grat_key_set_valid);
        rrcsmc_reset_saved_security_key_set_status(wrrc_as_id);
      }
      ...
      if ( rrcsmc_int_data.integrity_protection_info.ip_status == RRCSMC_SECURITY_STARTED )
      {
      ...
      else
      {
          ...
          )
        {
          ...
          WRRC_MSG3_HIGH("Domain:%d RRC State:%d SIM Update Status:%d", 
            domain_id,
            rrc_state,
            sim_update_req_ptr->security_update_info.security_update_status);
        }
    
        WRRC_MSG3_HIGH("CIPH START VAL: 0x%x 0x%x 0x%x", 
    ...
    */
     if (((rrc_state == RRC_STATE_CELL_DCH) ||
         ...
         && (!((sim_update_req_ptr->security_update_info.security_update_status 
            == RRC_SIM_UPDATE_VALUE_LTE_MAPPED) && (RRC_PS_DOMAIN_CN_ID == domain_id)
         && (rrcsmc_int_data.integrity_protection_info.ip_status == RRCSMC_SECURITY_STARTED )))
#endif
      )
      {
	  ...
	  if ( rrcsmc_int_data.integrity_protection_info.ip_status == RRCSMC_SECURITY_NOT_STARTED )
	  {
...
)
{
  ...
  max_start = rrcsmc_get_max_start_per_domain(wrrc_as_id,domain_id);

  if(domain_id == RRC_CS_DOMAIN_CN_ID)
  {
    if((rrcsmc_int_data.start_value_to_transmit_cs_hho) &&
        (rrcsmc_int_data.start_value_to_transmit_cs_hho >= max_start))
    {
      WRRC_MSG2_HIGH("Updating max_start:0x%x with SVTHHO:0x%x",
        max_start, rrcsmc_int_data.start_value_to_transmit_cs_hho);
      max_start = rrcsmc_int_data.start_value_to_transmit_cs_hho;
    }
    /* Reset SVTHHO */
    rrcsmc_int_data.start_value_to_transmit_cs_hho = 0;
  ...
  {
    prev_max_start = 
    rrcsmc_convert_START_to_count_c_i(wrrc_as_id,rrcsmc_int_ciph_info.int_ciph_info[domain_id].hfn_latest, 
                                      init_seq_num);
  }
  else
  {
    prev_max_start = 
    rrcsmc_convert_START_to_count_c_i(wrrc_as_id,rrcsmc_int_ciph_info.int_ciph_info[domain_id].hfn, 
                                      init_seq_num);
  }
  ...
  {
    rrcsmc_convert_count_c_i_to_START(wrrc_as_id,local_start_value, max_start);
...
)
{
  ...
  rrcsmc_int_data.extract_cause_rel_ind=TRUE;
  if(rrcsmc_int_ciph_info.int_start_status_for_conn[RRC_CS_DOMAIN_CN_ID])
  {
    rrcsmc_extract_latest_start_list_per_cn_domain(wrrc_as_id,RRC_CS_DOMAIN_CN_ID,
  ...
  {
    rrcsmc_extract_latest_start_list_per_cn_domain(wrrc_as_id,RRC_PS_DOMAIN_CN_ID,
                                                   RRCSMC_UPDATE_START_LIST);     
  }

  rrcsmc_int_data.extract_cause_rel_ind=FALSE;
  ...
  if(rrcsmc_int_data.integrity_protection_info.count_i_info[RRC_CS_DOMAIN_CN_ID].
  ...
  if(rrcsmc_int_data.integrity_protection_info.count_i_info[RRC_PS_DOMAIN_CN_ID].
     dl_new_key_flag_per_rb[DCCH_AM_RADIO_BEARER_ID] == TRUE)
  {
  ...
  if(rrcsmc_int_data.saved_security_key_set.pending_cs_key_to_be_activated)
  {
    rrcsmc_int_ciph_info.int_ciph_info[RRC_CS_DOMAIN_CN_ID].hfn[0]=0x0;
    rrcsmc_int_ciph_info.int_ciph_info[RRC_CS_DOMAIN_CN_ID].hfn[1]=0x0;
    rrcsmc_int_ciph_info.int_ciph_info[RRC_CS_DOMAIN_CN_ID].hfn[2]=0x0;
  ...
  WRRC_MSG6_HIGH("REL CS START: 0x%x 0x%x 0x%x REL PS START: 0x%x 0x%x 0x%x",
  ...
  rrcsmc_go_to_smc_initial_substate(wrrc_as_id);
...
)
{
  ...
  if(TRUE == rrcsmc_int_ciph_info.int_ciph_info[domain_id].ciph_keys_info.new_key_for_start_value)
  {
    ...
    if(domain_id == RRC_CS_DOMAIN_CN_ID)
    {
      WRRC_MSG3_HIGH("Appended CS-START: 0x%x 0x%x 0x%x", value[0], value[1], value[2]);
    }
    else
    {
      WRRC_MSG3_HIGH("Appended PS-START: 0x%x 0x%x 0x%x", value[0], value[1], value[2]);
    }  
  }
  else
  {
    rrcsmc_append_start_per_domain(wrrc_as_id,value,
             domain_id,
             start_type);
  }
...
)
{
  ...
  else
  {
    ...
    {
      rrcsmc_extract_latest_start_list_per_cn_domain(wrrc_as_id,domain_id,
...
)
{
  ...
  status =  rrcsmc_append_start_value_to_start_list (wrrc_as_id, start_list_ext,
                                                     start_type
...
)
{
  ...
  if(rrcidt_is_idt_waiting_for_l2ack_per_domain(wrrc_as_id,rrcsmc_int_ciph_info.latest_config_cn_id))
  {
    WRRC_MSG1_HIGH("IDT L2_ACK after SYNC_IND for %d",
             rrcsmc_int_ciph_info.latest_config_cn_id);

    rrcidt_send_open_session_cnf(wrrc_as_id,RRC_OPEN_SESS_SUCCESS,
                                 rrcsmc_int_ciph_info.latest_config_cn_id);
  ...
  /* Allocate memory for RRC_SYNC_IND command */
    ...
    WRRC_MSG3_HIGH("Latest CN ID=%d,sync_ind_sent status for CS=%d,PS=%d",
        rrcsmc_int_ciph_info.latest_config_cn_id,
        rrcsmc_int_ciph_info.is_sync_ind_sent_to_nas[RRC_CS_DOMAIN_CN_ID],
        rrcsmc_int_ciph_info.is_sync_ind_sent_to_nas[RRC_PS_DOMAIN_CN_ID]);
...
)
{
  ...
  WRRC_MSG2_HIGH("Reverting to back-up of cipher_config_db. Before reverting MAC-config-type:(CS : %d, PS: %d) ",
    rrcsmc_ext_data.cipher_config_db.mac_cipher_info[CS_DOMAIN].cipher_cfg_type,
    rrcsmc_ext_data.cipher_config_db.mac_cipher_info[PS_DOMAIN].cipher_cfg_type);
  /* Taking back up of global variable which is used to send ciphering configuraiton to MAC */
  rrcsmc_ext_data.cipher_config_db = backup_of_security_variables.cipher_config_db;
  ...
  for(i=0; i<rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.nchan; i++)
  {
    ...
    rrcsmc_get_rlc_ul_info(wrrc_as_id,local_hfn_type_ptr, 
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].lc_id, 
      &hfn,
      &ciph_act_sn,
      &hfn_valid);
    ...
    if(rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].ciph_act_sn <= ciph_act_sn)
    {
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].ciph_act_sn = ciph_act_sn;
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].hfn = hfn;
    }
    ...
    *     No need to change anything. Backup of rrcsmc_ext_data.cipher_config_db will take care.
    */  
    else if(rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].ciph_act_sn > ciph_act_sn)
    {
      //if hfn at RLC and RRC are NOT equal then it is roll over case. Need to apply immediate config
      if((rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].hfn != hfn) &&
         (hfn_valid == TRUE))
      {
        rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].ciph_act_sn = ciph_act_sn;
        rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].hfn = hfn;
    ...
    WRRC_MSG3_HIGH("UL_DBG:Cipher DB: LC:%d HFN:0x%x SN:%d",rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].lc_id,
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].hfn, 
      rrcsmc_ext_data.cipher_config_db.rlc_ul_cipher_info.ciphering[i].ciph_act_sn);
    
  }

  for(i=0; i<rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.nchan; i++)
  {
    hfn = 0;
    ciph_act_sn = 0;
    hfn_valid = FALSE;

    rrcsmc_get_rlc_dl_info(wrrc_as_id,local_hfn_type_ptr, 
      rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.ciphering[i].lc_id,
      &hfn,
      &ciph_act_sn,
      &hfn_valid);
    ...
    /* If sn at RLC > RRC then activation time already hit. Send act sn as Immediate. */
    if(rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.ciphering[i].ciph_act_sn <= ciph_act_sn)
    {
      rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.ciphering[i].ciph_act_sn = ciph_act_sn;
      rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.ciphering[i].hfn = hfn;
    }
    else if(rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.ciphering[i].ciph_act_sn > ciph_act_sn)
    {
      //if hfn at RLC and RRC are NOT equal then it is roll over case. Need to apply immediate config
      if((rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.ciphering[i].hfn != hfn) &&
         (hfn_valid == TRUE))
      {
        rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.ciphering[i].ciph_act_sn = ciph_act_sn;
        rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.ciphering[i].hfn = hfn;
    ...
    WRRC_MSG3_HIGH("DL_DBG:Cipher DB: LC:%d HFN:0x%x SN:%d",rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.ciphering[i].lc_id,
      rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.ciphering[i].hfn, 
      rrcsmc_ext_data.cipher_config_db.rlc_dl_cipher_info.ciphering[i].ciph_act_sn);    
  }
  rrc_free(local_hfn_type_ptr);
...
)
{
  ...
  WRRC_MSG2_HIGH("Taking back-up of cipher_config_db. Old MAC-config-type:(CS : %d, PS: %d) ",
    ...
    rrcsmc_ext_data.cipher_config_db.mac_cipher_info[PS_DOMAIN].cipher_cfg_type);
  /* Taking back up of global variable which is used to send ciphering configuraiton to MAC */
  backup_of_security_variables.cipher_config_db = rrcsmc_ext_data.cipher_config_db;
...
{
  /* Taking back up of global variable which is used to send ciphering configuraiton to MAC */
  backup_of_security_variables.ciphering_info = rrcsmc_int_data.ciphering_info;
  backup_of_security_variables.cipher_config_db = rrcsmc_ext_data.cipher_config_db;
  backup_of_security_variables.ciph_rb_info = rrcsmc_int_data.ciph_rb_info;
  backup_of_security_variables.integrity_protection_info = rrcsmc_int_data.integrity_protection_info;
  backup_of_security_variables.new_tm_rb_ind_flag = rrcsmc_int_data.NEW_TM_RB_IND_FLAG;
  backup_of_security_variables.rrcsmc_int_ciph_info_bkp = rrcsmc_int_ciph_info;
  backup_of_security_variables.start_value_to_transmit_flag = rrcsmc_int_data.START_VALUE_TO_TRANSMIT_FLAG;
  /* reset the global variable */
  rrcsmc_int_data.is_revert_to_old_cipher_variable_reqd = FALSE;
}
...
)
{
  ...
  if ((RRCSMC_WAIT_FOR_CIPH_CONFIG_CNF != rrcsmc_substate) || 
      (TRUE == rrcsmc_int_data.is_revert_to_old_cipher_variable_reqd)
     )
  {
    ...
    if (FALSE == rrcsmc_int_data.is_revert_to_old_cipher_variable_reqd)
    {
      ...
      */
      ...
      new_sn = rrcsmc_int_data.integrity_protection_info.count_i_info[local_cn_domain].ul_count_i[2] & 0xF;
    
      local_cn_domain = backup_of_security_variables.rrcsmc_int_ciph_info_bkp.latest_config_cn_id;
      ...
      WRRC_MSG3_HIGH("Reverting to old Integrity variables. cnt-i-SRB2: prev: %d, cur: %d, after: %d", 
        backup_of_security_variables.integrity_protection_info.
                                       count_i_info[local_cn_domain].ul_count_i[2], 
           rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id].ul_count_i[2], old_ul_count_i);
      ...
      *temp_rrcsmc_int_ciph_info_ptr = rrcsmc_int_ciph_info;
      ...
      rrcsmc_int_ciph_info = backup_of_security_variables.rrcsmc_int_ciph_info_bkp;

      /* copy the saved rrcsmc_int_ciph_info to backup variable */
      backup_of_security_variables.rrcsmc_int_ciph_info_bkp= *temp_rrcsmc_int_ciph_info_ptr;

      rrcsmc_int_data.integrity_protection_info = backup_of_security_variables.integrity_protection_info;
    }
    ...
    /* Save the value of current cipher_config_db because this will be put into backup_* 
       ...
       to RLC */
    *temp_cipher_config_db_ptr = rrcsmc_ext_data.cipher_config_db;
  
    rrcsmc_int_data.ciphering_info = backup_of_security_variables.ciphering_info;
    rrcsmc_ext_data.cipher_config_db =  backup_of_security_variables.cipher_config_db;
    rrcsmc_int_data.ciph_rb_info =  backup_of_security_variables.ciph_rb_info;
    rrcsmc_int_data.NEW_TM_RB_IND_FLAG = backup_of_security_variables.new_tm_rb_ind_flag;
    rrcsmc_int_data.START_VALUE_TO_TRANSMIT_FLAG = backup_of_security_variables.start_value_to_transmit_flag;
  ...
  else
  {
    ...
    */
    ...
    WRRC_MSG3_HIGH("Reverting to old Integrity variables. cnt-i-SRB2: prev: %d, cur: %d, after: %d | Setting is_revert_to_old_cipher_variable_reqd to TRUE", 
        backup_of_security_variables.integrity_protection_info.
                                     count_i_info[local_cn_domain].ul_count_i[2], 
         rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id].ul_count_i[2], old_ul_count_i);
    ...
    *temp_rrcsmc_int_ciph_info_ptr = rrcsmc_int_ciph_info;
    ...
    rrcsmc_int_data.is_revert_to_old_cipher_variable_reqd = TRUE;
  }
...
)
{
  ...
  step_2_ciph_act_time = (uint8)((wl1_seq_get_cfn(wrrc_as_id) + 250) % 256);
  ...
  step_2_start = rrcsmc_get_latest_transmitted_start_value_per_domain(wrrc_as_id, RRC_CS_DOMAIN_CN_ID);
  ...
  rrcsmc_ext_data.cipher_config_db.mac_cipher_info[RRC_CS_DOMAIN_CN_ID].
    cipher_cfg_type = NON_HO_STEP2_CIPHER_CFG;
    
  rrcsmc_ext_data.cipher_config_db.mac_cipher_info[RRC_CS_DOMAIN_CN_ID].hfn = 
    step_2_hfn;
    
  rrcsmc_ext_data.cipher_config_db.mac_cipher_info[RRC_CS_DOMAIN_CN_ID].ciph_act_time = 
    step_2_ciph_act_time;
      
  /*call llc function to update rlc/mac ciphering configuration for
    starting STEP_2 ciphering*/
  rrcllc_update_ciphering_config_info_for_first_tm_rb(wrrc_as_id);
...
)
{
  ...
  rrcsmc_backup_cipher_config_db(wrrc_as_id);
  ...
  //update MAC cipher DB for ciph algo
    rrcsmc_ext_data.cipher_config_db.mac_cipher_info[RRC_CS_DOMAIN_CN_ID].cipher_algo = 
  ...
  {
    rrcsmc_ext_data.cipher_config_db.mac_cipher_info[RRC_CS_DOMAIN_CN_ID].hfn = 
    (rrcsmc_get_latest_transmitted_start_value_per_domain(wrrc_as_id, RRC_CS_DOMAIN_CN_ID) << 4);
  }
  else
  {
    WRRC_MSG1_HIGH("mac d hfn presetn, updating cipher db, %d", mac_d_hfn);
    rrcsmc_ext_data.cipher_config_db.mac_cipher_info[RRC_CS_DOMAIN_CN_ID].hfn =mac_d_hfn;
  }
...
)
{
  ...
  if(rrcsmc_ext_data.cipher_config_db.mac_cipher_info[RRC_CS_DOMAIN_CN_ID].cipher_enable != TRUE)
  {
  ...
  (void)rrcsmc_update_start_value_to_transmit_cs_hho(wrrc_as_id);

  WRRC_MSG1_HIGH("HHO Step1 HFN:0x%x", rrcsmc_ext_data.cipher_config_db.mac_cipher_info[RRC_CS_DOMAIN_CN_ID].hfn);
...
)
{
  ...
  /*call llc function to update rlc/mac ciphering configuration for
    starting STEP_2 ciphering*/
  rrcllc_update_ciphering_config_info_for_handover(wrrc_as_id);
...
{
  rrcsmc_int_data.start_value_to_transmit_cs_hho = (rrcsmc_get_max_start_per_domain(wrrc_as_id, RRC_CS_DOMAIN_CN_ID));
  WRRC_MSG1_HIGH("updating SVTHHO:0x%x",rrcsmc_int_data.start_value_to_transmit_cs_hho);
}
...
)
{
  ...
  rrcsmc_int_data.saved_srns_info.ciph_algo_present     = FALSE;
  rrcsmc_int_data.saved_srns_info.fresh_present         = FALSE;
  rrcsmc_int_data.saved_srns_info.dpch_act_time_present = FALSE;    
  ...
  if(srns_info->integrity_info_present)
  {
    ...
    rrcsmc_int_data.saved_srns_info.fresh_present = TRUE;
    
    /*get the fresh value*/
    rrcsmc_int_data.saved_srns_info.fresh = b_unpackd(
    ...
    {
      rrcsmc_int_data.saved_srns_info.integrity_algo_present = TRUE;
      rrcsmc_int_data.saved_srns_info.integrity_algo = rrcsmc_get_int_integrity_algo_from_dl_sdu_ind(wrrc_as_id,
                &srns_info->integrity_protection_mode_info->integrityProtectionAlgorithm,
                MSG_REL7);
      WRRC_MSG1_HIGH("Integrity Algo present in SRNS info,Algo=%d[0:Invalid Algo,1:UIA1,2:UIA2]",rrcsmc_int_data.saved_srns_info.integrity_algo);
    }
  }
  ...
  if (srns_info->ciphering_mode_info_present)
  {
    if(rrcsmc_get_int_cipher_algo_from_dl_sdu_ind
        ( wrrc_as_id, &(srns_info->ciphering_mode_info->cipheringModeCommand.startRestart),
             msg_version)
        == rrcsmc_int_data.saved_security_mode_command_msg.ciph_algo)
    {
    ...
    {
      rrcsmc_int_data.saved_srns_info.ciph_algo_present = TRUE;
      rrcsmc_int_data.saved_srns_info.ciph_algo = rrcsmc_get_int_cipher_algo_from_dl_sdu_ind
        (wrrc_as_id,&(srns_info->ciphering_mode_info->cipheringModeCommand.startRestart),
             msg_version);
    }
    ...
    {
      rrcsmc_int_data.saved_srns_info.dpch_act_time_present = TRUE;
      rrcsmc_int_data.saved_srns_info.dpch_act_time_present = 
...
)
{
  ...
  if(srns_info->integrity_info_present)
  {
    ...
    if(srns_info->integrity_protection_mode_info->m.integrityProtectionAlgorithmPresent)
    {
      ...
      rrcsmc_int_data.saved_srns_info.integrity_algo = rrcsmc_get_int_integrity_algo_from_dl_sdu_ind(wrrc_as_id,
                &srns_info->integrity_protection_mode_info->integrityProtectionAlgorithm,
                MSG_REL99);
      WRRC_MSG1_HIGH("Integrity Algo present in SRNS info,Algo=%d[0:Invalid Algo,1:UIA1]",rrcsmc_int_data.saved_srns_info.integrity_algo);
    }
  }
  ...
  {
    if(rrcsmc_get_int_cipher_algo_from_dl_sdu_ind
        (wrrc_as_id,&(srns_info->ciphering_mode_info->cipheringModeCommand.u.startRestart),
             msg_version)
        == rrcsmc_int_data.saved_security_mode_command_msg.ciph_algo)
    {
      WRRC_MSG0_ERROR("Ciph Algo same. Invalid Config");
    ...
    {
      rrcsmc_int_data.saved_srns_info.ciph_algo_present = TRUE;
      rrcsmc_int_data.saved_srns_info.ciph_algo = 
    ...
    {
      rrcsmc_int_data.saved_srns_info.dpch_act_time_present = TRUE;
      rrcsmc_int_data.saved_srns_info.dpch_activation_time = 
        (uint8)srns_info->ciphering_mode_info->activationTimeForDPCH;
    }
  }
  return SUCCESS;
...
)
{
  ...
  if(rrcsmc_int_data.saved_srns_info.fresh_present == TRUE)
  {
    //commit the new fresh value received and set ip reconfig to true
    rrcf9_init_fresh_per_connection(wrrc_as_id,rrcsmc_int_data.saved_srns_info.fresh);

    //save the comitted fresh value as old fresh value to be used in
    //the next connection setup
    rrcsmc_int_data.saved_srns_info.fresh_old1 = rrcsmc_int_data.saved_srns_info.fresh_old;    
    rrcsmc_int_data.saved_srns_info.fresh_old = rrcsmc_int_data.saved_srns_info.fresh;
...
)
{
  ...
  if(rrcsmc_int_data.saved_srns_info.fresh_present == TRUE)
  {
    ...
    //commit the new fresh value received and set ip reconfig to true
    rrcf9_init_fresh_per_connection(wrrc_as_id,rrcsmc_int_data.saved_srns_info.fresh_old1);

    //save the comitted fresh value as old fresh value to be used in
    //the next connection setup
    rrcsmc_int_data.saved_srns_info.fresh_old = rrcsmc_int_data.saved_srns_info.fresh_old1;
...
)
{
  ...
  { 
    if(rrcsmc_get_srns_info_from_dl_sdu_ind_r9(wrrc_as_id,&(srns_security_info_7), cmd_ptr) != SUCCESS)
    {
...
#endif /* FEATURE_WCDMA_REL9 */
#ifdef FEATURE_WCDMA_REL10
  if (msg_version == MSG_REL10)
  { 
    if(rrcsmc_get_srns_info_from_dl_sdu_ind_r10(wrrc_as_id,&(srns_security_info_7), cmd_ptr) != SUCCESS)
    {
      return RRCSMC_SRNS_SECURITY_INFO_INVALID;  
    }
  }
  ...
  { 
    if(rrcsmc_get_srns_info_from_dl_sdu_ind_r8(wrrc_as_id,&(srns_security_info_7), cmd_ptr) != SUCCESS)
    {
      return RRCSMC_SRNS_SECURITY_INFO_INVALID;  
    }
  }
  ...
  { 
    if(rrcsmc_get_srns_info_from_dl_sdu_ind_r7(wrrc_as_id,&(srns_security_info_7), cmd_ptr) != SUCCESS)
    {
      return RRCSMC_SRNS_SECURITY_INFO_INVALID;  
    }
  }
  ...
  { 
    if(rrcsmc_get_srns_info_from_dl_sdu_ind_r6(wrrc_as_id,&(srns_security_info), cmd_ptr) != SUCCESS)
    {
      return RRCSMC_SRNS_SECURITY_INFO_INVALID;  
    }
  }
  else
  {
    ...
    {    
      if(rrcsmc_get_srns_info_from_dl_sdu_ind_r5(wrrc_as_id,&(srns_security_info), cmd_ptr) != SUCCESS)
      {
        return RRCSMC_SRNS_SECURITY_INFO_INVALID;  
      }
    }
    ...
    {
      if(rrcsmc_get_srns_info_from_dl_sdu_ind(wrrc_as_id,&(srns_security_info), cmd_ptr) != SUCCESS)
      {
        return RRCSMC_SRNS_SECURITY_INFO_INVALID;  
      }
    }
...
|| (msg_version == MSG_REL9)
  ...
  {
    if((rrcsmc_validate_and_save_srns_info_r7(wrrc_as_id,&srns_security_info_7,msg_version)) != SUCCESS)
    {
      return RRCSMC_SRNS_SECURITY_INFO_INVALID;  
    }
  }
  ...
  {
    if(rrcsmc_validate_and_save_srns_info(wrrc_as_id,&(srns_security_info),MSG_REL99) != SUCCESS)
    {
      return RRCSMC_SRNS_SECURITY_INFO_INVALID;  
    }
  }
...
|| (msg_version == MSG_REL9)
  ...
  )
  {
    if((srns_security_info_7.ciphering_mode_info_present == TRUE || 
       ...
       (rrcsmc_int_data.integrity_protection_info.reconfig == TRUE ||
        rrcsmc_int_data.ciphering_info.reconfig == TRUE))
    {
  ...
  if(integrity_info_present)
  {
    ...
    rb_id =  rrclcm_get_dl_rb_id(wrrc_as_id,cmd_ptr->cmd.downlink_sdu_ind.lc_id);
    ...
    if ( rrcsmc_process_dl_integrity_check_info(wrrc_as_id,
    ...
    if ( (rb_id < RRC_MAX_SRB) && ( rrcsmc_int_data.integrity_protection_info.modify_received == TRUE) &&
         ( rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id].
           dl_count_i_act_time_pending[rb_id] == TRUE) )
    {
      ...
      rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id].
      dl_count_i_act_time_pending[rb_id] = FALSE;
      rrcsmc_int_data.integrity_protection_info.dl_latest_config_cn_id_old_1_valid[rb_id] = FALSE;
      ...
      dl_count_i[rb_id] = 
        rrcsmc_convert_START_to_count_c_i(wrrc_as_id,
          ...
          (rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.
    ...
    {    
      ...
      local_hfn= rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id].
                 dl_count_i[rb_id];
    ...
    local_mac = rrcf9_generate_mac_for_message(wrrc_as_id,
                  local_count_i,
                  RRC_INTEGRITY_DIR_DOWNLINK,
                  cmd_ptr->cmd.downlink_sdu_ind.dl_sdu,
                  (cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_length * 8),
                  rrcsmc_int_ciph_info.latest_config_cn_id,
                  rrcsmc_int_data.saved_srns_info.integrity_algo
                  );

    WRRC_MSG4_HIGH("maci gen = 0x%x int_check_info.msg_auth_code = 0x%x CN domain %d[0=CS,1=PS], Integrity Algo = %d[0=Invalid,1=UIA1,2=UIA2] ",
    local_mac,
    int_check_info.msg_auth_code,
    rrcsmc_int_ciph_info.latest_config_cn_id,
    rrcsmc_int_data.saved_srns_info.integrity_algo
    ...
    rrcf9_init_fresh_per_connection(wrrc_as_id,rrcsmc_int_data.saved_srns_info.fresh_old);
...
)
{
  ...
  if (rrcsmc_ext_data.nv_integrity_enabled)
  {
...
/*lint -e527*/    
    switch (cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_type)
    {
      ...
      case rrc_DL_CCCH_Message_PDU:
        ...
        msg_type = 
        rrc_get_dl_ccch_message_type(wrrc_as_id,cmd_ptr->cmd.downlink_sdu_ind.dl_sdu,
                                     cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_length);
        switch (msg_type )
        {
      ...
      case rrc_DL_DCCH_Message_PDU :
        ...
        switch (msg_type)
        {
          ...
          case T_rrc_DL_DCCH_MessageType_uraUpdateConfirm:
            ...
            if (TRUE == 
                         rrcsmc_is_integrity_prot_mode_info_present(wrrc_as_id,
                                             cmd_ptr->cmd.downlink_sdu_ind.dl_sdu, msg_type))
            {
              ...
              cmd_ptr->cmd.downlink_sdu_ind.decoded_msg =
                    rrcasn1_decode_pdu(wrrc_as_id,cmd_ptr->cmd.downlink_sdu_ind.dl_sdu,(int )
              ...
              if (NULL != cmd_ptr->cmd.downlink_sdu_ind.decoded_msg)
              {
                ...
                if (((OC_NOT_SET == rrcllc_get_ordered_config_status(wrrc_as_id))
                     ...
                     && (FALSE == rrcdata_check_if_ota_is_buffered(wrrc_as_id))
#endif  /*FEATURE_WCDMA_ALLOW_LTA_DURING_ACT_TIME*/
                    ) && (FALSE == rrcsmc_is_security_reconfig_active(wrrc_as_id)))
                {
                  ...
                  rrcsmc_backup_global_security_variables(wrrc_as_id);
                }
...
/*lint +e527*/    
    ...
    if(rb_id < RRC_MAX_SRB)
    {
      if (( rrcsmc_int_data.integrity_protection_info.modify_received == TRUE) &&
           ( rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id].
             dl_count_i_act_time_pending[rb_id] == TRUE) )
      {
        is_dl_act_time_expired = RRC_CHECK_FOR_IP_ACT_TIME_EXPIRY(
                  (rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.
                  latest_config_cn_id_old].dl_count_i[rb_id] & RRC_SEQ_NUM_BIT_MASK),
                         rrcsmc_int_data.integrity_protection_info.dl_act_seq_num[rb_id], 
        ...
        if (TRUE == is_dl_act_time_expired)
        {
          ...
          if ((FALSE == rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id].
          ...
          WRRC_MSG8_HIGH(" DL act exp lcn_old %d lcn %d CI lcn_old %d CI lcnd %d lcn HFN %d ActSN %d MSN %d RBID%d",
          rrcsmc_int_ciph_info.latest_config_cn_id_old,
          rrcsmc_int_ciph_info.latest_config_cn_id,
          rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id_old].dl_count_i[rb_id],
          rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id].dl_count_i[rb_id],
          rrcsmc_int_ciph_info.int_ciph_info[rrcsmc_int_ciph_info.latest_config_cn_id].hfn,
          rrcsmc_int_data.integrity_protection_info.dl_act_seq_num[rb_id],
          int_check_info.msg_seq_num,
          rb_id);
        ...
        else
        {
          ...
          /*if activation time has not been reached for this rb_id, continue using old
              cn domain */
          if (FALSE == rrcsmc_int_data.integrity_protection_info.dl_latest_config_cn_id_old_1_valid[rb_id])
          {
            ...
            if(rrcsmc_int_data.integrity_protection_info.count_i_info[local_cn_domain].dl_new_key_flag_per_rb[rb_id]==TRUE)
            {
              WRRC_MSG3_HIGH("Using old key local_cn_domain: %d rrcsmc_int_ciph_info.latest_config_cn_id: %d key: for local_cn_domain: %d ", 
                local_cn_domain, rrcsmc_int_ciph_info.latest_config_cn_id, rrcsmc_int_data.integrity_protection_info.count_i_info[local_cn_domain].
            ...
            {
              WRRC_MSG3_HIGH("Dont use old keys local_cn_domain: %d rrcsmc_int_ciph_info.latest_config_cn_id: %d key for local_cn_domain: %d ", 
                local_cn_domain, rrcsmc_int_ciph_info.latest_config_cn_id, rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id].
                dl_new_key_flag_per_rb[rb_id]);
            }
          }
          else
      ...
      /*copy hfn to a local variable for further processing*/
      local_hfn= rrcsmc_int_data.integrity_protection_info.count_i_info[local_cn_domain].
                 dl_count_i[rb_id];      
      ...
      if (rrcsmc_int_data.integrity_protection_info.count_i_info[local_cn_domain].
          dl_count_i_init_flag[rb_id] == FALSE)
      {
        /*set the init_flag to TRUE*/  
        rrcsmc_int_data.integrity_protection_info.count_i_info[local_cn_domain].
        dl_count_i_init_flag[rb_id] = TRUE;
      }
      else
      {
    ...
    if (local_seq_no > int_check_info.msg_seq_num)
    {
      ...
      local_hfn++;
      local_hfn <<=4;
      WRRC_MSG3_HIGH("Local SN %d MSN %d HFN after rollover %d",local_seq_no, int_check_info.msg_seq_num,local_hfn);			
    }
    ...
    {
      rrcf9_init_key_values(wrrc_as_id,
    ...
    else if ((rb_id < RRC_MAX_SRB) && (TRUE == use_latest_config_cn_id_old_1_keys_flag))
    {
      ...
      rrcf9_init_key_values(wrrc_as_id,
        rrcsmc_int_data.integrity_protection_info.integrity_key_for_cn_id_old_1[rb_id],
        rrcsmc_int_data.integrity_protection_info.integrity_key_for_cn_id_old_1[rb_id]);
    }
    ...
    {
      WRRC_MSG3_ERROR("DL IP check failed. FRESH used: 0x%x, COUNT-I used: 0x%x, old_keys_used: %d", 
        rrcsmc_int_data.saved_srns_info.fresh_old, local_count_i, using_old_keys_flag);
...
)
{
         ...
         ul_count_i_act_time_pending[rb_id] == TRUE) )
  {
    ...
    if (((rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id_old].
          ul_count_i[rb_id]+1) & RRC_SEQ_NUM_BIT_MASK ) == 
        rrcsmc_int_data.integrity_protection_info.ul_act_seq_num[rb_id])
    {
      /*reset pending flag*/
      rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id].
    ...
    else
    {
      ...
      eg - CS Keys1 are in use now auth is done which gives new CS keys2 and PS keys2. Now SMC for PS domain comes
      so before the activation time we should use CS keys1*/
      if(rrcsmc_int_data.integrity_protection_info.count_i_info[local_cn_domain].ul_new_key_flag_per_rb[rb_id]==TRUE)
      {
        using_old_keys_flag = TRUE;
       
        WRRC_MSG3_HIGH("Using old key local_cn_domain: %d rrcsmc_int_ciph_info.latest_config_cn_id: %d key: for local_cn_domain: %d ", 
         local_cn_domain, rrcsmc_int_ciph_info.latest_config_cn_id, 
         rrcsmc_int_data.integrity_protection_info.count_i_info[local_cn_domain].ul_new_key_flag_per_rb[rb_id]);
      }
      else
      {
         WRRC_MSG3_HIGH("Dont use old keys local_cn_domain: %d rrcsmc_int_ciph_info.latest_config_cn_id: %d key for local_cn_domain: %d ", 
...
WRRC_MSG3_HIGH("maci gen = 0x%x  CN domain %d[0=CS,1=PS], Integrity Algo = %d[0=Invalid,1=UIA1,2=UIA2] ",
  ...
  local_seq_num = rrcsmc_int_data.integrity_protection_info.count_i_info[local_cn_domain].
                  ul_count_i[rb_id];
  local_seq_num &= 0xF;

  rrcsmc_append_mac_seq_no_to_ul_sdu(wrrc_as_id,ul_sdu, local_mac, local_seq_num);
...
{  
  ...
  if(rb_id < RRC_MAX_SRB)
  {
    ...
    {
      if (rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].
          dl_new_key_flag_per_rb[rb_id] == TRUE)
      {
      ...
      {
        local_hfn = rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].
                    dl_count_i[rb_id] & RRC_MSB_20BIT_HFN_BIT_MASK;
      }

      /*get the seq num of the old domain*/
      old_seq_no = (rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.
    ...
    {
      local_hfn = rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].dl_count_i[rb_id];
    }
        ...
        T_rrc_IntegrityProtectionModeCommand_startIntegrityProtection)
    {
      ...
      rrcf9_init_fresh_per_connection(wrrc_as_id,local_fresh);
    }
    ...
    (void) rrcsmc_process_dl_integrity_check_info(wrrc_as_id,cmd_ptr->cmd.downlink_sdu_ind.dl_sdu,
                                                  cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_length,
  ...
  local_mac = rrcf9_generate_mac_for_message(wrrc_as_id,
                ...
                (cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_length*8),
                domain_id,
                rrcsmc_get_int_integrity_algo_from_dl_sdu_ind(wrrc_as_id,
                &integrityProtModeInfo->integrityProtectionAlgorithm,
                MSG_REL99));

   WRRC_MSG4_HIGH("maci gen = 0x%x received_mac = 0x%x CN domain %d[0=CS,1=PS], Integrity Algo = %d[0=Invalid,1=UIA1,2=UIA2] ",
   local_mac,
   received_mac,
   domain_id,
    rrcsmc_get_int_integrity_algo_from_dl_sdu_ind(wrrc_as_id,
  ...
  if ( local_mac == received_mac)
  {
    if(rb_id < RRC_MAX_SRB)
    {
      ...
      if (rcvd_new_keys_flag == TRUE)
      {
        ...
        WRRC_MSG3_HIGH("Started using new keys, CIPH START VAL: 0x%x 0x%x 0x%x", 
           rrcsmc_int_ciph_info.int_ciph_info[domain_id].hfn[0],
      ...
      rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].dl_count_i[rb_id] = local_count_i;
         ...
         !(rcvd_new_keys_flag))
      {
        ...
        rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].ul_count_i[2] =         
          rrcsmc_convert_START_to_count_c_i(wrrc_as_id,
            rrcsmc_int_ciph_info.int_ciph_info[domain_id].hfn,
            rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id].
            ul_count_i[2]);
      ...
      else if(!rcvd_new_keys_flag) 
      {
          ...
          CN domain here when successfully check for SMC is done*/
        rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].ul_count_i[2]= 
        (rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].ul_count_i[2] & RRC_SEQ_NUM_BIT_MASK )| 
        (rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].ul_count_i[2] & RRC_MSB_20BIT_HFN_BIT_MASK);
      }
...
)
{
  ...
  for ( i=0; i < RRC_MAX_SRB; i++)
  {
    if ((TRUE == rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id_old].dl_count_i_act_time_pending[i]) &&
		    (FALSE == rrcsmc_int_data.integrity_protection_info.dl_latest_config_cn_id_old_1_valid[i]))
    {
      WRRC_MSG3_HIGH("dl_latest_config_cn_id_old_1_valid for SRB: %d, saving keys for domain latest_config_cn_id_old_1:%d(0:CS) ciph_keys_info.new_key: %d, if 1 using old IK keys else latest IK keys ", 
      ...
      rrcsmc_int_data.integrity_protection_info.dl_latest_config_cn_id_old_1_valid[i] = TRUE;
  	  	...
  	  	rrcsmc_int_ciph_info.int_ciph_info[rrcsmc_int_ciph_info.latest_config_cn_id_old_1].ciph_keys_info.new_key)
      {
        ...
        WCDMA_MEMCPY(rrcsmc_int_data.integrity_protection_info.integrity_key_for_cn_id_old_1[i],
                     sizeof(byte) * RRC_MAX_INTEGRITY_KEY_LENGTH,
    ...
    for this domain */
    rrcsmc_int_data.integrity_protection_info.dl_act_seq_num[i] = 
    rrcsmc_int_data.saved_security_mode_command_msg.srb_int_dl_act_time[i].seq_num;
    ...
    {
      rrcsmc_int_data.integrity_protection_info.ul_act_seq_num[i] =  
      (( (rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id_old].
    ...
    else if (rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.
...
)
{
  ...
  {
    rrcsmc_int_data.rrcsmc_failure_type = RRCSMC_PROTOCOL_ERROR;   
    rrcsmc_int_data.rrcsmc_prot_err_cause = rrc_ProtocolErrorCause_messageExtensionNotComprehended;   
  ...
  if( (rrcsmc_int_data.integrity_protection_info.reconfig == TRUE) || 
      (rrcsmc_int_data.ciphering_info.reconfig == TRUE)
    )
  {
    WRRC_MSG0_ERROR("Incompatible Simultaneous Ciph Reconfig");
    rrcsmc_int_data.rrcsmc_failure_type = RRCSMC_INCOMPATIBLE_SIMULTANEOUS_RECONFIG;
    return RRCSMC_INVALID_CONFIGURATION;
  }

  /*if both nv items are not enabled, then do not check any more IE's just
    return success*/
  if((!rrcsmc_ext_data.nv_ciphering_enabled) && (!rrcsmc_ext_data.nv_integrity_enabled))
  {
    ...
    )
  {
    ...
    rrcsmc_int_data.rrcsmc_failure_type = RRCSMC_INVALID_CONFIGURATION; 
    return RRCSMC_INVALID_CONFIGURATION;    
  }
...
)
{
  ...
  {
     WRRC_MSG0_ERROR("Incomplte Configuration"); 
    rrcsmc_int_data.rrcsmc_failure_type = RRCSMC_INCOMPLETE_CONFIGURATION; 
    return RRCSMC_INVALID_CONFIGURATION;    
  }
  ...
  if(RRCLCM_RLC_LC_ID_NOT_FOUND != rrclcm_check_dl_rlc_lc_id(wrrc_as_id,
                                   UE_LOGCHAN_DCCH,
                                   DCCH_DT_LOW_PRI_RADIO_BEARER_ID,
                                   UE_MODE_ACKNOWLEDGED_DATA))
  {
  ...
  if(rrclcm_get_rb_id_if_dl_lc_type_exists(wrrc_as_id,UE_LOGCHAN_TM_DCCH_DL, &srb5_rb_id) == RRCLCM_SUCCESS)
  {
    /* get the activation time list for RBs */
    local_rb_ciph_act_time_list = &(ciphModeInfo->rb_DL_CiphActivationTimeInfo);
     ...
     )
  {
    ...
    if((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL7) && rrcnv_data.snow3g_security_algo_supported)
    {
      cipher_algo_suppported += UEA_2_SUPPORTED;
    }
#endif

  cipher_algo_configured = rrcsmc_get_int_cipher_algo_from_dl_sdu_ind(wrrc_as_id,
  ...
  if(!(cipher_algo_suppported & cipher_algo_configured))
  {
    ...
    rrcsmc_int_data.rrcsmc_failure_type = RRCSMC_UNSUPPORTED_CONFIG; 
    return RRCSMC_INVALID_CONFIGURATION;    
  }
...
{  
  ...
  if(integrityProtModeInfo->m.integrityProtectionAlgorithmPresent)
  {
    ...
    int_algo_configured = rrcsmc_get_int_integrity_algo_from_dl_sdu_ind(wrrc_as_id,
  ...
  if (ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
         ...
         (rrcsmc_int_ciph_info.integrity_info_received == TRUE))
    {
      ...
      for (i=0; i < num_srb; i++)
      {
        ...
        if (rrcsmc_int_data.integrity_protection_info.modify_received == TRUE)
        {
          local_msg_seq_num_list->elem[i] = rrcsmc_int_data.integrity_protection_info.ul_act_seq_num[i];
        }
        else
        {
        ...
        (rrcsmc_int_ciph_info.ciphering_info_received == TRUE) )
    {
      ...
      local_ciph_act_info->n = rrcsmc_int_data.ciphering_info.dl_ciph_act_info.nchan;
      ...
      /*loop through all the channels which we suspended and have already
        ...
        Activation Times..*/
      for (i=0; i<rrcsmc_int_data.ciphering_info.ul_ciph_act_info.nchan; i++)
      {
        local_rb_id = rrclcm_get_ul_rb_id(wrrc_as_id,rrcsmc_int_data.ciphering_info.ul_ciph_act_info.
        lc_id_for_complete[i]);
        
        if(local_rb_id <= MAX_RB)
        {
          local_suspend_act_sn.sq_num[local_rb_id] = 
          rrcsmc_int_data.ciphering_info.ul_ciph_act_info.sq_num_for_complete[i];
        }
      }
      ...
      for (i=1; i<=MAX_RB; i++)
      {
        if(rrcsmc_int_data.ciphering_info.dl_ciph_act_info.act_time_present[i] == TRUE)
        {
    ...
    rrcqsh_log_uplink_dcch_ota_params(wrrc_as_id, WRRC_UL_DCCH_MESSAGETYPE_SECURITYMODECOMPLETE, msg_ptr);
#endif
    /* Send the SECURITY MODE COMPLETE on the Uplink DCCH
     */
    send_status = rrcsend_send_ul_sdu(wrrc_as_id,RRC_PROCEDURE_SMC,
                                      msg_ptr,
                                      ul_lc_id,
    ...
    if (send_status == RRCSEND_SUCCESS)
    {
         ...
         (rrcsmc_int_ciph_info.ciphering_info_received == TRUE) )
      {
        ...
        else
        {
          ...
          rrc_qsh_report_wait_l2_ack_event(wrrc_as_id, RRC_WAIT_L2_ACK_FOR_SMC);
#endif
        }
    }
    else
...
)
{
  ...
  /*set the correct failure cause and fill in the failure struct with
    info if any*/
  switch (rrcsmc_int_data.rrcsmc_failure_type)
  {
    ...
    case RRCSMC_PROTOCOL_ERROR:
      ...
      msg_ptr->message.u.securityModeFailure.failureCause.u.protocolError->
      diagnosticsType.u.type1->protocolErrorCause = rrcsmc_int_data.rrcsmc_prot_err_cause;
      break;
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
  /*Reports security mode failure to QSH*/
  rrc_qsh_report_failure_ota_msg_event(wrrc_as_id,WRRC_UL_DCCH_MESSAGETYPE_SECURITYMODEFAILURE);
#endif
  ...
  rrc_log_protocol_error( wrrc_as_id, (uint8) (rrc_get_state(wrrc_as_id)),
                          ...
                          (uint8) rrcsmc_int_data.rrcsmc_prot_err_cause 
                        );
...
)
{
  ...
  if (rrcsmc_int_data.smc_rb_suspend_or_stop_info.rlc_rb_suspended_or_stopped_flag == TRUE)
  {
    rrc_ul_resume_req_ptr->nchan = rrcsmc_int_data.smc_rb_suspend_or_stop_info.suspend_or_stopped_info.nchan;
    ...
    rrcsmc_int_data.smc_rb_suspend_or_stop_info.rlc_rb_suspended_or_stopped_flag = FALSE;
...
)
{
  ...
  if (rrcsmc_int_data.smc_rb_suspend_or_stop_info.rlc_rb_suspended_or_stopped_flag == TRUE )
  {
    ...
    rrc_send_l2_ul_cmd(wrrc_as_id, rlc_ptr);
  ...
  switch (cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_MODE_CHANGE_IND:
      ...
      {	
        ...
        if ((TRUE == rrcsmc_int_data.integrity_protection_info.modify_received) &&
            (rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id].
             ul_count_i_act_time_pending[CCCH_RADIO_BEARER_ID] == TRUE))
        {
          ...
          if(0 == rrcsmc_int_data.integrity_protection_info.ul_act_seq_num[CCCH_RADIO_BEARER_ID])
          {
          ...
          {
            rb_0_act_time_jump = 
              (rrcsmc_int_data.integrity_protection_info.ul_act_seq_num[CCCH_RADIO_BEARER_ID] - 1);
          }
          ...
          WRRC_MSG3_HIGH("SMComp L2 Ack rcvd; SRB0 -> updated RRC-SN: 0x%x; latest-count-I: 0x%x; previousDomain-count-I: 0x%x)",
            rb_0_act_time_jump, rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id].
            ul_count_i[CCCH_RADIO_BEARER_ID], rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id_old].
            ul_count_i[CCCH_RADIO_BEARER_ID]);
        ...
        if(rrcsmc_int_ciph_info.ciphering_info_received == TRUE)
        {
          ...
          rrcsmc_update_cphy_key_req(wrrc_as_id, CIPH_CONFIG_UL_KEYS);
          rrcsmc_send_ul_ciphering_key_req(wrrc_as_id);
        }
        ...
        rrcsmc_send_resume_req(wrrc_as_id);
        ...
        if((rrcsmc_int_ciph_info.ciphering_info_received == TRUE) &&
          (rrcsmc_int_ciph_info.integrity_info_received == TRUE))
        {
          rrcsmc_send_sync_ind_to_mm(wrrc_as_id, RRC_INTEGRITY_CIPHERING_ESTABLISHED);
  
        }
        else if((rrcsmc_int_ciph_info.ciphering_info_received != TRUE) &&
          (rrcsmc_int_ciph_info.integrity_info_received == TRUE))
        {
          rrcsmc_send_sync_ind_to_mm(wrrc_as_id, RRC_INTEGRITY_ESTABLISHED);
        }
        else if((rrcsmc_int_ciph_info.ciphering_info_received == TRUE) &&
         (rrcsmc_int_ciph_info.integrity_info_received != TRUE))
        {
          rrcsmc_send_sync_ind_to_mm(wrrc_as_id, RRC_CIPHERING_ESTABLISHED);
        }
        else
        {
          if(rrcsmc_ext_data.nv_integrity_enabled)
        ...
        rrcsmc_process_l2_ack_ind_to_dt_procedures(wrrc_as_id);
...
FUNCTION rrcsmc_repost_smc_message_if_it_is_buffered

DESCRIPTION
   This function reposts SMC message if it was buffered.
...
void rrcsmc_process_smc_message_if_it_is_buffered(sys_modem_as_id_e_type wrrc_as_id)
{
  if((rrcnv_data.rrc_buffer_smc_support.enable_smc_buffer) && 
     (rrcsmc_int_data.saved_smc_message.dl_sdu_ptr != NULL))
  {
    rrc_cmd_type *cmd_ptr;
    cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id);
    cmd_ptr->cmd_hdr.cmd_id = RRC_DOWNLINK_SDU_IND;
    cmd_ptr->cmd.downlink_sdu_ind.lc_id = rrclcm_get_dl_rlc_lc_id(wrrc_as_id,DCCH_AM_RADIO_BEARER_ID);
    cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_length = rrcsmc_int_data.saved_smc_message.dl_sdu_length;
    cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_type = rrc_DL_DCCH_Message_PDU;
    cmd_ptr->cmd.downlink_sdu_ind.dl_sdu = rrcsmc_int_data.saved_smc_message.dl_sdu_ptr;
    cmd_ptr->cmd.downlink_sdu_ind.decoded_msg = rrcasn1_decode_pdu(wrrc_as_id, 
                                                                   rrcsmc_int_data.saved_smc_message.dl_sdu_ptr,
                                                                   rrc_DL_DCCH_Message_PDU,
                                                                   rrcsmc_int_data.saved_smc_message.dl_sdu_length);
    
    WRRC_MSG0_HIGH("BUF_SMC: Buffered SMC is processed");
    /* Directly call the SMC event handler instead of putting this to Internal command queue*/
    rrcsmc_procedure_event_handler(wrrc_as_id,cmd_ptr);

    /* Free the command pointer. This also free's the dl_sdu pointer and decoded_msg pointer */
    wrrc_free_cmd_buf(wrrc_as_id,cmd_ptr);
				
    /* Reset the global data pointer to NULL. Memory for dl_sdu is already freed inside wrrc_free_cmd_buf */
    rrcsmc_int_data.saved_smc_message.dl_sdu_ptr = NULL;
    rrcsmc_int_data.saved_smc_message.dl_sdu_length = 0;
  }
}				
...
)
{
  switch (cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_DOWNLINK_L2ACK_IND:
      ...
      rrctmr_stop_timer(wrrc_as_id,RRCTMR_WAIT_FOR_L2_ACK_FOR_PCH_TRANSITION_TIMER);
      
      if (SUCCESS == cmd_ptr->cmd.downlink_l2ack_ind.status)
      {
        if(rrcmcm_get_rrc_mode(wrrc_as_id) == RRC_MODE_STANDBY)
        {
          WRRC_MSG1_HIGH("SMC state changed to %d",RRCSMC_WAIT_TILL_STANDBY_ENDS);
          rrcmcm_register_for_mode_change(wrrc_as_id,RRC_PROCEDURE_SMC,RRC_MODE_STANDBY,RRC_MODE_WILDCARD);
        ...
        rrcsmc_process_smc_message_if_it_is_buffered(wrrc_as_id);
      }
      }
      else
      {
...
)
{
  switch (cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_RECONFIG_COMPLETE_IND:
      ...
      {
        rrcsmc_perform_stop_or_suspend_functionality(wrrc_as_id, RRCSMC_SMC_CMD);
      }
      break;
...
)
{
  ...
  rrcsmc_int_ciph_info.latest_config_cn_id =  
  rrcsmc_int_data.saved_security_mode_command_msg.domain_id;
  ...
  if (rrcsmc_int_data.saved_security_mode_command_msg.dpch_ciph_act_time_present)
  {
    ...
    rrcsmc_int_data.tm_ciph_act_cfn = (uint8)rrcsmc_int_data.saved_security_mode_command_msg.dpch_ciph_act_time;
  }

  if (rrcsmc_int_data.saved_security_mode_command_msg.rb_ciph_dl_act_time_present)
  {

    rrcsmc_int_data.ciphering_info.dl_ciph_act_info.nchan = rrcsmc_int_data.saved_security_mode_command_msg.
      num_rb_ciph_dl_act_time;

    memset(rrcsmc_int_data.ciphering_info.dl_ciph_act_info.act_time_present, FALSE, (MAX_RB+1));

    for (i=0; i<rrcsmc_int_data.saved_security_mode_command_msg.num_rb_ciph_dl_act_time; i++)
    {
      ...
      rrcsmc_int_data.ciphering_info.dl_ciph_act_info.sq_num_for_complete
        [rrcsmc_int_data.saved_security_mode_command_msg.rb_ciph_dl_act_time[i].rb_id] =
      rrcsmc_int_data.saved_security_mode_command_msg.rb_ciph_dl_act_time[i].seq_num;

      rrcsmc_int_data.ciphering_info.dl_ciph_act_info.act_time_present
        [rrcsmc_int_data.saved_security_mode_command_msg.rb_ciph_dl_act_time[i].rb_id] = TRUE;
    }
  }
}
/*===========================================================================
...
)
{
  ...
  if (rrcsmc_int_data.saved_security_mode_command_msg.modify_chosen)
  {
    /*update activation time info structure*/
    rrcsmc_update_int_activation_time(wrrc_as_id);

    /*set the modify flag to TRUE*/
    rrcsmc_int_data.integrity_protection_info.modify_received = TRUE;
    ...
    else
    {
      ...
      rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id].
      ul_count_i[2] =
      (uint32)((rrcsmc_int_data.integrity_protection_info.count_i_info[rrcsmc_int_ciph_info.latest_config_cn_id].
...
===========================================================================*/
/*lint -e715*/
void rrcsmc_process_suspend_cnf(sys_modem_as_id_e_type wrrc_as_id,rlc_ul_stop_suspend_cnf_type *rrcsmc_suspend_cnf)
{
  uint32 i; 
  if ( (rrcsmc_ext_data.nv_integrity_enabled) &&
       (rrcsmc_int_ciph_info.integrity_info_received == TRUE) )
  {
    rrcsmc_process_int_security_mode_cmd_msg(wrrc_as_id);
  }
  ...
  if(rrcsmc_int_ciph_info.ciphering_info_received == TRUE)
  {
    ...
    rrcsmc_int_data.smc_rb_suspend_or_stop_info.rlc_rb_suspended_or_stopped_flag = TRUE;           

    rrcsmc_process_ciph_security_mode_cmd_msg(wrrc_as_id);

    //get the suspend cnf information and set the activation time for all existing
    //rlc am and um channels
    rrcsmc_int_data.ciphering_info.ul_ciph_act_info.nchan = rrcsmc_suspend_cnf->nchan;

    for (i=0; i< rrcsmc_int_data.ciphering_info.ul_ciph_act_info.nchan; i++)
    {
      if(i < (UE_MAX_UL_UM_CHANNEL + UE_MAX_AM_ENTITY)) /* Max array size of cnf is UE_MAX_UL_UM_CHANNEL + UE_MAX_AM_ENTITY */
      {
        rrcsmc_int_data.ciphering_info.ul_ciph_act_info.sq_num[rrcsmc_suspend_cnf->cnf[i].lc_id] = 
        rrcsmc_suspend_cnf->cnf[i].rlc_sn + RLC_CIPH_ACT_SN;
  
        rrcsmc_int_data.ciphering_info.ul_ciph_act_info.sq_num_for_complete[i] = 
        rrcsmc_suspend_cnf->cnf[i].rlc_sn + RLC_CIPH_ACT_SN;
  
        rrcsmc_int_data.ciphering_info.ul_ciph_act_info.lc_id_for_complete[i] = 
    ...
    rrcsmc_send_security_mode_complete(wrrc_as_id,rrcsmc_int_data.saved_security_mode_command_msg.transaction_id);

  }/*check for ciphering IE's are present*/
  else if ((rrcsmc_ext_data.nv_integrity_enabled) &&
       (rrcsmc_int_ciph_info.integrity_info_received == TRUE))
  {
    rrcsmc_int_data.smc_rb_suspend_or_stop_info.rlc_rb_suspended_or_stopped_flag = TRUE;           
...
)
{
  ...
  switch (cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_DOWNLINK_L2ACK_IND:
      ...
      if (SUCCESS == cmd_ptr->cmd.downlink_l2ack_ind.status)
      {
        rrcsmc_int_data.l2_ack_rcvd_for_smc_complete_flag = TRUE;
    ...
    case RRC_CIPH_CONFIG_CNF:
      ...
      if (TRUE == rrcsmc_int_data.is_revert_to_old_cipher_variable_reqd)
      {
        /* Revert the ciphering variables */
        WRRC_MSG0_HIGH("Reverting to old ciphering variables in ciph_config_cnf substate");
        rrcsmc_revert_to_old_security_variables(wrrc_as_id);
      ...
      if(rrcsmc_int_data.l2_ack_rcvd_for_smc_complete_flag == TRUE)
      {
        if(rrcsmc_int_ciph_info.ciphering_info_received == TRUE)
        {
          /*Lets Update L1 with UL cipher keys now.
...
)
{
  ...
  switch (cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_RE_EST_RLC_FOR_SRNS_REQ:
        ...
        {
          rrcsmc_int_data.saved_srns_info.rb_type = RRC_RE_EST_NON_RB_2_UM_AM;
        }
        
        //if re_est cnf was needed, set flag to true, else false
        if(cmd_ptr->cmd.rrc_re_est_srns_req.cnf_reqd == TRUE)
        {
          rrcsmc_int_data.saved_srns_info.reconfig_cnf_req = TRUE;
          //save the procedure name to send cnf
          rrcsmc_int_data.saved_srns_info.proc_type = cmd_ptr->cmd.rrc_re_est_srns_req.rrc_proc;
        }
        else
        {
          rrcsmc_int_data.saved_srns_info.reconfig_cnf_req = FALSE;    
        }        
        
        rrcsmc_get_latest_transmitted_start_values(wrrc_as_id,&start_list);
        
    WRRC_MSG2_HIGH("Re-EST Non-RB2 HFN_SRB %d, HFN_URB %d", 
         start_list.hfn_srb, start_list.hfn_rab);

        //START value and hfn are same for RB2      
        rrcsmc_re_establish_rlc_rb_for_srns_relocation(wrrc_as_id,
...
)
{
  ...
  switch (cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_LLC_RE_EST_RLC_CNF:
      ...
      if(rrcsmc_int_data.saved_srns_info.reconfig_cnf_req == TRUE)
      {
          int_cmd_ptr =  rrc_get_int_cmd_buf(wrrc_as_id);
          /* Fill in the command parameters for RRC_RE_EST_RLC_FOR_SRNS_CNF */
          int_cmd_ptr->cmd_hdr.cmd_id = RRC_RE_EST_RLC_FOR_SRNS_CNF;
          int_cmd_ptr->cmd.rrc_re_est_srns_cnf.status = SUCCESS;
          int_cmd_ptr->cmd.rrc_re_est_srns_cnf.rrc_proc = rrcsmc_int_data.saved_srns_info.proc_type;
          rrc_put_int_cmd(wrrc_as_id, int_cmd_ptr);  
      ...
      if(rrcsmc_int_data.saved_srns_info.rb_type == RRC_RE_EST_RB_2)
      {
      ...
      {
        WRRC_MSG3_HIGH("Re-EST of other RB's done",0,0,0);
        /* Purge all SRBs except RB2 to make sure no SRB message is 
           transmitted with stale start values after continue of RLC */
	rrcsmc_ul_purge_wm_except_srb2(wrrc_as_id);
        //for other rb's send continue_req and go to initial substate
        rrcsmc_send_continue_req(wrrc_as_id);
        
#ifdef FEATURE_UMTS_VOICE_CIPHERING_IND
        #error code not present
#endif

        if(rrcsmc_int_data.saved_srns_info.ciph_algo_present == TRUE)
        {
          WRRC_MSG2_HIGH("S_SMC_Algo:%d, S_SRNS_Algo:%d",
             rrcsmc_int_data.saved_security_mode_command_msg.ciph_algo,
             rrcsmc_int_data.saved_srns_info.ciph_algo);

          rrcsmc_int_data.saved_security_mode_command_msg.ciph_algo = rrcsmc_int_data.saved_srns_info.ciph_algo;
        }
        rrcsmc_int_data.ciphering_info.reconfig = FALSE;
...
)
{
  ...
  if (SUCCESS == rlc_rrc_get_latest_pending_hfn(wrrc_as_id, rrclcm_get_ul_rlc_lc_id(wrrc_as_id,DCCH_AM_RADIO_BEARER_ID),
  ...
  if (FALSE == local_pending_hfn_type.ul_pending_hfn_present)
  {
    ...
    for (i=0; i < local_hfn_type_ptr->ul_nchan; i++)
    {
      if(local_hfn_type_ptr->hfn_per_rb_ul[i].rlc_id == 
          rrclcm_get_ul_rlc_lc_id(wrrc_as_id,DCCH_AM_RADIO_BEARER_ID))
      {
  ...
  if (FALSE == local_pending_hfn_type.dl_pending_hfn_present)
  {
    ...
    for (i=0; i < local_hfn_type_ptr->dl_nchan; i++)
    {
      if(local_hfn_type_ptr->hfn_per_rb_dl[i].rlc_id == 
         rrclcm_get_dl_rlc_lc_id(wrrc_as_id,DCCH_AM_RADIO_BEARER_ID))
      {
  ...
  WRRC_MSG3_HIGH("RB2 re-est: SRB2-HFNs in UL 0x%x DL 0x%x Config HFN 0x%x", ul_rb2_hfn, dl_rb2_hfn, rb2_hfn);
      
  //START value and hfn are same for RB2      
  rrcsmc_re_establish_rlc_rb_for_srns_relocation(wrrc_as_id,RRC_RE_EST_RB_2, rb2_hfn, rb2_hfn);
  rrcsmc_substate = RRCSMC_WAIT_RE_EST_SRNS_CNF;
  rrc_free(local_hfn_type_ptr);
...
===========================================================================*/
boolean rrcsmc_is_rlc_re_est_for_srns_active(sys_modem_as_id_e_type wrrc_as_id)
{
...
)
{
  ...
  /*save transaction id*/
  rrcsmc_int_data.saved_security_mode_command_msg.transaction_id = local_smc_msg_ptr->
  ...
  {
    rrcsmc_int_data.saved_security_mode_command_msg.domain_id = RRC_PS_DOMAIN_CN_ID;
  }
  ...
  if(rrcsmc_int_ciph_info.integrity_info_received == TRUE)
  {
    ...
    {
      rrcsmc_int_data.saved_security_mode_command_msg.start_chosen = TRUE;
      rrcsmc_int_data.saved_security_mode_command_msg.modify_chosen = FALSE;
  
      /*get the fresh value*/
      rrcsmc_int_data.saved_security_mode_command_msg.start = 
      ...
      rrcsmc_int_data.saved_srns_info.fresh_old = rrcsmc_int_data.saved_security_mode_command_msg.start;
      ...
      rrcsmc_int_data.saved_srns_info.fresh_old1 = rrcsmc_int_data.saved_security_mode_command_msg.start;
    }
    else
    {
      WRRC_MSG1_HIGH("Saved FRESH is 0x%x",rrcsmc_int_data.saved_security_mode_command_msg.start);

      rrcsmc_int_data.saved_security_mode_command_msg.start_chosen = FALSE;
      rrcsmc_int_data.saved_security_mode_command_msg.modify_chosen = TRUE;
    ...
    rrcsmc_int_data.saved_security_mode_command_msg.integrity_algo = 
          rrcsmc_get_int_integrity_algo_from_dl_sdu_ind(wrrc_as_id,
            &local_smc_msg_ptr->integrityProtectionModeInfo.integrityProtectionAlgorithm,
             MSG_REL99);

    rrcsmc_int_ciph_info.saved_integrity_algo[rrcsmc_int_data.saved_security_mode_command_msg.domain_id] 
       = rrcsmc_get_int_integrity_algo_from_dl_sdu_ind(wrrc_as_id,
            &local_smc_msg_ptr->integrityProtectionModeInfo.integrityProtectionAlgorithm,
             MSG_REL99);
  ...
  if((rrcsmc_ext_data.nv_ciphering_enabled) &&
     (rrcsmc_int_ciph_info.ciphering_info_received == TRUE))
   {
    ...
    rrcsmc_int_data.saved_security_mode_command_msg.ciph_algo = 
          rrcsmc_get_int_cipher_algo_from_dl_sdu_ind(wrrc_as_id,
              &(local_smc_msg_ptr->cipheringModeInfo.cipheringModeCommand.u.startRestart),
              MSG_REL99);
  
    /*save to temp var*/
    rrcsmc_int_ciph_info.saved_ciph_algo[rrcsmc_int_data.saved_security_mode_command_msg.domain_id] = 
          rrcsmc_get_int_cipher_algo_from_dl_sdu_ind(wrrc_as_id,
              &(local_smc_msg_ptr->cipheringModeInfo.cipheringModeCommand.u.startRestart),
              MSG_REL99);
    ...
    {
      rrcsmc_int_data.saved_security_mode_command_msg.dpch_ciph_act_time_present = TRUE;
      rrcsmc_int_data.saved_security_mode_command_msg.dpch_ciph_act_time = 
    ...
    if (local_smc_msg_ptr->cipheringModeInfo.m.rb_DL_CiphActivationTimeInfoPresent)
    {
      ...
      rrcsmc_int_data.saved_security_mode_command_msg.rb_ciph_dl_act_time_present = TRUE;
  
      /*init local vars used for counter purposes to zero*/
      rrcsmc_int_data.saved_security_mode_command_msg.num_rb_ciph_dl_act_time = 0;
      ...
      while((local_rb_ciph_act_time->n > n_index) && 
           ...
           (rrcsmc_int_data.saved_security_mode_command_msg.num_rb_ciph_dl_act_time < (UE_MAX_DL_UM_CHANNEL + UE_MAX_AM_ENTITY)))
      {
        ...
        {
          rrcsmc_int_data.saved_security_mode_command_msg.rb_ciph_dl_act_time[rrcsmc_int_data.saved_security_mode_command_msg.num_rb_ciph_dl_act_time].rb_id = (uint8) 
            local_rb_ciph_act_time->elem[n_index].rb_Identity;
          rrcsmc_int_data.saved_security_mode_command_msg.rb_ciph_dl_act_time[rrcsmc_int_data.saved_security_mode_command_msg.num_rb_ciph_dl_act_time].seq_num = (uint16)
            local_rb_ciph_act_time->elem[n_index].rlc_SequenceNumber;           
          rrcsmc_int_data.saved_security_mode_command_msg.num_rb_ciph_dl_act_time++;
        }
        else
        {
          for ( count=0; count<MAX_RAB_TO_SETUP; count++ )
          {
            if (rrc_est_rabs.rabs[count].rab_id != RRC_INVALID_RAB_ID)
            {
              for (rb_index = 0; (rb_index < rrc_est_rabs.rabs[count].num_rbs_for_rab) && (rrcsmc_int_data.saved_security_mode_command_msg.num_rb_ciph_dl_act_time < (UE_MAX_DL_UM_CHANNEL + UE_MAX_AM_ENTITY)); rb_index++)
              {
...
)
{
  ...
  /*get the HFN for Step_2 HFN calculation*/
  step_2_hfn = rrcsmc_convert_START_to_count_c_i(wrrc_as_id,
    rrcsmc_int_ciph_info.int_ciph_info[RRC_CS_DOMAIN_CN_ID].hfn, 0);
...
)
{
  switch (cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_CIPH_CONFIG_CNF:
      ...
      rrcsmc_send_ho_ciph_config_cnf(wrrc_as_id,SUCCESS);      
      break;
...
)
{
  ...
  memset(&rrcsmc_int_data.ciphering_info.ul_ciph_act_info, 0, 
         sizeof(rrcsmc_int_data.ciphering_info.ul_ciph_act_info));
  memset(&rrcsmc_int_data.ciphering_info.dl_ciph_act_info, 0, 
         sizeof(rrcsmc_int_data.ciphering_info.dl_ciph_act_info));  
...
if ((CIPH_CONFIG_LTOW == ho_ciph_req.config_cause) || (CIPH_CONFIG_CS_PLUS_PS == ho_ciph_req.config_cause))
  ...
  rrcsmc_ext_data.cipher_config_db.mac_cipher_info[rrcsmc_int_ciph_info.latest_config_cn_id].
    cipher_enable = TRUE;
...
)
{
  ...
  if (TRUE == rrcsmc_int_data.feature_rollback_security_config)
  {
    if (TRUE == rrcsmc_is_security_reconfig_active(wrrc_as_id))
    {
          ...
          (RRCSMC_WAIT_FOR_CIPH_CONFIG_CNF == rrcsmc_substate))
      {
       ...
       WRRC_MSG4_HIGH("latest_config_cn_id new = %d, prev =%d dl_new_key_flag_per_rb= %d, ciph new_key =%d",
        domain_id,rrcsmc_int_ciph_info.latest_config_cn_id,
                       rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].dl_new_key_flag_per_rb[2],
        rrcsmc_int_ciph_info.int_ciph_info[domain_id].ciph_keys_info.new_key
       );
       
        /* If required revert to old integrity keys */
        if ((TRUE == rrcsmc_int_data.integrity_protection_info.count_i_info[domain_id].
        ...
        if( RRCSMC_WAIT_FOR_CIPH_CONFIG_CNF != rrcsmc_substate)
        {
          if ((rrcsmc_ext_data.nv_ciphering_enabled) && (TRUE == is_ciphering_info_received))
          {
            /* Send Abort cipher config req to llc */
              int_cmd_ptr =  rrc_get_int_cmd_buf(wrrc_as_id);
              /* Fill in the command parameters for RRC_CIPH_CONFIG_REQ*/
        ...
        if(rrcsmc_int_data.saved_smc_message.dl_sdu_ptr != NULL)
        {
          WRRC_MSG0_HIGH("BUF_SMC: Buffered SMC is discarded");
          rrc_free(rrcsmc_int_data.saved_smc_message.dl_sdu_ptr);
          rrcsmc_int_data.saved_smc_message.dl_sdu_length = 0;
        }				
        return TRUE;
...
===========================================================================*/
static void rrcsmc_update_latest_cn_id(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8 i = 0;
  ...
  if (rrcsmc_int_data.saved_security_mode_command_msg.modify_chosen)
  {
    ...
    for (i=0; i<RRC_MAX_SRB;i++)
    {
      if (TRUE == rrcsmc_int_data.integrity_protection_info.dl_latest_config_cn_id_old_1_valid[i])
      {
    ...
    WRRC_MSG3_HIGH("latest_cn_id_old_1: %d, latest_cn_id_old: %d, latest_cn_id: %d",
  ...
  /*set the rrcsmc_int_ciph_info.latest_config_cn_id to the domain_id for which we
    received security mode command*/
  rrcsmc_int_ciph_info.latest_config_cn_id =  rrcsmc_int_data.saved_security_mode_command_msg.domain_id;
}
/*===========================================================================
...
)
{
  ...
  if((!rrcsmc_ext_data.nv_ciphering_enabled) && (!rrcsmc_ext_data.nv_integrity_enabled))
  {
    WRRC_MSG2_ERROR("NV cipher = %d & NV integrity = %d not enabled", rrcsmc_ext_data.nv_ciphering_enabled,
                       rrcsmc_ext_data.nv_integrity_enabled);
    return RRCSMC_VALID_CONFIG;
  }
...
{  
  ...
  local_mac = rrcf9_generate_mac_for_message(wrrc_as_id,
   ...
   WRRC_MSG4_HIGH("local_mac = 0x%x, received_mac = 0x%x CN domain %d[0=CS,1=PS], Integrity Algo = %d[0=Invalid,1=UIA1,2=UIA2]",
   local_mac,
   received_mac,
   domain_id,
    rrcsmc_get_int_integrity_algo_from_dl_sdu_ind(wrrc_as_id,
...
)
{
  ...
  /*save transaction id*/
  rrcsmc_int_data.saved_security_mode_command_msg.transaction_id = (OSUINT8)
...
)
{
  ...
  if (RRCSMC_VALID_CONFIG != rrcsmc_validate_security_mode_command_r7(wrrc_as_id,dcch_msg_ptr) )
  {
    WRRC_MSG0_ERROR("SMC MSG VALIDATION FAILED");
    rrcsmc_send_security_mode_failure(wrrc_as_id,rrcsmc_trans_id);          
    return;
  }
  
  /*check if WCDMA security capablities are correct with UECI procedure*/
  if(rrcueci_verify_security_capability(wrrc_as_id,(&dcch_msg_ptr->message.u.
    ...
    securityModeCommand_r7.securityCapability)) == FALSE)
  {
    ...
    (void)rrc_transition_to_disconnected_state(wrrc_as_id,RRC_PROCEDURE_SMC,RRC_TX_TO_DISCON_OTHER);
    return;
  }
  ...
  {
    if(rrcueci_verify_inter_rat_security_capability(wrrc_as_id,
  ...
  if((rrcsmc_ext_data.nv_ciphering_enabled) && (rrcsmc_int_ciph_info.ciphering_info_received == TRUE))
  {  
    /* Validate the SECURITY MODE COMMAND.  If not valid then return without 
        processing the message further. Send SMC failure*/
    if (RRCSMC_VALID_CONFIG != rrcsmc_validate_ciph_security_mode_command_r7(wrrc_as_id,dcch_msg_ptr) )
    {
      ...
      (rrcsmc_int_ciph_info.integrity_info_received == TRUE))
  {
    ...
    if (RRCSMC_VALID_CONFIG != rrcsmc_validate_int_security_mode_command_r7(wrrc_as_id,dcch_msg_ptr) )
    {
    ...
    if (rrcsmc_check_integrity_for_security_mode_command_r7(wrrc_as_id, cmd_ptr) == FAILURE)
    {
    ...
    else
    {
      ...
      if (RRCCU_NOT_COMPLETED  == 
           rrccu_get_cell_update_complete_status(wrrc_as_id,RRC_PROCEDURE_SMC))
      {
  ...
  rrcsmc_save_contents_of_security_mode_command_r7(wrrc_as_id,dcch_msg_ptr);
  if(rrcsmc_ext_data.nv_integrity_enabled &&
     rrcsmc_int_ciph_info.integrity_info_received)
  {
    /* Update all integrity related info. */
    rrcsmc_update_latest_cn_id(wrrc_as_id);
  ...
  else
  {
    ...
    /* Send SECURITY MODE COMPLETE command */
    rrcsmc_send_security_mode_complete(wrrc_as_id,rrcsmc_int_data.saved_security_mode_command_msg.
                                      transaction_id);
  }
}

/*=========================================================================
FUNCTION          RRCSMC_SSGCCS_SEND_SECURITY_MODE_COMMAND_INFO

DESCRIPTION       This function sends the RRC connection release info to SSGCCS QMI client 
...
SIDE EFFECTS
                  None
=========================================================================*/
void rrcsmc_ssgccs_send_security_mode_command_info(sys_modem_as_id_e_type wrrc_as_id)
{
  wcdma_ssgccs_security_mode_command_info_ind_s security_mode_command_info;

  if(security_mode_command_3g_msg_handler == NULL)
  {
    WRRC_MSG0_ERROR("SSGCCS: security mode command msg handler is NULL");
    return;
  }
  memset(&security_mode_command_info,0,sizeof(wcdma_ssgccs_security_mode_command_info_ind_s));
  security_mode_command_info.wrrc_as_id = wrrc_as_id;
  security_mode_command_info.mcc = RRC_CSP_GET_INT_MCC(rrc_csp_int_data.curr_acq_entry.plmn_id);
  security_mode_command_info.mnc = RRC_CSP_GET_INT_MNC(rrc_csp_int_data.curr_acq_entry.plmn_id);
  security_mode_command_info.cellid = rrc_csp_int_data.curr_acq_entry.cell_id;
  security_mode_command_info.ciphering_info_received = rrcsmc_int_ciph_info.ciphering_info_received;
  security_mode_command_info.ciph_algo = rrcsmc_int_data.saved_security_mode_command_msg.ciph_algo;
  WRRC_MSG0_HIGH("SSGCCS: Security Mode Command info sent to SSGCCS QMI client");
  security_mode_command_3g_msg_handler(&security_mode_command_info);
}
/*===========================================================================
...
)
{
  ...
  switch (cmd_ptr->cmd_hdr.cmd_id)
  {
    ...
    case RRC_HO_CIPH_REQ:
      ...
      if(rrc_state != RRC_STATE_CELL_DCH)
      {
          ...
          rrcsmc_send_ho_ciph_config_cnf(wrrc_as_id, FAILURE);
        return;
      }

        /*process the Ciphering request for GtoW Handover*/
      WRRC_MSG0_HIGH("Step 1 Ciph for GtoW HO");
      rrcsmc_process_step_1_ho_ciph_req(wrrc_as_id,cmd_ptr->cmd.ho_ciph_req);
  ...
  case RRC_RE_EST_RLC_FOR_SRNS_REQ:
    ...
    {
      rrcsmc_int_data.saved_srns_info.rb_type = RRC_RE_EST_RB_2;
    }
    ...
    if(rrcsmc_int_data.saved_srns_info.integrity_algo_present == TRUE)
    {
    ...
    //send Stop req
    rrcsmc_perform_stop_or_suspend_functionality(wrrc_as_id, RRCSMC_SRNS_CMD);
    break;
    ...
    case RRC_DOWNLINK_SDU_IND:
      ...
      /*
        ...
        the UE shall
        1> ignore the received SECURITY MODE COMMAND and continue with any ongoing processes and procedures
        as if the SECURITY MODE COMMAND message has not been received.
      */
      if ((rrcsmc_int_data.feature_rollback_security_config == TRUE)
          &&
          (rrccu_get_cell_update_started_status(wrrc_as_id) == RRCCU_STARTED))
      {
      ...
      rrcdata_misc_info.rrc_protocol_error_reject = FALSE;
...
/*lint +e793*/
      ...
      {
        rrcdata_misc_info.rrc_protocol_error_reject = TRUE ;

        /*send a security mode failure command if there is a protocol error
         *in the received security mode command
         */
        rrcsmc_int_data.rrcsmc_failure_type = RRCSMC_PROTOCOL_ERROR; 
        rrcsmc_int_data.rrcsmc_prot_err_cause = rrc_ProtocolErrorCause_asn1_ViolationOrEncodingError;
        rrcsmc_send_security_mode_failure(wrrc_as_id,rrcsmc_trans_id);
      }
      else /* ASN1 decoding is successful */
      {
        ...
        {
          if((rrcllc_get_ordered_config_status(wrrc_as_id) == OC_SET_FOR_FACH_CELL_PCH_TRANS)||
            (rrcllc_get_ordered_config_status(wrrc_as_id) == OC_SET_FOR_DCH_CELL_PCH_TRANS)||
            (rrcllc_get_ordered_config_status(wrrc_as_id) == OC_SET_FOR_FACH_URA_PCH_TRANS)||
            (rrcllc_get_ordered_config_status(wrrc_as_id) == OC_SET_FOR_DCH_URA_PCH_TRANS))
          {
            /*send a security mode failure command if there is a protocol error
             *in the received security mode command
             */
            ...
            rrcsmc_int_data.rrcsmc_failure_type = RRCSMC_PROTOCOL_ERROR; 
            rrcsmc_int_data.rrcsmc_prot_err_cause = rrc_ProtocolErrorCause_messageNotCompatibleWithReceiverState;
            rrcsmc_send_security_mode_failure(wrrc_as_id,rrcsmc_trans_id);
            return;
          }
                  ...
                  T_rrc_SecurityModeCommand_later_than_r3)
        {
          if(rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL7)
          {
              ...
              criticalExtensions.t == T_rrc_SecurityModeCommand_criticalExtensions_1_r7)
            {
              ...
              rrcsmc_process_security_mode_command_r7(wrrc_as_id,dcch_msg_ptr,cmd_ptr);
            ...
            else
            {
              ...
              rrcsmc_int_data.rrcsmc_failure_type = RRCSMC_PROTOCOL_ERROR;
              rrcsmc_int_data.rrcsmc_prot_err_cause = rrc_ProtocolErrorCause_messageExtensionNotComprehended;
              rrcsmc_send_security_mode_failure(wrrc_as_id,dcch_msg_ptr->message.u.
          ...
          rrcsmc_ssgccs_send_security_mode_command_info(wrrc_as_id);
          return;
        }
        else
        {
        ...
        /*check to see that no critical extensions are added and no reconfig
          is currently being done*/
        if (RRCSMC_VALID_CONFIG != rrcsmc_validate_security_mode_command(wrrc_as_id,dcch_msg_ptr) )
        {
          WRRC_MSG0_ERROR("SMC MSG VALIDATION FAILED");
          rrcsmc_send_security_mode_failure(wrrc_as_id,rrcsmc_trans_id);          
          return;
        ...
        /*check if WCDMA security capablities are correct with UECI procedure*/
        if(rrcueci_verify_security_capability(wrrc_as_id,(&dcch_msg_ptr->message.u.securityModeCommand.u.r3.
            ...
            (rrcsmc_int_ciph_info.ciphering_info_received == TRUE))
        {
          /* Validate the SECURITY MODE COMMAND.  If not valid then return without 
             processing the message further. Send SMC failure*/
          if (RRCSMC_VALID_CONFIG != rrcsmc_validate_ciph_security_mode_command(wrrc_as_id,dcch_msg_ptr) )
          {
            WRRC_MSG0_ERROR("Ciphering SMC MSG VALIDATION FAILED");
            rrcsmc_send_security_mode_failure(wrrc_as_id,rrcsmc_trans_id);          
            return;
            ...
            (rrcsmc_int_ciph_info.integrity_info_received == TRUE))
        {
          ...
          /* Validate the RRC SECURITY MODE COMMAND.  If validation not successful 
             then return without processing the message further. 
             Send the security mode failureis set to 0*/
          if (RRCSMC_VALID_CONFIG != rrcsmc_validate_int_security_mode_command(wrrc_as_id,dcch_msg_ptr) )
          {
            WRRC_MSG0_ERROR("Integrity SMC MSG VALIDATION FAILED");
            rrcsmc_send_security_mode_failure(wrrc_as_id,rrcsmc_trans_id);          
            return;
          ...
          if (rrcsmc_check_integrity_for_security_mode_command(wrrc_as_id, cmd_ptr) == FAILURE)
          {
            WRRC_MSG0_ERROR("Integrity Check for SMC cmd Failed");
            /*just return. Unsuccessful integrity check for security mode cmd*/
            return;
        ...
        {
          /*save the contents of security mode command*/
          rrcsmc_save_contents_of_security_mode_command(wrrc_as_id,dcch_msg_ptr);
          if(rrcsmc_ext_data.nv_integrity_enabled &&
             rrcsmc_int_ciph_info.integrity_info_received)
          {
            /* Update all integrity related info. */
            ...
            !(rrcsmc_ext_data.nv_integrity_enabled))
        {
          ...
          /* Send SECURITY MODE COMPLETE command */
          rrcsmc_send_security_mode_complete(wrrc_as_id,rrcsmc_trans_id);
...
)
{
  ...
  {
    case RRCSMC_INITIAL: 
      rrcsmc_process_smc_initial_substate(wrrc_as_id, cmd_ptr);
      break;

    case RRCSMC_WAIT_FOR_L2_ACK:
      rrcsmc_process_wait_for_l2_ack_substate(wrrc_as_id, cmd_ptr);
      break;

    case RRCSMC_WAIT_FOR_RECONFIG_CNF:
      rrcsmc_process_wait_for_reconfig_cnf_substate(wrrc_as_id, cmd_ptr);
      break;

    case RRCSMC_WAIT_FOR_CIPH_CONFIG_CNF:
      rrcsmc_process_wait_for_ciph_config_cnf_substate(wrrc_as_id, cmd_ptr);
      break;

    case RRCSMC_WAIT_FOR_HO_STEP_1_CIPH_CONFIG_CNF:
      rrcsmc_process_wait_for_step_1_ciph_config_cnf_substate(wrrc_as_id, cmd_ptr);
      break;    

    case RRCSMC_WAIT_RE_EST_SRNS_CNF:
      rrcsmc_process_wait_re_est_srns_substate(wrrc_as_id, cmd_ptr);
      break;

    case RRCSMC_WAIT_SRNS_RELOC_L2_ACK_IND:
      rrcsmc_process_wait_srns_reloc_l2_ack_ind_substate(wrrc_as_id, cmd_ptr);
      break;
	  
    case RRCSMC_WAIT_TILL_STANDBY_ENDS:
      rrcsmc_process_wait_till_standby_ends_substate(wrrc_as_id, cmd_ptr);
      break;
...
FUNCTION rrcsmc_buffer_security_mode_command

DESCRIPTION
  This function buffers the SMC message to a global pointer. This message will be reposted after SMC 
  substate is moved to Initial.
...
void rrcsmc_buffer_security_mode_command(sys_modem_as_id_e_type wrrc_as_id,rrc_cmd_type *cmd_ptr)
{
  /* Save the DL SDU length */
  rrcsmc_int_data.saved_smc_message.dl_sdu_length = cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_length;

  /* Allocate memory to save the encoded message */
  rrcsmc_int_data.saved_smc_message.dl_sdu_ptr = 
    (uint8 *)rrc_malloc(cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_length);

  /* Copy the encoded message contents from command pointer */
  WCDMA_MEMCPY(rrcsmc_int_data.saved_smc_message.dl_sdu_ptr,
               cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_length,
               cmd_ptr->cmd.downlink_sdu_ind.dl_sdu, 
               cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_length);

  WRRC_MSG0_HIGH("BUF_SMC: SMC message is Buffered");
}
/*===========================================================================
...
)
{
  ...
  rrc_crash_debug_save_proc_substates(wrrc_as_id,RRC_PROCEDURE_SMC,rrcsmc_substate,cmd_ptr->cmd_hdr.cmd_id);
  ...
  {
    /* Get the message type */    
    message_choice = rrc_get_dl_dcch_message_type(wrrc_as_id,cmd_ptr->cmd.downlink_sdu_ind.dl_sdu,
                                                    cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_length);
    ...
    /* 
           SMC is received while UE is already waiting for L2 ack and the wait timer is running. Buffer this message and repost later. 
           Currently we can buffer only one SMC message at one time. Buffer the message only if the 
           buffer is free.
      */
    if((rrcnv_data.rrc_buffer_smc_support.enable_smc_buffer) && 
       (rrcsmc_int_data.saved_smc_message.dl_sdu_ptr == NULL) &&
       ((rrcsmc_substate == RRCSMC_WAIT_FOR_L2_ACK)||
        (rrcsmc_substate == RRCSMC_WAIT_FOR_CIPH_CONFIG_CNF)) &&
        (rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_WAIT_FOR_L2_ACK_FOR_PCH_TRANSITION_TIMER)))
    {
      rrcsmc_buffer_security_mode_command(wrrc_as_id,cmd_ptr);
    }
    else
    {
      if (dcch_msg_ptr == NULL) /* DCCH ASN1 decoding failed */
    ...
    else
    {
      ...
      switch (rrc_state)
      {
        case RRC_STATE_CELL_DCH:  
          ...
          rrcsmc_process_cell_dch_state(wrrc_as_id,cmd_ptr); 
          break;
    
        case RRC_STATE_CELL_FACH:    
          /* CELL_FACH and CELL_DCH only valid states to receive 
             SECURITY MODE COMMAND message */
          rrcsmc_process_cell_fach_state(wrrc_as_id,cmd_ptr); 
          break;
    
        case RRC_STATE_CELL_PCH:
        case RRC_STATE_URA_PCH:
          rrcsmc_process_pch_state(wrrc_as_id,cmd_ptr); 
          break;
        ...
        default: 
          ...
          WRRC_ERR1_FATAL("RRCSMC Msg received in Invalid State: %d",rrc_state);
          break;
      }  
    }
  }
...
)
{
  ...
  wnv_get_nv_msim(WNV_ID_RRC_INTEGRITY_ENABLED_I, &rrcsmc_ext_data.nv_integrity_enabled, sizeof(rrcsmc_ext_data.nv_integrity_enabled), wrrc_as_id);
  WRRC_MSG1_HIGH("NV_INTEGRITY_ENABLED is %d", rrcsmc_ext_data.nv_integrity_enabled);

  wnv_get_nv_msim(WNV_ID_RRC_CIPHERING_ENABLED_I, &rrcsmc_ext_data.nv_ciphering_enabled_status, sizeof(rrcsmc_ext_data.nv_ciphering_enabled_status), wrrc_as_id);
  WRRC_MSG1_HIGH("NV_CIPHERING_ENABLED is %d", rrcsmc_ext_data.nv_ciphering_enabled_status);

  /*set nv_ciphering_enabled to 1, as most of the code 
    is under this flag, We have to support UEA0 even if this Flag is set OFF */
  rrcsmc_ext_data.nv_ciphering_enabled = WNV_GET_DEFAULT(WNV_ID_RRC_CIPHERING_ENABLED_I);

}

/*===========================================================================

FUNCTION rrcsmc_init_int_data

DESCRIPTION
  This function initializes the internal global database
  procedure.
...
===========================================================================*/
void rrcsmc_init_int_data(sys_modem_as_id_e_type wrrc_as_id)
{

  rrcsmc_int_data.feature_rollback_security_config = TRUE;
...
)
{
  ...
  rrcsmc_init_start_value_at_power_up(wrrc_as_id);

  /* register for state change ind when UE goes to idle disconnected*/
  rrcscmgr_register_for_scn(wrrc_as_id,RRC_PROCEDURE_SMC,
                            RRC_STATE_WILDCARD,
  ...
  rrcsmc_init_int_data(wrrc_as_id);
} /* rrcsmc_init_procedure */
/*===========================================================================
...
)
{
  ...
  {
    if((TRUE == rrcllc_reconfig_in_progress(wrrc_as_id)) &&
       (oc_status != OC_NOT_SET) &&
       (proc_id == RRC_PROCEDURE_RBE) &&
       (RRCRB_WAIT_FOR_CELL_UPDATE_CNF == rrcrbe_get_current_sub_state(wrrc_as_id)) &&
       (TRUE == rrcsmc_get_start_value_to_transmit_flag_status(wrrc_as_id)))
    {
      if(TRUE == rrcsmc_get_calc_start_value(wrrc_as_id,start_value))
      {
        status = rrcsmc_append_already_computed_start_value_to_start_list(wrrc_as_id,start_list_ext,
                                                       start_value, 
                                                       start_type,
                                                       TRUE,
                                                       RRC_CS_DOMAIN_CN_ID);
  ...
  if(rrcsmc_get_security_start_status(wrrc_as_id) == TRUE)
  {
    /*Check if CS domains is in EST_RAB */
    if(rrcsmc_is_cn_associated_with_any_est_rab(wrrc_as_id, RRC_CS_DOMAIN_CN_ID) == TRUE)
    {
    ...
    if(rrcsmc_is_cn_associated_with_any_est_rab(wrrc_as_id, RRC_PS_DOMAIN_CN_ID) == TRUE)
    {
  ...
  WRRC_MSG3_HIGH("Domain status, CS=%d,PS=%d,Latest cn i.d=%d",is_cs_domain_open,
...
)
{
  ...
  for (i=0; i<rrcsmc_int_ciph_info.num_cn_domains; i++)
  {
    ...
    rrcsmc_append_start_per_domain (wrrc_as_id, (byte *) (start_list->elem[index].start_Value.data),
                                    domain_id,
                                    start_type);

    if(i == (uint32) RRC_CS_DOMAIN_CN_ID)
    {
      WRRC_MSG3_HIGH("Appended CS-START: 0x%x 0x%x 0x%x",start_list->elem[index].start_Value.data[0],
               start_list->elem[index].start_Value.data[1],
               start_list->elem[index].start_Value.data[2]);
    }
    else
    {
      WRRC_MSG3_HIGH("Appended PS-START: 0x%x 0x%x 0x%x", start_list->elem[index].start_Value.data[0],
               start_list->elem[index].start_Value.data[1],
               start_list->elem[index].start_Value.data[2]);
    }
#ifdef FEATURE_SMC_APPEND_ACTIVE_CN_DOMAIN_START_IN_CU
...
)
{
  ...
  {
    updated_hfn = 
    rrcsmc_convert_START_to_count_c_i(wrrc_as_id,rrcsmc_int_ciph_info.int_ciph_info[domain_id].hfn, 0);
  }
  //otherwise, START needs to be re-calculated
  else
  {
    // Do start value calculation for this domain
    rrcsmc_extract_latest_start_list_per_cn_domain(wrrc_as_id,domain_id, RRCSMC_UPDATE_START_LIST_LATEST);
    ...
    updated_hfn = 
    rrcsmc_convert_START_to_count_c_i(wrrc_as_id,rrcsmc_int_ciph_info.int_ciph_info[domain_id].hfn_latest, 0);
  }
...
)
{
  ...
  hfn_update = rrcsmc_extract_return_hfn_per_domain_and_update_start_flag(wrrc_as_id, RRC_PS_DOMAIN_CN_ID);
  
  // Since this is for AM channel, shift bits to make it a AM HFN
  hfn_update >>= RRC_RLC_AM_HFN_OFFSET;

  if(RRCLCM_RLC_LC_ID_NOT_FOUND == (rlc_id = rrclcm_get_dl_rlc_lc_id(wrrc_as_id,rb_id)))
  {
    WRRC_ERR1_FATAL("lc_id not found for rb_id %d",rb_id);
  }

  WRRC_MSG3_HIGH("Updated HFN is %d for LC_ID %d, direction %d", hfn_update, rlc_id, direction);

  // Call RLC interface function to overwrite HFN value
  if(FAILURE == rlc_rrc_overwrite_hfn(wrrc_as_id, rlc_id, hfn_update, direction))
  {
...
)
{
  ...
  {
    /* Send commands to L1 */
    (void) rrcllc_cphy_ciphering_key_req(wrrc_as_id);
  }
  else
  {
    WRRC_MSG2_ERROR("Error: cant config ul cipher key's,rlc_suspend=%d,cipher_reconfig=%d",
                            rrcsmc_int_data.smc_rb_suspend_or_stop_info.rlc_rb_suspended_or_stopped_flag,
                            rrcsmc_int_data.ciphering_info.reconfig);
  }
}
...
===========================================================================*/
rrc_cn_domain_identity_e_type rrcsmc_get_start_value_to_transmit_domain(sys_modem_as_id_e_type wrrc_as_id)
{
  return(rrcsmc_int_data.start_value_to_transmit_domain);
}
/*===========================================================================
...
===========================================================================*/
boolean rrcsmc_get_security_start_status(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8 cn_id;
  boolean status = FALSE;
...
)
{
  ...
  rrcsmc_int_data.saved_security_key_set.grat_key_set_valid = FALSE;
}
/*===========================================================================
...
{
  if(rrcsmc_return_saved_security_key_set_status(wrrc_as_id,
       domain_id) == TRUE)
  {
    rrc_state_e_type rrc_state = rrc_get_state(wrrc_as_id); /* local var to store rrc state*/
    /*local ciphering info type used to init global vars*/
    ...
    WCDMA_MEMCPY(local_int_ciph_info_ptr->int_ciph_info[RRC_CS_DOMAIN_CN_ID].integrity_key,
                 sizeof(byte) * RRC_MAX_INTEGRITY_KEY_LENGTH,
                 rrcsmc_int_data.saved_security_key_set.integrity_key,
                 RRC_MAX_INTEGRITY_KEY_LENGTH);
    ...
    */
    if (rrcsmc_int_ciph_info.int_ciph_info[RRC_CS_DOMAIN_CN_ID].ciph_keys_info.key_used == 1)
    {
      WCDMA_MEMCPY(rrcsmc_int_ciph_info.int_ciph_info[RRC_CS_DOMAIN_CN_ID].ciph_keys_info.key[0],
                   sizeof(byte) * RRC_MAX_CIPHERING_KEY_LENGTH,
                   rrcsmc_int_data.saved_security_key_set.ciphering_key,
                   RRC_MAX_CIPHERING_KEY_LENGTH);
    }
    else
    {
    ...
    /*Print integrity key here*/
    PRINT_CIPHER_INT_KEY(RRC_PRINT_INTEGRITY_KEY,rrcsmc_int_data.saved_security_key_set)
...
)
{
  ...
  if (rrcsmc_int_data.smc_rb_suspend_or_stop_info.rlc_rb_suspended_or_stopped_flag == TRUE )
  {
    ...
    for(i=0; i < rrcsmc_int_data.smc_rb_suspend_or_stop_info.dl_stop_info.nchan; i++)
    {
    ...
    /* Send command to L2 UL Task */
    rrc_send_l2_dl_cmd(wrrc_as_id, rlc_ptr_dl);
...
)
{
...
#ifdef FEATURE_SMC_APPEND_ACTIVE_CN_DOMAIN_START_IN_CU
  WRRC_MSG3_HIGH("start_type:%d, both_cn_domain_open:%d active_cn_domain:%d",
            append_start_type,
            both_cn_domain_open,
            active_cn_domain);
#endif
  ...
  for (i=0; i<rrcsmc_int_ciph_info.num_cn_domains; i++)
  {
    ...
    //append start value for this domain
    if ((domain_id == RRC_CS_DOMAIN_CN_ID) &&
        (RRC_CS_DOMAIN_CN_ID == rrcsmc_get_start_value_to_transmit_domain(wrrc_as_id)))
    {
      ...
      WCDMA_MEMCPY(start_list_ext->elem[index].start_Value.data,
             ...
             (RRC_PS_DOMAIN_CN_ID == rrcsmc_get_start_value_to_transmit_domain(wrrc_as_id)))
    {
      /* Copy the contents of the start value that was calculated for ps domain */
      WCDMA_MEMCPY(start_list_ext->elem[index].start_Value.data,
                   sizeof(start_list_ext->elem[index].start_Value.data),
    ...
    {
      rrcsmc_append_start_per_domain(wrrc_as_id,
    ...
    if(i == (uint32) RRC_CS_DOMAIN_CN_ID)
    {
      WRRC_MSG3_HIGH("Appended CS-START: 0x%x 0x%x 0x%x", start_list_ext->elem[index].start_Value.data[0],
               start_list_ext->elem[index].start_Value.data[1],
               start_list_ext->elem[index].start_Value.data[2]);
    }
    else
    {
      WRRC_MSG3_HIGH("Appended PS-START: 0x%x 0x%x 0x%x", start_list_ext->elem[index].start_Value.data[0],
               start_list_ext->elem[index].start_Value.data[1],
               start_list_ext->elem[index].start_Value.data[2]);
    }
    index++;
...
RETURN VALUE
  None
===========================================================================*/
void rrcsmc_revert_to_old_config_for_failure_message(sys_modem_as_id_e_type wrrc_as_id,boolean need_log)
{
  ...
  if(rrcsmc_ext_data.cipher_config_db.mac_cipher_info[RRC_CS_DOMAIN_CN_ID].cipher_cfg_type == HO_STEP1_CIPHER_CFG)
  {
    rrcsmc_revert_to_old_cipher_config_db(wrrc_as_id);
  }
  if(need_log)
  {
    WRRC_MSG3_HIGH("Resetting SVTHHO from 0x%x -> 0",rrcsmc_int_data.start_value_to_transmit_cs_hho,0,0);
  }  
  rrcsmc_int_data.start_value_to_transmit_cs_hho = 0;
}
/*===========================================================================
...
none
===========================================================================*/
rrcsmc_substate_e_type  rrcsmc_get_smc_sub_state(sys_modem_as_id_e_type wrrc_as_id)
{
...
#endif
rrcsmc_substate_e_type rrcsmc_return_substate(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcsmc_substate;
}
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
...
void rrcqsh_log_smc_ota_params(sys_modem_as_id_e_type wrrc_as_id)
{
  ((wrrc_qsh_metric_ota_s*)rrc_qsh_metric_db[WRRC_QSH_METRIC_OTA].data_ptr)->ota_info.smc_params.domain = rrcsmc_int_data.saved_security_mode_command_msg.domain_id; 
  ((wrrc_qsh_metric_ota_s*)rrc_qsh_metric_db[WRRC_QSH_METRIC_OTA].data_ptr)->ota_info.smc_params.int_algo = rrcsmc_int_data.saved_security_mode_command_msg.integrity_algo; 
  ((wrrc_qsh_metric_ota_s*)rrc_qsh_metric_db[WRRC_QSH_METRIC_OTA].data_ptr)->ota_info.smc_params.ciph_algo = rrcsmc_int_data.saved_security_mode_command_msg.ciph_algo; 
...
{
  qsh_mdump_collect_high(&rrcsmc_int_ciph_info,sizeof(rrcsmc_int_ciph_info));
  qsh_mdump_collect_high(&rrcsmc_int_data,sizeof(rrcsmc_int_data));
}
#endif
