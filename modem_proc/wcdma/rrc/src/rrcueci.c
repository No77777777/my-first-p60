...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcueci.c#3 $ $DateTime: 2020/02/20 23:21:46 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/22/17   sp      Mini dump phase-2 changes
06/23/17   sas     Changes to provide option for enabling truncated LTE-CA capability reporting for all PLMNs.
...
03/07/18   sas     Made changes to clear variant for the received Eutra capability response from LTE.
...
06/10/16   sr      Made changes to fill LTE band capability using LRRC api
05/20/16   sr      Changes to indicate PLMN ID in EUTRA capabilities request
...
04/19/16   vs      Changes to cleanup unnecessary f3s
11/30/15   vi      Made changes to avoid sending fach meas ind to WL1 in multisim mode
...
08/27/15   sr      Changes to decouple TDD PSHO NV from TDD CM measurements
...
05/20/15   vi      Made changes to avoid linker errors after disabling LTE
03/13/15   ad      Made changes to log important parms from OTAs
02/27/15   sp      Made changes for Mav debug snapshot
...
const uint8 rrc_macehs_cat = 20;
#elif defined(FEATURE_WCDMA_MIMO)
const uint8 rrc_macehs_cat = 18;
#else
const uint8 rrc_macehs_cat = 14;
#endif
...
rrcueci_ext_data_type rrcueci_ext_data_sim[MAX_RRC_STACKS];
#define rrcueci_ext_data rrcueci_ext_data_sim[WAS_ID_M]
#else
rrcueci_ext_data_type rrcueci_ext_data;
#endif
...
** ----------------------------------------------------------------------- */
#ifdef FEATURE_DUAL_WCDMA 
LOCAL rrcueci_int_data_type rrcueci_int_data_sim[MAX_RRC_STACKS];
#define rrcueci_int_data rrcueci_int_data_sim[WAS_ID_M]
#else
LOCAL rrcueci_int_data_type rrcueci_int_data;
#endif
...
SIDE EFFECTS
  None
===========================================================================*/
void rrcueci_reset_ueci_procedure(sys_modem_as_id_e_type wrrc_as_id)
{
  /* Set conter V304 to zero */
  rrcdata_timers_counters_info.rrc_v_304 = 0;

  /* Stop T304 timer - no need to check whether this timer is running or not because its
       taken care inside this function 
  */
  rrctmr_stop_timer(wrrc_as_id, RRCTMR_T_304_TIMER);

  /* Reset the UECI state machine */
  rrcueci_int_data.rrcueci_substate = RRCUECI_INITIAL;

#ifdef FEATURE_WCDMA_REL7_SPECRS
  rrcueci_int_data.rrcueci_ue_cap_originator = RRC_UE_CAPABILITY_ORIGINATOR_NONE;
#endif
#ifdef FEATURE_WCDMA_TO_LTE
  rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_req_posted = FALSE;
  rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_truncated_cap_requested = FALSE;
  rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_truncated_cap_appended = FALSE;

  /* Free the container and reset lte_rsp_received only on moving to idle */
  if(rrc_get_state(wrrc_as_id) == RRC_STATE_DISCONNECTED)
  {
    rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_rsp_received = FALSE;
    /* Freeing the capability contianer received from LTE */
    if(rrcueci_int_data.rrcueci_interrat_lte_capability_info.cap_cont_ptr)
    {
      rrc_free(rrcueci_int_data.rrcueci_interrat_lte_capability_info.cap_cont_ptr);
      rrcueci_int_data.rrcueci_interrat_lte_capability_info.cap_cont_ptr = NULL;
    }
  }
...
SIDE EFFECTS
  None
===========================================================================*/
static rrc_MaximumAM_EntityNumberRLC_Cap rrcueci_get_support_am_entity(sys_modem_as_id_e_type wrrc_as_id)
{
...
)
{
  ...
  /*lint -save -e778 */
  WRRC_MSG2_HIGH("nv_ciphering_enabled_status %d, nv_integrity_enabled %d", rrcsmc_ext_data.nv_ciphering_enabled_status, rrcsmc_ext_data.nv_integrity_enabled);
  
  if (rrcsmc_ext_data.nv_ciphering_enabled_status) 
  {
  ...
  if( (rrcsmc_ext_data.nv_integrity_enabled) && (status != FALSE) )
  { /*lint -save -e572 */
...
===========================================================================*/
static boolean rr_rrc_is_supported_band_internal(sys_modem_as_id_e_type wrrc_as_id, rr_internal_band_T band)
{
#ifdef FEATURE_DUAL_SIM
  return  rr_ds_is_supported_band(band, rrc_get_as_id(wrrc_as_id));
#else
...
uint8 num_gsm_bands_to_report = 0;
  ...
  for(index=0; index<ueci_band_ptr->num_supported_bands; index++)
  {
    ...
    if(((rrcmeas_ext_data.process_interfreq_interrat_meas_rel7  == FALSE) && 
         (rrcmeas_ext_data.process_interfreq_uei_meas_rel7 == FALSE)) ||
         ((rrccsp_channel_locking_nv_status(wrrc_as_id)) && !rrccsp_is_ue_in_true_limited_service(wrrc_as_id)))
    {
...
)
{
  ...
  rrcueci_ext_data.rrc_ue_pos_cap_pch = FALSE;
#endif
  ...
  if(ue_radio_access_capability_ptr != NULL)
  {
    ...
    rrcgps_get_cgps_ue_pos_capability(wrrc_as_id,&pos_capability);
    ...
    rrcueci_ext_data.rrc_ue_pos_cap_pch = pos_capability.up_meas_validity_cell_pch_and_ura_pch ;
    ...
    {    
      ...
      if (rrcmcm_is_dualmode_enabled_no_f3(wrrc_as_id))
      {
        ...
        if(rr_rrc_is_supported_band_internal(wrrc_as_id,BAND_EGSM) || rr_rrc_is_supported_band_internal(wrrc_as_id,BAND_PGSM))
        {
        ...
        ue_radio_access_capability_ptr->measurementCapability.
          downlinkCompressedMode.gsm_Measurements.dcs1800 = rr_rrc_is_supported_band_internal(wrrc_as_id, BAND_DCS);
  
        ue_radio_access_capability_ptr->measurementCapability.
          downlinkCompressedMode.gsm_Measurements.gsm1900 = rr_rrc_is_supported_band_internal(wrrc_as_id, BAND_PCS);
      }
    
      /* Compressed mode required to support Downlink FDD Mesaurements */
      if(((rrccsp_channel_locking_nv_status(wrrc_as_id)) && !rrccsp_is_ue_in_true_limited_service(wrrc_as_id)))
      {
        ue_radio_access_capability_ptr->measurementCapability.
    ...
    if(update_cause == RRCUECI_INT_CAP_UPDATE_2100)
    {
      ...
      WRRC_MSG1_MED("Send UE power class: %d (1->class1..4->class4)",
    ...
    ue_radio_access_capability_ptr->rlc_Capability.
      maximumAM_EntityNumber = rrcueci_get_support_am_entity(wrrc_as_id);
...
)
{
  ...
  {
    if(rrcdata_misc_info.rrc_capability_update_requirement.system_specific_cap_update_req_list.value[count] == RRC_EUTRA_CAP)
    {
      eutra_cap_req = TRUE;
      break;
    }
  ...
  if((!rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_rsp_received) ||
     (rrcueci_int_data.rrcueci_interrat_lte_capability_info.cap_cont_ptr == NULL))
  {
    if((!rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_rsp_received) && rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_req_posted)
    {
  ...
  {
    if(rrcdata_misc_info.rrc_capability_update_requirement.req_EUTRA_frequency_band_list_present == TRUE)
    {
      /*Save the complete capability info received earlier.*/
      cap_cont_length = rrcueci_int_data.rrcueci_interrat_lte_capability_info.cap_cont_length;
      cap_cont_ptr = rrcueci_int_data.rrcueci_interrat_lte_capability_info.cap_cont_ptr;
      rrcueci_send_eutra_capabilitites_req_to_lte_nw_req_bands_and_rcv(wrrc_as_id,  /*Fresh memory will be allocated to 'cap_cont_ptr'*/
         rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_truncated_cap_requested);
    }
    ...
    irat_ue_cap_860ext->eutra_RadioAccessCapability.ue_EUTRA_Capability.numocts =
         rrcueci_int_data.rrcueci_interrat_lte_capability_info.cap_cont_length ;
    irat_ue_cap_860ext->eutra_RadioAccessCapability.ue_EUTRA_Capability.data = 
        rtxMemAllocZ(&enc_ctxt, rrcueci_int_data.rrcueci_interrat_lte_capability_info.cap_cont_length);
    if(NULL == irat_ue_cap_860ext->eutra_RadioAccessCapability.ue_EUTRA_Capability.data)
    {
      WRRC_ERR0_FATAL("Unable to allocate memory for LTE specific capability");
    }
    WCDMA_MEMCPY((void*)irat_ue_cap_860ext->eutra_RadioAccessCapability.ue_EUTRA_Capability.data, 
                 rrcueci_int_data.rrcueci_interrat_lte_capability_info.cap_cont_length,
                 rrcueci_int_data.rrcueci_interrat_lte_capability_info.cap_cont_ptr,
                 rrcueci_int_data.rrcueci_interrat_lte_capability_info.cap_cont_length);
    ...
    { 
      ...
      /*Copy back the entire capability received prior to LTE_RRC_NW_REQ_EUTRA_CAPABILITIES_RSP.*/
      rrcueci_int_data.rrcueci_interrat_lte_capability_info.cap_cont_length = cap_cont_length;
      rrcueci_int_data.rrcueci_interrat_lte_capability_info.cap_cont_ptr = cap_cont_ptr;
      rrcdata_misc_info.rrc_capability_update_requirement.req_EUTRA_frequency_band_list_present = FALSE;
    }
    if(rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_truncated_cap_requested)
    {
      rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_truncated_cap_appended = TRUE;
    }
    return TRUE;
  }
}
...
)
{
  ...
  WRRC_MSG3_HIGH("Num supported bands: FDD:%d, FDD2:%d, Total:%d",
  ...
  {
    WRRC_ERR0_FATAL("Unsupported band-config. No FDD/FDD2 bands selected");
  }


  if(update_cause == RRCUECI_CAP_HO_INFO_TRANSFER)
  {
    plmn_id = mmumts_convert_nas_plmn_id_to_rrc_plmn_id(rrciho_ext_data.irat_plmn_id);
  }
  else
  {
    plmn_id = rrc_ccm_get_curr_camped_plmn(wrrc_as_id);
  }
...
#ifdef FEATURE_LTE_TO_WCDMA
          && !rrcmcm_is_lte_mode_enabled_wo_f3( wrrc_as_id)
#endif
        )|| ((RRCUECI_CAP_HO_INFO_TRANSFER != update_cause) && 
            !((rrcmcm_is_dualmode_enabled_no_f3(wrrc_as_id)) && 
            ((rr_rrc_is_supported_band_internal(wrrc_as_id,BAND_EGSM)) ||
             (rr_rrc_is_supported_band_internal(wrrc_as_id,BAND_PGSM)) ||
             (rr_rrc_is_supported_band_internal(wrrc_as_id,BAND_GSM_850)))))
             ))
  {
    ...
    {
      WRRC_MSG0_MED("2100 band for HO");
      ...
      rrcueci_append_ue_radio_access_capability_ext(wrrc_as_id,&local_fdd_list->elem[0], rrc_RadioFrequencyBandFDD_fdd2100, &ueci_band
#ifdef FEATURE_LTE_TO_WCDMA
    ...
    rrcueci_append_ue_radio_access_capability_non_ext(wrrc_as_id,ue_radio_access_capability_ptr, 
                                                    RRCUECI_INT_CAP_UPDATE_2100);
  }
  else
  {
...
#ifdef FEATURE_LTE_TO_WCDMA
     if((RRCUECI_CAP_HO_INFO_TRANSFER == update_cause) && (rrcmcm_is_lte_mode_enabled_wo_f3(wrrc_as_id)))
     {
       WRRC_MSG0_MED("EUTRA Enabled");
     }
     else
#endif
     {
       WRRC_MSG0_MED("Multi-Band or Non-2100 band");
     }
    //update extensions sent flag to true
    is_extensions_sent=TRUE;
       ...
       (num_total_supported_bands > RRC_UECI_MAX_BANDS_ALLOWED))
    {
      ...
      WRRC_MSG1_HIGH("MCC belongs to US %d", rrccsp_does_mcc_belong_to_US(wrrc_as_id,plmn_id));
      if(TRUE == rrccsp_does_mcc_belong_to_US(wrrc_as_id,plmn_id))
      {
      ...
      WRRC_MSG3_HIGH("After filtering: Num supported bands: FDD:%d, FDD2:%d, Total:%d",
    ...
    for(count = 0; count < ueci_band.num_supported_bands; count++)
    {
      ...
      if(update_cause == RRCUECI_CAP_HO_INFO_TRANSFER)
      {
        rrcueci_append_ue_radio_access_capability_ext(wrrc_as_id,&temp_fdd_list->elem[count], 
    ...
    {
      WRRC_MSG0_MED("We will not fill FDD2 if even one FDD band is present");
    }
    ...
    if(ueci_band.num_supported_bands_fdd2 > 0)
    {
      ...
      {
        rrcueci_append_ue_radio_access_capability_fdd2_ext(wrrc_as_id,&temp_fdd2_list->elem[count], 
      ...
      for(count = 0; (count < ueci_band.num_supported_bands) && (count < RRC_UECI_MAX_BANDS_ALLOWED);count++)
      {
        ...
        rrcueci_append_ue_radio_access_capability_fdd_ext(wrrc_as_id,&temp_fdd_list_ext->elem[count], 
          (rrc_RadioFrequencyBandFDD)ueci_band.band_type[count], &ueci_band);
...
static void rrc_update_db_dc_band_supported(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  uint8 count = 0;
  rrcueci_ext_data.db_dc_band_comb_list.num_db_dc_band_comb_supp = 0;

  if((TRUE == rfm_wcdma_get_dbdc_band_support(&num_band_comb_supported,&band_comb_list)) && (band_comb_list!=NULL))
  {
    for (count=0; (count < num_band_comb_supported) && (rrcueci_ext_data.db_dc_band_comb_list.num_db_dc_band_comb_supp < RRC_NUMBER_OF_DB_DC_BAND_COMB) ; count++)
    {
      if((band_comb_list[count].prim_band == SYS_BAND_WCDMA_I_IMT_2000 ) && (rrccsp_is_supported_band(wrrc_as_id,rrc_convert_sys_band_class_to_rrc_band(wrrc_as_id,SYS_BAND_WCDMA_I_IMT_2000))))
      {
        if ((band_comb_list[count].sec_band == SYS_BAND_WCDMA_VIII_900) && (rrccsp_is_supported_band(wrrc_as_id,rrc_convert_sys_band_class_to_rrc_band(wrrc_as_id,SYS_BAND_WCDMA_VIII_900)))
           && (rrc_get_band_comb_control_nv(wrrc_as_id) & (1 << (RRC_DB_DC_BAND1_BAND8-1))))
        {
          rrcueci_ext_data.db_dc_band_comb_list.db_dc_band_comb_list[rrcueci_ext_data.db_dc_band_comb_list.num_db_dc_band_comb_supp] = RRC_DB_DC_BAND1_BAND8;
          rrcueci_ext_data.db_dc_band_comb_list.num_db_dc_band_comb_supp++;
        }
        else if ((band_comb_list[count].sec_band == SYS_BAND_WCDMA_V_850) && (rrccsp_is_supported_band(wrrc_as_id,rrc_convert_sys_band_class_to_rrc_band(wrrc_as_id,SYS_BAND_WCDMA_V_850)))
               && (rrc_get_band_comb_control_nv(wrrc_as_id) & (1 << (RRC_DB_DC_BAND1_BAND5-1))))
        {
          rrcueci_ext_data.db_dc_band_comb_list.db_dc_band_comb_list[rrcueci_ext_data.db_dc_band_comb_list.num_db_dc_band_comb_supp] = RRC_DB_DC_BAND1_BAND5;
          rrcueci_ext_data.db_dc_band_comb_list.num_db_dc_band_comb_supp++;
        }
        else if ((band_comb_list[count].sec_band == SYS_BAND_WCDMA_IX_1700) && (rrccsp_is_supported_band(wrrc_as_id,rrc_convert_sys_band_class_to_rrc_band(wrrc_as_id,SYS_BAND_WCDMA_IX_1700)))
          && (rrc_get_band_comb_control_nv(wrrc_as_id) & (1 << (RRC_DB_DC_BAND1_BAND9-1))))
        {
          rrcueci_ext_data.db_dc_band_comb_list.db_dc_band_comb_list[rrcueci_ext_data.db_dc_band_comb_list.num_db_dc_band_comb_supp] = RRC_DB_DC_BAND1_BAND9;
          rrcueci_ext_data.db_dc_band_comb_list.num_db_dc_band_comb_supp++;
        }
      }
      else if((band_comb_list[count].prim_band == SYS_BAND_WCDMA_II_PCS_1900 ) && (rrccsp_is_supported_band(wrrc_as_id,rrc_convert_sys_band_class_to_rrc_band(wrrc_as_id,SYS_BAND_WCDMA_II_PCS_1900))))
      {
        if ((band_comb_list[count].sec_band == SYS_BAND_WCDMA_IV_1700) && (rrccsp_is_supported_band(wrrc_as_id,rrc_convert_sys_band_class_to_rrc_band(wrrc_as_id,SYS_BAND_WCDMA_IV_1700)))
         && (rrc_get_band_comb_control_nv(wrrc_as_id) & (1 << (RRC_DB_DC_BAND2_BAND4-1))))
        {
          rrcueci_ext_data.db_dc_band_comb_list.db_dc_band_comb_list[rrcueci_ext_data.db_dc_band_comb_list.num_db_dc_band_comb_supp] = RRC_DB_DC_BAND2_BAND4;
          rrcueci_ext_data.db_dc_band_comb_list.num_db_dc_band_comb_supp++;
        }
        else if((band_comb_list[count].sec_band == SYS_BAND_WCDMA_V_850) && (rrccsp_is_supported_band(wrrc_as_id,rrc_convert_sys_band_class_to_rrc_band(wrrc_as_id,SYS_BAND_WCDMA_V_850)))
        && (rrc_get_band_comb_control_nv(wrrc_as_id) & (1 << (RRC_DB_DC_BAND2_BAND5-1))))
        {
          rrcueci_ext_data.db_dc_band_comb_list.db_dc_band_comb_list[rrcueci_ext_data.db_dc_band_comb_list.num_db_dc_band_comb_supp] = RRC_DB_DC_BAND2_BAND5;
          rrcueci_ext_data.db_dc_band_comb_list.num_db_dc_band_comb_supp++;
        }
      }
    }
...
)
{
  ...
  WRRC_MSG0_HIGH("Populating later_than_R3 for UECI");  
  /* Check whether Radio Access capability is a valid pointer */
  if(ue_radio_access_capability_ptr != NULL)
  {
    ...
    */  
    if(update_cause == RRCUECI_CAP_UPDATE)
    {
...
#ifdef FEATURE_WCDMA_REL7_SPECRS
      WRRC_MSG7_MED("RRCHS:Setting HSDPA category rrcueci_ue_cap_originator %d ue_cap_change_requested.is_valid_mask 0x%x rrccouec.ue_cap_change_requested.hsdpa_category %d rrc_ue_cap_during_rrc_conn.is_valid_mask 0x%0x rrc_ue_cap_during_rrc_conn.hsdpa_category %d nv_hsdpa_category %d rrc_machs_cat %d", 
      rrcueci_int_data.rrcueci_ue_cap_originator, 
      rrccouec.ue_cap_change_requested.is_valid_mask, 
      rrccouec.ue_cap_change_requested.hsdpa_category, 
      rrccouec.rrc_ue_cap_during_rrc_conn.is_valid_mask, 
      rrccouec.rrc_ue_cap_during_rrc_conn.hsdpa_category, 
      rrcnv_data.nv_hsdpa_category, 
      rrc_machs_cat);

      if((rrcueci_int_data.rrcueci_ue_cap_originator == RRC_UE_CAPABILITY_ORIGINATOR_COUEC) && 
      ...
      else if(rrccouec.rrc_ue_cap_during_rrc_conn.is_valid_mask & HSDPA_CAT_INCL)
      {
        if( rrccouec.rrc_ue_cap_during_rrc_conn.hsdpa_category > 12 )
        {
          ue_radio_access_capability_ptr->physicalChannelCapability.fdd_hspdsch.u.
          supported->hsdsch_physical_layer_category = rrc_machs_cat;
        }
      ...
      if ((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL6)
          && (rrcnv_data.rrc_wcdma_hsupa_cm_enabled == FALSE)
          )
      {
      ...
      if(rrcueci_int_data.rrc_mimo_64qam_cm_enabled == TRUE)
      {
        /*
      ...
      if(rrcmcm_is_dualmode_enabled_no_f3(wrrc_as_id) && rrcueci_int_data.nv_wtog_nacc_enabled )
      {
  ...
  if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL6)
  {
    ...
    {    
      ...
      if(rrcnv_data.rrc_device_type_nv)
      {  
       ...
       WRRC_MSG4_MED("RRCEUL: optional feature EUL/HSUPA(0x%x->0x%x), rrcnv_data.wcdma_nv_hsupa_category %d default category %d",
        rrcnv_data.wcdma_nv_optional_feature_list, CHECK_WCDMA_OPTIONAL_FEATURE_HSUPA_SUPPORTED, rrcnv_data.wcdma_nv_hsupa_category, RRC_WCMDA_HSUPA_DEFAULT_CATEGORY);
      ...
      rtxMemAllocTypeZ(&enc_ctxt, rrc_PhysicalChannelCapability_edch_r6_supported );
        ...
        else if(rrccouec.rrc_ue_cap_during_rrc_conn.is_valid_mask & HSUPA_CAT_INCL)
        {
                ue_capability_container->ue_RadioAccessCapability_v690ext.
                  physicalchannelcapability_edch.fdd_edch.u.supported->
                    edch_PhysicalLayerCategory = 
                    rrccouec.rrc_ue_cap_during_rrc_conn.hsupa_category;
        }
        else
#endif
        {
          ue_capability_container->ue_RadioAccessCapability_v690ext.
            physicalchannelcapability_edch.fdd_edch.u.supported->
              edch_PhysicalLayerCategory = MIN(rrcnv_data.wcdma_nv_hsupa_category,RRC_WCMDA_HSUPA_DEFAULT_CATEGORY);
        }
      ...
      if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL7)
      {
        ...
        rrcueci_append_ue_radio_access_capability_rel7_capability(wrrc_as_id, ue_capability_container,
                                                                 RRCUECI_CAP_UPDATE
                                                                 );
      }
      ...
      if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL8)
      {
        ...
        rrcueci_append_ue_radio_access_capability_rel8_capability(wrrc_as_id,ue_capability_container);
#ifdef FEATURE_WCDMA_TO_LTE
        /*Here populate the EUTRA capability octer string in OTA*/
        if(rrcmcm_is_lte_mode_enabled_wo_f3(wrrc_as_id) &&
            rrcueci_append_lte_specific_capability(wrrc_as_id,&ue_capability_container->v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.
...
if(rrcueci_int_data.support_prio_based_resel)
{
        ...
        WRRC_MSG0_MED("ABSOL_PRI: Set support for intra UTRA priority reselection IE");
}
#endif
#ifdef FEATURE_WCDMA_MULTI_FREQ_BAND_INDICATOR 
#ifdef FEATURE_MSIM_L_PLUS_L
        if((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL9) && (rrcnv_data.rrc_mfbi_support_nv || lte_rrc_cap_mfbi_is_supported(rrc_get_as_id(wrrc_as_id), FALSE) || lte_rrc_cap_mfbi_is_supported(rrc_get_as_id(wrrc_as_id), TRUE)))
#else
        if((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL9) && (rrcnv_data.rrc_mfbi_support_nv || lte_rrc_cap_mfbi_is_supported(FALSE) || lte_rrc_cap_mfbi_is_supported(TRUE)))
#endif
        {
          ...
          if(rrcnv_data.rrc_mfbi_support_nv)
          {
            rce_v860_ext->v880NonCriticalExtensions.v890NonCriticalExtensions.v920NonCriticalExtensions.v970NonCriticalExtensions.
              va40NonCriticalExtensions.va60NonCriticalExtensions.va80NonCriticalExtensions.laterNonCriticalExtensions.
            ...
            WRRC_MSG0_MED("set the support of MFBI IE");
          }

#ifdef FEATURE_WCDMA_TO_LTE
#ifdef FEATURE_MSIM_L_PLUS_L
          if(lte_rrc_cap_mfbi_is_supported(rrc_get_as_id(wrrc_as_id), FALSE) || lte_rrc_cap_mfbi_is_supported(rrc_get_as_id(wrrc_as_id), TRUE))
#else
      ...
      if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL9)
      {
        ...
        if(rrcnv_data.rrc_meas_id_extn_support)
        {
...
#ifdef FEATURE_WCDMA_DB_DC_HSDPA
      if ((rrcueci_nv_supports_dc_hsdpa(wrrc_as_id) == TRUE) && (rrcueci_nv_supports_db_dc_hsdpa(wrrc_as_id) == TRUE))
      {
        rrc_update_db_dc_band_supported(wrrc_as_id);
        /*Call RF API to know number of band combinations supported and the band combination list*/
        if (rrcueci_ext_data.db_dc_band_comb_list.num_db_dc_band_comb_supp)      
        {
          if(!RRC_CHECK_MSG_TYPE_IE (ue_capability_container->v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
           v790NonCriticalExtensions.v860NonCriticalExtensions,v880NonCriticalExtensionsPresent))
          {
          ...
          temp_band_comb_list_ptr->n=rrcueci_ext_data.db_dc_band_comb_list.num_db_dc_band_comb_supp;
          ALLOC_ASN1ARRAY2(&enc_ctxt,temp_band_comb_list_ptr,rrc_BandComb);
      
          for (count=0; count < temp_band_comb_list_ptr->n; count++)
          {
            temp_band_comb_list_ptr->elem[count]=rrcueci_ext_data.db_dc_band_comb_list.db_dc_band_comb_list[count];
            WRRC_MSG1_MED("DB_DC_HSDPA: Add bit mask to support band comb %d ",temp_band_comb_list_ptr->elem[count]);
          }
        }
      }
     ...
     if((rrcueci_nv_supports_3c_hsdpa(wrrc_as_id) == TRUE) && (!rrccsp_check_if_em_plmn(wrrc_as_id)))
      ...
      /*
              ...
              */
  #ifdef FEATURE_WCDMA_DB_DC_HSDPA
        if((rrcueci_nv_supports_db_dc_hsdpa(wrrc_as_id) && rrcueci_nv_supports_dc_hsdpa(wrrc_as_id)) && 
           (rrcueci_ext_data.db_dc_band_comb_list.num_db_dc_band_comb_supp))
        {
          if(!(RRC_CHECK_MSG_TYPE_IE (ue_capability_container->v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.
             v790NonCriticalExtensions.v860NonCriticalExtensions.v880NonCriticalExtensions.v890NonCriticalExtensions.v920NonCriticalExtensions.v970NonCriticalExtensions.
             va40NonCriticalExtensions.ue_RadioAccessCapability_va40ext,ue_RadioAccessCapabBandCombList_va40extPresent)))
          {
            ...
            radio_access_capability->ue_RadioAccessCapabBandCombList_va40ext.n = rrcueci_ext_data.db_dc_band_comb_list.num_db_dc_band_comb_supp;
            ...
            for(count=0;count<temp_ue_RadioAccessCapabBandCombList_va40ext->n;count++)
            {
              ...
              if( rrcueci_ext_data.db_dc_band_comb_list.db_dc_band_comb_list[count] == RRC_DB_DC_BAND1_BAND8)
              {
               temp_ue_RadioAccessCapabBandCombList_va40ext->elem[count].carrierCombination21=TRUE;
             }
             else if( rrcueci_ext_data.db_dc_band_comb_list.db_dc_band_comb_list[count] == RRC_DB_DC_BAND2_BAND4)
             {
               temp_ue_RadioAccessCapabBandCombList_va40ext->elem[count].carrierCombination12=TRUE;
               temp_ue_RadioAccessCapabBandCombList_va40ext->elem[count].carrierCombination21=TRUE;
             }
             else if( rrcueci_ext_data.db_dc_band_comb_list.db_dc_band_comb_list[count] == RRC_DB_DC_BAND1_BAND5)
             {
               temp_ue_RadioAccessCapabBandCombList_va40ext->elem[count].carrierCombination12=TRUE;
                temp_ue_RadioAccessCapabBandCombList_va40ext->elem[count].carrierCombination21=TRUE;
              }
      ...
      WRRC_MSG0_MED("sib11bis: Add bit mask supportForSIB11bis to indicate SIB11bis support in ue_capability_container");
...
)
{
  ...
  classmark_params.gsm_supported = rrcmcm_is_dualmode_enabled_no_f3(wrrc_as_id);
  classmark_params.wcdma_supported = TRUE;
  classmark_params.wcdma_in_use = TRUE;
  classmark_params.MSCR99 = TRUE;
  classmark_params.as_id = rrc_get_as_id(wrrc_as_id);
#endif
  ...
  gs_read_coded_classmark_2(&classmark2,
    rrcmcm_is_dualmode_enabled_no_f3(wrrc_as_id), TRUE, TRUE, TRUE);
#endif
...
{  
  ...
  if(ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
    ...
    if (msg_ptr == NULL)
    {
       WRRC_ERR0_FATAL("Memory alloc failed for UE Cap Info Msg");
    }
    ...
    if(rrcueci_int_data.rrcueci_ue_cap_enquiry_trans_id < 4)
    { 
      ...
      msg_ptr->message.u.ueCapabilityInformation.rrc_TransactionIdentifier = 
        rrcueci_int_data.rrcueci_ue_cap_enquiry_trans_id;
    }
    ...
    {
      uint8 count;
      for(count = 0; (count < rrcdata_misc_info.rrc_capability_update_requirement.system_specific_cap_update_req_list.count) 
       ...
       )
    {
      ...
      WRRC_MSG2_HIGH("UECI FDD2 Status:%d, NV Release:%d",
        ...
        )
      {
...
#if defined (FEATURE_CGPS_UMTS_CP_WCDMA)
        if((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5) && (rrcueci_ext_data.rrc_ue_pos_cap_pch))
        {
        ...
        if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5)
        {
          ...
          {
            /* Set accessStratumReleaseIndicator to be Release 10. */
            msg_ptr->message.u.ueCapabilityInformation.v370NonCriticalExtensions.
              v380NonCriticalExtensions.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
              v4b0NonCriticalExtensions.ueCapabilityInformation_v4b0ext.
            ue_RadioAccessCapability_v4b0ext.accessStratumReleaseIndicator = rrc_AccessStratumReleaseIndicator_rel_10;
          }
          else
         ...
         /* Appends the Radio Access Capability to the RRC CONNECTION SETUP
                COMPLETE message */
          rrcueci_append_ue_radio_access_capability_later_than_r3(wrrc_as_id,&msg_ptr->message.u.ueCapabilityInformation.
    ...
    if (rrcmcm_is_dualmode_enabled_no_f3(wrrc_as_id))
    {
      if (rrcdata_misc_info.rrc_capability_update_requirement.system_specific_cap_update_req_present == TRUE) 
...
)
{
  ...
  /* Increment the counter V304 */
   rrcdata_timers_counters_info.rrc_v_304++;

   Minimum_T304 = rrc_get_minimum_t304_reqd(wrrc_as_id,rrcueci_ext_data.ueci_message_length );
   /*Start T304 as Max of the min calculated value and network configured value*/
   T304_optimum = MAX((uint32)rrcueci_convert_t304_to_ms(wrrc_as_id, RRC_GET_T304() ), Minimum_T304);
   
   /* Start the Timer T304 */
   rrctmr_start_timer(wrrc_as_id,RRCTMR_T_304_TIMER, T304_optimum );
...
)
{
  ...
  rrc_state = rrc_get_state(wrrc_as_id);          /* initialize RRC state       */

  if(cmd_ptr->cmd_hdr.cmd_id == RRC_MODE_STATUS_IND)
  {
    if(rrcueci_int_data.rrcueci_ue_cap_enquiry_trans_id != RRCUECI_INVALID_TRANSACTION_ID)
    {
       ...
       */
      rrcueci_int_data.rrcueci_ue_cap_enquiry_trans_id = (rrcueci_int_data.rrcueci_ue_cap_enquiry_trans_id + 1) & 0x3;
    }
    else
    {
      /* UECI initiated due to UE Capability changed
       ...
       */
      rrcueci_int_data.rrcueci_ue_cap_enquiry_trans_id = RRCUECI_INVALID_TRANSACTION_ID;
    }

    WRRC_MSG1_HIGH("DUAL:TransId used during mode change is - %d", rrcueci_int_data.rrcueci_ue_cap_enquiry_trans_id);

    /* As the bands would have changed moving from W to W+G.
      Band validation would have been done by CSP */
    memset(&rrcdata_misc_info.rrc_capability_update_requirement, 0, sizeof(rrc_capability_update_requirement_type));
    rrcdata_misc_info.rrc_capability_update_requirement.ue_radio_capability_update_requirement = TRUE;
    rrcdata_misc_info.rrc_capability_update_requirement.system_specific_cap_update_req_list.count = 1;
    rrcdata_misc_info.rrc_capability_update_requirement.system_specific_cap_update_req_list.value[0] = RRC_GSM_CAP;
    rrcdata_misc_info.rrc_capability_update_requirement.system_specific_cap_update_req_present = TRUE;
  }
  else
  {

    /* Verify the UE Capability Enquiry Transaction Id */
    if (rrcueci_int_data.rrcueci_ue_cap_enquiry_trans_id == 
  ...
  {
    rrcueci_int_data.rrcueci_ue_cap_originator = RRC_UE_CAPABILITY_ORIGINATOR_COUEC;
  }
#endif
  switch(rrc_get_state(wrrc_as_id))
  {
    ...
    case RRC_STATE_CELL_PCH:
      ...
      {
        WRRC_ERR1_FATAL("Unable to allocate memmory for: %d cmd", 
                   RRC_CELL_UPDATE_INITIATE_REQ);
      }
      rrcueci_int_data.rrcueci_substate = RRCUECI_WAIT_FOR_CELL_UPDATE_COMP_CNF;
      break;

    case RRC_STATE_CELL_FACH:

      if(RRCCU_NOT_COMPLETED == rrccu_get_cell_update_complete_status(wrrc_as_id,RRC_PROCEDURE_UECI) )
      {
      ...
      else /* Cell Update procedure is inactive */
      {
        ...
        if(RRCUECI_SUCCESS != rrcueci_send_ue_capability_info_msg(wrrc_as_id) )
        {
           WRRC_MSG2_HIGH("Failed to send UE Cap Info Msg: %d, %d", rrc_state, rrcueci_int_data.rrcueci_substate);
        }
        else
        {
          /* Increment the counter V304 and start T304*/
          rrcueci_increment_v304_start_t304(wrrc_as_id);

          rrcueci_int_data.rrcueci_substate = RRCUECI_WAIT_FOR_T304_TO_EXPIRE;
        }
      }
      break;
    ...
    default:
      WRRC_MSG2_HIGH("Invalid Event: %d in the UECI Substate: %d", 
                cmd_ptr->cmd_hdr.cmd_id, rrcueci_int_data.rrcueci_substate);
      break;
  }
...
)
{
  ...
  if(cmd_ptr->cmd_hdr.cmd_id == RRC_MODE_STATUS_IND)
  {
    WRRC_MSG2_HIGH("DUAL:%d(0: UE, 1: UTRAN) initiated UECI procedure in substate : %d(1:WaitCUCnf, 2:WaitT304Exp",
                                         RRC_UE_CAPABILITY_ORIGINATOR_UE, rrcueci_int_data.rrcueci_substate);
  }
  else    
  {
    WRRC_MSG2_HIGH("%d(0: UE, 1: UTRAN) initiated UECI procedure in substate : %d(1:WaitCUCnf, 2:WaitT304Exp",
                    cmd_ptr->cmd.initiate_ue_cap.originator, rrcueci_int_data.rrcueci_substate);
  }
  ...
  */
     ...
     )
  {
    ...
    */
    rrcueci_reset_ueci_procedure(wrrc_as_id);
    rrcueci_process_ueci_initial_substate(wrrc_as_id, cmd_ptr);
  }
  else
  {
...
)
{
  ...
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_T304_EXPIRED_IND:
      ...
      if(rrcdata_timers_counters_info.rrc_v_304 <= RRC_GET_N304())               
      {        
      ...
      else /* Radio Link Failure */
      {
        ...
        {  
          ...
          rrctmr_start_timer(wrrc_as_id, RRCTMR_T_304_TIMER,RRCTMR_T304_EXPIRED_N304TH_TIME_IN_MS);
          rrcueci_int_data.rrcueci_substate = RRCUECI_WAIT_FOR_T304_TO_EXPIRE;
          break;
        }
#endif
        ...
        {
          WRRC_ERR0_FATAL("Memory alloc failure for CELL UPDATE");
        }
      }
      break;
          ...
          else /* ASN1 decoding is successful */
          {
               ...
               T_rrc_UECapabilityInformationConfirm_later_than_r3 )
            {
              ...
              if ((dcch_msg_ptr->message.u.ueCapabilityInformationConfirm.
                  ...
                  rrc_TransactionIdentifier ==  rrcueci_int_data.rrcueci_ue_cap_enquiry_trans_id)||
                  (rrcueci_int_data.rrcueci_ue_cap_enquiry_trans_id == RRCUECI_INVALID_TRANSACTION_ID)
                  )
              {
                ...
                acceptanceOfChangeOfCapabilityPresent))
                {
                  ...
                  {
                    rrccouec_process_completion_of_ueci_req(wrrc_as_id, CHANGE_OF_CAP_REFUSED);
                  }
                  ...
                  {
                    rrccouec_process_completion_of_ueci_req(wrrc_as_id, CHANGE_OF_CAP_ACCEPTED);
                  }
                  ...
                  {
                    rrccouec_process_completion_of_ueci_req(wrrc_as_id, CHANGE_OF_CAP_WITH_RECONFIG);
                  }
                  else
                  {
                    rrccouec_process_completion_of_ueci_req(wrrc_as_id, CHANGE_OF_CAP_ACCEPTED);
              ...
              else /* Invalid message is received */
              {
                ...
                else /* Valid RLC Id */
                {
                  ...
                  else
                  {
                    ...
                    /* Start the Timer T304 */
                    rrctmr_start_timer(wrrc_as_id,RRCTMR_T_304_TIMER,
                     (uint32)rrcueci_convert_t304_to_ms(wrrc_as_id, RRC_GET_T304() ));
                  }
                } /* Valid RLC Id */
    ...
    */
    if(cmd_ptr->cmd.state_change_ind.new_state == RRC_STATE_DISCONNECTED)
    {
      WRRC_MSG1_HIGH("Reset UECI procedure as RRC state is Disconneted. Cur UECI state: %d",
                                                         rrcueci_int_data.rrcueci_substate);
      /* Reset the UECI state machine */
      rrcueci_reset_ueci_procedure(wrrc_as_id);
...
{
  switch (rrcueci_int_data.rrcueci_substate)
  {
    ...
    case RRCUECI_WAIT_FOR_CELL_UPDATE_COMP_CNF:
      ...
      {
        case RRC_CELL_UPDATE_COMPLETE_CNF:
          rrcueci_process_ueci_wait_for_cell_update_comp_cnf(wrrc_as_id, cmd_ptr);
          break;
        
        case RRC_MODE_STATUS_IND:
        case RRC_INITIATE_UE_CAPABILITY_REQ:
          rrcueci_process_ueci_in_non_initial_substate(wrrc_as_id, cmd_ptr);
          break;
    ...
    case RRCUECI_WAIT_FOR_T304_TO_EXPIRE: 
      ...
      {
        case RRC_T304_EXPIRED_IND:
          rrcueci_process_ueci_wait_for_t304_to_expire(wrrc_as_id, cmd_ptr);
          break;
...
)
{
  ...
  WRRC_MSG2_HIGH_OPT("function rrcueci_procedure_event_handler is called in rrc_state rrc_state_e_type_value%d in rrcueci_substate rrcueci_substate_e_type_value%d", rrc_state,rrcueci_int_data.rrcueci_substate);

  /* if RRC Connection release procedure is active then ignore the command,
      also reset UECI procedure variables.
  */
  rrc_crash_debug_log_proc_substates(wrrc_as_id,RRC_PROCEDURE_UECI,rrcueci_int_data.rrcueci_substate,cmd_ptr);
#ifdef FEATURE_WCDMA_TO_LTE
     ...
     (cmd_ptr->cmd.wcdma_rrc_lte_irat_cmd.cmd.msg_hdr.id == LTE_RRC_EUTRA_CAPABILITIES_RSP))
  {
    ...
    {
      rrcueci_save_eutra_capability(wrrc_as_id,&cmd_ptr->cmd.wcdma_rrc_lte_irat_cmd.cmd.lte_rrc_eutra_capabilites_rsp);
    }
    else
    {
  ...
  if (FALSE == rrcrcr_is_proc_active(wrrc_as_id))
  {
    switch (rrc_state)
    {
      case RRC_STATE_DISCONNECTED: 
        rrcueci_process_disconnected_state(wrrc_as_id, cmd_ptr);
        break;
  
      case RRC_STATE_CELL_FACH:    
        rrcueci_process_cell_fach_state(wrrc_as_id,cmd_ptr); 
        break;
  
      case RRC_STATE_CELL_DCH:  
        rrcueci_process_cell_dch_state(wrrc_as_id,cmd_ptr); 
        break;
  
      case RRC_STATE_CELL_PCH:
      case RRC_STATE_URA_PCH:
        rrcueci_process_cell_ura_pch_state(wrrc_as_id, cmd_ptr);
        break;
...
void rrcueci_init_procedure(sys_modem_as_id_e_type wrrc_as_id)
{
 ...
 /*set support for priority based resel to TRUE by default*/
  rrcueci_int_data.support_prio_based_resel = TRUE;
  ...
  rrcscmgr_register_for_scn(wrrc_as_id,RRC_PROCEDURE_UECI,
                            RRC_STATE_WILDCARD,
                            RRC_STATE_DISCONNECTED);
#ifdef FEATURE_WCDMA_TO_LTE
  rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_req_posted = rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_rsp_received = FALSE;
#endif
}
...
)
{
    ...
    rrcnv_data.nv_hsdpa_category = WNV_GET_DEFAULT(WNV_ID_HSDPA_CAT_I);
  ...
  wnv_get_nv_msim(WNV_ID_HSDPA_CAT_I, &rrcnv_data.nv_hsdpa_category, sizeof(rrcnv_data.nv_hsdpa_category), wrrc_as_id);

  if ((rrcnv_data.nv_hsdpa_category == 8) || (rrcnv_data.nv_hsdpa_category == 6) || (rrcnv_data.nv_hsdpa_category == 12)
#ifdef FEATURE_HSPA_PLUS_CAT24
      ||(rrcnv_data.nv_hsdpa_category == 24)
#endif /*FEATURE_HSPA_PLUS_CAT24*/
#ifdef FEATURE_HSPA_PLUS_CAT29
      ||(rrcnv_data.nv_hsdpa_category == 29)
#endif /*FEATURE_HSPA_PLUS_CAT24*/
#ifdef FEATURE_HSPA_PLUS_CAT9
      ||(rrcnv_data.nv_hsdpa_category == 9)
#endif
#ifdef FEATURE_HSPA_PLUS_CAT10
      || (rrcnv_data.nv_hsdpa_category == 10)
#endif
    || (rrcnv_data.nv_hsdpa_category > 8)
  )
  {
    WRRC_MSG1_HIGH("HSDPACATEGORY %d", rrcnv_data.nv_hsdpa_category);
  }
  else
  {

    rrcnv_data.nv_hsdpa_category = WNV_GET_DEFAULT(WNV_ID_HSDPA_CAT_I);
    WRRC_MSG1_HIGH("HSDPACATEGORY FORCIBLY %d", rrcnv_data.nv_hsdpa_category);
  }
#ifdef FEATURE_HSPA_PLUS_CAT10
  if (rrcnv_data.nv_hsdpa_category > 12)
  {
  ...
  {
    rrcnv_data.nv_hsdpa_category = RRC_DEFAULT_3C_HSDPA_CATEGORY;
    WRRC_MSG1_HIGH("HSDPACATEGORY FORCIBLY %d", rrcnv_data.nv_hsdpa_category);
  }
#endif
  ...
  if((rrcnv_data.nv_hsdpa_category > 14)&&(mcs_modem_has_capability(MCS_MODEM_CAPABILITY_FEATURE_HSPA_ABOVE_CAT14) == MCS_MODEM_CAP_UNAVAILABLE))
  {
    WRRC_MSG1_HIGH("HSDPACATEGORY %d is unsupported hence FORCIBLY set to 14", rrcnv_data.nv_hsdpa_category);
    rrcnv_data.nv_hsdpa_category= 14;
  }
#ifdef FEATURE_WCDMA_3C_HSDPA
  if(rrcnv_data.nv_hsdpa_category >= 29)
  {
...
#endif
#ifdef FEATURE_WCDMA_DC_HSDPA
  if(rrcnv_data.nv_hsdpa_category > 20) 
  {
    rrcnv_data.nv_hsdpa_category = 24;
  }
#endif
...
void rrcueci_fill_pos_cap_r7(sys_modem_as_id_e_type wrrc_as_id,rrc_UE_RadioAccessCapability_v770ext_IEs * rel7_ue_capablity)
{
  uint8 i = 0;
 ...
 if(pos_capability.ganss_supported == TRUE)
 {
   ...
   if(pos_capability.ganss_supported_list_incl)
   {
     if(pos_capability.list_cnt)
     {
       ...
       {
         WRRC_ERR0_FATAL("WTOL: Memory allocation failure");
       }

       for(i= 0 ; i< ganss_list_ptr->n ; i++)
       {
         WRRC_MSG3_HIGH("GANSS : GANSS ID INCL is %d GANSS sig id incl: %d GANSS mode : %d",pos_capability.ganss_supported_list[i].ganss_id_incl,pos_capability.ganss_supported_list[i].ganss_sig_id_incl,pos_capability.ganss_supported_list[i].ganss_mode);
 ...
 )
{
  ...
  if(rel7_ue_capablity != NULL )
  {
    ...
    if(update_cause == RRCUECI_CAP_UPDATE)
    {
          ...
          (rrccouec.ue_cap_change_requested.is_valid_mask & HSDPA_CAT_INCL) &&(rrccouec.ue_cap_change_requested.hsdpa_category >=8))
      {
        if(rrccouec.ue_cap_change_requested.hsdpa_category > rrc_macehs_cat)
        {
          ...
          rel7_ue_capablity->physicalChannelCapability.fddPhysChCapability.downlinkPhysChCapability.hsdsch_physical_layer_category_ext = 
            rrc_macehs_cat;
          WRRC_MSG1_MED("REL8:Using HSDPA CAT %d",rrc_macehs_cat);
        }
        else
        {
          RRC_MSG_COMMON_SET_BITMASK_IE(rel7_ue_capablity->physicalChannelCapability.fddPhysChCapability.downlinkPhysChCapability,
      ...
      else if((rrccouec.rrc_ue_cap_during_rrc_conn.is_valid_mask & HSDPA_CAT_INCL)&& (rrccouec.rrc_ue_cap_during_rrc_conn.hsdpa_category >=8))
      {
        if(rrccouec.rrc_ue_cap_during_rrc_conn.hsdpa_category > rrc_macehs_cat)
        {
          RRC_MSG_COMMON_SET_BITMASK_IE(rel7_ue_capablity->physicalChannelCapability.fddPhysChCapability.downlinkPhysChCapability, 
            rrc_DL_PhysChCapabilityFDD_v770ext,hsdsch_physical_layer_category_ext);
          rel7_ue_capablity->physicalChannelCapability.fddPhysChCapability.downlinkPhysChCapability.hsdsch_physical_layer_category_ext = 
      ...
      if(rrcnv_data.nv_hsdpa_category > rrc_macehs_cat)
      {
        ...
        rel7_ue_capablity->physicalChannelCapability.fddPhysChCapability.downlinkPhysChCapability.hsdsch_physical_layer_category_ext = 
            rrc_macehs_cat;
        WRRC_MSG1_MED("REL8:Using HSDPA CAT %d for R7 UTRANs",rrc_macehs_cat);
      }
      else
      if(rrcnv_data.nv_hsdpa_category >= 8 )
      {
     ...
     */
      ...
      {
  #ifdef FEATURE_WCDMA_MIMO
        if(rrcnv_data.nv_hsdpa_category < 15 )
        {
          WRRC_MSG1_MED("RRCHSPA+: MIMO feature defined but NV returns %d , should be 15 or above ",rrcnv_data.nv_hsdpa_category);
        }
  #endif       
      }
      else
      { 
        WRRC_MSG1_MED("RRCHSPA+: 64QAM feature defined but NV returns %d , should be 13 or 14",rrcnv_data.nv_hsdpa_category);
      }
      ...
      if(rrcnv_data.rrc_ganss_support_nv == TRUE)
      {
        rrcueci_fill_pos_cap_r7(wrrc_as_id,rel7_ue_capablity);
      }
      else
      {
        WRRC_MSG0_MED("GANSS NV not set");
      }
#endif
      ...
      if(rrcueci_nv_supports_cpc_dtx_on_dpcch(wrrc_as_id))
      {
        ...
        WRRC_MSG0_MED("RRCCPC:Indicate UE support CPC-DTX operation on UL DPCCH & UE support Slot Format #4 ");
        RRC_MSG_COMMON_SET_BITMASK_IE(rel7_ue_capablity->physicalChannelCapability.fddPhysChCapability.uplinkPhysChCapability,
...
boolean rrcueci_check_fe_fach_concurrency(sys_modem_as_id_e_type wrrc_as_id)
{
#ifdef FEATURE_DUAL_SIM
  uint16 nv_value = rrc_get_concurrency_nv(wrrc_as_id);
#endif

  if(rrcnv_data.rrc_fe_fach_support_nv == FE_FACH_NOT_SUPPORTED)
  {
    return FALSE;
  }
#ifdef FEATURE_DUAL_SIM
  ...
  {
    WRRC_MSG1_HIGH("FE_FACH : FE_FACH NV is disabled for device_mode %d", rrc_dsds_int_data.wcdma_device_mode);
    return FALSE;
  }
#endif
...
)
{
  ...
  WRRC_MSG0_HIGH("Populating REL8 radio access capability");
  ...
  {
    WRRC_MSG0_MED("REL8:HSDPA Category isn't set from 21 to 24.");
  }
  ...
  if(rrcmcm_is_lte_mode_enabled_wo_f3(wrrc_as_id))
  {
    ...
    if(lte_rrc_lte_fdd_is_supported(rrccsp_get_lte_band_pref(wrrc_as_id)))
    {
      ...
      if(rrcnv_data.wcdma_rrc_wtol_ps_ho_support_nv) 
      {
...
#ifdef FEATURE_WTOL_PS_HANDOVER
      if(rrcnv_data.wcdma_rrc_wtol_tdd_ps_ho_support_nv) 
      {
    ...
    if(rrcnv_data.rrc_wtol_cm_support_nv == TRUE)
    {
    ...
    #ifdef FEATURE_WCDMA_ABSOLUTE_PRIORITY_FACH
    if(rrcueci_check_fe_fach_concurrency(wrrc_as_id) == TRUE)
    {
      if(rrcnv_data.rrc_fe_fach_support_nv == FE_FACH_HIGH_PRIORITY_LAYERS)
      {
        fgi |= 0x02;
      }
      else if(rrcnv_data.rrc_fe_fach_support_nv == FE_FACH_ALL_LAYERS )
      { 
        fgi |= 0x01;
      }
    ...
    if(rrcueci_append_ue_radio_access_capability_fdd3_list
        (wrrc_as_id,&v860_radioaccesscapability_ptr->ue_RadioAccessCapabBandFDDList3,rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_band_pref))
    {
  ...
  if(rrcenhstate_nv_supports_e_fach_drx(wrrc_as_id))
  {
...
)
{
  ...
  if(rrcnv_data.rrc_disable_cpc_fdpch_nv & (NV_CPC_DTX_DISABLE|NV_FDPCH_DISABLE))
  {
    return FALSE;
  }
...
)
{
  ...
  /*
     Query L1 to check if DC config failed previously due to NW configuration issue
     This API is used as workaround to temporarily disable DC to avoid continuos call failures
  */
  if((l1_is_ue_dc_capable(wrrc_as_id) == FALSE)
     &&
     (l1_get_remaining_time_for_backoff_dc_cap(wrrc_as_id) != 0))
  {
    WRRC_MSG0_HIGH("L1 doesn't support DC due to previous failure");
    return FALSE;
  }
  return TRUE;
}
...
)
{
  ...
  if(rrcueci_nv_supports_mac_i(wrrc_as_id) == FALSE)
  {
    return FALSE;
  }
  if(rrcueci_nv_supports_dc_hsdpa(wrrc_as_id) == FALSE)
  {
    return FALSE;
  }
#ifdef FEATURE_DUAL_SIM
...
#endif
/*Disabling DC_HSUPA feature by default HI onwards*/
  return FALSE;
}
...
)
{
  ...
  if(!((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL9) && 
       (rrcnv_data.rrc_feature_nv_item_value & RRC_DB_DC_HSDPA_ENABLED)))
  {
    return FALSE;
  }
...
#endif
/*Disabling DB DC feature by default HI onwards*/
  return FALSE;
}
...
{
   if (rrcnv_data.rrc_feature_nv_item_value & RRC_EDPCCH_PWR_BOOST_ENABLED)
   {
      return TRUE;
   }
    return FALSE;
...
)
{
  ...
  rrcueci_int_data.nv_wtog_nacc_enabled = wtog_nacc_enabled;
  WRRC_MSG1_HIGH("WTOG NACC NV Support %d",rrcueci_int_data.nv_wtog_nacc_enabled );
}

/*===========================================================================

FUNCTION rrc_rr_absolute_priority_support
...
===========================================================================*/
boolean rrc_rr_absolute_priority_support(sys_modem_as_id_e_type wrrc_as_id)
{
#ifdef FEATURE_WCDMA_ABSOLUTE_PRIORITY
  if(rrcueci_int_data.support_prio_based_resel)
  {
...
FUNCTION rrcueci_is_truncated_ca_needed

DESCRIPTION
  This  function checks if the below NVs are configured and takes a decision on Truncated CA cap is needed or not from LTE. 
    WNV_ID_RRC_NV_REQ_LTE_FOR_TRUNCATED_CA
    WNV_ID_RRC_NV_REQ_LTE_FOR_TRUNCATED_CA_ON_MCC_LIST
  if the current camped MCC matches with any of the MCC present in NV MCC_LIST, then returns TRUE.
...
RETURN VALUE
  True if matches.
  False if doesnt match or NVs not configured.

===========================================================================*/
boolean rrcueci_is_truncated_ca_needed(sys_modem_as_id_e_type wrrc_as_id, rrc_plmn_identity_type src_rat_plmn_id /*Will be used for IHO cases*/)
{
  ...
  if((rrcnv_data.rrc_req_lte_for_truncated_ca_nv == FALSE) ||
     (rrcnv_data.rrc_req_lte_for_truncated_ca_on_mcc_list_nv.count > MCC_LIST_SIZE))
  {
    WRRC_MSG0_HIGH("NV_REQ_LTE_FOR_TRUNCATED_CA is not enabled (or) MCC list NV has invalid data.");
    return FALSE;
  }
  if((rrcnv_data.rrc_req_lte_for_truncated_ca_nv == TRUE) && 
     (rrcnv_data.rrc_req_lte_for_truncated_ca_on_mcc_list_nv.count == 0))
  {
    WRRC_MSG0_HIGH("TRUNCATED_CA is enabled for all PLMNs");
    return TRUE;
  }
  for(j = 0;j < rrcnv_data.rrc_req_lte_for_truncated_ca_on_mcc_list_nv.count; j++)      
  {
    WRRC_MSG2_HIGH("mmc_list[%d]:%d",j,rrcnv_data.rrc_req_lte_for_truncated_ca_on_mcc_list_nv.mcc_list[j]);
  }

  if(rrciho_is_gtow_ho_active(wrrc_as_id))  /*For IHO cases*/
  {
    plmn = (uint16)(src_rat_plmn_id.mcc[0]*100 + src_rat_plmn_id.mcc[1]*10 + src_rat_plmn_id.mcc[2]);
    WRRC_MSG1_HIGH("Current camped mcc from source RAT: %d",plmn);
    for(j = 0;j < rrcnv_data.rrc_req_lte_for_truncated_ca_on_mcc_list_nv.count; j++)		
    {
      if(rrcnv_data.rrc_req_lte_for_truncated_ca_on_mcc_list_nv.mcc_list[j] == plmn)
      {
        WRRC_MSG0_HIGH("Current camped mcc matched with the entries in MCC_LIST NV");
        return TRUE;
      }
    }
    return FALSE;
  ...
  mib_ptr = (rrc_MasterInformationBlock *)rrc_sibdb_return_sib_for_srv_cell(wrrc_as_id,rrc_MIB);  
  if(mib_ptr == NULL)
  {
    return FALSE;
  }
  if(rrc_is_nw_shared(wrrc_as_id,mib_ptr))
  {
    ...
    for(i = 0;i < num_of_shared_plmns; i++)
    {
      plmn = shared_plmn_list[i].mcc[0]*100 + shared_plmn_list[i].mcc[1]*10 + shared_plmn_list[i].mcc[2];
      for(j = 0;j < rrcnv_data.rrc_req_lte_for_truncated_ca_on_mcc_list_nv.count; j++)      
      {
        if(rrcnv_data.rrc_req_lte_for_truncated_ca_on_mcc_list_nv.mcc_list[j] == plmn)
        {
  ...
  else
  {
    ...
    {
      rrc_PLMN_Identity mib_plmn = mib_ptr->plmn_Type.u.gsm_MAP->plmn_Identity;
      plmn = mib_plmn.mcc.elem[0] *100 + mib_plmn.mcc.elem[1] *10 + mib_plmn.mcc.elem[2];
      for(j = 0;j < rrcnv_data.rrc_req_lte_for_truncated_ca_on_mcc_list_nv.count; j++)		
      {
        if(rrcnv_data.rrc_req_lte_for_truncated_ca_on_mcc_list_nv.mcc_list[j] == plmn)
        {
...
)
{
  ...
  if ((!rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_rsp_received) && rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_req_posted) 
  {
    ...
    if (count >= 3)
    {
      WRRC_ERR1_FATAL("WTOL: Didnt receive capability response from LTE for %d time",count);
    }
    else
    {
  ...
  eutra_cap_req.rat_capabilities.lte_band_preference = rrccsp_get_lte_band_pref(wrrc_as_id);


  rrccsp_get_wg_band_pref(wrrc_as_id,&eutra_cap_req.rat_capabilities.wcdma_band_preference,
  ...
  if((RRC_CAMPED_ON == rrc_ccm_get_curr_camping_status(wrrc_as_id, RRC_PROCEDURE_CSP)) && (RRC_MODE_ACTIVE == rrcmcm_get_rrc_mode(wrrc_as_id)))
  {
    eutra_cap_req.camped_plmn = mmumts_convert_rrc_plmn_id_to_nas_plmn_id(rrc_ccm_get_curr_camped_plmn(wrrc_as_id));
  }
  else
  {
  ...
  if(truncated_ca)
  {
    eutra_cap_req.lte_rrc_ca_req_type = LTE_RRC_CA_CAP_REQ_TYPE_TRUNCATED_CA; //Truncated CA combinations
    rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_truncated_cap_requested = TRUE;
  }
  else
  {
    eutra_cap_req.lte_rrc_ca_req_type = LTE_RRC_CA_CAP_REQ_TYPE_FULL_CA;
    rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_truncated_cap_requested = FALSE;
  }

  /*LTE_RRC_EUTRA_CAPABILITIES_REQ*/
  rrc_send_lte_cmd(wrrc_as_id,&eutra_cap_req.msg_hdr,LTE_RRC_EUTRA_CAPABILITIES_REQ,sizeof(lte_rrc_eutra_capabilities_req_s));
  WRRC_MSG1_HIGH("WTOL: LTE_RRC_EUTRA_CAPABILITIES_REQ sent to LTE RRC, truncated_cap_req:%d", rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_truncated_cap_requested);

  rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_req_posted = TRUE;
  rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_rsp_received = FALSE;
...
#ifdef LTE_BAND_NUM
  memset(&rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_band_pref,0,sizeof(sys_lte_band_mask_e_type));
#else
rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_band_pref = 0;
#endif
}

/*===========================================================================

FUNCTION RRCUECI_SEND_EUTRA_CAPABILITITES_REQ_TO_LTE_NW_REQ_BANDS_AND_RCV

DESCRIPTION
  This  function sends EUTRA capabilities request to LTE with the NW requested Bands 
  that are received in RRC connection setup or UE Capability enquiry msgs and waits 
  until a rsp is received and process the same.
...
===========================================================================*/
void rrcueci_send_eutra_capabilitites_req_to_lte_nw_req_bands_and_rcv
(
  sys_modem_as_id_e_type wrrc_as_id,
  boolean truncated_ca
)
{
  lte_rrc_nw_req_eutra_capabilities_req_s eutra_cap_req;
  uint8 index =0;
  rex_sigs_type sigs;
  boolean keep_waiting = TRUE;
  wcdma_rrc_lte_irat_cmd_type *lte_cmd_ptr;
  rrc_cmd_type *rrc_cmd;
  rrc_plmn_identity_type plmn_id;

  memset(&eutra_cap_req,0,sizeof(lte_rrc_nw_req_eutra_capabilities_req_s));
  ...
  /*Fill NW requested Bands*/
  eutra_cap_req.num_requested_bands = rrcdata_misc_info.rrc_capability_update_requirement.num_requested_bands;
  for(index=0; index < eutra_cap_req.num_requested_bands; index++)
  {
    eutra_cap_req.requested_bands[index] = rrcdata_misc_info.rrc_capability_update_requirement.requested_bands[index];
  }
  if(truncated_ca)
  {
    eutra_cap_req.lte_rrc_ca_req_type = LTE_RRC_CA_CAP_REQ_TYPE_TRUNCATED_CA; //Truncated CA combinations
  ...
  rrc_send_lte_cmd(wrrc_as_id,&eutra_cap_req.msg_hdr,LTE_RRC_NW_REQ_EUTRA_CAPABILITIES_REQ,sizeof(lte_rrc_nw_req_eutra_capabilities_req_s));
  WRRC_MSG0_HIGH("WTOL: LTE_RRC_NW_REQ_EUTRA_CAPABILITIES_REQ sent to LTE RRC");
  ...
  do
  {
    rrc_cmd = rrc_malloc(sizeof(rrc_cmd_type));
    (void) rex_wait(RRC_LTE_RRC_CMD_Q_SIG);
    (void) rex_clr_sigs(rex_self(),RRC_LTE_RRC_CMD_Q_SIG);
    while( (lte_cmd_ptr = q_get(&rrc_cmd_q.rrc_lte_cmd_q)) != NULL)
    {
      rrc_cmd->cmd_hdr.cmd_id = RRC_LTE_RRC_CMD;
      MSGR_CLEAR_VARIANT(lte_cmd_ptr->cmd.msg_hdr.id);
      WCDMA_MEMCPY(&(rrc_cmd->cmd.wcdma_rrc_lte_irat_cmd),
                     sizeof(wcdma_rrc_lte_irat_cmd_type),
                     lte_cmd_ptr,
                     sizeof(wcdma_rrc_lte_irat_cmd_type));
      rrc_crash_debug_save_lte_to_rrc_cmds(wrrc_as_id,lte_cmd_ptr->cmd.msg_hdr.id);
      if(lte_cmd_ptr->cmd.msg_hdr.id == LTE_RRC_NW_REQ_EUTRA_CAPABILITIES_RSP)
      {
        if(rrc_cmd->cmd.wcdma_rrc_lte_irat_cmd.cmd.lte_rrc_eutra_capabilites_rsp.dest_rat == LTE_RRC_IRAT_UTRA_FDD)
        {
          WRRC_MSG0_HIGH("Rcvd LTE_RRC_NW_REQ_EUTRA_CAPABILITIES_RSP from LTE RRC");
          rrcueci_save_eutra_capability(wrrc_as_id,&rrc_cmd->cmd.wcdma_rrc_lte_irat_cmd.cmd.lte_rrc_nw_req_eutra_capabilities_rsp);
          keep_waiting = FALSE;
        }
        else
        {
          WRRC_MSG1_HIGH("Rcvd LTE_RRC_NW_REQ_EUTRA_CAPABILITIES_RSP for other tech: %d. Ignored",
                    rrc_cmd->cmd.wcdma_rrc_lte_irat_cmd.cmd.lte_rrc_eutra_capabilites_rsp.dest_rat);
        }
      }
      else
      {
        WRRC_MSG1_HIGH("Another LTE Message:rrc_cmd_e_type_value_0x%x, received while waiting for capability rsp. Ignored for now",lte_cmd_ptr->cmd.msg_hdr.id);
      }
      q_put(&rrc_cmd_q.rrc_lte_free_cmd_q,&(lte_cmd_ptr->link));
    }
    rrc_free(rrc_cmd);
  } while (keep_waiting);
}
...
)
{
...
#ifdef LTE_BAND_NUM
  sys_lte_band_mask_e_type lte_band_pref = rrccsp_get_lte_band_pref(wrrc_as_id);
#else
  sys_band_mask_e_type     lte_band_pref = rrccsp_get_lte_band_pref(wrrc_as_id);
#endif
  ...
  rrcueci_int_data.rrcueci_interrat_lte_capability_info.cap_cont_length = dsm_len;

  rrcueci_int_data.rrcueci_interrat_lte_capability_info.cap_cont_ptr = (byte *)rrc_malloc(rrcueci_int_data.rrcueci_interrat_lte_capability_info.cap_cont_length);

  ASSERT(dsm_len == dsm_pullup(&dsm_ptr, rrcueci_int_data.rrcueci_interrat_lte_capability_info.cap_cont_ptr, (uint16)dsm_len) );

  rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_rsp_received = TRUE;
  rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_camped_band_mask = lte_eutra_capabilities_rsp->lte_camped_band_mask;
  rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_nghbr_band_mask = lte_eutra_capabilities_rsp->lte_neigh_band_mask;

#ifdef LTE_BAND_NUM
  #if (LTE_BAND_NUM == 256)
  WRRC_MSG4_HIGH("WTOL: LTE band mask passed by RRC = 0x%x  0x%x  0x%x 0x%x", 
    lte_band_pref.bits_1_64,lte_band_pref.bits_65_128,lte_band_pref.bits_129_192,lte_band_pref.bits_193_256);
  #else
  WRRC_MSG1_HIGH("WTOL: LTE band mask passed by RRC = 0x%x", lte_band_pref);
  #endif
#else
  WRRC_MSG1_HIGH("WTOL: LTE band mask passed by RRC = 0x%x", lte_band_pref);
#endif
...
#ifdef FEATURE_MSIM_L_PLUS_L
  lte_rrc_cmapi_get_lte_cap_band_preference(rrc_get_as_id(wrrc_as_id), &lte_band_pref);
#else
  lte_rrc_cmapi_get_lte_cap_band_preference(&lte_band_pref);
#endif


#ifdef LTE_BAND_NUM
  #if (LTE_BAND_NUM == 256)
    WRRC_MSG4_HIGH("WTOL: LTE band mask from LTE = 0x%x  0x%x	0x%x 0x%x", 
        lte_band_pref.bits_1_64,lte_band_pref.bits_65_128,lte_band_pref.bits_129_192,lte_band_pref.bits_193_256);
  #else
    WRRC_MSG1_HIGH("WTOL: LTE band mask from LTE = 0x%x", lte_band_pref);
  #endif
#else
    WRRC_MSG1_HIGH("WTOL: LTE band mask from LTE = 0x%x", lte_band_pref);
#endif

  rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_band_pref = lte_band_pref;
}
...
)
{
...
#endif
#ifdef LTE_BAND_NUM
  if(rrccsp_check_if_lte_band_pref_is_set(wrrc_as_id,lte_band_pref) == FALSE)
#else
  if(lte_band_pref == 0)
#endif
  {
  ...
  rrc_fill_lte_band_mask(wrrc_as_id,&num_lte_bands, &lte_band_pref, &lte_fdd_band_pref, &lte_tdd_band_pref);
  ...
  rrccsp_get_supported_band_list(wrrc_as_id,&ueci_band);
  WRRC_MSG2_MED("WTOL: No of bands supported- fdd :%d, fdd2: %d", ueci_band.num_supported_bands, ueci_band.num_supported_bands_fdd2);

  if(0 == (ueci_band.num_supported_bands + ueci_band.num_supported_bands_fdd2))
  {
    WRRC_ERR0_FATAL("WTOL: No W bands enabled");
  }
...
/*===========================================================================
FUNCTION rrcueci_append_ue_radio_access_capability_fdd4_list
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
===========================================================================*/
sys_band_mask_type rrcueci_get_static_band_mask(sys_modem_as_id_e_type wrrc_as_id)
{
...
void rrc_fill_priority_bitmask_array(sys_modem_as_id_e_type wrrc_as_id,sys_band_mask_type *ueci_priority_bmask)
{
  sys_band_mask_type static_band_bitmask = rrcueci_get_static_band_mask(wrrc_as_id);

  /* SIB 19 band bitmask */
#ifdef LTE_BAND_NUM
  ueci_priority_bmask[2] = sys_map_lte_band_mask_to_band_mask(rrcmeas_ext_data.rrcueci_sib19_band_mask);
#else
  ueci_priority_bmask[2] = rrcmeas_ext_data.rrcueci_sib19_band_mask;
#endif
...
#ifdef LTE_BAND_NUM
  ueci_priority_bmask[0] = (sys_map_lte_band_mask_to_band_mask(rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_camped_band_mask)| ueci_priority_bmask[1] );
#else
  ueci_priority_bmask[0] = (rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_camped_band_mask | ueci_priority_bmask[1] );
#endif
}
...
===========================================================================*/
#ifdef LTE_BAND_NUM
void rrc_fill_lte_band_mask(sys_modem_as_id_e_type wrrc_as_id,uint8* num_lte_bands, sys_lte_band_mask_e_type *lte_band_pref, sys_lte_band_mask_e_type *lte_fdd_band_pref, sys_lte_band_mask_e_type *lte_tdd_band_pref)
{
  uint8 lte_band_idx;
  uint8 idx = 0;
  ...
  *num_lte_bands = num_lte_band_pref;
  ...
  rrc_fill_priority_bitmask_array(wrrc_as_id,ueci_priority_bmask);
...
#else
void rrc_fill_lte_band_mask(sys_modem_as_id_e_type wrrc_as_id,uint8* num_lte_bands, sys_band_mask_e_type *lte_band_pref, sys_band_mask_e_type *lte_fdd_band_pref, sys_band_mask_e_type *lte_tdd_band_pref)
{
  uint8 lte_band_idx;
  uint8 idx = 0;
  uint8 num_lte_band_pref = 0;
...
/*===========================================================================
FUNCTION rrc_send_lte_rrc_wcdma_eutra_cap_sent_ind

DESCRIPTION
  This function sends LTE_RRC_WCDMA_EUTRA_CAP_SENT_IND to LTE only if LTE truncated cap was included in UL OTA.
  Based on this indication, LTE will decide whether to perfrom TAU or not after WtoL HO.
...
void rrc_send_lte_rrc_wcdma_eutra_cap_sent_ind(sys_modem_as_id_e_type wrrc_as_id)
{
  if(rrcueci_int_data.rrcueci_interrat_lte_capability_info.lte_truncated_cap_appended)
  {
    lte_rrc_wcdma_eutra_cap_sent_ind_s eutra_cap_sent_ind;
    rrc_send_lte_cmd(wrrc_as_id,&eutra_cap_sent_ind.msg_hdr,LTE_RRC_WCDMA_EUTRA_CAP_SENT_IND,sizeof(lte_rrc_wcdma_eutra_cap_sent_ind_s));
    WRRC_MSG0_HIGH("WTOL: LTE_RRC_WCDMA_EUTRA_CAP_SENT_IND sent to LTE RRC");
  }
}
...
#endif
uint8 rrcueci_get_current_substate(sys_modem_as_id_e_type wrrc_as_id)
{
  return ((uint8)rrcueci_int_data.rrcueci_substate);
}
/*===========================================================================
...
===========================================================================*/
  ...
  __attribute__((section(".uncompressible.text")))
#endif
uint8 rrcueci_return_ueci_substate(sys_modem_as_id_e_type wrrc_as_id)
{
  return (uint8)rrcueci_int_data.rrcueci_substate;
}
...
SIDE EFFECTS
  None
===========================================================================*/
uint16 rrcueci_get_cap_container_length(sys_modem_as_id_e_type wrrc_as_id)
{
#ifdef FEATURE_WCDMA_TO_LTE
  return rrcueci_int_data.rrcueci_interrat_lte_capability_info.cap_cont_length;
#else
  return 0;
#endif
}
...
{
  qsh_mdump_collect_high(&rrcueci_int_data,sizeof(rrcueci_int_data));
  qsh_mdump_collect_high(&rrcueci_ext_data,sizeof(rrcueci_ext_data));
}
#endif