...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcenhstateproc.c#2 $    $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
  
 when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
06/30/16   sg      Made changes for F3 reduction
12/30/15   sp      Added array boundary checks.
11/19/15   sp      Made changes to configure proper LC ID to MAC during dch to fach state transition on HSRACH cell.
03/03/15   aa      Made changes to provide RB Id and one-sided reestablishment flag status in RLC config procedures
...
)
{
     ...
     (rlc_dl_log_chl_id != RRCLCM_RLC_LC_ID_ERROR))
  {
    ...
    /* Also update the count and logical channel ID in Ordered Config */
    if(get_rlc_dl_um_info(wrrc_as_id,CCCH_RADIO_BEARER_ID, TRUE, ordered_config_ptr) == FAILURE)
    {
    ...
    if(num_chl < UE_MAX_DL_UM_CHANNEL)
    {
      ...
      rrcllc_cmd_under_process.chan_config.rlc_config_req_list.dl_um.chan_info[num_chl-1].rlc_size =
        rrcllc_get_dl_um_ccch_rlc_size(wrrc_as_id);//CRC_DEEPTI - Need to check
...
/*If cell supports HS-RACH set the LC ID for CCCH 
  All other elements for CCCH should have been filled while extracting the common edch info from SIB5 */
    if(TRUE == rrchsrach_get_ready_for_common_edch(wrrc_as_id,ordered_config_ptr)
      ||
      ((TRUE == rrcllc_check_dch_fach_reconfig_status(wrrc_as_id)) && (TRUE == rrchsrach_camped_cell_supports_hsrach(wrrc_as_id))))
    {
...
)
{
  ...
  if(rlc_dl_log_chl_id != RRCLCM_RLC_LC_ID_ERROR)
  {
    ...
    /* Boundary check before accessing the array */
...
)
{
  ...
  {
    hsdpa_action = rrcllc_get_hs_action_in_e_fach(wrrc_as_id,config_ptr);
    switch(hsdpa_action)
    {
    case  HSDPA_NOOP:
      if ((config_ptr->dest_freq_present == TRUE) &&
            (config_ptr->rrc_state_indicator == RRC_STATE_CELL_DCH) &&
          (rrc_get_hsdpa_sts(wrrc_as_id) == HSDPA_ACTIVE))
      {
    ...
    case HSDPA_START:
      ...
      rrcllc_cmd_under_process.chan_config.l1_req_list.l1_req[0].cmd.setup.hsdpa_l1_info =
        ...
        WRRC_MSG2_MED("EFACH:l1hsaction START. BitMask %x for MACHS:1/MACEHS:2 : %d",
...
//      rrcllc_cmd_under_process.chan_config.hsdpa_active_to_inactive = TRUE;
  ...
  {
    WRRC_MSG3_MED("EFACH: mac_hs_transition_type %d l1hsaction %d. BitMask %d",
...
)
{
  ...
  {
    if(rrcllc_process_drop_sccpch(wrrc_as_id) == FAILURE)
    {
      return FAILURE;
    }
  }
  if(rrcenhstate_add_hs_pdsch(wrrc_as_id) == FAILURE)
  {
    return FAILURE;
  }

  rrcllc_set_hs_action_in_e_fach(wrrc_as_id,ordered_config_ptr, HSDPA_START);

  if (rrcenhstate_set_hsdpa_l1_info_in_cmd_under_process(wrrc_as_id,ordered_config_ptr) == FAILURE)
  {
    WRRC_MSG0_ERROR("EFACH:HSDPA Action Setup FAILURE");
    rrcllc_set_hs_action_in_e_fach(wrrc_as_id,ordered_config_ptr, HSDPA_NOOP);
    return FAILURE;
  }
  else
  {
...
)
{
  ...
  if(rrcenhstate_reconfig_hs_pdsch(wrrc_as_id) == FAILURE)
  {
    return FAILURE;
  }

  rrcllc_set_hs_action_in_e_fach(wrrc_as_id,ordered_config_ptr, HSDPA_RECONFIG);
...
{
  if(rrcenhstate_release_hs_pdsch(wrrc_as_id) == FAILURE)
  {
    return FAILURE;
  }
...
)
{
 ...
 /* Get a new CCTrCH ID */
   dl_cctrch_id = rrcllc_get_cctrch_id(wrrc_as_id);
   ...
   WRRC_MSG1_MED("EFACH:Set CCTrCh Id %d in L1 structure",dl_cctrch_id);
...
)
{
  ...
  {
    if (rrcllc_get_hs_status_in_e_fach(wrrc_as_id,reselection_config_ptr) == TRUE)//CRC_DEEPTI: What if resel ptr is CC?
    {
           ...
           RRCLLC_INVALID_CCTRCH_ID)
      {
        ...
        rrcllc_set_hs_action_in_e_fach(wrrc_as_id,reselection_config_ptr,HSDPA_RECONFIG);
      }
      else
      {
        WRRC_ERR0_FATAL("EFACH: CCTrCH ID not assigned to HS PDSCH");
      }
    }
    else
    {
      ...
      rrcllc_set_hs_status_in_e_fach(wrrc_as_id,reselection_config_ptr,TRUE);
      rrcllc_set_hs_action_in_e_fach(wrrc_as_id,reselection_config_ptr,HSDPA_START);
   ...
   /* Get a new CCTrCH ID */
    ...
    reselection_config_ptr->mac_dl_parms.mac_hsdpa_action = rrcllc_get_hs_action_in_e_fach(wrrc_as_id,reselection_config_ptr);
  ...
  {
    if(rrcllc_get_hs_status_in_e_fach(wrrc_as_id,reselection_config_ptr) == TRUE)
    {
      ...
      {
        RRCLLC_ADD_CCTRCH_ID_TO_RELEASE_LIST(reselection_config_ptr->l1_hsdpa_info.hs_dsch_info.cctrch_id);
        rrcllc_set_hs_action_in_e_fach(wrrc_as_id,reselection_config_ptr, HSDPA_STOP);
        reselection_config_ptr->mac_dl_parms.mac_hsdpa_action = HSDPA_STOP;
        reselection_config_ptr->mac_dl_parms.mac_hs_reset_indicator = TRUE;
        rrcllc_set_hs_status_in_e_fach(wrrc_as_id,reselection_config_ptr,FALSE); 
      }
    }
    else
    {
      WRRC_MSG0_HIGH("EFACH:Reselection non eFACH -> non eFACH cell");
      rrcllc_set_hs_action_in_e_fach(wrrc_as_id,reselection_config_ptr, HSDPA_NOOP);
...
)
{
  ...
  hs_status = rrcllc_get_hs_status_in_e_fach(wrrc_as_id,current_config_ptr);
  if (hs_status == TRUE)
  {
...
)
{
  ...
  hs_action = rrcllc_get_hs_action_in_e_fach(wrrc_as_id,current_config_ptr);
  ...
  {
    WRRC_ERR2_FATAL("EFACH:Invalid HS status %d hs_action %d --> HSPDSCH release failed", hs_status,hs_action);
  }
  return SUCCESS;
}
...
{
  hsdpa_action_enum_type hs_action = rrcllc_get_hs_action_in_e_fach(wrrc_as_id,reselection_config_ptr);
  ...
  if(hs_action != HSDPA_NOOP)
  {
    ...
    if(cell_trans->setup_cmd.hsdpa_l1_ops != HSDPA_SETUP_OPS_STOP)
    {
...
#ifdef FEATURE_WCDMA_HS_RACH
        if ((rrchsrach_camped_cell_supports_hsrach(wrrc_as_id) == TRUE) && (reselection_config_ptr->ack_nack_support_on_hs_dpcch))
        {
          cell_trans->setup_cmd.hsdpa_l1_info->hs_req_mask |=  CPHY_HS_DPCCH_CFG_INCL;
          WRRC_MSG1_MED("RRCHSRACH: hs_req_mask 0x%x",cell_trans->setup_cmd.hsdpa_l1_info->hs_req_mask);
        }
#endif
          ...
          (curr_rrc_state == RRC_STATE_CONNECTING)))
      {
        ...
        if(rrcenhstate_get_e_fach_drx_status(wrrc_as_id,cell_trans->setup_cmd.hsdpa_l1_info))
        {
