...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcirat.c#3 $
...
rrcirat_ext_data_type rrcirat_ext_data_sim[MAX_RRC_STACKS];
#define rrcirat_ext_data rrcirat_ext_data_sim[WAS_ID_M]
#else
rrcirat_ext_data_type rrcirat_ext_data;
#endif
...
** ----------------------------------------------------------------------- */
#ifdef FEATURE_DUAL_WCDMA 
LOCAL rrcirat_int_data_type rrcirat_int_data_sim[MAX_RRC_STACKS];
#define rrcirat_int_data rrcirat_int_data_sim[WAS_ID_M]
#ifdef FEATURE_WCDMA_TO_LTE
rrcirat_eutra_freq_type *rrcirat_eutra_freq_list_ptr_sim[MAX_RRC_STACKS] = {rrcirat_int_data_sim[0].rrcirat_eutra_freq_list, rrcirat_int_data_sim[1].rrcirat_eutra_freq_list};
#define rrcirat_eutra_freq_list_ptr rrcirat_eutra_freq_list_ptr_sim[WAS_ID_M]
#endif

#else
LOCAL rrcirat_int_data_type rrcirat_int_data;
#ifdef FEATURE_WCDMA_TO_LTE
rrcirat_eutra_freq_type *rrcirat_eutra_freq_list_ptr = rrcirat_int_data.rrcirat_eutra_freq_list;
#endif
...
static sys_band_T rrc_get_gsm_band(sys_modem_as_id_e_type wrrc_as_id,l1_gsm_cell_info_type gsm_info)
{
  if (gsm_info.band_indicator == L1_DCS_1800_BAND)
  {
    return(rr_rrc_get_arfcn_band_internal(wrrc_as_id,gsm_info.bcch_arfcn.num, RRC_RR_DCS_1800));
  }
  else
  {
    return(rr_rrc_get_arfcn_band_internal(wrrc_as_id,gsm_info.bcch_arfcn.num, RRC_RR_PCS_1900));
  }
}
#endif /* FEATURE_GSM_QUADBAND */
...
)
{
  ...
  /* FR50563: Skip G freq measurements when fallback is active and G_LAC bar timer is running*/
  ...
  {
    WRRC_MSG0_HIGH("Don't configure G measurements when G_LAC bar timer is running");
    return FAILURE;
  }
...
if((((rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_INITIAL)
   ...
   &&(rrcnv_data.rrc_disable_meas_nv & RRC_DISABLE_WTOG_IDLE_MEAS))

   || ((rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_CELL_FACH)
       &&(rrcnv_data.rrc_disable_meas_nv & RRC_DISABLE_WTOG_FACH_MEAS))

   || ((rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_CELL_DCH)
       &&(rrcnv_data.rrc_disable_meas_nv & RRC_DISABLE_WTOG_CM_MEAS))
  )
{
  WRRC_MSG2_HIGH("Skip IRAT/IFREQ meas  Disable IRAT/IFREQ NV %d  in meas state  %d",rrcnv_data.rrc_disable_meas_nv,rrcmeas_ext_data.rrcmeas_current_substate);
  l1_inter_rat_sib_ptr->inter_sys_meas_info_incl = FALSE;
  return FAILURE;
}
  if(!rrcmeas_ext_data.process_interfreq_interrat_meas_rel7)
  {
  ...
  sib3_ptr = (rrc_SysInfoType3 *) rrc_sibdb_return_sib_for_srv_cell(wrrc_as_id, rrc_SIB3);
  if( sib3_ptr == NULL )
  {
  ...
  if (((rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_CELL_FACH) || (rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_PCH)) &&
    ((sib3_ptr->sib4indicator == TRUE)))
  {
    sib4_ptr = (rrc_SysInfoType4 *) rrc_sibdb_return_sib_for_srv_cell(wrrc_as_id, rrc_SIB4);
  ...
  else
  {
         ...
         rrc_MeasurementControlSysInfo_use_of_HCS_hcs_used))
    {
        ...
        interRATMeasurementSysInfoPresent))
      {
        ...
        if(rrcirat_read_sib11bis(wrrc_as_id,l1_inter_rat_sib_ptr, sib18_present, tmp_rrc_PLMNsOfInterRATCellsList_ptr)== FAILURE)
        {
      ...
      if (rrcirat_fill_hcs_info(wrrc_as_id,&sib11_ptr->measurementControlSysInfo.use_of_HCS.u.
    ...
    { /* HCS is NOT in use */
        ...
        * really need to give this cell to L1 */
          ...
          T_rrc_NewInterRATCell_B_technologySpecificInfo_gsm)
        {
          ...
          * the cell id back in passed cell_position variable */
          if (rrcirat_update_inter_rat_cell_list(wrrc_as_id,&cell_position,arfcn) == FAILURE)
          {
        ...
        else
        {
          ...
          * as they are present */
            ...
            cellSelectionReselectionInfoPresent)
          {
            ...
            if(rel5_ext_ptr != NULL)
            {
              if(RRC_MSG_LIST_BITMASK_IE_TYPE2(rel5_ext_ptr,deltaQrxlevmin))
              {
                ...
                WRRC_MSG3_HIGH("Delta-Qrxlev=%d, Qrxlev=%d, ARFCN=%d",
          ...
          if ((rrcirat_int_data.converted_gsm_band = rrc_get_gsm_band(wrrc_as_id,l1_inter_rat_sib_ptr->inter_s.add_cell[cell_count].u.gsm_cell.gsm_info)) !=
            INVALID_BAND)
          {
            l1_inter_rat_sib_ptr->inter_s.add_cell[cell_count].u.gsm_cell.gsm_info.bcch_arfcn.band =
              rrcirat_int_data.converted_gsm_band;
          }
          else
          {
        ...
        * the cell id back in passed cell_position variable */
        ...
        if(sib18_present)
        {
          if(tmp_rrc_PLMNsOfInterRATCellsList_ptr->n > idy)
          {
            ...
            {
              rrc_translate_gsm_map_plmn_id(wrrc_as_id,&tmp_rrc_PLMNsOfInterRATCellsList_ptr->elem[idy].plmn_Identity,
                &tmp_plmn_identity);
              if(rrc_bcch_and_nas_plmns_match(wrrc_as_id,tmp_plmn_identity, selected_plmn_identity) == FALSE)
              {
      ...
      if(rrcirat_read_sib11bis(wrrc_as_id,l1_inter_rat_sib_ptr, sib18_present, tmp_rrc_PLMNsOfInterRATCellsList_ptr) == FAILURE)
      {
        WRRC_MSG0_MED("SIB11bis: Reading Sib11bis FAILURE");
        l1_inter_rat_sib_ptr->inter_s.num_cell_add = prev_cell_count;
      }


      WRRC_MSG0_MED("IRAT CELL INFO = FALSE");
      rrcirat_int_data.interRAT_cell_info_status.interRAT_cell_info_present = FALSE;
      rrcirat_int_data.interRAT_cell_info_status.interRAT_cell_info_ind = 0;

      if (RRC_CHECK_COMMON_MSG_TYPE(sib11_ptr->measurementControlSysInfo.use_of_HCS,
           rrc_MeasurementControlSysInfo_use_of_HCS_hcs_used ))
      {
        WRRC_MSG0_MED("HCS=>TRUE");
        if(sib11_ptr->measurementControlSysInfo.use_of_HCS.u.hcs_used->m.
            interRATMeasurementSysInfoPresent)
        {
          ...
          {
            WRRC_MSG0_MED("IRAT CELL INFO = TRUE");
            rrcirat_int_data.interRAT_cell_info_status.interRAT_cell_info_present = TRUE;
            rrcirat_int_data.interRAT_cell_info_status.interRAT_cell_info_ind = 0;
            return SUCCESS;
          }
        }
  ...
  switch(removedInterRATCellList->t)
  {
    case T_rrc_RemovedInterRATCellList_removeAllInterRATCells:
         ...
         *num_removed_cells = L1_MAX_CELL_MEAS;
         rrcirat_initialize_cell_list(wrrc_as_id);
      break;
    ...
    case T_rrc_RemovedInterRATCellList_removeSomeInterRATCells:
         ...
         else
         {
           ...
           do
           {
             ...
             {
               rrcirat_ext_data.inter_rat_cell_list[inter_rat_cell_id].cell_position = VACANT;
  ...
  * one at a time */
  ...
  for ( sib11_a_cell=0 ; sib11_a_cell<sib11_added_cells; sib11_a_cell++ )
  {
    ...
    if ( match_found == 0 )
    {
      ...
      rrcirat_int_data.inter_sys_sib12_new_cell_list.add_cell[new_cell_list].intersys_cell_id =
        sib11_cell_list_ptr->add_cell[sib11_a_cell].intersys_cell_id;

      rrcirat_int_data.inter_sys_sib12_new_cell_list.add_cell[new_cell_list].measure_in_idle_pch_fach =
        sib11_cell_list_ptr->add_cell[sib11_a_cell].measure_in_idle_pch_fach;
      rrcirat_int_data.inter_sys_sib12_new_cell_list.add_cell[new_cell_list].cell_info_type =
        sib11_cell_list_ptr->add_cell[sib11_a_cell].cell_info_type;

      rrcirat_int_data.inter_sys_sib12_new_cell_list.add_cell[new_cell_list].u.gsm_cell =
  ...
  new_cell_list_ptr = &rrcirat_int_data.inter_sys_sib12_new_cell_list;
 ...
 )
{
  ...
  else if(rrcmeas_ext_data.rrcmeas_sib11->fach_meas_incl == TRUE)
  {
    l1_inter_rat_sib_ptr->fach_meas_incl = TRUE;

    /* Initialize the following value to Default values */
    l1_inter_rat_sib_ptr->fach.k_ind = rrcmeas_ext_data.rrcmeas_sib11->fach.k_ind;
    l1_inter_rat_sib_ptr->fach.inter_f_ind = rrcmeas_ext_data.rrcmeas_sib11->fach.inter_f_ind;
    l1_inter_rat_sib_ptr->fach.k =  rrcmeas_ext_data.rrcmeas_sib11->fach.k;
    l1_inter_rat_sib_ptr->fach.num_inter_sys = rrcmeas_ext_data.rrcmeas_sib11->fach.num_inter_sys;
    l1_inter_rat_sib_ptr->fach.inter_sys[0] = rrcmeas_ext_data.rrcmeas_sib11->fach.inter_sys[0];
  }
     ...
     rrc_MeasurementControlSysInfo_use_of_HCS_hcs_used ))
  {
        ...
        interRATMeasurementSysInfoPresent))
    {
      ...
      if(rrcmeas_ext_data.rrcmeas_sib11->inter_sys_meas_info_incl == TRUE)
      {
        WRRC_MSG0_HIGH("Copying inter rat cell info from SIB 11");
        l1_inter_rat_sib_ptr->inter_sys_meas_info_incl = TRUE;
        l1_inter_rat_sib_ptr->inter_s = rrcmeas_ext_data.rrcmeas_sib11->inter_s;
        l1_inter_rat_sib_ptr->hcs_used = TRUE;
      }
      return SUCCESS;
    }
    ...
    rrcirat_process_remove_cell_list(wrrc_as_id,&sib12_ptr->measurementControlSysInfo.use_of_HCS.u.hcs_used->
        ...
        if ((sib4_present == TRUE) && (sib4_ptr != NULL))
        {
             ...
             rrc_CellSelectReselectInfoSIB_3_4_modeSpecificInfo_fdd))
          {
              ...
              deltaQrxlevminPresent))
            {
              ...
              WRRC_MSG3_HIGH("SIB4 Delta-Qrxlev=%d, Qrxlev=%d, ARFCN=%d",
        ...
        else
        {
             ...
             rrc_CellSelectReselectInfoSIB_3_4_modeSpecificInfo_fdd))
          {
              ...
              deltaQrxlevminPresent))
            {
              ...
              WRRC_MSG3_HIGH("SIB3 Delta-Qrxlev=%d, Qrxlev=%d, ARFCN=%d",
    ...
    rrcirat_build_composite_cell_list_from_SIB11_SIB12(wrrc_as_id,
      ...
      &rrcmeas_ext_data.rrcmeas_sib11->inter_s, &l1_inter_rat_sib_ptr->inter_s, num_removed_cells);
    for(cell_count = 0; cell_count < l1_inter_rat_sib_ptr->inter_s.num_cell_add; cell_count++)
    {
       if(sib18_present)
       {
         if(tmp_rrc_PLMNsOfInterRATCellsList_ptr->n > cell_count)
         {
           ...
           {
             rrc_translate_gsm_map_plmn_id(wrrc_as_id,&tmp_rrc_PLMNsOfInterRATCellsList_ptr->elem[cell_count].plmn_Identity,
               &tmp_plmn_identity);
             if(rrc_bcch_and_nas_plmns_match(wrrc_as_id,tmp_plmn_identity, selected_plmn_identity) == FALSE)
             {
               if((rrccsp_check_for_eplmn(wrrc_as_id,
  ...
  else
  {
    ...
    /* Process the remove cell list of SIB12 */
    rrcirat_process_remove_cell_list(wrrc_as_id,&sib12_ptr->measurementControlSysInfo.use_of_HCS.u.hcs_not_used->
      interRATMeasurementSysInfo.interRATCellInfoList.removedInterRATCellList, &num_removed_cells);
     ...
     rrc_MeasurementControlSysInfo_use_of_HCS_hcs_used ))
    {
        ...
        interRATMeasurementSysInfoPresent))
      {
        if ((RRC_MSG_COMMON_BITMASK_IE(sib12_ptr->measurementControlSysInfo.use_of_HCS.u.hcs_used->
           interRATMeasurementSysInfo,rrc_InterRATMeasurementSysInfo,interRATCellInfoList)) ||
           (rrcmeas_ext_data.rrcmeas_sib11->inter_sys_meas_info_incl == TRUE))
        {
...
)
{
  ...
  if (*position_no_ptr == AVAILABLE_CELL_POSITION)
  {
    for (count = 0; count < L1_MAX_CELL_MEAS; count++)
    {
      if (rrcirat_ext_data.inter_rat_cell_list[count].cell_position == VACANT)
      {
        /* Store this cell in this first available vacant position */
        rrcirat_ext_data.inter_rat_cell_list[count].cell_position = OCCUPIED;
        rrcirat_ext_data.inter_rat_cell_list[count].cell_id = count;
        rrcirat_ext_data.inter_rat_cell_list[count].arfcn = arfcn;
  ...
  for (count = 0; count < L1_MAX_CELL_MEAS; count++)
  {
     ...
     * If yes, then overwrite on this cell */
      ...
      rrcirat_ext_data.inter_rat_cell_list[count].cell_id = *position_no_ptr;
...
)
{
  ...
  if (reportCriteria_ptr->t == T_rrc_InterRATReportCriteria_interRATReportingCriteria)
  {
    ...
    *mcm_rpt_crit_ptr = EVENT;
    ...
    if (rrcirat_process_event_criteria(wrrc_as_id,reportCriteria_ptr->u.interRATReportingCriteria,
    ...
    *mcm_rpt_crit_ptr = PERIODIC;
    ...
    else
    {
      ...
      inter_s_ptr->rpt_criteria.u.periodic_crit.rpt_interval =
        rrcirat_ret_periodic_rpt_intvl(wrrc_as_id,
        reportCriteria_ptr->u.periodicalReportingCriteria->
...
)
{
...
#ifdef FEATURE_WCDMA_CM_LTE_SEARCH
  if(rrcirat_is_meas_id_eutra_meas(wrrc_as_id,(uint16) rrc_mcm_ptr->measurementIdentity))
  {
    WRRC_MSG0_HIGH("MCM-R3 type EUTRA");
    interrat_type = RRCMEAS_IRAT_EUTRA;
  }    
  if((rrcnv_data.rrc_wtol_cm_support_nv == FALSE) && (interrat_type == RRCMEAS_IRAT_EUTRA))
  {
  ...
  if(((RRCMEAS_IRAT_EUTRA == interrat_type) && (rrcmcm_is_lte_mode_enabled(wrrc_as_id) == FALSE)))
  {
  ...
  if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_setup)
  {
    ...
    if(FAILURE == rrcirat_process_mcm_report_criteria(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.setup->u.interRATMeasurement->
    ...
    {
      result = rrcirat_process_cell_list_mcm(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.
      ...
      rrc_InterRATMeasurement,interRATCellInfoList))
    {
      for (count=0; count < L1_MAX_CELL_MEAS; count++)
      {
        WCDMA_MEMCPY(&rrcirat_ext_data.inter_rat_cell_list[count],
                     sizeof(inter_rat_cell_list_struct_type),
                     &rrcirat_int_data.local_neighbor_list[count],
                     sizeof(inter_rat_cell_list_struct_type));
      }
    }
    ...
    for ( count = 0; count < MAX_INTER_RAT_MEAS; count++ )
    {
      ...
      if (rrcirat_ext_data.inter_rat_meas_id_list[count].meas_id == l1_inter_rat_meas_params->meas_id)
      {
        rrcirat_ext_data.inter_rat_meas_id_list[count].meas_id = (uint16) rrc_mcm_ptr->measurementIdentity;
        rrcirat_ext_data.inter_rat_meas_id_list[count].rpt_crit = mcm_rpt_crit;
        rrcirat_ext_data.inter_rat_meas_id_list[count].interrat_type = RRCMEAS_IRAT_GSM;
    ...
    if (meas_exists_for_setup == FALSE)
    {
      ...
      {
        /* First check if this meas already exists */
        if (rrcirat_ext_data.inter_rat_meas_id_list[count].meas_id == INVALID_MEAS_ID)
        {
          rrcirat_ext_data.inter_rat_meas_id_list[count].meas_id = (uint16) rrc_mcm_ptr->measurementIdentity;
          rrcirat_ext_data.inter_rat_meas_id_list[count].rpt_crit = mcm_rpt_crit;
          rrcirat_ext_data.inter_rat_meas_id_list[count].interrat_type = RRCMEAS_IRAT_GSM;
  ...
  else if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_modify)
  {
    ...
    else
    {
      ...
      if(FAILURE == rrcirat_process_mcm_report_criteria(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.modify->
  ...
  * that this meas exists. So let us delete it from inter_rat_meas_id_list
    * Meas Identity, Transaction Id already set */
    ...
    rrcmeas_delete_addtl_meas(wrrc_as_id,&l1_inter_rat_meas_params->meas_id);
    ...
    for (count=0; count < MAX_INTER_RAT_MEAS; count++)
    {
      if (rrcirat_ext_data.inter_rat_meas_id_list[count].meas_id == l1_inter_rat_meas_params->meas_id)
      {
        rrcirat_ext_data.inter_rat_meas_id_list[count].meas_id = INVALID_MEAS_ID;

        /* Also remove any Addtl Meas associated with it */
        for (cnt = 0; cnt < MAX_ADDITIONAL_MEAS; cnt++)
        {
          rrcirat_ext_data.inter_rat_meas_id_list[count].additional_meas[cnt] = INVALID_MEAS_ID;
        }
  ...
  if(rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_setup)
  {
       ...
       rrc_InterRATMeasurement,interRATCellInfoList)))
    {
      ...
      {
        rrcirat_int_data.interRAT_cell_info_status.interRAT_cell_info_present=TRUE;
        rrcirat_int_data.interRAT_cell_info_status.interRAT_cell_info_ind=cell_info_ind;
      }
      else
      {
        rrcirat_int_data.interRAT_cell_info_status.interRAT_cell_info_present=FALSE;
  ...
  rrcmeas_send_l1_cmd(wrrc_as_id,NULL, l1_inter_rat_meas_params, NULL, L1_MEAS_CTRL_PARMS);
...
through local neighbour list. This has to be done to make sure that if the Meas Ctrl
  ...
  rrcirat_initialize_local_neighbor_list(wrrc_as_id);
  ...
  {
    WCDMA_MEMCPY(&rrcirat_int_data.local_neighbor_list[cnt],
                 sizeof(inter_rat_cell_list_struct_type),
                 &rrcirat_ext_data.inter_rat_cell_list[cnt],
                 sizeof(inter_rat_cell_list_struct_type));
  }
  ...
  if (rrcirat_fill_removed_inter_rat_cells(wrrc_as_id,&mcm_cell_list_ptr->removedInterRATCellList,
  ...
  * So check in the CELL_INFO_LIST */
  ...
  if ((mcm_cell_list_ptr->removedInterRATCellList.t ==
    ...
    (rrcirat_check_cell_absence_in_cell_list(wrrc_as_id) == SUCCESS))
  {
  ...
  * Get New cells to be added. UTRAN can just set it to NULL */
  ...
  {
    if (rrcirat_check_inter_rat_cell_order_mcm(wrrc_as_id,&mcm_cell_list_ptr->newInterRATCellList) == FAILURE)
    {
    ...
    /* Let first check the type of the cell to find if we
...
#ifdef FEATURE_GSM_QUADBAND
        if ((rrcirat_int_data.converted_gsm_band = rrc_get_gsm_band(wrrc_as_id,l1_cell_list_ptr->add_cell[cell_count].u.gsm_cell.gsm_info)) !=
          INVALID_BAND)
        {
      ...
      if (rrcirat_update_inter_rat_cell_list_mcm(wrrc_as_id,&cell_position,
        &l1_cell_list_ptr->add_cell[cell_count].intersys_cell_id, &cell_id, 
        (uint16)local_rrc_new_cell_list_ptr->elem[idx].technologySpecificInfo.u.gsm->bcch_ARFCN)
        == SUCCESS)
      {
    ...
    rrc_InterRATCellInfoList,cellsForInterRATMeasList))
  {
    ...
    while (local_cells_for_meas_ptr->n > idx)
    {
      ...
      if( rrcirat_find_cell_id(wrrc_as_id,&cell_id) == FAILURE)
      {
        ...
        rrc_Event3a,reportingCellStatus))
      {
        l1_event_criteria_ptr->evt[cnt].rpt_cell_status_incl = TRUE;
          ...
          if ((T_rrc_ReportingCellStatus_withinActSetOrVirtualActSet_InterRATcells != 
            ...
            (rrcmeas_fill_no_of_reporting_cells(wrrc_as_id,
...
)
{
  ...
  case T_rrc_RemovedInterRATCellList_removeAllInterRATCells:
    ...
    {
      rrcirat_int_data.local_neighbor_list[removed_cell_count].cell_id = INVALID_CELL_ID;
      rrcirat_int_data.local_neighbor_list[removed_cell_count].cell_position = VACANT;
    }
    break;
  ...
  case T_rrc_RemovedInterRATCellList_removeSomeInterRATCells:
    ...
    do
    {
      ...
      if (rrcirat_find_cell_id(wrrc_as_id,&inter_rat_cell_id) == FAILURE)
      {
      ...
      else
      {
        ...
        rrcirat_remove_cell_id(wrrc_as_id,&inter_rat_cell_id);
...
)
{
  ...
  for (count = 0; count < L1_MAX_CELL_MEAS; count++)
  {
    ...
    if ((rrcirat_int_data.local_neighbor_list[count].cell_position == OCCUPIED) &&
      (rrcirat_int_data.local_neighbor_list[count].cell_id == *intra_freq_cell_id_ptr))
    {
      return SUCCESS;
    }
  }
...
)
{
  ...
  for (count = 0; count < L1_MAX_CELL_MEAS; count++)
  {
    if ((count == (*inter_rat_cell_id_ptr)) &&
      (rrcirat_int_data.local_neighbor_list[count].cell_position == OCCUPIED))
    {
      WRRC_MSG1_HIGH("MCM:Removing cell id with id %d",
        *inter_rat_cell_id_ptr);

      rrcirat_int_data.local_neighbor_list[count].cell_position = VACANT;

      rrcirat_int_data.local_neighbor_list[count].cell_id =  INVALID_CELL_ID;
...
)
{
  ...
  if (*position_no_ptr == AVAILABLE_CELL_POSITION)
  {
    for (count = 0; count < L1_MAX_CELL_MEAS; count++)
    {
      if (rrcirat_int_data.local_neighbor_list[count].cell_position == VACANT)
      {
        /* Store this cell in this first available vacant position */
        rrcirat_int_data.local_neighbor_list[count].cell_position = OCCUPIED;
        rrcirat_int_data.local_neighbor_list[count].cell_id = count;
        rrcirat_int_data.local_neighbor_list[count].arfcn = arfcn;
  ...
  for (count = 0; count < L1_MAX_CELL_MEAS; count++)
  {
      ...
      (rrcirat_int_data.local_neighbor_list[count].cell_position == OCCUPIED) )
    {
      ...
      WRRC_MSG3_HIGH("Add:Cell Id %d at pos. %d removed[Cascading] by %d",
        rrcirat_int_data.local_neighbor_list[count].cell_id,count,*cell_id_ptr);


      /* Store the removed cell id */
      *cell_id_to_remove_ptr = rrcirat_int_data.local_neighbor_list[count].cell_id;

      /* Store the new cell id at this position */
      rrcirat_int_data.local_neighbor_list[count].cell_id = *cell_id_ptr;
      return SUCCESS;
    }
...
void rrcirat_initialize_cell_list(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8 cnt = 0;
  rrcirat_initialize_local_neighbor_list(wrrc_as_id);
  for (cnt =0; cnt <L1_MAX_CELL_MEAS; cnt++ )
  {
    rrcirat_ext_data.inter_rat_cell_list[cnt].cell_id = INVALID_CELL_ID;
    rrcirat_ext_data.inter_rat_cell_list[cnt].cell_position = VACANT;
  }
...
#ifdef FEATURE_WCDMA_TO_LTE
  rrcirat_initialize_eutra_freq_list(wrrc_as_id);
#endif
}
...
)
{
  ...
  for ( count = 0; count < MAX_INTER_RAT_MEAS; count++ )
  {
    ...
    rrcirat_ext_data.inter_rat_meas_id_list[count].meas_id = INVALID_MEAS_ID;
    rrcirat_ext_data.inter_rat_meas_id_list[count].meas_validity = STATE_DCH;
    rrcirat_ext_data.inter_rat_meas_id_list[count].rpt_crit = NOT_AVAILABLE;
    rrcirat_ext_data.inter_rat_meas_id_list[count].interrat_type = RRCMEAS_IRAT_GSM;
...
void rrcirat_initialize_local_neighbor_list(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8 cnt = 0;
  for (cnt =0; cnt <L1_MAX_CELL_MEAS; cnt++ )
  {
    rrcirat_int_data.local_neighbor_list[cnt].cell_id = INVALID_CELL_ID;
    rrcirat_int_data.local_neighbor_list[cnt].cell_position = VACANT;
  }
}
...
)
{
  ...
  {
    if((meas_id == rrcirat_ext_data.inter_rat_meas_id_list[cnt].meas_id) && (rrcirat_ext_data.inter_rat_meas_id_list[cnt].interrat_type == RRCMEAS_IRAT_EUTRA))
    {
      status = TRUE;
      break;
    }
...
)
{
    ...
    (0 != l1_meas_report_ptr->u.inter_sys_eutra_rpt.evt_res.num_freq))
  {
    ...
    if(num_valid_freq !=0)
    {
      ...
      rrcirat_set_inter_sys_event_id(wrrc_as_id,&l1_meas_report_ptr->u.inter_sys_eutra_rpt.evt_res.event_id,
         &msg_ptr->message.u.measurementReport.v390nonCriticalExtensions.laterNonCriticalExtensions.v4b0NonCriticalExtensions.
             v590NonCriticalExtensions.v5b0NonCriticalExtensions.v690NonCriticalExtensions.v770NonCriticalExtensions.
  ...
  if(measured_res_incl)
  {
    ...
    if (l1_meas_report_ptr->last_periodic_report == TRUE)
    {
      ...
      {
        if (rrcirat_ext_data.inter_rat_meas_id_list[meas_id_cnt].meas_id == l1_meas_report_ptr->meas_id)
        {
          WRRC_MSG1_HIGH("Delete Meas %d",l1_meas_report_ptr->meas_id);
          rrcirat_ext_data.inter_rat_meas_id_list[meas_id_cnt].meas_id = INVALID_MEAS_ID;
  
          rrcirat_ext_data.inter_rat_meas_id_list[meas_id_cnt].rpt_crit = NOT_AVAILABLE;

          /* Also initialize additional meas */
          for ( cnt = 0; cnt < MAX_ADDITIONAL_MEAS; cnt++ )
            rrcirat_ext_data.inter_rat_meas_id_list[meas_id_cnt].additional_meas[cnt] = INVALID_MEAS_ID;
...
)
{
...
#ifdef FEATURE_WCDMA_CM_LTE_SEARCH
  if(rrcirat_is_meas_id_eutra_meas(wrrc_as_id,l1_meas_report_ptr->meas_id))
  {
    /*Call the EUTRA function to report EUTRA measurements*/
    rrcirat_send_eutra_meas_report(wrrc_as_id,l1_meas_report_ptr);
    return;
  }
#endif
  ...
  if (l1_meas_report_ptr->rpt_crit == L1_PERIODIC_RPT)
  {
    ...
    if (rrcirat_validate_measured_results(wrrc_as_id,
  ...
  else
  {
    if (l1_meas_report_ptr->rpt_crit == L1_EVENT_TRIGGER_RPT)
    {
      ...
      if (rrcirat_validate_event_results(wrrc_as_id,&l1_meas_report_ptr->u.inter_sys_rpt.event_results) ==
        FAILURE)
      {
  ...
  * should be released by SEND_CHAIN using OSS compiler functions */
  ...
  /* First store the meas id */
  rrcmeas_fill_meas_id(wrrc_as_id,&msg_ptr->message.u.measurementReport,l1_meas_report_ptr->meas_id, INTER_SYS);
  ...
  if (l1_meas_report_ptr->rpt_crit == L1_EVENT_TRIGGER_RPT)
  {
    ...
    /* Set Event ID */
    rrcirat_set_inter_sys_event_id(wrrc_as_id,&l1_meas_report_ptr->u.inter_sys_rpt.event_results.event_id,
  ...
  else
  {
    ...
    gsm_cell_count++)
    {
        ...
        u.gsm.measured_cells[gsm_cell_count].gsm_carrier_rssi_incl == TRUE)
      {
        ...
        rrcirat_convert_rssi_value(wrrc_as_id,
          &l1_meas_report_ptr->u.inter_sys_rpt.measurements.
             ...
             (msg_ptr->message.u.measurementReport.eventResults.t == T_rrc_EventResults_interRATEventResults)))
  {
    ...
    if(rrcirat_int_data.interRAT_cell_info_status.interRAT_cell_info_present==TRUE)
    {
      ...
      msg_ptr->message.u.measurementReport.v390nonCriticalExtensions.laterNonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.v5b0NonCriticalExtensions.measurementReport_v5b0ext.interRATCellInfoIndication =
          rrcirat_int_data.interRAT_cell_info_status.interRAT_cell_info_ind ;
      WRRC_MSG1_HIGH("irat cell info ind added in meas report %d", rrcirat_int_data.interRAT_cell_info_status.interRAT_cell_info_ind);
    }
  }
...
)
{
  ...
  {
    if (rrcirat_ext_data.inter_rat_cell_list[local_neighbor_cnt].cell_id != INVALID_CELL_ID)
    {
      return FAILURE;
    }
  }
...
)
{
  ...
  rrcirat_delete_dup_non_verified_cells(wrrc_as_id,meas_results_ptr);
...
uint16 prev_cell_count=0;
    ...
    rrc_InterRATMeasurementSysInfo,interRATCellInfoList)))
  {
    ...
    if(rrcirat_read_sib11bis(wrrc_as_id,l1_sib_ptr,sib18_present,tmp_rrc_PLMNsOfInterRATCellsList_ptr)== FAILURE)
    {
      WRRC_MSG0_HIGH("SIB11bis: Inter RAT in SIB11bis Ignored");
    ...
    (cell_count < L1_MAX_CELL_MEAS) )
  {
    ...
    * really need to give this cell to L1 */
    ...
    else
    {
      ...
      if ((rrcirat_int_data.converted_gsm_band = rrc_get_gsm_band(wrrc_as_id,l1_sib_ptr->inter_s.add_cell[cell_count].u.gsm_cell.gsm_info)) !=
        INVALID_BAND)
      {
...
)
{
  ...
  {
    WCDMA_MEMCPY(&rrcirat_int_data.tmp_measured_cells[count],
  ...
  for (count = 0; count < meas_results_ptr->inter_sys_msmt_results[0].u.gsm.measured_gsm_cells -1; count++)
  {
            ...
            tmp_list_count < L1_MAX_REPORTED_GSM_CELLS; tmp_list_count++)
    {
      ...
      else /* BSIC NOT verified case */
      {
        ...
        if (rrcirat_int_data.tmp_measured_cells[tmp_list_count].bsic_choice == L1_BSIC_VERIFIED)
        {
        ...
        else /* BSIC NOT verified case */
        {
          ...
          if (CONVERT_ARFCN_TO_NUM(rrcirat_int_data.tmp_measured_cells[tmp_list_count].u.bsic_not_verified.bcch_arfcn) ==
            CONVERT_ARFCN_TO_NUM(meas_results_ptr->inter_sys_msmt_results[0].u.gsm.measured_cells[count].u.bsic_not_verified.bcch_arfcn))
          {
            WRRC_MSG5_HIGH( 
                  ...
                  CONVERT_ARFCN_TO_NUM(meas_results_ptr->inter_sys_msmt_results[0].u.gsm.measured_cells[count].u.bsic_not_verified.bcch_arfcn),
                  tmp_list_count, CONVERT_ARFCN_TO_NUM(rrcirat_int_data.tmp_measured_cells[tmp_list_count].u.bsic_not_verified.bcch_arfcn));

            /* Mark ARFCN as invalid so that it could be deleted later */
            CONVERT_ARFCN_TO_NUM(rrcirat_int_data.tmp_measured_cells[tmp_list_count].u.bsic_not_verified.bcch_arfcn) = RRC_INVALID_ARFCN_FLAG;

            /* Go to end of inner for loop */
          }
          else
          {
            WRRC_MSG3_HIGH("CellCnt %d != CellCnt %d ARF %d", count, tmp_list_count,
              CONVERT_ARFCN_TO_NUM(rrcirat_int_data.tmp_measured_cells[tmp_list_count].u.bsic_not_verified.bcch_arfcn));
  ...
  tmp_list_count < meas_results_ptr->inter_sys_msmt_results[0].u.gsm.measured_gsm_cells; tmp_list_count++)
  {
    if (rrcirat_int_data.tmp_measured_cells[tmp_list_count].bsic_choice == L1_BSIC_VERIFIED)
    {
      ...
      WCDMA_MEMCPY(&meas_results_ptr->inter_sys_msmt_results[0].u.gsm.measured_cells[count++],
                   sizeof(l1_measured_cells_struct_type),
                   &rrcirat_int_data.tmp_measured_cells[tmp_list_count],
                   sizeof(l1_measured_cells_struct_type));
    }
    else
    {
      if (CONVERT_ARFCN_TO_NUM(rrcirat_int_data.tmp_measured_cells[tmp_list_count].u.bsic_not_verified.bcch_arfcn) == RRC_INVALID_ARFCN_FLAG)
      {
  ...
  WRRC_MSG3_HIGH("L1 specifed %d cells. After purge dups, RRC has %d cells. New RRC cnt is %d", meas_results_ptr->inter_sys_msmt_results[0].u.gsm.measured_gsm_cells,count,count);
...
)
{
  ...
  if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r4_setup)
  {
    ...
    {
      result = rrcirat_process_cell_list_mcm_r5(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.
        u.setup->u.interRATMeasurement->interRATCellInfoList,
        &l1_inter_rat_meas_params->meas_object.u.inter_s.cell_list);
      if(result == RRCMEAS_FAIL)
      {
...
void rrcirat_add_irat_cell_info_ind(sys_modem_as_id_e_type wrrc_as_id,rrc_MeasurementReport* rpt_ptr)
{
  if(rrcirat_int_data.interRAT_cell_info_status.interRAT_cell_info_present==TRUE)
  {
...
)
{
    ...
    rrc_MeasurementCommand_r6_setup))
  {
    ...
    if(FAILURE == rrcirat_process_mcm_report_criteria(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.setup->
    ...
    {
      result = rrcirat_process_cell_list_mcm_r6(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.
        u.setup->u.interRATMeasurement->interRATCellInfoList,
        &l1_inter_rat_meas_params->meas_object.u.inter_s.cell_list);
      if(result == RRCMEAS_FAIL)
      {
  ...
  * that this meas exists. So let us delete it from inter_rat_meas_id_list
  ...
  if(RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand,rrc_MeasurementCommand_r6_setup))
  {
       ...
       rrc_InterRATMeasurement_r6,interRATCellInfoList)))
    {
      ...
      {
        rrcirat_int_data.interRAT_cell_info_status.interRAT_cell_info_present = TRUE;
        rrcirat_int_data.interRAT_cell_info_status.interRAT_cell_info_ind = rrc_mcm_ptr->measurementCommand.
    ...
    else
    {
      ...
      if (inter_rat_cell_list_ptr->elem[idx].technologySpecificInfo.u.gsm->m.cellSelectionReselectionInfoPresent)
      {
        ...
        if(rel5_ext_ptr != NULL)
        {
          if(RRC_MSG_LIST_BITMASK_IE_TYPE2(rel5_ext_ptr,deltaQrxlevmin))
          {
            WRRC_MSG3_HIGH("SIB11bis: Delta-Qrxlev=%d, Qrxlev=%d, ARFCN=%d",
              (rel5_ext_ptr->elem[idx].deltaQrxlevmin * 2),
              l1_inter_rat_sib_ptr->inter_s.add_cell[cell_count].u.gsm_cell.cell_sel_info.rxlev_min,
              CONVERT_ARFCN_TO_NUM(l1_inter_rat_sib_ptr->inter_s.add_cell[cell_count].u.gsm_cell.gsm_info.bcch_arfcn));
              l1_inter_rat_sib_ptr->inter_s.add_cell[cell_count].u.gsm_cell.cell_sel_info.rxlev_min +=
...
)
{
  ...
  * that this meas exists. So let us delete it from inter_rat_meas_id_list
    * Meas Identity, Transaction Id already set */
    ...
    /* Clear Meas Id from rrcirat_ext_data.inter_rat_meas_id_list List */
    for (count=0; count < MAX_INTER_RAT_MEAS; count++)
    {
      if (rrcirat_ext_data.inter_rat_meas_id_list[count].meas_id == l1_inter_rat_meas_params->meas_id)
      {
      ...
      case T_rrc_EUTRA_FrequencyRemoval_removeSomeFrequencies:
        ...
        if(rmv_freq_ptr != NULL)
        {
          ...
          do
          {
            ...
            if(FAILURE == rrcirat_remove_eutra_freq(wrrc_as_id,earfcn))
            {
    ...
    if(RRC_MSG_COMMON_BITMASK_IE_TYPE2_PTR(eutra_freq_list,eutraNewFrequencies))
    {
      ...
      if(freq_list_ptr->n != 0)
      {
        ...
        do
        {
          ...
          if(FAILURE == rrcirat_update_eutra_freq_list(wrrc_as_id,earfcn))
          {
...
/*    if(RRC_MSG_COMMON_BITMASK_IE(mcm_irat_ptr->interRATMeasQuantity.ratSpecificInfo.u.e_UTRA,))*/
  ...
  if(FAILURE == rrcirat_process_mcm_report_criteria(wrrc_as_id,&mcm_irat_ptr->reportCriteria,temp_inter_s_meas_ctrl,
  ...
  {
    if(FAILURE == rrcirat_process_eutra_freq_list_mcm(wrrc_as_id,mcm_irat_ptr->interRATMeasurementObjects.u.eutra_FrequencyList,
...
)
{
  ...
  if (((RRCMEAS_IRAT_GSM == interrat_type) && (rrcmcm_is_dualmode_enabled_no_f3(wrrc_as_id) == FALSE)) 
#ifdef FEATURE_WCDMA_CM_LTE_SEARCH
       ||((RRCMEAS_IRAT_EUTRA == interrat_type) && (rrcmcm_is_lte_mode_enabled_wo_f3(wrrc_as_id) == FALSE))
#endif
       )
  {
    WRRC_MSG1_HIGH("REL8:IRAT:Mode not enabled, IRAT_Type %d",interrat_type);
    ...
    rrc_MeasurementCommand_r8_setup))
  {
    ...
    if(RRCMEAS_IRAT_EUTRA == interrat_type)
    {
      ...
      WCDMA_MEMCPY(temp_freq_list,
                   ...
                   rrcirat_int_data.rrcirat_eutra_freq_list,
                   L1_MAX_EUTRA_FREQ * sizeof(rrcirat_eutra_freq_type));
      ...
      status = rrcirat_process_mcm_setup_modify_eutra(wrrc_as_id,rrc_mcm_ptr->measurementCommand.u.setup->u.interRATMeasurement,
                             l1_inter_rat_meas_params,transaction_id,&mcm_rpt_crit);
      rrcirat_eutra_freq_list_ptr = rrcirat_int_data.rrcirat_eutra_freq_list;
    ...
    if(RRCMEAS_IRAT_EUTRA == interrat_type)
    {
      ...
      WCDMA_MEMCPY(rrcirat_int_data.rrcirat_eutra_freq_list,
                   L1_MAX_EUTRA_FREQ * sizeof(rrcirat_eutra_freq_type),
                   temp_freq_list,
                   L1_MAX_EUTRA_FREQ * sizeof(rrcirat_eutra_freq_type));
    }
    ...
    for ( count = 0; count < MAX_INTER_RAT_MEAS; count++ )
    {
      ...
      if (rrcirat_ext_data.inter_rat_meas_id_list[count].meas_id == l1_inter_rat_meas_params->meas_id)
      {
        rrcirat_ext_data.inter_rat_meas_id_list[count].meas_id = (uint16) rrc_mcm_ptr->measurementIdentity;
        rrcirat_ext_data.inter_rat_meas_id_list[count].rpt_crit = mcm_rpt_crit;
        rrcirat_ext_data.inter_rat_meas_id_list[count].interrat_type = interrat_type;
        meas_exists_for_setup = TRUE;
        break;
      }
    }
  ...
  else if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r8_modify)
  {
    ...
    else
    {
      ...
      if(RRCMEAS_IRAT_EUTRA == interrat_type)
      {
        ...
        status = rrcirat_process_mcm_setup_modify_eutra(wrrc_as_id,rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interRATMeasurement,
                               l1_inter_rat_meas_params,transaction_id,&mcm_rpt_crit);
        rrcirat_eutra_freq_list_ptr = rrcirat_int_data.rrcirat_eutra_freq_list;
        if(FAILURE == status)
        {
  ...
  if(RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand,rrc_MeasurementCommand_r8_setup))
  {
        ...
        interRATMeasurementObjects)))
    {
          ...
          interRATMeasurementObjects,rrc_InterRATMeasurement_r8_interRATMeasurementObjects,interRATCellInfoList))
      {
        ...
        {
          rrcirat_int_data.interRAT_cell_info_status.interRAT_cell_info_present = TRUE;
          rrcirat_int_data.interRAT_cell_info_status.interRAT_cell_info_ind = rrc_mcm_ptr->measurementCommand.u.setup->u.interRATMeasurement->
  ...
  if(RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand,rrc_MeasurementCommand_r8_modify))
  {
        ...
        interRATMeasurementObjects)))
    {
        ...
        interRATMeasurementObjects,rrc_InterRATMeasurement_r8_interRATMeasurementObjects,interRATCellInfoList))
      {
        ...
        {
          rrcirat_int_data.interRAT_cell_info_status.interRAT_cell_info_present = TRUE;
          rrcirat_int_data.interRAT_cell_info_status.interRAT_cell_info_ind = rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.
...
/*    if(RRC_MSG_COMMON_BITMASK_IE(mcm_irat_ptr->interRATMeasQuantity.ratSpecificInfo.u.e_UTRA,))*/
  ...
  {
    if(FAILURE == rrcirat_process_eutra_freq_list_mcm_r9(wrrc_as_id,mcm_irat_ptr->interRATMeasurementObjects.u.eutra_FrequencyList,
        &l1_inter_rat_meas_params->meas_object.u.inter_s_eutra.eutra_freq_list))
    {
      rrcmeas_mcf_struct_type rrcmeas_mcf;
      /* Send Meas Ctrl Failure Msg */
...
)
{
  ...
  else
  {
    ...
    /*Derive if the measurement type is of LTE IRAT based on the stored measurement ID list*/
    /* Store meas in MEASUREMENT_IDENTITY List */
    if(rrcirat_is_meas_id_eutra_meas(wrrc_as_id,(uint16) meas_id))
    {
      interrat_type = RRCMEAS_IRAT_EUTRA;
    }
    if((rrcnv_data.rrc_wtol_cm_support_nv == FALSE) && (interrat_type == RRCMEAS_IRAT_EUTRA))
    ...
    rrc_MeasurementCommand_r9_setup))
  {
    ...
    if(RRCMEAS_IRAT_EUTRA == interrat_type)
    {
      ...
      /*Call the function passing the MCM setup structure that updates LTE info to the l1_inter_rat_meas_params*/
      status = rrcirat_process_mcm_setup_modify_eutra_r9(wrrc_as_id,rrc_mcm_ptr->measurementCommand.u.setup->u.interRATMeasurement,
                             l1_inter_rat_meas_params,transaction_id,&mcm_rpt_crit);
      rrcirat_eutra_freq_list_ptr = rrcirat_int_data.rrcirat_eutra_freq_list;
    ...
    /* Now check Additional Measurements */
    /* Process Additional Meas */
    if(rrcmeas_process_mcm_addnl_meas(wrrc_as_id,mcm_r9,transaction_id,
    ...
    for ( count = 0; count < MAX_INTER_RAT_MEAS; count++ )
    {
      ...
      if (rrcirat_ext_data.inter_rat_meas_id_list[count].meas_id == l1_inter_rat_meas_params->meas_id)
      {
        rrcirat_ext_data.inter_rat_meas_id_list[count].meas_id =  meas_id;
        rrcirat_ext_data.inter_rat_meas_id_list[count].rpt_crit = mcm_rpt_crit;
        rrcirat_ext_data.inter_rat_meas_id_list[count].interrat_type = interrat_type;
        meas_exists_for_setup = TRUE;
        break;
    ...
    if (meas_exists_for_setup == FALSE)
    {
      ...
      for ( count = 0; count < MAX_INTER_RAT_MEAS; count++ )
      {
        ...
        if (rrcirat_ext_data.inter_rat_meas_id_list[count].meas_id == INVALID_MEAS_ID)
        {
          if(meas_id_val != 0)
          {
            rrcirat_ext_data.inter_rat_meas_id_list[count].meas_id = meas_id_val;
            rrcirat_ext_data.inter_rat_meas_id_list[count].meas_id_legacy = rrc_mcm_ptr->measurementIdentity;
          }
          else
          {
            rrcirat_ext_data.inter_rat_meas_id_list[count].meas_id = rrc_mcm_ptr->measurementIdentity;
          }
          rrcirat_ext_data.inter_rat_meas_id_list[count].rpt_crit = mcm_rpt_crit;
          rrcirat_ext_data.inter_rat_meas_id_list[count].interrat_type = interrat_type;
          break;
  ...
  else if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r8_modify)
  {
      ...
      rrc_MeasurementCommand_r9_modify,measurementType)))
    {
      ...
      /* Check for additional measurements */
     if(rrcmeas_process_mcm_addnl_meas(wrrc_as_id,mcm_r9,transaction_id,&addtl_meas_incl,primary_meas_type,meas_id, &meas_validity) == FAILURE)
    {
      rrc_free(l1_inter_rat_meas_params);
      return FAILURE;
    }
    ...
    else
    {
      ...
      if(RRCMEAS_IRAT_EUTRA == interrat_type)
      {
        ...
        /*Call the function passing the MCM modify structure that updates LTE info to the l1_inter_rat_meas_params*/
        status = rrcirat_process_mcm_setup_modify_eutra_r9(wrrc_as_id,rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interRATMeasurement,
                               l1_inter_rat_meas_params,transaction_id,&mcm_rpt_crit);
        rrcirat_eutra_freq_list_ptr = rrcirat_int_data.rrcirat_eutra_freq_list;
        if(FAILURE == status)
        {
...
FUNCTION   RRCIRAT_PROCESS_MCM_R10
...
===========================================================================*/
uecomdef_status_e_type rrcirat_process_mcm_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  rrc_cmd_type *cmd_ptr,
  uint8 transaction_id
)
{
  ...
  if (RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand,
    rrc_MeasurementCommand_r10_setup))
  {
    /* Double check that it is INTER-RAT Meas */
    if (!(RRC_CHECK_COMMON_MSG_TYPE_PTR(rrc_mcm_ptr->measurementCommand.u.setup,
        rrc_MeasurementType_r9_interRATMeasurement)))
...
#endif
      (RRC_MSG_COMMON_BITMASK_IE_PTR(rrc_mcm_ptr, rrc_MeasurementControl_r10_IEs,measurementReportingMode))))
    {
      WRRC_MSG0_ERROR("SETUP:Invalid MeasQuan/ReportQuan/ReportCriteria/R.Mode");
  ...
  else
  {
    ...
    /*Validations for measurement modify*/
    if((RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand,rrc_MeasurementCommand_r10_modify)) &&
        (RRC_MSG_COMMON_BITMASK_IE_TYPE2_PTR(rrc_mcm_ptr->measurementCommand.u.modify,measurementType)))
    {
      /*
        As of now If interrat rat type is EUTRA, then, no other type should be GSM
    ...
    if((rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r10_modify) &&
        (rrc_mcm_ptr->measurementCommand.u.modify->m.measurementTypePresent))
    {
      irat_meas_cmd_ptr = rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interRATMeasurement;
      if(
  ...
  if (((RRCMEAS_IRAT_GSM == interrat_type) && (rrcmcm_is_dualmode_enabled_no_f3(wrrc_as_id) == FALSE)) 
#ifdef FEATURE_WCDMA_CM_LTE_SEARCH
       ||((RRCMEAS_IRAT_EUTRA == interrat_type) && (rrcmcm_is_lte_mode_enabled(wrrc_as_id) == FALSE))
#endif
       )
  {
    WRRC_MSG1_HIGH("REL10:IRAT:Mode not enabled, IRAT_Type %d",interrat_type);
    ...
    (rrc_mcm_ptr->dpch_CompressedModeStatusInfo.tgp_SequenceShortList.n != 0))
  {
    ...
    if (rrcmeas_process_compressed_mode_status_r10(wrrc_as_id,&rrc_mcm_ptr->dpch_CompressedModeStatusInfo,
      &l1_inter_rat_meas_params->cm_status_info) == FAILURE)
    {
      WRRC_MSG0_HIGH("Invalid Compressed Mode Status Info %d");
      /* Send Meas Ctrl Failure Message */
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(rrc_mcm_ptr, rrc_MeasurementControl_r10_IEs,
      measurementReportingMode))
  {
    /* Store reporting mode to TRUE */
    l1_inter_rat_meas_params->rpt_mode_incl = TRUE;
    ...
    rrc_MeasurementCommand_r10_setup))
  {
    ...
    /* Now check Additional Measurements */
    /* Process Additional Meas */
    if(rrcmeas_process_mcm_addnl_meas_r10(wrrc_as_id,mcm_r10,transaction_id,
       &addtl_meas_incl,primary_meas_type,meas_id,&meas_validity) == FAILURE)
    {
        rrc_free(l1_inter_rat_meas_params);
        return FAILURE;
    ...
    if (addtl_meas_incl == TRUE)
    {
      rrcmeas_check_addtl_meas_in_db_r10(wrrc_as_id,&primary_meas_type, mcm_r10,
        meas_id);
      WRRC_MSG0_HIGH("REL10:Additional Meas set in RRC DB");
    }
  ...
  else if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r10_modify)
  {
      ...
      rrc_MeasurementCommand_r10_modify,measurementType)))
    {
      ...
      /* Check for additional measurements */
     if(rrcmeas_process_mcm_addnl_meas_r10(wrrc_as_id,mcm_r10,transaction_id,&addtl_meas_incl,primary_meas_type,meas_id, &meas_validity) == FAILURE)
    {
      rrc_free(l1_inter_rat_meas_params);
      return FAILURE;
    }
    ...
    {
      if (!(RRC_CHECK_COMMON_MSG_TYPE2(rrc_mcm_ptr->measurementCommand.u.modify->measurementType,
          rrc_MeasurementType_r10,interRATMeasurement)))
      {
        WRRC_MSG0_ERROR("Modify:Meas Id is not Inter-RAT");
        /* Send Meas Ctrl Failure  */
  ...
  * that this meas exists. So let us delete it from inter_rat_meas_id_list
  ...
  if(RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand,rrc_MeasurementCommand_r10_setup))
  {
    if((RRC_MSG_COMMON_BITMASK_IE_TYPE2_PTR(rrc_mcm_ptr->measurementCommand.u.setup->u.interRATMeasurement,
        interRATMeasurementObjects)))
    {
...
FUNCTION   rrcirat_get_local_neighbor_list_ptr
...
inter_rat_cell_list_struct_type* rrcirat_get_local_neighbor_list_ptr(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcirat_int_data.local_neighbor_list;
}
/*===========================================================================

FUNCTION   rrcirat_get_eutra_freq_list_ptr
...
rrcirat_eutra_freq_type* rrcirat_get_eutra_freq_list_ptr(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcirat_int_data.rrcirat_eutra_freq_list;
}
