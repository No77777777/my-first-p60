...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcbmc.c#2 $    $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
  
 when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/04/16   sn      f3 density reduction changes
10/23/15   as      Made changes to send BMC stop ind on W stop
...
LOCAL boolean oos_indicator_sim[MAX_RRC_STACKS];
#define oos_indicator oos_indicator_sim[WAS_ID_M]
LOCAL boolean indicate_bmc_on_wcdma_stop_sim[MAX_RRC_STACKS];
#define indicate_bmc_on_wcdma_stop indicate_bmc_on_wcdma_stop_sim[WAS_ID_M]

#else
LOCAL boolean oos_indicator;
...
extern boolean rrcllc_get_ctch_enabled_status(sys_modem_as_id_e_type wrrc_as_id);
extern uint32 rrcllc_get_ctch_rlc_size(sys_modem_as_id_e_type wrrc_as_id);
extern void rrcllc_get_ctch_drx_info(sys_modem_as_id_e_type wrrc_as_id, uint8 *frame_offset, uint16 *alloc_period);
extern void rrcllc_get_bmc_schedul_info(sys_modem_as_id_e_type wrrc_as_id, uint16 *schedul_msg_period);
...
===========================================================================*/
static void rrcbmc_send_cb_info_to_cm(sys_modem_as_id_e_type wrrc_as_id, boolean ctch_is_supported)
{
  ...
  if ((cmd_ptr = cm_get_cmd_buf()) != NULL)
  {
    ...
    cmd_ptr->cmd.cell_bc_info.subscription_id = rrc_get_as_id(wrrc_as_id);
#endif
    ...
    WRRC_MSG1_MED("Tx CM_RRC_CB_INFO_IND to CM %d ",ctch_is_supported);
...
===========================================================================*/
void rrcbmc_send_rrc_new_cell_ind_message(sys_modem_as_id_e_type wrrc_as_id) 
{
...
#ifdef FEATURE_DUAL_SIM
  rrcbmc_cell_change_ind.as_id = rrc_get_as_id(wrrc_as_id);
#endif
  ...
  if ( !(oos_indicator)  && (RRC_CELL_LOSS_NO_NEW_CELL != rrc_ccm_get_serv_plmn_cellid(wrrc_as_id, &rrcbmc_cell_change_ind.cell_id,
  ...
  {
    ctch_present = rrcllc_get_ctch_enabled_status(wrrc_as_id);
                         ...
                         || curr_state == RRC_STATE_URA_PCH  ))
    {
      ...
      if (SUCCESS == rrc_get_lac_id(wrrc_as_id, &lai))
      {
      ...
      rrcllc_get_ctch_drx_info(wrrc_as_id, &rrcbmc_cell_change_ind.rrcbmc_ctch_info.cbs_frame_offset,
                               &rrcbmc_cell_change_ind.rrcbmc_ctch_info.ctch_allocation_period);

      rrcbmc_cell_change_ind.rrcbmc_ctch_info.rlc_lc_id = rrclcm_check_dl_rlc_lc_id(wrrc_as_id, UE_LOGCHAN_CTCH,
                                                                                    CTCH_RADIO_BEARER_ID,
                                                                                    UE_MODE_UNACKNOWLEDGED);

      rrcbmc_cell_change_ind.rrcbmc_ctch_info.rlc_pdu_size = (uint16) rrcllc_get_ctch_rlc_size(wrrc_as_id);
      /* ctch_rf_per_tti is not used yet... so just a default value for now */
      rrcbmc_cell_change_ind.rrcbmc_ctch_info.ctch_rf_per_tti = 1;

      rrcllc_get_bmc_schedul_info(wrrc_as_id, &rrcbmc_cell_change_ind.rrcbmc_ctch_info.bmc_schedul_msg_period);

      WRRC_MSG5_HIGH("CBS DRX info N = %d K = %d RLC size = %d RCL lc_id = %d Current cell id is %d",
...
#ifdef FEATURE_CM_CELL_BROADCAST
  rrcbmc_send_cb_info_to_cm(wrrc_as_id, rrcbmc_cell_change_ind.ctch_is_supported);
#endif

  /* All inforamtion necessary have been filled out.  Send cell_change_ind to BMC */
  WRRC_MSG0_MED("Sending RRC_CELL_CHANGE_IND to BMC");
...
)
{
  ...
  rrc_etws_prim_notification_ind.plmn_id = rrc_ccm_get_curr_camped_plmn(wrrc_as_id);

#ifdef FEATURE_DUAL_SIM
  rrc_etws_prim_notification_ind.as_id = rrc_get_as_id(wrrc_as_id);
#endif
  ...
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    ...
    case RRC_DOWNLINK_SDU_IND:
      ...
      if(dcch_msg_ptr != NULL)
      {
        ...
        rrcbmc_send_etws_information_ind_message(wrrc_as_id, dcch_msg_ptr->message.u.etwsPrimaryNotificationWithSecurity.
...
)
{
  ...
  rrcscmgr_register_for_scn(wrrc_as_id, RRC_PROCEDURE_BMC,
