...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcchk.c#2 $    $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
LOCAL rrc_RRC_TransactionIdentifier chk_transaction_id_sim[MAX_RRC_STACKS]; /* Variable to hold the transaction id */
#define chk_transaction_id chk_transaction_id_sim[WAS_ID_M]
#else
LOCAL rrc_RRC_TransactionIdentifier chk_transaction_id; /* Variable to hold the transaction id */
#endif
...
)
{
  ...
  if(RRCSEND_SUCCESS == rrcsend_send_ul_sdu(wrrc_as_id,
                                            RRC_PROCEDURE_CHK,
                                            msg,
                                            lc_id,
  ...
  {
    WRRC_ERR0_FATAL("Failed to send Counter Check Response");
  }
}
...
{ 
  ...
  rrc_crash_debug_save_proc_substates(wrrc_as_id, RRC_PROCEDURE_CHK,RRC_INVALID_PROC_SUBSTATE,cmd_ptr->cmd_hdr.cmd_id);
  
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_DOWNLINK_SDU_IND: 
      if (TRUE == rrcchk_process_counter_check_message(wrrc_as_id, cmd_ptr)) 
      {
        rrcchk_send_counter_check_response(wrrc_as_id);
      }
      break;
...
{
  rrcscmgr_register_for_scn(wrrc_as_id, RRC_PROCEDURE_CHK,
                            RRC_STATE_WILDCARD,
