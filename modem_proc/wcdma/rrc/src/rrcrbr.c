...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcrbr.c#4 $   

when       who     what, where, why
--------   ---     ---------------------------------------------------------
9/20/24    mahens  Made changes to fix KW error
...
11/03/17   ak     Made changes to clear toc usage change flag
...
10/08/15   sp      Made changes to trigger OOS if L2 ack for transition to PCH is received when inter freq resel is in progress
09/10/15   sp      Made changes to populate UL/DL Rb rates to PDCP
09/08/15   nr      Fix KW errors
...
typedef struct
{
  ...
  boolean rrcrbr_send_initiate_cu_flag;
  boolean rrcrbr_delete_crnti;
...
LOCAL rrcrbr_int_data_type rbr_sim[MAX_RRC_STACKS];
#define rbr rbr_sim[WAS_ID_M]
#else
...
{
  WRRC_MSG3_HIGH("Domain CS status %d (0:NOT open, 1:Open)/PS status %d (0:NOT open, 1:Open) for SCRI:%d(0- CS domain,1-PS domain)",rrcdt_is_cs_domain_open(wrrc_as_id),rrcdt_is_ps_domain_open(wrrc_as_id),signallingConnectionRelIndication);

  if (rrc_CN_DomainIdentity_cs_domain == signallingConnectionRelIndication)
  {
      if(rrcdt_is_cs_domain_open(wrrc_as_id))
      {
  ...
  {
      if(rrcdt_is_ps_domain_open(wrrc_as_id))
      {
        /* Need to release a CN domain */
        rbr.need_to_release_cn_domain = TRUE; 
      }
...
===========================================================================*/
static void rrcrbr_indicate_layer1_if_config_msg_release_voice_rab(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  if ((TRUE == rbr.rbs_tobe_released.is_valid) && 
       (TRUE == rrc_get_rb_id_list_associated_with_voice_rab(wrrc_as_id,&num_rb, rb_id_list)))
  {
    temp_num_rb = num_rb;
    ...
    if (0 == temp_num_rb)
    {
      rrc_indicate_layer1_next_config_will_release_voice_rab(wrrc_as_id);
    }
  }
}
/*===========================================================================
...
#endif
rrcrb_substate_e_type rrcrbr_get_current_sub_state(sys_modem_as_id_e_type wrrc_as_id)
{
...
{
  return rbr.rrcrbr_send_initiate_cu_flag;
}
/*===========================================================================
...
===========================================================================*/
boolean rrcrbr_is_proc_successful(sys_modem_as_id_e_type wrrc_as_id)
{
...
===========================================================================*/
void rrcrbr_update_failure_status(sys_modem_as_id_e_type wrrc_as_id,rrcrb_msg_status_e_type status )
{
...
)
{
  ...
  (void)rrcsmc_append_start_list(wrrc_as_id,&msg_ptr->message.u.radioBearerReleaseComplete.ul_CounterSynchronisationInfo.startList,
...
)
{
  ...
  /*Reports Radio bearer release failure to QSH*/
  rrc_qsh_report_failure_ota_msg_event(wrrc_as_id,WRRC_UL_DCCH_MESSAGETYPE_RADIOBEARERRELEASEFAILURE);
#endif

#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
  rrcqsh_log_uplink_dcch_ota_params(wrrc_as_id, WRRC_UL_DCCH_MESSAGETYPE_RADIOBEARERRELEASEFAILURE, ptr);
#endif
  ...
  if (ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
    ...
    {
      rrc_update_reconfig_waiting_for_l2ack_dch(wrrc_as_id, RRC_PROCEDURE_RBR);
      status = rrcsend_send_ul_sdu(wrrc_as_id,RRC_PROCEDURE_RBR,  /* Procedure is RBR    */
...
)
{
  ...
  if (srns_reloc == TRUE)
  {
    rrcrbr_append_start_list_to_rbr_complete(wrrc_as_id, ptr);
  }
  if((cipher_update_required == TRUE) && (TRUE == rrcsmc_tm_ciph_active(wrrc_as_id)))
  {
...
===========================================================================*/     
static void send_rbr_complete_message(sys_modem_as_id_e_type wrrc_as_id, boolean proc_wait_for_l2_ack )
{
  ...
  else
  {
    if (rrcllc_get_rlc_size_change_status(wrrc_as_id) 
      && ((rbr.state_indicated_by_msg == RRC_STATE_CELL_PCH || 
          ...
          || (rrc_get_rlc_size_change_status_for_procedure_id(wrrc_as_id,RRC_PROCEDURE_RBR) == TRUE)))
    {
      ...
      else if (rrc_get_rlc_size_change_status_for_procedure_id(wrrc_as_id,RRC_PROCEDURE_RBR) == TRUE)
      {
     ...
     (void)rrcsmc_append_start_list(wrrc_as_id,&ptr->message.u.radioBearerReleaseComplete.ul_CounterSynchronisationInfo.startList,
                           RRCSMC_APPEND_START);
  ...
  if ( ((rbr.hho_ciph_status == TRUE) || (rbr.trans_to_dch_ciph_config_needed == TRUE)) &&
       (TRUE == rrcsmc_tm_ciph_active(wrrc_as_id)))
  {
    if(! (RRC_MSG_COMMON_BITMASK_IE(ptr->message.u.radioBearerReleaseComplete,
           rrc_RadioBearerReleaseComplete,ul_CounterSynchronisationInfo)))
    {
  ...
  if (ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
    ...
    (void) rex_set_timer(&rbr.rrcrbr_temp_timer, 5000);   /* 5 seconds */

    rbr.rrcrbr_sig = rex_wait(wrrc_as_id, RRCRBR_TEMP_TIMER_SIG);
    MSG_LOW("RRCRBR signal:%d", rbr.rrcrbr_sig, 0, 0);
    (void) rex_clr_sigs(rex_self(), RRCRBR_TEMP_TIMER_SIG);
#endif /* FEATURE_RRC_DELAY_TX_ON_NEW_CFG */
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
    rrcqsh_log_uplink_dcch_ota_params(wrrc_as_id, WRRC_UL_DCCH_MESSAGETYPE_RADIOBEARERRELEASECOMPLETE, ptr);  
#endif
...
/* 
   ...
   For transition to PCH state, CLOSE_SESS_IND will be sent in rrcrbr_process_successful_proc_completion(sys_modem_as_id_e_type wrrc_as_id) so that
...
*/
      ...
      && (rbr.rbr_for_srns_relocation != TRUE))
  {
    if (rbr.need_to_release_cn_domain == TRUE)
    {
      ...
      {      
        (void)rrcscr_release_signalling_connection(wrrc_as_id, RRC_CS_DOMAIN_CN_ID);
      }
      else
      {
        (void)rrcscr_release_signalling_connection(wrrc_as_id, RRC_PS_DOMAIN_CN_ID);      
      }
...
)
{
  ...
  {
    rrc_RadioBearerRelease_r10_IEs *r10_rb_rel_msg_ptr  =
    &msg_ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.radioBearerRelease_r10;
  
    if(RRCRB_R10_MSG_IE_PRESENT(r10_rb_rel_msg_ptr,dl_InformationPerRL_List)) 	 
    {
      if(r10_rb_rel_msg_ptr->dl_InformationPerRL_List.elem[0].modeSpecificInfo.t ==
      T_rrc_DL_InformationPerRL_r8_modeSpecificInfo_fdd)
      { 
        rrcrbr_psc = 
                 r10_rb_rel_msg_ptr->dl_InformationPerRL_List.elem[0].modeSpecificInfo.
  ...
  WRRC_MSG3_HIGH("RRCRBR Received PSC %d result:%d (0-Not present, 1-Present)for msg_version:%d(0-MSG_REL99,1-MSG_REL5,2-MSG_REL6,3-MSG_REL7,4-MSG_REL8,5-MSG_REL9,6-MSG_REL10,other-MSG_INVALID)",rrcrbr_psc,result,msg_version);
  return result;
...
if (msg_version == MSG_REL9)
{
  ...
  if (msg_version == MSG_REL10)
  {
    ...
    criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.radioBearerRelease_r10),
    dl_CounterSynchronisationInfo))
    { 
      return TRUE; 
    }
...
)
{
...
#endif/* FEATURE_SCUDIF_ENHANCEMENT*/
  {
    ...
    if(rbr.rabs.rabs_in_msg[rab_idx].rab_type == RRCRB_CS_VOICE_RAB)
    {
      ...
      rrcrb_update_amr_cctrch_id_for_reconfig(wrrc_as_id,
    ...
    if(mm_ptr->cmd.rrc_sync_ind.rab_info.cs_rab_type == RRCMN_CS_VOICE_CALL)
    {
      ...
      if(rbr.rabs.rabs_in_msg[rab_idx].nas_Synchronisation_Indicator == WCDMA_IVOCODER_ID_AMRWB)
      {
...
)
{
  ...
  if(rab_list_ptr != NULL)
  {
    ...
    {
      rab_id = 
          rrcrb_translate_gsm_map_rab_id(wrrc_as_id,(rab_list_ptr->elem[rab_index].rab_Identity.u.gsm_MAP_RAB_Identity));
      ...
      if(rab_list_ptr->elem[rab_index].cn_DomainIdentity == rrc_CN_DomainIdentity_cs_domain )
      {
        ...
        else
        {
          ...
          WRRC_MSG3_HIGH("For RAB-Id %d, RAB type from NAS = %d, RAB type from EST_RABs = %d",rab_id, rab_type,
            rrc_get_rab_type_for_given_rab_id(wrrc_as_id,rbr.rabs.rabs_in_msg[rab_index].rab_id));
          ...
          {
            nas_sync_ind =  rrc_translate_nas_sync_indicator(wrrc_as_id,&(rab_list_ptr->elem[rab_index].nas_Synchronisation_Indicator));
...
)
{
  ...
  if ( RRCRBR_R7_MSG_IE_PRESENT(rb_release_ptr,rrc_RadioBearerRelease_r7_IEs,dl_HSPDSCH_Information))
  {
    ...
    if (rrc_hsdpa_initial_validation(wrrc_as_id,rb_release_ptr->rrc_StateIndicator, 
  ...
  if(RRCRBR_R7_MSG_IE_PRESENT(rb_release_ptr,rrc_RadioBearerRelease_r7_IEs,rb_InformationReconfigList))
  {
    ...
    {
      if(rrcrbr_update_rab_info_reconfiglist(wrrc_as_id,&rb_release_ptr->rab_InformationReconfigList)
         == FAILURE)
      {
        rbr.status.failure_status=RRCRB_MSG_UNSUPPORTED_CONFIG;
        return;
    ...
    else
    {
      ...
      rrc_set_ordered_codec_mode(wrrc_as_id,rrcdata_codec_info.rrc_active_codec);
  ...
  if (RRCRBR_R7_MSG_IE_PRESENT(rb_release_ptr,rrc_RadioBearerRelease_r7_IEs,dl_CounterSynchronisationInfo))
  {
      ...
      ( rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH)))
    {
      WRRC_MSG2_ERROR("SRNS reloc with invalid states cur_state %d, msg_state %d",
        rrc_get_state(wrrc_as_id), rbr.state_indicated_by_msg);
      rbr.status.failure_status=RRCRB_MSG_INVALID_CONFIG;
      return;
    }
    ...
    else
    {
      ...
      if ( RRCRBR_R7_MSG_IE_PRESENT(rb_release_ptr,rrc_RadioBearerRelease_r7_IEs,new_U_RNTI))
      {
        ...
        rrc_translate_urnti (wrrc_as_id, &(rb_release_ptr->new_U_RNTI),
          &rbr.new_urnti);
      }
    }
   ...
   */
  if((rbr.state_indicated_by_msg == RRC_STATE_CELL_DCH) && ( rrc_get_state(wrrc_as_id) != RRC_STATE_CELL_DCH))
  {
    ...
    (rbr.state_indicated_by_msg == RRC_STATE_URA_PCH))
  {
    ...
    if (rbr.state_indicated_by_msg == RRC_STATE_URA_PCH)
    {
      ...
      else
      {
        ...
        {
          rrccu_update_ura_id (wrrc_as_id,&(rb_release_ptr->ura_Identity));
        }
        else
        {
...
)
{
  ...
  {
    /* Check if network does not try to initiate HSDPA with invalid state */
    if (rrc_hsdpa_initial_validation_r6(wrrc_as_id,rb_release_ptr->rrc_StateIndicator, 
...
)
{
     ...
     )
  {
    ...
    if(rrcrbr_update_rab_info_reconfiglist(wrrc_as_id, &msg_ptr->u.r3.radioBearerRelease_r3.rab_InformationReconfigList)
         == FAILURE)
    {
      return;
    }
  ...
  /*check whether the SRNS relocation is within Cell_DCH or not.
    ...
    else
    {
      ...
      {
        rbr.new_urnti_valid = TRUE;
        rrc_translate_urnti (wrrc_as_id, &(msg_ptr->u.r3.radioBearerRelease_r3.new_U_RNTI),
                              &rbr.new_urnti);
      }
    }
  } /* dl counter sync info */
      ...
      (rbr.state_indicated_by_msg == RRC_STATE_URA_PCH))
  {
     ...
     if (rbr.state_indicated_by_msg == RRC_STATE_URA_PCH)
     {
       ...
       else
       {
         ...
         {
           rrccu_update_ura_id (wrrc_as_id,&(msg_ptr->u.r3.radioBearerRelease_r3.ura_Identity));
         }
         else
         {
           WRRC_MSG1_ERROR("Invalid URA ID length%d",
    ...
    */
    ...
    if( rbr.rbs_tobe_released.is_valid == TRUE )
    {
      for(rb_count=0; ((rb_count<rbr.rbs_tobe_released.num_rbs)&&(rb_index < MAX_RB)); rb_count++)
      {
        ...
        if(MAX_RB == rrcllc_check_if_rbid_present_in_chan_config_within_index(wrrc_as_id,
    ...
    if( (rrc_get_state(wrrc_as_id)) == (rbr.state_indicated_by_msg) )
    {
    ...
    if(rb_index < MAX_RB)
    {
      rrc_fill_rb_id_chan_req_from_standalone_db(wrrc_as_id,&rb_index, &ptr->cmd.chan_config_req);
    }
    ...
    if((rbr.state_indicated_by_msg == RRC_STATE_CELL_DCH)&&(rb_index < MAX_RB))
    {
      ...
      rrc_fill_rb_id_chan_req_from_rb_list_mapped_to_dch(wrrc_as_id,&rb_index, &ptr->cmd.chan_config_req);
      if((rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH)&&(rb_index < MAX_RB))
      {
        rrc_fill_srb_in_chan_config_req(wrrc_as_id,&rb_index, &ptr->cmd.chan_config_req);
      }
    }
    ...
    {
      rrc_fill_rb_id_chan_req_from_est_rabs(wrrc_as_id,&rb_index, &ptr->cmd.chan_config_req);
    }
    ...
    WRRC_MSG3_HIGH("CHANNEL_CONFIG_REQ RBs:%d, curr_st:%d, next_st:%d", 
             rb_index,rrc_get_state(wrrc_as_id),rbr.state_indicated_by_msg);
...
===========================================================================*/
static void rrcrbr_clear_procedure(sys_modem_as_id_e_type wrrc_as_id)
{
  uint32 count;
  ...
  rbr.rrcrbr_send_initiate_cu_flag = FALSE;
  rbr.rrcrbr_delete_crnti          = FALSE;
  ...
  for ( count=0; count < RRCRB_MAX_RAB_PER_MSG; count++ )
  {
    ...
    rbr.rabs.rabs_in_msg[count].nas_Synchronisation_Indicator = WCDMA_IVOCODER_ID_NONE;
  }
  rbr.rabs.num_rabs_in_msg = 0;
  ...
  rrc_common_procedure_clear(wrrc_as_id,RRC_PROCEDURE_RBR);
...
)
{
  ...
  if (rbr.cn_info_saved != FALSE)
  {
    ...
    if(rbr.mm_cmd_ptr->cmd.hdr.message_id == (byte) RRC_SERVICE_IND_FROM_OTA_MSG )
    {
      ...
      rrc_ccm_update_service_info(wrrc_as_id, rbr.mm_cmd_ptr);
    }
    /* Put the command on MM queue */
    rrc_send_rrc_message_to_mm(wrrc_as_id, rbr.mm_cmd_ptr);
    /* clear the flag */
    rbr.cn_info_saved = FALSE;
  }
      ...
      (indicate_rlc == TRUE))
  {
    ...
    rrcrb_register_with_rlc_for_ul_data_ind (wrrc_as_id, RRC_PROCEDURE_RBR);
  ...
  */
  ...
  rrcrbr_clear_procedure(wrrc_as_id);
  if (rrccu_return_cu_substate(wrrc_as_id) == RRCCU_WAIT_FOR_RB_PROC_COMPL_IND) 
  {
    rrcrb_send_rb_proc_compl_ind(wrrc_as_id);
  }

  rrccu_reset_registration_info_for_procedure(wrrc_as_id,RRC_PROCEDURE_RBR);
}    /* rrcrbr_process_successful_proc_completion */
...
)
{
  ...
  if (TRUE == rrcrbr_validate_srns_relocation_info(wrrc_as_id,msg_ptr, msg_version))
  {
      ...
      case RRCSMC_INCOMPATIBLE_SECURITY_RECONFIGURATION:
        ...
        else
        {
               ...
               (RRCCU_STARTED == rrccu_get_cell_update_started_status(wrrc_as_id) ))
          {
            ...
            {
              send_rbr_failure_message( wrrc_as_id, rbr.status.failure_status,
            ...
            else
            {
              ...
              (void)rrccu_register_for_cell_update_to_complete(wrrc_as_id, RRC_PROCEDURE_RBR);
  ...
  if ((rrcllc_get_ordered_config_status(wrrc_as_id) != OC_NOT_SET) ||
      ...
      (RRCCU_STARTED == rrccu_get_cell_update_started_status(wrrc_as_id) ))
  {
     WRRC_MSG3_HIGH("Incomatible simultaneous reconfig! Rejecting RBR message,transaction_id:%d,accepted_transaction:%d",
     ...
     return;
          ...
          &&
         (rrc_buffer_ota_info.procedure == RRC_PROCEDURE_RBR)
          &&
  ...
  {
    WRRC_MSG0_ERROR("BUF_OTA:Received Duplicate RBR, Ignoring");
    return ;
  }
#endif
  ...
  message */
  validate_rbrelease_r6(wrrc_as_id, msg_ptr);
  ...
  else
  {
    ...
    rrcrbr_process_high_mobility_ind_in_non_critical_ext(wrrc_as_id,msg_ptr,MSG_REL6);
    ...
    while ( rb_info_rel_list_ptr->n >  local_rb_idx)
    {
      ...
      if((rrclcm_get_dl_lc_type(wrrc_as_id,rrclcm_get_dl_rlc_lc_id(wrrc_as_id,rb_info_rel_list_ptr->elem[local_rb_idx]), &dl_sdu_type)) == RRCLCM_SUCCESS)
      {
      ...
      { 
        ...
        else if (!rrc_find_rb_in_est_rabs(wrrc_as_id,rb_info_rel_list_ptr->elem[local_rb_idx]))
        {
          WRRC_MSG1_HIGH("Ignore Non-exitent rb_id =%d in rbr msg",rb_info_rel_list_ptr->elem[local_rb_idx]);
          local_rb_idx++;
          continue;
          ...
          rrc_RadioBearerRelease_r6_IEs,rb_InformationReconfigList))
    {
      ...
      while ( rb_release_ptr->rb_InformationReconfigList.n >localrb_count )
      {
        ...
        {
          WRRC_MSG3_HIGH("rb-reconfig-info rb %d to %d[0:STOP/1:CONTINUE rb_count %d", 
          ...
          rrc_RadioBearerRelease_r6_IEs,rb_InformationAffectedList))
    {
      ...
      while ( rb_info_affected_list_ptr->n > localrb_count)
      {
        ...
        if(( rb_info_affected_list_ptr->elem[localrb_count].rb_Identity > DCCH_DT_LOW_PRI_RADIO_BEARER_ID)&&
           (! rrc_find_rb_in_est_rabs(wrrc_as_id,rb_info_affected_list_ptr->elem[localrb_count].rb_Identity)))
        {
          /* RB Message was invalid. */
    ...
    {
      rrcrbr_mark_cn_doamin_to_be_closed(wrrc_as_id,rb_release_ptr->signallingConnectionRelIndication);
      ...
      for ( rab_count = 0; rab_count< MAX_RAB_TO_SETUP; rab_count++ )
      {
        ...
        {
          rab_found = TRUE;
          for ( rb_count=0;((rb_count<rrc_est_rabs.rabs[rab_count].num_rbs_for_rab) && (rb_count<MAX_RB_PER_RAB)); rb_count++ )
          {
            rb_rel_srch = FALSE;
    ...
    if (RRC_MSG_COMMON_BITMASK_IE_PTR(rb_release_ptr, rrc_RadioBearerRelease_r6_IEs,cn_InformationInfo))
    {
      ...
      rbr.mm_cmd_ptr = rrc_ccm_buid_service_ind_with_nas_cn_info_r6(wrrc_as_id,
    ...
    rrcrbr_indicate_layer1_if_config_msg_release_voice_rab(wrrc_as_id);

    rrcrb_update_mac_rab_status(wrrc_as_id,RRC_PROCEDURE_RBR, RRC_CS_DOMAIN_CN_ID,
                                &rb_release_ptr->rb_InformationReleaseList,
                                FALSE);

    /* Set ORDERED_CONFIG so that LLC can process the
    necessary parameters. */
    oc_status = rrcllc_set_ordered_config(wrrc_as_id, RRC_PROCEDURE_RBR,
    ...
    switch ( oc_status )
    {
      case ORDERED_CONFIG_SET:
        ...
        current_state = rrc_get_state(wrrc_as_id); 
               ...
               ||(rbr.state_indicated_by_msg==RRC_STATE_URA_PCH)))
        {
          ...
          if (rrcrbr_is_pcpich_info_present(wrrc_as_id,msg_ptr,MSG_REL6))
          {
          ...
          if((rrcrb_validate_directed_cell_info (wrrc_as_id,directed_cell_info) == FALSE) && 
            (rrccsp_check_initiate_cell_selection_handling(wrrc_as_id,&directed_cell_info) == FALSE))
          {
            ...
            {
              rbr.rrcrbr_send_initiate_cu_flag = TRUE;
              rbr.rrcrbr_delete_crnti          = TRUE;
            }
            else
            {
            ...
            )
        {
             ...
             (rbr.state_indicated_by_msg==RRC_STATE_CELL_FACH))
          {
                ...
                &&(rbr.rrcrbr_send_initiate_cu_flag == FALSE))
            {
                 ...
                 rrc_RadioBearerRelease_r6_IEs,new_C_RNTI))
              {
                ...
                rrc_translate_crnti(wrrc_as_id,&rb_release_ptr->new_C_RNTI, 
                                    &c_rnti);
              ...
              rrcrb_send_initiate_cell_selection_req(wrrc_as_id, RRC_PROCEDURE_RBR,
          ...
          rrcrbr_send_channel_config_req(wrrc_as_id);
                 ...
                 (rbr.state_indicated_by_msg==RRC_STATE_CELL_PCH))
        {
          /*store the directed cell info */
           rbr.directed_cell_info = rrcrbr_is_pcpich_info_present(wrrc_as_id,msg_ptr,MSG_REL6);

           /*Send the rbr complete msg on old configuration */
           send_rbr_complete_message(wrrc_as_id,TRUE);
           /*wait for L2ack */
                 ...
                 (rbr.state_indicated_by_msg==RRC_STATE_URA_PCH)))
        {
          ...
          info  */
         // rbr.directed_cell_info = rrcrbr_is_pcpich_info_present(wrrc_as_id,msg_ptr);
      ...
      case ORDERED_CONFIG_CONFIGURATION_SET_DEFERRED:
        WRRC_MSG0_HIGH("BUF_OTA:RBR processing deferred due to LTA!");

        rbr.curr_substate = RRCRB_INITIAL;
        rrc_buffer_ota_info.transaction_id = rbr.tr_ids.accepted_transaction;
        /* Indicate that an error occured */
        ordered_config_err_occured = TRUE;
        break;
...
)
{
      ...
      (RRCCU_STARTED == rrccu_get_cell_update_started_status(wrrc_as_id) ))
  {
         ...
         (msg_ptr->u.later_than_r3.rrc_TransactionIdentifier != rbr.tr_ids.rejected_transaction))
     {
       ...
       {
         WRRC_MSG0_HIGH("Cell Update Started! Register for CU CNF");
         if (SUCCESS != rrccu_register_for_cell_update_to_complete(wrrc_as_id, RRC_PROCEDURE_RBR))
         {
  ...
  invalid or incompatible configuration given by the
  message */
  validate_rbrelease_r5(wrrc_as_id, msg_ptr);
  ...
  else
  {
    ...
    rrcrbr_process_high_mobility_ind_in_non_critical_ext(wrrc_as_id,msg_ptr,MSG_REL5);
    ...
    while ( rb_info_rel_list_ptr->n  > local_idx)
    {
      ...
      if((rrclcm_get_dl_lc_type(wrrc_as_id,rrclcm_get_dl_rlc_lc_id(wrrc_as_id, rb_info_rel_list_ptr->elem[local_idx]), &dl_sdu_type)) == RRCLCM_SUCCESS)
      {
        if(dl_sdu_type == UE_LOGCHAN_TM_DCCH_DL)
        {
          additional_srb = TRUE;
      ...
      { 
        ...
        else if(!rrc_find_rb_in_est_rabs(wrrc_as_id,rb_info_rel_list_ptr->elem[local_idx]))
        {
    ...
    if (RRC_MSG_COMMON_BITMASK_IE_PTR(rb_release_ptr, rrc_RadioBearerRelease_r5_IEs,cn_InformationInfo))
    {
       ...
       */
       primary_plmn_Id_present = rrcrbr_check_and_get_primary_plmn_id(wrrc_as_id,msg_ptr,
                                                                  &primary_plmn_Identity,MSG_REL5);
    
      rbr.mm_cmd_ptr = rrc_ccm_buid_service_ind_with_nas_cn_info(wrrc_as_id,
                     &(rb_release_ptr->cn_InformationInfo) 
      ...
      case ORDERED_CONFIG_SET:
       ...
       /* Populate the local variables to indicate if Re-direction info is present */
               ...
               ||(rbr.state_indicated_by_msg==RRC_STATE_URA_PCH)))
        {
          ...
          if (rrcrbr_is_pcpich_info_present(wrrc_as_id,msg_ptr,MSG_REL5))
          {
            ...
            )
        {
          ...
          {
             /*call the function that evaluates the directed cell info and set the flags accordingly */
            if ((rbr.rrcrbr_send_initiate_cu_flag == FALSE) &&
                (rrcrb_validate_directed_cell_info (wrrc_as_id,directed_cell_info)== FALSE))
            {
              if(RRC_MSG_COMMON_BITMASK_IE_PTR(rb_release_ptr,
        ...
        {
          /*store the directed cell info */
           rbr.directed_cell_info = rrcrbr_is_pcpich_info_present(wrrc_as_id,msg_ptr,MSG_REL5);
...
)
{
  ...
  invalid or incompatible configuration given by the
  message */
  validate_rbrelease_r7(wrrc_as_id, msg_ptr);
  ...
  else
  {
    ...
    while ( rb_info_rel_list_ptr->n > localrb_count)
    {
      ...
      else if (!rrc_find_rb_in_est_rabs(wrrc_as_id,rb_info_rel_list_ptr->elem[localrb_count]))
      {
        WRRC_MSG1_HIGH("Ignore Non-exitent rb_id =%d in rbr msg",rb_info_rel_list_ptr->elem[localrb_count]);
        localrb_count++;
        continue;
      ...
      case ORDERED_CONFIG_SET:
       ...
       /* Populate the local variables to indicate if Re-direction info is present */
               ...
               ||(rbr.state_indicated_by_msg==RRC_STATE_URA_PCH)))
        {
          ...
          if (rrcrbr_is_pcpich_info_present(wrrc_as_id,msg_ptr,MSG_REL7))
          {
        ...
        {
          /*store the directed cell info */
           rbr.directed_cell_info = rrcrbr_is_pcpich_info_present(wrrc_as_id,msg_ptr,MSG_REL7);
...
)
{
  ...
  if(rab_list_ptr != NULL)
  {
    for (rab_index=0; (rab_list_ptr->n > rab_index) && (rab_index < RRCRB_MAX_RAB_PER_MSG); rab_index++)
    {
      rab_id = rrcrb_translate_gsm_map_rab_id(wrrc_as_id,(rab_list_ptr->elem[rab_index].rab_Identity.u.gsm_MAP_RAB_Identity));
...
)
{
  ...
  {
    /* Save the linked list head pointer */
    if (FAILURE == rrcllc_validate_rb_info_reconfig_list_r8(wrrc_as_id,&rb_release_ptr->rb_InformationReconfigList))
    {
      rbr.status.failure_status = RRCRB_MSG_INVALID_CONFIG;
      return;
    }
    ...
    {
      if(rrcrbr_update_rab_info_reconfiglist_r8(wrrc_as_id,&rb_release_ptr->rab_InformationReconfigList) == FAILURE)
      {
        rbr.status.failure_status=RRCRB_MSG_UNSUPPORTED_CONFIG;
        return;
      }
    ...
    dl_CounterSynchronisationInfo))
  {
    ...
    else
    {
      ...
      {
        rbr.new_urnti_valid = TRUE;
        rrc_translate_urnti (wrrc_as_id, &(rb_release_ptr->new_U_RNTI), &rbr.new_urnti);
      }
    }
  } /* dl counter sync info */
  ...
  if((rbr.state_indicated_by_msg == RRC_STATE_CELL_DCH) && ( rrc_get_state(wrrc_as_id) != RRC_STATE_CELL_DCH))
  {
      ...
      T_rrc_DL_CommonInformation_r8_dl_dpchInfoCommon_dl_DPCH_InfoCommon))
    {
      ...
      if(rrcsmc_tm_ciph_active(wrrc_as_id) == TRUE)
      {
        ...
        loc_ret_val = rrcrb_get_mac_d_hfn(wrrc_as_id,( RRC_MSG_COMMON_BITMASK_IE_PTR(rb_release_ptr->dl_CommonInformation.dl_dpchInfoCommon.u.dl_DPCH_InfoCommon,
			  rrc_DL_DPCH_InfoCommon_r6,mac_d_HFN_initial_value)),
             &rb_release_ptr->dl_CommonInformation.dl_dpchInfoCommon.u.dl_DPCH_InfoCommon->mac_d_HFN_initial_value,
             &mac_d_hfn_l);
...
)
{
    ...
    ||(rbr.state_indicated_by_msg==RRC_STATE_CELL_PCH) ||(rbr.state_indicated_by_msg==RRC_STATE_URA_PCH)))
  {
    ...
    if(rrcrbr_is_pcpich_info_present(wrrc_as_id,msg_ptr,MSG_REL8))
    {
      if((RRCRB_R8_MSG_IE_PRESENT(rb_release_ptr,rrc_RadioBearerRelease_r8_IEs,
    ...
    (rbr.state_indicated_by_msg==RRC_STATE_CELL_DCH)))
  {
    if((current_state==RRC_STATE_CELL_FACH) && (rbr.state_indicated_by_msg==RRC_STATE_CELL_FACH))
    {
          ...
          (rrcrb_validate_directed_cell_info (wrrc_as_id,directed_cell_info)== FALSE))
      {
        ...
        {
          rbr.new_crnti_valid = TRUE;
          rrc_translate_crnti(wrrc_as_id,&rb_release_ptr->new_C_RNTI, &c_rnti);
          rbr.new_crnti = (uint16)c_rnti;
        }
        else
        {
        ...
        rrcllcoc_update_toc_with_oc_from_fach(wrrc_as_id);
        /*Initiate the cell selection*/
        rrcrb_send_initiate_cell_selection_req(wrrc_as_id, RRC_PROCEDURE_RBR, RRC_TRANSITION_FROM_FACH,
                                                 rbr.state_indicated_by_msg);
  ...
  else if((current_state==RRC_STATE_CELL_DCH) && (rbr.state_indicated_by_msg==RRC_STATE_CELL_FACH))
  {
    ...
    /*Initiate the cell selection*/
    rrcrb_send_initiate_cell_selection_req(wrrc_as_id, RRC_PROCEDURE_RBR, RRC_TRANSITION_FROM_DCH,
                                            RRC_STATE_CELL_FACH);
    rbr.curr_substate = RRCRB_WAIT_FOR_CELL_SELECTION_CNF;
  ...
  {
    /*store the directed cell info */
     rbr.directed_cell_info = rrcrbr_is_pcpich_info_present(wrrc_as_id,msg_ptr,MSG_REL8);
    ...
    (rbr.state_indicated_by_msg==RRC_STATE_URA_PCH)))
  {
    ...
    if(RRCRB_R8_MSG_IE_PRESENT(rb_release_ptr,rrc_RadioBearerRelease_r8_IEs,new_C_RNTI))
    {
      ...
      if (RRCRB_R8_MSG_IE_PRESENT(rb_release_ptr,rrc_RadioBearerRelease_r8_IEs,new_U_RNTI))
      {
        rrc_translate_urnti(wrrc_as_id,&rb_release_ptr->new_U_RNTI, &u_rnti);
        rnti_update_info.urnti_action    = RRC_RNTI_UPDATE; 
        rnti_update_info.urnti           = u_rnti; 
      }
      else
...
)
{
  ...
  validate_rbrelease_r8(wrrc_as_id, msg_ptr);
  ...
  if(rbr.status.failure_status != RRCRB_MSG_VALID )
  {
    ...
    /* Send the RB Release Failure Message */
    send_rbr_failure_message( wrrc_as_id, rbr.status.failure_status, rbr.status.prot_err_cause);
  ...
  else
  {
    ...
    if(RRCRB_R8_MSG_IE_PRESENT(rb_release_ptr,rrc_RadioBearerRelease_r8_IEs,rb_InformationReconfigList))
    {
      ...
      while (rb_release_ptr->rb_InformationReconfigList.n > localrb_count )
      {
        ...
        {
          WRRC_MSG3_HIGH("REL8: RB-Id %d to %d [0:STOP/1:CONTINUE] rb_count %d", 
              rbr.rbs_tobe_reconfigured.rb_info[localrb_count].rb_id, rb_info_reconfig_ptr->rb_StopContinue,localrb_count);
          /* over-ride rb_config with STOP or continue */
          if (rrc_RB_StopContinue_stopRB == rb_info_reconfig_ptr->rb_StopContinue)
          {
    ...
    oc_status = rrcllc_set_ordered_config(wrrc_as_id, RRC_PROCEDURE_RBR, rbr.state_indicated_by_msg,
                      rrc_DL_DCCH_Message_PDU, (void *)dcch_msg_ptr);

    switch ( oc_status )
    {
      case ORDERED_CONFIG_SET:
        rrcrbr_process_after_oc_config_set_r8(wrrc_as_id,dcch_msg_ptr, rb_release_ptr);
        break;
...
)
{
    ...
    ||(rbr.state_indicated_by_msg==RRC_STATE_CELL_PCH) ||(rbr.state_indicated_by_msg==RRC_STATE_URA_PCH)))
  {
    ...
    if(rrcrbr_is_pcpich_info_present(wrrc_as_id,msg_ptr,MSG_REL9))
    {
  ...
  {
    /*store the directed cell info */
     rbr.directed_cell_info = rrcrbr_is_pcpich_info_present(wrrc_as_id,msg_ptr,MSG_REL9);
...
)
{
  ...
  /* Validate the Message to make sure there is no invalid or incompatible configuration 
  given by the message */
  validate_rbrelease_r9(wrrc_as_id, msg_ptr);
  ...
  local_idx=0;
    ...
    if(RRCRB_R9_MSG_IE_PRESENT(rb_release_ptr,rb_InformationReconfigList))
    {
      ...
      rrc_RB_InformationReconfig_r8,rb_StopContinue))
        {
          WRRC_MSG3_HIGH("REL9: RB-Id %d to %d [0:STOP/1:CONTINUE] rb_count %d", 
              rbr.rbs_tobe_reconfigured.rb_info[localrb_count].rb_id, rb_info_reconfig_ptr->rb_StopContinue,localrb_count);
          /* over-ride rb_config with STOP or continue */
          if (rrc_RB_StopContinue_stopRB == rb_info_reconfig_ptr->rb_StopContinue)
          {
    ...
    {
      case ORDERED_CONFIG_SET:
        rrcrbr_process_after_oc_config_set_r9(wrrc_as_id,dcch_msg_ptr, rb_release_ptr);
        break;
...
/*===========================================================================
FUNCTION   rrcrbr_process_after_oc_config_set_r10

DESCRIPTION
  This function processes the R10 RBR OTA after OC is set by this OTA. So if we call this function, it means
  that ORDERED_CONFIG_SET is returned while setting OC by RBR OTA.
...
SIDE EFFECTS
  None.
===========================================================================*/
static void rrcrbr_process_after_oc_config_set_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  rrc_DL_DCCH_Message *dcch_msg_ptr,
...
)
{
  ...
  if(rrc_sib_is_current_event_sib_read_in_dch(wrrc_as_id))
  {
    WRRC_MSG0_HIGH("REL10: Force Init GPS ");
    rrcsib_force_sib_event_init(wrrc_as_id, RRCSIB_ABORT_DUE_SIB_READ_IN_DCH);
  }
  ...
  if((current_state==RRC_STATE_CELL_FACH) && ((rbr.state_indicated_by_msg==RRC_STATE_CELL_FACH)
    ||(rbr.state_indicated_by_msg==RRC_STATE_CELL_PCH) ||(rbr.state_indicated_by_msg==RRC_STATE_URA_PCH)))
  {
    if((RRCRB_R10_MSG_IE_PRESENT(rb_release_ptr,frequencyInfo))
      &&(RRC_CHECK_COMMON_MSG_TYPE(rb_release_ptr->frequencyInfo.modeSpecificInfo,
      rrc_FrequencyInfo_modeSpecificInfo_fdd)))
    {
      directed_cell_info.freq_info_present = TRUE;
    ...
    if(rrcrbr_is_pcpich_info_present(wrrc_as_id,msg_ptr,MSG_REL10))
    {
      if((RRCRB_R10_MSG_IE_PRESENT(rb_release_ptr,
        dl_InformationPerRL_List)) && (rb_release_ptr->dl_InformationPerRL_List.n != 0))
      {
        directed_cell_info.pscr_present = TRUE;
        directed_cell_info.pscr = rb_release_ptr->dl_InformationPerRL_List.elem[0].
    ...
    (rbr.state_indicated_by_msg==RRC_STATE_CELL_DCH)))
  {
    if((current_state==RRC_STATE_CELL_FACH) && (rbr.state_indicated_by_msg==RRC_STATE_CELL_FACH))
    {
      ...
      if ((rbr.rrcrbr_send_initiate_cu_flag == FALSE) &&
          (rrcrb_validate_directed_cell_info (wrrc_as_id,directed_cell_info)== FALSE))
      {
        if(RRCRB_R10_MSG_IE_PRESENT(rb_release_ptr,new_C_RNTI))
        {
          rbr.new_crnti_valid = TRUE;
          rrc_translate_crnti(wrrc_as_id,&rb_release_ptr->new_C_RNTI, &c_rnti);
          rbr.new_crnti = (uint16)c_rnti;
  ...
  {
    /*store the directed cell info */
     rbr.directed_cell_info = rrcrbr_is_pcpich_info_present(wrrc_as_id,msg_ptr,MSG_REL10);
    ...
    (rbr.state_indicated_by_msg==RRC_STATE_URA_PCH)))
  {
    ...
    if(RRCRB_R10_MSG_IE_PRESENT(rb_release_ptr,new_C_RNTI))
    {
      ...
      rnti_update_info.crnti           = (uint16)c_rnti;
      /* check whether URNTI is also present in the message */
      if (RRCRB_R10_MSG_IE_PRESENT(rb_release_ptr,new_U_RNTI))
      {
        rrc_translate_urnti(wrrc_as_id,&rb_release_ptr->new_U_RNTI, &u_rnti);
        rnti_update_info.urnti_action    = RRC_RNTI_UPDATE; 
        rnti_update_info.urnti           = u_rnti; 
...
/*===========================================================================
FUNCTION   VALIDATE_RBRELEASE_R10
...
SIDE EFFECTS
  None.
===========================================================================*/
static void validate_rbrelease_r10
(
   sys_modem_as_id_e_type wrrc_as_id, 
rrc_RadioBearerRelease *msg_ptr      /* Pointer to the RB Release message */
)
{
  rrc_RadioBearerRelease_r10_IEs* rb_release_ptr = NULL;
  rbr.status.failure_status = RRCRB_MSG_VALID;

  rb_release_ptr = &msg_ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.u.criticalExtensions.
    u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.radioBearerRelease_r10;
  ...
  if(rb_release_ptr->rb_InformationReleaseList.n == 0 )
  {
    WRRC_MSG0_HIGH("REL10: rb_InformationReleaseList is absent!");
    rbr.status.failure_status = RRCRB_MSG_PROTOCOL_ERROR;
    rbr.status.prot_err_cause = rrc_ProtocolErrorCause_asn1_ViolationOrEncodingError;
    return;
  }

  if(RRCRB_R10_MSG_IE_PRESENT(rb_release_ptr,rb_InformationReconfigList))
  {
    /* Save the linked list head pointer */
    if (FAILURE == rrcllc_validate_rb_info_reconfig_list_r8(wrrc_as_id,&rb_release_ptr->rb_InformationReconfigList))
    {
    ...
    if(RRCRB_R10_MSG_IE_PRESENT(rb_release_ptr,rab_InformationReconfigList))
    {
      if(rrcrbr_update_rab_info_reconfiglist_r8(wrrc_as_id,&rb_release_ptr->rab_InformationReconfigList) == FAILURE)
      {
        rbr.status.failure_status=RRCRB_MSG_UNSUPPORTED_CONFIG;
  ...
  {
    //check to see if dl-dpch-common info is present while going to dch
    if((RRCRB_R10_MSG_IE_PRESENT(rb_release_ptr,dl_CommonInformation)) 
      && (RRC_MSG_COMMON_BITMASK_IE(rb_release_ptr->dl_CommonInformation,
      ...
      T_rrc_DL_CommonInformation_r10_dl_dpchInfoCommon_dl_DPCH_InfoCommon))
    {
      ...
      if(rrcsmc_tm_ciph_active(wrrc_as_id) == TRUE)
      {
        ...
        if(rrcsmc_process_cipher_config_for_hho(wrrc_as_id,mac_d_hfn_l) == FAILURE)
        {
          WRRC_MSG0_HIGH("REL10:Process Ciphering Config for HHO failed");
        }
        rbr.trans_to_dch_ciph_config_needed = TRUE;
      }
      else
  ...
  if ((rbr.state_indicated_by_msg == RRC_STATE_CELL_PCH) ||(rbr.state_indicated_by_msg == RRC_STATE_URA_PCH))
  {
    ...
    if (!( RRCRB_R10_MSG_IE_PRESENT(rb_release_ptr,
      utran_DRX_CycleLengthCoeff)))
    {
      WRRC_MSG1_ERROR("REL10: UTRAN DRX Coef not preset, State:%d", rbr.state_indicated_by_msg);
      rbr.status.failure_status = RRCRB_MSG_INVALID_CONFIG;
      return;
    }
    if (rbr.state_indicated_by_msg == RRC_STATE_URA_PCH)
    {
      if (!( RRCRB_R10_MSG_IE_PRESENT(rb_release_ptr,ura_Identity)))  
      {
...
/*===========================================================================
FUNCTION   RRCRBR_PROCESS_RBRELEASE_R10
...
SIDE EFFECTS
  None.
===========================================================================*/
static void rrcrbr_process_rbrelease_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  rrc_cmd_type *cmd_ptr
)
{
  ...
  /* Store Msg Ptr in a local variable */
  rb_release_ptr = &msg_ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.
    u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.radioBearerRelease_r10;
  ...
  /* Validate the Message to make sure there is no invalid or incompatible configuration 
  given by the message */
  validate_rbrelease_r10(wrrc_as_id, msg_ptr);

  if(rbr.status.failure_status != RRCRB_MSG_VALID )
  {
    /* RB Message was invalid. */
    WRRC_MSG0_HIGH("REL10: RBR Message validation failed");
  ...
  {
    WRRC_MSG0_HIGH("REL10: Processing RBR Message");

    if(RRCRB_R10_MSG_IE_PRESENT(rb_release_ptr,ueMobilityStateIndicator))
    {
      if(rb_release_ptr->ueMobilityStateIndicator == rrc_High_MobilityDetected_high_MobilityDetected)
      {
        WRRC_MSG0_HIGH("High Mobility Ind Set to TRUE in RBR.");
  ...
  local_idx=0;
    while (rb_info_rel_list_ptr->n > local_idx)
    {
      ...
      {
        /* RB Message was invalid. */
        WRRC_MSG1_HIGH("REL10: Invalid RB id:%d in RBR Message", rb_info_rel_list_ptr->elem[local_idx]);
        /* Send the RB Release Failure Message */
        rbr.status.failure_status = RRCRB_MSG_INVALID_CONFIG;
        send_rbr_failure_message( wrrc_as_id, rbr.status.failure_status, rbr.status.prot_err_cause);
      ...
      else if( rb_info_rel_list_ptr->elem[local_idx] == DCCH_DT_LOW_PRI_RADIO_BEARER_ID)
      {
        ...
        {
          WRRC_MSG1_HIGH("Rel10:Ignore Non-exitent rb_id = %d in rbr msg",rb_info_rel_list_ptr->elem[local_idx]);
          local_idx++;
          continue;
        }
      }
    ...
    WRRC_MSG1_HIGH("REL10: Number of RBs to be released:%d", localrb_count);
    ...
    if(RRCRB_R10_MSG_IE_PRESENT(rb_release_ptr,rb_InformationReconfigList))
    {
      ...
      rrc_RB_InformationReconfig_r8,rb_StopContinue))
        {
          ...
          if (rrc_RB_StopContinue_stopRB == rb_info_reconfig_ptr->rb_StopContinue)
          {
            rbr.rbs_tobe_reconfigured.rb_info[localrb_count].ul_cfg_mask |= STOP_RB_CFG;
            rbr.rbs_tobe_reconfigured.rb_info[localrb_count].dl_cfg_mask |= STOP_RB_CFG;
            WRRC_MSG2_HIGH("REL10: RB-Id %d to STOP rb_count %d",
              rbr.rbs_tobe_reconfigured.rb_info[localrb_count].rb_id, localrb_count);
          }
          else if (rrc_RB_StopContinue_continueRB == rb_info_reconfig_ptr->rb_StopContinue)
          {
            rbr.rbs_tobe_reconfigured.rb_info[localrb_count].ul_cfg_mask |= CONTINUE_RB_CFG;
            rbr.rbs_tobe_reconfigured.rb_info[localrb_count].dl_cfg_mask |= CONTINUE_RB_CFG;
            WRRC_MSG2_HIGH("REL10: RB-Id %d to CONTINUE rb_count %d",
              rbr.rbs_tobe_reconfigured.rb_info[localrb_count].rb_id, localrb_count);
          }
        }
      ...
      rbr.rbs_tobe_reconfigured.num_rbs= localrb_count;
      WRRC_MSG1_HIGH("REL10: Number of RBs to be reconfigured:%d", localrb_count);
    }

    /*store the rb ids tobe affected */
    if(RRCRB_R10_MSG_IE_PRESENT(rb_release_ptr,rb_InformationAffectedList))
    {
      rbr.rbs_tobe_affected.is_valid = TRUE;
      rb_info_affected_list_ptr = 
      &rb_release_ptr->rb_InformationAffectedList;
      ...
      while ( rb_info_affected_list_ptr->n > localrb_count)
      {
        ...
        {
          /* RB Message was invalid. */
          WRRC_MSG1_HIGH("REL10: Invalid RB id:%d in RBR Message", rb_info_affected_list_ptr->elem[localrb_count].rb_Identity);
          /* Send the RB Release Failure Message */
          rbr.status.failure_status = RRCRB_MSG_INVALID_CONFIG;
          send_rbr_failure_message( wrrc_as_id, rbr.status.failure_status, rbr.status.prot_err_cause);
      ...
      rbr.rbs_tobe_affected.num_rbs= localrb_count;
      WRRC_MSG1_HIGH("REL10: Number of RBs tobe affected:%d", localrb_count);
    }
      ...
      signallingConnectionRelIndication))
    {
      ...
      if(rab_found==FALSE )
      {
        WRRC_MSG1_HIGH("REL10: RB lookup in EST_RABS for cn:%d failed", rbr.cn_to_release );
        /* send failure message */
        rbr.status.failure_status = RRCRB_MSG_INVALID_CONFIG;
        send_rbr_failure_message( wrrc_as_id, rbr.status.failure_status, rbr.status.prot_err_cause);
        /* Go back to initial substate */
    ...
    /* Check if there is CN information present. If so, we need to send the information to the Cell Change manager */
    if (RRCRB_R10_MSG_IE_PRESENT(rb_release_ptr,cn_InformationInfo))
    {
      WRRC_MSG0_HIGH("REL10: Saving CN info for NAS");
      rbr.mm_cmd_ptr = rrc_ccm_buid_service_ind_with_nas_cn_info_r6(wrrc_as_id,
        &(rb_release_ptr->cn_InformationInfo) );
      if (rbr.mm_cmd_ptr != NULL)
      {
    ...
    {
      case ORDERED_CONFIG_SET:
        rrcrbr_process_after_oc_config_set_r10(wrrc_as_id,dcch_msg_ptr, rb_release_ptr);
        break;
...
)
{
  ...
  if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5) 
  {
    if( msg_ptr->t != T_rrc_RadioBearerRelease_r3 )
    {
        ...
        
      {
        ...
        rrcrbr_process_rbrelease_r5(wrrc_as_id, cmd_ptr);
        return;
      }
      ...
      {
        rrcrbr_process_rbrelease_r6(wrrc_as_id, cmd_ptr);
        return;
      }
      ...
      {
        rrcrbr_process_rbrelease_r7(wrrc_as_id, cmd_ptr);
        return;
      }
      ...
      {
        rrcrbr_process_rbrelease_r8(wrrc_as_id, cmd_ptr);
        return;
      }
           ...
           rrc_RadioBearerRelease)))
     {
       rrcrbr_process_rbrelease_r9(wrrc_as_id, cmd_ptr);
       return;
     }
#endif /* FEATURE_WCDMA_REL9 */
#ifdef FEATURE_WCDMA_REL10
     ...
     rrc_RadioBearerRelease)))
  {
    rrcrbr_process_rbrelease_r10(wrrc_as_id, cmd_ptr);
    return;
  }
#endif /* FEATURE_WCDMA_REL10 */
      ...
      (RRCCU_STARTED == rrccu_get_cell_update_started_status(wrrc_as_id) ))
  {
     ...
     return;
         ...
         (rrc_buffer_ota_info.transaction_id != RRCRB_INVALID_TRANSACTION_ID)
          &&
         (rrc_buffer_ota_info.transaction_id == msg_ptr->u.r3.radioBearerRelease_r3.rrc_TransactionIdentifier))
  {
    WRRC_MSG0_ERROR("BUF_OTA:Received Duplicate RBR, Ignoring");
    return ;
  }
  ...
  invalid or incompatible configuration given by the
  message */
  validate_rbrelease_message( wrrc_as_id, msg_ptr );
  ...
  else
  {
    ...
    rrcrbr_process_high_mobility_ind_in_non_critical_ext(wrrc_as_id,msg_ptr,MSG_REL99);
    ...
    {
      rrcrbr_mark_cn_doamin_to_be_closed(wrrc_as_id,msg_ptr->u.r3.radioBearerRelease_r3.signallingConnectionRelIndication);
    ...
    switch ( oc_status )
    {
      case ORDERED_CONFIG_SET:
              ...
              || (rbr.state_indicated_by_msg==RRC_STATE_URA_PCH)))
        {
          ...
          if (rrcrbr_is_pcpich_info_present(wrrc_as_id,msg_ptr,MSG_REL99))
          {
            directed_cell_info.pscr_present = TRUE;
            ...
            )
        {
             ...
             (rbr.state_indicated_by_msg==RRC_STATE_CELL_FACH))
          {
                ...
                (rrcrb_validate_directed_cell_info (wrrc_as_id,directed_cell_info)== FALSE))
            {
              ...
              {
                rbr.new_crnti_valid = TRUE;
                rrc_translate_crnti(wrrc_as_id,&msg_ptr->u.r3.radioBearerRelease_r3.new_C_RNTI, 
                                    &c_rnti);
                rbr.new_crnti = (uint16)c_rnti;
              }
              else
        ...
        {
          /*store the directed cell info */
           rbr.directed_cell_info = rrcrbr_is_pcpich_info_present(wrrc_as_id,msg_ptr,MSG_REL99);
...
)
{
  ...
  if( ptr->chan_cfg_status != RRCLLC_CHAN_CFG_SUCCESS )
  {
    ...
    */
    rrcsmc_revert_max_count_c(wrrc_as_id);
   ...
   rrcllc_copy_current_hanging_rb_mapping_to_ordered(wrrc_as_id);
    ...
    rrcrb_update_mac_rab_status(wrrc_as_id,RRC_PROCEDURE_RBR, RRC_CS_DOMAIN_CN_ID,NULL, TRUE);
    ...
    if (ptr->chan_cfg_status == RRCLLC_CHAN_CFG_FAILURE_OTHER)
    {
      ...
      {
        rbr.curr_substate = RRCRB_WAIT_FOR_CELL_UPDATE_CNF;
        rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_RBR, RRC_UNRECOVERABLE_ERROR, FALSE);
        return; 
      }
      ...
      configurations */
      if (OC_NOT_SET == rrcllc_reset_ordered_config(wrrc_as_id,RRC_PROCEDURE_RBR
                                                    ,TRUE
                                                    ))
      {
        /* Then clear all local variables that may
      ...
      if(rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH && rbr.state_indicated_by_msg == RRC_STATE_CELL_DCH)
      {
    ...
    if (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH && rbr.state_indicated_by_msg == RRC_STATE_CELL_DCH)
    {
      ...
      rbr.status.failure_status = RRCRB_PHY_CHAN_FAILURE;
      rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_RBR, RRC_PHY_CHAN_FAILURE,TRUE);
    ...
    else if (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH && rbr.state_indicated_by_msg == RRC_STATE_CELL_DCH)
    {
      rbr.status.failure_status = RRCRB_PHY_CHAN_FAILURE;
      if(rrccu_return_cu_substate(wrrc_as_id) == RRCCU_WAIT_FOR_PENDING_CONFIG)
      {
        rbr.curr_substate = RRCRB_WAIT_FOR_CELL_UPDATE_CNF;
        rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_RBR, RRC_RADIO_LINK_FAILURE, FALSE);
        return; 
      }
      ...
      else if (ptr->chan_cfg_status == RRCLLC_CHAN_CFG_FAILURE_BACK_TO_OLD_CONFIG)
      {
        ...
        rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_RBR, RRC_RADIO_LINK_FAILURE,TRUE);
        /* Wait for CELL UPDATE to get a valid C_RNTI */
        rbr.curr_substate = RRCRB_WAIT_FOR_CELL_UPDATE_CNF;
        rbr.rrcrbr_send_initiate_cu_flag = FALSE;
        rbr.rrcrbr_delete_crnti          = FALSE;
    ...
    {
      /* go to idle */
        cmd=rrc_get_int_cmd_buf(wrrc_as_id);
        ...
        (ordered_config.set_status == OC_SET_FOR_DCH_URA_PCH_TRANS))
  {
    ...
    {
      rrcrb_send_upd_ota_parms_ind(wrrc_as_id,RRC_PROCEDURE_RBR, CPHY_HIGH_MOBILITY_INCL);
    }
  }
  ...
  if( rbr.rbs_tobe_released.is_valid == TRUE )
  {
    ...
    for( count=0; count < rbr.rbs_tobe_released.num_rbs; count++ )
    {
      ...
      rrc_remove_rb_from_established_rabs(wrrc_as_id, (rbr.rbs_tobe_released.rb_id[count]),
                                           &(rab_result));
  ...
  rrcsmc_update_count_c_released_rbs(wrrc_as_id);
...
#error code not present
      ...
      (void)rrcrbr_configure_rab_change(wrrc_as_id,count);
#endif /*FEATURE_SCUDIF_ENHANCEMENT*/
      amr_configured = TRUE;
    }
  ...
  {
      WRRC_MSG1_HIGH("WV_WRRC: Configuring WV Adapter with mode %d from RBR.",current_mode);
       ...
       (rbr.state_indicated_by_msg == RRC_STATE_URA_PCH)))
  {
    if (rbr.state_indicated_by_msg == RRC_STATE_CELL_PCH) 
    {
      ...
      else
      {
        ...
        rrcrb_send_l1_drx_req (wrrc_as_id,RRC_PROCEDURE_RBR);

        rrcrbr_process_successful_proc_completion (wrrc_as_id,TRUE);
      }
      /* return from here */
      return;
    }
    else if (rbr.state_indicated_by_msg == RRC_STATE_URA_PCH)
    {
      ...
      if ( rrccu_is_ura_update_reqd(wrrc_as_id) == TRUE)
      {
        rrcrb_send_ura_update_initiate_req(wrrc_as_id, RRC_PROCEDURE_RBR,
                                            RRC_URA_RESELECTION,
                                            FALSE);
      }
    ...
    /* Coming here implies the "rbr.rrcrbr_send_initiate_cu_flag" is FALSE.
     ...
     * initiated when there is a valid C-RNTI and rbr.rrcrbr_send_initiate_cu_flag
     * is FALSE.
     */
  ...
  else if(RRC_STATE_URA_PCH == curr_state)
  {
    ...
    if (!((rrccu_get_cell_update_started_status_with_cause(wrrc_as_id,&cu_cause)== RRCCU_STARTED) &&
        ...
        ( (rrcllc_get_toc_usage(wrrc_as_id) ==TOC_FOR_OOS)||
        (rrcllc_get_toc_usage(wrrc_as_id) ==TOC_FOR_OOS_WITHOUT_DCH_INFO)||
        (rrcllc_get_toc_usage(wrrc_as_id) ==TOC_FOR_OOS_WITH_DCH_INFO))))
    {
      /* check whether URA update is required */
      if ( rrccu_is_ura_update_reqd(wrrc_as_id) == TRUE)
      {
...
//  if (RRC_STATE_CELL_DCH == curr_state)
  {
    ...
    rrcrb_send_rlc_re_est_req_to_smc(wrrc_as_id,RRC_PROCEDURE_RBR, rb_type, TRUE);
  ...
  {   
    ...
    2> include the IE "Reconfiguration Status Indicator" and set it to TRUE.*/
      ...
      if((rrccu_return_cu_substate(wrrc_as_id) == RRCCU_WAIT_FOR_PENDING_CONFIG)
        && (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH))
      {
        if(rrccu_check_rb_in_estab_rab(wrrc_as_id))
        {
          ...
          (void) rex_clr_sigs(rex_self(), RRCRBR_TEMP_TIMER_SIG);
          (void) rex_set_timer(&rbr.rrcrbr_temp_timer, RRCRB_PND_CFG_DELAY); 
          rbr.rrcrbr_sig = rex_wait(RRCRBR_TEMP_TIMER_SIG);
          MSG_LOW("RRCRBR signal:%d", rbr.rrcrbr_sig, 0, 0);
          (void) rex_clr_sigs(rex_self(), RRCRBR_TEMP_TIMER_SIG);
          rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_RBR, RRC_UNRECOVERABLE_ERROR, FALSE);
...
)
{
 ...
 /* Check if E-RNTI is not given in OTA then initiate cell update*/
           ...
           ptr->new_state == RRC_STATE_URA_PCH)
  {
    ...
    {
      WRRC_MSG3_HIGH("Recvd state change %d to %d RBR state %d, No action",
...
)
{
  ...
  if ( ptr->status == SUCCESS )
  {
        ...
        (rbr.state_indicated_by_msg == RRC_STATE_URA_PCH))
    {
      ...
      else if(current_state==RRC_STATE_CELL_FACH) 
      {
        /* 
           ...
           to NW.
        */
        if(rrccsp_is_inter_frequency_cell_resel_in_progress(wrrc_as_id) == TRUE)
        {
          rrcrb_send_cell_update_initiate_req(wrrc_as_id, 
                                              RRC_PROCEDURE_RBR,
                                              RRC_REENTERED_SERVICE_AREA,
                                              TRUE);
          return;
        }
        if (RRCCU_STARTED == rrccu_get_cell_update_started_status_with_cause(wrrc_as_id,&cu_cause) &&
        ...
        else
        {
          if((rbr.initiate_cell_selection_trans_from_fach == TRUE )
            && (rrccsp_check_initiate_cell_selection_handling(wrrc_as_id,NULL) == TRUE))
          {
    ...
    if (rbr.rbr_for_srns_relocation)
    {
      if (RRC_STATE_CELL_DCH == rrc_get_state(wrrc_as_id))
      {
        ...
        rrcrb_send_rlc_re_est_req_to_smc(wrrc_as_id,RRC_PROCEDURE_RBR, rb_type, FALSE);
        /* send the nas cn info if stored */
        if (rbr.cn_info_saved != FALSE)
        {
          /* Put the command on MM queue */
  ...
  else
  {
    ...
    if (rbr.rbr_for_srns_relocation)
    {
      ...
      rrc_transition_to_disconnected_state(wrrc_as_id,RRC_PROCEDURE_RBR,RRC_TX_TO_DISCON_OTHER);
    }
  }
      ...
      (ptr->status != SUCCESS))
  {
    ...
    if (OC_SET_FOR_DCH_FACH_TRANS == rrcllc_get_ordered_config_status(wrrc_as_id))
    {
    ...
    {
      (void)rrcllc_reset_ordered_config(wrrc_as_id,RRC_PROCEDURE_RBR,TRUE);
    }
  }
  else
  {
  ...
  if(rbr.rbr_for_srns_relocation == TRUE)
  {
    rrcdormancy_update_dormancy_for_srns_hho_cco(wrrc_as_id, RRC_PROCEDURE_RBR);
  }
...
)
{
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    ...
    case RRC_DOWNLINK_SDU_IND:
      ...
      rrcrbr_process_rbrelease_message(wrrc_as_id,  cmd_ptr );
      break;
...
)
{
  ...
  switch ( cmd_ptr->cmd_hdr.cmd_id )
  {
    case RRC_STATE_CHANGE_IND:
      ...
      rrcrbr_process_state_change_ind(wrrc_as_id, &(cmd_ptr->cmd.state_change_ind) );
      break;
...
)
{
  ...
  case RRC_RE_EST_RLC_FOR_SRNS_CNF:
    ...
    rrcrbr_process_rlc_reestablishment_cnf(wrrc_as_id );
    break;
...
)
{
  ...
  case RRC_CELL_UPDATE_COMPLETE_CNF:
    ...
    else if (rbr.status.failure_status == RRCRB_MSG_VALID)
    {
      ...
      {
        send_rbr_complete_message(wrrc_as_id,FALSE);
      }
  ...
  case RRC_CHANNEL_CONFIG_CNF:
    ...
    rrcrbr_process_channel_config_cnf(wrrc_as_id, &(cmd_ptr->cmd.chan_config_cnf) );
    break;
    ...
    else if (rbr.status.failure_status == RRCRB_MSG_VALID)
    {
        ...
        if (rrcllc_get_toc_usage(wrrc_as_id) ==TOC_FOR_OOS)
        {
          WRRC_MSG1_HIGH("Send channel config req for OOS scenarios TOC usage %d  ",
            rrcllc_get_toc_usage(wrrc_as_id));
...
)
{
  ...
  case RRC_DOWNLINK_L2ACK_IND:
    ...
    /* Confirmation from LLC for the channel configurations */
    rrcrbr_process_l2_ack_ind(wrrc_as_id, &(cmd_ptr->cmd.downlink_l2ack_ind) );
    break;
...
)
{
  ...
  switch ( cmd_ptr->cmd_hdr.cmd_id )
  {
    ...
    case RRC_NEW_CELL_IND:
      rrcrbr_process_new_cell_ind(wrrc_as_id, &(cmd_ptr->cmd.new_cell_ind) );
      break;
...
)
{
  ...
  case RRC_RNTI_UPDATE_CNF:
    ...
    {
      rrcrbr_process_rnti_update_cnf(wrrc_as_id);
    }
    else
    {
...
)
{
  ...
  WRRC_MSG4_HIGH("RRCRBR:L2ack status %d <0-Success 1-Failure> for MUI:%d, Lc_Id:%d, rbe_subst:%d",
...
)
{
  ...
  case RRCRB_WAIT_FOR_CELL_UPDATE_CNF:
    /* Call the event handler for RRCRB_WAIT_FOR_CELL_UPDATE_CNF substate */
    rrcrbr_fach_wfcucnf_event_handler(wrrc_as_id,  cmd_ptr);
    break;

  case RRCRB_WAIT_FOR_CHAN_CFG_CNF:
    /* Call the event handler for RRCRB_WAIT_FOR_CHAN_CFG_CNF substate */
    rrcrbr_fach_wfchcnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
    break;

  case  RRCRB_WAIT_FOR_CELL_SELECTION_CNF:
    /* Call the event handler for RRCRB_WAIT_FOR_CHAN_CFG_CNF substate */
    rrcrbr_fach_wfcscnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
    break;

  case RRCRB_WAIT_FOR_CRNTI_UPDATE_CNF:
    /* Call the event handler for RRCRB_WAIT_FOR_CRNTI_UPDATE_CNF substate */
    rrcrbr_fach_wfcrntiupdatecnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
    break;

  case RRCRB_WAIT_FOR_L2_ACK:
    /* Call the event handler for RRCRB_WAIT_FOR_L2_ACK substate */
    rrcrbr_fach_wfl2ack_substate_event_handler(wrrc_as_id,  cmd_ptr );
    break;
...
)
{
  ...
  case RRCRB_WAIT_FOR_CELL_UPDATE_CNF:
    /* Call the event handler for RRCRB_WAIT_FOR_CELL_UPDATE_CNF substate */
    rrcrbr_pch_wfcucnf_event_handler(wrrc_as_id,  cmd_ptr);
    break;

  case RRCRB_WAIT_FOR_CELL_SELECTION_CNF:

    rrcrbr_pch_wfcscnf_event_handler(wrrc_as_id,  cmd_ptr);
    break;

  case RRCRB_WAIT_FOR_CHAN_CFG_CNF:

    /* Call the event handler for RRCRB_WAIT_FOR_CHAN_CFG_CNF substate */
    rrcrbr_pch_wfchcnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
    break;
  ...
  default:
    ...
    MSG_MED("Command %x not processed in substate%d rrc_state%d", 
            cmd_ptr->cmd_hdr.cmd_id, rbr.curr_substate, rrc_get_state(wrrc_as_id));
    break;
...
)
{
  ...
  switch ( rbr.curr_substate )
  {
    ...
    case RRCRB_WAIT_FOR_CHAN_CFG_CNF:
      ...
      rrcrbr_other_wfchcnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;

    case RRCRB_WAIT_FOR_L2_ACK:
      /* Call the event handler for WAIT_FOR_L2_ACK substate */
      rrcrbr_other_wfl2ack_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;

    case RRCRB_WAIT_FOR_CELL_UPDATE_CNF:
      rrcrbr_other_wfcucnf_substate_event_handler(wrrc_as_id,  cmd_ptr);
      break;

    case  RRCRB_WAIT_FOR_CELL_SELECTION_CNF:
      /* Call the event handler for RRCRB_WAIT_FOR_CHAN_CFG_CNF substate */
      rrcrbr_other_wfcscnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;
...
)
{
  ...
  rrc_crash_debug_save_proc_substates(wrrc_as_id,RRC_PROCEDURE_RBR,rbr.curr_substate,cmd_ptr->cmd_hdr.cmd_id);
  ...
  case RRC_STATE_CELL_FACH:    
    ...
    for CELL_FACH state */
    rrcrbr_cell_fach_event_handler(wrrc_as_id, cmd_ptr);
    break;

  case RRC_STATE_CELL_DCH:    
    /* If it's CELL_DCH state, call the event handler
    for CELL_DCH state */
    rrcrbr_cell_dch_event_handler(wrrc_as_id, cmd_ptr);
    break;
    ...
    case RRC_STATE_CELL_PCH:
      /* If it's CELL_DCH state, call the event handler
      for CELL_PCH state */
      rrcrbr_pch_event_handler(wrrc_as_id, cmd_ptr);
      break;
  ...
  case RRC_STATE_DISCONNECTED: 
    ...
    rrcrbr_other_state_event_handler(wrrc_as_id, cmd_ptr);
    break;
...
void rrcrbr_init_procedure(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  to disconnected state. */
  rrcscmgr_register_for_scn(wrrc_as_id, RRC_PROCEDURE_RBR,  /* Procedure name */
  ...
  rex_def_timer(&rbr.rrcrbr_temp_timer, rex_self(), RRCRBR_TEMP_TIMER_SIG);
...
{
  qsh_mdump_collect_high(&rbr,sizeof(rbr));
}
#endif