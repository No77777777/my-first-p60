...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrctvm.c#2 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------
03/08/17   sp    Made changes to support configuration with RB-ID 32
...
// Stores SIB params
mac_ul_traffic_meas_config_type  mac_sib_params_sim[MAX_RRC_STACKS];
tvm_meas_id_list_struct_type     tvm_meas_id_list_sim[MAX_RRC_STACKS][MAX_TRAFFIC_VOLUME_MEASUREMENTS]; 

#define mac_sib_params mac_sib_params_sim[WAS_ID_M]
#define tvm_meas_id_list tvm_meas_id_list_sim[WAS_ID_M]
...
)
{
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_TYPE2_PTR(sib_meas_ptr,trafficVolumeMeasSysInfo))
  {
    ...
    rrctvm_set_meas_identity(wrrc_as_id,&mac_sib_params.meas_id, &sib_meas_ptr->trafficVolumeMeasSysInfo); 
    ...
    rrc_TrafficVolumeMeasSysInfo,trafficVolumeReportingQuantity))
  {
    ...
    WRRC_MSG3_HIGH("TVM:Rpting Quan RLC%d A%d V%d", mac_sib_params.meas_report_qty.rlc_payload,
    ...
    rrc_TrafficVolumeReportCriteriaSysInfo_trafficVolumeReportingCriteria))
  {
    ...
    if ((rrctvm_process_event_criteria(wrrc_as_id,&sib_meas_ptr->trafficVolumeMeasSysInfo.reportCriteriaSysInf.u.
  ...
  rrctvm_set_reporting_criteria(wrrc_as_id,&mac_sib_params.meas_id, &sib_meas_ptr->
    trafficVolumeMeasSysInfo.reportCriteriaSysInf);
...
static boolean rrctvm_check_if_meas_is_valid_for_current_state(sys_modem_as_id_e_type wrrc_as_id,meas_validity_enum_type prim_meas_validity )
{
   ...
   /* Check for meas validity and current state/transition mismatch*/
    ...
    if (((prim_meas_validity == STATE_DCH) && ((rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_CELL_FACH)|| (oc_status == OC_SET_FOR_DCH_FACH_TRANS) || (oc_status == OC_SET_FOR_DCH_CELL_PCH_TRANS) || (oc_status == OC_SET_FOR_DCH_URA_PCH_TRANS)))
        ||
        ((prim_meas_validity == ALL_STATES_BUT_DCH) && ((rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_CELL_DCH) || ((RRC_STATE_CELL_FACH == rrc_get_state(wrrc_as_id)) && (oc_status == OC_SET_FOR_CELL_DCH)) )) 
      )
    {
      return FALSE;
    }
...
)
{
  ...
  /* Although we came here bec it is TVM, still make check to ensure UTRAN
    ...
    * not received, then configuration incomplete */
    ...
    if (rrctvm_read_traffic_vol_params(wrrc_as_id,rrc_mcm_ptr->measurementCommand.
    ...
    if (rrctvm_check_if_meas_is_valid_for_current_state(wrrc_as_id,prim_meas_validity) ==  FALSE)
    {
      WRRC_MSG0_HIGH("TVM: RRC val, MCM val differ");
    ...
    if (RRC_MSG_COMMON_BITMASK_IE_PTR(rrc_mcm_ptr,rrc_MeasurementControl_r3_IEs,additionalMeasurementList))
    {
      ...
      if (rrctvm_validate_additional_meas(wrrc_as_id,&prim_meas_validity, &rrc_mcm_ptr->additionalMeasurementList, 
    ...
    rrctvm_process_mcm_identity(wrrc_as_id,&mac_mcm_params.meas_id);
    
    /* Set parameters in RRC DB */
    if (rrctvm_config_tvm_db(wrrc_as_id,&mac_mcm_params,&prim_meas_validity) == FALSE)
    {
      rrcmeas_mcf.rrc_transaction_id = transaction_id;
    ...
    {
      rrcmeas_set_additional_meas_in_db(wrrc_as_id,&meas_type, &rrc_mcm_ptr->additionalMeasurementList,
  ...
  /* Delete chosen. It has already been verified in rrcmeas_find_owner
    * that this meas exists. So let us delete it from tvm_meas_id_list */
    ...
    rrcmeas_delete_addtl_meas(wrrc_as_id,&mac_mcm_params.meas_id);
    ...
    rrctvm_send_mac_cmd(wrrc_as_id,&mac_mcm_params);
...
)
{
  ...
  /* Delete chosen. It has already been verified in rrcmeas_find_owner
    * that this meas exists. So let us delete it from tvm_meas_id_list */
    ...
    alloc_idx = rrcmeas_get_int_trans_index_for_trans_id(wrrc_as_id,transaction_identifier);
    mac_mcm_params.config_ack_num = alloc_idx;
    rrctvm_send_mac_cmd(wrrc_as_id,&mac_mcm_params);
...
)
{
    ...
    rrc_TrafficVolumeMeasurement,trafficVolumeReportingQuantity))
  {
    ...
    WRRC_MSG3_HIGH("TVM:Rpting Quan RLC%d A%d V%d", mac_config_ptr->meas_report_qty.rlc_payload,
    ...
    rrc_TrafficVolumeReportCriteria_trafficVolumeReportingCriteria))
  {
    ...
    if (rrctvm_process_event_criteria(wrrc_as_id,&rrc_mcm_ptr->reportCriteria.
...
#ifdef FEATURE_WCDMA_HS_RACH
        if(rrctvm_check_if_edch_object_exists(wrrc_as_id,mac_config_ptr,mac_event_crit_ptr->trch_event_cfg[transport_channel_cnt].trch_info.trch_id))
        {
          mac_event_crit_ptr->trch_event_cfg[transport_channel_cnt].
...
)
{
  ...
  if (meas_report_ptr->num_reports_ind == TRUE)
  {
    ...
    rrcmeas_delete_addtl_meas(wrrc_as_id,&tvm_to_be_deleted);
  }
  ...
  rrcmeas_fill_meas_id(wrrc_as_id,&msg_ptr->message.u.measurementReport,meas_report_ptr->meas_id, MAC_MEAS);
  ...
  if (rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_CELL_FACH)
  {
    /* Append RACH report if RACH criteria satisfied and report available */
    if(rrcmeas_append_meas_rpt_on_rach(wrrc_as_id,
...
#ifdef FEATURE_WCDMA_HS_RACH
    && (FALSE == rrchsrach_get_ready_for_common_edch(wrrc_as_id,current_config_ptr))
#endif
    )    
    { 
...
)
{
  ...
  rrc_cmd_l2_req->mac_ul_traffic_vol_config = 
      (mac_ul_traffic_meas_config_type *)rrc_malloc_interlayer_cmd(wrrc_as_id, sizeof(mac_ul_traffic_meas_config_type));
  ...
  *(rrc_cmd_l2_req->mac_ul_traffic_vol_config) 
  ...
  if(rrc_qsh_dl_committed == FALSE)
  {
    ...
    rrc_qsh_log_meas_ctrl_params(wrrc_as_id, &tvm_params);
  }	
#endif
...
)
{
  ...
  for (count = 0; count < MAX_TRAFFIC_VOLUME_MEASUREMENTS; count++)
  {
    if (tvm_meas_id_list[count].meas_id != INVALID_MEAS_ID)
    {
      ...
      case NOT_PRESENT:
        ...
        rrctvm_send_mac_cmd(wrrc_as_id, &tvm_cmd);
  ...
  addtl_deleted_cnt++) 
  {
    if (prim_meas_to_be_deleted_from_addtl_db[addtl_deleted_cnt] != INVALID_MEAS_ID)
    {
      ...
      rrcmeas_delete_addtl_meas(wrrc_as_id,&tvm_meas_id);
    }
  }
  
  if ((tvm_meas_id = rrctvm_get_meas_id_from_sibs(wrrc_as_id)) == INVALID_MEAS_ID)
  {
  ...
  /* Find if any MCM is present in DB and has same id as one in tvm_meas_id  
    ...
    if (rrctvm_get_sibs_in_connected_state(wrrc_as_id) == TVM_SUCCESS)
    {
      ...
      if(rrcmeas_ext_data.dch_to_pch_in_progress == TRUE)
      {
...
)
{
  ...
  rrctvm_initialize(wrrc_as_id);
  
  /* Get traffic volume params from SIB11 */
  sib11_ptr = (rrc_SysInfoType11*) 
    rrc_sibdb_return_sib_for_srv_cell(wrrc_as_id, rrc_SIB11);
  ...
  sib12_ptr = (rrc_SysInfoType12*) 
    rrc_sibdb_return_sib_for_srv_cell(wrrc_as_id, rrc_SIB12);

  /* SIB 12 present */
  if ((sib11_ptr->sib12indicator == TRUE) && (sib12_ptr != NULL))
  {
    rrcmeas_ext_data.sib12_present = SIB12_PRESENT;
    
    /* Read traffic volume info from SIB12 and send meas req to MAC  */
    
    if (rrctvm_process_sib_info(wrrc_as_id,&sib12_ptr->measurementControlSysInfo)
      == TVM_FAIL )
    {
  ...
  else
  {
    ...
    /* Read traffic volume info from SIB12 and send meas req to MAC */
    if (rrctvm_process_sib_info(wrrc_as_id,&sib11_ptr->measurementControlSysInfo) 
      == TVM_FAIL)
    {
...
)
{
  ...
  rrcmeas_ext_data.sib12_present = SIB12_ABSENT;
  ...
  rrctvm_send_mac_cmd(wrrc_as_id,&mac_sib_params);
...
void rrctvm_process_tvm_on_cell_reselection(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8 count = 0;
  ...
  */
  ...
  for (count=0; count <MAX_TRAFFIC_VOLUME_MEASUREMENTS; count++)
  {
      ...
      (tvm_meas_id_list[count].meas_ctrl_msg == FALSE))
    {
      ...
      rrctvm_send_mac_cmd(wrrc_as_id,&traffic_vol_params); 
  ...
  else
  {
    ...
    if (rrctvm_process_tvm_sibs_in_connected_state(wrrc_as_id) == TVM_SUCCESS)
    {
...
)
{
  ...
  addtl_meas_present = mac_get_additional_traffic_vol_msmt_results(wrrc_as_id, *additional_meas_id_ptr, additional_tvm);
  ...
  {
    /* Get radio bearer Id */
    if (additional_tvm->rb_meas[count].rb_id > MAX_RB)
    {
...
)
{
  ...
  while (additional_list_ptr->n > idx)
  {
    ...
    additional_meas_type = rrcmeas_find_additional_meas_type(wrrc_as_id,&additional_meas_id, &a_rpt_crit, &addtl_meas_validity);
    ...
    if(tvm_meas_id_list[i].meas_id == 4) 
    {
        ...
        (tvm_meas_id_list[i].meas_validity == ALL_STATES || tvm_meas_id_list[i].meas_validity == ALL_STATES_BUT_DCH))
      {
        /*Call the MAC function to check the threshold and the event identity*/
         WCDMA_RRC_TASK_LOCK();
            status = mac_compute_traffic_volume_above_threshold_cu(wrrc_as_id, 4,rrcllc_get_rach_cctrch_ptr( wrrc_as_id)
#ifdef FEATURE_WCDMA_HS_RACH 
            ,rrcllc_get_hsrach_config_ptr( wrrc_as_id)
#endif
            );
        WCDMA_RRC_TASK_FREE();
  ...
  if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r9_setup)
  {
    ...
    * not received, then configuration incomplete */
    ...
    if(rrcmeas_process_mcm_addnl_meas
      (wrrc_as_id,mcm_r9,transaction_identifier,&addtl_meas_validation,
    ...
    /* Set Measurement Identity in tvm_meas_id_list for SETUP */
    rrctvm_process_mcm_identity_r9(wrrc_as_id,&mac_mcm_params.meas_id,mcm_r9);
    ...
    if (addtl_meas_validation == TRUE) 
    {
      rrcmeas_check_addtl_meas_in_db(wrrc_as_id,&meas_type, mcm_r9,
        ...
        rrc_MeasurementCommand_r9_modify,measurementType))
    {
      ...
      * by RRC as traffic volume is actually not Traffic Volume. Send
        ...
        &mac_mcm_params) == TVM_SUCCESS)
      {
        ...
        /* Process Additional Meas */
        if(rrcmeas_process_mcm_addnl_meas(wrrc_as_id,mcm_r9,transaction_identifier,&addtl_meas_validation,meas_type,mac_mcm_params.meas_id,&prim_meas_validity) == FAILURE)
        {
            return TVM_FAIL;
        }
...
FUNCTION   RRCTVM_CONFIG_TVM_DB_R10
...
boolean rrctvm_config_tvm_db_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  mac_ul_traffic_meas_config_type* mac_mcm_ptr,
  meas_validity_enum_type* p_meas_validity,
...
FUNCTION   RRCTVM_PROCESS_MEAS_IDENTITY_R10
...
SIDE EFFECTS
    ...
    None
===========================================================================*/
void rrctvm_process_mcm_identity_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  uint16* meas_id_ptr,
  rrc_MeasurementControl_r10 *mcm_r10 
...
DESCRIPTION  
    ...
    Process traffic volume information in a Meas
    Ctrl Message R10. Also sends a Meas Ctrl Failure
    Message in case of an error
...
)
{
  ...
  if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r10_setup)
  {
    ...
    * not received, then configuration incomplete */
    ...
    if(rrcmeas_process_mcm_addnl_meas_r10
      (wrrc_as_id,mcm_r10,transaction_identifier,&addtl_meas_validation,
       meas_type,mac_mcm_params.meas_id,&prim_meas_validity) == FAILURE)
    {
      return TVM_FAIL;
    }
    ...
    /* Set Measurement Identity in tvm_meas_id_list for SETUP */
    rrctvm_process_mcm_identity_r10(wrrc_as_id,&mac_mcm_params.meas_id,mcm_r10);
    ...
    if (addtl_meas_validation == TRUE) 
    {
      rrcmeas_check_addtl_meas_in_db_r10(wrrc_as_id,&meas_type, mcm_r10,
        mac_mcm_params.meas_id);
  ...
  else if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r10_modify)
  {
    ...
    if (RRC_MSG_COMMON_BITMASK_IE_PTR(rrc_mcm_ptr,
        rrc_MeasurementControl_r10_IEs,measurementReportingMode))
    {
      /* To read traffic volume reporting mode */
      mac_mcm_params.meas_report_mode.transfer_mode = 
        TVM_RET_REPORTING_MODE(rrc_mcm_ptr->
    ...
    {
      if (!(RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand.u.modify->measurementType,
        rrc_MeasurementType_r10_trafficVolumeMeasurement)))
      {
      /* If one comes here, then it means that meas id earlier determined
      * by RRC as traffic volume is actually not Traffic Volume. Send
        * Meas Ctrl failure Msg to UTRAN */
        ...
        &mac_mcm_params) == TVM_SUCCESS)
      {
        ...
        /* Process Additional Meas */
        if(rrcmeas_process_mcm_addnl_meas_r10(wrrc_as_id,mcm_r10,transaction_identifier,&addtl_meas_validation,meas_type,mac_mcm_params.meas_id,&prim_meas_validity) == FAILURE)
        {
            return TVM_FAIL;
        }
