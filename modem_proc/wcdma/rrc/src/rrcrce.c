...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcrce.c#5 $ $DateTime: 2020/07/13 12:41:39 $ $Author: pwbldsvc $
...
05/5/20   gcs     Added NV support to disable/enable feature FRLTE for LTOW CSFB due to supplementary service and Made changes to trigger FRLTE only if NV is enabled
...
03/21/18   gc      Fix KW error
02/13/18   sas     Made changes to Bar current camped cell upon MAX RACH attempts.
...
11/09/16   sr      Changes to guard the opt via NV
11/07/16   sr      Changes to bar the PSC for 1280 seconds if MAX attempts fail and the cell doesn't have any intra and inter f nbrs
08/03/16   sp      Made changes to ignore BPLMN suspend cnf if LLC has sent Serving BCCH drop to L1
05/20/16   sr      f3 reduction changes for RCE/UECI/DT
05/19/15   sn      f3 reduction changes for cell resel, unlock handling, pg1
05/17/16   vs      Changes to check sib7 validity when UE camps on after EST_REQ is received and setup BCCH pipe as per FEATURE_UPDATE_SIB7_IN_CONNECTING
05/09/16   nr      Made changes to read GCF NV value from default sub as it is device specific
...
03/21/16   vs      Changes to avoid abroting sib events after connection reject with freq redirection is received and before validation of the redirection request when FEATURE_UPDATE_SIB7_IN_CONNECTING is enabled
03/16/16   vs      Made changes for SIB7 reading enhancement in connecting state
03/02/16   sas     Made changes to not release lock for irat redirection cases after call release, instead exchange with irat_client.
02/22/16   nr      CR945560: Made changes to stop QTA blocking timer after receiving conn setup msg
01/12/16   nr      Made changes to report failure events to QSH
11/12/15   sp      Made changes to log call end info in all cases whenver call is released
11/05/15   sp      Made changes to add call end metrics - FR#30856
11/02/15   sas     Made Changes to implement 3Gpp CR5754 related to UE capability reporting on NW req LTE bands.
09/23/15   as      Made changes to stop QTA-CCCH timer after sending connection setup complete
08/21/15   gv      Made changes to stop priority change timer on closure of RRC connection
08/18/15   sr      Changes to update proc type in RB0 REL CNF state
...
06/01/15   sr      Handling immediate preemption for non-voice calls during signaling procedures
05/29/15   gv      Additional code changes to support TRM-U
05/19/15   sp      Made changes for Mav debug snapshot
05/05/15   vi      Changes to support  LTE-U feature protected by feature flag LTE_BAND_NUM
04/23/15   vi      Made changes to set rsp_to_rrc_conn_req variable to RRC_CONNECTION_REJECT whenever rrc connection reject is received with interRAT info
...
03/18/15   sr      Corrected the sizeof funcion arguement
...
** ----------------------------------------------------------------------- */
#define GET_RCE_TRANS_ID() (++rrcrce_int_data.chl_config_transaction_id % 255)
...
extern boolean rrcpg1_match_found_conn_mode_cn_id_sim[MAX_RRC_STACKS];
#define rrcpg1_match_found_conn_mode_cn_id rrcpg1_match_found_conn_mode_cn_id_sim[WAS_ID_M]
#endif

extern boolean rrcmeas_intra_f_nbr_empty_sim[MAX_RRC_STACKS];
#define rrcmeas_intra_f_nbr_empty rrcmeas_intra_f_nbr_empty_sim[WAS_ID_M]

extern boolean rrcmeas_inter_f_nbr_empty_sim[MAX_RRC_STACKS];
#define rrcmeas_inter_f_nbr_empty rrcmeas_inter_f_nbr_empty_sim[WAS_ID_M]
...
extern boolean rrcmeas_intra_f_nbr_empty;
extern boolean rrcmeas_inter_f_nbr_empty;
#endif
...
** ----------------------------------------------------------------------- */
#ifdef FEATURE_DUAL_WCDMA 
LOCAL rrcrce_int_data_type rrcrce_int_data_sim[MAX_RRC_STACKS];
#define rrcrce_int_data rrcrce_int_data_sim[WAS_ID_M]
#else
LOCAL rrcrce_int_data_type rrcrce_int_data;
#endif
/*===========================================================================
** -----------------------------------------------------------------------------------
...
===========================================================================*/
/***/ __WCDMA_RRC_CODE_SEGMENT_NON_DEMANDPAGED__ /***/
boolean rrcrce_is_rce_procedure_waiting_for_bplmn(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_state_e_type curr_rrc_state = rrc_get_state(wrrc_as_id);

  if ( ((curr_rrc_state == RRC_STATE_DISCONNECTED)
     && (rrcrce_int_data.substate != RRCRCE_INITIAL)) ||
     ((curr_rrc_state != RRC_STATE_DISCONNECTED)
     && (rrcrce_int_data.substate != RRCRCE_FINAL)))
  {
    WRRC_MSG2_HIGH("WTOW RCE is active sst %d curr_st %d", rrcrce_int_data.substate, curr_rrc_state);
    /* RCE is active */
    return(TRUE);
  }
    return(FALSE);
...
SIDE EFFECTS
  None
===========================================================================*/
static void rrcrce_delete_default_config_info(sys_modem_as_id_e_type wrrc_as_id)
{
  if(rrcrce_int_data.default_config_info.def_config_ptr_present == TRUE)
  {
    if(rrcrce_int_data.default_config_info.default_config_id == RRC_DEF_CONFIG_17)
    {
      /*free the msg_ptr previously allocated.. if failure, flag an error*/
      (void) rrcasn1_free_pdu(wrrc_as_id,(int)rrc_DL_DCCH_Message_PDU, 
       rrcrce_int_data.default_config_info.dcch_msg_ptr);
        /*clean the msg_ptr data structure*/
       rrcrce_int_data.default_config_info.def_config_ptr_present = FALSE;
    } 
    else
    if (rrcrce_int_data.default_config_info.rbs_ptr != NULL)
    {
      (void)rrcasn1_free_pdu(wrrc_as_id,(int)rrc_DL_DCCH_Message_PDU, 
      rrcrce_int_data.default_config_info.rbs_ptr);
      /*clean the msg_ptr data structure*/
      rrcrce_int_data.default_config_info.def_config_ptr_present = FALSE;
    }
    else
    {
      /*free the msg_ptr previously allocated.. if failure, flag an error*/
      (void)rrcasn1_free_pdu(wrrc_as_id,(int)rrc_SysInfoType16_PDU, 
      rrcrce_int_data.default_config_info.sib16_ptr);
      /*clean the msg_ptr data structure*/
      rrcrce_int_data.default_config_info.def_config_ptr_present = FALSE;
    } 
  }
  rrcrce_int_data.default_config_info.rbs_ptr = NULL;
  rrcrce_int_data.default_config_info.dcch_msg_ptr = NULL;
  rrcrce_int_data.default_config_info.sib16_ptr = NULL;
  rrcrce_int_data.default_config_info.config_type =  RRC_INVALID_CONFIG;
...
SIDE EFFECTS
  None
===========================================================================*/
void rrcrce_send_conn_setup_ind(sys_modem_as_id_e_type wrrc_as_id,rrc_conn_setup_ind_type conn_setup_ind_params);
...
SIDE EFFECTS
  None
===========================================================================*/
uint8 rrcrce_get_num_retries_count(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcrce_int_data.num_retries_count;
}
...
)
{
  ...
  /* The following code is added since de-registration mechanism is not place.
     ...
     change manager */
  rrcscmgr_register_for_scn(wrrc_as_id,RRC_PROCEDURE_RCE,
                            RRC_STATE_WILDCARD,
                            RRC_STATE_DISCONNECTED);

  /*Init the values*/
  rrcrce_int_data.est_cn_domain_id = RRC_CS_DOMAIN_CN_ID;
  rrcrce_int_data.est_call_type = RRC_CALL_TYPE_NONE;
  rrcrce_int_data.cmac_status = RACH_TX_SUCCESS;
  rrcrce_int_data.mm_abort_cause = RRC_MAX_ACCESS_FAILURE; 
...
#ifdef FEATURE_DUAL_SIM
  cmd_ptr->cmd.rrc_abort_cnf.as_id = rrc_get_as_id(wrrc_as_id);
#endif
  WRRC_MSG2_HIGH("Dispatching out MM cmd RRC_ABORT_CNF/ Send Abort Cnf in %d substate for dormancy rel_cause %d",rrcrce_int_data.substate,cmd_ptr->cmd.rrc_abort_cnf.conn_rel_cause);

  rrc_send_rrc_message_to_mm(wrrc_as_id, cmd_ptr);               /* Sends the command to MM */
}/*rrcrce_send_rrc_abort_cnf */
#endif
/*===========================================================================
...
)
{
  ...
  WRRC_MSG2_HIGH_OPT("function rrcrce_procedure_event_handler is called in rrc_state = rrc_state_e_type_value%d, and rrcrce_substate = rrcrce_substate_e_type_value%d", rrc_state, rrcrce_int_data.substate);
  rrc_crash_debug_save_proc_substates(wrrc_as_id,RRC_PROCEDURE_RCE,rrcrce_int_data.substate,cmd_ptr->cmd_hdr.cmd_id);
    ...
    )
  {
    ...
    rrcrce_send_rrc_abort_cnf_ue_init_dormancy(wrrc_as_id, RRC_UE_INIT_DORMANCY_ABORT_SUCCESS);
    return;
  }
  else if((cmd_ptr->cmd_hdr.cmd_id == RRC_ABORT_REQ)&&
     (cmd_ptr->cmd.rrc_abort_req.abort_cause == RRC_UE_INITIATED_DORMANCY)&&
     (RRC_NOT_CAMPED_ON == rrc_ccm_get_curr_camping_status(wrrc_as_id, RRC_PROCEDURE_RCE))
    )
  {
    /* If Abort is for UE initiated dormancy feature, check to see if UE is in OOS  */
    /*UE is in OOS so send abort cnf right here*/
    rrcrce_send_rrc_abort_cnf_ue_init_dormancy(wrrc_as_id, RRC_UE_INIT_DORMANCY_UE_IN_OOS);
    return;
  }
#endif
...
/* Send release cause as PS_ABORT_SUCCESS only when connection has already been released*/
  ...
  {
    if((rrc_state == RRC_STATE_DISCONNECTED) && (rrcrce_int_data.substate == RRCRCE_INITIAL))
    {
      rrcdata_misc_info.rrc_release_cause = RRC_REL_DUAL_STANDBY_PS_ABORT_SUCCESS;
    }
    else
    {
      rrcdata_misc_info.rrc_release_cause = RRC_REL_CAUSE_NORMAL;
    }
  }
#endif

  if((cmd_ptr->cmd_hdr.cmd_id == RRC_ABORT_REQ) && (rrccho_is_wtog_cco_active(wrrc_as_id) == TRUE))
  {
    rrcmcm_send_irat_abort_ind(wrrc_as_id);
  }

  switch (rrc_state)
  {
    case RRC_STATE_DISCONNECTED: 
      rrcrce_process_disconnected_state(wrrc_as_id, cmd_ptr);
      break;
    case RRC_STATE_CONNECTING:   
      rrcrce_process_connecting_state(wrrc_as_id, cmd_ptr); 
      break;
    case RRC_STATE_CELL_FACH:    
      rrcrce_process_cell_fach_state(wrrc_as_id, cmd_ptr); 
      break;
    case RRC_STATE_CELL_DCH:     
      rrcrce_process_cell_dch_state(wrrc_as_id, cmd_ptr); 
      break;
    case RRC_STATE_CELL_PCH:
    case RRC_STATE_URA_PCH:
      rrcrce_process_cell_ura_pch_state(wrrc_as_id, cmd_ptr);
      break;
    default:  
      WRRC_ERR1_FATAL("Event forwarded in invalid State: %d",rrc_state);
      break;
   }  

#ifdef FEATURE_WCDMA_FAST_RETURN_TO_LTE_AFTER_CSFB
  if((cmd_ptr->cmd_hdr.cmd_id == RRC_STATE_CHANGE_IND) && (cmd_ptr->cmd.state_change_ind.new_state == RRC_STATE_DISCONNECTED)
      && (rrc_csp_int_data.csfb_extended_status == CSFB_CS_RB_RELEASED))
  {
    ...
    rrcrce_int_data.crt_restarted_once = FALSE;
...
{
  return rrcrce_int_data.send_abort_cnf;
}
...
{
  boolean pcco_in_progress = rrccho_is_gtow_pcco_in_progress(wrrc_as_id);
  rrc_abort_cause_e_type rce_abort_ind_cause = RRC_FAIL_TO_CAMP_ON;
  
  if ((cmd_ptr->cmd_hdr.cmd_id == RRC_CPHY_OUT_OF_SERVICE_AREA_IND) || 
      (cmd_ptr->cmd_hdr.cmd_id == RRC_OUT_OF_SERVICE_INTERNAL_IND))
  {
    WRRC_EVENT_REPORT(EVENT_WCDMA_OUT_OF_SERVICE);

    WRRC_MSG2_HIGH("OOS in RCE disconnected state.rrcrce_substate %d  PCCO %d",rrcrce_int_data.substate,pcco_in_progress);

    if(cmd_ptr->cmd_hdr.cmd_id == RRC_OUT_OF_SERVICE_INTERNAL_IND)
    {
      rce_abort_ind_cause = RRC_NO_RESOURCE_AVAILABLE;
    }
    ...
    { 
      ...
      rrcrce_send_conn_setup_ind(wrrc_as_id,conn_setup_ind_params);
    ...
    if(rrcrce_int_data.send_abort_cnf == TRUE)
    {
      rrcrce_send_rrc_abort_cnf(wrrc_as_id); 
    }
    else
    {
      if(rrcrce_int_data.substate != RRCRCE_INITIAL)
      {
        rrcrce_send_rrc_abort_ind(wrrc_as_id, rce_abort_ind_cause);
        rrcrce_int_data.abort_ind_sent = TRUE;
      }
    }
    if(rrcrce_int_data.substate != RRCRCE_WAIT_FOR_RB0_SETUP_CNF)
    {
      rrcrce_int_data.substate = RRCRCE_INITIAL;
      (void) rrcrce_send_rrc_initiate_cell_selection_ind(wrrc_as_id,RRC_OUT_OF_SERVICE_AREA_IND,
  ...
  switch (rrcrce_int_data.substate)
  {
    case RRCRCE_INITIAL: 
      rrcrce_process_rce_initial_substate(wrrc_as_id, cmd_ptr);
      break;
    case RRCRCE_WAIT_FOR_UE_CAMP_ON:
      rrcrce_process_rce_wait_for_ue_camp_on_substate(wrrc_as_id, cmd_ptr);
      break;
    case RRCRCE_WAIT_FOR_SIB7_CURRENT:
      rrcrce_process_rce_wait_for_sib7_current(wrrc_as_id, cmd_ptr);
      break;
    case RRCRCE_WAIT_FOR_RB0_SETUP_CNF: 
      rrcrce_process_rce_wait_for_rb0_setup_cnf_substate(wrrc_as_id, cmd_ptr);
      break;
    case RRCRCE_WAIT_FOR_RRC_CONN_SETUP: 
      rrcrce_process_rce_wait_for_conn_setup_substate(wrrc_as_id, cmd_ptr);
      break;
    case RRCRCE_WAIT_FOR_SRB_SETUP_CNF:
      rrcrce_process_rce_wait_for_srb_setup_cnf_substate(wrrc_as_id, cmd_ptr);
      break;
    case RRCRCE_WAIT_FOR_REDIRECT_CNF:
      rrcrce_process_rce_wait_for_redirect_cnf_substate(wrrc_as_id, cmd_ptr);
      break;
    case RRCRCE_WAIT_FOR_WAIT_TIMER_TO_EXPIRE:
      rrcrce_process_rce_wait_for_wait_time_expire_substate(wrrc_as_id, cmd_ptr);
      break;
    case RRCRCE_WAIT_FOR_RRC_TIMER_TO_EXPIRE:
      rrcrce_process_rce_wait_for_rrc_timer_expire_substate(wrrc_as_id, cmd_ptr);
      break;
    case RRCRCE_WAIT_FOR_CU_COMPLETE_CNF:
      rrcrce_process_rce_wait_for_cu_complete_cnf_substate(wrrc_as_id, cmd_ptr);
      break;
    case RRCRCR_WAIT_FOR_SRB_REL_CNF:
      rrcrce_process_rce_wait_for_srb_rel_cnf_substate(wrrc_as_id, cmd_ptr);
      break;
    case RRCRCE_WAIT_FOR_SIG_CON_REL_COMPL:
      rrcrce_process_rce_wait_for_sig_con_rel_compl_substate(wrrc_as_id, cmd_ptr);
      break;
    case RRCRCE_FINAL: 
      rrcrce_process_rce_final_substate(wrrc_as_id, cmd_ptr);
      break;
    case RRCRCE_WAIT_FOR_RB0_REL_CNF:
      rrcrce_process_rce_wait_for_rb0_rel_cnf_substate(wrrc_as_id, cmd_ptr);
      break;
    case RRCRCE_WAIT_FOR_RESELCT_COMPL:
      rrcrce_process_rce_wait_for_reselct_compl_substate(wrrc_as_id, cmd_ptr);
      break;
    case RRCRCE_WAIT_FOR_RB0_PHYCHAN_RESETUP_CNF:
      rrcrce_process_rce_wait_for_rb0_phychan_resetup_cnf_substate(wrrc_as_id, cmd_ptr);
      break;
    default:           
      WRRC_MSG1_ERROR("Invalid substate in DISCONNECTED State: %d",
                rrcrce_int_data.substate);
      break;
  }  
...
)
{
  ...
  if (
      (cmd_ptr->cmd_hdr.cmd_id == RRC_CPHY_OUT_OF_SERVICE_AREA_IND) || 
      (cmd_ptr->cmd_hdr.cmd_id == RRC_OUT_OF_SERVICE_INTERNAL_IND)
     )
  {
    WRRC_EVENT_REPORT(EVENT_WCDMA_OUT_OF_SERVICE);

    WRRC_MSG2_HIGH("OOS in Connecting state.rrcrce_substate %d PCCO %d ",rrcrce_int_data.substate,pcco_in_progress);
    ...
    rrcrce_send_rrc_channel_config_req_for_rb0(wrrc_as_id,RELEASE_RB, 
                                               RRC_STATE_DISCONNECTED,
                                               FALSE);
    rrcrce_int_data.substate = RRCRCE_WAIT_FOR_RB0_REL_CNF;
    rrcrce_int_data.mm_abort_cause = rce_abort_ind_cause;
    return;
  }
  ...
  switch (rrcrce_int_data.substate)
  {
    ...
    case RRCRCE_WAIT_FOR_RB0_PHYCHAN_RESETUP_CNF:
      ...
      break;
    default:     
      WRRC_MSG1_HIGH("Invalid substate in CONNECTING State: %d",rrcrce_int_data.substate);
      break;
  }  
...
)
{
  ...
  if(    (cmd_ptr->cmd_hdr.cmd_id == RRC_ABORT_REQ)
      ...
      && (rrccsp_get_curr_scan(wrrc_as_id) != RRC_CSP_OUT_OF_SERVICE_AREA_SCAN)
      && (rrccsp_get_curr_scan_no_f3(wrrc_as_id) != RRC_CSP_CELL_DCH_TO_CELL_FACH_SCAN)
    )
  {
  ...
  switch (rrcrce_int_data.substate)
  {
    ...
    case RRCRCE_FINAL: 
      ...
      break;
   default:     
     WRRC_MSG1_HIGH("Invalid substate in CELL_FACH state: %d",rrcrce_int_data.substate);
     break;
  }  
...
)
{
  ...
  { 
    WRRC_MSG1_HIGH("N/W Auth Failed. Rcvd Abort Req from MM in RRC State: %d. ASet bar req sent to L1.", 
                                                                                  rrc_get_state(wrrc_as_id));
    rrccsp_send_cphy_cell_bar_req(wrrc_as_id,RRC_CSP_WCDMA_ASET_BARRED, 1280);
  }
  switch (rrcrce_int_data.substate)
  {
    ...
    case RRCRCE_FINAL: 
      ...
      break;
    default:     
      WRRC_MSG1_HIGH("Invalid substate in CELL_DCH State: %d",rrcrce_int_data.substate);
      break;
  }  
...
)
{
  ...
  switch (rrcrce_int_data.substate)
  {
    ...
    case RRCRCE_WAIT_FOR_RRC_TIMER_TO_EXPIRE:
      ...
      break;
    default:     
      WRRC_MSG1_HIGH("Invalid substate in PCH state: %d", rrcrce_int_data.substate);
      break;
  }  
...
)
{
  ...
  switch (rrcdata_sim_related_info.rrc_initial_ue_identity_choice)
  {
    case RRC_IMSI_UE_ID:
      ...
      rrc_initial_ue_identity_ext->u.imsi->n = rrcdata_sim_related_info.rrc_initial_ue_identity.imsi.num_digits;

      ALLOC_ASN1ARRAY2(&enc_ctxt, rrc_initial_ue_identity_ext->u.imsi, rrc_Digit);
        
      for(i = 0; i < rrcdata_sim_related_info.rrc_initial_ue_identity.imsi.num_digits; i++) 
      {  
        /* Update the digit value */
        rrc_initial_ue_identity_ext->u.imsi->elem[i] = 
          rrcdata_sim_related_info.rrc_initial_ue_identity.imsi.value[i];
    ...
    case RRC_TMSI_LAI_UE_ID:
      ...
      for (i = 0; i < RRC_MAX_GSMMAP_TMSI_LENGTH; i++)
      {
        ...
        *(rrc_initial_ue_identity_ext->u.tmsi_and_LAI->tmsi.data + i) =
          rrcdata_sim_related_info.rrc_initial_ue_identity.tmsi_lai.tmsi.value[i];
      }
      ...
      for (i = 0; i < RRCRCE_MCC_LENGTH ; i++)
      {
        ...
        rrc_initial_ue_identity_ext->u.tmsi_and_LAI->lai.plmn_Identity.mcc.elem[i] =
          rrcdata_sim_related_info.rrc_initial_ue_identity.tmsi_lai.lai.plmn.mcc[i];
      }

      rrc_initial_ue_identity_ext->u.tmsi_and_LAI->lai.plmn_Identity.mnc.n=
        rrcdata_sim_related_info.rrc_initial_ue_identity.tmsi_lai.lai.plmn.num_mnc_digits;
      
      ALLOC_ASN1ARRAY2 (&enc_ctxt, 
        &rrc_initial_ue_identity_ext->u.tmsi_and_LAI->lai.plmn_Identity.mnc, rrc_Digit);

      for (i = 0; i < rrcdata_sim_related_info.rrc_initial_ue_identity.tmsi_lai.lai.plmn.num_mnc_digits;
           i++)
      {
        /* Set Value for MNC */
        rrc_initial_ue_identity_ext->u.tmsi_and_LAI->lai.plmn_Identity.mnc.elem[i] =
          rrcdata_sim_related_info.rrc_initial_ue_identity.tmsi_lai.lai.plmn.mnc[i];
      }      
      ...
      for (i = 0; i < RRC_MAX_GSMMAP_LAC_LENGTH; i++)
      {
        ...
        *(rrc_initial_ue_identity_ext->u.tmsi_and_LAI->lai.lac.data + i ) = 
          rrcdata_sim_related_info.rrc_initial_ue_identity.tmsi_lai.lai.lac[i];
      } 
    ...
    case RRC_PTMSI_RAI_UE_ID:
      ...
      for (i = 0; i < RRC_MAX_GSMMAP_PTMSI_LENGTH; i++)
      {
        ...
        *(rrc_initial_ue_identity_ext->u.p_TMSI_and_RAI->p_TMSI.data + i) =
          rrcdata_sim_related_info.rrc_initial_ue_identity.ptmsi_rai.ptmsi.value[i];
      }
      ...
      for (i = 0; i < RRCRCE_MCC_LENGTH ; i++)
      {
        ...
        rrc_initial_ue_identity_ext->u.p_TMSI_and_RAI->rai.lai.plmn_Identity.
          mcc.elem[i] = rrcdata_sim_related_info.rrc_initial_ue_identity.ptmsi_rai.rai.lai.plmn.mcc[i];          

      }
      rrc_initial_ue_identity_ext->u.p_TMSI_and_RAI->rai.lai.plmn_Identity.mnc.n=
        rrcdata_sim_related_info.rrc_initial_ue_identity.ptmsi_rai.rai.lai.plmn.num_mnc_digits;

      ALLOC_ASN1ARRAY2 (&enc_ctxt, 
        &rrc_initial_ue_identity_ext->u.p_TMSI_and_RAI->rai.lai.plmn_Identity.mnc, rrc_Digit);

      for (i = 0; i < rrcdata_sim_related_info.rrc_initial_ue_identity.ptmsi_rai.rai.lai.plmn.num_mnc_digits;
           i++)
      {
        /* Set Value for MNC */
        rrc_initial_ue_identity_ext->u.p_TMSI_and_RAI->rai.lai.plmn_Identity.mnc.elem[i] =
          rrcdata_sim_related_info.rrc_initial_ue_identity.ptmsi_rai.rai.lai.plmn.mnc[i];
      }    
      ...
      for (i = 0; i < RRC_MAX_GSMMAP_LAC_LENGTH; i++)
      {
        ...
        *(rrc_initial_ue_identity_ext->u.p_TMSI_and_RAI->rai.lai.lac.data  + i ) = 
          rrcdata_sim_related_info.rrc_initial_ue_identity.ptmsi_rai.rai.lai.lac[i];
      }  
      ...
      for (i = 0; i < RRC_MAX_GSMMAP_RAC_LENGTH; i++)
      {
        ...
        *(rrc_initial_ue_identity_ext->u.p_TMSI_and_RAI->rai.rac.data + i ) = 
          rrcdata_sim_related_info.rrc_initial_ue_identity.ptmsi_rai.rai.rac[i];
      } 
      break;
    ...
    case RRC_IMEI_UE_ID:
      ...
      {            
        ...
        rrc_initial_ue_identity_ext->u.imei->elem[i] = 
          rrcdata_sim_related_info.rrc_initial_ue_identity.imei.value[i];
      }       
      break;

    default:
      //This should never happen. Initial UE Identity is already validated
      WRRC_ERR1_FATAL("Invalid Initial UE Id t is rcvd: %d",
...
)
{
  ...
  rrc_translate_establishment_cause(wrrc_as_id, rrcdata_misc_info.rrc_establishment_cause, 
...
)
{
  ...
  rrc_put_int_cmd(wrrc_as_id, cmd_ptr);          /* Sends the command to CSP */
...
)
{
  ...
  cmd_ptr->cmd.chan_config_req.transaction_id = GET_RCE_TRANS_ID();
  rrcrce_int_data.chl_config_transaction_id = (uint8) cmd_ptr->cmd.chan_config_req.transaction_id;
  WRRC_MSG1_HIGH("RB0 Chan config trans_id = %d",rrcrce_int_data.chl_config_transaction_id);
  ...
  rrc_put_int_cmd(wrrc_as_id, cmd_ptr);    /* sends the command to RRC LLC. */
}/*rrcrce_send_rrc_channel_config_req_for_rb0 */
...
#ifdef FEATURE_DUAL_SIM
  cmd_ptr->cmd.rrc_est_cnf.as_id = rrc_get_as_id(wrrc_as_id);
#endif
  WRRC_MSG0_HIGH("Dispatching out MM cmd / Sent RRC_EST_CNF to MM");

  /* Start timer to allow LTA after this timer expiry */
  if(TRM_ACCESS == rrc_determine_trm_priority(wrrc_as_id))
  {
    rrctmr_start_timer(wrrc_as_id, RRCTMR_ALLOW_LTA_DURING_ACCESS_PRIO_TIMER, (rrcnv_data.wcdma_rrc_time_to_allow_lta_during_access_priority * 1000));
  }
  rrc_send_rrc_message_to_mm(wrrc_as_id,cmd_ptr);               /* Sends the command to MM */

#ifdef FEATURE_WCDMA_TO_LTE
  /* Reset flag which indicates pre-redirection info has to be included or not in connection request msg. */
  rrccsp_reset_flag_for_excluding_pre_redir_info(wrrc_as_id);
#endif

  rrcrce_int_data.cell_resel_freq = 0;
  rrcrce_int_data.cell_resel_scr = 0;
}/*rrcrce_send_rrc_est_cnf */
/*===========================================================================

FUNCTION rrc_check_and_start_backoff_timer_for_disabling_dc

DESCRIPTION
  This starts a 20min timer which is maintained at L1 if RRC connection is released with cause unspecified. For normal release
  UE will set the dc capability status to TRUE if 20 min timer is not running.
DEPENDENCIES
  None
...
SIDE EFFECTS
  None
===========================================================================*/
void rrc_check_and_start_backoff_timer_for_disabling_dc
(
sys_modem_as_id_e_type wrrc_as_id
)
{
  /* 
       If UE Is DC capbable, but L1 disabled the DC capability due to failure, at RRC connection release start a 20 min timer,
       so that UE reports SC capability for next 20 mins
   */
  if(l1_is_ue_dc_capable(wrrc_as_id) == FALSE)
  {
    WRRC_MSG2_HIGH("Rel Cause %d, Back off timer val %d",
                    rrcdata_misc_info.rrc_release_cause,
                    l1_get_remaining_time_for_backoff_dc_cap(wrrc_as_id));

    if(l1_get_remaining_time_for_backoff_dc_cap(wrrc_as_id) == 0)
    {
      if(rrcdata_misc_info.rrc_release_cause == RRC_REL_CAUSE_UNSPEC)
      {
        l1_start_backoff_dc_cap_timer(wrrc_as_id);
      }
      else
      {
        /*
          20 min timer is yet to start or timer has expired, and RRC connection is released with cause other than Unspecified. 
          Set the DC capability to TRUE.
        */
        l1_set_dc_capability(TRUE,wrrc_as_id);
        l1_clear_backoff_dc_cap_timer(wrrc_as_id);
      }
    }
  }
}
...
)
{
  ...
  if(rrcrce_int_data.lai_rai_mismatch)
  {
...
#ifdef FEATURE_DUAL_SIM
  /* If connection establishment is aborted because of no resource from WL1
     then send abort indication with cause "no resource" so that NAS retry happens within 2 seconds*/
  if(TRUE == rrc_get_oos_due_to_no_resource_flag_status(wrrc_as_id))
  {
    cause = RRC_NO_RESOURCE_AVAILABLE;
  }  
#endif
...
#ifdef FEATURE_DUAL_SIM
  cmd_ptr->cmd.rrc_abort_ind.as_id = rrc_get_as_id(wrrc_as_id);

  /* NAS should look into this value only when abort cause is RRC_ABORT_CONN_REJECT_WAIT_TMR */
  cmd_ptr->cmd.rrc_abort_ind.conn_rej_wait_timer = rrcrce_int_data.wait_time;
#endif

  /* Update the Max HFNs values to be stored in SIM or USIM */ 
  (void) rrcsmc_append_hfns_to_rel_ind(wrrc_as_id,cmd_ptr->cmd.rrc_abort_ind.hfn_cs,
                                       cmd_ptr->cmd.rrc_abort_ind.hfn_ps);

  WRRC_MSG2_HIGH("Dispatching out MM cmd / Sent RRC_ABORT_IND to MM with cause: %d substate %d", cause,rrcrce_int_data.substate);
  rrc_send_rrc_message_to_mm(wrrc_as_id,cmd_ptr);               /* Sends the command to MM */
  ...
  {
    rrcrce_send_conn_rel_cause_event(wrrc_as_id, RRC_ACCESS_CLASS_NOT_ALLOWED);
  }
  else
  {
    rrcrce_send_conn_rel_cause_event(wrrc_as_id,cause);
  }

#ifdef FEATURE_DUAL_SIM
  rrc_release_lock(wrrc_as_id);
  rrc_dsds_int_data.nas_proc_type = SYS_PROC_TYPE_NONE;
  rrctmr_stop_timer(wrrc_as_id, RRCTMR_TRM_PRIORITY_CHANGE_TIMER);
#endif
  ...
  if(rrcdata_timers_counters_info.rrc_v_300)
  {
    /* Reset flag which indicates pre-redirection info has to be included or not in connection request msg. */
    rrccsp_reset_flag_for_excluding_pre_redir_info(wrrc_as_id);
  }
  ...
  rrc_check_and_start_backoff_timer_for_disabling_dc(wrrc_as_id);

  /* Initialize release cause */
  rrcdata_misc_info.rrc_release_cause = RRC_REL_CAUSE_UNSPEC;

  rrcrce_int_data.cell_resel_freq = 0;
  rrcrce_int_data.cell_resel_scr = 0;

#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
  rrcrce_qsh_log_connection_est_failure(wrrc_as_id);
#endif

  /* Vote for APS off when abort_ind is sent */
...
)
{
  ...
  rrcrce_int_data.send_abort_cnf = FALSE;

  rrcrce_int_data.sig_con_delay = FALSE;
  ...
  /* Update the Max HFNs values to be stored in SIM or USIM */ 
  (void) rrcsmc_append_hfns_to_rel_ind(wrrc_as_id,cmd_ptr->cmd.rrc_abort_cnf.hfn_cs,
                                       cmd_ptr->cmd.rrc_abort_cnf.hfn_ps);

  cmd_ptr->cmd.rrc_abort_cnf.conn_rel_cause = rrcdata_misc_info.rrc_release_cause;

  rrc_send_rrc_message_to_mm(wrrc_as_id,cmd_ptr);               /* Sends the command to MM */
WRRC_MSG2_HIGH("Dispatching out MM cmd / Sent RRC_ABORT_CNF to MM with %d substate, rel_cause %d",rrcrce_int_data.substate, cmd_ptr->cmd.rrc_abort_cnf.conn_rel_cause);

  rrcrce_send_conn_rel_cause_event(wrrc_as_id,(uint8)RRC_ABORT_CAUSE_TYPE_MAX + (uint8)rrcdata_misc_info.rrc_release_cause);   
#ifdef FEATURE_DUAL_SIM
  rrc_release_lock(wrrc_as_id);
  rrc_dsds_int_data.nas_proc_type = SYS_PROC_TYPE_NONE;
  rrctmr_stop_timer(wrrc_as_id, RRCTMR_TRM_PRIORITY_CHANGE_TIMER);
...
void rrcrce_qsh_log_connection_est_failure(sys_modem_as_id_e_type wrrc_as_id)
{
...
/*==========================================================================
FUNCTION          RRCRCE_SSGCCS_SEND_RRC_CONNECTION_REQUEST_INFO

DESCRIPTION       This function sends the RRC connection request info to SSGCCS QMI client 
...
SIDE EFFECTS
                  None
=============================================================================*/
void rrcrce_ssgccs_send_rrc_connection_request_info(sys_modem_as_id_e_type wrrc_as_id           )
{
  wcdma_ssgccs_rrc_connection_request_info_ind_s connection_request_info;
  if(connection_request_3g_msg_handler == NULL)
  {
    WRRC_MSG0_ERROR("SSGCCS:RRC Connection request info msg handler NULL");
    return;
  }
  memset(&connection_request_info,0,sizeof(wcdma_ssgccs_rrc_connection_request_info_ind_s));
  connection_request_info.wrrc_as_id = wrrc_as_id;
  connection_request_info.mcc = RRC_CSP_GET_INT_MCC(rrc_csp_int_data.curr_acq_entry.plmn_id);
  connection_request_info.mnc = RRC_CSP_GET_INT_MNC(rrc_csp_int_data.curr_acq_entry.plmn_id);
  connection_request_info.cellid = rrc_csp_int_data.curr_acq_entry.cell_id;
  connection_request_info.rrc_initial_ue_identity_choice = rrcdata_sim_related_info.rrc_initial_ue_identity_choice;
  connection_request_info.rrc_initial_ue_identity = rrcdata_sim_related_info.rrc_initial_ue_identity;
  WRRC_MSG0_HIGH("SSGCCS: Sending RRC connection request info to SSGCCS QMI Client ");
  connection_request_3g_msg_handler(&connection_request_info);
}
...
)
{
  ...
  {
    case RRC_EST_REQ: 
      rrcdata_misc_info.rrc_protocol_error_indicator = rrc_ProtocolErrorIndicator_noError;

      /* Resets the V300 variable */
      rrcdata_timers_counters_info.rrc_v_300 = 0;
   
      /* Initialize the Transaction Id for RRC CONNECTION SETUP to Invalid */
      rrcrce_int_data.setup_transaction_identifier = RRCRCE_INVALID_TRANSACTION_ID;
      
      /* Initialize the Transaction Id for RRC CONNECTION REJECT to Invalid */
      rrcrce_int_data.reject_transaction_identifier = RRCRCE_INVALID_TRANSACTION_ID;

      /* Initialize the Wait Time to zero seconds */
      rrcrce_int_data.wait_time = 0;

      /* Initialize abort flag to FALSE */
      rrcrce_int_data.send_abort_cnf = FALSE;

      rrcrce_int_data.abort_ind_sent = FALSE;
      rrcrce_int_data.lai_rai_mismatch = FALSE;
#ifdef FEATURE_DUAL_SIM
      rrc_dsds_int_data.nas_proc_type = cmd_ptr->cmd.rrc_est_req.proc_type;
#endif

      /* Initialize release cause to unspecified */
      rrcdata_misc_info.rrc_release_cause = RRC_REL_CAUSE_UNSPEC;

      if(RRCRCE_SUCCESS == rrcrce_validate_rrc_est_req(wrrc_as_id, cmd_ptr))
      {
        /*updates the establishment cause */
        rrcdata_misc_info.rrc_establishment_cause = cmd_ptr->cmd.rrc_est_req.cause;
      
        /* Updates Initial UE Identity */
        rrcdata_sim_related_info.rrc_initial_ue_identity_choice = cmd_ptr->cmd.rrc_est_req.ue_id_choice;

        rrcdata_sim_related_info.rrc_initial_ue_identity = cmd_ptr->cmd.rrc_est_req.initial_ue_id;

        rrcrce_int_data.est_cn_domain_id = cmd_ptr->cmd.rrc_est_req.cn_domain_id;

        rrcrce_int_data.est_call_type = cmd_ptr->cmd.rrc_est_req.call_type;

        rrcrce_int_data.est_req_for_rai = cmd_ptr->cmd.rrc_est_req.rai;

        WRRC_MSG8_HIGH(
             "RRC_EST_REQ for est_cause %d, proc type %d, cn_domain: %d, RAI PLMN %d-%d, RAC %d, LAC: %x %x",
              rrcdata_misc_info.rrc_establishment_cause, rrc_dsds_int_data.nas_proc_type, rrcrce_int_data.est_cn_domain_id,RRC_CSP_GET_INT_MCC(rrcrce_int_data.est_req_for_rai.lai.plmn),
             RRC_CSP_GET_INT_MNC(rrcrce_int_data.est_req_for_rai.lai.plmn),rrcrce_int_data.est_req_for_rai.rac[0], rrcrce_int_data.est_req_for_rai.lai.lac[0],
             rrcrce_int_data.est_req_for_rai.lai.lac[1]);

         /* Sending RRC connection request info to SSGCCS QMI client */
        rrcrce_ssgccs_send_rrc_connection_request_info(wrrc_as_id);

        rrcrce_send_conn_req_cause_event(wrrc_as_id,rrcdata_misc_info.rrc_establishment_cause);

        /* Function call to CSP to know whether UE is camped on  */
        if (RRC_CAMPED_ON == rrc_ccm_get_curr_camping_status(wrrc_as_id,RRC_PROCEDURE_RCE))
        {
          if(RRCRCE_SUCCESS == rrcrce_validate_cell_access_status(wrrc_as_id,
                                   rrcrce_int_data.est_cn_domain_id,
                                   rrcdata_misc_info.rrc_establishment_cause,
                                   &cause)
            )
          {
            if(WTOW_SEARCH_STOP != rrc_csp_bplmn_srch_in_progress(wrrc_as_id))
            {
               ...
               */
              rrcrce_int_data.substate = RRCRCE_WAIT_FOR_UE_CAMP_ON;
              /* With the below feature defined we will suspend and resume
               * the BPLMN search for any establishment cause received 
               * from NAS
               */
              rrc_csp_send_bplmn_suspend_req(wrrc_as_id,RRC_PROCEDURE_RCE, BPLMN_SUSPEND_REASON_REGISTRATION);
               ...
               */
              rrctmr_start_timer(wrrc_as_id,RRCTMR_UE_TO_CAMP_ON_TIMER, 
                            (unsigned long) RRCRCE_TIMER_VALUE_FOR_UE_TO_CAMP_ON);       
            }
#ifdef FEATURE_DUAL_SIM
            else if(wl1_is_bplmn_active(wrrc_as_id) && rrcllc_is_pccpch_active(wrrc_as_id))
            {
              rrcrce_int_data.substate = RRCRCE_WAIT_FOR_UE_CAMP_ON;
              rrctmr_start_timer(wrrc_as_id,RRCTMR_UE_TO_CAMP_ON_TIMER, 
                            (unsigned long) RRCRCE_TIMER_VALUE_FOR_UE_TO_CAMP_ON);
            ...
            {
#ifdef FEATURE_DUAL_SIM
              if(rrc_acquire_lock_for_est_req(wrrc_as_id,rrcdata_misc_info.rrc_establishment_cause,0xfffffff,RRCTMR_WRM_LOCK_WAIT_TIMER_FOR_EST_REQ_IN_MS) == FAILURE)
              {
                /* Send RRC_ABORT_IND command to the MM */
                rrcrce_send_rrc_abort_ind(wrrc_as_id, RRC_NO_RESOURCE_AVAILABLE);      
                return;
              }
#endif
               /* UE is camped on, then Check whether SIB7 is current */
              if(TRUE == rrcsib_check_and_update_sib7_validity (wrrc_as_id, RRC_PROCEDURE_RCE))
              { 
                 /* UE is camped on and SIB7 is current, then Setup RB0 */
                 rrcrce_send_rrc_channel_config_req_for_rb0(wrrc_as_id,ESTABLISH_RB,
                                                           RRC_STATE_CONNECTING,
                                                           TRUE ); 
                 rrcrce_int_data.substate = RRCRCE_WAIT_FOR_RB0_SETUP_CNF;     
              }    
              else
              {
                WRRC_MSG0_HIGH("RCE waits for SIB7 to become current"); 
                rrcrce_int_data.substate = RRCRCE_WAIT_FOR_SIB7_CURRENT;
              }
            }
          }
          else
          {
             ...
             */
            if(rrcdata_misc_info.rrc_establishment_cause == RRC_EST_EMERGENCY_CALL)
            {
              rrcrce_send_rrc_initiate_cell_selection_ind(wrrc_as_id, RRC_OUT_OF_SERVICE_AREA_IND,rrc_get_state(wrrc_as_id));
              rrccsp_set_est_req_for_emerg_call(wrrc_as_id);
            }
#ifdef FEATURE_WCDMA_CSFB_CALL_OPT
            else if((WCDMA_RRC_IDLE_FEATURES(RRC_CSFB_CALL_TIMELINE_OPT)) && rrc_csp_int_data.rrc_csfb_call_status)
            {
              //Trigger OOS to avoid UE being stuck in BCH.
              rrcrce_send_rrc_initiate_cell_selection_ind(wrrc_as_id, RRC_OUT_OF_SERVICE_AREA_IND,rrc_get_state(wrrc_as_id));
            }
#endif
            if(rrcrce_int_data.est_cn_domain_id == RRC_CS_DOMAIN_CN_ID)
            {
              rrcrce_send_rrc_abort_ind(wrrc_as_id, RRC_AC_NOT_ALLOWED_CS_DOMAIN);
            }
            else
            {
              rrcrce_send_rrc_abort_ind(wrrc_as_id, RRC_AC_NOT_ALLOWED_PS_DOMAIN);
            }
          }
        }
...
#endif
        /* Send RRC_ABORT_IND command to the MM */
        rrcrce_send_rrc_abort_ind(wrrc_as_id, RRC_UNSUPPORTED_INITIAL_UE_ID);     
    ...
    case RRC_STATE_CHANGE_IND:
      MSG_MED("Unexpected Event: %d in the substate:%d",
               cmd_ptr->cmd_hdr.cmd_id, rrcrce_int_data.substate, 0);
      break;
    ...
    case RRC_ABORT_REQ:
      ...
      rrcrce_send_rrc_abort_cnf(wrrc_as_id);      
        /* At this time, none of the RBs are setup. Simply reset the RCE
         substate */
      rrcrce_int_data.substate = RRCRCE_INITIAL; /* The RCE Procedure ends */
      break;

    default:     
      WRRC_MSG2_HIGH("Invalid Event: %d in the substate: %d", 
                cmd_ptr->cmd_hdr.cmd_id, rrcrce_int_data.substate);
      break;
  }  
    ...
    case RRC_SIB7_UPDATE_STATUS_IND: /* This command actually represents
      ...
      if (cmd_ptr->cmd.sib7_update_status.status == FAILURE)
      {
        ...
        { 
          ...
          rrcrce_int_data.substate = RRCRCE_INITIAL;    /* reset the RCE substate */
          break;
        }
        
        rrcrce_send_rrc_abort_ind(wrrc_as_id, RRC_INTERNAL_ERROR);      
        /* The RCE Procedure ends */
        rrcrce_int_data.substate = RRCRCE_INITIAL;    /* reset the RCE substate */
        break;
      }    
...
)
{
  ...
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_NEW_CELL_IND:  /* This command actually represents UE Camp on 
      ...
      rrctmr_stop_timer(wrrc_as_id, RRCTMR_UE_TO_CAMP_ON_TIMER);
    ...
    case RRC_BPLMN_SUSPEND_CNF:
#ifdef FEATURE_DUAL_SIM
      /* Wait for suspend cnf to be sent by LLC */
      if(wl1_is_bplmn_active(wrrc_as_id) && rrcllc_is_pccpch_active(wrrc_as_id))
      {
        WRRC_MSG0_HIGH("Ignoring RRC_BPLMN_SUSPEND_CNF as PCCPCH drop is not sent yet");
        return;
      }
#endif
      /* Stop the timer to UE to camp on */  
      ...
      {
        /* Send RRC_ABORT_IND command to the MM */
        rrcrce_send_rrc_abort_ind(wrrc_as_id, RRC_FAIL_TO_CAMP_ON);     
        rrcrce_int_data.substate = RRCRCE_INITIAL;    /* reset the RCE substate */
        }    
      break;
    case RRC_TIMER_TO_CAMP_ON_EXPIRED_IND:      

      WRRC_MSG1_HIGH("RRC_TIMER_TO_CAMP_ON_EXPIRED_IND in rce substate %d",rrcrce_int_data.substate);

      /* If GTOW is in progress,send a conn_setup_ind with failure to CHO.
         CHO would deactivate WCDMA and pass control back to GSM.Do not send
         RRC_ABORT_IND to MM if GTOW is active */
      if(rrccho_is_gtow_pcco_in_progress(wrrc_as_id))
      { 
        rrc_conn_setup_ind_type conn_setup_ind_params;
        conn_setup_ind_params.status = FAILURE;
        conn_setup_ind_params.conn_failure_reason = RRC_CONN_FAILURE_OTHER;
        
        WRRC_MSG1_HIGH("RRC_TIMER_TO_CAMP_ON_EXPIRED_IND when PCCO active in rce substate %d",rrcrce_int_data.substate);
        rrcrce_send_conn_setup_ind(wrrc_as_id,conn_setup_ind_params);
  ...
  else
  {
    if(sib19_ptr->m.eutra_FrequencyAndPriorityInfoListPresent)
    {
      ...
      while((eutra_freq_pri_list_ptr->n > idx))
      {
        ...
        if(lte_rrc_earfcn_is_supported(rrc_get_as_id(wrrc_as_id), earfcn, rrccsp_get_lte_band_pref(wrrc_as_id)))
#else
        if(lte_rrc_earfcn_is_supported(earfcn, rrccsp_get_lte_band_pref(wrrc_as_id)))
#endif
        {
          break;
        }
...
)
{
  ...
  rrc_RRCConnectionRequest_v860NonCriticalExtensions *rce_v7e0_ext = NULL;
  boolean supportoffrequencyband[2];
#endif

  rrc_T_300 rrcrce_t_300 = rrc_T_300_ms1000; /* Local parameter to store T300 */
  rrc_N_300 rrcrce_n_300;
  ...
  rrcrce_get_t300_from_serv_cell_db(wrrc_as_id,&rrcrce_t_300);

  rrcrce_get_n300_from_serv_cell_db(wrrc_as_id,&rrcrce_n_300);

  WRRC_MSG7_HIGH("Build RRC connection request REL NV : %d, Est cause %d, LTE mode %d CSFB call sts %d, T300 : %d, N300 : %d, Exclude pre redir info %d", rrc_nv_rel_indicator, rrcdata_misc_info.rrc_establishment_cause, rrcmcm_is_lte_mode_enabled_wo_f3(wrrc_as_id),rrc_csp_int_data.rrc_csfb_call_status,rrcrce_t_300, rrcrce_n_300, rrccsp_is_preredirection_info_to_be_excluded(wrrc_as_id));
  /* Set the connection establishment cause properly if there
   * was a GTOW reselection or GTOW PCCO
   */
  WRRC_MSG1_MED("Est Cause %d", rrcdata_misc_info.rrc_establishment_cause);
  rrcmcm_set_establishment_cause(wrrc_as_id);

  /* To update the proc id as RCE so that first IDT can be sent without cause */
  rrcidt_update_proc_id(wrrc_as_id);

  /* Initializes the RRC Connection Establishment cause */
  msg_ptr->message.u.rrcConnectionRequest.establishmentCause = 
    rrcrce_update_establishment_cause(wrrc_as_id);

  /* Initializes the initial UE Id parameter */
  rrcrce_update_initial_ue_identity(wrrc_as_id,
  ...
  msg_ptr->message.u.rrcConnectionRequest.protocolErrorIndicator = 
    rrcdata_misc_info.rrc_protocol_error_indicator;
 ...
 /* Function call to Measurement Control & Report procedure. */
  if(RRCMEAS_RACH_SUCCESSFUL == rrcmeas_append_meas_rpt_on_rach(wrrc_as_id,
  ...
  {
    WRRC_MSG0_MED("Setting REL 5 IN CON.REQ");
  ...
  {
    WRRC_MSG0_MED("RRCEUL: Setting REL 6 IN CON.REQ");
    ...
    if ((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL7) && (!(rrcnv_data.rrc_disable_cpc_fdpch_nv & NV_FDPCH_DISABLE)))
    {
      msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.
        v4b0NonCriticalExtensions.v590NonCriticalExtensions.v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.
      ...
      WRRC_MSG0_MED("FPDCH capability set");
    }
    ...
    {
      WRRC_MSG2_MED("RRCEUL: optional feature EUL/HSUPA(0x%x->0x%x) disabled through NV item",
        rrcnv_data.wcdma_nv_optional_feature_list, CHECK_WCDMA_OPTIONAL_FEATURE_HSUPA_SUPPORTED);
    ...
    if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL7)
    {
     RRC_RESET_MSG_IE_PRESENT(
      ...
      if(rrcenhstate_nv_supports_e_fach(wrrc_as_id) == TRUE)
      {
        ...
        WRRC_MSG0_MED("ENHSTATE:Sending Enh Cell_FACH capability");
      }
#endif /*FEATURE_WCDMA_HS_FACH*/

     if(rrcueci_nv_supports_cpc_dtx_on_dpcch(wrrc_as_id))
     {
        WRRC_MSG0_MED("RRCCPC:Indicate UE support CPC-DTX operation on UL DPCCH");
      ...
      if(l1_get_mobility_info(wrrc_as_id))
      {
        WRRC_MSG0_MED("High Mobility Detected");
       RRC_MSG_COMMON_SET_BITMASK_IE(msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.
          v4b0NonCriticalExtensions.v590NonCriticalExtensions.v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.
          v770NonCriticalExtensions.rrcConnectionRequest_v770ext,
      ...
      {
        WRRC_MSG0_MED("High Mobility not Detected");
      }
#endif

      if(!(rrcnv_data.rrc_disable_cpc_fdpch_nv & (NV_FDPCH_DISABLE|NV_EFDPCH_DISABLE)))
      {
        msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.
          v4b0NonCriticalExtensions.v590NonCriticalExtensions.v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.
    ...
    */
    WRRC_MSG3_MED("rrc_est_cn_domain_id = %d,rrcdata_misc_info.rrc_establishment_cause =%d,est_call_type = %d",
                                                                                                  rrcrce_int_data.est_cn_domain_id,
                                                                                                  rrcdata_misc_info.rrc_establishment_cause,
                                                                                                  rrcrce_int_data.est_call_type);
    switch (rrcrce_int_data.est_cn_domain_id) 
    {
      case RRC_CS_DOMAIN_CN_ID:
        ...
        if((rrcdata_misc_info.rrc_establishment_cause == RRC_EST_ORIG_CONV_CALL)||
           (rrcdata_misc_info.rrc_establishment_cause == RRC_EST_EMERGENCY_CALL)
          )
        {
          switch(rrcrce_int_data.est_call_type)
          {
            ...
            default:
              WRRC_MSG1_ERROR("Wrong est call type set by NAS = %d",rrcrce_int_data.est_call_type);
              msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.
                 v4b0NonCriticalExtensions.v590NonCriticalExtensions.v690NonCriticalExtensions.
                 rrcConnectionRequest_v690ext.domainIndicator.u.cs_domain.csCallType = rrc_RRCConnectionRequest_v690ext_IEs_csCallType_other; 
              break;
      ...
      default :
        msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.
          v4b0NonCriticalExtensions.v590NonCriticalExtensions.v690NonCriticalExtensions.
          rrcConnectionRequest_v690ext.domainIndicator.t = T_rrc_RRCConnectionRequest_v690ext_IEs_domainIndicator_ps_domain;
        WRRC_MSG1_ERROR("Invalid CN domain I.D=%d",rrcrce_int_data.est_cn_domain_id);
        break;
    }
#ifdef FEATURE_WCDMA_INTER_FREQ_RACH_REPORTING
    #error code not present
  ...
  if(rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL7)
  {
    WRRC_MSG0_MED("RRCEUL: Setting REL 7 IN CON.REQ");
    /* Indicate that we are a Release 6 UE */
    msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.
  ...
  {
    WRRC_MSG0_MED("REL8:Setting AS Rel Indicator in RRC Conn Request.");
   ...
   RRC_RESET_MSG_IE_PRESENT(msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
    ...
    if((rrcueci_nv_supports_dc_hsdpa(wrrc_as_id) == TRUE) && 
       (!rrccsp_check_if_em_plmn(wrrc_as_id)))
    {
      msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
          v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.
    ...
    if(rrcmcm_is_lte_mode_enabled_wo_f3(wrrc_as_id) &&
     !rrccsp_is_preredirection_info_to_be_excluded(wrrc_as_id) &&
     rrcrce_is_band_supported_for_eutra_neighbors(wrrc_as_id))
    {
      msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
          v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.
          v860NonCriticalExtensions.rrcConnectionRequest_v860ext.m.pre_redirectionInfoPresent = 1;

      if(lte_rrc_lte_fdd_is_supported(rrccsp_get_lte_band_pref(wrrc_as_id)))
      {
        msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
            v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.
      ...
      if(lte_rrc_lte_tdd_is_supported(rrccsp_get_lte_band_pref(wrrc_as_id)))
      {
        msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
            v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.
    ...
    if(rrcueci_nv_supports_mac_i(wrrc_as_id) == TRUE)
    {
      ...
      WRRC_MSG0_MED("MAC_I support added in RRC conn req");
      msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
          v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.
...
#endif /* FEATURE_MAC_I */
#ifdef FEATURE_WCDMA_HS_RACH
    if (TRUE == rrchsrach_nv_supports_hs_rach(wrrc_as_id))
    {
            /*Add the Mac-i support*/
          WRRC_MSG0_MED("RRCHSRACH HS-RACH support added in RRC conn req");
          msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
          v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.
  ...
  {
    WRRC_MSG0_MED("REL9:Setting AS Rel Indicator in RRC Conn Request.");
  ...
  {
    WRRC_MSG0_MED("REL10:Setting AS Rel Indicator in RRC Conn Request.");
    /* Indicate that we are a Release 9 UE */
    msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.
      rrcConnectionRequest_v4b0ext.accessStratumReleaseIndicator = rrc_AccessStratumReleaseIndicator_rel_10;
  
  #ifdef FEATURE_WCDMA_3C_HSDPA
  
    if(rrcueci_nv_supports_3c_hsdpa(wrrc_as_id) == TRUE)
    {
      msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
      v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.m.
      v7b0NonCriticalExtensionsPresent =1;
      ...
      msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
      v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.v860NonCriticalExtensions.m.
      v7e0NonCriticalExtensionsPresent = 1;
      
      RRC_RESET_MSG_IE_PRESENT(msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
      v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.
      v860NonCriticalExtensions.v7e0NonCriticalExtensions);
      
      msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
      v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.v860NonCriticalExtensions.
      v7e0NonCriticalExtensions.m.v7g0NonCriticalExtensionsPresent=1;
      
      RRC_RESET_MSG_IE_PRESENT(msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
      v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.
      v860NonCriticalExtensions.v7e0NonCriticalExtensions.v7g0NonCriticalExtensions);
      
      msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
      v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.v860NonCriticalExtensions.
      v7e0NonCriticalExtensions.v7g0NonCriticalExtensions.m.v920NonCriticalExtensionsPresent=1;
      
      RRC_RESET_MSG_IE_PRESENT(msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
      v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.
      v860NonCriticalExtensions.v7e0NonCriticalExtensions.v7g0NonCriticalExtensions.v920NonCriticalExtensions);
      
      msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
      v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.v860NonCriticalExtensions.
      v7e0NonCriticalExtensions.v7g0NonCriticalExtensions.v920NonCriticalExtensions.m.v940NonCriticalExtensionsPresent =1;
      
      RRC_RESET_MSG_IE_PRESENT(msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
      v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.
      v860NonCriticalExtensions.v7e0NonCriticalExtensions.v7g0NonCriticalExtensions.v920NonCriticalExtensions.v940NonCriticalExtensions);
      
      msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
      v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.v860NonCriticalExtensions.
      v7e0NonCriticalExtensions.v7g0NonCriticalExtensions.v920NonCriticalExtensions.v940NonCriticalExtensions.m.va40NonCriticalExtensionsPresent=1;
      
      RRC_RESET_MSG_IE_PRESENT(msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
      v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.
      v860NonCriticalExtensions.v7e0NonCriticalExtensions.v7g0NonCriticalExtensions.v920NonCriticalExtensions.v940NonCriticalExtensions.va40NonCriticalExtensions);
      
      
      RRC_RESET_MSG_IE_PRESENT(msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
      v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.v860NonCriticalExtensions.
      v7e0NonCriticalExtensions.v7g0NonCriticalExtensions.v920NonCriticalExtensions.v940NonCriticalExtensions.va40NonCriticalExtensions.rrcConnectionRequest_va40ext);
      
      msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
      v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.v860NonCriticalExtensions.
      v7e0NonCriticalExtensions.v7g0NonCriticalExtensions.v920NonCriticalExtensions.v940NonCriticalExtensions.va40NonCriticalExtensions.
      rrcConnectionRequest_va40ext.m.supportOfMoreThanTwoCellsPresent=1;
      /*
               spec 25.331, sec:	8.1.3.3 
            2>	if the UE supports multi-cell operation on three cells and/or four cells
            3>	if the UE is of HS-DSCH category 30 or 32 according to [35] 
            4>	include the IE "More than two cell support" and set it to "higher rate".
            3>	otherwise:
            4>	include the IE "More than two cell support" and set it to "lower rate".
			*/      
      msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.v690NonCriticalExtensions.
      v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.v770NonCriticalExtensions.v7b0NonCriticalExtensions.v860NonCriticalExtensions.v7e0NonCriticalExtensions.
      v7g0NonCriticalExtensions.v920NonCriticalExtensions.v940NonCriticalExtensions.va40NonCriticalExtensions.rrcConnectionRequest_va40ext.
      supportOfMoreThanTwoCells=rrc_RRCConnectionRequest_va40ext_IEs_supportOfMoreThanTwoCells_lowerRate;
    }
  #endif
  }
#endif
...
#endif
  {
    rrc_plmn_identity_type src_rat_plmn_id; /* dummy for non IHO cases*/
  ...
  if(TRUE == rrc_csp_int_data.rrc_csfb_call_status)
  {
...
#endif
    WRRC_MSG1_MED("CSFB: Setting CSFB Indication %d [0 : SET]",rce_v940_ext->va40NonCriticalExtensions.v9b0NonCriticalExtensions.
  ...
  /*check if we support any of the bands passed in SIB5*/
  rrcsib_check_for_redirection_bands_support(wrrc_as_id,supportoffrequencyband);

  /*indicate the capability of the supported band in RRC connection request message*/
  if(supportoffrequencyband[0] || supportoffrequencyband[1])
  {

    rce_v7e0_ext = &msg_ptr->message.u.rrcConnectionRequest.v3d0NonCriticalExtensions.
                   v4b0NonCriticalExtensions.v590NonCriticalExtensions.v690NonCriticalExtensions.v6b0NonCriticalExtensions.v6e0NonCriticalExtensions.
                   v770NonCriticalExtensions.v7b0NonCriticalExtensions.v860NonCriticalExtensions;
    rce_v7e0_ext->m.v7e0NonCriticalExtensionsPresent = 1;
    rce_v7e0_ext->v7e0NonCriticalExtensions.m.v7g0NonCriticalExtensionsPresent = 1;
    rce_v7e0_ext->v7e0NonCriticalExtensions.v7g0NonCriticalExtensions.m.v920NonCriticalExtensionsPresent = 1;
    rce_v7e0_ext->v7e0NonCriticalExtensions.v7g0NonCriticalExtensions.v920NonCriticalExtensions.m.v940NonCriticalExtensionsPresent = 1;
    rce_v7e0_ext->v7e0NonCriticalExtensions.v7g0NonCriticalExtensions.v920NonCriticalExtensions.v940NonCriticalExtensions.m.va40NonCriticalExtensionsPresent = 1;
    if(supportoffrequencyband[0])
    {
      rce_v7e0_ext->v7e0NonCriticalExtensions.v7g0NonCriticalExtensions.v920NonCriticalExtensions.v940NonCriticalExtensions.va40NonCriticalExtensions.rrcConnectionRequest_va40ext
                          .m.supportOf1stFrequencyBandPresent =  1;
      rce_v7e0_ext->v7e0NonCriticalExtensions.v7g0NonCriticalExtensions.v920NonCriticalExtensions.v940NonCriticalExtensions.va40NonCriticalExtensions.rrcConnectionRequest_va40ext
                          .supportOf1stFrequencyBand =  rrc_RRCConnectionRequest_va40ext_IEs_supportOf1stFrequencyBand_true;

      WRRC_MSG0_HIGH("first frequency band for redirection is supported");    
    }
    if(supportoffrequencyband[1])
    {
      rce_v7e0_ext->v7e0NonCriticalExtensions.v7g0NonCriticalExtensions.v920NonCriticalExtensions.v940NonCriticalExtensions.va40NonCriticalExtensions.rrcConnectionRequest_va40ext
                         .m.supportOf2ndFrequencyBandPresent =  1;
      rce_v7e0_ext->v7e0NonCriticalExtensions.v7g0NonCriticalExtensions.v920NonCriticalExtensions.v940NonCriticalExtensions.va40NonCriticalExtensions.rrcConnectionRequest_va40ext
                         .supportOf2ndFrequencyBand =  rrc_RRCConnectionRequest_va40ext_IEs_supportOf2ndFrequencyBand_true;

      WRRC_MSG0_HIGH("second frequency band for redirection is supported");  
    }
  }
#endif
#endif
...
if(rrcdata_misc_info.rrc_protocol_error_indicator == rrc_ProtocolErrorIndicator_errorOccurred)
{
  /*Reports Connection request protocol error to QSH*/
  rrc_qsh_report_failure_ota_msg_event(wrrc_as_id,WRRC_UL_CCCH_MESSAGETYPE_RRCCONNECTIONREQUEST);
}
#endif

#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
  conn_req_ota.cn_id = rrcrce_int_data.est_cn_domain_id;
  conn_req_ota.est_cause = rrcdata_misc_info.rrc_establishment_cause;
  ...
  rrc_qsh_log_conn_req_params(wrrc_as_id, &conn_req_ota);
#endif
} /* rrcrce_build_rrc_connection_request_msg  */
...
)
{
  ...
  else
  {
    /* The UE should have camped and received a valid SIB1 before UE initiates
       RRC Connection Establishment procedure. */
    WRRC_ERR0_FATAL("SIB Database does not have a valid SIB 1");
  }
  return (status);
...
)
{
  ...
  rrc_SysInfoType1 *rrcrce_sib1;            /* Pointer to SIB 1 */
    /* Read SIB 1 from the SIB database */
  rrcrce_sib1 = (rrc_SysInfoType1 *)rrc_sibdb_return_sib_for_srv_cell(wrrc_as_id, rrc_SIB1);
  
    /* Check whether SIB 1 is a valid one */
    if (rrcrce_sib1 != NULL)
    {
      *rrcrce_n_300= rrcrce_sib1 ->ue_IdleTimersAndConstants.n_300;
      WRRC_MSG1_MED("N300 is %d",rrcrce_sib1 ->ue_IdleTimersAndConstants.n_300); 
      status = RRCRCE_SUCCESS;
    }
    else
    {
...
)
{
  ...
  WRRC_MSG1_MED("T300 is %d ms",timer_value); 
  return (timer_value);
...
)
{
  ...
  {
    case RRC_ABORT_REQ:
      rrcrce_int_data.send_abort_cnf = TRUE;
      break;

    case RRC_STATE_CHANGE_IND:
      WRRC_MSG0_HIGH("State change ind received by RCE");
      /* Stop the timer T300 */
      rrctmr_stop_timer(wrrc_as_id, RRCTMR_T_300_TIMER);
      ...
      if (cmd_ptr->cmd.state_change_ind.new_state == RRC_STATE_DISCONNECTED)
      {
        ...
        {     
          rrcrce_send_rrc_abort_ind(wrrc_as_id, RRC_FAILURE_UNSPECIFIED);     
          rrcrce_int_data.substate = RRCRCE_INITIAL;    /* reset the RCE substate */
        }    
      }
    ...
    case RRC_CHANNEL_CONFIG_CNF:
      ...
      if(cmd_ptr->cmd.chan_config_cnf.procedure == RRC_PROCEDURE_RCE)
      {
        ...
        if(cmd_ptr->cmd.chan_config_cnf.transaction_id == rrcrce_int_data.chl_config_transaction_id)
        {
          if( RRCLLC_CHAN_CFG_SUCCESS == cmd_ptr->cmd.chan_config_cnf.chan_cfg_status ) 
          {
            if(rrcrce_int_data.send_abort_cnf)
            {
               /* Do not send command to LLC to release SRBs Instead send Initiate Cell Selection Indication */
               rrcrce_send_rrc_initiate_cell_selection_ind(wrrc_as_id,
            ...
            else
            {
                                    ...
                                    &cause))
              {
                ...
                rrcrce_send_rrc_connection_request_msg(wrrc_as_id);
                /* The RCE procedure waits for the RRC Connection Setup Message */
                rrcrce_int_data.substate = RRCRCE_WAIT_FOR_RRC_CONN_SETUP;

                /* Increments the re-transmission counter for RRC Connection
                   Request Message */
                WRRC_MSG1_HIGH("RRC Connection Request is sent: %d",rrcdata_timers_counters_info.rrc_v_300);

                rrcdata_timers_counters_info.rrc_v_300++;  
              }
              else
              {
                /* Release the RB 0 and request the RRC LLC to go to Idle disconnected */
                  ...
                  RRC_TRANSITION_TO_DISCONNECTED, RRC_STATE_DISCONNECTED);
                  /* The RCE Procedure ends */
                  rrcrce_int_data.substate = RRCRCE_WAIT_FOR_RB0_REL_CNF; /* change the RCE substate */
                  if(cause  ==  RRC_ACCESS_CLASS_NOT_ALLOWED)
                  {
                    if(rrcrce_int_data.est_cn_domain_id == RRC_CS_DOMAIN_CN_ID)
                    {
                      rrcrce_int_data.mm_abort_cause = RRC_AC_NOT_ALLOWED_CS_DOMAIN;
                    }
                    else
                    {
                      rrcrce_int_data.mm_abort_cause = RRC_AC_NOT_ALLOWED_PS_DOMAIN;
                    }
                  }
                  else
                  {
          ...
          {
            WRRC_EVENT_REPORT(EVENT_WCDMA_RRC_RB0_SETUP_FAILURE);

            if(rrcrce_int_data.send_abort_cnf)
            {
              rrcrce_send_rrc_abort_cnf(wrrc_as_id);
              /* Clear the ESTABLISHED_RABS while bringing down RRC Connection*/
              rrc_clear_established_rabs(wrrc_as_id);

              /* Reset Signalling Connections status */
              rrcscr_reset_signalling_connection_status(wrrc_as_id);
        ...
        else
        {
          WRRC_MSG2_HIGH("Ignoring tr_id %d in chl config cnf. Expected tr_id = %d", 
                     cmd_ptr->cmd.chan_config_cnf.transaction_id,
                      rrcrce_int_data.chl_config_transaction_id);
        }
      }
      else
      {
    ...
    case RRC_CHANNEL_CONFIG_CNF:
      ...
      if(cmd_ptr->cmd.chan_config_cnf.procedure == RRC_PROCEDURE_RCE)
      {
        ...
        if(cmd_ptr->cmd.chan_config_cnf.transaction_id == rrcrce_int_data.chl_config_transaction_id)
        {
          if( RRCLLC_CHAN_CFG_SUCCESS == cmd_ptr->cmd.chan_config_cnf.chan_cfg_status ) 
          {
            if(rrcrce_int_data.send_abort_cnf || rrcrce_int_data.abort_ind_sent)
            {
              /* Do not send command to LLC to release SRBs Instead send Initiate Cell Selection Indication */
               rrcrce_send_rrc_initiate_cell_selection_ind(wrrc_as_id,
               RRC_TRANSITION_TO_DISCONNECTED, RRC_STATE_DISCONNECTED);
          ...
          else /* Channel Config for RB0 failure */
          {
            ...
            WRRC_MSG1_HIGH("Channel Config for RB0 failure. rrcrce_substate %d",rrcrce_int_data.substate);
            ...
            if(rrccho_is_gtow_pcco_in_progress(wrrc_as_id))
            {
              ...
              WRRC_MSG1_HIGH("Channel Config for RB0 failure when PCCO active.rrcrce_substate %d",rrcrce_int_data.substate);
              rrcrce_send_conn_setup_ind(wrrc_as_id,conn_setup_ind_params);                  
            ...
            else if(!rrcrce_int_data.abort_ind_sent)
            {
              /* Send RRC_ABORT_IND command to the MM */
              rrcrce_send_rrc_abort_ind(wrrc_as_id, RRC_INTERNAL_ERROR);   
            }    
...
)
{
  ...
  switch (message_choice)
  {
   ...
   ///////////// Is this a security vulnerability.
        WRRC_ERR0_FATAL("Invalid Integrity Check Info Present");
      }    
  ...
  else /* Continue with peeking in to Initial UE Id */
  {
    ...
    switch(initial_ue_id_choice)
    {
          ...
          )
        {
            ...
            )
          {
            ...
            if(initial_ue_id.imsi.num_digits == 
                 rrcdata_sim_related_info.rrc_initial_ue_identity.imsi.num_digits)
            {
            ...
            if( (initial_ue_id_choice == rrcdata_sim_related_info.rrc_initial_ue_identity_choice) &&
                (initial_ue_id.imsi.num_digits == 
                 rrcdata_sim_related_info.rrc_initial_ue_identity.imsi.num_digits) &&
                (rrcrce_compare_arrays(initial_ue_id.imsi.value,
                                  rrcdata_sim_related_info.rrc_initial_ue_identity.imsi.value,
                                  (initial_ue_id.imsi.num_digits * 
                                   sizeof(uint8)  ) ) )
               )
            {
      ...
      case RRC_TMSI_LAI_UE_ID:
              ...
              )
        {
            ...
            ) 
          {
            ...
            if(rrcdata_sim_related_info.rrc_initial_ue_identity.tmsi_lai.lai.plmn.num_mnc_digits > RRC_MAX_MNC_DIGITS)
            {
            ...
            if( (initial_ue_id_choice == rrcdata_sim_related_info.rrc_initial_ue_identity_choice) &&
                ...
                rrcrce_compare_arrays(initial_ue_id.tmsi_lai.lai.plmn.mcc,
                                      rrcdata_sim_related_info.rrc_initial_ue_identity.tmsi_lai.lai.plmn.mcc,
                                      RRC_MAX_MCC_DIGITS) &&                
                rrcrce_compare_arrays(initial_ue_id.tmsi_lai.lai.plmn.mnc,
                                      rrcdata_sim_related_info.rrc_initial_ue_identity.tmsi_lai.lai.plmn.mnc,
                                      rrcdata_sim_related_info.rrc_initial_ue_identity.tmsi_lai.lai.plmn.num_mnc_digits) &&
                rrcrce_compare_arrays(initial_ue_id.tmsi_lai.tmsi.value,
                                      rrcdata_sim_related_info.rrc_initial_ue_identity.tmsi_lai.tmsi.value,
                                      RRC_MAX_GSMMAP_TMSI_LENGTH) &&
                rrcrce_compare_arrays(initial_ue_id.tmsi_lai.lai.lac,
                                      rrcdata_sim_related_info.rrc_initial_ue_identity.tmsi_lai.lai.lac,
      ...
      case RRC_PTMSI_RAI_UE_ID:
              ...
              )
        {
            ...
            ) 
          {
            ...
            if(rrcdata_sim_related_info.rrc_initial_ue_identity.ptmsi_rai.rai.lai.plmn.num_mnc_digits > RRC_MAX_MNC_DIGITS)
            {
              WRRC_MSG0_ERROR("Invalid number of MNC digits");
              return status;
            }
            ...
            if( (initial_ue_id_choice == rrcdata_sim_related_info.rrc_initial_ue_identity_choice) &&
                ...
                rrcrce_compare_arrays(initial_ue_id.ptmsi_rai.rai.lai.plmn.mcc,
                                rrcdata_sim_related_info.rrc_initial_ue_identity.ptmsi_rai.rai.lai.plmn.mcc,
                                RRC_MAX_MCC_DIGITS)  &&
                rrcrce_compare_arrays(initial_ue_id.ptmsi_rai.rai.lai.plmn.mnc,
                                rrcdata_sim_related_info.rrc_initial_ue_identity.ptmsi_rai.rai.lai.plmn.mnc,
                                rrcdata_sim_related_info.rrc_initial_ue_identity.ptmsi_rai.rai.lai.plmn.
                                num_mnc_digits)  &&
                rrcrce_compare_arrays(initial_ue_id.ptmsi_rai.ptmsi.value,
                                      rrcdata_sim_related_info.rrc_initial_ue_identity.ptmsi_rai.ptmsi.value,
                                      RRC_MAX_GSMMAP_PTMSI_LENGTH) &&
                rrcrce_compare_arrays(initial_ue_id.ptmsi_rai.rai.lai.lac,
                                      rrcdata_sim_related_info.rrc_initial_ue_identity.ptmsi_rai.rai.lai.lac,
                                      RRC_MAX_GSMMAP_LAC_LENGTH) &&
                rrcrce_compare_arrays(initial_ue_id.ptmsi_rai.rai.rac,
                                      rrcdata_sim_related_info.rrc_initial_ue_identity.ptmsi_rai.rai.rac,
          ...
          )
        {
          ...
          if( (initial_ue_id_choice == rrcdata_sim_related_info.rrc_initial_ue_identity_choice) &&
                (rrcrce_compare_arrays(initial_ue_id.imei.value,
                                  rrcdata_sim_related_info.rrc_initial_ue_identity.imei.value,
                                  (RRC_NUM_IMEI_DIGITS *
                                   sizeof(uint8)  )   ) )
               )
            {
      ...
      default:
        WRRC_ERR0_FATAL("This should never happen");
        break;
    }
  }
  return(status);
...
)
{
 ...
 /* Checks whether Logical Channel entity setup for CCCH logical
       Channel type */
  ul_lc_id = rrclcm_check_ul_rlc_lc_id (wrrc_as_id, UE_LOGCHAN_CCCH,
  ...
  if(ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
    ...
    msg_ptr = (rrc_UL_CCCH_Message *) rrc_malloc ( 
      ...
      rrcrce_build_rrc_connection_request_msg(wrrc_as_id,msg_ptr); 
      /* This places SDU into RLC watermark queue */
      if(RRCSEND_SUCCESS != 
         rrcsend_send_ul_sdu(wrrc_as_id,RRC_PROCEDURE_RCE,msg_ptr,ul_lc_id,
...
SIDE EFFECTS
  None
===========================================================================*/
static rrcrce_status_e_type rrcrce_process_default_config_info (sys_modem_as_id_e_type wrrc_as_id,uint32 config_id_received,
...
#ifdef FEATURE_WCDMA_REL10
     ||(config_id_received == RRC_DEF_CONFIG_24)
#endif
     )
  { 
    ...
    if(next_rrc_state == rrc_RRC_StateIndicator_cell_DCH)
    {
      ...
      if(config_id_received == RRC_DEF_CONFIG_17)
      {
        rrcrce_int_data.default_config_info.dcch_msg_ptr = NULL;
        rrcrce_int_data.default_config_info.dcch_msg_ptr = 
            (rrc_DL_DCCH_Message *)rrcasn1_decode_pdu(wrrc_as_id, (void *)default_config_info[config_id_received].
                                                   default_config_ptr, rrc_DL_DCCH_Message_PDU,
                                                   default_config_info[config_id_received].default_config_len
                                                   );
        if(rrcrce_int_data.default_config_info.dcch_msg_ptr == NULL)
        {  
          WRRC_ERR0_FATAL("No memory reserved for rb_setup_r6_ptr ptr");
        } 
      }
      else
         ...
         (config_id_received == RRC_DEF_CONFIG_14))
      {
        ...
        if(rrcrce_int_data.default_config_info.rbs_ptr == NULL)
        {  
          WRRC_ERR0_FATAL("No memory reserved for RBS ptr");
        } 
      }
      else
      {
        rrcrce_int_data.default_config_info.sib16_ptr = 
          (rrc_SysInfoType16 *)rrcasn1_decode_pdu( wrrc_as_id,(void *)default_config_info[config_id_received].
                                                   default_config_ptr, rrc_SysInfoType16_PDU,
                                                   default_config_info[config_id_received].default_config_len
                                                   );
  
        if(rrcrce_int_data.default_config_info.sib16_ptr == NULL)
        {  
          WRRC_ERR0_FATAL("No memory reserved for SIB16 ptr");
        } 
      }
      rrcrce_int_data.default_config_info.config_type = RRC_DEFAULT_CONFIG;
      rrcrce_int_data.default_config_info.def_config_ptr_present = TRUE;
      rrcrce_int_data.default_config_info.default_config_id = config_id_received;
...
)
{
...
#ifdef FEATURE_WCDMA_REL10
  rrc_RRCConnectionSetup_r10_IEs* rrc_conn_ptr_r10 = NULL;
  ...
  *msg_version_ptr = MSG_REL99;
...
#endif /* FEATURE_WCDMA_REL9 */
#ifdef FEATURE_WCDMA_REL10
    else if (RRCRCE_R10_CRITICAL_EXT_PRESENT && (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL10))
    {
      WRRC_MSG0_MED("REL10:RRC Conn Setup is rcvd");
      *msg_version_ptr = MSG_REL10;
    }
#endif /* FEATURE_WCDMA_REL10 */
  ...
  if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5)
  {
    memset((void*)&rrcrce_int_data.default_config_info, 0, sizeof(rrcrce_int_data.default_config_info));
    rrcrce_int_data.default_config_info.config_type = RRC_INVALID_CONFIG;
  }
  ...
  switch (*msg_version_ptr) 
  {
    case MSG_REL99:
          ...
          rrc_TransactionIdentifier > 3 )
      {
        WRRC_MSG1_HIGH("Invalid Transaction Id is received: %d", 
                 rrcrce_int_data.setup_transaction_identifier);
        return(status);
      }    
    ...
    case MSG_REL5:
      ...
      {
        rrcrce_int_data.default_config_info.config_type = RRC_COMPLETE_CONFIG;
              ...
              if(rrc_conn_ptr->rrc_StateIndicator == rrc_RRC_StateIndicator_cell_DCH)
              {
                  ...
                  (config_id_received == RRC_DEF_CONFIG_13))
                {
                  ...
                  rrcrce_srb5_information_setup_list_ptr = (rrc_SRB_InformationSetupList_r5 *)
                    &rrcrce_int_data.default_config_info.rbs_ptr->message.u.radioBearerSetup.u.later_than_r3.criticalExtensions.
                    u.criticalExtensions.u.r5.radioBearerSetup_r5.srb_InformationSetupList;
                }
                else
                {
                  rrcrce_int_data.default_config_info.sib16_ptr = (rrc_SysInfoType16 *)
                      rrcasn1_decode_pdu(wrrc_as_id, (void *)default_config_info[config_id_received].
                      default_config_ptr, rrc_SysInfoType16_PDU,
                      default_config_info[config_id_received].default_config_len);
                  ...
                  rrcrce_srb_information_setup_list_ptr = (rrc_SRB_InformationSetupList2 *)
                      &rrcrce_int_data.default_config_info.sib16_ptr->preDefinedRadioConfiguration.predefinedRB_Configuration.srb_InformationList;
                }
                rrcrce_int_data.default_config_info.config_type = RRC_DEFAULT_CONFIG;
                rrcrce_int_data.default_config_info.def_config_ptr_present = TRUE;
                rrcrce_int_data.default_config_info.default_config_id = config_id_received;
               ...
               rrc_RRCConnectionSetup_r6_IEs_specificationMode_preconfiguration))
      {
           ...
           T_rrc_RRCConnectionSetup_r6_IEs_preConfigMode_defaultConfig)
        {
          if(rrc_conn_ptr_r6->specificationMode.u.preconfiguration.preConfigMode.u.defaultConfig.defaultConfigMode == 
             rrc_DefaultConfigMode_fdd)
          {
            if (RRCRCE_SUCCESS != rrcrce_process_default_config_info(wrrc_as_id,
            ...
            if(rrcrce_int_data.default_config_info.default_config_id == RRC_DEF_CONFIG_17)
            {
              rb_setup_r6_ptr = rrcrce_get_rb_setup_r6_ptr_for_dc(wrrc_as_id);
           ...
           T_rrc_RRCConnectionSetup_r7_IEs_preConfigMode_defaultConfig)
        {
             ...
             rrc_DefaultConfigMode_fdd)
          {
            ...
            else
            {
              ...
              {
                /*lint -e740 */
                rrcrce_srb_information_setup_list_ptr = (rrc_SRB_InformationSetupList2 *)&rrcrce_int_data.default_config_info.sib16_ptr->preDefinedRadioConfiguration.predefinedRB_Configuration.srb_InformationList;
              }
            }
            /*lint +e740 */
          }
...
#ifdef FEATURE_WCDMA_REL7_SPECRS
      /*In RCE OTA "supportForChangeOfUE_Capability" IE is mandatory. We need to store it in
              "rrccouec.rrc_ordered_rnc_cap_change_support" so that later once the procedure is over successfully, global
              "rrc_rnc_cap_change_support" can be updated accordingly. */
    ...
    case MSG_REL9:
...
#ifdef FEATURE_WCDMA_REL10
      case MSG_REL10:
        rrc_conn_ptr_r10= &msg_ptr->message.u.rrcConnectionSetup.u.later_than_r3.criticalExtensions.u.
        criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.rrcConnectionSetup_r10;
        ...
        next_rrc_state = rrc_conn_ptr_r10->rrc_StateIndicator;
        ...
        "rrccouec.rrc_ordered_rnc_cap_change_support" so that later once the procedure is over successfully, global
        "rrc_rnc_cap_change_support" can be updated accordingly. */
        if(rrc_conn_ptr_r10->supportForChangeOfUE_Capability == TRUE)
        {
          rrccouec.rrc_ordered_rnc_cap_change_support = RNC_CAP_CHANGE_SUPPORTED;
        }
        else
      ...
      /* At present, we support complete & default configuration modes only */
      if (rrc_conn_ptr_r10->specificationMode.t == 
      T_rrc_RRCConnectionSetup_r10_IEs_specificationMode_complete)
      {
        rrcrce_int_data.default_config_info.config_type = RRC_COMPLETE_CONFIG;
        /*RRC_GKG: Make sure that we don't use the following SRB ptr for anything else because we are typecasting
        different releases of SRB Lists to R99 SRB List. It's fine so long as we use it for counting.*/
        rrcrce_srb_information_setup_list_ptr = (rrc_SRB_InformationSetupList2 *)
        &rrc_conn_ptr_r10->specificationMode.u.complete.srb_InformationSetupList;
      }
      else if (rrc_conn_ptr_r10->specificationMode.t == 
      T_rrc_RRCConnectionSetup_r10_IEs_specificationMode_preconfiguration)
      {
        if(rrc_conn_ptr_r10->specificationMode.u.preconfiguration.preConfigMode.t == 
        T_rrc_RRCConnectionSetup_r10_IEs_preConfigMode_defaultConfig)
        {
          if(rrc_conn_ptr_r10->specificationMode.u.preconfiguration.preConfigMode.u.defaultConfig.defaultConfigMode == 
           rrc_DefaultConfigMode_fdd)
          {
            if (RRCRCE_SUCCESS != rrcrce_process_default_config_info(wrrc_as_id,
            rrc_conn_ptr_r10->specificationMode.u.preconfiguration.preConfigMode.u.defaultConfig.defaultConfigIdentity,
            next_rrc_state))
            {
              return(status);
            }
            ...
            else
            {
              ...
              config_id_received = rrc_conn_ptr_r10->specificationMode.u.preconfiguration.preConfigMode.u.defaultConfig.defaultConfigIdentity;
              if((config_id_received == RRC_DEF_CONFIG_11) ||
              (config_id_received == RRC_DEF_CONFIG_12) ||
              (config_id_received == RRC_DEF_CONFIG_13) ||
              (config_id_received == RRC_DEF_CONFIG_14))
      ...
      dpch_info_present = (((RRC_MSG_COMMON_BITMASK_IE_PTR(rrc_conn_ptr_r10,
       rrc_RRCConnectionSetup_r10_IEs,ul_DPCH_Info)) && 
      (RRC_CHECK_COMMON_MSG_TYPE(rrc_conn_ptr_r10->ul_DPCH_Info.modeSpecificInfo, 
       rrc_UL_DPCH_Info_r7_modeSpecificInfo_fdd))) ||
      ((RRC_MSG_COMMON_BITMASK_IE_PTR(rrc_conn_ptr_r10,
       rrc_RRCConnectionSetup_r10_IEs,dl_InformationPerRL_List)) &&
      (RRC_MSG_COMMON_BITMASK_IE(rrc_conn_ptr_r10->dl_InformationPerRL_List.elem[0],
       rrc_DL_InformationPerRL_r8,dl_dpchInfo))));
  ...
  if((rrcrce_int_data.default_config_info.config_type == RRC_DEFAULT_CONFIG)&&
    (rrcrce_int_data.default_config_info.default_config_id == RRC_DEF_CONFIG_17))
  {
  ...
  rrcrce_int_data.number_of_rbs = number_of_rbs;
    ...
    switch(next_rrc_state)
    {
      case rrc_RRC_StateIndicator_cell_DCH:
        ...
        if (dpch_info_present)
        {
          ...
          parameters */
          if(ORDERED_CONFIG_SET == rrcllc_set_ordered_config(wrrc_as_id,
                                   RRC_PROCEDURE_RCE,
                                   next_state,                                         
                                   dl_sdu_type,
                                   msg_ptr) )
  ...
  if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5)
  {
    if(rrcrce_int_data.default_config_info.config_type == RRC_DEFAULT_CONFIG)
    {
      rrcrce_delete_default_config_info(wrrc_as_id);
    }
  }
...
)
{
  ...
  if(lte_redirection_info)
  {
    ...
    {
      rrcrce_int_data.rsp_to_rrc_conn_req = RRCRCE_CONNECTION_REJECT;
      rrcrce_send_lte_redirection_req(wrrc_as_id,msg_ptr);
    }
    else
    {
      mode_enabled = FALSE;
...
#endif
  {
      ...
      )
    {
      ...
      rrcrce_send_redirection_req(wrrc_as_id,msg_ptr, FALSE);    
      WRRC_MSG0_HIGH("iRAT Redirection Req sent to CSP");
      /* Wait for redirect confirm from Cell Selection Procedure */
      rrcrce_int_data.substate = RRCRCE_WAIT_FOR_REDIRECT_CNF;   
    }  
    else 
    {                  
      mode_enabled = FALSE;
...
#ifdef FEATURE_DISCONNECT_ON_UNSUPPORTED_IRAT_REDIRECTION
    if ((rrcdata_misc_info.rrc_establishment_cause == RRC_EST_EMERGENCY_CALL) &&
         rrccsp_check_gsm_bands_supported(wrrc_as_id))                               
    {
      rrcrce_send_rrc_initiate_cell_selection_ind(wrrc_as_id,
      ...
      /* Set Abort Cause */
      rrcrce_int_data.mm_abort_cause = RRC_UNSUPPORTED_REDIRECTION;
    }    
    else
#endif
    {
      ...
      /* Move to idle only for below EST cause because there is no NAS retry for 
         other EST cause */       
      if(rrc_dsds_int_data.wcdma_is_ue_in_dual_standby && ((rrcdata_misc_info.rrc_establishment_cause == RRC_EST_REGISTRATION) ||
         (rrcdata_misc_info.rrc_establishment_cause == RRC_EST_CALL_RE_ESTABLISH) ||
         (rrcdata_misc_info.rrc_establishment_cause == RRC_EST_INTER_RAT_CELL_RESELECT)))
      {
        ...
        /* Set Abort Cause */
        rrcrce_int_data.mm_abort_cause = RRC_ABORT_CONN_REJECT_WAIT_TMR;
      }
      else
#endif
      {
        rrctmr_start_timer(wrrc_as_id,RRCTMR_WAIT_TIME_TIMER, rrcrce_int_data.wait_time * 1000);  
        rrcrce_int_data.substate = RRCRCE_WAIT_FOR_WAIT_TIMER_TO_EXPIRE;
      }
    }
  }
}
...
/*================================================================
FUNCTION          RRCRCE_SSGCCS_SEND_RRC_CONNECTION_REJECT_INFO
...
SIDE EFFECTS
                  None
=================================================================*/
void rrcrce_ssgccs_send_rrc_connection_reject_info(sys_modem_as_id_e_type wrrc_as_id           ,rrc_DL_CCCH_Message *msg_ptr)
{
  wcdma_ssgccs_rrc_connection_reject_info_ind_s connection_reject_info;

  if(connection_reject_3g_msg_handler == NULL)
  {
    WRRC_MSG0_ERROR("SSGCCS:RRC Connection reject info msg handler NULL");
    return;
  }
  memset(&connection_reject_info,0,sizeof(wcdma_ssgccs_rrc_connection_reject_info_ind_s));
  connection_reject_info.wrrc_as_id = wrrc_as_id;
  connection_reject_info.mcc = RRC_CSP_GET_INT_MCC(rrc_csp_int_data.curr_acq_entry.plmn_id);
  connection_reject_info.mnc = RRC_CSP_GET_INT_MNC(rrc_csp_int_data.curr_acq_entry.plmn_id);
  connection_reject_info.cellid = rrc_csp_int_data.curr_acq_entry.cell_id;
  connection_reject_info.conn_rej_cause = msg_ptr->message.u.rrcConnectionReject.u.r3.rrcConnectionReject_r3.rejectionCause;
  connection_reject_info.wait_time = rrcrce_int_data.wait_time;
  ...
  {
    connection_reject_info.redir_info_present = TRUE;
    ...
    {
      connection_reject_info.redir_rat = RRC_FREQ_REDIRECTION;
    }
    else
    {
      connection_reject_info.redir_rat = RRC_INTER_SYSTEM_GSM;
    }
  }
  ...
  {
    connection_reject_info.redir_info_present = TRUE;
    connection_reject_info.redir_rat = RRC_INTER_SYSTEM_LTE;
  }
  WRRC_MSG0_HIGH("SSGCCS: Sending RRC connection reject info to SSGCCS QMI Client ");
  connection_reject_3g_msg_handler(&connection_reject_info);
}
...
)
{
  ...
  rrcrce_int_data.save_dl_ccch_msg.dl_sdu      = NULL;
  rrcrce_int_data.save_dl_ccch_msg.decoded_msg = NULL; 
  
  switch (cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_STATE_CHANGE_IND:
      WRRC_MSG1_HIGH("RRC_STATE_CHANGE_IND received.rrcrce_substate %d",rrcrce_int_data.substate);
         ...
         ( rrc_get_state(wrrc_as_id) == RRC_STATE_DISCONNECTED))
      {
        ...
        /* Stop the timer T300 */
        rrctmr_stop_timer(wrrc_as_id, RRCTMR_T_300_TIMER);
	/*CR945560: Stop CCCH QTA blocking timer to allow QTA as there is 
        no more CCCH transmission*/
	rrcwrm_stop_wl1_qta_ccch_timer(wrrc_as_id);
        if(rrccho_is_gtow_pcco_in_progress(wrrc_as_id))
        {
          rrc_conn_setup_ind_type conn_setup_ind_params;
          conn_setup_ind_params.status = FAILURE;
          ...
          WRRC_MSG1_HIGH("RRC_STATE_CHANGE_IND received when PCCO is active.rrcrce_substate %d",rrcrce_int_data.substate);      
          rrcrce_send_conn_setup_ind(wrrc_as_id,conn_setup_ind_params);                  
    ...
    case RRC_CMAC_STATUS_IND:
      {
        ...
        rrcrce_int_data.cmac_status = cmd_ptr->cmd.cmac_status_ind;
        ...
        (void) rrcrce_get_t300_from_serv_cell_db(wrrc_as_id,&rrcrce_t_300);
          
        /* start the timer T300 */
        rrctmr_start_timer(wrrc_as_id,RRCTMR_T_300_TIMER,rrcrce_convert_t300_to_ms(wrrc_as_id,rrcrce_t_300)); 
    ...
    case RRC_NEW_CELL_IND: 
      ...
      else
      {
        ...
        if(rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_T_300_TIMER) > 0)
        {
      ...
      if( RRCRCE_SUCCESS != 
          rrcrce_get_n300_from_serv_cell_db(wrrc_as_id,&rrcrce_n_300))
      {
        WRRC_ERR1_FATAL("N300 value %d is invalid", rrcrce_n_300);
      }
      
      if((RRCRCE_SUCCESS == rrcrce_validate_cell_access_status(wrrc_as_id,
                      rrcrce_int_data.est_cn_domain_id,rrcdata_misc_info.rrc_establishment_cause,&cause)) &&
        (rrcdata_timers_counters_info.rrc_v_300 <= rrcrce_n_300))
      {
      ...
      { 
        rrcrce_int_data.num_retries_count++;
        WRRC_MSG1_HIGH("num_retries_count = %d",rrcrce_int_data.num_retries_count);
        ...
        if(rrccho_is_gtow_pcco_in_progress(wrrc_as_id))
        {
          ...
          WRRC_MSG1_HIGH("Max RACH attempts reached when PCCO active.num_retries_count = %d",rrcrce_int_data.num_retries_count);
          rrcrce_send_conn_setup_ind(wrrc_as_id,conn_setup_ind_params);
        ...
        /* We found that network is not including the any inter or intra freq nbrs and hence UE is stuck on a weak cell. 
         * This is a workaround to address this issue.
         * Downside : In case the cell becomes good, due to this optimization, we may not camp on this.
         */
        WRRC_MSG2_HIGH("Intra f nbr empty %d, Inter f nbr empty %d",rrcmeas_intra_f_nbr_empty,rrcmeas_inter_f_nbr_empty);

        if((WCDMA_RRC_IDLE_FEATURES(RRC_BARRING_OPT_RACH_FAILURE))
           && (cmd_ptr->cmd_hdr.cmd_id == RRC_T300_EXPIRED_IND) 
#ifdef FEATURE_WCDMA_DEFERRED_SIB11_12_READING
           && !(rrcsib_is_sib11_11bis_12_read_deferred(wrrc_as_id, DEFERRED_SIB11)) 
#endif           
           && (rrcmeas_intra_f_nbr_empty) 
           && (rrcmeas_inter_f_nbr_empty))
        {
          ...
          acq_entry.freq = rrc_ccm_get_curr_camped_freq(wrrc_as_id);
          acq_entry.scr_code = (uint16)(rrc_ccm_get_curr_camped_psc(wrrc_as_id) << 4);
          rrc_csp_send_l1_cmd(wrrc_as_id,RRC_PROCEDURE_CSP, CPHY_CELL_BAR_REQ, &acq_entry);
        }
        /* Bar the cell upon no response from NW after N300 trials for a CSFB and MO voice Call, provided S-criteria is still good. */
        else if(rrcnv_data.rrc_cell_bar_upon_rach_failure_nv
        && (cmd_ptr->cmd_hdr.cmd_id == RRC_T300_EXPIRED_IND) 
        &&( rrc_csp_int_data.rrc_csfb_call_status 
        || (rrcdata_misc_info.rrc_establishment_cause == RRC_EST_ORIG_CONV_CALL) 
        || (rrcdata_misc_info.rrc_establishment_cause == RRC_EST_TERM_CONV_CALL))
        && rrc_is_s_criteria_met(wrrc_as_id,rrc_ccm_get_curr_camped_plmn(wrrc_as_id),
         rrc_ccm_get_curr_camped_cell_id(wrrc_as_id))
        )
        {
          rrc_csp_acq_entry_type acq_entry;
          rrc_csp_int_data.wcdma_cell_time_to_reselect = rrcnv_data.rrc_cell_bar_duration_secs_nv;
          rrc_csp_int_data.cell_bar_info = RRC_CSP_WCDMA_PSC_BARRED;
          acq_entry.freq = rrc_ccm_get_curr_camped_freq(wrrc_as_id);
          acq_entry.scr_code = (uint16)(rrc_ccm_get_curr_camped_psc(wrrc_as_id) << 4);
          rrc_csp_send_l1_cmd(wrrc_as_id,RRC_PROCEDURE_CSP, CPHY_CELL_BAR_REQ, &acq_entry);
        ...
        {
          WRRC_MSG2_HIGH("rrc_cell_bar_upon_rach_failure_nv:%d, rrc_cell_bar_duration_secs_nv:%dsecs",rrcnv_data.rrc_cell_bar_upon_rach_failure_nv, rrcnv_data.rrc_cell_bar_duration_secs_nv);
        }
    ...
    case RRC_DOWNLINK_SDU_IND:
      ...
      if( (cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_type == rrc_DL_CCCH_Message_PDU) &&
          (FALSE == rrc_ccm_is_reselection_in_progress(wrrc_as_id) ) 
        )
      {
        message_choice = 
          rrc_get_dl_ccch_message_type(wrrc_as_id,cmd_ptr->cmd.downlink_sdu_ind.dl_sdu,
	...
	/*CR945560: Stop CCCH QTA blocking timer after receiving connection setup message
         so that WL1 can allow QTA as there is no more CCCH transmission*/
	rrcwrm_stop_wl1_qta_ccch_timer(wrrc_as_id);
        ...
        switch( message_choice )
        {
          ...
          case T_rrc_DL_CCCH_MessageType_rrcConnectionSetup: /* RRC CONNECTION SETUP message
                                                 is received. */
            WRRC_MSG0_HIGH("RRC Connection Setup Msg is received");
            /* Reset the rrcrce_num_retries_count */
            rrcrce_int_data.num_retries_count =0;

            /* Stop the timer T300 */
            rrctmr_stop_timer(wrrc_as_id, RRCTMR_T_300_TIMER);
            /* Check whether ASN1 decoding as resulted in  PROTOCOL_ERROR_REJECT */
            if(rrcdata_misc_info.rrc_protocol_error_reject == TRUE)
            {
              rrc_log_protocol_error(wrrc_as_id, (uint8) (rrc_get_state(wrrc_as_id)),
                                      (uint8) RRC_PROCEDURE_RCE,
              ...
              if(rrcdata_timers_counters_info.rrc_v_300 <= rrcrce_n_300)
              {          
                rrcdata_misc_info.rrc_protocol_error_indicator = rrc_ProtocolErrorIndicator_errorOccurred;
              ...
              {
                WRRC_MSG1_HIGH("Max retries exceeded.rrcrce_substate %d",rrcrce_int_data.substate);
            ...
            else /* ASN1 successfully decoded the message */
            {
              ...
              if(rrcdata_timers_counters_info.rrc_v_300 != RRC_RACH_TX_STATS_ONE_TIME_RACH)
              {
              ...
              if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL10)
              {
                if (RRCRCE_R10_CRITICAL_EXT_PRESENT)
                {
                  msg_release_version = MSG_REL10;
                  state_indicator_instructed = msg_ptr->message.u.rrcConnectionSetup.u.later_than_r3.criticalExtensions.u.criticalExtensions.u.
                  criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.rrcConnectionSetup_r10.rrc_StateIndicator;
              
                  if(state_indicator_instructed == rrc_RRC_StateIndicator_cell_FACH)
                  {
                    if(RRC_MSG_COMMON_BITMASK_IE(msg_ptr->message.u.rrcConnectionSetup.u.later_than_r3.criticalExtensions.u.
                    criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.rrcConnectionSetup_r10,
                    rrc_RRCConnectionSetup_r10_IEs,frequencyInfo))
                    { 
                      WRRC_MSG0_HIGH("R10 ConnSetUp for FACH rcvd. Freq info is present"); 
                      freq_info_present = TRUE;
                      mode_received = msg_ptr->message.u.rrcConnectionSetup.u.later_than_r3.criticalExtensions.u.criticalExtensions.u.
                      criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.rrcConnectionSetup_r10.frequencyInfo.modeSpecificInfo.t;
                      dl_uarfcn = msg_ptr->message.u.rrcConnectionSetup.u.later_than_r3.criticalExtensions.u.criticalExtensions.u.
                      criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.rrcConnectionSetup_r10.frequencyInfo.modeSpecificInfo.u.fdd->uarfcn_DL;
                    } 
              
                    if(RRC_MSG_COMMON_BITMASK_IE(msg_ptr->message.u.rrcConnectionSetup.u.later_than_r3.criticalExtensions.u.
                    criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.rrcConnectionSetup_r10,
                    rrc_RRCConnectionSetup_r10_IEs,new_c_RNTI))
                    {
                      crnti_present = TRUE;
                    }
                ...
                ))
              {
                ...
                if(freq_info_present && (dl_uarfcn != rrc_ccm_get_curr_camped_freq(wrrc_as_id)))
                {
                  if(RRCRCE_SUCCESS != rrcrce_get_n300_from_serv_cell_db(wrrc_as_id,&rrcrce_n_300))
                  { 
                    WRRC_ERR1_FATAL("N300 value %d is invalid", rrcrce_n_300);
                  } 
                  ...
                  { 
                    ...
                    { 
                      ...
                      if(SUCCESS == validate_dl_arfcn(wrrc_as_id,dl_uarfcn, 
...
#endif
                      { 
                        rrcrce_int_data.wait_time = 1;
                        rrcrce_int_data.rsp_to_rrc_conn_req = RRCRCE_CONNECTION_SETUP;
                        ...
                        { 
                          ...
                          /* Allocated memory */
                          rrcrce_int_data.save_dl_ccch_msg.dl_sdu = rrc_malloc(cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_length);

                          /* Copy the contents received in the allocated memory */
                          WCDMA_MEMCPY(rrcrce_int_data.save_dl_ccch_msg.dl_sdu, 
                                       cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_length,
                                       cmd_ptr->cmd.downlink_sdu_ind.dl_sdu,
                                       cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_length);

                          /* Save the length and type of SDU */
                          rrcrce_int_data.save_dl_ccch_msg.dl_sdu_length = 
                                              cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_length;

                          rrcrce_int_data.save_dl_ccch_msg.dl_sdu_type = 
                                              cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_type;

                          /* Send the command to Cell Selection Procedure */
                          rrcrce_send_redirection_req(wrrc_as_id,msg_ptr, TRUE);        
                            WRRC_MSG2_HIGH("ConnSetUp inter-freq redirection from (%d,%d)", 
                                    rrc_ccm_get_curr_camped_freq(wrrc_as_id),
                                    rrc_ccm_get_curr_camped_psc(wrrc_as_id));

                            if(msg_release_version == MSG_REL99)
                            {
                              WRRC_MSG3_HIGH("new cell info. freq %d, psc %d",
                                        dl_uarfcn, 
                                        msg_ptr->
                          ...
                          else if ((msg_release_version == MSG_REL10) && 
                          (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL10))
                          {
                            ...
                            dl_uarfcn,
                            msg_ptr->message.u.rrcConnectionSetup.u.later_than_r3.criticalExtensions.u.criticalExtensions.u.
                            criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.rrcConnectionSetup_r10.
                            dl_InformationPerRL_List.elem[0].modeSpecificInfo.u.fdd->
                            primaryCPICH_Info.primaryScramblingCode, 0);
                          }
#endif /* FEATURE_WCDMA_REL10 */
                  ...
                  {
                    WRRC_MSG2_HIGH("rrc_v_300 %d > rrcrce_n_300 %d. release_rb. Goto idle", rrcdata_timers_counters_info.rrc_v_300, rrcrce_n_300);
                ...
                {
                  WRRC_MSG0_MED("ConnSetUp iFreq Redirection for the same freq or absent");
                }
              ...
              if((rrcrce_int_data.substate == RRCRCE_WAIT_FOR_RRC_CONN_SETUP) 
                  && (RRCRCE_SUCCESS != rrcrce_validate_rrc_connection_setup_msg(wrrc_as_id,msg_ptr,
                                         cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_type, &msg_version)))
              {
                ...
                if( RRCRCE_SUCCESS !=
                  rrcrce_get_n300_from_serv_cell_db(wrrc_as_id,&rrcrce_n_300))
                {
                  WRRC_ERR1_FATAL("N300 value is invalid %d", rrcrce_n_300);
                }
                if(rrcdata_timers_counters_info.rrc_v_300 <= rrcrce_n_300)
                {                       
                  /* Resend RRC Connection Request message and this function
              ...
              else if(rrcrce_int_data.substate == RRCRCE_WAIT_FOR_RRC_CONN_SETUP)
              {
                ...
                rrcdata_misc_info.rrc_capability_update_requirement.
                ...
                if (msg_version == MSG_REL99)
                {
                  ...
                  /*lint -save -e613 */
                  rrcrce_int_data.setup_transaction_identifier = msg_ptr->message.u.
                    rrcConnectionSetup.u.r3.rrcConnectionSetup_r3.
                    rrc_TransactionIdentifier;
                  
                  /* Update RRC State Indicator */

                  rrcrce_int_data.state_indicator = msg_ptr->message.u.
                    rrcConnectionSetup.u.r3.rrcConnectionSetup_r3.
                    rrc_StateIndicator;
                  /*lint -restore */
                  ...
                  /*lint -save -e613 */
                  rrcuece_copy_capability_update_requirement(wrrc_as_id,msg_ptr->message.u.
                    rrcConnectionSetup.u.r3.rrcConnectionSetup_r3.
                ...
                if (msg_version == MSG_REL5)
                {
                  ...
                  CONNECTION SETUP COMPLETE message */
                  /*lint -save -e613 */
                  rrcuece_copy_capability_update_requirement_later_than_r3(wrrc_as_id,msg_ptr->message.u.
                    rrcConnectionSetup.u.later_than_r3.criticalExtensions.u.
                    ...
                    (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL7))
                {
                  ...
                  CONNECTION SETUP COMPLETE message */
                  /*lint -save -e613 */
                  rrcuece_copy_capability_update_requirement_r7(wrrc_as_id,&msg_ptr->message.u.
                    ...
                    (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL8))
                {
                  ...
                  CONNECTION SETUP COMPLETE message */
                  /*lint -save -e613 */
                  rrcuece_copy_capability_update_requirement_r8(wrrc_as_id,&msg_ptr->message.u.rrcConnectionSetup.u.
                ...
                (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL10))
                {
                  ...
                  rrcrce_int_data.state_indicator = msg_ptr->message.u.
                  rrcConnectionSetup.u.later_than_r3.criticalExtensions.u.
                  criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.rrcConnectionSetup_r10.rrc_StateIndicator;
                  ...
                  rrcuece_copy_capability_update_requirement_r8(wrrc_as_id,&msg_ptr->message.u.rrcConnectionSetup.u.
                  later_than_r3.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.
                  criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.rrcConnectionSetup_r10.capabilityUpdateRequirement);
                  /*lint -restore */
                  /*Save 'Requested E-UTRA Frequency Band list' if present */
                  if((WCDMA_RRC_IDLE_FEATURES(RRC_NW_REQ_LTE_BAND_CAP_REPORT))&&
                     (rrcdata_misc_info.rrc_capability_update_requirement.system_specific_cap_update_req_present))
                  {
                    rrcuece_copy_req_eutra_freq_band_list_r10(wrrc_as_id, &msg_ptr->message.u.rrcConnectionSetup.u.
                     later_than_r3.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.
                     criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10);
                  }
                }
#endif /* FEATURE_WCDMA_REL10 */
                ...
                if(rrcrce_int_data.state_indicator == rrc_RRC_StateIndicator_cell_DCH)
                {
                  next_state = RRC_STATE_CELL_DCH;
                }
                else if(rrcrce_int_data.state_indicator == rrc_RRC_StateIndicator_cell_FACH)
                {
                  next_state = RRC_STATE_CELL_FACH;
                } 
                else
                {
                   ...
                   WRRC_ERR1_FATAL("Invalid RRC state Indicator is received: %d",
                              rrcrce_int_data.state_indicator);
                }
                
                rrcrce_send_rrc_channel_config_req_for_srb(wrrc_as_id,ESTABLISH_RB,
                                                           next_state,
                                                           TRUE) ;  
                rrcrce_int_data.substate = RRCRCE_WAIT_FOR_SRB_SETUP_CNF;     
              } /* RRC Connection Setup is successfully validated */
              else
              {
                WRRC_MSG1_HIGH("RCE waiting in sstate %d", rrcrce_int_data.substate);
              }
          ...
          case T_rrc_DL_CCCH_MessageType_rrcConnectionReject: /* RRC CONNECTION REJECT is received */
            ...
            { 
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
              wrrc_qsh_log_rlf_oos_reject_params(wrrc_as_id, WRRC_CON_REJ, WRRC_METRIC_UPDATE);
#endif
              /* Update Transaction Id for RRC Connection Reject Msg */
              /*lint -save -e613 */
              rrcrce_int_data.reject_transaction_identifier = msg_ptr->message.u.
              ...
              rrcrce_int_data.wait_time = msg_ptr->message.u.rrcConnectionReject.u.r3.rrcConnectionReject_r3.waitTime;

#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
              conn_rej_params.conn_rej_cause = msg_ptr->message.u.rrcConnectionReject.u.r3.rrcConnectionReject_r3.rejectionCause;
              conn_rej_params.wait_time = rrcrce_int_data.wait_time;
              ...
              rrc_qsh_log_conn_rej_params(wrrc_as_id, &conn_rej_params);
#endif
             /* Sending RRC connection reject info to SSGCCS QMI client */
             rrcrce_ssgccs_send_rrc_connection_reject_info(wrrc_as_id,msg_ptr);

              /* Validate Transaction Id and make sure critical externsions
                 are not selected */
              if( (rrcrce_int_data.reject_transaction_identifier <= 3 ) &&
                   (msg_ptr->message.u.rrcConnectionReject.t ==
                   T_rrc_RRCConnectionReject_r3 ) )
              {
                if(rrcrce_int_data.wait_time == 0)
                {
                  if(rrccho_is_gtow_pcco_in_progress(wrrc_as_id))
                  {
                    rrc_conn_setup_ind_type conn_setup_ind_params;
                 ...
                 rrcrce_int_data.mm_abort_cause = RRC_FAILURE_DUE_TO_CONGESTION;
#endif
                }
                else  /* Wait Time is non-zero */
                {
                  ...
                  if( rrcdata_timers_counters_info.rrc_v_300 <= rrcrce_n_300)
                  {
                           ...
                           rrcConnectionReject_r3,rrc_RRCConnectionReject_r3_IEs,redirectionInfo))
                    {
                             ...
                             rrcConnectionReject_r3.redirectionInfo.t)
                      {
                        case T_rrc_RedirectionInfo_frequencyInfo:   /* Redirection to new Freq */
                          ...
                          {
#ifdef FEATURE_MCC_AND_850_800_BAND_COMBINATION
                            if(SUCCESS == validate_dl_arfcn(wrrc_as_id,
...
#endif
                            {
                              ...
                              /* Send the command to Cell Selection Procedure */
                                rrcrce_send_redirection_req(wrrc_as_id,msg_ptr, FALSE);     
                                WRRC_MSG3_HIGH("RCE: Inter-freq redirection req for freq %d to CSP", 
                                                    msg_ptr->message.u.rrcConnectionReject.u.r3.
                            ...
                            {
                              WRRC_MSG3_HIGH("iFreq Redirection rcvd with unsupported freq %d",
                                               msg_ptr->message.u.rrcConnectionReject.u.r3.
                                               rrcConnectionReject_r3.redirectionInfo.u.frequencyInfo->
                          ...
                          {
                            WRRC_MSG3_HIGH("Received Unsupported Freq Info:%d in redirection",
                              msg_ptr->message.u.rrcConnectionReject.u.r3.
                              rrcConnectionReject_r3.redirectionInfo.u.frequencyInfo->
                      ...
                      case T_rrc_RedirectionInfo_interRATInfo: /* Redirection to new System */
                        ...
                        rrcrce_handle_interrat_redirection_info(wrrc_as_id,
                                                    msg_ptr
                                #ifdef FEATURE_WCDMA_TO_LTE
                                                    , FALSE
                                #endif
                    ...
                    {
                      rrcrce_handle_interrat_redirection_info(wrrc_as_id,msg_ptr, TRUE);
                    }
#endif
      ...
      else /* Received other than rrc_DL_CCCH_Message type */
      {
        ...
        else /* Reselection is in progress */
        {
          ...
          rrcrce_int_data.substate = RRCRCE_WAIT_FOR_RESELCT_COMPL;  
        }
  ...
  switch (cmd_ptr->cmd_hdr.cmd_id)
  {
    ...
    case RRC_STATE_CHANGE_IND:
         ...
         ( rrc_get_state(wrrc_as_id) == RRC_STATE_DISCONNECTED))
      {
        ...
        rrctmr_stop_timer(wrrc_as_id, RRCTMR_PHY_CHAN_FAILURE_TIMER);  
    ...
    case RRC_ABORT_REQ: 
     ...
     /* Do not send command to LLC to release SRBs Instead send Initiate Cell Selection Indication */
         rrcrce_send_rrc_initiate_cell_selection_ind(wrrc_as_id,
            RRC_TRANSITION_TO_DISCONNECTED, RRC_STATE_DISCONNECTED);
        /* Wait for successful release of RB0 */
        rrcrce_int_data.substate = RRCRCE_WAIT_FOR_RB0_REL_CNF; //RRCRCR_WAIT_FOR_SRB_REL_CNF; 
        rrcrce_int_data.send_abort_cnf = TRUE;
    ...
    case RRC_NEW_CELL_IND: 
      ...
      if((RRCRCE_SUCCESS == rrcrce_validate_cell_access_status
                       (wrrc_as_id,rrcrce_int_data.est_cn_domain_id,rrcdata_misc_info.rrc_establishment_cause,&cause)) &&
         (rrcdata_timers_counters_info.rrc_v_300 <= rrcrce_n_300))
      {        
        /* Resend RRC connection Request Message and this function restarts the
           T300 Timer */
        ...
        {
          /* Re-Acquire Lock for non CS call cases, as we have released during OOS in connecting State*/
          if(rrc_acquire_lock_for_est_req(wrrc_as_id, rrcdata_misc_info.rrc_establishment_cause,0xfffffff,RRCTMR_WRM_LOCK_WAIT_TIMER_FOR_EST_REQ_IN_MS) == FAILURE)
          {
            /* Send RRC_ABORT_IND command to the MM */
            rrcrce_send_rrc_abort_ind(wrrc_as_id, RRC_NO_RESOURCE_AVAILABLE);      
    ...
    case RRC_PHY_CHAN_FAILURE_TIMER_EXPIRED_IND:
      ...
      rrc_transition_to_disconnected_state(wrrc_as_id,RRC_PROCEDURE_RCE,RRC_TX_TO_DISCON_OTHER);
        /* Update the release cause */
        rrcdata_misc_info.rrc_release_cause = RRC_REL_DEEP_FADE;

        /* RRC_ABORT_IND will be sent rrcrce_process_rce_wait_for_rb0_rel_cnf_substate 
        upon a state change indication */ 
        rrcrce_int_data.substate = RRCRCE_WAIT_FOR_RB0_REL_CNF;
        rrcrce_int_data.mm_abort_cause = RRC_FAILURE_UNSPECIFIED;
...
)
{
                             ...
                             (wrrc_as_id,rrcrce_int_data.est_cn_domain_id,rrcdata_misc_info.rrc_establishment_cause,&cause))
  {
    ...
    if(rrccho_is_gtow_pcco_in_progress(wrrc_as_id))
    {
      ...
      WRRC_MSG1_HIGH("Access not allowed when PCCO is active.rrcrce_substate %d",rrcrce_int_data.substate);      
      rrcrce_send_conn_setup_ind(wrrc_as_id,conn_setup_ind_params);                  
...
)
{
      ...
      { 
        switch(cmd_ptr->cmd.redirect_cnf.cause)
        {
          case RRC_CAMPED_ON_REDIRECTED_FREQ:
            ...
            if(rrcrce_int_data.rsp_to_rrc_conn_req == RRCRCE_CONNECTION_REJECT)
            {
              ...
              if(SUCCESS == rrcrce_validate_and_handle_acc_class_restr_on_redirec_succ(wrrc_as_id))
              {
                /* Resend RRC Connection Request message and this function
                   restarts the T300 Timer*/
                rrcrce_send_rrc_connection_request_msg(wrrc_as_id);
            ...
            else if(rrcrce_int_data.rsp_to_rrc_conn_req == RRCRCE_CONNECTION_SETUP)
            {
              ...
              if(SUCCESS == rrcrce_validate_and_handle_acc_class_restr_on_redirec_succ(wrrc_as_id))
              {
                ...
                else if(rrcdata_timers_counters_info.rrc_v_300 <= rrcrce_n_300)
                {
                  WRRC_MSG2_HIGH("v300 (%d) <= n300 (%d). Send conn req again", rrcdata_timers_counters_info.rrc_v_300,rrcrce_n_300);
                ...
                {
                  WRRC_MSG2_HIGH("v300 (%d) > n300 (%d) on the new cell. Go to idle", rrcdata_timers_counters_info.rrc_v_300,rrcrce_n_300);    
            ...
            else
            {
              WRRC_MSG1_ERROR("ERR:RCE rcvd REDIRECT_SUCCESS when rsp_to_rrc_conn_req %d. Trigger error recovery.",
                                                                                  rrcrce_int_data.rsp_to_rrc_conn_req);
              if(rrccho_is_gtow_pcco_in_progress(wrrc_as_id))
              {
                rrc_conn_setup_ind_type conn_setup_ind_params;
                conn_setup_ind_params.status = FAILURE;
          ...
          case RRC_CAMPED_ON_REDIRECTED_CELL:
            ...
            /* Redirection success for inter-freq redirection via RRC Connection Setup */
            if(rrcrce_int_data.rsp_to_rrc_conn_req == RRCRCE_CONNECTION_SETUP)
            {
              /* Decode the saved message */
              rrcrce_int_data.save_dl_ccch_msg.decoded_msg = 
                     ((rrc_DL_CCCH_Message *) rrcasn1_decode_pdu(wrrc_as_id,(void *)rrcrce_int_data.save_dl_ccch_msg.dl_sdu,
                                                                 (int)rrcrce_int_data.save_dl_ccch_msg.dl_sdu_type,
                                                                 rrcrce_int_data.save_dl_ccch_msg.dl_sdu_length
                                                                 ));
              /* If failed to decode then go to idle */
              if(rrcrce_int_data.save_dl_ccch_msg.decoded_msg == NULL)
              {
                ...
                /* The agreement is that CSP would tear down RB0 and RCE
                   ...
                   * upon a state change indication */ 
                  rrcrce_int_data.substate = RRCRCE_WAIT_FOR_RB0_REL_CNF;
                  rrcrce_int_data.mm_abort_cause = RRC_FAIL_TO_CAMP_ON;
              }
              /* PDU was successfully decoded */
              else
              {
                /* Validate the RRC Connection Setup message */
                if(RRCRCE_SUCCESS != rrcrce_validate_rrc_connection_setup_msg(wrrc_as_id,
                                                rrcrce_int_data.save_dl_ccch_msg.decoded_msg,
                                                rrcrce_int_data.save_dl_ccch_msg.dl_sdu_type, 
                                                &msg_version))
                {
                ...
                else
                {
                  ...
                  if (msg_version == MSG_REL99)
                  {
                    /* Update RCE Transaction Identifier */
                    /*lint -save -e613 */
                    rrcrce_int_data.setup_transaction_identifier = rrcrce_int_data.save_dl_ccch_msg.decoded_msg->message.u.
                      rrcConnectionSetup.u.r3.rrcConnectionSetup_r3.rrc_TransactionIdentifier;

                    /* Update RRC State Indicator */
                    rrcrce_int_data.state_indicator = rrcrce_int_data.save_dl_ccch_msg.decoded_msg->message.u.
                      rrcConnectionSetup.u.r3.rrcConnectionSetup_r3.rrc_StateIndicator;
                    /*lint -restore */

                    /* Initialize the Capability Update Requirement variable */
                    rrcdata_misc_info.rrc_capability_update_requirement.system_specific_cap_update_req_present = FALSE;
                    rrcdata_misc_info.rrc_capability_update_requirement.ue_radio_capability_update_requirement = FALSE;

                    /* Save the RRC Capability Update Requirement to build RRC
                       CONNECTION SETUP COMPLETE message */
                    /*lint -save -e613 */
                    rrcuece_copy_capability_update_requirement(wrrc_as_id,rrcrce_int_data.save_dl_ccch_msg.decoded_msg->message.u.
                      rrcConnectionSetup.u.r3.rrcConnectionSetup_r3.
                      capabilityUpdateRequirement);
                    /*lint -restore */
                  }
                  ...
                  if (msg_version == MSG_REL5)
                  {
                    ...
                    CONNECTION SETUP COMPLETE message */
                    /*lint -save -e613 */
                    rrcuece_copy_capability_update_requirement_later_than_r3(wrrc_as_id,rrcrce_int_data.save_dl_ccch_msg.decoded_msg->message.u.
                      rrcConnectionSetup.u.later_than_r3.criticalExtensions.u.
                      criticalExtensions.u.r5.rrcConnectionSetup_r5.capabilityUpdateRequirement);
                    /*lint -restore */
                  }
                    ...
                    (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL7))
                  {
                    ...
                    CONNECTION SETUP COMPLETE message */
                    /*lint -save -e613 */
                    rrcuece_copy_capability_update_requirement_r7(wrrc_as_id,&rrcrce_int_data.save_dl_ccch_msg.decoded_msg->message.u.
                      rrcConnectionSetup.u.later_than_r3.criticalExtensions.u.criticalExtensions.u.
                      criticalExtensions.u.criticalExtensions.u.r7.rrcConnectionSetup_r7.capabilityUpdateRequirement);
                    /*lint -restore */
                  }
                    ...
                    (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL8))
                  {
                    ...
                    CONNECTION SETUP COMPLETE message */
                    /*lint -save -e613 */
                    rrcuece_copy_capability_update_requirement_r8(wrrc_as_id,&rrcrce_int_data.save_dl_ccch_msg.decoded_msg->message.u.
                      rrcConnectionSetup.u.later_than_r3.criticalExtensions.u.criticalExtensions.u.
                ...
                (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL10))
                {
                  ...
                  /* Update RRC State Indicator */
                  rrcrce_int_data.state_indicator = rrcrce_int_data.save_dl_ccch_msg.decoded_msg->message.u.rrcConnectionSetup.u.later_than_r3.
                  criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.rrcConnectionSetup_r10.rrc_StateIndicator;
                  /*lint -restore */
                  /* Initialize the Capability Update Requirement variable */
                  rrcdata_misc_info.rrc_capability_update_requirement.system_specific_cap_update_req_present = FALSE;
                  rrcdata_misc_info.rrc_capability_update_requirement.ue_radio_capability_update_requirement = FALSE;
                  ...
                  rrcuece_copy_capability_update_requirement_r8(wrrc_as_id,&rrcrce_int_data.save_dl_ccch_msg.decoded_msg->message.u.
                  rrcConnectionSetup.u.later_than_r3.criticalExtensions.u.criticalExtensions.u.
                  criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.rrcConnectionSetup_r10.capabilityUpdateRequirement);
                  /*lint -restore */
                  /*Save 'Requested E-UTRA Frequency Band list' if present */
                  if((WCDMA_RRC_IDLE_FEATURES(RRC_NW_REQ_LTE_BAND_CAP_REPORT))&&
                     (rrcdata_misc_info.rrc_capability_update_requirement.system_specific_cap_update_req_present))
                  {
                    rrcuece_copy_req_eutra_freq_band_list_r10(wrrc_as_id, &rrcrce_int_data.save_dl_ccch_msg.decoded_msg->message.u.
                     rrcConnectionSetup.u.later_than_r3.criticalExtensions.u.criticalExtensions.u.
                     criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10);
                  }
                }
                
#endif /* FEATURE_WCDMA_REL10 */

                    rrcrce_send_rrc_channel_config_req_for_srb(wrrc_as_id,ESTABLISH_RB, RRC_STATE_CELL_FACH, TRUE);  
                    rrcrce_int_data.substate = RRCRCE_WAIT_FOR_SRB_SETUP_CNF;     
            ...
            else
            {
              ...
              /* Release RB 0 and request RRC LLC to go to Idle Disconnected */
              /* Do not send command to LLC to release SRBs Instead send Initiate Cell Selection Indication */
              rrcrce_send_rrc_initiate_cell_selection_ind(wrrc_as_id,RRC_TRANSITION_TO_DISCONNECTED, RRC_STATE_DISCONNECTED);
                  /* The RCE Procedure ends */
              rrcrce_int_data.substate = RRCRCE_WAIT_FOR_RB0_REL_CNF;  /* Change the RCE substate */
        ...
        switch(cmd_ptr->cmd.redirect_cnf.cause)
        {
          ...
          case RRC_CAMPED_ON_DIFFERENT_FREQ:
            ...
            else
            {
              ...
              if(rrccho_is_gtow_pcco_in_progress(wrrc_as_id))
              {
                ...
                WRRC_MSG1_HIGH("Max Retries when PCCO is active.rrcrce_substate %d",rrcrce_int_data.substate);      
                rrcrce_send_conn_setup_ind(wrrc_as_id,conn_setup_ind_params);                  
          ...
          case RRC_FAILED_TO_CAMP:
            ...
            if(rrccho_is_gtow_pcco_in_progress(wrrc_as_id))
            {
              ...
              WRRC_MSG1_HIGH("RRC_FAILED_TO_CAMP received when PCCO is active.rrcrce_substate %d",rrcrce_int_data.substate);      
              rrcrce_send_conn_setup_ind(wrrc_as_id,conn_setup_ind_params);                  
          ...
          case RRC_REDIRECTION_NOT_SUPPORTED:                    
            ...
            WRRC_MSG2_HIGH("RCE rcvd RDRCT_FAILURE cause %d. Strtng wt time %d", cmd_ptr->cmd.redirect_cnf.cause, rrcrce_int_data.wait_time);
          ...
          case RRC_REDIRECT_GSM_FAILURE:
            ...
            /* Allocates the buffer to RRC Internal command */
              rce_out_cmd_ptr =  rrc_get_int_cmd_buf(wrrc_as_id);
              /* Fill in the command parameters for RRC_CHANNEL_CONFIG_REQ */
              rce_out_cmd_ptr->cmd_hdr.cmd_id = RRC_CHANNEL_CONFIG_REQ;
              rce_out_cmd_ptr->cmd.chan_config_req.procedure = RRC_PROCEDURE_RCE;
              rce_out_cmd_ptr->cmd.chan_config_req.transaction_id = GET_RCE_TRANS_ID();
              rrcrce_int_data.chl_config_transaction_id = (uint8) rce_out_cmd_ptr->cmd.chan_config_req.transaction_id;
              WRRC_MSG1_HIGH("Assigned transaction id = %d",rrcrce_int_data.chl_config_transaction_id);
              ...
              rrc_put_int_cmd(wrrc_as_id,rce_out_cmd_ptr);

            rrcrce_int_data.substate = RRCRCE_WAIT_FOR_RB0_PHYCHAN_RESETUP_CNF;
            break;
          ...
          default:
            ...
            if(rrccho_is_gtow_pcco_in_progress(wrrc_as_id))
            {
              ...
              WRRC_MSG1_HIGH("REDIRECT_FAIL received when PCCO is active.rrcrce_substate %d",rrcrce_int_data.substate);      
              rrcrce_send_conn_setup_ind(wrrc_as_id,conn_setup_ind_params);                  
   ...
   * was allocated)
   */
  if(rrcrce_int_data.substate != RRCRCE_WAIT_FOR_REDIRECT_CNF)
  {
    /* Free the memory allocated to the encoded string */
    if(rrcrce_int_data.save_dl_ccch_msg.dl_sdu != NULL)
    {
      rrc_free(rrcrce_int_data.save_dl_ccch_msg.dl_sdu);
    }

    /* Free the memory allocated to the decoded message */
    if(rrcrce_int_data.save_dl_ccch_msg.decoded_msg != NULL)
    {
      (void) rrcasn1_free_pdu(wrrc_as_id,(int)rrcrce_int_data.save_dl_ccch_msg.dl_sdu_type, rrcrce_int_data.save_dl_ccch_msg.decoded_msg);
      rrcrce_int_data.save_dl_ccch_msg.decoded_msg = NULL;
    }
  }
  ...
  {   
    ...
    case RRC_STATE_CHANGE_IND:
      /* Stop the timer for wait time */     
       rrctmr_stop_timer(wrrc_as_id,RRCTMR_WAIT_TIME_TIMER);  
...
)
{
  ...
  {     
    ...
    cmd_ptr->cmd.chan_config_req.rrc_channel_config_cnf_required = 
      confirm_required;
    WRRC_MSG2_HIGH("No. of RBs in Chan config Req = %d, transaction id: %d",rrcrce_int_data.number_of_rbs, rrcrce_int_data.chl_config_transaction_id);
    
    if(ordered_config_ptr->srb5_info.srb5_rb_exists  == TRUE)
    {
      if(rrcrce_int_data.number_of_rbs == 5)
      {
    ...
    {
      if (rrcrce_int_data.number_of_rbs == 4)
      {
        cmd_ptr->cmd.chan_config_req.rb[count].rb_id = DCCH_DT_LOW_PRI_RADIO_BEARER_ID;
        cmd_ptr->cmd.chan_config_req.rb[count].rb_config = action;
      }
    ...
    if(rrclcm_get_rb_id_if_dl_lc_type_exists(wrrc_as_id, UE_LOGCHAN_TM_DCCH_DL, &rb_id) == RRCLCM_SUCCESS)
    {
    ...
    rrc_put_int_cmd(wrrc_as_id, cmd_ptr);                 /* sends the command to RRC
                                                 LLC. */
  }
...
{ 
  ...
  switch (cmd_ptr->cmd_hdr.cmd_id)
  {
    ...
    case RRC_ABORT_REQ:
      ...
      rrcrce_int_data.substate = RRCRCR_WAIT_FOR_SRB_REL_CNF;
      rrcrce_int_data.send_abort_cnf = TRUE;
      rrcrce_int_data.chl_config_transaction_id = 0;
      break;
    ...
    case RRC_NEW_CELL_IND:          
      ...
      { 
        ...
        rrcrce_send_rrc_abort_ind(wrrc_as_id, RRC_MAX_ACCESS_FAILURE);     
        rrc_log_protocol_error( wrrc_as_id, (uint8) (rrc_get_state(wrrc_as_id)),
                                (uint8) RRC_PROCEDURE_RCE,
                                (uint8) RRCLOG_RachFailure,
                                (uint8) RRCLOG_PROTOCOL_ERROR_NA
      ...
      if(cmd_ptr->cmd.chan_config_cnf.procedure == RRC_PROCEDURE_RCE)
      {
        ...
        if(cmd_ptr->cmd.chan_config_cnf.transaction_id == rrcrce_int_data.chl_config_transaction_id)
        {
          ...
          { 
            ...
            switch(rrc_translate_state_indicator_to_state(wrrc_as_id,rrcrce_int_data.state_indicator))
            {
              case RRC_STATE_CELL_FACH:
                /* Query for valid C-RNTI */
                if (C_RNTI_NOT_VALID == rrcllc_get_current_crnti_status(wrrc_as_id) )
                {
                  /* Send RRC_CELL_UPDATE_INITIATE_REQ command to CELL UPDATE Proc */
                  rrcrce_send_cell_update_initiate_req(wrrc_as_id, RRC_PROCEDURE_RCE);
                  /* Wait for CELL UPDATE to get a valid C_RNTI */
                  rrcrce_int_data.substate = RRCRCE_WAIT_FOR_CU_COMPLETE_CNF;
                }
                else
                {
                  /* Send RRC Connection Setup Complete message */
                  if (RRCRCE_SUCCESS != 
                    rrcrce_send_rrc_connection_setup_complete_message(wrrc_as_id))
                  {
                    WRRC_ERR0_FATAL("Unable to send RRC Connection Complete Msg");
                  }
                  else
                  {
                    ...
                    rrcllc_print_trans_chnl_rate(wrrc_as_id);
                    /* Clear the Ordered Config */
                    rrcllc_clear_ordered_config(wrrc_as_id);

                    /* Update the necesary parameters here*/
                    rrcsmc_reset_smc_ip_started_flag(wrrc_as_id);

                    rrccu_start_t_305_timer(wrrc_as_id);

                    /* Send RRC_EST_CNF command to MM */
                    rrcrce_send_rrc_est_cnf(wrrc_as_id);     
              ...
              default:
                WRRC_ERR1_FATAL(" RRC entered into invalid state :%d",
                            rrcrce_int_data.state_indicator);
                break;
            }
          ...
          {  
            ...
            rrcllc_init_ordered_hanging_rb_mapping_info(wrrc_as_id);
#else
            rrcllc_copy_current_hanging_rb_mapping_to_ordered(wrrc_as_id);
#endif


            (void)rrcrce_get_n300_from_serv_cell_db(wrrc_as_id,&rrcrce_n_300);
            if(rrcdata_timers_counters_info.rrc_v_300 <= rrcrce_n_300)
            {  

              if (rrcrce_int_data.state_indicator == rrc_RRC_StateIndicator_cell_DCH && rrc_get_state(wrrc_as_id) == RRC_STATE_CONNECTING)
              { 
                /* reset the ordered_config */
                if (OC_NOT_SET != rrcllc_reset_ordered_config(wrrc_as_id,RRC_PROCEDURE_RCE
                                                              ,TRUE
                                                              ))
                {
                  /* Do not send command to LLC to release SRBs Instead send Initiate Cell Selection Indication */
                ...
                else
                {
                  ...
                  rrctmr_start_timer(wrrc_as_id,RRCTMR_PHY_CHAN_FAILURE_TIMER, RRCTMR_PHYCHANFAILURE_TIMER_IN_MS);     
                  /* Initiate Cell Selection and substate should be RRCRCE_WAIT_FOR_RESELCT_COMPL */
                  rrcrce_send_rrc_initiate_cell_selection_ind(wrrc_as_id,
                    RRC_OUT_OF_SERVICE_AREA_IND,rrc_get_state(wrrc_as_id));
                  rrcrce_int_data.substate = RRCRCE_WAIT_FOR_RESELCT_COMPL;
              ...
              {
#ifdef FEATURE_WCDMA_HS_RACH
                if ((rrchsrach_camped_cell_supports_hsrach(wrrc_as_id) == TRUE) && (rrcrce_int_data.state_indicator != rrc_RRC_StateIndicator_cell_DCH) && 
                    (rrc_get_state(wrrc_as_id) == RRC_STATE_CONNECTING))
                {
                  /* reset the ordered_config */
                  if (OC_NOT_SET != rrcllc_reset_ordered_config(wrrc_as_id,RRC_PROCEDURE_RCE,TRUE))
                  {
    ...
    case RRC_CELL_UPDATE_COMPLETE_CNF:
      if(cmd_ptr->cmd.cu_complete_cnf.procedure == RRC_PROCEDURE_RCE)
      {
        ...
        rrctmr_start_timer(RRCTMR_RCE_L2ACK_TIMER, 
          ...
          /* Reset the rrcrce_num_retries_count */
          WRRC_MSG0_MED("Setting num_retries_count to 0");
          rrcrce_int_data.num_retries_count = 0;
...
)
{
  ...
  WRRC_MSG5_HIGH("Build connection setup complete WCDMA rel : %d, LTE mode : %d, GSM mode : %d, DMCR IE : %d, MFBI : %d",
rrc_nv_rel_indicator, rrcmcm_is_lte_mode_enabled_wo_f3(wrrc_as_id),
rrcmcm_is_dualmode_enabled_no_f3(wrrc_as_id),
rrcsib_is_dmcr_ie_needed(wrrc_as_id),
rrcnv_data.rrc_mfbi_support_nv);
  ...
  if(ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
    ...
    if (rrcdata_misc_info.rrc_capability_update_requirement.
    ...
    { 
      ...
      ret_flag = rrcueci_append_ue_radio_access_capability(wrrc_as_id,
      ...
      WRRC_MSG3_MED("RCE FDD2 Status:%d, NV Release:%d, HO_GAN Status:%d",
         ...
         )
      {
          ...
          )
        {
         ...
         RRC_MSG_COMMON_SET_BITMASK_IE_TYPE2(msg_ptr->message.u.rrcConnectionSetupComplete.v370NonCriticalExtensions.
          ...
          {
            WRRC_MSG0_MED("Populating r3_add_ext in  for GAN HO support");
          ...
          if(rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL7)
          {
            ...
            WRRC_MSG0_MED("Indicate supportForTwoDRXSchemesInPCH");
            ...
            WRRC_MSG0_MED("Indicate supportEDPDCHPowerInterpolation");
#ifdef FEATURE_WCDMA_MIMO
            if(support_tx_diversity_mimo)
            {
              ...
              WRRC_MSG0_MED("MIMO:Sending Tx Diversity for DL Control channels");
            }
#endif
          }
        }
      ...
      {
        WRRC_MSG0_MED("UE Cap REL5");

#if defined (FEATURE_CGPS_UMTS_CP_WCDMA)
        if(rrcueci_ext_data.rrc_ue_pos_cap_pch)
        {
          RRC_MSG_COMMON_SET_BITMASK_IE_TYPE2(msg_ptr->message.u.rrcConnectionSetupComplete.v370NonCriticalExtensions.v380NonCriticalExtensions.
          ...
          WRRC_MSG0_MED("RRCGPS: PCH validity set");
        }
#endif
        ...
        if (rrc_cap_related_info.rrc_pdcp_enabled)
        { 
          msg_ptr->message.u.rrcConnectionSetupComplete.v370NonCriticalExtensions.
            v380NonCriticalExtensions.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
            v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.rrcConnectionSetupComplete_v4b0ext.
        ...
        if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL7)
        {
          ...
          if(rrcsib_is_dmcr_ie_needed(wrrc_as_id) == TRUE)
          {
              WRRC_MSG0_MED("Deferred SIB read:Appending Deferred meas read IE. in con. setup compl");
              msg_ptr->message.u.rrcConnectionSetupComplete.v370NonCriticalExtensions.
                 v380NonCriticalExtensions.v3a0NonCriticalExtensions.laterNonCriticalExtensions.
                 v3g0NonCriticalExtensions.v4b0NonCriticalExtensions.v590NonCriticalExtensions.
        ...
        /* Appends the Radio Access Capability to the RRC CONNECTION SETUP
           COMPLETE message */
        rrcueci_append_ue_radio_access_capability_later_than_r3(wrrc_as_id,&msg_ptr->message.u.rrcConnectionSetupComplete.
    ...
    if (rrcmcm_is_dualmode_enabled_wo_f3(wrrc_as_id))
    {
      if (rrcdata_misc_info.rrc_capability_update_requirement.
         ...
         be included even though UTRAN requests the same */
      {
        int count;
        num_rat=0;
        for(count = 0; count < rrcdata_misc_info.rrc_capability_update_requirement.system_specific_cap_update_req_list.count && count < 16;count++)
        {
          if(rrcdata_misc_info.rrc_capability_update_requirement.system_specific_cap_update_req_list.value[count] == RRC_GSM_CAP)
          {
            num_rat = 1;
            break;
          }
        ...
        {      
          ...
          rrcueci_append_gsm_specific_capability(wrrc_as_id,
    ...
    if(rrcrce_int_data.state_indicator == rrc_RRC_StateIndicator_cell_DCH)
    {
      rrc_update_reconfig_waiting_for_l2ack_dch(wrrc_as_id, RRC_PROCEDURE_RCE);
    }
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
    rrc_qsh_log_conn_setup_compl_params(wrrc_as_id, rrcueci_get_cap_container_length(wrrc_as_id));
#endif
  ...
  /*Stop CCCH QTA timer - so that WL1 can allow QTA as there is no more CCCH transmission*/
  rrcwrm_stop_wl1_qta_ccch_timer(wrrc_as_id);
  return (status);            
} /* rrcrce_send_rrc_connection_setup_complete_message */
...
)
{
  ...
  {     
    ...
    cmd_ptr->cmd.redirect_req.wait_time = rrcrce_int_data.wait_time;
                ...
                v690NonCriticalExtensionsPresent)
            {
                ...
                v690NonCriticalExtensions.rrcConnectionReject_v690ext.m.redirectionInfo_v690extPresent)
              {
                        ...
                        (cmd_ptr->cmd.redirect_req.u.GSM_cell_info_list.size < MAX_RRC_RR_GSM_CELL_LIST_SIZE))
                 {
                    ...
                    if((gsm_band
                            = rr_rrc_get_arfcn_band_internal(wrrc_as_id,(uint16)GSM_target_cell_info_ptr->elem[local_idx].bcch_ARFCN, freq_band))
                         != INVALID_BAND)
                    {
    ...
    {
      cmd_ptr->cmd.redirect_req.choice= RRC_FREQ_REDIRECTION;
      if(update_sib7_in_connecting == TRUE)
      {
        rrcsib_force_sib_event_init(wrrc_as_id, RRCSIB_ABORT_SIB7_UPDATE);
      }
      ...
      else
      {
        ...
        { 
         ...
         else if (RRCRCE_R10_CRITICAL_EXT_PRESENT)
         { 
           if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL10)
           {
             /* Safely assume REL-8 as other cases would have been filtered by the time we are here */
    ...
    {
      WRRC_ERR1_FATAL("ERR: incorrect enum type %d for connection rsp", rrcrce_int_data.rsp_to_rrc_conn_req);
    }
...
)
{
...
#ifdef FEATURE_DUAL_SIM
    cmd_ptr->cmd.rrc_rel_ind.as_id = rrc_get_as_id(wrrc_as_id);
#endif
    /* Fill in parameters fro RRC_REL_IND command */
    cmd_ptr->cmd.rrc_rel_ind.rel_cause = rrcdata_misc_info.rrc_release_cause;
    
    /* Update the Max HFNs values to be stored in SIM or USIM */ 
    (void) rrcsmc_append_hfns_to_rel_ind(wrrc_as_id,cmd_ptr->cmd.rrc_rel_ind.hfn_cs,
                                  cmd_ptr->cmd.rrc_rel_ind.hfn_ps);

    rrc_send_rrc_message_to_mm(wrrc_as_id,cmd_ptr);               /* Sends the command to MM */
   
    WRRC_MSG1_HIGH("Dispatching out MM cmd / RRC sends RRC_REL_IND to MM,Cause %d",rrcdata_misc_info.rrc_release_cause);

    rrcrce_send_conn_rel_cause_event(wrrc_as_id,(uint8)RRC_ABORT_CAUSE_TYPE_MAX + (uint8)rrcdata_misc_info.rrc_release_cause);

#ifdef FEATURE_DUAL_SIM
  if(rrc_csp_int_data.curr_scan != RRC_CSP_INTER_RAT_REDIRECTION_SCAN) /*Do not rel lock as the lock will be exchanged with irat_client*/
  {
    rrc_release_lock(wrrc_as_id);
  }
  rrc_dsds_int_data.nas_proc_type = SYS_PROC_TYPE_NONE;
  ...
  rrc_send_rab_rel_ind_on_ps_release(wrrc_as_id);
  ...
  {
    rrc_csp_check_n_send_suitable_srch_end_ind(wrrc_as_id);
    rrc_csp_send_disable_bplmn_ind(wrrc_as_id, FALSE);
  }
#endif
  ...
  {
    rrcpg1_send_page_ind(wrrc_as_id);
    ...
    case RRC_STATE_CHANGE_IND:
         ...
         ( rrc_get_state(wrrc_as_id) == RRC_STATE_DISCONNECTED))
      {
        if((RRC_MODE_ACTIVE == rrcmcm_get_rrc_mode(wrrc_as_id))
          ...
          */
          || ((RRC_MODE_STANDBY == rrcmcm_get_rrc_mode(wrrc_as_id))
             && (rrccsp_get_curr_scan(wrrc_as_id) == RRC_CSP_INTER_RAT_REDIRECTION_SCAN))
#endif
#endif
#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
         ||(RRC_PSEUDO_MEAS_WAIT_FOR_STATE_CHANGE == rrc_pseudo_meas_get_substate(wrrc_as_id))
#endif
#ifdef FEATURE_WRLF_SYSTEM_SEL
          || (RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS() && 
             (RRC_MODE_INACTIVE == rrcmcm_get_rrc_mode(wrrc_as_id)))
#endif
#ifdef FEATURE_DUAL_SIM
          /*This is possible if hard abort is received when waiting for 
             lock to resumse after LTA*/
          ||((RRC_MODE_INACTIVE == rrcmcm_get_rrc_mode(wrrc_as_id))&&
             (rrc_dsds_int_data.wl1_failure_cause == WL1_FAILED_NO_WRM_LOCK)
            )
#endif
         )
        {
          ...
          {
            rrc_dsds_int_data.wl1_failure_cause = WL1_FAILED_NORMAL;
          }
#endif
          ...
          {
              rrcrce_send_rrc_rel_ind(wrrc_as_id);
              /* Clear the ESTABLISHED_RABS while bringing down RRC Connection*/
              rrc_clear_established_rabs(wrrc_as_id);
  ...
  switch (cmd_ptr->cmd_hdr.cmd_id)
  {
    ...
    case RRC_STATE_CHANGE_IND:
      ...
      if (cmd_ptr->cmd.state_change_ind.new_state == RRC_STATE_DISCONNECTED)
      {
        ...
        {     
          if(!rrcrce_int_data.abort_ind_sent)
          {
            rrcrce_send_rrc_abort_ind(wrrc_as_id,rrcrce_int_data.mm_abort_cause);      
          }
          /* The RCE Procedure ends */
          rrcrce_int_data.substate = RRCRCE_INITIAL;    /* reset the RCE substate */
          rrcrce_int_data.abort_ind_sent = FALSE;
    ...
    case RRC_EST_REQ: 
      ...
      if(RRCRCE_SUCCESS == rrcrce_validate_rrc_est_req(wrrc_as_id,cmd_ptr))
      {
        ...
        WRRC_MSG6_HIGH(
             ...
             rrcrce_int_data.est_cn_domain_id,RRC_CSP_GET_INT_MCC(rrcrce_int_data.est_req_for_rai.lai.plmn),
             RRC_CSP_GET_INT_MNC(rrcrce_int_data.est_req_for_rai.lai.plmn),
             rrcrce_int_data.est_req_for_rai.lai.lac[0],rrcrce_int_data.est_req_for_rai.lai.lac[1],
             rrcrce_int_data.est_req_for_rai.rac[0]);
  ...
  switch (cmd_ptr->cmd_hdr.cmd_id)
  {
...
#ifdef FEATURE_WCDMA_FAST_RETURN_TO_LTE_AFTER_CSFB
      else if((rrc_csp_int_data.csfb_extended_status == CSFB_CS_RB_RELEASED) && 
...
#endif
#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
      else if(RRC_PSEUDO_MEAS_WAIT_FOR_STATE_CHANGE == rrc_pseudo_meas_get_substate(wrrc_as_id))
      {
	rrcrce_send_rrc_rel_ind(wrrc_as_id);     
      }
#endif
      ...
      /* Reset Signalling Connections status */
      rrcscr_reset_signalling_connection_status(wrrc_as_id);
      rrcrce_int_data.sig_conn_rel_req_for_both_domains = FALSE;
      break;
    ...
    case RRC_ABORT_REQ:
      ...
      if(initiate_cell_sel)
      {
        ...
        rrc_transition_to_disconnected_state(wrrc_as_id,RRC_PROCEDURE_RCE, RRC_TX_TO_DISCON_HARD_ABORT);
    ...
    case RRC_SIG_CONN_RELEASE_CNF:
      ...
      if(rrc_csp_int_data.csfb_extended_status == CSFB_CS_RB_RELEASED)
      {
        ...
        rrctmr_start_timer(wrrc_as_id,RRCTMR_RRC_CON_DELAY_TIMER, 
          (unsigned long) rrcnv_data.rrc_fast_return_to_lte_after_csfb_timer_nv); 
        
        WRRC_MSG1_HIGH("CSFB_HACK:Started wait for RRC conn release timer of value :%d ms",
          rrcnv_data.rrc_fast_return_to_lte_after_csfb_timer_nv); 
        /* Sending Close session Indication as otherwise RRC and NAS will go out of sync.*/
        rrcscr_send_close_session_ind(wrrc_as_id,RRC_PS_DOMAIN_CN_ID, RRC_MAX_SESSION_CLOSE_CAUSE);
        rrcrce_int_data.substate = RRCRCE_WAIT_FOR_RRC_TIMER_TO_EXPIRE;
        break;
      }
#endif
#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
      if(rrc_pseudo_meas_triggered_conn_abort_req(wrrc_as_id) == TRUE)
      {
        ...
        WRRC_PSEUDO_MEAS_DEBUG_1("Started wait for RRC conn release timer of value :%d ms",
          2000); 
        /* Sending Close session Indication as otherwise RRC and NAS will go out of sync.*/
        rrcscr_send_close_session_ind(wrrc_as_id,RRC_PS_DOMAIN_CN_ID, RRC_MAX_SESSION_CLOSE_CAUSE);
        rrcrce_int_data.substate = RRCRCE_WAIT_FOR_RRC_TIMER_TO_EXPIRE;
...
#endif
      if (rrcrce_int_data.sig_conn_rel_req_for_both_domains == TRUE)
      {
...
{ 
  ...
  oc_status = rrcllc_get_ordered_config_status(wrrc_as_id);
#endif 
  switch (cmd_ptr->cmd_hdr.cmd_id)
  {
  case RRC_DOWNLINK_SDU_IND:
  ...
  case RRC_STATE_CHANGE_IND:
    ...
    rrctmr_stop_timer(wrrc_as_id, RRCTMR_RRC_CON_DELAY_TIMER);
    /* Send RRC_ABORT_CNF command to the MM since RCE already in this substate
    since it has received RRC_ABORT_REQ */
...
#if defined(FEATURE_WCDMA_FAST_RETURN_TO_LTE_AFTER_CSFB) || defined(FEATURE_WCDMA_W2L_PSEUDO_MEAS)
    if( 0 
    #ifdef FEATURE_WCDMA_FAST_RETURN_TO_LTE_AFTER_CSFB
       ||
       (rrc_csp_int_data.csfb_extended_status == CSFB_CS_RB_RELEASED)
...
#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
       ||
       (rrc_pseudo_meas_triggered_conn_abort_req(wrrc_as_id) == TRUE)
#endif
      )
    {
      ...
      if (((rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_PCH) || (rrc_get_state(wrrc_as_id) == RRC_STATE_URA_PCH) 
      ...
      { 
        current_drx_cycle_ms = 10*(rrcllc_get_drx_info(wrrc_as_id));
        ...
        WRRC_MSG3_HIGH("CSFB_HACK:Check  Restart wait %d %d %d",
          rrcnv_data.rrc_fast_return_to_lte_after_csfb_timer_nv,current_drx_cycle_ms,RRC_GUARD_TIMER_FOR_PCH_TRANS_BEFORE_WTOL_FAST_REDIR); 
        ...
        {
           rrctmr_start_timer(wrrc_as_id,RRCTMR_RRC_CON_DELAY_TIMER, 
             (unsigned long) ( (2*current_drx_cycle_ms) + RRC_GUARD_TIMER_FOR_PCH_TRANS_BEFORE_WTOL_FAST_REDIR - rrcnv_data.rrc_fast_return_to_lte_after_csfb_timer_nv )); 
           rrcrce_int_data.crt_restarted_once = TRUE;
           break;
        }
      }
    }
#endif /*(FEATURE_WCDMA_FAST_RETURN_TO_LTE_AFTER_CSFB) || defined(FEATURE_WCDMA_W2L_PSEUDO_MEAS)*/

#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
/*Restarting the connection delay timer if any reconfig message comes before the conn delay timer expiry.*/
...
#endif
    {
      rrctmr_start_timer(wrrc_as_id,RRCTMR_RRC_CON_DELAY_TIMER,RRCTMR_RRC_CON_DELAY_TIMER_FOR_CONCURRENCY_CASES);
      break;
    }
    if(is_iho_w2g_active(wrrc_as_id) == TRUE)
    {
      WRRC_MSG0_HIGH("CSFB_HACK: HO to GSM/EUTRA active, Defer timer by 500 ms ");
      rrctmr_start_timer(wrrc_as_id,RRCTMR_RRC_CON_DELAY_TIMER,RRCTMR_RRC_CON_DELAY_TIMER_FOR_CONCURRENCY_CASES);
      break;
    }
    if (rrcrce_int_data.sig_con_delay == TRUE)
    {
      ...
      else
      {
        if(rrcscr_get_signalling_connection_sts(wrrc_as_id,RRC_CS_DOMAIN_CN_ID) )
        {
          ...
          /* Allocates the buffer to RRC Internal command */
          rrcrce_close_sess_cmd_ptr =  rrc_get_int_cmd_buf(wrrc_as_id);
          ...
          rrc_put_int_cmd(wrrc_as_id,rrcrce_close_sess_cmd_ptr);  
          cnt++;
        }
        ...
        if (cnt == 2)
        {
          ...
          rrcrce_int_data.sig_conn_rel_req_for_both_domains = TRUE;
        }
        
        /* Wait for signalling connection release to complete */
        rrcrce_int_data.substate = RRCRCE_WAIT_FOR_SIG_CON_REL_COMPL;
...
)
{
  ...
  switch (cmd_ptr->cmd_hdr.cmd_id)
  {
      ...
      else
      {
        ...
        if(rrcscr_get_signalling_connection_sts(wrrc_as_id,RRC_CS_DOMAIN_CN_ID) )
        {
          ...
          if (cmd_ptr->cmd.rrc_abort_req.delay_sig_con_release == TRUE)
          {
            ...
            rrcrce_int_data.sig_con_delay = TRUE;
#ifndef FEATURE_WCDMA_FIELD_OPTIMIZATIONS
    ...
    case RRC_DOWNLINK_L2ACK_IND:
       ...
       {
         rrc_send_lte_rrc_wcdma_eutra_cap_sent_ind(wrrc_as_id);
       }
       break;

#ifdef FEATURE_WCDMA_FAST_RETURN_TO_LTE_AFTER_CSFB
    case RRC_INTERNAL_CSFB_ABORT_REQ:
      WRRC_MSG1_HIGH("CSFB_HACK:RRC_INTERNAL_CSFB_ABORT_REQ rcvd in %d substate",rrcrce_int_data.substate);
...
#endif
#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
    case RRC_PSEUDO_MEAS_CONN_ABORT_REQ:
      if(rrcscr_get_signalling_connection_sts(wrrc_as_id,RRC_CS_DOMAIN_CN_ID) == FALSE)
      {
        if(rrcscr_get_signalling_connection_sts(wrrc_as_id,RRC_PS_DOMAIN_CN_ID))
        {
          WRRC_PSEUDO_MEAS_DEBUG_0("PS domain open, send SCRI.");
        ...
        {
          WRRC_PSEUDO_MEAS_DEBUG_1("PS domain not open. Started wait to redirect to L %d ms.",
          2000);

          /* start the timer for UE before it can release RRC Connection */
          rrctmr_start_timer(wrrc_as_id,RRCTMR_RRC_CON_DELAY_TIMER,2000); 
      ...
      {
        rrc_check_and_suspend_pseudo_meas_procedure(wrrc_as_id,RRC_PROCEDURE_RCE);
      }
      break;
#endif
    default:
...
{
  if(rrcrce_int_data.substate != RRCRCE_FINAL) 
  {
...
===========================================================================*/
boolean  rrcrce_is_rce_procedure_inactive(sys_modem_as_id_e_type wrrc_as_id)
{
  if ((rrcrce_int_data.substate == RRCRCE_INITIAL) ||
      (rrcrce_int_data.substate == RRCRCE_WAIT_FOR_UE_CAMP_ON))
  {
...
{
  WRRC_MSG1_HIGH("Changing substate %d to Final",rrcrce_int_data.substate);
  rrcrce_int_data.substate = RRCRCE_FINAL;
...
/* PPAC */
  ...
  {
    reg_allowed = rrc_csp_get_loc_reg_allowed_status(wrrc_as_id,cn_domain_id);
    if(reg_allowed)
    {
      WRRC_MSG1_MED("PPAC: Loc/Reg allowed for %d domain",cn_domain_id);
      return (status = RRCRCE_SUCCESS);
    }
    else if(!reg_allowed && rrc_csp_is_ppac_valid(wrrc_as_id))
    {
      WRRC_MSG1_MED("PPAC: Loc/Reg not allowed for %d domain",cn_domain_id);
  ...
  {
    page_rsp_allowed = rrc_csp_get_page_rsp_allowed_status(wrrc_as_id,cn_domain_id);
    if(page_rsp_allowed)
    {
  ...
  cell_access_status = rrc_csp_get_curr_cell_access_status(wrrc_as_id,cn_domain_id);
  ...
  switch (cell_access_status)
  {
    case MM_AS_ACCESS_ALL_CALLS:
      {
        status = RRCRCE_SUCCESS;
        WRRC_MSG1_MED("Cell Access allowed:MM_AS_ACCESS_ALL_CALLS, status=%d",status);
      }
      break;
    ...
    case MM_AS_ACCESS_EMERGENCY_ONLY:
      ...
      {
        status = RRCRCE_SUCCESS;
        WRRC_MSG1_MED("Cell Acces allowed:MM_AS_ACCESS_EMERGENCY_ONLY, status=%d",status);
      }
      else
      {
        *cause = RRC_ACCESS_CLASS_NOT_ALLOWED;
        status = RRCRCE_FAILURE_MINOR;
        WRRC_MSG2_MED("Cell Acces not allowed for regular voice calls status=%d, cause=%d ",status,*cause);
      }
      break;
    ...
    case MM_AS_ACCESS_NO_CALLS:
    {
      *cause = RRC_ACCESS_CLASS_NOT_ALLOWED;
      status = RRCRCE_FAILURE_MINOR;
      WRRC_MSG2_MED("Cell Acces not allowed for any calls status=%d, cause=%d ",status,*cause);
    }
    break;
    ...
    case MM_AS_ACCESS_NORMAL_ONLY:
      ...
      {
        status = RRCRCE_SUCCESS;
        WRRC_MSG0_MED("Cell Acces allowed for regular voice calls");
      }
      else
      {
        *cause = RRC_ACCESS_CLASS_NOT_ALLOWED;
        status = RRCRCE_FAILURE_MINOR;
        WRRC_MSG2_MED("Cell Access not allowed for the calls other than normal/regular voice calls status=%d, cause=%d ",status,*cause);
      }
    break;
  }
  return status;
...
)
{
  if(rrcrce_int_data.substate != RRCRCE_INITIAL)
  {
    WRRC_MSG1_HIGH("Force Initialize.Changing substate %d to Initial",rrcrce_int_data.substate);
  }
  ...
  else if((rrcrce_int_data.substate == RRCRCE_WAIT_FOR_UE_CAMP_ON) ||
          (rrcrce_int_data.substate == RRCRCE_WAIT_FOR_SIB7_CURRENT))
  {
    if(rrcrce_int_data.substate != RRCRCE_WAIT_FOR_SIB7_CURRENT)
    {
      /* Stop the timer to UE to camp on */  
      rrctmr_stop_timer(wrrc_as_id, RRCTMR_UE_TO_CAMP_ON_TIMER);
    }
  ...
  else if (rrcrce_int_data.substate == RRCRCE_WAIT_FOR_RB0_REL_CNF)
  {
    if (rrcrce_int_data.send_abort_cnf == TRUE)
    {
      /* Send RRC_ABORT_CNF command to the MM */
  ...
  else if (rrcrce_int_data.substate == RRCRCR_WAIT_FOR_SRB_REL_CNF)
  {
      /* Send RRC_ABORT_CNF command to the MM */
      rrcrce_send_rrc_abort_cnf(wrrc_as_id);     
      /* At this time, none of the RBs are setup. Simply reset the RCE
  ...
  else if ((rrcrce_int_data.substate == RRCRCE_WAIT_FOR_RB0_SETUP_CNF) ||
           (rrcrce_int_data.substate == RRCRCE_WAIT_FOR_WAIT_TIMER_TO_EXPIRE) ||
           (rrcrce_int_data.substate == RRCRCE_WAIT_FOR_SRB_SETUP_CNF) ||
           (rrcrce_int_data.substate == RRCRCE_WAIT_FOR_L2_ACK) ||
           (rrcrce_int_data.substate == RRCRCE_WAIT_FOR_RB0_PHYCHAN_RESETUP_CNF))
  {
    if (rrcrce_int_data.send_abort_cnf == TRUE)
    {
...
===========================================================================*/
struct rrc_SysInfoType16 * rrcrce_get_sib16_ptr(sys_modem_as_id_e_type wrrc_as_id)
{
  struct rrc_SysInfoType16 * sib16_ptr = NULL;

  if(rrcrce_int_data.default_config_info.config_type == RRC_DEFAULT_CONFIG)
  {
    sib16_ptr = rrcrce_int_data.default_config_info.sib16_ptr;
  }
...
===========================================================================*/
struct rrc_DL_DCCH_Message * rrcrce_get_rbs_ptr(sys_modem_as_id_e_type wrrc_as_id)
{
  struct rrc_DL_DCCH_Message * rbs_ptr = NULL;

  if(rrcrce_int_data.default_config_info.config_type == RRC_DEFAULT_CONFIG)
  {
    rbs_ptr = rrcrce_int_data.default_config_info.rbs_ptr;
  }
...
)
{
  ...
  if(rrcrce_int_data.default_config_info.config_type == RRC_DEFAULT_CONFIG)
  {
    if(rrcrce_int_data.default_config_info.dcch_msg_ptr != NULL)
    {
      rb_setup_r6_ptr = &(rrcrce_int_data.default_config_info.dcch_msg_ptr->message.u.radioBearerSetup.
...
)
{
    ...
    (rrcrce_int_data.default_config_info.default_config_id == RRC_DEF_CONFIG_17))
  {
    WRRC_MSG1_HIGH("DC I.D =%d config on HSPA channels",
             rrcrce_int_data.default_config_info.default_config_id );
...
{
  WRRC_MSG1_MED("Est Cause is %d", rrcdata_misc_info.rrc_establishment_cause);
  return rrcdata_misc_info.rrc_establishment_cause;      
} /* rrcrce_get_rrc_est_cause */

#ifdef FEATURE_DUAL_SIM
/*===========================================================================
FUNCTION rrcrce_is_high_priority_lock_needed

DESCRIPTION
  Returns TRUE if CS call or SMS or high priority call is in progress

DEPENDENCIES
  None

RETURN VALUE
  Returns TRUE if high priority lock is needed

SIDE EFFECTS
  None
===========================================================================*/
boolean rrcrce_is_high_priority_lock_needed
(
  sys_modem_as_id_e_type wrrc_as_id
)
{
  boolean ret_val = FALSE;
  WRRC_MSG1_MED("DSIM: NAS procedure type %d",rrc_dsds_int_data.nas_proc_type);

  if(
      (rrc_dsds_int_data.nas_proc_type == SYS_PROC_TYPE_CS_CALL) ||
      (rrc_dsds_int_data.nas_proc_type == SYS_PROC_TYPE_HIGH_PRIORITY_PS_CALL) ||
...
{
  WRRC_MSG2_HIGH("Updating Est Cause to %d, currently it is %d", RRC_EST_EMERGENCY_CALL,rrcdata_misc_info.rrc_establishment_cause);
  rrcdata_misc_info.rrc_establishment_cause = RRC_EST_EMERGENCY_CALL;
} /* rrcrce_get_rrc_est_cause */
#endif
/*===========================================================================
...
none
===========================================================================*/
boolean  rrcrce_is_rce_procedure_active(sys_modem_as_id_e_type wrrc_as_id)
{
  if(rrcrce_int_data.substate == RRCRCE_INITIAL)
  {
    /* RCE will not be considered "active" if in one
...
none
===========================================================================*/
boolean  rrcrce_is_rce_procedure_not_in_intial_or_final_substate(sys_modem_as_id_e_type wrrc_as_id)
{
  if(rrcrce_int_data.substate == RRCRCE_INITIAL
     ||
     rrcrce_int_data.substate == RRCRCE_FINAL)
...
#endif
rrcrce_substate_e_type  rrcrce_get_rce_sub_state(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcrce_int_data.substate;
...
none
===========================================================================*/
boolean rrcrce_check_for_rai_mismatch(sys_modem_as_id_e_type wrrc_as_id,rrc_plmn_identity_type camped_plmn, rrc_SysInfoType1 *sib1_ptr)
{
  ...
  if(rrcrce_int_data.est_cn_domain_id != RRC_CS_DOMAIN_CN_ID)
  {
    ...
    if(rrc_extract_rac_from_sib_info(wrrc_as_id,&camped_on_rai, &sib1_ptr->cn_DomainSysInfoList) == FALSE)
    {
  ...
  WRRC_MSG4_HIGH("Checking for LAI/RAI mismatch. Camped on PLMN: %d-%d, LAC: %x %x",
                  RRC_CSP_GET_INT_MCC(camped_on_rai.lai.plmn),
                  RRC_CSP_GET_INT_MNC(camped_on_rai.lai.plmn),
                  camped_on_rai.lai.lac[0],
                  camped_on_rai.lai.lac[1]
                );

  if((camped_on_rai.lai.plmn.num_mnc_digits ==
       rrcrce_int_data.est_req_for_rai.lai.plmn.num_mnc_digits ) &&

     rrcrce_compare_arrays(camped_on_rai.lai.plmn.mcc,
                           rrcrce_int_data.est_req_for_rai.lai.plmn.mcc,
                           RRC_MAX_MCC_DIGITS)  &&
     rrcrce_compare_arrays(camped_on_rai.lai.plmn.mnc,
                           rrcrce_int_data.est_req_for_rai.lai.plmn.mnc,
                           rrcrce_int_data.est_req_for_rai.lai.plmn.
                           num_mnc_digits)  &&
     rrcrce_compare_arrays(camped_on_rai.lai.lac,
                           rrcrce_int_data.est_req_for_rai.lai.lac,
                           RRC_MAX_GSMMAP_LAC_LENGTH) 
    )
  {
    if(!compare_rai)
    {
      WRRC_MSG0_MED("LAI MATCHED");
      return FALSE;
    }
    else if(rrcrce_compare_arrays(camped_on_rai.rac,
                             rrcrce_int_data.est_req_for_rai.rac,
                             RRC_MAX_GSMMAP_RAC_LENGTH))
    {
      WRRC_MSG0_MED("RAI MATCHED");
      return FALSE;
    }
  }
  else
  {
    ...
    rrcrce_int_data.lai_rai_mismatch = TRUE;
  }
  return TRUE;
}
...
SIDE EFFECTS
  None
===========================================================================*/
rrc_cn_domain_identity_e_type rrc_rce_get_est_cn_domain_id(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcrce_int_data.est_cn_domain_id;
}
...
)
{
  ...
  switch(conn_rel_cause) 
  {
    case (uint8)RRC_FAIL_TO_CAMP_ON:
      ...
      WRRC_EVENT_REPORT_PAYLOAD(EVENT_WCDMA_CONN_REL_CAUSE, sizeof(conn_rel_cause_event),
...
)
{
  ...
  if(conn_req_cause <= RRC_EST_TERM_UNKNOWN)
  {
    ...
    WRRC_EVENT_REPORT_PAYLOAD(EVENT_WCDMA_CONN_REQ_CAUSE, sizeof(conn_req_cause_event),
...
===========================================================================*/
void rrcrce_post_internal_csfb_abort_cmd(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_cmd_type *int_cmd_ptr;
...
{
  boolean lai_rai_mismatch = rrcrce_int_data.lai_rai_mismatch;
  boolean lai_mismatch = rrcrce_check_for_rai_mismatch(wrrc_as_id,plmn_id,sib1_ptr);

  /*Restore rrcrce_lai_rai_mismatch. If this is set we may end up 
    sending wrong cause in abort indication to NAS*/
  rrcrce_int_data.lai_rai_mismatch = lai_rai_mismatch;

  if((((rrcrce_int_data.substate == RRCRCE_WAIT_FOR_RRC_CONN_SETUP)&&
       (rrcdata_timers_counters_info.rrc_v_300 <= 1)&&
       (rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_T_300_TIMER) > 0)&&
       (rrcrce_int_data.cmac_status == RACH_TX_SUCCESS))||
       (rrcrce_int_data.substate == RRCRCE_WAIT_FOR_SRB_SETUP_CNF))&&
      (rrcrce_int_data.est_cn_domain_id == RRC_CS_DOMAIN_CN_ID)&&
      (rrcdata_misc_info.rrc_establishment_cause == RRC_EST_TERM_CONV_CALL)&&
  ...
  {
    rrcrce_int_data.cell_resel_freq = freq;
    rrcrce_int_data.cell_resel_scr = scr_code;
    return TRUE;
  }
  return FALSE;
}
...
===========================================================================*/
boolean rrcrce_ignore_resel(sys_modem_as_id_e_type wrrc_as_id,rrc_freq_type freq,rrc_scr_code_type scr_code)
{
  if((rrcrce_int_data.cell_resel_freq == freq)&&
     (rrcrce_int_data.cell_resel_scr == scr_code)
    )
  {
    return TRUE;
...
===========================================================================*/
  ...
  __attribute__((section(".uncompressible.text")))
#endif
rrc_abort_cause_e_type rrcrce_get_mm_abort_cause(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcrce_int_data.mm_abort_cause; 
}
...
===========================================================================*/
  ...
  __attribute__((section(".uncompressible.text")))
#endif
boolean rrcrce_return_abort_ind_sent_status(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcrce_int_data.abort_ind_sent;
}
/*===========================================================================
...
===========================================================================*/
  ...
  __attribute__((section(".uncompressible.text")))
#endif
boolean rrcrce_return_send_abort_cnf(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcrce_int_data.send_abort_cnf;
}
/*===========================================================================
FUNCTION rrcrce_check_rau_progress_for_ss

DESCRIPTION
  This function looks into RAU status for SS 
  
DEPENDENCIES
  None

RETURN VALUE
None if RAU is n progress timer will get restarted if RAU completed post Internal CSFB abort Indication 

SIDE EFFECTS
  None
===========================================================================*/
void rrcrce_check_rau_progress_for_ss
(
sys_modem_as_id_e_type wrrc_as_id
)
{
  if(((mm_per_subs_is_ss_in_progress(wrrc_as_id)) && (rrcnv_data.rrc_fast_return_to_lte_after_csfb_ss_nv)) && (rrc_csp_int_data.csfb_extended_status == CSFB_STARTED))
  {
   WRRC_MSG1_HIGH("RAU is in Progress %d",gmm_per_subs_is_rau_in_progress(wrrc_as_id));
   if(gmm_per_subs_is_rau_in_progress(wrrc_as_id))
   {
    WRRC_MSG0_HIGH("CSFB_HACK: RAU is still in Progress wait for RAU Complete timer restarted");
    /* start the timer for UE before it can release RRC Connection when RAU is in progress for SS*/	  
...
} /* rrcrce_check_rau_progress_for_ss */
...
{
  qsh_mdump_collect_high(&rrcrce_int_data,sizeof(rrcrce_int_data));
}
#endif
