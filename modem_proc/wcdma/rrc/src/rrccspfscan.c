...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrccspfscan.c#3 $    $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/31/17   ha      FR  42544 MPLMN periodic scan search
04/27/18   sn      Changes to correct WCDMA 1800 and 1700 bands frequency range checks
03/14/18   sn      Changes to check if raw scan results can be skipped for fine scan 
...
11/02/15   sr      Changes to not include LCF for PLMN scans
...
/*This macro picks the entry from wcdma freq list introduced for periodic frequency scan search */
#define  RRC_CSP_PICK_FREQ_FOR_ACQ()   \
  {                          \
    *ret_freq = rrc_csp_int_data.freq_scan_list.wcdma_freq_list[rrc_csp_int_data.mplmn_freq_list_index]; \
    rrc_csp_int_data.mplmn_freq_list_index ++;  \
  }
/* -----------------------------------------------------------------------
...
SIDE EFFECTS
                  None
===========================================================================*/
static boolean rrccsp_should_overlapping_freq_be_skipped(sys_modem_as_id_e_type wrrc_as_id,rrc_freq_type freq)
{
   ...
   frequency = rrccsp_derive_freq_from_uarfcn(wrrc_as_id,freq,rrc_csp_curr_select_data.current_band);
...
SIDE EFFECTS
                  None
===========================================================================*/
static boolean rrccsp_should_freq_be_scanned_without_skipping(sys_modem_as_id_e_type wrrc_as_id,rrc_freq_type freq)
{
   boolean ret_status = FALSE;
   uint16 i;
   uint16 frequency;
   ...
   {
     if(frequency == rrccsp_derive_freq_from_uarfcn(wrrc_as_id,rrc_csp_curr_select_data.barred_freq[i],rrc_csp_curr_select_data.band_corr_barred_freq[i]))
     return TRUE;
   }
...
SIDE EFFECTS
                  None
===========================================================================*/
static boolean rrccsp_validate_ctr_freq(sys_modem_as_id_e_type wrrc_as_id,uint16 frequency, rrc_csp_band_class_type curr_band)
{
...
SIDE EFFECTS
                  None
===========================================================================*/
static boolean rrccsp_validate_ctr_freq_ul(sys_modem_as_id_e_type wrrc_as_id,uint16 frequency, rrc_csp_band_class_type curr_band)
{
...
SIDE EFFECTS
                  None
===========================================================================*/
boolean rrccsp_get_uarfcn (sys_modem_as_id_e_type wrrc_as_id,rrc_freq_type barred_uarfcn, rrc_csp_band_class_type barred_band, rrc_csp_band_class_type current_band, rrc_freq_type *uarfcn)
{
   ...
   fdl_offset = 0;
   absol_freq = rrccsp_derive_freq_from_uarfcn(wrrc_as_id,barred_uarfcn,barred_band);
   ...
   *uarfcn = ((absol_freq - fdl_offset) / 2);
   ...
   return (rrccsp_validate_ctr_freq(wrrc_as_id,absol_freq, current_band));
...
SIDE EFFECTS
                  None
===========================================================================*/
boolean rrccsp_get_uarfcn_ul(sys_modem_as_id_e_type wrrc_as_id,rrc_freq_type barred_uarfcn, rrc_csp_band_class_type barred_band, rrc_csp_band_class_type current_band, rrc_freq_type *uarfcn)
{
   ...
   ful_offset = 0;
   absol_freq = rrccsp_derive_freq_from_uarfcn_ul(wrrc_as_id,barred_uarfcn,barred_band);
   ...
   *uarfcn = ((absol_freq - ful_offset) / 2);
   ...
   return (rrccsp_validate_ctr_freq_ul(wrrc_as_id,absol_freq, current_band));
  
}
/*===========================================================================

FUNCTION          RRCCSP_SHOULD_OVERLAP_IRAT_FREQ_BE_SKIPPED
 
DESCRIPTION       This function determines based on IRAT acquired frequencies,
                  whether a new frequency should be scanned or skipped.
...
SIDE EFFECTS
                  None
===========================================================================*/
boolean rrccsp_should_overlap_irat_freq_be_skipped(sys_modem_as_id_e_type wrrc_as_id,rrc_freq_type freq, uint16 resolution)
{
  boolean ret_status = FALSE;
     ...
     (rrc_csp_int_data.curr_scan == RRC_CSP_PLMN_LIST_SCAN))
  {
    ...
    frequency = rrccsp_derive_freq_from_uarfcn(wrrc_as_id,freq,curr_band);
    ...
    )
  {
    ...
    for(i=0; i<rrc_csp_int_data.umts_chls_from_gsm.size; i++)
    {
        ...
        )
      {
        ...
        {
          WRRC_MSG3_HIGH("Freq %d skipped, GSM prev_scanned %d, resolution %d",
                          freq, rrc_csp_int_data.umts_chls_from_gsm.cells[i].num,RRCCSP_GSM_CHANNEL_RAW_SCAN_SKIP_RANGE);
...
} /* rrccsp_should_overlap_irat_freq_be_skipped */
/*===========================================================================
...
boolean rrccsp_should_freq_be_skipped(sys_modem_as_id_e_type wrrc_as_id,rrc_freq_type freq, uint16 resolution)
{
  ...
  {
    if(rrc_ccm_get_curr_camped_freq(wrrc_as_id) == freq)
    {
      ret_status = TRUE;
      /*Derive current band. It might have got set to different band if serving frequency is 
        not first entry in ACQ DB*/
      rrc_csp_curr_select_data.current_band = rrc_get_frequency_band(wrrc_as_id,freq, rrc_ccm_get_curr_camped_plmn(wrrc_as_id));
      rrc_csp_add_freq_to_available_freq_list(wrrc_as_id,freq);
  ...
  if((rrc_csp_int_data.service_search_bgnd == SYS_SERVICE_SEARCH_CSG)&&
     ((rrc_csp_int_data.curr_scan == RRC_CSP_GTOW_BPLMN_MANUAL_SCAN)||
      (rrc_csp_int_data.curr_scan == RRC_CSP_WTOW_BPLMN_MANUAL_SCAN)||
      (rrc_csp_int_data.curr_scan == RRC_CSP_PLMN_LIST_SCAN))&&
      (rrcnv_data.wcdma_csg_srch_carrier_space_nv == TRUE))
  {
    if(resolution == RRCCSP_RAW_SCAN_SKIP_RANGE)
    {
      resolution = RRCCSP_RAW_SCAN_SKIP_CSG_RANGE;
  ...
  if((rrc_get_state(wrrc_as_id) == RRC_STATE_DISCONNECTED) 
     ...
     && (rrc_csp_int_data.lcf_in_idle_mode_skip_status == RRC_CSP_DO_NOT_SKIP_FREQ) &&
  ...
  for(i=0; i<rrc_csp_curr_select_data.num_available_freq; i++)
  {
       ...
       (freq <= rrc_csp_curr_select_data.available_freq_list[i] + resolution))
    {
      ...
      {
        WRRC_MSG3_HIGH("Raw Freq %d skipped, prev_scanned %d, resolution %d", 
                   freq, rrc_csp_curr_select_data.available_freq_list[i],resolution);
      }
      else
      {
   ...
   if((rrc_csp_int_data.curr_scan == RRC_CSP_CELL_SELECTION_SCAN) &&
      ...
      (rrc_csp_curr_select_data.scan_info.type_freq_scan == RRC_CSP_ACQ_SCAN) && 
      (rrccsp_should_freq_be_scanned_without_skipping(wrrc_as_id,freq))) 
   {
       ...
       (rrc_csp_curr_select_data.scanned_band_mask))
   {
     ...
     if(rrccsp_should_overlapping_freq_be_skipped(wrrc_as_id,freq))
     {
       return(ret_status = TRUE);
     }
   }

  ret_status = rrccsp_should_overlap_irat_freq_be_skipped(wrrc_as_id,freq,resolution);
...
SIDE EFFECTS
                  None
===========================================================================*/
static boolean rrccsp_is_fine_scan_required(sys_modem_as_id_e_type wrrc_as_id,rrc_freq_type* ret_freq)
{
  ...
  while(rrccsp_should_freq_be_skipped(wrrc_as_id,*ret_freq, RRCCSP_RAW_SCAN_SKIP_RANGE))
  {
...
SIDE EFFECTS
                  None
===========================================================================*/
static boolean rrccsp_is_acq_scan_required(sys_modem_as_id_e_type wrrc_as_id,rrc_freq_type* ret_freq)
{
  boolean acq_scan_required = TRUE;

  while(rrccsp_should_freq_be_skipped(wrrc_as_id,*ret_freq, RRCCSP_FINE_SCAN_SKIP_RANGE))
  {
    ...
    else if((rrc_csp_int_data.mplmn_wcdma_freq_list_present) && (rrc_csp_int_data.mplmn_freq_list_index < rrc_csp_int_data.freq_scan_list.total_wcdma_freq))
    {
      RRC_CSP_PICK_FREQ_FOR_ACQ();
    }
    else
    {
...
void rrc_csp_init_addl_chnl_scan_data(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  {
    WRRC_MSG1_MED("Addl chl scan needed for band %d", rrc_csp_curr_select_data.current_band);
    rrc_csp_curr_select_data.addl_chls_scan_needed = TRUE;
  }
  else
  {
    WRRC_MSG1_MED("Addl chl scan NOT needed for band %d",rrc_csp_curr_select_data.current_band);
    rrc_csp_curr_select_data.addl_chls_scan_needed = FALSE;
  }
  ...
  switch(rrc_csp_curr_select_data.scan_info.type_freq_scan)
  {
    case RRC_CSP_NO_SCAN:
    {
      ...
      else
      {
        ...
        {
          rrc_csp_get_addl_channel_band(wrrc_as_id,ret_freq);
        }
        else
        {
          ...
          *ret_freq = WCDMA_FS_RX_CHAN_MIN;
          ...
          if(rrc_csp_int_data.lcf_in_idle_mode_skip_status != RRC_CSP_SKIP_FREQ_FPLMN 
    ...
    case RRC_CSP_RAW_SCAN:
    {
      ...
      else  /* Raw scan got some frequencies */
      {
        ...
        while(rrccsp_should_overlap_irat_freq_be_skipped(wrrc_as_id,*ret_freq, RRCCSP_RAW_SCAN_SKIP_RANGE))
        {
          if((rrc_csp_curr_select_data.scan_info.raw_scan.curr_freq_index <
              rrc_csp_curr_select_data.scan_info.raw_scan.num_freq -1))
          {
          ...
          {
            WRRC_MSG0_HIGH("All raw scan results are skipped. No more frequency");
            return(FAILURE);
          }
        }
    ...
    case RRC_CSP_FINE_SCAN:
    {
      ...
      if(rrc_csp_curr_select_data.scan_info.fine_scan.num_freq > 0)
      {
        ...
        if(rrccsp_is_acq_scan_required(wrrc_as_id,ret_freq))
        {
          WRRC_MSG1_MED("Starting acq on UARFCN %d", *ret_freq);
          return_value = SUCCESS;
        }
        else
        {
          if(rrccsp_is_fine_scan_required(wrrc_as_id,ret_freq))
          {
            WRRC_MSG1_HIGH("Starting fine scan on UARFCN %d", *ret_freq);
            return_value = SUCCESS;
          }
    ...
    case RRC_CSP_ACQ_SCAN:
    {
      ...
      else if((rrc_csp_int_data.mplmn_wcdma_freq_list_present) && 
              (rrc_csp_int_data.mplmn_freq_list_index < rrc_csp_int_data.freq_scan_list.total_wcdma_freq))
      {
        RRC_CSP_PICK_FREQ_FOR_ACQ();
        if(rrccsp_is_acq_scan_required(wrrc_as_id,ret_freq))
        {
...
rrc_csp_update_freq_scan_entry(sys_modem_as_id_e_type wrrc_as_id,l1_freq_scan_cnf_type *freq_scan_cnf)
{
     ...
     /*lint +e506*/
  {
        ...
        && (rrc_csp_int_data.curr_scan != RRC_CSP_WTOW_BPLMN_MANUAL_SCAN))
    {
      if(  (rrc_csp_int_data.last_camped_freq_in_connected_mode  != 0)
        && (SUCCESS == validate_dl_arfcn(wrrc_as_id,rrc_csp_int_data.last_camped_freq_in_connected_mode, 
      ...
      {
        WRRC_MSG3_HIGH("Add freq %d, rssi -146 to fine scan list. Curr_scan %d, RRC state %d",
     ...
     * NOT due to Inter Frequency Redirection */
            ...
            && (SUCCESS == validate_dl_arfcn(wrrc_as_id,rrc_csp_int_data.last_camped_freq_in_idle_mode, 
                                             rrc_csp_curr_select_data.req_plmn.plmn_id,
                                             W_UARFCN_NOT_FROM_GSM_NBR_LIST
                                             , FALSE
                                            )
    ...
    {
      WRRC_MSG3_HIGH("Idle mode: Add freq %d, rssi -146 to fine scan list. Curr_scan %d, RRC state %d",
   ...
   if((rrc_csp_int_data.curr_scan == RRC_CSP_CELL_SELECTION_SCAN) && (rrc_csp_curr_select_data.num_of_barred_freq != 0) &&
        ...
        )
    {
      ...
      for(i=0; i < rrc_csp_curr_select_data.num_of_barred_freq; i++)
      {
        ...
        {
          if(rrccsp_get_uarfcn(wrrc_as_id,rrc_csp_curr_select_data.barred_freq[i], rrc_csp_curr_select_data.band_corr_barred_freq[i], 
     ...
     (freq_scan_cnf->step == L1_FREQ_LIST_SCAN))
  {
    ...
    */
    ...
    {
      WRRC_MSG6_HIGH("LFS_COOS: Freq = %d, From L1 Freq = %d, PLMN-ID (%d - %d), PLMN-TYPE (%d), RSSI (%d)", 
                      ...
                      rrc_csp_curr_select_data.scan_info.list_scan.freq_result[i].rssi);
    }
    return(SUCCESS);
  }
...
)
{
  ...
  uint32 idx=0;
  rrc_state_e_type rrc_state = rrc_get_state(wrrc_as_id);
  rrc_plmn_identity_type plmn_id = rrc_ccm_get_curr_camped_plmn(wrrc_as_id);
  rrc_int_cell_id_type cell_id = rrc_ccm_get_curr_camped_cell_id(wrrc_as_id);
  ...
  (void)rrc_ccm_get_curr_camped_common_plmn(wrrc_as_id,&plmn_id);

  if(rrc_ccm_get_curr_camping_status(wrrc_as_id, RRC_PROCEDURE_CSP) == RRC_NOT_CAMPED_ON)
  {
  ...
  if( sib11_ptr == NULL )
  {
    ...
    WRRC_MSG3_MED("WBP: ERR: SIB11 for MCC %d, MNC %d, Cell ID %d not present",
  ...
  if((rrc_state != RRC_STATE_DISCONNECTED) && (rrc_state != RRC_STATE_CONNECTING))
  {
    if(sib11_ptr->sib12indicator == TRUE)
    {
      ...
      {
        /* SIB12 not present */
        WRRC_MSG0_MED("WBP: SIB12 indicated but not present, using SIB11");
  ...
  WRRC_MSG2_MED("WBP: is_hcs_used: %d, is_rscp_used: %d",is_hcs_used,is_rscp_used);

  for(i = 0; ((i < num_of_inter_freqs) && (num_of_bands_already_present < MAX_NUM_OF_WCDMA_BANDS)); i++)
  {
    rrc_csp_band_class_type nbr_freq_band;
    nbr_freq_band = rrc_get_frequency_band(wrrc_as_id,inter_freq_list[i],
                                              plmn_id);
    if((RRC_CSP_BAND_MAX != nbr_freq_band) && 
       (rrccsp_is_supported_band(wrrc_as_id,nbr_freq_band)))
    {
      if(!rrccsp_is_band_already_added_to_prioritized_list(wrrc_as_id,nbr_freq_band, num_of_bands_already_present))
      {
...
rrccsp_prioritize_wcdma_bands_for_full_scan(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  {
    WRRC_MSG3_HIGH("WBP: Prioritizing the WCDMA bands used for full frequency scan, curr_scan is: %d, LCF in idle and connected modes %d, %d",
                    rrc_csp_int_data.curr_scan,rrc_csp_int_data.last_camped_freq_in_idle_mode,rrc_csp_int_data.last_camped_freq_in_connected_mode);
  }
  else
  {
    WRRC_MSG1_MED("WBP: Not Prioritizing the WCDMA bands , curr_scan is: %d.",rrc_csp_int_data.curr_scan);
    return;
  }

  if(rrcnv_data.rrc_prioritize_850_band_nv && rrccsp_is_supported_band(wrrc_as_id,RRC_CSP_WCDMA_850_BAND))
  {
  ...
  */
  ...
  last_camped_band = rrc_get_frequency_band(wrrc_as_id,last_camped_freq,
                                            rrc_ccm_get_curr_camped_plmn(wrrc_as_id));

  if((RRC_CSP_BAND_MAX != last_camped_band) &&
     (rrccsp_is_supported_band(wrrc_as_id,last_camped_band)) &&
      !rrccsp_is_band_already_added_to_prioritized_list(wrrc_as_id,last_camped_band, rrc_csp_curr_select_data.num_of_prioritized_bands))
  {
  ...
  */
  rrc_csp_curr_select_data.num_of_prioritized_bands = rrccsp_add_nbr_freq_bands_from_sib11_to_band_list(wrrc_as_id,
  ...
  */
  rrc_csp_curr_select_data.num_of_prioritized_bands = rrccsp_add_freq_bands_from_acq_db_to_band_list(wrrc_as_id,
                                                            rrc_csp_curr_select_data.prioritized_bands,
                                                            rrc_csp_curr_select_data.num_of_prioritized_bands);
}
