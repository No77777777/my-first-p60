...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcqm.c#2 $


when       who     what, where, why
--------   ---     ----------------------------------------------------------
11/26/14   vi      Made changes to free msg_ptr if no transport channels and no additional measured results are present in quality measurement report.
...
quality_meas_id_list_struct_type quality_meas_id_list_sim[MAX_RRC_STACKS][MAX_QUALITY_MEAS];
#define quality_meas_id_list quality_meas_id_list_sim[WAS_ID_M]
#else
...
)
{
  ...
  case T_rrc_MeasurementCommand_setup:
    ...
    if(FAILURE == rrcqm_fill_mcm_info(wrrc_as_id,rrc_mcm_ptr->measurementCommand.u.setup->u.qualityMeasurement,
    ...
    case T_rrc_MeasurementCommand_modify:
        ...
        rrc_MeasurementCommand_modify,measurementType))
      {
        ...
        if(FAILURE == rrcqm_fill_mcm_info(wrrc_as_id,rrc_mcm_ptr->measurementCommand.u.modify->
    ...
    case T_rrc_MeasurementCommand_release:
      ...
      rrcmeas_send_l1_cmd(wrrc_as_id,NULL, l1_qual_meas_params, NULL, L1_MEAS_CTRL_PARMS);
      rrc_free(l1_qual_meas_params);
      return SUCCESS;    
...
void rrcqm_initialize_meas_id_list(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8 count, cnt;
...
)
{
  ...
  * should be released by SEND_CHAIN using OSS compiler functions */
  ...
  rrcmeas_fill_meas_id(wrrc_as_id,&msg_ptr->message.u.measurementReport,l1_meas_report_ptr->meas_id, QUALITY_MEAS);
...
DESCRIPTION
  This function processes the Measurement Control Message for
  Quality Measurements for rel 10
...
)
{
    ...
    (rrc_mcm_ptr->dpch_CompressedModeStatusInfo.tgp_SequenceShortList.n != 0))
  {
    ...
    if (rrcmeas_process_compressed_mode_status_r10(wrrc_as_id,&rrc_mcm_ptr->dpch_CompressedModeStatusInfo,
      &l1_qual_meas_params->cm_status_info) == FAILURE)
    {
      WRRC_MSG0_HIGH("REL10:QUALITY:Invalid CompMode Status Info %d");
      /* Send Meas Ctrl Failure Message */
  ...
  switch(rrc_mcm_ptr->measurementCommand.t)
  {
  case T_rrc_MeasurementCommand_r10_setup:
    
    /* check if the quality reporting quantity is present if not return failure */
    if(!(RRC_MSG_COMMON_BITMASK_IE_TYPE2_PTR(rrc_mcm_ptr->measurementCommand.u.setup->u.qualityMeasurement,
      qualityReportingQuantity)))
    {
      WRRC_MSG0_ERROR("REL10:SETUP:Quality Rpt Quan not present");
      /* Set faliure t */
      rrcmeas_mcf.error_choice = T_rrc_FailureCauseWithProtErr_configurationIncomplete;
      rrcmeas_mcf.rrc_transaction_id = transaction_id;
      rrcmeas_send_meas_ctrl_failure_msg(wrrc_as_id,&rrcmeas_mcf);
    ...
    if(l1_qual_meas_params->rpt_mode_incl == FALSE)
    {
      WRRC_MSG0_ERROR("REL10:SETUP:meas Rpt mode mandatory");
      /* Set faliure t */
      rrcmeas_mcf.error_choice = T_rrc_FailureCauseWithProtErr_configurationIncomplete;
    ...
    if((RRC_CHECK_COMMON_MSG_TYPE2(rrc_mcm_ptr->measurementCommand.u.setup->u.qualityMeasurement->reportCriteria,
      ...
      (rrc_mcm_ptr->measurementCommand.u.setup->u.qualityMeasurement->qualityReportingQuantity.dl_TransChBLER == TRUE))
    {
      WRRC_MSG0_ERROR("REL10:SETUP:BLER rpt set to true for evnt trigg meas");
      /* Set faliure t */
      rrcmeas_mcf.error_choice = T_rrc_FailureCauseWithProtErr_invalidConfiguration;
      rrcmeas_mcf.rrc_transaction_id = transaction_id;
      rrcmeas_send_meas_ctrl_failure_msg(wrrc_as_id,&rrcmeas_mcf);
    ...
    if(RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand.u.setup->u.qualityMeasurement->qualityReportingQuantity.modeSpecificInfo,
       rrc_QualityReportingQuantity_modeSpecificInfo_tdd))
    {
      WRRC_MSG0_ERROR("REL10:SETUP:qual rept quantity for tdd chosen");
      /* Set faliure t */
      rrcmeas_mcf.error_choice = T_rrc_FailureCauseWithProtErr_configurationUnsupported;
      rrcmeas_mcf.rrc_transaction_id = transaction_id;
      rrcmeas_send_meas_ctrl_failure_msg(wrrc_as_id,&rrcmeas_mcf);
    ...
    case T_rrc_MeasurementCommand_r10_modify:
        ...
        & rrc_MeasurementCommand_r10_modify,measurementType))
      {
          ...
          qualityReportingQuantity))
        {
          ...
          if((RRC_CHECK_COMMON_MSG_TYPE2(rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.qualityMeasurement->reportCriteria,
            ...
            dl_TransChBLER == TRUE))
          {
            WRRC_MSG0_ERROR("REL10:Modify:BLER rpt set to true for evnt trigg meas");
            /* Set faliure t */
            rrcmeas_mcf.rrc_transaction_id = transaction_id;
            rrcmeas_mcf.error_choice = T_rrc_FailureCauseWithProtErr_invalidConfiguration;
            rrcmeas_send_meas_ctrl_failure_msg(wrrc_as_id,&rrcmeas_mcf);
          ...
          if(RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.qualityMeasurement->
            qualityReportingQuantity.modeSpecificInfo,
             rrc_QualityReportingQuantity_modeSpecificInfo_tdd))
          {
            WRRC_MSG0_ERROR("REL10:Modify:qual rept quantity for tdd chosen");
            /* Set faliure t */
            rrcmeas_mcf.error_choice = T_rrc_FailureCauseWithProtErr_configurationUnsupported;
            rrcmeas_mcf.rrc_transaction_id = transaction_id;
            rrcmeas_send_meas_ctrl_failure_msg(wrrc_as_id,&rrcmeas_mcf);
      ...
      {
        WRRC_MSG0_HIGH("REL10:Quality:Modify:MType absent");
      }
      /* validate the additional measurements*/
      if(rrcmeas_process_mcm_addnl_meas_r10(wrrc_as_id,mcm_r10,transaction_id,&addtl_meas_incl,primary_meas_type,meas_id, &meas_validity) == FAILURE)
      {
    ...
    case T_rrc_MeasurementCommand_r10_release:
      ...
      for (count = 0; count < MAX_QUALITY_MEAS; count++ )
      {
        if (quality_meas_id_list[count].meas_id
          == meas_id)
        {
          WRRC_MSG1_HIGH("REL10:Delete Meas %d",meas_id);
          quality_meas_id_list[count].meas_id = INVALID_MEAS_ID;
