...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcasu.c#2 $    $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/22/17   sp      Mini dump phase-2 change
02/21/18   sp      Made changes to ignore OTA with duplicate trid
/21/18   as      Added code to support LTA durting activation time
05/26/16   sg      Made changes to reduce F3s
04/21/16   nr      Fixing P2 level KW errors
01/13/16   nr      Made changes to report failure events to QSH
...
LOCAL rrcasu_int_data_type rrcasu_sim[MAX_RRC_STACKS];
#define rrcasu rrcasu_sim[WAS_ID_M]
#else
LOCAL rrcasu_int_data_type rrcasu;
#endif
...
)
{
  ...
  else
  {
    ...
    if(ptr !=NULL)
    {
      rrcasn1_free_pdu(wrrc_as_id,rrc_UL_DCCH_Message_PDU,ptr);
    }
  ...
  /*Reports ASU failure to QSH*/
  rrc_qsh_report_failure_ota_msg_event(wrrc_as_id,WRRC_UL_DCCH_MESSAGETYPE_ACTIVESETUPDATEFAILURE);
#endif
  ...
  rrcqsh_log_uplink_dcch_ota_params(wrrc_as_id, WRRC_UL_DCCH_MESSAGETYPE_ACTIVESETUPDATEFAILURE, ptr);
#endif
  ...
  if(ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
    ...
    /* ptr is freed in rrcsend_send_ul_sdu */
    status = rrcsend_send_ul_sdu(wrrc_as_id, RRC_PROCEDURE_ASU,  /* Procedure is RBE    */
...
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
  rrcqsh_log_uplink_dcch_ota_params(wrrc_as_id, WRRC_UL_DCCH_MESSAGETYPE_ACTIVESETUPDATECOMPLETE, ptr);
#endif
  /* If the logical channel id is not invalid, send the message to the
  send chain module */
  if(ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
...
===========================================================================*/
static void rrcasu_clear_vars(sys_modem_as_id_e_type wrrc_as_id )
{
  uint32 count;
...
===========================================================================*/
static void rrcasu_send_channel_config_req ( sys_modem_as_id_e_type wrrc_as_id )
{
...
)
{
  ...
  validate_asu_message(wrrc_as_id, trindex);
...
/*lint -e534 it is no need to check the returned value */
    rrc_ccm_send_nas_cn_info_r6(wrrc_as_id, &(asu_r6_ptr->cn_InformationInfo));
/*lint +e534 */
  }
  ...
  else
  {
    ...
    oc_status = rrcllc_set_ordered_config(wrrc_as_id, RRC_PROCEDURE_ASU,
                                          RRC_STATE_CELL_DCH,
  ...
  switch( oc_status )
  {
    case ORDERED_CONFIG_SET:
      ...
      if(rrc_sib_is_current_event_sib_read_in_dch(wrrc_as_id) == TRUE)
      {
        ...
        uint32 stored_psc = 0;
        if(
           (rrcllc_is_hs_repointing_happening(wrrc_as_id) == TRUE) ||
           (FAILURE == rrcllc_get_bcch_psc(wrrc_as_id, &stored_psc))
          )
        {
      ...
      rrcasu_send_channel_config_req( wrrc_as_id );
    ...
    case ORDERED_CONFIG_INCOMPATABLE_SIMULTANEOUS_RECONFIG:
      ...
      send_asu_failure_message( wrrc_as_id, trindex );
    ...
    case ORDERED_CONFIG_CONFIGURATION_SET_DEFERRED:
      WRRC_MSG0_HIGH("BUF_OTA:ASU processing deferred due to LTA!");
      rrcasu.tr[trindex].curr_substate = RRCASU_INITIAL;
      if(msg_ptr != NULL)
      {
        rrc_buffer_ota_info.transaction_id = msg_ptr->message.u.activeSetUpdate.u.later_than_r3.rrc_TransactionIdentifier;
      }
      break;
...
)
{
  ...
  if(RRC_MSG_COMMON_BITMASK_IE_PTR(asu_r7_ptr,rrc_ActiveSetUpdate_r7_IEs,cn_InformationInfo))
  {
    MSG_LOW("CN info present in ASU msg", 0, 0, 0);
/*lint -e534 it is no need to check the returned value */
    rrc_ccm_send_nas_cn_info_r6(wrrc_as_id, &(asu_r7_ptr->cn_InformationInfo));
/*lint +e534 */
  }
...
)
{
     ...
     cn_InformationInfo))
  {
    MSG_LOW("CN info present in ASU msg", 0, 0, 0);
/*lint -e534 it is no need to check the returned value */
    rrc_ccm_send_nas_cn_info_r6(wrrc_as_id, &(asu_r8_ptr->cn_InformationInfo));
/*lint +e534 */
  }
...
)
{
    ...
    cn_InformationInfo))
  {
    MSG_LOW("CN info present in ASU msg", 0, 0, 0);
/*lint -e534 it is no need to check the returned value */
    rrc_ccm_send_nas_cn_info_r6(wrrc_as_id, &(asu_r9_ptr->cn_InformationInfo));
/*lint +e534 */
  }
...
/*===========================================================================
FUNCTION   process_asu_message_r10
...
SIDE EFFECTS
  None.
===========================================================================*/
static void process_asu_message_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  rrc_DL_DCCH_Message *msg_ptr,    /* Pointer to the DCCH message      */ 
  uint32 trindex,                   /* Transaction index for this message   */
...
{
  rrcllc_oc_status_e_type oc_status;/* To hold ordered_config status          */
  rrc_ActiveSetUpdate_r10_IEs *asu_r10_ptr = 
    &asu_ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.activeSetUpdate_r10;

  if (asu_ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.m.activeSetUpdate_r10_add_extPresent)
  {
    WRRC_MSG0_ERROR("rrc_activeSetUpdate_r10_add_ext_present not supported");
  }

  validate_asu_message(wrrc_as_id, trindex);
   
  if(RRC_MSG_COMMON_BITMASK_IE_PTR(asu_r10_ptr,rrc_ActiveSetUpdate_r10_IEs,
    cn_InformationInfo))
  {
    MSG_LOW("CN info present in ASU msg", 0, 0, 0);
/*lint -e534 it is no need to check the returned value */
    rrc_ccm_send_nas_cn_info_r6(wrrc_as_id, &(asu_r10_ptr->cn_InformationInfo));
/*lint +e534 */
  }
  ...
  switch( oc_status )
  {
    case ORDERED_CONFIG_SET:
      ...
      if(rrc_sib_is_current_event_sib_read_in_dch(wrrc_as_id) == TRUE)
      {
        ...
        else if(RRC_MSG_COMMON_BITMASK_IE_PTR(asu_r10_ptr,
              rrc_ActiveSetUpdate_r10_IEs,rl_RemovalInformationList))
        {
          rrc_RL_RemovalInformationList * rrc_local_rl_RemovalInformationList;
          uint32 idx =0;
          rrc_local_rl_RemovalInformationList = &asu_r10_ptr->rl_RemovalInformationList;
          while(rrc_local_rl_RemovalInformationList->n > idx )
          {
            if(rrc_local_rl_RemovalInformationList->elem[idx].primaryScramblingCode == stored_psc)
            {
...
)
{
...
#ifdef FEATURE_WCDMA_ALLOW_LTA_DURING_ACT_TIME
  if((rrcdata_check_if_ota_is_buffered(wrrc_as_id) == TRUE)
     &&
     (rrc_buffer_ota_info.procedure == RRC_PROCEDURE_ASU)
     &&
     (rrc_buffer_ota_info.transaction_id != RRCRB_INVALID_TRANSACTION_ID)
     &&
     (trindex == rrc_buffer_ota_info.transaction_id))
  {
    WRRC_MSG0_HIGH("BUF_OTA:Duplicate ASU message, ignoring");
    return;
  }
#endif
  ...
  if (T_rrc_ActiveSetUpdate_later_than_r3 == ptr->t)
  {
    ...
    {
      process_asu_message_r6(wrrc_as_id, msg_ptr, trindex, ptr);
    }
    else
    ...
    {
      process_asu_message_r7(wrrc_as_id, msg_ptr, trindex, ptr);
    }
    else
    ...
    {
      process_asu_message_r8(wrrc_as_id, msg_ptr, trindex, ptr);
    }
    else
#ifdef FEATURE_WCDMA_REL9
    ...
    {
      process_asu_message_r9(wrrc_as_id, msg_ptr, trindex, ptr);
    }
    else
#endif /* FEATURE_WCDMA_REL9 */
#ifdef FEATURE_WCDMA_REL10
    if (T_rrc_ActiveSetUpdate_later_r9_criticalExtensions_r10 == 
       ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.t)
    {
      process_asu_message_r10(wrrc_as_id, msg_ptr, trindex, ptr);
    }
    else
#endif /* FEATURE_WCDMA_REL10 */
       ...
       rrc_ActiveSetUpdate_r3_IEs,cn_InformationInfo))
  {
    ...
    /* Send CN Information to NAS */
       ...
       */
       primary_plmn_Id_present = rrcasu_check_and_get_primary_plmn_id(wrrc_as_id,ptr,
                                                                      &primary_plmn_Identity,
                                                                      MSG_REL99);
/*lint -e534 it is no need to check the returned value */
    rrc_ccm_send_nas_cn_info(wrrc_as_id,
...
/*lint +e534 */
  ...
  if(rrcdata_misc_info.rrc_msg_failure_cause == RRC_MSG_INVALID_CONFIG)
  { 
    rrcdata_misc_info.rrc_msg_failure_cause = RRC_MSG_VALID;
  ...
  switch( oc_status )
  {
    ...
    case ORDERED_CONFIG_CONFIGURATION_SET_DEFERRED:
      ...
      if(msg_ptr != NULL)
      {
        rrc_buffer_ota_info.transaction_id = msg_ptr->message.u.activeSetUpdate.u.r3.activeSetUpdate_r3.rrc_TransactionIdentifier;
      }
      break;
#endif  /*FEATURE_WCDMA_ALLOW_LTA_DURING_ACT_TIME*/
    default:
...
)
{
  ...
  if( ptr->chan_cfg_status == RRCLLC_CHAN_CFG_SUCCESS )
  {
    ...
    send_asu_complete_message(wrrc_as_id, trindex );
  ...
  if(!rrcllc_reconfig_in_progress(wrrc_as_id))
  {
        ...
        ||(ptr->chan_cfg_status == RRCLLC_CHAN_CFG_FAILURE_OTHER))
    {
      ...
      configurations */
      (void)rrcllc_reset_ordered_config(wrrc_as_id, RRC_PROCEDURE_ASU,TRUE);
    }
    else
    {
...
===========================================================================*/
static void rrcasu_process_state_change_ind ( sys_modem_as_id_e_type wrrc_as_id )
{
...
)
{
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    case RRC_DOWNLINK_SDU_IND:
      ...
      process_asu_message( wrrc_as_id, dcch_msg_ptr, trindex );
      break;
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    case RRC_CHANNEL_CONFIG_CNF:
      ...
      rrcasu_process_channel_config_cnf
        (wrrc_as_id, &(cmd_ptr->cmd.chan_config_cnf), trindex ); 
      break;
    ...
    case RRC_STATE_CHANGE_IND:
      ...
      rrcasu_process_state_change_ind( wrrc_as_id );
      break;      
    ...
    case RRC_DOWNLINK_SDU_IND:
      ...
      if (dcch_msg_ptr != NULL) 
      {
                 ...
                 dcch_msg_ptr->message.u.activeSetUpdate.t)
        {
          ...
          if (T_rrc_ActiveSetUpdate_later_r9_criticalExtensions_r10== 
             dcch_msg_ptr->message.u.activeSetUpdate.u.later_than_r3.criticalExtensions.u.criticalExtensions.u.
             criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.t)
          {
            index = dcch_msg_ptr->message.u.activeSetUpdate.u.
               later_than_r3.rrc_TransactionIdentifier;
          
            WRRC_MSG1_HIGH("RRCEUL: Rel-10 ASU received, transaction id: %d", index);
...
)
{
  ...
  if((rrccu_get_cell_update_started_status(wrrc_as_id) == RRCCU_STARTED) 
#ifdef FEATURE_DUAL_SIM
      || (rrc_dsds_int_data.DS_Immediate_preempt_waiting_for_init_cell_sel == TRUE)
#endif
    )
  {
  ...
  index = find_transaction_for_command(wrrc_as_id, cmd_ptr );
  ...
  if( index < RRCASU_MAX_ASU_TRANSACTIONS )
  {
    ...
    rrc_crash_debug_save_proc_substates(wrrc_as_id, RRC_PROCEDURE_ASU,rrcasu.tr[index].curr_substate,cmd_ptr->cmd_hdr.cmd_id);
    switch( rrcasu.tr[index].curr_substate )
    {
      case RRCASU_INITIAL:
        /* Call the event handler for Initial substate */
        rrcasu_initial_substate_event_handler(wrrc_as_id, cmd_ptr, index );
        break;

      case RRCASU_WAIT_FOR_CHAN_CFG_CNF:
        /* Call the event handler for waiting for chan cfg cnf substate */
        rrcasu_wfchcnf_substate_event_handler(wrrc_as_id, cmd_ptr, index );
        break;
  ...
  else
  {
    ...
    switch ( cmd_ptr->cmd_hdr.cmd_id )
    {
      case RRC_DOWNLINK_SDU_IND:
        ...
        message. */
        send_asu_failure_message(wrrc_as_id, index );
        break;
...
{
  rrcasu_clear_vars(wrrc_as_id); /* Clear all the procedure variables */
  ...
  rrcscmgr_register_for_scn(wrrc_as_id, RRC_PROCEDURE_ASU,  /* Procedure name */
...
{
  qsh_mdump_collect_high(&rrcasu,sizeof(rrcasu));
}
#endif
