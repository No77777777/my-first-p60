...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrclcm.c#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/14/20   sp      Mini dump phase-2 changes
02/15/18   sp      Made changes to handle pending L2 acks
...
03/10/16   nr      Added check of NULL for context in function rrclcm_post_event_for_dl_sdu 
...
08/12/14   ad      Made changes for restructuring globals
...
typedef struct
{
  dsm_watermark_type rrclcm_rb32_bcch_dl_wmq;   
                                           /* WaterMark Queue used when SDU
                                              is received from RLC on RB 32 
                                              using BCCH Logical channel
                                              for serving cell. */
  q_type rrclcm_rb32_bcch_dl_q;        /* Queue around which 
                                              rrclcm_rb32_bcch_dl_wmq is 
                                              built. */
											  
  dsm_watermark_type rrclcm_rb33_bcch_dl_wmq;
                                           /* WaterMark Queue used when SDU
                                              is receiveed from RLC on RB 33
                                              using BCCH Logical Channel for 
                                              neighbor cell. */

  q_type rrclcm_rb33_bcch_dl_q;         /* Queue around which 
                                              rrclcm_rb33_bcch_dl_wmq is 
                                              built. */

  dsm_watermark_type rrclcm_rb34_bcch_fach_dl_wmq;
                                           /* WaterMark Queue used when SDU
  ...
  q_type rrclcm_rb34_bcch_fach_dl_q;   /* Queue around which 
                                              rrclcm_rb34_bcch_fach_dl_wmq is 
                                              built. */

  dsm_watermark_type rrclcm_rb35_pcch_dl_wmq;
                                           /* WaterMark Queue used when SDU
                                              is receiveed from RLC on RB 35
                                              using PCCH Logical Channel. */

  q_type rrclcm_rb35_pcch_dl_q;        /* Queue around which 
                                              rrclcm_rb35_pcch_dl_wmq is 
                                              built. */
  dsm_watermark_type rrclcm_rb0_ccch_dl_wmq;   
                                           /* WaterMark Queue used when SDU
                                              is received from RLC on RB 0 
                                              using CCCH Logical channel.*/
  q_type rrclcm_rb0_ccch_dl_q;         /* Queue around which 
                                              rrclcm_rb0_ccch_dl_wmq is 
                                              built. */

  dsm_watermark_type rrclcm_rb0_ccch_ul_wmq;
                                           /* WaterMark Queue used when SDU
                                              is transmitted to RLC on RB 0
                                              using CCCH Logical Channel. */

  q_type rrclcm_rb0_ccch_ul_q;         /* Queue around which 
                                              rrclcm_rb0_ccch_ul_wmq is 
                                              built. */
  dsm_watermark_type rrclcm_rb1_dcch_dl_wmq;   
                                           /* WaterMark Queue used when SDU
                                              is received from RLC on RB 1 
                                              using DCCH Logical channel. */
  q_type rrclcm_rb1_dcch_dl_q;         /* Queue around which 
                                              rrclcm_rb1_dcch_dl_wmq is 
                                              built. */

  dsm_watermark_type rrclcm_rb1_dcch_ul_wmq;
                                           /* WaterMark Queue used when SDU
                                              is transmitted to RLC on RB 1
                                              using DCCH Logical Channel. */

  q_type rrclcm_rb1_dcch_ul_q;         /* Queue around which 
                                              rrclcm_rb1_dcch_ul_wmq is 
                                              built. */
  dsm_watermark_type rrclcm_rb2_dcch_dl_wmq;   
                                           /* WaterMark Queue used when SDU
                                              is received from RLC on RB 2 
                                              using DCCH Logical channel. */
  q_type rrclcm_rb2_dcch_dl_q;         /* Queue around which 
                                              rrclcm_rb2_dcch_dl_wmq is 
                                              built. */

  dsm_watermark_type rrclcm_rb2_dcch_ul_wmq;
                                           /* WaterMark Queue used when SDU
                                              is transmitted to RLC on RB 2
                                              using DCCH Logical Channel. */

  q_type rrclcm_rb2_dcch_ul_q;         /* Queue around which 
                                              rrclcm_rb2_dcch_ul_wmq is 
                                              built. */
  dsm_watermark_type rrclcm_rb3_dcch_dl_wmq;   
                                           /* WaterMark Queue used when SDU
                                              is received from RLC on RB 3 
                                              using DCCH Logical channel. */
  q_type rrclcm_rb3_dcch_dl_q;         /* Queue around which 
                                              rrclcm_rb3_dcch_dl_wmq is 
                                              built. */

  dsm_watermark_type rrclcm_rb3_dcch_ul_wmq;
                                           /* WaterMark Queue used when SDU
                                              is transmitted to RLC on RB 3
                                              using DCCH Logical Channel. */

  q_type rrclcm_rb3_dcch_ul_q;         /* Queue around which 
                                              rrclcm_rb3_dcch_ul_wmq is 
                                              built. */
  dsm_watermark_type rrclcm_rb4_dcch_dl_wmq;   
                                           /* WaterMark Queue used when SDU
                                              is received from RLC on RB 4 
                                              using DCCH Logical channel. */
  q_type rrclcm_rb4_dcch_dl_q;         /* Queue around which 
                                              rrclcm_rb4_dcch_dl_wmq is 
                                              built. */

  dsm_watermark_type rrclcm_rb4_dcch_ul_wmq;
                                           /* WaterMark Queue used when SDU
                                              is transmitted to RLC on RB 4
                                              using DCCH Logical Channel. */

  q_type rrclcm_rb4_dcch_ul_q;         /* Queue around which 
                                              rrclcm_rb4_dcch_ul_wmq is 
                                              built. */

  dsm_watermark_type rrclcm_srb5_dcch_dl_wmq;   
                                           /* WaterMark Queue used when SDU
                                              is received from RLC on SRB#5 
                                              using DCCH_TM Logical channel
                                              for serving cell. */
  q_type rrclcm_srb5_dcch_dl_q;        /* Queue around which 
                                              rrclcm_srb5_dcch_dl_wmq is 
                                              built. */

  rrclcm_l2_ack_info_type   rrclcm_l2_ack_info[MAX_NUM_OF_PROCEDURES][UE_MAX_UL_AM_DATA_CHANNEL - 1];
                                           
  rrclcm_ul_rlc_lc_info_type rrclcm_ul_rlc_lc_info
                                  [UE_MAX_UL_LOGICAL_CHANNEL];

  rrclcm_dl_rlc_lc_info_type rrclcm_dl_rlc_lc_info
                                  [UE_MAX_DL_LOGICAL_CHANNEL];
}rrclcm_int_data_type;

#ifdef FEATURE_DUAL_WCDMA 
LOCAL rrclcm_int_data_type rrclcm_int_data_sim[MAX_RRC_STACKS];
LOCAL boolean rrclcm_init_current_mui_array_at_poweup_sim[MAX_RRC_STACKS] = {TRUE,TRUE};
#define rrclcm_int_data rrclcm_int_data_sim[WAS_ID_M]
#define rrclcm_init_current_mui_array_at_poweup rrclcm_init_current_mui_array_at_poweup_sim[WAS_ID_M]
#else
LOCAL rrclcm_int_data_type rrclcm_int_data;
LOCAL boolean rrclcm_init_current_mui_array_at_poweup = TRUE;
#endif
...
)
{
  ...
  /* Checks for Logical Channel Type and Radio Bearer Id match for Transparent
    ...
    case (UE_MODE_TRANSPARENT): 
      ...
      {
        if( dl_lc_type == rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_type && 
             rb_id == rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id &&
             rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].status == RRCLCM_LC_IN_USE)
        {
  ...
  if(dl_lc_id_found)
  {
    return (rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_id);
  }
  else
  {
...
)
{
  ...
  {
    WRRC_ERR1_FATAL("ul_lc_id out of bounds : 0x%x",ul_lc_id);
  }
  return rrclcm_int_data.rrclcm_ul_rlc_lc_info[ul_lc_id].rb_id ;
} /* rrclcm_get_rb_dl_id */
/***/ __WCDMA_RRC_CODE_SEGMENT_NON_DEMANDPAGED_END__ /***/
...
)
{
    /* Set up Queue for Transmitting the data to RLC on RB 0 using CCCH 
     Logical Channe. */
  (void) dsm_queue_init(&rrclcm_int_data.rrclcm_rb0_ccch_ul_wmq,8 * RRCLCM_WATERMARK_SIZE,&rrclcm_int_data.rrclcm_rb0_ccch_ul_q);
  rrclcm_int_data.rrclcm_rb0_ccch_ul_wmq.q_ptr = &rrclcm_int_data.rrclcm_rb0_ccch_ul_q;
  rrclcm_int_data.rrclcm_rb0_ccch_ul_wmq.dont_exceed_cnt = 8 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb0_ccch_ul_wmq.hi_watermark = 6 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb0_ccch_ul_wmq.lo_watermark = 2 * RRCLCM_WATERMARK_SIZE;

  /* Set up Queue for Transmitting the data to RLC on RB 1 using DCCH 
     Logical Channe. */
  (void) dsm_queue_init(&rrclcm_int_data.rrclcm_rb1_dcch_ul_wmq,8 * RRCLCM_WATERMARK_SIZE,&rrclcm_int_data.rrclcm_rb1_dcch_ul_q);
  rrclcm_int_data.rrclcm_rb1_dcch_ul_wmq.q_ptr = &rrclcm_int_data.rrclcm_rb1_dcch_ul_q;
  rrclcm_int_data.rrclcm_rb1_dcch_ul_wmq.dont_exceed_cnt = 8 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb1_dcch_ul_wmq.hi_watermark = 6 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb1_dcch_ul_wmq.lo_watermark = 2 * RRCLCM_WATERMARK_SIZE;

  /* Set up Queue for Transmitting the data to RLC on RB 2 using DCCH 
     Logical Channe. */
  (void) dsm_queue_init(&rrclcm_int_data.rrclcm_rb2_dcch_ul_wmq,8 * RRCLCM_WATERMARK_SIZE,&rrclcm_int_data.rrclcm_rb2_dcch_ul_q);
  rrclcm_int_data.rrclcm_rb2_dcch_ul_wmq.q_ptr = &rrclcm_int_data.rrclcm_rb2_dcch_ul_q;
  rrclcm_int_data.rrclcm_rb2_dcch_ul_wmq.dont_exceed_cnt = 8 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb2_dcch_ul_wmq.hi_watermark = 6 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb2_dcch_ul_wmq.lo_watermark = 2 * RRCLCM_WATERMARK_SIZE;

  /* Set up Queue for Transmitting the data to RLC on RB 3 using DCCH 
     Logical Channe. */
  (void) dsm_queue_init(&rrclcm_int_data.rrclcm_rb3_dcch_ul_wmq,8 * RRCLCM_WATERMARK_SIZE,&rrclcm_int_data.rrclcm_rb3_dcch_ul_q);
  rrclcm_int_data.rrclcm_rb3_dcch_ul_wmq.q_ptr = &rrclcm_int_data.rrclcm_rb3_dcch_ul_q;
  rrclcm_int_data.rrclcm_rb3_dcch_ul_wmq.dont_exceed_cnt = 8 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb3_dcch_ul_wmq.hi_watermark = 6 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb3_dcch_ul_wmq.lo_watermark = 2 * RRCLCM_WATERMARK_SIZE;

  /* Set up Queue for Transmitting the data to RLC on RB 4 using DCCH 
     Logical Channe. */
  (void) dsm_queue_init(&rrclcm_int_data.rrclcm_rb4_dcch_ul_wmq,8 * RRCLCM_WATERMARK_SIZE,&rrclcm_int_data.rrclcm_rb4_dcch_ul_q);
  rrclcm_int_data.rrclcm_rb4_dcch_ul_wmq.q_ptr = &rrclcm_int_data.rrclcm_rb4_dcch_ul_q;
  rrclcm_int_data.rrclcm_rb4_dcch_ul_wmq.dont_exceed_cnt = 8 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb4_dcch_ul_wmq.hi_watermark = 6 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb4_dcch_ul_wmq.lo_watermark = 2 * RRCLCM_WATERMARK_SIZE;

   /* Set up Queue for receiving the data from RLC on RB 32 using BCCH 
     Logical Channel for serving cell. */
  (void) dsm_queue_init(&rrclcm_int_data.rrclcm_rb32_bcch_dl_wmq,8 * RRCLCM_WATERMARK_SIZE,&rrclcm_int_data.rrclcm_rb32_bcch_dl_q);
  rrclcm_int_data.rrclcm_rb32_bcch_dl_wmq.q_ptr = &rrclcm_int_data.rrclcm_rb32_bcch_dl_q;
  rrclcm_int_data.rrclcm_rb32_bcch_dl_wmq.dont_exceed_cnt = 8 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb32_bcch_dl_wmq.hi_watermark = 6 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb32_bcch_dl_wmq.lo_watermark = 2 * RRCLCM_WATERMARK_SIZE;

  /* Set up Queue for receiving the data from RLC on RB 33 using BCCH 
     Logical Channel for neighbor cell. */
  (void) dsm_queue_init(&rrclcm_int_data.rrclcm_rb33_bcch_dl_wmq, 8 * RRCLCM_WATERMARK_SIZE,&rrclcm_int_data.rrclcm_rb33_bcch_dl_q);
  rrclcm_int_data.rrclcm_rb33_bcch_dl_wmq.q_ptr = &rrclcm_int_data.rrclcm_rb33_bcch_dl_q;
  rrclcm_int_data.rrclcm_rb33_bcch_dl_wmq.dont_exceed_cnt = 8 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb33_bcch_dl_wmq.hi_watermark = 6 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb33_bcch_dl_wmq.lo_watermark = 2 * RRCLCM_WATERMARK_SIZE;

  /* Set up Queue for receiving the data from RLC on RB 34 using BCCH 
     Logical Channel mapped to FACH transport channel. */
  (void) dsm_queue_init(&rrclcm_int_data.rrclcm_rb34_bcch_fach_dl_wmq,8 * RRCLCM_WATERMARK_SIZE,&rrclcm_int_data.rrclcm_rb34_bcch_fach_dl_q);
  rrclcm_int_data.rrclcm_rb34_bcch_fach_dl_wmq.q_ptr = &rrclcm_int_data.rrclcm_rb34_bcch_fach_dl_q;
  rrclcm_int_data.rrclcm_rb34_bcch_fach_dl_wmq.dont_exceed_cnt = 8 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb34_bcch_fach_dl_wmq.hi_watermark = 6 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb34_bcch_fach_dl_wmq.lo_watermark = 2 * RRCLCM_WATERMARK_SIZE;

   /* Set up Queue for receiving the data from RLC on RB 35 using PCCH 
     Logical Channel. */
  (void) dsm_queue_init(&rrclcm_int_data.rrclcm_rb35_pcch_dl_wmq,8 * RRCLCM_WATERMARK_SIZE,&rrclcm_int_data.rrclcm_rb35_pcch_dl_q);
  rrclcm_int_data.rrclcm_rb35_pcch_dl_wmq.q_ptr = &rrclcm_int_data.rrclcm_rb35_pcch_dl_q;
  rrclcm_int_data.rrclcm_rb35_pcch_dl_wmq.dont_exceed_cnt = 8 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb35_pcch_dl_wmq.hi_watermark = 6 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb35_pcch_dl_wmq.lo_watermark = 2 * RRCLCM_WATERMARK_SIZE;

/* Set up Queue for receiving the data from RLC on RB 0 using CCCH 
     Logical Channel. */
  (void) dsm_queue_init(&rrclcm_int_data.rrclcm_rb0_ccch_dl_wmq,8 * RRCLCM_WATERMARK_SIZE,&rrclcm_int_data.rrclcm_rb0_ccch_dl_q);
  rrclcm_int_data.rrclcm_rb0_ccch_dl_wmq.q_ptr = &rrclcm_int_data.rrclcm_rb0_ccch_dl_q;
  rrclcm_int_data.rrclcm_rb0_ccch_dl_wmq.dont_exceed_cnt = 8 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb0_ccch_dl_wmq.hi_watermark = 6 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb0_ccch_dl_wmq.lo_watermark = 2 * RRCLCM_WATERMARK_SIZE;

  /* Set up Queue for receiving the data from RLC on RB 1 using DCCH 
     Logical Channel. */
  (void) dsm_queue_init(&rrclcm_int_data.rrclcm_rb1_dcch_dl_wmq,8 * RRCLCM_WATERMARK_SIZE,&rrclcm_int_data.rrclcm_rb1_dcch_dl_q);
  rrclcm_int_data.rrclcm_rb1_dcch_dl_wmq.q_ptr = &rrclcm_int_data.rrclcm_rb1_dcch_dl_q;
  rrclcm_int_data.rrclcm_rb1_dcch_dl_wmq.dont_exceed_cnt = 8 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb1_dcch_dl_wmq.hi_watermark = 6 * RRCLCM_WATERMARK_SIZE;  
  rrclcm_int_data.rrclcm_rb1_dcch_dl_wmq.lo_watermark = 2 * RRCLCM_WATERMARK_SIZE;

  /* Set up Queue for receiving the data from RLC on RB 2 using DCCH 
     Logical Channel. */
  (void) dsm_queue_init(&rrclcm_int_data.rrclcm_rb2_dcch_dl_wmq,8 * RRCLCM_WATERMARK_SIZE,&rrclcm_int_data.rrclcm_rb2_dcch_dl_q);
  rrclcm_int_data.rrclcm_rb2_dcch_dl_wmq.q_ptr = &rrclcm_int_data.rrclcm_rb2_dcch_dl_q;
  rrclcm_int_data.rrclcm_rb2_dcch_dl_wmq.dont_exceed_cnt = 8 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb2_dcch_dl_wmq.hi_watermark = 6 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb2_dcch_dl_wmq.lo_watermark = 2 * RRCLCM_WATERMARK_SIZE;

  /* Set up Queue for receiving the data from RLC on RB 3 using DCCH 
     Logical Channel. */
  (void) dsm_queue_init(&rrclcm_int_data.rrclcm_rb3_dcch_dl_wmq,8 * RRCLCM_WATERMARK_SIZE,&rrclcm_int_data.rrclcm_rb3_dcch_dl_q);
  rrclcm_int_data.rrclcm_rb3_dcch_dl_wmq.q_ptr = &rrclcm_int_data.rrclcm_rb3_dcch_dl_q;
  rrclcm_int_data.rrclcm_rb3_dcch_dl_wmq.dont_exceed_cnt = 8 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb3_dcch_dl_wmq.hi_watermark = 6 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb3_dcch_dl_wmq.lo_watermark = 2 * RRCLCM_WATERMARK_SIZE;

  /* Set up Queue for receiving the data from RLC on RB 4 using DCCH 
     Logical Channel. */
  (void) dsm_queue_init(&rrclcm_int_data.rrclcm_rb4_dcch_dl_wmq,8 * RRCLCM_WATERMARK_SIZE,&rrclcm_int_data.rrclcm_rb4_dcch_dl_q);
  rrclcm_int_data.rrclcm_rb4_dcch_dl_wmq.q_ptr = &rrclcm_int_data.rrclcm_rb4_dcch_dl_q;
  rrclcm_int_data.rrclcm_rb4_dcch_dl_wmq.dont_exceed_cnt = 8 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb4_dcch_dl_wmq.hi_watermark = 6 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_rb4_dcch_dl_wmq.lo_watermark = 2 * RRCLCM_WATERMARK_SIZE;

  /* Set up Queue for Transmitting the data to RLC on RB 5 using DCCH 
     Logical Channel. */
  (void) dsm_queue_init(&rrclcm_int_data.rrclcm_srb5_dcch_dl_wmq,8 * RRCLCM_WATERMARK_SIZE,&rrclcm_int_data.rrclcm_srb5_dcch_dl_q);
  rrclcm_int_data.rrclcm_srb5_dcch_dl_wmq.q_ptr = &rrclcm_int_data.rrclcm_srb5_dcch_dl_q;
  rrclcm_int_data.rrclcm_srb5_dcch_dl_wmq.dont_exceed_cnt = 8 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_srb5_dcch_dl_wmq.hi_watermark = 6 * RRCLCM_WATERMARK_SIZE;
  rrclcm_int_data.rrclcm_srb5_dcch_dl_wmq.lo_watermark = 2 * RRCLCM_WATERMARK_SIZE;
...
void rrclcm_init_ul_rlc_lc_info(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8 i;
 ...
 /* Intializes Transparent mode Uplink RLC Logical Channel Ids. */
    ...
    switch(i)
  {
      case (RRCLCM_UL_TM_START_IDX): 
         rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_id = i;
         rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].status = RRCLCM_LC_NOT_IN_USE;
         rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_type = UE_LOGCHAN_CCCH;
         rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].mode = UE_MODE_TRANSPARENT;
         rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].rb_id = CCCH_RADIO_BEARER_ID;
         rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].tx_queue = &rrclcm_int_data.rrclcm_rb0_ccch_ul_wmq;
         break;
      default:
         rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_id = i;
         rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].status = RRCLCM_LC_NOT_IN_USE;
         rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_type = UE_LOGCHAN_DTCH;
         rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].mode = UE_MODE_TRANSPARENT;
         rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].rb_id = RRCLCM_RB_ID_NOT_ALLOCATED;
         rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].tx_queue = NULL;
         break;
    }
  }
       ...
       UE_MAX_UL_UM_CHANNEL); i++ )
  {
    ...
    switch(i)
    {
      case (RRCLCM_UL_UM_START_IDX) : 
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_id = i;
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].status = RRCLCM_LC_NOT_IN_USE;
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_type = UE_LOGCHAN_DCCH;
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].mode = UE_MODE_UNACKNOWLEDGED;
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].rb_id = DCCH_UM_RADIO_BEARER_ID;
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].tx_queue = &rrclcm_int_data.rrclcm_rb1_dcch_ul_wmq;
        break;
      default :
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_id = i;
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].status = RRCLCM_LC_NOT_IN_USE;
       ...
       i++ )
  {
    switch(i)
    {
      case (RRCLCM_UL_AM_DATA_START_IDX) :
        ...
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].status = RRCLCM_LC_NOT_IN_USE;
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_type = UE_LOGCHAN_DCCH;
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].mode = UE_MODE_ACKNOWLEDGED_DATA;
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].rb_id = DCCH_AM_RADIO_BEARER_ID;
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].tx_queue = &rrclcm_int_data.rrclcm_rb2_dcch_ul_wmq;
        break;
      case (RRCLCM_UL_AM_DATA_START_IDX +1) :
        ...
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_type = UE_LOGCHAN_DCCH;
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].mode = UE_MODE_ACKNOWLEDGED_DATA;
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].rb_id = DCCH_DT_HIGH_PRI_RADIO_BEARER_ID;
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].tx_queue = &rrclcm_int_data.rrclcm_rb3_dcch_ul_wmq;
        break;
      case (RRCLCM_UL_AM_DATA_START_IDX+2) :
        ...
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_type = UE_LOGCHAN_DCCH;
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].mode = UE_MODE_ACKNOWLEDGED_DATA;
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].rb_id = DCCH_DT_LOW_PRI_RADIO_BEARER_ID;
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].tx_queue = &rrclcm_int_data.rrclcm_rb4_dcch_ul_wmq;
        break;
      default:
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_id = i;
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].status = RRCLCM_LC_NOT_IN_USE;
  ...
  for( i = RRCLCM_DL_TM_START_IDX; i < UE_MAX_DL_TM_CHANNEL; i++ )
  {
    switch(i)
    {
      case (RRCLCM_DL_TM_START_IDX): 
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_id = i;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].status = RRCLCM_LC_NOT_IN_USE;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_type = UE_LOGCHAN_BCCH;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].mode = UE_MODE_TRANSPARENT;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id = BCCH_S_RADIO_BEARER_ID;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rx_queue = &rrclcm_int_data.rrclcm_rb32_bcch_dl_wmq;
        break;
      case (RRCLCM_DL_TM_START_IDX + 1): 
        ...
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_type = UE_LOGCHAN_BCCH;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].mode = UE_MODE_TRANSPARENT;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id = BCCH_N_RADIO_BEARER_ID;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rx_queue = &rrclcm_int_data.rrclcm_rb33_bcch_dl_wmq;
        break;
      case (RRCLCM_DL_TM_START_IDX + 2): 
        ...
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_type = UE_LOGCHAN_BCCH;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].mode = UE_MODE_TRANSPARENT;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id = BCCH_FACH_RADIO_BEARER_ID;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rx_queue = &rrclcm_int_data.rrclcm_rb34_bcch_fach_dl_wmq;
        break;
      case (RRCLCM_DL_TM_START_IDX + 3): 
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_id = i;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].status = RRCLCM_LC_NOT_IN_USE;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_type = UE_LOGCHAN_PCCH;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].mode = UE_MODE_TRANSPARENT;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id = PCCH_RADIO_BEARER_ID;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rx_queue = &rrclcm_int_data.rrclcm_rb35_pcch_dl_wmq;
        break;
      default:
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_id = i;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].status = RRCLCM_LC_NOT_IN_USE;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_type = UE_LOGCHAN_DTCH;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].mode = UE_MODE_TRANSPARENT;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id = RRCLCM_RB_ID_NOT_ALLOCATED;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rx_queue = NULL;
        break;
    }
  }
  ...
  {     
    switch(i)
    {
      case (RRCLCM_DL_UM_START_IDX) : 
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_id = i;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].status = RRCLCM_LC_NOT_IN_USE;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_type = UE_LOGCHAN_CCCH;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].mode = UE_MODE_UNACKNOWLEDGED;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id = CCCH_RADIO_BEARER_ID;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rx_queue = &rrclcm_int_data.rrclcm_rb0_ccch_dl_wmq;
        break;
      case (RRCLCM_DL_UM_START_IDX + 1) : 
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_id = i;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].status = RRCLCM_LC_NOT_IN_USE;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_type = UE_LOGCHAN_DCCH;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].mode = UE_MODE_UNACKNOWLEDGED;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id = DCCH_UM_RADIO_BEARER_ID;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rx_queue = &rrclcm_int_data.rrclcm_rb1_dcch_dl_wmq;
        break;

     case (RRCLCM_DL_UM_START_IDX + 2) : 
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_id = i;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].status = RRCLCM_LC_NOT_IN_USE;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_type = UE_LOGCHAN_CTCH;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].mode = UE_MODE_UNACKNOWLEDGED;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id = CTCH_RADIO_BEARER_ID;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rx_queue = NULL;
        break;
       ...
       i++ ) 
  {
    switch(i)
    {
      case (RRCLCM_DL_AM_DATA_START_IDX) :
        ...
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].status = RRCLCM_LC_NOT_IN_USE;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_type = UE_LOGCHAN_DCCH;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].mode = UE_MODE_ACKNOWLEDGED_DATA;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id = DCCH_AM_RADIO_BEARER_ID;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rx_queue = &rrclcm_int_data.rrclcm_rb2_dcch_dl_wmq;
        break;
      case (RRCLCM_DL_AM_DATA_START_IDX + 1) :
        ...
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_type = UE_LOGCHAN_DCCH;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].mode = UE_MODE_ACKNOWLEDGED_DATA;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id = DCCH_DT_HIGH_PRI_RADIO_BEARER_ID;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rx_queue = &rrclcm_int_data.rrclcm_rb3_dcch_dl_wmq;
        break;
      case (RRCLCM_DL_AM_DATA_START_IDX + 2) :
        ...
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_type = UE_LOGCHAN_DCCH;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].mode = UE_MODE_ACKNOWLEDGED_DATA;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id = DCCH_DT_LOW_PRI_RADIO_BEARER_ID;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rx_queue = &rrclcm_int_data.rrclcm_rb4_dcch_dl_wmq;
        break;
      default:
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_id = i;
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].status = RRCLCM_LC_NOT_IN_USE;
...
)
{
  ...
  for(ul_lc_id = 0; ul_lc_id < UE_MAX_UL_LOGICAL_CHANNEL; ul_lc_id ++)
  {
    ...
    ul_wm_ptr = rrclcm_get_ul_watermark_ptr(wrrc_as_id,ul_lc_id);
  ...
  {
    {
      ...
      dl_wm_ptr = rrclcm_get_dl_watermark_ptr(wrrc_as_id,dl_lc_id);
...
)
{
  ...
  /* Checks for Logical Channel Type and Radio Bearer Id match for Transparent
    ...
    case (UE_MODE_TRANSPARENT): 
      for( i = RRCLCM_UL_TM_START_IDX ; i < UE_MAX_UL_TM_CHANNEL ; i++ )
      {
        if(ul_lc_type == rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_type &&              
           rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].status == RRCLCM_LC_NOT_IN_USE)
        {
          if( rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].rb_id == rb_id ||
              rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].rb_id == RRCLCM_RB_ID_NOT_ALLOCATED )
              
          {
            if(rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].rb_id == RRCLCM_RB_ID_NOT_ALLOCATED )
            {
              rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].rb_id = rb_id;
            }
            *dsm_tx_queue = rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].tx_queue;
                                           /* Uplink DSM Watermark queue is 
                                              assigned */
            rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].status = RRCLCM_LC_IN_USE;
 ...
 /* Checks for Logical Channel Type and Radio Bearer Id match for 
  ...
  if(ul_lc_id_allocated)
  {
    WRRC_MSG3_MED("Allocated UL lc_id: %d, rb_id: %d, lc_type:%d",
             rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_id,
             rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].rb_id,
             rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_type);
    return (rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_id);
  }
  else
  {
    WRRC_MSG3_HIGH("Failed to allocate UL rb_id:%d, lc_type:%d, mode:%d",rb_id,ul_lc_type,mode);
    ...
    case (UE_MODE_TRANSPARENT): 
      for( i = RRCLCM_DL_TM_START_IDX ; i < UE_MAX_DL_TM_CHANNEL ; i++ )
      {
        if(dl_lc_type == UE_LOGCHAN_TM_DCCH_DL && 
          rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].status == RRCLCM_LC_NOT_IN_USE)
        {
          if( rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id == rb_id ||
              rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id == RRCLCM_RB_ID_NOT_ALLOCATED)
          {
            if(rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id == RRCLCM_RB_ID_NOT_ALLOCATED)
            {
              rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id = rb_id;
            }
            
            rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_type = UE_LOGCHAN_TM_DCCH_DL;
            ...
            /* Initially would be NULL;
               ...
               with RLC */
            *dsm_rx_queue = rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rx_queue;
 ...
 /* Checks for Logical Channel Type and Radio Bearer Id match for 
  ...
  if(dl_lc_id_allocated)
  {
    WRRC_MSG3_MED("Allocated DL lc_id: %d, rb_id: %d, lc_type:%d",
             rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_id,
             rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id,
             rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_type);
    return (rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_id);
  }
  else
  {
    WRRC_MSG3_HIGH("Failed to allocate DL rb_id:%d, lc_type:%d, mode:%d", rb_id, dl_lc_type, mode);
    return (RRCLCM_RLC_LC_ID_ERROR);       /* RLC Logical Channel Id allocation
                                              failed. */
  }
} /* rrclcm_allocate_dl_rlc_lc_id */
...
)
{
  ...
  /* Checks for Logical Channel Type and Radio Bearer Id match for Transparent
    ...
    case (UE_MODE_TRANSPARENT): 
      ...
      {
        if( ul_lc_type == rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_type && 
             rb_id == rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].rb_id &&
             rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].status == RRCLCM_LC_IN_USE)
        {
...
)
{
  ...
  {
    if( rb_id == rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].rb_id &&
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].status == RRCLCM_LC_IN_USE)
    {
      /* Return the lc id */
      return (rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_id);
...
)
{
  ...
  {
    if( rb_id == rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id &&
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].status == RRCLCM_LC_IN_USE)
    {
      /* Return the lc id */
      return (rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_id);  
...
)
{
  ...
  /* Checks for RLC Logical Channel Id match for Uplink RLC Logiacal Channel 
     entities. */
    if( ul_lc_id == rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_id &&
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].status == RRCLCM_LC_IN_USE)
    { 
      WRRC_MSG3_HIGH("Deallocating UL lc_id: %d, rb_id: %d, lc_type:%d", 
               rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_id,
               rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].rb_id,
               rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_type);
      ul_wm_ptr = rrclcm_get_ul_watermark_ptr(wrrc_as_id,ul_lc_id);
      ...
      switch(rrclcm_get_ul_rb_id(wrrc_as_id,ul_lc_id))
      {
...
)
{
  ...
  /* Checks for RLC Logical Channel Id match for Downlink RLC Logiacal Channel 
     entities. */
    if(dl_lc_id == rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_id &&
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].status == RRCLCM_LC_IN_USE)
    {
      WRRC_MSG3_HIGH("Deallocating DL lc_id: %d, rb_id: %d, lc_type: %d", 
               rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_id,
               rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id,
               rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_type);
      dl_wm_ptr = rrclcm_get_dl_watermark_ptr(wrrc_as_id,dl_lc_id);
      ...
      switch(rrclcm_get_dl_rb_id(wrrc_as_id,dl_lc_id))
      {
        case CCCH_RADIO_BEARER_ID:
        case DCCH_UM_RADIO_BEARER_ID:
        case DCCH_AM_RADIO_BEARER_ID:
...

{
  ...
  /* Checks for mapping between RLC Logical Channel id and RLC Logical Channel
    ...
    if(dl_lc_id == rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_id )
    {
      *dl_lc_type_ptr = rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_type;
...
)
{
  ...
  /* Checks for mapping between RLC Logical Channel id and RLC Logical Channel
    ...
    if(ul_lc_id == rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_id )
    {
      *ul_lc_type_ptr = rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_type;
      match_found = RRCLCM_SUCCESS;       /* The corresponding RLC logical
                                              channel type is identified. */
      break;
...
)
{
  ...
  {
    WRRC_MSG3_HIGH("Rcvd MUI: %d for RB id: %d status %d [0:S, 1:F]",cmd_ptr->cmd.downlink_l2ack_ind.mui,
    ...
    for(i = 0; i <  (uint8)MAX_NUM_OF_PROCEDURES ; i++)
    {
      ...
      {    
        ...
        if(rrclcm_int_data.rrclcm_l2_ack_info[i][rb_id_idx].mui == 
          cmd_ptr->cmd.downlink_l2ack_ind.mui)
        {
          *proc_name = rrclcm_int_data.rrclcm_l2_ack_info[i][rb_id_idx].rrc_proc;
          rrclcm_int_data.rrclcm_l2_ack_info[i][rb_id_idx].rrc_proc = RRC_PROCEDURE_NONE;
...
FUNCTION rrc_calculate_lta_block_timer_val

DESCRIPTION
  This function calculates the LTA block timer value.
...
uint16 rrc_calculate_lta_block_timer_val
(
  sys_modem_as_id_e_type wrrc_as_id,
...
)
{
  ...
  uint16 block_lta_tmr_val = rrcnv_get_msim_opt_block_lta_timer(wrrc_as_id);
     
  /* RCE and UECI UL messages can be very long. Add an additional time to account for transmission time */
  if(rrc_proc == RRC_PROCEDURE_RCE
     ||
     rrc_proc == RRC_PROCEDURE_UECI)
  {
    block_lta_tmr_val += L2_ACK_WAIT_ADD_TIME_FOR_RCE_UECI;
  }
  
  /* 
     If lta block timer is configured below 1 second, use a minimum value of 1 second to ensure 
     that LTAs are blocked for sufficiently long time
  */
  block_lta_tmr_val = RRC_GET_MAX_VAL(block_lta_tmr_val,RRC_MIN_LTA_BLOCK_TIME_VAL);

  return block_lta_tmr_val;
}
/*===========================================================================
...
)
{
  ...
  if(rb_id_idx < (UE_MAX_UL_AM_DATA_CHANNEL - 1))
  {
    ...
    if(l2ack_status == TRUE)
    {
      for(i = 0; i <  (uint8)MAX_NUM_OF_PROCEDURES ; i++)
      {
        /* Checks for the available index that is not allocated for any
           procedure */
        if(rrclcm_int_data.rrclcm_l2_ack_info[i][rb_id_idx].rrc_proc == RRC_PROCEDURE_NONE)
        {
          rrclcm_int_data.rrclcm_l2_ack_info[i][rb_id_idx].mui = rrclcm_current_mui[rb_id_idx]; 
    
          *mui = rrclcm_current_mui[rb_id_idx];    /* Assigns the MUI to procedure.*/
    
          rrclcm_int_data.rrclcm_l2_ack_info[i][rb_id_idx].rrc_proc = rrc_proc;
    ...
    {
      rrc_set_l2_ack_req_mask(wrrc_as_id,rrc_proc, *mui);
      if(rrcnv_get_msim_opt_enabled_nv(wrrc_as_id) & ALLOW_LTA_THOUGH_L2_ACK_PENDING)
      {
        rrctmr_start_timer(wrrc_as_id,RRCTMR_BLOCK_LTA_TIMER,
                           rrc_calculate_lta_block_timer_val(wrrc_as_id,rrc_proc));
      }
    }     
#endif
  }
...
)
{
  ...
  for(j = 0; j < (UE_MAX_UL_AM_DATA_CHANNEL - 1); j++)
  {
    for(i = 0; i <  (uint8)MAX_NUM_OF_PROCEDURES ; i++)
    {
      ...
      rrclcm_int_data.rrclcm_l2_ack_info[i][j].rrc_proc = RRC_PROCEDURE_NONE;
                                             /* Sets all RRC procedures to 
                                                NONE. */
      rrclcm_int_data.rrclcm_l2_ack_info[i][j].mui = 0;      /* Set the Initial MUIs to 0 */    
    }
  }
} /* rrclcm_init_l2_ack_info */
...
)
{
  ...
  /*There is no assignment to context anywhere after its declaration and before dereferencing, so there should be a check for the same*/
#ifdef FEATURE_DUAL_WCDMA
 if(context == NULL)
  {
    ERR_FATAL("Context was not sent by RLC",0,0,0);
  }
  wrrc_as_id = *((sys_modem_as_id_e_type *)context);
#else
  ...
  while( count > 0)
  {
    ...
    lcm_status = rrclcm_get_dl_lc_type(wrrc_as_id,dl_lc_id, &dl_spl_sdu_type);
    ...
    */
    ...
    else      
    {
      ...
      dl_rb_id = rrclcm_get_dl_rb_id(wrrc_as_id,dl_lc_id);
      switch(dl_rb_id)
      {
        case BCCH_S_RADIO_BEARER_ID: 
...
)
{
  ...
  uint8 rb_id_idx = 0; /* Local variable for RB Id 
                                          Index. */
  rb_id_idx = rrclcm_get_ul_rb_id(wrrc_as_id,dl_lc_id) - 2;
#endif
  
  if(rrcllc_curr_cmd_rel_all_req(wrrc_as_id) == TRUE)
  {
    WRRC_MSG3_HIGH("No need of posting a cmd as it is CRLC REL ALL REQ mui %d, status %d lc id %d",
                   mui,status,dl_lc_id);
    return;
  }
#ifndef FEATURE_DUAL_SIM
  for(i=0;i<MAX_NUM_OF_PROCEDURES;i++)
  {
    ...
    {
      /* Searches for MUI match in the Look Up table*/
      if(rrclcm_int_data.rrclcm_l2_ack_info[i][rb_id_idx].mui ==  mui)
      {
        mui_found =TRUE;
        break;
      }  
  ...
  if(mui_found == FALSE)
  {
    WRRC_MSG3_HIGH("L2 ack for MUI %d for unregistered  proc, status %d [0:S, 1:F] lc id %d",
                       mui,status,dl_lc_id);
    return;
  }
#endif
...
{
  return rrclcm_int_data.rrclcm_dl_rlc_lc_info[dl_lc_id].rx_queue ;
...
)
{
  ...
  return rrclcm_int_data.rrclcm_ul_rlc_lc_info[ul_lc_id].tx_queue ;
} /* rrclcm_get_ul_watermark_ptr */
/*===========================================================================
...
{
  return rrclcm_int_data.rrclcm_dl_rlc_lc_info[dl_lc_id].rb_id ;
} /* rrclcm_get_rb_dl_id */
/*=========================================================================*/
...
)
{
  ...
  /* Checks for RLC Logical Channel Id match for Uplink RLC Logiacal Channel 
        ...
        rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].status == RRCLCM_LC_IN_USE)
    {
      ...
      *tx_mode = rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].mode;
...
)
{
  ...
  /* Checks for RLC Logical Channel Id match for Downlink RLC Logiacal Channel 
        ...
        rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].status == RRCLCM_LC_IN_USE)
    {
      ...
      *rx_mode = rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].mode;
      status = RRCLCM_SUCCESS;
      break;
    }
  }
...
SIDE EFFECTS
  None
===========================================================================*/
boolean rrclcm_update_db_with_srb5_wmq_info(sys_modem_as_id_e_type wrrc_as_id,rlc_lc_id_type srb5_lc_id)
{
  boolean wm_ptr_initialized = FALSE;

  if(srb5_lc_id < UE_MAX_DL_TM_CHANNEL &&
     rrclcm_int_data.rrclcm_dl_rlc_lc_info[srb5_lc_id].status == RRCLCM_LC_IN_USE &&
     rrclcm_int_data.rrclcm_dl_rlc_lc_info[srb5_lc_id].lc_type == UE_LOGCHAN_TM_DCCH_DL)
  {
    rrclcm_int_data.rrclcm_dl_rlc_lc_info[srb5_lc_id].rx_queue = &rrclcm_int_data.rrclcm_srb5_dcch_dl_wmq;
...
)
{
  ...
  /* Checks for RLC Logical Channel Id match for Downlink RLC Logical Channel 
     entities. */
    if(rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].status == RRCLCM_LC_IN_USE &&
       rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_type == lc_type)
    {
      *rb_id = rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id;
      status = RRCLCM_SUCCESS;
      break;
...
boolean rrc_check_for_pending_ul_srb_data(sys_modem_as_id_e_type wrrc_as_id,boolean *l2_int_data_pending)
{
  int i=0;
  ...
  {
    if((rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].status == RRCLCM_LC_IN_USE) &&
        ((DCCH_AM_RADIO_BEARER_ID == rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].rb_id)||
         (DCCH_DT_HIGH_PRI_RADIO_BEARER_ID == rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].rb_id) ||
         (DCCH_DT_LOW_PRI_RADIO_BEARER_ID == rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].rb_id)
        )
      )
    {
      if(rlc_check_pending_sdu(wrrc_as_id, rrclcm_int_data.rrclcm_ul_rlc_lc_info[i].lc_id,l2_int_data_pending) == TRUE)
      {
        return (TRUE);
      }
    }
...
boolean rrc_check_for_pending_dl_srb_data(sys_modem_as_id_e_type wrrc_as_id)
{
  int i=0;
  ...
  {
    if((rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].status == RRCLCM_LC_IN_USE) &&
        ((DCCH_AM_RADIO_BEARER_ID == rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id)||
         (DCCH_DT_HIGH_PRI_RADIO_BEARER_ID == rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id) ||
         (DCCH_DT_LOW_PRI_RADIO_BEARER_ID == rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].rb_id)
        )
      )
    {
      if(rlc_check_pending_dl_sdu(wrrc_as_id, rrclcm_int_data.rrclcm_dl_rlc_lc_info[i].lc_id) == TRUE)
      {
        return (TRUE);
      }
    }
...
)
{
  ...
  for(i = 0; i <  (uint8)MAX_NUM_OF_PROCEDURES ; i++)
  {
    /* Checks for the available index that is not allocated for any
       procedure */
    if(rrclcm_int_data.rrclcm_l2_ack_info[i][rb_id_idx].rrc_proc == proc_id)
    {
      return rrclcm_int_data.rrclcm_l2_ack_info[i][rb_id_idx].mui; 
    }
  }
...
)
{
  ...
  mui = rrclcm_get_mui_from_proc_id(wrrc_as_id,proc_id, rb_id);;
  dl_lc_id = rrclcm_get_dl_rlc_lc_id(wrrc_as_id,rb_id);
  ...
  if(cmd_ptr != NULL)
  {
    ...
    WRRC_MSG3_HIGH("RRC_DOWNLINK_L2ACK_IND command is posted for proc-id rrc_proc_e_type_value%d, rb-id %d mui %d",
               proc_id,rb_id,mui);
...
{
  qsh_mdump_collect_high(&rrclcm_int_data,sizeof(rrclcm_int_data));
}
#endif
