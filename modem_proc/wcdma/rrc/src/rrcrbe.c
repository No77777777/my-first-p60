...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcrbe.c#3 $    $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/22/17   sp      Mini dump phase-2 changes
11/20/18   ak      Made changes for FR 53313
02/21/18   sp      Made changes to ignore OTA with duplicate trid
02/21/18   nr      Added code to support LTA during activation time
02/14/18   ak     Made changes to fix compilation error.
01/29/18   ak     Made changes to clear toc usage change flag
04/03/17   sas     Changes to take care of missed out global duplications for dual wcdma.
09/16/16   sp      Made changes to reset CU registration info once reconfig procedure is successful
03/16/16   sp      Made changes to avoid accessing rbe rabs array out of its boundary
03/07/16   sp      Made changes to index rbe global correctly when RB setup gives rab reconfig list and rab setup list
...
11/14/15   as      Made changes to reset pending dormancy count while establising new RAB
...
05/20/15   sp      Made changes to avoid configuring Data RB to lower layers for when OOS is received during fach to fach redirection cases
...
typedef struct
{
  ...
  boolean rrcrbe_send_initiate_cu_flag;
  boolean rrcrbe_delete_crnti;
...
LOCAL rrcrbe_int_data_type rbe_sim[MAX_RRC_STACKS];
rrc_cmd_type rrc_cmd_sim[MAX_RRC_STACKS];
#define rbe rbe_sim[WAS_ID_M]
#define rrc_cmd rrc_cmd_sim[WAS_ID_M]
#else
...
===========================================================================*/
static void rrcrbe_indicate_layer1_if_rb_setup_configure_voice_rab(sys_modem_as_id_e_type wrrc_as_id)
{
  uint32 rab_index;
...
#endif
rrcrb_substate_e_type rrcrbe_get_current_sub_state(sys_modem_as_id_e_type wrrc_as_id)
{
...
===========================================================================*/
rrc_state_e_type rrcrbe_get_dest_state(sys_modem_as_id_e_type wrrc_as_id)
{
  return rbe.state_from_msg;
}
/*===========================================================================

FUNCTION   rrcrbe_get_initiate_cu_flag()

DESCRIPTION
 This function is used to return rrcrbe_send_initiate_cu_flag 
...
===========================================================================*/
boolean rrcrbe_get_initiate_cu_flag(sys_modem_as_id_e_type wrrc_as_id)
{
  return rbe.rrcrbe_send_initiate_cu_flag;
}
...
===========================================================================*/
boolean rrcrbe_is_proc_successful(sys_modem_as_id_e_type wrrc_as_id)
{
...
===========================================================================*/
void rrcrbe_update_failure_status(sys_modem_as_id_e_type wrrc_as_id,rrcrb_msg_status_e_type status )
{
...
)
{
  ...
  rrcsmc_revert_to_old_config_for_failure_message(wrrc_as_id, FALSE);
  ...
  /*Reports Radio bearer setup failure to QSH*/
  rrc_qsh_report_failure_ota_msg_event(wrrc_as_id,WRRC_UL_DCCH_MESSAGETYPE_RADIOBEARERSETUPFAILURE);
#endif

#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
  rrcqsh_log_uplink_dcch_ota_params(wrrc_as_id, WRRC_UL_DCCH_MESSAGETYPE_RADIOBEARERSETUPFAILURE, ptr);
#endif
  ...
  if (ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
    ...
    {
      rrc_update_reconfig_waiting_for_l2ack_dch(wrrc_as_id, RRC_PROCEDURE_RBE);
      status = rrcsend_send_ul_sdu(wrrc_as_id,RRC_PROCEDURE_RBE,  /* Procedure is RBE    */
...
)
{
  ...
  return rrcsmc_append_start_list_with_already_computed_start(wrrc_as_id,
           start_list_ptr,
           start_value,
           append_start_type);
...
===========================================================================*/
static void send_rbs_complete_message(sys_modem_as_id_e_type wrrc_as_id,  boolean proc_wait_for_l2_ack )
{
  rrc_UL_DCCH_Message *ptr;     /* Pointer to uplink DCCH message type */
  rlc_lc_id_type ul_lc_id; /* Logical Channel Id for the message  */
  rrcsend_status_e_type status; /* Status from send chain module       */
  ...
  {
    if (rrcsmc_get_and_reset_start_value_to_transmit(wrrc_as_id,start_value) == TRUE)
    {
  ...
  /* in case its srns relocation then we need to set the 
    ...
    */
  (void) rrcsmc_get_and_reset_start_value_to_transmit(wrrc_as_id,start_value);
  
  if (rbe.rbe_for_srns_relocation == TRUE)
  {
    (void)rrcrbe_append_start_list_in_ul_counter_sync_info
      (wrrc_as_id, ptr, start_value, RRCSMC_EXTRACT_AND_APPEND_START);
  }
  else
  {
    if (rrcllc_get_rlc_size_change_status(wrrc_as_id) 
      && ((rbe.state_from_msg == RRC_STATE_CELL_PCH || 
             rbe.state_from_msg == RRC_STATE_URA_PCH)
          || (rrc_get_rlc_size_change_status_for_procedure_id(wrrc_as_id,RRC_PROCEDURE_RBE) == TRUE)))
    {
          ...
          rbe.state_from_msg == RRC_STATE_URA_PCH)
      {
        ...
        (void)rrcsmc_extract_return_hfn_per_domain_and_update_start_flag(wrrc_as_id, RRC_PS_DOMAIN_CN_ID);
      }
      else if (rrc_get_rlc_size_change_status_for_procedure_id(wrrc_as_id,RRC_PROCEDURE_RBE) == TRUE)
      {
        rrc_set_rlc_size_change_status(wrrc_as_id,RRC_PROCEDURE_NONE, 
                                       FALSE, RRC_INVALID_RB_ID ,
                                       RLC_RE_ESTABLISH_NONE);
      }

      (void)rrcrbe_append_start_list_in_ul_counter_sync_info
        (wrrc_as_id, ptr, start_value, RRCSMC_APPEND_START);
    }
  }
  ...
  if (ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
    ...
    rex_set_timer(&rbe.rrcrbe_temp_timer, 5000);   /* 5 seconds */

    rbe.rrcrbe_sig = rex_wait(wrrc_as_id, RRCRBE_TEMP_TIMER_SIG);
    MSG_LOW("RRCRBE signal:%d", rbe.rrcrbe_sig,0, 0);
    (void) rex_clr_sigs(rex_self(), RRCRBE_TEMP_TIMER_SIG);
#endif /* FEATURE_RRC_DELAY_TX_ON_NEW_CFG */

    new_tm_rb_setup = rrcsmc_is_new_tm_rb_setup(wrrc_as_id);
        ...
        (new_tm_rb_setup == TRUE) )
    {
     ...
     RRC_MSG_COMMON_SET_BITMASK_IE(ptr->message.u.radioBearerSetupComplete,
      ...
      ptr->message.u.radioBearerSetupComplete.count_C_ActivationTime =
        rbe.ciphering_activation_cfn;

      WRRC_MSG1_HIGH("Ciphering activation CFN = %d", rbe.ciphering_activation_cfn);
    }

    WRRC_MSG5_HIGH("hho_ciph_status: %d, trans_to_dch_ciph_config_needed: %d, new_tm_rb_setup: %d TM_CIPH_STATUS : %d FIRST TM RB STATUS :%d",
    rbe.hho_ciph_status, rbe.trans_to_dch_ciph_config_needed, new_tm_rb_setup,rrcsmc_tm_ciph_active(wrrc_as_id),rbe.first_tm_rb_setup);
    ...
    if (((rbe.hho_ciph_status == TRUE) || (rbe.trans_to_dch_ciph_config_needed == TRUE)) &&
        ((TRUE == rrcsmc_tm_ciph_active(wrrc_as_id)) && (FALSE == new_tm_rb_setup)))
    {
      (void)rrcrbe_append_start_list_in_ul_counter_sync_info(wrrc_as_id,
      ...
      ptr->message.u.radioBearerSetupComplete.count_C_ActivationTime =
        rrcsmc_step_2_cipher_config_for_hho(wrrc_as_id);
    ...
    if((TRUE == rrcsmc_tm_ciph_active(wrrc_as_id)) && (rbe.first_tm_rb_setup == TRUE))
    {
      ...
      ptr->message.u.radioBearerSetupComplete.count_C_ActivationTime =
      rrcsmc_step_2_cipher_config_for_first_tm_rb_setup(wrrc_as_id);
...
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
    rrcqsh_log_uplink_dcch_ota_params(wrrc_as_id, WRRC_UL_DCCH_MESSAGETYPE_RADIOBEARERSETUPCOMPLETE, ptr);
#endif
  ...
  rrc_send_rb_rate_info_packet(wrrc_as_id);

  rrc_pdcp_report_rb_rates(wrrc_as_id,FALSE);
...
)
{
  ...
  if (rbe.cn_info_saved != FALSE)
  {
    ...
    if(rbe.mm_cmd_ptr->cmd.hdr.message_id ==(byte) RRC_SERVICE_IND_FROM_OTA_MSG)
    {
      ...
      rrc_ccm_update_service_info(wrrc_as_id,rbe.mm_cmd_ptr);
    }
    /* Put the command on MM queue */
    rrc_send_rrc_message_to_mm(wrrc_as_id,rbe.mm_cmd_ptr);
    /* clear the flag */
  ...
  for (rab_index=0; rab_index < rbe.rabs.num_rabs_in_msg; rab_index++)
  {
         ...
         (rbe.rabs.rabs_in_msg[rab_index].rab_type == RRCRB_CS_DATA_RAB) )
    {
     ...
     mm_ptr = mm_rrc_get_cmd_buf(RRC_SYNC_IND);
      ...
      mm_ptr->cmd.rrc_sync_ind.as_id = rrc_get_as_id(wrrc_as_id);
#endif
      ...
      /* Put the command on MM queue */
      WRRC_MSG3_HIGH("Dispatching out MM cmd / Sending RRC_SYNC_IND to MM for RAB type %d RAB-id %d Codec %d ", 
      ...
      rrc_send_rrc_message_to_mm(wrrc_as_id, mm_ptr);
    }

    else if ( rbe.rabs.rabs_in_msg[rab_index].rab_type == RRCRB_PS_DATA_RAB )
    {

      if(rrcscrr_is_cn_domain_open(wrrc_as_id,RRC_PS_DOMAIN_CN_ID) == TRUE)
      {
  ...
  /* Reset the number of pending dormancy request to zero if any as new RAB is getting added.
   * Higher layers have to trigger dormancy again if inactivity is detected after addition of new RAB
   */
  rrcdormancy_reset_pending_dormancy_req(wrrc_as_id);
...
===========================================================================*/
static void  rrcrbe_send_crnti_update_req(sys_modem_as_id_e_type wrrc_as_id,uint16  crnti)
{
...
)
{
  ...
  else if (msg_version == MSG_REL10)
  {
    if (RRCRB_R10_MSG_IE_PRESENT((&msg_ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.u.
    criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.radioBearerSetup_r10),
    dl_CounterSynchronisationInfo))
    {
      return TRUE;
    }
...
)
{
  ...
  if(new_tm_rb)
  {
    for(i=0;i<MAX_RAB_TO_SETUP;i++)
    {
      ...
      {
        dl_mode_tm = FALSE;
        lc_id = rrclcm_get_dl_rlc_lc_id(wrrc_as_id,rrc_est_rabs.rabs[i].rb_for_rab[j].rb_id);
        if( lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
        {
          rrclcm_status = rrclcm_get_dl_rlc_mode(wrrc_as_id,lc_id, &rx_mode);
        ...
        if(dl_mode_tm == TRUE)
        {
          lc_id = rrclcm_get_ul_rlc_lc_id(wrrc_as_id,rrc_est_rabs.rabs[i].rb_for_rab[j].rb_id);
          if( lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
          {
            rrclcm_status = rrclcm_get_ul_rlc_mode(wrrc_as_id,lc_id, &rx_mode);
            if((rrclcm_status == RRCLCM_SUCCESS) && (rx_mode == UE_MODE_TRANSPARENT))
            {
              status = FALSE;
              break;
...
)
{
 ...
 /* set the default nas_sync_ind to the current codec used*/
  uint32 nas_sync_ind=rrc_get_current_codec_mode(wrrc_as_id);
  	...
  	dl_HSPDSCH_Information))
  {
    ...
    status = rrc_hsdpa_initial_validation(wrrc_as_id,rbsetup_ptr->rrc_StateIndicator, 
  	...
  	rrc_RadioBearerSetup_r5_IEs,rab_InformationSetupList))
  {
    ...
    do  /* going through the RAB info setup list */
    {
      ...
      do  /* going through the RB info setup list */
      {
        ...
        if (rrc_find_rb_in_est_rabs(wrrc_as_id,rb_setup_info_ptr->rb_Identity) == RRC_RB_PRESENT) 
        {
        ...
        if(new_tm_rb == FALSE)
        {
          ...
          {
            local_lc_id = rrclcm_get_dl_rlc_lc_id(wrrc_as_id,rb_setup_info_ptr->rlc_InfoChoice.u.same_as_RB);
            if(local_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
            {
              result = rrclcm_get_dl_rlc_mode(wrrc_as_id,local_lc_id, &rx_mode);
            ...
            local_lc_id = rrclcm_get_ul_rlc_lc_id(wrrc_as_id,rb_setup_info_ptr->rlc_InfoChoice.u.same_as_RB);
            if(local_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
            {
              result = rrclcm_get_ul_rlc_mode(wrrc_as_id,local_lc_id, &rx_mode);
  ...
  SRNS relocation information can come in reconfiguration messages
          ...
          ( rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH)))
    {
      WRRC_MSG2_ERROR("SRNS reloc with invalid states cur_state %d, msg_state %d",
          rrc_get_state(wrrc_as_id), rbe.state_from_msg);
      rbe.status.failure_status=RRCRB_MSG_INVALID_CONFIG;
      return;
    }
    ...
    else
    {
          ...
          rrc_RadioBearerSetup_r5_IEs,new_U_RNTI))
      {
        ...
        rrc_translate_urnti (wrrc_as_id, &(rbsetup_ptr->new_U_RNTI),
                             &rbe.new_urnti);
      }
    }
  	...
  	rrc_RadioBearerSetup_r5_IEs,rab_InformationSetupList))
  {
    ...
    {
      rab_id = rrcrb_translate_gsm_map_rab_id(wrrc_as_id,(rab_list_ptr->elem[rab_index].rab_Info.
    ...
    be updated later, when these RABs are established */
      ...
      if ( rab_list_ptr->elem[rab_index].rab_Info.cn_DomainIdentity == rrc_CN_DomainIdentity_cs_domain )
      {
...
#ifdef FEATURE_DUAL_SIM
        rab_type = rrcmn_verify_rab_subs( (radio_access_bearer_T)rab_id,rrc_get_as_id(wrrc_as_id));
#else
        rab_type = rrcmn_verify_rab((radio_access_bearer_T)rab_id );
#endif
        WRRC_MSG3_HIGH("For RAB-Id %d, RAB type from NAS = %d, RAB type from EST_RABs = %d",rab_id, rab_type,
            rrc_get_rab_type_for_given_rab_id(wrrc_as_id,rbe.rabs.rabs_in_msg[rab_index].rab_id));
        ...
        switch ( rab_type )
        {
          ...
          case RRCMN_CS_VOICE_CALL:
            ...
            if (rrc_find_if_voice_rab_exists_in_established_rabs(wrrc_as_id, rbe.rabs.rabs_in_msg[rab_index].rab_id )
              == TRUE)
            {
        ...
        if((rab_type == RRCMN_CS_VOICE_CALL )
           || 
           (rrc_check_nas_rab_modified(wrrc_as_id) == TRUE)
          )
        {
          ...
          {
            nas_sync_ind =  rrc_translate_nas_sync_indicator(wrrc_as_id,&(rab_list_ptr->elem[rab_index].rab_Info.nas_Synchronisation_Indicator));
            if( nas_sync_ind == WCDMA_IVOCODER_ID_NONE )
            {
              rbe.status.failure_status = RRCRB_MSG_UNSUPPORTED_CONFIG;
          ...
          rrc_set_ordered_codec_status(wrrc_as_id, TRUE);
#endif
        }
      }
      ...
      else if ( rab_list_ptr->elem[rab_index].rab_Info.cn_DomainIdentity == rrc_CN_DomainIdentity_ps_domain )
      {
        ...
        if ( rrclbt_is_lb_on(wrrc_as_id) == TRUE ||
#ifdef FEATURE_DUAL_WCDMA
             rabm_rabid_validate_per_sub(wrrc_as_id, RAB_ESTABLISH_QUERY, (uint8)rab_id )
#else
      ...
      /* set the ordered codec mode which is sent in the ordered config to MAC.
        ...
        * is passed to MAC 
        */
      rrc_set_ordered_codec_mode(wrrc_as_id,nas_sync_ind);
      rbe.rabs.rabs_in_msg[rab_index].nas_Synchronisation_Indicator = nas_sync_ind;
     ...
     )
  {
         ...
         rrc_DL_CommonInformation_r5,dl_DPCH_InfoCommon)))
    {
      ...
      */
      if (rrcsmc_tm_ciph_active(wrrc_as_id) == TRUE)
      {
        ...
        loc_ret_val = rrcrb_get_mac_d_hfn(wrrc_as_id,
        ...
        if (rrcsmc_process_cipher_config_for_hho(wrrc_as_id,mac_d_hfn_l) == FAILURE)
        {
  ...
  if(rbe.state_from_msg == RRC_STATE_CELL_DCH)
  {
    rbe.first_tm_rb_setup = rrcrbe_is_first_tm_rb_setup(wrrc_as_id,new_tm_rb);
    ...
    if(rbe.first_tm_rb_setup == TRUE)
    {
           ...
           rrc_DL_CommonInformation_r5,dl_DPCH_InfoCommon)))
      {
        ...
        */
        ...
        if (rrcsmc_process_cipher_config_for_first_tm_setup(wrrc_as_id,mac_d_hfn_l) == FAILURE)
        {
  ...
  if (rbe.state_from_msg == RRC_STATE_URA_PCH)
  {
    ...
    else
    {
      ...
      {
        rrccu_update_ura_id (wrrc_as_id,&(rbsetup_ptr->ura_Identity));
      }
      else
      {
...
)
{
  ...
  {
    /* Check if network does not try to initiate HSDPA with invalid state */
    status = rrc_hsdpa_initial_validation_r6(wrrc_as_id,rbsetup_ptr->rrc_StateIndicator,
      ...
      rrc_RadioBearerSetup_r6_IEs_specificationMode_complete))
  {
    ...
    if (rbsetup_ptr->specificationMode.u.complete.m.rab_InformationSetupListPresent)
    {
      ...
      while (rab_list_ptr->n > rab_index)  /* going through the RAB info setup list */
      {
        ...
        while (rab_list_ptr->elem[rab_index].rb_InformationSetupList.n > rb_index)
        {
          ...
          if (rrc_find_rb_in_est_rabs(wrrc_as_id,rb_info_ptr->rb_Identity) == RRC_RB_PRESENT) 
          {
            WRRC_MSG1_HIGH("RB id %d already exist in est rabs: Invalid config", 
          ...
          if(new_tm_rb == FALSE)
          {
            ...
            {
              local_lc_id = rrclcm_get_dl_rlc_lc_id(wrrc_as_id,rb_info_ptr->rlc_InfoChoice.u.same_as_RB);
              if(local_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
              {
                result = rrclcm_get_dl_rlc_mode(wrrc_as_id,local_lc_id, &rx_mode);
                if(result == RRCLCM_SUCCESS && rx_mode == UE_MODE_TRANSPARENT)
              ...
              local_lc_id = rrclcm_get_ul_rlc_lc_id(wrrc_as_id,rb_info_ptr->rlc_InfoChoice.u.same_as_RB);
              if(local_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
              {
                result = rrclcm_get_ul_rlc_mode(wrrc_as_id,local_lc_id, &rx_mode);
                if(dl_tm_flag == TRUE && result == RRCLCM_SUCCESS && rx_mode == UE_MODE_TRANSPARENT)
    ...
    if (rbsetup_ptr->specificationMode.u.complete.m.rab_InformationReconfigListPresent)
    {
      ...
      for (rab_index = 0;(rab_reconfig_list_ptr->n > rab_index) && (rab_index < RRCRB_MAX_RAB_PER_MSG); rab_index++)
      {
        ...
        rab_id = rrcrb_translate_gsm_map_rab_id(wrrc_as_id,(rab_reconfig_list_ptr->elem[rab_index].rab_Identity.u.gsm_MAP_RAB_Identity));
  
        /* Check if the RAB id is already in ESTABLISHED_RABS */
        rab_status = rrc_find_rab_in_established_rabs(wrrc_as_id,rab_id);
        ...
        if ( rab_reconfig_list_ptr->elem[rab_index].cn_DomainIdentity  == rrc_CN_DomainIdentity_cs_domain )
        {
          ...
          {
            nas_sync_ind =   rrc_translate_nas_sync_indicator(wrrc_as_id,&(rab_reconfig_list_ptr->elem[rab_index].nas_Synchronisation_Indicator));
            if( nas_sync_ind == WCDMA_IVOCODER_ID_NONE )
            {
              rbe.status.failure_status = RRCRB_MSG_UNSUPPORTED_CONFIG;
              return;
    ...
    if (rbsetup_ptr->specificationMode.u.complete.m.rab_InformationSetupListPresent)	
    {
      ...
      {
        rab_id = rrcrb_translate_gsm_map_rab_id(wrrc_as_id,(rab_list_ptr->elem[rab_list_index].rab_Info.
                                                  rab_Identity.u.gsm_MAP_RAB_Identity));
        ...
        if ( rab_list_ptr->elem[rab_list_index].rab_Info.cn_DomainIdentity == RRC_CS_DOMAIN_CN_ID )
        {
            ...
            )
          {
            ...
            {
              nas_sync_ind =  rrc_translate_nas_sync_indicator(wrrc_as_id,
  ...
  */
      ...
      (rbsetup_ptr->specificationMode.u.complete.m.rb_InformationReconfigListPresent))
  {
    ...
    if (FAILURE == rrcllc_validate_rb_info_reconfig_list_r6(wrrc_as_id,
...
)
{
     ...
     rb_index =0;
      ...
      do  /* going through the RB info setup list */
      {
       rb_infosetup_ptr = &rab_info_ptr->elem[rab_index].rb_InformationSetupList.elem[rb_index];
        if (rrc_find_rb_in_est_rabs(wrrc_as_id,rb_infosetup_ptr->rb_Identity) == RRC_RB_PRESENT) 
        {
          WRRC_MSG1_HIGH("RB id %d already exist in est rabs: Invalid config", 
        ...
        if(new_tm_rb == FALSE)
        {
          ...
          {
            local_lc_id = rrclcm_get_dl_rlc_lc_id(wrrc_as_id,rb_infosetup_ptr->rlc_InfoChoice.u.same_as_RB);
            if(local_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
            {
              result = rrclcm_get_dl_rlc_mode(wrrc_as_id,local_lc_id, &rx_mode);
              if(result == RRCLCM_SUCCESS && rx_mode == UE_MODE_TRANSPARENT)
            ...
            local_lc_id = rrclcm_get_ul_rlc_lc_id(wrrc_as_id,rb_infosetup_ptr->rlc_InfoChoice.u.same_as_RB);
            if(local_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
            {
              result = rrclcm_get_ul_rlc_mode(wrrc_as_id,local_lc_id, &rx_mode);
              if(dl_tm_flag == TRUE && result == RRCLCM_SUCCESS && rx_mode == UE_MODE_TRANSPARENT)
      ...
      rrc_RadioBearerSetup_r3_IEs,dl_CounterSynchronisationInfo))
  {
    ...
    else
    {
      ...
      {
        rbe.new_urnti_valid = TRUE;
        rrc_translate_urnti (wrrc_as_id, &(msg_ptr->u.r3.radioBearerSetup_r3.new_U_RNTI),
                              &rbe.new_urnti);
      }
    }
  }
     ...
     )
  {
    ...
    {
      rab_id = rrcrb_translate_gsm_map_rab_id(wrrc_as_id,
     ...
     (rrc_get_state(wrrc_as_id) != RRC_STATE_CELL_DCH))
  {
        ...
        rrc_DL_CommonInformation,dl_DPCH_InfoCommon)))
    {
      ...
      {
        WRRC_MSG0_HIGH("To-DCH-Step-1 config activated");
        if(rrcsmc_process_cipher_config_for_hho(wrrc_as_id, RRCSMC_INVALID_HFN_VALUE) == FAILURE)
        {
    ...
    if(rbe.first_tm_rb_setup == TRUE)
    {
          ...
          rrc_DL_CommonInformation,dl_DPCH_InfoCommon)))
      {
        ...
        */
        ...
        if(rrcsmc_process_cipher_config_for_first_tm_setup(wrrc_as_id, RRCSMC_INVALID_HFN_VALUE) == FAILURE)
        {
      ...
      (rbe.state_from_msg == RRC_STATE_URA_PCH))
  {
     ...
     if (rbe.state_from_msg == RRC_STATE_URA_PCH)
     {
       ...
       else
       {
         ...
         {
           rrccu_update_ura_id (wrrc_as_id,&(msg_ptr->u.r3.radioBearerSetup_r3.ura_Identity));
         }
         else
         {
           WRRC_MSG1_ERROR("Invalid URA ID length%d",
...
SIDE EFFECTS
  None.
===========================================================================*/
static uint8 rrcrbe_get_rab_index_from_rbe_db(sys_modem_as_id_e_type wrrc_as_id, rrc_rab_id_type rab_id)
{
...
)
{
  ...
  if(RRC_MSG_COMMON_BITMASK_IE(msg_ptr->u.r3.radioBearerSetup_r3,rrc_RadioBearerSetup_r3_IEs,srb_InformationSetupList))
  {
    ...
    while((srb_setup_ptr->n > srb_cnt ) && (srb_cnt < UE_MAX_SRB_SETUP))
    {
      ...
      */
      ...
      if(rrclcm_get_dl_rlc_lc_id(wrrc_as_id,rb_id) == RRCLCM_RLC_LC_ID_NOT_FOUND )
      {
    ...
    for (rab_index=0; rab_index < rbe.rabs.num_rabs_in_msg && (rab_info_ptr->n > rab_index); rab_index++)
    {
      if ( rbe.rabs.chan_config_needed[rab_index] == TRUE )
      {
        ...
        else
        {
          ...
          uint8 rbe_rab_index = rrcrbe_get_rab_index_from_rbe_db(wrrc_as_id,
                                    rrcrb_translate_gsm_map_rab_id(wrrc_as_id,rab_info_ptr->elem[rab_index].rab_Info.rab_Identity.u.gsm_MAP_RAB_Identity));
          rbe_rb_cnt = 0;
  ...
  if(rb_count < MAX_RB)
  {
    rrc_fill_rb_id_chan_req_from_standalone_db(wrrc_as_id,&rb_count, &rrc_cmd.cmd.chan_config_req);
  }
  ...
  if(rbe.state_from_msg == RRC_STATE_CELL_DCH)
  {
    ...
    if(rb_count < MAX_RB)
    {
      rrc_fill_rb_id_chan_req_from_rb_list_mapped_to_dch(wrrc_as_id,&rb_count, &rrc_cmd.cmd.chan_config_req);
    }
    if(rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH)
    {
      rrc_fill_srb_in_chan_config_req(wrrc_as_id,&rb_count, &rrc_cmd.cmd.chan_config_req);
    }
  ...
  if(((next_rrc_state == RRC_STATE_CELL_FACH) && 
       ...
       ||
      ((rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH) && (next_rrc_state == RRC_STATE_CELL_DCH))
    )
  {
    rrc_fill_rb_id_chan_req_from_est_rabs(wrrc_as_id,&rb_count, &rrc_cmd.cmd.chan_config_req);
  }

  /* Check if state change is required */
  if ( (rrc_get_state(wrrc_as_id)) == (rbe.state_from_msg) )
  {
     ...
     (rrc_cmd.cmd.chan_config_req.rrc_state_change_required))
  {
    ...
    rrcrb_send_initiate_cell_selection_req(wrrc_as_id,RRC_PROCEDURE_RBE,
  ...
  else
  {
       ...
       (rbe.state_from_msg==RRC_STATE_CELL_FACH))
    {
      ...
      if (rrcrb_validate_directed_cell_info (wrrc_as_id,directed_cell_info) == FALSE)
      {
        ...
        if(rrccsp_check_initiate_cell_selection_handling(wrrc_as_id, NULL) == FALSE)
        {
          WRRC_MSG0_HIGH("Freq_Redir:Skipping Freq Redir bcoz Cell Resel in progress");
       
          rbe.rrcrbe_send_initiate_cu_flag = TRUE;
          rbe.rrcrbe_delete_crnti          = TRUE;
        }
        else
        {
             ...
             rrc_RadioBearerSetup_r3_IEs,new_C_RNTI))
          {
            ...
            rrc_translate_crnti(wrrc_as_id,&msg_ptr->u.r3.radioBearerSetup_r3.new_C_RNTI,
                                &c_rnti);
          ...
          rrcllcoc_update_toc_with_oc_from_fach(wrrc_as_id);
    ...
    */
    if ((out_cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id))!=NULL)
    {
      WCDMA_MEMCPY(out_cmd_ptr,
                   sizeof(rrc_cmd_int_type),
                   &rrc_cmd,
                   sizeof(rrc_cmd_int_type));
      rrc_put_int_cmd(wrrc_as_id, out_cmd_ptr );
    }
  }
        ...
        (rbe.state_from_msg==RRC_STATE_CELL_FACH))
    {
      ...
      if (rrcrb_validate_directed_cell_info (wrrc_as_id,directed_cell_info) == FALSE)
      {
        ...
        else
        {
          ...
          {
            rbe.new_crnti_valid = TRUE;
            rrc_translate_crnti(wrrc_as_id,&rbsetup_ptr->new_C_RNTI,
                                &c_rnti);
            rbe.new_crnti = (uint16)c_rnti;
          }
          else
...
===========================================================================*/
static void rrcrbe_clear_procedure(sys_modem_as_id_e_type wrrc_as_id)
{
  uint32 count;
  ...
  rbe.rrcrbe_send_initiate_cu_flag = FALSE;
  rbe.rrcrbe_delete_crnti          = FALSE;
  ...
  for ( count=0; count < RRCRB_MAX_RAB_PER_MSG; count++ )
  {
    ...
    rbe.rabs.rabs_in_msg[count].nas_Synchronisation_Indicator = WCDMA_IVOCODER_ID_NONE;
  }
  rbe.rabs.num_rabs_in_msg = 0;
  ...
  rrccu_reset_registration_info_for_procedure(wrrc_as_id,RRC_PROCEDURE_RBE);
  rrc_common_procedure_clear(wrrc_as_id,RRC_PROCEDURE_RBE);
}
/*===========================================================================
    ...
    do  /*going through the RB info setup list*/
    {
      ...
      if(RRC_MSG_COMMON_BITMASK_IE_PTR(rb_info_ptr,rrc_RB_InformationSetup_r8,pdcp_Info))
      {
        ...
        if (rrc_cap_related_info.rrc_pdcp_enabled)
        {
          ...
          {
            if(rrcllc_validate_pdcp_info_in_rb_info_setup_list_r8(wrrc_as_id,rb_info_ptr) == FALSE)
            {
              rbe.status.failure_status = RRCRB_MSG_INVALID_CONFIG;
                              ...
                              && (rab_list_index < RRCRB_MAX_RAB_PER_MSG); rab_index++,rab_list_index++)
  {
    ...
    else
    {
      /* ASN1 structure 'rrc_RAB_InformationSetup' is validated at this point,
          hence update rab_type in structure 'rbe.rabs.rabs_in_msg[rab_index]'. */
          /* change argument */
      (void) rrc_update_rab_type_in_rab_info_r8(wrrc_as_id,&(rab_list_ptr->elem[rab_list_index]),
         &(rbe.rabs.rabs_in_msg[rab_index]));
    }
...
)
{
  ...
  if(RRCRB_R8_MSG_IE_PRESENT(rbsetup_ptr,rrc_RadioBearerSetup_r8_IEs,rab_InformationSetupList))
  {
    ...
    if(rrcrbe_prevalidate_rab_setup_list_r8(wrrc_as_id,&rbsetup_ptr->rab_InformationSetupList,&new_tm_rb)== FAILURE)
    {
      return;
    }
  }
  ...
  if(RRCRB_R8_MSG_IE_PRESENT(rbsetup_ptr,rrc_RadioBearerSetup_r8_IEs,dl_CounterSynchronisationInfo))
  {
    ...
    else
    {
      ...
      {
        rbe.new_urnti_valid = TRUE;
        rrc_translate_urnti (wrrc_as_id, &(rbsetup_ptr->new_U_RNTI), &rbe.new_urnti);
      }
    }
  }
  ...
  if(RRCRB_R8_MSG_IE_PRESENT(rbsetup_ptr,rrc_RadioBearerSetup_r8_IEs,rab_InformationReconfigList))
  {
    ...
    rbe.status.failure_status = rrc_update_rab_info_reconfig_param_r8(wrrc_as_id,
    &rbsetup_ptr->rab_InformationReconfigList, &rbe.rabs ,&nas_sync_ind);
    if( rbe.status.failure_status != RRCRB_MSG_VALID) 
    {
      return;
    }
  ...
  if(RRCRB_R8_MSG_IE_PRESENT(rbsetup_ptr,rrc_RadioBearerSetup_r8_IEs,rab_InformationSetupList))
  {
    if(rrcrbe_validate_rab_setup_list_r8(wrrc_as_id,&rbsetup_ptr->rab_InformationSetupList, 
  ...
  if((rbe.state_from_msg == RRC_STATE_CELL_DCH) && (rrc_get_state(wrrc_as_id) != RRC_STATE_CELL_DCH))
  {
    //check to see if dl-dpch-common info is present while going to dch
  ...
  {
    /* Save the linked list head pointer */
    if (FAILURE == rrcllc_validate_rb_info_reconfig_list_r8(wrrc_as_id,&rbsetup_ptr->rb_InformationReconfigList))
    {
      rbe.status.failure_status = RRCRB_MSG_INVALID_CONFIG;
      return;
    }
...
)
{
  ...
  if (TRUE == rrcrbe_validate_srns_relocation_info(wrrc_as_id,msg_ptr, msg_version))
  {
    ip_check_status = rrcsmc_check_integrity_for_srns_relocation(wrrc_as_id,cmd_ptr, msg_version);
    ...
    case RRCSMC_INCOMPATIBLE_SECURITY_RECONFIGURATION:
      ...
      else
      {
        ...
        if ( (rrc_ccm_is_reselection_in_progress(wrrc_as_id) == TRUE)     ||
             (RRCCU_STARTED == rrccu_get_cell_update_started_status(wrrc_as_id) ))
        {
          if(rbe.curr_substate == RRCRB_WAIT_FOR_CELL_UPDATE_CNF)
          {
            ...
            send_rbs_failure_message( wrrc_as_id, rbe.status.failure_status,
          ...
          else
          {
            ...
            (void)rrccu_register_for_cell_update_to_complete(wrrc_as_id, RRC_PROCEDURE_RBE);
  ...
  if ((rrcllc_get_ordered_config_status(wrrc_as_id) != OC_NOT_SET) ||
      (rrc_ccm_is_reselection_in_progress(wrrc_as_id) == TRUE)     ||
      (RRCCU_STARTED == rrccu_get_cell_update_started_status(wrrc_as_id) ))
  {
  ...
  else if((rrcdata_check_if_ota_is_buffered(wrrc_as_id) == TRUE)
          &&
         (rrc_buffer_ota_info.procedure == RRC_PROCEDURE_RBE)
          &&
         (rrc_buffer_ota_info.transaction_id != RRCRB_INVALID_TRANSACTION_ID)
          &&
         (rrc_buffer_ota_info.transaction_id == msg_ptr->u.later_than_r3.rrc_TransactionIdentifier))
  {
    WRRC_MSG0_ERROR("BUF_OTA:Received Duplicate RBS, Ignoring");
    return ;
  }
#endif
  ...
  validate_rbsetup_r8(wrrc_as_id, msg_ptr);
  ...
  if ( rbe.status.failure_status != RRCRB_MSG_VALID )
  {
    ...
    send_rbs_failure_message( wrrc_as_id, rbe.status.failure_status, rbe.status.prot_err_cause);

    /* Make sure we're back in the initial sub-state */
    rbe.curr_substate = RRCRB_INITIAL;
    rrcrbe_clear_procedure(wrrc_as_id);
  }
  /* If the message is valid, proceed further */
  else
  {
    rrcrbe_indicate_layer1_if_rb_setup_configure_voice_rab(wrrc_as_id);
    ...
    if(RRCRB_R8_MSG_IE_PRESENT(rbsetup_ptr,rrc_RadioBearerSetup_r8_IEs,rab_InformationSetupList))
    {
      ...
      rrcrb_update_mac_rab_status(wrrc_as_id,RRC_PROCEDURE_RBE,
    ...
    if (rrcsmc_is_ciphering_enabled(wrrc_as_id) == RRCSMC_SECURITY_STARTED)
    {
      if(RRCRB_R8_MSG_IE_PRESENT(rbsetup_ptr,rrc_RadioBearerSetup_r8_IEs,activationTime))
      {
        ...
        rbe.ciphering_activation_cfn = (uint8)((rbe.ciphering_activation_cfn/8)*8);
      ...
      else
      {
        ...
        this should give sufficient time for n/w side and ue side to have
        configured ciphering on the rb's successfully*/
        rbe.ciphering_activation_cfn = (uint8)((wl1_seq_get_cfn(wrrc_as_id) + 250) % 256);
        /*round it to 8 frame boundary */
        rbe.ciphering_activation_cfn = (uint8)((rbe.ciphering_activation_cfn/8)*8);
      }
    }
    ...
    rrcsmc_update_ciphering_activation_cfn(wrrc_as_id,rbe.ciphering_activation_cfn);
    ...
    {
      rbe.mm_cmd_ptr = rrc_ccm_buid_service_ind_with_nas_cn_info_r6(wrrc_as_id,
         ...
         )))
    {
      ...
      for (rab_index=0; rab_index < rbe.rabs.num_rabs_in_msg; rab_index++)
      {
        rab_status = rrc_find_rab_in_established_rabs(wrrc_as_id,rbe.rabs.rabs_in_msg[rab_index].rab_id );
        rbe.rabs.chan_config_needed[rab_index] = TRUE;

        if ((rab_status != RRC_RAB_NOT_FOUND) &&
             (rrc_find_if_voice_rab_exists_in_established_rabs(wrrc_as_id, rbe.rabs.rabs_in_msg[rab_index].rab_id )
               == FALSE))
        {
      ...
      oc_status = rrcllc_set_ordered_config(wrrc_as_id, RRC_PROCEDURE_RBE, rbe.state_from_msg,
      ...
      switch ( oc_status )
      {
        case ORDERED_CONFIG_SET:
          ...
          rrcrbe_send_channel_config_req_r8(wrrc_as_id,msg_ptr);
              ...
              (rrc_cmd.cmd.chan_config_req.rrc_state_change_required))
          {
            ...
            {
              rbe.new_crnti_valid = TRUE;
              rrc_translate_crnti(wrrc_as_id,&rbsetup_ptr->new_C_RNTI, &c_rnti);
              rbe.new_crnti = (uint16)c_rnti;
            }
            else
            {
        ...
        case ORDERED_CONFIG_CONFIGURATION_SET_DEFERRED:
          WRRC_MSG0_HIGH("BUF_OTA:RB setup processing deferred due to LTA!");

          rbe.curr_substate = RRCRB_INITIAL;
          rrc_buffer_ota_info.transaction_id = rbe.tr_ids.accepted_transaction;
          /* Indicate that an error occured */
          ordered_config_err_occured = TRUE;
          break;
        ...
        if (rbe.rbe_for_srns_relocation == TRUE)
        {
          rrcsmc_commit_fresh_for_srns_relocation(wrrc_as_id);
        }
    }
    else
    {
...
)
{
                              ...
                              &&(rab_list_index < RRCRB_MAX_RAB_PER_MSG) ; rab_index++,rab_list_index++)
  {
    ...
    else
    {
      ...
      hence update rab_type in structure 'rbe.rabs.rabs_in_msg[rab_index]'. */
      /* change argument */
      (void) rrc_update_rab_type_in_rab_info_r7(wrrc_as_id,&(rab_list_ptr->elem[rab_list_index]),
         &(rbe.rabs.rabs_in_msg[rab_index]));
    }
...
)
{
      ...
      rrc_RadioBearerSetup_r7_IEs_specificationMode_complete))
  {
    ...
    if (rbsetup_ptr->specificationMode.u.complete.m.rab_InformationSetupListPresent)
    {
      if(rrcrbe_prevalidate_rab_setup_list_r7(wrrc_as_id,&rbsetup_ptr->specificationMode.u.complete.
   ...
   /* Process RAB reconfig before RAB setup because we want to catch NW's which
    ...
    if (rbsetup_ptr->specificationMode.u.complete.m.rab_InformationReconfigListPresent)
    {
      ...
      rbe.status.failure_status = rrc_update_rab_info_reconfig_param(wrrc_as_id,
    ...
    if (rbsetup_ptr->specificationMode.u.complete.m.rab_InformationSetupListPresent)
    {
      if(rrcrbe_validate_rab_setup_list_r7(wrrc_as_id,&rbsetup_ptr->specificationMode.u.complete.
  ...
  */
  ...
  {
    /* Save the linked list head pointer */
    if (FAILURE == rrcllc_validate_rb_info_reconfig_list_r7(wrrc_as_id,
        &rbsetup_ptr->specificationMode.u.complete.rb_InformationReconfigList))
    {
      rbe.status.failure_status = RRCRB_MSG_INVALID_CONFIG;
      return;
...
)
{
  ...
  validate_rbsetup_r7(wrrc_as_id, msg_ptr);
         ...
         (rbsetup_ptr->specificationMode.u.complete.srb_InformationSetupList.n != 0))))
    {
      ...
      for (rab_index=0; rab_index < rbe.rabs.num_rabs_in_msg; rab_index++)
      {
        rab_status = rrc_find_rab_in_established_rabs
          (wrrc_as_id, rbe.rabs.rabs_in_msg[rab_index].rab_id );
        rbe.rabs.chan_config_needed[rab_index] = TRUE;
      ...
      This function will copy all RABs from the rxd msg to oc. */
      oc_status = rrcllc_set_ordered_config(wrrc_as_id, RRC_PROCEDURE_RBE,
                                             rbe.state_from_msg,
      ...
      switch ( oc_status )
      {
        case ORDERED_CONFIG_SET:
          ...
          rrcrbe_send_channel_config_req_r7(wrrc_as_id, msg_ptr );
...
)
{
  ...
  validate_rbsetup_r6(wrrc_as_id, msg_ptr);
         ...
         (rbsetup_ptr->specificationMode.u.complete.srb_InformationSetupList.n != 0))))
    {
      ...
      switch ( oc_status )
      {
        case ORDERED_CONFIG_SET:
          ...
          if((rrc_sib_is_current_event_sib_read_in_dch(wrrc_as_id)) &&
             (rrc_sib_get_proc_id_for_sib_read_in_dch(wrrc_as_id) == RRC_PROCEDURE_GPS))
          {
            WRRC_MSG0_HIGH("RRCGPS: Force Init GPS ");
            rrcsib_force_sib_event_init(wrrc_as_id, RRCSIB_ABORT_DUE_SIB_READ_IN_DCH);
          }
          ...
          /* Call the function that creates and sends a channel
          config request to LLC. */
          rrcrbe_send_channel_config_req_r6(wrrc_as_id, msg_ptr );
...
)
{
  ...
  validate_rbsetup_r5(wrrc_as_id, msg_ptr);
  ...
  else
  {
    ...
    if (RRC_MSG_COMMON_BITMASK_IE_PTR(rbsetup_ptr,rrc_RadioBearerSetup_r5_IEs,cn_InformationInfo))
    {
       ...
       */
       primary_plmn_Id_present = rrcrbe_check_and_get_primary_plmn_id(wrrc_as_id,msg_ptr,
                                                                  &primary_plmn_Identity,MSG_REL5);
      rbe.mm_cmd_ptr = rrc_ccm_buid_service_ind_with_nas_cn_info(wrrc_as_id,
                     &(rbsetup_ptr->cn_InformationInfo) 
               ...
               == TRUE))
      {
        ...
        switch ( oc_status )
        {
          case ORDERED_CONFIG_SET:
            ...
            /* Call the function that creates and sends a channel
            config request to LLC. */
            rrcrbe_send_channel_config_req_r5(wrrc_as_id, msg_ptr );
...
)
{
  ...
  if(rrccsp_is_selected_cell_utran_directed(wrrc_as_id)&&
      (rbe.new_crnti_valid)) 
  {
 ...
 /* If H-RNTI is not given in OTA then initiate cell update*/
  if(rrcenhstate_set_hrnti_status_for_transitions(wrrc_as_id))
  {
    rbe.rrcrbe_send_initiate_cu_flag = TRUE;
    rbe.rrcrbe_delete_crnti          = TRUE;
  }
 ...
 /* Check if E-RNTI is not given in OTA then initiate cell update*/
        if(rrchsrach_check_ernti_status(wrrc_as_id))
        {
          rbe.rrcrbe_send_initiate_cu_flag = TRUE;
          rbe.rrcrbe_delete_crnti          = TRUE;
        }
...
)
{
  ...
  validate_rbsetup_r9(wrrc_as_id, msg_ptr);
         ...
         )))
    {
      ...
      switch ( oc_status )
      {
        case ORDERED_CONFIG_SET:
          ...
          rrcrbe_send_channel_config_req_r9(wrrc_as_id,msg_ptr);
...
/*===========================================================================
FUNCTION   RRCRBE_SEND_CHANNEL_CONFIG_REQ_R10
...
SIDE EFFECTS
  None.
===========================================================================*/
static void rrcrbe_send_channel_config_req_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  rrc_RadioBearerSetup *msg_ptr      /* Pointer to the RB Setup message */
)
{
  ...
  rrc_RadioBearerSetup_r10_IEs* rbsetup_ptr = NULL;
  rrc_state_e_type next_rrc_state = rbe.state_from_msg;
  ...
  /* First fill up the Radio Bearers that need to be setup. For this we need to know how many RBs need to be setup.
  If RABs need to be setup, check how many RBs are mapped to RABs. */
  if(RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,srb_InformationSetupList))
  {
    /* Get the SRB setup list */
    srb_setup_ptr = &rbsetup_ptr->srb_InformationSetupList;
  ...
  if(RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,rab_InformationSetupList))
  {
    rab_info_ptr = &rbsetup_ptr->rab_InformationSetupList;
    for (rab_index=0; rab_index < rbe.rabs.num_rabs_in_msg && rab_info_ptr->n > rab_index; rab_index++)
    {
      if ( rbe.rabs.chan_config_needed[rab_index] == TRUE)
      {
        ...
        if ( rb_ptr->n == 0 )
        {
          WRRC_MSG0_HIGH("REL10:No RBs present in msg!");
        }
        else
        {
          /* Get the RBE db rab index */
  ...
  /* Now check if the message contains an RB information to be
     reconfig list. If so, we need to reconfigure some RBs */
  if(RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,rb_InformationReconfigList))
  {
    uint32 local_idx=0;
    ...
    while ( reconfig_rb_ptr->n > local_idx )
    {
      ...
      if ( rb_count >= MAX_RB )
      {
        WRRC_MSG0_ERROR("REL10:MAX RB count exceeded!");
        break;
      }
    } /* end while */
  }
  ...
  /* Now check if the message contains an RB information to be
   affected list. If so, we need to reconfigure some RBs */
  if(RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,rb_InformationAffectedList))
  {
   uint32 local_idx=0;
    ...
    if ( affected_rb_ptr->n == 0 )
    {
      WRRC_MSG0_HIGH("REL10:No affected RBs in msg!");
    }
    else
    {
      while ( affected_rb_ptr->n > local_idx )
      {
        ...
        if ( rb_count >= MAX_RB )
        {
          WRRC_MSG0_HIGH("REL10:MAX RB count exceeded!");
          break;
        }
      } /* end while */
    }
  ...
  else
  {
    ...
    if (((rrc_get_state(wrrc_as_id))==RRC_STATE_CELL_FACH)&&
        (rbe.state_from_msg==RRC_STATE_CELL_FACH))
    {
      if((RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,frequencyInfo)) &&
          (rbsetup_ptr->frequencyInfo.modeSpecificInfo.t == T_rrc_FrequencyInfo_modeSpecificInfo_fdd))
      {
        directed_cell_info.freq_info_present = TRUE;
        directed_cell_info.uarfcn_dl = rbsetup_ptr->frequencyInfo.modeSpecificInfo.u.fdd->uarfcn_DL;
      ...
      if ((RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,dl_InformationPerRL_List)) &&
          (rbsetup_ptr->dl_InformationPerRL_List.elem[0].modeSpecificInfo.t == 
          T_rrc_DL_InformationPerRL_r8_modeSpecificInfo_fdd))
      {
        directed_cell_info.pscr_present = TRUE;
      ...
      if (rrcrb_validate_directed_cell_info (wrrc_as_id,directed_cell_info) == FALSE)
      {
        ...
        {
          rrc_int_c_rnti_type      c_rnti;       /* Local C_RNTI value */
          if(RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,new_C_RNTI))
          {
            rbe.new_crnti_valid = TRUE;
            rrc_translate_crnti(wrrc_as_id,&rbsetup_ptr->new_C_RNTI,
                                &c_rnti);
...
/*===========================================================================
FUNCTION   VALIDATE_RBSETUP_R10

DESCRIPTION
  This function validates the received R10 RB Setup Message. This includes
  checking for unsupported or invalid configurations, unknown extensions
  and invalid IEs.
...
SIDE EFFECTS
  None.
===========================================================================*/
static void validate_rbsetup_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  rrc_RadioBearerSetup *msg_ptr      /* Pointer to the RB Setup message */
)
{
  ...
  rbsetup_ptr = &msg_ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.u.criticalExtensions.
    u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.radioBearerSetup_r10;
  ...
  switch (rbsetup_ptr->rrc_StateIndicator)
  {
    ...
    default: /* Just to satisfy LINT */
      WRRC_MSG0_ERROR("REL10:Invalid RRC state from Msg");
      rbe.status.failure_status = RRCRB_MSG_UNSUPPORTED_CONFIG;
      break;
  ...
  if(RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,dl_CounterSynchronisationInfo))
  {
    /*check whether the SRNS relocation is within Cell_DCH or not.
      SRNS relocation information can come in reconfiguration messages
      only when the state transition is Cell_DCH->Cell_DCH */
    if (!((rbe.state_from_msg == RRC_STATE_CELL_DCH) &&
          ( rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH)))
    {
      WRRC_MSG2_ERROR("REL10:SRNS reloc with invalid states cur_state %d, msg_state %d",
          rrc_get_state(wrrc_as_id), rbe.state_from_msg);
      rbe.status.failure_status=RRCRB_MSG_INVALID_CONFIG;
      return;
    }
    ...
    if ((RRC_MSG_COMMON_BITMASK_IE_TYPE2(rbsetup_ptr->dl_CounterSynchronisationInfo,
        ...
        rrc_DL_CounterSynchronisationInfo_r5,rb_PDCPContextRelocationList)))
    {
      WRRC_MSG0_HIGH("REL10:PDCP info in DL counter sync info not supported");
      rbe.status.failure_status=RRCRB_MSG_UNSUPPORTED_CONFIG;
      return;
    }
    else
    {
      ...
      rbe.rbe_for_srns_relocation = TRUE;
      /* store if new U-RNTI is present in the message */
      if (RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,new_U_RNTI))
      {
        rbe.new_urnti_valid = TRUE;
        rrc_translate_urnti (wrrc_as_id, &(rbsetup_ptr->new_U_RNTI), &rbe.new_urnti);
      }
  ...
  /* Process RAB reconfig before RAB setup because we want to catch NW's which
    are trying to setup and reconfig RAB with the same message */
  if(RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,rab_InformationReconfigList))
  {
  ...
  WRRC_MSG1_MED("REL10:%d RABs  in an RBSetup",  rbe.rabs.num_rabs_in_msg);
  ...
  {
    //check to see if dl-dpch-common info is present while going to dch
    if((RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,dl_CommonInformation)) && 
       (RRC_MSG_COMMON_BITMASK_IE(rbsetup_ptr->dl_CommonInformation,
       rrc_DL_CommonInformation_r10,dl_dpchInfoCommon)) &&
       (T_rrc_DL_CommonInformation_r10_dl_dpchInfoCommon_dl_DPCH_InfoCommon
       == rbsetup_ptr->dl_CommonInformation.dl_dpchInfoCommon.t))
    {
      //check if ciphering is active in mac for cs domain, if it is then,
      //start 2 tiered HHO procedure STEP_1 and STEP_2 for rl-re-establishment
      if(rrcsmc_tm_ciph_active(wrrc_as_id) == TRUE)
      {
        uint32 mac_d_hfn_l=RRCSMC_INVALID_HFN_VALUE;
        uecomdef_status_e_type loc_ret_val;
        WRRC_MSG0_HIGH("REL10:To-DCH-Step-1 config activated");
        loc_ret_val = rrcrb_get_mac_d_hfn(wrrc_as_id,
             ( RRC_MSG_COMMON_BITMASK_IE_PTR(rbsetup_ptr->dl_CommonInformation.dl_dpchInfoCommon.u.
               dl_DPCH_InfoCommon,rrc_DL_DPCH_InfoCommon_r6,mac_d_HFN_initial_value)),
              &rbsetup_ptr->dl_CommonInformation.dl_dpchInfoCommon.u.
        ...
        if (loc_ret_val == FAILURE)
        {
          WRRC_MSG0_HIGH("REL10:get_mac_d_hfn failed");
        }
        if(rrcsmc_process_cipher_config_for_hho(wrrc_as_id,mac_d_hfn_l) == FAILURE)
        {
           WRRC_MSG0_HIGH("REL10:SMC process ciphering config for HHO failed");
        }
        rbe.trans_to_dch_ciph_config_needed = TRUE;
      }
      else
      {
        WRRC_MSG0_HIGH("REL10:To-DCH-Step-1 not needed");
      }
    }
  }
    ...
    if(rbe.state_from_msg == RRC_STATE_CELL_DCH)
    {
      ...
      if(rbe.first_tm_rb_setup == TRUE)
      {
        //check to see if dl-dpch-common info is present while going to dch
        if((RRC_MSG_COMMON_BITMASK_IE_PTR(rbsetup_ptr,
           rrc_RadioBearerSetup_r10_IEs,dl_CommonInformation)) && 
           (RRC_MSG_COMMON_BITMASK_IE(rbsetup_ptr->dl_CommonInformation,
           rrc_DL_CommonInformation_r10,dl_dpchInfoCommon)) &&
           (T_rrc_DL_CommonInformation_r10_dl_dpchInfoCommon_dl_DPCH_InfoCommon == 
            rbsetup_ptr->dl_CommonInformation.dl_dpchInfoCommon.t))
      ...
      (rbe.state_from_msg  == RRC_STATE_URA_PCH))
  {
    ...
    if (!(RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,utran_DRX_CycleLengthCoeff)))
    {
      WRRC_MSG1_ERROR("REL10:UTRAN DRX Coef not preset, State:%d", rbe.state_from_msg);
      rbe.status.failure_status = RRCRB_MSG_INVALID_CONFIG;
      return;
    }
  }
  ...
  {
    /*should we do it after getting l2ack for response??*/
    if (!(RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,ura_Identity)))
    {
      WRRC_MSG0_HIGH("REL10:URA ID not present");
      rrccu_clear_ura_id (wrrc_as_id);
    }
    else
    {
      ...
      {
        WRRC_MSG1_ERROR("REL10:Invalid URA ID length%d", rbsetup_ptr->ura_Identity.numbits);
        rbe.status.failure_status = RRCRB_MSG_PROTOCOL_ERROR;
        rbe.status.prot_err_cause = rrc_ProtocolErrorCause_asn1_ViolationOrEncodingError;
        return;
      }
...
/*===========================================================================
FUNCTION   process_rbsetup_r10

DESCRIPTION
  This function processes the Rel 10 RB Setup Message. It first
  validates the message. If the message has an invalid or unsupported
  configuration, a Radio Bearer Setup Failure message will be sent to
  the UTRAN. If the message is valid, it will be processed as follows -
...
SIDE EFFECTS
  None.
===========================================================================*/
static void process_rbsetup_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  rrc_cmd_type *cmd_ptr
)
{
  ...
  boolean ordered_config_err_occured = FALSE; /* Used to indicate if an error
                                                 occured while setting ordered_config. */
  rrc_msg_ver_enum_type msg_version = MSG_REL10;
  ...
  WRRC_MSG1_MED("REL10:RRC cur_state %d", rbe.curr_substate);
  ...
  /* Store Msg Ptr in a local variable */
  rbsetup_ptr = &msg_ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.
    u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10.radioBearerSetup_r10;
  ...
  if (TRUE == rrcrbe_validate_srns_relocation_info(wrrc_as_id,msg_ptr, msg_version))
  {
    ...
    case RRCSMC_SRNS_INTEGRITY_CHECK_FAILURE:
      /*Discard the message and continue with ongoing procedure as if this
        message was not received */
      WRRC_MSG0_ERROR("REL10:IP check failed. Discarding RBS msg");
      break;
    ...
    case RRCSMC_INCOMPATIBLE_SECURITY_RECONFIGURATION:
      if (msg_ptr->u.later_than_r3.rrc_TransactionIdentifier
          == rbe.tr_ids.rejected_transaction)
      {
        WRRC_MSG1_HIGH("REL10:Duplicate RBE msg - rejected tr-id%d match!",
                 rbe.tr_ids.rejected_transaction);
        /* Nothing else to do, just ignore the message and return */
      }
      else
  ...
  if ((rrcllc_get_ordered_config_status(wrrc_as_id) != OC_NOT_SET) ||
      ...
      (RRCCU_STARTED == rrccu_get_cell_update_started_status(wrrc_as_id) ))
  {
    WRRC_MSG0_HIGH("REL10:Incomatible simultaneous reconfig! Rejecting RBS message");
    if ( (msg_ptr->u.later_than_r3.rrc_TransactionIdentifier)
         == rbe.tr_ids.accepted_transaction )
    {
      WRRC_MSG1_HIGH("REL10:Duplicate RBS msg! accepted tr-id%d match!",
               rbe.tr_ids.accepted_transaction);
      /* Nothing else to do, just return */
    }
    else if ( (msg_ptr->u.later_than_r3.rrc_TransactionIdentifier)
                == rbe.tr_ids.rejected_transaction )
    {
      WRRC_MSG1_HIGH("REL10:Duplicate RBS msg! rejected tr-id%d match!", rbe.tr_ids.rejected_transaction);
      /* Nothing else to do, just return */
    }
    else
    {
  ...
  validate_rbsetup_r10(wrrc_as_id, msg_ptr);

  if ( rbe.status.failure_status != RRCRB_MSG_VALID )
  {
    /* RB Message was invalid. */
    WRRC_MSG0_HIGH("REL10:RB Message validation failed");
    /* Send the RB Setup Failure Message */
    send_rbs_failure_message( wrrc_as_id, rbe.status.failure_status, rbe.status.prot_err_cause);
  ...
  else
  {
    ...
    if (rrcsmc_is_ciphering_enabled(wrrc_as_id) == RRCSMC_SECURITY_STARTED)
    {
      if(RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,activationTime))
      {
        /*256 is the CFN rollover length*/
        /*250 is used to activate ciphering after 250 10ms time interval,
        this should give sufficient time for n/w side and ue side to have
    ...
    /* Check if there is CN information present. */
    if(RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,cn_InformationInfo))
    {
      rbe.mm_cmd_ptr = rrc_ccm_buid_service_ind_with_nas_cn_info_r6(wrrc_as_id,
        &(rbsetup_ptr->cn_InformationInfo) );
      if (rbe.mm_cmd_ptr == NULL)
      {
        WRRC_MSG0_ERROR("REL10:Could not build SERVICE_IND cmd");
      }
      else
      {
        /* set cn_info_saved flag */
    ...
    /*If RBE OTA provides "supportForChangeOfUE_Capability" IE, we need to store it in
           "rrc_ordered_rnc_cap_change_support" so that later once the procedure is over successfully, global
          "rrc_rnc_cap_change_support" can be updated accordingly. */  	
    if(RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,supportForChangeOfUE_Capability))
    {
      if(rbsetup_ptr->supportForChangeOfUE_Capability == TRUE)
      {
        rrccouec.rrc_ordered_rnc_cap_change_support = RNC_CAP_CHANGE_SUPPORTED;
    ...
    if ((RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,rab_InformationSetupList)) ||
        (((RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,rb_InformationAffectedList))  
          ) ||
        ((RRCRB_R10_MSG_IE_PRESENT(rbsetup_ptr,srb_InformationSetupList)) 
         )))
    {
      /* Check if the RAB id is already in ESTABLISHED_RABS */
      for (rab_index=0; rab_index < rbe.rabs.num_rabs_in_msg; rab_index++)
      {
        ...
        if ((rab_status != RRC_RAB_NOT_FOUND) &&
             (rrc_find_if_voice_rab_exists_in_established_rabs(wrrc_as_id, rbe.rabs.rabs_in_msg[rab_index].rab_id )
               == FALSE))
        {
          WRRC_MSG1_HIGH("REL10:RAB %d already exists in EST_RABS!",
                   rbe.rabs.rabs_in_msg[rab_index].rab_id);
         /* send a failure message if the rab in the message  already
           exists in   established_rabs. Later add a check for the rbs
           associated with the rabs so that the failure will be sent only when
      ...
      switch ( oc_status )
      {
        case ORDERED_CONFIG_SET:
          if(rrc_sib_is_current_event_sib_read_in_dch(wrrc_as_id))
          {
             WRRC_MSG0_HIGH("REL10:RRCGPS: Force Init GPS ");
             rrcsib_force_sib_event_init(wrrc_as_id, RRCSIB_ABORT_DUE_SIB_READ_IN_DCH);
          }
          ...
          WRRC_MSG0_HIGH("REL10:Sending RRC_CHANNEL_CONFIG_REQ to LLC");
          rrcrbe_send_channel_config_req_r10(wrrc_as_id,msg_ptr);
        ...
        case ORDERED_CONFIG_CONFIGURATION_NOT_SUPPORTED:
          ...
          case ORDERED_CONFIG_CONFIGURATION_INVALID:
           ...
           break;
        case ORDERED_CONFIG_NOT_SET_OTHER:
          WRRC_MSG0_HIGH("REL10:LLC returned other failure!");
          /* When LLC sends this, there is a serious problem
          and we'll go back to disconnected state. No need
          to send the failure message here, we can just
          go back to initial substate */
        ...
        default:
          ...
          WRRC_MSG0_ERROR("REL10:Unknown return value from LLC");
          /* Unknown error, just go back to initial substate */
          rbe.curr_substate = RRCRB_INITIAL;
    ...
    else
    {
      /*If we are here, it means that RABs are not given in RB-Setup.
      Also, SRBs and RB Affected IEs are not given.*/
      WRRC_MSG0_ERROR("REL10:No RB IEs are given in the OTA.");
      rbe.status.failure_status= RRCRB_MSG_INVALID_CONFIG;
      rbe.status.prot_err_cause = rrc_ProtocolErrorCause_informationElementMissing;  
...
}/* end function process_rbsetup_message_r10 */
...
)
{
    ...
    if (rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5)
    {
      ...
      if ((msg_ptr->u.later_than_r3.criticalExtensions.t ==
          ...
          (msg_ptr->u.later_than_r3.criticalExtensions.u.criticalExtensions.t ==
           T_rrc_RadioBearerSetup_criticalExtensions_4_r5))
      {
        process_rbsetup_r5(wrrc_as_id, cmd_ptr);
        return;
      }
      ...
      {
        process_rbsetup_r6(wrrc_as_id, cmd_ptr);
        return;
      }
      /* Check if Rel 6 contents are present */
      ...
      {
        process_rbsetup_r7(wrrc_as_id, cmd_ptr);
        return;
      }
      /* Check if Rel 8 contents are present */
        ...
        (RRCRB_R8_CRITICAL_EXT_PRESENT_PTR(
                  ...
                  rrc_RadioBearerSetup)))
      {
        process_rbsetup_r8(wrrc_as_id, cmd_ptr);
        return;
      }
      ...
      /* Check if Rel 9 contents are present */
        ...
        (RRCRB_R9_CRITICAL_EXT_PRESENT_PTR(
                  ...
                  rrc_RadioBearerSetup)))
      {
        process_rbsetup_r9(wrrc_as_id, cmd_ptr);
        return;
      }
...
#ifdef FEATURE_WCDMA_REL10
      /* Check if Rel 10 contents are present */
      else if ((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL10) &&
      (RRCRB_R10_CRITICAL_EXT_PRESENT_PTR(msg_ptr, 
      rrc_RadioBearerSetup)))
      {
        process_rbsetup_r10(wrrc_as_id, cmd_ptr);
        return;
      }
      ...
      else
      {
        ...
        rrcrbe_send_rbsetup_failure_message_ext_not_comprehended(wrrc_as_id,msg_ptr);
  ...
  if (TRUE == rrcrbe_validate_srns_relocation_info(wrrc_as_id,msg_ptr, MSG_REL99))
  {
    ip_check_status = rrcsmc_check_integrity_for_srns_relocation(wrrc_as_id,cmd_ptr, MSG_REL99);
      ...
      (RRCCU_STARTED == rrccu_get_cell_update_started_status(wrrc_as_id) ))
  {
    ...
    return;
         ...
         (rrc_buffer_ota_info.transaction_id != RRCRB_INVALID_TRANSACTION_ID)
          &&
         (rrc_buffer_ota_info.transaction_id == msg_ptr->u.r3.radioBearerSetup_r3.rrc_TransactionIdentifier))
  {
    WRRC_MSG0_ERROR("BUF_OTA:Received Duplicate RBS, Ignoring");
    return ;
  }
  ...
  message */
  validate_rbsetup_message( wrrc_as_id, msg_ptr );
               ...
               == TRUE))
      {
        ...
        switch ( oc_status )
        {
          case ORDERED_CONFIG_SET:
            ...
            /* Call the function that creates and sends a channel
            config request to LLC. */
            rrcrbe_send_channel_config_req(wrrc_as_id, msg_ptr );
...
)
{
  ...
  curr_state = rrc_get_state(wrrc_as_id);
       ...
       (rbe.state_from_msg == RRC_STATE_URA_PCH)))
  {
   /*send the response message now */
    send_rbs_complete_message(wrrc_as_id, TRUE);
    /*wait for L2ack */
...
)
{
  ...
  uint32 current_mode = rrc_get_current_codec_mode(wrrc_as_id);
  ...
  if ( ptr->chan_cfg_status != RRCLLC_CHAN_CFG_SUCCESS )
  {
    ...
    rrc_initialize_rlc_size_change(wrrc_as_id);

    rrc_set_ordered_codec_mode(wrrc_as_id,current_mode);

    /* Roll back mac_rab_status */
    rrcrb_update_mac_rab_status(wrrc_as_id,RRC_PROCEDURE_RBE, RRC_CS_DOMAIN_CN_ID, NULL,TRUE);
    ...
    if (ptr->chan_cfg_status == RRCLLC_CHAN_CFG_FAILURE_OTHER)
    {
       ...
       */
      if(rrccu_return_cu_substate(wrrc_as_id) == RRCCU_WAIT_FOR_PENDING_CONFIG)
      {
        rbe.curr_substate = RRCRB_WAIT_FOR_CELL_UPDATE_CNF;
        rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_RBE, RRC_UNRECOVERABLE_ERROR, FALSE);
        return;
      }
      ...
      configurations */
       (void)rrcllc_reset_ordered_config(wrrc_as_id,RRC_PROCEDURE_RBE,TRUE);
      ...
      if(rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH && rbe.state_from_msg == RRC_STATE_CELL_DCH)
      {
        if((rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_T_305_TIMER) == 0)
            && (rrc_convert_t305_to_ms(wrrc_as_id,RRC_GET_T305()) != RRCTMR_INFINITY_VAL ))
        {
          WRRC_MSG0_HIGH("Chan_config_req failed for FACH->DCH. So Starting T305 with zero timeout.");        
          rrctmr_start_timer(wrrc_as_id,RRCTMR_T_305_TIMER, 0);
        }
      }	  
      return;
    } /* if (ptr->chan_cfg_status == RRCLLC_CHAN_CFG_FAILURE_OTHER) */
    ...
    rrcsmc_update_ciphering_config_info_for_signalling_release(wrrc_as_id, RRC_CS_DOMAIN_CN_ID);
    ...
    if (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH && rbe.state_from_msg == RRC_STATE_CELL_DCH)
    {
      ...
      rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_RBE, RRC_PHY_CHAN_FAILURE,TRUE);
    ...
    else if (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH && rbe.state_from_msg == RRC_STATE_CELL_DCH)
    {
      ...
      */
      ...
      {
        rbe.curr_substate = RRCRB_WAIT_FOR_CELL_UPDATE_CNF;
        rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_RBE, RRC_RADIO_LINK_FAILURE, FALSE);
        return;
      }
      ...
      else if (ptr->chan_cfg_status == RRCLLC_CHAN_CFG_FAILURE_BACK_TO_OLD_CONFIG)
      {
        ...
        /* Send RRC_CELL_UPDATE_INITIATE_REQ command to CELL UPDATE Proc */
        rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_RBE, RRC_RADIO_LINK_FAILURE,TRUE);
        /* Wait for CELL UPDATE to get a valid C_RNTI */
        rbe.curr_substate = RRCRB_WAIT_FOR_CELL_UPDATE_CNF;
        rbe.rrcrbe_send_initiate_cu_flag = FALSE;
        rbe.rrcrbe_delete_crnti          = FALSE;
  ...
  if(rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH && rbe.state_from_msg == RRC_STATE_CELL_DCH)
  {
    rrctmr_stop_timer(wrrc_as_id, RRCTMR_T_305_TIMER);
  }  
  ...
  for (rab_index=0; rab_index < rbe.rabs.num_rabs_in_msg; rab_index++)
  {
   ...
   /* Now we need to update the UE Variable ESTABLISHED_RABS.
      ...
      if ( (rrc_find_rab_in_established_rabs(wrrc_as_id, rbe.rabs.rabs_in_msg[rab_index].rab_id ))
           == RRC_RAB_NOT_FOUND )
      {
        /* We can add this RAB. */
        ...
        rrc_add_rab_to_established_rabs(wrrc_as_id, &(rbe.rabs.rabs_in_msg[rab_index]));
      }
      else
      {
        /*We are here. It implies that the RAB was already present in rrc_est_rabs.*/
        WRRC_MSG0_HIGH("RAB already in established RABs");
        if((rrc_find_if_voice_rab_exists_in_established_rabs(wrrc_as_id,rbe.rabs.rabs_in_msg[rab_index].rab_id) == TRUE))

        {
          existing_rab_updated = TRUE;
          rrc_update_rb_info_to_existing_rab(wrrc_as_id,&rbe.rabs.rabs_in_msg[rab_index]);
...
#error code not present
...
#endif /*FEATURE_SCUDIF_ENHANCEMENT*/
      {
        ...
        if ( rbe.rabs.rabs_in_msg[rab_index].rab_type == RRCRB_CS_VOICE_RAB )
        {
          if(rrc_get_idx_in_rrc_est_rabs_for_rab_id(wrrc_as_id,rbe.rabs.rabs_in_msg[rab_index].rab_id) 
                >= MAX_RAB_TO_SETUP) return; /*This to keep the legacy behaviour*/
            if((existing_rab_updated) && (rbe.rabs.rabs_in_msg[rab_index].nas_Synchronisation_Indicator!=current_mode))
            {
              rrc_set_current_codec_mode(wrrc_as_id,rbe.rabs.rabs_in_msg[rab_index].nas_Synchronisation_Indicator);
              current_mode = rbe.rabs.rabs_in_msg[rab_index].nas_Synchronisation_Indicator;
            rrc_change_codec_configuration(wrrc_as_id);
            }
            else
            {
              rrc_set_current_codec_mode(wrrc_as_id,rbe.rabs.rabs_in_msg[rab_index].nas_Synchronisation_Indicator);
            ...
            WRRC_MSG6_HIGH("WV_WRRC: configured codec_mode: %d, scr_mode: %d, ul_amr_a_id: %d, ul_amr_b_id:5d, ul_amr_c_id:%d, codec_mode_type: %d",
          ...
          WRRC_MSG0_MED("WV_WRRC:Updating WV Adapter for voice RAB");
        }
      }
  ...
  {
    rrcrb_update_amr_cctrch_id_for_reconfig(wrrc_as_id,current_mode);
  }
  ...
  if ((RRC_STATE_CELL_FACH == curr_state) &&
      ((rbe.state_from_msg == RRC_STATE_CELL_FACH) ||
       ...
       (rbe.state_from_msg == RRC_STATE_URA_PCH)))
  {
    if(rbe.rrcrbe_send_initiate_cu_flag 
       || (rrccu_return_cu_substate(wrrc_as_id) == RRCCU_WAIT_FOR_RB_PROC_COMPL_IND) 
       )
    {
      if (rbe.rrcrbe_delete_crnti)
      {
        ...
        rrcrb_send_rnti_update_req(wrrc_as_id,rnti_update_info);
        rbe.new_crnti_valid = FALSE;
      }
      /* Send RRC_CELL_UPDATE_INITIATE_REQ command to CELL UPDATE Proc */
      rrcrbe_send_cell_update_initiate_req(wrrc_as_id, RRC_PROCEDURE_RBE);
      /* Wait for CELL UPDATE to get a valid C_RNTI */
      rbe.status.failure_status = RRCRB_MSG_VALID;
      rbe.curr_substate = RRCRB_WAIT_FOR_CELL_UPDATE_CNF;
      rbe.rrcrbe_send_initiate_cu_flag = FALSE;
    ...
    else if((rrccu_return_cu_substate(wrrc_as_id) == RRCCU_WAIT_FOR_PENDING_CONFIG) &&
      (rrccu_get_cell_update_started_status_with_cause(wrrc_as_id,&cu_cause) == RRCCU_STARTED) && 
      (cu_cause == rrc_CellUpdateCause_rlc_unrecoverableError) && (rrccu_get_rb_id_unrec_error(wrrc_as_id) == 
      DCCH_AM_RADIO_BEARER_ID))
    {
      ...
      /* Send RRC_CELL_UPDATE_INITIATE_REQ command to CELL UPDATE Proc */
      rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_RBE,
    ...
    {
      /* Send a RNTI Update Request */
      rrcrbe_send_crnti_update_req(wrrc_as_id,rbe.new_crnti);
      /* Send a Radio Bearer Setup Complete later */
    }

    /* Query for valid C-RNTI */
    else if (C_RNTI_NOT_VALID == rrcllc_get_current_crnti_status(wrrc_as_id) )
    {
  ...
  if (rbe.rbe_for_srns_relocation == TRUE)
  {
    if (RRC_STATE_CELL_DCH == curr_state)
    {
      ...
      rrcrb_send_rlc_re_est_req_to_smc(wrrc_as_id,RRC_PROCEDURE_RBE, rb_type, TRUE);
  ...
  else /*normal reconfiguration without SRNS relocation */
  {
    ...
    else
    {
     ...
     2> include the IE "Reconfiguration Status Indicator" and set it to TRUE.*/
        ...
        && (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH))
      {
        ...
        (void) rex_clr_sigs(rex_self(), RRCRBE_TEMP_TIMER_SIG);
        (void) rex_set_timer(&rbe.rrcrbe_temp_timer, RRCRB_PND_CFG_DELAY);
        rbe.rrcrbe_sig = rex_wait(RRCRBE_TEMP_TIMER_SIG);
        MSG_LOW("RRCRBE signal:%d", rbe.rrcrbe_sig,0, 0);
        (void) rex_clr_sigs(rex_self(), RRCRBE_TEMP_TIMER_SIG);
        rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_RBE, RRC_UNRECOVERABLE_ERROR, FALSE);
      }
      ...
      (rrccu_get_cell_update_started_status_with_cause(wrrc_as_id,&cu_cause) == RRCCU_STARTED))
      {
        if((cu_cause == rrc_CellUpdateCause_rlc_unrecoverableError) && (rrccu_get_rb_id_unrec_error(wrrc_as_id) != DCCH_AM_RADIO_BEARER_ID))
        {
          /* Send RRC_CELL_UPDATE_INITIATE_REQ command to CELL UPDATE Proc */
          rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_RBE,
                                              RRC_UNRECOVERABLE_ERROR,
    ...
    rrcrbe_send_indication_to_upper_layers(wrrc_as_id);

    if (rrcllc_get_ordered_config_status_wo_f3(wrrc_as_id) != OC_SET_FOR_DCH_FACH_TRANS)
    {
      /* prints rate for each transport channel */
      rrcllc_print_trans_chnl_rate(wrrc_as_id);
    }
  ...
  else if (ptr->new_state == RRC_STATE_CELL_FACH)
  {
    ...
    if(ptr->previous_state == RRC_STATE_CELL_DCH)
    {
      ...
      else
      {
         ...
         * LLC 
         */
        if (rrccu_get_cell_update_started_status_with_cause(wrrc_as_id,&cu_cause)== RRCCU_STARTED &&
...
)
{
  ...
  if ( ptr->status == SUCCESS )
  {
    ...
    {
      if (RRC_STATE_CELL_DCH == rrc_get_state(wrrc_as_id))
      {
        ...
        rrcrb_send_rlc_re_est_req_to_smc(wrrc_as_id,RRC_PROCEDURE_RBE, rb_type, FALSE);
      ...
      {
        WRRC_MSG1_ERROR("SRNS reloc flag is ON in rrc state %d", rrc_get_state(wrrc_as_id));
      }
    }
  }
  else
  {
    ...
    if (RRC_STATE_CELL_FACH == rrc_get_state(wrrc_as_id) &&
        RRCCU_STARTED == rrccu_get_cell_update_started_status_with_cause(wrrc_as_id,&cu_cause) &&
        ...
        (rbe.state_from_msg == RRC_STATE_URA_PCH))
    {
      ...
      rrc_transition_to_disconnected_state(wrrc_as_id,RRC_PROCEDURE_RBE,RRC_TX_TO_DISCON_OTHER);
    }
  }
  ...
  if (rbe.rbe_for_srns_relocation == TRUE)
  {
    rrcdormancy_update_dormancy_for_srns_hho_cco(wrrc_as_id, RRC_PROCEDURE_RBE);
  }
...
)
{
  ...
  case RRC_DOWNLINK_SDU_IND:
    ...
    process_rbsetup_message(wrrc_as_id,  cmd_ptr );
    break;
...
)
{
  ...
  case RRC_RE_EST_RLC_FOR_SRNS_CNF:
    ...
    rrcrbe_process_rlc_reestablishment_cnf(wrrc_as_id  );
    break;

  case RRC_STATE_CHANGE_IND:

    /* Indication that RRC's state has changed */
    rrcrbe_process_state_change_ind(wrrc_as_id, &(cmd_ptr->cmd.state_change_ind) );
    break;
...
)
{
  ...
  case RRC_CHANNEL_CONFIG_CNF:
    ...
    rrcrbe_process_channel_config_cnf(wrrc_as_id, &(cmd_ptr->cmd.chan_config_cnf) );
    break;
  ...
  switch ( cmd_ptr->cmd_hdr.cmd_id )
  {
    case RRC_CELL_UPDATE_COMPLETE_CNF:
      ...
      else if(rbe.status.failure_status == RRCRB_MSG_VALID)
      {
        ...
        {
          send_rbs_complete_message(wrrc_as_id, FALSE);
        }
        /* send rbs indication to upperlayers */
        rrcrbe_send_indication_to_upper_layers(wrrc_as_id);
        /* prints rate for each transport channel */
  ...
  switch ( cmd_ptr->cmd_hdr.cmd_id )
  {
    case RRC_CELL_UPDATE_COMPLETE_CNF:
      ...
      else if(rbe.status.failure_status == RRCRB_MSG_VALID)
      {
        ...
        if (rrcllc_get_toc_usage(wrrc_as_id) ==TOC_FOR_OOS )
        {
          WRRC_MSG1_HIGH("DEBUG: Send channel config req for OOS scenarios TOC usage %d ",rrcllc_get_toc_usage(wrrc_as_id));
...
)
{
  ...
  case RRC_NEW_CELL_IND:
    rrcrbe_process_new_cell_ind(wrrc_as_id, &(cmd_ptr->cmd.new_cell_ind) );
    break;
    ...
    if(rbe.state_from_msg == RRC_STATE_CELL_FACH )
    {
      ...
      */
      ...
      if(chan_conf_needed == TRUE)
      {
        ...
        /* Also reset the rbe.rrcrbe_send_initiate_cu_flag to false as cell update
...
)
{
  ...
  case RRC_DOWNLINK_L2ACK_IND:
    ...
    /* Confirmation from LLC for the channel configurations */
    rrcrbe_process_l2_ack_ind(wrrc_as_id, &(cmd_ptr->cmd.downlink_l2ack_ind) );
    break;
...
)
{
  ...
  case RRC_RNTI_UPDATE_CNF:
    ...
    {
      rrcrbe_process_rnti_update_cnf(wrrc_as_id);
    }
    else
    {
...
)
{
      ...
      (rbe.curr_substate != RRCRB_WAIT_FOR_L2_ACK))
  {
    ...
    WRRC_MSG4_HIGH("RRCRBE:L2ack %d <0-Success 1-Failure> for MUI:%d, Lc_Id:%d, rbe_subst:%d",
...
)
{
      ...
      (rbe.curr_substate != RRCRB_WAIT_FOR_L2_ACK))
  {
    ...
    then just print the status */
      ...
      WRRC_MSG4_HIGH("L2ack %d <0-Success 1-Failure> for MUI:%d, Lc_Id:%d, rbe_subst:%d",
               cmd_ptr->cmd.downlink_l2ack_ind.status,
               cmd_ptr->cmd.downlink_l2ack_ind.mui,
               cmd_ptr->cmd.downlink_l2ack_ind.lc_id, rbe.curr_substate);
  ...
  case RRCRB_WAIT_FOR_CELL_UPDATE_CNF:
    ...
    rrcrbe_fach_wfcucnf_event_handler(wrrc_as_id,  cmd_ptr);
    break;

  case RRCRB_WAIT_FOR_CELL_SELECTION_CNF:
    rrcrbe_fach_wfcscnf_event_handler(wrrc_as_id,  cmd_ptr);
    break;

  case RRCRB_WAIT_FOR_CHAN_CFG_CNF:
    /* Call the event handler for RRCRBE_WAIT_FOR_CHAN_CFG_CNF substate */
    rrcrbe_fach_wfchcnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
    break;

  case RRCRB_WAIT_FOR_CRNTI_UPDATE_CNF:
    /* Call the event handler for RRCRBE_WAIT_FOR_CRNTI_UPDATE_CNF substate */
    rrcrbe_fach_wfcrntiupdatecnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
    break;

  case RRCRB_WAIT_FOR_L2_ACK:
    /* Call the event handler for RRCRBE_WAIT_FOR_L2_ACK substate */
    rrcrbe_fach_wfl2ack_substate_event_handler(wrrc_as_id,  cmd_ptr );
    break;
...
)
{
  ...
  case RRCRB_WAIT_FOR_CELL_UPDATE_CNF:
    /* Call the event handler for RRCRBE_WAIT_FOR_CELL_UPDATE_CNF substate */
    rrcrbe_pch_wfcucnf_event_handler(wrrc_as_id,  cmd_ptr);
    break;

  case RRCRB_WAIT_FOR_CELL_SELECTION_CNF:

    rrcrbe_pch_wfcscnf_event_handler(wrrc_as_id,  cmd_ptr);
    break;

  case RRCRB_WAIT_FOR_CHAN_CFG_CNF:

    /* Call the event handler for RRCRBE_WAIT_FOR_CHAN_CFG_CNF substate */
    rrcrbe_pch_wfchcnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
    break;
  ...
  default:
    ...
    MSG_MED("Command %x not processed in substate%d rrc_state%d",
            cmd_ptr->cmd_hdr.cmd_id, rbe.curr_substate, rrc_get_state(wrrc_as_id));
    break;
...
)
{
  ...
  case RRCRB_WAIT_FOR_CHAN_CFG_CNF:
    ...
    rrcrbe_other_wfchcnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
    break;

  case RRCRB_WAIT_FOR_L2_ACK:

    /* Call the event handler for WAIT_FOR_L2_ACK substate */
    rrcrbe_other_wfl2ack_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;

    case RRCRB_WAIT_FOR_CELL_UPDATE_CNF:
      rrcrbe_other_wfcucnf_substate_event_handler(wrrc_as_id,  cmd_ptr);
      break;

    case RRCRB_WAIT_FOR_CELL_SELECTION_CNF:
      rrcrbe_other_wfcscnf_event_handler(wrrc_as_id,  cmd_ptr);
      break;
...
)
{
  ...
  rrc_crash_debug_save_proc_substates(wrrc_as_id,RRC_PROCEDURE_RBE,rbe.curr_substate,cmd_ptr->cmd_hdr.cmd_id);
      ...
      (cmd_ptr->cmd.state_change_ind.new_state == RRC_STATE_DISCONNECTED)) 
  {
    ...
    rrcrbcommon_ext_data.mac_rab_status = NO_RABS;
    MSG_LOW("MAC_RAB status;%d", rrcrbcommon_ext_data.mac_rab_status, 0, 0);
  } 
  ...
  case RRC_STATE_CELL_FACH:
    ...
    for CELL_FACH state */
    rrcrbe_cell_fach_event_handler(wrrc_as_id, cmd_ptr);
    break;

  case RRC_STATE_CELL_DCH:
    /* If it's CELL_DCH state, call the event handler
    for CELL_DCH state */
    rrcrbe_cell_dch_event_handler(wrrc_as_id, cmd_ptr);
    break;
    ...
    case RRC_STATE_CELL_PCH:
      /* If it's CELL_DCH state, call the event handler
      for CELL_PCH state */
      rrcrbe_pch_event_handler(wrrc_as_id, cmd_ptr);
      break;
  ...
  case RRC_STATE_DISCONNECTED:
    ...
    rrcrbe_other_state_event_handler(wrrc_as_id, cmd_ptr);
    break;
...
===========================================================================*/
void rrcrbe_init_procedure(sys_modem_as_id_e_type wrrc_as_id)
{
  uint32 count;
  ...
  to disconnected state. */
  rrcscmgr_register_for_scn(wrrc_as_id, RRC_PROCEDURE_RBE,  /* Procedure name */
  ...
  rex_def_timer(&rbe.rrcrbe_temp_timer, rex_self(), RRCRBE_TEMP_TIMER_SIG);
...
===========================================================================*/
rrcrb_rab_info_per_msg_type  * rrcrbe_get_est_rab_ptr(sys_modem_as_id_e_type wrrc_as_id)
{
  return &(rbe.rabs);
}
/*====================================================================
...
boolean rrc_l1_query_is_amr_call_enabled(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8 rab_index;
...
)
{
  ...
  for (rab_index=0; rab_index<MAX_RAB_TO_SETUP; rab_index++)
  {
    if ((rrc_est_rabs.rabs[rab_index].cn_domain == RRC_PS_DOMAIN_CN_ID) &&
         ...
         &&(!((rrccu_get_rabs_need_to_be_released_T314(wrrc_as_id) && rrc_est_rabs.rabs[rab_index].re_est_timer == rrc_Re_EstablishmentTimer_useT314)||
         (rrccu_get_rabs_need_to_be_released_T315(wrrc_as_id) && rrc_est_rabs.rabs[rab_index].re_est_timer == rrc_Re_EstablishmentTimer_useT315)))
#endif 
        )
    {
      if ((rabm_ptr = rabm_get_cmd_buf()) == NULL)
...
boolean rrcrbe_is_cs_data_rab_present (sys_modem_as_id_e_type wrrc_as_id)
{
...
l1_rrc_config_rab_enum_type rrc_l1_query_psrab_data_64_64_is_present(sys_modem_as_id_e_type wrrc_as_id)
{
	  rrcllc_oc_set_status_e_type	  oc_status;
	 ...
	 /* Check whether this RAB exists in established_rab db */
	  for (rab_index=0; rab_index<MAX_RAB_TO_SETUP; rab_index++)
	  {
			...
			)
		{
		  ...
		  {
			  dl_rate = rrcllc_get_dl_ded_rate(wrrc_as_id, rrc_get_state(wrrc_as_id),  rrc_est_rabs.rabs[rab_index].rb_for_rab[i].rb_id);
			  ul_rate = rrcllc_get_ul_ded_rate(wrrc_as_id, rrc_get_state(wrrc_as_id),  rrc_est_rabs.rabs[rab_index].rb_for_rab[i].rb_id);
			  ps_rab_found = TRUE;
			  if ((64000 == dl_rate) && (dl_rate == ul_rate))
			  {
				WRRC_MSG3_HIGH("rrc_l1_query_psrab_data_64_64_is_present dl:%d ul:%d rb_id:%d",dl_rate, ul_rate, rrc_est_rabs.rabs[rab_index].rb_for_rab[i].rb_id);
				/*checking 64kbps*/
	  ...
	  if ((oc_status != OC_NOT_SET) && (ordered_config.set_by_proc == RRC_PROCEDURE_RBE))
	  {
		for(rab_index =0; rab_index < RRCRB_MAX_RAB_PER_MSG; rab_index++)
		{
			...
			)
		  {
			 ...
			 {
				dl_rate = rrcllc_get_dl_ded_rate(wrrc_as_id, rrc_get_state(wrrc_as_id),  rbe.rabs.rabs_in_msg[rab_index].rb_for_rab[i].rb_id);
				ul_rate = rrcllc_get_ul_ded_rate(wrrc_as_id, rrc_get_state(wrrc_as_id),  rbe.rabs.rabs_in_msg[rab_index].rb_for_rab[i].rb_id);
				ps_rab_found = TRUE;
				/*checking 64kbps*/
				if ((64000 == dl_rate) && (dl_rate == ul_rate))
				{
...
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
/*===========================================================================
FUNCTION rrc_get_mapping_combo_rb
...
===========================================================================*/
void rrc_get_mapping_combo_rb(sys_modem_as_id_e_type wrrc_as_id, rrc_proc_e_type proc_id, rrc_overall_ota_info_s* overall_ota_info_ptr)
{
  uint8 ix=0;
  uint8 rb_id = 0;
  ...
  overall_ota_info_ptr->mapping_combo_drb = rrc_get_mapping_combo(wrrc_as_id, rb_id);
...
===========================================================================*/
void rrc_get_mapping_combo_srb(sys_modem_as_id_e_type wrrc_as_id, rrc_overall_ota_info_s* overall_ota_info_ptr)
{
  overall_ota_info_ptr->mapping_combo_srb = rrc_get_mapping_combo(wrrc_as_id, DCCH_AM_RADIO_BEARER_ID);
}

/*===========================================================================
FUNCTION rrc_get_mapping_combo
...
===========================================================================*/
uint8 rrc_get_mapping_combo(sys_modem_as_id_e_type wrrc_as_id, uint8 rb_id)
{
  uint8 idx= 0; 
...
/*===========================================================================
FUNCTION rrc_get_one_sided_re_est_flag
...
void rrc_get_one_sided_re_est_flag(sys_modem_as_id_e_type wrrc_as_id, rrc_proc_e_type proc_id, rrc_overall_ota_info_s* overall_ota_info_ptr)
{
  ...
  entity_idx = get_am_index_wo_f3(wrrc_as_id, rb_id);
...
/*===========================================================================
FUNCTION rrcqsh_get_num_rbs_added
...
void rrcqsh_get_num_rbs_added(sys_modem_as_id_e_type wrrc_as_id, rrc_proc_e_type proc_id, rrc_overall_ota_info_s* overall_ota_info_ptr)
{
  uint8 ix = 0;
...
/*===========================================================================
FUNCTION rrc_get_rlc_size_type
...
void rrc_get_rlc_size_type(sys_modem_as_id_e_type wrrc_as_id, rrc_proc_e_type proc_id, rrc_overall_ota_info_s* overall_ota_info_ptr)
{
 uint8 rb_id = 0;
 uint8 ix=0;
 ...
 if(rrc_get_dl_am_rlc_pdu_size(wrrc_as_id, rb_id) == 0)
 {
...
/*===========================================================================
FUNCTION rrbrbe_srns_relocation
...
uint8 rrbrbe_srns_relocation(sys_modem_as_id_e_type wrrc_as_id)
{
  return rbe.rbe_for_srns_relocation;
}

/*===========================================================================
FUNCTION rrcrbe_return_send_initiate_cu_flag
...
===========================================================================*/
  ...
  __attribute__((section(".uncompressible.text")))
#endif
boolean rrcrbe_return_send_initiate_cu_flag(sys_modem_as_id_e_type wrrc_as_id)
{
  return rbe.rrcrbe_send_initiate_cu_flag;
}
#endif
...
===========================================================================*/
void rrcrbe_set_failure_cause(sys_modem_as_id_e_type wrrc_as_id,rrcrb_msg_status_e_type cause)
{
...
{
  qsh_mdump_collect_high(&rbe,sizeof(rbe));
}
#endif
