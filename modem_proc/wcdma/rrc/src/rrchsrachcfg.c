...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrchsrachcfg.c#3 $    $DateTime: 2020/02/20 23:21:46 $ $Author: pwbldsvc $
...
04/28/15   sa      Made changes to notify about the TM RB type configured
...
//  config_ptr->mac_hs_rach_config.mac_ul_rach_txcntl.asc= ;
  ...
  rrcllc_init_mac_hsrach_edch_info(wrrc_as_id,&mac_hs_rach_config_ptr->hs_rach_config.e_dch_info);
...
)
{
  ...
  rrcllc_init_mac_hsrach_info(wrrc_as_id,&config_ptr->mac_hs_rach_config);

    /*Initialize L1 EDL structure*/  
  rrcllc_init_l1_hsrach_edl_info(wrrc_as_id,&config_ptr->l1_hs_rach_e_info.e_dl_info);

    /*Initialize L1 EUL structure*/    
  rrcllc_init_l1_hsrach_eul_info(wrrc_as_id,&config_ptr->l1_hs_rach_e_info.e_ul_info);

/*   initialize l1 hs dpcch structure*/
  rrcllc_init_l1_hsrach_hsdpcch_info(wrrc_as_id,&config_ptr->l1_hsdpa_info.hs_dpcch_info,next_rrc_state);

/*   initialize l1 fdpch structure */  
  rrcllc_init_l1_hsrach_fdpch_info(wrrc_as_id,&config_ptr->l1_dl_chan_parms.hs_rach_dl_fdpch_info);

/*   initialize l1 aich structure */    
  rrcllc_init_l1_hsrach_aich_info(wrrc_as_id,&config_ptr->l1_dl_chan_parms.hs_rach_aich);

/*   initialize l1 common edch resource list*/      
  rrcllc_init_l1_hsrach_common_edch_resource_list(wrrc_as_id,&config_ptr->common_e_dch_resource_list);

/*   initialize l1 ul dpch structure */        
  rrcllc_init_l1_hsrach_ul_dpch_info(wrrc_as_id,&config_ptr->l1_ul_chan_parms.hs_ul_dpch_cfg);

/*   initialize l1 prach structure */          
  rrcllc_init_l1_hsrach_prach_info(wrrc_as_id,&config_ptr->l1_ul_chan_parms.hs_prach_cfg);
...
#endif
  /*Disabling HS_RACH feature by default HI onwards*/
  return FALSE;
...
)
{
  ...
  if(rrchsrach_nv_supports_hs_rach(wrrc_as_id) == FALSE)
  {
    WRRC_MSG0_MED("RRCHSRACH:HSRACH NV doesnt suppport HSRACH");
    return FALSE;
  }
  
  if(FALSE == rrcenhstate_camped_cell_supports_e_fach(wrrc_as_id))
  {
    return FALSE;
  }
  else
...
SIDE EFFECTS:
====================================================================*/
uint8  rrchsrach_find_e_mac_d_flow_idx(sys_modem_as_id_e_type wrrc_as_id,ordered_config_type *config_ptr, uint8 e_mac_d_flow_id)
{
...
SIDE EFFECTS:
====================================================================*/
uint8  rrchsrach_allocate_e_mac_d_flow_index(sys_modem_as_id_e_type wrrc_as_id,ordered_config_type *config_ptr)
{
  uint8 cnt = MAX_E_DCH_MAC_D_FLOW;
...
)
{
  ...
  index =  rrchsrach_find_e_mac_d_flow_idx(wrrc_as_id,config_ptr,e_mac_d_flow_id);

  /* E-MAC-d flow index not found, Allocate new index*/
  index = rrchsrach_allocate_e_mac_d_flow_index(wrrc_as_id,config_ptr);
...
)
{
  ...
  while ((edch_flow_info->n > idx) && (idx < MAX_E_DCH_MAC_D_FLOW))
  {
    ...
    if ((edch_flow_idx = rrchsrach_get_e_mac_dflow_idx(wrrc_as_id,config_ptr,(uint8)edch_flow_info->elem[idx].mac_d_FlowIdentity)) 
      ...
      else
      {
        if(edch_flow_info->elem[idx].m.mac_d_FlowMultiplexingListPresent)
        {
          hs_rach_config_ptr->e_mac_d_flow_info[edch_flow_idx].e_mac_d_flow_mux_list = 
                       rrcllcpcie_convert_asn1_oss_bitstring_to_uint8(wrrc_as_id,&edch_flow_info->elem[idx].mac_d_FlowMultiplexingList);
        }
        else
        {
...
)
{
  ...
  if((idx = rrchsrach_find_e_mac_d_flow_idx(wrrc_as_id,config_ptr,HS_RACH_CCCH_D_FLOW_ID))
      == MAX_E_DCH_MAC_D_FLOW)
  {
...
)
{
  ...
  if(asc_ptr == NULL)
  {
    WRRC_ERR0_FATAL("RRCHSRACH Invalid ASC PTR received");
  }
  //Added check to fix the KW error
  if(*num_of_asc == 0)
  {
    WRRC_ERR0_FATAL("RRCHSRACH Invalid num of ASC received");
  }
...
SIDE EFFECTS    None
====================================================================*/
static void  rrchsrach_select_asc_from_ac(sys_modem_as_id_e_type wrrc_as_id,ordered_config_type *config_ptr)
{
  ...
  else
  {
    ...
    {
      if(rrcdata_sim_related_info.rrc_hi_access_class & ac_mask)
      {
        config_ptr->mac_hs_rach_config.ccch_asc  =
...
)
{
  ...
  if(psflist_ptr == NULL)
  {
    WRRC_ERR0_FATAL("RRCHSRACH Null pointer received");
  }
...
/*Process preamble partitioning IE*/
  ...
  {
    if( FAILURE == rrchsrach_get_prach_partitioning(wrrc_as_id,config_ptr->rrc_hsrach_info.asc_info,
...
/*Process persistance scaling factor IE*/
  ...
  {
    if( FAILURE == rrchsrach_get_persist_scaling_factors(wrrc_as_id,config_ptr,
  ...
  /*Fill MAC ASC and ASC structure*/
    ...
    for(asc_idx = 0; ((asc_idx < num_of_asc)&& (asc_idx < MAX_ASC)); asc_idx++)
    {
      ...
      if(asc_idx >= 2)
      {
         WRRC_MSG6_HIGH("asc_idx: %d  pval: %d mul factor: %d dpl: %d psf: %d scaling factor: %d",
                                           asc_idx,
                                           pval,
    ...
    {
      if( FAILURE == rrchsrach_get_ac_to_asc_mapping(wrrc_as_id,config_ptr,
    ...
    rrchsrach_select_asc_from_ac(wrrc_as_id,config_ptr);

    /* Process the CPICH Tx power IE */
    if(prach_preamble_info_ptr->m.primaryCPICH_TX_PowerPresent)
    {
      if( FAILURE == rrchsrach_get_primary_cpich_tx_pwr(wrrc_as_id,config_ptr,
    ...
    {
      if( FAILURE == rrchsrach_get_constant_value(wrrc_as_id,config_ptr,
    ...
    {
      if( FAILURE == rrchsrach_get_prach_pwr_offset(wrrc_as_id,config_ptr,
    ...
    {
      if( FAILURE == rrchsrach_get_rach_xmit_parms(wrrc_as_id,config_ptr,
    ...
    {
      if( FAILURE == rrchsrach_get_aich_info(wrrc_as_id,config_ptr,
...
)
{
  ...
  if(e_dpcch_info_ptr->m.e_DPDCH_PowerInterpolationPresent)
  {
    ...
    mac_hs_rach_config_ptr->hs_rach_config.e_dch_info.e_dpcch_info.happy_bit_delay_in_ms = 
      rrcllcpcie_convert_asn1_happy_bit_delay_condn_to_ms(wrrc_as_id,
...
)
{
  ...
  while (n_value != 0)
  {
    ...
    if(config_ptr->l1_hs_rach_e_info.e_ul_info.e_dpcch_info.e_tfc_boost_info.e_tfci_boost_val  != 0xff)
    {
      ...
      {
        WRRC_MSG3_ERROR("RRCHSRACH :ref etfci %d more than etfci boost val %d and po value more than > 29",
    ...
    if(e_dpdch_info_ptr->num_ref_e_tfci > 0 )
    {
      ...
      {
        WRRC_MSG3_ERROR("RRCHSRACH Ref ETFCI not in ascending order in OTA RefTFCI %d <  Next RefTFCI %d num ETFCI %d",
             e_dpdch_info_ptr->ref_e_tfci_list[e_dpdch_info_ptr->num_ref_e_tfci-1].ref_e_tfci,
             e_dpdch_info_ptr->ref_e_tfci_list[e_dpdch_info_ptr->num_ref_e_tfci].ref_e_tfci,
  ...
  {
    rrcllc_sort_ref_e_tfci_list(wrrc_as_id,e_dpdch_info_ptr);
  }
  return(SUCCESS);
}
...
)
{
  ...
  {
    if (FAILURE == rrchsrach_convert_asn1_periodicity_of_sched_info_grant(wrrc_as_id,
...
)
{
  ...
  l1_e_dl_hs_rach_info_struct_type *e_dl_info_ptr = &config_ptr->l1_hs_rach_e_info.e_dl_info;
    ...
    if (FAILURE == rrcllcpcie_convert_asn1_e_dch_max_ch_codes(wrrc_as_id,
    ...
    if (FAILURE == rrchsrach_process_reference_e_tfcis(wrrc_as_id,
    ...
    if (FAILURE == rrchsrach_process_sched_info_config(wrrc_as_id,
    ...
    if (FAILURE == rrchsrach_process_two_and_three_step_threshold_info(wrrc_as_id,
    ...
    if (RRC_CHECK_MSG_TYPE_IE_PTR(e_dpdch_info_ptr,minReduced_E_DPDCH_GainFactorPresent))
    {
      ...
      rrcllcpcie_convert_asn1_minreduced_e_dpdch_gainfactor(wrrc_as_id,
...
)
{
  ...
  if( FAILURE == rrchsrach_process_ul_interference_commonedch(wrrc_as_id,config_ptr,common_edch_sys_info_ptr))
    {
      WRRC_MSG0_ERROR("RRCHSRACH: Failure in setting PRACH preamble info from SIB5");
      return FAILURE;
    }
  ...
  if( FAILURE == rrchsrach_process_common_edch_flowlist(wrrc_as_id,config_ptr,
  ...
  if( FAILURE == rrchsrach_process_prach_preamble_enhanceduplink(wrrc_as_id,config_ptr,
  ...
  if( FAILURE == rrchsrach_process_initial_serving_grant(wrrc_as_id,config_ptr,
  ...
  if( FAILURE == rrchsrach_process_edch_tti(wrrc_as_id, config_ptr,common_edch_sys_info_ptr))
    {
      WRRC_MSG0_ERROR("RRCHSRACH: Failure in setting PRACH preamble info from SIB5");
      return FAILURE;
    }
  ...
  if( FAILURE == rrchsrach_process_eagch_info(wrrc_as_id, config_ptr,common_edch_sys_info_ptr))
    {
      WRRC_MSG0_ERROR("RRCHSRACH: Failure in setting PRACH preamble info from SIB5");
      return FAILURE;
    }

  if( FAILURE == rrchsrach_process_harq_info(wrrc_as_id, config_ptr,common_edch_sys_info_ptr))
    {
      WRRC_MSG0_ERROR("RRCHSRACH: Failure in setting PRACH preamble info from SIB5");
      return FAILURE;
    }
  ...
  if( FAILURE == rrchsrach_process_uldpch_powerctrl_info(wrrc_as_id, config_ptr,&common_edch_sys_info_ptr->ul_DPCHpowerControlInfoForCommonEDCH))
    {
      WRRC_MSG0_ERROR("RRCHSRACH: Failure in setting PRACH preamble info from SIB5");
      return FAILURE;
    }

  if( FAILURE == rrchsrach_process_e_dpcch_info(wrrc_as_id, config_ptr,
  ...
  if( FAILURE == rrchsrach_process_e_dpdch_info(wrrc_as_id, config_ptr,
  ...
  if( FAILURE == rrchsrach_process_fdpch_info(wrrc_as_id, config_ptr,common_edch_sys_info_ptr))
    {
      WRRC_MSG0_ERROR("RRCHSRACH: Failure in setting PRACH preamble info from SIB5");
      return FAILURE;
    }
  ...
  config_ptr->mac_hs_rach_config.l1_e_dch_tx_cont_backoff = 
    rrchsrach_convert_asn1_edch_transmit_cont_offset(wrrc_as_id, common_edch_sys_info_ptr->e_dch_TransmitContinuationOffset);

  config_ptr->mac_hs_rach_config.max_e_dch_res_alloc_ccch = 
    rrchsrach_convert_asn1_edch_max_ccch_resource_alloc(wrrc_as_id, common_edch_sys_info_ptr->max_CCCH_ResourceAllocation);
...
//    config_ptr->l1_hsdpa_info.hs_req_mask|=  CPHY_HS_DPCCH_CFG_INCL;
  ...
  {
    if( FAILURE == rrchsrach_process_meas_feedback_info(wrrc_as_id,config_ptr,&common_edch_sys_info_ptr->measurement_Feedback_Info))
    {
  ...
  if( FAILURE == rrchsrach_update_common_edch_resource_list(wrrc_as_id,config_ptr,&common_edch_sys_info_ptr->common_E_DCH_ResourceInfoList))
  {
  ...
  if( FAILURE == rrchsrach_update_rb_mapping_info_for_ul_ccch_srb0(wrrc_as_id,config_ptr))
    {
      WRRC_MSG0_ERROR("RRCHSRACH: Failure in setting MAC D Flow info from SIB5");
      return FAILURE;
    }

  if( FAILURE == rrchsrach_update_edl_info(wrrc_as_id,&config_ptr->l1_hs_rach_e_info.e_dl_info,
                          common_edch_sys_info_ptr))
    {
      WRRC_MSG0_ERROR("RRCHSRACH: Failure in setting PRACH preamble info from SIB5");
      return FAILURE;
...
/*Initialize the HS-RACH structures*/
  rrcllc_init_hsrach_info(wrrc_as_id,config_ptr,next_rrc_state);

  /*The check whether the cell supports HSRACH or not must have been done by now*/
  if(rrchsrach_copy_sib5_to_config_db(wrrc_as_id,config_ptr, next_rrc_state) == FAILURE)
  {
...
/*====================================================================
FUNCTION: rrchsrach_print_cell_transition_req()

DESCRIPTION:
  Prints hsrach contents of cell transition request
...
void rrchsrach_print_cell_transition_req(sys_modem_as_id_e_type wrrc_as_id, l1_cell_transition_cmd_type *l1_cmd_ptr)
{
  ...
  {
    WRRC_MSG9_HIGH("RRCHSRACH :s_offset %d, f_dpch_chan_code_num %d, e_rgch_info_present %d, rg_idx %d signature_idx %d ch_code %d signature_idx %d scrambling_code_type %d scrambling_code %d",
  ...
  */
  WRRC_MSG5_HIGH("RRCHSRACH :EDL Info p_ernti_prst %d, p_ernti 0x%d, tti %d, srvg_grant %d, 3_idx_step_thres %d",
  ...
  WRRC_MSG4_HIGH("RRCHSRACH :EDL Info 2_idx_step_thres %d grant_selec %d ch_code %d, #Common resources %d",
...
/*
  WRRC_MSG6_HIGH("RRCHSRACH :EUL Info harq_rv_config %d, DPDCH e_tfci_table_idx %d, num_ref_e_tfci %d, min_beta_ed %d, e_max_ch_codes %d e_dpdch_pl_non_max %d ",
...
/*====================================================================
FUNCTION: rrchsrach_print_cphy_setup()

DESCRIPTION:
  Prints hsrach contents of cphy setup request
...
void rrchsrach_print_cphy_setup(sys_modem_as_id_e_type wrrc_as_id,l1_setup_cmd_type *l1_cmd_ptr)
{

  WRRC_MSG3_HIGH("RRCHSRACH CPHY_SETUP CphySetup 0x%x L1 HS_RACH_REQ_MASK:0x%x [1-START,2- RECFG,4-STOP], transition_to_dch:%d", 
                   l1_cmd_ptr->req_mask,
...
/*
    WRRC_MSG4_HIGH("RRCHSRACH  hsdpa_l1_ops 0x%d hsdpa_l1_info hs_req_mask %d transition_to_dch:%d Number of Common resources  %d",
...
/*
  ...
  if((l1_cmd_ptr->dl_phychan_db != NULL) && (l1_cmd_ptr->ul_phychan_db != NULL))
   {
   WRRC_MSG3_HIGH("RRCHSRACH :FDPCH info for start/reconfig dpc_info %d, tpc_cmd_err_rate_target %d, fdpch_slot_format %d ",
   ...
   WRRC_MSG4_HIGH("RRCHSRACH :AICH info for start ovsf %d, tt %d, tx_div_ind %d, pwr_offset %d ",
   ...
   WRRC_MSG7_HIGH("RRCHSRACH :UL DPCCH slot format %d, power ctrl info valid %d, dpcch_pwr_offset %d, pcp_len %d, srb_delay %d, pca %d, tpc_step_size %d",
   ...
   WRRC_MSG5_HIGH("RRCHSRACH :PRACH info avail_sig_mask %d, sf_min %d, sc_num %d, punct_lim %d, avail_sub_chan_mask %d",
   ...
   WRRC_MSG5_HIGH("RRCHSRACH :PRACH info num_asc %d, pwr_offset_P0 %d, max_preamble_tx %d, e_ai_indication %d, powerOffsetPp_e %d",
...
/*====================================================================
FUNCTION: rrchsrach_print_cmac_ul_config()

DESCRIPTION:
  Prints hsrach contents of cmac ul config
...
SIDE EFFECTS:
  ====================================================================*/
void rrchsrach_print_cmac_ul_config(sys_modem_as_id_e_type wrrc_as_id,mac_ul_config_type *l2_cmd_ptr)
{
...
uint8 count=0,count2=0;
  ...
  WRRC_MSG4_HIGH("RRCHSRACH :hs_rach_to_eul_trans:%d mac_hsrach_e_reset_indicator: %d, ERNTI info primary_e_rnti_present  %d primary_e_rnti %d  ",	
  ...
  WRRC_MSG4_HIGH("RRCHSRACH :HSRACH new IEs max_e_dch_res_alloc_ccch %d, l1_e_dch_tx_cont_backoff %d max_collision_resol %d, add_e_dch_tx_backoff %d ",	
  ...
  WRRC_MSG5_HIGH("RRCHSRACH :PRACH info ccch_asc %d mmax %d nASC %d, nbo1max %d, nbo1min %d",
  ...
  { 
    WRRC_MSG5_HIGH("RRCHSRACH :Flows info e_mac_d_flow_id %d, e_mac_d_flow_power_offset %d e_mac_d_flow_max_num_of_retx %d, e_mac_d_flow_mux_list %d num_logch %d",
    ...
    {
      WRRC_MSG6_HIGH("RRCHSRACH :LC info rb_id %d, rlc_id %d lc_id %d, rlc_mode %d chan_type %d, rlc_pdu_type %d ",
      ...
      {
        WRRC_MSG3_HIGH("RRCHSRACH :Flexible size params ul_li_size%d ul_flex_min_pdu_size %d ul_flex_max_pdu_size %d",
  ...
  WRRC_MSG7_HIGH("RRCHSRACH :EDCH info hppybit_delay %d e_tfci_table_index %d e_tfci_min_set %d, no_grant_period_of_si %d, grant_period_of_si %d po_si%d sch_tx_grant_harq_allo_2ms %d",
...
/*====================================================================
FUNCTION: rrchsrach_print_crlc_am_config()

DESCRIPTION:
  Prints contents of crlc am config
...
void rrchsrach_print_crlc_am_config(sys_modem_as_id_e_type wrrc_as_id,rlc_am_config_req_type *l2_cmd_ptr)
{
...
uint8 count=0;
  ...
  for (count=0; count<l2_cmd_ptr->nentity; count++)
  {
    ...
    {
        WRRC_MSG6_HIGH("RRCHSRACH :CRLC_AM_CONFIG:action UL %d, rb id %d ul_nchan %d, reestab_entity %d ul_flex_pdu_size %d Fixed size params ul_rlc_size%d",
    ...
    {
        WRRC_MSG8_HIGH("RRCHSRACH :CRLC_AM_CONFIG:action UL %d, rb id %d ul_nchan %d, reestab_entity %d ul_flex_pdu_size %d Flexible size params ul_li_size: %d ul_flex_min_pdu_size %d ul_flex_max_pdu_size %d",
        l2_cmd_ptr->am_act[count].action,
        l2_cmd_ptr->am_info[count]->common_config.rb_id,
        l2_cmd_ptr->am_info[count]->ul_nchan,
        l2_cmd_ptr->am_info[count]->reestab_entity,
...
/*====================================================================
FUNCTION: rrchsrach_print_crlc_dl_tm_config()

DESCRIPTION:
  Prints contents of crlc dl tm config
...
void rrchsrach_print_crlc_dl_tm_config(sys_modem_as_id_e_type wrrc_as_id,rlc_dl_tm_config_type *l2_cmd_ptr)
{
  int i=0;
  WRRC_MSG6_MED("RRCHSRACH :CRLC_DL_TM_CONFIG:nchan %d lc_id %d, lc_type %d seg_ind %d action %d act_incl %d",
  ...
  WRRC_MSG4_MED("RRCHSRACH :CRLC_DL_TM_CONFIG:act_incl %d act_time %d dl_cfg_mask %d ul_cfg_mask %d",	
    l2_cmd_ptr->dl_tm_act->act_incl,
  ...
  {
    WRRC_MSG6_HIGH("CRLC_DL_TM_CONFIG: LC-ID: %d LC-TYPE:%d ACTION:%d dl_cfg_mask:%d, ul_cfg_mask:%d RAB_TYPE:%d",
                               ...
                               l2_cmd_ptr->chan_info[i].rlc_dl_tm_rab);
  }
}

/*====================================================================
FUNCTION: rrchsrach_print_crlc_ul_tm_config()

DESCRIPTION:
  Prints contents of crlc ul tm config
...
void rrchsrach_print_crlc_ul_tm_config(sys_modem_as_id_e_type wrrc_as_id,rlc_ul_tm_config_type *l2_cmd_ptr)
{
uint8 count=0;
  for (count=0; count<l2_cmd_ptr->nchan; count++)
  {
    WRRC_MSG4_HIGH("RRCHSRACH :CRLC_UL_TM_CONFIG:action UL %d, lc_type%d lc_id %d rab_type:%d",
      l2_cmd_ptr->ul_tm_act[count].action,
      l2_cmd_ptr->chan_info[count].lc_type,
      l2_cmd_ptr->chan_info[count].lc_id,
      l2_cmd_ptr->chan_info[count].rlc_ul_tm_rab);
  }
}

/*====================================================================
FUNCTION: rrchsrach_print_crlc_dl_um_config()

DESCRIPTION:
  Prints contents of crlc dl um config
...
void rrchsrach_print_crlc_dl_um_config(sys_modem_as_id_e_type wrrc_as_id,rlc_dl_um_config_type *l2_cmd_ptr)
{
    WRRC_MSG8_HIGH("RRCHSRACH:CRLC_DL_UM_CONFIG:nchan %d lc_id %d, lc_type %d hfn: %d rlc_size %d ciph_algo %d alternate_e_bit %d li_size %d",
    l2_cmd_ptr->nchan,l2_cmd_ptr->chan_info->lc_id,
    l2_cmd_ptr->chan_info->lc_type,
    ...
    WRRC_MSG5_HIGH("RRCHSRACH CRLC_DL_UM_CONFIG:action %d act_incl %d act_time %d dl_cfg_mask %d ul_cfg_mask %d",	
...
/*====================================================================
FUNCTION: rrchsrach_print_crlc_ul_um_config()

DESCRIPTION:
  Prints contents of crlc ul um config
...
void rrchsrach_print_crlc_ul_um_config(sys_modem_as_id_e_type wrrc_as_id,rlc_ul_um_config_type *l2_cmd_ptr)
{
uint8 count=0;
  ...
  for (count=0; count<l2_cmd_ptr->nchan; count++)
  {
    ...
    {
      WRRC_MSG4_HIGH("RRCHSRACH CRLC_UL_UM_CONFIG:action UL %d, lc_type%d lc_id %d Fixed size params ul_rlc_size%d ",
    ...
    {
      WRRC_MSG5_HIGH("RRCHSRACH CRLC_UL_UM_CONFIG:action UL %d, lc_type%d lc_id %d Flexible size params ul_flex_min_pdu_size %d ul_flex_max_pdu_size %d",
      l2_cmd_ptr->ul_um_act[count].action,
      l2_cmd_ptr->chan_info[count].lc_type,
      l2_cmd_ptr->chan_info[count].lc_id,
...
/*====================================================================
FUNCTION: rrchsrach_get_previous_cell_supports_hsrach(sys_modem_as_id_e_type wrrc_as_id)
...
)
{
  ...
  rrc_ccm_get_previous_camped_cell_info(wrrc_as_id,&prev_plmn_id,&prev_cell_id);
  PRINT_PLMN_ID(prev_plmn_id )
  WRRC_MSG1_HIGH("RRCHSRACH:prev cell id=%d",prev_cell_id);

  sib5_ptr = (rrc_SysInfoType5 *) rrc_sibdb_return_sib_for_cell(wrrc_as_id,
                            rrc_SIB5, 
  ...
  if(FALSE == rrcllc_get_previous_cell_hs_status_in_e_fach(wrrc_as_id))
  {
    return FALSE;
  }
  else
