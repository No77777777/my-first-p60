...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcdormancy.c#2 $
...
03/18/16   sr      Changes for FR#32607
...
LOCAL rrcdorm_int_data_type rrcdorm_int_data_sim[MAX_RRC_STACKS];
#define rrcdorm_int_data rrcdorm_int_data_sim[WAS_ID_M]
#else
LOCAL rrcdorm_int_data_type rrcdorm_int_data;
#endif
...
{
  WRRC_MSG0_HIGH("RRCDORM:init saved_fast_dormancy_req");
  rrcdorm_int_data.saved_fast_dormancy_req.fast_dormancy_req_status = FALSE;
  rrcdorm_int_data.saved_fast_dormancy_req.mui = 0;
  rrcdorm_int_data.saved_fast_dormancy_req.mui_status = RRCDORMANCY_MUI_STATUS_INVALID;
  rrcdorm_int_data.substate = RRCDORMANCY_INITIAL;
}
...
)
{
  ...
  {
    if(rrcdorm_int_data.saved_fast_dormancy_req.fast_dormancy_req_status == FALSE)
    {
      WRRC_MSG0_HIGH("RRCDORM:Save fast_dormancy_req");
      rrcdorm_int_data.saved_fast_dormancy_req.fast_dormancy_req_status = TRUE;
      rrcdorm_int_data.saved_fast_dormancy_req.mui_status = RRCDORMANCY_MUI_STATUS_INVALID;
      rrcdorm_int_data.saved_fast_dormancy_req.mui = 0;
...
)
{
  ...
  if(rrcdorm_int_data.saved_fast_dormancy_req.fast_dormancy_req_status == TRUE)
  {
    ...
    if (ul_lc_id == RRCLCM_RLC_LC_ID_NOT_FOUND)
    {
      ...
      rrcdormancy_reset_n_send_fast_dormancy_cnf(wrrc_as_id, RRC_FAST_DORMANCY_FAILURE_APP_RETRY);
      return;
    }
    else
    {
      ...
      rrc_qsh_log_scri_params(wrrc_as_id, msg_ptr->message.u.signallingConnectionReleaseIndication.laterNonCriticalExtensions.
        v860nonCriticalExtentions.signallingConnectionReleaseIndication_v860ext.
      ...
      send_status = rrcsend_send_ul_sdu_with_mui(wrrc_as_id,RRC_PROCEDURE_DORMANCY,msg_ptr,ul_lc_id,
                                                 RRCSEND_L2ACK_REQUIRED, &msg_mui);

      if(rrcdorm_int_data.max_scri_for_fd != 0)
      {
        /* Decrement rrc_max_scri_for_fd as we have sent one SCRI */
        rrcdorm_int_data.max_scri_for_fd --;
      }

      if(FALSE == l2_ack_processing_required)
      {
        WRRC_MSG1_HIGH("SigConRel for Pending Fast dormancy sent cause %d ", 
            rrcdorm_int_data.saved_fast_dormancy_req.mui_status);

        rrcdormancy_init_saved_fast_dormancy_req(wrrc_as_id);
      ...
      {
        rrcdorm_int_data.substate = RRCDORMANCY_WAIT_FOR_L2ACK;
        rrcdorm_int_data.saved_fast_dormancy_req.mui= msg_mui;
  
        /* update the mui_status cause and also the flag to indicate that close_sess_req
               * or sig_con_rel_req has been already sent */
        rrcdorm_int_data.saved_fast_dormancy_req.mui_status = RRCDORMANCY_MUI_SCRI_CNF; 
...
)
{
  ...
  if( (cmd_ptr =  mm_rrc_get_cmd_buf(RRC_FAST_DORMANCY_CNF)) != NULL)
  {
    ...
    cmd_ptr->cmd.rrc_fast_dormancy_cnf.as_id = rrc_get_as_id(wrrc_as_id);
#endif
...
{
  if(rrcdorm_int_data.conn_timers_and_constants.T323_nw_support == TRUE)
    return TRUE;
  else
    return FALSE;
}
...
)
{
  ...
  WRRC_MSG2_HIGH("Current T323 VALUE: %d STATUS: %d",rrcdorm_int_data.conn_timers_and_constants.T323_duration,
    rrcdorm_int_data.conn_timers_and_constants.T323_current_status);  

  switch(rrcdorm_int_data.conn_timers_and_constants.T323_current_status)
  {
    case RRCDORMANCY_SRNS_GUARD_TIMER_RUNNING:
      rrctmr_stop_timer(wrrc_as_id, RRCTMR_T_323_TIMER);
...
{
  rrcdorm_int_data.conn_timers_and_constants.T323_nw_support = FALSE;
  rrcdorm_int_data.conn_timers_and_constants.T323_duration = rrc_T_323_s0;
  rrcdorm_int_data.conn_timers_and_constants.T323_current_status = RRCDORMANCY_T323_TIMER_INVALID;
}
...
)
{
  ...
  rrcllc_get_drx_info_values(wrrc_as_id,&cs_drx, &ps_drx, &drx_in_use);
  ...
  WRRC_MSG2_HIGH("shorter_cn_drx:%d v_316:%d",shorter_cn_drx,rrcdorm_int_data.v_316);
  ...
  else 
  {
    ...
    if(rrcdorm_int_data.v_316 < 1)
    {
      /* Increment V316 by 1*/
      rrcdorm_int_data.v_316++;
      return TRUE;
    }
    else
    {
    ...
    for(rab_idx = 0; rab_idx < MAX_RAB_TO_SETUP; rab_idx++)
    {
          ...
          (rrc_est_rabs.rabs[rab_idx].num_rbs_for_rab > 0))
      {
        ...
        for (rb_count=0; rb_count<rrc_est_rabs.rabs[rab_idx].num_rbs_for_rab; rb_count++)
        {
          ...
          local_lc_id = rrclcm_get_ul_rlc_lc_id(wrrc_as_id,local_rb_id);
          local_rlc_wm_size = rlc_rrc_get_wm_info_req(wrrc_as_id, local_lc_id);
          
          WRRC_MSG3_HIGH("RB:%d LC_ID:%d WM_SIZE:%d",local_rb_id,local_lc_id,local_rlc_wm_size);

          if((rrc_procedure == RRC_PROCEDURE_IDT) || (local_rlc_wm_size > 0))
          {
            /* Reset V316 and return */ 
            rrcdormancy_reset_v316(wrrc_as_id);
            return;
          }
        }
      }
...
{
  WRRC_MSG1_HIGH_OPT("function rrcdormancy_update_dormancy_for_srns_hho_cco is called from PROC:rrc_proc_e_type_value%d",rrc_proc);
  ...
  switch(rrc_proc)
  {
    case RRC_PROCEDURE_UMI: 
      {
        if(FALSE == rrcdormancy_check_if_nw_support_fast_dormancy(wrrc_as_id))        
        {
          ...
          if(rrcdorm_int_data.substate == RRCDORMANCY_WAIT_FOR_SIG_CON_REL_COMPL)
          {
            rrcscrr_abort_ps_rel_proc(wrrc_as_id);
          }

          rrcdormancy_check_and_abort_dormancy_req(wrrc_as_id, RRC_FAST_DORMANCY_FAILURE_APP_RETRY);
        }
      }
      break;  
    ...
    case RRC_PROCEDURE_IHO:
      {
        ...
        if((rrc_proc != RRC_PROCEDURE_CHO) &&
           ...
           (TRUE == rrcdormancy_get_nw_support_of_fast_dormancy_from_sib(wrrc_as_id)))
        {
        ...
        else
        {
          ...
          rrcdormancy_stop_or_restart_timer_T323(wrrc_as_id,RRCDORM_SRNS_GUARD_TIMER_VALUE,
...
/*===========================================================================
  ...
  FUNCTION 
    rrcdormancy_update_dormancy_params  

  DESCRIPTION
    This function will update T323 timer value and relevant fast dormancy parameters
    when NW supports Rel-8 dormancy
...
void rrcdormancy_update_dormancy_params(sys_modem_as_id_e_type wrrc_as_id,rrc_T_323 t323_duration)
{
  rrcdorm_int_data.conn_timers_and_constants.T323_nw_support = TRUE;
  rrcdorm_int_data.conn_timers_and_constants.T323_duration = t323_duration;
  rrcdorm_int_data.conn_timers_and_constants.T323_current_status = 
    rrcdormancy_stop_n_update_t323_timer_status(wrrc_as_id);

  WRRC_MSG3_HIGH("Updating T323 NW:%d Duration:%d Status:%d", 
   rrcdorm_int_data.conn_timers_and_constants.T323_nw_support,
   rrcdorm_int_data.conn_timers_and_constants.T323_duration,
   rrcdorm_int_data.conn_timers_and_constants.T323_current_status);
}
...
)
{
  ...
  if(rrcdorm_int_data.conn_timers_and_constants.T323_nw_support == TRUE)
  {
    switch(rrcdorm_int_data.conn_timers_and_constants.T323_duration)
    {
      case rrc_T_323_s0:
        ret_val = 0;
        break;
...
)
{
   ...
   */
  if((rlc_query_data_channel_activity(wrrc_as_id) == NO_DATA_TRANSFERRED) && rrc_is_ps_domain_rab_present(wrrc_as_id))
  {
    /* No data activity, mark num_pending_dormancy_req to TRUE */
    WRRC_MSG1_HIGH("RRCDORM: RRC state %d",rrc_get_state(wrrc_as_id));
    if(((rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH) || 
        (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH)) &&
       (rrcdorm_int_data.max_scri_for_fd != 0)
      )
    {
      WRRC_MSG1_HIGH("RRCDORM: in %d (FACH/DCH) increment pending ",rrc_get_state(wrrc_as_id));
      rrcdorm_int_data.saved_fast_dormancy_req.num_pending_dormancy_req++;
    }
    else if(((rrc_get_state(wrrc_as_id) == RRC_STATE_URA_PCH) || 
             (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_PCH)) &&
             (rrcdorm_int_data.max_scri_for_fd != 0)
           )
    {
      WRRC_MSG1_HIGH("RRCDORM: in %d(PCH) increment pending and set max_scri ",rrc_get_state(wrrc_as_id));
      rrcdorm_int_data.saved_fast_dormancy_req.num_pending_dormancy_req++;
      return;
    }
  }
  else
  {
    ...
    rrcdorm_int_data.saved_fast_dormancy_req.num_pending_dormancy_req =0;
  }

  if(rrcdorm_int_data.saved_fast_dormancy_req.num_pending_dormancy_req != 0)
  {
    WRRC_MSG1_HIGH("Pending Dormancy requests:%d",rrcdorm_int_data.saved_fast_dormancy_req.num_pending_dormancy_req);
    /* If all validation goes well then process fast dormancy req and start T323 timer*/
    if(TRUE == rrcdormancy_process_pending_fast_dormancy_req(wrrc_as_id))
    {
      if(TRUE == rrcdormancy_check_if_nw_support_fast_dormancy(wrrc_as_id))
      {
        rrcdormancy_stop_or_restart_timer_T323(wrrc_as_id,rrcdormancy_get_T323_timer_value(wrrc_as_id),
  ...
  if(TRUE == rrcdormancy_check_if_nw_support_fast_dormancy(wrrc_as_id))
    rrcdorm_int_data.conn_timers_and_constants.T323_current_status = RRCDORMANCY_T323_TIMER_IDLE;
  else
    rrcdorm_int_data.conn_timers_and_constants.T323_current_status = RRCDORMANCY_T323_TIMER_INVALID;

  WRRC_MSG1_HIGH("Resetting T323 Status: %d",rrcdorm_int_data.conn_timers_and_constants.T323_current_status);
}
...
)
{
  ...
  {
    if(0 != rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_T_323_TIMER))
    {
       rrctmr_stop_timer(wrrc_as_id, RRCTMR_T_323_TIMER);
    }
    rrcdorm_int_data.conn_timers_and_constants.T323_current_status = timer_status;

    WRRC_MSG1_HIGH("STOP T323, STATUS: %d",timer_status);  
    return;
  }
    
  remaining_value = rrctmr_get_remaining_time(wrrc_as_id, RRCTMR_T_323_TIMER);
  ...
  if(remaining_value < timer_value)
  {
    ...
    rrctmr_start_timer(wrrc_as_id,RRCTMR_T_323_TIMER, timer_value);
...
)
{
     ...
     (rrcdorm_int_data.saved_fast_dormancy_req.mui_status != RRCDORMANCY_MUI_STATUS_INVALID))
  {
    ...
    rrcdormancy_reset_n_send_fast_dormancy_cnf(wrrc_as_id,abort_type);
  }
  else
  {
    WRRC_MSG0_MED("No dormancy_req in progress");
  }
}
...
{
  if((rrcdorm_int_data.saved_fast_dormancy_req.fast_dormancy_req_status == TRUE) &&
     (rrcdorm_int_data.substate == RRCDORMANCY_WAIT_FOR_SIG_CON_REL_COMPL) &&
     (rrcdorm_int_data.saved_fast_dormancy_req.mui_status == RRCDORMANCY_MUI_SIG_CON_REL_CNF))
  {
...
)
{
  ...
  if((TRUE == rrcrb_is_srns_in_progress(wrrc_as_id)) ||
     (TRUE == rrcsmc_is_rlc_re_est_for_srns_active(wrrc_as_id)))
  {
  ...
  if(FALSE == rrcscrr_is_cn_domain_open(wrrc_as_id,RRC_PS_DOMAIN_CN_ID))
  {
    WRRC_MSG2_HIGH("SCRR active for PS domain. Aborting Dormancy dorm-state:%d rrc-state:%d",
      rrcdorm_int_data.substate, rrc_state);
    rrcdormancy_reset_n_send_fast_dormancy_cnf(wrrc_as_id,RRC_FAST_DORMANCY_SUCCESS_APP_NO_RETRY);    
    return TRUE;
  }
  else if(TRUE == is_iho_w2g_active(wrrc_as_id))
  {
  ...
  else if(TRUE == rrcidt_is_idt_active(wrrc_as_id,RRC_PS_DOMAIN_CN_ID))
  {
  ...
  else if(TRUE == rrcidt_is_idt_active(wrrc_as_id,RRC_CS_DOMAIN_CN_ID))
  {
  ...
  else if(TRUE == rrccho_is_wtog_cco_active(wrrc_as_id))
  {
...
)
{
  ...
  if ((status == RRCDORMANCY_VALIDATION_SUCCESS) &&
      (rrcscrr_is_cn_domain_open(wrrc_as_id,RRC_PS_DOMAIN_CN_ID) == FALSE))
  {
  ...
  /* Check to see if a signalling session to the requested PS domain is open */
  if ((status == RRCDORMANCY_VALIDATION_SUCCESS) &&
      (rrcscrr_is_cn_domain_open(wrrc_as_id,RRC_CS_DOMAIN_CN_ID) == TRUE)&&
      (TRUE == rrcdormancy_check_if_nw_support_fast_dormancy(wrrc_as_id)))
  {
  ...
  if ((status == RRCDORMANCY_VALIDATION_SUCCESS) &&
       ...
       (FALSE == rrcdormancy_check_if_nw_support_fast_dormancy(wrrc_as_id)))
  {
  ...
  if ((status == RRCDORMANCY_VALIDATION_SUCCESS) &&
       ((rrcrce_get_rrc_connection_status(wrrc_as_id) == RRCRCE_RRC_CONN_NOT_ESTABLISHED)||
        (rrcrcr_is_proc_active(wrrc_as_id) == TRUE)))
  {
    WRRC_MSG2_ERROR("RRCDORM:RRC connection not open[RCE state-%d,RCR state-%d]",
      rrcrce_get_rrc_connection_status(wrrc_as_id),
      rrcrcr_is_proc_active(wrrc_as_id));
    status = RRCDORMANCY_VALIDATION_FAIL_RRC_CON_NOT_OPEN;
  }
  ...
  if((status == RRCDORMANCY_VALIDATION_SUCCESS) &&
     (RRC_NOT_CAMPED_ON == rrc_ccm_get_curr_camping_status(wrrc_as_id,RRC_PROCEDURE_DORMANCY))&& 
  ...
  if((status == RRCDORMANCY_VALIDATION_SUCCESS) &&
     (0 != rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_T_323_TIMER))
    )
  {
    ...
    WRRC_MSG2_ERROR("Timer T323 is running RRC State:%d Dormancy Substate",
      rrc_get_state(wrrc_as_id), rrcdorm_int_data.substate);
  ...
  if((status == RRCDORMANCY_VALIDATION_SUCCESS) &&
     (rrcnv_data.rrc_dormancy_support_nv == RRC_NV_DORMANCY_DISABLED)
    )
  {
    status = RRCDORMANCY_VALIDATION_FAIL_RRC_CON_NOT_OPEN;
  }
  ...
  if((status == RRCDORMANCY_VALIDATION_SUCCESS) &&
     ((rrcnv_data.rrc_dormancy_support_nv == RRC_NV_LEGACY_DORMANCY_DISABLED)&&
      (FALSE == rrcdormancy_check_if_nw_support_fast_dormancy(wrrc_as_id)))
    )
  {
...
)
{
  ...
  if((rrcdormancy_sig_conn_req_ptr =  rrc_get_int_cmd_buf(wrrc_as_id)) != NULL)
  { 
    ...
    rrc_put_int_cmd(wrrc_as_id, rrcdormancy_sig_conn_req_ptr);   
  }
...
)
{
  ...
  if(TRUE == rrcdorm_check_n_send_cnf_for_failure_conditions(wrrc_as_id))
  {
  ...
  if(FALSE == rrcdormancy_check_if_nw_support_fast_dormancy(wrrc_as_id))
  {
    ...
    rrcdormancy_send_sig_conn_rel_req(wrrc_as_id);
    return FALSE;
  }
  ...
  switch(rrc_get_state(wrrc_as_id))
  {
    ...
    if(FALSE == rrcdormancy_check_drx_cycle_n_v316_to_send_scri(wrrc_as_id))
    {
      WRRC_MSG1_HIGH("Drop SCRI in rrc_state:%d ", rrc_get_state(wrrc_as_id));
      rrcdormancy_reset_n_send_fast_dormancy_cnf(wrrc_as_id, RRC_FAST_DORMANCY_FAILURE_APP_RETRY);
      return FALSE;
    }
    if(RRCCU_NOT_COMPLETED == rrccu_get_cell_update_complete_status(wrrc_as_id,RRC_PROCEDURE_DORMANCY))
    {
      /* Wait until Cell update procedure is completed */
      rrcdorm_int_data.substate = RRCDORMANCY_WAIT_FOR_CU_COMP_CNF;  
    }
    else /* Cell Update procedure is inactive */
    {
      ...
      if(0 != rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_T_320_TIMER))
      {
...
#endif
      {
        ...
        if(rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_T_323_TIMER))
        {
          rrcdormancy_reset_n_send_fast_dormancy_cnf(wrrc_as_id, RRC_FAST_DORMANCY_FAILURE_APP_RETRY);
          rrcdorm_int_data.saved_fast_dormancy_req.num_pending_dormancy_req++;
        }
        else
        {
          ...
          rrcdormancy_send_cell_update_initiate_req(wrrc_as_id, RRC_PROCEDURE_DORMANCY);
          rrcdorm_int_data.substate = RRCDORMANCY_WAIT_FOR_CU_COMP_CNF;  
        }
      }
    }    
  ...
  case RRC_STATE_CELL_FACH:
    ...
    else /* Cell Update procedure is inactive */
    {
      ...
      if(rrcdorm_int_data.max_scri_for_fd != 0)
      {
        rrcdormancy_send_scri_for_fast_dormancy(wrrc_as_id, TRUE);
      }
      status = TRUE;
    }    
    break;
...
)
{
  ...
  WRRC_MSG2_HIGH("Pending SCRR: %d RRC State:%d",rrcdorm_int_data.saved_fast_dormancy_req.num_pending_dormancy_req,
      rrc_get_state(wrrc_as_id));

  if( (TRUE == rrcrb_is_srns_in_progress(wrrc_as_id)) 
      || (TRUE == rrcsmc_is_rlc_re_est_for_srns_active(wrrc_as_id))
      || (FALSE == rrcscrr_is_cn_domain_open(wrrc_as_id,RRC_PS_DOMAIN_CN_ID))
      || (TRUE == is_iho_w2g_active(wrrc_as_id))
      || (TRUE == rrcidt_is_idt_active(wrrc_as_id,RRC_PS_DOMAIN_CN_ID))
      || (TRUE == rrcidt_is_idt_active(wrrc_as_id,RRC_CS_DOMAIN_CN_ID))
      || (TRUE == rrccho_is_wtog_cco_active(wrrc_as_id))
      || (FALSE == rrcdormancy_check_if_nw_support_fast_dormancy(wrrc_as_id))
      || (TRUE == rrcscrr_is_cn_domain_open(wrrc_as_id,RRC_CS_DOMAIN_CN_ID))
    )
  {
    rrcdorm_int_data.saved_fast_dormancy_req.num_pending_dormancy_req = 0;
  ...
  switch(rrc_get_state(wrrc_as_id))
  {
    case RRC_STATE_URA_PCH:
    case RRC_STATE_CELL_PCH:
      WRRC_MSG1_HIGH("RRC State %d Aborting pending SCRR",rrc_get_state(wrrc_as_id));
      rrcdorm_int_data.saved_fast_dormancy_req.num_pending_dormancy_req = 0;
      break;
      
    case RRC_STATE_CELL_DCH:
    case RRC_STATE_CELL_FACH:
      if(RRCCU_STARTED == rrccu_get_cell_update_started_status(wrrc_as_id))
      {
      ...
      else 
      {
        ...
        /* Re try SCRI only if rrc_max_scri_for_fd is non zero */
          if(rrcdorm_int_data.max_scri_for_fd != 0)
          {
            rrcdormancy_send_scri_for_fast_dormancy(wrrc_as_id, FALSE);
          }
          status = TRUE;
        }
      }    
    ...
    default:
      WRRC_MSG1_HIGH("Pending SCRR:Invalid Event in rrc state: %d",rrc_get_state(wrrc_as_id));
      rrcdorm_int_data.saved_fast_dormancy_req.num_pending_dormancy_req = 0;
      break;
  }  
  rrcdorm_int_data.saved_fast_dormancy_req.num_pending_dormancy_req = 0;
...
)
{
  ...
  rrcdorm_int_data.max_scri_for_fd = RRC_DORMANCY_MAX_SCRI_RETRY;


  rlc_reset_activity_flag(wrrc_as_id);

  if((validation_status = rrcdormancy_validate_fast_dormancy_req(wrrc_as_id,cmd_ptr)) != 
     RRCDORMANCY_VALIDATION_SUCCESS)
  {
    rrcdormancy_reset_n_send_fast_dormancy_cnf(wrrc_as_id,
    rrcdormancy_map_internal_cause_to_fast_dormancy_cnf_cause(wrrc_as_id,validation_status)
    );
    return;
  }

  if(RRCDORMANCY_SUCCESS != rrcdormancy_save_fast_dormancy_req(wrrc_as_id,cmd_ptr))
  {
  ...
  /* If all validation goes well then process fast dormancy req and start T323 timer*/
  if(TRUE == rrcdormancy_process_fast_dormancy_req(wrrc_as_id))
  {
    if(TRUE == rrcdormancy_check_if_nw_support_fast_dormancy(wrrc_as_id))
    {
      rrcdormancy_stop_or_restart_timer_T323(wrrc_as_id,rrcdormancy_get_T323_timer_value(wrrc_as_id),
...
)
{
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_DOWNLINK_L2ACK_IND:
      if((rrcdorm_int_data.saved_fast_dormancy_req.mui_status == RRCDORMANCY_MUI_SCRI_CNF) && 
         (cmd_ptr->cmd.downlink_l2ack_ind.mui == rrcdorm_int_data.saved_fast_dormancy_req.mui))
      {
        WRRC_MSG3_HIGH("L2 ACK for SCRI Status:%d, mui_status:%d, mui=%d",
          cmd_ptr->cmd.downlink_l2ack_ind.status,
          rrcdorm_int_data.saved_fast_dormancy_req.mui_status,
          rrcdorm_int_data.saved_fast_dormancy_req.mui);
    ...
    case RRC_FAST_DORMANCY_REQ:
      WRRC_MSG2_HIGH("RRCDORM:Invalid Event: 0x%x for dormancy in substate:%d", 
          cmd_ptr->cmd_hdr.cmd_id,rrcdorm_int_data.substate);
      /* Do not process next dormancy req till first one complets */
      rrcdormancy_send_fast_dormancy_cnf(wrrc_as_id, RRC_FAST_DORMANCY_FAILURE_APP_RETRY);
      break;
...
)
{
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_CELL_UPDATE_COMPLETE_CNF:
      ...
      {
         WRRC_MSG3_HIGH("RRCDORM:No Dormancy to process substate:%d REQ:%d MUI:%d", 
            rrcdorm_int_data.substate,
            rrcdorm_int_data.saved_fast_dormancy_req.fast_dormancy_req_status,
            rrcdorm_int_data.saved_fast_dormancy_req.mui_status);
      }
      break;
...
)
{
  switch (cmd_ptr->cmd_hdr.cmd_id)
  {
    ...
    case RRC_FAST_DORMANCY_REQ:
      /* Do not process next dormancy req till first one complets */
      rrcdormancy_send_fast_dormancy_cnf(wrrc_as_id, RRC_FAST_DORMANCY_SUCCESS_APP_RETRY);
      break;
...
)
{
  ...
  {
    case RRC_FAST_DORMANCY_REQ:
      WRRC_MSG1_HIGH("DORMANCY_REQ rcvd in %d substate",rrcdorm_int_data.substate);
      rrcdormancy_validate_and_process_fast_dormancy_req(wrrc_as_id, cmd_ptr);
      break;
...
{
  switch (rrcdorm_int_data.substate)
  {
    case RRCDORMANCY_INITIAL:
      switch(cmd_ptr->cmd_hdr.cmd_id)
      {
        case RRC_FAST_DORMANCY_REQ:
          rrcdormancy_process_dormancy_initial_substate(wrrc_as_id, cmd_ptr);
          break;
    ...
    case RRCDORMANCY_WAIT_FOR_CU_COMP_CNF: 
      switch(cmd_ptr->cmd_hdr.cmd_id)
      {
        ...
        case RRC_CELL_UPDATE_COMPLETE_CNF:
        case RRC_FAST_DORMANCY_REQ:
          rrcdormancy_process_wait_for_cu_cnf_substate(wrrc_as_id, cmd_ptr);
          break;
    ...
    case RRCDORMANCY_WAIT_FOR_L2ACK: 
      switch(cmd_ptr->cmd_hdr.cmd_id)
      {
        ...
        case RRC_DOWNLINK_L2ACK_IND:
        case RRC_FAST_DORMANCY_REQ:
          rrcdormancy_process_wait_for_l2_ack_substate(wrrc_as_id, cmd_ptr);
          break;
    ...
    case RRCDORMANCY_WAIT_FOR_SIG_CON_REL_COMPL:
      switch(cmd_ptr->cmd_hdr.cmd_id)
      {
        case RRC_SIG_CONN_RELEASE_CNF:
        case RRC_FAST_DORMANCY_REQ:
          rrcdormancy_process_wait_for_sig_con_rel_compl_substate(wrrc_as_id, cmd_ptr);
          break;
...
)
{
  ...
  WRRC_MSG2_HIGH_OPT("function rrcdormancy_procedure_event_handler is called in rrc_state = rrc_state_e_type_value%d and rrcdormancy_substate = rrcdorm_substate_e_type_value%d", rrc_state, rrcdorm_int_data.substate);
  rrc_crash_debug_save_proc_substates(wrrc_as_id, RRC_PROCEDURE_DORMANCY,rrcdorm_int_data.substate,cmd_ptr->cmd_hdr.cmd_id);

  if(RRC_STATE_CHANGE_IND == cmd_ptr->cmd_hdr.cmd_id)
  {
    WRRC_MSG3_HIGH("RRCDORM:State-Change-Ind dorm-state:%d rrc-state:%d new-state:%d",
       rrcdorm_int_data.substate,rrc_state,cmd_ptr->cmd.state_change_ind.new_state);
 ...
 #ifdef FEATURE_DUAL_SIM    
      mm_cmd_ptr->cmd.rrc_pch_state_ind.as_id = rrc_get_as_id(wrrc_as_id);
#endif
    ...
    switch (cmd_ptr->cmd.state_change_ind.new_state)
    {
      ...
      case RRC_STATE_CELL_FACH:
         ...
         */
        if((rlc_query_data_channel_activity(wrrc_as_id) == NO_DATA_TRANSFERRED) &&
           (rrc_check_psrab_data_is_present(wrrc_as_id) == FALSE))
        {
          WRRC_MSG1_HIGH("RRCDORM:Value of max_scri %d",rrcdorm_int_data.max_scri_for_fd);
          if(rrcdorm_int_data.saved_fast_dormancy_req.num_pending_dormancy_req != 0)
          {
            WRRC_MSG1_HIGH("RRCDORM: Pending Dormancy requests:%d in new FACH state",rrcdorm_int_data.saved_fast_dormancy_req.num_pending_dormancy_req);
            if(TRUE == rrcdormancy_check_if_nw_support_fast_dormancy(wrrc_as_id))
            {
              remaining_value = rrctmr_get_remaining_time(wrrc_as_id, RRCTMR_T_323_TIMER);
              if(remaining_value == 0)
              {
                rrctmr_start_timer(wrrc_as_id, RRCTMR_T_323_TIMER, RRC_DORMANCY_T323_RETRY_TIMER);
              }
              return;
            }
            else
      ...
      default:
        WRRC_MSG3_HIGH("Unexpected StateChangeInd dorm-state:%d rrc-state:%d new-state:%d",
          rrcdorm_int_data.substate,rrc_state,cmd_ptr->cmd.state_change_ind.new_state);
    }
  }
  else if(RRC_T323_EXPIRED_IND == cmd_ptr->cmd_hdr.cmd_id)
  {
    rrcdormancy_process_T323_expiry(wrrc_as_id, cmd_ptr);
  }
  else
  {
    switch (rrc_state)
    {
      ...
      case RRC_STATE_CELL_FACH:    
        rrcdormancy_process_cell_fach_state(wrrc_as_id,cmd_ptr); 
        break;

      case RRC_STATE_CELL_DCH:  
        rrcdormancy_process_cell_dch_state(wrrc_as_id,cmd_ptr); 
        break;

      case RRC_STATE_CELL_PCH:
      case RRC_STATE_URA_PCH:
        rrcdormancy_process_cell_ura_pch_state(wrrc_as_id, cmd_ptr);
        break;

      case RRC_STATE_DISCONNECTED:
      case RRC_STATE_CONNECTING:
        rrcdormancy_process_idle_state(wrrc_as_id, cmd_ptr);
        break;
...
===========================================================================*/
void rrc_dormancy_init(sys_modem_as_id_e_type wrrc_as_id)
{
 ...
 /* Also send registration to state change manager. We need to know when
  ...
  the BCCH status. */
  rrcscmgr_register_for_scn(wrrc_as_id, RRC_PROCEDURE_DORMANCY,  /* Procedure name */
...
FUNCTION rrcdormancy_reset_pending_dormancy_req
...
===========================================================================*/
void rrcdormancy_reset_pending_dormancy_req(sys_modem_as_id_e_type wrrc_as_id)
{
  rrcdorm_int_data.saved_fast_dormancy_req.num_pending_dormancy_req = 0;
}
...
{
  qsh_mdump_collect_high(&rrcdorm_int_data,sizeof(rrcdorm_int_data));
}
#endif
