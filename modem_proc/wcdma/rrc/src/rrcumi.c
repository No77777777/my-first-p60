...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcumi.c#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
12/26/16   nr      Made change to copy HRNTI received in UMI message to efach hrnti structure in TOC 
01/12/16   nr      Made changes to report failure events to QSH
09/08/15   sas     Made changes to update LAC,PLMN info to registered clients after UMI.
05/04/15   sp      Made changes to clear UMI procedure if IP check fails for UMI
04/22/15   sp      Made changes to update correct failure cause in the UMI complete
...
typedef struct {
  ...
  rrcumi_substate_e_type                     rrcumi_substate;
  rrcumi_ue_timer_const_rcvd_status_e_type   ue_timer_const_rcvd_status;
...
rrcumi_int_data_type umi_sim[MAX_RRC_STACKS];
#define umi umi_sim[WAS_ID_M]
#else
...
)
{
   ...
   */
  if(umi.ue_timer_const_rcvd_status==RRCUMI_UE_TIMER_CONST_RCVD)
  {
...
)
{
  ...
  /*Reports utran mobility information failure to QSH*/
  rrc_qsh_report_failure_ota_msg_event(wrrc_as_id,WRRC_UL_DCCH_MESSAGETYPE_UTRANMOBILITYINFORMATIONFAILURE);
#endif

#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
  rrcqsh_log_uplink_dcch_ota_params(wrrc_as_id, WRRC_UL_DCCH_MESSAGETYPE_UTRANMOBILITYINFORMATIONFAILURE, ptr);
#endif
  ...
  if (ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
    WRRC_MSG1_HIGH("Sending UMI Failure with cause %d", 
             ptr->message.u.utranMobilityInformationFailure.failureCause.t);
    status = rrcsend_send_ul_sdu(wrrc_as_id,RRC_PROCEDURE_UMI,  /* Procedure is UMI    */
...
===========================================================================*/
static void rrcumi_clear_procedure(sys_modem_as_id_e_type wrrc_as_id)
{
...
)
{
  ...
  if (umi.umi_for_srns_relocation == TRUE)
  {
    ...
    if (SUCCESS != rrcsmc_append_start_list(wrrc_as_id,&msg_ptr->message.u.utranMobilityInformationConfirm.
...
)
{
 ...
 /* Ensure that LCM found a valid logical channel id
   ...
   *and this is exerned.
   */  
    rrcumi_build_utran_mobility_info_confirm_msg(wrrc_as_id,&umi.utran_mob_inf_cnf, msg_ptr);

   /* Send the UTRAN Mobility Info on the Uplink DCCH
    */
    if (umi.umi_for_srns_relocation == TRUE)
    {
      send_status = rrcsend_send_ul_sdu(wrrc_as_id,RRC_PROCEDURE_UMI,
                                          msg_ptr,
                                          ul_lc_id,
                                        RRCSEND_L2ACK_REQUIRED);

      umi.curr_substate = RRCUMI_WAIT_FOR_L2_ACK;
      umi.rrcumi_substate = RRCUMI_WAIT_FOR_L2_ACK; 
    }
    else
    {
      send_status = rrcsend_send_ul_sdu(wrrc_as_id,RRC_PROCEDURE_UMI,
      ...
      umi.rrcumi_substate = RRCUMI_INITIAL;
      rrcumi_clear_procedure(wrrc_as_id);
    }
...
===========================================================================*/
static void rrcumi_process_rnti_info ( sys_modem_as_id_e_type wrrc_as_id )
{
  ...
  if ((umi.new_urnti_valid == TRUE) || (umi.new_crnti_valid == TRUE))
  {
    ...
    umi.rrcumi_substate = RRCUMI_WAIT_FOR_RNTI_UPDATE_CNF;
  ...
  else
  {
    ...
    rrcumi_send_utran_mobility_info_confirm( wrrc_as_id  );
  }
}
  ...
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_RNTI_UPDATE_CNF:
      ...
      else
      {
        ...
        rrcumi_send_umi_failure(wrrc_as_id,umi.utran_mob_inf_cnf.transaction_id, failure_status,protocolErrorCause);
    ...
    default:
      WRRC_MSG1_HIGH("Invalid message rcvd in UMI substate: %d", umi.rrcumi_substate);
      break;
  }
}
/*===========================================================================
...
)
{
  ...
  message_t = rrc_get_dl_dcch_message_type(wrrc_as_id,cmd_ptr->cmd.downlink_sdu_ind.dl_sdu,
  ...
  if(dcch_msg_ptr == NULL) /* DCCH ASN1 decoding is un-successful */
  {
    ...
    rrcumi_send_umi_failure (wrrc_as_id,transaction_id, failure_status,protocolErrorCause);
  ...
  { 
...
#ifdef FEATURE_WCDMA_HS_RACH
    if(TRUE == rrccu_get_hsrach_sib_change(wrrc_as_id))
    {
      WRRC_MSG3_HIGH("Delay CU flag is set. Do not handle UMI. Send UMI failure",0,0,0);
            /* Set the failure status */
      failure_status = RRCUMI_MSG_INVALID_CONFIG;
      /* Send UMI failure */
      rrcumi_send_umi_failure (wrrc_as_id,transaction_id,
                               failure_status,
    ...
    {
      primary_plmn_Id_present = rrcumi_check_and_get_primary_plmn_id(wrrc_as_id,msg_ptr,
    ...
    if (RRCUMI_SUCCESS != rrcumi_validate_utran_mobility_info_command(wrrc_as_id,dcch_msg_ptr) )
    {
    ...
    if (TRUE == rrcumi_validate_srns_relocation_info(wrrc_as_id,msg_ptr))
    {
    ...
    else if(rrcumi_validate_umi_ip_mode(wrrc_as_id,msg_ptr) == TRUE)
    {
      ip_check_status = rrcsmc_check_integrity_for_srns_relocation(wrrc_as_id,cmd_ptr, msg_version);
  ...
  ue_conn_timers_and_constants_v860ext_ptr = NULL;
    ...
    if(msg_version == MSG_REL99)
    {
      ...
      {
        umi.ue_timer_const_rcvd_status = RRCUMI_UE_TIMER_CONST_RCVD;
    ...
    {    
      (void)rrc_update_ue_conn_timers_and_constants
                  (wrrc_as_id,ue_conn_timers_and_constants_ptr,ext_ptr,
      ...
      if(rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5)
      {
        ...
        {  
          (void)rrc_update_constants_n312_and_n315_r5(wrrc_as_id,&(msg_ptr->u.
              later_than_r3.criticalExtensions.u.r5.utranMobilityInformation_r5.
              ue_ConnTimersAndConstants));
        } 
        else if(msg_version == MSG_REL7)
        {  
          (void)rrc_update_constants_n312_and_n315_r5(wrrc_as_id,&(r7_umi_msg_ptr->ue_ConnTimersAndConstants));
        }
      }
      ...
      if(rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH)
      {
        ...
        umi_info.N313 = rrc_translate_n313(wrrc_as_id,RRC_GET_N313());
    ...
    */
    umi.utran_mob_inf_cnf.transaction_id = transaction_id;

    /* Get the cn information if present and update upper layers */
    if(core_network_info != NULL)
    {
      if( rrc_ccm_send_nas_cn_info_full(wrrc_as_id,core_network_info,
      ...
      if(rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_TRM_PRIORITY_CHANGE_TIMER))
      {
    ...
    {
      rrc_translate_crnti(wrrc_as_id,msg_crnti_ptr, &umi.new_crnti);
    ...
    {
      rrc_translate_urnti(wrrc_as_id,msg_urnti_ptr, &umi.new_urnti);
      umi.new_urnti_valid = TRUE;
    }
    /*check if a new h_rnti is present and update rnti_update_req struct*/
    if(msg_hrnti_ptr != NULL)
    {
      (void)rrc_translate_hrnti(wrrc_as_id,msg_hrnti_ptr, &umi.new_hrnti);
      umi.new_hrnti_valid = TRUE;
#ifdef FEATURE_WCDMA_HS_FACH
    if ((TRUE == rrcenhstate_camped_cell_supports_e_fach(wrrc_as_id))
     &&((rrc_state == RRC_STATE_CELL_FACH))
     )
      {
        ...
        /*For HSFACH case, HRNTI received in UMI message needs to be copied to TOC. OC will be updated with TOC during channel config within FACH*/
        transition_config.toc_ptr->efach_hrnti          = umi.new_hrnti;
        ...
        rrc_set_hrnti_status(wrrc_as_id,ordered_config_ptr, HRNTI_VALID);
        rrc_set_hrnti_status(wrrc_as_id,transition_config.toc_ptr, HRNTI_VALID);
      }
#endif
    }
    ...
    if (umi.umi_for_srns_relocation == TRUE)
    {
      ...
      rrcrb_send_rlc_re_est_req_to_smc(wrrc_as_id,RRC_PROCEDURE_UMI, rb_type, TRUE);

      umi.curr_substate = RRCUMI_WAIT_FOR_REESTABLISH_SRB2;
      umi.rrcumi_substate = RRCUMI_WAIT_FOR_REESTABLISH_SRB2;

      rrcdormancy_update_dormancy_for_srns_hho_cco(wrrc_as_id, RRC_PROCEDURE_UMI);
    }
    else
    { /*normal reconfiguration without SRNS relocation */   
      /* Send the UMI Complete message */
      rrcumi_process_rnti_info(wrrc_as_id);
    }
          ...
          v860NonCriticalExtensions.utranMobilityInformation_v860ext.m.dedicatedPriorityInformationPresent))
    {
      ...
      rrcmeas_update_dedicated_priority_info_from_ota(wrrc_as_id,ded_pri_info_ptr);

      /*Post the command to measurement, so that meas can send the meas_req to WL1*/
      rrcmeas_post_dedicated_priority_change_indication(wrrc_as_id);
    }
#endif
          ...
          v860NonCriticalExtensions.utranMobilityInformation_v860ext.m.newPrimary_E_RNTIPresent))
    {
      ...
      (void)rrc_translate_ernti(wrrc_as_id,&dcch_msg_ptr->message.u.utranMobilityInformation.u.later_than_r3.criticalExtensions.u.criticalExtensions.u.r7.
    ...
    {
      rrcllc_establish_srb_rab_in_efach(wrrc_as_id,RRC_E_FACH_DEDICATED,RRC_PROCEDURE_UMI,FALSE);
    }
#endif
  }
} /* rrcumi_process_utran_mobility_info_command */
...
)
{
  ...
  if ( ptr->status == SUCCESS )
  {
    ...
    if (umi.umi_for_srns_relocation)
    {
      ...
      rrcrb_send_rlc_re_est_req_to_smc(wrrc_as_id,RRC_PROCEDURE_UMI, rb_type, FALSE);
    }
    else
    {
  ...
  else
  {
    ...
    (void)rrc_transition_to_disconnected_state(wrrc_as_id,RRC_PROCEDURE_UMI, RRC_TX_TO_DISCON_OTHER);
  }
...
)
{
  ...
  switch (umi.rrcumi_substate)
  {
    case RRCUMI_INITIAL:
      switch(cmd_ptr->cmd_hdr.cmd_id)
      {
        case RRC_DOWNLINK_SDU_IND:
          rrcumi_process_utran_mobility_info_command(wrrc_as_id, cmd_ptr);
          break;

        default:
          WRRC_MSG2_HIGH("Invalid event: %d in Substate: %d",
                   cmd_ptr->cmd_hdr.cmd_id,umi.rrcumi_substate);
          break;
      }
      break;

    case RRCUMI_WAIT_FOR_RNTI_UPDATE_CNF:
      rrcumi_process_rnti_update_cnf_substate(wrrc_as_id, cmd_ptr);
      break;

    case RRCUMI_WAIT_FOR_REESTABLISH_SRB2:
      rrcumi_process_rlc_reestablishment_cnf(wrrc_as_id);
      break;
    ...
    case RRCUMI_WAIT_FOR_L2_ACK:
      if (cmd_ptr->cmd_hdr.cmd_id == RRC_DOWNLINK_L2ACK_IND )
      {
        if (umi.umi_for_srns_relocation == TRUE)
        {
          rrcumi_process_l2_ack_ind(wrrc_as_id,&(cmd_ptr->cmd.downlink_l2ack_ind));
        }
        else
        {
    ...
    default:
      WRRC_MSG1_HIGH("Invalid Substate for UMI: %d", umi.rrcumi_substate);
      break;
  }
}
...
)
{
  ...
  WRRC_MSG2_HIGH_OPT("function rrcumi_procedure_event_handler is called in rrc_state = %d and rrcumi_substate = %d", rrc_state, umi.rrcumi_substate);
  rrc_crash_debug_save_proc_substates(wrrc_as_id,RRC_PROCEDURE_UMI,umi.rrcumi_substate,cmd_ptr->cmd_hdr.cmd_id);
  ...
  if(cmd_ptr->cmd_hdr.cmd_id == RRC_STATE_CHANGE_IND)
  {
    ...
    umi.new_urnti_valid         = FALSE;
    umi.new_hrnti_valid         = FALSE;
    umi.ue_timer_const_rcvd_status  = RRCUMI_UE_TIMER_CONST_NOT_RCVD;
    return;
  }
#ifdef FEATURE_DUAL_SIM
  ...
  if((rrc_csp_waiting_to_release_lock(wrrc_as_id)) && (cmd_ptr->cmd_hdr.cmd_id == RRC_DOWNLINK_SDU_IND))
  {
    WRRC_MSG3_HIGH("CSP is waiting to release lock. UMI is ignored", 0,0, 0);
    return;
  }
#endif
  switch (rrc_state)
  {
    ...
    rrcumi_process_cell_fach_state(wrrc_as_id,cmd_ptr); 
    break;
    ...
    case RRC_STATE_DISCONNECTED: 
      ...
      rrcumi_process_disconnected_state(wrrc_as_id, cmd_ptr);
      break;

  case RRC_STATE_CELL_DCH:
    rrcumi_process_cell_dch_state(wrrc_as_id, cmd_ptr);
    break;
    ...
    default: 
      ...
      WRRC_ERR1_FATAL("RRCUMI Msg received in Invalid State: %d",rrc_state);
      break;
   }  
} /* rrcumi_procedure_event_handler */
/*===========================================================================
...
)
{
  ...
  umi.new_urnti_valid = FALSE;
  umi.new_hrnti_valid = FALSE;
  rrcscmgr_register_for_scn(wrrc_as_id,RRC_PROCEDURE_UMI,
                            RRC_STATE_WILDCARD,
...
none
===========================================================================*/
uint8  rrcumi_get_current_substate(sys_modem_as_id_e_type wrrc_as_id)
{
  return ((uint8)umi.rrcumi_substate);
...
===========================================================================*/
  ...
  __attribute__((section(".uncompressible.text")))
#endif
rrcumi_substate_e_type rrcumi_return_substate(sys_modem_as_id_e_type wrrc_as_id)
{
...
===========================================================================*/
void rrcqsh_log_umi_ota_params(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  ((wrrc_qsh_metric_ota_s*)rrc_qsh_metric_db[WRRC_QSH_METRIC_OTA].data_ptr)->ota_info.umi_ota_params.timer_const_rcvd = umi.ue_timer_const_rcvd_status; 
}
#endif
...
{
  qsh_mdump_collect_high(&umi,sizeof(umi));
}
#endif