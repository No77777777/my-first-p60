...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrccspdb.c#3 $    $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/31/17   ha      FR42544 MPLMN periodic scan search
06/09/17   sas     Sharing of ACQ DB to alternate sub when ACQ is successful on existing acq db entry.
...
03/10/16   sn      Changes to remove end line character from EFS file names 
09/28/15   sn      Changes to fix logging issues for LAF
06/15/15   sn      Changes for new log packets for LAF
06/03/15   sn      Changes to fill RAT information in Finger printing log packet 
05/28/15   bc      Changes to share SIBDB and ACQDB among two SUBs in a W+W solution
05/11/15   sn      WCDMA RRC changes for FR 24494
04/15/15   sp      Remove unwanted/frequent message highs
03/10/15   sas     Added api interface fun to provide acqdb cell list info to LTE.
02/19/15   sas     Update ACQ-DB when SIB reading is skipped for VPLMN.
...
rrc_csp_acq_entry_type  list_db_sim[MAX_RRC_STACKS][MAX_ACQ_DB_ENTRIES];
...
** ----------------------------------------------------------------------- */
#ifdef FEATURE_DUAL_WCDMA 
LOCAL rrccspdb_int_data_type rrccspdb_int_data_sim[MAX_RRC_STACKS];
#define rrccspdb_int_data rrccspdb_int_data_sim[WAS_ID_M]
#else
LOCAL rrccspdb_int_data_type rrccspdb_int_data;
...
NOTE
                  ...
                  on powerup.
===========================================================================*/
void  rrc_csp_init_acq_db(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8  i;

  /* Initialize Current Read Index to -1 */
  rrccspdb_int_data.stored_acq_db.curr_rd_index = -1;

  /* Initialize Current Number of Pointers to 0 */
  rrccspdb_int_data.stored_acq_db.curr_wr_index = 0;

  rrccspdb_int_data.stored_acq_db.last_entry_invalid = TRUE;
  ...
  {
    rrccspdb_int_data.stored_acq_db.acq_list_indices[i] = i;
  }
  
#ifdef FEATURE_FEMTO_CSG
  /* Initialize Current Read Index to -1 */
  rrccspdb_int_data.stored_acq_db_temp.curr_rd_index = -1;

  /* Initialize Current Number of Pointers to 0 */
  rrccspdb_int_data.stored_acq_db_temp.curr_wr_index = 0;

  rrccspdb_int_data.stored_acq_db_temp.last_entry_invalid = TRUE;

  /* Initialize all values in ordered acquisition list in acq database to 
   */
  for (i=0; i < MAX_ACQ_DB_ENTRIES_TEMP; i++)
  {
    rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i] = i;
  }
#endif
}  /* rrc_csp_init_acq_db */
...
===========================================================================*/
static void rrc_csp_fill_acq_entry(sys_modem_as_id_e_type wrrc_as_id,rrc_csp_acq_entry_type * acq_ptr_1, rrc_csp_acq_entry_type* acq_ptr_2)
{
...
NOTE
                  None.
===========================================================================*/
static void rrc_csp_efs_convert_to_acq_list(sys_modem_as_id_e_type wrrc_as_id,const rrc_csp_acq_entry_efs_type *efs_db,
                                          rrc_csp_acq_entry_type *list_db_arg,
                                          uint8 index)
{
  uint8 i, j;
  ...
  {
    WCDMA_MEMCPY(&list_db_arg[i].plmn_id, 
                 sizeof(rrc_plmn_identity_type),
                 &efs_db[j].plmn_id, 
                 sizeof(rrc_plmn_identity_type));

    list_db_arg[i].freq = efs_db[j].freq;
#ifdef FEATURE_MCC_AND_850_800_BAND_COMBINATION
    list_db_arg[i].band_class = efs_db[j].band_class;
#else
    list_db_arg[i].band_class = rrc_get_frequency_band(wrrc_as_id,list_db[i].freq, list_db[i].plmn_id);
#endif
    list_db_arg[i].status_mask = efs_db[j].status_mask;
    list_db_arg[i].cell_id = efs_db[j].cell_id;
    list_db_arg[i].scr_code = efs_db[j].scr_code;
    list_db_arg[i].lac = efs_db[j].lac;
    list_db_arg[i].csg_id = efs_db[j].csg_id;

    list_db_arg[i].fp_w_macro_cnt = efs_db[j].fp_w_macro_cnt;
    list_db_arg[i].w_macro_write_idx = efs_db[j].w_macro_write_idx;
    WCDMA_MEMCPY(&list_db_arg[i].w_macro,
                 sizeof(list_db_arg[i].w_macro),
                 &efs_db[j].w_macro,
                 sizeof(efs_db[j].w_macro));
 
    list_db_arg[i].fp_lte_macro_cnt = efs_db[j].fp_lte_macro_cnt;
    list_db_arg[i].lte_macro_write_idx = efs_db[j].lte_macro_write_idx;
    WCDMA_MEMCPY(&list_db_arg[i].lte_macro,
                 sizeof(list_db_arg[i].lte_macro),
                 &efs_db[j].lte_macro,
...
NOTE
                  None.
===========================================================================*/
static void rrc_csp_acq_list_convert_to_efs(sys_modem_as_id_e_type wrrc_as_id,rrc_csp_acq_entry_efs_type *efs_db,
                                         const rrc_csp_acq_entry_type *list_db_arg,
                                         uint8 index)
{
uint8 i, j;
  ...
  for (j = 0,i = index; i < index+MAX_ACQ_LIST_ENTRIES_EFS; i++,j++)
  {
    WCDMA_MEMCPY(&efs_db[j].plmn_id,
                 sizeof(rrc_plmn_identity_type),
                 &list_db_arg[i].plmn_id,
                 sizeof(rrc_plmn_identity_type));

    efs_db[j].freq = list_db_arg[i].freq;
#ifdef FEATURE_MCC_AND_850_800_BAND_COMBINATION
    efs_db[j].band_class = list_db_arg[i].band_class;
#endif
    efs_db[j].status_mask = list_db_arg[i].status_mask;
    efs_db[j].cell_id = list_db_arg[i].cell_id;
    efs_db[j].scr_code = list_db_arg[i].scr_code;
    efs_db[j].lac = list_db_arg[i].lac;
    efs_db[j].csg_id = list_db_arg[i].csg_id;

    efs_db[j].fp_w_macro_cnt = list_db_arg[i].fp_w_macro_cnt;
    efs_db[j].w_macro_write_idx = list_db_arg[i].w_macro_write_idx;
    WCDMA_MEMCPY(efs_db[j].w_macro,
                 sizeof(efs_db[j].w_macro),
                 list_db_arg[i].w_macro,
                 sizeof(list_db_arg[i].w_macro));
 
    efs_db[j].fp_lte_macro_cnt = list_db_arg[i].fp_lte_macro_cnt;
    efs_db[j].lte_macro_write_idx = list_db_arg[i].lte_macro_write_idx;
    WCDMA_MEMCPY(efs_db[j].lte_macro,
                 sizeof(efs_db[j].lte_macro),
                 list_db_arg[i].lte_macro,
                 sizeof(list_db_arg[i].lte_macro));
    WRRC_MSG7_MED(
         "CSG: Entry %d, FPW: %d, FPL: %d, CSG ID %d, cell ID %d, band class %d, freq %d",
         i,efs_db[j].fp_w_macro_cnt,efs_db[j].fp_lte_macro_cnt,efs_db[j].csg_id,efs_db[j].cell_id,
         efs_db[j].band_class,efs_db[j].freq);
...
===========================================================================*/
void rrc_csp_recover_acq_db(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8 i;
  uint8 k = 0;
...
/* First we will find out the number of valid indices available */
  ...
  {
    if(rrccspdb_int_data.stored_acq_db.acq_list_indices[i] > MAX_ACQ_DB_INDEX)
    {
      WRRC_MSG1_ERROR("Stored Acq Db has a corrupted ACQ List index %d",rrccspdb_int_data.stored_acq_db.acq_list_indices[i]);
    }
    else
    {
      valid_list_indices[num_valid_idxs] = rrccspdb_int_data.stored_acq_db.acq_list_indices[i];
      num_valid_idxs++;
    }

    /*check all the valid indices*/
    if((i < (rrccspdb_int_data.stored_acq_db.curr_wr_index + !rrccspdb_int_data.stored_acq_db.last_entry_invalid))&&
       (RRC_CSP_BAND_MAX == rrc_get_frequency_band(wrrc_as_id,rrccspdb_int_data.acq_list[i].freq,rrccspdb_int_data.acq_list[i].plmn_id))) 
    {
      WRRC_MSG3_ERROR("Detected invalid frequency in ACQ DB index %d, freq %d",i,rrccspdb_int_data.acq_list[i].freq,0);
      memset(&rrccspdb_int_data.acq_list[i],0,sizeof(rrc_csp_acq_entry_type));
    }
  }
...
/* Replace the corrupt values with the missed entries */
  for( i=0; i< acqdb_size; i++ )
  {
    if(rrccspdb_int_data.stored_acq_db.acq_list_indices[i] > MAX_ACQ_DB_INDEX)
    {
      rrccspdb_int_data.stored_acq_db.acq_list_indices[i]= missed_list_indices[num_invalid_idxs++];
      WRRC_MSG2_ERROR("Replacing the corrupted location %d of acq_list_indices with value %d",i,rrccspdb_int_data.stored_acq_db.acq_list_indices[i]);
    }
  }
...
NOTE
                  None.
===========================================================================*/
void  rrc_csp_read_nv_acq_info(sys_modem_as_id_e_type wrrc_as_id)
{
  int  i;
  int  k=0;
...
#ifdef FEATURE_FEMTO_CSG
  strlcpy(rrccspdb_int_data.wcdma_csg_efs_list_2, "/nv/item_files/conf/wcdma_csg_efs_2.conf", sizeof(rrccspdb_int_data.wcdma_csg_efs_list_2));

  strlcpy(rrccspdb_int_data.wcdma_csg_efs_list_3, "/nv/item_files/conf/wcdma_csg_efs_3.conf", sizeof(rrccspdb_int_data.wcdma_csg_efs_list_3));

  strlcpy(rrccspdb_int_data.wcdma_csg_efs_list_4, "/nv/item_files/conf/wcdma_csg_efs_4.conf", sizeof(rrccspdb_int_data.wcdma_csg_efs_list_4));

  strlcpy(rrccspdb_int_data.wcdma_csg_efs_list_5, "/nv/item_files/conf/wcdma_csg_efs_5.conf", sizeof(rrccspdb_int_data.wcdma_csg_efs_list_5));
#endif

  strlcpy(rrccspdb_int_data.wcdma_csg_efs_list_1, "/nv/item_files/conf/wcdma_csg_efs_1.conf", sizeof(rrccspdb_int_data.wcdma_csg_efs_list_1));

  strlcpy(rrccspdb_int_data.wcdma_csg_efs_db, "/nv/item_files/conf/wcdma_csg_efs_db.conf", sizeof(rrccspdb_int_data.wcdma_csg_efs_db));

#ifndef FEATURE_FEMTO_CSG
  WRRC_MSG0_HIGH("Reading ACQ DB header from EFS");
  if(!rrc_get_from_efs(wrrc_as_id,rrccspdb_int_data.wcdma_csg_efs_db,&rrccspdb_int_data.stored_acq_db, sizeof(rrc_csp_acq_db_type)))
  {
    WRRC_MSG0_ERROR("CSG: Error reading ACQ DB header initialize now");
    rrc_csp_init_acq_db(wrrc_as_id);
  }
    
  /* Do a Sanity check - check read and write index. */
  if ((rrccspdb_int_data.stored_acq_db.curr_wr_index > MAX_ACQ_DB_INDEX) || 
      (rrccspdb_int_data.stored_acq_db.curr_rd_index > MAX_ACQ_DB_INDEX) ||
      (rrccspdb_int_data.stored_acq_db.curr_rd_index < -1) ||
      (rrccspdb_int_data.stored_acq_db.curr_rd_index > rrccspdb_int_data.stored_acq_db.curr_wr_index)||
      (rrccspdb_int_data.stored_acq_db.curr_wr_index < 0))
  {
    WRRC_MSG2_ERROR("Acq db invalid, Wr index %d Rd index %d",
                rrccspdb_int_data.stored_acq_db.curr_wr_index, rrccspdb_int_data.stored_acq_db.curr_rd_index);
  ...
  if( NV_DONE_S ==  (rrc_get_nv(wrrc_as_id, NV_ACQ_LIST_1_I, (nv_item_type *) &rrccspdb_int_data.nv_acq_list1 )))
  {
    /* Read the acq entries from the NV_ACQ_LIST_1 */
    for( i=0; i< MAX_NV_ACQ_DB_ENTRIES1; i++ )
    {
      rrccspdb_int_data.acq_list[k].plmn_id.mcc[0] = rrccspdb_int_data.nv_acq_list1.acq_entry[i].plmn_id.mcc[0];
      rrccspdb_int_data.acq_list[k].plmn_id.mcc[1] = rrccspdb_int_data.nv_acq_list1.acq_entry[i].plmn_id.mcc[1];
      rrccspdb_int_data.acq_list[k].plmn_id.mcc[2] = rrccspdb_int_data.nv_acq_list1.acq_entry[i].plmn_id.mcc[2];
      rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits = rrccspdb_int_data.nv_acq_list1.acq_entry[i].plmn_id.num_mnc_digits;
      if(rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits > RRC_MAX_MNC_DIGITS)
      {
        WRRC_MSG0_ERROR("NV_ACQ_LIST_1_I has invalid num_mnc_digits");
        rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits = RRC_MAX_MNC_DIGITS;
      }
      for(j=0; j<rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits; j++)
      {
        rrccspdb_int_data.acq_list[k].plmn_id.mnc[j] = rrccspdb_int_data.nv_acq_list1.acq_entry[i].plmn_id.mnc[j];
      }

      rrccspdb_int_data.acq_list[k].freq = (uint16)rrccspdb_int_data.nv_acq_list1.acq_entry[i].freq;
#ifdef FEATURE_MCC_AND_850_800_BAND_COMBINATION
      rrccspdb_int_data.acq_list[k].band_class = (uint16)rrccspdb_int_data.nv_acq_list1.acq_entry[i].band_class;
#else
      /* Get the band_class based on the frequency */
      rrccspdb_int_data.acq_list[k].band_class = rrc_get_frequency_band(wrrc_as_id,rrccspdb_int_data.acq_list[k].freq, rrccspdb_int_data.acq_list[k].plmn_id);
#endif
      rrccspdb_int_data.acq_list[k].status_mask = (uint16)rrccspdb_int_data.nv_acq_list1.acq_entry[i].status_mask;
      rrccspdb_int_data.acq_list[k].cell_id = rrccspdb_int_data.nv_acq_list1.acq_entry[i].cell_id;
      rrccspdb_int_data.acq_list[k].scr_code = (uint16)rrccspdb_int_data.nv_acq_list1.acq_entry[i].scr_code;
      rrccspdb_int_data.acq_list[k++].lac = (uint8)rrccspdb_int_data.nv_acq_list1.acq_entry[i].lac;
    }
  }
  else
  {
  ...
  if( NV_DONE_S ==  (rrc_get_nv(wrrc_as_id, NV_ACQ_LIST_2_I, (nv_item_type *) &rrccspdb_int_data.nv_acq_list2 )))
  {
    for( i=0; i< MAX_NV_ACQ_DB_ENTRIES1; i++ )
    {
      rrccspdb_int_data.acq_list[k].plmn_id.mcc[0] = rrccspdb_int_data.nv_acq_list2.acq_entry[i].plmn_id.mcc[0];
      rrccspdb_int_data.acq_list[k].plmn_id.mcc[1] = rrccspdb_int_data.nv_acq_list2.acq_entry[i].plmn_id.mcc[1];
      rrccspdb_int_data.acq_list[k].plmn_id.mcc[2] = rrccspdb_int_data.nv_acq_list2.acq_entry[i].plmn_id.mcc[2];
      rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits = rrccspdb_int_data.nv_acq_list2.acq_entry[i].plmn_id.num_mnc_digits;
      if(rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits > RRC_MAX_MNC_DIGITS)
      {
      ...
      for(j=0; j<rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits; j++)
      {
        rrccspdb_int_data.acq_list[k].plmn_id.mnc[j] = rrccspdb_int_data.nv_acq_list2.acq_entry[i].plmn_id.mnc[j];
      }

      rrccspdb_int_data.acq_list[k].freq = (uint16)rrccspdb_int_data.nv_acq_list2.acq_entry[i].freq;
#ifdef FEATURE_MCC_AND_850_800_BAND_COMBINATION
      rrccspdb_int_data.acq_list[k].band_class = (uint16)rrccspdb_int_data.nv_acq_list2.acq_entry[i].band_class;
#else
      /* Get the band_class based on the frequency */
      rrccspdb_int_data.acq_list[k].band_class = rrc_get_frequency_band(wrrc_as_id,rrccspdb_int_data.acq_list[k].freq, rrccspdb_int_data.acq_list[k].plmn_id);
#endif
      rrccspdb_int_data.acq_list[k].status_mask = (uint16)rrccspdb_int_data.nv_acq_list2.acq_entry[i].status_mask;
      rrccspdb_int_data.acq_list[k].cell_id = rrccspdb_int_data.nv_acq_list2.acq_entry[i].cell_id;
      rrccspdb_int_data.acq_list[k].scr_code = (uint16)rrccspdb_int_data.nv_acq_list2.acq_entry[i].scr_code;
      rrccspdb_int_data.acq_list[k++].lac = (uint8)rrccspdb_int_data.nv_acq_list2.acq_entry[i].lac;
    }
  }
  else
  {
  ...
  if( NV_DONE_S ==  (rrc_get_nv(wrrc_as_id, NV_ACQ_LIST_3_I, (nv_item_type *) &rrccspdb_int_data.nv_acq_list3 )))
  {
    for( i=0; i< MAX_NV_ACQ_DB_ENTRIES2; i++ )
    {
      rrccspdb_int_data.acq_list[k].plmn_id.mcc[0] = rrccspdb_int_data.nv_acq_list3.acq_entry[i].plmn_id.mcc[0];
      rrccspdb_int_data.acq_list[k].plmn_id.mcc[1] = rrccspdb_int_data.nv_acq_list3.acq_entry[i].plmn_id.mcc[1];
      rrccspdb_int_data.acq_list[k].plmn_id.mcc[2] = rrccspdb_int_data.nv_acq_list3.acq_entry[i].plmn_id.mcc[2];
      rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits = rrccspdb_int_data.nv_acq_list3.acq_entry[i].plmn_id.num_mnc_digits;
      if(rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits > RRC_MAX_MNC_DIGITS)
      {
      ...
      for(j=0; j<rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits; j++)
      {
        rrccspdb_int_data.acq_list[k].plmn_id.mnc[j] = rrccspdb_int_data.nv_acq_list3.acq_entry[i].plmn_id.mnc[j];
      }

      rrccspdb_int_data.acq_list[k].freq = (uint16)rrccspdb_int_data.nv_acq_list3.acq_entry[i].freq;
#ifdef FEATURE_MCC_AND_850_800_BAND_COMBINATION
      rrccspdb_int_data.acq_list[k].band_class = (uint16)rrccspdb_int_data.nv_acq_list3.acq_entry[i].band_class;
#else
      /* Get the band_class based on the frequency */
      rrccspdb_int_data.acq_list[k].band_class = rrc_get_frequency_band(wrrc_as_id,rrccspdb_int_data.acq_list[k].freq, rrccspdb_int_data.acq_list[k].plmn_id);
#endif
      rrccspdb_int_data.acq_list[k].status_mask = (uint16)rrccspdb_int_data.nv_acq_list3.acq_entry[i].status_mask;
      rrccspdb_int_data.acq_list[k].cell_id = rrccspdb_int_data.nv_acq_list3.acq_entry[i].cell_id;
      rrccspdb_int_data.acq_list[k].scr_code = (uint16)rrccspdb_int_data.nv_acq_list3.acq_entry[i].scr_code;
      rrccspdb_int_data.acq_list[k++].lac = (uint8)rrccspdb_int_data.nv_acq_list3.acq_entry[i].lac;
    }
  }
  else
  {
  ...
  if( NV_DONE_S ==  (rrc_get_nv(wrrc_as_id, NV_ACQ_LIST_4_I, (nv_item_type *) &rrccspdb_int_data.nv_acq_list4 )))
  {
    for( i=0; i< MAX_NV_ACQ_DB_ENTRIES2; i++ )
    {
      rrccspdb_int_data.acq_list[k].plmn_id.mcc[0] = rrccspdb_int_data.nv_acq_list4.acq_entry[i].plmn_id.mcc[0];
      rrccspdb_int_data.acq_list[k].plmn_id.mcc[1] = rrccspdb_int_data.nv_acq_list4.acq_entry[i].plmn_id.mcc[1];
      rrccspdb_int_data.acq_list[k].plmn_id.mcc[2] = rrccspdb_int_data.nv_acq_list4.acq_entry[i].plmn_id.mcc[2];
      rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits = rrccspdb_int_data.nv_acq_list4.acq_entry[i].plmn_id.num_mnc_digits;
      if(rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits > RRC_MAX_MNC_DIGITS)
      {
      ...
      for(j=0; j<rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits; j++)
      {
        rrccspdb_int_data.acq_list[k].plmn_id.mnc[j] = rrccspdb_int_data.nv_acq_list4.acq_entry[i].plmn_id.mnc[j];
      }

      rrccspdb_int_data.acq_list[k].freq = (uint16)rrccspdb_int_data.nv_acq_list4.acq_entry[i].freq;
#ifdef FEATURE_MCC_AND_850_800_BAND_COMBINATION
      rrccspdb_int_data.acq_list[k].band_class = (uint16)rrccspdb_int_data.nv_acq_list4.acq_entry[i].band_class;
#else
      /* Get the band_class based on the frequency */
      rrccspdb_int_data.acq_list[k].band_class = rrc_get_frequency_band(wrrc_as_id,rrccspdb_int_data.acq_list[k].freq, rrccspdb_int_data.acq_list[k].plmn_id);
#endif
      rrccspdb_int_data.acq_list[k].status_mask = (uint16)rrccspdb_int_data.nv_acq_list4.acq_entry[i].status_mask;
      rrccspdb_int_data.acq_list[k].cell_id = rrccspdb_int_data.nv_acq_list4.acq_entry[i].cell_id;
      rrccspdb_int_data.acq_list[k].scr_code = (uint16)rrccspdb_int_data.nv_acq_list4.acq_entry[i].scr_code;
      rrccspdb_int_data.acq_list[k++].lac = (uint8)rrccspdb_int_data.nv_acq_list4.acq_entry[i].lac;
    }
  }
  else
  {
  ...
  (void)rrc_get_from_efs(wrrc_as_id,rrccspdb_int_data.wcdma_csg_efs_list_1, &rrccspdb_int_data.acq_list[10], 5*sizeof(rrc_csp_acq_entry_type));
  ...
  if(rrc_get_from_efs(wrrc_as_id,rrccspdb_int_data.wcdma_csg_efs_list_1, efs_db, MAX_ACQ_LIST_ENTRIES_EFS*sizeof(rrc_csp_acq_entry_efs_type)))
  {
    rrc_csp_efs_convert_to_acq_list(wrrc_as_id,efs_db,rrccspdb_int_data.acq_list,0);
  }
  else
  {
  ...
  if(rrc_get_from_efs(wrrc_as_id,rrccspdb_int_data.wcdma_csg_efs_list_2, efs_db, MAX_ACQ_LIST_ENTRIES_EFS*sizeof(rrc_csp_acq_entry_efs_type)))
  {
    rrc_csp_efs_convert_to_acq_list(wrrc_as_id,efs_db,rrccspdb_int_data.acq_list,MAX_ACQ_LIST_ENTRIES_EFS);
  }
  else
  {
  ...
  if(rrc_get_from_efs(wrrc_as_id,rrccspdb_int_data.wcdma_csg_efs_list_3, efs_db, MAX_ACQ_LIST_ENTRIES_EFS*sizeof(rrc_csp_acq_entry_efs_type)))
  {
    rrc_csp_efs_convert_to_acq_list(wrrc_as_id,efs_db,rrccspdb_int_data.acq_list,2*MAX_ACQ_LIST_ENTRIES_EFS);
  }
  else
  {
  ...
  if(rrc_get_from_efs(wrrc_as_id,rrccspdb_int_data.wcdma_csg_efs_list_4, efs_db, MAX_ACQ_LIST_ENTRIES_EFS*sizeof(rrc_csp_acq_entry_efs_type)))
  {
    rrc_csp_efs_convert_to_acq_list(wrrc_as_id,efs_db,rrccspdb_int_data.acq_list,3*MAX_ACQ_LIST_ENTRIES_EFS);
  }
  else
  {
  ...
  if(rrc_get_from_efs(wrrc_as_id,rrccspdb_int_data.wcdma_csg_efs_list_5, efs_db,MAX_ACQ_LIST_ENTRIES_EFS*sizeof(rrc_csp_acq_entry_efs_type)))
  {
    rrc_csp_efs_convert_to_acq_list(wrrc_as_id,efs_db,rrccspdb_int_data.acq_list,4*MAX_ACQ_LIST_ENTRIES_EFS);
  }
  else
  {
  ...
  rrc_csp_recover_acq_db(wrrc_as_id);

  WRRC_MSG0_HIGH("Setting is_valid_nw_sharing to FALSE");
  for(i = 0; i<k; i++)
  {
    rrccspdb_int_data.acq_list[i].is_valid_nw_sharing = FALSE;
  }

  WRRC_MSG0_HIGH("Reading W freq (if present) for single freq scan");
  if( WNV_SUCCESS ==  (wnv_get_nv_msim( WNV_ID_WCDMA_DL_FREQ_ENABLED_I, &rrccspdb_int_data.nv_wcdma_dl_freq_enabled, sizeof(rrccspdb_int_data.nv_wcdma_dl_freq_enabled), wrrc_as_id)))
  {
    rrc_csp_int_data.wcdma_dl_freq_enabled = rrccspdb_int_data.nv_wcdma_dl_freq_enabled.wcdma_dl_freq_enabled;
    if(rrc_csp_int_data.wcdma_dl_freq_enabled)
    {
      if( WNV_SUCCESS == (wnv_get_nv_msim( WNV_ID_WCDMA_DL_FREQ_I, &rrccspdb_int_data.nv_wcdma_dl_freq, sizeof(rrccspdb_int_data.nv_wcdma_dl_freq), wrrc_as_id)))
      {
        rrc_csp_int_data.wcdma_dl_freq = (uint16)rrccspdb_int_data.nv_wcdma_dl_freq.wcdma_dl_freq;
...
NOTE
                  None.
===========================================================================*/
void  rrc_csp_write_nv_acq_info(sys_modem_as_id_e_type wrrc_as_id)
{
...
if(!rrc_put_efs(wrrc_as_id,rrccspdb_int_data.wcdma_csg_efs_db,&rrccspdb_int_data.stored_acq_db,sizeof(rrc_csp_acq_db_type)))
{
  WRRC_MSG0_ERROR("CSG: ACQ DB header write failed");
}

rrc_csp_acq_list_convert_to_efs(wrrc_as_id,efs_db,rrccspdb_int_data.acq_list, 0);
if(!rrc_put_efs(wrrc_as_id,rrccspdb_int_data.wcdma_csg_efs_list_1,efs_db,MAX_ACQ_LIST_ENTRIES_EFS*sizeof(rrc_csp_acq_entry_efs_type)))
{
  WRRC_MSG0_ERROR("CSG:1 Failed");
}

rrc_csp_acq_list_convert_to_efs(wrrc_as_id,efs_db,rrccspdb_int_data.acq_list, MAX_ACQ_LIST_ENTRIES_EFS);
if(!rrc_put_efs(wrrc_as_id,rrccspdb_int_data.wcdma_csg_efs_list_2,efs_db,MAX_ACQ_LIST_ENTRIES_EFS*sizeof(rrc_csp_acq_entry_efs_type)))
{
  WRRC_MSG0_ERROR("CSG: 2 write failed");
}

rrc_csp_acq_list_convert_to_efs(wrrc_as_id,efs_db,rrccspdb_int_data.acq_list, 2*MAX_ACQ_LIST_ENTRIES_EFS);
if(!rrc_put_efs(wrrc_as_id,rrccspdb_int_data.wcdma_csg_efs_list_3,efs_db,MAX_ACQ_LIST_ENTRIES_EFS*sizeof(rrc_csp_acq_entry_efs_type)))
{
  WRRC_MSG0_ERROR("CSG: 3 write failed");
}

rrc_csp_acq_list_convert_to_efs(wrrc_as_id,efs_db,rrccspdb_int_data.acq_list, 3*MAX_ACQ_LIST_ENTRIES_EFS);
if(!rrc_put_efs(wrrc_as_id,rrccspdb_int_data.wcdma_csg_efs_list_4,efs_db,MAX_ACQ_LIST_ENTRIES_EFS*sizeof(rrc_csp_acq_entry_efs_type)))
{
  WRRC_MSG0_ERROR("CSG:4 write failed");
}

rrc_csp_acq_list_convert_to_efs(wrrc_as_id,efs_db,rrccspdb_int_data.acq_list, 4*MAX_ACQ_LIST_ENTRIES_EFS);
if(!rrc_put_efs(wrrc_as_id,rrccspdb_int_data.wcdma_csg_efs_list_5,efs_db,MAX_ACQ_LIST_ENTRIES_EFS*sizeof(rrc_csp_acq_entry_efs_type)))
{
  ...
  for( i=0; i< MAX_NV_ACQ_DB_ENTRIES1; i++ )
  {
    rrccspdb_int_data.nv_acq_list1.acq_entry[i].plmn_id.mcc[0] = rrccspdb_int_data.acq_list[k].plmn_id.mcc[0];
    rrccspdb_int_data.nv_acq_list1.acq_entry[i].plmn_id.mcc[1] = rrccspdb_int_data.acq_list[k].plmn_id.mcc[1];
    rrccspdb_int_data.nv_acq_list1.acq_entry[i].plmn_id.mcc[2] = rrccspdb_int_data.acq_list[k].plmn_id.mcc[2];
    rrccspdb_int_data.nv_acq_list1.acq_entry[i].plmn_id.num_mnc_digits = rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits;
    for(j=0; j<rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits; j++)
    {
      rrccspdb_int_data.nv_acq_list1.acq_entry[i].plmn_id.mnc[j] = rrccspdb_int_data.acq_list[k].plmn_id.mnc[j];
    }

    rrccspdb_int_data.nv_acq_list1.acq_entry[i].freq = rrccspdb_int_data.acq_list[k].freq;
    rrccspdb_int_data.nv_acq_list1.acq_entry[i].status_mask = rrccspdb_int_data.acq_list[k].status_mask;
    rrccspdb_int_data.nv_acq_list1.acq_entry[i].cell_id = rrccspdb_int_data.acq_list[k].cell_id;
    rrccspdb_int_data.nv_acq_list1.acq_entry[i].scr_code = rrccspdb_int_data.acq_list[k].scr_code;
#ifdef FEATURE_MCC_AND_850_800_BAND_COMBINATION
    rrccspdb_int_data.nv_acq_list1.acq_entry[i].band_class = rrccspdb_int_data.acq_list[k].band_class;
#endif
    rrccspdb_int_data.nv_acq_list1.acq_entry[i].lac = rrccspdb_int_data.acq_list[k++].lac;
  }

  if( NV_DONE_S !=  (rrc_put_nv(wrrc_as_id, NV_ACQ_LIST_1_I, (nv_item_type *) &rrccspdb_int_data.nv_acq_list1 )))
  {
  ...
  for( i=0; i< MAX_NV_ACQ_DB_ENTRIES1; i++ )
  {
    rrccspdb_int_data.nv_acq_list2.acq_entry[i].plmn_id.mcc[0] = rrccspdb_int_data.acq_list[k].plmn_id.mcc[0];
    rrccspdb_int_data.nv_acq_list2.acq_entry[i].plmn_id.mcc[1] = rrccspdb_int_data.acq_list[k].plmn_id.mcc[1];
    rrccspdb_int_data.nv_acq_list2.acq_entry[i].plmn_id.mcc[2] = rrccspdb_int_data.acq_list[k].plmn_id.mcc[2];
    rrccspdb_int_data.nv_acq_list2.acq_entry[i].plmn_id.num_mnc_digits = rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits;
    for(j=0; j<rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits; j++)
    {
      rrccspdb_int_data.nv_acq_list2.acq_entry[i].plmn_id.mnc[j] = rrccspdb_int_data.acq_list[k].plmn_id.mnc[j];
    }

    rrccspdb_int_data.nv_acq_list2.acq_entry[i].freq = rrccspdb_int_data.acq_list[k].freq;
    rrccspdb_int_data.nv_acq_list2.acq_entry[i].status_mask = rrccspdb_int_data.acq_list[k].status_mask;
    rrccspdb_int_data.nv_acq_list2.acq_entry[i].cell_id = rrccspdb_int_data.acq_list[k].cell_id;
    rrccspdb_int_data.nv_acq_list2.acq_entry[i].scr_code = rrccspdb_int_data.acq_list[k].scr_code;
#ifdef FEATURE_MCC_AND_850_800_BAND_COMBINATION
    rrccspdb_int_data.nv_acq_list2.acq_entry[i].band_class = rrccspdb_int_data.acq_list[k].band_class;
#endif
    rrccspdb_int_data.nv_acq_list2.acq_entry[i].lac = rrccspdb_int_data.acq_list[k++].lac;
  }

  if( NV_DONE_S !=  (rrc_put_nv(wrrc_as_id, NV_ACQ_LIST_2_I, (nv_item_type *) &rrccspdb_int_data.nv_acq_list2 )))
  {
  ...
  for( i=0; i< MAX_NV_ACQ_DB_ENTRIES2; i++ )
  {
    rrccspdb_int_data.nv_acq_list3.acq_entry[i].plmn_id.mcc[0] = rrccspdb_int_data.acq_list[k].plmn_id.mcc[0];
    rrccspdb_int_data.nv_acq_list3.acq_entry[i].plmn_id.mcc[1] = rrccspdb_int_data.acq_list[k].plmn_id.mcc[1];
    rrccspdb_int_data.nv_acq_list3.acq_entry[i].plmn_id.mcc[2] = rrccspdb_int_data.acq_list[k].plmn_id.mcc[2];
    rrccspdb_int_data.nv_acq_list3.acq_entry[i].plmn_id.num_mnc_digits = rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits;
    for(j=0; j<rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits; j++)
    {
      rrccspdb_int_data.nv_acq_list3.acq_entry[i].plmn_id.mnc[j] = rrccspdb_int_data.acq_list[k].plmn_id.mnc[j];
    }

    rrccspdb_int_data.nv_acq_list3.acq_entry[i].freq = rrccspdb_int_data.acq_list[k].freq;
    rrccspdb_int_data.nv_acq_list3.acq_entry[i].status_mask = rrccspdb_int_data.acq_list[k].status_mask;
    rrccspdb_int_data.nv_acq_list3.acq_entry[i].cell_id = rrccspdb_int_data.acq_list[k].cell_id;
    rrccspdb_int_data.nv_acq_list3.acq_entry[i].scr_code = rrccspdb_int_data.acq_list[k].scr_code;
#ifdef FEATURE_MCC_AND_850_800_BAND_COMBINATION
    rrccspdb_int_data.nv_acq_list3.acq_entry[i].band_class = rrccspdb_int_data.acq_list[k].band_class;
#endif
    rrccspdb_int_data.nv_acq_list3.acq_entry[i].lac = rrccspdb_int_data.acq_list[k++].lac;
  }

  if( NV_DONE_S !=  (rrc_put_nv(wrrc_as_id, NV_ACQ_LIST_3_I, (nv_item_type *) &rrccspdb_int_data.nv_acq_list3 )))
  {
  ...
  for( i=0; i< MAX_NV_ACQ_DB_ENTRIES2; i++ )
  {
    rrccspdb_int_data.nv_acq_list4.acq_entry[i].plmn_id.mcc[0] = rrccspdb_int_data.acq_list[k].plmn_id.mcc[0];
    rrccspdb_int_data.nv_acq_list4.acq_entry[i].plmn_id.mcc[1] = rrccspdb_int_data.acq_list[k].plmn_id.mcc[1];
    rrccspdb_int_data.nv_acq_list4.acq_entry[i].plmn_id.mcc[2] = rrccspdb_int_data.acq_list[k].plmn_id.mcc[2];
    rrccspdb_int_data.nv_acq_list4.acq_entry[i].plmn_id.num_mnc_digits = rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits;
    for(j=0; j<rrccspdb_int_data.acq_list[k].plmn_id.num_mnc_digits; j++)
    {
      rrccspdb_int_data.nv_acq_list4.acq_entry[i].plmn_id.mnc[j] = rrccspdb_int_data.acq_list[k].plmn_id.mnc[j];
    }

    rrccspdb_int_data.nv_acq_list4.acq_entry[i].freq = rrccspdb_int_data.acq_list[k].freq;
    rrccspdb_int_data.nv_acq_list4.acq_entry[i].status_mask = rrccspdb_int_data.acq_list[k].status_mask;
    rrccspdb_int_data.nv_acq_list4.acq_entry[i].cell_id = rrccspdb_int_data.acq_list[k].cell_id;
    rrccspdb_int_data.nv_acq_list4.acq_entry[i].scr_code = rrccspdb_int_data.acq_list[k].scr_code;
#ifdef FEATURE_MCC_AND_850_800_BAND_COMBINATION
    rrccspdb_int_data.nv_acq_list4.acq_entry[i].band_class = rrccspdb_int_data.acq_list[k].band_class;
#endif
    rrccspdb_int_data.nv_acq_list4.acq_entry[i].lac = rrccspdb_int_data.acq_list[k++].lac;
  }


  if( NV_DONE_S !=  (rrc_put_nv(wrrc_as_id, NV_ACQ_LIST_4_I, (nv_item_type *) &rrccspdb_int_data.nv_acq_list4 )))
  {
  ...
  if(!rrc_put_efs(wrrc_as_id,rrccspdb_int_data.wcdma_csg_efs_list_1,&rrccspdb_int_data.acq_list[10],5*sizeof(rrc_csp_acq_entry_type)))
  {
    WRRC_MSG0_ERROR("CSG:1 Failed");
  }
  WRRC_MSG0_HIGH("CSG: ACQ DB is being written to EFS");
...
NOTE
                  None.
===========================================================================*/
void  rrc_csp_write_nv_acq_list_info(sys_modem_as_id_e_type wrrc_as_id,int index)
{
  int  i;
  int  j;
  int  k=0;
  ...
  switch(index)
  {
    ...
    case 2:
      ...
      if((rrc_queue_nv_write(wrrc_as_id, NV_ACQ_LIST_1_I, (nv_item_type *) &rrccspdb_int_data.nv_acq_list1, sizeof(nv_acq_list1_type))) == -1)
      {
        WRRC_MSG0_ERROR("NV_ACQ_LIST_1_I not written");
      }
      break;
    ...
    case 5:
      ...
      if((rrc_queue_nv_write(wrrc_as_id, NV_ACQ_LIST_2_I, (nv_item_type *) &rrccspdb_int_data.nv_acq_list2, sizeof(nv_acq_list1_type))) == -1)
      {
        WRRC_MSG0_ERROR("NV_ACQ_LIST_2_I not written");
      }
      break;
    ...
    case 7:
      ...
      if((rrc_queue_nv_write(wrrc_as_id, NV_ACQ_LIST_3_I, (nv_item_type *) &rrccspdb_int_data.nv_acq_list3, sizeof(nv_acq_list2_type))) == -1)
      {
        WRRC_MSG0_ERROR("NV_ACQ_LIST_3_I not written");
      }
      break;
    ...
    case 9:	
      ...
      if((rrc_queue_nv_write(wrrc_as_id, NV_ACQ_LIST_4_I, (nv_item_type *) &rrccspdb_int_data.nv_acq_list4, sizeof(nv_acq_list2_type) )) == -1)
      {
        WRRC_MSG0_ERROR("NV_ACQ_LIST_4_I not written");
      }
      break;
...
NOTE
                  None.
===========================================================================*/
void  rrc_csp_write_nv_acq_db_info(sys_modem_as_id_e_type wrrc_as_id)
{
  uint32  status;

  status = efs_async_put(rrccspdb_int_data.wcdma_csg_efs_db,&rrccspdb_int_data.stored_acq_db,sizeof(rrc_csp_acq_db_type),O_CREAT|O_AUTODIR, 0777);
  if(status == -1)
  {
...
#ifdef FEATURE_DUAL_WCDMA_ACQDB_SHARING
/*===========================================================================

FUNCTION          RRC_ADD_ACQ_ENTRY_TO_ALT_SUB

DESCRIPTION       This function sends RRC_ADD_ACQ_ENTRY command to alternate SUB.
                  The command asks the alternate SUB to add the ACQ entry to its ACQ DB.
...
===========================================================================*/
void rrc_add_acq_entry_to_alt_sub(sys_modem_as_id_e_type wrrc_as_id,rrc_csp_acq_entry_type* acq_entry_ptr)
{
  rrc_cmd_type *cmd_ptr = rrc_get_int_cmd_buf(WRRC_GET_ALT_SUB_ID(wrrc_as_id));
  cmd_ptr->cmd_hdr.cmd_id = RRC_ADD_ACQ_ENTRY;

  WCDMA_MEMCPY(&cmd_ptr->cmd.rrc_acq_db_entry.acq_entry,
                sizeof(rrc_csp_acq_entry_type),
                acq_entry_ptr,
                sizeof(rrc_csp_acq_entry_type));
  /* Post the command to alternate SUB internal command queue */
  rrc_put_int_cmd(WRRC_GET_ALT_SUB_ID(wrrc_as_id), cmd_ptr );
}
#endif
...
===========================================================================*/
uecomdef_status_e_type  rrc_csp_add_acq_entry(sys_modem_as_id_e_type wrrc_as_id,rrc_csp_acq_entry_type* acq_entry_ptr, boolean add_entry_to_alt_sub)
{
...
#ifdef FEATURE_DUAL_WCDMA_ACQDB_SHARING
  if(add_entry_to_alt_sub)
  {
    rrc_add_acq_entry_to_alt_sub(wrrc_as_id,acq_entry_ptr);
  }
#endif
  ...
  if ((rrccspdb_int_data.stored_acq_db.curr_wr_index == MAX_ACQ_DB_INDEX) && 
      (rrccspdb_int_data.stored_acq_db.last_entry_invalid == FALSE))
  {
    /* If all pointers in acq. dbase are filled up, the write index points
     * to a pointer entry which is the last one and is Valid.
     */
    num_valid_entries = rrccspdb_int_data.stored_acq_db.curr_wr_index + 1;
  }
  else 
  {
     ...
     */
    num_valid_entries = rrccspdb_int_data.stored_acq_db.curr_wr_index;
  }
  ...
  {
    if ((rrc_bcch_plmns_match(wrrc_as_id,rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].plmn_id, 
                              acq_entry_ptr->plmn_id)) &&
        (rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].freq == acq_entry_ptr->freq)
#ifdef FEATURE_FEMTO_CSG
        && (rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].csg_id == acq_entry_ptr->csg_id)
        && ((acq_entry_ptr->csg_id == SYS_CSG_ID_INVALID) ||
            (rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].scr_code == acq_entry_ptr->scr_code))
#endif
        )
    {
       ...
       * is unaffected.
       */
      wr_acq_list_index = rrccspdb_int_data.stored_acq_db.acq_list_indices[i];

      for (ptr_index = i; ptr_index > 0; ptr_index--)
      {
        rrccspdb_int_data.stored_acq_db.acq_list_indices[ptr_index] = 
          rrccspdb_int_data.stored_acq_db.acq_list_indices[ptr_index-1];
      }

#ifndef FEATURE_FEMTO_CSG
      rrccspdb_int_data.acq_list[wr_acq_list_index] = *acq_entry_ptr;
#else
      rrc_csp_fill_acq_entry(wrrc_as_id,&rrccspdb_int_data.acq_list[wr_acq_list_index],acq_entry_ptr);
#endif
      band_class = rrc_get_frequency_band(wrrc_as_id,acq_entry_ptr->freq, acq_entry_ptr->plmn_id);
      rrccspdb_int_data.acq_list[wr_acq_list_index].band_class = band_class;

      /* Initialize the topmost pointer to the most recent acq. list entry */
      rrccspdb_int_data.stored_acq_db.acq_list_indices[0] = (uint8)wr_acq_list_index;
      /*Save acq db info in NV. Only if WCDMA RRC Save ACQDB NV is set*/
#ifndef FEATURE_FEMTO_CSG
      if(rrcnv_data.rrc_save_acqdb_nv)
      {
        /* If first entry matches then no need to write ACQ DB
           header as none of its data members are modified */
        if(i!=0)
        {
          rrc_csp_write_nv_acq_db_info(wrrc_as_id);
        }
        rrc_csp_write_nv_acq_list_info(wrrc_as_id,wr_acq_list_index);
      }
#endif
      /* Return always a Success for the time being */
#ifdef FEATURE_FEMTO_CSG
      WRRC_MSG8_HIGH("Updated: PLMN %d-%d freq %d scr_code %d band %d CSG ID %d; num_csg_entries:%d, curr_wr_index:%d in Acq DB",
                      RRC_CSP_GET_INT_MCC(acq_entry_ptr->plmn_id),RRC_CSP_GET_INT_MNC(acq_entry_ptr->plmn_id),acq_entry_ptr->freq,
                      acq_entry_ptr->scr_code, band_class, acq_entry_ptr->csg_id,rrccspdb_int_data.stored_acq_db.num_of_csg_entries,
                      rrccspdb_int_data.stored_acq_db.curr_wr_index);
#else
      WRRC_MSG5_HIGH("Updated: PLMN %d-%d freq %d scr_code %d band %d in Acq DB", RRC_CSP_GET_INT_MCC(acq_entry_ptr->plmn_id),
                      RRC_CSP_GET_INT_MNC(acq_entry_ptr->plmn_id),acq_entry_ptr->freq, acq_entry_ptr->scr_code, band_class);
#endif
  ...
  rrc_log_acq_db_info(wrrc_as_id,acq_entry_ptr);
      ...
      (rrccspdb_int_data.stored_acq_db.last_entry_invalid == FALSE))
  {
      ...
      /* ACQ-DB is full */
        WRRC_MSG3_HIGH("CSG: num_of_csg_entries: %d, curr_wr_index %d, CSG ID to add: %d", 
        rrccspdb_int_data.stored_acq_db.num_of_csg_entries, rrccspdb_int_data.stored_acq_db.curr_wr_index, acq_entry_ptr->csg_id);
  ...
  {
    wr_acq_list_index = rrccspdb_int_data.stored_acq_db.curr_wr_index;
    if(acq_entry_ptr->csg_id != SYS_CSG_ID_INVALID)
    {
      if(rrccspdb_int_data.stored_acq_db.num_of_csg_entries == MAX_ACQ_DB_CSG_ENTRIES)
      {
        replace_csg_entry = TRUE;
      }
      else
      {
        rrccspdb_int_data.stored_acq_db.num_of_csg_entries++;
      }
    }
    else
    {
      if((rrccspdb_int_data.stored_acq_db.curr_wr_index - rrccspdb_int_data.stored_acq_db.num_of_csg_entries) == MAX_ACQ_DB_MACRO_ENTRIES)
      {
        replace_macro_entry = TRUE;
      }
    }
  ...
  if(replace_csg_entry || replace_macro_entry)
  {
    ...
    if(!((rrccspdb_int_data.stored_acq_db.curr_wr_index == MAX_ACQ_DB_INDEX) &&
      (rrccspdb_int_data.stored_acq_db.last_entry_invalid == FALSE)))
    {
      idx = rrccspdb_int_data.stored_acq_db.curr_wr_index-1;
    }

      for(; idx > 0; idx-- )
      {
        if((replace_csg_entry && (rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[idx]].csg_id != SYS_CSG_ID_INVALID))
          || ((replace_macro_entry) && (rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[idx]].csg_id == SYS_CSG_ID_INVALID)))
        {
          wr_acq_list_index = rrccspdb_int_data.stored_acq_db.acq_list_indices[idx];
  
          WRRC_MSG5_HIGH(
               "CSG: Replacing: PLMN %d-%d, CSG ID %d, freq %d scr_code %d", 
                  RRC_CSP_GET_INT_MCC(rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[idx]].plmn_id),
                  RRC_CSP_GET_INT_MNC(rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[idx]].plmn_id),
                  rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[idx]].csg_id, rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[idx]].freq,
                  rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[idx]].scr_code);
  ...
  if ((rrccspdb_int_data.stored_acq_db.curr_wr_index == MAX_ACQ_DB_INDEX) &&
      (rrccspdb_int_data.stored_acq_db.last_entry_invalid == FALSE))
  {
    wr_acq_list_index = rrccspdb_int_data.stored_acq_db.acq_list_indices[rrccspdb_int_data.stored_acq_db.curr_wr_index];
  }
  else
  {
    wr_acq_list_index = rrccspdb_int_data.stored_acq_db.curr_wr_index;
  ...
  {
    WRRC_ERR0_FATAL("Invalid write index.");
    return(FAILURE);
  }
...
/*Invalidate FP info*/
  rrccspdb_int_data.acq_list[wr_acq_list_index].w_macro_write_idx = 0;
  rrccspdb_int_data.acq_list[wr_acq_list_index].fp_w_macro_cnt = 0;
  rrccspdb_int_data.acq_list[wr_acq_list_index].lte_macro_write_idx = 0;
  rrccspdb_int_data.acq_list[wr_acq_list_index].fp_lte_macro_cnt = 0;
#endif
   ...
   */
  i = rrccspdb_int_data.stored_acq_db.curr_wr_index;
#ifdef FEATURE_FEMTO_CSG
  if(replace_csg_entry || replace_macro_entry)
  {
  ...
  for (; i > 0; i--)
  {
    rrccspdb_int_data.stored_acq_db.acq_list_indices[i] = 
      rrccspdb_int_data.stored_acq_db.acq_list_indices[i-1];
  }
  /* Initialize the topmost pointer to the most recent acq. list entry */
  rrccspdb_int_data.stored_acq_db.acq_list_indices[0] = (uint8)wr_acq_list_index;
  ...
  WRRC_MSG3_HIGH("Added: freq %d scr_code %d band %d in Acq DB", 
  ...
  WRRC_MSG5_HIGH(
          ...
          acq_entry_ptr->csg_id, rrccspdb_int_data.stored_acq_db.num_of_csg_entries, rrccspdb_int_data.stored_acq_db.curr_wr_index);
...
===========================================================================*/
void rrccsp_add_macro_fp_info_in_acq_db(sys_modem_as_id_e_type wrrc_as_id,rrc_csp_acq_entry_type* femto_info)
{
  ...
  uint8 num_valid_entries = rrc_csp_assert_acq_db_vars(wrrc_as_id);

  for (i = 0; i < num_valid_entries; i++)
  {
    if ((rrc_bcch_plmns_match(wrrc_as_id,rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].plmn_id, femto_info->plmn_id)) 
        && (rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].freq == femto_info->freq)
        && (rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].csg_id == femto_info->csg_id)
        && ((rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].scr_code == femto_info->scr_code)
            && (rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].cell_id == femto_info->cell_id))

        )
    {
      if(rrc_csp_int_data.last_csg_rat == RRC_CSG_LTE)
      {
        WRRC_MSG1_HIGH("CSG: num_of_fp_entries: %d", rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].fp_lte_macro_cnt);

        fp_wr_index = rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].lte_macro_write_idx;
        WRRC_MSG1_HIGH("CSG: LTE FP write index %d", fp_wr_index);
        if(fp_wr_index >= NUM_CSG_LTE_MACRO)
       {
         fp_wr_index = rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].lte_macro_write_idx = 0;
       }
        for(j = 0; j < rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].fp_lte_macro_cnt; j++)
        {
          if ((rrc_bcch_plmns_match(wrrc_as_id,rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].lte_macro[j].plmn_id,rrc_csp_int_data.fp_macro.plmn)) 
              && (rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].lte_macro[j].freq == rrc_csp_int_data.fp_macro.freq)
              && (rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].lte_macro[j].pci == rrc_csp_int_data.fp_macro.pci)
              && (rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].lte_macro[j].cell_id == rrc_csp_int_data.fp_macro.cell_id)
             )
          {
            WRRC_MSG0_HIGH("CSG: LTE macro mapping already exists. Setting prune counter to zero");
            rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].lte_macro[j].acq_fail_counter = 0;
            is_match = TRUE;
            break;
          }
        }
        ...
        rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].lte_macro[fp_wr_index].freq = rrc_csp_int_data.fp_macro.freq;
        rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].lte_macro[fp_wr_index].plmn_id = rrc_csp_int_data.fp_macro.plmn;
        rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].lte_macro[fp_wr_index].pci= rrc_csp_int_data.fp_macro.pci;
        rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].lte_macro[fp_wr_index].cell_id = rrc_csp_int_data.fp_macro.cell_id;
        rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].lte_macro[fp_wr_index].acq_fail_counter = 0;
        /* Recovery code */
        if(rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].fp_lte_macro_cnt >= NUM_CSG_LTE_MACRO)
        {
          rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].fp_lte_macro_cnt = NUM_CSG_LTE_MACRO;
        }
        if(rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].fp_lte_macro_cnt != NUM_CSG_LTE_MACRO)
        {
          rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].fp_lte_macro_cnt++;
        }
        rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].lte_macro_write_idx++;
      ...
      {
        WRRC_MSG1_HIGH("CSG: num_of_fp_entries: %d", rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].fp_w_macro_cnt);
        
        fp_wr_index = rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].w_macro_write_idx;
        WRRC_MSG1_HIGH("CSG: W FP write index %d", fp_wr_index);
        if(fp_wr_index >= NUM_CSG_WCDMA_MACRO)
       {
         fp_wr_index = rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].w_macro_write_idx = 0;
       }
        for(j = 0; j < rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].fp_w_macro_cnt; j++)
        {
          if ((rrc_bcch_plmns_match(wrrc_as_id,rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].w_macro[j].plmn_id,rrc_csp_int_data.fp_macro.plmn)) 
              && (rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].w_macro[j].freq == rrc_csp_int_data.fp_macro.freq)
              && (rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].w_macro[j].psc == rrc_csp_int_data.fp_macro.pci)
              && (rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].w_macro[j].cell_id == rrc_csp_int_data.fp_macro.cell_id)
             )
          {
            WRRC_MSG0_HIGH("CSG: W macro mapping already exists. Setting acq counter to zero");
            rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].w_macro[j].acq_fail_counter = 0;
            is_match = TRUE;
            break;
          }
        }
        ...
        rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].w_macro[fp_wr_index].freq = rrc_csp_int_data.fp_macro.freq;
        rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].w_macro[fp_wr_index].plmn_id = rrc_csp_int_data.fp_macro.plmn;
        rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].w_macro[fp_wr_index].psc= rrc_csp_int_data.fp_macro.pci;
        rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].w_macro[fp_wr_index].cell_id = rrc_csp_int_data.fp_macro.cell_id;
        rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].w_macro[fp_wr_index].acq_fail_counter = 0;
        /* Recovery code */
        if(rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].fp_lte_macro_cnt >= NUM_CSG_WCDMA_MACRO)
        {
          rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].fp_lte_macro_cnt = NUM_CSG_WCDMA_MACRO;
        }
        if(rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].fp_w_macro_cnt != NUM_CSG_WCDMA_MACRO)
        {
          rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].fp_w_macro_cnt++;
        }
        rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].w_macro_write_idx++;
...
===========================================================================*/
boolean
rrc_csp_should_freq_be_scanned(sys_modem_as_id_e_type wrrc_as_id,rrc_requested_plmn_type req_plmn, rrc_freq_type freq)
{
  int i;
  int num_valid_entries;
  ...
  /*To scan specific frequencies which belongs to specified band given from NAS*/
  if(rrc_csp_int_data.mplmn_wcdma_freq_list_present)
  {
    /*Scan only those frequencies corresponding to the bands supported by the UE*/
    if(rrccsp_is_supported_band(wrrc_as_id,rrc_get_frequency_band(wrrc_as_id,freq,rrc_ccm_get_curr_camped_plmn(wrrc_as_id))))
    {
      WRRC_MSG2_HIGH("MPLMN:Freq %d corresponding to %d band.Proceed Scanning",freq,rrc_get_frequency_band(wrrc_as_id,freq,rrc_ccm_get_curr_camped_plmn(wrrc_as_id)));
      return TRUE;
    }
    else
    {
  ...
  /* Compute Number of Valid entries */
  else if((rrccspdb_int_data.stored_acq_db.curr_wr_index == MAX_ACQ_DB_INDEX) &&
      (rrccspdb_int_data.stored_acq_db.last_entry_invalid == FALSE))
  {
    /* If all pointers in acq. dbase are filled up, the write index points
     * to a pointer entry which is the last one and is Valid.
  ...
  for (i = 0; i < num_valid_entries; i++)
  {
    ...
    if(rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].freq == freq)
    { 
       ...
       /*Scan non-white ACQ DB CSG entries which are skipped earlier. If ACQ DB has two entries with same frequency 
         ...
         scanned already*/
       if((rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].csg_id != SYS_CSG_ID_INVALID) && 
          !(rrccsp_csg_in_whitelist(wrrc_as_id,rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].csg_id,
                                       rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].plmn_id)))
       {
         continue;
       }
      ...
      if(rrccsp_is_supported_band(wrrc_as_id,rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].band_class))
      {
...
===========================================================================*/
static boolean
rrc_csp_has_acq_db_freq_been_scanned(sys_modem_as_id_e_type wrrc_as_id,rrc_freq_type freq)
{
  uint8 i;

  for (i = 0; i < rrccspdb_int_data.acq_db_freq_scanned_list.wr_index; i++)
  {
    if(freq == rrccspdb_int_data.acq_db_freq_scanned_list.freq[i])
    {
...
===========================================================================*/
static void
rrc_csp_update_acq_db_freq_scanned_list(sys_modem_as_id_e_type wrrc_as_id,rrc_freq_type freq)
{ 
  if(rrccspdb_int_data.acq_db_freq_scanned_list.wr_index >= MAX_ACQ_DB_SIZE)
  {
    WRRC_MSG0_ERROR("Max # of available entries in freq scanned list exceeded!");
    rrccspdb_int_data.acq_db_freq_scanned_list.wr_index = 0;
  }

  /* Add Frequency to the list */
  WRRC_MSG1_HIGH("Adding freq %d to the scanned list", freq);
  rrccspdb_int_data.acq_db_freq_scanned_list.freq[rrccspdb_int_data.acq_db_freq_scanned_list.wr_index++] = freq;
...
===========================================================================*/
static boolean rrc_csp_should_all_acq_db_be_scanned(sys_modem_as_id_e_type wrrc_as_id)
{
  boolean status = TRUE;
  ...
  switch(rrc_csp_int_data.curr_scan)
  {
    ...
    case RRC_CSP_GTOW_BPLMN_AUTOMATIC_SCAN:
      ...
      /* Under this feature we always scan all ACQ DB entries as we are not sure 
         ...
         WRRC_MSG0_MED("Do not skip ACQ DB entry scan");
#else
      ...
      {
        WRRC_MSG3_HIGH("Do NOT scan ALL ACQ DB. rd_ix %d, num_req %d, fscan_needed %d", 
    ...
    case RRC_CSP_WTOW_BPLMN_AUTOMATIC_SCAN:
      ...
      {
        status = FALSE;
        WRRC_MSG0_MED("CSG: Do not scan all ACQ DB during ASF srch.");
      }
      else
#endif
#ifdef FEATURE_HPPLMN_SRCH_WITH_MANY_PREF_PLMN
  ...
  WRRC_MSG2_MED("Scan Type %d. Scan ALL Acq DB Status = %d", rrc_csp_int_data.curr_scan, status);
  return (status);
...
rrc_csp_get_match_acq_entry(sys_modem_as_id_e_type wrrc_as_id,rrc_plmn_identity_type req_plmn_id, rrc_csp_acq_e_type req_type_acq)
{
   ...
   * write index.
   */
  num_valid_entries = rrc_csp_assert_acq_db_vars(wrrc_as_id);
  ...
  switch(req_type_acq)
  {
    case RRC_CSP_SPEC_PLMN:
      ...
      if(!rrc_csp_int_data.deep_sleep_acq_db_plmn_searchloop)
      {
        ...
        for (i = rrccspdb_int_data.stored_acq_db.curr_rd_index + 1; i < num_valid_entries; i++)
        {
          is_network_shared = rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].is_valid_nw_sharing;
          ...
          if(((rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].csg_id != SYS_CSG_ID_INVALID) && 
             !(rrccsp_csg_in_whitelist(wrrc_as_id,rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].csg_id,
                                       rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].plmn_id)))
              || ((rrccsp_asf_actively_in_prog(wrrc_as_id) && (rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].csg_id == SYS_CSG_ID_INVALID)))
              || ((rrccsp_asf_actively_in_prog(wrrc_as_id) && !rrccsp_fp_macro_present_in_acq_db(wrrc_as_id,rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]]))))
          {
            MSG_MED("CSG: Skip acq db entry %d as CSG ID %d is not in whitelist/ macro cell during ASF", rrccspdb_int_data.stored_acq_db.acq_list_indices[i],rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].csg_id,0);
            continue;
          }
#endif
            ...
            )
          {
                      ...
                      rrc_csp_int_data.curr_req_plmn.plmn_id))) == TRUE)
            {
              ...
              if((!is_network_shared) &&
                 (rrccsp_check_for_eplmn(wrrc_as_id,
                    mmumts_convert_rrc_plmn_id_to_nas_plmn_id(
                      rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].plmn_id)) == TRUE))
              {
                WRRC_MSG2_HIGH("PLMN %d-%d is in the eq PLMN list",
                           RRC_CSP_GET_INT_MCC(rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].plmn_id),
                           RRC_CSP_GET_INT_MNC(rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].plmn_id));
                is_eplmn = TRUE;
              }
              else if((is_network_shared) && 
                 ((rrccsp_nas_plmn_equivalent_to_any_of_shared_bcch_plmns(wrrc_as_id,
                                                                          rrc_csp_int_data.curr_req_plmn.plmn_id,
                                                                          rrccspdb_int_data.stored_acq_db.acq_list_indices[i])) == TRUE))
              {
                is_eplmn = TRUE;
              }
            }
          ...
          {
            WRRC_MSG3_HIGH("ACQ DB PLMN being compared %d-%d. Freq %d",
                     RRC_CSP_GET_INT_MCC(rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].plmn_id),
                     RRC_CSP_GET_INT_MNC(rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].plmn_id),
                     rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].freq);
          }
          ...
          if(rrc_csp_int_data.ue_in_deep_sleep && 
                  (rrccsp_is_supported_band(wrrc_as_id,rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].band_class)))
          { 
              rrc_csp_int_data.deep_sleep_acq_db_band_mask |= 
                                               rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].band_class;
          }
  #endif
          is_acq_db_plmn_matched = FALSE;
          if((!is_network_shared) &&
             (rrc_bcch_and_nas_plmns_match(wrrc_as_id,rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].plmn_id, 
                                           req_plmn_id)))
          {
            is_acq_db_plmn_matched = TRUE;
          }
          else if((is_network_shared) &&
                  (rrccsp_nas_plmn_match_any_of_bcch_plmns(wrrc_as_id,
                                                       req_plmn_id,
                                                       rrccspdb_int_data.stored_acq_db.acq_list_indices[i])))
          {
            is_acq_db_plmn_matched = TRUE;
          ...
          if((is_acq_db_plmn_matched
              || (is_eplmn)
              ) && (!rrccsp_should_freq_be_skipped(wrrc_as_id,
                rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].freq, RRCCSP_FINE_SCAN_SKIP_RANGE))
                && (rrccsp_is_supported_band(wrrc_as_id,rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].band_class))
                && (!rrc_csp_has_acq_db_freq_been_scanned(wrrc_as_id,
                          rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].freq)))
          {
             ...
             */
            rrccspdb_int_data.stored_acq_db.curr_rd_index = (int8)i;
            ret_acq_entry_ptr = &rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]];
  
            /* Update scanned list with the frequency to avoid multiple acquisition
             * attempts on the same frequency
             */
            rrc_csp_update_acq_db_freq_scanned_list(wrrc_as_id,rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].freq);
    ...
    case RRC_CSP_ANY_PLMN:
      i = rrccspdb_int_data.stored_acq_db.curr_rd_index + 1;
      while(i < num_valid_entries)
      {
        ...
        if((!(rrccsp_is_supported_band(wrrc_as_id,rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].band_class))) ||
           (rrccsp_should_freq_be_skipped(wrrc_as_id,
             rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].freq, RRCCSP_FINE_SCAN_SKIP_RANGE)) ||
           (rrc_csp_has_acq_db_freq_been_scanned(wrrc_as_id,rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].freq)))
        {
          i++;
        }
        else
  ...
  if(    (!ret_acq_entry_ptr)
      ...
      && (rrc_csp_should_all_acq_db_be_scanned(wrrc_as_id))
    )
  {
    ...
    while(i < num_valid_entries)
    {
      ...
      if( (rrccsp_should_freq_be_skipped(wrrc_as_id,
               rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].freq, RRCCSP_FINE_SCAN_SKIP_RANGE)) 
          || (!rrccsp_is_supported_band(wrrc_as_id,rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].band_class))
          || ((!is_network_shared && 
               rrc_bcch_and_nas_plmns_match(wrrc_as_id,rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].plmn_id, 
...
rrc_csp_get_first_match_acq_entry(sys_modem_as_id_e_type wrrc_as_id,rrc_plmn_identity_type req_plmn_id, rrc_csp_acq_e_type req_type_acq)
{
  /*Log ACQ DB at the start of search*/
  rrc_log_full_acq_db_info(wrrc_as_id);

  rrccspdb_int_data.rrc_csp_last_req_type_acq = req_type_acq;
  rrccspdb_int_data.rrc_csp_last_req_plmn_id = req_plmn_id;
  ...
  return(rrc_csp_get_match_acq_entry(wrrc_as_id, req_plmn_id, req_type_acq));
...
===========================================================================*/
rrc_csp_acq_entry_type*  
rrc_csp_get_next_match_acq_entry(sys_modem_as_id_e_type wrrc_as_id,rrc_plmn_identity_type req_plmn_id, rrc_csp_acq_e_type req_type_acq)
{
   ...
   */
  if( rrccspdb_int_data.rrc_csp_last_req_type_acq != req_type_acq )
  {
    WRRC_MSG2_HIGH("Passed acq type %d does not match last acq type %d", 
                                        req_type_acq, rrccspdb_int_data.rrc_csp_last_req_type_acq);
    return NULL;
  }
   ...
   */
  if(rrccspdb_int_data.rrc_csp_last_req_type_acq == RRC_CSP_SPEC_PLMN)
  {
    if(!RRC_CSP_SAME_MCC(req_plmn_id, rrccspdb_int_data.rrc_csp_last_req_plmn_id))
    {
        WRRC_MSG2_HIGH("Passed plmn.MCC %d does not match last plmn.MCC %d", 
                                         RRC_CSP_GET_INT_MCC(req_plmn_id), 
                        RRC_CSP_GET_INT_MCC(rrccspdb_int_data.rrc_csp_last_req_plmn_id));
        return NULL;
    }
    if(!RRC_CSP_SAME_MNC(req_plmn_id, rrccspdb_int_data.rrc_csp_last_req_plmn_id))
    {
        WRRC_MSG2_HIGH("Passed plmn.MNC %d does not match last plmn.MNC %d", 
                                        RRC_CSP_GET_INT_MNC(req_plmn_id), 
                       RRC_CSP_GET_INT_MNC(rrccspdb_int_data.rrc_csp_last_req_plmn_id));
        return NULL;
    }
  }
...
===========================================================================*/
void rrccsp_update_band_class_in_acq_db(sys_modem_as_id_e_type wrrc_as_id)
{
  int i;
  int num_valid_entries;
  ...
  for (i = 0; i < num_valid_entries; i++)
  {
    if ((rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].band_class & 
    ...
    else if((rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].band_class & 
...
===========================================================================*/
void rrccsp_bplmn_update_stored_acq_db_rd_index (sys_modem_as_id_e_type wrrc_as_id)
{

  if(rrccspdb_int_data.stored_acq_db.curr_rd_index >= 0)
  {
    --rrccspdb_int_data.stored_acq_db.curr_rd_index;
  }
  --rrccspdb_int_data.acq_db_freq_scanned_list.wr_index;
  WRRC_MSG2_HIGH("BPLMN: Setting curr_rd_ix=%d & acq_db_scanned_list_wr_ix=%d", 
                        rrccspdb_int_data.stored_acq_db.curr_rd_index, rrccspdb_int_data.acq_db_freq_scanned_list.wr_index);
}
...
===========================================================================*/
void rrccsp_bplmn_set_stored_acq_db_rd_index (sys_modem_as_id_e_type wrrc_as_id,int8 value )
{
  rrccspdb_int_data.stored_acq_db.curr_rd_index = value;
}
...
===========================================================================*/
int rrc_csp_assert_acq_db_vars(sys_modem_as_id_e_type wrrc_as_id)
{
  ASSERT(rrccspdb_int_data.stored_acq_db.curr_wr_index <= MAX_ACQ_DB_INDEX);
  ASSERT(rrccspdb_int_data.stored_acq_db.curr_rd_index <= MAX_ACQ_DB_INDEX);
  ASSERT(rrccspdb_int_data.stored_acq_db.curr_rd_index >= -1);
  ASSERT(rrccspdb_int_data.stored_acq_db.curr_rd_index <= rrccspdb_int_data.stored_acq_db.curr_wr_index);
      ...
      (rrccspdb_int_data.stored_acq_db.last_entry_invalid == FALSE))
  {
     ...
     * to a pointer entry which is the last one and is Valid.
     */
    return (rrccspdb_int_data.stored_acq_db.curr_wr_index + 1);
  }
  else 
  {
    /* If not all pointers in acq. dbase are filled up, the write index points
     * to a pointer entry which is Invalid and still needs to be initialized.
     */
    return (rrccspdb_int_data.stored_acq_db.curr_wr_index);
  }
...
===========================================================================*/
rrc_csp_acq_entry_type *rrc_csp_lfs_update_list_db_with_target_fdd_list(sys_modem_as_id_e_type wrrc_as_id)
{
  int i;
  int j;
...
SIDE EFFECTS       None
===========================================================================*/
void rrccsp_add_acq_db_and_nw_db_entries(sys_modem_as_id_e_type wrrc_as_id,rrc_csp_acq_entry_type* acq_entry_ptr,
                                         rrc_csp_nw_sharing_entry_type* nw_sharing_entry_ptr)
{
  ...
  {
    WRRC_MSG0_MED("NWS: Current cell is not shared, not adding shared plmns");
  }

  if(rrc_csp_add_acq_entry(wrrc_as_id,acq_entry_ptr, TRUE) == SUCCESS)
  {
...
SIDE EFFECTS       None
===========================================================================*/
void rrccsp_add_plmn_acq_db_and_nw_db_entries(sys_modem_as_id_e_type wrrc_as_id,
                                              ...
                                              boolean is_valid_nw_sharing)
{
...
#ifdef FEATURE_WCDMA_MULTIBAND
  acq_entry.band_class  = rrc_get_frequency_band(wrrc_as_id,
                                                 rrc_csp_int_data.curr_acq_entry.freq,
                                                 plmn_id);
  WRRC_MSG2_MED("band_class %d for freq %d", acq_entry.band_class, acq_entry.freq);
  ...
  rrccsp_add_acq_db_and_nw_db_entries(wrrc_as_id, &acq_entry, nw_sharing_entry_ptr);
}
...
===========================================================================*/
void rrccsp_nw_sharing_print_entry(sys_modem_as_id_e_type wrrc_as_id,rrc_csp_nw_sharing_entry_type* nw_sharing_entry_ptr)
{
  int i;
...
===========================================================================*/
boolean rrccsp_nas_plmn_equivalent_to_any_of_shared_bcch_plmns(sys_modem_as_id_e_type wrrc_as_id,
                                                               rrc_plmn_identity_type nas_plmn, 
                                                               uint8 index_in_acq_or_shared_db)
{
  rrc_plmn_identity_type bcch_plmn;
  rrc_csp_acq_entry_type *acq_entry = &rrccspdb_int_data.acq_list[index_in_acq_or_shared_db];
  int i;
...
===========================================================================*/
boolean rrccsp_nas_plmn_match_any_of_bcch_plmns(sys_modem_as_id_e_type wrrc_as_id,
                                                rrc_plmn_identity_type nas_plmn, 
                                                uint8 index_in_acq_or_shared_db)
{
  rrc_plmn_identity_type bcch_plmn;
  ...
  if(acq_entry.is_valid_nw_sharing)
  {
    ...
    {
      bcch_plmn = acq_entry.shared_list_of_plmns.shared_plmns_access_info[i].plmn_id;
      if(rrc_bcch_and_nas_plmns_match(wrrc_as_id,bcch_plmn, nas_plmn))
      {
        return TRUE;
      }
    }
...
===========================================================================*/
boolean rrccsp_is_any_of_bcch_plmns_in_bplmn_list(sys_modem_as_id_e_type wrrc_as_id,rrc_csp_nw_sharing_entry_type bplmn_nw_db_entry)
{
uint32  i;
uint32  j;
  rrc_plmn_identity_type      plmn_id;
...
)
{
  ...
  WRRC_MSG0_MED("WBP: Adding frequency bands from ACQ DB");

  for (i = 0; ((i < num_valid_entries) && (num_of_bands_already_present < MAX_NUM_OF_WCDMA_BANDS)); i++)
  {
    if((rrccsp_is_supported_band(wrrc_as_id,rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].band_class) &&
        !rrccsp_is_band_already_added_to_prioritized_list(wrrc_as_id,rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].band_class,
                                                          num_of_bands_already_present)))
    {
      WRRC_MSG1_HIGH("WBP: Adding ACQ DB band %d to prioritized band list",rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].band_class);
      band_list[num_of_bands_already_present++] = rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].band_class;
    }
  }
...
===========================================================================*/
static void rrc_csp_list_db_exhausted(sys_modem_as_id_e_type wrrc_as_id)
{
...
===========================================================================*/
rrc_csp_acq_entry_type*  
rrc_csp_get_list_db_match_acq_entry(sys_modem_as_id_e_type wrrc_as_id,
  ...
  */
  ...
  else
  {
          ...
          (curr_freq_index < L1_MAX_SCAN_FREQ))
    {
      ...
      WRRC_MSG2_MED("LFS_COOS: curr_freq_index = [%d]--->freq=[%d]",curr_freq_index,freq);
  ...
  if((rrc_csp_int_data.curr_scan == RRC_CSP_CELL_SELECTION_SCAN)&&
     ...
     (!rrccsp_is_csg_white(wrrc_as_id,rrccsp_get_curr_nas_requested_csg_id(wrrc_as_id),rrccsp_get_curr_nas_requested_plmn(wrrc_as_id))))
  {
      for(i=0;i<MAX_ACQ_DB_ENTRIES_TEMP;i++)
      {
        if( (freq  == rrccspdb_int_data.acq_list_temp[i].freq) &&
             (RRC_CSP_SAME_MCC(plmn_id, rrccspdb_int_data.acq_list_temp[i].plmn_id)) &&
             (RRC_CSP_SAME_MNC(plmn_id, rrccspdb_int_data.acq_list_temp[i].plmn_id))
          )
        {
          /* update the acq_db_freq_scanned_list with the matching freq 
               from the acq entry
          */
          WRRC_MSG1_MED("LFS_COOS: Matching acq entry. update the freq=%d into the scanned list",freq);
          rrc_csp_update_acq_db_freq_scanned_list(wrrc_as_id,freq);
          WRRC_MSG1_MED("LFS_COOS: new freq idx=%d ",
                   rrc_csp_curr_select_data.scan_info.list_scan.curr_freq_index);
          /* This is the acquisition entry with which CSP will try for acquisition */
          return &rrccspdb_int_data.acq_list_temp[i];  
        }
      }
  }
  else
#endif
  {
      for(i=0;i<MAX_ACQ_DB_SIZE;i++)
      {
        if( (freq  == rrccspdb_int_data.acq_list[i].freq) &&
             (RRC_CSP_SAME_MCC(plmn_id, rrccspdb_int_data.acq_list[i].plmn_id)) &&
             (RRC_CSP_SAME_MNC(plmn_id, rrccspdb_int_data.acq_list[i].plmn_id))
          )
        {
          /* update the acq_db_freq_scanned_list with the matching freq 
               from the acq entry
          */
          ...
          WRRC_MSG1_MED("LFS_COOS: new freq idx=%d ",
                   rrc_csp_curr_select_data.scan_info.list_scan.curr_freq_index);
          /* This is the acquisition entry with which CSP will try for acquisition */
          return &rrccspdb_int_data.acq_list[i];  
        }
      }
  }
    ...
    rrc_csp_list_db_exhausted(wrrc_as_id);
  }
...
===========================================================================*/
void rrccsp_update_match_acq_entries_into_list_db(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  WRRC_MSG0_MED("LFS_COOS: LIST DB UPDATE ");
...
#ifdef FEATURE_FEMTO_CSG
if((req_csg_id != SYS_CSG_ID_INVALID)&&(!(rrccsp_is_csg_white(wrrc_as_id,req_csg_id,req_plmn_id))))
{
  WRRC_MSG1_HIGH("CSG: Checking entry from temp db. CSG ID: %d",req_csg_id);
  ASSERT(rrccspdb_int_data.stored_acq_db_temp.curr_wr_index <= MAX_ACQ_DB_INDEX_TEMP);
  ASSERT(rrccspdb_int_data.stored_acq_db_temp.curr_rd_index <= MAX_ACQ_DB_INDEX_TEMP);
  ASSERT(rrccspdb_int_data.stored_acq_db_temp.curr_rd_index >= -1);
  ASSERT(rrccspdb_int_data.stored_acq_db_temp.curr_rd_index <= rrccspdb_int_data.stored_acq_db_temp.curr_wr_index);

    /* Compute Number of Valid entries */
  if ((rrccspdb_int_data.stored_acq_db_temp.curr_wr_index == MAX_ACQ_DB_INDEX_TEMP) && 
      (rrccspdb_int_data.stored_acq_db_temp.last_entry_invalid == FALSE))
  {
    /* If all pointers in acq. dbase are filled up, the write index points
     * to a pointer entry which is the last one and is Valid.
     */
    num_valid_entries = rrccspdb_int_data.stored_acq_db_temp.curr_wr_index + 1;
  }
  else 
  {
    /* If not all pointers in acq. dbase are filled up, the write index points
    * to a pointer entry which is Invalid and still needs to be initialized.
    */
    num_valid_entries = rrccspdb_int_data.stored_acq_db_temp.curr_wr_index;
  }
  ...
  for (i = 0; i < num_valid_entries; i++)
  {
    ...
    /* Collect band information for frequencies present in ACQ DB */
    if((rrc_csp_int_data.ue_in_deep_sleep) && 
        (rrccsp_is_supported_band(wrrc_as_id,rrccspdb_int_data.acq_list_temp[rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i]].band_class)))
    {
      rrc_csp_int_data.deep_sleep_acq_db_band_mask |= 
                                             rrccspdb_int_data.acq_list_temp[rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i]].band_class;
    }
#endif
    is_network_shared = rrccspdb_int_data.acq_list_temp[rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i]].is_valid_nw_sharing;
    ...
    if(rrc_csp_curr_select_data.acq_type == RRC_CSP_SPEC_PLMN)
    {
          ...
          is_acq_db_plmn_matched = FALSE;
          if((!is_network_shared) &&
             (rrc_bcch_and_nas_plmns_match(wrrc_as_id,rrccspdb_int_data.acq_list_temp[rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i]].plmn_id, 
                                           req_plmn_id))&&
             (rrccspdb_int_data.acq_list_temp[rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i]].csg_id == req_csg_id))
          {
            is_acq_db_plmn_matched = TRUE;
          }
          else if((is_network_shared) &&
                  (rrccsp_nas_plmn_match_any_of_bcch_plmns(wrrc_as_id,
                                                       req_plmn_id,
                                                       rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i]))&&
                  (rrccspdb_int_data.acq_list_temp[rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i]].csg_id == req_csg_id))
          {
            is_acq_db_plmn_matched = TRUE;
          }
    ...
    if((is_acq_db_plmn_matched
            ...
            rrccspdb_int_data.acq_list_temp[rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i]].freq, RRCCSP_FINE_SCAN_SKIP_RANGE))
            && (rrccsp_is_supported_band(wrrc_as_id,rrccspdb_int_data.acq_list_temp[rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i]].band_class))
            && (!rrc_csp_has_acq_db_freq_been_scanned(wrrc_as_id,
                    rrccspdb_int_data.acq_list_temp[rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i]].freq))))
    {
      ...
      {
        if (rrccspdb_int_data.acq_list_temp[rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i]].freq == 
      ...
      if(is_freq_add_into_lfs_list)
      {
         ...
         * return pointer to the acquisition list contained in this db pointer entry
         */
        rrccspdb_int_data.stored_acq_db_temp.curr_rd_index = (int8)i;
        ret_acq_entry_ptr = &rrccspdb_int_data.acq_list_temp[rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i]];
        ...
        WRRC_MSG3_HIGH("LFS_COOS: Added an entry in lfs table, freq=%d, plmn type = %d, num_freq=%d ",
               rrc_csp_curr_select_data.scan_info.list_scan.freq_result[rrc_csp_curr_select_data.scan_info.list_scan.num_freq].ctr_freq,
...
#endif
{
  ...
  WRRC_MSG2_HIGH("LFS_COOS: no of valid entries =%d, curr_rd_index=%d",
           num_valid_entries,rrccspdb_int_data.stored_acq_db.curr_rd_index);
  ...
  for (i = 0; i < num_valid_entries; i++)
  {
    ...
    if(rrc_csp_curr_select_data.acq_type == RRC_CSP_SPEC_PLMN)
    {
                                           ...
                                           req_plmn_id)))
          {
            is_acq_db_plmn_matched = TRUE;
#ifdef FEATURE_FEMTO_CSG
            if((req_csg_id != SYS_CSG_ID_INVALID)&&(!(rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].csg_id == req_csg_id)))
            {
              is_acq_db_plmn_matched = FALSE;
            }
#endif
                    ...
                    rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].freq))))
    {
      ...
      for (iterator =0;iterator<rrc_csp_curr_select_data.scan_info.list_scan.num_freq;iterator++)
      {
        if (rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].freq == 
            rrc_csp_curr_select_data.scan_info.list_scan.freq_result[iterator].ctr_freq
           )
        {
          is_freq_add_into_lfs_list = FALSE;
...
SIDE EFFECTS       None
===========================================================================*/
void wcdma_rrc_get_acqdb_cell_list(sys_modem_as_id_e_type wrrc_as_id, wcdma_acqdb_cell_list  *wcdma_acqdb_cell_list_ptr)
{
  int i;
  wcdma_acqdb_cell_list_ptr->num_valid_entries = 0;

  for(i=0; (i<(rrccspdb_int_data.stored_acq_db.curr_wr_index + !rrccspdb_int_data.stored_acq_db.last_entry_invalid)) && (wcdma_acqdb_cell_list_ptr->num_valid_entries < MAX_ACQ_DB_MACRO_ENTRIES); i++)
  {
#ifdef FEATURE_FEMTO_CSG
    /* Skip CSG cells */
    if(rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].csg_id == SYS_CSG_ID_INVALID)
#endif
    {
       wcdma_acqdb_cell_list_ptr->cellInfoList[wcdma_acqdb_cell_list_ptr->num_valid_entries].freq= rrccspdb_int_data.acq_list[rrccspdb_int_data.stored_acq_db.acq_list_indices[i]].freq;
...
SIDE EFFECTS       None
===========================================================================*/
uecomdef_status_e_type rrccsp_add_acq_db_and_nw_db_entries_temp(sys_modem_as_id_e_type wrrc_as_id,
                                                           ...
                                                           rrc_csp_nw_sharing_entry_type* nw_sharing_entry_ptr)
{
  ...
  if(rrc_csp_add_acq_entry_temp(wrrc_as_id,acq_entry_ptr) == SUCCESS)
  {
    status = SUCCESS;
    acq_entry_ptr->plmn_id = chosen_plmn;
  }
...
===========================================================================*/
uecomdef_status_e_type  rrc_csp_add_acq_entry_temp(sys_modem_as_id_e_type wrrc_as_id,rrc_csp_acq_entry_type* acq_entry_ptr)
{
   ...
   */
  if ((rrccspdb_int_data.stored_acq_db_temp.curr_wr_index > MAX_ACQ_DB_INDEX_TEMP) || 
      (rrccspdb_int_data.stored_acq_db_temp.curr_rd_index > MAX_ACQ_DB_INDEX_TEMP) ||
      (rrccspdb_int_data.stored_acq_db_temp.curr_rd_index < -1) ||
      (rrccspdb_int_data.stored_acq_db_temp.curr_rd_index > rrccspdb_int_data.stored_acq_db_temp.curr_wr_index))
  {
    WRRC_MSG2_ERROR("ERROR: Acq db invalid Wr index %d Rd index %d",
                rrccspdb_int_data.stored_acq_db_temp.curr_wr_index, rrccspdb_int_data.stored_acq_db_temp.curr_rd_index);
    return(FAILURE);
  }
  ...
  for (i = 0; i < num_valid_entries; i++)
  {
    if ((rrc_bcch_plmns_match(wrrc_as_id,rrccspdb_int_data.acq_list_temp[rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i]].plmn_id, 
                              acq_entry_ptr->plmn_id)) &&
        (rrccspdb_int_data.acq_list_temp[rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i]].freq == acq_entry_ptr->freq)&&
        (rrccspdb_int_data.acq_list_temp[rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i]].csg_id == acq_entry_ptr->csg_id))
    {
      /* Here, we found an acq. entry with same PLMN and freq. This acq. entry 
       * is overwritten with the acq. data provided in the parameter. All acq.
       * database pointers, starting from the first (topmost) and going till the
       ...
       * is unaffected.
       */
      wr_acq_list_index = rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i];

      for (ptr_index = i; ptr_index > 0; ptr_index--)
      {
        rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[ptr_index] = 
          rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[ptr_index-1];
      }

      rrccspdb_int_data.acq_list_temp[wr_acq_list_index] = *acq_entry_ptr;
      band_class = rrc_get_frequency_band(wrrc_as_id,acq_entry_ptr->freq, acq_entry_ptr->plmn_id);
      rrccspdb_int_data.acq_list_temp[wr_acq_list_index].band_class = band_class;

      /* Initialize the topmost pointer to the most recent acq. list entry */
      rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[0] = (uint8)wr_acq_list_index;
      /* Return always a Success for the time being */
      WRRC_MSG3_HIGH("Updated: freq %d scr_code %d band %d in Acq DB", 
                              acq_entry_ptr->freq, acq_entry_ptr->scr_code, band_class);
  ...
  if ((rrccspdb_int_data.stored_acq_db_temp.curr_wr_index == MAX_ACQ_DB_INDEX_TEMP) &&
      (rrccspdb_int_data.stored_acq_db_temp.last_entry_invalid == FALSE))
  {
    wr_acq_list_index = rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[rrccspdb_int_data.stored_acq_db.curr_wr_index];
  }
  else
  {
    wr_acq_list_index = rrccspdb_int_data.stored_acq_db_temp.curr_wr_index;
  }
   ...
   * this acq. entry in the first (topmost) acq. database pointer.
   */
  for (i = rrccspdb_int_data.stored_acq_db_temp.curr_wr_index; i > 0; i--)
  {
    rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i] = 
      rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[i-1];
  }
  /* Initialize the topmost pointer to the most recent acq. list entry */
  rrccspdb_int_data.stored_acq_db_temp.acq_list_indices[0] = (uint8)wr_acq_list_index;
  ...
  if (rrccspdb_int_data.stored_acq_db_temp.curr_wr_index < MAX_ACQ_DB_INDEX_TEMP)
  {
    rrccspdb_int_data.stored_acq_db_temp.curr_wr_index++;
    rrccspdb_int_data.stored_acq_db_temp.last_entry_invalid = TRUE;
  }
  else
  {
...
===========================================================================*/
boolean rrccsp_fp_macro_present_in_acq_db(sys_modem_as_id_e_type wrrc_as_id,rrc_csp_acq_entry_type acq_db_entry)
{
  ...
  {
    macro_plmn=rrc_ccm_get_curr_camped_plmn(wrrc_as_id);
    macro_freq = rrc_ccm_get_curr_camped_freq(wrrc_as_id);
    macro_psc= rrc_ccm_get_curr_camped_psc(wrrc_as_id);
    macro_cell_id= rrc_ccm_get_curr_camped_cell_id(wrrc_as_id);
  }
  ...
  if(is_lte)
  {
    ...
    {
      if(rrc_bcch_plmns_match(wrrc_as_id,acq_db_entry.lte_macro[j].plmn_id,macro_plmn)
  ...
  { 
    ...
    {
      if(rrc_bcch_plmns_match(wrrc_as_id,acq_db_entry.w_macro[j].plmn_id,macro_plmn)
...
===========================================================================*/
void rrccsp_prune_acqdb_after_acq(sys_modem_as_id_e_type wrrc_as_id,boolean acq_status)
{
  uint8 i,j,k;
  rrc_plmn_identity_type macro_plmn;
  uint32 macro_freq;
  ...
  for(i=0; i< (rrccspdb_int_data.stored_acq_db.curr_wr_index + !rrccspdb_int_data.stored_acq_db.last_entry_invalid); i++)
  {
    if((rrccspdb_int_data.acq_list[i].freq != rrc_csp_int_data.curr_acq_entry.freq)||
       (rrccspdb_int_data.acq_list[i].csg_id == SYS_CSG_ID_INVALID))
    {
      continue;
    }
    if(!is_lte)
    {
      for(j=0; j<rrccspdb_int_data.acq_list[i].fp_w_macro_cnt; j++)
      {
        if(rrc_bcch_plmns_match(wrrc_as_id,rrccspdb_int_data.acq_list[i].w_macro[j].plmn_id,macro_plmn)
           &&(rrccspdb_int_data.acq_list[i].w_macro[j].freq == macro_freq)
           &&(rrccspdb_int_data.acq_list[i].w_macro[j].psc == macro_psc)
           &&(rrccspdb_int_data.acq_list[i].w_macro[j].cell_id == macro_cell_id))
        {
          /* acq failure cases */
          if(!acq_status)
          {
            if(rrccspdb_int_data.acq_list[i].w_macro[j].acq_fail_counter >= rrcnv_data.wcdma_rrc_csg_max_prune_count_nv)
            {
              rrccspdb_int_data.acq_list[i].w_macro[j].acq_fail_counter = 0;
              rrccsp_remove_fp_info(wrrc_as_id,i,j,TRUE);
              WRRC_MSG2_HIGH("CSG: Removed FP assocation for entry %d FP entry %d",i,j);
            }
            else
            {
              rrccspdb_int_data.acq_list[i].w_macro[j].acq_fail_counter++;
              WRRC_MSG3_HIGH("CSG: Acq fail counter for entry %d FP entry %d : %d",i,j,rrccspdb_int_data.acq_list[i].w_macro[j].acq_fail_counter);
            }
          }
          else
          {
            match_found = FALSE;
            if(rrccspdb_int_data.acq_list[i].scr_code == rrc_csp_int_data.curr_acq_entry.scr_code)
            {
              match_found = TRUE;
            ...
            {
              if(rrccspdb_int_data.acq_list[i].scr_code == rrc_csp_int_data.scr_code_det[k].scr_code)
              {
                match_found = TRUE;
                WRRC_MSG2_HIGH("CSG: Match is found do not touch counter, entry %d, FP entry %d",i,j);
                break;
    ...
    {
      for(j=0; j<rrccspdb_int_data.acq_list[i].fp_lte_macro_cnt; j++)
      {
        if(rrc_bcch_plmns_match(wrrc_as_id,rrccspdb_int_data.acq_list[i].lte_macro[j].plmn_id,macro_plmn)
           &&(rrccspdb_int_data.acq_list[i].lte_macro[j].freq == macro_freq)
           &&(rrccspdb_int_data.acq_list[i].lte_macro[j].pci == macro_psc)
           &&(rrccspdb_int_data.acq_list[i].lte_macro[j].cell_id == macro_cell_id))
        {
          /* acq failure cases */
          if(!acq_status)
          {
            if(rrccspdb_int_data.acq_list[i].lte_macro[j].acq_fail_counter >= rrcnv_data.wcdma_rrc_csg_max_prune_count_nv)
            {
              rrccspdb_int_data.acq_list[i].lte_macro[j].acq_fail_counter = 0;
              rrccsp_remove_fp_info(wrrc_as_id,i,j,FALSE);
              WRRC_MSG2_HIGH("CSG: Removed FP assocation for entry %d FP entry %d",i,j);
            }
            else
            {
              rrccspdb_int_data.acq_list[i].lte_macro[j].acq_fail_counter++;
              WRRC_MSG3_HIGH("CSG: Acq fail counter for entry %d FP entry %d : %d",i,j,rrccspdb_int_data.acq_list[i].lte_macro[j].acq_fail_counter);
            }
          }
          else
          {
...
===========================================================================*/
void rrccsp_prune_acqdb_after_SIB(sys_modem_as_id_e_type wrrc_as_id,rrc_int_csg_id_type csg_id, boolean is_white)
{
  uint8 i,j;
  rrc_plmn_identity_type macro_plmn;
  uint32 macro_freq;
  ...
  {
    if((rrccspdb_int_data.acq_list[i].freq != rrc_csp_int_data.curr_acq_entry.freq)||
       (rrccspdb_int_data.acq_list[i].scr_code != rrc_csp_int_data.curr_acq_entry.scr_code)||
       (rrccspdb_int_data.acq_list[i].csg_id == SYS_CSG_ID_INVALID))
    {
      continue;
    }
    if(!is_lte)
    {
      for(j=0; j<rrccspdb_int_data.acq_list[i].fp_w_macro_cnt; j++)
      {
        if(rrc_bcch_plmns_match(wrrc_as_id,rrccspdb_int_data.acq_list[i].w_macro[j].plmn_id,macro_plmn)
           ...
           &&(rrccspdb_int_data.acq_list[i].w_macro[j].cell_id == macro_cell_id))
        {
          if(csg_id != rrccspdb_int_data.acq_list[i].csg_id)
          {
            if(rrccspdb_int_data.acq_list[i].w_macro[j].acq_fail_counter >= rrcnv_data.wcdma_rrc_csg_max_prune_count_nv)
            {
              rrccspdb_int_data.acq_list[i].w_macro[j].acq_fail_counter = 0;
...
===========================================================================*/
void rrccsp_remove_fp_info(sys_modem_as_id_e_type wrrc_as_id,uint8 acq_index, uint8 fp_index, boolean is_w_macro)
{
  uint8 i=0;

  if(is_w_macro)
  {
    for(i = fp_index; i < rrccspdb_int_data.acq_list[acq_index].fp_w_macro_cnt - 1; i++)
    {
      rrccspdb_int_data.acq_list[acq_index].w_macro[i] = rrccspdb_int_data.acq_list[acq_index].w_macro[i+1];
    }
    rrccspdb_int_data.acq_list[acq_index].fp_w_macro_cnt--;
  }
  else
  {
    for(i = fp_index; i < rrccspdb_int_data.acq_list[acq_index].fp_lte_macro_cnt - 1; i++)
    {
      rrccspdb_int_data.acq_list[acq_index].lte_macro[i] = rrccspdb_int_data.acq_list[acq_index].lte_macro[i+1];
    }
    rrccspdb_int_data.acq_list[acq_index].fp_lte_macro_cnt--;
  }
}
...
===========================================================================*/
void rrccsp_log_macro_fp_info(sys_modem_as_id_e_type wrrc_as_id,uint8 rat )
{
  ...
  log_record_ptr = (WCDMA_FP_INFO_type *)WRRC_LOG_ALLOC( WCDMA_FP_INFO, sizeof(WCDMA_FP_INFO_type));
  ...
  {
    plmn = rrc_ccm_get_curr_camped_plmn(wrrc_as_id);
    freq = rrc_ccm_get_curr_camped_freq(wrrc_as_id);
    psc = rrc_ccm_get_curr_camped_psc(wrrc_as_id);
    cell_id = rrc_ccm_get_curr_camped_cell_id(wrrc_as_id);
  }
  else
  {
  ...
  num_valid_entries = rrccspdb_int_data.stored_acq_db.curr_wr_index + !rrccspdb_int_data.stored_acq_db.last_entry_invalid;
  for(i = 0; i < num_valid_entries; i++)
  {
    if(rat == W_ASF_RAT_WCDMA)
    {
      for(j = 0;((j < rrccspdb_int_data.acq_list[i].fp_w_macro_cnt)&&(log_record_ptr->num_csg_entries < MAX_ACQ_DB_CSG_ENTRIES)); j++)
      {
        if(rrc_bcch_plmns_match(wrrc_as_id,rrccspdb_int_data.acq_list[i].w_macro[j].plmn_id,plmn)
           &&(rrccspdb_int_data.acq_list[i].w_macro[j].freq == freq)
           &&(rrccspdb_int_data.acq_list[i].w_macro[j].psc == psc)
           &&(rrccspdb_int_data.acq_list[i].w_macro[j].cell_id == cell_id)
           &&(rrccsp_is_csg_white(wrrc_as_id,rrccspdb_int_data.acq_list[i].csg_id,rrccspdb_int_data.acq_list[i].plmn_id)))
        {
          log_record_ptr->csg_cells[log_record_ptr->num_csg_entries].uarfcn = rrccspdb_int_data.acq_list[i].freq;
          log_record_ptr->csg_cells[log_record_ptr->num_csg_entries].psc = rrccspdb_int_data.acq_list[i].scr_code/16;
          log_record_ptr->csg_cells[log_record_ptr->num_csg_entries].cell_id = rrccspdb_int_data.acq_list[i].cell_id;
          log_record_ptr->csg_cells[log_record_ptr->num_csg_entries].csg_id = rrccspdb_int_data.acq_list[i].csg_id;
          log_record_ptr->csg_cells[log_record_ptr->num_csg_entries].mcc[0] = rrccspdb_int_data.acq_list[i].plmn_id.mcc[0];
          log_record_ptr->csg_cells[log_record_ptr->num_csg_entries].mcc[1] = rrccspdb_int_data.acq_list[i].plmn_id.mcc[1];
          log_record_ptr->csg_cells[log_record_ptr->num_csg_entries].mcc[2] = rrccspdb_int_data.acq_list[i].plmn_id.mcc[2];
          log_record_ptr->csg_cells[log_record_ptr->num_csg_entries].mnc[0] = rrccspdb_int_data.acq_list[i].plmn_id.mnc[0];
          log_record_ptr->csg_cells[log_record_ptr->num_csg_entries].mnc[1] = rrccspdb_int_data.acq_list[i].plmn_id.mnc[1];
          log_record_ptr->csg_cells[log_record_ptr->num_csg_entries].mnc[2] = rrccspdb_int_data.acq_list[i].plmn_id.mnc[2];
          if (plmn.num_mnc_digits == 2) 
          {
    ...
    {
      for(j = 0;((j < rrccspdb_int_data.acq_list[i].fp_lte_macro_cnt)&&(log_record_ptr->num_csg_entries < MAX_ACQ_DB_CSG_ENTRIES)); j++)
      {
        if(rrc_bcch_plmns_match(wrrc_as_id,rrccspdb_int_data.acq_list[i].lte_macro[j].plmn_id,plmn)
           &&(rrccspdb_int_data.acq_list[i].lte_macro[j].freq == freq)
           &&(rrccspdb_int_data.acq_list[i].lte_macro[j].pci == psc)
           &&(rrccspdb_int_data.acq_list[i].lte_macro[j].cell_id == cell_id)
           &&(rrccsp_is_csg_white(wrrc_as_id,rrccspdb_int_data.acq_list[i].csg_id,rrccspdb_int_data.acq_list[i].plmn_id)))
        {
          log_record_ptr->csg_cells[log_record_ptr->num_csg_entries].uarfcn = rrccspdb_int_data.acq_list[i].freq;
          log_record_ptr->csg_cells[log_record_ptr->num_csg_entries].psc = rrccspdb_int_data.acq_list[i].scr_code/16;
...
FUNCTION          WCDMA_RRC_GET_WCSG_LFP_INFO_SUB
...
===========================================================================*/
uint8 wcdma_rrc_get_wcsg_lfp_info_sub(sys_modem_as_id_e_type wrrc_as_id, const source_cell_info_type *l_macro_ptr, source_cell_info_type *w_csg_cells_ptr )
{
  ...
  for(i = 0; i < num_valid_entries; i++)
  {
      for(j = 0;((j < rrccspdb_int_data.acq_list[i].fp_lte_macro_cnt)&&(k < LTE_IRAT_MAX_MACRO_CELLS)); j++)
      {
        if(rrc_bcch_plmns_match(wrrc_as_id,rrccspdb_int_data.acq_list[i].lte_macro[j].plmn_id,plmn)
           &&(rrccspdb_int_data.acq_list[i].lte_macro[j].freq == freq)
           &&(rrccspdb_int_data.acq_list[i].lte_macro[j].pci == psc)
           ...
           &&(rrccsp_is_csg_white(wrrc_as_id,rrccspdb_int_data.acq_list[i].csg_id,rrccspdb_int_data.acq_list[i].plmn_id)))
        {
          w_csg_cells_ptr[k].frequency     = rrccspdb_int_data.acq_list[i].freq;
          w_csg_cells_ptr[k].phy_cell_id   = rrccspdb_int_data.acq_list[i].scr_code/16;
          w_csg_cells_ptr[k].cell_identity = rrccspdb_int_data.acq_list[i].cell_id;
          w_csg_cells_ptr[k].csg_id        = rrccspdb_int_data.acq_list[i].csg_id;
          w_csg_cells_ptr[k].plmn          = mmumts_convert_rrc_plmn_id_to_nas_plmn_id(rrccspdb_int_data.acq_list[i].plmn_id);
          k++;
        }
      }
...
uint8 wcdma_rrc_get_wcsg_lfp_info(const source_cell_info_type *l_macro_ptr, source_cell_info_type *w_csg_cells_ptr )
{
  ...
  return wcdma_rrc_get_wcsg_lfp_info_sub(SYS_MODEM_AS_ID_1, l_macro_ptr, w_csg_cells_ptr);
}
...
===========================================================================*/
boolean rrccsp_check_wcdma_fp_info(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  { 
    fp_entry_found = rrccsp_fp_macro_present_in_acq_db(wrrc_as_id,rrccspdb_int_data.acq_list[i]);
  }
...
/*====================================================================
FUNCTION: rrc_log_full_acq_db_info

DESCRIPTION:
  This function logs the unique ACQ DB frequencies.
  Log code 0x4802
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrc_log_full_acq_db_info(sys_modem_as_id_e_type wrrc_as_id)
{
  WCDMA_ACQDB_NEW_INFO_type* log_record_ptr = NULL;
  int8 num_valid_entries = 0;
  uint8 i,j,k=0;
  ...
  for(i = 0;i < num_valid_entries; i++)
  {
    for(j = 0;j < k;j++)
    {
      if(rrccspdb_int_data.acq_list[i].freq == log_record_ptr->info[j].uarfcn)
      {
        is_continue = TRUE;
      }
    }
    ...
    log_record_ptr->info[k].mcc[0] = rrccspdb_int_data.acq_list[i].plmn_id.mcc[0];
    log_record_ptr->info[k].mcc[1] = rrccspdb_int_data.acq_list[i].plmn_id.mcc[1];
    log_record_ptr->info[k].mcc[2] = rrccspdb_int_data.acq_list[i].plmn_id.mcc[2];
    log_record_ptr->info[k].mnc[0] = rrccspdb_int_data.acq_list[i].plmn_id.mnc[0];
    log_record_ptr->info[k].mnc[1] = rrccspdb_int_data.acq_list[i].plmn_id.mnc[1];
    log_record_ptr->info[k].mnc[2] = rrccspdb_int_data.acq_list[i].plmn_id.mnc[2];
    log_record_ptr->info[k].uarfcn = rrccspdb_int_data.acq_list[i].freq;
    log_record_ptr->info[k].cell_id = rrccspdb_int_data.acq_list[i].cell_id;
    log_record_ptr->info[k].csg_id = rrccspdb_int_data.acq_list[i].csg_id;
    log_record_ptr->info[k].psc = rrccspdb_int_data.acq_list[i].scr_code/16;
    k++;
