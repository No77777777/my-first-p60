...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrccspi.h#8 $    $DateTime: 2020/07/13 12:41:39 $ $Author: pwbldsvc $
...
06/13/17   ha      FR42544 MPLMN periodic scan search
...
02/29/16   vs      Changes to remove boolean abort_due_to_preemption from rrc_csp_handle_gtow_lock_failure_or_preemption()
02/23/16   sn      Changes to handle NBCH drop failure with modify band fail cause
...
10/26/15   vs      Made changes to remove the enum featurization
09/07/15   sas     Changes to double the wait timers during L2W redirection for Dual SIM cases.
...
/*20s guard timer for periodic frequency scan search*/
#define RRCTMR_PERIODIC_MPLMN_FREQ_SCAN_GUARD_SRCH_TIMER_IN_MS 20000UL
...
#ifdef FEATURE_DUAL_SIM
#define RRCTMR_LTE_TO_WCDMA_REDIR_SRCH_TIMER_IN_MS  ((rrc_dsds_int_data.wcdma_is_ue_in_dual_standby)? 2000:1000)
#define RRCTMR_LTE_TO_WCDMA_REDIR_EXT_SRCH_TIMER_IN_MS  ((rrc_dsds_int_data.wcdma_is_ue_in_dual_standby)? 10000:5000)
#else
...
/*32 (LTE_RRC_CSP_MAX_ACQ_DB_EXT_ENTRIES)+10 (MAX_EUTRA_FREQUENCIES)*/
#define MAX_FORBIDDEN_EARFCN 42
#endif
/* -----------------------------------------------------------------------
** Type Declarations
** ----------------------------------------------------------------------- */
...
typedef enum
{
  ...
  /* 24: Scan triggered when leaving CELL_FACH state for URA_PCH state */
  RRC_CSP_GSM_TO_WCDMA_REDIRECTION_SCAN

#ifdef FEATURE_DUAL_SIM
  /* 25 UMTS UNLOCK BY Scan */
...
typedef enum
{
  ...
  RRC_CSP_PREEMPT_IND_PENDING, /* FEATURE_DUAL_SIM */ 
...
typedef enum
{
  RRC_CSP_DO_NOT_SKIP_FREQ,          /*This is same as FALSE value before*/
  RRC_CSP_SKIP_FREQ,                 /*This is same as TRUE value before*/
  RRC_CSP_SKIP_FREQ_FPLMN,             /*This is introduced to remember the reason for skipping previously camped freq*/
  RRC_CSP_SKIP_FREQ_DIFF_PLMN          /*This is introduced to for skipping previously camped freq in case of diff plmn*/
}lcf_in_idle_mode_skip_status_type;
...
{
  uint8  count;
  rrc_plmn_identity_type plmn_list[MAX_PREFERRED_PLMN_LIST_SIZE];
}rrc_csp_preferred_plmn_list_type;

/*W and L frequency list for periodic scan search*/
typedef struct
{
  uint8  total_lte_freq;
  uint8  total_wcdma_freq;
  uint16  wcdma_freq_list[MAX_WCDMA_ARFCN_LIST_COUNT];
  uint32  lte_freq_list[MAX_LTE_ARFCN_LIST_COUNT]; 
  sys_srch_priority_e_type  priority_info;
  rrc_csp_preferred_plmn_list_type  pref_plmn_list;
}rrc_csp_mplmn_freq_info;
...
typedef struct
{
  ...
  rrc_csp_cmd_data_u_type cmd;

} rrc_csp_cmd_type;

typedef struct
{
  /* Number of forbidden EARFCNs */
  uint8 num_earfcn;

  /* Forbidden EARFCN list */
  uint32 earfcn[MAX_FORBIDDEN_EARFCN];
}rrc_forbidden_earfcn_list;
...
typedef struct
{
  ...
  rrc_csp_cmd_type              saved_rrc_cmd;
...
/* Stores the curr cell paging response status */
  ...
  */
  ...
  lcf_in_idle_mode_skip_status_type lcf_in_idle_mode_skip_status;
 ...
 */
  boolean bplmn_srch_status_svd;

  /* Indicates if BPLMN data is saved as a result of moving to connected mode */
  boolean bplmn_data_saved;

/* Indicates if BST band preference has valid information or not */
  boolean rrc_bst_band_pref_incl;

#ifdef FEATURE_BPLMN_SEARCH_320MS
  boolean wcdma_resume_failed;
#endif

#ifdef FEATURE_3GPP_CSFB
  boolean rrc_csfb_call_status;
#ifdef FEATURE_WCDMA_CSFB_CALL_OPT
  boolean rrc_csfb_reset_while_waiting_for_act_rsp;
#endif
#endif

  uint32 xtow_bplmn_search_overhead_timer;

  boolean rrccsp_interf_resel_in_prgs;
#ifdef FEATURE_WCDMA_FAST_RETURN_TO_LTE_AFTER_CSFB
  rrccsp_csfb_status_e_type csfb_extended_status;
  boolean cs_rab_established;
#endif
  uint32 clr_sib_db_index;  //Stores the SIBDB index that has to be deleted after SIB read Skip.

  boolean attempt_freq_only_acq;

#ifdef FEATURE_DUAL_SIM
  boolean rrc_use_high_priority_for_bplmn;
  uint32  rrc_drx_count;  
#endif
/*globals introduced for periodic frequency scan search*/
  rrc_csp_mplmn_freq_info  freq_scan_list;
  boolean  mplmn_wcdma_freq_list_present;
  uint8  mplmn_freq_list_index;
  /* Redirection failed due to forbidden earfcn list */
  rrc_forbidden_earfcn_list forbidden_earfcn;

  /* write index for forbidden earfcn list */
  uint8 forbidden_earfcn_wr_index;

  /*flag to track if call ended abnormally*/
  boolean abnormal_call_ended;

    /* The  MIB PLMN ID */
  rrc_plmn_identity_type ssgccs_nws_mib_plmn_id;

  /* Current Cell ID selected - received in SIB Confirmation message */
  rrc_int_cell_id_type         ssgccs_nws_cell_id;
 ...
 */
 ...
 /*boolean to track preferred PLMN list during periodic scan search*/
  boolean preferred_plmn_found;
 ...
 */
extern  rrc_csp_select_ctrl_data_type  rrc_csp_curr_select_data_sim[MAX_RRC_STACKS];

extern  rrc_csp_select_ctrl_data_type  prev_rrc_csp_curr_select_data_sim[MAX_RRC_STACKS];

extern rrc_cell_params_int_data_type rrc_cell_params_int_data_sim[MAX_RRC_STACKS];

#ifdef FEATURE_3GPP_CSFB
extern rrc_System_Information_Container_list_type rrc_SIB_container_sim[MAX_RRC_STACKS];
#endif

#define rrc_csp_int_data rrc_csp_int_data_sim[WAS_ID_M]
...
#define rrc_cell_params_int_data rrc_cell_params_int_data_sim[WAS_ID_M]
#define rrc_SIB_container rrc_SIB_container_sim[WAS_ID_M]
...
extern rrc_cell_params_int_data_type rrc_cell_params_int_data;
...
rrc_csp_find_cell_bar_status (sys_modem_as_id_e_type wrrc_as_id, rrc_plmn_identity_type  plmn_id,
                              rrc_int_cell_id_type  cell_id,
...
FUNCTION        rrc_csp_save_preferred_plmn_list

DESCRIPTION
                This function saves preferred plmn list from NAS and RRC
                stops periodic frequency search when any PLMN in the list is found
...
===========================================================================*/
void rrc_csp_save_preferred_plmn_list(sys_modem_as_id_e_type wrrc_as_id, sys_preferred_plmn_list_s_type  *preferred_plmn_list);
...
===========================================================================*/
uecomdef_status_e_type
rrc_csp_find_access_control(sys_modem_as_id_e_type wrrc_as_id, rrc_csp_access_control_e_type  *access_control,
...
===========================================================================*/
rrc_csp_cell_actual_vs_expected_e_type
rrc_csp_compare_expected_cell(sys_modem_as_id_e_type wrrc_as_id, rrc_csp_plmn_status_e_type cell_plmn_status,
                              rrc_csp_acq_entry_type* acq_entry_ptr,
                              rrc_csp_acq_e_type type_acq_expect);
...
===========================================================================*/
uecomdef_status_e_type
rrc_csp_send_sib_cmd(sys_modem_as_id_e_type wrrc_as_id, rrc_cmd_e_type sib_cmd, uint32 sib_mask,
                     rrc_csp_acq_entry_type *acq_entry_ptr,
...
DESCRIPTION       This function retrieves frequencies from NAS
...
SIDE EFFECTS
                  None
===========================================================================*/
void  rrc_csp_init_freq_scan_data(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
uecomdef_status_e_type
rrc_csp_get_next_freq_scan_entry(sys_modem_as_id_e_type wrrc_as_id, rrc_freq_type *ret_freq);
...
===========================================================================*/
uecomdef_status_e_type
rrc_csp_update_freq_scan_entry(sys_modem_as_id_e_type wrrc_as_id, l1_freq_scan_cnf_type *freq_scan_cnf);
...
===========================================================================*/
void rrc_csp_restore_saved_bplmn_data(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
void rrc_csp_send_mm_act_ind(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
rrc_csp_substate_e_type rrc_csp_release_bch_and_continue_cell_selection(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
void
rrccsp_bplmn_update_stored_acq_db_rd_index (sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
void rrccsp_bplmn_set_stored_acq_db_rd_index (sys_modem_as_id_e_type wrrc_as_id, int8 );
...
===========================================================================*/
void 
rrccsp_send_mode_change_req_to_mcm(sys_modem_as_id_e_type wrrc_as_id,  rrc_mode_e_type rrc_mode);
...
SIDE EFFECTS
                  None
===========================================================================*/
void rrc_csp_send_bch_release(sys_modem_as_id_e_type wrrc_as_id, boolean); 
...
===========================================================================*/
rrc_csp_substate_e_type
rrccsp_handle_freq_redirect_wait_timer_expiry(sys_modem_as_id_e_type wrrc_as_id, rrc_csp_substate_e_type curr_substate);
...
===========================================================================*/
void
rrccsp_send_rce_redirect_cnf(sys_modem_as_id_e_type wrrc_as_id, rrc_redirect_status_e_type redirect_status, 
...
===========================================================================*/
boolean
rrccsp_validate_redirection_req(sys_modem_as_id_e_type wrrc_as_id,  rrc_cmd_type *cmd_ptr );
...
===========================================================================*/
rrc_csp_substate_e_type 
rrccsp_handle_failure_during_freq_redirection(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
rrc_csp_substate_e_type
rrccsp_send_cell_update_initiate_req(sys_modem_as_id_e_type wrrc_as_id, rrc_cell_update_cause_e_type cause);
...
#endif
rrc_csp_substate_e_type  rrc_get_csp_substate(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
void rrccsp_update_bplmn_srch_vars(sys_modem_as_id_e_type wrrc_as_id,
...
===========================================================================*/
void rrccsp_reset_bplmn_list_search_state(sys_modem_as_id_e_type wrrc_as_id,
    wtow_bplmn_srch_status_type status,    
    boolean                     rce_waiting, 
...
SIDE EFFECTS      NONE
===========================================================================*/
wtow_bplmn_srch_status_type rrc_csp_bplmn_srch_in_progress(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
void rrccsp_send_bplmn_srch_resume_ind(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
boolean rrccsp_plmn_list_req_is_valid(sys_modem_as_id_e_type wrrc_as_id, rrc_plmn_list_req_type  *plmn_list_req_ptr);
...
===========================================================================*/
boolean rrccsp_plmn_list_req_when_resume_in_prog_is_valid(sys_modem_as_id_e_type wrrc_as_id, rrc_plmn_list_req_type  *plmn_list_req_ptr);
...
===========================================================================*/
rrc_csp_scan_e_type rrccsp_get_curr_scan(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
  ...
  __attribute__((section(".uncompressible.text")))
#endif
rrc_csp_scan_e_type rrccsp_get_curr_scan_no_f3(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
boolean rrccsp_check_if_3g_band_pref_changed(sys_modem_as_id_e_type wrrc_as_id, sys_band_mask_type nas_band_mask);
...
===========================================================================*/
boolean rrccsp_handle_plmn_list_req_in_bgnd(sys_modem_as_id_e_type wrrc_as_id, rrc_plmn_list_req_type  *plmn_list_req_ptr);
...
===========================================================================*/
void rrccsp_handle_passive_bplmn_suspend_req(sys_modem_as_id_e_type wrrc_as_id,  rrc_cmd_type *cmd_ptr );
...
===========================================================================*/
void rrc_csp_send_bplmn_suspend_cnf(sys_modem_as_id_e_type wrrc_as_id, rrc_proc_e_type proc, uecomdef_status_e_type status);
...
===========================================================================*/
void rrccsp_check_and_start_plmn_list_full_freq_scan_timer(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
rrc_csp_acq_e_type rrccsp_get_curr_type_acq(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
boolean rrccsp_is_gtow_bplmn_srch_in_progress(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
void rrccsp_update_acq_type_for_em_call_ifreq_redirection(sys_modem_as_id_e_type wrrc_as_id, rrc_csp_acq_e_type type_of_acq);
...
SIDE EFFECTS      None
===========================================================================*/
void rrc_csp_init_addl_chnl_scan_data(sys_modem_as_id_e_type wrrc_as_id);
...
boolean rrccsp_get_uarfcn(sys_modem_as_id_e_type wrrc_as_id, rrc_freq_type barred_uarfcn, rrc_csp_band_class_type barred_band, 
...
boolean rrccsp_get_uarfcn_ul(sys_modem_as_id_e_type wrrc_as_id, rrc_freq_type barred_uarfcn, rrc_csp_band_class_type barred_band, 
                          rrc_csp_band_class_type current_band, rrc_freq_type *uarfcn);
/*===========================================================================
...
SIDE EFFECTS      None
===========================================================================*/
void rrc_csp_get_addl_channel_band(sys_modem_as_id_e_type wrrc_as_id, rrc_freq_type *ret_freq);
...
===========================================================================*/
void 
rrccsp_prioritize_wcdma_bands_for_full_scan(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
boolean 
rrccsp_is_band_already_added_to_prioritized_list(sys_modem_as_id_e_type wrrc_as_id,
...
rrc_csp_get_list_db_match_acq_entry(sys_modem_as_id_e_type wrrc_as_id, rrc_csp_freq_scan_e_type *ret_type_freq_scan );
...
===========================================================================*/
boolean rrc_check_can_preemption_be_handled(sys_modem_as_id_e_type wrrc_as_id);
...
boolean rrc_check_preemption_can_be_handled_in_fach_dch(sys_modem_as_id_e_type wrrc_as_id);
...
boolean rrc_check_can_rrc_handle_preemption(sys_modem_as_id_e_type wrrc_as_id, rrccsp_preemption_cause_e_type cause_preemption );
...
boolean rrc_check_can_l1_handle_preemption_in_fach_dch(sys_modem_as_id_e_type wrrc_as_id);
...
rrc_csp_substate_e_type rrc_csp_handle_preemption_in_fach_dch(sys_modem_as_id_e_type wrrc_as_id, rrc_cmd_type *);
/*===========================================================================
...
rrc_csp_substate_e_type rrc_csp_initiate_CU_for_immediate_preempt_in_fach_dch(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
rrc_csp_substate_e_type rrc_handle_preemtpion_ind(sys_modem_as_id_e_type wrrc_as_id,   rrc_cmd_type * );
...
===========================================================================*/
rrc_csp_substate_e_type rrc_csp_handle_no_resource_from_l1(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
rrc_csp_substate_e_type rrc_csp_wait_for_wrm_lock(sys_modem_as_id_e_type wrrc_as_id, rrc_csp_freq_scan_e_type *type_freq_scan);
...
===========================================================================*/
rrc_csp_substate_e_type rrc_csp_continue_wait_for_wrm_lock(sys_modem_as_id_e_type wrrc_as_id, rrc_csp_freq_scan_e_type *type_freq_scan);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrc_send_no_resource_available_from_l1_to_csp(sys_modem_as_id_e_type wrrc_as_id,l1_setup_cnf_status_enum_type status);
...
===========================================================================*/
rrc_csp_substate_e_type rrc_csp_rel_lock_and_cont_cell_sel_on_prev_preempted(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
boolean rrc_csp_waiting_to_release_lock(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
boolean rrc_csp_is_preemption_handled(sys_modem_as_id_e_type wrrc_as_id);
...
FUNCTION        rrc_csp_acq_lock_for_wtog_bplmn

DESCRIPTION     This function will acquire lock for WTOG BPLMN for suspending WL1
...
===========================================================================*/
rrc_out_of_lock_wait_e_type rrc_csp_acq_lock_for_wtog_bplmn
(
sys_modem_as_id_e_type wrrc_as_id
);
...
FUNCTION rrcccm_send_rrc_initiate_cell_selection_ind

DESCRIPTION
  This function builds RRC_INITIATE_CELL_SELECTION_IND command to post to
  CSP procedure with the cause OOS
...
SIDE EFFECTS
  None
===========================================================================*/
void  rrccsp_ccm_send_rrc_initiate_cell_selection_ind
(
  sys_modem_as_id_e_type wrrc_as_id, 
...
===========================================================================*/
boolean rrccsp_check_wtog_abort_in_progress(sys_modem_as_id_e_type wrrc_as_id,
...
boolean rrccsp_check_cgi_scan(sys_modem_as_id_e_type wrrc_as_id);
...
void rrccsp_populate_req_bands_for_lock_during_cell_sel(sys_modem_as_id_e_type wrrc_as_id, rrc_csp_freq_scan_e_type type_freq_scan, wrm_freq_input_type *lock_req_bands);
#endif
/*===========================================================================
...
===========================================================================*/
boolean rrc_is_s_criteria_met(sys_modem_as_id_e_type wrrc_as_id, rrc_plmn_identity_type plmn_id, rrc_int_cell_id_type cell_id);
...
===========================================================================*/
void rrccsp_reset_flag_for_excluding_pre_redir_info(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
boolean rrccsp_is_preredirection_info_to_be_excluded(sys_modem_as_id_e_type wrrc_as_id);
#endif
...
===========================================================================*/
boolean rrc_check_serv_cell_s_crit(sys_modem_as_id_e_type wrrc_as_id, rrc_SysInfoType3 * sib3_ptr);
...
SIDE EFFECTS
                  None
===========================================================================*/
boolean rrc_get_rr_inter_rat_bplmn_prtl_result(sys_modem_as_id_e_type wrrc_as_id, sys_detailed_plmn_list_s_type *available_plmn_list);
...
rrc_csp_pending_e_type rrc_csp_return_pending_type(sys_modem_as_id_e_type wrrc_as_id);
...
void rrc_csp_cleanup_needed_bplmn_data(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS
                  None
===========================================================================*/
boolean rrc_csp_is_suitable_srch_ended(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS
  None
===========================================================================*/
boolean rrccsp_ignore_cu_due_to_resel_cell_change_in_progress(sys_modem_as_id_e_type wrrc_as_id);
#endif
#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
/*==========================================================================
FUNCTION rrccsp_ignore_low_thruput_due_to_resel_in_progress

DESCRIPTION
  This function is called to know if reselection is in progress to ignore LT
...
SIDE EFFECTS
  None
===========================================================================*/
boolean rrccsp_ignore_low_thruput_due_to_resel_in_progress(sys_modem_as_id_e_type wrrc_as_id);
#endif 
#ifdef FEATURE_LTE_TO_WCDMA
/*===========================================================================
...
#ifdef FEATURE_DUAL_SIM
/*===========================================================================
FUNCTION   is_redir_w2x_active

DESCRIPTION

 Returns whether W2L/W2G redirection is active or not
...
===========================================================================*/
boolean is_redir_w2x_active(sys_modem_as_id_e_type wrrc_as_id);
#endif
#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
/*===========================================================================
FUNCTION   rrccsp_check_is_earfcn_forbidden_for_pseudo_meas

DESCRIPTION

 Returns whether EARFCN belongs to forbidden list for pseudo measurement
...
===========================================================================*/
boolean rrccsp_check_is_earfcn_forbidden_for_pseudo_meas(sys_modem_as_id_e_type wrrc_as_id,lte_earfcn_t earfcn);

/*===========================================================================
FUNCTION   rrccsp_check_is_earfcn_already_in_forbidden_list

DESCRIPTION
 Returns whether EARFCN is already part of forbidden list for pseudo measurement
...
===========================================================================*/
boolean rrccsp_check_is_earfcn_already_in_forbidden_list(sys_modem_as_id_e_type wrrc_as_id,lte_earfcn_t earfcn);
#endif

#ifdef FEATURE_WCDMA_FAST_RETURN_TO_LTE_AFTER_CSFB
/*===========================================================================
FUNCTION   rrccsp_block_frlte_for_curr_camped_plmn

DESCRIPTION
 Returns whether PLMN is present in NV item to block FRLTE
...
===========================================================================*/
boolean rrccsp_block_frlte_for_curr_camped_plmn(sys_modem_as_id_e_type wrrc_as_id);
#endif
