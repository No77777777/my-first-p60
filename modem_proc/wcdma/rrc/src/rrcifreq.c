...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcifreq.c#2 $
...
05/20/16   vs      f3 density reduction changes
...
09/09/15   sas     Made changes to Copy/Process the cells in the same order of intra/Inter freq CELL_INFO_LIST for SIB18 filtering purposes.
03/13/15   sa      Fixed the compilation error for QTF
03/11/15   sas     Made Changes to implement 3Gpp CR3382 related to multiple PLMNs list in SIB18.
...
rrcifreq_ext_data_type rrcifreq_ext_data_sim[MAX_RRC_STACKS];
#define rrcifreq_ext_data rrcifreq_ext_data_sim[WAS_ID_M]

#ifdef FEATURE_WCDMA_INTER_FREQ_RACH_REPORTING
#error code not present
#endif /*FEATURE_WCDMA_INTER_FREQ_RACH_REPORTING*/

/* Boolean to indicate if inter frequency nbr list is empty */
boolean rrcmeas_inter_f_nbr_empty_sim[MAX_RRC_STACKS]= {TRUE} ;
#define rrcmeas_inter_f_nbr_empty rrcmeas_inter_f_nbr_empty_sim[WAS_ID_M]

#else
boolean rrcmeas_inter_f_nbr_empty = TRUE;

rrcifreq_ext_data_type rrcifreq_ext_data;
...
** ----------------------------------------------------------------------- */
#ifdef FEATURE_DUAL_WCDMA 
LOCAL rrcifreq_int_data_type rrcifreq_int_data_sim[MAX_RRC_STACKS];
#define rrcifreq_int_data rrcifreq_int_data_sim[WAS_ID_M]
#else
LOCAL rrcifreq_int_data_type rrcifreq_int_data;
#endif
...
rrcmeas_status_e_type result;
    ...
    (rrc_mcm_ptr->dpch_CompressedModeStatusInfo.tgp_SequenceShortList.n != 0))
  {
    ...
    if (rrcmeas_process_compressed_mode_status(wrrc_as_id,&rrc_mcm_ptr->dpch_CompressedModeStatusInfo,
  ...
  if (RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand,rrc_MeasurementCommand_setup))
  {
    ...
    if (rrcifreq_process_meas_quantity(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.setup->
    ...
    rrcifreq_fill_rpt_quan(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.setup->u.interFrequencyMeasurement->interFreqReportingQuantity,
    ...
    if (rrcifreq_process_reporting_criteria(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.setup->u.interFrequencyMeasurement->
    ...
    if (RRC_MSG_COMMON_BITMASK_IE_PTR(rrc_mcm_ptr, rrc_MeasurementControl_r3_IEs,additionalMeasurementList))
    {
     ...
     /* Call fn which makes check for additional measurements */
      if (rrcmeas_validate_additional_meas(wrrc_as_id,&rrc_mcm_ptr->additionalMeasurementList,
    ...
    result = rrcifreq_fill_inter_freq_info(wrrc_as_id,rrc_mcm_ptr->measurementCommand.u.setup->u.interFrequencyMeasurement,
    ...
    if (rrcmeas_check_mcm_for_setup_modify_additional(wrrc_as_id,
        ...
        &rrc_mcm_ptr->additionalMeasurementList) == FALSE)
    {
      ...
      {
        WCDMA_MEMCPY(&rrcifreq_ext_data.inter_freq_cell_list[count], 
                     sizeof(inter_freq_cell_list_struct_type),
                     &rrcifreq_int_data.inter_freq_cell_list_tmp[count], 
                     sizeof(inter_freq_cell_list_struct_type));
      }
      rrc_free(l1_inter_freq_meas_params);
      return FAILURE;
    ...
    if (rrcifreq_config_inter_freq_db(wrrc_as_id,l1_inter_freq_meas_params, &mcm_rpt_crit, meas_validity) == FALSE)
    {
    ...
    {
      rrcmeas_set_additional_meas_in_db(wrrc_as_id,&primary_meas_type, &rrc_mcm_ptr->additionalMeasurementList,
  ...
  else if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_modify)
  {
        ...
        rrc_MeasurementCommand_modify,measurementType)))
    {
      ...
      if (RRC_MSG_COMMON_BITMASK_IE_PTR(rrc_mcm_ptr, rrc_MeasurementControl_r3_IEs,additionalMeasurementList))
      {
        ...
        /* Call fn which makes check for additional measurements */
        if (rrcmeas_validate_additional_meas(wrrc_as_id,&rrc_mcm_ptr->additionalMeasurementList, transaction_id,
    ...
    else
    {
          ...
          rrc_InterFrequencyMeasurement,interFreqMeasQuantity))
      {
        ...
        if (rrcifreq_process_meas_quantity(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.modify->measurementType.
          u.interFrequencyMeasurement->interFreqMeasQuantity,
          &l1_inter_freq_meas_params->meas_object.u.inter_f.meas_quan) == RRCMEAS_SUCCESS)
        {
          /* Set Meas Quantity to TRUE */
          ...
          rrc_InterFrequencyMeasurement,interFreqReportingQuantity))
      {
        ...
        /* Set Reporting quantity */
        rrcifreq_fill_rpt_quan(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interFrequencyMeasurement->interFreqReportingQuantity,
          &l1_inter_freq_meas_params->meas_object.u.inter_f.rpt_quan);
      ...
      if (rrcifreq_process_reporting_criteria(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interFrequencyMeasurement->
        reportCriteria, &l1_inter_freq_meas_params->meas_object.u.inter_f, rel5_ext_ptr) == FAILURE)
      {
      ...
      /* Fill cell list, cell info */
      result = rrcifreq_fill_inter_freq_info(wrrc_as_id,
        rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interFrequencyMeasurement,
        l1_inter_freq_meas_params);
  ...
  else
  {
    ...
    rrcmeas_delete_addtl_meas(wrrc_as_id,&l1_inter_freq_meas_params->meas_id);

    /* Clear Meas Id from inter_freq_meas_id_list List */
    for (count=0; count < MAX_INTER_FREQ_MEAS; count++)
    {
      if (rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_id == l1_inter_freq_meas_params->meas_id)
      {
        rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_id = INVALID_MEAS_ID;
        rrcifreq_ext_data.inter_freq_meas_id_list[count].rpt_crit = NOT_AVAILABLE;
        rrcifreq_ext_data.inter_freq_meas_id_list[count].mcm_owned = FALSE;
        rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_validity = NOT_PRESENT;

        /* Also initialize additional meas */
        for ( cnt = 0; cnt < MAX_ADDITIONAL_MEAS; cnt++ )
          rrcifreq_ext_data.inter_freq_meas_id_list[count].additional_meas[cnt] = INVALID_MEAS_ID;
  ...
  if((!rrcmeas_ext_data.process_interfreq_interrat_meas_rel7) && (!rrcmeas_ext_data.process_interfreq_uei_meas_rel7))
  {
  ...
  alloc_idx = rrcmeas_get_int_trans_index_for_trans_id(wrrc_as_id,transaction_id);
  l1_inter_freq_meas_params->trans_id = alloc_idx;
  rrcmeas_send_l1_cmd(wrrc_as_id,NULL, l1_inter_freq_meas_params, NULL, L1_MEAS_CTRL_PARMS);
...
)
{
  ...
  while ((ifreq_event_list_ptr->n > idx) && (cnt < L1_MAX_MEAS_EVENT))
  {
    ...
    case L1_INTER_FREQ_EVENT_2A:
      ...
      {
        if (rrcmeas_fill_no_of_reporting_cells(wrrc_as_id,
...
)
{
  ...
  rrc_qsh_log_meas_rpt_params(wrrc_as_id, &mrm_params);
#endif

  if (rrcifreq_validate_meas_report(wrrc_as_id,l1_meas_report_ptr) == FALSE)
  {
    WRRC_MSG0_HIGH("IFREQ: Ignore Meas Report");
  /* Deallocate the cell list if any allocated by L1 */
    rrcifreq_free_memory_allocated_for_reporting(wrrc_as_id,l1_meas_report_ptr);
    return;
  }
  ...
  * should be released by SEND_CHAIN using OSS compiler functions */
  ...
  rrcmeas_fill_meas_id(wrrc_as_id,&msg_ptr->message.u.measurementReport,l1_meas_report_ptr->meas_id, INTER_FREQ);
  ...
  if (l1_meas_report_ptr->rpt_crit == L1_EVENT_TRIGGER_RPT)
  {
    ...
    rrcifreq_set_inter_freq_event_id(wrrc_as_id,&l1_meas_report_ptr->u.inter_f_rpt.event_results.evt_id,
  ...
  rrcmeas_process_additional_measured_results(wrrc_as_id,
    ...
    (l1_meas_report_ptr->rpt_crit == L1_PERIODIC_RPT))
  {
    ...
    {
      if (rrcifreq_ext_data.inter_freq_meas_id_list[cnt].meas_id == l1_meas_report_ptr->meas_id)
      {
        WRRC_MSG1_MED("IFREQ:Delete Meas %d",l1_meas_report_ptr->meas_id);
        rrcifreq_ext_data.inter_freq_meas_id_list[cnt].meas_id = INVALID_MEAS_ID;
        ...
        for ( count = 0; count < MAX_ADDITIONAL_MEAS; count++ )
          rrcifreq_ext_data.inter_freq_meas_id_list[cnt].additional_meas[count] = INVALID_MEAS_ID;
    ...
    rrcmeas_delete_addtl_meas(wrrc_as_id,&l1_meas_report_ptr->meas_id);
  ...
  else
  {
    ...
    if(l1_meas_report_ptr->meas_transfer_mode == ACKNOWLEDGED_MODE)
    {
      ...
      if(rrcmeas_ext_data.rrcmeas_pending_rpt >= 5)
      {
...
)
{
  ...
  case T_rrc_InterFreqReportCriteria_interFreqReportingCriteria:
    ...
    {
      if (rrcifreq_process_event_criteria(wrrc_as_id,ifreq_rpt_criteria_ptr->u.interFreqReportingCriteria,
  ...
  case T_rrc_InterFreqReportCriteria_periodicalReportingCriteria:
    ...
    {
      if (rrcmeas_fill_no_of_reporting_cells(wrrc_as_id,&ifreq_rpt_criteria_ptr->u.
  ...
  case T_rrc_InterFreqReportCriteria_noReporting:
    ...
    {
      if (rrcmeas_fill_no_of_reporting_cells(wrrc_as_id,&ifreq_rpt_criteria_ptr->u.noReporting->reportingCellStatus,
        &l1_rpt_params_ptr->rpt_cell_status) == SUCCESS)
      {
        l1_rpt_params_ptr->rpt_cell_status_incl = TRUE;
...
)
{
  ...
  if (l1_meas_report_ptr->rpt_crit == L1_EVENT_TRIGGER_RPT)
  {
    ...
    else
    {
      ...
      for (cnt = 0; (cnt < L1_MAX_FREQ) && (cnt < l1_meas_report_ptr->u.inter_f_rpt.event_results.num_freq); cnt++)
      {
        if (rrcifreq_validate_freq(wrrc_as_id,&l1_meas_report_ptr->u.inter_f_rpt.event_results.freq[cnt].freq_info) == FALSE)
        {
          return FALSE;
        }
  ...
  if ((l1_meas_report_ptr->u.inter_f_rpt.meas_results_included == TRUE) || (l1_meas_report_ptr->rpt_crit == L1_PERIODIC_RPT))
  {
    if (l1_meas_report_ptr->u.inter_f_rpt.inter_freq_measurements.num_msmt_results != 0)
    {
      ...
      for (cnt = 0; (cnt < L1_MAX_FREQ) && (cnt < l1_meas_report_ptr->u.inter_f_rpt.inter_freq_measurements.num_msmt_results); cnt++)
      {
        ...
        if (rrcifreq_validate_freq(wrrc_as_id,&l1_meas_report_ptr->u.inter_f_rpt.inter_freq_measurements.
          msmt_results[cnt].freq_info) == FALSE)
        {
          WRRC_MSG3_HIGH("IFreq: Invalid DFreq or UFreq, RSSI is %d, DFreq is %d, Num Cells per Freq is %d", l1_meas_report_ptr->u.inter_f_rpt.inter_freq_measurements.
  ...
  switch(removedInterFreqCellList->t)
  {
    case T_rrc_RemovedInterFreqCellList_removeAllInterFreqCells:
         ...
         *num_rmv_cells = L1_MAX_CELL_MEAS;
         rrcifreq_initialize_cell_list(wrrc_as_id);
      break;
    ...
    case T_rrc_RemovedInterFreqCellList_removeSomeInterFreqCells:
         ...
         else
         {
           ...
           do
           {
             ...
             {
               rrcifreq_ext_data.inter_freq_cell_list[inter_freq_cell_id].psc = INVALID_PSC;
               rrcifreq_ext_data.inter_freq_cell_list[inter_freq_cell_id].cell_position = VACANT;
               rrcifreq_ext_data.inter_freq_cell_list[inter_freq_cell_id].dl_freq = INVALID_INTER_FREQ;
...
===========================================================================*/
static void rrcifreq_check_if_multiplefrequency_bands_supported(sys_modem_as_id_e_type wrrc_as_id, rrc_MultipleFrequencyBandIndicatorListFDD *inter_multifreq_ptr,
    ...
    if(inter_multifreq_ptr->elem[i].t == T_rrc_FrequencyBandsIndicatorFDD_frequencyBandsIndicator1)
    {
      ...
      if((rrcsibproc_ext_data.Fdd1_internal_band[FDD1] != RRC_CSP_BAND_MAX_INDEX) &&
         (rrccsp_is_supported_band(wrrc_as_id,rrcnv_data.rrc_nv_band_priority_config.band_priority_list[rrcsibproc_ext_data.Fdd1_internal_band[FDD1]])))
          {
            *band_supported = TRUE;
        *band_class = rrcnv_data.rrc_nv_band_priority_config.band_priority_list[rrcsibproc_ext_data.Fdd1_internal_band[FDD1]];
      }
    }
    else if(inter_multifreq_ptr->elem[i].t == T_rrc_FrequencyBandsIndicatorFDD_frequencyBandsIndicator2)
    {
      ...
      if((rrcsibproc_ext_data.Fdd2_internal_band[FDD2] != RRC_CSP_BAND_MAX_INDEX) &&
         (rrccsp_is_supported_band(wrrc_as_id,rrcnv_data.rrc_nv_band_priority_config.band_priority_list[rrcsibproc_ext_data.Fdd2_internal_band[FDD2]])))
          {
            *band_supported = TRUE;
        *band_class=rrcnv_data.rrc_nv_band_priority_config.band_priority_list[rrcsibproc_ext_data.Fdd2_internal_band[FDD2]];
      }
    }
    i++;
  }
...
)
{
  ...
  /*If we find a supported band in the MFBI list,we convert the UARFCN of previous band into the UARFCN
   of the current band*/
  if(rrccsp_get_uarfcn(wrrc_as_id,mode_specific_info.u.fdd->uarfcn_DL, rrc_get_frequency_band(wrrc_as_id, mode_specific_info
  ...
  if (mode_specific_info.u.fdd->m.uarfcn_ULPresent)
  {
    ...
    if(rrccsp_get_uarfcn_ul(wrrc_as_id,mode_specific_info.u.fdd->uarfcn_UL, rrc_get_frequency_band(wrrc_as_id, mode_specific_info
...
/* Initialize all variables */
  ...
  * one at a time */
  ...
  for ( sib11_a_cell=0 ; sib11_a_cell<sib11_added_cells; sib11_a_cell++ )
  {
    ...
    for ( sib12_r_cell=0; sib12_r_cell < (*num_rmv_cells); sib12_r_cell++ )
    {
      ...
      if(removedInterFreqCellList->t == T_rrc_RemovedInterFreqCellList_removeSomeInterFreqCells)
      {
        ...
        {
          if(rrcifreq_int_data.tmp_inter_freq_cell_list[removedInterFreqCellList->u.removeSomeInterFreqCells->elem[idx]].psc !=
  ...
  sib12_added_cells++ )
  {
    ...
    {
      if(rrcifreq_ext_data.inter_freq_cell_list[cell_id].psc == sib12_cell_list_ptr->add_cell[sib12_added_cells].psc)
      {
        break;
      }
    }
    if((cell_id < L1_MAX_CELL_MEAS) && (rrcifreq_int_data.tmp_inter_freq_cell_list[cell_id].psc != INVALID_PSC))
    {
      for(count = 0; count < new_cell_list_from_sib11; count++)
      {
        if(rrcifreq_int_data.tmp_inter_freq_cell_list[cell_id].psc
  ...
  sib12_cell_list_ptr->num_cell_add = 0;

  MSG_LOW("There are %d cells in SIB12 cell list", new_cell_list, 0,0);

  /* Now copy the structure in SIB12 */
  for ( sib12_count_cells =0; sib12_count_cells < L1_MAX_CELL_MEAS;  sib12_count_cells++ )
  {
     /*Copy the cells in the same order of inter freq CELL_INFO_LIST for SIB18 filtering purposes later.
       InterFreq cell list should have been updated properly before calling this function.*/
    if(rrcifreq_ext_data.inter_freq_cell_list[sib12_count_cells].cell_position == OCCUPIED)
    {
      for(cell_id = 0; cell_id < new_cell_list; cell_id++)/*Parse through new cell list and get the index where there is a match.*/
      {
         if((rrcifreq_ext_data.inter_freq_cell_list[sib12_count_cells].psc == new_cell_list_ptr->add_cell[cell_id].psc) &&
               (rrcifreq_ext_data.inter_freq_cell_list[sib12_count_cells].dl_freq== new_cell_list_ptr->add_cell[cell_id].freq_info.uarfcn_dl))
         {
           sib12_cell_list_ptr->add_cell[sib12_cell_list_ptr->num_cell_add].psc =
             new_cell_list_ptr->add_cell[cell_id].psc;

           sib12_cell_list_ptr->add_cell[sib12_cell_list_ptr->num_cell_add].measure_in_idle_pch_fach =
             new_cell_list_ptr->add_cell[cell_id].measure_in_idle_pch_fach;

           sib12_cell_list_ptr->add_cell[sib12_cell_list_ptr->num_cell_add].cell_info =
             new_cell_list_ptr->add_cell[cell_id].cell_info;

           sib12_cell_list_ptr->add_cell[sib12_cell_list_ptr->num_cell_add].freq_info =
             new_cell_list_ptr->add_cell[cell_id].freq_info;
           sib12_cell_list_ptr->num_cell_add++;
           break;
         }
      }
    }
  ...
  if(sib12_cell_list_ptr->num_cell_add != new_cell_list)
  {
     WRRC_MSG2_ERROR("Something wrong: sib12_cell_list_ptr->num_cell_add = %d, new_cell_list = %d",sib12_cell_list_ptr->num_cell_add, new_cell_list);
  }
...
#error code not present
    ...
    (wrrc_as_id,rrc_SIB18)) != NULL)
  {
      /*
       ...
       2>otherwise,
                 the UE shall use, in place of any missing list, the corresponding one in the IE "Idle mode PLMN identities";
      */
    if((rrc_sibdb_is_sib11bis_present_in_srv_cell(wrrc_as_id)) &&
    ...
    else if((rrc_sibdb_is_sib11bis_present_in_srv_cell(wrrc_as_id)) &&
            (RRC_MSG_COMMON_BITMASK_IE_PTR(sib18_ptr, rrc_SysInfoType18, v6b0NonCriticalExtensions)) &&
            (RRC_MSG_COMMON_BITMASK_IE_TYPE2(sib18_ptr->v6b0NonCriticalExtensions.sysInfoType18_v6b0ext,
  ...
  if(rrc_ccm_get_serv_plmn_cellid(wrrc_as_id,&tmp_cell_id, &selected_plmn_identity) == RRC_CELL_LOSS_NO_NEW_CELL)
  {
    sib18_present = FALSE;
  }
  else
  ...
  if(rrccsp_is_ue_in_true_limited_service(wrrc_as_id) == TRUE)
  {
    sib18_present = FALSE;
  ...
  for(count = 0; count < L1_MAX_CELL_MEAS; count++)
  {
    rrcifreq_int_data.tmp_inter_freq_cell_list[count] = rrcifreq_ext_data.inter_freq_cell_list[count];
  }
       ...
       T_rrc_MeasurementControlSysInfo_use_of_HCS_hcs_used )
  {
      ...
      T_rrc_MeasurementControlSysInfo_cellSelectQualityMeasure_1_cpich_RSCP )
    {
        ...
        cellSelectQualityMeasure.u.cpich_RSCP->m.interFreqMeasurementSysInfoPresent))
      {
        ...
        if(l1_inter_freq_sib11_ptr->num_cell_add != 0)
        {
          ...
          rrcifreq_copy_sib11_inter_freq_cell_list_to_sib12(wrrc_as_id,l1_inter_freq_sib11_ptr, l1_inter_freq_sib12_ptr);
        ...
        rrc_InterFreqCellInfoSI_List_HCS_RSCP,removedInterFreqCellList)))
      {
        rrcifreq_process_remove_inter_freq_cells
          (wrrc_as_id,&hcs_inter_freq_meas_rscp_ptr->interFreqCellInfoSI_List.removedInterFreqCellList, &num_rmv_cells);
      }
...

      {
        ...
        /* Pointer to next cell is Non-NULL and cell count
            ...
            rrc_NewInterFreqCellSI_HCS_RSCP,frequencyInfo))
          {
            ...
            if (validate_frequency_info(wrrc_as_id, &hcs_rscp_cell_list_ptr->elem[idx].frequencyInfo) == FAILURE)
            {
          ...
          * else use the info from the previous cell */
          ...
          if (rrcmeas_fill_hcs_rscp_cell_info(wrrc_as_id,&hcs_rscp_cell_list_ptr->elem[idx].
          ...
          if ( rrcifreq_update_inter_freq_cell_list(wrrc_as_id,
            &cell_position,
            &l1_inter_freq_sib12_ptr->add_cell[cell_count].
        ...
        rrcifreq_build_composite_cell_list(wrrc_as_id,&hcs_inter_freq_meas_rscp_ptr->
        ...
        else
        {
          if(l1_inter_freq_sib11_ptr->num_cell_add != 0)
          {
            ...
            rrcifreq_copy_sib11_inter_freq_cell_list_to_sib12
              (wrrc_as_id,l1_inter_freq_sib11_ptr, l1_inter_freq_sib12_ptr);
          }
        }
      }
      for(cell_count = 0; cell_count < l1_inter_freq_sib12_ptr->num_cell_add; cell_count++)
      {
        if(sib18_present)
        {
          rrcmeas_process_sib18_filtering(wrrc_as_id,
    ...
    else /* rrc_hcs_used_cellSelectQualityMeasure_cpich_Ec_N0 */
    {
      ...
      {
        rrcifreq_process_remove_inter_freq_cells
          (wrrc_as_id,&hcs_inter_freq_meas_ecno_ptr->interFreqCellInfoSI_List.removedInterFreqCellList, &num_rmv_cells);
      }
...

      {
        ...
        /* Pointer to next cell is Non-NULL and cell count
          ...
          if (RRC_MSG_LIST_BITMASK_IE(hcs_ecno_cell_list_ptr,rrc_NewInterFreqCellSI_HCS_ECN0,frequencyInfo))
          {
            ...
            if (validate_frequency_info(wrrc_as_id, &hcs_ecno_cell_list_ptr->elem[idx].frequencyInfo) == FAILURE)
            {
              WRRC_MSG1_HIGH("Ignored cell with dl freq %d",
          ...
          * else use the info from the previous cell */
          ...
          /* First store cell info so that psc of this cell can be extracted */
          if (rrcmeas_fill_hcs_ecn0_cell_info(wrrc_as_id,&hcs_ecno_cell_list_ptr->elem[idx].
            cellInfo, &l1_inter_freq_sib12_ptr->add_cell[cell_count].
            cell_info, ((rel5_ext_ptr != NULL) ? ((rel5_ext_ptr->n > idx) ? (&rel5_ext_ptr->elem[idx]) : NULL) : NULL)) == FAILURE)
          {
            WRRC_MSG0_HIGH("IFreq:Ignore InterFreq Cell");
        ...
        rrcifreq_build_composite_cell_list(wrrc_as_id,&hcs_inter_freq_meas_ecno_ptr->
          interFreqCellInfoSI_List.removedInterFreqCellList,
          l1_inter_freq_sib11_ptr, l1_inter_freq_sib12_ptr, &num_rmv_cells);
  ...
  { /* HCS is NOT in use */
      ...
      T_rrc_MeasurementControlSysInfo_cellSelectQualityMeasure_1_cpich_RSCP )
    {
      ...
      {
        rrcifreq_process_remove_inter_freq_cells
          (wrrc_as_id,&inter_freq_meas_rscp_ptr->interFreqCellInfoSI_List.removedInterFreqCellList, &num_rmv_cells);
      }
...

      {
        ...
        /* Pointer to next cell is Non-NULL and cell count
          ...
          if (RRC_MSG_LIST_BITMASK_IE(rscp_cell_list_ptr,rrc_NewInterFreqCellSI_RSCP,frequencyInfo))
          {
            ...
            if (validate_frequency_info(wrrc_as_id, &rscp_cell_list_ptr->elem[idx].frequencyInfo) == FAILURE)
            {
#ifdef FEATURE_WCDMA_MULTI_FREQ_BAND_INDICATOR 
              
              /*As the frequency validation failed,we look into the MFBI list present for this UARFCN*/
              if( (rrcnv_data.rrc_mfbi_support_nv) && (sib12_ptr != NULL) && 
                             ...
                             .multipleFrequencyInfoListFDD.elem[mfbi_index].m.multipleFrequencyBandIndicatorListFDDPresent))
              {
                ...
                rrcifreq_check_if_multiplefrequency_bands_supported(wrrc_as_id, inter_freq_multifreq_ptr,
                                                                    &band_found,
                                                                    &tmp_band_class);
            ...
            if(band_found)
            {
              ...
              /*If we find a supported band in the MFBI list,we convert the UARFCN of previous band into the UARFCN
                of the current band*/
              if(rrcifreq_populate_sibs_if_band_found(wrrc_as_id,tmp_band_class,
            ...
            else
            {
              ...
              if (rscp_cell_list_ptr->elem[idx].
                frequencyInfo.modeSpecificInfo.u.fdd->uarfcn_DL > FDD_ARFCN)
              {
                WRRC_MSG3_HIGH("IFreq:Invalid DL ARFCN %d. Ignore Inter Freq Cell",rscp_cell_list_ptr->elem[idx].
                  frequencyInfo.modeSpecificInfo.u.fdd->uarfcn_DL,0,0);
                idx++;
                continue;
              }
          ...
          /* First store cell info so that psc of this cell can be extracted */
          if (rrcmeas_fill_rscp_cell_info(wrrc_as_id,&rscp_cell_list_ptr->elem[idx].
            cellInfo, &l1_inter_freq_sib12_ptr->add_cell[cell_count].
            cell_info, ((rel5_ext_ptr != NULL) ? ((rel5_ext_ptr->n > idx) ? (&rel5_ext_ptr->elem[idx]) : NULL) : NULL)) == FAILURE)
          {
        ...
        rrcifreq_build_composite_cell_list(wrrc_as_id,&inter_freq_meas_rscp_ptr->
          interFreqCellInfoSI_List.removedInterFreqCellList,
          l1_inter_freq_sib11_ptr, l1_inter_freq_sib12_ptr, &num_rmv_cells);
      } /* Inter freq cell info list is present */
      else
    ...
    else /* rrc_hcs_not_used_cellSelectQualityMeasure_cpich_Ec_N0 */
    {
      ...
      {
        rrcifreq_process_remove_inter_freq_cells
          (wrrc_as_id,&inter_freq_meas_ecno_ptr->interFreqCellInfoSI_List.removedInterFreqCellList, &num_rmv_cells);
      }
...

      {
        ...
        /* Pointer to next cell is Non-NULL and cell count
          ...
          if (RRC_MSG_LIST_BITMASK_IE(ecno_cell_list_ptr,rrc_NewInterFreqCellSI_ECN0,frequencyInfo))
          {
            ...
            if (validate_frequency_info(wrrc_as_id, &ecno_cell_list_ptr->elem[idx].frequencyInfo) == FAILURE)
            {
#ifdef FEATURE_WCDMA_MULTI_FREQ_BAND_INDICATOR 
 /*As the frequency validation failed,we look into the MFBI list present for this UARFCN*/
              if((rrcnv_data.rrc_mfbi_support_nv) && (sib12_ptr != NULL) && 
          ...
          /* First store cell info so that psc of this cell can be extracted */
          if (rrcmeas_fill_ecn0_cell_info(wrrc_as_id,&ecno_cell_list_ptr->elem[idx].
            cellInfo, &l1_inter_freq_sib12_ptr->add_cell[cell_count].
            cell_info, ((rel5_ext_ptr != NULL) ? ((rel5_ext_ptr->n > idx) ? (&rel5_ext_ptr->elem[idx]) : NULL) : NULL)) == FAILURE)
          {
            MSG_LOW("IFreq:Ignore InterFreq Cell",0,0,0);
        ...
        rrcifreq_build_composite_cell_list(wrrc_as_id,&inter_freq_meas_ecno_ptr->
          interFreqCellInfoSI_List.removedInterFreqCellList,
          l1_inter_freq_sib11_ptr, l1_inter_freq_sib12_ptr, &num_rmv_cells);
...
)
{
  ...
  if((sib18_ptr = (rrc_SysInfoType18*)  rrc_sibdb_return_sib_for_srv_cell
    (wrrc_as_id,rrc_SIB18)) != NULL)
  {
    if(((rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_CELL_DCH)
      || (rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_PCH)
      || (rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_CELL_FACH)))
    {
      /*
       1>if System Information Block type 11bis is scheduled on BCH:
       2>the PLMN lists to be considered are the ones included, when present, in the IE "Connected mode PLMN identities for SIB 11bis"; 
  ...
  else
  {
         ...
         T_rrc_MeasurementControlSysInfo_use_of_HCS_hcs_used )
    {
      ...
      if ((rrcifreq_fill_hcs_info(wrrc_as_id,l1_inter_freq_sib_ptr, 
    ...
    { /* HCS is NOT in use */
        ...
        T_rrc_MeasurementControlSysInfo_cellSelectQualityMeasure_1_cpich_RSCP )
      {
          ...
          cellSelectQualityMeasure.u.cpich_RSCP->m.interFreqMeasurementSysInfoPresent))
        {
          ...
          */
          if(rrcifreq_read_sib11bis(wrrc_as_id,l1_inter_freq_sib_ptr,sib18_present,tmp_rrc_PLMNsOfInterFreqCellsList_ptr, tmp_rrc_MultiplePLMNsOfInterFreqCellsList_ptr) == FAILURE)
          {
...
#ifdef FEATURE_WCDMA_MULTI_FREQ_BAND_INDICATOR 
                /*As the frequency validation failed,we look into the MFBI list present for this UARFCN*/
                if((rrcnv_data.rrc_mfbi_support_nv) && (sib11_ptr != NULL) && 
...
)
{
  ...
  * already a cell, then this variable stores the psc */
  ...
  if (*position_no_ptr == AVAILABLE_CELL_POSITION)
  {
    for (count = 0; count < L1_MAX_CELL_MEAS; count++)
    {
      if (rrcifreq_ext_data.inter_freq_cell_list[count].cell_position == VACANT)
      {
        /* Store this cell in the first available vacant position */
        rrcifreq_ext_data.inter_freq_cell_list[count].cell_position = OCCUPIED;
        rrcifreq_ext_data.inter_freq_cell_list[count].psc = *psc_ptr;

        rrcifreq_ext_data.inter_freq_cell_list[count].dl_freq = *dl_freq_ptr;
...
)
{
  ...
  {
    rrcifreq_ext_data.inter_freq_cell_list[cnt].psc = INVALID_PSC;
    rrcifreq_ext_data.inter_freq_cell_list[cnt].cell_position = VACANT;
    rrcifreq_ext_data.inter_freq_cell_list[cnt].dl_freq = INVALID_INTER_FREQ;
    rrcifreq_int_data.inter_freq_cell_list_tmp[cnt].psc = INVALID_PSC;
    rrcifreq_int_data.inter_freq_cell_list_tmp[cnt].cell_position = VACANT;
    rrcifreq_int_data.inter_freq_cell_list_tmp[cnt].dl_freq = INVALID_INTER_FREQ;
  }
#ifdef FEATURE_WCDMA_INTER_FREQ_RACH_REPORTING
  #error code not present
#endif

  /* This function gets called whenever inter frequency cell list is empty */
  rrcmeas_inter_f_nbr_empty = TRUE;
}
...
struct rrc_CellsForInterFreqMeasList *cellsForInterFreqMeasList_ptr = NULL;
  ...
  {
    WCDMA_MEMCPY(&rrcifreq_int_data.inter_freq_cell_list_tmp[cell_count], 
                 sizeof(inter_freq_cell_list_struct_type),
                 &rrcifreq_ext_data.inter_freq_cell_list[cell_count], 
                 sizeof(inter_freq_cell_list_struct_type));
    ...
    rrc_InterFreqCellInfoList,removedInterFreqCellList))
  {
    ...
    case T_rrc_RemovedInterFreqCellList_removeAllInterFreqCells:
      ...
      for (cell_count = 0; cell_count < L1_MAX_CELL_MEAS; cell_count++)
      {
        rrcifreq_ext_data.inter_freq_cell_list[cell_count].cell_position = VACANT;
        rrcifreq_ext_data.inter_freq_cell_list[cell_count].psc = INVALID_PSC;
        rrcifreq_ext_data.inter_freq_cell_list[cell_count].psc = INVALID_INTER_FREQ;
      }
    ...
    case T_rrc_RemovedInterFreqCellList_removeSomeInterFreqCells:
      ...
      do
      {
        ...
        if ( rrcifreq_find_psc_freq(wrrc_as_id,
        ...
        else
        {
          ...
          rrcifreq_remove_cellid(wrrc_as_id,inter_freq_cell_id);
    ...
    rrc_InterFreqCellInfoList,newInterFreqCellList))
  {
    ...
    /* Pointer to next cell is Non-NULL and cell count
      * hasn't exceeded the max. L1 can handle */
      ...
      /* Fill in Cell Info */
      if (rrcmeas_fill_mcm_cell_info(wrrc_as_id,
      ...
      if (RRC_MSG_LIST_BITMASK_IE(local_network_new_cell_list_ptr,rrc_NewInterFreqCell,frequencyInfo))
      {
        ...
        if (validate_frequency_info(wrrc_as_id, &local_network_new_cell_list_ptr->elem[idx].frequencyInfo) == FAILURE)
        {
      ...
      if ( rrcifreq_update_inter_freq_cell_list_mcm(wrrc_as_id,
        &cell_position,
        &l1_inter_freq_meas_ptr->cell_list.add_cell[cell_count].
       ...
       rrc_InterFreqCellInfoList,cellsForInterFreqMeasList))
  {
    ...
    while (cellsForInterFreqMeasList_ptr->n > idx)
    {
      ...
      if ( rrcifreq_find_psc_freq(wrrc_as_id,cell_id, &psc_to_measure, &freq_to_measure) == PSC_ABSENT)
      {
    ...
    if(!valid_cell_id_found)
    {
      /*None of the cells configured are valid*/
        ...
        WCDMA_MEMCPY(rrcifreq_ext_data.inter_freq_cell_list, 
                     sizeof(inter_freq_cell_list_struct_type) * L1_MAX_CELL_MEAS,
                     rrcifreq_int_data.inter_freq_cell_list_tmp,
                     sizeof(inter_freq_cell_list_struct_type) * L1_MAX_CELL_MEAS);
...
)
{
  ...
  if (rrcifreq_ext_data.inter_freq_cell_list[inter_freq_cell_id].cell_position == OCCUPIED)
  {
    /* Found the good psc */
    *psc_ptr = rrcifreq_ext_data.inter_freq_cell_list[inter_freq_cell_id].psc;

    *dl_freq_ptr = rrcifreq_ext_data.inter_freq_cell_list[inter_freq_cell_id].dl_freq;
  ...
  if (*position_no_ptr == AVAILABLE_CELL_POSITION)
  {
    for (count = 0; count < MAX_NO_OF_CELLS; count++)
    {
      if (rrcifreq_ext_data.inter_freq_cell_list[count].cell_position == VACANT)
      {
        ...
        WRRC_MSG3_HIGH("IFreq:Add Cell at pos=%d,psc=%d,freq=%d",count,*psc_ptr,*dl_freq_ptr);
  ...
  *  If yes, then this cell has to be removed and L1
    *  has to be informed about it. So store its psc  */
      ...
      (rrcifreq_ext_data.inter_freq_cell_list[count].cell_position == OCCUPIED) )
    {
      ...
      WRRC_MSG3_HIGH("IFreq:Add:PSC %d at pos=%d removed[Cascading] by %d",
        rrcifreq_ext_data.inter_freq_cell_list[count].psc,count,*psc_ptr);


      /* Store the removed psc */
      *psc_to_remove_ptr = rrcifreq_ext_data.inter_freq_cell_list[count].psc;

      *dl_freq_to_remove_ptr = rrcifreq_ext_data.inter_freq_cell_list[count].dl_freq;
      ...
      (rrcifreq_ext_data.inter_freq_cell_list[count].cell_position == VACANT) )
    {
...
if((((rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_INITIAL)
   || (rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_IDLE)
   ||(rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_PCH))
   &&(rrcnv_data.rrc_disable_meas_nv & RRC_DISABLE_WTOW_IDLE_MEAS))

   ||((rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_CELL_FACH) 
       &&(rrcnv_data.rrc_disable_meas_nv & RRC_DISABLE_WTOW_FACH_MEAS))
  
   || ((rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_CELL_DCH)
       && (rrcnv_data.rrc_disable_meas_nv & RRC_DISABLE_WTOW_CM_MEAS))
  )
{
   MSG_LOW("Skip IRAT/IFREQ meas  Disable IRAT/IFREQ NV %d  in meas state  %d",rrcnv_data.rrc_disable_meas_nv,rrcmeas_ext_data.rrcmeas_current_substate,0);
   rrcifreq_initialize_cell_list(wrrc_as_id);
   return;
}
  /* Now allocate memory */
  rrc_meas_out_cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id);
  ...
  rrc_meas_cmd_l1_ptr->meas.meas_ptr =
       (l1_meas_parm_type *)rrc_malloc_interlayer_cmd(wrrc_as_id, sizeof(l1_meas_parm_type));
  ...
  if ( *meas_choice_ptr == L1_INTER_FREQ_SIB_PARMS)
  {
    ...
    *l1_inter_freq_sib_ptr;
    ...
    {
      rrcmeas_inter_f_nbr_empty = FALSE;
    }
    ...
    rrc_log_meas_info(wrrc_as_id,NULL,NULL,NULL,*meas_choice_ptr,l1_inter_freq_sib_ptr);
  }
  ...
  rrc_put_int_cmd(wrrc_as_id, rrc_meas_out_cmd_ptr );
}
...
)
{
  ...
  for ( count = 0; count < MAX_INTER_FREQ_MEAS; count++ )
  {
    if (rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_id == l1_mcm_ptr->meas_id)
    {
      WRRC_MSG4_HIGH( 
            "IFreq:Meas Id = %d found in RRC db, Before commiting Id %d, Val %d, Owner %d",
            l1_mcm_ptr->meas_id,rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_id,
            rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_validity,
            rrcifreq_ext_data.inter_freq_meas_id_list[count].mcm_owned);

      /* If a setup, always overwrite validity */
      if (l1_mcm_ptr->meas_cmd == L1_MEAS_SETUP)
      {
        /* Set Validity */
        rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_validity = m_validity;
      }
      ...
      {
        WRRC_MSG1_HIGH("IFREQ:Retain Current Validity %d", rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_validity);
      }
      ...
      rrcifreq_ext_data.inter_freq_meas_id_list[count].mcm_owned = TRUE;

      /* Set Report Criteria */
      rrcifreq_ext_data.inter_freq_meas_id_list[count].rpt_crit = *rpt_crit_ptr;

      WRRC_MSG4_HIGH( 
            "IFreq:Meas Crit %d saved in RRC, After commiting Id %d, Val %d, Owner %d",
            rrcifreq_ext_data.inter_freq_meas_id_list[count].rpt_crit,rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_id,
            rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_validity,
            rrcifreq_ext_data.inter_freq_meas_id_list[count].mcm_owned);
  ...
  for ( count = 0; count < MAX_INTER_FREQ_MEAS; count++ )
  {
    ...
    if (rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_id == INVALID_MEAS_ID)
    {
      WRRC_MSG4_HIGH( 
      ...
      rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_id = l1_mcm_ptr->meas_id;
...

      {
        ...
        /* Pointer to next cell is Non-NULL and cell count
          ...
          * else use the info from the previous cell */
          ...
          /* First store cell info so that psc of this cell can be extracted */
          if (rrcmeas_fill_hcs_rscp_cell_info(wrrc_as_id,&rscp_cell_list_ptr->elem[idx].
            cellInfo, &l1_inter_freq_sib_ptr->add_cell[cell_count].
            cell_info, ((rel5_ext_ptr != NULL) ? ((rel5_ext_ptr->n > idx) ? (&rel5_ext_ptr->elem[idx]) : NULL) : NULL)) == FAILURE)
          {
            WRRC_MSG0_HIGH("IFreq:Ignore InterFreq Cell");
...

      {
        ...
        /* Pointer to next cell is Non-NULL and cell count
          ...
          * else use the info from the previous cell */
          ...
          /* First store cell info so that psc of this cell can be extracted */
          if (rrcmeas_fill_hcs_ecn0_cell_info(wrrc_as_id,&ecno_cell_list_ptr->elem[idx].
            cellInfo, &l1_inter_freq_sib_ptr->add_cell[cell_count].
            cell_info, ((rel5_ext_ptr != NULL) ? ((rel5_ext_ptr->n > idx) ? (&rel5_ext_ptr->elem[idx]) : NULL) : NULL)) == FAILURE)
          {
            WRRC_MSG0_HIGH("IFreq:Ignore InterFreq Cell");
...
)
{
  ...
  if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r4_setup)
  {
    ...
    if (rrcifreq_process_reporting_criteria_r5(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.setup->u.interFrequencyMeasurement->
    ...
    result = rrcifreq_fill_inter_freq_info_r5(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.setup->u.
  ...
  else if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r4_modify)
  {
    ...
    else
    {
      ...
      if (rrcifreq_process_reporting_criteria_r5(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interFrequencyMeasurement->
        reportCriteria, &l1_inter_freq_meas_params->meas_object.u.inter_f, mcm_590_ext_ptr) == FAILURE)
      {
      ...
      /* Fill cell list, cell info */
      result = rrcifreq_fill_inter_freq_info_r5(wrrc_as_id,
        &rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interFrequencyMeasurement->
    ...
    rrc_InterFreqCellInfoList_r4,newInterFreqCellList))
  {
    ...
    /* Pointer to next cell is Non-NULL and cell count
      * hasn't exceeded the max. L1 can handle */
      ...
      /* Fill in Cell Info */
      if (rrcmeas_fill_mcm_cell_info_r5(wrrc_as_id,
        &local_network_new_cell_list_ptr->elem[idx].cellInfo,
        &l1_inter_freq_meas_ptr->cell_list.add_cell[cell_count].
        cell_info) == FAILURE)
      {
...
)
{
  ...
  if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r6_setup)
  {
    ...
    if (rrcifreq_process_reporting_criteria_r6(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.setup->u.interFrequencyMeasurement->
  ...
  else if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r6_modify)
  {
    ...
    else
    {
      ...
      if (rrcifreq_process_reporting_criteria_r6(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interFrequencyMeasurement->
        reportCriteria, &l1_inter_freq_meas_params->meas_object.u.inter_f) == FAILURE)
      {
...
)
{
  ...
  case T_rrc_InterFreqReportCriteria_r6_interFreqReportingCriteria:
    ...
    if ((RRC_MSG_COMMON_BITMASK_IE_PTR(ifreq_rpt_criteria_ptr->u.interFreqReportingCriteria,
      ...
      (ifreq_rpt_criteria_ptr->u.interFreqReportingCriteria->interFreqEventList.n != 0))
    {
      if (rrcifreq_process_event_criteria_r6(wrrc_as_id,ifreq_rpt_criteria_ptr->u.interFreqReportingCriteria,
...
)
{
  ...
  if(!rrc_sibdb_is_sib11bis_present_in_srv_cell(wrrc_as_id))
  {
    WRRC_MSG0_MED("SIB11bis: SIB11bis is not present in Serving Cell");
    return FAILURE;
  }
  sib11bis_ptr = (rrc_SysInfoType11bis *)  rrc_sibdb_return_sib_for_srv_cell
    (wrrc_as_id, rrc_SIB11bis);
  ...
  else if(rrc_ccm_get_serv_plmn_cellid(wrrc_as_id,&tmp_cell_id, &selected_plmn_identity) != RRC_CELL_LOSS_NO_NEW_CELL)
  {
    WCDMA_MEMCPY(&tmp_plmn_identity, 
                 sizeof(rrc_plmn_identity_type),
                 &selected_plmn_identity, 
      ...
      case T_rrc_MeasurementControlSysInfoExtension_cellSelectQualityMeasure_1_cpich_RSCP:
        ...
        /* Pointer to next cell is Non-NULL and cell count
          ...
          if (inter_freq_meas_hcs_rscp_ptr->elem[idx].m.frequencyInfoPresent)
          {
            ...
            if (validate_frequency_info(wrrc_as_id, &inter_freq_meas_hcs_rscp_ptr->elem[idx].frequencyInfo) == FAILURE)
            {
#ifdef FEATURE_WCDMA_MULTI_FREQ_BAND_INDICATOR 
              /*As the frequency validation failed,we look into the MFBI list present for this UARFCN*/
              if((rrcnv_data.rrc_mfbi_support_nv) && (sib11bis_ptr != NULL) && 
          ...
          * else use the info from the previous cell */
          ...
          /* First store cell info so that psc of this cell can be extracted */
          if (rrcmeas_fill_hcs_rscp_cell_info(wrrc_as_id,&inter_freq_meas_hcs_rscp_ptr->elem[idx].
            cellInfo, &l1_inter_freq_sib_ptr->add_cell[cell_count].
            cell_info, ((rel5_ext_ptr != NULL) ? ((rel5_ext_ptr->n > idx) ? (&rel5_ext_ptr->elem[idx]) : NULL) : NULL)) == FAILURE)
          {
            WRRC_MSG0_HIGH("IFreq:Ignore InterFreq Cell");
      ...
      case T_rrc_MeasurementControlSysInfoExtension_cellSelectQualityMeasure_1_cpich_Ec_N0:
        ...
        /* Pointer to next cell is Non-NULL and cell count
          ...
          if (inter_freq_meas_hcs_ecn0_ptr->elem[idx].m.frequencyInfoPresent)
          {
            ...
            if (validate_frequency_info(wrrc_as_id, &inter_freq_meas_hcs_ecn0_ptr->elem[idx].frequencyInfo) == FAILURE)
            {
#ifdef FEATURE_WCDMA_MULTI_FREQ_BAND_INDICATOR 
 /*As the frequency validation failed,we look into the MFBI list present for this UARFCN*/
              if((rrcnv_data.rrc_mfbi_support_nv) && (sib11bis_ptr != NULL) && 
          ...
          * else use the info from the previous cell */
          ...
          /* First store cell info so that psc of this cell can be extracted */
          if (rrcmeas_fill_hcs_ecn0_cell_info(wrrc_as_id,&inter_freq_meas_hcs_ecn0_ptr->elem[idx].
            cellInfo, &l1_inter_freq_sib_ptr->add_cell[cell_count].
            cell_info, ((rel5_ext_ptr != NULL) ? ((rel5_ext_ptr->n > idx) ? (&rel5_ext_ptr->elem[idx]) : NULL) : NULL)) == FAILURE)
          {
            MSG_LOW("IFreq:Ignore InterFreq Cell",0,0,0);
      ...
      case T_rrc_MeasurementControlSysInfoExtension_cellSelectQualityMeasure_1_cpich_RSCP:
        ...
        /* Pointer to next cell is Non-NULL and cell count
          ...
          if (inter_freq_meas_rscp_ptr->elem[idx].m.frequencyInfoPresent)
          {
            ...
            if (validate_frequency_info(wrrc_as_id, &inter_freq_meas_rscp_ptr->elem[idx].frequencyInfo) == FAILURE)
            {
#ifdef FEATURE_WCDMA_MULTI_FREQ_BAND_INDICATOR 
/*As the frequency validation failed,we look into the MFBI list present for this UARFCN*/
              if((rrcnv_data.rrc_mfbi_support_nv) && (sib11bis_ptr != NULL) && 
          ...
          /* First store cell info so that psc of this cell can be extracted */
          if (rrcmeas_fill_rscp_cell_info(wrrc_as_id,&inter_freq_meas_rscp_ptr->elem[idx].
            cellInfo, &l1_inter_freq_sib_ptr->add_cell[cell_count].
            cell_info, ((rel5_ext_ptr != NULL) ? ((rel5_ext_ptr->n > idx) ? (&rel5_ext_ptr->elem[idx]) : NULL) : NULL)) == FAILURE)
          {
      ...
      case T_rrc_MeasurementControlSysInfoExtension_cellSelectQualityMeasure_1_cpich_Ec_N0:
        ...
        /* Pointer to next cell is Non-NULL and cell count
          ...
          if (inter_freq_meas_ecn0_ptr->elem[idx].m.frequencyInfoPresent)
          {
            ...
            if (validate_frequency_info(wrrc_as_id, &inter_freq_meas_ecn0_ptr->elem[idx].frequencyInfo) == FAILURE)
            {
#ifdef FEATURE_WCDMA_MULTI_FREQ_BAND_INDICATOR 
/*As the frequency validation failed,we look into the MFBI list present for this UARFCN*/
              if((rrcnv_data.rrc_mfbi_support_nv) && (sib11bis_ptr != NULL) && 
          ...
          /* First store cell info so that psc of this cell can be extracted */
          if (rrcmeas_fill_ecn0_cell_info(wrrc_as_id,&inter_freq_meas_ecn0_ptr->elem[idx].
            cellInfo, &l1_inter_freq_sib_ptr->add_cell[cell_count].
            cell_info, ((rel5_ext_ptr != NULL) ? ((rel5_ext_ptr->n > idx) ? (&rel5_ext_ptr->elem[idx]) : NULL) : NULL)) == FAILURE)
          {
...
)
{
  ...
  {
    rrcifreq_free_memory_allocated_for_reporting(wrrc_as_id, cmd_ptr->cmd.meas_ind.meas_rpt);
  }
}
...
)
{
  ...
  if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r7_setup)
  {
    ...
    if (rrcifreq_process_reporting_criteria_r7(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.setup->u.interFrequencyMeasurement->
      reportCriteria, &l1_inter_freq_meas_params->meas_object.u.inter_f) == FAILURE)
    {
  ...
  else if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r7_modify)
  {
    ...
    else
    {
      ...
      if (rrcifreq_process_reporting_criteria_r7(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interFrequencyMeasurement->
        reportCriteria, &l1_inter_freq_meas_params->meas_object.u.inter_f) == FAILURE)
      {
...
)
{
  ...
  if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r8_setup)
  {
    ...
    result = rrcifreq_fill_inter_freq_info_r8(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.setup->u.
      interFrequencyMeasurement->interFreqCellInfoList,
      l1_inter_freq_meas_params) ;
    /* Fill cell list, cell info */
    if (result == RRCMEAS_FAIL)
  ...
  else if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r8_modify)
  {
    ...
    else
    {
      ...
      /* Fill cell list, cell info */
      result = rrcifreq_fill_inter_freq_info_r8(wrrc_as_id,
        &rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interFrequencyMeasurement->
        interFreqCellInfoList, l1_inter_freq_meas_params);
      if (result == RRCMEAS_FAIL)
      {
      ...
      do
      {
          ...
          inter_freq_cell_id, &psc, &dl_freq) == PSC_ABSENT)
        {
          ...
          for (cell_count = 0; cell_count < L1_MAX_CELL_MEAS; cell_count++)
          {
            ...
            WCDMA_MEMCPY(&rrcifreq_ext_data.inter_freq_cell_list[cell_count], 
                         sizeof(inter_freq_cell_list_struct_type),
                         &rrcifreq_int_data.inter_freq_cell_list_tmp[cell_count],
                         sizeof(inter_freq_cell_list_struct_type));
          }
...
)
{
  ...
  for ( count = 0; count < MAX_INTER_FREQ_MEAS; count++ )
  {
    if (rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_id == l1_mcm_ptr->meas_id)
    {
      ...
      MSG_LOW("IFREQ:Before commiting ..IFREQ:Id %d, Val %d, Owner %d", rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_id,
        rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_validity,
        rrcifreq_ext_data.inter_freq_meas_id_list[count].mcm_owned);
      ...
      /* Set Report Criteria */
      rrcifreq_ext_data.inter_freq_meas_id_list[count].rpt_crit = *rpt_crit_ptr;
      WRRC_MSG1_HIGH("IFreq:Meas Crit %d saved in RRC", rrcifreq_ext_data.inter_freq_meas_id_list[count].rpt_crit);


      WRRC_MSG3_HIGH("IFREQ:After commiting .. IFREQ:Id %d, Val %d, Owner %d", rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_id,
        rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_validity,
        rrcifreq_ext_data.inter_freq_meas_id_list[count].mcm_owned);
  ...
  for ( count = 0; count < MAX_INTER_FREQ_MEAS; count++ )
  {
    ...
    if (rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_id == INVALID_MEAS_ID)
    {
      ...
      {
        rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_id = meas_id_val;

        /* Set this only if extended measurements is supported */
        /* This needs to be set while sending measurement report */
        rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_id_legacy = rrc_mcm_ptr->measurementIdentity;
      }
      else
      {
        rrcifreq_ext_data.inter_freq_meas_id_list[count].meas_id = rrc_mcm_ptr->measurementIdentity;
      }
    ...
    rrc_InterFreqCellInfoList_r9,newInterFreqCellList))
  {
    ...
    /* Pointer to next cell is Non-NULL and cell count
      * hasn't exceeded the max. L1 can handle */
      ...
      /* Fill in Cell Info */
      if (rrcmeas_fill_mcm_cell_info_r9(wrrc_as_id,
        &local_network_new_cell_list_ptr->elem[idx].cellInfo,
        &l1_inter_freq_meas_ptr->cell_list.add_cell[cell_count].
        cell_info) == FAILURE)
      {
...
)
{
  ...
  if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r9_setup)
  {
    ...
    if (rrcifreq_process_reporting_criteria_r9(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.setup->u.interFrequencyMeasurement->
      reportCriteria, &l1_inter_freq_meas_params->meas_object.u.inter_f) == FAILURE)
    {
    ...
    result = rrcifreq_fill_inter_freq_info_r9(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.setup->u.
      interFrequencyMeasurement->interFreqCellInfoList,
      l1_inter_freq_meas_params) ;
    /* Fill cell list, cell info */
    if (result == RRCMEAS_FAIL)
    ...
    /* Now check Additional Measurements */
    /* Process Additional Meas */
    if(rrcmeas_process_mcm_addnl_meas(wrrc_as_id,mcm_r9,transaction_id,&addtl_meas_incl,
    ...
    /* Now commit new meas params to RRC db.*/
    if (rrcifreq_config_inter_freq_db_r9(wrrc_as_id,l1_inter_freq_meas_params, &mcm_rpt_crit, meas_validity,mcm_r9) == FALSE)
    {
    ...
    if (addtl_meas_incl == TRUE)
    {
      rrcmeas_check_addtl_meas_in_db(wrrc_as_id,&primary_meas_type, mcm_r9,
  ...
  else if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r9_modify)
  {
    ...
    else
    {
      ...
      if (rrcifreq_process_reporting_criteria_r9(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interFrequencyMeasurement->
        reportCriteria, &l1_inter_freq_meas_params->meas_object.u.inter_f) == FAILURE)
      {
      ...
      /* Now check Additional Measurements */
      /* Process Additional Meas */
      if(rrcmeas_process_mcm_addnl_meas(wrrc_as_id,mcm_r9,transaction_id,&addtl_meas_incl,primary_meas_type,l1_inter_freq_meas_params->meas_id,&meas_validity) == FAILURE)
      {
            rrc_free(l1_inter_freq_meas_params);
            return FAILURE;
      }
      ...
      /* Fill cell list, cell info */
      result = rrcifreq_fill_inter_freq_info_r9(wrrc_as_id,
        &rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interFrequencyMeasurement->
        interFreqCellInfoList, l1_inter_freq_meas_params);
      if (result == RRCMEAS_FAIL)
      {
...
FUNCTION   RRCIFREQ_CONFIG_INTER_FREQ_DB_R10
...
boolean rrcifreq_config_inter_freq_db_r10
(
 sys_modem_as_id_e_type wrrc_as_id, 
 l1_meas_ctrl_parm_struct_type* l1_mcm_ptr,
 rpt_crit_enum_type* rpt_crit_ptr,
...
/*===========================================================================
FUNCTION  rrcifreq_fill_inter_freq_info_r10
...
SIDE EFFECTS
 None
===========================================================================*/
rrcmeas_status_e_type rrcifreq_fill_inter_freq_info_r10
(
 sys_modem_as_id_e_type wrrc_as_id, 
 rrc_InterFreqCellInfoList_r10* interFreqCellInfoList_ptr,
 l1_meas_ctrl_parm_struct_type* l1_meas_ptr
)
{
  inter_freq_removed_cell_struct_type cascaded_rem_cell[L1_MAX_CELL_MEAS];
  ...
  rrc_NewInterFreqCellList_r10* local_network_new_cell_list_ptr = NULL;
...
/*Need to add support for  rrc_FreqIndexListForEnhancedMeas freqIndexListForEnhancedMeas; (new IE added in rrc_InterFrequencyMeasurement_r10 structuer)*/
/*Need to add support for ASN1BitStr32 sNPLMonitorSetIndicator_TDD128 (New IE added in rrc_NewInterFreqCell_r10_tdd128 structure)*/

  if (RRC_MSG_COMMON_BITMASK_IE_PTR(interFreqCellInfoList_ptr,rrc_InterFreqCellInfoList_r10,
    removedInterFreqCellList))
  {
    WRRC_MSG1_HIGH("REL8:IFreq:Remove:Delete [1: ALL  2: SOME  3:NO ]:- %d cells selected",interFreqCellInfoList_ptr->removedInterFreqCellList.t);
    switch (interFreqCellInfoList_ptr->removedInterFreqCellList.t)
    ...
    case T_rrc_RemovedInterFreqCellList_removeSomeInterFreqCells:
      ...
      do
      {
        ...
        if ( rrcifreq_find_psc_freq(wrrc_as_id,
          inter_freq_cell_id, &psc, &dl_freq) == PSC_ABSENT)
        {
          WRRC_MSG0_HIGH("REL10:IFreq:Retain previous cell list");
      ...
      l1_inter_freq_meas_ptr->cell_list.num_cell_rmv =  removed_cell_count;
      WRRC_MSG1_HIGH("REL10:IFreq:Removed cell cnt = %d cells", removed_cell_count);
  ...
  /* Check if new cells present */
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(interFreqCellInfoList_ptr,
    rrc_InterFreqCellInfoList_r10,newInterFreqCellList))
  {
    ...
    /* Pointer to next cell is Non-NULL and cell count
      * hasn't exceeded the max. L1 can handle */
      ...
      if ((local_network_new_cell_list_ptr->elem[idx].
        ...
        cellInfo.modeSpecificInfo.u.fdd->m.primaryCPICH_InfoPresent)))
      {
        MSG_LOW("REL10:IFreq:Ignore TDD cell",0,0,0);
      ...
      if (rrcmeas_fill_mcm_cell_info_r9(wrrc_as_id,
        ...
        cell_info) == FAILURE)
      {
        MSG_LOW("REL10:IFreq:Ignoring Cell",0,0,0);
        idx++;
        continue;
      }
      ...
      /* Check if cell id present */
      if (RRC_MSG_LIST_BITMASK_IE(local_network_new_cell_list_ptr,
        rrc_NewInterFreqCell_r10,interFreqCellID))
      {
        cell_position = local_network_new_cell_list_ptr->elem[idx].interFreqCellID;

        WRRC_MSG1_MED("REL10:IFreq:Add:Cell at pos %d to be added",cell_position);

      } /*  End of if of inter-freq cell id present */
      else
      {
        WRRC_MSG0_HIGH("REL10:IFREQ:Add: Cell Id not present");
        cell_position = AVAILABLE_CELL_POSITION;
      }

      /* Check if freq present */
      if (RRC_MSG_LIST_BITMASK_IE(local_network_new_cell_list_ptr,
        rrc_NewInterFreqCell_r10,frequencyInfo))
      {
        if (local_network_new_cell_list_ptr->elem[idx].frequencyInfo.modeSpecificInfo.t ==
          T_rrc_FrequencyInfo_modeSpecificInfo_tdd)
        {
          MSG_LOW("REL10:IFreq:Ignore Cell. TDD cell",0,0,0);
          idx++;
          continue;
        }
      ...
      else
      {
        if (cell_count == 0)
        {
          WRRC_MSG0_HIGH("REL10:IFreq:Ignore Cell. DL Freq absent");
          idx++;
          continue;
        }
        else
    ...
    for (cell_count = 0; cell_count < psc_removed_count; cell_count++)
    {
      ...
      {
        /* Preventive check */
        WRRC_MSG0_HIGH("REL10:IFREQ:Unable to delete all cells");
        break;
      }
      /* Add this removed cell to the already built removed cell list for L1 */
      WRRC_MSG1_HIGH("REL10:IFREQ:Cascad:L1 removed cell list upd %d psc",
        cascaded_rem_cell[cell_count].psc);
      l1_inter_freq_meas_ptr->cell_list.rmv_cell[removed_cell_count].psc =
        cascaded_rem_cell[cell_count].psc;
  ...
  /* Check if cells for meas present */
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(interFreqCellInfoList_ptr,
    & rrc_InterFreqCellInfoList_r10,cellsForInterFreqMeasList))
  {
    ...
    while (cellsForInterFreqMeasList_ptr->n > idx)
    {
      ...
      if ( rrcifreq_find_psc_freq(wrrc_as_id,cell_id, &psc_to_measure, &freq_to_measure) == PSC_ABSENT)
      {
        WRRC_MSG0_HIGH("REL10:Retain previous cell list before MCM rcved");
...
FUNCTION   RRCIFREQ_PROCESS_EVENT_CRITERIA_R10
...
uecomdef_status_e_type rrcifreq_process_event_criteria_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  rrc_InterFreqReportingCriteria_r10* rrc_event_criteria_ptr,
  l1_inter_freq_report_crit_struct_type* l1_event_criteria_ptr
)
{
  /* Store the list in a local pointer */
  struct rrc_InterFreqEventList_r10* ifreq_event_list_ptr = NULL;
  ...
  struct rrc_NonUsedFreq2aParameterList_r10 * local_2a_non_used_param_ptr = NULL;
  struct rrc_NonUsedFreqParameterList_r10 * local_non_used_param_ptr = NULL;
  ...
  while ((ifreq_event_list_ptr->n > idx) && (cnt < L1_MAX_MEAS_EVENT))
  {
    ...
    case L1_INTER_FREQ_EVENT_2A:
      ...
      if (RRC_MSG_COMMON_BITMASK_IE_PTR(ifreq_event_list_ptr->elem[idx].u.event2a,
        rrc_Event2a_r10,reportingCellStatus))
      {
        if (rrcmeas_fill_no_of_reporting_cells_r10(wrrc_as_id,
          &ifreq_event_list_ptr->elem[idx].u.event2a->reportingCellStatus,
          &l1_event_criteria_ptr->evt[cnt].rpt_cell_status) == SUCCESS)
        {
          l1_event_criteria_ptr->evt[cnt].rpt_cell_status_incl = TRUE;
      ...
      /* Get Threshold Non-Used Freq */
      if (RRC_MSG_COMMON_BITMASK_IE_PTR(ifreq_event_list_ptr->elem[idx].u.event2a,
        rrc_Event2a_r10,nonUsedFreqParameterList))
      {
        local_2a_non_used_param_ptr =
          &ifreq_event_list_ptr->elem[idx].u.event2a->nonUsedFreqParameterList;
        /*Need to add support for the nonUsedFreqTriggeringConditionDetectedCells , new IE added in rrc_NonUsedFreq2aParameterList_r10 structure*/
        ...
        while ((local_2a_non_used_param_ptr->n > count) && (count < L1_MAX_NON_USED_FREQ))
        {
          ...
          l1_event_criteria_ptr->evt[cnt].non_used_f_parm_array[count].non_used_f_thresh
            =  0;
          /* Non used f */
          tmp = local_2a_non_used_param_ptr->elem[count].nonUsedFreqW;
          l1_event_criteria_ptr->evt[cnt].non_used_f_parm_array[count].w_non_used_f =
            (uint16) tmp;
    ...
    threshold and the estimated quality of a non-used frequency is above a certain
      ...
      if (RRC_MSG_COMMON_BITMASK_IE_PTR(ifreq_event_list_ptr->elem[idx].u.event2b,
        rrc_Event2b_r10,reportingCellStatus))
      {
        if (rrcmeas_fill_no_of_reporting_cells_r10(wrrc_as_id,
          &ifreq_event_list_ptr->elem[idx].u.event2b->reportingCellStatus,
          &l1_event_criteria_ptr->evt[cnt].rpt_cell_status) == SUCCESS)
      ...
      /* Get Threshold Non-Used Freq */
      if (RRC_MSG_COMMON_BITMASK_IE_PTR(ifreq_event_list_ptr->elem[idx].u.event2b,
        rrc_Event2b_r10,nonUsedFreqParameterList))
      {
        local_non_used_param_ptr =
          &ifreq_event_list_ptr->elem[idx].u.event2b->nonUsedFreqParameterList;
    ...
    case L1_INTER_FREQ_EVENT_2C:
      ...
      if (RRC_MSG_COMMON_BITMASK_IE_PTR(ifreq_event_list_ptr->elem[idx].u.event2c,
        rrc_Event2c_r10,reportingCellStatus))
      {
        if (rrcmeas_fill_no_of_reporting_cells_r10(wrrc_as_id,
          &ifreq_event_list_ptr->elem[idx].u.event2c->reportingCellStatus,
          &l1_event_criteria_ptr->evt[cnt].rpt_cell_status) == SUCCESS)
      ...
      if (RRC_MSG_COMMON_BITMASK_IE_PTR(
        ifreq_event_list_ptr->elem[idx].u.event2c,
        rrc_Event2c_r10,nonUsedFreqParameterList))
      {
        local_non_used_param_ptr =
          &ifreq_event_list_ptr->elem[idx].u.event2c->nonUsedFreqParameterList;
    ...
    case L1_INTER_FREQ_EVENT_2E:
      ...
      if (RRC_MSG_COMMON_BITMASK_IE_PTR(ifreq_event_list_ptr->elem[idx].u.event2e,
        rrc_Event2e_r10,reportingCellStatus))
      {
        if (rrcmeas_fill_no_of_reporting_cells_r10(wrrc_as_id,
          &ifreq_event_list_ptr->elem[idx].u.event2e->reportingCellStatus,
          &l1_event_criteria_ptr->evt[cnt].rpt_cell_status) == SUCCESS)
      ...
      /* Get Threshold Non-Used Freq */
      if (RRC_MSG_COMMON_BITMASK_IE_PTR(ifreq_event_list_ptr->elem[idx].u.event2e,
          rrc_Event2e_r10,nonUsedFreqParameterList))
      {
        local_non_used_param_ptr =
          &ifreq_event_list_ptr->elem[idx].u.event2e->nonUsedFreqParameterList;
    ...
    case L1_INTER_FREQ_EVENT_2D:
      ...
      if (RRC_MSG_COMMON_BITMASK_IE_PTR(
        ifreq_event_list_ptr->elem[idx].u.event2d,
        rrc_Event2d_r10,reportingCellStatus))
      {
        if (rrcmeas_fill_no_of_reporting_cells_r10(wrrc_as_id,
          &ifreq_event_list_ptr->elem[idx].u.event2d->reportingCellStatus,
          &l1_event_criteria_ptr->evt[cnt].rpt_cell_status) == SUCCESS)
    ...
    case L1_INTER_FREQ_EVENT_2F:
      ...
      if (RRC_MSG_COMMON_BITMASK_IE_PTR(
        ifreq_event_list_ptr->elem[idx].u.event2f,
        rrc_Event2f_r10,reportingCellStatus))
      {
        if (rrcmeas_fill_no_of_reporting_cells_r10(wrrc_as_id,
          &ifreq_event_list_ptr->elem[idx].u.event2f->reportingCellStatus,
          &l1_event_criteria_ptr->evt[cnt].rpt_cell_status) == SUCCESS)
...
FUNCTION   RRCIFREQ_PROCESS_REPORTING_CRITERIA_R10
...
uecomdef_status_e_type rrcifreq_process_reporting_criteria_r10
(
 sys_modem_as_id_e_type wrrc_as_id, 
 rrc_InterFreqReportCriteria_r10* ifreq_rpt_criteria_ptr,
 l1_inter_freq_meas_ctrl_struct_type* l1_rpt_params_ptr
)
{
  switch(ifreq_rpt_criteria_ptr->t)
  {
  case T_rrc_InterFreqReportCriteria_r10_interFreqReportingCriteria:
    ...
    if ((RRC_MSG_COMMON_BITMASK_IE_PTR(ifreq_rpt_criteria_ptr->u.interFreqReportingCriteria,
      & rrc_InterFreqReportingCriteria_r10,interFreqEventList)) &&
      (ifreq_rpt_criteria_ptr->u.interFreqReportingCriteria->interFreqEventList.n != 0))
    {
      if (rrcifreq_process_event_criteria_r10(wrrc_as_id,ifreq_rpt_criteria_ptr->u.interFreqReportingCriteria,
        &l1_rpt_params_ptr->u.inter_rpt_crit) == FAILURE)
      {
        return FAILURE;
      }
  ...
  case T_rrc_InterFreqReportCriteria_r10_intraFreqReportingCriteria :

    WRRC_MSG0_HIGH("IFreq: Intra Freq Event Rpt Crit. Setting rpt crit to L1_INTER_FREQ_NO_CHANGE");
    l1_rpt_params_ptr->rpt_mode = L1_INTER_FREQ_NO_CHANGE;
    break;

  case T_rrc_InterFreqReportCriteria_r10_periodicalReportingCriteria :
    ...
    /* Set reporting cells */
    if (RRC_MSG_COMMON_BITMASK_IE_PTR(ifreq_rpt_criteria_ptr->u.periodicalReportingCriteria,
      rrc_PeriodicalWithReportingCellStatus_r10,reportingCellStatus))
    {
      if (rrcmeas_fill_no_of_reporting_cells_r10(wrrc_as_id,&ifreq_rpt_criteria_ptr->u.periodicalReportingCriteria->reportingCellStatus,
        &l1_rpt_params_ptr->rpt_cell_status) == SUCCESS)
      {
        l1_rpt_params_ptr->rpt_cell_status_incl = TRUE;
  ...
  case T_rrc_InterFreqReportCriteria_r10_noReporting :


    l1_rpt_params_ptr->rpt_mode = L1_INTER_FREQ_NO_REPORTING;

    if (RRC_MSG_COMMON_BITMASK_IE_PTR(ifreq_rpt_criteria_ptr->u.noReporting,
        rrc_ReportingCellStatusOpt_r10,reportingCellStatus))
    {
      if (rrcmeas_fill_no_of_reporting_cells_r10(wrrc_as_id,&ifreq_rpt_criteria_ptr->u.noReporting->reportingCellStatus,
        &l1_rpt_params_ptr->rpt_cell_status) == SUCCESS)
      {
        l1_rpt_params_ptr->rpt_cell_status_incl = TRUE;
...
/*===========================================================================
FUNCTION   RRCIFREQ_PROCESS_MCM_R10
...
SIDE EFFECTS
    None
==========================================================================*/
uecomdef_status_e_type rrcifreq_process_mcm_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  rrc_cmd_type* cmd_ptr,
  uint8 transaction_id
)
{
  ...
  /* Set Compressed Mode Information */
  if ((RRC_MSG_COMMON_BITMASK_IE_PTR(rrc_mcm_ptr, rrc_MeasurementControl_r10_IEs,dpch_CompressedModeStatusInfo)) &&
    (rrc_mcm_ptr->dpch_CompressedModeStatusInfo.tgp_SequenceShortList.n != 0))
  {

    if (rrcmeas_process_compressed_mode_status_r10(wrrc_as_id,&rrc_mcm_ptr->dpch_CompressedModeStatusInfo,
      &l1_inter_freq_meas_params->cm_status_info) == FAILURE)
    {
      WRRC_MSG0_HIGH("REL10:IFREQ:Invalid CompMode Status Info %d");
      /* Send Meas Ctrl Failure Message */
  ...
  if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r10_setup)
  {

    /* Double check that it is INTER-FREQ Meas */
    if (rrc_mcm_ptr->measurementCommand.u.setup->t !=
        T_rrc_MeasurementType_r10_interFrequencyMeasurement)
    {
      WRRC_MSG1_HIGH("REL10:IFREQ:UnSupported Meas %d",rrc_mcm_ptr->measurementIdentity);
      /* Send Meas Ctrl Failure Message */
    ...
    /* Check Measurement Quantity present */
    if (!(RRC_MSG_COMMON_BITMASK_IE_PTR(rrc_mcm_ptr->measurementCommand.u.setup->u.interFrequencyMeasurement,
          rrc_InterFrequencyMeasurement_r10,interFreqMeasQuantity)))
    {
      WRRC_MSG0_ERROR("REL10:SETUP:MeasQuan absent");
    ...
    /* Check Reporting Quantity */
    if (!(RRC_MSG_COMMON_BITMASK_IE_PTR(rrc_mcm_ptr->measurementCommand.u.setup->u.interFrequencyMeasurement,
          rrc_InterFrequencyMeasurement_r10,interFreqReportingQuantity)))
    {
      WRRC_MSG0_ERROR("REL10:IFreq:SETUP:RptQuan absent");
    ...
    if(rrc_mcm_ptr->measurementCommand.u.setup->u.interFrequencyMeasurement->
      reportCriteria.t == T_rrc_InterFreqReportCriteria_r10_intraFreqReportingCriteria)
    {
      WRRC_MSG0_ERROR("REL10:IFreq:SETUP:inter freq rpt criteria set to Intra freq rpt criteria");
    ...
    /* Check if update present */
    if (RRC_MSG_COMMON_BITMASK_IE_PTR(rrc_mcm_ptr->measurementCommand.u.setup->u.interFrequencyMeasurement,
       rrc_InterFrequencyMeasurement_r10,interFreqSetUpdate))
    {
      if(RRC_CHECK_COMMON_MSG_TYPE2(rrc_mcm_ptr->measurementCommand.u.setup->u.interFrequencyMeasurement->interFreqSetUpdate,rrc_UE_AutonomousUpdateMode,
        onWithNoReporting))
      {
      ...
      {
        /* Since L1 does not support this, reject the MCM */
        WRRC_MSG0_HIGH("REL10:IFreq:Set Update t invalid");
        rrcmeas_mcf.rrc_transaction_id = transaction_id;
    ...
    if((rrc_mcm_ptr->measurementCommand.u.setup->u.interFrequencyMeasurement->reportCriteria.t
      == T_rrc_InterFreqReportCriteria_r10_interFreqReportingCriteria) &&
      (RRC_MSG_COMMON_BITMASK_IE_PTR(rrc_mcm_ptr->measurementCommand.u.setup->u.interFrequencyMeasurement->reportCriteria.
       u.interFreqReportingCriteria,rrc_InterFreqReportingCriteria_r10,interFreqEventList)))
    {
      ifreq_event_list_ptr = &rrc_mcm_ptr->measurementCommand.u.setup->u.interFrequencyMeasurement->
      reportCriteria.u.interFreqReportingCriteria->interFreqEventList;
      ...
      if(!(l1_inter_freq_meas_params->meas_object.u.inter_f.inter_freq_set_update_incl))
      {
        while(ifreq_event_list_ptr->n > idx)
        {
          if((ifreq_event_list_ptr->elem[idx].t != T_rrc_InterFreqEvent_r10_event2d )
            && (ifreq_event_list_ptr->elem[idx].t != T_rrc_InterFreqEvent_r10_event2f))
          {
             WRRC_MSG0_ERROR("REL10:IFreq:Ifreq Set update not set and evt id != 2d or 2f %d");
             rrcmeas_mcf.rrc_transaction_id = transaction_id;
    ...
    if (rrcifreq_process_reporting_criteria_r10(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.setup->u.interFrequencyMeasurement->
      reportCriteria, &l1_inter_freq_meas_params->meas_object.u.inter_f) == FAILURE)
    {
    ...
    result = rrcifreq_fill_inter_freq_info_r10(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.setup->u.
      interFrequencyMeasurement->interFreqCellInfoList,
      l1_inter_freq_meas_params) ;
    /* Fill cell list, cell info */
    if (result == RRCMEAS_FAIL)
    ...
    else
    {
      ...
      {
              /* Send Meas Ctrl Failure Message */
        WRRC_MSG0_ERROR("REL10:IFreq:SFN-ind set as True");
      /* Send Meas Ctrl Failure Msg */
    ...
    /* Now check Additional Measurements */
    /* Process Additional Meas */
    if(rrcmeas_process_mcm_addnl_meas_r10(wrrc_as_id,mcm_r10,transaction_id,&addtl_meas_incl,
       primary_meas_type,l1_inter_freq_meas_params->meas_id, &meas_validity) == FAILURE)
    {
      rrc_free(l1_inter_freq_meas_params);
      return FAILURE;
    ...
    if (RRC_MSG_COMMON_BITMASK_IE_PTR(rrc_mcm_ptr->measurementCommand.u.setup->u.interFrequencyMeasurement,
      rrc_InterFrequencyMeasurement_r10,measurementValidity))
    {
    ...
    /* Now commit new meas params to RRC db.*/
    if (rrcifreq_config_inter_freq_db_r10(wrrc_as_id,l1_inter_freq_meas_params, &mcm_rpt_crit, meas_validity,mcm_r10) == FALSE)
    {

      WRRC_MSG0_ERROR("REL10:IFreq:Unable to set MeasId in DB");
      rrcmeas_mcf.rrc_transaction_id = transaction_id;
    ...
    if (addtl_meas_incl == TRUE)
    {
      rrcmeas_check_addtl_meas_in_db_r10(wrrc_as_id,&primary_meas_type, mcm_r10,
        l1_inter_freq_meas_params->meas_id);
  ...
  else if (rrc_mcm_ptr->measurementCommand.t == T_rrc_MeasurementCommand_r10_modify)
  {
    /* Check if reporting Mode present */
    ...
    {
      WRRC_MSG0_HIGH("REL10:IFREQ:Modify:InterFreq:MType absent");
    ...
    else
    {
      ...
      if((rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interFrequencyMeasurement->
        ...
        reportCriteria.u.intraFreqReportingCriteria,
         rrc_IntraFreqReportingCriteria_r9,eventCriteriaList)))
      {
        WRRC_MSG0_ERROR("REL10:IFreq:Modify:rpt crit set to Intraf rpt crit and evnt list non null");
      ...
      if (rrcifreq_process_reporting_criteria_r10(wrrc_as_id,&rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interFrequencyMeasurement->
        reportCriteria, &l1_inter_freq_meas_params->meas_object.u.inter_f) == FAILURE)
      {

        if((rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interFrequencyMeasurement->
         reportCriteria.t == T_rrc_InterFreqReportCriteria_r10_interFreqReportingCriteria) &&
         !((RRC_MSG_COMMON_BITMASK_IE_PTR(rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interFrequencyMeasurement->
         reportCriteria.u.interFreqReportingCriteria,
         rrc_InterFreqReportingCriteria_r10,interFreqEventList)) &&
         (rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interFrequencyMeasurement->
      ...
      /* Now check Additional Measurements */
      /* Process Additional Meas */
      if(rrcmeas_process_mcm_addnl_meas_r10(wrrc_as_id,mcm_r10,transaction_id,&addtl_meas_incl,primary_meas_type,l1_inter_freq_meas_params->meas_id,&meas_validity) == FAILURE)
      {
            rrc_free(l1_inter_freq_meas_params);
            return FAILURE;
      }
      ...
      /* Fill cell list, cell info */
      result = rrcifreq_fill_inter_freq_info_r10(wrrc_as_id,
        &rrc_mcm_ptr->measurementCommand.u.modify->measurementType.u.interFrequencyMeasurement->
        interFreqCellInfoList, l1_inter_freq_meas_params);
      if (result == RRCMEAS_FAIL)
      {
