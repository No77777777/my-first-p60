...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcenhstatecfg.c#2 $    $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
  
 when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
01/03/18   sp      Made changes to support configuration with RB-ID 32
05/26/16   sg      Made changes to reduce F3s
05/06/16   sg      Made changes to remove F3s
...
{
  wnv_get_nv_msim(WNV_ID_RRC_FEATURE, &rrcnv_data.rrc_feature_nv_item_value, sizeof(rrcnv_data.rrc_feature_nv_item_value), wrrc_as_id);
  WRRC_MSG1_HIGH("EFACH NV item read is %x",rrcnv_data.rrc_feature_nv_item_value);
...
)
{
  ...
  rrcllc_init_efach_mapping_per_rb(wrrc_as_id,config_ptr, rb_id);

  /* Allocate LCH index */
  lc_index = rrcllc_allocate_lc_index_efach(wrrc_as_id,config_ptr, common_rb_mapping_info->logicalChannelIdentity);
  ...
  WRRC_MSG4_HIGH("EFACH:MAC-EHS Q %d for SRB%d, lc_index %d for LCH %d",common_rb_mapping_info->mac_ehs_QueueId,rb_id,lc_index,common_rb_mapping_info->logicalChannelIdentity);
...
)
{
  ...
  rlc_dl_log_chl_id = rrclcm_get_dl_rlc_lc_id (wrrc_as_id,BCCH_FACH_RADIO_BEARER_ID);
  
  lc_index = rrcllc_find_lc_index_for_efach(wrrc_as_id,config_ptr, BCCH_ON_HS_DSCH_LCH_ID +1,
  ...
  {
    /* Allocate LC index */
    lc_index = rrcllc_allocate_lc_index_efach(wrrc_as_id,config_ptr, BCCH_ON_HS_DSCH_LCH_ID +1
    );
  ...
  WRRC_MSG2_HIGH("lc_index %d allocated for LCH %d",lc_index, BCCH_ON_HS_DSCH_LCH_ID +1);
...
)
{
  ...
  while (mac_ehs_info->n > idx)
  {
    ...
    config_ptr->efach_common_queue[idx].window_size = 
      get_hs_window_size(wrrc_as_id,mac_ehs_info->elem[idx].mac_ehsWindowSize);
      
    config_ptr->efach_common_queue[idx].release_timer = 
      get_hs_release_timer(wrrc_as_id,mac_ehs_info->elem[idx].t1_ReleaseTimer);
    ...
    /*WRRC_MSG9_HIGH("EFACH_DEBUG:Queue-ID: %d\tCCTrchId %d\tRelease Timer %d\tWindow_Size %d\tReset Timer %d",
                                            config_ptr->efach_common_queue[idx]->queue_id,
...
)
{
  ...
  {
    hrnti_index = (uint8)(rrc_convert_initial_ue_identity_into_int_value( wrrc_as_id)
                    % num_of_hrnti);
  }
  ...
  if(rrc_translate_hrnti(wrrc_as_id,&ie_ptr ->elem[hrnti_index],
...
)
{
...
#ifdef  FEATURE_WCDMA_HS_FACH_DRX
  if(rrcenhstate_camped_cell_supports_e_fach_drx(wrrc_as_id))
  {
    hs_dsch_DrxCellfach_info = &sib5_ptr->v4b0NonCriticalExtensions.v590NonCriticalExtensions.\
      ...
      (void)rrcenhstate_set_hs_fach_drx_info(wrrc_as_id,config_ptr, hs_dsch_DrxCellfach_info);
  }

#endif

  /*CCCH Mapping is always taken from SIB5*/
  if( FAILURE == rrcenhstate_get_mapping_from_sib5(wrrc_as_id,CCCH_RADIO_BEARER_ID, config_ptr,
      ...
      (next_rrc_state == RRC_STATE_CELL_FACH))
  {
    ...
    {
      if(FAILURE == rrcenhstate_get_mapping_from_sib5(wrrc_as_id,DCCH_UM_RADIO_BEARER_ID,config_ptr,
  ...
  if( FAILURE == rrcenhstate_get_queue_from_sib5(wrrc_as_id,config_ptr,
  ...
  {
    if( FAILURE == rrcenhstate_get_hrnti_from_sib5(wrrc_as_id,config_ptr,
  ...
  {
    if( FAILURE == rrcenhstate_set_harq_info(wrrc_as_id,&config_ptr->l1_hsdpa_info.hs_dsch_info.harq_info,
...
//{
  if( UE_MAX_DL_LOGICAL_CHANNEL == rrcenhstate_get_bcch_mapping_from_sib5(wrrc_as_id,config_ptr))
  {
    WRRC_MSG0_ERROR("EFACH: Failure in setting BCCH Mapping on eFACH");
    return FAILURE;
  }
    if(rrc_translate_hrnti(wrrc_as_id,&hs_dsch_common_sys_info_ptr->bcchSpecific_H_RNTI, 
...
//}
  ...
  if( FAILURE == rrcenhstate_set_hsscch_info(wrrc_as_id,config_ptr, &hs_dsch_common_sys_info_ptr->hs_scch_SystemInfo))
  {
    WRRC_MSG0_ERROR("EFACH: Failure in setting SRB1 Mapping info from SIB5");
    return FAILURE;
  }
  ...
  config_ptr->l1_hsdpa_info.use_sttd = rrcllc_get_correct_pccpch_tx_diversity_from_sibs(wrrc_as_id,next_rrc_state);
  ...
  * Allocate a new index */
  for (cnt =0; cnt <UE_MAX_DL_LOGICAL_CHANNEL; cnt++)
  {
      ...
      (config_ptr->efach_dl_logchan_info[cnt].mac_id == INVALID_LOGICAL_CHANNEL_ID))
    {
      ...
      WRRC_MSG2_MED("EFACH:Assign Index %d for new LC ID %d",
...
{
  rrcllc_set_hs_status_in_e_fach(wrrc_as_id,ordered_config_ptr,FALSE);
  if(rrcllc_get_hs_status_in_e_fach(wrrc_as_id,current_config_ptr) == TRUE)
  {
    if(rrc_get_hsdpa_action(wrrc_as_id) != HSDPA_START)
    {
      ...
      rrcllc_determine_hsdpa_action_for_e_fach(wrrc_as_id);
    }
    else
    {
      WRRC_MSG1_MED("EFACH:hsdpa_action %d",rrc_get_hsdpa_action(wrrc_as_id));
    }
  }
#ifdef TEST_FRAMEWORK
  #error code not present
...
)
{
  ...
  {
    if(rrcllc_efach_mappings_are_given_for_rbs(wrrc_as_id,config_ptr) == FALSE)
    {
      return FALSE;
    }
...
)
{
  ...
  if( rrc_StateIndicator == rrc_RRC_StateIndicator_cell_FACH )
  {
    if((validate_directed_cell_info == TRUE)&&(validation_check == TRUE)&&
       (rrcenhstate_camped_cell_supports_e_fach(wrrc_as_id) == TRUE))
    {
      ...
      else
      {
        ...
        switch(rrc_StateIndicator)
        {
         ...
         /*   */
        if(rrcllc_check_efach_config_data(wrrc_as_id,transition_config.toc_ptr,
                                          next_rrc_state,
  ...
  {
    rrcenhstate_stop_efach_if_active(wrrc_as_id);
  }
  return status;
}
...
)
{
  ...
  if(rrcenhstate_camped_cell_supports_e_fach(wrrc_as_id)== FALSE)
  {
    return SUCCESS;
  }
  ...
  rrcllc_efach_update_dedicated_params(wrrc_as_id,ordered_config_ptr);
  
  if(rrcllc_check_if_hs_can_be_started_reconfigured_in_e_fach(wrrc_as_id,ordered_config_ptr,
  ...
  {
    rrcllc_set_hs_status_in_e_fach(wrrc_as_id,ordered_config_ptr, TRUE);
  }
  rrcllc_determine_hsdpa_action_for_e_fach(wrrc_as_id);
  return SUCCESS;
}
...
void rrcllc_init_efach_mapping_per_rb(sys_modem_as_id_e_type wrrc_as_id,ordered_config_type *config_ptr, uint16 rb_id)
{
  uint8 idx = 0;
...
SIDE EFFECTS  None.
====================================================================*/
uint8 rrcenhstate_get_queue_id(sys_modem_as_id_e_type wrrc_as_id,ordered_config_type *config_ptr, rb_id_type  rb_id)
{
  uint8 ix =0;
...
{
  rrcllc_set_hs_action_in_e_fach(wrrc_as_id,config_ptr,HSDPA_NOOP);
  config_ptr->l1_hsdpa_info.hs_dsch_info.efach_h_rnti_bmsk = 0;

  /*The check whether the cell supports EFACH or not must have been done*/
  if(rrcenhstate_copy_sib5(wrrc_as_id,config_ptr, next_rrc_state, rrc_e_fach_operation_mode) == FAILURE)
  {
  ...
  config_ptr->l1_hsdpa_info.hs_pdsch_rl_psc = rrc_ccm_get_curr_camped_psc(wrrc_as_id);
...
SIDE EFFECTS:
  None.
====================================================================*/
boolean rrcenhstate_verify_sib5_contents(sys_modem_as_id_e_type wrrc_as_id,rrc_SysInfoType5 *sib5_ptr )
{
...
)
{
  ...
  if( rrcenhstate_nv_supports_e_fach(wrrc_as_id) == FALSE )
  {
    return FALSE;
  }
  ...
  if(HSDSCH_COMMON_SYS_INFO_PRESENT_IN_SIB5(sib5_ptr))
  {
    ...
    if(rrcenhstate_verify_sib5_contents(wrrc_as_id,sib5_ptr) == TRUE)
    {
      return TRUE;
    }
    else
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcenhstate_print_cphy_setup(sys_modem_as_id_e_type wrrc_as_id, l1_setup_cmd_type *l1_cmd_ptr)
{

  WRRC_MSG5_HIGH("EFACH_DEBUG:Sig act UL %d, DL %d req_mask 0X%x, hho_setup_mask 0X%x call_type_mask 0X%x",
  ...
  WRRC_MSG4_HIGH("EFACH_DEBUG:freq_incl %d, rf_chan %d, drop_ul_phychan l1_ul_phychan_enum_type_value%d, drop_dl_phychan l1_dl_phychan_enum_type_value%d",
        l1_cmd_ptr->freq_incl,l1_cmd_ptr->rf_chan,
        l1_cmd_ptr->drop_ul_phychan,l1_cmd_ptr->drop_dl_phychan);
   
  /*
  WRRC_MSG3_HIGH("EFACH_DEBUG:dl_phychan l1_dl_phychan_enum_type_value%d, ul_phychan l1_ul_phychan_enum_type_value%d, hsdpa_l1_ops %d",
  ...
  */
  if(l1_cmd_ptr->hsdpa_l1_info != NULL)
  {
    ...
    WRRC_MSG8_HIGH("EFACH_DEBUG:HS-OP:%d, REQ-MASK 0X%x, HS-TYPE %d TB_size_alignment %d, PSC %d HRNTI 0X%x\tBCCH-HRNTI 0X%x\tBIT-MASK 0X%x",
    ...
    WRRC_MSG6_HIGH("EFACH_DEBUG:CCTrchId %d HARQ NumProc %d,Partition(Valid/Invalid::0/1) %d,sec_scr_code_valid %d, sec_scr_code %d num_ch_code %d",
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcenhstate_print_cell_transition_req(sys_modem_as_id_e_type wrrc_as_id, l1_cell_transition_cmd_type *l1_cmd_ptr)
{

   WRRC_MSG6_HIGH("EFACH_DEBUG:Signal act time to UL MAC %d, DL MAC %d dl_phychan %d, ul_phychan %d, hsdpa_l1_ops %d, cell_trans req_mask 0x%x",
        ...
        l1_cmd_ptr->setup_cmd.hsdpa_l1_ops,l1_cmd_ptr->setup_cmd.req_mask);

  if(l1_cmd_ptr->setup_cmd.hsdpa_l1_info != NULL)
  {
    WRRC_MSG8_HIGH("FACH_DEBUG:HS-OP:%d, REQ MASK 0X%x, HS TYPE %d TB_size_alignment %d, PSC %d HRNTI 0X%x\tBCCH-HRNTI 0X%x\tBIT-MASK 0X%x ",
    ...
    WRRC_MSG6_HIGH("CCTrchId %d HARQ NumProc %d, Partition(Valid/Invalid::0/1) %d, sec_scr_code_valid %d, sec_scr_code %d, num_ch_code %d ",
         ...
         l1_cmd_ptr->setup_cmd.hsdpa_l1_info->hs_scch_info.num_ch_code);
  }
}
/*====================================================================
...
)
{
  ...
  if(HSDSCH_COMMON_SYS_INFO_PRESENT_IN_SIB5(sib5_ptr))
  {
    ...
    {
      WRRC_MSG0_MED("EFACH:Invalid CCCH mapping in SIB5");
      status = FALSE;
    }
  }

  WRRC_MSG1_HIGH("EFACH:Get Prev Cell Status (0:F/1:T) = %d ",status);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcenhstate_print_cmac_dl_setup(sys_modem_as_id_e_type wrrc_as_id, mac_dl_config_type *mac_cmd_ptr)
{
  int i;

  WRRC_MSG8_HIGH("EFACH_DEBUG:wait_for_l1_sig %d, num_bchs %d, num_fachs, %d num_dchs, %d,bch_info:cctrch_id %d, bcch_id %d,reset_ind %d, restore_tsn %d",
        ...
        mac_cmd_ptr->mac_hs_reset_indicator, mac_cmd_ptr->mac_hs_restore_tsns);

  WRRC_MSG6_HIGH("EFACH_DEBUG:hsdpa_action %d, MAC-HS-TYPE[INVALID|HS|EHS] %d, Transition Type %d, rnti_valid %d, urnti %d, crnti %d",
    ...
    for (i=0; i< mac_cmd_ptr->efach_ndlchans;i++)
  {
     WRRC_MSG1_HIGH("EFACH_DEBUG:DL Logical Chan %d",i);
      ...
      WRRC_MSG6_HIGH("EFACH_DEBUG:Queue-ID: %d\tRB-ID %d \tRLC-ID %d\tMAC-ID %d\tChan Type %d\tRlc Mode %d",
                                           ...
                                           mac_cmd_ptr->efach_dl_logchan_info[i]->rlc_mode);
  }

    for (i=0; i< mac_cmd_ptr->efach_num_queue;i++)
  {
      WRRC_MSG5_HIGH("EFACH_DEBUG:Queue-ID: %d\tCCTrchId %d\tRelease Timer %d\tWindow_Size %d\tReset Timer %d",
                                            ...
                                            mac_cmd_ptr->efach_queue_info[i]->reset_timer);
    }
  }
...
{
  if((rrcenhstate_camped_cell_supports_e_fach(wrrc_as_id) == TRUE) &&
...
)
{
  ...
  if( rrcenhstate_nv_supports_e_fach_drx(wrrc_as_id) == FALSE)
  {
    return FALSE;
  }
  ...
  if (sib5_ptr == NULL)
  {
     WRRC_MSG0_MED("EFACH_DRX:SIB 5 not present in SIB DB for ");
     return FALSE;
  }
...
)
{
  ...
  if(hs_dsch_fach_drx != NULL)
  {
    ...
    config_ptr->l1_hsdpa_info.efach_drx_info.t_321 = rrcenhstate_get_t_321_value(wrrc_as_id,hs_dsch_fach_drx->t_321);
    config_ptr->l1_hsdpa_info.efach_drx_info.drx_cycle_length = rrcenhstate_get_drx_cycle_value(wrrc_as_id,hs_dsch_fach_drx->hs_dsch_DrxCycleFach);
    config_ptr->l1_hsdpa_info.efach_drx_info.rx_burst = rrcenhstate_get_drx_burst_value(wrrc_as_id,hs_dsch_fach_drx->hs_dsch_DrxBurstFach);
...
uint16 rrcenhstate_get_t_321_value(sys_modem_as_id_e_type wrrc_as_id, rrc_T_321 t_321)
{
  uint16 int_t_321 = 0;
...
uint8 rrcenhstate_get_drx_cycle_value(sys_modem_as_id_e_type wrrc_as_id,rrc_HS_DSCH_DrxCycleFach drx_cycle)
{
...
uint8 rrcenhstate_get_drx_burst_value(sys_modem_as_id_e_type wrrc_as_id,rrc_HS_DSCH_DrxBurstFach drx_burst)
{
...
)
{
  ...
  if(rrcenhstate_camped_cell_supports_e_fach_drx(wrrc_as_id) && 
    ...
    ((hsdpa_l1_info->efach_drx_info.can_hs_data_interrupt_drx)||
     (rrchsrach_camped_cell_supports_hsrach(wrrc_as_id))
    )
#else
    (hsdpa_l1_info->efach_drx_info.can_hs_data_interrupt_drx)
#endif
