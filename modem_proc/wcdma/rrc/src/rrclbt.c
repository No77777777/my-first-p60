...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrclbt.c#2 $    $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
03/28/16   as      Added support for loop back mode 4
...
LOCAL rrclbt_int_data_type rrclbt_int_data_sim[MAX_RRC_STACKS];
#define rrclbt_int_data rrclbt_int_data_sim[WAS_ID_M]
#else
LOCAL rrclbt_int_data_type rrclbt_int_data;
#endif
...
===========================================================================*/
static void rrclbt_init_data(sys_modem_as_id_e_type wrrc_as_id)
{                   

  /* Reset variables for each transaction */
  rrclbt_int_data.lbt_substate = RRCLBT_INITIAL;
  rrclbt_int_data.rrc_test_mode = RRCTC_NORMAL_MODE;
  rrclbt_int_data.rrc_dummy_signaling_test = FALSE;
}
...
===========================================================================*/
boolean rrclbt_is_lb_on(sys_modem_as_id_e_type wrrc_as_id )
{                   

  /* Check if RRC is in the Loopback test */
  if ( rrclbt_int_data.rrc_test_mode == RRCTC_RB_TEST_MODE ) {
    return ( TRUE );
  }
  else
  {
...
===========================================================================*/
boolean rrclbt_get_lb_mode_and_dummy_sig_enabled(sys_modem_as_id_e_type wrrc_as_id, rrc_lb_test_mode_e_type *lb_mode,
                                                  boolean  *dummy_signaling_enabled)
{                   
  boolean ans = TRUE;
  /* Check if RRC is in the Loopback test */
  if ((rrclbt_int_data.lbt_substate == RRCLBT_LB_IN_PROGRESS) &&
      (rrclbt_int_data.rrc_test_mode == RRCTC_RB_TEST_MODE)) 
  { 
    /* Set the LP test mode */
    if (rrclbt_int_data.rrc_lb_type == RRCTC_LB_MODE1)

    {
      *lb_mode = LB_TEST_MODE_1;
    }
    else if (rrclbt_int_data.rrc_lb_type == RRCTC_LB_MODE2)
    {
      *lb_mode = LB_TEST_MODE_2;
    }
    else if (rrclbt_int_data.rrc_lb_type == RRCTC_LB_MODE4)
    {
      *lb_mode = LB_TEST_MODE_4;
    }
    else
    {
      ans = FALSE;
    }
    *dummy_signaling_enabled = rrclbt_int_data.rrc_dummy_signaling_test;
  }
  else
  {
    ans = FALSE;
...
===========================================================================*/
static void rrclbt_request_channel_reconfig(sys_modem_as_id_e_type wrrc_as_id )
{
  ...
  if((cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id))!=NULL)
  {
    ...
    {
      cmd_ptr->cmd.chan_config_req.lb_test_mode = LB_TEST_MODE_4;
    }

    /* Set Dummy signaling test mode */
    cmd_ptr->cmd.chan_config_req.dummy_signaling_enabled = 
                                 rrclbt_int_data.rrc_dummy_signaling_test;
...
===========================================================================*/
static void rrclbt_send_lb_cnf(sys_modem_as_id_e_type wrrc_as_id )
{                   
  tc_cmd_type *cmd_ptr;
...
#ifdef FEATURE_DUAL_WCDMA
    cmd_ptr->cmd.rrc_test_loopback_cnf.as_id = rrc_get_as_id(wrrc_as_id);
#endif
    tc_put_cmd(cmd_ptr);
  }
  else
  {
    /* Out of memory!!  */
    WRRC_ERR0_FATAL("Can't allocate LB CNF buf!");
  }
  return;
}
...
===========================================================================*/
static void rrclbt_send_lb_rej(sys_modem_as_id_e_type wrrc_as_id )
{                   
  tc_cmd_type *cmd_ptr;
...
#ifdef FEATURE_DUAL_WCDMA
    cmd_ptr->cmd.rrc_test_loopback_rej.as_id = rrc_get_as_id(wrrc_as_id);
#endif
    tc_put_cmd(cmd_ptr);
  ...
  {
    /* Out of memory!!  */
    WRRC_ERR0_FATAL("Can't allocate LB REJ buf!");
  }
  return;
}
...
)
{
  ...
  { 
    ...
    rrclbt_send_lb_rej(wrrc_as_id);
    rrclbt_init_data(wrrc_as_id);
  }
  else
  {
    /* Inform TC that channel reconfig completed */
    rrclbt_send_lb_cnf(wrrc_as_id);

    /* Change LBT state to LP_IN_PROGRESS */
    rrclbt_int_data.lbt_substate = RRCLBT_LB_IN_PROGRESS;
  }
...
)
{
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    case RRC_TC_MODE_CHANGE:
      ...
      rrclbt_int_data.rrc_test_mode = cmd_ptr->cmd.mode_change.mode_type;
      WRRC_MSG1_HIGH("Mode Changed: %d", rrclbt_int_data.rrc_test_mode);
      break;
    ...
    case RRC_TC_TEST_LOOPBACK_IND:
      ...
      rrclbt_int_data.rrc_lb_type = cmd_ptr->cmd.test_loopback_ind.lb_mode_type;
      rrclbt_int_data.rrc_dummy_signaling_test = 
        cmd_ptr->cmd.test_loopback_ind.dummy_signaling_enabled;

      WRRC_MSG2_HIGH("Mode:%d, Dummy Signaling:%d", 
               rrclbt_int_data.rrc_lb_type, rrclbt_int_data.rrc_dummy_signaling_test);

      /* Request LLC to perform channel reconfiguration */
      if ((rrclbt_int_data.rrc_lb_type == RRCTC_LB_MODE1) || 
          (rrclbt_int_data.rrc_lb_type == RRCTC_LB_MODE2) ||
          (rrclbt_int_data.rrc_lb_type == RRCTC_LB_MODE4))
      {
        rrclbt_request_channel_reconfig(wrrc_as_id); 

        /* Change the substate */
        rrclbt_int_data.lbt_substate = RRCLBT_WAIT_FOR_CHAN_CFG_CNF;
      }
      #ifdef FEATURE_MBMS
      #error code not present
#endif
      else
      {
        ...
        rrclbt_send_lb_rej(wrrc_as_id);
        WRRC_MSG1_HIGH("Invalid LB Mode: %d", rrclbt_int_data.rrc_lb_type);
        rrclbt_init_data(wrrc_as_id);
      }
    ...
    default:
      ...
      WRRC_MSG2_HIGH("Ignoring LB event %x in substate %d",
              cmd_ptr->cmd_hdr.cmd_id, rrclbt_int_data.lbt_substate);
      break;
  }
}
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    case RRC_CHANNEL_CONFIG_CNF:
      ...
      rrclbt_process_channel_config_cnf
        (wrrc_as_id, &(cmd_ptr->cmd.chan_config_cnf) );  
      break;
...
)
{
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    ...
    case RRC_TC_TEST_LOOPBACK_IND:
      ...
      /* Store LB test type and perform the channel re-configuration */
      if (rrclbt_int_data.rrc_lb_type == cmd_ptr->cmd.test_loopback_ind.lb_mode_type &&
          rrclbt_int_data.rrc_dummy_signaling_test == cmd_ptr->cmd.test_loopback_ind.dummy_signaling_enabled)
      {
      ...
      else
      {
        ...
        if ((cmd_ptr->cmd.test_loopback_ind.lb_mode_type == RRCTC_LB_MODE1) ||
            ...
            (cmd_ptr->cmd.test_loopback_ind.lb_mode_type == RRCTC_LB_MODE4))
        {
          rrclbt_int_data.rrc_lb_type = cmd_ptr->cmd.test_loopback_ind.lb_mode_type;
          rrclbt_int_data.rrc_dummy_signaling_test = cmd_ptr->cmd.test_loopback_ind.dummy_signaling_enabled;
...
{
  WRRC_MSG1_HIGH_OPT("function rrclbt_procedure_event_handler is called in lbt_substate = rrclbt_substates_e_type_value%d",rrclbt_int_data.lbt_substate);
  rrc_crash_debug_save_proc_substates(wrrc_as_id,RRC_PROCEDURE_LBT,rrclbt_int_data.lbt_substate,cmd_ptr->cmd_hdr.cmd_id);

  /* Check the procedure substate and procedure the event accordingly */  
  switch( rrclbt_int_data.lbt_substate )
  {
    case RRCLBT_INITIAL:
      /* Call the event handler for Initial substate */
      rrclbt_initial_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;

    case RRCLBT_WAIT_FOR_CHAN_CFG_CNF:
      /* Call the event handler for waiting for chan cfg cnf substate */
      rrclbt_wfchcnf_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;
  
    case RRCLBT_LB_IN_PROGRESS:
      /* Call the event handler for LB in progress substate */
      rrclbt_lbinp_substate_event_handler(wrrc_as_id,  cmd_ptr );
      break;

    default:
      /* Invalid substate - should never happen. Clear
         all procedure vars */
      WRRC_MSG1_ERROR("Invalid LB Substate %d", rrclbt_int_data.lbt_substate);
      rrclbt_init_data(wrrc_as_id);
      break;
  }/* end switch */
}
...
===========================================================================*/
void rrclbt_init_procedure(sys_modem_as_id_e_type wrrc_as_id )
{
  rrclbt_init_data(wrrc_as_id); /* Initialize all the procedure variables */
  ...
  rrcscmgr_register_for_scn(wrrc_as_id, RRC_PROCEDURE_LBT,  /* Procedure name */
...
{
  return rrclbt_int_data.rrc_test_mode;
}
...
{
  return (uint8)rrclbt_int_data.lbt_substate;
}