...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcccm.c#3 $    $DateTime: 2020/04/29 07:36:02 $ $Author: pwbldsvc $
...
11/02/16   gv      Made changes to prevent sending new cell ind to bmc when interrat to W is in progress
05/04/16   sn      f3 density reduction changes
03/08/16   sr      Made changes to prevent Tx lock request for DCH/FACH to PCH transitions
02/15/16   gv      Made changes to prevent lock acquision for inter freq redirection in CCM
...
09/08/15   sas     Made changes to update correct LAC,PLMN info values to registered clients after UMI.
06/02/15   gv      Made changes to use TRM_ACCESS for both CS and PS signaling
05/28/15   bc      Changes to share SIBDB and ACQDB among two SUBs in a W+W solution
05/22/15   vi      Made changes to set default value of SIB 7 validity timer to 2 sec for legacy cell and 8sec for HS-FACH/EDRX cell
04/21/15   sr      TRM Unification changes from WCDMA RRC
02/25/15   sr      Made changes to update config_ptrs with sib contents after connection reject with redirection 
03/04/15   sr      Made changes to set meas substate to idle for CSFB calls in CCM
...
{\
  ...
  WRRC_MSG1_MED("DSAC: DSAC value at cell change req =%d {0=Not Present/1=Present}",rrc_ccm_cell_camping_info.curr_dsac_present);\
}
...
LOCAL  rrc_ccm_cell_camping_info_type   rrc_ccm_cell_camping_info_sim[MAX_RRC_STACKS];
/* This holds the Substate for RRC CCM */
LOCAL  rrc_ccm_substate_e_type      rrc_ccm_substate_sim[MAX_RRC_STACKS];

#define rrc_ccm_cell_camping_info rrc_ccm_cell_camping_info_sim[WAS_ID_M]
#define rrc_ccm_substate rrc_ccm_substate_sim[WAS_ID_M]
#else
/* This holds the current cell camping info for the UE */ 
...
void  
rrc_ccm_send_new_cell_ind(sys_modem_as_id_e_type wrrc_as_id, rrc_new_cell_status_e_type  new_cell_camp_status,
...
rrc_ccm_get_curr_camping_status(sys_modem_as_id_e_type wrrc_as_id, rrc_proc_e_type req_proc_id)
{
...
===========================================================================*/
void rrc_ccm_update_with_cell_loss(sys_modem_as_id_e_type wrrc_as_id)
{
 if((rrc_csp_int_data.curr_scan != RRC_CSP_WTOW_BPLMN_AUTOMATIC_SCAN)
  ...
  WRRC_MSG0_MED("DSAC/PPAC: Back up of DSAC/PPAC info during  ccm update with cell loss is done !");
...
=====================================================================================*/
rrc_freq_type rrc_ccm_calculate_ul_uarfcn(sys_modem_as_id_e_type wrrc_as_id, rrc_freq_type uarfcn_dl,rrc_plmn_identity_type plmn_id)
{
  ...
  band_class = rrc_get_frequency_band(wrrc_as_id, uarfcn_dl, plmn_id);
...
===========================================================================*/
static void rrc_ccm_log_cell_info(sys_modem_as_id_e_type wrrc_as_id)
{  
...
/* Do QXDM logging here */
  ...
  sib2_ptr = (rrc_SysInfoType2 *) rrc_sibdb_return_sib_for_cell(wrrc_as_id,
                            rrc_SIB2, 
                            plmn_id, 
                            rrc_ccm_cell_camping_info.curr_cell.cell_id);
  ...
  log_record_ptr = (WCDMA_CELL_ID_type *)WRRC_LOG_ALLOC(WCDMA_CELL_ID, log_record_length);
  ...
  if((rrc_state != RRC_STATE_DISCONNECTED) && (rrc_state != RRC_STATE_CONNECTING))
  {
    if(sib3_ptr->sib4indicator == TRUE)
    {
      ...
      {
        /* SIB4 not present */
        WRRC_MSG0_MED("SIB4 indicated but not present, using SIB3");
  ...
  if(log_record_ptr != NULL)
  {
    ...
    if (sib1_ptr != NULL)
    {
      (void)rrccsp_fill_nas_common_info(wrrc_as_id, sib1_ptr, &nas_common_info);
    ...
    log_record_ptr->ul_uarfcn = rrc_ccm_calculate_ul_uarfcn(wrrc_as_id, log_record_ptr->dl_uarfcn,plmn_id);
    WRRC_MSG2_MED("UL UARFCN : %d, DL UARFCN: %d",log_record_ptr->ul_uarfcn,log_record_ptr->dl_uarfcn);
    ...
    if(sib2_ptr != NULL)
    {
      rrc_translate_ura_id(wrrc_as_id, sib2_ptr->ura_IdentityList.elem, &ura_id);
    ...
    if(rrc_ccm_cell_camping_info.curr_dsac_present == TRUE)
    {
      ...
      WRRC_MSG1_MED("DSAC: CCM cell access for cell camping info",cell_access);
    }
    /* Update call_access */
...
SIDE EFFECTS      None.
===========================================================================*/
boolean  rrcccm_waiting_for_reselection_cnf(sys_modem_as_id_e_type wrrc_as_id)
{
...
SIDE EFFECTS      None.
===========================================================================*/
rrc_freq_type rrc_ccm_get_curr_camped_freq(sys_modem_as_id_e_type wrrc_as_id)
{
...
SIDE EFFECTS      None.
===========================================================================*/
rrc_scr_code_type rrc_ccm_get_curr_camped_psc(sys_modem_as_id_e_type wrrc_as_id)
{
...
SIDE EFFECTS      None.
===========================================================================*/
rrc_plmn_identity_type rrc_ccm_get_curr_camped_plmn(sys_modem_as_id_e_type wrrc_as_id)
{
...
SIDE EFFECTS      None.
===========================================================================*/
rrc_int_cell_id_type rrc_ccm_get_curr_camped_cell_id(sys_modem_as_id_e_type wrrc_as_id)
{
...
SIDE EFFECTS      None.
===========================================================================*/
rrc_int_csg_id_type rrc_ccm_get_curr_camped_csg_id(sys_modem_as_id_e_type wrrc_as_id)
{
...
rrc_ccm_get_curr_cell_access_status(sys_modem_as_id_e_type wrrc_as_id)
{
...
rrc_ccm_get_serv_plmn_cellid(sys_modem_as_id_e_type wrrc_as_id,rrc_int_cell_id_type *cell_id, rrc_plmn_identity_type *plmn_id)
{
  if((rrc_ccm_cell_camping_info.cell_camp_status != RRC_NEW_CELL_SELECTED) ||
...
SIDE EFFECTS      None.
===========================================================================*/
rrc_csp_acq_entry_type rrc_ccm_get_curr_cell_info(sys_modem_as_id_e_type wrrc_as_id)
{
...
SIDE EFFECTS      None
===========================================================================*/
boolean  rrc_ccm_is_reselection_in_progress(sys_modem_as_id_e_type wrrc_as_id)
{
...
===========================================================================*/
void  rrc_ccm_propagate_new_cell_to_all_procs(sys_modem_as_id_e_type wrrc_as_id, rrc_cmd_type  *cmd_ptr )
{
  ...
  if(cmd_ptr->cmd.new_cell_ind.new_cell_status == RRC_NEW_CELL_SELECTED)
  {
    ...
    */
    ...
    if(rrc_state == RRC_STATE_DISCONNECTED)
    {
      ...
      else if(rrcrce_get_rce_sub_state(wrrc_as_id)  ==  RRCRCE_WAIT_FOR_SIB7_CURRENT)
      {
   ...
   /*Restart the SIB 7 validity timer with correct value if we moved from HS-FACH cell to Non HS-FACH cell or vice versa
     Logic : Deduct the elapsed time when we read SIB 7 with the correct timer value and re-start the timer*/
   if((rrcnv_data.rrc_sib7_time_nv != 0)  &&  (rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_SIB7_EXPIRATION_TIMER) != 0))
   {
     if(rrcenhstate_camped_cell_supports_e_fach_drx(wrrc_as_id)  && rrcdata_hs_info.rrc_is_prev_cell_supports_e_drx == FALSE)
     {
       long exp_time = RRCSIB_DEFAULT_HS_SIB7_VALIDITY_TIME - (rrcnv_data.rrc_sib7_time_nv - rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_SIB7_EXPIRATION_TIMER));
       if(exp_time > 0)
       {
          WRRC_MSG1_HIGH("Prev-Curr cell mismatch -- restart sib7 validity timer for %d msec",exp_time);
          rrctmr_start_timer(wrrc_as_id,RRCTMR_SIB7_EXPIRATION_TIMER,exp_time);
       }
       rrcdata_hs_info.rrc_is_prev_cell_supports_e_drx = TRUE;
     }
     else if(!rrcenhstate_camped_cell_supports_e_fach_drx(wrrc_as_id) && rrcdata_hs_info.rrc_is_prev_cell_supports_e_drx == TRUE)
     {
       long exp_time = rrcnv_data.rrc_sib7_time_nv - (RRCSIB_DEFAULT_HS_SIB7_VALIDITY_TIME - rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_SIB7_EXPIRATION_TIMER));
       if(exp_time > 0)
       {
          WRRC_MSG1_HIGH("Prev-Curr cell mismatch -- restart sib7 validity timer for %d msec",exp_time);
          rrctmr_start_timer(wrrc_as_id,RRCTMR_SIB7_EXPIRATION_TIMER,exp_time);
  ...
  if((csp_out_cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id)) != NULL)
  {
    ...
    switch(current_rrc_state)
    {
      case RRC_STATE_CONNECTING:
        ...
        {
          WRRC_ERR2_FATAL("Cell chg type %d invalid in RRC st %d", 
                        cell_change_type, current_rrc_state); 
        }
        break;
      ...
      case RRC_STATE_CELL_FACH:
        if(cell_change_type == RRC_CHANGE_RETURN_TO_SERVICE_AREA)
        {
          ...
          ul_rlc_lc_id = rrclcm_check_ul_rlc_lc_id(wrrc_as_id, UE_LOGCHAN_DCCH, 
          ...
          if((rrclcm_get_rb_id_if_dl_lc_type_exists(wrrc_as_id, UE_LOGCHAN_TM_DCCH_DL, &rb_id) == RRCLCM_SUCCESS) && (count<MAX_RB))
          {
        ...
        else if(cell_change_type == RRC_CHANGE_CELL_FACH_TO_CELL_FACH)
        {
          ...
          csp_out_cmd_ptr->cmd.chan_config_req.release_phychls = FALSE;
          csp_out_cmd_ptr->cmd.chan_config_req.release_trchls = FALSE;
          if((rrcllc_get_toc_usage(wrrc_as_id) == TOC_FOR_OOS_WITHOUT_DCH_INFO) || 
      ...
      case RRC_STATE_DISCONNECTED:
        ...
        {
          WRRC_ERR2_FATAL("Cell Chg type %d invalid in RRC st %d", cell_change_type, current_rrc_state); 
        }
        break;
      ...
      case RRC_STATE_CELL_DCH:
        ...
        {
          WRRC_ERR1_FATAL("Cell Chg type %d rxed in CELL_DCH", cell_change_type);
        }
        break;

      default:
        WRRC_ERR1_FATAL("No Channel Config to LLC in RRC State %d", current_rrc_state);
    }
...
SIDE EFFECTS      None.
===========================================================================*/
static uecomdef_status_e_type rrc_csp_send_mm_service_ind(sys_modem_as_id_e_type wrrc_as_id)
{
...
)
{
  ...
  if ((rrc_ccm_get_serv_plmn_cellid(wrrc_as_id, &cell_id, &plmn_id) == RRC_CELL_LOSS_NO_NEW_CELL))
  {
  ...
  if(mm_buf_ptr->cmd.rrc_service_ind.service_status == MM_AS_SERVICE_AVAILABLE)
  {
    ...
    rrc_csp_band_class_type band_class = rrc_get_frequency_band(wrrc_as_id, rrc_ccm_get_curr_camped_freq(wrrc_as_id), rrc_ccm_get_curr_camped_plmn(wrrc_as_id));
    ...
    WRRC_MSG8_HIGH("DSAC/PPAC: DSAC, CS and PS cell access values are %d, %d, %d cell Access Info is %d PPAC valid, CS reg, PS reg, paging status values are %d, %d, %d, %d",
                        rrc_csp_int_data.is_dsac_valid,
                        rrc_csp_int_data.curr_cs_cell_access,
                        rrc_csp_int_data.curr_ps_cell_access,
                        rrc_csp_int_data.curr_cell_access_status,
      ...
      )
    {
          ...
          rrc_CN_InformationInfo_r6,primary_plmn_Identity))
      {
        ...
        rrc_translate_gsm_map_plmn_id 
         (wrrc_as_id, &cn_information_info_ptr->primary_plmn_Identity,
      ...
      {
        rrc_translate_gsm_map_plmn_id 
         (wrrc_as_id, &cn_information_info_ptr->plmn_Identity,
          &mm_buf_ptr->cmd.rrc_service_ind.service_info.selected_plmn.plmn_id
         );
      }
    ...
    else
    {
            ...
            WRRC_MSG3_HIGH("DSAC: Set mcc[0]=0x%x, mcc[1]=0x%x, mcc[2]=0x%x in RRC_SERVICE_IND_FROM_OTA_MSG ",
             ...
             rrc_CN_InformationInfo_r6,cn_CommonGSM_MAP_NAS_SysInfo))
    {
      ...
      if(rrc_get_lac_id_in_dch(wrrc_as_id,&(mm_buf_ptr->cmd.rrc_service_ind.service_info.gsm_map_common_info)) == FAILURE)
      {
         ...
         rrc_CN_InformationInfo_r6,cn_DomainInformationList))
    {
      ...
      do
      {
        if(rrc_ccm_fill_nas_cn_specific_info(wrrc_as_id, cn_information_info_ptr->cn_DomainInformationList.elem[idx],
      ...
      rrc_get_rac_id_in_dch(wrrc_as_id, &(cn_information_info_ptr->cn_DomainInformationList));
  ...
  {    
      ...
      )
    {
      ...
      if(primary_plmn_Id_present  == TRUE)
      {
        WRRC_MSG0_HIGH("NWS:Primary PLMN I.D present in OTA message");
        rrc_translate_gsm_map_plmn_id 
         (wrrc_as_id, primary_plmn_Identity,
          &mm_buf_ptr->cmd.rrc_service_ind.service_info.selected_plmn.plmn_id
         );
      }
      else
...
)
{
  ...
  {    
     ...
     )
    {
      ...
      {
        rrc_translate_gsm_map_plmn_id 
         (wrrc_as_id, &cn_information_info_full_ptr->plmn_Identity,
          &mm_buf_ptr->cmd.rrc_service_ind.service_info.selected_plmn.plmn_id
         );  
      }
      /* Update the current requested PLMN with the new PLMN */
      ...
      (void)rrc_csp_add_acq_entry(wrrc_as_id,&rrc_csp_int_data.curr_acq_entry, TRUE);
        ...
        cn_DomainInformationListFull))
    {
      ...
      do
      {
        if(rrc_ccm_fill_nas_cn_specific_info_full(wrrc_as_id,
                          ...
                          boolean inform_procedure_rce)
{
  ...
  {  
    ...
    WRRC_MSG3_HIGH("Post NEW_CELL_IND %d, %d, %d", 
...
===========================================================================*/
static uecomdef_status_e_type 
rrc_ccm_send_cell_change_cnf(sys_modem_as_id_e_type wrrc_as_id, rrc_cell_change_status_e_type  cell_change_status, 
...
===========================================================================*/
boolean rrc_ccm_cell_change_to_same_cell(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  WRRC_MSG4_HIGH("Cell info: curr cell id: %d curr freq: %d cur_psc:%d prev_psc:%d",
             rrc_ccm_cell_camping_info.curr_cell.cell_id,
  ...
  if( (rrc_bcch_plmns_match(wrrc_as_id, current_plmn_id, prev_plmn_id) == FALSE) ||
...
===========================================================================*/
static void rrc_ccm_handle_cell_change_success(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  rrcrce_substate_e_type  rrcrce_substate = rrcrce_get_rce_sub_state(wrrc_as_id);
  ...
  {
    if(rrc_csp_send_mm_service_ind(wrrc_as_id) == FAILURE)
    {
  ...
  if(rrc_ccm_send_cell_change_cnf(wrrc_as_id, RRC_CELL_CHANGE_SUCCESS,
                                  rrc_ccm_cell_camping_info.cell_camp_status,
                                  rrc_ccm_cell_camping_info.req_proc_id,
                                  &rrc_ccm_cell_camping_info.curr_cell)
     == FAILURE)
  {
    WRRC_ERR0_FATAL("Failure to send Cell Change Cnf");
  }
  ...
  if( (rrc_bcch_plmns_match(wrrc_as_id, current_plmn_id, 
                            prev_plmn_id) == FALSE) ||
      (rrc_ccm_cell_camping_info.curr_cell.cell_id != rrc_ccm_cell_camping_info.prev_cell.cell_id) )
  {
  ...
  else
  {
    ...
    {
      WRRC_ERR0_FATAL("Same cell/PLMN id during cell reselection");
    }
  }
  ...
  {
    if(rrcrce_get_rrc_est_cause(wrrc_as_id) != RRC_EST_EMERGENCY_CALL)
    {
      sib1_ptr = (rrc_SysInfoType1 *) rrc_sibdb_return_sib_for_srv_cell(wrrc_as_id, rrc_SIB1);
      // Compare the currently camped RAI vs the one given in EST_REQ
      if(rrcrce_check_for_rai_mismatch(wrrc_as_id, rrc_ccm_cell_camping_info.curr_cell.plmn_id,sib1_ptr))
      {
  ...
  if((rrc_ccm_cell_camping_info.cell_change_type != RRC_CHANGE_CELL_DCH_TO_CELL_FACH) &&
     ...
     && (!((rrc_ccm_cell_camping_info.cell_change_type == RRC_CHANGE_CELL_FACH_TO_CELL_FACH) &&
           (rrcrb_check_if_proc_waiting_for_new_cell_ind(wrrc_as_id, &proc_id) == TRUE )))
    )
  {
       ...
       )
    {
       ...
       */
      rrc_ccm_send_new_cell_ind(wrrc_as_id, RRC_NEW_CELL_SELECTED, 
                                rrc_ccm_cell_camping_info.req_proc_id,
                                cell_same_as_before,
                                inform_procedure_rce);
    }
    WRRC_MSG0_MED("calling BMC func");
    /* To prevent cell change ind going to CB without W RAT getting activated by NAS */
    if (rrcmcm_get_rrc_mode(wrrc_as_id) != RRC_MODE_INTERRAT_IN_PROGRESS)
    {
      rrcbmc_send_rrc_new_cell_ind_message(wrrc_as_id);
    }
  ...
  if((rrc_ccm_cell_camping_info.cell_change_type == RRC_CHANGE_CELL_FACH_TO_CELL_FACH)
     && (rrcrb_check_if_proc_waiting_for_new_cell_ind(wrrc_as_id, &proc_id) == TRUE ))
  {
    rrc_ccm_propagate_new_cell_to_waiting_proc(wrrc_as_id, proc_id, cell_same_as_before, inform_procedure_rce);
  }
  ...
  {
    rrc_csp_update_mode_band_pref_after_succ_camping(wrrc_as_id);
  }
#endif
#ifdef FEATURE_WCDMA_HS_FACH
  rrc_efach_epch_event.rrc_efach_hsrach_event = 0;
  rrc_efach_epch_event.rrc_efach_hsrach_event |= rrcenhstate_camped_cell_supports_e_fach(wrrc_as_id);
#ifdef FEATURE_WCDMA_HS_FACH_DRX
  /* Set the 3rd bit from LSB if cell supports efach drx */
  rrc_efach_epch_event.rrc_efach_hsrach_event |= ((rrcenhstate_camped_cell_supports_e_fach_drx(wrrc_as_id))<<2);
#endif
#ifdef FEATURE_WCDMA_HS_RACH
  /* Set the 4th bit from LSB if cell supports hsrach */
  rrc_efach_epch_event.rrc_efach_hsrach_event |= ((rrchsrach_camped_cell_supports_hsrach(wrrc_as_id))<<3);
#endif

  //WRRC_MSG3_HIGH("efach =%d epch =%d efach_drx = %d",rrc_efach_epch_event.efach,rrc_efach_epch_event.epch,rrc_efach_epch_event.efach_drx);

  WRRC_EVENT_REPORT_PAYLOAD(EVENT_WCDMA_CELL_EFACH_EPCH_ERACH_SUPPORT,
...
===========================================================================*/
static void rrc_ccm_handle_cell_change_failure(sys_modem_as_id_e_type wrrc_as_id,rrc_proc_e_type  proc_id, 
                                        rrc_csp_acq_entry_type  *acq_entry_ptr,
                                        rrc_cell_change_status_e_type cell_change_status)
{
  ...
  if(rrc_ccm_send_cell_change_cnf(wrrc_as_id, cell_change_status,
                                  rrc_ccm_cell_camping_info.cell_camp_status,
   ...
   */
  rrc_ccm_send_new_cell_ind(wrrc_as_id,RRC_CELL_LOSS_NO_NEW_CELL, proc_id, FALSE, FALSE);
...
rrc_ccm_update_state_change(sys_modem_as_id_e_type wrrc_as_id, rrc_cmd_type *cmd_ptr )
{
...
===========================================================================*/
static rrc_ccm_substate_e_type
rrc_ccm_wait_cell_reselection_handler(sys_modem_as_id_e_type wrrc_as_id, rrc_cmd_type  *cmd_ptr)
{
  ...
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_STATE_CHANGE_IND:
      next_substate = rrc_ccm_update_state_change(wrrc_as_id, cmd_ptr);
      break;
    ...
    case RRC_CELL_RESELECTION_CNF:
      ...
      if(cmd_ptr->cmd.cell_resel_cnf.status == FAILURE)
      {
        ...
        rrc_ccm_handle_cell_change_failure(wrrc_as_id,rrc_ccm_cell_camping_info.req_proc_id,
                                           &rrc_ccm_cell_camping_info.curr_cell,
      ...
      {
        MSG_MED("Cell change in RRC state %d", rrc_get_state(wrrc_as_id), 0, 0);

        /* Send a Cell parameter Req to L1 */
        rrc_csp_send_l1_cmd(wrrc_as_id, RRC_PROCEDURE_CCM, CPHY_CELL_PARM_REQ, &rrc_ccm_cell_camping_info.curr_cell);
      
        /* Send a Cell measurement Req to L1 */
        if((rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH)
           &&(rrccu_return_cu_substate(wrrc_as_id) == RRCCU_WAIT_FOR_CELL_UPDATE_CONFIRM)
           &&(rrc_CellUpdateCause_uplinkDataTransmission == rrccu_get_cu_cause(wrrc_as_id))
          )
        {
           ...
           rrcmeas_handle_cell_sel_meas_req_in_fach(wrrc_as_id);
        }
        else if((rrcmeas_cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id)) != NULL)
        {
        ...
        rrc_ccm_handle_cell_change_success(wrrc_as_id);

#if (!(defined(T_WINNT)))
        /* Log the state change with QXDM */
        rrc_ccm_log_cell_info(wrrc_as_id);
...
===========================================================================*/
static rrc_ccm_substate_e_type
rrc_ccm_wait_chan_config_handler(sys_modem_as_id_e_type wrrc_as_id, rrc_cmd_type  *cmd_ptr )
{
  rrc_cmd_type* rrcmeas_cmd_ptr;  
  ...
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    ...
    case RRC_CHANNEL_CONFIG_CNF:
      ...
      else /* Channel Config succedded */
      {
        ...
        if((rrcmeas_cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id)) != NULL)
        {
          rrcmeas_cmd_ptr->cmd_hdr.cmd_id = RRC_CELL_SELECTED_MEAS_REQ;
          rrcmeas_cmd_ptr->cmd.cell_selected_meas_req.gtow_ho_active = FALSE;
          rrc_put_int_cmd(wrrc_as_id, rrcmeas_cmd_ptr);
...
===========================================================================*/
static rrc_ccm_substate_e_type rrc_ccm_idle_handler(sys_modem_as_id_e_type wrrc_as_id, rrc_cmd_type  *cmd_ptr )
{
  rrc_ccm_substate_e_type  next_substate = RRC_CCM_IDLE;
  ...
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_CELL_CHANGE_REQ:
      ...
      else /* Request came from a valid Procedure ID */
      {
        ...
        if(
            (((rrcllc_get_ordered_config_status(wrrc_as_id) == OC_SET_FOR_CELL_DCH) || 
           ((rrcllc_get_ordered_config_status(wrrc_as_id) != OC_SET_FOR_DCH_FACH_TRANS) && 
          ...
          )
        {
          ...
          if(rrc_ccm_send_cell_change_cnf(wrrc_as_id, RRC_CELL_CHANGE_FAILURE_INCOMPATIBLE_STATE,
                                          rrc_ccm_cell_camping_info.cell_camp_status,
                                          cmd_ptr->cmd.cell_change_req.req_proc_id,
                                          &rrc_ccm_cell_camping_info.curr_cell)
             == FAILURE)
        ...
        else
        {
          ...
          if(cmd_ptr->cmd.cell_change_req.new_cell_camp_status == RRC_CELL_LOSS_NO_NEW_CELL)
          {
            ...
            if(rrc_ccm_cell_camping_info.cell_camp_status == RRC_NEW_CELL_SELECTED) 
            {
               ...
               */
              rrc_ccm_send_new_cell_ind(wrrc_as_id, RRC_CELL_LOSS_NO_NEW_CELL, 
                                        rrc_ccm_cell_camping_info.req_proc_id,
                                        FALSE, FALSE);
          ...
          {
            csp_sstate = rrc_get_csp_substate(wrrc_as_id);
            ...
            if( (rrc_csp_int_data.curr_scan == RRC_CSP_CELL_RESELECTION_SCAN) && 
                (rrc_bcch_plmns_match(wrrc_as_id, curr_plmnid, prev_plmnid) == TRUE) &&
                (rrc_ccm_cell_camping_info.curr_cell.cell_id == rrc_ccm_cell_camping_info.prev_cell.cell_id) )
            {
              /* Cell Reselection to the same cell - Fail it */
              WRRC_MSG0_ERROR("Cell Reselection to the same cell- failing reselection");

              rrcsib_print_cellid_psc_plmnid_from_sibdb(wrrc_as_id);
            ...
            if(cmd_ptr->cmd.cell_change_req.cell_change_type == RRC_CHANGE_CELL_RESELECTION)
            {
                 ...
                 || (csp_sstate == RRCCSP_ACCEPT_CELL_RESELECT_WT_CAMP_PENDING))
              {
               ...
               /*Delete the old cells sib based TVM before MAC is configured with new cells 
                ...
                {
                  rrctvm_delete_sib_tvm_config(wrrc_as_id);
                }
                if((cell_reselection_cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id)) != NULL)
                {
                ...
                /* Send the Internal RRC Command - LLC should directly handle it */
                rrc_put_int_cmd(wrrc_as_id, cell_reselection_cmd_ptr );
            ...
            else
            {
              ...
              if(FAILURE == rrcsib_change_serving_cell_status(wrrc_as_id, cell_selection_status, existing_serving_cell_valid, 
              ...
              if(
                  ...
                  rrc_csp_int_data.rrc_csfb_call_status && (rrc_state == RRC_STATE_DISCONNECTED)
                )
              {
                ...
                if ( rrcmeas_read_sib11(wrrc_as_id) == FAILURE )
                {
...
#endif
              {
  /* Let RRC take TRM lock as part of state transitions, recovering from connected mode OOS/RLF before sending
   * setup to WL1. This will ensure that TRM lock is taken with right priority upfront and in case of timeouts 
   * OOS can be reinitiated
   */
#ifdef FEATURE_DUAL_SIM
                if(rrc_dsds_int_data.wcdma_is_ue_in_dual_standby &&
                   (TRM_TRAFFIC != rrc_dsds_int_data.rrc_trm_priority) &&
                   /* To prevent RCR and RCE redirection procedures from getting lock.
                      RCE redirection already has lock */ 
                  ...
                  )
                {
                  ...
                  lock_req_bands.bands[0].band = SYS_BAND_CLASS_MAX;
                  wrm_lock_status = rrc_wait_for_wrm_lock(wrrc_as_id, priority, RRC_GET_LOCK_REQ_TIME(priority), lock_req_bands);
                  ...
                  if(wrm_lock_status != RRC_WRM_LOCK_ACQUIRED)
                  {
                    ...
                    rrccsp_ccm_send_rrc_initiate_cell_selection_ind(wrrc_as_id, RRC_OUT_OF_SERVICE_AREA_IND, rrc_state);
                    return RRC_CCM_IDLE; 
                  }
                }
#endif
                ...
                if(
                    (WCDMA_RRC_IDLE_FEATURES(RRC_CSFB_CALL_TIMELINE_OPT)) && 
                    rrc_csp_int_data.rrc_csfb_call_status && (rrc_state == RRC_STATE_CONNECTING)
                  )
                {
                   WRRC_MSG0_HIGH("CSFB_OPT:Skip sending CPHY_CELL_TRANSITION_REQ");
                }
                else
#endif
                {
                  rrc_csp_send_l1_cmd(wrrc_as_id, RRC_PROCEDURE_CCM, CPHY_CELL_TRANSITION_REQ, 
                                  &rrc_ccm_cell_camping_info.curr_cell);
                ...
                rrc_ccm_send_chan_config_cmd(wrrc_as_id,rrc_state,rrc_ccm_cell_camping_info.cell_change_type);
...
SIDE EFFECTS
                  None
===========================================================================*/
void rrc_ccm_update_cell_camped_status(sys_modem_as_id_e_type wrrc_as_id,rrc_new_cell_status_e_type status)
{
...
===========================================================================*/
void
rrc_ccm_update_for_gtow_handover(sys_modem_as_id_e_type wrrc_as_id,rrc_csp_acq_entry_type * acq_entry_ptr)
{
  rrc_ccm_cell_camping_info.status = STATUS_VALID;
  rrc_ccm_cell_camping_info.cell_camp_status = RRC_NEW_CELL_SELECTED;
...
===========================================================================*/
void
rrc_ccm_update_for_inter_freq_hard_handover(sys_modem_as_id_e_type wrrc_as_id,rrc_freq_type freq, rrc_scr_code_type pri_scr_code)
{
  ...
  rrc_csp_int_data.curr_acq_entry.band_class = rrc_get_frequency_band
                               (wrrc_as_id, freq, rrc_csp_int_data.curr_acq_entry.plmn_id);
#else
  ...
  /*Update CMAPI about current configured Freqs for DSI clock Tuning*/
...
===========================================================================*/
void  rrc_cell_change_manager(sys_modem_as_id_e_type wrrc_as_id, rrc_cmd_type  *cmd_ptr )
{
  
  rrc_crash_debug_save_proc_substates(wrrc_as_id, RRC_PROCEDURE_CCM,rrc_ccm_substate,cmd_ptr->cmd_hdr.cmd_id);
  ...
  else
  {
    ...
    {
      case RRC_CCM_IDLE:
        rrc_ccm_substate = rrc_ccm_idle_handler(wrrc_as_id, cmd_ptr);
        break;
      case RRC_CCM_WAIT_CHAN_CONFIG_CNF:
        rrc_ccm_substate = rrc_ccm_wait_chan_config_handler(wrrc_as_id, cmd_ptr);
        break;
      case RRC_CCM_WAIT_CELL_RESELECTION_CNF:
        rrc_ccm_substate = rrc_ccm_wait_cell_reselection_handler(wrrc_as_id, cmd_ptr);
        break;
      default:
...
===========================================================================*/
void rrcccm_force_init(sys_modem_as_id_e_type wrrc_as_id)
{
  /* The status valid is set to FALSE till a Cell Change Req is processed.
   */
  rrc_ccm_cell_camping_info.status = STATUS_INVALID_CELL_TO_BE_CAMPED;
...
NOTE
                  This function SHALL NOT send any cmd to any other procedure.
===========================================================================*/
void rrc_ccm_update_service_info(sys_modem_as_id_e_type wrrc_as_id,mm_cmd_type *service_info_ota_ptr)
{
  rrc_plmn_identity_type plmn_id;
  ...
  if(service_info_ota_ptr->cmd.rrc_service_ind.service_status== MM_AS_SERVICE_AVAILABLE)
  {
    ...
    if(rrc_csp_int_data.is_dsac_valid == TRUE)
    {
      ...
      service_info_ota_ptr->cmd.rrc_service_ind.service_info.ps_cell_access = 
                                                 rrc_csp_int_data.curr_ps_cell_access;
      WRRC_MSG3_HIGH("DSAC: DSAC, CS and PS cell access values:  %d,  %d, %d", 
                          service_info_ota_ptr->cmd.rrc_service_ind.service_info.dsac_present,
                          service_info_ota_ptr->cmd.rrc_service_ind.service_info.cs_cell_access,
                          service_info_ota_ptr->cmd.rrc_service_ind.service_info.ps_cell_access);
    }
    ...
    if(rrc_csp_int_data.is_dsac_valid == TRUE) 
    {
      ...
      rrc_ccm_cell_camping_info.curr_ps_cell_access =   
                                 service_info_ota_ptr->cmd.rrc_service_ind.service_info.ps_cell_access;
      WRRC_MSG3_HIGH("DSAC: DSAC, CS and PS A/C values in ccm cell camp info %d,  %d, %d", 
                               rrc_ccm_cell_camping_info.curr_dsac_present,
                               rrc_ccm_cell_camping_info.curr_cs_cell_access,
                               rrc_ccm_cell_camping_info.curr_ps_cell_access);
    }
    ...
    if(rrc_csp_int_data.is_ppac_valid == TRUE)
    {
      ...
      service_info_ota_ptr->cmd.rrc_service_ind.service_info.page_rsp_allowed = 
                                                rrc_csp_int_data.curr_cell_page_rsp_allowed;
      WRRC_MSG3_HIGH("PPAC: CS reg, PS reg and paging status values are %d, %d, %d", 
                          service_info_ota_ptr->cmd.rrc_service_ind.service_info.cs_reg_allowed,
    ...
    if(rrc_csp_int_data.is_ppac_valid == TRUE) 
    {
      ...
      rrc_ccm_cell_camping_info.curr_cell_page_rsp_allowed = 
                                  service_info_ota_ptr->cmd.rrc_service_ind.service_info.page_rsp_allowed;
      WRRC_MSG3_HIGH("PPAC: CS reg, PS reg and paging status values in ccm cell camp info %d,	%d, %d", 
                               rrc_ccm_cell_camping_info.curr_cell_cs_reg_allowed,
                               rrc_ccm_cell_camping_info.curr_cell_ps_reg_allowed,
                               rrc_ccm_cell_camping_info.curr_cell_page_rsp_allowed);
    }
...
NOTE
                  This function SHALL NOT send any cmd to any other procedure.
===========================================================================*/
void rrc_init_cell_change_manager(sys_modem_as_id_e_type wrrc_as_id)
{
  /* The status valid is set to FALSE till a Cell Change Req is processed.
   */
  rrc_ccm_cell_camping_info.status = STATUS_INVALID_CELL_TO_BE_CAMPED;
...
)
{
  ...
  if((cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id)) != NULL)
  {
    ...
    /* Send the Internal cmd to RRC */
    WRRC_MSG3_HIGH("Post NEW_CELL_IND %d, %d, %d", RRC_NEW_CELL_IND,
                                            cell_same_as_before, 
                                            inform_procedure_rce
                                            );
...
#endif
rrc_new_cell_status_e_type rrcccm_get_curr_cell_camped_status(sys_modem_as_id_e_type wrrc_as_id)
{
...
__attribute__((section(".uncompressible.text")))
#endif
rrc_ccm_cell_camping_status_valid_e_type rrcccm_get_camping_status(sys_modem_as_id_e_type wrrc_as_id)
{
...
__attribute__((section(".uncompressible.text")))
#endif
rrc_ccm_substate_e_type rrcccm_return_substate(sys_modem_as_id_e_type wrrc_as_id)
{
...
{
  qsh_mdump_collect_high(&rrc_ccm_substate,sizeof(rrc_ccm_substate));
  qsh_mdump_collect_high(&rrc_ccm_cell_camping_info,sizeof(rrc_ccm_cell_camping_info));
}
#endif
