...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcgpsmeas.c#3 $
...
07/01/15   rsr   Temp Changes for GPS Pos.
...
typedef struct
{
  ...
  uint32 rrc_gps_psc_stored;
#if defined (FEATURE_CGPS_UMTS_CP_WCDMA)

/* UMTS_CP_WCDMA call back function pointers */
  rrcgps_cgps_event_cb_type rrcgps_cgps_event_cb;
/*lint +e551 */
  rrcgps_cgps_ue_pos_capability_cb_type rrcgps_cgps_ue_pos_capability_cb;
#ifdef FEATURE_GANSS_SUPPORT
  boolean rrc_ganss_supported;
#endif
#endif
}rrcgps_int_data_type;

/* -----------------------------------------------------------------------
** Global Data Declarations
** ----------------------------------------------------------------------- */
/* Need to check duplication of below variables in W+W */
...
cell_info_type cell_id_dch_info_sim[MAX_RRC_STACKS];
...
** ----------------------------------------------------------------------- */
#ifdef FEATURE_DUAL_WCDMA 
LOCAL rrcgps_int_data_type rrcgps_int_data_sim[MAX_RRC_STACKS];
#define rrcgps_int_data rrcgps_int_data_sim[WAS_ID_M]
#else
LOCAL rrcgps_int_data_type rrcgps_int_data;
#endif
/*--------------------------------------------------------------------------
...
rrc_gps_send_sib_cmd(sys_modem_as_id_e_type wrrc_as_id)
{
  uint32 i;
  ...
  rrcgps_out_cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id);
  ...
  rrcgps_out_cmd_ptr->cmd.get_specific_sibs_req.freq = rrc_ccm_get_curr_camped_freq(wrrc_as_id);

  (void)l1_get_hs_psc_sib_read((uint16 *)&rrcgps_out_cmd_ptr->cmd.get_specific_sibs_req.scrambling_code, &pnpos, wrrc_as_id);
  ...
  rrc_put_int_cmd(wrrc_as_id, rrcgps_out_cmd_ptr );
  return(SUCCESS);
}
...
===========================================================================*/
static void rrc_gps_process_cell_id_req(sys_modem_as_id_e_type wrrc_as_id )
{
  ...
  rrc_csp_substate_e_type rrc_csp_sstate = rrc_get_csp_substate(wrrc_as_id);

  rrc_mode = rrcmcm_get_rrc_mode(wrrc_as_id);
  oc_status = rrcllc_get_ordered_config_status(wrrc_as_id);
  rrc_curr_state = rrc_get_state(wrrc_as_id);
  ...
  {
    rrc_send_cell_id_cnf_to_cm_in_idle(wrrc_as_id);
    handle_cell_id_req = FALSE;
  }
  else 
  if ( ( rrc_mode != RRC_MODE_ACTIVE ) ||
       ( RRC_NOT_CAMPED_ON == rrc_ccm_get_curr_camping_status(wrrc_as_id,RRC_PROCEDURE_GPS)) ||
       (RRCCSP_WT_FOR_CHAN_CFG_CNF == rrc_csp_sstate) ||
       (RRCCSP_WT_FOR_CHAN_CFG_CNF_PENDING == rrc_csp_sstate))
  {
    WRRC_MSG3_ERROR(" RRCGPS: Cannot handle RRC_CM_DCH_CELL_ID_REQUEST: mode %d, camped %d (0: camped), csp substate %d",
                rrc_mode,rrc_ccm_get_curr_camping_status(wrrc_as_id, RRC_PROCEDURE_GPS),rrc_csp_sstate);
    rrc_send_cell_id_cnf_to_cm(wrrc_as_id, NULL);
    handle_cell_id_req = FALSE;
  }
    ...
    switch(rrc_curr_state)
    {
      case RRC_STATE_CELL_DCH:
        /* send a Failure to GPS if : 
          ...
          */
        if( (oc_status != OC_NOT_SET) ||(rrcllc_get_toc_usage(wrrc_as_id) == TOC_FOR_OOS) || 
            (rrccu_get_cell_update_started_status_with_cause(wrrc_as_id,&cu_cause)== RRCCU_STARTED)) 
        {
          WRRC_MSG3_ERROR(" RRCGPS: RRC cannot handle RRC_CM_DCH_CELL_ID_REQUEST oc_status %d  TOC  OOS status  %d cu state %d ",
                                                                  oc_status,rrcllc_get_toc_usage(wrrc_as_id),rrccu_return_cu_substate(wrrc_as_id) );
          rrc_send_cell_id_cnf_to_cm(wrrc_as_id, NULL);
        }
        else if(rrcrcr_is_proc_active(wrrc_as_id))
        {
          WRRC_MSG0_ERROR(" RRCGPS: RRC cannot handle  RRC_CM_DCH_CELL_ID_REQUEST due to RCR active");
          rrc_send_cell_id_cnf_to_cm(wrrc_as_id, NULL);
        }
        else
        {
          if(rrc_gps_send_sib_cmd(wrrc_as_id) == FAILURE)
          {
            MSG_LOW("RRCGPS: Failure to send RRC_GET_SPECIFIC_SIBS_REQ into SIB ",0,0,0);
          }
          rrcgps_int_data.rrcgps_substate = RRCGPS_WAIT_FOR_SIB_CNF;
        }
      break;
...
//Temporary hack till GPS is able to not send cell-id request rrc_send_cell_id_cnf_to_cm(NULL);
      ...
      default:
       WRRC_MSG1_ERROR("RRCGPS: Received RRC_CM_DCH_CELL_ID_REQUEST in bad RRC state %d",rrc_curr_state);
      break;
    }
  }
}
...
)
{
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    ...
    case RRC_CM_DCH_CELL_ID_REQUEST:
      rrc_gps_process_cell_id_req(wrrc_as_id);
      WRRC_MSG1_HIGH("RRCGPS: Received RRC_CM_DCH_CELL_ID_REQUEST in rrc state %d",rrc_get_state(wrrc_as_id));
      break;
    ...
    default:
      ...
      WRRC_MSG2_HIGH("RRCGPS: Ignoring RRC GPS event %x in substate %d",
              cmd_ptr->cmd_hdr.cmd_id, rrcgps_int_data.rrcgps_substate);
      break;
  }
}
...
)
{
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    /*CM will request RRC to read CID in DCH state*/
    case RRC_CM_DCH_CELL_ID_REQUEST:
      WRRC_MSG0_ERROR(" RRCGPS: Did not expect a RRC_CM_DCH_CELL_ID_REQUEST while already processing one");
    break;

    case RRC_GET_SPECIFIC_SIBS_CNF:
      if(cmd_ptr->cmd.get_sibs_cnf.status == SIB_EVENT_SUCCEEDED)
      {
        rrc_send_cell_id_cnf_to_cm(wrrc_as_id, cmd_ptr );
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
        rrc_update_cell_id_in_serving_cell_metrics(wrrc_as_id,cmd_ptr->cmd.get_sibs_cnf.cell_id);
#endif
      }
      else
      {
      ...
      rrcgps_int_data.rrcgps_substate = RRCGPS_INITIAL;
      break;
    ...
    case RRC_CPHY_DL_WEAK_IND:
       ...
       rrcsib_force_sib_event_init(wrrc_as_id, RRCSIB_ABORT_DUE_SIB_READ_IN_DCH);
      break;
...
===========================================================================*/
static void rrcgps_init_data(sys_modem_as_id_e_type wrrc_as_id )
{

  /* Reset variables for each transaction */
  rrcgps_int_data.rrcgps_substate = RRCGPS_INITIAL;
  rrcgps_int_data.rrc_gps_psc_stored = RRC_GPS_INVALID_PSC;
#if defined (FEATURE_CGPS_UMTS_CP_WCDMA)
/* UMTS_CP_WCDMA call back function pointers */
  rrcgps_int_data.rrcgps_cgps_event_cb = NULL;
/*lint +e551 */
  rrcgps_int_data.rrcgps_cgps_ue_pos_capability_cb = NULL;
#ifdef FEATURE_GANSS_SUPPORT
  rrcgps_int_data.rrc_ganss_supported = FALSE;
#endif
#endif
}
...
)
{
  ...
  WRRC_MSG1_HIGH_OPT("function rrcgps_procedure_event_handler is called in rrcgps_substate = %d",rrcgps_int_data.rrcgps_substate);
  rrc_crash_debug_save_proc_substates(wrrc_as_id,RRC_PROCEDURE_GPS,rrcgps_int_data.rrcgps_substate,cmd_ptr->cmd_hdr.cmd_id);

  /* Check the procedure substate and procedure the event accordingly */  
  switch( rrcgps_int_data.rrcgps_substate )
  {
    case RRCGPS_INITIAL:
      /* Call the event handler for Initial substate */
      rrcgps_initial_substate_event_handler(wrrc_as_id, cmd_ptr );
      break;

    case RRCGPS_WAIT_FOR_SIB_CNF:
      /* Call the event handler for waiting for chan cfg cnf substate */
      rrcgps_wait_sib_cnf_substate_event_handler(wrrc_as_id, cmd_ptr );
      break;
  
     default:
      /* Invalid substate - should never happen. Clear
         all procedure vars */
     WRRC_MSG1_ERROR("RRCGPS: Invalid RRC GPS Substate %d", rrcgps_int_data.rrcgps_substate);
      rrcgps_init_data(wrrc_as_id);
      break;
  }/* end switch */
}
/*===========================================================================
...
void rrc_send_cell_id_ind_to_cm(sys_modem_as_id_e_type wrrc_as_id )
{
  ...
  RRC_CSP_GET_INT_MNC(cell_id_dch_info.plmn_id));
...
//HACK for unit testing
  ...
  cm_put_cmd(cmd_ptr);
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
  rrc_update_cell_id_in_serving_cell_metrics(wrrc_as_id,cell_id_dch_info.cell_id);
#endif
}
...
#ifdef FEATURE_DUAL_SIM
  cm_cmd_ptr->asubs_id = rrc_get_as_id(wrrc_as_id);
#endif
...
)
{
  ...
  else
  {
    ...
    cell_id_dch_info.psc  = rrcgps_int_data.rrc_gps_psc_stored;
...
===========================================================================*/
void rrcgps_set_psc_for_cell_id_req(sys_modem_as_id_e_type wrrc_as_id,uint32 psc)
{
  rrcgps_int_data.rrc_gps_psc_stored = SCR_CODE_TO_PRIMARY_SCR_CODE(psc);
}
...
===========================================================================*/
uint32 rrcgps_get_psc_for_cell_id_req(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcgps_int_data.rrc_gps_psc_stored;
}
...
/* Note: with FEATURE_MMGPS which is mutually exclusive from FEATURE_CGPS_xxx,
  ...
  boolean rrcgps_convert_assistance_data(sys_modem_as_id_e_type wrrc_as_id,
...
FUNCTION  rrcgps_register_cgps_event_cb_sub
...
boolean rrcgps_register_cgps_event_cb_sub(sys_modem_as_id_e_type wrrc_as_id, rrcgps_cgps_event_cb_type event_cb_fp )
{
  if ( event_cb_fp == NULL )
    return FALSE;

  rrcgps_int_data.rrcgps_cgps_event_cb = event_cb_fp;
  return TRUE;
}
...
{
  return rrcgps_register_cgps_event_cb_sub(SYS_MODEM_AS_ID_1, event_cb_fp);
}

/*===========================================================================

FUNCTION  rrcgps_register_cgps_ue_capability_cb_sub
...
boolean rrcgps_register_cgps_ue_pos_capability_cb_sub (sys_modem_as_id_e_type wrrc_as_id, rrcgps_cgps_ue_pos_capability_cb_type ue_cap_cb_fp)
{
  if ( ue_cap_cb_fp == NULL )
    return FALSE;

  rrcgps_int_data.rrcgps_cgps_ue_pos_capability_cb = ue_cap_cb_fp;
  return TRUE;
}
...
{
  return rrcgps_register_cgps_ue_pos_capability_cb_sub(SYS_MODEM_AS_ID_1, ue_cap_cb_fp);
}
...
DESCRIPTION
  This function is called by UECI to get the positioning capability
...
void rrcgps_get_cgps_ue_pos_capability(sys_modem_as_id_e_type wrrc_as_id,tm_umts_cp_wcdma_ue_pos_capability_struct_type *pos_capability)
{
  /* First get positioning capability from LSM */
  if(rrcgps_int_data.rrcgps_cgps_ue_pos_capability_cb == NULL)
  {
    /* This is temporary change for DUAL WCDMA, GPS support is not yet
      available for SUB-2 */
    if (wrrc_as_id == SYS_MODEM_AS_ID_1)
    {
      WRRC_ERR0_FATAL("GPS Call back function is NULL");
    }
    else
    {
      WRRC_MSG0_ERROR("GPS Call back function is NULL");
      rrcgps_int_data.rrc_ganss_supported = FALSE;
      pos_capability->ganss_supported = FALSE;
    }
  }
  else
  {
#ifdef FEATURE_DUAL_SIM
    pos_capability->as_id = rrc_get_as_id( wrrc_as_id);
#endif
    rrcgps_int_data.rrcgps_cgps_ue_pos_capability_cb(pos_capability);
#ifdef FEATURE_GANSS_SUPPORT
    rrcgps_int_data.rrc_ganss_supported = pos_capability->ganss_supported;
#endif
  }
}
...
)
{
       ...
       ue_Positioning_Measurement_v390ext))
    {
        ...
        rrc_UE_Positioning_Measurement_v390ext,measurementValidity))
      {
        ...
        if (!(RRC_CHECK_COMMON_MSG_TYPE(dcch_msg_ptr->message.u.measurementControl.u.r3.measurementControl_r3.measurementCommand,
           ...
           && (((pos_meas_validity == STATE_DCH) && (rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_CELL_FACH)) ||
           ((pos_meas_validity == ALL_STATES_BUT_DCH) && (rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_CELL_DCH))))
        {
       ...
       rrc_MeasurementCommand_setup))
  {
        ...
        rrc_MeasurementControl_r3_IEs,measurementReportingMode)))
    {
      ...
      rrcmeas_send_meas_ctrl_failure_msg(wrrc_as_id,&rrcmeas_mcf);
    ...
    if (rrcgps_verify_positioning_capability(wrrc_as_id,rpt_quan_ptr)
      == FAILURE)
    {
      /* Send Meas Ctrl Failure */
    ...
    /* Now call LSM fn to store Meas Object if FEATURE_MMGPS; call a local function to do the same if FEATURE_CGPS_xxx */
      if (rrcgps_convert_assistance_data(wrrc_as_id,&dcch_msg_ptr->message.u.measurementControl.u.r3.measurementControl_r3.measurementCommand.u.setup->u.
    ...
    if(rrcmeas_config_pos_db(wrrc_as_id,p_info_ptr, &pos_meas_validity, &rpt_mode, modify_meas_params_incl) == FALSE)
    {
       return FAILURE;   
    }
    ...
    rrc_MeasurementCommand_modify))
  {
        ...
        measurementType))
    {
      ...
      {
      /* Now call LSM fn to store Meas Object if FEATURE_MMGPS; call a local function to do the same if FEATURE_CGPS_xxx */
        if (rrcgps_convert_assistance_data(wrrc_as_id,&dcch_msg_ptr->message.u.measurementControl.u.r3.measurementControl_r3.measurementCommand.u.modify->measurementType.u.
          ue_positioning_Measurement->ue_positioning_GPS_AssistanceData,
          &(gps_meas_params->u.meas_ctrl_params.gps_assistance_data)) == FALSE)
        {
          WRRC_MSG0_ERROR("POS: Assistance Data conversion failed");
  ...
  else
  {
    ...
    rrcmeas_delete_addtl_meas(wrrc_as_id,&deleted_meas_id);
    
    rrcgps_send_lsm_cmd(wrrc_as_id,gps_meas_params);
    return SUCCESS;
  }
...
)
{
  ...
  ul_lc_id = rrclcm_check_ul_rlc_lc_id (wrrc_as_id,  UE_LOGCHAN_DCCH, rb_mode,
  ...
  * should be released by SEND_CHAIN using OSS compiler functions */
  ...
  rrcmeas_fill_meas_id(wrrc_as_id,&msg_ptr->message.u.measurementReport,lsm_meas_ptr->meas_id, POSITION_MEAS);
  ...
  if (lsm_meas_ptr->gps_measured_results_incl == TRUE)
  {
    ...
    if (rrcgps_validate_meas_results(wrrc_as_id,&lsm_meas_ptr->gps_measured_results) == FALSE )
    {
    ...
    rrcgps_set_meas_results(wrrc_as_id,&lsm_meas_ptr->gps_measured_results,
   ...
   #ifdef FEATURE_GANSS_SUPPORT
  if((rrcgps_int_data.rrc_ganss_supported == TRUE) && (lsm_meas_ptr->ganss_measured_results_incl == TRUE))
  {
      invalid_t = FALSE;
      rrcgps_fill_measured_results_ganss_incl(wrrc_as_id,msg_ptr,lsm_meas_ptr);
  }
#endif
  ...
  invalid_t=FALSE;
#ifdef FEATURE_GANSS_SUPPORT
    if(rrcgps_int_data.rrc_ganss_supported == TRUE)
    {
      rrcgps_fill_measured_results_pos_err_incl(wrrc_as_id,msg_ptr,lsm_meas_ptr);
    }
    else
  {
#endif
    RRC_MSG_COMMON_SET_BITMASK_IE_PTR(msg_ptr->message.u.measurementReport.measuredResults.u.
      ...
      if(rrcgps_validate_pos_error(wrrc_as_id,&lsm_meas_ptr->pos_error) == FALSE)
      {
        
        rrc_free(msg_ptr);
        return;
      }
    rrcgps_set_pos_error(wrrc_as_id,&lsm_meas_ptr->pos_error,
      &msg_ptr->message.u.measurementReport.measuredResults.u.
  ...
  invalid_t= FALSE;
#ifdef  FEATURE_GANSS_SUPPORT
if(rrcgps_int_data.rrc_ganss_supported)
 {
   if(FAILURE == rrcgps_fill_measured_result_pos_est_incl(wrrc_as_id,msg_ptr,lsm_meas_ptr))
   {
 ...
 {
 #endif
    if (rrcgps_validate_pos_estimate(wrrc_as_id,&lsm_meas_ptr->positioning_estimate) == FALSE)
    {
    ...
    rrcgps_set_pos_estimate(wrrc_as_id,&lsm_meas_ptr->positioning_estimate,
 ...
 if(invalid_t== TRUE)
  ...
  if (l2_ack_status == RRCSEND_L2ACK_REQUIRED)
  {
    ...
    status =  rrcsend_send_ul_sdu_with_mui(wrrc_as_id,RRC_PROCEDURE_MCMR, (void *) msg_ptr,
  ...
  {
    /* This places SDU into RLC watermark queue */
    status =  rrcsend_send_ul_sdu(wrrc_as_id,RRC_PROCEDURE_MCMR, (void *) msg_ptr,
  ...
  else
  {
    ...
    if(lsm_meas_ptr->tranfer_mode == (uint8) MODE_ACKNOWLEDGED)
    {
      rrcmeas_ext_data.rrcmeas_pending_rpt++; 
      WRRC_MSG1_HIGH("pending meas rpt = %d",rrcmeas_ext_data.rrcmeas_pending_rpt);
    }
  }
...
void rrcgps_send_lsm_cmd ( sys_modem_as_id_e_type wrrc_as_id,rrc_meas_params_struct_type* gps_meas_params_ptr )
{

 WRRC_MSG0_HIGH("POS: Sending GPS Meas Request");
  if(rrcgps_int_data.rrcgps_cgps_event_cb != NULL)
  {
    ...
    WCDMA_MEMCPY((void *)gps_meas_params_ptr2,
     ...
     /*lint -e522 */
#ifdef FEATURE_DUAL_SIM
    event_data.as_id = rrc_get_as_id( wrrc_as_id);
#endif
...
#ifdef FEATURE_GANSS_SUPPORT
    WRRC_MSG7_HIGH( "GPS : Meas id %3d, GPS AD incl %d, RefTime %d, RefLoc %d, UTC %d, Alm %d, Nav %d", 
    ...
    WRRC_MSG8_HIGH( "params_type %3d, ganss_assistance_data_incl %d, ganss_ref_time_incl %d, ganss_ref_pos_incl %d, ganss_addlono_model_incl %d, ganss_data_list_incl %d, list_cnt  %d, ganss_id_incl %d",
    ...
    WRRC_MSG8_HIGH("ganssId %d, time_mode_list_incl %d,add_navigation_models_incl %d, RealTime_Integrity_incl %d, ref_meas_info_incl %d, GANSS_Almanac_incl %d, add_utc_models_incl %d, Auxiliary_Info_incl %d",
    ...
    (void)rrcgps_int_data.rrcgps_cgps_event_cb(&event_data);
    /*lint +e522 */
  }
  else
  {
    WRRC_ERR0_FATAL("Callback func pointer is NULL");
  }
...
===========================================================================*/
static uint32 rrcgps_oss_bitstring_to_int(sys_modem_as_id_e_type wrrc_as_id, uint8 max_bytes, boolean *bitstring_failed, ASN1BitStr32 *bitstring_ptr )
{
    uint32 result = 0;
  ...
  rrc_assistance_data_struct_type* rrc_ptr )
{
    ...
    rrc_UE_Positioning_GPS_AssistanceData,ue_positioning_GPS_Almanac))
  {
    ...
    rrc_almanac->wn_a = 
      ( uint8 )rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof( uint8 ), &bs_failed, &asn1_almanac->wn_a );
        ...
        (asn1_info_ptr->n > q_Cnt)); q_Cnt++ )
    {
      ...
      rrc_info_ptr->a_Sqrt = 
        (uint32) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, &asn1_info_ptr->elem[q_Cnt].a_Sqrt );
      rrc_info_ptr->deltaI = 
        (uint16) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &asn1_info_ptr->elem[q_Cnt].deltaI );
      rrc_info_ptr->e = 
        (uint16) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &asn1_info_ptr->elem[q_Cnt].e );
      rrc_info_ptr->m0 = 
        (uint32) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, &asn1_info_ptr->elem[q_Cnt].m0 );
      rrc_info_ptr->omega0 = 
        (uint32) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, &asn1_info_ptr->elem[q_Cnt].omega0 );
      rrc_info_ptr->omega_dot = 
        (uint16) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &asn1_info_ptr->elem[q_Cnt].omegaDot );
      rrc_info_ptr->omega =
        (uint32) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, &asn1_info_ptr->elem[q_Cnt].omega );
      rrc_info_ptr->sat_health = 
        (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, &asn1_info_ptr->elem[q_Cnt].satHealth );
      rrc_info_ptr->t_oa =
        (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, &asn1_info_ptr->elem[q_Cnt].t_oa );
      rrc_info_ptr->af0 =
        (uint16) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &asn1_info_ptr->elem[q_Cnt].af0 );
      rrc_info_ptr->af1 =
        (uint16) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &asn1_info_ptr->elem[q_Cnt].af1 );
        ...
        (asn1_info_ptr->n > q_Cnt)); q_Cnt++ )
    {
      ...
      if ( asn1_info_ptr->elem[q_Cnt].m.ephemerisParameterPresent )
      {
        ...
        rrc_info_ptr->ephermeral_clock_params.a_Sqrt =
            (uint32) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.a_Sqrt );
        rrc_info_ptr->ephermeral_clock_params.af0 =
            (uint32) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.af0 );
        rrc_info_ptr->ephermeral_clock_params.af1 = 
            (uint16) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.af1 );
        rrc_info_ptr->ephermeral_clock_params.af2 =
            (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.af2 );
        rrc_info_ptr->ephermeral_clock_params.aodo =
            (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.aodo );
        rrc_info_ptr->ephermeral_clock_params.c_ic =
            (uint16) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.c_ic );
        rrc_info_ptr->ephermeral_clock_params.c_is =
            (uint16) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.c_is );
        rrc_info_ptr->ephermeral_clock_params.c_rc =
            (uint16) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.c_rc );
        rrc_info_ptr->ephermeral_clock_params.c_rs =
            (uint16) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.c_rs );
        rrc_info_ptr->ephermeral_clock_params.c_uc =
            (uint16) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.c_uc );
        rrc_info_ptr->ephermeral_clock_params.c_us =
            (uint16) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.c_us );
        rrc_info_ptr->ephermeral_clock_params.code_on_l2 =
            (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.codeOnL2 );
        rrc_info_ptr->ephermeral_clock_params.delta_n =
            (uint16) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.delta_n );
        rrc_info_ptr->ephermeral_clock_params.e =
            (uint32) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.e );
        rrc_info_ptr->ephermeral_clock_params.fit_interval =
            (boolean) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(boolean), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.fitInterval );
        rrc_info_ptr->ephermeral_clock_params.i0 =
            (uint32) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.i0 );
        rrc_info_ptr->ephermeral_clock_params.iDot =
            (uint16) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.iDot );
            rrc_info_ptr->ephermeral_clock_params.iodc =
            (uint16) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.iodc );
        rrc_info_ptr->ephermeral_clock_params.l2_pflag =
            (boolean) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(boolean), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.l2Pflag );
        rrc_info_ptr->ephermeral_clock_params.m0 =
            (uint32) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.m0 );
        rrc_info_ptr->ephermeral_clock_params.omega =
            (uint32) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.omega );
        rrc_info_ptr->ephermeral_clock_params.omega0 =
            (uint32) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.omega0 );
        rrc_info_ptr->ephermeral_clock_params.omegaDot =
            (uint32) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.omegaDot );
        rrc_info_ptr->ephermeral_clock_params.sat_health =
            (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.satHealth );
        rrc_info_ptr->ephermeral_clock_params.t_gd =
            (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.t_GD );
        rrc_info_ptr->ephermeral_clock_params.t_oc =
            (uint16) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.t_oc );
        rrc_info_ptr->ephermeral_clock_params.t_oe =
            (uint16) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.t_oe );
        rrc_info_ptr->ephermeral_clock_params.ura_index =
            (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, &asn1_info_ptr->elem[q_Cnt].ephemerisParameter.uraIndex );
      }
    ...
    if ( RRC_MSG_COMMON_BITMASK_IE_PTR(asn1_ref_time, rrc_UE_Positioning_GPS_ReferenceTime,gps_TOW_AssistList))
    {
      ...
      for ( q_Cnt = 0; ((q_Cnt < MAX_NO_OF_SATELLITES)&& (asn1_info_ptr->n > q_Cnt)); q_Cnt++ )
      {
        ...
        rrc_info_ptr->tlm_msg = 
            (uint16) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &asn1_info_ptr->elem[q_Cnt].tlm_Message );
        rrc_info_ptr->tlm_rsvd = 
            (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, &asn1_info_ptr->elem[q_Cnt].tlm_Reserved );
    ...
    rrc_UE_Positioning_GPS_AssistanceData,ue_positioning_GPS_IonosphericModel))
  {
    ...
    rrc_iono->alfa0 = (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &asn1_iono->alpha0 );
    rrc_iono->alfa1 = (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &asn1_iono->alpha1 );
    rrc_iono->alfa2 = (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &asn1_iono->alpha2 );
    rrc_iono->alfa3 = (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &asn1_iono->alpha3 );
    rrc_iono->beta0 = (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &asn1_iono->beta0 );
    rrc_iono->beta1 = (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &asn1_iono->beta1 );
    rrc_iono->beta2 = (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &asn1_iono->beta2 );
    rrc_iono->beta3 = (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &asn1_iono->beta3 );
  }
    ...
    rrc_UE_Positioning_GPS_AssistanceData,ue_positioning_GPS_UTC_Model))
  {
    ...
    rrc_utc->q_A0   = (uint32)rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint32), &bs_failed, &asn1_utc->a0);
    rrc_utc->q_A1 =   (uint32)rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint32), &bs_failed, &asn1_utc->a1);
    rrc_utc->u_DeltaTls = (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &asn1_utc->delta_t_LS );
    rrc_utc->u_DeltaTlsf = (uint8)rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &asn1_utc->delta_t_LSF);
    rrc_utc->u_DN = (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &asn1_utc->dn );
    rrc_utc->u_Tot = (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &asn1_utc->t_ot );
    rrc_utc->u_WNlsf = (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &asn1_utc->wn_lsf );
    rrc_utc->u_WNt = (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &asn1_utc->wn_t );
...
)
{
  ...
  if ( dcch_msg_ptr == NULL )
  {
    ...
    rrcmeas_send_rrc_status_msg(wrrc_as_id,&trans_id, &error_choice);
  ...
  if (rrcgps_convert_assistance_data(wrrc_as_id,&assistance_data_delievery_ptr->u.r3.assistanceDataDelivery_r3.
...
#ifdef FEATURE_GANSS_SUPPORT
  else if(rrcgps_int_data.rrc_ganss_supported)
  {
    rrcgps_fill_lsm_assistance_data(wrrc_as_id,dcch_msg_ptr,lsm_assistance_data);
  }
#endif
  ...
  rrcgps_send_lsm_cmd(wrrc_as_id,lsm_assistance_data);
...

{
  switch (lsm_pos_est_ptr->positioning_estimate)
  {
    ...
    if( rrcgps_validate_pos_estimate_choice(wrrc_as_id,lsm_pos_est_ptr) == FALSE)
    {
      return FALSE;
    }
  ...
  for (count = 0; count < MAX_POS_MEAS; count++)
  {
    if (pos_meas_id_list[count].meas_id != INVALID_MEAS_ID)
    {
      ...
      case NOT_PRESENT:
        ...
        rrcgps_send_lsm_cmd(wrrc_as_id,pos_cmd);
  ...
  for (count = 0; count < MAX_POS_MEAS; count++)
  {
    if (pos_meas_id_list[count].meas_id != INVALID_MEAS_ID)
    {
      ...
      case ALL_STATES:
        ...
        if (rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_PCH)
        {
...
)
{
  ...
  if (rrcgps_verify_positioning_capability_r5(wrrc_as_id, rpt_quan_ptr)
    == FAILURE)
  {
    /* Send Meas Ctrl Failure */
  ...
  {
  /* Now call LSM fn to store Meas Object if FEATURE_MMGPS; call a local function to do the same if FEATURE_CGPS_xxx */
    if (rrcgps_convert_assistance_data(wrrc_as_id,&up_Measurement_ptr->ue_positioning_GPS_AssistanceData,
    ...
    & rrc_UE_Positioning_Measurement_r4,measurementValidity))
  {
    ...
    /* Check if Meas needs to be restarted */
    if (((pos_meas_validity == STATE_DCH) && (rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_CELL_FACH)) ||
       ((pos_meas_validity == ALL_STATES_BUT_DCH) && (rrcmeas_ext_data.rrcmeas_current_substate == RRCMEAS_CELL_DCH)))
    {
      WRRC_MSG0_HIGH("POS: RRC val, MCM val differ");
  ...
  /* Store Pos Parameters in RRC Database */
  if(rrcmeas_config_pos_db(wrrc_as_id,pos_info_ptr, &pos_meas_validity, &rpt_mode, modify_meas_params_incl) == FALSE)
  {
    return FAILURE;
  }
  return SUCCESS;
...
)
{
  ...
  if (RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand,rrc_MeasurementCommand_r4_setup))
  {
    ...
    if(FAILURE == rrcgps_fill_mcm_info_r5(wrrc_as_id,p_info_ptr, rrc_mcm_ptr->measurementCommand.u.setup->u.
  ...
  else if (RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand,rrc_MeasurementCommand_r4_modify))
  {
    ...
    {
      if(FAILURE == rrcgps_fill_mcm_info_r5(wrrc_as_id,p_info_ptr, rrc_mcm_ptr->measurementCommand.u.modify->
...
===========================================================================*/
void rrcgps_init_procedure(sys_modem_as_id_e_type wrrc_as_id )
{
  rrcgps_init_data(wrrc_as_id); /* Initialize all the procedure variables */
  ...
  rrcscmgr_register_for_scn(wrrc_as_id, RRC_PROCEDURE_GPS,  /* Procedure name */
...
)
{
  ...
  if (RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand,rrc_MeasurementCommand_r7_setup))
  {
    ...
    if(FAILURE == rrcgps_fill_mcm_info_r7(wrrc_as_id,p_info_ptr, rrc_mcm_ptr->measurementCommand.u.setup->u.
      up_Measurement, &(gps_meas_params->u.meas_ctrl_params), transaction_id, FALSE))
    {
      return FAILURE;
    }
  ...
  else if (RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand,rrc_MeasurementCommand_r7_modify))
  {
    ...
    {
      if(FAILURE == rrcgps_fill_mcm_info_r7(wrrc_as_id,p_info_ptr, rrc_mcm_ptr->measurementCommand.u.modify->
         measurementType.u.up_Measurement, &(gps_meas_params->u.meas_ctrl_params), transaction_id,
         TRUE))
      {
        return FAILURE;
...
)
{
  ...
  if (rrcgps_verify_positioning_capability_r7(wrrc_as_id,rpt_quan_ptr)
    == FAILURE)
  {
    /* Send Meas Ctrl Failure */
    WRRC_MSG0_ERROR("POS: Pos Capability error");
    ...
    rrc_UE_Positioning_ReportingQuantity_r7,horizontalAccuracy))
  {
    ...
    /* Set the 7 bits */
    meas_ctrl_params_ptr->rpt_quan.horizontal_accuracy = (uint8)rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof( uint32 ), 
  ...
  {
  /* Now call LSM fn to store Meas Object if FEATURE_MMGPS; call a local function to do the same if FEATURE_CGPS_xxx */
    if (rrcgps_convert_assistance_data_r7(wrrc_as_id,&up_Measurement_ptr->ue_positioning_GPS_AssistanceData,
      &meas_ctrl_params_ptr->gps_assistance_data) == FALSE)
    {
      WRRC_MSG0_ERROR("POS: Assistance Data conversion failed");
...
===========================================================================*/
boolean rrcgps_convert_assistance_data_r7(sys_modem_as_id_e_type wrrc_as_id,
...
)
{
    ...
    rrc_MeasurementCommand_r8_setup))
  {
    ...
    if(FAILURE == rrcgps_fill_mcm_info_r8(wrrc_as_id,p_info_ptr, rrc_mcm_ptr->measurementCommand.u.setup->u.
      up_Measurement, &(gps_meas_params->u.meas_ctrl_params), transaction_id, FALSE))
    {
  ...
  else if (RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand,rrc_MeasurementCommand_r8_modify))
  {
    ...
    {
      if(FAILURE == rrcgps_fill_mcm_info_r8(wrrc_as_id,p_info_ptr, rrc_mcm_ptr->measurementCommand.u.modify->
         measurementType.u.up_Measurement, &(gps_meas_params->u.meas_ctrl_params), transaction_id,
         TRUE))
      {
        WRRC_MSG0_ERROR("filling gps info failed");
...
)
{
  ...
  if (rrcgps_verify_positioning_capability_r8(wrrc_as_id, rpt_quan_ptr)
    == FAILURE)
  {
    /* Send Meas Ctrl Failure */
    WRRC_MSG0_ERROR("POS: Pos Capability error");
  ...
  #ifdef FEATURE_GANSS_SUPPORT
  else if((rrcgps_int_data.rrc_ganss_supported == TRUE) && (up_Measurement_ptr->m.ue_positioning_GANSS_AssistanceDataPresent))
  {
...
if(rrcgps_int_data.rrc_ganss_supported == TRUE)
{
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(rpt_quan_ptr,rrc_UE_Positioning_ReportingQuantity_r8,verticalAccuracy))
  {
    ...
    meas_ctrl_params_ptr->rpt_quan.vertical_accuracy = (uint32) rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof( uint32 ), 
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(rpt_quan_ptr,rrc_UE_Positioning_ReportingQuantity_r8,gANSSPositioningMethods))
  {
    ...
    meas_ctrl_params_ptr->rpt_quan.ganss_pos_method = ( uint16 )rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof( uint16 ), 
  ...
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(rpt_quan_ptr,rrc_UE_Positioning_ReportingQuantity_r8,gANSSTimingOfCellWanted))
  {
    ...
    meas_ctrl_params_ptr->rpt_quan.ganss_timing_cell_wanted = ( uint32 )rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof( uint32 ), 
...
if(rrcgps_int_data.rrc_ganss_supported == TRUE)
{
  rrcgps_fill_meas_ctrl_ganss_assistance_data(wrrc_as_id,meas_ctrl_params_ptr, up_Measurement_ptr);
}
#endif  
  /* Store Pos Parameters in RRC Database */
...
)
{
  ...
  if(rrcgps_int_data.rrc_ganss_supported == TRUE)
  {
    ...
    if( (rpt_quan_ptr->m.gANSSPositioningMethodsPresent == TRUE) 
         && (verify_status != FAILURE)) 
    {
      uint16 ganss_pos_method = ( uint16 )rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof( uint16 ), 
...
*/
 ...
 void rrcgps_fill_measured_results_ganss_incl(sys_modem_as_id_e_type wrrc_as_id,rrc_UL_DCCH_Message* msg_ptr,rrc_meas_rpt_struct_type* lsm_meas_ptr)
 {
 rrc_GANSSGenericMeasurementInfo *ganss_gen_meas_info_ptr = NULL;
    ...
    rrcgps_set_bitmask_IE_ext770(wrrc_as_id,msg_ptr);
    msg_ptr->message.u.measurementReport.v390nonCriticalExtensions.laterNonCriticalExtensions.v4b0NonCriticalExtensions.
        v590NonCriticalExtensions.v5b0NonCriticalExtensions.v690NonCriticalExtensions.v770NonCriticalExtensions.
    ...
    {
      WRRC_ERR0_FATAL("Memory allocation failed");
    }
...
==================================
*/
void rrcgps_fill_measured_results_pos_err_incl(sys_modem_as_id_e_type wrrc_as_id,rrc_UL_DCCH_Message* msg_ptr,rrc_meas_rpt_struct_type *lsm_meas_ptr)
{
...
if(lsm_pos_error_ptr->error_cause == rrc_UE_Positioning_ErrorCause_assistanceDataMissing)
{
       ...
       (rrc_gps_pos_error_r7_ptr->errorReason == rrc_UE_Positioning_ErrorCause_assistanceDataMissing))
    {
      ...
      if (lsm_pos_error_ptr->addtl_assistance_data.nac_model_data_req_incl == TRUE)
      {
        ...
        {
          WRRC_ERR0_FATAL("Failed to allocate memory");
        }    
...
===============================================
*/
void rrcgps_fill_lsm_assistance_data(sys_modem_as_id_e_type wrrc_as_id,rrc_DL_DCCH_Message *dcch_msg_ptr,rrc_meas_params_struct_type *lsm_assistance_data_ptr)
{
            ...
            {             
              ...
              if(dganss_list_ptr->elem[info_list_index].m.dgansssignalInformationListPresent)
              {
                ...
                for(sig_infolist_idx = 0; (sig_infolist_idx < dganss_sig_infolist_ptr->n) && (sig_infolist_idx < MAX_GANSS_SAT);sig_infolist_idx++)
                {
                  ...
                  lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].DGANSS_corrections.dganss_info[info_list_index].
                    dganss_sig_info_list[sig_infolist_idx].iode = (uint16)rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &dganss_sig_infolist_ptr->elem[sig_infolist_idx].iode_dganss );
          ...
          if(ganss_gen_list_ptr->elem[index].m.uePositioningGANSSRealTimeIntegrityPresent)
          {
            ...
            for(rt_index=0; (rt_index < ganss_rtime_ptr->n) && (rt_index <MAX_GANSS_SAT); rt_index++)
            {
              ...
              if(ganss_rtime_ptr->elem[rt_index].m.bad_ganss_signalIdPresent)
              {
                ...
                lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].RealTime_Integrity[rt_index].
                  bad_ganss_signalId = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, &ganss_rtime_ptr->elem[rt_index].bad_ganss_signalId);
...
//CBSS -- Can take values	Real (-0.2..0.1 by step of 1/210)
              ...
              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].ref_meas_info.sat_info[sat_index].
                search_window = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &ganss_sat_info_ptr->elem[sat_index].codePhaseSearchWindow);                
        ...
        for(index =0; (index<gen_datalist_ext_ptr->n) && (index<MAX_GANSS); index++)
        {
          ...
          if(gen_datalist_ext_ptr->elem[index].m.uePositioningGANSSAddNavigationModelsPresent)
          {
            ...
            {              
              ...
              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               ...
               rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, &ganss_sat_infolist_ptr->elem[sat_index].svHealth);              

              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
                add_navigation_models.sat_info_list[sat_index].iod =
               rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &ganss_sat_infolist_ptr->elem[sat_index].iod);              
              ...
              if(ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.m.navClockModelPresent)
              {
                ...
                lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
                 add_navigation_models.sat_info_list[sat_index].ganss_clk_model.u.nav.nav_clk_model.navToc =                 
                  rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, 
                ...
                lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
                 add_navigation_models.sat_info_list[sat_index].ganss_clk_model.u.nav.nav_clk_model.navaf2 =                 
                  rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
                ...
                lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
                 add_navigation_models.sat_info_list[sat_index].ganss_clk_model.u.nav.nav_clk_model.navaf0 =                 
                  rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, 
          ...
          if(gen_datalist_ext_ptr->elem[index].m.uePositioningGANSSAddUTCModelsPresent)
          {
            ...
            if(gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.m.utcModel1Present)
            {
              ...
              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_1.utcA0 = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, 
                  &gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel1.utcA0);

              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_1.utcA1 = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, 
                  &gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel1.utcA1);

              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_1.utcA2 = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
                  &gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel1.utcA2);

              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_1.utcDeltaTls = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8),&bs_failed, 
                  &gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel1.utcDeltaTls);

              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_1.utcTot = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, 
                  &gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel1.utcTot);

              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_1.utcWNot = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, 
                  &gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel1.utcWNot);

              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_1.utcWNlsf = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
                  &gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel1.utcWNlsf);

              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_1.utcDN = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
                  &gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel1.utcDN);

              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_1.utcDeltaTlsf = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
            ...
            if(gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.m.utcModel2Present)
            {
              ...
              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_2.nA = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, 
                  &gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel2.nA);

              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_2.tauC = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, 
              ...
              if(gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel2.m.deltaUT1Present)
              {
                ...
                lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
                 add_utc_models.u.utc_model_2.b1 = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, 
                    &gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel2.deltaUT1.b1);

                lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
                 add_utc_models.u.utc_model_2.b2 = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, 
              ...
              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
                  add_utc_models.u.utc_model_2.kp = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
            ...
            if(gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.m.utcModel3Present)
            {
              ...
              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_3.utcA1wnt = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, 
                  &gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel3.utcA1wnt);

              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_3.utcA0wnt = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, 
                  &gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel3.utcA0wnt);

              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_3.utcTot = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
                  &gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel3.utcTot);

              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_3.utcWNt = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
                  &gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel3.utcWNt);

              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_3.utcDeltaTls = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
                  &gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel3.utcDeltaTls);

              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_3.utcWNlsf = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
                  &gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel3.utcWNlsf);

              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_3.utcDN = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
                  &gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel3.utcDN);

              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_3.utcDeltaTlsf = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
                  &gen_datalist_ext_ptr->elem[index].uePositioningGANSSAddUTCModels.utcModel3.utcDeltaTlsf);

              lsm_assistance_data_ptr->u.assistance_data_params.ganss_assist_data.ganss_data_list[index].
               add_utc_models.u.utc_model_3.utcStandardID = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
...
================================================
*/
void rrcgps_fill_meas_ctrl_ganss_assistance_data(sys_modem_as_id_e_type wrrc_as_id,rrc_meas_ctrl_struct_type *meas_ctrl_params_ptr,rrc_UE_Positioning_Measurement_r8 *up_Measurement_ptr)
{
boolean bs_failed = FALSE;
    ...
    & rrc_UE_Positioning_Measurement_r8,ue_positioning_GANSS_AssistanceData))
  {
                                       ...
                                       ue_positioning_GANSS_ReferenceTime))
    {
      ...
      if(ganss_assist_data->ue_positioning_GANSS_ReferenceTime.m.ganssTodUncertaintyPresent)
      {
        ...
        meas_ctrl_params_ptr->ganss_assistance_data.ganss_ref_time.ganss_tod = 
                              ( uint32 )rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof( uint32 ), 
                                       ...
                                       uePositioningGanssAddIonoModel))
    {
      ...
      meas_ctrl_params_ptr->ganss_assistance_data.ganss_addlono_model.data_id = 
        ( uint8 )rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof( uint8 ),&bs_failed,
      ...
      meas_ctrl_params_ptr->ganss_assistance_data.ganss_addlono_model.alpha_beta_param.alfa0 =
        (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed,&alpha_beta_ptr->alpha0 );
      
      meas_ctrl_params_ptr->ganss_assistance_data.ganss_addlono_model.alpha_beta_param.alfa1 = 
        (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &alpha_beta_ptr->alpha1 );
      
      meas_ctrl_params_ptr->ganss_assistance_data.ganss_addlono_model.alpha_beta_param.alfa2 = 
        (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &alpha_beta_ptr->alpha2 );
      
      meas_ctrl_params_ptr->ganss_assistance_data.ganss_addlono_model.alpha_beta_param.alfa3 = 
        (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &alpha_beta_ptr->alpha3 );
      
      meas_ctrl_params_ptr->ganss_assistance_data.ganss_addlono_model.alpha_beta_param.beta0 = 
        (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &alpha_beta_ptr->beta0 );
      
      meas_ctrl_params_ptr->ganss_assistance_data.ganss_addlono_model.alpha_beta_param.beta1 = 
        (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &alpha_beta_ptr->beta1 );
      
      meas_ctrl_params_ptr->ganss_assistance_data.ganss_addlono_model.alpha_beta_param.beta2 = 
        (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &alpha_beta_ptr->beta2 );
      
      meas_ctrl_params_ptr->ganss_assistance_data.ganss_addlono_model.alpha_beta_param.beta3 = 
        (uint8) rrcgps_oss_bitstring_to_int(wrrc_as_id,sizeof(uint8), &bs_failed, &alpha_beta_ptr->beta3 );
                                       ...
                                       ganssGenericDataList))
    {
      ...
      for(index =0; (index < ganss_gen_list_ptr->n) && (index < MAX_GANSS); index++)
      {
        ...
        if(ganss_gen_list_ptr->elem[index].m.ganssIdPresent)
        {
          rrcgps_fill_generic_data_ganss_id(wrrc_as_id,meas_ctrl_params_ptr,ganss_gen_list_ptr->elem[index].ganssId,index);
        }
        /*GANSSTimeModelsList*/
        if(ganss_gen_list_ptr->elem[index].m.ganssTimeModelsListPresent)
        {
            rrcgps_fill_generic_data_time_model_list(wrrc_as_id,meas_ctrl_params_ptr,ganss_gen_list_ptr->elem[index].ganssTimeModelsList,index);
        }
        /*UE-Positioning-DGANSSCorrections*/
        if(ganss_gen_list_ptr->elem[index].m.uePositioningDGANSSCorrectionsPresent)
        {
        ...
        {
           rrcgps_fill_generic_data_add_navigation_models(wrrc_as_id,meas_ctrl_params_ptr,ganss_gen_list_ptr->elem[index].uePositioningGANSSAddNavigationModels,index);
        }
        
        /*UE-Positioning-GANSS-RealTimeIntegrity*/
        if(ganss_gen_list_ptr->elem[index].m.uePositioningGANSSRealTimeIntegrityPresent)
        {
          rrcgps_fill_generic_data_real_time_integrity(wrrc_as_id,meas_ctrl_params_ptr,ganss_gen_list_ptr->elem[index].uePositioningGANSSRealTimeIntegrity,index);
        }
        /*UE-Positioning-GANSS-ReferenceMeasurementInfo*/
        if(ganss_gen_list_ptr->elem[index].m.uePositioningGANSSReferenceMeasurementInfoPresent)
        {
          rrcgps_fill_generic_data_ref_meas_info(wrrc_as_id,meas_ctrl_params_ptr,ganss_gen_list_ptr->elem[index].uePositioningGANSSReferenceMeasurementInfo,index);
        }
        /*UE-Positioning-GANSS-Almanac-r8*/                  
        if(ganss_gen_list_ptr->elem[index].m.uePositioningGANSSAlmanacPresent)
        {
          rrcgps_fill_generic_data_almanac_r8(wrrc_as_id,meas_ctrl_params_ptr,ganss_gen_list_ptr->elem[index].uePositioningGANSSAlmanac,index);
        }

        /*UE-Positioning-GANSS-AddUTCModels*/
        if(ganss_gen_list_ptr->elem[index].m.uePositioningGANSSAddUTCModelsPresent)
        {
          rrcgps_fill_generic_data_add_utc_models(wrrc_as_id,meas_ctrl_params_ptr,ganss_gen_list_ptr->elem[index].uePositioningGANSSAddUTCModels,index);
        }
        /*UE-Positioning-GANSS-AuxiliaryInfo*/
        if(ganss_gen_list_ptr->elem[index].m.uePositioningGANSSAuxiliaryInfoPresent)
        {
          rrcgps_fill_generic_data_aux_info(wrrc_as_id,meas_ctrl_params_ptr,ganss_gen_list_ptr->elem[index].uePositioningGANSSAuxiliaryInfo,index);
        }
      }
    }    
  }
...
boolean rrcgps_fill_measured_result_pos_est_incl(sys_modem_as_id_e_type wrrc_as_id,rrc_UL_DCCH_Message* msg_ptr,rrc_meas_rpt_struct_type *lsm_meas_ptr)
 {
 //rrc_UE_Positioning_PositionEstimateInfo_v770ext_r7 *rrc_gps_pos_est_ptr = NULL;
  ...
  if(lsm_meas_ptr->positioning_estimate.positioning_estimate == GANSS_REFERENCE_TIME_ONLY)
  {
    ...
    if(rrcgps_validate_pos_estimate_r7(wrrc_as_id,&lsm_meas_ptr->positioning_estimate) == FALSE)
    {
    ...
    rrcgps_set_pos_estimate_r7(wrrc_as_id,&lsm_meas_ptr->positioning_estimate,
...
void rrcgps_set_bitmask_IE_ext770(sys_modem_as_id_e_type wrrc_as_id,rrc_UL_DCCH_Message* msg_ptr)
{
       if(!RRC_CHECK_MSG_TYPE_IE(msg_ptr->message.u.measurementReport,v390nonCriticalExtensionsPresent))
       {
           RRC_MSG_COMMON_SET_BITMASK_IE_TYPE2(msg_ptr->message.u.measurementReport,v390nonCriticalExtensions);
...
===========================================================================*/
void rrcgps_fill_generic_data_ganss_id(sys_modem_as_id_e_type wrrc_as_id,rrc_meas_ctrl_struct_type *meas_ctrl_params_ptr,uint8 ganss_id,uint8 index)
{
...
===========================================================================*/
void rrcgps_fill_generic_data_time_model_list(sys_modem_as_id_e_type wrrc_as_id,rrc_meas_ctrl_struct_type *meas_ctrl_params_ptr,rrc_GANSSTimeModelsList ganssTimeModelsList,uint8 index)
{
   rrc_GANSSTimeModelsList *ganss_time_modlist_ptr =NULL;
   uint8 time_mode_idx = 0;
...
===========================================================================*/
void rrcgps_fill_generic_data_add_navigation_models(sys_modem_as_id_e_type wrrc_as_id,rrc_meas_ctrl_struct_type *meas_ctrl_params_ptr,rrc_UE_Positioning_GANSS_AddNavigationModels uePositioningGANSSAddNavigationModels,uint8 index)
{
  rrc_Ganss_Sat_Info_AddNavList *ganss_sat_infolist_ptr = NULL;
          uint8 sat_index = 0;
          boolean bs_failed = FALSE;
          ...
          {              
            ...
            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.
              sat_info_list[sat_index].sv_health = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
               &ganss_sat_infolist_ptr->elem[sat_index].svHealth);              

            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.
              sat_info_list[sat_index].iod = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, 
            ...
            if(ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.m.navClockModelPresent)
            {
              ...
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.
                sat_info_list[sat_index].ganss_clk_model.u.nav.nav_clk_model.navToc = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), 
                 &bs_failed,&ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.navClockModel.navToc);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.
                sat_info_list[sat_index].ganss_clk_model.u.nav.nav_clk_model.navaf2 = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), 
                  &bs_failed, &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.navClockModel.navaf2);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.
                sat_info_list[sat_index].ganss_clk_model.u.nav.nav_clk_model.navaf1 = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), 
                  &bs_failed,&ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.navClockModel.navaf1);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.
                sat_info_list[sat_index].ganss_clk_model.u.nav.nav_clk_model.navaf0 = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), 
                  &bs_failed, &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.navClockModel.navaf0);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.
                sat_info_list[sat_index].ganss_clk_model.u.nav.nav_clk_model.navTgd = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8),
            ...
            if(ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.m.cnavClockModelPresent)
            {
              ...
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_clk_model.u.cnav.cnav_clk_model.cnavToc = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16),&bs_failed,
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.cnavToc);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_clk_model.u.cnav.cnav_clk_model.cnavTop = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed,
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.cnavTop);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_clk_model.u.cnav.cnav_clk_model.cnavURA0 = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8),&bs_failed,
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.cnavURA0);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_clk_model.u.cnav.cnav_clk_model.cnavURA1 = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed,
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.cnavURA1);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_clk_model.u.cnav.cnav_clk_model.cnavURA2 = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8),&bs_failed,
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.cnavURA2);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_clk_model.u.cnav.cnav_clk_model.cnavAf2 = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16),&bs_failed,
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.cnavAf2);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_clk_model.u.cnav.cnav_clk_model.cnavAf1 = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed,
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.cnavAf1);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_clk_model.u.cnav.cnav_clk_model.cnavAf0 =rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, 
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.cnavAf0);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_clk_model.u.cnav.cnav_clk_model.cnavTgd = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, 
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.cnavTgd);
              ...
              if(ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.m.cnavISCl1cpPresent)
              {
                ...
                meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                  ganss_clk_model.u.cnav.cnav_clk_model.cnavISCl1cp = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, 
                    &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.cnavISCl1cp);                
              }
              if(ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.m.cnavISCl1cdPresent)
              {
                ...
                meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                  ganss_clk_model.u.cnav.cnav_clk_model.cnavISCl1cd = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, 
                   &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.cnavISCl1cd);
              }
              if(ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.m.cnavISCl1caPresent)
              {
                ...
                meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                  ganss_clk_model.u.cnav.cnav_clk_model.cnavISCl1ca = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, 
                    &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.cnavISCl1ca);
              ...
              if(ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.m.cnavISCl2cPresent)
              {
                ...
                meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                  ganss_clk_model.u.cnav.cnav_clk_model.cnavISCl2c = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, 
                   &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.cnavISCl2c);
              }
              if(ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.m.cnavISCl5i5Present)
              {
                ...
                meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                  ganss_clk_model.u.cnav.cnav_clk_model.cnavISCl5i5 = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, 
                   &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.cnavISCl5i5);
              }
              if(ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.m.cnavISCl5q5Present)
              {
                ...
                meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                  ganss_clk_model.u.cnav.cnav_clk_model.cnavISCl5q5 = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, 
                   &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.cnavClockModel.cnavISCl5q5);
              }               
            }
            /*GLONASSclockModel*/
            if(ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.m.glonassClockModelPresent)
            {
              ...
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_clk_model.u.glo.glonass_clk_model.gloTau = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, 
                 &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.glonassClockModel.gloTau); 

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_clk_model.u.glo.glonass_clk_model.gloGamma = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, 
                 &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.glonassClockModel.gloGamma); 
              ...
              if(ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.glonassClockModel.m.gloDeltaTauPresent)
              {
                ...
                meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                  ganss_clk_model.u.glo.glonass_clk_model.gloDeltaTau = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
                    &ganss_sat_infolist_ptr->elem[sat_index].ganssClockModel.glonassClockModel.gloDeltaTau);
              }
            }
            /*sbasClockModel currently not supported*/
            ...
            if(ganss_sat_infolist_ptr->elem[sat_index].ganssOrbitModel.m.glonassECEFPresent)
            {
              ...
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_orbit_model.glonass_ECEF.gloEn = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssOrbitModel.glonassECEF.gloEn);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_orbit_model.glonass_ECEF.gloP1 = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssOrbitModel.glonassECEF.gloP1);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_orbit_model.glonass_ECEF.gloP2 = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssOrbitModel.glonassECEF.gloP2);
              ...
              if(ganss_sat_infolist_ptr->elem[sat_index].ganssOrbitModel.glonassECEF.m.gloMPresent)
              {
                ...
                meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                  ganss_orbit_model.glonass_ECEF.gloM = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
                    &ganss_sat_infolist_ptr->elem[sat_index].ganssOrbitModel.glonassECEF.gloM);                
              }

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_orbit_model.glonass_ECEF.gloX = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, 
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssOrbitModel.glonassECEF.gloX);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_orbit_model.glonass_ECEF.gloXdot = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, 
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssOrbitModel.glonassECEF.gloXdot);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_orbit_model.glonass_ECEF.gloXdotdot = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssOrbitModel.glonassECEF.gloXdotdot);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_orbit_model.glonass_ECEF.gloY = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, 
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssOrbitModel.glonassECEF.gloY);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_orbit_model.glonass_ECEF.gloYdot = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, 
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssOrbitModel.glonassECEF.gloYdot);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_orbit_model.glonass_ECEF.gloYdotdot = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssOrbitModel.glonassECEF.gloYdotdot);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_orbit_model.glonass_ECEF.gloZ = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, 
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssOrbitModel.glonassECEF.gloZ);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_orbit_model.glonass_ECEF.gloZdot = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, 
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssOrbitModel.glonassECEF.gloZdot);

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_navigation_models.sat_info_list[sat_index].
                ganss_orbit_model.glonass_ECEF.gloZdotdot = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
                  &ganss_sat_infolist_ptr->elem[sat_index].ganssOrbitModel.glonassECEF.gloZdotdot);
...
===========================================================================*/
void rrcgps_fill_generic_data_real_time_integrity(sys_modem_as_id_e_type wrrc_as_id,rrc_meas_ctrl_struct_type *meas_ctrl_params_ptr,rrc_UE_Positioning_GANSS_RealTimeIntegrity uePositioningGANSSRealTimeIntegrity,uint8 index)
{
  rrc_UE_Positioning_GANSS_RealTimeIntegrity *ganss_rtime_ptr = NULL;
          uint8 rt_index = 0;
          boolean bs_failed = FALSE;
          ...
          for(rt_index=0; (rt_index < ganss_rtime_ptr->n) && (rt_index <MAX_GANSS_SAT); rt_index++)
          {
            ...
            if(ganss_rtime_ptr->elem[rt_index].m.bad_ganss_signalIdPresent)
            {
              ...
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].RealTime_Integrity[rt_index].
                bad_ganss_signalId = rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, 
...
===========================================================================*/
void rrcgps_fill_generic_data_ref_meas_info(sys_modem_as_id_e_type wrrc_as_id,rrc_meas_ctrl_struct_type * meas_ctrl_params_ptr,rrc_UE_Positioning_GANSS_ReferenceMeasurementInfo uePositioningGANSSReferenceMeasurementInfo,uint8 index)
{
 rrc_GANSSSatelliteInformationList *ganss_sat_info_ptr = NULL;
          uint8 sat_index = 0;
          boolean bs_failed = FALSE;
...
void rrcgps_fill_generic_data_almanac_r8(sys_modem_as_id_e_type wrrc_as_id,rrc_meas_ctrl_struct_type * meas_ctrl_params_ptr,rrc_UE_Positioning_GANSS_Almanac_r8 uePositioningGANSSAlmanac,uint8 index)
{
  boolean bs_failed = FALSE;
  meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].
            GANSS_Almanac_incl = TRUE;
          if(uePositioningGANSSAlmanac.m.alm_keplerianGLONASSPresent)
          {
            ...
            {                
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].GANSS_Almanac.almanac_set[almanac_set_index].gloAlmNA =
                rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &ganss_almanaclist_ptr->elem[almanac_set_index].gloAlmNA);  
              
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].GANSS_Almanac.almanac_set[almanac_set_index].gloAlmnA =
                rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed,&ganss_almanaclist_ptr->elem[almanac_set_index].gloAlmnA); 
              
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].GANSS_Almanac.almanac_set[almanac_set_index].gloAlmHA =
                rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed,&ganss_almanaclist_ptr->elem[almanac_set_index].gloAlmHA); 
              ...
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].GANSS_Almanac.almanac_set[almanac_set_index].gloAlmLambdaA =
                rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed, &ganss_almanaclist_ptr->elem[almanac_set_index].gloAlmLambdaA);
              ...
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].GANSS_Almanac.almanac_set[almanac_set_index].gloAlmTlambdaA =
                rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed,&ganss_almanaclist_ptr->elem[almanac_set_index].gloAlmTlambdaA);  
              ...
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].GANSS_Almanac.almanac_set[almanac_set_index].gloAlmDeltaIA =
                rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed,&ganss_almanaclist_ptr->elem[almanac_set_index].gloAlmDeltaIA);     
              ...
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].GANSS_Almanac.almanac_set[almanac_set_index].gloAkmDeltaTA =
                rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed,&ganss_almanaclist_ptr->elem[almanac_set_index].gloAkmDeltaTA);    
              ...
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].GANSS_Almanac.almanac_set[almanac_set_index].gloAlmDeltaTdotA =
                rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed,&ganss_almanaclist_ptr->elem[almanac_set_index].gloAlmDeltaTdotA);  
              ...
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].GANSS_Almanac.almanac_set[almanac_set_index].gloAlmEpsilonA =
                rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed,&ganss_almanaclist_ptr->elem[almanac_set_index].gloAlmEpsilonA); 
              ...
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].GANSS_Almanac.almanac_set[almanac_set_index].gloAlmOmegaA =
                rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed,&ganss_almanaclist_ptr->elem[almanac_set_index].gloAlmOmegaA); 
              
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].GANSS_Almanac.almanac_set[almanac_set_index].gloAlmTauA =
                rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed,&ganss_almanaclist_ptr->elem[almanac_set_index].gloAlmTauA);     
              
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].GANSS_Almanac.almanac_set[almanac_set_index].gloAlmCA =
                rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed,&ganss_almanaclist_ptr->elem[almanac_set_index].gloAlmCA);
              ...
              if(ganss_almanaclist_ptr->elem[almanac_set_index].m.gloAlmMAPresent)
              {
                ...
                meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].GANSS_Almanac.almanac_set[almanac_set_index].gloAlmMA =
                 rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed,&ganss_almanaclist_ptr->elem[almanac_set_index].gloAlmMA);                  
              }                  
            }
          }           
}
...
void rrcgps_fill_generic_data_add_utc_models(sys_modem_as_id_e_type wrrc_as_id,rrc_meas_ctrl_struct_type * meas_ctrl_params_ptr,rrc_UE_Positioning_GANSS_AddUTCModels uePositioningGANSSAddUTCModels,uint8 index)
{
boolean bs_failed = FALSE;
 meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models_incl = TRUE;
          ...
          if(uePositioningGANSSAddUTCModels.m.utcModel1Present)
          {
            ...
            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_1.utcA0 = 
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed, &(uePositioningGANSSAddUTCModels.utcModel1.utcA0));

            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_1.utcA1 = 
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel1.utcA1));

            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_1.utcA2 = 
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed, &(uePositioningGANSSAddUTCModels.utcModel1.utcA2));

            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_1.utcDeltaTls = 
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8),&bs_failed,&(uePositioningGANSSAddUTCModels.utcModel1.utcDeltaTls));

            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_1.utcTot = 
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel1.utcTot));

            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_1.utcWNot = 
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel1.utcWNot));

            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_1.utcWNlsf = 
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel1.utcWNlsf));

            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_1.utcDN = 
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel1.utcDN));

            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_1.utcDeltaTlsf = 
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel1.utcDeltaTlsf));
          ...
          if(uePositioningGANSSAddUTCModels.m.utcModel2Present)
          {
            ...
            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_2.nA = 
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel2.nA));

            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_2.tauC = 
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel2.tauC));
            ...
            if(uePositioningGANSSAddUTCModels.utcModel2.m.deltaUT1Present)
            {
              ...
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_2.b1 =
               rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel2.deltaUT1.b1));

              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_2.b2 = 
               rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint16), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel2.deltaUT1.b2));                
            }
            ...
            if(uePositioningGANSSAddUTCModels.utcModel2.m.kpPresent)
            {
              ...
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_2.kp =
               rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel2.kp));
            }
          }
          /*utcModel3*/
          if(uePositioningGANSSAddUTCModels.m.utcModel3Present)
          {
            ...
            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_3.utcA1wnt =
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel3.utcA1wnt));

            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_3.utcA0wnt =
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint32), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel3.utcA0wnt));

            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_3.utcTot =
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel3.utcTot));

            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_3.utcWNt =
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel3.utcWNt));

            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_3.utcDeltaTls =
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel3.utcDeltaTls));

            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_3.utcWNlsf =
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel3.utcWNlsf));

            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_3.utcDN =
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel3.utcDN));

            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_3.utcDeltaTlsf =
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel3.utcDeltaTlsf));

            meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].add_utc_models.u.utc_model_3.utcStandardID =
              rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed,&(uePositioningGANSSAddUTCModels.utcModel3.utcStandardID));
          }
}
...
void rrcgps_fill_generic_data_aux_info(sys_modem_as_id_e_type wrrc_as_id,rrc_meas_ctrl_struct_type * meas_ctrl_params_ptr,rrc_UE_Positioning_GANSS_AuxiliaryInfo uePositioningGANSSAuxiliaryInfo,uint8 index)
{
boolean bs_failed = FALSE;
              ...
              T_rrc_UE_Positioning_GANSS_AuxiliaryInfo_ganssID1)
          {
            ...
            for(sat_index=0;(sat_index<MAX_GANSS_SAT) && (sat_index<n_count);sat_index++)
            {
              ...
              meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].Auxiliary_Info.u.ganssID1[sat_index].signals_Available = 
               rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof(uint8), &bs_failed,&(uePositioningGANSSAuxiliaryInfo.u.
                 ganssID1->elem[sat_index].signalsAvailable));    
            }              
          }
          else
...
void rrcgps_fill_meas_ctrl_ganss_assistance_data_r9(sys_modem_as_id_e_type wrrc_as_id,rrc_meas_ctrl_struct_type *meas_ctrl_params_ptr,rrc_UE_Positioning_Measurement_r9 *up_Measurement_ptr)
{
  boolean bs_failed = FALSE;
...
FUNCTION    rrcgps_fill_generic_data_ref_meas_info_r10
...
===========================================================================*/
void rrcgps_fill_generic_data_ref_meas_info_r10(sys_modem_as_id_e_type wrrc_as_id,rrc_meas_ctrl_struct_type * meas_ctrl_params_ptr,rrc_UE_Positioning_GANSS_ReferenceMeasurementInfo_r10 uePositioningGANSSReferenceMeasurementInfo,uint8 index)
{
  rrc_GANSSSatelliteInformationList_r10 *ganss_sat_info_ptr = NULL;
  uint8 sat_index = 0;
  boolean bs_failed = FALSE;
  meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].ref_meas_info_incl = TRUE;
  ...
  /* need to add support for azimuthLSB, elevationLSB (New IEs added  in rrc_AzimuthAndElevation_r10 structue)*/
...
FUNCTION    rrcgps_fill_generic_data_almanac_r10

DESCRIPTION

 Fills almanac_r10 IE for generic data
...
void rrcgps_fill_generic_data_almanac_r10(sys_modem_as_id_e_type wrrc_as_id,rrc_meas_ctrl_struct_type * meas_ctrl_params_ptr,rrc_UE_Positioning_GANSS_Almanac_r10 uePositioningGANSSAlmanac,uint8 index)
{
  ...
  if(uePositioningGANSSAlmanac.m.alm_keplerianGLONASSPresent)
  {
    ...
    /*Need to add support for complete_Almanac_Provided sturctue (New IE added in rrc_UE_Positioning_GANSS_Almanac_r10 structue)*/
...
FUNCTION    rrcgps_fill_generic_data_time_model_list_r10
...
===========================================================================*/
void rrcgps_fill_generic_data_time_model_list_r10(sys_modem_as_id_e_type wrrc_as_id,rrc_meas_ctrl_struct_type *meas_ctrl_params_ptr,rrc_GANSSTimeModelsList_r10 ganssTimeModelsList,uint8 index)
{
  rrc_GANSSTimeModelsList_r10 *ganss_time_modlist_ptr =NULL;
  uint8 time_mode_idx = 0;
  meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].time_mode_list_incl = TRUE;
...
/*  Need to add support for deltaT  (New IE added in rrc_UE_Positioning_GANSS_TimeModel_r10 structue)*/
...
//(time_mode_idx < MAX_GANSS-1) as per spec -- should be changed below
  ...
  {            
    ...
    switch(ganss_time_modlist_ptr->elem[time_mode_idx].gnss_to_id)
    {
      case rrc_UE_Positioning_GANSS_TimeModel_r10_gnss_to_id_gps:
        meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].time_mode_list[time_mode_idx].ganss_to_id = GANSS_ID_GPS;                    
        break;
      case rrc_UE_Positioning_GANSS_TimeModel_r10_gnss_to_id_galileo:
        meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].time_mode_list[time_mode_idx].ganss_to_id = GANSS_ID_GALILEO;
        break;
      case rrc_UE_Positioning_GANSS_TimeModel_r10_gnss_to_id_qzss:
        meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].time_mode_list[time_mode_idx].ganss_to_id = GANSS_ID_QZSS;
        break;
      case rrc_UE_Positioning_GANSS_TimeModel_r10_gnss_to_id_glonass:
        meas_ctrl_params_ptr->ganss_assistance_data.ganss_data_list[index].time_mode_list[time_mode_idx].ganss_to_id = GANSS_ID_GLONASS;
        break;
      default:
        break;
...
FUNCTION    rrcgps_fill_meas_ctrl_ganss_assistance_data_r10

DESCRIPTION

 Fills meas ctrl Ganss Assistance Data
...
void rrcgps_fill_meas_ctrl_ganss_assistance_data_r10(sys_modem_as_id_e_type wrrc_as_id,rrc_meas_ctrl_struct_type *meas_ctrl_params_ptr,rrc_UE_Positioning_Measurement_r10 *up_Measurement_ptr)
{
  boolean bs_failed = FALSE;
  
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(up_Measurement_ptr,
    & rrc_UE_Positioning_Measurement_r10,ue_positioning_GANSS_AssistanceData))
  {
    rrc_UE_Positioning_GANSS_AssistanceData_r10* ganss_assist_data = NULL;
    ...
    /*UE-Positioning-GANSS-ReferenceTime*/
    if(RRC_MSG_COMMON_BITMASK_IE_PTR(ganss_assist_data, rrc_UE_Positioning_GANSS_AssistanceData_r10,
                                       ue_positioning_GANSS_ReferenceTime))
    {
      meas_ctrl_params_ptr->ganss_assistance_data.ganss_ref_time_incl = TRUE;
      ...
      /*Need to add support for ganss_Day_Cycle_Number (new ie added in  rrc_UE_Positioning_GANSS_ReferenceTime_r10 structue)*/
      meas_ctrl_params_ptr->ganss_assistance_data.ganss_ref_time.ganss_tod = 
        ganss_assist_data->ue_positioning_GANSS_ReferenceTime.ganssTod;
      ...
      if(ganss_assist_data->ue_positioning_GANSS_ReferenceTime.m.utran_ganssreferenceTimePresent)
      {
        ...
        if(ganss_assist_data->ue_positioning_GANSS_ReferenceTime.utran_ganssreferenceTime.mode.t == 
             T_rrc_UE_Positioning_GANSS_ReferenceTime_r10_mode_fdd)
        {
           meas_ctrl_params_ptr->ganss_assistance_data.ganss_ref_time.utran_ganss_ref_time.fdd_psc = 
               ganss_assist_data->ue_positioning_GANSS_ReferenceTime.utran_ganssreferenceTime.mode.u.fdd->
                                    primary_CPICH_Info.primaryScramblingCode;
    ...
    if(RRC_MSG_COMMON_BITMASK_IE_PTR(ganss_assist_data, rrc_UE_Positioning_GANSS_AssistanceData_r10,
                                       ganssGenericDataList))
    {
      rrc_GANSSGenericDataList_r10 *ganss_gen_list_ptr = NULL;
      uint8 index =0;
      ...
      for(index =0; (index < ganss_gen_list_ptr->n) && (index < MAX_GANSS); index++)
      {
        ...
        if(ganss_gen_list_ptr->elem[index].m.ganssTimeModelsListPresent)
        {
          rrcgps_fill_generic_data_time_model_list_r10(wrrc_as_id,meas_ctrl_params_ptr,ganss_gen_list_ptr->elem[index].ganssTimeModelsList,index);
        }
        /*UE-Positioning-DGANSSCorrections*/
        if(ganss_gen_list_ptr->elem[index].m.uePositioningDGANSSCorrectionsPresent)
        {
        ...
        if(ganss_gen_list_ptr->elem[index].m.uePositioningGANSSReferenceMeasurementInfoPresent)
        {
          rrcgps_fill_generic_data_ref_meas_info_r10(wrrc_as_id,meas_ctrl_params_ptr,ganss_gen_list_ptr->elem[index].uePositioningGANSSReferenceMeasurementInfo,index);
        }
        /*UE-Positioning-GANSS-Almanac-r8*/                  
        if(ganss_gen_list_ptr->elem[index].m.uePositioningGANSSAlmanacPresent)
        {
          rrcgps_fill_generic_data_almanac_r10(wrrc_as_id,meas_ctrl_params_ptr,ganss_gen_list_ptr->elem[index].uePositioningGANSSAlmanac,index);
        }
...
===========================================================================*/
boolean rrcgps_convert_assistance_data_r9(sys_modem_as_id_e_type wrrc_as_id,
...
meas_ctrl_params_ptr->rpt_quan.horizontal_accuracy = (uint8)rrcgps_oss_bitstring_to_int(wrrc_as_id, sizeof( uint32 ), 
  ...
  {
  /* Now call LSM fn to store Meas Object if FEATURE_MMGPS; call a local function to do the same if FEATURE_CGPS_xxx */
    if (rrcgps_convert_assistance_data_r9(wrrc_as_id,&up_Measurement_ptr->ue_positioning_GPS_AssistanceData,
      &meas_ctrl_params_ptr->gps_assistance_data) == FALSE)
    {
      WRRC_MSG0_ERROR("POS: Assistance Data conversion failed");
  ...
  if(rrcgps_int_data.rrc_ganss_supported)
  {
    rrcgps_fill_meas_ctrl_ganss_assistance_data_r9(wrrc_as_id,meas_ctrl_params_ptr, up_Measurement_ptr);
  }
#endif  
  /* Store Pos Parameters in RRC Database */
...
)
{
  ...
  if((rrcnv_data.rrc_meas_id_extn_support) && (TRUE == rrcmeas_check_if_meas_id_extn_is_given(wrrc_as_id,mcm_r9,&meas_id_val)))
  {
    ...
    rrc_MeasurementCommand_r9_setup))
  {
    ...
    if(FAILURE == rrcgps_fill_mcm_info_r9(wrrc_as_id,p_info_ptr,rrc_mcm_ptr->measurementCommand.u.setup->u.up_Measurement , &gps_meas_params->u.meas_ctrl_params, transaction_id, FALSE, mcm_r9))
    {
  ...
  else if (RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand,rrc_MeasurementCommand_r9_modify))
  {
    ...
    {
      if(FAILURE == rrcgps_fill_mcm_info_r9(wrrc_as_id,p_info_ptr, rrc_mcm_ptr->measurementCommand.u.modify->
         measurementType.u.up_Measurement, &gps_meas_params->u.meas_ctrl_params, transaction_id,
         TRUE, mcm_r9))
      {
...
)
{
  ...
  {
    if (rrcgps_process_mcm_r9(wrrc_as_id,msg_ptr, pos_info_ptr, meas_ctrl_tr_id) == SUCCESS)
      return SUCCESS;
    else
      return FAILURE;
  }
...
FUNCTION
  RRCGPS_CONVERT_ASSISTANCE_DATA_R10
...
===========================================================================*/
boolean rrcgps_convert_assistance_data_r10(sys_modem_as_id_e_type wrrc_as_id,
  rrc_UE_Positioning_GPS_AssistanceData_r10* asn1_ptr,
  rrc_assistance_data_struct_type* rrc_ptr )
{
  uint32 q_Cnt;
  uint8  sat_info_cnt = 0;
  ...
  /* Acq Assistance */
  if (RRC_MSG_COMMON_BITMASK_IE_PTR( asn1_ptr,
    rrc_UE_Positioning_GPS_AssistanceData_r10,ue_positioning_GPS_AcquisitionAssistance))
  {
    rrc_UE_Positioning_GPS_AcquisitionAssistance_r10 *asn1_acq_assist = 
      &asn1_ptr->ue_positioning_GPS_AcquisitionAssistance;
    rrc_gps_acq_assistance_struct_type *rrc_acq_assist = 
      &rrc_ptr->gps_acq_assistance;

    rrc_AcquisitionSatInfoList_r10 *asn1_info_ptr;
    rrc_gps_sat_info_struct_type *rrc_info_ptr;
    ...
    /* GPS - UTRAN Reference Time */
    if (RRC_MSG_COMMON_BITMASK_IE_PTR( asn1_acq_assist,
        rrc_UE_Positioning_GPS_AcquisitionAssistance_r10,utran_GPSReferenceTime))
    {
      rrc_acq_assist->gps_umts_reference_time_incl = TRUE;
      rrc_acq_assist->gps_umts_reference_time.msb_timing =
        asn1_acq_assist->utran_GPSReferenceTime.utran_GPSTimingOfCell.ms_part;
    ...
    for ( q_Cnt = 0; ((q_Cnt < MAX_NO_OF_SATELLITES)&& (asn1_info_ptr->n > q_Cnt)); q_Cnt++ )
    {
      ...
      if ( asn1_info_ptr->elem[q_Cnt].m.azimuthAndElevationPresent )
      {
      /* Need to add support for the azimuthLSB,elevationLSB new IE's added in rrc_AzimuthAndElevation_r10 structure*/
        rrc_info_ptr->azimuth_elevation_incl = TRUE;
        rrc_info_ptr->azimuth_elevation_info.azimuth =
          asn1_info_ptr->elem[q_Cnt].azimuthAndElevation.azimuth;
        rrc_info_ptr->azimuth_elevation_info.elevation =
    ...
    rrc_acq_assist->no_of_gps_sat = sat_info_cnt;
    if (RRC_MSG_COMMON_BITMASK_IE_PTR( asn1_acq_assist,
        rrc_UE_Positioning_GPS_AcquisitionAssistance_r10,ue_Positioning_GPS_ReferenceTimeUncertainty))
    {
      /*TBD*/
      MSG_LOW("Rel10 IE's TBD",0,0,0);
    }
  }
  
  /* Almanac */
  if ( RRC_MSG_COMMON_BITMASK_IE_PTR(asn1_ptr, rrc_UE_Positioning_GPS_AssistanceData_r10,
    ue_positioning_GPS_Almanac))
  {
  /*Need to add support in complete_Almanac_Provided (NEW IE added in rrc_UE_Positioning_GPS_Almanac_r10 structure) */
    rrc_UE_Positioning_GPS_Almanac_r10 *asn1_almanac =
          &asn1_ptr->ue_positioning_GPS_Almanac;
    rrc_gps_almanac_struct_type *rrc_almanac =
          &rrc_ptr->gps_almanc;
  ...
  if ( RRC_MSG_COMMON_BITMASK_IE_PTR(asn1_ptr, rrc_UE_Positioning_GPS_AssistanceData_r10,
    ue_positioning_GPS_ReferenceTime))
  {
  /*Need to add support for OSUINT8 gps_Week_Cycle_Number (New IE added in rrc_UE_Positioning_GPS_ReferenceTime_r10 sturcture )*/
    rrc_gps_reference_time_struct_type *rrc_ref_time =
       &rrc_ptr->gps_ref_time;
    rrc_UE_Positioning_GPS_ReferenceTime_r10 *asn1_ref_time =
       &asn1_ptr->ue_positioning_GPS_ReferenceTime;
    ...
    /* GPS Drift Rate */
    if ( RRC_MSG_COMMON_BITMASK_IE_PTR(asn1_ref_time, rrc_UE_Positioning_GPS_ReferenceTime_r10,
        utran_GPS_DriftRate))
    {
      rrc_ref_time->gps_drift_rate_incl = TRUE;
      rrc_ref_time->gps_drift_rate =
    ...
    {
      /*TBD*/
      MSG_LOW("Rel 10 GPS IE's TBD",0,0,0);
    }
  }
...
FUNCTION   RRCGPS_FILL_MCM_R10_INFO
...
uecomdef_status_e_type rrcgps_fill_mcm_info_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  pos_info_struct_type* pos_info_ptr,
  rrc_UE_Positioning_Measurement_r10 *up_Measurement_ptr,
  rrc_meas_ctrl_struct_type *meas_ctrl_params_ptr,
  uint8 transaction_id,
  boolean modify_meas_params_incl,
  rrc_MeasurementControl_r10 *mcm_r10
)
{
  meas_validity_enum_type pos_meas_validity = NOT_PRESENT;
  ...
  {
    rrc_UE_Positioning_GANSS_AssistanceData_r10* ganss_assist_data = &up_Measurement_ptr->ue_positioning_GANSS_AssistanceData;
    if(RRC_MSG_COMMON_BITMASK_IE_PTR(ganss_assist_data, rrc_UE_Positioning_GANSS_AssistanceData_r10,
       ganssGenericDataList))
    {
      rrc_GANSSGenericDataList_r10 *ganss_gen_list_ptr = NULL;
  ...
  {
    /*TBD*/
    MSG_LOW("Rel 10 IE's Support TBD",0,0,0);
  }
#ifdef FEATURE_GANSS_SUPPORT
  if(rrcgps_int_data.rrc_ganss_supported == TRUE)
  {
  ...
  {
    /* Now call LSM fn to store Meas Object if FEATURE_MMGPS; call a local function to do the same if FEATURE_CGPS_xxx */
    if (rrcgps_convert_assistance_data_r10(wrrc_as_id,&up_Measurement_ptr->ue_positioning_GPS_AssistanceData,
      &meas_ctrl_params_ptr->gps_assistance_data) == FALSE)
    {
      WRRC_MSG0_ERROR("POS: Assistance Data conversion failed");
  ...
  /* Check if Meas Validity present */
  if (RRC_MSG_COMMON_BITMASK_IE_PTR(up_Measurement_ptr,
  & rrc_UE_Positioning_Measurement_r10,measurementValidity))
  {
    WRRC_MSG1_HIGH("POS:Meas validity %d (0/d:1/f:2/a)", up_Measurement_ptr->measurementValidity.ue_State);
  ...
  if(rrcgps_int_data.rrc_ganss_supported)
  {
    rrcgps_fill_meas_ctrl_ganss_assistance_data_r10(wrrc_as_id,meas_ctrl_params_ptr, up_Measurement_ptr);
  }
  #endif  
  /* Store Pos Parameters in RRC Database */
...
FUNCTION   RRCGPS_PROCESS_MCM_R10

DESCRIPTION

    This function processes the rel 10 Measurement Control Message for
    A-GPS Measurements.
...
uecomdef_status_e_type rrcgps_process_mcm_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  void* pos_mcm_ptr,
  pos_info_struct_type* p_info_ptr
...
)
{
  ...
  rrc_MeasurementControl_r10_IEs* rrc_mcm_ptr = NULL; 
  ...
  rrc_MeasurementControl_r10 *mcm_r10 = NULL;
  uint8 meas_id_val = 0;
  gps_meas_params = &temp_gps;
    memset(gps_meas_params,0,sizeof(rrc_meas_params_struct_type));
  ...
  mcm_r10 = &dcch_msg_ptr->message.u.measurementControl.u.
    later_than_r3.criticalExtensions.u.later_than_r4.criticalExtensions.u.criticalExtensions.u.criticalExtensions.
    u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.u.r10;


  rrc_mcm_ptr = &mcm_r10->measurementControl_r10;
  /* Check if extended measurement is supported. If meas id extension is given then
     override the meas id given in legacy IE */
  if((rrcnv_data.rrc_meas_id_extn_support) && (TRUE == rrcmeas_check_if_meas_id_extn_is_given_r10(wrrc_as_id,mcm_r10,&meas_id_val)))
  {
    p_info_ptr->meas_id = meas_id_val;
  }
  else
    ...
    rrc_MeasurementCommand_r9_setup))
  {
    ...
    if (!(RRC_MSG_COMMON_BITMASK_IE_PTR(rrc_mcm_ptr, rrc_MeasurementControl_r10_IEs,measurementReportingMode)))
    {
      /* Send Meas Ctrl Failure */
      WRRC_MSG0_ERROR("Meas Reporting Mode missing for SETUP");
    ...
    if(FAILURE == rrcgps_fill_mcm_info_r10(wrrc_as_id,p_info_ptr,rrc_mcm_ptr->measurementCommand.u.setup->u.up_Measurement , &gps_meas_params->u.meas_ctrl_params, transaction_id, FALSE, mcm_r10))
    {
      
      return FAILURE;
    }
  } /* End of if of set up */
  else if (RRC_CHECK_COMMON_MSG_TYPE(rrc_mcm_ptr->measurementCommand,rrc_MeasurementCommand_r10_modify))
  {
    /* Addtl Meas processing later. Reporting crit stored in db later */
    gps_meas_params->u.meas_ctrl_params.meas_cmd = (uint8)RRC_MEAS_MODIFY;
    ...
    /* Check if meas type present */
    if (RRC_MSG_COMMON_BITMASK_IE_PTR(rrc_mcm_ptr->measurementCommand.u.modify,
         rrc_MeasurementCommand_r10_modify,measurementType))
    {
      if(FAILURE == rrcgps_fill_mcm_info_r10(wrrc_as_id,p_info_ptr, rrc_mcm_ptr->measurementCommand.u.modify->
         measurementType.u.up_Measurement, &gps_meas_params->u.meas_ctrl_params, transaction_id,
         TRUE, mcm_r10))
      {
...
FUNCTION   RRCMEAS_PROCESS_POS_MCM_R10
...
uecomdef_status_e_type rrcmeas_process_pos_mcm_r10
(
  sys_modem_as_id_e_type wrrc_as_id, 
  void* msg_ptr,
  pos_info_struct_type* pos_info_ptr,
...
)
{
  ...
  if (pos_info_ptr->pos_meas_cmd == RRC_MEAS_DELETE)
  {
    if (rrcgps_process_mcm_r10(wrrc_as_id,msg_ptr, pos_info_ptr, meas_ctrl_tr_id) == SUCCESS)
      return SUCCESS;
    else
      return FAILURE;
  }
