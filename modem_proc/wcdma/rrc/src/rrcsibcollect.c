...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcsibcollect.c#2 $    $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------
05/26/16   vs      Changes to print sib19 and sib11bis instead of printing sib31, when received from OTA
05/20/16   sn      Changes to ignore duplicate first segment
05/04/16   sn      f3 density reduction changes
04/06/16   sn      Added support for logging SIB events which are recevied in out of order
...
rrcsib_collect_data_type rrcsib_collect_data_sim[MAX_RRC_STACKS];
#define rrcsib_collect_data rrcsib_collect_data_sim[WAS_ID_M]
#else
rrcsib_collect_data_type rrcsib_collect_data;
#endif
/* -----------------------------------------------------------------------
...
)
{
  ...
  {
    sib = rrcsib_map_extension_sib_to_internal_sibtype(wrrc_as_id,0,sfn_prime);
  }
  else
  {
    /* Copy the received sib type. */
    sib = rrcsib_map_asn1_sibtype_to_int_sibtype(wrrc_as_id,seg->sib_Type);
  }
  ...
  if((sib < (rrc_MAX_SIB - 1)) && (sib_events_ptr->event[index].tsib[sib].sib_needed))
  {
    ...
    status = rrcsib_process_sib_seg_in_tsib_optim_db(wrrc_as_id,sib,index);
...
/* Copy the received sib type. */
  ...
  */
  ...
  if((sib < (rrc_MAX_SIB - 1)) && (sib_events_ptr->event[index].tsib[sib].sib_needed))
  {
    ...
    if(sib_events_ptr->event[index].tsib[sib].seg_index != (MAX_SEG_INDEX_FOR_A_SIB + 1))
    {
      WRRC_MSG1_MED("Dup. 1st seg. for sib %d", sib);
      return(SEG_PROC_DUPLICATE);
    }
    /* We have received the first segment of a SIB. We need to copy
    the bits into the SIB events database. First store
...
)
{
  ...
  if(seg->sib_Type == (uint32)rrc_SIB_Type_extensionType)
  {
    sib = rrcsib_map_extension_sib_to_internal_sibtype(wrrc_as_id,seg->segmentIndex, sfn_prime);
  }
  else
  {
    /* Copy the received sib type. */
  ...
  if((sib < (rrc_MAX_SIB - 1)) && (sib_events_ptr->event[index].tsib[sib].sib_needed))
  {
    ...
    {
      status = rrcsib_store_sib_seg_in_tsib_optim_db(wrrc_as_id,sib,
                                          sfn_prime,
                                          seg->segmentIndex,
      ...
      {
        WRRC_MSG3_HIGH("Gap in Seg Seq sib# %d seg# %d status %d", rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,sib), seg->segmentIndex,status);
        status = SEG_PROC_FAILED;
      }
      return( status );
    }
...
)
{
  ...
  rrcsib_collect_data.curr_sfn = msg->sfn_Prime;
#endif
  switch( msg->payload.t )
  {
    ...
    case( T_rrc_SystemInformation_BCH_payload_firstSegment ):
      ...
      WRRC_MSG0_MED("First Segment chosen");
      /* Process the First Segment */
      seg_status = process_first_seg(wrrc_as_id,  &(msg->payload.u.firstSegment), index, msg->sfn_Prime );
      if(seg_status == SEG_PROC_COMPLETE)
      {
        /*For the case where last segment in TSIB and first segment is received later*/
        rrc_log_complete_sib(wrrc_as_id,msg->payload.u.firstSegment.sib_Type,
                             0,
                             msg->sfn_Prime);
    ...
    case( T_rrc_SystemInformation_BCH_payload_subsequentSegment ):
      ...
      seg_status = process_subsequent_seg(wrrc_as_id,  &(msg->payload.u.subsequentSegment), 
                                           index, msg->sfn_Prime );
      if(seg_status == SEG_PROC_COMPLETE)
      {
        rrc_log_complete_sib(wrrc_as_id,msg->payload.u.subsequentSegment.sib_Type,
                             msg->payload.u.subsequentSegment.segmentIndex,
                             msg->sfn_Prime);
        status = SIBS_COMPLETED;
      }
      break;
    ...
    case( T_rrc_SystemInformation_BCH_payload_lastSegmentShort ):
      ...
      seg_status = process_short_last_seg(wrrc_as_id,  &(msg->payload.u.lastSegmentShort),
                                           index, msg->sfn_Prime );
         ...
         )
      {
        ...
        if(msg->payload.u.lastSegmentShort.sib_Type == rrc_SIB_Type_extensionType)
        {
          sib_int_type = rrcsib_map_extension_sib_to_internal_sibtype(wrrc_as_id,msg->payload.u.lastSegmentShort.segmentIndex,msg->sfn_Prime);
          WRRC_MSG2_HIGH("Received SIB %d, Sfn prime %d", sib_int_type, msg->sfn_Prime);
        }
        else
        {
        ...
        rrc_log_complete_sib(wrrc_as_id,msg->payload.u.lastSegmentShort.sib_Type,
    ...
    case( T_rrc_SystemInformation_BCH_payload_lastAndFirst ):
      ...
      seg_status = process_short_last_seg(wrrc_as_id,  &(msg->payload.u.lastAndFirst.lastSegmentShort), 
                                           index, msg->sfn_Prime );
      /* Since this is the last segment, if segment processing is succesful,
      a SIB has been completely received. */
      if((seg_status == SEG_PROC_SUCCEEDED )
      ...
      {
#ifdef FEATURE_RRC_SIB_DEBUG
        if(msg->payload.u.lastAndFirst.lastSegmentShort.sib_Type == rrc_SIB_Type_extensionType)
        {
          sib_int_type = rrcsib_map_extension_sib_to_internal_sibtype(wrrc_as_id,msg->payload.u.lastAndFirst.lastSegmentShort.segmentIndex,msg->sfn_Prime);
          WRRC_MSG2_HIGH("Received SIB %d, Sfn prime %d", sib_int_type, msg->sfn_Prime);
        }
        else
        {
...
#endif /* FEATURE_RRC_SIB_DEBUG */
        rrc_log_complete_sib(wrrc_as_id,msg->payload.u.lastAndFirst.lastSegmentShort.sib_Type,
                             msg->payload.u.lastAndFirst.lastSegmentShort.segmentIndex,
                             msg->sfn_Prime);
        status = SIBS_COMPLETED;
      }
      /* Then process the First Segment */
      seg_status = process_short_first_seg(wrrc_as_id, &(msg->payload.u.lastAndFirst.firstSegment),
                                            index, msg->sfn_Prime );
      if(seg_status == SEG_PROC_COMPLETE)
      {
        status = SIBS_COMPLETED;
    ...
    case( T_rrc_SystemInformation_BCH_payload_lastAndComplete ):
      ...
      /* First process the last segment */
      seg_status = process_short_last_seg(wrrc_as_id,  &(msg->payload.u.lastAndComplete.lastSegmentShort),
                                           index, msg->sfn_Prime );
      ...
      {
#ifdef FEATURE_RRC_SIB_DEBUG
        if(msg->payload.u.lastAndComplete.lastSegmentShort.sib_Type == rrc_SIB_Type_extensionType)
        {
          sib_int_type = rrcsib_map_extension_sib_to_internal_sibtype(wrrc_as_id,msg->payload.u.lastAndComplete.lastSegmentShort.segmentIndex,msg->sfn_Prime);
          WRRC_MSG2_HIGH("Received SIB %d, Sfn prime %d", sib_int_type, msg->sfn_Prime);
        }
        else
        {
...
#endif /* FEATURE_RRC_SIB_DEBUG */
        rrc_log_complete_sib(wrrc_as_id,msg->payload.u.lastAndComplete.lastSegmentShort.sib_Type,
      ...
      {
        seg_status = process_short_complete_sib(wrrc_as_id,  temp_node->data, index, msg->sfn_Prime );
        ...
        {
#ifdef FEATURE_RRC_SIB_DEBUG
          if(((rrc_CompleteSIBshort*)(temp_node->data))->sib_Type == rrc_SIB_Type_extensionType)
          {
            sib_int_type = rrcsib_map_extension_sib_to_internal_sibtype(wrrc_as_id,0,msg->sfn_Prime);
            WRRC_MSG2_HIGH("Received SIB %d, Sfn prime %d", sib_int_type, msg->sfn_Prime);
          }
          else
          {
...
#endif /* FEATURE_RRC_SIB_DEBUG */
          rrc_log_complete_sib(wrrc_as_id,((rrc_CompleteSIBshort*)(temp_node->data))->sib_Type,0,msg->sfn_Prime);
          status = SIBS_COMPLETED;
        }
    ...
    and a First segment included in this message */
      ...
      /* First process the last segment */
      seg_status = process_short_last_seg(wrrc_as_id,  &(msg->payload.u.lastAndCompleteAndFirst.lastSegmentShort),
                                     index, msg->sfn_Prime );
      ...
      {
#ifdef FEATURE_RRC_SIB_DEBUG
        if(msg->payload.u.lastAndCompleteAndFirst.lastSegmentShort.sib_Type == rrc_SIB_Type_extensionType)
        {
          sib_int_type = rrcsib_map_extension_sib_to_internal_sibtype(wrrc_as_id,msg->payload.u.lastAndCompleteAndFirst.lastSegmentShort.segmentIndex,msg->sfn_Prime);
          WRRC_MSG2_HIGH("Received SIB %d, Sfn prime %d", sib_int_type, msg->sfn_Prime);
        }
        else
        {
...
#endif /* FEATURE_RRC_SIB_DEBUG */
        rrc_log_complete_sib(wrrc_as_id,msg->payload.u.lastAndCompleteAndFirst.lastSegmentShort.sib_Type,
      ...
      /* Then Process the First segment */
      seg_status = process_short_first_seg(wrrc_as_id,  &(msg->payload.u.lastAndCompleteAndFirst.firstSegment),
                                            index, msg->sfn_Prime);
      if(seg_status == SEG_PROC_COMPLETE)
      {
        status = SIBS_COMPLETED;
    ...
    case( T_rrc_SystemInformation_BCH_payload_completeAndFirst ):
      ...
      /* Then Process the First segment */
      seg_status = process_short_first_seg(wrrc_as_id,  &(msg->payload.u.completeAndFirst.firstSegment),
                                            index, msg->sfn_Prime);
      if(seg_status == SEG_PROC_COMPLETE)
      {
        rrc_log_complete_sib(wrrc_as_id,msg->payload.u.completeAndFirst.firstSegment.sib_Type,0,msg->sfn_Prime);
        status = SIBS_COMPLETED;
      }
      temp_node = NULL;
      list_ptr = NULL;
    ...
    case( T_rrc_SystemInformation_BCH_payload_completeSIB ):
      ...
      seg_status = process_complete_sib(wrrc_as_id, &(msg->payload.u.completeSIB), index, msg->sfn_Prime);
      ...
      {
#ifdef FEATURE_RRC_SIB_DEBUG
        if(msg->payload.u.completeSIB.sib_Type == rrc_SIB_Type_extensionType)
        {
          sib_int_type = rrcsib_map_extension_sib_to_internal_sibtype(wrrc_as_id,0,msg->sfn_Prime);
          WRRC_MSG2_HIGH("Received SIB %d, Sfn prime %d", sib_int_type, msg->sfn_Prime);
        }
...
#endif /* FEATURE_RRC_SIB_DEBUG */
        rrc_log_complete_sib(wrrc_as_id,msg->payload.u.completeSIB.sib_Type,0,msg->sfn_Prime);
        status = SIBS_COMPLETED;
      }
      break;
    ...
    case( T_rrc_SystemInformation_BCH_payload_lastSegment ):
      ...
      seg_status = process_last_seg(wrrc_as_id,  &(msg->payload.u.lastSegment), index, msg->sfn_Prime);
  ...
  if( (WCDMA_RRC_IDLE_FEATURES(RRC_ISS_FOR_SIB_SEGMENTS)) && 
      ...
      ( seg_status  == SEG_PROC_FAILED ||
        seg_status  == SEG_PROC_DUPLICATE ||
        seg_status  == SEG_PROC_NONE)
      )
  {
    status = SIBS_FAILED_DUPLICATE;
  }
...
)
{
  ...
  memset(rrcsib_collect_data.tsib_opt_db_look_up_table,FALSE,rrc_MAX_SIB);
}
/*===========================================================================
...
)
{
  ...
  for(i = 0;i<MAX_TEMP_SIB_SEGMENTS_TO_HOLD;i++)
  {
    if((rrcsib_collect_data.tsib_optimization_db[i].sib_type == rrc_sib_type)&&
       (rrcsib_collect_data.tsib_optimization_db[i].index_in_use == TRUE))
    {
      rrcsib_collect_data.tsib_optimization_db[i].index_in_use = FALSE;
      rrcsib_collect_data.tsib_optimization_db[i].seg_data_length = 0;
      rrcsib_collect_data.tsib_optimization_db[i].seg_index = 0;
  ...
  rrcsib_collect_data.tsib_opt_db_look_up_table[rrc_sib_type] = FALSE;
}
/*===========================================================================
...
)
{
  ...
  for(i = 0;i<MAX_TEMP_SIB_SEGMENTS_TO_HOLD;i++)
  {
    if((rrcsib_collect_data.tsib_optimization_db[i].index_in_use == TRUE)&&
        ...
        (rrcsib_collect_data.tsib_optimization_db[i].sib_seg_type == sib_seg_type)
      )
    {
    ...
    else if((rrcsib_collect_data.tsib_optimization_db[i].index_in_use == FALSE)&&
...
)
{
  ...
  */
  ...
  else
  {
    ...
    index = rrcsib_find_free_index_in_tsib_optim_db(wrrc_as_id,rrc_sib_type,
                                            seg_index,
                                            sib_seg_type);
    ...
    else
    {
      ...
      WRRC_MSG2_HIGH("Store sib seg=%d in tsib opti db index=%d",rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,rrc_sib_type),index);
      /*Found a free index in tsib optimization db
        continue storing rexd sib segment in tsib 
        optimization db*/
      rrcsib_collect_data.tsib_optimization_db[index].index_in_use = TRUE;
      rrcsib_collect_data.tsib_optimization_db[index].sib_type = rrc_sib_type;
      rrcsib_collect_data.tsib_optimization_db[index].sib_seg_type = sib_seg_type;
      rrcsib_collect_data.tsib_optimization_db[index].seg_data_length = seg_data_length;
      rrcsib_collect_data.tsib_optimization_db[index].sfn_prime = sfn_prime;
      rrcsib_collect_data.tsib_optimization_db[index].seg_index = seg_index;
  
      /* Now copy the actual data bits. Use the bit copy function.
         Destination is the sib data pointer inside the sib events database.
         Source is the received segment's data pointer.  Destination offset
         is the next bit after the total received bits so far. */
      dest = (void *) rrcsib_collect_data.tsib_optimization_db[index].seg_data_ptr;
      src = seg_data_ptr;
...
{
  return(rrcsib_collect_data.tsib_opt_db_look_up_table[rrc_sib_type]);
}
/*===========================================================================
...
)
{
  ...
  if(rrcsib_is_sib_seg_in_tsib_optim_db(wrrc_as_id,rrc_sib_type) == TRUE)
  {
    void *src;         /* Source pointer for the b_copy function */
    void *dest;        /* Destination pointer for the b_copy function */
    ...
    WRRC_MSG1_HIGH("Sib segment for sib type =%d, found in tsib optim db ",rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,rrc_sib_type));
    ...
    do
    {
      ...
      for(i = 0;i<MAX_TEMP_SIB_SEGMENTS_TO_HOLD;i++)
      {
           ...
           (rrcsib_collect_data.tsib_optimization_db[i].sib_type == rrc_sib_type))
        {
          ...
          switch(rrcsib_collect_data.tsib_optimization_db[i].sib_seg_type)
          {
            ...
            case firstSegment_chosen:
              WRRC_MSG1_HIGH("Concatinating first segment for sib %d ", rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,rrc_sib_type));
              search_continue_flag = TRUE;
              /* Now copy the actual data bits. Use the bit copy function.
              Destination is the sib data pointer inside the sib events database.
              Source is the received segment's data pointer. */
              dest = (void *)sib_events_ptr->event[index].tsib[rrc_sib_type].sib_data_ptr;
              src = (void *)rrcsib_collect_data.tsib_optimization_db[i].seg_data_ptr;
            
              b_copy( src,  /* Source Pointer */
                      0,    /* Source Offset is always 0 since we copy all the received bits */
                      dest, /* Destination Pointer */
                      0,    /* Destination Offset is 0 since this is the first segment */
                      rrcsib_collect_data.tsib_optimization_db[i].seg_data_length/* Length of the first segment is fixed */
                    );
            
              /* Note the current length of this SIB */
              sib_events_ptr->event[index].tsib[rrc_sib_type].sib_tot_length = rrcsib_collect_data.tsib_optimization_db[i].seg_data_length;
            ...
            case subsequentSegment_chosen:
              if( sib_events_ptr->event[index].tsib[rrc_sib_type].seg_index == 
                  (uint32)(rrcsib_collect_data.tsib_optimization_db[i].seg_index - 1))
              {
                WRRC_MSG2_HIGH("Concatinating Subsequent segment index=%d for SIB type=%d",rrcsib_collect_data.tsib_optimization_db[i].seg_index,rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,rrc_sib_type));
                search_continue_flag = TRUE;
                dest = (void *)sib_events_ptr->event[index].tsib[rrc_sib_type].sib_data_ptr;
                src = (void *)rrcsib_collect_data.tsib_optimization_db[i].seg_data_ptr;
                /* Ok to typecast to word here since maximum value for length is 3552 */
                ...
                sib_events_ptr->event[index].tsib[rrc_sib_type].sib_tot_length = 
                 (sib_events_ptr->event[index].tsib[rrc_sib_type].sib_tot_length + 
                 FIXED_SIB_DATA_LENGTH_BITS);
                sib_events_ptr->event[index].tsib[rrc_sib_type].seg_index = rrcsib_collect_data.tsib_optimization_db[i].seg_index;
                /*Clear Tsib db index right here*/
                rrcsib_collect_data.tsib_optimization_db[i].index_in_use = FALSE;
                rrcsib_collect_data.tsib_optimization_db[i].seg_data_length = 0;
                rrcsib_collect_data.tsib_optimization_db[i].seg_index = 0;
              ...
              case lastSegmentShort_chosen:
                if(sib_events_ptr->event[index].tsib[rrc_sib_type].seg_index == 
                  (uint32)(rrcsib_collect_data.tsib_optimization_db[i].seg_index - 1))
                {
                  WRRC_MSG2_HIGH("Concatinating Last segment index=%d for SIB type=%d",rrcsib_collect_data.tsib_optimization_db[i].seg_index,rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,rrc_sib_type));
                  search_continue_flag = FALSE;
                  dest = (void *)sib_events_ptr->event[index].tsib[rrc_sib_type].sib_data_ptr;
                  src = (void *)rrcsib_collect_data.tsib_optimization_db[i].seg_data_ptr;
                  /* Ok to typecast to word here since maximum value for length is 3552 */
                  ...
                  b_copy( src,  /* Source Pointer */
                          ...
                          dest, /* Destination Pointer */
                          dest_offset,  /* Destination Offset */
                          rrcsib_collect_data.tsib_optimization_db[i].seg_data_length/* Length of the subsequent segment is fixed */
                        );
                  ...
                  sib_events_ptr->event[index].tsib[rrc_sib_type].sib_tot_length = 
                   (sib_events_ptr->event[index].tsib[rrc_sib_type].sib_tot_length + 
                   rrcsib_collect_data.tsib_optimization_db[i].seg_data_length);
                  ...
                  WRRC_MSG1_HIGH("Received SIB %d",rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,rrc_sib_type));
                  rrcsib_clear_a_sib_from_tsib_optim_db(wrrc_as_id,rrc_sib_type);
                }
                break;
              ...
              default:
                WRRC_MSG1_HIGH("Invalid seg type =%d",rrcsib_collect_data.tsib_optimization_db[i].sib_seg_type);
                break;
          }
        }
      }
...
)
{
  ...
  {
    rrcsib_collect_data.deferred_sib_info.deferred_sib_seg_db[i].index_in_use = FALSE;
    rrcsib_collect_data.deferred_sib_info.deferred_sib_seg_db[i].seg_data_length = 0;
    rrcsib_collect_data.deferred_sib_info.deferred_sib_seg_db[i].seg_index = 0;
    rrcsib_collect_data.deferred_sib_info.deferred_sib_seg_db[i].sfn_prime = 0;
    rrcsib_collect_data.deferred_sib_info.deferred_sib_seg_db[i].sib_seg_type = noSegment_chosen;
    rrcsib_collect_data.deferred_sib_info.deferred_sib_seg_db[i].sib_type = rrc_MAX_SIB;
    memset(rrcsib_collect_data.deferred_sib_info.deferred_sib_seg_db[i].seg_data_ptr,0x00, FIXED_SIB_DATA_LENGTH_BYTES);
  }
  
  for(i = 0;i < MAX_TEMP_SIB_SEGMENTS_TO_HOLD;i++)
   {
     if((rrcsib_collect_data.tsib_optimization_db[i].sib_type == rrc_SIB11)||
        (rrcsib_collect_data.tsib_optimization_db[i].sib_type == rrc_SIB12)||
        (rrcsib_collect_data.tsib_optimization_db[i].sib_type == rrc_SIB11bis) ||
        (rrcsib_collect_data.tsib_optimization_db[i].sib_type == rrc_SIB18)
#ifdef FEATURE_WCDMA_TO_LTE
        ||(rrcsib_collect_data.tsib_optimization_db[i].sib_type == rrc_SIB19)
#endif
        )
     {
       rrcsib_collect_data.deferred_sib_info.deferred_sib_seg_db[j] = rrcsib_collect_data.tsib_optimization_db[i];
