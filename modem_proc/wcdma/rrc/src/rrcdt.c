...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcdt.c#7 $
...
12/18/17   sp      Made changes to initialize dds_sub global to NONE
11/02/17   sp      Made changes to indicate ul comp support only for DDS sub
06/10/16   sr      Changes to reset 'rrc_ul_comp_cap_support' only on receiving ack for the UDT that carrries compression info.
05/19/16   sr      f3 reduction changes for RCE/UECI/DT
05/04/16   sn      f3 density reduction changes
12/08/15   sp      Made changes to fix QSH Active set metric, UL OTA parameters metrics 
08/07/15   sas     Made changes to fail Open session req with cause "RRC_OPEN_SESS_FAILURE_CONN_REL_IN_PROG" while waiting for conn rel during FRTLte.
08/05/15   sas     Skip changing priority upon CS session close and rely completely on NAS proc Ind.
...
03/20/15   sas     Send failure to open_session_req when the Domain is already active.
...
/* Indicates if N/W supports UL compression or not */
  boolean    rrc_ul_comp_cap_support;
  uint8      rrc_ul_comp_cap_mui; /*Stores the MUI of UDT that carries compression capability*/
#endif
...
boolean nv_fake_security_enabled_sim[MAX_RRC_STACKS];
 ...
 */
#ifdef FEATURE_DUAL_WCDMA 
LOCAL rrcdt_int_data_type  rrcdt_int_data_sim[MAX_RRC_STACKS];
#define rrcdt_int_data rrcdt_int_data_sim[WAS_ID_M]
#else
LOCAL rrcdt_int_data_type        rrcdt_int_data;
#endif
...
boolean rrcscr_get_signalling_connection_sts
(
...
)
{
  ...
  switch (cn_domain_id)
  {
    ...
    default:
      WRRC_ERR1_FATAL("Invalid CN domain: %d", cn_domain_id);
...
===========================================================================*/
boolean rrcscr_get_signalling_connection_status_sub
(
  sys_modem_as_id_e_type wrrc_as_id,
  rrc_cn_domain_identity_e_type cn_domain_id
)
{
  return rrcscr_get_signalling_connection_sts(wrrc_as_id, cn_domain_id);
}
...
{
  return rrcscr_get_signalling_connection_sts(SYS_MODEM_AS_ID_1,cn_domain_id);
}
/***/ __WCDMA_RRC_CODE_SEGMENT_NON_DEMANDPAGED_END__ /***/
...
)
{
  ...
  {
    if(rrcdt_int_data.rrcidt_cs_saved_open_session_req.cu_init_by_idt == TRUE ||
      rrcdt_int_data.rrcidt_ps_saved_open_session_req.cu_init_by_idt == TRUE)
    {
      status = TRUE;
    }
  }
...
)
{
  ...
  switch(cn_domain_id)
  {
    case RRC_CS_DOMAIN_CN_ID:
      ...
      {
        if(RRCDT_SUCCESS == rrcscr_indicate_close_session(wrrc_as_id,cn_domain_id, cause))
        {
          ...
          rrcudt_delete_all_saved_data_req(wrrc_as_id, RRC_CS_DOMAIN_CN_ID);
          if(rrcudt_any_saved_data_req_for_cn_domain(wrrc_as_id,RRC_PS_DOMAIN_CN_ID) == FALSE)
          {
            WRRC_MSG0_HIGH("There is no saved UDT for PS domain, set udt substate to init");
            rrcdt_int_data.rrcudt_substate = RRCUDT_INITIAL;
          }
        }
        else
        {
    ...
    case RRC_PS_DOMAIN_CN_ID:
      ...
      if(rrcdt_int_data.ps_domain_open == TRUE)
      {
        if(RRCDT_SUCCESS == rrcscr_indicate_close_session(wrrc_as_id,cn_domain_id, cause))
        {
          ...
          status = RRCSCR_SIG_CONN_RELEASED;
          rrcudt_delete_all_saved_data_req(wrrc_as_id, RRC_PS_DOMAIN_CN_ID);
          if(rrcudt_any_saved_data_req_for_cn_domain(wrrc_as_id,RRC_CS_DOMAIN_CN_ID) == FALSE)
          {
...
)
{
  ...
  {
    save_close_session_req_ptr = &rrcdt_int_data.rrcscrr_saved_cs_close_session_req;
  }
  else
  {
    save_close_session_req_ptr = &rrcdt_int_data.rrcscrr_saved_ps_close_session_req;
  }
    ...
    case RRC_CS_DOMAIN_CN_ID :
          ...
          (save_close_session_req_ptr->mui_status == RRCSCRR_MUI_SIG_CON_REL_CNF))
      {
          ...
          ( save_close_session_req_ptr->close_sess_req_status == TRUE))
        {
          ...
          rrcscrr_send_close_session_cnf(wrrc_as_id,RRC_CLOSE_SESS_SUCCESS,RRC_CS_DOMAIN_CN_ID);
          status = FALSE;
        }
        else if((save_close_session_req_ptr->mui_status == RRCSCRR_MUI_SIG_CON_REL_CNF)||
           (save_close_session_req_ptr->rrc_sig_con_rel_status == TRUE))
        {
          WRRC_MSG3_HIGH("mui_status=%d, rrc_sig_con_rel_status =%d proc: rrc_proc_e_type_value%d",save_close_session_req_ptr->mui_status,
            save_close_session_req_ptr->rrc_sig_con_rel_status,
            save_close_session_req_ptr->rrc_proc);
          rrcscrr_send_sig_conn_release_cnf(wrrc_as_id,
                        SUCCESS, 
                        save_close_session_req_ptr->rrc_proc);
          status = FALSE;
        }
        ...
        *any condition
        */
        rrcscrr_delete_close_session_req(wrrc_as_id, RRC_CS_DOMAIN_CN_ID);
        if(rrcdt_int_data.rrcscrr_saved_ps_close_session_req.mui_status != RRCSCRR_MUI_STATUS_FALSE)
        {
          WRRC_MSG2_HIGH("Scrr sub state is %d,cont.. in this state as mui state of PS is %d ",rrcdt_int_data.rrcscrr_substate,
            rrcdt_int_data.rrcscrr_saved_ps_close_session_req.mui_status);
        }
        else if((rrcdt_int_data.rrcscrr_saved_ps_close_session_req.close_sess_req_status == TRUE)||
          (rrcdt_int_data.rrcscrr_saved_ps_close_session_req.rrc_sig_con_rel_status == TRUE))
        {
          WRRC_MSG3_HIGH("Scrr sub state is %d,cont.. in this state as close_req=%d sig_con_req=%d ",rrcdt_int_data.rrcscrr_substate,
            rrcdt_int_data.rrcscrr_saved_ps_close_session_req.close_sess_req_status,
            rrcdt_int_data.rrcscrr_saved_ps_close_session_req.rrc_sig_con_rel_status);
        }
        else
        {
          rrcdt_int_data.rrcscrr_substate = RRCSCRR_INITIAL; 
        }
        WRRC_MSG2_HIGH("Scrr sub state is %d,cn domain=%d",rrcdt_int_data.rrcscrr_substate,domain_id);
      }
      else
      {
        /*Its safe to call this function to delete rrcscrr_saved_cs_close_session_req here with out 
        *any condition
        */
        WRRC_MSG3_HIGH("Scrr sub state is %d,cn domain=%d,PS mui_status=%d",
               rrcdt_int_data.rrcscrr_substate,
               domain_id,
               rrcdt_int_data.rrcscrr_saved_ps_close_session_req.mui_status);
        rrcscrr_delete_close_session_req(wrrc_as_id, RRC_CS_DOMAIN_CN_ID);
        if(rrcdt_int_data.rrcscrr_saved_ps_close_session_req.mui_status != RRCSCRR_MUI_STATUS_FALSE)
    ...
    case RRC_PS_DOMAIN_CN_ID :
          ...
          (save_close_session_req_ptr->mui_status == RRCSCRR_MUI_SIG_CON_REL_CNF))
      {
          ...
          ( save_close_session_req_ptr->close_sess_req_status == TRUE))
        {
          WRRC_MSG2_HIGH("mui_status=%d, close_sess_req_status =%d",save_close_session_req_ptr->mui_status,
            save_close_session_req_ptr->close_sess_req_status);
          rrcscrr_send_close_session_cnf(wrrc_as_id,RRC_CLOSE_SESS_SUCCESS,RRC_PS_DOMAIN_CN_ID);
          status = FALSE;
        }
        else if((save_close_session_req_ptr->mui_status == RRCSCRR_MUI_SIG_CON_REL_CNF)||
           (save_close_session_req_ptr->rrc_sig_con_rel_status == TRUE))
        {
          WRRC_MSG3_HIGH("mui_status=%d, rrc_sig_con_rel_status =%d proc:%d",save_close_session_req_ptr->mui_status,
            save_close_session_req_ptr->rrc_sig_con_rel_status,
            save_close_session_req_ptr->rrc_proc);
          rrcscrr_send_sig_conn_release_cnf(wrrc_as_id,
                        SUCCESS,
        ...
        *any condition
        */
        rrcscrr_delete_close_session_req(wrrc_as_id, RRC_PS_DOMAIN_CN_ID);
        if(rrcdt_int_data.rrcscrr_saved_cs_close_session_req.mui_status != RRCSCRR_MUI_STATUS_FALSE)
        {
          WRRC_MSG2_HIGH("Scrr sub state is %d,cont.. in this state as mui state of CS is %d ",rrcdt_int_data.rrcscrr_substate,
            rrcdt_int_data.rrcscrr_saved_cs_close_session_req.mui_status);
        }
        else if((rrcdt_int_data.rrcscrr_saved_cs_close_session_req.close_sess_req_status == TRUE)||
          (rrcdt_int_data.rrcscrr_saved_cs_close_session_req.rrc_sig_con_rel_status == TRUE))
        {
          WRRC_MSG3_HIGH("Scrr sub state is %d,cont.. in this state as close_req=%d sig_con_req=%d ",rrcdt_int_data.rrcscrr_substate,
            rrcdt_int_data.rrcscrr_saved_cs_close_session_req.close_sess_req_status,
            rrcdt_int_data.rrcscrr_saved_cs_close_session_req.rrc_sig_con_rel_status);
        }
        else
        {
          rrcdt_int_data.rrcscrr_substate = RRCSCRR_INITIAL; 
      ...
      else
      {
        ...
        *any condition
        */
        WRRC_MSG3_HIGH("Scrr sub state is %d,cn domain=%d,CS mui_status=%d",
             rrcdt_int_data.rrcscrr_substate,
             domain_id,
             rrcdt_int_data.rrcscrr_saved_cs_close_session_req.mui_status);
        rrcscrr_delete_close_session_req(wrrc_as_id, RRC_PS_DOMAIN_CN_ID);
...
)
{
  ...
  if(rrcscrr_is_close_session_ind_needed(wrrc_as_id,cn_domain_id) == TRUE)
  {
    status = rrcscr_send_close_session_ind(wrrc_as_id,cn_domain_id, cause);
    ...
    if(RRC_PS_DOMAIN_CN_ID == cn_domain_id)
    {
      ...
      rrcdormancy_check_and_abort_dormancy_req(wrrc_as_id, RRC_FAST_DORMANCY_SUCCESS_APP_NO_RETRY);
    }
  }
...
RETURN VALUE
  ...
  FALSE:Otherwise.
===========================================================================*/
static boolean rrcidt_is_idt_buffered(sys_modem_as_id_e_type wrrc_as_id,rrc_cn_domain_identity_e_type domain_id)
{
  boolean status = FALSE;
    ...
    {
      case RRC_CS_DOMAIN_CN_ID :
      if(rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_sess_req_status == TRUE)
      {
        status = TRUE;
      }
      break;
      ...
      case RRC_PS_DOMAIN_CN_ID :
      if(rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_sess_req_status == TRUE)
      {
        status = TRUE;
      }
      break;
...
)
{
  ...
  if((llc_cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id)) != NULL)
  {
    ...
    WRRC_MSG3_HIGH("Purging RLC SRB3:%d, SRB4:%d, proc_id:%d",purge_srb3,purge_srb4,proc_id);
...
)
{
  ...
  if(WTOW_SEARCH_STOP != rrc_csp_bplmn_srch_in_progress(wrrc_as_id))
  {
    ...
    rrc_csp_send_bplmn_suspend_req(wrrc_as_id,RRC_PROCEDURE_IDT,suspend_cause);
  }
}
...
)
{
  ...
  {
    case RRC_CS_DOMAIN_CN_ID:
      if(((rrcdt_int_data.rrcscrr_saved_cs_close_session_req.close_sess_req_status == TRUE)||
          (rrcdt_int_data.rrcscrr_saved_cs_close_session_req.rrc_sig_con_rel_status == TRUE)) &&
         (rrcdt_int_data.rrcscrr_saved_cs_close_session_req.mui_status == RRCSCRR_MUI_STATUS_FALSE))
      {
        WRRC_MSG0_HIGH("Processing saved SCRR for CS domain after IDT indication");
        rrcscrr_process_sig_conn_rel_req(wrrc_as_id,domain_id);
        ret_val = TRUE;
      }
      break;

    case RRC_PS_DOMAIN_CN_ID:
      if(((rrcdt_int_data.rrcscrr_saved_ps_close_session_req.close_sess_req_status == TRUE)||
          (rrcdt_int_data.rrcscrr_saved_ps_close_session_req.rrc_sig_con_rel_status == TRUE)) &&
         (rrcdt_int_data.rrcscrr_saved_ps_close_session_req.mui_status == RRCSCRR_MUI_STATUS_FALSE))
      {
...
)
{
  ...
  {
    if((rrcdt_int_data.rrcidt_est_cause_info.valid == TRUE) && (rrcdt_int_data.rrcidt_est_cause_info.to_be_sent_by == proc_id))
    {
      ret_val = TRUE;
      *est_cause = rrcdt_int_data.rrcidt_est_cause_info.est_cause;
      *call_type = rrcdt_int_data.rrcidt_est_cause_info.call_type;
    }
  }
  return ret_val;
}
...
)
{
  ...
  if ((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5)) 
  {
    if(rrcdt_int_data.rrcidt_est_cause_info.valid == TRUE)
    {
      rrcdt_int_data.rrcidt_est_cause_info.to_be_sent_by=proc_id;
    }
  }
}
...
)
{
  ...
  if ((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5)) 
  {
    if(rrcidt_check_if_est_cause_available(wrrc_as_id,RRC_PROCEDURE_IDT, &est_cause
                                                                    ,&call_type
                                                                    )== TRUE)
    {
     ...
     /* Enable the laterNonCriticalExtension field in v3a0NoncriticalExtension structure*/
      ...
      {
        est_cause = rrcdt_int_data.rrcidt_cs_saved_open_session_req.est_cause;
      }
      else
      {
        est_cause = rrcdt_int_data.rrcidt_ps_saved_open_session_req.est_cause;
      }
  
      rrc_translate_establishment_cause(wrrc_as_id,est_cause, &trans_cause);
...
)
{
  ...
  if ((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5)) 
  {
    ...
    if(rrcidt_check_if_est_cause_available(wrrc_as_id,proc_id,&est_cause
                                                                    ,&call_type
                                                                   )==TRUE)
    {
      rrcdt_int_data.rrcidt_est_cause_info.sent_by_cu= TRUE;
    }
  }
}
...
)
{
  ...
  if ((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5)) 
  {
    ...
    if(rrcdt_int_data.rrcidt_est_cause_info.last_proc_sent == RRC_PROCEDURE_RCE)
    {/*First IDT will not carry EST_cause*/
      rrcdt_int_data.rrcidt_est_cause_info.est_cause = est_cause;
      rrcdt_int_data.rrcidt_est_cause_info.last_proc_sent = proc_id;
      rrcdt_int_data.rrcidt_est_cause_info.valid = FALSE;
    }
    else
    {  /*All subsequent IDT's will carry EST_cause which 
      ...
      rrcdt_int_data.rrcidt_est_cause_info.valid = TRUE;
      rrcdt_int_data.rrcidt_est_cause_info.est_cause = est_cause;
      rrcdt_int_data.rrcidt_est_cause_info.to_be_sent_by = proc_id;

      if(cn_domain_id == RRC_CS_DOMAIN_CN_ID)
      {
        rrcdt_int_data.rrcidt_est_cause_info.call_type = rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_session_req.call_type;
      }
    ...
    WRRC_MSG3_HIGH("last_proc_sent = %d, est_cause=%d & is valid =%d ",rrcdt_int_data.rrcidt_est_cause_info.last_proc_sent,
                                                                                                    rrcdt_int_data.rrcidt_est_cause_info.est_cause,
                                                                                                    rrcdt_int_data.rrcidt_est_cause_info.valid);
    WRRC_MSG2_HIGH("multi_rab status =%d, prcd to be sent by =%d",multi_rab,rrcdt_int_data.rrcidt_est_cause_info.to_be_sent_by);
  }
}
/*===========================================================================
...
===========================================================================*/
void rrcidt_update_proc_id(sys_modem_as_id_e_type wrrc_as_id)
{
  rrcdt_int_data.rrcidt_est_cause_info.last_proc_sent = RRC_PROCEDURE_RCE;
}
/*===========================================================================
...
)
{
  ...
  if ((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5)) 
  {
    ...
    {
      if(rrcdt_int_data.rrcidt_est_cause_info.sent_by_cu==FALSE)
      {
        rrcdt_int_data.rrcidt_est_cause_info.to_be_sent_by =RRC_PROCEDURE_IDT;
      }
    }
  }
}
  ...
  rrc_proc_e_type proc_id)
{
  ...
  if ((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5)) 
  {
    ...
    {
      rrcdt_int_data.rrcidt_est_cause_info.valid = FALSE;
      rrcdt_int_data.rrcidt_est_cause_info.est_cause = RRC_EST_TERM_UNKNOWN;
      rrcdt_int_data.rrcidt_est_cause_info.to_be_sent_by = RRC_PROCEDURE_NONE;
      rrcdt_int_data.rrcidt_est_cause_info.sent_by_cu = FALSE;

      rrcdt_int_data.rrcidt_est_cause_info.call_type = RRC_CALL_TYPE_NONE;
    }
  }
}
...
)
{
  if(cn_domain_id == RRC_CS_DOMAIN_CN_ID)
  {
    rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_sess_req_status = FALSE;
    rrcdt_int_data.rrcidt_cs_saved_open_session_req.mui_status = FALSE;
    rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_sess_cnf_required = FALSE;
    rrcdt_int_data.rrcidt_cs_saved_open_session_req.cu_init_by_idt = FALSE;
    rrcdt_int_data.rrcidt_cs_saved_open_session_req.fresh_start_value_needed = FALSE;

    /*delete heap memory if it were allocated*/
    if(rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_session_req.nas_msg.value != NULL)
    {
      rrc_free( rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_session_req.nas_msg.value);

      rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_session_req.nas_msg.value = NULL;
      rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_session_req.nas_msg.length = 0;
    }
    rrcdt_int_data.rrcidt_cs_saved_open_session_req.est_cause = RRC_EST_TERM_UNKNOWN;
    rrcidt_reset_est_cause(wrrc_as_id, RRC_PROCEDURE_IDT);

    rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_session_req.call_type = RRC_CALL_TYPE_NONE;
  }
  else if(cn_domain_id == RRC_PS_DOMAIN_CN_ID)
  {
    rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_sess_req_status = FALSE;
    rrcdt_int_data.rrcidt_ps_saved_open_session_req.mui_status = FALSE;
    rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_sess_cnf_required = FALSE;
    rrcdt_int_data.rrcidt_ps_saved_open_session_req.cu_init_by_idt = FALSE;
    rrcdt_int_data.rrcidt_ps_saved_open_session_req.fresh_start_value_needed = FALSE;

    /*delete heap memory if it were allocated*/
    if(rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_session_req.nas_msg.value != NULL)
    {
      rrc_free( rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_session_req.nas_msg.value);

      rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_session_req.nas_msg.value = NULL;
      rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_session_req.nas_msg.length = 0;

    }
    rrcdt_int_data.rrcidt_ps_saved_open_session_req.est_cause=RRC_EST_TERM_UNKNOWN;
    rrcidt_reset_est_cause(wrrc_as_id, RRC_PROCEDURE_IDT);
  }
  else
  {
    WRRC_ERR1_FATAL("Invalid Domain ID %d", cn_domain_id);
  }

  /* Check and send any blocked SCRR */
  (void)rrcdt_check_and_send_pending_scrr(wrrc_as_id,cn_domain_id);
...
)
{
  ...
  switch(domain_id)
  {
    case RRC_CS_DOMAIN_CN_ID:
      ...
      if((rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_sess_req_status == TRUE)||
         (rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_sess_cnf_required == TRUE))
      {
        ret_val = TRUE;
      }
      break;
    ...
    case RRC_PS_DOMAIN_CN_ID:
      ...
      /* second condition says that we are waiting for l2ack */
      if((rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_sess_req_status == TRUE)||
         (rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_sess_cnf_required == TRUE))
      {
        ret_val = TRUE;
      }
      break;
...
)
{
  ...
  {
    case RRC_CS_DOMAIN_CN_ID:
      if(rrcdt_int_data.rrcidt_cs_saved_open_session_req.cu_init_by_idt == FALSE)
      {
        rrcidt_delete_open_session_req(wrrc_as_id, RRC_CS_DOMAIN_CN_ID);
        status = TRUE;
        if(FALSE == rrcidt_is_idt_active(wrrc_as_id,RRC_PS_DOMAIN_CN_ID))
        {
          rrcdt_int_data.rrcidt_substate = RRCIDT_INITIAL;  
        }
      }
      break;

    case RRC_PS_DOMAIN_CN_ID:
      if(rrcdt_int_data.rrcidt_ps_saved_open_session_req.cu_init_by_idt == FALSE)
      {
        rrcidt_delete_open_session_req(wrrc_as_id, RRC_PS_DOMAIN_CN_ID);
        status = TRUE;
        if(FALSE == rrcidt_is_idt_active(wrrc_as_id,RRC_CS_DOMAIN_CN_ID))
        {
          rrcdt_int_data.rrcidt_substate = RRCIDT_INITIAL;  
        }
      }
...
)
{
  ...
  if( rrcidt_is_idt_active(wrrc_as_id,domain_id) == TRUE)
  {
...
)
{
  ...
  if( (cmd_ptr =  mm_rrc_get_cmd_buf(RRC_OPEN_SESSION_CNF)) != NULL)
  {
    ...
    cmd_ptr->cmd.rrc_open_session_cnf.as_id = rrc_get_as_id(wrrc_as_id);
#endif
...
#ifdef FEATURE_DUAL_SIM
    cmd_ptr->cmd.rrc_sync_ind.as_id = rrc_get_as_id(wrrc_as_id);
#endif
...
{
  boolean status = FALSE;
  if((RRCRCE_RRC_CONN_ESTABLISHED == rrcrce_get_rrc_connection_status(wrrc_as_id) ) 
      ||
     (((rrcrce_get_rce_sub_state(wrrc_as_id) == RRCRCE_WAIT_FOR_RRC_TIMER_TO_EXPIRE)||
        (rrcrce_get_rce_sub_state(wrrc_as_id) == RRCRCE_WAIT_FOR_SIG_CON_REL_COMPL))&&
      (cmd_ptr->cmd.open_sess_req.est_cause == RRC_EST_EMERGENCY_CALL)&&
      (RRCCU_NOT_COMPLETED == rrccu_get_cell_update_complete_status(wrrc_as_id, RRC_PROCEDURE_IDT ) )
     )
    )
  {
    ...
    WRRC_MSG3_HIGH("RCE sub state =%d,Est cause = %d, CU state = %d",rrcrce_get_rce_sub_state(wrrc_as_id),
                                        cmd_ptr->cmd.open_sess_req.est_cause,
                                        rrccu_get_cell_update_complete_status(wrrc_as_id,  RRC_PROCEDURE_IDT ));
    status = TRUE;
  }
...
)
{
  ...
  *session_status=RRC_OPEN_SESS_FAILURE;
   ...
   {
      WRRC_MSG1_HIGH("CSFB_HACK:csfb_extended_status=%d, Fail OPEN_SESSION_REQ",rrc_csp_int_data.csfb_extended_status);
   ...
   if (rrcrcr_is_proc_active(wrrc_as_id) == TRUE)
   {
   ...
   if (rrcidt_check_rrc_connection_status(wrrc_as_id,cmd_ptr) == FALSE)
   {
  ...
  {
     if(!rrc_bcch_and_nas_plmns_match(wrrc_as_id,rrccsp_get_curr_nas_requested_plmn(wrrc_as_id),cmd_ptr->cmd.open_sess_req.selected_plmn_id))
     {
   ...
   if(rrcrce_validate_cell_access_status(wrrc_as_id,cmd_ptr->cmd.open_sess_req.cn_domain_id,
                     cmd_ptr->cmd.open_sess_req.est_cause,
...
{       
  ...
  {
    case RRC_CS_DOMAIN_CN_ID :
      if((rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_sess_req_status == FALSE) &&
         (rrcdt_int_data.rrcidt_cs_saved_open_session_req.mui_status == FALSE))
      {
        rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_session_req.cn_domain_id = RRC_CS_DOMAIN_CN_ID;
        /* Save the Intra Domain NAS Node selector */
        rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_session_req.intra_domain_nas_node_selector.corenetwork_choice = 
          cmd_ptr->cmd.open_sess_req.intra_domain_nas_node_selector.corenetwork_choice;
    
        rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_session_req.intra_domain_nas_node_selector.
          routing_info.routing_basis=
          cmd_ptr->cmd.open_sess_req.intra_domain_nas_node_selector.
          routing_info.routing_basis;
         ...
         * Initial Direct Transfer Message
        */
        rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_session_req.nas_msg.value = payload;
        rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_session_req.nas_msg.length = 
          (unsigned short) cmd_ptr->cmd.open_sess_req.nas_msg.length;
        /* Set status flag to TRUE */
        rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_sess_req_status = TRUE;
        rrcdt_int_data.rrcidt_cs_saved_open_session_req.est_cause = cmd_ptr->cmd.open_sess_req.est_cause;

        /* Set fresh_start_value_needed flag to TRUE, So that 1st IDT carries START 
         *calculate according to 8.5.9   */
        rrcdt_int_data.rrcidt_cs_saved_open_session_req.fresh_start_value_needed = TRUE;
  
       rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_session_req.call_type = cmd_ptr->cmd.open_sess_req.call_type;
  
        /*Save selected PLMN I.D*/
        rrcdt_int_data.rrcidt_cs_saved_open_session_req.selected_plmn_id = cmd_ptr->cmd.open_sess_req.selected_plmn_id;
        save_status = RRCDT_SUCCESS;
      }
      break;
    
    case RRC_PS_DOMAIN_CN_ID :
      if((rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_sess_req_status == FALSE)&&
         (rrcdt_int_data.rrcidt_ps_saved_open_session_req.mui_status == FALSE))
      {
        rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_session_req.cn_domain_id = RRC_PS_DOMAIN_CN_ID;
        /* Save the Intra Domain NAS Node selector */
        rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_session_req.intra_domain_nas_node_selector.corenetwork_choice = 
          cmd_ptr->cmd.open_sess_req.intra_domain_nas_node_selector.corenetwork_choice;
    
        rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_session_req.intra_domain_nas_node_selector.
          routing_info.routing_basis=
          cmd_ptr->cmd.open_sess_req.intra_domain_nas_node_selector.
          routing_info.routing_basis;
         ...
         * Initial Direct Transfer Message
         */
        rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_session_req.nas_msg.value = payload;
        rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_session_req.nas_msg.length = 
          (unsigned short) cmd_ptr->cmd.open_sess_req.nas_msg.length;

        /* Set status flag to TRUE */
        rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_sess_req_status = TRUE;
        rrcdt_int_data.rrcidt_ps_saved_open_session_req.est_cause = cmd_ptr->cmd.open_sess_req.est_cause;

        /* Set fresh_start_value_needed flag to TRUE, So that 1st IDT carries START 
         *calculate according to 8.5.9   */
        rrcdt_int_data.rrcidt_ps_saved_open_session_req.fresh_start_value_needed = TRUE;
  
        /*Save selected PLMN I.D*/
        rrcdt_int_data.rrcidt_ps_saved_open_session_req.selected_plmn_id = cmd_ptr->cmd.open_sess_req.selected_plmn_id;
        save_status = RRCDT_SUCCESS;
      }
      break;
    
    default :
      WRRC_ERR1_FATAL("Invalid Domain ID %d", 
        cmd_ptr->cmd.open_sess_req.cn_domain_id);
      break;
  }
  ...
  {
    rrcdt_check_and_suspend_bplmn(wrrc_as_id,cmd_ptr->cmd.open_sess_req.est_cause);
  }
...
{       
  ...
  if (ul_lc_id != RRCLCM_RLC_LC_ID_NOT_FOUND)
  {
    ...
    */
    ...
    switch(open_sess->open_session_req.intra_domain_nas_node_selector.corenetwork_choice)
    {
      ...
      default:
        WRRC_ERR0_FATAL("Other than GSM MAP CN are not supported");
        break;
    ...
    if(open_sess->fresh_start_value_needed == TRUE)
    {
      ...
      rrcsmc_check_new_keys_and_reset_append_start_per_domain(wrrc_as_id,(byte *) (msg_ptr->message.u.initialDirectTransfer.
                                     v3a0NonCriticalExtensions.
    ...
    */    
    ...
    if ((rrc_nv_rel_indicator & RRC_NV_REL_INDICATOR_REL5)) 
    {
      rrcidt_fill_est_cause(wrrc_as_id,msg_ptr, domain_id);    
    }
    ...
    */
    ...
    if(rrccsp_is_camped_cell_network_shared(wrrc_as_id) == TRUE)
    {
      ...
      rrcdt_fill_plmn_id_in_idt(wrrc_as_id,&msg_ptr->message.u.initialDirectTransfer.v3a0NonCriticalExtensions.
        laterNonCriticalExtensions.v590NonCriticalExtensions.
        v690NonCriticalExtensions.initialDirectTransfer_v690ext.plmn_Identity,
          domain_id);
    }
#ifdef FEATURE_FEMTO_CSG
    if(rrc_csg_int_data.is_whitelist_valid == TRUE)
    {
      if((!(RRC_MSG_COMMON_BITMASK_IE_PTR((&msg_ptr->message.u.initialDirectTransfer.v3a0NonCriticalExtensions),
        rrc_InitialDirectTransfer_v3a0NonCriticalExtensions_,
          laterNonCriticalExtensions))))
    ...
    rrc_qsh_log_idt_params(wrrc_as_id, rrcdt_int_data.rrcidt_cs_saved_open_session_req.est_cause,domain_id);
#endif

    send_status = rrcsend_send_ul_sdu_with_mui(wrrc_as_id,RRC_PROCEDURE_IDT,msg_ptr,ul_lc_id,
    ...
    if (send_status == RRCSEND_SUCCESS)
    {
     ...
     open_sess->mui = idt_mui;
      ...
      {
        rrcrce_set_rrc_est_cause_for_emergency_call(wrrc_as_id);
      }
#endif

      /* Wait for L2 ACK if there is no pending message for other Domain*/
      if(((domain_id == RRC_PS_DOMAIN_CN_ID ) && (rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_sess_req_status == FALSE))
          || ((domain_id == RRC_CS_DOMAIN_CN_ID ) && (rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_sess_req_status == FALSE))
        )
      {
        rrcdt_int_data.rrcidt_substate = RRCIDT_WAIT_FOR_L2ACK; 
      } 
    }
    else
    {
      ...
      rrcidt_delete_open_session_req(wrrc_as_id,domain_id);
  ...
  else
  {
    ...
    rrcidt_send_open_session_cnf(wrrc_as_id,RRC_OPEN_SESS_FAILURE,
      domain_id);
...
)
{
  ...
  switch(rrcdt_int_data.rrcidt_substate)
  {
    case RRCIDT_INITIAL:
    ...
    case RRCIDT_WAIT_FOR_L2ACK:
      ...
      if(((rrcdt_int_data.rrcidt_cs_saved_open_session_req.mui_status == TRUE) ||
          (rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_sess_req_status == TRUE)) && 
         (rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_session_req.nas_msg.length != 0))
      {
        if(rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_session_req.nas_msg.length > 255)
        {
          WRRC_ERR0_FATAL("Len of NAS Msg is > 255");
        }
        ...
        WCDMA_MEMCPY(nas_data_ind.nas_data.payload, 
                     ...
                     rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_session_req.nas_msg.value,
                     rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_session_req.nas_msg.length);

        /* Send the message to RR */
        rr_send_rrc_message_to_rr_internal(wrrc_as_id,(void *)(&nas_data_ind),
                                   RR_NAS_DATA_IND,
      ...
      if(((rrcdt_int_data.rrcidt_ps_saved_open_session_req.mui_status == TRUE) ||
          (rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_sess_req_status == TRUE)) && 
         (rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_session_req.nas_msg.length != 0))
      {
        if(rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_session_req.nas_msg.length > 255)
        {
          WRRC_ERR0_FATAL("Len of NAS Msg is > 255");
        }
        ...
        WCDMA_MEMCPY(nas_data_ind.nas_data.payload, 
                     sizeof(uint8) * RRC_RR_MAX_GSM_MESSAGE_LENGTH,
                     rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_session_req.nas_msg.value,
                     rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_session_req.nas_msg.length);
    ...
    default:  
      WRRC_ERR1_FATAL("Invalid RRCIDT substate: %d ",rrcdt_int_data.rrcidt_substate);
      break;
   }
...
)
{
  ...
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    ...
    case RRC_OPEN_SESSION_REQ:
      ...
      if((RRCCU_NOT_COMPLETED == rrccu_get_cell_update_complete_status(wrrc_as_id, RRC_PROCEDURE_IDT ) )&&
        (rrcidt_check_rrc_connection_status(wrrc_as_id,cmd_ptr) == TRUE))
      {
        if(is_ue_oos_and_em_call_attempted(wrrc_as_id,cmd_ptr, RRC_PROCEDURE_IDT))
        {
          rrcdt_int_data.rrcidt_substate = RRCIDT_INITIAL;  
          return;
        }
       ...
       */
      if(RRCDT_SUCCESS != rrcidt_validate_open_session_req(wrrc_as_id,cmd_ptr,&session_status))
      {
        /* Send open session failure with appropriate failure status */
        rrcidt_send_open_session_cnf(wrrc_as_id,session_status,
                                    cmd_ptr->cmd.open_sess_req.cn_domain_id);
        return;
      }
       ...
       */
      if(RRCDT_SUCCESS != rrcidt_save_open_session_req(wrrc_as_id,cmd_ptr))
      {
        /* Send RRC_OPEN_SESS_CNF Primitive to MM with Failure status */
        rrcidt_send_open_session_cnf(wrrc_as_id,RRC_OPEN_SESS_FAILURE,
                                    cmd_ptr->cmd.open_sess_req.cn_domain_id);
      ...
      switch(rrc_get_state(wrrc_as_id))
      {
        ...
        case RRC_STATE_CELL_PCH: 
          if(RRCCU_COMPLETED == rrccu_get_cell_update_complete_status(wrrc_as_id,RRC_PROCEDURE_IDT))
          {
            rrcidt_set_est_cause(wrrc_as_id,cmd_ptr->cmd.open_sess_req.est_cause, 
                      RRC_PROCEDURE_CU, cmd_ptr->cmd.open_sess_req.cn_domain_id);
            switch (cmd_ptr->cmd.open_sess_req.cn_domain_id)
            {
              case RRC_CS_DOMAIN_CN_ID :
                rrcdt_int_data.rrcidt_cs_saved_open_session_req.cu_init_by_idt = TRUE;
              break;
            ...
            rrcdt_send_cell_update_initiate_req(wrrc_as_id, RRC_PROCEDURE_IDT);
            /* Wait until Cell update procedure is completed */
            rrcdt_int_data.rrcidt_substate = RRCIDT_WAIT_FOR_CELL_UPDATE_COMP_CNF;  
          }
          else
          {
            if(is_ue_oos_and_em_call_attempted(wrrc_as_id,cmd_ptr, RRC_PROCEDURE_IDT))
            ...
            {
              rrcidt_set_est_cause(wrrc_as_id,cmd_ptr->cmd.open_sess_req.est_cause, RRC_PROCEDURE_IDT,
                  cmd_ptr->cmd.open_sess_req.cn_domain_id);
        ...
        case RRC_STATE_CELL_DCH:
          ...
          else /* Cell Update procedure is inactive */
          {
            ...
            if(rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_sess_req_status == TRUE)
            {
              ...
              rrc_check_and_suspend_pseudo_meas_procedure(wrrc_as_id, RRC_PROCEDURE_IDT);
#endif
              rrcidt_send_initial_direct_transfer_msg(wrrc_as_id,RRC_CS_DOMAIN_CN_ID,&rrcdt_int_data.rrcidt_cs_saved_open_session_req);
            }
            if(rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_sess_req_status == TRUE)
            {
              rrcidt_send_initial_direct_transfer_msg(wrrc_as_id,RRC_PS_DOMAIN_CN_ID,&rrcdt_int_data.rrcidt_ps_saved_open_session_req);
            }
          }
          break;
        ...
        default:
          WRRC_MSG2_HIGH("Invalid Event: %d for IDT in substate:%d", 
                   cmd_ptr->cmd_hdr.cmd_id,rrcdt_int_data.rrcidt_substate);
        break;
      }
    ...
    case RRC_NAS_DATA_TRANSFER_IND:
      rrcidt_send_nas_data_ind(wrrc_as_id);   
      break;
...
)
{
  ...
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_CELL_UPDATE_COMPLETE_CNF:
      ...
      switch(rrc_get_state(wrrc_as_id))
      {
        ...
        case RRC_STATE_CELL_PCH:   
                 ...
                 rrccu_get_cell_update_complete_status(wrrc_as_id, RRC_PROCEDURE_IDT ) )
          {
            ...
            rrcidt_change_owner_est_cause(wrrc_as_id, RRC_PROCEDURE_CU);
            if(rrcidt_is_idt_buffered(wrrc_as_id,RRC_CS_DOMAIN_CN_ID) == TRUE)
            {
              rrcdt_int_data.rrcidt_cs_saved_open_session_req.cu_init_by_idt = TRUE;
            }
            if(rrcidt_is_idt_buffered(wrrc_as_id,RRC_PS_DOMAIN_CN_ID) == TRUE)
            {
              rrcdt_int_data.rrcidt_ps_saved_open_session_req.cu_init_by_idt = TRUE;
            }
      ...
      rrcidt_check_if_transmission_success(wrrc_as_id, RRC_PROCEDURE_CU);
      rrcidt_reset_est_cause(wrrc_as_id, RRC_PROCEDURE_CU);
      ...
      /* Reset the state machine */
      if((rrcdt_int_data.rrcidt_cs_saved_open_session_req.mui_status == FALSE)  &&
         (rrcdt_int_data.rrcidt_ps_saved_open_session_req.mui_status == FALSE) 
         )
      {
        rrcdt_int_data.rrcidt_substate = RRCIDT_INITIAL;  
...
)
{
  ...
  if((cn_domain == RRC_CS_DOMAIN_CN_ID) && 
     (rrcdt_int_data.rrcidt_cs_saved_open_session_req.mui_status == TRUE))
  {
    rrcidt_clear_idt_data_for_race_conditions(wrrc_as_id,cn_domain);
    status = TRUE;
  }
  else if((cn_domain == RRC_PS_DOMAIN_CN_ID) && 
          (rrcdt_int_data.rrcidt_ps_saved_open_session_req.mui_status == TRUE))
  {
    rrcidt_clear_idt_data_for_race_conditions(wrrc_as_id,cn_domain);
    status = TRUE;
  }
...
)
{
  ...
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    ...
    WRRC_MSG6_HIGH("CS domain IDT status: mui_status %d, mui %d, PS domain IDT status: mui_status %d, mui %d Received IDT L2-ACK status %d mui %d ",
           rrcdt_int_data.rrcidt_cs_saved_open_session_req.mui_status,
           rrcdt_int_data.rrcidt_cs_saved_open_session_req.mui,
           rrcdt_int_data.rrcidt_ps_saved_open_session_req.mui_status,
           rrcdt_int_data.rrcidt_ps_saved_open_session_req.mui,
           cmd_ptr->cmd.downlink_l2ack_ind.status, 
           cmd_ptr->cmd.downlink_l2ack_ind.mui);

      /* Send RRC_OPEN_SESSION_CNF to MM */
      if((rrcdt_int_data.rrcidt_cs_saved_open_session_req.mui_status == TRUE) &&
         (rrcdt_int_data.rrcidt_cs_saved_open_session_req.mui == cmd_ptr->cmd.downlink_l2ack_ind.mui))
      {
         ...
         */
        if(rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_sess_cnf_required == TRUE)
        {
          /* Check and send any blocked SCRR for this domain */
          boolean cs_scrr_active = rrcdt_check_and_send_pending_scrr(wrrc_as_id, RRC_CS_DOMAIN_CN_ID);
             ...
             (TRUE == cs_scrr_active))
          {
            ...
            if(nv_fake_security_enabled)
            {
              ...
              rrcidt_send_sync_ind(wrrc_as_id,RRC_INTEGRITY_ESTABLISHED,
                                   RRC_CS_DOMAIN_CN_ID);
            }
          }
          else
      ...
      else if((rrcdt_int_data.rrcidt_ps_saved_open_session_req.mui_status == TRUE) &&
              (rrcdt_int_data.rrcidt_ps_saved_open_session_req.mui == 
               cmd_ptr->cmd.downlink_l2ack_ind.mui)
              )
      {
        /*if L2_ACK for SMC is not received before L2_ACK for IDT,
          ...
          been sent by SMC just before sending SYNC_IND
         */
        if(rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_sess_cnf_required == TRUE)
        {
          /* Check and send any blocked SCRR for this domain */
          boolean ps_scrr_active = rrcdt_check_and_send_pending_scrr(wrrc_as_id, RRC_PS_DOMAIN_CN_ID);
    ...
    case RRC_OPEN_SESSION_REQ:
      ...
      else
      {
        ...
        else if(rrcdt_int_data.rrcidt_ps_saved_open_session_req.open_sess_req_status == TRUE)
        {
          rrcidt_send_initial_direct_transfer_msg(wrrc_as_id,RRC_PS_DOMAIN_CN_ID,&rrcdt_int_data.rrcidt_ps_saved_open_session_req);
        }
      }
  ...
  switch (rrcdt_int_data.rrcidt_substate)
  {
    case RRCIDT_INITIAL:
      switch(cmd_ptr->cmd_hdr.cmd_id)
      {
        ...
        case RRC_NAS_DATA_TRANSFER_IND:
          rrcidt_process_idt_initial_substate(wrrc_as_id, cmd_ptr);
          break;
        
        default:
          WRRC_MSG1_HIGH("IDT:Invalid eventin Substate: %d",
                    rrcdt_int_data.rrcidt_substate);
          break;
      }
      break;
    ...
    case RRCIDT_WAIT_FOR_CELL_UPDATE_COMP_CNF: 
      switch(cmd_ptr->cmd_hdr.cmd_id)
      {
        ...
        case RRC_OPEN_SESSION_REQ:
        case RRC_NAS_DATA_TRANSFER_IND:
          rrcidt_process_idt_wait_for_cell_update_comp_cnf_substate(wrrc_as_id, cmd_ptr);
          break;
    ...
    case RRCIDT_WAIT_FOR_L2ACK:
      switch(cmd_ptr->cmd_hdr.cmd_id)
      {
        ...
        case RRC_OPEN_SESSION_REQ:
        case RRC_NAS_DATA_TRANSFER_IND:
          rrcidt_process_idt_wait_for_l2ack_substate(wrrc_as_id, cmd_ptr);
          break;
...
)
{
  ...
  WRRC_MSG2_HIGH_OPT("function rrcidt_procedure_event_handler is called in rrc_state = rrc_state_e_type_value%d and rrcidt_substate = rrcidt_substate_e_type_value%d", rrc_state, rrcdt_int_data.rrcidt_substate);
  rrc_crash_debug_save_proc_substates(wrrc_as_id,RRC_PROCEDURE_IDT,rrcdt_int_data.rrcidt_substate,cmd_ptr->cmd_hdr.cmd_id);

  if(RRC_STATE_CHANGE_IND == cmd_ptr->cmd_hdr.cmd_id)
  {
    WRRC_MSG2_HIGH("State-Change-Ind idt-state %d in rrc-state %d",rrcdt_int_data.rrcidt_substate,rrc_state);
  ...
  else
  {
    ...
    {
      case RRC_STATE_CELL_FACH:    
      rrcidt_process_cell_fach_state(wrrc_as_id,cmd_ptr); 
      break;
  
      case RRC_STATE_CELL_DCH:  
      rrcidt_process_cell_dch_state(wrrc_as_id,cmd_ptr); 
      break;
  
      case RRC_STATE_CELL_PCH:
      case RRC_STATE_URA_PCH:
      rrcidt_process_cell_ura_pch_state(wrrc_as_id, cmd_ptr);
      break;
...
===========================================================================*/
boolean rrcdt_is_cs_domain_open(sys_modem_as_id_e_type wrrc_as_id)
{
...
===========================================================================*/
boolean rrcdt_is_ps_domain_open(sys_modem_as_id_e_type wrrc_as_id)
{
...
)
{
  ...
  {
    if((rrcdt_int_data.rrcudt_cs_saved_data[count].mui == mui) && 
       (rrcdt_int_data.rrcudt_cs_saved_data[count].mui_status == TRUE)) 
    {
  ...
  if(domain_found != TRUE)
  {
    ...
    {
      if((rrcdt_int_data.rrcudt_ps_saved_data[count].mui == mui) && 
         (rrcdt_int_data.rrcudt_ps_saved_data[count].mui_status == TRUE)) 
      {
...
)
{
  ...
  switch (cn_domain_id)
  {
    case RRC_CS_DOMAIN_CN_ID :
      for (count = 0; count <MAX_NAS_MESSAGES_FOR_CS_DOMAIN; count++ )
      {
        if(rrcdt_int_data.rrcudt_cs_saved_data[count].mui_status || 
           rrcdt_int_data.rrcudt_cs_saved_data[count].data_req_status)
        {
          status = TRUE;
          break;
        }
    ...
    case RRC_PS_DOMAIN_CN_ID :
      for (count = 0; count <MAX_NAS_MESSAGES_FOR_PS_DOMAIN; count++ )
      {
        if(rrcdt_int_data.rrcudt_ps_saved_data[count].mui_status || 
           rrcdt_int_data.rrcudt_ps_saved_data[count].data_req_status)
        {
          status = TRUE;
          break;
        }
    ...
    case RRC_CS_DOMAIN_CN_ID :
      for (count = 0; count <MAX_NAS_MESSAGES_FOR_CS_DOMAIN; count++ )
      {
        if(rrcdt_int_data.rrcudt_cs_saved_data[count].mui_status || 
           rrcdt_int_data.rrcudt_cs_saved_data[count].data_req_status)
        {
          rrcdt_int_data.rrcudt_cs_saved_data[count].mui_status = FALSE;
          rrcdt_int_data.rrcudt_cs_saved_data[count].data_req_status = FALSE;

          if(rrcdt_int_data.rrcudt_cs_saved_data[count].data_req.nas_msg.value != NULL)
          {
            rrc_free(rrcdt_int_data.rrcudt_cs_saved_data[count].data_req.nas_msg.value);
            rrcdt_int_data.rrcudt_cs_saved_data[count].data_req.nas_msg.value = NULL;
            rrcdt_int_data.rrcudt_cs_saved_data[count].data_req.nas_msg.length = 0;
    ...
    case RRC_PS_DOMAIN_CN_ID :
      for (count = 0; count <MAX_NAS_MESSAGES_FOR_PS_DOMAIN; count++ )
      {
        if(rrcdt_int_data.rrcudt_ps_saved_data[count].mui_status || 
           rrcdt_int_data.rrcudt_ps_saved_data[count].data_req_status)
        {
          rrcdt_int_data.rrcudt_ps_saved_data[count].mui_status = FALSE;
          rrcdt_int_data.rrcudt_ps_saved_data[count].data_req_status = FALSE;

          if(rrcdt_int_data.rrcudt_ps_saved_data[count].data_req.nas_msg.value != NULL)
          {
            rrc_free(rrcdt_int_data.rrcudt_ps_saved_data[count].data_req.nas_msg.value);
            rrcdt_int_data.rrcudt_ps_saved_data[count].data_req.nas_msg.value = NULL;
            rrcdt_int_data.rrcudt_ps_saved_data[count].data_req.nas_msg.length = 0;
#ifdef FEATURE_NAS_UDT_OOS
            #error code not present
#endif
          }
...
#ifdef FEATURE_WCDMA_UL_COMPR
  rrcdt_int_data.rrc_ul_comp_cap_support = FALSE;
#endif
} /* rrcudt_delete_all_saved_data_req */
/*===========================================================================
...
)
{
  ...
  for (count = 0; count < MAX_NAS_MESSAGES_FOR_CS_DOMAIN; count++ )
  {
    if(rrcdt_int_data.rrcudt_cs_saved_data[count].mui_status == TRUE)
    {
      if( (rrcdt_int_data.rrcudt_cs_saved_data[count].mui == rcvd_mui) &&
          (rrcdt_int_data.rrcudt_cs_saved_data[count].data_req.priority == priority) )
      {
        rrcdt_int_data.rrcudt_cs_saved_data[count].mui_status = FALSE;
        rrcdt_int_data.rrcudt_cs_saved_data[count].data_req_status = FALSE;
        if (rrcdt_int_data.rrcudt_cs_saved_data[count].data_req.nas_msg.value !=
            NULL)
        {
          rrc_free(rrcdt_int_data.rrcudt_cs_saved_data[count].data_req.nas_msg.value);
          rrcdt_int_data.rrcudt_cs_saved_data[count].data_req.nas_msg.value = NULL;
  ...
  for (count = 0; count < MAX_NAS_MESSAGES_FOR_PS_DOMAIN; count++ )
  {
    if(rrcdt_int_data.rrcudt_ps_saved_data[count].mui_status == TRUE)
    {
      if( (rrcdt_int_data.rrcudt_ps_saved_data[count].mui == rcvd_mui) &&
          (rrcdt_int_data.rrcudt_ps_saved_data[count].data_req.priority == priority) )
      {
        rrcdt_int_data.rrcudt_ps_saved_data[count].mui_status = FALSE;
        rrcdt_int_data.rrcudt_ps_saved_data[count].data_req_status = FALSE;
        if (rrcdt_int_data.rrcudt_ps_saved_data[count].data_req.nas_msg.value !=
            NULL)
        {
          rrc_free(rrcdt_int_data.rrcudt_ps_saved_data[count].data_req.nas_msg.value);
          rrcdt_int_data.rrcudt_ps_saved_data[count].data_req.nas_msg.value = NULL;
...
)
{
  ...
  switch (cn_domain_id)
  {
    case RRC_CS_DOMAIN_CN_ID :
      for (count = 0; count <MAX_NAS_MESSAGES_FOR_CS_DOMAIN; count++ )
      {
        if((rrcdt_int_data.rrcudt_cs_saved_data[count].mui_status == FALSE) &&
           (rrcdt_int_data.rrcudt_cs_saved_data[count].data_req_status ==FALSE) )
        {
          status = SUCCESS;
          *index = count;
    ...
    case RRC_PS_DOMAIN_CN_ID :
      for (count = 0; count <MAX_NAS_MESSAGES_FOR_PS_DOMAIN; count++ )
      {
        if((rrcdt_int_data.rrcudt_ps_saved_data[count].mui_status == FALSE) &&
           (rrcdt_int_data.rrcudt_ps_saved_data[count].data_req_status == FALSE) )
        {
          status = SUCCESS;
          *index = count;
...
)
{
  ...
  {
    if((rrcdt_int_data.rrcudt_cs_saved_data[count].mui_status == TRUE) ||
       (rrcdt_int_data.rrcudt_cs_saved_data[count].data_req_status == TRUE) )
    {
      status = SUCCESS;
      break;
    }
  ...
  {
    if((rrcdt_int_data.rrcudt_ps_saved_data[count].mui_status == TRUE) ||
       (rrcdt_int_data.rrcudt_ps_saved_data[count].data_req_status == TRUE) )
    {
      status = SUCCESS;
      break;
    }
...
)
{
  ...
  if(rrcudt_check_if_msg_status_can_be_saved(wrrc_as_id,
       cmd_ptr->cmd.data_req.cn_domain_id, &index ) == SUCCESS)
  {
    switch (cmd_ptr->cmd.data_req.cn_domain_id)
    {
      case RRC_CS_DOMAIN_CN_ID :
        ...
        save_temp_ptr          = &rrcdt_int_data.rrcudt_cs_saved_data[index].data_req;
        save_data_req_status_ptr = &rrcdt_int_data.rrcudt_cs_saved_data[index].data_req_status;
        save_temp_ptr->cn_domain_id = RRC_CS_DOMAIN_CN_ID;
        break;
      
      case RRC_PS_DOMAIN_CN_ID :
        save_temp_ptr          = &rrcdt_int_data.rrcudt_ps_saved_data[index].data_req;
        save_data_req_status_ptr = &rrcdt_int_data.rrcudt_ps_saved_data[index].data_req_status;
...
)
{
  ...
  if(!rrcnv_data.rrc_ul_compr_cap_nv)
  {
    WRRC_MSG0_MED("ULCOMP : UL compression NV is disabled");
    return FALSE;
  }
  ...
  {
    WRRC_MSG1_MED("ULCOMP : UL compression NV is disabled for device_mode %d", rrc_dsds_int_data.wcdma_device_mode);
    return FALSE;
  }
  if((rrc_get_dds_sub_id(wrrc_as_id) != SYS_MODEM_AS_ID_NONE) /* DDS sub ID will be None only if NAS has not sent DDS_SUB_IND */
      && 
     (rrc_get_dds_sub_id(wrrc_as_id) != rrc_get_as_id(wrrc_as_id)))
  {
    WRRC_MSG0_MED("Current sub is not DDS sub. Disable UL compression");
    return FALSE;
  }
#endif

  WRRC_MSG0_MED("ULCOMP NV enabled");
  return TRUE;
}
#endif
          ...
          count_index++)
    {
      if( count_domain == 0)
      {
        save_data_req_status_ptr = 
          &rrcdt_int_data.rrcudt_cs_saved_data[count_index].data_req_status;
        save_temp_ptr = &rrcdt_int_data.rrcudt_cs_saved_data[count_index].data_req;
      }
      else 
      { 
        ...
        {
          save_data_req_status_ptr = 
            &rrcdt_int_data.rrcudt_ps_saved_data[count_index].data_req_status;
          save_temp_ptr = &rrcdt_int_data.rrcudt_ps_saved_data[count_index].data_req;
        }
        else
        {
      ...
      {
        if(rrcdt_int_data.rrcscrr_substate != RRCSCRR_INITIAL)
        {
        ...
        WRRC_MSG3_HIGH("RRC_ULCOMP: rrc_ul_comp_cap_support %d, UL compr NV %d , Alt_sub DACC enabled: %d", rrcdt_int_data.rrc_ul_comp_cap_support, rrcnv_data.rrc_ul_compr_cap_nv, rlc_check_ul_comp_enabled_on_alt_sub(wrrc_as_id));
        if((TRUE == rrcdt_nv_supports_ul_comp(wrrc_as_id)) && (rrcdt_int_data.rrc_ul_comp_cap_support) && (FALSE == rlc_check_ul_comp_enabled_on_alt_sub(wrrc_as_id)))
        {          
          ...
          WRRC_MSG3_HIGH("RRC_ULCOMP: Comp mechanism %d %d %d",ul_comp_cap_list_ptr->elem[0].compressionMechanism,ul_comp_cap_list_ptr->elem[0].compressionVersion.u.implicit,ul_comp_cap_list_ptr->elem[0].memoryBuffer);
        }
#endif
          ...
          */
          send_status = rrcsend_send_ul_sdu_with_mui(wrrc_as_id,RRC_PROCEDURE_UDT,
                                            msg_ptr,
                                            ul_lc_id,
          ...
          if (send_status == RRCSEND_SUCCESS)
          {
            switch(save_temp_ptr->cn_domain_id)
            {
              case RRC_CS_DOMAIN_CN_ID :
                ...
                {
                  rrcdt_int_data.rrcudt_cs_saved_data[count_index].mui_status = TRUE;
                  rrcdt_int_data.rrcudt_cs_saved_data[count_index].mui = mui;
#ifdef FEATURE_WCDMA_UL_COMPR
                  if((TRUE == rrcdt_nv_supports_ul_comp(wrrc_as_id)) && (rrcdt_int_data.rrc_ul_comp_cap_support))
                  {
                    rrcdt_int_data.rrc_ul_comp_cap_mui = mui;
                  }
#endif
#ifdef FEATURE_WCDMA_MTPL_THERMAL_BACKOFF 
              ...
              case RRC_PS_DOMAIN_CN_ID :
                ...
                if(count_index < MAX_NAS_MESSAGES_FOR_PS_DOMAIN)
                {
                  rrcdt_int_data.rrcudt_ps_saved_data[count_index].mui_status = TRUE;
                  rrcdt_int_data.rrcudt_ps_saved_data[count_index].mui = mui;
#ifdef FEATURE_WCDMA_UL_COMPR
                  if((TRUE == rrcdt_nv_supports_ul_comp(wrrc_as_id)) && (rrcdt_int_data.rrc_ul_comp_cap_support))
                    {
                      rrcdt_int_data.rrc_ul_comp_cap_mui = mui;
              ...
              default :
                WRRC_ERR1_FATAL("Invalid Domain ID %d", 
                           save_temp_ptr->cn_domain_id);
                break;
            }
  
            WRRC_MSG0_HIGH("Sent Uplink Direct Transfer Message");  
            rrcdt_int_data.rrcudt_substate = RRCUDT_WAIT_FOR_L2ACK;
          }
          else
          {
...
)
{
  ...
  {
    if( (rrcdt_int_data.rrcudt_cs_saved_data[count_index].mui == retransmit_mui) &&
        (rrcdt_int_data.rrcudt_cs_saved_data[count_index].data_req.priority == retransmit_msg_priority) &&
        (rrcdt_int_data.rrcudt_cs_saved_data[count_index].mui_status == TRUE)
        )
    {
      //force data_req_status to true bcos we want to resend this again
      rrcdt_int_data.rrcudt_cs_saved_data[count_index].data_req_status = TRUE;
      save_data_req_status_ptr = 
        &rrcdt_int_data.rrcudt_cs_saved_data[count_index].data_req_status;
      save_temp_ptr = &rrcdt_int_data.rrcudt_cs_saved_data[count_index].data_req;
      msg_found = TRUE;
  ...
  if(msg_found != TRUE)
  {
    ...
    {
      if( (rrcdt_int_data.rrcudt_ps_saved_data[count_index].mui == retransmit_mui) &&
          (rrcdt_int_data.rrcudt_ps_saved_data[count_index].data_req.priority == retransmit_msg_priority) &&
          (rrcdt_int_data.rrcudt_ps_saved_data[count_index].mui_status == TRUE)
          )
      {
        //force data_req_status to true bcos we want to resend this again
        rrcdt_int_data.rrcudt_ps_saved_data[count_index].data_req_status = TRUE;
        save_data_req_status_ptr = 
          &rrcdt_int_data.rrcudt_ps_saved_data[count_index].data_req_status;
        save_temp_ptr = &rrcdt_int_data.rrcudt_ps_saved_data[count_index].data_req;
        msg_found = TRUE;
...
)
{
  ...
  if(rrcudt_check_if_msg_status_can_be_saved(wrrc_as_id,RRC_CS_DOMAIN_CN_ID, &index ) != SUCCESS)
  {
    WRRC_MSG0_HIGH("Can't process NAS DATA in CS domain");
    return RRCDT_FAILURE_MINOR;  
  }

  rrcdt_int_data.rrcudt_cs_saved_data[index].data_req.cn_domain_id = RRC_CS_DOMAIN_CN_ID;
  ...
  if(cmd_ptr->cmd.interrat_nas_data_ind.SAPI == 0)
  {
    ...
    rrcdt_int_data.rrcudt_cs_saved_data[index].data_req.priority = 
  ...
  {
    WRRC_ERR0_FATAL("Invalid SAPI is received");
  }
   ...
   */
  rrcdt_int_data.rrcudt_cs_saved_data[index].data_req.nas_msg.value = payload;
  rrcdt_int_data.rrcudt_cs_saved_data[index].data_req.nas_msg.length = 
    (unsigned short) cmd_ptr->cmd.interrat_nas_data_ind.l3_message_length;
    
  /* set the flag to indicate that DATA_REQ cmd contents are saved and 
   * return success status
   */
  rrcdt_int_data.rrcudt_cs_saved_data[index].data_req_status = TRUE;
...
)
{
...
#ifdef FEATURE_DUAL_SIM
    mm_cmd_ptr->cmd.rrc_activation_ind.as_id = rrc_get_as_id(wrrc_as_id);
#endif

    /* Put the command on MM queue */
    rrc_send_rrc_message_to_mm(wrrc_as_id, mm_cmd_ptr);
    WRRC_MSG0_HIGH("Dispatching out MM cmd / RRC_ACTIVATION_IND sent to MM");

    /*update the cause for activation indication*/
    rrcmcm_update_wcdma_activation_cause(wrrc_as_id, RRCMCM_INTERRAT_HANDOVER_GTOW);
  ...
  if(RRCDT_SUCCESS != rrcudt_save_g2w_nas_data_ind(wrrc_as_id,nas_msg_ptr))
  {
    /* save udt message */
    WRRC_MSG2_HIGH("Failed to Save NAS Data Ind in : %d, %d",
             rrc_get_state(wrrc_as_id),rrcdt_int_data.rrcudt_substate);
    return RRCDT_FAILURE_MINOR;
  }    
  
  /* Build and send the Uplink Direct Transfer to Send Chain */
  rrcudt_send_uplink_direct_transfer_msg(wrrc_as_id);
...
)
{
  ...
  rr_nas_data_ind_type nas_data_ind;    /* Local variable to store
                                           NAS message to be sent to RR */
  switch(rrcdt_int_data.rrcudt_substate)
  {
    case RRCUDT_INITIAL:
    ...
    case RRCUDT_WAIT_FOR_L2ACK:
      if(SUCCESS == rrcudt_check_saved_message_status(wrrc_as_id) )
      {
        ...
        for (count = 0; count < MAX_NAS_MESSAGES_FOR_CS_DOMAIN; count++ )
        {
          ...
          if(((rrcdt_int_data.rrcudt_cs_saved_data[count].mui_status == TRUE) ||
              ...
              (rrcdt_int_data.rrcudt_cs_saved_data[count].data_req.nas_msg.length != 0))
          {
            if (rrcdt_int_data.rrcudt_cs_saved_data[count].data_req.nas_msg.length > 255)
            {
              WRRC_ERR0_FATAL("Len of NAS msg > 255");
            }

            nas_data_ind.nas_data.message_length = (uint8)
                rrcdt_int_data.rrcudt_cs_saved_data[count].data_req.nas_msg.length;

            WCDMA_MEMCPY(nas_data_ind.nas_data.payload, 
                         sizeof(uint8) * RRC_RR_MAX_GSM_MESSAGE_LENGTH,
                         rrcdt_int_data.rrcudt_cs_saved_data[count].data_req.nas_msg.value,
                         rrcdt_int_data.rrcudt_cs_saved_data[count].data_req.nas_msg.length );
        ...
        for (count = 0; count < MAX_NAS_MESSAGES_FOR_PS_DOMAIN; count++ )
        {
          ...
          if(((rrcdt_int_data.rrcudt_ps_saved_data[count].mui_status == TRUE) ||
              ...
              (rrcdt_int_data.rrcudt_ps_saved_data[count].data_req.nas_msg.length != 0))
          {
            if(rrcdt_int_data.rrcudt_ps_saved_data[count].data_req.nas_msg.length > 255)
            {
              WRRC_ERR0_FATAL("Len of NA msg > 255");
            }

            nas_data_ind.nas_data.message_length = (uint8)
                rrcdt_int_data.rrcudt_ps_saved_data[count].data_req.nas_msg.length;

            WCDMA_MEMCPY(nas_data_ind.nas_data.payload, 
                         sizeof(uint8) * RRC_RR_MAX_GSM_MESSAGE_LENGTH,
                         rrcdt_int_data.rrcudt_ps_saved_data[count].data_req.nas_msg.value,
                         rrcdt_int_data.rrcudt_ps_saved_data[count].data_req.nas_msg.length );
    ...
    default:  
      WRRC_ERR1_FATAL("Invalid RRCUDT substate: %d ",rrcdt_int_data.rrcudt_substate);
      break;
   }   
}/* rrcudt_send_nas_data_ind */
/*===========================================================================
...
)
{
   if((rrc_rr_cmd_e_type)cmd_ptr->cmd_hdr.cmd_id == RRC_NAS_DATA_IND)
   {
      ...
      (void)rrcudt_process_g2w_nas_data_ind(wrrc_as_id, cmd_ptr);
   }
   else
   {
    switch(cmd_ptr->cmd_hdr.cmd_id)
    ...
    case RRC_DATA_REQ:
      ...
      if(RRCDT_SUCCESS != rrcudt_validate_data_req(wrrc_as_id,cmd_ptr))
      {
      ...
      if(RRCDT_SUCCESS != rrcudt_save_data_req(wrrc_as_id,cmd_ptr))
      {
        /* save udt message */
        /*lint -e666 */
      ...
      switch(rrc_get_state(wrrc_as_id))
      {
        ...
        case RRC_STATE_CELL_DCH:
          ...
          //is active, it registers for the complete and enters the substate
          //RRCIDT_WAIT_FOR_CELL_UPDATE_COMP_CNF. 
          if(RRCCU_NOT_COMPLETED == rrccu_get_cell_update_complete_status(wrrc_as_id,RRC_PROCEDURE_UDT) )
          {
            if(is_ue_oos_and_em_call_attempted(wrrc_as_id,cmd_ptr, RRC_PROCEDURE_UDT))
            {
              rrcdt_int_data.rrcudt_substate = RRCUDT_INITIAL;  
            }
            else
            {
              /* Wait until Cell update procedure is completed */
              rrcdt_int_data.rrcudt_substate = RRCUDT_WAIT_FOR_CELL_UPDATE_COMP_CNF;  
            }
          }
          else /* Cell Update procedure is inactive */
          {
        ...
        default:
          WRRC_MSG2_HIGH("UDT:Invalid Event: %d in the substate: %d", 
                   cmd_ptr->cmd_hdr.cmd_id,rrcdt_int_data.rrcudt_substate);
          break;
      }
      break;
    case RRC_NAS_DATA_TRANSFER_IND:
      rrcudt_send_nas_data_ind(wrrc_as_id);
      break;
    ...
    case RRC_IHO_OPEN_DOMAIN_IND:
      ...
      rrcudt_process_open_domain_ind_from_iho(wrrc_as_id, cmd_ptr);
      break;
  ...
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    ...
    case RRC_DATA_REQ:
      ...
      else
      {
        ...
        (void) rrcudt_save_data_req(wrrc_as_id,  cmd_ptr );
      }
      break;
  ...
  else
  {
   switch(cmd_ptr->cmd_hdr.cmd_id)
   {
    case RRC_DOWNLINK_L2ACK_IND:
      ...
      rb_id = rrclcm_get_ul_rb_id(wrrc_as_id,cmd_ptr->cmd.downlink_l2ack_ind.lc_id);

      if((rb_id < DCCH_DT_HIGH_PRI_RADIO_BEARER_ID) || 
         (rb_id > DCCH_DT_LOW_PRI_RADIO_BEARER_ID))
      {
        WRRC_ERR0_FATAL("L2 ACK routing is incorrect");
      }
      ...
      {
        if(SUCCESS == rrcudt_delete_saved_data_req(wrrc_as_id,(rrc_nas_msg_priority_e_type) nas_msg_priority,
                                                    cmd_ptr->cmd.downlink_l2ack_ind.mui))
        {

#ifdef FEATURE_WCDMA_UL_COMPR
          if((TRUE == rrcdt_nv_supports_ul_comp(wrrc_as_id)) && (rrcdt_int_data.rrc_ul_comp_cap_support) && 
             (rrcdt_int_data.rrc_ul_comp_cap_mui == cmd_ptr->cmd.downlink_l2ack_ind.mui))
          {
            rrcdt_int_data.rrc_ul_comp_cap_support = FALSE;
          }
#endif
          if(FAILURE == rrcudt_check_saved_mui_status(wrrc_as_id))
          {
            rrcdt_int_data.rrcudt_substate = RRCUDT_INITIAL;
          }
          else
      ...
      else /* L2 Failed to transmit the message */
      {
        ...
        if(cmd_ptr->cmd.downlink_l2ack_ind.failure_reason != WM_PURGE_REQ)
        {
          ...
          domain_id = rrcudt_get_cn_domain_for_mui(wrrc_as_id,cmd_ptr->cmd.downlink_l2ack_ind.mui);
          ...
          if(pending_count > 1)
          {
            ...
            rrcdt_purge_rlc_wm_rb3_rb4(wrrc_as_id,TRUE,FALSE,RRC_PROCEDURE_UDT);
          }
        }
        /* Re-transmission of the message */
        if(FAILURE == rrcudt_re_send_uplink_direct_transfer_msg(wrrc_as_id,cmd_ptr->cmd.downlink_l2ack_ind.mui, 
                                                                nas_msg_priority))
        {
          WRRC_MSG0_ERROR("Retransmission attempt failed");
          (void)rrcudt_delete_saved_data_req(wrrc_as_id,(rrc_nas_msg_priority_e_type) nas_msg_priority,
                                              cmd_ptr->cmd.downlink_l2ack_ind.mui);
        }
      }
      break;
...
)
{
  switch (rrcdt_int_data.rrcudt_substate)
  {
    ...
    default:           
      WRRC_MSG1_HIGH("UDT:Invalid substate: %d in DISCONNECTED", rrcdt_int_data.rrcudt_substate);
      break;
  }  
  ...
  switch (rrcdt_int_data.rrcudt_substate)
  {
    case RRCUDT_INITIAL:
      switch(cmd_ptr->cmd_hdr.cmd_id)
      {
        case RRC_DATA_REQ:
        case RRC_NAS_DATA_TRANSFER_IND:
          rrcudt_process_udt_initial_substate(wrrc_as_id, cmd_ptr);
          break;
        
        default:
          WRRC_MSG1_HIGH("UDT:Invalid event in Substate: %d",
                    rrcdt_int_data.rrcudt_substate);
          break;
      }
      break;
    ...
    case RRCUDT_WAIT_FOR_CELL_UPDATE_COMP_CNF: 
      switch(cmd_ptr->cmd_hdr.cmd_id)
      {
        ...
        case RRC_DATA_REQ:
        case RRC_NAS_DATA_TRANSFER_IND:
          rrcudt_process_udt_wait_for_cell_update_comp_cnf_substate(wrrc_as_id, cmd_ptr);
          break;
    ...
    case RRCUDT_WAIT_FOR_L2ACK:
      switch(cmd_ptr->cmd_hdr.cmd_id)
      {
        ...
        case RRC_DATA_REQ:
        case RRC_NAS_DATA_TRANSFER_IND:
          rrcudt_process_udt_wait_for_l2ack_substate(wrrc_as_id, cmd_ptr);
          break;
...
)
{
  ...
  rrc_state_e_type rrc_state = rrc_get_state(wrrc_as_id);
  WRRC_MSG1_HIGH_OPT("function rrcudt_procedure_event_handler is called in rrcudt_substate rrcudt_substate_e_type_value%d", rrcdt_int_data.rrcudt_substate);
  rrc_crash_debug_save_proc_substates(wrrc_as_id,RRC_PROCEDURE_UDT,rrcdt_int_data.rrcudt_substate,cmd_ptr->cmd_hdr.cmd_id);

  if(RRC_STATE_CHANGE_IND == cmd_ptr->cmd_hdr.cmd_id)
  {
    WRRC_MSG2_HIGH("State-Change-Ind udt-state %d in rrc-state %d",rrcdt_int_data.rrcudt_substate,rrc_state);
    //Any pending message memory needs to be de-allocated here 
    rrcudt_delete_all_saved_data_req(wrrc_as_id, RRC_CS_DOMAIN_CN_ID);
    rrcudt_delete_all_saved_data_req(wrrc_as_id, RRC_PS_DOMAIN_CN_ID);
    rrcdt_int_data.rrcudt_substate = RRCUDT_INITIAL;  
  ...
  else
  {
    switch (rrc_state)
    {
      case RRC_STATE_DISCONNECTED: 
        rrcudt_process_disconnected_state(wrrc_as_id);
        break;

      case RRC_STATE_CELL_FACH:    
        rrcudt_process_cell_fach_state(wrrc_as_id,cmd_ptr); 
        break;
      ...
      case RRC_STATE_CELL_DCH:  
         ...
         */   
        rrcudt_process_cell_dch_state(wrrc_as_id,cmd_ptr); 
        break;

      case RRC_STATE_CELL_PCH:
      case RRC_STATE_URA_PCH:
        rrcudt_process_cell_ura_pch_state(wrrc_as_id, cmd_ptr);
        break;
...
)
{
  ...
  if((cmd_ptr->cmd.close_sess_req.close_session_cause == RRC_UE_INITIATED_DORMANCY)&&
     (RRC_NOT_CAMPED_ON == rrc_ccm_get_curr_camping_status(wrrc_as_id,RRC_PROCEDURE_SCRR) )&&
     (!(curr_scan == RRC_CSP_CELL_DCH_TO_CELL_FACH_SCAN)))
  {
    ...
    */
    rrcscrr_send_close_session_cnf(wrrc_as_id,RRC_CLOSE_SESS_FAILURE_UE_INIT_DORMANCY_UE_IN_OOS, 
                                cmd_ptr->cmd.close_sess_req.cn_domain_id);  
...
#endif
  {
    ...
    if (domain_open == FALSE)
    {
      WRRC_MSG1_HIGH("CN Domain ID not open: %d", 
                cmd_ptr->cmd.close_sess_req.cn_domain_id);
      if (rrcidt_is_idt_buffered(wrrc_as_id,cmd_ptr->cmd.close_sess_req.cn_domain_id)== TRUE)
      {
        if(rrcidt_can_buffered_idt_be_deleted(wrrc_as_id,cmd_ptr->cmd.close_sess_req.cn_domain_id) == TRUE)
        {
          WRRC_MSG1_HIGH("Deleted IDT for domain %d & sending close cnf to NAS",cmd_ptr->cmd.close_sess_req.cn_domain_id);
    
          if(RRC_NOT_CAMPED_ON == rrc_ccm_get_curr_camping_status(wrrc_as_id,RRC_PROCEDURE_SCRR))
          {
            rrcscrr_send_close_session_cnf(wrrc_as_id,RRC_CLOSE_SESS_UE_OOS, 
                                       cmd_ptr->cmd.close_sess_req.cn_domain_id);  
          }
          else

          {
            rrcscrr_send_close_session_cnf(wrrc_as_id,RRC_CLOSE_SESS_SUCCESS, 
                                       cmd_ptr->cmd.close_sess_req.cn_domain_id);  
          }
      ...
      else if(cmd_ptr->cmd.close_sess_req.close_session_cause == RRC_UE_INITIATED_DORMANCY)
      {
        ...
        */
        rrcscrr_send_close_session_cnf(wrrc_as_id,RRC_CLOSE_SESS_SUCCESS_UE_INIT_DORMANCY, 
                                    cmd_ptr->cmd.close_sess_req.cn_domain_id);  
        status = RRCDT_FAILURE_MINOR;
      }
  #endif
      else
      {
        ...
        rrcscrr_send_close_session_cnf(wrrc_as_id,RRC_CLOSE_SESS_FAILURE, 
              cmd_ptr->cmd.close_sess_req.cn_domain_id);
        status = RRCDT_FAILURE_MINOR;
      }
    }
...
)
{
  ...
  {
    case RRC_CS_DOMAIN_CN_ID :
      rrcdt_int_data.rrcscrr_saved_cs_close_session_req.close_sess_req_status = FALSE;
      rrcdt_int_data.rrcscrr_saved_cs_close_session_req.mui_status = RRCSCRR_MUI_STATUS_FALSE;
      rrcdt_int_data.rrcscrr_saved_cs_close_session_req.rrc_sig_con_rel_status = FALSE;
      rrcdt_int_data.rrcscrr_saved_cs_close_session_req.rrc_proc = RRC_PROCEDURE_NONE;
    break;

    case RRC_PS_DOMAIN_CN_ID :
      rrcdt_int_data.rrcscrr_saved_ps_close_session_req.close_sess_req_status = FALSE;
      rrcdt_int_data.rrcscrr_saved_ps_close_session_req.mui_status = RRCSCRR_MUI_STATUS_FALSE;
      rrcdt_int_data.rrcscrr_saved_ps_close_session_req.rrc_sig_con_rel_status = FALSE;
      rrcdt_int_data.rrcscrr_saved_ps_close_session_req.rrc_proc = RRC_PROCEDURE_NONE;
      break;
    default:
      WRRC_MSG1_ERROR("Invalid CN Domain: %d" , cn_domain_id);
      break;
...
)
{
     ...
     (save_close_session_req_ptr->rrc_sig_con_rel_status == TRUE)) 
  {
    ...
    if (ul_lc_id == RRCLCM_RLC_LC_ID_NOT_FOUND)
    {
      ...
      rrcscrr_delete_close_session_req(wrrc_as_id,domain_id);
      return;
    }
    else
    {
      ...
      {
        case RRC_CS_DOMAIN_CN_ID :
          rrcsmc_update_ciphering_config_info_for_signalling_release(wrrc_as_id,domain_id);        
        ...
        default :
          ...
          WRRC_ERR1_FATAL("Invalid Domain ID %d",domain_id);
          break;
      }
      
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
      rrc_qsh_log_scri_params(wrrc_as_id, 0xFF,domain_id);
#endif

      /* Send the Signalling Connection Release Request Message on the Uplink DCCH */
      send_status = rrcsend_send_ul_sdu_with_mui(wrrc_as_id,RRC_PROCEDURE_SCRR,msg_ptr,ul_lc_id,
                                                 RRCSEND_L2ACK_REQUIRED, &msg_mui);
      if (send_status == RRCSEND_SUCCESS)
      {
        rrcdt_int_data.rrcscrr_substate = RRCSCRR_WAIT_FOR_L2ACK;
...
)
{
  switch(rrc_get_state(wrrc_as_id))
  {
    ...
    case RRC_STATE_CELL_PCH:
      ...
      if(rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_T_320_TIMER))
      {
        /* Register for CU complete. Upon CUcomplete IND, SCRI will be sent */
        rrccu_set_cu_cause_for_wait_timer(wrrc_as_id, RRC_UL_DATA_TRANSMISSION);
        rrccu_register_for_cell_update_to_complete(wrrc_as_id, RRC_PROCEDURE_SCRR);
      }
      /* If CU is already started, Update the CU cause to UL data ind and 
         register for complete notification*/
      else if(RRCCU_STARTED == rrccu_get_cell_update_started_status(wrrc_as_id))
      {
        rrccu_set_cu_cause_to_ul_data_indication(wrrc_as_id);
        rrccu_register_for_cell_update_to_complete(wrrc_as_id, RRC_PROCEDURE_SCRR);
      }
      else
      {
        rrcdt_send_cell_update_initiate_req(wrrc_as_id, RRC_PROCEDURE_SCRR);
      }
      rrcdt_int_data.rrcscrr_substate = RRCSCRR_WAIT_FOR_CELL_UPDATE_COMP_CNF;  
      break;
    ...
    * is active, it registers for the complete and enters the substate
    * RRCIDT_WAIT_FOR_CELL_UPDATE_COMP_CNF */ 
      if(RRCCU_NOT_COMPLETED == rrccu_get_cell_update_complete_status(wrrc_as_id,RRC_PROCEDURE_SCRR) )
      {
        /* Wait until Cell update procedure is completed */
        rrcdt_int_data.rrcscrr_substate = RRCSCRR_WAIT_FOR_CELL_UPDATE_COMP_CNF;  
      }
#ifdef FEATURE_WCDMA_HS_RACH
      else if(TRUE == rrccu_get_hsrach_sib_change(wrrc_as_id))
      { 
        rrcrb_send_cell_update_initiate_req(wrrc_as_id,RRC_PROCEDURE_SCRR,RRC_CELL_RESELECTION,TRUE);
        rrcdt_int_data.rrcscrr_substate = RRCSCRR_WAIT_FOR_CELL_UPDATE_COMP_CNF; 
      }
#endif
      ...
      else if(rrc_check_if_proc_waiting_for_l2_ack_for_pch_transition(wrrc_as_id))
      {
        rrcdt_int_data.rrcscrr_substate = RRCSCRR_WAIT_FOR_CELL_UPDATE_COMP_CNF;  
        rrccu_register_for_cell_update_to_complete(wrrc_as_id, RRC_PROCEDURE_SCRR);
      }
      else /* Cell Update procedure is inactive */
      {
         ...
         rrcscrr_send_sig_conn_rel_req(wrrc_as_id,domain_id);
      }
      break;

    case RRC_STATE_CELL_DCH:
      if(rrc_check_if_proc_waiting_for_l2_ack_for_pch_transition(wrrc_as_id))
      {
        rrcdt_int_data.rrcscrr_substate = RRCSCRR_WAIT_FOR_CELL_UPDATE_COMP_CNF;  
        rrccu_register_for_cell_update_to_complete(wrrc_as_id, RRC_PROCEDURE_SCRR);
      }
    ...
    default:
      WRRC_MSG1_HIGH("SCRR:Invalid Event in rrc state: %d",rrc_get_state(wrrc_as_id));
      break;
  }
}
...
)
{
  ...
  {
    case RRC_CS_DOMAIN_CN_ID :
      if (( rrcdt_int_data.rrcscrr_saved_cs_close_session_req.close_sess_req_status == FALSE )&&
          ( rrcdt_int_data.rrcscrr_saved_cs_close_session_req.rrc_sig_con_rel_status == FALSE) &&
          (rrcdt_int_data.rrcscrr_saved_cs_close_session_req.mui_status == RRCSCRR_MUI_STATUS_FALSE)
         )
      {
        ...
        rrcdt_int_data.rrcscrr_saved_cs_close_session_req.rrc_proc = 
    ...
    case RRC_PS_DOMAIN_CN_ID :
      if (( rrcdt_int_data.rrcscrr_saved_ps_close_session_req.close_sess_req_status == FALSE ) &&
          ( rrcdt_int_data.rrcscrr_saved_ps_close_session_req.rrc_sig_con_rel_status == FALSE ) &&
          ( rrcdt_int_data.rrcscrr_saved_ps_close_session_req.mui_status == RRCSCRR_MUI_STATUS_FALSE)
         )
      {
        rrcdt_int_data.rrcscrr_saved_ps_close_session_req.rrc_sig_con_rel_status = TRUE;
        rrcdt_int_data.rrcscrr_saved_ps_close_session_req.rrc_proc = 
          cmd_ptr->cmd.rrc_sig_conn_req.rrc_proc;
        save_status = RRCDT_SUCCESS;
      }
      else
...
)
{
  ...
  switch (cmd_ptr->cmd.close_sess_req.cn_domain_id)
  {
    case RRC_CS_DOMAIN_CN_ID :
      ...
      {
        rrcdt_int_data.rrcscrr_saved_cs_close_session_req.close_sess_req_status = TRUE;
        save_status = RRCDT_SUCCESS;
      }
      else
      {
    ...
    case RRC_PS_DOMAIN_CN_ID :
      ...
      {
        rrcdt_int_data.rrcscrr_saved_ps_close_session_req.close_sess_req_status = TRUE;
        save_status = RRCDT_SUCCESS;
      }
      else
      {
...
)
{
...
#ifdef FEATURE_WCDMA_FAST_RETURN_TO_LTE_AFTER_CSFB
  WRRC_MSG7_HIGH(
    "CSFB_HACK: csfb_extended_status = %d,CSFB for SS is in progress = %d, RAU is in Progress = %d,CSFB fast return to LTE for SS NV enabled = %d, close session for domain %d, PS domain status %d Lte mode enabled status %d ",
    rrc_csp_int_data.csfb_extended_status,mm_per_subs_is_ss_in_progress(wrrc_as_id),gmm_per_subs_is_rau_in_progress(wrrc_as_id),rrcnv_data.rrc_fast_return_to_lte_after_csfb_ss_nv,cn_domain_id,rrcdt_int_data.ps_domain_open, rrcmcm_is_lte_mode_enabled(wrrc_as_id)
    );

  if((RRC_CLOSE_SESS_SUCCESS == session_status) && (rrc_csp_int_data.csfb_extended_status == CSFB_STARTED) && (!(rrccsp_block_frlte_for_curr_camped_plmn(wrrc_as_id)))
  && (cn_domain_id == RRC_CS_DOMAIN_CN_ID) && ((rrc_csp_int_data.cs_rab_established) || (((mm_per_subs_is_ss_in_progress(wrrc_as_id)) && (rrcnv_data.rrc_fast_return_to_lte_after_csfb_ss_nv)) && !(gmm_per_subs_is_rau_in_progress(wrrc_as_id)))))
  {
    /* If LTE mode is not enabled reset csfb extended status such that blind direction doesn't happen */
    if(FALSE == rrcmcm_is_lte_mode_enabled(wrrc_as_id))
    {
  ...
  if( (cmd_ptr =  mm_rrc_get_cmd_buf(RRC_CLOSE_SESSION_CNF)) != NULL)
  {
    ...
    rrcsmc_update_sync_ind_info_for_signalling_release(wrrc_as_id,cn_domain_id);

    cmd_ptr->cmd.rrc_close_session_cnf.status = session_status;

#ifdef FEATURE_DUAL_SIM
    cmd_ptr->cmd.rrc_close_session_cnf.as_id = rrc_get_as_id(wrrc_as_id);
#endif

    rrc_send_rrc_message_to_mm(wrrc_as_id,cmd_ptr);               /* Sends the command to MM */

    rrccho_indicate_ps_close(wrrc_as_id);
...
)
{
  ...
  if( (cmd_ptr =  rrc_get_int_cmd_buf(wrrc_as_id)) != NULL)
  {
    ...
    rrc_put_int_cmd(wrrc_as_id, cmd_ptr);                    /* Sends the command to MM */
  ...
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    ...
    case RRC_SIG_CONN_RELEASE_REQ:
      ...
      if(cmd_ptr->cmd_hdr.cmd_id == RRC_CLOSE_SESSION_REQ)
      {
        ...
        /* Assume that the domain associated with the flow ID is closed */
        if(RRCDT_SUCCESS != rrcscrr_validate_close_session_req(wrrc_as_id,cmd_ptr))
        {
          /*Moved sending Close Session Cnf in rrcscrr_validate_close_session_req function*/
          return;
        }
      
        if(RRCDT_SUCCESS != rrcscrr_save_close_session_req(wrrc_as_id,cmd_ptr))
        {
          /* If there are multiple requests,then just trash it */
          /*lint -e666*/
          WRRC_MSG2_HIGH("Failed to Save Close Sess Req in: %d, %d",
                   rrc_get_state(wrrc_as_id),rrcdt_int_data.rrcscrr_substate);
          /*lint +e666*/
      ...
      else if(cmd_ptr->cmd_hdr.cmd_id == RRC_SIG_CONN_RELEASE_REQ)
      {
        ...
        if(RRCDT_SUCCESS != rrcscrr_save_sig_release_req(wrrc_as_id,cmd_ptr))
        {
      ...
      if(FALSE == rrcidt_is_idt_active(wrrc_as_id,domain_id))
      {
        rrcscrr_process_sig_conn_rel_req(wrrc_as_id,domain_id);
      }
      else
    ...
    default:
      WRRC_MSG2_HIGH("Invalid Event: %d for SCRR in substate:%d", 
                cmd_ptr->cmd_hdr.cmd_id,rrcdt_int_data.rrcscrr_substate);
      break;
  }/*end of switch stmt*/
  ...
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_CELL_UPDATE_COMPLETE_CNF:
         ...
         (rrcdt_int_data.rrcscrr_saved_cs_close_session_req.mui_status == RRCSCRR_MUI_STATUS_FALSE))
      {
        ...
        rrcscrr_send_sig_conn_rel_req(wrrc_as_id, RRC_CS_DOMAIN_CN_ID);
      }
         ...
         (rrcdt_int_data.rrcscrr_saved_ps_close_session_req.mui_status == RRCSCRR_MUI_STATUS_FALSE))
      {
        ...
        rrcscrr_send_sig_conn_rel_req(wrrc_as_id, RRC_PS_DOMAIN_CN_ID);
      }
      break;
...
)
{
  ...
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_DOWNLINK_L2ACK_IND:
      ...
      if((rrcdt_int_data.rrcscrr_saved_cs_close_session_req.mui_status != RRCSCRR_MUI_STATUS_FALSE) && 
         (cmd_ptr->cmd.downlink_l2ack_ind.mui == rrcdt_int_data.rrcscrr_saved_cs_close_session_req.mui))
      {
        if(rrcdt_int_data.rrcscrr_saved_cs_close_session_req.mui_status == RRCSCRR_MUI_CLOSE_SESSION_CNF)
        {
        ...
        else if(rrcdt_int_data.rrcscrr_saved_cs_close_session_req.mui_status == RRCSCRR_MUI_SIG_CON_REL_CNF)
        {
      ...
      else if((rrcdt_int_data.rrcscrr_saved_ps_close_session_req.mui_status != RRCSCRR_MUI_STATUS_FALSE) && 
              (cmd_ptr->cmd.downlink_l2ack_ind.mui == rrcdt_int_data.rrcscrr_saved_ps_close_session_req.mui))
      {
        //check if the sig_conn_rel_ind was for close_session_req or sig_con_rel_req
        //and send the appropriate RSP to RCE
        if(rrcdt_int_data.rrcscrr_saved_ps_close_session_req.mui_status == RRCSCRR_MUI_CLOSE_SESSION_CNF)
        {
        ...
        else if(rrcdt_int_data.rrcscrr_saved_ps_close_session_req.mui_status == RRCSCRR_MUI_SIG_CON_REL_CNF)
        {
          //send cnf based on L2-ACK status
          if(SUCCESS == cmd_ptr->cmd.downlink_l2ack_ind.status)
          {
    ...
    case RRC_SIG_CONN_RELEASE_REQ:
      if(cmd_ptr->cmd_hdr.cmd_id == RRC_CLOSE_SESSION_REQ)
      {
        ...
        if(RRCDT_SUCCESS != rrcscrr_save_close_session_req(wrrc_as_id,cmd_ptr))
        {
          ...
          rrcscrr_send_close_session_cnf(wrrc_as_id,RRC_CLOSE_SESS_FAILURE,cmd_ptr->cmd.close_sess_req.cn_domain_id);
          return;
        }
      }
      else if(cmd_ptr->cmd_hdr.cmd_id == RRC_SIG_CONN_RELEASE_REQ)
      ...
      the SigConnRelInd out.This is to avoid a race condition where IDT reaches
      the network after SCRR while the reverse was intended */
      if(FALSE == rrcidt_is_idt_active(wrrc_as_id,local_domain_id))
      {
        rrcscrr_send_sig_conn_rel_req(wrrc_as_id,local_domain_id);
      }
      else
      {
...
===========================================================================*/
uint8 rrcscrr_is_cn_domain_open(sys_modem_as_id_e_type wrrc_as_id,rrc_cn_domain_identity_e_type cn_domain)
{
  ...
  if(cn_domain == RRC_CS_DOMAIN_CN_ID)
  {
    if(rrcdt_int_data.cs_domain_open == TRUE)
    {
      ...
      if((rrcdt_int_data.rrcscrr_saved_cs_close_session_req.mui_status == RRCSCRR_MUI_SIG_CON_REL_CNF)||
         (rrcdt_int_data.rrcscrr_saved_cs_close_session_req.mui_status == RRCSCRR_MUI_CLOSE_SESSION_CNF))
      {
  ...
  else if(cn_domain == RRC_PS_DOMAIN_CN_ID)
  {
    ...
    {
      /*Checking if signalling Release in progress*/
      if((rrcdt_int_data.rrcscrr_saved_ps_close_session_req.mui_status == RRCSCRR_MUI_SIG_CON_REL_CNF)||
         (rrcdt_int_data.rrcscrr_saved_ps_close_session_req.mui_status == RRCSCRR_MUI_CLOSE_SESSION_CNF))
      {
        /*Since DT has sent Signalling connection Release Request to UTRAN & waiting for the
        L2-ACK, So discard the DDT message in this sall window*/
        WRRC_MSG1_HIGH("Signalling connection Release is in progress for cn domain =%d",cn_domain);
...
{
  switch (rrcdt_int_data.rrcscrr_substate)
  {
    case RRCSCRR_INITIAL:
      switch(cmd_ptr->cmd_hdr.cmd_id)
      {
        case RRC_CLOSE_SESSION_REQ:
        case RRC_SIG_CONN_RELEASE_REQ:
          rrcscrr_process_scrr_initial_substate(wrrc_as_id, cmd_ptr);
          break;
    ...
    case RRCSCRR_WAIT_FOR_CELL_UPDATE_COMP_CNF: 
      switch(cmd_ptr->cmd_hdr.cmd_id)
      {
        ...
        case RRC_CLOSE_SESSION_REQ:
        case RRC_SIG_CONN_RELEASE_REQ:
          rrcscrr_process_scrr_wait_for_cell_update_comp_cnf_substate(wrrc_as_id, cmd_ptr);
          break;
    ...
    case RRCSCRR_WAIT_FOR_L2ACK: 
      switch(cmd_ptr->cmd_hdr.cmd_id)
      {
        ...
        case RRC_CLOSE_SESSION_REQ:
        case RRC_SIG_CONN_RELEASE_REQ:
          rrcscrr_process_scrr_wait_for_l2_ack_substate(wrrc_as_id, cmd_ptr);
          break;
...
)
{
  ...
  WRRC_MSG1_HIGH_OPT("function rrcscrr_procedure_event_handler is called in rrcscrr_substate rrcscrr_substate_e_type_value%d", rrcdt_int_data.rrcscrr_substate);
  rrc_crash_debug_save_proc_substates(wrrc_as_id,RRC_PROCEDURE_SCRR,rrcdt_int_data.rrcscrr_substate,cmd_ptr->cmd_hdr.cmd_id);

  if(RRC_STATE_CHANGE_IND == cmd_ptr->cmd_hdr.cmd_id)
  {
    WRRC_MSG2_HIGH("State-Change-Ind scrr-state %d in rrc-state %d",rrcdt_int_data.rrcudt_substate,rrc_state);
    //Any pending message memory needs to be de-allocated here 
    rrcscrr_delete_close_session_req(wrrc_as_id, RRC_CS_DOMAIN_CN_ID);
    rrcscrr_delete_close_session_req(wrrc_as_id, RRC_PS_DOMAIN_CN_ID);
    rrcdt_int_data.rrcscrr_substate = RRCSCRR_INITIAL;  
  ...
  else
  {
      ...
      {
        if(TRUE == rrcdt_update_scrr_to_handle_dormancy(wrrc_as_id,cmd_ptr))
          return;
      }
    ...
    {
      case RRC_STATE_CELL_FACH:    
        rrcscrr_process_cell_fach_state(wrrc_as_id,cmd_ptr); 
        break;

      case RRC_STATE_CELL_DCH:  
        rrcscrr_process_cell_dch_state(wrrc_as_id,cmd_ptr); 
        break;

      case RRC_STATE_CELL_PCH:
      case RRC_STATE_URA_PCH:
        rrcscrr_process_cell_ura_pch_state(wrrc_as_id, cmd_ptr);
        break;
...
)
{
     ...
     (cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_type == rrc_DL_DCCH_Message_PDU) )
  {
        ...
        T_rrc_DL_DCCH_MessageType_downlinkDirectTransfer)
    {
      ...
      if(rrcdata_misc_info.rrc_protocol_error_reject == TRUE) /* DCCH ASN1 decoding failed */
      {
      ...
      else /* ASN1 decoding is successful */
      {
        ...
        if( (( rrc_CN_DomainIdentity_cs_domain ==
               ...
               (rrcscrr_is_cn_domain_open(wrrc_as_id,RRC_CS_DOMAIN_CN_ID))) || 
            (( rrc_CN_DomainIdentity_ps_domain == 
               dcch_msg_ptr->message.u.downlinkDirectTransfer.u.r3.
               downlinkDirectTransfer_r3.cn_DomainIdentity) &&
               (rrcscrr_is_cn_domain_open(wrrc_as_id,RRC_PS_DOMAIN_CN_ID))) )
        {        
          ...
          else /* Valid Message Length */
          {
            ...
            {
              rrcdt_int_data.rrc_ul_comp_cap_support = TRUE;
              rrcdt_int_data.rrc_ul_comp_cap_mui = 0xFF;
              WRRC_MSG0_HIGH("RRC_ULCOMP: Enabling rrc_ul_comp_cap_support flag");
            }
            else 
            {
              WRRC_MSG0_MED("RRC_ULCOMP: Not enabling rrc_ul_comp_cap_support flag");   
            }
#endif
            /* Check whether Security mode procedure is active */
            if (rrcsmc_get_smc_procedure_complete_status(wrrc_as_id,cn_id, rrclcm_get_dl_rb_id(wrrc_as_id,cmd_ptr->cmd.downlink_sdu_ind.lc_id)) == FALSE)
            {
              ...
              if (payload != NULL)
              {
                ...
                if( (mm_cmd_ptr =  mm_rrc_get_cmd_buf(RRC_DATA_IND)) != NULL)
                {
...
#ifdef FEATURE_DUAL_SIM
                  mm_cmd_ptr->cmd.rrc_data_ind.as_id = rrc_get_as_id(wrrc_as_id);
#endif
                  ...
                  else
                  {
                    ...
                    {
                      WRRC_MSG3_HIGH("Restarting Inactivity timer %d + remain T305 %d T305 actual val %d",INACTIVITY_TIMER_IN_MS,
                  ...
                  if(rrcidt_is_idt_waiting_for_l2ack_per_domain(wrrc_as_id,mm_cmd_ptr->cmd.rrc_data_ind.cn_domain_id))
                  {
                      WRRC_MSG1_HIGH("Treating DDT as Implicit L2 Ack for IDT in %d Domain", mm_cmd_ptr->cmd.rrc_data_ind.cn_domain_id);
                      rrcidt_send_open_session_cnf(wrrc_as_id,RRC_OPEN_SESS_SUCCESS,  mm_cmd_ptr->cmd.rrc_data_ind.cn_domain_id);
            ...
            else /* Security mode procedure is active */
            {
              ...
              {
                if(rrcdt_int_data.rrcddt_saved_data[count].data_ind_status == FALSE)
                {
                  break;
                }
              }
                           ...
                           MAX_NAS_MESSAGES_FOR_PS_DOMAIN) )
              {
                if( rrc_CN_DomainIdentity_cs_domain ==
                    ...
                    downlinkDirectTransfer_r3.cn_DomainIdentity)
                {
                  rrcdt_int_data.rrcddt_saved_data[count].data_ind.cn_domain_id = RRC_CS_DOMAIN_CN_ID;
                }
                else if(rrc_CN_DomainIdentity_ps_domain ==
                  dcch_msg_ptr->message.u.downlinkDirectTransfer.u.r3.
                  downlinkDirectTransfer_r3.cn_DomainIdentity)
                {
                  rrcdt_int_data.rrcddt_saved_data[count].data_ind.cn_domain_id = RRC_PS_DOMAIN_CN_ID;
                }
                else
                {
                  WRRC_ERR0_FATAL("Invalid CN domain Id");
                }
                /* Update the length of NAS message */

                rrcdt_int_data.rrcddt_saved_data[count].data_ind.nas_msg.length = 
                  dcch_msg_ptr->message.u.downlinkDirectTransfer.u.r3.
                  downlinkDirectTransfer_r3.nas_Message.numocts;

                /* Allocate memory for NAS messages */
                rrcdt_int_data.rrcddt_saved_data[count].data_ind.nas_msg.value = 
             (unsigned char *) nas_mem_alloc(
                                    dcch_msg_ptr->message.u.
                  downlinkDirectTransfer.u.r3.downlinkDirectTransfer_r3.nas_Message.numocts);
                /* Memory allocation is sucessful. Copy the payload */
                WCDMA_MEMCPY(rrcdt_int_data.rrcddt_saved_data[count].data_ind.nas_msg.value,
                             dcch_msg_ptr->message.u.downlinkDirectTransfer.u.r3.downlinkDirectTransfer_r3.nas_Message.numocts,
                             dcch_msg_ptr->message.u.downlinkDirectTransfer.u.r3.downlinkDirectTransfer_r3.nas_Message.data,
                             dcch_msg_ptr->message.u.downlinkDirectTransfer.u.r3.downlinkDirectTransfer_r3.nas_Message.numocts);

                /* Set the status Flag to TRUE indicating that message is saved*/
                rrcdt_int_data.rrcddt_saved_data[count].data_ind_status = TRUE;
              }
  ...
  else if(RRC_SMC_DDT_L2_ACK_IND == cmd_ptr->cmd_hdr.cmd_id)
  {
                                   ...
                                   MAX_NAS_MESSAGES_FOR_PS_DOMAIN); count++)
    {
      ...
      if(rrcdt_int_data.rrcddt_saved_data[count].data_ind_status == TRUE)
      {
        /* Allocate memory for RRC_DATA_IND command */
        if( (mm_cmd_ptr =  mm_rrc_get_cmd_buf(RRC_DATA_IND)) != NULL)
        {
          ...
          mm_cmd_ptr->cmd.rrc_data_ind.cn_domain_id = 
            rrcdt_int_data.rrcddt_saved_data[count].data_ind.cn_domain_id;
          ...
          rrcdt_int_data.rrcddt_saved_data[count].data_ind_status = FALSE;
        }
      } /* Check if any messages are queued */ 
    }
...
)
{
  ...
  rrc_crash_debug_save_proc_substates(wrrc_as_id,RRC_PROCEDURE_DDT,RRC_INVALID_PROC_SUBSTATE,cmd_ptr->cmd_hdr.cmd_id);
  ...
  if(cmd_ptr->cmd_hdr.cmd_id == RRC_STATE_CHANGE_IND)
  {
                             ...
                             MAX_NAS_MESSAGES_FOR_PS_DOMAIN); count++)
    {
      if(rrcdt_int_data.rrcddt_saved_data[count].data_ind_status == TRUE)
      {
        modem_mem_free(rrcdt_int_data.rrcddt_saved_data[count].data_ind.nas_msg.value, 
        MODEM_MEM_CLIENT_NAS_CRIT);
        rrcdt_int_data.rrcddt_saved_data[count].data_ind.nas_msg.value = NULL;
        rrcdt_int_data.rrcddt_saved_data[count].data_ind_status = FALSE;
      }
    }
  }
  else
  {
    switch (rrc_state)
    {
      ...
      case RRC_STATE_CELL_DCH:  
        ...
        rrcddt_process_downlink_direct_transfer_msg(wrrc_as_id,cmd_ptr); 
        break;
      ...
      default: 
        ...
        WRRC_ERR1_FATAL("Event fwded to DDT in invalid State: %d",rrc_state);
        break;
    }  
  }
} /* rrcddt_procedure_event_handler */
...
)
{
...
#ifdef FEATURE_WCDMA_FAST_RETURN_TO_LTE_AFTER_CSFB
  WRRC_MSG6_HIGH("CSFB_HACK: csfb_extended_status = %d,CSFB for SS is in progress = %d, RAU is in progress = %d,CSFB fast return to LTE for SS NV enabled = %d, close session for domain %d, PS domain status %d",
          rrc_csp_int_data.csfb_extended_status,mm_per_subs_is_ss_in_progress(wrrc_as_id),gmm_per_subs_is_rau_in_progress(wrrc_as_id),rrcnv_data.rrc_fast_return_to_lte_after_csfb_ss_nv,cn_domain_id,rrcdt_int_data.ps_domain_open);

  if((rrc_csp_int_data.csfb_extended_status == CSFB_STARTED) &&
      (cn_domain_id == RRC_CS_DOMAIN_CN_ID) && (!(rrccsp_block_frlte_for_curr_camped_plmn(wrrc_as_id)))
       && ((rrc_csp_int_data.cs_rab_established) || (((mm_per_subs_is_ss_in_progress(wrrc_as_id)) && (rrcnv_data.rrc_fast_return_to_lte_after_csfb_ss_nv)) && !(gmm_per_subs_is_rau_in_progress(wrrc_as_id))))
       ...
       )
  {
    ...
    {
    rrcrce_post_internal_csfb_abort_cmd(wrrc_as_id);
    WRRC_MSG1_HIGH("CSFB_HACK: csfb_extended_status = %d",rrc_csp_int_data.csfb_extended_status);
    rrc_csp_int_data.cs_rab_established = FALSE;
    return RRCDT_SUCCESS;
    }
  }
  ...
  if( (cmd_ptr =  mm_rrc_get_cmd_buf(RRC_CLOSE_SESSION_IND)) != NULL)
  {
    ...
    rrcsmc_update_ciphering_config_info_for_signalling_release(wrrc_as_id,cn_domain_id);
    rrcsmc_update_sync_ind_info_for_signalling_release(wrrc_as_id,cn_domain_id);
...
#ifdef FEATURE_DUAL_SIM
    cmd_ptr->cmd.rrc_close_session_ind.as_id = rrc_get_as_id(wrrc_as_id);
#endif
      ...
      else /* ASN1 decoding is successful */
      {
...
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
        rrc_qsh_log_scrr_params(wrrc_as_id, cn_domain_id);
#endif

        if(rrcscr_check_active_rabs_in_sig_conn_rel_domain(wrrc_as_id,cn_domain_id) == 
        ...
        else//only if active rab is not found in CN Domain we r trying to release
        {
          ...
          {        
            ...
            else if(RRCDT_SUCCESS == rrcscr_send_close_session_ind(wrrc_as_id,cn_domain_id, RRC_MAX_SESSION_CLOSE_CAUSE))
            {
              WRRC_MSG0_HIGH("Sent RRC_CLOSE_SESSION_IND");
              switch(cn_domain_id)
              {
                case RRC_CS_DOMAIN_CN_ID:
                  rrcidt_clear_idt_data_for_race_conditions(wrrc_as_id, RRC_CS_DOMAIN_CN_ID);
                  /* Set the Signalling Connection status to closed */
                  rrcdt_int_data.cs_domain_open = FALSE;
                  break;
                ...
                case RRC_PS_DOMAIN_CN_ID:
                  /* Set the Signalling Connection Status to closed */
                  rrcidt_clear_idt_data_for_race_conditions(wrrc_as_id, RRC_PS_DOMAIN_CN_ID);
                  rrcdt_int_data.ps_domain_open = FALSE;
                ...
                default:
                  WRRC_ERR0_FATAL("This should never_happen");
                  break;
              }
            }
            else
...
)
{
  ...
  rrc_crash_debug_save_proc_substates(wrrc_as_id,RRC_PROCEDURE_SCR,RRC_INVALID_PROC_SUBSTATE,cmd_ptr->cmd_hdr.cmd_id);
  ...
  switch (rrc_state)
  {
    ...
    case RRC_STATE_CELL_DCH:  
      /* These two states are valid to receive Downlink Direct Transfer message */
      rrcscr_process_signaling_connection_release_msg(wrrc_as_id,cmd_ptr); 
      break;
...
)
{
  ...
  switch(cn_domain_id)
  {
    case RRC_CS_DOMAIN_CN_ID:
      ...
      */
      if((rrcdt_int_data.cs_domain_open == TRUE) &&
         (rrcidt_is_idt_active(wrrc_as_id,RRC_CS_DOMAIN_CN_ID) == FALSE))
      {
        if(RRCDT_SUCCESS == rrcscr_indicate_close_session(wrrc_as_id,cn_domain_id, RRC_MAX_SESSION_CLOSE_CAUSE))
        {
      ...
      {
        WRRC_MSG3_HIGH("No need to close Doamin =%d,domain_open=%d,IDT active =%d",
           cn_domain_id,rrcdt_int_data.cs_domain_open,rrcidt_is_idt_active(wrrc_as_id, RRC_CS_DOMAIN_CN_ID));
      }
    ...
    case RRC_PS_DOMAIN_CN_ID:
      ...
      */
      if((rrcdt_int_data.ps_domain_open == TRUE)&&
         (rrcidt_is_idt_active(wrrc_as_id,RRC_PS_DOMAIN_CN_ID) == FALSE))
      {
        if(RRCDT_SUCCESS == rrcscr_indicate_close_session(wrrc_as_id,cn_domain_id, RRC_MAX_SESSION_CLOSE_CAUSE))
        {
      ...
      {
        WRRC_MSG3_HIGH("No need to close Doamin =%d,domain_open=%d,IDT active =%d",
           cn_domain_id,rrcdt_int_data.ps_domain_open,rrcidt_is_idt_active(wrrc_as_id, RRC_PS_DOMAIN_CN_ID));
      }
      break;
...
boolean rrcdt_is_idt_waitingfor_l2ack(sys_modem_as_id_e_type wrrc_as_id)
{

   boolean ret_val=FALSE;
   if(rrcdt_int_data.rrcidt_substate == RRCIDT_WAIT_FOR_L2ACK)
   {
     ret_val =  TRUE;
   }
   return (ret_val);
...
)
{
   ...
   * take UE to idle
   */
  if(RRC_NOT_CAMPED_ON == rrc_ccm_get_curr_camping_status(wrrc_as_id,rrc_proc))
  {
    switch (rrc_proc) 
    {
      case RRC_PROCEDURE_IDT:
        if(cmd_ptr->cmd.open_sess_req.est_cause == RRC_EST_EMERGENCY_CALL)
        {
          ...
          (void)rrc_transition_to_disconnected_state(wrrc_as_id,RRC_PROCEDURE_IDT, RRC_TX_TO_DISCON_EM_CALL);
      ...
      case RRC_PROCEDURE_UDT:
        if(cmd_ptr->cmd.data_req.is_emergency_call == TRUE)
        {
          ...
          /* Send initiate cell sel command to CSP so that UE is taken to Idle */
          (void)rrc_transition_to_disconnected_state(wrrc_as_id,RRC_PROCEDURE_UDT, RRC_TX_TO_DISCON_EM_CALL);
...
/*Using this function to update misc global to identify if EM call was trigerred. This is for MAV requiremnt to block FRLTE for E call scenarios*/
  if(rrcnv_data.rrc_delay_return_to_lte_nv.nv_enabled)
  {
    switch (rrc_proc) 
      {
        case RRC_PROCEDURE_IDT:
          if(cmd_ptr->cmd.open_sess_req.est_cause == RRC_EST_EMERGENCY_CALL)
          {
            WRRC_MSG1_HIGH("FRLTE_DELAY_ENH: Setting Emergency call true for PROC %d", rrc_proc);
            rrcmisc_int_data.is_emergency_call = TRUE;
          }
          break;
        ...
        default: 
          WRRC_MSG1_ERROR("FRLTE_DELAY_ENH:  ERR: Incorrect proc_id %d passed to the func", rrc_proc);
          break;
      }/* end switch statement */
  }
  return (status);
...
void rrcdt_init_procedure(sys_modem_as_id_e_type wrrc_as_id)

{
  ...
  rrcdt_int_data.rrcidt_cs_saved_open_session_req.selected_plmn_id.mcc[0]= 0;
  rrcdt_int_data.rrcidt_cs_saved_open_session_req.selected_plmn_id.mcc[1]= 0;
  rrcdt_int_data.rrcidt_cs_saved_open_session_req.selected_plmn_id.mcc[2]= 0;
  rrcdt_int_data.rrcidt_cs_saved_open_session_req.selected_plmn_id.mnc[0]= 0;
  rrcdt_int_data.rrcidt_cs_saved_open_session_req.selected_plmn_id.mnc[1]= 0;
  rrcdt_int_data.rrcidt_cs_saved_open_session_req.selected_plmn_id.mnc[2]= 0;
  rrcdt_int_data.rrcidt_cs_saved_open_session_req.selected_plmn_id.num_mnc_digits = 0;
  rrcdt_int_data.rrcidt_ps_saved_open_session_req.selected_plmn_id = 
      rrcdt_int_data.rrcidt_cs_saved_open_session_req.selected_plmn_id;
  rrcdt_int_data.rrcidt_est_cause_info.valid = FALSE;
  rrcdt_int_data.rrcidt_est_cause_info.est_cause = RRC_EST_TERM_UNKNOWN;
  rrcdt_int_data.rrcidt_est_cause_info.to_be_sent_by = RRC_PROCEDURE_NONE;
  rrcdt_int_data.rrcidt_est_cause_info.last_proc_sent = RRC_PROCEDURE_NONE;
...
===========================================================================*/
boolean rrcdt_check_if_emergency_call_initiated(sys_modem_as_id_e_type wrrc_as_id)
{
  boolean status=FALSE;
  int i;
  ...
  if(((rrcdt_int_data.rrcidt_cs_saved_open_session_req.mui_status== TRUE) ||
     (rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_sess_req_status == TRUE))
     &&
     (rrcdt_int_data.rrcidt_cs_saved_open_session_req.est_cause==RRC_EST_EMERGENCY_CALL)
    )
  {
    WRRC_MSG3_HIGH("IDT is in process for Ecall mui_status=%d,open_sess_req_status =%d, est_cause =%d",rrcdt_int_data.rrcidt_cs_saved_open_session_req.mui_status,
                                                                                  rrcdt_int_data.rrcidt_cs_saved_open_session_req.open_sess_req_status,
                                                                                  rrcdt_int_data.rrcidt_cs_saved_open_session_req.est_cause);
    return (TRUE);
  }
  /*Check to see if is_emergency_call is set in any any UDT message*/
  if(rrcdt_int_data.rrcudt_substate == RRCUDT_WAIT_FOR_L2ACK)
  {
    for(i=0; i < MAX_NAS_MESSAGES_FOR_CS_DOMAIN ;i++)
    {
      if(((rrcdt_int_data.rrcudt_cs_saved_data[i].mui_status == TRUE) ||
         (rrcdt_int_data.rrcudt_cs_saved_data[i].data_req_status == TRUE))
         &&
         (rrcdt_int_data.rrcudt_cs_saved_data[i].data_req.is_emergency_call == TRUE)
        )
      {
        WRRC_MSG3_HIGH("UDT is in process for Ecall mui_status=%d,data_req_status =%d, is_emergency_call =%d",rrcdt_int_data.rrcudt_cs_saved_data[i].mui_status,
                                                                                  rrcdt_int_data.rrcudt_cs_saved_data[i].data_req_status,
                                                                                  rrcdt_int_data.rrcudt_cs_saved_data[i].data_req.is_emergency_call);
        return (TRUE);
      }
    }
  }
...
)
{
  ...
  rrc_plmn_identity_type selected_plmn_id;     /* Temporary varibale to update linked list members */       
  rrc_MCC *temp_mcc = NULL;
  rrc_MNC *temp_mnc = NULL;

  WRRC_MSG1_HIGH("Fill PLMN I.D from CN I.D= %d",cn_id);
  /*Get the selected_plmn_id from the stored PLMN depending on CN domain*/
  if(cn_id == RRC_CS_DOMAIN_CN_ID)
  {
    selected_plmn_id = rrcdt_int_data.rrcidt_cs_saved_open_session_req.selected_plmn_id;
  }
  else if(cn_id == RRC_PS_DOMAIN_CN_ID)
  {
    selected_plmn_id = rrcdt_int_data.rrcidt_ps_saved_open_session_req.selected_plmn_id;
  }
  else
  {
  ...
  PRINT_PLMN_ID(selected_plmn_id)

  /*Check for number of digits in MNC either its 2 or 3 & then  print */
  if((selected_plmn_id.num_mnc_digits != 3)&&
    (selected_plmn_id.num_mnc_digits != 2))
  {
    ...
    selected_plmn_id.num_mnc_digits = 3;
    WRRC_MSG1_ERROR("NWS:Invalid Number of MNC digits = %d",selected_plmn_id.num_mnc_digits);
  }
  ...
  {
    temp_mcc->elem[n_index] = selected_plmn_id.mcc[i];
    n_index++;
  }

  /* Update the pointer to the linked list */
  temp_mnc = &(plmn_id_msg_ie->mnc);
  temp_mnc->n = selected_plmn_id.num_mnc_digits;
  n_index = 0;
  
  ALLOC_ASN1ARRAY2(&enc_ctxt,temp_mnc,rrc_Digit);

  for(i = 0;i < selected_plmn_id.num_mnc_digits;i++)
  {
    temp_mnc->elem[n_index] = selected_plmn_id.mnc[i];
    n_index++;
  }
}
...
)
{
  ...
  if((RRC_CLOSE_SESSION_REQ == cmd_ptr->cmd_hdr.cmd_id) ||
     (RRC_SIG_CONN_RELEASE_REQ ==  cmd_ptr->cmd_hdr.cmd_id))
  {
    if((TRUE == rrcdormancy_check_dormancy_initiated_scrr_active(wrrc_as_id)) &&
         ...
         )
      {
        ...
        switch(rrcdt_int_data.rrcscrr_substate)
        {
          ...
          case RRCSCRR_WAIT_FOR_L2ACK:
            ...
            {
              WRRC_MSG0_HIGH("Mapping Dormancy initiated SCRI to SIG_CONN_REL");     
              rrcdt_int_data.rrcscrr_saved_ps_close_session_req.mui_status = RRCSCRR_MUI_SIG_CON_REL_CNF;
              rrcdt_int_data.rrcscrr_saved_ps_close_session_req.rrc_sig_con_rel_status = FALSE;              
              rrcdt_int_data.rrcscrr_saved_ps_close_session_req.rrc_proc = cmd_ptr->cmd.rrc_sig_conn_req.rrc_proc;
            }
            status = TRUE;
            break;
        }
...
FUNCTION rrcdt_check_fake_security_and_send_sync_ind_

DESCRIPTION
  This function sends SYNC_IND for integrity
  protection
...
===========================================================================*/
void rrcdt_check_fake_security_and_send_sync_ind(sys_modem_as_id_e_type wrrc_as_id, rrc_cn_domain_identity_e_type cn_domain_id)
{
 if(nv_fake_security_enabled)
 {
   /* For PS send the RRC_SYNC_IND for Integrity Protection */
   rrcidt_send_sync_ind(wrrc_as_id,RRC_INTEGRITY_ESTABLISHED, cn_domain_id);
 }
}
...
===========================================================================*/
boolean rrcscrr_is_conn_rel_in_progress(sys_modem_as_id_e_type wrrc_as_id,rrc_cn_domain_identity_e_type cn_domain_id)
{

  switch(cn_domain_id)
  {
    case RRC_CS_DOMAIN_CN_ID:
      if((rrcdt_int_data.rrcscrr_saved_cs_close_session_req.close_sess_req_status) ||
         (rrcdt_int_data.rrcscrr_saved_cs_close_session_req.rrc_sig_con_rel_status)
        )
      {
        return TRUE;
    ...
    case RRC_PS_DOMAIN_CN_ID:
      if((rrcdt_int_data.rrcscrr_saved_ps_close_session_req.close_sess_req_status) ||
         (rrcdt_int_data.rrcscrr_saved_ps_close_session_req.rrc_sig_con_rel_status)
        )
      {
        return TRUE;
...
===========================================================================*/
void rrcscrr_abort_ps_rel_proc(sys_modem_as_id_e_type wrrc_as_id)
{
  rrcdt_int_data.rrcscrr_saved_ps_close_session_req.rrc_sig_con_rel_status = FALSE;
  rrcdt_int_data.rrcscrr_saved_ps_close_session_req.mui_status = RRCSCRR_MUI_STATUS_FALSE;
  rrcdt_int_data.rrcscrr_substate = RRCSCRR_INITIAL;
...
FUNCTION rrcidt_get_cs_est_cause

DESCRIPTION
  This function returns the CS domain establishment cause
  
DEPENDENCIES
  None

RETURN VALUE
  Returns the IDT CS establishment cause

===========================================================================*/
rrc_establish_cause_e_type rrcidt_get_cs_est_cause(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcdt_int_data.rrcidt_cs_saved_open_session_req.est_cause;
}

/*===========================================================================
FUNCTION rrcidt_check_cu_init_by_cs_idt

DESCRIPTION
  This function checks if CU is initiated due to CS IDT
  
DEPENDENCIES
  None

RETURN VALUE
  Returns TRUE if CS IDT is the reason for CU

===========================================================================*/
boolean rrcidt_check_cu_init_by_cs_idt(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcdt_int_data.rrcidt_cs_saved_open_session_req.cu_init_by_idt;
}
/*===========================================================================
...
{
  return rrcdt_int_data.rrcidt_substate;
}
...
{
  return rrcdt_int_data.rrcudt_substate;
}
...
{
  return rrcdt_int_data.rrcscrr_substate;
}
...
{
  qsh_mdump_collect_high(&rrcdt_int_data,sizeof(rrcdt_int_data));
}
#endif
