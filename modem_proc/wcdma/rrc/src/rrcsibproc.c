...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcsibproc.c#6 $    $DateTime: 2021/11/24 22:05:03 $ $Author: pwbldsvc $

when       who     what, where, why
11/08/21   sn      Changes to check LA mismatch only for cell reselection cases
11/01/21   sn      Changes to initialize PLMN variable before checking LA mismatch
11/20/18   ak      Made changes for FR 53313
03/15/19   ha      Made changes to avoid buffer overflow when maximum segmants received per BCCH block
04/20/18   dj      Made changes to have an L1 api for wakeup cancel.
04/20/18   sas     Skip posting UPDATE_SIB event for six hr timer expiry during csfb redirection.
04/20/18   sas     Restart SIB wait timer for DMCR case when expired in LTA gap.
04/12/18   ak      Made changes to read complete sibs independent of their entry in SIBdb.
12/27/17   gc      Checking pccpch status, instead of checking lc_id alone 
01/29/18   sas     Changes to correct the use of memcmp() function
10/19/16   ha      Clear the active serving cell information so that BCCH block received can be ignored when WL1 is in idle state. 
--------   ---     ----------------------------------------------------------
06/23/17   dj      Logging of Uarfcn/PSC information of SIBs present in SIB DB using Channel Type 240.
04/25/17   dj      Made changes to have uarfcn and psc for wcdma signalling messages.
04/24/17   sas     Changes to report PLMN present in UMI instead of MIB-PLMN upon GPS cell-id request in DCH.
09/14/16   sn      Changes to compare MIB before applying freq/psc optimization
07/26/16   vs      Changes to support the logging of extension sibs in 0x412f when they are read from sibdb
07/20/16   sas     Ignore Update_sibs when not camped.
05/26/16   sr      Changes to reset SIB event when moving to FACH state
05/26/16   vs      Changes to support logging SB1/SB2/SIB18 in WCDMA_SIGNALLING_MESSAGE log packet.
...
05/18/16   sn      Changes to avoid poking if W is holding Tx lock 
05/18/16   sr      Changes to not drop BCCH only in disconnected/connecting for SIB7 event
05/16/16   sn      f3 reduction changes BPLMN scenarios
05/12/16   vs      Changes for Manual BST Update APIs to support per sub extension
05/11/16   vs      KW error fixes.
05/09/16   sas     Changes to avoid sending NULL mask to L1 if number of sib segments are changed while reading them.
05/09/16   vs      Changes to allow FMO when UE moves from connecting to FACH state and stays in FACH state
05/09/16   vs      Changes to drop BCCH pipe when UPDATE SIBS EVENT in disconnected state completes and RCE is inactive.
...
04/19/16   sr      Made changes to block QTA if NBCCH setup is in progress for inter f reselection
04/15/16   sn      Changes for f3 logging reduction
04/13/16   vs      Changes to fix KW errors
03/21/16   vs      Changes to prevent starting sib_wait_timer when UE is in connecting state or is in disconnected state and not waiting for sib7 update, post EST_REQ from NAS
03/17/16   vs      Check if SIB4 is decoded before accessing it
03/16/16   sn      Chanes to fix legacy DMCR issue
03/16/16   sas     Changes to revert CR966691
03/16/16   vs      Made changes for SIB7 reading enhancement in connecting state
03/07/16   vs      Changes to avoid S-criteria check in case of plmn mismatch in NAS/BCCH plmn for X2W reselection case.
02/03/16   vs      Changes to update serving cell parameters from sib3/4 to L1
03/01/16   sas     Changes to implement FR:31479 ISS for segments
02/11/16   vs      Changes to invalidate stale sibs of alternate sub in w plus w
02/11/16   vs      Changes to remove irrelevant f3s
02/15/16   gv      SIB module to not start SIB7 event in FACH when OC is not set for FACH to PCH transition
02/11/16   sas     Backup the contents of tsib, tsib_optimization_db into deferred_sib_info, only if they have valid content.
01/11/16   vi      Changes to add a new cause when CPHY_SETUP_CNF is received with no lock for NBCH
01/05/16   sn      Changes to not share SIBDB if MIB value tag is invalid
12/18/15   vs      Chnages to avoid waiting for SIB5/SIB5bis in bplmn search when acqusition bands is neither IMT2K nor AWS
12/11/15   sas     Made changes to avoid poking holes for other sub page during sib read after CM_inv timer expiry.
12/07/15   vs      Chnages to avoid waiting for sib5/6 in bplmn search when acqusition bands are not 1 or 4
12/10/15   vi      Made changes to pass the correct reason in transition request to wl1 when reselection is failed due to pending l2 ack
11/24/15   vs      Changes to cleanup unnecessary f3s
11/05/15   sn      Modified SIB wait timer for DCH to FACH and FACH to FACH cell selection cases to 4 secs.
10/19/15   sn      Changes to support poking before MIB reception and during update SIBs event
10/15/15   sp      Made changes to add resel failure, rrc conn rel events
09/28/15   sr      Made changes to drop NBCCH for inter freq before setting up RB0
09/18/15   sn      Changes to fix poking issues
09/15/15   sn      Re-ordering changes to retain poking to prevent sending NULL mask
09/07/15   sas     Made Changes to have a SIB wait factor of 4 and MIB wait time as 4secs during reselection for dual WCDMA cases.
09/02/15   sn      Added debug message for test team purpose
08/24/15   gv      Updated the API which indicates if deferred SIB event is in progress
08/14/15   sas     Update Freq,PSC values in to SIBDB while reading sibs from sysinfo_Container during CSFB redirection.
08/11/15   vi      Made changes to correctly change the lock priority CM<->CM_INV while SIB reading.
07/16/15   bc      Changes to call NAS API on finding a new cell in W during either cell selection/reselection, bplmn.
07/22/15   sas     Made changes to skip SIB read after MIB/SIB1/SIB3/4 on Non NAS requested PLMN.
07/17/15   bc      Changes to send SIBs of a cell to alternate SUB only if no SIB event(except SIB7 event)
                   is active on the cell.
07/09/15   sn      Changes to toggle SIB reading priority based on partition timer
                   expiry and to allow all page decode lock request during SIB read
                   except for collision with SIB with max rep rate greater than 64
...
06/10/15   vi      Made changes to correctly clear SIB7 Expiration timer.
...
05/21/15   vi      Made changes to avoid linker errors after disabling LTE
05/14/15   bc      Changes to read SIBs across DRX for CGI scan.
05/11/15   sn      WCDMA RRC changes for FR 24494
03/18/15   vi      Made changes to add DMCR IE only when DMCR is supported in SIB3
...
02/23/15   sn      Changes to not query wl1 api to get SFN information if STOP 
                   is already sent and when MIB is not yet received
02/20/15   sa      Made changes to add the stub functions for QTF
02/20/15   sas     Changes to reduce the barring time of the cell from 1280 secs to 320secs if any scheduling info is missed.
02/19/15   sas     Made changes to log events for cfsb call opt feature
02/19/15   sn      Changes to pass correct frequency and PSC to wl1 api for cell reselection cases to get SFN
...
02/19/15   sn      Changes to send NULL mask at the end of SIB event for DMCR cases
01/21/15   sa      Made changes for QTF with CRM
01/16/15   sn      Changes to prevent sending NULL mask at the end of SIB event
...
/*CM Inversion kicks in at 2*MAX_REP_RATE so better to use SIB Wait factor of 4*MAX_REP_RATE for dual WCDMA*/
#define RRCSIB_BPLMN_NBCCH_SIB_WAIT_FACTOR  ((rrc_dsds_int_data.wcdma_is_ue_in_dual_standby)? 40:30)
#define RRCSIB_NBCCH_MIB_WAIT_TIMER  ((rrc_dsds_int_data.wcdma_is_ue_in_dual_standby)? 4000:2000)
#else
...
#ifdef FEATURE_DUAL_SIM
#define RRCSIB_STOP_PARTITION_WAIT_FACTOR 20
#endif
...
#define MAX_SEGMENTS_PER_BCCH_BLOCK 18
...
sib_active_event_database_type *sib_events_ptr_sim[MAX_RRC_STACKS] = {NULL};
rrcsibproc_ext_data_type rrcsibproc_ext_data_sim[MAX_RRC_STACKS];

#define sib_events_ptr sib_events_ptr_sim[WAS_ID_M]
#define rrcsibproc_ext_data rrcsibproc_ext_data_sim[WAS_ID_M]
#else
/* Declare a 'sib_events' database. This is used to store information
about each active SIB processing event. */
sib_active_event_database_type *sib_events_ptr = NULL;
rrcsibproc_ext_data_type rrcsibproc_ext_data;
#endif /* FEATURE_DUAL_WCDMA  */
/* -----------------------------------------------------------------------
** Local Object Definitions
** ----------------------------------------------------------------------- */
#ifdef FEATURE_DUAL_WCDMA 
LOCAL rrcsibproc_int_data_type rrcsibproc_int_data_sim[MAX_RRC_STACKS] = {0};
#ifdef FEATURE_SIB19_DECODE_FAIL_OPT
LOCAL uint32 sib19_decode_fail_cnt_sim[MAX_RRC_STACKS]; /*Used to track number of SIB19 decoding failures */
#endif
#define rrcsibproc_int_data rrcsibproc_int_data_sim[WAS_ID_M]
#define sib19_decode_fail_cnt sib19_decode_fail_cnt_sim[WAS_ID_M]
#else
LOCAL rrcsibproc_int_data_type rrcsibproc_int_data;
#ifdef FEATURE_SIB19_DECODE_FAIL_OPT
...
** ----------------------------------------------------------------------- */
#define GET_SIB_TRANS_ID() (++rrcsibproc_int_data.transaction_id % 255)
...
)
{
  ...
  if(rrcsibproc_int_data.update_sibs_event_pending == TRUE)
  {
    status = TRUE;
    WRRC_MSG0_HIGH("Update SIBs event is pending");
  }
  ...
  WRRC_MSG3_MED("Update SIBs pending flag: %d, SIB events ptr: %d, status: %d",
                       rrcsibproc_int_data.update_sibs_event_pending,sib_events_ptr,status);
...
===========================================================================*/
/***/ __WCDMA_RRC_CODE_SEGMENT_NON_DEMANDPAGED__ /***/
boolean rrc_sib_is_current_event_sib7_in_fach(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8 count;
  boolean status = FALSE;
  for( count = 0; count < (uint32)MAX_SIB_EVENTS_INDEX; count++ )
      ...
      if(sib_events_ptr->event[count].event_name == RRCSIB_GET_SPECIFIC_SIBS)
      {
        if((sib_events_ptr->event[count].int_event.int_event_type == RRCSIB_INT_SIB7_EVENT)&&
           ...
           ((rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH)
            || ((update_sib7_in_connecting == TRUE) && (rrc_get_state(wrrc_as_id) == RRC_STATE_CONNECTING))
           )
          )
        {
          status = TRUE;
...
===========================================================================*/
/***/ __WCDMA_RRC_CODE_SEGMENT_NON_DEMANDPAGED__ /***/
boolean rrc_sib_is_sib_segment_needed_by_rrc_fach(sys_modem_as_id_e_type wrrc_as_id,uint16 sfn)
{
  boolean status = FALSE;
  ...
  REX_ISR_LOCK(&rrcsibproc_ext_data.sib7_in_fach_active_mutex);
  sib7_in_fach_status = rrcsibproc_int_data.is_sib7_in_fach_active;
  REX_ISR_UNLOCK(&rrcsibproc_ext_data.sib7_in_fach_active_mutex);
  ...
  if(sib7_in_fach_status == TRUE)
  {
    ...
    uint8 bit_mask = 0xFF;
    bit_mask = rrcsib_gen_bit_mask_for_schedule_table(wrrc_as_id,byte_offset);
    if((rrcsibproc_int_data.sib_sleep_schedule_table_0[byte_pos]&bit_mask) != 0)
    {
      status = TRUE;
    }
  }
...
#else
  if(rrc_sib_is_current_event_sib7_in_fach(wrrc_as_id) == TRUE)
  {
    uint16 byte_pos = sfn>>3;                /* (sfn/8) Gives the byte location in sib sleep schedule table */
    uint8 byte_offset = (sfn & 0x07);     /* (sfn%8)Gives the offset within the byte in sib sleep table */
    uint8 bit_mask = 0xFF;
...
===========================================================================*/
/***/ __WCDMA_RRC_CODE_SEGMENT_NON_DEMANDPAGED__ /***/
boolean rrc_sib_can_l1_start_fmo(sys_modem_as_id_e_type wrrc_as_id)
{
  boolean  status = FALSE;
  ...
  {
    if(rrcsib_is_sib_active(wrrc_as_id) != TRUE)
    {
...
===========================================================================*/
boolean rrcsib_check_band_mismatch_from_sib5_sib6(sys_modem_as_id_e_type wrrc_as_id,uint32 index);
...
)
{
  ...
  {
    mib_ptr = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_MIB,dbindex);
    ...
    {
      sb2_ptr = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_SB2,dbindex);
      ...
      if(ext_siblist2_ptr == NULL)
      {
        sb1_ptr = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_SB1,dbindex);
    ...
    if(is_sib19_present == FALSE)
    {
      ...
      {
        rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType19_PDU,
...
)
{
  ...
  WRRC_MSG2_MED("SIB%d indication present: %d", 
                      rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,rrc_sib_type),status);
  return status;
}
...
===========================================================================*/
unsigned long  rrcsib_get_sib7_exp_timer_value(sys_modem_as_id_e_type wrrc_as_id, uint32 sib_db_index )
{
...
#ifdef FEATURE_WCDMA_HS_FACH_DRX
    if(rrcenhstate_camped_cell_supports_e_fach(wrrc_as_id)
        && rrcenhstate_camped_cell_supports_e_fach_drx(wrrc_as_id)
         && (rrc_get_state(wrrc_as_id) ==RRC_STATE_CELL_FACH))
   {
  ...
  if((sib_ptr = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_SIB7, index)) != NULL)
  {
  ...
  if((sib_ptr = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_MIB, index)) != NULL)
  {
    /* If the MIB is valid */
    ...
    {
      sib7exp = rrcsib_convert_sib7_rep_time_to_ms(wrrc_as_id,&sb_ptr->elem[count].scheduling, exp_factor);
    }
  }/* end if MIB is valid */
  else
  {
  ...
  if( (sib7_found == FALSE) && (sb1_found == TRUE) )
  {
    ...
    if((sib_ptr = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_SB1, index)) != NULL)
    {
      /* If the SB1 is valid */
      ...
      if( (n_value != 0) && (sib7_found == TRUE) )
      {
        sib7exp = rrcsib_convert_sib7_rep_time_to_ms(wrrc_as_id,&ptr->elem[count].scheduling, exp_factor);
      }
    }
  ...
  else if( (sib7_found == FALSE) && (sb2_found == TRUE) )
  {
    ...
    if((sib_ptr = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_SB2, index)) != NULL)
    {
      /* If the SB2 is valid */
...
)
{
  ...
  {
    rrc_sibdb_free_sib(wrrc_as_id,  rrc_MasterInformationBlock_PDU,
  ...
  {
    rrc_sibdb_free_sib(wrrc_as_id,  rrc_SysInfoType3_PDU,
...
)
{
  ...
  if(( bcch_cmd_ptr = rrc_get_int_cmd_buf(wrrc_as_id) ) != NULL )
  {
    ...
    {
      bcch_cmd_ptr->cmd.chan_config_req.transaction_id = GET_SIB_TRANS_ID();
      WRRC_MSG3_HIGH("Trans Id for BCCH Channel configuration: %d, S-BCCH action: %d, N-BCCH action: %d", 
               bcch_cmd_ptr->cmd.chan_config_req.transaction_id,s_bcch_action,n_bcch_action);
    }
  }

  if(n_bcch_action == NBCCH_SETUP)
  {
    WRRC_MSG0_MED("Setting up N-BCCH");
    ...
    rrcsib_initialize_sib_sleep_schedule_table(wrrc_as_id);
#endif
  }

  if(s_bcch_action == SBCCH_TEARDOWN)
  {
    WRRC_MSG0_MED("Tearing down S-BCCH");
  ...
  {
    WRRC_MSG0_MED("Setting up S-BCCH");
#if defined(FEATURE_WCDMA_DRX_MANAGER)
  ...
  rrc_put_int_cmd(wrrc_as_id,  bcch_cmd_ptr );
...
===========================================================================*/
static void rrcsib_send_command_for_pending_update_sibs_event(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_cmd_type *ptr; /* To send an UPDATE_SIBS_IND cmd if needed */

  /* Post a command to ourselves for updating the SIBs */
  if( (ptr = rrc_get_int_cmd_buf(wrrc_as_id)) != NULL)
  {     
    ...
    rrc_put_int_cmd(wrrc_as_id, ptr);   
  } 
...
===========================================================================*/
static void rrcsib_send_command_for_saved_update_sibs_event(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_cmd_type *ptr; /* To send an UPDATE_SIBS_IND cmd if needed */
  ...
  {     
    ...
    /* MIB value tag should be as before */
    ptr->cmd.update_sibs_ind.mib_value_tag = 
      rrcsibproc_int_data.rrcsib_cmd_store.saved_cmd.cmd.update_sibs_ind.mib_value_tag;
    ptr->cmd.update_sibs_ind.real_sib_mod = 
      rrcsibproc_int_data.rrcsib_cmd_store.saved_cmd.cmd.update_sibs_ind.real_sib_mod;

    /* Procedure name */
    ptr->cmd.update_sibs_ind.procedure = rrcsibproc_int_data.rrcsib_cmd_store.saved_cmd.cmd.update_sibs_ind.procedure;
    rrc_put_int_cmd(wrrc_as_id, ptr);   
  } 
}/* end function rrcsib_send_command_for_pending_update_sibs_event */

...
)
{
    ...
    )
  {
    ...
    if(rrcsibproc_int_data.sib7info.sib_ext_proc_update_req == TRUE)
    {
      /* Set the value back to FASLE */
      rrcsibproc_int_data.sib7info.sib_ext_proc_update_req = FALSE;
      if( (ptr = rrc_get_int_cmd_buf(wrrc_as_id))!=NULL )
      {
        ...
        ptr->cmd.sib7_update_status.procedure = rrcsibproc_int_data.sib7info.req_proc;
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
        if(event_status == FAILURE)
        {
          rrc_qsh_report_sib_decode_failure_event(wrrc_as_id, SIB_EVENT_FAILED_OTHER);
        }
#endif
    ...
    else if(rrcsibproc_int_data.sib7info.sib_timer_update_req == TRUE)
    {
      /* Timer based update of SIB7 has been completed. */
      rrcsibproc_int_data.sib7info.sib_timer_update_req = FALSE;
#ifdef FEATURE_UPDATE_SIB7_IN_FACH
      if(rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH)
    ...
    {
      /* Post a command to ourselves for updating the SIBs */
      rrcsib_send_command_for_pending_update_sibs_event(wrrc_as_id);
      WRRC_MSG0_HIGH("Pending UPDATE_SIBS event activated");
      rrcsibproc_int_data.update_sibs_event_pending = FALSE;
    }
  }
}
...
static void  rrcsib_send_sib_sleep_table(sys_modem_as_id_e_type wrrc_as_id,uint8 *table_ptr)
{
  ...
  WRRC_MSG0_MED("SIB_Sleep:CPHY_SIB_SCHED_CHANGE_IND sent to L1");
}
...
===========================================================================*/
static void  rrcsib_intro_holes_in_table(sys_modem_as_id_e_type wrrc_as_id,uint8 *table_ptr)
{

  uint8 table_mask[RRCSIB_SIB_SLEEP_SCHEDULE_TABLE_SIZE*8],sum;
  uint16 i = 0,j = 0,k;
  uint16 cur_sfn,num_sfn_in_mask;
  boolean pattern = TRUE;

  memset(table_mask,0x00,RRCSIB_SIB_SLEEP_SCHEDULE_TABLE_SIZE*8);

  cur_sfn = rrcsib_get_curr_sfn(wrrc_as_id);

  if(cur_sfn == 0xFFFF)
  {
    cur_sfn = rrcsib_collect_data.curr_sfn*2;
  }
  ...
  WRRC_MSG2_HIGH("ISS: Current SFN is cur_sfn %d, RRC SFN %d",cur_sfn,rrcsib_collect_data.curr_sfn);
  /*Number of SFNs for which a particular mask has to be applied*/
  num_sfn_in_mask = MAX(sib_events_ptr->scheduling_max_rept_rate,RRCSIB_MIN_MASK_INV_TIMER);

  /*Code to set 10 SFNs from current SFN  to NULL*/
  for(k = 0; k < RRCSIB_EMPTY_FRAMES; k++,cur_sfn++)
  {
    table_mask[cur_sfn%4096] = 0;
  }

  //cur_sfn--;
  WRRC_MSG2_HIGH("ISS: Current SFN is cur_sfn %d, num_sfn_in_mask %d",cur_sfn,num_sfn_in_mask);
  for(k = RRCSIB_EMPTY_FRAMES; k < RRCSIB_SIB_SLEEP_SCHEDULE_TABLE_SIZE*8; k++,i++,cur_sfn++,j++)
  {
    if(i < num_sfn_in_mask)
     {
       if(j < RRCSIB_SLEEP_SFN)
       {
         table_mask[cur_sfn%4096] = pattern;
         continue;
       }
       if(j < RRCSIB_SLEEP_SFN + RRCSIB_WAKE_SFN)
       {
         table_mask[cur_sfn%4096] = !pattern;
         continue;
       }
     ...
     /*Reset everything and invert pattern */
     j = i = -1;
     cur_sfn--;
     k--;
     pattern = !pattern;
...
)
{
                ...
                rrc_PLMN_Type_gsm_MAP )))
  {
    rrc_translate_gsm_map_plmn_id
                 (wrrc_as_id,&(sib_events_ptr->event[event_index].tempholder.mib->plmn_Type.u.gsm_MAP->plmn_Identity), &plmn);
  }
  else
  {
  ...
  if(rrc_sib_is_current_event_sib_read_in_dch(wrrc_as_id) == TRUE)
  {
...
#ifdef FEATURE_UPDATE_SIB7_IN_FACH
  if(rrctmr_get_remaining_time(wrrc_as_id, RRCTMR_SIB_WAIT_TIMER) != (uint64)0)
#endif
  {
    /* Stop the SIB wait timer in case it's running. */
    rrctmr_stop_timer(wrrc_as_id,  RRCTMR_SIB_WAIT_TIMER );
#ifdef FEATURE_DUAL_SIM
    rrctmr_stop_timer(wrrc_as_id, RRCTMR_SIB_STOP_PARTITION_TIMER);
#endif
  }
      ...
      (sib_events_ptr->event[event_index].req_proc == RRC_PROCEDURE_CSP)) 
  {
    if (sib_events_ptr->event[event_index].sib_db_index_found == TRUE)
    {
     ...
     if((dbindex != SIBDB_INVALID_INDEX)&&
      ...
      {
        rrc_sibdb_clear_a_cell_index(wrrc_as_id,dbindex);
      }
    }
  }
  ...
  /* In case the tempholer pointers are not freed, free them
  now */
  rrcsib_free_tsib(wrrc_as_id,event_index);
  ...
  { 
    case RRCSIB_GET_SPECIFIC_SIBS:
          ...
          != RRC_PROCEDURE_SIB )
      {
        ...
        {
          if( (cnf_ptr = rrc_get_int_cmd_buf(wrrc_as_id))!=NULL )
          {
            ...
            /*Boolean to indicate whether SIB read over OTA or through the 
                      SYS_INFO_CONTAINER*/
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
            rrc_qsh_report_sib_decode_failure_event(wrrc_as_id, failure_cause);
#endif
            ...
            rrc_put_int_cmd(wrrc_as_id, cnf_ptr );
#ifdef FEATURE_DUAL_SIM
            wl1_ds_toggle_prio_inv_for_sib_read(FALSE, wrrc_as_id);
#endif
          }
        }
      }
      ...
      {
        rrcsib_process_int_sib_event_complete(wrrc_as_id, event_index, SUCCESS );
      }
      break;
    ...
    case RRCSIB_UPDATE_SIBS:
      ...
      rrc_sibdb_clear_active_cell_sib_change(wrrc_as_id);
      ...
      else
      {
        rrcsib_process_int_sib_event_complete
          ( wrrc_as_id, event_index, FAILURE );
      }
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
      rrc_qsh_report_sib_decode_failure_event(wrrc_as_id, failure_cause);
#endif
    ...
    )
  {
    ...
    if(
       ((rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH)||
       (OC_SET_FOR_CELL_FACH == rrcllc_get_ordered_config_status(wrrc_as_id)))
#ifdef FEATURE_WCDMA_HS_RACH
       &&(rrcsib_update_sib7_hs_rach(wrrc_as_id) == TRUE)
#endif
      )
    {
    ...
    if(
       ...
       ((rrcllc_get_ordered_config_status(wrrc_as_id) != OC_SET_FOR_FACH_CELL_PCH_TRANS)&&
       (rrcllc_get_ordered_config_status(wrrc_as_id) != OC_SET_FOR_FACH_URA_PCH_TRANS))
#ifdef FEATURE_WCDMA_HS_RACH
       &&(rrcsib_update_sib7_hs_rach(wrrc_as_id) == TRUE)
#endif
       )
...
#endif
    {
      ...
      if(( sbcch_teardown_required == TRUE ) && (sib_events_ptr->event[event_index].for_serving_cell))
      {
        ...
        //S_BCCH setup
        (void)rrcsib_configure_bcch(wrrc_as_id,SBCCH_TEARDOWN, NBCCH_NOOP
                                 ,chan_config_reason);
      }
    }
  }
  ...
  rrcsibproc_int_data.no_mib_bar_cell_count = 0;
#endif

#ifdef FEATURE_UPDATE_SIB7_IN_FACH
#ifdef FEATURE_WCDMA_FREE_FLOATING_TASK
  REX_ISR_LOCK(&rrcsibproc_ext_data.sib7_in_fach_active_mutex);
  if(rrcsibproc_int_data.is_sib7_in_fach_active == TRUE)
  {
    rrcsibproc_int_data.is_sib7_in_fach_active = FALSE;
    WRRC_MSG1_HIGH("SIB7 in cell fach event active is set to %d",rrcsibproc_int_data.is_sib7_in_fach_active);
  }
  REX_ISR_UNLOCK(&rrcsibproc_ext_data.sib7_in_fach_active_mutex);
#endif
#endif
...
)
{
  ...
  if(sib_events_ptr->scheduling_max_rept_rate < sib_events_ptr->sib_sched[rrc_sib_type].repitition_rate)
  {
    ...
    {
      if((sib_events_ptr->scheduling_max_rept_rate > rrcsibproc_int_data.max_sib_rept_rate) &&
         (sib_events_ptr->scheduling_max_rept_rate > RRC_SIB_MAX_REP_RATE_SIB_WAIT_TIMER))
      {
        rrcsibproc_int_data.max_sib_rept_rate = sib_events_ptr->scheduling_max_rept_rate;

        timer_value_pending   = (uint32)rrctmr_get_remaining_time(wrrc_as_id, RRCTMR_SIB_WAIT_TIMER);
        timer_value_exhausted = rrcsibproc_int_data.timer_value_started - timer_value_pending;

        /* Deducting the time that has already been exhausted */
        diff_timer_value = (rrcsibproc_int_data.max_sib_rept_rate * RRC_RADIO_FRAME_TO_MILLISECOND_CONV) - timer_value_exhausted;

        if(diff_timer_value > (int32)timer_value_pending)
        {
          rrctmr_stop_timer(wrrc_as_id,  RRCTMR_SIB_WAIT_TIMER );

	      rrcsibproc_int_data.timer_value_started = diff_timer_value;

          WRRC_MSG1_HIGH("Start SIB_WAIT timer for S_BCCH max_rep_rate %d", rrcsibproc_int_data.max_sib_rept_rate);
          rrctmr_start_timer(wrrc_as_id, RRCTMR_SIB_WAIT_TIMER,(unsigned long) (diff_timer_value));
        }
      }
    }
#endif
  ...
  WRRC_MSG6_MED( 
           ...
           rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,rrc_sib_type), 
           sib_events_ptr->sib_sched[rrc_sib_type].num_segments,
...
)
{
  ...
  WRRC_MSG1_HIGH("Checking TSIB DB for SIB %d",rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,rrc_sib_type));
  for(i = 0;i<MAX_TEMP_SIB_SEGMENTS_TO_HOLD;i++)
  {
    if(rrcsib_collect_data.tsib_optimization_db[i].index_in_use == TRUE && rrcsib_collect_data.tsib_optimization_db[i].sib_type == rrc_sib_type)
    {
      valid_sib = FALSE;
      sfn_offset = rrcsib_collect_data.tsib_optimization_db[i].sfn_prime<<1;
      if(rrcsib_collect_data.tsib_optimization_db[i].seg_index < sib_events_ptr->sib_sched[rrc_sib_type].num_segments)
      {
        ...
        if(valid_sib == FALSE)
        {
          WRRC_MSG2_HIGH("Seg %d of SIB %d was received at wrong SFN",rrcsib_collect_data.tsib_optimization_db[i].seg_index,
                           rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,rrc_sib_type));
          rrcsib_collect_data.tsib_optimization_db[i].index_in_use = FALSE;
          rrcsib_collect_data.tsib_optimization_db[i].seg_data_length = 0;
          rrcsib_collect_data.tsib_optimization_db[i].seg_index = 0;
          rrcsib_collect_data.tsib_optimization_db[i].sfn_prime = 0;
          rrcsib_collect_data.tsib_optimization_db[i].sib_seg_type = noSegment_chosen;
  
          rrcsib_collect_data.tsib_optimization_db[i].sib_type = rrc_MIB;
          memset(rrcsib_collect_data.tsib_optimization_db[i].seg_data_ptr,0x00, FIXED_SIB_DATA_LENGTH_BYTES);
        }
      }
      else
      {
        WRRC_MSG3_HIGH("Segment Index greater than number of segments. Invalid segment index %d for SIB %d. Total number of segments = %d",
                          rrcsib_collect_data.tsib_optimization_db[i].seg_index,rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,rrc_sib_type)
                          ,sib_events_ptr->sib_sched[rrc_sib_type].num_segments);
        rrcsib_collect_data.tsib_optimization_db[i].index_in_use = FALSE;
        rrcsib_collect_data.tsib_optimization_db[i].seg_data_length = 0;
        rrcsib_collect_data.tsib_optimization_db[i].seg_index = 0;
  ...
  {
    WRRC_MSG1_MED("Concatenating segments for SIB # %d received before SB",rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,rrc_sib_type));
    status = rrcsib_process_sib_seg_in_tsib_optim_db(wrrc_as_id,rrc_sib_type,index);
  }
...
)
{
  ...
  for(i = 0;i < MAX_TEMP_SIB_SEGMENTS_TO_HOLD;i++)
  {
    if(rrcsib_collect_data.tsib_optimization_db[i].index_in_use == TRUE && rrcsib_collect_data.tsib_optimization_db[i].sib_type == rrc_ExtensionType)
    {
      sfn_offset = rrcsib_collect_data.tsib_optimization_db[i].sfn_prime<<1;
      if(rrcsib_collect_data.tsib_optimization_db[i].seg_index < sib_events_ptr->sib_sched[rrc_sib_type].num_segments)
      {
        for(j=0;(j<sib_events_ptr->sib_sched[rrc_sib_type].num_segments) && (j < MAX_SEGMENTS_FOR_A_SIB);j++)
        {
          ...
          {
            num_seg++;
            rrcsib_collect_data.tsib_optimization_db[i].sib_type = rrc_sib_type;
            break;
          }
        }
      }
  ...
  if(num_seg <= sib_events_ptr->sib_sched[rrc_sib_type].num_segments)
  {
    rrcsib_collect_data.tsib_opt_db_look_up_table[rrc_sib_type] = TRUE;
...
)
{
  ...
  switch(pdu_num)
  {
    ...
    case rrc_MIB:
      ...
      {
        rrcsib_get_sib_type_from_sibsb_type_and_tag(wrrc_as_id,&sibSb_ptr->elem[index].sibSb_Type,
                                                    &sib_type_tag);
        if(sib_type_tag.sib_type != rrc_MAX_SIB)
        {
          rrcsib_update_pos_offset_in_sched_info(wrrc_as_id,
      ...
      if(EXTENSION_SIB_SCHED_PRESENT_IN_MIB(mib_ptr))
      {
        ...
        while(ext_siblist_ptr->n > index)
        {
          if(ext_siblist_ptr->elem[index].extensionSIB_Type.t & T_rrc_SIB_TypeExt_systemInfoType11bis)
          {
            ...
            rrcsib_update_pos_offset_in_sched_info(wrrc_as_id,rrc_SIB11bis, &ext_siblist_ptr->elem[index].schedulingInfo);
          }
          else
          {
            ...
            v860NonCriticalExtensions.masterInformationBlock_v860ext.m.extSIBTypeInfoSchedulingInfo_ListPresent))
      {
        ...
        while(ext_siblist2_ptr->n > index)
        {
          ...
          {
            WRRC_MSG1_MED("sib19:Sched present for SIB19 in MIB, bit mask val =%x",
                ext_siblist2_ptr->elem[index].extensionSIB_Type2.t);

            rrcsib_update_pos_offset_in_sched_info(wrrc_as_id,rrc_SIB19, &ext_siblist2_ptr->elem[index].schedulingInfo);
          }
          else
          {
      ...
      {
        rrcsib_get_sib_type_from_sibsb_type_and_tag(wrrc_as_id,&sib_sb_ptr_tsib->elem[index].sibSb_Type,
                                                      &sib_type_tag_tsib);
        if(sib_type_tag_tsib.sib_type != rrc_MAX_SIB)
        {
          if(rrcsib_is_sib_seg_in_tsib_optim_db(wrrc_as_id,sib_type_tag_tsib.sib_type) == TRUE)
          {
            status = rrcsib_check_if_sched_info_match(wrrc_as_id,sib_type_tag_tsib.sib_type,event_index);
            if(status == SEG_PROC_COMPLETE)
            {
              WRRC_MSG1_HIGH("All segments were received prior to scheduling info. Received SIB %d",
                        rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,sib_type_tag_tsib.sib_type));
              rrc_log_complete_sib(wrrc_as_id,rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,sib_type_tag_tsib.sib_type),0,0);
           ...
           sib_events_ptr->event[event_index].event_name == RRCSIB_GET_SPECIFIC_SIBS)
      {
           ...
           (sib_events_ptr->sib_sched[rrc_SIB3].repitition_rate != INVALID_SIB_SCHEDULE_REP))
        {
          ...
          if(sib_events_ptr->event[event_index].sib_scan_type == RRC_SIB_CELL_ID_READ_IN_DCH_SCAN)
          {
            ...
            rrctmr_start_timer(wrrc_as_id, RRCTMR_SIB_WAIT_TIMER,
        ...
        else
        {
          ...
          else  //valid config, check max scheduling rate of SB1 and SB2 and start timer
          {
            ...
            if(sib_events_ptr->event[event_index].sib_scan_type == RRC_SIB_CELL_ID_READ_IN_DCH_SCAN)
            {
              rrctmr_start_timer(wrrc_as_id, RRCTMR_SIB_WAIT_TIMER,(unsigned long) (max_rept_rate * RRCSIB_CELL_ID_SIB3_WAIT_FACTOR));
            }
            else
            {
              rrctmr_start_timer(wrrc_as_id, RRCTMR_SIB_WAIT_TIMER,
      ...
      else if(sib_events_ptr->event[event_index].sib_scan_type == RRC_SIB_NORMAL)
      {
           ...
           (sib_events_ptr->sib_sched[rrc_SB2].repitition_rate != INVALID_SIB_SCHEDULE_REP))
        {
          ...
          {
#ifdef FEATURE_DUAL_SIM
            if(rrcsibproc_int_data.g_in_data_call)
            {
              rrctmr_stop_timer(wrrc_as_id, RRCTMR_SIB_WAIT_TIMER);

              /*SIB wait timer is max of 3000, 60*rep_rate*/
              rrctmr_start_timer(wrrc_as_id,RRCTMR_SIB_WAIT_TIMER, MAX(3000,60*max_rept_rate));


              rrctmr_start_timer(wrrc_as_id,RRCTMR_SIB_STOP_PARTITION_TIMER, MAX(2500,40*max_rept_rate));
              rrcsibproc_int_data.rrcsib_stop_partition = FALSE;
            }
            else
#endif
            {
              ...
              rrctmr_start_timer(wrrc_as_id, RRCTMR_SIB_WAIT_TIMER,
                                  (unsigned long) (max_rept_rate * RRCSIB_BPLMN_NBCCH_SIB_WAIT_FACTOR));
#ifdef FEATURE_DUAL_SIM
              rrctmr_start_timer(wrrc_as_id,RRCTMR_SIB_STOP_PARTITION_TIMER, RRCSIB_STOP_PARTITION_WAIT_FACTOR*max_rept_rate);
#endif
        ...
        else
        {
          ...
          if(sib_events_ptr->scheduling_max_rept_rate == 0)
          {
            ...
            rrctmr_start_timer(wrrc_as_id, RRCTMR_SIB_WAIT_TIMER, RRCSIB_MAX_WAIT_TIME_FOR_SIBS );
          }
          else
          {
            if(sib_events_ptr->event[event_index].for_serving_cell == FALSE)
            {
              ...
              if(rrcsibproc_int_data.g_in_data_call)
              {
                ...
                /*SIB wait timer is max of 3000, 60*rep_rate*/
                rrctmr_start_timer(wrrc_as_id,RRCTMR_SIB_WAIT_TIMER, MAX(3000,60*sib_events_ptr->scheduling_max_rept_rate));


                rrctmr_start_timer(wrrc_as_id,RRCTMR_SIB_STOP_PARTITION_TIMER, MAX(2500,40*sib_events_ptr->scheduling_max_rept_rate));
                rrcsibproc_int_data.rrcsib_stop_partition = FALSE;
              }
              else
#endif
...
#ifdef FEATURE_DUAL_SIM
                rrctmr_start_timer(wrrc_as_id,RRCTMR_SIB_STOP_PARTITION_TIMER, RRCSIB_STOP_PARTITION_WAIT_FACTOR*sib_events_ptr->scheduling_max_rept_rate);
                /*wl1 api to reduce priority to channel maintenance */
                wl1_ds_toggle_prio_inv_for_sib_read(FALSE, wrrc_as_id);
#endif
              }
            }
    ...
    case rrc_SB2:
      ...
      while(sb_ptr->n > index)
      {
        ...
        rrcsib_get_sib_type_from_sib_type_and_tag(wrrc_as_id,&sb_ptr->elem[index].sib_Type,
                                                    &sib_type_tag);
        if(sib_type_tag.sib_type != rrc_MAX_SIB)
        {
          rrcsib_update_pos_offset_in_sched_info(wrrc_as_id,
      ...
      if(ext_siblist2_ptr != NULL)
      {
        ...
        while(ext_siblist2_ptr->n > index)
        {
          if(ext_siblist2_ptr->elem[index].extensionSIB_Type2.t & T_rrc_SIB_TypeExt2_systemInfoType19)
          {
            WRRC_MSG1_HIGH("sib19:Sched present for SIB19 in SB1/SB2, bit mask val =%x",
                ext_siblist2_ptr->elem[index].extensionSIB_Type2.t);
      ...
      {
        rrcsib_get_sib_type_from_sib_type_and_tag(wrrc_as_id,&sb_ptr_tsib->elem[index].sib_Type,
                                                      &sib_type_tag_tsib);
        if(sib_type_tag_tsib.sib_type != rrc_MAX_SIB)
        {
          if(rrcsib_is_sib_seg_in_tsib_optim_db(wrrc_as_id,sib_type_tag_tsib.sib_type) == TRUE)
      ...
      if(ext_siblist_ptr_tsib != NULL)
      {
        ...
        while(ext_siblist_ptr_tsib->n > index)
        {
          ...
          {
            if(rrcsib_is_sib_seg_in_tsib_optim_db(wrrc_as_id,rrc_ExtensionType) == TRUE)
            {
              status = rrcsib_check_if_sched_info_match_for_ext_sibs(wrrc_as_id,rrc_SIB11bis,event_index);
              if(status == SEG_PROC_COMPLETE)
              {
                ...
                WRRC_EVENT_REPORT_PAYLOAD(EVENT_RRC_COMPLETE_SIB_RECEIVED, sizeof(sib_rcvd_event),
                                    (void *)&sib_rcvd_event);
      ...
      if(ext_siblist2_ptr_tsib != NULL)
      {
        ...
        while(ext_siblist2_ptr_tsib->n > index)
        {
              ...
              T_rrc_SIB_TypeExt2_systemInfoType19)
          {
            if(rrcsib_is_sib_seg_in_tsib_optim_db(wrrc_as_id,rrc_ExtensionType) == TRUE)
            {
              status = rrcsib_check_if_sched_info_match_for_ext_sibs(wrrc_as_id,rrc_SIB19,event_index);
              if(status == SEG_PROC_COMPLETE)
              {
           ...
           sib_events_ptr->event[event_index].event_name == RRCSIB_GET_SPECIFIC_SIBS)
      {
        if(sib1_or_sib3_config_in_un_received_sb == TRUE)
        {
             ...
             (sib_events_ptr->sib_sched[rrc_SIB3].repitition_rate != INVALID_SIB_SCHEDULE_REP))
          {
            ...
            WRRC_MSG2_MED("SIB1,3 in MIB SIB 1 rep %d, SIB 3 rep %d", 
                     sib_events_ptr->sib_sched[rrc_SIB1].repitition_rate,
                     sib_events_ptr->sib_sched[rrc_SIB3].repitition_rate);
      ...
      else if(sib_events_ptr->event[event_index].sib_scan_type == RRC_SIB_NORMAL)
      {
           ...
           (sib_events_ptr->sib_sched[rrc_SB2].repitition_rate != INVALID_SIB_SCHEDULE_REP))
        {
          ...
          if(TRUE == all_scheduling_blocks_received)
          {
            ...
            if(rrcsibproc_int_data.g_in_data_call)
            {
              WRRC_MSG0_HIGH("Complete scehduling is received. Updated MIB SB flag to send new pattern");
              rrcsibproc_int_data.mib_sb_received = TRUE; 
            }
#endif
...
)
{
  ...
  if((sib_ptr = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_MIB, dbindex)) != NULL)
  {
    WRRC_MSG2_HIGH("Sched. Info DB-Idx %d for SIB %d", dbindex, rrc_MIB);
    rrcsib_update_sched_info_from_mib_sb(wrrc_as_id,(uint32)rrc_MIB, sib_ptr, event_index);
  }
  else
  {
  ...
  if((sib_ptr = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_SB1, dbindex)) != NULL)
  {
    WRRC_MSG2_HIGH("Sched. Info DB-Idx %d for SIB %d", dbindex, rrc_SB1);
    rrcsib_update_sched_info_from_mib_sb(wrrc_as_id,(uint32)rrc_SB1,sib_ptr, event_index);
  }

  if((sib_ptr = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_SB2, dbindex)) != NULL)
  {
    WRRC_MSG2_HIGH("Sched. Info DB-Idx %d for SIB %d", dbindex, rrc_SB2);
    rrcsib_update_sched_info_from_mib_sb(wrrc_as_id,(uint32)rrc_SB2, sib_ptr, event_index);
  }
}

/*===========================================================================
...
)
{
  ...
  for( count = 0; count < RRC_TOT_SIBS; count++ )
  {
    switch(count)
    {
      ...
      case rrc_SB2:
      case rrc_SIB2:
        rrcsibproc_int_data.supported_sibs_lookup_table[count] = TRUE;
        break;
      ...
      case rrc_SIB11bis:
        ...
        {
         rrcsibproc_int_data.supported_sibs_lookup_table[count] = FALSE;
       }
        break;
...
===========================================================================*/
static int rrcsib_check_if_next_sfn_expected(sys_modem_as_id_e_type wrrc_as_id,rrc_SIB_type sib,uint16 cur_sfn,uint8 seg_index)
{
  ...
  WRRC_MSG3_HIGH("relative_diff_bw_curr_and_next_sib_sfn is  %d for SIB %d and cur_sfn is %d", relative_diff_bw_curr_and_next_sib_sfn, rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,sib),cur_sfn);
...
===========================================================================*/
boolean rrcsib_check_if_sibs_scheduled_during_pich(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  cur_sfn = dl_get_cell_timing((rrc_csp_int_data.curr_acq_entry.scr_code/16),
    rrc_csp_int_data.curr_acq_entry.freq,wrrc_as_id);
  ...
  no_of_sfns = (rrc_csp_int_data.xtow_bplmn_search_overhead_timer + RRC_PICH_AND_MEASUREMENTS_TIME)/20;
  ...
  for(i = 0; i < sizeof(rrc_sib_type); i++)
  {
    ...
    {
      relative_diff = rrcsib_check_if_next_sfn_expected(wrrc_as_id,rrc_sib_type[i],cur_sfn,0);
      if((relative_diff < no_of_sfns) &&
         ((relative_diff + ((sib_events_ptr->sib_sched[rrc_sib_type[i]].repitition_rate )/2))>=(rrcllc_get_drx_info(wrrc_as_id)/2)))
      {
        return TRUE;
      }
    }
...
)
{
       ...
       (cmd_ptr->cmd.update_sibs_ind.cause == RRCSIB_DEFERRED_SIB_READ)))
  {
    ...
    rrcsibproc_int_data.is_sib_cont_present = FALSE;
  }
#endif
  ...
  rrc_csp_int_data.clr_sib_db_index = 0xFFFF;

#ifdef FEATURE_DUAL_SIM
  rrcsibproc_int_data.rrcsib_stop_partition = FALSE;
  rrcsibproc_int_data.g_in_data_call = WRM_G2W_TA_MODE_ACTIVE(wrrc_as_id);
#endif

  //invalidate sib-scheduling info as we are setting up a new event
  rrcsib_invalidate_sib_scheduling_info(wrrc_as_id,index);
...
#ifdef FEATURE_RRC_SIB_SUPPORT_MAX_REPT_RATE
  rrcsibproc_int_data.max_sib_rept_rate = 0;
  rrcsibproc_int_data.timer_value_started  = RRCSIB_MAX_WAIT_TIME_FOR_SIBS;
#endif

  /*Initialize tsib optimization data base*/
  rrcsib_clear_tsib_optimization_db(wrrc_as_id);
  ...
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_UPDATE_SIBS_IND:
      ...
      for( count = 0; count < RRC_TOT_SIBS; count++ )
      {
        if(rrcsibproc_int_data.supported_sibs_lookup_table[count] == FALSE)
        {
    ...
    case RRC_GET_SPECIFIC_SIBS_REQ:
      ...
      for( count = 0; count < RRC_TOT_SIBS; count++ )
      {
        ...
        if((cmd_ptr->cmd.get_specific_sibs_req.sib_scan_type == RRC_SIB_WTOW_MANUAL_BPLMN_SCAN)||
           ...
           ((cmd_ptr->cmd.get_specific_sibs_req.sib_scan_type == RRC_SIB_MANUAL_BPLMN_SCAN)
            ...
            && (!rrccsp_check_cgi_scan(wrrc_as_id))
#endif
           )
          )
        {
        ...
        {
          if((rrc_get_frequency_band(wrrc_as_id, rrc_csp_int_data.curr_acq_entry.freq, rrc_csp_int_data.curr_acq_entry.plmn_id) == RRC_CSP_WCDMA_1700_2100_BAND) ||
             (rrc_get_frequency_band(wrrc_as_id, rrc_csp_int_data.curr_acq_entry.freq, rrc_csp_int_data.curr_acq_entry.plmn_id) == RRC_CSP_IMT2K_BAND))
          {
        ...
        )
      {
        ...
        rrcsib_update_mib_sb_sched_info_from_sib_db(wrrc_as_id,sib_events_ptr->event[index].sib_db_index, index);
        ...
        if(TRUE == rrc_sibdb_check_if_sb_present(wrrc_as_id,rrc_SB1,sib_events_ptr->event[index].sib_db_index))
        {
          sib_events_ptr->event[index].tsib[rrc_SB1].sib_received = TRUE;
          sib_events_ptr->event[index].tsib[rrc_SB1].sib_decoded = TRUE;
        }
        
        if(TRUE == rrc_sibdb_check_if_sb_present(wrrc_as_id,rrc_SB2, sib_events_ptr->event[index].sib_db_index))
        {
        ...
        if(rrc_sib_is_current_event_sib7_in_fach(wrrc_as_id) == TRUE)
        {
          ...
          rrcsib_build_sib7_schedule_table(wrrc_as_id,rrc_SIB7,SIB_SRV_INDEX,0);
#else
          rrcsib_build_sib7_schedule_table(wrrc_as_id,rrc_SIB7,SIB_SRV_INDEX);
#endif
        }
        else
#endif
        {
          rrcsib_update_sib_sleep_schedule_table(wrrc_as_id, SIB_SRV_INDEX);
        }
      }
      ...
      event we should clear the update sibs flag */
               ...
               (rrcsibproc_int_data.update_sibs_event_pending == TRUE) )
      {
        rrcsibproc_int_data.update_sibs_event_pending = FALSE;
      }
      ...
      if((sib_events_ptr->event[index].int_event.int_event_type == RRCSIB_INT_SIB7_EVENT)&&
         ...
         (sib_events_ptr->event[index].for_serving_cell == TRUE)&&
         ((rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH) ||
         (rrc_get_state(wrrc_as_id) == RRC_STATE_CONNECTING))
        )
      {
        WRRC_MSG0_HIGH("SIB7 in cell fach event active");
        REX_ISR_LOCK(&rrcsibproc_ext_data.sib7_in_fach_active_mutex);
        rrcsibproc_int_data.is_sib7_in_fach_active = TRUE;
        REX_ISR_UNLOCK(&rrcsibproc_ext_data.sib7_in_fach_active_mutex);
      }
      else
      {
...
)
{
  ...
  if((sib_events_ptr->event[old_index].for_serving_cell == TRUE)||
    (rrcllc_get_s_bcch_setup_status(wrrc_as_id)==TRUE))
  {
    ...
    if( srv_cell != TRUE )
    {
      ...
      if(rrcsib_configure_bcch(wrrc_as_id,SBCCH_TEARDOWN, NBCCH_SETUP
           ...
           ) == SUCCESS)
      {
           ...
           (sib_events_ptr->event[old_index].event_name == RRCSIB_UPDATE_SIBS) )
        {
          ...
          rrc_sibdb_invalidate_mib_sb_value_tags_for_interrupted_sib_update(wrrc_as_id,sibdb.active_cell_index);
        }
        ...
        {
          rrcsib_process_int_sib_event_complete(wrrc_as_id, old_index, SUCCESS );
        }
      }
    ...
    else
    {
      ...
      else if((rrc_state == RRC_STATE_CELL_PCH) || (rrc_state == RRC_STATE_URA_PCH))
      {
               ...
               )
        {
          ...
          if((rrcsibproc_int_data.sib7info.sib_ext_proc_update_req == TRUE) && (rrcsibproc_int_data.sib7info.req_proc == RRC_PROCEDURE_CU))
          {
            rrcsib_send_sib7_ind(wrrc_as_id);
          }
          bcch_status = RRCSIB_BCCH_EVENT_SAVED;
        }
      }
      ...
      else if(rrc_state == RRC_STATE_CELL_FACH)
      {
          ...
          )
        {
          ...
          /* Table ptr can be assigned to any sleep table as we're going to memset the
             whole array anyways */
          table_ptr = rrcsibproc_int_data.sib_sleep_schedule_table_1;
          ...
          rrcsib_send_sib_sleep_table(wrrc_as_id,table_ptr);
        }
        else
#endif
        {
      ...
      else
      {
        ...
        rrcsib_clear_procedure
          (wrrc_as_id,SIB_EVENT_FAILED_INVALID_EVENT); /* Clear existing procedure to be safe */
        bcch_status = RRCSIB_BCCH_SETUP_FAILED;
      }      
  ...
  else
  {
    ...
    if( srv_cell != TRUE )
    {
      ...
      if(rrcsib_configure_bcch(wrrc_as_id,SBCCH_NOOP, NBCCH_SETUP
                ,RRCLLC_CHAN_CFG_REASON_NONE
            )== SUCCESS)
      {
    ...
    else
    {
      ...
      else
      {
        ...
        rrcsib_clear_procedure
          (wrrc_as_id,SIB_EVENT_FAILED_INVALID_EVENT); /* Clear existing events to be safe */
        bcch_status = RRCSIB_BCCH_SETUP_FAILED;
      }
    }/* end if srv_cell != TRUE */
...
WRRC_MSG2_MED(" Serving cell event %d and LLC S-BCCH setup status %d ",
             sib_events_ptr->event[SIB_SRV_INDEX].event_active ,
             rrcllc_get_s_bcch_setup_status(wrrc_as_id));
     ...
     (cmd_ptr->cmd.get_specific_sibs_req.procedure == RRC_PROCEDURE_CSP)))
  {
      ...
      return(bcch_reconfigure_for_new_sib_event(wrrc_as_id, cmd_ptr, srv_cell, SIB_SRV_INDEX));
    ...
    {
      /* There is an event active! */
      return(bcch_reconfigure_for_new_sib_event(wrrc_as_id, cmd_ptr, srv_cell, SIB_NGH_INDEX));
      /* Only one event is active at a time so
         no need to continue the for loop if we found
         one active event. */
  ...
  {
    /* Get S Bcch status */
    if (rrcllc_get_s_bcch_setup_status(wrrc_as_id))
    {
#ifdef FEATURE_UPDATE_SIB7_IN_FACH
      if(((rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH)
       || ((update_sib7_in_connecting == TRUE) && ((rrc_get_state(wrrc_as_id) == RRC_STATE_CONNECTING)||(rrc_get_state(wrrc_as_id) == RRC_STATE_DISCONNECTED))))
       && (RRC_CAMPED_ON == rrc_ccm_get_curr_camping_status(wrrc_as_id, RRC_PROCEDURE_SIB))
              )
#endif
      {
  ...
  {
    /* Get N-bcch status */
    if(rrcllc_get_n_bcch_setup_status(wrrc_as_id))
    {
  ...
  if( srv_cell == TRUE )
  {
    ...
    if(cmd_ptr->cmd_hdr.cmd_id == RRC_GET_SPECIFIC_SIBS_REQ)
    {
      ...
      (void)rrcsib_configure_bcch(wrrc_as_id,SBCCH_SETUP, NBCCH_NOOP
            ,chan_config_reason);
  ...
  {
    (void)rrcsib_configure_bcch(wrrc_as_id,SBCCH_NOOP, NBCCH_SETUP
        ,RRCLLC_CHAN_CFG_REASON_NONE
        );
...
)
{
  ...
  bcch_status = sib_bcch_setup_req(wrrc_as_id, cmd_ptr );
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    case RRC_UPDATE_SIBS_IND:
      ...
      create_new_sib_event(wrrc_as_id, SIB_SRV_INDEX, cmd_ptr );
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
      if(cmd_ptr->cmd.update_sibs_ind.cause == RRCSIB_DEFERRED_SIB_READ)
      { 		 
        rrc_qsh_report_sib_event(wrrc_as_id,RRC_SIB_READ_DEFERRED_SIB_UPDATE);
      }
      else if(cmd_ptr->cmd.update_sibs_ind.cause == RRCSIB_REAL_SIB_MOD)
      { 		 
        rrc_qsh_report_sib_event(wrrc_as_id, RRC_SIB_READ_SIB_MOD);
      }
#endif
...
#ifdef FEATURE_DUAL_SIM
      rrctmr_start_timer(wrrc_as_id, RRCTMR_SIB_STOP_PARTITION_TIMER, 6000);
#endif
    ...
    case RRC_GET_SPECIFIC_SIBS_REQ:
      ...
      else
      {
        ...
        create_new_sib_event(wrrc_as_id, SIB_NGH_INDEX, cmd_ptr );
      }
      /* For all new SIB events, we start a SIB wait timer. 
      This timer is the maximum time to wait for SIBs in a cell.
      ...
      {
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH		 
        rrc_qsh_report_sib_event(wrrc_as_id,RRC_SIB_READ_BPLMN_SCAN);
#endif
        rrctmr_start_timer(wrrc_as_id, RRCTMR_SIB_WAIT_TIMER, RRCSIB_BPLMN_MIB_WAIT_TIMER);
      }
      else if(cmd_ptr->cmd.get_specific_sibs_req.sib_scan_type == RRC_SIB_CELL_ID_READ_IN_DCH_SCAN)
      {
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH	 
        rrc_qsh_report_sib_event(wrrc_as_id,RRC_SIB_READ_SIB_READ_IN_DCH_FOR_CELL_ID);
#endif
        ...
        rrctmr_start_timer(wrrc_as_id, RRCTMR_SIB_WAIT_TIMER, RRCSIB_MAX_SIB3_WAIT_TIMER);
      }
      else if(cmd_ptr->cmd.get_specific_sibs_req.for_serv_cell == FALSE)
      {
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH	 
        rrc_qsh_report_sib_event(wrrc_as_id,RRC_SIB_READ_CELL_RESELECTION);
#endif
        rrctmr_start_timer(wrrc_as_id, RRCTMR_SIB_WAIT_TIMER, RRCSIB_NBCCH_MIB_WAIT_TIMER);
      }
      ...
      /*This code can be clubbed with following else if condition if feature is mainlined in future*/
              ...
              && ((update_sib7_in_connecting == FALSE) || (rrcrce_get_rce_sub_state(wrrc_as_id) == RRCRCE_WAIT_FOR_SIB7_CURRENT))
             )
      {
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH	 
        rrc_qsh_report_sib_event(wrrc_as_id,RRC_SIB_READ_SIB7_CONNECTING);
#endif
        timer_value = 3*rrcsib_get_sib7_exp_timer_value(wrrc_as_id,sibdb.active_cell_index);
        rrctmr_start_timer(wrrc_as_id, RRCTMR_SIB_WAIT_TIMER, 
                         timer_value );
      }
#ifdef FEATURE_UPDATE_SIB7_FOR_PCH_TO_FACH
      ...
      {
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH	 
          rrc_qsh_report_sib_event(wrrc_as_id,RRC_SIB_READ_SIB7_FROM_PCH_TO_FACH);
#endif
        timer_value = rrcsib_get_sib7_exp_timer_value(wrrc_as_id,sibdb.active_cell_index);
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
          rrc_qsh_report_sib_event(wrrc_as_id,RRC_SIB_READ_SIB7_WITHIN_FACH);
#endif
          WRRC_MSG0_HIGH("sib7:This is SIB7 update event in FACH/connecting, do not start SIB wait timer");
        }
        else
#endif
        {
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
          rrc_qsh_report_sib_event(wrrc_as_id,RRC_SIB_READ_CELL_SELECTION);
#endif
          if((rrccsp_get_curr_scan_no_f3(wrrc_as_id) == RRC_CSP_CELL_DCH_TO_CELL_FACH_SCAN) ||
              (rrccsp_get_curr_scan_no_f3(wrrc_as_id) == RRC_CSP_CELL_FACH_TO_CELL_FACH_SCAN)
            )
          {
            rrctmr_start_timer(wrrc_as_id, RRCTMR_SIB_WAIT_TIMER, RRCTMR_DCH_FACH_CELL_SELECTION_TIMER_IN_MS);
#ifdef FEATURE_DUAL_SIM
            rrctmr_start_timer(wrrc_as_id, RRCTMR_SIB_STOP_PARTITION_TIMER, RRCTMR_DCH_FACH_CELL_SELECTION_TIMER_IN_MS/2);
#endif
          }
          else
          {
      ...
      /* If the search is for WTW BPLMN,initialize the bplmn sibs variables */
       if(((cmd_ptr->cmd.get_specific_sibs_req.sib_scan_type == RRC_SIB_WTOW_AUTO_BPLMN_SCAN) 
        ...
        &&((cmd_ptr->cmd.get_specific_sibs_req.freq == rrcsibproc_int_data.rrcsib_bplmn_sibs.prev_freq) && 
          (cmd_ptr->cmd.get_specific_sibs_req.scrambling_code == rrcsibproc_int_data.rrcsib_bplmn_sibs.prev_scr)))
        {
          /* Free and initialize the bplmns sibs */
          if(rrcsibproc_int_data.rrcsib_bplmn_sibs.mib != NULL)
          {
...
#ifdef FEATURE_FEMTO_CSG
            if(CSG_IND_IN_MIB(rrcsibproc_int_data.rrcsib_bplmn_sibs.mib) &&
               rrc_csg_int_data.is_whitelist_valid) 
            {
            ...
            {
              sibSb_ptr = &rrcsibproc_int_data.rrcsib_bplmn_sibs.mib->sibSb_ReferenceList;
              for(ix=0;ix< sibSb_ptr->n;ix++)
              {
                rrcsib_get_sib_type_from_sibsb_type_and_tag(wrrc_as_id,&sibSb_ptr->elem[ix].sibSb_Type,
                                                            &sib_type_tag);
...
#endif
            rrcsib_update_sched_info_from_mib_sb(wrrc_as_id,(uint32)rrc_MIB, rrcsibproc_int_data.rrcsib_bplmn_sibs.mib, SIB_SRV_INDEX);
          }
          if(rrcsibproc_int_data.rrcsib_bplmn_sibs.sib1!= NULL)
          {
          ...
          if(rrcsibproc_int_data.rrcsib_bplmn_sibs.sib3!= NULL)
          {
...
#ifdef FEATURE_FEMTO_CSG
            if(CSG_ID_IN_SIB3(rrcsibproc_int_data.rrcsib_bplmn_sibs.sib3) &&
               (rrc_csg_int_data.is_whitelist_valid))
            {
              sib_events_ptr->event[SIB_SRV_INDEX].csg_id = rrc_translate_cell_id
                              (wrrc_as_id, &(rrcsibproc_int_data.rrcsib_bplmn_sibs.sib3->v4b0NonCriticalExtensions.v590NonCriticalExtension.
          ...
          if(rrcsibproc_int_data.rrcsib_bplmn_sibs.sb1!= NULL)
          {
            ...
            {
              sb_ptr = &rrcsibproc_int_data.rrcsib_bplmn_sibs.sb1->sib_ReferenceList;
              ix=0;
              for(ix=0;ix< sb_ptr->n;ix++)
              {
                rrcsib_get_sib_type_from_sib_type_and_tag(wrrc_as_id,&sb_ptr->elem[ix].sib_Type,
                                                      &sib_type_tag);
                if(sib_type_tag.sib_type == rrc_SIB5 ||
                    sib_type_tag.sib_type == rrc_SIB5bis)
                {
...
#endif
            rrcsib_update_sched_info_from_mib_sb(wrrc_as_id,(uint32)rrc_SB1, rrcsibproc_int_data.rrcsib_bplmn_sibs.sb1, SIB_SRV_INDEX);
          }
          if(rrcsibproc_int_data.rrcsib_bplmn_sibs.sb2!= NULL)
          {
            ...
            {
              sb_ptr = &rrcsibproc_int_data.rrcsib_bplmn_sibs.sb2->sib_ReferenceList;
              ix=0;
              for(ix=0;ix< sb_ptr->n;ix++)
              {
                rrcsib_get_sib_type_from_sib_type_and_tag(wrrc_as_id,&sb_ptr->elem[ix].sib_Type,
...
#endif
            rrcsib_update_sched_info_from_mib_sb(wrrc_as_id,(uint32)rrc_SB2, rrcsibproc_int_data.rrcsib_bplmn_sibs.sb2, SIB_SRV_INDEX);
          }
        ...
        {
          rrcsib_free_bplmn_sibs(wrrc_as_id);
          if((cmd_ptr->cmd.get_specific_sibs_req.sib_scan_type == RRC_SIB_WTOW_AUTO_BPLMN_SCAN) 
           ||(cmd_ptr->cmd.get_specific_sibs_req.sib_scan_type == RRC_SIB_WTOW_MANUAL_BPLMN_SCAN)
#ifdef FEATURE_LTE_TO_WCDMA
           ||((cmd_ptr->cmd.get_specific_sibs_req.sib_scan_type == RRC_SIB_MANUAL_BPLMN_SCAN)
          ...
          {
            rrcsibproc_int_data.rrcsib_bplmn_sibs.prev_freq = cmd_ptr->cmd.get_specific_sibs_req.freq ;
            rrcsibproc_int_data.rrcsib_bplmn_sibs.prev_scr = cmd_ptr->cmd.get_specific_sibs_req.scrambling_code;
          }
        }
      ...
      if(((cmd_ptr->cmd.get_specific_sibs_req.sib_scan_type == RRC_SIB_NORMAL)
         ...
         (cmd_ptr->cmd.get_specific_sibs_req.procedure == RRC_PROCEDURE_CSP))
      {
        WRRC_MSG0_HIGH("Free ASN.1 decoded sibs for all entries");
        ...
        {
          rrcasn1_free_pdu(wrrc_as_id,rrc_System_Information_Container_PDU,(void *)sibdb.rrc_decoded_sib_cont);
          sibdb.rrc_decoded_sib_cont = NULL;

          /* Clearing the SIB DB index as the information inside as not used anymore */
          rrc_sibdb_clear_a_cell_index(wrrc_as_id,sibdb.sib_cont_sibdb_index);
    ...
    if(ext_siblist2_ptr != NULL)
    {
      ...
      while(ext_siblist2_ptr->n > n_index)
      {
        if(ext_siblist2_ptr->elem[n_index].extensionSIB_Type2.t &
            T_rrc_SIB_TypeExt2_systemInfoType19)
        {
          WRRC_MSG0_MED("sib19:Sched present for sib19 ");
          status = TRUE;
        }
        else
        {
...
)
{
  ...
  switch(pdu_num)
  {
    ...
    case rrc_MIB:
      ...
      while( n_value != 0 )
      {
        rrcsib_get_sib_type_from_sibsb_type_and_tag(wrrc_as_id,&sibSb_ptr->elem[n_index].sibSb_Type,
                                                    &sib_type_tag);
        if(sib_type_tag.sib_type != rrc_MAX_SIB)
        {
    ...
    case rrc_SB2:
      ...
      if(TRUE == sib_events_ptr->event[index].sched_present_for_sibs[rrc_MIB])
      {
        ...
        while( n_value != 0 )
        {
          ...
          rrcsib_get_sib_type_from_sib_type_and_tag(wrrc_as_id,&sb_ptr->elem[n_index].sib_Type,
                                                      &sib_type_tag);
  ...
  if(rrcsib_check_if_scheduling_info_received_for_extended_sibs(wrrc_as_id,
...
#ifdef FEATURE_WCDMA_TO_LTE
  if(rrcsib_check_if_scheduling_info_received_for_sib19(wrrc_as_id,
           sib_ptr,pdu_num,index) == TRUE)
  {
    WRRC_MSG0_MED("sib19:Sched present for extended sib");
...
===========================================================================*/
static void rrcsib_process_sib_scheduling_info_incomplete(sys_modem_as_id_e_type wrrc_as_id, sib_events_index_e_type event_index )
{
  sib_events_index_e_type count = event_index;          /* Local counter */

  rrccsp_send_cphy_cell_bar_req(wrrc_as_id,RRC_CSP_WCDMA_PSC_BARRED, 320);
...
{  
  ...
  switch(sib_type)
  {
    ...
    case (uint32)rrc_SB1:
      msg_type = RRCLOG_SIB_SB1;
      sib_encoded_ptr = sibdb.cell[dbindex].sibs.sb1_encoded_ptr;
      sib_encoded_length = sibdb.cell[dbindex].sibs.sb1_encoded_pdu_length;
      break;

    case (uint32)rrc_SB2:
      msg_type = RRCLOG_SIB_SB2;
      sib_encoded_ptr = sibdb.cell[dbindex].sibs.sb2_encoded_ptr;
      sib_encoded_length = sibdb.cell[dbindex].sibs.sb2_encoded_pdu_length;
      break;

    case (uint32)rrc_SIB18:
      msg_type = RRCLOG_SIB_18;
      sib_encoded_ptr = sibdb.cell[dbindex].sibs.sib18_encoded_ptr;
      sib_encoded_length = sibdb.cell[dbindex].sibs.sib18_encoded_pdu_length;
      break;

    case (uint32)rrc_SIB11bis:
      msg_type = RRCLOG_EXT_SIB;
      sib_encoded_ptr = sibdb.cell[dbindex].sibs.sib11bis_encoded_ptr;
      sib_encoded_length = sibdb.cell[dbindex].sibs.sib11bis_encoded_pdu_length;
      break;

#ifdef FEATURE_WCDMA_TO_LTE
      case (uint32)rrc_SIB19:
        msg_type = RRCLOG_EXT_SIB;
        sib_encoded_ptr = sibdb.cell[dbindex].sibs.sib19_encoded_ptr;
        sib_encoded_length = sibdb.cell[dbindex].sibs.sib19_encoded_pdu_length;
        break;
#endif
  ...
  log_record_ptr = (WCDMA_SIGNALLING_MESSAGE_type *)WRRC_LOG_ALLOC( WCDMA_SIGNALLING_MESSAGE, log_record_length);
  ...
  if(log_record_ptr != NULL)
  {
    ...
    /*Setting UARFCN*/
    log_record_ptr->uarfcn = sibdb.cell[dbindex].freq;

    /*Setting PSC*/
    log_record_ptr->psc  = sibdb.cell[dbindex].scr_code/16;
...
)
{
  ...
  switch( sib )
  {
    case rrc_MIB:
       ...
       )
     {
       ...
       comparisons */
       if((rrcsib_is_event_sixhrs_sib_update(wrrc_as_id,event_index) == TRUE)||
           (rrcsib_is_sixhrs_sib_timer_expired(wrrc_as_id,event_index) == TRUE)
         )
       {
    ...
    default:
      ...
      WRRC_MSG1_HIGH("Invalid type for value-tag comparison %d", rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,sib));
      break;
  }/* end switch */
  ...
  if( check_results == TRUE )
  {
    ...
    for( count=0; count<RRC_TOT_SIBS; count++ )
    {
      ...
      {
        if((rrcnv_data.rrc_read_sibs_enabled_nv) && (sib_events_ptr->event[event_index].event_name) == RRCSIB_GET_SPECIFIC_SIBS )
        { /* Read SIBs, irrespective of the SIBDB contents*/
          WRRC_MSG0_HIGH("Invalidating ValueTags as rrc_read_sibs_enabled_nv is enabled");
          result.result[count] =  VALUE_TAG_DID_NOT_MATCH;
        }
        ...
        if( result.result[count] ==  VALUE_TAG_MATCHED )
        {
          ...
          rrcsib_log_sys_inf(wrrc_as_id,db_index,count);
          ...
          {
            sb1 = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_SB1,db_index);
            if(sb1 != NULL)
            {
              if(FAILURE == rrcsib_check_if_scheduling_info_received_for_all_sibs(wrrc_as_id,(void *)sb1,
                                                                      rrc_SB1, event_index))
              {
                rrcsib_process_sib_scheduling_info_incomplete(wrrc_as_id,event_index); 
                return;
              }
              else
              {
                WRRC_MSG0_HIGH("Updating Sched from SIBDB SB1");
                rrcsib_update_sched_info_from_mib_sb(wrrc_as_id,(uint32)rrc_SB1, 
          ...
          {
            sb2= rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_SB2,db_index);
            if(sb2!= NULL)
            {
              if(FAILURE == rrcsib_check_if_scheduling_info_received_for_all_sibs(wrrc_as_id,(void *)sb2,
                                                                          rrc_SB2, event_index))
              {
                rrcsib_process_sib_scheduling_info_incomplete(wrrc_as_id,event_index); 
                return;
              ...
              rrcsib_update_sched_info_from_mib_sb(wrrc_as_id,(uint32)rrc_SB2, 
...
{
  uint8 seg_index[MAX_SEGMENTS_PER_BCCH_BLOCK];
  uint32 sib_type_asn1[MAX_SEGMENTS_PER_BCCH_BLOCK] = {0}; // Zero initialization done to suppress Lint error.
  uint32 rrc_sib_type[MAX_SEGMENTS_PER_BCCH_BLOCK] = {0}; // Zero initialization done to suppress Lint error.
  uint32 count=0, i = 0;
  boolean status = FALSE;
  uint8 j = 0;
  ...
  uint32 sfn_prime[MAX_SEGMENTS_PER_BCCH_BLOCK] = {0};
  sib_segment_e_type seg_type[MAX_SEGMENTS_PER_BCCH_BLOCK] = {noSegment_chosen};
  uint32 seg_length[MAX_SEGMENTS_PER_BCCH_BLOCK] = {0};
  unsigned char* value[MAX_SEGMENTS_PER_BCCH_BLOCK] = {0};
  ...
  if(INVALID_SIB_SCHEDULE_REP == sib_events_ptr->sib_sched[sib_type_asn1[rrc_MIB]].repitition_rate)
  {
    ...
    if (((off_set) & 0x7) != 0)
    {
      WRRC_MSG1_MED("MIB not rcvd yet. Offset is %d", off_set);
      status=FALSE;
      return status;
    }
    else
    {
      ...
      WRRC_MSG0_MED("First seg of MIB rcvd. Init MIB-Sched");
      sib_events_ptr->sib_sched[rrc_sib_type[rrc_MIB]].repitition_rate = 8;
      sib_events_ptr->sib_sched[rrc_sib_type[rrc_MIB]].sfn_offset[0] =0;
      sib_events_ptr->sib_sched[rrc_sib_type[rrc_MIB]].sfn_offset[1] =2;
      sib_events_ptr->sib_sched[rrc_sib_type[rrc_MIB]].sfn_offset[2] =4;
  ...
  memset (seg_index, 0x0, MAX_SEGMENTS_PER_BCCH_BLOCK);
  memset (sib_type_asn1, 0x0, MAX_SEGMENTS_PER_BCCH_BLOCK);
  memset (rrc_sib_type, 0x0, MAX_SEGMENTS_PER_BCCH_BLOCK);
...
*/
  if(sib_events_ptr->event[index].tsib[rrc_MIB].sib_received != TRUE)
  {
      ...
      )
    {
      ...
      if((off_set%MIB_POSITION) == 0)
      {
        ...
        if(mib_found == FALSE)
        {
          rrcsibproc_int_data.no_mib_bar_cell_count++;
          WRRC_MSG1_HIGH("MIB not found at MIB position, count=%d",rrcsibproc_int_data.no_mib_bar_cell_count);
          if(rrcsibproc_int_data.no_mib_bar_cell_count == MAX_TIME_MIB_WAIT)
          {
            rrcsibproc_int_data.no_mib_bar_cell_count = 0;
  ...
  for(i=0;i<count;i++)
  {
    ...
    {
      rrc_sib_type[i] = rrcsib_map_asn1_sibtype_to_int_sibtype(wrrc_as_id,(rrc_SIB_Type)sib_type_asn1[i]);
      ...
      if(sib_events_ptr->sib_sched[rrc_sib_type[i]].repitition_rate != INVALID_SIB_SCHEDULE_REP)
      {
        ...
        {
          WRRC_ERR0_FATAL("Potential Div by zero");
        }
           ...
           (off_set % sib_events_ptr->sib_sched[rrc_sib_type[i]].repitition_rate))
        {
          ...
          WRRC_MSG5_HIGH( 
                ...
                rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,(rrc_SIB_type)rrc_sib_type[i]), 
        ...
        {
          WRRC_MSG3_ERROR("Sib_type %d, seg_idx %d, offset %d", rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,(rrc_SIB_type)rrc_sib_type[i]), seg_index[i], off_set);
        }
      }
      else
      {
  
          if(rrcsibproc_int_data.supported_sibs_lookup_table[rrc_sib_type[i]] == TRUE)
          {
            if(sib_events_ptr->sib_sched[rrc_sib_type[i]].repitition_rate == INVALID_SIB_SCHEDULE_REP)
            {
                WRRC_MSG3_HIGH("TSIB - SIB segment received before scheduling information SIB %d, seg_idx %d, offset %d",
                         sib_type_asn1[i],seg_index[i],off_set);
                seg_status = rrcsib_store_sib_seg_in_tsib_optim_db(wrrc_as_id,(rrc_SIB_type)rrc_sib_type[i],
                                                      sfn_prime[i],
                                                      seg_index[i],
                                                      seg_type[i],
                                                      seg_length[i],
                ...
                {
                  WRRC_MSG1_MED("TSIB - Segment saved for SIB %d", sib_type_asn1[i]);
                }
                else
                {
                  /*Simply set this to SEG_PROC_FAILED once more as 
                  rrcsib_store_sib_seg_in_tsib_optim_db(sys_modem_as_id_e_type wrrc_as_id) would have taken care of*/
    ...
    else
    {
      ...
      {
        if(rrcsibproc_int_data.supported_sibs_lookup_table[j] == TRUE)
        {
      ...
      if(sched_info_not_received == TRUE)
      {
        ...
        seg_status = rrcsib_store_sib_seg_in_tsib_optim_db(wrrc_as_id,rrc_ExtensionType,
                                              sfn_prime[i],
                                              seg_index[i],
                                              seg_type[i],
                                              seg_length[i],
...
)
{
  ...
  if( *sib_ptr != NULL )
  {
    rrc_sibdb_free_sib(wrrc_as_id, (int)pdu_num,
                        (void *)*sib_ptr );
    *sib_ptr=NULL;
  }
    ...
    (rrc_sib_type == rrc_SIB5bis))
  {
    conn_mode_sibs_present = rrcsib_check_if_connected_mode_sibs_present
      (wrrc_as_id,sib_events_ptr->event[event_index].tsib[rrc_sib_type].sib_data_ptr[0], 
      rrc_sib_type);
    WRRC_MSG2_MED("conn_mode_sibs_present = %d in sib = %d", conn_mode_sibs_present, 
      rrc_sib_type);
  }
  ...
  sib_ptr_tmp =  rrcasn1_decode_pdu( wrrc_as_id,
    ...
    *sib_encoded_pdu_length = pdu_length ;
    ...
    if(rrc_sib_is_current_event_sib7_in_fach(wrrc_as_id) == TRUE)
    {
      ...
      if(rrc_sib_type == rrc_SIB7)
      {
        ...
        (void)rrcllc_update_configs_with_sib7_info_in_fach(wrrc_as_id);
#ifdef FEATURE_WCDMA_HS_FACH_DRX
        /* Sucessfully received SIB7. Build scheduling table and send indication */
        rrcsib_build_sib7_schedule_table(wrrc_as_id,rrc_SIB7,SIB_SRV_INDEX,RRC_SKIP_SFN);
#endif
      }
    }
    else
#endif
    {
      ...
      {
        rrc_log_dl_sig_message_for_ext_sibs(wrrc_as_id,rrc_sib_type, event_index);
      }
    ...
    if((rrc_sib_type == rrc_SIB7) && (rrcnv_data.rrc_sib7_time_nv != 0))
    {
      unsigned long exp_time = rrcnv_data.rrc_sib7_time_nv;
      boolean HS_cell = FALSE;
      exp_time = rrcsib_get_sib7_exp_timer_value(wrrc_as_id,db_index);
      /*Check if current cell support HS-FACH/EDRX, if yes then start 8 sec timer otherwise start timer with NV configured value
        Note: if SIB 5 is not yet decoded then we will start timer for 2sec /NV configured value, we will later restart the timer in CCM
              if we find the cell support HS-FACH/EDRX
      */
      HS_cell = rrcenhstate_camped_cell_supports_e_fach_drx(wrrc_as_id);
      if(HS_cell)
      {
         WRRC_MSG0_HIGH("Cell support HSFACH/EDRX");
         rrcdata_hs_info.rrc_is_prev_cell_supports_e_drx = TRUE;
         exp_time = (exp_time > RRCSIB_DEFAULT_HS_SIB7_VALIDITY_TIME)? exp_time : RRCSIB_DEFAULT_HS_SIB7_VALIDITY_TIME;
      }
      else
      {
         exp_time = (exp_time > rrcnv_data.rrc_sib7_time_nv)? exp_time : rrcnv_data.rrc_sib7_time_nv;
         rrcdata_hs_info.rrc_is_prev_cell_supports_e_drx = FALSE;
      }
      rrctmr_stop_timer(wrrc_as_id, RRCTMR_SIB7_EXPIRATION_TIMER);
      rrctmr_start_timer(wrrc_as_id,RRCTMR_SIB7_EXPIRATION_TIMER,exp_time);
...
)
{
  ...
  {
    if((FAILURE == rrcllc_validate_t_barred_criteria_for_pich_info(wrrc_as_id,&(sibdb.cell[dbindex].sibs.sib5->sCCPCH_SystemInformationList))) || 
       (FAILURE == rrcllc_validate_t_barred_criteria_for_aich_info(wrrc_as_id,&(sibdb.cell[dbindex].sibs.sib5->prach_SystemInformationList))))
    {
      return TRUE;
    }
  }
  ...
  {
    if((FAILURE == rrcllc_validate_t_barred_criteria_for_pich_info(wrrc_as_id,&(sibdb.cell[dbindex].sibs.sib6->sCCPCH_SystemInformationList))) || 
       (FAILURE == rrcllc_validate_t_barred_criteria_for_aich_info(wrrc_as_id,&(sibdb.cell[dbindex].sibs.sib6->prach_SystemInformationList))))
    {
      return TRUE;
    }
  }
...
)
{
  ...
  if(sib5_read == TRUE)
  {
    ...
    {
      /* ASN.1 decode the PDU */
      sibdb.cell[dbindex].sibs.sib5=  rrcasn1_decode_pdu( wrrc_as_id,
      ...
      if(sibdb.cell[dbindex].sibs.sib5== NULL)
      {
        WRRC_ERR0_FATAL("SIB_HEAP:Asn.1 decode failed from SIB5");
      }
      WRRC_MSG0_MED("SIB_HEAP:Decode Sib5 in sibdb");
    }
    else
    {
      WRRC_MSG1_MED("SIB_HEAP:SIB5 already decoded,sib5_ptr=%d", sibdb.cell[dbindex].sibs.sib5);
    }
  }
  if(sib6_read == TRUE)
  {
    ...
    {
      /* ASN.1 decode the PDU */
      sibdb.cell[dbindex].sibs.sib6=  rrcasn1_decode_pdu( wrrc_as_id,
      ...
      if(sibdb.cell[dbindex].sibs.sib6== NULL)
      {
        WRRC_ERR0_FATAL("SIB_HEAP:Asn.1 decode failed from SIB6");
      }
      WRRC_MSG0_HIGH("SIB_HEAP:Decode Sib6 in sibdb");
    }
    else
    {
      WRRC_MSG1_MED("SIB_HEAP:SIB6 already decoded,sib6_ptr=%d", sibdb.cell[dbindex].sibs.sib6);
    }
  }

  WRRC_MSG2_MED("SIB5 %d, SIB6 %d read status", sib5_read, sib6_read);
  ...
  if((rrc_state == RRC_STATE_CONNECTING) || (rrc_state == RRC_STATE_DISCONNECTED))
  {
    if(TRUE == sib5_read)
    {
      if(TRUE == rrcsib_check_pich_aich_barred_criteria(wrrc_as_id,dbindex, rrc_SIB5))
      {
  ...
  else
  {
    ...
    {      
      ...
      if((FAILURE == rrcllc_validate_t_barred_criteria_for_pich_info(wrrc_as_id,&(sibdb.cell[dbindex].sibs.sib5->
                                                                     sCCPCH_SystemInformationList))) && 
         (FAILURE == rrcllc_validate_t_barred_criteria_for_pich_info(wrrc_as_id,&(sibdb.cell[dbindex].sibs.sib6->
                                                                     sCCPCH_SystemInformationList))))
      {
      ...
      if ((FAILURE == rrcllc_validate_t_barred_criteria_for_aich_info(wrrc_as_id,&(sibdb.cell[dbindex].sibs.sib5->
                                                                      prach_SystemInformationList))) &&
          (FAILURE == rrcllc_validate_t_barred_criteria_for_aich_info(wrrc_as_id,&(sibdb.cell[dbindex].sibs.sib6->
                                                                      prach_SystemInformationList))))
      {
    ...
    else if (TRUE == sib5_read)    //else only sib5 read
    {
      ...
      cell_barred_status = rrcsib_check_pich_aich_barred_criteria(wrrc_as_id,dbindex, rrc_SIB5);
    }
    else if (TRUE == sib6_read)   //else only sib6 read
    {
      WRRC_MSG0_HIGH("SIB6 only AICH/PICH info not present");
      cell_barred_status = rrcsib_check_pich_aich_barred_criteria(wrrc_as_id,dbindex, rrc_SIB6);
    }
  }
  ...
  if(TRUE == cell_barred_status)
  {
     ...
     */
    rrccsp_send_cphy_cell_bar_req(wrrc_as_id,RRC_CSP_WCDMA_PSC_BARRED, 1280);
    rrcsib_post_process_failed_event(wrrc_as_id,index, TRUE, SIB_EVENT_FAILED_OTHER, TRUE );
    return FAILURE;
  }
...
)
{
  ...
  if(sib == rrc_SB1)
  {
    ...
    sb1_ptr =
      ((rrc_SysInfoTypeSB1 *)rrcasn1_decode_pdu( wrrc_as_id,
  ...
  else
  {
    ...
    sb2_ptr =
      ((rrc_SysInfoTypeSB2 *)rrcasn1_decode_pdu( wrrc_as_id,
        (void *)(sib_events_ptr->event[index].
  ...
  else
  {
    ...
    if(FAILURE == rrcsib_check_if_scheduling_info_received_for_all_sibs(wrrc_as_id,local_sb_ptr, sib, index))
    {
      status= FAILURE;
    }
    else
    {
      rrcsib_update_sched_info_from_mib_sb(wrrc_as_id,(uint32)sib, local_sb_ptr, index);
    }
  }
  ...
  if(local_sb_ptr != NULL)
  {
    if(sib == rrc_SB1)
    { 
      rrc_sibdb_free_sib(wrrc_as_id, rrc_SysInfoTypeSB1_PDU,
                          local_sb_ptr);
    }
    else
    {
      rrc_sibdb_free_sib(wrrc_as_id, rrc_SysInfoTypeSB2_PDU,
                          local_sb_ptr);
    }
    local_sb_ptr = NULL;
  }
      ...
      switch( count )
      {
        case rrc_SIB1 :
          ...
          if (rrcsibproc_int_data.rrcsib_bplmn_sibs.sib1 == NULL)
          {
            ...
            rrcsibproc_int_data.rrcsib_bplmn_sibs.sib1 = 
            (rrc_SysInfoType1 *)rrcasn1_decode_pdu( wrrc_as_id,
                               (void *)(sib_events_ptr->event[index].
            ...
            if(rrccsp_check_sib1_validity(wrrc_as_id,rrcsibproc_int_data.rrcsib_bplmn_sibs.sib1) != TRUE)
            {
        ...
        case rrc_SB1:
          ...
          if (rrcsibproc_int_data.rrcsib_bplmn_sibs.sb1 == NULL)
          {
            ...
            /* Decode and store SIB1 in bplmn sib database */
            rrcsibproc_int_data.rrcsib_bplmn_sibs.sb1 = 
            (rrc_SysInfoTypeSB1*)rrcasn1_decode_pdu( wrrc_as_id,
                               (void *)(sib_events_ptr->event[index].
          ...
          if(FAILURE == rrcsib_process_scheduling_block_sched_info_before_sib_db_decoded
            (wrrc_as_id,index, rrc_SB1))
          {
            rrcsib_process_sib_scheduling_info_incomplete(wrrc_as_id,index); 
            return TRUE;
          }
        ...
        case rrc_SB2:
          ...
          if (rrcsibproc_int_data.rrcsib_bplmn_sibs.sb2 == NULL)
          {
            ...
            /* Decode and store SIB1 in bplmn sib database */
            rrcsibproc_int_data.rrcsib_bplmn_sibs.sb2 = 
            (rrc_SysInfoTypeSB2*)rrcasn1_decode_pdu( wrrc_as_id,
                               (void *)(sib_events_ptr->event[index].
        ...
        default:
          WRRC_MSG0_MED("WTOW BPLMN: SIB not SIB1.Ingoring"); 
          break;
      }
    }
  }
...
)
{
  ...
  else
  {
    ...
    {
      /* ASN.1 decode the PDU */
      sibdb.cell[dbindex].sibs.sib1=  rrcasn1_decode_pdu( wrrc_as_id,
      ...
      WRRC_MSG0_MED("SIB_HEAP:Decode Sib1 in sibdb");
    }
    else
    {
      WRRC_MSG1_MED("SIB_HEAP:SIB1 already decoded,sib1_ptr=%d", sibdb.cell[dbindex].sibs.sib1);
    }

    /* call csp function to know if LAC is valid or not*/
    if(rrccsp_check_sib1_validity(wrrc_as_id,sibdb.cell[dbindex].sibs.sib1) == TRUE)
    {
      status = SUCCESS;
    }
    else
...
)
{
  ...
  if ( FAILURE == rrcsib_check_sib5_sib6_barred_criteria(wrrc_as_id,dbindex, index))
  {
  ...
  if(rrcsib_check_lac_validity_from_sib1(wrrc_as_id,dbindex, index) == FAILURE)
  {
  ...
  )
  {
    ...
    decode_succeeded = 
    rrcsib_decode_and_save_sib_for_current_event(wrrc_as_id,(void *)&(sibdb.cell[dbindex].sibs.sb1),
    ...
    if(decode_succeeded == TRUE)
    {
      ...
      if(sib_events_ptr->event[index].event_name == RRCSIB_UPDATE_SIBS) 
      {
        ...
        rrc_sibdb_save_active_cell_sib_change(wrrc_as_id,rrc_SB1 );        
      }

      //check if scheduling info is received for all sibs
      if(FAILURE == rrcsib_check_if_scheduling_info_received_for_all_sibs(wrrc_as_id,(void *)sibdb.cell[dbindex].sibs.sb1,
                                                                          rrc_SB1, index))
      {
        rrcsib_process_sib_scheduling_info_incomplete(wrrc_as_id,index); 
        return;
      ...
      value tags in that block to the value tags already stored
        for this event. */
      update_event_for_received_value_tags(wrrc_as_id,
  ...
  {
    decode_succeeded = 
    rrcsib_decode_and_save_sib_for_current_event(wrrc_as_id,(void *)&(sibdb.cell[dbindex].sibs.sb2),
    ...
    if(decode_succeeded == TRUE)
    {
      ...
      if(sib_events_ptr->event[index].event_name == RRCSIB_UPDATE_SIBS) 
      {
        /* This means a SIB has changed for the serving cell
           and it has been decoded correctly. */
        rrc_sibdb_save_active_cell_sib_change(wrrc_as_id,rrc_SB2);        
      }

      //check if scheduling info is received for all sibs
      if(FAILURE == rrcsib_check_if_scheduling_info_received_for_all_sibs(wrrc_as_id,(void *)sibdb.cell[dbindex].sibs.sb2, 
                                                                          rrc_SB2, index))
      {
        rrcsib_process_sib_scheduling_info_incomplete(wrrc_as_id,index); 
        return;
      ...
      )
    {
      ...
      switch( count )
      {
        case rrc_MIB :
          ...
          decode_succeeded = 
          rrcsib_decode_and_save_sib_for_current_event(wrrc_as_id,(void *)&(sibdb.cell[dbindex].sibs.mib),
        ...
        case rrc_SIB1 :
          ...
          decode_succeeded = 
          rrcsib_decode_and_save_sib_for_current_event(wrrc_as_id,(void *)&(sibdb.cell[dbindex].sibs.sib1),
        ...
        case rrc_SIB2 :
          ...
          decode_succeeded = 
          rrcsib_decode_and_save_sib_for_current_event(wrrc_as_id,(void *)&(sibdb.cell[dbindex].sibs.sib2),
        ...
        case rrc_SIB3 :
          ...
          decode_succeeded = 
          rrcsib_decode_and_save_sib_for_current_event(wrrc_as_id,(void *)&(sibdb.cell[dbindex].sibs.sib3),
          ...
          if(decode_succeeded == TRUE)
          {
            rrcsib_update_sib3_params(wrrc_as_id, dbindex);
          }
          if((decode_succeeded == TRUE)&&
             (sib_events_ptr->event[index].for_serving_cell != TRUE)&&
             (!(sib_events_ptr->event[index].tempholder.sib3_decoded)))
          {
            rrc_translate_gsm_map_plmn_id(wrrc_as_id,&(sibdb.cell[dbindex].sibs.mib->plmn_Type.u.gsm_MAP->plmn_Identity),&plmn);
            ...
            {
              rrccsp_initial_suitability_check(wrrc_as_id, sibdb.cell[dbindex].sibs.mib,
            ...
            {
              rrcsib_post_process_failed_event(wrrc_as_id,(sib_events_index_e_type)index,
                                                TRUE,
                                                status, 
                                                TRUE);
              return;
        ...
        case rrc_SIB4 :
          ...
          decode_succeeded = 
          rrcsib_decode_and_save_sib_for_current_event(wrrc_as_id,(void *)&(sibdb.cell[dbindex].sibs.sib4),
        ...
        case rrc_SIB5 :
          ...
          decode_succeeded = 
          rrcsib_decode_and_save_sib_for_current_event(wrrc_as_id,(void *)&(sibdb.cell[dbindex].sibs.sib5),
        ...
        case rrc_SIB6 :
          ...
          decode_succeeded = 
          rrcsib_decode_and_save_sib_for_current_event(wrrc_as_id,(void *)&(sibdb.cell[dbindex].sibs.sib6),
        ...
        case rrc_SIB7 :
          ...
          decode_succeeded = 
          rrcsib_decode_and_save_sib_for_current_event(wrrc_as_id,(void *)&(sibdb.cell[dbindex].sibs.sib7),
        ...
        case rrc_SIB11 :
          ...
          decode_succeeded = 
          rrcsib_decode_and_save_sib_for_current_event(wrrc_as_id,(void *)&(sibdb.cell[dbindex].sibs.sib11),
        ...
        case rrc_SIB12 :
          ...
          decode_succeeded = 
          rrcsib_decode_and_save_sib_for_current_event(wrrc_as_id,(void *)&(sibdb.cell[dbindex].sibs.sib12),
        ...
        case rrc_SIB18 :
          ...
          decode_succeeded = 
          rrcsib_decode_and_save_sib_for_current_event(wrrc_as_id,(void *)&(sibdb.cell[dbindex].sibs.sib18),
        ...
        case rrc_SIB11bis :
          // call the function that decodes sib18 and returns true/false based on decode status
          decode_succeeded = 
          rrcsib_decode_and_save_sib_for_current_event(wrrc_as_id,(void *)&(sibdb.cell[dbindex].sibs.sib11bis),
        ...
        case rrc_SIB19 :
          // call the function that decodes sib18 and returns true/false based on decode status
          decode_succeeded = 
          rrcsib_decode_and_save_sib_for_current_event(wrrc_as_id,(void *)&(sibdb.cell[dbindex].sibs.sib19),
      ...
      if( sib_events_ptr->event[index].event_name == RRCSIB_UPDATE_SIBS )
      {
        if( decode_succeeded == TRUE )
        {
          /* This means a SIB has changed for the serving cell
          and it has been decoded correctly. */
          rrc_sibdb_save_active_cell_sib_change(wrrc_as_id, (rrc_SIB_type)count );        
        }
      }
...
)
{
  ...
  {
    return rrccsp_is_current_band(wrrc_as_id,band_class,plmn);
  }
  else
  {
...
)
{
  ...
  if(sib_events_ptr->event[index].tempholder.mib != NULL)
  {
      ...
      )
    {
      ...
      status = FALSE;
      rrcsib_post_process_failed_event(wrrc_as_id, index, /* Event Index                 */
                                            TRUE,  /* Confirm Command is required */
                                            SIB_EVENT_FAILED_INVALID_SIBS, /* Failure cause */
                                            sbcch_teardown_required   /* Tear down S_BCCH if it's up */
                                          );
...
)
{
     ...
     (sib_events_ptr->event[index].tsib[rrc_SB1].sib_decoded == TRUE))
  {
    ...
    if(rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_SB1,sib_events_ptr->event[index].sib_db_index) == NULL)
    {
      WRRC_ERR0_FATAL("SB1 is received and decoded but not available in SIB DB");
    }
    ...
    if(FAILURE == rrcsib_check_if_scheduling_info_received_for_all_sibs
      (wrrc_as_id,sibdb.cell[sib_events_ptr->event[index].sib_db_index].sibs.sb1, 
       rrc_SB1, 
       index))
    {
      rrcsib_process_sib_scheduling_info_incomplete(wrrc_as_id,index); 
     ...
     (sib_events_ptr->event[index].tsib[rrc_SB2].sib_decoded == TRUE))
  {
    ...
    if(rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_SB2,sib_events_ptr->event[index].sib_db_index) == NULL)
    {
      WRRC_ERR0_FATAL("SB2 is received and decoded but not available in SIB DB");
    }
    /* Now do the value-tag comparisons */
    update_event_for_received_value_tags( wrrc_as_id,
    rrc_SB2,       /* For the SB2 */
    ...
    if(FAILURE == rrcsib_check_if_scheduling_info_received_for_all_sibs
       (wrrc_as_id,sibdb.cell[sib_events_ptr->event[index].sib_db_index].sibs.sb2, 
        rrc_SB2, 
        index))
    {
      rrcsib_process_sib_scheduling_info_incomplete(wrrc_as_id,index); 
...
rrc_sib_status_e_type status = SIB_EVENT_SUCCEEDED;
  ...
  rrc_csp_scan_e_type curr_scan_type = rrccsp_get_curr_scan_no_f3(wrrc_as_id);
     ...
     (sib_events_ptr->event[index].sib_scan_type == RRC_SIB_AUTO_BPLMN_SCAN))
  {
    ...
    if(rrcsibproc_int_data.rrcsib_bplmn_sibs.mib != NULL)
    {
      if(TRUE == decode_completed_sibs_for_bplmn(wrrc_as_id, index))
      {
        return TRUE;
      }
    }
  ...
  if( sib_events_ptr->event[index].event_name == RRCSIB_UPDATE_SIBS )
  {
    rrcsib_decode_sib7_in_update_sibs(wrrc_as_id,index);
  }
     ...
     (!sib_events_ptr->event[index].tsib[rrc_SIB1].sib_decoded))
  {
    ...
    sib1_ptr =(rrc_SysInfoType1*)rrcasn1_decode_pdu(wrrc_as_id,(void *)
                                                   (sib_events_ptr->event[index].
    ...
    { 
      ...
      if(sib_events_ptr->event[index].for_serving_cell == FALSE)
      {
        ...
        if(sib_events_ptr->event[index].sib_db_index_found)
        {
          ...
          rrc_translate_gsm_map_plmn_id(wrrc_as_id,
        ...
        if(rrcrce_abort_resel(wrrc_as_id,plmn,sib1_ptr,sib_events_ptr->ngh_bcch.freq,sib_events_ptr->ngh_bcch.scr_code))
        {
          rrcsib_post_process_failed_event(wrrc_as_id,index,
                                           TRUE,
                                           SIB_EVENT_FAILED_OTHER, 
                                           FALSE);
          rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType1_PDU,(void *)sib1_ptr);
          return FALSE;
        }
      }

      if(rrccsp_check_sib1_validity(wrrc_as_id,sib1_ptr) == FALSE)
      {
        rrcsib_post_process_failed_event(wrrc_as_id,(sib_events_index_e_type)index,
                                          TRUE,
      ...
      {               
        if(sib_events_ptr->event[index].sib_db_index_found )
        {
          ...
          dbindex = sib_events_ptr->event[index].sib_db_index;
          temp_mib_ptr = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_MIB,dbindex);

          if(temp_mib_ptr != NULL)
          {
            if(rrccsp_check_for_lai_forbidden(wrrc_as_id,sib1_ptr,temp_mib_ptr))
            { 
              rrcsib_post_process_failed_event(wrrc_as_id,(sib_events_index_e_type)index,
                                                TRUE,
        ...
        {
          if(rrccsp_check_for_lai_forbidden(wrrc_as_id,sib1_ptr,sib_events_ptr->event[index].tempholder.mib))
          { 
            rrcsib_post_process_failed_event(wrrc_as_id,(sib_events_index_e_type)index,
                                              TRUE,
                                              SIB_EVENT_FAILED_FB_LA, 
  ...
  if( sib_events_ptr->event[index].sib_db_index_found )
  {
    ...
    decode_completed_sibs(wrrc_as_id, index);
  }
  else
  {
      ...
      && (!(sib_events_ptr->event[index].tempholder.mib_decoded)))
    {
      ...
      sib_events_ptr->event[index].tempholder.mib =
        (rrc_MasterInformationBlock *)rrcasn1_decode_pdu( wrrc_as_id,
          (void *)(sib_events_ptr->event[index].
      ...
      {
        if(rrcsib_check_if_plmn_is_gsm_map(wrrc_as_id,index,FALSE) != FALSE)
        {
          rrc_translate_gsm_map_plmn_id(wrrc_as_id,&(sib_events_ptr->event[index].tempholder.mib->plmn_Type.u.gsm_MAP->plmn_Identity),&plmn);
          if(sib_events_ptr->event[index].for_serving_cell)
          {
            band_class = rrc_get_frequency_band(wrrc_as_id,sib_events_ptr->srv_bcch.freq, plmn);
          }
          else
          {
            band_class = rrc_get_frequency_band(wrrc_as_id,sib_events_ptr->ngh_bcch.freq, plmn);
          }
          sys_band = rrc_convert_band_to_sys_band(wrrc_as_id,band_class);
          nas_plmn = mmumts_convert_rrc_plmn_id_to_nas_plmn_id(plmn);
#ifdef FEATURE_DUAL_SIM
          sd_update_manual_search_bst_per_subs(nas_plmn,SYS_SYS_MODE_WCDMA,sys_band,rrc_get_as_id(wrrc_as_id));
#else
          sd_update_manual_search_bst(nas_plmn,SYS_SYS_MODE_WCDMA,sys_band);
#endif
        }
        else
        {
        ...
        WRRC_MSG1_MED("CSG: CSG indicator %d",sib_events_ptr->event[index].is_csg);
           ...
           (sib_events_ptr->event[index].req_proc == RRC_PROCEDURE_CSP))
        {
          ...
          if(sib_events_ptr->event[index].for_serving_cell == TRUE)
          {         
            if(is_freq_in_overlapping_850_and_800_bands(wrrc_as_id,sib_events_ptr->srv_bcch.freq)
               && (FALSE == rrccsp_is_sib_mcc_and_850_800_current_band_comb_valid(wrrc_as_id,plmn)))
            {
              /* Save the barred freq to not skip scanning of the same in other overlapping band*/
              rrc_csp_save_barred_freq(wrrc_as_id);
          ...
          {
            if(is_freq_in_overlapping_850_and_800_bands(wrrc_as_id,sib_events_ptr->ngh_bcch.freq)
               && (FALSE == rrccsp_is_mcc_and_850_800_supported_band_comb_valid(wrrc_as_id,
          ...
          )
        {
            ...
            if(rrccsp_skip_sibs_for_fplmn(wrrc_as_id,plmn
                                          ,sib_events_ptr->event[index].tempholder.mib
              ) == TRUE)
            {
              WRRC_MSG0_HIGH("FPLMN: Cell belongs to Forbidden PLMN. Skip reading SIBs");
              if(rrc_csp_int_data.curr_acq_entry.freq == rrc_csp_int_data.last_camped_freq_in_idle_mode)
              {
                rrc_csp_int_data.lcf_in_idle_mode_skip_status = RRC_CSP_SKIP_FREQ_FPLMN;
              }
              rrcsib_post_process_failed_event(wrrc_as_id,(sib_events_index_e_type)index,
                                               TRUE,
            ...
            if(rrccsp_skip_sibs_for_vplmn(wrrc_as_id,plmn, sib_events_ptr->event[index].tempholder.mib) == TRUE)
            {
           ...
           if((rrc_csp_int_data.curr_scan  == RRC_CSP_GSM_TO_WCDMA_REDIRECTION_SCAN) && 
              (FALSE  == rrccsp_check_plmn_match(wrrc_as_id,plmn,sib_events_ptr->event[index].tempholder.mib, TRUE)))
           {
             ...
             rrccsp_reset_num_tries_dest_freq(wrrc_as_id);
             return FALSE;
           }
           ...
           )
        {
            ...
            if(rrcsibproc_int_data.rrcsib_bplmn_sibs.mib == NULL)
            {
              ...
              rrcsibproc_int_data.rrcsib_bplmn_sibs.mib = (rrc_MasterInformationBlock *)rrcasn1_decode_pdu( wrrc_as_id,
                (void *)(sib_events_ptr->event[index].
                         tsib[rrc_MIB].sib_data_ptr), /* Encoded MIB */
                rrc_MasterInformationBlock_PDU,       /* MIB's PDU number        */
              ...
              if(rrcsibproc_int_data.rrcsib_bplmn_sibs.mib != NULL)
              {
                 ...
                 {
#ifdef FEATURE_RRC_BAND_TYPE_DETECTION_FOR_BPLMN
                WRRC_MSG2_MED("SB1 needed = %d SB2 needed = %d",sib_events_ptr->event[index].tsib[rrc_SB1].sib_needed,
                ...
                for(ix=0;ix< sibSb_ptr->n;ix++)
                {
                  ...
                  ((sib_type_tag.sib_type == rrc_SIB5) ||
                    (sib_type_tag.sib_type == rrc_SIB5bis)))
                {
                  WRRC_MSG0_MED("BTD:-SIB5/SIB5bis found in MIB");
                  sib_events_ptr->event[index].tsib[rrc_SB1].sib_needed = FALSE;
                  sib_events_ptr->event[index].tsib[rrc_SB2].sib_needed = FALSE;
                }
#endif
        ...
        //check if scheduling info is received for all sibs
        if(FAILURE == rrcsib_check_if_scheduling_info_received_for_all_sibs(wrrc_as_id,
        ...
        if( sib_events_ptr->event[index].event_name != RRCSIB_UPDATE_SIBS )
        {
          rrcsib_update_sched_info_from_mib_sb(wrrc_as_id,(uint32)rrc_MIB,
           ...
           (sib_events_ptr->event[index].event_name == RRCSIB_GET_SPECIFIC_SIBS))
        {
            ...
            /*In Network sharing cell there may be more than one PLMN broadcasted
               in a cell, so pass MIB pointer.this csp function will check if its N/W 
               shared cell then it will extract all the PLMN's for its processing*/
            if(FALSE == rrccsp_is_in_bplmn_list(wrrc_as_id,plmn
                                              ,sib_events_ptr->event[index].tempholder.mib
              ))
            {
              ...
              for( count = 0; count < (uint32)MAX_SIB_EVENTS_INDEX; count++ )
              {
                if (sib_events_ptr->event[count].event_active == TRUE)
                {
                  rrcsib_post_process_failed_event(wrrc_as_id,(sib_events_index_e_type)count,TRUE,SIB_EVENT_FAILED_OTHER, FALSE);
                }
        ...
        )
      {
        ...
        if((sib_events_ptr->event[index].tempholder.mib->mib_ValueTag ==
           ...
           ||(rrcsib_is_event_sixhrs_sib_update(wrrc_as_id,index) == TRUE)
          )
        {
          ...
          if(update_vtags_of_sibs_in_sb1_sb2_if_received(wrrc_as_id,index))
          {
            return TRUE;
          }
          ...
          (curr_scan_type == RRC_CSP_CELL_FACH_TO_URA_PCH_SCAN)))
      {
        ...
        db_index = rrc_sibdb_find_cell_index_for_plmn_freq_psc(wrrc_as_id,plmn,sib_events_ptr->srv_bcch.freq,
        ...
        if( db_index != SIBDB_INVALID_INDEX )
        {
          ...
          /* Not using invalid SIB index check here as it is already present inside the
             below function */
          sib3 =  rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_SIB3,db_index);
...
#endif
          {
            if(sibdb.cell[db_index].ten_min_count < MAX_CELL_INDEX_COUNT_TENMIN)
            {
              ...
              if((sibdb.cell[db_index].sibs.mib_encoded_pdu_length == GET_PDU_LENGTH_IN_BYTES(sib_events_ptr->event[index].tsib[rrc_MIB].sib_tot_length)) &&
                 (!memcmp((void*) sibdb.cell[db_index].sibs.mib_encoded_ptr, (void*) sib_events_ptr->event[index].tsib[rrc_MIB].sib_data_ptr, 
                  sibdb.cell[db_index].sibs.mib_encoded_pdu_length)))
              {
                ...
                sib_events_ptr->event[index].sib_db_index = 
                rrc_sibdb_create_cell_index(wrrc_as_id,
         ...
         (sib_events_ptr->event[index].for_serving_cell == FALSE))   
      {
        ...
        db_index = rrc_sibdb_find_cell_index_for_plmn_freq_psc(wrrc_as_id,plmn,sib_events_ptr->ngh_bcch.freq,
       ...
       if(db_index != SIBDB_INVALID_INDEX)
       {
        ...
        /*Compare MIB before applying freq/psc optimization*/
        if((sibdb.cell[db_index].sibs.mib_encoded_pdu_length == GET_PDU_LENGTH_IN_BYTES(sib_events_ptr->event[index].tsib[rrc_MIB].sib_tot_length))&&
           (!memcmp((void*) sibdb.cell[db_index].sibs.mib_encoded_ptr, (void*) sib_events_ptr->event[index].tsib[rrc_MIB].sib_data_ptr,
            sibdb.cell[db_index].sibs.mib_encoded_pdu_length))
#ifdef FEATURE_FEMTO_CSG
       /* Apply freq and PSC optimization only for macro cells */ 
           &&(!sib_events_ptr->event[index].is_csg) && ((NULL != sib3) && (!CSG_ID_IN_SIB3(sib3)))
#endif
          )
        {
         ...
         rrc_csp_int_data.curr_acq_entry.cell_is_hybrid = FALSE;
          ...
          if( sib_events_ptr->event[index].sib_db_index != SIBDB_INVALID_INDEX )
          {
            ...
            if((sib_events_ptr->event[index].tsib[rrc_SIB1].sib_received)&&
               (sib_events_ptr->event[index].tsib[rrc_SIB1].sib_decoded))
            {
              sib1_ptr = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_SIB1,
                                                           sib_events_ptr->event[index].sib_db_index);
              if(rrcrce_abort_resel(wrrc_as_id,plmn,sib1_ptr,sib_events_ptr->ngh_bcch.freq,sib_events_ptr->ngh_bcch.scr_code))
              {
                 rrcsib_post_process_failed_event(wrrc_as_id,index,
          ...
          {
           WRRC_MSG0_MED("Not applying freq psc sibdb opt for CSG cell/ MIB didnt match");
          }
        }
      ...
      if((sib_events_ptr->event[index].tempholder.mib_decoded == TRUE) &&
          ((sib_events_ptr->event[index].sib_db_index_found != TRUE) ||
           (!check_event_completion_status(wrrc_as_id,index,rrcsib_can_nw_support_deferred_sib(wrrc_as_id,sib_events_ptr->event[index].sib_db_index))))
        )
      {
      ...
      && (!(sib_events_ptr->event[index].tempholder.sib3_decoded)) )
    {
      ...
      (wrrc_as_id,sib_events_ptr->event[index].tsib[rrc_SIB3].sib_data_ptr[0],
      rrc_SIB3);

      /* Call rrcasn1_decode_pdu to do ASN1 decoding. Since it returns a 
      void pointer, typecast it to the MIB's type. */
      sib_events_ptr->event[index].tempholder.sib3 =
        (rrc_SysInfoType3 *)rrcasn1_decode_pdu( wrrc_as_id,
          (void *)(sib_events_ptr->event[index].
      ...
      else
      {
        sib_events_ptr->event[index].tempholder.sib3_decoded = TRUE;
              ...
              (rrc_csg_int_data.is_whitelist_valid))
          {
            sib_events_ptr->event[index].csg_id = rrc_translate_cell_id
                            (wrrc_as_id, &(sib_events_ptr->event[index].tempholder.sib3->v4b0NonCriticalExtensions.v590NonCriticalExtension.
                                v5c0NoncriticalExtension.v670NonCriticalExtension.v770NonCriticalExtension.
                                v830NonCriticalExtension.v860NonCriticalExtension.sysInfoType3_v860ext.csgIdentity));
        ...
        WRRC_MSG1_MED("CSG:  Cell_is_hybrid %d",rrc_csp_int_data.curr_acq_entry.cell_is_hybrid);		  		
                ...
                (sib_events_ptr->event[index].for_csg))
        {
          ...
          if((rrccsp_asf_actively_in_prog(wrrc_as_id))&&(rrc_ccm_get_curr_camped_freq(wrrc_as_id) == rrccsp_get_serving_frequency(wrrc_as_id))&&
              sib_events_ptr->event[index].sib_scan_type == RRC_SIB_WTOW_AUTO_BPLMN_SCAN)
          {
        ...
        if(sib_events_ptr->event[index].tempholder.mib == NULL)
        {
          temp_mib = rrcsibproc_int_data.rrcsib_bplmn_sibs.mib;
        }
        else
        {
          temp_mib = sib_events_ptr->event[index].tempholder.mib;
        }
        rrc_translate_gsm_map_plmn_id(wrrc_as_id,&(temp_mib->plmn_Type.u.gsm_MAP->plmn_Identity),&plmn);
       ...
       if((sib_events_ptr->event[index].sib_scan_type != RRC_SIB_CELL_ID_READ_IN_DCH_SCAN)
         ...
         )
        {
          /* Below function performs, White list check, cell bar status. If it finds a cell as barred sends cell bar req to l1 */
           rrccsp_initial_suitability_check(wrrc_as_id,temp_mib,
                                           sib_events_ptr->event[index].tempholder.sib3,
                                           plmn,
#ifdef FEATURE_FEMTO_CSG
                                           sib_events_ptr->event[index].csg_id,
      ...
      /* Checking S-Criteria */
        ...
        {
          if(FALSE == rrc_check_serv_cell_s_crit(wrrc_as_id,sib_events_ptr->event[index].tempholder.sib3))
          {
      ...
      { 
          ...
          )
        {
          ...
          if(rrcsibproc_int_data.rrcsib_bplmn_sibs.sib3 == NULL)
          {
            ...
            /* Decode and store SIB1 in bplmn sib database */
            rrcsibproc_int_data.rrcsib_bplmn_sibs.sib3 = 
            (rrc_SysInfoType3 *)rrcasn1_decode_pdu( wrrc_as_id,
                               (void *)(sib_events_ptr->event[index].
                               tsib[rrc_SIB3].sib_data_ptr), /* Encoded SIB3 */
                               rrc_SysInfoType3_PDU, /* SIB3's PDU number         */
          ...
          rrcsib_free_tsib(wrrc_as_id,index);
        }
        else
        {
          if(sib_events_ptr->event[index].sib_scan_type == RRC_SIB_CELL_ID_READ_IN_DCH_SCAN)
          ...
          else
          {
            if(sib_events_ptr->event[index].tempholder.mib!= NULL)
            {
              ...
              cell = rrc_translate_cell_id
                      (wrrc_as_id, &(sib_events_ptr->event[index].tempholder.sib3->cellIdentity));
              WRRC_MSG4_HIGH( 
...
#ifdef FEATURE_FEMTO_CSG
              sib_events_ptr->event[index].sib_db_index = rrc_sibdb_create_cell_index(wrrc_as_id, plmn, cell,index,sib_events_ptr->event[index].csg_id,sib_events_ptr->event[index].is_csg);
#else
              sib_events_ptr->event[index].sib_db_index = rrc_sibdb_create_cell_index(wrrc_as_id, plmn, cell,index);
#endif
#ifdef FEATURE_WCDMA_FREE_FLOATING_TASK
#ifdef FEATURE_DUAL_WCDMA_SIBDB_SHARING 
              if(!sibdb.last_plmn_and_cell_id_matched)
              {
                /* SIB DB entry doesnt exist for the PLMN, CELL ID combination in this SUB. check if an entry exists in alternate SUB */
                if(rrcff_get_sibdb_entry_from_alt_sub(wrrc_as_id,plmn,cell,&sibdb.cell[sib_events_ptr->event[index].sib_db_index]))
                {
                  WRRC_MSG0_HIGH("DUAL_W:SIBDB entry exists for the current cell in alternate SIBDB");
                  rrc_sibdb_set_decoded_sibs_to_null(wrrc_as_id,sib_events_ptr->event[index].sib_db_index);
                  sibdb.cell[sib_events_ptr->event[index].sib_db_index].age_index = 0;
                  sibdb.last_plmn_and_cell_id_matched = TRUE;
                  sibdb.cell_found_in_alt_sibdb = TRUE;
                }
              }
#endif
#endif
              ...
              if((rrc_ccm_get_serv_plmn_cellid(wrrc_as_id,&cell_id,&plmn_id) == RRC_NEW_CELL_SELECTED)&&
                 (sib_events_ptr->event[index].for_serving_cell == FALSE)&&
                 (sib_events_ptr->event[index].sib_scan_type == RRC_SIB_NORMAL)
                )
              {
                if((cell_id == cell)&&
                  (rrc_bcch_plmns_match(wrrc_as_id,plmn_id,plmn) == TRUE)
                  )
                {
                  sibdb.cell[sib_events_ptr->event[index].sib_db_index].six_hr_count = 
                    MAX_CELL_INDEX_COUNT_SIXHR_CELL_SEL_RESEL;
                  WRRC_MSG3_ERROR("Target cell same as Current Camped Cell, Cell I.D %d, PLMN I.D MCC %d MNC %d",
                                   cell,
                                   RRC_CSP_GET_INT_MCC(plmn_id),
                                   RRC_CSP_GET_INT_MNC(plmn_id)
                                 ); 
                   rrcsib_post_process_failed_event(wrrc_as_id,(sib_events_index_e_type)index,TRUE,SIB_EVENT_FAILED_OTHER, FALSE);
                   rrcsib_initiate_cell_selection(wrrc_as_id);
                   return FALSE;
                }
              }
              /* Since we've found the database index and we've gotten the
              ...
              update_event_for_received_value_tags( wrrc_as_id,
                ...
                sib_events_ptr->event[index].sib_db_index);
#ifdef FEATURE_DUAL_WCDMA_SIBDB_SHARING
              if((sibdb.vtag_mismatch == TRUE)&& (sibdb.cell_found_in_alt_sibdb == TRUE))
              {
                rrcsib_clear_altsub_sibdb_cell_index(wrrc_as_id,plmn,cell);
              }
              /* After the command is sent, cell_found_in_alt_sibdb should be set to FASLE again*/
              sibdb.cell_found_in_alt_sibdb = FALSE;
              sibdb.vtag_mismatch = FALSE;
#endif
              if(update_vtags_of_sibs_in_sb1_sb2_if_received(wrrc_as_id,index))
              {
...
)
{
  ...
  if( sb1_found == TRUE )
  {
    ...
    if( sibdb.cell[dbindex].sib_valid[rrc_SB1] )
    {
      ...
      {
        WRRC_MSG0_ERROR("SB1 pointer is NULL");
        return FALSE;
      }
    }
    else
  ...
  if( sb2_found == TRUE )
  {
    ...
    if( sibdb.cell[dbindex].sib_valid[rrc_SB2] )
    {
      ...
      {
        WRRC_MSG0_ERROR("SB2 pointer is NULL");
        return FALSE;
      }
    }
    else
...
)
{
     ...
     ))
  {
    if(rrcsibproc_int_data.rrcsib_bplmn_sibs.mib != NULL)
    {
#ifdef FEATURE_RRC_BAND_TYPE_DETECTION_FOR_BPLMN
      if(sib_events_ptr->event[index].tsib[rrc_SB1].sib_needed == TRUE &&
         rrcsibproc_int_data.rrcsib_bplmn_sibs.sb1 == NULL)
      {
        WRRC_MSG0_HIGH("BAND_TYPE_DETECTION - SB1 not read so far ");
        return FALSE;
      }
      if(sib_events_ptr->event[index].tsib[rrc_SB2].sib_needed == TRUE &&
         rrcsibproc_int_data.rrcsib_bplmn_sibs.sb2 == NULL)
      {
      ...
      rrcsibproc_int_data.rrcsib_bplmn_sibs.prev_freq=0;
      rrcsibproc_int_data.rrcsib_bplmn_sibs.prev_scr=0;
      return TRUE;
    }
    return FALSE;
  }
  ...
  if((rrcsibproc_int_data.rrcsib_bplmn_sibs.mib != NULL) && 
     (rrcsibproc_int_data.rrcsib_bplmn_sibs.sib3 != NULL) &&(
  #ifdef FEATURE_FEMTO_CSG   
     (sib_events_ptr->event[index].for_csg)||
  #endif
      (rrcsibproc_int_data.rrcsib_bplmn_sibs.sib1 != NULL)))
  {
#ifdef FEATURE_RRC_BAND_TYPE_DETECTION_FOR_BPLMN
    if((sib_events_ptr->event[index].sib_scan_type == RRC_SIB_WTOW_MANUAL_BPLMN_SCAN)||
       (sib_events_ptr->event[index].sib_scan_type == RRC_SIB_MANUAL_SCAN) ||
...
)
{
  ...
  if((rrc_csp_int_data.rrc_csfb_call_status == TRUE) &&
     (sib_events_ptr->event[index].sib_scan_type == RRC_SIB_NORMAL) &&
     (sib_events_ptr->event[index].event_name == RRCSIB_GET_SPECIFIC_SIBS)&&
     ((rrcnv_data.rrc_csfb_skip_sib11_opt_nv == TRUE) 
#ifdef FEATURE_WCDMA_DEFERRED_SIB11_12_READING
      || 
     ((deferred_meas_read_supported == TRUE) &&
       (RRC_STATE_DISCONNECTED == rrc_get_state(wrrc_as_id)))
#endif
      )
    )
  {
      ...
      if(sib_events_ptr->event[index].sib_db_index_found == TRUE)
      {
          ...
          for( count=1; count<RRC_TOT_SIBS; count++ )
          {
            if(sib_events_ptr->event[index].tsib[count].sib_decoded == FALSE)
            {
              ...
              /*WRRC_MSG2_HIGH("CSFB Status TRUE, Invalidating SIB %d for DB index %d as value tags have changed",
                    count,sib_events_ptr->event[index].sib_db_index);*/
  ...
  for( count = 0; count < RRC_TOT_SIBS; count++ )
  {
        ...
        (!(sib_events_ptr->event[index].tsib[count].sib_decoded)) )
    {
      ...
      switch( count )
      {
        ...
        blocks */
        if( (rrcsib_is_sib_scheduled(wrrc_as_id, rrc_SIB2,
                dbindex
#ifdef FEATURE_WCDMA_DEFERRED_SIB11_12_READING
              , FALSE
              ,NULL
        ...
        {
          WRRC_MSG0_MED("SIB2 is scheduled in MIB or SB");
          return( FALSE );
        }
        else
        {
          sib_events_ptr->event[index].tsib[count].sib_needed = FALSE;
          WRRC_MSG0_MED("SIB2 is not scheduled in MIB or SB");
        }
        break;
        ...
        /* Check is SIB18 is scheduled in the MIB or the Scheduling
           blocks */
        if( (rrcsib_is_sib_scheduled(wrrc_as_id, rrc_SIB18,
              dbindex 
#ifdef FEATURE_WCDMA_DEFERRED_SIB11_12_READING
            , FALSE
            , NULL
...
#endif
          {
            WRRC_MSG0_MED("SIB18 is scheduled in MIB or SB");
            return( FALSE );
          }
        }
        else
        {
          sib_events_ptr->event[index].tsib[count].sib_needed = FALSE;
          WRRC_MSG0_MED("SIB18 is not scheduled in MIB or SB");
        }
        break;
        ...
        case rrc_SIB4:
             ...
             (sib_events_ptr->event[index].tsib[rrc_SIB3].sib_decoded))
          {
            if(rrcsib_check_if_connected_mode_sibs_present
              (wrrc_as_id,sibdb.cell[dbindex].sibs.sib3_encoded_ptr[0],
              rrc_SIB3) == TRUE )
            {
              ...
              {
                WRRC_MSG0_MED("SIB4 is scheduled in MIB or SB");
                return( FALSE );
              }
              else
              {
                WRRC_MSG0_MED("SIB4 is not scheduled in MIB or SB");
              }
            }
          }
          else
        ...
        case rrc_SIB6:
             ...
             (sib_events_ptr->event[index].tsib[rrc_SIB5].sib_decoded))
          {
            ...
            if(rrcsib_check_if_connected_mode_sibs_present
              (wrrc_as_id,sibdb.cell[dbindex].sibs.sib5_encoded_ptr[0],
              rrc_SIB5) == TRUE )
            {
              ...
              {
                WRRC_MSG0_MED("SIB6 is scheduled in MIB or SB");
                return( FALSE );
              }
              else
              {
                WRRC_MSG0_MED("SIB6 is not scheduled in MIB or SB");
              }
            }
          }
          else
        ...
        case rrc_SIB12:
             ...
             (sib_events_ptr->event[index].tsib[rrc_SIB11].sib_decoded))
          {
            ...
            if(rrcsib_check_if_connected_mode_sibs_present
              (wrrc_as_id,sibdb.cell[dbindex].sibs.sib11_encoded_ptr[0],
              rrc_SIB11) == TRUE )
            {
              ...
              {
                WRRC_MSG0_MED("SIB 12 is scheduled in MIB or SB");
                return( FALSE );
              }
              else
              {
                WRRC_MSG0_MED("SIB 12 is not scheduled in MIB or SB");
              }
            }
          }
          else
        ...
        case rrc_SB1:
          if(rrc_sibdb_check_if_sb_present(wrrc_as_id,rrc_SB1, dbindex))
          {
        ...
        case rrc_SB2:
          if(rrc_sibdb_check_if_sb_present(wrrc_as_id,rrc_SB2, dbindex))
          {
            /* Scheduling block present but not received yet */
        ...
        case rrc_SIB11bis:
          ...
          {
            if(rrcsib_check_if_extension_sib_present(wrrc_as_id,dbindex) == TRUE)
            {
#ifdef FEATURE_WCDMA_DEFERRED_SIB11_12_READING
              if(deferred_meas_read_supported == FALSE)
#endif
            ...
            else
            {
              ...
              WRRC_MSG0_MED("sib11bis:SIB11bis not present in this cell");
        ...
        case rrc_SIB19:
          ...
          {
            if(rrcsib_check_if_sib19_present(wrrc_as_id,dbindex) == TRUE)
            {
#ifdef FEATURE_WCDMA_DEFERRED_SIB11_12_READING
          /* If DMCR is supported and LTOW redirection is in progress though SIB19 is present
            ...
            else
            {
              ...
              WRRC_MSG0_MED("sib19:SIB19 not present in this cell");
...
#ifdef FEATURE_WCDMA_TO_LTE
  rrcsib_check_and_remove_sib19_if_not_scheduled(wrrc_as_id,index);
#endif
     ...
     (sib_events_ptr->event[index].sib_scan_type == RRC_SIB_MANUAL_BPLMN_SCAN))
  {
    if(rrcsibproc_int_data.rrcsib_bplmn_sibs.mib != NULL)
    {
      ...
      for(ix=0;ix< sibSb_ptr->n;ix++)
      {
        ...
        {
          WRRC_MSG0_MED("BTD:-SIB5bis present in MIB. set is_sib5bis_scheduled to TRUE");
          return TRUE;
        }
        else if(RRC_CHECK_COMMON_MSG_TYPE(sibSb_ptr->elem[ix].sibSb_Type,
         rrc_SIBSb_TypeAndTag_sysInfoType5))
        {
          WRRC_MSG0_MED("BTD:-SIB5 present in MIB. set is_sib5bis_scheduled to FALSE");
          return FALSE;
        }
      }
    }
    if(rrcsibproc_int_data.rrcsib_bplmn_sibs.sb1!=NULL)
    {
      ...
      for(ix=0;ix< sb_ptr->n;ix++)
      {
        ...
        {
          WRRC_MSG0_MED("BTD:-SIB5bis present in SB1. set is_sib5bis_scheduled to TRUE");
          return TRUE;
        }
        else if(RRC_CHECK_COMMON_MSG_TYPE(sb_ptr->elem[ix].sib_Type,
            rrc_SIB_TypeAndTag_sysInfoType5))
        {
          WRRC_MSG0_MED("BTD:-SIB5 present in SB1. set is_sib5bis_scheduled to FALSE");
          return FALSE;
        }
      }
    }
    ...
    if(rrcsibproc_int_data.rrcsib_bplmn_sibs.sb2 !=NULL)
    {
      ...
      for(ix=0;ix< sb_ptr->n;ix++)
      {
        if(RRC_CHECK_COMMON_MSG_TYPE(sb_ptr->elem[ix].sib_Type,
         rrc_SIB_TypeAndTag_sysInfoType5bis))
        {
          WRRC_MSG0_MED("BTD:-SIB5bis present in SB2 set is_sib5bis_scheduled to TRUE");
          return TRUE;
        }
        else if(RRC_CHECK_COMMON_MSG_TYPE(sb_ptr->elem[ix].sib_Type,
         rrc_SIB_TypeAndTag_sysInfoType5))
        {
          WRRC_MSG0_MED("BTD:-SIB5 present in SB2 set is_sib5bis_scheduled to FALSE");
          return FALSE;
        }
      }
    }
...
)
{
     ...
     && (sib_events_ptr->event[index].sib_scan_type != RRC_SIB_AUTO_BPLMN_SCAN))
  {
    ...
    if (sib_events_ptr->event[index].int_event.int_event_type != RRCSIB_INT_SIB7_EVENT)
    {
      rrc_sibdb_store_value_tags_for_new_cell(wrrc_as_id,dbindex
      #ifdef FEATURE_WCDMA_DEFERRED_SIB11_12_READING
                                             ,index
      #endif
                                              );
  ...
  if( (sib_events_ptr->event[index].event_name == RRCSIB_UPDATE_SIBS) ||
      (sib_events_ptr->event[index].req_proc == RRC_PROCEDURE_SIB)
      ||(rrc_sib_is_current_event_sib_read_in_dch(wrrc_as_id) == TRUE)
    )
  {
#ifdef FEATURE_UPDATE_SIB7_IN_FACH
    if(
    ...
    {
      rrcllc_chan_cfg_reason_e_type chan_config_reason = RRCLLC_CHAN_CFG_REASON_NONE;
      if((update_sib7_in_connecting == TRUE)&&((rrcrce_is_rce_procedure_active(wrrc_as_id)) && ((rrc_get_state(wrrc_as_id) == RRC_STATE_DISCONNECTED) || (rrc_get_state(wrrc_as_id) == RRC_STATE_CONNECTING))))
      {
        WRRC_MSG0_HIGH("SIB7 Update in disconnected/connecting state, So skip Tearing down S-BCCH");
      }
      else
      {
        if(rrc_sib_is_current_event_sib_read_in_dch(wrrc_as_id) == TRUE)
  ...
  if( sib_events_ptr->event[index].event_name == RRCSIB_UPDATE_SIBS )
  {
    ...
    rrc_sibdb_notify_sib_change_to_procs(wrrc_as_id);

    /* Once that's done, clear the saved SIB change
    indications */
    rrc_sibdb_clear_active_cell_sib_change(wrrc_as_id);
    /*Reset six hrs sib count for serving cell sib db*/
    if((rrcsib_is_event_sixhrs_sib_update(wrrc_as_id,index) == TRUE)||
        (rrcsib_is_sixhrs_sib_timer_expired(wrrc_as_id,index) == TRUE)
      )
    {
    ...
    rrcsibproc_ext_data.sib_change_safe_guard = TRUE;
    ...
    {
      rrctmr_start_timer(wrrc_as_id, RRC_SIB_CHANGE_GUARD_TIMER,RRCSIB_SIB_CHANGE_GUARD_TMR_FOR_DEFERRED_SIB );
    }
    else
#endif
    {
    rrctmr_start_timer(wrrc_as_id, RRC_SIB_CHANGE_GUARD_TIMER,RRCSIB_SIB_CHANGE_GUARD_TIMER_VALUE );
  }
  }
  else
  {
    ...
    if( sib_events_ptr->event[index].req_proc != RRC_PROCEDURE_SIB )
    {
      if( (cnf_ptr = rrc_get_int_cmd_buf(wrrc_as_id))!=NULL )
      {
        ...
        {
          /* Copy the PLMN id */
          if( RRCGET_PLMN_TYPE((&rrcsibproc_int_data.rrcsib_bplmn_sibs.mib->plmn_Type))
                == RRCGET_PLMN_TYPE_TAG(gsm_MAP))
          {
            rrc_translate_gsm_map_plmn_id
              (wrrc_as_id,&(rrcsibproc_int_data.rrcsib_bplmn_sibs.mib->plmn_Type.u.gsm_MAP->plmn_Identity), &plmn);
          }
          cnf_ptr->cmd.get_sibs_cnf.plmn_id = plmn;
          
#ifdef FEATURE_RRC_BAND_TYPE_DETECTION_FOR_BPLMN
          if(rrcsib_is_sib5bis_scheduled(wrrc_as_id,index) == TRUE)
          {
          ...
          {
            /* Copy all Cell Id */
            cnf_ptr->cmd.get_sibs_cnf.cell_id = rrc_translate_cell_id(wrrc_as_id,&rrcsibproc_int_data.rrcsib_bplmn_sibs.sib3->cellIdentity);
#ifdef FEATURE_FEMTO_CSG
            cnf_ptr->cmd.get_sibs_cnf.csg_id = sib_events_ptr->event[index].csg_id;
            cnf_ptr->cmd.get_sibs_cnf.is_csg = sib_events_ptr->event[index].is_csg;
#endif
        ...
        else if(sib_events_ptr->event[index].sib_scan_type == RRC_SIB_CELL_ID_READ_IN_DCH_SCAN)
        {
          ...
          if(sib_events_ptr->event[index].tempholder.mib != NULL)
          {
            ...
            {
              rrc_translate_gsm_map_plmn_id 
                (wrrc_as_id,&(sib_events_ptr->event[index].tempholder.mib->plmn_Type.u.gsm_MAP->plmn_Identity),
                 &plmn
                );
            }
            if((rrccsp_is_primary_plmn_id_present(wrrc_as_id)) &&
               (rrc_is_nw_shared(wrrc_as_id,sib_events_ptr->event[index].tempholder.mib)))
            {
              rrc_plmn_identity_type shared_plmn_list[MAX_SHARED_PLMNS];
              uint8 count, num_of_shared_plmns;
              num_of_shared_plmns = rrccsp_construct_multiple_plmn_list_from_mib(wrrc_as_id, sib_events_ptr->event[index].tempholder.mib ,shared_plmn_list);
              for(count = 0;count < num_of_shared_plmns; count++)
              {
                if(rrc_bcch_plmns_match(wrrc_as_id,rrc_csp_int_data.curr_req_plmn.plmn_id,shared_plmn_list[count]))
                {
                  plmn = rrc_csp_int_data.curr_req_plmn.plmn_id;
                  break;
                }
              }
            }
          ...
          if(sib_events_ptr->event[index].tempholder.sib3 != NULL)
          {
             cnf_ptr->cmd.get_sibs_cnf.cell_id  = rrc_translate_cell_id
                      (wrrc_as_id, &(sib_events_ptr->event[index].tempholder.sib3->cellIdentity));
              WRRC_MSG3_HIGH("RRCGPS:Rexd Cell I.D CellId %d and MCC %d MNC %d", 
          ...
          if(sib_events_ptr->event[index].tsib[rrc_SIB1].sib_received == TRUE)
          {
           ...
           sib1_pdu_length = GET_PDU_LENGTH_IN_BYTES
            ...
            if(sib1 !=NULL) 
            {
              (void)rrccsp_fill_nas_common_info(wrrc_as_id,sib1, &nas_common_info);
              cnf_ptr->cmd.get_sibs_cnf.lac[0]=nas_common_info.value[0];
              cnf_ptr->cmd.get_sibs_cnf.lac[1]=nas_common_info.value[1];
              (void)rrcasn1_free_pdu(wrrc_as_id,rrc_SysInfoType1_PDU,(void*)sib1);
              sib1 = NULL;
            }
            else
            {
    ...
    else
    {
      ...
      rrcsib_process_int_sib_event_complete(wrrc_as_id, index, SUCCESS );
    }
...
)
{
  ...
  if( ptr->cmd.state_change_ind.new_state == RRC_STATE_DISCONNECTED ) 
  {
    if(sib_events_ptr == NULL)
    {
      ...
      rrc_sibdb_clear_active_cell_status(wrrc_as_id);
      WRRC_MSG0_HIGH("sib_events_ptr freed.");
      return;
    }

    rrcsibproc_int_data.rrcsib_cmd_store.pending_flag = FALSE;
    rrcsibproc_int_data.update_sibs_event_pending = FALSE;
  ...
  {
#ifdef FEATURE_WCDMA_DEFERRED_SIB11_12_READING
    rrcsib_update_sib_for_deferred_sib(wrrc_as_id);
#endif

    /* We've moved to CELL_FACH., reset the SIB event if the previous state was connecting */ 
    if(ptr->cmd.state_change_ind.previous_state == RRC_STATE_CONNECTING)
    {
      for( count = SIB_SRV_INDEX; count < MAX_SIB_EVENTS_INDEX; count++ )
      {
        if( sib_events_ptr->event[count].event_active == TRUE )
        {
           if(sib_events_ptr->event[count].event_name == RRCSIB_GET_SPECIFIC_SIBS)
           {
             if((sib_events_ptr->event[count].int_event.int_event_type == RRCSIB_INT_SIB7_EVENT) &&
              ...
              (sib_events_ptr->event[count].for_serving_cell == TRUE))
             {
               WRRC_MSG1_HIGH("sib7:Going out of connecting SIB event. Reset the event %d", 
                 sib_events_ptr->event[count].event_name);
               /* Post-process the failed event. No need to
                 tear down S-BCCH since it's already torn down
...
#ifdef FEATURE_UPDATE_SIB7_IN_FACH
#ifdef FEATURE_WCDMA_HS_RACH
  if(rrcsib_update_sib7_hs_rach(wrrc_as_id) == TRUE)
#endif
  {
    ...
    )
  {
            ...
            )
          {
            WRRC_MSG1_HIGH("sib7:Going out of FACH SIB event %d failed", 
             ...
             /* Post-process the failed event. No need to
                 ...
                 by LLC while going to PCH. */
            rrcsib_post_process_failed_event
              ( wrrc_as_id, (sib_events_index_e_type)count, /* Event Index                 */
                FALSE,  /* Confirm Command is required */
                SIB_EVENT_FAILED_INCOMPATIBLE_STATE, /* Failure cause */
                FALSE  /* Don't tear down S_BCCH */
               );
    ...
    if((drop_serving_bcch == TRUE)&&(rrcllc_get_s_bcch_setup_status(wrrc_as_id) == TRUE))
    {
      (void)rrcsib_configure_bcch(wrrc_as_id,SBCCH_TEARDOWN, NBCCH_NOOP
                                  ,RRCLLC_CHAN_CFG_REASON_NONE
                                  );
...
SIDE EFFECTS
  ...
  BCCH logical channels may be teared down. 
===========================================================================*/
static void rrcsib_process_sib_wait_timer_expiration(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  for( count=SIB_SRV_INDEX; count<MAX_SIB_EVENTS_INDEX; count++ )
  {
    if( sib_events_ptr->event[count].event_active == TRUE )
    {
      ...
      { 
        ...
        rrcsib_post_process_failed_event(wrrc_as_id,(sib_events_index_e_type)count,TRUE,SIB_EVENT_FAILED_TIMEOUT, FALSE);
        break;
      }
      else
      {
...
===========================================================================*/
static void rrcsib_process_sib7_exp_timer_expiration(sys_modem_as_id_e_type wrrc_as_id)
{
...
#ifdef FEATURE_WCDMA_HS_RACH
  if(rrcsib_update_sib7_hs_rach(wrrc_as_id) == FALSE)
    return;
#endif
  if( (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_FACH) ||
      (rrc_get_state(wrrc_as_id) == RRC_STATE_CONNECTING) )
  {
    ...
    command  */
    if( rrcsibproc_int_data.sib7info.sib_ext_proc_update_req == FALSE )
    {
      if((cmd = rrc_get_int_cmd_buf(wrrc_as_id))!=NULL)
      {
        ...
        rrcsibproc_int_data.sib7info.sib_timer_update_req = TRUE;
      }
    }
    else
    {
...
===========================================================================*/
static void rrcsib_process_bcch_setup_failure(sys_modem_as_id_e_type wrrc_as_id, rrc_sib_status_e_type fail_reason)
{
...
)
{
  ...
  if((rrclcm_get_dl_rb_id(wrrc_as_id,cmd_ptr->cmd.downlink_sdu_ind.lc_id)) == 
    BCCH_S_RADIO_BEARER_ID)
  {
    index = SIB_SRV_INDEX;  /* Message is from Serving cell BCCH */
  }
  else if((rrclcm_get_dl_rb_id(wrrc_as_id,cmd_ptr->cmd.downlink_sdu_ind.lc_id)) == 
  ...
  if( index < MAX_SIB_EVENTS_INDEX )
  {
    ...
    if( sib_events_ptr->event[index].event_active == FALSE )
    {
      ...
      {
        if(rrcsib_can_sib7_event_start(wrrc_as_id) == TRUE)
        {
          rrcsibproc_int_data.sib7info.sib_ext_proc_update_req = FALSE;
          rrcsib_process_sib7_exp_timer_expiration(wrrc_as_id);
        }
        else
        {
          WRRC_MSG0_MED("Ignoring the message since event not active");
        }
      }
      else
#endif
    ...
    else
    {
      ...
      if((rrcsib_check_sib_schedule_for_sibs(wrrc_as_id,msg
                                             ,index
                                             ) == FALSE)
        )
      {
      ...
      result = process_sys_info_msg(wrrc_as_id, msg, index );
      ...
      if( result == SIBS_COMPLETED )
      {
        ...
        if(FALSE == process_completed_sibs(wrrc_as_id, index ))
        {
          WRRC_MSG0_MED("Aborting SIB reading");
          return;
        }
      }
      if( ( WCDMA_RRC_IDLE_FEATURES(RRC_ISS_FOR_SIB_SEGMENTS) && (result != SIBS_FAILED_DUPLICATE)) ||
          (!WCDMA_RRC_IDLE_FEATURES(RRC_ISS_FOR_SIB_SEGMENTS) && (result == SIBS_COMPLETED))
        )
      {
        ...
        {
          if((rrcnv_data.wcdma_rrc_sib_sleep_sb_nv == FALSE)
#ifdef FEATURE_DUAL_SIM
              ||(rrcsibproc_int_data.g_in_data_call)
#endif
             )
          {
            ...
            {
              rrcsib_update_sib_sleep_schedule_table(wrrc_as_id,index);
            }
#ifdef FEATURE_DUAL_SIM
            else if(rrcsibproc_int_data.mib_sb_received)
            {
              rrcsibproc_int_data.mib_sb_received = FALSE;
              rrcsib_send_g2w_table(wrrc_as_id,index);
            }
#endif
          }
          else
      ...
      if(sib_events_ptr->event[index].sib_scan_type == RRC_SIB_CELL_ID_READ_IN_DCH_SCAN)
      {
        ...
        if(check_event_completion_status_for_cell_id_read(wrrc_as_id,(uint32)index) == TRUE )
        {
          post_process_completed_event(wrrc_as_id, index );
        }
      }
      else
      ...
      if((sib_events_ptr->event[index].sib_scan_type == RRC_SIB_WTOW_AUTO_BPLMN_SCAN) ||
         ...
         (sib_events_ptr->event[index].sib_scan_type == RRC_SIB_AUTO_BPLMN_SCAN))
      {
        if(check_event_completion_status_for_bplmn(wrrc_as_id,(uint32)index) == TRUE )
        {
          post_process_completed_event(wrrc_as_id, index );
        }
      }
      else 
      {
...
#ifdef FEATURE_WCDMA_DEFERRED_SIB11_12_READING
         if(rrcsib_can_event_support_deferred_sib(wrrc_as_id,index)== TRUE)
         {
           if(sib_events_ptr->event[index].sib_db_index_found == TRUE)
           {
             deferred_meas_read_supported =
               rrcsib_can_nw_support_deferred_sib(wrrc_as_id,sib_events_ptr->event[index].sib_db_index);
           }
         }
#endif
        ...
        if( check_event_completion_status( wrrc_as_id,(uint32)index 
#ifdef FEATURE_WCDMA_DEFERRED_SIB11_12_READING
        ,deferred_meas_read_supported
#endif
          ) == TRUE )
        {
          ...
          if(deferred_meas_read_supported == TRUE)
          {
            rrcsib_post_process_deferred_sib(wrrc_as_id,index, sib_events_ptr->event[index].sib_db_index);
          }
#endif
          ...
          {
            band_mismatch = rrcsib_check_band_mismatch_from_sib5_sib6(wrrc_as_id,(uint32)index);
          }
          ...
          if(TRUE == band_mismatch) 
          {
            ...
            rrccsp_send_cphy_cell_bar_req(wrrc_as_id,RRC_CSP_WCDMA_FREQ_BARRED, 1280);
          ...
          else
          {
            if(sib_events_ptr->event[index].for_serving_cell == FALSE)
            {
              if(rrctmr_get_remaining_time(wrrc_as_id, RRCTMR_WAIT_FOR_L2_ACK_FOR_PCH_TRANSITION_TIMER) != 0)
              {
        ...
        else if(WCDMA_RRC_IDLE_FEATURES(RRC_SKIP_SIBS_ON_NON_NAS_REQ_PLMN_OPT))
        {
          rrcsib_skip_sibs_for_non_nas_req_plmn(wrrc_as_id,index);
        }
      }
    }/* end Else */
...
SIDE EFFECTS
  None. 
===========================================================================*/
static boolean rrcsib_check_for_duplicate_update_sibs_cmd(  sys_modem_as_id_e_type wrrc_as_id, uint32 mib_value_tag )
{
...
===========================================================================*/
void rrcsib_update_tenmin_sib_timer(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8 count;
  ...
  rrctmr_start_timer(wrrc_as_id, RRCTMR_TENMIN_SIB_UPDATE_TIMER,
...
)
{
  ...
  rrc_crash_debug_save_proc_substates(wrrc_as_id,RRC_PROCEDURE_SIB,RRC_INVALID_PROC_SUBSTATE,cmd_ptr->cmd_hdr.cmd_id);
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    case RRC_GET_SPECIFIC_SIBS_REQ:
      ...
      WRRC_MSG4_HIGH("RRC_GET_SPECIFIC_SIBS_REQ cmd received contents:for_serving_cell %d,scan type %d, LLC SBCCH setup status %d, serving cell event %d",
      ...
      if(((cmd_ptr->cmd.get_specific_sibs_req.sib_scan_type == RRC_SIB_WTOW_AUTO_BPLMN_SCAN)||
          (cmd_ptr->cmd.get_specific_sibs_req.sib_scan_type == RRC_SIB_WTOW_MANUAL_BPLMN_SCAN)) &&
         (rrc_csp_return_pending_type(wrrc_as_id) != RRC_CSP_WTOW_BPLMN_DRX_SRCH_TIMER_PENDING)&&
          (WTOW_SEARCH_RESUME != rrc_csp_bplmn_srch_in_progress(wrrc_as_id)))
      {                                                                      
      ...
      if(((cmd_ptr->cmd.get_specific_sibs_req.sib_scan_type == RRC_SIB_NORMAL)||
          (cmd_ptr->cmd.get_specific_sibs_req.sib_scan_type == RRC_SIB_MANUAL_SCAN)) &&
          (rrc_csp_waiting_to_release_lock(wrrc_as_id)))
      {                                                                      
        ...
        )
      {
          ...
          )
        {
          ...
          else if(RRC_CAMPED_ON != rrc_ccm_get_curr_camping_status(wrrc_as_id, RRC_PROCEDURE_SIB))
          {
            WRRC_MSG0_HIGH( "sib7: UE not camped on. Ignoring sib7 event");
            break;
          }
        }
      }
      ...
      {
        if(rrc_csp_is_reselection_cleared(wrrc_as_id))
        { 
          ...
          rrcsib_post_process_failed_event(wrrc_as_id, SIB_NGH_INDEX, TRUE,
                                          SIB_EVENT_FAILED_OTHER, 
                                          FALSE  
                                        );
          break;
...
#ifdef FEATURE_3GPP_CSFB
      if((cmd_ptr->cmd.get_specific_sibs_req.sib_scan_type == RRC_SIB_READ_FROM_SIB_CONTAINER) &&
         (rrcsib_read_sibs_from_sys_info_container(wrrc_as_id,cmd_ptr) == TRUE))
      {
        csfb_sib_event.sib_container_sts = RRC_SIB_CONTAINER_VALID;
        WRRC_EVENT_REPORT_PAYLOAD(EVENT_WCDMA_RRC_CSFB_SIB_CONTAINER, sizeof(csfb_sib_event),
                       (void *)&csfb_sib_event);

        WRRC_MSG0_HIGH("SIB reading successful from SYS_INFO_CONTAINER");

#ifdef FEATURE_WCDMA_CSFB_CALL_OPT
       if(WCDMA_RRC_IDLE_FEATURES(RRC_CSFB_CALL_TIMELINE_OPT) && rrc_csp_int_data.rrc_csfb_call_status)
       {
         /* For CSFB call, if SIB container read is success post GET_SPEC_SIBS_CNF here itself */
         rrcsibproc_int_data.is_sib_cont_present = TRUE;
         rrc_sib_post_complete_for_csfb_sib_read(wrrc_as_id, SIB_EVENT_SUCCEEDED);

         event_payload.status = RRC_BCCH_SETUP_SKIPPED;   //Logging the event as the BCCH setup is skipped.
         WRRC_EVENT_REPORT_PAYLOAD(EVENT_WCDMA_CSFB_CALL_OPT, sizeof(event_payload),(void *)&event_payload);
       }
#endif
      }
      else
      {
        register_new_sib_event(wrrc_as_id, cmd_ptr );  
      }
#else
      register_new_sib_event(wrrc_as_id, cmd_ptr );
#endif
    ...
    case RRC_UPDATE_SIBS_IND:
      ...
      if((rrcsib_is_current_event_due_to_sfn_drift(wrrc_as_id) == TRUE)&&
        ...
        )
      {
        ...
        WRRC_MSG0_HIGH("Overwriting Update_sibs due to SFN drift with six hrs sib");
          ...
          (rrcsib_is_sib_active(wrrc_as_id) == TRUE)
        )
      {
        WRRC_MSG0_HIGH("Ignored UPDATE_SIBS cmd as a sib event is active");
      }
      else if(rrc_ccm_get_curr_camping_status(wrrc_as_id, RRC_PROCEDURE_SIB) == RRC_NOT_CAMPED_ON)
      {
        WRRC_MSG0_HIGH("Ignored UPDATE_SIBS cmd as there is no active cell");
      }
      else
      {
        if( (rrcsib_check_for_duplicate_update_sibs_cmd(wrrc_as_id,cmd_ptr->cmd.update_sibs_ind.mib_value_tag)) == TRUE )
        {
        ...
        else if((OC_SET_FOR_CELL_DCH == rrcllc_get_ordered_config_status(wrrc_as_id))||
                    (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH)
                  )
        {
          WRRC_MSG0_HIGH("Ignoring UPDATE_SIBS_IND since OC set for DCH");
        }
        else if(rrc_get_state(wrrc_as_id) == RRC_STATE_CONNECTING)
        {
          WRRC_MSG0_HIGH("Ignoring UPDATE_SIBS_IND since state is CONNECTING");
        }
        else if((rrc_get_state(wrrc_as_id) == RRC_STATE_DISCONNECTED) && (rrcrce_get_rce_sub_state(wrrc_as_id) != RRCRCE_INITIAL))
        {
        ...
        else
        {
          ...
          if(rrcsibproc_int_data.rrcsib_cmd_store.pending_flag == TRUE)
          {
            ...
            else
            {
              ...
              WCDMA_MEMCPY(&rrcsibproc_int_data.rrcsib_cmd_store.saved_cmd,
                           sizeof(rrc_sib_cmd_type),
                           cmd_ptr,
                           sizeof(rrc_cmd_int_type));
              rrcsibproc_int_data.update_sibs_event_pending = TRUE;
            }
          }
          else if(WTOW_SEARCH_STOP != rrc_csp_bplmn_srch_in_progress(wrrc_as_id))
          {
            rrcsibproc_int_data.rrcsib_cmd_store.pending_flag = TRUE;
            rrcsibproc_int_data.update_sibs_event_pending = TRUE;
  
            rrc_csp_send_bplmn_suspend_req(wrrc_as_id,RRC_PROCEDURE_SIB,BPLMN_SUSPEND_REASON_BCCH_MODIFICATION);
          ...
          else
          {
            ...
            if(rrcllc_get_n_bcch_setup_status(wrrc_as_id) == TRUE)
            {
              rrcsibproc_int_data.rrcsib_cmd_store.pending_flag = TRUE;
              rrcsibproc_int_data.update_sibs_event_pending = TRUE;
              ...
              if(rrctmr_get_remaining_time(wrrc_as_id, RRC_SIB_CHANGE_GUARD_TIMER) == 0)
              {
               /*Start 1sec timer for safe guarding processing of sib_change_ind against BPLMN*/
                rrctmr_start_timer(wrrc_as_id, RRC_SIB_CHANGE_GUARD_TIMER,RRCSIB_PENDING_UPDATE_SIB_TIMER_VALUE );
              }
            } 
            else
            {  
              ...
              cmd_ptr->cmd.chan_config_cnf.transaction_id)
          {
            ...
            if( cmd_ptr->cmd.chan_config_cnf.chan_cfg_status == RRCLLC_CHAN_CFG_SUCCESS )
            {
                 ...
                 ((rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_PCH)||(rrc_get_state(wrrc_as_id) == RRC_STATE_URA_PCH)))
              {
                if(rrcsibproc_int_data.sib7info.sib_ext_proc_update_req == TRUE)
                {
                  ...
                  ptr = rrc_get_int_cmd_buf(wrrc_as_id);
                  WRRC_MSG0_HIGH("Sending RRC_SIB7_UPDATE_STATUS_IND");
                  ptr->cmd_hdr.cmd_id = RRC_SIB7_UPDATE_STATUS_IND;
            ...
            else
            {
              ...
              if((RRCSIB_GET_SPECIFIC_SIBS == sib_events_ptr->event[count].event_name)&&
                 (sib_events_ptr->event[count].sib_scan_type == RRC_SIB_READ_FROM_SIB_CONTAINER))
              {
                rrc_sib_post_complete_for_csfb_sib_read(wrrc_as_id, SIB_EVENT_FAILED_LOWER_LAYERS);
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
                rrc_qsh_report_sib_decode_failure_event(wrrc_as_id, SIB_EVENT_FAILED_LOWER_LAYERS);
#endif
              }
              else
#endif
              {
                /*If NBCH setup failed due to no lock send SIB_EVENT_FAILED_NBCH_NO_LOCK as the failure reason*/
                if((sib_events_ptr->event[SIB_NGH_INDEX].event_active == TRUE) && (cmd_ptr->cmd.chan_config_cnf.chan_cfg_status == RRCLLC_CHAN_CFG_FAILURE_NO_LOCK ))
                {
                  rrcsib_process_bcch_setup_failure(wrrc_as_id, SIB_EVENT_FAILED_NBCH_NO_LOCK);
                }
                else
                {
                  rrcsib_process_bcch_setup_failure(wrrc_as_id, SIB_EVENT_FAILED_LOWER_LAYERS);
                }
              }
            }
          }
    ...
    case RRC_DOWNLINK_SDU_IND:
      ...
      if(cmd_ptr->cmd.downlink_sdu_ind.decoded_msg != NULL)
      {
        if(cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_type == rrc_BCCH_BCH_Message_PDU)
        {
          ...
          process_bcch_message(wrrc_as_id, cmd_ptr );
        }
          ...
          {  
           /*Compare the value of IE "MIB value tag" in the IE "BCCH modification info" 
                                           ...
                                           systemInformationChangeIndication_v860ext),rrc_,etws_Information)))
            {
              ...
              rrcbmc_send_etws_information_ind_message(wrrc_as_id,bcch_fach_msg_ptr->message.u.systemInformationChangeIndication.
                                                       laterNonCriticalExtensions.v860NonCriticalExtensions.
                                                       systemInformationChangeIndication_v860ext.etws_Information,NULL);
            }
#endif
  
            if((rrc_get_serving_cell_mib_value_tag(wrrc_as_id)) != (bcch_fach_msg_ptr->message.u.systemInformationChangeIndication.
                  bcch_ModificationInfo.mib_ValueTag))
            {
              ...
              {
                rrcpg1_process_bcch_mod_info_msg_ie(wrrc_as_id,
    ...
    case RRC_STATE_CHANGE_IND:
      ...
      rrcsib_process_state_change_ind(wrrc_as_id,  cmd_ptr );
      break;
    ...
    case RRC_SIB_WAIT_TIMER_EXPIRED_IND:
      ...
      rrcsib_process_sib_wait_timer_expiration(wrrc_as_id);
      break;
    ...
    case RRC_SIB_STOP_PARTITION_TMR_EXPIRED_IND:
      ...
      if(!rrcsibproc_int_data.g_in_data_call)
      {
        /*wl1 api to increase priority to channel maintenance inversion*/
        wl1_ds_toggle_prio_inv_for_sib_read(TRUE, wrrc_as_id);
        return;
      }
      rrcsibproc_int_data.rrcsib_stop_partition = TRUE;
      rrcsib_update_sib_sleep_schedule_table(wrrc_as_id,count);
      break;
#endif
    ...
    case RRC_BPLMN_SUSPEND_CNF:
      ...
      if(cmd_ptr->cmd.rrc_csp_bplmn_suspend_cnf.status == SUCCESS)
      {
        ...
        if( (rrcsibproc_int_data.rrcsib_cmd_store.pending_flag == TRUE) &&
            (rrcsibproc_int_data.rrcsib_cmd_store.saved_cmd.cmd_hdr.cmd_id == RRC_UPDATE_SIBS_IND) )
        {
          ...
          rrcsibproc_int_data.rrcsib_cmd_store.saved_cmd.cmd.update_sibs_ind.procedure;

          sib_events_ptr->update_sibs_cause = 
          rrcsibproc_int_data.rrcsib_cmd_store.saved_cmd.cmd.update_sibs_ind.cause;

          sib_events_ptr->update_sibs_real_sib_mod = 
		  rrcsibproc_int_data.rrcsib_cmd_store.saved_cmd.cmd.update_sibs_ind.real_sib_mod;
          /* Call the function to do the necessary processing */
          register_new_sib_event(wrrc_as_id,(rrc_cmd_type *)&rrcsibproc_int_data.rrcsib_cmd_store.saved_cmd);
        }
        else
        {
    ...
    /* Clear the command store */
      ...
      case RRC_SIX_HRS_SIB_TIMER_UPDATE_IND:
        rrcsib_update_sixhr_sib_timer(wrrc_as_id);
        break;

      case RRC_TEN_MIN_SIB_TIMER_UPDATE_IND:
        rrcsib_update_tenmin_sib_timer(wrrc_as_id);
        break;

      case RRC_SIB_CHANGE_GUARD_TIMER_EXPIRED_IND:
          rrcsibproc_ext_data.sib_change_safe_guard = FALSE;
          ...
          {
            /* Post a command to ourselves for updating the SIBs */
            rrcsib_send_command_for_saved_update_sibs_event(wrrc_as_id);
    ...
    case RRC_GET_SIBDB_ENTRY_FOR_CELL:
      cmd_out_ptr = rrc_get_alt_sub_ff_cmd_buf(WRRC_GET_ALT_SUB_ID(wrrc_as_id));
      cmd_out_ptr->cmd_hdr.cmd_id = RRC_GET_SIBDB_ENTRY_FOR_CELL_CNF;
      cmd_out_ptr->cmd.rrc_get_sibdb_entry_cnf.cell_found = FALSE;
      if(!sibdb.database_empty)
      {
        index = rrc_sibdb_find_valid_cell_index(wrrc_as_id,cmd_ptr->cmd.rrc_get_sibdb_entry.plmn_id, cmd_ptr->cmd.rrc_get_sibdb_entry.cell_id,MAX_SIB_EVENTS_INDEX);
        if(index != SIBDB_INVALID_INDEX)
        {
          /* This is a case where current sub is not yet activated but has SIB database*/
          if(sib_events_ptr != NULL)
          {
            for(count = SIB_SRV_INDEX; count < MAX_SIB_EVENTS_INDEX;count++)
            {
              if (sib_events_ptr->event[count].event_active == TRUE)
              {
                if((sib_events_ptr->event[count].sib_db_index_found == TRUE) && (sib_events_ptr->event[count].sib_db_index == index)  
#ifdef FEATURE_UPDATE_SIB7_IN_FACH
                   && (sib_events_ptr->event[count].int_event.int_event_type == RRCSIB_INT_MAX_EVENT)
#endif
                )
                {
                  WRRC_MSG0_HIGH("DUAL_W: Required SIBDB index is currently active, so ignoring the request from other SUB");
                  rrc_put_alt_sub_ff_cmd(WRRC_GET_ALT_SUB_ID(wrrc_as_id),cmd_out_ptr );
                  return;
                }
              }
            }
          ...
          /*Do not share SIBDB if MIB value tag is invalid*/
          if(sibdb.cell[index].sib_value_tag[rrc_MIB] != INVALID_MIB_SB_VALUE_TAG)
          {
            WRRC_MSG0_HIGH("DUAL_W: Valid cell is found for the plmn id, cell id combo");
            cmd_out_ptr->cmd.rrc_get_sibdb_entry_cnf.cell = rrc_malloc(sizeof(rrc_sibdb_cell_header_type));
            memset(cmd_out_ptr->cmd.rrc_get_sibdb_entry_cnf.cell, 0, sizeof(rrc_sibdb_cell_header_type));
            WCDMA_MEMCPY(cmd_out_ptr->cmd.rrc_get_sibdb_entry_cnf.cell,
                         sizeof(rrc_sibdb_cell_header_type),
                         &sibdb.cell[index],
                         sizeof(rrc_sibdb_cell_header_type));
            cmd_out_ptr->cmd.rrc_get_sibdb_entry_cnf.cell_found = TRUE;
          }
        }
      }
      rrc_put_alt_sub_ff_cmd(WRRC_GET_ALT_SUB_ID(wrrc_as_id),cmd_out_ptr );
...
#ifdef FEATURE_DUAL_WCDMA_SIBDB_SHARING 
       case RRC_CLEAR_SIBDB_INDEX_IND:
       dbindex = rrc_sibdb_find_cell_index(wrrc_as_id,cmd_ptr->cmd.rrc_clear_sibdb_index.plmn_id,cmd_ptr->cmd.rrc_clear_sibdb_index.cell_id);
       WRRC_MSG2_HIGH("RRC_CLEAR_SIBDB_INDEX_IND: active cell index-%d and index received-%d",sibdb.active_cell_index,dbindex);
       if(sibdb.active_cell_index  !=  dbindex)
       {
         rrc_sibdb_clear_a_cell_index(wrrc_as_id, dbindex);
       }
       break;
...
===========================================================================*/
void rrc_sib_proc_events_init(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  WRRC_MSG0_MED("Setting SIB cmd store pending flag to FALSE");
  rrcsibproc_int_data.rrcsib_cmd_store.pending_flag = FALSE;
  ...
  leaks. */
  if( rrcsibproc_ext_data.rrcsib_powerup_init_done == TRUE )
  {
    ...
    for( count = SIB_SRV_INDEX; count < MAX_SIB_EVENTS_INDEX; count++ )
    {
      rrcsib_free_tsib(wrrc_as_id,count);
    }
  }
...
===========================================================================*/
static void rrc_sib_database_init(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  may have to free any used SIB database pointers to avoid memory
  leaks. */
  if( rrcsibproc_ext_data.rrcsib_powerup_init_done == FALSE )
  {
    rrcsibproc_int_data.transaction_id = 0;
    rrc_sibdb_init(wrrc_as_id);
  }
  else
  {
    ...
    for( count=0; count<=SIBDB_MAX_INDEX; count++ )
    {
      if(sibdb.cell[count].index_used == TRUE)
      {
        ...
        rrc_sibdb_cell_header_init(wrrc_as_id,count);/* This frees all the SIBs */
      }
    }
...
===========================================================================*/
void rrc_sib_proc_init(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  rrc_sib_database_init(wrrc_as_id);
  ...
  the BCCH status. */
  rrcscmgr_register_for_scn(wrrc_as_id, RRC_PROCEDURE_SIB,  /* Procedure name */
  ...
  rrcscmgr_register_for_scn( wrrc_as_id,
                             RRC_PROCEDURE_SIB,   /* Procedure name */ 
                             RRC_STATE_WILDCARD,  /* From State     */     
  ...
  rrctmr_start_timer(wrrc_as_id, RRCTMR_SIXHR_SIB_UPDATE_TIMER,
                        RRCSIB_SIX_HR_SIB_UPDATE_TIMER );

  rrctmr_start_timer(wrrc_as_id, RRCTMR_TENMIN_SIB_UPDATE_TIMER,
                        RRCSIB_TENMIN_UPDATE_TIMER );
  /* Note that power-up initialization has been done */
  rrcsibproc_ext_data.rrcsib_powerup_init_done = TRUE;

  rrcsibproc_ext_data.Fdd1_internal_band[0] = RRC_CSP_IMT2K_BAND_INDEX;
  rrcsibproc_ext_data.Fdd1_internal_band[1] = RRC_CSP_PCS_BAND_INDEX;
  rrcsibproc_ext_data.Fdd1_internal_band[2] = RRC_CSP_WCDMA_1800_BAND_INDEX;
  rrcsibproc_ext_data.Fdd1_internal_band[3] = RRC_CSP_WCDMA_800_BAND_INDEX;
  rrcsibproc_ext_data.Fdd1_internal_band[4] = RRC_CSP_WCDMA_1700_2100_BAND_INDEX;
  rrcsibproc_ext_data.Fdd1_internal_band[5] = RRC_CSP_WCDMA_850_BAND_INDEX;
  rrcsibproc_ext_data.Fdd1_internal_band[6] = RRC_CSP_BAND_MAX_INDEX;
  rrcsibproc_ext_data.Fdd1_internal_band[7] = RRC_CSP_BAND_MAX_INDEX;

  rrcsibproc_ext_data.Fdd2_internal_band[0] = RRC_CSP_WCDMA_900_BAND_INDEX;
  rrcsibproc_ext_data.Fdd2_internal_band[1] = RRC_CSP_WCDMA_1700_BAND_INDEX;
  rrcsibproc_ext_data.Fdd2_internal_band[2] = RRC_CSP_BAND_MAX_INDEX;
  rrcsibproc_ext_data.Fdd2_internal_band[3] = RRC_CSP_WCDMA_1500_BAND_INDEX;
  rrcsibproc_ext_data.Fdd2_internal_band[4] = RRC_CSP_BAND_MAX_INDEX;
  rrcsibproc_ext_data.Fdd2_internal_band[5] = RRC_CSP_BAND_MAX_INDEX;
  rrcsibproc_ext_data.Fdd2_internal_band[6] = RRC_CSP_BAND_MAX_INDEX;
  rrcsibproc_ext_data.Fdd2_internal_band[7] = RRC_CSP_BAND_MAX_INDEX;
  rrcsibproc_ext_data.Fdd2_internal_band[8] = RRC_CSP_BAND_MAX_INDEX;
  rrcsibproc_ext_data.Fdd2_internal_band[9] = RRC_CSP_BAND_MAX_INDEX;
  rrcsibproc_ext_data.Fdd2_internal_band[10] = RRC_CSP_BAND_MAX_INDEX;
  rrcsibproc_ext_data.Fdd2_internal_band[11] = RRC_CSP_WCDMA_BC19_BAND_INDEX;
  rrcsibproc_ext_data.Fdd2_internal_band[12] = RRC_CSP_BAND_MAX_INDEX;
  rrcsibproc_ext_data.Fdd2_internal_band[13] = RRC_CSP_BAND_MAX_INDEX;
  rrcsibproc_ext_data.Fdd2_internal_band[14] = RRC_CSP_BAND_MAX_INDEX;
  rrcsibproc_ext_data.Fdd2_internal_band[15] = RRC_CSP_BAND_MAX_INDEX;
...
)
{
  ...
  if( new_cell_selection_succeeded == TRUE )
  {
    ...
    if( ( temp_index = rrc_sibdb_find_cell_index(wrrc_as_id, plmn, cell )) !=
        SIBDB_INVALID_INDEX )
    {
      WRRC_MSG1_HIGH("Serving Cell Index is now %d. Free ASN.1 decoded sibs for all entries", temp_index);
      ...
      */
      if( (rrcumi_check_ue_timer_constants_update_status(wrrc_as_id,TRUE))
          == RRC_CONN_TIMER_AND_CONSTANTS_UPDATE_REQ )
      {
        sib1 = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,
          rrc_SIB1,temp_index);
        if(sib1 != NULL)
        {
      ...
      else
      {
        ...
        if(sib1 != NULL)
        {
          (void)rrc_update_ue_conn_timers_and_constants(wrrc_as_id,NULL, NULL, 
             ((sib1->m.v3a0NonCriticalExtensionsPresent) &&
              (sib1->v3a0NonCriticalExtensions.m.v860NonCriticalExtentionsPresent))? 
              &(sib1->v3a0NonCriticalExtensions.v860NonCriticalExtentions.
              sysInfoType1_v860ext.ue_ConnTimersAndConstants):
    ...
    if( rrcsibproc_int_data.update_sibs_event_pending == TRUE )
    {
      ...
      if(rrctmr_get_remaining_time(wrrc_as_id, RRC_SIB_CHANGE_GUARD_TIMER) != 0)
      {
        rrctmr_stop_timer(wrrc_as_id,  RRC_SIB_CHANGE_GUARD_TIMER);
      }
    }
    rrcpg1_check_and_abort_bcch_mod_info_processing(wrrc_as_id);
  ...
  else
  {
    ...
    {
      WRRC_MSG0_HIGH("Serving Cell no longer valid! Free ASN.1 decoded sibs for all entries");
      for(count = 0; count <SIBDB_INVALID_INDEX; count++)
      {
        if(sibdb.cell[count].index_used == TRUE)
        {
...
)
{
  rrc_cmd_type *cmd; /* To send a SIB command if needed */
  uint32 count;      /* Local Counter */
  boolean result = FALSE;    /* Store result  */
...
/* when CSFB is active or SIB container is present bypass reading SIB7 */
  if((rrc_csp_int_data.rrc_csfb_call_status)||(rrcsibproc_int_data.is_sib_cont_present))
  {
    for( count = SIB_SRV_INDEX; count < MAX_SIB_EVENTS_INDEX; count++ )
    {
      if( sib_events_ptr->event[count].event_active == TRUE )
...
/*lint -e527 statement unreachable */
  ...
  if( sib_events_ptr->event[SIB_NGH_INDEX].event_active == TRUE)
  {
    ...
    if(rrccsp_is_inter_frequency_cell_resel_in_progress(wrrc_as_id))
    {
      ...
      rrcsib_post_process_failed_event
      ( wrrc_as_id, SIB_NGH_INDEX, /* Event Index                 */
        TRUE,  /* Confirm Command is required */
        SIB_EVENT_FAILED_OTHER, /* Failure reason */
  ...
  else if (rrcllc_get_n_bcch_setup_status(wrrc_as_id) == TRUE)
  {
      ...
      )
  {
    ...
    if( rrcsibproc_int_data.sib7info.sib_ext_proc_update_req == FALSE )
    {
...
#endif
      if(!((rrctmr_get_remaining_time(wrrc_as_id, RRCTMR_SIB7_EXPIRATION_TIMER) == 0) ||
         (rrccsp_is_inter_frequency_cell_resel_in_progress(wrrc_as_id))))
      {
      ...
      if (((update_sib7_in_connecting == FALSE) && result == FALSE)
          ||(update_sib7_in_connecting == TRUE))
      {
        if((cmd = rrc_get_int_cmd_buf(wrrc_as_id))!=NULL)
        {
          cmd->cmd_hdr.cmd_id = RRC_GET_SPECIFIC_SIBS_REQ;
          ...
          rrc_put_int_cmd(wrrc_as_id,  cmd );
          /*set the sib7_info parameters only if RCE is waiting for sib7 update*/
          if((update_sib7_in_connecting == FALSE) || 
             ((update_sib7_in_connecting == TRUE) &&(result == FALSE)))
          {
            /* Set an internal variable to indicate that another
            procedure requested the SIB7 update */
            rrcsibproc_int_data.sib7info.sib_ext_proc_update_req = TRUE;
            rrcsibproc_int_data.sib7info.req_proc = procedure;
          }
        }
      }
    }
...
)
{
  ...
  if(! rrchsrach_camped_cell_supports_hsrach(wrrc_as_id))
    return TRUE;
...
===========================================================================*/
void rrcsib_force_sib_event_init(  sys_modem_as_id_e_type wrrc_as_id,rrcsib_event_abort_cause_e_type abort_cause) 
{
  uint32  count; /* Counter used in loops */
  uint32  dbindex;
  uint32  sib;
...
#ifdef FEATURE_DUAL_SIM
  rrc_csp_scan_e_type curr_scan = rrccsp_get_curr_scan_no_f3(wrrc_as_id);

  rrc_dsds_int_data.rrc_drx_exp_when_wtng_for_mib = FALSE;
  ...
  {
    rrccsp_reset_wait_l1_rel_lock(wrrc_as_id);
  }
  wl1_ds_toggle_prio_inv_for_sib_read(FALSE, wrrc_as_id);
#endif
  ...
  switch (abort_cause)
  {
      ...
      for( count = 0; count < (uint32)MAX_SIB_EVENTS_INDEX; count++ )
      {
        ...
        if (sib_events_ptr->event[count].event_active == TRUE)
        {
          ...
          if( (sib_events_ptr->event[count].event_name == RRCSIB_GET_SPECIFIC_SIBS) &&
              ...
              (rrc_sib_is_current_event_sib7_in_fach(wrrc_as_id) == FALSE))
#endif
            )
          {
            ...
            {
              rrcsib_post_process_failed_event(wrrc_as_id,(sib_events_index_e_type)count,FALSE,SIB_EVENT_FAILED_OTHER, FALSE);
            }
            else //if bcch_action == SIB_BCCH_SETUP_COMPLETE, or bcch_action == SIB_BCCH_SETUP_PENDING
            {
              ...
              rrcsib_post_process_failed_event(wrrc_as_id,(sib_events_index_e_type)count,FALSE,SIB_EVENT_FAILED_OTHER, TRUE);   
            }
          }
        }
      }
    ...
    case RRCSIB_ABORT_DUE_SIB_READ_IN_DCH_NO_PCCPCH_DROP:
      ...
      for( count = 0; count < (uint32)MAX_SIB_EVENTS_INDEX; count++ )
      {
        if (sib_events_ptr->event[count].event_active == TRUE)
        {
            ...
            (sib_events_ptr->event[count].req_proc == RRC_PROCEDURE_GPS))
          {
             ...
             *of S-BCCH is taken care by SIB in that case rrcsib_force_sib_event_init(sys_modem_as_id_e_type wrrc_as_id) is 
            ...
            if(RRCSIB_ABORT_DUE_SIB_READ_IN_DCH == abort_cause)
            {
              ...
              rrcsib_post_process_failed_event(wrrc_as_id,(sib_events_index_e_type)count,TRUE,SIB_EVENT_FAILED_OTHER, TRUE);
            }
            else
            {
          ...
          else
          {
            if(RRCSIB_ABORT_DUE_TO_NGH_OOS != abort_cause)
            {
              ...
              {
                rrcsib_post_process_failed_event(wrrc_as_id,(sib_events_index_e_type)count,FALSE,SIB_EVENT_FAILED_TIMEOUT, TRUE);
              }
              else
              {
                rrcsib_post_process_failed_event(wrrc_as_id,(sib_events_index_e_type)count,FALSE,SIB_EVENT_FAILED_OTHER, FALSE);
            ...
            {
              if((rrcllc_get_s_bcch_setup_status(wrrc_as_id) == TRUE)||
      ...
      if(((RRCSIB_ABORT_DUE_TO_OOS == abort_cause)||
        ...
        (rrc_get_state(wrrc_as_id) != RRC_STATE_CONNECTING)&&
        (!((rrc_get_state(wrrc_as_id) == RRC_STATE_DISCONNECTED)&& (rrcrce_get_rce_sub_state(wrrc_as_id) != RRCRCE_INITIAL))))
...
SIDE EFFECTS
  ...
  None
===========================================================================*/
static boolean rrcsib_check_if_bands_are_supported(sys_modem_as_id_e_type wrrc_as_id,rrc_MultipleFrequencyBandIndicatorListFDD multi_band_list,
                                                   rrc_plmn_identity_type plmn,boolean for_serving_cell)
{
  ...
  for(i=0;i<multi_band_list.n;i++)
  {
    if(multi_band_list.elem[i].t == T_rrc_FrequencyBandsIndicatorFDD_frequencyBandsIndicator1)
    {
      ...
      if((rrcsibproc_ext_data.Fdd1_internal_band[FDD1] != RRC_CSP_BAND_MAX_INDEX) && 
         (rrcsib_check_if_band_supported(wrrc_as_id,rrcnv_data.rrc_nv_band_priority_config.band_priority_list[rrcsibproc_ext_data.Fdd1_internal_band[FDD1]],for_serving_cell,plmn)))
      {
        return FALSE;
      }
      else if((rrcsibproc_ext_data.Fdd1_internal_band[FDD1] != RRC_CSP_BAND_MAX_INDEX) &&
               rrccsp_is_supported_band(wrrc_as_id,rrcnv_data.rrc_nv_band_priority_config.band_priority_list[rrcsibproc_ext_data.Fdd1_internal_band[FDD1]]))
      {
    ...
    else if(multi_band_list.elem[i].t == T_rrc_FrequencyBandsIndicatorFDD_frequencyBandsIndicator2)
    {
      ...
      if((rrcsibproc_ext_data.Fdd2_internal_band[FDD2] != RRC_CSP_BAND_MAX_INDEX) && 
         (rrcsib_check_if_band_supported(wrrc_as_id,rrcnv_data.rrc_nv_band_priority_config.band_priority_list[rrcsibproc_ext_data.Fdd2_internal_band[FDD2]],for_serving_cell,plmn)))
      {
        return FALSE;
      }
      else if((rrcsibproc_ext_data.Fdd2_internal_band[FDD2] != RRC_CSP_BAND_MAX_INDEX) &&
               rrccsp_is_supported_band(wrrc_as_id,rrcnv_data.rrc_nv_band_priority_config.band_priority_list[rrcsibproc_ext_data.Fdd2_internal_band[FDD2]]))
      {
        /* If the band in the MFBI list is supported by the UE but if it is not the current scanned band
           then bar the UARFCN. */
        return TRUE;
...
===========================================================================*/
static boolean rrcsib_check_multibands_supported_from_sib6(sys_modem_as_id_e_type wrrc_as_id,rrc_SysInfoType6 *sib6_ptr,rrc_plmn_identity_type plmn,boolean for_serving_cell)
{
  ...
  if( (rrcnv_data.rrc_mfbi_support_nv) && (sib6_ptr != NULL) && 
       ...
       .v770NonCriticalExtensions.va40NonCriticalExtensions.va80NonCriticalExtensions.sysInfoType6_va80ext.m.multipleFrequencyBandIndicatorListFDDPresent))
  {
    ...
    return rrcsib_check_if_bands_are_supported(wrrc_as_id,rrc_va80_6ext->multipleFrequencyBandIndicatorListFDD,plmn,for_serving_cell);
...
===========================================================================*/
static boolean rrcsib_check_multibands_supported_from_sib5(sys_modem_as_id_e_type wrrc_as_id,rrc_SysInfoType5 *sib5_ptr,rrc_plmn_identity_type plmn,boolean for_serving_cell)
{
  ...
  if((rrcnv_data.rrc_mfbi_support_nv) && (sib5_ptr != NULL) && 
                ...
                va80NonCriticalExtensions.sysInfoType5_va80ext.m.multipleFrequencyBandIndicatorListFDDPresent))
  {
    ...
    return rrcsib_check_if_bands_are_supported(wrrc_as_id,rrc_va80_5ext->multipleFrequencyBandIndicatorListFDD,plmn,for_serving_cell);
  }
  return TRUE;
}
/*===========================================================================
...
void rrcsib_check_for_redirection_bands_support(sys_modem_as_id_e_type wrrc_as_id,boolean *supportoffrequencyband)
{
  /* Local variables */ 
  rrc_SysInfoType5 *sib5_ptr = NULL;
  uint32 i;
    ...
    if(rrc_va40_ext->frequencyBandsIndicatorSupport.modeSpecificInfo.t == T_rrc_FrequencyBandsIndicatorSupport_modeSpecificInfo_fdd)
    {
        for(i=0;i<rrc_va40_ext->frequencyBandsIndicatorSupport.modeSpecificInfo.u.fdd->frequencyBandsSupportFDD.n;i++)
      {
        if(rrc_va40_ext->frequencyBandsIndicatorSupport.modeSpecificInfo.u.fdd->frequencyBandsSupportFDD.elem[i].t == T_rrc_FrequencyBandsIndicatorFDD_frequencyBandsIndicator1)
        {
          ...
          if(rrccsp_is_supported_band(wrrc_as_id,rrcnv_data.rrc_nv_band_priority_config.band_priority_list[rrcsibproc_ext_data.Fdd1_internal_band[FDD1]]))
          {
        ...
        else if(rrc_va40_ext->frequencyBandsIndicatorSupport.modeSpecificInfo.u.fdd->frequencyBandsSupportFDD.elem[i].t == T_rrc_FrequencyBandsIndicatorFDD_frequencyBandsIndicator2)
        {
          ...
          if(rrccsp_is_supported_band(wrrc_as_id,rrcnv_data.rrc_nv_band_priority_config.band_priority_list[rrcsibproc_ext_data.Fdd2_internal_band[FDD2]]))
          {
            WRRC_MSG0_HIGH("band is supported");
            supportoffrequencyband[i] = TRUE;
          }
...
boolean rrcsib_check_band_mismatch_from_sib5_sib6(sys_modem_as_id_e_type wrrc_as_id,uint32 index)
{
  ...
  if(for_serving_cell)
  {
    band_class = rrc_get_frequency_band(wrrc_as_id,
                            rrccsp_get_serving_frequency(wrrc_as_id), rrccsp_get_curr_nas_requested_plmn(wrrc_as_id));
  }
  else
  {
    band_class = rrc_get_frequency_band(wrrc_as_id,
                            rrccsp_get_neighbor_frequency(wrrc_as_id), rrccsp_get_curr_nas_requested_plmn(wrrc_as_id));
  }
  ...
  mib_ptr = (rrc_MasterInformationBlock *)rrc_sibdb_return_sib_for_db_index
                                     (wrrc_as_id, (rrc_SIB_type)rrc_MIB,
                                       dbindex
                                     );
  ...
  {
    /* Get PLMN */
    rrc_translate_gsm_map_plmn_id(wrrc_as_id,&(mib_ptr->plmn_Type.u.gsm_MAP->plmn_Identity),&plmn);
  }
  ...
  if((rrc_state != RRC_STATE_CONNECTING) && (rrc_state != RRC_STATE_DISCONNECTED))
  {
    if(TRUE == sib6_read)
    {
      sib6_ptr = (rrc_SysInfoType6 *)rrc_sibdb_return_sib_for_db_index
                                     (wrrc_as_id, (rrc_SIB_type)rrc_SIB6,
                                       dbindex
                                     );
      if((sib6_ptr != NULL) && 
       ...
       )
      {
        ...
        {
          if(rrcsib_check_if_band_supported(wrrc_as_id,RRC_CSP_WCDMA_800_BAND,for_serving_cell,plmn) || 
             rrcsib_check_if_band_supported(wrrc_as_id,RRC_CSP_WCDMA_BC19_BAND,for_serving_cell,plmn))
          {
            return FALSE;
          }
          else
          {
            if(rrccsp_is_supported_band(wrrc_as_id, RRC_CSP_WCDMA_800_BAND))
            {
            ...
            return rrcsib_check_multibands_supported_from_sib6(wrrc_as_id,sib6_ptr,plmn,for_serving_cell);
          }
        }
        ...
        .v770NonCriticalExtensions.va40NonCriticalExtensions.va80NonCriticalExtensions.sysInfoType6_va80ext.m.multipleFrequencyBandIndicatorListFDDPresent))
      {
        ...
        {
          if(rrccsp_is_supported_band(wrrc_as_id,band_class))
          {
            WRRC_MSG1_MED("No band mismatch, Allow camping on band :%d",band_class);
            return FALSE;
          }
          else
          {
  ...
  sib5_ptr = (rrc_SysInfoType5 *)rrc_sibdb_return_sib_for_db_index
                                             (wrrc_as_id, (rrc_SIB_type)rrc_SIB5,
                                               dbindex
                                             );
   ...
   )
  {
          ...
          rrc_RadioFrequencyBandFDD_bandVI)
    {
      ...
      else
      {
        ...
        else if(rrcsib_check_if_band_supported(wrrc_as_id, RRC_CSP_WCDMA_BC19_BAND,for_serving_cell,plmn))
        {
          /*
        ...
        WRRC_MSG0_HIGH("The band in FBI is not supported.Look into the MFBI list.");
        /*The band in FBI is not supported by UE,look into the MFBI list*/
        return rrcsib_check_multibands_supported_from_sib5(wrrc_as_id,sib5_ptr,plmn,for_serving_cell);
      }
    }
    ...
    if(TRUE == sib6_read)
    {
       ...
       )
      {
                     ...
                     rrc_RadioFrequencyBandFDD_bandVI)
        {
          if(!rrcsib_check_if_band_supported(wrrc_as_id,RRC_CSP_WCDMA_800_BAND,for_serving_cell,plmn))  
          {
            if(rrcsib_check_if_band_supported(wrrc_as_id,RRC_CSP_WCDMA_BC19_BAND,for_serving_cell,plmn))
            {
              /*
                As per 36.800, (R4-091781), Clause  5.5
                However, in order to support the existing legacy UE which has the capability for only Band VI, 
        ...
        {
          if(!rrcsib_check_if_band_supported(wrrc_as_id,RRC_CSP_WCDMA_850_BAND,for_serving_cell,plmn))  
          {
        ...
        {
          if(!rrcsib_check_if_band_supported(wrrc_as_id,RRC_CSP_WCDMA_1800_BAND,for_serving_cell,plmn))  
          {
        ...
        {
          if(!rrcsib_check_if_band_supported(wrrc_as_id,RRC_CSP_PCS_BAND,for_serving_cell,plmn))  
          {
        ...
        {
          if(!rrcsib_check_if_band_supported(wrrc_as_id,RRC_CSP_IMT2K_BAND,for_serving_cell,plmn))  
          {
        ...
        {
          if(!rrcsib_check_if_band_supported(wrrc_as_id,RRC_CSP_WCDMA_1700_2100_BAND,for_serving_cell,plmn))  
          {
                  ...
                  rrc_RadioFrequencyBandFDD_extension_indicator)
        {
           ...
           * by the IE "Frequency band indicator 2" (i.e., Band VIII or beyond), bar the frequency */
           if(!rrccsp_is_supported_band(wrrc_as_id,RRC_CSP_WCDMA_900_BAND) && 
              !rrccsp_is_supported_band(wrrc_as_id,RRC_CSP_WCDMA_1700_BAND) &&
              !rrccsp_is_supported_band(wrrc_as_id,RRC_CSP_WCDMA_1500_BAND) &&
              !rrccsp_is_supported_band(wrrc_as_id,RRC_CSP_WCDMA_BC19_BAND))
           {
        ...
        ) 
      {
        ...
        {
          if(!rrcsib_check_if_band_supported(wrrc_as_id,RRC_CSP_WCDMA_900_BAND,for_serving_cell,plmn))  
          {
        ...
        {
          if(!rrcsib_check_if_band_supported(wrrc_as_id,RRC_CSP_WCDMA_1700_BAND,for_serving_cell,plmn))  
          {
        ...
        {
          if(!rrcsib_check_if_band_supported(wrrc_as_id,RRC_CSP_WCDMA_1500_BAND,for_serving_cell,plmn))  
          {
...
)
{
  ...
  {
    case rrc_MIB:
      ptr = (void *)rrcsibproc_int_data.rrcsib_bplmn_sibs.mib; 
      break;

    case rrc_SIB1:
      ptr = (void *)rrcsibproc_int_data.rrcsib_bplmn_sibs.sib1; 
      break;

    case rrc_SIB3:
      ptr = (void *)rrcsibproc_int_data.rrcsib_bplmn_sibs.sib3; 
      break;

    default:
      WRRC_MSG1_ERROR("Invalid SIB query for sib_type %d",rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,req_sib));
      break;
  }
...
)
{
  if( rrcsibproc_int_data.rrcsib_bplmn_sibs.mib != NULL )
  {
    (void)rrcasn1_free_pdu(wrrc_as_id,rrc_MasterInformationBlock_PDU, rrcsibproc_int_data.rrcsib_bplmn_sibs.mib );
  }

  if( rrcsibproc_int_data.rrcsib_bplmn_sibs.sib1 != NULL )
  {
    (void)rrcasn1_free_pdu(wrrc_as_id, rrc_SysInfoType1_PDU, rrcsibproc_int_data.rrcsib_bplmn_sibs.sib1 );
  }

  if( rrcsibproc_int_data.rrcsib_bplmn_sibs.sib3 != NULL )
  {
    (void)rrcasn1_free_pdu(wrrc_as_id, rrc_SysInfoType3_PDU, rrcsibproc_int_data.rrcsib_bplmn_sibs.sib3 );
  }

#ifdef FEATURE_RRC_BAND_TYPE_DETECTION_FOR_BPLMN
  if( rrcsibproc_int_data.rrcsib_bplmn_sibs.sb1!= NULL )
  {
    (void)rrcasn1_free_pdu(wrrc_as_id, rrc_SysInfoTypeSB1_PDU, rrcsibproc_int_data.rrcsib_bplmn_sibs.sb1);
  }
  if( rrcsibproc_int_data.rrcsib_bplmn_sibs.sb2 != NULL )
  {
    (void)rrcasn1_free_pdu(wrrc_as_id, rrc_SysInfoTypeSB2_PDU, rrcsibproc_int_data.rrcsib_bplmn_sibs.sb2);
  }
#endif
  rrcsibproc_int_data.rrcsib_bplmn_sibs.mib = NULL;
  rrcsibproc_int_data.rrcsib_bplmn_sibs.sib1 = NULL;
  rrcsibproc_int_data.rrcsib_bplmn_sibs.sib3 = NULL;
#ifdef FEATURE_RRC_BAND_TYPE_DETECTION_FOR_BPLMN
  rrcsibproc_int_data.rrcsib_bplmn_sibs.sb1 = NULL;
...
)
{
  ...
  memset(rrcsibproc_int_data.sib_sleep_schedule_table_0,0xFF,RRCSIB_SIB_SLEEP_SCHEDULE_TABLE_SIZE);
  memset(rrcsibproc_int_data.sib_sleep_schedule_table_1,0xFF,RRCSIB_SIB_SLEEP_SCHEDULE_TABLE_SIZE);
  rrcsibproc_int_data.sib_sleep_schedule_table_in_use = 0;
  WRRC_MSG0_MED("SIB_Sleep: Initialized SIB Sleep Schedule Tables to all 1s");
}
...
===========================================================================*/
void rrcsib_set_bit_pos_before_sb_pos(sys_modem_as_id_e_type wrrc_as_id,uint32 num_bits_req, uint16 array_idx, uint8 *table_ptr)
{
  uint8 max_idx = 0;
  uint8 loop_idx = 0;
...
)
{
  ...
  {
    if(rrcsib_can_event_support_deferred_sib(wrrc_as_id,event_index)== TRUE)
    {
      event_status = check_event_completion_status(wrrc_as_id,event_index,rrcsib_can_nw_support_deferred_sib(wrrc_as_id,sib_events_ptr->event[event_index].sib_db_index));
    }
  }
#ifdef FEATURE_DUAL_SIM
  WRRC_MSG2_MED("ISS: MIB SB flag %d, stop partition flag %d",rrcsibproc_int_data.mib_sb_received,rrcsibproc_int_data.rrcsib_stop_partition);
#endif

  if(rrcsibproc_int_data.sib_sleep_schedule_table_in_use == 0)
  {
    table_ptr = rrcsibproc_int_data.sib_sleep_schedule_table_1;
#ifdef FEATURE_DUAL_SIM
    prev_table_ptr = rrcsibproc_int_data.sib_sleep_schedule_table_0;
#endif
    work_table = 1;
  }
  else
  {
    table_ptr = rrcsibproc_int_data.sib_sleep_schedule_table_0;
#ifdef FEATURE_DUAL_SIM
    prev_table_ptr = rrcsibproc_int_data.sib_sleep_schedule_table_1;
#endif
    work_table = 0;
  }
  ...
  for(count = 1; count < RRC_TOT_SIBS; count++)
  {
      ...
      )
    {
      ...
      seg_count = 0;
      if((WCDMA_RRC_IDLE_FEATURES(RRC_ISS_FOR_SIB_SEGMENTS)) &&
         (sib_events_ptr->event[event_index].tsib[count].sib_tot_length !=0))
      {/*Segments till 'seg_index' are received and stored, so skip processing them*/
        seg_count = sib_events_ptr->event[event_index].tsib[count].seg_index +1; 
      }
      for (; seg_count < sib_events_ptr->sib_sched[count].num_segments; seg_count++)
      {
        ...
        uint8 bit_mask = 0xFF;
        boolean skip_segment = FALSE;  /*Skip the segment if present in tsib_optimization_db*/
        if( (WCDMA_RRC_IDLE_FEATURES(RRC_ISS_FOR_SIB_SEGMENTS)) &&
            (seg_count >0) && (rrcsib_is_sib_seg_in_tsib_optim_db(wrrc_as_id,count) == TRUE))
        {
          for(i = 0;i<MAX_TEMP_SIB_SEGMENTS_TO_HOLD;i++)
          { 
            if((rrcsib_collect_data.tsib_optimization_db[i].index_in_use == TRUE)&&
               (rrcsib_collect_data.tsib_optimization_db[i].sib_type == count) &&
               (rrcsib_collect_data.tsib_optimization_db[i].seg_index == seg_count))
            {
              skip_segment = TRUE; /*Segment Present in tsib_optimization_db*/
              break;
            }
          }
          if(skip_segment)
        ...
        for(loop_count = 0; loop_count < rep_count; loop_count++)
        {
          ...
          if(rep_byte_pos >= RRCSIB_SIB_SLEEP_SCHEDULE_TABLE_SIZE)
          {
            memset(table_ptr,0xFF,RRCSIB_SIB_SLEEP_SCHEDULE_TABLE_SIZE);
            rrcsib_send_sib_sleep_table(wrrc_as_id,table_ptr);
            rrcsibproc_int_data.sib_sleep_schedule_table_in_use = work_table;
            return;
          }
              ...
              (seg_count == (uint32)(sib_events_ptr->sib_sched[count].num_segments-1)))
          {
            ...
            if(bit_offset)
            { 
              rrcsib_set_bit_pos_before_sb_pos(wrrc_as_id,bit_offset, rep_byte_pos, table_ptr);
            }
...
#ifdef FEATURE_DUAL_SIM
    /*This is for the case where GSM is in data call and recieved a SIB(not MIB/SB1). For these cases we need to retain the 
      pattern. So we need to perform AND operation with previous table. This wont be an issue as previous table is super set*/
    if((rrcsibproc_int_data.g_in_data_call)&&(!rrcsibproc_int_data.rrcsib_stop_partition)&&(!rrcsibproc_int_data.mib_sb_received))
    {
      for(count = 0; count < RRCSIB_SIB_SLEEP_SCHEDULE_TABLE_SIZE; count++)
      {
        table_ptr[count] = table_ptr[count]&prev_table_ptr[count];
      }
    }
    /*This is the case where GSM is in data call and MIB/SBs are received. So we need to re-generate the pattern*/
    else if((rrcsibproc_int_data.g_in_data_call)&&(rrcsibproc_int_data.mib_sb_received)&&(!rrcsibproc_int_data.rrcsib_stop_partition))
    {
      rrcsib_intro_holes_in_table(wrrc_as_id,table_ptr);
    }
    /*This is to retain G page poking from previous table which is sent in last 10ms*/
    else if(rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_SIB_POKE_TIMER) != 0)
    {
      rrcsib_modify_table(wrrc_as_id, G_PAGE_HOLES, table_ptr, rrcsibproc_int_data.start_holes_sfn);
    }
  ...
  {
     /*WRRC_MSG2_HIGH("SIB_Sleep: Table Contents byte[%d] = %0X",count,table_ptr[count]);*/
...
{
  rrcsib_initialize_sib_sleep_schedule_table(wrrc_as_id);
  return rrcsibproc_int_data.sib_sleep_schedule_table_0;
}
#endif
/*===========================================================================
...
void rrcsib_update_sixhr_sib_timer(sys_modem_as_id_e_type wrrc_as_id)
{
    ...
    )
  {
    ...
    rrc_state = rrc_get_state(wrrc_as_id);
    sib_active = rrcsib_is_sib_active(wrrc_as_id);
    ...
    */
    if((sib_active == FALSE)&&
        ...
        (rrcrce_is_rce_procedure_active(wrrc_as_id) == FALSE)&&
        (rrcpg1_return_bcch_mod_info_status(wrrc_as_id) == FALSE)&&
#ifdef FEATURE_WCDMA_CSFB_CALL_OPT
         /* Call is imminent so skip posting UPDATE SIBS */
        !(WCDMA_RRC_IDLE_FEATURES(RRC_CSFB_CALL_TIMELINE_OPT) && rrc_csp_int_data.rrc_csfb_call_status)
#endif
      )
    {
...
===========================================================================*/
boolean rrcsib_is_need_to_cleanup_cell(sys_modem_as_id_e_type wrrc_as_id,  sib_events_index_e_type  event_index)
{
  ...
  if((sib_events_ptr->event[event_index].event_name == RRCSIB_GET_SPECIFIC_SIBS) &&
      ...
      (sib_events_ptr->event[event_index].req_proc == RRC_PROCEDURE_CSP))
  {
    WRRC_MSG0_MED("6hr sib:For this event check six hr Criteria if Index found in SIB db");
    return TRUE;
  }
  else
  {
     WRRC_MSG0_MED("6hr sib:No need to check six hr Criteria for this SIB event");
    return FALSE;
  }
}
...
===========================================================================*/
boolean rrcsib_is_event_sixhrs_sib_update(sys_modem_as_id_e_type wrrc_as_id,  sib_events_index_e_type  event_index)
{
  boolean status = FALSE;
...
===========================================================================*/
boolean rrcsib_is_sixhrs_sib_timer_expired(sys_modem_as_id_e_type wrrc_as_id,  sib_events_index_e_type  event_index)
{
  boolean status = FALSE; 
...
===========================================================================*/
boolean rrcsib_is_current_event_due_to_sfn_drift(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8 count;
  boolean status = FALSE;
  for( count = 0; count < (uint32)MAX_SIB_EVENTS_INDEX; count++ )
...
SIDE EFFECTS
  None.
===========================================================================*/
boolean rrcsib_can_sib7_event_start(sys_modem_as_id_e_type wrrc_as_id)
{
    ...
    )
  {
    ...
    oc_set_status = rrcllc_get_ordered_config_status(wrrc_as_id);
    if(!((OC_SET_FOR_CELL_DCH == oc_set_status)||
           ...
           (TRUE == rrccu_check_if_cu_ordered_state_is_pch_and_oc_is_not_set(wrrc_as_id))
          )
      )
    {
      if((rrcsib_is_sib_active(wrrc_as_id) == FALSE)&&
          (sibdb.active_cell_sibs_valid == TRUE)&&
          (RRC_CAMPED_ON == rrc_ccm_get_curr_camping_status(wrrc_as_id, RRC_PROCEDURE_SIB))
        )
      {
...
)
{
  ...
  WRRC_MSG2_HIGH("SIB_Sleep:Updating SIB Sleep Schedule Table for SIB :%d Current table in use:%d",
            rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,rrc_sib_type),rrcsibproc_int_data.sib_sleep_schedule_table_in_use);
  ...
  if(rrc_sib_is_current_event_sib7_in_fach(wrrc_as_id) ==TRUE)
  {
    ...
    for(count = 1; count < RRC_TOT_SIBS; count++)
    {
         ...
         (sib_events_ptr->sib_sched[count].repitition_rate != INVALID_SIB_SCHEDULE_REP)) 
      {
        ...
        start_count = (rrcsib_collect_data.curr_sfn*2)/sib_events_ptr->sib_sched[count].repitition_rate;
    ...
    /* Debug print */
    WRRC_MSG3_HIGH("SIB_Sleep Current table in use:%d table after update by SIB %d :Max repetition rate is %d",
              rrcsibproc_int_data.sib_sleep_schedule_table_in_use, rrcsib_map_int_sibtype_to_asn1_sibtype(wrrc_as_id,rrc_sib_type),sib_events_ptr->scheduling_max_rept_rate);
#ifdef FEATURE_WCDMA_HS_FACH_DRX
    rrcsib_send_sib_sleep_table(wrrc_as_id,rrcsibproc_int_data.sib_sleep_schedule_table_0);
#endif
...
SIDE EFFECTS
  None.
===========================================================================*/
boolean rrc_sib_is_current_event_sib_read_in_dch(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8 count;
  boolean status = FALSE;
  for( count = 0; count < (uint32)MAX_SIB_EVENTS_INDEX; count++ )
...
SIDE EFFECTS
  None.
===========================================================================*/
rrc_proc_e_type rrc_sib_get_proc_id_for_sib_read_in_dch(sys_modem_as_id_e_type wrrc_as_id)
{
  uint8 count;
...
)
{
  ...
  else
  {
    if(rrc_sib_is_current_event_sib7_in_fach(wrrc_as_id) == TRUE)
    {
      ...
      rrcsibproc_int_data.sib7info.sib_ext_proc_update_req = FALSE;
      sib_events_ptr->srv_bcch.bcch_action = SIB_BCCH_NO_ACTION;
      rrcsib_force_sib_event_init(wrrc_as_id, RRCSIB_ABORT_SIB7_UPDATE);
    }
    status = TRUE;
  }
  return status;
...
)
{
  ...
  /*New SIB7 has been received*/
    ...
    decode_succeeded = 
        rrcsib_decode_and_save_sib_for_current_event(wrrc_as_id,(void *)&(sibdb.cell[sibdb.active_cell_index].sibs.sib7),
...
/*Decode of SIB7 successful. Indicate the change in sibdb. Also, update LLC config pointers with SIB7 content */
  if( decode_succeeded == TRUE )
    {
      rrc_sibdb_save_active_cell_sib_change(wrrc_as_id, rrc_SIB7);
      (void)rrcllc_update_configs_with_sib7_info_in_fach(wrrc_as_id);
    }
  }
...
)
{
  ...
  {
    sib3_ptr = rrc_sibdb_return_sib_for_db_index(wrrc_as_id,rrc_SIB3,dbindex);
    if(sib3_ptr!=NULL)
    {
      ...
      {
        WRRC_MSG1_MED("Deferred Measurement reading not supported for dbindex %d",dbindex);
      }
    }
  }
  return status;
...
)
{
  ...
  if( (sib_events_ptr->event[SIB_SRV_INDEX].event_active == TRUE) && 
      (sib_events_ptr->event[SIB_SRV_INDEX].event_name == RRCSIB_UPDATE_SIBS )
    )
  {
    if(sibdb.active_cell_sibs_valid == TRUE)
    {
...
)
{
  ...
  if(rrcsib_is_sib11_11bis_12_read_deferred(wrrc_as_id, DEFERRED_UPDATE_SIB)==TRUE)
  {
    rrc_cmd_type *rrc_cmd_ptr;            /* Pointer to the RRC Command */
    rrc_MasterInformationBlock * mib_ptr;  /*Pointer to MIB*/
    {
  ...
  {
    WRRC_MSG1_MED("Deferred SIB:SIB11 already read.Val tag %d", sibdb.cell[dbindex].sib_value_tag[rrc_SIB11]);
  }
}
/*===========================================================================
...
)
{
  ...
  if(sibdb.active_cell_sibs_valid  == TRUE)
  {
    if(rrcsib_can_nw_support_deferred_sib(wrrc_as_id,dbindex) == TRUE)
    {
      switch(deferred_sib_type) 
      {
        case DEFERRED_SIB11:
          ...
          break;
        case DEFERRED_SIB12:
          if((rrcsib_check_if_sib12_present_for_dbindex(wrrc_as_id,dbindex) == TRUE)&&
        ...
        case DEFERRED_ANY_ALLOWED_SIB:
          if((sibdb.cell[dbindex].sib_value_tag[rrc_SIB11] == INVALID_MIB_SB_VALUE_TAG)||
             ((rrcsib_check_if_sib12_present_for_dbindex(wrrc_as_id,dbindex) == TRUE)&&
             (sibdb.cell[dbindex].sib_value_tag[rrc_SIB12]
                  == INVALID_MIB_SB_VALUE_TAG))
            ||((rrcsib_check_if_extension_sib_present(wrrc_as_id,dbindex) == TRUE)&&
                              (sibdb.cell[dbindex].sib_value_tag[rrc_SIB11bis] == INVALID_MIB_SB_VALUE_TAG))
           )
          {
            status = TRUE;
        ...
        case DEFERRED_UPDATE_SIB:
          if((sibdb.cell[dbindex].sib_value_tag[rrc_SIB11] == INVALID_MIB_SB_VALUE_TAG)||
             ...
             |((rrcsib_check_if_extension_sib_present(wrrc_as_id,dbindex) == TRUE)&&
               (sibdb.cell[dbindex].sib_value_tag[rrc_SIB11bis] == INVALID_MIB_SB_VALUE_TAG))
#ifdef FEATURE_WCDMA_TO_LTE
           || ((rrcsib_check_if_sib19_present(wrrc_as_id,dbindex) == TRUE)&&
               (sibdb.cell[dbindex].sib_value_tag[rrc_SIB19] == INVALID_MIB_SB_VALUE_TAG))
#endif
           || ((sibdb.cell[dbindex].sib_value_tag[rrc_SIB18] == INVALID_MIB_SB_VALUE_TAG)  &&
               ((rrcsib_is_sib_scheduled(wrrc_as_id, rrc_SIB18,dbindex, FALSE,NULL))))
            )
          {
            status = TRUE;
          }
  ...
  {
    WRRC_MSG0_MED("Deferred SIB:SIBS not deferred.");
  }
  return status;
}
...
boolean rrcsib_is_dmcr_ie_needed(sys_modem_as_id_e_type wrrc_as_id)
{

  rrc_SysInfoType3 *sib3_ptr = (rrc_SysInfoType3*)rrc_sibdb_return_sib_for_srv_cell(wrrc_as_id,rrc_SIB3);
  if(sib3_ptr == NULL)
  {
    WRRC_MSG0_HIGH("SIB3 is NULL do not add DMCR IE");
  }
  else if(rrcsib_is_sib11_11bis_12_read_deferred(wrrc_as_id,DEFERRED_ANY_ALLOWED_SIB) &&
...
{
  if((rrcsib_is_sib_scheduled(wrrc_as_id, rrc_SIB12,
                                                       dbindex
#ifdef FEATURE_WCDMA_DEFERRED_SIB11_12_READING
                                                       , FALSE
                                                       ,NULL
                                                       ...
                                                       )) == TRUE)
  {
    if( sibdb.cell[dbindex].sib_valid[rrc_SIB11] == TRUE)
    {
      if(rrcsib_check_if_connected_mode_sibs_present(wrrc_as_id,sibdb.cell[dbindex].sibs.sib11_encoded_ptr[0],
          rrc_SIB11) == TRUE )
      {
        return TRUE;
      }
...
void rrcsib_post_process_deferred_sib(sys_modem_as_id_e_type wrrc_as_id,sib_events_index_e_type index,   uint32 dbindex)
{
  if((sib_events_ptr->event[index].tsib[rrc_SIB11].sib_needed)&&
     !(sib_events_ptr->event[index].tsib[rrc_SIB11].sib_decoded))
  {
    ...
    rrcsib_collect_data.deferred_sib_info.dsib.sib11.seg_index = 
        sib_events_ptr->event[index].tsib[rrc_SIB11].seg_index;

    rrcsib_collect_data.deferred_sib_info.dsib.sib11.sib_tot_length = 
    ...
    b_copy( sib_events_ptr->event[index].tsib[rrc_SIB11].sib_data_ptr,  /* Source Pointer */
             ...
             rrcsib_collect_data.deferred_sib_info.dsib.sib11.sib_data_ptr, /* Destination Pointer */
     ...
     )
  {
    if(rrcsib_check_if_sib12_present_for_dbindex(wrrc_as_id,dbindex)==TRUE)
    {
      ...
      rrcsib_collect_data.deferred_sib_info.dsib.sib12.seg_index = 
          sib_events_ptr->event[index].tsib[rrc_SIB12].seg_index;
      
      rrcsib_collect_data.deferred_sib_info.dsib.sib12.sib_tot_length = 
      ...
      b_copy( sib_events_ptr->event[index].tsib[rrc_SIB12].sib_data_ptr,  /* Source Pointer */
                0,    /* Source Offset is always 0 since we copy all the received bits */
               rrcsib_collect_data.deferred_sib_info.dsib.sib12.sib_data_ptr, /* Destination Pointer */
               0,  /* Destination Offset */
               sib_events_ptr->event[index].tsib[rrc_SIB12].sib_tot_length
              );
    }
    ...
    )
  {
    if(rrcsib_check_if_extension_sib_present(wrrc_as_id,dbindex) == TRUE)
    {
      ...
      rrcsib_collect_data.deferred_sib_info.dsib.sib11bis.seg_index = 
          sib_events_ptr->event[index].tsib[rrc_SIB11bis].seg_index;
  
      rrcsib_collect_data.deferred_sib_info.dsib.sib11bis.sib_tot_length = 
      ...
      b_copy( sib_events_ptr->event[index].tsib[rrc_SIB11bis].sib_data_ptr,  /* Source Pointer */
                        0,    /* Source Offset is always 0 since we copy all the received bits */
                rrcsib_collect_data.deferred_sib_info.dsib.sib11bis.sib_data_ptr, /* Destination Pointer */
                0,  /* Destination Offset */
                sib_events_ptr->event[index].tsib[rrc_SIB11bis].sib_tot_length
              );
    }
  ...
  {
    if((rrcsib_is_sib_scheduled(wrrc_as_id, rrc_SIB18,dbindex, FALSE,NULL) == TRUE))
    {
      WRRC_MSG0_HIGH("Deferred SIB: SIB18 not read");
      sibdb.cell[dbindex].sib_value_tag[rrc_SIB18] = INVALID_MIB_SB_VALUE_TAG;

      rrcsib_collect_data.deferred_sib_info.dsib.sib18.seg_index = 
          sib_events_ptr->event[index].tsib[rrc_SIB18].seg_index;
  
      rrcsib_collect_data.deferred_sib_info.dsib.sib18.sib_tot_length = 
      ...
      b_copy( sib_events_ptr->event[index].tsib[rrc_SIB18].sib_data_ptr,  /* Source Pointer */
                        0,    /* Source Offset is always 0 since we copy all the received bits */
                rrcsib_collect_data.deferred_sib_info.dsib.sib18.sib_data_ptr, /* Destination Pointer */
                0,  /* Destination Offset */
                sib_events_ptr->event[index].tsib[rrc_SIB18].sib_tot_length  /* Length of the subsequent segment is fixed */
              );
    }
  }
    ...
    )
  {
    if(rrcsib_check_if_sib19_present(wrrc_as_id,dbindex) == TRUE)
    {
      ...
      rrcsib_collect_data.deferred_sib_info.dsib.sib19.seg_index = 
          sib_events_ptr->event[index].tsib[rrc_SIB19].seg_index;
  
      rrcsib_collect_data.deferred_sib_info.dsib.sib19.sib_tot_length = 
      ...
      b_copy( sib_events_ptr->event[index].tsib[rrc_SIB19].sib_data_ptr,  /* Source Pointer */
                        0,    /* Source Offset is always 0 since we copy all the received bits */
                rrcsib_collect_data.deferred_sib_info.dsib.sib19.sib_data_ptr, /* Destination Pointer */
                0,  /* Destination Offset */
                sib_events_ptr->event[index].tsib[rrc_SIB19].sib_tot_length  /* Length of the subsequent segment is fixed */
              );
    }
  }
  ...
  {
    rrcsib_store_sib_seg_in_deferred_sib_db(wrrc_as_id);
    rrcsib_collect_data.deferred_sib_info.cell_id = sibdb.cell[dbindex].cell_id;
    rrcsib_collect_data.deferred_sib_info.plmn_id = sibdb.cell[dbindex].plmn_id;
  }
...
)
{
  ...
  if( (dbindex = rrc_sibdb_find_cell_index(wrrc_as_id,plmn, cell)) == SIBDB_INVALID_INDEX )
  {
...
{  
 ...
 dl_sdu_len = cmd_ptr->cmd.get_specific_sibs_req.sys_info_cont_ptr->cont_len + 1;
  ...
  WRRC_EVENT_REPORT_PAYLOAD(EVENT_RRC_MESSAGE_RECEIVED, sizeof(sig_msg_event),
                       (void *)&sig_msg_event);
  ...
  if(log_record_ptr != NULL)
  {
    ...
    log_record_ptr->channel_type |= 0x80;

    log_record_ptr->uarfcn = (uint16)cmd_ptr->cmd.get_specific_sibs_req.freq;

    log_record_ptr->psc =(uint16)cmd_ptr->cmd.get_specific_sibs_req.scrambling_code/16;
...
)
{
  ...
  if((rrccsp_check_sib1_validity(wrrc_as_id,&local_sys_info_cont_ptr->sysInfoType1) == FALSE) &&
      (ret_val == TRUE))
  {
...
)
{
  ...
  cell = rrc_translate_cell_id(wrrc_as_id,&(local_sys_info_cont_ptr->sysInfoType3.cellIdentity));
  
  /* Get the SIB Database index for this cell */
  WRRC_MSG3_HIGH("CSFB: Checking SIBDB index CellId %d and MCC %d MNC %d", cell, 
        RRC_CSP_GET_INT_MCC(plmn), 
        RRC_CSP_GET_INT_MNC(plmn));
  
#ifdef FEATURE_FEMTO_CSG
  sib_db_index = rrc_sibdb_create_cell_index(wrrc_as_id,plmn,cell,event_index,SYS_CSG_ID_INVALID,FALSE);
#else
  sib_db_index = rrc_sibdb_create_cell_index(wrrc_as_id,plmn, cell, event_index);
#endif
...
)
{
  ...
  rrc_log_sys_info_container(wrrc_as_id, cmd_ptr);

  /* Process System Information Container */
  sibdb.rrc_decoded_sib_cont = (rrc_System_Information_Container *)rrcasn1_decode_pdu( wrrc_as_id,
    (void *)cmd_ptr->cmd.get_specific_sibs_req.sys_info_cont_ptr->cont_ptr, 
  ...
  if (FALSE == rrcsib_validate_sys_info_container(wrrc_as_id,sibdb.rrc_decoded_sib_cont))
  {
    WRRC_MSG0_ERROR("Not valid SIB Container");    
    cmd_ptr->cmd.get_specific_sibs_req.sib_scan_type = RRC_SIB_NORMAL;
    if(rrcasn1_free_pdu(wrrc_as_id,rrc_System_Information_Container_PDU,(void *)sibdb.rrc_decoded_sib_cont))
    {
      WRRC_ERR0_FATAL("Unable to free the rrc_System_Information_Container_PDU");
    }
  ...
  sib_db_index = rrcsib_get_sibdb_index_for_sys_info_container(wrrc_as_id,sibdb.rrc_decoded_sib_cont, index);
  ...
  if(sib_db_index == SIBDB_INVALID_INDEX)
  {
    ...
    if(rrcasn1_free_pdu(wrrc_as_id,rrc_System_Information_Container_PDU,(void *)sibdb.rrc_decoded_sib_cont))
    {
      WRRC_ERR0_FATAL("Unable to free the decoded PDU");
    }
  ...
  rrcsib_update_sibdb_csfb_sib_cont(wrrc_as_id,sib_db_index,index,sibdb.rrc_decoded_sib_cont);
  ...
  cell = rrc_translate_cell_id(wrrc_as_id,&(sibdb.rrc_decoded_sib_cont->sysInfoType3.cellIdentity));
...
)
{
  ...
  {
    rrc_sibdb_free_sib(wrrc_as_id,rrc_MasterInformationBlock_PDU, sibdb.cell[sib_db_index].sibs.mib );
  }
  ...
  {
    rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType1_PDU, sibdb.cell[sib_db_index].sibs.sib1 );
  }
  ...
  {
    rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType3_PDU, sibdb.cell[sib_db_index].sibs.sib3 );
  }
  ...
  {
    rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType5bis_PDU, sibdb.cell[sib_db_index].sibs.sib5 );
  }
  ...
  {
    rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType7_PDU, sibdb.cell[sib_db_index].sibs.sib7 );
  }
  ...
  if(rrc_dec_sys_info_container->m.sysInfoType11Present) 
  {
    ...
    {
      rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType11_PDU, sibdb.cell[sib_db_index].sibs.sib11);
    }
  ...
  if(rrc_dec_sys_info_container->m.sysInfoType11bisPresent) 
  {
    ...
    {
      rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType11bis_PDU, sibdb.cell[sib_db_index].sibs.sib11bis);
    }
  ...
  if(rrc_dec_sys_info_container->m.sysInfoType12Present) 
  {
    ...
    {
      rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoType12_PDU, sibdb.cell[sib_db_index].sibs.sib12);
    }
  ...
  if(rrc_dec_sys_info_container->m.sysInfoTypeSB1Present) 
  {
    ...
    {
      rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoTypeSB1_PDU, sibdb.cell[sib_db_index].sibs.sb1);
    }
  ...
  if(rrc_dec_sys_info_container->m.sysInfoTypeSB2Present) 
  {
    ...
    {
      rrc_sibdb_free_sib(wrrc_as_id,rrc_SysInfoTypeSB2_PDU, sibdb.cell[sib_db_index].sibs.sb2);
    }
...
)
{
  ...
  switch(sib_Type)
  {
    ...
    case rrc_SIB_Type_extensionType:
      sib_int_type = rrcsib_map_extension_sib_to_internal_sibtype(wrrc_as_id,seg_index,sfn_prime);
...
===========================================================================*/
void rrcsib_send_sib7_ind(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  if( (cnf_ptr = rrc_get_int_cmd_buf(wrrc_as_id))!=NULL )
  {
    ...
    cnf_ptr->cmd.sib7_update_status.procedure = rrcsibproc_int_data.sib7info.req_proc;
    rrc_put_int_cmd(wrrc_as_id, cnf_ptr );
  }
}
/*===========================================================================
...
boolean rrcsib_update_sibs_active_for_bcch_mod(sys_modem_as_id_e_type wrrc_as_id)
{
   if(rrcsib_is_event_update_sibs_active(wrrc_as_id)
...
/*===========================================================================
FUNCTION RRCSIB_SKIP_SIBS_FOR_NON_NAS_REQ_PLMN

DESCRIPTION

  1> During Cell selection scan due to specific-PLMN MM service req or
  2> During OOS(Connected/Connecting/Idle).
   if BCCH PLMN Differs from (NAS requested) or (RLF) PLMN or their EQPLMNs and If MIB,SIB1,SIB3,SIB4 are read then
   Skip SIB Reading and send Success_cnf to CSP for indicating PLMN to NAS after cell evaluation.
   CSP will inturn does cell evaluation based on SIB3/SIB4 and indicates the PLMN to NAS if suitable.
...
void rrcsib_skip_sibs_for_non_nas_req_plmn
(
  sys_modem_as_id_e_type wrrc_as_id, 
  sib_events_index_e_type index /* Index of the event in the sib event database */
)
{
  ...
  rrc_plmn_identity_type plmn;
  if(
     ((rrccsp_get_curr_scan_no_f3(wrrc_as_id) == RRC_CSP_CELL_SELECTION_SCAN) ||
      (rrccsp_get_curr_scan_no_f3(wrrc_as_id) == RRC_CSP_OUT_OF_SERVICE_AREA_SCAN)||
      (rrccsp_get_curr_scan_no_f3(wrrc_as_id) == RRC_CSP_OUT_OF_SERVICE_AREA_IN_IDLE_STATE_SCAN)||
      (rrccsp_get_curr_scan_no_f3(wrrc_as_id) == RRC_CSP_OUT_OF_SERVICE_AREA_IN_CONNECTING_STATE_SCAN)||
       RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS()
      )&&
     (sib_events_ptr->event[index].event_name == RRCSIB_GET_SPECIFIC_SIBS) &&
     (sib_events_ptr->event[index].req_proc == RRC_PROCEDURE_CSP)&&
     (sib_events_ptr->event[index].sib_scan_type == RRC_SIB_NORMAL)&&
     (sib_events_ptr->event[index].sib_db_index_found == TRUE)&&        /*If SibDB index is found then we would have read MIB,SIB3*/
     (sib_events_ptr->event[index].tsib[rrc_SIB1].sib_received == TRUE)&&
     ((sibdb.cell[dbindex].sibs.sib3->sib4indicator != TRUE ) ||
      ((sibdb.cell[dbindex].sibs.sib3->sib4indicator == TRUE ) && (sib_events_ptr->event[index].tsib[rrc_SIB4].sib_received == TRUE))
     )&&
     (rrc_get_csp_substate(wrrc_as_id)== RRCCSP_CELL_SEL_WT_SIB)
#ifdef FEATURE_DUAL_SIM
     &&!(rrc_dsds_int_data.csp_waiting_for_wl1_to_release_lock) /*CSP is in a state to handle the Success_cnf */
#endif
    )
    {
      rrc_translate_gsm_map_plmn_id(wrrc_as_id, &(sibdb.cell[dbindex].sibs.mib->plmn_Type.u.gsm_MAP->plmn_Identity),&plmn);
      if(FALSE == rrccsp_check_plmn_match(wrrc_as_id,plmn,sibdb.cell[dbindex].sibs.mib, FALSE))    //Check if the BCCH PLMN matches with the NAS req PLMN
      {
#ifdef FEATURE_WRLF_SYSTEM_SEL  //Check if the BCCH PLMN matches with the RLF PLMN
        if(RRC_IS_CONN_MODE_OOS_SRCH_IN_PROGRESS())
        {
          rrc_plmn_identity_type temp_plmnid = rrc_csp_int_data.curr_req_plmn.plmn_id;
          rrc_csp_int_data.curr_req_plmn.plmn_id = rrc_csp_int_data.rlf_plmn_id;  //Overwrite NAS requested PLMN with RLF PLMN and revert after plmn_match funtion call
          if(rrccsp_check_plmn_match(wrrc_as_id,plmn,sibdb.cell[dbindex].sibs.mib, FALSE))  //If matches continue reading SIBs as usual
          {
            rrc_csp_int_data.curr_req_plmn.plmn_id = temp_plmnid; 
            return; 
          }
          rrc_csp_int_data.curr_req_plmn.plmn_id = temp_plmnid;
        }
#endif
        WRRC_MSG0_HIGH("Skip SIB Reading and send Success_cnf to CSP for indicating PLMN to NAS after evaluation");
        rrc_csp_int_data.clr_sib_db_index = dbindex;   //Store the DB Index to be deleted
        post_process_completed_event(wrrc_as_id, index);
      }
    }
}
...
===========================================================================*/
boolean rrcsib_check_if_sib_g_page_collide(sys_modem_as_id_e_type wrrc_as_id,uint8 gap_duration,uint16 *cur_sfn)
{
  uint16 no_of_sfns,relative_diff;
  uint8 count,seg_index,i;
  ...
  /*If this request is not for page then ignore*/
  if(!wrm_is_next_tuneaway_page_decode(wrrc_as_id))
  {
    return TRUE;
  }
  ...
  *cur_sfn = rrcsib_get_curr_sfn(wrrc_as_id);
  ...
  /*If MIB is not received yet return FALSE*/
  if(sib_events_ptr->event[event_index].tsib[rrc_MIB].sib_received == FALSE)
  {
    return FALSE;
  }
  ...
  for(count = 1; count < RRC_TOT_SIBS; count++)
  {
    if((sib_events_ptr->event[event_index].tsib[count].sib_needed == TRUE) &&
       ...
       /*Allow all page decodes except for ones which collide with SIB of max rep rate greater than 64*/
       (sib_events_ptr->scheduling_max_rept_rate > 64))
    {
       seg_index = 0;
      if((WCDMA_RRC_IDLE_FEATURES(RRC_ISS_FOR_SIB_SEGMENTS)) &&
         (sib_events_ptr->event[event_index].tsib[count].sib_tot_length !=0))
       {/*Segments till 'seg_index' are received and stored, so skip processing them*/
         seg_index = sib_events_ptr->event[event_index].tsib[count].seg_index +1;
       }
       for(; seg_index < sib_events_ptr->sib_sched[count].num_segments; seg_index++)
       {
         boolean skip_segment = FALSE; /*Skip the segment if present in tsib_optimization_db*/
         if( (WCDMA_RRC_IDLE_FEATURES(RRC_ISS_FOR_SIB_SEGMENTS)) &&
             (seg_index >0) && (rrcsib_is_sib_seg_in_tsib_optim_db(wrrc_as_id,count) == TRUE))
         {
           for(i = 0;i<MAX_TEMP_SIB_SEGMENTS_TO_HOLD;i++)
           { 
             if((rrcsib_collect_data.tsib_optimization_db[i].index_in_use == TRUE)&&
                (rrcsib_collect_data.tsib_optimization_db[i].sib_type == count) &&
                (rrcsib_collect_data.tsib_optimization_db[i].seg_index == seg_index))
             {
               skip_segment = TRUE; /*Segment Present in tsib_optimization_db*/
               break;
             }
         ...
         relative_diff = rrcsib_check_if_next_sfn_expected(wrrc_as_id,count,*cur_sfn,seg_index);
         if(relative_diff < no_of_sfns)
         {
           return TRUE;
         }
...
===========================================================================*/
void rrcsib_introduce_holes_for_g_page(sys_modem_as_id_e_type wrrc_as_id,uint8 duration)
{
  ...
  if( rrcsib_check_if_sib_g_page_collide(wrrc_as_id,duration,&curr_sfn) || 
     (rrctmr_get_remaining_time(wrrc_as_id, RRCTMR_SIB_STOP_PARTITION_TIMER) == 0) || /*Safe check to avoid poking after CM Inversion timer expiry*/
      WRM_STATE_IS_IN_LOCK(WCDMA_TRM_TX, wrrc_as_id)) /*Avoid poking if W is holding Tx lock*/
  {
    return;
  }
  WRRC_MSG1_HIGH("Started poking holes for other SUB page from SFN %d",curr_sfn);

  rrcsibproc_int_data.start_holes_sfn = curr_sfn;

  /*Copy the contents to SIB sleep that is already sent to the one that is being sent now*/
  if(rrcsibproc_int_data.sib_sleep_schedule_table_in_use == 0)
  {
    table_ptr = rrcsibproc_int_data.sib_sleep_schedule_table_1;
    WCDMA_MEMCPY(rrcsibproc_int_data.sib_sleep_schedule_table_1,sizeof(rrcsibproc_int_data.sib_sleep_schedule_table_1),rrcsibproc_int_data.sib_sleep_schedule_table_0,sizeof(rrcsibproc_int_data.sib_sleep_schedule_table_0));
    rrcsibproc_int_data.sib_sleep_schedule_table_in_use = 1;
  }
  else
  {
    table_ptr = rrcsibproc_int_data.sib_sleep_schedule_table_0;
    WCDMA_MEMCPY(rrcsibproc_int_data.sib_sleep_schedule_table_0,sizeof(rrcsibproc_int_data.sib_sleep_schedule_table_0),rrcsibproc_int_data.sib_sleep_schedule_table_1,sizeof(rrcsibproc_int_data.sib_sleep_schedule_table_1));
    rrcsibproc_int_data.sib_sleep_schedule_table_in_use = 0;
  }

  /*If MIB is not received, set all bits before poking*/
  if(sib_events_ptr->event[SIB_SRV_INDEX].event_active == TRUE)
  {
    if(sib_events_ptr->event[SIB_SRV_INDEX].tsib[rrc_MIB].sib_received == FALSE)
    {
      memset(table_ptr, 0xFF,RRCSIB_SIB_SLEEP_SCHEDULE_TABLE_SIZE);
    }
  }
  ...
  {
    if(sib_events_ptr->event[SIB_NGH_INDEX].tsib[rrc_MIB].sib_received == FALSE)
    {
      memset(table_ptr, 0xFF,RRCSIB_SIB_SLEEP_SCHEDULE_TABLE_SIZE);
    }
  }
  ...
  rrcsib_modify_table(wrrc_as_id, duration, table_ptr, curr_sfn);

  /*To remember that a hole is introduced for other sub page*/
  rrctmr_start_timer(wrrc_as_id,RRCTMR_SIB_POKE_TIMER,15);
...
FUNCTION RRC_HANDLE_WAKEUP_CANCEL

DESCRIPTION

  This function is an api to L1 whichsends whether to ignore
  wakeup cancel or handle the wake up cancel
...
===========================================================================*/
boolean rrc_handle_wakeup_cancel(sys_modem_as_id_e_type wrrc_as_id, uint32 reserve_occasion_ms)
{
  sib_events_index_e_type count; /* Counter used in loop */
  ...
  for( count = SIB_SRV_INDEX; count < MAX_SIB_EVENTS_INDEX; count++ )
  {
    if( sib_events_ptr->event[count].event_active == TRUE )
    {
      if(sib_events_ptr->event[count].tsib[rrc_MIB].sib_received == TRUE)
      {
        if(((!sib_events_ptr->event[count].sched_present_for_sibs[rrc_SB1])||
           ((sib_events_ptr->event[count].sched_present_for_sibs[rrc_SB1])&&
            (sib_events_ptr->event[count].tsib[rrc_SB1].sib_received == TRUE)))&&
           ((!sib_events_ptr->event[count].sched_present_for_sibs[rrc_SB2])||
           ((sib_events_ptr->event[count].sched_present_for_sibs[rrc_SB2])&&
            (sib_events_ptr->event[count].tsib[rrc_SB2].sib_received == TRUE))))
        {
          remaining_time = rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_SIB_STOP_PARTITION_TIMER);
          if(remaining_time > 0 && reserve_occasion_ms > (uint32)remaining_time)
          {
            return FALSE;
          }
          else
...
/*===========================================================================
FUNCTION    RRCSIB_GET_CURR_SFN 

DESCRIPTION
  Returns current SFN of cell on which SIB reading is being performed
...
===========================================================================*/
uint16 rrcsib_get_curr_sfn(sys_modem_as_id_e_type wrrc_as_id)
{

  sib_events_index_e_type event_index = MAX_SIB_EVENTS_INDEX;
  uint16 curr_sfn = 0xFFFF;
  
  /* If W is going to be stopped, return here*/
  if(RRCMCM_WAIT_STOP_CNF == rrcmcm_get_substate(wrrc_as_id))
  {
    return curr_sfn;
  }
  ...
  if(event_index == SIB_NGH_INDEX)
  {
    curr_sfn = dl_get_cell_timing((rrc_csp_int_data.new_cell_acq_entry.scr_code/16), 
      rrc_csp_int_data.new_cell_acq_entry.freq,wrrc_as_id);
  }
  else
  {
    curr_sfn = dl_get_cell_timing((rrc_csp_int_data.curr_acq_entry.scr_code/16), 
      rrc_csp_int_data.curr_acq_entry.freq,wrrc_as_id);
  }
...
/*===========================================================================
FUNCTION    RRCSIB_MODIFY_TABLE

DESCRIPTION
  Helper function to introduce holes in table with given duration
...
)
{
  ...
  for(i = curr_sfn; i < curr_sfn + duration/10; i++)
  {
    ...
    if (i%2 == 0) 
    {
      /*This is needed as wl1 can return current SFN as 4095. 
        i values greater than 4095 will lead in buffer overflow*/
      byte_pos = (i % 4096) >> 3;
      byte_offset = (i % 4096) & 0x07;

      mask = ~ (rrcsib_gen_bit_mask_for_schedule_table(wrrc_as_id,byte_offset));
      table_ptr[byte_pos] &= mask;
    }
  }
}
/*===========================================================================
FUNCTION    RRCSIB_RESTART_SIB_WAIT_TIMER

DESCRIPTION
  Restarts SIB wait timer for the event, based on max reprate.
...
void rrcsib_restart_sib_wait_timer(  sys_modem_as_id_e_type wrrc_as_id, sib_events_index_e_type  event_index)
{
  if(sib_events_ptr->event[event_index].event_active == FALSE)
  {
    WRRC_MSG0_HIGH("Sib event not active");
    return;
  }
  if(sib_events_ptr->event[event_index].tsib[rrc_MIB].sib_received)
  {
    rrctmr_start_timer(wrrc_as_id, RRCTMR_SIB_WAIT_TIMER, 
   (unsigned long) (sib_events_ptr->scheduling_max_rept_rate * RRCSIB_BPLMN_NBCCH_SIB_WAIT_FACTOR));
  }
  else
  { /*Probability of SIB wait timer expiry for DMCR in LTA gap before MIB is received is very less*/
    rrctmr_start_timer(wrrc_as_id, RRCTMR_SIB_WAIT_TIMER, RRCSIB_MAX_WAIT_TIME_FOR_UPDATE_SIBS);
  }
  /*Toggling Priority is not needed as SIB wait time has expired already*/
}

#endif
/*===========================================================================

FUNCTION rrcsib_return_update_sibs_event_pending
...
#endif
boolean rrcsib_return_update_sibs_event_pending(sys_modem_as_id_e_type wrrc_as_id)
{
  return rrcsibproc_int_data.update_sibs_event_pending;
}

#ifdef FEATURE_DUAL_WCDMA_SIBDB_SHARING
/*===========================================================================

FUNCTION rrcsib_clear_altsub_sibdb_cell_index
...
===========================================================================*/
void rrcsib_clear_altsub_sibdb_cell_index(sys_modem_as_id_e_type wrrc_as_id,rrc_plmn_identity_type plmn,rrc_int_cell_id_type cell)
{
  rrc_cmd_type *cmd_ptr;
  sys_modem_as_id_e_type alt_sub = WRRC_GET_ALT_SUB_ID(wrrc_as_id);
  cmd_ptr = rrc_get_int_cmd_buf(alt_sub); 

  if(cmd_ptr != NULL)
  {
    WRRC_MSG0_HIGH("Sending RRC_CLEAR_SIBDB_INDEX_IND to alternate sub");
    cmd_ptr->cmd_hdr.cmd_id = RRC_CLEAR_SIBDB_INDEX_IND;
    cmd_ptr->cmd.rrc_clear_sibdb_index.plmn_id = plmn;
    cmd_ptr->cmd.rrc_clear_sibdb_index.cell_id = cell;
    rrc_put_int_cmd(alt_sub, cmd_ptr);
  }
}
#endif
/*===========================================================================

FUNCTION rrcsib_update_sib3_params

DESCRIPTION
  This function checks which state RRC is in and accordingly calls WL1 api
  srchset_update_sib3_paras_for_cand_nbr to update qqaulmin and q_rxlevmin
  paramenters from sib3 or sib4
...
===========================================================================*/
void rrcsib_update_sib3_params
(
sys_modem_as_id_e_type wrrc_as_id,
uint32 dbindex
)
{
  ...
  sib3_ptr = rrc_sibdb_get_sib_ptr_for_cell(wrrc_as_id, dbindex, rrc_SIB3 );
  rrc_state = rrc_get_state(wrrc_as_id);

  if(sib_events_ptr->event[SIB_NGH_INDEX].event_active == TRUE)
  {
    if((rrc_state == RRC_STATE_CELL_FACH) ||(rrc_state == RRC_STATE_CONNECTING))
    {
      return;
    }
    if((sib3_ptr != NULL) && (sib3_ptr->sib4indicator == TRUE) &&
       (rrc_state != RRC_STATE_DISCONNECTED))
    {
      if((sib_events_ptr->event[SIB_NGH_INDEX].tsib[rrc_SIB4].sib_received)&&(sib_events_ptr->event[SIB_NGH_INDEX].tsib[rrc_SIB4].sib_decoded))
      {
        sib4_ptr = rrc_sibdb_get_sib_ptr_for_cell(wrrc_as_id, dbindex, rrc_SIB4 );
        if(sib4_ptr != NULL)
        {
          rxlev_min = (uint16)((sib4_ptr->cellSelectReselectInfo.modeSpecificInfo.u.fdd->q_RxlevMin * 2)+1);
          qual_min = sib4_ptr->cellSelectReselectInfo.modeSpecificInfo.u.fdd->q_QualMin;
          srchset_update_sib3_params_for_cand_nbr(qual_min, rxlev_min, wrrc_as_id);
          WRRC_MSG0_MED("Updating sib3 parameters from latest sib4");
        }
      }
    }
    else if(sib3_ptr != NULL)
    {
      rxlev_min = (uint16)((sib3_ptr->cellSelectReselectInfo.modeSpecificInfo.u.fdd->q_RxlevMin * 2)+1);
      qual_min = sib3_ptr->cellSelectReselectInfo.modeSpecificInfo.u.fdd->q_QualMin;
      srchset_update_sib3_params_for_cand_nbr(qual_min, rxlev_min, wrrc_as_id);
      WRRC_MSG0_MED("Updating sib3 parameters from latest sib3");
    }
  }
}
...
FUNCTION rrcsib_get_nbcch_action

DESCRIPTION
  This function returns the current NBCCH action
...
===========================================================================*/
sib_bcch_action_e_type rrcsib_get_nbcch_action(sys_modem_as_id_e_type wrrc_as_id)
{
  return sib_events_ptr->ngh_bcch.bcch_action;
}
