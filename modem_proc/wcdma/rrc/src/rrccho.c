...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrccho.c#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/14/20   sp      Mini dump phase-2 changes
02/21/18   as      Added code to support LTA during activation time
01/11/18   sas     Added PLMN info in WtoG CCO req.
...
01/13/14   sr      To add as id to a new primitive added as part of w2g cco abort FR
...
11/14/14   sr      Made changes to send dummy service ind and move to idle SIB DB 
                   is not available for the current camped cell after WTOG CCO fail
11/25/14   gv      Removal of code changes under GAN feature
11/24/14   sn      Changes to not release lock for G2W CCO success 
...
#endif
  rrccho_gtow_info_type gtow_cho_info;

  /* RRCCHO Substate */
  rrccho_state_e_type rrccho_substate;

  /* Procedure  statue */
  rrccho_procedure_info_type rrccho_proc_info;

  /* Field to remember the Activation cause */
  rrccho_activation_cause_e_type rrccho_act_cause;
  boolean ps_clsd_after_suspend_req_in_cco;
} rrccho_int_data_type;
...
LOCAL rrccho_int_data_type rrccho_int_data_sim[MAX_RRC_STACKS] = {0};
#define rrccho_int_data rrccho_int_data_sim[WAS_ID_M]
#else
LOCAL rrccho_int_data_type rrccho_int_data = {0};
#endif
...
void rrccho_indicate_ps_close
(
  sys_modem_as_id_e_type wrrc_as_id
)
{
  rrccho_int_data.ps_clsd_after_suspend_req_in_cco = TRUE;
}
...
)
{
  ...
  rrccho_int_data.rrccho_substate = RRCCHO_INITIAL;

  //reset wtog cco flags
  rrccho_int_data.rrccho_proc_info.cco_in_progress = FALSE;
  rrccho_int_data.rrccho_proc_info.cco_send_ind_to_csp = FALSE;
  rrccho_int_data.rrccho_proc_info.act_time_present = FALSE;

  //reset gtow cco flags
  rrccho_int_data.gtow_cho_info.cc_order_in_progress = FALSE;
  rrccho_int_data.gtow_cho_info.failure_reason = RRC_WCDMA_NONE;

  //reset GERAN sysinfo
  rrccho_int_data.rrccho_proc_info.cell_change_order_req.geran_sysinfo_choice = GERAN_INFO_TYPE_NONE;
}
...
static void rrccho_send_wl1_stop_ind(sys_modem_as_id_e_type wrrc_as_id)
{
  rr_interrat_wl1_stop_ind_type rrc_stop_ind;

  rrc_stop_ind.cause = RR_WL1_STOP_IND_W2G_CCO;
  
  rr_send_rrc_message_to_rr_internal(wrrc_as_id, (void *)(&(rrc_stop_ind)), RR_INTERRAT_WL1_STOP_IND,
...
)
{
  ...
  if(rrccho_int_data.rrccho_proc_info.cco_send_ind_to_csp == TRUE)
  {
...
===========================================================================*/
static void rrccho_send_change_mode_req(sys_modem_as_id_e_type wrrc_as_id, rrc_mode_e_type mode )
{
...
{
  if(rrccho_int_data.rrccho_proc_info.cco_in_progress == TRUE)
  {
    return TRUE;
  }
  return FALSE;
...
{
  if(RRCCHO_CELL_UPDATE_COMPLETE_CNF == rrccho_int_data.rrccho_substate)
  {
    return TRUE;
  }
  return FALSE;
...
)
{
  ...
  if( rrccho_int_data.rrccho_substate == RRCCHO_CELL_UPDATE_COMPLETE_CNF )
  {
    rrccho_int_data.rrccho_proc_info.cco_send_ind_to_csp = FALSE;
    return FALSE;
  }
  else
  {
    if(rrccho_int_data.rrccho_proc_info.cco_in_progress == TRUE)
    {
      rrccho_int_data.rrccho_proc_info.cco_send_ind_to_csp = TRUE;
      return TRUE;
    }
    else
    {
...
static void rrccho_send_pcco_cnf(sys_modem_as_id_e_type wrrc_as_id)
{
  /* Cnf command */
  ...
  cc_order_cnf.status = rrccho_int_data.gtow_cho_info.status;
  cc_order_cnf.failure_reason = rrccho_int_data.gtow_cho_info.failure_reason;

  /* Send the RRC_INTERRAT_CC_ORDER_CNF message to RR */
  rr_send_rrc_message_to_rr_internal(wrrc_as_id,(void *)(&(cc_order_cnf)), RRC_INTERRAT_CC_ORDER_CNF,
  ...
  if (rrccho_int_data.gtow_cho_info.status != CCO_SUCCESS) 
  {
    rrctmr_stop_timer(wrrc_as_id, RRCTMR_WRM_LOCK_TIMER);
...
)
{
  ...
  switch(cc_order_cnf.status)
  {
...
#ifdef FEATURE_DUAL_SIM
      mm_cmd_ptr->cmd.rrc_wtog_cco_complete_ind.as_id = rrc_get_as_id(wrrc_as_id);
#endif
    ...
    /* Change mode to Inactive */
      rrccho_send_change_mode_req(wrrc_as_id, RRC_MODE_INACTIVE);
      rrccho_int_data.rrccho_substate = RRCCHO_WAIT_INACTIVE_CNF;
      
      /* Abort Dormancy if CCO proceed to successful processing. */
      rrcdormancy_update_dormancy_for_srns_hho_cco(wrrc_as_id, RRC_PROCEDURE_CHO);
      break;
    ...
    case CCO_FAILURE_BEFORE_ACT_IND:
      ...
      {
        rrccsp_send_cphy_cell_bar_req(wrrc_as_id, RRC_CSP_GSM_CELL_BARRED, 320);
      }

      rrccho_int_data.rrccho_proc_info.failure_cause = T_rrc_InterRAT_ChangeFailureCause_physicalChannelFailure;
      rrccho_send_change_mode_req(wrrc_as_id, RRC_MODE_ACTIVE);
      rrccho_int_data.rrccho_substate = RRCCHO_WAIT_ACTIVE_CNF;
      break;
    ...
    case CCO_FAILURE_AFTER_ACT_IND:
      ...
      rrccho_int_data.rrccho_act_cause = RRCCHO_INTERRAT_CC_ORDER_WTOG_FAILED;
      rrcmcm_update_wcdma_activation_cause(wrrc_as_id, RRCMCM_INTERRAT_CC_ORDER_WTOG_FAILED);
      rrccho_int_data.rrccho_substate = RRCCHO_WAIT_ACTIVATION_RSP;
      break;
...
)
{
    ...
    switch(reject.failure_reason)
    {
      case RR_GSM_CCO_OTHER_REASON:
        rrccho_int_data.rrccho_proc_info.failure_cause =
      ...
      default:
        WRRC_ERR0_FATAL("Invalid failure cause");
        break;
    }
...
)
{
  ...
  {
     /* set transaction id */
    rrccho_int_data.rrccho_proc_info.transaction_id =
      msg_ptr->u.r3.cellChangeOrderFromUTRAN_IEs.rrc_TransactionIdentifier;
    rrccho_int_data.rrccho_proc_info.failure_cause =
        T_rrc_InterRAT_ChangeFailureCause_unspecified;
    rrccho_int_data.rrccho_proc_info.protocol_error = rrc_ProtocolErrorCause_messageNotCompatibleWithReceiverState;
  ...
  if ((rrcllc_get_ordered_config_status(wrrc_as_id) != OC_NOT_SET)
       ...
       || (TRUE == rrcdata_check_if_ota_is_buffered(wrrc_as_id))
  ...
  {
    rrccho_int_data.rrccho_proc_info.transaction_id = 0;
    rrccho_int_data.rrccho_proc_info.failure_cause =
    T_rrc_InterRAT_ChangeFailureCause_protocolError;
    rrccho_int_data.rrccho_proc_info.protocol_error = rrc_ProtocolErrorCause_messageExtensionNotComprehended;
  ...
  /*W PLMN info*/
  rrccho_int_data.rrccho_proc_info.cell_change_order_req.requested_PLMN = 
  ...
  rrccho_int_data.rrccho_proc_info.cell_change_order_req.BCCH_ARFCN.num = (uint16)
  ...
  if( (gsm_band = rr_rrc_get_arfcn_band_internal(wrrc_as_id, rrccho_int_data.rrccho_proc_info.cell_change_order_req.BCCH_ARFCN.num,
                                        frequency_band)) == INVALID_BAND)
  {
    /* set transaction id */
    rrccho_int_data.rrccho_proc_info.transaction_id = msg_ptr->u.r3.cellChangeOrderFromUTRAN_IEs.rrc_TransactionIdentifier;
    rrccho_int_data.rrccho_proc_info.failure_cause = T_rrc_InterRAT_ChangeFailureCause_configurationUnacceptable;
    WRRC_MSG0_ERROR("Unsupported GSM band in CellChangeOrderFromUTRAN Command");
    return(FALSE);
  }

  /* Fill in the band value */
  rrccho_int_data.rrccho_proc_info.cell_change_order_req.BCCH_ARFCN.band = gsm_band;

  rrccho_int_data.rrccho_proc_info.cell_change_order_req.BSIC_BCC = (uint8)
      msg_ptr->u.r3.cellChangeOrderFromUTRAN_IEs.
      interRAT_TargetCellDescription.technologySpecificInfo.u.gsm->bsic.bcc;

  rrccho_int_data.rrccho_proc_info.cell_change_order_req.BSIC_NCC = (uint8)
      msg_ptr->u.r3.cellChangeOrderFromUTRAN_IEs.
  ...
  if(RRC_MSG_COMMON_BITMASK_IE_TYPE2_PTR(msg_ptr->u.r3.cellChangeOrderFromUTRAN_IEs.
       ...
       ncMode))
  {
    rrccho_int_data.rrccho_proc_info.cell_change_order_req.NC_Mode_present = TRUE;
    ...
    {
      case 0:
        rrccho_int_data.rrccho_proc_info.cell_change_order_req.NC_Mode = NC_MODE_0;
        break;
      case 4:
        rrccho_int_data.rrccho_proc_info.cell_change_order_req.NC_Mode = NC_MODE_1;
        break;
      case 2:
        rrccho_int_data.rrccho_proc_info.cell_change_order_req.NC_Mode = NC_MODE_2;
        break;
      default:
  ...
  {
    rrccho_int_data.rrccho_proc_info.cell_change_order_req.NC_Mode_present = FALSE;
  }

  rrccho_int_data.rrccho_proc_info.cell_change_order_req.T309_value = RRC_GET_T309();
     ...
     )
  {
    ...
    rrccho_int_data.rrccho_proc_info.act_time_present = TRUE;
    rrccho_int_data.rrccho_proc_info.activation_time =
            ...
            rrc_CellChangeOrderFromUTRAN_v590ext_IEs,geran_SystemInfoType))
        {
          ...
          if(RRC_STATE_CELL_DCH == rrc_get_state(wrrc_as_id))
          {
            ...
            if(cco_v590ext_ie->geran_SystemInfoType.t == T_rrc_CellChangeOrderFromUTRAN_v590ext_IEs_geran_SystemInfoType_sI)
            {
              ...
              rrccho_int_data.rrccho_proc_info.cell_change_order_req.geran_sysinfo_choice = GERAN_INFO_TYPE_SI;
            }
            else
            {
              ...
              /* Set the SI type in the CCO req ommand */
              rrccho_int_data.rrccho_proc_info.cell_change_order_req.geran_sysinfo_choice = GERAN_INFO_TYPE_PSI;
            }

            /* Extract the SI blocks and populate the cco_req */
            while(local_geran_SI->n > block_count)
            {
              WCDMA_MEMCPY(rrccho_int_data.rrccho_proc_info.cell_change_order_req.geran_SI[block_count].msg_string,
                           sizeof(rrccho_int_data.rrccho_proc_info.cell_change_order_req.geran_SI[block_count].msg_string),
                     local_geran_SI->elem[block_count].data,
                     MIN(local_geran_SI->elem[block_count].numocts,RRC_RR_MAX_SI_BLOCK_LENGTH));

              rrccho_int_data.rrccho_proc_info.cell_change_order_req.geran_SI[block_count].msg_length =
            ...
            rrccho_int_data.rrccho_proc_info.cell_change_order_req.num_blocks = block_count;

            WRRC_MSG2_HIGH("Geran Info populated.Type %d, num_blocks %d",
                      rrccho_int_data.rrccho_proc_info.cell_change_order_req.geran_sysinfo_choice,
                      rrccho_int_data.rrccho_proc_info.cell_change_order_req.num_blocks);
          }
          else
          {
...
===========================================================================*/
static void rrccho_send_cell_change_order_req(sys_modem_as_id_e_type wrrc_as_id)
{

  rrccsp_fill_info_for_rr(wrrc_as_id, &(rrccho_int_data.rrccho_proc_info.cell_change_order_req.network_select_mode));

#ifdef FEATURE_RAT_PRIORITY_LIST
  rrc_csp_get_rat_pri_list_info(wrrc_as_id, &rrccho_int_data.rrccho_proc_info.cell_change_order_req.rat_pri_list_info);
#endif

#ifdef FEATURE_WCDMA_DEDICATED_PRI_INFO
/*copying the dedicated priority info from global structure(rrc_dedicated_priority_info) to rr_interrat_cc_order_req_type*/
  memset(&rrccho_int_data.rrccho_proc_info.cell_change_order_req.dedicated_priority_info,0,sizeof(lte_irat_dedicated_priority_info_s));
  rrcmeas_populate_dedicated_priority_info_for_irat_req(wrrc_as_id, &rrccho_int_data.rrccho_proc_info.cell_change_order_req.dedicated_priority_info);
#endif
  /* Send the RR_INTERRAT_CC_ORDER_REQ message to RR */
  rr_send_rrc_message_to_rr_internal(wrrc_as_id, (void *)(&(rrccho_int_data.rrccho_proc_info.cell_change_order_req)),
...
===========================================================================*/
static void rrccho_send_cell_change_order_abort_req(sys_modem_as_id_e_type wrrc_as_id)
{
  rr_send_rrc_message_to_rr_internal(wrrc_as_id, (void *)(&rrccho_int_data.rrccho_proc_info.cell_change_abort_req),
...
===========================================================================*/
static void rrccho_send_channel_config_req_to_llc(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_cmd_type *cmd_ptr;
...
)
{
  ...
  {
    WRRC_ERR0_FATAL("Can't find RLC ID for UL RB 2");
  }
  ...
  if(rrccho_int_data.rrccho_proc_info.failure_cause ==
       T_rrc_InterRAT_ChangeFailureCause_protocolError)
  {
     msg->message.u.cellChangeOrderFromUTRANFailure.u.r3.
    ...
    msg->message.u.cellChangeOrderFromUTRANFailure.u.r3.cellChangeOrderFromUTRANFailure_r3.
      ...
      rrccho_int_data.rrccho_proc_info.protocol_error;

#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
  if(rrc_qsh_dl_committed == FALSE)
  {
    uplink_ota.ul_ota_type = RRC_OTA_CHO;
    uplink_ota.msg_id = WRRC_UL_DCCH_MESSAGETYPE_CELLCHANGEORDERFROMUTRANFAILURE;
    uplink_ota.ul_ota.ul_ota_parms.failure_cause = rrccho_int_data.rrccho_proc_info.failure_cause;
  }
  else
  {
    rrc_qsh_log_cco_fail_params(wrrc_as_id, rrccho_int_data.rrccho_proc_info.failure_cause);
  }
#endif
  ...
  /*Reports CCO failure to QSH*/
  rrc_qsh_report_failure_ota_msg_event(wrrc_as_id,WRRC_UL_DCCH_MESSAGETYPE_CELLCHANGEORDERFROMUTRANFAILURE);
#endif

  /*log the error*/
    rrc_log_protocol_error(wrrc_as_id, (uint8) (rrc_get_state(wrrc_as_id)),
                            (uint8)RRC_PROCEDURE_CHO,
                            (uint8) (rrcho_map_inter_rat_cause_to_diag_cause(wrrc_as_id, rrccho_int_data.rrccho_proc_info.failure_cause)),
                            (uint8) rrccho_int_data.rrccho_proc_info.protocol_error
                          );

  /* Send the signalling message */
  if(rrcsend_send_ul_sdu(wrrc_as_id, RRC_PROCEDURE_CHO,
                         msg,
                         lc_id,
  ...
  {
    WRRC_ERR0_FATAL("failed to send Cell Change Order from UTRAN Failure");
  }
...
)
{
...
#endif
  rrccho_int_data.rrccho_proc_info.failure_cause = T_rrc_InterRAT_HO_FailureCause_unspecified;
  ...
  {
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
    if(rrc_qsh_events_db.cco_start.status == TRUE)
    {
      QSH_LOG(QSH_CLT_WRRC,QSH_CAT_EVENT,QSH_MSG_TYPE_HIGH,"Sub:%d CCO started",(wrrc_as_id+1));
      rrc_qsh_report_event(wrrc_as_id, WRRC_QSH_EVENT_CCO_START);
    }
#endif
    if(cmd_ptr->cmd.downlink_sdu_ind.dl_sdu_type == rrc_DL_DCCH_Message_PDU)
    {
      ...
      if( message_choice == T_rrc_DL_DCCH_MessageType_cellChangeOrderFromUTRAN )
      {
        ...
        ps_open_status = (rrcscr_get_signalling_connection_sts(wrrc_as_id, RRC_PS_DOMAIN_CN_ID) &&
                          !rrcscrr_is_conn_rel_in_progress(wrrc_as_id, RRC_PS_DOMAIN_CN_ID) &&
                          !rrcdormancy_is_dormancy_active(wrrc_as_id));
        cs_open_status = rrcscr_get_signalling_connection_sts(wrrc_as_id, RRC_CS_DOMAIN_CN_ID);
        ...
        if(rrcrce_is_abort_in_progress(wrrc_as_id) == TRUE)
        {
        ...
        {
          rrccho_int_data.rrccho_proc_info.transaction_id = 0;
          rrccho_int_data.rrccho_proc_info.failure_cause = T_rrc_InterRAT_ChangeFailureCause_protocolError;
          rrccho_int_data.rrccho_proc_info.protocol_error = rrc_ProtocolErrorCause_asn1_ViolationOrEncodingError;
          WRRC_MSG0_ERROR("ASN.1 decoding failed");
        }
        /*lint -save -e613 */
        if((rrcdata_misc_info.rrc_protocol_error_reject == TRUE) || (rrccho_validate_and_update_info_from_cco_from_utran_msg(wrrc_as_id,
           ...
           u.cellChangeOrderFromUTRAN)) == FALSE))
        {
          if(rrc_state == RRC_STATE_CELL_FACH)
          {
            if(RRCCU_NOT_COMPLETED ==
               rrccu_get_cell_update_complete_status(wrrc_as_id,RRC_PROCEDURE_CHO) )
            {
              /* CELL UPDATE is in process of getting a valid C-RNTI. */
              rrccho_int_data.rrccho_substate = RRCCHO_CELL_UPDATE_COMPLETE_CNF;
            }
            else
            {
              rrccho_send_cell_change_order_failure_msg(wrrc_as_id);
            }
          }
        ...
        /*lint -restore */
        else if (rrcmcm_is_dualmode_enabled(wrrc_as_id) == FALSE)
        {
        ...
        {
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
          cco_params.BCCH_ARFCN.band = rrccho_int_data.rrccho_proc_info.cell_change_order_req.BCCH_ARFCN.band;
          cco_params.BCCH_ARFCN.num = rrccho_int_data.rrccho_proc_info.cell_change_order_req.BCCH_ARFCN.num;
          cco_params.BSIC_BCC = rrccho_int_data.rrccho_proc_info.cell_change_order_req.BSIC_BCC;
          cco_params.BSIC_NCC = rrccho_int_data.rrccho_proc_info.cell_change_order_req.BSIC_NCC;
          rrc_qsh_log_cco_params(wrrc_as_id, &cco_params);
#endif
           ...
           * by 60 ms
           */
          if(!rrccho_int_data.rrccho_proc_info.act_time_present)
          {
            ...
            (void) rex_set_timer(&rrccho_int_data.rrccho_temp_timer, 100);

            rrccho_int_data.rrccho_sig = rex_wait(RRCCHO_TEMP_TIMER_SIG);
            MSG_LOW("rrccho_sig %d",rrccho_int_data.rrccho_sig,0,0);
            (void) rex_clr_sigs(rex_self(), RRCCHO_TEMP_TIMER_SIG);
          }
          ...
          if(WTOW_SEARCH_STOP != rrc_csp_bplmn_srch_in_progress(wrrc_as_id))
          {
            ...
            rrc_csp_send_bplmn_suspend_req(wrrc_as_id, RRC_PROCEDURE_CHO, BPLMN_SUSPEND_REASON_OTHER);
          }

          rrccho_send_change_mode_req(wrrc_as_id, RRC_MODE_STANDBY);
          rrccho_int_data.rrccho_substate = RRCCHO_WAIT_STANDBY_CNF;
          rrccho_int_data.rrccho_proc_info.cco_in_progress = TRUE;
          rrccho_int_data.ps_clsd_after_suspend_req_in_cco = FALSE;
        }
      }
...
)
{
  switch(cmd_ptr->cmd_hdr.cmd_id)
  {
    case RRC_DOWNLINK_SDU_IND:
      rrccho_process_cell_change_order_from_utran_msg(wrrc_as_id, cmd_ptr);
      break;

    default:
      WRRC_MSG2_HIGH("Invalid Event: %d in the substate: %d",
                cmd_ptr->cmd_hdr.cmd_id, rrccho_int_data.rrccho_substate);
      break;
  }
...
)
{
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    ...
    case RRC_CHANGE_MODE_CNF:
      ...
      if( ps_open_status && (!rrccho_int_data.ps_clsd_after_suspend_req_in_cco))
      
      {
        /* Initiate Cell Change Order from UTRAN */
        rrccho_send_cell_change_order_req(wrrc_as_id);

        /* Change the procedure state to wait for Cell Change Order confirmation */
        rrccho_int_data.rrccho_substate = RRCCHO_WAIT_CELL_CHANGE_ORDER_CNF;
      }
      else
      {
        rrccho_int_data.ps_clsd_after_suspend_req_in_cco = FALSE;
...
)
{
  if((((uint32)cmd_ptr->cmd_hdr.cmd_id & RRC_RR_CMD_ID_BASE) == RRC_RR_CMD_ID_BASE) )
  {
    switch( (rrc_rr_cmd_e_type)cmd_ptr->cmd_hdr.cmd_id )
    {
      ...
      case RR_INTERRAT_CC_ORDER_CNF:
        ...
        /*lint -e740*/
        rrccho_process_cell_change_order_cnf(wrrc_as_id,((rrc_rr_cmd_type *) cmd_ptr)->cmd.interrat_cc_order_cnf);
        /*lint +e740*/
        break;
      ...
      case RR_INTERRAT_CC_ORDER_REJ:
        ...
        /*lint -e740*/
        rrccho_process_cell_change_order_reject(wrrc_as_id,
      ...
      default:
        /* No other events are normal in this
                  substate */
        WRRC_MSG2_ERROR("Unexpected event %x in state %d",
           (rrc_rr_cmd_e_type)cmd_ptr->cmd_hdr.cmd_id, rrccho_int_data.rrccho_substate);
        break;
    }
  }
  else
  {
    ...
    {
      case RRC_IRAT_ABORT_IND:
        rrccho_send_cell_change_order_abort_req(wrrc_as_id);
        rrccho_int_data.rrccho_substate = RRCCHO_WAIT_CELL_CHANGE_ORDER_ABORT_CNF;
        break;
...
)
{
  switch(cc_order_abort_cnf.abort_status)
  {
    case RRC_RR_SUCCESS:
      ...
      rrccho_int_data.rrccho_proc_info.failure_cause = T_rrc_InterRAT_ChangeFailureCause_unspecified;
      rrccho_send_change_mode_req(wrrc_as_id, RRC_MODE_ACTIVE);
      rrccho_int_data.rrccho_substate = RRCCHO_WAIT_ACTIVE_CNF;
      break;

    default:
      WRRC_ERR1_FATAL("Invalid status in cc_order_cnf command from RR:%d",cc_order_abort_cnf.abort_status);
      break;
  }
}
...
)
{
  ...
  switch( (rrc_rr_cmd_e_type)cmd_ptr->cmd_hdr.cmd_id )
  {
    ...
    case RR_INTERRAT_CC_ORDER_ABORT_CNF:
      ...
      /*lint -e740*/
      rrccho_process_cell_change_order_abort_cnf(wrrc_as_id, ((rrc_rr_cmd_type *) cmd_ptr)->cmd.interrat_cc_order_abort_cnf);
      /*lint +e740*/
      break;
    /* Cell Change Order Reject from GSM */
    /*lint -e408*/
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    ...
    case RRC_CHANGE_MODE_CNF:
      if(rrccho_int_data.rrccho_proc_info.cco_send_ind_to_csp == TRUE)
      {
        ...
        rrccho_send_csp_wcdma_resume_ind(wrrc_as_id);
        rrccho_initialize_internal_data(wrrc_as_id);
      }
      else
      {
        if(cmd_ptr->cmd.change_mode_cnf.status == TRUE)
        {
          rrccho_send_channel_config_req_to_llc(wrrc_as_id);
          rrccho_int_data.rrccho_substate = RRCCHO_WAIT_CHANNEL_CONFIG_CNF;
        }
        else /* change mode failure */
        {
          ...
          {
            if(RRCCU_NOT_COMPLETED == rrccu_get_cell_update_complete_status(wrrc_as_id,RRC_PROCEDURE_CHO) )
            {
              /* CELL UPDATE is in process of getting a valid C-RNTI. */
              rrccho_int_data.rrccho_substate = RRCCHO_CELL_UPDATE_COMPLETE_CNF;
            }
...
)
{
  ...
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    ...
    default:
      ...
      WRRC_MSG2_ERROR("Unexpected event %x in state %d",cmd_ptr->cmd_hdr.cmd_id, rrccho_int_data.rrccho_substate);
      break;
  }
...
===========================================================================*/
static void rrccho_process_wait_activation_rsp_event_hdlr(sys_modem_as_id_e_type wrrc_as_id, rrc_cmd_type *cmd_ptr )
{
  ...
  if((((uint32)cmd_ptr->cmd_hdr.cmd_id & RRC_RR_CMD_ID_BASE) == RRC_RR_CMD_ID_BASE) )
  {
    switch ((rrc_rr_cmd_e_type)cmd_ptr->cmd_hdr.cmd_id)
    {
      case RRC_INTERRAT_CC_ORDER_ABORT_REQ:
        rrccho_int_data.gtow_cho_info.status = CCO_FAILURE_AFTER_ACT_IND;
        rrccho_int_data.gtow_cho_info.failure_reason = RRC_WCDMA_NONE;
        rrccho_send_change_mode_req(wrrc_as_id, RRC_MODE_INACTIVE);
      ...
      default:
        /* Unexpected command */
        WRRC_MSG2_ERROR("Unexpected cmd %x in CHO state %d", (rrc_rr_cmd_e_type)cmd_ptr->cmd_hdr.cmd_id, rrccho_int_data.rrccho_substate);
        break;      
    }
  }
  else
  {
    switch( cmd_ptr->cmd_hdr.cmd_id )
    {
      case RRC_ACTIVATION_RSP:
        if(rrccho_int_data.rrccho_act_cause == RRCCHO_INTERRAT_CC_ORDER_WTOG_FAILED)
        {
          ...
          if(sib3_ptr == NULL)
          {
            ...
            rrc_csp_send_service_ind_dummy(wrrc_as_id);
            rrccho_int_data.rrccho_proc_info.cco_send_ind_to_csp = TRUE;
          }
          else
          {
            /* Send a service ind to MM.MM should not send any RAU/EST_REQ */
            rrc_csp_send_service_ind(wrrc_as_id);
          }
        ...
        else if(rrccho_int_data.rrccho_act_cause == RRCCHO_INTERRAT_CC_ORDER_GTOW)
        {
          ...
          rrccho_int_data.rrccho_substate = RRCCHO_WAIT_CONN_SETUP_IND;
        }
        else
        {
        ...
        rrccho_int_data.rrccho_act_cause = RRCCHO_ACT_CAUSE_NONE;
        break;

      default:
       /* Unexpected command */
       WRRC_MSG2_ERROR("Unexpected cmd %x in CHO state %d", cmd_ptr->cmd_hdr.cmd_id, rrccho_int_data.rrccho_substate);
       break;
    }
  }
...
)
{
  rrc_state_e_type rrc_state;
  rrc_state = rrc_get_state(wrrc_as_id);
  switch (rrccho_int_data.rrccho_substate)
  {
    
    case RRCCHO_INITIAL:
      rrccho_process_cho_initial_substate(wrrc_as_id, cmd_ptr);
      break;
    case RRCCHO_WAIT_STANDBY_CNF:
      rrccho_process_cho_wait_standby_cnf_substate(wrrc_as_id, cmd_ptr);
      break;
    case RRCCHO_WAIT_CELL_CHANGE_ORDER_CNF:
      rrccho_process_cho_wait_cell_change_order_cnf(wrrc_as_id, cmd_ptr);
      break;
    case RRCCHO_WAIT_ACTIVE_CNF:
      rrccho_process_cho_wait_active_cnf(wrrc_as_id, cmd_ptr);
      break;
    case RRCCHO_CELL_UPDATE_COMPLETE_CNF:
      rrccho_process_cho_cell_update_cnf(wrrc_as_id, cmd_ptr);
      break;
    case RRCCHO_WAIT_CHANNEL_CONFIG_CNF:
      rrccho_process_cho_wait_channel_config_cnf(wrrc_as_id, cmd_ptr);
      break;
    case RRCCHO_WAIT_ACTIVATION_RSP:
      rrccho_process_wait_activation_rsp_event_hdlr(wrrc_as_id, cmd_ptr);
      break;
    case RRCCHO_WAIT_CELL_CHANGE_ORDER_ABORT_CNF:
      rrccho_process_cho_wait_cell_change_order_abort_cnf(wrrc_as_id, cmd_ptr);
      break;
    default:
      WRRC_MSG2_HIGH("Invalid substate in %d State: %d",rrc_state,rrccho_int_data.rrccho_substate);
      break;
  }
} /* rrccho_process_cell_fach_state */
...
)
{
  ...
  if(rrccho_int_data.rrccho_proc_info.act_time_present == TRUE)
  {
    *act_time = (uint16)(rrccho_int_data.rrccho_proc_info.activation_time);
    return SUCCESS;
  }
  else
  {
...
{
  return rrccho_int_data.gtow_cho_info.cc_order_in_progress;
}
...
===========================================================================*/
static void rrccho_process_wait_inactive_cnf_event_handler(sys_modem_as_id_e_type wrrc_as_id, rrc_cmd_type *cmd_ptr )
{
  /* Switch on the received command. */
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    case RRC_CHANGE_MODE_CNF:
      ...
      {
        if(rrccho_int_data.gtow_cho_info.cc_order_in_progress)
        {
          ...
          rrccho_send_pcco_cnf(wrrc_as_id);
        }
        if(rrccho_int_data.rrccho_proc_info.cco_in_progress == TRUE)
        {
          rrccho_send_wl1_stop_ind(wrrc_as_id);
        }
        /* Reset state machine */
        rrccho_initialize_internal_data(wrrc_as_id);
      }
    ...
    default:
      /* Unexpected command */
      WRRC_MSG2_ERROR("Unexpected CMD %x in state %d", cmd_ptr->cmd_hdr.cmd_id, rrccho_int_data.rrccho_substate);
      break;
  }
...
===========================================================================*/
static void rrccho_process_wait_conn_setup_ind_event_hdlr(sys_modem_as_id_e_type wrrc_as_id, rrc_cmd_type *cmd_ptr )
{
  /* Switch on the received command. */
  switch( cmd_ptr->cmd_hdr.cmd_id )
  {
    case RRC_CONN_SETUP_IND:
      ...
      if(cmd_ptr->cmd.rrc_conn_setup_ind.status == SUCCESS)
      {
        ...
        /* Stop Timer T3174 */
        rrctmr_stop_timer(wrrc_as_id, RRCTMR_T_3174_TIMER);

        /* Update RRC Mode to Active since the PCCO was successful */
        rrcmcm_update_rrc_mode_to_active(wrrc_as_id);

        /* Send the PCCO CNF to RR indicating Success */
        rrccho_int_data.gtow_cho_info.status = CCO_SUCCESS;
        rrccho_int_data.gtow_cho_info.failure_reason = RRC_WCDMA_NONE;
        rrccho_send_pcco_cnf(wrrc_as_id);
      ...
      else
      {
        ...
        {
          rrccho_int_data.gtow_cho_info.failure_reason = RRC_WCDMA_MAX_RACH_ATTEMPTS;
        }
        else if(cmd_ptr->cmd.rrc_conn_setup_ind.conn_failure_reason == RRC_CONNECTION_REJECT)
        {
          rrccho_int_data.gtow_cho_info.failure_reason = RRC_WCDMA_CONNECTION_REJECT;
        }
        else
        {
          /* Set a generic failure cause */
          rrccho_int_data.gtow_cho_info.failure_reason = RRC_WCDMA_CELL_CHANGE_FAILURE;
        }
...
===========================================================================*/
static void rrccho_process_wait_camp_cnf_event_hdlr(sys_modem_as_id_e_type wrrc_as_id, rrc_cmd_type *cmd_ptr )
{
  mm_cmd_type *mm_cmd_ptr = NULL;
  ...
  if((((uint32)cmd_ptr->cmd_hdr.cmd_id & RRC_RR_CMD_ID_BASE) == RRC_RR_CMD_ID_BASE) )
  {
    ...
    {
      case RRC_INTERRAT_CC_ORDER_ABORT_REQ:
        rrccho_int_data.gtow_cho_info.status = CCO_FAILURE_BEFORE_ACT_IND;
        rrccho_int_data.gtow_cho_info.failure_reason = RRC_WCDMA_NONE;
        rrccho_send_change_mode_req(wrrc_as_id, RRC_MODE_INACTIVE);
        /* go to inactive substate so that we abort WCDMA*/
        rrccho_int_data.rrccho_substate = RRCCHO_WAIT_INACTIVE_CNF;
      ...
      case RRC_CSP_CAMP_CNF:
       if(cmd_ptr->cmd.rrc_csp_camp_cnf.camping_status == SUCCESS)
       {
        ...
        WRRC_MSG0_HIGH("Dispatching out MM cmd RRC_ACTIVATION_IND sent to MM");
          ...
          /* Update the cause for activation indication */
          rrccho_int_data.rrccho_act_cause = RRCCHO_INTERRAT_CC_ORDER_GTOW;
          rrcmcm_update_wcdma_activation_cause(wrrc_as_id, RRCMCM_INTERRAT_CC_ORDER_GTOW);
        ...
        else
        {
          ...
          /* Set PCCO status and failure reason */
          rrccho_int_data.gtow_cho_info.status = CCO_FAILURE_BEFORE_ACT_IND;
          rrccho_int_data.gtow_cho_info.failure_reason = cmd_ptr->cmd.rrc_csp_camp_cnf.failure_cause;
...
===========================================================================*/
static void rrccho_send_camp_req(sys_modem_as_id_e_type wrrc_as_id)
{
  rrc_cmd_type *cmd_ptr;
  ...
  cmd_ptr->cmd.rrc_csp_camp_req.freq = rrccho_int_data.gtow_cho_info.freq;

  /* Fill in the scrambling code */
  cmd_ptr->cmd.rrc_csp_camp_req.scr_code = (uint16)(rrccho_int_data.gtow_cho_info.scr_code * 16);

  /* Fill in the PLMN Id */
  cmd_ptr->cmd.rrc_csp_camp_req.requested_plmn = rrccho_int_data.gtow_cho_info.requested_plmn;

  /* Fill the network select mode */
  cmd_ptr->cmd.rrc_csp_camp_req.network_select_mode = rrccho_int_data.gtow_cho_info.network_select_mode;
...
===========================================================================*/
static void rrccho_process_wait_irat_in_progress_cnf_event_hdlr(sys_modem_as_id_e_type wrrc_as_id, rrc_cmd_type *cmd_ptr )
{
  if((((uint32)cmd_ptr->cmd_hdr.cmd_id & RRC_RR_CMD_ID_BASE) == RRC_RR_CMD_ID_BASE) )
  {
    switch( (rrc_rr_cmd_e_type)cmd_ptr->cmd_hdr.cmd_id )
  ...
  else
  {
    switch( cmd_ptr->cmd_hdr.cmd_id )
    {
      ...
      case RRC_CHANGE_MODE_CNF:
        ...
        rrccho_send_camp_req(wrrc_as_id);
  
        /* Set the procedure state to wait for Acquisition confirmation */
        rrccho_int_data.rrccho_substate = RRCCHO_WAIT_CAMP_CNF;
...
)
{
  ...
  rrccho_int_data.gtow_cho_info.requested_plmn = mmumts_convert_nas_plmn_id_to_rrc_plmn_id(cmd_ptr->cmd.interrat_cc_order_req.requested_PLMN);
  rrccho_int_data.gtow_cho_info.freq = cmd_ptr->cmd.interrat_cc_order_req.UARFCN;
  rrccho_int_data.gtow_cho_info.scr_code = cmd_ptr->cmd.interrat_cc_order_req.scrambling_code;
  rrccho_int_data.gtow_cho_info.network_select_mode = cmd_ptr->cmd.interrat_cc_order_req.network_select_mode;
  rrccho_int_data.gtow_cho_info.t3174 = cmd_ptr->cmd.interrat_cc_order_req.t3174;
  ...
  rrcmeas_update_dedicated_priority_info_from_irat_req(wrrc_as_id, &cmd_ptr->cmd.interrat_cc_order_req.dedicated_priority_info,
                           mmumts_convert_nas_plmn_id_to_rrc_plmn_id(cmd_ptr->cmd.interrat_cc_order_req.requested_PLMN));
#endif
#ifdef FEATURE_RAT_PRIORITY_LIST
  rrc_csp_update_rat_pri_list_info(wrrc_as_id, &cmd_ptr->cmd.interrat_cc_order_req.rat_pri_list_info);
#endif

  WRRC_MSG3_HIGH("PCCO params: UARFCN %d,PSC %d,T3174 %d msec",
                  rrccho_int_data.gtow_cho_info.freq,
                  rrccho_int_data.gtow_cho_info.scr_code,
                  rrccho_int_data.gtow_cho_info.t3174);

  WRRC_MSG1_HIGH("PCCO params contd: Network Select Mode is %d",
              rrccho_int_data.gtow_cho_info.network_select_mode);

  /* Set T3174 to atleast 5 secs */
  if(rrccho_int_data.gtow_cho_info.t3174 < 5000)
  {
    WRRC_MSG0_HIGH("Setting T3174 to 5 secs");
    rrccho_int_data.gtow_cho_info.t3174 = 5000;
  }

#ifdef FEATURE_DUAL_SIM
  rrc_perform_trm_exchange(wrrc_as_id);
  rrc_wrm_populate_bands_for_lock_req(wrrc_as_id, &lock_req_bands, rrccho_int_data.gtow_cho_info.freq);

  /* Acquire lock before starting WCDMA for GTOW CCO */
  if(rrc_acquire_lock_for_IHO_or_CCO(wrrc_as_id, rrccho_int_data.gtow_cho_info.t3174, lock_req_bands, TRM_ACCESS) != RRC_WRM_LOCK_ACQUIRED)
  {
    WRRC_MSG0_HIGH("DSIM: Lock not acquired for CCO.");
    /* Set PCCO status and failure reason */
    rrccho_int_data.gtow_cho_info.status = CCO_FAILURE_BEFORE_ACT_IND;
    rrccho_int_data.gtow_cho_info.failure_reason = RRC_WCDMA_INAVALID_STATE;
  ...
  rrctmr_start_timer(wrrc_as_id, RRCTMR_T_3174_TIMER,rrctmr_get_remaining_time(wrrc_as_id,RRCTMR_WRM_LOCK_TIMER));
  rrctmr_stop_timer(wrrc_as_id, RRCTMR_WRM_LOCK_TIMER);
#else
  /* Start timer T3174.T3174 is in ms */
  rrctmr_start_timer(wrrc_as_id, RRCTMR_T_3174_TIMER,(rrccho_int_data.gtow_cho_info.t3174));
#endif

  /* Request MCM to change the RRC mode Inactive to Interrat in Progress */
  rrccho_send_change_mode_req(wrrc_as_id, RRC_MODE_INTERRAT_IN_PROGRESS);

  /* Change the procedure state to wait for change mode confirmation */
  rrccho_int_data.rrccho_substate = RRCCHO_WAIT_IRAT_IN_PROGRESS_CNF;
...
)
{
  ...
  *gsm_arfcn = rrccho_int_data.rrccho_proc_info.cell_change_order_req.BCCH_ARFCN;
  *gsm_ncc   = rrccho_int_data.rrccho_proc_info.cell_change_order_req.BSIC_NCC;
  *gsm_bcc   = rrccho_int_data.rrccho_proc_info.cell_change_order_req.BSIC_BCC;
} /* rrccho_fill_gsm_cell_info_for_cell_bar */
...
)
{
  ...
  WRRC_MSG2_HIGH_OPT("function rrccho_procedure_event_handler is called in rrc_state = rrc_state_e_type_value%d and rrccho_substate rrccho_state_e_type_value%d", rrc_state, rrccho_int_data.rrccho_substate);
  rrc_crash_debug_save_proc_substates(wrrc_as_id, RRC_PROCEDURE_CHO,rrccho_int_data.rrccho_substate,cmd_ptr->cmd_hdr.cmd_id);

  if(cmd_ptr->cmd_hdr.cmd_id == RRC_STATE_CHANGE_IND)
  {
    if(rrccho_int_data.gtow_cho_info.cc_order_in_progress)
    {
      WRRC_MSG1_HIGH("RRC_STATE_CHANGE_IND when GTOW PCCO in progress,cho_substate %d",rrccho_int_data.rrccho_substate);
      ...
      if(rrccho_int_data.rrccho_substate != RRCCHO_WAIT_INACTIVE_CNF)
      {
        /* Stop Timer T3174 */
        rrctmr_stop_timer(wrrc_as_id, RRCTMR_T_3174_TIMER);

        /* Set PCCO status and failure reason */
        if( (rrccho_int_data.rrccho_substate == RRCCHO_WAIT_IRAT_IN_PROGRESS_CNF)||
            (rrccho_int_data.rrccho_substate == RRCCHO_WAIT_CAMP_CNF) )
        {
          rrccho_int_data.gtow_cho_info.status = CCO_FAILURE_BEFORE_ACT_IND;
        }
    ...
    {
      MSG_MED("RRC State Change to IDLE in %d", rrccho_int_data.rrccho_substate, 0, 0);
    }
    if(rrccho_int_data.rrccho_substate == RRCCHO_WAIT_INACTIVE_CNF)
    {
      if(rrccho_int_data.rrccho_proc_info.cco_in_progress == TRUE)
  ...
  else if(cmd_ptr->cmd_hdr.cmd_id == RRC_T3174_EXPIRED_IND)
  {
    if(rrccho_int_data.gtow_cho_info.cc_order_in_progress)
    {
      WRRC_MSG1_HIGH("RRC_T3174_EXPIRED_IND in CHO sub-state %d", rrccho_int_data.rrccho_substate);
      ...
      if(rrccho_int_data.rrccho_substate != RRCCHO_WAIT_INACTIVE_CNF)
      {
        WRRC_MSG1_HIGH("Stopping WCDMA in CHO sub-state %d", rrccho_int_data.rrccho_substate);
        ...
        rrccho_int_data.gtow_cho_info.failure_reason = RRC_WCDMA_T3174_EXPIRED;
  ...
  else
  {
    switch(rrc_state)
    {
      ...
      case RRC_STATE_CELL_DCH:
        rrccho_process_cell_fach_state(wrrc_as_id, cmd_ptr);
        break;
      ...
      case RRC_STATE_CONNECTING:
        switch(rrccho_int_data.rrccho_substate)
        {
            ...
            switch((rrc_rr_cmd_e_type)cmd_ptr->cmd_hdr.cmd_id)
            {
              ...
              /*lint +e408*/
                ...
                else
                {
                  ...
                  rrccho_int_data.gtow_cho_info.cc_order_in_progress = TRUE;

                  /* Process the GTOW PCCO request */
                  /*lint -e740*/
                  rrccho_process_pcco_to_utran_command(wrrc_as_id, (rrc_rr_cmd_type *)cmd_ptr);
                  /*lint +e740*/
                }
                break;
              ...
              default:
                /* Unexpected event */
                WRRC_MSG3_HIGH("Unexpected cmd %x in CHO: RRC st: %d, CHO st: %d",
                   cmd_ptr->cmd_hdr.cmd_id,rrc_state,rrccho_int_data.rrccho_substate);
                break;
            }
            break;

          case RRCCHO_WAIT_IRAT_IN_PROGRESS_CNF:
            rrccho_process_wait_irat_in_progress_cnf_event_hdlr(wrrc_as_id, cmd_ptr);
            break;

          case RRCCHO_WAIT_CAMP_CNF:
            rrccho_process_wait_camp_cnf_event_hdlr(wrrc_as_id, cmd_ptr);
            break;
          ...
          case RRCCHO_WAIT_CONN_SETUP_IND:
            rrccho_process_wait_conn_setup_ind_event_hdlr(wrrc_as_id, cmd_ptr);
            break;

          case RRCCHO_WAIT_INACTIVE_CNF:
            rrccho_process_wait_inactive_cnf_event_handler(wrrc_as_id, cmd_ptr);
            break;

          default:
            /* Unexpected event */
            WRRC_MSG2_ERROR("Unexpected event to CHO. RRC st: %d, CHO st: %d", rrc_state,rrccho_int_data.rrccho_substate);
            break;
        }
        break;
...
void rrccho_init_procedure(sys_modem_as_id_e_type wrrc_as_id)
{
  ...
  rrcscmgr_register_for_scn(wrrc_as_id, RRC_PROCEDURE_CHO,      /* Procedure name */
  ...
  suspending WCDMA */
  rex_def_timer(&rrccho_int_data.rrccho_temp_timer, rex_self(), RRCCHO_TEMP_TIMER_SIG);
#endif /* FEATURE_RRC_DELAY_CELL_CHANGE_ORDER */
}
...
{
  return (uint8)rrccho_int_data.rrccho_substate;
}
/*===========================================================================
...
{
  return (uint8)rrccho_int_data.gtow_cho_info.status;
}
...
{
  return (uint8)rrccho_int_data.gtow_cho_info.failure_reason;
}
...
{
  qsh_mdump_collect_high(&rrccho_int_data,sizeof(rrccho_int_data));
}
#endif
