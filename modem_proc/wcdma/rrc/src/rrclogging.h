...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrclogging.h#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
03/07/15   sa      Made changes to add the EVENT_WCDMA_RRC_HSPA_RNTI
...
/*Defines to identify type of service/PLMN search requests */
#define RRCLOG_NAS_SERVICE_REQ     1     /*Service request from NAS*/
#define RRCLOG_NAS_PLMN_REQ        2     /*PLMN request from NAS*/
#define RRCLOG_GSM_PLMN_REQ        3     /*PLMN request from GSM*/
#define RRCLOG_LTE_PLMN_REQ        4     /*PLMN request from LTE*/

#define RRCLOG_NUM_PLMNS           40

/*Defines to identify type of service/PLMN search confirms */
#define RRCLOG_NAS_SERVICE_CNF     1     
#define RRCLOG_NAS_PLMN_CNF        2
#define RRCLOG_NAS_SERVICE_IND     3
#define RRCLOG_LTE_PLMN_CNF        4
#define RRCLOG_GSM_PLMN_CNF        5

/*Defines confirmation status for log packet 0x4803- search confirmation*/
#define SERVICE_AVAILABLE          1
#define NO_SERVICE                 2
#define PLMN_SUCCESS               3
#define PLMN_FAIL                  4
...
LOG_RECORD_DEFINE(WCDMA_SRCH_REQ_INFO)
...
typedef PACKED struct PACKED_POST
{
  ...
  /* CSG PLMN MNC */
  uint8 mnc[3];
}wcdma_plmn_id_log_struct_type;

LOG_RECORD_DEFINE(WCDMA_SRCH_CNF_INFO)
  
  /* Version info */
  uint8 version;   

  /* Search request confirm type */
  uint8 cnf_type;

  uint8 service_status;

  /*PLMN on which we are camped*/
  wcdma_plmn_id_log_struct_type selected_plmn;

  /* Number of found PLMNs */
  uint8 num_plmns;

  wcdma_plmn_id_log_struct_type plmns[RRCLOG_NUM_PLMNS];

LOG_RECORD_END

LOG_RECORD_DEFINE(WCDMA_ACQDB_NEW_INFO)
  ...
  wcdma_csg_cell_info_log_struct_type info[10];
...
typedef PACKED struct PACKED_POST
{
  ...
  /*scan type*/
  uint8 curr_scan;
}wcdma_acq_info_struct_type;

LOG_RECORD_DEFINE(WCDMA_BAND_SCAN_INFO)
  ...
  wcdma_acq_info_struct_type info[25];
...
typedef PACKED struct PACKED_POST
{
  ...
  rrclog_hrb_mapping_info_type hrb_mapping_info[MAX_MAPPING_COMBO];
} rrclog_hrb_info_type;

LOG_RECORD_DEFINE(WCDMA_HANGING_RB_INFO)
  uint8 version;
  uint8 num_rb_in_use;
  rrclog_hrb_info_type  rrclog_hrb_info[MAX_HANGING_RB_MAPPING_INFO];
...
typedef PACKED struct PACKED_POST
{
  ...
  rrclog_hrb2_mapping_info_type hrb2_mapping_info[MAX_MAPPING_COMBO];
} rrclog_hrb2_info_type;

LOG_RECORD_DEFINE(WCDMA_HANGING_RB_MAPPING_INFO)
  uint8 version;
  uint8 num_rb_in_use;
  rrclog_hrb2_info_type  rrclog_hrb2_info[MAX_HANGING_RB_MAPPING_INFO];
LOG_RECORD_END
...
LOG_RECORD_DEFINE(WCDMA_CMAC_UL_R99_LOGGING)
  ...
  uint8 num_tfs_entries;
  rrclog_ul_tfs_info_type tfs_info[UE_MAX_TRCH];
...
#define RRCLOG_SIB_SB1   27
#define RRCLOG_SIB_SB2   28
#define RRCLOG_SIB_18    26
#define RRCLOG_EXT_SIB   31
...
typedef PACKED struct {
  uint16 rrc_hspa_rnti_stored_cell_pch_event;
} rrclog_hspa_rnti_stored_cell_pch_event_type;
#endif
typedef PACKED struct {
  uint32 crnti_value;
} rrclog_crnti_event;
