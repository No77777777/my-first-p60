...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/src/rrcllcoc.h#4 $ $DateTime: 2020/04/29 07:36:02 $ $Author: pwbldsvc $
...
02/15/18   sp      Made changes to adaptively calculate defer time for unlock required cases
08/14/17   sp      Changes for LTA timeline optimization
06/03/16   sg      Changes for saving log packet in crashdump
03/14/16   sp      Made changes to update default no grant timer for 10msec TTI as 500msec
02/01/16   sp      Made changes to Avoid overwriting TOC if OOS is triggered twice before reentry to service area
...
10/14/15   sp      Made changes to add call end metrics - FR#30856
08/11/15   as      Made changes to acquire lock with target frequency if QTA is triggered during HHO
06/30/15   sp      Fixed QSH indentation errors
06/22/15   sg      Made changes to fix compilation errors
05/28/15   sa      Made changes to support eAMR for FR25951 Single VoiceSession
04/27/15   sp      Made changes to report data rates to DS based on ETFI table,max channelization codes for EUL
04/15/15   sp      Made changes to report data rates to DS
...
12/15/14   aa      Made changes to exclude the structure to store new CM list from the internal database of config pointer
...
/* For 10 MS, the default value is 50 TTI ms, so it is 50*10 = 500 ms*/
#define  RRCEUL_NO_GRANT_PERIODICITY_DEFAULT_VALUE 500
...
#define RRC_EARLY_CHANNEL_RESETUP_FOR_RESUME_TO_BE_DONE ((dl_is_lta_tl_opts_nv_enabled() == TRUE)\
                                                         &&\
                                                         (curr_scan == RRC_CSP_UNLOCK_BY_SCAN)\
                                                         &&\
                                                         (rrc_get_state(wrrc_as_id) == RRC_STATE_CELL_DCH)\
                                                         &&\
                                                         (proc_id == RRC_PROCEDURE_CSP))
...
typedef struct
{
  ...
  uint32  amr_mode;
  boolean   scr_mode;
  uint32 mode_type; /* contains the codec mode that mvs needs to be configured with*/
...
typedef struct
{
  uint32 curr_sccpch_index;
   /* Global to track last active cell capability */
  rrc_cell_capability_type last_active_cell_capability;

} rrcllc_ext_data_type;
...
typedef struct
{
  ...
  rrc_state_e_type  rrc_ordered_state;  
  
  /* HSDPA + CM is enabled by default */
  uint8 hsdpa_cm_enabled;


#ifdef FEATURE_WCDMA_MIMO
  rrc_mimo_status_enum_type rrc_mimo_status;
  ...
  */
  rrcllc_sib5_change_type is_sib5_change_in_progress;
  
  boolean ul_reestab_for_pch_trans;
  ...
  */
  boolean ul_tfcs_needed;
  boolean dl_tfcs_needed;
  boolean ul_tfcs_rcvd;
  boolean dl_tfcs_rcvd;
  
  boolean rrc_predef_po3_not_needed;
  
  /* Introduced this global to give the dl_amr_mode */
  uint32 rrc_dl_amr_mode;

  rrcllc_prach_sel_info_type *prach_sel_info[UE_MAX_PRACH];

#ifdef FEATURE_WCDMA_MIMO
#ifdef FEATURE_WCDMA_SCPICH_MIMO_MODE_PRESENT
  int8 rrc_mimo_s_cpich_po;
  ...
  rrcllc_cm_status_e_type cm_status_from_ota_msg;

#ifdef FEATURE_WCDMA_HS_FACH
  boolean validate_directed_cell_info;
#endif

}rrcllcoc_int_data_type;

/*===================================================================
                        DATA DECLARATIONS
=====================================================================*/
#ifdef FEATURE_DUAL_WCDMA 
extern rrcllc_rrc_cmd_process_data_type  rrcllc_cmd_under_process_sim[MAX_RRC_STACKS];
extern rrcllcoc_int_data_type rrcllcoc_int_data_sim[MAX_RRC_STACKS];
/* Stores parameters required to setup HSDPA Action */
extern rrc_hsdpa_msg_struct_type hsdpa_msg_params_sim[MAX_RRC_STACKS];
...
extern rrcllc_ext_data_type rrcllc_ext_data_sim[MAX_RRC_STACKS];
...
*********************************************************************/
extern ordered_config_type *ordered_config_ptr_sim[MAX_RRC_STACKS];
extern ordered_config_type *current_config_ptr_sim[MAX_RRC_STACKS];
extern ordered_config_type *reselection_config_ptr_sim[MAX_RRC_STACKS];

extern rrcllc_oc_complete_status_type ordered_config_sim[MAX_RRC_STACKS];
extern rrcllc_config_e_type config_ptr_in_use_sim[MAX_RRC_STACKS];
extern rrcllc_transition_config_type  transition_config_sim[MAX_RRC_STACKS];
...
extern rrcllc_semi_permanent_data_type  rrcllc_semi_permanent_data_sim[MAX_RRC_STACKS];
#define rrcllc_cmd_under_process rrcllc_cmd_under_process_sim[WAS_ID_M]
#define rrcllcoc_int_data rrcllcoc_int_data_sim[WAS_ID_M]
#define hsdpa_msg_params hsdpa_msg_params_sim[WAS_ID_M]
#ifdef FEATURE_WCDMA_HS_FACH
...
#define rrcllc_ext_data rrcllc_ext_data_sim[WAS_ID_M]
#define rrc_est_rabs rrc_est_rabs_sim[WAS_ID_M]
#define ordered_config_ptr ordered_config_ptr_sim[WAS_ID_M]
#define current_config_ptr current_config_ptr_sim[WAS_ID_M]
#define reselection_config_ptr reselection_config_ptr_sim[WAS_ID_M]
#define ordered_config ordered_config_sim[WAS_ID_M]
#define config_ptr_in_use config_ptr_in_use_sim[WAS_ID_M]
#define transition_config transition_config_sim[WAS_ID_M]
#define rrcllc_semi_permanent_data rrcllc_semi_permanent_data_sim[WAS_ID_M]
...
extern rrcllc_ext_data_type rrcllc_ext_data;
...
====================================================================*/
uint32 get_mac_d_hfn_cucnf(sys_modem_as_id_e_type wrrc_as_id,rrc_CellUpdateConfirm_r5_IEs *cu_confirm_ptr);
...
uecomdef_status_e_type  get_mac_d_hfn(sys_modem_as_id_e_type wrrc_as_id,rrc_DL_DPCH_InfoCommon_r4 *dpch_ie_ptr, 
                                      uint32 *mac_d_val);
...
SIDE EFFECTS:
  None.
====================================================================*/
extern  void  rrcllc_set_fach_included(sys_modem_as_id_e_type wrrc_as_id, boolean status);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_pdcp_parms(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
====================================================================*/
void rrcllc_init_ordered_and_current_config_ptrs(sys_modem_as_id_e_type wrrc_as_id);

void  rrcllc_init_config_data(sys_modem_as_id_e_type wrrc_as_id,
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_ordered_config_status_and_data(sys_modem_as_id_e_type wrrc_as_id,
  rrc_state_e_type  rrc_state
);

void rrcllc_init_ordered_config_mac_info(sys_modem_as_id_e_type wrrc_as_id,
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_before_msg_processing (sys_modem_as_id_e_type wrrc_as_id);
...
====================================================================*/
uecomdef_status_e_type rrcllc_copy_sib_to_oc(sys_modem_as_id_e_type wrrc_as_id, rrc_state_e_type next_rrc_state);
...
=============================================================================*/
uecomdef_status_e_type
rrcllc_update_oc_with_tfc_subset_list(sys_modem_as_id_e_type wrrc_as_id, void  *ie_ptr);
...
====================================================================*/
uint32 rrcllc_get_dl_um_ccch_rlc_size(sys_modem_as_id_e_type wrrc_as_id);
...
RETURN VALUE:
  void.
====================================================================*/
void rrcllc_print_trans_chnl_rate(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
  ...
  this TrCH, except for PCH/FACH, where it is the second index.
====================================================================*/
uint32 rrcllc_get_ctch_rlc_size(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
====================================================================*/
boolean rrcllc_get_ctch_enabled_status(sys_modem_as_id_e_type wrrc_as_id);
/*====================================================================
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_get_bmc_schedul_info(sys_modem_as_id_e_type wrrc_as_id, uint16 *schedul_msg_period);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_dl_pch_trch_idx(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type* config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
uint8 rrcllc_get_dl_pch_trch_idx(sys_modem_as_id_e_type wrrc_as_id,
  ordered_config_type  *config_ptr,
...
SIDE EFFECTS:
  None.
====================================================================*/
uint8 rrcllc_get_dl_pch_trch_idx_for_sccpch_with_ctch(sys_modem_as_id_e_type wrrc_as_id,
  ordered_config_type  *config_ptr,
  tr_ch_id_type transportChannelIdentity);
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_clear_dl_pch_trch_idx(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_dl_fach_trch_idx_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
====================================================================*/
uint8 rrcllc_get_dl_fach_trch_idx(sys_modem_as_id_e_type wrrc_as_id,
  ordered_config_type  *config_ptr,
  tr_ch_id_type  trch_id,
  uint8 pch_trch_idx
);
...
SIDE EFFECTS:
  trch_idx = 0xFF -> indicates no DL FACH TrCH Indexes are available.
====================================================================*/
uint8 rrcllc_get_dl_fach_trch_idx_for_sccpch_with_ctch(sys_modem_as_id_e_type wrrc_as_id,
  ordered_config_type  *config_ptr,
  tr_ch_id_type  trch_id,
  uint8 pch_trch_idx,
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_clear_dl_fach_trch_idx(sys_modem_as_id_e_type wrrc_as_id, uint8 trch_idx);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_ul_rach_trch_idx_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_clear_ul_rach_trch_idx(sys_modem_as_id_e_type wrrc_as_id, uint8 trch_idx);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_dl_dch_trch_idx_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
====================================================================*/
uint8 rrcllc_allocate_dl_dch_trch_idx(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_deallocate_dl_dch_trch_idx(sys_modem_as_id_e_type wrrc_as_id, uint8 trch_idx);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_ul_dch_trch_idx_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
====================================================================*/
uint8 rrcllc_allocate_ul_dch_trch_idx(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_deallocate_ul_dch_trch_idx(sys_modem_as_id_e_type wrrc_as_id, uint8 trch_idx);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_rlc_parms(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type  *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void  rrcllc_init_rnti_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type  *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void  rrcllc_init_l1_dl_dpch_chan_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type  *config_ptr);
...
SIDE EFFECTS:
=============================================================================*/
 void  rrcllc_sort_ref_e_tfci_list(sys_modem_as_id_e_type wrrc_as_id,
...
SIDE EFFECTS:
  None
====================================================================*/
void rrc_print_cphy_setup(sys_modem_as_id_e_type wrrc_as_id, l1_setup_cmd_type *l1_cmd_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_mac_e_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_mac_sec_eul_e_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
#endif /* FEATURE_WCDMA_DC_HSUPA */
...
SIDE EFFECTS:
  None.
====================================================================*/
void  rrcllc_init_l1_dl_sec_fdpch_chan_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type  *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_reset_mac_and_l1_e_info_on_llc_cfg_cnf(sys_modem_as_id_e_type wrrc_as_id);
/*====================================================================
...
SIDE EFFECTS:
====================================================================*/
uint8  rrcllc_allocate_e_mac_d_flow_index(sys_modem_as_id_e_type wrrc_as_id);
...
====================================================================*/
uecomdef_logch_mode_e_type rrcllc_get_e_dch_log_chan_mode(sys_modem_as_id_e_type wrrc_as_id, uint32 rb_id);
...
uecomdef_status_e_type rrcllc_verify_contiguous_carriers(sys_modem_as_id_e_type wrrc_as_id, uint32 *carrier_freq, uint8 num_carriers);
#endif
#ifdef FEATURE_WCDMA_DC_HSUPA
...
SIDE EFFECTS:
====================================================================*/
boolean rrcllc_is_cm_active(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type  *config_ptr);
...
SIDE EFFECTS:
====================================================================*/
uint8  rrcllc_allocate_dflow_index(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_hspdsch_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type  *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_hsdpa_mac_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type  *config_ptr);
...
void rrc_print_emac_dflow_info_in_oc(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
  None.
====================================================================*/
void  rrcllc_init_l1_sccpch_chan_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type  *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void  rrcllc_init_l1_sccpch_for_ctch_chan_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type  *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_bch_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_pch_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_fach_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr, boolean init_rb_mapping_info);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_dl_dch_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_dl_cctrch_info_for_sccpch_with_ctch(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_cbs_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_dl_ctfcs_for_sccpch_with_ctch(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_dl_cctrch_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_dl_ctfcs(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_l1_ul_dpch_chan_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type  *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_rach_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr, boolean init_rb_map_info);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_ul_dch_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_check_ul_tfs_cnt(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_check_dl_tfs_cnt(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
  None.
====================================================================*/
void  rrcllc_reset_rach_rlc_size_lists(sys_modem_as_id_e_type wrrc_as_id,
...
SIDE EFFECTS    None
====================================================================*/
void  select_asc_from_ac(sys_modem_as_id_e_type wrrc_as_id,ordered_config_type *config_ptr);
...
SIDE EFFECTS    None
====================================================================*/
void  get_persist_scaling_default_factors(sys_modem_as_id_e_type wrrc_as_id,ordered_config_type *config_ptr);
...
SIDE EFFECTS:
  ...
  data is incremented.
====================================================================*/
uint8 rrcllc_get_dl_dch_trch_idx(sys_modem_as_id_e_type wrrc_as_id, tr_ch_id_type  trch_id);
...
SIDE EFFECTS:
  ...
  data is incremented.
====================================================================*/
uint8 rrcllc_get_ul_dch_trch_idx(sys_modem_as_id_e_type wrrc_as_id, tr_ch_id_type  trch_id);


void  rrcllc_fill_holes_in_dl_dch_trch_idx(sys_modem_as_id_e_type wrrc_as_id);

void rrcllc_copy_dl_dch_trch_info_from_one_to_next_index(sys_modem_as_id_e_type wrrc_as_id,
      uint8 dest_trch_idx,
      uint8 src_trch_idx
);
...
uint8  rrcllc_find_dl_fach_trch_idx(sys_modem_as_id_e_type wrrc_as_id);
uint8  rrcllc_find_dl_dch_trch_idx(sys_modem_as_id_e_type wrrc_as_id, tr_ch_id_type  trch_id);
...
void  rrcllc_fill_holes_in_ul_dch_trch_idx(sys_modem_as_id_e_type wrrc_as_id);
...
void rrcllc_copy_ul_dch_trch_info_from_one_to_next_index(sys_modem_as_id_e_type wrrc_as_id,
      uint8 dest_trch_idx,
      uint8 src_trch_idx
);
...
SIDE EFFECTS:
====================================================================*/
uint8  rrcllc_find_ul_dch_trch_idx(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr,
                                   tr_ch_id_type  trch_id);

uint8  rrcllc_find_ul_rach_trch_idx(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
  None.
====================================================================*/
void  set_dl_rm_restriction_info_to_invalid(sys_modem_as_id_e_type wrrc_as_id,
...
SIDE EFFECTS:
  None.
====================================================================*/
void  rrcllc_init_mac_dl_ptrs(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type  *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void  rrcllc_init_mac_ul_ptrs(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type  *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void  rrcllc_init_l1_ul_ptrs(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type  *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void  rrcllc_init_rlc_ptrs(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type  *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void  rrcllc_init_activation_time(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type  *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_reconfig_needed(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type  *config_ptr);
...
====================================================================*/
rrcllc_oc_status_e_type
rrcllc_update_oc_in_fach_with_msg(sys_modem_as_id_e_type wrrc_as_id,
  uint32            dl_sdu_num,
  void             *msg_ptr
);

void  rrcllc_update_oc_from_fach_to_disconnected(sys_modem_as_id_e_type wrrc_as_id,
  uint32            dl_sdu_num,
  void             *msg_ptr
);
...
====================================================================*/
rrcllc_oc_status_e_type
rrcllc_update_oc_from_dch_to_fach_with_msg(sys_modem_as_id_e_type wrrc_as_id,
  uint32            dl_sdu_num,
  void             *msg_ptr
);
...
====================================================================*/
rrcllc_oc_status_e_type
rrcllc_update_oc_from_connecting_to_dch(sys_modem_as_id_e_type wrrc_as_id,
  uint32            dl_sdu_num,
  void             *msg_ptr
);
...
====================================================================*/
rrcllc_oc_status_e_type
rrcllc_update_oc_from_fach_to_dch(sys_modem_as_id_e_type wrrc_as_id,
  uint32            dl_sdu_num,
  void             *msg_ptr
);
...
====================================================================*/
rrcllc_oc_status_e_type
rrcllc_update_oc_from_cell_pch_to_fach(sys_modem_as_id_e_type wrrc_as_id);
...
====================================================================*/
uecomdef_status_e_type
rrcllc_update_oc_from_dch_to_fach_with_sibs(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_update_oc_in_fach_with_toc(sys_modem_as_id_e_type wrrc_as_id
);
...
SIDE EFFECTS:
  None.
====================================================================*/
boolean rrcllc_check_dcch_fach_mapping(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_update_oc_in_fach_with_toc_for_rl_failure(sys_modem_as_id_e_type wrrc_as_id
);
...
====================================================================*/
rrcllc_oc_status_e_type
rrcllc_update_oc_in_dch(sys_modem_as_id_e_type wrrc_as_id,
  uint32            dl_sdu_num,
  void             *msg_ptr
);
...
===========================================================================*/
uecomdef_status_e_type
rrcllc_chan_config_dch_to_fach(sys_modem_as_id_e_type wrrc_as_id, rrc_cmd_type *cmd_ptr);
...
===========================================================================*/
uecomdef_status_e_type
rrcllc_chan_config_dch_to_pch(sys_modem_as_id_e_type wrrc_as_id, rrc_cmd_type *cmd_ptr);
...
====================================================================*/
rrcllc_oc_status_e_type
rrcllc_update_oc_in_fach(sys_modem_as_id_e_type wrrc_as_id,
  uint32            dl_sdu_num,
  void             *msg_ptr
);
...
====================================================================*/
rrcllc_oc_status_e_type
rrcllc_update_oc_from_connecting_to_fach(sys_modem_as_id_e_type wrrc_as_id,
  uint32            dl_sdu_num,
  void             *msg_ptr
);
...
====================================================================*/
uecomdef_status_e_type
rrcllc_update_oc_from_disconnected_to_connecting(sys_modem_as_id_e_type wrrc_as_id);
...
====================================================================*/
rrcllc_oc_status_e_type
rrcllc_update_oc_from_disconnected_to_dch(sys_modem_as_id_e_type wrrc_as_id,
  uint32            dl_sdu_num,
  void             *msg_ptr
);
...
====================================================================*/
uecomdef_status_e_type
rrcllc_init_oc_cc_in_diconnected(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_copy_ccch_info_from_cc_to_oc(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
=============================================================================*/
void  rrcllc_copy_ul_dch_trch_info_from_buffer(sys_modem_as_id_e_type wrrc_as_id, uint32 trch_idx);
...
SIDE EFFECTS:
=============================================================================*/
void  rrcllc_copy_ul_dch_trch_info_to_buffer(sys_modem_as_id_e_type wrrc_as_id, uint32 trch_idx);
...
SIDE EFFECTS:
=============================================================================*/
void  rrcllc_copy_dl_dch_trch_info_from_buffer(sys_modem_as_id_e_type wrrc_as_id, uint32 trch_idx);
...
SIDE EFFECTS:
=============================================================================*/
void  rrcllc_copy_dl_dch_trch_info_to_buffer(sys_modem_as_id_e_type wrrc_as_id, uint32 trch_idx);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_dch_rlc_size_info(sys_modem_as_id_e_type wrrc_as_id, tr_ch_id_type trch_id);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_fach_rlc_size_info(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
=============================================================================*/
void rrcllc_register_for_reconfig_cnf(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
=============================================================================*/
void rrcllc_discard_oc(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
  None.
====================================================================*/
void  rrcllc_copy_cell_rach_rb_mapping_info(sys_modem_as_id_e_type wrrc_as_id,
  ordered_config_type  *dest_config,
  ordered_config_type  *src_config
  );
...
SIDE EFFECTS:
  None.
====================================================================*/
void  rrcllc_copy_cell_fach_rb_mapping_info(sys_modem_as_id_e_type wrrc_as_id,
  ordered_config_type  *dest_config,
  ordered_config_type  *src_config
  );
...
====================================================================*/
void
rrcllc_get_timers_and_const_in_conn_mode_for_handover(sys_modem_as_id_e_type wrrc_as_id);
...
boolean  rrcllc_is_cmd_under_process_for_solo_bch_operation(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
  None.
====================================================================*/
void  rrcllc_copy_bch_info_from_cc_to_oc(sys_modem_as_id_e_type wrrc_as_id);
...
====================================================================*/
rrcllc_oc_status_e_type
rrcllc_update_oc_from_dch_to_pch_with_msg(sys_modem_as_id_e_type wrrc_as_id,
  uint32            dl_sdu_num,
  void             *msg_ptr,
  rrc_state_e_type  next_rrc_state
);
...
====================================================================*/
rrcllc_oc_status_e_type
rrcllc_update_oc_from_fach_to_pch_with_msg(sys_modem_as_id_e_type wrrc_as_id,
  uint32            dl_sdu_num,
  void             *msg_ptr,
  rrc_state_e_type  next_rrc_state
);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_dl_common_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
boolean rrcllc_timing_init_in_progress(sys_modem_as_id_e_type wrrc_as_id);
...
====================================================================*/
void rrcllc_print_pdcp_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
#endif /* FEATURE_UMTS_PDCP */
...
====================================================================*/
void rrcllc_print_mac_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
====================================================================*/
void rrcllc_print_l1_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_copy_pdcp_parms(sys_modem_as_id_e_type wrrc_as_id,
  ordered_config_type  *dest_config,
  ordered_config_type  *src_config
  );
...
void rrcllc_copy_rlc_parms(sys_modem_as_id_e_type wrrc_as_id,
  ordered_config_type  *dest_config,
  ordered_config_type  *src_config
  );
...
void  rrcllc_copy_cell_fach_rlc_size_list(sys_modem_as_id_e_type wrrc_as_id,
  ordered_config_type  *dest_config,
  ordered_config_type  *src_config
  );
...
===========================================================================*/
uint32 rrcllc_get_dl_amr_mode(sys_modem_as_id_e_type wrrc_as_id, uint32 tb_size);
...
===========================================================================*/
uint32 rrcllc_get_dl_amr_mode_for_rb
(
  sys_modem_as_id_e_type wrrc_as_id,
  rrc_RB_Identity  rb_id, 
  ordered_config_type *config_ptr
...
SIDE EFFECTS
====================================================================*/
void rrcllc_init_srb5_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
boolean rrcllc_compare_rlc_size_change(sys_modem_as_id_e_type wrrc_as_id, uint32 old_size, uint32 new_size);
...
SIDE EFFECTS:
  None.
====================================================================*/
uint16 rrcllc_get_dch_fach_act_time_in_ms(sys_modem_as_id_e_type wrrc_as_id);
/*==========================================================
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_init_tfcs_needed_flags(sys_modem_as_id_e_type wrrc_as_id);
/*====================================================================
...
SIDE EFFECTS:
  None.
====================================================================*/
boolean rrcllc_is_tfcs_provided_on_add_delete_trch(sys_modem_as_id_e_type wrrc_as_id, rrc_RRC_StateIndicator dest_state);
/*==========================================================
...
====================================================================*/
uint8 rrcllc_get_cctrch_id(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_clear_cctrch_id(sys_modem_as_id_e_type wrrc_as_id, uint8 cctrch_id);
...
SIDE EFFECTS: None.
============================================================*/
ordered_config_type *rrcllc_get_config_ptr_in_use(sys_modem_as_id_e_type wrrc_as_id);
...
====================================================================*/
uecomdef_status_e_type
rrcllc_get_amr_mapping(sys_modem_as_id_e_type wrrc_as_id,  rrcllc_amr_mapping_info_type *ptr );
...
SIDE EFFECTS
  None.
===========================================================================*/
void rrc_deconfigure_voice_adapter
(
sys_modem_as_id_e_type wrrc_as_id
);
/*===========================================================================

FUNCTION   rrc_send_select_owner_event

DESCRIPTION

  This function sends WCDMA_IVOICE_EVENT_SELECT_OWNER event unconditionally
...
void rrc_send_select_owner_event(sys_modem_as_id_e_type wrrc_as_id);

/*===========================================================================
FUNCTION rrc_configure_voice_adapter
...
===========================================================================*/
void rrc_configure_voice_adapter
(
  sys_modem_as_id_e_type wrrc_as_id,
  rrcllc_amr_mapping_info_type * amr_info,
  boolean rrc_ul_direction,
  boolean rrc_dl_direction,
  uint32 wrrc_voice_adapter_event
);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_copy_sib_to_oc_for_efach(sys_modem_as_id_e_type wrrc_as_id, rrc_state_e_type next_rrc_state ,
...
====================================================================*/
void rrcllc_print_rlc_info(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type *config_ptr);
...
/*============================================================================
FUNCTION: rrcllcpcie_process_dl_common_info_r10()
...
SIDE EFFECTS:
=============================================================================*/
uecomdef_status_e_type rrcllcpcie_process_dl_common_info_r10
(
  sys_modem_as_id_e_type wrrc_as_id,
  OSS_UINT32 dl_common_info_present,
  rrc_DL_CommonInformation_r10 *dl_common_info_ptr,
  rrc_RRC_StateIndicator rcvd_state_indicator
);

/*====================================================================
FUNCTION        get_dl_info_common_for_all_rl_r10()
...
SIDE EFFECTS
====================================================================*/
uecomdef_status_e_type get_dl_info_common_for_all_rl_r10
(
  sys_modem_as_id_e_type wrrc_as_id,
  /* Pointer to the DL CommonInformation IE */
  rrc_DL_CommonInformation_r10 *ie_ptr,
...
/*============================================================================
FUNCTION: rrcllcpcie_process_rl_addition_info_list_r10()

DESCRIPTION:
  This function updates IE "rrc_RL_AdditionInformationList_r10 " in ordered config
  and update e_dch parameters
...
SIDE EFFECTS:
=============================================================================*/
uecomdef_status_e_type rrcllcpcie_process_rl_addition_info_list_r10
(
  sys_modem_as_id_e_type wrrc_as_id,
  OSS_UINT32 rl_additon_info_list_present,
  rrc_RL_AdditionInformationList_r10*rl_add_ptr
);

/*===========================================================================
FUNCTION        get_rl_add_info_r10()
...
SIDE EFFECTS    The RRC config database as pointed to by the input parameters
                will be updated.
===========================================================================*/
uecomdef_status_e_type get_rl_add_info_r10
(
  sys_modem_as_id_e_type wrrc_as_id,
  rrc_RL_AdditionInformation_r10 *ie_ptr,
  l1_dl_phychan_db_struct_type *dl_phychan,
  boolean *duplicate_add_attempted
);

/*====================================================================
FUNCTION: validate_fdpch_information_r10()
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type validate_fdpch_information_r10
(
  sys_modem_as_id_e_type wrrc_as_id,
  rrc_RL_AdditionInformationList_r10 *rl_AdditionInformationList_ptr
);
...
SIDE EFFECTS    Ordered config is updated with the new RL info.
===========================================================================*/
boolean is_current_camped_psc_in_rl_info_r8(sys_modem_as_id_e_type wrrc_as_id,rrc_DL_InformationPerRL_List_r8 *ie_ptr);
...
#ifdef FEATURE_WCDMA_DC_HSDPA
/*============================================================================
FUNCTION: rrcllcpcie_process_dc_hsdpa_params_r10()

DESCRIPTION:
  This function processes IE "DL_SecondaryCellInfoFDD" content in ordered config for R10 OTAs. Basically,
  this function updates the OC with the IE content and does some valiadation.
...
SIDE EFFECTS:
=============================================================================*/
uecomdef_status_e_type rrcllcpcie_process_sec_hsdpa_params_r10
(
  sys_modem_as_id_e_type wrrc_as_id,
  rrc_DL_SecondaryCellInfoFDD_r10* dc_hsdpa_params,
  rrc_RRC_StateIndicator rrc_state,
  OSS_UINT32 dc_hsdpa_params_present_ota
  ,OSS_UINT32 additional_sec_carriers_present_ota,
  rrc_AdditionalDLSecCellInfoListFDD * additionalDLSecCellInfoListFDD
);
#endif
...
/*============================================================================
FUNCTION: 
void rrcllcpcie_fill_sec_carrier_info

DESCRIPTION:
This function fills secondary carrier freqs if configured.
...
SIDE EFFECTS:
=============================================================================*/
boolean rrcllcpcie_fill_sec_carrier_info(sys_modem_as_id_e_type wrrc_as_id, uint16* secondary_freq, uint8* num_sec_carriers);
...
/*====================================================================
FUNCTION: rrcllc_get_sib5_change_in_progress_flag_wo_f3()
...
#endif
rrcllc_sib5_change_type rrcllc_get_sib5_change_in_progress_flag_wo_f3
(
sys_modem_as_id_e_type wrrc_as_id
);
...
SIDE EFFECTS:
  None.
====================================================================*/
void rrcllc_copy_sib_to_oc_for_hsrach(sys_modem_as_id_e_type wrrc_as_id, rrc_state_e_type next_rrc_state);
...
boolean rrcllc_check_if_hs_rach_can_be_started_reconfigured(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type* config_ptr);
/*====================================================================
...
SIDE EFFECTS:
  None.
====================================================================*/
boolean rrcllc_check_hsrach_mappings_for_all_rbs_present(sys_modem_as_id_e_type wrrc_as_id);
...
void rrchsrach_set_hsrach_action_to_stop_if_active_ota(sys_modem_as_id_e_type wrrc_as_id);
/*====================================================================
...
uecomdef_status_e_type rrcllcpcie_check_for_duplicate_rl_entry(sys_modem_as_id_e_type wrrc_as_id);
#ifdef FEATURE_WCDMA_REL9
...
/*====================================================================
FUNCTION: rrcllc_handover_to_utran_process_preconfig_r10()
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_handover_to_utran_process_preconfig_r10
(
  sys_modem_as_id_e_type wrrc_as_id,
  rrc_HandoverToUTRANCommand_r10_IEs *msg_ptr
);


/*====================================================================
FUNCTION: rrcllc_rrc_conn_setup_ie_r10()

DESCRIPTION:
  This function updates ordered_config with the latest IEs recv'd 
  in R10 RRC Connection Setup Message.
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_rrc_conn_setup_ie_r10
(
  sys_modem_as_id_e_type wrrc_as_id,
  rrc_RRCConnectionSetup_r10 *rrc_conn_setup_r10_ota
);


/*============================================================================
FUNCTION: rrcllcpcie_dl_common_mode_specific_info_r10()
...
uecomdef_status_e_type
rrcllcpcie_dl_common_mode_specific_info_r10
(
  sys_modem_as_id_e_type wrrc_as_id,
  struct  rrc_DL_CommonInformation_r10_fdd * ie_ptr,
  l1_dl_common_info_struct_type *dl_common
);

/*============================================================================
FUNCTION: set_compressed_mode_info_r10()
...
SIDE EFFECTS:
=============================================================================*/
uecomdef_status_e_type set_compressed_mode_info_r10
(
  sys_modem_as_id_e_type wrrc_as_id,
  rrc_DPCH_CompressedModeInfo_r10 * rrc_compressed_mode_ptr
);

/*====================================================================
FUNCTION: rrcllc_active_set_update_ie_r10()
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_active_set_update_ie_r10
(
  sys_modem_as_id_e_type wrrc_as_id,
  rrc_ActiveSetUpdate_r10 * active_set_update_r10_ota
);

/*====================================================================
FUNCTION: rrcllc_phychan_reconfig_ie_r10()

DESCRIPTION:
  This function updates ordered_config with the latest IEs recv'd 
  in the Physical Channel Reconfiguration Message for Rel 10
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_phychan_reconfig_ie_r10
(
  sys_modem_as_id_e_type wrrc_as_id,
  rrc_PhysicalChannelReconfiguration_r10 * phys_chan_reconfig_r10_ota
);

/*====================================================================
FUNCTION: rrcllc_rb_release_ie_r10()

DESCRIPTION:
  This function updates ordered_config with the latest IEs recv'd 
  in the Release 8 Radio Bearer Release Message R10
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_rb_release_ie_r10
(
  sys_modem_as_id_e_type wrrc_as_id,
  rrc_RadioBearerRelease_r10 * rb_release_r10_ota
);


/*====================================================================
FUNCTION: rrcllc_trch_reconfig_ie_r10()

DESCRIPTION:
  This function updates ordered_config with the latest IEs recv'd 
  in the Release 10 Transport Channel Reconfiguration Message
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_trch_reconfig_ie_r10
(
  sys_modem_as_id_e_type wrrc_as_id,
  rrc_TransportChannelReconfiguration_r10 * trch_reconfig_r10_ota
);

/*====================================================================
FUNCTION: rrcllc_cell_update_confirm_ie_r10()
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_cell_update_confirm_ie_r10
(
  sys_modem_as_id_e_type wrrc_as_id,
  uint32 dl_sdu_num,
  void * cell_update_r10_msg_ptr

);
/*====================================================================
FUNCTION: rrcllc_rb_setup_ie_r10()
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_rb_setup_ie_r10
(
  sys_modem_as_id_e_type wrrc_as_id,
  rrc_RadioBearerSetup_r10 * rb_setup_r10_ota
);

/*====================================================================
FUNCTION: rrcllc_rb_reconfig_ie_r10()

DESCRIPTION:
  This function updates ordered_config with the latest IEs recv'd 
  in the Radio Bearer Reconfiguration Message R10
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_rb_reconfig_ie_r10
(
  sys_modem_as_id_e_type wrrc_as_id,
  rrc_RadioBearerReconfiguration_r10 * rb_reconfig_r10_ota
);

/*====================================================================
FUNCTION: rrcllc_handover_to_utran_ie_r10()
...
SIDE EFFECTS:
  None.
====================================================================*/
uecomdef_status_e_type rrcllc_handover_to_utran_ie_r10
(
  sys_modem_as_id_e_type wrrc_as_id,
  rrc_HandoverToUTRANCommand_LaterCriticalExtensions_r10 *msg_ptr
);
...
SIDE EFFECTS:
====================================================================*/
boolean rrcllc_is_hs_repointing_happening(sys_modem_as_id_e_type wrrc_as_id);
...
uecomdef_status_e_type rrcllcpcie_validate_dbdc_band_combination(sys_modem_as_id_e_type wrrc_as_id);
/*============================================================================
FUNCTION: 
...
SIDE EFFECTS:
=============================================================================*/
uint8 rrcllcpcie_return_num_sec_hsdpa_carriers(sys_modem_as_id_e_type wrrc_as_id, ordered_config_type* config_ptr);
/*===========================================================================
...
/*====================================================================
FUNCTION: rrc_init_rrcllcoc_data()

DESCRIPTION:
  This function initializes rrcllcoc data
...
SIDE EFFECTS:
====================================================================*/
void rrc_init_rrcllcoc_data(sys_modem_as_id_e_type wrrc_as_id);
/*====================================================================
FUNCTION: rrc_init_rrcllcpcie_data()

DESCRIPTION:
  This function initializes rrcllcpcie data
...
SIDE EFFECTS:
====================================================================*/
void rrc_init_rrcllcpcie_data(sys_modem_as_id_e_type wrrc_as_id);
/*====================================================================
FUNCTION: rrc_check_and_update_max_data_rate

DESCRIPTION:This function updates maximum possible data rates to DS bases on 
current RRC configuration
...
void rrc_check_and_update_max_data_rate(sys_modem_as_id_e_type wrrc_as_id);
/*====================================================================
FUNCTION:  euledpch_max_configured_data_rate

DESCRIPTION
  This function returns the maximum data rate possible based on current
  semi-static configuration.

DEPENDENCIES
  To be called by RRC after L1 sends CPHY_SETUP_CNF

RETURN VALUE
  1) 0 if EUL is not configured
  2) Max Data rate in bits per second in uint32 if EUL is configured

SIDE EFFECTS
  None
===========================================================================*/
extern uint32 euledpch_max_configured_data_rate(sys_modem_as_id_e_type wrrc_as_id);
...
#endif
/*====================================================================
FUNCTION: rrcllc_get_target_frequency_if_inter_freq_hho_configuration_in_progress()
...
SIDE EFFECTS:
====================================================================*/
boolean rrcllc_get_target_frequency_if_inter_freq_hho_configuration_in_progress(sys_modem_as_id_e_type wrrc_as_id, rrc_freq_type *freq);
/*============================================================================
...
FUNCTION: rrcllc_set_chan_config_for_oos_in_progress_flag

DESCRIPTION:
   Below global tracks if channel config for OOS is being in progress.
   This global is set to true when channel config for oos in fach is 
   called and reset when reentry within fach function is called
   This global ensures TOC is not overwritten if OOS with fach function
   is called twice before reentry
...
void rrcllc_set_chan_config_for_oos_in_progress_flag(sys_modem_as_id_e_type wrrc_as_id,boolean val);
/*===========================================================================

FUNCTION: rrcllc_get_chan_config_for_oos_in_progress_flag
...
boolean rrcllc_get_chan_config_for_oos_in_progress_flag(sys_modem_as_id_e_type wrrc_as_id);
...
void rrcllc_post_reconfig_complete_ind_for_failure(sys_modem_as_id_e_type wrrc_as_id);
/*===========================================================================
...
===========================================================================*/
boolean rrctfcc_is_reconfig_in_progress(sys_modem_as_id_e_type wrrc_as_id);
...
void rrc_print_cmac_ul_config(sys_modem_as_id_e_type wrrc_as_id, wcdma_l2_ul_cmd_type* l2_ul_cmd_ptr);
...
/*====================================================================
FUNCTION: rrcllc_is_rrc_waiting_for_act_time_expiry()

DESCRIPTION:
  This function checks if activation time is yet to expire

DEPENDENCIES:
  None.

RETURN VALUE:
  TRUE   Act time yet to be reached
  FALSE  Act time reached.

SIDE EFFECTS:
  None.

=====================================================================*/
boolean rrcllc_is_rrc_waiting_for_act_time_expiry(sys_modem_as_id_e_type wrrc_as_id);
/*====================================================================
...
FUNCTION   rrcllcoc_update_cm_status_info_to_config_ptr

DESCRIPTION

 This function updates CM status info to OC/CC
...
void rrcllcoc_update_cm_status_info_to_config_ptr(sys_modem_as_id_e_type wrrc_as_id,
                                                                      ordered_config_type *config_ptr);

#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
/*===========================================================================

FUNCTION rrcllcoc_deactivate_existing_cm_patterns()

DESCRIPTION

  This function populates and deactivates all existing CM patterns
...
void  rrcllcoc_deactivate_existing_cm_patterns(sys_modem_as_id_e_type wrrc_as_id,
                                      l1_meas_ctrl_parm_struct_type *meas_ptr);
/*===========================================================================

FUNCTION rrcllcoc_activate_existing_cm_patterns()
...
void  rrcllcoc_activate_existing_cm_patterns(sys_modem_as_id_e_type wrrc_as_id,
                                      l1_meas_ctrl_parm_struct_type *meas_ptr);
/*===========================================================================

FUNCTION rrcllcoc_check_if_cm_pattern_active()

DESCRIPTION

  This function checks if network has activated the compressed mode params.
...
===========================================================================*/
boolean rrcllcoc_check_if_cm_pattern_active(sys_modem_as_id_e_type wrrc_as_id);
