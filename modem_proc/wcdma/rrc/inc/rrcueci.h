...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/inc/rrcueci.h#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
01/09/16   sr      Made changes to fill LTE band capability using LRRC api
...
#define  RRC_IS_MAC_I_NV_TURNED_ON \
  ...
  (rrcnv_data.rrc_feature_nv_item_value & RRC_MAC_I_ENABLED))
...
typedef struct
{
  ...
  boolean lte_truncated_cap_requested;/*Set to TRUE when LTE truncated cap is requested */
  boolean lte_truncated_cap_appended; /*Set to TRUE if LTE truncated cap is appended in UL OTA i.e in Setup_complete or UCI*/
...
typedef struct
{
  boolean nv_wtog_nacc_enabled;
  boolean rrc_mimo_64qam_cm_enabled;
  boolean support_prio_based_resel;
  /* This variable holds substate for UE Capability Information Procedure  */
  rrcueci_substate_e_type rrcueci_substate;

  /* This stores the transaction Id for UE Capability Enquiry message */  
  rrc_RRC_TransactionIdentifier rrcueci_ue_cap_enquiry_trans_id;

#ifdef FEATURE_WCDMA_REL7_SPECRS
  rrc_ue_capability_originator_e_type rrcueci_ue_cap_originator;
#endif

#ifdef FEATURE_WCDMA_TO_LTE
  rrcueci_interrat_lte_capability_info_type rrcueci_interrat_lte_capability_info;
#endif
}rrcueci_int_data_type;


typedef struct
{
  uint16 ueci_message_length;
  ...
  boolean rrc_ue_pos_cap_pch;
#endif
}rrcueci_ext_data_type;
...
extern rrcueci_ext_data_type rrcueci_ext_data_sim[MAX_RRC_STACKS];
#define rrcueci_ext_data rrcueci_ext_data_sim[WAS_ID_M]
#else
extern rrcueci_ext_data_type rrcueci_ext_data;
#endif
...
===========================================================================*/
boolean rrcueci_is_truncated_ca_needed
(
  sys_modem_as_id_e_type wrrc_as_id, 
  rrc_plmn_identity_type src_rat_plmn_id /*Will be used for IHO cases only*/
);
...
SIDE EFFECTS:
  None.
====================================================================*/
boolean rrcueci_nv_supports_edpcch_pwr_boost(sys_modem_as_id_e_type wrrc_as_id);
