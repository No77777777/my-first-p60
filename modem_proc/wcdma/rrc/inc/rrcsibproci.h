...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/inc/rrcsibproci.h#2 $    $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
05/21/15   vi      Made changes to set default value of SIB 7 validity timer to 2 sec for
                   legacy cell and 8sec for HS-FACH/EDRX cell
...
#define RRCSIB_DEFAULT_HS_SIB7_VALIDITY_TIME 8000
...
typedef enum
{
  ...
  SIBS_FAILED_DUPLICATE /* Sib Processing failed or Duplicate SIBs received */
...
typedef struct
{
  ...
  boolean is_sib7_in_fach_active;
#endif
#endif
...
uint32 max_sib_rept_rate;
int32 timer_value_started;
#endif

#ifdef FEATURE_DUAL_SIM
boolean rrcsib_stop_partition;
boolean g_in_data_call;
boolean mib_sb_received;
uint16  start_holes_sfn;
#endif
...
} rrcsibproc_int_data_type;

#ifdef FEATURE_DUAL_WCDMA
extern rrcsib_collect_data_type rrcsib_collect_data_sim[MAX_RRC_STACKS];

/* The database is externalized so that the rrcsibcollect
module can use the database. */
extern sib_active_event_database_type *sib_events_ptr_sim[MAX_RRC_STACKS];

#define rrcsib_collect_data rrcsib_collect_data_sim[WAS_ID_M]
#define sib_events_ptr sib_events_ptr_sim[WAS_ID_M]

#else
extern rrcsib_collect_data_type rrcsib_collect_data;
...
boolean rrcsib_update_sibs_active_for_bcch_mod
(
  sys_modem_as_id_e_type wrrc_as_id
);
#endif /* RRCSIBPROCI_H */
