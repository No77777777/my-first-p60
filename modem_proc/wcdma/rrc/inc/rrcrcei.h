...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/inc/rrcrcei.h#3 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
#define RRCTMR_RRC_CON_DELAY_TIMER_FOR_CONCURRENCY_CASES  500
...
#define RRCRCE_R10_CRITICAL_EXT_PRESENT \
    ((msg_ptr->message.u.rrcConnectionSetup.u.  \
    later_than_r3.criticalExtensions.t ==  \
    T_rrc_RRCConnectionSetup_criticalExtensions_5_criticalExtensions) && \
    (msg_ptr->message.u.rrcConnectionSetup.u. \
    ...
    (msg_ptr->message.u.rrcConnectionSetup.u. \
     ...
     (msg_ptr->message.u.rrcConnectionSetup.u. \
     later_than_r3.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.t == \
     T_rrc_RRCConnectionSetup_later_r8_criticalExtensions_later_r9_criticalExtensions) && \
     (msg_ptr->message.u.rrcConnectionSetup.u. \
     later_than_r3.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.criticalExtensions.u.later_r8_criticalExtensions.u.later_r9_criticalExtensions.t== \
     T_rrc_RRCConnectionSetup_later_r9_criticalExtensions_r10))
...
typedef struct
{
  /* RCE Substate variable initialized to RCE_INITIAL during the RRC TASK 
     startup. */
  rrcrce_substate_e_type  substate;
  
  uint32 number_of_rbs;     /* Indicates number of RBs that are 
                                            received thru RRC CONNECTION
                                            SETUP message */
  rrc_RRC_TransactionIdentifier setup_transaction_identifier;
                                         /* This stores the transaction Id for
                                            RRC Connection Setup message */
  rrc_RRC_TransactionIdentifier reject_transaction_identifier;
                                         /* This stores the transaction Id for
                                            RRC Connection Reject message */
  ...
  rrc_WaitTime wait_time;  /* This stores the Wait Time received
                                           through RRC Connection Reject
                                           message */
  boolean abort_ind_sent;
  /* Flag set when there is a higher layer abort */
  boolean send_abort_cnf;
  
  uint8 num_retries_count;
  
  boolean sig_con_delay;
  
  boolean sig_conn_rel_req_for_both_domains;
  ...
  uint8  chl_config_transaction_id;
  
  /* Stores the RRC Connection Setup message */
  dl_ccch_msg_type save_dl_ccch_msg;
  
  /* Stores the information that redirection is via Connection
   * reject or Connection Setup */
  rrcrce_conn_rsp_e_type rsp_to_rrc_conn_req;
  
  rrcrce_default_config_info_type default_config_info;

  boolean lai_rai_mismatch;
  
  mac_status_ind_e_type cmac_status;

  rrc_abort_cause_e_type mm_abort_cause;

#ifdef FEATURE_WCDMA_FAST_RETURN_TO_LTE_AFTER_CSFB
  boolean crt_restarted_once;
#endif

  rrc_freq_type cell_resel_freq;

  rrc_scr_code_type cell_resel_scr;

/*CN Domain Identity*/
  rrc_cn_domain_identity_e_type est_cn_domain_id;

  /*Call Type*/
  rrc_call_type_in_est_req_e_type  est_call_type;

  /* Routing Area ID */
  rrc_rai_type  est_req_for_rai;
}rrcrce_int_data_type;
