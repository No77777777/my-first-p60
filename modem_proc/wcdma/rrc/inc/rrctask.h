...
$Header: //components/rel/wcdma.mpss/8.4.0/rrc/inc/rrctask.h#2 $    $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
DESCRIPTION
  This function is the entry point for RRC Task2. It initializes RRC state
  and individual procedures by calling rrc_init(sys_modem_as_id_e_type wrrc_as_id, ). The ACK signal is sent back
  to TMC and a wait is done on Task Start Signal from TMC. Once a Task Start
  Signal is received, rrc_dispatcher(sys_modem_as_id_e_type wrrc_as_id, ) is called that takes control now onwards.
