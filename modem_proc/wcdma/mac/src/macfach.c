...
Copyright (c) 2002-2005, 2008-2009 by Qualcomm Technologies Incorporated.  All Rights Reserved.
...
$Header: //components/rel/wcdma.mpss/8.4.0/mac/src/macfach.c#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
==================================================================================================*/
#define   PROCESS_BCCH_FACH_CONFIG(dl_table_ptr)\
\
...
\
      ...
      WL2_MSG_1(HIGH, dl_table_ptr, "invalid BCCH ID %d",oc_fach_ptr->bcch_id);  \
    } \
...
==================================================================================================*/
#define   PROCESS_CCCH_FACH_CONFIG(dl_table_ptr)\
\
...
\
        ...
        if (DL_RLC_ID_IS_INVALID(oc_fach_ptr->ccch_id)){  \
            WL2_MSG_1(HIGH, dl_table_ptr, "invalid CCCH ID %d",oc_fach_ptr->ccch_id);  \
    } \
...
==================================================================================================*/
#define   PROCESS_CTCH_FACH_CONFIG(dl_table_ptr)\
\
...
\
    ...
    if (DL_RLC_ID_IS_INVALID(oc_fach_ptr->ctch_id)){  \
      WL2_MSG_1(HIGH, dl_table_ptr, "invalid CTCH ID %d", oc_fach_ptr->ctch_id); \
    } \
    else {  \
      /* Set CTCH as enabled */ \
  ...
  )
{
  ...
  for (i=0 ; i < oc_fach_ptr->ndlchan; i++)
  {
    ...
    {
      WL2_MSG_2(HIGH, dl_table_ptr , "invalid DCCH/DTCH%d ID %d",
                                                  i, oc_dlc_ptr->rlc_id);
    }
    ...
    else
    {
      ...
      {
        WL2_MSG_1(HIGH, dl_table_ptr , "invalid MAC ID %d",
                                                    oc_dlc_ptr->mac_id);
      }
      else
      {
    ...
    if (valid == TRUE)
    {
      ...
      WL2_MSG_1(LOW, dl_table_ptr , "FACH/DCCH or DTCH ID = %d",
                                                 oc_dlc_ptr->rlc_id);
    }
  }
}
...
============================================================================================*/
#define   PROCESS_BCCH_TB_ON_FACH(tb_num, dl_table_ptr)\
\
...
\
    ...
    WL2_MSG_1(HIGH, dl_table_ptr, "TB %d BCCH not enabled",tb_num); \
    }
...
============================================================================================*/
#define   PROCESS_CCCH_TB_ON_FACH(tb_num, dl_table_ptr)\
\
...
\
    ...
    WL2_MSG_1(HIGH, dl_table_ptr, "TB%d CCCH not enabled",tb_num); \
  }
...
============================================================================================*/
#define   PROCESS_CTCH_TB_ON_FACH(tb_num, dl_table_ptr)\
\
...
\
  ...
  /* CTCH not enabled */  \
    ...
    WL2_MSG_1(HIGH, dl_table_ptr, "TB%d CTCH not enabled", tb_num);  \
    }
...
DESCRIPTION   This macro processes header info of FACH TB that is mapped to DCCH/DTCH with
        URNTI
============================================================================================*/
#define   PROCESS_DCCH_DTCH_TB_WITH_URNTI_ON_FACH(tb_num, dl_table_ptr)\
\
  /* URNTI field of TB */ \
...
\
  /* DL TB URNTI is invalid */  \
    if (DL_TB_URNTI_IS_INVALID(urnti, dl_table_ptr)) {  \
    WL2_MSG_2(HIGH, dl_table_ptr, "TB %d with invalid URNTI %x",tb_num, urnti);  \
  } \
\
...
\
    ...
    /* DCCH/DTCH not enabled */ \
      ...
      WL2_MSG_1(HIGH, dl_table_ptr, "TB %d with disabled DCCH/DTCH", tb_num); \
    } \
  } \
...
\
    if (DL_CT_FLD_IS_INVALID(int_fach_ptr->num_dlc,ct_field)) { \
      WL2_MSG_2(HIGH, dl_table_ptr, "TB %d with invalid C/T %x", tb_num, ct_field); \
    } \
    else {  \
      /* Pointer to internal DCCH/DTCH config info */ \
...
\
      ...
      /* DCCH/DTCH not enabled */ \
        ...
        WL2_MSG_2(HIGH, dl_table_ptr, "TB%d with disabled DCCH/DTCH",tb_num, ct_field); \
      } \
    } \
  }
...
DESCRIPTION   This macro processes header info of FACH TB that is mapped to DCCH/DTCH with
        CRNTI
============================================================================================*/
#define   PROCESS_DCCH_DTCH_TB_WITH_CRNTI_ON_FACH(tb_num, dl_table_ptr)\
\
  /* CRNTI of TB */ \
...
\
  /* If C-RNTI is valid, check the C/T field */ \
    if (DL_TB_CRNTI_IS_INVALID(crnti, dl_table_ptr)) {  \
    WL2_MSG_2(HIGH, dl_table_ptr, "TB %d with invalid CRNTI %x",tb_num, crnti); \
  } \
  ...
  )
{
  ...
  l1_dl_tb_hdr_struct_type      *tb_hdr_ptr;
    ...
    if (tbs_hdr_ptr==NULL)
    {
        WL2_MSG_0(HIGH, dl_table_ptr, "TBS_HDR_PTR NULL\n");
        return;
    }
  ...
  for (i=0 ; i <num_tb; i++)
  {
    ...
    {
      WL2_MSG_1(HIGH, dl_table_ptr , "TB%d header pointer is NULL",
                                                  i);
    }
    else
    {
      ...
      if (valid == TRUE)
      {
        ...
        if ((status = rlc_dl_decode_pdu_hdr (dl_table_ptr->as_id, &rlc_hdr_info, &rlc_cipher_info)) == TRUE)
        {
        ...
        dl_tb_info_ptr = GET_DL_TB_INFO_PTR(tb_id + i, dl_table_ptr);
        if (status == TRUE)
        {
...
)
{
  ...
  dl_table_ptr->num_active_fachs = 0;

  for (i=0;i<2*MAX_TRCH;i++)
  {
    dl_table_ptr->fach_id_to_idx[i].valid = FALSE;
  }

  /* Invalid number of FACHs */
  if (num_fachs > UE_MAX_TRCH)
  {
    WL2_MSG_1(HIGH, dl_table_ptr , "invalid number of FACHs %d",
                                                num_fachs);
  }
  ...
  else
  {
    for (i=0; i < num_fachs; i++)
    {
      ...
      if (oc_fach_ptr == NULL)
      {
        WL2_MSG_1(HIGH, dl_table_ptr , "FACH%d config pointer is NULL",
                                                    i);
      }
      /* Invalid TrCh ID */
      else if (DL_TRCH_ID_IS_INVALID(oc_fach_ptr->trch_id))
      {
        WL2_MSG_2(HIGH, dl_table_ptr , "FACH%d with invalid TrCh ID %d",
                                                    i, oc_fach_ptr->trch_id);
      }
      /* Invalid CCTrCh ID */
      else if (DL_CCTRCH_ID_IS_INVALID(oc_fach_ptr->cctrch_id))
      {
        WL2_MSG_2(HIGH, dl_table_ptr , "FACH%d with invalid CCTrCh ID",
      ...
      else
      {
        ...
        int_fach_ptr = &dl_table_ptr->fach_cfg_info[dl_table_ptr->num_active_fachs];

        /* Get Transport ID to Index map Table */
        id_to_idx_ptr = FACH_ID_TO_IDX_PTR(oc_fach_ptr->trch_id, dl_table_ptr);
        id_to_idx_ptr->valid = TRUE;
        id_to_idx_ptr->idx = dl_table_ptr->num_active_fachs;
        ...
        WL2_MSG_3(LOW, dl_table_ptr , "FACH%d Config Data : TrChID=%d CCTrChID=%d",
                                                   i,
                                                   int_fach_ptr->trch_id,
                                                   int_fach_ptr->cctrch_id);

        /* Process BCCH data */
        PROCESS_BCCH_FACH_CONFIG(dl_table_ptr)
          WL2_MSG_1(LOW, dl_table_ptr , "FACH/BCCH id = %d",
                                                     int_fach_ptr->bcch_id);

        /* Process CCCH data if any */
        PROCESS_CCCH_FACH_CONFIG(dl_table_ptr)
          WL2_MSG_1(LOW, dl_table_ptr , "FACH/CCCH id = %d",
                                                     int_fach_ptr->ccch_id);

        /* Process CTCH data if any */
        PROCESS_CTCH_FACH_CONFIG(dl_table_ptr)
          WL2_MSG_1(LOW, dl_table_ptr , "FACH/CTCH id = %d",
                                                     int_fach_ptr->ctch_id);


        /* Process DCCH/DTCH data */
        process_dcch_dtch_fach_config (oc_fach_ptr, int_fach_ptr, dl_table_ptr);

        /* Increment the number of active FACHs */
        dl_table_ptr->num_active_fachs++;
      }
...
log_wcdma_trch_type mac_dl_check_fach_trch(rlc_lc_id_type rlc_id, rb_id_type rb_id, mac_dl_table_type *dl_table_ptr)
{
  log_wcdma_trch_type return_trch_val = WCDMA_TRCH_INVALID;
  fach_config_info_type *fach_cfg_ptr = NULL;
  int i, j;

  for(i = 0; i < dl_table_ptr->num_active_fachs; i++) 
  {
    fach_cfg_ptr = &dl_table_ptr->fach_cfg_info[i];
    do 
    {
      /* Case when BCCH is mapped on to FACH */
      if((fach_cfg_ptr->bcch_enable == TRUE) &&
          (fach_cfg_ptr->bcch_id == rlc_id))
      {
          return_trch_val = WCDMA_TRCH_FACH;
          break;        
      }
      /* Case when CCCH is mapped on to FACH */
      if((fach_cfg_ptr->ccch_enable == TRUE) &&
          (fach_cfg_ptr->ccch_id == rlc_id))
      {
          return_trch_val = WCDMA_TRCH_FACH;
          break;        
      }
      /* Case when CTCH is mapped on to FACH */
      if((fach_cfg_ptr->ctch_enable == TRUE) &&
          (fach_cfg_ptr->ctch_id == rlc_id))
      {
          return_trch_val = WCDMA_TRCH_FACH;
          break;        
      }           
    ...
    for(j = 0; j < fach_cfg_ptr->num_dlc; j++)
    {
      if((fach_cfg_ptr->dlcinfo[j].enable == TRUE) && 
         (fach_cfg_ptr->dlcinfo[j].rlc_id == rlc_id))
       {
          return_trch_val = WCDMA_TRCH_FACH;
          break;
       }
