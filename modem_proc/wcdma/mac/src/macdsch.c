...
$Header: //components/rel/wcdma.mpss/8.4.0/mac/src/macdsch.c#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
  ...
  )
{
  ...
  for (i=0 ; i < oc_dsch_ptr->ndlchan; i++)
  {
    ...
    if (valid == TRUE)
    {
      ...
      WL2_MSG_1(LOW, dl_table_ptr , "DL DCH/DCCH or DTCH ID = %d",
                                                 oc_dlc_ptr->rlc_id);
    }
  }
}
  ...
  )
{
  ...
  dl_table_ptr->num_active_dschs = 0;

  /* Initialize DSCH ID to Index Mapping Table */
  for (i=0; i<MAX_TRCH; i++)
  {
    dl_table_ptr->dsch_id_to_idx[i].valid = FALSE;
  }

  /* Invalid number of DSCHs */
  if (num_dschs > UE_MAX_TRCH)
  {
    WL2_MSG_1(HIGH, dl_table_ptr , "invalid number of DSCHs %d",
                                                num_dschs);
  }
  ...
  else
  {
    for (i=0; i < num_dschs; i++)
    {
      ...
      if (oc_dsch_ptr == NULL)
      {
        WL2_MSG_1(HIGH, dl_table_ptr , "DSCH%d config pointer is NULL",
                                                    i);
      }
      /* Invalid TrCh ID */
      else if (DL_TRCH_ID_IS_INVALID(oc_dsch_ptr->trch_id))
      {
        WL2_MSG_2(HIGH, dl_table_ptr , "DSCH%d with invalid TrCh ID %d",
      ...
      {        
        ...
        int_dsch_ptr = &dl_table_ptr->dsch_cfg_info[dl_table_ptr->num_active_dschs];

        /* Get Transport ID to Index map Table */
        id_to_idx_ptr = DSCH_ID_TO_IDX_PTR(oc_dsch_ptr->trch_id, dl_table_ptr);
        id_to_idx_ptr->valid = TRUE;
        id_to_idx_ptr->idx = dl_table_ptr->num_active_dschs;

        /* Save CCTrCh ID in internal DSCH Config Table */
        int_dsch_ptr->cctrch_id = oc_dsch_ptr->cctrch_id;

        WL2_MSG_3(LOW, dl_table_ptr , "DSCH%d Config Data:TrChID=%d CCTrChID=%d",
                                                   i,
                                                   int_dsch_ptr->trch_id,
                                                   int_dsch_ptr->cctrch_id);

        /* Process DCCH/DTCH data */
        process_dcch_dtch_dsch_config (oc_dsch_ptr, int_dsch_ptr, dl_table_ptr);

        /* Increment the number of active DSCHs */
        dl_table_ptr->num_active_dschs++;
      }
    }
  }
}
...
DESCRIPTION   This macro processes header info of DSCH TB that is mapped to DSCH with
        URNTI
==========================================================================================*/
#define   PROCESS_DCCH_DTCH_TB_WITH_URNTI_ON_DSCH(tb_num, dl_table_ptr)\
\
...
\
    ...
    /* DCCH/DTCH is not enabled */  \
      ...
      WL2_MSG_1(HIGH, dl_table_ptr, "TB %d: DCCH/DTCH not enabled",tb_id); \
    } \
  } \
  /* Multiple DCCH/DTCHs are mapped to DSCH */  \
  else if (int_dsch_ptr->num_dlc > 1){  \
...
\
    /* C/T field is invalid */  \
      ...
      WL2_MSG_2(HIGH, dl_table_ptr, "TB %d with invalid C/T %d", tb_num, ct_field); \
    } \
    /* Valid C/T field */ \
    else {  \
      /* Pointer to internal DCCH/DTCH config info */ \
...
DESCRIPTION   This macro processes header info of DSCH TB that is mapped to DSCH with
        CRNTI
==========================================================================================*/
#define   PROCESS_DCCH_DTCH_TB_WITH_CRNTI_ON_DSCH(tb_num, dl_table_ptr)\
\
  /* C-RNTI value */  \
...
\
    ...
    /* DCCH/DTCH is not enabled */  \
      ...
      WL2_MSG_1(HIGH, dl_table_ptr, "DCCH/DTCH of TB%d not enabled",tb_num); \
    } \
    } \
  /* Multiple DCCH/DTCHs are mapped to DSCH */  \
  else if (int_dsch_ptr->num_dlc > 1){  \
  ...
  )
{
  ...
  l1_dl_tb_hdr_struct_type    *tb_hdr_ptr;
    ...
    if (tbs_hdr_ptr==NULL)
    {
        WL2_MSG_0(ERROR, dl_table_ptr, "DSCH_TBS_HDR_PTR is NULL");
        return;
    }
  ...
  for (i=0 ; i < num_tb; i++)
  {
    ...
    {
      WL2_MSG_1(HIGH, dl_table_ptr , "TB%d header info pointer is NULL",
                                                  i);
    }
    /* if two MSBits are 00, then it is DCCH/DTCH TB with U-RNTI */
    else if ((tb_hdr_ptr->tb_header[0] & 0xc0) == 0x00)
    {
      PROCESS_DCCH_DTCH_TB_WITH_URNTI_ON_DSCH(i, dl_table_ptr)
    }
    /* if two MSBits are 01, then it is DCCH/DTCH TB with C-RNTI */
    else if ((tb_hdr_ptr->tb_header[0] & 0xc0) == 0x40)
    {
      PROCESS_DCCH_DTCH_TB_WITH_CRNTI_ON_DSCH(i, dl_table_ptr)
    }
    /* TB with invalid RNTI header */
    else
    {
      WL2_MSG_2(HIGH, dl_table_ptr , "TB%d with invalid RNTI header %x",
...
===============================================================================================*/
log_wcdma_trch_type mac_dl_check_dsch_trch(rlc_lc_id_type rlc_id, rb_id_type rb_id, mac_dl_table_type *dl_table_ptr)
{
  log_wcdma_trch_type return_trch_val = WCDMA_TRCH_INVALID;
  int i, j;
  dsch_config_info_type *dsch_cfg_info_ptr = NULL;

  for(i = 0; i < dl_table_ptr->num_active_dschs; i++)
  {
    dsch_cfg_info_ptr = &dl_table_ptr->dsch_cfg_info[i];
    for(j = 0; j < dsch_cfg_info_ptr->num_dlc; j++)
    {
      if((dsch_cfg_info_ptr->dlcinfo[j].enable == TRUE) && 
         (dsch_cfg_info_ptr->dlcinfo[j].rlc_id == rlc_id) && 
         (dsch_cfg_info_ptr->dlcinfo[j].rb_id == rb_id))
      {
  ...
  WL2_MSG_1(HIGH, dl_table_ptr, "function mac_dl_check_dsch_trch returning %d",
                                             return_trch_val);
