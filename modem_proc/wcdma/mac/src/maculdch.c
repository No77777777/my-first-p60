...
$Header: //components/rel/wcdma.mpss/8.4.0/mac/src/maculdch.c#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $

when      who   what, where, why
--------  ---   -----------------------------------------------------------------------------
12/03/15  as    MAC F3 reduction.
...
)
{
  ...
  sys_modem_as_id_e_type as_id = ul_table_ptr->as_id;
  ...
  {
    WL2_MSG_0(HIGH, ul_table_ptr , "CCTrCh Config pointer is NULL");
  }
  else if (ded_cctrch_ptr->ndchs > UE_MAX_TRCH)
  {
    WL2_MSG_1(HIGH, ul_table_ptr , "invalid number of DCHs per CCTrCh",
  ...
  else
  {
    ...
    for (i=0; i<ndchs; i++)
    {
      ...
      /* Valid Transport channel parameters */
        ...
        WL2_MSG_3(LOW, ul_table_ptr , "DCH%d Configuration: TrCh ID %d, # of DCCH/DTCHs = %d",
                                                   i,
                                                   oc_dch_ptr->trch_id,
                                                   oc_dch_ptr->ndlchan);
        ...
        for (j=0; j <ndlchan; j++){
          ...
          if (oc_dlcptr == NULL) { /*lint !e774*/
            WL2_MSG_0(HIGH, ul_table_ptr , "DCCH/DTCH%d config pointer is NULL");
          }
          /* Invalid RLC ID */
          ...
          /* All Parameters are valid */
            ...
            if (int_lc_ptr->rlc_mode == UE_MODE_TRANSPARENT)
            {
              ...
              ul_table_ptr->mac_tm_bearers_count++;
            }
            ...
            {
              WL2_MSG_2(ERROR, ul_table_ptr, "Index %d to get the LC id for which MAC needs the buffer status exceeds maximum",
                                                          rlc_count,
            ...
            WL2_MSG_3(LOW, ul_table_ptr , "RLC%d:HdrSize= %d Hdr[0]= %x",
                  oc_dlcptr->rlc_id,
...
log_wcdma_trch_type mac_ul_check_dch_trch(rlc_lc_id_type rlc_id, rb_id_type rb_id, mac_ul_table_type *ul_table_ptr)
{
  ...
  if(int_lc_ptr == NULL)
  {
    WL2_MSG_0(ERROR, ul_table_ptr, "mac_ul_check_dch_trch: GET_UL_LC_INFO_PTR returns NULL");
  }
  ...
  { 
    WL2_MSG_2(HIGH, ul_table_ptr, "mac_ul_check_dch_trch: rrc_rb_id (%d) does not match l2_rb_id",
