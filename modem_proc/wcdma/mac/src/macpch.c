...
$Header: //components/rel/wcdma.mpss/8.4.0/mac/src/macpch.c#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
)
{
  ...
  {
    dl_table_ptr->pch_cfg_info[i].pch_enable = FALSE;
  }

  /* PCH ordered config data pointer is NULL */
  if (oc_pch_info == NULL)
  {
    WL2_MSG_1(HIGH, dl_table_ptr , "PCH%d ordered config pointer is NULL",
                                                i);
  }
  /* Invalid number of CCTrChs */
  else if (num_pchs > MAC_DL_MAX_CCTRCH)
  {
    WL2_MSG_1(HIGH, dl_table_ptr , "invalid number of PCH Type CCTrChs",
                                                num_pchs);
  }
  /* Process each CCTrCh */
  else
  {
    for (i=0; i<num_pchs; i++)
    {
      ...
      {
        WL2_MSG_2(HIGH, dl_table_ptr , "PCH%d with invalid CCTrCh ID",
                                                    i,
      ...
      {
        WL2_MSG_2(HIGH, dl_table_ptr , "PCH%d with invalid PCCH ID %d",
      ...
      else
      {
        ...
        int_pch_ptr = GET_PCH_INFO_PTR(oc_pch_info[i].cctrch_id, dl_table_ptr);

        /* Save PCH info in config info table */
        int_pch_ptr->pch_enable = TRUE;
        int_pch_ptr->pcch_id = oc_pch_info[i].pcch_id;

        WL2_MSG_3(LOW, dl_table_ptr , "PCH%d: CCTrCh ID=%d PCCH ID=%d",
                                                   i,
                                                   oc_pch_info[i].cctrch_id,
...
)
{
  ...
  for (i=0 ; i <tbs_hdr_ptr->num_tb; i++)
  {
    ...
    else
    {
      ...
      WL2_MSG_3(LOW, dl_table_ptr , "TB%d:TB ID=%d, RLC ID=%d",
                                                 i,
                                                 tb_hdr_ptr->tb_id,
               int_pch_ptr->pcch_id);
    }
