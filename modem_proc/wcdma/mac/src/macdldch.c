...
$Header: //components/rel/wcdma.mpss/8.4.0/mac/src/macdldch.c#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
)
{
  ...
  for (i=0 ; i < oc_dch_ptr->ndlchan; i++)
  {
    ...
    {
      WL2_MSG_2(HIGH, dl_table_ptr , "DCCH/DTCH%d with invalid RLC ID %d",
                                                  i, oc_dlc_ptr->rlc_id);
    }
    /* RLC Mode is invalid */
    else if (DL_RLC_MODE_IS_INVALID(oc_dlc_ptr->rlc_mode))
    {
      WL2_MSG_2(HIGH, dl_table_ptr , "DCCH/DTCH%d with invalid RLC Mode %d",
                                                  i, oc_dlc_ptr->rlc_mode);
    }
    /* RB ID is invalid */
    else if (DL_RB_ID_IS_INVALID(oc_dlc_ptr->rb_id))
    {
      WL2_MSG_2(HIGH, dl_table_ptr , "DCCH/DTCH%d with invalid RB ID",
                                                  i, oc_dlc_ptr->rb_id);
    }
    ...
    else if (int_dch_ptr->num_dlc > 1)
    {
      ...
      {
        WL2_MSG_2(HIGH, dl_table_ptr , "DCCH/DTCH%d with invalid MAC ID %d",
                                                    i, oc_dlc_ptr->mac_id);
      }
      else
      {
...
#endif
      WL2_MSG_3(LOW, dl_table_ptr , "DCCH/DTCH%d: RLC ID =%d, Mode =%d",
                                                 i,
                                                 oc_dlc_ptr->rlc_id,
                                                 oc_dlc_ptr->rlc_mode);
    }
...
)
{
  ...
  dl_table_ptr->num_active_dl_dchs = 0;

  /* Initialize DCH ID to Index Mapping Table */
  for (i=0; i<MAX_TRCH; i++)
  {
    dl_table_ptr->dl_dch_id_to_idx[i].valid = FALSE;
  }
  ...
  memset(&dl_table_ptr->lb_delay_type, 0x00, sizeof(lb_delay_struct_type));
  ...
  {
    WL2_MSG_1(HIGH, dl_table_ptr , "invalid number of DCHs %d",
                                                num_dchs);
  }
  ...
  else
  {
    ...
    for (i=0; i < num_dchs; i++)
    {
      ...
      if (oc_dch_ptr == NULL)
      {
        WL2_MSG_1(HIGH, dl_table_ptr , "DCH%d config pointer is NULL",
                                                    i);
      }
      /* Invalid TrCh ID */
      else if (DL_TRCH_ID_IS_INVALID(oc_dch_ptr->trch_id))
      {
        WL2_MSG_2(HIGH, dl_table_ptr , "DCH%d with invalid TrCh ID %d",
      ...
      else
      {
        ...
        int_dch_ptr = &dl_table_ptr->dl_dch_cfg_info[dl_table_ptr->num_active_dl_dchs];

        /* Get Transport ID to Index map Table */
        id_to_idx_ptr = DL_DCH_ID_TO_IDX_PTR(oc_dch_ptr->trch_id, dl_table_ptr);
        id_to_idx_ptr->valid = TRUE;
        id_to_idx_ptr->idx = dl_table_ptr->num_active_dl_dchs;
        ...
        WL2_MSG_3(LOW, dl_table_ptr , "DL DCH%d: TrChID=%d CCTrChID=%d",
                                                   i,
                                                   int_dch_ptr->trch_id,
                                                   int_dch_ptr->cctrch_id);

        /* Process DCCH/DTCH data */
        process_dl_dcch_dtch_dch_config (oc_dch_ptr, int_dch_ptr, dl_table_ptr);

        /* Increment the number of active DL DCHs */
        dl_table_ptr->num_active_dl_dchs++;
      }
    }
  }
}
...
)
{
  ...
  if (tbs_hdr_ptr == NULL)
  {
      WL2_MSG_0(ERROR, dl_table_ptr, "TBS_HDR_PTR NULL\n");
      return;
  }
  ...
  for (i=0 ; i < num_tb; i++)
  {
    ...
    {
      WL2_MSG_1(HIGH, dl_table_ptr , "DCH TB%d header pointer is NULL",
                                                  i);
    }
    ...
    else
    {
      ...
      if ((ul_table_ptr->loopback_test_mode == MAC_LOOPBACK_MODE_2) &&
           ...
           && (tbs_hdr_ptr->info.non_hs.tb_num_bits == 0))
      {
        ...
        if (dlcptr->enable == TRUE)
        {
          ...
          /*---------------------------------------------------------------------------------------
           ...
           ---------------------------------------------------------------------------------------*/
          if ((dl_table_ptr->cfn_marked == FALSE) && (dl_table_ptr->lb_delay_type.cur_idx < MAX_NUM_CFN))
          {
            dl_table_ptr->lb_delay_type.dl_cfn[dl_table_ptr->lb_delay_type.cur_idx] = current_tbs_cfn;
            dl_table_ptr->lb_delay_type.cur_idx++;
            dl_table_ptr->lb_delay_type.cur_idx = dl_table_ptr->lb_delay_type.cur_idx % MAX_NUM_CFN;
            WL2_MSG_3(HIGH, dl_table_ptr, "cur_dl_cfn %d ntb %d rlc_id %d",
                                                       current_tbs_cfn,
                                                       num_tb,
                                                       rlc_hdr_info.rlc_id);
            dl_table_ptr->cfn_marked = TRUE;
          }
        }
      ...
      else if (int_dch_ptr->num_dlc > 1)
      {
        ...
        {
          WL2_MSG_2(HIGH, dl_table_ptr , "TB%d with invalid C/T %x",
                                                      tb_id, ct_field);
        ...
        else
        {
              ...
              && !tb_hdr_ptr->tb_header[3] && !tb_hdr_ptr->tb_header[4] && !tb_hdr_ptr->tb_header[5])
          {
            ...
            {
              valid = FALSE;
              WL2_MSG_1(HIGH, dl_table_ptr, "DCH Block headers are 0, discarding %d tb",
                                                         i);
            }
          }
        }
    ...
    if (valid == TRUE)
    {
      ...
      if (dlcptr->enable == TRUE)
      {
        ...
        {
          status = rlc_dl_decode_pdu_hdr (dl_table_ptr->as_id, &rlc_hdr_info, &rlc_cipher_info);
        }
        ...
        if (status == TRUE)
        {
          ...
          WL2_MSG_2(LOW, dl_table_ptr, "TrchId->%d;TrchType->%d;",
                                                    int_dch_ptr->trch_id,
                                                    dlcptr->chan_type);
          ...
          if (dlcptr->chan_type != UE_LOGCHAN_TM_DCCH_DL)
          {
            if (dlcptr->rlc_mode == UE_MODE_TRANSPARENT)
            {
              ...
              if ((mac_cipher_info_ptr->cfn_ciphering_table[current_tbs_cfn].cipher_enable) &&
                  ((mac_cipher_info_ptr->cfn_ciphering_table[current_tbs_cfn].cipher_algo == UE_WCDMA_CIPHER_ALGO_UEA1) 
#ifdef FEATURE_WCDMA_REL7
                  ||(mac_cipher_info_ptr->cfn_ciphering_table[current_tbs_cfn].cipher_algo == UE_WCDMA_CIPHER_ALGO_UEA2)
#endif
              ))
              {
                ...
                countc =  ((mac_cipher_info_ptr->cfn_hfn_table[index][current_tbs_cfn] << 8) | current_tbs_cfn);

                /* Setting this flag to TRUE at run time will provide debug messages to
                figure out what is going wrong with ciphering */
                if ((i==0) && (dl_table_ptr->ciphering_garbled &&
                               (current_tbs_cfn % dl_table_ptr->mac_dl_debug_print_interval == 0)) &&
                               (current_tbs_cfn != dl_table_ptr->prev_print_cfn))
                {
                  WL2_MSG_3(HIGH, dl_table_ptr, "DL count_c 0x%x dl_cfn 0x%x key 0x%x",
                                                             countc,
                                                             current_tbs_cfn,
                                                             mac_cipher_info_ptr->cfn_ciphering_table[current_tbs_cfn].key_idx);
                  ...
                  WL2_MSG_3(HIGH, dl_table_ptr, "PDU %x %x %x",
                                                             cipher_seg[0],
                                                             cipher_seg[1],
                                                             cipher_seg[2]);
                ...
                tb_cipher_ptr->ciphering_algo = mac_cipher_info_ptr->cfn_ciphering_table[current_tbs_cfn].cipher_algo;
                tb_cipher_ptr->ciphering_key_id  = mac_cipher_info_ptr->cfn_ciphering_table[current_tbs_cfn].key_idx;
                ...
                dl_table_ptr->prev_print_cfn = current_tbs_cfn;
        ...
        if (status == TRUE)
        {
          ...
          if ((dlcptr->chan_type==UE_LOGCHAN_DTCH) &&
             (((dlcptr->rlc_mode == UE_MODE_TRANSPARENT) &&(ul_table_ptr->loopback_test_mode != MAC_LOOPBACK_NA))
             || (((dlcptr->rlc_mode != UE_MODE_TRANSPARENT))&&(ul_table_ptr->loopback_test_mode == MAC_LOOPBACK_MODE_2))))
          {
            /*---------------------------------------------------------------------------------------
            register the downlink CFN when the transport block was received in the
            downlink. This block would be transmitted in the uplink after applying
            ...
            re-enter it. If cfn_marked is set to TRUE then this cfn exists in the database already
            ---------------------------------------------------------------------------------------*/
            if (dl_table_ptr->cfn_marked == FALSE)
            {
              dl_table_ptr->lb_delay_type.dl_cfn[dl_table_ptr->lb_delay_type.cur_idx] = current_tbs_cfn;
              dl_table_ptr->lb_delay_type.cur_idx++;
              dl_table_ptr->lb_delay_type.cur_idx = dl_table_ptr->lb_delay_type.cur_idx % MAX_NUM_CFN;
...
===============================================================================================*/
log_wcdma_trch_type mac_dl_check_dch_trch(rlc_lc_id_type rlc_id, rb_id_type rb_id, mac_dl_table_type *dl_table_ptr)
{
  log_wcdma_trch_type return_trch_val = WCDMA_TRCH_INVALID;
  int i, j;
  dl_dch_config_info_type *dch_config_info_ptr = NULL;

  for(i = 0; i < dl_table_ptr->num_active_dl_dchs; i++)
  {
    dch_config_info_ptr = &dl_table_ptr->dl_dch_cfg_info[i];
    for(j = 0; j < dch_config_info_ptr->num_dlc; j++)
    {
      if((dch_config_info_ptr->dlcinfo[j].enable == TRUE) &&
         (dch_config_info_ptr->dlcinfo[j].rlc_id == rlc_id) &&
         (dch_config_info_ptr->dlcinfo[j].rb_id == rb_id))
      {
