...
$Header: //components/rel/wcdma.mpss/8.4.0/mac/src/machstask.c#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
rex_timer_type  mac_hs_dl_dog_rpt_timer_blk[L2_NUM_W_SUBS];
  ...
  dog_report_type   wcdma_mac_hs_dog_rpt_var[L2_NUM_W_SUBS]           = {0};      /* Initial Safety */
...
=============================================================================================*/
#ifdef FEATURE_SEGMENT_LOADING
void wcdma_mac_hs_dl_task(sys_modem_as_id_e_type as_id) __attribute__((section(".task_text")));
void wcdma_mac_hs_dl_0_task(dword dummy) __attribute__((section(".task_text")));
void wcdma_mac_hs_dl_1_task(dword dummy) __attribute__((section(".task_text")));
#else
void wcdma_mac_hs_dl_task(sys_modem_as_id_e_type as_id);
#endif /* FEATURE_SEGMENT_LOADING */
void wcdma_mac_hs_dl_0_task(dword dummy)
{
  wcdma_mac_hs_dl_task(SYS_MODEM_AS_ID_1);
}
void wcdma_mac_hs_dl_1_task(dword dummy)
{
#ifdef FEATURE_DUAL_WCDMA
  wcdma_mac_hs_dl_task(SYS_MODEM_AS_ID_2);
#endif
}
	 ...
	 )
{
  ...
  hs_table_ptr = GET_MAC_HS_TABLE_PTR(as_id);
	 ...
	 rex_def_timer(&mac_hs_dl_dog_rpt_timer_blk[as_id], wcdma_mac_hs_dl_get_tcb(hs_table_ptr),
			MAC_HS_DL_DOG_RPT_TIMER_SIG);

	 (void)rex_set_timer(&mac_hs_dl_dog_rpt_timer_blk[as_id], DOG_MAC_HS_DL_RPT_TIME);
#else
	    /*Register with DOG, get the Task ID*/
	 wcdma_mac_hs_dog_rpt_var[as_id] = dog_hb_register_rex((rex_sigs_type)MAC_HS_DL_DOG_RPT_TIMER_SIG);
	
#endif

	 /* Initilize the parameters for DL mac */
	 mac_hs_dl_init(as_id);
	 ...
	 while (1)
	 {
			...
			if (sigs & MAC_HS_DL_DOG_RPT_TIMER_SIG)
			{
				 ...
				 (void)rex_set_timer(&mac_hs_dl_dog_rpt_timer_blk[as_id], DOG_MAC_HS_DL_RPT_TIME);
#else
                          dog_hb_report(wcdma_mac_hs_dog_rpt_var[as_id]);
			...
			if (sigs & MAC_HS_DL_CONFIG_REQ_SIG)
			{
				 ...
				 mac_hs_dl_config_req_handler(hs_table_ptr);
			...
			if (sigs & MAC_HS_DL_DATA_IND_SIG)
			{
				 ...
				 mac_hs_dl_data_handler(hs_table_ptr);
			}
			...
			if (sigs & MAC_HS_RESET_BASE_TIMER_SIG)
			{
				 ...
				 mac_hs_reset_base_timer_handler(hs_table_ptr);
			}
			...
			for (i = 0; i < MAC_HS_DL_MAX_QUEUES; i++)
			{
				 ...
				 if (timer_sig)
				 {
						...
						mac_hs_dl_timer_expiry_handler( i, hs_table_ptr);
				 }
			}
			...
			if (sigs & MAC_HS_DL_STATUS_LOG_TIMER_SIG)
			{
				 ...
				 mac_hs_dl_status_log_timer_expiry_handler(hs_table_ptr);
			}
#endif
	 ...
	 dog_hb_deregister(wcdma_mac_hs_dog_rpt_var[as_id]);
#endif
#endif
}/* mac_hs_dl_task */
