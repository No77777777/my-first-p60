...
$Header: //components/rel/wcdma.mpss/8.4.0/mac/src/mactraffic.c#3 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $

when      who   what, where, why
--------  ---   ----------------------------------------------------------------------------------
01/16/18   sg      Made changes for to log interlayer commands for ML
12/04/17   nk   Added a check to prevent the the overflow of array index
...
rlc_ul_buf_status_type           traffic_ul_buf_status[L2_NUM_W_SUBS];
...
void mac_ul_traffic_volume_init (mac_ul_table_type *ul_table_ptr)
{
  ...
  for (i=0;i<UE_NUM_TRAFFIC_MEAS; i++)
  {
    ...
    -------------------------------------------------------------------------------*/
    traffic_info_ptr->ul_traffic_info_table[i].valid=FALSE;
    traffic_info_ptr->ul_traffic_info_table[i].traffic_cfg_ptr = NULL;
  }
  ...
  ---------------------------------------------------------------------------------*/
  memset(&traffic_ul_buf_status[ul_table_ptr->as_id],0, sizeof(rlc_ul_buf_status_type));

  traffic_info_ptr->tx_interrupt_tmr = 0;
  traffic_info_ptr->tx_interrupt_cnt = 0;
#ifdef FEATURE_WCDMA_DS_WEB_BOOST
  traffic_info_ptr->trig_tvm_params.frc_trig_tvm     = FALSE;
  traffic_info_ptr->trig_tvm_params.is_e4a_cnf       = FALSE;
  traffic_info_ptr->trig_tvm_params.max_prohibit_e4b = MAC_TVM_E4B_PROHIBIT_CNT; 
  traffic_info_ptr->trig_tvm_params.prohibit_e4b_cnt = 0;
  traffic_info_ptr->trig_tvm_params.max_cnf_thrshld  = 0;
#endif  
  traffic_info_ptr->num_event_measurements = 0;
  traffic_info_ptr->num_periodic_measurements = 0;
  traffic_info_ptr->mac_tvm_event_timer_started = FALSE;
  traffic_info_ptr->mac_tvm_periodic_timer_started = FALSE;
  traffic_info_ptr->block_transmission = FALSE;
  ...
  for (i=0;i<MAX_RB;i++)
  {
    ...
    ------------------------------------------------------------------------------*/
    memset(traffic_info_ptr->rb_raw_payload[i],0,TRAFFIC_MAX_SAMPLES*sizeof(uint32));
  }

  // Define the traffic volume timer.
  rex_def_timer(&traffic_info_ptr->mac_ul_tvm_event_tmr_blk,    wcdma_l2_ul_get_tcb(ul_table_ptr->as_id), MAC_UL_TVM_EVENT_TMR_EXP_SIG);
  rex_def_timer(&traffic_info_ptr->mac_ul_tvm_periodic_tmr_blk, wcdma_l2_ul_get_tcb(ul_table_ptr->as_id), MAC_UL_TVM_PERIODIC_TMR_EXP_SIG);

  traffic_info_ptr->prepare_addtl_meas_incl = FALSE;
  traffic_info_ptr->rolled_over = FALSE;
  traffic_info_ptr->cur_index = 0;
  traffic_info_ptr->tvm_debug = 0;
...
void mac_reset_periodic_event_trigger_counters (mac_ul_table_type *ul_table_ptr)
{
  ...
  if ((traffic_info_ptr->num_event_measurements == 0) && (traffic_info_ptr->num_periodic_measurements == 0))
  {
    return;
  }
  ...
  for (i=1;i<=MAC_UL_MAX_MEAS_ID; i++)
  {
    ...
    if (int_cfg_param_ptr->valid)
    {
      ...
      if (int_cfg_param_ptr->traffic_cfg_ptr != NULL)
      {
        ...
        {
          WL2_MSG_2(ERROR, ul_table_ptr, "NumTrch %d > %d",
          ...
          if (traffic_cfg_ptr->report_mode == EVENT_TRIGGERED_MODE)
          {
            ...
            if (traffic_cfg_ptr->event_trch_incl)
            {
                  ...
                  && traffic_cfg_ptr->event_num_trch < UE_MAX_TRCH;j++)
              {
                ...
                {
                  WL2_MSG_2(HIGH, ul_table_ptr, "Number of events %d configured for TrchId %d",
...
)
{
  ...
  {
    WL2_MSG_0(ERROR, mac_ul_table_ptr , "Could not get RRC cmd buffer");
  }
...
)
{
  ...
  {
    WL2_MSG_0(HIGH, mac_ul_table_ptr , "Sending RRC_CMAC_UL_TRAFFIC_IND to RRC");
...
)
{
  ...
  if (oc_traffic_cfg_ptr->meas_report_incl)
  {
        ...
        oc_traffic_cfg_ptr->meas_report_qty.var_payload)
    {
      ...
      {
        status_ind=FALSE;
        WL2_MSG_0(ERROR, ul_table_ptr, "Measurement interval not specified");
      }
      else
      {
        ...
        {
          status_ind=FALSE;
          WL2_MSG_0(ERROR, ul_table_ptr, "Measurement Interval not valid");
        }
      }
    }
    /*------------------------------------------------------------------------
    ...
    else if (!oc_traffic_cfg_ptr->meas_report_qty.rlc_payload)
    {
      ...
      WL2_MSG_0(HIGH, ul_table_ptr, "Report quantity not specified");
    }
  }
...
#endif  
  {
    ...
    for (i=0;i<MAX_TRCH;i++)
    {
      ...
      if (id_to_idx_ptr->valid)
      {
        ...
        {
          WL2_MSG_1(ERROR, ul_table_ptr, "MAC_TVM_ERR: trch_info array out of bounds - j %d",
                                                      j);
          break;
        }
      ...
      if (int_cfg_param_ptr->traffic_cfg_ptr != NULL)
      {
        ...
        else
        {
          ...
          for (j=0; j<traffic_cfg_ptr->num_trch ; j++)
          {
            ...
            {
              traffic_cfg_ptr->trch_info[j].trch_id = ul_table_ptr->int_ul_trch_info[0].trch_id;
          ...
          if (traffic_cfg_ptr->report_mode == EVENT_TRIGGERED_MODE)
          {
            ...
            {
              WL2_MSG_2(ERROR, ul_table_ptr, "Event NumTrch %d > %d",
                                                          traffic_cfg_ptr->event_num_trch,
                                                          UE_MAX_TRCH);
            ...
            else
            {
              for (j=0;j<traffic_cfg_ptr->event_num_trch;j++)
              {
                ...
                {
                  traffic_cfg_ptr->event_trch_info[j].trch_id = ul_table_ptr->int_ul_trch_info[0].trch_id;
                  // There can be only one RACH, hence break after finding the first one.
                  break;
                }
...
)
{
  ...
  {
    WL2_MSG_1(ERROR, ul_table_ptr, "Currently support %d trch's",
                                                UE_MAX_TRCH);
    status=FALSE;
  }
  else
  {
    ...
    for (i=0;i<ntrch;i++)
    {
          ...
          (oc_trch_event_param_ptr->num_events > MAX_NUM_EVENTS))
      {
        ...
        ---------------------------------------------------------------------------*/
        status=FALSE;
        WL2_MSG_0(ERROR, ul_table_ptr, "Number of events can't be 0");
        break;
      }
    ...
    if (status)
    {
...
#ifdef FEATURE_WCDMA_DS_WEB_BOOST
      ul_table_ptr->traffic_info.trig_tvm_params.is_e4a_cnf      = FALSE;
      ul_table_ptr->traffic_info.trig_tvm_params.max_cnf_thrshld = 0;
#endif
      if (event_trch_incl)
      {
        /*---------------------------------------------------------------------------
      ...
      for (i=0;i<ntrch;i++)
      {
        ...
        for (j=0;j<int_tmp_cfg_ptr->num_events[i];j++)
        {
          ...
          {
            WL2_MSG_1(ERROR, ul_table_ptr, "Event ID not specified for trch %d",
                                                        i);
          }
          /*----------------------------------------------------------------------
      ...
      for (i=0;i<ntrch;i++)
      {
        ...
        for (k=0;k<int_tmp_cfg_ptr->event_num_trch;k++)
        {
               ...
               (int_tmp_cfg_ptr->event_trch_info[k].trch_type == oc_trch_event_param_ptr->trch_info.trch_type)))
          {
            ...
            WL2_MSG_2(HIGH, ul_table_ptr, "TVM:Replacing with modify MCM events TR_CH Id = %d,cctrch_type = %d ",
            ...
            for (j=0;j<oc_trch_event_param_ptr->num_events;j++)
            {
              ...
              for (l=0;l<int_tmp_cfg_ptr->num_events[k];l++)
              {
                    ...
                    oc_trch_event_param_ptr->event_param[j].event_id )
                {
                  ...
                  // modified.
                  WL2_MSG_3(HIGH, ul_table_ptr, "TVM:Modify num_events %d,evt %d, trch_id %d",
        ...
        if ((found_trch == FALSE) && (k < UE_MAX_TRCH))
        {
          ...
          for (j=0;j<oc_trch_event_param_ptr->num_events;j++)
          {
            WL2_MSG_1(HIGH, ul_table_ptr, "TVM:Appending Modify num_events %d",
                                                       oc_trch_event_param_ptr->num_events);
...
)
{
  ...
  if (oc_periodic_cfg_ptr->report_interval/10 < 2)
  {
    ...
    WL2_MSG_0(ERROR, ul_table_ptr, "Report interval not valid");
    status_ind = FALSE;
  }
...
)
{
  ...
  {
    status_ind=FALSE;
    WL2_MSG_1(ERROR, ul_table_ptr, "Invalid measurement ID %d",
                                                meas_id);
  }
  ...
  {
    WL2_MSG_0(HIGH, ul_table_ptr, "MAC UL transport channels not configured");
    status_ind=FALSE;
  }
  else
  {
    ...
    if (int_cfg_param_ptr->traffic_cfg_ptr != NULL)
    {
      WL2_MSG_1(HIGH, ul_table_ptr, "Another setup command recvd for meas_id %d",
                                                 meas_id);
      int_tmp_cfg_ptr = int_cfg_param_ptr->traffic_cfg_ptr;
      
      if ((int_tmp_cfg_ptr->report_mode == EVENT_TRIGGERED_MODE) && (--traffic_info_ptr->num_event_measurements == 0))
      {
        (void)rex_clr_timer(&traffic_info_ptr->mac_ul_tvm_event_tmr_blk);
         traffic_info_ptr->mac_tvm_event_timer_started = FALSE;
         WL2_MSG_1(HIGH, ul_table_ptr, "Num_event_meas = %d",
                                                    traffic_info_ptr->num_event_measurements);
      }
      else if ((int_tmp_cfg_ptr->report_mode == PERIODIC_MODE) && (--traffic_info_ptr->num_periodic_measurements == 0))
      {
        (void)rex_clr_timer(&traffic_info_ptr->mac_ul_tvm_periodic_tmr_blk);
        traffic_info_ptr->mac_tvm_periodic_timer_started = FALSE;
        WL2_MSG_1(HIGH, ul_table_ptr, "num_periodic_measurements = %d",
                                                   traffic_info_ptr->num_periodic_measurements);
      }
    ...
    else
    {
      ...
      {
        status_ind = FALSE;
        WL2_MSG_0(ERROR, ul_table_ptr, "Failed to allocate memory");
      }
      else
      {
        memset(int_tmp_cfg_ptr,0x00, sizeof(maci_ul_traffic_cfg_param));
    ...
    if (status_ind)
    {
      if (oc_traffic_cfg_ptr->meas_obj_incl)
      {
        ...
        for (i=0;i<int_tmp_cfg_ptr->num_trch && i < UE_MAX_TRCH;i++) /*lint !e613*/
        {
          ...
          {
            int_tmp_cfg_ptr->trch_info[i].trch_id = ul_table_ptr->int_ul_trch_info[0].trch_id; /*lint !e613*/
          }
        }
      ...
      else
      {
        ...
        ----------------------------------------------------------------------------*/
        int_tmp_cfg_ptr->num_trch= populate_object_list(int_tmp_cfg_ptr->trch_info, ul_table_ptr, eul_table_ptr); /*lint !e613*/
      }
      ...
      if (status_ind && check_meas_report_qty(oc_traffic_cfg_ptr, ul_table_ptr))
      {
        /*-----------------------------------------------------------------------
        ...
        if (report_mode==PERIODIC_MODE)
        {
          ...
          ---------------------------------------------------------------------*/
          status_ind= check_n_copy_periodic_params(int_tmp_cfg_ptr, oc_traffic_cfg_ptr, ul_table_ptr);
        }
        else if (report_mode==EVENT_TRIGGERED_MODE)
        {
          ...
          status_ind = check_n_copy_event_trigger_params(int_tmp_cfg_ptr,
                       &oc_traffic_cfg_ptr->meas_report_criteria.event_cfg, ul_table_ptr);
        }
      }
      /*------------------------------------------------------------------------
...
#endif
        WL2_MSG_0(ERROR, ul_table_ptr, "Setup configuration error");
      }
      else
      {
        /*----------------------------------------------------------------------
        ...
        if (oc_traffic_cfg_ptr->meas_start_incl)
        {
          ...
          if ((traffic_info_ptr->num_event_measurements > 0) && (!traffic_info_ptr->mac_tvm_event_timer_started))
          {
            (void)rex_set_timer(&traffic_info_ptr->mac_ul_tvm_event_tmr_blk,(uint32)MAC_UL_TRAFFIC_VOLUME_TMR_BASE);
            traffic_info_ptr->mac_tvm_event_timer_started = TRUE;
            WL2_MSG_1(HIGH, ul_table_ptr, "Starting mac_ul_tvm_event_tmr_blk timer %d ms",
                                                       MAC_UL_TRAFFIC_VOLUME_TMR_BASE);
          }
          if ((traffic_info_ptr->num_periodic_measurements > 0) && (!traffic_info_ptr->mac_tvm_periodic_timer_started))
          {
            (void)rex_set_timer(&traffic_info_ptr->mac_ul_tvm_periodic_tmr_blk,
                   (uint32)(MAC_UL_TRAFFIC_VOLUME_TMR_BASE * MAC_UL_PERIODIC_TVM_TIMER_SCALE));
            traffic_info_ptr->mac_tvm_periodic_timer_started = TRUE;
            WL2_MSG_1(HIGH, ul_table_ptr, "Starting mac_ul_tvm_periodic_tmr_blk timer %d ms",
                                                       (MAC_UL_TRAFFIC_VOLUME_TMR_BASE*MAC_UL_PERIODIC_TVM_TIMER_SCALE));
          }
          
          WL2_MSG_2(HIGH, ul_table_ptr, "Starting measurements num_event_meas %d, num_periodic_meas %d",
                                                     traffic_info_ptr->num_event_measurements,
                                                     traffic_info_ptr->num_periodic_measurements);
        }
        else
        {
          ...
          traffic_info_ptr->mac_tvm_event_timer_started = FALSE;
          traffic_info_ptr->mac_tvm_periodic_timer_started = FALSE;
          WL2_MSG_0(HIGH, ul_table_ptr, "Setup recvd but suspended for now ");
        }
				 ...
				 )
{
  ...
  uint32                      pdu_size, total_data_bits, uncomp_data, nbytes;
  uint16                      npdus = 0;
  uint16                      hdr_len;
#ifdef FEATURE_WCDMA_HS_RACH
  uint8                       rlc_hdr_len;
  ...
  mac_traffic_info_type *traffic_info_ptr;
  rlc_ul_buf_status_type *buf_status_ptr;
#ifdef FEATURE_WCDMA_DS_WEB_BOOST
  boolean                     is_first_channel = TRUE;
#endif
  data_table_ptr = GET_MAC_RLC_DATA_TABLE_PTR(ul_table_ptr->as_id);
  tfcs_info_ptr = &ul_table_ptr->tfcs_info;
  traffic_info_ptr = &ul_table_ptr->traffic_info;
  buf_status_ptr = &traffic_ul_buf_status[ul_table_ptr->as_id];
  
  /* Clear the previous buffer status */
  memset(buf_status_ptr,0, sizeof(rlc_ul_buf_status_type));

#ifdef FEATURE_WCDMA_HS_RACH
  /* HS-RACH active - Only single EDCH channel can be setup */
  if(eul_table_ptr->mac_hs_rach_info.hs_rach_active == TRUE)
  {
    lc_info_ptr = get_mac_hs_ul_lc_info_ptr(eul_table_ptr);
    hs_lc_macd_flow_map = get_mac_hs_logch_macd_flow_mapping_ptr(eul_table_ptr);

    if(lc_info_ptr->nchan > 0)
    {
      //Mutex Protection required ??
      rlc_ul_enh_buffer_status(lc_info_ptr, buf_status_ptr, TVM_QUERY, NULL, ul_table_ptr);
    } 
    ...
    for(i=0; i<lc_info_ptr->nchan; i++)
    {
      ...
      /*-------------------------------------------------------------------------
        ...
        --------------------------------------------------------------------------*/
      lc_buf_ptr = &buf_status_ptr->chan_info[rlc_id];

      if(hs_lc_macd_flow_map[rlc_id].logical_ch_enabled == FALSE)
      {
        WL2_MSG_1(ERROR, ul_table_ptr, "MAC_ERR: Logical Channel with RLC ID %d not enabled",
                                                    rlc_id);
        continue;
      }
      ...
      switch (lc_buf_ptr->type)
      {
        ...
        case FLEX_PDUS:
          ...
          if ((lc_buf_ptr->new_data_bytes) && (pdu_size > 0))
          {
            ...
            if(traffic_info_ptr->tvm_debug == 1)
            {
              WL2_MSG_7(HIGH, ul_table_ptr,
                ...
                lc_buf_ptr->ctrl_data_bytes,npdus, pdu_size, hdr_len );
            }
            
          }
	  nbytes = lc_buf_ptr->nbytes;
          #ifdef FEATURE_WCDMA_DS_WEB_BOOST
	  if(TRUE == MAC_TVM_UPDATE_TRAFFIC_THRESHOLD(traffic_info_ptr->trig_tvm_params.frc_trig_tvm,
	  	                                      is_first_channel,
	  	                                      hs_lc_macd_flow_map[rlc_id].rlc_mode,
	  	                                      hs_lc_macd_flow_map[rlc_id].chan_type))
	  {
            nbytes             = MAX(nbytes, traffic_info_ptr->trig_tvm_params.max_cnf_thrshld);
	    is_first_channel   = FALSE;
            lc_buf_ptr->nbytes = nbytes;
	  }
	  #endif
          
          traffic_info_ptr->rb_raw_payload[hs_lc_macd_flow_map[rlc_id].rb_id][traffic_info_ptr->cur_index] = nbytes;
          
          if (nbytes > 0)
          {
            WL2_MSG_2(HIGH, ul_table_ptr, "HS_RACH_TVM: rlc_id %d, buf_status %d", rlc_id, nbytes);
          }
          break;
          
        default:
        WL2_MSG_0(LOW, ul_table_ptr, "Traffic volume not computed for TM pdus");
        break;
      } /* switch pdu type */
    }
  }
...
#endif
 {
  if (data_table_ptr->mac_ul_lc_info.nchan > 0 )
  {
    REX_ISR_LOCK(&(rlc_mutexes[ul_table_ptr->as_id].r99_ul_isr_l2_datapath_mutex));
    rlc_ul_enh_buffer_status(&data_table_ptr->mac_ul_lc_info,buf_status_ptr, TVM_QUERY, NULL, ul_table_ptr);
          ...
          j < UE_MAX_UL_LOGICAL_CHANNEL ;j++)
      {
        ...
        if (lcinfo_ptr->enable)
        {
          ...
          case FLEX_PDUS:
            ...
            {
              pdu_size = tfcs_info_ptr->trch_min_tb_size[id_to_idx_ptr->idx];
              hdr_len = 1; // number of bytes of header
            }
            else
            {
	    ...
	    if(TRUE == MAC_TVM_UPDATE_TRAFFIC_THRESHOLD(traffic_info_ptr->trig_tvm_params.frc_trig_tvm,
	      ...
	      lc_buf_ptr->nbytes = nbytes;    
            ...
            traffic_info_ptr->rb_raw_payload[lcinfo_ptr->rb_id][traffic_info_ptr->cur_index] = nbytes;
            if (nbytes > 0)
            {
              WL2_MSG_2(HIGH, ul_table_ptr, "rlc_id %d, buf_status %d", rlc_id, nbytes);
            }
            break;
  ...
  #ifdef FEATURE_WCDMA_DS_WEB_BOOST
  if((TRUE == traffic_info_ptr->trig_tvm_params.frc_trig_tvm) &&
     (TRUE == is_first_channel))
  {
    traffic_info_ptr->trig_tvm_params.frc_trig_tvm = FALSE;
    WL2_MSG_0(HIGH, ul_table_ptr, "MAC_FRC_TVM: No DTCH channels present in UM or AM. Reset force trig");    
  }
  #endif
  traffic_info_ptr->cur_index++;
  if (traffic_info_ptr->cur_index >= TRAFFIC_MAX_SAMPLES)
  {
    traffic_info_ptr->rolled_over = TRUE;
  }
  traffic_info_ptr->cur_index = traffic_info_ptr->cur_index%TRAFFIC_MAX_SAMPLES;
}
                               ...
                               mac_ul_table_type           *ul_table_ptr )
{
  ...
  if(trch_index == 0xFF)
  {
    WL2_MSG_0(ERROR, ul_table_ptr, "TVM_ERR: No EDCH Trch idex found in TVM conifg params" );
  }
  else
  {
    if(ul_table_ptr->traffic_info.tvm_debug == 1)
    {
      WL2_MSG_5(HIGH, ul_table_ptr,
...
)
{
  ...
  {
    WL2_MSG_0(ERROR, ul_table_ptr, "MAC_TVM_ERR: oc_hs_rachptr is NULL");
  }
  else
  {
    ...
    if(hs_rach_config_ptr != NULL)
    {
      ...
      for(j=0; j<hs_rach_config_ptr->num_e_mac_d_flow; j++)
      {
        ...
        {
          WL2_MSG_1(ERROR, ul_table_ptr, "MAC_TVM_ERR: e-mac-d flow pointer NULL %d",
                                                      j);
        }
      }
    }
    ...
    {
      WL2_MSG_0(ERROR, ul_table_ptr, "MAC_TVM_ERR: hs-rach config pointer NULL ");
    }
  }
  return nchan;
}
...
)
{
  ...
  {
    int_mac_ul_lc_info.nchan = populate_hs_rach_log_chan_info(oc_hs_rachptr, int_mac_ul_lc_info.rlc_id, ul_table_ptr);  
  }
  else 
#endif
  {  
  ...
  if (int_mac_ul_lc_info.nchan > 0 )
  {
    ...
    rlc_ul_enh_buffer_status(&int_mac_ul_lc_info, int_traffic_ul_buf_status_ptr, TVM_QUERY, NULL, ul_table_ptr);
    ...
    for (j = 0; ((j < int_mac_ul_lc_info.nchan) && (j < UE_MAX_UL_LOGICAL_CHANNEL)); j++)
    {
      ...
      switch(lc_buf_ptr->type)
      {
        ...
        if ((lc_buf_ptr->new_data_bytes) && (pdu_size > 0))
        {
          ...
          rlc_id, lc_buf_ptr->new_data_bytes, lc_buf_ptr->retx_data_bytes,
          lc_buf_ptr->ctrl_data_bytes, npdus, pdu_size, hdr_len_bytes );
          
        }

        WL2_MSG_2(HIGH, ul_table_ptr, "MAC_TVM: rlc_id %d, buf_status %d",
  ...
  if (id_to_idx_ptr->valid)
  {
    for (j = 0; ((j < oc_rachptr->ndlchan) && (i < MAX_DED_LOGCHAN_PER_TRCH)); j++)
    {
      ...
      switch (lc_buf_ptr->type)
      {
        ...
        {
          WL2_MSG_2(HIGH, ul_table_ptr, "TVM:rlc_id %d, buf_status %d",
                                         rlc_id, lc_buf_ptr->nbytes);
        }
        break;
  ...
  {
    WL2_MSG_1(ERROR, ul_table_ptr, "MAC_ERR: Invalid RACH TrCh Id %d in MAC data base",
                                                oc_rachptr->trch_id);
  }
}
}
...
)
{
  ...
  else
  {
    ...
    {
      WL2_MSG_0(ERROR, ul_table_ptr, "Entry with invalid parameters in TVM info table");
      return FALSE;
    }  
  ...
  for (i = 0; ((i < ntrch) && (i < UE_MAX_TRCH) && (traffic_param_ptr!=NULL)); i++)
  {
    ...
    if(hs_rach_active == TRUE)
    {
      ...
      if(trch_type == EDCH)
      {
        ...
        ndlchn = populate_hs_rach_log_chan_info(oc_hs_rachptr, hs_rach_rlc_id, ul_table_ptr);
        if(ul_table_ptr->traffic_info.tvm_debug == 1)
        {
          WL2_MSG_2(HIGH, ul_table_ptr, "MAC_TVM: LCs mapped on EDCH %d, trch idx %d",
                                                     ndlchn, i);
        }
      }
      else
      { /* do nothing - ndlchn is zero */
        WL2_MSG_2(LOW, ul_table_ptr, "MAC_TVM: HS_RACH - Invalid TrCh type - %d, index - %d",
                                                  trch_type, i);
      }
    }
    else
   ...
   for (j = 0; (j < nevents) && (j < MAX_NUM_EVENTS); j++)
   {
     ...
     if (event_id == EVENT_4A)
     {
       ...
       WL2_MSG_3(HIGH, ul_table_ptr, "MAC_TVM: TrCh_BO %d, report_threshold %d, Trch_bo_crossed_threshold %d",
                                                  trch_bo,
...
)
{
  ...
  if (meas_id == 4)
  {
    ...
    {
      WL2_MSG_0(ERROR, ul_table_ptr, "MAC UL transport channels not configured");
      return ret_value;
    }     
  ...
  #endif
        oc_rachptr, 
        &int_traffic_ul_buf_status, ul_table_ptr, eul_table_ptr
    );
...
)
{
  ...
  if (!TRCH_ID_TO_IDX_IS_INVALID(trch_id))
  {
    ...
    if (!id_to_idx_ptr->valid)
    {
      WL2_MSG_1(LOW, ul_table_ptr, "Transport channel invalid, trch_id %d",
                                                trch_id);
    }
    else
    {
          ...
          nlchan < UE_MAX_UL_LOGICAL_CHANNEL; nlchan++)
      {
        ...
        {
          lc_buf_ptr = &traffic_ul_buf_status[ul_table_ptr->as_id].chan_info[rlc_id];
...
)
{
  ...
  mac_traffic_info_type            *traffic_info_ptr = NULL;
#ifdef FEATURE_WCDMA_HS_RACH
  rlc_ul_lc_info_type         *lc_info_prt = NULL;
  mac_hs_ul_logical_ch_mac_d_flow_info_type *hs_lc_macd_flow_map = NULL;
#endif
  traffic_info_ptr = &ul_table_ptr->traffic_info;
  memset (&traffic_info_ptr->meas_ind,0,sizeof(mac_ul_traffic_meas_ind_type));
  ...
  else
  {
    ...
    traffic_info_ptr->meas_ind.meas_id = meas_id;
    /*--------------------------------------------------------------------------------
    Copy the report mode. This can be either periodic or event triggered.
    --------------------------------------------------------------------------------*/
    traffic_info_ptr->meas_ind.report_mode = int_traffic_param_ptr->report_mode;
  
    /*--------------------------------------------------------------------------------
    Copy the report transfer mode. This can be either unacknowledged or acknowledged
    --------------------------------------------------------------------------------*/
    traffic_info_ptr->meas_ind.report_transfer_mode = int_traffic_param_ptr->transfer_mode;
  
    /*--------------------------------------------------------------------------------
    Initialize number of rb's to 0.
    --------------------------------------------------------------------------------*/
    traffic_info_ptr->meas_ind.num_rbs = 0;
    ...
    --------------------------------------------------------------------------------*/
    traffic_info_ptr->meas_ind.num_reports_ind = FALSE;
    ...
    {
      if (traffic_info_ptr->rolled_over || (int_traffic_param_ptr->current_idx>=
    ...
    for (i=0; (i < ntrch) && (i < UE_MAX_TRCH)  ; i++)
    {
      ...
      if(eul_table_ptr->mac_hs_rach_info.hs_rach_active == TRUE)
      {
        ...
        {
          lc_info_prt = get_mac_hs_ul_lc_info_ptr(eul_table_ptr);
      ...
      for (j=0;j< nlchan && j < UE_MAX_UL_LOGICAL_CHANNEL;j++)
      {
...
#endif
        {
          if(NULL == trch_info_ptr)
          {
            WL2_MSG_0(ERROR, ul_table_ptr, "NULL trch_info_ptr");
            return;
          }
        ...
        if (lc_enabled)
        {
...
#endif
          {
            if(NULL == lcinfo_ptr)
            {
              WL2_MSG_0(ERROR, ul_table_ptr, "NULL lcinfo_ptr");
              return;
            }
          ...
          if ((rb_id == 0) || (rlc_mode == UE_MODE_TRANSPARENT))
          {
            continue;
          }
          ...
          computations.
          ------------------------------------------------------------------------*/
          rb_idx = traffic_info_ptr->meas_ind.num_rbs;
              ...
              || int_traffic_param_ptr->var_payload)
          {
            if (rb_idx < MAX_RB)
            {
              traffic_info_ptr->meas_ind.rb_meas[rb_idx].rb_id = rb_id;
            }
            else
            {
              WL2_MSG_1(ERROR, ul_table_ptr, "num of RBs %d exceeded max rb_meas bound",
                                                          traffic_info_ptr->meas_ind.num_rbs);
            }
          ...
          if (rb_idx < MAX_RB)
          {
            traffic_info_ptr->meas_ind.rb_meas[rb_idx].raw_rlc_payload = 0xFFFFFFFF;
            traffic_info_ptr->meas_ind.rb_meas[rb_idx].variance_rlc_payload = 0xFFFFFFFF;
            traffic_info_ptr->meas_ind.rb_meas[rb_idx].average_rlc_payload  = 0xFFFFFFFF;
          }
          else
          {
            WL2_MSG_2(ERROR, ul_table_ptr, "MAC_ERR: num of RBs %d exceeded max rb_meas bound, rb_idx %d",
                                                        traffic_info_ptr->meas_ind.num_rbs,
                                                        rb_idx);
          }
          /*-----------------------------------------------------------------------
          ...
          if (int_traffic_param_ptr->rlc_payload)
          {
            ...
            {
              traffic_info_ptr->meas_ind.rb_meas[rb_idx].raw_rlc_payload = traffic_info_ptr->rb_raw_payload[rb_id][idx];
              WL2_MSG_2(LOW, ul_table_ptr, "RB_ID RAW_PAYLOAD %d %d",
                                                        rb_id,
                                                        traffic_info_ptr->rb_raw_payload[rb_id][idx]);
            }
          }
	    ...
	    if(TRUE == traffic_info_ptr->trig_tvm_params.frc_trig_tvm)
	    {
	      nsamples = 1;
            ...
            if (nsamples==0)
            {
              WL2_MSG_0(LOW, ul_table_ptr, "Interval is zero");
            }
            else
            {
              /*-------------------------------------------------------------------
                    ...
                    cnt++,fr_idx++)
              {
                ...
                raw_payload = traffic_info_ptr->rb_raw_payload[rb_id][fr_idx];
                sum += raw_payload;
                WL2_MSG_2(LOW, ul_table_ptr, "RAW PAYLOAD %d RB_ID %d",
                                                          raw_payload, rb_id);
              ...
              if ((int_traffic_param_ptr->var_payload) && (rb_idx < MAX_RB))
              {
                ...
                traffic_info_ptr->meas_ind.rb_meas[rb_idx].variance_rlc_payload = var;
                WL2_MSG_2(LOW, ul_table_ptr, "RB_ID VAR_PAYLOAD %d %d",
                                                          rb_id, var);
              }
              /*----------------------------------------------------------------------
              Compute average.
              ...
              {
                traffic_info_ptr->meas_ind.rb_meas[rb_idx].average_rlc_payload = avg;
                WL2_MSG_2(LOW, ul_table_ptr, "RB_ID AVG_PAYLOAD %d %d",
                                                          rb_id, avg);
              }
            }/* nsamples>0*/
    ...
    if (traffic_info_ptr->prepare_addtl_meas_incl == TRUE)
    {
      traffic_info_ptr->prepare_addtl_meas_incl = FALSE;
      return;
    }
    if (int_traffic_param_ptr->report_mode==PERIODIC_MODE)
    {
      ...
      /*if (meas_ind.num_rbs>0) */
      {
        ...
        {
          traffic_info_ptr->meas_ind.num_reports_ind = TRUE;
        }
        /*----------------------------------------------------------------------------
        Post the measurement results to RRC.
        -----------------------------------------------------------------------------*/
        maci_post_traffic_volume_meas_report(ul_table_ptr->as_id, &traffic_info_ptr->meas_ind);
        mac_ul_send_tvm_report_packet(&traffic_info_ptr->meas_ind, int_traffic_param_ptr->nreports_sent, ul_table_ptr);
      }
    }
    else if (int_traffic_param_ptr->report_mode == EVENT_TRIGGERED_MODE) /*event_triggered_mode */
    {
      /*if (meas_ind.num_rbs>0)*/
      {
        WCDMA_MEMCPY(&traffic_info_ptr->meas_ind.event_result, 
        ...
        mac_ul_send_tvm_report_packet(&traffic_info_ptr->meas_ind, 0, ul_table_ptr);
    ...
    {
      traffic_info_ptr->trig_tvm_params.frc_trig_tvm     = FALSE;
      traffic_info_ptr->trig_tvm_params.prohibit_e4b_cnt = traffic_info_ptr->trig_tvm_params.max_prohibit_e4b;
      WL2_MSG_1(HIGH,ul_table_ptr,"MAC_FRC_TVM: MRM due to Force Trig. e4b_prohibit_cnt - %d",
                                   traffic_info_ptr->trig_tvm_params.prohibit_e4b_cnt);
    }
    #endif
  }
...
)
{
  ...
  else
  {
    ...
    {
      status = TRUE;
      ul_table_ptr->traffic_info.prepare_addtl_meas_incl = TRUE;
      prepare_n_send_traffic_meas_report(meas_id, ul_table_ptr, eul_table_ptr);

      WCDMA_MEMCPY(addtl_meas_ind, 
                   sizeof(mac_ul_traffic_meas_ind_type),
                   &ul_table_ptr->traffic_info.meas_ind, 
                   sizeof(mac_ul_traffic_meas_ind_type));
    }
  }
...
)
{
  ...
  mac_ul_traffic_event_id_e_type           event_id;
  boolean                                  trch_bo_crossed_threshold=FALSE;
  boolean                                  prev_bo_below_threshold  = FALSE;
  boolean                                  mrab_send_status;
  mac_traffic_info_type *traffic_info_ptr;
#ifdef FEATURE_WCDMA_DS_WEB_BOOST  
  mac_frc_trig_tvm_type                   *mac_frc_trig_ptr;
#endif
  ...
  traffic_param_ptr = traffic_cfg_idx_ptr->traffic_cfg_ptr;
#ifdef FEATURE_WCDMA_DS_WEB_BOOST
  mac_frc_trig_ptr   = &(traffic_info_ptr->trig_tvm_params);
#endif
  ...
  for (j=0;j< nevents && j < MAX_NUM_EVENTS;j++)
  {
...
#endif /* FEATURE_WCDMA_MRAB_TVM_MAC_OPT */
    {
      ...
      if (event_cfg_ptr[j].prohibit_tmr_cnt)
      {
                            ...
                            event_cfg_ptr[j].trigger_prohibit_tmr))
        {
          ...
          WL2_MSG_2(HIGH, ul_table_ptr, "PendingTmr exp'd evtid %d trid %d",
                                                     event_id,
                                                     (uint32)trch_id);
    ...
    if(traffic_info_ptr->tvm_debug == 1)
    {
      WL2_MSG_8(HIGH, ul_table_ptr,
        "HS_RACH_TVM: j %d, event_id %d, prohibit_tmr_cnt %d, trigger_prohibit_tmr %d, send_status %d, prohibit_status %d, not_1st_meas %d, event_id %d ",
         ...
         send_status,prohibit_status,  cur_evt_cfg_ptr[j].evt_not_first_meas, event_id );
    }
    ...
    ----------------------------------------------------------------------------*/
    prev_bo_below_threshold = (((event_id == EVENT_4A) && (cur_evt_cfg_ptr[j].prev_bo <= event_cfg_ptr[j].report_threshold)) ||
                                  ((event_id==EVENT_4B)&&( cur_evt_cfg_ptr[j].prev_bo >= event_cfg_ptr[j].report_threshold )));
    
    #ifdef FEATURE_WCDMA_DS_WEB_BOOST
    if ((EVENT_4B == event_id)                    &&
	(mac_frc_trig_ptr->prohibit_e4b_cnt > 0)
       )
    {
      prohibit_status = TRUE;
      mac_frc_trig_ptr->prohibit_e4b_cnt--;
      if (mac_frc_trig_ptr->prohibit_e4b_cnt == 0)
      {
        WL2_MSG_0(HIGH, ul_table_ptr, "MAC_FRC_TVM: e4b prohibit expired after force e4a was triggerd");  
      }
    }	
    #endif
    
    /* Make sure the threashold crosssed - Next Event can be triggered */
    if((FALSE == trch_bo_crossed_threshold) || (!prohibit_status))
    {
      cur_evt_cfg_ptr[j].evt_not_first_meas = FALSE;
      if(traffic_info_ptr->tvm_debug == 1)
      {
        WL2_MSG_3(HIGH, ul_table_ptr, "MAC_TVM: Next measurement can be triggered- Event id %d , Bo %d, prohibit_status %d",
                                                   event_id,
                                                   trch_bo,
                                                   prohibit_status);
      }
    ...
    if(cur_evt_cfg_ptr[j].first_event_trch_id != trch_id)
    {
      if(traffic_info_ptr->tvm_debug == 1)
      {
        WL2_MSG_2(HIGH, ul_table_ptr, "Other Trch event triggered trch_id %d, prev_trch_id %d ",
                                                   trch_id,
    ...
    if (send_status)
    {
      ...
      if (trch_bo_crossed_threshold)
      {
         ...
         time to trigger.
         ----------------------------------------------------------------------------*/
         if (prev_bo_below_threshold)
         {
           if(traffic_info_ptr->tvm_debug == 1)
          {
             WL2_MSG_2(HIGH, ul_table_ptr, "TVM_DBG: TrCh BO crossed threshold %d, prev_bo_crossed %d, trigger %d",
                                            trch_bo_crossed_threshold,
                                            prev_bo_below_threshold);
          }
           /*-------------------------------------------------------------------------
           ...
           else if (event_cfg_ptr[j].time_to_trigger==0)
           {
             ...
             //cur_evt_cfg_ptr[j].first_event_trch_id = trch_id;
             WL2_MSG_3(HIGH, ul_table_ptr, "Prev_bo = %d, cur_bo=%d, trch_id = %d",
         ...
         ----------------------------------------------------------------------------*/
                   ...
                   ((++cur_evt_cfg_ptr[j].trigger_tmr_cnt * 10)>event_cfg_ptr[j].time_to_trigger))
          {
            ...
            cur_evt_cfg_ptr[j].evt_not_first_meas = TRUE;
            //cur_evt_cfg_ptr[j].first_event_trch_id = trch_id;
            WL2_MSG_3(HIGH, ul_table_ptr, "TimeToTrigger exp'd evtid %d trid %d, probhibit_status %d",
                                                       event_id,
                                                       (uint32)trch_id,
                                                       prohibit_status);
          ...
          else if (cur_evt_cfg_ptr[j].trigger_tmr_cnt == 0)
          {
            ...
            {
              cur_evt_cfg_ptr[j].trigger_tmr_cnt++;
              WL2_MSG_3(HIGH, ul_table_ptr, "Evt_First_Meas/PendingTmrExpd: TTT started, %d, %d, %d",
            ...
            {
              WL2_MSG_3(HIGH, ul_table_ptr, "Evt_First_Meas : trch_bo %d, trch_id %d, prohibit_status %d",
                                                         trch_bo,
                                                         trch_id,
                                                         prohibit_status);
              send_status = TRUE;
        ...
        else if (cur_evt_cfg_ptr[j].trigger_tmr_cnt > 0)
        {
          ...
          cur_evt_cfg_ptr[j].trigger_tmr_cnt=0;
          WL2_MSG_3(HIGH, ul_table_ptr, "TimeToTrigger stopped evtid %d trchid %d, prohibit_status %d",
                                                     event_id,
                                                     (uint32)trch_id,
                                                     prohibit_status);
          /* Don't send */
    ...
    {
        WL2_MSG_8(HIGH, ul_table_ptr,
          "trch_bo_cros_thres %d, prev_bo_below_thresh %d,threshold %d, prev_bo %d, ttt %d, trig_tmr_cnt %d, prohibit_status %d, first_meas %d",
           trch_bo_crossed_threshold, prev_bo_below_threshold, event_cfg_ptr[j].report_threshold, cur_evt_cfg_ptr[j].prev_bo,
           event_cfg_ptr[j].time_to_trigger,cur_evt_cfg_ptr[j].trigger_tmr_cnt, prohibit_status, cur_evt_cfg_ptr[j].evt_not_first_meas);
    }
    ...
    if(traffic_info_ptr->tvm_debug == 1)
    {
      WL2_MSG_2(HIGH, ul_table_ptr, "Send Status %d, prev_bo - %d assigned",
                                                 send_status,
                                                 cur_evt_cfg_ptr[j].prev_bo);
    }
    ...
    if (trch_type == RACH)
    {
      ...
      ---------------------------------------------------------------------------------*/
      if (traffic_info_ptr->block_transmission)
      {
          /*-------------------------------------------------------------------------
        ...
        {
            WL2_MSG_0(HIGH, ul_table_ptr, "Event 4A reported, expecting RRC to send block rach req");
          ...
          if ((traffic_info_ptr->tx_interrupt_tmr-(traffic_info_ptr->tx_interrupt_cnt*10)) > event_cfg_ptr[j].tx_interrupt_tmr)
          {
            traffic_info_ptr->tx_interrupt_tmr = event_cfg_ptr[j].tx_interrupt_tmr;
            traffic_info_ptr->tx_interrupt_cnt = 0;
            WL2_MSG_3(HIGH, ul_table_ptr, "Resetting the timer tx_intp_tmr %d, new_tx_tmr %d, tx_cnt %d",
                                                       traffic_info_ptr->tx_interrupt_tmr,
                                                       event_cfg_ptr[j].tx_interrupt_tmr,
                                                       traffic_info_ptr->tx_interrupt_cnt);
          }
          else
          {
            // Let the ealier timer continue to run.
            WL2_MSG_3(HIGH, ul_table_ptr, "Continuing the tx interrupt timer tx_intp_tmr %d, new_tx_tmr %d, tx_cnt %d",
                                                       traffic_info_ptr->tx_interrupt_tmr,
                                                       event_cfg_ptr[j].tx_interrupt_tmr,
                                                       traffic_info_ptr->tx_interrupt_cnt);
          }
      ...
      else if (send_status && (event_id == EVENT_4A))
      {
        traffic_info_ptr->tx_interrupt_tmr = event_cfg_ptr[j].tx_interrupt_tmr;
        traffic_info_ptr->tx_interrupt_cnt = 0;
        WL2_MSG_1(HIGH, ul_table_ptr, "e4a reported, expecting RRC to send block rach req %d",
                                                   traffic_info_ptr->tx_interrupt_tmr);
      }
    }
    else
    {
      traffic_info_ptr->block_transmission = FALSE;
      traffic_info_ptr->tx_interrupt_tmr = traffic_info_ptr->tx_interrupt_cnt = 0;
    }
    ...
    if (send_status)
    {
      traffic_param_ptr->current_idx = traffic_info_ptr->cur_index;
      ...
      WL2_MSG_3(HIGH, ul_table_ptr, "Trigger MR meas_id %d, event_id %d, trch_type %d",
                                                 meas_id,
      ...
      if (event_cfg_ptr[j].trigger_prohibit_tmr)
      {
        ...
        WL2_MSG_2(HIGH, ul_table_ptr, "Pending timer started for event %d trchid=%d",
...
)
{
  ...
  else
  {
    ...
    if(eul_table_ptr->mac_hs_rach_info.hs_rach_active == TRUE)
    {
      hsrach_trch_index = find_hsrach_trch_index(traffic_param_ptr, ul_table_ptr);
      ntrch = 1;
    }
    ...
    for (i=0 ; i < ntrch && i < UE_MAX_TRCH ; i++)
    {
      ...
      if(eul_table_ptr->mac_hs_rach_info.hs_rach_active == TRUE)
      {
        ...
        {
          WL2_MSG_1(ERROR, ul_table_ptr, "TVM_ERR: No TrCh Type of EDCH, hsrach_trch_index %d is invalid index ",
                                                      hsrach_trch_index);
          break;
        }
      }
      ...
      {
        WL2_MSG_2(ERROR, ul_table_ptr, "Transport Channel Index: %d exceeds maximum number of channels: %d",
                                                    cur_evt_cfg_index,
                                                    UE_MAX_TRCH);
	return;
      }
      ...
      if(eul_table_ptr->mac_hs_rach_info.hs_rach_active == TRUE)
      {
         ...
         if (trch_bo > 0)
         {
          WL2_MSG_1(HIGH, ul_table_ptr, "MAC_TVM: Trch BO on EDCH %d", trch_bo);
         }
         proc_event_trigger_criteria_enhanced(meas_id, trch_id, trch_type, nevents, trch_bo, event_cfg_ptr, cur_evt_cfg_ptr, ul_table_ptr, eul_table_ptr);
      }
      else
      #endif /* FEATURE_WCDMA_HS_RACH */
      {
        ...
        if (!TRCH_ID_TO_IDX_IS_INVALID(trch_id))
        {
          ...
          if ((trch_type==GET_UL_CCTRCH_TYPE)&&(id_to_idx_ptr->valid))
          {
            ...
            ---------------------------------------------------------------------------------*/
            trch_bo = compute_trch_traffic_volume(trch_id, ul_table_ptr);
            
            if (trch_bo > 0)
            {
              WL2_MSG_2(HIGH, ul_table_ptr, "Trch Id %d, Trch BO %d", trch_id, trch_bo);
            }
            proc_event_trigger_criteria_enhanced(meas_id, trch_id, trch_type, nevents, trch_bo, event_cfg_ptr, cur_evt_cfg_ptr, ul_table_ptr, eul_table_ptr);
          }
        }
...
)
{
  ...
  else
  {
    ...
    -----------------------------------------------------------------------*/
    WL2_MSG_3(HIGH, ul_table_ptr, "Timer_count %d, report_interval %d, meas_id %d",
...
#endif /* FEATURE_WCDMA_MRAB_TVM_MAC_OPT */
    {
      if (((traffic_param_ptr->periodic_tmr_cnt * 
         (10 * MAC_UL_PERIODIC_TVM_TIMER_SCALE))>=traffic_param_ptr->report_interval)
          #ifdef FEATURE_WCDMA_DS_WEB_BOOST
	  || ((TRUE  == traffic_info_ptr->trig_tvm_params.frc_trig_tvm) &&
              (FALSE == traffic_info_ptr->trig_tvm_params.is_e4a_cnf))
	  #endif
	 )
      {
        ...
        else
        {
          ...
          (void)mac_ul_delete_traffic_volume_meas(meas_id, ul_table_ptr);
          WL2_MSG_2(HIGH, ul_table_ptr, "Sent %d reports. Deleting %d",
...
void mac_ul_handle_tvm_event_tmr (mac_ul_table_type *ul_table_ptr, mac_eul_table_type *eul_table_ptr)
{
  uint16                            meas_id;
  mac_ul_traffic_report_mode_e_type report_mode;
  mac_traffic_info_type *traffic_info_ptr;
  maci_ul_traffic_cfg_index_type    *traffic_cfg_idx_ptr;
  #ifdef FEATURE_WCDMA_DS_WEB_BOOST
  mac_frc_trig_tvm_type             *trig_tvm_ptr;
  #endif

  traffic_info_ptr = &ul_table_ptr->traffic_info;
  #ifdef FEATURE_WCDMA_DS_WEB_BOOST
  trig_tvm_ptr     = &ul_table_ptr->traffic_info.trig_tvm_params;
  #endif
     ...
     )
  {
    ...
    (void)rex_set_timer(&traffic_info_ptr->mac_ul_tvm_event_tmr_blk,(uint32)MAC_UL_TRAFFIC_VOLUME_TMR_BASE);
    WL2_MSG_0(LOW, ul_table_ptr, "MAC UL transport channels not configured yet");
  }
  else if (traffic_info_ptr->num_event_measurements==0)
  {
    WL2_MSG_0(LOW, ul_table_ptr, "No measurements to make");
  }
  else
  {
    /*------------------------------------------------------------------------------
    ...
    if (GET_UL_CCTRCH_TYPE == RACH)
    {
      ...
      {
        // Increment the timer
        if ((traffic_info_ptr->tx_interrupt_tmr > 0 )&& ((++traffic_info_ptr->tx_interrupt_cnt*10) >= traffic_info_ptr->tx_interrupt_tmr))
        {
          traffic_info_ptr->block_transmission = FALSE;
          traffic_info_ptr->tx_interrupt_tmr = traffic_info_ptr->tx_interrupt_cnt = 0;
          WL2_MSG_0(HIGH, ul_table_ptr, "TxInterruptTmr expired");
        }
      }
    }
    ...
    if(traffic_info_ptr->tvm_debug == 1)
    {
      WL2_MSG_0(HIGH, ul_table_ptr, "TVM_DBG: Resetting TVM event timer");
    }
    ...
    radio bearer.
    -----------------------------------------------------------------------------*/
    compute_obj_list_raw_payload(ul_table_ptr, eul_table_ptr);
    ...
    for (meas_id = MAC_UL_MIN_MEAS_ID; meas_id <= MAC_UL_MAX_MEAS_ID; meas_id++)
    {
          ...
          (traffic_cfg_idx_ptr->traffic_cfg_ptr!=NULL))
      {
        ...
        {
          proc_event_trigger_criteria(meas_id, ul_table_ptr, eul_table_ptr);
        }
      }/* if traffic_cfg_ptr is valid */
      else if((traffic_info_ptr->tvm_debug == 1)&&(meas_id == 4))
      {
        WL2_MSG_3(HIGH, ul_table_ptr, "TVM_DBG: Failed to check event trigger criteria - %d %d %d",
...
================================================================================*/
void mac_ul_handle_tvm_periodic_tmr (mac_ul_table_type *ul_table_ptr, mac_eul_table_type *eul_table_ptr)
{
  uint16                            meas_id;
  maci_ul_traffic_cfg_index_type    *traffic_cfg_idx_ptr;
  mac_ul_traffic_report_mode_e_type report_mode;
     ...
     )
  {
    ...
    (void)rex_set_timer(&traffic_info_ptr->mac_ul_tvm_periodic_tmr_blk,
                (uint32)(MAC_UL_TRAFFIC_VOLUME_TMR_BASE * MAC_UL_PERIODIC_TVM_TIMER_SCALE));
    WL2_MSG_0(HIGH, ul_table_ptr, "TVM_PERIODIC: MAC UL transport channels not configured yet");
  }
  else if (traffic_info_ptr->num_periodic_measurements == 0)
  {
    WL2_MSG_0(HIGH, ul_table_ptr, "TVM_PERIODIC:No periodic measurements to make");
  }
  else
  {
...
//      If it contains valid parameters and if its not temporarily suspend meas id
          ...
          (traffic_cfg_idx_ptr->traffic_cfg_ptr!=NULL))
      {
        ...
        if (report_mode==PERIODIC_MODE)
        {
          proc_periodic_mode_criteria(meas_id, ul_table_ptr, eul_table_ptr);
        }
      }/* if traffic_cfg_ptr is valid */
    }/* for */
  } /* num_measurements>0) */
...
)
{
  ...
  else
  {
        ...
        (traffic_cfg_idx_ptr->traffic_cfg_ptr==NULL))
    {
      ...
      traffic_cfg_idx_ptr->traffic_cfg_ptr=NULL;
      WL2_MSG_2(ERROR, ul_table_ptr, "Measurment ID doesn't exist %d %d",
                                                  !traffic_cfg_idx_ptr->valid,
    ...
    else
    {
      ...
      if (traffic_cfg_idx_ptr->traffic_cfg_ptr->report_mode == EVENT_TRIGGERED_MODE) 
      {
        ...
        if (traffic_info_ptr->num_event_measurements == 0)
        {
          /*--------------------------------------------------------------------------------
      ...
      else if (traffic_cfg_idx_ptr->traffic_cfg_ptr->report_mode == PERIODIC_MODE) 
      {
        ...
        if (traffic_info_ptr->num_periodic_measurements == 0)
        {
          /*--------------------------------------------------------------------------------
                      Stop the timer as there are no traffic volume measurements to make.
                    ---------------------------------------------------------------------------------*/
      ...
      WL2_MSG_3(HIGH, ul_table_ptr, "Deleted meas_id %d, num_event_meas %d, num_periodic %d",
                                                 meas_id,
                                                 traffic_info_ptr->num_event_measurements,
                                                 traffic_info_ptr->num_periodic_measurements);
...
boolean mac_ul_delete_all_traffic_volume_meas (mac_ul_table_type *ul_table_ptr)
{
  ...
  {
    delete_status_ind = FALSE;
    WL2_MSG_0(MED, ul_table_ptr, "There are no measurements to delete");
  }
  else
  {
    /*-----------------------------------------------------------------------------
    ...
    WL2_MSG_0(HIGH, ul_table_ptr, "Cleared Traffic Volume Timers");
  ...
  for (i=0;i<UE_NUM_TRAFFIC_MEAS;i++)
  {
    if (traffic_info_ptr->ul_traffic_info_table[i].traffic_cfg_ptr!=NULL)
    {
#ifdef FEATURE_WCDMA_USE_MODEM_HEAP
      modem_mem_free(traffic_info_ptr->ul_traffic_info_table[i].traffic_cfg_ptr, MODEM_MEM_CLIENT_WCDMA_RLC_CRIT);
#else
      mem_free((&tmc_heap),traffic_info_ptr->ul_traffic_info_table[i].traffic_cfg_ptr);
#endif
      traffic_info_ptr->ul_traffic_info_table[i].traffic_cfg_ptr=NULL;
    }
    traffic_info_ptr->ul_traffic_info_table[i].valid=FALSE;
...
)
{
  ...
  if (oc_traffic_cfg_ptr->meas_report_incl)
  {
        ...
        (oc_traffic_cfg_ptr->meas_report_qty.var_payload))
    {
      ...
      {
        status_ind=FALSE;
        WL2_MSG_1(ERROR, ul_table_ptr, "Invalid measurement interval %d",
  ...
  else if (int_tmp_cfg_ptr->avg_payload || int_tmp_cfg_ptr->var_payload)
  {
    ...
    {
      status_ind=FALSE;
      WL2_MSG_0(ERROR, ul_table_ptr, "Invalid measurement interval");
  ...
  if (status_ind && oc_traffic_cfg_ptr->meas_criteria_incl)
  {
    ...
    {
      status_ind=
      check_n_copy_periodic_params(int_tmp_cfg_ptr, oc_traffic_cfg_ptr, ul_table_ptr);
    }
    else
    { /* No Reporting */
      int_tmp_cfg_ptr->report_mode = report_mode;
...
)
{
  ...
  if (TRAFFIC_MEAS_ID_IS_INVALID(meas_id))
  {
    ...
    WL2_MSG_1(ERROR, ul_table_ptr, "Measurement ID out of range %d",
                                                meas_id);
  }
  ...
  if(int_tmp_cfg_ptr != NULL)
  {
        ...
        (oc_traffic_cfg_ptr->meas_report_mode.report_mode == EVENT_TRIGGERED_MODE))
    {
      ...
      WL2_MSG_1(HIGH, ul_table_ptr, "TVM_MODIFY : periodic_to_event_mode_change %d",
            ...
            (oc_traffic_cfg_ptr->meas_report_mode.report_mode == PERIODIC_MODE))
    {
      ...
      WL2_MSG_1(HIGH, ul_table_ptr, "TVM_MODIFY : event_to_periodic_mode_change %d",
  ...
  if (modify_status_ind)
  {
        ...
        (int_tmp_cfg_ptr==NULL)) /*lint !e644*/
    {
      ...
      WL2_MSG_0(ERROR, ul_table_ptr, "Measurement ID does not exist");
    }
    /* Check for the validity of the modify parameters that RRC has specified */
    else if (check_modify_params(oc_traffic_cfg_ptr,int_tmp_cfg_ptr, ul_table_ptr))
    {
      /*----------------------------------------------------------------------------------
      ...
      if (traffic_info_ptr->num_event_measurements == 0)
      {
        (void)rex_clr_timer(&traffic_info_ptr->mac_ul_tvm_event_tmr_blk);
         traffic_info_ptr->mac_tvm_event_timer_started = FALSE;
         WL2_MSG_1(HIGH, ul_table_ptr, "TVM_MODIFY : Num_event_meas = %d",
                                                    traffic_info_ptr->num_event_measurements);
      }
      else if ((traffic_info_ptr->num_event_measurements > 0) && (!traffic_info_ptr->mac_tvm_event_timer_started))
      {
        (void)rex_set_timer(&traffic_info_ptr->mac_ul_tvm_event_tmr_blk,(uint32)MAC_UL_TRAFFIC_VOLUME_TMR_BASE);
        traffic_info_ptr->mac_tvm_event_timer_started = TRUE;
        WL2_MSG_1(HIGH, ul_table_ptr, "TVM_MODIFY : Starting mac_ul_tvm_event_tmr_blk timer %d ms",
                                                   MAC_UL_TRAFFIC_VOLUME_TMR_BASE);
      }
      ...
      if (traffic_info_ptr->num_periodic_measurements == 0)
      {
        (void)rex_clr_timer(&traffic_info_ptr->mac_ul_tvm_periodic_tmr_blk);
        traffic_info_ptr->mac_tvm_periodic_timer_started = FALSE;
        WL2_MSG_1(HIGH, ul_table_ptr, "TVM_MODIFY : num_periodic_measurements = %d",
                                                   traffic_info_ptr->num_periodic_measurements);
      }
      else if ((traffic_info_ptr->num_periodic_measurements > 0) && (!traffic_info_ptr->mac_tvm_periodic_timer_started))
      {
        (void)rex_set_timer(&traffic_info_ptr->mac_ul_tvm_periodic_tmr_blk,
               (uint32)(MAC_UL_TRAFFIC_VOLUME_TMR_BASE * MAC_UL_PERIODIC_TVM_TIMER_SCALE));
        traffic_info_ptr->mac_tvm_periodic_timer_started = TRUE;
        WL2_MSG_1(HIGH, ul_table_ptr, "TVM_MODIFY : Starting mac_ul_tvm_periodic_tmr_blk timer %d ms",
                                                   (MAC_UL_TRAFFIC_VOLUME_TMR_BASE*MAC_UL_PERIODIC_TVM_TIMER_SCALE));
      }
      
      WL2_MSG_2(HIGH, ul_table_ptr, "TVM_MODIFY : num_event_meas %d, num_periodic_meas %d",
                                                 traffic_info_ptr->num_event_measurements,
                                                 traffic_info_ptr->num_periodic_measurements);
      ...
      if (oc_traffic_cfg_ptr->meas_obj_incl)
      {
        ...
        for (i=0;i<int_tmp_cfg_ptr->num_trch;i++)
        {
          ...
          {
            int_tmp_cfg_ptr->trch_info[i].trch_id = ul_table_ptr->int_ul_trch_info[0].trch_id;
          }
        }
      }
      /*---------------------------------------------------------------------------------
...
void mac_ul_block_rach_tx (boolean block_rach, mac_ul_table_type *ul_table_ptr)
{
 ...
 traffic_info_ptr = &ul_table_ptr->traffic_info;
  ...
  if (GET_UL_CCTRCH_TYPE != RACH)
  {
    WL2_MSG_0(ERROR, ul_table_ptr, "Invalid BLOCK command recvd");
    traffic_info_ptr->block_transmission = FALSE;
    traffic_info_ptr->tx_interrupt_tmr = traffic_info_ptr->tx_interrupt_cnt = 0;
  }
  else
  { /* CCTRCH_TYPE = RACH */
    /* block transmission */
    traffic_info_ptr->block_transmission = block_rach;
...
void proc_block_dtch_tx_on_rach(rlc_ul_buf_status_type *ul_buf_status_ptr, mac_ul_table_type *ul_table_ptr)
{
 ...
 identity.
  ...
  else
  {
    ...
    if (traffic_cfg_idx_ptr->valid)
    {
      ...
      /* Reset the counter */
      mac_reset_periodic_event_trigger_counters(ul_table_ptr);
    ...
    {
      WL2_MSG_1(HIGH, ul_table_ptr, "Save: Meas_Id %d not present",
                                                 meas_id);
    }
  }
  ...
  else
  {
    ...
    if (traffic_cfg_idx_ptr->valid)
    {
      ...
      else
      {
        ...
        uplink transport channels currently configured in MAC.
        ----------------------------------------------------------------------------*/
        traffic_cfg_ptr->num_trch= populate_object_list(traffic_cfg_ptr->trch_info, ul_table_ptr, eul_table_ptr);
      }
      
      if (traffic_cfg_ptr->report_mode == EVENT_TRIGGERED_MODE)
      {
        if (!traffic_info_ptr->mac_tvm_event_timer_started)
        {
          // Start the traffic volume timer ticking again.
          (void)rex_set_timer(&traffic_info_ptr->mac_ul_tvm_event_tmr_blk,(uint32)MAC_UL_TRAFFIC_VOLUME_TMR_BASE);
          WL2_MSG_2(HIGH, ul_table_ptr, "Starting event triggering TVM timer meas_id = %d, num_meas = %d",
                                                     meas_id,
                                                     traffic_info_ptr->num_event_measurements);
        }
        else
        {
          WL2_MSG_2(HIGH, ul_table_ptr, "TVM event triggering timer already running, meas_id = %d, num_meas = %d",
                                                     meas_id,
                                                     traffic_info_ptr->num_event_measurements);
        }
      }
      ...
      {
        if (!traffic_info_ptr->mac_tvm_periodic_timer_started)
        {
          // Start the traffic volume timer ticking again.
            (void)rex_set_timer(&traffic_info_ptr->mac_ul_tvm_periodic_tmr_blk,(uint32)(MAC_UL_TRAFFIC_VOLUME_TMR_BASE * MAC_UL_PERIODIC_TVM_TIMER_SCALE));
          WL2_MSG_2(HIGH, ul_table_ptr, "Starting TVM periodic timer meas_id = %d, num_meas = %d",
                                                     meas_id,
                                                     traffic_info_ptr->num_periodic_measurements);
        }
        else
        {
          WL2_MSG_2(HIGH, ul_table_ptr, "TVM periodic timer already running, meas_id = %d, num_meas = %d",
                                                     meas_id,
                                                     traffic_info_ptr->num_periodic_measurements);
        }
      }
    ...
    {
      WL2_MSG_1(HIGH, ul_table_ptr, "Resume: Meas_Id %d not present",
                                                 meas_id);
    }
  }
...
)
{
  ...
  {
    WL2_MSG_0(ERROR, ul_table_ptr, "Null config pointer");
  }
  else /* oc_traffic_cfg_ptr is not NULL */
  {

    WL2_MSG_2(HIGH, ul_table_ptr, "Recvd %d cmd for meas_id %d",
    ...
    case MAC_TRAFFIC_MEAS_START:
      config_sucess = mac_ul_setup_traffic_volume_meas(oc_traffic_cfg_ptr, ul_table_ptr, eul_table_ptr);
      break;
    ...
    case MAC_TRAFFIC_MEAS_MODIFY:
      config_sucess = mac_ul_modify_traffic_volume_meas(oc_traffic_cfg_ptr, ul_table_ptr, eul_table_ptr);
      break;
    ...
    case MAC_TRAFFIC_MEAS_DELETE:
      config_sucess = mac_ul_delete_traffic_volume_meas(oc_traffic_cfg_ptr->meas_id, ul_table_ptr);
      break;
    ...
    case MAC_TRAFFIC_MEAS_DELETE_ALL:
      config_sucess = mac_ul_delete_all_traffic_volume_meas(ul_table_ptr);
      break;

    case MAC_TRAFFIC_MEAS_RESUME:
      config_sucess = mac_ul_resume_traffic_volume_meas(oc_traffic_cfg_ptr->meas_id, ul_table_ptr, eul_table_ptr);
      break;

    case MAC_TRAFFIC_MEAS_SAVE:
      config_sucess = mac_ul_save_traffic_volume_meas(oc_traffic_cfg_ptr->meas_id, ul_table_ptr);
      break;

    default:
      WL2_MSG_0(ERROR, ul_table_ptr, "Unknown command");
      break;
    }

    #ifdef FEATURE_WCDMA_DS_WEB_BOOST
      mac_config_frc_tvm_trig_threshold(eul_table_ptr->as_id, oc_traffic_cfg_ptr);
    #endif
    /*------------------------------------------------------------------------------
    ...
    if (oc_traffic_cfg_ptr->config_conf_ind)
    {
      ...
      ---------------------------------------------------------------------------*/
      maci_post_cfg_cnf_msg(ul_table_ptr->as_id, oc_traffic_cfg_ptr->config_ack_num,
  ...
  for (i=0;i<MAX_TRCH;i++)
  {
    ...
    if (id_to_idx_ptr->valid)
    {
      ...
      if(found == FALSE)
      {
        ...
        ntrch++;
        WL2_MSG_3(HIGH, ul_table_ptr, "Added trch_id = %d, trch_type = %d, ntrch = %d",
                                                   trch_id,
                                                   GET_UL_CCTRCH_TYPE,
                                                   ntrch);
      }
...
==================================================================================*/
boolean mac_ul_is_meas_valid(uint16 meas_id, sys_modem_as_id_e_type as_id) 
{
  ...
  if (TRAFFIC_MEAS_ID_IS_INVALID(meas_id))
  {
    WL2_MSG_1(ERROR, ul_table_ptr, "MAC_ERR: Invalid measurement ID %d",
                                                meas_id);
    ret_val = FALSE;
  }
  else
  ...
  WL2_MSG_2(HIGH, ul_table_ptr, "MAC_TVM: Meas ID %d validity %d (0-Invalid,1-Valid)",
                                             meas_id, ret_val);
  return ret_val;
}
...
/*==================================================================================
 ...
 PURPOSE :   This function configures the maximum threshold to be
             configured 
==================================================================================*/
void mac_config_frc_tvm_trig_threshold(sys_modem_as_id_e_type              as_id, 
                                       mac_ul_traffic_meas_config_type    *oc_traffic_cfg_ptr)
{
  uint8                            meas_id, trch_idx, nevents;
  uint32                           threshold;
  boolean                          is_periodic = FALSE;
  mac_ul_table_type               *ul_table_ptr;
  maci_ul_traffic_cfg_index_type  *traffic_cfg_idx_ptr;
  mac_frc_trig_tvm_type           *mac_frc_trig_tvm_ptr;
  mac_ul_traffic_event_id_e_type   event_id;
                          

  ul_table_ptr         = GET_MAC_UL_TABLE_PTR(as_id);
  mac_frc_trig_tvm_ptr = &(ul_table_ptr->traffic_info.trig_tvm_params);

  mac_frc_trig_tvm_ptr->is_e4a_cnf      = FALSE;
  mac_frc_trig_tvm_ptr->max_cnf_thrshld = 0;

  if((ul_table_ptr->traffic_info.num_event_measurements    == 0) &&
     (ul_table_ptr->traffic_info.num_periodic_measurements == 0))
  {
    mac_frc_trig_tvm_ptr->prohibit_e4b_cnt = 0;
    mac_frc_trig_tvm_ptr->frc_trig_tvm     = FALSE;
  }
  else
  {
    for (meas_id = MAC_UL_MIN_MEAS_ID; meas_id <= MAC_UL_MAX_MEAS_ID; meas_id++)
    { 
      traffic_cfg_idx_ptr = GET_UL_TRAFFIC_CFG_PARAM_PTR(meas_id);  
      if ((TRUE  == traffic_cfg_idx_ptr->valid)            &&
    	  (FALSE == traffic_cfg_idx_ptr->suspend_meas)     &&
          (traffic_cfg_idx_ptr->traffic_cfg_ptr != NULL))
      {
        if(EVENT_TRIGGERED_MODE == traffic_cfg_idx_ptr->traffic_cfg_ptr->report_mode)
        {
          if(oc_traffic_cfg_ptr->meas_report_criteria.event_cfg.num_trch <= UE_MAX_TRCH)        
          {
            for(trch_idx = 0; trch_idx < UE_MAX_TRCH; trch_idx++) 
            {          
              for(nevents = 0; nevents < MAX_NUM_EVENTS; nevents++)
              {          
          	event_id = traffic_cfg_idx_ptr->traffic_cfg_ptr->trch_event_param[trch_idx][nevents].event_id;
                if(EVENT_4A == event_id)
                {
                  threshold = traffic_cfg_idx_ptr->traffic_cfg_ptr->trch_event_param[trch_idx][nevents].report_threshold + 1;
                  mac_frc_trig_tvm_ptr->max_cnf_thrshld = MAX(threshold, mac_frc_trig_tvm_ptr->max_cnf_thrshld);
                  mac_frc_trig_tvm_ptr->is_e4a_cnf      = TRUE;
                }
              }          
            }
          }
          ...
          {
            WL2_MSG_1(ERROR, ul_table_ptr, "MAC_FRC_TVM:Currently support %d trch's", 
                                            oc_traffic_cfg_ptr->meas_report_criteria.event_cfg.num_trch);
          }
        }
        else if(PERIODIC_MODE == traffic_cfg_idx_ptr->traffic_cfg_ptr->report_mode)
        {
          is_periodic = TRUE;
        }
      }
    }
    if (FALSE == mac_frc_trig_tvm_ptr->is_e4a_cnf)
    {      
      mac_frc_trig_tvm_ptr->prohibit_e4b_cnt = 0;
      if (TRUE == is_periodic)
      {
        mac_frc_trig_tvm_ptr->max_cnf_thrshld = MAC_TVM_PERIODIC_THRSHLD;
      }
    }
    WL2_MSG_3(HIGH, ul_table_ptr, "MAC_FRC_TVM: Max Threshold - %d, e4a_cnf - %d, Periodic - %d", mac_frc_trig_tvm_ptr->max_cnf_thrshld, mac_frc_trig_tvm_ptr->is_e4a_cnf, is_periodic);
  }
}
/*==================================================================================
 FUNCTION:   WCDMA_MAC_DS_TRIGGER_TVM_FORCIBLY

 PURPOSE :   This function triggers TVM forcibly when called by DS
==================================================================================*/
void wcdma_mac_ds_trigger_tvm_forcibly (sys_modem_as_id_e_type sub_id, rlc_ds_tvm_trigger_reason reason)
{
  sys_modem_as_id_e_type  as_id;
  mac_ul_table_type      *ul_table_ptr;
  ...
  WL2_MSG_3(HIGH, ul_table_ptr, "MAC_FRC_TVM: Force event called with reason - %d. L1_state - %d. e4a configured - %d", 
  	                         reason, l1m_get_state(as_id), ul_table_ptr->traffic_info.trig_tvm_params.is_e4a_cnf);
  if(FALSE == l1m_is_connected(as_id))
  {
    if (RLC_DS_INVALID_QUERY == reason)
    {
      return;
    }
    else    
    {
      ul_table_ptr->traffic_info.trig_tvm_params.frc_trig_tvm = TRUE;
      if((TRUE == ul_table_ptr->traffic_info.trig_tvm_params.is_e4a_cnf) && 
         (ul_table_ptr->traffic_info.num_event_measurements > 0)) 
      {
        (void)rex_set_sigs( wcdma_l2_ul_get_tcb(as_id), MAC_UL_TVM_EVENT_TMR_EXP_SIG );    
      }    
      else if(ul_table_ptr->traffic_info.num_periodic_measurements > 0)
      {
        (void)rex_set_sigs( wcdma_l2_ul_get_tcb(as_id), MAC_UL_TVM_PERIODIC_TMR_EXP_SIG );    		
      }
      else
      {
        ul_table_ptr->traffic_info.trig_tvm_params.frc_trig_tvm = FALSE;  
        WL2_MSG_0(HIGH, ul_table_ptr,"MAC_FRC_TVM: TVM not configured. Ignoring Force trigger by DS");
      }      
    }  
  }
}
