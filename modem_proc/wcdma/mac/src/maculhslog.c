...
$Header: //components/rel/wcdma.mpss/8.4.0/mac/src/maculhslog.c#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
(
  const mac_e_config_s_type   *hs_ul_config_ptr, boolean mac_e_reset_indicator, mac_eul_table_type *eul_table_ptr
)
{
  ...
  (WCDMA_MAC_HS_UL_CONFIG_LOG_PACKET_V1_type *)wcdma_log_alloc(WCDMA_MAC_HS_UL_CONFIG_LOG_PACKET_V1, pkt_len, eul_table_ptr->as_id);
  ...
  if (mac_hs_ul_cfg_log_ptr != NULL)
  {
    ...
    *log_ptr++ =
    ...
    /*MAC-E or MAC-I*/
    #ifdef FEATURE_MAC_I
    *log_ptr++ = eul_table_ptr->mac_hs_ul_pdu_type;
    #else
    *log_ptr++ = 0;
    #endif
    ...
    *log_ptr++ = mac_e_reset_indicator;
    ...
    if (TRUE == eul_table_ptr->mac_hs_ul_sec_car_info.tsn_field_extn_flag)
    {
      *log_ptr++ = 14;
    }
    else
	  ...
	  *log_ptr++ = 2;
        ...
        *log_ptr++= ue_logch_ptr->include_in_scheduling_info;
        ...
        WL2_MSG_8(HIGH, eul_table_ptr,
                    ...
                    ue_logch_ptr->rlc_mode, ue_logch_ptr->include_in_scheduling_info, ue_logch_ptr->rlc_pdu_type, ue_logch_ptr->num_rlc_size_ddi_info );
        
        if ((eul_table_ptr->mac_hs_ul_pdu_type == MAC_I_PDU_HDR) && (ue_logch_ptr->rlc_pdu_type == RLC_FLEXIBLE_PDU_SIZE))
        {
          /*PDU Size - FLEXIBLE*/
          *log_ptr++ = 1;
          ...
          *log_ptr++= (uint8)((ue_logch_ptr->ul_flex_max_pdu_size & 0xFF00) >> 8);
          ...
          WL2_MSG_3(HIGH, eul_table_ptr, "MAC_I_CFG: Li_Size%d,Min_Pdu_Size%d,Max_Pdu_Size %d",
                                                     ue_logch_ptr->ul_li_size,
                                                     ue_logch_ptr->ul_flex_min_pdu_size,
                                                     ue_logch_ptr->ul_flex_max_pdu_size);
        }
        else if ((eul_table_ptr->mac_hs_ul_pdu_type == MAC_E_PDU_HDR) || 
                 ((eul_table_ptr->mac_hs_ul_pdu_type == MAC_I_PDU_HDR) && (ue_logch_ptr->rlc_pdu_type == RLC_FIXED_PDU_SIZE)))
        #endif /*  FEATURE_MAC_I  */
        {
          /*PDU Size - FIXED*/
	  *log_ptr++ = 0;
          ...
          *log_ptr++=  ue_logch_ptr->num_rlc_size_ddi_info;
          ...
          for (k=0;k< ue_logch_ptr->num_rlc_size_ddi_info;k++)
          {
            ...
            *log_ptr++= (uint8)((ue_logch_ptr->rlc_size_ddi_info[k].rlc_pdu_size_in_bits & 0xFF00) >> 8);
            ...
            WL2_MSG_2(HIGH, eul_table_ptr, "MAC_I_CFG: DDI %d, PDU_SIZE %d",
  ...
  {
    WL2_MSG_0(ERROR, eul_table_ptr, "MAC HS Logging of config pkt failed");
  }
  //log ptr != NULL
...
void mac_hs_ul_process_header_log_packet(mac_eul_table_type *eul_table_ptr)
{
  ...
  mac_eul_logging_info *eul_log_info_ptr;
  eul_log_info_ptr = &eul_table_ptr->eul_log_info;
  
  if (eul_log_info_ptr->mac_hs_ul_current_hdr_log_buf_flag == TRUE)
  {
    hdr_log_ptr = &(eul_log_info_ptr->mac_hs_ul_header_prev_log_info);
  }
  else
  {
    hdr_log_ptr = &(eul_log_info_ptr->mac_hs_ul_header_log_info);
  }

  if(GET_UL_DEBUG_EUL_LOGGING(eul_table_ptr) & DEBUG_EUL_MAC_ETFCI_LOG_PKT)
  {
    WL2_MSG_2(HIGH, eul_table_ptr, "DBG_0x4322: Number of sample - %d. 0x4322_Hdr_Curr_buff - %d", 
		                    hdr_log_ptr->num_samples,
  	                            eul_log_info_ptr->mac_hs_ul_current_hdr_log_buf_flag);
  }
  ...
  {
    WL2_MSG_1(ERROR, eul_table_ptr, "Num log samples exceeds count,num_samples = %d ",
                                                hdr_log_ptr->num_samples);
  ...
  if (hdr_log_ptr->num_samples > 0)
  {
    ...
    (WCDMA_MAC_HS_UL_HEADER_STATUS_LOG_PACKET_type *)wcdma_log_alloc(
          WCDMA_MAC_HS_UL_HEADER_STATUS_LOG_PACKET, pkt_len, eul_table_ptr->as_id);
    ...
    if (log_ptr != NULL)
    {
      ...
      {
        WL2_MSG_3(HIGH, eul_table_ptr, "num_new_tx %d, num_si %d, num_mac_hdrs %d",
                                                   num_new_tx,
                                                   num_si,
                                                   num_mac_hdrs);
      }
    ...
    {
      WL2_MSG_0(ERROR, eul_table_ptr, "Unable to allocate memory for the log_alloc ");
    }
  }
  // clear the dirty_bit and num_samples
  hdr_log_ptr->num_samples = 0;
...
void mac_hs_ul_process_etfci_status_log_packet(mac_eul_table_type *eul_table_ptr)
{
  ...
  if (eul_log_info_ptr->mac_hs_ul_current_etfci_log_buf_flag == TRUE)
  {
    etfci_log_ptr = &(eul_log_info_ptr->mac_hs_ul_etfci_status_prev_log_info);
  }
  else
  {
    etfci_log_ptr = &(eul_log_info_ptr->mac_hs_ul_etfci_status_log_info);
  }

  if(GET_UL_DEBUG_EUL_LOGGING(eul_table_ptr) & DEBUG_EUL_MAC_ETFCI_LOG_PKT)
  {
    WL2_MSG_2(HIGH, eul_table_ptr, "DBG_0x4323: Number of sample - %d. 0x4323_Hdr_Curr_buff - %d", 
                                    etfci_log_ptr->num_samples,
  	                            eul_log_info_ptr->mac_hs_ul_current_etfci_log_buf_flag);
  }
  ...
  if (etfci_log_ptr->num_samples > 0)
  {
    ...
    (WCDMA_MAC_HS_UL_ETFCI_STATUS_LOG_PACKET_type *)wcdma_log_alloc(
          WCDMA_MAC_HS_UL_ETFCI_STATUS_LOG_PACKET, pkt_len, eul_table_ptr->as_id);
    ...
    if (log_ptr != NULL)
    {
      ...
      if (num_new_tx != 0)
      {
        WL2_MSG_2(HIGH, eul_table_ptr, "num_new_tx %d, etfci_num_samples %d",
                                                   num_new_tx,
                                                   etfci_num_samples);
      }
...
void mac_hs_ul_process_maci_is_log_packet(mac_eul_table_type *eul_table_ptr)
{
  ...
  ernti = eul_table_ptr->mac_hs_rach_info.primary_e_rnti;
  #endif
  
  if (eul_log_info_ptr->mac_hs_ul_maci_current_log_buf_flag == TRUE)
  {
    maci_is_log_ptr = &(eul_log_info_ptr->mac_hs_ul_maci_is_prev_log_packet_info);
  }
  else
  {
    maci_is_log_ptr = &(eul_log_info_ptr->mac_hs_ul_maci_is_log_packet_info);
  }
  ...
  if(maci_is_log_ptr->log_pkt_num_samples > MAC_HS_UL_NUM_MACI_IS_STATUS_LOG_PKT)
   {
    WL2_MSG_1(ERROR, eul_table_ptr, "MAC_I_LOG: Num log samples exceeds count,num_samples = %d ",
  ...
  { 
     WL2_MSG_2(HIGH, eul_table_ptr, "MAC_I_LOG: mismatch in MAX-40 allowed and num TTI %d and nsamples %d ",
  ...
  if(maci_is_log_ptr->num_tti_logged > 0)
  {
    ...
    if((num_continue_tx != maci_is_log_ptr->num_continue) && (GET_UL_DEBUG_EUL_LOGGING(eul_table_ptr) & DEBUG_EUL_MAC_ETFCI_LOG_PKT))
    {
      WL2_MSG_2(HIGH, eul_table_ptr, "number of continues do not match num_continue_tx - %d maci_is_log_ptr->num_continue - %d",
                                      num_continue_tx,
                                      maci_is_log_ptr->num_continue);
    }           
    mac_hs_ul_maci_is_log_pkt_type_ptr = (WCDMA_MAC_HS_UL_MAC_I_LOG_PACKET_type *)wcdma_log_alloc( WCDMA_MAC_HS_UL_MAC_I_LOG_PACKET, pkt_len, eul_table_ptr->as_id);
    ...
    if(log_ptr != NULL)
    {
      ...
      *log_ptr |= ((maci_is_log_ptr->tti << 7) & 0x80);
      ...
      //ERNTI
      *log_ptr++ = (uint8)(ernti & 0x00FF);
      *log_ptr++ = (uint8)((ernti & 0xFF00) >> 8);
        ...
        *log_ptr |= ((maci_per_tti_ptr->ctrl_info.happy_bit & MAC_HS_UL_DC_CARRIER_HB_MASK) << MAC_HS_UL_DC_CARRIER_HB_SHIFT);
        ...
        if((maci_per_tti_ptr->ctrl_info.tx_type == 2) || (maci_per_tti_ptr->ctrl_info.tx_type == 1))
        {
          ...
          *log_ptr++ = (uint8)((maci_per_tti_ptr->l1_sg_payload & 0xFF00) >> 8);
          if (GET_UL_DEBUG_EUL_LOGGING(eul_table_ptr) & DEBUG_EUL_MAC_ETFCI_LOG_PKT)
          {          
            WL2_MSG_7(HIGH, eul_table_ptr,
                      "MAC_DC_LOG: RETX or DTX case  idx %d, etfci_reason %d, S buf %d, NS buf %d, sup payld %d, sg payld %d, log_ch_hdr_idx %d",
        ...
        else if(maci_per_tti_ptr->ctrl_info.tx_type == 3)
        {
          ...
          if (GET_UL_DEBUG_EUL_LOGGING(eul_table_ptr) & DEBUG_EUL_MAC_ETFCI_LOG_PKT)
          {
            WL2_MSG_2(HIGH, eul_table_ptr, "MAC_DC_LOG:N_sample %d CONTINUE_TX, type %d",
                                                       idx,
        ...
        {
          log_ptr++;
          WL2_MSG_2(ERROR, eul_table_ptr, "MAC_DC_LOG: Invalid TxTYPe %d, idx %d ",
                                                      idx,
                                                      maci_per_tti_ptr->ctrl_info.tx_type);
          continue;
        }
             ...
             (log_ch_hdr_idx < MAC_HS_UL_NUM_MACI_IS_STATUS_LOG_PKT)); continue_tx_type_indx++)
        {
          if(continue_tx_type_indx != 0)
          {
            ...
            *log_ptr |= ((maci_per_tti_ptr->ctrl_info.happy_bit & MAC_HS_UL_DC_CARRIER_HB_MASK) << MAC_HS_UL_DC_CARRIER_HB_SHIFT);
            ...
            if (GET_UL_DEBUG_EUL_LOGGING(eul_table_ptr) & DEBUG_EUL_MAC_ETFCI_LOG_PKT)
            {
              WL2_MSG_2(HIGH, eul_table_ptr, "MAC_I_LOG: Tx_Type continue, n maci_hdrs %d,idx %d ",
        ...
        if((maci_per_tti_ptr->ctrl_info.tx_type == 0) && (maci_per_tti_ptr->num_mac_i_hdrs == 0))
        {
          if (GET_UL_DEBUG_EUL_LOGGING(eul_table_ptr) & DEBUG_EUL_MAC_ETFCI_LOG_PKT)
          {
            WL2_MSG_2(HIGH, eul_table_ptr, "MAC_I_LOG: SI only Triggered or No-data Tx- idx %d, maci_hdr_idx %d, ",
                                                       idx, log_ch_hdr_idx);
          }
          //SS-TSN
        ...
        if((GET_UL_DEBUG_EUL_LOGGING(eul_table_ptr) & DEBUG_EUL_MAC_ETFCI_LOG_PKT) && (log_ch_hdr_idx != idx))
        {
          WL2_MSG_2(ERROR, eul_table_ptr, "MAC_I_LOG: Not on same index- log_ch_hdr_idx %d , idx %d",
                                                      log_ch_hdr_idx, idx);
        }
      }
    ...
    {
      WL2_MSG_0(ERROR, eul_table_ptr, "MAC_I_LOG: Unable to allocate memory for Mac-i-is log_alloc");
    }
  }
  else
  {
    WL2_MSG_0(HIGH, eul_table_ptr, "MAC_I_LOG: num samples zero ");
  }
...
void mac_hs_ul_process_hsrach_timers_log_pkt(mac_eul_table_type *eul_table_ptr)
{
  ...
  if(eul_log_info_ptr->hs_rach_mac_timers_log_pkt_status == TRUE)
  {
    if(eul_log_info_ptr->hs_rach_mac_timers_cur_log_buf_flag == TRUE)
    {
      hs_rach_mac_timers_submit_log_pk_ptr = &eul_log_info_ptr->hs_rach_mac_timers_prev_log_pkt_info; 
    }
    else
    {
      hs_rach_mac_timers_submit_log_pk_ptr = &eul_log_info_ptr->hs_rach_mac_timers_log_pkt_info;
    }
    ...
    {
       WL2_MSG_0(ERROR, eul_table_ptr, "MAC_HSRACH_LOG: hs_rach_mac_timers_submit_log_pk_ptr is NULL ");
    }
    
    mac_hsrach_timers_log_pkt_ptr = (WCDMA_HS_RACH_MAC_TIMERS_LOG_PACKET_type *)wcdma_log_alloc( WCDMA_HS_RACH_MAC_TIMERS_LOG_PACKET, pkt_len, eul_table_ptr->as_id);
      ...
      if(hs_rach_mac_timers_submit_log_pk_ptr != NULL)
      {
        ...
        *log_ptr++ = (uint8)((hs_rach_mac_timers_submit_log_pk_ptr->hsrach_mac_timer_info[0].cfn_sfn & (0xFF00)) >> 8 );
        ...
        {  
           ...
           *log_ptr++ = (uint8) (hs_rach_mac_timers_submit_log_pk_ptr->hsrach_mac_timer_info[loop_idx].misc_vals & (0x00FF));

           *log_ptr++ = (uint8) ((hs_rach_mac_timers_submit_log_pk_ptr->hsrach_mac_timer_info[loop_idx].misc_vals & 0xFF00) >> 8);

           
           if(GET_UL_DEBUG_EUL_LOGGING(eul_table_ptr) & DEBUG_EUL_MAC_HSRACH_LOGGING)
           {
             WL2_MSG_9(HIGH, eul_table_ptr,
                 "MAC_HSRACH_LOG: loop_idx %d, cch_bo %d ,ccch_timer %d, tb_timer %d, dxch_bo %d, misc 0x%x, agch_timer %d, cfn %d, cur_log-buf_flag %d",
                 loop_idx,
                 ...
                 eul_log_info_ptr->hs_rach_mac_timers_cur_log_buf_flag);
           }
        }
    ...
    {
      WL2_MSG_0(ERROR, eul_table_ptr, "MAC_HSRACH_LOG: Unable to allocate memory for HSRACH timers log_alloc ");
    }
  }
  else
  {
    WL2_MSG_1(ERROR, eul_table_ptr, "MAC_HSRACH_LOG: mac timer log packet disabled but signal set: %d",
      eul_log_info_ptr->hs_rach_mac_timers_log_pkt_status);
  }
}
