...
Copyright (c) 2002, 2004-2006, 2008-2009 by Qualcomm Technologies Incorporated.  All Rights Reserved.
...
$Header: //components/rel/wcdma.mpss/8.4.0/mac/src/macdl.h#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
#define     GET_DL_TB_INFO_PTR(tb_index, dl_table_ptr)    &dl_table_ptr->dl_tb_info[tb_index]

/* Pointer to internal BCH config info */
#define     GET_BCH_INFO_PTR(cctrch_id, dl_table_ptr)\
            &dl_table_ptr->bch_cfg_info[cctrch_id - MAC_DL_MIN_CCTRCH_ID]

/* Pointer to internal PCH config info */
#define     GET_PCH_INFO_PTR(cctrch_id, dl_table_ptr)\
            &dl_table_ptr->pch_cfg_info[cctrch_id - MAC_DL_MIN_CCTRCH_ID]

/* Pointer to internal DL DCH config info */
#define     GET_DL_DCH_INFO_PTR(trch_id, dl_table_ptr)\
            &dl_table_ptr->dl_dch_cfg_info[dl_table_ptr->dl_dch_id_to_idx[trch_id - MAC_DL_MIN_TRCH_ID].idx]

/* Pointer to internal DSCH config info */
#define     GET_DSCH_INFO_PTR(trch_id, dl_table_ptr)\
            &dl_table_ptr->dsch_cfg_info[dl_table_ptr->dsch_id_to_idx[trch_id - MAC_DL_MIN_TRCH_ID].idx]

/* Pointer to internal FACH config info */
#define     GET_FACH_INFO_PTR(trch_id, dl_table_ptr)\
            &dl_table_ptr->fach_cfg_info[dl_table_ptr->fach_id_to_idx[trch_id - MAC_DL_MIN_TRCH_ID].idx]

/* Condition for enabled DL DCH */
#define     DL_DCH_IS_ENABLED(trch_id, dl_table_ptr)\
            dl_table_ptr->dl_dch_id_to_idx[trch_id - MAC_DL_MIN_TRCH_ID].valid

/* DL DCH ID to IDX Map Pointer */
#define     DL_DCH_ID_TO_IDX_PTR(trch_id, dl_table_ptr)\
            &dl_table_ptr->dl_dch_id_to_idx[trch_id - MAC_DL_MIN_TRCH_ID]

/* Condition for enabled DSCH */
#define     DSCH_IS_ENABLED(trch_id, dl_table_ptr)\
            dl_table_ptr->dsch_id_to_idx[trch_id - MAC_DL_MIN_TRCH_ID].valid

/* DSCH ID to IDX Map Pointer */
#define     DSCH_ID_TO_IDX_PTR(trch_id, dl_table_ptr)\
            &dl_table_ptr->dsch_id_to_idx[trch_id - MAC_DL_MIN_TRCH_ID]

/* Condition for enabled FACH */
#define     FACH_IS_ENABLED(trch_id, dl_table_ptr)\
            dl_table_ptr->fach_id_to_idx[trch_id - MAC_DL_MIN_TRCH_ID].valid

/* FACH ID to IDX Map Pointer */
#define     FACH_ID_TO_IDX_PTR(trch_id, dl_table_ptr)\
            &dl_table_ptr->fach_id_to_idx[trch_id - MAC_DL_MIN_TRCH_ID]

#define     DL_TB_CRNTI_IS_INVALID(crnti, dl_table_ptr)\
    (!(dl_table_ptr->dl_mac_crnti_valid) || (crnti != dl_table_ptr->dl_mac_crnti))

#define     DL_TB_URNTI_IS_INVALID(urnti, dl_table_ptr)\
    (!(dl_table_ptr->dl_mac_urnti_valid) || (urnti != dl_table_ptr->dl_mac_urnti))
...
// Pointer to DL MAC Ciphering Config Info
  mac_cipher_config_type   cipherptr[], mac_dl_table_type *dl_table_ptr
  );
