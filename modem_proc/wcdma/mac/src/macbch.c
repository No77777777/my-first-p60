...
Copyright (c) 2005, 2008-2009 by Qualcomm Technologies Incorporated.  All Rights Reserved.
...
$Header: //components/rel/wcdma.mpss/8.4.0/mac/src/macbch.c#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
)
{
   ...
   for (i=0; i<UE_MAX_CCTRCH; i++) {
      dl_table_ptr->bch_cfg_info[i].bch_enable = FALSE;
   }

   /* Invalid number of BCHs */
   if (num_bchs > MAC_DL_MAX_CCTRCH) {
      WL2_MSG_1(HIGH, dl_table_ptr , "invalid number of BCHs %d",
                                                  num_bchs);
   }
   /* Ordered BCH Info pointer is NULL */
   else if (oc_bch_info == NULL) {
      WL2_MSG_0(HIGH, dl_table_ptr , "BCH ordered config info pointer is NULL");
   }
   /* Valid Number of BCHs */
      ...
      for (i=0; i<num_bchs; i++) {
         ...
         if (DL_CCTRCH_ID_IS_INVALID(oc_bch_info[i].cctrch_id)) {
            WL2_MSG_2(HIGH, dl_table_ptr , "BCH%d CCTrCh ID is invalid",
                                                        i,
                                                        oc_bch_info[i].cctrch_id);
         }
         /* BCCH ID is invalid */
         else if (DL_RLC_ID_IS_INVALID(oc_bch_info[i].bcch_id)) {
            WL2_MSG_2(HIGH, dl_table_ptr , "BCH%d BCCH ID %d is invalid",
                                                        i,
         ...
         /* BCH parameters are valid */
            ...
            int_bch_ptr = GET_BCH_INFO_PTR(oc_bch_info[i].cctrch_id, dl_table_ptr);

            /* Save BCH info in config info table */
            int_bch_ptr->bch_enable = TRUE;
            int_bch_ptr->bcch_id = oc_bch_info[i].bcch_id;

            WL2_MSG_3(LOW, dl_table_ptr , "BCH%d: CCTrCh ID = %d BCCH ID %d",
                                                       i,
                                                       oc_bch_info[i].cctrch_id,
                                                       oc_bch_info[i].bcch_id);
         }
...
)
{
  ...
  for (i=0 ; i <tbs_hdr_ptr->num_tb; i++) {
    ...
    if (tb_hdr_ptr == NULL){ /*lint !e774*/
      WL2_MSG_0(HIGH, dl_table_ptr , "TB%d Header Info pointer is NULL");
    }
    else {
      /* Set L1 parameters */
