...
$Header: //components/rel/wcdma.mpss/8.4.0/mac/src/maculdata.c#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
01/29/18  pr      Replace global l1 variable with function wl1m_is_ue_in_wplt_mode
...
mac_rlc_data_table_type    mac_data_table[L2_NUM_W_SUBS];
...
static void mac_profile_dump_start(mac_ul_table_type *ul_table_ptr)
{
  mac_ul_debug_ino_type *debug_info_ptr;
  debug_info_ptr = &ul_table_ptr->debug_info;
  
  wl1_get_profile_point(&debug_info_ptr->frame_qty, &debug_info_ptr->cx8_qty, ul_table_ptr->as_id);
  debug_info_ptr->l2_profile_dump[debug_info_ptr->l2_profile_idx].frame_qty_start = debug_info_ptr->frame_qty;
  debug_info_ptr->l2_profile_dump[debug_info_ptr->l2_profile_idx].cx8_qty_start = debug_info_ptr->cx8_qty;
}
...
static void mac_profile_dump_end(mac_ul_table_type *ul_table_ptr)
{
  uint8 loop_index =0;
  mac_ul_debug_ino_type *debug_info_ptr;
  debug_info_ptr = &ul_table_ptr->debug_info;
  ...
  wl1_get_profile_point(&debug_info_ptr->frame_qty, &debug_info_ptr->cx8_qty, ul_table_ptr->as_id);
  debug_info_ptr->l2_profile_dump[debug_info_ptr->l2_profile_idx].frame_qty_end = debug_info_ptr->frame_qty;
  debug_info_ptr->l2_profile_dump[debug_info_ptr->l2_profile_idx].cx8_qty_end = debug_info_ptr->cx8_qty;
  debug_info_ptr->l2_profile_idx++;

  if(debug_info_ptr->l2_profile_idx >= MAC_NUM_PROFILE_SAMPLES)
  {
    debug_info_ptr->l2_profile_idx = 0;

    if (debug_info_ptr->l2_print_profile_data)
    {
      for(loop_index = 0; loop_index > (MAC_NUM_PROFILE_SAMPLES/4); loop_index++)
      {
        WL2_MSG_8(HIGH, ul_table_ptr, "mac_profile_dump: (%d, %d), (%d, %d), (%d, %d), (%d, %d),",
         debug_info_ptr->l2_profile_dump[loop_index].cx8_qty_start, debug_info_ptr->l2_profile_dump[loop_index].cx8_qty_end,
         debug_info_ptr->l2_profile_dump[loop_index+1].cx8_qty_start, debug_info_ptr->l2_profile_dump[loop_index+1].cx8_qty_end,
         debug_info_ptr->l2_profile_dump[loop_index+2].cx8_qty_start, debug_info_ptr->l2_profile_dump[loop_index+2].cx8_qty_end,
         debug_info_ptr->l2_profile_dump[loop_index+3].cx8_qty_start, debug_info_ptr->l2_profile_dump[loop_index+4].cx8_qty_end
        );
      }
    }
  }
...
===============================================================================================*/
void mac_rach_srb_rxd_manage(boolean data_present_on_srb, mac_ul_table_type *ul_table_ptr)
{
  ...
  if (GET_UL_CCTRCH_TYPE == RACH)
  {
    ...
    {
      if (ul_table_ptr->mac_rach_rxd_enable == TRUE)
      {
        ul_table_ptr->mac_rach_rxd_enable = FALSE;
        WL2_MSG_0(ERROR, ul_table_ptr, "MAC_RACH: Call RxD FACH Disable as RACH-SRB data NOT available");
        wl1_rxd_fach_signalling_data_available(FALSE, ul_table_ptr->as_id);
      }
    }
    else
    {
       ...
       * which enables the RxD (if not yet enabled) and manages FMO */
      if (ul_table_ptr->mac_rach_rxd_enable == FALSE)
      {
        ul_table_ptr->mac_rach_rxd_enable = TRUE;
        WL2_MSG_0(ERROR, ul_table_ptr, "MAC_RACH: Call RxD FACH Enable as RACH-SRB data available");
        wl1_rxd_fach_signalling_data_available(TRUE, ul_table_ptr->as_id);
      }
    }
  }
}
...
)
{
  ...
  rv = FALSE;   /* function code starts here */
  
  L2_ACQ_UL_LW_LOCK(ul_table_ptr->as_id, UE_MAX_UL_LOGICAL_CHANNEL);
  
  rlc_ul_enh_buffer_status(mac_ul_lc_info_ptr, &data_table_ptr->ul_buf_status, NON_BFI_NON_TVM_QUERY, rlc_wait_for_ack, ul_table_ptr);
  
  L2_ENH_ACQ_UL_LW_UNLOCK(ul_table_ptr->as_id, UE_MAX_UL_LOGICAL_CHANNEL);
  ...
  for (i = 0; i < mac_ul_lc_info_ptr->nchan; i++)
  {
    ...
    lc_status_ptr = &data_table_ptr->ul_buf_status.chan_info[rlc_id];
    ...
    {
      WL2_MSG_7(HIGH, ul_table_ptr,"RLC %d: nBytes %d, ctrl_data %d, new_data %d, retx_data %d, un_comp %d win_size %d",
    ...
    switch (lc_status_ptr->type)
    {
      ...
      case FIXED_PDUS:
        ...
        {
          WL2_MSG_3(LOW, ul_table_ptr , "RLC%d: pduSize=%d, nPDUs=%d",
                                                     rlc_id,
      ...
      case FLEX_PDUS:
        if ((lc_status_ptr->nbytes > 0) || ((lc_status_ptr->new_data_uncomp_bytes > 0) && 
            (lc_status_ptr->effect_win_size > 0)))
        {
          WL2_MSG_2(LOW, ul_table_ptr , "RLC%d: nBytes=%d",
                                                     rlc_id,
...
boolean l1_query_mac_rach_data (sys_modem_as_id_e_type as_id, boolean *srb_data_present)
{
  ...
  if(rach_info_ptr->rach_state == RACH_IN_IDLE)
  {
    ...
    buffer_staus_not_empty_flag = rlc_buffers_are_not_empty(&(data_table_ptr->mac_ul_lc_info), &rlc_wait_for_ack, ul_table_ptr);
    
    if (data_table_ptr->mac_ul_lc_info.nchan > 0 )
    {
      mac_compute_pdus_from_rlc_status(data_table_ptr->mac_ul_lc_info.nchan, data_table_ptr->mac_ul_lc_info.rlc_id, &data_table_ptr->ul_buf_status, &data_present_on_srb, ul_table_ptr);
    }
    
    mac_rach_srb_rxd_manage(data_present_on_srb, ul_table_ptr);
#ifdef FEATURE_WCDMA_HS_FACH_DRX
    if (TRUE == wl1_edrx_get_edrx_configured_status(as_id))
    {
      if (TRUE != buffer_staus_not_empty_flag)
      {
        if (TRUE != rlc_wait_for_ack)
        {
          buffer_staus_not_empty_flag = mac_rlc_check_pending_dl_data(hs_table_ptr);
        }
        else
        {
          WL2_MSG_0(HIGH, ul_table_ptr, "MAC_EDRX_FS: L2 UL is waiting for ACK");
  ...
  else if(rach_info_ptr->rach_state == RACH_WAITING_FOR_STATUS)
  {
...
)
{
  ...
  oc_tfc_ptr = &oc_tfcs_ptr->tfc_info[GET_SELECTED_TFCI];
  ...
  for (i=0; i < rlc_frame_data_ptr->nchan; i++)
  {
    ...
    if (!UL_RLC_ID_IS_INVALID(lc_data_ptr->rlc_id))
    {
      ...
      for (j=0 ;j <lc_data_ptr->npdus; j++)
      {
        ...
        tbptr->pdu_offset = lc_data_ptr->pdu_info[j].pdu_offset;
        ...
        if (lc_info_ptr->enable)
        {
          ...
          else if (GET_UL_CCTRCH_TYPE != INVALID_UL_CCTRCH )
          {
            ...
            case UE_MODE_TRANSPARENT:
              ...
              if (ul_table_ptr->mac_cipher_info.cur_ul_cipher_info[index].cipher_enable &&
              (ul_table_ptr->mac_cipher_info.cur_ul_cipher_info[i].cipher_cfg_type != SMC_INITIATED_NON_HO_CIPHER_CFG) &&
              ((ul_table_ptr->mac_cipher_info.cur_ul_cipher_info[index].cipher_algo == UE_WCDMA_CIPHER_ALGO_UEA1)
#ifdef FEATURE_WCDMA_REL7
              || (ul_table_ptr->mac_cipher_info.cur_ul_cipher_info[index].cipher_algo == UE_WCDMA_CIPHER_ALGO_UEA2)
#endif
              ))
              {
                if (lc_info_ptr->rb_id != 0)
                {
                  ...
                  tbptr->cipher_algo = (uint32)ul_table_ptr->mac_cipher_info.cur_ul_cipher_info[index].cipher_algo;
                  tbptr->ciphering_key_id = ul_table_ptr->mac_cipher_info.cur_ul_cipher_info[index].key_idx;
                  tbptr->count = ul_table_ptr->mac_cipher_info.count_c[index];
                  tbptr->offset = 0;
                }
              }
              if ((0 == lc_info_ptr->rb_id) && (RACH == GET_UL_CCTRCH_TYPE ))
              {
                ...
                WL2_MSG_0(HIGH, ul_table_ptr, "MAC_RACH: CCCH data present in this RACH TX");
                ul_table_ptr->rach_info.mac_rach_ccch_data_presence = TRUE;
              }
      ...
      oc_tf_ptr=
      &oc_tfcs_ptr->tfs_info[trch_idx].tf_info[oc_tfc_ptr->tfi[trch_idx]];
      if ((UE_LOGCHAN_CCCH == lc_info_ptr->lc_type) ||
          (UE_LOGCHAN_DCCH == lc_info_ptr->lc_type))
      {
        txbufptr->srb_data_present = TRUE;
        txbufptr->srb_data_num_tti = oc_tfcs_ptr->tfs_info[trch_idx].tti_info;
      }

      if (l1_trch_data_ptr->num_tb != oc_tf_ptr->ntblks)
      {
        WL2_MSG_3(ERROR, ul_table_ptr, "Mismatch in num_tb %d for LC %d, TFCI %d",
    ...
    {
      WL2_MSG_0(HIGH, ul_table_ptr , "Invalid RLC ID in UL RLC Frame Data");
    }
  }
  ...
  {
    /* Transport Format Pointer */
    oc_tf_ptr= &oc_tfcs_ptr->tfs_info[i].tf_info[oc_tfc_ptr->tfi[i]];
    ...
    if (trch_select_flag[i] == TRUE)
    {
      ...
      {
        txbufptr->num_trch = 0;
        WMAC_ERR_FATAL(ul_table_ptr->as_id, "Mismatch in num_tb %d tb_size %d, TFCI %d",
    ...
    (tfcs_info_ptr->current_tti >= oc_tfcs_ptr->tfs_info[i].tti_info))
    {
      txbufptr->num_trch++;
      ...
      for (j=0; j < oc_tf_ptr->ntblks; j++)
      {
        ...
        txbufptr->tx_data[i].trch_data[j].pdu_offset = 0;
                                      ...
                                      mac_ul_table_type             *ul_table_ptr)
{
  ...
  for (i=0;i< nlchan;i++)
  {
    ...
    {
      WL2_MSG_1(ERROR, ul_table_ptr, "MAC_ERROR: number of logical channels - %d is greater than max logical channels allowed",
                                                  nlchan);
      break;
    }
    rlc_id = rlc_id_ptr[i];
    ...
    if (buf_status_ptr->type == FIXED_PDUS)
    {
      ...
      {
        WL2_MSG_3(HIGH, ul_table_ptr, "MAC_HS_RACH: TM lc_id %d, npdus %d, pdu_size %d",
                                                   rlc_id,
    ...
    else
    {
      ...
      if (buf_status_ptr->nbytes)
      {
        ...
        wmac_qsh_metric_r99_stat[ul_table_ptr->as_id].mac_r99ul_bo_bytes += buf_status_ptr->nbytes;
#endif
...
#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
  wmac_qsh_metric_r99_stat[ul_table_ptr->as_id].num_r99_bo_qery++;
#endif
}
...
)
{
  ...
  memset (&data_table_ptr->ul_buf_status, 0, sizeof (rlc_ul_buf_status_type));
  
#ifndef FEATURE_WCDMA_MAC_UL_ZI_REDUCTION
  data_table_ptr->rlc_ul_data_ptr = &data_table_ptr->rlc_ul_data_info;
#else
  #error code not present
  ...
  data_table_ptr->rlc_ul_data_ptr->nchan = 0;
  ...
  if (data_table_ptr->mac_ul_lc_info.nchan > 0 )
  {
    rlc_ul_enh_buffer_status(&data_table_ptr->mac_ul_lc_info,&data_table_ptr->ul_buf_status, BFI_QUERY, NULL, ul_table_ptr);
  ...
  if (ul_table_ptr->traffic_info.block_transmission && (GET_UL_CCTRCH_TYPE==RACH))
    proc_block_dtch_tx_on_rach(&data_table_ptr->ul_buf_status, ul_table_ptr);

  /* Do not send SRB info during this TTI */
  if (bld_frame_ind_ptr->srb_omit)
  {
    mac_ignore_ul_srbs (&data_table_ptr->ul_buf_status, ul_table_ptr);
  }
  rlc_profile_dump_start(ul_table_ptr->as_id,&ul_table_ptr->debug_info.rlc_tfc_profile_dump);
  /* Select suitable TFC */
  select_tfc (bld_frame_ind_ptr->tti, &data_table_ptr->ul_buf_status, &data_table_ptr->ul_rlc_frame_fmt, ul_table_ptr);
  ul_table_ptr->debug_info.rlc_tfc_profile_dump.profile_array[ul_table_ptr->debug_info.rlc_tfc_profile_dump.dump_idx].add_on3 = 
                            GET_SELECTED_TFCI;
  rlc_profile_dump_end(ul_table_ptr->as_id, &ul_table_ptr->debug_info.rlc_tfc_profile_dump);

  /* Number of RLC Logical Channels selected is greater than zero */
  if (data_table_ptr->ul_rlc_frame_fmt.nchan >0)
  {
    if (GET_UL_CCTRCH_TYPE == RACH)
    {
      /* Selected ASC */
      asc = mac_asc_selection(&data_table_ptr->ul_rlc_frame_fmt, ul_table_ptr);
      ...
      {
        /* Clear the memory */
        memset(data_table_ptr->rlc_ul_data_ptr, 0x00, sizeof(rlc_ul_frame_data_type));

        /* Get uplink RLC PDUs */
        rlc_ul_build_pdus (ul_table_ptr->as_id, &data_table_ptr->ul_rlc_frame_fmt, data_table_ptr->rlc_ul_data_ptr, ul_table_ptr->bld_frame_ind_info.cfn);
      }
      else
      {
        WL2_MSG_1(HIGH, ul_table_ptr, "There is no assigned subchan for this ASC %d",
                                                   asc);
    ...
    else
    {
      ...
      mac_profile_dump_start(ul_table_ptr);
			
      /* Get uplink RLC PDUs */
      rlc_ul_build_pdus (ul_table_ptr->as_id, &data_table_ptr->ul_rlc_frame_fmt, data_table_ptr->rlc_ul_data_ptr, ul_table_ptr->bld_frame_ind_info.cfn);

      mac_profile_dump_end(ul_table_ptr);
    }
  }

  /* Attach MAC header and ciphering parameters */
  attach_mac_parameters ( data_table_ptr->rlc_ul_data_ptr, bld_frame_ind_ptr->tx_buf_ptr, ul_table_ptr);
  ...
  for (i=0; i <UE_MAX_TRCH; i++)
  {
    ...
    if (int_tc_ptr->prev_tx_flag == FALSE)
    {
      ...
      (oc_tfcs_ptr->tfs_info[i].tf_info[oc_tfc_ptr->tfi[i]].ntblks >0) ? TRUE:FALSE;
  ...
  WL2_MSG_3(LOW, ul_table_ptr , "TFCI(%d) PDU Count (%d/%d)",
  bld_frame_ind_ptr->tx_buf_ptr->TFCI,
  data_table_ptr->rlc_ul_data_ptr->nchan, data_table_ptr->ul_rlc_frame_fmt.nchan);

  WL2_MSG_2(LOW, ul_table_ptr, "TFCI %d, ASC %d",
                                            bld_frame_ind_ptr->tx_buf_ptr->TFCI,
