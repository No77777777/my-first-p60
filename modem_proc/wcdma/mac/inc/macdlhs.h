...
Copyright (c) 2005-2006, 2008-2009 by Qualcomm Technologies, Incorporated.  All Rights Reserved.
...
$Header: //components/rel/wcdma.mpss/8.4.0/mac/inc/macdlhs.h#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
05/16/16   kv       MAC to discard rcvd_TSN, when rcvd_TSN is equal to lower edge of MAC – window
...
#endif
#define MAC_HS_DL_MAX_NUM_TSN                      (1 << hs_table_ptr->mac_hs_dl_tsn_bits)
...
#define MAC_EHS_NO_OF_TSN_LOG          10
...
extern mac_dl_table_type mac_dl_table[];
 ...
 typedef struct
 {
   ...
   boolean  mac_hs_reconfig_in_progress;
   boolean  mac_hs_reset_in_progress; /* MAC-HS reset is in progress */
 
   uint8 mac_hs_saved_tsns_cnt;
   uint8 mac_hs_old_cctrch_id;
   uint8 mac_hs_dl_tsn_bits;
 
   uint32 mac_hs_rlc_id_bitmap; /*bitmap storing RLC Ids on HS/EHS */ 
   uint32 mac_hs_holes_cnt;     /* No of holes flushed to RLC per 100 MAC-HS PDUs */
 
   /* Holds all info necessary for performing re-ordering and MAC-d de-multiplexing */
   mac_hs_reordering_entity_type    mac_hs_reordering_entity;
   
   /*---------------------------------------------------------------------------
    *   While processing the Header_Ind received from L1, the TSN and QUEUE Id
    *   are stored at a particular index and the index is passed down to L1 as
    *   part of the tb_id.
    *
    *   When tb_id is received back from L1 as part of the Data_Ind, the index
    *   is retrieved and TSN/QUEUE to which this data belongs is retrieved from
    *   the mapping table.
    *-------------------------------------------------------------------------*/
   mac_hs_tsn_to_queue_mapping_info_type
      tsn_to_queue_map[MAC_HS_DL_MAX_TSN_PER_10_MS_FRAME];
   
   mac_hs_saved_tsns_during_reconfig_type
       mac_hs_saved_tsn_data[ MAC_HS_MAX_SAVED_TSNS_DURING_RECONFIG];
   
   mac_hs_queue_config_info_type
     mac_hs_pending_config[MAC_HS_DL_MAX_QUEUE_ID + 1];
   
   /*---------------------------------------------------------------------------
    *   This table is used to keep track of the maximum number of TSNs received
    *   per QUEUE from L1 in the ISR context that have not yet been processed
    *   in the MAC-HS task context.
    *-------------------------------------------------------------------------*/
   uint16 mac_hs_dl_max_new_tsn_rcvd[MAC_HS_DL_MAX_QUEUES];

   mac_hs_dl_state_type mac_hs_dl_new_state;
   mac_hs_dl_state_type mac_hs_dl_curr_state;

   mac_hs_dl_config_req_type  mac_hs_dl_config;
#if ((defined (FEATURE_HSDPA_MAC_EHS)) && (defined (FEATURE_WCDMA_REL7)))
   
   /* Q of TSN info structures */
   /* MAC will enqueue the TSN(s) containig Partial PDU into this queue before
    * delivering to RLC. Once all the partial PDUs in this TSN are processed, 
    * this and the following non-partial pdu TSNs will be delivered in sequence
    * to RLC 
    */
   q_type mac_partial_tsn_q[MAC_HS_DL_MAX_QUEUES];
   
   uint8 macehs_num_dl_lgch;
   mac_dl_ehs_logch_config_type macehs_dl_lgch_info[UE_MAX_DL_LOGICAL_CHANNEL];
   mac_hs_e_type mac_hs_or_ehs;
      
#endif /* ((defined (FEATURE_HSDPA_MAC_EHS )) && (defined (FEATURE_WCDMA_REL7))) */

#if defined (FEATURE_HSDPA_MAC_EHS) && defined (FEATURE_WCDMA_REL7)
   /** Information for all RLC PDUs that can be constructed out of multiple partial PDUs. */
   mcalwcdma_dec_rlc_pdu_concatenate_info_struct_type mac_hs_dl_rlc_pdu_concatenate_info;
 
   a2_iovec_t a2_dec_src_iovec_list[MCALWCDMA_DEC_MAX_DMOV_SOURCE_DESCRIPTOR_LEN_MOVE_IN];
#endif /*defined (FEATURE_HSDPA_MAC_EHS) && defined (FEATURE_WCDMA_REL7)*/

#ifdef FEATURE_WCDMA_HS_FACH
   rex_timer_type mac_hs_reset_base_timer;   /* 10 ms base timer - used for TRESET */
   boolean mac_hs_reset_base_timer_running;   
   boolean mac_hs_reset_timer_valid_state;
   boolean mac_hs_pending_reset_timer_valid_state;
#endif

   mac_hs_dbg_enum_type mac_hs_logging;

#ifdef FEATURE_MAC_HS_QXDM_LOGGING
 /* Storage for collecting upto a maximum of 100 Status Reports for logging */
   mac_hs_log_status_info_type *mac_hs_status_pkt_list;

   uint8  mac_hs_status_pkt_cnt; /* Number of status reports collected so far */
   
 /* Timer block for 1 sec timer for status report log packet. 
    Status report log packets are sent out either every sec or when 100 reports have
    accumulated, whichever happens first */
   rex_timer_type   mac_hs_dl_status_log_timer_blk;
 
#if defined (FEATURE_HSDPA_MAC_EHS) && defined (FEATURE_WCDMA_REL7)
   mac_ehs_log_tsn_buf_type *mac_ehs_tsn_log_buf;
#endif
#endif /*FEATURE_MAC_HS_QXDM_LOGGING*/

#if ((defined (FEATURE_HSDPA_MAC_EHS)) && (defined (FEATURE_WCDMA_REL7)))
   uint32 mac_ehs_logging; /* set to 1 for controlled logging in EHS path */
   uint32 mac_ehs_p_to_c_logging;
#endif
   rex_tcb_type* wcdma_mac_hs_dl_tcb_ptr; /* WCDMA MAC HS DL TCB handle */
 }mac_hs_table_type;

extern mac_hs_table_type mac_hs_table[]; 
#define GET_MAC_HS_TABLE_PTR(as_id) &(mac_hs_table[as_id]);
...
/*--------------------------------------------------------------------------
 * Routine that indicates if T1 is before than (<) T2 and greater than or 
 * equal to the lower edge of the window. Here t2 HAS to be the next expected
 * TSN and rcvd_tsn(t1) should already be checked if it's outside the window
 *-------------------------------------------------------------------------*/
...
#define MAC_HS_RLC_ID_SET_BITMAP(bitmap, rlc_id)\
  bitmap = (bitmap | (1 << rlc_id))
...
#define MAC_HS_FACH_OR_PCH_STATE()  (hs_table_ptr->mac_hs_reset_timer_valid_state)
...
#define MAC_HS_GET_SAVED_TSN_PTR(i, tsn_info_ptr) \
        tsn_info_ptr = hs_table_ptr->mac_hs_saved_tsn_data[i].tsn_info 

#define MAC_HS_PUT_SAVED_TSN_PTR(i, tsn_info_ptr) \
        hs_table_ptr->mac_hs_saved_tsn_data[i].tsn_info = tsn_info_ptr 
...
#define MAC_HS_GET_REORDERING_QUEUE_PTR(int_queue_ptr,queue_id) \
         int_queue_ptr = hs_table_ptr->mac_hs_reordering_entity.reordering_queue[queue_id]

#if ((defined (FEATURE_HSDPA_MAC_EHS )) && (defined (FEATURE_WCDMA_REL7)))

#define HSPA_PLUS_MAC_HS_TYPE_VALID_CHECK(mac_hs_or_ehs, table_ptr) \
if(mac_hs_or_ehs == MAC_HS_TYPE_INVALID) \
{ \
  WL2_MSG_0(ERROR, table_ptr, "EHS_ERR: MAC_HS_TYPE is Invalid"); \
}
...
SIDE EFFECTS:
  None
===========================================================================*/
void process_dl_hs_dsch_tbs_hdr
(
  uint8 tbs_id,
...
SIDE EFFECTS:
  None
===========================================================================*/
void mac_hs_dl_init(sys_modem_as_id_e_type as_id);
...
SIDE EFFECTS:
  None
===========================================================================*/
void mac_hs_dl_data_handler( mac_hs_table_type *hs_table_ptr );
...
SIDE EFFECTS:
  None
===========================================================================*/
void mac_hs_dl_timer_expiry_handler(uint8 queue_id, mac_hs_table_type *hs_table_ptr);
...
void mac_hs_queue_init(uint8 queue_id, mac_hs_table_type *hs_table_ptr);
...
SIDE EFFECTS:
  None
===========================================================================*/
void mac_hs_queue_free(uint8 queue_id, mac_hs_table_type *hs_table_ptr);
...
SIDE EFFECTS:
  None
===========================================================================*/
void mac_hs_dl_init_ZI_opt( mac_hs_table_type *);
...
SIDE EFFECTS:
  None
===========================================================================*/
void mac_hs_dl_free_mem_ZI_opt( mac_hs_table_type * );
...
SIDE EFFECTS:
  None
===========================================================================*/
void process_dl_hs_dsch_config(mac_dl_config_type *cfgptr, mac_hs_table_type *hs_table_ptr);
...
SIDE EFFECTS:
  None
===========================================================================*/
void mac_hs_dl_start_reset_req( mac_hs_table_type *hs_table_ptr );
...
SIDE EFFECTS:
  None
===========================================================================*/
void mac_hs_dl_finish_reset_req( mac_hs_table_type *hs_table_ptr );
...
SIDE EFFECTS:
  None
===========================================================================*/
void mac_hs_dl_validate_config_req(mac_dl_config_type *cfgptr, mac_hs_table_type *hs_table_ptr);
...
SIDE EFFECTS:
  None
===========================================================================*/
void mac_hs_dl_config_req_handler(mac_hs_table_type *hs_table_ptr);
...
SIDE EFFECTS:
  None
===========================================================================*/
void mac_hs_dl_send_hs_config_log_packet(mac_dl_config_type *cfgptr, mac_hs_table_type *hs_table_ptr);
...
SIDE EFFECTS:
  None
===========================================================================*/
void mac_hs_dl_status_log_timer_expiry_handler(mac_hs_table_type *hs_table_ptr);

#endif /* FEATURE_MAC_HS_QXDM_LOGGING  */

void mac_hs_dl_reset_local_config_data(mac_hs_table_type *hs_table_ptr);
#if ((defined (FEATURE_HSDPA_MAC_EHS )) && (defined (FEATURE_WCDMA_REL7)))
/*===========================================================================
...
void mac_enhs_dl_reset_local_config_data(mac_hs_table_type *hs_table_ptr);
#endif 
...
void process_dl_hs_enh_dsch_tbs_data(uint8 *mac_hs_num_tsn_rcvd, mac_hs_table_type *hs_table_ptr);
...
void mac_ehs_provide_partial_to_complete_info(uint16 allowed_length,
            mcalwcdma_dec_rlc_pdu_concatenate_info_struct_type *rlc_p_to_c_ptr, mac_hs_table_type *hs_table_ptr);
...
uint8 mac_ehs_dl_get_rlc_id(uint8      mac_id, mac_hs_table_type *hs_table_ptr);
...
void mac_ehs_update_queue_due_to_invalid_combo(uint8 queue_id, 
                                              l1_dec_hs_tsn_info_type *clear_upto_tsn_ptr, mac_hs_table_type *hs_table_ptr);
...
SIDE EFFECTS:
  None
===========================================================================*/
boolean mac_ehs_dl_pass_tsn_pdus_to_partial_tsn_q(l1_dec_hs_tsn_info_type *tsn_info_ptr, mac_hs_table_type *hs_table_ptr);
...
void mac_hs_discard_dsm_item(dsm_item_type **dsm_ptr, mac_hs_table_type *hs_table_ptr);
...
void mac_hs_tsn_empty_recovery( mac_hs_table_type *hs_table_ptr );


#if ((defined (FEATURE_HSDPA_MAC_EHS)) && (defined (FEATURE_WCDMA_REL7)))
void mac_ehs_dl_send_hs_config_log_packet(mac_dl_config_type *cfgptr, mac_hs_table_type *hs_table_ptr);
...
SIDE EFFECTS:
  None
===========================================================================*/
void mac_ehs_pass_comp_pdu_to_rlc(mac_hs_table_type *hs_table_ptr);
...
SIDE EFFECTS:
  None
===========================================================================*/
void mac_hs_reset_base_timer_handler(mac_hs_table_type *hs_table_ptr);
...
void mac_hs_reset_the_reordering_queue (mac_hs_queue_info_type* int_queue_ptr, mac_hs_table_type *hs_table_ptr);
...
SIDE EFFECTS:
  None
===========================================================================*/
void process_dl_hs_enh_dsch_tm_data(l1_dec_hs_tsn_info_type *tsn_info_ptr, mac_hs_table_type *hs_table_ptr);
...
===========================================================================*/
void wcdma_mac_hs_dl_init_tcb(mac_hs_table_type *hs_table_ptr);
...
/*===========================================================================
 ...
 SIDE EFFECTS
   An assertion will fail if the lookup fails.
===========================================================================*/
rex_tcb_type* wcdma_mac_hs_dl_get_tcb(mac_hs_table_type *hs_table_ptr);
...
/*===========================================================================
 ...
 SIDE EFFECTS
   None
===========================================================================*/
boolean mac_dl_hs_check_pending_tsns(mac_hs_table_type *hs_table_ptr);
