...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/srchcmmeasparms.h#3 $ $DateTime: 2020/01/22 08:56:27 $ $Author: pwbldsvc $

when        who     what, where, why
--------    ---     --------------------------------------------------------
08/25/15    cc      Add X2G Stop duration for X2G cleanup
...
#define SRCHCM_GSM_T_START_QS_PWR_SCAN (GL1_HW_CM_X2G_STARTUP_CMD_DURATION_QS(WL1_MSIM_GET_AS_ID_FOR_G()))
/* This includes the time required for cleanup */
#define SRCHCM_GSM_T_CLEAN_QS_PWR_SCAN ((GL1_HW_CM_CLEANUP_CMD_OVERHEAD_QS) + (GL1_HW_CM_X2G_CLEANUP_CMD_DURATION_QS(WL1_MSIM_GET_AS_ID_FOR_G())))

#define SRCHCM_GSM_T_X2G_STOP_QS (GL1_HW_CM_X2G_STOP_CMD_DURATION_QS(WL1_MSIM_GET_AS_ID_FOR_G()))
...
#define SRCHCM_GSM_PWR_SCAN_BURST_MEAS_WIN_SIZE_QS ((GL1_HW_CM_X2G_PWR_MEAS_CMD_OVERHEAD_QS(WL1_MSIM_GET_AS_ID_FOR_G())) + (SRCHCM_GSM_PWR_SCAN_BURST_SIZE_QS))

/* T start and cleanup for FCCH detect */
#define SRCHCM_GSM_T_START_QS_FCCH_DETECT (((GL1_HW_CM_X2G_STARTUP_CMD_DURATION_QS(WL1_MSIM_GET_AS_ID_FOR_G()))) + (GL1_HW_CM_X2G_ACQ_CMD_OVERHEAD_QS(WL1_MSIM_GET_AS_ID_FOR_G())))
#define SRCHCM_GSM_T_CLEAN_QS_FCCH_DETECT ((GL1_HW_CM_CLEANUP_CMD_OVERHEAD_QS) + ((GL1_HW_CM_X2G_CLEANUP_CMD_DURATION_QS(WL1_MSIM_GET_AS_ID_FOR_G()))))

/* T start and cleanup for SCH detect */
#define SRCHCM_GSM_T_START_QS_SCH_DEC (((GL1_HW_CM_X2G_STARTUP_CMD_DURATION_QS(WL1_MSIM_GET_AS_ID_FOR_G()))) + (GL1_HW_CM_X2G_ASYNC_RX_CMD_OVERHEAD_QS(WL1_MSIM_GET_AS_ID_FOR_G())))
#define SRCHCM_GSM_T_CLEAN_QS_SCH_DEC ((GL1_HW_CM_CLEANUP_CMD_OVERHEAD_QS) + ((GL1_HW_CM_X2G_CLEANUP_CMD_DURATION_QS(WL1_MSIM_GET_AS_ID_FOR_G()))))
/* SCH burst size */
