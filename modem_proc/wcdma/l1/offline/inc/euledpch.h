...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/euledpch.h#3 $
...
08/14/15   ar      Make sure QXDM command to disable power compression is backwards compatible
07/29/15   ar      Ensure that power compression is enabled by default for Secondary carrier
07/22/15   kv      Adding eul_calc_frnum_or_subfn in l1macdata.h
07/21/15   ssv     Enhancement to AGCH monitoring after ROTA gap
07/17/15   cjz     Disable max power reduction (MPR) re-computation for single carrier EUL
07/02/15   ar      Added support for WL UL Feature bitmask NV
06/23/15   gp      Compilation and logging fixes for FEATURE_WCDMA_DC_HSUPA undefined
06/08/15   cjz     DC-HSUPA and UL-16QAM logging coexistence
05/23/15   vr      Debug code to limit the ETFCI: send_data 75 4 24 0 65 0xXX 0x00 0x00 0x00
05/22/15   vr      Remove support to reuse SS tables, always rebuild.
04/22/15   gp      Fix to avoid clearing of indication flag for achievable TPUT estimation from DS
03/28/15   vr      Correctlng 0x4346/0x4323/0x4322 flush logic when log buffer in not full
03/19/15   ac      FR 25733: UL/DL throughput reporting 
03/11/15   rsr     Phase 4 W+W Checkin.
03/11/15   vr      Enhancing 0x4035 to log per carrier per slot TXAGC.
03/05/15   vr      API to L2 indicating ratio between DPCCH to EUL power.
02/20/15   vr      QXDM command support for DC-HSUPA activation via fake hs-scch order 
02/19/15   vr      A QXDM command to set FW async debug interface for capturing FW modulator output logging
...
/* Number of HARQ processes for 10ms E-TTI */
#define EUL_NUM_10MS_HARQ  8

/* Invalid datarate for EUL */
#define EUL_INVALID_DATA_RATE 0xFFFF

#ifdef FEATURE_WCDMA_DC_HSUPA
#define IS_DCHSUPA_SEC_CARR_ACTIVE() ((eulcfg_ext_cntrl_params.e_ul_sec_edpch_state == E_UL_SEC_EDPCH_STATE_TX) || \
                                      (eulcfg_ext_cntrl_params.e_ul_sec_edpch_state == E_UL_SEC_EDPCH_STATE_DISABLED) || \
                                      (eulcfg_ext_cntrl_params.e_ul_sec_edpch_recfg_state == E_UL_SEC_CFG_WHEN_ACTIVE))
#else
#define IS_DCHSUPA_SEC_CARR_ACTIVE() (FALSE)
...
typedef struct
{
  /* 0 or 1 for double buffer, EULEDPCH_DBL_BUF_NEITHER_ACTIVE for no-active
   buffer */
  uint8   curr_tab_index;

  /* current inactive table index */
  uint8  inactive_tab_index;
...
/* Enumeration for different QXDM commands for disabling power compression */
typedef enum
{
  WL1_EUL_QXDM_POWER_COMP_ENABLE,
  WL1_EUL_QXDM_POWER_COMP_DISABLE,
  WL1_EUL_QXDM_POWER_COMP_NUM_STATES
} wl1_eul_qxdm_power_comp_enum_type;
...
typedef struct {
  ...
  /*Signal to start accumulating power limited data rate for EUL */
  boolean filter_power_limited_data_rate;

  /* Filtered maximum achievable data rate based on power in bits per second */
...
typedef enum
{
  WL1_ENABLE_MPR_BUT_DISABLE_EMPR = 0,
  WL1_DISABLE_MPR                 = 1,
  WL1_ENABLE_MPR_AND_ENABLE_EMPR  = 2
} eul_mpr_empr_enum_type;
#endif /*defined FEATURE_WCDMA_THOR_MODEM !! FEATURE_WCDMA_TABASCO_MODEM*/
...
extern boolean euledpch_validate_nw_config(e_ul_cfg_info_struct_type* config_param_ptr, wsub_id_e_type wsub_id);
...
typedef enum
{
  EUL_IRQ_DELAY_DTX_NO_DELAY                             = 0, /* FW Async params and PDU both on time    */
  EUL_IRQ_DELAY_DTX_ASYNC_PARAM_WRITE_DELAYED            = 1, /* FW Async params delayed, PDU is on time */
  EUL_IRQ_DELAY_DTX_SW_PDU_BUILDING_DELAYED              = 2, /* FW Async params on time, PDU is delayed */
  EUL_IRQ_DELAY_DTX_ASYNC_PARAM_AND_PDU_BUILDING_DELAYED = 3, /* FW Async params and PDU both delayed    */
  EUL_IRQ_DELAY_DTX_NUM_OPS
} eul_irq_delay_dtx_enum_type;

typedef enum
{
  EUL_DEBUG_DISABLE_ETFC_PR = 0,
  EUL_DEBUG_F3_MSG_BITMASK = 1, 
  EUL_DEBUG_TRIGGER_EDCH_CRC_ERROR = 3,
  EUL_DEBUG_DISABLE_MPR = 5,
  EUL_DEBUG_DISABLE_HDET_AND_MPR = 7,
  EUL_DEBUG_TOGGLE_POWER_COMPRESSION = 9,
  EUL_DEBUG_ENABLE_A2_ERAM_DATA_DUMP = 20,
  EUL_STATS_UPDATE_APS_VOTING = 21,
  EUL_DEBUG_TRIGGER_FW_PANIC_CMD = 50, /* Send Panic CMD to FW */
  EUL_DEBUG_HS_RACH_LOGGING_ENABLED = 53,
  EUL_DEBUG_ENABLE_L2_DATA_DUMP = 55,
  EUL_DC_HSUPA_F3S_ENABLE = 57, 
  EUL_DC_HSUPA_FORCE_ORDER_ACTIVATION = 58,
  EUL_DC_HSUPA_FORCE_HHO_ACTIVATION = 59,
  EUL_STATS_USE_TPUT_FILTER_COEFF = 60, 
  EUL_STATS_TPUT_DATA_LOGGING_F3 = 61,
  EUL_16_QAM_DEBUG = 62,
  EUL_DEBUG_SW_FW_DEBUG_ASYNC = 63,
  EUL_DEBUG_TRIG_SCCH_ORD = 64,
  EUL_DEBUG_LIMITING_ETFCI = 65,
  EUL_DEBUG_ENABLE_Q6_SCALING = 66,
  EUL_DEBUG_NUM_OPS
} eul_debug_switch_type;

typedef struct {
   /* CM parameters of the next EDCH frame*/
   l1_cm_ul_param_struct_type ul_cm_param;
   /*flag to know that next frame is in CM*/
   boolean next_tti_is_in_cm_frame;
   /*flag to indicare that CM factor update is needed*/
   boolean need_cm_factor_update;
    /* Is the first TX of a HARQ in CM frame for 10 ms TTI ?
   * indexed by HARQ ID 0..7 */
   boolean first_10ms_tx_is_cm[EUL_NUM_10MS_HARQ];
   /* cm_tab_idx = 15 - num_tx_slots for first TX in case of 10ms TTI
   * indexed by HARQ ID 0..7 */
   uint8 first_10ms_tx_cm_tab_idx[EUL_NUM_10MS_HARQ];
   /*Number of pilot bits per slot in compressed mode*/
   uint8 first_10ms_tx_Npilot[EUL_NUM_10MS_HARQ];
   /*Number of pilot bits per slot in compressed mode.retx*/
   uint8  retx_Npilot;
   /*flag to know that next frame is in CM for 2 ms TTI*/
   boolean next_2ms_tti_is_cm ;
   uint8  last_cm_slot_plus_one;
   uint8  num_cm_slots_txed_in_2ms_subfr;
} l1_edch_cm_info_struct_type;
...
typedef struct
{
  ...
  /* double buffer of semi-static tables of E-TFC coding */
  eul_etfc_coding_tab_struct_type  etfc_coding_tab_buf[2];
} eul_etfc_coding_tab_dbl_buf_struct_type;

typedef struct
{
  /* 0 or 1 for double buffer, EULEDPCH_DBL_BUF_NEITHER_ACTIVE for no-active
  buffer */
  uint8   curr_active_buf_idx;

  /* buffer to update at config time */
  uint8   spare_buf_idx;

  /* Whether need to switch buffers at activation
   * Actually always to switch for ETFC coding tables
   * Just to keep uniformity and easy code maintainence */
  boolean need_to_switch;

  /* Is table built and ready for activation */
  boolean table_ready;

  /* Derived L1 EDPCH MAX chan code and PL_MAX */
  eul_max_rate_cfg_param_struct_type max_rate_param[2];
} eul_max_rate_cfg_param_dbl_buf_struct_type;

#ifdef FEATURE_WCDMA_DC_HSUPA
typedef struct
{
  /* flag to indicate if UPH is ever updated
  **  if TRUE, use cir buffer(existing code) to calc UPH from maint evt handler
  **  if FALSE, calculate instantaneous UPH from FIQ*/
  boolean is_uph_updated;

  /* filtered uph to report */
  int32  curr_sec_uph;

  /* slot index in a frame for both MTPL and DPCCH, range 0..14*/
  uint32  pwr_arr_slot_idx;

  /* element in circular buffer to update next , range 0..9*/
  uint32  cir_buffer_idx;

  /*to read and save values from FW, Sec DPCCH pwr every slot*/
  int16  sec_dpcch_pwr_db10[SLOTS_PER_FRAME];

  /*to read and save values from FW for MTPL every slot*/
  int16  mtpl_db10[SLOTS_PER_FRAME];

  /*Average Linear pwr calc for Sec DPCCH over last frame or valid 15 slots
   ** CPC DTX slots or CM slots are not considered*/
  uint64 sec_dpcch_pwr_linear[EULEDPCH_NUM_UPH_MEAS_FILTER_FR];

  /*Average MTPL calc for Sec DPCCH over last frame or valid 15 slots*/
  uint64 mtpl_linear[EULEDPCH_NUM_UPH_MEAS_FILTER_FR];
} euledpch_sec_ups_meas_struct_type;
#endif /* FEATURE_WCDMA_DC_HSUPA */
...
typedef struct
{
  ...
  eul_pwr_SGP_val_interp_offset_tab_struct_type  pwr_SGP_val_interp_offset_tabs_buf[2];
} eul_pwr_SGP_val_interp_offset_tab_dbl_buf_struct_type;
...
typedef struct
{
  ...
  /* double buffer of semi-static tables of power CM factors */
  eul_pwr_cm_factor_tab_struct_type*  pwr_cm_factor_tab_ptrs_buf[2];
} eul_pwr_cm_factor_tab_dbl_buf_struct_type;

#ifdef FEATURE_WCDMA_DC_HSUPA
/* SW database to store power gains read from FW. */
typedef struct
{
  /* DPCCH power in dB10. */
  int16  dpcch;
  /* BETA-C linear gain. */
  uint16 betaCgain;
  /* BETA-C linear gain for HSDPCCH half slot. */
  uint16 betaCaltGain;
  /* RateAdj linear gain. */
  uint32 sumBetaSqGain;
  /* RateAdj linear gain for HSDPCCH half slot. */
  uint32 sumBetaSqAltGain;
} eul_pwr_per_slot_struct_type;

/* SW database is double buffered. Above values are stored per carrier,
** over 15 slots at a time. Once 15 slots worth of data is accumulated
** TxAGC per carrier is then calculated. */
typedef struct
{
  uint8                        idx;
  eul_pwr_per_slot_struct_type eul_pwr_per_slot[2][WL1_MAX_UL_CARR_IDX][SLOTS_PER_FRAME];
} eul_pwr_per_frame_struct_type;
#endif /*FEATURE_WCDMA_DC_HSUPA */

typedef struct{
 /* number of new EDCH transmissions. */
 uint8 num_edch_new_tx; 
 /* number of tti of stats collected so far. */
 uint8 num_tti;
 /* number of tti monitored to update APS voting*/
 uint16 num_samples;  
 /* Threshold to turn ON APS voting */
 uint16 threshold_edch_new_tx; 
}eul_aps_stats_struct_type;

typedef struct
{
  /* Local Mutex for the protection of EUL cfg params in ISR and task context*/
  rex_crit_sect_type e_ul_cfg_params_mutex;
  /* Local Mutex for the protection of EUL state params in ISR and task context*/
  rex_crit_sect_type e_ul_state_params_mutex;
  /* Mutex for protection of EUL ISR firing time reconfiguration from 2ms TTI 
  ** This is only applicable if EUL ISR is fired by SW. */
  rex_crit_sect_type e_ul_isr_firing_time_mutex;

  /* Timer to detect FW stuck after EDL reconfig*/
  timer_type e_dl_config_timeout_timer;

  eul_pwr_SGP_val_tab_dbl_buf_struct_type *eul_pwr_SGP_val_tab_dbl_buf_ptr;
  /* current in use semi-static table of E-TFC coding */
  eul_etfc_coding_tab_struct_type         *eul_curr_etfc_coding_tab_ptr;
  /* current in use semi-static table of power CM factors */
  eul_pwr_cm_factor_tab_struct_type       *eul_curr_pwr_cm_factor_tab_ptr;
  eul_pwr_R_val_tab_dbl_buf_struct_type   *eul_pwr_R_val_tab_dbl_buf_ptr;
  /* current in use derived L1 EDPCH max_chan_code and pl_max */
  eul_max_rate_cfg_param_struct_type      *eul_curr_max_rate_cfg_param_ptr;
  /* current in use semi-static table of R_j and R_combo */
  eul_pwr_R_val_tab_struct_type           *eul_curr_pwr_R_val_tab_ptr;

  WfwTxEULAsyncReadStruct        *wfw_intf_tx_eul_async_read_ptr[WL1_MAX_UL_CARR_IDX];
  WfwTxEULAsyncWriteStruct       *wfw_intf_tx_eul_async_write_ptr[WL1_MAX_UL_CARR_IDX];
  WfwTxEULCommonAsyncReadStruct  *wfw_intf_tx_eul_common_async_read_ptr;
  WfwTxEULCommonAsyncWriteStruct *wfw_intf_tx_eul_common_async_write_ptr;

  /* P2 and Data mover HW */
  l1_l2_data_type l1_l2_data_buf;
  uint32 l1_p2_ul_ctrl_wrd[EULENC_MAX_NUM_PDU_BLD];
  uint32 l1_p2_data_buf[2*EULENC_MAX_NUM_PDU_BLD];

  uint16  debug_etfci_value;
  boolean cpc_dtx_mac_index;
  /*Global to maintain the pduDoneCnt written Via A2*/
  uint32  ul_update_pdu_count;
  uint8   curr_harq_idx[MAX_NUM_DCHSUPA_ACTIVE_CARR];
  uint16  euledpch_cfn_to_suspend_bfi;
  /* SLAMMING THE PDU DONE TIME IN Cx8*/
  uint32  e_ul_pdu_done_time_cx8;
  /*Initialized to 1 as it's toggled before populating */
  uint16  empr_param_db_idx;
  /* EDCH SRB delay state */
  euledpch_srb_state_enum_type euledpch_srb_state;
  /* history buffer for DPCCH_Initial_Pwr + CM_delta for ETFC PR and UPH
  ** mod 4 is much easier than mod 3 */
  int32  euledpch_dip_cm_delta_hist[EUL_HARQ_RTT_10MS];
  /* history buffer for MTPL_no_mpr for ETFC PR */
  uint16 euledpch_mtpl_no_mpr_hist[EUL_MAX_HARQ_RTT];
  euledpch_ups_meas_struct_type euledpch_ups_meas_buf;

  /* Double buffer to store the EUL RF TX log Pkt*/
  UPLINK_TX_RF_DEBUG_LOG_PKT_type ul_tx_rf_log_pkt[2];

  uint16 cpc_dtx_mac_inactivity_counter[WL1_MAX_UL_CARR_IDX][E_UL_LOG_DOUBLE_BUF][E_UL_LOG_MAX_SFN_INDEX];

  #ifdef FEATURE_WCDMA_REL7_EDPCCH_POWER_BOOST
  /* Array to store sum of EDPCCH and EDPDCH powers over which ETFCI 
  ** restriction due to MP is done. */
  uint64 *pwr_A_ec_norm_R_comb_sum_tab;
  /* E-DPCCH power boost*/
  boolean apply_boost;
  /* Pointer to the current pwr Aec table*/
  eul_pwr_A_ec_norm_tab_struct_type *eul_curr_pwr_A_ec_norm_tab_ptr;

    #ifdef FEATURE_WCDMA_ZI_OPT_PHASE2
    eul_pwr_A_ec_norm_tab_dbl_buf_struct_type *eul_pwr_A_ec_norm_tab_dbl_buf_ptr;
    #endif
  #endif /* FEATURE_WCDMA_REL7_EDPCCH_POWER_BOOST */

  #ifdef FEATURE_WCDMA_DC_HSUPA 
  /* Local Mutex for the protection of Sec EUL cfg params in ISR and task context*/
  rex_crit_sect_type e_ul_sec_cfg_params_mutex;
  /* Local Mutex for the protection of Sec EUL state params in ISR and task context*/
  rex_crit_sect_type e_ul_sec_state_params_mutex;

  uint16 euledpch_sec_ul_cfn_to_suspend_bfi;

  WCDMA_MULTI_CARR_EUL_MPR_LOG_PACKET_type eul_dc_new_mpr_log_pkt;
  /*Back up data buffer for 0x435A*/
  eul_power_alloc_dc_logging_sample dc_pow_alloc_backup[3];
  WCDMA_MULTI_CARR_EUL_POWER_ALLOC_LOG_PACKET_type eul_dc_power_alloc_log_pkt;
  #endif /* FEATURE_WCDMA_DC_HSUPA */

  #ifdef FEATURE_WCDMA_REL7P1_MPR
  /* Flag to enable debug logging for MPR 7.1*/
  boolean debug_rel7_mpr_logging;
  #endif /* FEATURE_WCDMA_REL7P1_MPR */

  #if defined(FEATURE_QTA) && defined(FEATURE_WCDMA_CPC_DTX)
  int16 e_ul_cpc_dtx_qta_block_cnt[WL1_MAX_UL_CARR_IDX];
  #endif /* defined(FEATURE_QTA) && defined(FEATURE_WCDMA_CPC_DTX) */

  #if defined(FEATURE_QTA) && defined(FEATURE_WCDMA_CPC_DRX)
  /* UE is expected to check max(T,K) right after coming out of gap. This flag, tells whther UE was in gap or not.\
  ** This is set to TRUE only when CPC-DRX is actove and there is a QTA gap.\
  ** If false is found, UE behaviour is same as non-QTA */
  boolean e_ul_cpc_drx_was_in_qta_gap;
  boolean e_ul_cpc_drx_was_in_rota_gap;
  #endif /* defined(FEATURE_QTA) && defined(FEATURE_WCDMA_CPC_DRX) */

  #ifdef FEATURE_WCDMA_CPC_DTX
  uint16 eul_cpc_dtx_cache_sgp[WL1_MAX_UL_CARR_IDX];
  /*cache buffer of max etfci due to pwr and SGP for MAC BO query*/
  uint8  eul_cpc_dtx_cache_max_etfci_pwr[WL1_MAX_UL_CARR_IDX];
  #endif /* FEATURE_WCDMA_CPC_DTX */

  #ifdef FEATURE_WCDMA_MODEM_TPUT_STATS_API 
  /* Internal structure to hold data rate and plus flag stats */
  euledpch_stats_struct_type eul_stats;
  /* Mutex lock for resetting plus flag variables */
  rex_crit_sect_type eul_stats_lock;
  #endif /* FEATURE_WCDMA_MODEM_TPUT_STATS_API */
  ...
  /* This is used to collect EUL stats used to update APS voting */
  eul_aps_stats_struct_type eul_aps_info;
} euledpch_ext_cntrl_params_type;

extern euledpch_ext_cntrl_params_type euledpch_ext_cntrl_params;

typedef struct
{
  /* Global to store if EUL APS stats are updated through Qxdm cmd */
  boolean update_eul_aps_stats;
  /*set the received val to pwr restriction flag*/
  boolean debug_disable_eul_etfc_pr;
  /*type of f3 message to be enabled based on val*/
  uint32  DEBUG_EUL_DL_LOGGING;
  boolean euledpch_crc_error_enabled;
  #if defined(FEATURE_WCDMA_THOR_MODEM) || defined(FEATURE_WCDMA_TABASCO_MODEM)
  eul_mpr_empr_enum_type l1_debug_disable_mpr; // default = WL1_ENABLE_MPR_BUT_DISABLE_EMPR
  #else
  boolean l1_debug_disable_mpr;
  #endif /*defined FEATURE_WCDMA_THOR_MODEM !! FEATURE_WCDMA_TABASCO_MODEM*/
  boolean euledpch_debug_power_compresn_disabled;
  boolean eulenc_a2_cb_dump_tbs;
  boolean eulenc_a2_cb_disable_dump_tbs_req;
  boolean eulenc_debug_input_data_dump;
  boolean eulenc_debug_input_data_dump_level2;

  #ifdef FEATURE_WCDMA_HS_RACH
  /* Set Global Variable to trigger PANIC to FW and EF SW*/
  boolean hsrach_debug_enable_f3;
  #endif /*FEATURE_WCDMA_HS_RACH*/

  #ifdef FEATURE_WCDMA_16_QAM
  /* send_data 75 4 24 0 62 X X X X */
  uint32  eul_16_qam_qxdm_debug;
  #endif /* FEATURE_WCDMA_16_QAM */

  #ifdef FEATURE_WCDMA_DC_HSUPA
  uint32  debug_eul_dc_hsupa_f3s_enable_bmsk;
  boolean dc_hsupa_activate_without_order;
  uint8   rumi_ignore_disable_tx;
  #endif /* FEATURE_WCDMA_DC_HSUPA  */

  #ifdef FEATURE_WCDMA_MODEM_TPUT_STATS_API
  uint32  WCDMA_MODEM_STATS_LOGGING;
  ...
  uint8   debug_etfci_limit;
  boolean   debug_eul_q6_scaling_disable;
} euledpch_qxdm_cntrl_params_type;

extern euledpch_qxdm_cntrl_params_type euledpch_qxdm_cntrl_params;

typedef struct
{
   /* Records whether PDU got built in TTI X. Used to increment pduConeCnt to FW */
   boolean eul_pdu_built_in_prev_frame;
   uint32	edpch_sw_maint_pdu_done_dtx_cnt[WL1_MAX_UL_CARR_IDX];
   uint32	edpch_sw_maint_param_dtx_cnt[WL1_MAX_UL_CARR_IDX];
   /* This variable keeps the cm info for the current TTI and for the last one HARQ RTT
    ** frames cm info */
   l1_edch_cm_info_struct_type edch_cm_info;
	
   /* This variable keeps the cm info for the last TTI */
   l1_edch_cm_info_struct_type edch_prev_tti_cm_info;
   #ifdef FEATURE_WCDMA_DC_HSUPA 
     euledpch_sec_ups_meas_struct_type euledpch_sec_ups_meas_buffer;
   #endif /* FEATURE_WCDMA_DC_HSUPA */

   #ifdef FEATURE_WCDMA_REL7
   /* double buffer of semi-static tables of SGP second req table for interpolation */
     eul_pwr_SGP_val_interp_offset_tab_dbl_buf_struct_type eul_pwr_SGP_val_interp_offset_tab_dbl_buf;
   #endif /* FEATURE_WCDMA_REL7 */

} euledpch_int_cntrl_params_mdump_type;

typedef struct
{
  euledpch_int_cntrl_params_mdump_type euledpch_int_cntrl_params_mdump;
  /* double buffer of semi-static tables of E-TFC coding */
  eul_etfc_coding_tab_dbl_buf_struct_type eul_etfc_coding_tab_dbl_buf;
  /* current in use semi-static table of SGP */
  eul_pwr_SGP_val_tab_struct_type        *eul_curr_pwr_SGP_val_tab_ptr;
  eul_max_rate_cfg_param_dbl_buf_struct_type eul_max_rate_cfg_param_dbl_buf;
  /* double buffer of semi-static tables of power CM factors */
  eul_pwr_cm_factor_tab_dbl_buf_struct_type  eul_pwr_cm_factor_tab_dbl_buf;

  /* Parameters for MAC to do E-TFC selection */
  l1_eul_serving_grant_update_info_type edpch_sg_info[WL1_MAX_UL_CARR_IDX];
  mdspasync_edpch_param_db_struct_type  edpch_fw_params_tab[WL1_MAX_UL_CARR_IDX][EULSG_MAX_NUM_HARQ];

  /* Number of scheduled data bits per HARQ */
  uint16  euledpch_sched_num_bits_tab[WL1_MAX_UL_CARR_IDX][EULSG_MAX_NUM_HARQ];
  boolean debug_disable_hw_write;
  /* The parameters passed to calculate MPR values */
  ul_mpr_calc_parms mpr_params;

  ul_tx_rf_log_pkt_struct_type *ul_tx_rf_log_pkt_sample_ptr;
  uint8   ul_tx_rf_logging_buf_idx;
  int16   ul_tx_rf_log_tx_agc_buf[3];
  uint8   ul_tx_rf_log_tx_agc_buf_idx;
  uint8   ul_rf_tx_submit_buf_idx;

  #ifdef FEATURE_WCDMA_DC_HSUPA 
  /*Counter to monitor consecutive frames that sec DPCCH pwr is invalid */
  uint8  avg_dpcch_invalid_cfns;
  uint64 power_allocated[WL1_MAX_UL_CARR_IDX];

  ul_sec_tx_rf_log_pkt_struct_type *ul_sec_tx_rf_log_pkt_sample_ptr;
  eul_pwr_per_frame_struct_type eul_pwr_per_frame;
  #endif /* FEATURE_WCDMA_DC_HSUPA */

  #ifdef FEATURE_WCDMA_REL7
  /* current in use semi-static table for second part of SGP calcualation
  ** for interpolation*/
  eul_pwr_SGP_val_interp_offset_tab_struct_type        *eul_curr_pwr_SGP_val_interp_offset_tab_ptr;
  #endif /* FEATURE_WCDMA_REL7 */

  #ifdef FEATURE_WCDMA_CPC_DTX
  l1_edch_cm_info_struct_type edch_dtx_cm_info;
  /*This variable keeps the cm info for the last frame*/
  l1_edch_cm_info_struct_type edch_prev_tti_dtx_cm_info;
  #endif /* FEATURE_WCDMA_CPC_DTX */

  #ifdef FEATURE_WCDMA_REL7_EDPCCH_POWER_BOOST
  #ifdef FEATURE_WCDMA_ZI_OPT_PHASE2
  #else
  /* Pwr Aec table: double buffer of semi-static tables which stores scaled EDPCCH power*/
  eul_pwr_A_ec_norm_tab_dbl_buf_struct_type eul_pwr_A_ec_norm_tab_dbl_buf;
  #endif /* FEATURE_WCDMA_ZI_OPT_PHASE2 */
  #endif /* FEATURE_WCDMA_REL7_EDPCCH_POWER_BOOST */
} euledpch_int_cntrl_params_type;

extern void euledpch_ext_cntrl_params_init( void);

#define GET_EUL_APS_STATS()            (euledpch_qxdm_cntrl_params.update_eul_aps_stats )

#define GET_EULEDPCH_ETFC_PR_DIS()     (euledpch_qxdm_cntrl_params.debug_disable_eul_etfc_pr)

#define GET_EULEDPCH_EDL_LOG_BMSK()    (euledpch_qxdm_cntrl_params.DEBUG_EUL_DL_LOGGING)

#define GET_EULEDPCH_CRC_ERR_ENA()     (euledpch_qxdm_cntrl_params.euledpch_crc_error_enabled)

#define GET_EULEDPCH_DBG_MPR_DIS()     (euledpch_qxdm_cntrl_params.l1_debug_disable_mpr)

#define GET_EULEDPCH_PWR_CMP_DIS()     ((euledpch_qxdm_cntrl_params.euledpch_debug_power_compresn_disabled)||(wenc_ext_persub_cntrl_params[wsub_id].wl1_ul_feature_bitmask_nv_config.power_comp))

#define GET_EULEDPCH_QXDM_A2_CB_ENA()  (euledpch_qxdm_cntrl_params.eulenc_a2_cb_dump_tbs)
#define SET_EULEDPCH_QXDM_A2_CB_ENA(x) (euledpch_qxdm_cntrl_params.eulenc_a2_cb_dump_tbs = x)

#define GET_EULEDPCH_QXDM_A2_CB_DIS()  (euledpch_qxdm_cntrl_params.eulenc_a2_cb_disable_dump_tbs_req)
#define SET_EULEDPCH_QXDM_A2_CB_DIS(x) (euledpch_qxdm_cntrl_params.eulenc_a2_cb_disable_dump_tbs_req = x)

#define GET_EULEDPCH_IN_DATA_DUMP()    (euledpch_qxdm_cntrl_params.eulenc_debug_input_data_dump)
#define GET_EULEDPCH_IN_DATA_DUMP_L2() (euledpch_qxdm_cntrl_params.eulenc_debug_input_data_dump_level2)

#ifdef FEATURE_WCDMA_HS_RACH
#define GET_EULEDPCH_HSRACH_F3_ENA()   (euledpch_qxdm_cntrl_params.hsrach_debug_enable_f3)
#endif /*FEATURE_WCDMA_HS_RACH*/

#ifdef FEATURE_WCDMA_DC_HSUPA
#define GET_EULEDPCH_DCHSUPA_F3_ENA()  (euledpch_qxdm_cntrl_params.debug_eul_dc_hsupa_f3s_enable_bmsk)
#define GET_EULEDPCH_DCHSUPA_ACT_ORD() (euledpch_qxdm_cntrl_params.dc_hsupa_activate_without_order)
#endif /* FEATURE_WCDMA_DC_HSUPA  */

#ifdef FEATURE_WCDMA_MODEM_TPUT_STATS_API
#define GET_EULEDPCH_MSTATS_LOG_ENA()  (euledpch_qxdm_cntrl_params.WCDMA_MODEM_STATS_LOGGING)

#define GET_EULEDPCH_DRATE_FLT_ENA()   (euledpch_qxdm_cntrl_params.is_dbg_datarate_filter_coeff)

#define GET_EULEDPCH_DRATE_FLT_VAL()   (euledpch_qxdm_cntrl_params.eul_datarate_filter_coeff)
#define SET_EULEDPCH_DRATE_FLT_VAL(x)  (euledpch_qxdm_cntrl_params.eul_datarate_filter_coeff = x)
#endif /* FEATURE_WCDMA_MODEM_TPUT_STATS_API */

#define GET_EULEDPCH_MAX_DEBUG_ETFCI() (euledpch_qxdm_cntrl_params.debug_etfci_limit)

#define GET_EULEDPCH_Q6_SCALING_DISABLE() ((euledpch_qxdm_cntrl_params.debug_eul_q6_scaling_disable) || (wenc_ext_persub_cntrl_params[wsub_id].wl1_ul_feature_bitmask_nv_config.eul_q6_scaling_disable))

#define SET_EULEDPCH_Q6_SCALING_DISABLE(x) (euledpch_qxdm_cntrl_params.debug_eul_q6_scaling_disable = (x))

#ifdef FEATURE_WCDMA_16_QAM
/* Bitmasks to check which bit is set in QXDM command. */
#define GET_16_QAM_QXDM_BMSK(x) (euledpch_qxdm_cntrl_params.eul_16_qam_qxdm_debug & x)
...
#ifdef FEATURE_WCDMA_DC_HSUPA
/*===========================================================================
FUNCTION      EULEDPCH_SAVE_PWR_PER_SLOT

DESCRIPTION   This function copies over DPCCH, BETA-C and RATE-ADJ from FW.
              This is a set of values read per carrier per slot every SUBFN.
              Values are read in POST-FIQ, and only when DCHSUPA is active.
              Values read in current SUBFN are ones pertaining to previous
              SUBFN.
              Values are only read here, computation is done in
              'euledpch_calc_pwr_per_frame'

DEPENDENCIES  DCHSUPA and FW support.

RETURN VALUE  None

SIDE EFFECTS  None
===========================================================================*/
extern void euledpch_save_pwr_per_slot(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION      EULEDPCH_CALC_PWR_PER_FRAME

DESCRIPTION   This function calculates the total power per carrier, from the
              values saved in "eul_pwr_per_frame". This function is called
              from 'enc_get_data', once in a CFN at around 20th BPG.
              TxAgc_per_carr = DPCCH + RateAdj.
...
SIDE EFFECTS  None
===========================================================================*/
extern void euledpch_calc_pwr_per_frame(int16 *eul_tot_pwr_per_slot, wsub_id_e_type wsub_id);
#endif /*FEATURE_WCDMA_DC_HSUPA */
...
==========================================================================*/
extern boolean eul_edpch_is_transmitting( wsub_id_e_type wsub_id);
...
===========================================================================*/
void euledpch_init( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  As above
===========================================================================*/
void euledpch_activate_ss_tables( wsub_id_e_type wsub_id);
...
===========================================================================*/
void euledpch_uph_meas_reset( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  E-DPCH DTX
===========================================================================*/
void euledpch_tx_disable( wsub_id_e_type wsub_id);
...
===========================================================================*/
void euledpch_tx_disable_during_recfg( wsub_id_e_type wsub_id);
/*===========================================================================
...
==========================================================================*/
void euledpch_suspend_bfi_one_frame_before_act_time(uint16 local_act_time, wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  E-DPCH resumes
===========================================================================*/
void euledpch_tx_re_enable( wsub_id_e_type wsub_id);
...
===========================================================================*/
void euledpch_set_edpcch_power_boost_enabled_flag(boolean val, wsub_id_e_type wsub_id);
#endif /*FEATURE_WCDMA_REL7_EDPCCH_POWER_BOOST*/
...
===========================================================================*/
extern void eul_set_var_through_qxdm( uint8 debug_switch_type, uint32 val, wsub_id_e_type wsub_id);
...
void e_ul_fill_logpkts_in_not_tx_state(
                 ...
                 uint8 enc_pwr_buf_idx,uint8 local_subfn, wsub_id_e_type wsub_id);
...
SIDE EFFECTS None.
===========================================================================*/
extern void e_ul_get_max_rate_cfg_params(boolean *dpdch_cfged,uint8 *ue_max_sf, wsub_id_e_type wsub_id);
...
SIDE EFFECTS  None
===========================================================================*/
extern int16 euledpch_get_avg_dpcch_pwr_in_db12( wsub_id_e_type wsub_id);
...
SIDE EFFECTS None
===========================================================================*/
extern void ul_tx_rf_send_log_pkt_to_diag( wsub_id_e_type wsub_id);

#ifdef FEATURE_WCDMA_DC_HSUPA 
extern void eul_new_log_send_log_pkt_to_diag ( wsub_id_e_type wsub_id);
void eul_mpr_log_pkt_submit_local_cmd ( wsub_id_e_type wsub_id);
void eul_new_mpr_log_pkt_populate( wsub_id_e_type wsub_id);
#endif

#ifdef FEATURE_WCDMA_DC_HSUPA  
extern void eul_pow_alloc_send_log_pkt_to_diag ( wsub_id_e_type wsub_id);
void eul_pow_alloc_log_pkt_submit_local_cmd ( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void euledpch_uph_meas_update( wsub_id_e_type wsub_id);
...
/*===========================================================================
 ...
 SIDE EFFECTS
   None
===========================================================================*/
extern void euledpch_cleanup_edch_cm_info( wsub_id_e_type wsub_id);
...
/*===========================================================================
 ...
 SIDE EFFECTS
   None
===========================================================================*/
extern void euledpch_cpy_last_frame_cm_info( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
===========================================================================*/
extern boolean euledpch_is_next_subframe_in_cm(uint16 sub_fr, wsub_id_e_type wsub_id);
...
SIDE EFFECTS: None
==========================================================================*/
extern void e_ul_cpc_populate_cm_bitmask( wsub_id_e_type wsub_id);
...
extern boolean euledpch_is_next_frame_cm_gap_exist( wsub_id_e_type wsub_id);
...
extern boolean euledpch_cm_gap_exists_atleast_curr_or_next_two_frames( wsub_id_e_type wsub_id);
...
extern boolean euledpch_is_this_frame_cm_gap_exist( wsub_id_e_type wsub_id);
...
extern void euledpch_cpy_last_frame_dtx_cm_info( wsub_id_e_type wsub_id);
...
extern void euledpch_cleanup_edch_dtx_cm_info( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
===========================================================================*/
boolean euledpch_is_next_edch_tx_in_cm_for_cpc_dtx(uint16 sub_fr, uint8 cfn_val, wsub_id_e_type wsub_id);
...
===========================================================================*/
void euledpch_dtx_fr_during_tti_recfg( wsub_id_e_type wsub_id);
...
=========================================================================*/
extern uint32 e_ul_get_max_num_bits_for_ue_cat(uint16 ue_cat, e_tti_enum_type tti, wsub_id_e_type wsub_id);
...
=========================================================================*/
extern l1_e_max_ch_codes_enum_type e_ul_get_max_chan_code_for_ue_cat(uint16 ue_cat, wsub_id_e_type wsub_id);
...
(mdspasync_edpch_param_db_struct_type**  next_tti_fw_param_ptr,
  uint8 enc_pwr_buf_idx,uint32  sub_fr, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void eul_accumulate_stats( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION   EUL_TPUT_STATS_INIT

DESCRIPTION This function inits TPUT stats DB during EUL cleanup/add
...
SIDE EFFECTS
  None
===========================================================================*/
void eul_tput_stats_init(wsub_id_e_type wsub_id);
...
extern void eul_calc_max_allowed_sg_po_bits(eul_edch_harq_sg_info_struct_type *harq_sg_info_ptr, 
                                            uint8 enc_pwr_buf_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
===========================================================================*/
extern void eul_stats_init_for_tti(uint8 curr_cfn, wsub_id_e_type wsub_id); 
#endif /* FEATURE_WCDMA_MODEM_TPUT_STATS_API */
...
extern void euledpch_write_dc_mpr_bins_to_fw(
    ...
    l1_e_max_ch_codes_enum_type etfc_coding_sf,uint8 carr_idx, uint8 enc_pwr_buf_idx, wsub_id_e_type wsub_id);
...
extern void euledpch_sec_read_dpcch_mtpl_from_fw( wsub_id_e_type wsub_id);
...
extern void euledpch_sec_uph_meas_reset( wsub_id_e_type wsub_id);
...
extern void euledpch_sec_uph_meas_update( wsub_id_e_type wsub_id);
...
extern void euledpch_sec_cal_uph( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  E-DPCH resumes
===========================================================================*/
void euledpch_sec_tx_re_enable_oos( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  E-DPCH resumes
===========================================================================*/
void euledpch_sec_tx_re_enable_recfg( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  E-DPCH resumes
===========================================================================*/
extern void euledpch_sec_tx_enable( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  E-DPCH resumes
===========================================================================*/
extern void euledpch_sec_tx_deactivate( wsub_id_e_type wsub_id);
...
extern void e_dl_sec_tx_deactivate( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  E-DPCH DTX
===========================================================================*/
extern void euledpch_sec_tx_disable_recfg( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  E-DPCH DTX
===========================================================================*/
extern void euledpch_sec_tx_disable_during_oos( wsub_id_e_type wsub_id);

#ifdef FEATURE_WCDMA_EVT6D_ENH
/*===========================================================================
FUNCTION  euledpch_calc_linear_power_from_db10

DESCRIPTION
  This function calculates the linear power <<36
  from the given db10 value input from FW 

DEPENDENCIES
  Called by euledpch_calc_remaining_pwr_ttop_threshold() in FIQ

RETURN VALUE
  uint64 linear power <<36

SIDE EFFECTS
  None
===========================================================================*/
extern uint64 euledpch_calc_linear_power_from_db10(uint16 local_db10, wsub_id_e_type wsub_id);
#endif /* FEATURE_WCDMA_EVT6D_ENH */
...
uint32 eul_process_cm_info(uint32 *r_combo_cm_tab_idx, uint32 *cm_tab_idx, boolean *next_tti_dtx, eul_edch_harq_sg_info_struct_type *harq_sg_info_ptr, 
                             mdspasync_edpch_param_db_struct_type** next_tti_fw_param_ptr, 
                             l1_eul_serving_grant_update_info_type* edpch_sg_info_ptr, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void euledpch_call_maint_event(eul_edch_harq_sg_info_struct_type *harq_sg_info_ptr, wsub_id_e_type wsub_id);
...
boolean euledpch_check_edpch_state_and_subfn(eul_edch_harq_sg_info_struct_type *harq_sg_info_ptr, boolean *next_tti_dtx, 
                                                           mdspasync_edpch_param_db_struct_type** next_tti_fw_param_ptr, uint8 *enc_pwr_buf_idx, wsub_id_e_type wsub_id);
...
void euledpch_calc_a_ed_quant(  l1_e_max_ch_codes_enum_type *etfc_coding_sf, 
                                   ...
                                   uint32 *a_ed_j_harq, uint32 *etfci, wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
void euledpch_prepare_ul_tx_rf_log_pkt(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void euledpch_update_sw_maint_dtx_count(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
extern uint16 wl1_ul_convert_slot_subframe(uint16 slot_num, wsub_id_e_type wsub_id);
#endif /* FEATURE_QTA */

#ifdef FEATURE_QSH_MDUMP 
/*===========================================================================
FUNCTION     wl1_qsh_mdump_collect_euledpch_int_params

DESCRIPTION
  Dump EULEDPCH INT CNTRL PARAMS
...
SIDE EFFECTS
  None
===========================================================================*/
QSH_MDUMP_FN_ATTR void wl1_qsh_mdump_collect_euledpch_int_params(wsub_id_e_type wsub_id);
#endif
#endif /* EULEDPCH_H */
