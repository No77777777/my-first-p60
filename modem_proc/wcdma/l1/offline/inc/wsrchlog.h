...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/wsrchlog.h#3 $ $DateTime: 2020/01/22 08:56:27 $ $Author: pwbldsvc $
...
04/20/16    vs      FRM log packet changes.
...
09/22/15    da      Changes to support frequency info for 0x4177 and 0x4178 log packets
06/17/15    vn      Log packet changes for step1, NASTT and PN searches for Jolokia.
...
11/05/14    jd      Changes to include rf tune info for step1 and nastt log packet.
...
/* the log code for the seach step 1 packet */
  ...
  #define SRCH_PARALLEL_STEP1_LOG_PACKET SRCH_STEP1_0x4177_LOG_PACKET
    ...
    #ifdef FEATURE_WCDMA_ENH_SRCH_LOGGING
      #define SRCH_STEP1_DCH_LOG_PKT SRCH_STEP1_0x4177_LOG_PACKET
      #define SRCH_STEP1_NON_DCH_LOG_PKT SRCH_STEP1_0x4177_LOG_PACKET
    #endif
...
/*--------------------------------------------------------------------------
                 LOG PACKET: SRCH_STEP1_LOG_PACKET (DCH and Non-DCH)
                     
This structure is use to log search step 1 information.
--------------------------------------------------------------------------*/
#if defined (FEATURE_DC_HSDPA_LOGGING) && defined (FEATURE_WCDMA_ENH_SRCH_LOGGING)
typedef PACKED struct PACKED_POST {
   ...
   //3C logging
    ...
    int16 rx_CARR2_agc_1;
#endif /* FEATURE_WCDMA_3C_HSDPA */
} srchlog_dch_RxAGC_info_struct_type;
...
typedef PACKED struct PACKED_POST {
    ...
    int16 rx_CARR0_agc_0;
    int16 rx_CARR0_agc_1;
}srchlog_non_dch_RxAGC_info_struct_type;
...
typedef PACKED struct PACKED_POST {
    ...
    boolean to_be_pruned;
#endif
} srchlog_step1_psc_peak_DCH_struct_type;
...
typedef PACKED struct PACKED_POST {
    ...
    /* Frequency hypothesis index used for this peak */
    uint8 freq_hypo_index ;
}srchlog_step1_psc_peak_non_DCH_struct_type;

LOG_RECORD_DEFINE(SRCH_STEP1_DCH_LOG_PKT)
  uint8 version;
  ...
  srchlog_dch_RxAGC_info_struct_type Rx_AGC;
  uint8 ss_info;
  uint8 nn;
  uint8 num_peaks_req;
  uint8 step1_config;
...
#ifdef FEATURE_WCDMA_3C_HSDPA
  uint16 step1_uarfcn[3];
#else 
  uint16 step1_uarfcn[2];
#endif

  srchlog_step1_psc_peak_DCH_struct_type srchlog_psc_peak_info[SRCH_MAX_PSC_PEAKS];
 LOG_RECORD_END

 LOG_RECORD_DEFINE(SRCH_STEP1_NON_DCH_LOG_PKT)
  uint8 version;
  uint8 ecio_conv_tbl_ver;
#ifdef FEATURE_WCDMA_PN_PACKET_VER4
  ...
  uint8 rf_tune_info;
#endif
  srchlog_non_dch_RxAGC_info_struct_type Rx_AGC;
  uint8 ss_info;
...
#endif
  uint16 step1_uarfcn;
  srchlog_step1_psc_peak_non_DCH_struct_type srchlog_psc_peak_info[SRCH_MAX_PSC_PEAKS];
...
**************************************************************************/
  ...
  /* 
   ...
   /* Frequency on which the current search is issued */
...
#define SRCH_PN_LOG_PACKET SRCH_PN_0x4179_LOG_PACKET
  #ifdef FEATURE_WCDMA_ENH_SRCH_LOGGING
  #define SRCH_PN_DCH_LOG_PKT SRCH_PN_0x4179_LOG_PACKET
  #define SRCH_PN_NON_DCH_LOG_PKT SRCH_PN_0x4179_LOG_PACKET
  #endif
#else 
...
LOG_RECORD_DEFINE(SRCH_PN_DCH_LOG_PKT)
  ...
  uint8 rf_tune_info;
#endif
  srchlog_dch_RxAGC_info_struct_type RxAGC;
  uint8 ss_info;
...
{  
  ...
  uint8   rscp;
#endif
} srchlog_filt_eng_non_dch_per_task_struct_type;

/* Logging result structure for non-RxD normal searches */
typedef PACKED struct PACKED_POST
{
  srchlog_filt_eng_non_dch_per_task_struct_type filt_eng;
  srchlog_pn_peak_set_struct_type       result;  
} srchlog_pn_rslt_no_rxd_normal_non_dch_struct_type;
...
typedef PACKED struct PACKED_POST
{
  srchlog_filt_eng_non_dch_per_task_struct_type filt_eng;
  srchlog_pn_peak_set_struct_type       result[2];  
} srchlog_pn_rslt_rxd_normal_non_dch_struct_type;
...
typedef PACKED struct PACKED_POST
{
  srchlog_filt_eng_non_dch_per_task_struct_type filt_eng;
  srchlog_pn_peak_set_struct_type       result[3];  
} srchlog_pn_rslt_no_rxd_shoulder_non_dch_struct_type;
...
{  
  srchlog_filt_eng_non_dch_per_task_struct_type filt_eng;
  srchlog_pn_peak_set_struct_type       result[6];  
} srchlog_pn_rslt_rxd_shoulder_non_dch_struct_type;

/* Union of all result tasks */
typedef PACKED union PACKED_POST
{
  srchlog_pn_rslt_no_rxd_normal_non_dch_struct_type    no_rxd_normal;
  srchlog_pn_rslt_rxd_normal_non_dch_struct_type  no_rxd_shoulder; 
  srchlog_pn_rslt_no_rxd_shoulder_non_dch_struct_type       rxd_normal;  
  srchlog_pn_rslt_rxd_shoulder_non_dch_struct_type     rxd_shoulder;  
} srchlog_pn_rslt_non_dch_union_type;


LOG_RECORD_DEFINE(SRCH_PN_NON_DCH_LOG_PKT)
  uint8 version;
  ...
  uint8 rf_tune_info;
#endif
  srchlog_non_dch_RxAGC_info_struct_type RxAGC;
  uint8 ss_info;
  ...
  */
  ...
  srchlog_pn_rslt_non_dch_union_type   rslt[SRCH_NORMAL_MODE_MAX_PN_TASKS];

LOG_RECORD_END

#endif /*defined (FEATURE_WCDMA_ENH_SRCH_LOGGING) && defined (FEATURE_DC_HSDPA_LOGGING)*/
...
/*--------------------------------------------------------------------------
              LOG PACKET: WCDMA_IDLE_MEAS_LOG_PKT (0x41CF)
                     
  This structure is used to log idle measurement  information. This is logged every drx cycle.
--------------------------------------------------------------------------*/

typedef enum
{
  //NA_SKIPPED_SRCH_REASON       = 0x0, parser use 0 to print NA
  NOT_ENOUGH_DURATION          = 0x1,
  MEAS_RULES_NOT_SATISFIED     = 0x2,
  TIMER_NOT_EXPIRED            = 0x4,
  NO_CELLS_AVAILABLE           = 0x8,
  BARRED                       = 0x10,
  BLOCKED_BY_NBR_SRCH_OPT      = 0x20,
  BLOCKED_BY_CTCH              = 0x40,
  BLOCKED_BY_TA                = 0X80,
  BLOCKED_BY_TRM               = 0x100,
  BLOCKED_BY_TESTER            = 0x200,
  RnN_NOT_POSSIBLE             = 0x400,
  MEAS_GAP_NOT_ALLOWED         = 0x800,
  BLOCKED_BY_G2WTA             = 0x1000,
  BLOCKED_BY_TX_ACTIVITY       = 0x2000,
  LTE_DET_ON_THE_SAME_DRX      = 0x4000
} skipped_srch_reason_enum_type;

typedef enum
{
  PICH = 1,//parser use 0 to print NA
  MEAS_ONLY,
  OTHERS
} gap_enum_type;

/* WCDMA idle measurement (non-rssi) log info */
typedef PACKED struct PACKED_POST
{
  /* type of search*/
  uint16 srch_type;

  /* skipped srch + gap type*/
  uint8 gap_type_and_skip_srch;

 /*Max Timer (frames)*/
  uint32 max_timer;
  
/*time Since last Search (frames)*/
  uint32 time_since_last_srch;
  
 /* expected srch duration in ms*/
  uint8 expected_srch_duration;
  
  /* actual srch duration in ms*/
  uint8 actual_srch_duration;
  ...
  /*fail count*/
  uint8 trm_failed_cnt;

  uint8 reason;

} wcdma_idle_meas_non_rssi_log_struct_type;

#define MAX_GSM_FREQ_BANDS 32
#define MAX_NON_RSSI_MEAS  23
/* WCDMA idle measurement (rssi) log info */
typedef PACKED struct PACKED_POST
{
  /* type of search*/
  uint8 srch_type;
...
/*ime Since last Search (frames)*/
 ...
 /* expected srch duration in ms*/
  ...
  /* frequency */  
  uint16 freq[MAX_GSM_FREQ_BANDS];

    /* search band */
  uint8 band[MAX_GSM_FREQ_BANDS];


  /* RSSI */
  int16 rssi[MAX_GSM_FREQ_BANDS];
...
} wcdma_idle_meas_rssi_log_struct_type;

/* Idle Meas log info */
LOG_RECORD_DEFINE(WCDMA_IDLE_MEAS_LOG_PACKET)
  ...
  uint8 s_fail_count;
  uint8 reacq_fail;
  uint8 is_ue_panic_mode;
  uint8 non_rssi_meas_num;
  uint8 rssi_meas_num;

  wcdma_idle_meas_non_rssi_log_struct_type 
      non_rssi_meas[MAX_NON_RSSI_MEAS];

  wcdma_idle_meas_rssi_log_struct_type rssi_meas;
...
/* FRM reservation activity types */
typedef enum
{
  INTRAF_SEARCH_ACTIVITY,
  RACH_ACTIVITY,
  FMO_SEARCH_ACTIVITY, 
  RXD_ACTIVITY, 
  QTA_ACTIVITY
} srchfach_frm_reservation_activity;

/* FRM reservation type for which the FRM reservation is being requested for. This is a subtype of reservation activity */
typedef enum
{
  ...
  LOW_PRIORITY_TYPE, 
  FACH_SIGNALLING
} srchfach_frm_reservation_type;

/* FRM reservation failure reason type */
typedef enum
{
  NOTAPPLICABLE, 
  DENIED_BY_QTA, 
  ...
  BLOCKED_SFAIL, 
  TRM_DENIAL_REASON, 
  SUSPENDED_FOR_SBCH_NBCH, 
  ABORT_FOR_QTA
} srchfach_frm_failure_reason;

/* FRM reservation result type */
typedef enum
{
  FAILURE_RESULT, 
  SUCCESS_RESULT
} srchfach_frm_result_type;

/* FRM reservation logging type */
typedef enum
{
  RESERVE_TYPE, 
  UNRESERVE_TYPE
} srchfach_frm_request_type;


/* FRM log packet related scructure - srch_fach_frm_reservation_info_type */
typedef PACKED struct PACKED_POST {
  /* ReqID which requested for FRM Reservation / unreservation */
  srchfach_frm_reservation_activity   activity; 

  /* Reason for which the ReqID is requesting for reservation / unreservation.*/
  srchfach_frm_reservation_type   reservation_type; 

  /* Reserve or Unreserve information */
  srchfach_frm_request_type request_type;

  /* SFN at which the reservation/unreservation is made.*/
  uint16 frame_num;

  /* BPG of the frame at which the reservation/unreservation request is made.*/
  uint8 bpg_num;

  /* SFN at which the reservation is going to start. */
  uint16 start_frame;

  /* Number of frames asked in the reservation or the time diff from reservation to unreservation. */
  uint8  duration;

  /* Tells whether the reservation/Un-reservation was a success or a failure. */
  srchfach_frm_result_type   reservation_status;

  /* Logs the reason for FRM reservation failure. */
  srchfach_frm_failure_reason   failure_reason;

  /* Bitmask providing the info on what all prior reqIDs are reserved, when an FRM reservation is denied. */
  uint8   req_id_mask;
} srch_fach_frm_reservation_info_type; /* 12 bytes */

/* FRM log packet Record*/
LOG_RECORD_DEFINE(SRCH_FACH_FRM_LOG_PKT)
  uint8   version;
  uint8   ue_state;         /* Includes the DL / UL / FE-FACH states of the UE */
  uint8   sccpch_offset;
  uint8   num_entries;
  srch_fach_frm_reservation_info_type frm_reservation_info_table[20];
LOG_RECORD_END              /* 20*12 + 4 = 244 bytes */
...
// Log size = 1(PICH) + 256(CTCH occasions b/w 2 POs) + 1(PICH)
#define CTCH_LOG_TABLE_SIZE 258
...
//Considered atmost 3 rows per cell and 32 cells
#define SRCH_SET_MVMT_LOG_PKT_INTRAF_MAX_ROWS 96
#define SRCH_SET_MVMT_LOG_PKT_INTERF_MAX_ROWS 128
#define SRCH_SET_MVMT_LOG_PKT_GSM_MAX_ROWS 96
#define SRCH_SET_MVMT_LOG_PKT_LTE_MAX_ROWS 32

/* Logging result structure for RxD shoulder searches */
typedef PACKED struct PACKED_POST
{  
  uint32 cell_ptr_address;
  uint8 carr_idx;
  uint16 freq;
  uint16 psc;
  uint8 source_set;
  uint8 dest_set;
} srchlog_intraf_set_movement_struct_type ;
...
{  
  ...
  uint16 cpich_tx_pwr;
  uint8 sttd;
} srchlog_interf_set_movement_struct_type ;
...
{  
  ...
  uint8 source_set;
  uint8 dest_set;
} srchlog_gsm_set_movement_struct_type ;

typedef PACKED struct PACKED_POST
{  
  uint32 earfcn;
  boolean is_tdd;
  uint8 num_eutra_freq;
  uint8 num_fdd_freq;
  uint8 num_tdd_freq;
} srchlog_lte_set_struct_type;


LOG_RECORD_DEFINE(WSRCH_SET_MOVEMENT_LOG_PACKET)
...
uint8 num_gsm_rows;
uint8 num_lte_rows;
srchlog_intraf_set_movement_struct_type intraf_set_movement[SRCH_SET_MVMT_LOG_PKT_INTRAF_MAX_ROWS];
srchlog_interf_set_movement_struct_type interf_set_movement[SRCH_SET_MVMT_LOG_PKT_INTERF_MAX_ROWS];
srchlog_gsm_set_movement_struct_type gsm_set_movement[SRCH_SET_MVMT_LOG_PKT_GSM_MAX_ROWS];
srchlog_lte_set_struct_type lte_cell_info[SRCH_SET_MVMT_LOG_PKT_LTE_MAX_ROWS];

LOG_RECORD_END

#if defined (FEATURE_WCDMA_ENH_SRCH_LOGGING) && defined (FEATURE_DC_HSDPA_LOGGING)
 /* Determine the sizes of Step1 log packet in DCH */
#define STEP1_DCH_PACKET_LEN(step1_peaks)  ( \
    FPOS(SRCH_STEP1_DCH_LOG_PKT_type, srchlog_psc_peak_info)+  \
    ( step1_peaks * sizeof(srchlog_step1_psc_peak_DCH_struct_type)))

/* Determine the sizes of Step1 log packet in non DCH */
#define STEP1_NON_DCH_PACKET_LEN(step1_peaks)  ( \
    FPOS(SRCH_STEP1_NON_DCH_LOG_PKT_type, srchlog_psc_peak_info)+  \
    ( step1_peaks * sizeof(srchlog_step1_psc_peak_non_DCH_struct_type)))
#endif
...
#if defined (FEATURE_WCDMA_ENH_SRCH_LOGGING) && defined (FEATURE_DC_HSDPA_LOGGING)
/* Determine the sizes of PN log packet in DCH */
#define PN_DCH_PACKET_LEN(num_tasks, num_results_per_task)  ( \
    FPOS(SRCH_PN_DCH_LOG_PKT_type, task)+  \
...
/* Determine the sizes of PN log packet in non DCH */
#define PN_NON_DCH_PACKET_LEN(num_tasks, num_results_per_task)  ( \
    FPOS(SRCH_PN_NON_DCH_LOG_PKT_type, task)+  \
     num_tasks * (sizeof(srchlog_pn_input_struct_type) + \
                  sizeof(srchlog_filt_eng_non_dch_per_task_struct_type) + \
                  num_results_per_task*sizeof(srchlog_pn_peak_set_struct_type)))
#endif
...
/* Determine the sizes of SET MOVEMENT log packet */
#define WSRCH_SET_MOVEMENT_LOG_PACKET_LEN(num_intraf_rows, num_interf_rows, num_gsm_rows, num_lte_rows) ( \
    FPOS(WSRCH_SET_MOVEMENT_LOG_PACKET_type,intraf_set_movement)+ \
        ( num_intraf_rows * sizeof(srchlog_intraf_set_movement_struct_type))+ \
	    ( num_interf_rows * sizeof(srchlog_interf_set_movement_struct_type))+ \
	    ( num_gsm_rows * sizeof(srchlog_gsm_set_movement_struct_type)) + \
	    ( num_lte_rows * sizeof(srchlog_lte_set_struct_type)))
