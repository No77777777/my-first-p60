...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/wsrchltedrv.h#3 $ DateTime: $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/10/20   ac      Mini dump framework checkin
12/04/17   sk      Redesigning W2L idle srch abort call flow
12/04/17   nga     Wait on SRCH_LTE_INIT_CNF_RCVD local cmd is handled to issue LTE search
12/04/17   nga     L+L: Added handler for ABORT_CNF
11/14/17   nga     NULL2L code cleanup
11/07/16   nga     Changing the EARFCN declaration from uint16 to uint32
08/20/15   ks      Double instantiating wsrchltedrv
05/13/15   wz      cleanup IRAT code for W+W
...
typedef void (*WSRCHLTEDRV_SIGNAL_SRCH_SCHED_CB_FUNC_TYPE)(wsub_id_e_type wsub_id);

/* Callback function for DEINIT_CNF done */
typedef void (*WSRCHLTEDRV_DEINIT_DONE_CB_FUNC_TYPE)(wsub_id_e_type wsub_id);

/* Callback function for sending a blacklist message after an INIT_CNF */
typedef void (*WSRCHLTEDRV_INIT_CNF_CB_FUNC_TYPE) (wsub_id_e_type wsub_id);

#ifdef FEATURE_WCDMA_CM_LTE_SEARCH
/* Callback function type for LTE CM measurement */
typedef void (*WSRCHLTEDRV_TIMED_SRCH_MEAS_DONE_CB_FUNC_TYPE)(wsub_id_e_type wsub_id);
#endif

/* Callback for checking if DEINIT REQ came after INIT was sent */
typedef void (*WSRCHLTEDRV_CHECK_FOR_DEINIT_CB_FUNC_TYPE)(wsub_id_e_type wsub_id);

/* Callback for handling ABORT REQ for W2L Idle mode meas */
typedef void (*WSRCHLTEDRV_ABORT_CNF_CB_FUNC_TYPE)(wsub_id_e_type wsub_id);
...
typedef struct
{
  /* Gap scheduled info from LTE. Tells if scheduler needs to move to next freq */
  lte_ml1_irat_gap_scheduled_info_s wsrchltedrv_gap_sched_info;
}wsrchltedrv_ext_cntrl_params_type;

extern wsrchltedrv_ext_cntrl_params_type wsrchltedrv_ext_cntrl_params[WCDMA_NUM_SUBS];

void wsrchltedrv_ext_cntrl_params_init(void);

void wsrchltedrv_int_cntrl_params_init(void);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wsrchltedrv_init( wsub_id_e_type wsub_id);
...
DESCRIPTION: 
  This function returns the state of wsrchltedrv_int_cntrl_params.wsrchltedrv_init_cnf_done flag.

DEPENDENCIES
  None.

RETURN VALUE
  TRUE, if wsrchltedrv_int_cntrl_params.wsrchltedrv_init_cnf_done is TRUE
  FALSE, otherwise

SIDE EFFECTS
  None
===========================================================================*/
extern boolean wrsrchltedrv_get_init_cnf_state(wsub_id_e_type wsub_id);
...
FUNCTION  WSRCHLTEDRV_SET_ABORT_CB

DESCRIPTION
  This function sets the callback function for the abort_cnf message
  from LTE received in Idle Mode. 
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wsrchltedrv_set_abort_cb(
  /* Callback function */
  WSRCHLTEDRV_ABORT_CNF_CB_FUNC_TYPE wsrchlte_abort_handler,
  wsub_id_e_type wsub_id);
...
extern void wsrchltedrv_send_blacklist_msg(
  /* Blacklist params */
  wsrchltedrv_blacklist_info_struct_type *blacklist_info, wsub_id_e_type wsub_id);
...
extern void wsrchltedrv_send_init_req_msg( 
  ...
  boolean maintain_timing, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wsrchltedrv_send_deinit_req_msg( wsub_id_e_type wsub_id);

/*===========================================================================

FUNCTION  WSRCHLTEDRV_SEMD_RFM_MEAS_ENTER

DESCRIPTION:
  This function sends request to LTE for calling rfm_meas_enter before starting 
  the search. The common RF pointer contains source params.
...
SIDE EFFECTS
  None
===========================================================================*/
void wsrchltedrv_send_rfm_meas_enter(
  /* Pointer to CM search/meas info object */
  wsrchltedrv_timed_srch_meas_info_struct_type* cm_srch_meas_info, wsub_id_e_type wsub_id);

/*===========================================================================

FUNCTION  WSRCHLTEDRV_SEND_RFM_MEAS_EXIT

DESCRIPTION:
  This function sends request to LTE for calling rfm_meas_exit() after the search. 
  The common RF pointer contains source params.
...
SIDE EFFECTS
  None
===========================================================================*/
void wsrchltedrv_send_rfm_meas_exit(
  rfm_meas_common_param_type* rf_info, 
...
extern void wsrchltedrv_send_timed_srch_meas_req_msg(
  ...
  boolean is_connected_meas, wsub_id_e_type wsub_id);

/*===========================================================================

FUNCTION  wsrchltedrv_prepare_build_script_req

DESCRIPTION:
  This function fills the RF info and sends RFM_MEAS_ENTER and BUILD RF SCRIPT REQ to LTE.
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wsrchltedrv_prepare_build_script_req (void *rf_buf, uint32 earfcn, wsub_id_e_type wsub_id);

/*===========================================================================

FUNCTION  WSRCHLTEDRV_SEND_BUILD_SCRIPT_REQ_MSG

DESCRIPTION:
  This function sends build RF script request to LTE. 
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wsrchltedrv_send_build_script_req
( 
  lte_cphy_irat_meas_rf_param_s rf_param, 
  wsub_id_e_type wsub_id
);

/*===========================================================================

FUNCTION  WSRCHLTEDRV_SEND_ABORT_REQ_MSG

DESCRIPTION:
  This function sends the abort request to LTE after build RF script req
  has been sent to LTE.  
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wsrchltedrv_send_abort_req(wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern wsrchltedrv_lte_cell_info_db_struct_type* wsrchltedrv_get_cell_det_result_ptr(wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern wsrchltedrv_lte_meas_info_db_struct_type* wsrchltedrv_get_cell_meas_result_ptr(wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern wsrchltedrv_lte_results_db_struct_type* wsrchltedrv_get_cm_results_ptr(wsub_id_e_type wsub_id);
#endif
...
extern void wsrchltedrv_lte_msg_handler(
  /* Message received */
  msgr_hdr_struct_type* hdr, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wsrchltedrv_give_eutra_snapshot_data(lte_l1_lte_stmr_snapshot_s *snpsht_ptr, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
boolean wsrchltedrv_query_lte_timing_maintained(wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wsrchltedrv_lte_gap_init( wsub_id_e_type wsub_id);
...
FUNCTION  WSRCHLTEDRV_RESET_SIGNAL_SRCH_SCHED_CB

DESCRIPTION:
  This function resets the CB
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wsrchltedrv_reset_signal_srch_sched_cb(wsub_id_e_type wsub_id);
/*===========================================================================

FUNCTION  WSRCHLTEDRV_STATUS_SIGNAL_SRCH_SCHED_CB
DESCRIPTION:
  This function returns true callback is not NULL
DEPENDENCIES
  None.
...
SIDE EFFECTS
  None
===========================================================================*/
boolean wsrchltedrv_status_signal_srch_sched_cb(wsub_id_e_type wsub_id);

#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
FUNCTION wl1_qsh_mdump_collect_irat_wsrchltedrv
...
SIDE EFFECTS
  None
===========================================================================*/
extern QSH_MDUMP_FN_ATTR void wl1_qsh_mdump_collect_irat_wsrchltedrv(wsub_id_e_type wsub_id);
#endif
