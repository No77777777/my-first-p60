...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/l1cmmgr.h#12 $
$DateTime: 2021/12/22 00:00:32 $
$Author: pwbldsvc $

when        who     what, where, why
--------    ---     --------------------------------------------------------
08/13/20    rs      Do not send blacklist command to ML1 if deinint is in progress.
06/12/20    ds      Do not let rrc trigger pseduo if let deinit is ongoing
03/19/20    rs      Concurrency Manager : Changes to handle measurement trigger failure
...
08/26/19    rs      Support for FR53313 WCDMA to LTE Pseudo measurement
11/09/15    ac      Disable LVA for HLS CM
11/04/15    vn      API to get slots to start CM
07/24/15    svh     Added HS client to compressed mode activity notification framework
...
03/10/15    wz      clean CM state machine change
...
typedef enum
{
  L1_CM_STATE_INACTIVE,
  L1_CM_STATE_GOING_ACTIVE,
  L1_CM_STATE_NOTIFICATION,
  L1_CM_STATE_ACTIVE,
  L1_CM_STATE_FREEZE_PENDING,
  L1_CM_STATE_FROZEN,
  L1_CM_STATE_UNFREEZE_PENDING,
  L1_CM_STATE_SUSPEND_PENDING,
  L1_CM_STATE_RESUME_PENDING,
  L1_CM_STATE_SUSPENDED,
  L1_CM_STATE_NONE
} l1_cm_state_enum_type;

typedef enum
{
  L1_CM_CON_TGMP_WCDMA_INTERF,
  L1_CM_CON_TGMP_GSM,
  L1_CM_CON_TGMP_LTE,
} l1_cm_tgmp_enum_type;

typedef struct
{
  /* indicate that there is some TGPS valid or not. This can be removed eventually */
  boolean l1_cm_no_tgps_valid;
  ...
  boolean l1_cm_info_valid[L1_CM_MAX_TGP_SEQ_COUNT];
  /* transmission gap pattern sequence info */
  l1_cm_tgp_seq_info_struct_type l1_cm_tgp_seq_info[L1_CM_MAX_TGP_SEQ_COUNT];

} l1_cm_tgp_ctrl_info_struct_type;

typedef struct
{
  /* Variables to track the CFN mismatch to keep CM state machine in sane state */
  /* This variable is reset every CM state machine init */ 
  boolean l1_cm_prev_cfn_served_valid;
  /* This is saved at every CM update frame event */
  uint8 l1_cm_prev_cfn_served;
  /* prev served CFN TGMP */
  l1_cm_tg_meas_purpose_enum_type l1_cm_prev_served_tgmp;
  /* indicate that if prev served CFN is GAP and is it multi frame gap */
  boolean l1_cm_prev_cfn_served_has_multifr_gap;
  /* CFN of second frame of multiframe GAP */
  uint8 l1_cm_prev_cfn_served_next;
} l1_cm_tgp_cfn_info_struct_type;
...
typedef enum
{
  ...
  L1_CM_CLIENT_ASDIV,
#endif
  L1_CM_CLIENT_WPWR_LOGPKT,
  L1_CM_CLIENT_W2L_CME,
  L1_CM_NUM_CLIENTS
...
/* ========================================================================= */
/*                Callback function declaration types                        */
/* ========================================================================= */
/* Call back function pointer types used with act/deact notifications and 
   client status checks  */
typedef void (l1_cm_act_deact_notif_func_type)(l1_cm_active_deactive_enum_type dep_patterns_active, wsub_id_e_type wsub_id);
typedef boolean (l1_cm_client_status_func_type)(wsub_id_e_type wsub_id);

/* Callback function definition */
typedef void L1_CM_CME_CB_FUNC_TYPE(boolean,wsub_id_e_type wsub_id);
typedef boolean L1_CM_CME_STATUS_CB_FUNC_TYPE(wsub_id_e_type wsub_id);

/* nfi (normal frame indication) definitions.  Used to notify nfi clients via 
   cb during normal (non CM) frames */
typedef void (l1_cm_nfi_cb_type)(wsub_id_e_type wsub_id);

typedef void L1_CM_SUSPEND_DONE_CB_FUNC(void);

typedef void (l1_cm_cleanup_done_cb_type)(void *, wsub_id_e_type wsub_id);
/* =========================================================================== */
...
typedef struct
{
  ...
  const l1_cm_static_client_info_struct_type *static_info_ptr;
...
typedef struct
{
  /* this array stores the TGPS going (in)active and CFN at which it will
   go (in)active. This is used to send TGPS status active to L1 CM meas module
   This only handles the condition when TGPS is marked (in)active pending and
   this is done by measurement control message */
  boolean l1_cm_tgps_going_active[L1_CM_MAX_TGP_SEQ_COUNT];
  uint8 l1_cm_tgps_going_active_cfn[L1_CM_MAX_TGP_SEQ_COUNT];
  boolean l1_cm_tgps_active_state[L1_CM_MAX_TGP_SEQ_COUNT];

  boolean l1_cm_tgps_going_inactive[L1_CM_MAX_TGP_SEQ_COUNT];
  uint8 l1_cm_tgps_going_inactive_cfn[L1_CM_MAX_TGP_SEQ_COUNT];
  l1_cm_tgps_deact_reason_enum_type l1_cm_tgps_going_inactive_reason[L1_CM_MAX_TGP_SEQ_COUNT];
} l1_cm_tgps_act_info_struct_type;

typedef struct
{
  uint8 cme_act_deact;
  L1_CM_CME_CB_FUNC_TYPE *cme_act_deact_cb_func;
  L1_CM_CME_STATUS_CB_FUNC_TYPE *cme_status_cb_func;

  uint8 w2l_cme_act_deact;
  L1_CM_CME_CB_FUNC_TYPE *w2l_cme_act_deact_cb_func;
  L1_CM_CME_STATUS_CB_FUNC_TYPE *w2l_cme_status_cb_func;
} l1_cm_cme_client_ctrl_struct_type;

typedef struct
{
   /* if this flag is set, we'll forcefully start the "client notification check" 
   process in our every-frame processing */
   boolean notif_check_pending;

   /* Stores information on when tgmps can be notified to clients as active or 
   notified to clients as deactive based on the notification timing parameters
   and the history/future of tgmp activations and deactivations.  (only need
   one of these arrays currently since there is only one constant set of 
   notification timing parameters right now) */
   l1_cm_tgmp_act_deact_notification_state_struct_type act_deact_notify_states[L1_CM_NUM_TGMP_TYPE];

   /* The client-notification information table.  One entry for each client. */
   l1_cm_act_deact_client_info_struct_type client_info[L1_CM_NUM_CLIENTS];

   l1_cm_cme_client_ctrl_struct_type cme_client_info;

} l1_cm_client_notif_struct_type;


/* --- l1cmmgr CLEANUP related variables --- */
typedef struct
{
  /* the callback l1cmmgr will call when done cleaning up */
  l1_cm_cleanup_done_cb_type *cleanup_done_cb;

  /* this is the argument cleanup_done_cb will return with.  Used by cleanup 
     clients to regain context when re-initiated by cleanup callback.*/
  void *cleanup_done_cb_arg;

  /* In the case where cleanup is delayed due to compressed frames still 
     happening, and we setup an event to trigger an interrupt at the end of the
     CM gap, this field indicates how many event triggers we're have remaining 
     before it's safe to finish the cleanup.  Will be decremented immediately 
     every time the event interrupt fires.  If it hits zero, then we can 
     disable the event and trigger the resumption of the cleanup processing */
  uint8 num_event_interrupts_remaining;
} l1_cm_cleanup_db_type;

typedef struct
{
  /* becomes TRUE as soon as someone requests a cleanup of l1cmmgr, and remains
     TRUE until l1cmmgr finishes cleaning up */
  boolean cleanup_pending;

  /* cleanup info.  Only valid if l1_cm_cleanup_pending is TRUE */
  l1_cm_cleanup_db_type cleanup_db;
} l1_cm_cleanup_info_struct_type;


typedef struct
{
  #ifdef FEATURE_WCDMA_CM_LTE_SEARCH  
  /* indicate if the TGMP with LTE meas exists */
  boolean tgmp_lte_meas_exist;
  #endif
  /* indicate if the TGMP with GSM RSSI meas exists */
  boolean tgmp_fdd_meas_exist;
  /* indicate if the TGMP with GSM RSSI meas exists */
  boolean tgmp_gsm_rssi_meas_exist;
  /* indicate if the TGMP with GSM BSIC identify exists */
  boolean tgmp_gsm_bsic_identify_exist;
  /* indicate if the TGMP with GSM BSIC reconfirm exists */
  boolean tgmp_gsm_bsic_reconf_exist;
  /* number of compressed mode gaps for N identify abort */
  uint16 gsm_bsic_id_abort_gap_count;
  /* indicate the number if sec in 0.5 unit resolution in unit of
  WCDMA frame in which BSIC reconformation will expire */
  uint16 gsm_bsic_reconf_abort_wframe_unit;
  /* Following variable indicate the pending update for rssi meas exist,
   N_identify abort and T_recon abort for TGPS activating/deactivating
   at future CFN
   After all the TGPS have been activated or deactivated then there
   should not be any pending for these. If it exists then it means that
   2 TGPS with same TGP are active */
  uint8 tgpsi_fdd_meas_to_set;
  uint8 tgpsi_gsm_tgmp_rssi_meas_to_set;
  uint8 tgpsi_gsm_tgmp_bsic_identify_to_set;
  uint8 tgpsi_gsm_tgmp_bsic_reconf_to_set;

  #ifdef FEATURE_WCDMA_CM_LTE_SEARCH  
  uint8 tgpsi_lte_meas_to_set;
  /* This flag indicates that GSM was active when LTE INIT for W2L CM 
   was last attempted. LTE INIT should be sent at the next possible 
   W frame when GSM is done */
  boolean lte_init_pending;
  #endif

  /* This array stores the TGPSI for a given TGMP */
  uint8 cm_tgpsi_for_tgmp[L1_CM_NUM_TGMP_TYPE];

} l1_cm_tgmp_info_struct_type;

typedef struct
{
  /* information index to be used by current coming frame */
  uint8 l1_cm_current_stat_head;
  /* This array maintains the CM action in advance for CM frames (4 prev + (8+1) adv stat + 1 current */
  l1_cm_action_info_struct_type l1_cm_adv_action_info[L1_CM_NUM_FRAME_INFO];
  /* prev used CM action */
  l1_cm_action_info_struct_type *l1_cm_prev_cm_action_info;
  /* currently used CM action */
  l1_cm_action_info_struct_type *l1_cm_cur_cm_action_info;
  /* indicate which TGPSI need the info init */
  boolean l1_cm_state_adv_init_reqd[L1_CM_MAX_TGP_SEQ_COUNT];
  /* transmission gap pattern sequence internal stats */
  uint8 l1_cm_cur_tgps_stat_info_idx;

  /* The following array retains the information of the compressed mode state
   of N frame ahead. This is a circular buffer */
  l1_cm_tgps_stat_struct_type l1_cm_tgps_stat_adv[L1_CM_NUM_FRAME_INFO][L1_CM_MAX_TGP_SEQ_COUNT];

  /* TGPS going active/inactive info and state */
  l1_cm_tgps_act_info_struct_type l1_cm_tgps_act_deact_info;

  /* Pending activate or deactivate CM TGP seq info */
  l1_cm_pending_tgp_seq_info_struct_type l1_cm_pending_tgp_seq_info[L1_CM_MAX_TGP_SEQ_COUNT];
  /* CM info sequence number to help establish temporal ordering
   Indicates when pending CM info was received. At time of CM
   suspend, a snapshot of the current seq number will be taken
   and on (deact all) resume all ACT received before that will
   be discarded */
   uint8 l1_cm_pending_info_curr_seq_num;
} l1_cm_action_ctrl_info_struct_type;
...
typedef struct
{
  ...
  /* CFN of second frame of multiframe GAP */
  uint8 l1_cm_prev_cfn_served_next;
} l1_cm_sanity_check_params_struct_type;
...
typedef struct
{
  /* this array indicate the CFN that has been blocked for intra frequency purpose */
  boolean cfn_blocked_intraf_reserved[MAX_CFN_COUNT];
  /* CFN at which intra-frequency switching reservation starts */
  uint8 intraf_rsv_start_cfn;
  /* Flag indicating the CFN given above is a valid indicator of reservation */
  boolean intraf_rsv_cfn_valid;
  /* Flag indicating whether a reserve or unreserve request is successful */
  boolean intraf_rsv_status_ok;
  /* CM gap reserve for intra freq use info */
  l1_cm_gap_reserve_req_info_struct_type gap_reserve_req_info[L1_CM_GAP_NUM_RESERVE_REQ_ID];
} l1_cm_intraf_gap_reserve_info_struct_type;

typedef struct
{
  /* CFN at which CM suspend is received to calculate time
     for which CM was suspended */
  uint8 suspend_cfn_rece;

  /* CFN at which CM state machine was suspened */
  uint8 suspend_cfn;

  /* Number of frames CM state machine is suspended */
  uint32 num_frames_suspended;

  L1_CM_SUSPEND_DONE_CB_FUNC *suspend_done_cb_func;

  /* The pending info sequence number at time of suspend request sent.
   This will store the sequence number before any potential new CM
   info would arrive to be used after resumption. */
  uint8 pending_info_seq_num_at_suspend;

  boolean deactivate_all_tgps_at_resume;
  boolean suspended_state;
  boolean suspend_request_pending;
  boolean resume_request_rece;
  uint16 suspend_req_start_cfn;
    
} l1_cm_suspend_params_struct_type;

typedef struct
{
  boolean freeze_normal_frame;

  boolean frozen_state;
  boolean freeze_pending;
  boolean unfreeze_pending;
  boolean freeze_normal_frame_pending;
  uint8 unfreeze_start_cfn;
  uint8 freeze_start_cfn;

  #if defined(FEATURE_DUAL_SIM) && defined(FEATURE_QTA)
  /* indicate that compressed actions are frozen because of QTA */
  boolean frozen_for_qta;
  #endif

  /* indicate that compressed actions are frozen because of FAST LTA OPTs */
  boolean frozen_for_fast_lta;

  /* stored cb's used for nfi (normal frame indication) to clients */
  l1_cm_nfi_cb_type *nfi_cbs[L1_CM_NUM_NFI_CLIENTS];


} l1_cm_freeze_params_struct_type;


typedef struct
{
  /* CFN at which CM state needs to be changed */
  uint16 cm_state_action_cfn;

  /* CM gap in progress information */
  boolean cm_gap_in_progress;
  uint8 l1_cm_first_non_cm_frame;

  /* indicate that there is some TGPS valid or not */
  boolean l1_cm_no_tgps_valid;

  /* MM marker: Is this variable needed */
  boolean l1_cm_update_info_as_inactive[L1_CM_MAX_TGP_SEQ_COUNT];

  /* Client notification system */
  l1_cm_client_notif_struct_type client_notif_info;

  /* TGP state info in terms of patterns valid and action info */
  l1_cm_tgp_ctrl_info_struct_type l1_cm_tgp_state;
  ...
  /* Freeze params */
  l1_cm_freeze_params_struct_type cm_freeze_params;

  /* Compressed mode suspend state params */
  l1_cm_suspend_params_struct_type cm_suspend_params;

  l1_cm_sanity_check_params_struct_type sanity_check_params;

  timer_type l1_cm_gap_end_timer;

} l1_cm_ctrl_info_struct_type;

typedef struct
{
  /* This variable will indicate if the compressed mode info is present */
  boolean cm_info_present;

  /* This variable saves the compressed mode DPCH info */
  l1_cm_info_struct_type *phychan_db_cm_info;

  /* High level control info including states and action CFN */
  l1_cm_ctrl_info_struct_type ctrl_info;

  /* transmission gap pattern sequence info */
  l1_cm_tgp_seq_info_struct_type l1_cm_tgp_seq_info[L1_CM_MAX_TGP_SEQ_COUNT];

  /* Current stat head and adv action info */
  l1_cm_action_ctrl_info_struct_type l1_cm_action_info;

  l1_cm_tgmp_info_struct_type tgmp_related_info;
  ...
  /* back up during HHO attempt, If HHO fails the data will be restored from 
     this structure */
  l1_cm_hho_suspend_info_struct_type l1_cm_hho_suspend_saved_info;

  /* This stores the info received from QXDM to 
   enable or disable skip_cm_measurements */
  l1_cm_qxdm_skip_cm_meas_info_params_struct_type l1_cm_qxdm_skip_cm_meas_info;

  /* cleanup info.  Only valid if l1_cm_cleanup_pending is TRUE */
  l1_cm_cleanup_info_struct_type cleanup_info;

#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
 /*indicate that w2l pseduo meas is ongoing*/
  boolean l1_cm_w2l_pseudo_meas_flag;
 /*save the start cfn at the time of activation*/
  uint8 l1_cm_w2l_pseudo_act_cfn[L1_CM_MAX_TGP_SEQ_COUNT];
#endif
} l1_cm_mgr_struct_type;

#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
#define L1_PSEUDO_W2L_ADJ_CFN 4
#endif

typedef struct
{
  /* indicate that compressed mode in DL can't be activated with the given config info */
  boolean l1_cm_method_invalid;
  /* cmEbNtAdj history for logging purpose */
  boolean l1_cm_delta_sir_applied[16];
  uint16 l1_cm_ebnt_adj_val[16];
  /* This variable indicate the validity of compressed mode information */
  boolean l1_cm_info_invalid;
  /* Create L1CMMGR CM in progress critical section */
  rex_crit_sect_type l1cmmgr_cm_in_prog_crit_sect;
  /* Create L1CMMGR critical section */
  rex_crit_sect_type l1cmmgr_crit_sect;
  /* Indicate the intraf reservation info */
  l1_cm_intraf_gap_reserve_info_struct_type l1_cm_intraf_gap_reservation_info;
  l1_cm_tgmp_enum_type clock_request_to_con_manager [L1_CM_NUM_TGMP];
  boolean l1_cm_con_man_release_at_gap_end;
}l1cmmgr_ext_cntrl_params_type;

extern l1cmmgr_ext_cntrl_params_type l1cmmgr_ext_cntrl_params[WCDMA_NUM_SUBS];


typedef struct
{
  l1_cm_mgr_struct_type l1_cm_mgr_info;
  #ifndef FEATURE_CM_UNIT_TEST
  tlm_tfn_struct_type cm_sfn_snapshot;
  #endif
  #ifndef FEATURE_CM_UNIT_TEST
  /* This variable saves the DPCH info in which compressed mode info is passed to */
  l1_dl_phychan_db_struct_type *l1_phychan_db_w_cm_info;
  #endif
  /* This variable saves W2L de-init transition information */
  boolean l1_cm_wsrchlte_deinit_in_progress;
}l1cmmgr_int_cntrl_params_type;
...
** ======================================================================= */
extern void l1cmmgr_int_cntrl_params_init( wsub_id_e_type wsub_id);

extern void l1cmmgr_ext_cntrl_params_init( wsub_id_e_type wsub_id);
/*===========================================================================
FUNCTION l1_cm_init
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_cm_init(boolean init, wsub_id_e_type wsub_id);
...
extern boolean l1_cm_tgmp_conflict_validate(
  ...
  l1_cm_info_struct_type *cm_info, wsub_id_e_type wsub_id);

  
/*===========================================================================
FUNCTION l1_cm_freeze_cm_status

DESCRIPTION
  This function processes the CM info and inform whether cm is frozen or not.

DEPENDENCIES
  None

RETURN VALUE
  TRUE or FALSE based on "frozen_state"

SIDE EFFECTS
  None
===========================================================================*/
extern boolean l1_cm_freeze_cm_status (wsub_id_e_type wsub_id);
...
extern void l1_cm_update_info_tgps_activate_common(
  /* TGPS Id */
  uint8 tgpsi, wsub_id_e_type wsub_id);
...
extern void l1_cm_init_tgps_stat(
  ...
  l1_cm_tgps_stat_struct_type *tgp_seq_stat, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_cm_end_compressed_mode( wsub_id_e_type wsub_id);
...
extern void l1_cm_init_tgps_stat_adv(
  ...
  uint8 adv_stat_depth, wsub_id_e_type wsub_id);
...
extern void l1_cm_get_next_state(
  ...
  uint8 next_cfn,
  /* CM TGPS stta structure pointer */
  l1_cm_tgps_stat_struct_type *this_tgp_seq_stat, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_cm_state_update( wsub_id_e_type wsub_id);
...
extern void l1_cm_state_update_advance(
  ...
  boolean *stat_init_use_prev_stat_info, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_cm_set_ebnt_adj_val( wsub_id_e_type wsub_id);
...
extern void l1_cm_update_adv_action_info(
  ...
  uint8 cfn,
  /* advanced info index */
  uint8 adv_cm_info_index, wsub_id_e_type wsub_id);
...
extern boolean l1_cm_change_cm_state_control(
  ...
  /* next action info */
  l1_cm_action_info_struct_type *next_action, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean l1_cm_query_cm_in_prog( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_cm_exit_cm( wsub_id_e_type wsub_id);
...
extern boolean l1_cm_query_cm_ul(
  ...
  uint8 cfn, wsub_id_e_type wsub_id);
...
void l1_cm_get_ul_params(
  ...
  l1_cm_ul_param_struct_type* cm_ul_params, wsub_id_e_type wsub_id);
...
extern void l1_cm_get_stat_seq_info(
  ...
  l1_cm_tgp_seq_info_struct_type **l1_cm_req_tgp_seq_info, wsub_id_e_type wsub_id);
...
extern void l1_cm_freeze_cm_action(
  ...
  uint16 cfn, wsub_id_e_type wsub_id);
...
extern void l1_cm_freeze_cm_for_qta( wsub_id_e_type wsub_id);
...
extern void l1_cm_unfreeze_cm_for_qta( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean l1_cm_freeze_cm_on_normal_frame( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_cm_normal_frame_frozen( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean l1_cm_query_gsm_clk_turned_on( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean l1_cm_query_action_freeze_pending( wsub_id_e_type wsub_id);
...
extern boolean l1_cm_query_tgmp_for_gsm_meas(
  ...
  l1_cm_tg_meas_purpose_enum_type tgmp, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint16 l1_cm_query_bsic_reconf_abort_timer( wsub_id_e_type wsub_id);
...
extern boolean l1_cm_query_delta_sir_applied_in_tti(
  ...
  uint8 num_fr_in_tti, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean l1_cm_query_gsm_tgmp_active( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION l1_cm_query_interf_tgmp_active

DESCRIPTION
  This function checks if any TGPSI with interf TGMP is active.
...
extern boolean l1_cm_query_interf_tgmp_active( wsub_id_e_type wsub_id);
/*===========================================================================
FUNCTION l1_cm_freeze_cm_status
...
extern boolean l1_cm_reserve_for_intraf_validate(
  ...
  uint8 num_frames, wsub_id_e_type wsub_id);
...
extern boolean l1_cm_reserve_for_intraf_possible(
  ...
  uint8 *intraf_reserve_start_cfn, wsub_id_e_type wsub_id);
...
extern boolean l1_cm_unreserve_for_intraf(
  /* intra-freq GAP freq switch block req Id */
  l1_cm_gap_reserve_req_id_enum_type req_id, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_cm_suspend_cleanup( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_cm_resume_init( wsub_id_e_type wsub_id);
...
extern boolean l1_cm_suspend_cm(
  ...
  boolean save_pending_info, wsub_id_e_type wsub_id);
...
extern void l1_cm_resume_cm(
  ...
  boolean deact_all_tgps, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_cm_syncup_cm_state_at_resume( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_cm_get_active_tgpsi_info(l1_cm_active_tgpsi_info_struct_type* tgpsi_info, wsub_id_e_type wsub_id);
...
extern boolean l1_cm_is_cm_info_present(
  ...
  l1_ext_cmd_type *ext_cmd, wsub_id_e_type wsub_id);
...
extern uint8 l1_cm_info_update_cm_activation_time(
  ...
  boolean missed_activation_time, wsub_id_e_type wsub_id);
...
extern void l1_cm_validate_cm_method(
  ...
  l1_setup_cmd_type* cmd, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void l1_cm_send_cm_info( wsub_id_e_type wsub_id);
...
SIDE EFFECTS      None
===========================================================================*/
extern void l1_cm_phychan_cm_config( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean l1_cm_intraf_cm_reserve (boolean reserve, wsub_id_e_type wsub_id);
...
SIDE EFFECTS      None
===========================================================================*/
extern boolean l1_cm_validate_cm_params(uint16 opt_parm_presence_bitmask, l1_dl_common_info_struct_type *dl_common, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1_cm_client_off_done(l1_cm_act_deact_client_enum_type client, wsub_id_e_type wsub_id);
...
boolean l1_cm_register_client_for_act_deact_notif( 
  ...
  l1_cm_client_status_func_type *client_status_cb_ptr, wsub_id_e_type wsub_id);
...
extern void l1_cm_register_cme_cb_func(
  ...
  L1_CM_CME_STATUS_CB_FUNC_TYPE *status_func_ptr, wsub_id_e_type wsub_id);

extern void l1_cm_deregister_cme_cb_func( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION l1_cm_register_w2l_cme_cb_func
         l1_cm_deregister_w2l_cme_cb_func
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_cm_register_w2l_cme_cb_func(
  /* Call back function pointer */
  L1_CM_CME_CB_FUNC_TYPE *func_ptr,
  /* Call back function for CME status check */
  L1_CM_CME_STATUS_CB_FUNC_TYPE *status_func_ptr, wsub_id_e_type wsub_id);

extern void l1_cm_deregister_w2l_cme_cb_func( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_cm_cme_handler( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION l1_cm_w2l_cme_handler

DESCRIPTION
  This function handles the local command put by l1cmmgr to call W2L CME callbacks.
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_cm_w2l_cme_handler( wsub_id_e_type wsub_id);
...
extern boolean l1_cm_request_nfi(
  ...
  l1_cm_nfi_cb_type *cb, wsub_id_e_type wsub_id);
...
extern boolean l1_cm_cleanup_req(
  ...
  boolean save_old_cm_config, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void l1_cm_finish_cleanup( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint32 l1_cm_duration_cm( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint32 l1_cm_parallel_step1_overhead(uint8 num_interf_carrier, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void l1_cm_set_skip_cm_meas_through_qxdm(uint8 val, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION  L1_CM_PATTERN_WITH_HLS

DESCRIPTION
  This function sees if there is a CM pattern active with HLS
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean l1_cm_pattern_with_hls(wsub_id_e_type wsub_id);
...
/*===========================================================================
FUNCTION l1_cm_query_gsm_active_this_cfn

DESCRIPTION
  This function evaluates whether there is a GSM search ongoing 
  in the CFN passed in the argument

DEPENDENCIES
  None

RETURN VALUE
  TRUE if GSM search is ongoing in the current frame
  FALSE otherwise.
 
SIDE EFFECTS
  None
===========================================================================*/
extern boolean l1_cm_query_gsm_active_this_cfn(uint8 check_cfn, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION l1_cm_query_interf_meas_active_this_cfn

DESCRIPTION
  This function evaluates whether there is a interf search ongoing 
  in the CFN passed in the argument
...
RETURN VALUE
  TRUE if interf search is ongoing in the current frame
  FALSE otherwise.
...
extern boolean l1_cm_query_interf_meas_active_this_cfn(uint8 check_cfn,wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION l1_cm_set_info

DESCRIPTION
  This function sets the conurency manager monitor variable for given TGMP
...
extern void l1_cm_set_info(l1_cm_tgmp_enum_type tgmp, wsub_id_e_type wsub_id);
/*===========================================================================
FUNCTION l1_cm_reset_info

DESCRIPTION
  This function resets the conurency manager monitor variable for given TGMP
...
extern void l1_cm_reset_info(l1_cm_tgmp_enum_type tgmp, wsub_id_e_type wsub_id);
/*===========================================================================
FUNCTION l1_cm_query_con_status

DESCRIPTION
  This function returns the conurency manager monitor variable for given TGMP
...
extern l1_cm_tgmp_enum_type l1_cm_query_con_status(l1_cm_tgmp_enum_type tgmp, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean l1_cm_find_next_cm_gap (uint8 *cm_gap_cfn, uint8 *cm_gap_start_slot, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION l1_cm_suspend_cleanup_for_lta_opts

DESCRIPTION
  This function will check if CM gap is in progress. If YES, it will setup a timer to expire at the end
  of the gap frame boundary and call the Callback function to resume WL1 cleanup for LTA.

DEPENDENCIES
  Caller must not intlock around calls to this function 

RETURN VALUE
  TRUE: Cleanup can proceed
  FALSE: Wait for CM to end

SIDE EFFECTS
  None

===========================================================================*/
extern boolean l1_cm_suspend_cleanup_for_lta_opts(void *cb_func, wsub_id_e_type wsub_id);

#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
/*===========================================================================
FUNCTION l1_cm_update_pseudo_w2l

DESCRIPTION
  update the structure to be filled for pseudo w2l
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_cm_update_pseudo_w2l(l1_cm_tgp_seq_info_struct_type* tgp_seq_info
,wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION l1_cm_pseudo_w2l_is_active

DESCRIPTION
  Informs if w2l pseudo measurement is ongoing
...
extern boolean l1_cm_pseudo_w2l_is_active(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION l1_cm_resume_pseudo_w2l

DESCRIPTION
  This function will be called while resuming back to the old CM confguration, it updates the start_CFN so that it macthes
  with the newtork running cm pattern.
...
extern void l1_cm_resume_pseudo_w2l(l1_cm_tgp_seq_info_struct_type* tgp_seq_info,wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION l1_cm_is_tgpsi_going_active_nearby

DESCRIPTION
  This function checks if any tgpsi is going to get active in next 9 or less CFN.
  And LTE measurement purpose going active or deactive in nearby future.

DEPENDENCIES
 NONE

RETURN VALUE
 TRUE:   if any tgpsi is going active in next 9 or less CFN or 
         LTE measurement purpose going active or deactive in nearby future.
 FALSE:  otherwise

SIDE EFFECTS
  None
===========================================================================*/
boolean l1_cm_is_tgpsi_going_active_nearby(wsub_id_e_type wsub_id);

#endif

/*===========================================================================
FUNCTION        l1_cm_validate_rm_method_meas_req

DESCRIPTION     This function verifies that meas req CM can be activated
                with current physical channel and CCTrCh config. Following
                configuration is not allowed
                1. DL physical channel configuration SF4 for CM by SF reduction
                2. CCrCh info DTX method FIXED_POS for CM by HLS
...
SIDE EFFECTS    Updates variable l1_cm_method_invalid. This is used in actual
                compressed mode validation.
===========================================================================*/
extern boolean l1_cm_validate_rm_method_meas_req( l1_cm_info_struct_type *cm_info, wsub_id_e_type wsub_id);

#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
FUNCTION wl1_qsh_mdump_collect_irat

DESCRIPTION
  This is a minidump function for dumping the selected IRAT global variables
...
SIDE EFFECTS
  None
===========================================================================*/
extern QSH_MDUMP_FN_ATTR void wl1_qsh_mdump_collect_irat(wsub_id_e_type wsub_id);
#endif
/*===========================================================================
FUNCTION l1_cm_deinit_in_progress_reset

DESCRIPTION
  This function resets l1_cm_wsrchlte_deinit_in_progress
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_cm_deinit_in_progress_reset(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION l1_cm_deinit_in_progress_set

DESCRIPTION
  This function sets l1_cm_wsrchlte_deinit_in_progress
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_cm_deinit_in_progress_set(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION l1_cm_deinit_in_progress
...
RETURN VALUE
TRUE: If l1_cm_wsrchlte_deinit_in_progress is set
FALSE: Otherwise

SIDE EFFECTS
  None
===========================================================================*/
extern boolean l1_cm_deinit_in_progress(wsub_id_e_type wsub_id);
