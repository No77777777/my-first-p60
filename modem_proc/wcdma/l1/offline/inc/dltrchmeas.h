...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/dltrchmeas.h#3 $ $DateTime: 2020/01/22 08:56:27 $ $Author: pwbldsvc $

when        who     what, where, why
--------    ---     --------------------------------------------------------
01/10/20   ac      Mini dump framework checkin
05/13/16    ms      Update DL weak IND criteria for MSIM (with ISS).
03/01/16    sl      Optimization related to power control sync detection after LTA gaps
01/06/16    sg      New handlers to qsh testing
12/18/15    sl      Don't send physical channel establishment indication to RRC in CELL_FACH state
11/18/15    ac      Add dl_get_tpr_ntio() to get cell NtIo from FW-SW intf directly
11/17/15    ap      Changes to clear back up syn db if LTA resume fails
10/16/15    ap      Initial check-in for FR 30856
10/20/15    sl      QXDM cmd changes for DEMOD
10/15/15    sl      Don't reset sync_enabled flag on Sec_UL when CPHY_ in progress
09/25/15    ad      Made changes to block QTA is sync procedure is ongoing
10/12/15    sr      Changes to backup the DL sync db before LTA
...
05/29/15    ac      Double BLER log packet for W+W
...
11/20/14    sl      Changes to freeze TxAccum on Secondary until Primary comes back in sync after LTA
...
/* Minimum BCH block errors in a given accumulation when alt sub is in data */
#define DL_BCH_HIGH_BLER_OOS_MIN_ERR_COUNT_MSIM    12
/* Minimum percentage BLER on BCH TrCh when alt sub is in data */
#define DL_BCH_HIGH_BLER_OOS_MIN_BLER_PERCENT_MSIM 80
...
#define WL1_DL_SYNC_CMN_MSG(str, a, b, c) \
if (dl_sync_debug_msg_enable) \
{ \
  WL1_CMN_MSG3(HIGH,str, a, b, c); \
}\
else \
{ \
  WL1_CMN_MSG3(LOW,str, a, b, c); \
}
...
#define WL1_DL_SYNC_MSG_7(str, a, b, c, d, e, f, g) \
if (dl_sync_debug_msg_enable) \
{ \
  WL1_MSG7(HIGH, str, a, b, c, d, e, f, g);\
}\
else \
{ \
  WL1_MSG7(LOW, str, a, b, c, d, e, f, g);\
}
...
#define DL_TRCH_FILTER_ENERGY_FOR_PH1(carr_idx,temp_dpcch_eng) \
  ...
  WL1_DL_SYNC_MSG("sync_detect: PH1 accum %d, head_eng: %d, dpcch_eng: %d",                     \
                               dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_eng_accum_ph1,                                     \
                               dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_eng_data_ph1[dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_filter_head_ph1],  \
                               temp_dpcch_eng);                                                              \
  \
  dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_eng_accum_ph1 -=                                                                  \
    dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_eng_data_ph1[dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_filter_head_ph1];                             \
  \
  dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_eng_data_ph1[dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_filter_head_ph1] = temp_dpcch_eng;             \
  \
  dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_eng_accum_ph1 += temp_dpcch_eng;                                                  \
  \
  if (++dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_filter_head_ph1 == DPCCH_ENG_FILTER_LEN_PH1)                               \
    dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_filter_head_ph1 = 0;                                                           \
  } while(0)
...
#define DL_TRCH_FILTER_ENERGY_FOR_PH2(carr_idx,temp_dpcch_eng) \
  do { \
    WL1_DL_SYNC_MSG("sync_detect: PH2 accum %d, head_eng: %d, dpcch_eng: %d",                               \
    						  dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_eng_accum_ph2,                                          \
    						  dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_eng_data_ph2[dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_filter_head_ph2],   \
    						  temp_dpcch_eng);                                                               \
    dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_eng_accum_ph2 -=                                                                 \
      dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_eng_data_ph2[dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_filter_head_ph2];                          \
    \
    dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_eng_data_ph2[dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_filter_head_ph2] = temp_dpcch_eng;          \
    \
    dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_eng_accum_ph2 += temp_dpcch_eng;                                                \
    \
    if (++dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_filter_head_ph2 == DPCCH_ENG_FILTER_LEN_PH2)                              \
      dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_filter_head_ph2 = 0;                                                         \
  } while(0)

#define DL_TRCH_POST_PROCESS_ENERGY_PH1_FOR_FPDCH_WITH_CPC(carr_idx) \
  do { \
      dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].spill_dpcch_eng =     \
       ((dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].slot_level_accum_eng) - ((dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_eng_data_ph1[dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_filter_head_ph1])*dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].num_valid_slots))/SLOTS_PER_FRAME;  \
      dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_eng_accum_ph1 += dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].spill_dpcch_eng;   \
      dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].is_spill_eng_used  = TRUE;   \
      WL1_DL_SYNC_MSG("sync_detect: PH1 partial frame's num slots %d accum energy: %d",dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].num_valid_slots, dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_eng_accum_ph1, 0);   \
  } while(0)

#define DL_TRCH_POST_PROCESS_ENERGY_PH2_FOR_FPDCH_WITH_CPC(carr_idx) \
  do { \
    dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].spill_dpcch_eng =        \
      ((dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].slot_level_accum_eng) - ((dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_eng_data_ph2[dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_filter_head_ph2])*dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].num_valid_slots))/SLOTS_PER_FRAME;  \
    dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_eng_accum_ph2 += dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].spill_dpcch_eng ;   \
    dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].is_spill_eng_used = TRUE; \
    WL1_DL_SYNC_MSG("sync_detect: PH2 partial frame's num slots: %d accum energy: %d",dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].num_valid_slots, dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].dpcch_eng_accum_ph2, 0); \
  } while(0)
...
#define DL_TRCH_UPDATE_CRC_ERR_CNT(nblks,nerr) \
  do {  \
    dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.num_tb_rece_with_crc_accum -=   \
      dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.num_tb_with_crc[dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.tb_with_crc_cnt_head];  \
    dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.num_tb_with_crc[dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.tb_with_crc_cnt_head] = nblks;  \
    dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.num_tb_rece_with_crc_accum += nblks;  \
    \
    dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.num_tb_rece_with_crc_err_accum -=  \
    dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.num_tb_with_crc_err[dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.tb_with_crc_cnt_head];  \
    dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.num_tb_with_crc_err[dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.tb_with_crc_cnt_head] = nerr;  \
    dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.num_tb_rece_with_crc_err_accum += nerr;  \
    \
    if (++dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.tb_with_crc_cnt_head == DPCH_SYNC_MAX_CRC_ERR_FRAME)  \
      dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.tb_with_crc_cnt_head = 0;                                 \
    \
    if ((nblks > 0) && (nerr == nblks))   \
    {  \
      dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.consec_bad_trblk_cnt += nerr;  \
    } \
    WL1_DL_SYNC_MSG("LLSYNC2:CRC accum %u %u consec %u",  \
               dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.num_tb_rece_with_crc_accum,        \
               dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.num_tb_rece_with_crc_err_accum,    \
               dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.consec_bad_trblk_cnt);     \
  } while(0)
 ...
 */	
#define DL_TRCH_GENERATE_CRC_SYNC_SIG(carr_idx,nblks,nerr,crc_in_sync_sig,crc_out_of_sync_sig)  \
  do {  \
    if ((nblks == 0) &&                                                    \
    	(((dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.num_tb_rece_with_crc_accum -                    \
    	   dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.num_tb_rece_with_crc_err_accum) > 0) ||          \
    	 (dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.num_tb_rece_with_crc_accum == 0)))                \
    {                                                                         \
      crc_in_sync_sig = TRUE;                                                 \
    }                                                                          \
    else if ((nblks > 0) && (nerr < nblks))                                    \
    {                                                                           \
      dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.consec_bad_trblk_cnt = 0;                                 \
      crc_in_sync_sig = TRUE;                                                    \
    }                                                                            \
    if ((dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.consec_bad_trblk_cnt >= DPCH_SYNC_MAX_CRC_ERR_TB) &&  \
    	(dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.num_tb_rece_with_crc_accum > 0) &&                    \
    	(dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.num_tb_rece_with_crc_accum ==                         \
    	 dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.num_tb_rece_with_crc_err_accum) &&                  \
    	 (dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].in_sync_forty_frames == FALSE))                     \
    {                                                                           \
      crc_out_of_sync_sig = TRUE;                                               \
    }                                                                              \
    if(dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].in_sync_forty_frames == TRUE)                           \
    { \
      crc_in_sync_sig = TRUE;                                                          \
    }                                                                            \
  } while(0)
...
#define DL_TRCH_UPDATE_SYNC_CNT(carr_idx,in_sync_ind,out_sync_ind)            \
  do {                                                                \
    ...
    {                                                                  \
      WL1_MSG3(ERROR,"LLSYNC:Both in sync and out of sync ind", 0, 0, 0);   \
    }                                                                  \
    ...
    {                                                                   \
      ...
      {                                                                 \
        ...
        if ((dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].in_sync_cnt <= dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.N312) ||  \
           (dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].in_sync_cnt <= dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.N315))     \
        {                                                               \
          dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].in_sync_cnt++;                               \
        }                                                               \
        dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].out_sync_cnt = 0;                              \
      }                                                                 \
      else                                                              \
      {                                                                 \
        WL1_DL_SYNC_MSG("LLSYNC:out of sync indication", 0, 0, 0);      \
        dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].in_sync_cnt = 0;                               \
        if (dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].out_sync_cnt <= dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.N313)     \
        {                                                               \
          dltrchmeas_ext_cntrl_params[wsub_id].dl_dch_sync_vars.per_carrier_info[carr_idx].out_sync_cnt++;                              \
        }                                                               \
      }                                                                 \
    }                                                                   \
  }while(0)
...
typedef struct {
  ...
  /* We need to wait for 16 frames after LTA and check if we are in sync 
  ** for certain cases. If not in sync, we need to declare RLF.*/
  
  /*True if LTA just got over and we need to wait for 1 in-sync. Else, false.*/
  boolean dsds_in_sync_check_after_lta;
  /*Counts the number of frames after LTA if we are waiting for 1 in-sync.*/
  uint8 dsds_in_sync_check_after_lta_cntr;

  /* Accmulates the TA duration until the completion of 1st successful 160ms filtering */
  uint16 tuneaway_dur;
#endif
...
typedef enum
{
  FBI_ZERO = WFW_TX_DPCCH_FBI_ZERO,
  FBI_ONE = WFW_TX_DPCCH_FBI_ONE,
  FBI_CM_GAP_IND = WFW_TX_DPCCH_FBI_CM_GAP
}dl_tx_dpcch_tpc_enum_type;

typedef struct
{
  dl_sync_det_var_struct_type dl_dch_sync_vars;
}dltrchmeas_ext_cntrl_params_type;

extern dltrchmeas_ext_cntrl_params_type dltrchmeas_ext_cntrl_params[WCDMA_NUM_SUBS];
...
#error code not present
#endif
extern WfwTxDpcchAsyncReadStruct   *dlt_wfw_intf_dpcch_async_read_ptr[WL1_MAX_UL_CARR_IDX];
/* =======================================================================
...
extern boolean dl_trch_handle_sync_for_lta_on_sec( wsub_id_e_type wsub_id);
...
extern void dl_trch_meas_init( wsub_id_e_type wsub_id);
...
extern void dl_trch_bler_info_update(
  ...
  uint8 crc_results,
  /* CRC errors */
  uint8 crc_errors, wsub_id_e_type wsub_id);
...
extern void dl_trch_bler_info_update_w_tb_info(
  ...
  uint32 tb_crc_result, wsub_id_e_type wsub_id);
...
extern void dl_bler_periodic_report(
  ...
  uint16 report_dur_ms, wsub_id_e_type wsub_id);
...
extern void dl_trblk_crc_status_rpt(
  ...
  uint8 nfail, wsub_id_e_type wsub_id);
...
extern void dl_dch_sync_init( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
===========================================================================*/
extern void dl_trch_submit_pwrctrl_sync_logpkt( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
===========================================================================*/
extern void dl_trch_submit_ultpc_debug_logpkt( wsub_id_e_type wsub_id);
...
extern void dl_trch_submit_fdpch_debug_logpkt( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
===========================================================================*/
void dl_trch_init_multi_comb_tpc_logpkt( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
===========================================================================*/
void dl_trch_init_l1_ultpc_debug_logpkt( wsub_id_e_type wsub_id);
...
void dl_trch_submit_multi_comb_tpc_logpkt( wsub_id_e_type wsub_id);
...
extern void dl_trblk_crc_status_cmd(
  ...
  dl_trblk_crc_status_type *cmd, wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None
===========================================================================*/
extern void dl_upd_sync_parms(l1_upd_dl_sync_parms_cmd_type *cmd, wsub_id_e_type wsub_id);
...
SIDE EFFECTS     None
===========================================================================*/
void dl_trch_submit_cltd_log_packet( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
===========================================================================*/
extern boolean dl_trch_meas_get_sync_state(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
extern void dl_sync_detect(
  ...
  uint16 nerr,
  /* the current DPCCH energy sample */
  int16 dpcch_eng, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
      None
===========================================================================*/
extern void dl_trch_ilpc_debug_logpkt( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
      None
===========================================================================*/
extern boolean dl_get_sync_det_enable_status(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
      None
===========================================================================*/
extern void dl_dch_per_carr_sync_params_init(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
      None
===========================================================================*/
extern void dl_set_sync_det_enable_status(boolean status, wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
      None
===========================================================================*/
extern boolean dl_trch_is_sync_established(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
      None
===========================================================================*/
extern void dl_put_sec_carr_in_dormant_mode(boolean, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
 ===========================================================================*/
extern void dl_update_fdpch_sire_rep_status(boolean status, wsub_id_e_type wsub_id);
/*===========================================================================
FUNCTION  DL_TRCH_IS_PHASE_1_SYNC_ESTABLISHED

DESCRIPTION     
      This function checks if C0 is in PHASE-1 of Power control SYNC procedure & DPCH has been established or not
      API is newly introduced and shall be called by DSDS module to decide if QTA needs to be allowed or not

DEPENDENCIES
      None

RETURN VALUE
      TRUE if C0 is in PHASE-1 of SYNC procedure & DPCH has been established (QTA can be allowed)
      FALSE if C0 if DPCH has not been established (QTA needs to be blocked)

SIDE EFFECTS
      None
===========================================================================*/
extern boolean dl_trch_is_phase_1_sync_established(wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
 ===========================================================================*/
extern boolean wl1_qsh_dl_metric_cb(wl1_qsh_metric_e metric_id, uint8* addr_ptr, wl1_qsh_metric_action_enum_type action, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_qsh_voice_bler_metric_cb(wl1_qsh_metric_e metric_id, uint8* addr_ptr, wl1_qsh_metric_action_enum_type action, wsub_id_e_type wsub_id);
#endif /* WL1_FEATURE_QSH_EVENT_METRIC */

#ifdef FEATURE_WCDMA_TPR_BASED_MEASUREMENTS
#error code not present
#endif /* #ifdef FEATURE_WCDMA_TPR_BASED_MEASUREMENTS*/
...
/*===========================================================================
FUNCTION wl1_qsh_mdump_collect_demod

DESCRIPTION
  This function collects mdump from all the mdump structures which are not sub specific.
  This has been done so that this function gets called only one time. Putthing this api
  inside the wl1_qsh_mdump_collect_demod will cause the dump collect apis to be called 
  twice, once for each sub.
...
SIDE EFFECTS
  None
===========================================================================*/
extern QSH_MDUMP_FN_ATTR void wl1_qsh_mdump_collect_demod_common();
...
DESCRIPTION
  This function collects mdump from all the mdump structures which are sub specific.
...
SIDE EFFECTS
  None
===========================================================================*/
extern QSH_MDUMP_FN_ATTR void wl1_qsh_mdump_collect_demod(wsub_id_e_type wsub_id);
#endif
