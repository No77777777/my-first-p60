...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/l1utils.h#5 $
$DateTime: 2020/03/03 06:12:10 $
$Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------
03/03/20   mk      Clade specific changes for WCDMA onHabanero 
05/24/16   ac      Disable FET during DSR transitions
10/17/17   am      Do not send FW sleep cmd in FTM mode during connected mode cleanup.
01/16/18   nd     Moved logging of XO parameters to frame boundary event.
12/07/17   sl      EDRX + MSIM design revisit for Long Tune Away gaps
06/10/16   nd/ap   QSH changes to support SSIM on SUB2 when DUAL WCDMA is not defined
01/20/16   nd      Fixed compilation error
01/06/16   sg      New handlers to qsh testing
10/16/15   rsr     Added Changes for Common Messages.
11/20/15   kr      Change NPA req with MCPM boost for 20 msec
11/04/15   vn      API to get slots to start CM
10/15/15   pr      Check if UE is in WplusW specific FTM mode for cxm specific activities.
09/11/15   ap/sr   QSH releated changes for THOR
07/30/15   pv      API to dump RF WTR/ASM registers.
08/03/15   gn      Fix Compilation warnings
07/22/15   vn      Externed function to get RF Device ID
07/17/15   pvr     QDTA changes
06/06/15   kr      Corrt JO featurization
06/03/15   svh     TA featurisation.
06/03/15   pr      W+W Phase8 Checkin.
06/03/15   sl      Code changes for new f/w log packet 0x41C4 V1
06/02/15   sks     Remove usage of disable_rtr_power_collapse variable.
...
04/24/15   gsk     Adding SUBID for custom level F3s
04/10/15   ks      New RFM meas interface changes
04/08/15   pr      WL1 changes for FR25951:Single Voice Session
03/23/15   rsr     W + W Phase 6 Checkin.
03/19/15   gsk     Enabling custom level F3s
...
#if defined (FEATURE_WCDMA_QSH_DUMP) || defined(FEATURE_WCDMA_QSH_EVENT_METRIC) || defined(FEATURE_QSH_EVENT_NOTIFY_TO_QSH) 
#include "wl1_qsh_ext.h"
#endif /* defined (FEATURE_WCDMA_QSH_DUMP) || defined(FEATURE_WCDMA_QSH_EVENT_METRIC) || defined(FEATURE_QSH_EVENT_NOTIFY_TO_QSH) */
...
/* variable to maintain remainining time to decode page for qsh */
...
#define WL1_DSR_CALLBACK_FET    0x10
...
/* get n th byte from lsb */
#define GET_NBYTE_LSB(word, nthbyte) ((uint8)((word >>((nthbyte-1)*8))&(0x000000FF))) 
...
typedef struct 
{
  ...
  wl1_dsr_pending_type transition_pending;
}wl1_dsr_db_type;
...
typedef enum
{
  /* in this mode SW won't send any commands to RF for adjusting the tuner */
  WL1_ATUNER_INACTIVE_MODE,
  /* in this mode, we periodically allow RF apply many small tuner updates at 
     frame boundaries to minimize harm from abrupt phase jumps */
  WL1_ATUNER_NORMAL_MODE,
  /* in this mode, we allow RF to apply full tuner updates at the edges of CM
     measurement gaps, where abrupt phase jump don't matter */
  WL1_ATUNER_CM_PATTERN_MODE,
  /* in this mode, we allow RF to apply full tuner updates at the edges of FMO
     measurement gaps, where abrupt phase jump don't matter */
  WL1_ATUNER_FMO_PATTERN_MODE,
} wl1_atuner_ctrl_mode_type;

typedef struct
{
  /* atuner module's mode. Changes with respect to FMO/CM gaps active and 
     init/deinit */
  wl1_atuner_ctrl_mode_type mode;
  /* timer used to trigger periodic processing */
  timer_type timer;
} wl1_atuner_ctrl_info_type;
...
typedef struct{
  ...
  int32 xoRPushValue;
  int32 xoAdjAccumCurr;
  int32 xoUnBiasedWtFreqSum;
  int16 freq_common;
  uint8 xoUseFreqHypothesis;
}wl1_xo_params_struct_type;
...
typedef struct
{
  ...
  boolean wl1_mcpmdrv_w2g_client_status;
  #endif
  /* offset value in the log buffer*/
  uint16 wcdma_periodic_stat_sample_buf_offset;
  rex_crit_sect_type wl1_gsm_clk_ctrl_crit_sect;
  /* Flag for the status of the log code in QXDM */
  boolean wcdma_periodic_stat_logging_enabled_qxdm;
  /* Flag indicating whether a packet has been submitted to diag */
  boolean wcdma_periodic_stat_log_submit;
  /* Buffer index where samples are accumulated */
  uint8 wcdma_periodic_stat_log_info_buf_idx;
  boolean wl1_fw_app_suspended;  
  #ifdef FEATURE_DUAL_SIM
  boolean wl1_use_backed_up_tcxo_mgr_rot;
  #endif
  /* Log buffer availability status database */
  boolean wcdma_periodic_stat_log_buf_avail_status[WCDMA_PERIODIC_STAT_LOG_NUM_BUF];
  /* Flag to store whether a HS log submit local command is pending or not */
  boolean wcdma_periodic_stat_log_cmd_pending;
  /* The log buffer index to be submitted */
  uint8 wcdma_periodic_stat_log_buf_idx_to_submit;
#ifdef FEATURE_XO
  /* Log XO related parameters */
  wl1_xo_params_struct_type xo_params;
#endif
  /* Mutex for all HS configuration */
  rex_crit_sect_type dsr_crit_sec;
  /* The log buffer where samples are accumulated */
  WCDMA_PERIODIC_STAT_LOG_PKT_type wcdma_periodic_stat_log_pkt_buf[WCDMA_PERIODIC_STAT_LOG_NUM_BUF];
  wcdma_icommon_event_callback_fn_t wl1_audio_cb;
  boolean enable_mvs_cb_notification;
}l1utils_ext_cntrl_params_type;

extern l1utils_ext_cntrl_params_type l1utils_ext_cntrl_params[WCDMA_NUM_SUBS];
...
typedef boolean wl1_metric_cb_type(wl1_qsh_metric_e metric_id, uint8* addr_ptr, wl1_qsh_metric_action_enum_type metric_action, wsub_id_e_type wsub_id);
...
}wl1_qsh_metric_per_sub_struct_type;

typedef struct
{
  uint8 metric_expiry_bitmask;
  wl1_qsh_metric_per_sub_struct_type per_sub_info[WCDMA_NUM_QSH_SUBS][WL1_QSH_METRIC_MAX];
...
extern wl1_qsh_irat_info_struct_type* wl1_qsh_dump_tag_8_ptr;
extern wl1_qsh_datapath_info_struct_type* wl1_qsh_dump_tag_9_ptr;
#ifdef FEATURE_DUAL_SIM
extern wl1_dsds_mini_debug_struct_type* wl1_qsh_dump_tag_10_ptr;
#endif /*FEATURE_DUAL_SIM*/
/*extern wfw_qsh_dump_s * wfw_qsh_dump_tag_ptr;*/
...
)
{
  ...
  if (ptr == NULL)
  {
    WL1_ERR_FATAL("modem_mem_alloc failed for WL1, requested size %d bytes", size, 0, 0);
  }
...
INLINE void* wl1_clade_mem_alloc
(
  size_t               size       /*!< Number of bytes to allocate */
)
{
  void *ptr;
  /* Allocate 'size' bytes from clade mem heap */
  ptr = clade_heap_alloc (size);

  if (ptr == NULL)
  {
    WL1_ERR_FATAL("clade_heap_alloc failed for WL1, requested size %d bytes", size, 0, 0);
  }
  memset(ptr, 0, size);
...
INLINE void wl1_clade_mem_free
(
  void                *ptr        /*!< Memory to free */
)
{
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void l1_nv_process_nv_cmd_sig( wsub_id_e_type wsub_id);

/*==========================================================================
FUNCTION WL1_GET_RF_DEVICE_ID

DESCRIPTION
  This function is determines the device ID corresponding to the freq that W SUB is tuned to currently.
...
extern void wl1_qsh_report_event(wl1_qsh_event_e event_id,wsub_id_e_type sub_id);
/*==========================================================================
FUNCTION WL1_QSH_REPORT_EVENT

DESCRIPTION
  This function is used to report events to QSH.
...
===========================================================================*/
#endif
extern rfm_device_enum_type wl1_get_rf_device_id (wsub_id_e_type wsub_id);
/*===========================================================================
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void wl1_gsm_clk_ctrl_lock_mutex( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void wl1_gsm_clk_ctrl_unlock_mutex( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern boolean wl1_query_gsm_clock_state( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void wl1_enable_fee_clock( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void wl1_disable_fee_clock( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  TX clock is enabled.
===========================================================================*/
extern void wl1_enable_tx_and_clock(rfcom_tx_multi_carrier_cfg_type rf_carr_cfg, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void wl1_disable_tx_and_clock( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void wl1_entering_sleep( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void wl1_exiting_sleep( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void wl1_enable_clocks_exiting_sleep( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void wl1_disable_clocks_entering_sleep( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void wl1_enable_clocks( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void wl1_disable_clocks( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void wl1_disable_slp_fast_clock( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void wl1_enable_slp_fast_clock( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean wl1_write_fbiber_threshold(uint16 threshold, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean wl1_read_fbiber_threshold(uint16 *threshold, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern int32 wl1_convert_freq_err_to_ppm(int32 raw_freq_error, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern int16 wl1_convert_ppm_to_freq_err(int16 ppm_times_one_hundred, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void wl1_update_rgs_values(int32 mdsp_pdm, int32 mdsp_rot, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern tcxomgr_vco_info_type wl1_get_stored_vco( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern uint32 wl1_check_gstmr_rd_status( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void l1utils_init ( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void wl1_wfw_set_tcxo_freeze_flag(boolean tcxo_frozen, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void wl1_wfw_set_use_freq_hypo_flag(boolean use_freq_hypo, wsub_id_e_type wsub_id);

#ifdef FEATURE_WCDMA_TABASCO_MODEM
/*===========================================================================
FUNCTION     WL1_SET_CURR_OFFLINE_CLK_SPEED_STATE
...
===========================================================================*/
extern void wl1_set_curr_srch_clk_speed_state(uint32 new_srch_clk_speed, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION     WL1_GET_CURR_SRCH_CLK_SPEED_STATE
...
===========================================================================*/
extern uint32 wl1_get_curr_srch_clk_speed_state( wsub_id_e_type wsub_id);
#else
extern void wl1_set_curr_srch_clk_speed_state(wl1_srch_clk_speed_state_type new_srch_clk_speed, wsub_id_e_type wsub_id);
extern wl1_srch_clk_speed_state_type wl1_get_curr_srch_clk_speed_state( wsub_id_e_type wsub_id);
#endif
...
uint32 wl1_get_carrier_frequency(uint8 carr_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern int32 wl1_convert_ppm_to_freq_err_for_rxfe(int32 ppm_times_one_hundred, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void wl1_afc_ttl_init( wsub_id_e_type wsub_id);
/*===========================================================================
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void wl1_get_xo_params( wsub_id_e_type wsub_id);

#endif /* FEATURE_XO */

#ifdef FEATURE_WCDMA_MCPM_DRIVER
boolean wl1_wait_for_gsm_frame_boundary( wsub_id_e_type wsub_id);
#endif 
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_is_fw_app_enabled(wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_is_fw_app_suspended( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void wl1_dsr_controller(uint32 callback_mask, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
=============================================================================*/
extern boolean wl1_dsr_state_machine_active( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
=============================================================================*/
extern void wl1_dsr_cleanup( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
=============================================================================*/
extern void wl1_dsr_qual_meas_update(uint8 num_tbs, uint8 num_err, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
=============================================================================*/
extern void wl1_dsr_init( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
=============================================================================*/
extern void wl1_dsr_read_nv_item( wsub_id_e_type wsub_id);
...
/*===========================================================================
 ...
 SIDE EFFECTS
   None.
===========================================================================*/
extern void wl1_freeze_xo_loops( wsub_id_e_type wsub_id);
/*===========================================================================
 ...
 SIDE EFFECTS
   None.
===========================================================================*/
extern void wl1_unfreeze_xo_loops( wsub_id_e_type wsub_id);
#endif /* FEATURE_XO */
/*===========================================================================
...
===========================================================================*/
extern rex_tcb_type* wcdma_l1_get_tcb(wsub_id_e_type wsub_id);
...
extern void wl1_advance_snapshot_to_cfn_time(
  ...
  wl1_gwl_snapshot_type *out_snapshot, wsub_id_e_type wsub_id);
...
extern void wl1_get_irat_snapshot(
  ...
  wl1_gwl_snapshot_type *snapshot, wsub_id_e_type wsub_id);
...
/*===========================================================================
 ...
 SIDE EFFECTS
   An assertion will fail if the lookup fails.
===========================================================================*/
extern void wcdma_l1_init_tcb( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  Sets global flag l1utils_int_cntrl_params.wl1_atuner_proc_period_ms
=============================================================================*/
extern void wl1_read_atuner_nv_settings( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_atuner_init( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_atuner_deinit( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_atuner_evt_processor(wl1_atuner_event_type evt, wsub_id_e_type wsub_id);
...
extern void wl1_atuner_fmo_state_change_handler(
  ...
  boolean meas_start, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wcdma_periodic_stat_log_init( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wcdma_periodic_stat_log_update( wsub_id_e_type wsub_id);
...
DEPENDENCIES
  l1utils_ext_cntrl_params.wcdma_periodic_stat_log_info_buf_idx should be valid while making
...
SIDE EFFECTS
  ===========================================================================*/
void wcdma_periodic_stat_stop_logging ( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void wcdma_periodic_stat_send_log_submit_cmd(uint8 wcdma_periodic_stat_log_info_buf_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
void wcdma_periodic_stat_submit_log_buffer( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void wcdma_periodic_stat_submit_log_cmd( wsub_id_e_type wsub_id);

#ifdef FEATURE_DUAL_SIM
extern void wl1_backup_freq_error_at_acq_succ( wsub_id_e_type wsub_id);
#endif
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_is_gsm_clk_enabled_by_module(uint8 module_bmask, wsub_id_e_type wsub_id);
...
extern void wl1_tcxo_set_restriction(tcxomgr_restriction_type restriction,
                                     wl1_tcxomgr_ack_type ack, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void wl1_tcxo_release( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void wl1_trm_set_starting_trk_lo( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern boolean wl1_tcxo_frozen( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void wl1_tcxo_disable_pdm_for_sleep( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void wl1_tcxo_enable_pdm_for_wake( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern boolean wl1_tcxo_rpush(tcxomgr_client_state_type state, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void wl1_tcxo_rpush_init( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void wl1_tcxo_rpush_cleanup( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void wl1_tcxo_request( wsub_id_e_type wsub_id);
...
/*===========================================================================
FUNCTION wl1_is_mode_FTM_NONSIG

DESCRIPTION
  This function returns whether the UE is in FTM non-signalling mode or not

DEPENDENCIES
  RF

RETURN VALUE
  TRUE - If mode is WL1_RFM_WCDMA_FTM_NONSIG
  FALSE - Otherwise.

SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_is_mode_FTM_NONSIG(void);

/*===========================================================================
FUNCTION wl1_is_mode_ftm_dual_wcdma

DESCRIPTION
  This function returns whether the UE is in DUAL WCDMA Specific FTM mode or not

DEPENDENCIES
  RF

RETURN VALUE
  TRUE - If mode is (FTM_W_PLUS_W/FTM_CAL)
  FALSE - Otherwise.
...
extern boolean wl1_is_mode_ftm_dual_wcdma(void);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_read_supported_clk_speeds( wsub_id_e_type wsub_id);
/*===========================================================================
...
SIDE EFFECTS
  None
===========================================================================*/
extern wl1_max_clk_speeds_type wl1_get_max_supported_q6_speeds( wsub_id_e_type wsub_id);
#endif /*FEATURE_JOLOKIA_MODEM*/
extern boolean wl1_set_gsm_clk_mask_status(uint8 module_bmask, wsub_id_e_type wsub_id);
extern boolean wl1_reset_gsm_clk_mask_status(uint8 module_bmask, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION l1utils_int_cntrl_params_init
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1utils_int_cntrl_params_init( wsub_id_e_type wsub_id);

extern void wl1_start_stop_voice_adapter(wsub_id_e_type wsub_id, boolean start_stop);

extern void wl1_fill_source_rf_params(rfm_meas_common_param_type* rf_info, boolean is_qdta, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void wl1_dsr_debug_dump(wl1_qsh_datapath_info_struct_type *dump_ptr);
#endif /* FEATURE_WCDMA_QSH_DUMP*/
/*===========================================================================
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_qsh_cb_process(qsh_client_cb_params_s* wl1_qsh_params_ptr, wsub_id_e_type wsub_id);
...
#endif /* FEATURE_WCDMA_QSH_EVENT_METRIC || defined FEATURE_WCDMA_QSH_DUMP*/

/*===========================================================================
FUNCTION wl1_get_xo_async_write_buf

DESCRIPTION
  This function provides the SW/FW xo write interface.
...
SIDE EFFECTS
  None
===========================================================================*/
extern WfwMiscXoAsyncWriteStruct* wl1_get_xo_async_write_buf(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_get_xo_async_read_buf

DESCRIPTION
  This function provides the SW/FW xo read interface.
...
SIDE EFFECTS
  None
===========================================================================*/
extern WfwMiscXoAsyncReadStruct* wl1_get_xo_async_read_buf(wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wcdma_timeline_profiling_log(wsub_id_e_type wsub_id);

#endif /*FEATURE_WCDMA_TABASCO_MODEM */

/*===========================================================================
FUNCTION wl1_dump_rf_registers

DESCRIPTION
  This API dumps the RF WTR and ASM register and prints it.
...
SIDE EFFECTS
  None
===========================================================================*/
void wl1_dump_rf_registers(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_srch_get_curr_freq

DESCRIPTION
  This function returns the current freq
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint16 wl1_srch_get_curr_freq(wsub_id_e_type wsub_id);
/*===========================================================================
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint16 wl1_slot_to_start_cm (wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION        wl1_check_if_target_time_elapsed

DESCRIPTION     This API checks if target time has elapsed. The logic is, from 
a common reference time, number of frames to the current time and number of 
frames to the target time is determined. If num_frames to target time is more 
than num_frames to current time then we haven't reached the target time else
we have already passed the target time

DEPENDENCIES    None

RETURN VALUE    TRUE -if target time is not reached, FALSE-other

SIDE EFFECTS    None
===========================================================================*/
extern boolean wl1_check_if_target_time_elapsed(
  int16 ref_time, int16 cur_time, int16 target_time, 
  int16 delta_time, int16 max_time_count, wsub_id_e_type wsub_id);
#endif /* L1UTILS_H */
