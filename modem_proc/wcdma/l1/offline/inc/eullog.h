...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/eullog.h#3 $
...
11/17/15    raj     Fix the timestamp of 0x4344 log packet during ASU
08/27/15    ar      Ensure that spreading factor field bitmask of 0x435C packet covers reserved bits
08/12/15    cjz     data type fix for tx_agc_buf in 0x4346
06/22/15    gp      Compilation and logging fixes for FEATURE_WCDMA_DC_HSUPA undefined
06/15/15    ac      Fix compilation errors for BO
06/08/15    cjz     DC-HSUPA and UL-16QAM logging coexistence
05/23/15    vr      Remove incorrect DCUPA featurization in ul_tx_rf_send_log_pkt_to_diag.
05/20/15    ssv     Changing Apex Version for Jolokia to get mac- inactivty table generated in 0x422c log packet.
03/28/15    vr      Correctlng 0x4346/0x4323/0x4322 flush logic when log buffer in not full
01/16/15    vr      Global reorg for W + W.
01/13/15    ash     Enhancing 0x4344 log pkt with TX Blanking Information
...
#define e_ul_flush_logpkts(reset_l1_mac_logpkts)\
        e_ul_edpch_info_pkt_handler(wsub_id);\
        e_ul_send_etfc_restrict_info_to_diag(wsub_id);\
        e_ul_send_rm_info_to_diag(wsub_id);\
        e_ul_cleanup_log_pkts_data(reset_l1_mac_logpkts ,wsub_id);\
        eul_dtx_send_fw_log_pkt_to_diag(wsub_id); \
        ul_tx_rf_send_log_pkt_to_diag(wsub_id)
#else
#define e_ul_flush_logpkts(reset_l1_mac_logpkts)\
        e_ul_edpch_info_pkt_handler(wsub_id);\
        e_ul_send_etfc_restrict_info_to_diag(wsub_id);\
...
#define EUL_GET_ETTI()           eulcfg_ext_cntrl_params.e_ul_curr_phychan_cfg_db_ptr->tti
#define EUL_GET_ETFCI_TAB_IDX()  eulcfg_ext_cntrl_params.e_ul_curr_phychan_cfg_db_ptr->e_tfci_table_idx
...
/* The bitmask covers the reserved bits also to ensure they get set to 0
 * and don't cause any out of bounds access in other fields.
 */
#define EUL_UL_LOG_PKT_R_COMBO_TABLE_SF_BMSK            0xFF
...
#define EUL_UL_LOG_PKT_COMBINED_L1_MAC_TX_BLANKING_BMSK      0x20
#define EUL_UL_LOG_PKT_COMBINED_L1_MAC_TX_BLANKING_SHFT        5
...
typedef PACKED struct PACKED_POST {
 ...
 /*COMB_HICH:Indicates the combined ACK/NAK for the transport block 
   ...
   TX BLANKING: 
   1: Indicates TX Blanking in this TTI
   0: Indicates No TX Blanking in this TTI
 */
 uint8 hich_rgch_dtx_happy_cmgap_tx_blanking_info;
...
#define EUL_L1_MAC_LOG_PKT_FLUSH(flush_log_pkt, carr_idx) \
    (flush_log_pkt == TRUE) &&(eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].valid == FALSE)

#define EUL_L1_MAC_LOG_PKT_UPDATE_TIMESTAMP_HARQ_RT(timestamp, harq_rtt, tti, index)\
do{\
            timestamp[1] = 0;\
            timestamp[0] = index * ((tti == E_TTI_10MS)? EUL_LOG_MS_IN_10_MS_TTI : EUL_LOG_MS_IN_2_MS_TTI );\
            time_conv_ms_to_timestamp( timestamp);\
            timestamp[0] &= EUL_LOG_TIMESTAMP_BMSK;\
            harq_rtt = ((tti == E_TTI_10MS)? EUL_HARQ_RTT_10MS : EUL_HARQ_RTT_2MS );\
}while(0)
...
#define EUL_L1_MAC_LOG_PKT_FILL_FLUSH_DATA_HDR_DB(cur_hdr_idx, num_samples,timestamp_inc, carr_idx, harq_rtt) \
    eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].hdr_info= \
                      eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_sample_info[carr_idx].hdr_info[cur_hdr_idx];\
    eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].hdr_info.start_frame_num -= num_samples;\
    qw_sub(eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].hdr_info.time_stamp,\
           eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].hdr_info.time_stamp,\
           (timestamp_inc));\
    eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].hdr_info.harq_id =\
           (eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].hdr_info.harq_id-num_samples\
                                      + harq_rtt)%harq_rtt
...
#define EUL_L1_MAC_LOG_PKT_FILL_FLUSH_DATA_HDR_SB(cur_hdr_idx, num_samples, timestamp_inc, carr_idx, harq_rtt) \
    eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].valid = TRUE;\
    eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].hdr_info=\
                      eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_sample_info[carr_idx].hdr_info[cur_hdr_idx];\
    qw_add(eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].hdr_info.time_stamp,\
           eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].hdr_info.time_stamp,\
           (timestamp_inc));\
    eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].hdr_info.start_frame_num += num_samples;\
    eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].hdr_info.harq_id =\
           (eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].hdr_info.harq_id  +\
                                            num_samples )%harq_rtt
...
#define EUL_L1_MAC_LOG_PKT_FILL_FLUSH_DL_DATA_HDR(carr_idx)\
    ...
    {\
      eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].hdr_info.edch_cell_tpcid_info[index]=\
                                           EUL_GET_ACTIVE_RL_TPC_IDX(index, carr_idx);\
      eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].hdr_info.edch_cell_psc_info[index]=\
                                           EUL_GET_ACTIVE_RL_PSC(index, carr_idx);\
      eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].hdr_info.edch_cell_rlsid_info[index]=\
                                           EUL_GET_ACTIVE_RL_RG_COMB_IDX(index, carr_idx);\
    }\
    eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].hdr_info.num_cells = (EUL_GET_EDCH_NUM_CELLS())-1
    
#define EUL_L1_MAC_LOG_PKT_SEND_FLUSH_DATA_TO_DIAG(num_samples, carr_idx)\
     (eulcfg_ext_cntrl_params.e_ul_l1_mac_log_pkt_flush_info[carr_idx].valid == TRUE ) &&( num_samples > 0)
...
#define UL_RF_TX_LOG_PKT_START_CFN_BMSK   0x0FFF
#define UL_RF_TX_LOG_PKT_TTI_BMSK   0x1000
...
typedef PACKED struct PACKED_POST
{
  ...
  int16 tx_agc_buffered;    /* Buffered TxAGC value in AGC units */
...
LOG_RECORD_DEFINE(UPLINK_TX_RF_DEBUG_LOG_PKT)
  uint8                            version;
  uint8                            carrier_state;
  uint16                           ul_tx_rf_start_cfn;  /* for 2ms, this field is populated with SubFN in Version-2*/
...
#ifdef FEATURE_WCDMA_DC_HSUPA
#define EUL_FW_DTX_LOG_PKT_VER         4
#else
...
===========================================================================*/
extern void e_ul_edpch_info_pkt_handler( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_ul_l1_mac_pkt_handler( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_ul_l1_mac_pkt_flush_handler(boolean edl_recfg, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void   e_ul_send_etfc_restrict_info_to_diag( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void   e_ul_send_rm_info_to_diag( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void   e_ul_send_l1_mac_stats_info_to_diag( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void   e_ul_send_edpch_info_pkt_to_diag( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_ul_logging_cmd_handler(eul_ul_logging_done_cmd_enum_type log_pkt_type, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void dc_hsupa_activation_logging_cmd_handler( wsub_id_e_type wsub_id);
/*===========================================================================
...
===========================================================================*/
extern void dc_hsupa_deactivation_logging_cmd_handler( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_ul_cleanup_log_pkts_data(boolean reset_l1_mac_logpkts, wsub_id_e_type wsub_id);
#endif /*EULLOG_H*/
