...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/l1macdata.h#4 $ $DateTime: 2021/02/10 03:14:23 $ $Author: pwbldsvc $

when        who     what, where, why
--------    ---     --------------------------------------------------------
02/09/21    sp      Compiler upgradation changes ATHERTON 
04/12/16    sg      Fix to double L2 datapath mutex to avoid EUL ISR delay in W+W scenario
03/14/16    raj     Fix to log ORD after SCCH order received in 0x4344 packet
11/06/15    mk      RACH vs QTA changes
10/21/15    gp      Fix for QTA reservation denied due to RACH frames reservation when no data in MAC buffer for Tx 
09/14/15    kv      Update L2 structure with ETFCI reason and expose in l1macdata.h
08/18/15    cjz     L1 change for 0x4345 EUL Link Statistics log packet 
...
03/26/15    kv      Expose wrapper API to L1 to ne called for flushing log packets during seamless reconfig
03/19/15    aa      Removed extern declarations for some unused L2 APIs
...
typedef enum{
  ...
  ** TEBS == 0 have been satisfied*/
  ...
  /* Release as UE couldn't resync after QTA */ 
  WL1_HS_RACH_EDCH_REL_MSIM_QTA,

  /* Release as moving to LTA and force SI-0 is triggered*/
  WL1_HS_RACH_EDCH_REL_MSIM_LTA,
...
{
  INVALID_SI = 0
 ,SI_QTA_END          /* Standard SI triggered in L2 after QTA end*/
  #ifdef FEATURE_WCDMA_EFACH_MSIM
 ,SI_0_FORCED           /* Stand alone SI - 0 triggered even when TEBS is non-zero in HS-RACH*/
  #endif
} si_trigger;
...
#ifdef __GNUC__
#define L1_DL_TB_SET_HDR(hdr_struct_ptr,member) hdr_struct_ptr->info.non_hs.member
#else /* __GNUC__ */
#define L1_DL_TB_SET_HDR(hdr_struct_ptr,member) hdr_struct_ptr->info.non_hs.##member
#endif /* __GNUC__ */
...
typedef enum
{
  ...
  ETFCI_LTD_RE_TX  = 10,
  /* UE is activating DC-HSUPA secondary carrier */
  ETFCI_LTD_BY_ORD = 13,  
  /* Applicable when ETFCI is restricted by Config*/
  ETFCI_LTD_BY_CFG = 14,
  /* Applicable when ETFCI is restricted by QTA*/
  ETFCI_LTD_BY_QTA = 15,
...
typedef struct
{
  ...
  rex_crit_sect_type e_ul_isr_l2_datapath_mutex[WCDMA_NUM_SUBS];
  uint32 l1_p2_ul_ctrl_wrd_idx;
} euledpch_exp_cntrl_params_type;

extern euledpch_exp_cntrl_params_type euledpch_exp_cntrl_params;

typedef struct
{
  #ifdef FEATURE_WCDMA_FREE_FLOATING_TASK
  rex_crit_sect_type wl1_tfc_excess_pwr_states_mutex;
  #endif /* FEATURE_WCDMA_FREE_FLOATING_TASK */
  #ifdef TEST_FRAMEWORK
  #error code not present
#endif
} wenc_exp_cntrl_params_type;

extern wenc_exp_cntrl_params_type wenc_exp_cntrl_params;

#define GET_WL1_EUL_ISR_DATAPATH_MUTEX(as_id)  (&euledpch_exp_cntrl_params.e_ul_isr_l2_datapath_mutex[as_id])
#define GET_UL_L1_P2_UL_CTRL_WRD_IDX()    (euledpch_exp_cntrl_params.l1_p2_ul_ctrl_wrd_idx)
#define GET_UL_DEBUG_EUL_DL_LOGGING()     (0)

#ifdef FEATURE_WCDMA_FREE_FLOATING_TASK
#define MAC_WL1_TFC_EXCESS_PWR_INTLOCK()   REX_ISR_LOCK(&wenc_exp_cntrl_params.wl1_tfc_excess_pwr_states_mutex)
#define MAC_WL1_TFC_EXCESS_PWR_INTFREE()   REX_ISR_UNLOCK(&wenc_exp_cntrl_params.wl1_tfc_excess_pwr_states_mutex)
#else
...
=============================================================================================*/
extern void l1_phy_dl_flush_tsn(sys_modem_as_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_phy_dl_trblk_hs_data_ind(l1_dl_hs_tti_enum_type tti_type, sys_modem_as_id_e_type wsub_id);
...
(
  ...
  a2_iovec_t a2_dec_src_iovec_list[], wsub_id_e_type wsub_id
);
...
/*===========================================================================
FUNCTION    MAC_HS_UL_TRIGGER_SI_DUE_TO_TA

DESCRIPTION  Function called by L1 to trigger SI after QTA gap or Force SI-0
             before start of LTA if EFACH_MSIM is enabled
===========================================================================*/
extern void mac_hs_ul_trigger_si_due_to_ta(sys_modem_as_id_e_type as_id, si_trigger type_of_trigger);
...
(
  ...
  l1_prach_status_enum_type   status  , sys_modem_as_id_e_type    as_id
);
...
extern uint8 l1_edch_query_mac_pwr_offset_info( 
  ...
  , l1_eul_carr_idx_enum_type carr_idx,
   sys_modem_as_id_e_type as_id
);
...
, boolean sec_carr_active, sys_modem_as_id_e_type as_id
);
/*===========================================================================
...
extern  boolean l1_edch_req_low_grant_timer_state(l1_eul_carr_idx_enum_type  l1_eul_carr_idx, sys_modem_as_id_e_type as_id);
...
(
  ...
  l1_l2_data_type *data_buf, sys_modem_as_id_e_type as_id
);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_rxd_fach_signalling_data_available(boolean status, wsub_id_e_type wsub_id);
...
SIDE EFFECTS:
==========================================================================*/
extern boolean l1_query_mac_rach_data(sys_modem_as_id_e_type as_id, boolean *srb_data_present);
...
SIDE EFFECTS  None
===========================================================================*/
extern void l1_mac_hsrach_eagch_decode_ind(boolean agch_decode_status, sys_modem_as_id_e_type as_id);
...
SIDE EFFECTS  None
===========================================================================*/
extern void l1_mac_hsrach_upd_valid_timer_status
(
  l1_mac_hs_rach_eul_timers_status_struct_type *timer_status, 
  sys_modem_as_id_e_type as_id 
#ifdef FEATURE_WCDMA_EFACH_MSIM
  , boolean e_ul_hsrach_in_gap
#endif
);
/*===========================================================================
...
SIDE EFFECTS  None
===========================================================================*/
extern void l1_mac_hsrach_edch_resrc_rel_ind(l1_mac_hs_rach_edch_rsrc_rel_cause_enum_type rel_cause, sys_modem_as_id_e_type as_id);
...
SIDE EFFECTS  None
===========================================================================*/
extern void l1_mac_hsrach_cell_resel_ind(sys_modem_as_id_e_type as_id);
...
SIDE EFFECTS  None
===========================================================================*/
extern void l1_mac_hsrach_first_edch_tx_ind(sys_modem_as_id_e_type as_id);
...
SIDE EFFECTS  None
===========================================================================*/
extern void l1_mac_hsrach_preamble_proc_status(l1_hsrach_status_enum_type  status, sys_modem_as_id_e_type as_id);
...
SIDE EFFECTS  None
===========================================================================*/
extern boolean l1_mac_query_tebs_zero(sys_modem_as_id_e_type as_id);
...
SIDE EFFECTS  None
===========================================================================*/
extern void l1_mac_hsrach_get_mac_config_params(l1_mac_hs_rach_config_params *config_params,sys_modem_as_id_e_type as_id);
...
/* Wrapper functions for L2 to access L1 decoder data */

/*===========================================================================
FUNCTION     DL_DEC_TSN_Q_INTLOCK

DESCRIPTION
  Wrapper function for locking dec_tsn_q_crit_sect to be used by L2.
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void dl_dec_tsn_q_intlock(void);

/*===========================================================================
FUNCTION     DL_DEC_TSN_Q_INTFREE

DESCRIPTION
  Wrapper function for unlocking dec_tsn_q_crit_sect to be used by L2.
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void dl_dec_tsn_q_intfree(void);

/*===========================================================================
FUNCTION mcalwcdma_decdata_get_tsn_info_q

DESCRIPTION
  Returns pointer to tsn_info_q (for use outside L1)
 
DEPENDENCIES
  None

RETURN VALUE
  Returns pointer to tsn_info_q

SIDE EFFECTS
  None
==========================================================================*/
extern q_type* mcalwcdma_decdata_get_tsn_info_q(void);

/*===========================================================================
FUNCTION mcalwcdma_decdata_get_tsn_info_free_q

DESCRIPTION
  Returns pointer to tsn_info_free_q (for use outside L1)
 
DEPENDENCIES
  None

RETURN VALUE
  Returns pointer to tsn_info_free_q

SIDE EFFECTS
  None
==========================================================================*/
extern q_type* mcalwcdma_decdata_get_tsn_info_free_q(void);

/*===========================================================================
FUNCTION mcalwcdma_decdata_get_concatenated_pdu_tsn_q

DESCRIPTION
  Returns pointer to concatenated_pdu_tsn_q (for use outside L1)
 
DEPENDENCIES
  None

RETURN VALUE
  Returns pointer to concatenated_pdu_tsn_q

SIDE EFFECTS
  None
==========================================================================*/
extern q_type* mcalwcdma_decdata_get_concatenated_pdu_tsn_q(void);

/*===========================================================================
FUNCTION mcalwcdma_decdata_get_concatenated_pdu_tsn_free_q

DESCRIPTION
  Returns pointer to concatenated_pdu_tsn_free_q (for use outside L1)
 
DEPENDENCIES
  None

RETURN VALUE
  Returns pointer to concatenated_pdu_tsn_free_q

SIDE EFFECTS
  None
==========================================================================*/
extern q_type* mcalwcdma_decdata_get_concatenated_pdu_tsn_free_q(void);

#ifndef FEATURE_WCDMA_L1_ZI_OPT 
/*===========================================================================
FUNCTION mcalwcdma_decdata_is_tsn_not_within_buf

DESCRIPTION
  Checks if given pointer is within mcalwcdma_dec_tsn_info_buf[] range
 
DEPENDENCIES
  None

RETURN VALUE
  TRUE if TSN pointer is outside mcalwcdma_dec_tsn_info_buf[].
  FALSE if TSN pointer is within mcalwcdma_dec_tsn_info_buf[].

SIDE EFFECTS
  None
==========================================================================*/
extern boolean mcalwcdma_decdata_is_tsn_not_within_buf(l1_dec_hs_tsn_info_type *tsn_info_buf_ptr);
#endif

#if defined(FEATURE_WCDMA_DC_HSUPA) && defined(FEATURE_WCDMA_EVT6D_ENH)
/*===========================================================================
FUNCTION     EUL_DC_IS_DPCCH_GRT_THAN_EUL_PWR

DESCRIPTION  This function is an API exported to L2, which indicated if DPCCH
             power is greater that 2 times EUL power.

DEPENDENCIES This is called by L2 if 6D is configured and half TTI has expired. 

RETURN VALUE TRUE or FALSE.

SIDE EFFECTS 
===========================================================================*/
extern boolean eul_dc_is_dpcch_grt_than_eul_pwr(void);
#endif /* FEATURE_WCDMA_DC_HSUPA && FEATURE_WCDMA_EVT6D_ENH */

/*==========================================================================
FUNCTION: L1_EDCH_UPDATE_MAC_LOG_PACKETS_WRAPPER_FUNCTION

DESCRIPTION:
  This wrapper function is called by L1 whenever there is a seamless reconfig
  happening.
==========================================================================*/
void l1_edch_update_mac_log_packets_wrapper(sys_modem_as_id_e_type as_id);
...
SIDE EFFECTS
  None.
==========================================================================*/
extern uint32 eul_calc_frnum_or_subfn 
( 
  e_tti_enum_type     tti, 
  wsub_id_e_type  wsub_id
);
