...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/hscfg.h#4 $
...
02/05/16   ac     Update LNA switchpoints for RxD
...
06/15/15   ac     Removed confusing HS_PDSCH_MAX_VIR_IR_BUF_SZ macro
06/01/15   svh    Fix to not reframe QICE when only HS-DPCCH start happens.
02/27/15   ap     Initial check-in for WL1 QSH framework 
04/22/15   ac     Datapath W+W changes
03/19/15   ac     W + W datapath init cleanup
01/26/15   jhl    Global reorg for W+W
...
#define HS_CFG_MCPM_HSDPA_REQ    0x01
#define HS_CFG_MCPM_HSDPA_REL    0x02
#define HS_CFG_MCPM_MIMO_REQ     0x04
#define HS_CFG_MCPM_MIMO_REL     0x08
#define HS_CFG_MCPM_DC_HSDPA_REQ 0x10
#define HS_CFG_MCPM_DC_HSDPA_REL 0x20

#define HS_CFG_INFO_TABLE_UPD   0x0001
#define HS_CFG_DL_CFG_UPD       0x0002
#define HS_CFG_UL_CFG_UPD       0x0004
#define HS_CFG_MAC_HS_RESET     0x0008
#define HS_CFG_ONLY_DPCCH_START 0x0010

#define HS_CFG_ACTION_SET_PARAM(val) \
  (hscfg_int_cntrl_params.hs_cfg_action_bmsk |= (HS_CFG_##val))

#define HS_CFG_ACTION_GET_PARAM(val) \
  (hscfg_int_cntrl_params.hs_cfg_action_bmsk & (HS_CFG_##val))
...
/* Primary/diversity chain index for HS LNA switchpoints */
#define LNA_HS_PRX_IDX      0
#define LNA_HS_DRX_IDX      1
#define LNA_HS_MAX_CHAINS   2

/* default APS ON threshold. */
#define HSCFG_APS_ON_DEFAULT_THRESHOLD  (20)
/* default num TTI to collect. */
#define HSCFG_APS_ON_DEFAULT_NUM_TTI    (100)
...
typedef void HS_CHAN_EVENT_FUNC_TYPE(wsub_id_e_type wsub_id);

/* HS configuration done post call back function type */
typedef void HS_CHANNEL_CFG_DONE_POST_CB_FUNC_TYPE(boolean status, wsub_id_e_type wsub_id);

/* HS configuration done post call back function type */
typedef void WL1_DSR_HS_TRANS_IND_CB_FUNC_TYPE(boolean hs_active, wsub_id_e_type wsub_id);

/* Callback prototype of function that is called by CME to resume HS config */
typedef void HS_RESUME_CONFIG_CME_DONE_CB_FUNC_TYPE( wsub_id_e_type wsub_id);
...
typedef void HS_CME_STATE_CHANGE_CB_FUNC_TYPE(
  hs_cme_state_change_cb_struct_type *hs_cme_ptr, wsub_id_e_type wsub_id);
...
typedef struct
{
  uint8 hs_cur_ul_info_idx;
  /* Currently used DL/UL HSDPA config DB index */
  uint8 hs_cur_dl_info_idx;
  /* HS DL config DB index corresponding to HSDPA information
   tables indices */
  uint8 hs_dl_cfg_db_idx_for_info_table[HS_INFO_TABLE_COUNT];
  /* This variable indicate that HSDPA channel is active or not.
     At initialization it is reset to FALSE */
  boolean hs_channel_active;
}hscfg_ext_cntrl_params_mdmp_type;

typedef struct
{

  uint8 hs_cm_cfg_ul_info_idx;
  /* HSDPA DL/UL information table index to use in HS+CM subframe
   calculation when HSDPA config is pending */
  uint8 hs_cm_cfg_dl_info_idx;
  /* The last CFN for which mod/demod control tables are updated.
   This is uint16 because the valid range of CFN's would consume
   all uint8 leaving no space for INVALID CFN */
  uint16 hs_cm_last_cfn_analyzed;
  /* This is the CFN at which CQI override disable command is sent to MDSP */
  uint16 hs_disable_cqi_override_cfn;
  /* availability status of HSDPA information tables */
  boolean hs_info_table_avail_status[HS_INFO_TABLE_COUNT];
  /* Indicates that HS_CFG_LOG_PKT is pending to be sent at next HS_DECODE_STATUS_LOG_PKT */
  boolean hs_cfg_log_pkt_pending;
  /* This indicate that MAC HS config indication is sent to MAC one frame
   ...
   action */
  boolean hs_mac_cfg_indication;
  /* HSDPA ordered config saved for break in context */
  l1_hsdpa_info_struct_type *hs_saved_info;
  /* Mutex for all HS logging */
  rex_crit_sect_type hs_logging_crit_sec;
  /* Mutex for all HS configuration */
  rex_crit_sect_type hs_cfg_crit_sec;
  /* Structure to hold minidump variables. */
  hscfg_ext_cntrl_params_mdmp_type mdmp;
}hscfg_ext_cntrl_params_type;

typedef struct
{
  /* threshold SCCH rate to turn ON APS. */
  uint8 aps_on_threshold;
  /* TTIs of stats to collect. */
  uint16 tti_to_collect;
  /* number of SCCH attempted so far. */
  uint16 num_attempted;
  /* number of valid SCCH attempts so far. */
  uint16 num_valid;
  /* number of TTIs of stats collected so far. */
  uint8 num_tti;
} hs_aps_stats_struct_type;

extern hscfg_ext_cntrl_params_type hscfg_ext_cntrl_params;

#define HS_CFG_INTLOCK()  REX_ISR_LOCK(&hscfg_ext_cntrl_params.hs_cfg_crit_sec)
#define HS_CFG_INTFREE()  REX_ISR_UNLOCK(&hscfg_ext_cntrl_params.hs_cfg_crit_sec)
#define HS_LOG_INTLOCK()  REX_ISR_LOCK(&hscfg_ext_cntrl_params.hs_logging_crit_sec)
#define HS_LOG_INTFREE()  REX_ISR_UNLOCK(&hscfg_ext_cntrl_params.hs_logging_crit_sec)
...
extern void hs_get_ue_category_from_nv( wsub_id_e_type wsub_id);
...
extern void hs_harq_buff_flush_nv_read( wsub_id_e_type wsub_id);
...
extern void hs_register_cme_hs_state_change_cb(
  ...
  HS_CME_STATE_CHANGE_CB_FUNC_TYPE *hs_cme_cb, wsub_id_e_type wsub_id);

extern void hs_deregister_cme_hs_state_change_cb( wsub_id_e_type wsub_id);
...
extern boolean hs_is_hs_dpcch_configured( wsub_id_e_type wsub_id);
...
extern boolean hs_is_hs_dpcch_active( wsub_id_e_type wsub_id);
...
extern uint8 hs_get_ue_category( wsub_id_e_type wsub_id);
...
extern  uint8 hs_get_cell_db_idx(l1_hsdpa_carrier_enum_type carrier, wsub_id_e_type wsub_id);
...
extern uint16 hs_get_serv_cell_psc( wsub_id_e_type wsub_id);
...
extern uint16 hs_get_ordered_mc_psc(
  ...
  l1_hsdpa_carrier_enum_type carrier, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION hs_get_ordered_mc_freq
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint16 hs_get_ordered_mc_freq(
  /* CPHY_SETUP_REQ command pointer */
  l1_setup_cmd_type *setup,
  l1_hsdpa_carrier_enum_type carrier, wsub_id_e_type wsub_id);
#endif /* FEATURE_WCDMA_DC_HSDPA */
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean hs_get_64qam_state(l1_hsdpa_carrier_enum_type carrier, wsub_id_e_type wsub_id);
#endif
...
SIDE EFFECTS
  None
===========================================================================*/
extern hs_mac_ver_enum_type hs_get_mac_hs_or_ehs( wsub_id_e_type wsub_id);
#endif
...
===========================================================================*/
extern boolean hs_query_e_fach_state( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean hs_query_e_fach_state_for_db_idx(uint8 hs_dl_cfg_db_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean hs_query_dhrnti (uint16 *h_rnti, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint8 hs_get_intf_buf_idx( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint8 hs_get_num_harq_configured( wsub_id_e_type wsub_id);
...
extern uint16 hs_get_ordered_psc(
  /* CPHY_SETUP_REQ command pointer */
  l1_setup_cmd_type *setup, wsub_id_e_type wsub_id);
...
extern void hs_get_call_info(
  ...
  /* HS info index */
  uint8 info_table_idx, wsub_id_e_type wsub_id);
...
extern mcalwcdma_demod_tx_div_mode_enum_type hs_get_txd_state(l1_hsdpa_carrier_enum_type carrier, wsub_id_e_type wsub_id);

/*===========================================================================

FUNCTION hscfg_deinit_connected

DESCRIPTION
  This function initialize HS configuration and maintainance related
  variables when L1 leaves connected state
...
extern void hscfg_deinit_connected( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION hscfg_init_stack
...
extern void hscfg_init_connected( wsub_id_e_type wsub_id);
...
extern boolean hs_dsch_validate_config(
  ...
  l1_hsdpa_info_struct_type *hs_info, wsub_id_e_type wsub_id);
...
extern boolean hs_scch_validate_config(
  ...
  l1_dl_hs_scch_info_struct_type *hs_scch_info, wsub_id_e_type wsub_id);
...
extern boolean hs_dpcch_validate_config(
  ...
  l1_ul_hs_dpcch_info_struct_type *hs_dpcch_info, wsub_id_e_type wsub_id);
...
extern boolean hs_cell_trans_cmd_validate_config(
  ...
  l1_setup_cmd_type *cell_trans_cmd, wsub_id_e_type wsub_id);
#endif /* FEATURE_WCDMA_HS_FACH */
...
extern void hs_calulate_scch_part1_scrambler(
  ...
  uint32* scch_part1_scrambler_array_ptr, wsub_id_e_type wsub_id);
...
extern void hs_update_dl_channel_cfg_db(
  ...
  l1_hsdpa_info_struct_type *hs_info,
  /* HSDPA information table index */
  uint8 hs_info_table_idx[L1_HSDPA_MAX_CARRIERS], wsub_id_e_type wsub_id);
...
extern void hs_set_suspended_state(boolean state, wsub_id_e_type wsub_id);
...
extern void hsdpa_process_config_cmd(
  ...
  HS_CHANNEL_CFG_DONE_POST_CB_FUNC_TYPE *cfg_done_cb_func, wsub_id_e_type wsub_id);
...
extern void hs_generate_serv_cell_change_evt(
  /* HSDPA configuration operation type */
  hsdpa_setup_ops_enum_type hs_ops, wsub_id_e_type wsub_id);
...
extern void hs_verify_combiner_cfg_cmd_window( wsub_id_e_type wsub_id);
...
extern void hs_dl_comb_chan_evt_unsuspend_cfg_cmd( wsub_id_e_type wsub_id);
...
extern void hs_flush_hs_info_mac_hs_reset( wsub_id_e_type wsub_id);
...
extern void hs_dl_switch_config( wsub_id_e_type wsub_id);
...
extern void hs_enable_dl_comb_chan_event_handler(
  ...
  /* event firing tick */
  uint8 firing_time, wsub_id_e_type wsub_id);
...
extern void hs_dl_comb_chan_event_handler( wsub_id_e_type wsub_id);
...
extern void hs_verify_ul_channel_cfg_cmd_window( wsub_id_e_type wsub_id);
...
extern void hs_ul_chan_evt_unsuspend_cfg_cmd( wsub_id_e_type wsub_id);
...
extern void hs_ul_switch_config( wsub_id_e_type wsub_id);
...
extern void hs_ul_switch_beta_table( wsub_id_e_type wsub_id);
...
extern void hs_ul_chan_event_handler( wsub_id_e_type wsub_id);
...
extern void hs_update_chan_active_state(
  ...
  hs_channel_cfg_mdsp_action_enum_type action_type, wsub_id_e_type wsub_id);
...
extern void hs_chan_cfg_done_send_cmd(boolean status, wsub_id_e_type wsub_id);
...
extern void hscfg_clk_regime_deregister_client( wsub_id_e_type wsub_id);
/*===========================================================================
...
extern void hs_handle_hs_cfg_log_pkt( wsub_id_e_type wsub_id);
...
extern void hs_enable_finger_update_handler(
  ...
  boolean is_assigned, wsub_id_e_type wsub_id);
...
extern hs_dl_channel_cfg_db_struct_type* hs_get_cur_dl_cfg_ptr( wsub_id_e_type wsub_id);
extern hs_ul_channel_cfg_db_struct_type* hs_get_cur_ul_cfg_ptr( wsub_id_e_type wsub_id);
...
extern void hs_initiate_dc_drop_operation(
    ...
    HS_CHANNEL_CFG_DONE_POST_CB_FUNC_TYPE *post_dc_cfg_hs_cb, wsub_id_e_type wsub_id);
#endif /* FEATURE_WCDMA_DC_HSDPA */
...
extern void hs_send_submit_log_cmd( wsub_id_e_type wsub_id);
...
extern void hs_do_log_submit( wsub_id_e_type wsub_id);
...
extern void hs_read_commit_pending_log_pkts( wsub_id_e_type wsub_id);
...
extern uint8 hs_query_hs_cell_idx_for_psc(
  ...
  uint16 pri_scr_code, wsub_id_e_type wsub_id);
...
extern void hs_cm_init( wsub_id_e_type wsub_id);
...
extern boolean hs_is_hs_config_pending( wsub_id_e_type wsub_id);
...
extern void hs_update_cm_ctrl_context(
  ...
  hs_cm_ctrl_context_enum_type context, wsub_id_e_type wsub_id);
...
extern void hs_set_prev_cur_cm_params(uint8 prev_cfn, wsub_id_e_type wsub_id);
...
extern void hs_get_start_end_cm_gaps(
  ...
  hs_cm_param_struct_type* frame, wsub_id_e_type wsub_id);
...
extern void hs_stop_dl_during_cm_gaps(
  ...
  uint16 hs_pdsch_dl_start_global_sub_fr_num, wsub_id_e_type wsub_id);
...
extern void hs_stop_ul_during_cm_gaps(
  ...
  uint16 hs_pdsch_ul_start_global_sub_fr_num, wsub_id_e_type wsub_id);
...
extern void hs_stop_hs_during_cm_gaps(
  ...
  uint8 target_cfn, wsub_id_e_type wsub_id);
...
extern void hs_cm_dl_update_logging( wsub_id_e_type wsub_id);
...
extern void hs_cm_dl_send_log_submit_cmd(
  ...
  uint8 buffer_idx, wsub_id_e_type wsub_id);
...
extern void hs_cm_demod_ctrl_submit_log_buffer( wsub_id_e_type wsub_id);
...
extern void hs_cm_ul_update_logging( wsub_id_e_type wsub_id);
...
SIDE EFFECTS None
===========================================================================*/
extern uint8 hs_get_dpch_offset( wsub_id_e_type wsub_id);
...
extern uint8 hs_get_cctrch_id_for_cfg_db_idx(uint8 hs_cfg_db_idx, wsub_id_e_type wsub_id);
...
extern hs_mac_ver_enum_type hs_get_mac_hs_or_ehs_for_cfg_db_idx(uint8 hs_cfg_db_idx, wsub_id_e_type wsub_id);
...
extern uint8 hs_get_q_id_mapping_for_lc_id(uint8 lc_id, uint8 hs_cfg_db_idx, wsub_id_e_type wsub_id);
...
extern uint8 hs_get_efach_h_rnti_bmsk_for_cfg_db_idx(uint8 hs_cfg_db_idx, wsub_id_e_type wsub_id);
#endif /* FEATURE_WCDMA_HS_FACH */
...
SIDE EFFECTS
  None
===========================================================================*/
extern void hs_chan_deactivate_done(boolean status, wsub_id_e_type wsub_id);
...
SIDE EFFECTS None
===========================================================================*/
extern uint8 hscfg_get_hs_rl_tto( wsub_id_e_type wsub_id);
...
SIDE EFFECTS None
===========================================================================*/
extern void hscfg_register_dsr_callback(WL1_DSR_HS_TRANS_IND_CB_FUNC_TYPE *dsr_cb, wsub_id_e_type wsub_id);
/*===========================================================================
...
SIDE EFFECTS None
===========================================================================*/
extern void hscfg_deregister_dsr_callback( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void hs_update_dsr_cb(boolean hs_enabled, wsub_id_e_type wsub_id);
...
extern void hscfg_get_txdiv_for_hs_rl(
  ...
  mcalwcdma_demod_tx_div_mode_enum_type *sec_carrier_txdiv, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void hs_fast_hs_dpcch_setup ( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean hs_channel_implicit_drop_indication (l1_setup_cmd_type *cmd, wsub_id_e_type wsub_id);
...
extern void hs_submit_stop_events_to_dm( wsub_id_e_type wsub_id);
...
extern void hs_submit_start_recfg_events_to_dm( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint16 hs_get_start_gsfn_for_cfn (uint8 target_cfn, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean hs_get_activation_status(l1_hsdpa_carrier_enum_type carrier_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void hs_update_activation_status_in_cur_db(uint8 carrier_idx, boolean activation_status_value, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint8 hs_get_category_for_mcpm( wsub_id_e_type wsub_id);
...
extern void hs_cm_ul_send_log_submit_cmd(uint8 buffer_idx, wsub_id_e_type wsub_id);
...
extern hs_ul_channel_cfg_db_struct_type const* hs_get_ul_channel_cfg_db( wsub_id_e_type wsub_id);
/*===========================================================================
...
extern hs_dl_channel_cfg_db_struct_type const* hs_get_dl_channel_cfg_db( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void wl1_hs_debug_dump(wl1_qsh_datapath_info_struct_type *dump_ptr);
#endif /* FEATURE_WCDMA_QSH_DUMP*/

 #ifdef FEATURE_WCDMA_EFACH_MSIM
/*===========================================================================
FUNCTION hs_get_start_cfn_for_gsfn

DESCRIPTION
  This function return the CFN for a given gSFN
...
SIDE EFFECTS
  None
===========================================================================*/
uint8 hs_get_start_cfn_for_gsfn (uint16 target_gsfn, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void hs_cm_info_upd(wsub_id_e_type wsub_id);
#endif

#ifdef FEATURE_WCDMA_DL_QUAL_MEAS_REPORTING
/*===========================================================================
FUNCTION hs_get_current_cell_metrics

DESCRIPTION
  This function gets the cell Ec/Io, Nt/Io

DEPENDENCIES
  None

RETURN VALUE
  TRUE if Ec/Io, Nt/Io are valid
  FALE if results are invalid

SIDE EFFECTS
  None
===========================================================================*/
boolean hs_get_current_cell_metrics(l1_hsdpa_carrier_enum_type carrier, uint16 *ecio_out, int32 *ntio_out, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION hs_get_current_mpo

DESCRIPTION
  This function returns the current MPO 

DEPENDENCIES
  None

RETURN VALUE
  uint16 MPO in Q10 format, 0xFFFF if HS is not enabled

SIDE EFFECTS
  None
===========================================================================*/
uint16 hs_get_current_mpo(l1_hsdpa_carrier_enum_type carrier, wsub_id_e_type wsub_id);
#endif /* FEATURE_WCDMA_DL_QUAL_MEAS_REPORTING */

/*===========================================================================
FUNCTION hs_update_aps_info_cmd_handler

DESCRIPTION
 This function collects SCCH stats which is then used to update APS.
...
SIDE EFFECTS
  None
===========================================================================*/
extern void hs_update_aps_info_cmd_handler(wl1_hs_dl_update_aps_type *cmd_ptr, wsub_id_e_type wsub_id);
/*===========================================================================
FUNCTION hs_update_rf_lna_switch_tech

DESCRIPTION
 This function updates the HS/R99 switchpoints for RF LNA states
...
SIDE EFFECTS
  None
===========================================================================*/
extern void hs_update_rf_lna_switch_tech(rfcom_tech_type tech, boolean div_only, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION hs_init_scch_stats
...
SIDE EFFECTS
  None
===========================================================================*/
extern void hs_collect_scch_stats(uint8 num_carr, boolean *scch_valid, boolean *scch_attempted, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION hs_get_num_ch_codes_on_given_carrier

DESCRIPTION
 This function retrieves the number of HS-SCCH channelization codes allocated on a given carrier

INPUT
  carr_idx: Carrier for which the number of channelization codes need to be known

DEPENDENCIES
  Modules calling this API need to ensure valid carrier index is being passed and HS is active when this API is queried

RETURN VALUE
  uint8: Number of HS-SCCH channelization codes alllocated on the given carrier
  
SIDE EFFECTS
  None
===========================================================================*/
extern uint8 hs_get_num_hs_scch_ch_codes(uint8 carr_idx, wsub_id_e_type wsub_id);

#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
FUNCTION  wl1_qsh_mdump_collect_hs

DESCRIPTION
  Collect HS variables.
...
SIDE EFFECTS
  None
===========================================================================*/
extern QSH_MDUMP_FN_ATTR void wl1_qsh_mdump_collect_hs(wsub_id_e_type wsub_id);
#endif
