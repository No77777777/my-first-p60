...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/rxdiv_v.h#3 $
...
09/04/18    br      Change rxd request duration from 0  to 1 sclk when RRC unlock is pending 
08/16/17    ss     Changes for LTA timeline optimization
06/07/16    rsr     Added Changes for RxD in Inter SIB-Sleep.
04/10/16    svh     Add handling to delay RxD state update if an important search is to
                    be performed.
01/20/16    npn     Store RxD events at setup and deallocate all active events at cleanup.
01/29/15    kcm     Clear the RxD_ACQ_BCH_FAIL cfg mask at acq bch cleanup
12/03/15    svh     Change request time for diversity in idle mode.
10/28/15    svh     Call RxD API to do enter mode in BCH_SLEEP if tune is pending at wakeup. 
10/23/15    svh     Handle WRM grant for RxD immediately.
10/01/15    kcm     Fixes/cleanup changes for RxD ACQ BCH handling in BCH inter
                    sib sleep scenarios.
10/02/15    kcm     Modified CQI on/off adjustments during DTA/QDTA gaps
09/25/15    kcm     RxD ACQ BCH handling in BCH inter sib sleep scenarios
09/22/15    sks     Reserve for RxD with the same TA type as PRx's.
08/17/15    pv      Make the diversity lock duration as 36ms for RXD IDLE.
08/24/15    us      Updates for including rfm_path_type
08/17/15    svh     Ensure correct secondary carrier status is updated at RxD 
                    in case of DBDC.
08/07/15    rsr     Added change to Ensure that we bring RF out of LPM for 
                    EDRX prior to State Transition.
07/23/15    br      Resolve Kw errors
07/17/15    pvr     QDTA changes
06/11/15    sad     RF state machine changes for handling meas device
06/01/15    rsr     Added changes to ensure that Qual trigger is modified 
...
02/20/14    sm      Add debug array to keep track of RxD reason bmsk
02/20/15    qj      Fix FET pre procssing operation prior to Rxd transition
01/28/15    sm      Advanced Rcvr W2W Global Reorg
01/13/15    gm      Mods to reduce min req time to 28ms in non conn states.
...
/* Trigger that is set when RxD needs to be enabled in ACQ/BCH/BCH_SLEEP states. */
#define RXD_ACQ_BCH_FON_TRIGGER_BMSK 0x20000

/* Trigger that is set when RxD needs to be disabled in ACQ/BCH/BCH_SLEEP states. */
#define RXD_ACQ_BCH_FOFF_TRIGGER_BMSK 0x40000
...
#define RXD_TRIG(str) \
  ((RXD_TRIG_BMSK(str) & rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_trig) ? TRUE:FALSE)

/* Macro for setting a particular trigger, only need to
    supply the relevant string as an argument */
#define RXD_SET_TRIG(str) \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_trig |= RXD_TRIG_BMSK(str))

/* Defintion for setting two triggers simultaneously,
    only need to supply relevant strings */
#define RXD_SET_TRIG2(str1, str2) \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_trig |= (RXD_TRIG_BMSK(str1) | RXD_TRIG_BMSK(str2)))
...
#define RXD_CLEAR_TRIG(str) \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_trig &= (uint32)((uint32)~(RXD_TRIG_BMSK(str))))
...
#define RXD_CLEAR_TRIG2(str1, str2) \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_trig &= (uint32)((uint32)~(RXD_TRIG_BMSK(str1)|RXD_TRIG_BMSK(str2))))

#define RXD_CLEAR_ACQ_BCH_FAIL_TRIGS() \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_trig &= (uint32)((uint32)~(RXD_TRIG_BMSK(ACQ_BCH_FOFF)|RXD_TRIG_BMSK(ACQ_BCH_FON))))

#define RXD_CLEAR_RDDS_TRIGS() \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_trig &= (uint32) ((uint32)~(RXD_TRIG_BMSK(QUAL)|RXD_TRIG_BMSK(CAP)|RXD_TRIG_BMSK(RSCP))))

#define RXD_CLEAR_HS_TRIGS() \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_trig &= (uint32) ((uint32)~(RXD_TRIG_BMSK(ARD)|RXD_TRIG_BMSK(RXDPM)| \
                                   RXD_TRIG_BMSK(ARD_V2)|RXD_TRIG_BMSK(RXDPM_V2)|RXD_TRIG_BMSK(STATIC_RXD))))

#define RXD_CLEAR_PS_TRIGS() \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_trig &= (uint32) ((uint32)~(RXD_TRIG_BMSK(PS_SF))))

/* Check on whether a particular type of trigger is set as reason for state or not */
#define RXD_STATE_RSN(str) \
  ((RXD_TRIG_BMSK(str) & rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_state_reason_bmsk) ? TRUE:FALSE)

/* Macro for setting a particular trigger as reason for state, only need to
    supply the relevant string as an argument */
#define RXD_SET_STATE_RSN(str) \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_state_reason_bmsk |= RXD_TRIG_BMSK(str))

/* Macro for clearing up a particular trigger from the complete mask.
     Only need to supply the relevant string as an argument to the macro */
#define RXD_CLEAR_STATE_RSN(str) \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_state_reason_bmsk &= (uint32)((uint32)~(RXD_TRIG_BMSK(str))))

/* Macro for clearing the reason for state mask */
#define RXD_CLEAR_ALL_STATE_RSN() (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_state_reason_bmsk = 0x00)

#define RXD_CLEAR_RDDS_STATE_RSN() \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_state_reason_bmsk &= (uint32) ((uint32)~(RXD_TRIG_BMSK(QUAL)|RXD_TRIG_BMSK(CAP)|RXD_TRIG_BMSK(RSCP))))

#define RXD_CLEAR_HS_STATE_RSN() \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_state_reason_bmsk &= (uint32) ((uint32)~(RXD_TRIG_BMSK(ARD)|RXD_TRIG_BMSK(RXDPM)| \
                                                RXD_TRIG_BMSK(ARD_V2)|RXD_TRIG_BMSK(RXDPM_V2)|RXD_TRIG_BMSK(STATIC_RXD))))

#define RXD_CLEAR_PS_STATE_RSN() \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_state_reason_bmsk &= (uint32) ((uint32)~(RXD_TRIG_BMSK(PS_SF))))
...
#define RXD_CFG(str) \
  ((RXD_CFG_BMSK(str) & rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_cfg) ? TRUE:FALSE)
...
#define RXD_SET_CFG(str) \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_cfg |= RXD_CFG_BMSK(str))
...
#define RXD_SET_CFG2(str1, str2) \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_cfg |= (RXD_CFG_BMSK(str1) | RXD_CFG_BMSK(str2)))
...
#define RXD_CLEAR_CFG(str) \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_cfg &= (uint16)((uint16)~(RXD_CFG_BMSK(str))))

/* Macro for clearing all the config masks */
#define RXD_CLEAR_ALL_CFG() (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_cfg = 0x00)
...
#define RXD_OPS(str) \
  ((RXD_OPS_BMSK(str) & rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_ops) ? TRUE:FALSE)
...
#define RXD_SET_OPS(str) \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_ops |= RXD_OPS_BMSK(str))
...
#define RXD_SET_OPS2(str1, str2) \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_ops |= (RXD_OPS_BMSK(str1) | RXD_OPS_BMSK(str2)))
...
#define RXD_CLEAR_OPS(str) \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_ops &= (uint16)((uint16)~(RXD_OPS_BMSK(str))))
...
#define RXD_STATE_TRANSITION(str1, str2) \
  rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_pend_state = str1; rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_cmd_proc_mode = str2

/* Generating the action mask based on ops type's bit mask */
#define RXD_ACT(str) \
  ((RXD_OPS_BMSK(str) & rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_act) ? TRUE:FALSE)

/* Macro for clearing up a particular ops type from the complete action mask.
    Only need to supply the relevant string (viz EQ, PCTL QUAL, CAP, TMR,
    TRI, SRCH, AGC, RF) as an argument to the macro */
#define RXD_CLEAR_ACT(str) \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_act &= (uint16)((uint16)~(RXD_OPS_BMSK(str))))
...
#define RXD_ACT_MASK(str) \
   (((0x001<<str) & rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_act) ? TRUE:FALSE)

/* Checking macro for whether al the action ops are cleared or not */
#define RXD_ALL_ACT_CLEAR() \
  (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_act == 0x000)
...
#define WL1_RXD_PROC_CLIENT_SET_ALL(void) \
  rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_proc_client_pending_bmsk = WL1_RXD_PROC_CLIENT_ALL

/* Sets mask for a single client */
#define WL1_RXD_PROC_CLIENT_SET(_client) \
  rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_proc_client_pending_bmsk |= _client
...
#define WL1_RXD_CLIENT_CHECK(_client) \
  ((_client & rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_proc_client_pending_bmsk) ? TRUE:FALSE)

/* Sets mask for a single FOFF client */
#define WL1_RXD_FOFF_CLIENT_SET(_client) \
  rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_forced_state_db.rxd_div_chain_foff_client_mask |= (uint16)(1 << (uint16)_client)
...
#define WL1_RXD_FOFF_CLIENT_GET(_client) \
  (((uint16)(1 << ((uint16)_client))) & rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_forced_state_db.rxd_div_chain_foff_client_mask)

/* Checks mask for a FOFF client */
#define WL1_RXD_FOFF_CLIENT(_client) \
  ((((uint16)(1 << ((uint16)_client))) & rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_forced_state_db.rxd_div_chain_foff_client_mask) ? TRUE:FALSE)

/* Sets mask for rxd_pre_proc_bmsk*/
#define RXD_PRE_PROC_SET(_client) \
  rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_pre_proc_bmsk |= (uint8)(1 << (uint8)_client)
...
#define RXD_PRE_PROC_CHECK(_client) \
  ((((uint8)(1 << ((uint8)_client))) & rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_pre_proc_bmsk) ? TRUE:FALSE)

/* Macro for checking whether a particular mask for an NV item is set or not */
#define RXD_NV(str) ((RXD_NV_##str & rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_nv_db.rxd_nv_ctrl) ? TRUE:FALSE)

/* Macro to set a bit corresponding to an allocated eventt */
#define WL1_RXD_ACTIVE_EVT_SET(evt_id) \
  rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_dynamic_db.active_evt_bmsk |= (uint16)(1 << (uint16)evt_id)

/* Macro to clear a bit corresponding to an allocated event */
#define WL1_RXD_ACTIVE_EVT_CLR(evt_id) \
  rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_dynamic_db.active_evt_bmsk &= (~((uint16)(1 << ((uint16)evt_id))))
...
#define RXD_SEC_CARR_IS_DEACT() \
   (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_sec_carr_deactivated)

   /* activate the second carrier */
#define RXD_SEC_CARR_SET_ACTIVE() \
   (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_sec_carr_deactivated = FALSE)

   /* deactivate the second carrier */
#define RXD_SEC_CARR_SET_DEACT() \
   (rxdiv_int_cntrl_params[wsub_id].mdmp.rxd_state_db.rxd_sec_carr_deactivated = TRUE)
...
/* Minimum time for an RxD WRM Request in SCLK time set for 33ms*/
#define RXD_MIN_REQUEST_TIME_IN_SCLK   ((36 * TIMETICK_NOMINAL_FREQ_HZ)/1000)

/* Time to request to get imeediate grant after QTA or when LTA unlock is pending */
#define RXD_REQUEST_TIME_IN_1_SCLK  1

/* Minimum time for an RxD WRM Request for PCH and ACQ in SCLK time set for 20 ms.
   Changing this to 20ms on TA because TA is SR target. 
...
#define RXD_AGC_DBMTODB10(x) ((106 + x) * 10 - 512)
#define RXD_AGC_DB10TODBM(x) ((x + 512) / 10 - 106)
...
typedef void RXD_HS_RXD_TRANS_IND_CB_FUNC_TYPE(boolean transition, wsub_id_e_type wsub_id);

typedef void RXD_CME_TRANS_CB_FUNC_TYPE(boolean rxd_state,wsub_id_e_type wsub_id);

/* RxD state call back function type */
typedef void RXD_DSR_TRANS_IND_CB_FUNC_TYPE(boolean rxd_state, wsub_id_e_type wsub_id);
...
typedef enum
{
  ...
  /*RXD_SUSPENDED: (Dual Rx chain) RxD is suspended
     for QDTA , Search and Triage must be called to re-enable RxD */
  RXD_SUSPENDED,
...
typedef enum
{
  ...
  RXD_QDTA_STATUS_PRE_GAP,
  RXD_QDTA_STATUS_TO_GAP,
  RXD_QDTA_STATUS_IN_GAP,
  RXD_QDTA_STATUS_END_GAP
...
/* List of CFG that external modules can modify */
typedef enum
{
  L1_DCH,
  DCH_INIT,
...
typedef struct
{
  ...
  */
  boolean wl1_rxd_pch_oride_val_force;
  uint8 wl1_rxd_pch_oride_val;
  uint8 wl1_rxd_pch_oride_val_force_value;
  /* enables PCH debug f3s */
  boolean wl1_rxd_pch_debug_f3;
...
typedef struct{
  ...
  boolean demfront_type;
  uint8  start_cfn;
  uint8  cfn_gap;
}wl1_rxd_qta_struct_type;

/* struct to hold unlock request return value */
typedef struct{
  boolean qdta;
  boolean wrm_cancel_in_progress;
}wl1_rxd_unlock_return_type;
...
typedef struct{
  rex_crit_sect_type rxd_srch_proc_mutex;
}rxdiv_ext_cntrl_params_type;

extern rxdiv_ext_cntrl_params_type rxdiv_ext_cntrl_params[WCDMA_NUM_SUBS];
...
None
===========================================================================*/
extern void wl1_rxd_handle_olpc_qual_meas_update(uint8 num_tbs, uint8 num_err, wsub_id_e_type wsub_id);
...
None
===========================================================================*/
extern void rxd_div_chain_clk_turn_on( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_div_chain_clk_turn_off( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_div_ant_lockmode_parameters_init( wsub_id_e_type wsub_id);
...
extern void rxd_init( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_rxd_pre_proc_rxd_on_trans_ops( wsub_id_e_type wsub_id);
...
boolean rxd_register_cme_trans_cb(
  ...
  RXD_CME_TRANS_CB_FUNC_TYPE *cm_cme_cb, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void rxd_deregister_cme_trans_cb( wsub_id_e_type wsub_id);
...
SIDE EFFECTS 
  None
===========================================================================*/
extern void rxd_register_dsr_callback(RXD_DSR_TRANS_IND_CB_FUNC_TYPE *dsr_cb, wsub_id_e_type wsub_id);
/*===========================================================================
...
SIDE EFFECTS 
  None
===========================================================================*/
extern void rxd_deregister_dsr_callback( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_rxd_pch_init( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_rxd_pch_cleanup( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_rxd_pch_start( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern rxd_pch_mode_enum_type wl1_rxd_pch_wakeup_state_update( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_rxd_pch_teardown_diversity( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern rxd_pch_mode_enum_type wl1_rxd_pch_calculate_next_mode(uint8 pi_val, uint16 filt_ecio, boolean no_resp_to_page, boolean s_crit_check, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern int32 wl1_rxd_pch_get_pisum_thresh( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_rxd_pch_update_lockmode(rxd_pch_imbalance_check_enum_type imbalance_check_type, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_rxd_pch_rxd_out_of_lock( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_wakeup_div_chain
         
DESCRIPTION
  This function is to be called when UE wakes up during BCH ISS or for PICH, after enabling 
  primary clocks.
  In PCH:
  If RxD is to be enabled for this wake cycle. If tune was 
  not done previously, this function makes sure Div chain is enabled and tuned.
  If RxD mode is Fast Diversity, this function initiates the SSBI write to put 
  Div chain in low power mode.
  In BCH:
  This API will turn on the clocks for Diversity wakeup.
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_rxd_wakeup_div_chain(boolean enable_rf, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_notify_new_sleep_cycle

DESCRIPTION
This function is called by wl1sleep when ue goes to sleep in SLEEP state 
after calculating next wakeup time. Rxd gets sleep_interval_written_sc and 
reserves Diversity chain at wakeup if next wakeup is with Diversity.
...
None
===========================================================================*/
extern void wl1_rxd_notify_new_sleep_cycle(trm_tuneaway_enum_type tuneaway_reason_for_reserve, wsub_id_e_type wsub_id);
...
extern void wl1_rxd_local_command_trace_func(uint8 l1_state, 
                  wl1_rxd_debug_enum_type module_posted, uint16 curr_cfn, uint16 client, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_rxd_pch_full_div( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean rxd_is_active( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_swap_rx_chains_at_entry_of_traffic_state( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_save_rxd_ctrl_from_nv( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_idle_init( wsub_id_e_type wsub_id);
...
extern void rxd_state_update( wsub_id_e_type wsub_id);
...
extern boolean rxd_config_update( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_set_signalling_trigger( wsub_id_e_type wsub_id);
...
extern void rxd_set_forced_off(rxd_div_chain_resource_client_type client_id, boolean foff, rxd_foff_cb_type cb_func, boolean blind_foff, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_bring_rf_out_of_lpm_in_cpc_e_drx
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_rxd_bring_rf_out_of_lpm_in_cpc_e_drx( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean rxd_set_for_asd(boolean rxd_on, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean rxd_get_rxd_for_as_div( wsub_id_e_type wsub_id);
/*===========================================================================
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint8 rxd_get_ctrl_nv_value( wsub_id_e_type wsub_id);
#endif
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_local_cb_cmd( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_cap_meas_config_update(l1_sf_enum_type sf_type, uint8 n_pilot, uint8 num_rl, wsub_id_e_type wsub_id);
...
extern void rxd_cap_meas_update(uint32 g, uint32 h, wsub_id_e_type wsub_id);
...
extern void rxd_qual_meas_update(uint8 num_tbs, uint8 num_pass, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_rxdpm_meas_update(boolean* scch_valid, boolean* scch_demod_attempted, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_enable( wsub_id_e_type wsub_id);
...
extern void rxd_disable( wsub_id_e_type wsub_id);
...
void rxd_meas_logging_update( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void rxd_submit_meas_log_pkt( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_submit_ard_log( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_submit_ard_v2_log( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void rxd_hho_save_imbalance_info( wsub_id_e_type wsub_id);
...
SIDE EFFECTS    
===========================================================================*/
extern void rxd_state_update_event_handler( wsub_id_e_type wsub_id);
...
SIDE EFFECTS    
===========================================================================*/
extern void rxd_evaluate_state_update_event( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_configure(uint8 call_type_mask, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void rxd_fach_cleanup( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_dch_cleanup( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void rxd_deallocate_rxlm_buffer( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_manage_rxlm_buffer(boolean allocate, wsub_id_e_type wsub_id);
...
extern void rxd_register_hs_callback(
  /* Callback function pointer */
  RXD_HS_RXD_TRANS_IND_CB_FUNC_TYPE *hs_cb, wsub_id_e_type wsub_id);
...
extern void rxd_deregister_hs_callback( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean rxd_is_off( wsub_id_e_type wsub_id);
...
extern boolean wl1_rxd_api_oride_cmd(uint8 api_identifier, uint32 input_arg_1, uint32 input_arg_2, 
  uint32 input_arg_3, uint32 input_arg_4, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_register_cm_cb( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_rxd_fach_hs_rach_status(boolean status, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_dsds_status_update(boolean dsds_state, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint32 wl1_rxd_get_rxd_reason_bitmask( wsub_id_e_type wsub_id);
...
None
===========================================================================*/
extern uint8 wl1_rxd_get_rxdpm_reason_bitmask( wsub_id_e_type wsub_id);
...
None
===========================================================================*/
extern uint8 wl1_rxd_get_ard_reason_bitmask( wsub_id_e_type wsub_id);
...
None
===========================================================================*/
extern void rxd_client_update_done_srch(boolean srch_return, wsub_id_e_type wsub_id);
...
None
===========================================================================*/
extern void rxd_client_update_done_tri(boolean tri_return, wsub_id_e_type wsub_id);
...
None
===========================================================================*/
extern boolean wl1_rxd_skip_short_cycle_cpc_drx( wsub_id_e_type wsub_id);
...
None
===========================================================================*/
extern boolean wl1_rxd_skip_cpc_drx( wsub_id_e_type wsub_id);
...
None
===========================================================================*/
extern boolean wl1_rxd_supported( wsub_id_e_type wsub_id);
...
None
===========================================================================*/
void wl1_rxd_ctrl_cmd_handler(wl1_rxd_ctrl_cmd_type *cmd_ptr, wsub_id_e_type wsub_id);
/*===========================================================================
...
None
===========================================================================*/
extern void wl1_rxd_qta_state_update(boolean in_gap, wsub_id_e_type wsub_id);
...
None
===========================================================================*/
void wl1_rxd_timer_set(wl1_rxd_tmr_rsn_enum_type state_enum, wsub_id_e_type wsub_id);
...
None
===========================================================================*/
extern void wl1_set_pch_oride_val(uint8 set, wsub_id_e_type wsub_id);
...
None
=============================================================================*/
extern void rxd_release_div_chain(boolean freeze_filters, rxd_div_chain_resource_client_type client_mask, wsub_id_e_type wsub_id);
...
None
===========================================================================*/
void wl1_rxd_dsda_omrd_enable(boolean dsda_enabled, wsub_id_e_type wsub_id);
...
None
===========================================================================*/
void wl1_rxd_dsds_qta_enable(boolean dsds_enabled, wsub_id_e_type wsub_id);
...
None
===========================================================================*/
extern boolean wl1_rxd_tune_pending( wsub_id_e_type wsub_id);
...
None
===========================================================================*/
extern boolean wl1_rxd_is_omrd_disabled_for_dsda( wsub_id_e_type wsub_id);
...
None
===========================================================================*/
boolean wl1_rxd_is_omrd_freeze_flag_set( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_rxd_acq_bch_failure( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_request_rxd_for_lta
         
DESCRIPTION
  API called in ACQ to request RxD in case of LTA.
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_request_rxd_for_lta(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_active_for_lta
         
DESCRIPTION
  API called ACQ to check if RxD is supposed to turn ON.
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_rxd_active_for_lta(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_combined_tune_allowed
         
DESCRIPTION
  API called in ACQ to check if combined tune is allowed.
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_rxd_combined_tune_allowed(wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_acq_bch_cleanup( wsub_id_e_type wsub_id);
...
None
===========================================================================*/
extern boolean wl1_rxd_in_div_only_mode( wsub_id_e_type wsub_id);
...
PARAMETERS
  wrm_secondary_unlock_cb_data* unlock_data
...
SIDE EFFECTS
  None.
===========================================================================*/
extern boolean wl1_rxd_unlock_callback(wrm_secondary_unlock_cb_data* unlock_data, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void wl1_rxd_grant_callback(boolean grant, wsub_id_e_type wsub_id);
...
None
===========================================================================*/
void wl1_rxd_wait_for_agc_tune_done( wsub_id_e_type wsub_id);
...
None
===========================================================================*/
void wl1_rxd_wait_for_agc_cfg_done( wsub_id_e_type wsub_id);
...
None
===========================================================================*/
void wl1_rxd_handle_mdsp_agc_done( wsub_id_e_type wsub_id);
...
/*===========================================================================
FUNCTION wl1_rxd_attempt_div_at_wakeup
...
SIDE EFFECTS
  None
===========================================================================*/
boolean wl1_rxd_attempt_div_at_wakeup( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_record_buffer_deallocation(uint8 client, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_record_buffer_allocation(uint8 client, wsub_id_e_type wsub_id);
/*===========================================================================
...
None
===========================================================================*/
extern boolean rxd_rf_is_asleep( wsub_id_e_type wsub_id);
...
None
===========================================================================*/
extern boolean wl1_rxd_skip_wrm_request_resources_for_secondary(wsub_id_e_type wsub_id);
/*===========================================================================
FUNCTION rxdiv_int_cntrl_params_init

DESCRIPTION
  This function define local variables for RxD
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxdiv_int_cntrl_params_init( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_get_freeze_flag

DESCRIPTION
  Returns if the flag to freeze dynamic filters is set
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_rxd_get_freeze_flag( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_get_pch_f3_enabled

DESCRIPTION
  Returns a boolean indicating if extra debug f3s should be enabled
  for PCH RXD
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_rxd_get_pch_f3_enabled( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_get_div_ant_id

DESCRIPTION
Gets the antenna id of the diversity rf chain
DEPENDENCIES
  None
...
SIDE EFFECTS
  None
===========================================================================*/
extern rxd_div_ant_lock_mode_enum_type wl1_rxd_get_div_ant_id( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_get_prim_ant_id

DESCRIPTION
  Gets the antenna id of the primary RF chain
...
SIDE EFFECTS
  None
===========================================================================*/
extern rxd_div_ant_lock_mode_enum_type wl1_rxd_get_prim_ant_id( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_get_debug_do_not_request_trm

DESCRIPTION
  Debug flag set by QXDM that does not allow wrm requests if
  The unlock request has been forcefully send by way of a QXDM command
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_rxd_get_debug_do_not_request_trm( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_get_div_ant_lock_mode

DESCRIPTION
  Returns if the diversity RF chain is in lock
...
SIDE EFFECTS
  None
===========================================================================*/
extern rxd_div_ant_lock_mode_enum_type wl1_rxd_get_div_ant_lock_mode( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_set_div_ant_lock_mode

DESCRIPTION
  Allows updating of the Diversity Antenna lock mode
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_rxd_set_div_ant_lock_mode(rxd_div_ant_lock_mode_enum_type lock_mode, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_get_div_clk_drop

DESCRIPTION
  Allows external mosule to drop rxd clocks
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_rxd_get_div_clk_drop( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_get_sec_carr_deact

DESCRIPTION
  Allows querying the secondary carrie ractive state by an external module
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_rxd_get_sec_carr_deact( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_set_sec_carr_deact

DESCRIPTION
  Allows deactivating/activating the sec carrier by an external module
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_rxd_set_sec_carr_deact(boolean active, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_clear_all_cfg

DESCRIPTION
  Allows clearing of all CFG by and external module
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_rxd_clear_all_cfg( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_set_cfg

DESCRIPTION
  Allows setting the CFG bitmask by an external module
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_rxd_set_cfg(wl1_rxd_ext_cfg_enum str, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION  wl1_rxd_clear_cfg

DESCRIPTION
  Allows clearing the CFG bmsk by an external module
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_rxd_clear_cfg(wl1_rxd_ext_cfg_enum str, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION  wl1_rxd_in_qdta_gap
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_rxd_in_qdta_gap(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION  wl1_release_rxd_in_qta

DESCRIPTION
  This function releases the RxD chain inside a QTA gap and updates a state variable.
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_release_rxd_in_qta(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION  wl1_request_rxd_tune

DESCRIPTION
  Request secondary resource and tune for new info.
...
SIDE EFFECTS
  None
===========================================================================*/
boolean wl1_request_rxd_tune(wrm_duration_type duration, wrm_freq_input_type band_info, boolean do_foff, wsub_id_e_type wsub_id);

#if defined(FEATURE_WCDMA_ATLAS_MODEM) && defined(FEATURE_WCDMA_RFM_PATH_SUPPORTED)
/*===========================================================================
FUNCTION  wl1_get_rxd_entered_path

DESCRIPTION
  Get current diversity path.
...
SIDE EFFECTS
  None
===========================================================================*/
rfm_path_type wl1_get_rxd_entered_path(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION  wl1_set_rxd_entered_path

DESCRIPTION
  Set current diversity path.
...
SIDE EFFECTS
  None
===========================================================================*/
void wl1_set_rxd_entered_path(rfm_path_type rfm_path, wsub_id_e_type wsub_id);
#else /* (FEATURE_WCDMA_ATLAS_MODEM) && (FEATURE_WCDMA_RFM_PATH_SUPPORTED) */
/*===========================================================================
FUNCTION  wl1_get_rxd_entered_device

DESCRIPTION
  Get current diversity device.
...
SIDE EFFECTS
  None
===========================================================================*/
uint16 wl1_get_rxd_entered_device(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION  wl1_set_rxd_entered_device

DESCRIPTION
  Set current diversity device.
...
SIDE EFFECTS
  None
===========================================================================*/
void wl1_set_rxd_entered_device(uint16 device, wsub_id_e_type wsub_id);
#endif /* (FEATURE_WCDMA_ATLAS_MODEM) && (FEATURE_WCDMA_RFM_PATH_SUPPORTED) */
#endif

/*===========================================================================
FUNCTION  rxd_sec_carr_act_deact

DESCRIPTION
  Set the current activation status of secondary carrier.
...
SIDE EFFECTS
  None
===========================================================================*/
extern void rxd_sec_carr_act_deact(boolean sec_carr_deactivated, boolean force_tune, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_acq_bch_teardown
         
DESCRIPTION
  API called by sleep before triggering inter-sib sleep.
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_rxd_acq_bch_teardown(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_acq_bch_turnon
         
DESCRIPTION
  API called by sleep during wakeup after inter-sib sleep.
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_rxd_acq_bch_turnon(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION wl1_rxd_deallocate_active_evts
         
DESCRIPTION
  Deallocate all active events on Gen TL as part of connected mode cleanup.
...
SIDE EFFECTS
  None
===========================================================================*/
void wl1_rxd_deallocate_active_evts(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION  wl1_rxd_update_srch_processing

DESCRIPTION
  Update SCCH order processing status.
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_rxd_update_srch_processing(boolean status, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION  wl1_rxd_vote_for_acq_bch_suspend

DESCRIPTION
Set the Suspend Status for ACQ/BCH
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_rxd_vote_for_acq_bch_suspend(boolean suspend, wsub_id_e_type wsub_id);

#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
FUNCTION  wl1_qsh_mdump_collect_rxd

DESCRIPTION
  Collect RxD variables.
...
SIDE EFFECTS
  None
===========================================================================*/
extern QSH_MDUMP_FN_ATTR void wl1_qsh_mdump_collect_rxd(wsub_id_e_type wsub_id);
#endif
