...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/ul.h#3 $ $DateTime: 2020/01/22 08:56:27 $ $Author: pwbldsvc $
...
/* Semistatic parameters */
  /* TTI */
#define CCTRCH_TTI(trch) (ul_ext_cntrl_params.ul_curr_cctrch_ptr->semi_static_parms_ptr[trch]->tti)
  /* Coding Type */
#define CCTRCH_CODING_TYPE(trch) \
  (ul_ext_cntrl_params.ul_curr_cctrch_ptr->semi_static_parms_ptr[trch]->coding)
  /* Static RM */
#define CCTRCH_STATIC_RM(trch) \
  (ul_ext_cntrl_params.ul_curr_cctrch_ptr->semi_static_parms_ptr[trch]->staticRM)
  /* CRC */
#define CCTRCH_CRC(trch) (ul_ext_cntrl_params.ul_curr_cctrch_ptr->semi_static_parms_ptr[trch]->crc)

/* Gain Factors */
/* GF calculation type */
#define CCTRCH_GF_CALC_TYPE(tfc_id) \
  (ul_ext_cntrl_params.ul_curr_cctrch_ptr->gain_factor_parms_ptr[tfc_id]->algo)
  /* Control Channel gain */
#define CCTRCH_GF_BETAC(tfc_id) \
  (ul_ext_cntrl_params.ul_curr_cctrch_ptr->gain_factor_parms_ptr[tfc_id]->BetaC)
  /* Data Channel gain */
#define CCTRCH_GF_BETAD(tfc_id) \
  (ul_ext_cntrl_params.ul_curr_cctrch_ptr->gain_factor_parms_ptr[tfc_id]->BetaD)
  /* PRACH power offset valid flag */
#define CCTRCH_PRACH_PWR_OFF_VALID(tfc_id) \
  (ul_ext_cntrl_params.ul_curr_cctrch_ptr->gain_factor_parms_ptr[tfc_id]->valid_Pp_m)
  /* PRACH power offset value */
#define CCTRCH_PRACH_PWR_OFF_VAL(tfc_id) \
  (ul_ext_cntrl_params.ul_curr_cctrch_ptr->gain_factor_parms_ptr[tfc_id]->pwr_offset_Pp_m)
...
#define Nx_j(trch_id) ul_ext_cntrl_params.ul_trch_coding_parms[trch_id].pre_rm_bits
...
typedef struct 
{
   ...
   boolean ul_per_slot_txagc;
} uplink_log_status_type;

typedef struct
{
  /* # bits to be punctured (if +ve) or repeated (if -ve) */
  int32 delta_Nij[L1_UL_TRCH_MAX];
  /* Total # of bits available for a CCTrCH in a 10 ms radio frame with TFC j.
  ** This is derived from SF. */
  uint32 Ndata_j;
  /* Bit masks for available signature and sub-channels */
  ul_asc_avail_mask_struct_type ul_asc_mask[L1_UL_ASC_MAX];
  /* current TFCS restriction info index in use */
  uint8 ul_curr_tfcs_restrict_info_idx;
  /* RRC may modify the pointer values so in order to avoid that
   we maintain a local copy of cctrch cfg parameters */
  l1_ul_semi_static_parms_struct_type ul_semi_static_local_parms_buf[L1_UL_TRCH_MAX][UL_CFG_MAX];
  ...
  ** Note that SF = 256/2**k, 0<=k<=6. See 25.211/5.2.1. */
  ul_sf_data_struct_ary_type *ul_sf_data_tab;
  l1_ul_cctrch_cfg_struct_type *ul_curr_cctrch_ptr;
  ul_trch_coding_struct_type ul_trch_coding_parms[L1_UL_TRCH_MAX];
  /* Spreading Factor Vs (Data Sizes, Min(RM) * Ndata) table.
   * Ndata is defined as number of bits in a radio frame 15*(10 * 2**k).
   * Note that SF = 256/2**k, 0<=k<=6. See 25.211/5.2.1. */
  ul_sf_data_struct_ary_type ul_sf_data_tab_nm[L1_UL_NUM_SF];
  ul_sf_data_struct_ary_type ul_sf_data_tab_cm[L1_UL_NUM_SF];
  /* CCTrCH data buffers definition */
  l1_ul_cctrch_cfg_struct_type ul_cctrch_buf[UL_CFG_MAX];
  mac_ul_tfcs_type *ul_local_tfcs_buf_ptr[UL_CFG_MAX];
  /* Uplink log status top level struct */
  uplink_log_status_type ul_log_status_info;
  /* Turbo interleaving parameters */
  ul_trch_turbo_ileav_parms_type ul_trch_turbo_ileav_parms[L1_UL_TRCH_MAX];
  /* TFCS restriction info buffers */
  l1_ul_tfcs_restrict_info_struct_type ul_tfcs_restrict_info[UL_CFG_MAX];
  /* Rate Matching error parameters */
  ul_rm_parms_struct_type ul_rm_parms[L1_UL_TRCH_MAX];
  /* uplink physical channel configuration */
  l1_ul_phychan_cfg_struct_type ul_phychan_buf[UL_CFG_MAX];
  /* current TFCS restriction info pointer */
  l1_ul_tfcs_restrict_info_struct_type *ul_curr_tfcs_restrict_info;
  l1_ul_phychan_cfg_struct_type *ul_curr_phychan_ptr;
  l1_ul_gain_factor_parms_struct_type ul_gain_factor_local_parms_buf[L1_UL_TFC_MAX][UL_CFG_MAX];
} ul_ext_cntrl_params_type;

extern ul_ext_cntrl_params_type ul_ext_cntrl_params;

typedef struct
{
  /* accumulated sum of RMx * Nxj product upto each trch index i */
  uint32 accum_sum_RMx_Nxj[L1_UL_TRCH_MAX];
  /* UL CM parameter buffer */
  l1_cm_ul_param_struct_type cm_ul_params[L1_TTI_80MS_VAL/L1_TTI_10MS_VAL];
  /* Sum over all TrCHs of RMx * Nxj, used in Zij calculations 25.212/4.2.7 */
  uint32 Sum_RMx_Nxj;
}ul_int_cntrl_params_type;

#define UL_GET_UL_RACH_RAM_DUMP_LOG_STATUS()                   (ul_ext_cntrl_params.ul_log_status_info.ul_rach_ram_dump)
#define UL_SET_UL_RACH_RAM_DUMP_LOG_STATUS(value)              (ul_ext_cntrl_params.ul_log_status_info.ul_rach_ram_dump = value)

#define UL_GET_UPLINK_TX_RF_DEBUG_LOG_STATUS()                 (ul_ext_cntrl_params.ul_log_status_info.uplink_tx_rf_debug)
#define UL_SET_UPLINK_TX_RF_DEBUG_LOG_STATUS(value)            (ul_ext_cntrl_params.ul_log_status_info.uplink_tx_rf_debug = value)

#define UL_GET_UL_DATA_DUMP_LOG_STATUS()                       (ul_ext_cntrl_params.ul_log_status_info.ul_data_dump)
#define UL_SET_UL_DATA_DUMP_LOG_STATUS(value)                  (ul_ext_cntrl_params.ul_log_status_info.ul_data_dump = value)

#define UL_GET_UL_PHYCHAN_DPCH_LOG_STATUS()                    (ul_ext_cntrl_params.ul_log_status_info.ul_phychan_dpch)
#define UL_SET_UL_PHYCHAN_DPCH_LOG_STATUS(value)               (ul_ext_cntrl_params.ul_log_status_info.ul_phychan_dpch = value)

#define UL_GET_UL_FW_RACH_DEBUG_LOG_STATUS()                   (ul_ext_cntrl_params.ul_log_status_info.ul_fw_rach_debug)
#define UL_SET_UL_FW_RACH_DEBUG_LOG_STATUS(value)              (ul_ext_cntrl_params.ul_log_status_info.ul_fw_rach_debug = value)

#define UL_GET_UL_RACH_DEBUG_LOG_STATUS()                      (ul_ext_cntrl_params.ul_log_status_info.ul_rach_debug)
#define UL_SET_UL_RACH_DEBUG_LOG_STATUS(value)                 (ul_ext_cntrl_params.ul_log_status_info.ul_rach_debug = value)

#ifdef FEATURE_WCDMA_HS_RACH
#define UL_GET_UL_HSRACH_CONFIG_LOG_STATUS()                   (ul_ext_cntrl_params.ul_log_status_info.ul_hsrach_config)
#define UL_SET_UL_HSRACH_CONFIG_LOG_STATUS(value)              (ul_ext_cntrl_params.ul_log_status_info.ul_hsrach_config = value)

#define UL_GET_UL_HSRACH_ALLOC_EDCH_RSRC_LOG_STATUS()          (ul_ext_cntrl_params.ul_log_status_info.ul_hsrach_alloc_edch_rsrc)
#define UL_SET_UL_HSRACH_ALLOC_EDCH_RSRC_LOG_STATUS(value)     (ul_ext_cntrl_params.ul_log_status_info.ul_hsrach_alloc_edch_rsrc = value)

#define UL_GET_UL_HSRACH_CHAN_TIMING_LOG_STATUS()              (ul_ext_cntrl_params.ul_log_status_info.ul_hsrach_chan_timing)
#define UL_SET_UL_HSRACH_CHAN_TIMING_LOG_STATUS(value)         (ul_ext_cntrl_params.ul_log_status_info.ul_hsrach_chan_timing = value)

#define UL_GET_UL_HSRACH_PREAMBLE_STATUS_LOG_STATUS()          (ul_ext_cntrl_params.ul_log_status_info.ul_hsrach_preamble_statue)
#define UL_SET_UL_HSRACH_PREAMBLE_STATUS_LOG_STATUS(value)     (ul_ext_cntrl_params.ul_log_status_info.ul_hsrach_preamble_statue = value)
#endif /* FEATURE_WCDMA_HS_RACH */

#define UL_GET_UL_PER_SLOT_TXAGC_LOG_STATUS()                  (ul_ext_cntrl_params.ul_log_status_info.ul_per_slot_txagc)
#define UL_SET_UL_PER_SLOT_TXAGC_LOG_STATUS(value)             (ul_ext_cntrl_params.ul_log_status_info.ul_per_slot_txagc = (value))
/*=========================================================================
...
=========================================================================*/
void ul_set_log_status_periodic( wsub_id_e_type wsub_id);
...
=========================================================================*/
extern uint8 ul_get_ul_cm_dpcch_slot_fmt( wsub_id_e_type wsub_id);
/*=========================================================================
...
extern void ul_calc_cm_tfcs_restrict_info(
  ...
  /* TFC info */
  mac_ul_tfcs_type *tfcs_info, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
uint8 get_next_frame_pilot_bits( uint8 num_slots_txed, wsub_id_e_type wsub_id);
/*===========================================================================
...
extern boolean ul_query_sf_reduced_in_cm_by_hls(
  ...
  l1_sf_enum_type sf, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION        UL_EXT_CNTRL_PARAMS_INIT
...
SIDE EFFECTS
===========================================================================*/
extern void ul_ext_cntrl_params_init(void);
