...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/mcalwcdma_evt.h#3 $
...
01/16/18    ss      Added api to combine all the event masks into one var
12/23/15    raj     Ensure that EUL start and stop are not sent in the same frame
12/03/15    sl      Ensure GenTL events are cleared when W gets deactivated
...
#define mcalwcdma_evt_dl_phychan_comb_chan_event_handler(evt) mcalwcdma_dl_phychan_comb_chan_event_handler(evt ,wsub_id)
...
#define l1_tx_gen_tl_event_array mcalwcdma_evt_ext_cntrl_params[wsub_id].mcalwcdma_evt_tx_tl_event_array
...
typedef struct
{
  uint8 mcalwcdma_evt_rx_tl_event_array[NUM_RX_GEN_TL_EVENTS];
  uint8 mcalwcdma_evt_gen_tl_array[SEQ_NUM_GEN_TL_EVENTS];
  uint8 mcalwcdma_evt_eul_tl_event_array[NUM_EUL_GEN_TL_EVENTS];
  uint8 mcalwcdma_evt_tx_tl_event_array[NUM_TX_GEN_TL_EVENTS];
  seq_event_frame_type mcalwcdma_evt_tl_array[MCALWCDMA_EVT_NUM_INTR];
  rex_crit_sect_type mcal_evt_gen_tl_mutex;
  rex_crit_sect_type mcal_evt_demod_cfg_crit_sect;
}mcalwcdma_evt_ext_cntrl_params_type;

extern mcalwcdma_evt_ext_cntrl_params_type mcalwcdma_evt_ext_cntrl_params[WCDMA_NUM_SUBS];


typedef struct
{
  mcalwcdma_gen_evt_id_mapping_info_struct_type mcalwcdma_gen_evt_id_mapping_info[MCALWCDMA_GEN_TL_EVT_AVAIL_POOL];
  boolean mcalwcdma_evt_comb_cfg_in_progress;
  wl1_modem_mvs_client_type wl1_mvs_client_maint_ops;
  /* copy of event masks enabled for each STMR interrupt */
  uint16 mcalwcdma_evt_stmr_events_mask[MCALWCDMA_EVT_NUM_INTR];
}mcalwcdma_evt_int_cntrl_params_type;
...
SIDE EFFECTS
  None
===========================================================================*/
extern void mcalwcdma_evt_gen_tl_handler( uint16 event_mask , wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None
===========================================================================*/
void mcalwcdma_evt_rx_tl_handler(uint16 event, wsub_id_e_type wsub_id);
...
do {                                                         \
        mcalwcdma_evt_ext_cntrl_params[wsub_id].mcalwcdma_evt_tl_array[intr].event_mask |= evt_mask;   \
       stmr_enable_events( (stmr_int_type)intr, (evt_mask), mcalwcdma_evt_ext_cntrl_params[wsub_id].mcalwcdma_evt_tl_array[intr].firing_time_ptr,wsub_id);   \
} while(0)
...
do {                                                        \
        stmr_disable_events( (stmr_int_type)intr, (evt_mask)  ,wsub_id);               \
        mcalwcdma_evt_ext_cntrl_params[wsub_id].mcalwcdma_evt_tl_array[intr].event_mask &= ~(evt_mask);  \
} while(0)
...
do {                                                                 \
  mcalwcdma_evt_ext_cntrl_params[wsub_id].mcalwcdma_evt_tl_array[intr].firing_time_ptr[event_num]= firing_time; \
} while(0)

/* This macro returns the event firing time of the specified event in units
 * of 256 chips
 */
#define MCALWCDMA_EVT_GET_FIRING_TIME( intr, event_num ) (mcalwcdma_evt_ext_cntrl_params[wsub_id].mcalwcdma_evt_tl_array[intr].firing_time_ptr[event_num])
...
*/
/*=========================================================================*/
extern void mcalwcdma_evt_init( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void mcalwcdma_evt_save_event_masks_for_sleep ( wsub_id_e_type wsub_id);
...
===========================================================================*/
void mcalwcdma_evt_prepare_for_suspend( wsub_id_e_type wsub_id);
...
extern void mcalwcdma_evt_seq_generic_evt_configure(
  ...
  uint8 tick_cnt, 
  /** event type */
  seq_generic_event_enum_type generic_event_type, wsub_id_e_type wsub_id);

extern seq_generic_event_enum_type mcalwcdma_evt_get_configured_generic_evt_type( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void mcalwcdma_evt_control_demod_concurrency(boolean grab_mutex, wsub_id_e_type wsub_id);
...
extern void mcalwcdma_dl_configure_nondch_fdpch_frame_event(boolean configure, uint8 offset, wsub_id_e_type wsub_id);
...
extern uint8 mcalwcdma_evt_gen_tl_allocate_evt_id( wsub_id_e_type wsub_id);
...
extern boolean mcalwcdma_evt_gen_tl_de_allocate_evt_id(
  ...
  uint8 evt_id_to_be_deallocated, wsub_id_e_type wsub_id);
...
extern void mcalwcdma_evt_gen_tl_configure_evt(
  ...
  MCALWCDMA_GEN_EVENT_CB_TYPE evt_cb_func, wsub_id_e_type wsub_id);                                 
...
extern void mcalwcdma_evt_gen_tl_deconfigure_evt(
  ...
  uint8 evt_id_to_be_de_configured, wsub_id_e_type wsub_id);
...
extern void mcalwcdma_evt_generic_tl_handler(uint8 evt, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void mcalwcdma_evt_restore_event_masks_after_sleep( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void mcalwcdma_evt_enable_event_on_gen_tl(stmr_int_type intr, seq_gen_tl_event_id_enum_type evt_id, uint8 fire_time, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern uint8 mcalwcdma_evt_query_fire_time_for_event_on_gen_tl(seq_gen_tl_event_id_enum_type evt_id, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void mcalwcdma_evt_tx_mvs_timing_evt_handler( wsub_id_e_type wsub_id);
/*===========================================================================

FUNCTION mcalwcdma_evt_ext_cntrl_params_init

DESCRIPTION
  This function initializes the mcal evt ext cntrl params.
...
SIDE EFFECTS
  None
===========================================================================*/
extern  void mcalwcdma_evt_ext_cntrl_params_init( wsub_id_e_type wsub_id);
/*===========================================================================

FUNCTION     MCALWCDMA_EVT_SANITY_CHECK_BEFORE_SUSPEND

DESCRIPTION
  This function is called while sending SUSPEND_WCDMA_CNF to RRC
  API ensures all the events on GenTL are de-allocated as part of cleanup
  In case if there is any event active then do EF righaway
...
SIDE EFFECTS
===========================================================================*/
void mcalwcdma_evt_sanity_check_before_suspend( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION             mcalwcdma_evt_get_combined_evt_masks


DESCRIPTION          This function will combine all the event masks into one variable.
  This is done purely for F3 printing purposes and has no other functionality.
  All evt masks are 16 bits each, so combined all four of them in one 64bits value.

DEPENDENCIES
  None.

RETURN VALUE
  Unsigned integer of 64bit.
  order of arrangement in all_evt_masks:
  <16bit>|<16bit>|< 16bit>|<16bit>|
  EUL_TL | TX_TL | GEN_TL | RX_TL 


SIDE EFFECTS
  none
===========================================================================*/
uint64 mcalwcdma_evt_get_combined_evt_masks( wsub_id_e_type wsub_id);
#ifdef FEATURE_QSH_MDUMP 
/*===========================================================================
FUNCTION        mcalwcdma_evt_qsh_mdump_collect_sub
...
SIDE EFFECTS    None.
===========================================================================*/
QSH_MDUMP_FN_ATTR void mcalwcdma_evt_qsh_mdump_collect_sub( wsub_id_e_type wsub_id);
#endif
