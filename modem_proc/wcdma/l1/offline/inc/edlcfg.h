...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/edlcfg.h#3 $
$DateTime: 2020/01/22 08:56:27 $
$Author: pwbldsvc $

when        who     what, where, why
--------    ---     --------------------------------------------------------
12/23/16    raj     Fix for KW errors
03/23/16    sg      Fix for EDL result log packets not submitted during HSRACH
11/24/15    gp      Fix for spurious AG decodes when secondary carrier is active
09/15/15    ar      Ensure EDL CXM entities are deregistered for HSRACH Drop
...
05/19/15    vr      Compiler warnings and error fixes.
03/11/15    rsr     Phase 4 W+W Checkin.
02/20/15    vr      Make ordered page for EDL channels valid after populating 
...
01/15/15    vr      Replacing globals with MACROs.
...
#define WL1_E_DL_OPS_INTLOCK() REX_ISR_LOCK(&edl_ext_cntrl_params.e_dl_config_ops_crit_sect);
#define WL1_E_DL_OPS_INTFREE() REX_ISR_UNLOCK(&edl_ext_cntrl_params.e_dl_config_ops_crit_sect);

/* Default number of new EUL Tx to enable APS voting */
#define EUL_APS_ON_THRESHOLD    20
/* Default number of TTI monitored to update APS voting */
#define EUL_APS_ON_DEFAULT_NUM_TTI   100
...
#define E_DL_GET_DC_HSUPA_DL_STATE() edl_ext_cntrl_params.e_dl_config_db.e_dl_secondary_state

#ifdef FEATURE_WCDMA_DC_HSUPA
#define GET_CURR_CARR_STATE_IND() \
        (((eulcfg_ext_cntrl_params.e_ul_sec_edpch_state == E_UL_SEC_EDPCH_STATE_TX)||(eulcfg_ext_cntrl_params.e_ul_sec_edpch_state == E_UL_SEC_EDPCH_STATE_DISABLED) \
             || (eulcfg_ext_cntrl_params.e_ul_sec_edpch_recfg_state == E_UL_SEC_CFG_WHEN_ACTIVE)) ? DUAL_CARRIER_ACTIVE : \
         (( \
           (eulcfg_ext_cntrl_params.e_ul_sec_edpch_state == E_UL_SEC_EDPCH_STATE_SETUP) || \
           (eulcfg_ext_cntrl_params.e_ul_sec_edpch_state == E_UL_SEC_EDPCH_STATE_CFGD) \
           ) ? DUAL_CARRIER_CONFIGURED : SINGLE_CARRIER \
          ))
#else
#define GET_CURR_CARR_STATE_IND() (SINGLE_CARRIER)
#endif /* FEATURE_WCDMA_DC_HSUPA */

#define WL1_EDL_IS_E_DL_CHAN_VALID(x) (x < E_DL_NUM_CHAN)
#define IS_AGCH_ENTITY_VALID()         WL1_EDLDRX_ENTITY_ID_VALID(edl_ext_cntrl_params.e_dl_config_db.e_dl_drx_params.agch_entity_id) 
#define IS_S_RGCH_ENTITY_VALID()       WL1_EDLDRX_ENTITY_ID_VALID(edl_ext_cntrl_params.e_dl_config_db.e_dl_drx_params.s_rgch_entity_id)
#define IS_NS_RGCH_ENTITY_VALID()      WL1_EDLDRX_ENTITY_ID_VALID(edl_ext_cntrl_params.e_dl_config_db.e_dl_drx_params.ns_rgch_entity_id)
#define IS_HICH_ENTITY_VALID()         WL1_EDLDRX_ENTITY_ID_VALID(edl_ext_cntrl_params.e_dl_config_db.e_dl_drx_params.hich_entity_id)
...
typedef struct 
{
  ...
  /* Global to check if there is any pending cphy which L1 has received during on going EDL configuration for fast resume which is cleared in EDL frame boundary handler */
  boolean cphy_pending_on_edl_config;
...
typedef struct
{
    /*Mutex for the protection of EDL configuration in ISR and task context*/
  rex_crit_sect_type  e_dl_config_ops_crit_sect; 
  uint8 rg_soft_comb_idx[WL1_MAX_UL_CARR_IDX][E_DL_CONFIG_DB_MAX_PAGES][EUL_MAX_RL];
  e_dl_agch_dbl_buf_logging_struct_type e_dl_agch_dbl_buf_logging;
  e_dl_chan_dbl_buf_logging_struct_type e_dl_rgch_dbl_buf_logging;
  e_dl_chan_dbl_buf_logging_struct_type e_dl_hich_dbl_buf_logging;
  /* EUL DL channels config database. It contains both current and ordered
  ** pages for each of the DL channels - AGCH, HICH, RGCH. There can be at
  ** most 1 AGCH and upto 4 HICH and 4 RGCH channels configured */
  e_dl_config_db_struct_type e_dl_config_db;
  /* This flag is an indication to MAC that there is a serving EDCH RLS change */
  boolean e_dl_edch_serv_cell_and_rls_change_ind;
  WCDMA_EUL_DL_CHAN_INFO_LOG_PACKET_type e_dl_log_pkt;
  /* This structure is defined so that the number of RLs in the ext_edl_cntrl->e_dl_config_db
  ** are arranged consecutively with no holes in the array.*/
  e_dl_active_rl_info_struct_type e_dl_active_rl_info[WL1_MAX_UL_CARR_IDX][E_DL_CONFIG_DB_MAX_PAGES][EUL_MAX_RL];
  uint16 e_dl_logging_bmsk;
  e_dl_thresh_param_info_struct_type rgch_thresh_params[WL1_MAX_UL_CARR_IDX][E_DL_CONFIG_DB_MAX_PAGES][EUL_MAX_RL];
  /* Boolean that keeps track of a forced sync A when EUL is active*/
  boolean e_dl_active_during_forced_sync_a;
  /* This feature implements an algorithm to detect
  ** if the granted E-AGCH is a false alarm */
  uint32 agch_threshold[WL1_MAX_UL_CARR_IDX];
  boolean e_dl_drop_precedes_asu_cmd_cphy_setup[WL1_MAX_UL_CARR_IDX];
  /* EDL CM BITMASK FIX: Flag to indicate whether we see reconfig with Timing change */
  e_dl_cm_bitmask_rcfg_with_tc_enum_type eul_debug_cm_recfg_with_tc_bitmask_status[WL1_MAX_UL_CARR_IDX];
  /* Structures to hold HICH and RGCH threshold related paramters for each RL*/
  e_dl_thresh_param_info_struct_type hich_thresh_params[WL1_MAX_UL_CARR_IDX][E_DL_CONFIG_DB_MAX_PAGES][EUL_MAX_RL];
  /* These global variables are for reading the per target sub FN result
  ** buffers. These have been defined to reduce passing these parameters
  ** to function and to keep things modular*/
  uint8 e_dl_read_idx_ag_hich_buf[WL1_MAX_UL_CARR_IDX];
  uint8 e_dl_read_idx_ns_rg_buf[WL1_MAX_UL_CARR_IDX];
  /* The soft combining index for HICH and RGCH stored as an array*/
  uint8 hich_soft_comb_idx[WL1_MAX_UL_CARR_IDX][E_DL_CONFIG_DB_MAX_PAGES][EUL_MAX_RL];
  ...
  ** then the false alarm detection will be turned OFF*/
  boolean e_dl_rgch_fa_algo_single_cell_active[WL1_MAX_UL_CARR_IDX];
  /* The Serving Cell Change (SCC) type when EDL is active
  ** This will be used to fill information in the EUL_SERVING_CELL_CHANGE event*/
  e_dl_scc_during_hho_enum_type e_dl_scc_during_hho;
} edl_ext_cntrl_params_type;

extern edl_ext_cntrl_params_type edl_ext_cntrl_params;

typedef struct
{
  uint16 e_dl_chan_results_log_pkt_samples;
  boolean e_dl_asu_drop_ind_per_carr[WL1_MAX_UL_CARR_IDX];
  /* structure to store the CM params for current and next frames for EUL */
  eul_cm_frames_param_struct_type eul_cm_params;

  #ifdef FEATURE_L1_LOG_ON_DEMAND
  /* This indicates whether the log packet information is valid*/
  boolean e_dl_chan_info_log_pkt_valid;
  #endif

  #if (!defined(FEATURE_QSBSCS))
  wcdma_layer1_eul_tti_reconfig_evt_type     tti_reconfig_event;
  wcdma_layer1_eul_reconfig_or_asu_evt_type  eul_recfg_or_asu_evt2;
  wcdma_layer1_eul_reconfig_or_asu_evt_type  eul_recfg_or_asu_evt1;
  wcdma_layer1_eul_serv_cell_change_evt_type serv_cell_change_event;
  #endif

  #ifdef FEATURE_WCDMA_DC_HSUPA
  e_dl_scc_during_hho_enum_type e_dl_sec_scc_during_hho;
  #endif
} edl_int_cntrl_params_type;
...
#define WL1_GET_CURR_EDL_STATE()    (ulstates_ext_persub_cntrl_params[wsub_id].wl1_ul_state_db.e_dl_state)
#define WL1_SET_CURR_EDL_STATE(edl) (ulstates_ext_persub_cntrl_params[wsub_id].wl1_ul_state_db.e_dl_state =  (edl))
#define WL1_CHK_CURR_EDL_STATE(edl) (ulstates_ext_persub_cntrl_params[wsub_id].wl1_ul_state_db.e_dl_state == (edl))

#define WL1_CHK_CURR_CONN_EDL_STATE(edl) (WL1_CHK_CURR_EDL_STATE(edl))

/* MACRO to check whether EDL state is in config start, config reconfig*/
#define IS_EDL_STATE_CONFIG_START() (WL1_CHK_CURR_CONN_EDL_STATE(E_DL_STATE_CONFIG_START))
#define IS_EUL_INACTIVE()           (WL1_CHK_CURR_CONN_EDL_STATE(E_DL_STATE_INACTIVE))
#define IS_EUL_ACTIVE()             (WL1_CHK_CURR_CONN_EDL_STATE(E_DL_STATE_ACTIVE))
#define IS_EUL_ACTIVE_OR_RECFG()    ((WL1_CHK_CURR_CONN_EDL_STATE(E_DL_STATE_ACTIVE)) || \
                                     (WL1_CHK_CURR_CONN_EDL_STATE(E_DL_STATE_CONFIG_RECFG)))

#ifdef FEATURE_WCDMA_DC_HSUPA 
#define IS_DC_HSUPA_CFGD() ((edl_ext_cntrl_params.e_dl_config_db.e_dl_secondary_state != MCALWCDMA_E_DL_SECONDARY_STATE_INIT) && \
                            (edl_ext_cntrl_params.e_dl_config_db.e_dl_secondary_state != MCALWCDMA_E_DL_SECONDARY_STATE_CONFIG_STOP))
#else
...
#define IS_DC_HSUPA_ACTIVE_OR_RECONFIG_ACTIVE() \
         ((edl_ext_cntrl_params.e_dl_config_db.e_dl_secondary_state == E_DL_SECONDARY_STATE_ACTIVE) || \
          (((edl_ext_cntrl_params.e_dl_config_db.e_dl_secondary_state == E_DL_SECONDARY_STATE_CONFIG_RECONFIG) || \
            (edl_ext_cntrl_params.e_dl_config_db.e_dl_secondary_state == E_DL_SECONDARY_STATE_CONFIG_START))&& \
           (edl_ext_cntrl_params.e_dl_config_db.e_dl_next_secondary_state == E_DL_SECONDARY_STATE_ACTIVE)))

#define IS_DC_HSUPA_IN_CONFIG() \
          ((edl_ext_cntrl_params.e_dl_config_db.e_dl_secondary_state == E_DL_SECONDARY_STATE_CONFIG_START) || \
            (edl_ext_cntrl_params.e_dl_config_db.e_dl_secondary_state == E_DL_SECONDARY_STATE_CONFIG_RECONFIG) || \
              (edl_ext_cntrl_params.e_dl_config_db.e_dl_secondary_state == E_DL_SECONDARY_STATE_CONFIG_STOP))


#define IS_SEC_EDL_ACTIVE_OR_CFG_RECFG() \
    ((edl_ext_cntrl_params.e_dl_config_db.e_dl_secondary_state == E_DL_SECONDARY_STATE_CFGD) || \
            (edl_ext_cntrl_params.e_dl_config_db.e_dl_secondary_state == E_DL_SECONDARY_STATE_ACTIVE) || \
              (edl_ext_cntrl_params.e_dl_config_db.e_dl_secondary_state == E_DL_SECONDARY_STATE_CONFIG_RECONFIG))
#endif /*FEATURE_WCDMA_DC_HSUPA */

/* Macros to check whether the current TTI configured is for 10ms r 2ms*/
#define IS_E_CURR_TTI_10MS() (edl_ext_cntrl_params.e_dl_config_db.curr_eul_tti == E_TTI_10MS)
#define IS_E_CURR_TTI_2MS()  (edl_ext_cntrl_params.e_dl_config_db.curr_eul_tti == E_TTI_2MS)

/* Macros to check whether the ordered TTI configured is for 10ms r 2ms*/
#define IS_E_ORD_TTI_10MS() (edl_ext_cntrl_params.e_dl_config_db.ord_eul_tti == E_TTI_10MS)
#define IS_E_ORD_TTI_2MS()  (edl_ext_cntrl_params.e_dl_config_db.ord_eul_tti == E_TTI_2MS)

/* Macro to check whether the reconfig is because of TTI change*/
#define IS_EUL_IN_TTI_RECFG() (edl_ext_cntrl_params.e_dl_config_db.eul_tti_changed == TRUE)
...
#define EDL_GET_NEW_HICH_CFG_ACTIVE_SUB_FN(carr_idx) edl_ext_cntrl_params.e_dl_config_db.hich_cfg_active_sub_fn[carr_idx]
#define EDL_GET_NEW_RGCH_CFG_ACTIVE_SUB_FN(carr_idx) edl_ext_cntrl_params.e_dl_config_db.rgch_cfg_active_sub_fn[carr_idx]
#define EDL_GET_NEW_AGCH_CFG_ACTIVE_SUB_FN(carr_idx) edl_ext_cntrl_params.e_dl_config_db.agch_cfg_active_sub_fn[carr_idx]

#define EDL_SET_NEW_HICH_CFG_ACTIVE_SUB_FN(value,i) \
  (edl_ext_cntrl_params.e_dl_config_db.hich_cfg_active_sub_fn[i] = value)
#define EDL_SET_NEW_RGCH_CFG_ACTIVE_SUB_FN(value,i) \
  (edl_ext_cntrl_params.e_dl_config_db.rgch_cfg_active_sub_fn[i] = value)
#define EDL_SET_NEW_AGCH_CFG_ACTIVE_SUB_FN(value,i) \
  (edl_ext_cntrl_params.e_dl_config_db.agch_cfg_active_sub_fn[i] = value)
...
#define  EDL_GET_RG_TPC_INFO_CUR_PAGE(carr_idx)  (edl_ext_cntrl_params.e_dl_config_db.rg_tpc_info_curr_page[carr_idx])
#define  EDL_GET_RG_TPC_INFO_ORD_PAGE(carr_idx)  (edl_ext_cntrl_params.e_dl_config_db.rg_tpc_info_ord_page[carr_idx])

#define  EDL_ALLOCATE_RG_TPC_INFO_ORD_PAGE(carr_idx)  \
           (edl_ext_cntrl_params.e_dl_config_db.rg_tpc_info_ord_page[carr_idx] = (\
            (EDL_GET_RG_TPC_INFO_CUR_PAGE(carr_idx) == E_DL_CONFIG_DB_PAGE_TWO) ? \
             E_DL_CONFIG_DB_PAGE_ONE : E_DL_CONFIG_DB_PAGE_TWO))
#define  EDL_SET_RG_TPC_INFO_CUR_PAGE(page, carr_idx)  (edl_ext_cntrl_params.e_dl_config_db.rg_tpc_info_curr_page[carr_idx] = page)
#define  EDL_SET_RG_TPC_INFO_ORD_PAGE(page, carr_idx)  (edl_ext_cntrl_params.e_dl_config_db.rg_tpc_info_ord_page[carr_idx] = page)

#define EDL_GET_AGCH_CORRECTION_FACTOR(page, carr_idx) \
            edl_ext_cntrl_params.e_dl_config_db.agch_info[carr_idx].agch_info_page[page].correction_factor

#define EDL_GET_HICH_CORRECTION_FACTOR(page, e_rl_idx, carr_idx) \
            edl_ext_cntrl_params.e_dl_config_db.e_dl_rl_info[carr_idx][e_rl_idx].hich_info_page[page].correction_factor

#define EDL_GET_RGCH_CORRECTION_FACTOR(page, e_rl_idx, carr_idx) \
            edl_ext_cntrl_params.e_dl_config_db.e_dl_rl_info[carr_idx][e_rl_idx].rgch_info_page[page].correction_factor
...
#define EDL_COMPUTE_TAU_HICH_2MS(tau_dpch)  \
  ...
  #define EUL_GET_ACTIVE_RL_TPC_IDX(rl_idx, carr_idx) \
                  edl_ext_cntrl_params.e_dl_active_rl_info[carr_idx][edl_ext_cntrl_params.e_dl_config_db.rg_tpc_info_curr_page[carr_idx]][rl_idx].tpc_idx

  /*Macro to be used by EUL module to get the PSC of an RL*/
  #define EUL_GET_ACTIVE_RL_PSC(rl_idx, carr_idx) \
                  edl_ext_cntrl_params.e_dl_active_rl_info[carr_idx][edl_ext_cntrl_params.e_dl_config_db.rg_tpc_info_curr_page[carr_idx]][rl_idx].psc

  /*Macro to be used by EUL module to get the RG comb idx */
  #define EUL_GET_ACTIVE_RL_RG_COMB_IDX(rl_idx, carr_idx) \
                  edl_ext_cntrl_params.e_dl_active_rl_info[carr_idx][edl_ext_cntrl_params.e_dl_config_db.rg_tpc_info_curr_page[carr_idx]][rl_idx].rg_comb_idx

  /*Macro to be used by EUL module to get the idx into rgch result buf*/
  #define EUL_GET_ACTIVE_RL_RG_SC_IDX(rl_idx, carr_idx) \
                  edl_ext_cntrl_params.e_dl_active_rl_info[carr_idx][edl_ext_cntrl_params.e_dl_config_db.rg_tpc_info_curr_page[carr_idx]][rl_idx].rg_sc_idx

  /*Macro to be used by EUL module to get the idx into hich result buf*/
  #define EUL_GET_ACTIVE_RL_HICH_SC_IDX(rl_idx, carr_idx) \
                  edl_ext_cntrl_params.e_dl_active_rl_info[carr_idx][edl_ext_cntrl_params.e_dl_config_db.rg_tpc_info_curr_page[carr_idx]][rl_idx].hich_sc_idx

  /*Macro to be used by EUL module to get the num RG RLS */
  #define EUL_GET_NUM_RG_RLS(carr_idx) edl_ext_cntrl_params.e_dl_config_db.num_rls[carr_idx][edl_ext_cntrl_params.e_dl_config_db.rg_tpc_info_curr_page[carr_idx]]

  /*Macro to be used by EUL module to get the num hich RLS */
  #define EUL_GET_NUM_HICH_RLS(carr_idx) edl_ext_cntrl_params.e_dl_config_db.num_tpc_set[carr_idx][edl_ext_cntrl_params.e_dl_config_db.rg_tpc_info_curr_page[carr_idx]]
  ...
  #define EUL_GET_ACTIVE_RL_IS_SERV_CELL(rl_idx, carr_idx) \
                  edl_ext_cntrl_params.e_dl_active_rl_info[carr_idx][edl_ext_cntrl_params.e_dl_config_db.rg_tpc_info_curr_page[carr_idx]][rl_idx].serv_cell



/* Macro to get the cell index using the e_rl_idx*/
#define EDL_GET_CELL_IDX(e_rl_idx,idx)  edl_ext_cntrl_params.e_dl_config_db.e_dl_rl_info[idx][e_rl_idx].cell_idx
...
#define EDL_SET_RL_DROPPED_R99_ASET_FLAG(e_rl_idx, val, carr_idx) \
  edl_ext_cntrl_params.e_dl_config_db.e_dl_rl_info[carr_idx][e_rl_idx].rl_dropped_from_r99_aset = val
#define EDL_GET_RL_DROPPED_R99_ASET_FLAG(e_rl_idx, carr_idx) \
  edl_ext_cntrl_params.e_dl_config_db.e_dl_rl_info[carr_idx][e_rl_idx].rl_dropped_from_r99_aset

#define E_DL_GET_DPCH_STTD_FOR_CELL(dpch_handle, cell_db_idx)  \
  (dl_phch_get_txdiv_info_for_cell(dpch_handle, cell_db_idx ,wsub_id) != MCALWCDMA_TX_DIV_NONE)

/* Macros to get/set current/ordered channel page and to allocate 
 * ordered channel page
 */
#define  EDL_GET_AGCH_CUR_PAGE(carr_idx)  (edl_ext_cntrl_params.e_dl_config_db.agch_info[carr_idx].cur_page)
#define  EDL_GET_AGCH_ORD_PAGE(carr_idx)  (edl_ext_cntrl_params.e_dl_config_db.agch_info[carr_idx].ord_page)
...
#define  EDL_SET_AGCH_CUR_PAGE(page,carr_idx)  (edl_ext_cntrl_params.e_dl_config_db.agch_info[carr_idx].cur_page = page)
#define  EDL_SET_AGCH_ORD_PAGE(page,carr_idx)  (edl_ext_cntrl_params.e_dl_config_db.agch_info[carr_idx].ord_page = page)

#define  EDL_GET_HICH_CUR_PAGE(e_dl_rl_idx,carr_idx)  (edl_ext_cntrl_params.e_dl_config_db.e_dl_rl_info[carr_idx][e_dl_rl_idx].hich_cur_page)
#define  EDL_GET_HICH_ORD_PAGE(e_dl_rl_idx,carr_idx)  (edl_ext_cntrl_params.e_dl_config_db.e_dl_rl_info[carr_idx][e_dl_rl_idx].hich_ord_page)
...
#define  EDL_SET_HICH_CUR_PAGE(e_dl_rl_idx,page,carr_idx)  (edl_ext_cntrl_params.e_dl_config_db.e_dl_rl_info[carr_idx][e_dl_rl_idx].hich_cur_page = page)
#define  EDL_SET_HICH_ORD_PAGE(e_dl_rl_idx,page,carr_idx)  (edl_ext_cntrl_params.e_dl_config_db.e_dl_rl_info[carr_idx][e_dl_rl_idx].hich_ord_page = page)

#define  EDL_GET_RGCH_CUR_PAGE(e_dl_rl_idx,carr_idx)  (edl_ext_cntrl_params.e_dl_config_db.e_dl_rl_info[carr_idx][e_dl_rl_idx].rgch_cur_page)
#define  EDL_GET_RGCH_ORD_PAGE(e_dl_rl_idx,carr_idx)  (edl_ext_cntrl_params.e_dl_config_db.e_dl_rl_info[carr_idx][e_dl_rl_idx].rgch_ord_page)
...
#define  EDL_SET_RGCH_CUR_PAGE(e_dl_rl_idx,page,carr_idx)  (edl_ext_cntrl_params.e_dl_config_db.e_dl_rl_info[carr_idx][e_dl_rl_idx].rgch_cur_page = page)
#define  EDL_SET_RGCH_ORD_PAGE(e_dl_rl_idx,page,carr_idx)  (edl_ext_cntrl_params.e_dl_config_db.e_dl_rl_info[carr_idx][e_dl_rl_idx].rgch_ord_page = page)
...
#define EDL_GET_NUM_CELLS_PER_TPCSI(num_cells, tpc_idx_val, idx) \
  ...
  {\
    if(tpc_idx_val == edl_ext_cntrl_params.e_dl_config_db.e_dl_tpc_set_info[idx].tpc_idx)\
    {\
      num_cells = edl_ext_cntrl_params.e_dl_config_db.e_dl_tpc_set_info[idx].num_cells;\
    }\
  }\
...
for(idx = 0; idx < MAX_NUM_TPC_IDX; idx++) \
{\
  if(rg_comb_idx_val == edl_ext_cntrl_params.e_dl_config_db.e_dl_rls_info[idx].rg_comb_idx)\
  {\
    num_cells = edl_ext_cntrl_params.e_dl_config_db.e_dl_rls_info[idx].num_cells;\
  }\
}\
...
#define E_DL_UPDATE_FARTHEST_CH2DPCH_OFFSET(offset,carr_idx)\
    edl_ext_cntrl_params.e_dl_config_db.farthest_ch2dpch_offset[carr_idx] =\
         (offset > edl_ext_cntrl_params.e_dl_config_db.farthest_ch2dpch_offset[carr_idx]) ? \
          offset : edl_ext_cntrl_params.e_dl_config_db.farthest_ch2dpch_offset[carr_idx];
...
#define E_DL_GET_THIS_RL_TPC_IDX(e_rl_idx, carr_idx) \
  (edl_ext_cntrl_params.e_dl_config_db.e_dl_rl_info[carr_idx][e_rl_idx].e_rl_tpc_idx)

/* Macro to get the RG COMB index for this RL*/
#define E_DL_GET_THIS_RL_RG_COMB_IDX(e_rl_idx, carr_idx) \
  (edl_ext_cntrl_params.e_dl_config_db.e_dl_rl_info[carr_idx][e_rl_idx].e_rl_rg_comb_idx)

/*Macro to be used by EUL module  to get the serving RL index*/
#define EUL_GET_SRV_CELL_IDX(carr_idx) (edl_ext_cntrl_params.e_dl_config_db.e_serv_rl_idx[carr_idx])

/*Macro to be used by EUL module to get the number of RLs in EUL*/
#define EUL_GET_EDCH_NUM_CELLS(carr_idx) (edl_ext_cntrl_params.e_dl_config_db.num_e_rl[carr_idx])

/* Macro for reading the flag for serving RLS change that is indicated to MAC for
** SI trigger.*/
#define EDL_GET_SERV_RLS_CHANGE_FLAG() edl_ext_cntrl_params.e_dl_edch_serv_cell_and_rls_change_ind

/*Macro to set the SERVING CELL CHANGE IND to MAC as FALSE*/
#define EDL_SET_SERV_RLS_CHANGE_FLAG(value) \
    edl_ext_cntrl_params.e_dl_edch_serv_cell_and_rls_change_ind = value


/* MACRO to set the EDL SCC information during HHO*/
#define EDL_SET_SCC_TYPE_DURING_HHO(scc_type) \
  edl_ext_cntrl_params.e_dl_scc_during_hho = scc_type

/* MACRO to set the FORCED SYNC A flag when EDL is active*/
#define EDL_SET_FORCED_SYNC_FLAG(value) \
  edl_ext_cntrl_params.e_dl_active_during_forced_sync_a = value
...
#define E_DL_IS_CONFIG_DB_PAGE_VALID(page)  (page < E_DL_CONFIG_DB_MAX_PAGES)

#define E_DL_IS_CONFIG_DB_PAGE_INVALID(page)  (page == E_DL_CONFIG_DB_INVALID_PAGE)

#ifdef FEATURE_WCDMA_HS_RACH
#define EDL_SET_HS_RACH_E_DL_RL_IDX(rl_idx) \
  	      edl_ext_cntrl_params.e_dl_config_db.hs_rach_e_dl_rl_idx = rl_idx

#define EDL_GET_HS_RACH_E_DL_RL_IDX() edl_ext_cntrl_params.e_dl_config_db.hs_rach_e_dl_rl_idx

#define EDL_SET_HS_RACH_E_DL_AGCH_CELL_IDX(agch_cur_page, val, carr_idx) \
        edl_ext_cntrl_params.e_dl_config_db.agch_info[carr_idx].agch_info_page[agch_cur_page].cell_idx = val
#define EDL_SET_HS_RACH_E_DL_HICH_CELL_IDX(hs_rach_e_dl_rl_idx, hich_cur_page, val, carr_idx) \
        edl_ext_cntrl_params.e_dl_config_db.e_dl_rl_info[carr_idx][hs_rach_e_dl_rl_idx].hich_info_page[hich_cur_page].cell_idx = val

#define EDL_SET_CELL_IDX(e_rl_idx, val, carr_idx)  edl_ext_cntrl_params.e_dl_config_db.e_dl_rl_info[carr_idx][e_rl_idx].cell_idx = val
#endif /**/
...
SIDE EFFECTS    None
===========================================================================*/
extern void e_dl_agch_threshold_init ( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean eul_validate_config(l1_setup_cmd_type* setup, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean e_dl_phychan_cfg_validate(l1_setup_cmd_type *setup, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean e_dl_validate_rl(l1_setup_cmd_type *setup, wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_init(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_config_flush_log_pkts(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);

/*===========================================================================

FUNCTION        E_DL_SET_NUM_LOGGING_SAMP

DESCRIPTION     This function sets the number of samples based on the ordered TTI.
...
===========================================================================*/
extern void e_dl_set_num_logging_samp(wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_config(uint8 e_req_mask, l1_setup_cmd_type *setup_cmd, wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
void e_dl_config_start(l1_e_dl_info_struct_type *e_dl_info_ptr, wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
void e_dl_config_recfg(l1_e_dl_info_struct_type *e_dl_info_ptr, wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
void e_dl_config_stop(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_switch_config(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_send_comb_chan_cfg_cmd ( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_comb_chan_cfg_cmd_handler( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_send_frm_bdry_cmd ( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_comb_chan_frm_bdry_cmd_handler( wsub_id_e_type wsub_id);
...
extern void e_dl_check_serv_rls_change(l1_e_dl_info_struct_type *e_dl_info_ptr,
                                       uint8 new_serv_rg_comb_idx, wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
extern void e_dl_drop_rl(uint8 rl_idx, wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_check_and_update_for_tau_dpch_change(uint16 psc, uint16 new_tau_dpch, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_check_sttd_reconfig( wsub_id_e_type wsub_id);
...
extern void e_dl_add_rl(uint8 recfg_idx, l1_e_dl_info_struct_type *e_dl_info_ptr,
                        uint8 serv_rg_comb_idx, wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_complete_cleanup( wsub_id_e_type wsub_id);
...
extern boolean e_dl_check_ernti_changed(e_dl_agch_info_page_type* curr_page_ptr,
                                                                 e_dl_agch_info_page_type* ord_page_ptr, wsub_id_e_type wsub_id);
/*===========================================================================
...
extern void e_dl_submit_dl_chan_info_log_pkt ( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_calc_num_rls_tpcset_curr_config(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_calc_index_for_soft_combining(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_update_thresh_params_during_config(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
extern void e_dl_invalidate_soft_combine_index(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
/*===========================================================================
...
===========================================================================*/
extern uint8 e_dl_populate_fing_log_info(uint16 psc, PACKED uint16 *chan, dl_tri_carr_id_enum_type demod_carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean eul_sec_validate_config(l1_setup_cmd_type* setup, wsub_id_e_type wsub_id);
/*===========================================================================
...
===========================================================================*/
extern void e_dl_signal_eul_sg_info_change(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
#ifdef FEATURE_L1_LOG_ON_DEMAND
/*===========================================================================
...
===========================================================================*/
void e_dl_log_init( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_chan_info_log_on_demand( wsub_id_e_type wsub_id);
#endif
/*===========================================================================
...
===========================================================================*/
extern void e_dl_set_next_cfn_cm_gap_params(uint8 num_carr, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_prepare_mcal_data(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_upd_cfg_db_start_final_sub_fn_from_mcal( wl1_ul_carr_id_enum_type carr_idx , wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_init_per_target_sub_fn_result_buffer( boolean all_buffs, wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_set_sw_buf_read_idx(uint32 target_sub_fn, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_ul_logging_results_update( wsub_id_e_type wsub_id);
...
============================================================================*/
extern void e_dl_ul_logging_get_num_rls_tpc_set(uint8 *num_rls, uint8 *num_tpc_set, wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
============================================================================*/
extern void e_dl_ul_reset_logging_results( wsub_id_e_type wsub_id);
/*===========================================================================
...
============================================================================*/
extern void e_dl_submit_dl_chan_results_logpkt( wsub_id_e_type wsub_id);
...
extern void e_dl_submit_eul_recfg_or_asu_event(uint8 e_req_mask, wsub_id_e_type wsub_id);
/*===========================================================================
...
===========================================================================*/
 extern void e_dl_update_s_rgch_false_alarm_det(eul_sg_srv_rg_enum_type *rgch_val,
                                    eul_serv_rgch_results_struct_type * s_rg_info, uint16 serv_rl_idx, wsub_id_e_type wsub_id);      
...
===========================================================================*/
extern boolean e_dl_l1m_config_handler(uint8 l1m_e_req_mask, l1_e_dl_info_struct_type *l1m_e_dl_info_ptr, wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_init_edl_drx_params( wsub_id_e_type wsub_id);
#endif /*FEATURE_WCDMA_CPC_DRX*/
...
===========================================================================*/
extern uint16 e_dl_calc_delta_frames_decode_to_apply_agch (uint16 agch2dpch_offset, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern uint16 e_dl_calc_delta_frames_decode_to_apply_hich (uint16 hich2dpch_offset, wsub_id_e_type wsub_id);
...
extern uint16 e_dl_calc_delta_frames_decode_to_apply_rgch (uint16 rgch2dpch_offset,
                                                    boolean serving_cell, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern e_dl_delta_sub_frames_info_struct_type e_dl_calc_delta_sub_frames_decode_to_apply_agch (uint16 agch2dpch_offset, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern e_dl_delta_sub_frames_info_struct_type e_dl_calc_delta_sub_frames_decode_to_apply_hich (uint16 hich2dpch_offset, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern e_dl_delta_sub_frames_info_struct_type e_dl_calc_delta_sub_frames_decode_to_apply_rgch(uint16 rgch2dpch_offset, boolean serving_cell, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_config_db_cell_idx_update(uint8 hs_rach_eul_dl_cell_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
void e_ul_submit_start_recfg_events_to_dm( wsub_id_e_type wsub_id);
...
===========================================================================*/
void e_ul_submit_stop_events_to_dm( wsub_id_e_type wsub_id);
...
===========================================================================*/
void cpc_submit_start_recfg_events_to_dm( wsub_id_e_type wsub_id);
...
===========================================================================*/
void cpc_submit_stop_events_to_dm( wsub_id_e_type wsub_id);
#endif
...
===========================================================================*/
extern boolean e_dl_dch_cleanup( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_asu_drop_handler( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION        EDL_EXT_CNTRL_PARAMS_INIT
...
SIDE EFFECTS
===========================================================================*/
extern void edl_ext_cntrl_params_init( void );

/*===========================================================================
FUNCTION        E_DL_CONFIG_OPS

DESCRIPTION     This function returns the status of whether a CPHY suspend is needed based on the EDL configuration 
...
===========================================================================*/
extern boolean e_dl_config_ops(wsub_id_e_type wsub_id);

#ifdef FEATURE_WCDMA_MULTISIM_COEXISTENCE_SW
#ifdef FEATURE_WCDMA_EFACH_MSIM
/*===========================================================================

FUNCTION        e_dl_invalidate_cxm_registrations

DESCRIPTION     This function deregisters EDL entities from CxM if any pending 
                registrations exist 
...
SIDE EFFECTS    EDL Registrations with Cxm will be deregistered

===========================================================================*/
extern void e_dl_invalidate_cxm_registrations(wsub_id_e_type wsub_id);
