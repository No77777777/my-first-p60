...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/stmr.h#3 $ $DateTime: 2020/01/22 08:56:27 $ $Author: pwbldsvc $

when        who   what, where, why
--------    ---   --------------------------------------------------------
01/10/20   ac      Mini dump framework checkin
01/03/18    rn    GTS : Updating ustmr and qtime close to each other
12/08/15    ap    STMR Interrupt debug code
07/20/15    pr    Use vfr id corresponding to the lte cap for the sub.
03/29/15    pr    v2 WplusW changes for Diag F3.
03/13/15    pr    Split Stmr init to view init and evt tbl init
...
#define STMR_STATUS_DUMP_LOCK()  REX_ISR_LOCK(&stmr_ext_cntrl_params[wsub_id].stmr_status_dump_mutex)
#define STMR_STATUS_DUMP_UNLOCK()  REX_ISR_UNLOCK(&stmr_ext_cntrl_params[wsub_id].stmr_status_dump_mutex)
...
do{  \
      ...
      stmr_count = vstmr_wcdma_get_current_wstmr(stmr_ext_cntrl_params[wsub_id].stmr_view[STMR_FRAME_REF_COUNTER]);  \
...
#define STMR_LOAD_FRAME_REF_COUNTER( cmd ) \
  WL1_MSG3(ERROR,"WCDMA_BOLT_DEMOD:Commented stmr_load_frc for Bringup , counter %d ", cmd, 0, 0);
...
#define STMR_SET_WALL_TIME_MODIFIER( slot_counter, frame_counter )   \
  WL1_MSG3(ERROR,"Commented for Bringup(SLEEP) ,slt ctr%d  fr ctr %d", slot_counter, frame_counter, 0);
...
#define STMR_GET_CNTR_SRC_VALUE_FOR_RX_GEN(cntr_src)                 \
    (stmr_int_cntrl_params[wsub_id].stmr_ref_sel_for_rx_gen[cntr_src])
...
#define STMR_GET_VSTMR_SUB(wsub_id)          \
  ((wsub_id == WCDMA_SUB_ID_0) ? VSTMR_RTC_WCDMA_SUB0 : VSTMR_RTC_WCDMA_SUB1)

#define STMR_GET_WSUB_ID(vstmr_subid)          \
  ((vstmr_subid == VSTMR_RTC_WCDMA_SUB0) ? WCDMA_SUB_ID_0 : WCDMA_SUB_ID_1)
...
typedef void stmr_test_evt_dispatch_func_type
(
  uint16 event_mask, wsub_id_e_type wsub_id
);
typedef struct {
   ...
   */
    ...
    */
   stmr_test_evt_dispatch_func_type* event_proc_func;
}stmr_test_evt_frame_type;
...
typedef struct
{
  rex_crit_sect_type stmr_status_dump_mutex;
  /* stmr mutex for protecting register writes*/
  rex_crit_sect_type stmr_mutex;
#ifdef FEATURE_DEBUG_STMR_TEST
  boolean debug_stmr_test_enable;
#endif
  vstmr_wcdma_view_s *stmr_view[STMR_MAX_NUM_COUNTERS];
}stmr_ext_cntrl_params_type;

extern stmr_ext_cntrl_params_type stmr_ext_cntrl_params[WCDMA_NUM_SUBS];


typedef struct
{
  uint32 stmr_wcdma_xo_cnt;
  /* The event bitmask per interrupt. 
   This is actually a shadow of the bitmask in the STMR H/W register */
  uint16 stmr_event_bitmask[STMR_NUM_INT];
  stmr_event_mask_type stmr_int_events_to_be_processed_mask[STMR_NUM_INT];

  boolean vfr_in_use;

  /* current counter source driving the STMR interrupts */
  stmr_counter_source_enum_type stmr_cntr_src[STMR_NUM_INT];

  vstmr_wcdma_event_tbl_s stmr_evt_table[STMR_NUM_INT];

  uint8 stmr_ref_sel_for_rx_gen[STMR_MAX_NUM_COUNTERS];
  vstmr_rtc_id vstmr_wsub_id;
  stmr_wcdma_vfr_type stmr_vfr_id;

#ifdef FEATURE_DEBUG_STMR_TEST
  uint8 stmr_test_tx_evt_array[4];
  stmr_test_evt_frame_type stmr_test_tx_tl;
  stmr_test_evt_frame_type stmr_test_c0_tl;
  stmr_int_type intr2;
  uint8 stmr_test_c0_evt_array[4];
  vstmr_wstmr_t stmr_vstmr_status_dump[STMR_MAX_NUM_COUNTERS];
  stmr_int_type intr;
#endif /*FEATURE_DEBUG_STMR_TEST*/
}stmr_int_cntrl_params_type;
...
void stmr_default_isr( stmr_event_mask_type event_mask , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void stmr_init( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void stmr_deact_init( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void stmr_enable_events(stmr_int_type intr, uint16 event_mask, uint8* firing_time, wsub_id_e_type wsub_id);
/*===========================================================================
...
DESCRIPTION
   ...
   the argument stmr_int_cntrl_params[wsub_id].intr. It comands a status dump as part of the operation.
...
===========================================================================*/
extern uint8 stmr_get_combiner_cfn(stmr_counter_source_enum_type combiner, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern uint32 stmr_get_cntr_time_chipx8 (stmr_counter_source_enum_type cntr, wsub_id_e_type wsub_id);
...
===========================================================================*/
boolean stmr_is_event_pending_process(stmr_int_type intr, uint8 evt, wsub_id_e_type wsub_id);
...
===========================================================================*/
void stmr_update_event_processing_status(stmr_int_type intr, uint8 evt, wsub_id_e_type wsub_id);
...
extern void stmr_advance_combiner_count(stmr_counter_source_enum_type combiner, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void stmr_retard_combiner_count(stmr_counter_source_enum_type combiner, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void stmr_status_dump( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint32 STMR_COMBINER_COUNT_DYNAMIC(uint8 counter, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint16 STMR_FRAME_REF_COUNT_DYNAMIC( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint32 STMR_REF_COUNT_DYNAMIC( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint32 STMR_COMBINER_COUNT_STATUS(stmr_counter_source_enum_type counter, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint32 STMR_TX_SYS_TIME_STATUS( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint32 STMR_TX_SYS_TIME_FULL_SEG_STATUS( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint32 STMR_GET_TX_TTI_COUNT_STATUS( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint32 STMR_FRAME_REF_COUNT_STATUS( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint32 STMR_FRAME_NUM_STATUS( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint32 STMR_FRAME_REF_STATUS( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void stmr_ctrl_vfr(boolean enable, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
 extern void stmr_adjust_cntr_slew(uint32 cell_ref_pos, wsub_id_e_type wsub_id);

 extern void stmr_int_cntrl_params_init(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION stmr_update_vfr_id

DESCRIPTION
  Acts upon the MSGR indication from NAS and updates the 
  internal database with the VFR ID to be used for each sub
...
SIDE EFFECTS
  None
===========================================================================*/
 extern void stmr_update_vfr_id(wl1_msgr_cmd_type* cmd_ptr, wsub_id_e_type wsub_id);
...
do{  \
      vstmr_wstmr_t stmr_count;  \
      stmr_count = vstmr_wcdma_get_current_wstmr(stmr_ext_cntrl_params[wsub_id].stmr_view[STMR_COMBINER_COUNTER0]);  \
...
do{  \
      vstmr_wstmr_t stmr_count;  \
      stmr_count = vstmr_wcdma_get_current_wstmr(stmr_ext_cntrl_params[wsub_id].stmr_view[STMR_COMBINER_COUNTER1]);  \
      conn_frame_number = stmr_count.conn_frame_no;  \
      frame_cx8 = stmr_count.subframecx8;  \
   }while(0)
/*PR_NOTES: New API for C1 dynamic to read the frame and cx1 value*/
...
do{  \
      vstmr_wstmr_t stmr_count;  \
      stmr_count = vstmr_wcdma_get_current_wstmr(stmr_ext_cntrl_params[wsub_id].stmr_view[STMR_COMBINER_COUNTER2]);  \
      conn_frame_number = stmr_count.conn_frame_no;  \
      frame_cx8 = stmr_count.subframecx8;  \
   }while(0)
...
do{  \
      vstmr_wstmr_t stmr_count;  \
      stmr_count = vstmr_wcdma_get_current_wstmr(stmr_ext_cntrl_params[wsub_id].stmr_view[STMR_TX_SYSTEM_TIME_COUNTER]);  \
      conn_frame_number = stmr_count.conn_frame_no;  \
      frame_cx8 = stmr_count.subframecx8;  \
   }while(0)
...
/*PR_NOTES: New API for Tx dynamic to read the frame and cx1 value*/
  ...
  ===========================================================================*/
  void stmr_test_c0_evt_handler(uint16 mask, wsub_id_e_type wsub_id);
  ...
  ===========================================================================*/
  void stmr_test_tx_evt_handler(uint16 mask, wsub_id_e_type wsub_id);
  ...
  ===========================================================================*/
  void stmr_test_init( wsub_id_e_type wsub_id);
  ...
  ===========================================================================*/
  extern void stmr_test_views( wsub_id_e_type wsub_id);
  ...
  ===========================================================================*/
  void stmr_c0_test_evt_0( wsub_id_e_type wsub_id);
  ...
  ===========================================================================*/
  void stmr_c0_test_evt_1( wsub_id_e_type wsub_id);
  ...
  ===========================================================================*/
  void stmr_c0_test_evt_2( wsub_id_e_type wsub_id);
  ...
  ===========================================================================*/
  void stmr_c0_test_evt_3( wsub_id_e_type wsub_id);
  ...
  ===========================================================================*/
  void stmr_tx_test_evt_0( wsub_id_e_type wsub_id);
  ...
  ===========================================================================*/
  void stmr_tx_test_evt_1( wsub_id_e_type wsub_id);
  ...
  ===========================================================================*/
  void stmr_tx_test_evt_2( wsub_id_e_type wsub_id);
  ...
  ===========================================================================*/
  void stmr_tx_test_evt_3( wsub_id_e_type wsub_id);
  ...
  ===========================================================================*/
  extern void stmr_test_intrs( wsub_id_e_type wsub_id);

#endif /*FEATURE_DEBUG_STMR_TEST*/

  /*===========================================================================

  FUNCTION stmr_one_time_init

  DESCRIPTION
   Do the one time init of stmr structures
  ...
  ===========================================================================*/
  extern void stmr_one_time_init(void);
  /*===========================================================================

  FUNCTION stmr_view_init

  DESCRIPTION
   Performs the init of the stmr view
  ...
  ===========================================================================*/
  extern void stmr_view_init( wsub_id_e_type wsub_id);
  /*===========================================================================

  FUNCTION stmr_evt_tbl_init

  DESCRIPTION
   Performs the stmr init of the evt table and interrupts.
  ...
  ===========================================================================*/
  extern void stmr_evt_tbl_init( wsub_id_e_type wsub_id);

/*===========================================================================

FUNCTION stmr_get_xo_count

DESCRIPTION
 Provides the stmr_wcdma_xo_cnt value
...
===========================================================================*/
extern uint32 stmr_get_xo_count(wsub_id_e_type wsub_id);

/*===========================================================================

  FUNCTION stmr_interrupt_sanity

  DESCRIPTION
   Performs a check to see if there is a mismatch between evt table data and evt bmsk.
  ...
  ===========================================================================*/
  extern void stmr_interrupt_sanity(wsub_id_e_type wsub_id);
#ifdef FEATURE_QSH_MDUMP 
/*===========================================================================

  FUNCTION stmr_qsh_mdump_collect

  DESCRIPTION
   To dump stmr related parameters
  ...
  ===========================================================================*/
QSH_MDUMP_FN_ATTR
void stmr_qsh_mdump_collect(wsub_id_e_type wsub_id);
#endif
#endif /* STMR_H */
