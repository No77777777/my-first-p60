...
/*===========================================================================
  ...
  $Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/wl1drxmanager.h#3 $
...
06/01/15   rs      Macro to check if DRX manager is registered or not
...
02/19/15   pv      DRX/WSLEEP Code Reorg before W+W.
...
02/27/08   scm     Extern wl1drxmanager_ext_cntrl_params.drx_timeout_timer for use in wl1drxentity.c.
...
#define DRX_MANAGER_REGISTERED() (ext_wl1drxmanager_cntrl->drx_manager_entity_handle != (drx_entity_handle_type)DRX_ENTITY_INVALID )
...
RETURN VALUE
  None.
===========================================================================*/
typedef void (*drx_entity_wakeup_cb_type)(drx_entity_handle_type handle, wsub_id_e_type wsub_id);
...
RETURN VALUE
  None.
===========================================================================*/
typedef void (*drx_entity_to_sleep_cb_type)(drx_entity_handle_type handle, wsub_id_e_type wsub_id);
...
typedef struct
{
  boolean l1m_bplmn_in_progress;
  /* Timer to detect timeouts on responses from SRCH and Channel Manager. */
  timer_type drx_timeout_timer;
  rex_crit_sect_type wl1drxmgr_wake_srch_buffr_mutex;
...
** the UE from sleep on RRC command reception, i.e. sleep abort. */
  drx_entity_handle_type drx_manager_entity_handle;
  /* Special handling for PICH requires that we keep special track of
** whether PICH is physically active or not. */
  boolean drx_manager_pich_awake;
}wl1drxmanager_ext_cntrl_params_type;

extern wl1drxmanager_ext_cntrl_params_type wl1drxmanager_ext_cntrl_params[WCDMA_NUM_SUBS];
...
RETURN VALUE
  None.
===========================================================================*/
extern void drx_manager_wake_from_sleep_for_sub(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION     DRX_MANAGER_WAKE_FROM_SLEEP_SUB0
...
RETURN VALUE
  None.
===========================================================================*/
extern void drx_manager_release_drx_mode( wsub_id_e_type wsub_id);
...
extern uint32 drx_manager_update_time_to_next_wakeup(int32 *time_to_next_wakeup_usec, wsub_id_e_type wsub_id);
...
RETURN VALUE
  None.
===========================================================================*/
extern void drx_manager_start_ue_sleep( wsub_id_e_type wsub_id);
...
RETURN VALUE
  None.
===========================================================================*/
boolean wl1drxmgr_clear_hndle_from_wkup_srch_buf(drx_entity_handle_type handle, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean drx_manager_process_cmd(const drx_manager_cmd_type *cmd_ptr, wsub_id_e_type wsub_id);
...
RETURN VALUE
  None.
===========================================================================*/
extern void drx_deregister_entity(drx_entity_handle_type handle, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean drx_suspend_entity(drx_entity_handle_type handle, wsub_id_e_type wsub_id);
...
RETURN VALUE
  boolean - TRUE on success.  FALSE on failure.
===========================================================================*/
extern boolean drx_resume_entity(drx_entity_handle_type handle, wsub_id_e_type wsub_id);
...
RETURN VALUE
  None.
===========================================================================*/
extern void drx_entity_schedule_update(drx_entity_handle_type handle, wsub_id_e_type wsub_id);
...
RETURN VALUE
  None.
===========================================================================*/
extern void drx_entity_sched_update_wo_local_cmd(drx_entity_handle_type handle, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern uint32 drx_manager_get_current_entity_mask( wsub_id_e_type wsub_id);
...
RETURN VALUE
  None.
===========================================================================*/
extern void drx_dl_to_sleep_done(boolean status, wsub_id_e_type wsub_id);
...
RETURN VALUE
  None.
===========================================================================*/
extern void drx_srch_asleep_done( wsub_id_e_type wsub_id);
...
RETURN VALUE
  None.
===========================================================================*/
extern void drx_manager_psc_scan_done( wsub_id_e_type wsub_id);
...
RETURN VALUE
  None.
===========================================================================*/
extern void drx_manager_init( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION     DRX_MANAGER_START_BACKGROUND_SEARCH

DESCRIPTION
  This function is called right before the UE wants to initiate sleep because
  all channels are currently inactive.  It checks if we should start a
  background search in this order of priority:

  (1) BPLMN search.
  (2) PSC scan.

  Only one can execute at a given time.  And we only start one if there is
  enough time until the UE needs to be awake again.

DEPENDENCIES
  None.

PARAMETERS
  msec_to_sleep - Time to next wakeup, i.e. time available to perform
                  a background search.

RETURN VALUE
  TRUE if background search started.  FALSE otherwise.
===========================================================================*/
extern boolean drx_manager_start_background_search(uint32 msec_to_sleep, wsub_id_e_type wsub_id );
...
RETURN VALUE
  None.
===========================================================================*/
extern void drx_rxd_pch_update_wakeup_entity(drx_entity_type handle_waking_up,wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION    DRX_IS_ALT_SUB_IN_DATA

DESCRIPTION
   Checks if alt sub in data for deciding on:
   1. R@/RnN for idle sub's measurements
   2. Stopping BPLMN
   3. preventing lock release during ISS during cell selection/reselection when alt sub is W/L
...
RETURN VALUE
  TRUE/FALSE
===========================================================================*/
extern boolean drx_is_alt_sub_in_data(wsub_id_e_type wsub_id);

extern void wl1drxmanager_ext_cntrl_params_init( wsub_id_e_type wsub_id);

extern void wl1drxmanager_int_cntrl_params_init(void);
...
extern QSH_MDUMP_FN_ATTR void wl1_qsh_mdump_collect_wl1drxmanager_params(wsub_id_e_type wsub_id);
#endif
