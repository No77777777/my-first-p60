...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/eulcfg.h#3 $
...
09/14/15   raj     Activating CPC if PCR with serving cell change request is received
                   along with no change in CPC parameters
08/24/15   cjz     Logging fix for E_TTI logging during EUL TTI reconfig
...
07/06/15   gp      Avoid skiping of CPC STOP command when START command is queued for WFW
07/01/15   cjz     No 0xFF as invalid buffer index for e_ul_cpc_dtx_curr_cfg_db_indx
06/26/15   vr      Code cleanup to use MACROs for UL/EUL state globals.
06/25/15   tsk     Enhance diag submission logic by using boolean instead of array index overflow for 0x435D log.
05/22/15   vr      Remove support to reuse SS tables, always rebuild.
03/29/15   pr      v2 WplusW changes for Diag F3.
03/16/15   vr      Move EDL and CPC init into 'validate init'.
...
#define E_UL_GET_FINAL_SUB_FN_TTI_RECFG()    (eulcfg_ext_cntrl_params.e_ul_sw_config_final_subfn_during_tti_reconfig)
#define E_UL_SET_FINAL_SUB_FN_TTI_RECFG(val) (eulcfg_ext_cntrl_params.e_ul_sw_config_final_subfn_during_tti_reconfig = val)

#ifdef FEATURE_WCDMA_HS_RACH
/* A MACRO that is used to set the EUL global variable that holds the CFN value*/
#define EUL_SET_EUL_SW_CURR_CFN(val) (eulcfg_ext_cntrl_params.e_ul_curr_cfn_val = val)
...
#define EUL_IS_CPC_DTX_ACTIVE(carr_idx)            (eulcfg_ext_cntrl_params.e_ul_cpc_dtx_state[carr_idx] != E_UL_CPC_DTX_STATE_INIT)

#define EUL_IS_CPC_DTX_CYCLE1_OR_CYCLE2(carr_idx)  ((eulcfg_ext_cntrl_params.e_ul_cpc_dtx_state[carr_idx] == E_UL_CPC_DTX_STATE_ACTIVATED_CYCLE1) || \
                                                    (eulcfg_ext_cntrl_params.e_ul_cpc_dtx_state[carr_idx] == E_UL_CPC_DTX_STATE_ACTIVATED_CYCLE2))

#define EUL_CPC_DTX_SET_RECONFIG_ACTIVE_CFN(cfn)   (eulcfg_ext_cntrl_params.e_ul_cpc_dtx_state_change_to_reconfig_cfn = cfn)

#define EUL_CPC_DTX_GET_RECONFIG_ACTIVE_CFN()      (eulcfg_ext_cntrl_params.e_ul_cpc_dtx_state_change_to_reconfig_cfn)

#define EUL_CPC_DTX_RECONFIG_SET_SW_FW_INTF_VAR(val) (eulcfg_ext_cntrl_params.e_ul_cpc_dtx_reconfig_set_sw_fw_intf_var = val)

#define EUL_CPC_DTX_RECONFIG_GET_SW_FW_INTF_VAR()  (eulcfg_ext_cntrl_params.e_ul_cpc_dtx_reconfig_set_sw_fw_intf_var)

#define EUL_CPC_DTX_PRI_CARR_IN_CONFIG_RECONFIG()  (eulcfg_ext_cntrl_params.e_ul_cpc_dtx_ord_cfg_db_indx_valid == TRUE)
#endif /* FEATURE_WCDMA_CPC_DTX */

#ifdef FEATURE_WCDMA_DC_HSUPA 
#define IS_SEC_EUL_CFGD() ((eulcfg_ext_cntrl_params.e_ul_sec_edpch_state == E_UL_SEC_EDPCH_STATE_CFGD) || \
                           (eulcfg_ext_cntrl_params.e_ul_sec_edpch_state == E_UL_SEC_EDPCH_STATE_TX))
#endif /*FEATURE_WCDMA_DC_HSUPA */

#ifdef FEATURE_WCDMA_16_QAM
#define WL1_E_UL_16QAM_GET_STATE()  (eulcfg_ext_cntrl_params.wl1_e_ul_16qam_mode_on)
#define WL1_E_UL_16QAM_SET_STATE(x) (eulcfg_ext_cntrl_params.wl1_e_ul_16qam_mode_on = x)
...
*/
#define E_UL_GET_AG_TAB_IDX() (eulcfg_ext_cntrl_params.e_ul_curr_phychan_cfg_db_ptr->ag_tab_idx)

/* SG table index configured by NW for 16QAM.
** SG_TAB_IDX = 0, when 16QAM not configured, 
** SG_TAB_IDX = 1, when 16QAM configured. 
*/
#define E_UL_GET_SG_TAB_IDX(x) ((x == 0) ? eulcfg_ext_cntrl_params.e_ul_phychan_cfg_db_info[eulcfg_ext_cntrl_params.e_ul_ord_phychan_cfg_db_idx].sg_tab_idx: \
                                           eulcfg_ext_cntrl_params.e_ul_curr_phychan_cfg_db_ptr->sg_tab_idx)
...
#define WL1_E_UL_16QAM_MODIFY_AG_IDX(idx, cidx)       \
        ...
        {                                             \
          if (eulsg_ext_cntrl_params.applied_dl_results[cidx].ag_info.ag_val == 2) \
          {                                           \
            idx = 0;                                  \
          }                                           \
          if (eulsg_ext_cntrl_params.applied_dl_results[cidx].ag_info.ag_val > 2)  \
          ...
          if (GET_EULEDPCH_EDL_LOG_BMSK() & DEBUG_EUL_DL_LOGGING_SG)           \
          {                                                             \
            WL1_MSG3(HIGH,"16QAM_CFG: SG Idx modified: AGval=%d, newIdx=%d", \
                     eulsg_ext_cntrl_params.applied_dl_results[cidx].ag_info.ag_val, idx, 0);  \
          }                                                             \
        }
#else
...
#define UL_GET_EUL_COMBINED_L1_MAC_LOG_STATUS()                (eulcfg_ext_cntrl_params.eul_log_status_info.eul_combined_l1_mac)
#define UL_SET_EUL_COMBINED_L1_MAC_LOG_STATUS(value)           (eulcfg_ext_cntrl_params.eul_log_status_info.eul_combined_l1_mac = value)

#define UL_GET_EUL_L1_MAC_STATS_LOG_STATUS()                   (eulcfg_ext_cntrl_params.eul_log_status_info.eul_l1_mac_stats)
#define UL_SET_EUL_L1_MAC_STATS_LOG_STATUS(value)              (eulcfg_ext_cntrl_params.eul_log_status_info.eul_l1_mac_stats = value)

#define UL_GET_EUL_UL_RATE_MATCHING_INFO_LOG_STATUS()          (eulcfg_ext_cntrl_params.eul_log_status_info.eul_ul_rate_matching_info)
#define UL_SET_EUL_UL_RATE_MATCHING_INFO_LOG_STATUS(value)     (eulcfg_ext_cntrl_params.eul_log_status_info.eul_ul_rate_matching_info = value)

#define UL_GET_EUL_UL_ETFC_RESTRICT_INFO_LOG_STATUS()          (eulcfg_ext_cntrl_params.eul_log_status_info.eul_ul_etfc_restrict_info)
#define UL_SET_EUL_UL_ETFC_RESTRICT_INFO_LOG_STATUS(value)     (eulcfg_ext_cntrl_params.eul_log_status_info.eul_ul_etfc_restrict_info = value)

#define UL_GET_EUL_IRQ_FLOW_CTRL_INFO_LOG_STATUS()             (eulcfg_ext_cntrl_params.eul_log_status_info.eul_irq_flow_ctrl_info)
#define UL_SET_EUL_IRQ_FLOW_CTRL_INFO_LOG_STATUS(value)        (eulcfg_ext_cntrl_params.eul_log_status_info.eul_irq_flow_ctrl_info = value)

#define UL_GET_EUL_UL_CHAN_CFG_INFO_LOG_STATUS()               (eulcfg_ext_cntrl_params.eul_log_status_info.eul_ul_chan_cfg_info)
#define UL_SET_EUL_UL_CHAN_CFG_INFO_LOG_STATUS(value)          (eulcfg_ext_cntrl_params.eul_log_status_info.eul_ul_chan_cfg_info = value)

#define UL_GET_EUL_R_COMBO_TABLE_INFO_LOG_STATUS()             (eulcfg_ext_cntrl_params.eul_log_status_info.eul_r_combo_table_info)
#define UL_SET_EUL_R_COMBO_TABLE_INFO_LOG_STATUS(value)        (eulcfg_ext_cntrl_params.eul_log_status_info.eul_r_combo_table_info = value)

#define UL_GET_EUL_UL_EDPCH_INFO_LOG_STATUS()                  (eulcfg_ext_cntrl_params.eul_log_status_info.eul_ul_edpch_info)
#define UL_SET_EUL_UL_EDPCH_INFO_LOG_STATUS(value)             (eulcfg_ext_cntrl_params.eul_log_status_info.eul_ul_edpch_info = value)

#define UL_GET_WCDMA_EUL_DL_DECODE_INFO_LOG_STATUS()            (eulcfg_ext_cntrl_params.eul_log_status_info.eul_dl_decode_info)
#define UL_SET_WCDMA_EUL_DL_DECODE_INFO_LOG_STATUS(value)       (eulcfg_ext_cntrl_params.eul_log_status_info.eul_dl_decode_info = value)

#define UL_GET_WCDMA_EUL_DL_CHAN_INFO_LOG_STATUS()              (eulcfg_ext_cntrl_params.eul_log_status_info.eul_dl_chan_info)
#define UL_SET_WCDMA_EUL_DL_CHAN_INFO_LOG_STATUS(value)         (eulcfg_ext_cntrl_params.eul_log_status_info.eul_dl_chan_info = value)

#define UL_GET_WCDMA_EUL_DL_AGCH_RESULTS_INFO_LOG_STATUS()      (eulcfg_ext_cntrl_params.eul_log_status_info.eul_dl_agch_results)
#define UL_SET_WCDMA_EUL_DL_AGCH_RESULTS_INFO_LOG_STATUS(value) (eulcfg_ext_cntrl_params.eul_log_status_info.eul_dl_agch_results = value)

#define UL_GET_WCDMA_EUL_DL_HICH_RESULTS_INFO_LOG_STATUS()      (eulcfg_ext_cntrl_params.eul_log_status_info.eul_dl_hich_results)
#define UL_SET_WCDMA_EUL_DL_HICH_RESULTS_INFO_LOG_STATUS(value) (eulcfg_ext_cntrl_params.eul_log_status_info.eul_dl_hich_results = value)

#define UL_GET_WCDMA_EUL_DL_RGCH_RESULTS_INFO_LOG_STATUS()      (eulcfg_ext_cntrl_params.eul_log_status_info.eul_dl_rgch_results)
#define UL_SET_WCDMA_EUL_DL_RGCH_RESULTS_INFO_LOG_STATUS(value) (eulcfg_ext_cntrl_params.eul_log_status_info.eul_dl_rgch_results = value)

#ifdef FEATURE_WCDMA_CPC_DTX
#define UL_GET_CPC_DTX_FW_STATE_MACHINE_LOG_STATUS()            (eulcfg_ext_cntrl_params.eul_log_status_info.cpc_dtx_fw_state_machine)
#define UL_SET_CPC_DTX_FW_STATE_MACHINE_LOG_STATUS(value)       (eulcfg_ext_cntrl_params.eul_log_status_info.cpc_dtx_fw_state_machine = value)

#define UL_GET_EUL_DTX_CONFIG_RECONFIG_LOG_STATUS()             (eulcfg_ext_cntrl_params.eul_log_status_info.eul_dtx_config_reconfig)
#define UL_SET_EUL_DTX_CONFIG_RECONFIG_LOG_STATUS(value)        (eulcfg_ext_cntrl_params.eul_log_status_info.eul_dtx_config_reconfig = value)
#endif /* #ifdef FEATURE_WCDMA_CPC_DTX */

#ifdef FEATURE_WCDMA_DC_HSUPA 
#define UL_GET_EUL_MULTI_CARR_NEW_MPR_LOG_STATUS()              (eulcfg_ext_cntrl_params.eul_log_status_info.eul_multi_carr_new_mpr)
#define UL_SET_EUL_MULTI_CARR_NEW_MPR_LOG_STATUS(value)         (eulcfg_ext_cntrl_params.eul_log_status_info.eul_multi_carr_new_mpr = value)
#endif

#ifdef FEATURE_WCDMA_DC_HSUPA 
#define UL_GET_EUL_MULTI_CARR_POW_ALLOC_LOG_STATUS()            (eulcfg_ext_cntrl_params.eul_log_status_info.eul_multi_carr_pow_alloc)
#define UL_SET_EUL_MULTI_CARR_POW_ALLOC_LOG_STATUS(value)       (eulcfg_ext_cntrl_params.eul_log_status_info.eul_multi_carr_pow_alloc = value)
#endif
...
#define E_UL_CPC_DTX_CFG_ISR_INTLOCK() REX_ISR_LOCK(&eulcfg_ext_cntrl_params.e_ul_cpc_dtx_cfg_isr_mutex)
#define E_UL_CPC_DTX_CFG_ISR_INTFREE() REX_ISR_UNLOCK(&eulcfg_ext_cntrl_params.e_ul_cpc_dtx_cfg_isr_mutex)


#define E_UL_CPC_DTX_ACTION_CFN_CURR_CFN_MIN_DIFF 3

#ifdef FEATURE_WCDMA_CPC_DRX
#define E_DL_CPC_DRX_STATE_INTLOCK() REX_ISR_LOCK(&eulcfg_ext_cntrl_params.e_dl_cpc_drx_state_mutex)
#define E_DL_CPC_DRX_STATE_INTFREE() REX_ISR_UNLOCK(&eulcfg_ext_cntrl_params.e_dl_cpc_drx_state_mutex)
#endif
#endif /* FEATURE_WCDMA_CPC_DTX */
...
#define EUL_DYNAMIC_MEM_ALLOC_AND_CHK(var, type, size) \
        do {                                                                      \
          ...
          if(var == NULL)                                                         \
          {                                                                       \
            WL1_ERR_FATAL("EUL_ERROR: Mem Alloc failed for EUL CFG params", 0, 0, 0); \
          }                                                                       \
        } while (0)
...
typedef struct
{
  /* EUL internal UE cat, it will be 6 when DC is configed while 8 when DC actived*/
  uint8 eul_curr_ue_cat;

  e_ul_cfg_info_struct_type e_ul_cfg_info;
  /* double buffer of E-DPCH channel configuration database.This databse contains
  ** the signalled parameters received from RRC as part of cphy set up req */
  e_ul_sig_cfg_db_struct_type e_ul_phychan_cfg_db_info[EUL_MAX_CFG_DB_COUNT];
  /* pointer to current active E-DCH configuration database E-DCH module shall use
  ** this pointer for accessing current active configuration */
  e_ul_sig_cfg_db_struct_type *e_ul_curr_phychan_cfg_db_ptr;
  ...
  ** done to FALSE */
  uint16 e_ul_sw_config_final_subfn_during_tti_reconfig;

  /* EDPCH Phychan drop pending flag .This flag is used to cleanup eul encoder
  ** driver and eul uplink module data during the EDPCH stop */
  boolean e_ul_drop_pending;
  /* This variable is used to make sure that the next 3 subframes are DTXed after
  ** the subfr at which the EDCH config cmd is issued to mDSP for 2ms TTI */
  uint8 e_ul_stop_edch_tx_bf_setup;
  /* indicate whether to activate SS tables */
  boolean eul_activate_ss_tables_pending;
  /* hho_setup_mask copied from RRC config parameters at EUL validation
  ** At this point, L1 global HHO flags are not set yet. */
  uint16 e_ul_hho_req_mask;

  /* Global variable to keep track of whether the client for MCPM has 
  ** been relinquished or not*/
  boolean e_ul_release_hsupa_client;
  /*EUL client to request for high clk speeds*/
  npa_client_handle eul_npa_client;
  boolean           eul_npa_registered;
  /* EUL Log packets status info */
  eul_log_status_type eul_log_status_info;

  /* The following variables give the max number of samples for EUL UL logpkts
  ** by default hold the max number of samples*/
  uint8 e_ul_log_pkt_max_num_samples;
  uint8 e_ul_log_pkt_max_num_4311_samples;
  int8  e_ul_log_pkt_max_hich_sample_buf_diff;
  uint8 e_ul_log_pkt_max_num_10ms_fr_samples;

  /* Double buffer used for EUL L1 MAC stats log pkt */
  EUL_L1_MAC_STATS_LOG_PKT_type e_ul_l1_mac_stats_info_log_pkt[2];
  eul_log_l1_mac_stats_sample_struct_type e_ul_l1_mac_stats_sum_data[WL1_MAX_UL_CARR_IDX][2];
  uint32  e_ul_l1_mac_stats_log_pkt_sample_num[WL1_MAX_UL_CARR_IDX][2];
  boolean e_ul_l1_mac_stats_log_pkt_num_dirty_bit[2];
  uint8   e_ul_l1_mac_stats_log_pkt_buf_idx;
  uint8   e_ul_l1_mac_stats_log_pkt_log_submit_buf_idx;

  /* Double buffer used for EUL RM log pkt */
  EUL_UL_RATE_MATCHING_INFO_LOG_PKT_type e_ul_rate_match_info_log_pkt[2];
  uint32  e_ul_rm_log_pkt_sample_num[WL1_MAX_UL_CARR_IDX][2];
  boolean e_ul_rate_match_info_dirty_bit[2];
  uint8   e_ul_rate_match_info_buf_idx;
  uint8   e_ul_rate_match_info_log_submit_buf_idx;

  /* Double buffer used for EUL ETFC Restrict info log pkt */
  EUL_UL_ETFC_RESTRICT_INFO_LOG_PKT_type e_ul_etfc_restrict_info_log_pkt[2];
  uint32  e_ul_etfc_restrict_log_pkt_frame_num[2];
  uint32  e_ul_etfc_restrict_log_pkt_sample_num[2];
  boolean e_ul_etfc_restrict_dirty_bit[2];
  boolean e_ul_etfc_restrict_submit_to_diag[2];
  uint8   e_ul_etfc_restrict_buf_idx;
  uint8   e_ul_etfc_restrict_log_submit_buf_idx;

  /*Double buffer used for EUL Combined L1 MAC Log pkt*/
  EUL_COMBINED_L1_MAC_LOG_PKT_type e_ul_l1_mac_info_log_pkt;
  boolean e_ul_l1_mac_log_sample_info_dirty_bit[2];
  uint8   e_ul_l1_mac_log_sample_info_buf_idx;
  uint8   e_ul_l1_mac_log_sample_info_log_submit_buf_idx;

  /* Double buffer used for EUL EDPCH INFO Log pkt */
  EUL_UL_EDPCH_INFO_LOG_PKT_type e_ul_edpch_info_log_pkt[2];
  eul_edpch_log_pkt_struct_type e_ul_edpch_log_pkt_samples[WL1_MAX_UL_CARR_IDX][2];
  boolean e_ul_edpch_log_pkt_dirty_bit[2];
  uint8   e_ul_edpch_log_pkt_buf_idx;
  uint8   e_ul_edpch_log_pkt_log_submit_buf_idx;

  eul_l1_mac_info_buf_struct_type   e_ul_l1_mac_log_pkt_sample_info[WL1_MAX_UL_CARR_IDX];
  eul_l1_mac_info_flush_struct_type e_ul_l1_mac_log_pkt_flush_info[WL1_MAX_UL_CARR_IDX];

  #ifdef FEATURE_WCDMA_DC_HSUPA 
  e_ul_sec_sig_cfg_db_struct_type *e_ul_sec_curr_phychan_cfg_db_ptr;
  e_ul_sec_edpch_state_enum_type e_ul_sec_edpch_state;
  e_ul_sec_edpch_recfg_state_enum_type e_ul_sec_edpch_recfg_state;
  boolean e_ul_sec_edpch_active_pending;
  #endif /* FEATURE_WCDMA_DC_HSUPA */
  ...
  ** changes, the EUL module will not use the seq_get_cfn() to get the CFN value*/
  uint16 e_ul_curr_cfn_val;
  /* The below global variable holds the EUL sub FN value. This is got from the TLM API*/
  uint16 e_ul_curr_sub_fn_val;
  #endif /* FEATURE_WCDMA_HS_RACH */

  #if (!defined(FEATURE_QSBSCS))
  wcdma_layer1_cpc_dtx_action_evt_type cpc_dtx_cfg_event2;
  wcdma_layer1_cpc_dtx_action_evt_type cpc_dtx_cfg_event1;
  #endif /* FEATURE_QSBSCS */

  #ifdef FEATURE_WCDMA_CPC_DTX
  /* Holds current and ordered DTX configs */
  e_ul_cpc_dtx_cfg_db_struct_type e_ul_cpc_dtx_cfg_db_info[EUL_SIG_DTX_CFG_DB_COUNT];
  /* Pointer to current DTX configuration */
  e_ul_cpc_dtx_cfg_db_struct_type *e_ul_cpc_dtx_curr_cfg_db_ptr;
  /* Same as the above description but this one stores the ETFCI selection possible sub FNs based
  ** on MAC DTX cycle configured*/
  e_ul_cpc_dtx_etfci_sel_info_struct_type e_ul_cpc_dtx_etfci_sel_info;
  /* Points to the ordered configuration index */
  uint32  e_ul_cpc_dtx_ord_cfg_db_indx;
  boolean e_ul_cpc_dtx_ord_cfg_db_indx_valid;
  /* Points to the current configuration index */
  uint32  e_ul_cpc_dtx_curr_cfg_db_indx;
  boolean e_ul_cpc_dtx_curr_cfg_db_indx_valid;

  /* CPC DTX state machine. States change depending on RRC requests, EUL
  ** state changes and HS-SCCH orders. */
  e_ul_cpc_dtx_state_enum_type e_ul_cpc_dtx_state[WL1_MAX_UL_CARR_IDX];
  /*This state variable is used only during eul uplink recfg.*/
  e_ul_edpch_recfg_state_enum_type     e_ul_edpch_recfg_state;
  /* CPC ENUM TYPE that stores the information regarding the CPC DTX CFG command
  ** sent to FW or being HELD for sync procedure to get done. The below variable tracks the
  ** same */
  e_ul_cpc_dtx_cfg_cmd_state_enum_type e_ul_cpc_dtx_cfg_cmd_state;

  /* a snapshot of E_TTI specific for EUL logging during TTI recfg*/
  e_tti_enum_type eul_tti_for_logging; 

  /* MAC inactivity threshold */
  uint16  e_ul_cpc_dtx_mac_inact_threshold;
  /* Flag to indicate whether CPC start mdsp sync cmd has been sent
  ** This flag is used only in DCUPA to determine if secondary CPC should start 
  ** along with priamry, or at a different CFN */
  boolean e_ul_cpc_dtx_mdsp_start_cmd_sent;
  ...
  ** change the state of E_UL_CPC_STATE*/
  uint16  e_ul_cpc_dtx_state_change_to_reconfig_cfn;
  /* Records the Pending SFN where Hs-SCCH order needs to be activated */
  uint16  e_ul_cpc_dtx_hsscch_order_subfn;
  /* ul inactivity time for cycle 2 */
  uint16  e_ul_cpc_dtx_inact_timer_for_cycle_2;
  ...
  ** sub FN */
  e_ul_cpc_dtx_burst_pos_info_struct_type e_ul_cpc_dtx_burst_cycle1_info;
  /* Variable to track whether the SW FW interface variable to be set*/
  boolean e_ul_cpc_dtx_reconfig_set_sw_fw_intf_var;
  /* Variable holding the DPCCH slot format. This is needed for 0x422B logging*/
  uint8 e_ul_cpc_dtx_copy_ul_dpcch_slt_fmt_for_logging;
  /* Same as above description but for cycle 2*/
  e_ul_cpc_dtx_burst_pos_info_struct_type e_ul_cpc_dtx_burst_cycle2_info;
  /* Variable to find out whether PA has been turned on 
  ** before DTX Cfg cmd can be sent */
  boolean e_ul_cpc_dtx_pa_turned_on_during_cfg;

  /* Local Mutex for protection of DTX Config variables used in the EUL ISR as well*/
  rex_crit_sect_type e_ul_cpc_dtx_cfg_isr_mutex;

  #ifdef FEATURE_WCDMA_CPC_DRX
  /* Mutex for CPC DRX state protection */
  rex_crit_sect_type e_dl_cpc_drx_state_mutex;
  #endif /* FEATURE_WCDMA_CPC_DRX */
 
  /*Timing info needed to start DTX/DRX at L1 for CPC*/
  l1_dtx_drx_timing_info_struct_type e_ul_saved_dtx_drx_timing_info;
  
  /*All the info needed for DTX operation at L1 for CPC*/
  l1_dtx_info_struct_type e_ul_saved_dtx_info;
  
  #endif /* FEATURE_WCDMA_CPC_DTX */

  #ifdef FEATURE_WCDMA_16_QAM
  /* This is used for run time, when new config is activated.
  ** It is different from below, in the sense that we need to
  ** keep two different variables such that new config related
  ** stuff is not used until that config is activated. */
  boolean wl1_e_ul_16qam_mode_on;
  #endif /* FEATURE_WCDMA_16_QAM */
} eulcfg_ext_cntrl_params_type;

extern eulcfg_ext_cntrl_params_type eulcfg_ext_cntrl_params;


typedef struct
{
  #ifdef FEATURE_WCDMA_CPC_DTX
  #ifdef FEATURE_CPC_DTX_HS_SCCH_REVERT_HHO_RECOVERY 
  e_ul_cpc_dtx_scch_ord_bkup_rstr_struct_type e_ul_cpc_dtx_scch_ord_bkup_rstr_info;
  /* This variable is set to TRUE when the backed up order is disable.Just in case 
  ** the HHO fails and we revert back, this variable is checked to initialize the 
  ** state properly */
  boolean e_ul_cpc_dtx_wait_for_hs_scch_enable[WL1_MAX_UL_CARR_IDX];
  #endif /* FEATURE_CPC_DTX_HS_SCCH_REVERT_HHO_RECOVERY */
  #endif /* FEATURE_WCDMA_CPC_DTX */
}eulcfg_ext_persub_cntrl_params_type;

extern eulcfg_ext_persub_cntrl_params_type eulcfg_ext_persub_cntrl_params[WCDMA_NUM_SUBS];

typedef struct
{
  /* double buffer of DPCH database containing parameters that effect semistatic
  ** tables of EDPCH */
  e_ul_dpch_cfg_db_struct_type e_ul_dpch_cfg_db_info[EUL_MAX_CFG_DB_COUNT];
  /* index of the  ordered configuration into the double buffer of DPCH config db
  ** that shall be used in the current ss table build */
  uint32 e_ul_ord_dpch_cfg_db_idx;
  /* index of current active configuration in double buffer of DPCH config db that
  ** is used in previous ss table build */
  uint32 e_ul_curr_dpch_cfg_db_idx;

  /* Active mdsp E-DCH config parameters buffer This contains the mDSP config db
  ** params. Before issuing config cmd, this struct shall be used to fill the
  ** shadow buffer */
  e_ul_chan_action_enum_type e_ul_mdsp_cfg_action;

  uint16 e_ul_log_start_fn;
  uint16 e_ul_log_end_fn;
  uint32 e_ul_l1_mac_log_pkt_flush_srv_cell_idx[WL1_MAX_UL_CARR_IDX];
  EUL_UL_CHAN_CFG_INFO_LOG_PKT_type eul_ul_chan_cfg_info_log_pkt;
  ...
  #ifdef FEATURE_WCDMA_DC_HSUPA
  e_ul_sec_sig_cfg_db_struct_type e_ul_sec_phychan_cfg_db_info[EUL_MAX_CFG_DB_COUNT];
  /* index of the  ordered configuration in the double buffer of Sec EDPCH config db.
  ** The ordered configuration becomes active configuration after successful FW
  ** config cmd */
  uint32 e_ul_sec_ord_phychan_cfg_db_idx;
  /* index of current active configuration in double buffer of Sec EDPCH config db*/
  uint32 e_ul_sec_curr_phychan_cfg_db_idx;

  boolean e_ul_primary_and_sec_config_incl;
  e_ul_sec_chan_action_enum_type e_ul_fw_sec_cfg_action;
  #endif /* FEATURE_WCDMA_DC_HSUPA */

  #ifdef FEATURE_WCDMA_CPC_DTX
  /* Ensure that the CPC dtx done command is sent only once*/
  boolean e_ul_cpc_dtx_done_cmd_sent;

  EUL_DTX_CONFIG_RECONFIG_LOG_PKT_type eul_dtx_cfg_log_pkt;

  #endif /* FEATURE_WCDMA_CPC_DTX */

  #ifdef FEATURE_L1_LOG_ON_DEMAND
  boolean e_ul_chan_cfg_log_pkt_valid;
  #endif /* FEATURE_L1_LOG_ON_DEMAND */

  /* This variable stores whether a EUL config command has
   * been sent to FW in this frame or not
   */
  boolean eul_mdsp_cmd_sent_this_frame;
} eulcfg_int_cntrl_params_type;

extern void eulcfg_ext_cntrl_params_init();
...
#ifdef FEATURE_WCDMA_HS_RACH
#define EUL_GET_CURR_PAGE() eulcfg_ext_cntrl_params.e_ul_curr_phychan_cfg_db_idx
#define EUL_GET_ORD_PAGE()  eulcfg_ext_cntrl_params.e_ul_ord_phychan_cfg_db_idx
#endif /*FEATURE_WCDMA_HS_RACH*/

#define EUL_CFG_INTLOCK()   REX_ISR_LOCK(&euledpch_ext_cntrl_params.e_ul_cfg_params_mutex)
#define EUL_CFG_INTFREE()   REX_ISR_UNLOCK(&euledpch_ext_cntrl_params.e_ul_cfg_params_mutex)
#define EUL_STATE_INTLOCK() REX_ISR_LOCK(&euledpch_ext_cntrl_params.e_ul_state_params_mutex)
#define EUL_STATE_INTFREE() REX_ISR_UNLOCK(&euledpch_ext_cntrl_params.e_ul_state_params_mutex)

#define EUL_ISR_FIRING_TIME_INTLOCK() REX_ISR_LOCK(&euledpch_ext_cntrl_params.e_ul_isr_firing_time_mutex)
#define EUL_ISR_FIRING_TIME_INTFREE() REX_ISR_UNLOCK(&euledpch_ext_cntrl_params.e_ul_isr_firing_time_mutex)

#ifdef FEATURE_WCDMA_DC_HSUPA 
#define EUL_SEC_CFG_INTLOCK()   REX_ISR_LOCK(&euledpch_ext_cntrl_params.e_ul_sec_cfg_params_mutex)
#define EUL_SEC_CFG_INTFREE()   REX_ISR_UNLOCK(&euledpch_ext_cntrl_params.e_ul_sec_cfg_params_mutex)
#define EUL_SEC_STATE_INTLOCK() REX_ISR_LOCK(&euledpch_ext_cntrl_params.e_ul_sec_state_params_mutex)
#define EUL_SEC_STATE_INTFREE() REX_ISR_UNLOCK(&euledpch_ext_cntrl_params.e_ul_sec_state_params_mutex)
#endif /*FEATURE_WCDMA_DC_HSUPA */


#define WL1_GET_CURR_EUL_STATE()      (ulstates_ext_persub_cntrl_params[wsub_id].wl1_ul_state_db.e_ul_state)
#define WL1_SET_CURR_EUL_STATE(eulSt) (ulstates_ext_persub_cntrl_params[wsub_id].wl1_ul_state_db.e_ul_state =  (eulSt))
#define WL1_CHK_CURR_EUL_STATE(eulSt) (ulstates_ext_persub_cntrl_params[wsub_id].wl1_ul_state_db.e_ul_state == (eulSt))

#define WL1_CHK_CURR_CONN_EUL_STATE(eulSt) (WL1_CHK_CURR_EUL_STATE(eulSt))

#define WL1_GET_CURR_EDPCH_STATE()        (ulstates_ext_persub_cntrl_params[wsub_id].wl1_ul_state_db.e_ul_edpch_state)
#define WL1_SET_CURR_EDPCH_STATE(edpchSt) (ulstates_ext_persub_cntrl_params[wsub_id].wl1_ul_state_db.e_ul_edpch_state =  (edpchSt))
#define WL1_CHK_CURR_EDPCH_STATE(edpchSt) (ulstates_ext_persub_cntrl_params[wsub_id].wl1_ul_state_db.e_ul_edpch_state == (edpchSt))

#define WL1_CHK_CURR_CONN_EDPCH_STATE(edpchSt) (WL1_CHK_CURR_EDPCH_STATE(edpchSt))
...
===========================================================================*/
 ...
 /*===========================================================================
FUNCTION        E_UL_CFG_SET_MIN_BETAS

DESCRIPTION     This function sets the min betas for PRI and SEC carrier.
                This is called in FTM code path only.
...
SIDE EFFECTS    NONE.
===========================================================================*/
extern void e_ul_cfg_set_min_betas(uint8 num_carr);
 ...
 SIDE EFFECTS    None
 ===========================================================================*/
extern uint16 e_ul_get_cfn_val( wsub_id_e_type wsub_id);

extern boolean e_ul_validate_edpch_cfg(l1_e_ul_info_struct_type *e_ul_phychan_ptr, wsub_id_e_type wsub_id);

extern void e_ul_edpch_copy_cfg_data ( l1_e_ul_info_struct_type *e_ul_phychan_ptr, wsub_id_e_type wsub_id);

/*===========================================================================

FUNCTION       EUL_DISABLE_FIQ

DESCRIPTION    This function disables the mdsp fiq . Pass NULL address to
...
===========================================================================*/
extern void eul_disable_mdsp_fiq( wsub_id_e_type wsub_id);
...
=========================================================================*/
void eul_set_log_status_periodic( wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None
===========================================================================*/
extern void e_ul_set_eul_sw_local_cfn( wsub_id_e_type wsub_id);
/*===========================================================================
...
SIDE EFFECTS    None
===========================================================================*/
void e_ul_set_eul_sw_local_sub_fn( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean e_ul_dynamic_mem_alloc( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_ul_dynamic_mem_dealloc( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void eul_dynamic_mem_alloc( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void eul_dynamic_mem_dealloc( wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None
===========================================================================*/
extern boolean e_ul_phychan_cfg_validate(l1_setup_cmd_type *setup, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean e_ul_process_cphy_setup_req( uint8 e_mask, l1_setup_cmd_type *setup_cmd, wsub_id_e_type wsub_id);
...
extern void e_ul_set_ul_dpch_info(uint32 ul_dpcch_slot_fmt_nm,
                                        boolean dpdch_cfged, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_ul_build_ss_tables( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean e_ul_validate_network_cfg( wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None.
===========================================================================*/
extern void e_ul_switch_ss_table_cfg_ptrs( wsub_id_e_type wsub_id);
...
===========================================================================*/
void eul_change_fiq_firing_time_during_tti_recfg(e_tti_enum_type e_tti, wsub_id_e_type wsub_id);
...
=========================================================================*/
extern void e_ul_cleanup( wsub_id_e_type wsub_id);
...
=========================================================================*/
extern boolean e_ul_is_eul_active(wsub_id_e_type wsub_id);
...
=========================================================================*/
extern boolean e_ul_is_in_tx_and_cfg_db_activated(wsub_id_e_type wsub_id);
...
extern void e_ul_enable_ss_table_pending_flag( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern e_ul_edpch_recfg_state_enum_type e_ul_is_edpch_info_changed( wsub_id_e_type wsub_id);
/*===========================================================================
...
===========================================================================*/
void e_ul_edpcch_pre_reconfig( wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None
===========================================================================*/
void e_ul_process_chan_cfg_log_on_demand( wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None
===========================================================================*/
boolean e_ul_check_is_tti_changed (e_tti_enum_type tti, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_ul_cpc_dtx_cfg_event_handler( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_ul_cpc_dtx_build_burst_pos_etfci_sel_tables( wsub_id_e_type wsub_id); 
...
SIDE EFFECTS    None
===========================================================================*/
extern void e_ul_cpc_dtx_update_stop_params( wsub_id_e_type wsub_id);
...
extern boolean e_ul_cpc_dtx_burst_start_subfn(uint32,e_ul_cpc_dtx_state_enum_type, wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None
===========================================================================*/
extern void e_ul_process_dtx_cfg_req(l1_setup_cmd_type *setup_cmd_ptr, wsub_id_e_type wsub_id);
/*===========================================================================
...
SIDE EFFECTS    None
===========================================================================*/
extern void e_ul_process_dtx_reconfig_req(l1_setup_cmd_type *setup_ptr, wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None
===========================================================================*/
extern void e_ul_log_cpc_dtx_cfg_recfg_log_pkt( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean e_ul_cpc_dtx_update_curr_backup_hs_scch_ord( wsub_id_e_type wsub_id);
...
extern void e_ul_cpc_dtx_send_evt_to_diag( e_ul_cpc_dtx_action_enum_type dtx_action, 
  uint8 enab_delay, boolean scch_order, wsub_id_e_type wsub_id);
...
SIDE EFFECTS None
===========================================================================*/
extern uint16 e_ul_cfg_get_dtx_cfg_cfn( wsub_id_e_type wsub_id);
#endif /* FEATURE_WCDMA_CPC_DTX */
...
===========================================================================*/
extern uint16 e_ul_get_ul_cat( wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None
===========================================================================*/
void e_ul_relinquish_mcpm_config(uint8 carr_idx, wsub_id_e_type wsub_id);
#endif
...
===========================================================================*/
extern void e_ul_activate_config( wsub_id_e_type wsub_id);
...
SIDE EFFECTS    Updates the current config to the ordered config.
===========================================================================*/
extern void e_ul_set_fiq_offset(e_tti_enum_type e_tti, wsub_id_e_type wsub_id);
...
SIDE EFFECTS None
===========================================================================*/
extern void e_ul_int_vars_for_eul_resume(e_tti_enum_type e_tti, wsub_id_e_type wsub_id);
/*===========================================================================
...
SIDE EFFECTS None
===========================================================================*/
extern void e_ul_init_vars_for_eul_add( wsub_id_e_type wsub_id);
...
/*===========================================================================
 ...
 ===========================================================================*/
extern void e_ul_config_a2_modem_clks( wsub_id_e_type wsub_id);
...
extern void e_ul_deconfig_a2_modem_clks( wsub_id_e_type wsub_id);
 ...
 SIDE EFFECTS    None
 ===========================================================================*/
extern uint16 e_ul_get_sub_fn_val( wsub_id_e_type wsub_id);
...
============================================================================*/
extern void e_dl_ul_logging_edl_config_update(eul_logging_edl_config_info_struct_type *e_dl_cfg_info_ptr, wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None
===========================================================================*/
boolean e_ul_sec_phychan_cfg_validate(  l1_setup_cmd_type *setup, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean e_ul_sec_process_cphy_setup_req(boolean pri_sec_eul_incl, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_ul_sec_edpch_cfg_event_handler( wsub_id_e_type wsub_id);
...
SIDE EFFECTS  
===========================================================================*/
extern void e_ul_sec_edpch_set_state(e_ul_sec_edpch_state_enum_type next_state, wsub_id_e_type wsub_id);
...
void e_ul_sec_edpch_set_action(e_ul_sec_chan_action_enum_type next_action, wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None
===========================================================================*/
extern void e_ul_cpc_dtx_cfg_done_event_handler( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean e_ul_disable_cpc_dtx( wsub_id_e_type wsub_id);

/*===========================================================================
  FUNCTION:        E_UL_CPC_DTX_RECONFIG_SERV_CELL_CHANGE
  
  DESCRIPTION:  If CPC is already configured, this function checks if serving cell is getting modified or not.
 ...
 extern  void e_ul_cpc_dtx_reconfig_serv_cell_change(wsub_id_e_type wsub_id );

#ifdef FEATURE_QSH_MDUMP 
/*===========================================================================
FUNCTION     wl1_qsh_mdump_collect_eulcfg_int_params

DESCRIPTION
  Dump EULCFG INT CNTRL PARAMS
...
SIDE EFFECTS
  None
===========================================================================*/
QSH_MDUMP_FN_ATTR void wl1_qsh_mdump_collect_eulcfg_int_params(wsub_id_e_type wsub_id);
#endif
