...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/srchcmmeas.h#5 $ $DateTime: 2020/03/09 06:07:26 $ $Author: pwbldsvc $
...
10/16/15    mk      Do not update the SCH state machine in case SCH found during parallel process
07/22/15    vn      Externed function to get RF Device ID07/21/15    ks      Only call meas_complete for FMO gaps when both AGC tune done and gap are done
07/21/15    ks      Only call meas_complete for FMO gaps when both AGC tune done and gap are done
07/13/15    wz/ar   Add prototypes to query if W2G/W2L CM/FMO searches are ongoing
...
11/25/14    psr     Added function prototypes for srchcm_query_interf_fmo_srch_type and srchcm_query_interf_fmo_freq
11/24/14    sr      Changes to issue suspended step1 srch by FMO scheduler during EDRX call
11/21/14    mm      Extern prototype of init IRAT cell DB
...
#define SRCHCMMEAS_INTLOCK()  REX_ISR_LOCK(&srchcmmeas_ext_cntrl_params[wsub_id].srchcmmeas_crit_sect)
#define SRCHCMMEAS_INTFREE()  REX_ISR_UNLOCK(&srchcmmeas_ext_cntrl_params[wsub_id].srchcmmeas_crit_sect)
...
#define SRCHCM_INTERF_STEP1_EXPIRY_EGY_THRESH 249
#else
...
typedef void SRCHCM_GSM_CMD_CLEANUP_DONE_CB_FUNC_TYPE(wsub_id_e_type wsub_id);
...
/* This enum defines RF_SKIP_TUNE states*/
typedef enum
{
  SRCHCM_INTERF_NO_RF_SKIP_TUNE,
  SRCHCM_INTERF_RF_SKIP_TUNE,
  SRCHCM_INTERF_RF_SKIP_TUNE_PREV_GAP
}srchcm_srch_state_rf_skip_tune_enum_type;
...
typedef enum
{
  ...
  SRCHCM_INTERF_STATE_SRCH_INVALID
...
typedef void SRCHCM_INTERF_SEARCH_DONE_CB_FUNC(wsub_id_e_type wsub_id);
...
typedef struct
{
  ...
  srchcm_gsm_cell_struct_type *gsm_cell_ptr[SRCH_SET_GSM_CELL_MAX];

  /* number of new cells just being added */
  uint8 gsm_new_cell_cnt;
  /* number of GSM cell marked deleted in RSSI measurement set */
  uint8 gsm_del_cell_cnt;
  /* GSM cells participating that are newely added */
  srchcm_gsm_cell_struct_type *gsm_cell_new_ptr[SRCH_SET_GSM_CELL_MAX];
} srchcm_gsm_cell_db_struct_type;

/* GSM ARFCN  database */
typedef struct
  {
  /* Count and list of unique ARFCN in GSM cell list */
  uint8 num_arfcn_info;
  srchcm_gsm_arfcn_info_struct_type arfcn_info[SRCH_SET_GSM_CELL_MAX];
} srchcm_gsm_arfcn_db_struct_type;

/* GSM CM measurement scheduling info structure */
typedef struct
    {
  /* Current usable index of measurement info structure */
  uint8 gsm_cur_info_idx;
  uint8 gsm_adv_info_idx;
  /* pointer to the measurement info set bt trigger GSM measure function */
  srchgsm_meas_info_struct_type *gsm_meas_info;

  /* number of cells for BSIC verification */
  uint8 gsm_cell_bsic_ver_cnt;
  /* GSM cells for BSIC verification */
  srchcm_gsm_cell_struct_type *gsm_cell_bsic_ver_ptr[SRCHCM_SET_GSM_CELL_BSIC_IDENTIFY_MAX];
  /* cell count in various BSIC verification state */
  uint8 gsm_cell_bsic_id_total_cnt;
  uint8 gsm_cell_bsic_id_cnt;
  uint8 gsm_cell_bsic_id_abort_cnt;
  uint8 gsm_cell_bsic_reconf_cnt;
  /* first cell index in the cell verification list for various BSIC verification states */
  uint8 gsm_cell_bsic_reconf_first_idx;
  /* current cell that is under BSIC confirmation */
  srchcm_gsm_cell_struct_type *cur_gsm_cell_bsic_id;
  /* this indicate that BSIC Id attempts have expired for the current cell under measurement */
  boolean cell_id_abort_after_bsic_id_done;
  /* current cell index that is under BSIC reconfirmation */
  srchcm_gsm_cell_struct_type *cur_gsm_cell_bsic_reconf;
  /* this indicate that BSIC reconf attempts have expired for the current cell under measurement */
  boolean cell_reconf_abort_after_bsic_reconf_done;
  boolean cell_reconf_force_abort;

  /* Counter of how many consecutive RSSI gaps are used for FCCH detection */
  uint8 rssi_for_fcch_oride;

  /* this indicate that as soon as the BSIC Id/reconf measurement is done
     check for BSIC reconf/Id state and send signal to L1 task to process
     the RSSI reault */
  boolean rssi_update_after_bsic_id_meas;
  boolean rssi_update_after_bsic_reconf_meas;

  boolean cell_bsic_ver_status_update_after_bsic_id_done;
  boolean cell_bsic_ver_status_update_after_bsic_reconf_done;
  boolean bsic_ver_status_to_update;

  /* RSSI measurement age in units of WDMA frame (10 ms)
     This value is set to 0 in function srchcm_meas_sort_rssi_update_bsic_ver_list
     in task context and is intended to be accessed by measurement eval and report
     module in task context only. Marking of RSSI valid and age to 0 is not protceted
     by WCDMA_INTLOCK.
     It is updated in function srchcm_meas_gsm_bsic_reconf_abort_timer_update.
     Value is applicable to cell with RSSI valid flag set to TRUE.
     It is not inited in any init function.
     This value will have have inaccuracy at resume when compressed mode manager
     is suspended. Inaccuracy will by suspend duration. */
  uint32 rssi_meas_age;

  /* this variables ignore the setting of signal and RSSI processing.
     This can be used in case of timeouts */
  boolean ignore_rssi_meas_processing;

  uint32 rssi_gaps_was_id_gap;
  uint32 rssi_gaps_was_reconf_gap;

  uint32 bsic_id_gaps_was_rssi_gap;
  uint32 bsic_id_gaps_was_id_gap;
  uint32 bsic_id_gaps_was_reconf_gap;

  uint32 bsic_reconf_gaps_was_id_gap;
  uint32 bsic_reconf_gaps_was_reconf_gap;

  boolean cell_bsic_ver_status_update_pending;

  /* RSSI burst is getting triggered inside SCH gap */
  boolean rssi_in_sch_trigger_status;

  /* Last time stamp at which RSSI samples were trimmed */
  uint16 last_arfcn_sample_trim_ts;

  /* this variable count the number of frame since no cmd pending */
  uint8 num_frame_since_no_pending_cmd;
  /* call back function for all GSM command done */
  SRCHCM_GSM_CMD_CLEANUP_DONE_CB_FUNC_TYPE* gsm_cmd_cleanup_done_cb_func;
  /* This is the number of GSM frame to wait for command cleanup done. */
  uint8 gsm_cleanup_done_wait_gframes;

  /* this indicates the index of the GSM cell last measured for RSSI */
  uint8 last_gsm_cell_idx_rssi_meas;
  /* indicates that this RSSI measurement for this period is in progress */
  boolean rssi_meas_in_progress;
  /* indicate last sample of ARFCN for RSSI meas hit */
  boolean last_arfcn_sample_hit;

  /* timer to keep track of how long the previous RSSI report was signaled */
  uint8 last_rssi_meas_signaled_timer;

  /* this variable force FCCH detect, SCH decode result to fail. It is used while cleanup */
  boolean force_result_fail;
} srchcm_gsm_meas_local_sched_info_struct_type;

typedef struct
{
  /* this indicate that CM interRAT search is done */
  boolean rssi_meas_done_status;
  boolean bsic_verified_status;
  /* Prepare for sch window frame num in advance to use for scheduling RSSI bursts in SCH */
  uint8 prep_sch_decode_frame_for_rssi;
  /* Prepare for sch window offset of the beginning from the frame start to use for scheduling 
     RSSI bursts in SCH*/
  uint16 prep_sch_decode_offset_qs_for_rssi; 
  /* Log the estimation num of RSSI meas cells based on the length of the GAP before/after SCH window */
  uint8 num_rssi_meas_before_sch;
  uint8 num_rssi_meas_after_sch;
  /* Log the num of pwr scan cmds before SCH window */
  uint8 num_pwr_scan_cmds_before_sch;
  /* current cell index is has its BSIC verified.*/
  srchcm_gsm_cell_struct_type *gsm_cell_bsic_verified;

} srchcm_gsm_meas_ext_sched_info_struct_type;

#define L1_CM_LOG_BUF_COUNT 2

/* GSM CM measurement log info structure */
typedef struct
{
  L1_CM_GSM_MEAS_LOG_PKT_type gsm_meas_log_pkt[L1_CM_LOG_BUF_COUNT];

  uint16 log_buf_used_size[L1_CM_LOG_BUF_COUNT];
  uint8 cur_log_pkt;

  PACKED uint8 *cur_log_buf_ptr;
  uint16 size_log_buf_used;
  uint8 num_wframe_since_first_log_update;
  boolean log_submit_pending;

  l1_cm_gsm_rssi_meas_log_info_struct_type *rssi_meas_log_info;
  l1_cm_gsm_rssi_burst_log_info_struct_type *rssi_burst_log_info;
  l1_cm_fcch_acq_log_info_struct_type *fcch_acq_log_info;

  /* Log packet location for the pending command */
  l1_cm_fcch_acq_log_info_struct_type *fcch_acq_log_info_pending;
  l1_cm_sch_dec_log_info_struct_type *sch_dec_log_info[SRCHCM_GSM_MAX_SCH_RESULT];

  uint8 cur_rssi_log_buf_idx;
  boolean rssi_result_log_info_update_pending[L1_CM_LOG_BUF_COUNT];
  uint8 cur_fcch_log_buf_idx;

  /* Log buffer index that is pending */
  uint8 fcch_log_buf_idx_pending;
  boolean fcch_result_log_info_update_pending[L1_CM_LOG_BUF_COUNT];

  uint8 num_sch_result_pending[L1_CM_LOG_BUF_COUNT];
  uint8 sch_result_log_buf_idx[SRCHCM_GSM_MAX_SCH_RESULT];
}  srchcm_gsm_meas_log_info_struct_type;
...
typedef struct
{
  /* number of inter RAT cell count */
  uint8 inter_rat_cell_cnt;

  /* Inter RAT cell structure pool */
  srchcm_inter_rat_cell_struct_type inter_rat_cells[SRCH_SET_INTER_RAT_CELL_MAX * 2];

  /* Unused inter-rat set pointers */
  srchcm_inter_rat_cell_struct_type *inter_rat_cell_ptr[SRCH_SET_INTER_RAT_CELL_MAX * 2];
}srchcm_inter_rat_cell_db_struct_type;

/* ================================================================================== */
/*                        GSM search/meas master structure                                        */
/* ================================================================================== */
typedef struct
{
  /* GSM ARFCN information */
  srchcm_gsm_arfcn_db_struct_type srchcm_gsm_arfcn_db;
  /* GSM inter-RAT cell database */
  srchcm_inter_rat_cell_db_struct_type srchcm_inter_rat_cell_db;
  /* GSM measurement scheduling info local to the module */
  srchcm_gsm_meas_local_sched_info_struct_type gsm_meas_int_sched_info;
  /* GSM CM measurement logging related information */
  srchcm_gsm_meas_log_info_struct_type srchcm_gsm_meas_log_info; 
} srchcm_gsm_int_cntrl_params_type;

typedef struct
{
  /* GSM CM cell information */
  srchcm_gsm_cell_db_struct_type srchcm_gsm_cell_db;
  /* GSM measurement external scheduling information exported to other modules */
  srchcm_gsm_meas_ext_sched_info_struct_type gsm_meas_ext_sched_info;
} srchcm_gsm_ext_cntrl_params_type;

/* GSM CM measurement scheduling information exposed to external modules */
extern srchcm_gsm_ext_cntrl_params_type srchcm_gsm_ext_cntrl_params[WCDMA_NUM_SUBS];

/* ================================================================================== */
/*                        Srchcmmeas inter-freq search/meas master structure          */
/* ================================================================================== */

typedef struct{
  rex_crit_sect_type srchcmmeas_crit_sect;
#ifdef FEATURE_WCDMA_DC_HSUPA
  srchcm_interf_step1_info_struct_type srchcm_dc_step1_result;
#endif
  srchcm_irat_drv_init_status_struct_type srchcm_irat_drv_init_status;
  boolean srchcm_interf_search_done_status;
  boolean srchcm_interf_search_last_cell_in_reconf_list_hit;
  srchcm_interf_mode_enum_type srchcm_interf_mode;
}srchcmmeas_ext_cntrl_params_type;

extern srchcmmeas_ext_cntrl_params_type srchcmmeas_ext_cntrl_params[WCDMA_NUM_SUBS];


typedef struct{
  dll_struct_type *srchcm_interf_reconf_cell_list_to_trigger;
  dll_struct_type srchcm_interf_empty_cell_list;
  boolean srchcm_interf_psc_acq_list_srch_in_progress[MCALWCDMA_SRCH_CM_MAX_NUM_CMQ];
  dll_struct_type srchcm_interf_reconf_cell_list[MCALWCDMA_SRCH_CM_MAX_NUM_CMQ];
  srchcm_interf_rf_set_info_struct_type srchcm_interf_rf_setup_info;
  boolean srchcm_interf_reconf_list_srch_in_progress[MCALWCDMA_SRCH_CM_MAX_NUM_CMQ];
  uint16 srchcm_time_counter;
  uint8 srchcm_step1_non_coh_int_done_len;
  int16 srchcm_interf_num_reconf_srch_cells_doable;
  srchcm_srch_state_enum_type srchcm_interf_step1_srch_state;
  srchcm_interf_cell_struct_type srchcm_interf_cell_info[SRCHCM_SET_INTERF_CELL_MAX];
  srchcm_interf_info_struct_type srchcm_interf_info[SRCHCM_SET_INTERF_MAX];
  boolean srchcm_interf_step1_srch_in_progress;
  srchcm_interf_undet_cell_srch_state_type srchcm_interf_undet_cell_srch_state;
  uint8 srchcm_step1_sched_nn[MCALWCDMA_SRCH_CM_MAX_NUM_CMQ];
  boolean srchcm_interf_reconf_cell_valid[SRCHCM_PN_SRCH_MAX_TASK];
  srchcm_interf_search_type_enum_type srchcm_interf_last_search_type;
  srchcm_srch_state_enum_type srchcm_interf_list_srch_state;
  dll_struct_type *srchcm_interf_reconf_cell_list_ptr_at_q_idx[MCALWCDMA_SRCH_CM_MAX_NUM_CMQ];
  uint8 srchcm_fach_events_received[MCALWCDMA_SRCH_CM_MAX_NUM_CMQ];

  /* Variables used for RF_SKIP_TUNE workaround*/
   /* to store type of search schedules in immediate CM gap, this will be use to figure out for 
      which scheduled measurement WL1 recieve RF_SKIP_TUNE*/
  srchcm_interf_search_type_enum_type srch_state[MCALWCDMA_SRCH_CM_MAX_NUM_CMQ];
   /* variables to store  RF_SKIP_TUNE state fro differnet interf measurement searches */  
  srchcm_srch_state_rf_skip_tune_enum_type step1_meas_rf_tune_is_skipped[MCALWCDMA_SRCH_CM_MAX_NUM_CMQ];
  srchcm_srch_state_rf_skip_tune_enum_type psc_acq_meas_rf_tune_is_skipped[MCALWCDMA_SRCH_CM_MAX_NUM_CMQ];
  srchcm_srch_state_rf_skip_tune_enum_type reconf_meas_rf_tune_is_skipped[MCALWCDMA_SRCH_CM_MAX_NUM_CMQ];
  boolean send_abort_to_rf[MCALWCDMA_SRCH_CM_MAX_NUM_CMQ];
   /* cfn at which WL1 configures step1 search, this variable is use to know if scheduled 
       consecutive step1 searches in back to back CM gap */
  uint8 step1_srch_configured_cfn;  
  boolean step1_consective_gaps_scheduled;
  boolean step1_blow_next_gap_searches;
   /* storing PSC's scheduled for PN search, this will be use to reschedule this PSC's 
      if WL1 recive RF_SKIP_TUNE for PN search script building */
  uint16 psc_acq_configured_scr[MCALWCDMA_SRCH_CM_MAX_NUM_CMQ][SRCHCM_MAX_NUM_PSC_ACQ_SRCH_PER_GAP];
   /* no. of psc scheduled on command queue 0 */  
  uint8 psc_acq_configured_scr_cmq0_count;
   /* no. of psc scheduled on command queue 1 */
  uint8 psc_acq_configured_scr_cmq1_count;
   /* to record RF_TUNE_SKIP count*/
  uint8 rf_tune_skip_count;

}srchcmmeas_int_cntrl_params_type;

srchcmmeas_int_cntrl_params_type srchcmmeas_int_cntrl_params[WCDMA_NUM_SUBS];

/* indicate that freq info at index is valid */
#define SRCHCM_INTERF_FREQ_IDX_VALID(freq_idx) \
  (srchcmmeas_int_cntrl_params[wsub_id].srchcm_interf_info[freq_idx].valid)
...
#define SRCHCM_INTERF_FREQ_EVAL_RPT_IMMEDIATE(freq_idx) \
  (srchcmmeas_int_cntrl_params[wsub_id].srchcm_interf_info[freq_idx].eval_rpt_before_proc_waiting_host_update_list)
#define SRCHCM_INTERF_FREQ_ALL_RECONF_PERIOD(freq_idx) \
  (srchcmmeas_int_cntrl_params[wsub_id].srchcm_interf_info[freq_idx].last_reconf_period_all_cells_w_frame)
/* freq of freq index info */
#define SRCHCM_INTERF_FREQ_AT_INFO_IDX(freq_idx) \
  (srchcmmeas_int_cntrl_params[wsub_id].srchcm_interf_info[freq_idx].freq)

#define SRCHCM_INTER_FREQ_STEP1_NEEDS_REPORTING_CHECK(freq_idx) \
  (srchcmmeas_int_cntrl_params[wsub_id].srchcm_interf_info[freq_idx].step1_needs_reporting_check)
...
SIDE EFFECTS
  None.
===========================================================================*/
extern srchcm_interf_search_type_enum_type srchcm_query_interf_fmo_srch_type(wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern uint16 srchcm_query_interf_fmo_freq(wsub_id_e_type wsub_id);
...
/*==========================================================================
 ...
 ===========================================================================*/
extern uint32 srchcm_get_dirat_value(uint8 num_tg_slots, uint8 tgsn, wsub_id_e_type wsub_id);
...
extern void srchcm_interf_list_srch_log_packet_alloc( wsub_id_e_type wsub_id);

/*===========================================================================

FUNCTION : WL1_X2W_SRCH_MDSP_SYNC_RF_TUNE_SKIP_DONE_ISR

DESCRIPTION :

  This ISR is associated with the RF TUNE SKIP done Interrupt issued by the mDSP
  to the Host. The SR done interrupt comes if the mDSP firmware doesnt have enough time
  to tune the RF for the search. The handler calls the event done callback function installed for
  this ISR with the event flag. ML1 needs to clear the empty gap in L2W driver after this.
...
extern void srchcm_w2w_srch_mdsp_sync_rf_tune_skip_done_isr(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION srchcm_enque_srch_state

DESCRIPTION
  This function is called by do_inter_f_search and it stores the the search schedule in 
  the immidiate CM gap. This store values use to identify measurement purpose for which  
  WL1 recives RF_TUNE_SKIP from WFW, eventually from RF.
  srch_state:                  measurement purpose:
   1.                              step1
   2.                              pn
   3.                              reconf
   4.                              no_search
...
extern void srchcm_enque_srch_state(srchcm_interf_search_type_enum_type srch_state, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION srchcm_deque_srch_state

DESCRIPTION
  This function is called by srchcm_interf_gap_end_tune_done and it clears the 0th index
  results store by srchcm_enque_srch_state() in srch_state[] and moves whatever value 
  store at index[1] to index[0]  and overwrites index[1] with INVALID value
...
extern void srchcm_deque_srch_state(wsub_id_e_type wsub_id);
...
/*==========================================================================
 ...
 ===========================================================================*/
extern void srchcm_interf_list_srch_log_packet_dealloc(wsub_id_e_type wsub_id);
/*===========================================================================
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srchcm_meas_init( wsub_id_e_type wsub_id);
/*===========================================================================
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srchcm_meas_reset_interf_step1_srch(wsub_id_e_type wsub_id);
/*===========================================================================
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srchcm_update_time_counter( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srchcm_init_irat_cell_db( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern dll_struct_type* srchcm_meas_interf_get_cells_to_reconf( wsub_id_e_type wsub_id);
...
extern srchcm_interf_search_type_enum_type srchcm_meas_get_next_inter_f_srch_type(
  ...
  uint8 min_num_tg_slots, wsub_id_e_type wsub_id);
...
extern srchcm_interf_srch_doability_enum_type srchcm_meas_trigger_inter_f_srch(
  ...
  uint32* nbr_rxlm_buf_index, wsub_id_e_type wsub_id);
...
extern void srchcm_update_freq_srch_allowed(
  uint8 srch_allowed, wsub_id_e_type wsub_id);
...
extern void srchcm_update_all_freq_cell_reconf_timer_expiry_vals(
  ...
  uint32 reconf_timer_val, wsub_id_e_type wsub_id);
...
extern void srchcm_update_cell_detect_timer_expiry_val(
  ...
  uint32 detection_timer_val, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srchcm_interf_set_cm_step1_age_expiry_val( wsub_id_e_type wsub_id);
...
/* the step1 results to check against the cell list */
  srchcm_step1_srch_result_struct_type *step1_results, wsub_id_e_type wsub_id);
...
extern void srchcm_step1_srch_done_handler(
  ...
  uint8 q_index, wsub_id_e_type wsub_id);
...
extern boolean srchcm_perform_sidelobe_detection(
  ...
  uint8 peak_idx,
  /* Antenna index */
  uint8 ant_index, wsub_id_e_type wsub_id);
...
void srchcm_pn_srch_for_step1_peaks_done_handler(
  /* q index of list search done */
  uint8 trans_id, wsub_id_e_type wsub_id);
...
extern void srchcm_interf_abort_step1_srch(uint8 idx, wsub_id_e_type wsub_id);
...
extern void srchcm_interf_abort_psc_acq_list_srch(uint8 idx, wsub_id_e_type wsub_id);
...
extern void srchcm_interf_abort_reconf_list_srch(uint8 index_q, wsub_id_e_type wsub_id);
...
extern void srchcm_interf_increment_timers(uint16 frame_inc, wsub_id_e_type wsub_id);
...
extern void srchcm_meas_release_w2w_rf_buf(
  ...
  uint8 cleanup_ptr_index, wsub_id_e_type wsub_id);
...
extern boolean srchcm_interf_query_srch_in_progress(wsub_id_e_type wsub_id);
...
extern srchcm_interf_cell_struct_type* srchcm_interf_get_next_cell_to_update( wsub_id_e_type wsub_id);
...
extern void srchcm_interf_release_cell_for_meas(
  ...
  srchcm_interf_cell_struct_type *cell_ptr, wsub_id_e_type wsub_id);
...
extern boolean srchcm_interf_cleanup_interf_meas(
  ...
  SRCHCM_INTERF_SEARCH_DONE_CB_FUNC *cb_func, wsub_id_e_type wsub_id);

#ifdef FEATURE_WCDMA_TABASCO_MODEM
/*===========================================================================
FUNCTION srchcm_meas_get_gap_action_time_for_inter_f_srch
...
SIDE EFFECTS
=============================================================================*/
boolean srchcm_query_w2x_dch_fach_meas_ongoing(wsub_id_e_type wsub_id);
...
SIDE EFFECTS
=============================================================================*/
boolean srchcm_query_w2w_detected_cell_condition(wsub_id_e_type wsub_id);
...
extern void srchcm_interf_update_info( wsub_id_e_type wsub_id);
...
extern boolean srchcm_meas_remove_interf_cells(
  ...
  uint8 num_cells,
  /* scermabling code list */
  srchcm_interf_cell_struct_type *del_cell_ptr[], wsub_id_e_type wsub_id);
...
extern void srchcm_meas_remove_all_interf_cells( wsub_id_e_type wsub_id);
...
extern boolean srchcm_meas_adjust_reconf_period_for_ttt(
  ...
  /* If any_TTT_active is TRUE, this should be the smallest TTT value of those
     active TTTs (in ms) */
  uint16 ttt_min, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION SRCHCM_INTERF_GAP_END_TUNE_DONE

DESCRIPTION
  This function is a handler to handle when interF CM gap closed and tune back to
  HOME freq from MDSP. 
...
SIDE EFFECTS
  None.
===========================================================================*/
extern boolean srchcm_interf_gap_end_tune_done(uint8 evt_id, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION srchcm_waiting_for_gap_end

DESCRIPTION
  This function checks if srchfach_unreserve and AGC tune done have been 
...
RETURN VALUE
  FALSE - Done with gap. 
  TRUE - Still waiting for either srchfach_unreserve or AGC tune done at the end of the gap

SIDE EFFECTS
  None.
===========================================================================*/
extern boolean srchcm_waiting_for_gap_end (wsub_id_e_type wsub_id);
extern void srchcm_clear_fach_events_received (wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern srchgsm_meas_info_struct_type* srchcm_gsm_get_meas_info(
...
extern void srchcm_gsm_update_adv_meas_info(
  ...
  srchgsm_gap_param_gsm_struct_type *gap_param_for_gsm, wsub_id_e_type wsub_id);
...
extern boolean srchcm_meas_trigger_gsm_meas(
  ...
  /* Frozen state value for W2G log packets */
  boolean cm_freeze,
  /* Suspend state value for W2G log packets */
  boolean cm_suspend, wsub_id_e_type wsub_id);
...
extern void srchcm_meas_gsm_meas_gap_unusable(
  ...
  /* Gap length in slots for logging */
  uint8 gap_length,
  /* Gap start slot number */
  uint8 gap_start_slot_num,
  /* GSP measure purpose */
  uint8 meas_purpose, wsub_id_e_type wsub_id);


/*===========================================================================
FUNCTION srchgsm_update_rssi_num_cmd_sent
         srchgsm_fcch_acq_result_set_acq_start_cmd_sent
         srchgsm_fcch_acq_result_set_acq_stop_cmd_sent
         srchgsm_update_sch_decode_cmd_sent
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srchgsm_update_rssi_num_cmd_sent(wsub_id_e_type wsub_id);
extern void srchgsm_fcch_acq_result_set_acq_start_cmd_sent(wsub_id_e_type wsub_id);
extern void srchgsm_fcch_acq_result_set_acq_stop_cmd_sent(wsub_id_e_type wsub_id);
extern void srchgsm_update_sch_decode_cmd_sent(wsub_id_e_type wsub_id);
extern void srchgsm_meas_cleanup_cmd_cb_0( wsub_id_e_type wsub_id);
extern void srchgsm_meas_cleanup_cmd_cb_1( wsub_id_e_type wsub_id);
...
extern void srchcm_do_gsm_meas(
  ...
  uint8 cfn,
  /* GSM measurement purpose */
  srchgsm_meas_purpose_enum_type meas_purpose, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srch_meas_dummy_process_rssi_result( wsub_id_e_type wsub_id);
...
extern srch_gsm_search_trigger_enum_type srchcm_meas_trigger_gsm_rssi_meas(
  ...
  uint32 dirat, wsub_id_e_type wsub_id);
...
extern void srchcm_meas_update_rssi_meas_prd(
  ...
  boolean rssi_tgmp_exist, wsub_id_e_type wsub_id);
...
extern void srchcm_meas_process_rssi_meas_result( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srchcm_meas_sort_rssi_update_bsic_ver_list( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srchcm_gsm_check_reset_bsic_id_abort_cell_list( wsub_id_e_type wsub_id);
...
extern void srchcm_gsm_update_bsic_ver_list( wsub_id_e_type wsub_id);
...
extern void srchcm_gsm_update_bsic_ver_status(
  ...
  boolean bsic_ver_reqd, wsub_id_e_type wsub_id);
...
extern void srchcm_meas_bsic_id_abort_cell(
  ...
  srchcm_gsm_cell_struct_type *cell_ptr, wsub_id_e_type wsub_id);
...
extern uint8 srchcm_meas_query_known_cell_qs_offset(
  ...
  uint16 *qs_offset, wsub_id_e_type wsub_id);
...
extern void srchcm_meas_update_gsm_bsic_id_abort_count(
  /* BSIC id abort count */
  uint16 num_gaps_bsic_id_abort, wsub_id_e_type wsub_id);
...
extern void srchcm_get_previous_forbidden_offset 
(
  uint8 *frame_offset, 
  uint16 *qs_offset, 
  uint8 idx
...
extern void srchcm_meas_poll_rssi_meas_result(
  ...
  uint8 cur_gfn, wsub_id_e_type wsub_id);
...
extern boolean srchcm_meas_update_bsic_reconf_result_after_sch_decode(
  ...
  srchcm_gsm_cell_struct_type *cell_ptr,
  /* SCH decode result */
  boolean sch_decode_result, wsub_id_e_type wsub_id);
...
extern void srchcm_meas_gsm_bsic_reconf_abort_timer_update(
  ...
  uint16 num_wcdma_frame_reconf_abort, wsub_id_e_type wsub_id);
...
extern void srchcm_meas_force_reconf_abort(
  ...
  boolean mark_last_qs_forbiden, wsub_id_e_type wsub_id);
...
extern srch_gsm_search_trigger_enum_type srchcm_meas_trigger_gsm_bsic_reconf_meas(
  /* GSM meas info struct pointer */
  srchgsm_meas_info_struct_type *meas_info, wsub_id_e_type wsub_id);
...
extern boolean srchcm_meas_gsm_sch_decode_possible(
  /* SCH decode GFN */
  uint8 sch_decode_frame_num, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srchcm_gsm_cmd_cleanup_done(uint8 num_cmd_pending, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srchcm_gsm_cmd_pre_cleanup( wsub_id_e_type wsub_id);
...
extern boolean srchcm_cleanup_gsm_pending_meas(
  ...
  SRCHCM_GSM_CMD_CLEANUP_DONE_CB_FUNC_TYPE* cb_func, wsub_id_e_type wsub_id);
...
extern void srchcm_gsm_add_cell_to_ver_list(
  ...
  /* indicate if the cell is added based on the RSSI it is added
     at the end of list */
  boolean sort_by_rssi, wsub_id_e_type wsub_id);
...
extern void srchcm_log_info_update_submit( wsub_id_e_type wsub_id);
...
extern srchcm_inter_rat_cell_struct_type* srchcm_get_empty_inter_rat_cell( wsub_id_e_type wsub_id);
...
extern boolean srchcm_put_inter_rat_cell_to_set(
  ...
  srch_inter_rat_cell_type_enum_type set_type, wsub_id_e_type wsub_id);
...
extern void srchcm_gsm_trim_arfcn_sample_tail_to_max_age(
  ...
  uint16 max_age, wsub_id_e_type wsub_id);
...
extern boolean srchcm_gsm_get_arfcn_rssi(
  ...
  boolean ignore_min_requirement, wsub_id_e_type wsub_id);
...
extern boolean srchcm_gsm_proceed_arfcn_rssi_meas_processing(
  ...
  uint16 *min_oldest_rssi_sample_age, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srchcm_meas_init_drv_status( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srchcm_meas_reset_drv_status( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean srchcm_meas_allocate_w2w_rf_buf( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srchcm_gsm_meas_init( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srchcm_init_cm_irat_drv_on_resume( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean srchcm_get_interf_srch_in_progress(wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean srchcm_get_rssi_meas_in_progress(wsub_id_e_type wsub_id);
#endif
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void srchcm_report_w2w_interf_meas_info_to_cxm(uint16 uarfcn, boolean is_cleanup, uint8 index, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean srchcm_is_step1_srch_suspended(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION srchcm_get_bsic_ver_status_update_pending

DESCRIPTION
  This is a utility function to return the flag that indicates if 
  bsic verification update is pending.
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean srchcm_get_bsic_ver_status_update_pending(wsub_id_e_type wsub_id);

#if defined(FEATURE_WCDMA_ATLAS_MODEM) && defined(FEATURE_WCDMA_RFM_PATH_SUPPORTED)
/*===========================================================================
FUNCTION     SRCHCM_GET_ALLOCATED_RFM_PATH
...
SIDE EFFECTS
  None.
===========================================================================*/
extern rfm_path_type srchcm_get_allocated_rfm_path(wsub_id_e_type wsub_id);
#else  /* (FEATURE_WCDMA_ATLAS_MODEM) && (FEATURE_WCDMA_RFM_PATH_SUPPORTED) */
/*===========================================================================
FUNCTION     SRCHCM_GET_ALLOCATED_DEVICE_ID
...
SIDE EFFECTS
  None.
===========================================================================*/
extern rfm_device_enum_type srchcm_get_allocated_device_id(wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srchcm_get_irat_interf_info(void *debug_buf, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srchcm_get_cm_gsm_meas_info(void *debug_buf, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION srchcm_query_w2w_meas_ongoing

DESCRIPTION
  This function is to determine if W2W meas is in the progress
  for the specified call process state: DCH or FMO
...
SIDE EFFECTS
=============================================================================*/
extern boolean srchcm_query_w2w_meas_ongoing(boolean wl1_in_dch, wsub_id_e_type wsub_id);

#endif /* FEATURE_WCDMA_QSH_DUMP*/

/*===========================================================================
FUNCTION srchcm_w2g_fcch_logging_get_curr_gsm_idx

DESCRIPTION
  This function is to retreive current gsm index value used for fcch result structure
...
SIDE EFFECTS
=============================================================================*/
extern uint8 srchcm_w2g_fcch_logging_get_curr_gsm_idx(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION srchcm_w2g_get_gsm_result_pending_idx

DESCRIPTION
  This function is to retreive pending gsm index value used for logging
...
SIDE EFFECTS
=============================================================================*/
extern uint8 srchcm_w2g_get_gsm_result_pending_idx(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION enqueue_meas_result_idx

DESCRIPTION
  This function is to enqueue meas result index value used for logging
...
SIDE EFFECTS
=============================================================================*/
extern void enqueue_meas_result_idx(uint8 buf_index, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION dequeue_meas_result_idx

DESCRIPTION
  This function is to dequeue meas result index value used for logging
...
SIDE EFFECTS
=============================================================================*/
extern void dequeue_meas_result_idx( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION srchcm_get_rssi_meas_age

DESCRIPTION
  This function is to retreive rssi mea age value used for W2G logging
...
SIDE EFFECTS
=============================================================================*/
extern uint32 srchcm_get_rssi_meas_age(wsub_id_e_type wsub_id);

#ifdef FEATURE_QSH_MDUMP
/*===========================================================================
FUNCTION wl1_qsh_mdump_collect_irat_srchcm

DESCRIPTION
  This is a minidump function for dumping the selected IRAT global variables for W2W and W2G 
  connected mode.
...
SIDE EFFECTS
  None
===========================================================================*/
extern QSH_MDUMP_FN_ATTR void wl1_qsh_mdump_collect_irat_srchcm(wsub_id_e_type wsub_id);
#endif
