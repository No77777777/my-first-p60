...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/eulsg.h#3 $
...
03/23/16    raj     Reduce Q6 scaling inactivity threshold for EUL 10ms TTI to 6 TTIs
03/14/16    raj     Fix to log ORD after SCCH order received in 0x4344 packet
10/16/15    ssv     QTA decision being made sure to be exclusiive and as per current hsrach state
09/14/15    gp      Fix for ETFCI sel reason logged as power restricted during TTI reconfig
...
08/04/15    yw      Trigger SI after QTA/LTA (L1 part) 
07/22/15    tsk     Save and restore activated HARQ Bitmask after HHO/LTA.
07/14/15    vr      Save CMD_STATE and CMD_STATUS for R99, C0 & C1.
07/15/15    cjz     is_scheduled_tx needs to consider next_tti_dtx
06/30/15    tsk     FR 26237: DSDS with Rel.8 E-FACH HSRACH + MSIM Changes.
06/09/15    cjz     Adjust 0x422C log packet length and featurization
05/29/15    ssv     Removing clock speed from profile buffer due to performance impact
05/20/15    ar      No 0xFF as invalid buffer index: sg_cur_page_idx and sg_ord_page_idx 
02/27/15    ap      Initial check-in for WL1 QSH framework 
03/28/15    vr      Change to fire EUL ISR from VSTMR context
03/05/15    vr      API to L2 indicating ratio between DPCCH to EUL power.
02/19/15    vr      Including clock speed in profile buffer.
...
typedef enum
{
  ...
  ENC_ERAMA_DATA_END,
  ENC_CMD_STATE_STATUS_0,
  ENC_CMD_STATE_STATUS_1,
  ...
  EUL_CMD_STATE_STATUS_C0,
  EUL_CMD_STATE_STATUS_C1,
...
typedef struct{
  ...
  uint8  eul_cmd_state_status[WL1_MAX_UL_CARR_IDX];
...
extern void eulsg_timeprofile_alloc_heapmem( wsub_id_e_type wsub_id);
extern void eulsg_timeprofile_dealloc_heapmem( wsub_id_e_type wsub_id);
extern void eulsg_capture_timing_info(tx_profile_enum_type
            tx_profile_type, tx_status_enum_type tx_profile_pt, wsub_id_e_type wsub_id);

#if defined (FEATURE_WCDMA_BOLT_2_0) && !defined (FEATURE_WCDMA_JOLOKIA_MODEM)
#define GET_ENC_EUL_CMD_STATE_STATUS(cmd_type)                   \
  (((mcalwcdma_wenc_get_enc_state(cmd_type, wsub_id)      & 0x03) << 0) | \
   ((mcalwcdma_wenc_get_enc_cmd_status(cmd_type, wsub_id) & 0x3F) << 2))
#endif /* (FEATURE_WCDMA_BOLT_2_0) && !defined (FEATURE_WCDMA_JOLOKIA_MODEM) */
...
#define EUL_SG_SET_ACT_TIME(act_time, carr_idx) \
          eulsg_ext_cntrl_params.eul_sg_recfg_act_time[carr_idx] = act_time
...
#define EUL_CPC_DTX_SET_CURR_HS_SCCH_ORDER(order) \
	eulcfg_ext_persub_cntrl_params[wsub_id].e_ul_cpc_dtx_scch_ord_bkup_rstr_info.current_hs_scch_order = order
#endif
...
#define EULSG_SET_HS_RACH_DPCCH_ONLY_TX_CNT(val)  eulsg_ext_cntrl_params.eul_sg_hs_rach_dpcch_only_tx_cnt = val
#define EULSG_GET_HS_RACH_DPCCH_ONLY_TX_CNT()     eulsg_ext_cntrl_params.eul_sg_hs_rach_dpcch_only_tx_cnt
#define EULSG_SET_HS_RACH_HARQ_TX_BMSK(harq_id)   eulsg_ext_cntrl_params.eul_sg_hs_rach_harq_tx_bmsk |= (uint8)(1 << harq_id)
#define EULSG_RESET_HS_RACH_HARQ_TX_BMSK(harq_id) eulsg_ext_cntrl_params.eul_sg_hs_rach_harq_tx_bmsk &= ~((uint8)(1 << harq_id))
#define EULSG_GET_HS_RACH_HARQ_TX_BMSK()  (eulsg_ext_cntrl_params.eul_sg_hs_rach_harq_tx_bmsk)
#define EULSG_INIT_HS_RACH_HARQ_TX_BMSK() (eulsg_ext_cntrl_params.eul_sg_hs_rach_harq_tx_bmsk = 0)
#define EUL_SG_HS_RACH_GET_EUL_TX_STATE() (eulsg_ext_cntrl_params.eul_sg_hs_rach_eul_tx_state)
#define EUL_SG_HS_RACH_SET_EUL_TX_STATE(var) (eulsg_ext_cntrl_params.eul_sg_hs_rach_eul_tx_state = var)
...
#ifdef FEATURE_WCDMA_DC_HSUPA
#define EULEPDCH_SKIP_PDU_BUILD_DURING_INTERRUPTION() \
        ((ulcmd_ext_cntrl_params.eul_skip_pdu_build.num_pdu_built_before_interruption == 0) && \
         (ulcmd_ext_cntrl_params.eul_skip_pdu_build.num_subfn_till_interrption_ends > 0))
#endif
...
typedef enum
{
   /* HARQ Transmitted and got an ACK */
   HARQ_TX_AND_ACK,
   /* HARQ Transmitted and got a NAK */
   HARQ_TX_AND_NAK,
   /* HARQ NOT Transmitted and HICH is marked as NAK  */
   HARQ_DTX_AND_NAK
}eul_sg_harq_hist_bef_qta_enum_type;
#endif /* FEATURE_QTA */
...
typedef enum
{
  /* the timer is not active  for starting the timer outside the FIQ handler,
  it should be in NEEDS_ACTIVATION state*/
  EUL_SG_TIMER_INACTIVE,
  /*There is a request within sg module to start the timer.So activate timer*/
  EUL_SG_TIMER_NEEDS_ACTIVATION,
  /*timer is in active state. if we need to restart within FIQ handler then
  we simply put the timer in state ACTIVE */
  EUL_SG_TIMER_ACTIVE

}eul_sg_timer_state_enum_type;
...
typedef struct {
  e_ul_cpc_dtx_state_enum_type state;
  uint16 start_subfn;
  uint16 num_subfns;
}e_ul_cpc_dtx_cm_cm_fill_calc_info_struct_type;

typedef struct
{
  /* The cfn # at which timer is started*/
  uint32 timer_count;
  /* the timer status*/
  eul_sg_timer_state_enum_type timer_state;

}eul_sg_timer_struct_type;

#endif /*FEATURE_WCDMA_CPC_DRX*/

typedef struct
{
  uint8 eul_q6_cntrl_inactivity_count_threshold[NUM_E_TTI];
  uint8 eul_q6_cntrl_inactivity_count;
} eul_q6_cntrl_struct_type;

typedef enum
{
  EULSG_Q6_CNTRL_INACT_CNT_THRESH_0,
  EULSG_Q6_CNTRL_INACT_CNT_THRESH_1,
  EULSG_Q6_CNTRL_INACT_CNT_THRESH_2,
  EULSG_Q6_CNTRL_INACT_CNT_THRESH_3,
  EULSG_Q6_CNTRL_INACT_CNT_THRESH_4,
  EULSG_Q6_CNTRL_INACT_CNT_THRESH_5,
  EULSG_Q6_CNTRL_INACT_CNT_THRESH_6,
  EULSG_Q6_CNTRL_INACT_CNT_THRESH_7,
  EULSG_Q6_CNTRL_INACT_CNT_THRESH_8,
  EULSG_Q6_CNTRL_INACT_CNT_THRESH_9,
  EULSG_Q6_CNTRL_INACT_CNT_THRESH_10,
  EULSG_Q6_CNTRL_INACT_CNT_THRESH_11,
  EULSG_Q6_CNTRL_INACT_CNT_THRESH_12,
  EULSG_Q6_CNTRL_INACT_CNT_THRESH_13,
  EULSG_Q6_CNTRL_INACT_CNT_THRESH_14,
  EULSG_Q6_CNTRL_INACT_CNT_THRESH_15,
  EULSG_Q6_CNTRL_INACT_CNT_THRESH_MAX
}eulsg_q6_cntrl_inact_cnt_thresholds_e_type;

typedef enum
{
  EULSG_Q6_SPEED_HIGH,
  EULSG_Q6_SPEED_EARLY_TRANSITION_UP,
  EULSG_Q6_SPEED_TRANSITION_UP,
  EULSG_Q6_SPEED_LOW,
  EULSG_Q6_SPEED_TRANSITION_DOWN,
  EULSG_Q6_SPEED_NUM_SPEEDS
}eulsg_q6_speed_e_type;

typedef struct
{
  /* double buffer data for maintaining the serving grant information */
  eul_sg_cfg_db_struct_type eul_sg_cfg_info[WL1_MAX_UL_CARR_IDX][EUL_MAX_CFG_DB_COUNT];
  /*the input structure required for getting power offset of the highest priority
  logical channel carrying data and  SI info for the transmission
  on the same HARQ  */
  l1_eul_pwr_offset_req_struct_type eul_pwr_offset_si_req_info[WL1_MAX_UL_CARR_IDX];

  eul_dl_per_target_sub_fn_ns_rg_result_type ns_rgch_results[WL1_MAX_UL_CARR_IDX][EUL_MAX_NON_SRV_RGCH_RESULT_BUF_LEN];
  eul_dl_per_target_sub_fn_result_type       agch_serv_rgch_hich_results[WL1_MAX_UL_CARR_IDX][EUL_MAX_AGCH_RESULT_BUF_LEN];
  eul_sg_applied_dl_results_struct_type      applied_dl_results[WL1_MAX_UL_CARR_IDX];

  /* Serving_Grant indicates the maximum E-DPDCH to DPCCH power ratio that the UE
  ** is allowed to use for scheduled data in the following transmission. The value
  ** in the appropriate state variable will be provided to the E-TFC selection
  ** function to help in selecting the best format for the upcoming transmission.
  ** Possible values are: given in the SG table. */
  uint32 available_sg_idx[WL1_MAX_UL_CARR_IDX];
  /* variable used as the initial SG of secondary carrier once it is activated */
  uint32 sec_init_sg_idx;
  /* The cur page always points to the sg data that is being used. When the ord
  ** page is valid,serving grant module switch to the information in ord page in
  ** the sub fn mentioned in the ord page. */
  uint32 sg_cur_page_idx[WL1_MAX_UL_CARR_IDX];

  /*  Represents if current TTI was DTX'ed due to L1/MAC state*/
  boolean dtx_due_to_inactive_edpch_flag;
  
  boolean sg_cur_page_idx_valid[WL1_MAX_UL_CARR_IDX];  

  boolean e_ul_cm_func_called;
  /* Boolean indicating that the LOW GRANT activation is because of CM */
  boolean eul_allow_etfci_sel_pdu_build_in_low_grant;
  uint16  hsrach_agch_decode_result;
  /* This global value holds the activation time for the SG information during
  reconfig/start */
  uint16  eul_sg_recfg_act_time[WL1_MAX_UL_CARR_IDX];

  /* The following variables contain the CFN and subFN values and should be
  ** accessed only inside the EUL FIQ */
  uint8   eul_fiq_global_cur_fn;
  uint16  eul_fiq_global_cur_subfn;
  boolean e_ul_dtx_invalid_subfn;
  uint32  cur_harq_process_id;
  /* Counter to ensure processing sequence between FIQ1 and FIQ2 remains consistent. This count
  ** takes a discrete set of values and incrementing this counter is not intuitive. This global variable
  ** eul_irq1_irq2_count can take values 0, 1 and 2 ONLY*/
  uint8   eul_irq1_irq2_count;
  eul_irq_cycle_enum_type eul_irq_cycle;
  uint16 eul_pre_fiq_delay_cnt;
  ...
  ** can later be used to fill information in EUL log packets*/
  eul_logging_dl_results_struct_type eul_logging_dl_results[WL1_MAX_UL_CARR_IDX];
  eul_logging_agch_info_struct_type  eul_logging_agch_info[WL1_MAX_UL_CARR_IDX][EUL_MAX_AGCH_RESULT_BUF_LEN];

  #ifdef FEATURE_WCDMA_CPC_DRX
  eul_edch_edldrx_params_struct_type eul_edch_edldrx_params;
  #endif

  #ifdef FEATURE_WCDMA_CPC_DTX
  uint16 e_ul_cpc_dtx_cm_start_slot_burst[WL1_MAX_UL_CARR_IDX][2];
  /* array to store the start slots and number of DPCCH burst + pre+postamble slots
  ** for the next frame. Only for case when there are <= two bursts in a frame.   
  ** Burst is there every subframe when cycle length is 1 subframe. */
  uint16 e_ul_cpc_dtx_cm_num_slots_burst[WL1_MAX_UL_CARR_IDX][2];
  eul_cpc_dtx_per_tti_upd_params_struct_type eul_cpc_dtx_per_tti_upd_params[WL1_MAX_UL_CARR_IDX];

  /*************Logging code for CPC DTX FW info*****************/
  CPC_DTX_FW_STATE_MACHINE_LOG_PKT_type cpc_dtx_fw_log_pkt;
  #endif

  #ifdef FEATURE_WCDMA_BOLT_2_0
  /* The Pre-FIQ firing times indexed by the previous Post FIQ Sub Fn (within a frame) */
  uint8 eulsg_pre_fiq_firing_times_array[5];
  /* The Post-FIQ firing times indexed by the previous Pre FIQ Sub Fn  (within a frame) */
  uint8 eulsg_post_fiq_firing_times_array[5];
  #endif

  #ifdef FEATURE_WCDMA_HS_RACH
  eul_sg_hs_rach_eul_tx_enum_type eul_sg_hs_rach_eul_tx_state;
  uint16 eul_sg_hs_rach_dpcch_only_tx_cnt;
  uint8  eul_sg_hs_rach_harq_tx_bmsk;
  #endif

  #ifdef FEATURE_WCDMA_DC_HSUPA 
  l1_eul_pwr_offset_sec_ul_non_sg_struct_type eul_pwr_offset_sec_ul_non_sg;
  #endif

  /* Current state of Q6 clock for HSUPA processing */
  eulsg_q6_speed_e_type eulsg_q6_speed_state;
  /* Mutex for Q6 clock speed change processing */
  rex_crit_sect_type eulsg_q6_speed_state_mutex;

  #ifdef FEATURE_WCDMA_EFACH_MSIM
  uint8  eul_sg_hs_rach_si_only_tx_cnt;
  uint8 rel_cause_postponed;
  boolean e_ul_rel_resource_once;
  boolean qta_force_reserve;
  #endif /* FEATURE_WCDMA_EFACH_MSIM */
} eulsg_ext_cntrl_params_type;

extern eulsg_ext_cntrl_params_type eulsg_ext_cntrl_params;

typedef struct
{
  e_ul_sg_e_dl_res_return_params_struct_type e_ul_sg_e_dl_res_sg_call_mac_if_func[WL1_MAX_UL_CARR_IDX];
  /*maintain the above info for each harq */
  eul_sg_harq_param_struct_type     eul_sg_harq_params[WL1_MAX_UL_CARR_IDX][EUL_MAX_HARQ_RTT];
  /* structure used to send the serving grant information to E-DCH module */
  eul_edch_harq_sg_info_struct_type edch_harq_sg_info[WL1_MAX_UL_CARR_IDX];

  eul_sg_timer_struct_type non_srv_rg_timer[WL1_MAX_UL_CARR_IDX];
  eul_sg_timer_struct_type ag_timer[WL1_MAX_UL_CARR_IDX];

  eul_logging_data_struct_type eul_logging_data[WL1_MAX_UL_CARR_IDX];
  uint32 flush_hich_index;
  /* log_sfn will hold current sub fn value for 2ms and current CFN value for 10ms */
  uint16 log_sfn;
  uint8  local_sub_fn_val;

  debug_eul_isr_timing_struct_type *debug_eul_isr_timing_ptrs[MAX_EUL_SZ_TIMING_ARRAY];
  debug_r99_isr_timing_struct_type *debug_r99_isr_timing_ptrs[MAX_R99_SZ_TIMING_ARRAY];
  uint32  e_ul_irq_split_second_fiq_overshot;
  boolean timeprofile_heapmem_allocated;
  uint16  debug_r99_timing_idx;
  uint16  debug_eul_timing_idx;
  uint32  eul_irq1_starting_time;

  /* used for not triggering SI after getting AG and used for field performance */
  uint32  previous_sg_idx[WL1_MAX_UL_CARR_IDX];
  uint32  sg_ord_page_idx[WL1_MAX_UL_CARR_IDX];
  boolean sg_ord_page_idx_valid[WL1_MAX_UL_CARR_IDX];
  uint32  max_available_sg_idx[2];
  uint32  stored_secondary_grant_idx[WL1_MAX_UL_CARR_IDX];

  /* Variable indicating whether the UE's serving grant is only affected by Primary
  ** Absolute Grants and Relative Grants (i.e. not by Secondary Absolute Grants). */
  boolean primary_grant_available[WL1_MAX_UL_CARR_IDX];
  /* FEATURE_WCDMA_REL8 */

  /* flag to know that the serving rg is configured */
  boolean serving_rgch_cfged[WL1_MAX_UL_CARR_IDX];
  /* Set the global variable that the MAC HARQ PO function was called*/
  boolean e_ul_call_harq_po_func_in_second_fiq_ind;
  
  /* QTA end indicator flag */
  boolean eul_qta_end_ind_flag;

  #ifdef FEATURE_WCDMA_CPC_DTX
  uint16  e_ul_cpc_dtx_cm_curr_burst_slots_cont_from_last_fr_burst[2];
  /* Checks whether PDU is built for this TTI */
  boolean eul_cpc_dtx_tx_prev_tti[WL1_MAX_UL_CARR_IDX];
  /* Variable to track if the default SG is already applied or still needs to be applied
  ** Serving Grant value has to applied as soon as we enter the DTX cycle 2 so set this to TRUE once it enters cycle 1*/
  eul_sg_cpc_dtx_default_sg_apply_enum_type default_sg_available[2];
  uint8 fw_scch_dtx_order_save;
  /* The variable tx_curr_tti_ota will hold the status of what is happening on the 
  ** OTA this very current TTI. This is different from the variable passed to this function
  ** That is the Transmission for the upcoming TTI*/
  boolean tx_curr_tti_ota[2];
  /* For debugging */
  eul_cpc_dtx_fw_state_log_struct_type *dtx_fw_log_pkt_sample_ptr;
  /* Considering present frame, this var will hold subfn in the next radio frame where EDCH tx can possibly happen. */
  uint16 e_ul_cpc_dtx_subfn_edch_tx_nrf[WL1_MAX_UL_CARR_IDX];
  uint16 cpc_dtx_tx_protect_mask;
  /* This global variable stores the serving grant value at which the UE declared LOW GRANT
  **. This value is needed because using this the L1 SW will predict if the updated eulsg_ext_cntrl_params.available_sg_idx
  ** is updated to a value greater than the stored value and thereby makes a trial to exit from the 
  ** LOW GRANT state. This global is only used for CPC DTX implementation*/
  uint16 avail_sg_during_low_grant[2];
  uint16 e_ul_cpc_dtx_sw_fw_intf_cntr_for_hs_scch_ord;
  e_ul_cpc_dtx_eul_isr_params_struct_type e_ul_cpc_dtx_data_pre_fiq_to_fiq[MAX_NUM_UL_CARR];
  /*number of DPCCH burst or postamble slots in next frame that gets extended to next next frame */
  uint16 e_ul_cpc_dtx_cm_nrf_burst_ext_to_nnrf_slots[2];
  #endif /* FEATURE_WCDMA_CPC_DTX */

  #if defined(FEATURE_WCDMA_CPC_DTX) && defined(FEATURE_WCDMA_MULTISIM_COEXISTENCE_SW)
  /* The below flag is used internally to decide whether to make tx priorty  high or not for given CFN or not */
  boolean e_ul_cpc_dtx_mac_inact_timer_zero[WL1_MAX_UL_CARR_IDX];
  /* Below 3 are variables for timing calculations in bpg  */
  uint32  e_ul_cpc_dtx_starting_time_bpg;
  /* A counter to count missed protections  */
  uint32  e_ul_cpc_dtx_missed_protection;
  #endif /* defined(FEATURE_WCDMA_CPC_DTX) && defined(FEATURE_WCDMA_MULTISIM_COEXISTENCE_SW) */

  #if defined(FEATURE_DUAL_SIM) && defined(FEATURE_QTA)
  eul_sg_harq_hist_bef_qta_enum_type harq_acked_bef_qta[WL1_MAX_UL_CARR_IDX][EUL_HARQ_RTT_2MS];
  #endif /* defined(FEATURE_DUAL_SIM) && defined(FEATURE_QTA) */

  #if defined(FEATURE_WCDMA_DC_HSUPA) && defined(FEATURE_WCDMA_EVT6D_ENH)
  boolean isDpcchGrtThanEul;
  #endif /* FEATURE_WCDMA_DC_HSUPA && FEATURE_WCDMA_EVT6D_ENH */

  #if defined(FEATURE_DUAL_SIM)
  /*A flag to indicate if SI should be trigger after LTA*/
  boolean trigger_si_in_next_tti_lta[WL1_MAX_UL_CARR_IDX];

  /*A flag to indicate if SI should be trigger after QTA*/
  boolean trigger_si_in_next_tti_qta;
  #endif /* FEATURE_DUAL_SIM */
  eul_q6_cntrl_struct_type eul_q6_cntrl_db;
} eulsg_int_cntrl_params_type;

#define EUL_Q6_SPEED_STATE_MUTEX_LOCK()   REX_ISR_LOCK(&eulsg_ext_cntrl_params.eulsg_q6_speed_state_mutex)
#define EUL_Q6_SPEED_STATE_MUTEX_UNLOCK() REX_ISR_UNLOCK(&eulsg_ext_cntrl_params.eulsg_q6_speed_state_mutex)


typedef struct {
  /* Below saved globals are for HHO, reverting, and etc */
  uint32  eul_sg_saved_ref_etpr[WL1_MAX_UL_CARR_IDX][EUL_MAX_HARQ_RTT];
  boolean eul_sg_saved_grant_selection[WL1_MAX_UL_CARR_IDX];
  uint32  eul_sg_saved_sg_to_resume[WL1_MAX_UL_CARR_IDX];
  boolean eul_sg_saved_secondary_grant_idx[WL1_MAX_UL_CARR_IDX];
  #ifdef FEATURE_WCDMA_REL8
  uint32 eul_sg_saved_ref_etpr2[WL1_MAX_UL_CARR_IDX][EUL_MAX_HARQ_RTT];
  #endif /* FEATURE_WCDMA_REL8 */
  uint8 harq_process_saved_state[WL1_MAX_UL_CARR_IDX][EUL_MAX_HARQ_RTT];
} eulsg_int_persub_cntrl_params_type;

#ifdef FEATURE_WCDMA_CPC_DRX
#define IS_NEXT_TTI_SCHEDULED_TX(carr_idx) (eulsg_ext_cntrl_params.eul_edch_edldrx_params.is_scheduled_tx[carr_idx])
#define IS_TEBS_NON_ZERO(carr_idx)         (eulsg_ext_cntrl_params.eul_edch_edldrx_params.is_tebs_non_zero[carr_idx])
#endif /*FEATURE_WCDMA_CPC_DRX*/
...
/*===========================================================================
FUNCTION        e_ul_get_max_num_retrans_allowed
...
SIDE EFFECTS
===========================================================================*/
uint32 e_ul_get_max_num_retrans_allowed(uint8 carr_idx, uint32 harq_id);
...
SIDE EFFECTS 
===========================================================================*/
void eul_hs_rach_preprocess_fiq(void);
#endif /*FEATURE_WCDMA_HS_RACH*/
...
===========================================================================*/
extern void  eul_sg_harq_module_init(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
extern void  eul_sg_harq_module_process_cfg_data
                  (eul_dl_sg_info_struct_type  *e_dl_sg_info_ptr, wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_dl_populate_dl_chan_results_for_next_tti( eul_sg_applied_dl_results_struct_type* applied_dl_results, wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
extern uint32 eul_sg_get_cur_sg_idx( wsub_id_e_type wsub_id);
...
SIDE EFFECTS  None
===========================================================================*/
extern l1_e_grant_config_info_struct_type eul_sg_get_cur_sg_cfg_info( wsub_id_e_type wsub_id);
...
extern boolean eul_sg_get_sg_available_status( wsub_id_e_type wsub_id);
...
SIDE EFFECTS  None
===========================================================================*/
extern void eul_sg_save_sg(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void eul_sg_flush_all_harq(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS  None
===========================================================================*/
extern void eul_save_state_info(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS  None
===========================================================================*/
extern void eul_restore_state_info(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS None
===========================================================================*/
eul_ernti_cfg_info_enum_type eul_sg_get_cur_ernti_info(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS  None
===========================================================================*/
uint8  eul_sg_get_cur_three_idx_step_thresh_info( wsub_id_e_type wsub_id);
...
SIDE EFFECTS  None
===========================================================================*/
uint8  eul_sg_get_cur_two_idx_step_thresh_info( wsub_id_e_type wsub_id);
...
SIDE EFFECTS None
===========================================================================*/
boolean eul_sg_is_serving_rg_cfged(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
void eul_sg_tti_recfg_done( wsub_id_e_type wsub_id);
...
extern boolean eul_sg_is_tti_recfg_in_progress( wsub_id_e_type wsub_id);
...
uint32 eul_sg_get_curr_harq_id( wsub_id_e_type wsub_id);
...
void e_ul_cpc_dtx_init_cm_globals(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS None
===========================================================================*/
extern void eul_dtx_send_fw_log_pkt_to_diag( wsub_id_e_type wsub_id);
...
SIDE EFFECTS None
===========================================================================*/
void eulsg_cpc_dtx_collect_FW_log_pkt_sample( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void e_ul_cpc_dtx_init( wsub_id_e_type wsub_id);
...
boolean e_ul_cpc_dtx_etfci_select_for_subfn(uint32 subfr,  wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
===========================================================================*/
boolean eulsg_check_and_ret_tx_subfn_for_cpc_dtx (uint16 *tx_subfn, wl1_ul_carr_id_enum_type carr_idx , wsub_id_e_type wsub_id);
...
===========================================================================*/
void e_ul_sg_process_scch_order( wsub_id_e_type wsub_id);
...
SIDE EFFECTS  
===========================================================================*/
extern void e_ul_sg_set_low_grant_sg_val(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
void eul_sg_hs_rach_proc( wsub_id_e_type wsub_id);
/*===========================================================================
...
===========================================================================*/
uint16 eul_sg_hs_rach_serv_grant_update_proc(uint16 eul_cur_fn, wsub_id_e_type wsub_id);
...
extern void e_dl_sec_tx_enable( wsub_id_e_type wsub_id);
...
extern void e_ul_sec_cpc_dtx_order_cfg_stop( wsub_id_e_type wsub_id);
...
extern void e_ul_sec_cpc_dtx_order_cfg_start(uint32 dtx_cfg_db_idx, wsub_id_e_type wsub_id);
...
/*===========================================================================
FUNCTION        EULSG_EXT_CNTRL_PARAMS_INIT
...
SIDE EFFECTS
===========================================================================*/
extern void eulsg_int_cntrl_params_init(wl1_ul_carr_id_enum_type carr_idx, wsub_id_e_type wsub_id);


#ifdef FEATURE_WCDMA_QSH_DUMP
/*===========================================================================
FUNCTION     e_ul_sg_get_eul_timing_debug_idx

DESCRIPTION
  Returns the debug_eul_timing_idx
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint16 e_ul_sg_get_eul_timing_debug_idx(void);
#endif /* FEATURE_WCDMA_QSH_DUMP*/
/*===========================================================================
FUNCTION        eul_set_q6_cntrl_inact_thresh

DESCRIPTION     This function sets the inactivity threshold for reduceing the 
                Q6 clock speed
...
SIDE EFFECTS    Will update the inactivity threshold global variable
===========================================================================*/
extern void eul_set_q6_cntrl_inact_thresh(uint8 inact_thresh, e_tti_enum_type tti, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION eul_update_aps_info

DESCRIPTION
 This function collects EUL stats which is then used to update EUL APS vote.
...
SIDE EFFECTS
  None
===========================================================================*/
extern void eul_update_aps_info(wsub_id_e_type wsub_id);
#ifdef FEATURE_QSH_MDUMP 
/*===========================================================================
FUNCTION     wl1_qsh_mdump_collect_eulsg_int_params

DESCRIPTION
  Dump EULSG INT CNTRL PARAMS
...
SIDE EFFECTS
  None
===========================================================================*/
QSH_MDUMP_FN_ATTR void wl1_qsh_mdump_collect_eulsg_int_params(wsub_id_e_type wsub_id);
#endif
#endif /* EULSG_H */