...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/mcalwcdma_dl.h#3 $
...
12/08/17    ta      API to compute the channel config time difference
                    and invalidate previous config WRC on starting WCDMA
08/16/17   ss     Changes for LTA timeline optimization
12/26/16    ta      Macro for checking if the Channel idx is valid
01/26/16   ap       Changes to update phch set cnt in mcal db  
...
#define MCALWCDMA_DL_NUM_MAX_SW_COMBINERS          (WFW_DEMOD_NUM_MAX_COMBINERS + 5)
#else
...
/*--------------------------------------------------------------------------
Macro for checking if the Channel idx is valid
--------------------------------------------------------------------------*/
#define MCALWCDMA_DL_IS_CH_IDX_INVALID(ch_idx)    \
    ((ch_idx == MCALWCDMA_DL_INVALID_CHAN_ID) || \
     (ch_idx >= MCALWCDMA_DL_NUM_MAX_SW_COMBINERS))
...
typedef struct {
  ...
  /*Indicates FW about the PI bits start position in stmr 
    time format.
   [12 bits frame no | 8 bits bpg no| 11 bits cx8]*/
  uint32 piStartTime;
#endif
...
/*============================================================================*/
/**
  * This function registers R99 chan cfg done per sub
  */
/*============================================================================*/
extern void mcalwcdma_dl_sub_specific_init(wsub_id_e_type wsub_id);
...
extern uint8 mcalwcdma_dl_phychan_add(
  ...
  mcalwcdma_dl_phch_cfg_parms_struct_type *phch_cfg_parms_ptr, wsub_id_e_type wsub_id);
...
void mcalwcdma_dl_phychan_cfg_done_ind(
  ...
  uint8 ccfn, wsub_id_e_type wsub_id);
...
void mcalwcdma_dl_phychan_slam_new_stmr_counter(
  ...
  /** current cfn */
  tlm_tfn_struct_type *wrc_cfn_diff, wsub_id_e_type wsub_id);

extern uint16 mcalwcdma_dl_cctrch_curr_time(uint8 cctrch_idx, wsub_id_e_type wsub_id);
...
uint8 mcalwcdma_dl_get_free_and_rsv_comb_ch( 
  ...
  uint8 slot_format_index, wsub_id_e_type wsub_id);
 ...
 */
/*============================================================================*/
void mcalwcdma_dl_phychan_comb_chan_event_handler(uint8 evt, wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None
===========================================================================*/
void mcalwcdma_dl_get_wrc_cfn_diff_for_combiner(uint8 hwch, tlm_tfn_struct_type* wrc_cfn_diff, tlm_cfn_struct_type* ch_config_cfn, wsub_id_e_type wsub_id);
...
*/
/*===========================================================================*/
extern void mdsp_sync_send_comb_and_phychan_cfg_cmd_post_cb(uint16 status, wsub_id_e_type wsub_id);
...
*/
/*===========================================================================*/
extern uint8 mcalwcdma_dl_allocate_interface_buf( wsub_id_e_type wsub_id);
...
*/
/*===========================================================================*/
extern boolean mcalwcdma_dl_deallocate_interface_buf(uint8 idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None
===========================================================================*/
extern uint8 mcalwcdma_dl_get_interface_buffer_id(uint8 channel_type, uint8 cell_idx, uint8 cctrch_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None
===========================================================================*/
extern mcalwcdma_dl_phch_cfg_cmd_struct_type* mcalwcdma_dl_get_interface_buffer_ptr(uint8 buffer_id, wsub_id_e_type wsub_id);
...
===========================================================================*/
void mcalwcdma_dl_log_submit(wsub_id_e_type wsub_id);
...
*/
/*===========================================================================*/
extern void mcalwcdma_dl_mdsp_post_cb(uint16 status, wsub_id_e_type wsub_id);
...
/*===========================================================================
FUNCTION mcalwcdma_dl_update_phychan_set_cnt

DESCRIPTION
  This function updates the num rls' in mcalwcdma_phychan_db
...
===========================================================================*/
extern void mcalwcdma_dl_update_phychan_set_cnt(uint8 comb_id, uint8 num_rl, wsub_id_e_type wsub_id);

/*============================================================================*/
/** This function is the interrupt handler for R99 Channel Configuration done notification */
/*============================================================================*/
void mcalwcdma_dl_r99_chan_drop_done_interrupt_handler( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION         MCALWCDMA_DL_COMPUTE_CHANNEL_CFG_TIME_DIFF

DESCRIPTION
  This function does the below actions,
  1) Computes channel config time difference if previous channel config time is valid
  2) Checks if the channel config commands on the same combiner are seperated by one frame

DEPENDENCIES
  None
  
RETURN VALUE 
  TRUE - if preious channel config time is valid and 
         channel config commands on the same combiner are seperated by one frame
  
SIDE EFFECTS
  None
  
===========================================================================*/
boolean mcalwcdma_dl_compute_chan_cfg_time_diff(uint8 ch_idx, tlm_tfn_struct_type curr_chan_cfg_time, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION         MCALWCDMA_DL_INVALIDATE_CHAN_CFN_INFO

DESCRIPTION
  This function initializes all channels config time info to invalid value  
...
===========================================================================*/
extern void mcalwcdma_dl_invalidate_chan_cfg_wrc(wsub_id_e_type wsub_id);

#ifdef FEATURE_QSH_MDUMP 
/*===========================================================================
FUNCTION         mcalwcdma_dl_qsh_mdump_collect

DESCRIPTION  To dump minidump related parameters
...
===========================================================================*/
QSH_MDUMP_FN_ATTR void mcalwcdma_dl_qsh_mdump_collect();

QSH_MDUMP_FN_ATTR void mcalwcdma_dl_qsh_mdump_collect_sub(wsub_id_e_type wsub_id);
#endif
