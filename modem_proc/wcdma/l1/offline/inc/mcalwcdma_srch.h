...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/inc/mcalwcdma_srch.h#4 $
$DateTime: 2020/03/09 06:07:26 $
$Author: pwbldsvc $

when        who     what, where, why
--------    ---     --------------------------------------------------------
09/03/20    rs      W2W Recovery: handling RF_SKIP  on queue basis.
01/10/20    ac      Mini dump framework checkin
12/11/19    ac      Searcher Mini Dump Change
02/18/16    vn      Fix to allocate alternate cm queues to avoid race condition
05/22/15    da      Setting srchUseFreezeFlag to 0 upon scheduler mode change to INTRAF in ACQ state
...
#define SRCH_INTLOCK()        REX_ISR_LOCK(&mcalwcdma_srch_ext_cntrl_params[wsub_id].wsrch_crit_sect)
#define SRCH_INTFREE()        REX_ISR_UNLOCK(&mcalwcdma_srch_ext_cntrl_params[wsub_id].wsrch_crit_sect)
...
typedef void (*mcalwcdma_srch_status_callback_fn_type) (mcalwcdma_srch_status_type status, srch_intraf_queue_enum_type srch_queue_idx, wsub_id_e_type wsub_id);
typedef void (*mcalwcdma_srch_cm_status_callback_fn_type) (mcalwcdma_srch_status_type status, uint8 queue_idx, wsub_id_e_type wsub_id);
...
#define MCAL_SRCH_Q0_SRCH_DONE_FlAG WFW_SW_Q0_SEARCH_DONE_FLAG
#define MCAL_SRCH_Q1_SRCH_DONE_FlAG WFW_SW_Q1_SEARCH_DONE_FLAG
#define MCAL_SRCH_GET_X2W_NASTT_PN_SRCH_DONE_FLAG WFW_SW_Q1_SEARCH_DONE_FLAG
#define MCAL_SRCH_GET_X2W_NASTT_PN_Q SRCH_NASTT_PN_Q
#else
#define MCAL_SRCH_Q0_SRCH_DONE_FlAG WFW_SW_SEARCH_DONE_FLAG
#define MCAL_SRCH_Q1_SRCH_DONE_FlAG WFW_SW_OVERLAPPED_SRCH_DONE_FLAG
#define MCAL_SRCH_GET_X2W_NASTT_PN_SRCH_DONE_FLAG WFW_SW_SEARCH_DONE_FLAG
#define MCAL_SRCH_GET_X2W_NASTT_PN_Q SRCH_S1_Q
#endif
...
#define SRCH_STEP1_SYNC_PARAMS_CLEAR() (wsrch_ext_cntrl_params[wsub_id].wsrch_ext_cntrl_params_mdump.srch_step1_sync_parms.carrierCount = 0)

#define SRCH_STEP1_SYNC_PARAMS_ADD_CARRIER(carr_idx) (wsrch_ext_cntrl_params[wsub_id].wsrch_ext_cntrl_params_mdump.srch_step1_sync_parms.carrierSel[wsrch_ext_cntrl_params[wsub_id].wsrch_ext_cntrl_params_mdump.srch_step1_sync_parms.carrierCount++] = carr_idx)

#define SRCH_STEP1_RESET_PEAK_IDX() {srchsched_set_cur_psc_peak_indx(CARR_0, 0 ,wsub_id); \
									 srchsched_set_cur_psc_peak_indx(CARR_1, 0,wsub_id);\
									 if(WL1_MAX_NUM_CARRIER_IDX==3) srchsched_set_cur_psc_peak_indx(CARR_2, 0,wsub_id);}
/* PN sync parms */
...
typedef enum
{
  MCALWCDMA_SRCH_INACTIVE,
  MCALWCDMA_SRCH_IN_PROGRESS
} mcalwcdma_srch_state_type;

typedef struct
{
  /* Current ID */
  mcalwcdma_srch_type cur_id;
  /* State of the queue */
  mcalwcdma_srch_state_type state;
} mcalwcdma_srch_state_db_struct_type;

typedef struct
{
  /* Current ID */
  mcalwcdma_srch_cm_type cur_id;
  /* State of the queue */
  mcalwcdma_srch_state_type state;
} mcalwcdma_srch_cm_state_db_struct_type;

typedef struct
{
  /* This variable stores the current search sync parameters */
  mcalwcdma_srch_sync_parms_type* cur_sync_params;

  /* This variable stores the current buffer flag for this search */
  boolean cur_update_buffer;

} mcalwcdma_srch_pended_params_for_offline_clk_chg_struct_type;
 ...
 */
  rex_crit_sect_type wsrch_crit_sect;
  rex_crit_sect_type srch_cm_crit_sect;
}mcalwcdma_srch_ext_cntrl_params_type;

extern mcalwcdma_srch_ext_cntrl_params_type mcalwcdma_srch_ext_cntrl_params[WCDMA_NUM_SUBS];

typedef struct
{
    /* Contains the id and state for current srch */
  mcalwcdma_srch_state_db_struct_type mcalwcdma_srch_state_db[SRCH_NUM_Q];
  /* Flag to indicate if current search is aboroted */
  boolean mcalwcdma_srch_aborted;
  /* This variable stores the pending search command id */
  mcalwcdma_srch_type mcalwcdma_srch_pending_id[SRCH_NUM_Q];
  /*Stores the last cm queue index used. Helps in using Queues alternatively */ 
  uint8 cm_queue_last_used;
  /* This variable stores the pending search sync parameters */
  mcalwcdma_srch_sync_parms_type *mcalwcdma_sync_parms_pending[SRCH_NUM_Q];
  /* Holds the address of the cm sync write buffer structure */
  WfwCmSrchqCmdStruct *mcalwcdma_wfw_intf_srch_cm_q_sync_write_ptr;
  /* Holds the address of the Async read buffer structure */
  WfwSrchAsyncReadStruct *mcalwcdma_wfw_intf_srch_async_read_ptr;
  /* Holds the address of the Async write buffer structure */
  WfwSrchoffLineAsyncWriteStruct *mcalwcdma_wfw_intf_srch_async_write_ptr;
  
} mcalwcdma_srch_int_cntrl_params_mdump_type; 


typedef struct
{
  /* Sleep clock value when a search response is received */
  uint32 mcalwcdma_srch_sc_at_srch_rsp;


  uint16 debug_tmp_timer;
  timer_type mcalwcdma_srch_cm_q0_lost_timer;

  /* Flags that indicated if the cm asyn params are updated or not */
  boolean mcalwcdma_srch_cm_async_parms_updated[MCALWCDMA_SRCH_CM_NUM_TYPES];
  /* Since we dont need async params for the "none" search, 1 element less is required 
 * in following array.
 */
  boolean mcalwcdma_srch_async_parms_updated[MCALWCDMA_SRCH_NUM_TYPES];
  /* Sleep clock value when a search command is issued */
  uint32 mcalwcdma_srch_sc_at_srch_cmd;
  #if defined(FEATURE_WPLT) || defined(FEATURE_WCDMA_PLT_MODE)
  uint32 debug_sample_server_data[1024];
  #endif
  boolean rumi2_error_fatal_on_srch_lost;
  /* Debug variable to avoid sending abort srch to FW */
  boolean srch_debug_no_abort;
  /* Contains the id and state for each cm queue */
  mcalwcdma_srch_cm_state_db_struct_type mcalwcdma_srch_cm_state_db[MCALWCDMA_SRCH_CM_MAX_NUM_CMQ];

  /* This array stores the call back functions from the MCAL SRCH clients */
  mcalwcdma_srch_status_callback_fn_type mcalwcdma_srch_cb[MCALWCDMA_SRCH_NUM_TYPES];
  /* Debug var to print type of srch scheduled\done */
  boolean mcal_srch_debug;
  #if defined(FEATURE_WPLT) || defined(FEATURE_WCDMA_PLT_MODE)
  uint32 debug_enable_sampsrv_read;
  #endif


  timer_type mcalwcdma_srch_cm_q1_lost_timer;
  boolean srch_debug_needed;

  /* Contains the current search parameters that are pended for an offline clock change */
  mcalwcdma_srch_pended_params_for_offline_clk_chg_struct_type *mcalwcdma_offline_clk_chg_pended_params;
  /* Stores the compressed mode call back functions */
  mcalwcdma_srch_cm_status_callback_fn_type mcalwcdma_srch_cm_cb[MCALWCDMA_SRCH_CM_NUM_TYPES];
  
    /*Mini Dump: Variables that need to be dumped moved to this structure*/
  mcalwcdma_srch_int_cntrl_params_mdump_type mcalwcdma_srch_int_cntrl_params_mdump; 
}mcalwcdma_srch_int_cntrl_params_type;
...
SIDE EFFECTS
  None
===========================================================================*/
extern void mcalwcdma_srch_init( wsub_id_e_type wsub_id); 
...
SIDE EFFECTS
  None
===========================================================================*/
extern void mcalwcdma_srch_cm_init( wsub_id_e_type wsub_id);
...
extern void mcalwcdma_programfreqtorxfe_reset( wsub_id_e_type wsub_id);
...
extern void mcalwcdma_proc_srch_done(srch_intraf_queue_enum_type srch_queue_idx, wsub_id_e_type wsub_id);
...
DESCRIPTION
  ...
  mcalwcdma_srch_int_cntrl_params[wsub_id].mcalwcdma_srch_pending_id - identifies the search type.
  mcalwcdma_srch_int_cntrl_params[wsub_id].mcalwcdma_sync_parms_pending - contains the sync parameters.
...
SIDE EFFECTS
  None
===========================================================================*/
extern void mcalwcdma_srch_do_pending_srch( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void mcalwcdma_srch_abort( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean mcalwcdma_is_srch_abort_pending( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void mcalwcdma_set_srch_abort_pending(boolean val, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void mcalwcdma_srch_cm_done_isr ( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint8 mcalwcdma_srch_get_avail_cm_cmd_q( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void mcalwcdma_srch_deactivate( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void mcalwcdma_srch_reset_srch( srch_intraf_queue_enum_type srch_queue_idx , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean mcalwcdma_is_srch_state_active( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean mcalwcdma_is_pending_srch_state_active( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void mcalwcdma_reset_pending_srch_type( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean mcalwcdma_is_srch_queue_active(srch_intraf_queue_enum_type queue, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
mcalwcdma_srch_type mcalwcdma_get_q0_cur_srch_id( wsub_id_e_type wsub_id);
...
mcalwcdma_srch_type mcalwcdma_get_q1_cur_srch_id( wsub_id_e_type wsub_id);
...
/*===========================================================================
FUNCTION mcalwcdma_srch_int_cntrl_params_init
...
SIDE EFFECTS
    None.
===========================================================================*/
extern void mcalwcdma_srch_int_cntrl_params_init( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION   mcalwcdma_srch_get_step1_cmd_buf

DESCRIPTION
  This function provides the address of step1 command buffer based on wsub_id.
...
SIDE EFFECTS
  None
===========================================================================*/
extern WfwSrchStep1LpqHpqStruct* mcalwcdma_srch_get_step1_cmd_buf(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION   mcalwcdma_srch_get_step1_rslt_buf

DESCRIPTION
  This function provides the address of step1 result buffer based on wsub_id.
...
SIDE EFFECTS
  None
===========================================================================*/
extern WfwSrchStep1RsltStruct* mcalwcdma_srch_get_step1_rslt_buf(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION   mcalwcdma_srch_get_nastt_cmd_buf

DESCRIPTION
  This function provides the address of nastt command buffer based on wsub_id.
...
SIDE EFFECTS
  None
===========================================================================*/
extern WfwSrchNASTTLpqHpqStruct* mcalwcdma_srch_get_nastt_cmd_buf(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION   mcalwcdma_srch_get_nastt_rslt_buf

DESCRIPTION
  This function provides the address of nastt result buffer based on wsub_id.
...
SIDE EFFECTS
  None
===========================================================================*/
extern WfwSrchNASTTRsltStruct* mcalwcdma_srch_get_nastt_rslt_buf(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION   mcalwcdma_srch_get_pn_cmd_buf

DESCRIPTION
  This function provides the address of pn command buffer based on wsub_id.
...
SIDE EFFECTS
  None
===========================================================================*/
extern WfwSrchPNLpqHpqStruct* mcalwcdma_srch_get_pn_cmd_buf(srch_intraf_queue_enum_type srch_queue_idx, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION   mcalwcdma_srch_get_pn_rslt_buf

DESCRIPTION
  This function provides the address of pn result buffer based on wsub_id.
...
SIDE EFFECTS
  None
===========================================================================*/
extern WfwSrchListRsltStruct* mcalwcdma_srch_get_pn_rslt_buf(srch_intraf_queue_enum_type srch_queue_idx, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION   mcalwcdma_srch_get_async_write_buf

DESCRIPTION
  This function provides the address of  async write buffer based on wsub_id.
...
SIDE EFFECTS
  None
===========================================================================*/
extern WfwSrchoffLineAsyncWriteStruct* mcalwcdma_srch_get_async_write_buf(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION   mcalwcdma_srch_get_async_read_buf

DESCRIPTION
  This function provides the address of  async read buffer based on wsub_id.
...
SIDE EFFECTS
  None
===========================================================================*/
extern WfwSrchAsyncReadStruct* mcalwcdma_srch_get_async_read_buf(wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void mcalwcdma_srch_update_nastt_usefreezeflag_intraf_acq(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION   mcalwcdma_srch_cm_queue_last_used_index

DESCRIPTION
  This function return cm_queue_last_used_index.
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint8 mcalwcdma_srch_cm_queue_last_used_index(wsub_id_e_type wsub_id);

#ifdef FEATURE_QSH_MDUMP 
/*===========================================================================
  FUNCTION     mcalwcdma_srch_qsh_mdump_collect

  DESCRIPTION
  This function dumps all the required parameters from mcalwcdma_srch internal and 
  external control parameter structures.
  For now nothing is needed from mcalwcdma_srch external control parameters
  ...
  None
  ===========================================================================*/
extern QSH_MDUMP_FN_ATTR void mcalwcdma_srch_qsh_mdump_collect(wsub_id_e_type wsub_id);
#endif
