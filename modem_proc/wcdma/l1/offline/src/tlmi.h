...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/src/tlmi.h#3 $ $DateTime: 2020/01/22 08:56:27 $ $Author: pwbldsvc $

when        who     what, where, why
--------    ---     ---------------------------------------------------------10/05/15    rn      Debug code to check if there is unexpected jump in CPC start CFN
...
*/
#define TLM_GET_ALIGN_TYPE (seq_get_current_align_type(wsub_id))
...
#define TLM_SET_ANY_ALIGN_HAPPENED()        \
  do {        \
       tlm_ext_cntrl_params[wsub_id].tlm_db.any_align_happened = TRUE;        \
  } while (0)

#define TLM_RESET_ANY_ALIGN_HAPPENED()        \
  do {        \
       tlm_ext_cntrl_params[wsub_id].tlm_db.any_align_happened = FALSE;        \
  } while (0)
...
#define TLM_LOG_SUB_CHIP_OPERATION(result, minuend, subtrahend)         \
  do {         \
  WL1_MSG3(LOW,"sub_chip_oper: res:%d min:%d sub:%d", result, minuend, subtrahend);         \
  } while (0)

#define TLM_LOG_ADD_CHIP_OPERATION(result, minuend, subtrahend)         \
  do {         \
  WL1_MSG3(LOW,"add_chip_oper: res:%d min:%d sub:%d", result, minuend, subtrahend);         \
  } while (0)

#define TLM_LOG_TLM_WRC(tlm_wrc)         \
  do {         \
    WL1_MSG3(LOW,"tlm_wrc: (f,cx8):%d, %d", (tlm_wrc).frame_qty, (tlm_wrc).chip_offset_cx8_qty, 0);         \
  }while(0)

#define TLM_LOG_TLM_TFN(tlm_tfn)         \
  do {         \
    WL1_MSG3(LOW,"tlm_tfn: (f,cx8):%d, %d", (tlm_tfn).frame_qty, (tlm_tfn).chip_offset_cx8_qty, 0);         \
  }while(0)

#define TLM_LOG_TLM_CFN(tlm_cfn)         \
  do {         \
    WL1_MSG3(LOW,"tlm_cfn: (f,cx8):%d, %d", (tlm_cfn).frame_qty, (tlm_cfn).chip_offset_cx8_qty, 0);         \
  }while(0)

#define TLM_LOG_TLM_FRC(frc_quant)         \
  do {         \
    WL1_MSG3(LOW,"tlm_frc: (f,cx8):%d, %d", (frc_quant).frame_qty, (frc_quant).chip_offset_cx8_qty, 0);         \
  }while(0)

#define TLM_LOG_TLM_TST(tlm_tst)         \
  do {         \
    WL1_MSG3(LOW,"tlm_tst: (f,cx8):%d, %d", (tlm_tst).frame_qty, (tlm_tst).chip_offset_cx8_qty, 0);         \
  }while(0)

#define TLM_LOG_NW_DIFF_QTY(nw_diff)         \
  do {         \
    WL1_MSG3(HIGH,"tlm_nw_diff: (f,cx1):%d, %d", (nw_diff).frame_qty, (nw_diff).chip_offset_cx1_qty, 0);         \
  }while(0)

#define TLM_LOG_TLM_WRC_TFN_DIFF(wrc_tfn)         \
  do {         \
    WL1_MSG3(HIGH,"wrc_tfn_diff: (f,cx8):%d, %d", (wrc_tfn).frame_qty, (wrc_tfn).chip_offset_cx8_qty, 0);         \
  }while(0)

#define TLM_LOG_SRCH_SFN_WRC_DIFF(sfn_wrc)         \
  do {         \
    WL1_MSG3(LOW,"sfn_wrc: vld:%d fr:%d cx8:%d", (sfn_wrc).sfn_wrc_valid,         \
      (sfn_wrc).sfn_wrc_diff.frame_qty, (sfn_wrc).sfn_wrc_diff.chip_offset_cx8_qty);         \
  } while(0)

#define TLM_LOG_TLM_SFN_WRC_DIFF(sfn_wrc)         \
  do {         \
    WL1_MSG3(HIGH,"sfn_wrc: (f, cx8):%d, %d, %d",         \
...
#define TLM_PREP_WRC_FROM_STMR_DYNAMIC(frc_quant, stmr_read)    \
  do {    \
     stmr_read = STMR_REF_COUNT_DYNAMIC(wsub_id); \
...
#define TLM_PREP_WRC_FOR_STMR_INPUT(frc_quant, stmr_read)    \
  do {    \
    (frc_quant).frame_qty = ((TLM_STRIP_FRAMES_FROM_FRC_DYNAMIC(stmr_read)));    \
    (frc_quant).chip_offset_cx8_qty = ((TLM_STRIP_CX1_FROM_FRC_DYNAMIC(stmr_read)) << 3);    \
  }while(0)
...
/*#define TLM_ENABLE_EVENTS_AFTER_ALIGN()        \
  do {        \
    ...
    {        \
      ...
      WL1_MSG3(HIGH,"Keeping CM events alive for interF HHO", 0, 0, 0);        \
    }        \
    else        \
    {        \
...
#define TLM_ALIGN_IS_PARAMS_OK(cntr, cell_info_ptr, ch_off_ptr)    \
  do {    \
    ...
    {    \
      WL1_ERR_FATAL("invalid params to tlm_set_align cntr:%d cell_info:%d ch_off:%d", cntr, cell_info_ptr, ch_off_ptr);    \
    }    \
  } while(0)
...
#define TLM_COMP_WRC_FROM_TFN(wrc, tfn)    \
  do {    \
    TLM_ADD_TWO_SFN(wrc, tfn, tlm_ext_cntrl_params[wsub_id].tlm_db.wrc_tfn_diff);    \
    if (TLM_IS_FRC_ALIGNED())    \
    {    \
      tlm_tfn_struct_type ref_cnt;    \
      TLM_BUILD_RCA_COMPLETE(ref_cnt);    \
...
#define TLM_GET_SFN_FOR_FRC_ALIGNMENT(tfn)    \
  do {    \
    ...
    tlm_get_snapshot(&ss, TLM_SNAPSHOT_MASK_FRC, STMR_MAX_NUM_COUNTERS ,wsub_id);    \
...
#define TLM_COMP_NOM_DPCH_TFN_FROM_TST_TFN(tst_tfn, nom_dpch_tfn)    \
  do {    \
    TLM_ADD_TWO_SFN(nom_dpch_tfn, tst_tfn, tlm_int_cntrl_params[wsub_id].nom_dpch_tst_diff_qty_in_tfn);    \
  } while(0)
...
extern void tlm_adjust_sfn_wrc_diff(tlm_cell_info_struct_type *cell_info, 
    sfn_wrc_diff_type *sfn_wrc_bef_adj, sfn_wrc_diff_type *sfn_wrc_aft_adj, wsub_id_e_type wsub_id);
...
DESCRIPTION          This function takes a snapshot of the necessary
                     variables in the ext_tlm_cntrl->tlm_db before align change.
...
SIDE EFFECTS         None
===========================================================================*/
extern void tlm_handle_wrc_tfn_diff_bef_align( wsub_id_e_type wsub_id);
...
DESCRIPTION          This function takes a snapshot of the necessary
                     variables in the ext_tlm_cntrl->tlm_db after align change. It re-computes
                     the wrc_tfn_diff with the help of the snapshot variables
                     in ext_tlm_cntrl->tlm_db.
...
SIDE EFFECTS         None
===========================================================================*/
extern void tlm_handle_wrc_tfn_diff_aft_align(tlm_tfn_struct_type sfn, int16 cfn_frame_qty, wsub_id_e_type wsub_id);
...
SIDE EFFECTS         None
===========================================================================*/
void tlm_get_sfn_wrc_diff_with_adj(tlm_cell_info_struct_type *cell_info, sfn_wrc_diff_type *out, wsub_id_e_type wsub_id);
...
SIDE EFFECTS         None
===========================================================================*/
extern void tlm_get_sfn_wrc_diff(tlm_cell_info_struct_type *cell_info, sfn_wrc_diff_type *out, wsub_id_e_type wsub_id);
...
SIDE EFFECTS         None
===========================================================================*/
extern tlm_ch_time_comp_scenario_enum_type tlm_get_ch_computation_type(boolean sfn_wrc_valid, wsub_id_e_type wsub_id);
