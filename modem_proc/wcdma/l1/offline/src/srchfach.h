...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/src/srchfach.h#7 $
...
03/08/19    gmr     Pend Meas Req if GSM search is ongoing
02/19/18   mk      Extend the FMO gap to 3 frames when issuing full search on inter frequency in a W+L config.
06/13/16   gn      Ceil gap_length conversion from us to slots and add extra slot for WFW overhead
04/20/16   mk      FRM log packet changes.
04/19/16   ar      Featurize QICE deactivation for LTE FMO
11/26/15   nga     Changes to deallocate Gen Event created by FRM in case of state transitions.
11/06/15   mk      RACH vs QTA changes
09/21/15   wz      clean compiler err when disable MSIM 
09/18/15   db      Clear GSM RSSI num samples if the last GSM meas results were ignored 
                   due to W suspend for LTA
08/07/15   gsk     Double instantiating srchfach and wsrchlte dB
06/29/15   nga     FMO log packet fixes
06/17/15   mk      Updating the log packet 0x4721 with correct H/C-RNTI
06/15/15   vs      FR 24632: Searcher enhancement in compressed mode  (Handling Intra Full searches during CM/FMO ) 
...
12/22/14   vs      FR 24632: Searcher enhancement in compressed mode  (Handling Intra Full searches during CM/FMO ) 
11/19/14   gsk     FACH FMO log packet and adding support for 0x4027 for FE-FACH
11/11/14   psr     Moved srchcrutils_find_highest_priority_rat to srchcrutils
...
03/26/13   ar      Skip the GSM FMO if the local srchfach_int_cntrl_params.cmd handling for FMO is not handled in the same frame
...
04/12/12   stk     Added support to send FMO GSM srchfach_int_cntrl_params.cmd 1 frame before to FW, when HS FACH is configured.
...
06/15/09   uk      Support for sending START_FACH_MEAS srchfach_int_cntrl_params.cmd to fw before 
...
/* FRM log packet related macros */
#define SRCH_FACH_FRM_DL_STATE_BMSK 0x3
#define SRCH_FACH_FRM_DL_STATE_SHFT 0

#define SRCH_FACH_FRM_UL_STATE_BMSK 0xC
#define SRCH_FACH_FRM_UL_STATE_SHFT 2

#define SRCH_FACH_FRM_FEFACH_STATE_BMSK 0x70
#define SRCH_FACH_FRM_FEFACH_STATE_SHFT 4

#define SRCH_FACH_FRM_UE_STATE_SET_VAL(buf, field, val) \
  (buf) &= (~SRCH_FACH_FRM_##field##_BMSK); \
  (buf) |= (((val) << SRCH_FACH_FRM_##field##_SHFT) & \
            (SRCH_FACH_FRM_##field##_BMSK))
...
/* Time in us for 1 W Slot used for W2L FEFACH */
#define SRCH_FACH_W_SLOT_IN_US 667

/* Minimum W2L gap length for FE-FACH in number of slots*/
#define SRCHFACH_W2L_GAP_LEN_MIN 10

/*num frame for opts for TA30*/
#define SRCHFACH_INTER_NUMFRAME_LTA 3

#define SRCHFACH_INTER_INVALID_NUMFRAME_LTA 0xFF
...
#if defined(FEATURE_WCDMA_TO_LTE_FACH) && defined(FEATURE_WCDMA_ABSOLUTE_PRIORITY_FACH)
#define SRCHFACH_LTE_FMO_ENABLED() (srchfach_ext_cntrl_params[wsub_id].srchfach_ext_cntrl_params_mdump.srchfach_n_intra_meas_parms.inter_sys_lte_ind == TRUE)
#endif
...
extern const srchgsm_meas_purpose_enum_type
...
typedef enum
{
  ...
  SRCH_FACH_INTRAF_HIGH_PRIORITY_RACH,  /* RACH request for SRB */
  SRCH_FACH_INTER_MEAS,
  SRCH_FACH_RACH_RXD,
  SRCH_FACH_INTRAF_LOW_PRIORITY_RACH,   /* RACH request for data */
  SRCH_FACH_HIGH_PRIORITY_QTA,          /* QTA request for Page or any high priority gap */
  SRCH_FACH_LOW_PRIORITY_QTA,           /* QTA request for Measurement gap */
...
#define SRCH_FACH_GET_RACH_REQ_ID_BMSK (SRCH_FACH_RESERVE_REQ_ID_BMSK(SRCH_FACH_INTRAF_HIGH_PRIORITY_RACH) | \
     SRCH_FACH_RESERVE_REQ_ID_BMSK(SRCH_FACH_INTRAF_LOW_PRIORITY_RACH))

#define SRCH_FACH_GET_QTA_REQ_ID_BMSK (SRCH_FACH_RESERVE_REQ_ID_BMSK(SRCH_FACH_HIGH_PRIORITY_QTA) | \
     SRCH_FACH_RESERVE_REQ_ID_BMSK(SRCH_FACH_LOW_PRIORITY_QTA))

#ifdef FEATURE_FULL_SRCH_FRAME_RESERVATION_ENABLED
#error code not present
#else
#define SRCH_FACH_RESERVE_ALL_INTRA_REQ_BMSK \
  (SRCH_FACH_GET_RACH_REQ_ID_BMSK | \
   SRCH_FACH_RESERVE_REQ_ID_BMSK(SRCH_FACH_RACH_RXD))
#endif  /* FEATURE_FULL_SRCH_FRAME_RESERVATION_ENABLED */

#define SRCHFACH_NONDRX_FACH() ((wsrch_ext_cntrl_params[wsub_id].wsrch_ext_cntrl_params_mdump.search_state == SRCH_FACH_STATE) && (!srchfachedrx_ext_cntrl_params.srchfachdrx_db.edrx_active))

#define SRCHFACH_ENABLE_GSM_FMO_MEAS() \
  ((wsrch_ext_cntrl_params[wsub_id].wsrch_ext_cntrl_params_mdump.search_state == SRCH_FACH_STATE) ? (srchfach_ext_cntrl_params[wsub_id].srchfach_ext_cntrl_params_mdump.srchfach_n_intra_meas_parms.inter_sys_gsm_ind ? SRCHCR_MEAS_GSM : SRCHCR_MEAS_NONE): SRCHCR_MEAS_NONE)
  
#define SRCHFACH_ENABLE_INTERF_FMO_MEAS() \
  ((wsrch_ext_cntrl_params[wsub_id].wsrch_ext_cntrl_params_mdump.search_state == SRCH_FACH_STATE) ? (srchfach_ext_cntrl_params[wsub_id].srchfach_ext_cntrl_params_mdump.srchfach_n_intra_meas_parms.inter_f_ind ? SRCHCR_MEAS_INTER : SRCHCR_MEAS_NONE): SRCHCR_MEAS_NONE)


#ifdef FEATURE_WCDMA_DISABLE_QICE_FOR_LTE_FMO   
...
typedef void srchfach_cme_cb_func_type(boolean,wsub_id_e_type wsub_id);
#endif
#endif
...
typedef struct
{
  /* Flag to provide info whether to Block RACH reservation or not */
  boolean block_rach;
  /* Gen Event Id which was allocated for the event used to block RACH for 10 msec period time */
  uint8 gen_event_id_allocated;
}srchfach_block_rach_reservation_for_qta_type;
...
typedef struct
{
  ...
  boolean fmo_enabled;
  uint16 w2l_rpt_cntr;
  boolean fach_cell_rptd;
}srchfach_w2l_pseudo_meas_type;
 ...
 {
   /* FACH GSM meas state */
   srchfach_gsm_meas_state_enum_type srchfach_gsm_meas_state;
   /* FACH GSM measurement parameters */
   srchfach_gsm_meas_parms_struct_type srchfach_gsm_meas_parms;
   /* FACH GSM BSIC Initial ID parameters */
   srchfach_gsm_bsic_id_parms_struct_type srchfach_gsm_bsic_id_parms;
   /* FACH GSM BSIC reconfirmation parameters */
   srchfach_gsm_bsic_rec_parms_struct_type srchfach_gsm_bsic_rc_parms;
   
 #ifdef FEATURE_WCDMA_DISABLE_QICE_FOR_LTE_FMO 
  #if defined(FEATURE_WCDMA_ABSOLUTE_PRIORITY_FACH) && defined(FEATURE_WCDMA_TO_LTE_FACH)
   srchfach_cme_cb_func_type *srchfach_cme_cb_func_ptr;
  #endif
  #endif
   /* FACH non-intra-F measurement parameters */
	srchfach_n_intra_meas_parms_struct_type srchfach_n_intra_meas_parms;
  #ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
   srchfach_n_intra_meas_parms_struct_type srchfach_n_intra_meas_bkp_parms;
#endif
  #ifdef FEATURE_DUAL_SIM_WCDMA_TUNEAWAY
   /* global to indicate that UE has resumed back to W from Tuneaway */
   boolean srchfach_resuming_fach_tuneaway;
  #endif
 
   srchfach_fmo_state_mc_info_struct_type srchfach_fmo_state_info;
   /* FACH GSM RSSI Measurement parameters */
   srchfach_gsm_rssi_parms_struct_type srchfach_gsm_rssi_parms;
   
 }srchfach_ext_cntrl_params_mdump_type;
...
typedef struct
{
  ...
  srchfach_ext_cntrl_params_mdump_type srchfach_ext_cntrl_params_mdump;

  /* FACH search substate */
  srchfach_substate_enum_type srchfach_substate;
  uint8 srchfach_s_fail_cnt;

  srchfach_meas_oksn_sys_enum_type srchfach_meas_oksn_sys_tab[SRCHFACH_MEAS_OKSN_SYS_NUM];

  /* Backed up snapshot of substate for resuming searches */
  srchfach_substate_enum_type srchfach_prev_substate;
  #ifdef FEATURE_DUAL_SIM_WCDMA_TUNEAWAY
...
** since L1_ENA_CELL_RESEL_TIMER_SIG expired when UE was tuned away and MDSP was not active */
  boolean srchfach_pending_srchcr_init;
  #endif
  #ifdef FEATURE_PRIORITIZE_FMO_OVER_RACH_IN_LOW_S_RXAGC
  #ifdef FEATURE_WCDMA_HS_FACH
  /* 5 = 5 *20ms ASET srch = 100ms */
  uint8 srchfach_if_fmo_fail_cnt;
  #endif
  #endif
  srch_fach_meas_oksn_info_type srchfach_fmo_log_pkt_info[20];
  srch_fach_frm_reservation_info_type srchfach_frm_log_pkt_info[20];
  /* Create SRCHFACH critical section */
  srchfach_crit_sect_type srchfach_crit_sect;
  uint8 srchfach_fmo_log_pkt_idx;
  uint8 srchfach_frm_log_pkt_idx;

  #ifdef FEATURE_PRIORITIZE_FMO_OVER_RACH_IN_LOW_S_RXAGC
  #ifdef FEATURE_WCDMA_HS_FACH
  uint8 srchfach_gsm_fmo_fail_cnt;
  #endif
  #endif
 
  #ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
  srchfach_w2l_pseudo_meas_type srchfach_w2l_pseudo_meas;
#endif
}srchfach_ext_cntrl_params_type;


extern srchfach_ext_cntrl_params_type srchfach_ext_cntrl_params[WCDMA_NUM_SUBS];

void srchfach_ext_cntrl_params_init( wsub_id_e_type wsub_id);

void srchfach_int_cntrl_params_init( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srch_fach_init( wsub_id_e_type wsub_id);

/*===========================================================================
...
SIDE EFFECTS    
===========================================================================*/
extern uint16 srchfach_calculate_tm(srch_cell_struct_type *cell_ptr, wsub_id_e_type wsub_id);

/*===========================================================================
...
SIDE EFFECTS    
===========================================================================*/
extern uint16 srchfach_adjust_off(srch_cell_struct_type *cell_ptr, wsub_id_e_type wsub_id);

/*===========================================================================
...
SIDE EFFECTS 
===========================================================================*/   
extern void srchfach_schedule_nbr_sfn_decode( wsub_id_e_type wsub_id);

/*===========================================================================
...
===========================================================================*/
void srchfach_aset_update_srch_done( srch_done_cmd_type *cmd_ptr , wsub_id_e_type wsub_id);

/*===========================================================================
...
SIDE EFFECTS
  None.
===========================================================================*/
void srchfach_send_oos( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
void srchfach_declare_reacq_status( wsub_id_e_type wsub_id);

/*===========================================================================
...
SIDE EFFECTS    
===========================================================================*/
extern boolean srch_fach_cleanup( l1m_state_enum_type next_state , wsub_id_e_type wsub_id);
...
SIDE EFFECTS 
===========================================================================*/ 
extern void srchfach_update_sfn_sfn_val(srch_sfn_wrc_diff_cmd_done_type *srch_cmd, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern boolean srchfach_gsm_query_bsic_id_cell_power_level( srch_gsm_cell_struct_type *this_cell, wsub_id_e_type wsub_id);
...
SIDE EFFECTS    
===========================================================================*/
boolean srchfach_meas_calc_oksn( wsub_id_e_type wsub_id);
...
SIDE EFFECTS    
===========================================================================*/
void srchfach_meas_n_intra_init( boolean clearall , wsub_id_e_type wsub_id);
...
SIDE EFFECTS    
===========================================================================*/
void srchfach_gsm_meas_init( wsub_id_e_type wsub_id);
...
===========================================================================*/
boolean srchfach_gsm_meas_start( wsub_id_e_type wsub_id);
...
===========================================================================*/
boolean srchfach_gsm_meas_pause( wsub_id_e_type wsub_id);
...
===========================================================================*/
boolean srchfach_gsm_meas_cleanup( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srchfach_gsm_meas_cmd_cleanup_done(uint8 num_cmd_pending, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
void srchfach_gsm_process_rssi_meas_result( wsub_id_e_type wsub_id);
...
===========================================================================*/
void srchfach_gsm_process_bsic_recf_result( boolean verified , wsub_id_e_type wsub_id);
...
===========================================================================*/
void srchfach_gsm_process_bsic_id_result( boolean verfied , wsub_id_e_type wsub_id);
...
===========================================================================*/
void srchfach_gsm_bsic_id_clear_failed_cell_record( wsub_id_e_type wsub_id);
...
===========================================================================*/
void srchfach_gsm_bsic_id_pick_next_cell( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
uint8 srchfach_gsm_get_next_nset_for_rssi( uint8 cell_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srchfach_update_rnti(uint16 rnti, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srchfach_sccpch_evt_handler( wsub_id_e_type wsub_id);
...
===========================================================================*/
void srchfach_disable_sccpch_evt( wsub_id_e_type wsub_id);
...
===========================================================================*/
void srchfach_enable_sccpch_evt( wsub_id_e_type wsub_id);
...
DESCRIPTION     Update srchfach_ext_cntrl_params.srchfach_n_intra_meas_parms.next_meas_oksn_sfn
...
===========================================================================*/
void srchfach_meas_update_oksn( wsub_id_e_type wsub_id);
...
===========================================================================*/
boolean srch_fach_meas_oksn_due( wsub_id_e_type wsub_id);
...
===========================================================================*/
void srch_fach_update_timers( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  ...
  reserrved
===========================================================================*/
boolean srchfach_reserve_inter_meas_oksn(srchfach_meas_oksn_sys_enum_type type, uint8 num_frames, wsub_id_e_type wsub_id);
...
===========================================================================*/
boolean srchfach_wait_for_sib_mod_update( wsub_id_e_type wsub_id);

#define SRCHFACH_IS_MEAS_OCCASION_SPECIFIED \
            srchfach_ext_cntrl_params[wsub_id].srchfach_ext_cntrl_params_mdump.srchfach_n_intra_meas_parms.c_rnti_valid
...
extern void srch_fach_reserve_mgr_init( wsub_id_e_type wsub_id);
...
DEPENDENCIES    srchfach_ext_cntrl_params.srchfach_n_intra_meas_parms must be valid
...
extern boolean srch_fach_query_inter_fach_meas_oksn_exists(
  ...
  uint8 num_fr, wsub_id_e_type wsub_id);
...
extern boolean srch_fach_reserve_frame(
  ...
  boolean req_mode_force, wsub_id_e_type wsub_id);

/*===========================================================================

FUNCTION        SRCH_FACH_QUERY_RESERVED_QTA_REQ_ID

DESCRIPTION     This function checks which QTA reqID is reserved and returns the QTA reqID.
                 If no QTA is reserved currently, invalid reqID is returned.

DEPENDENCIES    None

RETURN VALUE    srch_fach_reserve_req_id_enum_type
  
SIDE EFFECTS    None

===========================================================================*/

extern srch_fach_reserve_req_id_enum_type srch_fach_query_reserved_qta_req_id(wsub_id_e_type wsub_id);

/*===========================================================================

FUNCTION        SRCH_FACH_UNRESERVE_FRAME_FOR_RACH

DESCRIPTION     This function unreserves any remaning number of frame for
                uplink module. This is a wrapper API which should be used ONLY by uplink module.
...
extern void srch_fach_unreserve_frame_for_rach(
  /* entity that is trying to unreserve the frames */
  srch_fach_reserve_req_id_enum_type req_id, boolean abort_for_qta, wsub_id_e_type wsub_id);
...
extern void srch_fach_unreserve_frame(
  /* entity that is trying to unreserve the frames */
  srch_fach_reserve_req_id_enum_type req_id, wsub_id_e_type wsub_id);


/*===========================================================================

FUNCTION        SRCH_FACH_SWAP_RESERVATION_FOR_RACH

DESCRIPTION     This function swaps the reservations for RACH from HIGH priority to LOW priority

DEPENDENCIES    The HIGH priority RACH reservation should have been made before.

RETURN VALUE    None
  
SIDE EFFECTS    If swap is done successfully, it will update reservation variable accordingly.
                Make sure to have the mutex across both the unreserve and reserve API so that no one else a chance to go in between these 2 API calls.

===========================================================================*/

extern void srch_fach_swap_reservation_for_rach(wsub_id_e_type wsub_id);
...
extern void srch_fach_update_reserve_info(
  /* current frame CFN */
  uint8 this_fr_cfn, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean srch_fach_query_inter_s_reservation(uint8 *first_free_cfn, wsub_id_e_type wsub_id);
...
SIDE EFFECTS    
===========================================================================*/
extern boolean srch_fach_was_fmo_on(uint16 sfn, wsub_id_e_type wsub_id);
...
SIDE EFFECTS    sets srchfach_int_cntrl_params.srchfach_inters_meas_suspended to TRUE
                
===========================================================================*/
extern void srchfach_suspend_inters_meas_for_bch( wsub_id_e_type wsub_id);
...
SIDE EFFECTS    sets srchfach_int_cntrl_params.srchfach_inters_meas_suspended to FALSE
                
===========================================================================*/
extern void srchfach_resume_inters_meas_for_bch( wsub_id_e_type wsub_id);
#endif
/*===========================================================================
...
===========================================================================*/
extern void srchfach_cleanup_oksn_info( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srchfach_cm_init( wsub_id_e_type wsub_id);

/*===========================================================================
...
===========================================================================*/
extern boolean srchfach_inter_f_do_meas(boolean relentless_detect, wsub_id_e_type wsub_id);
...
extern void srchfach_interf_proc_meas( wsub_id_e_type wsub_id);
...
extern void srchfach_proc_sync_nset( wsub_id_e_type wsub_id);

void srchfach_gsm_bsic_parms_update( wsub_id_e_type wsub_id);
uint8 srchfach_get_sccpch_offset ( uint8 *hwch , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean srchfach_is_fach_gsm_meas_ongoing( wsub_id_e_type wsub_id);
...
SIDE EFFECTS    
===========================================================================*/
extern void srchfach_l1m_resuming_from_fach_tuneaway_resume_gsm( wsub_id_e_type wsub_id);
#endif
...
SIDE EFFECTS    Update srchfach_ext_cntrl_params.srchfach_n_intra_meas_parms

===========================================================================*/
extern void srchfach_get_accurate_ref_time( wsub_id_e_type wsub_id);
...
SIDE EFFECTS    Assign to srchfach_ext_cntrl_params.srchfach_n_intra_meas_parms.n_tti

===========================================================================*/
extern boolean srchfach_get_sccpch_longest_fach_tti( wsub_id_e_type wsub_id);
...
PARAMETERS
  ...
  req_id - SRCH_FACH_HIGH_PRIORITY_QTA or
           SRCH_FACH_LOW_PRIORITY_QTA
...
=====================================================================*/
extern boolean srchfach_confirm_qta_reservation (uint8 cfn, srch_fach_reserve_req_id_enum_type req_id, uint8 *client_bmask, wsub_id_e_type wsub_id);
...
=====================================================================*/
extern void srchfach_request_future_qta_reservation (uint8 cfn, srch_fach_reserve_req_id_enum_type req_id, wsub_id_e_type wsub_id);
...
=====================================================================*/
boolean srchfach_frame_resv_with_qta_active(srch_fach_reserve_req_id_enum_type reqid, uint8 *reserve_start_cfn,uint16 num_reserve_frames, wsub_id_e_type wsub_id);
...
SIDE EFFECTS    
===========================================================================*/
extern boolean srch_fach_query_client_reservation(srch_fach_reserve_req_id_enum_type req_id, wsub_id_e_type wsub_id);
#endif /* FEATURE_QTA */
...
SIDE EFFECTS    
===========================================================================*/
extern boolean srch_fach_query_rach_reservation( wsub_id_e_type wsub_id);

#endif /* FEATURE_WCDMA_HS_FACH_DRX */

extern uint32 srchfach_get_mrep( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srchfach_step1_done(srchsched_step1_nastt_status_struct_type *step1_nastt_status, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srchfach_nastt_done (srchsched_step1_nastt_status_struct_type *step1_nastt_status, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srchfach_nset_srch_done(srch_done_cmd_type *cmd_ptr, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void srchfach_aset_search_done(srch_done_cmd_type *cmd_ptr, wsub_id_e_type wsub_id);
...
extern void srchfach_start_cell_reselect_eval( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern boolean srchfach_can_scheduler_start_search(srch_ops_code_enum_type srch_type, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION        SRCHFACH_SUSPEND_FMO

DESCRIPTION     This function returns the current FMO status. It is used to block 
the CPHY command if a FMO is already scheduled and srchfach_int_cntrl_params.cmd sent to FW. Otherwise 
...
srchfach_fmo_op_client_enum_type client_id, wsub_id_e_type wsub_id);
...
SIDE EFFECTS    
===========================================================================*/
void srchfach_resume_fmo(srchfach_fmo_op_client_enum_type client_id, wsub_id_e_type wsub_id);
...
void srchfach_resume_meas_parm_update_cb( wsub_id_e_type wsub_id);
...
SIDE EFFECTS    
===========================================================================*/
void srchfach_resume_client_ops_aft_fmo( wsub_id_e_type wsub_id);
void srchfach_post_resume_fmo_client_ops( wsub_id_e_type wsub_id);
...
SIDE EFFECTS    
===========================================================================*/
srchfach_meas_oksn_sys_enum_type srchfach_calc_next_fmo_rat_type( wsub_id_e_type wsub_id);

extern uint16 srchfach_get_allowed_srch_mask( wsub_id_e_type wsub_id);
...
None
===========================================================================*/
extern void srchfach_register_act_deact_cme_cb_func(srchfach_cme_cb_func_type* cb_func, wsub_id_e_type wsub_id);
...
None
===========================================================================*/
extern void srchfach_deregister_act_deact_cme_cb_func( wsub_id_e_type wsub_id);
#endif
...
===========================================================================*/
extern boolean srchfach_start_lte_meas( wsub_id_e_type wsub_id);
...
None
===========================================================================*/
extern void srchfach_update_interf_timers( wsub_id_e_type wsub_id);
...
None
===========================================================================*/
extern void srchfach_get_ch_sfn(uint16 *sfn,uint32 *cx8_offset_qty, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srchfach_check_cell_ref_age_and_trim_mset_uset( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION        srchfach_gsm_do_meas

DESCRIPTION     This function issues IRAT GSM meas commands
...
===========================================================================*/
extern boolean srchfach_gsm_do_meas( wsub_id_e_type wsub_id);


#if defined(FEATURE_DUAL_SIM) && defined(FEATURE_DUAL_SIM_WCDMA_TUNEAWAY)
/*===========================================================================

FUNCTION        SRCHFACH_GSM_MEAS_CLEANUP_FOR_LTA

DESCRIPTION     Cleanup FACH GSM RSSI measurement variables if RSSI scan cycle was 
                completed but the last RSSI measurements were not processed due to WCDMA
                suspend for LTA so that fresh measurements can start after WCDMA resume.

DEPENDENCIES    None

RETURN VALUE    None
  
SIDE EFFECTS    Last RSSI measurements are lost

===========================================================================*/
extern void srchfach_gsm_meas_cleanup_for_lta(wsub_id_e_type wsub_id);
#endif
...
===========================================================================*/
extern void srchfach_log_meas_oksn_info( boolean submit_log_pkt,wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION        SRCHFACH_LOG_FRM_RESERVATION_INFO(0x41DB)

FILE NAME       

DESCRIPTION     Logs the FRM reservation and unreservation info.
...
===========================================================================*/
void srchfach_log_frm_reservation_info( srchfach_frm_request_type sample_type, srchfach_frm_result_type result_type, 
               srch_fach_reserve_req_id_enum_type req_id, srchfach_frm_failure_reason failure_reason, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION        SRCHFACH_SUBMIT_FRM_RESERVATION_INFO(0x41DB)

FILE NAME       

DESCRIPTION     Submits the FRM log packet if 20 samples are filled.
...
===========================================================================*/
void srchfach_submit_frm_reservation_info( boolean force_submit,wsub_id_e_type wsub_id);


/*=====================================================================

FUNCTION       SRCH_FACH_UNBLOCK_RACH_RESERVATION

DESCRIPTION
  This function is called as a callback when the 10msec event fires (The event was setup after QTA reservation ends)
  to unblock the RACH FRM reservation.

PARAMETERS
  evt_id  - 	allocated event id
  wsub_id
...
void srch_fach_unblock_rach_reservation(uint8 evt_id, wsub_id_e_type wsub_id);

#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
 /*===========================================================================
 
 FUNCTION        SRCHFACH_UPDATE_PSEUDO_W2L_PARAMS
 
 DESCRIPTION     function to support FR , this function resets all the variables which we set when FR got activated/deactivated
 ...
 ===========================================================================*/
extern void srchfach_update_pseudo_w2l_params(l1_meas_priority_eutra_cell_list_info_type *info,wsub_id_e_type wsub_id);
#endif

#ifdef FEATURE_QSH_MDUMP 
/*===========================================================================
FUNCTION WL1_QSH_MDUMP_COLLECT_SRCFACH_INT_PARAMS

DESCRIPTION
  This is a minidump function for dumping to QSH the selected srchfach int global variables for minidump
...
SIDE EFFECTS
  None
===========================================================================*/
extern QSH_MDUMP_FN_ATTR void wl1_qsh_mdump_collect_srchfach_int_params(wsub_id_e_type wsub_id);
#endif
