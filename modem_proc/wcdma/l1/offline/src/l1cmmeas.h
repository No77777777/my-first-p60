...
DESCRIPTION
  ...
  compressed mode l1cmmeas_int_cntrl_params.measurements required, cells to be measured, and perform
...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/src/l1cmmeas.h#6 $
...
11/03/15   db      Double existing tgpsi info array to make it exclusive for each subscription
08/07/15   gsk     Double instantiating srchfach and wsrchlte dB
02/24/15   jk      KW error searcher changes for Thor.
...
06/13/14   db      Do not send periodic l1cmmeas_int_cntrl_params.measurements untill first CM measurement gap after LTA gap.
...
01/31/11   sa\nd   Added callback function that will post local command to process 
                   periodic l1cmmeas_int_cntrl_params.measurements instead of processing it in timer context.
12/8/10    sa      Removed compilation warnings.
11/24/10   sa      Added code to support W2L l1cmmeas_int_cntrl_params.measurements under feature
...
#define SRCHCM_REMOVE_GSM_CELL_FOR_MEAS(cell_ptr) \
           srchcm_delete_inter_rat_cell_to_set( (cell_ptr), SRCH_INTER_RAT_CELL_GSM_TYPE  ,wsub_id)

#ifdef FEATURE_WTOL_TDD_CM_SUPPORT
/* An invalid CER SNR threshold */
#define L1CMMEAS_CER_SNR_INVALID_THRESH 0xffff 

/* The maximum number of l1cmmeas_int_cntrl_params.measurements that affects the CER_SNR threshold */
...
#define L1CMMEAS_FILTER_EUTRA_ENG(filt_eng,rsrp,elapsed_time, filt_coeff, srch_period)      \
    srch_interf_filter_eng(filt_eng, rsrp, elapsed_time,  filt_coeff,srch_period ,wsub_id)
...
to be passed on to l1cmmeas_int_cntrl_params.measurements layer */
...
typedef enum
{
  L1CMSRCH_DCH_INACTIVE,
  L1CMSRCH_DCH_ACTIVE
} l1cmsrch_state_enum_type;
...
typedef struct
{
  ...
  uint16  measid;
    /* Measurement Id Carrying Configuration */
} l1cmmeas_tgps_info_struct_type;

typedef struct
{
  W_ARFCN_T   bcch_arfcn;
  /* ARFCN	Integer (0..1023) */
  uint16  associated_cell_count;
 /* Number of cells with this ARFCN */
} l1cmmeas_arfcn_struct_type;

typedef struct
{
  #ifdef FEATURE_WCDMA_CM_LTE_SEARCH
  l1cmsrch_inter_rat_event_info_struct_type l1_eutra_inter_rat_event_array[L1_MAX_EUTRA_MEAS_EVENT];
  #endif
  #ifdef FEATURE_WCDMA_CM_LTE_SEARCH
  /* flag to tell if the cm meas done signal is for LTE searcher or not */
  boolean srchcm_eutra_inter_rat_search_done_status;
  #endif
  #ifdef FEATURE_WCDMA_CM_LTE_SEARCH
  /* Frequency waitlist database, used if the MCM contains more than 4 FDD or 4
   TDD frequencies */
  wsrchiratmeas_freq_list_struct_type l1cmmeas_eutra_freq_waitlist_db;
  #endif
  l1cmsrch_inter_rat_event_info_struct_type l1_inter_rat_event_array[L1_MAX_MEAS_EVENT];
}l1cmmeas_ext_cntrl_params_type;

extern l1cmmeas_ext_cntrl_params_type l1cmmeas_ext_cntrl_params[WCDMA_NUM_SUBS];


typedef struct
{
  /* Flag used to set the timer whenever TTT is set in event 3A, 3B, 3C.
  The timer needs to be started only once */
  boolean ttt_timer_started;
  #if defined(FEATURE_WPLT)
  l1_cm_info_struct_type test_cm_info;
  #endif
  l1cmsrch_inter_rat_event_info_struct_type tmp_inter_rat_event_info;
  #if defined(FEATURE_WPLT)
  l1_meas_ctrl_parm_struct_type *measctrl_ptr;
  #endif
  uint16 l1cmmeas_no_of_active_arfcns;
  #if defined(FEATURE_WPLT)
  l1_meas_parm_type measctrl_meas_parm;
  #endif
  #if defined(FEATURE_WPLT)
  uint32 l1cmmeas_num_cell_rmv;
  #endif
  #if defined(FEATURE_WPLT)
  l1_inter_sys_meas_result_struct_type measurements;
  #endif
  /* initialize the reconf abort timer */
  uint16 l1cmmeas_reconf_abort_timer;
  /* run nset list cells if any bsic_verify_reqd, set all cells to verify */
  boolean l1cmsrch_bsic_verify_reqd_state;
  #if defined(FEATURE_WPLT)
  uint32 l1cmmeas_num_cell_add;
  #endif
  rex_timer_type ttt_check_timer;
  ...
  boolean l1cmsrch_gsm_cell_cur_bsic_ver_status;
  #if defined(FEATURE_WPLT)
  uint32 l1cmmeas_num_cell_meas;
  #endif
  #if defined(FEATURE_WPLT)
  rrc_rr_interrat_measurement_data_type msmt_data;
  #endif
  l1cmmeas_arfcn_struct_type l1cmmeas_unique_arfcn_array[L1_MAX_CELL_MEAS];
  #if defined(FEATURE_WPLT) || defined(FEATURE_WCDMA_PLT_MODE)
  /* Default Expected RSSI Value to feed to CM Driver in WPLT Build */
  int16 l1cmmeas_expected_rssi_dbx16;
  #endif
  /***
 ***  Filter Coefficient Storage
 ***/
  l1_gsm_filter_coef_enum_type l1cmsrch_interrat_filter_coef[L1_MAX_FILTER_COEF_STORAGE];
  /* DPCH Compressed Mode Info IE struct for l1cmmgr */
  uint32 l1cmmeas_cm_config_num_tgps;
  #if defined(FEATURE_WPLT)
  l1_cm_tgp_seq_info_struct_type tgp_info[3];
  #endif
  /* DPCH Compressed Mode Info - used for translation from DPCH CM Status Info 
 *                             when performing DPCH CM Status Info update for
 *                             measurement control message processing
 */
  l1_cm_info_struct_type l1cmmeas_cm_config;
  #if defined(FEATURE_WPLT)
  l1_meas_cmd_type measctrl_meas_cmd;
  #endif
  l1cmsrch_state_enum_type l1cmsrch_state;
  /*
 * NSET Sorting Controls
 */
  uint32 l1cmsrch_nset_sort_required;
  boolean l1cmsrch_bsic_verify_update_required;
  l1_cm_tgp_seq_info_struct_type l1cmmeas_tgps_info[L1_MAX_TGPS * 2];
  boolean first_mcm_recvd_in_dch_state;
  boolean l1cmmeas_first_mcm_cell_list_processing;
  #if defined(FEATURE_WPLT)
  uint32 l1cmmeas_meas_id;
  #endif
  /* initialize the staleness timer value */
  uint16 l1cmmeas_rssi_aging_timer_value;
  /*
   * Indexed by TGPSI, this array holds information about transmission
   * gap patterns that is useful to the Measurement Control & Report
   * code.
  */
  l1cmmeas_tgps_info_struct_type l1cmmeas_tgps_info_array[L1_MAX_TGPS+1];
  /* +1 as valid TGPSI is 1..L1_MAX_TGPS, ignore 0 slot */
}l1cmmeas_int_cntrl_params_type;
...
=========================================================================*/
void l1cmmeas_init ( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
boolean l1_compressed_mode_validate_tgps_config( l1_cm_info_struct_type * p_cm_info , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
boolean l1_compressed_mode_tgps_config( l1_cm_info_struct_type * p_cm_info , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
boolean l1cmsrch_msmt_ctrl_msg_processing( l1_meas_cmd_type *meas_cmd , wsub_id_e_type wsub_id);
...
DESCRIPTION
  ...
  when compressed mode inter-freq/inter-RAT l1cmmeas_int_cntrl_params.measurements have been completed
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_process_cm_meas_done_sig( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_local_cm_meas_done_processing( l1m_local_cmd_type *lcmd , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_transition_into_cell_dch( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_transition_from_cell_dch( wsub_id_e_type wsub_id);
...
=========================================================================*/
int32  l1cmmeas_cio_value_scaled( int16 cio,wsub_id_e_type wsub_id );
...
=========================================================================*/
boolean l1_is_tgps_registered ( uint16 tgpsi , wsub_id_e_type wsub_id);
...
=========================================================================*/
void l1_unregister_tgps ( uint16 tgpsi , wsub_id_e_type wsub_id);
...
=========================================================================*/
void l1cmmeas_tgps_runtime_error ( uint16 tgpsi , wsub_id_e_type wsub_id);
...
void l1cmsrch_send_l1_measurement_control_msg( uint32 choice , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmmeas_reset_cell_reporting_state( wsub_id_e_type wsub_id);
...
void l1cmmeas_event_report_evaluation( wsub_id_e_type wsub_id);
...
void l1cmsrch_remove_interrat_measurement_event( uint32 event_idx ,                  
                  l1cmsrch_inter_rat_event_info_struct_type *l1_inter_rat_events_db, wsub_id_e_type wsub_id);
...
=========================================================================*/
boolean l1cmmeas_update_tgps_info(uint16 tgpsi,uint16 measid, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
uint16 l1cmmeas_gsm_time_diff( uint32 mul51_frame_offset, uint32 qs_offset , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_send_inter_sys_event_msmt_rpt_msg(uint32 event_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_send_inter_sys_periodic_msmt_rpt_msg(uint32 event_idx, boolean  last_periodic_rpt, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
uint32 l1cmsrch_get_empty_event( wsub_id_e_type wsub_id);
...
uint32 l1cmsrch_find_event( uint32 measid ,
                       l1cmsrch_inter_rat_event_info_struct_type *l1_inter_rat_event_db, wsub_id_e_type wsub_id);
...
DESCRIPTION
  This function will perform event 3b processing when l1cmmeas_int_cntrl_params.measurements
  have been completed.
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_process_event3b( uint32 event_idx , wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION     L1CMSRCH_PROCESS_EVENT3C

DESCRIPTION
  This function will perform event 3c processing when l1cmmeas_int_cntrl_params.measurements
  have been completed.
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_process_event3c( uint32 event_idx , wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION     L1CMSRCH_PROCESS_EVENT3A

DESCRIPTION
  This function will perform event 3a processing when l1cmmeas_int_cntrl_params.measurements
  have been completed.
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_process_event3a( uint32 event_idx , wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION     L1CMSRCH_PROCESS_EVENT3D

DESCRIPTION
  This function will perform event 3d processing when l1cmmeas_int_cntrl_params.measurements
  have been completed.
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_process_event3d( uint32 event_idx , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
uint32 l1cmsrch_add_filter_coef( l1_gsm_filter_coef_enum_type filter_coef , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_remove_filter_coef( uint32 fc_index, l1_gsm_filter_coef_enum_type filter_coef , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  The variable l1cmmeas_int_cntrl_params.l1cmmeas_no_of_active_arfcns may be changed.
===========================================================================*/
void l1cmmeas_remove_arfcn_array_index( uint16 arfcn_idx , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
srch_gsm_cell_struct_type * l1cmmeas_inter_s_find_cell( W_ARFCN_T arfcn, uint8 bsic_ncc, uint8 bsic_bcc , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
uint16  l1cmmeas_get_empty_arfcn_array_idx( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
boolean l1cmsrch_msmt_ctrl_msg_inter_rat_processing( l1_meas_cmd_type *meas_cmd , wsub_id_e_type wsub_id);
...
===========================================================================*/
/*ARGSUSED*/
boolean l1cmsrch_msmt_ctrl_msg_inter_freq_processing( l1_meas_cmd_type *meas_cmd , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  l1cmmeas_int_cntrl_params.l1cmmeas_cm_config & l1cmmeas_int_cntrl_params.l1cmmeas_tgps_info global variables are set
===========================================================================*/
boolean l1cmmeas_translate_dpch_status_info(
                                    l1_cm_status_info_struct_type * p_cm_status_info, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
boolean l1cmmeas_validate_dpch_status_info(l1_cm_status_info_struct_type * p_cm_status_info, wsub_id_e_type wsub_id);
...
void l1cmmeas_perform_filter_processing(srch_gsm_cell_struct_type *p_cell, 
                                                    boolean rssi_becoming_valid_flag, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_remove_meas(uint32 meas_id,l1_rat_type_enum_type rat, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
boolean l1cmsrch_msmt_ctrl_msg_eutra_inter_rat_processing( l1_meas_cmd_type *meas_cmd , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_remove_eutra_freq_info(uint16 num_earfcn, uint16 *list_earfcn, wsub_id_e_type wsub_id);
...
l1cmmeas_eutra_freq_add_status_type l1cmsrch_add_eutra_freq_info(l1_meas_eutra_frequency_list_type*
                                                                 add_freq_list, wsub_id_e_type wsub_id);
/*===========================================================================
...
SIDE EFFECTS
  None
===========================================================================*/
uint32 l1cmsrch_eutra_add_filter_coef( l1_common_filter_coef_enum_type filter_coef , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_eutra_remove_filter_coef( uint32 fc_index,l1_common_filter_coef_enum_type filter_coef , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
uint32 l1cmsrch_get_empty_eutra_event( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
boolean l1cmmmeas_get_eutra_nbr_freq_info( l1_inter_sys_eutra_freq_list_struct_type * p_freq_list , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
 boolean l1cmsrch_remove_eutra_freq_db(uint32 earfcn, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
 void l1cmmeas_eutra_event_report_evaluation( uint8 freq_idx , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_process_eutra_event3b( uint32 event_idx ,uint8 freq_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_process_eutra_event3c( uint32 event_idx,uint8 freq_idx , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_process_eutra_event3a( uint32 event_idx ,uint8 freq_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_process_eutra_event3d( uint32 event_idx,uint8 freq_idx , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmmeas_reset_eutra_freq_db( wsub_id_e_type wsub_id);
...
DESCRIPTION
  This function will print all active l1cmmeas_int_cntrl_params.measurements parameters.
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmmeas_print_meas_db( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmmeas_reset_cell_db(uint8 freq_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
 void l1cmsrch_eutra_process_periodic_rpts( 
                             uint32 event_idx, wsub_id_e_type wsub_id);
...
void l1cmsrch_send_eutra_event_msmt_rpt_msg(
                                 uint32 event_idx,
                                 uint8 freq_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1cmsrch_eutra_reset_ttt_array_state( uint8 event_idx , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
boolean l1cmmeas_should_send_lte_meas_rpt(uint16 freq_indx, wsub_id_e_type wsub_id);
...
void l1cmmeas_sort_eutra_meas_cell_list(uint32 event_idx, l1_eutra_measured_results_struct_type* msmts, wsub_id_e_type wsub_id);
...
uint8 l1cmmeas_get_num_freqs_used(uint32 earfcn, wsub_id_e_type wsub_id);
...
void l1cmmeas_process_interrat_periodic_rpts(uint32 event_idx, wsub_id_e_type wsub_id);
...
/*===========================================================================
  ...
  DESCRIPTION
    ...
    the given EARFCN and ID. If found, its index in ext_wsrchiratmeas_cntrl->wsrchiratmeas_sorted_lte_cell
    is returned.
...
/*===========================================================================
  ...
  SIDE EFFECTS
    None
 ===========================================================================*/
void l1cmsrch_send_suspended_irat_periodic_meas( wsub_id_e_type wsub_id);
/*===========================================================================
  FUNCTION
  ...
  SIDE EFFECTS
    None
 ===========================================================================*/
void l1cmsrch_send_suspended_interrat_periodic_meas( wsub_id_e_type wsub_id);
#ifdef FEATURE_WCDMA_CM_LTE_SEARCH
/*===========================================================================
  FUNCTION
  ...
  SIDE EFFECTS
    None
 ===========================================================================*/
void l1cmsrch_send_suspended_eutra_periodic_meas( wsub_id_e_type wsub_id);

#endif /*FEATURE_WCDMA_CM_LTE_SEARCH*/

/*===========================================================================
FUNCTION l1cmmeas_int_cntrl_params_init
...
SIDE EFFECTS
    None.
===========================================================================*/
extern void l1cmmeas_int_cntrl_params_init( wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION    L1CMSRCH_REMOVE_ALL_EUTRA_CELL_INFO

DESCRIPTION
    This function removes all etura cells from meas layer and driver layer
...
SIDE EFFECTS
    None.
===========================================================================*/
extern void l1cmsrch_remove_all_eutra_cell_info(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION      L1CMMEAS_RESET_ALL_IRAT_GSM_EVENT_TIMERS

DESCRIPTION
    This function resets all irat gsm event timers.
...
SIDE EFFECTS
    None.
===========================================================================*/
extern void l1cmmeas_reset_all_irat_gsm_event_timers(wsub_id_e_type wsub_id);
#endif /* L1CMMEAS_H */
