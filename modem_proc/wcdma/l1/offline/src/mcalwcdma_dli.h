...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/src/mcalwcdma_dli.h#3 $
$DateTime: 2020/01/22 08:56:27 $
$Author: pwbldsvc $

when        who     what, where, why
--------    ---     --------------------------------------------------------
08/21/17    ss      Fast DL channel setup incorrectly kicking in for FACH LTA
08/21/17    ss      Changes for LTA timeline optimization, phase-2
09/23/15    pr      Consider frame boundary(bpg 0) case when deciding to add 10bpg to the DPCH cfg evt time.
08/14/15    pr      Add 10bpg to the DPCH cfg evt time if it falls too close to the frame boundary
07/21/15    sg      Config window shrink for DPCH reconfig and hs active case
07/14/15    sr      Enhanced fix to process chan cfg done for correct SUB
05/20/15    sl      Revert Fix for CR812954 & disable SFN/CFN gen tl event while slamming to new counter
...
/* Modified config window for DPCH reconfig and HS active case*/
#ifdef FEATURE_WCDMA_TABASCO_MODEM 
#define MCALWCDMA_DL_RECFG_WINDOW_START (40 << CHIPX8_TO_BPG_RSHIFT)
#define MCALWCDMA_DL_RECFG_WINDOW_END (115 << CHIPX8_TO_BPG_RSHIFT)
#endif /*FEATURE_WCDMA_TABASCO_MODEM */
#define MCALWCDMA_DL_CFG_EVT_FIRING_START_TIME (41 << CHIPX8_TO_BPG_RSHIFT)

/* During FAST DL Channel Setup Procedures CFG window [10,125]bpgs */
#define MCALWCDMA_DL_CFG_WINDOW_EARLY_START (10 << CHIPX8_TO_BPG_RSHIFT)
#define MCALWCDMA_DL_CFG_EVT_FIRING_EARLY_START_TIME (11 << CHIPX8_TO_BPG_RSHIFT)
...
#define MCALWCDMA_DL_CFG_EVT_FIRING_TIME_BOUNDARY (0 << CHIPX8_TO_BPG_RSHIFT)

#define MCALWCDMA_DL_IS_FAST_DL_CHANNEL_SETUP_ENABLED(channel_type) DL_NV_IS_LTA_TL_OPTS_ENABLED() && \
        (l1m_get_state(wsub_id) == L1M_DCH) &&\
        WL1_RESUMING_AFTER_LTA_GAP(wsub_id) && \
        ((channel_type == MDSP_DPCH) || (channel_type == MDSP_FDPCH))
	   


/* Structure for internal params */
typedef struct
{
  uint32 mcalwcdma_dl_phch_cfg_upd_bmsk;
  uint32 mcalwcdma_dl_phch_cfg_sent_bmsk;
} mcalwcdma_dl_int_cntrl_params_type;
...
SIDE EFFECTS    None
===========================================================================*/
uint8 mcalwcdma_dl_get_evt_id_for_comb_chan(uint8 comb_id, wsub_id_e_type wsub_id);
...
boolean mcalwcdma_dl_rl_phychan_parms_validate(
  ...
  uint8 cnt, wsub_id_e_type wsub_id);
...
boolean mcalwcdma_dl_phychan_add_validate(
  /** Parameters for setting up the downlink physical channel */
  mcalwcdma_dl_phch_cfg_parms_struct_type *phychan_cfg_parms, wsub_id_e_type wsub_id);
...
boolean mcalwcdma_dl_phychan_drop_validate(
  ...
  uint8 comb_id, wsub_id_e_type wsub_id);
...
===========================================================================*/
void mcalwcdma_dl_mdsp_pre_cb( wsub_id_e_type wsub_id);
...
===========================================================================*/
void mcalwcdma_dl_compute_config_cfn(uint8 ch_idx, wsub_id_e_type wsub_id);
...
/** combiner id */
  uint8 db_idx, wsub_id_e_type wsub_id);
 ...
 */
/*============================================================================*/
uint8 mcalwcdma_dl_get_comb_id_for_interface_buf(uint32 interface_buf_id, wsub_id_e_type wsub_id);
