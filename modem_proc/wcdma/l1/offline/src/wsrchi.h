...
$Header: //components/rel/wcdma.mpss/8.4.0/l1/offline/src/wsrchi.h#7 $ $DateTime: 2024/09/13 05:01:06 $ $Author: pwbldsvc $


when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/21/20   vb      Added extra checks for customer specific PSC, Rxlevmin, UARFCN values
09/18/19   jb      Modifying API srch_compute_ecio_based_on_nc_nn
08/08/19   rs      Support for FR53313 WCDMA to LTE Pseudo measurement
05/01/18   hs      ALlow rxdiv state to be updated in BCH_SLEEP state
03/15/18   nc      Using the correct table to convert linear energy to ECIO on ACQ/BPLMN confirm report
02/20/18   nc      Ensure that the better RxAGC is choosen to calculate RSCP of serving cell
...
07/08/16   vn      Adding ACQ and BCH state on the list of states to update srch_rxdiv
                   variable.
...
12/16/15   vn      Enhancement for software attenuator
11/23/15   vn      Guard srchdch_rxdiv_state from getting set/reset in IDLE mode
10/12/15   gj      Extern prototype to determine if ACQ is CONN->IDLE ACQ
09/30/15   vn      Added invalid RXAGC Logging Macro.
...
06/22/15   vn      Changes for attenuating W signal through QXDM 
06/16/15   jk      Checking in DEBUG code to log srch_interf_k_val values during MCM processing, 
                   DCH_INIT, Multi carr config and Stack initialization.
06/09/15   amj     Corrected Compiler error for S1PPer 
06/05/15   amj     Changes for Step1 Peak Pruner algorithm.
05/07/15   skk     FR24494: BCH Inter SIB sleep initial check in.
04/22/15   vn      Reissue searches if FW aborts it.
02/19/15   vn      Using correct EcIo table for s criteria calculations while in BPLMN
...
08/18/06   rc      Externed ext_wsrch_cntrl->srch_interf_store_meas_db flag.
...
12/01/03   cpe     Added a couple of fields to ext_srchmeas_cntrl->srch_intra_freq_meas_db structure
...
03/21/03   yus      Declared extern variable ext_wsrch_cntrl->srch_meas_db_init
...
03/21/02   cpe     Added an extern for ext_srchhbevt_cntrl->srch_ops_ctl_parms defined in srchevt.c
...
#define SRCHBCH_EARLY_SCRIT_QMIN -20
#define SRCHBCH_EARLY_SCRIT_LVMIN -105
#define CUST_CELL_FREQ 10663
#define CUST_CELL_PSC 386
#define CUST_CELL_QRXLEVMIN -59
#define CUST_CELL_MCC 460
#define CUST_CELL_MNC 01
...
/* Valid range of raw rxagc is -512 to +511. So, defining invalid rxagc here. */
#define SRCH_INVALID_RXAGC_LOGGING 512
...
#define SRCH_RXD_ACQ_MODE(srch_state) (((srch_state == L1M_ACQ) ||       \
                                       (srch_state == L1M_BCH)  ||       \
                                       (srch_state == L1M_BCH_SLEEP)) && \
                                       rxd_is_active(wsub_id))
...
typedef enum
{
  SRCH_ONE_TIME_INIT,
  SRCH_MCM_PROCESSING,
  SRCH_MULTI_CARR_CONFIG,
  SRCH_DCH_INIT
} srchinterf_sub_module_enum_type;

typedef struct 
{
  l1m_state_enum_type l1m_state;
  wsub_id_e_type wsub_id;
  int8 srch_interf_meas_k_val_cache;    
  srchinterf_sub_module_enum_type module_id;
} srch_interf_k_val_dbg_type;
...
typedef struct
{
  ...
  /* num of cells forbidden for this event */
    ...
    } srchmeas_evt_trig_struct_mdump_type ;
...
typedef struct
{
  ...
  srchmeas_evt_trig_struct_mdump_type  evt_trig_s_mdump;
...
} srch_intra_freq_meas_parms_struct_mdump_type;
...
typedef struct
{
  ...
  /*Mini Dump: Variables that need to be dumped moved to this structure*/
  srch_intra_freq_meas_parms_struct_mdump_type srch_intra_freq_meas_db_mdump;
...
#define SRCH_OPS_S1_NASTT_PN_Q_ENABLED() (srchhbevt_ext_cntrl_params[wsub_id].srchhbevt_ext_cntrl_params_mdump.srch_ops_sanity_chk_mdump.s1_nastt_pn_q_enabled[SRCH_S1_Q] || \
                                                                                    srchhbevt_ext_cntrl_params[wsub_id].srchhbevt_ext_cntrl_params_mdump.srch_ops_sanity_chk_mdump.s1_nastt_pn_q_enabled[SRCH_NASTT_PN_Q] )
...
extern void srch_evt_ctl_reset( wsub_id_e_type wsub_id);
...
#define SRCHZZ_PRLL_OP_MUTEX_LOCK() REX_ISR_LOCK(&wsrch_ext_cntrl_params[wsub_id].srchzz_parallel_op_mutex_lock)
#define SRCHZZ_PRLL_OP_MUTEX_UNLOCK() REX_ISR_UNLOCK(&wsrch_ext_cntrl_params[wsub_id].srchzz_parallel_op_mutex_lock)
...
#define SRCH_GET_PN_SRCHED_CELL(idx) (wsrch_ext_cntrl_params[wsub_id].wsrch_ext_cntrl_params_mdump.srch_pn_srch_cell_ptr[idx])
#define SRCH_GET_PN_SRCH_CELL_CNT() (wsrch_ext_cntrl_params[wsub_id].wsrch_ext_cntrl_params_mdump.srch_pn_srch_cell_cnt)
...
#define SRCH_GET_PN_ENG(idx)                      (wsrch_ext_cntrl_params[wsub_id].srch_pn_ecio_tab_ptr[idx])
#define SRCH_GET_NASTT_ENG(idx)                   (wsrch_ext_cntrl_params[wsub_id].srch_nastt_ecio_tab_ptr[idx])

/* Use these Macros for the cases where Nc = 256 and Nn=16 for PN srches */
#define SRCH_GET_PN_ENG_256_16_SRCH(idx)          (wsrch_ext_cntrl_params[wsub_id].srch_nastt_pn_nonsttd_ecio_tab_ptr_nc256[idx])

/* during initial acqusition when we use Nc=128 and Nn=36 for NASTT/PN searches,
  use the following macros to get the energy */
#define SRCH_GET_NASTT_ACQ_ENG(idx,sttd)          ((sttd) ? wsrch_ext_cntrl_params[wsub_id].srch_nastt_pn_sttd_ecio_tab_ptr_nc128[idx] :      \
                                                         wsrch_ext_cntrl_params[wsub_id].srch_nastt_pn_nonsttd_ecio_tab_ptr_nc128[idx])

#define SRCH_GET_PN_ACQ_ENG(idx,sttd)             ((sttd) ? wsrch_ext_cntrl_params[wsub_id].srch_nastt_pn_sttd_ecio_tab_ptr_nc128[idx] :      \
                                                         wsrch_ext_cntrl_params[wsub_id].srch_nastt_pn_nonsttd_ecio_tab_ptr_nc128[idx])
                                                          
#define SRCH_GET_STEP1_ENG(idx)                   ((srchdch_ext_cntrl_params[wsub_id].srch_rxdiv_state) ? wsrch_ext_cntrl_params[wsub_id].srch_step1_ecio_rxd_tab_ptr[idx] : \
                                                         wsrch_ext_cntrl_params[wsub_id].srch_step1_ecio_tab_ptr[idx])
#define SRCH_GET_STEP1_NN_20_ENG(idx)             (wsrch_ext_cntrl_params[wsub_id].srch_step1_nn_20_ecio_tab_ptr[idx])
#define SRCH_GET_STEP1_NN_40_ENG(idx)             (wsrch_ext_cntrl_params[wsub_id].srch_step1_nn_40_ecio_tab_ptr[idx])
#define SRCH_GET_STEP1_NN_30_ENG(idx)             ((srchdch_ext_cntrl_params[wsub_id].srch_rxdiv_state) ? srch_step1_nn_30_srch_rxd_ecio_tab[idx] : \
                                                                                    srch_step1_nn_30_srch_ecio_tab[idx])
#define SRCH_GET_STEP1_ENG_IDLE_MODE(idx)         (wsrch_ext_cntrl_params[wsub_id].srch_step1_ecio_tab_ptr[idx])
...
#define SRCH_CONVERT_ENG_TO_ECIO_BPLMN_STTD(eng)  srch_eng_to_ecio(eng, wsrch_ext_cntrl_params[wsub_id].srch_nastt_pn_sttd_ecio_tab_ptr_nc128, wsub_id)
#define SRCH_CONVERT_ENG_TO_ECIO_BPLMN_NON_STTD(eng)  srch_eng_to_ecio(eng, wsrch_ext_cntrl_params[wsub_id].srch_nastt_pn_nonsttd_ecio_tab_ptr_nc128, wsub_id)



#define SRCH_GET_ENG_FRM_ECIO_LUT(sttd,idx)       SRCH_GET_PN_ENG(idx)
#define SRCH_CONVERT_ENG_TO_ECIO(eng,sttd)        srch_eng_to_ecio(eng, wsrch_ext_cntrl_params[wsub_id].srch_pn_ecio_tab_ptr, wsub_id)

#define SRCH_CONVERT_ENG_TO_ECIO_STEP1_DCH_NON_CPC(eng)     (                                                                                \
              (srchdch_ext_cntrl_params[wsub_id].srch_rxdiv_state) ? srch_eng_to_ecio(eng, wsrch_ext_cntrl_params[wsub_id].srch_step1_ecio_rxd_tab_ptr, wsub_id) : srch_eng_to_ecio(eng, wsrch_ext_cntrl_params[wsub_id].srch_step1_ecio_tab_ptr, wsub_id))

#define SRCH_CONVERT_ECIO_TO_ENG(ecio,sttd)       ((ecio >= ECIO_TAB_SIZ) ? SRCH_GET_PN_ENG(ECIO_TAB_SIZ-1): SRCH_GET_PN_ENG(ecio))

#define SRCH_CONVERT_ACQ_ENG_TO_ECIO(eng,sttd)        srch_eng_to_ecio(eng, wsrch_ext_cntrl_params[wsub_id].srch_nastt_pn_sttd_ecio_tab_ptr_nc128, wsub_id) 
...
#define SRCHCR_PN_SRCH_THRESH_ENG(sttd)           SRCH_GET_ENG_FRM_ECIO_LUT(sttd,SRCHPARM_PN_SRCH_ENG_THRESH_IDX)
...
#define SRCH_UNMON_NASTT_THRESH_ENG()             ((srchdch_ext_cntrl_params[wsub_id].srch_rxdiv_state) ? SRCH_GET_NASTT_ENG(SRCH_UNMON_NASTT_RXD_THRESH_IDX): \
                                                                           SRCH_GET_NASTT_ENG(SRCH_UNMON_NASTT_NO_RXD_THRESH_IDX))

#define SRCH_ASYNC_NASTT_THRESH_ENG()             ((srchdch_ext_cntrl_params[wsub_id].srch_rxdiv_state) ? SRCH_GET_NASTT_ENG(SRCH_ASYNC_NASTT_RXD_THRESH_IDX): \
...
#define SRCH_IS_ASET_UPDATE_SRCH(wsub_id)   (wsrch_ext_cntrl_params[wsub_id].wsrch_ext_cntrl_params_mdump.srch_in_asu)


#define IS_SRCH_RXDIV_ENABLED()         (srchdch_ext_cntrl_params[wsub_id].srch_rxdiv_state)
#define SET_SRCH_RXDIV_STATE(new_state) (((WL1_GET_L1M_STATE() == L1M_DCH) || (WL1_GET_L1M_STATE() == L1M_FACH) \
                                             || (WL1_GET_L1M_STATE() == L1M_BCH) || ((WL1_GET_L1M_STATE() == L1M_ACQ)) \
                                             || (WL1_GET_L1M_STATE() == L1M_BCH_SLEEP)) && \
                                             (srchdch_ext_cntrl_params[wsub_id].srch_rxdiv_state = new_state))


extern uint16 srchmeas_get_meas_idx_based_on_evt_id(
                l1_intra_freq_event_enum_type evt_id, wsub_id_e_type wsub_id);

extern void srchmeas_get_cells_for_measurement_info(uint8 carr_idx,
      l1_intra_freq_cell_list_struct_type *cell_list_ptr, uint16 meas_id, wsub_id_e_type wsub_id);

/* keep track of num EDCH active set cells */

/* Critical Section variables/calls used to protect intra freq meas database */

#define SRCHMEAS_INTRA_FREQ_DB_INTLOCK()  REX_ISR_LOCK(&wsrch_ext_cntrl_params[wsub_id].srch_intra_freq_db_crit_sect)
#define SRCHMEAS_INTRA_FREQ_DB_INTFREE()  REX_ISR_UNLOCK(&wsrch_ext_cntrl_params[wsub_id].srch_intra_freq_db_crit_sect)

/* Critical Section variables/calls used to protect inter freq meas database */

#define SRCHMEAS_INTER_FREQ_DB_INTLOCK()  REX_ISR_LOCK(&wsrch_ext_cntrl_params[wsub_id].srch_inter_freq_db_crit_sect)
#define SRCHMEAS_INTER_FREQ_DB_INTFREE()  REX_ISR_UNLOCK(&wsrch_ext_cntrl_params[wsub_id].srch_inter_freq_db_crit_sect)

/* Critical Section variables/calls used to protect freq scan substate */

#define SRCHFS_SUBSTATE_INTLOCK()  REX_ISR_LOCK(&wsrch_ext_cntrl_params[wsub_id].srchfs_scan_substate_crit_sect)
#define SRCHFS_SUBSTATE_INTFREE()  REX_ISR_UNLOCK(&wsrch_ext_cntrl_params[wsub_id].srchfs_scan_substate_crit_sect)

/* Critical Section variables/calls used to protect internal  meas database */

#define SRCH_INTERNAL_MEAS_DB_INTLOCK()  REX_ISR_LOCK(&wsrch_ext_cntrl_params[wsub_id].srch_internal_meas_db_crit_sect)
#define SRCH_INTERNAL_MEAS_DB_INTFREE()  REX_ISR_UNLOCK(&wsrch_ext_cntrl_params[wsub_id].srch_internal_meas_db_crit_sect)


/* Critical section used to arbitrate concurrent access to search module timestamp counter ext_wsrch_cntrl->srch_ts_cnt */


#define SRCHTSCNT_INTLOCK()  REX_ISR_LOCK(&wsrch_ext_cntrl_params[wsub_id].srch_ts_cnt_crit_sect)
#define SRCHTSCNT_INTFREE()  REX_ISR_UNLOCK(&wsrch_ext_cntrl_params[wsub_id].srch_ts_cnt_crit_sect)

/* Critical section used to arbitrate concurrent access to ext_srchhbevt_cntrl->srch_ops_sanity_chk */


#define SRCH_OPS_SANITY_CHK_LOCK()  REX_ISR_LOCK(&wsrch_ext_cntrl_params[wsub_id].srch_ops_sanity_chk_crit_sect)
#define SRCH_OPS_SANITY_CHK_FREE()  REX_ISR_UNLOCK(&wsrch_ext_cntrl_params[wsub_id].srch_ops_sanity_chk_crit_sect)

#ifdef FEATURE_WCDMA_CPC_DRX
/* critical section to protect search mode while in CPC DRX */


#define SRCHDCH_CPC_DRX_SRCH_MODE_LOCK()  REX_ISR_LOCK(&wsrch_ext_cntrl_params[wsub_id].srchdch_cpc_srch_mode_chk_crit_sect)
#define SRCHDCH_CPC_DRX_SRCH_MODE_FREE()  REX_ISR_UNLOCK(&wsrch_ext_cntrl_params[wsub_id].srchdch_cpc_srch_mode_chk_crit_sect)
#endif




#define SRCHTBAR_INTLOCK()  REX_ISR_LOCK(&wsrch_ext_cntrl_params[wsub_id].srch_tbar_elapsed_cnt_crit_sect)
#define SRCHTBAR_INTFREE()  REX_ISR_UNLOCK(&wsrch_ext_cntrl_params[wsub_id].srch_tbar_elapsed_cnt_crit_sect)
...
#define SRCHSCHED_INTRAF_INTLOCK()           \
  do {   \
    REX_ISR_LOCK(&wsrch_ext_cntrl_params[wsub_id].srchsched_intraf_searches_crit_sect); \
  } while (0)

#define SRCHSCHED_INTRAF_INTFREE()           \
  do {   \
    REX_ISR_UNLOCK(&wsrch_ext_cntrl_params[wsub_id].srchsched_intraf_searches_crit_sect); \
  } while (0)

#define SRCHSCHED_CNTR_INTLOCK()           \
  do {   \
    REX_ISR_LOCK(&wsrch_ext_cntrl_params[wsub_id].srchsched_intraf_counter_crit_sect); \
  } while (0)

#define SRCHSCHED_CNTR_INTFREE()           \
  do {   \
    REX_ISR_UNLOCK(&wsrch_ext_cntrl_params[wsub_id].srchsched_intraf_counter_crit_sect); \
  } while (0)
...
===========================================================================*/
extern boolean srch_resend_srch_cmd ( srch_intraf_queue_enum_type srch_queue_idx , wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean srch_is_resend_srch_needed(srch_step_enum_type srch_step, srch_ops_code_enum_type srch_type,wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srchfs_parameters_init(boolean is_list_search , wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srch_abort_all( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srch_abort_all_but_mdsp ( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean srch_check_if_abort_pending( wsub_id_e_type wsub_id);
...
=========================================================================*/
void srch_log_aset_packet_v2( wsub_id_e_type wsub_id);
...
=========================================================================*/
void srch_log_nset_packet( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srch_list_win_ctr ( srch_queue_enum_type que , wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean srch_aset_srch_cmd ( wsub_id_e_type wsub_id);

extern boolean srch_aset_srch_cmd_for_asu ( wsub_id_e_type wsub_id);
...
extern void srchdch_sort_reporting_cells( uint8 carr_idx,uint16 meas_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srch_filter_srch_result( srch_queue_enum_type que , wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srch_filter_s3_result( srch_cell_struct_type *cell_ptr , wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srch_calculate_cpich_rscp (srch_cell_struct_type *cell_ptr, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean srch_event_timer_exp (uint16 start_ts, uint16 time_to_trig, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srch_print_debug_srch_params( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srchacq_set_vctcxo_parms_flag( boolean val, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean srchacq_get_reset_vctcxo_param_flag( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern srchacq_bin_type srchacq_get_bin_type( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void srchacq_init_vctcxo_and_rotator(boolean read_rot_from_tcxo_mgr, wsub_id_e_type wsub_id);
...
extern uint32 srchdch_calculate_tm(srch_cell_struct_type *cell_ptr, wsub_id_e_type wsub_id);

extern void srchdch_adjust_off_and_tm(srch_cell_struct_type *cell_ptr, wsub_id_e_type wsub_id);

#ifdef FEATURE_WCDMA_CNT_C_SFN_DIFF
extern uint16 srchdch_get_cnt_c_sfn_diff(srch_cell_struct_type *cell_ptr, wsub_id_e_type wsub_id);
#endif

#ifdef FEATURE_REL5
extern void srchdch_send_initial_evt_1d_meas_report ( uint8 carr_idx, uint16 meas_idx , wsub_id_e_type wsub_id);
#endif

extern void srchmeas_reset_intraf_meas_dbase(uint8, wsub_id_e_type wsub_id);
extern uint16 srchmeas_get_oldest_uset_idx(uint8, wsub_id_e_type wsub_id);

extern void srchmeas_initialise_default_filter( wsub_id_e_type wsub_id);
...
#define GET_FILT_IDX_FOR_SFN_DEC() (srchdch_filt_idx_for_interf_irat_meas(SRCH_DEFAULT_FILTER_IDX ,wsub_id))
/*===========================================================================
...
===========================================================================*/
extern void srchzz_proc_srch_done(srch_done_cmd_type *cmd_ptr, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srchzz_proc_srch_ops(srch_ops_cmd_type  *cmd_ptr, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srchdch_sort_active_set(uint8, l1_meas_quan_enum_type quan_type,uint16 filt_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srchdch_reset_evt_rpt_dbase(uint8, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srchdch_remove_from_evt_rpt_cell_list(srch_cell_struct_type *cell_ptr, wsub_id_e_type wsub_id);

/* Prototypes of event evaluation functions */
extern boolean srchdch_evt_1a_1b_eval(srch_cell_struct_type *cell_ptr, uint16 meas_idx, wsub_id_e_type wsub_id);
extern boolean srchdch_evt_1c_eval(srch_cell_struct_type *cell_ptr, uint16 meas_idx, wsub_id_e_type wsub_id);
extern boolean srchdch_evt_1d_eval(srch_cell_struct_type *cell_ptr, uint16 meas_idx, wsub_id_e_type wsub_id);
extern boolean srchdch_evt_1e_1f_eval(srch_cell_struct_type *cell_ptr, uint16 meas_idx, wsub_id_e_type wsub_id);
extern boolean srchdch_evt_1j_eval(srch_cell_struct_type *cell_ptr, uint16 meas_idx, wsub_id_e_type wsub_id);
extern void srchdch_setup_initial_evt_1a_meas_for_aset ( wsub_id_e_type wsub_id);
extern void srchdch_setup_initial_evt_1e_meas_for_aset ( wsub_id_e_type wsub_id);

#ifdef FEATURE_MDSP_MULTI_RF_CHAIN_INTERFACE
extern void srch_set_div_ant_delay(int16 new_delay, wsub_id_e_type wsub_id);
#endif /* FEATURE_MDSP_MULTI_RF_CHAIN_INTERFACE */

extern boolean srch_find_tx_div_ind(uint16 scr_code, uint8 carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
uint16 srchdch_filt_idx_for_interf_irat_meas(uint16, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  i
===========================================================================*/
extern void srchdch_reset_filt_params_all_timing_known_cells(uint16 filt_idx_new, wsub_id_e_type wsub_id);
...
=======================================================================*/
extern void srchmeas_initialize_cells_for_meas_idx(uint8 carr_idx,uint16 meas_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srch_handle_txd_det_done(uint8 notify_id, boolean status, wsub_id_e_type wsub_id);
...
===========================================================================*/         
extern void srchacq_pn_search_for_best_eng_task(uint16 num_tasks, uint16 peaks_per_task, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srch_pn_determine_nc_nn(srch_cell_struct_type  * cell_ptr, uint16 *nc, uint16 *nn, wsub_id_e_type wsub_id);
 ...
 =========================================================================*/
extern boolean srch_send_srch_cleanup_done( wsub_id_e_type wsub_id);
...
/*=========================================================================
 ...
 =========================================================================*/
extern srch_step_enum_type srch_map_mcal_srch_to_whl1_srch_type(mcalwcdma_srch_type srch_type, wsub_id_e_type wsub_id);
...
=========================================================================*/
extern void srch_acq_set_num_freq_hypo(uint8 num_hypo, wsub_id_e_type wsub_id);
...
=========================================================================*/
extern void srch_acq_set_freq_hypo(uint8 idx, uint16 val, wsub_id_e_type wsub_id);

#ifdef FEATURE_WTOW_BACKGROUND_PLMN_SEARCH
extern void srch_bplmn_set_samp_serv_freeze(srch_step_enum_type srch_step, wsub_id_e_type wsub_id);
#endif

extern boolean srch_deconfig_all_cells_during_state_transition( l1m_state_enum_type next_state , wsub_id_e_type wsub_id);

extern void srch_set_samp_serv_freeze(srch_step_enum_type srch_step, wsub_id_e_type wsub_id);
...
extern void srch_step1_post_processing(srch_state_enum_type srch_state, wsub_id_e_type wsub_id);
...
DESCRIPTION
  ...
  ext_srchmeas_cntrl->srch_intra_freq_meas_db.    
...
extern srch_intra_freq_meas_parms_struct_type* 
      srchmeas_get_intra_freq_evt_info_for_vaset(l1_intra_freq_event_enum_type VASET_EVT, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srch_update_pn_sync_parms( wsub_id_e_type wsub_id);

extern void srch_mcal_whl1_status_cb(mcalwcdma_srch_status_type status, srch_intraf_queue_enum_type srch_queue_idx, wsub_id_e_type wsub_id);

extern void srch_mcal_handle_no_srch_cb(mcalwcdma_srch_status_type status, srch_intraf_queue_enum_type srch_queue_idx, wsub_id_e_type wsub_id);

/* RxD callback, gets called once a search with RxD ON completes so that RxDiv module can turn
 * OFF the div path.  
 */
typedef void SRCH_RXD_TRANS_IND_CB_FUNC_TYPE (boolean srch_rxd_trans_success,wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srch_update_rxd_info(boolean enabled, SRCH_RXD_TRANS_IND_CB_FUNC_TYPE *rxd_proc_srch_cb, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean srch_send_cmd_for_rxd_cb( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srch_process_sample_server_cmd_done (uint16 status, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean srch_is_rxd_enabled_on_nastt( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srch_speed_up_full_srch( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern srch_cell_txd_status srch_determine_cpich_tx_div(l1_setup_cmd_type *setup, uint16 scr_code, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean srch_determine_cpich_tx_div_for_dc(uint16 freq, uint16 scr_code, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srch_override_freq_cmn_in_fw(uint16 val, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean srch_should_neighbor_searches_run( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern uint16 srch_get_aset_rscp( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern uint16 srch_get_aset_rscp_prim( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern uint8 srch_get_aset_ecio( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern uint16 srch_get_aset_rscp_div( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern uint8 srch_get_aset_ecio_div( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern uint8 srch_get_aset_ecio_prim( wsub_id_e_type wsub_id);
...
=========================================================================*/
extern int16 srch_get_s_intra_search( wsub_id_e_type wsub_id);
...
=========================================================================*/
extern int16 srch_get_s_inter_search( wsub_id_e_type wsub_id);
...
=========================================================================*/
extern int16 srch_get_s_interrat_search( wsub_id_e_type wsub_id);
...
=========================================================================*/
extern int16 srch_get_rxagc(rfm_path_type rfm_transceiver,uint8 carr_idx , wsub_id_e_type wsub_id);
...
=========================================================================*/
extern uint16 srch_get_rounded_rscp(rfm_path_type rfm_path, uint16 ecio,uint8 carr_idx, wsub_id_e_type wsub_id);
...
=========================================================================*/
extern int16 srch_get_rxagc(rfcom_device_enum_type rfcom_transceiver,uint8 carr_idx , wsub_id_e_type wsub_id);
...
=========================================================================*/
extern uint16 srch_get_rounded_rscp(rfcom_device_enum_type rfcom_transceiver, uint16 ecio,uint8 carr_idx, wsub_id_e_type wsub_id);

#endif /* (FEATURE_WCDMA_ATLAS_MODEM) && (FEATURE_WCDMA_RFM_PATH_SUPPORTED) */

/*=========================================================================

FUNCTION     : srch_get_better_rxagc_val_dbm 
               
DESCRIPTION  : This function returns the higher RxAGC value
  
DEPENDENCIES : None

RETURN VALUE : RxAGC in dBm
      
SIDE EFFECTS : None

=========================================================================*/
extern int16 srch_get_better_rxagc_val_dbm(wsub_id_e_type wsub_id);
...
=========================================================================*/
extern uint16 srch_get_cell_current_ecio(srch_cell_struct_type *cell_ptr, wsub_id_e_type wsub_id);
...
=========================================================================*/
extern uint16 srch_get_cell_current_rscp(srch_cell_struct_type *cell_ptr, wsub_id_e_type wsub_id);
...
=========================================================================*/
extern void wcdma_process_signal_info( wsub_id_e_type wsub_id);
...
===========================================================================*/ 
extern void wsrch_update_cell_ref_pos(srch_cell_struct_type *cell_ptr, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  ...
  4. ecio
===========================================================================*/
extern uint32 srch_combine_path_engy( srch_cell_struct_type *cell_ptr, uint8 ant_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS    
===========================================================================*/
extern void srch_control_measurements(uint8 carr_idx,boolean start_meas, wsub_id_e_type wsub_id);
...
SIDE EFFECTS    
===========================================================================*/
extern boolean srch_get_measurement_status(uint8 carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern int16 srch_get_rxagc_from_upd_rf_api(srch_rxagc_val_units_enum rxagc_units, boolean for_div,uint8 carr_idx, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
===========================================================================*/
extern boolean srch_conn_state_cleanup(l1m_state_enum_type next_state, wsub_id_e_type wsub_id);
/*===========================================================================
...
SIDE EFFECTS    
===========================================================================*/
extern boolean srch_validate_ref_pos_update(srch_cell_struct_type *cell_ptr, uint32 candidate_pos, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srch_crash_on_unxpectd_results_from_srch(boolean pn_srch, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srch_disable_debug_crash_on_srch_done( wsub_id_e_type wsub_id);


/*===========================================================================

FUNCTION SRCH_DISABLE_DENY_TRM_LOCK_PER_RAT
...
===========================================================================*/
extern void srch_disable_deny_trm_lock_per_rat( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srch_init_rumi_bringup_db( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srch_enable_and_update_crash_info(uint8 wl1_state_now, uint8 bmask, uint8 carr_idx, 
        uint8 type_of_search, uint8 thresh, uint16 freq, uint16 psc, wsub_id_e_type wsub_id);



/*===========================================================================

FUNCTION SRCH_ENABLE_AND_UPDATE_DENY_TRM_LOCK_PER_RAT_INFO

DESCRIPTION
  This function enables the trm lock per rat and updates the structure
  with the trm lock per rat info received from the QxDM command. This function will be called
  when the qxdm command is sent.
...
===========================================================================*/
extern void srch_enable_and_update_deny_trm_lock_per_rat_info(uint8 irat_type_bmask, uint8 trm_lock_deny_number, wsub_id_e_type wsub_id);
/*===========================================================================
...
===========================================================================*/
extern void srch_enable_and_update_attenuate_info(uint8 attenuate_value, uint8 attenuated_sub_id, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void srch_async_parm_init_on_rxd_active( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern boolean srch_is_dc_configured_in_ul( wsub_id_e_type wsub_id);
#endif
...
SIDE EFFECTS
  None.
===========================================================================*/
extern boolean srch_skip_eval_on_carrier(uint8 carr_idx, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srch_proc_rxd_upd_cb_cmd( wsub_id_e_type wsub_id);
...
===========================================================================*/
extern int16 srch_get_max_rscp(int16 rscp_pri, int16 rscp_div, uint8 sttd, wsub_id_e_type wsub_id);

#ifdef FEATURE_DUAL_SIM
extern void srch_triage_set_srch_done_non_drx(srch_done_cmd_type *cmd_ptr, wsub_id_e_type wsub_id);
#endif
...
SIDE EFFECTS    
===========================================================================*/
extern void srch_update_desense_status(boolean desense_info, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srch_prune_step1_result_for_false_peaks(wsub_id_e_type wsub_id);
...
===========================================================================*/
extern void srch_remove_all_pruned_peaks(wsub_id_e_type wsub_id);
#endif


/*===========================================================================
FUNCTION     SRCH_INTERF_K_VAL_LOG

DESCRIPTION
  This function logs srch_interf_meas_k_val during various stages like DCH_INIT,
MCM Processing, Multi_carr_config, Stack Init for debugging purposes.
...
SIDE EFFECTS
  None
===========================================================================*/
extern void srch_interf_k_val_log(srchinterf_sub_module_enum_type module_id_, wsub_id_e_type wsub_id_);

/*===========================================================================
 
 FUNCTION        SRCHACQ_DETERMINE_CONN_IDLE_ACQ
 
 DESCRIPTION
   This function determines if current acq request from RRC is for connected to idle transition 
 
 DEPENDENCIES
 RRC command pointer and subscription ID
 ...
 ===========================================================================*/
extern boolean srchacq_determine_conn_idle_acq(l1_acq_cmd_type* cmd, wsub_id_e_type wsub_id);

/*===========================================================================
  FUNCTION     srch_compute_ecio_based_on_nc_nn

  DESCRIPTION
  This function computes and returns the correct the ECIO value
...
extern uint16 srch_compute_ecio_based_on_nc_nn(uint32 eng,srch_cell_struct_type* cell_ptr, wsub_id_e_type wsub_id);
#endif

#ifdef FEATURE_WCDMA_W2L_PSEUDO_MEAS
/*===========================================================================
  FUNCTION     SRCH_COMPUTE_RXAGC_PSEUDO_MEAS

  DESCRIPTION
  This function computes total rxagc to be reported to RRC for pseudo meas support 
  ...
  None
  ===========================================================================*/
void srch_compute_rxagc_pseudo_meas(wsub_id_e_type wsub_id);
#endif
