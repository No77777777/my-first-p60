...
$Header: //components/rel/wcdma.mpss/8.4.0/api/rrcrrif.h#3 $   
...
FUNCTION          rrc_check_mcc_cnfgd_in_fallback_nv

DESCRIPTION       RR/RRC uses this API to check if the MCC is configured in the NV 'WNV_ID_RRC_GSM_FALLBACK_INFO'

DEPENDENCIES
                  None
                  
RETURN VALUE
                 TRUE:  if MCC is configured in NV (or) no Valid MCC entries are configured in NV.
...
boolean rrc_check_mcc_cnfgd_in_fallback_nv(uint16 camped_mcc, sys_modem_as_id_e_type as_id);
