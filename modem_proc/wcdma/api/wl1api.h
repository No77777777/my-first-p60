...
$Header: //components/rel/wcdma.mpss/8.4.0/api/wl1api.h#3 $
$DateTime: 2020/01/24 03:56:36 $
$Author: pwbldsvc $

when       who    what, where, why
-------    ---    ---------------------------------------------------------- 
12/21/15   ar     Add function to indicate if FW app can be disabled per SUB ID
11/20/15   ar     Add API to send HS-DPCCH Config Command to FW in FTM mode
11/03/15   rn     Added Change to Remove HARDCODE_SUB_ID. 
10/22/15   jk     Featurizing RF Config command only for pre-Atlas targets. 
08/13/15   gm     AS_ID cleanup.
07/01/15   ar     Changes for FR26888 : MTPL Backoff for desense
...
--------------------------------------------------------------------------*/
typedef void (*post_cb_fn)(uint16 status, wsub_id_e_type wsub_id);
typedef void (*pre_cb_fn)( wsub_id_e_type wsub_id);

  /*Callback to manage old prototype */
typedef void (*post_cb_fn_old)(uint16 status);
typedef void (*pre_cb_fn_old)( void);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern boolean wl1_dsds_allow_geran_xomgr_update(uint32 lock_held_ms, wsub_id_e_type wsub_id );
...
(
  ...
  pre_cb_fn_old pre_cb,

  /* post-command callback function */
  post_cb_fn_old post_cb
 );
...
===========================================================================*/
extern int16 demoddrv_retrieve_freqcommon( wsub_id_e_type wsub_id);
...
FUNCTION        WL1_MDSP_SYNC_SEND_TXLM_CFG_CMD
...
===========================================================================*/
extern boolean wl1_mdsp_sync_send_txlm_cfg_cmd(
...
FUNCTION        hscfg_send_hs_ul_cfg_cmd_for_ftm

DESCRIPTION     This function allows RF to set up HSDPCCH in FTM mode. 
                It will send HS-DPCCH config command to FW with hard-coded parameters
                The DPCCH offset can be set by RF based on FTM requirement.
...
===========================================================================*/
extern void hscfg_send_hs_ul_cfg_cmd_for_ftm(boolean enable, uint8 dpcch_offset, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean wl1_is_camped_in_idle(wsub_id_e_type wsub_id);
...
/*===========================================================================
FUNCTION     WL1TRM_ALLOW_WCDMA_FW_APP_DISABLE
 
DESCRIPTION
  This function is called during X2W searcher deinit to check if WFW can be disabled
...
RETURN VALUE
  TRUE  - OK to disable FW_APP_WCDMA.
  FALSE - NOT OK disable FW_APP_WCDMA.
===========================================================================*/
extern boolean wl1trm_allow_wcdma_fw_app_disable(sys_modem_as_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint32 wl1_stmr_get_current_rx_tl_time( wsub_id_e_type wsub_id );
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean wl1_l1m_cphy_setup_in_progress( wsub_id_e_type wsub_id);
...
#ifdef FEATURE_WCDMA_MTPL_BACKOFF_DESENSE
/*===========================================================================
FUNCTION wl1_set_mcs_tx_pwr_limit

DESCRIPTION
  This is a wrapper function to set the MCS MTPL to RF
  It performs sanity check to ensure that MCS MTPL is valid,
  else the regular MTPL is written to FW.
...
SIDE EFFECTS
  FW-SW interface is updated with the MCS MTPL
===========================================================================*/
extern void wl1_set_mcs_tx_pwr_limit(int16 tx_limit_dbm);
#endif //FEATURE_WCDMA_MTPL_BACKOFF_DESENSE
