...
$Header: //components/rel/wcdma.mpss/8.4.0/api/l1rrcif.h#11 $ $DateTime: 2024/09/13 04:51:44 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
01/30/20   ak      Made changes for exposing API to L1 to get the current requested PLMN ID.
...
04/02/18   ss      Workaround for bad DC config sent by NW
06/15/17   am      Ignore wakeup cancel if RRC is about to toggle SIB priority.
02/27/18   th      Addition of new wrm activities
11/17/17   nc      l1_get_serving_cell_power API changes for is_camped flag
05/15/17   kr      Send FW to STOP instead of Sleep in 1xSRLTE mode
11/03/17   th      Changes to remove LTE capability indication
08/16/17   ss     Changes for LTA timeline optimization
08/23/17   nc      l1_get_serving_cell_power API changes for is_stale flag
08/05/16   sp      Provide API to RRC to check if bplmn is still active at L1
05/18/16   ad      Added API to get the LTA overhead duration
03/03/16   nga     Add an API to update sib3 params 
03/02/16   as      Added support for loop back mode 4
02/22/16   sr      Changes to handle modify band fail for cell resel failure case
02/16/16   nga     Do not clear nbr list if reselection failed due to INTERNAL_FAIL_CAUSE from RRC
...
12/09/15   ad      Added return type to rrcwrm_update_rrc_lock_status API
12/10/15    nga     Adding one more resel fail cause L1_RESEL_FAIL_L2_ACK_WAIT
11/28/15   svh     Expose new API to RRC to check whether MDSP Recovery is in progress.
11/19/15   sl      New API to determine if WL1 can handle ACT CANCEL CMD or not
11/18/15   pg      Mods to not release lock for START_CNF if RRC has sent a
                   valid band information 
11/03/15   ad      Made changes to introduce new activity for CGI request
11/05/15   sp      Made changes to expose RRC API to check if trch is mapped to voice rab
11/04/15   sr      Made changes to indicate if W sub is multimode or not
11/03/15   rn      Passing sub_id as argument for l1_get_rssi() 
10/26/2015 vs      Made changes to remove the enum featurization
10/16/15   nr      Added a macro which holds the min time for WL1 to send CNF during QTA to WRRC
10/12/15   rsr     Added Wrapper for RRC to update LTE Capability Ind.
09/23/15   ad      Expose API to stop the block QTA timer to RRC. 
                   Removed changes in L1 to stop the block QTA timer
09/14/15   gm      Mods to add an API to reutrn default band to RRC.
09/02/15   ac      Change hs_get_cur_state() to hs_is_hsdpa_active(sub_id)
08/17/15   skk     Updated CTCH activities. 
08/17/15   ar      Add interface to include wrm activity as part of CPHY_ACQ_CMD
08/11/15   rsr     Added Support to ensure that FW is put to Sleep, prior 
                   to Entry of Traffic State. 
08/04/15   vs      Changes to support InterF and IRAT wrm activity.
07/22/15   gm      mods to add new RRC activity RRC_SERVICE_SEARCH_INV
07/09/15   gm      RRC APIs to return unlock type and to clear it.
07/01/15   ar      TRM-U Phase3 enhancements
06/25/15   ar      Add support to send info about the activity as part of start_wcdma
07/02/15   ms      Fix to use wcdma_convert_stack_index_to_subid for AS_ID
06/05/15   gm      Mods to take care of delay that may be caused due to zipping of RRC files by qzip.
04/27/15   nga     Correction in logging of resel end event
04/21/15   jd      tcxo cleanup.
03/17/15   sr      Indicate LTE capability to WL1
03/09/15   rsr     W+W Api Interface Change.
02/28/15   rsr     W+W Phase 2 Checkin.
12/03/14   sp      Made changes to save one byte status in cmd_ptr to crash debug global
10/17/14   sd      wl1_qta_overhead_time_in_ms comments
...
#define WCDMA_QTA_START_STOP_DEADLOCK_TIME_IN_MS      500
#endif
...
/* Max EUTRA freq to avoid decode failures for pseudo meas*/
#define L1_EUTRA_MAX_FREQ 2
...
#define L1_MEAS_ID_VALID(measid)    ( ( ( ((uint16) (measid)) > 0 ) \
                                       ...
                                       || ( ((uint16) (measid)) == RRC_PSEUDO_MEAS_ID ))
#else
#define L1_MEAS_ID_VALID(measid)    ( ( ((uint16) (measid)) > 0 ) \
                                      && ((uint16) (measid)) <= L1_MAX_MEAS_ID )
#endif
...
typedef enum
{
  W2L_PSEUODO_MEAS_INVALID = 0,
  W2L_PSEUODO_MEAS_ACTIVATED = 1,
  W2L_PSEUODO_MEAS_DEACTIVATED = 2
}w2l_pseudo_meas_status_e_type;

/* Compressed mode Transmission gap pattern information for all seq. */
typedef struct
{
  w2l_pseudo_meas_status_e_type w_to_l_fake_meas_active;
...
typedef enum
{
  ...
  L1_LOOPBACK_MODE_4  /**< loopback mode 4 */
...
typedef enum
{
  ...
  L1_IRAT_IHO,
  L1_IRAT_CCO,
  L1_IRAT_DUAL_SIM_TUNEAWAY, /* FEATURE_DUAL_SIM */
  L1_IRAT_NONE
...
typedef struct
{
  ...
  /*Indicates if the measurement is stale*/
  boolean is_stale;

  /*Indicates if the sub has camped*/
  boolean is_camped;
...
typedef enum
{
 ...
 /*Dual SIM Commands*/
  ...
  CPHY_START_QTA_REQ              =   0x36, /* FEATURE_QTA */
  
  CPHY_STOP_QTA_REQ               =   0x37, /* FEATURE_QTA */
...
typedef struct
{
  ...
  w2l_pseudo_meas_status_e_type w2l_pseudo_meas_status;
...
/* enum for all the activities to be used between L1 <> RRC WRM interactions */
typedef enum
{
  ...
  WL1_ACTIVITY_SETUP_NBCH_INTRA_F = 10,
  WL1_ACTIVITY_SETUP_NBCH_INTER_F = 11,
  WL1_ACTIVITY_SETUP_PCH = 12,
  WL1_ACTIVITY_SETUP_CTCH_FACH = 13,
  WL1_ACTIVITY_SETUP_DTCH_FACH = 14,
  WL1_ACTIVITY_SETUP_DCH = 15,
  WL1_ACTIVITY_PAGING = 16,
  WL1_ACTIVITY_CELL_RESEL_START = 17,
  WL1_ACTIVITY_CELL_TRANS_REQ = 18,
  WL1_ACTIVITY_CHANNEL_ACTIVE = 19,
  WL1_ACTIVITY_BPLMN = 20,
  WL1_ACTIVITY_BPLMN_BCCH_DROP = 21,
  WL1_ACTIVITY_GEN_CPHY_SETUP_REQ = 22,
  WL1_ACTIVITY_CELL_SELECTION_REQ = 23,
  WL1_ACTIVITY_DRX = 24,
  WL1_ACTIVITY_REQUEST_PRIMARY = 25,
  WL1_ACTIVITY_DATA_CALL = 26,
  WL1_ACTIVITY_ACQ_INV = 27,
  WL1_ACTIVITY_IDLE_NGBR_MEAS = 28,
  WL1_ACTIVITY_IDLE_NGBR_MEAS_INTRA_F = 29,
  WL1_ACTIVITY_IDLE_NGBR_MEAS_INTER_F = 30,
  WL1_ACTIVITY_CM_NGBR_MEAS_INTER_F = 31,
  WL1_ACTIVITY_NGBR_MEAS_IRAT = 32,
  WL1_ACTIVITY_RX_DIVERSITY = 33,
  WL1_ACTIVITY_REQUEST_PRIMARY_CA = 34,
  WL1_ACTIVITY_REQUEST_SECONDARY_CA = 35,
  WL1_ACTIVITY_BCCH_DECODE = 36,
  WL1_ACTIVITY_CTCH =37,
  WL1_ACTIVITY_CTCH_INV = 38,

  /* RRC's activity's list */
  RRC_VOICE_CALL  =  39,           /*SYS_PROC_TYPE_CS_CALL*/                /*TRM_TRAFFIC*/
  RRC_PS_REGISTRATION  =  40,      /*SYS_PROC_TYPE_PS_REGISTRATION*/        /*TRM_ACCESS*/
  RRC_CS_REGISTRATION  =  41,      /*SYS_PROC_TYPE_CS_REGISTRATION*/        /*TRM_ACCESS*/
  RRC_SERVICE_SEARCH  =  42,       /*SYS_PROC_TYPE_ACQUISITION*/            /*TRM_ACQUISITION*/
  RRC_OOS_SEARCH  =  43,           /*SYS_PROC_TYPE_OOS_SCAN*/               /*TRM_ACQUISITION*/
  RRC_DATA_CALL  =  44,            /*SYS_PROC_TYPE_PS_CALL_USER_PLANE*/     /*TRM_BG_TRAFFIC*/
  RRC_PS_HANDOVER  =  45,          /*SYS_PROC_TYPE_PS_HANDOVER*/            /*TRM_ACCESS*/
  RRC_CSFB_CALL  =  46,            /*SYS_PROC_TYPE_CSFB*/                   /*TRM_TRAFFIC*/
  RRC_IRAT_REDIRECTION  =  47,     /*SYS_PROC_TYPE_REDIRECTION*/            /*TRM_ACQUISITION*/
  RRC_IRAT_RESELECTION  =  48,     /*SYS_PROC_TYPE_IRAT_CELL_RESELECTION*/  /*TRM_ACQUISITION*/
  RRC_BPLMN_SEARCH  =  49,         /*SYS_PROC_TYPE_BPLMN_SEARCH*/           /*TRM_ACQUISITION*/
  RRC_MPLMN_SEARCH  =  50,         /*SYS_PROC_TYPE_MANUAL_PLMN*/            /*TRM_ACQUISITION*/
  RRC_SMS  =  51,                  /*SYS_PROC_TYPE_SMS*/                    /*TRM_ACCESS*/
  RRC_SUPPL_SERVICE  =  52,        /*SYS_PROC_TYPE_SUPPLEMENTARY_SERVICE*/  /*TRM_ACCESS*/
  RRC_PS_SIGNALLING  =  53,        /*SYS_PROC_TYPE_PS_CALL_SIGNALING*/      /*TRM_ACCESS*/
  RRC_CELL_CHANGE_ORDER  =  54,    /*SYS_PROC_TYPE_CCO*/                    /*TRM_ACCESS*/
  RRC_SRVCC_CALL  =  55,           /*SYS_PROC_TYPE_SRVCC*/                  /*TRM_TRAFFIC*/
  RRC_CSG_SEARCH  =  56,           /*SYS_PROC_TYPE_CSG_SEARCH*/             /*TRM_ACQUISITION*/
  RRC_CS_HANDOVER  =  57,          /*SYS_PROC_TYPE_CS_HANDOVER*/            /*TRM_ACCESS*/
  RRC_HIGH_PRIO_PS  =  58,         /*SYS_PROC_TYPE_HIGH_PRIORITY_PS_CALL*/  /*TRM_TRAFFIC*/
  RRC_SERVICE_SEARCH_INV = 59,     /*SYS_PROC_TYPE_ACQUISITION*/            /*TRM_ACQUISITION_INV*/
  RRC_IRAT_CGI = 60,               /*SYS_PROC_TYPE_CGI*/                   /*TRM_ACQUISITION_INV*/
  RRC_MMS = 61,                    /*SYS_PROC_TYPE_MMS*/                   /*TRM_ACCESS*/
  RRC_NON_IMS_OTHERS = 62,         /*SYS_PROC_TYPE_NON_IMS_OTHERS*/        /*TRM_ACCESS*/
  RRC_RS_RA = 63,                  /*SYS_PROC_TYPE_RS_RA*/                  /*TRM_ACCESS*/
  WRM_MAX_ACTIVITIES = 64
...
typedef struct
{
  ...
  boolean lte_capability_ind;  
  wrm_activity_enum_type reason;  /*Reason why START cmd is being sent to L1*/
...
#define MAX_RESULTS_EARFCN 8
typedef struct
{
  uint8 count;
  uint32 EARFCN[MAX_RESULTS_EARFCN];
}l1_pseudo_meas_results_type;
...
*/
  ...
  /*This field is used by RRC/L1 to populate the activity for which we are processing
  this ACQ cmd. This will be populated by RRC/L1 when we take/release lock*/
  wrm_activity_enum_type  acq_activity;
...
typedef enum
{
  ...
  L1_SETUP_STATUS_WRM_LOCK_FAIL, /* FEATURE_DUAL_SIM */
  L1_SETUP_STATUS_MODIFY_BAND_FAIL, /* FEATURE_DUAL_SIM */
...
#define  RRC_MAX_MCC_COUNT                3
#define  RRC_MAX_MNC_COUNT                3
/*-------------------------------------------------------------------
STRUCTURE: rrc_plmn_id_type
...
typedef struct
{
  ...
  byte mcc[RRC_MAX_MCC_COUNT];
  /* Number of MNC digits */
  byte num_mnc_digits;
  /* Mobile Network Code - one digit per byte (range 0..9) */
  byte mnc[RRC_MAX_MNC_COUNT];
}rrc_plmn_id_type;
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean l1_rrc_is_common_edch_resrc_active(wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern l1_eutra_det_state_enum_type rrc_l1_get_eutra_detect_status(wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None
===========================================================================*/
void l1_get_rssi(l1_rssi_info_struct_type *rssi, wsub_id_e_type wsub_id );
...
SIDE EFFECTS
  None
===========================================================================*/
boolean l1_get_mobility_info(wsub_id_e_type wsub_id);
...
extern uint16 rrc_get_hsupa_category(sys_modem_as_id_e_type wrrc_as_id);
...
/*=============================================================================
  ...
  SIDE EFFECTS : None
=============================================================================*/
void l1_get_eutra_measurement_data(lte_irat_timing_tx_info_s* p_msmt_data, wsub_id_e_type wsub_id);
...
=========================================================================*/
void l1_get_interrat_measurement_data( rrc_rr_interrat_measurement_data_type * p_msmt_data, wsub_id_e_type wsub_id);
...
(
 uint16 meas_id, wsub_id_e_type wsub_id
 );
...
SIDE EFFECTS:
  None.
====================================================================*/
extern void srchmeas_set_inter_freq_measdb_status_during_state_transition(boolean status, wsub_id_e_type wsub_id);
...
extern boolean l1_get_additional_interf_msmt_results(
            uint32  meas_id,
            l1_inter_freq_meas_rpt_struct_type *p_msmtres , wsub_id_e_type wsub_id );
...
extern boolean l1_get_additional_internal_msmt_results(
            uint32  meas_id,
            l1_ue_internal_meas_rpt_struct_type *p_msmtres , wsub_id_e_type wsub_id );
...
SIDE EFFECTS
  None.
===========================================================================*/
extern uint16 l1_wcdma_get_trk_lo_adj_pdm( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern boolean l1_get_reference_cell_info(uint16 *freq, uint16 *psc, wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None.
===========================================================================*/
extern boolean mdsp_is_wcdma_image_loaded( wsub_id_e_type wsub_id);
...
===========================================================================*/
boolean l1_get_best_psc_in_aset(uint16 *scr_code, uint32 *pn_pos, wsub_id_e_type wsub_id );
...
===========================================================================*/
boolean l1_get_hs_psc_sib_read(uint16 *scr_code, uint32 *pn_pos, wsub_id_e_type wsub_id );

/*===========================================================================
FUNCTION hs_is_hsdpa_active
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean hs_is_hsdpa_active(wsub_id_e_type wsub_id);
...
extern uint8 hs_get_num_configured_carriers( wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None
===========================================================================*/
extern uint8 dl_phychan_cctrch_id_in_use(boolean *dpch_active,   wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None
===========================================================================*/
extern boolean ul_phychan_is_ul_active ( wsub_id_e_type wsub_id );
...
SIDE EFFECTS    None
===========================================================================*/
extern boolean ul_phychan_is_ul_dpch_active (wsub_id_e_type wsub_id);
...
SIDE EFFECTS    None
===========================================================================*/
extern boolean ul_phychan_is_ul_prach_active ( wsub_id_e_type wsub_id );


/*===========================================================================

FUNCTION          rrc_check_if_csfb_call_active

DESCRIPTION       This function is called by L1 in to check the status of CSFB call

DEPENDENCIES      NONE

RETURN VALUE      
                  TRUE:  CSFB call ative
                  FALSE: Otherwise
                                  
SIDE EFFECTS

===========================================================================*/
extern boolean rrc_check_if_csfb_call_active(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
extern boolean l1_rrc_is_bplmn_active(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
extern boolean rrciho_is_gtow_ho_active(sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
extern boolean rrcpg1_modify_sibs_for_sfn_update( sys_modem_as_id_e_type wrrc_as_id );
...
===========================================================================*/
extern boolean l1m_state_is_fach(wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean hs_is_supported( wsub_id_e_type wsub_id );
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean hs_get_mc_state(l1_hsdpa_carrier_enum_type carrier, wsub_id_e_type wsub_id);
...
extern boolean rrc_sib_can_l1_start_fmo (sys_modem_as_id_e_type wrrc_as_id);
...
===========================================================================*/
extern boolean rrcllc_check_srb_mapped_to_hsdsch_present(sys_modem_as_id_e_type wrrc_as_id);
/*============================================================================
...
SIDE EFFECTS:
=============================================================================*/
extern tr_ch_id_type rrcllc_get_ctch_trch_id(sys_modem_as_id_e_type wrrc_as_id);
/*===========================================================================
...
SIDE EFFECTS    None
===========================================================================*/
void l1_get_rssi_during_wtow_bplmn(l1_rssi_info_struct_type *rssi, wsub_id_e_type wsub_id);
...
extern boolean l1_cm_query_compressed_mode_active(wsub_id_e_type wsub_id);
...
extern uint8 l1_cm_query_num_of_cm_patterns_active(wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean rrc_l1_query_is_amr_call_enabled(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS
  None
===========================================================================*/
extern boolean rrcwrm_release_rrc_lock_on_l1_req( sys_modem_as_id_e_type wrrc_as_id );
...
SIDE EFFECTS
  None.
===========================================================================*/
extern boolean rrcwrm_update_rrc_lock_status(sys_modem_as_id_e_type wrrc_as_id);
...
extern void rrc_persistent_page_collision_event_cb
(
  sys_modem_as_id_e_type wrrc_as_id, 
...
(
  uint16 freq, uint16 scr_code, wsub_id_e_type wsub_id
 );
...
(
  ...
  boolean *is_non_whte_csg, boolean *is_bar_resel, wsub_id_e_type wsub_id
 );
...
SIDE EFFECTS:
=============================================================================*/
void wl1_clear_barred_cells(wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean wl1_ok_for_tuneaway( wsub_id_e_type wsub_id );
...
RETURN VALUE
  boolean - TRUE if tuneaway request approved.  FALSE if not.
===========================================================================*/
extern boolean wl1_ok_for_qta( wsub_id_e_type wsub_id );
...
===========================================================================*/
extern void rrc_release_lock( sys_modem_as_id_e_type wrrc_as_id );
...
SIDE EFFECTS
  None.
===========================================================================*/
extern void wl1_qta_cancel_ind( wsub_id_e_type wsub_id );
...
===========================================================================*/
extern uint8 wl1_qta_overhead_time_in_ms( wsub_id_e_type wsub_id );
...
FUNCTION  WL1_LTA_OVERHEAD_TIME_IN_MS

DESCRIPTION
  This function is called to detect the LTA over head time in ms
...
RETURN VALUE
  Uint8 - overhead in ms
===========================================================================*/
extern uint8 wl1_lta_overhead_time_in_ms( wsub_id_e_type wsub_id);
...
SIDE EFFECTS
  None
===========================================================================*/
void l1_get_serving_cell_power(l1_serving_cell_power_info_type* l1_serving_cell_info, wsub_id_e_type wsub_id );
...
SIDE EFFECTS
  None
===========================================================================*/
void l1_get_active_set_info(l1_active_set_info_type* l1_active_set_info, wsub_id_e_type wsub_id );
...
void l1_get_monitored_set_info(l1_monitored_set_info_type* l1_monitored_set_info, wsub_id_e_type wsub_id );
...
SIDE EFFECTS:
=============================================================================*/
boolean rrcllc_check_srb_mapped_to_trch(sys_modem_as_id_e_type wrrc_as_id, tr_ch_id_type trch_id );
...
===========================================================================*/
extern void wl1_drx_page_ind(boolean true_page, wsub_id_e_type wsub_id );
...
===========================================================================*/
extern rex_tcb_type* wcdma_rrc_get_tcb(sys_modem_as_id_e_type wrrc_as_id);
...
SIDE EFFECTS:
=============================================================================*/
extern boolean wl1_is_serv_freq_considered_barred(uint16 freq, uint16 psc, wsub_id_e_type wsub_id);
...
RETURN VALUE
  boolean - TRUE if tuneaway request approved.  FALSE if not.
===========================================================================*/
extern void rrcllc_update_sec_hsdpa_hs_scch_order_activation_status_to_oc(sys_modem_as_id_e_type wrrc_as_id, uint8 activation_status, boolean oc_update_needed);
...
RETURN VALUE
===========================================================================*/
extern void drx_inform_l1_abt_bplmn_status(boolean bplmn_status, wsub_id_e_type wsub_id);
/*===========================================================================
...
SIDE EFFECTS    None.
===========================================================================*/
extern boolean dlchmgr_is_ext_pccpch_active(wsub_id_e_type wsub_id);
...
SIDE EFFECTS           None
===========================================================================*/
extern uint16 dl_get_cell_timing(uint16 psc, uint16 freq, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION     WL1_LTA_OVERHEAD_TIME_IN_MS

DESCRIPTION: This API is queried by RRC to know the amount of time W takes to perform its suspend
             operation. New API is being introduced in WL1, so RRC need evaluate if fast suspend 
             is enabled or not and the value can be modified anytime. Hence API is introduced at L1.
             This API gives the time taken to release lock for ISS in BCH/BCH_SLEEP/PCH_SLEEP.

DEPENDENCIES: 

RETURN VALUE:
             uint16 - number of ms before which RRC has to send SUSPEND_WCDMA_REQ so that
             WL1 can complete its cleanup operations successfully
...
extern uint8 wl1_lta_overhead_time_in_ms(wsub_id_e_type wsub_id);
/*===========================================================================
FUNCTION     DL_IS_LTA_TL_OPTS_NV_ENABLED

DESCRIPTION:

DEPENDENCIES: NV70188 (enable_lta_tl_opts) is introduced to bring in the optimization done to reduce LTA timeline
                         under NV control. Optimizations are done at RRC level, hence exporting the NV item to RRC

RETURN VALUE
             TRUE then LTA timeline optimization shall kick in
             FALSE otherwise

SIDE EFFECTS
             None
===========================================================================*/
extern boolean dl_is_lta_tl_opts_nv_enabled(void);

/*===========================================================================

FUNCTION     SEQ_CAN_WL1_HANDLE_ACT_CANCEL_CMD

DESCRIPTION
  RRC queries the below API to know if it can send ACTIVATION CANCEL CMD to wl1
...
RETURN VALUE
  Boolean - 
  TRUE if WL1 can handle ACT CANCEL CMD 
  FALSE otherwise.

SIDE EFFECTS
  None
===========================================================================*/
boolean seq_can_wl1_handle_act_cancel_cmd(wsub_id_e_type wsub_id);
/*===========================================================================
FUNCTION               wcdma_rrc_intlock
FILE NAME              wl1m.h

DESCRIPTION            This function locks WCDM mutes for RRC
...
SIDE EFFECTS           None
===========================================================================*/
extern void wcdma_rrc_intlock(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION               wcdma_rrc_intlfree
FILE NAME              wl1m.h
...
SIDE EFFECTS           None
===========================================================================*/
extern void wcdma_rrc_intlfree(wsub_id_e_type wsub_id);

/*===========================================================================

FUNCTION  wcdma_convert_stack_index_to_subid
...
===========================================================================*/
extern sys_modem_as_id_e_type wcdma_convert_stack_index_to_subid(sys_modem_as_id_e_type stack_index);
/*===========================================================================

FUNCTION  rrcwrm_return_trm_unlock_type

DESCRIPTION

  This function tells the unlock type
...
===========================================================================*/
extern trm_unlock_event_enum_t rrcwrm_return_trm_unlock_type(sys_modem_as_id_e_type wrrc_as_id);

/*===========================================================================

FUNCTION  rrcwrm_clear_trm_unlock_type

DESCRIPTION

  This function clears the trm unlock type and stops unlock by timer
...
===========================================================================*/
extern void rrcwrm_clear_trm_unlock_type(sys_modem_as_id_e_type wrrc_as_id);
...
DESCRIPTION
  ...
  This function Gets the SIB schedule BMSK
...
===========================================================================*/
extern uint8 rrcsib_gen_bit_mask_for_schedule_table
(
  sys_modem_as_id_e_type wrrc_as_id, 
  uint8 byte_offset
);
/*===========================================================================
 ...
 SIDE EFFECTS
   None
===========================================================================*/
extern uint16 wl1_rfm_get_default_chan(wsub_id_e_type wsub_id);
...
extern boolean rrc_is_sub_multimode(sys_modem_as_id_e_type wrrc_as_id);

/*===========================================================================

FUNCTION wl1_ds_stop_qta_block_timer

DESCRIPTION
  This function stops the timer when RRC procedure is finished
...
SIDE EFFECTS
  None
===========================================================================*/
extern void wl1_ds_stop_qta_block_timer(wsub_id_e_type wsub_id);
...
/*===========================================================================
 ...
 SIDE EFFECTS
   None
===========================================================================*/
extern boolean wl1_is_l1_in_mdsp_recovery(wsub_id_e_type wsub_id);

/*===========================================================================
 FUNCTION  rrc_get_sub_reason

 DESCRIPTION
   This function returns RRC SUB reason.
 ...
 SIDE EFFECTS
   None
===========================================================================*/
extern sys_proc_type_e_type  rrc_get_sub_reason(sys_modem_as_id_e_type wrrc_as_id);
/*===========================================================================
FUNCTION         srchset_update_sib3_for_cand_nbr

DESCRIPTION      
    This function is an API to update qualmin and rxlevmin in the "srch_inter_to_intraf_cell" struct called by RRC
...
SIDE EFFECTS
    None
===========================================================================*/
extern void srchset_update_sib3_params_for_cand_nbr(int16 qual_min, int16 rxlev_min, wsub_id_e_type wsub_id);
/*=========================================================================
  FUNCTION    WL1_IS_BPLMN_ACTIVE

  DESCRIPTION
  Get whether L1 is in pch_bplmn state
  ...
  RETURN VALUE
  TRUE  - L1 is in pch_bplmn
  FALSE - L1 is in any other state.
  =========================================================================*/
extern boolean wl1_is_bplmn_active(wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean rrc_return_1x_srlte_capability_ind(sys_modem_as_id_e_type wrrc_as_id);

/*===========================================================================
FUNCTION     boolean rrc_handle_wakeup_cancel

DESCRIPTION 
  This function checks RRC SIB priority inversion timer and returns whether to process wakeup cancel or not.

DEPENDENCIES
  None

RETURN VALUE
  TRUE -  Process wakeup cancel
  FALSE - Ignore wakeup cancel

SIDE EFFECTS
  None
===========================================================================*/
extern boolean rrc_handle_wakeup_cancel(sys_modem_as_id_e_type wrrc_as_id, uint32 reserve_occasion_ms);

/*===========================================================================
FUNCTION     l1_is_ue_dc_capable

DESCRIPTION 
  API to query the dc capability state from L1

DEPENDENCIES
  None

RETURN VALUE
  TRUE : UE is DC capable. 
  FALSE: UE is SC capable. 

SIDE EFFECTS
  None
===========================================================================*/
extern boolean l1_is_ue_dc_capable(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION     l1_set_dc_capability

DESCRIPTION 
  API to set the dc capability state in L1.

DEPENDENCIES
  capability should be indicated from RRC
...
extern void l1_set_dc_capability(boolean capability, wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION     l1_start_backoff_dc_cap_timer

DESCRIPTION 
  API to set the 20 min timer. For this duration, UE will show SC capability.
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_start_backoff_dc_cap_timer(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION     l1_get_remaining_time_for_backoff_dc_cap

DESCRIPTION 
  Will return the remaining time for which UE will show SC capability.
...
SIDE EFFECTS
  None
===========================================================================*/
extern uint32 l1_get_remaining_time_for_backoff_dc_cap(wsub_id_e_type wsub_id);

/*===========================================================================
FUNCTION     l1_clear_backoff_dc_cap_timer

DESCRIPTION 
  API to clear the 20 min timer.
...
SIDE EFFECTS
  None
===========================================================================*/
extern void l1_clear_backoff_dc_cap_timer(wsub_id_e_type wsub_id);

/*===========================================================================
  FUNCTION     SRCH_RPT_RXAGC_PSEUDO_MEAS

  DESCRIPTION
  This function reports avg rxagc to RRC for pseudo meas support 
...
extern int16 srch_rpt_rxagc_pseudo_meas(wsub_id_e_type wsub_id);
...
/*===========================================================================
  ...
  None
  ===========================================================================*/
extern int16 rrc_pseudo_meas_get_thresh_rsrp(sys_modem_as_id_e_type wrrc_as_id);
...
/*===========================================================================
  ...
  None
  ===========================================================================*/
extern int16 rrc_pseudo_meas_get_thresh_rsrq(sys_modem_as_id_e_type wrrc_as_id);

/*===========================================================================
FUNCTION rrc_pass_curr_req_plmn

DESCRIPTION
This function pass the current requested PLMN to L1.
...
===========================================================================*/
extern rrc_plmn_id_type rrc_get_curr_req_plmn(sys_modem_as_id_e_type wrrc_as_id);
#endif /* L1RRCIF_H */
