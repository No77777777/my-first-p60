...
--------   ---     ---------------------------------------------------------
06/28/16   pg     New events for QSH
06/28/16   sg     New events for QSH
12/15/15   sg     Compilation fix for WL1 errors for QSH changes
12/3/15    sg     Merge error fix for CR911283
10/03/15   ap     Addition of metrics WL1_QSH_METRIC_SERV_CELL_RF_PRIM_DIV and
                  WL1_QSH_METRIC_VOICE_BLER
10/06/15   sg     changes to add new event to qsh
...
07/15/15   jk     Removing F3's from QSH dump code.
===========================================================================*/
...
typedef PACKED struct PACKED_POST
{
  boolean is_tls_key_initialized;
  boolean is_arbiter_initialized;
  uint64 tls_key;
  uint8 conn_sub_id;
  int32 wsub_id_list[WCDMA_NUM_SUBS];
  uint8 fw_app_status[WCDMA_NUM_SUBS];
} wl1_debug_common_msim_arbiter_info_struct_type;
...
typedef PACKED struct PACKED_POST
{
  ...
  /*l1m info*/
  wl1_debug_common_l1m_info_struct_type l1m_info[WCDMA_NUM_SUBS];
  /*Sync cmd exchange info*/
  wl1_debug_common_mdsp_sync_cmd_exchange_info_struct_type mdsp_sync_cmd_exchange_info[WCDMA_NUM_SUBS	];
  /* MSIM Arbiter info */
  wl1_debug_common_msim_arbiter_info_struct_type msim_arbiter_info;
...
/* Database which provides info common across all subs */
typedef PACKED struct PACKED_POST
{
  /* Database to debug cpc drx issues*/
  wl1_debug_srchcpcdrx_dbg_info_struct_type srchcpcdrx_dbg_info;
  
}wl1_qsh_searcher_common_info_struct_type;

/* Database which provides per-sub info */
typedef PACKED struct PACKED_POST
{
...
}wl1_qsh_searcher_per_sub_info_struct_type;
...
typedef PACKED struct PACKED_POST
{
  ...
  qsh_dump_tag_hdr_s hdr;
  /* Database which provides info common across all subs */
  wl1_qsh_searcher_common_info_struct_type searcher_common;
  /* Database which provides per-sub info */
  wl1_qsh_searcher_per_sub_info_struct_type searcher_sub_info[WCDMA_NUM_SUBS];
...
typedef PACKED struct PACKED_POST
{
    /* this header will have a tag and length */
  qsh_dump_tag_hdr_s hdr;
   wl1_debug_drx_slp_timeline_info_struct_type drx_slp_timeline_info[WCDMA_NUM_SUBS];
   wl1_debug_drx_state_machine_info_struct_type drx_state_machine_info[WCDMA_NUM_SUBS];
   wl1_debug_drx_entity_db_info_struct_type drx_entity_info[WCDMA_NUM_SUBS];
   wl1_debug_drx_sampserv_info_struct_type drx_sampserv_info[WCDMA_NUM_SUBS];
   wl1_debug_drx_g2w_ta_info_struct_type drx_g2w_ta_info[WCDMA_NUM_SUBS];
   wl1_debug_drx_miscellaneous_info_struct_type drx_miscellaneous_info[WCDMA_NUM_SUBS];   
...
typedef PACKED struct PACKED_POST
{
    /* this header will have a tag and length */
  qsh_dump_tag_hdr_s hdr;
  wl1_debug_srchcr_info_struct_type srchcr[WCDMA_NUM_SUBS];
  wl1_debug_srchzz_info_struct_type srchzz[WCDMA_NUM_SUBS];
...
typedef PACKED struct PACKED_POST {
  ...
  /* Available PRACH signatures */
  uint8                        avail_sig_mask;
  /* min spreading factor */
  uint8                        sf_min;
  /* scrambling code num */
  uint8                        sc_num; 
  /* puncturing lim for RACH */
  uint8                        punct_lim;
  /* available sub channels where the UE may transmit */
  uint8                        avail_sub_chan_mask;
  /* trch ID for RACH */
  uint8                        trch_id;
  /* Preamble to Message pwr offset */
  int8                         pwr_offset_P0;
  /* max number of preambles cfgd */
  uint8                        max_preamble_tx;
  
} wl1_debug_ul_prach_cfg_struct_type;

typedef PACKED struct PACKED_POST  {
  /* num preambles attempted in this pmbl procedure */  
  uint8                      num_preambles;
  /* AICH ACK, NAK or NO ACK */
  uint8  aich_status;
  /* 10ms or 20ms TTI */
  ...
  */
  ...
  /* tx power for final pmbl */
  int16                      last_pmbl_power;
  /* message Tx power */
  int16                      message_tx_power;

} wl1_debug_ul_prach_status_struct_type;

/****************************HS_PRACH******************************/

typedef PACKED struct PACKED_POST
{
 /* AI Status. ACK, NACK, NO ACK */
 uint8                  ai_status;
 /* overall ACK or NAK */
 uint8                 hsrach_status;
 /* EAI Cfgd or not */
 uint8           eai_status;
 /* Reason for EDCH Resource release */
 uint8  wl1_hs_rach_edch_rsrc_rel_cause;
 /* Additional EDCH BAckoff timer */
 uint8                                      addtnl_edch_backoff_timer;
 /* Max ttis in which collission must be resolved with AG decode */
 uint8                                      maxAGCHCollResolution;
 /* available signature mask */
 uint8                                      avail_sig_mask;
 /* available sub channels */
 uint8                                      avail_subchan_mask;
 /* EDCH Backoff. DPCCH only Tx */
 uint16                                     edchTxContBackOff;
 /* Max CCCH Allocation */
 uint16                                     maxCCCHAllocation;
 
}wl1_debug_ul_hs_rach_config_params_struct_type;


/******************************UL_DPCH*****************************/
typedef PACKED struct PACKED_POST {
  /* DPCCH power offset cfgd by NW */
  int16                                      dpcch_pwr_offset;
  /* PCP Delay value */
  uint8                                      pcp_len;
  /* SRB Delay value */
  uint8                                      srb_delay;
  /* power control algo configured */
  uint8                        pca;             
  /* long or short scrambling code type */
...
} wl1_debug_ul_dpch_cfg_struct_type;
...
typedef PACKED struct PACKED_POST {
  ...
  uint8                                asc_num;
  /* in this frame the number of active trch */
...
}wl1_debug_ul_tx_data_struct_type;
...
} wl1_debug_ul_e_ul_cpc_dtx_cfg_db_struct_type;
...
} wl1_debug_ul_edch_cm_info_struct_type;
...
typedef PACKED struct PACKED_POST{
  ...
  uint8 cfn;
  uint8 sub_fn;
} wl1_debug_ul_eul_isr_timing_struct_type;
...
}wl1_debug_ul_e_agch_info_struct_type;
...
}wl1_debug_ul_e_dl_rl_info_struct_type;
...
} wl1_debug_ul_e_dl_config_db_struct_type;
...
typedef PACKED struct PACKED_POST
{
  ...
  uint16 rxd_ops;
    ...
    /* holds the current rf path for RxD */
  rfm_path_type  wl1_rxd_rf_path ;
  #else
...
typedef PACKED struct PACKED_POST
{
   ...
   wl1_debug_irat_meas_enum_type meas_type; 
   /* meas info. 95 bytes */
   union
   {
...
/* this is the general master struct for IRAT debug API. struct name with "debug_" means it's for this debug API only.
   other structs and constants are defined and used in working code. 184 bytes */
typedef PACKED struct PACKED_POST
{
/* IRAT does not have common_info_struct_type */
  wl1_irat_mini_debug_info_struct_type irat_sub_info[WCDMA_NUM_SUBS];
} wl1_qsh_irat_info_struct_type;
...
#define WL1_DEBUG_L1_CCTRCH_ID_COUNT 7
...
typedef PACKED struct PACKED_POST
{
  ...
  uint8 agc_pending_a0c2 : 1;                               /* 1 bit - Pending RxAGC command */
  uint8 agc_pending_a1c2 : 1;                               /* 1 bit - Pending RxAGC command */
  uint8 agc_pending_a0c3 : 1;                               /* 1 bit - Pending RxAGC command */
  uint8 agc_pending_a1c3 : 1;                               /* 1 bit - Pending RxAGC command */
  
  uint8 cctrch_index_sub_id[WL1_DEBUG_L1_CCTRCH_ID_COUNT];  /* 7B - Sub ID for each CCTrCh index */
...
} wl1_datapath_common_info_struct_type;


typedef PACKED struct PACKED_POST
{
  /* CCTrCh table */
  uint8 dl_cctrch_id_table_index[WL1_DEBUG_L1_CCTRCH_ID_COUNT];    /* 7B - table index for each CCTrCh ID (7) */
  wl1_debug_dec_hdr_rd_isr_info_struct_type dec_hdr_rd_isr[2];     /* 48B - timestamps and counters for last 2 header read events */
} wl1_datapath_per_sub_info_struct_type;

typedef PACKED struct PACKED_POST
{
  wl1_datapath_common_info_struct_type datapath_common;
  wl1_datapath_per_sub_info_struct_type datapath_per_sub[WCDMA_NUM_SUBS];
} wl1_qsh_datapath_info_struct_type;
...
typedef PACKED struct PACKED_POST 
{
  ...
  #if defined(FEATURE_WCDMA_ATLAS_MODEM) && defined(FEATURE_WCDMA_RFM_PATH_SUPPORTED)
  /**RFM path allocated for this resource**/
  rfm_path_type rfm_path; // = 1
  #else
...
} wl1_dsds_mini_debug_per_sub_struct_type; // = 263

typedef PACKED struct PACKED_POST 
{
  wl1_dsds_mini_debug_per_sub_struct_type per_sub_info[WCDMA_NUM_SUBS];
}wl1_dsds_mini_debug_struct_type;
...
typedef struct
{
  ...
  wl1_qsh_datapath_info_struct_type datapath;
#ifdef FEATURE_DUAL_SIM
...
/* enum of events being reported to QSH */
typedef enum {
WL1_QSH_EVENT_RLF = 0, /* RLF event */
WL1_QSH_EVENT_OOS = 1, /* OOS event */
WL1_QSH_EVENT_PAGE_WAKEUP = 2, /* Page wakeup event */
WL1_QSH_EVEN_ISST = 3, /* Inter-SIB Sleep event */
WL1_QSH_EVENT_OUT_OF_SYNC = 4, /* Out of Sync event */
WL1_QSH_EVENT_DL_WEAK_IND=5,
WL1_QSH_EVENT_PAGE_TIME_REM=6, /*remaining time to decode page*/
WL1_QSH_EVENT_REACQ_FAILED_FOR_3DRX =7, /* Reports if reacq fails in 3 consecutive DRX cyles*/
WL1_QSH_EVENT_RESEL_FAIL =8, /* Reselection fail event*/
WL1_QSH_EVENT_EMPTY_QTA_GAP = 9, /* Empty QTA Gap when W is data tech*/
WL1_QSH_EVENT_LOCK_FAILED_TIMEOUT_FOR_ACQ = 10, /* WRM Timeout for ACQ Procedure*/
WL1_QSH_EVENT_SCCPCH_CRC_FAIL = 11, /* CRC fail at good Ec/Io. */
WL1_QSH_EVENT_CQI_DROP = 12, /* CQI drop after QTA gap. */
WL1_QSH_EVENT_CONN_FREQ_COMMON_JUMP = 13,/* To report frequency jump */
WL1_QSH_EVENT_IDLE_FREQ_COMMON_JUMP = 14,
WL1_QSH_EVENT_LOW_PI_SUM = 15, /* LOW PI_SUM */
WL1_QSH_EVENT_PI_SUM0 = 16, /* PI_SUM == 0 */
WL1_QSH_EVENT_FIELD_DEBUG_ANY_FAILURE =17, /* Enabling all failure events*/
WL1_QSH_EVENT_MAX
}wl1_qsh_event_e;
