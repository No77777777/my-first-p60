...
$Header: //components/rel/wcdma.mpss/8.4.0/api/wl2api.h#3 $
...
08/12/15   kv     RAVE - removing extern declaration for ds API and including ds header
...
04/19/15   kv     L2-RRC interdependent variable to indicate type of CS-RAB
...
===========================================================================*/
extern boolean l1m_is_connected(wsub_id_e_type wsub_id);

extern uint8 l1m_get_state(wsub_id_e_type wsub_id);
...
typedef enum
{
  ...
  RLC_LOOPBACK_MODE_4  /* loopback mode 4 */
...
typedef struct {                          
  ...
  uint16  timer_discard;                /* Elapsed time in milliseconds    */
                                        ...
                                        /* the transmitter                 */
  rlc_rrc_tm_rab_type rlc_ul_tm_rab;
...
typedef struct {
  ...
  void (*rlc_am_sdu_cnf_cb) (sys_modem_as_id_e_type as_id,
...
typedef enum
{
  ...
  MAC_LOOPBACK_MODE_3, /* loopback mode 3 */
  MAC_LOOPBACK_MODE_4  /* loopback mode 4 */  
...
typedef enum
{
  RLC_DS_INVALID_QUERY = 0,
  RLC_DS_DNS_QUERY,
  RLC_DS_MAX_QUERY
} rlc_ds_tvm_trigger_reason;
...
typedef enum
{
  ...
  CMAC_QSH_PROCESS_METRIC_COMMAND_REQ,
...
typedef union
{
  ...
  qsh_client_cb_params_s     wmac_rlc_qsh_cfg;
...
/*--------------------------------------------------------------
  Enums to be shared with DS for RTT & Queuing Delay Computation 
  --------------------------------------------------------------*/
typedef enum
{
  RTT_DELAY,
...
typedef struct
 {
   uint16 num_samples;
   uint32 min_delay_in_ms; 
   uint32 max_delay_in_ms; 
   uint32 average_delay_in_ms;
} rlc_traffic_time_metrics;

extern void rlc_query_traffic_time_metrics ( uint8 sub_ID, uint8 rlc_id, delay_metric_type delay_type,rlc_traffic_time_metrics* rlc_ul_traffic_metrics );
extern void rlc_reset_traffic_time_metrics ( uint8 sub_ID, uint8 rlc_id );
...
sys_modem_as_id_e_type wcdma_convert_stack_index_to_subid(sys_modem_as_id_e_type stack_index);
#ifdef FEATURE_QSH_MDUMP
QSH_MDUMP_FN_ATTR
#endif
sys_modem_as_id_e_type wcdma_convert_subid_to_stack_index(sys_modem_as_id_e_type sub_id);

/*==========================================================================
FUNCTION: WCDMA_L2_START_UPLINK_RATE_CALCULATION

DESCRIPTION:
  Enables or disables RAVE and provides the periodicity 
  [Min periodicity 100ms, Max Periodicity 5000ms] for capacity calculation. 
  Called by Data Services. If interval is greater than the specified interval, 
  enable command is ignored
==========================================================================*/
extern void wcdma_l2_start_uplink_rate_calculations
(
...
DESCRIPTION:
  This API is called by DS and will forcibly report TVM to NW if TVM is
  enabled
==========================================================================*/
extern void wcdma_mac_ds_trigger_tvm_forcibly
(
  sys_modem_as_id_e_type    sub_id,
  rlc_ds_tvm_trigger_reason reason
);
