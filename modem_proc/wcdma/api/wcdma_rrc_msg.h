...
$Header: //components/rel/wcdma.mpss/8.4.0/api/wcdma_rrc_msg.h#6 $    $DateTime: 2020/04/29 07:36:02 $ $Author: pwbldsvc $
...
06/09/15   vi      Made changes to featurize LTE-U changes to avoid compilation error when LTE_BAND_NUM feature is not enabled. 
...
enum
{
  ...
  MSGR_DEFINE_UMID(WCDMA,RRC,REQ,QMI_IDLE_DRX_INFO,0x0F,wcdma_rrc_qmi_idle_drx_info_req_type),
  MSGR_DEFINE_UMID(WCDMA,RRC,REQ,QMI_PRIORITIZE_LTE,0x10,wcdma_rrc_qmi_prioritize_lte_req_type),
  MSGR_DEFINE_UMID(WCDMA,RRC,REQ,QMI_CURRENT_FREQS_INFO,0x11,wcdma_rrc_qmi_current_freqs_info_req_type),
};
...
enum
{
  ...
  MSGR_DEFINE_UMID(WCDMA,RRC,RSP,QMI_IDLE_DRX_INFO,0x0E,wcdma_rrc_qmi_idle_drx_info_rsp_type),
  MSGR_DEFINE_UMID(WCDMA,RRC,RSP,QMI_PRIORITIZE_LTE,0x0F,wcdma_rrc_qmi_prioritize_lte_rsp_type),
  MSGR_DEFINE_UMID(WCDMA,RRC,RSP,QMI_CURRENT_FREQS_INFO,0x10,wcdma_rrc_qmi_idle_drx_info_rsp_type),
};
/* Below #defines are added for logging purpose only */
...
enum
{
  ...
  MSGR_DEFINE_UMID(WCDMA,RRC,IND,QMI_CURRENT_FREQS_INFO,0x03,wcdma_rrc_qmi_current_freqs_info_ind_type),
};
/* Below #defines are added for logging purpose only */
...
/*----------------------------------------------------------------------------
                         WCDMA_RRC_QMI_IDLE_DRX_INFO_REQ
----------------------------------------------------------------------------*/
...
} wcdma_rrc_qmi_idle_drx_info_req_type;

/*----------------------------------------------------------------------------
                         WCDMA_RRC_QMI_IDLE_DRX_INFO_RSP
----------------------------------------------------------------------------*/
...
} wcdma_rrc_qmi_idle_drx_info_rsp_type;

/*----------------------------------------------------------------------------
                         WCDMA_RRC_QMI_PRIORITIZE_LTE_REQ
----------------------------------------------------------------------------*/
...
} wcdma_rrc_qmi_prioritize_lte_req_type;

/*----------------------------------------------------------------------------
                         WCDMA_RRC_QMI_PRIORITIZE_LTE_RSP
----------------------------------------------------------------------------*/
...
typedef struct
{
  ...
  /* True/False - depending on whether we prioritized LTE or not */
  boolean     success;
} wcdma_rrc_qmi_prioritize_lte_rsp_type;

/*----------------------------------------------------------------------------
                         WCDMA_RRC_QMI_CURRENT_FREQS_INFO_REQ
----------------------------------------------------------------------------*/
...
} wcdma_rrc_qmi_current_freqs_info_req_type;

/*----------------------------------------------------------------------------
                         WCDMA_RRC_QMI_CURRENT_FREQS_INFO_IND
----------------------------------------------------------------------------*/

#define RRC_MAX_INTER_FREQ_PRIMARY         2

#define RRC_MAX_INTER_FREQ_SECONDARY       2

#define RRC_MAX_INTER_FREQ RRC_MAX_INTER_FREQ_PRIMARY + RRC_MAX_INTER_FREQ_SECONDARY

#define RRC_MAX_GSM_NBR_FREQ       32

#define RRC_MAX_LTE_NBR_FREQ       8

typedef struct
{
  boolean serving_freq_valid;
  uint16  serving_freq;   /*WCDMA UARFCN*/
  
  boolean secondary_freq_valid;
  uint16  secondary_freq; /*WCDMA UARFCN*/

  sys_wcdma_rrc_state_e_type rrc_state;

  uint8   ifreq_nbr_cnt;
  uint16  ifreq_nbr[RRC_MAX_INTER_FREQ]; /*WCDMA nbr UARFCNS*/

  uint8   gsm_nbr_cnt;
  uint16  gsm_arfcns[RRC_MAX_GSM_NBR_FREQ];  /*GSM nbr ARFCNS*/

  uint8   lte_nbr_cnt;
  uint32  lte_earfcns[RRC_MAX_LTE_NBR_FREQ]; /*LTE nbr EARFCNS*/

} wcdma_rrc_qmi_current_freqs_type;

typedef struct
{
  /* message header type */
  msgr_hdr_s  msg_hdr;
  wcdma_rrc_qmi_current_freqs_type rrc_qmi_current_freqs;
} wcdma_rrc_qmi_current_freqs_info_ind_type;

/*----------------------------------------------------------------------------
                         WCDMA_RRC_QMI_CURRENT_FREQS_INFO_RSP
----------------------------------------------------------------------------*/
/*! Same structure as WCDMA_RRC_QMI_CURRENT_FREQS_INFO_IND*/
typedef wcdma_rrc_qmi_current_freqs_info_ind_type wcdma_rrc_qmi_current_freqs_info_rsp_type;
