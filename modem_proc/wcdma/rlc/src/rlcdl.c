...
$Header: //components/rel/wcdma.mpss/8.4.0/rlc/src/rlcdl.c#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $

when      who     what, where, why
--------  ---     -----------------------------------------------------------
01/16/18   sg      Made changes for to log interlayer commands for ML
12/15/16  nk      Fixed KW errors
08/16/16  kv      To discard UM out of SN PDU if CRC passed but FW result is 0
...
04/22/16  as      RLC to discard TB decode in Header Read when RLC is in STOP mode01/12/16   kc     Adding changes for WM register/deregister confirmation
03/28/16  kc      Adding changes for WM register/deregister confirmation
11/11/15  as      Added mutex protection to avoid reset procedure running in L2 DL task 
                  while channel release happens in L2 UL task.
09/21/15  as      Added code changes to enable RACH on getting L2 ACK and there is pending 
...
07/15/15  as      Added code changes to fix compilation errors for enabling FEATURE_UMTS_BMC_MSIM
07/09/15  scv     Check for NON-NULL dl_reseq_q pointer before dereferencing
...
02/04/15  aa      Fix KW errors from W+W
01/29/15  aa      L2 global reorg for W+W
01/20/15  kv      Fix to process Poll when status report and status prohibit are true for particular rlc-id
...
** ----------------------------------------------------------------------- */
rlci_downlink_table_type downlink_table[L2_NUM_W_SUBS];

#define MAX_PDUS_IN_Q 1550

#ifdef FEATURE_UMTS_BMC_MSIM
#define RLC_NUM_BYTES_TO_PASS_TO_NAS 6
#endif
...
)
{
  ...
  if (index < UE_MAX_DL_RLC_ENTITY)
  {
    ctl_blk_ptr = &(rlc_dl_table_ptr->ctrl_blk[index]);
  }
...
)
{
  ...
  {
    index = rlc_dl_table_ptr->dl_ctl_blk_id_tbl[rlc_id];
  }
...
{
  rlci_dl_ctrl_blk_type *ctl_blk_ptr = NULL;
  uint8 index = rlci_get_dl_ctl_blk_index_for_rlc_id(rlc_dl_table_ptr, rlc_id);

  if ((INIT_CTL_BLK_ID != index) && (index < UE_MAX_DL_RLC_ENTITY))
  {
    ctl_blk_ptr = &(rlc_dl_table_ptr->ctrl_blk[index]);
  }
  return ctl_blk_ptr;
...
{
  if(rlc_dl_table_ptr->rlc_dl_data_q.rlc_enh_data == NULL)
  {
    rlc_dl_table_ptr->rlc_dl_data_q.rlc_enh_data = (rlc_enh_data_ind_type *)modem_mem_alloc(RLC_MAX_TTI_DATA_DL * sizeof(rlc_enh_data_ind_type), 
                                                      MODEM_MEM_CLIENT_WCDMA_RLC_CRIT);
    if(rlc_dl_table_ptr->rlc_dl_data_q.rlc_enh_data != NULL)
    {
      WL2_MSG_0(HIGH, rlc_dl_table_ptr, "Allocated memory for rlc_enh_data");
    }
    else
    {
      WRLC_ERR_FATAL(rlc_dl_table_ptr->as_id, "RLC_ERR: Cannot allocate memory for rlc_enh_data",0,0,0); 
    }
  }
  else
  {
    WL2_MSG_0(HIGH, rlc_dl_table_ptr, "rlc_enh_data dynamic allocation already done");
  }
}
...
void rlc_dl_deallocate_mem_ZI_opt(rlci_downlink_table_type *rlc_dl_table_ptr)
{
  if(rlc_dl_table_ptr->rlc_dl_data_q.rlc_enh_data != NULL)
  {
    modem_mem_free(rlc_dl_table_ptr->rlc_dl_data_q.rlc_enh_data, MODEM_MEM_CLIENT_WCDMA_RLC_CRIT);
    WL2_MSG_0(HIGH, rlc_dl_table_ptr, "Freeing memory for rlc_enh_data");
    rlc_dl_table_ptr->rlc_dl_data_q.rlc_enh_data = NULL;     
  }
}
/*===========================================================================
...
uint8 dl_assign_ctl_blk(rlci_downlink_table_type *rlc_dl_table_ptr)
{
  ...
  {
    if (rlci_get_dl_ctl_blk_ptr_by_index(rlc_dl_table_ptr, i)->lc_state == DL_NULL_STATE)
    {
      ret_val = (uint8)i;
      break;
    }
...
===========================================================================*/
void rlc_dl_init(sys_modem_as_id_e_type as_id)
{
  int i;
  ...
  rlc_mutexes_type *rlc_mutex_ptr = &(rlc_mutexes[as_id]);
  
  WL2_MSG_0(LOW, rlc_dl_table_ptr, "In RLC Downlink Initialization");

  /*-------------------------------------------------------------------------
    Initialize the downlink table
  -------------------------------------------------------------------------*/
  (void)memset(rlc_dl_table_ptr, 0, sizeof(rlci_downlink_table_type));
  rlc_dl_table_ptr->as_id = as_id;
  rlc_dl_table_ptr->dl_pdu_decode_result = TRUE;
  #ifdef FEATURE_BMC_RLC_MULTIPLE_PDU_SUPPORT
  rlc_dl_table_ptr->ctch_first_pdu = TRUE;
  #endif
  /*-------------------------------------------------------------------------
  ...
  {
    rlc_dl_table_ptr->dl_ctl_blk_id_tbl[i] = INIT_CTL_BLK_ID;
  }

  rlc_dl_allocate_mem_ZI_opt(rlc_dl_table_ptr);

  /*-------------------------------------------------------------------------
    Init internal receiving queue.
  -------------------------------------------------------------------------*/
  (void)q_init(&rlc_dl_table_ptr->rlc_dl_data_q.rlc_tsn_info_q);
  (void)q_init(&rlc_dl_table_ptr->rlc_dl_data_q.rlc_enh_data_q);
  (void) q_init(&rlc_dl_table_ptr->rlc_dl_data_q.rlc_enh_free_data_q);
  /*Initialize the mutex*/
  rex_init_crit_sect(&(rlc_mutex_ptr->rlc_tsn_mutex));
  for (i = 0; i < RLC_MAX_TTI_DATA_DL; i++)
  {
    (void) q_link(&(rlc_dl_table_ptr->rlc_dl_data_q.rlc_enh_data[i]), &(rlc_dl_table_ptr->rlc_dl_data_q.rlc_enh_data[i].link));
    q_put(&(rlc_dl_table_ptr->rlc_dl_data_q.rlc_enh_free_data_q), &(rlc_dl_table_ptr->rlc_dl_data_q.rlc_enh_data[i].link));
  }

  /*-------------------------------------------------------------------------
    Initialize DL mutex.
  -------------------------------------------------------------------------*/
  rex_init_crit_sect(&(rlc_mutex_ptr->rlc_dl_mutex));
  rex_init_crit_sect(&(rlc_mutex_ptr->rlc_dl_data_process_mutex));

  rex_init_crit_sect(&(rlc_mutex_ptr->rlc_dl_tm_mutex));
  rex_init_crit_sect(&(rlc_mutex_ptr->rlc_mac_dl_pdu_mutex));
  rex_init_crit_sect(&(rlc_mutex_ptr->rlc_channel_release_mutex));

#ifndef FEATURE_WCDMA_RLC_ZI_REDUCTION_PHASE3  
  /*-------------------------------------------------------------------------
    Perform the AM initialization.
  -------------------------------------------------------------------------*/
  rlci_dl_am_init(rlc_dl_table_ptr);
#else
  ...
  -------------------------------------------------------------------------*/
  rex_def_timer_ex(&rlc_dl_table_ptr->rlc_dl_am_base_tmr, rlci_am_base_timer_cb, (RLC_DL_AM_BASE_TIMER_SIG | rlc_dl_table_ptr->as_id << 31));
  rlc_dl_table_ptr->rlc_dl_am_base_tmr_start = FALSE;
#endif
#endif
  ...
  ----------------------------------------------------------------------*/
  rex_def_timer(&rlc_dl_table_ptr->rlc_dl_logging.dl_tmr_log_blk, wcdma_l2_dl_get_tcb(as_id), RLC_DL_AM_LOG_TIMER_EXP_SIG);

  (void) rex_set_timer(&rlc_dl_table_ptr->rlc_dl_logging.dl_tmr_log_blk,(uint32)RLC_AM_LOG_TIMER_BASE); 

#ifdef FEATURE_RLC_ERROR_LOG_SUBMIT
  rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_var |= RLC_DBG_ERR_LOG_SUBMIT;
#endif
...
)
{
  ...
  WL2_MSG_1(HIGH, rlc_dl_table_ptr, "RLC_DBG: recevied DL cipher config for nchan %d ",
                                             cipher_ptr->nchan);
  ...
  for (i = 0; i < cipher_ptr->nchan; i++)
  {
    ...
    -----------------------------------------------------------------------*/
    ctl_blk = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, cipher_ptr->ciphering[i].lc_id);
    /*-----------------------------------------------------------------------
      If RLC downlink is in Data Transfer Ready state, copy details
     to the downlink table entry.
    -----------------------------------------------------------------------*/
    /*lint -e655 */
    if ((NULL != ctl_blk) && (ctl_blk->lc_state & DL_DATA_TRANSFER_READY))
    /*lint +e655 */
    {
      /*---------------------------------------------------------------------
      ...
      {
        WL2_MSG_3(LOW, rlc_dl_table_ptr, "DL cipher config, LC %d, hfn %x, act_seq %x",
        ...
        if (ctl_blk->lc_mode == UE_MODE_UNACKNOWLEDGED)
        {
          switch (ctl_blk->cipher.cipher_mode)
          {
            ...
            case CIPHER_ON:
              ...
              else
              {
                ...
                if (cipher_ptr->ciphering[i].ciph_act_sn != ctl_blk->ctrl.um_ctrl.vr_ur)
                {
                  ...
                  WL2_MSG_3(HIGH, rlc_dl_table_ptr,
                    "Future ciphering - DL Cipher config, LC %d no_cipher/cipher_on act_sn_old %d vr_ur %d",
                    ctl_blk->ctrl.um_ctrl.rlc_id, ctl_blk->cipher.act_sn, ctl_blk->ctrl.um_ctrl.vr_ur);
                }
                else
#endif
                {  
                  ...
                  WL2_MSG_3(HIGH, rlc_dl_table_ptr, "Cipher ON DL UM, LC %d, Key_id %d, count %x",
                          cipher_ptr->ciphering[i].lc_id,
            ...
            default:
              WL2_MSG_0(ERROR, rlc_dl_table_ptr, "Invalid Ciphering State");
          }
        }
        /*---------------------------------------------------------------------
          ...
          switch (ctl_blk->cipher.cipher_mode)
          {
            ...
            case CIPHER_ON:
              ...
              else
              {
                ...
                ctl_blk->cipher.act_sn = cipher_ptr->ciphering[i].ciph_act_sn;
                WL2_MSG_3(HIGH, rlc_dl_table_ptr, "Cipher ON DL AM, LC %d, Key_id %d, count %x",
                          cipher_ptr->ciphering[i].lc_id,
                                                           ctl_blk->cipher.cipher_key_id,
                                                           ctl_blk->cipher.count);
              }
      ...
      {
        ctl_blk->cipher.cipher_mode = NO_CIPHER;
        WL2_MSG_1(HIGH, rlc_dl_table_ptr, "DL Cipher config, LC %d cipher OFF",
                                                   cipher_ptr->ciphering[i].lc_id);
      }
    }
    else
  ...
  {
    WL2_MSG_0(LOW, rlc_dl_table_ptr, "LOG DL Cipher config");
    rlc_dl_tx_cipher_log_packet(rlc_dl_table_ptr, cipher_ptr);
  }
  else
  {
    WL2_MSG_0(LOW, rlc_dl_table_ptr, "NO LOG DL Cipher config");
  }
#endif
...
)
{
  ...
  {
    WRLC_ERR_FATAL(as_id, "Could not get command buf",0,0,0);
  }
} /* crlc_dl_hfn_cnf() */
...
)
{
  ...
  rlc_dl_hfn_cnf_type *dl_hfn_cnf_ptr = &(rlc_dl_table_ptr->dl_hfn_cnf);
  /*------------------------------------------------------------------------
    Initialize the number of channels and index to 0.
  ------------------------------------------------------------------------*/
  dl_hfn_cnf_ptr->nchan = 0;
  index = 0;
  /*-------------------------------------------------------------------------
  ...
  {
    count = rlc_dl_table_ptr->active_ch;
    array_len = sizeof(dl_hfn_cnf_ptr->hfn_info)/sizeof(rlc_dl_hfn_info_type);

    while ((count > 0) && (dl_hfn_cnf_ptr->nchan < array_len))
    {
      ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_index(rlc_dl_table_ptr, index);
      if (ctl_blk_ptr->lc_state != DL_NULL_STATE)
      {
        count--;
        if (ctl_blk_ptr->lc_mode != UE_MODE_TRANSPARENT)
        {
          for (i=0; i<UE_MAX_DL_LOGICAL_CHANNEL; i++)
          {
            if (rlci_get_dl_ctl_blk_index_for_rlc_id(rlc_dl_table_ptr, i)== index)
            {
              lc_id = i;
              break;
            }
          }
          dl_hfn_cnf_ptr->hfn_info[dl_hfn_cnf_ptr->nchan].lc_id = lc_id;
          dl_hfn_cnf_ptr->hfn_info[dl_hfn_cnf_ptr->nchan].hfn  = ctl_blk_ptr->cipher.hfn;
          dl_hfn_cnf_ptr->nchan++;
        }
      }
      index++;
    }
    crlc_dl_hfn_cnf(rlc_dl_table_ptr->as_id, dl_hfn_cnf_ptr);
  }
  else
  {
    /*-----------------------------------------------------------------------
      ...
      to send the information to RRC.
    -----------------------------------------------------------------------*/
    ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, hfn_ptr->lc_id);
    if ((NULL != ctl_blk_ptr) &&
        (ctl_blk_ptr->lc_state != DL_NULL_STATE))
    {
      dl_hfn_cnf_ptr->hfn_info[dl_hfn_cnf_ptr->nchan].lc_id = hfn_ptr->lc_id;
      dl_hfn_cnf_ptr->hfn_info[dl_hfn_cnf_ptr->nchan].hfn = ctl_blk_ptr->cipher.hfn;
      dl_hfn_cnf_ptr->nchan++;
      crlc_dl_hfn_cnf(rlc_dl_table_ptr->as_id, dl_hfn_cnf_ptr);
    }
    ...
    {
      ERR("Logical Channel %d does not exist", hfn_ptr->lc_id, 0, 0);
    }
  }
} /* crlc_dl_hfn_handler() */
...
)
{
  ...
  if (rrc_rlc_cmd_ptr == NULL)
  {
    WRLC_ERR_FATAL(rlc_dl_table_ptr->as_id, "Could not get command buf",0,0,0);
    /*lint -e527 */
    return;
    /*lint +e527 */
  }
  ...
  for (i = 0; i < tm_ptr->nchan; i++)
  {
    ...
    else
    {
      ...
      if(tm_ptr->dl_tm_act[i].action == RLC_MODIFY)
      {
        ...
        {
          action = rlci_get_config_action( rlc_dl_table_ptr, (&(tm_ptr->dl_tm_act[i].dl_cfg_mask)) );
          switch(action)
          {
            case RLCI_MODIFY:
              /*-------------------------------------------------------------------
              ...
              -------------------------------------------------------------------*/
              index = rlci_get_dl_ctl_blk_index_for_rlc_id(rlc_dl_table_ptr, tm_ptr->chan_info[i].lc_id);
              if (index < UE_MAX_DL_RLC_ENTITY)
              {
                WL2_MSG_2(LOW, rlc_dl_table_ptr, "DL TM Modify LC %d, Index %d",
                                                  tm_ptr->chan_info[i].lc_id,
                                                  index);
                ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_index(rlc_dl_table_ptr, index);
                ctl_blk_ptr->lc_type = tm_ptr->chan_info[i].lc_type;
                ctl_blk_ptr->ctrl.tm_ctrl.seg_ind = tm_ptr->chan_info[i].seg_ind;
                ctl_blk_ptr->ctrl.tm_ctrl.act_incl = tm_ptr->dl_tm_act[i].act_incl;
                ctl_blk_ptr->ctrl.tm_ctrl.act_time = tm_ptr->dl_tm_act[i].act_time;
                rrc_rlc_cmd_ptr->cmd.crlc_ul_tm_config_cnf.tm_config_result[i].result = SUCCESS;
    #ifdef FEATURE_RLC_QXDM_LOGGING
                if (j < UE_MAX_DL_TM_CHANNEL)
                {
                  rlc_dl_table_ptr->rlc_dl_logging.dl_tm_log_ptr.chan_info[j] = tm_ptr->chan_info[i];
                  j++;
                }
    #endif
              }
      ...
      else
      {
        switch(tm_ptr->dl_tm_act[i].action)
        {
          ...
          ---------------------------------------------------------------------*/
            ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, tm_ptr->chan_info[i].lc_id);
	    if ((NULL == ctl_blk_ptr) &&
                ((index = dl_assign_ctl_blk(rlc_dl_table_ptr)) != INIT_CTL_BLK_ID))
            {
              WL2_MSG_2(HIGH, rlc_dl_table_ptr, "DL TM Establish LC %d, Index %d",
                                                         tm_ptr->chan_info[i].lc_id,
                                                         index);
              rlc_dl_table_ptr->dl_ctl_blk_id_tbl[tm_ptr->chan_info[i].lc_id] = index;
	      ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_index(rlc_dl_table_ptr, index);
              ...
              ctl_blk_ptr->ctrl.tm_ctrl.act_incl = tm_ptr->dl_tm_act[i].act_incl;
              ctl_blk_ptr->ctrl.tm_ctrl.act_time = tm_ptr->dl_tm_act[i].act_time;
              ctl_blk_ptr->ctrl.tm_ctrl.rlc_dl_tm_rab = tm_ptr->chan_info[i].rlc_dl_tm_rab;
              ctl_blk_ptr->lc_state = DL_DATA_TRANSFER_READY;
              ctl_blk_ptr->dl_wm_ptr = NULL;
              ctl_blk_ptr->rlc_post_rx_func_ptr = NULL;
              rlc_dl_table_ptr->active_ch++;
              rrc_rlc_cmd_ptr->cmd.crlc_ul_tm_config_cnf.tm_config_result[i].result = SUCCESS;
  #ifdef FEATURE_RLC_QXDM_LOGGING
              rlc_dl_table_ptr->rlc_dl_logging.dl_tm_log_ptr.chan_info[j] = tm_ptr->chan_info[i];
              j++;
              rlc_dl_table_ptr->rlc_dl_logging.dl_log_state.dl_state[k].rlc_id = tm_ptr->chan_info[i].lc_id;
              rlc_dl_table_ptr->rlc_dl_logging.dl_log_state.dl_state[k].lc_state = TM_DL_DATA_TRANSFER_READY;
              k++;
  #endif
            }
            else
	    ...
	    if (index < UE_MAX_DL_RLC_ENTITY)
            {
              WL2_MSG_2(HIGH, rlc_dl_table_ptr, "DL TM Release LC %d, Index %d",
                                                         tm_ptr->chan_info[i].lc_id,
                                                         index);
	      ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_index(rlc_dl_table_ptr, index);
              ctl_blk_ptr->lc_state = DL_NULL_STATE;
              rlc_dl_table_ptr->active_ch--;
              rlc_dl_table_ptr->dl_ctl_blk_id_tbl[tm_ptr->chan_info[i].lc_id] = INIT_CTL_BLK_ID;
              rrc_rlc_cmd_ptr->cmd.crlc_ul_tm_config_cnf.tm_config_result[i].result = SUCCESS;
  
  #ifdef FEATURE_RLC_QXDM_LOGGING
              rlc_dl_table_ptr->rlc_dl_logging.dl_log_state.dl_state[k].rlc_id = tm_ptr->chan_info[i].lc_id;
              rlc_dl_table_ptr->rlc_dl_logging.dl_log_state.dl_state[k].lc_state = RLC_DL_NULL_STATE;
              k++;
  #endif
  ...
  #error code not present
#endif
  rrc_put_rlc_cmd(rlc_dl_table_ptr->as_id, rrc_rlc_cmd_ptr);

#ifdef FEATURE_RLC_QXDM_LOGGING
  if (j > 0)
  {
    rlc_dl_table_ptr->rlc_dl_logging.dl_tm_log_ptr.nchan = j;
    rlc_dl_send_tm_log_packet(rlc_dl_table_ptr);
  }
  if (k > 0)
  {
    rlc_dl_table_ptr->rlc_dl_logging.dl_log_state.nchan = k;
    rlc_dl_send_state_log_packet(rlc_dl_table_ptr);
  }
#endif
...
)
{
  ...
  if (rrc_rlc_cmd_ptr == NULL)
  {
    WL2_MSG_0(ERROR, rlc_dl_table_ptr, "RLC_ERR: Could not get command buf");
    return;
  }
  ...
  for (i = 0; i < um_ptr->nchan; i++)
  {
    ...
    {
      WL2_MSG_1(ERROR, rlc_dl_table_ptr, "RLC_ERR: Invalid DL UM LC Id %d", um_ptr->chan_info[i].lc_id);
      
      rrc_rlc_cmd_ptr->cmd.crlc_dl_um_config_cnf.um_config_result[i].result = FAILURE;
    }
    else
    {
      switch(um_ptr->dl_um_act[i].action)
      {
        case RLC_ESTABLISH:
          /*-------------------------------------------------------------------
            ...
            channel. Send a SUCCESS indication in the confirm message to RRC
          -------------------------------------------------------------------*/
          ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, um_ptr->chan_info[i].lc_id);
          if ((NULL == ctl_blk_ptr) &&
             ((index = dl_assign_ctl_blk(rlc_dl_table_ptr)) != INIT_CTL_BLK_ID))
          {
            rlc_dl_table_ptr->dl_ctl_blk_id_tbl[um_ptr->chan_info[i].lc_id] = index;
            ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_index(rlc_dl_table_ptr, index);
            
            rlci_establish_dl_um(rlc_dl_table_ptr, ctl_blk_ptr,
                                 &um_ptr->chan_info[i],
                                 &um_ptr->dl_um_act[i]);
            
            rlc_enh_establish_dl_um(rlc_dl_table_ptr, ctl_blk_ptr);
            rrc_rlc_cmd_ptr->cmd.crlc_dl_um_config_cnf.um_config_result[i].result = SUCCESS;
            rlc_dl_table_ptr->active_ch++;
#ifdef FEATURE_RLC_QXDM_LOGGING
            rlci_generate_dl_config_event(ctl_blk_ptr, um_ptr->chan_info[i].rlc_size, rlc_dl_table_ptr);
            rlc_dl_table_ptr->rlc_dl_logging.dl_um_log_ptr.chan_info[j] = um_ptr->chan_info[i];
            est_or_reest[j] = TRUE;
            j++;
            rlc_dl_table_ptr->rlc_dl_logging.dl_log_state.dl_state[k].rlc_id = um_ptr->chan_info[i].lc_id;
            rlc_dl_table_ptr->rlc_dl_logging.dl_log_state.dl_state[k].lc_state = UM_DL_DATA_TRANSFER_READY;
            k++;
#endif
          }
          else
          {
            ...
            WL2_MSG_1(ERROR, rlc_dl_table_ptr, "RLC_ERR: DL LC %d in use - No Establish",
                                                        um_ptr->chan_info[i].lc_id);
          }
          break;
        case RLC_RELEASE:
          /*-------------------------------------------------------------------
            ...
            to RRC.
          -------------------------------------------------------------------*/
          L2_MUTEX_LOCK(&rlc_mutexes[rlc_dl_table_ptr->as_id].rlc_channel_release_mutex);
          index = rlci_get_dl_ctl_blk_index_for_rlc_id(rlc_dl_table_ptr, um_ptr->chan_info[i].lc_id);
          if (index < UE_MAX_DL_RLC_ENTITY)
          {
            ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_index(rlc_dl_table_ptr, index);
            WL2_MSG_2(HIGH, rlc_dl_table_ptr, "DL UM Release LC %d, Index %d",
                                                       um_ptr->chan_info[i].lc_id,
                                                       index);

            rlci_release_dl_um(rlc_ul_table_ptr, ctl_blk_ptr);

            rlc_dl_table_ptr->dl_ctl_blk_id_tbl[um_ptr->chan_info[i].lc_id] = INIT_CTL_BLK_ID;
            rrc_rlc_cmd_ptr->cmd.crlc_dl_um_config_cnf.um_config_result[i].result = SUCCESS;
            rlc_dl_table_ptr->active_ch--;
#ifdef FEATURE_RLC_QXDM_LOGGING
            rlc_dl_table_ptr->rlc_dl_logging.dl_log_state.dl_state[k].rlc_id = um_ptr->chan_info[i].lc_id;
          ...
          else
          {
            ...
            -----------------------------------------------------------------*/
            rrc_rlc_cmd_ptr->cmd.crlc_dl_um_config_cnf.um_config_result[i].result = FAILURE;
            WL2_MSG_1(ERROR, rlc_dl_table_ptr, "RLC_ERR: DL LC %d Not config'd - No Release",
                                                        um_ptr->chan_info[i].lc_id);
          }
          L2_MUTEX_UNLOCK(&rlc_mutexes[rlc_dl_table_ptr->as_id].rlc_channel_release_mutex);
          break;
        ...
        case RLC_MODIFY:
          ...
          if(um_ptr->dl_um_act[i].action == RLC_MODIFY)
          {
            ...
            {
              action =rlci_get_config_action( rlc_dl_table_ptr, (&(um_ptr->dl_um_act[i].dl_cfg_mask)) );
              switch(action)
              {
                case RLCI_MODIFY:
                  /*-------------------------------------------------------------------
		  ...
		  if (index < UE_MAX_DL_RLC_ENTITY)
                  {
                    WL2_MSG_2(HIGH, rlc_dl_table_ptr, "DL UM Modify LC %d, Index %d",
                                                               um_ptr->chan_info[i].lc_id,
                                                               index);

                    ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_index(rlc_dl_table_ptr, index);
		    rlci_modify_dl_um(rlc_dl_table_ptr, ctl_blk_ptr, &um_ptr->chan_info[i],
                                      &um_ptr->dl_um_act[i]);
                  ...
                  else
                  {
                    ...
                    -----------------------------------------------------------------*/
                    rrc_rlc_cmd_ptr->cmd.crlc_dl_um_config_cnf.um_config_result[i].result = FAILURE;
                    ERR("DL LC %d not config'd -- No Mod", um_ptr->chan_info[i].lc_id,0,0);
                  }
                  break;
		  ...
		  index = rlci_get_dl_ctl_blk_index_for_rlc_id(rlc_dl_table_ptr, um_ptr->chan_info[i].lc_id);
                  if (index < UE_MAX_DL_RLC_ENTITY)
                  {
                    ...
                    /*lint -e641 -e655 -e64 */
                    ctl_blk_ptr->lc_state |= DL_STOP;
                    ctl_blk_ptr->lc_state &= (~DL_DATA_TRANSFER_READY);
                    rrc_rlc_cmd_ptr->cmd.crlc_dl_um_config_cnf.um_config_result[i].result = SUCCESS;
                    /*lint +e641 +e655 +e64 */
                    WL2_MSG_1(HIGH, rlc_dl_table_ptr, "DL UM LC %d - Stop",
                                                               um_ptr->chan_info[i].lc_id);
                  }
                  else
                  {
		  ...
		  index = rlci_get_dl_ctl_blk_index_for_rlc_id(rlc_dl_table_ptr, um_ptr->chan_info[i].lc_id);
                  if (index < UE_MAX_DL_RLC_ENTITY)
                  {
                    ...
                    rrc_rlc_cmd_ptr->cmd.crlc_dl_um_config_cnf.um_config_result[i].result = SUCCESS;
                    /*lint +e64 +e641 +e655 */
                    WL2_MSG_1(HIGH, rlc_dl_table_ptr, "DL UM LC %d - Continue",
                                                               um_ptr->chan_info[i].lc_id);
                  }
                  else
                  {
		  ...
		  index = rlci_get_dl_ctl_blk_index_for_rlc_id(rlc_dl_table_ptr, um_ptr->chan_info[i].lc_id);
                  ...
                  {
                    ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_index(rlc_dl_table_ptr, index);
                    rlci_re_establish_dl_um(rlc_ul_table_ptr, rlc_dl_table_ptr, ctl_blk_ptr, &um_ptr->chan_info[i]);
                    rrc_rlc_cmd_ptr->cmd.crlc_dl_um_config_cnf.um_config_result[i].result = SUCCESS;
        #ifdef FEATURE_RLC_QXDM_LOGGING
                    rlci_generate_dl_config_event(ctl_blk_ptr, um_ptr->chan_info[i].rlc_size, rlc_dl_table_ptr);
                    if (j < UE_MAX_DL_UM_CHANNEL)
  ...
  if (j > 0)
  {
    rlc_dl_table_ptr->rlc_dl_logging.dl_um_log_ptr.nchan = j;
    rlc_dl_send_um_log_packet(rlc_dl_table_ptr, &rlc_dl_table_ptr->rlc_dl_logging.dl_um_log_ptr, est_or_reest);
  }
  if (k > 0)
  {
    rlc_dl_table_ptr->rlc_dl_logging.dl_log_state.nchan = k;
...
)
{
  ...
  for (i=0; i < am_config_req_ptr->nentity; i++)
  {
    ...
    {
      action = rlci_get_config_action( rlc_dl_table_ptr , (&(am_config_req_ptr->am_act[i].ul_cfg_mask)) );
    ...
    {
      WL2_MSG_6(HIGH, rlc_dl_table_ptr,
         "Padding_DBG: DL lc_id %d, action %d, dl_cfg_mask %d, \
          ...
          am_config_req_ptr->am_info[i]->rlc_dl_am_config.dl_rlc_size );
    }
#endif
        ...
        (common_ptr->dl_data_id < UE_MAX_DL_LOGICAL_CHANNEL))
    {
      ...
      {
        case RLC_ESTABLISH:
	  ctl_blk_id = rlci_get_dl_ctl_blk_index_for_rlc_id(rlc_dl_table_ptr, common_ptr->dl_data_id);
          if ((ctl_blk_id == INIT_CTL_BLK_ID) &&
              ((ctl_blk_id = dl_assign_ctl_blk(rlc_dl_table_ptr)) != INIT_CTL_BLK_ID))
          {
            rlc_dl_table_ptr->dl_ctl_blk_id_tbl[common_ptr->dl_data_id] = ctl_blk_id;
            rlc_dl_table_ptr->dl_ctl_blk_id_tbl[common_ptr->dl_control_id] = ctl_blk_id;
	    ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_index(rlc_dl_table_ptr, ctl_blk_id);
            rlc_dl_table_ptr->active_ch +=
              rlci_establish_dl_am(rlc_ul_table_ptr, ctl_blk_ptr, am_config_req_ptr->am_info[i]
#ifdef FEATURE_HSDPA_MAC_EHS
                                   , &result
#endif 
               );          
            result = rlci_enh_establish_dl_am(rlc_dl_table_ptr, ctl_blk_ptr, common_ptr->dl_data_id);
            ...
            WL2_MSG_2(LOW, rlc_dl_table_ptr, "DL AM Establish:Data ID = %d, index = %d",
                                              common_ptr->dl_data_id,
                                              ctl_blk_id);
            /*---------------------------------------------------------------------
              Delay timer setting until this RLC's first received PDU.
            ----------------------------------------------------------------------*/
            ctl_blk_ptr->ctrl.am_ctrl.dl_state_var.fst_rx_pdu = TRUE;          
            ctl_blk_ptr->ctrl.am_ctrl.dl_tmr_sts = RLC_AM_TIMER_UNDEF_STATUS;

#ifdef FEATURE_RLC_QXDM_LOGGING
            rlci_generate_dl_config_event(ctl_blk_ptr, 0, rlc_dl_table_ptr);
            /*---------------------------------------------------------------------
              Update the state information for the particular AM entity.
            ----------------------------------------------------------------------*/
            if (j < UE_MAX_AM_ENTITY)
            {
              rlc_dl_table_ptr->rlc_dl_logging.dl_am_log_ptr.am_info[j] = am_config_req_ptr->am_info[i];
              est_or_reest[j] = TRUE;
              j++;
            }
            if (k < UE_MAX_DL_LOGICAL_CHANNEL)
            {
              rlc_dl_table_ptr->rlc_dl_logging.dl_log_state.dl_state[k].rlc_id = common_ptr->dl_data_id;
              rlc_dl_table_ptr->rlc_dl_logging.dl_log_state.dl_state[k].lc_state = AM_DL_DATA_TRANSFER_READY;
              k++;
            }
#endif
          }
          ...
          {
            ul_cmd_ptr->cmd_data.dl_config_cnf.am_cnf[i].result = FAILURE;
            WL2_MSG_1(ERROR, rlc_dl_table_ptr, "RLC_ERR: DL AM LC %d in use, No Establish ",
                                                        common_ptr->dl_data_id);
          }
        ...
        case RLC_RELEASE:
          L2_MUTEX_LOCK(&rlc_mutexes[rlc_dl_table_ptr->as_id].rlc_channel_release_mutex);
	  ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, common_ptr->dl_data_id);
          if (NULL != ctl_blk_ptr)
          {
            rlc_dl_table_ptr->active_ch -=
              rlci_release_dl_am(rlc_ul_table_ptr, rlc_dl_table_ptr, ctl_blk_ptr);
              
            rlc_dl_table_ptr->dl_ctl_blk_id_tbl[common_ptr->dl_data_id] = INIT_CTL_BLK_ID;
            rlc_dl_table_ptr->dl_ctl_blk_id_tbl[common_ptr->dl_control_id] = INIT_CTL_BLK_ID;

            for (index = 0, act_dl_am_chnl = 0; index < UE_MAX_DL_LOGICAL_CHANNEL; index++)
            {
              ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, index);
              if ((NULL != ctl_blk_ptr) && 
                  (DL_NULL_STATE != ctl_blk_ptr->lc_state) && 
            ...
            if ((0 == act_dl_am_chnl) && (TRUE == rlc_dl_table_ptr->rlc_dl_am_base_tmr_start))
            {
              /* Since all AM channels are released, stop base dl am timer */
              (void) rex_clr_timer(&rlc_dl_table_ptr->rlc_dl_am_base_tmr);
              rlc_dl_table_ptr->rlc_dl_am_base_tmr_start = FALSE;
              
              WL2_MSG_1(HIGH, rlc_dl_table_ptr, "RLC_TMR_DBG: Stopping DL base timer, active_dl_chan %d",
                                                         rlc_dl_table_ptr->active_ch);
            }
          
            WL2_MSG_2(HIGH, rlc_dl_table_ptr, "DL AM LC %d - Release, acitve_dl_channels %d",
                                                       common_ptr->dl_data_id,
                                                       rlc_dl_table_ptr->active_ch);
            ul_cmd_ptr->cmd_data.dl_config_cnf.am_cnf[i].result = SUCCESS;
#ifdef FEATURE_RLC_QXDM_LOGGING
          ...
          {
            ul_cmd_ptr->cmd_data.dl_config_cnf.am_cnf[i].result = FAILURE;
            WL2_MSG_1(ERROR, rlc_dl_table_ptr, "RLC_ERR: DL AM LC %d not config'd, No Release",
                                                        common_ptr->dl_data_id);
          }
          L2_MUTEX_UNLOCK(&rlc_mutexes[rlc_dl_table_ptr->as_id].rlc_channel_release_mutex);
          break;
      ...
      case RLC_MODIFY:
        ...
        if(am_config_req_ptr->am_act[i].action == RLC_MODIFY)
        {
          ...
          {
            action = rlci_get_config_action( rlc_dl_table_ptr, (&(am_config_req_ptr->am_act[i].dl_cfg_mask)) );
            switch(action)
            {
              case RLCI_MODIFY :
                ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, common_ptr->dl_data_id);
                if (NULL != ctl_blk_ptr)
                {
                  rlci_modify_dl_am(rlc_ul_table_ptr, rlc_dl_table_ptr, ctl_blk_ptr, am_config_req_ptr->am_info[i]);
                  WL2_MSG_1(LOW, rlc_dl_table_ptr, "DL AM LC %d - Modify",
                                                    common_ptr->dl_data_id);
                  ul_cmd_ptr->cmd_data.dl_config_cnf.am_cnf[i].result = SUCCESS;
              ...
              case RLCI_STOP:
                ...
                if (NULL != ctl_blk_ptr)
                {
                  ...
                  ctl_blk_ptr->lc_state &= (~DL_DATA_TRANSFER_READY);
                  /*lint +e64 +e641 +e655 */
                  WL2_MSG_1(HIGH, rlc_dl_table_ptr, "DL AM LC %d - Stop",
                                                             common_ptr->dl_data_id);
                  ul_cmd_ptr->cmd_data.dl_config_cnf.am_cnf[i].result = SUCCESS;
                }
                else
              ...
              case RLCI_CONTINUE:
                ...
                if (NULL != ctl_blk_ptr)
                {
                  ...
                  ctl_blk_ptr->lc_state |= DL_DATA_TRANSFER_READY;
                  /*lint +e64 +e641 +e655 */
                  WL2_MSG_1(HIGH, rlc_dl_table_ptr, "DL AM LC %d - Continue",
                                                             common_ptr->dl_data_id);
                  ul_cmd_ptr->cmd_data.dl_config_cnf.am_cnf[i].result = SUCCESS;
                }
                else
              ...
              case RLCI_RE_ESTABLISH:
                ...
                if(NULL != ctl_blk_ptr)
                {
                  rlci_re_establish_dl_am(rlc_ul_table_ptr, rlc_dl_table_ptr, ctl_blk_ptr, am_config_req_ptr->am_info[i]);
                  WL2_MSG_1(HIGH, rlc_dl_table_ptr, "Re-establish DL AM LC %d",
                                                             common_ptr->dl_data_id);
      
                  rlc_enh_free_reseq_q(rlc_ul_table_ptr, rlc_dl_table_ptr, ctl_blk_ptr, RLC_RE_ASSEMBLY_Q_SIZE, 0);
                  ul_cmd_ptr->cmd_data.dl_config_cnf.am_cnf[i].result = SUCCESS;
      ...
      default:
          ul_cmd_ptr->cmd_data.dl_config_cnf.am_cnf[i].result = FAILURE;
          WL2_MSG_1(ERROR, rlc_dl_table_ptr, "RLC_ERR: DL AM %d, Invalid Action",
                                                      common_ptr->dl_data_id);
      }
    }
    else
...
//  wcdma_l2_ul_put_cmd(ul_cmd_ptr);
  ...
  rlci_dl_config_cnf_handler(rlc_ul_table_ptr, rlc_dl_table_ptr, &(ul_cmd_ptr->cmd_data.dl_config_cnf));
  l2_free_cmd_buf(ul_cmd_ptr, rlc_dl_table_ptr);


#ifdef FEATURE_RLC_QXDM_LOGGING
  if (j > 0)
  {
    rlc_dl_table_ptr->rlc_dl_logging.dl_am_log_ptr.nentity = j;
    rlc_dl_send_am_log_packet(rlc_dl_table_ptr, &rlc_dl_table_ptr->rlc_dl_logging.dl_am_log_ptr, est_or_reest);
  }
  if (k > 0)
  {
    rlc_dl_table_ptr->rlc_dl_logging.dl_log_state.nchan = k;
...
)
{
  ...
  for (i = 0; i < reg_ptr->nchan; i++)
  {
    ...
    ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, reg_ptr->rlc_data[i].lc_id);
    if ((NULL != ctl_blk_ptr) &&
        (ctl_blk_ptr->lc_state != DL_NULL_STATE))
    {
      /* Task or Interrupt Context */
      ctl_blk_ptr->context = reg_ptr->rlc_data[i].context;
      
      /* For CS Voice RB, context is always true */ 
      if (RLC_RRC_CS_VOICE == ctl_blk_ptr->ctrl.tm_ctrl.rlc_dl_tm_rab)
      {
        ctl_blk_ptr->context = TRUE;  
      }
      
      if (reg_ptr->rlc_data[i].dl_wm_ptr != NULL)
      {
        ctl_blk_ptr->dl_wm_ptr = reg_ptr->rlc_data[i].dl_wm_ptr;
        wm_reg_rsp = RLC_WM_REQ_SUCCESS;
        WL2_MSG_6(HIGH, rlc_dl_table_ptr,"RLC_WM:DL: RLC-id - %d, Lo_Wm_Cnt - %d, Hi_Wm_Cnt - %d, Do_Not_Exceed - %d context %d Funct Ptr 0x%x",
                        reg_ptr->rlc_data[i].lc_id,
                        ctl_blk_ptr->dl_wm_ptr->lo_watermark,
                        ctl_blk_ptr->dl_wm_ptr->hi_watermark,
                        ctl_blk_ptr->dl_wm_ptr->dont_exceed_cnt,
                        reg_ptr->rlc_data[i].context,
                        reg_ptr->rlc_data[i].rlc_post_rx_proc_func_ptr);
      }
      else
      {
        WL2_MSG_1(ERROR, rlc_dl_table_ptr, "RLC_ERR: WM ptr NULL, LC %d",
                                                    reg_ptr->rlc_data[i].lc_id);
      }

      if (reg_ptr->rlc_data[i].rlc_post_rx_proc_func_ptr != NULL)
      {
        ctl_blk_ptr->rlc_post_rx_func_ptr = reg_ptr->rlc_data[i].rlc_post_rx_proc_func_ptr;
        ctl_blk_ptr->rlc_post_rx_func_ptr_para = reg_ptr->rlc_data[i].rlc_post_rx_func_ptr_para;
      }
    }
    else
    {
      WL2_MSG_1(ERROR, rlc_dl_table_ptr, "RLC_ERR: LC %d not config yet",
                                                  reg_ptr->rlc_data[i].lc_id);
    ...
    {
      WL2_MSG_2(HIGH, rlc_dl_table_ptr, "RLC_WM: Sending reg WM response status %d for LC %d",
...
{
  rlc_dl_register_srvc_handler_per_sub(L2_SUB_1,reg_ptr);
}

/*===========================================================================
...
)
{
  ...
  L2_MUTEX_LOCK(&(rlc_mutexes[as_id].rlc_dl_tm_mutex));
  ...
  for (i = 0; i < de_reg_ptr->nchan; i++)
  {
    ...
    ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, de_reg_ptr->rlc_id[i]);
    if (NULL != ctl_blk_ptr)
    {
      /* Task or Interrupt Context */
      WL2_MSG_1(HIGH, rlc_dl_table_ptr, "DeRegister DL WM, LC %d",
                                                 de_reg_ptr->rlc_id[i]);
      ctl_blk_ptr->dl_wm_ptr = NULL;
      wm_dereg_rsp = RLC_WM_REQ_SUCCESS;

      WL2_MSG_1(LOW, rlc_dl_table_ptr, "DeRegister Rx CB fcn, LC %d",
                                                de_reg_ptr->rlc_id[i]);
      ctl_blk_ptr->rlc_post_rx_func_ptr = NULL;
    }
    else
    ...
    {
      WL2_MSG_2(HIGH, rlc_dl_table_ptr, "RLC_WM: Sending DL WM dereg status %d for LC %d",
  ...
  L2_MUTEX_UNLOCK(&(rlc_mutexes[as_id].rlc_dl_tm_mutex));
...
{
  rlc_dl_deregister_srvc_handler_per_sub(L2_SUB_1,de_reg_ptr);
}

/*===========================================================================
...
)
{
  ...
  {
    ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, dl_sdu_discard_cb->sdu_discard_cb[i].lc_id);
    if ((NULL != ctl_blk_ptr) &&
        (ctl_blk_ptr->lc_state != DL_NULL_STATE))
    {
      if (dl_sdu_discard_cb->sdu_discard_cb[i].rlc_am_sdu_discard_cb != NULL)
      {
        WL2_MSG_1(HIGH, rlc_dl_table_ptr, "Register DL WM, LC %d", dl_sdu_discard_cb->sdu_discard_cb[i].lc_id);
        ctl_blk_ptr->ctrl.am_ctrl.sdu_discard_cb =
                  dl_sdu_discard_cb->sdu_discard_cb[i].rlc_am_sdu_discard_cb;
      }
      else
      {
        WL2_MSG_1(ERROR, rlc_dl_table_ptr, "RLC_ERR: SDU discard cb fcn ptr NULL, LC %d",
...
/*Status flag to return the PDU decode status*/
...
#ifdef FEATURE_RLC_QXDM_LOGGING
  rlc_dl_pdu_cipher_info_type *rlci_dl_pdu_cipher_log_ptr = 
                               &(rlc_dl_table_ptr->rlc_dl_logging.rlci_dl_pdu_cipher_log);
#endif //FEATURE_RLC_QXDM_LOGGING
  ...
  ctl_blk = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, hdr_ptr->rlc_id);
  if (NULL == ctl_blk)
  {
    WL2_MSG_1(ERROR, rlc_dl_table_ptr, "RLC_ERR: rlc_id %d is not yet configured",
                                                hdr_ptr->rlc_id);
    status = FALSE;
    return(status);
  }
  ...
  {
    WL2_MSG_2(ERROR, rlc_dl_table_ptr, "RLC_ERR: lc_state %d is NULL for rlc_id %d",
                                                ctl_blk->lc_state,
                                                hdr_ptr->rlc_id);
    status = FALSE;
    return(status);
  ...
  else
  {
    ...
    {
      if (rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_var & RLC_DBG_DL_CIPHERING)
      {
        WL2_MSG_3(HIGH, rlc_dl_table_ptr, "B4 cipher %x %x %x",
      ...
      if (ctl_blk->lc_mode == UE_MODE_UNACKNOWLEDGED)
      {
        ...
        if (rlci_dl_um_cipher_pdu(rlc_ul_table_ptr, ctl_blk, cipher_ptr, rx_sn))
        {
             ...
             )
          {
            ...
            if (rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_var & RLC_DBG_DL_CIPHERING)
            {
              WL2_MSG_3(HIGH, rlc_dl_table_ptr, "Cipher Key %x, cnt_c %x, offset %x",
        ...
        if (!(ctl_blk->lc_state & DL_STOP))
        {
          if (rlc_pdu_header & 0x8000)
          {
            ...
            if (ctl_blk->lc_state & DL_RESET_PENDING)
            {
              ...
              if (rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_var & RLC_DBG_DL_CIPHERING)
              {
                WL2_MSG_3(HIGH, rlc_dl_table_ptr, "NA: LC %d is in RESET PENDING - DATA PDU Hdr: 0x%x Payload: 0x%x in this TTI",
                                                            hdr_ptr->rlc_id,
                                                            rlc_pdu_header,
            ...
            if (ctl_blk->cipher.cipher_mode != NO_CIPHER &&
                rlci_dl_am_cipher_pdu(ctl_blk, rx_sn, rlc_dl_table_ptr))
            {
              /*---------------------------------------------------------------------
              ...
              {
                ctl_blk->cipher.new_cipher_on = FALSE;
                cipher_ptr->count_c = rlci_dl_am_get_count_c(rlc_ul_table_ptr, ctl_blk, rx_sn,
                                                        &(cipher_ptr->cipher_key_id),
      ...
      ---------------------------------------------------------------------*/
      if (rlc_dl_table_ptr->rlc_dl_logging.rlci_dl_pdu_cipher_log_on && !rlci_dl_pdu_cipher_log_ptr->cipher_pdu_logging &&
           ...
           (rx_sn < 20)))
      {
        ...
        if (rlci_dl_pdu_cipher_log_ptr->num_pdu < MAX_CIPHER_PDU)
        {
          cipher_idx = rlci_dl_pdu_cipher_log_ptr->num_pdu;
          rlci_dl_pdu_cipher_log_ptr->cipher_log_data[cipher_idx].log_dl_rlc_id= hdr_ptr->rlc_id;
          rlci_dl_pdu_cipher_log_ptr->cipher_log_data[cipher_idx].cipher_key_idx =
                                                            cipher_ptr->cipher_key_id;
          rlci_dl_pdu_cipher_log_ptr->cipher_log_data[cipher_idx].cipher_algo=
                                                          ctl_blk->cipher.cipher_algo;
          rlci_dl_pdu_cipher_log_ptr->cipher_log_data[cipher_idx].count_c= cipher_ptr->count_c;
          rlci_dl_pdu_cipher_log_ptr->cipher_log_data[cipher_idx].b4_cipher =
                      b_unpackd(hdr_ptr->hdr_ptr, hdr_ptr->rlc_pdu_start_offset, 32);
          /*------------------------------------------------------------------
            After filling in the PDU cipher parameters, increase the index
            to the next entry.
          --------------------------------------------------------------------*/
          rlci_dl_pdu_cipher_log_ptr->num_pdu ++;
        }
      }
#endif //FEATURE_RLC_QXDM_LOGGING
    } /* NON TM */
...
{
  rlci_dl_ctrl_blk_type *dl_ctrl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, lc_id);
...
)
{
  ...
  ctrl_blk = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, dl_rlc_data_id);
  if (NULL == ctrl_blk)
  {
    L2_ACQ_DL_UNLOCK();
    WL2_MSG_1(ERROR, rlc_dl_table_ptr, "Control Block NULL, DL RLC ID %d",
    	                                       dl_rlc_data_id);
    return;
  }
  ...
  if (cmd_id == RLCI_UL_RESET_DONE)
  {
    ...
    datapath_mutex = L2_ACQ_UL_LW_LOCK(rlc_ul_table_ptr->as_id, ctrl_blk->ctrl.am_ctrl.am_common->ul_data_id);

    ul_ctrl_blk_ptr =  rlci_get_ul_ctl_blk_ptr_by_rlc_id(rlc_ul_table_ptr, ctrl_blk->ctrl.am_ctrl.am_common->ul_data_id);
    /* Complete the RESET procedure. Return back to the original state. */
    /*lint -e64 -e641 */
    ctrl_blk->lc_state &= (~DL_RESET_PENDING);
    /*lint +e64 +e641 */
    if ((rlci_dl_fc_status(rlc_ul_table_ptr))&&  (ul_ctrl_blk_ptr != NULL)
        ...
        )
    {
      ...
      rlc_dl_multi_client_fc_modify_max_flow
        (rlc_ul_table_ptr, ctrl_blk->ctrl.am_ctrl.am_common->ul_data_id,
         ctrl_blk->ctrl.am_ctrl.dl_state_var.rx_win,
         FALSE);
      ...
      {
        rlci_change_peer_tx_win(rlc_ul_table_ptr, &(ul_ctrl_blk_ptr->ctrl.am_ctrl), rlc_win_size);
      }
      WL2_MSG_1(HIGH, rlc_dl_table_ptr, "Sending  win SUFI with win size %d configured by FC before RESET",
                                                 rlc_win_size);
    }
    else
    {
      ...
      if(ctrl_blk->ctrl.am_ctrl.dl_ctl_cnfg->rlc_info.rx_window_size > MAX_RX_WINDOW_SIZE)
      {
        WL2_MSG_1(LOW, rlc_dl_table_ptr, "Max window size on DL = %d",
                                                  MAX_RX_WINDOW_SIZE);
        ctrl_blk->ctrl.am_ctrl.dl_state_var.rx_win = MAX_RX_WINDOW_SIZE;
        ...
        rlc_dl_multi_client_fc_modify_max_flow
          (rlc_ul_table_ptr, ctrl_blk->ctrl.am_ctrl.am_common->ul_data_id,
           ctrl_blk->ctrl.am_ctrl.dl_state_var.rx_win,
           FALSE);
        rlci_change_peer_tx_win(rlc_ul_table_ptr, &(ul_ctrl_blk_ptr->ctrl.am_ctrl),
    ...
    if (rlci_ul_fc_status(rlc_ul_table_ptr))
    {
      ...
      )
      {
         ...
         rlci_ul_fc_calc_new_tx_win_size(rlc_dl_table_ptr, ul_ctrl_blk_ptr);
      }
    }
    rlci_initiate_reset_for_lower_dl_hfn(rlc_ul_table_ptr, rlc_dl_table_ptr, dl_rlc_data_id);

#ifdef FEATURE_RLC_QXDM_LOGGING
    rlc_dl_table_ptr->rlc_dl_logging.dl_log_state.dl_state[0].rlc_id = dl_rlc_data_id;
    rlc_dl_table_ptr->rlc_dl_logging.dl_log_state.dl_state[0].lc_state = AM_DL_DATA_TRANSFER_READY;
    rlc_dl_table_ptr->rlc_dl_logging.dl_log_state.nchan = 1;
    rlc_dl_send_state_log_packet(rlc_dl_table_ptr);
#endif
  ...
  else if (cmd_id == RLCI_DL_RESET_REQ)
  {
    ...
    rlci_reset_dl_am(rlc_ul_table_ptr, rlc_dl_table_ptr, ctrl_blk, FALSE);
  }
  else
  {
    WL2_MSG_0(HIGH, rlc_dl_table_ptr, "Unknown DL RLC reset msg");
  }
...
)
{
  ...
  if (rrc_rlc_cmd_ptr == NULL)
  {
   WRLC_ERR_FATAL(rlc_dl_table_ptr->as_id, "Could not get cmd buffer",0,0,0);
   /*lint -e527 */
   return;
   /*lint +e527 */
  }
  rrc_rlc_cmd_ptr->cmd_hdr.cmd_id = RRC_CRLC_RELEASE_ALL_CNF;
  WL2_MSG_1(LOW, rlc_dl_table_ptr, "Release ALL Downlink Channels, No.of DL-LC %d",
                                            rlc_dl_table_ptr->active_ch);
  ...
  -------------------------------------------------------------------------*/
  L2_MUTEX_LOCK(&rlc_mutexes[rlc_dl_table_ptr->as_id].rlc_channel_release_mutex);
  while (rlc_dl_table_ptr->active_ch > 0)
  {
    /*-----------------------------------------------------------------------
      Check if the channel is in NULL state. If it is not, release
      the channel.
    -----------------------------------------------------------------------*/
    ctrl_blk_id = rlci_get_dl_ctl_blk_index_for_rlc_id(rlc_dl_table_ptr, i);
    if (ctrl_blk_id < UE_MAX_DL_RLC_ENTITY)
    {
      num_ch_released++;
      ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_index(rlc_dl_table_ptr, ctrl_blk_id);
      ...
      else
      {
        ...
        if((ctl_blk_ptr->lc_mode == UE_MODE_UNACKNOWLEDGED) 
           ...
           ||(ctl_blk_ptr->lc_mode == UE_MODE_DL_CTCH)
#endif
          )
        {
          {
      ...
      {
        rlc_dl_table_ptr->rlc_dl_logging.dl_log_state.dl_state[j].rlc_id = i;
        rlc_dl_table_ptr->rlc_dl_logging.dl_log_state.dl_state[j].lc_state = RLC_DL_NULL_STATE;
        j++;
      }
#endif
    }
  ...
  WL2_MSG_2(HIGH, rlc_dl_table_ptr, "Num DL LC released %d, No.of active DL-LC %d",
                                     num_ch_released,
                                     rlc_dl_table_ptr->active_ch);
  for (index = 0, act_dl_am_chnl = 0; index < UE_MAX_DL_LOGICAL_CHANNEL; index++)
  {
  ...
  if (j > 0)
  {
    rlc_dl_table_ptr->rlc_dl_logging.dl_log_state.nchan = j;
    rlc_dl_send_state_log_packet(rlc_dl_table_ptr);
  }
#endif
...
uint16 rlci_get_rx_win_size (rlci_downlink_table_type *rlc_dl_table_ptr, rlc_lc_id_type rlc_id)
{
  ...
  if ((ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, rlc_id)) != NULL)
  {
...
)
{
  ...
  {
    WL2_MSG_0(MED, rlc_dl_table_ptr, "Sending log packet");
    rlc_dl_tx_am_statistic_pkt(rlc_dl_table_ptr);
  }
...
{
  uint8  count;                        /* Number of active Channels       */
  uint8  index = 0;
  uint8 array_len = 0;
  rlci_dl_ctrl_blk_type *ctl_blk_ptr;
  /*------------------------------------------------------------------------
    	...
    	(ctl_blk_ptr->lc_state != DL_NULL_STATE))
    {
          ...
          ctl_blk_ptr->cipher.cipher_mode != NO_CIPHER)
      {
        if (ctl_blk_ptr->lc_mode == UE_MODE_ACKNOWLEDGED)
        {
          ...
          hfn_req->hfn_per_rb_dl[hfn_req->dl_nchan].hfn = ctl_blk_ptr->cipher.count >> 12;
          hfn_req->hfn_per_rb_dl[hfn_req->dl_nchan].rlc_sn = ctl_blk_ptr->ctrl.am_ctrl.dl_state_var.vr_h;          
          hfn_req->hfn_per_rb_dl[hfn_req->dl_nchan].rlc_id = ctl_blk_ptr->ctrl.am_ctrl.am_common->dl_data_id;
        }
        else
        {
          /*----------------------------------------------------------------
          ...
          hfn_req->hfn_per_rb_dl[hfn_req->dl_nchan].hfn  = ctl_blk_ptr->cipher.count >> 7;
          hfn_req->hfn_per_rb_dl[hfn_req->dl_nchan].rlc_sn = ctl_blk_ptr->ctrl.um_ctrl.vr_ur;
          hfn_req->hfn_per_rb_dl[hfn_req->dl_nchan].rlc_id = ctl_blk_ptr->ctrl.um_ctrl.rlc_id;
        }

        WL2_MSG_3(HIGH, rlc_dl_table_ptr, "RLC_DBG: RRC req DL Cipher LC %d, rlc_sn %d hfn 0x%x",
  ...
  while(count > 0 && index < UE_MAX_DL_RLC_ENTITY)
  {
    	...
    	(ctl_blk_ptr->lc_state != DL_NULL_STATE))
    {
          ...
          (count_c_req->dl_nchan < (UE_MAX_DL_UM_CHANNEL + UE_MAX_AM_ENTITY)))
      {
        if (ctl_blk_ptr->lc_mode == UE_MODE_UNACKNOWLEDGED)
        {
          count_c_req->count_c_per_rb_dl[count_c_req->dl_nchan].count_c = ctl_blk_ptr->cipher.count;
          count_c_req->count_c_per_rb_dl[count_c_req->dl_nchan].rlc_id = ctl_blk_ptr->ctrl.um_ctrl.rlc_id;
        }
        else
        {
          count_c_req->count_c_per_rb_dl[count_c_req->dl_nchan].count_c = ctl_blk_ptr->cipher.count;
          count_c_req->count_c_per_rb_dl[count_c_req->dl_nchan].rlc_id = ctl_blk_ptr->ctrl.am_ctrl.am_common->dl_data_id;
        }
...
void rlc_dl_am_backup_timer(rlci_downlink_table_type *rlc_dl_table_ptr)
{
  ...
  active_ch = rlc_dl_table_ptr->active_ch;
  while(active_ch > 0 && i < UE_MAX_DL_RLC_ENTITY)
  {
    ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_index(rlc_dl_table_ptr, i);
    if ((ctl_blk_ptr != NULL) &&
    	(ctl_blk_ptr->lc_state != DL_NULL_STATE))
    {
      ...
      if ((ctl_blk_ptr->lc_mode == UE_MODE_ACKNOWLEDGED) &&
          ((ctl_blk_ptr->lc_state == DL_DATA_TRANSFER_READY) ||
           (ctl_blk_ptr->lc_state == DL_RESET_PENDING)))
      {
        /*---------------------------------------------------------------------
          Do not resume the timer before it actually starts.
        ----------------------------------------------------------------------*/
        if(!(ctl_blk_ptr->ctrl.am_ctrl.dl_state_var.fst_rx_pdu) &&
           ++(ctl_blk_ptr->ctrl.am_ctrl.dl_state_var.backup_timer_cnt) > RLC_BACKUP_TIMER_CNT)
        {
          ctl_blk_ptr->ctrl.am_ctrl.dl_state_var.backup_timer_cnt = 0;

          ctl_blk_ptr->ctrl.am_ctrl.dl_tmr_sts = RLC_AM_TIMER_SUSPEND;
          WL2_MSG_1(LOW, rlc_dl_table_ptr, "Set timer from backup, LC %d",
                                                     ctl_blk_ptr->ctrl.am_ctrl.am_common->dl_data_id);
        }
      }
      active_ch --;
    }
...
{
  rlci_dl_ctrl_blk_type *ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, lc_id);
  if (NULL != ctl_blk_ptr)
  {
    L2_ACQ_DL_LOCK();
    ctl_blk_ptr->ctrl.am_ctrl.dl_tmr_sts = RLC_AM_TIMER_STOP;
    L2_ACQ_DL_UNLOCK();
  }
...
)
{
  ...
  {
    L2_ACQ_DL_LOCK();
    ctl_blk_ptr->ctrl.am_ctrl.dl_tmr_sts = RLC_AM_TIMER_RUNNING;
    L2_ACQ_DL_UNLOCK();
  }
...
)
{
  ...
  {
    dl_ctl_blk = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, mac_hs_reset_req->rlc_id_list[i]);
    /*lint -save -e655 */
    if((dl_ctl_blk == NULL) ||
       (dl_ctl_blk->lc_mode < UE_MODE_ACKNOWLEDGED) ||
       (dl_ctl_blk->lc_state != DL_DATA_TRANSFER_READY))
    {
      WL2_MSG_1(ERROR, rlc_dl_table_ptr, "No status report after HS reset, LC:%d",
                                                  mac_hs_reset_req->rlc_id_list[i]);
      continue;
    }
    /*lint -restore */

    rlci_rx_status_changed(rlc_ul_table_ptr, dl_ctl_blk->ctrl.am_ctrl.am_common->ul_control_id,
                         FALSE);
    dl_ctl_blk->ctrl.am_ctrl.dl_status_opt_bitmap.next_sn_valid = FALSE;
    rlci_set_ul_num_sn_li(rlc_ul_table_ptr, dl_ctl_blk->ctrl.am_ctrl.am_common->ul_data_id,
...
)
{
  ...
  for(i=0; i<abort_cipher_info->nchan; i++)
  {
    ...
    -----------------------------------------------------------------------*/
    ctl_blk = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, abort_cipher_info->lc_id[i]);
    if (NULL != ctl_blk)
    {
      WL2_MSG_2(HIGH, rlc_dl_table_ptr, "Aborting DL Cipher Config for RbId: %d,LC: %d",
...
{
  rlci_dl_ctrl_blk_type *dl_ctrl_blk = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, rlc_id);
  if (NULL != dl_ctrl_blk)
  {
  ...
  {
    WL2_MSG_1(HIGH, rlc_dl_table_ptr, "Invalid RLC DL LC, %d",
                                               rlc_id);
  }
}
...
)
{
  ...
  dl_ctrl_blk = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, rlc_id);
  if (NULL != dl_ctrl_blk)
  {
...
)
{
  rlc_dl_pdu_cipher_info_type *rlci_dl_pdu_cipher_log_ptr = 
                               &(rlc_dl_table_ptr->rlc_dl_logging.rlci_dl_pdu_cipher_log);
  if(rlci_dl_pdu_cipher_log_ptr->num_pdu)
  {
    /*-----------------------------------------------------------------------
      Lock the cipher pdu log info updation by setting the flag.
    -----------------------------------------------------------------------*/
    rlci_dl_pdu_cipher_log_ptr->cipher_pdu_logging = TRUE;

    /*-----------------------------------------------------------------------
      The logging data is filled in at the time before a PDU is decoded.
    -----------------------------------------------------------------------*/
    rlc_dl_tx_pdu_cipher_log_pkt(rlc_dl_table_ptr, rlci_dl_pdu_cipher_log_ptr);

    /*-----------------------------------------------------------------------
      Reset num_pdu after sending logging data to DIAG.
    -----------------------------------------------------------------------*/
    rlci_dl_pdu_cipher_log_ptr->num_pdu = 0;

    /*-----------------------------------------------------------------------
      Unlock cipher pdu log info updation.
    -----------------------------------------------------------------------*/
    rlci_dl_pdu_cipher_log_ptr->cipher_pdu_logging = FALSE;
  }
...
{
  rlci_downlink_table_type *rlc_dl_table_ptr = RLCI_GET_DL_TBL_PTR(as_id);
  if(rlc_dl_table_ptr->rlc_dl_logging.rlci_dl_pdu_cipher_log.num_pdu)
  {
    /*-----------------------------------------------------------------------
      Set the RLC_DL_LOG_CIPHER_PKT_SIG rex signal, if ciher PDUs 
      needs to be logged
    -----------------------------------------------------------------------*/
    (void)rex_set_sigs(wcdma_l2_dl_get_tcb(as_id), RLC_DL_LOG_CIPHER_PKT_SIG);
  }
...
)
{
  ...
  ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, cleanup_wm_cmd->lc_id);
  if (ctl_blk_ptr != NULL)
  {
    if(ctl_blk_ptr->dl_wm_ptr == NULL)
    {
...
)
{
  ...
  if(rrcsmc_dl_stop_req->nchan < (UE_MAX_DL_UM_CHANNEL + UE_MAX_AM_ENTITY))
  {
    for (i = 0; i < rrcsmc_dl_stop_req->nchan; i++)
    {
      ...
      ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, lc_id);
      ...
      if (NULL != ctl_blk_ptr)
      {
        ...
        WL2_MSG_1(HIGH, rlc_dl_table_ptr, "DL  LC %d - Stop", lc_id);
      }
      else
      {
        WL2_MSG_1(ERROR, rlc_dl_table_ptr, "DL LC %d not config'd, No Stop",
                                                    lc_id);
      }
    }
  }
  ...
  {
    WL2_MSG_1(ERROR, rlc_dl_table_ptr, "RLC_ERR: Num Chan %d provided by RRC out of bounds",
...
)
{
  ...
  for (i = 0; i < continue_ptr->nchan; i++)
  {
    ...
    /*lint -restore */
    {
      ...
      WL2_MSG_1(HIGH, rlc_dl_table_ptr, "DL LC %d Continue", lc_id);
    }
    else
    {
      WL2_MSG_1(HIGH, rlc_dl_table_ptr, "DL LC %d not in Stop State",
                                                 lc_id);
    }
  }
...
void rlc_debug_print_data(uint32 num_valid_words, rlci_downlink_table_type *rlc_dl_table_ptr)
{
  uint32 log_offset;
  uint32 *debug_array;
  
  debug_array =  rlc_dl_table_ptr->rlc_dl_logging.rlc_debug_pdu_dump_array;
  log_offset = 0;
  
  while (num_valid_words > 0)
  {
    WL2_MSG_8(ERROR, rlc_dl_table_ptr, "RLC_PDU_DUMP: 0x%4x 0x%4x 0x%4x 0x%4x 0x%4x 0x%4x 0x%4x 0x%4x", 
...
void rlc_enh_logging_data_dump
(
  rlci_downlink_table_type *rlc_dl_table_ptr,
...
)
{
  ...
  memset(rlc_dl_table_ptr->rlc_dl_logging.rlc_debug_pdu_dump_array, 0, (sizeof(uint32) * RLC_DEBUG_PDU_DUMP_WORDS));
  rlc_dl_table_ptr->rlc_dl_logging.rlc_debug_pdu_size_bits = pdu_size_in_bits;
  num_words = CONVERT_BITS_TO_WORDS(pdu_size_in_bits);
  num_bytes = CONVERT_BITS_TO_BYTES(pdu_size_in_bits);
  
  /* Store the first word */
  rlc_dl_table_ptr->rlc_dl_logging.rlc_debug_pdu_dump_array[array_offset] = temp_word;
  array_offset++;
  ...
  while (bytes_read < num_bytes)
  {
                     ...
                     rlc_debug_pdu_bytes, bytes_to_read)) > 0)
    {
      ...
      {
        rlc_dl_table_ptr->rlc_dl_logging.rlc_debug_pdu_dump_array[array_offset] = rlc_copy_to_word(rlc_debug_pdu_bytes + bytes_written);
        array_offset++;
        bytes_written += 4;
      }
    }
  ...
  WL2_MSG_2(HIGH, rlc_dl_table_ptr, "RLC_PDU_DUMP: rlc_id %d - PDU Size %d bits",
                                             complete_pdu_ptr->rlc_id,
                                             pdu_size_in_bits);
  
  rlc_debug_print_data(num_words, rlc_dl_table_ptr);
}
...
===========================================================================*/
void rlc_enh_logging_data_handler
(
  rlci_uplink_table_type *rlc_ul_table_ptr,
  rlci_downlink_table_type *rlc_dl_table_ptr,
  rlci_dl_ctrl_blk_type *ctrl_blk_ptr, 
...
)
{
  ...
  if (ctrl_blk_ptr->lc_mode == UE_MODE_ACKNOWLEDGED)
  {
    ...
    if (rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_var & RLC_DBG_DL_PDU_DUMP)
    {
      ...
      {
        WL2_MSG_1(ERROR, rlc_dl_table_ptr, "DL PDU: 0x%x",
    ...
    if(dcbit == 0)
    {
      ...
      if(rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_var & RLC_DBG_DL_CTRL)
      {
        WL2_MSG_3(ERROR, rlc_dl_table_ptr, "RLC_ERR: NA: rlc_id %d, rlc_hdr_data 0x%x, temp_word 0x%x",
                 complete_pdu_ptr->rlc_id,
  ...
  else
  {
    ...
    if (ctrl_blk_ptr->lc_mode == UE_MODE_UNACKNOWLEDGED)
    {
      ...
      pdu_size_in_bits = (((uint16) (complete_pdu_ptr->iovec.length + 
                                      RLC_UM_PDU_HDR_SIZE_BYTES)) << 3);
#if ((defined (FEATURE_HSDPA_MAC_EHS)) && (defined (FEATURE_WCDMA_REL7)))
      dl_chan_um_pdu_ptr->pdu_size[dl_chan_um_pdu_ptr->num_pdu] = pdu_size_in_bits;
#else     
      dl_chan_um_pdu_ptr->pdu_size = pdu_size_in_bits;
#endif
      if ((dsm_extract(complete_pdu_ptr->iovec.ptr, 
                       complete_pdu_ptr->iovec.offset, temp_data, 3)) > 0
         )
  ...
  if (rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_pdu_dump)
  {
   rlc_enh_logging_data_dump(rlc_dl_table_ptr, ctrl_blk_ptr, complete_pdu_ptr);
  }
}
...
#if ((defined (FEATURE_HSDPA_MAC_EHS)) && (defined (FEATURE_WCDMA_REL7)))
    if ((hs_path == FALSE) || 
        (mac_ehs_query_hs_type(rlc_dl_table_ptr->as_id) != MAC_EHS) || 
        (lc_id < RLC_DL_AM_USR_PLANE_START_LC_ID))
#endif
    {
      rlc_dl_enh_tx_am_pdu_log_pkt(rlc_dl_table_ptr, ctrl_blk_ptr, lc_id);
    }
  }
  else
  {
...
#endif
      {
        rlc_dl_enh_tx_um_pdu_log_pkt(rlc_dl_table_ptr, ctrl_blk_ptr);
      }
    }
#endif
  }
...
)
{
  ...
  while (ptr != NULL)
  {
    if (ptr->app_field == 0)
    {
      if (rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_var & RLC_DBG_REASSEMBLY_1)
      {
        WL2_MSG_8(HIGH, rlc_ul_table_ptr,
              "RLC DBG DL: rlc_sdu_discard: dsm_ptr 0x%x, pkt_ptr 0x%x, data_ptr 0x%x, ref_cnt %d, app_field 0x%x, dup_ptr 0x%x, used 0x%x size 0x%x",
               ptr, ptr->pkt_ptr, ptr->data_ptr, ptr->references, ptr->app_field, ptr->dup_ptr, ptr->used, ptr->size );
      }
...
)
{
  ...
  rlci_uplink_table_type *rlc_ul_table_ptr = RLCI_GET_UL_TBL_PTR(as_id);
  L2_MUTEX_LOCK(&(rlc_mutexes[rlc_ul_table_ptr->as_id].rlc_mac_dl_pdu_mutex));
  ...
  if (ptr != NULL)
  {
    if (ptr->app_field > 0)
    {
      ...
      if (ptr->app_field == 0)
      {
        if(rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_var & RLC_DBG_MEM_LEAK)
        {
    ...
    {
      WRLC_ERR_FATAL(rlc_ul_table_ptr->as_id, "RLC_ERR: rlc_discard_dsm_item: app_field is already zero", 
                 0, 0, 0);
    }
  }
  else
  {
    WRLC_ERR_FATAL(rlc_ul_table_ptr->as_id, "RLC_ERR: rlc_discard_dsm_item: dsm_ptr is NULL ptr", 
               0, 0, 0);
  }

  L2_MUTEX_UNLOCK(&(rlc_mutexes[rlc_ul_table_ptr->as_id].rlc_mac_dl_pdu_mutex));
...
)
{
  ...
  {
    rlc_discard_dsm_item(rlc_dl_table_ptr->as_id, &(complete_pdu_ptr->iovec.ptr));
    complete_pdu_ptr->iovec.ptr = NULL;
  }
  else
  {
    WL2_MSG_1(ERROR, rlc_dl_table_ptr, "RLC_ERR: NULL iovec ptr for LC %d",
                                                complete_pdu_ptr->rlc_id);
  }
...
uint8 rlc_enh_calc_rlc_hdr_length(sys_modem_as_id_e_type as_id, rlc_lc_id_type rlc_id, uint8 first_byte, boolean *is_tm_mode)
{
  uint8 length = 0;
  rlci_dl_ctrl_blk_type           *ctrl_blk_ptr;
  ...
  ctrl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, rlc_id);
  ...
  {
    WL2_MSG_1(ERROR, rlc_dl_table_ptr, "RLC_ERR: No Control Blk exists for this rlc_id %d",
                                                rlc_id);
  }
...
)
{
  ...
  num_sn_li = (ctl_blk_ptr != NULL) ? ctl_blk_ptr->ctrl.am_ctrl.num_sn_li : 0;
  return num_sn_li;
}
...
void rlci_enh_update_nak_list(rlci_uplink_table_type *rlc_ul_table_ptr, rlci_dl_ctrl_blk_type *ctrl_blk_ptr)
{
  ...
  ---------------------------------------------------------------------*/
  datapath_mutex = L2_ACQ_UL_LW_LOCK(rlc_ul_table_ptr->as_id, am_ctrl_ptr->am_common->ul_control_id);
  ...
  if (RLCI_SEQ_GT(dl_state_ptr->vr_r, dl_state_ptr->old_vr_r, RLC_AM_MODULUS))
  {
          ...
          (vr_r = RLCI_AM_SEQ_INC(vr_r)))
    {
      ...
      if (rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_var & RLC_DBG_DL_AM)
      {
        WL2_MSG_2(HIGH, rlc_ul_table_ptr, "BITMAP_TST:num_sn_li %d, rcvd_sn %d",
                                                   num_sn_li, vr_r );
      }
    }
  }
  ...
  if (rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_var & RLC_DBG_DL_AM)
  {
    WL2_MSG_3(HIGH, rlc_ul_table_ptr, "BITMAP_TST:num_sn_li %d, vr_r %d, max_rx_sn %d",
                                ...
                                rlci_dl_ctrl_blk_type *ctrl_blk_ptr)
{
  ...
  rlci_set_ul_num_sn_li(rlc_ul_table_ptr, am_ctrl_ptr->am_common->ul_data_id,
                        am_ctrl_ptr->num_sn_li);
  if(rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_var & RLC_DBG_DL_CTRL)
  {
    WL2_MSG_3(HIGH, rlc_ul_table_ptr, "DL_CTRL: Sending status rpt for lc %d - report_vr_h %d, vr_r %d",
  ...
  /* Let the UL know that it has to send a status report to the peer entity */
  rlci_rx_status_changed(rlc_ul_table_ptr, am_ctrl_ptr->am_common->ul_data_id, TRUE);
      ...
      am_ctrl_ptr->dl_ctl_cnfg->rlc_info.status_info.status_prohibit)
  {
    ...
    if(rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_var & RLC_DBG_DL_CTRL)
    {    
      WL2_MSG_2(HIGH, rlc_ul_table_ptr, "DL_CTRL: Turn on Status Prohibit LC %d, timer_val %d",
               am_ctrl_ptr->am_common->dl_data_id, 
                                                 am_ctrl_ptr->dl_ctl_cnfg->rlc_info.status_info.status_prohibit);
    }
  }
                                     ...
                                     rlci_dl_ctrl_blk_type *ctrl_blk_ptr)
{
  ...
  rlci_enh_update_nak_list(rlc_ul_table_ptr, ctrl_blk_ptr);
  ...
  if (dl_state_var_ptr->rcvd_poll == TRUE) 
  {
    ...
    {
      rlci_enh_status_generation(rlc_ul_table_ptr, ctrl_blk_ptr);
    }
    L2_ACQ_UL_LW_UNLOCK(datapath_mutex);
    L2_ACQ_DL_UNLOCK();
  }
  
  if (rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_var & RLC_DBG_REASSEMBLY)
  {
    WL2_MSG_2(HIGH, rlc_ul_table_ptr, "reassembly_post_pdu: old vrR %d, vrR %d",
  ...
  if (dl_state_var_ptr->old_vr_r != dl_state_var_ptr->vr_r)
  {
    if (rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_var & RLC_DBG_REASSEMBLY)
    {
      WL2_MSG_0(HIGH, rlc_ul_table_ptr, "reassembly_post_pdu: Calling reassemble");
    }
    
    rlci_enh_dl_am_reassemble_data_pdus(rlc_ul_table_ptr, rlc_dl_table_ptr, ctrl_blk_ptr, dl_state_var_ptr->old_vr_r);
  /* Since there is no lock used inside rlci_enh_dl_am_reassemble_data_pdus(), there is 
     ...
     at a given time*/
#if 0
      WL2_MSG_0(HIGH, rlc_ul_table_ptr, "reassembly_post_pdu: Posting Reassembly SIGNAL");
                                       ...
                                       mac_ul_table_type *ul_table_ptr)
{
...
#ifdef FEATURE_UMTS_BMC_MSIM
  dsm_item_type *temp_dsm_item_ptr = NULL;
  uint8 partial_sdu_raw[6];
  uint16 length = 0;
#endif

  if (ctrl_blk_ptr == NULL)
  {
    WL2_MSG_1(ERROR, rlc_ul_table_ptr, "RLC_ERR: post processing on prev_rlc_id %d - NULL Ctrl Blk",
                                                prev_rlc_id);
    return;
  }
  ...
  if (ctrl_blk_ptr->lc_mode == UE_MODE_ACKNOWLEDGED)
  {
    rlci_enh_dl_reassembly_post_pdu(rlc_ul_table_ptr, rlc_dl_table_ptr, ctrl_blk_ptr);
    
    #ifdef FEATURE_WCDMA_TRIGGER_BASED_RACH
    rlc_enh_hs_rach_check_rach_trigger(rlc_ul_table_ptr, prev_rlc_id);
    #endif
  }
  else if (ctrl_blk_ptr->lc_mode == UE_MODE_UNACKNOWLEDGED)
  {
    dl_pdu_info_ptr = rlci_enh_get_temp_pdu_info_ptr(rlc_dl_table_ptr);
    if (dl_pdu_info_ptr->dsm_ptr != NULL)
    {
      if (dl_pdu_info_ptr->dsm_ptr->references > 0) 
      {
        rlci_enh_update_um_partial_sdu_ptr(rlc_ul_table_ptr, ctrl_blk_ptr, 
                                         dl_pdu_info_ptr, NULL);
      #ifdef FEATURE_UMTS_BMC_MSIM
        if ((ctrl_blk_ptr->lc_type == UE_LOGCHAN_CTCH) &&
	    (FALSE == ctrl_blk_ptr->ctrl.um_ctrl.checked_for_cmas))
	{
	  temp_dsm_item_ptr = ctrl_blk_ptr->ctrl.um_ctrl.partial_sdu_ptr;
          while (temp_dsm_item_ptr != NULL)
          {
            length += temp_dsm_item_ptr->used;
            if (length >= RLC_NUM_BYTES_TO_PASS_TO_NAS)  /*Pass the 1st 6 bytes of SDU to NAS*/
            {
              dsm_extract(ctrl_blk_ptr->ctrl.um_ctrl.partial_sdu_ptr, 0 , partial_sdu_raw, sizeof(partial_sdu_raw));
                cb_get_start_pdu_from_rlc(partial_sdu_raw, RLC_NUM_BYTES_TO_PASS_TO_NAS, wcdma_convert_stack_index_to_subid(rlc_dl_table_ptr->as_id));
              ctrl_blk_ptr->ctrl.um_ctrl.checked_for_cmas = TRUE;
              WL2_MSG_0(HIGH, rlc_ul_table_ptr, "Calling NAS API since partial SDU size > 6 bytes");
              break;
            }
            temp_dsm_item_ptr = temp_dsm_item_ptr->pkt_ptr;    
          }
	}
      #endif	
      }
      else  
      {
         ...
         */
        WL2_MSG_3(HIGH, rlc_ul_table_ptr, "RLC_DBG: DSM-item (within dl_pdu_info) is already freed LC-id %d vr_ur %d dl_pdu_info_ptr 0x%x",
                                                   prev_rlc_id,
  ...
  if (rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_var & RLC_DBG_REASSEMBLY)
  {
    WL2_MSG_1(HIGH, rlc_ul_table_ptr, "In Post processing - nSDU %d",
                                               ctrl_blk_ptr->n_sdu);
  }
...
#ifdef FEATURE_RLC_QXDM_LOGGING
  if (((ctrl_blk_ptr->lc_mode == UE_MODE_ACKNOWLEDGED) &&
        RLCI_GET_AM_LOGGING_CONFIG(rlc_dl_table_ptr, prev_rlc_id)) ||
      ((ctrl_blk_ptr->lc_mode == UE_MODE_UNACKNOWLEDGED) && 
        RLCI_GET_UM_LOGGING_CONFIG(rlc_dl_table_ptr)))
  {
    rlc_enh_logging_post_data_handler(rlc_dl_table_ptr, ctrl_blk_ptr, prev_rlc_id, hs_path);
  }
#endif
                                    ...
                                    mac_ul_table_type *ul_table_ptr)
{
  ...
  {
    WL2_MSG_0(ERROR, rlc_ul_table_ptr, "RLC_ERR: RLC Internal TSN Info Q is empty");
    return;
  }

#ifdef FEATURE_WCDMA_DL_DATAPATH_LOG_PKT
  rlc_dl_table_ptr->rlc_global_sub_fn = tsn_info_buf_ptr->global_sub_fn;
#endif
  ...
  {
    WRLC_ERR_FATAL(rlc_dl_table_ptr->as_id, "TSN %d - Not expected Num partial in RLC - Partial %d, Complete %d", 
  ...
  for (index = 0; index < tsn_info_buf_ptr->num_complete_pdus; index++)
  {
    ...
    if (prev_rlc_id != complete_pdu_ptr->rlc_id)
    {
      if (prev_rlc_id != 0xFF)
      {
        ...
        rlc_enh_rcvd_data_post_processing(rlc_ul_table_ptr, rlc_dl_table_ptr, ctrl_blk_ptr, prev_rlc_id, TRUE, ul_table_ptr);
      }
      
      /* Get the new ctrl Index and ctrl blk ptr */
      ctrl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, complete_pdu_ptr->rlc_id);
      if (NULL == ctrl_blk_ptr)
      {
        WL2_MSG_1(ERROR, rlc_ul_table_ptr, "RLC_ERR: rlc_id %d is not yet configured - Discard PDU",
                                                    complete_pdu_ptr->rlc_id);
        rlc_discard_dsm_item(rlc_dl_table_ptr->as_id, &(complete_pdu_ptr->iovec.ptr));
        continue;
      }
      ...
      {
        WL2_MSG_2(ERROR, rlc_ul_table_ptr, "RLC_ERR: lc_state %d is NULL for rlc_id %d - Discard PDU",
    ...
    if (NULL == ctrl_blk_ptr)
    {
      WL2_MSG_1(ERROR, rlc_ul_table_ptr, "ctrl_blk_ptr is NULL for LC id %d",
                                                  complete_pdu_ptr->rlc_id);
      continue;
    }
#ifdef FEATURE_RLC_QXDM_LOGGING
    if (((ctrl_blk_ptr->lc_mode == UE_MODE_ACKNOWLEDGED) &&
        ...
        ((ctrl_blk_ptr->lc_mode == UE_MODE_UNACKNOWLEDGED) &&
         (RLCI_GET_UM_LOGGING_CONFIG(rlc_dl_table_ptr))))
    {
      rlc_enh_logging_data_handler(rlc_ul_table_ptr, rlc_dl_table_ptr, ctrl_blk_ptr, complete_pdu_ptr, 
    ...
    switch (ctrl_blk_ptr->lc_mode)
    {
      case UE_MODE_ACKNOWLEDGED:
        rlci_enh_dl_process_am_pdu(rlc_ul_table_ptr, rlc_dl_table_ptr, ctrl_blk_ptr, complete_pdu_ptr, 
                                   first_pdu_on_this_lch);
       break;
      ...
      case UE_MODE_DL_CTCH:
        ...
        rlci_enh_dl_process_um_pdus(rlc_ul_table_ptr, rlc_dl_table_ptr, ctrl_blk_ptr, complete_pdu_ptr);
       break;
       
      case UE_MODE_TRANSPARENT:
       WL2_MSG_0(ERROR, rlc_ul_table_ptr, "RLC_ERR: rlc_enh_dl_pdu_data_handler_hs shouldn't get TM PDU");
       break;
       
      default:
       WL2_MSG_1(ERROR, rlc_ul_table_ptr, "RLC_ERR: rlc_enh_dl_pdu_data_handler_hs - Unknown Mode",
  ...
  if (mac_ehs_query_hs_type(rlc_dl_table_ptr->as_id) == MAC_EHS)
  {
    rlc_dl_enh_r7_pdu_log_pkt(rlc_dl_table_ptr);
  }
#endif
#endif

  /* Call the MAC-HS API to add processed TSN to free TSN Queue */
  mac_hs_enqueue_free_tsn(rlc_dl_table_ptr->as_id, tsn_info_buf_ptr);
}
...
)
{
  ...
  /* Process the PDUs recevied in the reverse order for TM data so that they will 
     be queued in the order C > B > A. */
  for (i = (rlc_enh_data_ptr->num_complete_pdus-1) ; i >= 0; i-- )
  {
    complete_pdu_ptr = &(rlc_enh_data_ptr->complete_pdu[i]);
    
    /* Get the new ctrl index and ctrl blk ptr */
    ctrl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, complete_pdu_ptr->rlc_id);
    if (NULL == ctrl_blk_ptr)
    {
      WL2_MSG_1(ERROR, rlc_ul_table_ptr, "RLC_ERR: rlc_id %d is not yet configured - Discard PDU in Task-context",
                                                  complete_pdu_ptr->rlc_id);
      continue;
    }
    ...
    if (ctrl_blk_ptr->lc_state == DL_NULL_STATE)
    {
      WL2_MSG_2(ERROR, rlc_ul_table_ptr, "RLC_ERR: lc_state %d is NULL for rlc_id %d - Discard PDU in Task-context",
                                                  ctrl_blk_ptr->lc_state,
                                                  complete_pdu_ptr->rlc_id);
      continue;
    }
        ...
        (ctrl_blk_ptr->context == TRUE))
    {
      ...
      L2_MUTEX_LOCK(&(rlc_mutexes[rlc_dl_table_ptr->as_id].rlc_dl_tm_mutex));

      if (NULL != ctrl_blk_ptr->dl_wm_ptr)
      {
        rlci_enh_dl_reassemble_tm_pdus(rlc_ul_table_ptr, ctrl_blk_ptr, complete_pdu_ptr);
        complete_pdu_ptr->iovec.ptr = NULL;
        num_processed++;
      }
      else
      {
        WL2_MSG_2(ERROR, rlc_ul_table_ptr, "RLC_ERR: WM NULL for LC %d, lc_state %d, Discard PDU in Task-context",
                                                    complete_pdu_ptr->rlc_id,
                                                    ctrl_blk_ptr->lc_state);
      }

      L2_MUTEX_UNLOCK(&(rlc_mutexes[rlc_dl_table_ptr->as_id].rlc_dl_tm_mutex));
    }
  }
...
void rlc_enh_dl_pdu_data_handler_non_hs(rlci_uplink_table_type *rlc_ul_table_ptr,
                                        rlci_downlink_table_type *rlc_dl_table_ptr,
                                        rlc_enh_data_ind_type  *rlc_enh_data_ptr,
                                        mac_ul_table_type *ul_table_ptr)
{
  ...
  {
    WL2_MSG_0(ERROR, rlc_dl_table_ptr, "RLC_ERR: RLC Internal Info Q is empty");
    return;
  }
  rlc_dl_table_ptr->dl_pdu_decode_result = TRUE;
  ...
  for (index = 0; index < rlc_enh_data_ptr->num_complete_pdus; index++)
  {
    ...
    if (prev_rlc_id != complete_pdu_ptr->rlc_id)
    {
      ...
      {
        /* Call the post processing function for the data rcvd on prev_rlc_id */
        rlc_enh_rcvd_data_post_processing(rlc_ul_table_ptr, rlc_dl_table_ptr, ctrl_blk_ptr, prev_rlc_id, FALSE, ul_table_ptr);
      }
      ...
      if (NULL == ctrl_blk_ptr)
      {
        WL2_MSG_1(ERROR, rlc_dl_table_ptr, "RLC_ERR: rlc_id %d is not yet configured - Discard PDU",
                                                    complete_pdu_ptr->rlc_id);
        rlc_discard_pdu(rlc_dl_table_ptr, complete_pdu_ptr);
        continue;
      }
      
      if (ctrl_blk_ptr->lc_state == DL_NULL_STATE)
      {
        WL2_MSG_2(ERROR, rlc_dl_table_ptr, "RLC_ERR: lc_state %d is NULL for rlc_id %d - Discard PDU",
                                                    ctrl_blk_ptr->lc_state,
                                                    complete_pdu_ptr->rlc_id);
        rlc_discard_pdu(rlc_dl_table_ptr, complete_pdu_ptr);
        continue;
    ...
    if (NULL == ctrl_blk_ptr)
    {
      WL2_MSG_1(ERROR, rlc_dl_table_ptr, "ctrl_blk_ptr is NULL for LC id %d",
                                                  complete_pdu_ptr->rlc_id);
      continue;
    }
#ifdef FEATURE_RLC_QXDM_LOGGING
    ...
    {
      case UE_MODE_ACKNOWLEDGED:
        rlc_dl_table_ptr->dl_pdu_decode_result = rlc_enh_data_ptr->decode_result;
        rlci_enh_dl_process_am_pdu(rlc_ul_table_ptr, rlc_dl_table_ptr, ctrl_blk_ptr, complete_pdu_ptr, 
                                   first_pdu_on_this_lch);
       break;
	...
	rlc_dl_table_ptr->dl_pdu_decode_result = rlc_enh_data_ptr->decode_result;
      ...
      case UE_MODE_TRANSPARENT:
        ...
        {
          WL2_MSG_1(ERROR, rlc_dl_table_ptr, "RLC_ERR: Discarding PDU for LC %d, not processed in ISR",
                                                    complete_pdu_ptr->rlc_id);
      ...
      default:
       WL2_MSG_1(ERROR, rlc_dl_table_ptr, "RLC_ERR: Unknown Mode %d",
                                                   ctrl_blk_ptr->lc_mode);
       break;
    }
  ...
  if (rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_var & RLC_DBG_REASSEMBLY)
  {
    ...
    WL2_MSG_0(HIGH, rlc_dl_table_ptr, "RLC_DL:R99 DATA PTR FREE: enqueue back to free Q ");
  }
  q_put(&rlc_dl_table_ptr->rlc_dl_data_q.rlc_enh_free_data_q, &(rlc_enh_data_ptr->link));
}
...
===========================================================================*/
void rlc_enh_dl_pdu_data_handler(rlci_uplink_table_type *rlc_ul_table_ptr, rlci_downlink_table_type *rlc_dl_table_ptr,
                                   mac_ul_table_type *ul_table_ptr)
{
  rlc_enh_data_ind_type       *rlc_enh_data_ptr;
  l1_dec_hs_tsn_info_type     *tsn_info_buf_ptr;
  rlc_mutexes_type            *rlc_mutex_ptr = RLC_GET_MUTEX_PTR(rlc_dl_table_ptr->as_id);
  uint32                       num_tsn;

  /* Store the UM and AM log info */
  (void) rlci_enh_dl_log_info_handler(rlc_dl_table_ptr);
  
  L2_MUTEX_LOCK(&(rlc_mutex_ptr->rlc_dl_data_process_mutex));
  
  /* Process the Non-HS data, if received */
  if ((num_tsn = q_cnt(&rlc_dl_table_ptr->rlc_dl_data_q.rlc_enh_data_q)) > 0)
  {
    while (num_tsn != 0)
    {
      /* Get the RLC Info from the rlc_enh_data_queue */
      if ((rlc_enh_data_ptr = (rlc_enh_data_ind_type *)q_get(&(rlc_dl_table_ptr->rlc_dl_data_q.rlc_enh_data_q))) 
             != NULL)
      {
        rlc_enh_dl_pdu_data_handler_non_hs(rlc_ul_table_ptr, rlc_dl_table_ptr, rlc_enh_data_ptr, ul_table_ptr);
      }
      num_tsn--;
    }
  }
  
  L2_MUTEX_LOCK(&(rlc_mutex_ptr->rlc_tsn_mutex));

  if ((l2_get_release_flag_indicator(rlc_ul_table_ptr->as_id) == L2_REL_INDICATOR_5) && 
      ((num_tsn = q_cnt(&rlc_dl_table_ptr->rlc_dl_data_q.rlc_tsn_info_q)) > 0))
  {
    rlc_clear_frc_unreco_flags(rlc_ul_table_ptr->as_id);
    while (num_tsn != 0)
    {
      /* Get the TSN from the rlc_tsn_info_q */
      if ((tsn_info_buf_ptr = (l1_dec_hs_tsn_info_type *)q_get(&rlc_dl_table_ptr->rlc_dl_data_q.rlc_tsn_info_q)) 
             != NULL)
      {
        /*Set the trace value to DL_TSN_RLC*/
        tsn_info_buf_ptr->tracer = DL_TSN_RLC;
        rlc_enh_dl_pdu_data_handler_hs(rlc_ul_table_ptr, rlc_dl_table_ptr, tsn_info_buf_ptr, ul_table_ptr);
      }
      num_tsn--;
    }
  }
  L2_MUTEX_UNLOCK(&(rlc_mutex_ptr->rlc_tsn_mutex));
...
{
  rlci_downlink_table_type *rlc_dl_table_ptr = RLCI_GET_DL_TBL_PTR(as_id);
  return((rlc_enh_data_ind_type *)q_get(&rlc_dl_table_ptr->rlc_dl_data_q.rlc_enh_free_data_q));
}
...
{
  rlci_downlink_table_type *rlc_dl_table_ptr = RLCI_GET_DL_TBL_PTR(as_id);
  q_put(&rlc_dl_table_ptr->rlc_dl_data_q.rlc_enh_free_data_q, &(rlc_enh_data_ind_ptr->link));
}
...
void rlc_enh_dl_pdu_data_ind(sys_modem_as_id_e_type as_id, rlc_enh_data_ind_type *rlc_enh_data_ptr)
{
  ...
  rlc_enh_handle_tm_data(rlc_ul_table_ptr, rlc_dl_table_ptr, rlc_enh_data_ptr);
  
  if (rlc_enh_data_ptr->num_complete_pdus > 0)
  {
    /* Enqueue the RLC entry into rlc_enh_data_queue */
    q_put(&(rlc_dl_table_ptr->rlc_dl_data_q.rlc_enh_data_q), &(rlc_enh_data_ptr->link));
    
    /* Post a signal to RLC DL Task to process in DL Task context */
    (void) rex_set_sigs(wcdma_l2_dl_get_tcb(as_id), RLC_DL_DATA_IND_SIG);  
  }
  else
  {
...
void rlc_enh_dl_tsn_flush_hs(sys_modem_as_id_e_type as_id)
{
  ...
  L2_MUTEX_LOCK(&(rlc_mutexes[rlc_dl_table_ptr->as_id].rlc_tsn_mutex));
  
  while ((tsn_info_buf_ptr = (l1_dec_hs_tsn_info_type *)q_get(&rlc_dl_table_ptr->rlc_dl_data_q.rlc_tsn_info_q)) 
             != NULL)
  {
    /*Discard the DSM item and enqueue in free q*/
    mac_hs_dl_discard_tsn(as_id, tsn_info_buf_ptr);
    mac_hs_enqueue_free_tsn(as_id, tsn_info_buf_ptr);
  }
  
  L2_MUTEX_UNLOCK(&(rlc_mutexes[rlc_dl_table_ptr->as_id].rlc_tsn_mutex));
...
void rlc_enh_dl_pdu_data_ind_hs(sys_modem_as_id_e_type as_id, l1_dec_hs_tsn_info_type *tsn_info_buf_ptr)
{
  ...
  {
     rlc_discard_dsm_item(rlc_dl_table_ptr->as_id, 
  ...
  {
    //WL2_MSG_1(HIGH, rlc_dl_table_ptr, "TSN %d with no complete pdus",tsn_info_buf_ptr->tsn);
  ...
  else
  {
    ...
    /* Enqueue the tsn entry into rlc_tsn_info_q */
    q_put(&rlc_dl_table_ptr->rlc_dl_data_q.rlc_tsn_info_q, &(tsn_info_buf_ptr->link));
  ...
  if(UE_LOGCHAN_DTCH == ctrl_blk->lc_type)
  {
    switch (ctrl_blk->lc_mode)
    {
      ...
      case UE_MODE_ACKNOWLEDGED_DATA:
        ...
        break;
      default:
        WL2_MSG_1(ERROR, rlc_dl_table_ptr, "RLC_ERR:Invalid rlc mode for rlc config event %d",
                                                    ctrl_blk->lc_mode);
        return;
    }
    WL2_MSG_3(HIGH, rlc_dl_table_ptr, "rlc config event rlc id %d direction and rlc type %d size %d",
             event_wcdma_rlc_config_data.rlc_id,
             event_wcdma_rlc_config_data.direction_and_rlc_type,
             event_wcdma_rlc_config_data.size); 
    WL2_EVENT_REPORT_PAYLOAD(rlc_dl_table_ptr,
	                     EVENT_WCDMA_RLC_CONFIG,
	                     4,
	                     &event_wcdma_rlc_config_data);
  }
}
...
===========================================================================*/
void  rlci_reset_status_timer_cmd(rlci_uplink_table_type *rlc_ul_table_ptr, rlci_downlink_table_type *rlc_dl_table_ptr)
{
  rlci_dl_ctrl_blk_type *rlc_dl_ctl_blk_ptr;
  uint16 i;
  rlc_ul_am_reset_table_type *ul_reset_table = &(rlc_ul_table_ptr->ul_reset_table);

  L2_ACQ_DL_LOCK();

  for (i=0; i<UE_MAX_AM_ENTITY; i++)
  {
    if (ul_reset_table->rlc_rst_satus_tmr_tbl[i] != INIT_CTL_BLK_ID)
    {
      rlc_dl_ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, ul_reset_table->rlc_rst_satus_tmr_tbl[i]);
      
      if(NULL == rlc_dl_ctl_blk_ptr)
      {
        WL2_MSG_1(ERROR, rlc_dl_table_ptr, "RLC-UL LC Id:%d not found",
                                                    i);
        continue;
      }
      ul_reset_table->rlc_rst_satus_tmr_tbl[i] = INIT_CTL_BLK_ID;
...
)
{
  ...
  if ( (sdu_len > 0 ) && (sdu_len <= RLC_MAX_SDU_SIZE))
  {
    ...
    ctrl_blk_ptr->n_sdu++;
    if(rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_var & RLC_DBG_DL_WM_CNT)
    {
      WL2_MSG_2(HIGH, rlc_ul_table_ptr, "RLC_WM_CNT: Wm_Count - %d, number of SDUs - %d",
  ...
  {
    WL2_MSG_3(ERROR, rlc_ul_table_ptr, "RLC_ERR: LCID %d Discard SDU due to invalid length %d, sdu_ptr 0x%x",
...
{
  dsm_item_type *dl_sdu_ptr;
  
  WL2_MSG_2(ERROR,rlc_ul_table_ptr,"RLC_WM: Discard pending SDU's in RLC-ID %d WM q_cnt %d",rlc_id,
                                                             q_cnt(ctrl_blk_ptr->dl_wm_ptr->q_ptr));
  dl_sdu_ptr = dsm_dequeue(ctrl_blk_ptr->dl_wm_ptr);

  while (dl_sdu_ptr != NULL)
  {
    dsm_free_packet(&dl_sdu_ptr);
    dl_sdu_ptr = dsm_dequeue(ctrl_blk_ptr->dl_wm_ptr);
  }
}
...
)
{
  ...
  L2_MUTEX_LOCK(&rlc_mutexes[rlc_dl_table_ptr->as_id].rlc_channel_release_mutex);
  if ((ctl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, lc_id)) == NULL)
  {
    WL2_MSG_1(ERROR, rlc_dl_table_ptr, "UL LC ID %d not present, ctrl_blk_ptr is NULL",
                                                lc_id);
  }
  ...
  if(ctl_blk_ptr != NULL)
  {
	...
	(FALSE == rlc_ul_table_ptr->rlc_config_pending_var.rlc_am_config_pending) &&
	(NULL != ctl_blk_ptr->ctrl.am_ctrl.dl_reseq_q) &&
	(NULL != ctl_blk_ptr->ctrl.am_ctrl.dl_reseq_q->pdu_info))     
    {
      ...
      { 
        WL2_MSG_3(HIGH, rlc_dl_table_ptr, "pending msg on AM DL old_vr_r|vr_r 0x%x, vr_h 0x%x, partial_sdu_ptr 0x%x",
       ...
       (NULL != rlc_dl_table_ptr->dl_um_pdu_info.dsm_ptr))
    {
      WL2_MSG_2(HIGH, rlc_dl_table_ptr, "pending msg on UM DL dl_um_pdu_info.dsm_ptr 0x%x, lc_id %d",
                                                 rlc_dl_table_ptr->dl_um_pdu_info.dsm_ptr,
                                                 lc_id);
      ret_val = TRUE;
    }
    else
    {
      WL2_MSG_3(HIGH, rlc_dl_table_ptr, "LC id - %d, LC mode - %d, Config_Pending - %d",
...
boolean rlc_check_pending_dl_active_rb_data(sys_modem_as_id_e_type as_id)
{
  ...
  uint8 active_rb = rlc_dl_table_ptr->active_ch;
  uint8 lc_idx = 0;
  rlci_dl_ctrl_blk_type *ctrl_blk_ptr = NULL;
  
  
  while ((active_rb > 0) && (lc_idx < UE_MAX_DL_LOGICAL_CHANNEL))
  {
    if ((ctrl_blk_ptr = rlci_get_dl_ctl_blk_ptr_by_rlc_id(rlc_dl_table_ptr, lc_idx)) != NULL)
    {
      active_rb--;
      if ((ctrl_blk_ptr->lc_mode == UE_MODE_ACKNOWLEDGED) ||
          (ctrl_blk_ptr->lc_mode == UE_MODE_UNACKNOWLEDGED))
      {
        ret_val = rlc_check_pending_dl_sdu(as_id, (rlc_lc_id_type)lc_idx);
      }
      if (TRUE == ret_val)
      {
        break;
