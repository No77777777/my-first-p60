...
$Header: //components/rel/wcdma.mpss/8.4.0/rlc/src/rlcdltm.c#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
===========================================================================*/
void rlci_enh_dl_reassemble_tm_pdus
(
...
)
{
  ...
  {
    WL2_MSG_0(ERROR, rlc_ul_table_ptr, "RLC_ERR: WM Not registered - Dropping the PDU");
  ...
  else if((RLC_RRC_CS_VOICE == ctrl_blk_ptr->ctrl.tm_ctrl.rlc_dl_tm_rab) &&
          (q_cnt(ctrl_blk_ptr->dl_wm_ptr->q_ptr) > 0))
  {
    rlci_dl_empty_sdus_in_wm(rlc_ul_table_ptr,ctrl_blk_ptr,complete_pdu_ptr->rlc_id);
  }
  #ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
  rlc_qsh_metric_global[rlc_ul_table_ptr->as_id].rlc_dl_tput += iovec_ptr->length;
  #endif  
  /*-------------------------------------------------------------------------
  ...
  if (iovec_ptr->ptr != NULL)
  {
    ...
    iovec_ptr->ptr->app_field = iovec_ptr->length;
#endif
    if (rlc_ul_table_ptr->rlc_qxdm_globals.rlc_debug_var & RLC_DBG_DL_TM)
    {
      WL2_MSG_4(HIGH, rlc_ul_table_ptr,
              "TM DL PDU Payload: 0x%x  0x%x  0x%x pdu len %d",
               ...
               rlc_copy_to_word(iovec_ptr->ptr->data_ptr+8), iovec_ptr->ptr->app_field );
    }
  ...
  {
    WL2_MSG_1(ERROR, rlc_ul_table_ptr, "RLC_ERR: NULL TM PDU, LC %d",
