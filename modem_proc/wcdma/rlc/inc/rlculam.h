...
$Header: //components/rel/wcdma.mpss/8.4.0/rlc/inc/rlculam.h#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $

when      who     what, where, why
--------  ---     ---------------------------------------------------------- 
04/22/16  scv     Migrate RLC code to new requirements for DSM Priority Field
11/05/15  scv     Fix to prioritize TCP_ACKs over new SDUs previously dequeued from WM
09/01/15  as      Added code changes to transmit the data in the order in which 
                  it is de-queued from RLC WM instead giving priority to TCP ACK's.
08/04/15  scv     Prevent taking DACC Mutex after EUL ISR Mutex in Task context
07/20/15  scv     Changes for DSM Multiple DeQ/EnQ
06/26/15  aa      Removing extern declarations from L2 code
05/29/15  kc      RLC to consider status_rpt_cnt for building CTRL PDU
...
#define RLC_ENH_RETX_PDU_INFO_DUMP(num_pdu_built, rlc_flex_retx_pdu_info_ptr, temp_pdu_info_index, rlc_ul_table_ptr) \
  do \
  { \
...
#define RLC_UL_CHECK_RETX_DUE_TO_POLL(rlc_dl_table_ptr, ctrl_blk, status_ptr) \
            (!(ctrl_blk->lc_state & UL_RESET_PENDING) && \
             /*lint +e655 */                             \
              !(rlci_chk_dl_reset_pending(rlc_dl_table_ptr, ctrl_blk->ctrl.am_ctrl.am_common->dl_data_id)) && \
...
FUNCTION RLC_ULC_GLOBALS_INIT

DESCRIPTION
  Initialize the ULC globals
...
===========================================================================*/
void rlc_ulc_globals_init(rlc_ulc_params_type *rlc_ulc_params_ptr);
rlc_ctrl_pdu_type* rlc_ul_free_ul_comp_ctrl_dsm_pool(rlci_uplink_table_type *rlc_ul_table_ptr, rlc_lc_id_type   rlc_id);

#define RLC_FREE_SDU_DSM(rlc_ul_table_ptr, sdu_ptr,am_ctrl_ptr)\
do\
{\
  if (RLC_GET_UL_COMP_CTRL_SDU_PRESENT(sdu_ptr->app_field))\
  {\
    rlc_ul_free_ul_comp_ctrl_dsm_pool(rlc_ul_table_ptr, am_ctrl_ptr->am_common->ul_data_id);\
  }\
  else\
  {\
...
#define RLC_UL_AM_SEND_SDU_CNF(rlc_ul_table_ptr, am_ctrl_ptr,sdu_ptr,status,fail_reason) \
do\
{\
  ...
  {\
    ...
    {\
      am_ctrl_ptr->sdu_cnf_cb(rlc_ul_table_ptr->as_id, am_ctrl_ptr->am_common->ul_data_id, status, \
                            RLC_UL_AM_GET_SDU_MUI(sdu_ptr->app_field), fail_reason);\
    }\
    RLC_FREE_SDU_DSM(rlc_ul_table_ptr, sdu_ptr,am_ctrl_ptr);\
  }\
}while(0);
...
**=========================================================================*/
 ...
 ===========================================================================*/
 void rlc_enh_initialize_mutex(sys_modem_as_id_e_type as_id);
...
===========================================================================*/
void rlci_ul_am_init(rlci_uplink_table_type *rlc_ul_table_ptr);
/*===========================================================================
...
===========================================================================*/
void rlc_ul_am_update_li_len(rlci_uplink_table_type *rlc_ul_table_ptr, rlci_ul_am_ctrl_type *am_ctl_blk_ptr , uint8 old_li_len);
...
=============================================================================*/
void rlc_enh_update_retx_bytes(rlci_uplink_table_type *rlc_ul_table_ptr,
                               rlci_downlink_table_type *rlc_dl_table_ptr,
                               rlci_ul_ctrl_blk_type *ctrl_blk, 
                               rlc_ul_lc_buf_status_type *status_ptr,
                               uint16     max_sn,
...
=============================================================================*/
void rlc_enh_update_retx_bytes_due_to_polling
(
  rlci_uplink_table_type *rlc_ul_table_ptr,
  rlci_ul_ctrl_blk_type *ctrl_blk, 
  rlc_ul_lc_buf_status_type *status_ptr,
...
===========================================================================*/
void rlc_ul_enh_print_rlc_hdr(rlci_uplink_table_type *rlc_ul_table_ptr, rlc_ul_hdr_buff_type    *ul_hdr_ptr);
#ifdef FEATURE_MAC_I
/*===========================================================================
...
===========================================================================*/
void rlc_enh_build_partial_pdu_end(rlci_uplink_table_type *rlc_ul_table_ptr,
                                   rlci_ul_ctrl_blk_type       *ctrl_blk,  
                                   rlc_ul_logchan_format_type  *chan_info, 
...
===========================================================================*/
void rlc_enh_build_partial_pdu_middle(rlci_uplink_table_type *rlc_ul_table_ptr, 
                                      rlci_ul_ctrl_blk_type 	  *ctrl_blk,  
                                      rlc_ul_logchan_format_type  *chan_info, 
                                      l1_l2_data_type             *l1_ul_tx_buf_ptr,
                                      uint16                      src_desc_idx);
...
===========================================================================*/
void rlc_ul_enh_build_poll_sufi(rlci_uplink_table_type *rlc_ul_table_ptr, 
                                rlci_downlink_table_type *rlc_dl_table_ptr,
...
(
  rlci_ul_statistics_var_type *ul_stats_ptr
);
...
===========================================================================*/
rlc_ctrl_pdu_type *rlc_ul_enh_am_allocate_ctrl_dsm (rlci_uplink_table_type *rlc_ul_table_ptr, uint16 pdu_size);
...
===========================================================================*/
rlc_ctrl_pdu_type* rlc_ul_get_ctrl_dsm_pool(rlci_uplink_table_type *rlc_ul_table_ptr, rlc_am_ctrl_dsm_type *ctrl_pdu_dsm_pool_ptr);
...
===========================================================================*/
static inline void rlc_ul_am_update_special_li_field(rlci_uplink_table_type  *rlc_ul_table_ptr,
                                                     rlci_ul_ctrl_blk_type   *ctrl_blk_ptr,
                                                     uint8                    li_len,
                                                     uint16                   rem_pdu_size,
...
dsm_item_type *rlc_ul_am_dsm_dequeue(rlci_uplink_table_type *rlc_ul_table_ptr, rlci_ul_ctrl_blk_type *ctl_blk_ptr);
...
#ifdef FEATURE_WCDMA_UL_COMPR
void rlc_ul_am_comp_dsm_alloc(rlci_uplink_table_type *rlc_ul_table_ptr);
