...
$Header: //components/rel/wcdma.mpss/8.4.0/rlc/inc/rlcul.h#3 $ $DateTime: 2021/02/10 03:14:23 $ $Author: pwbldsvc $


when      who     what, where, why
--------  ---     ---------------------------------------------------------- 
02/09/21   sp     Compiler upgradation changes ATHERTON 
10/15/18   sg     Made changes to ignore PS reset fail when CS call is up
01/19/17  nk      Typecasting to fix error of PDU Size not matching on LCID 19
...
09/21/15  as      Added code changes to enable RACH on getting L2 ACK and there is pending data to be transmitted.
...
07/29/15  scv     Revert to dsm_length_packet() API instead of dsm_item_get_pkt_len() 
...
06/23/15  as      Fix to log LI value correctly in 0x41CC & 0x41CB log packets.
...
05/29/15  kc      Send status PDU for 3 TTIs to optimize DCH to FACH transition
...
05/18/15  kc      Adding new DACC interface changes
...
#ifdef FEATURE_WCDMA_RLC_ZI_REDUCTION
#include "modem_mem.h"
#endif /*FEATURE_WCDMA_RLC_ZI_REDUCTION*/
...
#define NUM_MRW_BUF 128

#define RLCI_GET_UL_TBL_PTR(x) (&uplink_table[x])

#define MIN_RLC_UNREC_TIMER_VALUE       4000 /* Min Timer value to trig RLC_Unrec_Error */

#ifdef FEATURE_WCDMA_UL_COMPR
#define MAX_UL_COMP_CTRL_DSM_ITEMS       3
#define RLCI_PTP_SUFI_LENGTH            250
#endif /*FEATURE_WCDMA_UL_COMPR*/
...
#define RLC_UL_AM_IGNORE_MAXRST(ctrl_blk) ((!mcfg_gcf_nv_get_status()) && \
    (ctrl_blk->lc_type == UE_LOGCHAN_DTCH) && (rrc_query_mrab_status(rlc_ul_table_ptr->as_id) == TRUE) )

#ifdef FEATURE_WCDMA_LAYER2_CONFIG_ENH
#define RLC_LC_TYPE_VALID(lc_type) ((lc_type > UE_LOGCHAN_NONE) && (lc_type < UE_LOGCHAN_MAX_NUM))
#endif
...
typedef struct{
  ...
  uint8  num_of_li;                         /* Number of LIs */
  uint8  padded_li;                         /* Size of Padded LI */  
  uint8  r99_frame_num;                     /* log the FN corresponding to Tx in R99 path*/
  uint16 eul_frame_num;                     /* log the FN corresponding to Tx in EUL Path*/  
...
/*--------------------------------------------------------------------------- 
  Structure contains the timer stats to trigger RRC to post cell update due
  to RLC Unrecoverable error
---------------------------------------------------------------------------*/
typedef struct
{ 
  boolean   poll_sent;              /* Set to True when poll is sent */
  
  uint32    curr_tmr_count;	    /* This indicates the amount of 
     				       ps rab timer expired	     */											 	
  uint32    max_tmr_val_cnf;        /* This contains the ps rab tmr
                                       configured valued             */ 
}rlc_frc_trig_unreco_tmr;
...
typedef struct {
  ...
  void (*sdu_cnf_cb) (sys_modem_as_id_e_type as_id,
                      rlc_lc_id_type ul_data_lc_id, 
...
rlc_frc_trig_unreco_tmr     force_unreco_tmr_stats; /* Contains timer stats which is used to trigger
                                                       RLC un-recoverable error */
#ifdef FEATURE_RLC_QXDM_LOGGING
...
typedef struct{
  rlc_lc_id_type am_ul_reset_table[UE_MAX_AM_ENTITY];
  rlc_lc_id_type am_ul_reset_done_table[UE_MAX_AM_ENTITY];
  rlc_lc_id_type am_proc_ul_reset_table[UE_MAX_AM_ENTITY];
...
}rlc_ul_am_reset_table_type;

typedef struct{
  uint32 rlc_debug_var;
  uint32 num_max_dsm_item_per_tti;
  uint32 num_max_dsm_item_per_pdu;
  boolean buf_status_log;
  boolean rlc_qxdm_corrupt_ack;
  boolean rlc_debug_pdu_dump;
  boolean rlc_debug_print_sdu;
  
  #ifdef FEATURE_MAC_I
  boolean rlc_am_poll_sufi_support;
  boolean mac_i_flex_partial_support;
  boolean maci_rlc_sp_he_f3_enable;
  boolean maci_rlc_enh_build_pdu_dump;
  #endif
  
  #ifdef FEATURE_WCDMA_UL_COMPR
  uint8 RLC_ENABLE_UL_COMP_QXDM;
  rlci_ptp_sufi_type rlc_qxdm_ptp_response;
  rlci_ptp_sufi_rsp_reason_type rlc_qxdm_rsp_reason;
  boolean rlc_ulc_logging_debug_var;
  boolean rlc_ptp_debug_var;
  #endif
}rlci_qxdm_globals_type;
...
typedef struct {
  rlci_ul_am_sdu_tx_cnf_type am_ul_sdu_tx_cnf[UE_MAX_AM_ENTITY];
  boolean delay_sdu_tx_cnf;            /* If TRUE-delay the SDU confirm */
                                       /* callback till RLC moves from  */
                                       /* Interrupt to Task context     */
}rlc_sdu_tx_delay_params_type;
...
typedef struct{
  /*-------------------------------------------------------------------------
      ...
      -------------------------------------------------------------------------*/   
  rlc_um_sdu_q_type rlc_free_sdu_q_non_edch;
  rlc_um_sdu_q_type rlc_free_sdu_q_edch;
	
  /*-------------------------------------------------------------------------
        Buffers to hold the free SDU pointers. Current design supports 200 buffers
        for EDCH channel & 100 Buffers for all Non-EDCH Channels.
       -------------------------------------------------------------------------*/
  free_sdu_buffer_type free_sdu_buffer_edch [MAX_SDU_PER_TTI_EDCH];
  free_sdu_buffer_type free_sdu_buffer_non_edch [MAX_SDU_PER_TTI_NON_EDCH];

}rlc_sdu_buffer_params_type;

typedef struct{
  rlci_mrwq_type rlc_mrw_pool[NUM_MRW_BUF];
  q_type rlc_free_mrw_q;
}rlci_mrw_params_type;
...
typedef struct{
  ...
  uint16  eul_frame_num;
  boolean rlci_ul_pdu_cipher_log_on;  
  rlc_ul_pdu_cipher_info_type rlci_ul_pdu_cipher_log;
  rlc_ul_pdu_cipher_info_type rlci_eul_pdu_cipher_log;
  rlc_ul_log_state_type ul_am_log_state;
  rex_timer_type ul_tmr_log_blk;         /* Timer for qxdm logging purposes */
  #ifdef FEATURE_UM_PDU_LOGGING
  uint16 rlc_ul_channel_cnt;
  uint16 rlc_eul_channel_cnt;
  #endif
}rlc_ul_logging_var_type;
#endif

#ifdef FEATURE_WCDMA_UL_COMPR
typedef struct 
{
  rlc_ctrl_pdu_type ctrl_pdu[MAX_UL_COMP_CTRL_DSM_ITEMS];
  uint8 num_item;   /* No. of DSM items used. */
}rlc_am_ul_comp_dsm_type;

typedef struct{
  uint32 MAX_STMR_TICKS_PER_COMP_USAGE;
  uint32 RLC_NUM_BYTES_COMP_ADVANCE; 
  uint32 RLC_NUM_BYTES_COMP_ADV_IN_ONE_GO;
  uint32 RLC_NUM_BYTES_UNCOMP_IN_ONE_GO;
  uint32 RLC_NUM_DSM_UNCOMP_IN_ONE_GO;
  uint32 comp_bytes_bo_threshold;
  uint32 uncomp_bytes_bo_threshold;
  uint32 num_tti_uncomp_report_zero_bo;
  uint32 num_tti_zero_bo_reported;
  uint32 overwrite_threshold_qxdm;
	
  dacc_pkt_comp_mode_type UL_COMP_MODE;
  dacc_compress_e_type UL_COMP_HDR_ONLY;
	
  ul_comp_perf_metrics_type ul_comp_per_metrics_info;

  uint8 CONFIG_MEM_SIZE;
  uint8 CONFIG_HDR_LENGTH;
  uint8 CONFIG_GAIN_THRESH; 
  boolean CONFIG_PREFILL;
  boolean rlc_ptp_ul_comp_event_enabled;
  boolean rlc_ptp_ul_comp_resource_crunch;

  uint8 ptp_msg_rcvd[RLCI_PTP_SUFI_LENGTH];
  uint8 rlc_dl_ptp_response[RLCI_PTP_SUFI_LENGTH];

  uint8 debug_temp_mult;
...
typedef struct{
  uint8 no_mrw_sent;
  uint8 no_mrw_ack_sent;
  uint8 no_reset_ack_sent;
  uint8 no_reset_sent;
}rlci_ul_statistics_var_type;

typedef struct{
  // Flag indicating if RLC config is in progress
  boolean rlc_config_pending;
  boolean rlc_am_config_pending;
  rlc_data_activity_type rlc_activity_flag;
}rlc_config_pending_var_type;

#ifdef FEATURE_QXDM_DSM_TRACING
typedef struct{
  fc_dsm_event_hits_stat_type rlc_fc_dsm_event_hits[FC_MAX_POOL_ID];
  uint32 num_rlc_non_srb_entities_up;
}rlc_dsm_trace_params_type;
#endif
...
typedef struct {
  ...
  wm_cb_type data_ind_in_fach_func_ptr; /* CB to indicate RRC of data presence
                                                                           in FACH state*/ 
  rex_timer_type rlc_ul_am_base_tmr;    /* This is base 10ms timer for RLC AM channels
                                                                           at expiry, periodic status/control related
                                                                           procedures are triggered*/
  boolean rlc_ul_am_base_tmr_start;
  
  boolean is_tuneaway;                  /* True if either LTA/ QTA is in progress */																		   
  
  /* Table to store the reset messages for during the RESET */
  rlc_ul_am_reset_table_type ul_reset_table;
  rlc_sdu_tx_delay_params_type sdu_tx_delay_params;
  
  rlc_sdu_buffer_params_type rlc_sdu_q;
  boolean compressed_data;
  boolean rlc_ul_data_avail_for_tx;
  rlci_mrw_params_type rlc_mrw_params;
  rlci_am_txq_type rlci_am_txq_arr[UE_MAX_UL_AM_CHANNEL];
  rlc_am_ctrl_dsm_type ctrl_pdu_dsm_pool;
  rlci_ul_statistics_var_type ul_stats_var;
  boolean piggyback_status_pdu;

  uint32 num_dsm_used_in_this_tti;
  uint16 rlc_acti_non_srb_am_ch;
  ...
  uint8  ul_ctl_blk_id_tbl[UE_MAX_UL_LOGICAL_CHANNEL];        /* rlc_id to ul control block index table */  
  rlc_config_pending_var_type rlc_config_pending_var;
  rlc_ul_pdu_build_params_type ul_pdu_build_params;
  rlci_qxdm_globals_type rlc_qxdm_globals;
  rlc_profile_var_type rlc_profile_var;
  ...
  #ifdef FEATURE_MAC_I
  boolean maci_rlc_enh_build_pdu_dump;
  rlc_am_mac_i_dsm_type  mac_i_ctrl_pdu_dsm_pool;
  rlc_mac_i_ctrl_dsm_free_type  rlc_free_mac_i_ctrl_q;
  rlc_flex_retx_pdu_info_type *rlc_flex_retx_pdu_info[UE_MAX_USER_PLAN_AM_ENTITIES];
  #endif
  ...
  #ifdef FEATURE_RLC_QXDM_LOGGING
  rlc_ul_logging_var_type rlc_ul_logging;
  #endif

  #ifdef FEATURE_WCDMA_UL_COMPR  
  rlc_am_ul_comp_dsm_type  ul_compr_ctrl_pdu_dsm_pool;
  rlc_ulc_params_type rlc_ulc_params;
  #ifdef FEATURE_RLC_QXDM_LOGGING
  rlc_ul_compression_log_info_type rlci_comp_log_info;
  #endif
  #endif
  
  #ifdef FEATURE_QXDM_DSM_TRACING
  rlc_dsm_trace_params_type rlc_dsm_trace_params;
  #endif  
...
#endif
  #ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
  rlc_qsh_reset_info rlc_reset_info;
  #endif
} rlci_uplink_table_type;

#ifdef FEATURE_WCDMA_QSH_EVENT_METRIC
/* Structure to store the QSH Metrics */
typedef struct
{
  uint16 num_rlc_rst_dl;     /* Indicates number of RESET PDU received in DL */
  uint16 num_rlc_rst_ul;     /* Indicates number of RESET PDU sent in UL */
  uint16 rlc_fc_win_size;    /* Indicates avg window size across Uplink Data RBs */
  uint16 num_rlc_urecov_err;
  uint32 num_pdu_bytes_tx;
  uint32 num_pdu_bytes_retx;
  uint32 rlc_dl_tput;        /* Indicates DL PDU throughput in kbps */
  uint32 rlc_ul_tput;        /* Indicates UL PDU throughput in kbps */
  void *metric_context_id;
  void *metric_addr;
}rlc_qsh_metric_data_type;

extern rlc_qsh_metric_data_type rlc_qsh_metric_global[]; 
#endif
...
===========================================================================*/
void rlci_ul_am_base_timer_handler (rlci_uplink_table_type *rlc_ul_table_ptr,
...
FUNCTION RLC_UL_REGISTER_SRVC_HANDLER_SUB
...
FUNCTION RLC_UL_DEREGISTER_SRVC_HANDLER_SUB
...
===========================================================================*/
void crlc_release_all_handler(rlci_uplink_table_type *rlc_ul_table_ptr, rlci_downlink_table_type *rlc_dl_table_ptr);
...
(
  sys_modem_as_id_e_type as_id,
  rlc_lc_id_type *reset_done_table_ptr,
  rlc_lc_id_type  rlc_id
);
...
===========================================================================*/
void rlci_post_sdu_tx_cnf(rlci_uplink_table_type *rlc_ul_table_ptr);
...
===========================================================================*/
dsm_item_type* rlc_ul_get_pre_alloc_pdu_dsm(  rlci_uplink_table_type *rlc_ul_table_ptr,uint16 pdu_len_bytes,uint16 *pdu_offset);
...
===========================================================================*/
void rlc_ul_init_ctrl_dsm_pool(rlci_uplink_table_type *rlc_ul_table_ptr);
...
boolean rlc_enh_ul_pdu_pending (rlci_uplink_table_type *rlc_ul_table_ptr, rlci_ul_ctrl_blk_type    *ul_ctrl_blk_ptr);
...
===========================================================================*/
void rlci_update_retxq_sema(
rlci_uplink_table_type *rlc_ul_table_ptr,
rlc_lc_id_type rlc_id, boolean status);
...
(
  ...
  uint32 retx_size       /* PDU Sequencial number                */
);
...
===========================================================================*/
void rlci_generate_dl_status(rlci_uplink_table_type *rlc_ul_table_ptr,
                               rlci_downlink_table_type *rlc_dl_table_ptr,
                                              rlc_lc_id_type rlc_id);
...
===========================================================================*/
void rlci_empty_dl_control_q(rlci_uplink_table_type *rlc_ul_table_ptr,
                                rlci_downlink_table_type *rlc_dl_table_ptr, uint8 lc_id);
...
===========================================================================*/
void rlci_reset_am_retx_q(rlci_uplink_table_type *rlc_ul_table_ptr, rlc_lc_id_type rlc_id);
...
#endif /*FEATURE_WCDMA_IGNORE_DATA_RB_DURING_CALL_REL*/
...
===========================================================================*/
//void rlc_ul_register_trigger_based_rach_api_active_rbs();
...
===========================================================================*/
void rlc_enh_hs_rach_check_rach_trigger(rlci_uplink_table_type *rlc_ul_table_ptr, uint8 rlc_id);
...
SIDE EFFECTS:
  None
===========================================================================*/ 
void rlc_ul_update_mac_i_ctrl_dsm_item (rlci_uplink_table_type *rlc_ul_table_ptr, 
                                             rlc_ctrl_pdu_type *   mac_i_ctrl_dsm);
...
===========================================================================*/
rlc_ctrl_pdu_type* rlc_ul_get_mac_i_rlc_id_ctrl_dsm_ptr(
rlci_uplink_table_type *rlc_ul_table_ptr,
rlc_am_mac_i_dsm_type *ctrl_pdu_dsm_pool_ptr,
rlc_lc_id_type rlc_id
);
/*===========================================================================
...
===========================================================================*/
rlc_ctrl_pdu_type* rlc_ul_alloc_mac_i_ctrl_dsm_pool(rlci_uplink_table_type *rlc_ul_table_ptr, rlc_am_mac_i_dsm_type *ctrl_pdu_dsm_pool_ptr);
...
===========================================================================*/
void rlc_ul_enh_print_src_desc(rlci_uplink_table_type *rlc_ul_table_ptr, l1_l2_data_type  *l1_ul_tx_buf_ptr);
...
===========================================================================*/
void rlc_ul_enh_store_mac_i_ctrl_pdu(rlci_uplink_table_type *rlc_ul_table_ptr,
mac_i_seg_status mac_segment_needed, rlci_ul_ctrl_blk_type *ctrl_blk_ptr, uint16 pdu_size_bytes, uint8 *pdu_ptr);
...
void rlc_ul_am_re_enq_sdu_temp_q(rlci_uplink_table_type  *rlc_ul_table_ptr, 
                                 rlci_ul_ctrl_blk_type   *ctrl_blk_ptr,
...
===========================================================================*/
void rlc_enh_build_partial_pdu_start(rlci_uplink_table_type *rlc_ul_table_ptr,
...
FUNCTION RLC_ENH_BUILD_PARTIAL_MIDDLE_END
...
===========================================================================*/
boolean  rlc_enh_build_partial_middle_end(rlci_uplink_table_type *rlc_ul_table_ptr,
                                         rlc_ul_logchan_format_type  *chan_info,
                                         rlci_ul_ctrl_blk_type       *ctrl_blk,
                                         l1_l2_data_type            *l1_ul_tx_buf_ptr
                                         #ifdef FEATURE_RLC_QXDM_LOGGING
...
FUNCTION RLC_UL_AM_ENQ_ORIG_SDU_TO_WM
...
===========================================================================*/
void rlc_ul_am_requeue_dequeued_sdus_to_wm (rlci_ul_ctrl_blk_type *ul_ctrl_blk_ptr, rlci_uplink_table_type *rlc_ul_table_ptr );
void rlc_ul_am_enq_orig_sdu_to_wm(rlci_ul_ctrl_blk_type *ctl_blk_ptr, rlci_uplink_table_type *rlc_ul_table_ptr, boolean IsRelease);

/*===========================================================================
FUNCTION: RLC_UL_ALLOC_UL_COMP_CTRL_DSM_POOL

DESCRIPTION:
  Get one DSM item from the UL Compr control PDU DSM pool.
===========================================================================*/ 
rlc_ctrl_pdu_type* rlc_ul_alloc_ul_comp_ctrl_dsm_pool(rlci_uplink_table_type *rlc_ul_table_ptr);

void rlc_ul_am_comp_init_var(rlci_uplink_table_type *rlc_ul_table_ptr, rlci_ul_am_ctrl_type  *am_ctl_blk_ptr);

rlc_ctrl_pdu_type *rlc_dl_ptp_func_mgmt_response(rlci_uplink_table_type *rlc_ul_table_ptr,
                                                 rlci_ptp_sufi_type      ret_response,
                                                 rlci_ptp_sufi_rsp_reason_type  ul_comp_rsp_reason);
void rlc_dl_ul_comp_config_current_dump(rlci_uplink_table_type *rlc_ul_table_ptr, rlci_ul_am_ctrl_type  *ul_am_ctrl_ptr);

boolean rlc_config_deconfig_ul_compressor(rlci_uplink_table_type *rlc_ul_table_ptr,
	                                  rlci_ul_am_ctrl_type  *am_ctrl_blk_ptr, boolean compression_enable, boolean IsRelease);

void rlc_ul_comp_disable_config_internal(rlci_uplink_table_type *rlc_ul_table_ptr, 
                                          rlci_ul_ctrl_blk_type *ctl_blk_ptr, boolean IsRelease);

boolean rlc_ul_am_comp_resp_pending(rlci_uplink_table_type *rlc_ul_table_ptr, rlc_lc_id_type ul_data_id);
#endif /*FEATURE_WCDMA_UL_COMPR*/
...
void rlci_ul_am_handle_qxdm_log_timer(rlci_uplink_table_type *rlc_ul_table_ptr, uint32 timer_sig);
...
===========================================================================*/
void rlci_process_comp_log_pkt(rlci_uplink_table_type *rlc_ul_table_ptr);

void rlc_dl_ul_comp_config_tx_tl_event(boolean , boolean *, rlci_uplink_table_type *rlc_ul_table_ptr);
#endif /*FEATURE_WCDMA_UL_COMPR*/
...
/* ================================================================================
  Definitions & Structures to calculate the average & current RTT  for AM Channels 
 ================================================================================= */
 
#define MAX_NUM_RTT_ENTRIES_PER_LC    5
#define MAX_NUM_AM_CHANNELS_RTT       UE_MAX_AM_ENTITY
#define RLC_GET_RTT_TBL_INDEX(rtt_delay_global_ptr, lc_id) \
        (rtt_delay_global_ptr->ul_tbl_rtt_index[lc_id - RLC_UL_AM_START_LC_ID ])
#define RLC_SET_RTT_TBL_INDEX(rtt_delay_global_ptr, lc_id, rtt_tbl_idx) \
        (rtt_delay_global_ptr->ul_tbl_rtt_index[lc_id - RLC_UL_AM_START_LC_ID ] = rtt_tbl_idx)
#define RLC_GET_CURRENT_TIMESTAMP     PS_GET_TIMESTAMP_3_4_MS
#define RLC_GET_DSM_TIMESTAMP         PS_DSM_GET_TIMESTAMP
#define RLC_SET_DSM_TIMESTAMP         PS_DSM_SET_TIMESTAMP

typedef struct
{
  uint16 seq_num;         /* sequence number of the PDU */
  uint32 time_of_tx;      /* records the timestamp at which the PDU was originally transmitted */
} rlc_am_rtt_type;

typedef struct
{
  boolean in_use;
  rlc_am_rtt_type rtt_entries [ MAX_NUM_RTT_ENTRIES_PER_LC ];
  uint8 num_curr_entires_for_lc;
} rlc_am_rtt_entries_per_lc;
...
typedef struct
{
  ...
  uint32 avg_delay;
  uint16 num_samples;
} rlc_delay_metric_type;

typedef struct
{
  uint8 ul_tbl_rtt_index [UE_MAX_AM_ENTITY];           /* maps an AM RLC ID to an index in rlc_am_rtt_table[] */ 
  rlc_delay_metric_type rtt_metrics[UE_MAX_AM_ENTITY];   /* At max, there are 16 possible AM channels */
  rlc_delay_metric_type qdelay_metrics[UE_MAX_AM_ENTITY];
  
  rlc_am_rtt_entries_per_lc rlc_am_rtt_table [MAX_NUM_AM_CHANNELS_RTT];
  uint8 num_active_rtt_channels;
  boolean poll_true_for_this_tti;
} rtt_delay_global_type;

extern rtt_delay_global_type rtt_delay_global[L2_NUM_W_SUBS];

#define RLC_GET_RTT_GLOBAL_PTR(as_id)   (&(rtt_delay_global[as_id]))

/* returns the current timestamp in ms */
uint32 rlc_get_current_timestamp_ms (void);

/* sets the field passed in as parameter to the current timestamp */
void rlc_set_current_timestamp_ms ( uint32 *timestamp_field);

/* returns the difference between the two arguments MODULUS (max value possible) */
uint32 timestamp_difference_ms ( uint32 timestamp1, uint32 timestamp2 );

/* update the RTT entry for a particular logical channel and SN number */
...
DESCRIPTION
  Initializes the RTT entry table.
...
DESCRIPTION
  Validates that an RTT entry exists for the AM channel, and resets the 
  variables upon RESET/Re-EST 
...
DESCRIPTION:
  This computes computes the RTT for any new Sequence Numbers for which 
  ACK has been received, and accordingly updates the RTT traffic metrics.
===========================================================================*/
