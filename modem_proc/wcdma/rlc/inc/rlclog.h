...
$Header: //components/rel/wcdma.mpss/8.4.0/rlc/inc/rlclog.h#2 $ $DateTime: 2020/01/24 03:56:36 $ $Author: pwbldsvc $
...
#endif /* __GNUC__ */
{
  uint8  ul_rlc_id;                      /* Data Logical Channel UL RLC ID(0..18)      */
  uint8  num_of_li;                      /* Number of LIs present in this TTI          */
  uint8  padded_li;                      /* Size of padded LI(in Bytes)in current TTI  */
  uint16 num_pdu;                        /* Number of PDUs logged                      */
  uint16 pdu_size;                       /* PDU size in octets of each PDU             */
  uint16 frame_num;                      /* Frame Number at which this PDU is Tx'ed    */
...
#endif /* __GNUC__ */
{
  uint8 dl_rlc_id;                    /* Data Logical Channel DL RLC ID    */
  uint8 num_pdu;                      /* Num_PDUs logged                        */
  uint8 rlc_mode_li_length;           /* RLC-Mode - bit1; Li_length - bit 0 */
} rlc_log_flex_dl_pdu_data_type;
...
#endif /* __GNUC__ */
{
  uint8   ul_rlc_id;                  /* Data Logical Channel DL RLC ID                */
  uint8   num_pdu_li_length;          /* num_pdus logged: 6 bits, li_length: 2 bits   */
  uint8   ciph_mode_ID_algo_rlc_mode;
  uint16  act_sn;                     /* SN to start ciphering                                 */
  uint8   new_tx_bo[3];               /* New data buffer                                       */
  uint8   re_tx_bo[3];                /* Re-Tx data buffer                                    */  
  uint16  ctrl_data_bo; 			  /* Control Data to be built				      */
  uint8   num_sdus;                   /* Number of SDU's built in this TSN/RLC-id  */
  uint8   num_of_li;                  /* Number of LIs present in this TTI              */
  uint8   padded_li;                  /* Size of padded LI(in Bytes)in current TTI   */
  uint16  frame_num;                  /* Frame Number at which this PDU is Tx'ed   */
...
#endif /* __GNUC__ */ 
{
  uint8  ul_rlc_id;                      /* Data Logical Channel UL RLC ID(0..18)      */
  uint8  num_of_li;                      /* Number of LIs present in this TTI          */
  uint8  padded_li;                      /* Size of padded Bytes in current TTI        */
  uint16 num_pdu;                        /* Number of PDUs logged                      */
  uint16 pdu_size;                       /* PDU size in octets of each PDU             */
  uint16 frame_num;                      /* Frame Number at which this PDU is Tx'ed    */
}rlc_ul_um_log_header_type ;
...
/*---------------------------------------------------------------------------
                   LOG PACKET: WCDMA_UL_FIXED_AM_USR_PDU_LOG_PACKET_V1
 --------------------------------------------------------------------------*/

LOG_RECORD_DEFINE(WCDMA_UL_FIXED_AM_USR_PDU_LOG_PACKET_V1)
...
#ifdef FEATURE_UM_PDU_LOGGING
LOG_RECORD_DEFINE(WCMDA_UL_FIXED_UM_PDU_LOG_PACKET_V1)
...
LOG_RECORD_DEFINE(WCDMA_DL_FLEXIBLE_RLC_PDU_LOG_PACKET_V1)
  ...
  /*-------------------------------------------------------------------------
    For each RLC AM Uplink entity. 
  -------------------------------------------------------------------------*/
  rlc_log_flex_dl_pdu_data_type dl_flex_pdu_data[UE_MAX_AM_ENTITY];

  /*-------------------------------------------------------------------------
    For each PDU received - SIZE and DATA information. 
  -------------------------------------------------------------------------*/
  rlc_log_flex_dl_pdu_info_type pdu_info[MAX_DL_RLC_LOG_PDUS];
...
#endif
/*---------------------------------------------------------------------------
                   LOG PACKET: WCDMA_UL_RLC_AM_STATUS_PDU_LOG_PACKET
 --------------------------------------------------------------------------*/


LOG_RECORD_DEFINE(WCDMA_UL_RLC_AM_STATUS_PDU_LOG_PACKET)
    
  /*-------------------------------------------------------------------------
    NAK PDU information.
  -------------------------------------------------------------------------*/
  rlc_log_ul_am_status_pdu_type ul_am_status_pdu;
...
/*---------------------------------------------------------------------------
              LOG PACKET: WCDMA_UL_FIXED_AM_SIG_PDU_LOG_PACKET_V1
 --------------------------------------------------------------------------*/


LOG_RECORD_DEFINE(WCDMA_UL_FIXED_AM_SIG_PDU_LOG_PACKET_V1)
...
#define RLC_UL_AM_USR_PLANE_PDU_LOG_PACKET_LEN(num_of_pdu) ( \
  FPOS(WCDMA_UL_FIXED_AM_USR_PDU_LOG_PACKET_V1_type, ul_am_pdu_data)+ \
      (sizeof(rlc_ul_am_log_header_type) + (num_of_pdu * 4)) )

#ifdef FEATURE_UM_PDU_LOGGING
#define RLC_UL_UM_USR_PLANE_PDU_LOG_PACKET_LEN(num_of_pdu) ( \
  FPOS(WCMDA_UL_FIXED_UM_PDU_LOG_PACKET_V1_type, ul_um_pdu_data)+ \
      (sizeof(rlc_ul_um_log_header_type) + (num_of_pdu * 4)) )
#endif
...
#define RLC_UL_AM_SIG_PLANE_PDU_LOG_PACKET_LEN(num_of_pdu) ( \
  FPOS(WCDMA_UL_FIXED_AM_SIG_PDU_LOG_PACKET_V1_type, ul_am_pdu_data)+ \
      (sizeof(rlc_ul_am_log_header_type) + (num_of_pdu * 4)) )
...
#define RLC_DL_AM_USR_PLANE_PDU_LOG_PACKET_LEN(num_of_pdu) ( \
  FPOS(WCDMA_DL_RLC_AM_USR_PLANE_PDU_LOG_PACKET_type, dl_am_pdu_data)+ \
      (sizeof(rlc_dl_am_log_header_type) + (num_of_pdu * 4)) )
...
#define RLC_DL_UM_USR_PLANE_PDU_LOG_PACKET_LEN(num_of_pdu) ( \
  FPOS(WCDMA_DL_RLC_UM_USR_PLANE_PDU_LOG_PACKET_type, dl_um_pdu_data)+ \
      (sizeof(rlc_dl_um_log_header_type) + (num_of_pdu * 4)) )
#endif
...
/*---------------------------------------------------------------------------
  Length of DL AM User Plane PDU log packet.
---------------------------------------------------------------------------*/
#define RLC_DL_FLEXIBLE_PDU_LOG_PACKET_LEN(num_entity,num_of_pdu)( \
  FPOS(WCDMA_DL_FLEXIBLE_RLC_PDU_LOG_PACKET_V1_type, dl_flex_pdu_data)+ \
      (sizeof(rlc_log_flex_dl_pdu_data_type) * num_entity) +  \
      (sizeof(rlc_log_flex_dl_pdu_info_type) * num_of_pdu))
#endif
...
/*---------------------------------------------------------------------------
  Length of UL AM STATUS PDU log packet.
---------------------------------------------------------------------------*/
#define RLC_UL_AM_STATUS_PDU_LOG_PACKET_LEN() ( \
  FPOS(WCDMA_UL_RLC_AM_STATUS_PDU_LOG_PACKET_type, ul_am_status_pdu)+ \
      sizeof(rlc_log_ul_am_status_pdu_type) )
