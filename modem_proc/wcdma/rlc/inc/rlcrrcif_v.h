...
$Header: //components/rel/wcdma.mpss/8.4.0/rlc/inc/rlcrrcif_v.h#3 $
...
boolean rlc_rrc_set_status_pdu_tx_cnt(sys_modem_as_id_e_type as_id, rlc_lc_id_type lc_id, uint8 set_count);

void rrccu_rlc_ul_data_ind (dsm_watermark_type *wm_ptr, void *ctrl_blk);
...
FUNCTION RLC_CHECK_UL_COMP_ENABLED_ON_ALT_SUB

DESCRIPTION
  Checks if tcompression enabled on other sub
DEPENDENCIES
  None.

RETURN VALUE
  TRUE - if Copression enabled
  FALSE - otherwise 
...
boolean rlc_check_ul_comp_enabled_on_alt_sub(sys_modem_as_id_e_type as_id);
#endif 
