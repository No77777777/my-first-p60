...
Copyright (c) 1993 - 2019 by Qualcomm Technologies, Inc.  All Rights Reserved.
===========================================================================*/
...
$Header: //components/rel/wcdma.mpss/8.4.0/common/inc/wcdmadiag.h#3 $
...
01/06/16   raj    Add RACH statistic 
...
/* First command code*/
#define WCDMA_DIAG_CMD_CODE_START          WCDMA_DIAG_VERNO_F
...
#define WCDMA_UPDATE_DRX_DEBUG_INFO_F            58

#define WCDMA_DENY_TRM_LOCK_PER_RAT_F     59

#define WCDMA_DL_QXDM_DEMOD_INTERNAL_F     60

#define WCDMA_FW_DEBUG_CMD_F              100

/* Last command code*/
#define WCDMA_DIAG_CMD_CODE_END           WCDMA_FW_DEBUG_CMD_F


#define WCDMA_DIAG_IMSI_SIZE              (sizeof(imsi_data_T))

/* as_id must be the last entry in the req packet */
#define WCDMA_DIAGPKT_REQ_END wsub_id_e_type as_id; \
                              DIAGPKT_REQ_END
...
/* WCDMA internal diag command table entry */
typedef struct 
{
  uint16 cmd_code;
  uint16 req_pkt_size;
  void* (*func_ptr) (void * req_pkt_ptr, sys_modem_as_id_e_type as_id);
} wcdma_diag_pkt_cmd_type;

/* Compile-time check to make sure as_id field is present in structure - must evaluate to 0 */
#define WCDMA_DIAG_PKT_AS_ID_CHECK(cmd_type) (sizeof(((cmd_type*)NULL)->as_id) * 0)

/* Shorthand macro when defining internal command entry  */
#define WCDMA_DIAG_PKT(cmd_code, func_ptr) {cmd_code, sizeof(DIAG_SUBSYS_WCDMA_##cmd_code##_req_type) + WCDMA_DIAG_PKT_AS_ID_CHECK(DIAG_SUBSYS_WCDMA_##cmd_code##_req_type), func_ptr}
...
/*---------------------------------------------------------------------------
    WCDMA L1 SUBSYSTEM VARIABLE UPDATE
---------------------------------------------------------------------------*/
DIAGPKT_SUBSYS_REQ_DEFINE(WCDMA, WCDMA_DENY_TRM_LOCK_PER_RAT_F)
  uint8 irat_type_bmask;
  uint8 trm_lock_deny_number;

WCDMA_DIAGPKT_REQ_END

DIAGPKT_SUBSYS_RSP_DEFINE(WCDMA, WCDMA_DENY_TRM_LOCK_PER_RAT_F)
  uint8 status;
DIAGPKT_RSP_END
...
/* Request packet */
DIAGPKT_SUBSYS_REQ_DEFINE(WCDMA, WCDMA_UPDATE_DRX_DEBUG_INFO_F)
  /*Timeout value used to trigger DL WEAK IND to RRC 
    during cell selection if W doesnt get lock. Value
    should be given in frames*/
  uint8 no_lock_frames;
WCDMA_DIAGPKT_REQ_END

/* Response packet*/
DIAGPKT_SUBSYS_RSP_DEFINE(WCDMA, WCDMA_UPDATE_DRX_DEBUG_INFO_F)
  uint8 status;
DIAGPKT_RSP_END
...
DIAGPKT_SUBSYS_REQ_DEFINE(WCDMA, WCDMA_DL_QXDM_DEMOD_INTERNAL_F)
  uint16 psc;
  uint16 freq;
  uint8 l1m_state_to_crash;
  uint8 debug_switch_type;
  uint32 val;
WCDMA_DIAGPKT_REQ_END

DIAGPKT_SUBSYS_RSP_DEFINE(WCDMA, WCDMA_DL_QXDM_DEMOD_INTERNAL_F)
  uint8 status;
DIAGPKT_RSP_END
...
RETURN VALUE    void *
===========================================================================*/
extern void * wcdma_log_alloc(log_code_type log_code, unsigned int log_pkt_len, wsub_id_e_type wsub_id);
...
===========================================================================*/
extern boolean wcdma_log_submit(void * log_ptr, wsub_id_e_type wsub_id);
...
RETURN VALUE    Boolean
===========================================================================*/
extern void wcdma_log_set_timestamp(void * log_ptr, wsub_id_e_type wsub_id);

#ifdef FEATURE_DUAL_WCDMA
/*================================================================================
FUNCTION        WL1_GET_FRAME_NUMBER_FOR_DIAG_EXT
...
================================================================================*/
extern uint32 wl1_get_frame_number_for_diag_ext(uint32 diag_sub_id);
