...
Copyright (c) 2001 - 2019 by Qualcomm Technologies, Inc.  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
$Header: //components/rel/wcdma.mpss/8.4.0/common/src/wcdmadiag.c#4 $
$DateTime: 2020/01/22 08:56:27 $
$Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
09/11/19   rv      Removing unnused QICE specific QXDM cmds which uses deprecated PLT APIs.
04/29/16   sl      Enhance WCDMA logging of frame numbers to be SUB specific for DUAL_WCDMA
06/01/16   raj     Add RACH transmission Stats
12/16/15   vn      Enhancement for software attenuator
11/03/15   rn      Passing sub_id as argument for l1_get_rssi()
10/20/15   sl      QXDM cmd changes for DEMOD
09/29/15   pr      Check GSM_PLT feature only when ATLAS is not defined.
09/14/15   zt      Adding QXDM support to deny trm lock per irat
08/14/15   rsr     Added change to Support CFN print for SUB2 MSG's/Packets. 
07/31/15   scm     Adjust FEATURE_GSM_PLT usage in order to allow GSM_PLT build to compile.
06/23/15   skk     Mods to change DL WEAK IND for no lock timer during cell selection based on QXDM cmd. 
06/22/15   vn      Addng QXDM support to attenuate WCDMA signal
06/03/15   ac      Fix compiler warnings
06/03/15   pr      W+W Phase8 Checkin.
05/20/15   aa      W+W changes for CFCM
04/22/15   ac      Datapath W+W changes
04/10/15   sr      Updated RRC to use correct as_id
...
extern void drx_set_pwr_strobe(boolean enabled, wsub_id_e_type wsub_id);
...
#if !defined(FEATURE_WCDMA_ATLAS_MODEM) && defined(FEATURE_GSM_PLT)
#error code not present
#else
#ifdef FEATURE_WCDMA
...
void* wcdma_diag_pkt_internal_dispatcher(void * req_pkt_ptr, uint16 pkt_len);
...
PACKED void *wcdmadiag_verno (PACKED void* req_pkt, sys_modem_as_id_e_type as_id);
PACKED void *wcdma_diag_change_bler_rate(PACKED void* req_pkt, sys_modem_as_id_e_type as_id);
PACKED void *wcdmadiag_reacq_slew_stats (PACKED void* req_pkt, sys_modem_as_id_e_type as_id);
PACKED void *wcdmadiag_wcdma_stats (PACKED void* req_pkt, sys_modem_as_id_e_type as_id);
PACKED void *wcdmadiag_clear_wcdma_stats (PACKED void* req_pkt, sys_modem_as_id_e_type as_id);
PACKED void *wcdmadiag_reacq_srch_stats (PACKED void* req_pkt, sys_modem_as_id_e_type as_id);
PACKED void *wcdmadiag_reacq_clear_stats (PACKED void* req_pkt, sys_modem_as_id_e_type as_id);
PACKED void *wcdmadiag_set_trk_lo_pdm (PACKED void* req_pkt, sys_modem_as_id_e_type as_id);
PACKED void *wcdmadiag_call_orig (PACKED void* req_pkt, sys_modem_as_id_e_type as_id);
PACKED void *wcdmadiag_call_end (PACKED void* req_pkt, sys_modem_as_id_e_type as_id);
PACKED void *wcdmadiag_status (PACKED void* req_pkt, sys_modem_as_id_e_type as_id);
PACKED void *wcdmadiag_additional_status (PACKED void* req_pkt, sys_modem_as_id_e_type as_id);
#ifdef FEATURE_MBMS
#error code not present
#endif
#ifdef FEATURE_WCDMA_TBSET_DEBUG_DUMP
...
#endif
#ifdef FEATURE_HSDPA
PACKED void * wcdmadiag_wcdma_cqi_params_handler(PACKED void *req_ptr,sys_modem_as_id_e_type as_id);
#endif
PACKED void * wcdmadiag_wcdma_fw_debug_cmd_handler(PACKED void *req_ptr,sys_modem_as_id_e_type as_id);
#ifdef FEATURE_CPU_BASED_FLOW_CONTROL
PACKED void * wcdmadiag_rlc_flow_control_config_handler(PACKED void *req_ptr,sys_modem_as_id_e_type as_id);
#endif
#ifdef FEATURE_WCDMA_HSUPA
PACKED void * wcdmadiag_manual_eul_ul_var_upd_handler(PACKED void *req_ptr,sys_modem_as_id_e_type as_id);
#endif
#ifdef FEATURE_WCDMA_DC_HSDPA
PACKED void * wcdmadiag_manual_setup_dc_test_cfg(PACKED void *req_ptr,sys_modem_as_id_e_type as_id);
#endif
...
#ifdef FEATURE_WCDMA_RX_DIVERSITY
PACKED void * wcdmadiag_rxd_api_oride_handler(PACKED void *req_ptr,sys_modem_as_id_e_type as_id);
#endif
PACKED void * wcdmadiag_manual_ul_var_upd_handler(PACKED void *req_ptr,sys_modem_as_id_e_type as_id);
#ifdef FEATURE_WCDMA_SAMPLE_SERVER_DATA_LOGGING
#error code not present
#endif
#ifdef FEATURE_WCDMA_L1_PSC_SCANNER
PACKED void * wcdmadiag_l1_psc_scanner_config_handler(PACKED void* req_pkt, sys_modem_as_id_e_type as_id);
#endif
PACKED void * wcdma_diag_write_fbiber_thresh_handler(PACKED void* req_pkt, sys_modem_as_id_e_type as_id);
PACKED void * wcdma_diag_read_fbiber_thresh_handler(PACKED void* req_pkt, sys_modem_as_id_e_type as_id);
/* Code added for Skip CM Measurements from QXDM */
PACKED void * wcdmadiag_skip_cm_meas_var_upd_handler(PACKED void *req_ptr, sys_modem_as_id_e_type as_id);
PACKED void * wcdmadiag_manual_setup_rlc_dbg_cfg(PACKED void *req_pkt,sys_modem_as_id_e_type as_id);
PACKED void * wcdmadiag_manual_setup_mac_dbg_cfg(PACKED void *req_pkt,sys_modem_as_id_e_type as_id);
#ifdef FEATURE_WCDMA_SET_HS_CAT
PACKED void * wcdmadiag_rrc_set_hs_cat(PACKED void *req_ptr,sys_modem_as_id_e_type as_id);
#endif
PACKED void * wcdmadiag_meas_req_handler(PACKED void *req_pkt,sys_modem_as_id_e_type as_id);
#ifdef FEATURE_WCDMA_SECURITY_SETTINGS
PACKED void * wcdmadiag_manual_security_set_integ(PACKED void *req_ptr,sys_modem_as_id_e_type as_id);
PACKED void * wcdmadiag_manual_security_set_ciph(PACKED void *req_ptr,sys_modem_as_id_e_type as_id);
PACKED void * wcdmadiag_manual_security_set_fksec(PACKED void *req_ptr,sys_modem_as_id_e_type as_id);
#endif
PACKED void* wcdmadiag_gpio_pwr_prof_handler(PACKED void* req_pkt,sys_modem_as_id_e_type as_id);
PACKED void * wcdmadiag_debug_crash_on_srch_done_handler(PACKED void* req_pkt,sys_modem_as_id_e_type as_id);
PACKED void * wcdmadiag_attenuate_signal_handler(PACKED void* req_pkt,sys_modem_as_id_e_type as_id);
PACKED void * wcdmadiag_manual_setup_datapath_dbg_cfg(PACKED void *req_pkt,sys_modem_as_id_e_type as_id);
PACKED void * wcdmadiag_update_drx_debug_info_handler(PACKED void* req_pkt,sys_modem_as_id_e_type as_id);
PACKED void * wcdmadiag_deny_trm_lock_per_rat_handler(PACKED void* req_pkt,sys_modem_as_id_e_type as_id);
PACKED void * wcdmadiag_dl_dbg_qxmd_demod_internal_handler(PACKED void *req_pkt, sys_modem_as_id_e_type as_id);
...
/* DO NOT CHANGE. Command table sent to diag - all WCDMA commands must be
   between WCDMA_DIAG_CMD_CODE_START and WCDMA_DIAG_CMD_CODE_END */
static const diagpkt_user_table_entry_type wcdmadiag_tbl[] =
{
  {WCDMA_DIAG_CMD_CODE_START, WCDMA_DIAG_CMD_CODE_END, wcdma_diag_pkt_internal_dispatcher}
};

/* Internal dispatch table. Add command ID and handler function here */
static const wcdma_diag_pkt_cmd_type wcdmadiag_internal_tbl[] = 
{
  WCDMA_DIAG_PKT(WCDMA_DIAG_VERNO_F,                wcdmadiag_verno),
  WCDMA_DIAG_PKT(WCDMA_DIAG_BLER_RATE_CHANGE_F,     wcdma_diag_change_bler_rate),
  WCDMA_DIAG_PKT(WCDMA_DIAG_REACQ_SLEW_STATS_F,     wcdmadiag_reacq_slew_stats),
  WCDMA_DIAG_PKT(WCDMA_DIAG_REACQ_SRCH_STATS_F,     wcdmadiag_reacq_srch_stats),
  WCDMA_DIAG_PKT(WCDMA_DIAG_REACQ_CLEAR_STATS_F,    wcdmadiag_reacq_clear_stats),
  WCDMA_DIAG_PKT(WCDMA_DIAG_SET_TRK_LO_PDM_F,       wcdmadiag_set_trk_lo_pdm),
  WCDMA_DIAG_PKT(WCDMA_DIAG_ORIG_F,                 wcdmadiag_call_orig),
  WCDMA_DIAG_PKT(WCDMA_DIAG_END_F,                  wcdmadiag_call_end),
  WCDMA_DIAG_PKT(WCDMA_STATUS_F,                    wcdmadiag_status),
  WCDMA_DIAG_PKT(WCDMA_ADDITIONAL_STATUS_F,         wcdmadiag_additional_status)
#ifdef FEATURE_WCDMA_SAMPLE_SERVER_DATA_LOGGING
  #error code not present
#endif /* FEATURE_WCDMA_SAMPLE_SERVER_DATA_LOGGING */
#ifdef FEATURE_WCDMA_HSUPA
  ,WCDMA_DIAG_PKT(WCDMA_DIAG_EUL_UL_VAR_UPD_F,      wcdmadiag_manual_eul_ul_var_upd_handler)
#endif
#ifdef FEATURE_CPU_BASED_FLOW_CONTROL
  ,WCDMA_DIAG_PKT(WCDMA_RLC_FLOW_CONTROL_CONFIG_F,  wcdmadiag_rlc_flow_control_config_handler)
#endif //FEATURE_CPU_BASED_FLOW_CONTROL
#ifdef FEATURE_WCDMA_L1_PSC_SCANNER
  ,WCDMA_DIAG_PKT(WCDMA_L1_PSC_SCANNER_CONFIG_F,    wcdmadiag_l1_psc_scanner_config_handler)
#endif //FEATURE_WCDMA_L1_PSC_SCANNER
  ,WCDMA_DIAG_PKT(WCDMA_DIAG_WCDMA_STATS_F,         wcdmadiag_wcdma_stats)
  ,WCDMA_DIAG_PKT(WCDMA_DIAG_WCDMA_CLEAR_STATS_F,   wcdmadiag_clear_wcdma_stats)
  ...
  #error code not present
#endif
  ,WCDMA_DIAG_PKT(WCDMA_DIAG_UL_VAR_UPD_F,          wcdmadiag_manual_ul_var_upd_handler)
#ifdef FEATURE_HSDPA
  ,WCDMA_DIAG_PKT(WCDMA_CQI_OVERIDE_PARAMS_F,       wcdmadiag_wcdma_cqi_params_handler)
#endif
#if defined(FEATURE_WCDMA_PLT) || defined(FEATURE_WCDMA_PLT_MODE)
  #error code not present
#endif /* defined(FEATURE_WCDMA_PLT) || defined(FEATURE_WCDMA_PLT_MODE) */
...
#ifdef FEATURE_WCDMA_RX_DIVERSITY
  ,WCDMA_DIAG_PKT(WCDMA_RXD_API_ORIDE_F,            wcdmadiag_rxd_api_oride_handler)
#endif
  ,WCDMA_DIAG_PKT(WCDMA_FW_DEBUG_CMD_F,             wcdmadiag_wcdma_fw_debug_cmd_handler)
  ,WCDMA_DIAG_PKT(WCDMA_DIAG_WRITE_FBIBER_THRESH_F, wcdma_diag_write_fbiber_thresh_handler)
  ,WCDMA_DIAG_PKT(WCDMA_DIAG_READ_FBIBER_THRESH_F,  wcdma_diag_read_fbiber_thresh_handler)
#ifdef FEATURE_WCDMA_DC_HSDPA
  ,WCDMA_DIAG_PKT(WCDMA_DIAG_DC_HS_SETUP,           wcdmadiag_manual_setup_dc_test_cfg)
#endif
  ,WCDMA_DIAG_PKT(WCDMA_DIAG_RLC_DBG_F,             wcdmadiag_manual_setup_rlc_dbg_cfg)
  ,WCDMA_DIAG_PKT(WCDMA_DIAG_MAC_DBG_F,             wcdmadiag_manual_setup_mac_dbg_cfg)
#ifdef FEATURE_WCDMA_SET_HS_CAT
  ,WCDMA_DIAG_PKT(WCDMA_DIAG_RRC_HS_CAT,            wcdmadiag_rrc_set_hs_cat)
#endif
  ,WCDMA_DIAG_PKT(WCDMA_DIAG_QUERY_MEAS_F,          wcdmadiag_meas_req_handler)
#ifdef FEATURE_WCDMA_SECURITY_SETTINGS
  ,WCDMA_DIAG_PKT(WCDMA_DIAG_INTEGRITY_ENABLE_F,    wcdmadiag_manual_security_set_integ)
  ,WCDMA_DIAG_PKT(WCDMA_DIAG_CIPHERING_ENABLE_F,    wcdmadiag_manual_security_set_ciph)
  ,WCDMA_DIAG_PKT(WCDMA_DIAG_FAKE_SECURITY_ENABLE_F,wcdmadiag_manual_security_set_fksec)
#endif
  ,WCDMA_DIAG_PKT(WCDMA_GPIO_PWR_PROF_F,            wcdmadiag_gpio_pwr_prof_handler)
  ,WCDMA_DIAG_PKT(WCDMA_DIAG_DATAPATH_DBG_F,        wcdmadiag_manual_setup_datapath_dbg_cfg)
  /* Code added for Skip CM Measurement Cmd from QXDM */
  ,WCDMA_DIAG_PKT(WCDMA_DIAG_SKIP_CM_MEAS_VAR_UPD_F,wcdmadiag_skip_cm_meas_var_upd_handler)
  ,WCDMA_DIAG_PKT(WCDMA_DEBUG_CRASH_ON_SRCH_DONE_F, wcdmadiag_debug_crash_on_srch_done_handler)
  ,WCDMA_DIAG_PKT(WCDMA_ATTENUATE_SIGNAL_F, wcdmadiag_attenuate_signal_handler)
  ,WCDMA_DIAG_PKT(WCDMA_UPDATE_DRX_DEBUG_INFO_F, wcdmadiag_update_drx_debug_info_handler)
    ,WCDMA_DIAG_PKT(WCDMA_DENY_TRM_LOCK_PER_RAT_F, wcdmadiag_deny_trm_lock_per_rat_handler)
  ,WCDMA_DIAG_PKT(WCDMA_DL_QXDM_DEMOD_INTERNAL_F, wcdmadiag_dl_dbg_qxmd_demod_internal_handler)
};
...
FUNCTION  wcdma_diag_pkt_internal_dispatcher

DESCRIPTION
  WCDMA internal dispatcher for all commands. Does sanity checks on
  length and extracts AS_ID.

============================================================================*/
void* wcdma_diag_pkt_internal_dispatcher(void * req_pkt_ptr, uint16 pkt_len)
{
  uint32 idx;
  uint32 expected_pkt_len;
  sys_modem_as_id_e_type as_id;
  void *ret_val;

  uint32 cmd_code = diagpkt_subsys_get_cmd_code(req_pkt_ptr);
  uint32 num_cmds = sizeof(wcdmadiag_internal_tbl)/sizeof(wcdmadiag_internal_tbl[0]);

  /* Reference to request - may be replaced by local allocation if command
     is shorter than expected */
  void *local_req_pkt_ptr = req_pkt_ptr;

  WL1_CMN_MSG2(HIGH, "Received WCDMA subsys command %d, length %d", cmd_code, pkt_len);

  /* Search internal table for command handler*/
  for(idx = 0; idx < num_cmds; idx++)
  {
    if(cmd_code == wcdmadiag_internal_tbl[idx].cmd_code)
    {
      break;
    }
  }
  ...
  if(idx == num_cmds)
  {
    WL1_CMN_MSG1(ERROR, "Unrecognized WCDMA diag command %d", cmd_code);
    return NULL;
  }

  expected_pkt_len = wcdmadiag_internal_tbl[idx].req_pkt_size;

  /* Sanity check on length. If shorter than expected, pad with 0 to allow for old scripts that aren't
     providing as_id.*/
  if(pkt_len < expected_pkt_len)
  {
    WL1_CMN_MSG2(ERROR, "Command size %d smaller than expected, expecting %d. Padding with 0.", pkt_len, expected_pkt_len);   

    /* Allocate buffer and copy contents from diag buffer to new buffer */
    local_req_pkt_ptr = modem_mem_alloc(expected_pkt_len, MODEM_MEM_CLIENT_WCDMA_EXT_CMD_CRIT);
    if(local_req_pkt_ptr == NULL)
    {
      WL1_CMN_MSG1(ERROR, "Could not allocate %d bytes.", expected_pkt_len);   
      return NULL;
    }
    memset(local_req_pkt_ptr, 0, expected_pkt_len);
    WCDMA_MEMCPY(local_req_pkt_ptr, expected_pkt_len, req_pkt_ptr, pkt_len);
  }
  else if(pkt_len > expected_pkt_len)
  {
    WL1_CMN_MSG2(ERROR, "Command size %d larger than expected, expecting %d. Command rejected.", pkt_len, expected_pkt_len);
    return NULL;
  }

  /* Extract as_id from end of request packet */
  as_id = *(sys_modem_as_id_e_type*)((uint8*)local_req_pkt_ptr + expected_pkt_len - sizeof(sys_modem_as_id_e_type));

  /* Sanity check on as_id */
  if((as_id < 0) || (as_id >= WCDMA_NUM_SUBS))
  {
    WL1_CMN_MSG2(ERROR, "Invalid as_id %d, max is %d. Defaulting to 0.", as_id, WCDMA_NUM_SUBS-1);
    as_id = SYS_MODEM_AS_ID_1;
  }

  /* Call command handler*/
  WL1_CMN_MSG2(HIGH, "Processing WCDMA subsys command %d, as_id %d", cmd_code, as_id);
  ret_val = wcdmadiag_internal_tbl[idx].func_ptr(local_req_pkt_ptr, as_id);

  /* Free locally-allocated buffer */
  if(local_req_pkt_ptr != req_pkt_ptr)
  {
    modem_mem_free(local_req_pkt_ptr, MODEM_MEM_CLIENT_WCDMA_EXT_CMD_CRIT);
  }
...
PACKED void *wcdma_diag_change_bler_rate(PACKED void* req_pkt, sys_modem_as_id_e_type as_id)
{
  ...
  wl1dec_ext_cntrl_per_sub_params_type *ext_wl1dec_cntrl_per_sub = &wl1dec_ext_cntrl_per_sub_params[as_id];

  ext_wl1dec_cntrl_per_sub->dl_bler_meas_timer_update = TRUE;
  ext_wl1dec_cntrl_per_sub->dl_bler_meas_duration_ms = req_ptr->bler_meas_duration_ms;
  ext_wl1dec_cntrl_per_sub->dl_override_bler_meas_duration = TRUE;
...
PACKED void *wcdmadiag_reacq_slew_stats (PACKED void* req_pkt, sys_modem_as_id_e_type as_id)
{
  ...
  srchzz_ext_cntrl_params_type *ext_srchzz_cntrl = &srchzz_ext_cntrl_params[as_id];
  ...
  {
    rsp_ptr->srchzz_reacq_slew_stats = ext_srchzz_cntrl->srchzz_ext_cntrl_params_mdump.srchzz_reacq_slew_stats;
  }
...
PACKED void *wcdmadiag_wcdma_stats (PACKED void* req_pkt, sys_modem_as_id_e_type as_id)
{
  ...
  wl1dec_ext_cntrl_per_sub_params_type *ext_wl1dec_cntrl_per_sub = &wl1dec_ext_cntrl_per_sub_params[as_id];
  drx_ext_cntrl_params_type *ext_drx_cntrl = &drx_ext_cntrl_params[as_id];
  srchzz_ext_cntrl_params_type *ext_srchzz_cntrl = &srchzz_ext_cntrl_params[as_id];
  ...
  else
  {
    ...
    /* Reacq search stats*/
    rsp_ptr->srchzz_reacq_srch_stats = ext_srchzz_cntrl->srchzz_reacq_srch_stats;
  
    /* Number of times SFN mismatch is detected */
    rsp_ptr->dl_num_times_sfn_mismatch = ext_wl1dec_cntrl_per_sub->dl_num_times_sfn_mismatch;
    ...
    #error code not present
#else
    rsp_ptr->page_stats.rrcpg1_num_pages = rrcpg1_get_num_pages(as_id) - ext_drx_cntrl->reset_rrcpg1_num_pages;
    rsp_ptr->page_stats.rrcpg1_num_pages_matched = rrcpg1_get_num_pages_matched(as_id) -
                                                          ext_drx_cntrl->reset_rrcpg1_num_pages_matched;
  #endif /* FEATURE_WCDMA_PLT */
    rsp_ptr->page_stats.drx_pich_demod_error = 
                                   ext_drx_cntrl->drx_parms.num_zero_pisum + ext_drx_cntrl->drx_parms.num_po_overruns;
    rsp_ptr->page_stats.drx_cycle_len = ext_drx_cntrl->drx_parms.drx_cycle_len;
    rsp_ptr->page_stats.num_drx_cycles = ext_drx_cntrl->drx_parms.total_drx_cycles;
  
  #ifdef FEATURE_WCDMA_PLT
    #error code not present
#else
    /* Various Intra freq cell reselection statistics */
    rsp_ptr->intra_freq_resel_stats.intra_freq_resel_success_cnt = rrc_debug_get_cel_resel_success_cnt(as_id);
    rsp_ptr->intra_freq_resel_stats.intra_freq_nbcch_setup_failure_cnt = rrc_debug_get_cel_resel_nbcch_set_up_fail_cnt(as_id);
    rsp_ptr->intra_freq_resel_stats.intra_freq_sib_wait_timeout_cnt = rrc_debug_get_cel_resel_sib_wait_timeout_cnt(as_id);
    rsp_ptr->intra_freq_resel_stats.intra_freq_transition_fail_cnt = rrc_debug_get_cel_resel_transition_fail_cnt(as_id);
    rsp_ptr->intra_freq_resel_stats.intra_freq_failed_other_causes_cnt = rrc_debug_get_cel_resel_failed_other_causes_cnt(as_id);
  
    /* Various WCDMA to GSM cell reselection statistics */
    rsp_ptr->wtog_resel_stats.wtog_resel_cnt = rrc_debug_get_wtog_cel_resel_cnt(as_id);
    rsp_ptr->wtog_resel_stats.wtog_resel_failure_cnt = rrc_debug_get_wtog_cel_resel_fail_cnt(as_id);
  
    /* Various OOS related statistics */
    rsp_ptr->oos_stats.oos_idle_cnt = rrc_debug_get_oos_idle_mode_cnt(as_id);
    rsp_ptr->oos_stats.oos_connected_cnt = rrc_debug_get_oos_connected_mode_cnt(as_id);
  
    /* Various RACH transmission related statistics,This is currently TBD. Its values are set to Zero for now*/
    rsp_ptr->rach_transmission_stats.succful_rrc_tran_retry_cnt = rrc_debug_get_succful_rrc_trans_retry_cnt(as_id);
    rsp_ptr->rach_transmission_stats.rrc_retry_fail_due_to_l2_ack_ind = rrc_debug_get_rrc_retry_fail_due_to_l2_ack_ind_cnt(as_id);
    rsp_ptr->rach_transmission_stats.rrc_retry_fail_due_to_others = rrc_debug_get_rrc_retry_fail_due_to_others_cnt(as_id);
  #endif /* FEATURE_WCDMA_PLT */
  
  /* RACH Transmission Preamble related statistics */

    memset((void*)(rsp_ptr->rach_transmission_preamble_stats.rach_ack_prmbl_cnt),0x00,sizeof(rsp_ptr->rach_transmission_preamble_stats.rach_ack_prmbl_cnt));
    
    rsp_ptr->rach_transmission_preamble_stats.rach_noack_cnt = 0;
    ...
    rsp_ptr->rach_transmission_preamble_stats.rach_prmbl_detected_max_pwr_dB =
                                                      RACH_STATS_INITIAL_TXAGC;
    rsp_ptr->rach_transmission_preamble_stats.rach_detected_last_prmbl_avg_pwr_dB =
                                                      0;
    rsp_ptr->rach_transmission_preamble_stats.rach_noack_last_prmbl_avg_pwr_dB = 0;
    rsp_ptr->rach_transmission_preamble_stats.rach_noack_prmbl_max_pwr_dB =RACH_STATS_INITIAL_TXAGC;
...
PACKED void *wcdmadiag_clear_wcdma_stats (PACKED void* req_pkt, sys_modem_as_id_e_type as_id)
{
  ...
  ** Because we dont want to reset the counters themselves. */
  ...
  ext_drx_cntrl->reset_rrcpg1_num_pages = rrcpg1_get_num_pages(as_id);
  ext_drx_cntrl->reset_rrcpg1_num_pages_matched = rrcpg1_get_num_pages_matched(as_id);
#endif /* FEATURE_WCDMA_PLT */
  ext_drx_cntrl->drx_parms.num_zero_pisum = 0;
  ext_drx_cntrl->drx_parms.num_po_overruns = 0;
  ext_drx_cntrl->drx_parms.total_drx_cycles = 0;

  /* Clear out the SFN mismatch counter */
  ext_wl1dec_cntrl_per_sub->dl_num_times_sfn_mismatch = 0;

  /* Clear out all reacquisition statistics. */
  memset((void*)&ext_srchzz_cntrl->srchzz_ext_cntrl_params_mdump.srchzz_reacq_slew_stats, 0x00, sizeof(srchzz_reacq_slew_stats_type));
  memset((void*)&ext_srchzz_cntrl->srchzz_reacq_srch_stats, 0x00, sizeof(srchzz_reacq_srch_stats_type));

#ifndef FEATURE_WCDMA_PLT
  /* Clear out all RRC statistics. */
  RRC_DEBUG_CLEAR_ALL_STATS(as_id);
#endif /* FEATURE_WCDMA_PLT */
...
PACKED void *wcdmadiag_set_trk_lo_pdm (PACKED void* req_pkt, sys_modem_as_id_e_type as_id)
{
  ...
  wl1_freeze_and_set_trk_lo_pdm(req_ptr->tcxo_pdm_val, as_id);
#endif
...
PACKED void *wcdmadiag_call_orig (PACKED void* req_pkt, sys_modem_as_id_e_type wsub_id)
{
...
PACKED void *wcdmadiag_call_end (PACKED void* req_pkt, sys_modem_as_id_e_type as_id)
{
  ...
  for (i = 0; i < CM_CALL_ID_MAX; i++)
  {
    if ( cm_call_id_is_allocated ((cm_call_id_type) i) )
    {
      ...
      WL1_CMN_MSG1(HIGH , "WCMDADIAG::END_REQ: Adding call_id %d to the end list",
                      end_params.call_id);
...
PACKED void *wcdmadiag_status (PACKED void* req_pkt, sys_modem_as_id_e_type wsub_id)
{
...
============================================================================*/
PACKED void *wcdmadiag_additional_status (PACKED void* req_pkt, sys_modem_as_id_e_type wsub_id)
{
  ...
  wsub_id_e_type  as_id = rrc_get_as_id(wsub_id);
  ...
  else
  {
    ...
    rsp_ptr->l1_state = l1m_get_state(wsub_id);
  }
...
)
{
   ...
   mdspasync_update_wfw_debug_cmd(config_req_ptr->debug_pkt, as_id);
...
)
{
   ...
   rlc_ul_set_fc_config(as_id,
                     config_req_ptr->ws_step_period,
                     config_req_ptr->ws_step_stage,
                     config_req_ptr->init_ws);
   #endif

   #ifdef FEATURE_DL_CPU_BASED_FC
   rlc_dl_set_fc_config(as_id,
                     config_req_ptr->ws_step_period,
                     config_req_ptr->ws_step_stage,
                     config_req_ptr->init_ws);
   #endif
...
)
{
   ...
   eul_set_var_through_qxdm(
         upd_req_ptr->debug_enable_f3_message_type,upd_req_ptr->debug_var_val, as_id);
...
)
{
   ...
   {
     WL1_CMN_MSG1(ERROR, "Incorrect op_type %d", upd_req_ptr->enable);
     upd_rsp_ptr->status = 0;
   }
   else
   {
     upd_rsp_ptr->status = 
       wl1_dl_carrcfg_setup_debug_dc_cfg(upd_req_ptr->dc_psc, upd_req_ptr->dc_freq, upd_req_ptr->enable, as_id);
   }
...
)
{
   ...
   oride_rsp_ptr->status = wl1_rxd_api_oride_cmd(oride_req_ptr->api_identifier,
           oride_req_ptr->input_arg_1, oride_req_ptr->input_arg_2, oride_req_ptr->input_arg_3, oride_req_ptr->input_arg_4, as_id);
...
)
{
   ...
   l1_cm_set_skip_cm_meas_through_qxdm(upd_req_ptr->debug_var_val, as_id);
...
PACKED void * wcdmadiag_l1_psc_scanner_config_handler(PACKED void* req_pkt, sys_modem_as_id_e_type as_id)
{
  ...
  else
  {
      ...
      )
    {
      ...
      srch_mview_post_scan_state_event(scan_state, as_id);
    }
    else
    {
...
PACKED void * wcdma_diag_write_fbiber_thresh_handler(PACKED void* req_pkt, sys_modem_as_id_e_type as_id)
{
  ...
  {
    rsp_ptr->status = wl1_write_fbiber_threshold(req_ptr->fbiBER_threshold, as_id);
  }
...
PACKED void * wcdma_diag_read_fbiber_thresh_handler(PACKED void* req_pkt, sys_modem_as_id_e_type as_id)
{
  ...
  {
    rsp_ptr->status = wl1_read_fbiber_threshold(&threshold, as_id);
...
)
{
   ...
   /* Call the RLC function that will take the values from diag request packet
      and update the variable */
   wcdma_rlc_set_dbg_var_through_qxdm(as_id,
...
)
{
   ...
   /* Call the MAC function that will take the values from diag request packet
      and update the variable */
   wcdma_mac_set_dbg_var_through_qxdm(as_id,
         req_ptr->debug_var_type,req_ptr->debug_var_val);
...
)
{
   ...
   /* Generate the response */
     ...
     upd_rsp_ptr->status = 
       rrc_qxdm_set_hs_cat(as_id, upd_req_ptr->hs_cat);
...
)
{
   ...
   /* Generate the response */
   int_rsp_ptr->status = rrc_qxdm_integrity_enable_diagcmd(as_id, int_req_ptr->enable);  
...
)
{
  ...
  /* Generate the response */
  ciph_rsp_ptr->status = rrc_qxdm_ciphering_enable_diagcmd(as_id, ciph_req_ptr->enable);
...
)
{
  ...
  /* Generate the response */
  fk_rsp_ptr->status = rrc_qxdm_fakesecurity_enable_diagcmd(as_id, fk_req_ptr->enable);
...
)
{
   ...
   drx_set_pwr_strobe(req_ptr->enabled, as_id);
...
)
{
  ...
  wcdma_datapath_set_datapath_f3_dbg_var_through_qxdm(
    req_ptr->debug_var_type,req_ptr->debug_var_val, as_id);
...
)
{
  ...
  srch_enable_and_update_crash_info(req_ptr->l1m_state, req_ptr->bmask,
     req_ptr->carr_idx, req_ptr->type_of_search, req_ptr->thresh, req_ptr->freq, req_ptr->psc, as_id);

  return (void*) rsp_ptr;
} 

/*===========================================================================
FUNCTION        WCDMADIAG_DL_DBG_QXDM_DEMOD_INTERNAL_HANDLER

DESCRIPTION     This function gets invoked upon issuing a QXDM command and calls
                the high-level command handling function that sets DEMOD global variables
                that helps to crash UE upon various scenarios.
...
SIDE EFFECTS    
                None
===========================================================================*/
PACKED void * wcdmadiag_dl_dbg_qxmd_demod_internal_handler
(
   PACKED void *req_pkt,
   sys_modem_as_id_e_type as_id
)
{
  ...
  DIAG_SUBSYS_WCDMA_WCDMA_DL_QXDM_DEMOD_INTERNAL_F_req_type *req_ptr;
  DIAG_SUBSYS_WCDMA_WCDMA_DL_QXDM_DEMOD_INTERNAL_F_rsp_type *rsp_ptr;
  req_ptr = (DIAG_SUBSYS_WCDMA_WCDMA_DL_QXDM_DEMOD_INTERNAL_F_req_type*) req_pkt;

  /* Prepare the response packet */
  rsp_ptr = (DIAG_SUBSYS_WCDMA_WCDMA_DL_QXDM_DEMOD_INTERNAL_F_rsp_type*)
            diagpkt_alloc
            (
             diagpkt_get_cmd_code(req_pkt),
             sizeof(DIAG_SUBSYS_WCDMA_WCDMA_DL_QXDM_DEMOD_INTERNAL_F_rsp_type)
             );
  ...
  /* Call the function to update the debug crash info */
  dl_dbg_qxdm_demod_internal(req_ptr->psc, req_ptr->freq, req_ptr->l1m_state_to_crash,req_ptr->debug_switch_type, req_ptr->val, as_id);
...
============================================================================*/
PACKED void * wcdmadiag_deny_trm_lock_per_rat_handler
(
	 PACKED void *req_pkt,
   sys_modem_as_id_e_type as_id
)
{
	DIAG_SUBSYS_WCDMA_WCDMA_DENY_TRM_LOCK_PER_RAT_F_req_type *req_ptr;
  DIAG_SUBSYS_WCDMA_WCDMA_DENY_TRM_LOCK_PER_RAT_F_rsp_type *rsp_ptr;
  req_ptr = (DIAG_SUBSYS_WCDMA_WCDMA_DENY_TRM_LOCK_PER_RAT_F_req_type*) req_pkt;

  /* Prepare the response packet */
  rsp_ptr = (DIAG_SUBSYS_WCDMA_WCDMA_DENY_TRM_LOCK_PER_RAT_F_rsp_type*)
            diagpkt_alloc
            (
             diagpkt_get_cmd_code(req_pkt),
             sizeof(DIAG_SUBSYS_WCDMA_WCDMA_DENY_TRM_LOCK_PER_RAT_F_rsp_type)
             );
  ...
  rsp_ptr->status = 1;
	...
	/* Call the function to update the deny trm lock per rat info */

  srch_enable_and_update_deny_trm_lock_per_rat_info(req_ptr->irat_type_bmask, req_ptr->trm_lock_deny_number, as_id);
...
)
{
  ...
  rsp_ptr->status = 1;
	...
	/* Call the function to update the debug crash info */
  srch_enable_and_update_attenuate_info(req_ptr->attenuate_value, req_ptr->attenuated_sub_id, as_id);

  return (void*) rsp_ptr;
} 
/*===========================================================================

FUNCTION  wcdmadiag_udpate_drx_debug_info_handler

DESCRIPTION
  This function updates the debug info used by DRX module.

============================================================================*/
PACKED void * wcdmadiag_update_drx_debug_info_handler
(
	 PACKED void *req_pkt,
   sys_modem_as_id_e_type as_id
)
{
  DIAG_SUBSYS_WCDMA_WCDMA_UPDATE_DRX_DEBUG_INFO_F_req_type *req_ptr;
  DIAG_SUBSYS_WCDMA_WCDMA_UPDATE_DRX_DEBUG_INFO_F_rsp_type *rsp_ptr;
  req_ptr = (DIAG_SUBSYS_WCDMA_WCDMA_UPDATE_DRX_DEBUG_INFO_F_req_type*) req_pkt;

  /* Prepare the response packet */
  rsp_ptr = (DIAG_SUBSYS_WCDMA_WCDMA_UPDATE_DRX_DEBUG_INFO_F_rsp_type*)
             diagpkt_alloc( diagpkt_get_cmd_code(req_pkt),
                            sizeof(DIAG_SUBSYS_WCDMA_WCDMA_UPDATE_DRX_DEBUG_INFO_F_rsp_type)
                           );
  ...
  /* Call the function to update the debug crash info */
  drx_update_debug_info_from_qxdm_cmd(req_ptr->no_lock_frames, as_id);
...
{ 
 ...
 {
   WL1_CMN_MSG0(ERROR, "wcdmadiag already initialized");
   return;
 }
...
uint32 wl1_get_frame_number_for_diag(void)
{
  wsub_id_e_type wsub_id = WL1_DEFAULT_SUB_ID;
  wl1m_ext_cntrl_params_type *ext_wl1m_cntrl = &wl1m_ext_cntrl_params[wsub_id];
 #ifdef FEATURE_DUAL_WCDMA
  wsub_id_e_type alt_wsub_id  = WL1_GET_ALT_SUB_ID(WL1_DEFAULT_SUB_ID);
  wl1m_ext_cntrl_params_type *ext_alt_wl1m_cntrl  = &wl1m_ext_cntrl_params[alt_wsub_id];
 #endif 

  uint32 frame_number = 0xFFFFFFFF;


  if ((ext_wl1m_cntrl->l1m_state == L1M_STOPPED) || (ext_wl1m_cntrl->l1m_state == L1M_DEEP_SLEEP) ||
      (ext_wl1m_cntrl->l1m_state == L1M_DEACTIVE))
  {
#ifdef FEATURE_DUAL_WCDMA
    if ((ext_alt_wl1m_cntrl->l1m_state == L1M_STOPPED) || (ext_alt_wl1m_cntrl->l1m_state == L1M_DEEP_SLEEP) ||
        (ext_alt_wl1m_cntrl->l1m_state == L1M_DEACTIVE))
    {
    ...
    {
      if (l1m_is_connected(alt_wsub_id))
      {
        frame_number = wl1_seq_get_cfn(alt_wsub_id);
      }
      else
      {
        frame_number = wl1_seq_get_sfn(alt_wsub_id);
      }
   }   
#endif
  }
  ...
  {
    if (l1m_is_connected(wsub_id))
    {
      frame_number = wl1_seq_get_cfn(wsub_id);
    }
    else
    {
      frame_number = wl1_seq_get_sfn(wsub_id);
    }
  }
...
DESCRIPTION     This function returns the frame number for diag base on the SUB ID

PARAMETER       SUB-ID for which frame number is requested.

RETURN VALUE    uint32 value.
================================================================================*/
uint32 wl1_get_frame_number_for_diag_ext(uint32 diag_sub_id)
{
  uint32 frame_number = 0xFFFFFFFF;
  wsub_id_e_type wsub_id = (wsub_id_e_type)(diag_sub_id - 1);
  if(wsub_id < WCDMA_NUM_SUBS)
  {
    if (l1m_is_connected(wsub_id))
    {
      frame_number = wl1_seq_get_cfn(wsub_id);
...
#endif  /*FEATURE_WCDMA*/
#endif /*!defined(FEATURE_WCDMA_ATLAS_MODEM) && defined(FEATURE_GSM_PLT)*/
/*===========================================================================
FUNCTION        wcdma_log_alloc

DESCRIPTION     This function allocates the memory for the log packet as specified with the log code and pkt length

PARAMETER       log code, log pkt length and W sub id.

RETURN VALUE    void *
===========================================================================*/
void * wcdma_log_alloc(log_code_type log_code, unsigned int log_pkt_len, wsub_id_e_type wsub_id)
{
#ifdef FEATURE_DUAL_WCDMA
  uint32 diag_sub_id = (uint32) wl1_diag_sub_id[wsub_id];
  return log_alloc_ext(log_code, log_pkt_len, 1, &diag_sub_id);
#else
  return log_alloc(log_code, log_pkt_len);
#endif /*FEATURE_DUAL_WCDMA*/
}

/*===========================================================================
FUNCTION        wcdma_log_submit

DESCRIPTION     This function returns the status of the log packet submission to diag

PARAMETER       log ptr and W sub id.

RETURN VALUE    Boolean
===========================================================================*/
boolean wcdma_log_submit(void * log_ptr, wsub_id_e_type wsub_id)
{
#ifdef FEATURE_DUAL_WCDMA
  uint32 diag_sub_id = (uint32) wl1_diag_sub_id[wsub_id];
  return log_submit_ext(log_ptr, 1, &diag_sub_id);
#else
  return log_submit(log_ptr);
#endif /*FEATURE_DUAL_WCDMA*/
}

/*===========================================================================
FUNCTION        wcdma_log_set_timestamp

DESCRIPTION     Sets the time-stamp/frame-number to be logged

PARAMETER       log ptr and W sub id.

RETURN VALUE    Boolean
===========================================================================*/
void wcdma_log_set_timestamp(void * log_ptr, wsub_id_e_type wsub_id)
{
#ifdef FEATURE_DUAL_WCDMA
  uint32 diag_sub_id = (uint32)wl1_diag_sub_id[wsub_id];
  return log_set_timestamp_ext(log_ptr,1,&diag_sub_id);
#else
  return log_set_timestamp(log_ptr);
#endif /*FEATURE_DUAL_WCDMA*/
}