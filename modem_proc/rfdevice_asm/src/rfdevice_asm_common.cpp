...
$Header: //components/rel/rfdevice_asm.mpss/1.32/src/rfdevice_asm_common.cpp#4 $
$Author: mplcsds1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
12/17/24   sr      Added ag_script_present flag to check if disable scripts are present
01/08/20   smi     Fix RF leakage in RBSC
12/31/19   smi     Add support to get RFFE device register address list
12/31/19   smi     update SV voting to handle ASM sleep
03/01/18   dbc     Fix KW errors
12/19/17   dbc     Add support for 16 bit product ids
01/14/16   dbc     Initialize the instance_num in the constructor
01/11/16   dbc     Abort sleep if ASM is enabled when sleep API is invoked 
11/18/15   yb      Manage Non Fatal RFC bit
11/11/15   dbc     Fix KW warnings
11/03/15   dbc     Always check LTE_active_bands and lte_dlca_rfm_vote votes in
                   disable_rx API
09/30/15   dbc     Added ASM votes for LTE ULCA and RFM scenarios
09/11/15   dbc     Add RF Device log messsage for WLAN\WWAN ANt sharing
08/31/15   dbc     Add support for WWAN/WLAN antenna sharing
07/24/15   jfc     Ignore RFC_NONFATAL_IF_MISSING_BIT_IND in self_test() 
07/23/15   px      Add check for LTE mode before updating CA vote flag in disable_rx
...
rfdevice_asm_data * rfdevice_asm_data_create(uint16 mfg_id, uint16 prd_id, uint8 rev_id);
    ...
    rfc_logical_device_info_type *logical_device_info)
{
  ...
  this->separate_regs_for_ca = FALSE;
  rfdevice_asm_common::parent_physical_device_ptr = NULL;
  this->asm_data_ptr = NULL;
  rfdevice_asm_cfg asm_params;
  ...
  /*reset vote bank*/
  memset(&rx_tx_vote[0][0],0,sizeof(uint64) * RFM_NUM_MODES * 2);
  memset(&sleep_wakeup_vote[0][0],0,sizeof(uint64) * RFM_NUM_MODES * 2);
  memset(&lte_ca_vote[0][0],0,sizeof(uint64) * RFM_MAX_WAN_DEVICES * RFDEVICE_ASM_RX_TX_MAX);
  memset(&sv_vote[0],0,sizeof(uint32) * RFM_MAX_WAN_DEVICES);
  ...
  if((phy_device_info != NULL) && (logical_device_info != NULL) && (logical_device_info->rf_device_type == RFDEVICE_ASM))
  {
    ...
    this->rev_id           &= (~RFC_NONFATAL_IF_MISSING_BIT_IND);
    ...
    {
      this->asm_data_ptr = rfdevice_asm_data_create(this->mfg_id, this->prd_id, (uint8)this->rev_id);
    }

    if (this->asm_data_ptr != NULL)
    {
      rfdevice_asm_info_type asm_info = {0};
	    this->asm_data_ptr->device_info_get(&asm_info);
      this->separate_regs_for_ca = asm_info.separate_regs_for_ca;
      ...
      asm_params.trigger_type = RFDEVICE_ASM_TRIGGER_NONE;
      sleep(&asm_params, NULL, RFDEVICE_EXECUTE_IMMEDIATE, 0);
    }
    else
    {
      this->init_status = FALSE;
                                    ...
                                    int16 timing)
{
  ...
  asm_cfg.ag_script_present = TRUE;
  ...
  if (!status && this->mfg_id == 0x1b0 && this->prd_id == 0x3660 && asm_cfg.req == RFDEVICE_ASM_OFF_DATA)  //ASM_CXA4498GC_01 doesn't have disable settings
  {
    RFDEVICE_LOG_MSG_5(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_LOW,"rfdevice_asm_common::config ASM does not have settings for Script: %d; Usid=0x%x; Bus:%d; Tech=%d; Band=%d ",
                        asm_cfg.req, 
                        this->assigned_usid,
                        this->rffe_bus,
                        asm_cfg_p->mode, 
                        asm_cfg_p->band.lte_band);
    return TRUE;    
  }
  ...
  {
    if(asm_enable == FALSE && asm_cfg.ag_script_present == FALSE)
    {
      //Since disable scripts are not mandatory
  	  status = TRUE;
    }
    else
	{
                                       ...
                                       int16 script_timing)
{
  ...
  if(!status)
  {
    RF_MSG_6(RF_ERROR, "config_rx failed ASM usid=0x%x, device=%d, mode=%d, band=%d  exec_type=%d, timing=%d",
             this->assigned_usid, asm_cfg_p->rfm_device, asm_cfg_p->mode, asm_cfg_p->band.lte_band, execution_type, script_timing);
  }
  else
  {
    /* Update Votes */
    this->rx_tx_vote[asm_cfg_p->mode][RFDEVICE_ASM_RX_0] = 
      this->rx_tx_vote[asm_cfg_p->mode][RFDEVICE_ASM_RX_0] |( ((uint64)0x01) << asm_cfg_p->band.lte_band );

    this->sv_vote[asm_cfg_p->rfm_device] = this->sv_vote[asm_cfg_p->rfm_device] | (((uint32)0x01) << asm_cfg_p->mode);

    /* Track active LTE bands (for CA) based on RFM Device */
    if (asm_cfg_p->mode==RFCOM_LTE_MODE || asm_cfg_p->mode==RFM_LTE_FTM_MODE)
    {
      this->lte_ca_vote[asm_cfg_p->rfm_device][RFDEVICE_ASM_RX_0] =
        this->lte_ca_vote[asm_cfg_p->rfm_device][RFDEVICE_ASM_RX_0] |( ((uint64)0x01) << asm_cfg_p->band.lte_band );
    }

    /* TIME PROFILING  */
    RFDEVICE_LOG_MSG_7(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_HIGH,"ASM Enable Rx for usid=0x%x, device=0x%x, tech=%d, band=%d exec_type=%d,"
                       " timing=%d completed in %d us", this->assigned_usid, asm_cfg_p->rfm_device, asm_cfg_p->mode, 
                       asm_cfg_p->band.lte_band, execution_type, script_timing, rf_time_get_elapsed(start_tick,RF_USEC));
                                       ...
                                       int16 script_timing)
{
  ...
  else
  {
    ...
    /* Update Votes */
    this->rx_tx_vote[asm_cfg_p->mode][RFDEVICE_ASM_TX_0] = this->rx_tx_vote[asm_cfg_p->mode][RFDEVICE_ASM_TX_0] |( ((uint64)0x01) << asm_cfg_p->band.lte_band );
     
    /* Track active Tx LTE Bands (for CA) based on RFM Device */
    if (asm_cfg_p->mode==RFCOM_LTE_MODE || asm_cfg_p->mode==RFM_LTE_FTM_MODE)
    {
      this->lte_ca_vote[asm_cfg_p->rfm_device][RFDEVICE_ASM_TX_0] = this->lte_ca_vote[asm_cfg_p->rfm_device][RFDEVICE_ASM_TX_0] | 
            ( ((uint64)0x01) << asm_cfg_p->band.lte_band );
    }

    /* TIME PROFILING  */
    RFDEVICE_LOG_MSG_7(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_HIGH,"ASM Enable Tx for usid=0x%x,tech=%d, band=%d, device=%d, exec_type=%d, timing=%d completed in %d us",
                       this->assigned_usid, asm_cfg_p->mode, asm_cfg_p->band.lte_band, asm_cfg_p->rfm_device, execution_type, script_timing,rf_time_get_elapsed(start_tick,RF_USEC));
  }
                                        ...
                                        int16 script_timing)
{
  ...
  if (!(this->rx_tx_vote[asm_cfg_p->mode][RFDEVICE_ASM_RX_0] & ( ((uint64)0x01) << asm_cfg_p->band.lte_band )))
  {
    RFDEVICE_LOG_MSG_3(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_HIGH, "ASM Disable_rx sequence anomaly since enable_rx was not called for usid=0x%x," 
                       "tech %d, band %d", this->assigned_usid, asm_cfg_p->mode, asm_cfg_p->band.lte_band);
  }

  /* reset the bit for specified band/mode/RX */
  this->rx_tx_vote[asm_cfg_p->mode][RFDEVICE_ASM_RX_0] = this->rx_tx_vote[asm_cfg_p->mode][RFDEVICE_ASM_RX_0] & 
                                                           (~( ((uint64)0x01) << asm_cfg_p->band.lte_band ));

  /* LTE Voting require special handling because of contiguous & non-contiguous CA cases */
  if (asm_cfg_p->mode==RFCOM_LTE_MODE || asm_cfg_p->mode==RFM_LTE_FTM_MODE)
  {
    /* reset the CA vote for RX */
    this->lte_ca_vote[asm_cfg_p->rfm_device][RFDEVICE_ASM_RX_0]=this->lte_ca_vote[asm_cfg_p->rfm_device][RFDEVICE_ASM_RX_0] &
                                                                (~( ((uint64)0x01) << asm_cfg_p->band.lte_band ));
    /* update the RX rx_tx_vote for LTE, since rx_tx_vote does not track RFM device */
    for ( int i=0; i<RFM_MAX_WAN_DEVICES; i++) 
    {
      /* make sure the RX rx_tx_vote tracks CA, CA vote uses same band based bit mask */
      this->rx_tx_vote[asm_cfg_p->mode][RFDEVICE_ASM_RX_0] = this->rx_tx_vote[asm_cfg_p->mode][RFDEVICE_ASM_RX_0] | 
                                                             this->lte_ca_vote[i][RFDEVICE_ASM_RX_0];
    }
  }

  for ( int i=0; i<RFM_MAX_WAN_DEVICES; i++) 
  {
    if ( (this->lte_ca_vote[i][RFDEVICE_ASM_RX_0] && !(this->separate_regs_for_ca)) || this->sv_vote[i])
    {
      RFDEVICE_LOG_MSG_7(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_HIGH, "ASM Disable_rx cannot be executed as there are active techs for usid=0x%x" 
                         " tech %d, band %d lte_dlca_vote=0x%lx%08lx, sv_vote=0x%x, for device=%d",
                         this->assigned_usid, asm_cfg_p->mode, asm_cfg_p->band.lte_band,(this->lte_ca_vote[i][RFDEVICE_ASM_RX_0] >> 32), 
                         (this->lte_ca_vote[i][RFDEVICE_ASM_RX_0] & 0xffffffff), this->sv_vote[i], i);
      return TRUE;
    }
  }
  ...
  if(!status)
  {
    RF_MSG_6(RF_ERROR, "config_rx failed ASM usid=0x%x, device=%d, tech=%d, band=%d exec_type=%d, timing=%d",
             this->assigned_usid, asm_cfg_p->rfm_device, asm_cfg_p->mode, asm_cfg_p->band.lte_band, execution_type, script_timing);
  }
  else
  {
    ...
    /* TIME PROFILING  */
    RFDEVICE_LOG_MSG_7(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_HIGH,"ASM Disable Rx for usid=0x%x, device=%d, tech=%d, band=%d exec_type=%d," 
                       " timing=%d completed in %d us", this->assigned_usid, asm_cfg_p->rfm_device, asm_cfg_p->mode, 
                       asm_cfg_p->band.lte_band, execution_type, script_timing,rf_time_get_elapsed(start_tick,RF_USEC));
                                        ...
                                        int16 script_timing)
{
  ...
  if( !(this->rx_tx_vote[asm_cfg_p->mode][RFDEVICE_ASM_TX_0] & ( ((uint64)0x01) << asm_cfg_p->band.lte_band )))
  {
    RFDEVICE_LOG_MSG_3(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_HIGH, 
                       "ASM Disable_tx sequence anomaly since enable_tx was not called for the ASM usid=0x%x, tech %d, band %d",
                       this->assigned_usid, asm_cfg_p->mode, asm_cfg_p->band.lte_band);

  }

  /*reset the bit for specified band/mode/TX */
  this->rx_tx_vote[asm_cfg_p->mode][RFDEVICE_ASM_TX_0] = this->rx_tx_vote[asm_cfg_p->mode][RFDEVICE_ASM_TX_0] & 
                                                          (~( ((uint64)0x01) << asm_cfg_p->band.lte_band ));

  /* Update the LTE Tx (CA) vote */
  if (asm_cfg_p->mode==RFCOM_LTE_MODE || asm_cfg_p->mode==RFM_LTE_FTM_MODE)
  {
    this->lte_ca_vote[asm_cfg_p->rfm_device][RFDEVICE_ASM_TX_0] = this->lte_ca_vote[asm_cfg_p->rfm_device][RFDEVICE_ASM_TX_0] &
                                                                    (~( ((uint64)0x01) << asm_cfg_p->band.lte_band ));

    /* update the TX rx_tx_vote for LTE using the lte_ca_vote, since rx_tx_vote does not track RFM device */
    for ( int i=0; i<RFM_MAX_WAN_DEVICES; i++) 
    {
        /* make sure the TX rx_tx_vote tracks CA, CA vote uses same band based bit mask */
        this->rx_tx_vote[asm_cfg_p->mode][RFDEVICE_ASM_TX_0] = this->rx_tx_vote[asm_cfg_p->mode][RFDEVICE_ASM_TX_0] | 
                                                               this->lte_ca_vote[i][RFDEVICE_ASM_TX_0];
    }
  ...
  {
    RFDEVICE_LOG_MSG_3(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_HIGH, 
                       "ASM disable_tx cannot be executed since ASM is used for RX, ASM usid=0x%x, tech %d, band %d",
                       this->assigned_usid,asm_cfg_p->mode, asm_cfg_p->band.lte_band);
    return TRUE;
  ...
  for ( int i=0; i<RFM_MAX_WAN_DEVICES; i++) 
  {
    if ( this->lte_ca_vote[i][RFDEVICE_ASM_TX_0] && !(this->separate_regs_for_ca) )
    {
      RFDEVICE_LOG_MSG_6(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_HIGH, "ASM disable_tx cannot be executed as there are active LTE techs for ASM" 
                         " usid=0x%x tech %d, band %d lte_ca_vote=0x%lx%08lx for device=%d",
                         this->assigned_usid, asm_cfg_p->mode, asm_cfg_p->band.lte_band,(this->lte_ca_vote[i][RFDEVICE_ASM_TX_0] >> 32), 
                         (this->lte_ca_vote[i][RFDEVICE_ASM_TX_0] & 0xffffffff), i);
  ...
  {
    /* TIME PROFILING  */
    RFDEVICE_LOG_MSG_7(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_HIGH,"ASM Disable tx for usid=0x%x, tech=%d, band=%d, device=%d exec_type=%d, timing=%d completed in %d us",
                       this->assigned_usid, asm_cfg_p->mode, asm_cfg_p->band.lte_band, asm_cfg_p->rfm_device,  execution_type, script_timing,rf_time_get_elapsed(start_tick,RF_USEC));
  }
...
//Validity check for requested ASM settings
  ...
  /*==========================================================================*/
  /* DO NOT SLEEP IF ASM IS STILL IN USE                                      */
  /*==========================================================================*/

  for(uint8 device_idx = 0; device_idx < RFM_MAX_WAN_DEVICES; device_idx++)
  {
    is_sv_vote_active |= sv_vote[device_idx];
    if(is_sv_vote_active == TRUE)
    {
      break;
    }
  }
  ...
  /*==========================================================================*/
  /* UPDATE SLEEP/WAKEUP VOTE                                                 */
  /*==========================================================================*/

  /*reset the bit for specified band/mode */
  this->sleep_wakeup_vote[asm_cfg_p->mode][asm_cfg_p->asm_rx_tx] = 
    this->sleep_wakeup_vote[asm_cfg_p->mode][asm_cfg_p->asm_rx_tx] & 
                                                                   (~( ((uint64)0x01) << asm_cfg_p->band.lte_band ));

    RFDEVICE_LOG_MSG_8(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_HIGH, 
                     "ASM sleep(): , usid=0x%x, device %d, RX sleep_wakeup_vote = 0x%lx%08lx "
                     "TX sleep_wakeup_vote = 0x%lx%08lx is_sv_vote_active = %d for mode %d",
                     this->assigned_usid, asm_cfg_p->rfm_device, 
                     (this->sleep_wakeup_vote[asm_cfg_p->mode][RFDEVICE_ASM_RX_0] >> 32), 
                     (this->sleep_wakeup_vote[asm_cfg_p->mode][RFDEVICE_ASM_RX_0] & 0xffffffff),
                     (this->sleep_wakeup_vote[asm_cfg_p->mode][RFDEVICE_ASM_TX_0] >> 32), 
                     (this->sleep_wakeup_vote[asm_cfg_p->mode][RFDEVICE_ASM_TX_0] & 0xffffffff), is_sv_vote_active, asm_cfg_p->mode ); 
  

  if (asm_cfg_p->mode==RFCOM_LTE_MODE || asm_cfg_p->mode==RFM_LTE_FTM_MODE)
  {
    /* update the sleep_wakeup_vote for LTE, since sleep_wakeup_vote does not track RFM device it may
     * get reset in intra-band non-contiguous CA
     */
    for ( int i=0; i<RFM_MAX_WAN_DEVICES; i++) 
    {
        /* make sure the sleep_wakeup_vote tracks CA, CA vote uses same band based bit mask */
        this->sleep_wakeup_vote[asm_cfg_p->mode][asm_cfg_p->asm_rx_tx] = 
          this->sleep_wakeup_vote[asm_cfg_p->mode][asm_cfg_p->asm_rx_tx] | 
                                                                         this->lte_ca_vote[i][asm_cfg_p->asm_rx_tx];
    }
  }

  if( is_sv_vote_active || this->rx_tx_vote[asm_cfg_p->mode][RFDEVICE_ASM_RX] || 
      this->rx_tx_vote[asm_cfg_p->mode][RFDEVICE_ASM_TX] )
  {
    RFDEVICE_LOG_MSG_4(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_HIGH, 
                       "ASM sleep cannot be executed as there are active techs using the ASM  usid=0x%x asm_cfg_p->mode %d, rx_tx_vote[rx] :%d, rx_tx_vote[tx]:%d",
             this->assigned_usid,asm_cfg_p->mode, this->rx_tx_vote[asm_cfg_p->mode][RFDEVICE_ASM_RX], this->rx_tx_vote[asm_cfg_p->mode][RFDEVICE_ASM_TX]);
    return TRUE;
  } 
  ...
  /*==========================================================================*/
  for ( int i=0; i<RFM_NUM_MODES; i++) 
  {
    if ( this->sleep_wakeup_vote[i][RFDEVICE_ASM_RX_0] || this->sleep_wakeup_vote[i][RFDEVICE_ASM_TX_0]) 
    {
      RFDEVICE_LOG_MSG_7(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_HIGH, 
                         "ASM sleep cannot be executed because there are active techs, usid=0x%x, device %d, RX sleep_wakeup_vote = 0x%lx%08lx "
                         "TX sleep_wakeup_vote = 0x%lx%08lx for mode %d",
                         this->assigned_usid, asm_cfg_p->rfm_device, 
                         (this->sleep_wakeup_vote[i][RFDEVICE_ASM_RX_0] >> 32), 
                         (this->sleep_wakeup_vote[i][RFDEVICE_ASM_RX_0] & 0xffffffff),
                         (this->sleep_wakeup_vote[i][RFDEVICE_ASM_TX_0] >> 32), 
                         (this->sleep_wakeup_vote[i][RFDEVICE_ASM_TX_0] & 0xffffffff), i );
      return TRUE;
    }
  }
  
  /* WWAN/WLAN Antenna sharing */
  if ( this->wlan_port_override != 0xFF )
  {
      /* Program the ASM to the WLAN switch position before sleep */
      rfdevice_asm_cfg wlan_cfg;
      wlan_cfg.rfm_device = RFM_DEVICE_0;
      wlan_cfg.mode = RFCOM_WLAN_MODE;
      wlan_cfg.asm_rx_tx = RFDEVICE_ASM_RX_0;
      wlan_cfg.asm_cell = RFDEVICE_ASM_CELL_0;
      wlan_cfg.trigger_type = RFDEVICE_ASM_TRIGGER_ENABLE_RX;
      
      if  ( !this->config(&wlan_cfg, TRUE, script_buffer, execution_type, script_timing ) )
      {
        MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"Set WLAN port failed!", 0);
      }
      else
      {
        RFDEVICE_LOG_MSG_3(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_HIGH, 
                           "Setting ASM switch to WLAN port, port=%d usid=0x%x asm_cfg_p->mode %d",
                           this->wlan_port_override, 
                           this->assigned_usid,
                           asm_cfg_p->mode);
      }
  }
  ...
  if(status)
  {
    ...
    if (num_trans < RFDEVICE_ASM_MAX_REG_SIZE)
    {
      status &= rffe_prepare_buffer(script_buffer, 
                                    this->rffe_script,
                                    execution_type, 
                                    this->rffe_bus,
                                    this->assigned_usid, 
  ...
  {
    print_device_info();
    RFDEVICE_LOG_MSG_5(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_ERROR, 
                       "No Sleep script support! (ASM usid 0x%x, device=%d, tech=%d, band=%d, timing=%d",
                       this->assigned_usid, 
                       asm_cfg_p->rfm_device, 
                       asm_cfg_p->mode, 
                       asm_cfg_p->band.lte_band, 
                       script_timing);
  return TRUE;
  ...
  if (status == FALSE)
  {
    ...
    MSG_3(MSG_SSID_RF, MSG_LEGACY_ERROR,
          "Failed to sleep ASM! (mode=%d, band=%d, timing=%d)",
          asm_cfg_p->mode, 
          asm_cfg_p->band.lte_band, 
          script_timing);
  }
  ...
  {
    RFDEVICE_LOG_MSG_6(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_MED, 
                       "ASM sleep for usid 0x%x, device %d, tech %d, band %d, exec_type=%d, timing %d",
                       this->assigned_usid, 
                       asm_cfg_p->rfm_device,
                       asm_cfg_p->mode, 
                       asm_cfg_p->band.lte_band, 
...
/*!
  ...
  @details
  This function can be used to pull ASM out of sleep

  @param asm_cfg_p
  pointer to asm config structure

  @param script_buffer
  pointer to address of buffer to hold script

  @param execution_type
  rf_device_execution_type to indicate write to hw immediately or prepare script
                                    ...
                                    int16 script_timing)
{
  ...
  if (asm_cfg_p->asm_rx_tx == RFDEVICE_ASM_RX_0)
  {
    this->sleep_wakeup_vote[asm_cfg_p->mode][RFDEVICE_ASM_RX_0] = 
      this->sleep_wakeup_vote[asm_cfg_p->mode][RFDEVICE_ASM_RX_0] |
      ( ((uint64)0x01) << asm_cfg_p->band.lte_band );
  }
  else if (asm_cfg_p->asm_rx_tx == RFDEVICE_ASM_TX_0)
  {
    this->sleep_wakeup_vote[asm_cfg_p->mode][RFDEVICE_ASM_TX_0] = 
      this->sleep_wakeup_vote[asm_cfg_p->mode][RFDEVICE_ASM_TX_0] |
      ( ((uint64)0x01) << asm_cfg_p->band.lte_band );
  }

  RFDEVICE_LOG_MSG_7(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_HIGH, 
                     "ASM wakeup(): , usid=0x%x, device %d, RX sleep_wakeup_vote = 0x%lx%08lx "
                     "TX sleep_wakeup_vote = 0x%lx%08lx is_sv_vote_active = %d for mode %d",
                     this->assigned_usid, asm_cfg_p->rfm_device, 
                     (this->sleep_wakeup_vote[asm_cfg_p->mode][RFDEVICE_ASM_RX_0] >> 32), 
                     (this->sleep_wakeup_vote[asm_cfg_p->mode][RFDEVICE_ASM_RX_0] & 0xffffffff),
                     (this->sleep_wakeup_vote[asm_cfg_p->mode][RFDEVICE_ASM_TX_0] >> 32), 
                     (this->sleep_wakeup_vote[asm_cfg_p->mode][RFDEVICE_ASM_TX_0] & 0xffffffff), asm_cfg_p->mode );
  

  RFDEVICE_LOG_MSG_6(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_MED, 
                     "ASM wakeup for usid 0x%x, device %d, tech %d, band %d, exec_type=%d, timing %d",
                     this->assigned_usid, 
                     asm_cfg_p->rfm_device,
                     asm_cfg_p->mode, 
                     asm_cfg_p->band.lte_band, 
  ...
  /* use port 0, wakeup script is the same for all ports */
  asm_cfg.port = 0;
  status = asm_data_ptr->settings_data_get(&asm_cfg, &asm_settings);
  ...
  else
  {
    ...
    RFDEVICE_LOG_MSG_5(LOG_RFA_DEVICE_ASM_C, MSG_LEGACY_ERROR, 
                       "No wakeup script support! (ASM usid 0x%x, device=%d, tech=%d, band=%d, timing=%d",
                       this->assigned_usid, 
                       asm_cfg_p->rfm_device, 
                       asm_cfg_p->mode, 
                       asm_cfg_p->band.lte_band, 
  ...
  if (status == FALSE)
  {
    print_device_info();
    MSG_3(MSG_SSID_RF, MSG_LEGACY_ERROR,
          "Failed to wakeup ASM! (mode=%d, band=%d, timing=%d)",
          asm_cfg_p->mode, 
          asm_cfg_p->band.lte_band, 
          script_timing);
  }
...
)
{
  ...
  if ( (mode == RFM_WLAN_MODE) && status )
  {     
    /* force switch position to WLAN port */
    rfdevice_asm_cfg asm_cfg_s;
    asm_cfg_s.rfm_device = RFM_DEVICE_0;
    asm_cfg_s.mode = RFCOM_WLAN_MODE;
    asm_cfg_s.band.lte_band = RFCOM_NUM_LTE_BANDS;
    ...
    if ( !(status = this->sleep(&asm_cfg_s, NULL,RFDEVICE_EXECUTE_IMMEDIATE, 0) ) )
    {
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"Set WLAN port failed!", 0);
    }  
  }
...
boolean rfdevice_asm_common::self_test(uint32 mfg_id, uint32 prd_id, uint32 rev_id)
{
  ...
  rev_id           &= (~RFC_NONFATAL_IF_MISSING_BIT_IND);
                                      ...
                                      int32 *data_ptr, uint8 size)
{
  ...
  if ( data_ptr == NULL || rxtx >= RFDEVICE_ASM_RXTX_MAX || rfm_device >= RFM_MAX_DEVICES)
  {
    print_device_info();
    MSG_4(MSG_SSID_RF, MSG_LEGACY_ERROR,"Invalid band_port_map_info. asm_cfg_p->mode = %d band = %d rxtx = %d, device = %d",
          mode, band.lte_band, rxtx, rfm_device );
    return status;
  }
  ...
  for (i = 1; i < size; i++)
  {
    if ( this->mfg_id == RFDEVICE_ASM_MFG_ID_GET(data_ptr[i]) &&
         (this->prd_id & 0xFF) == RFDEVICE_ASM_PRD_ID_GET(data_ptr[i]))
    {
                                    ...
                                    rfcom_band_type_u band, rfdevice_asm_rxtx_enum_type rxtx)
{
  ...
  if ( rxtx >= RFDEVICE_ASM_RXTX_MAX || rfm_device >= RFM_MAX_DEVICES )
  {
    return port;
  }
                                     ...
                                     int16 script_timing)
{
  ...
  /* At this time all supported 3rd party ASMS that support wakeup/sleep are using 
   * LPM sleep/wakeup per MIPI spec 0x1C=0x0 (wakeup), 0x1C => 0x80 (sleep) which 
   * does not require a trigger.  If future devices require a trigger for sleep/wakeup 
   * a LPM flag will be needed in the driver based upon device to enable/disable a trigger
   * for sleep/wakeup
   */
    ...
    rfdevice_mod_reg_info_type *asm_mod_reg_ag)
{
  ...
  default:
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"Invalid register type information %d", asm_mod_cfg->reg_type);
    return FALSE;
    break;
  }
