...
$Header: //components/rel/rfdevice_asm.mpss/1.32/api/rfdevice_asm_common.h#1 $
$Author: pwbldsvc $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
12/31/19   smi    Add support to get RFFE device register address list
11/17/15   dbc     Rework of voting scheme to cover more ASM sharing scenarios
09/30/15   dbc     Added ASM votes for LTE ULCA and RFM scenarios
09/27/15   sr      removed the xsw and grfc based asdiv_asm, xsw device creation prototypes. 
09/24/15   sr/mc   Added Support for mipi tuner, 3rd party XSW and ASDiv based ASM devices 
...
rfdevice_logical_component* create_gen_asm_object(rfdevice_physical_device *rfdevice_physical_third_party_p,
  ...
  private:
    ...
    virtual uint8 get_port(rfm_device_enum_type rfm_device, rfcom_mode_enum_type mode, rfcom_band_type_u band,
                           rfdevice_asm_rxtx_enum_type rxtx);

    virtual boolean set_port(rfm_device_enum_type rfm_device, rfcom_mode_enum_type mode, rfcom_band_type_u band,
