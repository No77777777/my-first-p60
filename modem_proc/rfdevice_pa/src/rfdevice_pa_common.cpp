...
$Header: //components/rel/rfdevice_pa.mpss/1.32/src/rfdevice_pa_common.cpp#1 $
$Author: pwbldsvc $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
05/29/20   vp      Added support for QPA2625
...
08/16/16   dbc     Added get_rffev20_usid_p3_setting() to program USID using
                   RFFE v2.0 P3 burst procedure
3/8/2016   wxl     [Ported]PA initialize after PA object created(for 3rd PA to save current consumption) Fix the previous version
...
boolean rfdevice_pa_program_device_id_phy(rfc_phy_device_info_type *phy_device_info)
{
  ...
  if (pa_dev != NULL)
  {
    ...
    if (pa_dev->get_rffev20_usid_p3_setting())
    {
      /* use RFFE2 burst script for 16 bit product IDs*/
      rffe_program_assigned_usid_burst(phy_device_info);
    }
    else
    {
...
rfdevice_pa_common::rfdevice_pa_common(rfdevice_physical_device *phy_dev_obj_ptr,rfc_phy_device_info_type *phy_device_info, rfc_logical_device_info_type *logical_device_info)
{
  ...
  this->rffe_bus = 0xFF;
  this->default_usid = 0xFF;
  this->assigned_usid = 0xFF;
  this->gsid = 0xFF;
...
//Validity check on requested PA settings
  ...
  //setup a default value to enable Bias regsiter existence verification
  pa_reg_ag.bias_reg_idx = RF_REG_INVALID;
  
  if((pa_params_p->mode == RFCOM_GSM_MODE) && (!pa_params_p->is_mod_type_gmsk))
  {
    pa_params_cfg.req = RFDEVICE_PA_SET_BIAS_EDGE_DATA;
    found_script = pa_data_ptr->settings_data_get(&pa_params_cfg, &pa_reg_ag);
  }
  
  if(found_script == FALSE)
  {
  ...
  if(found_script == TRUE)
  {
    ...
    if(!status)
    {
      RF_MSG(RF_ERROR, "PA Set TXAGC: Failed in preparing internal buffer with bias script");
      return status;
    }
  }
  else
  {
    RFDEVICE_LOG_MSG(LOG_RFA_DEVICE_PA_C, MSG_LEGACY_MED, "PA Set TXAGC: No Default bias settings for this device");
  }
  ...
  if (found_script == TRUE)
  {
    ...
    if(!status)
    {
      RF_MSG(RF_ERROR, "PA Set TXAGC: Failed in preparing internal buffer with Gain script");
      return status;
    }
  }
  else
  {
    RFDEVICE_LOG_MSG(LOG_RFA_DEVICE_PA_C, MSG_LEGACY_MED, "PA Set TXAGC: No Gain settings for this device");
  }
...
)
{
  ...
  if((this->mfg_id == 0x217) && (this->prd_id == 0x14F))
  {
    pa_params_cfg.port = get_port(pa_params_p->mode, pa_params_p->band, pa_params_p->is_mod_type_gmsk);
  }
  else
  {
    pa_params_cfg.port  = 0;
...
)
{
  ...
  {
    if (this->mfg_id == RFDEVICE_MFG_ID_GET(data_ptr[index]) && 
        (this->prd_id & 0xFF)== RFDEVICE_PRD_ID_GET(data_ptr[index]))
    {
...
/*----------------------------------------------------------------------------*/
boolean rfdevice_pa_common::get_max_bw_supported
(
...
{
  *max_bw_khz = pa_data_ptr->get_max_bw_supported();
...
/*!
  @brief
  Get boolean indicatiing whether the RFFE v2.0 burst procedure should be used
  to program the USID

  @param None.
  @return boolean. TRUE if the RFFE v2.0 burst procedure should be used
                   to program the USID
*/
boolean rfdevice_pa_common::get_rffev20_usid_p3_setting()
{
  rfdevice_pa_info_type pa_device_info;

  this->pa_data_ptr->device_info_get(&pa_device_info);

  return pa_device_info.rffev20_usid_p3;
}
