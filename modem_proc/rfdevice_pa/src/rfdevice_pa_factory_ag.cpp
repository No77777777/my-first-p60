...
Generated from:  4.0.82 of RFDevice_PA.xlsm
*/
...
$Header: //components/rel/rfdevice_pa.mpss/1.32/src/rfdevice_pa_factory_ag.cpp#1 $ 
...
rfdevice_pa_data* rfdevice_pa_data_create (uint16 mfg_id, uint16 prd_id, uint8 prd_rev)
{
  ...
  else if ( mfg_id ==  0x217 && prd_id == 0x14F  && prd_rev == 1)
  {
    pa_data = rfdevice_pa_qpa2625_data_ag::get_instance();
  }

  else if ( mfg_id ==  0x034E && prd_id == 0x04  && prd_rev == 0)
  {
    pa_data = rfdevice_pa_nz5596g_data_ag::get_instance();
  }
  else if ( mfg_id ==  0x034E && prd_id == 0x05  && prd_rev == 0)
  {
    pa_data = rfdevice_pa_nz5627g_data_ag::get_instance();
  }
#ifdef FEATURE_RF_HAS_TP_CARDS
  #error code not present
#endif
