...
/*===========================================================================
  ...
  $Header: //components/rel/rftech_wcdma.mpss/3.11/ftm/src/ftm_wcdma_ber.c#1 $
  $DateTime: 2021/03/09 05:08:47 $
  $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
03/08/17   ag      Interface change to pass sub-id to WL1 in BER_RSCP_REQ
11/18/15   ag      Fix KW warnings
09/22/15   ag      Skip turbo clock voting on TA
09/17/15   ka      Set turbo clock, required for FBRx 
09/14/15   ska	   Remove hardcoding of devices to get RSSI
04/29/15   ag      Setting some LOW priority messages to HIGH
...
/* For calling MCPM to set TURBO clock */
#include "mcpm_api.h"
...
#define FTM_WCDMA_BER_MCPM_UPDATE_INFO (MCPM_MCVS_VPE_UPDATE | MCPM_MCVS_CLKBUS_UPDATE | MCPM_MCVS_Q6_CLK_UPDATE)
#define FTM_WCDMA_BER_Q6_TURBO 844800
#define FTM_WCDMA_BER_VPE_TURBO 799000
...
/* Used to set TURBO clock */
mcpm_request_parms_type ftm_wmcpmdrv_request_parms;
mcpm_request_type ftm_wmcpmdrv_req;
...
void ftm_wcdma_ber_loopback_handler(uint8 wm_id)
{
  ...
  FTM_MSG_MED( "Rx LB data from wm: %d",  wm_id, 0, 0 );

  if( wm_id == 1)
  {

    FTM_MSG_HIGH( "Empty WM1", 0, 0, 0 );
  ...
  while ( dsm_item_ptr != NULL )
  {
    ...
    if ( lb_from_rlc_data != NULL )
    {
      ...
      // Loopback the data
      FTM_MSG_HIGH( "Looping back: wm: %d, size: %d",  wm_id,  lb_tx_item_size, 0 );
...
************************************************************************/
      ...
      case FTM_WCDMA_BER_RMC_DCH_SETUP_V2:
      {
        ...
        if( cmd_status != FTM_WCDMA_BER_SYNC_STATUS_GEN_FAILURE )
        {
          ...
          /* Set VPE and Q6 clock to TURBO, required for FBRx */
          ftm_wmcpmdrv_req = MCPM_WCDMA_VOICE_START_REQ;

          ftm_wmcpmdrv_request_parms.update_info |=MCPM_MCVS_SCALE_UPDATE;
          ftm_wmcpmdrv_request_parms.mcvs_request.mcvs_update_info = FTM_WCDMA_BER_MCPM_UPDATE_INFO;
          ftm_wmcpmdrv_request_parms.mcvs_request.mcvs_req_type = MCVS_FULL_REQUEST;  
          /*update the MCPM structure - (Q6, VPE) TURBO  */
          ftm_wmcpmdrv_request_parms.mcvs_request.modem_vpe_KHz= FTM_WCDMA_BER_VPE_TURBO;    
          ftm_wmcpmdrv_request_parms.mcvs_request.q6_clock_KHz= FTM_WCDMA_BER_Q6_TURBO;
         
          /*On Tabasco, Turbo mode is not required and  MCPM_MCVS_VPE_UPDATE is not supported  as well */     
          #ifdef FEATURE_THOR_MODEM
          /* Send the computed config command to MCPM */
          MCPM_Config_Modem(ftm_wmcpmdrv_req, &ftm_wmcpmdrv_request_parms);
          #endif
    ...
    if ( FTM_WCDMA_BER_STATE_DCH == ftm_wcdma_ber_current_state)
      {
	 rfcom_device_enum_type first_prx_dev = rfwcdma_data_get_first_prx_device();

         if(first_prx_dev >= RFM_MAX_WAN_DEVICES)
         {
           FTM_MSG_ERROR("FTM_WCDMA_BER_RX_LEVEL_REQ: Wrong first primary device:%d",first_prx_dev,0,0);
           break;
	 }
	
        if (rx_level_req->rx_chain == PRIMARY_RX_CHAIN)
          {
            ftm_wcdma_ber_rx_lev_rpt.rssi         = rfm_get_receive_agc_val(first_prx_dev,RFCOM_SERVINGCELL_RXAGC_RAW,RFCOM_RXAGC_IN_DB10);
            ftm_wcdma_ber_rx_lev_rpt.rx_agc_dbm10 = RF_MIN_RX_POWER_DBM*RF_RXAGC_RESOLUTION  + ( rfm_get_rx_raw_agc_val(first_prx_dev) - RFAGC_MIN_AGC_VAL);
            cmd_status   = FTM_WCDMA_BER_SYNC_STATUS_SUCCESS;
          }
        else if (rx_level_req->rx_chain == SECONDARY_RX_CHAIN)
          {
            ftm_wcdma_ber_rx_lev_rpt.rssi         = rfm_get_receive_agc_val(rfwcdma_data_get_associated_device_rx(first_prx_dev),RFCOM_SERVINGCELL_RXAGC_RAW,RFCOM_RXAGC_IN_DB10);
            ftm_wcdma_ber_rx_lev_rpt.rx_agc_dbm10 = RF_MIN_RX_POWER_DBM*RF_RXAGC_RESOLUTION  + ( rfm_get_rx_raw_agc_val(rfwcdma_data_get_associated_device_rx(first_prx_dev)) - RFAGC_MIN_AGC_VAL);
            cmd_status   = FTM_WCDMA_BER_SYNC_STATUS_SUCCESS;
          }
        else
          {
    ...
    break;
      ...
      case FTM_WCDMA_BER_RSCP_REQ:
      {
          ...
          {
             wcdma_sig_info = wcdma_get_signal_info(WCDMA_SUB_ID_0);
