...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_wcdma.mpss/3.11/ftm/src/ftm_wcdma_radio.c#1 $
...
)
{
  ...
  uint32 rflm_handle = 255;
  ftm_wcdma_scenarios_type scenario = FTM_WCDMA_DBDC;
  

/*With Rx-Tx Split Any device can be assigned to Rx/Tx. 
Enter mode now needs to be done for every device irrespective of rx or tx*/
  ...
  enter_param.device_type = device_type;

  if (device_type == RFM_WCDMA_TX_DEVICE) 
  {
  ...
  FTM_MSG_2( FTM_HIGH, "ftm_wcdma_radio_enter_mode: device %d device type %d", device, device_type);
  ...
  if(RFM_IMT_MODE == rf_mode)
  {
    ...
    /* Check if device is the PRx device on Sub 2 but not DRx device
    Also check, PRx device on Sub 1 is already tuned  */
    if ( (scenario == FTM_WCDMA_DRDS_W_PLUS_W) &&
         ...
         (device_type == RFM_WCDMA_PRIMARY_RX_DEVICE)
       )
    {
      #ifdef FEATURE_DUAL_WCDMA
      /* Call WL1 function to issue the command to FW */
        wl1_ftm_fw_ctrl_sub1(TRUE);

       FTM_MSG_2(FTM_HIGH,"ftm_wcdma_radio_enter_mode:Send sync to WL1 to start WFW on Sub 2: w mode scenario: %d, device: %d",
                 scenario, device );
      #endif
    }
...
/*@akash : QRCT Radio test will have to call rfm_power_on_off explicitly. Removing this internal call*/
      ret_val = TRUE;
    }/* if ( TRUE ==  init_ftm_env)*/
  }
  else
...
)
{
  ...
  FTM_MSG_1( FTM_HIGH, "ftm_wcdma_radio_pre_tune: dev_status=%d", 
               dev_status );
...
)
{
  ...
  if((RFWCDMA_MC_STATE_SLEEP == dev_status) && (device < RFM_MAX_WAN_DEVICES))
  {
...
)
{
  ...
  path = rfcommon_core_device_to_path(device);
  dev_status = rfwcdma_mc_get_path_state(path);
 /*@akash : Needs update after new state machine is checked-in*/

  if(RFWCDMA_MC_STATE_SLEEP != dev_status)
  {
  ...
  FTM_MSG_2( FTM_HIGH, "ftm_wcdma_radio_rx_sleep: device = %d, ret_val=%d", 
                  device, ret_val );
...
)
{
 ...
 // ftm_wcdma_set_tx(pre_tune_req_ptr->device, TRUE);
  ...
  /*Fill the params*/
  tx_params.carrier_mask = 0x1;
  tx_params.device = pre_tune_req_ptr->device;
  tx_params.txlm_handle = ftm_wcdma_txlm_buffer.buf_idx;
  tx_params.tx_channel[0] = pre_tune_req_ptr->channel;
  ftm_wcdma_set_tx_v2(pre_tune_req_ptr->device, TRUE, tx_params);
...
)
{
  ...
  rfm_device_enum_type device; 
  boolean ret_val;
  rfm_wcdma_tx_enable_params tx_params = {0};
  ...
  device = pre_tune_req_ptr->device;
	...
	FTM_MSG_2( FTM_HIGH, "ftm_wcdma_radio_tx_enable: device = %d channel=%d", 
					  tx_params.device, tx_params.tx_channel[0]);

	
    rfm_wcdma_tx_enable(&tx_params, NULL, NULL);
    ret_val = TRUE;

    FTM_MSG_2( FTM_HIGH, "ftm_wcdma_radio_tx_enable: device = %d ret_val=%d", 
                  device, ret_val );
...
)
{
  ...
  device = pre_tune_req_ptr->device;
    ...
    FTM_MSG_2( FTM_HIGH, "ftm_wcdma_radio_tx_disable: device = %d ret_val=%d", 
                  device, ret_val );
...
)
{
  ...
  /* Response Packets */
  ftm_rsp_pkt_type ftm_rsp_data;          /* Packet returned to Diag */
  rfm_wcdma_tx_wakeup_params tx_params = {0}; 
  rfm_wcdma_error_code_type dev_status = RFM_WCDMA_ERROR_NULL_STATE_DATA;
  boolean ret_val = FALSE;

  pre_tune_req_ptr = 
      (struct ftm_wcdma_pre_tune_req_s *)ftm_req_data;

  /*Fill Tx Params to do Tx wakeup*/
  tx_params.device = pre_tune_req_ptr->device;
  tx_params.txlm_handle = rfwcdma_data_get_rflm_buf_idx(tx_params.device);
  tx_params.carrier_mask = rfwcdma_data_get_carrier_mask(tx_params.device);
  tx_params.tx_channel[0] = rfwcdma_data_get_curr_chan(tx_params.device, RFCOM_SINGLE_CARRIER) ;
 /*@akash : Hardcoded path to 0. Update once the new device based state machine is available*/

  FTM_MSG_2(FTM_HIGH,"ftm_wcdma_radio_tx_wakeup: device - %d, Channel = %d",
 			tx_params.device, tx_params.tx_channel);
 
  dev_status = rfm_wcdma_tx_wakeup(&tx_params, NULL, NULL);

  FTM_MSG( FTM_HIGH, "ftm_wcdma_radio_tx_wakeup: Done" );
  
  if(dev_status == RFM_WCDMA_HEALTHY_STATE)
    {
       ret_val = TRUE;
       ftm_rsp_data.cmd = FTM_RSP_DO_LEGACY;
    }
...
)
{
 ...
 rx_agc_enabled = rfwcdma_core_util_is_rxagc_enabled(device, carrier_id);
...
/*!
  @brief
  Prep tune to specific channel. 

  @details
   This function used to do prep tune to specific channel.
...
)
{
  ...
  FTM_MSG_3(FTM_HIGH, "ftm_wcdma_radio_prep_tune: device = %d, channel 0 = %d band = %d",
  				device, tune_req_ptr->channel_0, band);
...
/*@akash : FTM concurrency manager might need an update after the new RFC format. Revisit this once available.*/
  //if((ftm_concurrency_manager_validate_operation(&wcdma_cc_params)))
  //{
    rfm_wcdma_rx_prep_tune(&tune_params, NULL, NULL);
    ret_val = TRUE;                       
  ...
  FTM_MSG_1( FTM_HIGH, "ftm_wcdma_radio_prep_tune: ret_val=%d", ret_val );
...
/*!
  @brief
  Exec tune to specific channel. 
  For now script based HO carrier tune are not supported. Hence this will fall back to legacy for higher order tunes. 
...
)
{
  ...
  FTM_MSG_3(FTM_HIGH, "ftm_wcdma_radio_exec_tune device = %d, channel 0 is %d band %d",
  	      device, tune_req_ptr->channel_0, band);
...
/*@akash : FTM concurrency manager might need an update after the new RFC format. Revisit this once available.*/
  ...
  FTM_MSG_1( FTM_HIGH, "ftm_wcdma_radio_exec_tune: ret_val=%d", ret_val );
...
/*!
  @brief
  Prep Wakeup RF devices. 

  @details
   This function used to Prep wakeup RF devices.
...
)
{
  ...
  FTM_MSG_1( FTM_HIGH, "ftm_wcdma_radio_rx_prep_wakeup: device=%d", 
					  device);

  enable_wakeup_optimization_flag = TRUE;
  /*Do Script based tune for wakeup*/
  
  if(RFWCDMA_MC_STATE_SLEEP == dev_status)
  {
    ftm_wcdma_prep_wakeup(device);
    ret_val = TRUE;
  }
  else
  {
  ...
  FTM_MSG_1( FTM_HIGH, "ftm_wcdma_radio_rx_prep_wakeup: ret_val=%d", 
                  ret_val );
...
)
{
  ...
  FTM_MSG_1( FTM_HIGH, "ftm_wcdma_radio_rx_exec_wakeup: device=%d", 
					  device);

  if(RFWCDMA_MC_STATE_RX_PREP == dev_status)
  {
    ftm_wcdma_exec_wakeup(device);
    ret_val = TRUE;
  }
  else
  {
...
/*!
  @brief
  Turn on the Vreg vote for the specified device. 

  @details
   This function used to turn on the vreg vote for the specified device.
...
{
  rfm_wcdma_error_code_type dev_status = RFM_WCDMA_ERROR_NULL_STATE_DATA;
  ftm_rsp_pkt_type ftm_rsp_data = {0};
  ftm_wcdma_scenarios_type scenario = FTM_WCDMA_DBDC;
  uint8 count_rx_tune = 0;
  rfm_device_enum_type temp_device = RFM_INVALID_DEVICE;

	PACK(struct) ftm_wcdma_power_on_s /* Input parameter structure*/
	{
	  uint8  cmd;
	  uint8  sub_sys;
	  uint16 mode;
  ...
  rfm_device_enum_type device;
  power_on_req_ptr = (struct ftm_wcdma_power_on_s *)ftm_req_data;
  rfm_wcdma_power_mgmt_param pwr_param;

  scenario = ftm_wcdma_get_current_test_scenario();
  device = power_on_req_ptr->device;
  pwr_param.device = device;
  pwr_param.on_off = TRUE;

  /* Count the number of PRx devices already tuned*/
  for (temp_device = RFM_DEVICE_0; temp_device < RFM_MAX_WAN_DEVICES ; temp_device++)
  {
    if ((rfwcdma_data_get_device_type(temp_device) == RFM_WCDMA_PRIMARY_RX_DEVICE)&&
        (rfwcdma_data_get_rf_state(temp_device) == RFWCDMA_STATE_RX))
    {
      count_rx_tune++;
    }
  }
  /* Call flow is Enter mode-> Prep Tune -> Power on -> Exec Tune -> Sleep -> Power off -> Exit mode
  So, Device state is still Enter mode for Sub 2 where Sub1 would have been Tuned*/

  /* Also, Sub 1 is enabled first followed by Sub 2*/
  if ( (scenario == FTM_WCDMA_DRDS_W_PLUS_W) &&
       (rfwcdma_data_get_device_type(device) == RFM_WCDMA_PRIMARY_RX_DEVICE) &&
       (rfwcdma_data_get_rf_state(device) == RFWCDMA_STATE_RX_PREP) &&
       (count_rx_tune == 1) // Number of Primary Rx Devices tuned should be 1 for Sub 2 Power on
      )
  {
    FTM_MSG_1( FTM_HIGH, "ftm_wcdma_radio_power_on: Sub 2 tune device = %d", device);
    pwr_param.sub_type = RFM_WCDMA_SUB_1; // Sub 2
  }

  /* For Diversity device on sub 2*/
  else if ( (scenario == FTM_WCDMA_DRDS_W_PLUS_W) &&
            (rfwcdma_data_get_device_type(device) == RFM_WCDMA_DIVERSITY_RX_DEVICE) &&
            (rfwcdma_data_get_rf_state(device) == RFWCDMA_STATE_RX_PREP) &&
            (count_rx_tune == 2) // Number of Primary Rx Devices tuned should be 2 for Sub 2 diversity tune
           )
  {
    FTM_MSG_1( FTM_HIGH, "ftm_wcdma_radio_power_on: Sub 2 tune device = %d", device);
    pwr_param.sub_type = RFM_WCDMA_SUB_1; // Sub 2
  ...
  {
    pwr_param.sub_type = RFM_WCDMA_SUB_0; // Sub 1
  }

  FTM_MSG_1( FTM_HIGH, "ftm_wcdma_radio_power_on: device = %d", device);

  dev_status = rfm_wcdma_power_on_off(&pwr_param,NULL,NULL);

  FTM_MSG_1( FTM_HIGH, "ftm_wcdma_radio_power_on: status = %d", dev_status);
...
/*!
  @brief
  Turn off the Vreg vote for the specified device. 

  @details
   This function used to turn off the vreg vote for the specified device.
...
)
{
  ...
  ftm_rsp_pkt_type ftm_rsp_data = {0};
	...
	PACK(struct) ftm_wcdma_power_off_s /* Input parameter structure*/
	{
		 uint8  cmd;
		 uint8  sub_sys;
		 uint16 mode;
  ...
  rfm_device_enum_type device;
  rfm_wcdma_power_mgmt_param pwr_param;
  power_off_req_ptr = (struct ftm_wcdma_power_off_s *)ftm_req_data;
	
  device = power_off_req_ptr->device;	

  pwr_param.device = device;
  pwr_param.on_off = FALSE;

  /* Get the correct Sub type from the state machine */
  pwr_param.sub_type = rfwcdma_data_get_subscription_type(device);

  FTM_MSG_1( FTM_HIGH, "ftm_wcdma_radio_power_off: device=%d", device);
  
  dev_status = rfm_wcdma_power_on_off(&pwr_param,NULL,NULL);
  
  FTM_MSG_1( FTM_HIGH, "ftm_wcdma_radio_power_off: status = %d", dev_status);
...
/*!
  @brief
  Decfg the specified Tx Device 

  @details
   This function used to decfg the tx on the specified Tx Device.
...
rfm_device_enum_type device;
  ...
  PACK(struct) ftm_wcdma_decfg_tx_s /* Input parameter structure*/
  {
	uint8	cmd;
	uint8	sub_sys;
	uint16 mode;
...
decfg_tx_req_ptr = (struct ftm_wcdma_decfg_tx_s *)ftm_req_data;			
device = decfg_tx_req_ptr->device;
/*@akash: decfg need to accept device as a param. Needs Vikram to add it and remove hardcode to.
                   rfm device 0''. For now just a call for compilation to pass*/
dev_status = rfm_wcdma_decfg_tx();
FTM_MSG_1( FTM_HIGH, "ftm_wcdma_radio_decfg_tx: ret_val=%d", 
						  dev_status );
...
/*!
  @brief
  De-Init the Tx on the specified device. 

  @details
   This function used to de-init the tx on the specified tx device.
...
)
{
	rfm_wcdma_error_code_type dev_status = RFM_WCDMA_ERROR_NULL_STATE_DATA;
	ftm_rsp_pkt_type ftm_rsp_data = {0};
	rfm_wcdma_tx_deinit_params tx_params = {0};
		
	  PACK(struct) ftm_wcdma_deinit_tx_s /* Input parameter structure*/
	  {
		uint8	cmd;
		uint8	sub_sys;
		uint16  mode;
	...
	deinit_tx_req_ptr = (struct ftm_wcdma_deinit_tx_s *)ftm_req_data;				
	tx_params.device = deinit_tx_req_ptr->device;	

	FTM_MSG_1( FTM_HIGH, "ftm_wcdma_radio_deinit_tx: device=%d", tx_params.device);
	
	dev_status = rfm_wcdma_tx_deinit(&tx_params, NULL, NULL);
	
	FTM_MSG_1( FTM_HIGH, "ftm_wcdma_radio_deinit_tx: ret_val=%d", dev_status );
...
/*!
  @brief
  Exit the Radio device. 

  @details
   This function used to exit the specified tx/rx device.
...
)
{
	...
	rfcom_mode_enum_type to_rfmode = RFM_PARKED_MODE;
	ftm_rsp_pkt_type ftm_rsp_data = {0};
	int32 dev_status = 0;
        ftm_wcdma_scenarios_type scenario = FTM_WCDMA_DBDC;
	
	PACK(struct) ftm_wcdma_device_exit_s /* Input parameter structure*/
	{
	  uint8   cmd;
	  uint8   sub_sys;
	  uint16  mode;
  ...
  device_exit_req_ptr = (struct ftm_wcdma_device_exit_s *)ftm_req_data;
  device = device_exit_req_ptr->device;
  scenario = ftm_wcdma_get_current_test_scenario();

  FTM_MSG_1( FTM_HIGH, "ftm_wcdma_radio_device_exit: device=%d", device);
  
  /* Send disable WFW on Sub 2 command in W+W mode only for PRx device on Sub 2*/
  if( (scenario == FTM_WCDMA_DRDS_W_PLUS_W) &&
      (rfwcdma_data_get_device_type(device) == RFM_WCDMA_PRIMARY_RX_DEVICE) &&
    ...
    )
  {
      ...
      FTM_MSG_2(FTM_HIGH,"ftm_wcdma_radio_device_exit:Send sync to WL1 to stop WFW on Sub 2: w mode scenario: %d, device: %d",
                scenario, device );
    #endif
  }
  dev_status = rfm_exit_mode(device, to_rfmode);

  FTM_MSG_1( FTM_HIGH, "ftm_wcdma_radio_device_exit: status=%d", dev_status);
  
  /* No response required, just return the original request. */
  if (dev_status == 0) /*Exit mode returns '0' for healthy state*/
  {
    ftm_rsp_data.cmd = FTM_RSP_DO_LEGACY;
  }
  else
