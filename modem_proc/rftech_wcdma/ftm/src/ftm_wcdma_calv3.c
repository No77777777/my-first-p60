...
/*===========================================================================
  ...
  $Header: //components/rel/rftech_wcdma.mpss/3.11/ftm/src/ftm_wcdma_calv3.c#1 $
...
05/16/16   ag      Disable NBEE cal variable during composite cal exit for following serial cal to work
01/19/16   ak      Changes to Fix the DC Calibration offset
12/03/15   ak      Changes to support Rx/Tx Split 
11/18/15   ag      Fix KW warnings
09/17/15   kma     Added auto pin support for Pin cal
09/04/15   joz     fix WCDMA Cal failures on SSKU card (on 8996 MTP) due to negative M-Line compression
08/12/15   cac     Support for FBRx Gain Agc
08/05/15   ak      Fix for Tx device hopping during Cal and Char
07/27/15   ljz     Add device index for ftm set pa state
07/14/15   ska	   Update the correct band info for remapper API
6/29/15    vbh     populate PRx device param for enter mode
06/13/15   ag      Fix compiler warnings
06/03/15   vbh     Use the Tx params from state machine to set Tx OFF- needed for Tx device hopping
05/11/15   ak      [Rx-Tx Split]DRX Cal Fixes
05/08/15   ak      [Rx-Tx Split]Fixes to pass correct LNA and DVGA offsets to tools
05/08/15   ak      [Rx-Tx Split]Add Support to Cal 6 Rx Devices
...
04/24/15   ak      [Rx-Tx Split]Changes to Support Cal with the new Split Design
04/15/15   ak      [Rx-Tx Split]Changes to Support Cal with the new Split Design
4/15/15    vbh     [Rx/Tx split] Call set PA range with device
...
04/09/15   vbh     Enter mode call update
03/16/15   sd      Fix for NBEE cal failures on Dev 2 and 3
...
02/22/15   xs      Add support of disable NBEE() mode in ftm_wcdma_calv3_radio_setup API
02/09/15   ak      Calculate the DVGA offsets correctly when NBEE is disabled and for Serial Cal
01/14/15   aa      Fix FBRx status check due to new FBRx error codes added
...
/*----------------------------------------------------------------------------*/
static rftx_pdm_limit_type ftm_wcdma_calv3_pdm_limits = {0, 100}; /*!< Tx PDM 
min and max limits */  
...
rfcom_wcdma_band_type curr_cal_band = RFCOM_BAND_INVALID;
/*This is needed to keep the band information required for other Cal APIs.
     Will will moved to FTM state machine*/
...
)
{
  ...
  /*Radio setup should have been called. So we should be having this info*/
  source_device_info.band = curr_cal_band; 
  source_device_info.rxtx = RFC_CONFIG_RX;
  source_device_info.tech = RFM_IMT_MODE;
  source_device_info.alt_path = 0; 
  rfm_device_enum_type remapped_device;
  ...
  */
  ...
  {
   	source_device_info.device = RFM_DEVICE_0;
	/* Step (1.b) : Remapping the Rx Device to the new RFM device for the new cards with the new RFC format*/
	rfc_common_get_remapped_device_info(&source_device_info, &remapped_device_info);
	/*Override the device to the remapped device for rest of the API to use it accordingly*/
	remapped_device = remapped_device_info.device;	
    /* Check if it is DVGA cal */
    if(rx_config->cal_action == RX_ACTION_LNA0_CAL)
    {
      ftm_wcdma_get_dvga_gain_offset_setup(remapped_device);
    }
    else
    {
      ftm_wcdma_get_lna_offset_setup(remapped_device, lna_range);
    }
  }
  ...
  {
   	source_device_info.device = RFM_DEVICE_1;
	/* Step (1.b) : Remapping the Rx Device to the new RFM device for the new cards with the new RFC format*/
	rfc_common_get_remapped_device_info(&source_device_info, &remapped_device_info);
	/*Override the device to the remapped device for rest of the API to use it accordingly*/
	remapped_device = remapped_device_info.device;	  
  ...
  {
   	source_device_info.device = RFM_DEVICE_2;
	/* Step (1.b) : Remapping the Rx Device to the new RFM device for the new cards with the new RFC format*/
	rfc_common_get_remapped_device_info(&source_device_info, &remapped_device_info);
	/*Override the device to the remapped device for rest of the API to use it accordingly*/
	remapped_device = remapped_device_info.device;	  
  ...
  {
   	source_device_info.device = RFM_DEVICE_3;
	/* Step (1.b) : Remapping the Rx Device to the new RFM device for the new cards with the new RFC format*/
	rfc_common_get_remapped_device_info(&source_device_info, &remapped_device_info);
	/*Override the device to the remapped device for rest of the API to use it accordingly*/
	remapped_device = remapped_device_info.device;	  
  ...
  /*Adding Support for 6 Rx devices.*/

  /* Setup for DRx */
  if(rx_config->device_flag & DEVICE_FLAG_4)
  {
   	source_device_info.device = RFM_DEVICE_4;
	/* Step (1.b) : Remapping the Rx Device to the new RFM device for the new cards with the new RFC format*/
	rfc_common_get_remapped_device_info(&source_device_info, &remapped_device_info);
	/*Override the device to the remapped device for rest of the API to use it accordingly*/
	remapped_device = remapped_device_info.device;	  
  ...
  /* Setup for DRx */
  if(rx_config->device_flag & DEVICE_FLAG_5)
  {
   	source_device_info.device = RFM_DEVICE_5;
	/* Step (1.b) : Remapping the Rx Device to the new RFM device for the new cards with the new RFC format*/
	rfc_common_get_remapped_device_info(&source_device_info, &remapped_device_info);
	/*Override the device to the remapped device for rest of the API to use it accordingly*/
	remapped_device = remapped_device_info.device;  
...
{  
  ...
  ovr_params.device = rfwcdma_data_get_tx_device();
...
boolean ftm_wcdma_calv3_sweep_tear_down_radio(void)
{
  ...
  prof_start_time = timetick_get();
  rfm_device_enum_type rfm_device = RFM_MAX_WAN_DEVICES;
  rfm_device_enum_type paired_rx_device = RFM_MAX_WAN_DEVICES;  
  ...
  /*Set Tx will be passed only init params. Enable/Disable Tx will be called internally deriving the enable/disable params from init params*/
  rfm_wcdma_power_mgmt_param pwr_param = {0};
  ftm_wcdma_disable_NBEE();
  
  FTM_MSG_1(FTM_HIGH, "WCDMA CalV3 DBG - ftm_wcdma_calv3_sweep_tear_down_radio: elapse time %d",0 );

  //return TRUE;			

  
  tx_i_params.carrier_mask = 0x01;
  tx_i_params.device = rfwcdma_data_get_tx_device();
  tx_i_params.txlm_handle = ftm_wcdma_txlm_buffer.buf_idx;
  tx_i_params.tx_channel[0] = rfwcdma_data_get_curr_chan(tx_i_params.device, RFCOM_SINGLE_CARRIER);
  if(ftm_wcdma_tx_on_status == TRUE)
  {
  ftm_wcdma_set_tx_v2(rfwcdma_data_get_tx_device(),FALSE,tx_i_params);   /* Disable Tx */
  /* Reset PDM to lowest */
  ftm_wcdma_set_pdm(FTM_PDM_TX_AGC_ADJ, (int2)ftm_wcdma_calv3_pdm_limits.min_pdm+1);
  }

  /*Tear Down Rx : Put all active Rx devices to sleep*/
  for(rf_device = RFM_MAX_WAN_DEVICES - 1; rf_device >= RFM_DEVICE_0; rf_device--)
  {
  counter++;
  FTM_MSG_1(FTM_HIGH, "WCDMA CalV3 DBG - ftm_wcdma_calv3_sweep_tear_down_radio: counter  %d",counter );
	 if((rfwcdma_data_get_rf_state(rf_device) != RFWCDMA_STATE_RX_SLEEP)
	 	&&((rfwcdma_data_get_power_mask() & (1<<rf_device))!=0)/*Disable All active Devices*/
		&&(rfwcdma_data_get_device_type(rf_device)!=RFM_WCDMA_TX_DEVICE)) /*Enter only for Rx. Check for rx type inside*/
	 {		
		if(rfwcdma_data_get_device_type(rf_device) == RFM_WCDMA_PRIMARY_RX_DEVICE)
		{
		  /*Tear Down Master Devices*/
		  memset(&sleep_params, 0, sizeof(rfm_wcdma_rx_sleep_params)); 
  		  sleep_params.api_params[rf_device].is_valid = TRUE;
          sleep_params.api_params[rf_device].rxlm_handle = ftm_wcdma_rxlm_buffer[rf_device].buf_idx;
          sleep_params.api_params[rf_device].skip_agc_disable = FALSE; 
          rfm_wcdma_rx_sleep(&sleep_params,NULL,NULL);
		  pwr_param.device = rf_device;
		  pwr_param.on_off = FALSE;
		  pwr_param.sub_type = RFM_WCDMA_SUB_0;
		  rfm_wcdma_power_on_off(&pwr_param,NULL,NULL);
		}
		else if((rfwcdma_data_get_device_type(rf_device) == RFM_WCDMA_DIVERSITY_RX_DEVICE)
	 	&&((rfwcdma_data_get_power_mask() & (1<<rf_device))!=0))/*Disable All active Devices*/
		 /*Enter only for Rx. Check for rx type inside*/			/*Device is RFM_WCDMA_DIVERSITY_RX_DEVICE*/
		{
                  /*Tear Down slave devices*/
                  ftm_wcdma_set_secondary_chain(rf_device, FALSE);
                 //ftm_rfmode_exit(rf_device,FTM_STATE_PARK);
		}
		  
     	  rfm_exit_mode(rf_device,RFM_PARKED_MODE);
	  }
   }

 
  #if 0 /*@akash : Remove this after BU. Keeping this now for reference*/
  /* Sleep Rx on Dev2,3 */
  ...
  curr_cal_band = RFCOM_BAND_INVALID;

  /*@akash : Check with dimeng if exit mode is needed in tear down routine or "radio setup with mdev disabled" will take care of it*/
...
)
{
  ...
  rfwcdma_core_get_freq_from_ul_arfcn( rfwcdma_data_get_curr_chan(device, RFCOM_SINGLE_CARRIER));
...
)
{
  ...
  while ( dev_mask != 0 )
  {
    ...
    device = (rfm_device_enum_type)(
     ...
     /*Remap the rfm device to the new rfm device to support old cards without params change after Rx-Tx Split*/
 /*=================================================================================================================*/
     remapped_rx_device = ftm_wcdma_get_remapped_rx_device(device);
     remapped_device = ftm_wcdma_get_remapped_tx_device(remapped_rx_device);
    ...
    if ( rfcommon_debug_flags.use_rflm_txagc_override )
    {
      ...
      ret_val &= ftm_wcdma_xpt_set_txagc_override_params( remapped_device,
    ...
    {
    ret_val &= ftm_wcdma_xpt_override_txagc(remapped_device,
                   override_row.curr_tx_override.xpt_mode,
                   override_row.curr_tx_override.amam_addr.mem_bank_id,
                   override_row.curr_tx_override.amam_addr.start_offset,
                   override_row.curr_tx_override.amam_addr.tbl_size,
    ...
    dev_mask = rf_extract_first_one_bit_mask( dev_temp );
     ...
     if ( ftm_wcdma_calv3_dbg_msg_flag )
     {
       FTM_MSG_6( FTM_HIGH, "ftm_wcdma_calv3_sweep_tx_override: Dev %d "
                  "Tx Param 0x%x, PA %d, RGI %d, Bias %d old device %d ", 
                  remapped_device, 
                  param_mask,
                  curr_ovr_params.pa_state,
                  curr_ovr_params.rgi,
                  curr_ovr_params.papm_bias , device);
     }
  } /* while ( dev_mask != 0 ) */
...
ftm_rsp_pkt_type ftm_wcdma_calv3_configure_radio(void *ftm_req_data)
{
  ...
  /* Set Lowest PA State  */
  ftm_wcdma_set_pa_range(rfwcdma_core_util_get_pa_range_from_nv(RFM_DEVICE_0,0), RFM_DEVICE_0);
...
ftm_rsp_pkt_type ftm_wcdma_calv3_radio_setup(void *ftm_req_data)
{
  ...
  rfm_device_enum_type tx_off_dev;
  rfm_device_enum_type rfm_device = RFM_MAX_WAN_DEVICES;
  rfm_device_enum_type paired_rx_device = RFM_MAX_WAN_DEVICES;
  rfdevice_rx_mode_type rx_mode = RFDEVICE_INVALID_OPER_MODE;
  /*This is needed to support Rx-Tx Split where Tx can be on any rfm device and not necessarily tied to master device. */
  rfm_device_enum_type txdev = RFM_MAX_WAN_DEVICES; 
  ...
  rfm_wcdma_power_mgmt_param pwr_param = {0};
  rfm_wcdma_enter_mode_param tx_enter_param = {0};
  ...
  rfm_wcdma_tx_init_params tx_deinit_params = {0};
  rfc_cal_device_remap_info_type source_device_info = {0};
  rfc_cal_device_remap_info_type remapped_device_info = {0};
  ...
  /*===============================================================================*/
  /*Step:1 Get the Mapped Master Device*/
  /* Step (1.a): Fill Source Params for mapping the PRX device*/  
  source_device_info.device = mdev;
  source_device_info.band = band;
  source_device_info.rxtx = RFC_CONFIG_RX;
  source_device_info.tech = RFM_IMT_MODE;
  source_device_info.alt_path = 0; 
  
  /* Step (1.b) : Mapping the Prx device to the correct RFM device so support backward compatibility with the new RFC format*/
  rfc_common_get_remapped_device_info(&source_device_info, &remapped_device_info);

  /*Step (1.c) : Override the params from the tools with the new mapping returned from the RFC*/ 
  mdev = remapped_device_info.device;

  /*===============================================================================*/  
  /*Step 2: Get the Mapped Slave Device*/
  /* Step (2.a): Fill Source Params for mapping the DRX device*/  
  source_device_info.device = sdev;
  source_device_info.band = band;
  source_device_info.rxtx = RFC_CONFIG_RX;
  source_device_info.tech = RFM_IMT_MODE;
  source_device_info.alt_path = 0; 
  
  /* Step (2.b) : Mapping the Prx device to the correct RFM device so support backward compatibility with the new RFC format*/
  rfc_common_get_remapped_device_info(&source_device_info, &remapped_device_info);

  /*Step (2.c) : Override the params from the tools with the new mapping returned from the RFC*/ 
  sdev = remapped_device_info.device;

  /*===============================================================================*/
  /*Step 3: Get the associated Tx device to the master device*/
  txdev = rfc_common_get_preferred_associated_tx_device(mdev);
  ...
  if(num_of_chan==1)
  {
	...
	tx_init_params.tx_channel[0] = tune_ul_channel_list[0];
  ...
  else 
  {
	...
	tune_ul_channel_list[0] = tune_ul_channel_list[0];
	tune_ul_channel_list[1] = tune_ul_channel_list[1];
	tx_init_params.carrier_mask = 0x3;
    }
	...
	tune_ul_channel_list[0] = tune_ul_channel_list[0];
	tune_ul_channel_list[1] = tune_ul_channel_list[1];
	tune_ul_channel_list[2] = tune_ul_channel_list[2];
	tx_init_params.carrier_mask = 0x7;
    }
    else
    {
       ...
       ( mdev_state == MASTER_DEVICE_RXTX ) )
  {
    ...
    if( mdev < RFM_MAX_WAN_DEVICES  )
    {
      /*Loop through all the devices to disable all the already active rx devices*/
	  for(rfm_device = RFM_DEVICE_0; rfm_device < RFM_MAX_WAN_DEVICES; rfm_device++)
	    {
		  if((rfwcdma_data_get_rf_state(rfm_device) != RFWCDMA_STATE_INIT) /*Disable All active Devices*/
		  	&&(rfwcdma_data_get_device_type(rfm_device)!=RFM_WCDMA_TX_DEVICE)) /*Enter only for Rx. Check for rx mode inside*/
		  { 
            paired_rx_device = rfc_common_get_preferred_associated_rx_device(rfm_device);
			rx_mode = (rfwcdma_data_get_device_type(rfm_device)==RFM_WCDMA_PRIMARY_RX_DEVICE)? 
							RFDEVICE_PRX_OPER_MODE : RFDEVICE_DRX_OPER_MODE ;
			
			uint8 sub_id = rfwcdma_data_get_subscription_type(rfm_device);
			
			rfdevice_wcdma_rx_disable(rfm_device,
      ...
      /* Master Device has to enter as Primary device*/
      ...
      rfm_enter_mode( mdev, RFCOM_WCDMA_MODE, 
                      &enter_param, NULL, ftm_wcdma_rxlm_buffer[mdev].buf_idx);
	  /*Enable Rx. This API does nothing for now. Deprecate this after BU*/
      rfm_enable_rx(mdev, NULL, 0); 
      tx_init_params.txlm_handle = ftm_wcdma_txlm_buffer.buf_idx;
    }
    else
    {
      FTM_MSG(FTM_ERROR, "Wrong device index");
   ...
   /* Check if TX on then turn off TX */
    ...
    /*For Tx OFF use the parameters from the state machine. This is essential in case of Tx device hop
    when caling RF bands on 2 WTRs*/

    if (ftm_wcdma_tx_on_status == TRUE)
    {
      tx_off_dev = rfwcdma_data_get_tx_device();
      
      if(tx_off_dev >= RFM_MAX_WAN_DEVICES)
      {
        FTM_MSG_1(FTM_ERROR, "ftm_wcdma_calv3_radio_setup failed: Wrong tx device %d", tx_off_dev);
      }

      else
      {
        tx_deinit_params.device = tx_off_dev;
        tx_deinit_params.txlm_handle = rfwcdma_data_get_rflm_buf_idx(tx_off_dev);
        tx_deinit_params.carrier_mask = rfwcdma_data_get_carrier_mask(tx_off_dev);
        tx_deinit_params.tx_channel[0]=rfwcdma_data_get_curr_chan(tx_off_dev, RFCOM_SINGLE_CARRIER);
        ftm_wcdma_set_tx_v2(tx_off_dev,FALSE,tx_deinit_params);
      }
    }
    ...
    if(num_of_chan == 1)
    {
      ...
      if( mdev < RFM_MAX_WAN_DEVICES )
      {
        pwr_param.device = mdev;
		pwr_param.on_off = TRUE;
		pwr_param.sub_type = RFM_WCDMA_SUB_0;
		rfm_wcdma_power_on_off(&pwr_param,NULL,NULL);
		/*Turn on Vregs. RF should handle this. WL1 to remove all RF calls in FTM mode*/
    ...
    if((num_of_chan == 2)||(num_of_chan == 3))
    {
      ...
      if( mdev < RFM_MAX_WAN_DEVICES )
      {
		...
		rfm_wcdma_power_on_off(&pwr_param,NULL,NULL);
		/*Turn on Vregs. RF should handle this. WL1 to remove all RF calls in FTM mode*/      
       ftm_wcdma_tune_to_multi_chan_v2( mdev, tune_ul_channel_list);
      }
      else
      {
         FTM_MSG(FTM_ERROR, "Wrong device index");
    ...
    if(mdev_state == MASTER_DEVICE_RXTX)
    {
   	...
   	/*Ready to Enable Tx*/
	txdev = ftm_wcdma_get_remapped_tx_device(mdev);  
        tx_init_params.device = txdev;
        ftm_wcdma_set_tx_v2(txdev, TRUE, tx_init_params);        
    }
  } /* if ( ( mdev_state == MASTER_DEVICE_RX ) ||
          ( mdev_state == MASTER_DEVICE_RXTX ) ) */
  ...
  if ( mdev_state == MASTER_DEVICE_RETUNE )
  {
        ...
        && (ftm_current_rf_mode != FTM_DB_RF_WCDMA_BC19))
    {
      ...
      if( mdev < RFM_MAX_WAN_DEVICES )
      {
		...
		pwr_param.sub_type = RFM_WCDMA_SUB_0;
		rfm_wcdma_power_on_off(&pwr_param,NULL,NULL);
	/*@akash : Deprecate this API after BU*/
        rfm_enable_rx(mdev, NULL, 0); 
      }
      else
      {
	...
	/*For Slave device the type must be filled as DRX*/
       ...
       /*For secondary Direct enter mode is called instead of set secondary chain. Hence need to do explicit power ON*/
		pwr_param.device = sdev;
		pwr_param.on_off = TRUE;
		pwr_param.sub_type = RFM_WCDMA_SUB_0;		
		rfm_wcdma_power_on_off(&pwr_param,NULL,NULL);		
        /* Print debug msg if enabled */
...
)
{
 ...
 /*=================================================================================================================*/
     /*Step:1 Get the Mapped Master Device*/   
     /* Step (1.a): Fill Source Params for mapping the RX device*/ 
	source_device_info.device = device;
	/*Radio setup should have been called. So we should be having this info*/
	source_device_info.band = curr_cal_band; /*Need to Fix this. Check with Dimeng. How can we get band info at this point*/
	source_device_info.rxtx = RFC_CONFIG_RX;
	source_device_info.tech = RFM_IMT_MODE;
	source_device_info.alt_path = 0; 
  ...
  if ( (remapped_device < FTM_CAL_V3_RX_MAX_DEV) && (device < FTM_CAL_V3_RX_MAX_DEV)  )
  {   
      /* RX_MEAS_LNA_OFFSET */
 ...
 else
  {
   ...
   if(ftm_wcdma_rxcal_opt_enabled())
   {
    ...
    } /* if ( device < RFM_DEVICE_6) */
    else
    {
      ret_val = FALSE;
      FTM_MSG_2( FTM_ERROR, "ftm_wcdma_calv3_rx_measure: Dev %d or remapped_device %d "
                 "not valid", device, remapped_device );
      break;
...
)
{
  ...
  rfm_device_enum_type device; /* RFM Device */
  rfm_device_enum_type remapped_rx_device;
  rfm_device_enum_type remapped_tx_device;
  boolean ret_val = TRUE;
  ...
  while ( dev_mask != 0 )
  {
	...
	if((device < RFM_DEVICE_0) || (device >= FTM_CAL_V3_TX_MAX_DEV))
		 {
	    RF_MSG_1(RF_HIGH, "ftm_wcdma_calv3_tx_measure: device %d is Invalid", device);
            return  FALSE ;
	 }
 ...
 /*=================================================================================================================*/
    /*Step:1 Get the Mapped Master Device*/   
    /* Step (1.a): Fill Source Params for mapping the TX device*/ 
     remapped_rx_device = ftm_wcdma_get_remapped_rx_device(device);
	 remapped_tx_device = ftm_wcdma_get_remapped_tx_device(remapped_rx_device);
	
    /* Check if device of interest is valid. Device Should be in Tx state at this moment */
    if(rfwcdma_data_get_device_type(remapped_tx_device) != RFM_WCDMA_TX_DEVICE)
    {
    ...
    else
    {
           ...
           ( 1 << TX_MEAS_HDET ) ) != 0 )
      {
        ...
        for( hdet_idx = 0; hdet_idx < hdet_sample; hdet_idx++)
        {
          ...
          do
          {
            rfdevice_hdet_wcdma_read_hdet( remapped_tx_device,
                                           ftm_curr_wcdma_mode,
                                           FALSE, 
                                           &hdet_output );
           ...
           ( 1 << TX_MEAS_LINEARIZER_THERM ) ) != 0 )
      {
        ...
        rfdevice_wcdma_tx_cmd_dispatch( 
              remapped_tx_device,
              rfwcdma_data_get_curr_band(remapped_tx_device),
        ...
        ftm_cal_data_set_therm_adc( 
                 rfwcdma_data_get_curr_chan(remapped_tx_device, RFCOM_SINGLE_CARRIER), 
           ...
           ( 1 << TX_MEAS_FB_GAIN ) ) != 0 )
      {
        ...
        fb_meas_inp.tx_device_id = remapped_tx_device;
        ...
        fb_meas_inp.band.wcdma_band = rfwcdma_data_get_curr_band(remapped_tx_device);
        ...
        if ( fb_meas_res.status != RFCOMMON_FBRX_ERROR )
        {
          ...
          cal_result->tx_meas_param[device].fb_gain_meas.fb_meas_res.fbrx_mode = ftm_common_fbrx_get_mode(RFCOM_WCDMA_MODE, remapped_tx_device, 0);
           ...
           ( 1 << TX_MEAS_FB_PWR ) ) != 0 )
      {
        ...
        hdet_obj = ftm_common_get_tuner_object(remapped_tx_device, RFDEVICE_HDET);
      ...
      /* Perform FB Gain agc measurement */
      if ( ( tx_measure->tx_meas_param[device].tx_meas_action & 
           ( 1 << TX_MEAS_FB_GAIN_AGC ) ) != 0 )
      {
		ftm_common_fbrx_capture_input_v2_type fb_meas_inp_agc;
		ftm_common_fbrx_capture_result_v2_type fb_meas_res_agc;

        /* Fetch Input Params for FBRx capture */
        fb_meas_inp_agc.tx_device_id = remapped_tx_device;
        fb_meas_inp_agc.fbrx_gain_state = tx_measure->tx_meas_param[device].fb_gain_agc.fb_gain_agc_params.fb_gain_state;       
        fb_meas_inp_agc.fbrx_action = tx_measure->tx_meas_param[device].fb_gain_agc.fb_gain_agc_params.fbrx_action;
        fb_meas_inp_agc.store_capture_data = (boolean)tx_measure->tx_meas_param[device].fb_gain_agc.fb_gain_agc_params.debug_mode_en;
        fb_meas_inp_agc.tech = RFCOM_WCDMA_MODE;
        fb_meas_inp_agc.band.wcdma_band = rfwcdma_data_get_curr_band(remapped_tx_device);

        /* Call common FB Gain Measurement function */
        fb_meas_res_agc = ftm_common_fbrx_capture_and_process_v2(fb_meas_inp_agc);

        if ( ftm_wcdma_calv3_dbg_msg_flag == TRUE )
        {
          FTM_MSG_9( FTM_MED, "WCDMA CalV3 DBG - FB Gain Cal agc Res: Status %d , Gain = %d,"
                              "Residual DC = %d, RSB = %d, GI = %d, PI = %d, LS_ERR = %d, XCORR_LOG peak idx = %d, XCORR_LOG peak val = %d", 
                     fb_meas_res_agc.status, 
                     fb_meas_res_agc.results[0].gain, fb_meas_res_agc.results[0].residual_dc, fb_meas_res_agc.results[0].rsb, 
                     fb_meas_res_agc.results[0].gain_imbalance, fb_meas_res_agc.results[0].phase_imbalance, 
                     fb_meas_res_agc.results[0].ls_err, fb_meas_res_agc.results[0].xcorr_peak_idx, fb_meas_res_agc.results[0].xcorr_peak_val );
        }

        /* Check if FB Gain Measurement was a success */
        if ( fb_meas_res_agc.status != RFCOMMON_FBRX_ERROR )
        {
          /* Populate results data structure */ 
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.num_captures = fb_meas_res_agc.num_of_captures;
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_gain_state = fb_meas_res_agc.results[0].fb_gain_state;
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fbrx_mode = fb_meas_res_agc.results[0].fbrx_mode;
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_meas_stat = fb_meas_res_agc.status;
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_gain_val = fb_meas_res_agc.results[0].gain;
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_ubias = fb_meas_res_agc.results[0].ubias;
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_rxscale = fb_meas_res_agc.results[0].rxscale;
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_residual_dc = fb_meas_res_agc.results[0].residual_dc;
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_rsb = fb_meas_res_agc.results[0].rsb;
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_meas_gain_imb = fb_meas_res_agc.results[0].gain_imbalance;
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_meas_phase_imb = fb_meas_res_agc.results[0].phase_imbalance;

          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_meas_tx_tx_conj_corr = fb_meas_res_agc.results[0].tx_tx_conj_corr;
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_meas_tx_tx_corr = fb_meas_res_agc.results[0].tx_tx_corr;
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_meas_tx_sum = fb_meas_res_agc.results[0].tx_sum;

          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_meas_rx_tx_conj_corr = fb_meas_res_agc.results[0].rx_tx_conj_corr;
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_meas_rx_tx_corr = fb_meas_res_agc.results[0].rx_tx_corr;
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_meas_rx_sum = fb_meas_res_agc.results[0].rx_sum;

          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_meas_rx_rx_conj_corr = fb_meas_res_agc.results[0].rx_rx_conj_corr;
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_meas_rx_rx_corr = fb_meas_res_agc.results[0].rx_rx_corr;

		  cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_rms_signal_level = fb_meas_res_agc.results[0].rms_sig_level;
		  cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_meas_ls_err = fb_meas_res_agc.results[0].ls_err;
		  cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_max_corr_peak = fb_meas_res_agc.results[0].xcorr_peak_val;
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_max_corr_peak_idx = fb_meas_res_agc.results[0].xcorr_peak_idx;

          for( log_ind = 0; log_ind < RFLM_FBRX_XCORR_LOG_SIZE; log_ind++ )
          {
            cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_meas_xcorr_log[log_ind] = fb_meas_res_agc.results[0].xcorr_log[log_ind];
          }
        }
        else
        {
          FTM_MSG_1( FTM_ERROR, "WCDMA CalV3 DBG - ftm_wcdma_calv3_tx_measure: Dev %d "
                                "FB Gain Agc Measurement Failed", device );
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_meas_stat = 
            fb_meas_res_agc.status;
        }
        ...
        {
          FTM_MSG_1( FTM_MED, "WCDMA CalV3 DBG - ftm_wcdma_calv3_tx_measure: "
                               "FB Gain Agc Measurement Status = %d", 
          cal_result->tx_meas_param[device].fb_gain_agc_meas.fb_agc_meas_res.fb_meas_stat );
        } /* if ( tx_measure->debug_mode_en == TRUE ) */
      ...
      if ( ftm_wcdma_calv3_dbg_msg_flag == TRUE )
      {
        FTM_MSG_3( FTM_HIGH, "WCDMA CalV3 DBG - ftm_wcdma_calv3_tx_measure:"
                   " Dev %d, tx_meas_action %d, hdet_read %d", remapped_tx_device, 
                   tx_measure->tx_meas_param[device].tx_meas_action,
 ...
 */
      ...
      && (ftm_current_rf_mode != FTM_DB_RF_WCDMA_BC19))
  {
    ...
    rfm_enter_mode(RFCOM_TRANSCEIVER_0, RFCOM_WCDMA_MODE, &enter_param, NULL, ftm_wcdma_rxlm_buffer[RFM_DEVICE_0].buf_idx);
...
(uint32 rx_flag, byte lna_range, word expected_agc_val, ftm_calibration_rx_meas_data_type* result_buffer)
{
  ...
  int2 agc_read_val_dev4 = 0;     /* RFM_DEVICE_4 AGC accumulation */
  int2 agc_read_val_dev5 = 0;     /* RFM_DEVICE_5 AGC accumulation */
  ...
  if (lna_range == (byte)FTM_LNA_RANGE_0)
  {
    ...
    {
      /*All agc will run as an{n=0,1}c0 only*/
      if((rx_flag & DEVICE_FLAG_0) == DEVICE_FLAG_0)
      {
      ...
      {
        agc_read_val_dev2 += rfm_get_rx_raw_agc_val(RFM_DEVICE_2);
      }
      if((rx_flag & DEVICE_FLAG_3) == DEVICE_FLAG_3)
      {
        agc_read_val_dev3 += rfm_get_rx_raw_agc_val(RFM_DEVICE_3);
      }
      if((rx_flag & DEVICE_FLAG_4) == DEVICE_FLAG_4)
      {
        agc_read_val_dev4 += rfm_get_rx_raw_agc_val(RFM_DEVICE_4);
      }
      if((rx_flag & DEVICE_FLAG_5) == DEVICE_FLAG_5)
      {
        agc_read_val_dev5 += rfm_get_rx_raw_agc_val(RFM_DEVICE_5);
      }  
      DALSYS_BusyWait(100);
    }
    ...
    /* Compute and set DVGA gain offset for Device 4 */
    if((rx_flag & DEVICE_FLAG_4) == DEVICE_FLAG_4)
    {
      agc_read_val_dev4 = agc_read_val_dev4/FTM_NUM_OF_AVG_RX_AGC_READ;
      drx_offset = (agc_read_val_dev4 - ((int2) expected_agc_val) - rfwcdma_msm_get_dvga_offset(RFM_DEVICE_4));

      /* Range checking */
      drx_offset = RF_SATURATE(drx_offset, FTM_DVGA_GAIN_OFFSET_MIN, FTM_DVGA_GAIN_OFFSET_MAX);

      /* Set dvga gain offset */
      ftm_wcdma_set_dvga_gain_offset(RFM_DEVICE_4, drx_offset );
      ...
      if(ftm_wcdma_calv3_dbg_msg_flag)
      {
        FTM_MSG_3(FTM_HIGH,
                  "WCDMA CalV3 DBG - ftm_wcdma_calv3_get_dvga_lna_gain_offset: Rx chain = %d, Exp Rx_AGC = %d, DVGA offset = %d", 
                  DEVICE_FLAG_4, (int16)expected_agc_val, drx_offset
				  );
      }
    }	
    ...
    if((rx_flag & DEVICE_FLAG_5) == DEVICE_FLAG_5)
    {
      agc_read_val_dev5 = agc_read_val_dev5/FTM_NUM_OF_AVG_RX_AGC_READ;
      drx_offset = (agc_read_val_dev5 - ((int2) expected_agc_val) - rfwcdma_msm_get_dvga_offset(RFM_DEVICE_5));

      /* Range checking */
      drx_offset = RF_SATURATE(drx_offset, FTM_DVGA_GAIN_OFFSET_MIN, FTM_DVGA_GAIN_OFFSET_MAX);

      /* Set dvga gain offset */
      ftm_wcdma_set_dvga_gain_offset(RFM_DEVICE_5, drx_offset );
      ...
      if(ftm_wcdma_calv3_dbg_msg_flag)
      {
        FTM_MSG_3(FTM_HIGH,
                  "WCDMA CalV3 DBG - ftm_wcdma_calv3_get_dvga_lna_gain_offset: Rx chain = %d, Exp Rx_AGC = %d, DVGA offset = %d", 
                  DEVICE_FLAG_5, (int16)expected_agc_val, drx_offset
				  );
      }
    }		
  } /* DVGA gain offset operations */
  ...
  else if (lna_range < (byte)FTM_LNA_RANGE_MAX)
  {
    ...
    {
      agc_read_val_dev4 = agc_read_val_dev4/FTM_NUM_OF_AVG_RX_AGC_READ;
      drx_offset = (((int2)expected_agc_val) - agc_read_val_dev4)+rfwcdma_msm_get_dvga_offset(RFM_DEVICE_4);

       /* Set LNA offset */
      ftm_wcdma_set_lna_offset( RFM_DEVICE_4, lna_range , drx_offset);
    ...
    {
      agc_read_val_dev5 = agc_read_val_dev5/FTM_NUM_OF_AVG_RX_AGC_READ;
      drx_offset = (((int2)expected_agc_val) - agc_read_val_dev5)+rfwcdma_msm_get_dvga_offset(RFM_DEVICE_5);

       /* Set LNA offset */
      ftm_wcdma_set_lna_offset( RFM_DEVICE_5, lna_range , drx_offset);
      ...
      if(ftm_wcdma_calv3_dbg_msg_flag)
      {
        FTM_MSG_4(FTM_HIGH,
                  "WCDMA CalV3 DBG - ftm_wcdma_calv3_get_dvga_lna_gain_offset: Rx chain = %d, LNA range = %d, Exp Rx_AGC = %d, LNA offset = %d", 
                  DEVICE_FLAG_5, lna_range, (int16)expected_agc_val, result_buffer[result_count]
                  );
      }
    }	
  } /* LNA offset operations */
...
(uint32 rx_flag, byte lna_range, word expected_agc_val, ftm_calibration_rx_meas_data_type* result_buffer)
{
  ...
  static int2 dvga_offset_dev3 = 0;     /* Calculated DVGA offset for RFM_DEVICE_3*/
  static int2 dvga_offset_dev4 = 0;     /* Calculated DVGA offset for RFM_DEVICE_4*/
  static int2 dvga_offset_dev5 = 0;     /* Calculated DVGA offset for RFM_DEVICE_5*/
  ...
  int2 prx_offset_dev4 = 0;      /* Calculated LNA offset for RFM_DEVICE_4*/
  int2 prx_offset_dev5 = 0;      /* Calculated LNA offset for RFM_DEVICE_5*/
  uint32 agc_read_val_prx = 0;     /* PRx AGC accumulation */
  uint32 agc_read_val_drx = 0;     /* DRx AGC accumulation */
  uint32 agc_read_val_dev2 = 0;     /* RFM_DEVICE_2 AGC accumulation */
  uint32 agc_read_val_dev3 = 0;     /* RFM_DEVICE_3 AGC accumulation */
  uint32 agc_read_val_dev4 = 0;     /* RFM_DEVICE_4 AGC accumulation */
  uint32 agc_read_val_dev5 = 0;     /* RFM_DEVICE_5 AGC accumulation */
  ...
  if (lna_range == (byte)FTM_LNA_RANGE_0)
  {
    for (num_of_agc_reads = 0; num_of_agc_reads < ftm_num_nbee_read; num_of_agc_reads++)
    {
      if((rx_flag & DEVICE_FLAG_0) == DEVICE_FLAG_0)
      {
        source_device_info.device = RFM_DEVICE_0;
   	    rfc_common_get_remapped_device_info(&source_device_info, &remapped_device_info);
		remapped_device = remapped_device_info.device;
        agc_read_val_prx += rfwcdma_msm_read_nbpwr(remapped_device ,  ftm_wcdma_rxlm_buffer[remapped_device].buf_idx );
      }
      if((rx_flag & DEVICE_FLAG_1) == DEVICE_FLAG_1)
      {
        ...
        source_device_info.device = RFM_DEVICE_1;
   	    rfc_common_get_remapped_device_info(&source_device_info, &remapped_device_info);
		remapped_device = remapped_device_info.device;    
        agc_read_val_drx += rfwcdma_msm_read_nbpwr(remapped_device ,  ftm_wcdma_rxlm_buffer[remapped_device].buf_idx );
      }
      if((rx_flag & DEVICE_FLAG_2) == DEVICE_FLAG_2)
      {      
        source_device_info.device = RFM_DEVICE_2;
   	    rfc_common_get_remapped_device_info(&source_device_info, &remapped_device_info);
		remapped_device = remapped_device_info.device;       
        agc_read_val_dev2 += rfwcdma_msm_read_nbpwr(remapped_device ,  ftm_wcdma_rxlm_buffer[remapped_device].buf_idx );
      }
      if((rx_flag & DEVICE_FLAG_3) == DEVICE_FLAG_3)
      {
        source_device_info.device = RFM_DEVICE_3;
   	    rfc_common_get_remapped_device_info(&source_device_info, &remapped_device_info);
		remapped_device = remapped_device_info.device;         
        agc_read_val_dev3 +=rfwcdma_msm_read_nbpwr(remapped_device ,  ftm_wcdma_rxlm_buffer[remapped_device].buf_idx );
      }
      if((rx_flag & DEVICE_FLAG_4) == DEVICE_FLAG_4)
      {
        source_device_info.device = RFM_DEVICE_4;
   	    rfc_common_get_remapped_device_info(&source_device_info, &remapped_device_info);
		remapped_device = remapped_device_info.device;         
        agc_read_val_dev4 +=rfwcdma_msm_read_nbpwr(remapped_device ,  ftm_wcdma_rxlm_buffer[remapped_device].buf_idx );
      }
      if((rx_flag & DEVICE_FLAG_5) == DEVICE_FLAG_5)
      {
        source_device_info.device = RFM_DEVICE_5;
   	    rfc_common_get_remapped_device_info(&source_device_info, &remapped_device_info);
		remapped_device = remapped_device_info.device;         
        agc_read_val_dev5 +=rfwcdma_msm_read_nbpwr(remapped_device ,  ftm_wcdma_rxlm_buffer[remapped_device].buf_idx );
      }	  
    ...
    if((rx_flag & DEVICE_FLAG_0) == DEVICE_FLAG_0)
    {
      ...
      prx_dvga_offset = (   rfwcdma_msm_convert_raw_nbee_to_10dBm(agc_read_val_prx) - zero_db_gain - ((int2) expected_agc_val)) - rfwcdma_msm_get_dvga_offset(remapped_device);
    ...
    if((rx_flag & DEVICE_FLAG_1) == DEVICE_FLAG_1)
    {
      ...
      drx_dvga_offset = ( rfwcdma_msm_convert_raw_nbee_to_10dBm(agc_read_val_drx) - zero_db_gain - ((int2) expected_agc_val)) - rfwcdma_msm_get_dvga_offset(remapped_device);
    ...
    if((rx_flag & DEVICE_FLAG_2) == DEVICE_FLAG_2)
    {
      ...
      dvga_offset_dev2 = ( rfwcdma_msm_convert_raw_nbee_to_10dBm(agc_read_val_dev2) - zero_db_gain - ((int2) expected_agc_val)) - rfwcdma_msm_get_dvga_offset(remapped_device);
    ...
    if((rx_flag & DEVICE_FLAG_3) == DEVICE_FLAG_3)
    {
      ...
      dvga_offset_dev3 = ( rfwcdma_msm_convert_raw_nbee_to_10dBm(agc_read_val_dev3) - zero_db_gain - ((int2) expected_agc_val)) - rfwcdma_msm_get_dvga_offset(remapped_device);
     ...
     if(ftm_wcdma_calv3_dbg_msg_flag)
     {
	    ...
	    /* Compute and set DVGA gain offset*/
    if((rx_flag & DEVICE_FLAG_4) == DEVICE_FLAG_4)
    {
   	  source_device_info.device = RFM_DEVICE_4;
	  rfc_common_get_remapped_device_info(&source_device_info, &remapped_device_info);
      ...
      agc_read_val_dev4 = agc_read_val_dev4/ftm_num_nbee_read;

      dvga_offset_dev4 = ( rfwcdma_msm_convert_raw_nbee_to_10dBm(agc_read_val_dev4) - zero_db_gain - ((int2) expected_agc_val)) - rfwcdma_msm_get_dvga_offset(remapped_device);

      /* Range checking */
      dvga_offset_dev4 = RF_SATURATE(dvga_offset_dev4, FTM_DVGA_GAIN_OFFSET_MIN, FTM_DVGA_GAIN_OFFSET_MAX);

      /* Set dvga gain offset */
      //ftm_wcdma_set_dvga_gain_offset(RFM_DEVICE_4, dvga_offset_dev4 );

      /* Write result buffer */
      result_buffer[result_count] = (ftm_calibration_rx_meas_data_type)dvga_offset_dev4;
     ...
     if(ftm_wcdma_calv3_dbg_msg_flag)
     {
        FTM_MSG_5(FTM_HIGH,
        	  "ftm_wcdma_calv3_get_dvga_lna_gain_offset_nbpwr: Rx chain = %d, agc_read_val_drx =%d, zero_db_gain =%d, Exp Rx_AGC = %d, DRX DVGA offset = %d", 
		  DEVICE_FLAG_4, agc_read_val_dev4, zero_db_gain, (int16)expected_agc_val, dvga_offset_dev4);
	 }
    ...
    if((rx_flag & DEVICE_FLAG_5) == DEVICE_FLAG_5)
    {
      ...
      agc_read_val_dev5 = agc_read_val_dev5/ftm_num_nbee_read;

      dvga_offset_dev5 = ( rfwcdma_msm_convert_raw_nbee_to_10dBm(agc_read_val_dev5) - zero_db_gain - ((int2) expected_agc_val)) - rfwcdma_msm_get_dvga_offset(remapped_device);

      /* Range checking */
      dvga_offset_dev5 = RF_SATURATE(dvga_offset_dev5, FTM_DVGA_GAIN_OFFSET_MIN, FTM_DVGA_GAIN_OFFSET_MAX);

      /* Set dvga gain offset */
      //ftm_wcdma_set_dvga_gain_offset(RFM_DEVICE_5, dvga_offset_dev5 );

      /* Write result buffer */
      result_buffer[result_count] = (ftm_calibration_rx_meas_data_type)dvga_offset_dev5;
     ...
     if(ftm_wcdma_calv3_dbg_msg_flag)
     {
        FTM_MSG_5(FTM_HIGH,
        	  "ftm_wcdma_calv3_get_dvga_lna_gain_offset_nbpwr: Rx chain = %d, agc_read_val_drx =%d, zero_db_gain =%d, Exp Rx_AGC = %d, DRX DVGA offset = %d", 
		  DEVICE_FLAG_5, agc_read_val_dev5, zero_db_gain, (int16)expected_agc_val, dvga_offset_dev5);
	 }
  ...
  else if (lna_range < (byte)FTM_LNA_RANGE_MAX)
  {
    ...
    if((rx_flag & DEVICE_FLAG_0) == DEVICE_FLAG_0)
    {
      ...
      prx_offset = (( (int2)expected_agc_val) + prx_dvga_offset + zero_db_gain - rfwcdma_msm_convert_raw_nbee_to_10dBm(agc_read_val_prx) ) +  rfwcdma_msm_get_dvga_offset(remapped_device);
    ...
    if((rx_flag & DEVICE_FLAG_1) == DEVICE_FLAG_1)
    {
      ...
      drx_offset = (( (int2)expected_agc_val) + drx_dvga_offset + zero_db_gain - rfwcdma_msm_convert_raw_nbee_to_10dBm(agc_read_val_drx) ) +  rfwcdma_msm_get_dvga_offset(remapped_device);
    ...
    if((rx_flag & DEVICE_FLAG_2) == DEVICE_FLAG_2)
    {
      ...
      prx_offset_dev2 = (( (int2)expected_agc_val) + dvga_offset_dev2 + zero_db_gain - rfwcdma_msm_convert_raw_nbee_to_10dBm(agc_read_val_dev2) ) +  rfwcdma_msm_get_dvga_offset(remapped_device);
    ...
    if((rx_flag & DEVICE_FLAG_3) == DEVICE_FLAG_3)
    {
      ...
      agc_read_val_dev3 = agc_read_val_dev3/ftm_num_nbee_read;
      prx_offset_dev3 = (( (int2)expected_agc_val) + dvga_offset_dev3 + zero_db_gain - rfwcdma_msm_convert_raw_nbee_to_10dBm(agc_read_val_dev3) ) +  rfwcdma_msm_get_dvga_offset(remapped_device);
    ...
    if((rx_flag & DEVICE_FLAG_4) == DEVICE_FLAG_4)
    {
      ...
      agc_read_val_dev4 = agc_read_val_dev4/ftm_num_nbee_read;
      prx_offset_dev4 = (( (int2)expected_agc_val) + dvga_offset_dev4 + zero_db_gain - rfwcdma_msm_convert_raw_nbee_to_10dBm(agc_read_val_dev4) ) +  rfwcdma_msm_get_dvga_offset(remapped_device);
      
      /* Write result buffer */
      result_buffer[result_count] = (ftm_calibration_rx_meas_data_type)prx_offset_dev4;
    ...
    if((rx_flag & DEVICE_FLAG_5) == DEVICE_FLAG_5)
    {
      ...
      agc_read_val_dev5 = agc_read_val_dev5/ftm_num_nbee_read;
      prx_offset_dev5 = (( (int2)expected_agc_val) + dvga_offset_dev5 + zero_db_gain - rfwcdma_msm_convert_raw_nbee_to_10dBm(agc_read_val_dev5) ) +  rfwcdma_msm_get_dvga_offset(remapped_device);
      
      /* Write result buffer */
      result_buffer[result_count] = (ftm_calibration_rx_meas_data_type)prx_offset_dev5;
...
)
{
  ...
  {
  remapped_rx_device = ftm_wcdma_get_remapped_rx_device(payload->primary_device);
  remapped_tx_device = ftm_wcdma_get_remapped_tx_device(remapped_rx_device);  
    ...
    ret_val &= ftm_wcdma_xpt_cal_config(remapped_tx_device, 
                                         device_fb,
                                         payload->xpt_mode,
                                         TRUE );
   
    /* Initialize data module for the current channel */
    ret_val &= ftm_cal_data_init_cur_channel_map(rfwcdma_data_get_curr_chan(remapped_tx_device,RFCOM_SINGLE_CARRIER));      
...
)
{
  ...
  remapped_rx_device = ftm_wcdma_get_remapped_rx_device(payload->primary_device);
  remapped_tx_device = ftm_wcdma_get_remapped_tx_device(remapped_rx_device);
  txdev = remapped_device_info.device;
  ...
  else
  {
    ...
    { 
      ...
      if ( buff_alloc == TRUE )
      {
        ...
        iq_cap_status =  ftm_wcdma_xpt_iq_capture(remapped_tx_device,
        ...
        if ( iq_cap_status == FALSE )
        {
          ...
          {
          ftm_wcdma_xpt_cal_config( remapped_tx_device, 
                                    payload->feedback_device,
...
/*!
  @brief
  Returns the remapped Rx device
 
  @details
  This function returns the Remapped rx device given the legacy Rx Device
...
ftm_wcdma_get_remapped_rx_device
( 
  rfm_device_enum_type legacy_rx_device
)
{
  rfcom_wcdma_band_type curr_band = RFCOM_BAND_INVALID;
  rfm_device_enum_type mapped_rx_device = RFM_MAX_WAN_DEVICES;  
  ...
  temp_device = rfwcdma_data_get_first_prx_device();

  curr_band = rfwcdma_data_get_curr_band(temp_device);

  //curr_band = curr_cal_band;
  
	/*Initialize output to invalid device*/
  remapped_device_info.device = RFM_INVALID_DEVICE;
  source_device_info.device = legacy_rx_device; 
  source_device_info.tech = RFM_IMT_MODE;
  source_device_info.band = curr_band;
  source_device_info.alt_path = 0;
  source_device_info.rxtx = RFC_CONFIG_RX;
  
  rfc_common_get_remapped_device_info(&source_device_info, &remapped_device_info );
  mapped_rx_device = remapped_device_info.device;

  FTM_MSG_3(FTM_HIGH,"ftm_wcdma_get_remapped_rx_device: "
                     "Mapped Dev %d Band %d to Dev %d",
                     legacy_rx_device, curr_band, mapped_rx_device );

  return mapped_rx_device;  
}


/*!
  @brief
  Returns the remapped Tx device
 
  @details
  This function returns the Remapped Tx device given the remapped Rx Device
...
rfm_device_enum_type
ftm_wcdma_get_remapped_tx_device
(
  rfm_device_enum_type mapped_rx_device
)
{
  rfm_device_enum_type tx_device = RFM_MAX_DEVICES;

  /*Override the Tranceiver based Tx device with the actual Tx device to support Rx/Tx split
    We do assume at this point that Corresponding Rx is already tuned to give us the needed band info*/
  tx_device = ftm_common_get_preferred_tx_device(rfwcdma_data_get_curr_band(mapped_rx_device), FTM_RF_TECH_WCDMA);

  FTM_MSG_2(FTM_HIGH,"ftm_wcdma_get_remapped_tx_device: "
                     "Mapped Rx Dev %d to Tx Dev %d",
                     mapped_rx_device, tx_device);
