...
/*==============================================================================
  ...
  $Header: //components/rel/rftech_wcdma.mpss/3.11/ftm/src/ftm_wcdma_selftest.c#1 $

when       who     what, where, why
--------   ---     -------------------------------------------------------------
10/08/15   ak      KW Fixes
09/02/15   tjm/dbz Selftest Re-architecture
08/17/15   zbz     Obtain correct WCDMA band
08/03/15   zbz     Changes to support RFFE coupler
07/15/15   zbz     pass device id into ftm iq processing
07/10/15   zbz     Hardcode subsys_cmd_code in rsp pkt
...
)
{
  ...
  *ftm_rf_wcdma_payload_tx_params = &(ftm_rf_fbrx_iq_acquired_samples_data->meas_results);
  ...
  ftm_rf_fbrx_meas_tx_params_rsp_pkt->subsys_cmd_code = FTM_WCDMA_C;
...
)
{
  ...
  fbrx_gs_info.com_band.wcdma_band = ftm_curr_wcdma_mode;
...
)
{
  ...
  if (params.tx_measurement_config & FTM_FBRX_MEASURE_EVM)
  {
    ...
    if (ftm_rf_wcdma_payload_tx_params->evm_value>20000) {

      ERR_FATAL("WCDMA EVM bad %d", ftm_rf_wcdma_payload_tx_params->evm_value, 0,0);
    }
    #endif
    /*Populate EVM data*/
    memscpy(&ftm_rf_fbrx_meas_tx_params_rsp_pkt->result[payload_size_count],
...
)
{
  ...
  /*Sanity Check for TXLM handle and self test error*/
  if ((selftest_error) || (tx_handle >= RXLM_MAX_BUFFERS))
  {
    status = FTM_FBRX_TEST_GENERAL_FAILURE;
    return 1;
  }

  band = ftm_curr_wcdma_mode;
  ...
  ftm_common_selftest_get_coupler_script(RFM_IMT_MODE, com_band, params.device);

  ftm_common_set_coupler_direction(FTM_COUPLER_DIRECTION_FWD);
  ...
  if (vswr_from_rxrms)
  {
    ...
    ftm_common_set_coupler_direction(FTM_COUPLER_DIRECTION_REV); //COUPLER_DIRECTION_REVERSE);
    ftm_wcdma_selftest_capture_and_fft(
                 params_mod_for_vswr_pwr_ratio,
                 fft_size,
                 pcfl_data_size,
    ...
    ftm_common_set_coupler_direction(FTM_COUPLER_DIRECTION_FWD); //COUPLER_DIRECTION_FORWARD);
  ...
  {
    fb_modem_chain = (uint8)(rfc_wcdma_get_fb_chain(rfwcdma_data_get_tx_device(),com_band.wcdma_band));
  ...
  ftm_common_selftest_delete_coupler_script();
  // update error code
...
boolean ftm_wcdma_obtain_fbrx_gain_switchpoint_nv_item(const rfcom_wcdma_band_type band, rfnv_item_id_enum_type *nv_item)
{
  boolean ret_value = TRUE;
...
boolean ftm_wcdma_obtain_fbrx_gain_stage(uint8 * gain_stage)
{
  rfcom_wcdma_band_type band;
  rfnv_item_id_enum_type nv_id;
  boolean nv_valid;
  ...
  /* determine the correct NV item location */
  nv_valid =  ftm_wcdma_obtain_fbrx_gain_switchpoint_nv_item(band, &nv_id);
...
void ftm_wcdma_obtain_fbrx_noise_pwr(ftm_common_power_per_bin_struct_type * noise_pwr_to_subtract)
{
  rfcom_wcdma_band_type band;
  ...
  status = ftm_common_selftest_obtain_fbrx_noise_nv_item(FTM_RF_TECH_WCDMA, (uint8) band, &nv_item_to_read );
  if (status == FALSE)
  {
    memset((void *)noise_pwr_to_subtract, 0, sizeof(ftm_common_power_per_bin_struct_type));
...
boolean ftm_wcdma_obtain_fbrx_noise_nv_item(const uint8 nv_band, rfnv_item_id_enum_type * nv_item_to_write)
{
  boolean ret_value = TRUE;
