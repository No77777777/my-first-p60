...
/*===========================================================================
  ...
  $Header: //components/rel/rftech_wcdma.mpss/3.11/ftm/src/ftm_wcdma_log.c#1 $
  $DateTime: 2021/03/09 05:08:47 $
  $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/12/15   rmb     Remove hard coding RFM devices in FTM Enhanced and C1 logging.
12/12/14   rmb     Add RxAGC for Diversity in FTM Enhanced log packet.
...
void ftm_log_wcdma_agc_cb(int4 interval)
{
  ...
  prx_device = rfwcdma_data_get_first_prx_device();
  if ( !rfm_is_sleeping(prx_device) )
  {
    log_config_ptr = ftm_log_find_log_id( FTM_LOG_WCDMA_AGC);
    ...
    rfm_get_receive_agc_val((rfcom_device_enum_type)prx_device,RFCOM_SERVINGCELL_RXAGC_RAW, RFCOM_RXAGC_IN_DB10);
    ...
    ftm_agc_wcdma_data[buf].lna_state        = rfwcdma_core_util_get_lna_state(prx_device);
...
void ftm_log_wcdma_agc_enh_cb(int4 interval)
{
  ...
  if ( !rfm_is_sleeping(prx_device) )
  {
    ...
    rfm_get_receive_agc_val((rfcom_device_enum_type)prx_device,RFCOM_SERVINGCELL_RXAGC_RAW, RFCOM_RXAGC_IN_DB10);
    div_device = rfc_common_get_preferred_associated_rx_device(prx_device);
    ftm_agc_wcdma_enh_data[buf].rxagc_carr0_chain1   =
    rfm_get_receive_agc_val((rfcom_device_enum_type)div_device,RFCOM_SERVINGCELL_RXAGC_RAW, RFCOM_RXAGC_IN_DB10);
    ...
    ftm_agc_wcdma_enh_data[buf].lna_state        = rfwcdma_core_util_get_lna_state(prx_device);
...
void ftm_log_wcdma_agc_c1_cb(int4 interval)
{
  ...
  ftm_agc_wcdma_c1_data[buf].lna_state        = rfwcdma_core_util_get_lna_state(prx_device);
  ...
  ftm_agc_wcdma_c1_data[buf].lna_state_c1     = rfwcdma_core_util_get_lna_state(div_device);
