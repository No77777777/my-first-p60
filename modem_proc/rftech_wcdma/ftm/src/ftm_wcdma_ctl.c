...
/*===========================================================================
  ...
  $Header: //components/rel/rftech_wcdma.mpss/3.11/ftm/src/ftm_wcdma_ctl.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/04/20   nik     Changes to pass sub_id as param to WTR APIs to handle MSIM
01/24/20   nik     Changes for Minidump
01/20/20   nik     FR 55657: Replace RF Enter/Exit with RF Sleep/Wakeup for L+L DSDS awake 
                   - Params changes in ftm_wcdma_tune_with_lte_settings()
12/21/19   nik     Change in ftm_wcdma_wakeup() to fix tune issue in QRCT (Enter->Power->Rx Tune)
02/22/18   nik     KW error fix
10/25/17   nik     Compiler changes for RFA for migration of 6.x LLVM to 8.x
12/22/16   bg      Fix for KW errors
03/23/16   sk      Update but not write FBRX ADC settings for LTE-tune away support
02/18/16   tik     Add support for FBRx RSB changes
02/12/16   ak      Remove deprecated ftm xo cal api 
01/08/16   zhh    release the temp memory to avoid memory exhaust
12/17/15   ak      Fix the issue in Phone mode sleep command
12/03/15   ak      Changes to support Rx/Tx Split 
11/30/15   nv      Made further changes for DC-HSUPA which are required for TA modem
11/19/15   zhh    skip temp comp check if it is not in WCDMA mode
11/23/15   dw      Reset FTM RFMODE state when set secondary chain off
09/04/15   ak      Changes to Enable XO Cal on non-zero rfm device with Rx/Tx Split
09/24/15   sd      FR30081 add support for AsDiv alt gain param in device APIs
09/25/15   asn     Fix problem caused in DCHSUPA char with Tx/Rx split
09/24/15   asn     Update band information
09/24/15   ljz     Enable NBEE serial calibration for W
09/23/15   ska	   Set default coupler gain and output type
09/14/15   asn     Fic legacy Tx API calls
09/10/15   dbz     [selftest] Add RSB correction for selftest captures
09/01/15   dw      Fix DCHSUPA char with Tx/Rx split
09/02/15   tjm/dbz Modified measure Tx parameters handler
08/12/15   ak      Changes to update the Multisyth status API inline with Rx-Tx Split architecture
08/13/15   vbh     Added support for the rx meas command
08/07/15    ka     Dynamic ADC settings code cleanup
07/29/15   ljz     Add device index for ftm set pa state
...
07/10/15   ag      Set eLNA gain state 
07/08/15    JJ     FBRx ADC re-config for current WTR
06/30/15   sd      Fix in ftm_wcdma_set_mode to call power clock on after enter mode 
06/29/15   vbh     Populate PRx device param for enter mode
06/29/15   Saul    Use Tx device for fbrx-related RFC queries
06/23/15   dbc     Add ftm_wcdma_set_pa_tunable_cap()
06/22/15   joz     Use RxFE dynamic settings to update 4905 ADC registers during xPT captures 
06/09/15   ag      Fix klocwork warnings
06/5/15    dw      Fix IQ capture issue on device 4
06/03/15   dw      Added ftm_wcdma_get_cur_tx_device
06/01/15   ak      Changes to Fix the Crash during band transition during Cal
04/30/15   vkm     [selftest] Remove unused memfree
05/08/15   kr      replace FEATURE_JOLOKIA_MODEM by FEATURE_TABASCO_MODEM
05/30/15   dw      [1] Added FTM state machine
                   [2] Fix Tx and Rx FTM RF functionality
                   [3] Fix IQ capture
05/26/15   br      Remove device hardcoding during ftm_wcdma_feedback_setup
05/19/15   sd      Include user configurable DL power in sensitivity estimation 
05/15/15   ak      Addition of WCDMA Init API
05/11/15   ak      [Rx-Tx Split]Fixes to pass correct LNA and DVGA offsets to tools
05/08/15   ak      [Rx/Tx Split] Add API to get the rfcom band from the phone band type passed from tools
05/08/15   ak      [Rx/Tx Split] Changes to support Cal for 6 Rx Chain
05/06/15   ak      [Rx/Tx Split] Changes for proper WCDMA cleanup
05/12/15   vbh     Update call to antenna_tuner_program_tx_mode with new interface
05/01/15   ak      [Rx-Tx Split]Changes to Support Cal with the new Split Design
04/29/15   rmb     Remove reference to Tx DAC Cal command.
04/27/15   dw      Fix DCHSUPA FTM sequence issue
04/24/15   ak      [Rx-Tx Split]Changes to support Cal with CRAT design 
04/23/15   ak      [Rx-Tx Split]Changes to Invalidate the txfe handle while doing override txagc
04/15/15   ak      [Rx-Tx Split]Changes to Support Cal with the new Split Design
04/21/15   ac      w plus w ftm bring up changes for rxagc
4/15/15    vbh     Call get PA range with device
04/10/15   vbh     Provide additional parameters to driver interface apis
04/09/15   vbh     Enter mode, power on/off call updates
09/04/15   ak      Changes to support Rx/Tx device Split
04/04/15   ak      Rx/Tx Split Changes
03/27/15   ag      Include file to enable 3G flavor compilation
...
03/12/15   gh      Update FTM temp comp to use prep_temp_comp and exec_temp_comp
                   as oppose to do_temp_comp

03/02/15   rmb     Add channel type as UL channel to Tx params for WTR Tx Tune and
                   add new API for tuning Tx to DL channel.Also fix set_secondary_chain API 
                   for Device 3 tune in case of DR_DSDS.
03/06/15   sd      Fix KW warning
03/05/15   sd      Added interface for FTM WCDMA Sensitivity Estimation
03/02/15   rmb     Fix set_secondary_chain API for Device 3 tune in case of DR_DSDS.
03/02/15   dw      In ftm_wcdma_set_tx_multi_carrier, retune Tx BW after enable Tx
02/26/15   kr      replace FEATURE_JOLOKIA_MODEM by FEATURE_TABASCO_MODEM
02/24/15   rmb     Fix for correct conversion from UL channel to band in Multi-Carrier Tune.
02/22/15   xsh     Add ftm_wcdma_disable_NBEE API
02/05/15   dw      Replace new power on/off API call in FTM wakeup/sleep
02/09/15   ak      Calculate the DVGA offsets correctly when NBEE is disabled and for Serial Cal
02/10/15   rmb     Fix to include ftm concurrency manager cleanup in the ftm_wcdma_set_mode() 
                   and Diversity disable APIs.
02/03/15   joz     Replace hard-coded modem chain index to query from RFC
...
01/21/15   joz     changes to query RFC Path info to WTR
...
extern uint8 ftm_common_get_selftest_capture_flag(void);
extern uint8 selftest_fbrx_gain_stage;
...
static rftx_pdm_limit_type ftm_wcdma_pdm_limits = {0,100};
...
static ftm_get_multi_synth_lock_data_type ftm_wcdma_aggregated_multi_synth_lock_status[RFM_MAX_WAN_DEVICES] =
{
  {FALSE, FTM_SYNTH_STATUS_UNDETERMINED, FTM_SYNTH_STATUS_UNDETERMINED },
  {FALSE, FTM_SYNTH_STATUS_UNDETERMINED, FTM_SYNTH_STATUS_UNDETERMINED },
  {FALSE, FTM_SYNTH_STATUS_UNDETERMINED, FTM_SYNTH_STATUS_UNDETERMINED },
  {FALSE, FTM_SYNTH_STATUS_UNDETERMINED, FTM_SYNTH_STATUS_UNDETERMINED },
  {FALSE, FTM_SYNTH_STATUS_UNDETERMINED, FTM_SYNTH_STATUS_UNDETERMINED },
...
boolean ftm_wcdma_div_status[RFM_MAX_WAN_DEVICES] = { FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE };

/* FTM Tx ON indicator */
boolean ftm_wcdma_tx_on_status = FALSE;

ftm_lm_buf_type ftm_wcdma_rxlm_buffer[RFM_MAX_WAN_DEVICES] = { { 0, FALSE }, { 0, FALSE }, { 0, FALSE }, { 0, FALSE },
...
extern rfm_mode_enum_type rfm_get_current_mode( rfm_device_enum_type device);

int16 ftm_wcdma_tx_power_dbm;

extern uint32 ftm_rf_current_test_scenario;

ftm_rf_test_radio_config_unpack_data_t *ftm_rftest_dbg[MAX_RFTEST_IDX] = { NULL };

uint8 ftm_rftest_cntr = 0;

ftm_wcdma_state_type ftm_wcdma_state =
{
   RFCOM_BAND_INVALID ,
   RFCOM_BAND_INVALID ,
   RFCOM_BAND_INVALID ,
...
===========================================================================*/
void ftm_wcdma_set_uplink_waveform(void);
...
void ftm_wcdma_set_pdm(ftm_pdm_id_type pdm_id, int2 val)
{
  ...
  case FTM_PDM_TX_AGC_ADJ:
    ...
    device_ptr = rfdevice_wcdma_tx_get_common_dev_ptr(device, tx_band.wcdma_band);
    ...
    if (!ftm_wcdma_smps_pa_bias_override)
    {
      ...
      ftm_wcdma_set_smps_pa_bias_val(apt_config.default_smps_val, device);
    }
    else
    {
      /*SMPS overriding enabled, write the last overriden value*/

      ftm_wcdma_set_smps_pa_bias_val(ftm_wcdma_smps_pa_bias_override_val, device);
    }
...
SIDE EFFECTS
   None
===========================================================================*/
void ftm_wcdma_set_smps_pa_bias_val(uint16 val, rfm_device_enum_type device)
{
...
SIDE EFFECTS
   None
===========================================================================*/
void ftm_wcdma_set_pa_current(uint8 val, rfm_device_enum_type device)
{

  rfdevice_wcdma_pa_set_current_bias(device,
                                     ftm_curr_wcdma_mode,
                                     cur_pa_range,
                                     (uint16)val,
                                     NULL,
...
void ftm_wcdma_set_lna_range(rfm_device_enum_type device, byte ftm_lna_range)
{
  ...
  boolean status_set_index = TRUE;
  rfm_device_enum_type paired_dev;

  if (rfwcdma_data_get_device_type(device) == RFM_WCDMA_PRIMARY_RX_DEVICE)
  {
    lna_state.rx_mode = RFDEVICE_PRX_OPER_MODE;
  }
  else if (rfwcdma_data_get_device_type(device) == RFM_WCDMA_DIVERSITY_RX_DEVICE)
  {
    lna_state.rx_mode = RFDEVICE_DRX_OPER_MODE;
  }
  else
  {
    lna_state.rx_mode = RFDEVICE_INVALID_OPER_MODE;
  }

  paired_dev = rfwcdma_data_get_associated_device_rx(device);

  lna_state.paired_device = paired_dev;


  path = rfcommon_core_device_to_path(device);
  lna_state.band          = rfwcdma_data_get_curr_band(device);
  lna_state.device        = device;
  lna_state.carrier       = rfwcdma_data_get_num_carriers(device);
  ...
  if (ftm_wcdma_rxcal_opt_enabled() && IS_FTM_IN_WCAL_MODE())
  {
    ...
    rfdevice_wcdma_rx_lna_setup(device, rfwcdma_data_get_curr_band(device), &lna_state, RFDEVICE_EXECUTE_IMMEDIATE, buffer);

status_set_index = rfdevice_wcdma_lna_set_gain( device,
                                                    rfwcdma_data_get_curr_band(device),
                                                    buffer,
                                                    RFDEVICE_EXECUTE_IMMEDIATE,
                                                    0,
                                                    lna_state.current_state,
                                                    FALSE);
  ...
  else
  {
    ...
    if ((device < RFM_MAX_WAN_DEVICES) &&  (ftm_wcdma_rxlm_buffer[device].is_buf_allocated))
    {
...
ftm_rsp_pkt_type ftm_wcdma_set_mode(rfm_device_enum_type device, ftm_mode_id_type mode)
{
  ...
  sys_band_class_e_type sys_band = 0;
  rfm_wcdma_power_mgmt_param pwr_param;

  /*Reference to this API should be fixed now*/
  uint8 path = rfcommon_core_device_to_path(device);

  /*Override the device based on the phone mode*/
  sys_band = ftm_common_convert_ftm_mode_to_sys_band(mode);  
  if((rfc_common_check_band_supported(sys_band, device) == FALSE) && (mode != FTM_PHONE_MODE_SLEEP))
  {
    device = rfc_common_get_preferred_rx_device(sys_band);
    FTM_MSG_1(FTM_ERROR, "Device Phone mode Mismatch. Setting mode on the supported device %d"
		"Allowed only in Xo Cal sequence", device);
  }
  ...
  {
    /* Set mode will be called for Primary Rx device only*/
    ...
    /* Check if device is the PRx device on Sub 2 */
    if ( (scenario == FTM_WCDMA_DRDS_W_PLUS_W) &&
         ( device != rfwcdma_data_get_first_prx_device() ) &&
         (rfwcdma_data_get_rf_state(rfwcdma_data_get_first_prx_device())== RFWCDMA_STATE_RX)
       )
    {
      /* Call WL1 function to issue the command to WFW on Sub 2 */
      #ifdef FEATURE_DUAL_WCDMA
      wl1_ftm_fw_ctrl_sub1(TRUE);
    
      FTM_MSG_2(FTM_HIGH, "ftm_wcdma_set_mode: Send sync cmd to WL1 to start WFW on sub 2: W scenario: %d, device: %d",
                scenario, device);
      #endif

      pwr_param.sub_type = RFM_WCDMA_SUB_1; // As this is Sub 2
    }

    else
    {
      pwr_param.sub_type = RFM_WCDMA_SUB_0; // For all other cases, this is Sub 1
    }
    ...
    {
      rfm_enter_mode(device, RFCOM_WCDMA_MODE, &enter_param, NULL, ftm_wcdma_rxlm_buffer[device].buf_idx);
    }

    FTM_MSG_1(FTM_HIGH, "ftm_wcdma_set_mode, turning on RF power: device = %d", device);

    /* Turning on RF power*/
    pwr_param.device = device;
    pwr_param.on_off = TRUE;

    /* Enter mode first followed by Power On */
    (void)rfm_wcdma_power_on_off(&pwr_param, NULL, NULL);
...
void ftm_wcdma_set_tx(rfm_device_enum_type device, boolean state)
{
  ...
  if ((state == ON) && (ftm_wcdma_tx_on_status == FALSE))
  {
    ...
    /* ldo_off_flag */
    rfwcdma_core_tx_ftm_disable_ldo_off(rfwcdma_data_get_rflm_buf_idx(device), TRUE);
...
SIDE EFFECTS
  None
===========================================================================*/
void ftm_wcdma_set_pa_range(byte paRange, rfm_device_enum_type rfm_device)
{
  rfdevice_lut_idx_pa_range_type lut_pa_data;
  ...
  rfdevice_wcdma_pa_set_gain_range(rfm_device,
                                   ftm_curr_wcdma_mode,
...
/*===========================================================================
FUNCTION  FTM_WCDMA_SET_PA_TUNABLE_CAP

DESCRIPTION
  This function applies an offset to the default value for the PA Tunable Cap.
...
SIDE EFFECTS
  None
===========================================================================*/
void ftm_wcdma_set_pa_tunable_cap(rfm_device_enum_type device, byte offset)
{
  if ( !rfdevice_wcdma_set_pa_output_cap(device, ftm_curr_wcdma_mode,(int8)offset) )
  {
    FTM_MSG_ERROR("ftm_wcdma_set_pa_tunable_cap: failed for device:%d, band: %d, offset: %d", 
                  device, ftm_curr_wcdma_mode, offset);
  }
}
...
void ftm_uplink_sweep_cal_isr(void)
{
  ...
  rfm_device_enum_type device = rfwcdma_data_get_tx_device();
  ...
  /* Enable HDET */
  /*! @todo Need to get the rfm_device info from the caller function */
  rfdevice_wcdma_tx_cmd_dispatch(device, ftm_curr_wcdma_mode, RFDEVICE_ENABLE_HDET, NULL);
  ...
  for (num_of_hdet_reads = 0; num_of_hdet_reads < ftm_wcdma_num_of_avg_hdet_read; num_of_hdet_reads++)
  {
    ...
    {
      /*! @todo Need to get the rfm_device info from the caller function */
      rfdevice_wcdma_tx_cmd_dispatch(device, ftm_curr_wcdma_mode, RFDEVICE_GET_HDET_VALUE, &local_temp_hdet_val);
  ...
  /* Disable HDET */
  /*! @todo Need to get the rfm_device info from the caller function */
  rfdevice_wcdma_tx_cmd_dispatch(device, ftm_curr_wcdma_mode, RFDEVICE_DISABLE_HDET, NULL);
...
void ftm_hdet_tracking_cb(int4 interval)
{
  ...
  {
    /*! @todo Need to get the rfm_device info from the caller function */
    rfdevice_wcdma_tx_cmd_dispatch(device, ftm_curr_wcdma_mode,
...
{
  /*Keeping this API as is for the Cal validation with Split. Will move to new common Anuj's interface after valiating Cal*/
  /*For Cal validation this API suffice. Move to new Generic API after cal validation.*/
  ...
  ftm_wcdma_scenarios_type scenario = FTM_WCDMA_DBDC;
  rfc_rx_path_type rx_path = RFC_INVALID_PATH;
  ftm_concurrency_manager_params_type wcdma_cc_params;
  rfm_wcdma_enter_mode_param enter_param = { 0 };
  rfm_device_enum_type paired_rx_dev = RFM_MAX_WAN_DEVICES;
  rfm_device_enum_type tx_dev = RFM_MAX_WAN_DEVICES;
  rfcom_band_type_u band_type_u  = {0};
  rfm_device_enum_type pref_dev = RFM_INVALID_DEVICE;

  band = rfwcdma_core_convert_ul_chan_to_band(chan);
  band_type_u.wcdma_band = band;
  /*Get the preferrred device for this channel/band*/
  pref_dev = rfc_common_get_preferred_rx_device(rfcommon_core_convert_rfcom_band_to_sys_band(RFM_IMT_MODE, band_type_u));

  /*If the device on which tune is attempted on a non-supported device, flag error message and tune the channel to the supported prx device*/
  ...
  {
    device = pref_dev;
    FTM_MSG_1(FTM_ERROR, "Device Channel Mismatch. Tuning Channel on the supported primary device." 
		"Allowed only in XO Cal sequence %d", device); 
  }

  paired_rx_dev = rfc_common_get_preferred_associated_rx_device(device);
  /*Override the traceiver based Tx Device with the actual tx device*/
  tx_dev = ftm_common_get_preferred_tx_device(band, FTM_RF_TECH_WCDMA);
  ...
  if (device >= RFM_MAX_WAN_DEVICES)
  {
    ...
    if((tx_dev < RFM_DEVICE_0) || (tx_dev >= RFM_WCDMA_MAX_DEVICE))
   {
     FTM_MSG_ERROR("ftm_wcdma_tune_to_chan: Invalid tx_dev id %d", tx_dev, 0, 0);
     return;
   }
  ...
  /* This function is always called for PRx device; so check for first PRx device */

  /* Carrier index for tune - 
     - First PRx/DRx device is SC 
     - If scenario is DBDC and first PRx is Rx tuned = DBDC tune
     - If scenario is W+W and first PRx is Rx tuned = Sub 2 WCDMA Tune (Carr index = 4)
     - Not first PRx and not DBDC scenario/W+W scenario/ PRx not Rx tuned = SC
     - RFCal is always SC*/

  if (rfwcdma_data_get_first_prx_device() == device) 
  {
    carrier_index = 1;
  }
  ...
  {
    if ( (scenario == FTM_WCDMA_DBDC) && 
         (rfwcdma_data_get_rf_state(rfwcdma_data_get_first_prx_device())== RFWCDMA_STATE_RX)
       )
    {
      carrier_index = 2;
      FTM_MSG_2(FTM_HIGH, "ftm_wcdma_tune_to_chan: DBDC - device:%d, band: %d,",
                device, band);
    }
    ...
    {
      carrier_index = 4;
      FTM_MSG_2(FTM_HIGH, "ftm_wcdma_tune_to_chan: W+W - device:%d, band: %d,",
                device, band);
    }
    ...
    {
      carrier_index = 1;
      FTM_MSG_3(FTM_HIGH, "ftm_wcdma_tune_to_chan: scenario %d - device:%d, band: %d,",
                scenario, device, band);
    }

    wcdma_cc_params.carrier_index = 1;
  }
  
  if (IS_FTM_IN_WCAL_MODE()) /*For Cal : All devices should be using carrier mask '0x01'. We will stop a0c0, a1c1 and run all device agc there */
  {
    carrier_index = 0x1;
  }
  ...
  /*This may fail as the tx device would not have entered yet*/
  if (rfwcdma_data_get_rf_state(tx_dev) != RFWCDMA_STATE_INIT) 
  {
    rfwcdma_data_set_curr_chan(tx_dev, RFCOM_SINGLE_CARRIER, chan);
  }
  ...
  {
    FTM_MSG_1(FTM_ERROR, "ftm_wcdma_tune_to_chan: Invalid device:%d ", device);
  }

  if (FTM_DB_RF_SLEEP == ftm_current_rf_mode)
  {
    rfm_enter_mode(device, RFM_IMT_MODE, &enter_param, NULL, ftm_wcdma_rxlm_buffer[device].buf_idx);
  }

  /* All tune types : Higher order/DBDC should be supported by FW. Do all tunes as script based*/
  device_mask |= 1 << device;
  /*If paired Rx dev is tuned, Tune both PRX and DRX with script based tune*/
  if(!((paired_rx_dev < RFM_DEVICE_0) || (paired_rx_dev >= RFM_WCDMA_MAX_DEVICE)))
  {  
    if (ftm_wcdma_div_status[paired_rx_dev] == TRUE)
    {
      device_mask |= 1 << paired_rx_dev;
      wcdma_cc_params.rfm_dev = paired_rx_dev;
      wcdma_cc_params.band.wcdma_band = band;
  ...
  /*Both device and its paired device is tuned*/
  }
   
  else
 {
   FTM_MSG_ERROR("ftm_wcdma_tune_to_chan: Invalid paired_rx_dev id %d", paired_rx_dev, 0, 0);
  }
  ...
  /*Check with Dimeng : Not Sure Why we are doing these Tx operations here*/
  if (ftm_wcdma_tx_on_status == TRUE)
  {
    ...
    if (rfwcdma_core_xpt_is_enabled(ftm_curr_wcdma_mode))
    {
      ...
      rfdevice_wcdma_tx_set_channel(tx_dev, RFCOM_WCDMA_MODE,
  ...
  rfdevice_wcdma_asm_enable_tx(tx_dev,
                               ftm_curr_wcdma_mode,
                               NULL,
                               RFDEVICE_EXECUTE_IMMEDIATE,
                               RFCOM_INVALID_TIMING_OFFSET);

  rfdevice_wcdma_pa_on_off(tx_dev,
                           ftm_curr_wcdma_mode,
                           TRUE,
                           NULL,
                           RFDEVICE_EXECUTE_IMMEDIATE,
...
FUNCTION FTM_TX_FREQ_OVERRIDE

DESCRIPTION
  This function tunes the Tx device to the given DL channel number.

PARAMETERS
  @device
  RFM Device for Tx to be tuned

  @chan
  DL channel for Tx to be tuned to

DEPENDENCIES
  First, Tx should be enabled and tuned to a regular UL channel of the same band.
  Only then this override function is called to Tune Tx to a DL frequency
...
void ftm_wcdma_override_tx_freq(rfm_device_enum_type device, uint16 chan)
{
  /* Transceiver tx tune parameters */
  rfdevice_wcdma_tx_tune_param_type tx_params;
  tx_params.band       = rfwcdma_core_convert_chan_to_band(chan);
  tx_params.bw         = RFDEV_WCDMA_BW_INVALID;
  tx_params.dpd_state  = RFDEVICE_DPD_NUM;

  /* Check if the current band is equal to the requested band */
  if (ftm_curr_wcdma_mode != tx_params.band)
  {
    FTM_MSG_ERROR("ftm_wcdma_override_tx_freq: Cuurent Band %d, Requested band %d and channel %d",
                  ftm_curr_wcdma_mode, tx_params.band, chan);
    return;
  }

  /* Tx PLL */
  if (rfwcdma_core_xpt_is_enabled(tx_params.band))
  {
    tx_params.bw        = RFDEV_WCDMA_BW_1X;
    tx_params.channel[0] = chan;
    tx_params.ul_dl = RFWCDMA_CORE_UARFCN_DL; // This is the only place where Tx channel is overridden to DL chan
    tx_params.dpd_state = RFDEVICE_DPD_ENABLED;
    ...
    FTM_MSG_MED("wcdma tx freq override: DPD enabled.", 0, 0, 0);
  }
  else
  {
    tx_params.bw        = RFDEV_WCDMA_BW_1X;
    ...
    FTM_MSG_MED("wcdma tx freq override: DPD disabled.", 0, 0, 0);
  }

  FTM_MSG_HIGH("wcdma tx freq override to chan %d for device %d", chan, device, 0);
}
/*===========================================================================
...
void ftm_wcdma_tune_to_multi_chan(rfm_device_enum_type device, uint16 chan[])
{
  ...
  /* Transceiver tx tune parameters */
  rfdevice_wcdma_tx_tune_param_type tx_params;
  FTM_MSG_ERROR("ftm_wcdma_tune_to_chan: Entered Tune multichan %d", device, 0, 0);
...
#if 0 /*Removing this Scenario specific code. Will be added upon final discussion with Dimeng and Arun*/
...
if(!((paired_rx_dev < RFM_DEVICE_0) || (paired_rx_dev >= RFM_WCDMA_MAX_DEVICE)))

  {
     ...
     if (ftm_wcdma_div_status[paired_rx_dev] == TRUE)
      {
        ...
        wcdma_cc_params.tech = RFM_IMT_MODE; // Carrier index is already set
         ...
         rfm_tune_to_multi_chan(paired_rx_dev,
                             chan,
                             mc_flag,
                             RFM_NUM_TUNES,
                             NULL,
                             NULL,
                             ftm_wcdma_rxlm_buffer[paired_rx_dev].buf_idx);
  ...
  else
   {
    ...
    ftm_wcdma_update_aggregated_multisynth_status(tx_dev);
  }
  else
  {
...
FUNCTION FTM_WCDMA_TUNE_TO_MULTI_CHAN SCRIPT BASED
...
===========================================================================*/
void ftm_wcdma_tune_to_multi_chan_v2(rfm_device_enum_type device, uint16 chan[])
{
  uint16 tx_chan = chan[0];
  rfcom_wcdma_band_type band = 0;
  rfc_rx_path_type rx_path = 0;
  rfcom_multi_carrier_hspa_id_type mc_flag = RFCOM_DUAL_CARRIER;
  uint8 device_mask = 0, carrier_index = 3; // set the default value of carrier index for DC
  ftm_concurrency_manager_params_type wcdma_cc_params;
  ftm_wcdma_scenarios_type scenario = FTM_WCDMA_DBDC;

  /* Transceiver tx tune parameters */
  rfdevice_wcdma_tx_tune_param_type tx_params;
  FTM_MSG_ERROR("ftm_wcdma_tune_to_chan: Entered Tune multichan script based %d", device, 0, 0);
 ...
 if((tx_dev < RFM_DEVICE_0) || (tx_dev >= RFM_WCDMA_MAX_DEVICE))
      {
      FTM_MSG_ERROR("ftm_wcdma_tune_to_chan: Invalid tx_dev %d", tx_dev, 0, 0);
      return;
      }
  ...
  /* Carrier index for SBDC - 
   - First PRx device is Sub 1 SBDC 
   - If scenario is W+W and first PRx is Rx tuned = Sub 2 WCDMA Tune to DC (Carr index = 12)
   - Not first PRx and not DBDC scenario/W+W scenario/ PRx not Rx tuned = Sub 1 SBDC */

  if (rfwcdma_data_get_first_prx_device() == device)
  {
    carrier_index = 3;
    FTM_MSG_2(FTM_HIGH, "ftm_wcdma_tune_to_multi_chan_v2: DC tune - device:%d, band: %d,",
              device, band);
  }
  ...
  else
  {
    ...
    {
      carrier_index = 12;
      FTM_MSG_2(FTM_HIGH, "ftm_wcdma_tune_to_multi_chan_v2: W+W DC - device:%d, band: %d,",
                device, band);
    }
    ...
    {
      carrier_index = 3;
      FTM_MSG_3(FTM_ERROR, "ftm_wcdma_tune_to_multi_chan_v2: Unexpected scenario %d - device:%d, band: %d,",
                scenario, device, band);
    }
  }
...
if(!((paired_rx_dev < RFM_DEVICE_0) || (paired_rx_dev >= RFM_WCDMA_MAX_DEVICE)))
    ...
    ftm_wcdma_tune_to_chan_scripts(device_mask, chan, carrier_index);
...
===========================================================================*/
void ftm_wcdma_tx_rx_freq_cal_isr()
{
  ...
  if (ftm_tx_rx_freq_cal_cur_seg < ftm_tx_rx_steps_per_freq * ftm_tx_rx_num_freqs)
  {
    ...
    if ((cur_seg_within_freq == ftm_tx_rx_steps_per_freq - 1) && (device < RFM_MAX_WAN_DEVICES))
    {
      if (rfwcdma_mc_state.rx_carriers[device] == RFCOM_DUAL_CARRIER)
      {
        ...
        ftm_wcdma_tune_to_multi_chan_v2(device, channels);
      }
      else if (rfwcdma_mc_state.rx_carriers[device] == RFCOM_3_CARRIER)
      {
        channels[0] = ftm_tx_rx_freq_cal_channels[(ftm_tx_rx_freq_cal_cur_seg /
                                                   ftm_tx_rx_steps_per_freq + 1) %
                                                  ftm_tx_rx_num_freqs];
      ...
      {
        /* last step of this frequency, tune to new channel */
        ftm_wcdma_tune_to_chan(device, ftm_tx_rx_freq_cal_channels[(ftm_tx_rx_freq_cal_cur_seg /
                                                                          ftm_tx_rx_steps_per_freq + 1) %
                                                                         ftm_tx_rx_num_freqs]);
      }
    ...
    else
    {
            ...
            FTM_TX_RX_SWEEP_NO_ACTION)
      {
        ...
        ftm_wcdma_set_pa_range(ftm_tx_rx_freq_cal_pa_ranges[cur_seg_within_freq], device);
                                         ...
                                         rfm_device_enum_type device)
{
  ...
  {
    ftm_wcdma_tune_to_chan(device, ftm_tx_rx_freq_cal_channels[0]);
  }
  /* Move the set PDM to the start to avoid any false trigger */
  ftm_wcdma_set_pdm(FTM_PDM_TX_AGC_ADJ, FTM_UPLINK_SWEEP_CAL_MIN);
  ftm_wcdma_set_tx(device, TRUE);
  ftm_wcdma_set_pa_range(ftm_tx_rx_freq_cal_pa_ranges[0], device);
  ...
  )
{
 ...
 if((paired_rx_dev < RFM_DEVICE_0) || (paired_rx_dev >= RFM_WCDMA_MAX_DEVICE))
     ...
     {
  	{
  	  FTM_MSG_ERROR("ftm_wcdma_tune_to_chan: Invalid paired_rx_dev %d", paired_rx_dev, 0, 0);
          return;
        }
     }
  
  pwr_param.device = device;
  pwr_param.on_off = on_off;
  pwr_param.sub_type = rfwcdma_data_get_subscription_type(paired_rx_dev);

  /*Set Secondary Chain : This API should be called only when an associated rx has already entered as Primary*/
  /*This is must as the params to tune for this device will be fetched from its paired device state machine*/
  if (rfwcdma_data_get_device_type(paired_rx_dev) == RFM_WCDMA_PRIMARY_RX_DEVICE)
  {
    if ((on_off == TRUE) && (ftm_wcdma_div_status[device] == FALSE))
    {
      ...
      (void)rfm_enter_mode(device, RFCOM_WCDMA_MODE, &enter_param, NULL, ftm_wcdma_rxlm_buffer[device].buf_idx);

      rfm_wcdma_power_on_off(&pwr_param, NULL, NULL);

      /*Asymmetric tune on secondary chain(DRX) in terms of num carriers, band , chan etc. will not be supported*/
      for (i = 0;(i <= rfwcdma_data_get_num_carriers(paired_rx_dev))&& (i!=RFCOM_MAX_CARRIERS); i++)
      { 
        rx_channels[i] = rfwcdma_data_get_curr_chan(paired_rx_dev, (rfcom_multi_carrier_hspa_id_type)i);
      }

      /* Get the carrier mask for Diversity from Primary device as Primary device would have been tuned*/
      carrier_mask = rfwcdma_data_get_carrier_mask(paired_rx_dev);
      band =  rfwcdma_data_get_curr_band(paired_rx_dev);
      wcdma_cc_params.carrier_index = i - 1;
      device_mask = 1 << device;

      /*Scenarios can't be tagged against a device. Discuss with Dimeng*/
      /*Check and add scenario specific code here if its really required*/
      ...
      ftm_wcdma_tune_to_chan_scripts(device_mask, rx_channels, carrier_mask);
    ...
    else if ((on_off == FALSE) && (ftm_wcdma_div_status[device] == TRUE))
    {
      ...
      //ftm_rfmode_exit(device, FTM_STATE_PARK);
      rfm_exit_mode(device, RFM_PARKED_MODE);

      ftm_wcdma_reset_device(device);
...
void ftm_do_dc_cal(void)
{
  ...
  while (i < cnt)
  {
    ...
    ftm_wcdma_set_lna_range(rfwcdma_data_get_tx_device(), i % FTM_NUMBER_OF_INTERBAND_GAIN_STATE_USED);
    ftm_clk_rex_wait(3);
    i++;
  }
...
{
  lm_status_type status;
  rfm_api_status_t api_status = RFM_WCDMA_ERROR_FAILURE;

  if (ftm_wcdma_rxlm_buffer[chain].is_buf_allocated)
  {
    /* Call RFWCDMA MDSP data cleanup before de-allocate handle */
    api_status = rfm_wcdma_handle_cleanup(ftm_wcdma_rxlm_buffer[chain].buf_idx);

    if (api_status != RFM_WCDMA_HEALTHY_STATE)
    {
      RF_MSG_1(RF_ERROR,  "ftm_wcdma_deallocate_rxlm: Handle cleanup failed for RFLM handle %d", ftm_wcdma_rxlm_buffer[chain].buf_idx);
    }
...
void ftm_wcdma_deallocate_txlm(void)
{
  ...
  {
    /* Call RFWCDMA MDSP data cleanup before de-allocate handle */
    api_status = rfm_wcdma_handle_cleanup(ftm_wcdma_txlm_buffer.buf_idx);

    if (api_status != RFM_WCDMA_HEALTHY_STATE)
    {
      RF_MSG_1(RF_ERROR,  "ftm_wcdma_deallocate_txlm: Handle cleanup failed for RFLM handle %d", ftm_wcdma_txlm_buffer.buf_idx);
    }
                                   ...
                                   byte index, word expected_agc_val)
{
  ...
  if (index > 0)
  {
    ...
    current_agc_val = agc_read_val / FTM_NUM_OF_AVG_RX_AGC_READ;
    lna_offset = (((int2)expected_agc_val) - current_agc_val + rfwcdma_msm_get_dvga_offset(device) );
...
=============================================================================*/
boolean ftm_wcdma_set_pa_state(uint8 pa_state, rfm_device_enum_type rfm_device)
{
  boolean status = FALSE;
  uint8 pa_range;

  /*====================== pass device from dispatch ==========================@vbhide*/

  if (pa_state < 8)
  {
    /* Returns the PA range map from RF NV */
    pa_range = rfwcdma_core_util_get_pa_range_from_nv(rfm_device, pa_state);

    /* This function sets PA range */
    ftm_wcdma_set_pa_range(pa_range, rfm_device);

    status = TRUE;

    FTM_MSG_LOW("ftm_wcdma_set_pa_state: PA State = %d : PA Range = %d Rfm Device = %d", pa_state, pa_range, rfm_device);
  }
  else
  {
                                 ...
                                 uint8 enable_xo_freq_adjust)
{
  ...
  rfm_device_enum_type dev = rfwcdma_data_get_tx_device();
  ...
  dev = rfwcdma_data_get_first_prx_device();

  if((dev >= RFM_DEVICE_0) && (dev < RFM_MAX_WAN_DEVICES))
   {

    //Apply calculated frequency adjustment to WCDMA
     rfwcdma_core_tx_set_freq_adjust(
       rfwcdma_data_get_rflm_buf_idx(dev),
       rfwcdma_data_get_curr_band(dev),
       rfwcdma_data_get_curr_chan(dev, RFCOM_SINGLE_CARRIER),
    ...
    {
       MSG_ERROR("ftm_wcdma_set_freq_adjust:not able to find associated an PRx device, device %d is Invalid", dev, 0, 0);
    }
  ...
  )
{
  ...
  lm_tech_type tech = LM_UMTS;
  lm_status_type status;
  uint8 fbrx_gain_stage;
  const rfc_common_logical_device_params_type *cmn_logical_dev = NULL;
  rfdevice_rxtx_common_class *device_ptr = rfdevice_wcdma_tx_get_common_dev_ptr(device, tx_band.wcdma_band);
  rfcommon_msm_num_carr_bw num_fbrx_carr_bw;
#ifdef FEATURE_RF_HAS_QTUNER
  ...
  band_u.wcdma_band = ftm_curr_wcdma_mode;
  ...
  atuner_cfg_req.chan_num = rfwcdma_data_get_curr_chan(device, RFCOM_SINGLE_CARRIER);

  if (rfwcdma_data_get_num_carriers(device) == RFCOM_SINGLE_CARRIER)
  {
    atuner_cfg_req.carrier_id = RFCMN_ATUNER_CARRIER_ID_0;
  }
  else if (rfwcdma_data_get_num_carriers(device) == RFCOM_DUAL_CARRIER)
  {
    atuner_cfg_req.carrier_id = RFCMN_ATUNER_CARRIER_ID_1;
  }
  else
  {
    RF_MSG_2(RF_ERROR, "ftm_wcdma_xpt_cal_config: unexpected carrier_id -%d returned  on dev %d",
             rfwcdma_data_get_num_carriers(device), device);
    atuner_cfg_req.carrier_id = RFCMN_ATUNER_CARRIER_ID_INVALID;
  }
    ...
    default:
	  ...
	  coupler_tech_cfg.output_type = RFDEVICE_COUPLER_OUTPUT_TYPE_DEFAULT;
                                              ...
                                              RFDEVICE_TUNER_MANAGER))
        {
          ...
          rfwcdma_core_antenna_tuner_program_tx_mode(&atuner_cfg_req,
                                                     RFDEVICE_EXECUTE_IMMEDIATE,
                                                     NULL,
                                                     NULL);
        }
        ...
        // FBRX selftst capture supports single carrier
        num_fbrx_carr_bw.num_carr = RFCOM_SINGLE_CARRIER;

        if (ftm_common_get_selftest_capture_flag())
        {
          fbrx_gain_stage = selftest_fbrx_gain_stage;
        }
        else
        {
          /* hardcoded to selftest gain state 4, it will be converted to WTR3925_FBRX_G44 */
          fbrx_gain_stage = 4;
        }
        
        rfcommon_msm_update_fbrx_iqmc(ftm_wcdma_xpt_fb_path_buffer.buf_idx, 
                                      device, 
                                      (rxlm_chain_type)device_fb, 
                                      LM_UMTS, 
                                      fbrx_gain_stage, 
                                      ftm_curr_wcdma_mode, 
                                      num_fbrx_carr_bw);
        ...
        ret_val &= rfcommon_fbrx_mc_enable_rf_dev(device_ptr, RFLM_FBRX_GAIN_STATE_3);

        /* Step3: Set WTR ET Path */
        rfc_wcdma_setup_fb_path(device,
                                ftm_curr_wcdma_mode,
                                TRUE);

#ifdef FEATURE_RF_HAS_WTR2965        
		/* Apply FBRx RSB coefficients */						
		fbrx_band.wcdma_band = ftm_curr_wcdma_mode;						
		ret_val &= rfcommon_fbrx_mc_update_rsb_coeff( device,
        ...
        if (device_ptr != NULL)
        {
          ...
          rfcommon_fbrx_msm_config_dynamic_adc_settings(ftm_wcdma_xpt_fb_path_buffer.buf_idx,
                                                        (uint32)rfdevice_id_get((void *)device_ptr),3,RFLM_TECH_WCDMA);                     
        }
           else
           {
          RF_MSG( RF_ERROR, "ftm_wcdma_xpt_cal_config: NULL device, cannot config adc settings");
        }
  ...
  else /* xPT Cal Deconfig */
  {
    ...
    default:
      ...
      {
        /* Call RFWCDMA MDSP data cleanup before de-allocate handle */
        api_status = rfm_wcdma_handle_cleanup(ftm_wcdma_xpt_fb_path_buffer.buf_idx);

        if (api_status != RFM_WCDMA_HEALTHY_STATE)
        {
          RF_MSG_1(RF_ERROR,  "ftm_wcdma_xpt_cal_config: Handle cleanup failed for RFLM handle %d", ftm_wcdma_xpt_fb_path_buffer.buf_idx);
        }
...
#ifdef FEATURE_RF_HAS_QTUNER
      /* temporary fix to set the PDET coupler */
      tx_freq = rfwcdma_core_get_freq_from_ul_arfcn(rfwcdma_data_get_curr_chan(device, RFCOM_SINGLE_CARRIER));
      /*HDET enable*/
      rfdevice_hdet_wcdma_init(device,
                               NULL,
                               RFDEVICE_EXECUTE_IMMEDIATE,
                               ftm_curr_wcdma_mode,
                               tx_freq);
  ...
  )
{
  ...
  iq_cap_data.channel = rfwcdma_data_get_curr_chan(device, RFCOM_SINGLE_CARRIER);
  ...
  )
{
  ...
  {
    pa_range = rfwcdma_core_util_get_pa_range_from_nv(device, pa_state);
    /* Set PA Range */
  ...
  )
{
  ...
  curr_tx_ovr.pa_range = rfwcdma_core_util_get_pa_range_from_nv(device, pa_state);
  curr_tx_ovr.pa_curr = pa_curr;
  curr_tx_ovr.txfe0_handle_id = 0xFFFFFFFF;
...
void ftm_wcdma_set_et_path(uint8 pa_state, boolean enable_et_path)
{
  const rfc_common_logical_device_params_type *cmn_logical_dev = NULL;
  rfm_device_enum_type tx_dev = RFM_MAX_WAN_DEVICES;
  tx_dev = rfwcdma_data_get_tx_device();
  /* Get pointer to the common logical device */
  cmn_logical_dev = rfc_common_get_logical_device_params(tx_dev);
...
ftm_iq_capture_error_code_type ftm_wcdma_iq_capture_acquire_samples(rfm_device_enum_type device, ftm_iq_capture_acquire_samples_type *data)
{
  ...
  {
    if (rfwcdma_data_get_num_carriers(device) != RFCOM_DUAL_CARRIER)
    {
  ...
  if (ret_value == FTM_IQ_SUCCESS)
  {
    ...
    {
      for (sample_number = 0; sample_number < ftm_wcdma_iq_acquired_samples_data.req_samples; sample_number++) ftm_wcdma_iq_acquired_samples_data.iq_buf[sample_number] = iq_buffer_temp[sample_number];
    }
...
ftm_iq_capture_error_code_type ftm_wcdma_iq_capture_analyze_samples(rfm_device_enum_type device, ftm_iq_capture_analyze_samples_type *data, ftm_rsp_pkt_type *rsp_pkt)
{
  ...
  {
    FTM_MSG_ERROR("WCDMA IQ Capture: Unable to allocate memory for iq_acquire_data", 0, 0, 0);
    return ret_value;
  }
  ...
  if(NULL != iq_acquire_data)
  {
    ftm_free(iq_acquire_data);
    iq_acquire_data = NULL;
  }
  ...
  cmd_id = (rfwcdma_data_get_device_type(device) == RFM_WCDMA_PRIMARY_RX_DEVICE) ? FTM_WCDMA_C : FTM_WCDMA_RX_2_C;
...
void ftm_wcdma_iq_capture_get_samples(rfm_device_enum_type device, ftm_iq_capture_get_samples_type *data, ftm_rsp_pkt_type *rsp_pkt)
{
  ...
  ftm_wcdma_iq_capture_get_samples_rsp_pkt.subsys_cmd_code = (rfwcdma_data_get_device_type(device) == RFM_WCDMA_PRIMARY_RX_DEVICE) ? FTM_WCDMA_C : FTM_WCDMA_RX_2_C;
  ...
  )
{
  ...
  if (api_status == TRUE)
  {
    ...
    {
      rfc_lte_manage_vregs(sub_id, device, band, RF_PATH_RXTX_STATE);
      ...
      rfdevice_lte_tx_tune_to_chan(device, band, freq, 0, RFDEVICE_DPD_DISABLED, &lte_script, TRUE,(rfdevice_trx_sub_type)sub_id);
                                    ...
                                    uint32 multi_carrier_idx)
{
  ...
  rfm_wcdma_tx_init_params tx_params_i = {0};
  rfm_device_enum_type associated_rx_device = rfwcdma_data_get_associated_device_prx_from_tx(device);
  int32 tx_bpg_count;

  tx_params_i.carrier_mask = RFWCDMA_MC_SC;
  tx_params_i.device = device;
  tx_params_i.txlm_handle = ftm_wcdma_txlm_buffer.buf_idx;
  tx_params_i.tx_channel[RFCOM_SINGLE_CARRIER] = rfwcdma_core_map_rx_to_tx_chan(rfwcdma_data_get_curr_chan(associated_rx_device,RFCOM_SINGLE_CARRIER),rfwcdma_data_get_curr_band(associated_rx_device));
  tx_params_i.tx_channel[RFCOM_DUAL_CARRIER] = rfwcdma_core_map_rx_to_tx_chan(rfwcdma_data_get_curr_chan(associated_rx_device,RFCOM_DUAL_CARRIER),rfwcdma_data_get_curr_band(associated_rx_device));

  MSG_6(MSG_SSID_FTM, MSG_LEGACY_HIGH, "Set Multi-carrier Tx, device=%d, state=%d, multi_carrier_idx=%d, associated rx device:%d, chan 0:%d, chan 1:%d",
        device, state, multi_carrier_idx, associated_rx_device, tx_params_i.tx_channel[RFCOM_SINGLE_CARRIER],tx_params_i.tx_channel[RFCOM_DUAL_CARRIER]) ;

  if (state == 1)
  {
   /*Start in DC CFGed mode, so set both the flags for RF as FALSE and BB as TRUE*/     
   ftm_wcdma_10mhz_mode_rf = FALSE;
   ftm_wcdma_10mhz_mode_bb = TRUE; 

    ftm_wcdma_set_tx_v2(device,TRUE,tx_params_i);
    ...
    rf_fwsw_intf_addr_ptr->txAsyncWriteBuf.betaOverride[0].betaGain0 = 0x5A81;
    rf_fwsw_intf_addr_ptr->txAsyncWriteBuf.betaOverride[1].enable = 1;
    rf_fwsw_intf_addr_ptr->txAsyncWriteBuf.betaOverride[1].betaGain0 = 0x5A81;
#ifndef FEATURE_TABASCO_MODEM
   ...
   #if (!defined(TEST_FRAMEWORK) && (defined(FEATURE_BOLT_MODEM) || defined(FEATURE_TABASCO_MODEM))) 
   /*Converting cx8 qty to bpg granulaty*/
   tx_bpg_count = ((wl1_get_tlm_tx_sys_time_cx8_qty()) >> 11);
   #else
   tx_bpg_count =0;
   #endif
	  
   MSG_HIGH("TxSysTime BPG Cnt %d", tx_bpg_count, 0 , 0);
		
   /*Check for current stmr time too close to frame boundary*/  
   if(tx_bpg_count>120)
   {
    DALSYS_BusyWait(2000);  
    MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH,"Waiting till the next slot. current tx_bpg_count:%d", tx_bpg_count);
   }
	  
   /* Prepare a Uplink DPCCH Config command to mDSP*/   
   ...
   /* Wait for 1 frame to avoid that next cmd come in the same frame */
   DALSYS_BusyWait(10000);	

    /* Send DPCCH cfg cmd to FW to enable DC */
    mdsp_sync_send_ul_dpcch_cfg_cmd
      (
  ...
  {
    ftm_wcdma_set_tx_v2(device, FALSE, tx_params_i);
  }
 ...
 )
{
  ...
  int32 rsb_a                   = 0;
  int32 rsb_b                   = 0;
  rfwcdma_mdsp_dyn_rxlm_cmd_param_type dyn_rxlm_update_cmd_param =
  {{0}
    ,0,0};
  uint32 chain_mask = 0;
...
void ftm_wcdma_wakeup(rfm_device_enum_type device)
{
  ...
  memset(&pwr_param, 0x0, sizeof(rfm_wcdma_power_mgmt_param));
  ...
  )
{
  ...
  *tx_synth_lock = FTM_SYNTH_STATUS_INVALID;
  ...
  /* Check if it is a Rx Device*/
  if((rfwcdma_data_get_device_type(device) == RFM_WCDMA_PRIMARY_RX_DEVICE) ||
  	    (rfwcdma_data_get_device_type(device) == RFM_WCDMA_DIVERSITY_RX_DEVICE))
  {
    /*For Rx Devices Tx PLL status is not applicable.*/
    *tx_synth_lock = FTM_SYNTH_STATUS_NOT_APPLICABLE;

    switch (rfwcdma_data_get_rf_state(device))
    {
    case RFWCDMA_STATE_INIT:
    case RFWCDMA_STATE_ENTER_MODE:		
    case RFWCDMA_STATE_RX_PREP:		
    case RFWCDMA_STATE_RX_PREP_WAKEUP:
    case RFWCDMA_STATE_RX_SLEEP:
    case RFWCDMA_STATE_POWER_OFF:
	/*Rx Synth status for the above RF state is Disabled since Rx is not Tuned yet*/	
      *rx_synth_lock = FTM_SYNTH_STATUS_DISABLED;
    break;

    case RFWCDMA_STATE_RX:
    ...
    default:
      FTM_MSG_1(FTM_HIGH, "Unexpected state %d", rfwcdma_data_get_rf_state(device));
      break;
    }
  }
  /*If the device is Tx*/
  else if(rfwcdma_data_get_device_type(device) == RFM_WCDMA_TX_DEVICE)
  {
    switch (rfwcdma_data_get_rf_state(device)) 
    {
    /*For Tx Devices Rx PLL status is not applicable.*/
    *rx_synth_lock = FTM_SYNTH_STATUS_NOT_APPLICABLE;  
    case RFWCDMA_STATE_TX_PREP:
    case RFWCDMA_STATE_TX_SLEEP:	  
    case RFWCDMA_STATE_POWER_OFF:
	/*Tx Synth status for the above RF state is Disabled since Tx is not Tuned yet*/	
      *tx_synth_lock = FTM_SYNTH_STATUS_DISABLED;
    break;

    case RFWCDMA_STATE_TX:
      rfm_get_synth_lock_status(device, RFCOM_WCDMA_MODE,
  ...
  FTM_MSG_4(FTM_MED, "Latest values device %d Type %d rx_synth_lock %d tx_synth_lock %d",
            device, rfwcdma_data_get_device_type(device), *rx_synth_lock, *tx_synth_lock);
}
...
{ 
  ...
  rf_time_tick_type prof_tick = rf_time_get_tick();
  uint8 selftest_error = ftm_common_selftest_main(params, NULL, rsp_pkt);

  if (selftest_error == FTM_FBRX_TEST_SUCCESS)
  {
    rf_time_type prof_time = rf_time_get_elapsed(prof_tick, RF_MSEC);
    FTM_MSG_1(FTM_HIGH, "Selftest Time profiling WCDMA ExecTime %d ms", prof_time);
  }
  
  else
  {
    FTM_MSG(FTM_HIGH, "Selftest failed");
  }
}
  ...
  )
{
  ...
  ftm_rf_fbrx_meas_noise_params_rsp_pkt.subsys_cmd_code = (rfwcdma_data_get_device_type(device) == RFM_WCDMA_PRIMARY_RX_DEVICE) ? FTM_WCDMA_C : FTM_WCDMA_RX_2_C;
  ...
  )
{
  ...
  ftm_rf_fbrx_iq_capture_trigger_rsp_pkt.subsys_cmd_code = (rfwcdma_data_get_device_type(device) == RFM_WCDMA_PRIMARY_RX_DEVICE) ? FTM_WCDMA_C : FTM_WCDMA_RX_2_C;
  ...
  )
{
  ...
  if (rsp_pkt != NULL)
  {
    ...
    ftm_rf_fbrx_iq_capture_get_samples_rsp_pkt.subsys_cmd_code = (rfwcdma_data_get_device_type(device) == RFM_WCDMA_PRIMARY_RX_DEVICE) ? FTM_WCDMA_C : FTM_WCDMA_RX_2_C;
                              ...
                              boolean fb_rxlm_enable)
{
  ...
  rfm_device_enum_type tx_device = rfwcdma_data_get_tx_device();
        ...
        FEEDBACK_SETUP_ACTION_MASK_CONFIGURE_WTR)
  {
    ...
    device_ptr = rfdevice_wcdma_tx_get_common_dev_ptr(tx_device, ftm_curr_wcdma_mode);
        ...
        FEEDBACK_SETUP_ACTION_MASK_CONFIGURE_RXLM_FOR_FBAGC)
  {
    ...
    {
      /* @todo: remove hard-coding, tech to update the fbrx chain from rfc */
      rfcommon_fbrx_msm_config_dynamic_adc_settings(fbrx_lm_handle,
                                                    (uint32)rfdevice_id_get((void *)device_ptr),3,RFLM_TECH_LTE);                 
       }
       else
       {
      RF_MSG( RF_ERROR, "ftm_wcdma_feedback_setup: NULL device, cannot config adc settings");
    }

    if (ret_value == RFCOMMON_FBRX_SUCCESS)
    {
     fbrx_modem_chain = (uint8)(rfc_wcdma_get_fb_chain(rfwcdma_data_get_tx_device(),rfwcdma_data_get_curr_band(rfwcdma_data_get_tx_device())));

      /* @todo: remove hard-coding, tech to update the fbrx chain from rfc */
      ret_value = rfcommon_fbrx_enable(fbrx_lm_handle, fbrx_modem_chain);
                                    ...
                                    uint32 carrier_mask)
{
  ...
  while (num_devices != 0)
  {
    RF_MSG_2(RF_ERROR, "ftm_wcdma_tune_to_chan_scripts: Num %d mask %d", num_devices, device_mask);
    device = (rfm_device_enum_type)rf_extract_first_one_bit_lsb_location(num_devices);

    if (device >= RFM_MAX_WAN_DEVICES || device < RFM_DEVICE_0)
    {
    ...
    if ( (rfwcdma_core_is_band_supported_on_dev(band,device)) == FALSE )
    {
  ...
  pwr_param.device = device;
  pwr_param.on_off = TRUE;
  pwr_param.sub_type = RFM_WCDMA_SUB_0;
...
ftm_wcdma_scenarios_type ftm_wcdma_get_current_test_scenario()
{
  ftm_wcdma_scenarios_type ftm_wcdma_test_scenario = 0;

  ftm_wcdma_test_scenario = ftm_rf_current_test_scenario & 0xFF;
  ...
  return ftm_wcdma_test_scenario;
...
void ftm_wcdma_execute_temp_comp(void)
{
  ...
  boolean wcdma_mode_ind = FALSE;

  /* Call temp comp only in FTM RF mode for a valid Tx Device */
  ...
  /*check the current mode
            - no device in WCDMA mode: do nothing
            - has at least one device in WCDMA mode:
                             do temp comp*/
  for(rfm_device =RFM_DEVICE_0; rfm_device<RFM_MAX_WAN_DEVICES; rfm_device++)
  {
    current_mode = rfm_get_current_mode(rfm_device);
    if(RFM_IMT_MODE == current_mode)
    {
      wcdma_mode_ind = TRUE;
      break;
    }
  ...
  /* Sanity: Check if currently all following are true
     ...
     */
  if(TRUE == wcdma_mode_ind)
  {
    rfm_device = rfwcdma_data_get_tx_device();
     
    if( ( ftm_get_mode() == FTM_MODE )&& 
        ( rfm_get_calibration_state() == FALSE) &&
        ( RFWCDMA_STATE_TX == rfwcdma_data_get_rf_state(rfm_device))
      )
    {
      /* Get Tx device */
      tc_param.device = rfm_device;
...
void ftm_wcdma_execute_temp_comp_cb(rfm_cb_event_enum_type temp_comp_status, void* user_data)
{
   ...
   /* Get Tx device */
   tc_param.device = rfwcdma_data_get_tx_device();
...
/*!
  @brief
  Prep Wake-up on the specified Rx device
 
  @details
  This function is called to build the scripts for rx tune on the specified rx device
...
void ftm_wcdma_prep_wakeup(rfm_device_enum_type device)
{
  rfwcdma_data_debug_flags_type *dbg_flags;
  rf_path_enum_type path;
  rfm_wcdma_rx_wakeup_params rx_wakeup_params;
  ...
  /*Don't turn on Vregs. User needs to turn one explicitly from the QRCT interface*/
  /* Turn ON vregs*/
  if (dbg_flags->enable_opt_wakeup == TRUE)
  {
    FTM_MSG(FTM_HIGH, " ftm_wcdma_prep_wakeup: Optimized wakeup routine execution");
  ...
  {
    // Do Nothing for prep for non-script based wakeup. Will let exec_Wakeup to fall back to rfm_wakeup.
#if 0
...
/*!
  @brief
  Exec Wake-up on the specified Rx device
 
  @details
  This function is called to execute the scripts(via FW) to tune rx on the specified device
...
void ftm_wcdma_exec_wakeup(rfm_device_enum_type device)
{
  rfwcdma_data_debug_flags_type *dbg_flags;
  rf_path_enum_type path;
  rfm_wcdma_rx_wakeup_params rx_wakeup_params;
...
/*!
  @brief
  Wrapper function to get current Tx device from RF state machine
...
*/
rfm_device_enum_type ftm_wcdma_get_cur_tx_device(void)
{
  return rfwcdma_data_get_tx_device();
}

void
ftm_wcdma_set_tx_v2(rfm_device_enum_type device, boolean state, rfm_wcdma_tx_init_params tx_params)
{
  /* ftm_wcdma_set_pdm() will overwrite cur_tx_agc_adj_pdm; hence save its
     current value, before initializing the pdm to zero */
  int2 cur_tx_agc_adj_pdm_prev = cur_tx_agc_adj_pdm;
  ...
  rfm_wcdma_tx_deinit_params tx_deinit_params;
  rfm_wcdma_enter_mode_param enter_param = { 0 };
  rfm_wcdma_power_mgmt_param tx_pwr_param = { 0 };
  ...
  if( device >= RFM_MAX_WAN_DEVICES ) 
  {
    FTM_MSG_1( FTM_ERROR,"ftm_wcdma_set_tx_v2: Invalid RFM device = %d ", device);
    return ;
  }
  ...
  enter_param.device_type = RFM_WCDMA_TX_DEVICE;
  tx_e_params.device = device;
  tx_d_params.device = device;
  tx_deinit_params.device = device;

  /*Turn Off Vregs. RF should handle this. WL1 to remove all RF calls in FTM mode*/
  tx_pwr_param.device = device;
  tx_pwr_param.on_off = state;
  tx_pwr_param.sub_type = RFM_WCDMA_SUB_0;
  ...
  if ((state == ON) && (ftm_wcdma_tx_on_status == FALSE))
  {
    ...
    /* restore the saved value */
    cur_tx_agc_adj_pdm = cur_tx_agc_adj_pdm_prev;
    /*Every device has to do an enter mode.*/

    ftm_rfmode_enter(device, FTM_STATE_WCDMA);

    enter_param.prx_device = rfwcdma_data_get_associated_device_prx_from_tx(device);

    rfm_enter_mode(device, RFCOM_WCDMA_MODE, &enter_param, NULL, ftm_wcdma_txlm_buffer.buf_idx);

    rfm_wcdma_power_on_off(&tx_pwr_param, NULL, NULL);
    if (ftm_wcdma_10mhz_mode_rf)
    {
      tx_i_params.carrier_mask = 3;
      rfm_wcdma_tx_init(&tx_params, NULL, NULL);
    }
    else
    {
      tx_i_params.carrier_mask = 1;
    ...
    FTM_MSG_4(FTM_HIGH, "ftm_wcdma_radio_tx_init: mask %d, device %d handle %d channnel %d",
              tx_params.carrier_mask, tx_params.device, tx_params.txlm_handle, tx_params.tx_channel[0]);

    tx_e_params.device = tx_params.device;
    tx_e_params.tx_channel[0] = tx_params.tx_channel[0];
    rfm_wcdma_tx_enable(&tx_e_params, NULL, NULL);

    FTM_MSG_2(FTM_HIGH, "ftm_wcdma_radio_enable: device %d channnel %d",
              tx_e_params.device, tx_e_params.tx_channel[0]);
    /* ldo_off_flag */
    rfwcdma_core_tx_ftm_disable_ldo_off(rfwcdma_data_get_rflm_buf_idx(device), TRUE);
    ...
    if(FALSE == ftm_wcdma_10mhz_mode_bb)
    {
     /*Send TXLM command only for SC case */
     ftm_send_txlm_cfg_cmd(ftm_wcdma_txlm_buffer.buf_idx, TRUE);
        /* We really want to see a WCDMA pilot so we will go ahead and turn
     ** on right here. At this time there is no real reason to do
     ** it elsewhere.
  ...
  else if ((state == FALSE) && (ftm_wcdma_tx_on_status == ON))
  {
    ...
    if (ftm_wcdma_txlm_buffer.is_buf_allocated)
    {
      /*Fill the Disable Tx params and Call Tx Disable*/
      tx_d_params.device = tx_params.device;
      rfm_wcdma_tx_disable(&tx_d_params, NULL, NULL);
    }
    else
    {
    ...
    rfm_wcdma_power_on_off(&tx_pwr_param, NULL, NULL);
    rfm_exit_mode(device, RFM_PARKED_MODE);
    ftm_rfmode_exit(tx_params.device, FTM_STATE_PARK);
    FTM_MSG_HIGH("Setting TX OFF", 0, 0, 0);
  }     /* else */
}
...
/*!
  @brief
  Does the WCDMA FTM radio test init
 
  @details
  This function is used to do the FTM WCDMA radio test init.
  All the APIs to be called by the common FTM RF test framework should be registered here
*/
void
ftm_wcdma_init
(
  ...
  )
{
  ...
  ftm_rf_test_radio_config_register_tech_cb(RFM_IMT_MODE, ftm_wcdma_start_radiotest);
  ftm_rf_test_rx_measure_register_tech_cb(RFM_IMT_MODE,ftm_wcdma_rftest_rx_measure);
  /*Register all future commands here*/
}
...
/*!
  @brief
  API to map the phone band mode passed from tools to rfcom wcdma band
...
*/
rfcom_wcdma_band_type
ftm_wcdma_convert_phone_mode_to_wcdma_band(uint16 band)
{
...
/*!
  @brief
  API to get current Tx AGC Adj PDM value 
 
  @retval
  Current Tx AGC Adj PDM value
*/
int2 ftm_wcdma_get_cur_tx_pdm_value(void)
{
 return cur_tx_agc_adj_pdm;
}
...
{
  //ftm_wcdma_state_type *ptr_to_ftm_wcdma_data=ftm_wcdma_data_get_wcdma_data();
  
  qsh_mdump_collect_high((void *)&ftm_wcdma_state,sizeof(ftm_wcdma_state));
  qsh_mdump_collect_high((void *)ftm_wcdma_rxlm_buffer,sizeof(ftm_wcdma_rxlm_buffer));
