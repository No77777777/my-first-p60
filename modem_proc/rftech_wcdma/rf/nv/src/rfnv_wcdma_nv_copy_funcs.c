...
$Header: //components/rel/rftech_wcdma.mpss/3.11/rf/nv/src/rfnv_wcdma_nv_copy_funcs.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
02/14/18   sk      KW error fix
...
{
  memscpy(&rfnv_tbl_ptr->hdet_vs_temp[0], (sizeof(int16)*NV_TEMP_TABLE_SIZ), (int8*)item_ptr,(sizeof(int8)*NV_TEMP_TABLE_SIZ));
}
...
/*!
  @brief
  Load the Tx  static NVs.

  @details
  AsDiv eLNA FR 30081, Tx gain offsets are loaded
...
*/
void rfnv_wcdma_copy_tx_static_data
(
  rfnv_wcdma_nv_tbl_type *rfnv_tbl_ptr,
  rfnv_item_type *item_ptr,
  rfcom_wcdma_band_type band
...
{
  rfnv_wcdma_load_tx_static_data_nv(rfnv_tbl_ptr,item_ptr,band);
}
...
)
{
  ...
  if ( rx_band_cal_data.cal_124_data != NULL )
  {
    modem_mem_free ( rx_band_cal_data.cal_124_data, 
                         MODEM_MEM_CLIENT_RFA);
  }
  return;
}
...
)
{
  ...
  {
    if(version == RFWCDMA_NV_PA_STATIC_VER_1 )
    {
    ...
    else if (version == RFWCDMA_NV_PA_STATIC_VER_2) 
    {
       memscpy(&rfnv_tbl_ptr->pa_static, sizeof(rfcommon_nv_cmn_pa_static_v2_type),
              (rfcommon_nv_cmn_pa_static_v2_type*)item_ptr, sizeof(rfcommon_nv_cmn_pa_static_v2_type));

       for(num_lin =0; num_lin<RFCOM_TXLIN_NUM_LINEARIZERS; num_lin++)
       {
          rfnv_tbl_ptr->lin_temp_comp_data.skip_pout_normalization[num_lin] = 
                                 ((rfcommon_nv_cmn_pa_static_v2_type*)item_ptr)->pout_control[num_lin];
       }/*for(num_lin =0; num_lin<RFCOM_TXLIN_NUM_LINEARIZERS; num_lin++)*/
...
)
{
  ...
  if( (version < RFWCDMA_NV_PA_STATIC_VER_MAX) && (version > RFWCDMA_NV_LEGACY_FOUR_STATE_PA) )
  {
    ...
    {
      memscpy(&rfnv_tbl_ptr->dc_pa_static, sizeof(rfcommon_nv_cmn_pa_static_v2_type),
              (rfcommon_nv_cmn_pa_static_v2_type*)item_ptr, sizeof(rfcommon_nv_cmn_pa_static_v2_type));
      for(num_lin =0; num_lin<RFCOM_TXLIN_NUM_LINEARIZERS; num_lin++)
      {
          rfnv_tbl_ptr->dc_lin_temp_comp_data.skip_pout_normalization[num_lin] = 
                                 ((rfcommon_nv_cmn_pa_static_v2_type*)item_ptr)->pout_control[num_lin];
      }/*for(num_lin =0; num_lin<RFCOM_TXLIN_NUM_LINEARIZERS; num_lin++)*/
    }/*else if (version == RFWCDMA_NV_PA_STATIC_VER_2) */
  }/*if( (version < RFWCDMA_NV_PA_STATIC_VER_MAX) && (version > RFWCDMA_NV_LEGACY_FOUR_STATE_PA) )*/
