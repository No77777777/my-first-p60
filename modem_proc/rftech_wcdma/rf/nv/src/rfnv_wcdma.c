...
$Header: //components/rel/rftech_wcdma.mpss/3.11/rf/nv/src/rfnv_wcdma.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
06/03/16   ag      Fix KW error
05/17/16   ak      Change to avoid diversity lna swicth point corruption
05/16/16   ak      Change to avoid diversity lna swicth point corruption
05/12/15   sd      F3 logging error fix in load rx static NV
05/12/15   sd      Fix to skip Rx Static V2 version copy to data structure
10/30/15   rmb     Max out default LNA switchpoints which are not used.
09/29/15   sd      FR30081 AsDiv with eLNA gain compensation 
09/29/15   sd      Fix KW error
09/28/15   vbh     Use nv container id for Rx V2 NVs
09/16/15   sd      NV support for FR 30081 Add WCDMA Tx static NVs and new datatypes in Rx static NV 
09/10/15   rmb     Change the NUM LNA STATES to 5 from embedded side so that this is 
                   in line with static rx and dynamic rx data v2 NVs supporting 5 LNA Rise/Fall.
07/22/15   dw      Populate char/cal channels for DC lin temp comp data
07/23/15   vs      Support B9 band alone in absence of B3
06/15/15   sd      Resolve B3/B9 pa static nv version conflict
06/09/15   vbh     path_id and chain_id limit check from Path 4 to max_path
06/08/15   vbh     rfnv_wcdma_get_band_for_nv_table - use core API to get band support
05/08/15   ak      [Rx/Tx Split]Changes to support 6 Rx Path Cal
03/23/15   ska	   Add support to DGLNA feature
04/24/15   sd      Corrected center channel in load Rx dynamic V2 NV
...
void rfnv_wcdma_init_nv_copy_function_list(void)
{
  ...
  nv_copy_funcs.copy_funcs_type.copy_tx_static_data = rfnv_wcdma_copy_tx_static_data;
}
...
rfcom_wcdma_band_type rfnv_wcdma_get_band_for_nv_table(rfnv_wcdma_table_type nv_band)
{
  ...
  switch (nv_band)
  {
    case RFNV_WCDMA_TABLE_IMT_2100:
        band = RFCOM_BAND_IMT;
        if (rfwcdma_core_is_band_supported(band)== FALSE) 
      {
    ...
    case RFNV_WCDMA_TABLE_1800:
      if (rfwcdma_core_is_band_supported(RFCOM_BAND_BC3)== TRUE) 
      {
        band = RFCOM_BAND_BC3;
      }
      else if (rfwcdma_core_is_band_supported(RFCOM_BAND_BC9)== TRUE) 
      {
        band = RFCOM_BAND_BC3;
      }
      else
  ...
  void (*task_wait_handler)( rex_sigs_type ))
{
  ...
  for (nv_band_ctr=RFNV_WCDMA_TABLE_IMT_2100; nv_band_ctr<RFNV_WCDMA_NUM_TABLES; nv_band_ctr++)
  {
    ...
    if (band != RFCOM_BAND_INVALID)
    {
      ...
      rfnv_wcdma_set_tx_static_data_to_default(nv_tables[nv_band_ctr]);
...
)
{
  ...
  for ( freq_index = 0; freq_index < NV_FREQ_TABLE_SIZ; freq_index++ )
  {
    ...
    if ( item_ptr->wcdma_1500_tx_cal_chan[freq_index] != 0 )
    {
      ...
      nv_tbl_ptr->dc_lin_temp_comp_data.tx_char_chan[freq_index] = 
                         (uint16)item_ptr->wcdma_1500_tx_cal_chan[freq_index];
      nv_tbl_ptr->dc_lin_temp_comp_data.tx_char_freq[freq_index] =
        rfwcdma_core_get_freq_from_uarfcn( 
                        band,
                        (uint16)item_ptr->wcdma_1500_tx_cal_chan[freq_index],
                        RFWCDMA_CORE_UARFCN_UL );
  ...
  nv_tbl_ptr->dc_lin_temp_comp_data.tx_char_chan_size = freq_index;
...
)
{
  ...
  switch(element)
  {
    case RX_CAL_CHAN_LIST:
      rx_data_ptr->rx_cal_chan[(rfnv_wcdma_container_enum_type)device_id][index] = val;
      rx_data_ptr->rx_cal_chan[(rfnv_wcdma_container_enum_type)db_device_id][index] = val;
      break;

    case VGA_OFFSET:
      rx_data_ptr->rx_offsets[(rfnv_wcdma_container_enum_type)device_id][carr_id].vga_offset = val;
      rx_data_ptr->rx_offsets[(rfnv_wcdma_container_enum_type)db_device_id][carr_id].vga_offset = val;
      break;

    case LNA_OFFSET:
      rx_data_ptr->rx_offsets[(rfnv_wcdma_container_enum_type)device_id][carr_id].lna_offsets[index] = val;
      rx_data_ptr->rx_offsets[(rfnv_wcdma_container_enum_type)db_device_id][carr_id].lna_offsets[index] = val;
      break;

    case VGA_OFFSET_VS_FREQ:
      rx_data_ptr->freq_offset[(rfnv_wcdma_container_enum_type)device_id][index].vga_gain_offset_vs_freq = val;
      rx_data_ptr->freq_offset[(rfnv_wcdma_container_enum_type)db_device_id][index].vga_gain_offset_vs_freq = val;
      break;

    case LNA_OFFSET_VS_FREQ_1:
      rx_data_ptr->freq_offset[(rfnv_wcdma_container_enum_type)device_id][index].lna_offset_vs_freq_1 = val;
      rx_data_ptr->freq_offset[(rfnv_wcdma_container_enum_type)db_device_id][index].lna_offset_vs_freq_1 = val;
      break;

    case LNA_OFFSET_VS_FREQ_2:
      rx_data_ptr->freq_offset[(rfnv_wcdma_container_enum_type)device_id][index].lna_offset_vs_freq_2 =  val;
      rx_data_ptr->freq_offset[(rfnv_wcdma_container_enum_type)db_device_id][index].lna_offset_vs_freq_2 =  val;
      break;

    case LNA_OFFSET_VS_FREQ_3:
      rx_data_ptr->freq_offset[(rfnv_wcdma_container_enum_type)device_id][index].lna_offset_vs_freq_3 = val;
      rx_data_ptr->freq_offset[(rfnv_wcdma_container_enum_type)db_device_id][index].lna_offset_vs_freq_3 = val;
      break;

    case LNA_OFFSET_VS_FREQ_4:
      rx_data_ptr->freq_offset[(rfnv_wcdma_container_enum_type)device_id][index].lna_offset_vs_freq_4 = val;
      rx_data_ptr->freq_offset[(rfnv_wcdma_container_enum_type)db_device_id][index].lna_offset_vs_freq_4 = val;
      break;
...
)
{
  ...
  rx_data_ptr->is_consolidated = FALSE;
...
(
  rfnv_wcdma_container_enum_type nv_id,
...
)
{
  ...
  else
  {
    ...
    {
      case RX_CAL_CHAN_LIST:
         ret_val = rx_data_ptr->rx_cal_chan[nv_id][index];
        break;

      case VGA_OFFSET:
         ret_val = rx_data_ptr->rx_offsets[nv_id][carr_id].vga_offset;
        break;

      case LNA_OFFSET:
         ret_val = rx_data_ptr->rx_offsets[nv_id][carr_id].lna_offsets[index];
        break;

      case VGA_OFFSET_VS_FREQ:
         ret_val = rx_data_ptr->freq_offset[nv_id][index].vga_gain_offset_vs_freq;
        break;

      case LNA_OFFSET_VS_FREQ_1:
         ret_val = rx_data_ptr->freq_offset[nv_id][index].lna_offset_vs_freq_1;
        break;

      case LNA_OFFSET_VS_FREQ_2:
         ret_val = rx_data_ptr->freq_offset[nv_id][index].lna_offset_vs_freq_2;
        break;

      case LNA_OFFSET_VS_FREQ_3:
         ret_val = rx_data_ptr->freq_offset[nv_id][index].lna_offset_vs_freq_3;
        break;

      case LNA_OFFSET_VS_FREQ_4:
         ret_val = rx_data_ptr->freq_offset[nv_id][index].lna_offset_vs_freq_4;
...
)
{
  ...
  else
  {
    ...
    {
      case LNA_RISE:
         ret_val = rx_data_ptr->rx_lna_swpts[nv_id][carr_id].lna_rise[index];
        break;
 
      case LNA_FALL:
         ret_val = rx_data_ptr->rx_lna_swpts[nv_id][carr_id].lna_fall[index];
         break;                              
       case LNA_HS_RISE:                     
         ret_val = rx_data_ptr->rx_lna_swpts[nv_id][carr_id].lna_hs_rise[index];
         break;                              
 
       case LNA_HS_FALL:                     
         ret_val = rx_data_ptr->rx_lna_swpts[nv_id][carr_id].lna_hs_fall[index];
        break;
     }//end switch
...
)
{
  ...
  /*Keeps track of NV container*/
  int8 nv_container = 0;

  /*Keeps track of AsDiv switch position*/
  int8 switch_pos = 0;

  /*Reserved field in the NV, not used now*/
  int8 reserved = 0;
  
  /* Temp hold the values*/
  rfwcdma_nv_rx_asdiv_elna_gain_offset_type *asdiv_elna_offset_values;
  rfwcdma_nv_rx_asdiv_elna_switchpoint_type *asdiv_elna_switchpoint_values; 

  /*Flags to indicate if FR NVs are loaded*/
  boolean asdiv_offset_flag[RFNV_CONTAINER_ID_MAX] = {FALSE};
  ...
  /* Do not copy NV version into data structure, initialized to 2 by default*/
  ...
  for(i=0;i<rfnv_ptr->rx_static_data.rx_stat_variant_marker.num_elements;i++)
  {
    ...
    switch (type_id)
    {
      case RFNV_DATA_TYPE_LNA_SWPTS_V1:
        ...
        if(chain_id >=RF_PATH_MAX)
        {
          MSG_1( MSG_SSID_RF, MSG_LEGACY_ERROR,"rfnv_wcdma_load_rx_static_data_nv: "
                       "chain_id= %d is not within range(>=RF_PATH_MAX)",chain_id);
          return;
        }
      ...
      /* -----Additional cases to address AsDiv eLNA FR 30081-------------------------------------------------------------*/
	  ...
	  case RFNV_DATA_TYPE_ASDIV_RX_GAIN_OFFSET:        
		...
		/* Read NV container id */
		  nv_element_size = sizeof(nv_container);
	      memscpy((void*)&nv_container ,nv_element_size,nv_item_ptr,nv_element_size);
	      num_bytes_parsed += nv_element_size;
	      nv_item_ptr 	 += nv_element_size;

		  if(nv_container >= RFNV_CONTAINER_ID_MAX)
		  {
			MSG_1( MSG_SSID_RF, MSG_LEGACY_ERROR,"rfnv_wcdma_load_rx_static_data_nv: "
       		       "nv_container= %d is not within range(>= RFNV_CONTAINER_ID_MAX)",nv_container);
			return;
		  }

		/* Read the reserved field */
		  nv_element_size = sizeof(reserved);
	      memscpy((void*)&reserved ,nv_element_size,nv_item_ptr,nv_element_size);
	      num_bytes_parsed += nv_element_size;
	      nv_item_ptr 	 += nv_element_size;
		  
		/* Read Switch position */
		  nv_element_size = sizeof(switch_pos);
	      memscpy((void*)&switch_pos ,nv_element_size,nv_item_ptr,nv_element_size);
		  num_bytes_parsed += nv_element_size;
	      nv_item_ptr	  += nv_element_size;
		  
		  if(switch_pos >= RFWCDMA_NUM_ASDIV_POS)
		  {
			MSG_1( MSG_SSID_RF, MSG_LEGACY_ERROR,"rfnv_wcdma_load_rx_static_data_nv: "
			       "switch_pos= %d is not within range(>= ASDIV_POSITION_MAX)",switch_pos);
			return;
		  }

        /* Read gain offset values */
		  nv_element_size = sizeof(rfwcdma_nv_rx_asdiv_elna_gain_offset_type);
		  asdiv_elna_offset_values = (rfwcdma_nv_rx_asdiv_elna_gain_offset_type*)modem_mem_alloc(nv_element_size, MODEM_MEM_CLIENT_RFA);

		  if(asdiv_elna_offset_values != NULL)
		  {
		  memscpy((void*)asdiv_elna_offset_values,nv_element_size,nv_item_ptr,nv_element_size);
		  rx_stat->rx_asdiv_elna_offset_ptr[nv_container][switch_pos] = asdiv_elna_offset_values;
		  }
		  num_bytes_parsed += nv_element_size;
	      nv_item_ptr	  += nv_element_size;

          asdiv_offset_flag[nv_container] = TRUE;
		  break;
	  ...
	  case RFNV_DATA_TYPE_ASDIV_RX_SWITCH_POINTS:
        ...
        /* Read gain offset values */
		  nv_element_size = sizeof(rfwcdma_nv_rx_asdiv_elna_switchpoint_type);
		  asdiv_elna_switchpoint_values = (rfwcdma_nv_rx_asdiv_elna_switchpoint_type*)modem_mem_alloc(nv_element_size, MODEM_MEM_CLIENT_RFA);

		  if(asdiv_elna_switchpoint_values != NULL)
		  {
		  memscpy((void*)asdiv_elna_switchpoint_values,nv_element_size,nv_item_ptr,nv_element_size);
		  rx_stat->rx_asdiv_elna_switchpoint_ptr[nv_container][switch_pos] = asdiv_elna_switchpoint_values;
		  }
		  num_bytes_parsed += nv_element_size;
	      nv_item_ptr	  += nv_element_size;
  ...
  if(rx_stat != NULL)
  {
    rx_stat->is_consolidated = TRUE;
  }

  else
  {
    MSG( MSG_SSID_RF, MSG_LEGACY_ERROR,"rfnv_wcdma_load_rx_static_data_nv: "
	"Either id type is invalid or Nv ptr payload is Null");
	return;
  }

#ifdef FEATURE_RF_ASDIV
  /* FR30081, If an AsDiv position has valid entries,set other AsDiv position values to 0 */
  /* 0 values instead of NULL will allow a reconfig when switched back to default position */
  rx_stat = (rfwcdma_nv_rx_static_payload_v2_type *)&rfnv_ptr->rx_static_data.payload;

  for(nv_container = 0; nv_container < RFNV_CONTAINER_ID_MAX; nv_container++)
  {
    if(asdiv_offset_flag[nv_container]== TRUE)
    {
      
      for(switch_pos = 0; switch_pos < RFWCDMA_NUM_ASDIV_POS; switch_pos++)
      {
        if(rx_stat->rx_asdiv_elna_offset_ptr[nv_container][switch_pos]==NULL)
        {
          rx_stat->rx_asdiv_elna_offset_ptr[nv_container][switch_pos] = 
		  	     (rfwcdma_nv_rx_asdiv_elna_gain_offset_type*)modem_mem_calloc(1,sizeof(rfwcdma_nv_rx_asdiv_elna_gain_offset_type), MODEM_MEM_CLIENT_RFA);		  
        }
		if(rx_stat->rx_asdiv_elna_switchpoint_ptr[nv_container][switch_pos]==NULL)
		{
		  rx_stat->rx_asdiv_elna_switchpoint_ptr[nv_container][switch_pos] = 
	             (rfwcdma_nv_rx_asdiv_elna_switchpoint_type*)modem_mem_calloc(1,sizeof(rfwcdma_nv_rx_asdiv_elna_switchpoint_type), MODEM_MEM_CLIENT_RFA);
		}
      }
    }
  }
...
/*!
 @brief
   Loading the Tx static NV into the internal structure
   
  @details
    The NV is currently active only for AsDiv eLNA FR 30081. 
    Tx gain offsets for each chain and antenna postiton are loaded. 
...
*/
void rfnv_wcdma_load_tx_static_data_nv
(
  rfnv_wcdma_nv_tbl_type *rfnv_ptr,
  rfnv_item_type *item_ptr,
  rfcom_wcdma_band_type band
)
{
  ...
  rfwcdma_nv_tx_static_payload_v1_type *tx_stat = NULL;
  ...
  /* Read Version */
  nv_element_size = sizeof(uint16);
  memscpy((void*)&rfnv_ptr->tx_static_data.tx_stat_variant_marker.version ,
  	                                     nv_element_size,nv_item_ptr,nv_element_size);
  num_bytes_parsed += nv_element_size;
  nv_item_ptr += nv_element_size;
  ...
  {
    /* Memset all data to 0 */
    memset((void*)&rfnv_ptr->tx_static_data,0,sizeof(rfwcdma_nv_tx_static_data_type));
    /* Set NV to default values before copying */
    rfnv_wcdma_set_tx_static_data_to_default(rfnv_ptr);
    init_flag[band] = TRUE;
  }
  
  /* Read number of elements */
  memscpy((void*)&rfnv_ptr->tx_static_data.tx_stat_variant_marker.num_elements,
                                         nv_element_size,nv_item_ptr,nv_element_size);
  num_bytes_parsed += nv_element_size;
  nv_item_ptr      += nv_element_size;
  ...
  for(i=0;i<rfnv_ptr->rx_static_data.rx_stat_variant_marker.num_elements;i++)
  {
    ...
    switch (type_id)
    {
      ...
      case RFNV_DATA_TYPE_ASDIV_TX_GAIN_OFFSET:
	  	tx_stat = (rfwcdma_nv_tx_static_payload_v1_type *)&rfnv_ptr->tx_static_data.payload;
        ...
        /* Read gain offset values */ 
          ...
          nv_element_size = sizeof(int16);
          memscpy((void*)&(tx_stat->tx_asdiv_elna_offset[nv_container][switch_pos]),nv_element_size,nv_item_ptr,nv_element_size);
          num_bytes_parsed += nv_element_size;
          nv_item_ptr 	 += nv_element_size;
...
)
{
  ...
  rfnv_rx_cal_data_typeid_118_type *nv_cal_118_data = NULL;
  rfnv_rx_cal_data_typeid_124_type *nv_cal_124_data = NULL;
  ...
  uint32 num_of_cal_118_elements = 0;
  uint32 num_of_cal_124_elements = 0;
  ...
  if(rfnv_ptr == NULL || rx_band_cal_data_ptr ==  NULL)
  {
  ...
  num_of_cal_118_elements = rx_band_cal_data_ptr->variant_marker.num_elements - rx_band_cal_data_ptr->num_of_cal_124_elements;
  num_of_cal_124_elements = rx_band_cal_data_ptr->num_of_cal_124_elements;

  if(num_of_cal_118_elements !=0 && rx_band_cal_data_ptr->cal_118_data == NULL )
  {
    MSG( MSG_SSID_RF, MSG_LEGACY_ERROR,"rfnv_wcdma_load_rx_dynamic_data_v2_nv: "
                                         "NULL item ptr for cal type 118 data");
    return;
  }

  if(num_of_cal_124_elements !=0 && rx_band_cal_data_ptr->cal_124_data == NULL )
  {
    MSG( MSG_SSID_RF, MSG_LEGACY_ERROR,"rfnv_wcdma_load_rx_dynamic_data_v2_nv: "
                                         "NULL item ptr for cal type 124 data");
    return;
  }

  rx_dyn = (rfwcdma_nv_rx_dynamic_payload_v2_type *)&rfnv_ptr->rx_dyn_data.payload;

  for(element_index=0; element_index < num_of_cal_118_elements; element_index++)
  { 
    nv_cal_118_data = &rx_band_cal_data_ptr->cal_118_data[element_index];
    path_idx = nv_cal_118_data->path_idx;
    MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH, "rfnv_wcdma_load_rx_dynamic_data_v2_nv: nv_cal_118_data path_idx is %d",
                                      nv_cal_118_data->path_idx);
    for (chan_num =0; chan_num <16; chan_num ++)
    {
      if (nv_cal_118_data->channel_list[chan_num] ==0)
      {
        break;
      }
    }
    ...
    chan_val = nv_cal_118_data->channel_list[center_chan];
    ...
    MSG_3(MSG_SSID_RF, MSG_LEGACY_HIGH, "rfnv_wcdma_load_rx_dynamic_data_v2_nv: chan_num %d center channel %d, index %d",chan_num, chan_val, center_chan);
	  ...
	  if(path_idx >= RF_PATH_MAX)
    {
      MSG_1( MSG_SSID_RF, MSG_LEGACY_ERROR,"rfnv_wcdma_load_rx_dynamic_data_v2_nv: "
             "path_idx= %d is not within range(>= RF_PATH_MAX)", path_idx);
    }
    else
    {
      switch (rx_band_cal_data_ptr->cal_118_data[element_index].cal_type)
      {
        case RFNV_RX_CAL_DATA_NV_TYPE_WCDMA_SC:        
          rx_dyn->rx_offsets[path_idx][0].vga_offset = nv_cal_118_data->freq_comp_offset[0][center_chan];
          rx_dyn->rx_offsets[path_idx][0].lna_offsets[0] = nv_cal_118_data->freq_comp_offset[1][center_chan];
          rx_dyn->rx_offsets[path_idx][0].lna_offsets[1] = nv_cal_118_data->freq_comp_offset[2][center_chan];
          rx_dyn->rx_offsets[path_idx][0].lna_offsets[2] = nv_cal_118_data->freq_comp_offset[3][center_chan];
          rx_dyn->rx_offsets[path_idx][0].lna_offsets[3] = nv_cal_118_data->freq_comp_offset[4][center_chan];
          rx_dyn->rx_offsets[path_idx][0].lna_offsets[4] = nv_cal_118_data->freq_comp_offset[5][center_chan];

          for (chan_index =0; chan_index < RFNV_NUMBER_OF_PCLS; chan_index++)
          {
            /* Update cal channels list */
            rx_dyn->rx_cal_chan[path_idx][chan_index] = nv_cal_118_data->channel_list[chan_index];

            /* Copy vga_gain_offset_vs_freq */
            rx_dyn->freq_offset[path_idx][chan_index].vga_gain_offset_vs_freq =
                nv_cal_118_data->freq_comp_offset[0][chan_index] -
                rx_dyn->rx_offsets[path_idx][0].vga_offset;

            /* Copy lna_offset_vs_freq_1 */
            rx_dyn->freq_offset[path_idx][chan_index].lna_offset_vs_freq_1 =
                nv_cal_118_data->freq_comp_offset[1][chan_index] -
                rx_dyn->rx_offsets[path_idx][0].lna_offsets[0];
          
            /* Copy lna_offset_vs_freq_2 */
            rx_dyn->freq_offset[path_idx][chan_index].lna_offset_vs_freq_2 =
                nv_cal_118_data->freq_comp_offset[2][chan_index] -
                rx_dyn->rx_offsets[path_idx][0].lna_offsets[1];

            /* Copy lna_offset_vs_freq_3 */
            rx_dyn->freq_offset[path_idx][chan_index].lna_offset_vs_freq_3 =
                nv_cal_118_data->freq_comp_offset[3][chan_index] -
                rx_dyn->rx_offsets[path_idx][0].lna_offsets[2];
 
            /* Copy lna_offset_vs_freq_4 */
            rx_dyn->freq_offset[path_idx][chan_index].lna_offset_vs_freq_4 =
                nv_cal_118_data->freq_comp_offset[4][chan_index] -
                rx_dyn->rx_offsets[path_idx][0].lna_offsets[3];
          }
          break;
      
        case RFNV_RX_CAL_DATA_NV_TYPE_WCDMA_DC:        
          rx_dyn->rx_offsets[path_idx][1].vga_offset = nv_cal_118_data->freq_comp_offset[0][0];
          rx_dyn->rx_offsets[path_idx][1].lna_offsets[0] = nv_cal_118_data->freq_comp_offset[1][0];
          rx_dyn->rx_offsets[path_idx][1].lna_offsets[1] = nv_cal_118_data->freq_comp_offset[2][0];
          rx_dyn->rx_offsets[path_idx][1].lna_offsets[2] = nv_cal_118_data->freq_comp_offset[3][0];
          rx_dyn->rx_offsets[path_idx][1].lna_offsets[3] = nv_cal_118_data->freq_comp_offset[4][0];
          rx_dyn->rx_offsets[path_idx][1].lna_offsets[4] = nv_cal_118_data->freq_comp_offset[5][0];
          break;

        case RFNV_RX_CAL_DATA_NV_TYPE_WCDMA_3C:        
          rx_dyn->rx_offsets[path_idx][2].vga_offset = nv_cal_118_data->freq_comp_offset[0][0];
          rx_dyn->rx_offsets[path_idx][2].lna_offsets[0] = nv_cal_118_data->freq_comp_offset[1][0];
          rx_dyn->rx_offsets[path_idx][2].lna_offsets[1] = nv_cal_118_data->freq_comp_offset[2][0];
          rx_dyn->rx_offsets[path_idx][2].lna_offsets[2] = nv_cal_118_data->freq_comp_offset[3][0];
          rx_dyn->rx_offsets[path_idx][2].lna_offsets[3] = nv_cal_118_data->freq_comp_offset[4][0];
          rx_dyn->rx_offsets[path_idx][2].lna_offsets[4] = nv_cal_118_data->freq_comp_offset[5][0];
          break;

        default:
          MSG_1( MSG_SSID_RF, MSG_LEGACY_ERROR,"rfnv_wcdma_load_rx_dynamic_data_v2_nv: "
                          "Invalid Variant Marker %d",nv_cal_118_data->cal_type);		
      }/* switch (type_id) */
    }/* if(nv_cal_data[element_index]->path_idx < RF_PATH_MAX) */
  }/* for(element_index=0; element_index < num_of_cal_118_elements; element_index++) */

  for(element_index=0; element_index < num_of_cal_124_elements; element_index++)
  { 
    nv_cal_124_data = &rx_band_cal_data_ptr->cal_124_data[element_index];
    path_idx = nv_cal_124_data->path_idx;
    MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH, "rfnv_wcdma_load_rx_dynamic_data_v2_nv: nv_cal_124_data path_idx is %d",
                                      nv_cal_124_data->path_idx);
	  if(path_idx >= RF_PATH_MAX)
    {
      MSG_1( MSG_SSID_RF, MSG_LEGACY_ERROR,"rfnv_wcdma_load_rx_dynamic_data_v2_nv: "
             "path_idx= %d is not within range(>= RF_PATH_MAX)", path_idx);
    ...
    {
      rx_dyn->rsb_cal_data[path_idx].valid = TRUE;
      rx_dyn->rsb_cal_data[path_idx].rsb_type = nv_cal_124_data->rsb_type;
      rx_dyn->rsb_cal_data[path_idx].dl_channel = nv_cal_124_data->dl_channel;
      rx_dyn->rsb_cal_data[path_idx].freq_offset_khz = nv_cal_124_data->freq_offset_khz;
      rx_dyn->rsb_cal_data[path_idx].bw = nv_cal_124_data->bw;
      memscpy(&rx_dyn->rsb_cal_data[path_idx].rsb_data,
              sizeof(rsb_data_list_per_gain)*RFNV_MAX_LNA_GAIN_STATES,
              &nv_cal_124_data->rsb_data,
              sizeof(rsb_data_list_per_gain)*RFNV_MAX_LNA_GAIN_STATES);
    }/* if(nv_cal_data[element_index]->path_idx < RF_PATH_MAX) */
  }/* for(element_index=0; element_index < num_of_cal_124_elements; element_index++) */
...
void rfnv_wcdma_set_rx_static_data_to_default(rfnv_wcdma_nv_tbl_type *rfnv_ptr)
{
  ...
  /* Additional data for AsDiv eLNA FR 30081, ptrs for Rx gain offsets and switchpoints*/
  for(nv_id = 0; nv_id < RFNV_CONTAINER_ID_MAX; nv_id++)
  {
    for(ant_index = 0; ant_index < RFWCDMA_NUM_ASDIV_POS; ant_index++)
    {
      rx_data_ptr->rx_asdiv_elna_offset_ptr[nv_id][ant_index] = NULL;
	  rx_data_ptr->rx_asdiv_elna_switchpoint_ptr[nv_id][ant_index] = NULL;
    }
  }
...
/*!
  @brief
    Set the Tx static NV data to default values
...
*/
void rfnv_wcdma_set_tx_static_data_to_default(rfnv_wcdma_nv_tbl_type *rfnv_ptr)
{
  rfwcdma_nv_tx_static_version_type tx_stat_version;
  rfwcdma_nv_tx_static_data_type *tx_stat_data = NULL;
  rfwcdma_nv_tx_static_payload_v1_type *tx_data_ptr = NULL ;
  uint8 nv_id = 0;
  uint8 ant_index = 0;  	

  /* Access the NV structure through the payload. There is only one versio V1 */
  tx_stat_data = &(rfnv_ptr->tx_static_data);
  tx_stat_data->tx_stat_variant_marker.version = RFWCDMA_NV_TX_STATIC_DATA_V1;
  tx_data_ptr = &(tx_stat_data->payload.tx_stat_v1);

  tx_stat_version = tx_stat_data->tx_stat_variant_marker.version ;

  for(nv_id = 0; nv_id < RFNV_CONTAINER_ID_MAX; nv_id++)
  {
    for(ant_index = 0; ant_index < RFWCDMA_NUM_ASDIV_POS; ant_index++)
	{
      tx_data_ptr->tx_asdiv_elna_offset[nv_id][ant_index] = 0;
...
void rfnv_wcdma_set_rx_cal_data_to_default(rfnv_wcdma_nv_tbl_type *rfnv_ptr)
{
  ...
  for(i = 0; i < RFNV_CONTAINER_ID_MAX ; i++)
  {
...
)
{
  ...
  if (band_convert < RFCOM_NUM_WCDMA_BANDS)
  {
    rfnv_wcdma_pa_static_nv_version[band_convert] = version;
  }

  if((version >= RFWCDMA_NV_PA_STATIC_VER_MAX) &&(band_convert < RFCOM_NUM_WCDMA_BANDS))
  {
    rfnv_wcdma_pa_static_nv_version[band_convert] = RFWCDMA_NV_PA_STATIC_VER_INVALID;
  }
}
