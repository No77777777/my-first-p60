...
$Header: //components/rel/rftech_wcdma.mpss/3.11/rf/core/src/rfwcdma_core_util.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/19/16   sub     Fix for F3 flood.
04/19/16   sub     RF F3 cleanup
02/12/16   ak      Read band info from rfwcdma_data in pa_range_map_nv_index
09/22/15   ak      Changes to fetch the BTF value correctly per band
07/22/15   asn     New RxAGC read API
06/24/15   ag      Fix the wrong allocation of tuner carrier id
06/18/15   vbh     Map carrier mask to carr_id for ant tuner 
06/12/15   aa      API to return number of active PA states
05/28/15   ac      add band as a paramter for rxagc 
05/18/15   vbh     Get device to antenna - logic fix 
04/30/15   ac      rxagc bringup related changes for w plus w
4/16/15    vbh     rfwcdma_core_util_is_sleeping - update argument to device
4/10/15    vbh     [Rx/Tx split] Updates for WCDMA data routine
04/04/15   ak      Rx/Tx Split Changes
02/11/15   dw      Added util function to get device pair and paired device
12/12/14   rmb     Change enum type for irat messurement.
...
int16 rfwcdma_core_util_get_rflm_txagc_dB10( void )
{
  ...
  if(rfwcdma_data_get_rf_state(device)== RFWCDMA_STATE_TX)
  {
    ...
    mdsp_param.rflm_handle = rfwcdma_data_get_rflm_buf_idx(device);
...
boolean rfwcdma_core_util_get_rflm_tx_on( void )
{
  ...
  {
     RF_MSG_1(RF_ERROR, "rfwcdma_core_util_get_rflm_tx_on: Failed as Tx device%d", device);
  }
  return tx_on;  
}
/*----------------------------------------------------------------------------*/
...
)
{
  ...
  param.data.get_rxagc_val.car_idx = rfwcdma_data_get_carrier_mask(device);
...
{
  rfwcdma_mdsp_get_data_param_type param;
  rfm_wcdma_antenna_index ant_idx = RFM_WCDMA_INVALID_ANTENNA;
  ant_idx = rfwcdma_core_util_device_to_antenna(device);

  param.get_data_cmd = RFWCDMA_MDSP_GET_RXAGC_VAL;
  param.data.get_rxagc_val.ant_idx = (uint8)ant_idx;
  param.data.get_rxagc_val.car_idx = (uint8)car_id;
  ...
  {
    MSG_2(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfwcdma_core_util_is_rxagc_on. device:%d,car_id:%d",
          device,car_id);
    return FALSE;
  }
...
)
{
  ...
  temp_rf_path = rfcommon_core_device_to_path(device);
  band = rfwcdma_data_get_curr_band(device);

  /* Band information extracted at this point */

  param.get_data_cmd = RFWCDMA_MDSP_GET_RXAGC_VAL;
  param.data.get_rxagc_val.ant_idx = rfwcdma_core_util_device_to_antenna(device);
  param.data.get_rxagc_val.car_idx = (uint8)car_id;
  ...
  /* MDSP API failed */
  {
    MSG_4(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfwcdma_core_util_get_rxagc failed. device:%d, rxagc_type:%d, car_id:%d, rxagc_unit:%d ",
          device,rxagc_type,car_id,rxagc_unit);
    *rxagc_val = -512;
  }
...
/*!
  ...
  @retval
  Boolean flag indicating api status
  Returns FALSE for invalid input RxAGC parameters.

*/

boolean rfwcdma_core_util_get_rxagc_info
(
  rfwcdma_core_util_rxagc_input_params *rxagc_input_info,
  rfwcdma_core_util_rxagc_output_params *rxagc_output_info
)
{
  rfwcdma_mdsp_get_data_param_type param;
  int16 rxagcval = 0; // Erroneous RxAGC
  ...
  if (NULL == rxagc_input_info) 
  {
       RF_MSG( RF_ERROR, "rfwcdma_core_util_get_rxagc_info: RxAGC input parameters is a NULL pointer");
       return FALSE;
  }

  if (NULL == rxagc_output_info) 
  {
       RF_MSG( RF_ERROR, "rfwcdma_core_util_get_rxagc_info: Structure to update RxAGC output parameters is a NULL pointer");
       return FALSE;
  }

  if ((rxagc_input_info->device < RFM_DEVICE_0) || (rxagc_input_info->device >= RFM_WCDMA_MAX_DEVICE))
  {
     RF_MSG_1(RF_ERROR, "rfwcdma_core_util_get_rxagc_info: Invalid device=%d is being passed", rxagc_input_info->device);
     return FALSE;
  }
  
  if ((rxagc_input_info->ant_idx < RFM_WCDMA_PRIMARY_ANTENNA) || (rxagc_input_info->ant_idx >= RFM_WCDMA_MAX_ANTENNA))
  {
     RF_MSG_2(RF_ERROR, "rfwcdma_core_util_get_rxagc_info: Invalid antenna index=%d is being passed for device=%d", rxagc_input_info->ant_idx, rxagc_input_info->device);
     return FALSE;
  }

  if ((rxagc_input_info->car_id < RFCOM_MULTI_CARRIER_ID__0) || (rxagc_input_info->car_id >= RFCOM_MULTI_CARRIER_ID__CUMULATIVE))
  {
     RF_MSG_3(RF_ERROR, "rfwcdma_core_util_get_rxagc_info: Invalid carrier index=%d is being passed for device=%d and antenna idx=%d", rxagc_input_info->car_id, rxagc_input_info->device, rxagc_input_info->ant_idx);
     return FALSE;
  }

  if ((rxagc_input_info->band < RFCOM_BAND_IMT) || (rxagc_input_info->band >= RFCOM_NUM_WCDMA_BANDS))
  {
     RF_MSG_3(RF_ERROR, "rfwcdma_core_util_get_rxagc_info: Invalid band=%d is being passed for device=%d and antenna idx=%d", rxagc_input_info->band, rxagc_input_info->device, rxagc_input_info->ant_idx);
     return FALSE;
  }

  rxagc_output_info->rxagc_on = FALSE;
  rxagc_output_info->rxagc_val = -512;

  /* Band information extracted at this point */

  param.get_data_cmd = RFWCDMA_MDSP_GET_RXAGC_VAL;
  param.data.get_rxagc_val.ant_idx = rxagc_input_info->ant_idx;
  param.data.get_rxagc_val.car_idx = (uint8)rxagc_input_info->car_id;

  /* Get RxAGC data from MDSP module */
  if (rfwcdma_mdsp_async_get_rx_data(&param))
  {
     /* Store the RxAGC on status to Output info */
     rxagc_output_info->rxagc_on = param.data.get_rxagc_val.rxagc_on;
    ...
    else
    {
      ...
      if ((rxagc_input_info->rxagc_type == RFCOM_INGAP_RXAGC) 
          || (rxagc_input_info->rxagc_type == RFCOM_SERVINGCELL_RXAGC)
          || (rxagc_input_info->rxagc_type == RFCOM_SERVINGCELL_FORCE_READ_RXAGC))
      {
         if (rfnv_wcdma_tbl_ptr[rxagc_input_info->band] == NULL)
         {
           rxagc_output_info->rxagc_val = -512;
           RF_MSG_1(RF_ERROR, "rfwcdma_core_util_get_rxagc_info(): NV table pointer is NULL for band %d", rxagc_input_info->band);
           return FALSE;
         }

         noise_floor = (float)(rfnv_wcdma_tbl_ptr[rxagc_input_info->band]->noise_floor);
       ...
       /* Check if we need to convert it to dBm unit */
       if (rxagc_input_info->rxagc_unit == RFCOM_RXAGC_IN_DBM)
       {
         rxagc_output_info->rxagc_val = (RF_MIN_RX_POWER_DBM + ((rxagcval - RFAGC_MIN_AGC_VAL) / 10));
       }

       else
       {
         rxagc_output_info->rxagc_val = rxagcval;
       }
  ...
  /* MDSP API failed */
  {
    MSG_5(MSG_SSID_RF, MSG_LEGACY_ERROR, "rfwcdma_core_util_get_rxagc_info failed. rxagc_type:%d, car_id:%d, rxagc_unit:%d, antenna idx:%d, band:%d",
          rxagc_input_info->rxagc_type, rxagc_input_info->car_id, rxagc_input_info->rxagc_unit, rxagc_input_info->ant_idx, rxagc_input_info->band);
    rxagc_output_info->rxagc_on = FALSE;
    rxagc_output_info->rxagc_val = -512;
    return FALSE;
  }
...
uint8 rfwcdma_core_util_get_lna_state(rfm_device_enum_type device)
{
  ...
  param.rflm_handle = rfwcdma_data_get_rflm_buf_idx(device);
...
{
  rfcom_wcdma_band_type band = rfwcdma_data_get_curr_band(device);// Use Rx band as Tx band has not been initialised
...
*/
uint8 rfwcdma_core_util_get_pa_range_from_nv(rfm_device_enum_type device,uint8 pa_index)
{
...
uint16 rfwcdma_core_util_get_beta_scaling_comp(uint8 mpr_idx)
{
  ...
  band = rfwcdma_data_get_curr_band(tx_dev);
...
*/
boolean rfwcdma_core_util_is_sleeping( rfm_device_enum_type device )
{
...
uint16 rfwcdma_core_get_enc_btf_val(rfcom_wcdma_band_type band)
{
...
uint8 rfwcdma_core_util_get_pa_range_map_nv_index(uint8 pa_range)
{
  ...
  band = rfwcdma_data_get_curr_band(rfwcdma_data_get_tx_device()); 
  ...
  /* PA range not found */
  RF_MSG_1(RF_ERROR,"rfwcdma_core_util_get_pa_range_map_nv_index: PA range not found for band: %d",band);
  return (0xFF);
}
...
*/
rfm_wcdma_antenna_index rfwcdma_core_util_device_to_antenna( rfcom_device_enum_type device )
{
  rfm_wcdma_antenna_index temp_antenna = RFM_WCDMA_INVALID_ANTENNA;
 /*Deprecate this API once state machine provides this info. Update for the current validation.*/
  if(RFM_WCDMA_PRIMARY_RX_DEVICE == rfwcdma_data_get_device_type(device))
  {
    temp_antenna = RFM_WCDMA_PRIMARY_ANTENNA;
  }
  /*We should be able to determine the device to antenna mapping even without the device doing an enter mode*/
  /*To Check for diversity : If the associated device of the device  passed is in primary mode*/
  else if(RFM_WCDMA_PRIMARY_RX_DEVICE == rfwcdma_data_get_device_type(rfc_common_get_preferred_associated_rx_device(device)))
  {
	temp_antenna = RFM_WCDMA_DIVERSITY_ANTENNA;
  }
  else /*@akash: Need to add case for DBDC devices and map them to antenna '0'*/
  {
    RF_MSG_1(RF_ERROR, "rfwcdma_core_util_device_to_antenna: Invalid device=%d no antenna index found.", device);
    temp_antenna = RFM_WCDMA_INVALID_ANTENNA;
  }
...
*/
uint8 rfwcdma_core_util_device_to_lna_id( rfcom_device_enum_type device, rfwcdma_mdsp_rx_irat_meas_enum_type meas_scenario, uint32 multi_carrier_id )
{
  uint8 lna_id = 0;

  /* workaround logic till CCS double buffer implementation is put in place   -----@vbhide*/

  if (meas_scenario == RFLM_WCDMA_HOME_MEAS) 
  {
    if (multi_carrier_id == 2)
    {
       lna_id = 1;
    }
...
/*!
  @details
  This function will return the paired device (PRx/DRx pair)

  @param device: device ID

  @return rfm_device_enum_type: paired device 
*/

rfm_device_enum_type rfwcdma_core_get_paired_device(rfm_device_enum_type device)
{
  ...
  {
  case RFM_DEVICE_0:
    paired_device = RFM_DEVICE_1;
    break;
    
  case RFM_DEVICE_1:
    paired_device = RFM_DEVICE_0;
    break;

  case RFM_DEVICE_2:
    paired_device = RFM_DEVICE_3;
    break;

  case RFM_DEVICE_3:
    paired_device = RFM_DEVICE_2;
    break;

  case RFM_DEVICE_4:
    paired_device = RFM_DEVICE_5;
    break;

  case RFM_DEVICE_5:
    paired_device = RFM_DEVICE_4;
    break;

  default:
    paired_device = RFM_INVALID_DEVICE;
    break;
  }
...
/*!
  @details
  This function will the device pair index for the given device

  @param device: device ID

  @return uint8 device pair ID
*/

uint8 rfwcdma_core_get_device_pair(rfm_device_enum_type device)
{
  uint8 dev_pair;
...
/*!
  @brief
  This function gets the min and max Rx carrier freq (in KHz) of a WCDMA band
  ...
  boolean TRUE if band is supported, FALSE otherwise
*/
boolean rfwcdma_core_util_get_min_max_rx_carrier_freqs(rfcom_wcdma_band_type band, uint32* min, uint32* max)
{
  boolean ret_val= TRUE;
       ...
       (max != NULL) )
  {
    ...
    {
      case RFCOM_BAND_IMT:
        *min = BSP_RF_RX_FREQ_WCDMA_MIN + 2400;
        *max = BSP_RF_RX_FREQ_WCDMA_MAX - 2400;
      break;

      case RFCOM_BAND_1900:
        *min = BSP_RF_RX_FREQ_WCDMA_BC2_MIN + 2400;
        *max = BSP_RF_RX_FREQ_WCDMA_BC2_MAX - 2400;
      break;

      case RFCOM_BAND_BC3:
        *min = BSP_RF_RX_FREQ_WCDMA_BC3_MIN + 2400;
        *max = BSP_RF_RX_FREQ_WCDMA_BC3_MAX - 2400;
      break;

      case RFCOM_BAND_BC4:
        *min = BSP_RF_RX_FREQ_WCDMA_BC4_MIN + 2400;
        *max = BSP_RF_RX_FREQ_WCDMA_BC4_MAX - 2400;
      break;

      case RFCOM_BAND_800:
        *min = BSP_RF_RX_FREQ_WCDMA_BC5_MIN + 2400;
        *max = BSP_RF_RX_FREQ_WCDMA_BC5_MAX - 2400;
      break;

      case RFCOM_BAND_BC8:
        *min = BSP_RF_RX_FREQ_WCDMA_BC8_MIN + 2400;
        *max = BSP_RF_RX_FREQ_WCDMA_BC8_MAX - 2400;
        break;

      case RFCOM_BAND_BC9:
        *min = BSP_RF_RX_FREQ_WCDMA_BC9_MIN + 2400;  //CORRECTED
        *max = BSP_RF_RX_FREQ_WCDMA_BC9_MAX - 2400;  //CORRECTED
        break;

      case RFCOM_BAND_BC11:
        *min = BSP_RF_RX_FREQ_WCDMA_BC11_MIN + 2400;
        *max = BSP_RF_RX_FREQ_WCDMA_BC11_MAX - 2400;
        break;

      case RFCOM_BAND_BC19:
        *min = BSP_RF_RX_FREQ_WCDMA_BC19_MIN + 2400;
        *max = BSP_RF_RX_FREQ_WCDMA_BC19_MAX - 2400;
        break;

      default:
        MSG_1(MSG_SSID_RF, MSG_LEGACY_MED,"rfwcdma_core_util_get_min_max_rx_carrier_freqs : Invalid WCDMA band %d!", band);  
        ret_val = FALSE;
    } // end switch
  }
  else
  {
    MSG_1(MSG_SSID_RF,MSG_LEGACY_ERROR,"NULL pointers detected in rfwcdma_core_util_get_min_max_rx_carrier_freqs for band %d",
                                       band);
    ret_val = FALSE;
  }

  return ret_val;
} /*rfwcdma_core_util_get_min_max_rx_carrier_freqs*/

/*----------------------------------------------------------------------------*/
/*!
  @brief
  This function gets the min and max Tx carrier freq (in KHz) of a WCDMA band
  ...
  boolean TRUE if band is supported, FALSE otherwise
*/
boolean rfwcdma_core_util_get_min_max_tx_carrier_freqs(rfcom_wcdma_band_type band, uint32* min, uint32* max)
{
  boolean ret_val= TRUE;
       ...
       (max != NULL) )
  {
    ...
    {
      case RFCOM_BAND_IMT:
        *min = BSP_RF_TX_FREQ_WCDMA_MIN + 2400;
        *max = BSP_RF_TX_FREQ_WCDMA_MAX - 2400;
        break;

      case RFCOM_BAND_1900:
        *min = BSP_RF_TX_FREQ_WCDMA_BC2_MIN + 2400;
        *max = BSP_RF_TX_FREQ_WCDMA_BC2_MAX - 2400;
        break;

      case RFCOM_BAND_BC3:
        *min = BSP_RF_TX_FREQ_WCDMA_BC3_MIN + 2400;
        *max = BSP_RF_TX_FREQ_WCDMA_BC3_MAX - 2400;
        break;

      case RFCOM_BAND_BC4:
        *min = BSP_RF_TX_FREQ_WCDMA_BC4_MIN + 2400;
        *max = BSP_RF_TX_FREQ_WCDMA_BC4_MAX - 2400;
        break;

      case RFCOM_BAND_800:
        *min = BSP_RF_TX_FREQ_WCDMA_BC5_MIN + 2400;
        *max = BSP_RF_TX_FREQ_WCDMA_BC5_MAX - 2400;
        break;

    case RFCOM_BAND_BC8:
        *min = BSP_RF_TX_FREQ_WCDMA_BC8_MIN + 2400;
        *max = BSP_RF_TX_FREQ_WCDMA_BC8_MAX - 2400;
        break;

    case RFCOM_BAND_BC9:
        *min = BSP_RF_TX_FREQ_WCDMA_BC9_MIN + 2400;
        *max = BSP_RF_TX_FREQ_WCDMA_BC9_MAX - 2400;
        break;

    case RFCOM_BAND_BC11:
        *min = BSP_RF_TX_FREQ_WCDMA_BC11_MIN + 2400;
        *max = BSP_RF_TX_FREQ_WCDMA_BC11_MAX - 2400;
        break;

    case RFCOM_BAND_BC19:
        *min = BSP_RF_TX_FREQ_WCDMA_BC19_MIN + 2400;
        *max = BSP_RF_TX_FREQ_WCDMA_BC19_MAX - 2400;
        break;

    default:
        MSG_1(MSG_SSID_RF, MSG_LEGACY_MED,"rfwcdma_core_util_get_min_max_tx_carrier_freqs : Invalid WCDMA band %d!", band);  
        ret_val = FALSE;
    } // end switch
  }
  else
  {
    MSG_1(MSG_SSID_RF,MSG_LEGACY_ERROR,"NULL pointers detected in rfwcdma_core_util_get_min_max_tx_carrier_freqs for band %d",
                                       band);
    ret_val = FALSE;
  }
  return ret_val;
} /*rfwcdma_core_util_get_min_max_tx_carrier_freqs*/

uint8 rfwcdma_core_get_num_pa_states(rfcom_wcdma_band_type band)
{
  return rfwcdma_core_txlin_get_num_valid_pa_state(band);
  
}
/*----------------------------------------------------------------------------*/
/*!
  @brief
  This util routine maps the carrier mask to id as needed for antenna tuner apis
 
  @param carr_mask
  The carrier mask for the Rx/Tx scenario
 
  @return
  rfcommon_atuner_carrier_id_type enum 

  @details
 	                 Sub 1	Sub 0	
  Carrier Status	 C3	C2	C1 C0	       Carrier_id
  SC            	 0	0	  0	1	         RFCMN_ATUNER_CARRIER_ID_0
  DBDC        	     0	0	  1	0	         RFCMN_ATUNER_CARRIER_ID_1
  SBDC             	 0	0	  1	1	         RFCMN_ATUNER_CARRIER_ID_0_PLUS_1
  SC	             0	1	  0	0	         RFCMN_ATUNER_CARRIER_ID_0
  DBDC         	     1	0	  0	0	         RFCMN_ATUNER_CARRIER_ID_1
  SBDC             	 1	1	  0	0	         RFCMN_ATUNER_CARRIER_ID_0_PLUS_1

*/


rfcommon_atuner_carrier_id_type rfwcdma_core_map_carr_mask_to_atuner_id(uint32 carr_mask)
{
  uint8 carr_id =0;
  uint32 sub_mask = 0x0003;
  uint8 num_carr = rf_count_one_bits(carr_mask);

  rfcommon_atuner_carrier_id_type atuner_carr_id = RFCMN_ATUNER_CARRIER_ID_INVALID; 

  if (carr_mask == 0)
  {
     RF_MSG_1(RF_ERROR, "rfwcdma_core_map_carr_mask_to_atuner_id: Invalid carrier mask = %d", carr_mask);
     return RFCMN_ATUNER_CARRIER_ID_INVALID;
  }

  /*Check if mode is Sub0 or Sub1, if Sub1, right shift the carrier mask*/
  if ((carr_mask & (~sub_mask))!= 0)
  {
    carr_mask = carr_mask >> rf_count_one_bits(sub_mask);
  }

  carr_id = rf_extract_first_one_bit_lsb_location(carr_mask);

  if((carr_id == RFCOM_SINGLE_CARRIER) && (num_carr == 1))
  {
    atuner_carr_id = RFCMN_ATUNER_CARRIER_ID_0;
  }
  else if ((carr_id == RFCOM_SINGLE_CARRIER) && (num_carr == 2)) 
  {
    atuner_carr_id = RFCMN_ATUNER_CARRIER_ID_0_PLUS_1;
  }
  else if (carr_id == RFCOM_DUAL_CARRIER) 
  {
    atuner_carr_id = RFCMN_ATUNER_CARRIER_ID_1;
  }
  else
  {
   RF_MSG_2(RF_ERROR,"rfwcdma_core_map_carr_mask_to_atuner_id:invalid carr_id %d, carr_mask%d",
                                                                            carr_id,carr_mask );
  }

  return atuner_carr_id;
