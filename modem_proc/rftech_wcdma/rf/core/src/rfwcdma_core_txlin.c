...
$Header: //components/rel/rftech_wcdma.mpss/3.11/rf/core/src/rfwcdma_core_txlin.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/18/17   sk      Send Tx handle to XPT api's for LTE
07/18/16   ag      Correct the autopin check for temp comp update 
...
01/12/15   ag      Apply Pin Ref at Tx tune
12/09/15   ag      Don't apply pout in temp comp udate when autopin is enabled
11/20/15   sk      Pass Tx APT CHAR data by reference to avoid stack overflow
11/03/15   ag      Pout compensation to be applied on Tabasco modem irrespective of the FBRX state
10/06/15    ak     Fix for Power cap and Power boost not working
09/24/15   asn     Fix KW error
09/23/15    pv     Added APT char for Intra-band ULCA LUT construction.
07/20/15   nv      Update DPD scaling fo DC case properly 
07/27/15   cdb     Add runtime check for Autopin enabled
06/23/15    pv     Intra-band ULCA LUT hook up.
06/12/15   aa      autopin feature
05/27/15   vbh     Do not fail txlin update if temp comp fails    
...
02/12/14   dw      Change AMAM expansion ratio of DC to the same as SC (4)
                                   ...
                                   uint32 txlm_handle )
{
  ...
  boolean status_flag = TRUE, temp_comp_flag = TRUE;
  uint8 pa_index = 0;
  ...
  rfm_bw.wcdma_bw = RFCOM_BW_WCDMA_5MHz;
  cal_rfm_bw.wcdma_bw = RFCOM_BW_WCDMA_5MHz;
  ...
  {
    rf_common_xpt_init_dpd( device, NULL );
  }
  ...
  **********************************************************************/
  for ( pa_index = 0; pa_index < rfwcdma_core_current_txlin_data.num_valid_pa_state; pa_index++)
  {
     ...
     *************************************************************************************************/
    {
      ...
      temp_comp_flag  &= rfcommon_core_txlin_temp_vs_freq_comp_update(
                                                        rfwcdma_core_current_txlin_data.temp_comp_algo,
                                                        &txlin_cur_table,
                                                        &txlin_final_table,
                                                        &txlin_first_sweep_tbl,
    ...
    /* Common load linearizer */
    {
      ...
      status_flag &= rfwcdma_core_txctl_get_device_info( device,
                                          band, 
                                          &device_info );

      if (FALSE == status_flag) 
      {
         RF_MSG_2(RF_ERROR, "rfwcdma_core_txlin_update: rfwcdma_core_txctl_get_device_info failed for device=%d and band=%d", device, band);
         status_flag = TRUE;
         continue;
      }
      ...
      pa_stm_info.pa_range = rfwcdma_core_util_get_pa_range_from_nv(device,pa_index);

      /* Configure the linearizer to RFLM through common Tx AGC */ 
      status_flag &= rfcommon_core_txlin_config_init_linearizer( RFM_IMT_MODE, device, 
  ...
  if(rfwcdma_core_xpt_is_enabled(band))
  {
    rf_common_xpt_commit_dpd_data( device, NULL );
  }
...
)
{
  ...
  /*No need to populate linearizer data to MDSP memory in FTM mode*/
  if ( (IS_FTM_IN_TEST_MODE()&&(rfm_get_calibration_state() == TRUE))||
       (rfwcdma_data_get_rf_state(device) != RFWCDMA_STATE_TX))
  {
    return FALSE;  
  }
  ...
  for ( pa_index = 0; pa_index < rfwcdma_core_current_txlin_data.num_valid_pa_state; pa_index++)
  {
    ...
    /* Search first sweep linearizer data */
    curr_band_tx = rfwcdma_data_get_curr_band(device);
    ...
    txlin_cur_table.aux_lin_data = rfwcdma_core_current_txlin_data.current_aux_data[pa_index];
      ...
      if ( !(rfcommon_autopin_is_enabled_flag_check(rfwcdma_data_get_rflm_buf_idx(device)) && rflm_autopin_sanity_check(rfwcdma_data_get_rflm_buf_idx(device))
            && is_autopin_supported) || ( txlin_cur_table.aux_lin_data == TX_LINEARIZER_NO_AUX_DATA ))
      {    
           status_flag  &= rfcommon_core_txlin_temp_vs_freq_comp_update(
                                                          ...
                                                          pin_comp_val[pa_index],
                                                          pout_comp_val[pa_index],
                                                          rfwcdma_core_current_txlin_data.txlin_valid_entries[pa_index],FALSE);
      }
    ...
    if(rfcommon_fbrx_mc_is_fbrx_enabled(RFLM_TECH_WCDMA))
    {
      if ( !(rfcommon_autopin_is_enabled_flag_check(rfwcdma_data_get_rflm_buf_idx(device)) && rflm_autopin_sanity_check(rfwcdma_data_get_rflm_buf_idx(device)))
          || ( txlin_cur_table.aux_lin_data == TX_LINEARIZER_NO_AUX_DATA ))
      {    
            status_flag  &= rfcommon_core_txlin_temp_vs_freq_comp_update(
                                                          rfwcdma_core_current_txlin_data.temp_comp_algo,
                                                          &txlin_cur_table,
                                                          ...
                                                          pin_comp_val[pa_index],
                                                          0,
                                                          rfwcdma_core_current_txlin_data.txlin_valid_entries[pa_index],TRUE);
            pout_comp_val[pa_index] = 0;
       }
       else
       {   
    ...
    {
      /* Update common band information */
      rfcom_band.wcdma_band = rfwcdma_data_get_curr_band(device);;
      ...
      if (rfwcdma_data_get_rf_state(device) != RFWCDMA_STATE_TX)
      {
              ...
              && is_autopin_supported) || ( txlin_cur_table.aux_lin_data == TX_LINEARIZER_NO_AUX_DATA ))
        {
          status_flag &= rfcommon_core_txlin_config_update_linearizer( 
                           RFM_IMT_MODE, device,
                           rfcom_band, rfwcdma_data_get_rflm_buf_idx(device),
                           &txlin_final_table, 
                           &txlin_first_sweep_tbl,
                           &pa_stm_info, FALSE );
         }
          ...
          if(rfcommon_fbrx_mc_is_fbrx_enabled(RFLM_TECH_WCDMA))
          {
            if ( !(rfcommon_autopin_is_enabled_flag_check(rfwcdma_data_get_rflm_buf_idx(device)) 
                && rflm_autopin_sanity_check(rfwcdma_data_get_rflm_buf_idx(device)))
                ||(txlin_cur_table.aux_lin_data == TX_LINEARIZER_NO_AUX_DATA ))
           {
              status_flag &= rfcommon_core_txlin_config_update_linearizer(
                               RFM_IMT_MODE, device,
  ...
  txlin_temp_vs_freq.set_data_cmd = RFWCDMA_MDSP_SET_PIN_POUT;
  txlin_temp_vs_freq.rflm_handle = (rflm_dm_handle_id_t)rfwcdma_data_get_rflm_buf_idx(device);
  rfwcdma_mdsp_async_set_data(&txlin_temp_vs_freq);
...
)
{
  ...
  cap_boost_data.lin_cap_boost_nv_val = rfwcdma_core_xpt_get_cfg_param( 
    ...
    RF_MSG_2( RF_MED, "rfwcdma_core_txlin_auto_pin_comp_update current_band = %d, tx_freq = %d, pa_index =%d " , curr_band_tx,  search_param.tx_freq);
    ...
    RF_MSG_1( RF_MED, "WCDMA Tx Lin update auto pin: aux_lin_data = %d ", txlin_cur_table.aux_lin_data );

    if ( ( txlin_cur_table.aux_lin_data != TX_LINEARIZER_NO_AUX_DATA ) && ( txlin_cur_table.aux_lin_data != TX_LINEARIZER_DPD_INVALID ) )
      {
  ...
  RF_MSG_1( RF_LOW, "TxLin AutoPin Comp finshed, Pin comp values:%d ", pin_comp_val);
