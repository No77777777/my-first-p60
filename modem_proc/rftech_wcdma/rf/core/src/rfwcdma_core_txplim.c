...
$Header: //components/rel/rftech_wcdma.mpss/3.11/rf/core/src/rfwcdma_core_txplim.c#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
08/25/16   ag      Use correct state machine for tx handle during meter unlock
06/30/16   vs      Pass correct tx device while querying the MTPL per asdiv pos
04/28/16   sub     Initialize the MTPL every Tx Config
04/19/16   sub     RF F3 cleanup
04/05/15   ak      FR 32751:SAR based AsDiv Disable changes 
01/06/16   ak      Fix to get band info from rfwcdma_data in SAR MTPL calculations 
12/17/15   ag      Release meter lock for hdet in SW callback if Tx is disabled before
12/01/15   ag      Fix KW error
11/30/15   sd      Fix mismatch in SAR backoff value and NV settings
11/23/15   ag      Update meter in progress to false in callback API in SW
08/18/14   sd      Added an API to return expected MTPL based on AdDiv position and SAR NV
06/29/14   sd      FR 27572 update SAR backoff value based on AsDiv position
07/01/15   zhh     take the mtpl_adj_db10 into the mtpl calc
06/02/15   gh/zhh  Add timing information MSG for METER read
06/24/15   vs      Fix dev mapping for HDET read
...
/* last software value for MCS Max Tx power limit (MTPL) as an Q5 value */
/* this TxAGC value in 0..1023 min..max units shifted by 5 */
LOCAL uint32  rfwcdma_core_txplim_mcs_mtpl_q5 = RFTXPLIM_INT_TO_Q5(RFTXPLIM_MAX_TX_PWR_LIM_IN_SW_UNIT);
...
void rfwcdma_core_txplim_init(rfcom_wcdma_band_type band,uint16 channel, uint32 multi_carrier_idx)
{
  ...
  {
    power_meter_config_param.rflm_handle = (rflm_dm_handle_id_t)rfwcdma_data_get_rflm_buf_idx(rfm_tx_device);
  }
  ...
  if(rfm_tx_device < RFM_MAX_WAN_DEVICES)
  {
    set_periodicity_param.rflm_handle = (rflm_dm_handle_id_t)rfwcdma_data_get_rflm_buf_idx(rfm_tx_device);
  }
...
void rfwcdma_core_txplim_set_mtpl( int16 utran_limit_dBm, rfcom_txplim_channel_type chan_type)
{
  ...
  mpr_data.rflm_handle = (rflm_dm_handle_id_t)rfwcdma_data_get_rflm_buf_idx(rfm_tx_device);
...
/*!
  @brief
   This function set the MCS Max tx power limit(MTPL) based on a new MCS TX 
   power limit value which is specified in dbm unit.

   @details


*/
void rfwcdma_core_txplim_set_mcs_mtpl( int16 mcs_limit_dBm) 
{
  int16 mcs_limit_db10;

  mcs_limit_db10 = (mcs_limit_dBm-RF_MIN_TX_POWER_DBM) * RF_TXAGC_RESOLUTION; /*lint !e734 */
 
  /* MCS Max Tx Power Limit (MTPL) */
  rfwcdma_core_txplim_mcs_mtpl_q5 = RF_SATURATE( RFTXPLIM_INT_TO_Q5(mcs_limit_db10),
                                            RFTXPLIM_INT_TO_Q5(RFTXPLIM_TX_AGC_MIN),
                                            RFTXPLIM_INT_TO_Q5(RFTXPLIM_TX_PWR_LIM_MAX)); /*lint !e734 */
}
...
LOCAL void rfwcdma_core_txplim_set_desired_limit(void)
{
  ...
  rfcom_wcdma_band_type band = RFCOM_BAND_INVALID;
   ...
   /*Update the rfm_device for tx*/
  rfm_tx_device = rfwcdma_data_get_tx_device();
   /*Make sure Tx band information is updated before that*/ 
  ...
  if ((!IS_FTM_IN_TEST_MODE()) || (rfm_get_calibration_state() == FALSE))
  {
    ...
    log_data = rfcommon_core_txlin_get_log_data(RFCOM_WCDMA_MODE,rfm_tx_device);
...
/*!
  @brief
  Returns MCS Max TX power limit(MTPL) obtained by the limiting loop

  @details

*/
int16 rfwcdma_core_txplim_get_mcs_mtpl(void)
{
  return(RFTXPLIM_Q5_TO_INT(rfwcdma_core_txplim_mcs_mtpl_q5) + RFAGC_MIN_AGC_VAL);
}
...
void rfwcdma_core_txplim_do_vbatt_based_backoff(void)
{
 ...
 rfm_tx_device = rfwcdma_data_get_tx_device();
 band = rfwcdma_data_get_curr_band(rfm_tx_device);
 /*Temp based compensation should happen only in traffic state. Tx device should have done enter mode. 
 	Code flow should make sure we have the band information updated at this point*/
...
LOCAL void rfwcdma_core_txplim_adjust_mtpl(void)
{
  ...
  {
    mtpl_adj_db10 = -rfwcdma_mdsp_async_get_mtpl_adj_db10(rfwcdma_data_get_rflm_buf_idx( device));
  }
  else
  {
    RF_MSG_1(RF_ERROR,"rfwcdma_core_txplim_adjust_mtpl: Invalid device %d", device);
  }
  /* HDET Cal Error */
...
{  
  rfwcdma_asdiv_position_enum_type asdiv_pos = ASDIV_POS_0;
  rfm_device_enum_type rfm_tx_device = RFM_MAX_WAN_DEVICES; 
  rfcom_wcdma_band_type band = RFCOM_BAND_INVALID; 
  ...
  rfm_tx_device = rfwcdma_data_get_tx_device();
  band = rfwcdma_data_get_curr_band(rfm_tx_device);
  /*Band information should be fetched from the tx device.*/

  RF_MSG_2(RF_HIGH,"rfwcdma_core_txplim_set_sar_limit API called with sar_index %d, band is %d",sar_index, band);
  ...
  if(sar_index > RF_SAR_STATE_DEFAULT && sar_index < RF_SAR_STATE_MAX)
  {
    ...
    /* NV contains 8 elements as it does not include the default state */
      ...
      if (band < RFCOM_NUM_WCDMA_BANDS)
      {
		...
		asdiv_pos = rfcommon_asdiv_get_current_position(RFM_DEVICE_0);
		#endif
		
		RF_MSG_1(RF_HIGH,"rfwcdma_core_txplim_set_sar_limit current asdiv position is %d", asdiv_pos);
				
		if(asdiv_pos == ASDIV_POS_0) 
		{
		   /*Load legacy SAR NV values*/
          if (rfnv_wcdma_tbl_ptr[band] == NULL)
          {
            RF_MSG_1(RF_ERROR,
		...
		else if(asdiv_pos == ASDIV_POS_1)
		{
		  /*Check if the new SAR NV is enabled for the band, else load legacy NV values*/
		  if (rfcommon_nv_tbl.rfwcdma_nv_sar_asdiv_table[band].band_enabled == TRUE)
		  {		 
		    
			RF_MSG_1(RF_HIGH,"rfwcdma_core_txplim_set_sar_limit SAR NV enabled for AsDiv pos 1, band %d",band);   
		    rfwcdma_core_txplim_sar_limit_db10 = rfcommon_nv_tbl.rfwcdma_nv_sar_asdiv_table[band].sar_back_off_table_asdiv_pos_1[sar_index-1];
		  }
		  else
		  {
		    /*Load legacy SAR NV values*/
		...
		{
		  RF_MSG_1(RF_ERROR,
                   "rfwcdma_core_txplim_set_sar_limit: AsDiv position is incorrect %d",
                   asdiv_pos);	
        }
  ...
  /*FR 32751- Check if the SAR limit is AsDiv forced antenna limit.*/  
  if(rfwcdma_core_txplim_sar_limit_db10 == RF_SAR_FORCE_OTHER_ANTENNA)
  {
    /*Update to default limit to avoid uint16 integer overflow*/
    rfwcdma_core_txplim_sar_limit_db10 = RFTXPLIM_TX_PWR_LIM_MAX - RFTXPLIM_SAR_NV_SCALE_FACTOR;
  }
  ...
  RF_MSG_1(RF_HIGH,"rfwcdma_core_txplim_set_sar_limit SAR limit set, value in AGC uints %d",rfwcdma_core_txplim_sar_limit_db10);   
...
)
{
  ...
  rfm_tx_device = rfwcdma_data_get_tx_device();
   ...
   if (rfm_tx_device >= RFM_MAX_WAN_DEVICES ) 
   {    
     RF_MSG_1( RF_ERROR, "rfwcdma_core_txplim_do_mtpl: invalid tx device %d, returning from here", 
                          rfm_tx_device);
     return;
   }
  ...
  rf_band = rfwcdma_data_get_curr_band(rfm_tx_device);
  rf_state = rfwcdma_data_get_rf_state(rfm_tx_device);
  irat_script = rfwcdma_mc_state.irat_script;

  if((RFWCDMA_STATE_TX != rf_state)||(irat_script== TRUE))
  {
    RF_MSG_4( RF_HIGH, "rfwcdma_core_txplim_do_mtpl: invalid rf_state=%d or irat_script_building %d"
                        " device=%d rf_band=%d", 
                         rf_state,irat_script, rfm_tx_device, rf_band);
    ...
    {
      rflm_wcdma_unlock_meter_trigger((rflm_dm_handle_id_t)rfwcdma_data_get_rflm_buf_idx(rfm_tx_device));
    }
  }
  ...
  if(TRUE == hdet_read_en)
  {
     ...
     if (!fbrx_data->uses_ilpc_data)
     {
    (void)rfdevice_hdet_wcdma_read_hdet( rfm_tx_device,
                                         rf_band,
                                         TRUE,
                                         &hdet_output );
       rflm_wcdma_unlock_meter_trigger( (rflm_dm_handle_id_t)rfwcdma_data_get_rflm_buf_idx(rfm_tx_device) );
     ...
     else
     {
        ...
        /* @TODO: Remove debug msg later */
        RF_MSG_7( RF_HIGH,
                  "rfwcdma_core_txplim_do_mtpl_handler DBG: "
    ...
    mdsp_param.rflm_handle = (rflm_dm_handle_id_t)rfwcdma_data_get_rflm_buf_idx(rfm_tx_device);
    ...
    set_power_meter_state.rflm_handle = (rflm_dm_handle_id_t)rfwcdma_data_get_rflm_buf_idx(rfm_tx_device);
    set_power_meter_state.data.gen_b = FALSE;
    rfwcdma_mdsp_async_set_data(&set_power_meter_state);
  }

       
  buffer_valid = rflm_dm_get_handle_tech_data((rflm_dm_handle_id_t)rfwcdma_data_get_rflm_buf_idx(rfm_tx_device), (void *)&wcdma_dm_data_ptr, RFLM_DM_TECH_DATA_GET_PTR);
...
/*!
  @brief
  Returns expected MTPL value based on AsDiv Posotion and SAR backoff NV

  @details
  Calculates 
  1) SAR Backoff limit for the asdiv pos
  2) Expected MTPLl value based on the SAR backoff

  @param asdiv pos
  
*/
int16 rfwcdma_core_txplim_calculate_mtpl_asdiv_pos(int8 asdiv_position)
{
  int16 txplim_mtpl_q5 = RFTXPLIM_INT_TO_Q5(RFTXPLIM_MAX_TX_PWR_LIM_IN_SW_UNIT);  
  rfcom_wcdma_band_type band = RFCOM_BAND_INVALID;
  boolean mtpl_first_time_flag = rfwcdma_core_txplim_first_time_flag;
  int16 old_limit = rfwcdma_core_txplim_desired;
  int16 txplim_mtpl_diff = 0;
  int16 txplim_ntpl = RFTXPLIM_MAX_TX_PWR_LIM_IN_SW_UNIT;
  int16 txplim_lin = RFTXPLIM_MAX_TX_PWR_LIM_IN_SW_UNIT;
  const rfcommon_core_txlin_log_data_type* log_data;
  int16 sar_limit_db10 = RFTXPLIM_MAX_TX_PWR_LIM_IN_SW_UNIT;
  int16 txplim_desired = RFTXPLIM_MAX_TX_PWR_LIM_IN_SW_UNIT;
  int16 txplim_desired_prev = rfwcdma_core_txplim_desired;
  rfm_common_sar_state_type sar_index = RF_SAR_STATE_DEFAULT;
  rfwcdma_asdiv_position_enum_type asdiv_pos = (rfwcdma_asdiv_position_enum_type)asdiv_position;
  rfm_device_enum_type rfm_tx_device;
  /******STEP 1 Get SAR index, calculate SAR BACKOFF LIMIT*******************************************************************/
  
  /*Fetch the Tx device*/ 
  rfm_tx_device = rfwcdma_data_get_tx_device();
  if((rfm_tx_device < RFM_DEVICE_0) || (rfm_tx_device >= RFM_WCDMA_MAX_DEVICE))
  {
    RF_MSG_1(RF_MED, "rfwcdma_core_txplim_calculate_mtpl_asdiv_pos: Invalid tx device %d",rfm_tx_device);
	return 0;
  }
  /*Band information should be fetched from the tx device.*/
  band = rfwcdma_data_get_curr_band(rfm_tx_device);
  
  sar_index = rfcommon_core_sar_get_state();
  ...
  if(sar_index > RF_SAR_STATE_DEFAULT && sar_index < RF_SAR_STATE_MAX)
  {
    ...
    if (band < RFCOM_NUM_WCDMA_BANDS)
	{
	  ...
	  if (rfnv_wcdma_tbl_ptr[band] == NULL)
	  {
		RF_MSG_1(RF_ERROR, "rfwcdma_core_txplim_calculate_mtpl_asdiv_pos: NULL NV tbl ptr for band %d",band);
	  }
	  else
	  {
		sar_limit_db10 = rfnv_wcdma_tbl_ptr[band]->wcdma_db10_sar_back_off_limit[sar_index-1];
	  }	  
	  ...
	  else if(asdiv_pos == ASDIV_POS_1)
	  {
		...
		if (rfcommon_nv_tbl.rfwcdma_nv_sar_asdiv_table[band].band_enabled == TRUE)
		{		  
		  sar_limit_db10 = rfcommon_nv_tbl.rfwcdma_nv_sar_asdiv_table[band].sar_back_off_table_asdiv_pos_1[sar_index-1];
		}
		else
		{
		  /*Load legacy SAR NV values*/
	  ...
	  {
		RF_MSG_1(RF_ERROR,"rfwcdma_core_txplim_calculate_mtpl_asdiv_pos: AsDiv position is incorrect %d", asdiv_pos);  
		return 0;
	  }
	...
	{
	   RF_MSG_1(RF_ERROR,"rfwcdma_core_txplim_calculate_mtpl_asdiv_pos: Not a valid band %d",band);
	   return 0; 
	}
  }
  else
  {
    sar_limit_db10 = RFTXPLIM_TX_PWR_LIM_MAX - RFTXPLIM_SAR_NV_SCALE_FACTOR;
  }

  /*FR 32751- Check if the SAR limit is AsDiv forced antenna limit.*/  
  if((uint16)sar_limit_db10 == RF_SAR_FORCE_OTHER_ANTENNA)
  {
    /*Update to default limit to avoid uint16 integer overflow*/
    sar_limit_db10 = RFTXPLIM_TX_PWR_LIM_MAX - RFTXPLIM_SAR_NV_SCALE_FACTOR;
  }
  ...
  /* Convert absoulte dB10 to AGC units */
  sar_limit_db10 += RFTXPLIM_SAR_NV_SCALE_FACTOR;

  RF_MSG_2(RF_HIGH,"rfwcdma_core_txplim_calculate_mtpl_asdiv_pos: STEP 1 SAR backoff limit value calculated in AGC units %d, band %d",sar_limit_db10,band);


  /********STEP 2: Calculate the DESIRED LIMIT based on UTRAN TX power limit value and NV TX power limit along with lim_vs_freq***************************/

  txplim_ntpl = rfwcdma_core_txplim_get_mtpl_nv_item(rfwcdma_core_txplim_chan_type, band);

  /* Get current log data, because mode is not FTM_TEST_MODE or calibration_state */
  log_data = rfcommon_core_txlin_get_log_data(RFCOM_WCDMA_MODE,rfm_tx_device);

  if (NULL == log_data)
  {
    RF_MSG( RF_ERROR,"rfwcdma_core_txplim_calculate_mtpl_asdiv_pos: Null pointer access to Log data" );
  }
  else
  {
    txplim_lin = (log_data->max_calibrated_power) +700;
  }
  
  /* SAR Limit */
  txplim_ntpl = MIN(txplim_ntpl,(sar_limit_db10 + rfwcdma_core_txlim_utran_vs_temp_offset_val));
  
  /* Multi-TX SAR Limit */
  txplim_ntpl = MIN(txplim_ntpl,(rfwcdma_core_txplim_multi_tx_sar_limit_db10+rfwcdma_core_txlim_utran_vs_temp_offset_val));

 /* Desired Limit */
  txplim_desired = MIN(txplim_ntpl,(rfwcdma_core_txplim_utpl + rfwcdma_core_txlim_utran_vs_temp_offset_val));  

  txplim_desired = MIN(txplim_desired, txplim_lin);
  
  /* Add Freq Comp factor */
  txplim_desired += rfwcdma_core_hdet_vs_freq_val;
	
  txplim_desired += rfwcdma_core_desired_lim_vs_freq_val;

  /* Add thermal mitigation backoff */
  txplim_desired += rfwcdma_core_txplim_thermal_backoff_db10;

  RF_MSG_1(RF_HIGH,"rfwcdma_core_txplim_calculate_mtpl_asdiv_pos: STEP 2 Desired MTPL limit value calculated %d",txplim_desired);

  /********STEP 3: Calculate the MTPL value***********************************************************************************/

  /* Update the difference in desired MTPL due to AsDiv change*/
  txplim_mtpl_q5 = rfwcdma_core_txplim_mtpl_q5 + RFTXPLIM_INT_TO_Q5(txplim_desired - txplim_desired_prev);

  RF_MSG_1(RF_HIGH,"rfwcdma_core_txplim_calculate_mtpl_asdiv_pos: STEP 2 MTPL limit q5 value calculated %d",txplim_mtpl_q5);
  
  /********STEP 4: Return the expected MTPL value to L1************************************************************************/

  /*Convert MTPL value from Q5 to integer*/
  return(RFTXPLIM_Q5_TO_INT(txplim_mtpl_q5) + RFAGC_MIN_AGC_VAL);
}
...
/*!
  ...
  none
*/
void rfwcdma_core_txplim_reset_mtpl_during_temp_comp(rfm_device_enum_type device)
{
  int16 mtpl_adj_db10;
  int16 final_mtpl;
