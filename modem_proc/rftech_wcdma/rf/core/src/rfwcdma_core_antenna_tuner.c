...
$Header: //components/rel/rftech_wcdma.mpss/3.11/rf/core/src/rfwcdma_core_antenna_tuner.c#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------
11/10/15   asn/yb  Add ASM update in AOL tick. Fix KW error
10/28/15   asn     Add critical section between AOL update and Tx disable
09/29/15   yb      Fix to not update PRx Tuner Device if Tx and PRx tuners 
                   are same during AOL
08/20/15   yb      Fix AOL update
08/24/15   asn      asm grfc changes
08/24/15   asn      clean up,Concurrency of ASDiv and Tuner configuration & Concurrency of AOL and Update Tuner 
...
06/18/15   ndb     Fix the KW warnings
06/10/15   ndb     Cleanup the unused enums
03/09/15   yb      Fix for snum exhaustion
05/27/15   ndb     Update the tune code override interface
...
)
{
  ...
  rfm_device_enum_type device = atuner_cfg_req->rfm_device;

  /* Get Qtuner object from RFC */
  qtuner_mgr_obj = rfc_wcdma_get_device( device, 
                                         RFC_CONFIG_RX, 
                                         atuner_cfg_req->band.wcdma_band, 
...
)
{
  ...
  rfm_device_enum_type rfm_device = atuner_cfg_req->rfm_device;
...
)
{
  ...
  rfcom_band_type_u band_u;
  uint32 script_token =0;
  rfm_device_enum_type rfm_device = atuner_config_req->rfm_device;

  
  /* Get Tuner manager object from RFC */
  qtuner_mgr_obj = rfc_wcdma_get_device( rfm_device, 
                                         RFC_CONFIG_TX, 
                                         atuner_config_req->band.wcdma_band, 
                                         RFDEVICE_TUNER_MANAGER);
  ...
  {
    RF_MSG_1(RF_MED,"ANT_TUNER: No Antenna tuner managers found from RFC for path %d", rfm_device);
    success = FALSE;
  }
  ...
  if(success)
  {   
    //RF_MSG_1(RF_HIGH,"ANT_TUNER: Programmed in Tx mode on device path#%d", rfm_device); 
  }
  else
  {
    RF_MSG_1(RF_ERROR,"ANT_TUNER: Program Tx mode on device path#%d failed !", rfm_device);
  }
  return success;
...
)
{
  ...
  /* Get Qtuner object from RFC */
  qtuner_mgr_obj = rfc_wcdma_get_device( atuner_cfg_req->rfm_device, 
                                         RFC_CONFIG_RX, 
                                         atuner_cfg_req->band.wcdma_band, 
                                         RFDEVICE_TUNER_MANAGER );
  ...
  if (qtuner_mgr_obj != NULL)
  {
    while(qtuner_mgr_obj[device_idx] != NULL)
      { 
        /* Disable tuner */
        success &= rfcommon_atuner_disable(qtuner_mgr_obj[device_idx], 
                                           atuner_cfg_req, 
                                           &script_token,
                                           dev_action,
                                                     script_buffer, 
  ...
  {
    RF_MSG_1(RF_MED,"ANT_TUNER: No Antenna tuner managers found from RFC for path %d", atuner_cfg_req->rfm_device);
    success = FALSE;
  }

  if(success)
  {       
    //RF_MSG_1(RF_HIGH,"ANT_TUNER: Disabled on device path#%d", atuner_cfg_req->rfm_device);    
  }
  else
  {
   RF_MSG_1(RF_ERROR,"ANT_TUNER: Disable on device path#%d failed !", atuner_cfg_req->rfm_device);
  }
                                                      ...
                                                      uint8 *tuner_id_ptr)
{
  ...
  if (RFM_WCDMA_TX_DEVICE == rfwcdma_data_get_device_type(device)) 
  {
  ...
  {
    RF_MSG_2(RF_MED,"ANT_TUNER: Anntenna tuner manager found from RFC for path %d, band %d", device, band);
  }
...
boolean rfwcdma_core_antenna_tuner_incremental_update ( void )
{
 ...
 rfcom_band_type_u band_u;
 void **qtuner_mgr_obj = NULL;
 void **qtuner_mgr_obj_tx = NULL;
 rfm_device_enum_type rfm_device;
 rfm_device_enum_type associated_rfm_tx_device;
 ...
 rflm_handle = (rflm_dm_handle_id_t)rfwcdma_data_get_rflm_buf_idx(rfm_tx_device);
 ...
 if(!rfwcdma_mdsp_associate_event_id(RF_WCDMA_EVENT_TUNER_OPEN_LOOP,rflm_handle))
 {
   ...
   {
     RF_MSG_1(RF_HIGH,"rfwcdma_core_update_tuner_code: [ASDIV] Switch in progress, Skip update",0);
     update_tune_code_in_progress = FALSE;
     rf_common_leave_critical_section(update_tuner_crit_sec);
     return TRUE;
   }
 ...
 for( rfm_device = RFM_DEVICE_0; rfm_device < RFM_MAX_WAN_DEVICES; rfm_device++)
 {
   ...
   if((rfwcdma_data_get_rf_state(rfm_device) == RFWCDMA_STATE_RX) ||
       (rfwcdma_data_get_rf_state(rfm_device) == RFWCDMA_STATE_TX))
   {
     /* Program tuner scenario */
     band_u.wcdma_band = rfwcdma_data_get_curr_band(rfm_device);

     if(!rfdevice_wcdma_asm_update_state(rfm_device,
                                     band_u.wcdma_band,
                                     NULL,
                                     RFDEVICE_EXECUTE_IMMEDIATE,
                                     RFCOM_INVALID_TIMING_OFFSET))
     {
       RF_MSG_2(RF_HIGH,"ASM_AOL_UPDATE: rfdevice_wcdma_asm_update_state: rfm_device:%d, band:%d", 
         rfm_device, band_u.wcdma_band);
     }

    if(rfwcdma_data_get_rf_state(rfm_device) == RFWCDMA_STATE_TX)
    {
    ...
    {
      associated_rfm_tx_device = RFM_MAX_WAN_DEVICES;

      if (rfwcdma_data_get_device_type(rfm_device)== RFM_WCDMA_PRIMARY_RX_DEVICE)
      {
        associated_rfm_tx_device = rfwcdma_data_get_associated_device_tx(rfm_device);
      }
      ...
      if(associated_rfm_tx_device < RFM_MAX_WAN_DEVICES )
      {
        qtuner_mgr_obj_tx = rfc_wcdma_get_device( associated_rfm_tx_device, 
                                                  RFC_CONFIG_TX, 
                                                  rfwcdma_data_get_curr_band(associated_rfm_tx_device), 
                                                  RFDEVICE_TUNER_MANAGER );

        if((NULL != qtuner_mgr_obj) && (NULL != qtuner_mgr_obj_tx))
        {
          if(qtuner_mgr_obj[0] == qtuner_mgr_obj_tx[0])
          {
            RF_MSG(RF_HIGH,"ANT_TUNER: PRx and Tx Tuner is same, skipping for ");
            continue;
          }
        }
      }
     ...
     else 
     {
        ...
        while(qtuner_mgr_obj[device_idx] != NULL)
        {
            if (num_tuner_device < RFC_MAX_DEVICE_INSTANCES_PER_BAND -1)
            {
              /* 4. Assign script object and tuner device pointer to call back data */
              tuner_cb_data->tuner_data[num_tuner_device].ant_tuner = (rfcommon_atuner_intf *)(qtuner_mgr_obj[device_idx]);

              /* 5. Obtain address of token from call back data*/
              chain_token_ptr = &(tuner_cb_data->tuner_data[num_tuner_device].tuner_token);

              /*Increment the number of tuner devices*/
              num_tuner_device++;
            }
            else
            {
              RF_MSG_2(RF_ERROR,"ANT_TUNER:Max Number of Tuners reached in AOL event NumTuners:%d rfm_device:%d",num_tuner_device,rfm_device);
              update_tuner_api_status = FALSE;
              break;
            }
            ...
            else if (rfwcdma_data_get_device_type(rfm_device)== RFM_WCDMA_TX_DEVICE)
            {
              atuner_cfg_req.rx_tx = RFCMN_ATUNER_TX_DEVICE;
            }
            else if (rfwcdma_data_get_device_type(rfm_device)== RFM_WCDMA_INVALID_DEVICE) 
            {
              atuner_cfg_req.rx_tx = RFCMN_ATUNER_INVALID_DEVICE;
            }

            atuner_cfg_req.rfm_device = rfm_device;
            atuner_cfg_req.band = band_u;
            atuner_cfg_req.mode = RFCOM_WCDMA_MODE;

            atuner_cfg_req.carrier_id = rfwcdma_core_map_carr_mask_to_atuner_id(rfwcdma_data_get_carrier_mask(rfm_device));

            success = rfcommon_atuner_update_tune_code((rfcommon_atuner_intf *)(qtuner_mgr_obj[device_idx]),
                                             &atuner_cfg_req,
                                             chain_token_ptr,
...
*/
  boolean rfwcdma_core_antenna_tuner_program_qfe_scenario ( void )
  {
   ...
   for( rfm_device = RFM_DEVICE_0; rfm_device < RFM_MAX_WAN_DEVICES; rfm_device++)
   {
     ...
     /*making sure the device is tuned either in Rx or Tx state*/
     if((rfwcdma_data_get_rf_state(rfm_device) == RFWCDMA_STATE_RX) || 
        (rfwcdma_data_get_rf_state(rfm_device) == RFWCDMA_STATE_TX))
     {
       /* Program tuner scenario */
       ...
       else 
       {
          ...
          if(qtuner_mgr_obj[device_idx] != NULL)
          {
           ...
           if(cell_type == RFCOM_INVALID_CELL) 
           {
           ...
           else
           {
            ...
            atuner_cfg_req.carrier_id = rfwcdma_core_map_carr_mask_to_atuner_id(rfwcdma_data_get_carrier_mask(rfm_device));
              ...
              success = rfcommon_atuner_force_detune((rfcommon_atuner_intf *)(qtuner_mgr_obj[device_idx]),
                                                     &atuner_cfg_req,
                                                     &dummy_token,
                                                     RFDEVICE_EXECUTE_IMMEDIATE,
                                                     NULL,
...
/*!
  @brief
  Program the tuner for tune-away
  ...
  Flag indicating if the api operation was successful (TRUE) or failed (FALSE)
*/
boolean rfwcdma_core_antenna_tuner_tune_away
( 
  rfcommon_atuner_config_req_type *atuner_cfg_req,
  rf_device_execution_type rfwcdma_exec_type,
  rf_buffer_intf* buffer_ptr
)
{
  ...
  {
    RF_MSG_1(RF_MED,"ANT_TUNER: No Antenna Tuner Manager found from RFC for path %d",rfm_device);
    success = FALSE;
  }

  if(success)
  {         
    RF_MSG_1(RF_HIGH,"ANT_TUNER: Programmed in Tuneaway mode on device path#%d",rfm_device);
  }

  return success;
} /* rfwcdma_core_antenna_tuner_tune_away */
...
/*!
  ...
  Flag indicating if the api operation was successful (TRUE) or failed (FALSE)
*/
boolean rfwcdma_core_antenna_tuner_tune_back
( 
  rfcommon_atuner_config_req_type *atuner_cfg_req,
...
)
{
  ...
  if(success)
  {         
    RF_MSG_1(RF_HIGH,"ANT_TUNER: Programmed in Tuneback mode on device path#%d",
      rfm_device);
  }

  return success;
} /* rfwcdma_core_antenna_tuner_tune_back */
#endif /* FEATURE_RF_HAS_QTUNER */
