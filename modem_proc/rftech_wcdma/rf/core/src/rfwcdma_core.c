...
$Header: //components/rel/rftech_wcdma.mpss/3.11/rf/core/src/rfwcdma_core.c#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
07/17/15   avi     Moved util APis invoked by devices to a separate file
06/05/14   vbh     [1] rfwcdma_core_is_band_supported - loop on all devices 
                   [2] rfwcdma_core_is_band_supported_on_dev - check specific device
05/04/15   kr      Handle B19 and B5(Band 800) rx channel while rx freq offset update 
05/04/15   kr      Handle B9 and B3 rx channel while rx freq offset update 
...
/*!
  @brief
  This function returns a flag indicating if a band is supported on a given device

  @param band rfi_band_type WCDMA band
  
  @param device rfm_device_enum_type device to check band support on

  @return boolean indicating band support
*/
boolean rfwcdma_core_is_band_supported_on_dev(rfcom_wcdma_band_type band, rfm_device_enum_type device)
{
  ...
  if ( (rfcommon_nv_get_rx_device_bc_config(device) & sys_band_mask) != 0)
  {
    return TRUE;
  }
  else
...
/*!
  @brief
  This function returns a flag indicating if a band is supported on any device
...
boolean rfwcdma_core_is_band_supported(rfcom_wcdma_band_type band)
{
  ...
  {
    band_supported |= rfwcdma_core_is_band_supported_on_dev(band,device);
