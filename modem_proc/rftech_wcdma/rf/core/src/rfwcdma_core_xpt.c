...
/*==============================================================================  
  ...
  $Header: //components/rel/rftech_wcdma.mpss/3.11/rf/core/src/rfwcdma_core_xpt.c#1 $
  
When       who     what, where, why 
------------------------------------------------------------------------------- 
10/22/15   rmb     Fix to use actual number of channels from XPT Delay NV.
08/31/15   dw      Added xpt delay interpolation across channels
09/24/14   rmb     Fix CW.
...
)
{
  ...
  int16 delay_nv_tbl[RFWCDMA_XPT_PATH_DELAY_ARRAY_SIZE] = {0};
  uint32 upper_chan_freq_tbl[RFWCDMA_XPT_PATH_DELAY_ARRAY_SIZE]= {0};
      ...
      (nv_tbl->tx_dc_delay_offsets.channels_enabled > 0) )
  {
    ...
    // Actual number of Channels in Least Significant 7 bits
    num_of_channels_enabled_for_delay = (xpt_dc_delay_tbl->num_of_active_entries) & 0x7F; 

    /* Find out if Delay NV was chared or calibrated 
       MSB bit will be turned ON if delay NV is calibrated, else NOT */
    bit_cal_or_char = (xpt_dc_delay_tbl->num_of_active_entries) & 0x80; 
  }
  else
  {
    ...
    // Actual number of Channels in Least Significant 7 bits
    num_of_channels_enabled_for_delay = (xpt_delay_tbl->num_of_active_entries) & 0x7F;

    /* Find out if Delay NV was chared or calibrated 
       MSB bit will be turned ON if delay NV is calibrated, else NOT */
    bit_cal_or_char = (xpt_delay_tbl->num_of_active_entries) & 0x80;
  }

  if (num_of_channels_enabled_for_delay > RFWCDMA_XPT_PATH_DELAY_ARRAY_SIZE)
  {
    RF_MSG_1( RF_ERROR,"num_of_channels_enabled_for_delay:%d over limit!",num_of_channels_enabled_for_delay);
    num_of_channels_enabled_for_delay = RFWCDMA_XPT_PATH_DELAY_ARRAY_SIZE;
  }
  ...
  else
  {
    ...
    for(i = 0; i < num_of_channels_enabled_for_delay; i++)
    {
      ...
      {
        upper_chan_freq_tbl[i] = rfwcdma_core_get_freq_from_ul_arfcn(xpt_dc_delay_tbl->upper_bound_tx_chan[i]);
        delay_nv_tbl[i] = xpt_dc_delay_tbl->dc_delay[i];
      }
      else
      {
        upper_chan_freq_tbl[i] = rfwcdma_core_get_freq_from_ul_arfcn(xpt_delay_tbl->upper_bound_tx_chan[i]);
        delay_nv_tbl[i] = xpt_delay_tbl->delay_value[i];
      }
    }

    xpt_path_delay_val = rf_lininterp_u32_s16(upper_chan_freq_tbl, 
                                              delay_nv_tbl, 
                                              num_of_channels_enabled_for_delay, 
                                              rfwcdma_core_get_freq_from_ul_arfcn(rf_chan_tx)) + delay_comp_x10ns ;
