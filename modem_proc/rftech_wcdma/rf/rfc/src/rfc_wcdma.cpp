...
$Header: //components/rel/rftech_wcdma.mpss/3.11/rf/rfc/src/rfc_wcdma.cpp#1 $

when       who     what, where, why
--------------------------------------------------------------------------------
03/02/18   nik     Adding QAT3522 XSW support for WCDMA
10/25/17   nik     Compiler changes for migration from 6.x LLVM to 8.x
...
10/28/15   ak      Changes to support WCDMA band 6
10/13/15   Saul    GNSS Blanking
09/10/15   rmb     Return status as TRUE if cross switch is not present
                   since all RF cards may not have cross switch device.
08/20/15   sd      Check and set device enable bit for B6 if B5 is active   
07/28/15   asn     Update rx_tx variable in rfc cfg params
07/15/15   dyc     Un-paired (dummy) tuner support.
07/10/14   ag      Create eLNA device in RFC
...
05/25/15   ak      [Rx/TX Split]Changes to Enable ASDIV
05/12/15   rcui    change vreg voting to be device based
05/08/15   ak      [Rx/Tx Split]Chages to support 6 Rx path Cal 
...
04/13/15   ak      [Rx/Tx split]Changes to create the WCDMA RFC
03/19/15   rmb     Mode config data is device independent.
12/22/14   vv      API support for rfc to capture the modem chain to trx mapping  
...
void rfc_wcdma::config_mode( rfm_device_enum_type device )
{
  ...
  if ( (device >= RFM_MAX_WAN_DEVICES) || ( device == RFM_ALL_DEVICES ))
  {
  ...
  MSG_1(MSG_SSID_RF, MSG_LEGACY_HIGH,"RFC WCDMA Get Data called on %d",cfg.logical_device );

  /*Initialize all the GRFC/GPIOs/RF Signals to the init value*/
  rfc_data->sig_cfg_data_get(&cfg, &rf_signals);
  rfc_common_config_signal(rf_signals, TRUE /* gnss_blanking_enabled */ );
...
void rfc_wcdma::rx_band_config(rfm_device_enum_type device, rfcom_wcdma_band_type curr_band)
{
  ...
  if ((RFCOM_BAND_INVALID != rx_band[device]) && ((RFCOM_BAND_IMT <= rx_band[device]) || (RFCOM_NUM_WCDMA_BANDS > rx_band[device]))) 
  {
...
void rfc_wcdma::tx_band_config(rfm_device_enum_type device, rfcom_wcdma_band_type curr_band)
{
  ...
  to their default state*/
  rf_signals = NULL;
  if ((RFCOM_BAND_INVALID != tx_band[device]) && ((RFCOM_BAND_IMT <= tx_band[device]) || (RFCOM_NUM_WCDMA_BANDS > tx_band[device])))
  {
...
*/
void *rfc_wcdma::mode_config_get(rfc_mode_config_data_enum_type config_data)
{
  void* data_config_ptr = NULL;
  rfc_sig_cfg_type *rf_signals = NULL;
  ...
  switch (config_data)
  {
    ...
    default:            /* Invalid mode operation */
      MSG_1(MSG_SSID_RF, MSG_LEGACY_FATAL,"Unknown RFC data request %d",config_data);
      break;
  }
...
int32 rfc_wcdma::sleep( rfm_device_enum_type device, rfcom_wcdma_band_type rx_wcdma_band)
{
  rfc_cfg_params_type cfg = { RF_PATH_0, RFM_DEVICE_0, RFC_CONFIG_RX, RFCOM_NUM_WCDMA_BANDS, RFC_REQ_INIT};
  rfc_sig_cfg_type *rf_signals = NULL;
  rfc_wcdma_data *rfc_data = rfc_wcdma_data::get_instance();
...
boolean rfc_wcdma::init_rf_devices(rfc_cfg_params_type *cfg, rfc_device_info_type *device_info_ptr)
{
  ...
  if(((cfg->logical_device < RFM_DEVICE_0) || (cfg->logical_device >= RFM_MAX_WAN_DEVICES)) ||
  ...
  if (rfc_common_get_device_capability(cfg->logical_device) == RFM_DEVICE_RX_SUPPORTED )
  {
    if (!rfwcdma_core_is_band_supported_on_dev((rfcom_wcdma_band_type)cfg->band, cfg->logical_device))
  {
     RF_MSG_2(RF_MED, "Band %d not supported on log_dev %d. Skipping device mapping.", cfg->band, cfg->logical_device);
     return TRUE;
  }
}
  else if(rfc_common_get_device_capability(cfg->logical_device) == RFM_DEVICE_TX_SUPPORTED )
  {
   if (!rfwcdma_core_is_band_supported((rfcom_wcdma_band_type)cfg->band))
   {
      RF_MSG_2(RF_MED, "Band %d not supported on log_dev %d. Skipping device mapping.", cfg->band, cfg->logical_device);
      return TRUE;
 }
  ...
  sys_band = rfcommon_core_convert_rfcom_band_to_sys_band( RFM_IMT_MODE, rf_band );
  ...
  while (i < device_info_ptr->num_devices)
  {
    ...
    else
    {
        ...
        case RFDEVICE_TRANSCEIVER:
          ...
          else
          {
            ...
            if (tmp_status == TRUE)
            {
              ...
              /* For B5, need to check if B6 is also enabled. Need to set B6 explicitly because B5 and B6 share same RFCOM_WCDMA_BAND*/
              if ( ( sys_band == SYS_BAND_WCDMA_V_850 ) && 
                   (rfc_common_check_band_supported(SYS_BAND_WCDMA_VI_800, cfg->logical_device) == TRUE))
              {
                rfc_dynamic_params.sys_band = SYS_BAND_WCDMA_VI_800;
                (void) rfc_cmn->generate_rfcard_dynamic_properties( &rfc_dynamic_params );
              }
            }
            else
        ...
        case RFDEVICE_XSW:
           ...
           else
           {
             ...
             if (cfg->req == RFC_REQ_ENABLE_ANT_SWAP_1)
             {
              ...
              xsw_cfg.mode = RFM_IMT_MODE;
              xsw_cfg.band = rf_band;
               
              rfdevice_xsw_set_band_map_swap(&xsw_cfg, cmn_dev_obj,
                                                              device_info_ptr->rf_asic_info[i].data, 
                                                              RFC_ASIC_INFO_DATA_SIZE);
             }/* !RFC_CONFIG_RX */
             else if (cfg->req == RFC_REQ_DISABLE_ANT_SWAP_1)
             {
              ...
              rfdevice_xsw_set_band_map_default(&xsw_cfg, cmn_dev_obj, 
                                                                device_info_ptr->rf_asic_info[i].data, 
                                                                RFC_ASIC_INFO_DATA_SIZE);
             }/* !RFC_CONFIG_TX */ 
             else
        ...
        case RFDEVICE_TUNER:
          ...
          if ( (cfg->rx_tx == RFC_CONFIG_TX) || (cfg->rx_tx == RFC_CONFIG_RX) )
          {
           ...
           /* Make sure there is a valid Antenna tuner manager */
            if (cmn_dev_obj != NULL)
            {
              wcdma_rf_devices[cfg->logical_device][cfg->rx_tx][cfg->band][RFDEVICE_TUNER_MANAGER][dev_inst_count[dev_type]] = cmn_dev_obj;
      ...
      case RFDEVICE_LNA:
        ...
        {
            lna_params_c.rfm_device = cfg->logical_device;
            lna_params_c.mode = RFCOM_WCDMA_MODE;
            lna_params_c.band =rf_band;
            lna_params_c.trigger_type = RFDEVICE_LNA_TRIGGER_NONE;
            lna_params_c.lna_gain= RFDEVICE_LNA_GAIN_INVALID; 
            lna_params_c.system_gain= RFDEVICE_LNA_GAIN_INVALID; 

            wcdma_rf_devices[cfg->logical_device][cfg->rx_tx][cfg->band][dev_type][dev_inst_count[dev_type]] = cmn_dev_obj; 
            if (cfg->rx_tx == RFC_CONFIG_RX)
            {
              ((rfdevice_lna *)cmn_dev_obj)->set_band_map(&lna_params_c, device_info_ptr->rf_asic_info[i].data, 
                                                          RFC_ASIC_INFO_DATA_SIZE);
            }/* !RFC_CONFIG_RX */

          } /* end IF cmn_dev_obj==NULL */

        break; /* end case RFDEVICE_LNA */
  ...
  */
   ...
   */
   ...
   if (tuner_dev_instance == 0 ) 
   {
     ...
     {
        wcdma_rf_devices[cfg->logical_device][cfg->rx_tx][cfg->band][RFDEVICE_TUNER_MANAGER][tuner_dev_instance] = tuner_mgr_obj;
...
boolean rfc_wcdma::init_rfdevices_for_all_bands(void)
{
  ...
  MSG(MSG_SSID_RF, MSG_LEGACY_HIGH, "Processing asdiv_config info table for WCDMA...");
  ...
  cfg.alternate_path = 0;
  cfg.req = RFC_REQ_DEFAULT_GET_DATA;
  for(rfm_device = RFM_DEVICE_0; rfm_device < RFM_MAX_WAN_DEVICES; rfm_device = (rfm_device_enum_type)((int)rfm_device + 1))
  {
    if((rfc_common_get_device_capability(rfm_device) & RFM_DEVICE_RX_SUPPORTED)!=0)/*device type rx*/
  { 
      cfg.logical_device = rfm_device; //fill the cfg.logical_device
      cfg.rx_tx = RFC_CONFIG_RX;  //fill the cfg.rxtx
  }
  else if ((rfc_common_get_device_capability(rfm_device) & RFM_DEVICE_TX_SUPPORTED)!=0) /*device type tx*/
  {
    cfg.logical_device = rfm_device; //fill the cfg.logical_device
      cfg.rx_tx = RFC_CONFIG_TX;  //fill the cfg.rxtx 
  }
  else 
  {
      MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"Invalid Device capability", 0);
  }

          /*Loop through all the bands and create device objects*/  
    for ( band = RFCOM_BAND_IMT; band < RFCOM_NUM_WCDMA_BANDS; band++ )
      {
      cfg.band = band;    
      rfc_data->devices_cfg_data_get(&cfg, &device_info_ptr);
  ...
  /*@akash: Commenting out the old algo. Clean up after BU*/
  #if 0
  cfg.alternate_path = 0;
  cfg.logical_device = RFM_DEVICE_0;
  cfg.rx_tx = RFC_CONFIG_RX;
       ...
       if(rfc_data->devices_cfg_data_get(&cfg, &device_info_ptr)== TRUE)
       {
           ...
           )
         {
              RF_MSG_3( RF_ERROR, " rfc_set_fbrx_mapping() failed for rfm device %d. "
                                  ...
                                  rfm_device,RFCOM_WCDMA_MODE,band);
         } 
       }
    }
  }
...
*/
uint32 rfc_wcdma::get_fb_chain(rfm_device_enum_type tx_device, rfcom_wcdma_band_type tx_band)
{
  rfc_cfg_params_type cfg;
  ...
  if (device_info_ptr == NULL)
  {
    MSG_1(MSG_SSID_RF, MSG_LEGACY_ERROR,"get_fb_chain: RFC WCDMA Data device_info_ptr for Tx is NULL", 0);
...
)
{
  ...
  //                   "position %d", position);
   ...
   /* udpate XSW port to use if needed */
       ...
       {           
         case RFDEVICE_XSW:
           ...
           if( device_obj != NULL )
           {
             if (immediate == 0) 
              {
                status = rfdevice_xsw_wakeup(&xsw_config,(void*)device_obj,asdiv_script_ptr,(rf_device_execution_type)(!immediate),0);
                status = rfdevice_xsw_set_cross_switch_config(&xsw_config,(void*)device_obj,asdiv_script_ptr,(rf_device_execution_type)(!immediate),0);
                status = rfdevice_xsw_trigger(&xsw_config,(void*)device_obj,asdiv_script_ptr,(rf_device_execution_type)(!immediate),0);
              }
            else
              { 
                status = rfdevice_xsw_wakeup(&xsw_config,(void*)device_obj,asdiv_script_ptr,(rf_device_execution_type)(!immediate),0);
...
/*!
  ...
  True on success and FALSE on failure
*/
boolean rfc_wcdma::configure_asm_xsw_port
( 
...
)
{
  ...
  if ( rfc_data == NULL )
  {
    RF_MSG( RF_ERROR, "rfc_wcdma::configure_asm_xsw_port: "
                      "rfc_data ptr is null !!");
    return FALSE;
  }
   ...
   for (i=0;i < device_info_ptr->num_devices;i++)
   {
     ...
     {           
       case RFDEVICE_ASM:
         ...
         if( device_obj != NULL )
         {
           status = ((rfdevice_asm *)device_obj)->set_asm_xsw_port_config( device, RFM_IMT_MODE, band_u );
         }
         else
         {
           status = FALSE;
