...
$Header: //components/rel/rftech_wcdma.mpss/3.11/rf/mc/src/rfwcdma_cmd_proc.c#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------------------------------
11/05/15   ag      Add dispatch handler for AUTOPIN READ
06/02/15   gh/zhh  Add dispatch handler for RFLM_WCDMA_DO_THERM_READ_CMD
06/12/15   aa      autopin feature MSGR registration
...
)
{
  ...
  case MSGR_ID_VAL(RFA_RF_WCDMA_TEMP_COMP_UPDATE_CMD):
  	/*Payload '0' is populated with the device passed from WL1*/
    rfwcdma_temp_comp_update_handler((rfm_device_enum_type)cmd->payload[0]);
    break;
...
)
{
  ...
  ret_val &= rf_dispatch_register_command( rf_task_get_dispatch_handle(),
   ...
   /* Register command to perform NPA ISSUE request operation  */
  ret_val &= rf_dispatch_register_command( rf_task_get_dispatch_handle(),
                                           MSGR_TECH_RFLM, 
                                           RFLM_WCDMA_NPA_ISSUE_CMD, 
                                           rflm_wcdma_npa_issue_cmd_handler,
                                           NULL,
                                           1<<RF_DISPATACH_MSGR_REG );
  /* Register command to perform NPA cancel request operation  */
  ret_val &= rf_dispatch_register_command( rf_task_get_dispatch_handle(),
                                           MSGR_TECH_RFLM, 
                                           RFLM_WCDMA_NPA_CANCEL_CMD, 
  ...
  ret_val &= rf_dispatch_register_command( rf_task_get_dispatch_handle(),
                                           MSGR_TECH_RFLM, 
                                           RFLM_WCDMA_PROCESS_AUTO_PIN_CMD, 
