...
$Header: //components/rel/rftech_wcdma.mpss/3.11/rf/mc/src/rfm_wcdma.c#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
01/13/20   nik     FEATURE_WCDMA_DB_DC_HSDPA is disabled in HA. Passing device to rfm_write_tech_dependent_nv_items_to_mdsp
...
09/29/15   asn     Remove critical sections 
09/22/15   ak      Changes to fetch the BTF value correctly per band
09/07/15   ak      Changes to update the LNA switch points correctly for HS
...
08/25/15   asn     Fix asm grfc
08/25/15   asn     agc logging
08/25/15   asn     asm grfc changes
08/18/14   sd      Added an API to return expected MTPL based on AsDiv position and SAR NV
...
07/02/15   ac      remove the critical section from tx_disable and deinit
06/18/15   joz     FR 25072 - Dynamic enable/disable of one of two WTRs on 9x45 
06/17/15   ag      Include parameter for device in set lpm API in all cases
06/16/15   vbh     Add calls to mc validate state before proceeding with RFM API
06/13/15   ag      Fix Compiler warnings
06/10/15   vbh     Exec wakeup - fix the state check to only enable RxAGC
06/10/15   ag      Fix F3 messages print bug
...
05/15/15   ak      Changes to Call Direct write tune if Tx is Enabled
04/21/15   ac      ftm w plus w rxagc bringup
04/09/15   vbh     Interface update for power ON/OFF API
4/8/15     vbh     [Rx/Tx split][1] Clean-up calls to old MC state machine
                   [2] Remove RF state validation for Rx APIs
                   [3] Remove bypass in tune APIs for Tx on or DC
07/04/15   ak      Migrating to new WCDMA state machine
07/04/15   ak      Rx/Tx Split Changes
4/2/15     vbh     Added implementation for rfm tx wakeup api
3/31/15    vbh     [Rx/Tx split] Add APIs for get and set primary path
04/23/15   rmb     Add a new api for handle based cleanup of WCDMA mdsp data.
02/12/14   dw      Added rfm_wcdma_power_on_off()
...
/*==================TO BE DEPRECATED after switching to new AGC log packet =========================*/
static rfm_wcdma_log_agc_handle_type rfm_log_handle = 0;
static boolean log_agc_mem_allocated = FALSE;
/*----------------------------------------------------------------------------*/
/* WCDMA RF FTM state */
...
{
  rfwcdma_data_set_prx_path(primary_path);
}
/*----------------------------------------------------------------------------*/
/*!
  @brief
...
{
  rf_path_enum_type primary_path =0;

  primary_path=rfwcdma_data_get_prx_path();

  RF_MSG_1(RF_MED, "rfm_wcdma_get_prx_path: Primary path is %d",primary_path);
...
void rfm_wcdma_set_mcs_tx_pwr_limit (int16 tx_limit_dbm)
{
  ...
  RFM_DEBUG_EN_Q(&rfm_wcdma_set_mcs_tx_pwr_limit, 0, tx_limit_dbm, 0, 0);

  RFWCDMA_MC_V(cmd_dispatch)(RFCOM_TRANSCEIVER_0, (int32)RFWCDMA_MC_SET_MCS_TX_PWR_LIMIT, &pwr_limit);
}
...
{
  uint32 tx_rx_ratio = 0;

  RFM_DEBUG_EN_Q(&rfm_tx_rx_freq_ratio, 0, 0, 0, 0);

  RFWCDMA_MC(cmd_dispatch, tx_rx_ratio)(device, RFWCDMA_MC_GET_TX_RX_RATIO,&channel);
                               ...
                               uint16 *tx_rot_ang_pa_11_p)
{
  ...
  RFWCDMA_MC_V(cmd_dispatch)(device, (int32)RFWCDMA_MC_GET_TX_ROTATOR_ANGLE, &tx_rot_angle);
}
...
{
  rfm_wcdma_log_agc_param_type intf_param = {0};

  /* Populate PRx devices followed by DRx devices for both Sub*/
  intf_param.rfm_device_prx[RFCOM_MULTI_CARRIER_ID__0].rfm_device = rfwcdma_data_map_ant_carr_to_device(0, 0, FALSE);
  intf_param.rfm_device_prx[RFCOM_MULTI_CARRIER_ID__1].rfm_device = rfwcdma_data_map_ant_carr_to_device(0, 1, FALSE);
  intf_param.rfm_device_prx[RFCOM_MULTI_CARRIER_ID__2].rfm_device = rfwcdma_data_map_ant_carr_to_device(0, 2, FALSE);
  intf_param.rfm_device_prx[RFCOM_MULTI_CARRIER_ID__3].rfm_device= rfwcdma_data_map_ant_carr_to_device(0, 3, FALSE);

  intf_param.rfm_device_rxd[RFCOM_MULTI_CARRIER_ID__0].rfm_device= rfwcdma_data_map_ant_carr_to_device(1, 0, FALSE);
  intf_param.rfm_device_rxd[RFCOM_MULTI_CARRIER_ID__1].rfm_device= rfwcdma_data_map_ant_carr_to_device(1, 1, FALSE);
  intf_param.rfm_device_rxd[RFCOM_MULTI_CARRIER_ID__2].rfm_device= rfwcdma_data_map_ant_carr_to_device(1, 2, FALSE);
  intf_param.rfm_device_rxd[RFCOM_MULTI_CARRIER_ID__3].rfm_device= rfwcdma_data_map_ant_carr_to_device(1, 3, FALSE);

  intf_param.rfm_device_tx = rfwcdma_data_get_tx_device();
  intf_param.single_instance= TRUE; // Single log

  /* Allocate memory for log packet only once */
  if (log_agc_mem_allocated == FALSE) 
  {
    rfm_log_handle = rfm_wcdma_log_agc_create(SYS_MODEM_AS_ID_1);
    log_agc_mem_allocated = TRUE;
  }

  /* This is new version (Ver 2) of log packet 0x4176*/
  rfwcdma_core_log_agc_data(rfm_log_handle,&intf_param );
}
...
/*!
  ...
  @param : Band for which the BTF value needs to be returned.
*/
uint16 rfm_wcdma_get_enc_btf_val(rfcom_wcdma_band_type band)
{
  int32 btf_val = 0;
  /*Passing Dummy Device 0. Band is the one which is used to fetch the correct BTF value*/
  RFWCDMA_MC(cmd_dispatch, btf_val)(RFCOM_TRANSCEIVER_0, (int32)RFWCDMA_MC_GET_ENC_BTF_VAL, &band );
  return (uint16)btf_val;
}
...
rfi_band_type rfm_wcdma_get_curr_band( rfm_device_enum_type device )
{
  ...
  {
    ret_val = RFWCDMA_MC(cmd_dispatch, ret_val)(device, (int32)RFWCDMA_MC_GET_CURR_BAND, 0);
  }
  else 
  {
    RF_MSG_1(RF_ERROR,"rfm_wcdma_get_curr_band: Device%d in INIT state", device);
  }
...
void rfm_write_tech_dependent_nv_items_to_mdsp(rfm_device_enum_type device, rfcom_wcdma_band_type band,rfcom_tech_type tech)
{
  ...
  RFWCDMA_MC_V(cmd_dispatch)(device, (int32)RFWCDMA_MC_WRITE_TECH_DEPENDENT_CONFIG_TO_MDSP, &tech);
}
...
void rfm_log_agc(rfcom_device_enum_type device)
{
  ...
  intf_param.rfm_device_tx = rfwcdma_data_get_tx_device();
  intf_param.single_instance= FALSE; // No single log
...
{
  rfwcdma_core_log_agc_data_flush(rfm_log_handle);
}
...
/*!
  ...
  @details
  This function gets the current value of MCS Tx power limit in units of
  Tx AGC counts. (-512 to +511).
...
{
  int32 ret_val = 0;
  RFWCDMA_MC(cmd_dispatch, ret_val)( RFCOM_TRANSCEIVER_0, (int32)RFWCDMA_MC_GET_MCS_TX_PWR_LIMIT, 0 );
  return (int16) ret_val;
}
...
{  
  //Call rxagc get API in rfwcdme_mc.c
  if (TRUE == rfwcdma_mc_get_rxagc_info(rxagc_input_info, rxagc_output_info))
  {
                               ...
                               uint32 c1_rxlm_buf_idx)
{
  ...
  RFWCDMA_MC(cmd_dispatch,ret_val)(device,
...
)
{
  ...
  if (rfwcdma_data_get_device_type(tx_retune_bw_params->device)!= RFM_WCDMA_TX_DEVICE) 
  {
    RF_MSG_2(RF_ERROR, "rfm_wcdma_tx_retune_bw: Passed device %d is of type %d not Tx",
                        tx_retune_bw_params->device,
                        rfwcdma_data_get_device_type(tx_retune_bw_params->device));
    return ret_val;
  }
  else
  {
  ...
  if (ret_val != RFM_WCDMA_HEALTHY_STATE)
  {
    ...
    RF_MSG_4( RF_ERROR, "rfm_wcdma_tx_retune_bw: [FAILURE %d] - Dev %d, "
              "multi_carrier_idx %d, API Time %d uS ", 
              ret_val, 
              tx_retune_bw_params->device,
              tx_retune_bw_params->multi_carrier_idx,
              profile_time );
...
)
{
  ...
  if (RFM_WCDMA_HEALTHY_STATE != rfwcdma_mc_validate_state(device_mask, RFM_WCDMA_PREP_WAKEUP))
  {
     RF_MSG_1(RF_ERROR, "rfm_wcdma_rx_prep_wakeup: State validation failed on dev mask %d", device_mask);
...
)
{
  ...
  rf_time_tick_type profile_tick; /* Profile Tick */
  rfm_cb_event_enum_type cb_event = RFM_EXECUTION_FAILURE; /* CB event type */
  uint8 device_mask = 0, device_mask_copy =0, device =0; 
  uint32 rx_handle_pool[RFM_WCDMA_MAX_DEVICE] = {0};
  boolean agc_enabled = FALSE; 
  ...
  if (RFM_WCDMA_HEALTHY_STATE != rfwcdma_mc_validate_state(device_mask, RFM_WCDMA_EXEC_WAKEUP))
  {
     RF_MSG_1(RF_ERROR, "rfm_wcdma_rx_exec_wakeup: State validation failed on dev mask %d", device_mask);
        /* Acquire Critical Section Lock */
  ...
  if(device_mask != 0)
  {
    device = (rfm_device_enum_type)rf_extract_first_one_bit_lsb_location(device_mask);
      ...
      if((device < RFM_DEVICE_0) || (device >=RFM_MAX_WAN_DEVICES))
        {
         RF_MSG_1(RF_HIGH, "rfm_wcdma_rx_exec_wakeup: device %d is invalid due to memory corruption  ", device);
         return 0;
         }

    if ( RFWCDMA_STATE_RX == rfwcdma_data_get_rf_state(device))
    {
      /* 
...
)
{
  ...
  if (RFM_WCDMA_HEALTHY_STATE != rfwcdma_mc_validate_state(device_mask, RFM_WCDMA_SLEEP))
  {
     RF_MSG_1(RF_ERROR, "rfm_wcdma_rx_sleep: State validation failed on dev mask %d", device_mask);
        /* Acquire Critical Section Lock */
  }
  else
  {
...
)
{
  ...
  for (device=0; device< RFM_WCDMA_MAX_DEVICE; device++)
  {
    if (rx_tune_params -> api_params[device].is_valid == TRUE)
    {
      ...
      if((rfwcdma_data_val_dev_type(device,RFM_WCDMA_PRIMARY_RX_DEVICE)== TRUE) ||
          (rfwcdma_data_val_dev_type(device,RFM_WCDMA_DIVERSITY_RX_DEVICE)== TRUE))
      {
        device_mask |= 1<< device;
      }
      else 
        RF_MSG_1(RF_HIGH,"rfm_wcdma_rx_prep_tune: Device %d not a valid Rx device", device);
     }
  }

  if (RFM_WCDMA_HEALTHY_STATE != rfwcdma_mc_validate_state(device_mask, RFM_WCDMA_PREP_TUNE))
  {
     RF_MSG_1(RF_ERROR, "rfm_wcdma_rx_prep_tune: State validation failed on dev mask %d", device_mask);
...
)
{
  ...
  uint8 carrier0 =0, carrier_index=0; 
  ...
  if (RFM_WCDMA_HEALTHY_STATE != rfwcdma_mc_validate_state(device_mask, RFM_WCDMA_EXEC_TUNE))
  {
     RF_MSG_1(RF_ERROR, "rfm_wcdma_rx_exec_tune: State validation failed on dev mask %d", device_mask);
...
)
{
  ...
  if ( tc_data->tc_state == TEMP_COMP_READY )
  {
    if (rfwcdma_data_get_rf_state(device) == RFWCDMA_STATE_TX)
    {
      ...
      temp_comp_cmd->payload[0] = device;
       /*Populate the payload '0' with the device so as to invoke device/sub based temp comp*/
    ...
    } /* If device is Tx compatible*/ 

    else
    {
      ret_val = RFM_WCDMA_ERROR_BAD_STATE;
      RF_MSG_2( RF_ERROR, "rfm_wcdma_prep_temp_comp: [Invalid State] Device %d, "
                "State %d", device, rfwcdma_data_get_rf_state(device) );
    } /*If passed device is not Tx */
...
void rfm_wcdma_set_ftm_state(rfm_wcdma_ftm_state_type state)
{
  ...
  case RFM_WCDMA_FTM_RXONLY:
  case RFM_WCDMA_FTM_W_PLUS_W:
    temp_state = state;
    break;
  }
  ...
  RF_MSG_2( RF_HIGH, "rfm_wcdma_set_ftm_state: Asked state %d, current state %d",
            state, temp_state);
...
)
{
  ...
  rfm_wcdma_carrier_mask_type tx_carr_mask;
  ...
  tx_dev       = tx_params->device;
  tx_handle    = tx_params->txlm_handle;
  tx_carr_mask = tx_params->carrier_mask;

  //if(rfwcdma_data_val_dev_type(tx_dev, RFM_WCDMA_TX_DEVICE)== TRUE) 
  //{
    status = rfwcdma_mc_init_tx(tx_dev, 
                                tx_handle, 
                                tx_carr_mask,tx_params->tx_channel);
  //}
  //else
  //{
  //  RF_MSG_1(RF_ERROR, "rfm_wcdma_tx_init: Device %d not a Tx device", tx_dev);
  //  return RFM_WCDMA_ERROR_BAD_STATE;
  //}
...
)
{
  ...
  status = rfwcdma_mc_enable_tx(tx_params->device,tx_params->tx_channel);
                                         ...
                                         rfcom_wcdma_band_type band )
{
  ...
  #if 0
  RF_MSG_4(RF_LOW, "rfm_wcdma_get_tx_max_power_nv: Device - %d, chan_type %d, band %d - Max Tx %d",
...
/*!
  ...
  @details
  This function is the successor of the legacy rfm_power_clock_on() and
  rfm_power_clock_off() which now need to device as parameter
  ...
  This function is called in standalone mode only not IRAT.
...
)
{
  ...
  profile_tick = rf_time_get_tick();
    ...
    /* Throw F3 Message */
  RF_MSG( RF_HIGH, "rfm_wcdma_power_on_off : START" );

  /* Log the API */
  RFM_DEBUG_EN_Q( &rfm_wcdma_power_on_off,&callback,0,0,0 );

  RF_MSG_3(RF_HIGH, "Calling rfwcdma_mc_power_on_off: device %d, API_device %d, sub_id %d", pwr_param->device, rfwcdma_data_get_device_type(pwr_param->device), pwr_param->sub_type);
  
  if (rfwcdma_mc_power_on_off(pwr_param, rfwcdma_data_get_device_type(pwr_param->device)) == FALSE) 
  {
    ret_val = RFM_WCDMA_ERROR_FAILURE;
  }
  else
  {
    ret_val = RFM_WCDMA_HEALTHY_STATE;
  }
  ...
  if ( callback != NULL )
  {
   ...
   {
     /* Debug message */
     RF_MSG_5( RF_ERROR, "rfm_wcdma_power_on_off: [FAILURE %d], on_off:%d" 
               "device:%d, sub %d, time: %d uS", 
               ret_val, 
               pwr_param->on_off,
               pwr_param->device,
               pwr_param->sub_type,
   ...
   {
     /* Debug message */
     RF_MSG_5( RF_HIGH, "rfm_wcdma_power_on_off: success:%d, on_off:%d" 
               "device:%d, sub %d time: %d uS", 
               ret_val, 
               pwr_param->on_off,
               pwr_param->device,
...
/*!
  @brief
  Wakeup Tx after deep sleep

  @details
  This is a subset of the Tx init API and relies on the fact the TX is waking after deep sleep.
  This does the TX reconfiguration which is lost during a power collapse. 
  ...
  A function pointer that will be called when the entire wakeup sequence
  is complete.  This function will be passed with
  RFM_WCDMA_TX_WAKEUP_COMPLETE event when all actions associated with this 
  API are finished.
...
/* Start Profiling of API */
 ...
 /* Throw F3 Message */
 RF_MSG( RF_HIGH, "rfm_wcdma_tx_wakeup : START" );

 /* Log the API */
 RFM_DEBUG_EN_Q( &rfm_wcdma_tx_wakeup,&callback,0,0,0 );

 status = rfwcdma_mc_tx_wakeup(tx_params->device, 
                               tx_params->txlm_handle, 
                               tx_params->carrier_mask,tx_params->tx_channel[0]);

 /* Based on Status of MC execution, set CB Event Flag */
 if ( status == TRUE )
 {
   /* Flag Callback event as successful */
   cb_event = RFM_WCDMA_TX_WAKEUP_COMPLETE;   
 } /* if ( status == TRUE ) */
 ...
 {
   /* Debug message */
   RF_MSG_3( RF_ERROR, "rfm_wcdma_tx_wakeup: [FAILURE %d] -," "Dev 0x%x Init_Tx Time %d uS", 
             ret_val, 
             tx_params->device,
             profile_time );
 } /* if (ret_val != RFM_CDMA_HEALTHY_STATE) */
 ...
 {
   /* Debug message */
   RF_MSG_3( RF_HIGH, "rfm_wcdma_tx_wakeup: [Status %d] -, ""Dev 0x%x, Init_Tx Time %d uS", 
              ret_val, 
              tx_params->device,
              profile_time );
 } /* if ! (ret_val != RFM_CDMA_HEALTHY_STATE) */
...
{
  rfm_api_status_t ret_val = RFM_WCDMA_ERROR_FAILURE;
  if (rfwcdma_mc_handle_cleanup(rflm_handle) == TRUE) 
  {
    ret_val = RFM_WCDMA_HEALTHY_STATE;
  }
  ...
  else
  {
    ...
    /* Debug message */
    RF_MSG_1( RF_HIGH, "rfm_wcdma_handle_cleanup: Failed for RFLM handle %d",
              rflm_handle );
  }
...
/*! 
  @brief
  Funtion to provide associated Rx device by quering RFC
 
  @details
  This routine will be used by WL1 to query the associated RF device from RFC
 
  @param rfm_device
  RFM Device whose paired device is queried
...
{
  rfm_device_enum_type asso_dev = RFM_INVALID_DEVICE;

  if (rfm_dev >= RFM_MAX_WAN_DEVICES) 
  {
    RF_MSG_1(RF_ERROR, "rfm_wcdma_get_asso_rx_dev: Invalid device %d passed", rfm_dev);
    return asso_dev;
  }

  asso_dev = rfc_common_get_preferred_associated_rx_device(rfm_dev);
  RF_MSG_2(RF_MED, "rfm_wcdma_get_asso_rx_dev: device%d associated with passed device%d",asso_dev,rfm_dev);
  return asso_dev;
...
/*! 
  @brief
  Funtion to provide sys band mask from RFI band
 
  @details
  This routine will be used by WL1 to query the sys band mask from RFI band
 
  @param band
  RFI band type band
 
  @return
  sys_band_mask_type band mask
*/
sys_band_mask_type rfm_wcdma_get_sys_band_mask_from_rfi(rfi_band_type band)
{
  if ((band >= RFI_IMT_BAND) && (band <= RFI_WCDMA_BC19_BAND))
  {
    return rfwcdma_core_get_sys_band_mask_from_band_rfi(band);
  }
  else
  {
    RF_MSG_1(RF_ERROR, "rfm_wcdma_get_sys_band_mask_from_rfi: non WCDMA band passed %d", band);
...
{
  int32 ret_val = 0;
  ret_val = rfwcdma_core_txplim_calculate_mtpl_asdiv_pos(asdiv_pos);
  
  RF_MSG_2(RF_HIGH, "rfm_wcdma_calculate_mtpl_asdiv_pos: asdiv_pos: %d, Expected MTPL: %d",asdiv_pos,ret_val);
...
void rfm_wcdma_reduce_mptl_db_for_therm(uint8 backoff, rfm_wcdma_subscription_type sub_id)
{
  ...
  device = rfwcdma_data_get_tx_dev_frm_sub(sub_id);
  RFWCDMA_MC(cmd_dispatch, ret_val)(device, RFWCDMA_REDUCE_MTPL_FOR_THERM, (void*)(&backoff));
}
...
{
  rfm_wcdma_log_agc_handle_type log_handle = RFM_WCDMA_LOG_AGC_MAX_HANDLE;
  rf_lock_data_type* agc_logging_crit_sec = NULL;
  agc_logging_crit_sec = rfwcdma_get_agc_log_lock_data();

  /* Enter critical section to allow agc logging when W is on both subs */
  rf_common_enter_critical_section(agc_logging_crit_sec);

  log_handle = rfwcdma_log_agc_create(subscriber_id);
  
  rf_common_leave_critical_section(agc_logging_crit_sec);
...
boolean rfm_wcdma_log_agc_delete (rfm_wcdma_log_agc_handle_type  log_handle)
{
  ...
  ret_val = rfwcdma_log_agc_delete( (rfm_wcdma_log_agc_handle_type)log_handle );
...
{
  return rfwcdma_core_log_agc_data(log_handle, intf_param);
} /* rfm_wcdma_log_agc */
/*--------------------------------------------------------------------------------------------------------------------*/
/*!
  @brief
...
{
  return rfwcdma_core_log_agc_data_flush(log_handle);
} /* rfm_wcdma_log_agc */
...
/*!
  ...
  @details
  This function is exposed for WL1 to update the preferred antenna position beore calling prep tune so that
  rf build scripts for the position passed by WL1
...
#endif
  RF_MSG_1( RF_HIGH, "rfm_init_antenna ant_sw_pos = %d", ant_sw_pos);
...
/*!
  ...
  Bandmask
*/
uint64 rfm_wcdma_get_band_mask ( rfcom_device_enum_type device )
{
  return rfwcdma_mc_get_band_mask(device);
}
