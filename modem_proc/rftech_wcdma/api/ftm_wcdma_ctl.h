...
/*===========================================================================
  ...
  $Header: //components/rel/rftech_wcdma.mpss/3.11/api/ftm_wcdma_ctl.h#1 $ 
...
09/22/15   ljz     Enable serial NBEE calibration for W 
...
06/10/15   dbc     Add ftm_wcdma_set_pa_tunable_cap() declaration
06/03/15   dw      FTM char related fixes
05/30/15   dw      Added FTM state machine 
05/15/15   ak      Addition of WCDMA Init API
05/08/15   ak      Add API to map phone band mode from tools to rfcom wcdma band
04/29/15   rmb     Remove reference to Tx DAC Cal command.
04/22/15   ak      Adding support for script based tune for DC
04/21/14   ac      w plus w bring up changes
04/13/15   ak      Rx/Tx Split Changes to support new CRAT design
04/04/15   ak      Rx/Tx Split Changes
03/15/15   gh      Added API ftm_wcdma_execute_temp_comp_cb
03/02/15   rmb     Add new API for tuning Tx to DL channel.
03/05/15   sd      Added support for FTM WCDMA Sensitivity Estimation
02/22/15   xsh     Add ftm_wcdma_disable_NBEE API Definition
...
/* This is Enum to support different actions depending upon various WCDMA Scenarios*/
typedef enum
{
  FTM_WCDMA_DBDC = 0,
  FTM_WCDMA_DRDS_G_PLUS_W = 1,
  FTM_WCDMA_DRDS_W_PLUS_W = 2,
  FTM_WCDMA_DRDS_L_PLUS_W = 3, 
  FTM_WCDMA_INVALID_MODE,
} ftm_wcdma_scenarios_type;

/* FTM WCDMA state machine. Right now only has band info*/
typedef struct
{
  /* Note the device indexing here is before the device remapping */
  rfcom_wcdma_band_type ftm_wcdma_band[RFM_MAX_WAN_DEVICES];
}ftm_wcdma_state_type;
...
===========================================================================*/
ftm_rsp_pkt_type ftm_wcdma_set_cw(rfm_device_enum_type device, boolean on_off);
...
===========================================================================*/
void ftm_wcdma_set_offset_cw(rfm_device_enum_type device, int32 freq_offset, boolean on_off);
...
FUNCTION FTM_WCDMA_TUNE_TO_MULTI_CHAN V2

DESCRIPTION
  This function tunes the radio to a given uplink channel number.
  This uses the script based approach.
...
DESCRIPTION
This API will be called to return the current FTM_WCDMA_RF test scenario 			
...
FUNCTION 
ftm_wcdma_convert_phone_mode_to_wcdma_band

DESCRIPTION
This API will be called to map the phone band mode to rfcom wcdma ban			
...
ftm_wcdma_state_type
ftm_wcdma_data_get_wcdma_data
(
  void	
);

/*----------------------------------------------------------------------------*/
/*! Structure to hold WCDMA FTM data */
typedef struct
{
  /* TA doesn't have any unified global for FTM, add all the required globals here */
  ftm_wcdma_iq_acquired_samples_data_type *iq_capture_data;
  /*!< Variable to hold IQ capture data */
} ftm_wcdma_data_type;
