...
$Header: //components/rel/rftech_wcdma.mpss/3.11/api/rfnv_wcdma.h#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/16/16   ak      Change to avoid diversity lna swicth point corruption
10/1/15    vbh     Added a enum in rfwcdma_nv_pa_static_ver_type
09/16/15   vbh     Rename Rx V2 NV size with NV container ids
...
05/08/15   ak      [Rx/Tx Split]Add support to Cal MAX Rx devices
03/23/15   ska	   Add support for DGLNA feature
...
#define RFWCDMA_NUM_LNA_STATES_ASDIV 8
...
#define RFWCDMA_NUM_ASDIV_POS 2
...
/* Enum defining RFNV ids to be used to access the NV structures. 
   The enum should be kept up to date with respect to rfm_device_enum_type*/
typedef enum
{
  /*! RFNV ID  0 */
  RFNV_CONTAINER_ID_0,
  ...
  /*! Max NV Id */
  RFNV_CONTAINER_ID_MAX

}rfnv_wcdma_container_enum_type;
...
typedef enum
{
  RFWCDMA_NV_TX_STATIC_DATA_V1 = 1,
  RFWCDMA_NV_TX_STATIC_DATA_MAX
}rfwcdma_nv_tx_static_version_type;
...
typedef enum 
{
 ...
 /*!> CMM PA static NV Version 2 */
  RFWCDMA_NV_PA_STATIC_VER_2 = 2,
...
}rfwcdma_nv_rx_rsb_type;

/*------------Datatypes for AsDiv with eLNA FR 30081---------------------------------------------------------------*/

/*Rx LNA gain offset values in db10*/
typedef struct 
{
  int16 rx_gain_offset[RFWCDMA_NUM_LNA_STATES_ASDIV];
}rfwcdma_nv_rx_asdiv_elna_gain_offset_type;


/*Rx LNA Switchpoints in db10*/
typedef struct
{
 int16 lna_rise;
 int16 lna_fall;
}rfwcdma_nv_rx_lna_switchpoint_per_gain_state_type;

typedef struct
{
  rfwcdma_nv_rx_lna_switchpoint_per_gain_state_type lna_rise_fall[RFWCDMA_NUM_LNA_STATES_ASDIV-1];
}rfwcdma_nv_rx_asdiv_elna_switchpoint_type;
...
{
  /* Rx LNA rise/fall switchpoints per chain and per carrier */
  rfwcdma_nv_rx_lna_switchpoints_type rx_lna_swpts[RFNV_CONTAINER_ID_MAX][RFCOM_MAX_CARRIERS] ;

  /* Additional data for AsDiv eLNA FR 30081 - Rx gain offsets and switchpoints */
  
  rfwcdma_nv_rx_asdiv_elna_gain_offset_type *rx_asdiv_elna_offset_ptr[RFNV_CONTAINER_ID_MAX][RFWCDMA_NUM_ASDIV_POS];
  
  rfwcdma_nv_rx_asdiv_elna_switchpoint_type *rx_asdiv_elna_switchpoint_ptr[RFNV_CONTAINER_ID_MAX][RFWCDMA_NUM_ASDIV_POS];
  
  /*Additional flag to indicate whether v2 structure is populated using consolidated NV*/
...
/*----------------- Tx static NV addded for AsDiv eLNA FR 30081 ----------------------------------------------------------*/
typedef struct
{
  /* Tx Gain offsets*/
  int16 tx_asdiv_elna_offset[RFM_MAX_WAN_DEVICES][RFWCDMA_NUM_ASDIV_POS];
  	
}rfwcdma_nv_tx_static_payload_v1_type;

typedef union
{
  rfwcdma_nv_tx_static_payload_v1_type tx_stat_v1;
  
}rfwcdma_nv_tx_static_payload_type;

typedef struct
{
   /*Variant Marker*/
  rfnv_data_variant_marker_type tx_stat_variant_marker;

  /* Static Payload type */
  rfwcdma_nv_tx_static_payload_type payload;

}rfwcdma_nv_tx_static_data_type;
...
typedef struct
{
  ...
  uint16 rx_cal_chan[RFNV_CONTAINER_ID_MAX][NV_FREQ_TABLE_SIZ];

  /* Offset Vs Freq */
  rfwcdma_nv_rx_offset_vs_freq_type freq_offset[RFNV_CONTAINER_ID_MAX][NV_FREQ_TABLE_SIZ];

  /* Rx LNA and VGA offsets per chain and per carrier */
  rfwcdma_nv_rx_offset_type rx_offsets[RFNV_CONTAINER_ID_MAX][RFCOM_MAX_CARRIERS];
  
  /* Rx RSB Cal data */
  rfwcdma_nv_rx_rsb_type rsb_cal_data[RFNV_CONTAINER_ID_MAX];
...
typedef struct
{
 ...
 /* Static Tx data - offsets for AsDiv Elna FR 30081 */
  rfwcdma_nv_tx_static_data_type tx_static_data;
...
/* ----------------------------------------------------------------------- */
void rfnv_wcdma_set_tx_static_data_to_default
(
  rfnv_wcdma_nv_tbl_type *rfnv_ptr
);
