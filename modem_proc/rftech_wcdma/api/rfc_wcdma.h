...
$Header: //components/rel/rftech_wcdma.mpss/3.11/api/rfc_wcdma.h#1 $

when       who     what, where, why
------------------------------------------------------------------------------- 
07/01/16   sk      GRFC pin in W Rx path swing during idle if GPS is ON. Updated with correct band
10/28/15   dbc     Added configure_asm_xsw_port declaration
03/19/15   rmb     Mode config data is device independent.
4/22/15    vbh     Fix the rfc wcdma tx, rx band array size leading to memory corruption
03/19/15   rmb     Mode config data is device independent.
04/28/15   rmb     Remove feature flag RF_HAS_MEAS_INTF_TH_2_0 but retain code
                   added under this flag.
03/19/15   rmb     Mode config data is device independent. Do it with Meas interface flag.
...
public:
  ...
  virtual void *mode_config_get(rfc_mode_config_data_enum_type config_data);
  ...
  virtual int32 sleep(rfm_device_enum_type device, rfcom_wcdma_band_type rx_wcdma_band);
  ...
  virtual uint32 get_fb_chain(rfm_device_enum_type tx_device, rfcom_wcdma_band_type tx_band);
...
protected:
  ...
  rfcom_wcdma_band_type rx_band[RFM_MAX_WAN_DEVICES];
  rfcom_wcdma_band_type tx_band[RFM_MAX_WAN_DEVICES];
...
private:
  ...
  void *wcdma_rf_devices[RFM_MAX_WAN_DEVICES][RFC_CONFIG_RXTX_MAX][RFCOM_NUM_WCDMA_BANDS][RFDEVICE_TYPE_MAX_NUM][RFC_MAX_DEVICE_INSTANCES_PER_BAND];
