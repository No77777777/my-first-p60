...
/*===========================================================================
  ...
  $Header: //components/rel/rftech_wcdma.mpss/3.11/api/rfwcdma_mdsp_types.h#1 $
...
11/03/15   rmb     Make event handle an array of RFLM handles to support concurrent Tune 
                   in Multi Sim scenarios.
09/29/15   sd      FR30081 AsDiv with eLNA gain compensation 
06/04/15   rmb     Add support
06/02/15   gh/zhh  Added support to set and get RFLM THERM_ENABLE state
04/22/14   dw      Added RFWCDMA_MDSP_TXLM_UPDATE_CMD_ID
12/12/14   rmb     Add Meas config and update the same in DM. 
...
typedef enum
{
  ...
  /*! Command Id to inidicate TxLM update for SC/DC reconfiguration*/
  RFWCDMA_MDSP_TXLM_UPDATE_CMD_ID,
...
*/
typedef rflm_wcdma_irat_meas_enum_type rfwcdma_mdsp_rx_irat_meas_enum_type;
...
/*! 
  @brief
  Structure definition of RFWCDMA Measurement parameters in Build script routine
 
*/
typedef rflm_wcdma_meas_build_script_cfg_type rfwcdma_mdsp_meas_build_script_cfg_type;

/*----------------------------------------------------------------------------*/
/*! 
  @brief
  Structure definition of RFWCDMA Measurement parameters in Meas Enter routine
 
*/
typedef rflm_wcdma_meas_enter_cfg_type rfwcdma_mdsp_meas_enter_cfg_type;
...
typedef struct
{
  ...
  /*! < Flag to notify FW increase the DC loop gain for a short period of duration
       0: Normal mode
       1: Increase DC loop gain for short period of durtaion */
  boolean dc_loop_gain_burst_mode;
...
typedef struct
{
  /*! Specifies event handle per RFLM handle*/
  rfcommon_mdsp_event_handle* event_handle[RXLM_MAX_BUFFERS];
...
typedef enum
{
  ...
  /*! Configure IRAT related parameters in Build Script routine */
  RFWCDMA_MDSP_CONFIG_MEAS_BUILD_SCRIPT_PARAM,

  /*! Configure IRAT related parameters in Meas enter routine */
  RFWCDMA_MDSP_CONFIG_MEAS_ENTER_PARAM,

  /*! Set RFLM based Therm enable state */
  RFWCDMA_MDSP_SET_RFLM_THERM_ENABLE_STATE,

  /* Set the AsDiv Tx offset for FR 30081*/
  RFWCDMA_MDSP_SET_ASDIV_TX_GAIN_OFFSET,
  
  /*! Set selftest running indication */
  RFWCDMA_MDSP_SET_SELFTEST_IND,

  /*! Update the current Tx Freq to MDSP */
  RFWCDMA_MDSP_SET_TX_FREQ,

  /*! Update CAL mode to Tx dm status*/
  RFWCDMA_MDSP_SET_CAL_MODE,
  
  /* Set the AsDiv Tx offset for FR 30081*/
  RFWCDMA_MDSP_APPEND_ASDIV_QAT_TX_GAIN_OFFSET,
...
/*! 
  @brief
  FR 300081 Structure definition to override AsDiv TxAGC offset to FED 
...
typedef struct
{
  ...
  union
  {
    ...
    /*! Configure IRAT related parameters in Build Script routine */
    rfwcdma_mdsp_meas_build_script_cfg_type meas_build_script_param;

    /*! Configure IRAT related parameters in Meas Enter routine */
    rfwcdma_mdsp_meas_enter_cfg_type meas_enter_param;

    /*! Set AsDiv Tx gain offset structure*/
    rfwcdma_mdsp_set_asdiv_tx_gain_offset asdiv_tx_offset;
