...
$Header: //components/rel/rftech_wcdma.mpss/3.11/api/rfwcdma_core_rxctl.h#1 $
...
07/27/15   rmb     Put RxAGC enable parameters in a structure.
...
05/12/15   vbh     Add device type, carrier id to enable tuner
...
03/19/15   rmb     Add Rx specific GRFC script based API and also mode config GRFC API.
...
typedef struct
{
  ...
  rfwcdma_mdsp_agc_action_type cmd_action[RFM_MAX_WAN_DEVICES];
  uint32 rxlm_buf_idx[RFM_MAX_WAN_DEVICES];
  uint8 ant_idx[RFM_MAX_WAN_DEVICES];
  boolean rf_warmup_en;
  boolean nbr_en_flag;
  uint8 nbr_rxlm_buf_idx;
}rfwcdma_core_rxctl_enable_rxagc_params;
...
/*! This function is called to update the RX RSB coeffiecients per LNA state.*/
