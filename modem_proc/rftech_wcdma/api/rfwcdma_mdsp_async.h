...
/*===========================================================================
  ...
  $Header: //components/rel/rftech_wcdma.mpss/3.11/api/rfwcdma_mdsp_async.h#1 $
  $DateTime: 2021/03/09 05:08:47 $
  $Author: pwbldsvc $

when       who     what, where, why 
--------   ---     ---------------------------------------------------------- 
08/04/15   ac      prefetch for timeline opt
...
03/27/15   rmb     Add an api to deconfig Rx in FED DM.
...
/*! 
  @brief
  Funtion to do RF WCDMA MDSP deconfigures Rx for the requested RFLM handle
 
  @details
  Perform all necessary cleanup related to RF WCDMA MDSP/FED module for Rx
  that is specific to requested RFLM handle
 
  @return
  TRUE if cleanup is successful
*/
boolean rfwcdma_mdsp_async_deconfig_rx
(
...
/*! 
  @brief
  Funtion to submit the tx-agc dm function in mc
 ...
 Funtion to submit the tx-agc dm function in mc
  ...
  boolean
*/
boolean rfwcdma_mdsp_async_txagc_prefetch
(
  rflm_dm_handle_id_t rflm_handle,rfcom_device_enum_type tx_device
);
/*----------------------------------------------------------------------------*/
/*! 
  @brief
