...
$Header: //components/rel/hdr.mpss/6.0/api/hdrcom_api.h#2 $ $DateTime: 2019/08/29 22:50:51 $ $Author: pwbldsvc $

when         who     what, where, why
----------   ---     --------------------------------------------------------
08/30/2019   svu     Fixed Compiler error
02/19/2015   vko     Added support for D2L redirection
...
#define hdrcom_srchtclte_rpt_cell_struct_type  hdrl1_srchtclte_rpt_cell_struct_type
#define hdrcom_srchtclte_rpt_struct_type    hdrl1_srchtclte_rpt_struct_type
#define hdrind_srchtclte_rpt_struct_type    hdrcom_srchtclte_rpt_struct_type
...
/*~ IF ( _DISC_ == HDRIND_OVHD_NLB_CACHE_UPDATED ) 
  ...
  hdrcom_srchtclte_rpt_struct_type redirect_rpt;
} hdrcom_ind_data_union_type;

typedef enum
{
  HDRLOG_DIPSW_RESERVED        = HDRCOM_BITMASK(0), 
    /* 0 - reserved, not compatible with current QXDM interface */

  HDRLOG_DIPSW_40S_SLEEP_OFF   = HDRCOM_BITMASK(1), 
    /* 0 - 40s sleep enabled,             1 - disabled          */

  HDRLOG_DIPSW_SLEEP_OFF       = HDRCOM_BITMASK(2),
    /* 0 - Sleep enabled, 1 - disable sleeping all together     */

  HDRLOG_DIPSW_RPC_OFF         = HDRCOM_BITMASK(3),
    /* 0 - Reverse Power Control enabled, 1 - disabled          */

  HDRLOG_DIPSW_THROTTLING_OFF  = HDRCOM_BITMASK(4),
    /* 0 - Throttling enabled,            1 - disabled          */

  HDRLOG_DIPSW_EARLY_TERM_OFF  = HDRCOM_BITMASK(5),
    /* 0 - Early termination enabled,     1 - disabled          */

  HDRLOG_DIPSW_RL_RATE_INERTIA = HDRCOM_BITMASK(6),
    /* 0 - RL Rate Inertia disabled,      1 - enabled          */

  HDRLOG_DIPSW_ANT1_TEST_MODE  = HDRCOM_BITMASK(7),
    /* 0 - Ant1 test mode disabled,       1 - enabled          */

  HDRLOG_DIPSW_DDTM            = HDRCOM_BITMASK(8),
    /* 0 - DDTM disabled,                 1 - enabled           */

  HDRLOG_DIPSW_VT_RATE_INERTIA = HDRCOM_BITMASK(9),
    /* 0 - RMAC1 rate_params normal,      1 - rate_params VT    */

  HDRLOG_DIPSW_TAP_TCOFS       = HDRCOM_BITMASK(10),
    /* 0 - TC OFS is disabled,            1 - enabled          */

  HDRLOG_DIPSW_FLAT_OFF        = HDRCOM_BITMASK(11),
    /* 0 - FLAT off is disabled (FLAT is on), 1 - enabled      */

  HDRLOG_DIPSW_DISABLE_MUP_FILTERING  = HDRCOM_BITMASK(12),
    /* 0 - MUP Filtering Enabled, 1 - MUP filtering Disabled */

  HDRLOG_DIPSW_MAX = HDRCOM_BITMASK(HDRCOM_LOG_DIPSW_NUM - 1) 
