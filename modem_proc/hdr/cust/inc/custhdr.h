...
DESCRIPTION
  ...
  Copyright (c) 2001 - 2020 by Qualcomm Technologies Incorporated. All Rights Reserved.
===========================================================================*/
...
$Header: //components/rel/hdr.mpss/6.0/cust/inc/custhdr.h#3 $ $DateTime: 2020/07/22 06:10:14 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
07/14/20   svu     Fixed comp errors generated for C2K compileout flovours
12/04/19   vaa     Changes to support clk plan V2.0 FR changes
12/22/17   vke     FR42394 PBR randomization for non-DR idle feature
08/24/17   rmv     Disabled Access TA for UMTS2 (DO+W)
08/24/17   rmv     Mainline FEATURE_HDR_QTA_THRU_TRM 
08/24/17   vko     Enable FEATURE_HDR_SUPPORTS_FW_TIMING_STATE for 
                   1xSRLTE+W QTA
11/04/16   vko     FR 37336: Migration to MSGR VARIANT from Inst ID - HDR
11/18/15   vko     Enabled FEATURE_HDR_TO_LTE_REDIRECTION for TA2.1/2.2
08/18/15   arm     Moved DR-DSDS feature under FEATURE_DUAL_SIM
06/18/15   vlc     Enabled FEATURE_HDR_TO_LTE_REDIRECTION for THOR only.
06/11/15   vke     Enabled FC removal feature for TA
06/09/15   tnp     Update feature flag configuration
06/02/15   mbs     Enabled DR-DSDS feature
05/28/15   rmv     Enabled FEATURE_HDR_D2L_RF_MEAS on TA 
05/26/15   vke     Defined TRM unification macro for all targets
05/04/15   mbs     Supported QTA using TRM API
04/23/15   vke     Added TRM unification changes
04/13/15   vlc     Added FEATURE_HDR_D2L_RF_MEAS for RF API changes. 
04/13/15   vke     Enabled FEATURE_HDR_QTA for Tabasco builds only
03/25/15   sat     Enabled FEATURE_HDR_RUMI for RUMI builds only
...
// error : inserted by coan: "#define FEATURE_DEDICATED_C2K_MODE_DATA" contradicts -U or --implicit at D:\Builds\hdr.mpss.3.2.cr_feature\modem_proc\hdr\cust\inc\custhdr.h(460)
...
#ifdef T_MDM9X00A
// error : inserted by coan: "#define FEATURE_HDR_DIV_SRCH_WORKAROUND" contradicts -U or --implicit at D:\Builds\hdr.mpss.3.2.cr_feature\modem_proc\hdr\cust\inc\custhdr.h(490)
    /* MDM9K 1.1 workaround for search with div enabled */

// error : inserted by coan: "#define FEATURE_HDR_FORCE_NORMAL_AGC_ACQ" contradicts -U or --implicit at D:\Builds\hdr.mpss.3.2.cr_feature\modem_proc\hdr\cust\inc\custhdr.h(493)
...
/*This is enabled for RUMI builds*/
...
#define FEATURE_HDR_CLK_BOOST_QTA_GAP
  /* QTA clk boost for HI PL */
#endif /* FEATURE_QTA */

#define FEATURE_HDR_BIMC_BOOST_DIV_ON
  /* Diversity Enable clk boost for HI PL */

/* for testing only */
#define FEATURE_HDR_IRAT_CLK_BOOST
  /* IRAT clk boost for HI PL */
...
#define FEATURE_HDR_PBR_RANDOMIZATION
...
#define FEATURE_HDR_TRM_UNIFICATION
...
#define FEATURE_HDR_MSGR_VARIANT
#endif /* FEATURE_MSGR_VARIANT */


#define FEATURE_HDR_SUPPORTS_FW_TIMING_STATE
 /* Enable FW timing state for DO + W support on TA */
