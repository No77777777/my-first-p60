...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdralmps.c#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $
...
/* To INACTIVE  INIT   IDLE   CONNECTED  SUSPEND */
    ...
    { TRUE,     TRUE,  TRUE,  FALSE,     TRUE     }, /* From HDRALMPS_CONNECTED */
#else
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (hdrstream_stream_is_assigned(HDRHAI_APP_PACKET_APP_ANY))
  {
    ...
    (void) sd_ss_ind_hdr_opr_start_activity( 
                 ...
                 NULL, SD_SS_HDR_ACT_MASK_TRAFFIC );
  }
  else
  {
	...
	(void) sd_ss_ind_hdr_opr_start_activity( 
                 ...
                 hdrmc_get_hdr_sub_id(),
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
                 NULL, SD_SS_HDR_ACT_MASK_SESSION );
  }
...
#ifdef FEATURE_HDR_TO_LTE_REDIRECTION
  else if(next_state == HDRALMPS_SUSPEND)
  {
	  hdrrup_deactivate(HDRHAI_AIR_LINK_MANAGEMENT_PROTOCOL);
	  hdrovhd_deactivate(HDRHAI_AIR_LINK_MANAGEMENT_PROTOCOL);
	  hdrind_give_ind( HDRIND_ALMP_RETURNING_TO_IDLE, NULL ); /* Clear the SLP queues */
  }
#endif /* FEATURE_HDR_TO_LTE_REDIRECTION */
  else
  {
...
LOCAL void hdralmps_exit_suspend( hdralmps_state_enum_type next_state )
{
  ...
  hdralmp_clr_hdr_to_lte_resel_info();
