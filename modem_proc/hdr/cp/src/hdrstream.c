...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdrstream.c#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while ((total_bits_remaining - attrib_len) >= HDRSTREAM_MIN_ATTRIB_LEN)
  {
    ...
    if (HDRSTREAM_UNPACK_CON_REQ_LEN(item_ptr, &attrib_len_bytes, 
     ...
     */
    attrib_len = attrib_len_bytes * HDRSTREAM_BITS_PER_BYTE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while ((total_bits_remaining - attrib_len) >= HDRSTREAM_CRSP_ATTRIB_LEN)
  {
    ...
    if (HDRSTREAM_UNPACK_CON_RSP_LEN(item_ptr, &attrib_len_bytes, attrib_offset) !=
        E_SUCCESS)
    {
