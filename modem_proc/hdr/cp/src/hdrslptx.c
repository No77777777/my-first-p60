...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdrslptx.c#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
11/23/15   vko     Removed ASSERT for pkt fragmented size more than the original size
...
)
{
  ...
  else
  {
    ...
    *(((uint8*) &hdrslp_fragment_header) + 1) = 
    ...
    {
        /* something is wrong. Ignore this fragment buffer and continue*/
        HDR_MSG_PROT_3( MSG_LEGACY_MED, "Ignore pkt !! frg incorrect, seq=%d, %d/%d bytes\n",	
                                         hdrslp_fragment_seqno - 1,
                                         hdrslp_fragment_tx_count,
                                         org_sdu_ptr->length);
        
        dsm_free_packet(&(frg_sdu_ptr->msg_ptr));
		/* reset the frg buffer	 */
		hdrslp_fragment_buffer	 = NULL;
		hdrslp_fragment_header	 = 0;
		hdrslp_fragment_pointer  = 0;
		
		return 0; /* This will reset the rexmit buffer entry in the caller */
    }
