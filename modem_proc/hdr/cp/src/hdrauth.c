...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdrauth.c#1 $
...
)
{
  ...
  while ((total_bits_remaining - attrib_len) >= HDRAUTH_MIN_ATTRIB_LEN)
  {
    ...
    if( HDRAUTH_UNPACK_CON_REQ_LEN( item_ptr, &attrib_len_bytes, attrib_offset ) != 
        E_SUCCESS )
    {
     ...
     */
    attrib_len = attrib_len_bytes * HDRAUTH_BITS_PER_BYTE;
