...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdridlec.c#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  */
  ...
  while ((total_bits_remaining - attrib_len) >= HDRIDLEC_MIN_ATTRIB_LEN)
  {
    ...
    if (HDRIDLEC_UNPACK_CON_RSP_LEN(item_ptr, &attrib_len_bytes, attrib_offset) !=
        E_SUCCESS)
    {
     ...
     */
    attrib_len = attrib_len_bytes * HDRIDLEC_BITS_PER_BYTE;
