...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdrpcp_slp_pkt_app.c#1 $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/12/16   svu     Fixed KW issues
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    for( i = 0; i < num_outcomes; i++ )
    {
      ...
      if( (crr_tx_outcome->carrier_id == cb_ptr->carrier_id) && (combo_id < HDRPCP_MAX_COMBOS) )
      {
