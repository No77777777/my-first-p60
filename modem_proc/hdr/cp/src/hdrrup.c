...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdrrup.c#2 $ $DateTime: 2020/04/27 09:08:35 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ------------------------------------------------------------ 
04/22/20   par     Fixed KW issue
...
11/20/15   vko     Fixed TCA with Incorrect feedback_rev_ch_id (Value is 0)
...
LOCAL struct
{
  ...
  #if  defined(FEATURE_LTE_TO_HDR_OH)
  ...
  boolean system_loss_while_nset_update;
} hdrrup;
...
)
{
  ...
  ---------------------------------------------------------------------------*/
  if ( hdrutil_fs_get( HDRRUP_1X_TUNEAWAY_IN_SES_CFG_ENABLED,
...
)
{
  ...
  hdrrup.system_loss_while_nset_update = FALSE;
...
#ifdef GPP2_SUPPORT_ON_SEC_SUB 
    cmd_ptr->hdr_meas_resp.hdr.asubs_id = hdrmc_get_hdr_sub_id(); 
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  case HDRIND_ACMAC_TRANSMISSION_SUCCESSFUL:
      ...
      ret_val = sd_ss_ind_hdr_opr_acc_term(
#ifdef GPP2_SUPPORT_ON_SEC_SUB 
                                  hdrmc_get_hdr_sub_id(),
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
...
FUNCTION hdrrup_check_avoided_channel_in_aset

DESCRIPTION
  This function checks if an avoided channel is in aset or not.
...
SIDE EFFECTS
  None.
===========================================================================*/
LOCAL boolean hdrrup_check_avoided_channel_in_aset(const uint8 num_avoided_channels,
                                                        const sys_avoided_channel_record_type   *avoided_list_ptr, 
                                                        const uint8 num_aset_channels,
                                                        const sys_channel_type *aset_list_ptr)


{
    uint8 i = 0;
	uint8 j = 0;
	boolean avoided_channel_in_aset = FALSE;
    /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
	
	for(i = 0; i < num_avoided_channels; i++)
	{
        for(j = 0; j < num_aset_channels; j++)
        {
            if(SYS_CHAN_CMP_EQUAL(avoided_list_ptr[i].channel, aset_list_ptr[j]))
            {
				avoided_channel_in_aset = TRUE;
                      HDR_MSG_PROT_2 (MSG_LEGACY_HIGH,"Avoided channel %d band %d is in ASET",
                                        avoided_list_ptr[i].channel.chan_num,avoided_list_ptr[i].channel.band);                      
				break;
            }

        }
		if(avoided_channel_in_aset == TRUE)
			break;

	}

	return avoided_channel_in_aset;
...
LOCAL void hdrrup_process_ovhd_info_updated_ind( void )
{
  ...
  sys_channel_type aset_channel_list[HDRSRCH_MAX_NUM_SUBASETS];

  uint8 num_aset_channels = 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* Check for ASET Channel list */
  num_aset_channels = hdrlmac_get_aset_channel_list(aset_channel_list);

  if(num_aset_channels > HDRSRCH_MAX_NUM_SUBASETS)
  {
    HDR_MSG_PROT_2 (MSG_LEGACY_ERROR, "num_aset_channels is %d greater than "
                    "max allowed. limiting to max %d.", num_aset_channels, 
                                                HDRSRCH_MAX_NUM_SUBASETS);
    num_aset_channels = HDRSRCH_MAX_NUM_SUBASETS;
  }
  hdrrup.system_loss_while_nset_update = hdrrup_check_avoided_channel_in_aset(num_avoided_channels, 
  	                                                                          avoided_chan_rec,
  	                                                                          num_aset_channels,
  	                                                                          aset_channel_list  );
  
  HDR_MSG_PROT_2 (MSG_LEGACY_HIGH, "Avoided channel in aset returned %d. num aset channels =%d ",
  	                                hdrrup.system_loss_while_nset_update, num_aset_channels);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for (subaset_id = 0; subaset_id < tca_msg_ptr->num_sub_asets; subaset_id++)
  {
    ...
    for (cnt = 0; cnt < tca_msg_ptr->num_sectors; cnt++)
    {
      ...
      /* Number of pilots shouldn't be greater than maximum ASET size */
      ASSERT(num_pilots < HDRSRCH_ASET_MAX_SIZE);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /* Initializing the tca_link structure values*/  
  memset( &tca_link, 0, sizeof(sys_link_type));
  ...
  do
  {
    ...
    for( cnt = 0; cnt < hdrrup.tca_msg.num_sub_asets; cnt++ )  
    {
      ...
      if (hdrrup.tca_msg.sub_aset[cnt].ftc_params.feedback_enabled)
      {
        ...
        offset += HDRRUP_TCA_FEEDBACK_REV_CH_INDEX_SIZE;
        if (hdrrup.tca_msg.sub_aset[cnt].ftc_params.feedback_rev_ch_id == 0)
        {
          /* The feedback_rev_ch_id should not be 0*/
          HDR_MSG_PROT(MSG_LEGACY_ERROR, "FeedbackReverseChannelIndex set to 0");
          unpack_result = E_PROT_UNKN_MSG;
          break;
        }
      }
...
/*===========================================================================
FUNCTION hdrrup_is_system_loss_in_progress

DESCRIPTION
  This function returns whether system loss can happen now, because of avoided channel in aset.
...
RETURN VALUE
  TRUE - If system loss can happen now
  FALSE - If there is no system loss

SIDE EFFECTS
  None.
===========================================================================*/
boolean hdrrup_is_system_loss_in_progress( void )
{
   return hdrrup.system_loss_while_nset_update;
}/* hdrrup_is_system_loss_in_progress */
