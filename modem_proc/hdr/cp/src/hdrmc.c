...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdrmc.c#5 $ $DateTime: 2020/04/27 03:53:32 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
04/24/20   vaa     Set pending_auto_deactivate_send to TRUE on RTT timer expiry
04/13/20   svu     FR 55482: Extension of elevator mode to C2K RAT
01/21/20   vaa     Fixed Compiler warnings
11/25/19   vko     Fixed wdog reporting issue
11/12/18   svu     Update lmac about bootup subid 
09/11/18   vaa     Starting a timer on receiving RTT req from LTE
09/06/18   svu     Added variant ID changes for LTE mode change indication
08/01/18   svu     Fix bootup session restoration
04/13/18   svu     Fixed sending auto deactivate indication to MMODE
04/13/18   svu     Fixed raced condition between LTE_HDR_HO_CNF and deact_ind
04/13/18   vko     Fixed race condition between CGI abort and CGI RSP
02/12/18   vko     Trigger D2L redirection if mode pref changed and includes LTE
01/29/18   svu     Updating the srch subid info in inactive state.
01/29/18   vko     Return failure if MCFG refresh command is recieved for incorrect SUB id
01/29/18   svu     Fixed subid check while updatig the FPLMN list
01/25/18   svu     Maintaining two seperate LTE FPLMN ID lists 
01/22/18   pga     Fixing KW issue
01/18/18   svu     Fixed restoring session on to the corresponding subid
01/17/18   svu     HDR registers for MCFG refresh cmd cb on both SUB1 and SUB2
01/17/18   svu     Fixed eHRPD credentials erasure Issue
01/16/18   svu     Fixed HDR provisioning eHRPD credentials based on activating SUB id  
01/10/18   svu     Fixed ignoring MMGSDI events during NV relao
12/27/17   svu     Fixed calling MSGR API for filling the correct Variant ID
11/21/17   vko     Revert temperory DDS fix, i.e. disable QTA if LTE sub is not present
10/30/17   vko     Fix eHRPD not to disable, if HDR can be active on either of the SUB
10/30/17   vko     Fix eHRPD not working on 2nd SUB
...
08/07/17   svu     Fixed compiler errors
08/03/17   vko     Use NO VARIANT for msgr registering of non-SUB msgs
08/03/17   vko     MSGR Instance ID to Variant ID change
08/02/16   vko     Fix UE NOT sending UE mode change request to MMODE
05/27/16   vko     FR 35747: Graceful handling of UE mode collision and X2L IRAT
05/27/16   vko     QSH framework for events capturing
4/28/16    vko     Fix Heap memory exhaustion issue when TC_MODE cmd is received,
                   when HDR inactive continuosly
01/19/16   vko     Fixed HDR sending PROT_DEACT_CNF instead of AUTO_PROT_DEACT_CNF
...
09/30/15   vko     Send abort reques to L1, if CP received deactivate while D2L 
                   is in progress
08/10/15   vko     Fixed crash while D2L redirection failed rsp is received
...
05/18/15   tnp     Fix compiler warnings
...
03/13/15   wsh     Added NV entry for scan mode to .conf file
...
#define HDRMC_WRITE_EFS_NV_CONF_FILE( config_fd, file_path ) \
    { \
      result = hdrutil_fs_write( (config_fd), (file_path), strlen((file_path)) ); \
      result += hdrutil_fs_write( (config_fd), new_line, 1 ); \
      if ( result != ( strlen((file_path)) + 1 ) ) \
      { \
        HDR_MSG_PROT_1( MSG_LEGACY_ERROR, \
                        "Error writing to config file errno %d", hdrutil_fs_errno() ); \
...
typedef struct
{
  ...
  sys_plmn_id_s_type fplmnid[HDRCP_MAX_FPLMN_LIST_SIZE];
    /* Forbidden PLMN list */

} hdrmc_lte_fplmn_list_type;
...
#define HDRMC_1X_PROV_SEC_SESSION  3
#ifdef FEATURE_EHRPD
...
LOCAL struct
{
  ...
  boolean pending_auto_deactivate_send ; 
    /* Indicates whether handoff is aborted by LTE in HDR inactive state */
  ...
  mmgsdi_session_id_type mmgsdi_session_id_sub0;
  mmgsdi_session_id_type mmgsdi_session_id_sub1;
  boolean mmgsdi_session_ready_sub0;
  boolean mmgsdi_session_ready_sub1;
#ifdef FEATURE_EHRPD
  ...
  mmgsdi_slot_id_enum_type sec_slot_id; 

  boolean ehrpd_credentials_are_provisioned_sub0;
  boolean ehrpd_credentials_are_provisioned_sub1;
#endif /* FEATURE_EHRPD */
#endif /* FEATURE_MMGSDI */
  ...
  boolean sub_id_is_valid;
  boolean mcfg_refresh_is_in_progress;
  hdrmc_ue_mode_switch_cnf_info_type ue_mode_switch_cnf;	
  boolean sub_id_change_in_progress;
  hdrmc_lte_fplmn_list_type lte_fplmn_list_sub0;
  hdrmc_lte_fplmn_list_type lte_fplmn_list_sub1;
  sys_modem_as_id_e_type bootup_subid;	
  hdrmc_tc_mode_info_type tc_mode_info; /* tc mode info */
  uint8 status;
  rex_timer_type lte_to_hdr_rtt_timer;
} hdrmc;
...
#undef  DOG_HDRMC_RPT
#define DOG_HDRMC_RPT hdrmc_dog_rpt_var
...
boolean hdrmc_check_ehrpd_credentials_valid( void );
void hdrmc_update_lte_fplmn ( uint32 num_fplmn,sys_plmn_id_s_type *fplmn, sys_modem_as_id_e_type sub_id);
#ifdef FEATURE_HDR_DEBUG_INFO
...
/*============================================================================
FUNCTION HDRMC_MSGR_CLIENT_INIT
...
===========================================================================*/
void hdrmc_msgr_client_init ( void )
{
...
===========================================================================*/
void hdrmc_msgr_msg_init ( sys_modem_as_id_e_type as_id )
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  estatus = msgr_register_variant ( MSGR_HDR_CP,
...
===========================================================================*/
void hdrmc_msgr_msg_stop ( sys_modem_as_id_e_type as_id )
{
...
#ifdef FEATURE_LTE_TO_HDR
  estatus = msgr_deregister_variant ( MSGR_HDR_CP,
                              &hdrmc.msgr_client_id,
                              HDR_CP_LTE_REDIR_REQ,
                              as_id);
...
/*===========================================================================
FUNCTION HDRMC_MSGR_CLIENT_STOP
...
===========================================================================*/
void hdrmc_msgr_client_stop ( void )
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  HDR_MSG_PROT_6( MSG_LEGACY_MED, 
                  "refresh cmd, type (%d), slot (%d), sub (%d), ref ID (%d) hdr state (%d) sub_id (%d)",
                  refresh_info->type, refresh_info->slot_index,
                  refresh_info->sub_index, refresh_info->reference_id, 
                  hdrmc.state, hdrmc_get_hdr_sub_id() );

  if((hdrmc.state == HDRMC_ONLINE_STATE) && ((sys_modem_as_id_e_type) refresh_info->sub_index != hdrmc_get_hdr_sub_id()))
  {
     HDR_MSG_PROT( MSG_LEGACY_MED, 
                      "Refresh cmd received on different SUB than HDR active. Returning failure");
     return FALSE;
...
void hdrmc_task_init( void )
{
  ...
  hdrmc.sub_id_change_in_progress = FALSE;
  hdrmc.sub_id =  SYS_MODEM_AS_ID_1;
  hdrmc.sub_id_is_valid = TRUE;
#ifdef FEATURE_EHRPD
  hdrmc.ehrpd_credentials_are_provisioned_sub0 = FALSE;
  hdrmc.ehrpd_credentials_are_provisioned_sub1 = FALSE;
#endif /* FEATURE_EHRPD */
  ...
  /* HDR on primary and secondary session */
  ...
  hdr_qsh_register(QSH_CLT_HSRCH);
#endif
} /* hdrmc_task_init() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else if (lte_to_hdr_ho_info_ptr->irat_ho_type == HDRMC_IRAT_LTE_TO_HDR_REDIRECT)
  {
    ...
    while (( chan_idx > (num_freq -1) ) &&
           ...
           (band_idx < HDR_CP_MAX_CDMA2000_BANDS - 1))
    {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  HDR_MSG_PROT(MSG_LEGACY_HIGH, "Clearing HO type");
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    msgr_init_hdr_variant ( &irat_ho_rsp.lte_redirect_failed.hdr, 
                            MSGR_HDR_CP, 
                            HDR_CP_LTE_REDIR_FAILED_RSP, hdrmc_get_hdr_sub_id() );
  ...
  {
    msgr_init_hdr_variant ( &irat_ho_rsp.lte_reselect_failed.hdr, 
                            MSGR_HDR_CP, 
                            HDR_CP_LTE_RESEL_FAILED_RSP, hdrmc_get_hdr_sub_id() );
  ...
  {
    msgr_init_hdr_variant ( &irat_ho_rsp.lte_rev_tt.hdr, 
                            MSGR_HDR_CP,
                            HDR_CP_LTE_REVERSE_TIMING_TRANSFER_RSP, hdrmc_get_hdr_sub_id() );

    irat_ho_rsp.lte_rev_tt.status = HDRCP_LTE_REVERSE_TIME_TRANSFER_FAILED;

    estatus = msgr_send ( &irat_ho_rsp.lte_rev_tt.hdr,
                          sizeof ( hdrcp_msg_rsp_u ) );
    hdrmc_stop_lte_to_hdr_rtt_timer();
  }
  else if (hdrmc.lte_to_hdr_ho_info.irat_ho_type == HDRMC_IRAT_LTE_TO_HDR_GET_CGI)
  {
    msgr_init_hdr_variant ( &irat_ho_rsp.lte_get_cgi_rsp.hdr, 
                            MSGR_HDR_CP,
                            HDR_CP_LTE_GET_CGI_RSP, hdrmc_get_hdr_sub_id() );
  ...
  {
    msgr_init_hdr_variant ( &irat_ho_rsp.lte_active_ho_failed.hdr, 
                            MSGR_HDR_CP,
                            HDR_CP_LTE_ACTIVE_HO_FAILED_RSP, hdrmc_get_hdr_sub_id() );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  msgr_init_hdr_variant ( &tt_rsp.lte_rev_tt.hdr, MSGR_HDR_CP,
                          HDR_CP_LTE_REVERSE_TIMING_TRANSFER_RSP, hdrmc_get_hdr_sub_id() );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  msgr_init_hdr_variant ( &cgi_rsp.lte_get_cgi_rsp.hdr, MSGR_HDR_CP,
                          HDR_CP_LTE_GET_CGI_RSP, hdrmc_get_hdr_sub_id() );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (hdrmc.lte_to_hdr_ho_info.irat_ho_type == HDRMC_IRAT_LTE_TO_HDR_REDIRECT)
  {
    msgr_init_hdr_variant ( &irat_ho_abort_rsp.hdr, 
                            MSGR_HDR_CP, 
                            HDR_CP_LTE_REDIR_ABORT_RSP, hdrmc_get_hdr_sub_id() );
  ...
  else if (hdrmc.lte_to_hdr_ho_info.irat_ho_type == HDRMC_IRAT_LTE_TO_HDR_RESELECT)
  {
    msgr_init_hdr_variant ( &irat_ho_abort_rsp.hdr, 
                            MSGR_HDR_CP, 
                            HDR_CP_LTE_RESEL_ABORT_RSP, hdrmc_get_hdr_sub_id() );
  ...
  else if (hdrmc.lte_to_hdr_ho_info.irat_ho_type == HDRMC_IRAT_HDR_TO_LTE_TIME_TRANSFER)
  {
    msgr_init_hdr_variant ( &irat_ho_abort_rsp.hdr, 
                            MSGR_HDR_CP, 
                            HDR_CP_LTE_REVERSE_TIMING_TRANSFER_ABORT_RSP, hdrmc_get_hdr_sub_id() );
  ...
  else if (hdrmc.lte_to_hdr_ho_info.irat_ho_type == HDRMC_IRAT_LTE_TO_HDR_ACTIVE_HO)
  {
    msgr_init_hdr_variant ( &irat_ho_abort_rsp.hdr, 
                            MSGR_HDR_CP, 
                            HDR_CP_LTE_ACTIVE_HO_ABORT_RSP, hdrmc_get_hdr_sub_id() );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  msgr_init_hdr_variant ( &msg_rsp.lte_capabilities_rsp.hdr, 
                          MSGR_HDR_CP, HDR_CP_LTE_1X_HRPD_CAPABILITIES_RSP, hdrmc_get_hdr_sub_id() );
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
    hdr_qsh_event_notify(hdrutil_get_hdr_sub_id(),QSH_CLT_HSRCH, HSRCH_QSH_EVENT_CALL_ORIG_FAILURE);
#endif
  }
  else
  {
...
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
                                 PROT_ACT_GWL_REDIR, PROT_STATE_ONLINE_HDR);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
     if(hdrmc.pending_auto_deactivate_send  == TRUE)
     {
       hdrmc.pending_auto_deactivate_send  = FALSE;

       HDR_MSG_PROT_1( MSG_LEGACY_HIGH, 
                       "IRAT: no IRAT in progress.redir_allowed : %d"
                       ,cmd_ptr->msgr_cmd.msg.redir_protocol_cnf.is_irat_ho_allowed );
       if(cmd_ptr->msgr_cmd.msg.redir_protocol_cnf.is_irat_ho_allowed == TRUE)
       {
       HDR_MSG_PROT( MSG_LEGACY_HIGH, 
                       "Calling mmoc_rpt_prot_auto_deactd_ind" );
#ifdef GPP2_SUPPORT_ON_SEC_SUB
      mmoc_rpt_prot_auto_deactd_ind_subs( PROT_DEACT_REDIR_ABORT, 
...
#endif /* FEATURE_HDR_DEBUG_INFO */
       }/*cmd_ptr->msgr_cmd.msg.redir_protocol_cnf.is_irat_ho_allowed == TRUE*/  
       else
       {
          HDR_MSG_PROT( MSG_LEGACY_HIGH, 
                        "IRAT: no IRAT in progress.Avoided mmoc_rpt_prot_auto_deactd_ind API call" );
       }    
     }
     else
     {
  ...
  if (cmd_ptr->msgr_cmd.msg.redir_protocol_cnf.is_irat_ho_allowed == TRUE)
  {
    ...
    if (hdrmc.lte_to_hdr_ho_info.irat_ho_type == HDRMC_IRAT_LTE_TO_HDR_REDIRECT)
    {
      ...
      sd_action = sd_ss_ind_hdr_user_redir_lte( 
#ifdef GPP2_SUPPORT_ON_SEC_SUB 
                                 hdrmc_get_hdr_sub_id(),
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
                                   hdrmc_extract_lte_to_hdr_ho_chan, 
...
{
  HDR_MSG_PROT( MSG_LEGACY_HIGH, "Start LTE to HDR timer for RTT" );
  rex_def_timer_ex( &hdrmc.lte_to_hdr_rtt_timer,
                     hdrmc_lte_to_hdr_rtt_timer_expiry_cb, 0);
  ( void ) rex_set_timer( &hdrmc.lte_to_hdr_rtt_timer,
...
FUNCTION hdrmc_reload_NV

DESCRIPTION
  This function reloads the NV after subscription change.
...
===========================================================================*/
void hdrmc_reload_NV( )
{
 
  HDR_MSG_PROT ( MSG_LEGACY_HIGH, "HDR NV Reloading !!");
  /* We close session as SIM is changed. */
  if ( hdrsmp_session_is_open(HDRHAI_HDRMC) )
  {    
    HDR_MSG_PROT(MSG_LEGACY_HIGH, "reload NV, close session");
  ...
  HDR_MSG_PROT ( MSG_LEGACY_HIGH, "HDR NV Reload done");
} /* hdrmc_reload_NV() */
...
#ifdef FEATURE_HDR_MSGR_VARIANT
  variant = MSGR_GET_VARIANT(&cmd_ptr->msgr_cmd.msg.hdr);

  MSGR_CLEAR_VARIANT(cmd_ptr->hdr.cmd);

  if((hdrmc.sub_id_is_valid == TRUE) && (variant != hdrmc.sub_id))
  {
    HDR_MSG_PROT_3 (MSG_LEGACY_ERROR,
                    "====================== Unexpected msg %d received on variant %d curr_sub_id %d ===========",
                    cmd_ptr->hdr.cmd, variant, hdrmc.sub_id );
    return;
  }
#endif /* FEATURE_HDR_MSGR_VARIANT */  
#ifdef FEATURE_HDR_DEBUG_INFO
...
#ifdef GPP2_SUPPORT_ON_SEC_SUB 
         mmoc_rpt_ph_stat_chgd_cnf2( cmd_ptr->msgr_cmd.msg.ph_status_chgd.trans_id,
                                   mmoc_convert_sys_mode_asubsid_to_mmoc_mode(SYS_SYS_MODE_HDR, hdrmc_get_hdr_sub_id()), 
#ifdef MMOC_API_PH_STAT_CNF_WITH_STATUS
                                 MMOC_PH_STAT_ERR_NV_ERR,
#endif
                                 hdrmc_get_hdr_sub_id());
#else
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
      hdr_qsh_event_notify(hdrutil_get_hdr_sub_id(),QSH_CLT_HSRCH, HSRCH_QSH_EVENT_D2L_RESEL_FAILURE);
#endif
...
#ifdef FEATURE_HDR_DEBUG_INFO
           hdrmc_log_sent_rpt(hdrmc.ue_mode_switch_cnf.trans_id, HDR_MMOC_RPT_UE_MODE_SWITCH_CNF);
#endif /* FEATURE_HDR_DEBUG_INFO */
          mmoc_hdr_rpt_ue_mode_switch_cnf(hdrmc.ue_mode_switch_cnf.trans_id,hdrmc_get_hdr_sub_id());
 ...
 -------------------------------------------------------------------------*/
...
#ifdef FEATURE_HDR_DEBUG_INFO
        hdrmc_log_sent_rpt(cmd_ptr->msgr_cmd.msg.ue_mode_chg_req.trans_id, HDR_MMOC_RPT_UE_MODE_SWITCH_CNF);
#endif /* FEATURE_HDR_DEBUG_INFO */
	    mmoc_hdr_rpt_ue_mode_switch_cnf(cmd_ptr->msgr_cmd.msg.ue_mode_chg_req.trans_id, hdrmc_get_hdr_sub_id());
   ...
   break;
...
#ifdef FEATURE_QSH_EVENT_NOTIFY_TO_QSH
      hdr_qsh_event_notify(hdrutil_get_hdr_sub_id(),QSH_CLT_HSRCH, HSRCH_QSH_EVENT_D2L_REDIR_FAILURE);
#endif
...
#ifdef FEATURE_HDR_TO_LTE
        hdrovhd_update_earfcn_plmnid_info(&cmd_ptr->msgr_cmd.msg.lte_rrc_acq_db_earfcn_rsp); /* D2L reselection */
#endif
#ifdef FEATURE_HDR_TO_LTE_REDIRECTION
    ...
    case HDRMC_CM_LTE_FPLMN_UPDATE_REQ:
      HDR_MSG_PROT( MSG_LEGACY_HIGH, " Online rcv LTE FPLMN UPDATE req from CM" );
#ifdef FEATURE_HDR_MSGR_VARIANT
      hdrmc_update_lte_fplmn( cmd_ptr->msgr_cmd.msg.cm_lte_fplmn.plmn_list_size,
                              cmd_ptr->msgr_cmd.msg.cm_lte_fplmn.plmn,
                              variant);
#else
      hdrmc_update_lte_fplmn( cmd_ptr->msgr_cmd.msg.cm_lte_fplmn.plmn_list_size,
                              cmd_ptr->msgr_cmd.msg.cm_lte_fplmn.plmn,
                              INST_ID_TO_SYS_AS_ID(cmd_ptr->msgr_cmd.msg.hdr.inst_id));
#endif /* FEATURE_HDR_MSGR_VARIANT */

     

#ifdef FEATURE_HDR_MSGR_VARIANT
     if(hdrmc_get_hdr_sub_id() == variant)
     {
#else
     if(hdrmc_get_hdr_sub_id() == INST_ID_TO_SYS_AS_ID(cmd_ptr->msgr_cmd.msg.hdr.inst_id))
     {
 #endif /* FEATURE_HDR_MSGR_VARIANT */
     ...
     {
       HDR_MSG_PROT( MSG_LEGACY_HIGH, " Ignored updating LTE FPLMN list" );
     }
    break;
 ...
 /*-------------------------------------------------------------------------
   ...
   case HDRMC_CM_TRM_PRIORITY_IND:		
    ...
    /*-------------------------------------------------------------------------
               PRIORITIZE LTE MEAS 
    -------------------------------------------------------------------------*/
     case HDRMC_QMI_NAS_PRIORITIZE_LTE_IND:

      HDR_MSG_PROT(MSG_LEGACY_HIGH, "Online rcv Prioritize LTE meas");

      hdralmp_prio_lte_meas_pref();
       
       break;

    /*-------------------------------------------------------------------------
    Mode Preference changed Command
    -------------------------------------------------------------------------*/
     case HDRMC_MM_SD_LTE_MODE_CHANGE_IND:

      HDR_MSG_PROT_1 (MSG_LEGACY_HIGH, "Online rcv Mode pref changed indication with %d",
			                               cmd_ptr->msgr_cmd.msg.sd_mode_pref.mode_change);

       if(cmd_ptr->msgr_cmd.msg.sd_mode_pref.mode_change == SD_LTE_MODE_ADDED)
       {
          hdralmp_update_mode_pref();
       }
       else
       {
          HDR_MSG_PROT( MSG_LEGACY_HIGH, "Mode change is not LTE addition. Ignore" );
       }
       break;
#endif /* FEATURE_HDR_TO_LTE_REDIRECTION */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
#endif
  if(hdrmc_get_hdr_sub_id() == SYS_MODEM_AS_ID_1)
  {
   hdrovhd_update_lte_fplmn(hdrmc.lte_fplmn_list_sub0.num_fplmn,hdrmc.lte_fplmn_list_sub0.fplmnid,FALSE);
  }
  else
  {
   hdrovhd_update_lte_fplmn(hdrmc.lte_fplmn_list_sub1.num_fplmn,hdrmc.lte_fplmn_list_sub1.fplmnid,FALSE);
  }
  if(((hdrmc_get_hdr_sub_id() == SYS_MODEM_AS_ID_1 )&& (hdrmc.ehrpd_credentials_are_provisioned_sub0 != TRUE) )||
    ((hdrmc_get_hdr_sub_id() == SYS_MODEM_AS_ID_2 )&& (hdrmc.ehrpd_credentials_are_provisioned_sub1 != TRUE) ))

  {
      HDR_MSG_PROT_3( MSG_LEGACY_HIGH, 
             "eHRPD not provisined for the current active SUB %d on which HDR is activated, sub0 %d sub1 %d",hdrmc_get_hdr_sub_id(),hdrmc.ehrpd_credentials_are_provisioned_sub0,
                          hdrmc.ehrpd_credentials_are_provisioned_sub1);
  ...
  {
    HDR_MSG_PROT( MSG_LEGACY_HIGH, 
                  "eHRPD auth is not in USIM so mark ehrpd credentials as invalid, when entering ONLINE " );
    /* Set eHRPD credentials available flag */
    hdrscp_set_ehrpd_credentials_are_valid( FALSE );
  }
  else
  {
    switch(hdrmc_get_hdr_sub_id())
    {
      case SYS_MODEM_AS_ID_1: if(hdrmc.ehrpd_credentials_are_provisioned_sub0 == TRUE)
                              hdrscp_set_ehrpd_credentials_are_valid( TRUE );
                              HDR_MSG_PROT( MSG_LEGACY_HIGH,"eHRPD credentials are provisioned on entering online on SUB0");
                              break;
      case SYS_MODEM_AS_ID_2:if(hdrmc.ehrpd_credentials_are_provisioned_sub1 == TRUE)
                              hdrscp_set_ehrpd_credentials_are_valid( TRUE );
                              HDR_MSG_PROT( MSG_LEGACY_HIGH,"eHRPD credentials are provisioned on entering online on SUB1");
                              break;
      default               : HDR_MSG_PROT( MSG_LEGACY_HIGH," HDR on Invalid SUB. ");
                              break;
    }
  }
  ...
  HDR_MSG_PROT_1(MSG_LEGACY_HIGH, "primary slot id returned after mmgsdi_session_get_info_sync:%d", hdrmc.pri_slot_id);

  /* Query the slot id for hdr secondary session id */
  mcmmgsdi_query.query_type = MMGSDI_SESSION_INFO_QUERY_BY_ID;
  mcmmgsdi_query.query_key.session_id = hdrmc.sec_session_id;
  if (mmgsdi_session_get_info_sync( mcmmgsdi_query,  &mcmmgsdi_get_session_info ) == MMGSDI_SUCCESS)
  {
    HDR_MSG_PROT(MSG_LEGACY_HIGH, " mmgsdi_session_get_info_sync returned success");
    hdrmc.sec_slot_id = mcmmgsdi_get_session_info.slot_id;
  }
  else
  {
    HDR_MSG_PROT(MSG_LEGACY_HIGH, " mmgsdi_session_get_info_sync returned failure");
  ...
  HDR_MSG_PROT_1(MSG_LEGACY_HIGH, "secondary slot id returned after mmgsdi_session_get_info_sync:%d", hdrmc.sec_slot_id);
...
#ifdef GPP2_SUPPORT_ON_SEC_SUB 
  hdrlmac_update_subscriber_id(hdrmc.sub_id);
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
...
#ifdef FEATURE_HDR_MSGR_VARIANT
      hdrmc_msgr_msg_stop(SYS_MODEM_AS_ID_1);
      hdrmc_msgr_msg_stop(SYS_MODEM_AS_ID_2);
      hdrmc_msgr_client_stop();
#else
      hdrmc_msgr_stop();
#endif /* FEATURE_HDR_MSGR_VARIANT */
    }              /* end TASK_STOP_SIG */
...
hdrmc_state_type hdrmc_inactive_process_cmd( hdrmc_msg_type * cmd_ptr )
{
  ...
  prot_trans_type trans_id=0;
#endif /*  FEATURE_HDR_DEBUG_INFO */
 boolean is_lte_present = TRUE;
 boolean hdr_sub_chg = FALSE;
#ifdef FEATURE_HDR_MSGR_VARIANT
  sys_modem_as_id_e_type variant;
#endif /* FEATURE_HDR_MSGR_VARIANT */
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch ( cmd_ptr->hdr.cmd )
  {
    ...
    case HDRMC_PROT_ACT_REQ:
      ...
      HDR_MSG_PROT_3 (MSG_LEGACY_HIGH, "Inact rcv prot act reason=%d bootup_subid = %d status : %d",
                      cmd_ptr->msgr_cmd.msg.activate_protocol.act_reason,hdrmc.bootup_subid, hdrmc.status);
    ...
    case HDRMC_PROT_PH_STAT_CHGD_REQ:
      ...
      HDR_MSG_PROT_5 (MSG_LEGACY_HIGH,
                       "Inact rcv PhStatChg type=%d oprtMode=%d id=%d bootup_subid = %d subs=%x",
                      cmd_ptr->msgr_cmd.msg.ph_status_chgd.chg_type,
                      cmd_ptr->msgr_cmd.msg.ph_status_chgd.oprt_mode,
                      cmd_ptr->msgr_cmd.msg.ph_status_chgd.trans_id,hdrmc.bootup_subid,
                      cmd_ptr->msgr_cmd.msg.ph_status_chgd.subs_capability);
      ...
      {
#ifdef FEATURE_HDR_MSGR_VARIANT
        if((hdrmc.sub_id != variant) && (cmd_ptr->msgr_cmd.msg.ph_status_chgd.is_cdma_subs_avail == TRUE) )
        {
            hdr_sub_chg = TRUE;
            HDR_MSG_PROT_3( MSG_LEGACY_MED, 
                            "HDR SUB ID valid %d Changed from %d to %d",hdrmc.sub_id_is_valid, 
                            hdrmc.sub_id,variant);
           hdrmc.sub_id =  variant;
        }
        hdrmc.sub_id_is_valid = TRUE;
#else
        if((hdrmc.sub_id != INST_ID_TO_SYS_AS_ID(cmd_ptr->msgr_cmd.msg.ph_status_chgd.hdr.inst_id))
            && (cmd_ptr->msgr_cmd.msg.ph_status_chgd.is_cdma_subs_avail == TRUE) )
        {
            hdr_sub_chg = TRUE;
            HDR_MSG_PROT_3( MSG_LEGACY_MED, 
                            "HDR SUB ID valid %d Changed from %d to %d",hdrmc.sub_id_is_valid, 
                            hdrmc.sub_id,INST_ID_TO_SYS_AS_ID(cmd_ptr->msgr_cmd.msg.ph_status_chgd.hdr.inst_id));
            hdrmc.sub_id =  INST_ID_TO_SYS_AS_ID(cmd_ptr->msgr_cmd.msg.ph_status_chgd.hdr.inst_id);
        }      
        hdrmc.sub_id_is_valid = TRUE;
#endif /* FEATURE_HDR_MSGR_VARIANT */
      ...
      if(hdr_sub_chg == TRUE)
      {
        hdrmc.sub_id_change_in_progress = TRUE;
        hdrmc_reload_NV();
        hdrmc.sub_id_change_in_progress = FALSE;
        hdrlmac_update_subscriber_id(hdrmc.sub_id);
        HDR_MSG_PROT( MSG_LEGACY_MED, "Sub change, call hdrlmac_update_subscriber_id()");
      }
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
      is_lte_present = (cmd_ptr->msgr_cmd.msg.ph_status_chgd.subs_capability) & (1<< ((uint32)SYS_SYS_MODE_LTE));

      if( is_lte_present == FALSE)
      { /* lte not present */
        hdrlmac_set_force_lta(FALSE);
        HDR_MSG_PROT (MSG_LEGACY_HIGH,"Ignoring : Force LTA as LTE cap is not present on this SUB");
      }
      else
      {
        hdrlmac_set_force_lta(FALSE);
        HDR_MSG_PROT (MSG_LEGACY_HIGH,"Do Not Force LTA as LTE cap is present on this SUB");
      }
      ...
      HDR_MSG_PROT_3 (MSG_LEGACY_HIGH, "Inact send phStatChgdCnf id=%d variant %d hdrmc_subid %d",
                       cmd_ptr->msgr_cmd.msg.ph_status_chgd.trans_id,variant,hdrmc.sub_id);                     
...
#ifdef GPP2_SUPPORT_ON_SEC_SUB
         mmoc_rpt_ph_stat_chgd_cnf2( cmd_ptr->msgr_cmd.msg.ph_status_chgd.trans_id,
                                     mmoc_convert_sys_mode_asubsid_to_mmoc_mode(SYS_SYS_MODE_HDR, variant), 
#ifdef MMOC_API_PH_STAT_CNF_WITH_STATUS
                                 MMOC_PH_STAT_ERR_NV_ERR,
#endif
                                 variant);
    ...
    case HDRMC_LTE_ABORT_REVERSE_TIMING_TRANSFER_REQ:
    case HDRMC_LTE_ABORT_CGI_REQ:
      HDR_MSG_PROT_1(MSG_LEGACY_HIGH, "IRAT: Inactive rcv abort LTE to HDR handoff : HO type %d",
                                       hdrmc.lte_to_hdr_ho_info.irat_ho_type);

      if((hdrmc_lte_to_hdr_get_cgi_is_in_progress() == TRUE) || 
          (hdrmc_lte_to_hdr_ho_is_in_progress() == TRUE) ||
          (hdrmc_hdr_to_lte_time_transfer_is_in_progress() == TRUE))
      {

            HDR_MSG_PROT(MSG_LEGACY_HIGH, "IRAT: mmoc_rpt_prot_auto_deactd_ind will be called after HO_CNF");
            hdrmc.pending_auto_deactivate_send  = TRUE;          
      }
      else
      {
        HDR_MSG_PROT(MSG_LEGACY_HIGH, 
          "Ignore: Inactive rcv abort when NO LTE to HDR handoff is in progress. not calling mmoc_rpt_prot_auto_deactd_ind");
      }
    ...
    -------------------------------------------------------------------------*/
    case HDRMC_CM_LTE_FPLMN_UPDATE_REQ:
      HDR_MSG_PROT( MSG_LEGACY_HIGH, "Inact rcv LTE FPLMN UPDATE req from CM" );
#ifdef FEATURE_HDR_MSGR_VARIANT
      hdrmc_update_lte_fplmn( cmd_ptr->msgr_cmd.msg.cm_lte_fplmn.plmn_list_size,
                              cmd_ptr->msgr_cmd.msg.cm_lte_fplmn.plmn,
                              variant);
 ...
 /*-------------------------------------------------------------------------
	...
	break;
     ...
     case HDRMC_MM_SD_LTE_MODE_CHANGE_IND:
      ...
      HDR_MSG_PROT_1 (MSG_LEGACY_HIGH, "Ignored :: Inactive rcv Mode pref changed indication with %d",
                 cmd_ptr->msgr_cmd.msg.sd_mode_pref.mode_change);
...
/*-------------------------------------------------------------------------
    ...
    -------------------------------------------------------------------------*/
     case HDRMC_QMI_NAS_PRIORITIZE_LTE_IND:
      ...
      HDR_MSG_PROT(MSG_LEGACY_HIGH, "Ignore :: Inactive rcv Prioritize LTE meas");
...
)
{
  ...
  HDR_MSG_PROT_4( MSG_LEGACY_HIGH,"event ID: %d event_session_id %d"
                 " mmg session id SUB0 %d mmg session id SUB1 %d" , event->evt,
                  event->session_id,hdrmc.mmgsdi_session_id_sub0,
                  hdrmc.mmgsdi_session_id_sub1);
  switch( event->evt )
  {
    case MMGSDI_SUBSCRIPTION_READY_EVT:
 
      if (event->session_id == hdrmc.mmgsdi_session_id_sub0)
      {
        hdrmc.mmgsdi_session_ready_sub0 = TRUE;
      }
      else if(event->session_id == hdrmc.mmgsdi_session_id_sub1)
      {
        hdrmc.mmgsdi_session_ready_sub1 = TRUE;
      }      

#ifdef FEATURE_EHRPD
      /* Handle eHRPD credentials only for primary seesion, i.e. Slot ID 1.
       * Only Slot ID 1 can have ehrpd/CDMA sunscription. */
    if ( (event->data.subscription_ready.app_info.app_data.app_type == MMGSDI_APP_USIM) )
    {
      if(event->session_id == hdrmc.pri_session_id)
      {
              HDR_MSG_PROT( MSG_LEGACY_HIGH, 
                      "MMGSDI_SUBSCRIPTION_READY_EVT eHRPD on primary session id");
                /* Set eHRPD credentials available flag */
              hdrscp_set_ehrpd_credentials_are_valid( TRUE );
              hdrmc.ehrpd_credentials_are_provisioned_sub0 = TRUE;
      }
	  else if(event->session_id == hdrmc.sec_session_id)
	  {
              HDR_MSG_PROT( MSG_LEGACY_HIGH, 
                      "MMGSDI_SUBSCRIPTION_READY_EVT eHRPD on secondary session id");
				/* Set eHRPD credentials available flag */
              hdrscp_set_ehrpd_credentials_are_valid( TRUE );
              hdrmc.ehrpd_credentials_are_provisioned_sub1 = TRUE;
      }
      else
      {
              HDR_MSG_PROT( MSG_LEGACY_HIGH, 
                      "MMGSDI_SUBSCRIPTION_READY_EVT. Not matching primary/secondary session id");
      }
    }

      HDR_MSG_PROT_4( MSG_LEGACY_HIGH, 
                      "MMGSDI_SUBSCRIPTION_READY_EVT received, session id %d, app type %d updated sub0 %d sub1 %d", 
                      event->session_id,
                      event->data.subscription_ready.app_info.app_data.app_type,
                      hdrmc.ehrpd_credentials_are_provisioned_sub0,
                      hdrmc.ehrpd_credentials_are_provisioned_sub1 );
    ...
    case MMGSDI_SESSION_CHANGED_EVT:
      ...
      /* Handle eHRPD credentials only for primary seesion, i.e. Slot ID 1.
       * Only Slot ID 1 can have ehrpd/CDMA sunscription. */
    if ( (event->data.session_changed.activated == 0) &&
       (event->data.session_changed.app_info.app_data.app_type == MMGSDI_APP_USIM) )
      {
        if(event->session_id == hdrmc.pri_session_id)
        {
          hdrmc.ehrpd_credentials_are_provisioned_sub0 = FALSE;
              HDR_MSG_PROT( MSG_LEGACY_HIGH, 
                      "MMGSDI_SUBSCRIPTION_READY_EVT. primary session id ehrpd deactivated");
        }
        else if(event->session_id == hdrmc.sec_session_id)
        {
          hdrmc.ehrpd_credentials_are_provisioned_sub1 = FALSE;
              HDR_MSG_PROT( MSG_LEGACY_HIGH, 
                      "MMGSDI_SUBSCRIPTION_READY_EVT. secondary session id ehrpd deactivated");

        }
        if(hdrmc_check_ehrpd_credentials_valid() == FALSE)
        {
        /* Set eHRPD credentials available flag */
        hdrscp_set_ehrpd_credentials_are_valid( FALSE );
      }
      ...
      HDR_MSG_PROT_4( MSG_LEGACY_HIGH, 
                      "MMGSDI_SESSION_CHANGED_EVT received, activated %d, app type %d updated sub0 %d sub1 %d",
    ...
    case MMGSDI_CARD_ERROR_EVT:
            ...
            * Only Slot ID 1 can have ehrpd/CDMA subscription. */
            if ((hdrmc.state == HDRMC_ONLINE_STATE) && 
                            (event->data.card_error.info == MMGSDI_CARD_ERR_CARD_REMOVED))
          {
            if(event->data.card_error.slot == hdrmc.pri_slot_id)
            {
              hdrmc.ehrpd_credentials_are_provisioned_sub0 = FALSE;
              HDR_MSG_PROT( MSG_LEGACY_HIGH, 
                      "MMGSDI_SUBSCRIPTION_READY_EVT. primary slot id ehrpd deactivated");
            }
            else if(event->data.card_error.slot == hdrmc.sec_slot_id)
            {
              hdrmc.ehrpd_credentials_are_provisioned_sub1 = FALSE;
              HDR_MSG_PROT( MSG_LEGACY_HIGH, 
                      "MMGSDI_SUBSCRIPTION_READY_EVT. secondary slot id ehrpd deactivated");
            }
            ...
            if(hdrmc_check_ehrpd_credentials_valid() == FALSE)
            {
              /* Set eHRPD credentials available flag */
              HDR_MSG_PROT( MSG_LEGACY_HIGH, 
                      "hdrmc_check_ehrpd_credentials_valid returned FALSE");
                        hdrscp_set_ehrpd_credentials_are_valid( FALSE );
            }                                                  
           ...
           HDR_MSG_PROT_7( MSG_LEGACY_HIGH, 
                           "MMGSDI_CARD_ERROR_EVT received for slot ID %d info %d hdrmc.slot %d hdrmc.pri_sessid %d \
                            hdrmc.sec_sessid %d updated sub0 %d sub1 %d", 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if ((cnf == MMGSDI_SESSION_OPEN_CNF) && (status == MMGSDI_SUCCESS))
  {
    ...
    {
      hdrmc.mmgsdi_session_id_sub0 =
          cnf_ptr->session_open_cnf.response_header.session_id;
     HDR_MSG_PROT_1( MSG_LEGACY_HIGH, 
                     "SUB0 session registered cb with ID: %d",hdrmc.mmgsdi_session_id_sub0);
    }
    else if (cnf_ptr->response_header.client_data == HDRMC_1X_PROV_SEC_SESSION)
    {
      hdrmc.mmgsdi_session_id_sub1 =
          cnf_ptr->session_open_cnf.response_header.session_id;
     HDR_MSG_PROT_1( MSG_LEGACY_HIGH, 
                     "SUB1 session registered cb with ID: %d",hdrmc.mmgsdi_session_id_sub1);
    }
...
)
{
  if(hdrmc_get_hdr_sub_id() == SYS_MODEM_AS_ID_1)
  {
    if (hdrmc.mmgsdi_session_ready_sub0)
  {
      *sid = hdrmc.mmgsdi_session_id_sub0;
    }
    HDR_MSG_PROT_2(MSG_LEGACY_HIGH,"returning SUB0 session id: %d ready %d",
                 hdrmc.mmgsdi_session_id_sub0,
                 hdrmc.mmgsdi_session_ready_sub0);
    return hdrmc.mmgsdi_session_ready_sub0;
  }
  else
  {
    if (hdrmc.mmgsdi_session_ready_sub1)
    {
      *sid = hdrmc.mmgsdi_session_id_sub1;
    }
    HDR_MSG_PROT_2(MSG_LEGACY_HIGH,"returning SUB1 session id: %d ready %d",
                 hdrmc.mmgsdi_session_id_sub1,
                 hdrmc.mmgsdi_session_ready_sub1);
    return hdrmc.mmgsdi_session_ready_sub1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( hdrutil_fs_get( HDRMC_REVC_FEATURES, 
                       &revc_features,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( hdrutil_fs_get( HDRCP_JCDMA_MODE, &jcdma_mode, 1 )
       == MCFG_FS_STATUS_OK )
  {
    HDR_MSG_PROT( MSG_LEGACY_MED, 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/ 
  ...
  config_fd = hdrutil_fs_open( HDRCP_EFS_NV_CONF_FILE_PATH, O_RDWR|O_APPEND );
  if ( config_fd >= 0 )
  {
  ...
  else
  {
    ...
    if ( ENOENT == hdrutil_fs_errno() )
    {
      HDR_MSG_PROT( MSG_LEGACY_HIGH, "Creating EFS NV item config file" );

      config_fd = hdrutil_fs_open( HDRCP_EFS_NV_CONF_FILE_PATH, 
    ...
    else
    {
      ...
      HDR_MSG_PROT_1( MSG_LEGACY_ERROR,
                      "Error opening config file errno %d", hdrutil_fs_errno() );
      return;
    }
    ...
    HDRMC_WRITE_EFS_NV_CONF_FILE( config_fd, HDRINIT_SCAN_MODE_SETTING_FILENAME );
    HDRMC_WRITE_EFS_NV_CONF_FILE( config_fd, HDRINIT_CUSTOM_BAND_NUM_FILENAME );
    HDRMC_WRITE_EFS_NV_CONF_FILE( config_fd, HDRINIT_CUSTOM_CHANNEL_NUM_FILENAME );
...
void hdrmc_task( uint32 dummy )
{
  ...
  hdrmc_tcb_ptr = rex_self();
   /* Read EFS item, for subid */
  if ( hdrutil_fs_get( HDRMC_SUBID_BOOTUP,
                       &hdrmc.sub_id,
                       sizeof(hdrmc.sub_id) ) == 
       MCFG_FS_STATUS_OK )
  {
    HDR_MSG_PROT_1( MSG_LEGACY_HIGH,"HDR SUBID at bootup %d",hdrmc.sub_id);
    hdrmc.bootup_subid = hdrmc.sub_id;
  }
  else
  {
    HDR_MSG_PROT_1( MSG_LEGACY_HIGH,"HDR SUBID reading failed, default %d",hdrmc.sub_id);
    hdrmc.bootup_subid = SYS_MODEM_AS_ID_MAX+1;
  }
  hdrlmac_update_subscriber_id(hdrmc.sub_id);
  hdrmc.status = 7;
  /* Put message router before RCINIT handshake to avoid race condition */
#ifdef FEATURE_HDR_MSGR_VARIANT
  hdrmc_msgr_client_init();
  
  hdrmc_msgr_msg_init(SYS_MODEM_AS_ID_1);

  hdrmc_msgr_msg_init(SYS_MODEM_AS_ID_2);
#else
  hdrmc_msgr_init();
#endif /* FEATURE_HDR_MSGR_VARIANT */
  ...
  */
  if ( rcevt_signal_name(HDRMC_RCEVT_READY) == RCEVT_NULL )
  {
...
#ifdef FEATURE_MMGSDI
   hdrmc.mmgsdi_session_ready_sub0 = FALSE;
   hdrmc.mmgsdi_session_ready_sub1 = FALSE;
...
{
  rex_timer_cnt_type remaining_time = 0;
  if (hdrmc_hdr_to_lte_time_transfer_is_in_progress())
  {
    remaining_time = rex_get_timer(&hdrmc.lte_to_hdr_rtt_timer);
    HDR_MSG_PROT_1( MSG_LEGACY_HIGH, "Remaining time for RTT is %d" , remaining_time);
    *time_allowed_ms = remaining_time;
  }
  else
  {
...
boolean hdrmc_get_transaction_id(prot_trans_type *transaction_id, hdrmc_msg_type *cmd_ptr)
{
  ...
  *transaction_id = 0;
  ...
  case HDRMC_UE_MODE_CHANGE_REQ:
     *transaction_id = cmd_ptr->msgr_cmd.msg.ue_mode_chg_req.trans_id;
     break;
...
/*===========================================================================
FUNCTION HDRMC_GET_HDR_SUB_ID

DESCRIPTION
  This function gets the current sub-id on which HDR is activated
...
RETURN VALUE
  sub-id on which HDR is activated

SIDE EFFECTS
  None
===========================================================================*/
sys_modem_as_id_e_type hdrmc_get_hdr_sub_id(void)
{
#ifdef GPP2_SUPPORT_ON_SEC_SUB 
  if(hdrmc.sub_id_is_valid == TRUE)
  {
     return hdrmc.sub_id;
  }
  else
  {
    HDR_MSG_PROT( MSG_LEGACY_ERROR, "HDR Sub ID is not Valid. Returning default");
    return SYS_MODEM_AS_ID_1;
  }
#else
    HDR_MSG_PROT( MSG_LEGACY_ERROR, "HDR Sub ID is not applicable. Returning default");
    return SYS_MODEM_AS_ID_1;
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
} /* hdrmc_get_hdr_sub_id() */

/*===========================================================================
FUNCTION HDRMC_SUB_ID_CHANGE_IN_PROGRESS

DESCRIPTION
  This function checks if sub id change is in progress
...
SIDE EFFECTS
  None
===========================================================================*/
boolean hdrmc_sub_id_change_in_progress()
{
  HDR_MSG_PROT_1( MSG_LEGACY_HIGH, "SUB_ID change in progress: %d", 
                  hdrmc.sub_id_change_in_progress );
  
  return hdrmc.sub_id_change_in_progress;
} /* hdrmc_sub_id_change_in_progress() */

/*===========================================================================
FUNCTION HDRMC_CHECK_EHRPD_CREDENTIALS_VALID

DESCRIPTION
  This function checks if ehrpd is valid on any sub or not
...
SIDE EFFECTS
  None
===========================================================================*/
boolean hdrmc_check_ehrpd_credentials_valid()
{
  return(hdrmc.ehrpd_credentials_are_provisioned_sub0 || hdrmc.ehrpd_credentials_are_provisioned_sub1);
}
/*===========================================================================
FUNCTION HDRMC_UPDATE_LTE_FPLMN

DESCRIPTION
  This function updates the lte fplmn list

DEPENDENCIES 
  None 
 
PARAMETERS 
  num_fplmn - number of fplmn ID's
  fplmn     - fplmn list pointer
  sub_id    - Instantaneous ID on which the update was received.
...
void hdrmc_update_lte_fplmn ( uint32 num_fplmn,sys_plmn_id_s_type *fplmn, sys_modem_as_id_e_type sub_id)
{
   ...
   {
      HDR_MSG_PROT_1( MSG_LEGACY_HIGH, " Rcvd LTE LTE FPLMN LIST UPDATE for SUB0 with size:%d",num_fplmn );
     hdrmc.lte_fplmn_list_sub0.num_fplmn = num_fplmn;
     memset( hdrmc.lte_fplmn_list_sub0.fplmnid, 0, sizeof(hdrmc.lte_fplmn_list_sub0.fplmnid) );
     memscpy( hdrmc.lte_fplmn_list_sub0.fplmnid, sizeof(hdrmc.lte_fplmn_list_sub0.fplmnid), 
              fplmn, num_fplmn*sizeof(sys_plmn_id_s_type) );

   }
   else if(sub_id == SYS_MODEM_AS_ID_2)
   {
     HDR_MSG_PROT_1( MSG_LEGACY_HIGH, " Rcvd LTE LTE FPLMN LIST UPDATE for SUB1 with size:%d",num_fplmn );
     hdrmc.lte_fplmn_list_sub1.num_fplmn = num_fplmn;
     memset( hdrmc.lte_fplmn_list_sub1.fplmnid, 0, sizeof(hdrmc.lte_fplmn_list_sub1.fplmnid) );
     memscpy( hdrmc.lte_fplmn_list_sub1.fplmnid, sizeof(hdrmc.lte_fplmn_list_sub1.fplmnid), 
              fplmn, num_fplmn*sizeof(sys_plmn_id_s_type) );
   }
   else
   {
     HDR_MSG_PROT( MSG_LEGACY_HIGH, " Rcvd LTE LTE FPLMN LIST UPDATE for Invalid SUBID" );
     /*Ignore the update*/
   }
...
/*===========================================================================
FUNCTION HDRMC_SET_HDR_SUB_ID_FROM_DB

DESCRIPTION
  This function sets the hdr sub-id 
...
SIDE EFFECTS
  None
===========================================================================*/
void hdrmc_set_hdr_sub_id_from_db(sys_modem_as_id_e_type subid, uint8 status)
{
  HDR_MSG_PROT_2( MSG_LEGACY_HIGH, "HDR Sub ID from db %d current %d",subid,hdrmc.sub_id);
  hdrmc.sub_id_is_valid = TRUE;
  hdrmc.status = status;

} /* hdrmc_set_hdr_sub_id_from_db () */


/*===========================================================================
FUNCTION HDRMC_CREATE_EFS_ITEM_SUBID_FILE

DESCRIPTION
  APi to help store the subid info in to EFS item
...
NOTE
  Please list all the EFS/NV item nos. & description in the commnents below
===========================================================================*/
void hdrmc_store_subid_in_efs( void )
{
  int32              config_fd, result;

  /* Open efs file. If it doesn't exist create it and reopen it. */
  config_fd = hdrutil_fs_open_default( HDRMC_SUBID_BOOTUP, O_RDWR);
  
  if(config_fd < 0)
  {
    if ( ENOENT == hdrutil_fs_errno() ) /* Conf file does not exist */
    {
      HDR_MSG_SRCH( MSG_LEGACY_HIGH, "Creating EFS NV item subid file");

      config_fd = hdrutil_fs_open_default( HDRMC_SUBID_BOOTUP, 
                                          O_WRONLY|O_CREAT|O_TRUNC );

      if ( 0 > config_fd )
      {
        HDR_MSG_SRCH_1( MSG_LEGACY_ERROR, 
                        "Error creating EFS NV item subid file errno %d",
                        hdrutil_fs_errno() );
        return;
      }
    ...
    {
      HDR_MSG_SRCH_1( MSG_LEGACY_ERROR, 
                      "Error opening subid file errno %d while creating", 
                      hdrutil_fs_errno() );
      return;
    }
  }
 
    result = hdrutil_fs_write( config_fd,&hdrmc.sub_id,
                               sizeof( hdrmc.sub_id ) );
    if(result != sizeof( hdrmc.sub_id ) )
    {
      HDR_MSG_SRCH_1( MSG_LEGACY_ERROR, 
                      "Error writing to subid efs file errno %d", 
                      hdrutil_fs_errno() );
      result = hdrutil_fs_close( config_fd );
      return;
    }
    ...
    {
      HDR_MSG_SRCH_1( MSG_LEGACY_ERROR,
                      "Error closing newly created EFS NV item subid file errno %d",
                      hdrutil_fs_errno() );
      return;
    }
    return;
...
FUNCTION HDRMC_STOP_LTE_TO_HDR_RTT_TIMER

DESCRIPTION
  This function stops the lte to hdr rtt timer if it is active
...
===========================================================================*/
void hdrmc_stop_lte_to_hdr_rtt_timer(void)
{
  if( rex_get_timer( &hdrmc.lte_to_hdr_rtt_timer) !=0 )
  {
    /* stop the timer if it is active */
    HDR_MSG_PROT (MSG_LEGACY_HIGH, "Stop lte to hdr RTT timer");
    ( void ) rex_clr_timer( &hdrmc.lte_to_hdr_rtt_timer);
  }
}/* hdrmc_stop_lte_to_hdr_rtt_timer() */

/*===========================================================================

FUNCTION HDRMC_LTE_TO_HDR_RTT_TIMER_EXPIRY_CB

DESCRIPTION
  This function is called on expiry of lte to hdr RTT timer
...
void hdrmc_lte_to_hdr_rtt_timer_expiry_cb
(
  uint32 timer_id
)
{

  HDR_MSG_PROT (MSG_LEGACY_HIGH, "LTE to HDR RTT timer expired");
  ( void ) rex_clr_timer( &hdrmc.lte_to_hdr_rtt_timer);
  /* Auto deactivate ind to be sent to Mmode */
  hdrmc.pending_auto_deactivate_send  = TRUE;
  /* Send timing info to LTE */
  hdrmc_send_timing_info_to_lte();

} /* hdrmc_lte_to_hdr_rtt_timer_expiry_cb() */
