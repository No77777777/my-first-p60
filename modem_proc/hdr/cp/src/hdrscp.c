...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdrscp.c#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $
...
06/01/15   kss     Fixed possible integer overflow issue.  
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (pack_default_subtype && (index < HDRSCP_MAX_SUBTYPES))
  {
  ...
  if (pack_hardlink_subtype && (index < HDRSCP_MAX_SUBTYPES))
  {
...
/* Check to ensure subtype array did not buffer overflow */
  HDR_ASSERT (index <= HDRSCP_MAX_SUBTYPES);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (hdramp_previous_session_opened(reason == HDRSCP_PS_RESTORE_SILENT))
  {
    ...
    (void) sd_ss_ind_hdr_opr_ses_opened(
...
)
{
  ...
  uint8  attrib_len_bytes = 0; /* Byte length of attribute */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  while ((total_bits_remaining - attrib_len) >= HDRSCP_MIN_ATTRIB_LEN)
  {
    ...
    if (HDRSCP_UNPACK_CON_REQ_LEN(item_ptr, &attrib_len_bytes, attrib_offset) !=
        E_SUCCESS)
    {
     ...
     */
    attrib_len = attrib_len_bytes * HDRSCP_BITS_PER_BYTE;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
          ...
          (HDRSCP_MIN_ATTRIB_LEN + HDRSCP_VALUE_ID_SIZE))
  {
    ...
    if (HDRSCP_UNPACK_CON_RSP_LEN(item_ptr, &attrib_len_bytes,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ehrpd_auth_in_usim == TRUE)
  {
    ...
    if((hdrmc_mcfg_refresh_in_progress() != TRUE) && (hdrmc_sub_id_change_in_progress() != TRUE))
    {
    ...
    {
       HDR_MSG_PROT_1(MSG_LEGACY_HIGH,
         "MCFG refresh (or) SUB_ID is in progress. Do not updated ehrpd credentials. current value %d",
  ...
  if ( hdrutil_fs_get( HDRSCP_EHRPD_NETWORK_SETTING_FILENAME,
...
#endif /* FEATURE_EHRPD */
  current_config->subid = hdrmc_get_hdr_sub_id();
