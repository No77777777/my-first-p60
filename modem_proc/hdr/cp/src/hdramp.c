...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdramp.c#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/29/18   svu     Added macro for AS_ID to slotID conversion.
08/30/17   vko     update nv_rtre_control() to pass sub_id
09/21/16   svu     HDR changes for FR-CT SMS AUTO REGISTRATION
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (nv_rtre_control_ext(hdrmc_get_hdr_sub_id()) == NV_RTRE_CONTROL_USE_RUIM)
  {
  ...
  if ( session_open )
  {
    ...
    /* detect card swap sceanrio: 
       ...
       AT 1 w/ Card A */
              ...
              (nv_rtre_control_ext(hdrmc_get_hdr_sub_id()) == NV_RTRE_CONTROL_USE_RUIM) )
    {

      first_update_after_powerup = FALSE;

      get_esn_me_change_flag = mmgsdi_get_esn_me_change_flag(HDRMC_AS_ID_TO_SLOT_ID(hdrmc_get_hdr_sub_id()));
