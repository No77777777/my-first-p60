...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdrinit.c#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
09/11/18   vaa     Starting a timer on receiving RTT req from LTE 
10/30/17   vko     FR 29153: Support HDR/CDMA on the 2nd sub 
10/14/16   vko     Reduce hystersis timer value to 60sec for L2D reselection scenario
11/19/15   vko     Reduce the HDR hysteresis timer for L2D redirection
...
#define HDRINIT_LTE_TO_HDR_HO_RESEL_HYSTR_MSECS  60000
#endif /* FEATURE_LTE_TO_HDR */
...
LOCAL void hdrinit_process_pilot_acquire_failed ( void )
{
  ...
  #endif
  {
	  (void ) sd_ss_ind_hdr_acq_failed_pilot(
#ifdef GPP2_SUPPORT_ON_SEC_SUB         
                       hdrmc_get_hdr_sub_id(),
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
                       &rtrn_ss_act);
...
)
{
  ...
  (void) sd_ss_ind_hdr_misc_meas_rprt(
#ifdef GPP2_SUPPORT_ON_SEC_SUB         
                     hdrmc_get_hdr_sub_id(),
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
                     0, TRUE, &rtrn_ss_act);  //lint !e530 !e534
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  (void)sd_ss_ind_hdr_acq_failed_sync(
#ifdef GPP2_SUPPORT_ON_SEC_SUB         
                     hdrmc_get_hdr_sub_id(),
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
                     &rtrn_ss_act); //lint !e530 !e534
...
LOCAL void hdrinit_process_lte_to_hdr_ho_hystr_timeout ( void )
{
  ...
  {
     HDR_MSG_PROT (MSG_LEGACY_HIGH, 
            "LTE to HDR Resel HO hystr timer timeout during L2D");   
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* rev number is not in the valid range */
    ...
    hdrcmac_deactivate(HDRHAI_INITIALIZATION_STATE_PROTOCOL);
    hdrinits_change_state(HDRINITS_NET_DET_STATE);
    (void) sd_ss_ind_hdr_acq_prot_mis(
#ifdef GPP2_SUPPORT_ON_SEC_SUB         
                       hdrmc_get_hdr_sub_id(),
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
                       &rtrn_ss_act); //lint !e530 !e534
...
/*----------------------------------------------------------------------- */
  ...
  if ( hdrutil_fs_get( HDRINIT_SCAN_MODE_SETTING_FILENAME,
	                     &hdrinit.scan_mode, 
					             1 ) == MCFG_FS_STATUS_OK )
  {
  ...
  if(hdrinit.scan_mode == TRUE)
  {
	 if ( ( hdrutil_fs_get( HDRINIT_CUSTOM_BAND_NUM_FILENAME, &hdrinit.custom_channel_info.band,1) == MCFG_FS_STATUS_OK ) &&
		    ( hdrutil_fs_get( HDRINIT_CUSTOM_CHANNEL_NUM_FILENAME, &hdrinit.custom_channel_info.chan_num,2) == MCFG_FS_STATUS_OK ) )
	 {
...
void hdrinit_start_lte_to_hdr_ho_hystr_timer ( void )
{
  ...
  if ( hdrmc_hdr_to_lte_time_transfer_is_in_progress() )
  {
     ...
     * RTT hystersis timer. */ 
      ...
      HDR_MSG_PROT( MSG_LEGACY_HIGH, "Start LTE to HDR hystr timer for RTT" );
      ( void ) rex_set_timer( &hdrinit.lte_to_hdr_ho_hystr_timer,
                               time_allowed_ms );

  }
  else if ( hdrmc_lte_to_hdr_resel_is_in_progress() )
  {
    /* For Reselection, we use 60s timer as HDR cannot get the chain, due to 1x call.
           If the 1x call is longer, it will cause HDR SRCH stalled after 80 sec*/
    HDR_MSG_PROT( MSG_LEGACY_HIGH, "Start LTE to HDR hystr timer 60s for L2D Resel" );
    ( void ) rex_set_timer( &hdrinit.lte_to_hdr_ho_hystr_timer,
                            HDRINIT_LTE_TO_HDR_HO_RESEL_HYSTR_MSECS );
  }  
  else
  {
