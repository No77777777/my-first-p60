...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdrsmp.c#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
10/29/17   vko     FR 29153: Support HDR/CDMA on the 2nd sub
12/14/15   vko     Fixed KW issues 
09/16/15   vko     Report session invalid, if session close already started
...
)
{
  ...
  if(hdrsmp.session_closing == TRUE) /* This check is needed as session can start closing, 
                                                                         when ehrpd status is being determined */
  {
    HDR_MSG_PROT(MSG_LEGACY_MED, "Session already started closing. returing invalid");
    ret_val = FALSE;
  }
  return ret_val;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while ((total_bits_remaining - attrib_len) >= HDRSMP_MIN_ATTRIB_LEN)
  {
    ...
    if (HDRSMP_UNPACK_CON_REQ_LEN(item_ptr, &attrib_len_bytes,
     ...
     */
    attrib_len = attrib_len_bytes * HDRSMP_BITS_PER_BYTE;
