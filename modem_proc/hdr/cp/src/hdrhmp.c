...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdrhmp.c#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $
...
void hdrhmp_powerup_init( void )
{
  ...
  if ( hdrutil_fs_get( HDRHMP_BACKOFF_TIME_TABLE_SESSION_FAILED,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
   ...
   */
  sd_action = sd_ss_ind_hdr_opr_sys_lost( 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (hdrhmp.sclose.count >= HDRHMP_SCLOSE_MAX)
  {
    ...
    if ( hdralmps_get_state() != HDRALMPS_INACTIVE )
    {
      if (close_reason == HDRHAI_CLOSE_CONN_OPEN_FAIL)
      {
        sd_action = sd_ss_ind_hdr_opr_ses_nego_timeout( 
#ifdef GPP2_SUPPORT_ON_SEC_SUB         
                 hdrmc_get_hdr_sub_id(),
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
                 NULL );
      ...
      {
        /* Assume we can not establish a session on this sector */
        sd_action = sd_ss_ind_hdr_opr_ses_abort(
#ifdef GPP2_SUPPORT_ON_SEC_SUB         
                 hdrmc_get_hdr_sub_id(),
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
                 NULL );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( (msg_id == 0x52) &&  hdrscp_config_is_locked() )
  {
