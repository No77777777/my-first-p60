...
INITIALIZATION AND SEQUENCING REQUIREMENTS
  ...
  Copyright (c) 2005 - 2018 by Qualcomm Technologies, Inc.  All Rights Reserved.
*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*/
...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdrscmdb.c#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
08/01/18   svu     Fix bootup session restoration
01/18/18   svu     Fixed restoring session to the corresponding subid
05/16/16   vko     Reduce excessive F3
05/04/16   vko     Reduce excessive F3 logging
02/04/16   vko     Reduce excessive F3 logging
06/23/15   vko     Add wrapper to compress2 API, to pass alloc/free
                           functions to be used from modem memheap
...
/*---------------------------------------------------------------------------*/
  ...
  if( hdrutil_fs_stat("/hdr/hdr_protocol.z", &sbuf) != MCFG_FS_STATUS_OK ) 
  {
  ...
  fd = hdrutil_fs_open( "/hdr/hdr_protocol.z", O_RDONLY );
  ...
  {
    bytes_read = hdrutil_fs_read(fd, buffer + offset, bytes_remaining);
    if (bytes_read <= 0) 
    {
      err_no = E_FAILURE;
      break;
  ...
  if(err_no == E_FAILURE)
  {
     ...
     (void) hdrutil_fs_delete("/hdr/hdr_protocol.z");
  }
...
/*---------------------------------------------------------------------------*/
  ...
  fd = hdrutil_fs_open( "/hdr/hdr_protocol.cfq", O_RDONLY );
  ...
  else /* Config file opened successfully for read */
  {
    ...
    Read build info (compile date and time) 
    ------------------------------------------------------------------------*/
    if (hdrutil_fs_read(fd, &hdrscmdb.session_db_status,
                        sizeof(hdrscmdb_session_db_status_enum_type)) == -1)
    {
      ERR( "Cannot read session status", 0, 0, 0);
      err_no = E_FAILURE;
    }
    else if (hdrutil_fs_read(fd, &hdrscmdb.build_info, 
    ...
    Read Address Management Protocol's config data
    ------------------------------------------------------------------------*/
    else if (hdrutil_fs_read(fd, &hdrscmdb.amp_info, 
    ...
    Read Session Configuration Protocol's hardlink config params
    ------------------------------------------------------------------------*/
    else if (hdrutil_fs_read(fd, &hdrscmdb.scp_info, 
    ...
    Read Location Update Protocol's config data
    ------------------------------------------------------------------------*/
    else if (hdrutil_fs_read(fd, &hdrscmdb.lup_info, 
    ...
    {
       HDR_MSG_PROT(MSG_LEGACY_MED, " EFS read failure and deleting the existing EFS file stored ");
       (void) hdrutil_fs_delete("/hdr/hdr_protocol.cfq");
    }
...
/*---------------------------------------------------------------------------*/
  if (hdrscmdb.session_db_status == HDRSCMDB_SESSION_CURRENT)
  {
    ...
    HDR_MSG_PROT_2(MSG_LEGACY_HIGH,
        "Writing current session to EFS subid %d session stats %d",hdrscmdb.scp_info.subid,hdrscmdb.session_db_status);
  }
  else if (hdrscmdb.session_db_status != HDRSCMDB_SESSION_INVALID)
  {
    ...
    HDR_MSG_PROT_2(MSG_LEGACY_HIGH,
     "Writing previous session to EFS subid %d session stats %d",hdrscmdb.prev_scp_info.subid,hdrscmdb.session_db_status);
  }
  ...
  /* do while(0) loop is used to use to break easily in case of an error 
    ...
    HDR_MSG_PROT_1(MSG_LEGACY_HIGH,
         "Debug: EFS Write for sub id %d", 
         hdrscmdb.scp_info.subid );
  do  
  {
    /*------------------------------------------------------------------------
    ...
    if (err_no != E_FAILURE)
    {
...
#ifdef FEATURE_HDR_EFS_COMPRESS	
      if(  (compress_status = hdrutil_compress((uint8 *)hdr_cfg_data_compressed, 
                                 &compress_size, 
	 ...
	 break;  /* break out of the do while(0) loop*/
...
#endif /*#ifdef *FEATURE_HDR_EFS_COMPRESS*/
      fd = hdrutil_fs_open( temp_file, O_WRONLY | O_CREAT );
      ...
      while (bytes_remaining > 0) 
      {
        bytes_written = hdrutil_fs_write (fd, buffer + offset, bytes_remaining);
        if (bytes_written <= 0) 
        {
...

  {
    ...
    if ( ( hdrutil_fs_rename(temp_file, final_file) == -1) )
      {
     ...
     if(err_no != E_FAILURE)
     {
       HDR_MSG_PROT( MSG_LEGACY_ERROR, "config file write passed" );
     }
...
#endif /* FEATURE_EHRPD */
  scp_info->subid = hdrmc_get_hdr_sub_id();
  HDR_MSG_PROT_1(MSG_LEGACY_HIGH,"Writing SCP info to db for subid %d",scp_info->subid);
...
boolean hdrscmdb_powerup_init( void )
{
  ...
  if ( hdrmc_is_first_init() )
  {
         ...
         (hdrscmdb_read() == E_SUCCESS))
    {
      ...
      hdrmc_set_hdr_sub_id_from_db(hdrscmdb.scp_info.subid,hdrscmdb.session_db_status );
    }
    else
    {
      HDR_MSG_PROT (MSG_LEGACY_HIGH, 
      ...
      if ( hdrutil_fs_mkdir("/hdr", O_CREAT) == -1 )
      {
