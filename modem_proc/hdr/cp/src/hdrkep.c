...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdrkep.c#1 $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
06/03/19   svu     MSG_STR_ERROR call is removed as it is being deprecated
...
)
{
  ...
  while ((total_bits_remaining - attrib_len) >= HDRKEP_MIN_ATTRIB_LEN)
  {
    ...
    if( HDRKEP_UNPACK_CON_REQ_LEN( item_ptr, &attrib_len_bytes, attrib_offset ) !=
        E_SUCCESS )
    {
     ...
     */
    attrib_len = attrib_len_bytes * HDRKEP_BITS_PER_BYTE;
