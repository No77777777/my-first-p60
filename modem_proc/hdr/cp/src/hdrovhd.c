...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdrovhd.c#2 $ $DateTime: 2020/04/27 09:08:35 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
04/23/20   par     Fixed Klocwork issue
10/29/17   vko     FR 29153: Support HDR/CDMA on the 2nd sub 
08/03/17   vko     MSGR Instance ID to Variant ID change
12/12/16   svu     Fixed Klocwork issue
08/23/16   vko     Remove de-prioritized LTE EARFCNs during D2L redirection
04/15/16   vko     Fixed redefining avoided earfcn timer
02/04/16   vko     Reduce excessive F3 logging
01/22/16   vko     Fixed ORNL EARFCN not added when it is not present in ACQ DB
07/24/15   vko     Skip earfcn for D2L reselection, if reselection failed with forbidden TA
                   reason
06/30/15   vko     Reset cache entry state to valid, if hashed channel is different from 
                           unpack SP msg and update cache entry.
07/08/15   vko     Fixed LTE nodes not added when D2L reselection enabled
...
05/04/15   sjj     FR 26262 : LTE-U band support
...
typedef struct
{
  ...
  /* Info containing mapping between plmnid and earfcn */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( hdrutil_fs_get( HDROVHD_PREF_DEFAULT_CACHE_PERIOD_FILENAME,
  ...
  ---------------------------------------------------------------------------*/
  if ( hdrutil_fs_get( HDROVHD_HDR_TO_LTE_RESEL_ENABLED_FILENAME,
  ...
  if ( hdrutil_fs_get( HDROVHD_MCC_NO_PRIOR_SESSION_FILENAME,
                       &hdrovhd.no_ps_mcc, 
...
#ifdef FEATURE_LTE_DEPRIORITIZATION
  memset( &hdrovhd.depri_freq, 0, 
        sizeof(hdrovhd.depri_freq) );
#endif /* FEATURE_LTE_DEPRIORITIZATION */
...
LOCAL void hdrovhd_process_an_redirected(void)
{
  ...
  switch( sd_ss_ind_hdr_opr_redir( 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /* Retrive the earfcn list from LTE ACQ DB */
  lte_rrc_acq_db_earfcn = hdrovhd.lte_rrc_acq_db_earfcn;
  if(lte_rrc_acq_db_earfcn.num_earfcn_count == 0)
  {
	HDR_MSG_PROT(MSG_LEGACY_HIGH, "HDR DLR:LTE ACQ DB returned 0 EARFCN.");	
  }
  HDR_MSG_PROT_1(MSG_LEGACY_HIGH, "HDR DLR:LTE ACQ DB returned %d EARFCN.",lte_rrc_acq_db_earfcn.num_earfcn_count);	
  ...
  for ( i = 0; i < lte_nset_update_params->num_eutra_freqs; i++)
  {
    ...
    else
    {
              ...
              lte_nset_update_params->earfcn_params_arr[i].earfcn, pruned_plmn ) )
      {
        ...
        {
          HDR_MSG_PROT_1( MSG_LEGACY_HIGH,
		  	              "PLMNID included. 0 PLMN ID is allowed for LTE freq = %d from ORNL message",
		...
		{
          HDR_MSG_PROT_1( MSG_LEGACY_HIGH,
		  	              "Searching earfcn value %d", lte_nset_update_params->earfcn_params_arr[i].earfcn );
						  
	      for(l = 0;l < lte_rrc_acq_db_earfcn.num_earfcn_count; l++)
          {
              HDR_MSG_PROT_2 (MSG_LEGACY_MED, "HDR DLR: LTE ACQ DB Earfcn value %d ORNL earfcn %d",
			                                   lte_rrc_acq_db_earfcn.earfcn_list[l].earfcn,
			                                   lte_nset_update_params->earfcn_params_arr[i].earfcn);	
			  
              if(lte_rrc_acq_db_earfcn.earfcn_list[l].earfcn == lte_nset_update_params->earfcn_params_arr[i].earfcn)
              {
                for(j=0;j<lte_rrc_acq_db_earfcn.earfcn_list[l].plmn_list.num_plmns;j++)
                {
                   is_in_fplmn = FALSE;

				   HDR_MSG_PROT_3( MSG_LEGACY_MED, 
					               "HDR DLR: PLMN ID (num_mnc_digits: %d, mcc: %d, mnc: %d) ",
					                lte_rrc_acq_db_earfcn.earfcn_list[l].plmn_list.plmn[j].num_mnc_digits,  
					                lte_rrc_acq_db_earfcn.earfcn_list[l].plmn_list.plmn[j].mcc,
					                lte_rrc_acq_db_earfcn.earfcn_list[l].plmn_list.plmn[j].mnc  );

				   /* Check whether PLMN is in the FPLMN list */
                   HDR_MSG_PROT_1(MSG_LEGACY_HIGH, "HDR DLR:FPLMN list has %d ",hdrovhd.fplmn_list.num_fplmn);
	  			   for ( k = 0; k < hdrovhd.fplmn_list.num_fplmn; k++ )
                   {
					  if ( sys_plmn_match( hdrovhd_lte_rrc_plmn_to_sys_plmn_id(lte_rrc_acq_db_earfcn.earfcn_list[l].plmn_list.plmn[j]), 
						                   hdrovhd.fplmn_list.fplmnid[k] ))
                      {
                          is_in_fplmn = TRUE;
						  HDR_MSG_PROT_3( MSG_LEGACY_MED, 
                                          "HDR DLR: PLMN ID is in FPLMN list (num_mnc_digits: %d, mcc: %d, mnc: %d)",
                                           lte_rrc_acq_db_earfcn.earfcn_list[l].plmn_list.plmn[j].num_mnc_digits,  
                                           lte_rrc_acq_db_earfcn.earfcn_list[l].plmn_list.plmn[j].mcc,
                                           lte_rrc_acq_db_earfcn.earfcn_list[l].plmn_list.plmn[j].mnc  );

                          sys_plmn_get_mcc_mnc2(hdrovhd.fplmn_list.fplmnid[k],
						                        &plmn_is_undefined, &mnc_includes_pcs_digit,
						                        &fplmn_mcc, &fplmn_mnc );

                          HDR_MSG_PROT_4( MSG_LEGACY_MED, 
                                          "FPLMN (defined: %d, pcs: %d, mcc: %d, mnc: %d) is in the FPLMN list",
                   ...
                   if(is_in_fplmn)
                   {
                      HDR_MSG_PROT_1 (MSG_LEGACY_MED, "HDR DLR: PLMNID is forbidden for earfcn %d. Check for next plmnid",
				                      lte_nset_update_params->earfcn_params_arr[i].earfcn);
					  continue;
                   }
                   else
				   {
				      /* There is atleast one PLMNID for this earfcn which is not forbidden */
                      HDR_MSG_PROT_1 (MSG_LEGACY_MED, "HDR DLR: There is atleast one PLMNID which is not forbidden for earfcn %d",
                                      lte_nset_update_params->earfcn_params_arr[i].earfcn);
					  break;
                   }
               }
					 
               HDR_MSG_PROT_2 (MSG_LEGACY_MED,"HDR DLR: is_in_fplmn is %d for earfcn %d",is_in_fplmn,lte_nset_update_params->earfcn_params_arr[i].earfcn);
               if(is_in_fplmn)
               {				        
                 keep_earfcn = FALSE;				 	    
               }	
               HDR_MSG_PROT_2 (MSG_LEGACY_MED,"HDR DLR:keep earfcn %d for %d ",keep_earfcn,lte_nset_update_params->earfcn_params_arr[i].earfcn);
               break;
             }/*  if(lte_rrc_acq_db_earfcn.earfcn_list[l].earfcn == lte_nset_update_params->earfcn_params_arr[i].earfcn) */
         }/* for(l = 0;l < lte_rrc_acq_db_earfcn.num_earfcn_count; l++) */

		}/* else lte_nbr_ptr->plmnid_included */

      	}/* if (!hdrovhd_get_plmnids_associated_with_earfcn) */

    	}/* else lte_nbr_ptr->lte_plmnid_list[i].plmn_same_as_prev_channel  */
         ...
         if ( keep_earfcn )
         {
                ...
                lte_band_pref_static ))
           {
              ...
              {
                 keep_earfcn = TRUE;
				 HDR_MSG_PROT_1 (MSG_LEGACY_HIGH, "DLR :keep_earfcn TRUE for %d",lte_nset_update_params->earfcn_params_arr[i].earfcn);
              }
           }
           else
           {
    ...
    if (keep_earfcn)
    {
       ...
       */ 
          ...
          num_eutra_freqs++;
          HDR_MSG_PROT_1 (MSG_LEGACY_HIGH, "Added earfcn %d ",lte_nset_update_params->earfcn_params_arr[i].earfcn);
  		}
		else
		{
          HDR_MSG_PROT_1 (MSG_LEGACY_HIGH, "earfcn %d NOT added",lte_nset_update_params->earfcn_params_arr[i].earfcn);
		}/* else for keep_earfcn */
  
  
        }/*   for ( i = 0; i < lte_nset_update_params->num_eutra_freqs; i++) */

  pruned_lte_nset_params.num_eutra_freqs = num_eutra_freqs;
  HDR_MSG_PROT_1 (MSG_LEGACY_HIGH, "DLR Number of eutra freq %d ",pruned_lte_nset_params.num_eutra_freqs);
  
  for(l=0;l<pruned_lte_nset_params.num_eutra_freqs;l++)
  {
      HDR_MSG_PROT_1 (MSG_LEGACY_HIGH, "DLR Earfcn %d ",pruned_lte_nset_params.earfcn_params_arr[l].earfcn);
  }
...
)
{
  ...
  if ( hdrovhd.ap_state != HDROVHD_AP_UPDATED )
  {
    ...
    if ( !hdralmp_connection_is_open(HDRHAI_OVERHEAD_MESSAGES_PROTOCOL) )
    {
      ...
      {
		  (void) sd_ss_ind_hdr_opr_sys_usable( 
#ifdef GPP2_SUPPORT_ON_SEC_SUB 
                                  hdrmc_get_hdr_sub_id(),
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
                                  SD_SYS_USABLE_NOT_OK, NULL );
      }
      else
      {
		  (void) sd_ss_ind_hdr_opr_sys_usable( 
  ...
  if( continue_curr_hdr_system )
  {
         ...
         !hdrrup_is_band_chan_supported(&cache_entry_ptr->sp_entry.sp_msg.hash_channel) )
    {
	  ...
	  sd_action = sd_ss_ind_hdr_opr_bad_sys(
                                  ...
                                  hdrmc_get_hdr_sub_id(),
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
                                  SD_SS_BAD_SYS_HASH_INV_BAND, NULL );
      ...
      if (!chan_was_changed)
      {
               ...
               hdrovhd.qc_info.access_signature) )
        {
          hdrovhd_ap_info_updated( &ind_to_give );
	  	  ...
	  	  /*-----------------------------------------------------------------------
        Reset cache entry state valid, if hashed channel is different from unpack SP msg and update cache entry.
        and if the current channel is same as the hashed channel while updating cache entry.
         -----------------------------------------------------------------------*/	  

     if((ind_to_give.need_to_give_ind == TRUE) && (ind_to_give.ind_name == HDRIND_OVHD_INFO_UPDATED))
   	 {
   	     if(!SYS_CHAN_CMP_EQUAL(hdrovhd.sp_buffer.hash_channel,((hdrovhd.curr_cache_entry_ptr)->sp_entry).sp_msg.hash_channel))
   	     {
            (hdrovhd.curr_cache_entry_ptr)->cache_entry_state = HDROVHD_ENTRY_VALID;
            (hdrovhd.curr_cache_entry_ptr)->sp_entry.state = HDROVHD_BUF_NOT_IN_USE;
		 HDR_MSG_PROT_2(MSG_LEGACY_HIGH, "Resetting hash channel values %d %d ",
		                                   hdrovhd.sp_buffer.hash_channel.chan_num,
										   ((hdrovhd.curr_cache_entry_ptr)->sp_entry).sp_msg.hash_channel.chan_num);
   	     }
   	 }
...
FUNCTION hdrovhd_update_earfcn_plmnid_info
...
void hdrovhd_update_earfcn_plmnid_info(lte_rrc_acq_db_earfcn_rsp_s *lte_rrc_acq_db_earfcn_rsp_s)
{
    memset(&hdrovhd.lte_rrc_acq_db_earfcn, 0,sizeof(hdrovhd.lte_rrc_acq_db_earfcn));
	
	hdrovhd.lte_rrc_acq_db_earfcn = *lte_rrc_acq_db_earfcn_rsp_s;
	
 	if(hdrovhd.lte_rrc_acq_db_earfcn.num_earfcn_count == 0)
    {
        HDR_MSG_PROT(MSG_LEGACY_HIGH, "LTE ACQ DB returned 0 EARFCN");
    }
}
...
FUNCTION HDROVHD_IS_FPLMN

DESCRIPTION
  This function checks if the PLMNID is forbidden or not. 
...
)
{
  ...
  for ( k = 0; k < hdrovhd.fplmn_list.num_fplmn; k++ )
  {
	if ( sys_plmn_match( plmn_id, hdrovhd.fplmn_list.fplmnid[k] ) )
	{
	   is_in_fplmn = TRUE;
	   break;
	}
...
FUNCTION HDROVHD_GET_PRUNED_EARFCN_FROM_ORNL

DESCRIPTION
  This function retrieves the eligible earfcn from ORNL msg. 
...
void hdrovhd_get_pruned_earfcn_from_ornl
(
  lte_earfcn_t *pruned_earfcn,
  uint8 *earfcn_cnt
)
{    
	hdrsrch_lte_nset_update_params_type *pruned_lte_nset_params;
...
uint8 i=0;
	...
	*earfcn_cnt = 0;
   if(hdrovhd_ornl_is_updated(HDRHAI_OVERHEAD_MESSAGES_PROTOCOL) == FALSE)   
   {	
   	return;
   }   
   /* Get the entry at which the ORNL message need to be extracted */	
   ...
   pruned_lte_nset_params = &ornl_msg_ptr->other_rat[0].other_rat_rec.lte_nbr.lte_nset;

   *earfcn_cnt =  pruned_lte_nset_params->num_eutra_freqs;
   
   for(i = 0; i < pruned_lte_nset_params->num_eutra_freqs; i++)
   {
      pruned_earfcn[i] = pruned_lte_nset_params->earfcn_params_arr[i].earfcn;
   }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *cc_changed = FALSE;
            ...
            ( hdrovhd.sp_buffer.sec_color_code_count != HDROVHD_SCC_UNSPECIFIED ) )
  {
     ...
     */
    for ( i = 0; (i < hdrovhd.sp_buffer.sec_color_code_count) && (i < HDROVHD_MAX_SEC_COLORCODES)
                            && (i < LTE_RRC_MAX_HDR_PREREG_ZONES ); i++ )
    {
...
FUNCTION HDROVHD_CHECK_LTE_FREQ_DEPRIORITIZED

DESCRIPTION
  This function checks if the earfcn is in the LTE deprioritized frequencies list

DEPENDENCIES
  None.

PARAMETERS
  earfcn - earfcn to be checked

RETURN VALUE
  TRUE - if earfcn is present in deprioritized frequencies list
  FALSE - if earfcn is NOT present in deprioritized frequencies list

SIDE EFFECTS
  None.
=============================================================================*/
boolean hdrovhd_check_lte_freq_deprioritized
( 
  lte_earfcn_t earfcn
)
{
  ...
  {
     HDR_MSG_PROT_1( MSG_LEGACY_HIGH, 
                     "freq %d is deprioritized. All freq deprioritized", earfcn );
    return TRUE;
  }
  else
  {
    for ( i = 0; i < hdrovhd.depri_freq.num_depri_freq; i++ )
    {
      if ( hdrovhd.depri_freq.depri_freq[i] == earfcn )
      {
           HDR_MSG_PROT_1( MSG_LEGACY_HIGH, 
                           "freq %d is deprioritized", earfcn );
           return TRUE;
      }
    }
  }
  HDR_MSG_PROT_1( MSG_LEGACY_HIGH, 
                  "freq %d is NOT deprioritized", earfcn );
  return FALSE;
}
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( qc_info_ptr != NULL )
  {
           ...
           hdrovhd.first_splmn_after_acq ) )
    {
      ...
      memset(&hdrovhd.splmn_info, 0, sizeof(sd_splmn_info_list_s));
      sd_ss_ind_hdr_update_splmn_list(
#ifdef GPP2_SUPPORT_ON_SEC_SUB 
                                  hdrmc_get_hdr_sub_id(),
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
                                  &hdrovhd.splmn_info );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  sd_misc_sys_avoid_list(
                                  ...
                                  hdrmc_get_hdr_sub_id(),
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
                                  SYS_SYS_MODE_HDR, avoided_chan_rec, 
