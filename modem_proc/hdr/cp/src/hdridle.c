...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdridle.c#2 $ $DateTime: 2020/01/11 11:07:14 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
01/08/20   vaa     Fixed KW error
06/01/18   svu     Fixed UE not waiting for ORNL msg
10/30/17   vlc     Provide subscription id for CM_HDR_CALL_CONNECTED_F.
...
04/22/15   vko     Mainline feature FEATURE_COEX_RPT_RAT_L3_STATE
...
#ifndef FEATURE_HDR_CC_DECODE_FAIL
...
)
{
  ...
  HDR_MSG_PROT_2( MSG_LEGACY_HIGH, "Send CM rpt %d asubs_id %d",
                                    cmd_ptr->hdr.cmd, cmd_ptr->hdr.asubs_id);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ((cmd_ptr = cm_mc_rpt_get_buf_else_err_fatal()) != NULL)
  {
	...
	HDR_MSG_PROT_1 (MSG_LEGACY_MED,
					"Sent CM_HDR_INCOMING_CALL_F for callid=%d",
					cmd_ptr->incoming_call.call_id);
...
/* ----------------------------------------------------------------------- */
  ...
  hdridle.last_wakeup_reason = ind_data_ptr->wakeup_status.wakeup_reason;
      ...
      HDR_MSG_PROT_1(MSG_LEGACY_HIGH, 
                 "Recvd wake up ind with wakeup reason %d",
                 hdridle.last_wakeup_reason);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  memset(&qp_link,0,sizeof(sys_link_type));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  HDR_MSG_PROT_4( MSG_LEGACY_HIGH, "ORNL sleep bit status %d rcv_end_of_sync_cap_ind %d last_wakeup_reason %d ORNL updated %d", 
  status, 
  hdridle.rcv_end_of_sync_cap_ind, 
  hdridle.last_wakeup_reason,
  hdrovhd_ornl_is_updated(HDRHAI_IDLE_STATE_PROTOCOL) );

  sleep_mask |= (status << HDRIDLE_SLEEP_ENTIRE_CAPSULE_RXED_FOR_ORNL);
      ...
      (HDRIDLE_SLEEP_FULL_CONDITIONS_MET( sleep_mask )))
  {
    ...
    {
      act = sd_ss_ind_hdr_opr_resel_ok(
#ifdef GPP2_SUPPORT_ON_SEC_SUB         
                 hdrmc_get_hdr_sub_id(),
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
                 NULL);
