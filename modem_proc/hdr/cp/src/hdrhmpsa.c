...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdrhmpsa.c#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $
...
void hdrhmpsa_check_ovhd( void )
{
  if ( hdrhmpsa.state == HDRHMPSA_WAITING_FOR_OVERHEAD_MESSAGES )
  {
    ...
    {
        if(hdrrup_is_system_loss_in_progress() == TRUE)
        {
           hdrhmpsa.state = HDRHMPSA_START;
           HDR_MSG_PROT (MSG_LEGACY_HIGH,  
		   	             "HMPSA: Not starting access attempt. System loss will happen now due to avoided channel in aset");
        }
		else
		{
