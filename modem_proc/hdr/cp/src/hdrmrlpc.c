...
$Header: //components/rel/hdr.mpss/6.0/cp/src/hdrmrlpc.c#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $

when       who   what, where, why
--------   ---   ------------------------------------------------------- 
02/04/16   vko   Reduce excessive F3 logging
12/14/15   vko   Fixed KW issues 
06/11/15   kss   Fixed possible integer overflow issues. 
...
#define HDRMRLPC_PROT_PARM_REC_MAX_BYTE_SIZE  31
...
)
{
 ...
 /*- - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - - - -*/
         ...
         attrib_len)
  {
    ...
    /* Check that length is not more than the total attribute length left */
    if ((curr_rsv_qos_rsp.value.length > HDRMRLP_MAX_PROFILE_LENGTH)  ||
        (err) ||
        (curr_rsv_qos_rsp.value.length > 
        ((attrib_len - (attrib_val_offset + HDRMRLPC_RESKK_QOS_ATTRIB_MIN_VAL_SIZE)) / 8 )))
    {
      HDR_MSG_PROT_4(MSG_LEGACY_ERROR,
...
)
{
 ...
 /*- - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - - - -*/
         ...
         <= attrib_len)
  {
    ...
    /* Check that length value is sane */
    if (curr_flow_prot.value.length > HDRMRLPC_PROT_PARM_REC_MAX_BYTE_SIZE)
    {
      HDR_MSG_PROT_2(MSG_LEGACY_ERROR, 
                     "Flow%dFlowProtParmFwd: Length %d not valid",
...
)
{
 ...
 /*- - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - - - -*/
         ...
         <= attrib_len)
  {
    ...
    {
      HDR_MSG_PROT_2(MSG_LEGACY_ERROR, 
                     "Flow%dFlowProtParmRev: Length %d not valid",
                     flow_id, curr_flow_prot.value.length );
      break;
    }
...
)
{
 ...
 /*- - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - - - -*/
         ...
         <= attrib_len)
  {
    ...
    /* Check that length value is sane */
    if (curr_route_prot.value.length > HDRMRLPC_PROT_PARM_REC_MAX_BYTE_SIZE)
    {
      HDR_MSG_PROT_2(MSG_LEGACY_ERROR, 
                     "Flow%dRouteProtParmFwd: Length %d not valid",
...
)
{
 ...
 /*- - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - - - -*/
         ...
         <= attrib_len)
  {
    ...
    {
      HDR_MSG_PROT_2(MSG_LEGACY_ERROR, 
                     "Flow%dRouteProtParmRev: Length %d not valid",
                     flow_id, curr_route_prot.value.length ); 
      break;
    }
