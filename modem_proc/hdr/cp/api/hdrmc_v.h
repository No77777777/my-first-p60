...
$Header: //components/rel/hdr.mpss/6.0/cp/api/hdrmc_v.h#2 $
...
01/29/18   svu     Added macro for AS_ID to slotID conversion.
01/18/18   svu     Fixed restoring session to the corresponding subid
10/30/17   vko     Fix eHRPD not working on 2nd SUB
08/29/17   vko     FR 29153: Support HDR/CDMA on the 2nd sub 
08/03/17   svu     Added API that returns current subid
08/02/16   vko     support for logging UE mode change request from MMODE
05/31/16   vko     FR 35747: Graceful handling of UE mode collision and X2L IRAT
...
/*Mapping of  AS_ID to Slot_ID*/
#define HDRMC_AS_ID_TO_SLOT_ID(as_id)\
        ((as_id == SYS_MODEM_AS_ID_1)? MMGSDI_SLOT_1 : MMGSDI_SLOT_2)   
#define HDRMC_SUBID_BOOTUP "/nv/item_files/modem/hdr/cp/almp/bootup_subid"
/*-------------------------------------------------------------------------*/
...
HDRMC_MM_SD_LTE_MODE_CHANGE_IND = MM_SD_LTE_MODE_CHANGE_IND,

HDRMC_QMI_NAS_PRIORITIZE_LTE_IND = QMI_NAS_PRIORITIZE_LTE_IND
...
typedef enum
{
  ...
  HDR_MMOC_RPT_UE_MODE_SWITCH_CNF
}hdrmc_sent_cmd_type;
...
SIDE EFFECTS
  None
===========================================================================*/
boolean hdrmc_sub_id_change_in_progress(void);
/*===========================================================================
FUNCTION HDRMC_SET_HDR_SUB_ID_FROM_DB

DESCRIPTION
  This function sets the hdr sub-id to a specific vlaue
...
void hdrmc_lte_to_hdr_rtt_timer_expiry_cb(uint32 timer_id);
