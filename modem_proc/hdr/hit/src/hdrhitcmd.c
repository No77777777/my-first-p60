...
$Header: //components/rel/hdr.mpss/6.0/hit/src/hdrhitcmd.c#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $
...
)
{
  ...
  if (hdr_hit_enable && hdrhitcmd_acquired)
  {
    ...
    if ( param_ptr->rl_mode == 0x1 ) /* Rev A specific configuration */
    {
      ...
      rfm_hdr_set_tx_power_adjust( hdrsrchrf_get_rf_dev(HDRSRCHRF_TX_CHAIN),
                                   0 );
    }
