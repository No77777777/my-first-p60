...
$Header: //components/rel/hdr.mpss/6.0/hit/src/hdrhitmsg.c#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $

when         who      what, where, why
----------   ---      -------------------------------------------------------- 
08/20/2015   arm      Atlas RUMI changes, disable div by default.
07/13/2015   wsh      Fixed DIV mode not set properly for NS mode 
...
/*-------------------------------------------------------------------------*/   
    ...
    rfm_hdr_set_nominal_pwr( hdrsrchrf_get_rf_dev(HDRSRCHRF_TX_CHAIN), access_params_ptr->open_loop_adjust );
    (void) rfm_hdr_set_tx_power_adjust( hdrsrchrf_get_rf_dev(HDRSRCHRF_TX_CHAIN), access_params_ptr->probe_initial_adjust );  
    ...
    total_pwr = rfm_hdr_get_tx_total_power(hdrsrchrf_get_rf_dev(HDRSRCHRF_TX_CHAIN),
                                          RFM_HDR_CARRIER_ID__0,
                                          RFM_CDMA_AGC_FORMAT__DBM256) /256;
  
    pilot_pwr = rfm_hdr_get_tx_pilot_power(hdrsrchrf_get_rf_dev(HDRSRCHRF_TX_CHAIN),
                                          RFM_HDR_CARRIER_ID__0,
                                          RFM_CDMA_AGC_FORMAT__DBM256) /256;
...
/* Sanity check; plus make this code backward compatible */
    ...
    for ( i=0; i < rmac_params_ptr->revb_num_carriers; i++ )
    {
       ...
       (void) hdrhitcmd_modulator_cfg(i, TRUE, TRUE, 
        ...
        /* Compute pilot power */  
        total_pwr[i] = rfm_hdr_get_tx_total_power(hdrsrchrf_get_rf_dev(HDRSRCHRF_TX_CHAIN),
                                            (rfm_cdma_carrier_type) i,
                                            RFM_CDMA_AGC_FORMAT__DBM256) /256;
    
        pilot_pwr[i] = rfm_hdr_get_tx_pilot_power(hdrsrchrf_get_rf_dev(HDRSRCHRF_TX_CHAIN),
                                             (rfm_cdma_carrier_type) i,
                                            RFM_CDMA_AGC_FORMAT__DBM256) /256;
   }
