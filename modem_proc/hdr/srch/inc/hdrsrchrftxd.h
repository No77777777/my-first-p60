...
$Header: //components/rel/hdr.mpss/6.0/srch/inc/hdrsrchrftxd.h#1 $
$DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $

when         who     what, where, why
--------     ---     --------------------------------------------------------- 
01/24/18     vke     Changes to resume txd in traffic only after rf tune
07/11/16     vke     Enum changes in dualsim_mode_type based on multirat
07/04/16     wsh     Skip OFS if ASDiv switch is pending
08/05/15     wsh     Added per band antenna imbalance support
...
#define HDRSRCH_EFS_NV_STXD_ANT_IMBAL_CONFIG "/nv/item_files/modem/hdr/srch/hdr_stxd_ant_imbal_config"
...
typedef enum
{
  HDRSRCHRFTXD_SS = HDRMULTIRAT_SINGLE_STANDBY
  ,HDRSRCHRFTXD_DSDS = HDRMULTIRAT_DUAL_SIM_DUAL_STANDBY
  ,HDRSRCHRFTXD_DSDA = HDRMULTIRAT_DUAL_SIM_DUAL_ACTIVE
#ifdef FEATURE_HDR_DR_DSDS
  ,HDRSRCHRFTXD_DRDSDS = HDRMULTIRAT_DR_DUAL_SIM_DUAL_STANDBY
#endif
...
FUNCTION HDRSRCHRFTXD_SWITCH_IS_PENDING

DESCRIPTION 
  This function checks if there is a pending ASDiv switch
 
DEPENDENCIES
  None

RETURN VALUE
  True - if there is a switch pending
  False - if no switch pending

SIDE EFFECTS
  None

=========================================================================*/
boolean hdrsrchrftxd_switch_is_pending( void );

/*=========================================================================

FUNCTION HDRSRCHRFTXD_RESUME_STM_TRAFFIC

DESCRIPTION 
  This function posts command to resume the traffic state operation
...
=========================================================================*/
extern void hdrsrchrftxd_resume_stm_traffic( void );
