...
$Header: //components/rel/hdr.mpss/6.0/srch/inc/hdrsrchsm.h#2 $ $DateTime: 2020/04/27 09:08:35 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
04/22/15   par     Changed memcpy() to memscpy() 
11/24/15   wsh     Added logic to handle DIV grant after TX in QTA
06/16/15   wsh     Added delay for DIV release/request 
...
#define HDRSRCH_STM_ALLOC_SIZE_PUT_INTERNAL_CMD( sm, sm_group, cmd, size, d_ptr )  \
        {                                                                    \
          ...
          memscpy(_payload, size, (d_ptr), size);                            \
          stm_put_internal_cmd_payload( (void *)_payload );                  \
        }
#else
#define HDRSRCH_STM_ALLOC_SIZE_PUT_INTERNAL_CMD( sm, sm_group, cmd, size, d_ptr )  \
...
typedef enum
{
  ...
  HDRSRCHRFDIV_WAIT_DONE_CMD,
    /* needed to add delay between release and request */

  HDRSRCHRFDIV_REDO_WARMUP_CMD,
    /* needed to re-do DIV enable */
