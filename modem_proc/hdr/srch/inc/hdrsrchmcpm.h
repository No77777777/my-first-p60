...
$Header: //components/rel/hdr.mpss/6.0/srch/inc/hdrsrchmcpm.h#3 $ $DateTime: 2020/02/10 05:22:27 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     --------------------------------------------------------- 
02/06/20   vaa     FR54762: HDR Concurrency Mgr changes
12/04/19   vke     Changes to support clk plan V2.0 FR changes
...
typedef struct
{
  uint32 scenario;
  uint64 timeToApply;
} hdrsrchmcpm_scenario_struct_type;

/* Macros used for bit operations */
#define HDRSRCHMCPM_SCENARIO_START   (1)
#define HDRSRCHMCPM_SCENARIO_STOP    (0)
#define HDRSRCHMCPM_NO_TRIGGER       (0)

#define HDRSRCHMCPM_SCENARIO_MASK_SHIFT         (1)
#define HDRSRCHMCPM_SCENARIO_TRIG_MASK_SHIFT    (8)

#define hdrsrchmcpm_mcvs_config_release( ) do{MCPM_MCVSConfig_Release(MCPM_DO_TECH);}while(0)
  /* Macro for config release */
...
void hdrsrchmcpm_cmgr_trans_req
(
  uint32 scenario, 
  boolean trigger
);
...
void hdrsrchmcpm_cmgr_steady_req
(
  uint32 scenario
);
