...
$Header: //components/rel/hdr.mpss/6.0/srch/api/hdrsrchtypes.h#2 $
$DateTime: 2020/04/14 08:32:30 $ $Author: pwbldsvc $
...
/* D2L redirection MinOtherRATMeasurementReportInterval enum types */
typedef enum
{
  HDRSRCH_ORAT_REDIRECT_NOT_SUPPORTED,
  HDRSRCH_ORAT_REDIRECT_SUPPORTED_NO_MIN_RPT_INT,
  HDRSRCH_ORAT_REDIRECT_SUPPORTED_WITH_MIN_RPT_INT
}
hdrsrch_orat_redirect_enum_type;

/* LTE redirection call back function, to notify DO CP of
   LTE redirection reporting information */
typedef
void (*hdrsrch_lte_redirect_cb_type)
(
  hdrsrchtclte_rpt_struct_type                 *redirect_rpt_ptr
    /* Ptr to redirect report content (containing LTE EARFCNs and
       physical cell IDs of winning cells) */

);

/* LTE neighbor list struct, used in DO->LTE redirection during DO connected state
   EARFCN:  Evolved Absolute Radio Frequency Channel Number (LTE frequency number) */
typedef struct
{
  msgr_hdr_struct_type              msg_hdr;      
  ...
  uint8                             earfcn_cnt;
    /* Number of LTE frequencies in this record */
    
  lte_earfcn_t                      earfcn_arr [ HDRSRCH_LTE_FREQ_ARR_SIZE ];
    /* Arrray of Evolved Absolute Radio Frequency Channel numbers or LTE freq numbers */

  uint8                             max_rpt_cnt;
    /* If AutonomousReportEnabled is set to '1', the AN shall set
       this field to (MaxReportCount + 1) numbers of autonomous
       OtherRATMeasurementReport message AT can send;
       otherwise the access network shall omit this field.
    */

  uint8                             serving_thresh_trig;
    /* Serving threshold trigger point that allows AT to send
  ...
  uint8                             threshxq_trig;
    /* Minimum required Squal threshold used on EARFCN for trigger AT
       to send an OtherRATMeasurementReport message.
       If ThreshXQIncluded is set to ‘1’, the access network shall set this field in
       the range 0 to 31; otherwise, the access network shall omit this field.
       The actual threshold value in dB used to compare against the measured
       reference signal received quality (RSRQ) is equal to ThreshXQ as
       specified in [16].
    */

    uint8                           threshx_trig;
     /* Minimum required Srxlev threshold used on EARFCN for trigger AT
        to send an OtherRATMeasurementReport message.
        If ThreshXIncluded is set to ‘1’, the access network shall set this field in
        the range 0 to 31; otherwise, the access network shall omit this field.
        The actual threshold value in dB used to compare against the measured
        reference signal received power (RSRP) is equal to ThreshX × 2 as
        specified in[16]. */
  ...
  hdrsrch_lte_redirect_cb_type      redirect_cb;
    /* Call back to request upper layer to send OtherRatMeasurementReport */
  uint16                           tbackoff_earfcn_arr [ HDRSRCH_LTE_FREQ_ARR_SIZE ]; 
   /*Backoff Duration per Earfcn*/
  boolean                          is_fast_lte_search;
   /*Indicates whether we are in fast LTE procedure mode*/
  boolean                          tbackoff_reset_required [HDRSRCH_LTE_FREQ_ARR_SIZE];
   /*Indicates whether Tbackoff reset is required for the Particular EARFCN*/
}
hdrsrch_lte_nset_update_redir_params_type;
