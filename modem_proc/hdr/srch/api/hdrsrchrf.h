...
$Header: //components/rel/hdr.mpss/6.0/srch/api/hdrsrchrf.h#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $
...
05/05/2017   rmv     Mainline FEATURE_HDR_QTA_THRU_TRM 
...
06/26/2015   vke     removed hdrsrchrf_band_grant_cb()
...
04/23/2015   vke     Added TRM unification changes
04/20/2015   vke     Map HDRSRCHRF_HP_BROADCAST to TRM_ACCESS_URGENT
04/01/2015   mbs     Updated prototype to support RF interface change
...
typedef enum
{
  ...
  HDRSRCHRF_TX_ANY = TRM_TX_ANY,
    /* Any Tx RF chain */

  HDRSRCHRF_TX_BEST = TRM_TX_BEST,
    /* Best Tx RF chain */

  HDRSRCHRF_INVALID_RESOURCE = 255
    /* Invalid resource, used for internal checking */
}
...
typedef enum
{
  ...
  HDRSRCHRF_ACCESS_URGENT = TRM_ACCESS,
    /* The remapping is done so that we may use the reason
       to change the behavior within the same priority bucket
       if we need to later.
     */
#else
...
/*----------------------------------------------------------------------------
  The TRM PBR RF grant status (based on trm_grant_event_enum_t)
  ----------------------------------------------------------------------------*/
#ifdef FEATURE_HDR_PBR_RANDOMIZATION
typedef enum
{
  HDRSRCHRF_PBR_RF_DENIED = TRM_DENIED
    /* RF chain denied for page demod */
  
  ,HDRSRCHRF_PBR_RF_GRANTED = TRM_GRANTED_CHAIN0
    /* RF chain granted for page demod */
}hdrsrchrf_pbr_grant_event_enum_t;
...
typedef struct
{
  ...
  trm_tuneaway_enum_type         ta_info;
    /* TA status from TRM (QTA or LTA) */   
#endif /* FEATURE_HDR_QTA */

  boolean                        ext_unlock;
    /* Track external unlock request from TRM */
}
...
typedef enum
{
  HDRSRCHRF_TRM_START_STATE = TRM_START_STATE,
  HDRSRCHRF_TRM_END_STATE = TRM_END_STATE
}hdrsrchrf_trm_client_state_enum_type;

typedef enum
{
  HDRSRCHRF_APS_REQUESTED = TRM_APS_REQUESTED,
  HDRSRCHRF_APS_SUPPORTED = TRM_APS_SUPPORTED
}hdrsrchrf_aps_enum_type;
