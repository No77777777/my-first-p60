...
$Header: //components/rel/hdr.mpss/6.0/common/src/hdrutil.c#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
08/01/18   svu     Fix bootup session restoration
10/30/17   vko     Fix HDR read MEID/ESN from default SUB only
10/28/17   vko     FR 29153: Support HDR/CDMA on the 2nd sub 
10/30/17   vko     Fix HDR read MEID/ESN from default SUB only
10/28/17   vko     FR 29153: Support HDR/CDMA on the 2nd sub 
05/27/16   vko     Added support for HDR on 2nd sub
05/04/16   vko     Reduce F3 logging
06/23/15   vko     Add wrapper to compress2 API, to pass alloc/free functions
                           to be used from modem memheap
...
LOCAL struct
{
  ...
  /* Current subid on which HDR is active */
...
static mcfg_fs_sub_id_e_type hdrefs_map_as_id_to_mcfg_id(
...
void hdrutil_powerup_init( void )
{
  ...
  (void) dsm_init_pool(DSM_HDR_MSG_ITEM_POOL,
   ...
   hdrutil.sub_id_is_valid = FALSE;
   hdrutil.sub_id = SYS_MODEM_AS_ID_1;
...
)
{
  ...
  hdrutil.nv_cmd_ext.nvcmd = &hdrutil.nv_cmd;
  hdrutil.nv_cmd_ext.context = (uint16)hdrmc_get_hdr_sub_id();

  nv_cmd_ext(&hdrutil.nv_cmd_ext);    
} /* hdrutil_nv_write() */
...
{
  static nv_cmd_ext_type cmd_ext;
  ...
  cmd->done_q_ptr = NULL;                   
  cmd_ext.nvcmd = cmd;
  cmd_ext.context = (uint16)hdrmc_get_hdr_sub_id();
...
)
{
  ...
  static nv_cmd_ext_type hdr_nv_cmd_buf_ext;
  ...
  hdr_nv_cmd_buf_ext.nvcmd = &hdr_nv_cmd_buf;
  hdr_nv_cmd_buf_ext.context = (uint16)hdrmc_get_hdr_sub_id();

  if(item_code != NV_MEID_ME_I && item_code != NV_ESN_ME_I)
  {
     hdr_nv_cmd_buf_ext.context = (uint16)hdrmc_get_hdr_sub_id();
  }
  else
  {
    hdr_nv_cmd_buf_ext.context = SYS_MODEM_AS_ID_1;
    HDR_MSG_PROT_1 (MSG_LEGACY_HIGH, 
                    "hdrutil_read_nv_item : using default SUB SYS_MODEM_AS_ID_1 for reading NV %d", 
                     item_code );
  }    
  /* Clear signal, issue the command, wait for the response */

  (void) rex_clr_sigs(HDRMC_TASK_PTR, HDRMC_NV_CMD_SIG);
  nv_cmd_ext(&hdr_nv_cmd_buf_ext);
...
/*==========================================================================
  ...
  FUNCTION HDRUTIL_FS_ERRNO

  DESCRIPTION
    This function is a wrapper to mcfg_fs_errno() which returns the
    error number from prev. hdrutil_fs_xxx() call
  ...
  RETURN VALUE
    Error number from previous hdrutil_fs_xxx() call.

  SIDE EFFECTS
    None
===========================================================================*/
int hdrutil_fs_errno( void )
{
  return mcfg_fs_errno( HDR_FS_TYPE );
}

/*===========================================================================

FUNCTION HDRUTIL_FS_GET

DESCRIPTION
  This function is a wrapper to mcfg_fs_read(). It is similiar to efs_get()
  in usage. This is the prefer way (vs hdrutil_fs_open + hdrutil_fs_read).
...
PARAMETERS
  ...
  data_ptr - Buffer to read the file to
  length   - Buffer size pointed by data_ptr
...
===========================================================================*/
mcfg_fs_status_e_type hdrutil_fs_get
(
  const char *path, 
  void *data_ptr, 
  uint32 length
...
{
  return mcfg_fs_read( path, 
                       data_ptr,
                       length,
                       HDR_FS_TYPE,
                       hdrefs_map_as_id_to_mcfg_id(hdrmc_get_hdr_sub_id()) );
}

/*===========================================================================

FUNCTION HDRUTIL_FS_PUT

DESCRIPTION
  This function is a wrapper to mcfg_fs_wrte(). It is similiar to efs_put()
  in usage. This is the prefer way (vs hdrutil_fs_open + hdrutil_fs_write).
...
===========================================================================*/
mcfg_fs_status_e_type hdrutil_fs_put
(
  const char *path, 
  void *data_ptr, 
  uint32 length
...
FUNCTION HDRUTIL_FS_OPEN

DESCRIPTION
  This function is a wrapper to mcfg_fopen(), which opens the given
  file and returns the file descriptor on success
...
PARAMETERS
  path     - The path of the file to be read
  oflag    - mode flag such as read_only, create, etc
 
RETURN VALUE
  File descriptor to be used for fs_close/fs_read/write, or -1 on error
...
FUNCTION HDRUTIL_FS_CLOSE

DESCRIPTION
  This function is a wrapper to mcfg_fclose(), which closes the file
...
PARAMETERS
  filedes - The file descriptor obtained from hdrutil_fs_open
...
===========================================================================*/
int hdrutil_fs_close
(
   int filedes
)
{
  return mcfg_fclose( filedes,
                      HDR_FS_TYPE );
}

/*===========================================================================

FUNCTION HDRUTIL_FS_WRITE

DESCRIPTION
  This function is a wrapper to mcfg_fwrite(), which writes up to nbyte of
  data pointed by buf
...
FUNCTION HDRUTIL_FS_READ

DESCRIPTION
  This function is a wrapper to mcfg_fread(), which reads up to nbytes of
  data pointed by buf
...
PARAMETERS
  filedes - The file descriptor obtained from hdrutil_fs_open
  buf     - data buffer to read to
  nbyte   - size of buf

RETURN VALUE
  num of bytes read or -1 if error
...
FUNCTION HDRUTIL_FS_STAT

DESCRIPTION
  This function is a wrapper to mcfg_fs_stat(). 
...
===========================================================================*/
mcfg_fs_status_e_type hdrutil_fs_stat 
(
  const char *path, 
  struct fs_stat *buf
)
{
  return mcfg_fs_stat( path, 
                       buf,
                       HDR_FS_TYPE,
                       hdrefs_map_as_id_to_mcfg_id(hdrmc_get_hdr_sub_id()) );
}
...
FUNCTION HDRUTIL_FS_RENAME

DESCRIPTION
  This function is a wrapper to mcfg_fs_rename(). 
...
PARAMETERS
  oldpath - Path of the file to be renamed
  newpath - Path of the file to be renamed to
...
===========================================================================*/
int hdrutil_fs_rename 
(
  const char *oldpath, 
  const char *newpath 
)
{
  return mcfg_fs_rename( oldpath, 
                         newpath, 
                         HDR_FS_TYPE, 
                         hdrefs_map_as_id_to_mcfg_id(hdrmc_get_hdr_sub_id()) );
}
...
FUNCTION HDRUTIL_FS_DELETE

DESCRIPTION
  This function is a wrapper to mcfg_fs_delete(). 
...
===========================================================================*/
mcfg_fs_status_e_type hdrutil_fs_delete
(
  const char *path
)
{
  return mcfg_fs_delete( path, 
                         HDR_FS_TYPE, 
                         hdrefs_map_as_id_to_mcfg_id(hdrmc_get_hdr_sub_id()) );
}
...
FUNCTION HDRUTIL_FS_MKDIR

DESCRIPTION
  This function is a wrapper to mcfg_fs_mkdir(). 
...
===========================================================================*/
int hdrutil_fs_mkdir 
(
  const char *path, 
  int mode
)
{
  return mcfg_mkdir( path, 
                     mode, 
                     HDR_FS_TYPE );
}
...
===========================================================================*/
void* hdr_modem_mem_alloc(void* handle,unsigned items,unsigned size  )
{
  uint32 total_size = items * size;
  handle = NULL;  
  return modem_mem_alloc(total_size, MODEM_MEM_CLIENT_HDR_CRIT);

}

/*===========================================================================
  
FUNCTION HDR_MODEM_MEM_FREE
...
===========================================================================*/
void hdr_modem_mem_free(void* handle,void* addr  )
{
  handle = NULL;
  modem_mem_free(addr, MODEM_MEM_CLIENT_HDR_CRIT);
}


/*===========================================================================
  
FUNCTION HDRUTIL_COMPRESS

DESCRIPTION
  This function is a wrapper to compress2 API. It passes hdr_modem_mem_alloc/hdr_modem_mem_free,
...
PARAMETERS
  dest - memory pointing to compressed memory
  destLen - destLen is the total size of the destination buffer, which must be at least 0.1%
            larger than sourceLen plus 12 bytes.U pon exit, destLen is the actual size of 
            the compressed buffer.
  source - memory pointing to uncompressed memory
  sourceLen -  byte length of the source buffer
  level - memory level
...
)    
{
    ...
    int err;
    HDR_MSG_PROT( MSG_LEGACY_HIGH, 
                  "compress using hdr custom function");
    ...
    stream.zalloc = (alloc_func)hdr_modem_mem_alloc;
    stream.zfree = (free_func)hdr_modem_mem_free;
...
FUNCTION hdrefs_map_as_id_to_mcfg_id
...
/*===========================================================================
FUNCTION HDRUTIL_SET_HDR_SUB_ID

DESCRIPTION
  This function set the current sub-id on which HDR is activated
...
SIDE EFFECTS
  None
===========================================================================*/
void hdrutil_set_hdr_sub_id(sys_modem_as_id_e_type hdr_subid)
{
  hdrutil.sub_id_is_valid = TRUE;
  hdrutil.sub_id = hdr_subid;
}

/*===========================================================================
FUNCTION HDRUTIL_GET_HDR_SUB_ID
...
SIDE EFFECTS
  None
===========================================================================*/
sys_modem_as_id_e_type hdrutil_get_hdr_sub_id(void)
{
#ifdef GPP2_SUPPORT_ON_SEC_SUB 
  if(hdrutil.sub_id_is_valid == TRUE)
  {
     return hdrutil.sub_id;
  }
  else
  {
    ...
    return SYS_MODEM_AS_ID_1;
#endif /* GPP2_SUPPORT_ON_SEC_SUB */
} /* hdrutil_get_hdr_sub_id() */
/*===========================================================================

FUNCTION HDRUTIL_FS_OPEN_DEFAULT

DESCRIPTION
  This function is a wrapper to mcfg_fopen(), which opens the given
  file and returns the file descriptor on success on SUB0
...
)
{
  return mcfg_fopen( path, 
                     ...
                     HDR_FS_PERM,
                     HDR_FS_TYPE,
                     hdrefs_map_as_id_to_mcfg_id(SYS_MODEM_AS_ID_1) );
}
