...
$Header: //components/rel/hdr.mpss/6.0/common/src/hdrcp_api.c#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $
 
when       who     what, where, why
--------   ---     ----------------------------------------------------------
12/05/17   pga     API to get the EVDO release version
11/12/15   vko     API updated to check if redir is given to LTE and is in progress
10/13/15   vko     API to check if reselection is given to LTE and is in progress
...
FUNCTION HDRCP_ALMP_RESELECTION_IN_PROGRESS
		
DESCRIPTION
  This is a wrapper function to report not only reselection, but both D2L redirection/reselection in progress. 
  The name is not updated, as that would require change in iratman, caller of this API.
...
RETURN VALUE
  TRUE - if reselection (or) redirection is in progress
  FALSE - if reselection (and) redirection is NOT in progress
		
SIDE EFFECTS
  None
===========================================================================*/
boolean hdrcp_almp_reselection_in_progress( void )
{
	
#if defined( FEATURE_HDR_TO_LTE ) || defined( FEATURE_HDR_TO_LTE_REDIRECTION )
  if ( hdralmps_get_state() == HDRALMPS_SUSPEND )
  {
    HDR_MSG_PROT (MSG_LEGACY_HIGH, "Querying D2L IRAT : In progress");
    return TRUE;
  }
  else
  {
    HDR_MSG_PROT (MSG_LEGACY_HIGH, "Querying D2L IRAT : NOT in progress");
    return FALSE;
  }
#endif /* defined( FEATURE_HDR_TO_LTE ) || defined( FEATURE_HDR_TO_LTE_REDIRECTION ) */
	
  HDR_MSG_PROT (MSG_LEGACY_HIGH, "D2L IRAT : NOT supported");
  return FALSE;

}
/*===========================================================================
		 
FUNCTION HDRCP_MC_GET_EVDO_RELEASE_VERSION
		
DESCRIPTION
  This is an external function to get current EVDO release version
		
DEPENDENCIES
  None
		
PARAMETERS
  as_id   - Subscriber ID on which HDR is active
...
uint32 hdrcp_mc_get_evdo_release_version
( 
  sys_modem_as_id_e_type     asid 
)
{
  sys_modem_as_id_e_type cur_asid = hdrmc_get_hdr_sub_id();

  uint32 evdo_rel_ver = (uint32) EVDO_REL_A;
  /*-----------------------------------------------------------------------*/

  if( asid != cur_asid )
  {
    HDR_MSG_PROT_2 (MSG_LEGACY_HIGH, "Queried with wrong AS_ID : %d Current AS_ID : %d",asid,cur_asid);	 
  }
  else
  {
    HDR_MSG_PROT_1 (MSG_LEGACY_HIGH, "Queried with correct AS_ID %d",asid);	 
  }
  

  if(hdrscp_phone_is_set_to_revA_mode() == TRUE)
  {
     evdo_rel_ver = (uint32) EVDO_REL_A;
     HDR_MSG_PROT_1 (MSG_LEGACY_HIGH, "Current release version Rev A : %d",evdo_rel_ver);	 
  }
  else if(hdrscp_phone_is_set_to_rev0_mode() == TRUE)
  {
	  evdo_rel_ver = (uint32) EVDO_REL_0;
      HDR_MSG_PROT_1 (MSG_LEGACY_HIGH, "Current release version Rev 0 : %d",evdo_rel_ver);	 
  }
  else
  {
	  HDR_MSG_PROT_1 (MSG_LEGACY_HIGH, "Unsupported release version Not Rev A/0. Returning default : %d",evdo_rel_ver);	  
  }
