...
$Header: //components/rel/hdr.mpss/6.0/common/src/hdrmultirat.c#5 $ $DateTime: 2021/05/18 05:54:37 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ----------------------------------------------------------
05/18/21   vaa     Fix for compilation errors in MSIM disable build flavor
02/06/20   vaa     FR54762: HDR Concurrency Mgr changes
01/08/20   vaa     Update QTA START state to FW in QTA CFG msg
12/04/19   vaa     Changes to support clk plan V2.0 FR changes
04/04/18   svu     F3 reduction
02/19/18   svu     Fixed KW issues
08/28/17   vko     FR 29153: Support HDR/CDMA on the 2nd sub 
08/24/17   rmv     Mainline FEATURE_HDR_QTA_THRU_TRM
01/11/16   vke     Check whether the primary chain is owned before 
                   confirming the valid freq id to cxm
12/14/16   wsh     Fixed TxNumCarriers being overwritten with junk values
12/14/15   wsh     Fixed ASDiv switch due to DSDX ChanID mismatch and 
                   NumCarrier being 0
11/21/15   arm     Resolved compilation error
11/18/15   rmv     Added an F3 for debug after QTA start msg to FW is sent 
11/18/15   arm     Make sure we have valid chan id before rf warmup
07/30/15   vke     Fixed klockwork issues
06/04/15   wsh     Fixed FreqID mismatch 
05/04/15   mbs     Supported QTA using TRM API
04/09/15   rmv     Move checks is_mdsp_active, check_dsp_clk close to sending 
                   message to FW
03/16/15   wsh     Reduced stack usage to avoid overflow 
01/22/15   arm     Added Band Avoidance feature control NV. 
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  HDR_MSG_PROT_1( MSG_LEGACY_LOW, "HDR DSDA: priority %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  HDR_MSG_PROT( MSG_LEGACY_LOW, "HDR DSDA: received freq id list from MCS" );

  ASSERT( freqid_list != NULL );
     ...
     )
  {
...
#ifdef FEATURE_HDR_FW_CHAN_ID
    chan_info.rf_device = hdrsrchrf_get_rf_dev( HDRSRCHRF_PRI_CHAIN );
    ...
    else
    {
      ...
      if ( hdrsrchrf_get_rf_dev( HDRSRCHRF_DIV_CHAIN )!= RFM_INVALID_DEVICE )
      {
        chan_info.rf_device = hdrsrchrf_get_rf_dev( HDRSRCHRF_DIV_CHAIN );
        chan_info.techid = CXM_TECH_HDR;
        chan_id = cxm_get_channel_id( chan_info );
    ...
    for ( cnt = 0; cnt < freq_data->num_link_info_sets; cnt++ )
    {
      ...
      {
        HDR_MSG_PROT_1( MSG_LEGACY_MED, " HDR DSDA: freq id %d is DIV type!",
    ...
    if ( hdrmdsp_is_mdsp_active()  )
    {
      ...
      if ( !hdrsrchrf_has_lock(HDRSRCHRF_PRI_CHAIN) )
      {
        ...
        HDR_MSG_PROT_2( MSG_LEGACY_HIGH, 
                        "HDR DSDA: Chain not owned! rxNumcarriers: %d txNumcarriers: %d",
                        dsda_frequency_cfg_msg.rxNumcarriers, 
                        dsda_frequency_cfg_msg.txNumcarriers );
      } /* If chain is not owned */

      hdrmultirat.rx_reg = ( dsda_frequency_cfg_msg.rxNumcarriers != 0 );
      hdrmultirat.tx_reg = ( dsda_frequency_cfg_msg.txNumcarriers != 0 );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  HDR_MSG_PROT( MSG_LEGACY_LOW, "HDR DSDA: Sending freq id list" );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( channel_cnt > HDRSRCH_MAX_NUM_SUBASETS)
  {
...
#endif 
       hdrmultirat.nv_config.band_avoidance 
      )
    {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  HDR_MSG_PROT_1( MSG_LEGACY_MED, "HDR DSDA: Sending freq id list ,div channel count :%d"
                         ,hdrmultirat.coex_info.div.channel_cnt );
  hdrmultirat_send_freq_config_msg ( );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
     ...
     )
  {
    ...
    HDR_MSG_PROT_1( MSG_LEGACY_LOW, "HDR DSDA:Sending priority %d",priority );
...
FUNCTION HDRMULTIRAT_CONFIRM_VALID_FREQ_ID

DESCRIPTION
  Check if freq id is valid at this point, if not then get the valid chan id
  and send it.
DEPENDENCIES
  None
...
void hdrmultirat_confirm_valid_freq_id ( void )
{
   hdrfw_dsdx_frequency_msg_t  dsda_frequency_cfg_msg;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  HDR_MSG_PROT( MSG_LEGACY_MED, 
                "HDR DSDA: Sending dsda frequency Ids to FW:" );
  ...
  if ( hdrmdsp_is_mdsp_active( ) && 
       ( hdrsrchrf_get_rf_dev( HDRSRCHRF_PRI_CHAIN ) != 
         RFM_INVALID_DEVICE ) )
  {
    /* Send hdrfw_dsdx_frequency_msg only when Primary chain is valid */
    memset( &dsda_frequency_cfg_msg,
            0,
            sizeof( hdrfw_dsdx_frequency_msg_t ) );
      /* Clear the message */
    ...
    /* Copy Rx/Tx Freq ID. Note this may be out-of-sync, but will not
       cause crash */
    for ( cnt = 0; cnt < HDRMULTIRAT_MAX_NUM_CARRIERS; cnt++ )
    {
    ...
    HDR_MSG_PROT_6( 
        MSG_LEGACY_MED, 
        "HDR DSDA: Forced update, RxFreq:%d/%d/%d TxFreq:%d/%d/%d", 
        dsda_frequency_cfg_msg.rxfrequencyId[0],
        dsda_frequency_cfg_msg.rxfrequencyId[1],
        dsda_frequency_cfg_msg.rxfrequencyId[2],
        dsda_frequency_cfg_msg.txfrequencyId[0],
        dsda_frequency_cfg_msg.txfrequencyId[1],
        dsda_frequency_cfg_msg.txfrequencyId[2] );
    ...
    chan_id = cxm_get_channel_id( chan_info );
    dsda_frequency_cfg_msg.rxChannelId[0] = chan_id.rx_channel_id;
    dsda_frequency_cfg_msg.rxNumcarriers = 1;

      if ( hdrsrchrf_get_rf_dev( HDRSRCHRF_TX_CHAIN ) != RFM_INVALID_DEVICE )
      {
      dsda_frequency_cfg_msg.txNumcarriers = 1;
      dsda_frequency_cfg_msg.txChannelId = chan_id.tx_channel_id;
    }
    else
    {
    ...
    HDR_MSG_PROT_5( MSG_LEGACY_HIGH, 
                    "HDR DSDA: Rx cnt:%d chan id %d Div chain id %d Tx cnt:%d chan id %d", 
                    dsda_frequency_cfg_msg.rxNumcarriers,
                      dsda_frequency_cfg_msg.rxChannelId[0],
                      dsda_frequency_cfg_msg.rxChannelId[1],
                    dsda_frequency_cfg_msg.txNumcarriers,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  HDR_MSG_PROT_2( MSG_LEGACY_MED, "HDR DSDA: Sending dsda with rx priority:%d tx priority:%d"
                                       ,rx_priority,tx_priority);
...
)
{
  ...
  trm_client_enum_t           ta_client;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  HDR_MSG_PROT_1( MSG_LEGACY_HIGH, "HDR DSDx: send QTA cfg msg %d", state );
  ...
  ta_client = hdrsrchtc_get_ta_winning_client();

#ifdef FEATURE_HDR_CLK_BOOST_QTA_GAP
    if ( state == HDRMULTIRAT_QTA_START )
    {
       if( ta_client == TRM_UMTS ||
           ta_client == TRM_UMTS2 ||
           ta_client == TRM_LTE ||
           ta_client == TRM_LTE_SUB2)
       {
         /* Boost clk for all nQTA or when WCDMA or LTE is gap tech */
          HDR_MSG_PROT_3( MSG_LEGACY_HIGH, "HDR DSDx: Boosting clock for Do2X ta_client:%d, Q6 %d, VPE %d",
                                            ta_client, 
                                            HDRMULTIRAT_DEFAULT_QTA_Q6_CLK_KHZ, 
                                            HDRMULTIRAT_DEFAULT_QTA_VPE_CLK_KHZ);
          hdrsrchmcpm_cmgr_trans_req(CMGR_HDR_NQTA , TRUE);

            /* HDR and WCDMA or LTE will use different tile and hence need boost */
            hdrsrchmcpm_mcvs_config_modem( HDR_NQTA, 
                                           HDRSRCHMCPM_SCENARIO_START, 
                                           0, 
                                           HDRMULTIRAT_DEFAULT_QTA_Q6_CLK_KHZ, 
                                           0, 
                                           0 );
            /* nQTA scenario start with Q6 and VPE boost */
       }
    }
    else if ( state == HDRMULTIRAT_QTA_END )
    {
      HDR_MSG_PROT( MSG_LEGACY_HIGH, "HDR DSDx: Releasing any prior boosted clock");
     /* Release tech request for bumped clk_bus rate. */
      hdrsrchmcpm_cmgr_trans_req(CMGR_HDR_NQTA , FALSE);

      hdrsrchmcpm_mcvs_config_modem( HDR_NQTA, 
                                     HDRSRCHMCPM_SCENARIO_STOP,
                                     0, 0, 0, 0 );
        /* nQTA scenario stop */

        hdrsrchmcpm_mcvs_config_release();
    }
#endif /* FEATURE_HDR_CLK_BOOST_QTA_GAP */
  
  hdrmultirat_send_msg( &qta_cfg_msg.hdr, sizeof( hdrfw_qta_cfg_msg_t ) );

  HDR_MSG_PROT( MSG_LEGACY_MED, "HDR DSDx: sent QTA cfg msg");
...
#ifdef FEATURE_HDR_BAND_AVOIDANCE
/*===========================================================================
FUNCTION HDRMULTIRAT_IS_BAND_AVOIDANCE_ENABLED

DESCRIPTION
  This function returns if band avoidance enabled currently
...
RETURN VALUE
  True - if band avoidance is enabled
  False - if and avoidance is disabled

SIDE EFFECTS
  None.
===========================================================================*/
boolean hdrmultirat_is_band_avoidance_enabled( void )
{
  if ( hdrmultirat.nv_config.band_avoidance == 1)
    return TRUE;
  else
    return FALSE;
} /* hdrmultirat_is_dsda_enabled( ) */
...
/*===========================================================================
FUNCTION HDRMULTIRAT_CREATE_EFS_ITEM_CONF_FILE
...
DESCRIPTION
  ...
  to QCN files. For HDR MULTIRAT, use "/nv/item_files/conf/hdrmultirat_config_info.conf"
  for conf_file_path.
...
NOTE
  ...
  HDRMULTIRAT EFS ID is 73633.
===========================================================================*/
void hdrmultirat_create_efs_item_conf_file( void )
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/ 
  ...
  config_fd = efs_open( HDRMULTIRAT_EFS_NV_CONF_FILE_PATH, O_RDWR|O_APPEND );
  if ( config_fd >= 0  )
  {
  ...
  else 
  {
    if ( ENOENT == efs_errno ) /* Conf file does not exist */
    {
      ...
      config_fd = efs_open( HDRMULTIRAT_EFS_NV_CONF_FILE_PATH, 
    ...
    {
      HDR_MSG_PROT_1( MSG_LEGACY_ERROR, "Error opening config file errno %d", efs_errno );
      return;
    }

    result = efs_write( config_fd, HDRMULTIRAT_EFS_NV_BAND_AVOID_ENABLE,
                      strlen( HDRMULTIRAT_EFS_NV_BAND_AVOID_ENABLE ) );
    result += efs_write( config_fd, new_line, 1 );
    if ( result != ( strlen( HDRMULTIRAT_EFS_NV_BAND_AVOID_ENABLE )+1 ) )
    {
      HDR_MSG_PROT_1( MSG_LEGACY_ERROR, "Error writing to config file errno %d", efs_errno );
      result = efs_close( config_fd );
      return;
    }
...
/*===========================================================================
FUNCTION HDRMULTIRAT_READ_EFS_NV

DESCRIPTION
  Read value of EFS NV items for multirat module.
DEPENDENCIES
  None
...
NOTE
  ...
  HDRMULTIRAT EFS ID is 73633.
===========================================================================*/
void hdrmultirat_read_efs_nv( void )
{

  if ( hdrutil_fs_get( HDRMULTIRAT_EFS_NV_BAND_AVOID_ENABLE,
                &hdrmultirat.nv_config,
                sizeof( hdrmultirat.nv_config ) ) == MCFG_FS_STATUS_OK )
  {
    HDR_MSG_PROT_1( MSG_LEGACY_HIGH, 
             "HDR BA:: hdr_band_avoid_enable =%d",
             hdrmultirat.nv_config.band_avoidance);
  }
  else
  {
    HDR_MSG_PROT( MSG_LEGACY_HIGH, 
            "HDR BA: hdrmultirat.nv_config.band_avoidance is not available in NV, set it: 0");
    hdrmultirat.nv_config.band_avoidance = 0;  
      /* Disable band avoidance if the NV is not available */ 
  }
...
/*===========================================================================
FUNCTION HDRMULTIRAT_IS_FREQ_REG_COMPLETE

DESCRIPTION
  This function returns if carrier registration is pending for Rx or RxTx
  (There is no use case for Tx only)
 
DEPENDENCIES
  None

PARAMETERS
  RxTx - Whether we need both RxTx to be register or only Rx

RETURN VALUE
  True - if registered for given config

SIDE EFFECTS
  None

===========================================================================*/
boolean hdrmultirat_is_freq_reg_complete
( 
  boolean rxtx 
)
{
  boolean reg = hdrmultirat.rx_reg;

  if ( rxtx )
  {
    reg &= hdrmultirat.tx_reg;
  }

  HDR_MSG_SRCH_3( MSG_LEGACY_MED, 
                  "FreqUpdate rx:%d tx:%d reg:%d", 
                  hdrmultirat.rx_reg,
                  hdrmultirat.tx_reg,
                  reg );
