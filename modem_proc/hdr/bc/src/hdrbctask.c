...
$Header: //components/rel/hdr.mpss/6.0/bc/src/hdrbctask.c#1 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  */
  if ( rcevt_wait_name(HDRDEC_RCEVT_READY) == RCEVT_NULL )
  {
    HDR_MSG_SRCH( MSG_LEGACY_ERROR,"Unexpected Run Control Event Notification err" );
  }
  if ( rcevt_signal_name(HDRBC_RCEVT_READY) == RCEVT_NULL )
  {
    HDR_MSG_SRCH( MSG_LEGACY_ERROR,"Unexpected Run Control Event Notification err" );
  }
