...
$Header: //components/rel/hdr.mpss/6.0/mac/src/hdrrmac4drv.c#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
04/28/16   vke     Changes to cancel QTA when PA ON after RLSI collides
...
)
{
  ...
  qw_inc(rlsi_frame_end, (hdrrmac4drv_common->silence_duration+1));
    /* End of silence is silence duration + 1 frame for PA ON scheduling */
  ...
  HDR_MSG_PROT_4( MSG_LEGACY_HIGH, "Silence End @ 0x%x %x QTA Start %x Silence duration %d",
                  qw_hi(rlsi_frame_end), qw_lo(rlsi_frame_end),
                  qw_lo(frame_start), hdrrmac4drv_common->silence_duration );
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  rfm_hdr_notify_data_rate( hdrsrchrf_get_rf_dev(HDRSRCHRF_TX_CHAIN), 
