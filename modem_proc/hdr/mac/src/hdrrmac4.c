...
$Header: //components/rel/hdr.mpss/6.0/mac/src/hdrrmac4.c#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------- 
11/11/15   rmv     Switch to T2P boost state INACTIVE on RMAC suspend cancel 
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  else if ( ( lolat_subpackets_available >= 
              HDRRMAC4DRV_PA_4_SUBPACKETS_AVAILABLE ) &&
       ( hdrrmac4_var_ptr->t2p_boost.state == HDRRMACCMN_T2P_BOOST_PRE_TA_ACTIVE ) )
  {
    /* Tuneaway must have been cancelled, switch to T2P boost Inactive */
    hdrrmac4_var_ptr->t2p_boost.state = HDRRMACCMN_T2P_BOOST_INACTIVE;

    HDR_MSG_PROT( MSG_LEGACY_HIGH, "T2P boost Inactive" );
  }
#endif /* FEATURE_HDR_RMAC_TUNEAWAY_OPTIMIZATION_T2P_BOOST */
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  hdrrmac_process_cmd_on_tune_back();
    /* Check if RMAC is pending on shutdown or deactivate cmd if so process it */
#endif
