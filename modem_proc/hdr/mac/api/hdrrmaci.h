...
$Header: //components/rel/hdr.mpss/6.0/mac/api/hdrrmaci.h#1 $ $DateTime: 2019/06/19 22:12:26 $ $Author: pwbldsvc $
   
when       who     what, where, why
--------   ---     ----------------------------------------------------------
10/29/15   vko     Added debug code for HDRMC stuck waiting for 
                   RMAC deactivate
10/12/15   vke     Added enum for cmds which pend on TA
...
/* The enum defines cmds pending on tune-away */
typedef enum
{
  HDRRMAC_NO_CMD_PENDING          = 0x0,
  HDRRMAC_SHUTDOWN_CMD_PENDING    = 0x1,
  HDRRMAC_DEACTIVATE_CMD_PENDING  = 0x2
} hdrrmac_cmds_pend_on_ta_enum_type;
...
{
  /* Cmnds pending on TA */
  hdrrmac_cmds_pend_on_ta_enum_type pend_on_ta;
