...
$Header: //components/rel/rfdevice_qfe2340.mpss/1.32/api/qfe2340v3p0_asm.h#1 $

when       who    what, where, why
--------   ---   ---------------------------------------------------------------
01/11/16   dbc    Added Votes for ULCA\DLCA. 
01/05/16   sml    Skip ASM wakeup when TX is enabled (except during cal) due to
                  shared register bit between ASM and PA
...
#include "rfdevice_physical_device.h"
  ...
  protected:
    ...
    boolean asm_tx_enabled;
	
    uint32 lte_dlca_rfm_vote;

    uint32 lte_ulca_rfm_vote;
  };
